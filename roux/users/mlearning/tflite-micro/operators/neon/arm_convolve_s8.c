			...
			for (int i_out_y = 0; i_out_y < output_y; i_out_y++) {
				...
				for (int32_t i_ker_x = 0; i_ker_x < kernel_x; i_ker_x++) {
					...
					for (int32_t i_ker_y = 0; i_ker_y < kernel_y; i_ker_y++) {
						k_y += dilation_y;
						arm_memcpy_s8(im2col_buf, input_data + (k_y * input_x + k_x) * input_ch,
									  input_ch);
						im2col_buf += input_ch;
					}
				}
				...
				arm_q7_to_q15_with_offset(im2col_buf - rhs_cols, im2col_buf_start_s16, rhs_cols,
				...
				if (lhs_rows & 2) {
					out = arm_nn_mat_mult_kernel_s8_s16(filter_data, buffer_a, output_ch,
														output_shift, output_mult, out_offset,
														out_activation_min, out_activation_max,
														rhs_cols, aligned_rhs_cols, bias_data, out);
		...
		if (lhs_rows != 0) {
			...
			for (i = 0; i < output_ch; i++) {
				...
				while (col_count) {
					...
					ker_a += 8;
					ip_as_col += 8;
					res_s32 = vmlal_s16(res_s32, vget_low_s16(input_s16), vget_low_s16(filter_s16));
					res_s32 =
						vmlal_s16(res_s32, vget_high_s16(input_s16), vget_high_s16(filter_s16));
					col_count--;
				}
