...
int8_t *arm_nn_mat_mult_kernel_s8_s16(const int8_t *input_a, const int16_t *input_b,
									  const uint16_t output_ch, const int32_t *out_shift,
									  const int32_t *out_mult, const int32_t out_offset,
									  ...
									  const int32_t *const output_bias, int8_t *out_0)
{
	...
	while (row_count) {
		...
		const int8_t *ip_a[4] = {ip_a0, ip_a0 + num_col_a, ip_a0 + 2 * num_col_a,
								 ip_a0 + 3 * num_col_a};
		...
		while (col_count) {
			...
			for (int i = 0; i < 4; i++) {
				...
				res[i * 2] =
					vmlal_s16(res[i * 2], vget_low_s16(filter_s16[i]), vget_low_s16(input_s16[0]));
				res[i * 2 + 1] = vmlal_s16(res[i * 2 + 1], vget_low_s16(filter_s16[i]),
										   vget_low_s16(input_s16[1]));
				res[i * 2] = vmlal_s16(res[i * 2], vget_high_s16(filter_s16[i]),
									   vget_high_s16(input_s16[0]));
				res[i * 2 + 1] = vmlal_s16(res[i * 2 + 1], vget_high_s16(filter_s16[i]),
	...
	if (output_ch & 0x1) {
		...
		while (col_count) {
			...
			ip_b0 += 8;
			ip_b1 += 8;
			res[0] = vmlal_s16(res[0], vget_low_s16(filter_s16), vget_low_s16(input_0_s16));
			res[1] = vmlal_s16(res[1], vget_low_s16(filter_s16), vget_low_s16(input_1_s16));
			res[0] = vmlal_s16(res[0], vget_high_s16(filter_s16), vget_high_s16(input_0_s16));
			res[1] = vmlal_s16(res[1], vget_high_s16(filter_s16), vget_high_s16(input_1_s16));
			col_count--;
		}
