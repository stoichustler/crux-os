 ...
 * be compatible with the ROUX license:
 *
 ****************************************************************************/
...
static void dodim(void)
{
	...
	case DIMSTRID:
		...
		case 5:
			dimvar = dimension(name, 5, (int)dims[0], (int)dims[1], (int)dims[2], (int)dims[3],
							   (int)dims[4]);
			break;
		}
		break;
...
static int donext(void)
{
	...
	if (nfors) {
		...
		*lv.dval += g_forstack[nfors - 1].step;
		if ((g_forstack[nfors - 1].step < 0 && *lv.dval < g_forstack[nfors - 1].toval) ||
			(g_forstack[nfors - 1].step > 0 && *lv.dval > g_forstack[nfors - 1].toval)) {
			nfors--;
			return 0;
		} else {
...
static FAR void *getdimvar(FAR struct mb_dimvar_s *dv, ...)
{
	...
	if (dv->type == FLTID) {
		...
		case 3:
			answer =
				&dv->dval[index[2] * (dv->dim[0] * dv->dim[1]) + index[1] * dv->dim[0] + index[0]];
			break;

		case 4:
			answer =
				&dv->dval[index[3] * (dv->dim[0] + dv->dim[1] + dv->dim[2]) +
						  index[2] * (dv->dim[0] * dv->dim[1]) + index[1] * dv->dim[0] + index[0]];
			break;

		case 5:
			answer =
				&dv->dval[index[4] * (dv->dim[0] + dv->dim[1] + dv->dim[2] + dv->dim[3]) +
		...
		case 3:
			answer =
				&dv->str[index[2] * (dv->dim[0] * dv->dim[1]) + index[1] * dv->dim[0] + index[0]];
			break;
		...
		case 5:
			answer =
				&dv->str[index[4] * (dv->dim[0] + dv->dim[1] + dv->dim[2] + dv->dim[3]) +
						 index[3] * (dv->dim[0] + dv->dim[1] + dv->dim[2]) +
						 index[2] * (dv->dim[0] + dv->dim[1]) + index[1] * dv->dim[0] + index[0]];
			break;
		}
...
static FAR struct mb_variable_s *addfloat(FAR const char *id)
{
	...
	vars = realloc(g_variables, (g_nvariables + 1) * sizeof(struct mb_variable_s));
	if (vars) {
		g_variables = vars;
		strlcpy(g_variables[g_nvariables].id, id, sizeof(g_variables[g_nvariables].id));
...
static FAR struct mb_dimvar_s *adddimvar(FAR const char *id)
{
	...
	vars = realloc(g_dimvariables, (g_ndimvariables + 1) * sizeof(struct mb_dimvar_s));
	if (vars) {
		g_dimvariables = vars;
		strlcpy(g_dimvariables[g_ndimvariables].id, id, sizeof(g_dimvariables[g_ndimvariables].id));
...
static int isstring(int tokenid)
{
	if (tokenid == STRID || tokenid == QUOTE || tokenid == DIMSTRID || tokenid == CHRSTRING ||
		tokenid == STRSTRING || tokenid == LEFTSTRING || tokenid == RIGHTSTRING ||
