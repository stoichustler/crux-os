...
static int check_and_mount_romfs(void)
{
	...
	desc.nsectors =
		NSECTORS(romfs_cpython_modules_img_len);	 /* The number of sectors in the ROM disk */
	...
	ret = mount(MOUNT_DEVNAME, CONFIG_CPYTHON_ROMFS_MOUNTPOINT, "romfs", MS_RDONLY, NULL);
	if (ret < 0) {
