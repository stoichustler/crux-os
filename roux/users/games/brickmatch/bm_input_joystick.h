...
int dev_input_init(FAR struct input_state_s *dev)
{
	...
	ret = ioctl(dev->fd_joy, DJOYIOC_SUPPORTED, (unsigned long)((uintptr_t)&tmp));
	if (ret < 0) {
	...
	ret = ioctl(dev->fd_joy, DJOYIOC_REGISTER, (unsigned long)((uintptr_t)&notify));
	if (ret < 0) {
