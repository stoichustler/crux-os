...
#else
static const char g_default_fbdev[] = CONFIG_GAMES_BRICKMATCH_LED_MATRIX_PATH;
#endif
...
int prev_board[ROW][COL] = {0};

/* Colors used in the game plus Black */

static const uint16_t pallete[NCOLORS + 1] = {
	RGB16_BLACK, RGB16_BLUE, RGB16_GREEN, RGB16_RED, RGB16_CYAN, RGB16_WHITE, RGB16_GOLD,
};
...
static void draw_rect(FAR struct screen_state_s *state, FAR struct fb_area_s *area, int color)
{
	FAR uint16_t *dest;
	FAR uint8_t *row;
	int x;
...
void update_screen(FAR struct screen_state_s *state, FAR struct fb_area_s *area)
{
	int ret;

#ifdef CONFIG_FB_UPDATE
	ret = ioctl(state->fd_fb, FBIO_UPDATE, (unsigned long)((uintptr_t)area));
	if (ret < 0) {
		int errcode = errno;
...
void draw_board(FAR struct screen_state_s *state, FAR struct fb_area_s *area,
				...
				FAR struct game_screen_s *screen)
{
	...
	if (result != 4 * N_LEDS) {
		fprintf(stderr, "ws2812_main: write failed: %d  %d\n", result, errno);
	}
}
...
void move_board(int dir)
{
	...
	for (row = row_ini; row != row_lim; row += row_dir) {
		for (col = col_ini; col != col_lim; col += col_dir) {
			...
			if ((row != 0 && col != 0) && (row != 0 && col != BOARDX_SIZE + 1) &&
	...
	for (row = 1; row < BOARDY_SIZE; row++) {
		for (col = 1; col < BOARDX_SIZE; col++) {
			if (board[row][col] != 0) {
				if (fabs(board[row][col]) == fabs(board[row][col + 1])) {
					...
					if (fabs(board[row][col]) == fabs(board[row + 1][col]) &&
						fabs(board[row][col + 1]) == fabs(board[row + 1][col + 1])) {
						found = 1;
						board[row][col] = board[row][col] > 0 ? -board[row][col] : board[row][col];
						board[row][col + 1] =
							board[row][col + 1] > 0 ? -board[row][col + 1] : board[row][col + 1];
						board[row + 1][col] =
							board[row + 1][col] > 0 ? -board[row + 1][col] : board[row + 1][col];
					...
					if (fabs(board[row][col + 1]) == fabs(board[row + 1][col + 1])) {
						found = 1;
						board[row][col] = board[row][col] > 0 ? -board[row][col] : board[row][col];
						board[row][col + 1] =
							board[row][col + 1] > 0 ? -board[row][col + 1] : board[row][col + 1];
			...
			if (board[row + 1][col] != 0) {
				...
				if (fabs(board[row + 1][col]) == fabs(board[row + 1][col + 1])) {
					if (fabs(board[row + 1][col + 1]) == fabs(board[row][col + 1])) {
						found = 1;
						board[row][col + 1] =
							board[row][col + 1] > 0 ? -board[row][col + 1] : board[row][col + 1];
						board[row + 1][col] =
	...
	for (row = 1; row < BOARDY_SIZE + 1; row++) {
		...
		if (h >= 2) {
			for (int i = hpos; i <= hpos + h; i++) {
				board[row][i] = board[row][i] > 0 ? -board[row][i] : board[row][i];
				found = 1;
			}
		}
	...
	for (col = 1; col < BOARDX_SIZE + 1; col++) {
		...
		if (v >= 2) {
			for (int i = vpos; i <= vpos + v; i++) {
				board[i][col] = board[i][col] > 0 ? -board[i][col] : board[i][col];
				found = 1;
			}
		}
...
int main(int argc, FAR char *argv[])
{
	...
	ret = ioctl(state.fd_fb, FBIOGET_VIDEOINFO, (unsigned long)((uintptr_t)&state.vinfo));
	if (ret < 0) {
		int errcode = errno;
	...
	screen.xres = state.vinfo.xres > state.vinfo.yres ? state.vinfo.yres : state.vinfo.xres;
	screen.yres = state.vinfo.yres > state.vinfo.xres ? state.vinfo.xres : state.vinfo.yres;
	...
	ret = ioctl(state.fd_fb, FBIOGET_PLANEINFO, (unsigned long)((uintptr_t)&state.pinfo));
	if (ret < 0) {
		int errcode = errno;
	...
	state.fbmem = mmap(NULL, state.pinfo.fblen, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FILE,
					   state.fd_fb, 0);
