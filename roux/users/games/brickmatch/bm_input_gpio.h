...
int dev_input_init(FAR struct input_state_s *dev)
{
	...
	if (fd_list.fd_up < 0) {
		fprintf(stderr, "ERROR: Failed to open %s: %d\n", CONFIG_GAMES_BRICKMATCH_UP_KEY_PATH,
				errno);
		return -ENODEV;
	}
	...
	if (fd_list.fd_down < 0) {
		fprintf(stderr, "ERROR: Failed to open %s: %d\n", CONFIG_GAMES_BRICKMATCH_DOWN_KEY_PATH,
				errno);
		return -ENODEV;
	}
	...
	if (fd_list.fd_down < 0) {
		fprintf(stderr, "ERROR: Failed to open %s: %d\n", CONFIG_GAMES_BRICKMATCH_LEFT_KEY_PATH,
				errno);
		return -ENODEV;
	}
...
int dev_read_input(FAR struct input_state_s *dev)
{
	...
	ret = ioctl(fd->fd_right, GPIOC_READ, (unsigned long)((uintptr_t)&invalue));
	if (ret < 0) {
		int errcode = errno;
		fprintf(stderr, "ERROR: Failed to read value from %s: %d\n",
