...
static const char g_board_path[] = CONFIG_GAMES_SNAKE_LED_MATRIX_PATH;

struct snake_item snake = {0};

struct game_item food = {0};
...
static const uint32_t palette[] = {
	RGB24_BLACK, RGB24_BLUE, RGB24_CYAN, RGB24_RED, RGB24_WHITE, RGB24_GREENYELLOW,
};

/* Second block of score value to print (.e.g 1x) */

const uint64_t second_score_block[] = {0x0007050505070000, 0x0004040404040000, 0x0007010704070000,
									   0x0007040704070000, 0x0004040705050000, 0x0007040701070000,
									   0x0007050701070000};

/* First block of score value to print (.e.g x1) */

const uint64_t first_score_block[] = {0x0070505050700000, 0x0040404040400000, 0x0070107040700000,
									  0x0070407040700000, 0x0040407050500000, 0x0070407010700000,
									  0x0070507010700000, 0x0040404040700000, 0x0070507050700000,
									  0x0070407050700000};
...
static void print_score(int score, int fd)
{
	...
	uint64_t val = second_score_block[second_idx] | first_score_block[first_idx];
...
static bool check_food_snake_collision(void)
{
	...
	for (i = 0; i < snake.tail_len; i++) {
		if (snake.tail[i].pos_x == food.pos_x && snake.tail[i].pos_y == food.pos_y) {
			return true;
		}
	}
...
int main(int argc, FAR char *argv[])
{
	...
	int board_x;
	int board_y;
	struct snake_item prev_snake = {0};
	...
	while (!game_over) {
		...
		if (game_over) {
			...
			for (i = 0; i < 3; i++) {
				...
				board[prev_snake.tail[0].pos_x][prev_snake.tail[0].pos_y] = SNAKE_HEAD;
