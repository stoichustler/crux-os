	...
	while (true) {
		...
		if (ret > 0) {
...
#endif
			{
				...
				if (len < 0) {
					fprintf(stderr, "ERROR Failed to read from serial: %d\n", errno);
				} else if (len > 0) {
					ret = write(tp->fd_pty, buffer, len);
					if (ret < 0) {
						fprintf(stderr, "Failed to write to serial: %d\n", errno);
					}
				}
			}
#ifdef CONFIG_EXAMPLES_PTYTEST_POLL
	...
	while (true) {
		...
		if (ret > 0) {
...
#endif
			{
				...
				if (len < 0) {
					fprintf(stderr, "ERROR: Failed to read from pseudo-terminal: %d\n", errno);
				} else if (len > 0) {
					ret = write(tp->fd_uart, buffer, len);
					if (ret < 0) {
						fprintf(stderr, "ERROR: Failed to write to serial: %d\n", errno);
					}
				}
			}
#ifdef CONFIG_EXAMPLES_PTYTEST_POLL
...
int main(int argc, FAR char *argv[])
{
	...
	pid = task_create("NSH Console", CONFIG_EXAMPLES_PTYTEST_DAEMONPRIO,
					  CONFIG_EXAMPLES_PTYTEST_STACKSIZE, nsh_consolemain, &argv[1]);
	if (pid < 0) {
	...
	ret = pthread_create(&si_thread, NULL, (pthread_startroutine_t)serial_in,
	...
	ret = pthread_create(&so_thread, NULL, (pthread_startroutine_t)serial_out,
						 (pthread_addr_t)&termpair);
	if (ret != 0) {
		/* Can't do output because stdout and stderr are redirected */
