...
int main(int argc, FAR char *argv[])
{
	...
	if (res != TEEC_SUCCESS) {
		EMSG("TEEC_Opensession failed with code 0x%08x origin 0x%08x", res, err_origin);
		goto exit_with_ctx;
	}
	...
	if (err_origin != TEEC_ORIGIN_TRUSTED_APP || res != TEEC_ERROR_SHORT_BUFFER) {
		EMSG("TEEC_InvokeCommand failed: code 0x%08x origin 0x%08x", res, err_origin);
	...
	if (!op.params[0].tmpref.buffer) {
		EMSG("Failed to allocate %zu bytes of memory to share with TEE", op.params[0].tmpref.size);
		goto exit_with_session;
	}
