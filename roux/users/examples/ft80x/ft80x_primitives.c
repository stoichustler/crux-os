...
int ft80x_prim_bitmaps(int fd, FAR struct ft80x_dlbuffer_s *buffer)
{
	...
	cmds[4] = FT80X_BITMAP_SIZE(FT80X_FILTER_NEAREST, FT80X_WRAP_BORDER, FT80X_WRAP_BORDER,
								bmhdr->width, bmhdr->height);
	...
	cmds[16] =
		FT80X_VERTEX2F(-10 * 16, -10 * 16); /* For -ve coordinates use vertex2f instruction */
	cmds[17] = FT80X_END();
...
int ft80x_prim_points(int fd, FAR struct ft80x_dlbuffer_s *buffer)
{
	...
	cmds[5] = FT80X_VERTEX2F((FT80X_DISPLAY_WIDTH / 5) * 16, (FT80X_DISPLAY_HEIGHT / 2) * 16);

	cmds[6] = FT80X_COLOR_RGB(0, 128, 0);
	cmds[7] = FT80X_POINT_SIZE(15 * 16);
	cmds[8] = FT80X_VERTEX2F(((FT80X_DISPLAY_WIDTH * 2) / 5) * 16, (FT80X_DISPLAY_HEIGHT / 2) * 16);

	cmds[9] = FT80X_COLOR_RGB(0, 0, 128);
	cmds[10] = FT80X_POINT_SIZE(25 * 16);
	cmds[11] =
		FT80X_VERTEX2F(((FT80X_DISPLAY_WIDTH * 3) / 5) * 16, (FT80X_DISPLAY_HEIGHT / 2) * 16);

	cmds[12] = FT80X_COLOR_RGB(128, 128, 0);
	cmds[13] = FT80X_POINT_SIZE(35 * 16);
	cmds[14] =
		FT80X_VERTEX2F(((FT80X_DISPLAY_WIDTH * 4) / 5) * 16, (FT80X_DISPLAY_HEIGHT / 2) * 16);
...
int ft80x_prim_lines(int fd, FAR struct ft80x_dlbuffer_s *buffer)
{
	...
	cmds[4] =
		FT80X_VERTEX2F((FT80X_DISPLAY_WIDTH / 4) * 16, ((FT80X_DISPLAY_HEIGHT - height) / 2) * 16);
	cmds[5] =
		FT80X_VERTEX2F((FT80X_DISPLAY_WIDTH / 4) * 16, ((FT80X_DISPLAY_HEIGHT + height) / 2) * 16);
	cmds[6] = FT80X_COLOR_RGB(0, 128, 0);
...
int ft80x_prim_linestrip(int fd, FAR struct ft80x_dlbuffer_s *buffer)
{
	...
	cmds[5] =
		FT80X_VERTEX2F(((FT80X_DISPLAY_WIDTH * 2) / 3) * 16, (FT80X_DISPLAY_HEIGHT * 2 / 3) * 16);
	cmds[6] = FT80X_VERTEX2F((FT80X_DISPLAY_WIDTH - 80) * 16, (FT80X_DISPLAY_HEIGHT - 20) * 16);
...
int ft80x_prim_stencil(int fd, FAR struct ft80x_dlbuffer_s *buffer)
{
	...
	for (i = 100; i > 0; i--) {
		...
		for (j = 0; j <= ((dispr - displ) / gridsize); j++) {
			cmds.b.vertex2f1.cmd = FT80X_VERTEX2F((displ + j * gridsize) * 16, dispa * 16);
			cmds.b.vertex2f2.cmd = FT80X_VERTEX2F((displ + j * gridsize) * 16, dispb * 16);
		...
		for (j = 0; j <= ((dispb - dispa) / gridsize); j++) {
			cmds.b.vertex2f1.cmd = FT80X_VERTEX2F(displ * 16, (dispa + j * gridsize) * 16);
			cmds.b.vertex2f2.cmd = FT80X_VERTEX2F(dispr * 16, (dispa + j * gridsize) * 16);
		...
		for (j = 1; j <= STENCIL_NUM_LINES; j++) {
			...
			asmooth = (int32_t)smoothlerp((float)((float)asize / (2 * (float)rball)), 0,
										  2 * (float)rball);
