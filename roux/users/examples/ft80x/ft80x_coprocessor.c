...
int ft80x_coproc_clock(int fd, FAR struct ft80x_dlbuffer_s *buffer)
{
	...
	cmds.e.vertex2f1.cmd =
		FT80X_VERTEX2F((xoffset - radius + 10) * 16, (yoffset - radius + 10) * 16);
	cmds.e.vertex2f2.cmd =
		FT80X_VERTEX2F((xoffset + radius - 10) * 16, (yoffset + radius - 10) * 16);
	...
	cmds.e.vertex2f3.cmd =
		FT80X_VERTEX2F((xoffset - radius + 12) * 16, (yoffset - radius + 12) * 16);
	cmds.e.vertex2f4.cmd =
		FT80X_VERTEX2F((xoffset + radius - 12) * 16, (yoffset + radius - 12) * 16);
	cmds.e.end2.cmd = FT80X_END();
	cmds.e.colormask2.cmd = FT80X_COLOR_MASK(1, 1, 1, 1);
	cmds.e.blend.cmd = FT80X_BLEND_FUNC(FT80X_BLEND_DST_ALPHA, FT80X_BLEND_ONE_MINUS_DST_ALPHA);
	...
	cmds.f.bitmaplayout.cmd = FT80X_BITMAP_LAYOUT(bmhdr->format, bmhdr->stride, bmhdr->height);
	cmds.f.bitmapsize.cmd = FT80X_BITMAP_SIZE(FT80X_FILTER_BILINEAR, FT80X_WRAP_BORDER,
											  FT80X_WRAP_BORDER, 2 * radius, 2 * radius);
	cmds.f.vertex2f.cmd = FT80X_VERTEX2F((xoffset - radius) * 16, (yoffset - radius) * 16);
	cmds.f.end.cmd = FT80X_END();
	cmds.f.blend.cmd = FT80X_BLEND_FUNC(FT80X_BLEND_SRC_ALPHA, FT80X_BLEND_ONE_MINUS_SRC_ALPHA);
...
int ft80x_coproc_gauge(int fd, FAR struct ft80x_dlbuffer_s *buffer)
{
	...
	cmds.f.blend.cmd = FT80X_BLEND_FUNC(FT80X_BLEND_DST_ALPHA, FT80X_BLEND_ONE_MINUS_DST_ALPHA);
	...
	cmds.g.bitmaplayout.cmd = FT80X_BITMAP_LAYOUT(bmhdr->format, bmhdr->stride, bmhdr->height);
	cmds.g.bitmapsize.cmd = FT80X_BITMAP_SIZE(FT80X_FILTER_BILINEAR, FT80X_WRAP_BORDER,
											  FT80X_WRAP_BORDER, 2 * radius, 2 * radius);
	cmds.g.vertex2f.cmd = FT80X_VERTEX2F((xoffset - radius) * 16, (yoffset - radius) * 16);
	cmds.g.end.cmd = FT80X_END();
	cmds.g.blend.cmd = FT80X_BLEND_FUNC(FT80X_BLEND_SRC_ALPHA, FT80X_BLEND_ONE_MINUS_SRC_ALPHA);
...
int ft80x_coproc_calibrate(int fd, FAR struct ft80x_dlbuffer_s *buffer)
{
	...
	ft80x_info("Transform A-F: {%08" PRIx32 ", %08" PRIx32 ", %08" PRIx32 ", %08" PRIx32
			   ", %08" PRIx32 ", %08" PRIx32 "}\n",
...
int ft80x_coproc_screensaver(int fd, FAR struct ft80x_dlbuffer_s *buffer)
{
	...
	cmds.b.bitmaplayout.cmd = FT80X_BITMAP_LAYOUT(bmhdr->format, bmhdr->stride, bmhdr->height);
	cmds.b.bitmapsize.cmd =
		FT80X_BITMAP_SIZE(FT80X_FILTER_BILINEAR, FT80X_WRAP_BORDER, FT80X_WRAP_BORDER,
						  3 * bmhdr->width, 3 * bmhdr->height);
