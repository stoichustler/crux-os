...
int main(int argc, FAR char *argv[])
{
	...
	struct pollfd pfds[] = {{.fd = baro_fd, .events = POLLIN},
	...
	while (seconds > 0) {
		...
		for (int i = 0; i < NB_LOWERHALFS; i++) {
			if (pfds[i].revents & POLLIN) {
				ret = read(pfds[i].fd, sensor_data[i].data_struct, sensor_data[i].data_size);
	...
	printf("Temperature [C] = %f\n"
		   ...
		   baro_data.temperature, baro_data.pressure, humi_data.humidity, gas_data.gas_resistance);
