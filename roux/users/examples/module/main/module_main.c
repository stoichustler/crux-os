...
int main(int argc, FAR char *argv[])
{
...
#endif
#if defined(CONFIG_EXAMPLES_MODULE_BUILTINFS) && defined(CONFIG_EXAMPLES_MODULE_ROMFS)
	struct boardioc_romdisk_s desc;
#endif
#if defined(CONFIG_EXAMPLES_MODULE_FSMOUNT) && defined(CONFIG_EXAMPLES_MODULE_FSREMOVEABLE)
	struct stat buf;
#endif
	FAR void *handle;
	char buffer[128];
	...
	ret = mount(CONFIG_EXAMPLES_MODULE_DEVPATH, MOUNTPT, CONFIG_EXAMPLES_MODULE_FSTYPE, MS_RDONLY,
				CONFIG_EXAMPLES_MODULE_FSDATA);
	if (ret < 0) {
		printf("ERROR: mount(%s, %s, %s) failed: %d\n", CONFIG_EXAMPLES_MODULE_DEVPATH,
			   CONFIG_EXAMPLES_MODULE_FSTYPE, MOUNTPT, errno);
	}
