...
int main(int argc, FAR char *argv[])
{
	...
	if (txbuffer == NULL) {
		fprintf(stderr, "ERROR: failed to allocate TX I/O buffer of size %ju\n",
				(uintmax_t)info.blocksize);
		close(fd);
		return 1;
	}
	...
	if (rxbuffer == NULL) {
		fprintf(stderr, "ERROR: failed to allocate IRX /O buffer of size %ju\n",
				(uintmax_t)info.blocksize);
		free(txbuffer);
		close(fd);
		return 1;
			...
			for (i = 0; i < info.blocksize && nerrors <= 100; i++) {
				if (txbuffer[i] != rxbuffer[i]) {
					fprintf(stderr,
							...
							(uintmax_t)blockno, (uintmax_t)i, rxbuffer[i], txbuffer[i]);
					nerrors++;
				}
			}
