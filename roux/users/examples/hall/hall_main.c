...
int main(int argc, FAR char *argv[])
{
	...
	for (nloops = 0; !g_hallexample.nloops || nloops < g_hallexample.nloops; nloops++) {
		...
		ret = ioctl(fd, SNIOC_GET_POSITION, (unsigned long)((uintptr_t)&position));
		if (ret < 0) {
