 ...
 * has it's own global memory. In roux in flat build that single object is
 * shared between *all* tasks. You could probably get away with this if you
 * use embedlog only in one task, but it's usually not the case. So in roux
...
static void el_print_options(void)
{
	...
	el_oprint(OELF, "or enable only nanosecond resolution "
					"(not that it makes much sense on roux)");
	el_ooption(&g_el, EL_TS, EL_TS_LONG);
	...
	/* roux runs on rather slow cpus, so millisecond precision is enough */
