...
static inline FAR struct nxtext_glyph_s *nxtext_allocglyph(FAR struct nxtext_state_s *st)
{
...
static FAR struct nxtext_glyph_s *nxtext_findglyph(FAR struct nxtext_state_s *st, uint8_t ch)
{
	int i;
...
nxtext_renderglyph(FAR struct nxtext_state_s *st, FAR const struct nx_fontbitmap_s *fbm, uint8_t ch)
{
	FAR struct nxtext_glyph_s *glyph = NULL;
...
static FAR struct nxtext_glyph_s *nxtext_getglyph(NXHANDLE hfont, FAR struct nxtext_state_s *st,
												  uint8_t ch)
{
...
static FAR const struct nxtext_bitmap_s *nxtext_addchar(NXHANDLE hfont,
														FAR struct nxtext_state_s *st, uint8_t ch)
{
					 ...
					 FAR const struct nxtext_bitmap_s *bm)
{
	...
	if (!nxgl_nullrect(&intersection)) {
		...
		src = glyph->bitmap;
		ret = nx_bitmap((NXWINDOW)hwnd, &intersection, &src, &bm->pos, (unsigned int)glyph->stride);
		if (ret < 0) {
