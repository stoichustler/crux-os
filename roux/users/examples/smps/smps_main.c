...
static void parse_args(FAR struct args_s *args, int argc, FAR char **argv)
{
	...
	for (index = 1; index < argc;) {
		...
		case 'v': {
			...
			if (f_value <= 0.0 || (f_value * 1000 > CONFIG_EXAMPLES_SMPS_OUT_VOLTAGE_LIMIT)) {
		...
		case 'i': {
			...
			if (f_value <= 0.0 || (f_value * 1000 > CONFIG_EXAMPLES_SMPS_OUT_CURRENT_LIMIT)) {
		...
		case 'p': {
			...
			if (f_value <= 0.0 || (f_value * 1000 > CONFIG_EXAMPLES_SMPS_OUT_POWER_LIMIT)) {
...
static int validate_args(FAR struct args_s *args)
{
	...
	if (args->power < 0 || args->power > (((float)CONFIG_EXAMPLES_SMPS_OUT_POWER_LIMIT) / 1000.0)) {
...
static void print_info(struct smps_limits_s *limits, struct smps_params_s *params, uint8_t *mode,
					   struct args_s *args)
{
	printf("-------------------------------------\n");
...
int main(int argc, char *argv[])
{
	...
	smps_params.v_out =
		(args->voltage > 0 ? args->voltage
						   : (float)CONFIG_EXAMPLES_SMPS_OUT_VOLTAGE_DEFAULT / 1000.0);
	smps_params.i_out =
		(args->current > 0 ? args->current
						   : (float)CONFIG_EXAMPLES_SMPS_OUT_CURRENT_DEFAULT / 1000.0);
	smps_params.p_out =
		(args->power > 0 ? args->power : (float)CONFIG_EXAMPLES_SMPS_OUT_POWER_DEFAULT / 1000.0);
	...
	while (terminate != true) {
		...
		if (smps_state.state > SMPS_STATE_RUN) {
			printf("Smps state = %d, fault = %d\n", smps_state.state, smps_state.fault);
			terminate = true;
		}
