...
#define NSECTORS(b)                                                                                \
	(((b) + CONFIG_EXAMPLES_ROMFS_SECTORSIZE - 1) / CONFIG_EXAMPLES_ROMFS_SECTORSIZE)
...
static void readdirectories(const char *path, struct node_s *entry)
{
	...
	for (direntry = readdir(dirp); direntry; direntry = readdir(dirp)) {
		...
		snprintf(g_scratchbuffer, sizeof(g_scratchbuffer), "%s/%s", path, direntry->d_name);
