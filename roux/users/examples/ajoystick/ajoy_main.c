...
static const char *g_ajoynames[AJOY_NBUTTONS] = {
	"SELECT", "FIRE", "JUMP", "BUTTON 4", "BUTTON 5", "BUTTON 6", "BUTTON 7", "BUTTON 8",
};
...
static void show_joystick(FAR const struct ajoy_sample_s *sample)
{
	...
	if (g_calibrated) {
		...
		if ((g_lispositive && tmp >= 0) || (!g_lispositive && tmp < 0)) {
			x = tmp * g_xlslope;
		} else {
			x = tmp * g_xrslope;
		}
		...
		if ((g_fispositive && tmp >= 0) || (!g_fispositive && tmp < 0)) {
			y = tmp * g_yfslope;
		} else {
			y = tmp * g_ybslope;
		}
