...
static int nxterm_initialize(void)
{
	...
	if (g_nxterm_vars.hnx) {
		...
		struct boardioc_vncstart_s vnc = {0, g_nxterm_vars.hnx};
...
int main(int argc, FAR char *argv[])
{
	...
	g_nxterm_vars.hwnd = nxtk_openwindow(g_nxterm_vars.hnx, 0, &g_nxtermcb, NULL);
	...
	printf("nxterm_main: Set window size to (%d,%d)\n", g_nxterm_vars.wndo.wsize.w,
		   g_nxterm_vars.wndo.wsize.h);
	...
	ret = nxtk_opentoolbar(g_nxterm_vars.hwnd, CONFIG_EXAMPLES_NXTERM_TOOLBAR_HEIGHT, &g_nxtoolcb,
						   NULL);
	if (ret < 0) {
	...
	nxcreate.wndo.wsize.h -= (CONFIG_EXAMPLES_NXTERM_TOOLBAR_HEIGHT + 2 * CONFIG_NXTK_BORDERWIDTH);
	...
	g_nxterm_vars.pid = task_create("NxTerm", CONFIG_EXAMPLES_NXTERM_PRIO,
									CONFIG_EXAMPLES_NXTERM_STACKSIZE, nxterm_task, NULL);
