...
static const uint16_t g_host_addr[8] = {
	HTONS(0xfe80), HTONS(0),	  HTONS(0),		 HTONS(0),
	HTONS(0),	   HTONS(0x00ff), HTONS(0xfe00), HTONS(CONFIG_EXAMPLES_MLD_IPADDR)};

static const uint16_t g_dr_addr[8] = {
	HTONS(0xfe80), HTONS(0),	  HTONS(0),		 HTONS(0),
	HTONS(0),	   HTONS(0x00ff), HTONS(0xfe00), HTONS(CONFIG_EXAMPLES_MLD_DRIPADDR)};

static const uint16_t g_netmask[8] = {HTONS(0xfffff), HTONS(0xfffff), HTONS(0xfffff),
									  HTONS(0xfffff), HTONS(0xfffff), HTONS(0xfffff),
									  HTONS(0xfffff), HTONS(0)};
#endif

static const uint16_t g_grp_addr[8] = {
	HTONS(0xff02), HTONS(0), HTONS(0), HTONS(0),
	HTONS(0),	   HTONS(0), HTONS(0), HTONS(CONFIG_EXAMPLES_MLD_GRPADDR)};
...
int main(int argc, FAR char *argv[])
{
	...
	if (ret >= 0) {
		...
		ret = sendto(sockfd, g_garbage, sizeof(g_garbage), 0, (FAR struct sockaddr *)&target,
