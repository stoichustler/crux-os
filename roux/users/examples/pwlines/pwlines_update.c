...
void pwlines_circle(FAR struct pwlines_state_s *st)
{
	...
	for (wndx = 0; wndx < 3; wndx++) {
		...
		ret = nxtk_fillcirclewindow(wndo->hwnd, &wndo->center, radius, st->facecolor);
		if (ret < 0) {
		...
		ret = nxtk_drawcirclewindow(wndo->hwnd, &wndo->center, radius,
									CONFIG_EXAMPLES_PWLINES_BORDERWIDTH, st->bordercolor);
		if (ret < 0) {
...
void pwlines_update(FAR struct pwlines_state_s *st)
{
	...
	for (wndx = 0; wndx < 3; wndx++) {
		...
		ret = nx_drawline(wndo->hwnd, &wndo->previous, CLEAR_WIDTH, st->facecolor, NX_LINECAP_NONE);
		if (ret < 0) {
		...
		ret = nx_drawline(wndo->hwnd, &vector, CONFIG_EXAMPLES_PWLINES_LINEWIDTH, st->linecolor,
						  NX_LINECAP_NONE);
		if (ret < 0) {
			printf("pwlines_update: nx_drawline failed clearing: %d\n", ret);
		}
