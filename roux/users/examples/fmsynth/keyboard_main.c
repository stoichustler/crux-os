...
static void app_user_cb(unsigned long arg, FAR struct audio_msg_s *msg, FAR bool *running);
...
static struct nxaudio_callbacks_s cbs = {app_dequeue_cb, app_complete_cb, app_user_cb};

static struct key_convert_s key_convert[] = {
	{OCTAVE(4, MUSIC_SCALE_C), 'a', "O4C"},	  {OCTAVE(4, MUSIC_SCALE_CS), 'w', "O4C+"},
	{OCTAVE(4, MUSIC_SCALE_D), 's', "O4D"},	  {OCTAVE(4, MUSIC_SCALE_DS), 'e', "O4D+"},
	{OCTAVE(4, MUSIC_SCALE_E), 'd', "O4E"},	  {OCTAVE(4, MUSIC_SCALE_F), 'f', "O4F"},
	{OCTAVE(4, MUSIC_SCALE_FS), 't', "O4F+"}, {OCTAVE(4, MUSIC_SCALE_G), 'g', "O4G"},
	{OCTAVE(4, MUSIC_SCALE_GS), 'y', "O4G+"}, {OCTAVE(4, MUSIC_SCALE_A), 'h', "O4A"},
	{OCTAVE(4, MUSIC_SCALE_AS), 'u', "O4A+"}, {OCTAVE(4, MUSIC_SCALE_B), 'j', "O4B"},
	{OCTAVE(5, MUSIC_SCALE_C), 'k', "O5C"},	  {OCTAVE(5, MUSIC_SCALE_CS), 'o', "O5C+"},
	{OCTAVE(5, MUSIC_SCALE_D), 'l', "O5D"},	  {OCTAVE(5, MUSIC_SCALE_DS), 'p', "O5D+"},
...
static void tick_callback(unsigned long arg)
{
	...
	if (scale != -1) {
		fmsynthsnd_set_soundfreq(kbd->sound, musical_scale[scale]);
		kbd->request_scale = -1;
	}
}
...
static void app_dequeue_cb(unsigned long arg, FAR struct ap_buffer_s *apb)
{
	...
	if (kbd->request_scale != -1) {
		apb->nbytes = fmsynth_rendering(kbd->sound, (FAR int16_t *)apb->samp,
										apb->nmaxbytes / sizeof(int16_t), kbd->nxaudio.chnum,
										tick_callback, (unsigned long)kbd);
	} else {
		apb->nbytes =
			fmsynth_rendering(kbd->sound, (FAR int16_t *)apb->samp,
							  apb->nmaxbytes / sizeof(int16_t), kbd->nxaudio.chnum, NULL, 0);
	}
...
static pthread_t create_audio_thread(FAR struct kbd_s *kbd)
{
	...
	pthread_create(&pid, &tattr, audio_loop_thread, (pthread_addr_t)kbd);
...
static int configure_option(FAR struct app_options *option, int argc, FAR char **argv)
{
	int opt;
...
static void print_keyusage(void)
{
	...
	for (i = 0; i < MAX_KEYCONVERT; i++) {
		printf("   [KEY]: %c, [CODE]: %s\n", key_convert[i].key_str, key_convert[i].dispstr);
	}
	printf("\n");
}
...
int main(int argc, FAR char *argv[])
{
	...
	for (i = 0; i < g_kbd.nxaudio.abufnum; i++) {
		app_dequeue_cb((unsigned long)&g_kbd, g_kbd.nxaudio.abufs[i]);
	}
