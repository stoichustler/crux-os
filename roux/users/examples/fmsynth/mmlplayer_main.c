...
{
	printf("%c: O%d%c : %d\n", LR ? 'R' : 'L', index / 12, "CcDdEFfGgAaB"[index % 12], length);
}
...
{
	printf("%c: [O%d%c, O%d%c] : %d\n", LR ? 'R' : 'L', index1 / 12, "CcDdEFfGgAaB"[index1 % 12],
		   index2 / 12, "CcDdEFfGgAaB"[index2 % 12], length);
}
...
static void update_righthand_note(FAR struct mmlplayer_s *fmmsc)
{
	...
	do {
		...
		case MML_TYPE_NOTE:
			...
			fmsynthsnd_set_soundfreq(fmmsc->rsound[0], musical_scale[mml_result.note_idx[0]]);
		...
		case MML_TYPE_CHORD:
			fmmsc->rtick = mml_result.length;
			fmsynthsnd_set_soundfreq(fmmsc->rsound[0], musical_scale[mml_result.note_idx[0]]);
			fmsynthsnd_set_soundfreq(fmmsc->rsound[1], musical_scale[mml_result.note_idx[1]]);
			fmsynthsnd_set_volume(fmmsc->rsound[0], CARRIER_LEVEL);
			fmsynthsnd_set_volume(fmmsc->rsound[1], CARRIER_LEVEL);
			print_chord(1, mml_result.note_idx[0], mml_result.note_idx[1], mml_result.length);
			break;
...
static void update_lefthand_note(FAR struct mmlplayer_s *fmmsc)
{
	...
	do {
		...
		case MML_TYPE_NOTE:
			...
			fmsynthsnd_set_soundfreq(fmmsc->lsound, musical_scale[mml_result.note_idx[0]]);
...
static void app_dequeue_cb(unsigned long arg, FAR struct ap_buffer_s *apb)
{
	...
	apb->nbytes = fmsynth_rendering(mmlplayer->lsound, (FAR int16_t *)apb->samp,
									apb->nmaxbytes / sizeof(int16_t), mmlplayer->nxaudio.chnum,
...
static pthread_t create_audio_thread(FAR struct mmlplayer_s *mmlplayer)
{
	...
	pthread_create(&pid, &tattr, audio_loop_thread, (pthread_addr_t)mmlplayer);
...
static int init_mmlplayer_sound(FAR struct mmlplayer_s *mmlplayer, int fs, int mode)
{
...
static int configure_option(FAR struct app_options *option, int argc, char **argv)
{
	int opt;
...
int main(int argc, FAR char *argv[])
{
	...
	for (i = 0; i < g_mmlplayer.nxaudio.abufnum; i++) {
		app_dequeue_cb((unsigned long)&g_mmlplayer, g_mmlplayer.nxaudio.abufs[i]);
	}
