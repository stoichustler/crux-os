...
#define EXAMPLES_CONFIGDATA_BUFSIZE (CONFIG_RAMMTD_ERASESIZE * CONFIG_EXAMPLES_CONFIGDATA_NEBLOCKS)
#endif
...
static void configdata_showmemusage(struct mallinfo *mmbefore, struct mallinfo *mmafter)
{
	...
	while ((nextdeleted = configdata_getnextdeleted()) != -1) {
		...
		if (x < CONFIG_EXAMPLES_CONFIGDATA_MAXENTRIES) {
			...
			printf("  Overwrite entry %d, OLD=%04X  NEW=%04X\n", nextdeleted,
				   g_entries[nextdeleted].id, g_entries[x].id);
#endif
