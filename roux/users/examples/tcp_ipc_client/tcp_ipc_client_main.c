...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		memcpy(buffer_to_send, (unsigned char *)&tprotocol, sizeof(protocolo_ipc));
		...
		bytes_read_from_server =
			recv(socket_client, rcv_buffer, sizeof(protocolo_ipc), TCP_DATA_RCV_WITHOUT_FLAGS);
		...
		if (bytes_read_from_server < 0) {
			...
			memcpy((unsigned char *)&tprotocol, rcv_buffer, sizeof(protocolo_ipc));
