...
static int hexstr_to_bytes(const char *hexstr, unsigned char *bytes, size_t max_bytes)
{
	int len;
	int i;
	...
	while ((opt = getopt(argc, argv, "x:t:le")) != -1) {
		...
		case 'x':
			...
			if (config->num_bytes <= 0 || config->num_bytes > TX_BUFFER_SIZE) {
				fprintf(stderr, "Error: Invalid number of bytes for write mode.\n");
				return -1;
			}
...
static int write_mode(program_config_t *config, int fd)
{
	...
	for (i = 0; i < config->num_bytes; i++) {
		len = snprintf(ptr, sizeof(data_str) - (ptr - data_str), "%02X ", config->tx_buffer[i]);
