...
static int wgetjson_postdebug_callback(FAR char **buffer, int offset, int datend, FAR int *buflen,
									   FAR void *arg)
{
...
static int wgetjson_callback(FAR char **buffer, int offset, int datend, FAR int *buflen,
							 FAR void *arg)
{
	...
	if (!g_json_buff) {
		...
		if (new_json_buff) {
			...
			memcpy(&g_json_buff[g_json_bufflen], &((*buffer)[offset]), len);
...
int main(int argc, FAR char *argv[])
{
	...
	if (is_post) {
		...
		if (is_post_multi) {
			...
			web_posts_str(post_buff, &post_buff_len, post_multi_names, post_multi_values,
						  MULTI_POST_NDATA);
		} else {
