...
static int test_region(int fd, struct esp_himem_par *param)
{
	...
	ret = ioctl(fd, HIMEMIOC_ALLOC_MAP_RANGE, (unsigned long)((uintptr_t)param));
	if (ret < 0) {
...
free_rammem:
	ret = ioctl(fd, HIMEMIOC_FREE_MAP_RANGE, (unsigned long)((uintptr_t)param));
	if (ret < 0) {
...
int main(int argc, FAR char *argv[])
{
	...
	printf("Himem has %dKiB of memory, %dKiB of which is free.\
          \nTesting the free memory...\n",
		   (int)param.memcnt / 1024, (int)param.memfree / 1024);
