...
static struct watcher_info_s g_watcher_info = {.watcher_pid = -1,
...
int watched_read_watcher_info(struct watched_info_s *info)
{
	...
	if (g_watcher_info.watcher_pid < 0) {
		...
		if (fp == NULL) {
			int errcode = errno;
			fprintf(stderr, "ERROR: Failed to open %s: %d\n", g_watcher_info.info_file_name,
					errcode);
			ret = errcode;
			goto errout;
		}

		fscanf(fp, "%d %d %d %d %d", (int *)&(g_watcher_info.watcher_pid), &(g_watcher_info.signal),
			   &(g_watcher_info.sub_cmd), &(g_watcher_info.feed_cmd), &(g_watcher_info.unsub_cmd));
...
int watched_subscribe(struct watched_info_s *info)
{
	...
	ret = sigqueue(g_watcher_info.watcher_pid, g_watcher_info.signal, info->sub_value);
	if (ret == ERROR) {
		int errcode = errno;
...
int watched_unsubscribe(struct watched_info_s *info)
{
	...
	ret = sigqueue(g_watcher_info.watcher_pid, g_watcher_info.signal, info->unsub_value);
	if (ret == ERROR) {
		int errcode = errno;
...
int watched_feed_dog(struct watched_info_s *info)
{
	...
	ret = sigqueue(g_watcher_info.watcher_pid, g_watcher_info.signal, info->feed_value);
	if (ret == ERROR) {
		int errcode = errno;
