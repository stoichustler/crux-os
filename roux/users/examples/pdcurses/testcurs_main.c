...
static const char *acs_names[] = {"ACS_ULCORNER", "ACS_URCORNER", "ACS_LLCORNER", "ACS_LRCORNER",
...
#if HAVE_WIDE
static const cchar_t *wacs_values[] = {WACS_ULCORNER,
									   WACS_URCORNER,
									   WACS_LLCORNER,
									   WACS_LRCORNER,
									   WACS_LTEE,
...
static const wchar_t russian[] = {0x0420, 0x0443, 0x0441, 0x0441, 0x043a, 0x0438, 0x0439,
								  L' ',	  0x044f, 0x0437, 0x044b, 0x043a, 0};

static const wchar_t greek[] = {0x0395, 0x03bb, 0x03bb, 0x03b7, 0x03bd, 0x03b9, 0x03ba, 0x03ac, 0};

static const wchar_t georgian[] = {0x10e5, 0x10d0, 0x10e0, 0x10d7, 0x10e3, 0x10da,
								   0x10d8, L' ',   0x10d4, 0x10dc, 0x10d0, 0};
#endif

#if HAVE_COLOR
static const short colors[] = {COLOR_BLACK, COLOR_RED,	   COLOR_GREEN,	 COLOR_BLUE,
							   COLOR_CYAN,	COLOR_MAGENTA, COLOR_YELLOW, COLOR_WHITE};

static const char *colornames[] = {"COLOR_BLACK", "COLOR_RED",	   "COLOR_GREEN",  "COLOR_BLUE",
								   "COLOR_CYAN",  "COLOR_MAGENTA", "COLOR_YELLOW", "COLOR_WHITE"};
#endif
...
static void intro_test(WINDOW *win)
{
	...
	cbreak();
	mvwaddstr(win, 1, 1, "You should have a rectangle in the middle of the screen");
...
{
	static const char *text = "This string placed in clipboard by PDCurses test program, testcurs.";
	char *ptr = NULL;
	long length = 0;
	long i;
#ifdef CONFIG_PDCURSES_MULTITHREAD
	...
	clear();
	mvaddstr(1, 1, "This test will place the following string in the system clipboard:");
	mvaddstr(2, 1, text);
...
static void color_test(WINDOW *win)
{
	...
	if (can_change_color()) {
		...
		for (i = 0; i < MAXCOL; i++) {
			color_content(i, &(orgcolors[i].red), &(orgcolors[i].green), &(orgcolors[i].blue));
		}
