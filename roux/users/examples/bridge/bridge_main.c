...
#if defined(CONFIG_EXAMPLES_BRIDGE_NET1_DHCPC) || defined(CONFIG_EXAMPLES_BRIDGE_NET2_DHCPC)
...
static int briget_net1_setup(void)
{
	...
	struct in_addr addr;
#if defined(CONFIG_EXAMPLES_BRIDGE_NET1_DHCPC) || defined(CONFIG_EXAMPLES_BRIDGE_NET1_NOMAC)
...
#ifndef CONFIG_NSH_NETINIT
	struct in_addr addr;
#if defined(CONFIG_EXAMPLES_BRIDGE_NET2_DHCPC) || defined(CONFIG_EXAMPLES_BRIDGE_NET2_NOMAC)
	uint8_t mac[IFHWADDRLEN];
#endif
...
static int bridge_net1_worker(int argc, char *argv[])
{
	...
	printf("NET1: Create receive socket: %u.%u.%u.%u:%u\n", ip4_addr1(receiver.sin_addr.s_addr),
		   ip4_addr2(receiver.sin_addr.s_addr), ip4_addr3(receiver.sin_addr.s_addr),
		   ip4_addr4(receiver.sin_addr.s_addr), htons(receiver.sin_port));
	...
	printf("NET1: Create send socket: %u.%u.%u.%u:INPORT_ANY\n", ip4_addr1(g_net2_ipaddr),
		   ip4_addr2(g_net2_ipaddr), ip4_addr3(g_net2_ipaddr), ip4_addr4(g_net2_ipaddr));
	...
	for (;;) {
		...
		printf("NET1: Received %ld bytes from %u.%u.%u.%u:%u\n", (long)nrecvd,
			   ip4_addr1(fromaddr.sin_addr.s_addr), ip4_addr2(fromaddr.sin_addr.s_addr),
			   ip4_addr3(fromaddr.sin_addr.s_addr), ip4_addr4(fromaddr.sin_addr.s_addr),
		...
		printf("NET1: Sending %ld bytes on network 2: %u.%u.%u.%u:%u\n", (long)nrecvd,
			   ip4_addr1(toaddr.sin_addr.s_addr), ip4_addr2(toaddr.sin_addr.s_addr),
			   ip4_addr3(toaddr.sin_addr.s_addr), ip4_addr4(toaddr.sin_addr.s_addr),
			   htons(toaddr.sin_port));

		nsent = sendto(sndsd, g_net1_buffer, nrecvd, 0, (struct sockaddr *)&toaddr,
...
static int bridge_net2_worker(int argc, char *argv[])
{
	...
	printf("NET2: Create send socket: %u.%u.%u.%u:INPORT_ANY\n", ip4_addr1(g_net1_ipaddr),
		   ip4_addr2(g_net1_ipaddr), ip4_addr3(g_net1_ipaddr), ip4_addr4(g_net1_ipaddr));
	...
	for (;;) {
		...
		nsent = sendto(sndsd, g_net2_buffer, nrecvd, 0, (struct sockaddr *)&toaddr,
					   sizeof(struct sockaddr_in));
...
int main(int argc, FAR char *argv[])
{
	...
	net1_worker = task_create("NET1 Worker", CONFIG_EXAMPLES_BRIDGE_NET1_PRIORITY,
							  CONFIG_EXAMPLES_BRIDGE_NET1_STACKSIZE, bridge_net1_worker, NULL);
	if (net1_worker < 0) {
	...
	net2_worker = task_create("NET2 Worker", CONFIG_EXAMPLES_BRIDGE_NET2_PRIORITY,
							  CONFIG_EXAMPLES_BRIDGE_NET2_STACKSIZE, bridge_net2_worker, NULL);
	if (net2_worker < 0) {
