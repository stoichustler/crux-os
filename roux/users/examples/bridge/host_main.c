...
int main(int argc, char *argv[])
{
	...
	printf(LABEL "Create receive socket: IPHOST=%08x RECVPORT=%d\n", EXAMPLES_BRIDGE_RECV_IPHOST,
		   EXAMPLES_BRIDGE_SEND_HOSTPORT);
	...
	nsent = sendto(sndsd, g_sndmessage, sizeof(g_sndmessage), 0, (struct sockaddr *)&toaddr,
	...
	printf(LABEL "Received %ld bytes from %s:%u\n", (long)nrecvd, inet_ntoa(fromaddr.sin_addr),
