...
int main(int argc, FAR char *argv[])
{
	...
	for (x = 0; x < fmt.nsectors >> 1; x++) {
		...
		logsector = inode->u.i_bops->ioctl(inode, BIOC_ALLOCSECT, (unsigned long)-1);
		if (logsector < 0) {
		...
		snprintf(buffer, fmt.availbytes, "Logical sector %d sequence %d\n", sectors[x], seqs[x]);

		readwrite.logsector = sectors[x];
		readwrite.offset = 0;
		readwrite.count = strlen(buffer) + 1;
		readwrite.buffer = (uint8_t *)buffer;
		inode->u.i_bops->ioctl(inode, BIOC_WRITESECT, (unsigned long)&readwrite);
	...
	for (x = 0; x < fmt.nsectors >> 1; x++) {
		...
		readwrite.buffer = (uint8_t *)buffer;
		ret = inode->u.i_bops->ioctl(inode, BIOC_READSECT, (unsigned long)&readwrite);
		...
		snprintf(&buffer[100], fmt.availbytes - 100, "Logical sector %d sequence %d\n", sectors[x],
				 seqs[x]);
	...
	for (x = 0; x < fmt.nsectors >> 1; x++) {
		...
		snprintf(buffer, fmt.availbytes, "Appended data in sector %d\n", sectors[x]);
		readwrite.logsector = sectors[x];
		readwrite.offset = 64;
		readwrite.count = strlen(buffer) + 1;
		readwrite.buffer = (uint8_t *)buffer;
	...
	for (x = 0; x < fmt.nsectors >> 1; x++) {
		...
		if (sectors[x] != 0xffff) {
			inode->u.i_bops->ioctl(inode, BIOC_FREESECT, (unsigned long)sectors[x]);
		}
	}
