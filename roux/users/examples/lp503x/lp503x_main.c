...
static struct lp503x_cmd_s g_lp503x_cmds[] = {
	{"q", "", lp503x_cmd_quit, LP503X_HELP_TEXT("Exit lp503x")},
	{"h", "", lp503x_cmd_help, LP503X_HELP_TEXT("Display help for commands")},
	{"?", "", lp503x_cmd_help, LP503X_HELP_TEXT("Display help for commands")},
	{"help", "", lp503x_cmd_help, LP503X_HELP_TEXT("Display help for commands")},
	{"m", "", lp503x_cmd_mode, LP503X_HELP_TEXT("send default mode to the device")},
	{"c", "hex", lp503x_cmd_setcolour, LP503X_HELP_TEXT("Hex RGB colour required")},
	{"l", "led, 0..11", lp503x_cmd_set_rgbled,
	 LP503X_HELP_TEXT("set RGB LED selected to current colour")},
	{"n", "led, 0..35", lp503x_cmd_setled,
	 LP503X_HELP_TEXT("set individual LED to current brightness X")},
	{"p", "num of leds in pattern (1..12)", lp503x_cmd_pattern,
	 LP503X_HELP_TEXT("play LED pattern")},
	{"b", "brightness, 0-255", lp503x_cmd_brightness,
	 LP503X_HELP_TEXT("set brightness of all leds")},
	{"d", "disable=1", lp503x_cmd_disable, LP503X_HELP_TEXT("globally disable ALL leds")},
	{"k", "led num", lp503x_cmd_set_bank_mode, LP503X_HELP_TEXT("enable bank mode for LED ")},
	{"i", "led num 0..11", lp503x_cmd_set_individual_mode,
	 LP503X_HELP_TEXT("enable individual mode for LED ")},
	{"A", "", lp503x_cmd_set_banka_colour, LP503X_HELP_TEXT("set bankA colour")},
	{"B", "", lp503x_cmd_set_bankb_colour, LP503X_HELP_TEXT("set bankB colour")},
	{"C", "", lp503x_cmd_set_bankc_colour, LP503X_HELP_TEXT("set bankC colour")},
	{"X", "0..255", lp503x_cmd_set_bank_brightness,
	 LP503X_HELP_TEXT("Bank or individual LED brightness required")},
};

static const int g_lp503x_cmd_count = sizeof(g_lp503x_cmds) / sizeof(struct lp503x_cmd_s);
...
static int lp503x_cmd_pattern(FAR char *parg)
{
	...
	int colour_lookup[12] = {0x0000ff, 0x00ff00, 0xff0000, 0x00ffff, 0xff00ff, 0xffff33,
							 0xffffff, 0xffa500, 0xff00a5, 0x8080ff, 0x80ff80, 0xff8080};

	int pattern_brightness[5] = {50, 100, 150, 200, 255};
...
static int lp503x_cmd_help(FAR char *parg)
{
	...
	for (x = 0; x < g_lp503x_cmd_count; x++) {
		len = strlen(g_lp503x_cmds[x].cmd) + strlen(g_lp503x_cmds[x].arghelp);
		if (len > maxlen) {
			maxlen = len;
		}
	}
	...
	for (x = 0; x < g_lp503x_cmd_count; x++) {
		...
		printf("  %s %s", g_lp503x_cmds[x].cmd, g_lp503x_cmds[x].arghelp);

		/* Calculate number of spaces to print before the help text */

		len = maxlen - (strlen(g_lp503x_cmds[x].cmd) + strlen(g_lp503x_cmds[x].arghelp));
		for (c = 0; c < len; c++) {
			printf(" ");
		}
