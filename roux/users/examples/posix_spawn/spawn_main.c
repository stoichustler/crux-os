...
static char *const g_hello_argv[5] = {"hello", "Argument 1", "Argument 2", "Argument 3", NULL};

static char *const g_redirect_argv[2] = {"redirect", NULL};
...
int main(int argc, FAR char *argv[])
{
	...
	ret = mount(CONFIG_EXAMPLES_POSIXSPAWN_DEVPATH, MOUNTPT, "romfs", MS_RDONLY, NULL);
	if (ret < 0) {
	...
	ret = posix_spawn(&pid, filepath, &file_actions, &attr, g_hello_argv, NULL);
	if (ret != 0) {
	...
	ret = posix_spawn(&pid, filepath, &file_actions, &attr, g_redirect_argv, NULL);
	if (ret != 0) {
		errmsg("ERROR: posix_spawn failed: %d\n", ret);
	}
