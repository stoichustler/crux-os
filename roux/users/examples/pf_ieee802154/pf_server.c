...
int main(int argc, FAR char *argv[])
{
	...
	for (offset = 0; offset < 256; offset++) {
		...
		recvlen = addrlen;
		nbytes = recvfrom(sockfd, inbuf, 1024, 0, (struct sockaddr *)&client, &recvlen);
		...
		if (client.sa_addr.s_mode == IEEE802154_ADDRMODE_SHORT) {
			...
			printf("server: %d. Received %d bytes from "
				   ...
				   offset, nbytes, client.sa_addr.s_eaddr[0], client.sa_addr.s_eaddr[1],
				   client.sa_addr.s_eaddr[2], client.sa_addr.s_eaddr[3], client.sa_addr.s_eaddr[4],
				   client.sa_addr.s_eaddr[5], client.sa_addr.s_eaddr[6], client.sa_addr.s_eaddr[7]);
		} else {
