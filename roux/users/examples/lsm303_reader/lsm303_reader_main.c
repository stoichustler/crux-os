...
int main(int argc, FAR char *argv[])
{
	...
	for (;;) {
		ret = ioctl(fileno(sensor), SNIOC_LSM303AGRSENSORREAD, (unsigned long)&sensor_data);
		if (ret < 0) {
			printf("IOCTL READ failed %d\n", ret);
		}

		printf("time:%d x:%d y:%d z:%d m_x:%d m_y:%d m_z:%d temp:%d\n", sensor_data.timestamp,
			   sensor_data.x_data, sensor_data.y_data, sensor_data.z_data, sensor_data.m_x_data,
			   sensor_data.m_y_data, sensor_data.m_z_data, sensor_data.temperature);
