...
int main(int argc, FAR char *argv[])
{
	...
	for (nloops = 0; !g_capexample.nloops || nloops < g_capexample.nloops; nloops++) {
		...
		ret = ioctl(fd, CAPIOC_DUTYCYCLE, (unsigned long)((uintptr_t)&dutycycle));
		if (ret < 0) {
		...
		ret = ioctl(fd, CAPIOC_FREQUENCE, (unsigned long)((uintptr_t)&frequence));
		if (ret < 0) {
		...
		ret = ioctl(fd, CAPIOC_EDGES, (unsigned long)((uintptr_t)&edges));
		if (ret < 0) {
