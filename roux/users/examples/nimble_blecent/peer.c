...
FAR static struct peer_svc *peer_svc_find_range(FAR struct peer *peer, uint16_t attr_handle);
FAR static struct peer_svc *peer_svc_find(FAR struct peer *peer, uint16_t svc_start_handle,
										  struct peer_svc **out_prev);
static int peer_svc_is_empty(const struct peer_svc *svc);

static uint16_t chr_end_handle(FAR const struct peer_svc *svc, FAR const struct peer_chr *chr);
static int chr_is_empty(FAR const struct peer_svc *svc, FAR const struct peer_chr *chr);
FAR static struct peer_chr *peer_chr_find(FAR const struct peer_svc *svc, uint16_t chr_def_handle,
										  FAR struct peer_chr **out_prev);
static void peer_disc_chrs(FAR struct peer *peer);

static int peer_dsc_disced(uint16_t conn_handle, FAR const struct ble_gatt_error *error,
...
FAR static struct peer_dsc *peer_dsc_find_prev(FAR const struct peer_chr *chr, uint16_t dsc_handle)
{
...
FAR static struct peer_dsc *peer_dsc_find(FAR const struct peer_chr *chr, uint16_t dsc_handle,
...
static int peer_dsc_add(FAR struct peer *peer, uint16_t chr_val_handle,
		...
		SLIST_FOREACH(chr, &svc->chrs, next)
		{
			if (!chr_is_empty(svc, chr) && SLIST_EMPTY(&chr->dscs) &&
				...
				rc = ble_gattc_disc_all_dscs(peer->conn_handle, chr->chr.val_handle,
											 chr_end_handle(svc, chr), peer_dsc_disced, peer);
				if (rc != 0) {
...
FAR static struct peer_chr *peer_chr_find_prev(const struct peer_svc *svc, uint16_t chr_val_handle)
{
...
FAR static struct peer_chr *peer_chr_find(FAR const struct peer_svc *svc, uint16_t chr_val_handle,
										  FAR struct peer_chr **out_prev)
{
	FAR struct peer_chr *prev;
	FAR struct peer_chr *chr;
...
static int peer_chr_add(FAR struct peer *peer, uint16_t svc_start_handle,
...
static int peer_chr_disced(uint16_t conn_handle, FAR const struct ble_gatt_error *error,
...
FAR static struct peer_svc *peer_svc_find_prev(FAR struct peer *peer, uint16_t svc_start_handle)
{
...
FAR const struct peer_svc *peer_svc_find_uuid(FAR const struct peer *peer,
...
FAR const struct peer_chr *peer_chr_find_uuid(FAR const struct peer *peer,
...
FAR const struct peer_dsc *peer_dsc_find_uuid(FAR const struct peer *peer,
											  FAR const ble_uuid_t *svc_uuid,
											  FAR const ble_uuid_t *chr_uuid,
...
static int peer_svc_add(FAR struct peer *peer, FAR const struct ble_gatt_svc *gatt_svc)
{
	FAR struct peer_svc *prev;
	FAR struct peer_svc *svc;
...
static int peer_svc_disced(uint16_t conn_handle, FAR const struct ble_gatt_error *error,
...
int peer_disc_all(uint16_t conn_handle, FAR peer_disc_fn *disc_cb, FAR void *disc_cb_arg)
{
	FAR struct peer_svc *svc;
	FAR struct peer *peer;
	int rc;
...
int peer_init(int max_peers, int max_svcs, int max_chrs, int max_dscs)
{
	...
	rc = os_mempool_init(&peer_svc_pool, max_svcs, sizeof(struct peer_svc), peer_svc_mem,
						 "peer_svc_pool");
	if (rc != 0) {
		rc = BLE_HS_EOS;
		goto err;
	...
	rc = os_mempool_init(&peer_chr_pool, max_chrs, sizeof(struct peer_chr), peer_chr_mem,
						 "peer_chr_pool");
	if (rc != 0) {
		rc = BLE_HS_EOS;
		goto err;
	...
	rc = os_mempool_init(&peer_dsc_pool, max_dscs, sizeof(struct peer_dsc), peer_dsc_mem,
						 "peer_dsc_pool");
	if (rc != 0) {
		rc = BLE_HS_EOS;
		goto err;
