...
static int blecent_on_read(uint16_t conn_handle, FAR const struct ble_gatt_error *error,
						   FAR struct ble_gatt_attr *attr, FAR void *arg)
{
	printf("Read complete; status=%d conn_handle=%d", error->status, conn_handle);
	if (error->status == 0) {
...
static int blecent_on_write(uint16_t conn_handle, FAR const struct ble_gatt_error *error,
							FAR struct ble_gatt_attr *attr, FAR void *arg)
{
...
static int blecent_on_subscribe(uint16_t conn_handle, FAR const struct ble_gatt_error *error,
								FAR struct ble_gatt_attr *attr, FAR void *arg)
{
...
static void blecent_read_write_subscribe(FAR const struct peer *peer)
{
	...
	value[0] = 99;
	value[1] = 100;
	rc = ble_gattc_write_flat(peer->conn_handle, chr->chr.val_handle, value, sizeof value,
							  blecent_on_write, NULL);
	if (rc != 0) {
	...
	dsc = peer_dsc_find_uuid(peer, BLE_UUID16_DECLARE(BLE_SVC_BAS_UUID16),
...
static void blecent_on_disc_complete(FAR const struct peer *peer, int status, FAR void *arg)
{
	if (status != 0) {
...
static void blecent_connect_if_interesting(FAR const struct ble_gap_disc_desc *disc)
{
	uint8_t own_addr_type;
	int rc;
...
{
	ble_svc_gap_device_name_set("ROUX NimBLE CENT");
	nimble_port_run();
}
...
int main(int argc, FAR char **argv)
{
	...
	ret = ble_npl_task_init(&s_task_hci, "hci_sock", ble_hci_sock_task, NULL, TASK_DEFAULT_PRIORITY,
	...
	ret = ble_npl_task_init(&s_task_host, "ble_host", ble_host_task, NULL, TASK_DEFAULT_PRIORITY,
							BLE_NPL_TIME_FOREVER, TASK_DEFAULT_STACK, TASK_DEFAULT_STACK_SIZE);
	if (ret != 0) {
