 ...
 * should be forwarded from network 0 to network1 by the ROUX IP forwarding
 * logic.
 */

#ifdef CONFIG_NET_IPv6
static const uint16_t g_tun0_laddr[8] = {
	HTONS(0x7c00), HTONS(0), HTONS(0), HTONS(0), HTONS(0), HTONS(0), HTONS(0), /* Netork 0 */
	HTONS(0x0097),
};

static const uint16_t g_tun1_laddr[8] = {
	HTONS(0x7c00), HTONS(0), HTONS(0), HTONS(0), HTONS(0), HTONS(0), HTONS(0x0001), /* Netork 1 */
	HTONS(0x0139),
};
...
static const uint16_t g_netmask[8] = {
	...
	HTONS(0xffff), HTONS(0xffff), HTONS(0xffff), HTONS(0),
};
#else
...
static int ipfwd_netconfig(FAR struct ipfwd_tun_s *tun, IPADDR_TYPE ipaddr, IPADDR_TYPE netmask)
{
	int ret;
...
static uint16_t check_sum(uint16_t sum, FAR const uint8_t *data, uint16_t len)
{
...
static uint16_t common_chksum(FAR uint8_t *buffer, uint8_t proto)
{
	...
	sum = check_sum(sum, (FAR uint8_t *)&ipv6->srcipaddr, 2 * sizeof(net_ipv6addr_t));
	...
	sum = check_sum(sum, (FAR uint8_t *)&ipv4->srcipaddr, 2 * sizeof(in_addr_t));
...
#else
		sol = (FAR struct icmpv6_neighbor_solicit_s *)&fwd->ia_buffer[IPv6_HDRLEN];
#endif
#else
...
#else
		sol = (FAR struct icmpv6_neighbor_solicit_s *)&fwd->ia_buffer[IPv4_HDRLEN];
#endif
#endif
