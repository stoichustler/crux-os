...
#ifdef CONFIG_PWM_MULTICHAN
	printf("pwm_main: starting output with frequency: %" PRIu32, info.frequency);

	for (i = 0; i < CONFIG_PWM_NCHANNELS; i++) {
		info.channels[i].channel = g_pwmstate.channels[i];
		info.channels[i].duty =
			g_pwmstate.duties[i] ? b16divi(uitoub16(g_pwmstate.duties[i]) - 1, 100) : 0;
		printf(" channel: %d duty: %08" PRIx32, info.channels[i].channel, info.channels[i].duty);
	}

	printf("\n");

#else
	info.duty = g_pwmstate.duty ? b16divi(uitoub16(g_pwmstate.duty) - 1, 100) : 0;
#ifdef CONFIG_PWM_PULSECOUNT
