...
{
	printf("handle=%d our_ota_addr_type=%d our_ota_addr=", desc->conn_handle,
...
static int bleprph_gap_event(FAR struct ble_gap_event *event, FAR void *arg)
{
	...
	case BLE_GAP_EVENT_CONNECT:
		...
		printf("connection %s; status=%d ", event->connect.status == 0 ? "established" : "failed",
...
static void nimble_host_task(FAR void *param)
{
	ble_svc_gap_device_name_set("ROUX NimBLE PRPH");
	nimble_port_run();
}
