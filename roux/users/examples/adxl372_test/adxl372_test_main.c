...
static int adxl372_test(int is_interactive, FAR char *path)
{
	...
	unsigned char tstchars[] = {0x5e, 0xc5, 0x00};
	...
	do {
		...
		if (bfr[0] != ADXL372_DEVID_AD_VALUE && bfr[1] != ADXL372_DEVID_MST_VALUE &&
			bfr[2] != ADXL372_PARTID_VALUE) {
			printf(CRED "ERROR: Sensor ID is 0x%02X%02X%02X%02X, "
						"expected 0x%02X%02X%02Xxx." CRESET "\n",
				   bfr[0], bfr[1], bfr[2], bfr[3], ADXL372_DEVID_AD_VALUE, ADXL372_DEVID_MST_VALUE,
