...
static void nxhello_redraw(NXWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool morem,
						   FAR void *arg);
...
static void nxhello_mousein(NXWINDOW hwnd, FAR const struct nxgl_point_s *pos, uint8_t buttons,
							FAR void *arg);
#endif

#ifdef CONFIG_NX_KBD
static void nxhello_kbdin(NXWINDOW hwnd, uint8_t nch, FAR const uint8_t *ch, FAR void *arg);
#endif
...
static void nxhello_redraw(NXWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool more,
						   FAR void *arg)
{
...
static void nxhello_center(FAR struct nxgl_point_s *pos, FAR const struct nx_font_s *fontset)
{
...
static void nxhello_initglyph(FAR uint8_t *glyph, uint8_t height, uint8_t width, uint8_t stride)
{
...
#warning "More logic is needed for the case where CONFIG_NX_PLANES > 1"
#endif
			RENDERER((FAR nxgl_mxpixel_t *)glyph, fheight, fwidth, fstride, fbm,
