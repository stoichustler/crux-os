...
static int foc_handler_run(FAR struct foc_motor_f32_s *motor, FAR struct foc_device_s *dev)
{
	...
	foc_handler_state_f32(&motor->handler, &motor->foc_state, &motor->mod_state);
...
static int foc_model_state_get(FAR struct foc_motor_f32_s *motor, FAR struct foc_device_s *dev)
{
	int i = 0;
...
static void foc_float_nxscope(FAR struct foc_nxscope_s *nxs, FAR struct foc_motor_f32_s *motor,
...
int foc_float_thr(FAR struct foc_ctrl_env_s *envp)
{
	...
	while (motor.mq.quit == false) {
		...
		foc_model_run_f32(&motor.model, FOC_MODEL_LOAD, &motor.foc_state.vab);
#endif
