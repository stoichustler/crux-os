...
static int foc_motor_configure(FAR struct foc_motor_f32_s *motor)
{
	...
	ret = foc_ramp_init_f32(&motor->ramp, motor->per, RAMP_CFG_THR, (motor->envp->cfg->acc / 1.0f),
...
#error FOC modulation not selected
	...
	ret = foc_handler_init_f32(&motor->handler, foc_ctrl, foc_mod);
	if (ret < 0) {
	...
	motor_phy_params_init(&motor->phy, CONFIG_EXAMPLES_FOC_MOTOR_POLES,
	...
	ret = foc_model_init_f32(&motor->model, &g_foc_model_pmsm_ops_f32);
	if (ret < 0) {
...
static int foc_motor_torq(FAR struct foc_motor_f32_s *motor, uint32_t torq)
{
	...
	motor->torq.des =
		(motor->dir * torq * SETPOINT_INTF_SCALE * motor->envp->cfg->torqmax / 1000.0f);
...
static int foc_motor_vel(FAR struct foc_motor_f32_s *motor, uint32_t vel)
{
	...
	motor->vel.des = (motor->dir * vel * SETPOINT_INTF_SCALE * motor->envp->cfg->velmax / 1000.0f);
...
static int foc_motor_pos(FAR struct foc_motor_f32_s *motor, uint32_t pos)
{
	...
	motor->pos.des = (motor->dir * pos * SETPOINT_INTF_SCALE * motor->envp->cfg->posmax / 1000.0f);
...
static int foc_motor_run(FAR struct foc_motor_f32_s *motor)
{
	...
	case FOC_MMODE_VEL: {
		...
		f_saturate(&motor->vel.des, -motor->vel_sat, motor->vel_sat);
		...
		if (motor->time % VEL_CONTROL_PRESCALER == 0) {
			...
			ret = foc_ramp_run_f32(&motor->ramp, motor->vel.des, motor->vel.now, &motor->vel.set);
			if (ret < 0) {
...
#endif
			{
				...
				motor->torq.des = pi_controller(&motor->vel_pi, vel_err);
#else
#error Missing velocity controller
	...
	case FOC_MMODE_TORQ: {
		...
		f_saturate(&motor->torq.des, -motor->torq_sat, motor->torq_sat);
...
#ifdef CONFIG_EXAMPLES_FOC_FEEDFORWARD
	foc_feedforward_pmsm_f32(&motor->phy, &motor->foc_state.idq, motor->vel.now, &motor->vdq_comp);
#else
...
static int foc_motor_ang_get(FAR struct foc_motor_f32_s *motor)
{
	...
	if (aout.type == FOC_ANGLE_TYPE_ELE) {
		...
		motor->angle_el = fmodf(motor->angle_m * motor->phy.p, MOTOR_ANGLE_E_MAX);
	}
...
int foc_motor_init(FAR struct foc_motor_f32_s *motor, FAR struct foc_ctrl_env_s *envp)
{
#ifdef CONFIG_EXAMPLES_FOC_HAVE_OPENLOOP
	...
	foc_angle_init_f32(&motor->openloop, &g_foc_angle_ol_f32);
	...
	ret = foc_angle_init_f32(&motor->qenco, &g_foc_angle_qe_f32);
	if (ret < 0) {
	...
	ret = foc_angle_init_f32(&motor->hall, &g_foc_angle_hl_f32);
	if (ret < 0) {
		PRINTFV("ERROR: foc_angle_init_f32 failed %d!\n", ret);
		goto errout;
	}
	...
	ret = foc_angle_init_f32(&motor->ang_smo, &g_foc_angle_osmo_f32);
	if (ret < 0) {
		PRINTFV("ERROR: foc_angle_init_f32 failed %d!\n", ret);
		goto errout;
	}
	...
	ret = foc_angle_init_f32(&motor->ang_nfo, &g_foc_angle_onfo_f32);
	if (ret < 0) {
		PRINTFV("ERROR: foc_angle_init_f32 failed %d!\n", ret);
		goto errout;
	}
	...
	ret = foc_velocity_init_f32(&motor->vel_div, &g_foc_velocity_odiv_f32);
	if (ret < 0) {
		PRINTFV("ERROR: foc_angle_init_f32 failed %d!\n", ret);
		goto errout;
	}
	...
	ret = foc_velocity_init_f32(&motor->vel_pll, &g_foc_velocity_opll_f32);
	if (ret < 0) {
		PRINTFV("ERROR: foc_angle_init_f32 failed %d!\n", ret);
		goto errout;
	}
	...
	pi_controller_init(&motor->vel_pi, (motor->envp->cfg->vel_pi_kp / 1000000.0f),
...
int foc_motor_handle(FAR struct foc_motor_f32_s *motor, FAR struct foc_mq_s *handle)
{
	int ret = OK;
