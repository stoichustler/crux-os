...
static int foc_motor_ident(FAR struct foc_motor_b16_s *motor, FAR bool *done)
{
	...
	if (ret == FOC_ROUTINE_RUN_DONE) {
		...
		if (final.res < FOC_FLOAT_IDENT_RES_MIN || final.res > FOC_FLOAT_IDENT_RES_MAX) {
		...
		if (final.ind < FOC_FLOAT_IDENT_IND_MIN || final.ind > FOC_FLOAT_IDENT_IND_MAX) {
...
static int foc_motor_configure(FAR struct foc_motor_b16_s *motor)
{
	...
	ret = foc_ramp_init_b16(&motor->ramp, motor->per, ftob16(RAMP_CFG_THR),
...
#error FOC modulation not selected
	...
	ret = foc_handler_init_b16(&motor->handler, foc_ctrl, foc_mod);
	if (ret < 0) {
	...
	motor_phy_params_init_b16(&motor->phy, CONFIG_EXAMPLES_FOC_MOTOR_POLES,
	...
	ret = foc_model_init_b16(&motor->model, &g_foc_model_pmsm_ops_b16);
	if (ret < 0) {
...
#if defined(CONFIG_EXAMPLES_FOC_HAVE_OPENLOOP) || defined(CONFIG_EXAMPLES_FOC_VELOBS)
/****************************************************************************
...
static int foc_motor_state(FAR struct foc_motor_b16_s *motor, int state)
{
	...
	case FOC_EXAMPLE_STATE_STOP: {
		...
		motor->dq_ref.q = ftob16(CONFIG_EXAMPLES_FOC_STOP_CURRENT / 1000.0f);
		motor->dq_ref.d = 0;
#else
...
#error STOP state for sensored mode requires velocity support
	...
	if ((motor->mq.app_state == FOC_EXAMPLE_STATE_FREE ||
		...
		(state == FOC_EXAMPLE_STATE_CW || state == FOC_EXAMPLE_STATE_CCW)) {
...
static void foc_motor_openloop_angobs(FAR struct foc_motor_b16_s *motor)
{
	...
	if (motor->openloop_now == FOC_OPENLOOP_ENABLED) {
		if (vel_abs >= motor->ol_thr) {
			...
			motor->angle_step = b16mulb16(motor->angle_err, ftob16(ANGLE_MERGE_FACTOR));
...
static int foc_motor_run(FAR struct foc_motor_b16_s *motor)
{
	...
	case FOC_MMODE_VEL: {
		...
		f_saturate_b16(&motor->vel.des, -motor->vel_sat, motor->vel_sat);
		...
		if (motor->time % VEL_CONTROL_PRESCALER == 0) {
			...
			ret = foc_ramp_run_b16(&motor->ramp, motor->vel.des, motor->vel.now, &motor->vel.set);
			if (ret < 0) {
...
#endif
			{
				...
				motor->torq.des = pi_controller_b16(&motor->vel_pi, vel_err);
#else
#error Missing velocity controller
	...
	case FOC_MMODE_TORQ: {
		...
		f_saturate_b16(&motor->torq.des, -motor->torq_sat, motor->torq_sat);
...
#ifdef CONFIG_EXAMPLES_FOC_FEEDFORWARD
	foc_feedforward_pmsm_b16(&motor->phy, &motor->foc_state.idq, motor->vel.now, &motor->vdq_comp);
#else
	motor->vdq_comp.q = 0;
	motor->vdq_comp.d = 0;
#endif
...
static int foc_motor_ang_get(FAR struct foc_motor_b16_s *motor)
{
	...
	if (aout.type == FOC_ANGLE_TYPE_ELE) {
		...
		motor->angle_el = (b16muli(motor->angle_m, motor->phy.p) % MOTOR_ANGLE_E_MAX_B16);
	}
...
int foc_motor_init(FAR struct foc_motor_b16_s *motor, FAR struct foc_ctrl_env_s *envp)
{
#ifdef CONFIG_EXAMPLES_FOC_HAVE_OPENLOOP
	...
	foc_angle_init_b16(&motor->openloop, &g_foc_angle_ol_b16);
	...
	ret = foc_angle_init_b16(&motor->qenco, &g_foc_angle_qe_b16);
	if (ret < 0) {
	...
	snprintf(motor->qedpath, sizeof(motor->qedpath), "%s%d", CONFIG_EXAMPLES_FOC_QENCO_DEVPATH,
			 motor->envp->id);
	...
	ret = foc_angle_init_b16(&motor->hall, &g_foc_angle_hl_b16);
	if (ret < 0) {
		PRINTFV("ERROR: foc_angle_init_b16 failed %d!\n", ret);
		goto errout;
	}
	...
	snprintf(motor->hldpath, sizeof(motor->hldpath), "%s%d", CONFIG_EXAMPLES_FOC_HALL_DEVPATH,
			 motor->envp->id);
	...
	ret = foc_angle_init_b16(&motor->ang_smo, &g_foc_angle_osmo_b16);
	if (ret < 0) {
		PRINTFV("ERROR: foc_angle_init_b16 failed %d!\n", ret);
		goto errout;
	}
	...
	ret = foc_angle_init_b16(&motor->ang_nfo, &g_foc_angle_onfo_b16);
	if (ret < 0) {
		PRINTFV("ERROR: foc_angle_init_b16 failed %d!\n", ret);
		goto errout;
	}
	...
	ret = foc_velocity_init_b16(&motor->vel_div, &g_foc_velocity_odiv_b16);
	if (ret < 0) {
		PRINTFV("ERROR: foc_angle_init_b16 failed %d!\n", ret);
		goto errout;
	}
	...
	ret = foc_velocity_init_b16(&motor->vel_pll, &g_foc_velocity_opll_b16);
	if (ret < 0) {
		PRINTFV("ERROR: foc_angle_init_b16 failed %d!\n", ret);
		goto errout;
	}
	...
	pi_controller_init_b16(&motor->vel_pi, ftob16(motor->envp->cfg->vel_pi_kp / 1000000.0f),
	...
	align_cfg.offset_steps =
		(CONFIG_EXAMPLES_FOC_NOTIFIER_FREQ * CONFIG_EXAMPLES_FOC_ALIGN_SEC / 1000);
	...
	ident_cfg.res_current = ftob16(motor->envp->cfg->ident_res_curr / 1000.0f);
	ident_cfg.res_ki = ftob16(motor->envp->cfg->ident_res_ki / 1000.0f);
	ident_cfg.ind_volt = ftob16(motor->envp->cfg->ident_ind_volt / 1000.0f);
	ident_cfg.res_steps =
		(CONFIG_EXAMPLES_FOC_NOTIFIER_FREQ * motor->envp->cfg->ident_res_sec / 1000);
	ident_cfg.ind_steps =
		(CONFIG_EXAMPLES_FOC_NOTIFIER_FREQ * motor->envp->cfg->ident_ind_sec / 1000);
	...
	defined(CONFIG_EXAMPLES_FOC_HAVE_ALIGN) || defined(CONFIG_EXAMPLES_FOC_HAVE_IDENT)
errout:
#endif
	return ret;
}
...
int foc_motor_handle(FAR struct foc_motor_b16_s *motor, FAR struct foc_mq_s *handle)
{
	int ret = OK;
