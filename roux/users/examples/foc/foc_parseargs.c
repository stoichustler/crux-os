...
static struct option g_long_options[] = {{"time", required_argument, 0, 't'},
...
void parse_args(FAR struct args_s *args, int argc, FAR char **argv)
{
	...
	while (1) {
		c = getopt_long(argc, argv, "ht:f:m:o:r:v:a:d:x:s:j:", g_long_options, &option_index);
...
int validate_args(FAR struct args_s *args)
{
	...
	if (args->cfg.fmode != FOC_FMODE_IDLE && args->cfg.fmode != FOC_FMODE_VOLTAGE &&
	...
	if (args->state != FOC_EXAMPLE_STATE_FREE && args->state != FOC_EXAMPLE_STATE_STOP &&
		args->state != FOC_EXAMPLE_STATE_CW && args->state != FOC_EXAMPLE_STATE_CCW) {
