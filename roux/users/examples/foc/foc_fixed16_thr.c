...
static int foc_handler_run(FAR struct foc_motor_b16_s *motor, FAR struct foc_device_s *dev)
{
	...
	foc_handler_state_b16(&motor->handler, &motor->foc_state, &motor->mod_state);
...
static int foc_model_state_get(FAR struct foc_motor_b16_s *motor, FAR struct foc_device_s *dev)
{
	int i = 0;
...
static void foc_fixed16_nxscope(FAR struct foc_nxscope_s *nxs, FAR struct foc_motor_b16_s *motor,
...
int foc_fixed16_thr(FAR struct foc_ctrl_env_s *envp)
{
	...
	while (motor.mq.quit == false) {
		...
		foc_model_run_b16(&motor.model, ftob16(FOC_MODEL_LOAD), &motor.foc_state.vab);
#endif
