...
#if defined(CONFIG_EXAMPLES_FOC_HAVE_ADC) || defined(CONFIG_EXAMPLES_FOC_HAVE_BUTTON) ||           \
...
static const int g_state_list[5] = {FOC_EXAMPLE_STATE_FREE, FOC_EXAMPLE_STATE_CW,
									FOC_EXAMPLE_STATE_STOP, FOC_EXAMPLE_STATE_CCW, 0};
#endif
...
static int foc_button_init(FAR struct foc_intf_btn_s *intf)
{
	...
	intf->fd = open(CONFIG_EXAMPLES_FOC_BUTTON_DEVPATH, (O_RDONLY | O_NONBLOCK));
	if (intf->fd < 0) {
...
static int foc_button_update(FAR struct foc_intf_btn_s *intf, FAR struct foc_intf_data_s *data)
{
...
static int foc_adc_update(FAR struct foc_intf_adc_s *intf, FAR struct foc_intf_data_s *data)
{
	...
	if (intf->adc_trigger == true) {
		...
		ret = read(intf->fd, adc_sample, (ADC_SAMPLES * sizeof(struct adc_msg_s)));
		if (ret < 0) {
			if (errno != EAGAIN) {
...
static int foc_charctrl_update(FAR struct foc_intf_chc_s *intf, FAR struct foc_intf_data_s *data)
{
	char c = 0;
	int ret = OK;
