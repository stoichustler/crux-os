...
static void rmtchar_devpath(FAR struct rmtchar_state_s *rmtchar, FAR const char *devpath,
							bool is_tx)
{
...
static void rmtchar_help(FAR struct rmtchar_state_s *rmtchar)
{
	...
	printf("  [-i items] selects the number of words (items) to be transmitted"
		   ...
		   rmtchar->rmtchar_items ? rmtchar->rmtchar_items : CONFIG_EXAMPLES_RMTCHAR_ITEMS);
#ifdef CONFIG_EXAMPLES_RMTCHAR_TX
	printf("  [-t devpath] selects the RMT transmitter character device path. "
		   "Default: %s Current: %s\n",
		   CONFIG_EXAMPLES_RMTCHAR_TX_DEVPATH, rmtchar->txdevpath ? rmtchar->txdevpath : "NONE");
#endif
#ifdef CONFIG_EXAMPLES_RMTCHAR_RX
	printf("  [-r devpath] selects the RMT receiver character device path. "
		   "Default: %s Current: %s\n",
		   CONFIG_EXAMPLES_RMTCHAR_RX_DEVPATH, rmtchar->rxdevpath ? rmtchar->rxdevpath : "NONE");
#endif
...
static void parse_args(FAR struct rmtchar_state_s *rmtchar, int argc, FAR char **argv)
{
	FAR char *ptr;
	FAR char *str;
	int value;
