...
#define NSECTORS(b)                                                                                \
	(((b) + CONFIG_EXAMPLES_UNIONFS_SECTORSIZE - 1) / CONFIG_EXAMPLES_UNIONFS_SECTORSIZE)
...
int main(int argc, FAR char *argv[])
{
	...
	ret = mount(MOUNT_DEVNAME_A, CONFIG_EXAMPLES_UNIONFS_TMPA, "romfs", MS_RDONLY, NULL);
	if (ret < 0) {
	...
	ret = mount(MOUNT_DEVNAME_B, CONFIG_EXAMPLES_UNIONFS_TMPB, "romfs", MS_RDONLY, NULL);
	if (ret < 0) {
	...
	ret = mount(NULL, CONFIG_EXAMPLES_UNIONFS_MOUNTPT, "unionfs", 0,
				"fspath1=" CONFIG_EXAMPLES_UNIONFS_TMPA ",fspath2=" CONFIG_EXAMPLES_UNIONFS_TMPB
				",prefix2=offset");
	if (ret < 0) {
