...
static struct bt_uuid_s g_sensor_uuid = {.type = BT_UUID_128,
										 .u.u128 = {0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0,
													0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde,
													0xf0}};

static struct bt_uuid_s g_temp_uuid = {
	.type = BT_UUID_16, .u.u16 = 0x2a6e, /* Temperature UUID */
};

static struct bt_uuid_s g_hum_uuid = {
	.type = BT_UUID_16, .u.u16 = 0x2a6f, /* Humidity UUID */
};

static struct bt_uuid_s g_press_uuid = {
	.type = BT_UUID_16, .u.u16 = 0x2a6d, /* Pressure UUID */
};
...
static int read_name(FAR struct bt_conn_s *conn, FAR const struct bt_gatt_attr_s *attr,
...
static int read_appearance(FAR struct bt_conn_s *conn, FAR const struct bt_gatt_attr_s *attr,
						   FAR void *buf, uint8_t len, uint16_t offset)
{
...
static int read_temperature(FAR struct bt_conn_s *conn, FAR const struct bt_gatt_attr_s *attr,
							FAR void *buf, uint8_t len, uint16_t offset)
{
...
static int read_humidity(FAR struct bt_conn_s *conn, FAR const struct bt_gatt_attr_s *attr,
						 FAR void *buf, uint8_t len, uint16_t offset)
{
...
static int read_gas(FAR struct bt_conn_s *conn, FAR const struct bt_gatt_attr_s *attr,
					FAR void *buf, uint8_t len, uint16_t offset)
{
...
static int read_pressure(FAR struct bt_conn_s *conn, FAR const struct bt_gatt_attr_s *attr,
						 FAR void *buf, uint8_t len, uint16_t offset)
{
...
static const struct bt_gatt_attr_s attrs[] = {
	...
	BT_GATT_DESCRIPTOR(NAME_DSC, &g_device_name_uuid, BT_GATT_PERM_READ, read_name, NULL, NULL),
	BT_GATT_CHARACTERISTIC(APPEARANCE_CHR, &g_appearance_chrc),
	BT_GATT_DESCRIPTOR(APPEARANCE_DSC, &g_appearance_uuid, BT_GATT_PERM_READ, read_appearance, NULL,
					   NULL),
...
static void start_scanning(void)
{
	...
	int ret = ioctl(sockfd, SIOCBTSCANSTART, (unsigned long)((uintptr_t)&btreq));
	if (ret < 0) {
