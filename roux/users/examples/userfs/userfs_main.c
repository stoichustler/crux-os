...
static int ufstest_open(FAR void *volinfo, FAR const char *relpath, int oflags, mode_t mode,
						FAR void **openinfo);
static int ufstest_close(FAR void *volinfo, FAR void *openinfo);
static ssize_t ufstest_read(FAR void *volinfo, FAR void *openinfo, FAR char *buffer, size_t buflen);
static ssize_t ufstest_write(FAR void *volinfo, FAR void *openinfo, FAR const char *buffer,
							 size_t buflen);
static off_t ufstest_seek(FAR void *volinfo, FAR void *openinfo, off_t offset, int whence);
static int ufstest_ioctl(FAR void *volinfo, FAR void *openinfo, int cmd, unsigned long arg);
static int ufstest_sync(FAR void *volinfo, FAR void *openinfo);
static int ufstest_dup(FAR void *volinfo, FAR void *oldinfo, FAR void **newinfo);
static int ufstest_fstat(FAR void *volinfo, FAR void *openinfo, FAR struct stat *buf);
static int ufstest_truncate(FAR void *volinfo, FAR void *openinfo, off_t length);
static int ufstest_opendir(FAR void *volinfo, FAR const char *relpath, FAR void **dir);
static int ufstest_closedir(FAR void *volinfo, FAR void *dir);
static int ufstest_readdir(FAR void *volinfo, FAR void *dir, FAR struct dirent *entry);
...
static int ufstest_mkdir(FAR void *volinfo, FAR const char *relpath, mode_t mode);
...
static int ufstest_stat(FAR void *volinfo, FAR const char *relpath, FAR struct stat *buf);
static int ufstest_destroy(FAR void *volinfo);
static int ufstest_fchstat(FAR void *volinfo, FAR void *openinfo, FAR const struct stat *buf,
						   int flags);
static int ufstest_chstat(FAR void *volinfo, FAR const char *relpath, FAR const struct stat *buf,
						  int flags);
...
static struct ufstest_file_s g_rootdir[UFSTEST_NFILES] = {
	{{DTYPE_FILE, "File1"}, UFSTEST_INIT_FILE1_SIZE, UFSTEST_FILE1_MXSIZE, g_file1_data},
	{{DTYPE_FILE, "File2"}, UFSTEST_INIT_FILE2_SIZE, UFSTEST_FILE2_MXSIZE, g_file2_data},
	{{DTYPE_FILE, "File3"}, UFSTEST_INIT_FILE3_SIZE, UFSTEST_FILE3_MXSIZE, g_file3_data}};

static const struct userfs_operations_s g_ufstest_ops = {
	ufstest_open,	 ufstest_close,	   ufstest_read,	ufstest_write,	   ufstest_seek,
	ufstest_ioctl,	 ufstest_sync,	   ufstest_dup,		ufstest_fstat,	   ufstest_truncate,
	ufstest_opendir, ufstest_closedir, ufstest_readdir, ufstest_rewinddir, ufstest_statfs,
	ufstest_unlink,	 ufstest_mkdir,	   ufstest_rmdir,	ufstest_rename,	   ufstest_stat,
	ufstest_destroy, ufstest_fchstat,  ufstest_chstat};
						...
						FAR void **openinfo)
{
	...
	if (file != NULL) {
		opriv = (FAR struct ufstest_openfile_s *)malloc(sizeof(struct ufstest_openfile_s));
		if (opriv == NULL) {
			return -ENOMEM;
		}
...
static int ufstest_close(FAR void *volinfo, FAR void *openinfo)
{
	FAR struct ufstest_openfile_s *opriv = (FAR struct ufstest_openfile_s *)openinfo;
...
static int ufstest_dup(FAR void *volinfo, FAR void *oldinfo, FAR void **newinfo)
{
	FAR struct ufstest_openfile_s *opriv = (FAR struct ufstest_openfile_s *)oldinfo;
...
static int ufstest_fstat(FAR void *volinfo, FAR void *openinfo, FAR struct stat *buf)
{
	...
	buf->st_blocks = (opriv->file->inuse + UFSTEST_FS_BLOCKSIZE - 1) / UFSTEST_FS_BLOCKSIZE;
	buf->st_atime = 0;
	buf->st_mtime = 0;
	buf->st_ctime = 0;
	return OK;
...
static int ufstest_opendir(FAR void *volinfo, FAR const char *relpath, FAR void **dir)
{
	...
	if (!relpath || relpath[0] == '\0') {
		...
		odir = (FAR struct ufstest_opendir_s *)malloc(sizeof(struct ufstest_opendir_s));
		if (odir == NULL) {
			return -ENOMEM;
		}
...
static int ufstest_statfs(FAR void *volinfo, FAR struct statfs *buf)
{
	...
	for (i = 0; i < UFSTEST_NFILES; i++) {
		inuse += (g_rootdir[i].inuse + UFSTEST_FS_BLOCKSIZE - 1) / UFSTEST_FS_BLOCKSIZE;
	}
...
static int ufstest_rename(FAR void *volinfo, FAR const char *oldrelpath, FAR const char *newrelpath)
{
	FAR struct ufstest_file_s *file;
...
static int ufstest_stat(FAR void *volinfo, FAR const char *relpath, FAR struct stat *buf)
{
	...
	if (*relpath == '\0') {
		...
		buf->st_mode = (S_IFDIR | S_IRWXU | S_IRUSR | S_IRGRP | S_IRWXG | S_IROTH | S_IRWXO);
		buf->st_blksize = UFSTEST_FS_BLOCKSIZE;
...
int ufstest_daemon(int argc, char *argv[])
{
	...
	ret = userfs_run(UFSTEST_MOUNTPOUNT, &g_ufstest_ops, g_rootdir, UFSTEST_MXWRITE);
...
int main(int argc, FAR char *argv[])
{
	...
	nshargv[0] = NULL;
	pid = task_create("UserFS", CONFIG_EXAMPLES_USERFS_PRIORITY, CONFIG_EXAMPLES_USERFS_STACKSIZE,
					  ufstest_daemon, (FAR char *const *)nshargv);
	if (pid < 0) {
		return EXIT_FAILURE;
	}
