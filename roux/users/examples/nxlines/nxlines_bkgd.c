...
static void nxlines_redraw(NXWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool morem,
						   FAR void *arg);
...
static void nxlines_mousein(NXWINDOW hwnd, FAR const struct nxgl_point_s *pos, uint8_t buttons,
							FAR void *arg);
#endif

#ifdef CONFIG_NX_KBD
static void nxlines_kbdin(NXWINDOW hwnd, uint8_t nch, FAR const uint8_t *ch, FAR void *arg);
#endif
...
static void nxlines_redraw(NXWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool more,
						   FAR void *arg)
{
...
void nxlines_test(NXWINDOW hwnd)
{
	...
	ret =
		nx_drawcircle((NXWINDOW)hwnd, &center, radius, CONFIG_EXAMPLES_NXLINES_BORDERWIDTH, color);
	if (ret < 0) {
		printf("nxlines_test: nx_fillcircle failed: %d\n", ret);
	}
