...
static void dac_devpath(FAR struct dac_state_s *dac, FAR const char *devpath)
{
	if (dac->devpath) {
		free(dac->devpath);
	}
...
static void print_cmds(FAR const char *header, FAR const struct command *cmds, size_t ncmds,
					   FAR const char *trailer)
{
	printf("%s", header);
	while (ncmds--) {
		printf("  %s %s %c", cmds->name, cmds->args, (ncmds > 0) ? '\n' : ' ');
		cmds++;
	}
...
static const struct command *find_cmd(FAR const char *name, FAR const struct command *cmds,
									  size_t ncmds)
{
	while (ncmds--) {
...
static int execute_cmd(int argc, FAR const char *argv[], FAR const struct command *cmds,
					   size_t ncmds)
{
...
static int dac_put(FAR const char *devpath, FAR struct dac_msg_s *msg, size_t nmsgs, int delay)
{
	size_t retries;
	size_t i;
	int fd;
...
static int parse_args(FAR struct dac_state_s *dac, int argc, const char *argv[])
{
