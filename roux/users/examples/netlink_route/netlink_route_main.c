...
static void dump_neighbor(void)
{
	...
	allocsize = CONFIG_NET_IPv6_NCONF_ENTRIES * sizeof(struct neighbor_entry_s);
	...
	for (i = 0; i < nentries; i++) {
		...
		printf("  Dest: %s MAC Type: %2u Size: %3u Addr: ", buffer, nb->ne_addr.na_lltype,
			   nb->ne_addr.na_llsize);
