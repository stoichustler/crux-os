...
static int timer_gpio_daemon(int argc, char *argv[])
{
	...
	ret = ioctl(fd_timer, TCIOC_SETTIMEOUT, CONFIG_EXAMPLES_TIMER_GPIO_INTERVAL);
	if (ret < 0) {
		int errcode = errno;
...
int main(int argc, FAR char *argv[])
{
	...
	strlcpy(g_devtim, CONFIG_EXAMPLES_TIMER_GPIO_TIM_DEVNAME, sizeof(g_devtim));
	strlcpy(g_devgpio, CONFIG_EXAMPLES_TIMER_GPIO_GPIO_DEVNAME, sizeof(g_devgpio));
	...
	ret = task_create("timer_gpio_daemon", CONFIG_EXAMPLES_TIMER_GPIO_PRIORITY,
					  CONFIG_EXAMPLES_TIMER_GPIO_STACKSIZE, timer_gpio_daemon, NULL);
	if (ret < 0) {
		int errcode = errno;
