...
static int header_callback(FAR const char *line, bool truncated, FAR void *arg)
{
...
static int sink_callback(FAR char **buffer, int offset, int datend, FAR int *buflen, FAR void *arg)
{
	FAR struct download_context_s *ctx = (FAR struct download_context_s *)arg;
	...
	if (length > 0) {
		...
		flash_area_write(ctx->fa, ctx->fa_offset, &((*buffer)[offset]), length);

		ctx->fa_offset += length;

#ifdef CONFIG_EXAMPLES_MCUBOOT_UPDATE_AGENT_DL_VERIFY_MD5
		md5_update(&ctx->md5_ctx, (FAR const unsigned char *)&((*buffer)[offset]), length);
#endif

		progress = (ctx->fa_offset * 100) / ctx->image_size;

		printf("Received: %-8" PRIu32 " of %zd bytes [%" PRIu32 "%%]\n", ctx->fa_offset,
			   ctx->image_size, progress);
	}
