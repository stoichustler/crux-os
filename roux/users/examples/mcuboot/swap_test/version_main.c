...
static int boot_read_v1_header(uint8_t area_id, struct mcuboot_raw_header_v1_s *raw_header)
{
	...
	rc = flash_area_read(fa, 0, raw_header, sizeof(struct mcuboot_raw_header_v1_s));
	flash_area_close(fa);
	if (rc) {
		return rc;
	}
...
int main(int argc, FAR char *argv[])
{
	...
	printf("Image version %d.%d.%d.%" PRId32 "\n", raw_header.version.major,
		   raw_header.version.minor, raw_header.version.revision, raw_header.version.build_num);
