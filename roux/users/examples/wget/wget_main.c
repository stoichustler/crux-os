...
static int callback(FAR char **buffer, int offset, int datend, FAR int *buflen, FAR void *arg)
{
...
int main(int argc, FAR char *argv[])
{
	...
	if (argc >= 3) {
		ctx.url = argv[argc - 1];
		g_fd = open(argv[argc - 2], O_WRONLY | O_CREAT | O_TRUNC, S_IRWXU | S_IRWXG | S_IRWXO);
		if (g_fd < 0) {
