...
static const char g_wndomsg1[] = "ROUX is cool!";
#endif
#if CONFIG_EXAMPLES_PWFB_NWINDOWS > 1
static const char g_wndomsg2[] = "ROUX is fun!";
#endif
#if CONFIG_EXAMPLES_PWFB_NWINDOWS > 2
static const char g_wndomsg3[] = "ROUX is groovy!";
#endif
...
static bool pwfb_server_initialize(FAR struct pwfb_state_s *st)
{
	...
	if (st->hnx) {
		...
		struct boardioc_vncstart_s vnc = {0, st->hnx};
...
static bool pwfb_state_initialize(FAR struct pwfb_state_s *st)
{
	...
	st->wndo[0].fcache =
		nxf_cache_connect(CONFIG_EXAMPLES_PWFB_FONTID, CONFIG_EXAMPLES_PWFB_FONTCOLOR,
						  CONFIG_EXAMPLES_PWFB_COLOR1, CONFIG_EXAMPLES_PWFB_BPP, 8);
	...
	st->wndo[1].fcache =
		nxf_cache_connect(CONFIG_EXAMPLES_PWFB_FONTID, CONFIG_EXAMPLES_PWFB_FONTCOLOR,
						  CONFIG_EXAMPLES_PWFB_COLOR2, CONFIG_EXAMPLES_PWFB_BPP, 8);
	...
	st->wndo[2].fcache =
		nxf_cache_connect(CONFIG_EXAMPLES_PWFB_FONTID, CONFIG_EXAMPLES_PWFB_FONTCOLOR,
						  CONFIG_EXAMPLES_PWFB_COLOR3, CONFIG_EXAMPLES_PWFB_BPP, 8);
...
static bool pwfb_putc(FAR struct pwfb_state_s *st, FAR struct pwfb_window_s *wndo,
					  FAR struct nxgl_point_s *fpos, uint8_t ch)
{
	...
	src = glyph->bitmap;
	ret = nxtk_bitmapwindow(wndo->hwnd, &bounds, &src, fpos, (unsigned int)glyph->stride);
	if (ret < 0) {
...
static bool pwfb_configure_window(FAR struct pwfb_state_s *st, int wndx,
								  FAR struct nxgl_size_s *size, FAR struct nxgl_point_s *pos,
								  FAR const char *text, double deltax, double deltay)
{
...
#ifdef CONFIG_NX_SWCURSOR
static bool pwfb_configure_cursor(FAR struct pwfb_state_s *st, FAR struct nxgl_point_s *pos,
...
int main(int argc, FAR char *argv[])
{
	...
	wstate.wndo[0].hwnd =
		nxtk_openwindow(wstate.hnx, NXBE_WINDOW_RAMBACKED, &g_pwfb_wncb, (FAR void *)&wstate);
