...
static void parsearg(int argc, FAR char *argv[], FAR struct mqttc_cfg_s *cfg, FAR int *n);
static int init_conn(FAR const struct mqttc_cfg_s *cfg, FAR struct mqtt_conn_context_s *ctx,
...
static int init_conn(FAR const struct mqttc_cfg_s *cfg, FAR struct mqtt_conn_context_s *conn,
					 FAR mqtt_pal_socket_handle *socketfd)
{
	...
	ret =
		mbedtls_ctr_drbg_seed(ctr_drbg, mbedtls_entropy_func, entropy, additional, additional_len);
	if (ret != 0) {
	...
	if (cfg->ca_file != NULL) {
		...
		ret = mbedtls_x509_crt_parse(ca_crt, (FAR const unsigned char *)TEST_CA_CRT_RSA_SHA256_PEM,
	...
	ret = mbedtls_ssl_config_defaults(ssl_conf, MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM,
	...
	ret = mbedtls_net_connect(net_ctx, cfg->host, cfg->port, MBEDTLS_NET_PROTO_TCP);
	if (ret != 0) {
...
int main(int argc, FAR char *argv[])
{
	...
	pthread_t thrdid;
	int n = 1;
	struct mqttc_cfg_s mqtt_cfg = {.host = "broker.hivemq.com",
#ifndef MQTT_USE_MBEDTLS
								   .port = "1883",
#else
								   .port = "8883",
	...
	mqtterr = mqtt_init(&mqtt_conn.client, socketfd, mqtt_conn.sendbuf, sizeof(mqtt_conn.sendbuf),
						mqtt_conn.recvbuf, sizeof(mqtt_conn.recvbuf), NULL);
	if (mqtterr != MQTT_OK) {
		printf("ERRPR! mqtt_init() failed.\n");
		goto err_with_conn;
	}

	mqtterr = mqtt_connect(&mqtt_conn.client, mqtt_cfg.id, NULL, /* Will topic */
						   NULL,								 /* Will message */
						   0,									 /* Will message size */
	...
	while (n--) {
		mqtterr = mqtt_publish(&mqtt_conn.client, mqtt_cfg.topic, mqtt_cfg.msg,
							   strlen(mqtt_cfg.msg), mqtt_cfg.qos);
		if (mqtterr != MQTT_OK) {
