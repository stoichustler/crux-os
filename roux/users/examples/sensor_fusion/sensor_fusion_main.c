...
void read_mpu6050(int fd, struct sensor_accel *acc_data, struct sensor_gyro *gyro_data);
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < iterations; i++) {
		...
		FusionAhrsUpdateNoMagnetometer(&ahrs, gyroscope, accelerometer, acq_period);
		euler = FusionQuaternionToEuler(FusionAhrsGetQuaternion(&ahrs));

		printf("Yaw: %.3f | Pitch: %.3f | Roll: %.3f\n", euler.angle.yaw, euler.angle.pitch,
			   euler.angle.roll);
