...
static int send_uart_lorawan_at_commands(unsigned char *ptr_at_cmd, int size_at_cmd);
static int read_uart_lorawan_resp(unsigned char *ptr_response_buffer, int size_response_buffer,
								  int time_to_wait_ms);
								  ...
								  int time_to_wait_ms)
{
	...
	do {
		bytes_read_uart = read(fd_uart, full_response + total_bytes_read_uart, 1);
	...
	if (pos_msg_downlink == NULL) {
		...
		snprintf((char *)ptr_response_buffer, size_response_buffer, "%s", pos_msg_downlink + 3);
...
void lorawan_radioenge_init(config_lorawan_radioenge_t config_lorawan)
{
	...
	bytes_written_at_cmd = send_uart_lorawan_at_commands(full_at_cmd, strlen((char *)full_at_cmd));
...
int lorawan_radioenge_send_msg(unsigned char *pt_uplink_hexstring, int size_uplink,
							   unsigned char *pt_downlink_hexstring, int max_size_downlink,
							   int time_to_wait_ms)
{
	...
	snprintf((char *)at_cmd_send_message, FULL_AT_CMD_MAX_SIZE, "AT+SENDB=5:%s\n\r",
			 pt_uplink_hexstring);
	...
	while (number_of_attempts < MAX_ATTEMPTS_TO_SEND) {
		...
		bytes_rcv_downlink_msg =
			read_uart_lorawan_resp(pt_downlink_hexstring, max_size_downlink, time_to_wait_ms);
