...
static void *thread_socket_client(void *arg)
{
	...
	while (1) {
		bytes_read_from_server = read(socket_client_fd, rx_buffer, RX_BUFFER_SIZE);
		...
		if (bytes_read_from_server == 0) {
			...
			if (tprotocol.msg_size >= 0) {
				memcpy(tx_buffer, (unsigned char *)&tprotocol, sizeof(protocolo_ipc));
				ret_send = send(socket_client_fd, tx_buffer, sizeof(protocolo_ipc),
...
int main(int argc, char *argv[])
{
	...
	snprintf(lora_config.network_session_key, NW_SESSION_KEY_SIZE,
			 "00:00:00:00:00:00:00:00:00:00:00:00:00:00:00:00");
	snprintf(lora_config.application_eui, APP_EUI_SIZE, "00:00:00:00:00:00:00:00");
	snprintf(lora_config.device_address, DEVICE_ADDRESS_SIZE, "00:00:00:00");
	snprintf(lora_config.channel_mask, CHANNEL_MASK_SIZE, "00FF:0000:0000:0000:0000:0000");
	...
	ret_bind = bind(socket_server_fd, (struct sockaddr *)&server_add, sizeof(server_add));
	if (ret_bind < 0) {
	...
	while (1) {
		...
		new_socket_client_fd =
			accept4(socket_server_fd, (struct sockaddr *)&server_storage, &addr_size, SOCK_CLOEXEC);
		...
		if (pthread_create(&tid[socket_clients_counter++], NULL, thread_socket_client,
						   &new_socket_client_fd) != 0) {
