...
void send_msg_to_lpwan(unsigned char *msg, protocolo_ipc *pt_protocol)
{
	...
	case SEND_MESSAGE:
		...
		bytes_recv =
			lorawan_radioenge_send_msg(tprotocol.msg, tprotocol.msg_size, buf_recv_downlink,
									   sizeof(buf_recv_downlink), LORAWAN_DOWNLINK_TIME_MS);
		...
		if (bytes_recv < MAX_MESSAGE_SIZE) {
			...
			snprintf((char *)pt_protocol->msg, MAX_MESSAGE_SIZE, "%s", buf_recv_downlink);
		}
		break;
