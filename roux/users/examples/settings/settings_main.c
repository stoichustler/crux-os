...
#error LIBC_TMPDIR must be defined
	...
	printf("Example of settings usage to file: %s and %s:", bin_path, text_path);
	printf("--------------------------------------------------------------\n");
	...
	if (stype == SETTING_STRING) {
		...
		ret = settings_get("v1", SETTING_INT, &testval, CONFIG_SYSTEM_SETTINGS_VALUE_SIZE);
		if (ret < 0) {
			printf("settings retrieve failed: %d\n", ret);
			goto end;
		}
	...
	printf("Changing setting to an IP value (s1) with value:%d.%d.%d.%d\n",
		   (int)(save_ip.s_addr >> 24) & 0xff, (int)(save_ip.s_addr >> 16) & 0xff,
		   (int)(save_ip.s_addr >> 8) & 0xff, (int)(save_ip.s_addr >> 0) & 0xff);
