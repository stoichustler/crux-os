...
void tcpblaster_server(void)
{
	...
	optval = 1;
	if (setsockopt(listensd, SOL_SOCKET, SO_REUSEADDR, (FAR void *)&optval, sizeof(int)) < 0) {
	...
	printf("Binding to IPv6 Address: "
		   ...
		   "%02x%02x:%02x%02x:%02x%02x:%02x%02x\n",
		   myaddr.sin6_addr.s6_addr[0], myaddr.sin6_addr.s6_addr[1], myaddr.sin6_addr.s6_addr[2],
		   myaddr.sin6_addr.s6_addr[3], myaddr.sin6_addr.s6_addr[4], myaddr.sin6_addr.s6_addr[5],
		   myaddr.sin6_addr.s6_addr[6], myaddr.sin6_addr.s6_addr[7], myaddr.sin6_addr.s6_addr[8],
		   myaddr.sin6_addr.s6_addr[9], myaddr.sin6_addr.s6_addr[10], myaddr.sin6_addr.s6_addr[11],
		   myaddr.sin6_addr.s6_addr[12], myaddr.sin6_addr.s6_addr[13], myaddr.sin6_addr.s6_addr[14],
		   myaddr.sin6_addr.s6_addr[15]);
#else
	...
	for (;;) {
		...
		if (++recvcount >= GROUPSIZE) {
			...
			strftime(timebuff, 100, "%Y-%m-%d %H:%M:%S.000", localtime(&curr.tv_sec));
			...
			printf("[%s] %d: Received %d buffers: %7.1f KB (buffer average"
				   ...
				   timebuff, groupcount, recvcount, fkbsent, fkbsent / recvcount, felapsed,
				   fkbsent / felapsed);
