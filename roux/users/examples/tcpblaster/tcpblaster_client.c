...
void tcpblaster_client(void)
{
	...
	printf("Connecting to IPv6 Address: "
		   ...
		   g_tcpblasterserver_ipv6[0], g_tcpblasterserver_ipv6[1], g_tcpblasterserver_ipv6[2],
		   g_tcpblasterserver_ipv6[3], g_tcpblasterserver_ipv6[4], g_tcpblasterserver_ipv6[5],
	...
	for (;;) {
		...
		if (++sendcount >= GROUPSIZE) {
			...
			printf("[%s] %d: Sent %d %d-byte buffers: %7.1fKB "
				   ...
				   timebuff, groupcount, sendcount, SENDSIZE, fkbrecvd, fkbrecvd / sendcount,
				   felapsed, fkbrecvd / felapsed);
