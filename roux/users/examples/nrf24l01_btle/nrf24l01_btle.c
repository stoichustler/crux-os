...
static uint8_t mac[6] = {0x79, 0x6a, 0x64, 0x77, 0x62, 0x6a};
...
const uint8_t adve_name[5] = {'n', 'R', 'F', '2', '4'};
...
int nrf24_cfg(int fd)
{
	...
	nrf24l01_retrcfg_t retrcfg = {.count = 0, .delay = DELAY_1000us};
	...
	nrf24l01_pipecfg_t pipe0cfg = {
		.en_aa = false,
		.payload_length = 32,
		.rx_addr = {swapbits(0x8e), swapbits(0x89), swapbits(0xbe), swapbits(0xd6)}};
	...
	ioctl(fd, NRF24L01IOC_SETTXADDR, (unsigned long)((uint8_t *)&pipe0cfg.rx_addr));
...
static void adv_packet(int wl_fd)
{
	...
	ioctl(wl_fd, WLIOC_SETRADIOFREQ, (unsigned long)((uint32_t *)&frequency[current]));
