...
static void *null_reader(pthread_addr_t pvarg)
{
	...
	if (fd < 0) {
		fprintf(stderr, "null_reader: Failed to open FIFO %s for reading, errno=%d\n", FIFO_PATH2,
				errno);
...
int interlock_test(void)
{
	...
	if (nbytes < 0) {
		fprintf(stderr, "interlock_test: write failed, errno=%d\n", errno);
		ret = 10;
		goto errout_with_null_reader_thread;
	} else if (ret != 0) {
		fprintf(stderr, "interlock_test: Wrote %ld bytes of data -- aborting: %d\n", (long)nbytes,
				errno);
		ret = 11;
		goto errout_with_null_reader_thread;
	}
...
errout_with_fifo:
	...
	if (ret != 0) {
		fprintf(stderr, "interlock_test: remove failed with errno=%d\n", errno);
	}
