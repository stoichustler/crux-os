	...
	for (i = 0; i < NWRITES; i++) {
		for (nbytes = 0; nbytes < WRITE_SIZE;) {
			...
			if (ret < 0) {
				...
				fprintf(stderr, "transfer_writer: Unexpected zero write size\n");
				return (void *)(uintptr_t)2;
			}
...
int transfer_test(int fdin, int fdout, int boost_reader, int boost_writer)
{
	...
	ret = pthread_create(&readerid, &rattr, transfer_reader, (void *)(intptr_t)fdin);
	...
	ret = pthread_create(&writerid, &wattr, transfer_writer, (void *)(intptr_t)fdout);
