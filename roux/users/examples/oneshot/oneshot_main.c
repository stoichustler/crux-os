...
static void show_usage(FAR const char *progname)
{
	...
	fprintf(stderr,
			"\tSpecifies the oneshot delay in microseconds."
			" Default %ld\n",
...
int main(int argc, FAR char *argv[])
{
	...
	maxus = (uint64_t)ts.tv_sec * USEC_PER_SEC + (uint64_t)ts.tv_nsec / NSEC_PER_USEC;
	...
	while (usecs > 0) {
		...
		if (usecs < maxus) {
			...
			printf("Starting oneshot timer with delay %" PRId64 " microseconds\n", maxus);
