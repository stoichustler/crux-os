...
#define USRSOCKTEST_DAEMON_CONF_DEFAULTS                                                           \
	{                                                                                              \
		.max_sockets = UINT_MAX, .supported_domain = AF_INET, .supported_type = SOCK_STREAM,       \
		.supported_protocol = 0, .delay_all_responses = false, .endpoint_addr = "127.0.0.1",       \
		.endpoint_port = 255, .endpoint_block_connect = false, .endpoint_block_send = false,       \
		.endpoint_recv_avail_from_start = true, .endpoint_recv_avail = 4,                          \
	}

/* Test case macros */

#define utest_match(type, should_be_value, test_value)                                             \
	do {                                                                                           \
		...
		if (!usrsocktest_assert_print_value(__func__, __LINE__,                                    \
											...
											tmp_utest_test_value, tmp_utest_should_value)) {       \
			usrsocktest_test_failed = true;                                                        \
			return;                                                                                \
		}                                                                                          \
	} while (false)

#define utest_match_buf(should_be_valuebuf, test_valuebuf, test_valuebuflen)                       \
	do {                                                                                           \
		if (!usrsocktest_assert_print_buf(__func__, __LINE__,                                      \
										  "memcmp(" #should_be_valuebuf ", " #test_valuebuf        \
										  ", " #test_valuebuflen ") == 0",                         \
										  should_be_valuebuf, test_valuebuf, test_valuebuflen)) {  \
			usrsocktest_test_failed = true;                                                        \
			return;                                                                                \
		}                                                                                          \
	} while (false)

#define RUN_TEST_CASE(g, t)                                                                        \
	do {                                                                                           \
...
#define TEST_GROUP(g)                                                                              \
	const char *usrsocktest_group_##g = #g;                                                        \
	void usrsocktest_group_##g##_run(void)
...
bool usrsocktest_assert_print_value(FAR const char *func, const int line, FAR const char *check_str,
									long int test_value, long int should_be);

bool usrsocktest_assert_print_buf(FAR const char *func, const int line, FAR const char *check_str,
								  FAR const void *test_buf, FAR const void *expect_buf,
								  size_t buflen);
