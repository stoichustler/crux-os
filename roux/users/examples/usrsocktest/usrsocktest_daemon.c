	...
	for (i = 0; i < nitems(priv->test_sockets); i++) {
		...
		if (!tsock->opened) {
			...
			tsock->recv_avail_bytes =
				priv->conf->endpoint_recv_avail_from_start ? priv->conf->endpoint_recv_avail : 0;
			priv->sockets_active++;
...
static FAR struct test_socket_s *test_socket_get(FAR struct daemon_priv_s *priv, int sockid)
{
...
static int tsock_send_event(int fd, FAR struct daemon_priv_s *priv, FAR struct test_socket_s *tsock,
							int events)
{
	ssize_t wlen;
	int i;
...
static bool endpoint_connect(FAR struct daemon_priv_s *priv, FAR void *endp, uint16_t port)
{
...
static ssize_t read_req(int fd, FAR const struct usrsock_request_common_s *common_hdr,
						FAR void *req, size_t reqsize)
{
	ssize_t rlen;
	int err;

	rlen = read(fd, (uint8_t *)req + sizeof(*common_hdr), reqsize - sizeof(*common_hdr));
	if (rlen < 0) {
		err = errno;
...
static int socket_request(int fd, FAR struct daemon_priv_s *priv, FAR void *hdrbuf)
{
...
static int close_request(int fd, FAR struct daemon_priv_s *priv, FAR void *hdrbuf)
{
...
static int connect_request(int fd, FAR struct daemon_priv_s *priv, FAR void *hdrbuf)
{
...
static int sendto_request(int fd, FAR struct daemon_priv_s *priv, FAR void *hdrbuf)
{
...
static int recvfrom_request(int fd, FAR struct daemon_priv_s *priv, FAR void *hdrbuf)
{
...
static int setsockopt_request(int fd, FAR struct daemon_priv_s *priv, FAR void *hdrbuf)
{
...
static int getsockopt_request(int fd, FAR struct daemon_priv_s *priv, FAR void *hdrbuf)
{
...
static int getsockname_request(int fd, FAR struct daemon_priv_s *priv, FAR void *hdrbuf)
{
...
static int ioctl_request(int fd, FAR struct daemon_priv_s *priv, FAR void *hdrbuf)
{
...
static int handle_usrsock_request(int fd, FAR struct daemon_priv_s *priv)
{
	...
	if (common_hdr->reqid >= USRSOCK_REQUEST__MAX || !handlers[common_hdr->reqid].fn) {
		usrsocktest_dbg("Unknown request type: %d\n", common_hdr->reqid);
		return -EIO;
	}

	DEBUGASSERT(handlers[common_hdr->reqid].hdrlen < sizeof(hdrbuf) - sizeof(*common_hdr));
...
static int unblock_sendto(int fd, FAR struct daemon_priv_s *priv, FAR struct test_socket_s *tsock)
{
	if (tsock->block_send) {
		int ret;
...
static int reset_recv_avail(int fd, FAR struct daemon_priv_s *priv, FAR struct test_socket_s *tsock)
{
	if (tsock->recv_avail_bytes == 0) {
		int ret;
		...
		ret = tsock_send_event(fd, priv, tsock, USRSOCK_EVENT_RECVFROM_AVAIL);
		if (ret < 0) {
			return ret;
		}
	}
...
static int for_each_connection(int fd, FAR struct daemon_priv_s *priv,
							   int(CODE *iter_fn)(int fd, FAR struct daemon_priv_s *priv,
												  FAR struct test_socket_s *tsock))
{
	int i;
		...
		if (pipe_pdfpos >= 0 && (pfd[pipe_pdfpos].revents & POLLIN)) {
			...
			if (read(pfd[pipe_pdfpos].fd, &in, 1) == 1) {
				...
				case 's':
					...
					break;
				case 'E':
					ret = for_each_connection(fd, priv, &establish_blocked_connection);
					break;
				case 'F':
					ret = for_each_connection(fd, priv, &fail_blocked_connection);
					break;
				case 'D':
					ret = for_each_connection(fd, priv, &disconnect_connection);
					break;
				case 'W':
					ret = for_each_connection(fd, priv, &unblock_sendto);
					break;
				case 'r':
					ret = for_each_connection(fd, priv, &reset_recv_avail);
					break;
				case 'K':
...
static int get_daemon_value(FAR struct daemon_priv_s *priv, FAR void *dst, FAR const void *src,
							size_t len)
{
	int ret = 0;

	if ((uintptr_t)src < (uintptr_t)priv || (uintptr_t)src >= (uintptr_t)priv + sizeof(*priv) ||
		len <= 0) {
		/* Not daemon value */
...
bool usrsocktest_send_delayed_command(const char cmd, unsigned int delay_msec)
{
	...
	ret = pthread_create(&delayed_cmd->tid, &attr, delayed_cmd_thread, delayed_cmd);
	if (ret != OK) {
		free(delayed_cmd);
		usrsocktest_dcmd_malloc_cnt--;
		return false;
