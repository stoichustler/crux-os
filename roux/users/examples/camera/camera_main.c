...
static int camera_prepare(int fd, enum v4l2_buf_type type, uint32_t buf_mode, uint32_t pixformat,
						  uint16_t hsize, uint16_t vsize, FAR struct v_buffer **vbuf,
						  uint8_t buffernum, int buffersize);
static void free_buffer(FAR struct v_buffer *buffers, uint8_t bufnum);
static int parse_arguments(int argc, FAR char *argv[], FAR int *capture_num,
						   FAR enum v4l2_buf_type *type);
static int get_camimage(int fd, FAR struct v4l2_buffer *v4l2_buf, enum v4l2_buf_type buf_type);
...
int main(int argc, FAR char *argv[])
{
	...
	if (capture_num != 0) {
		...
		ret = camera_prepare(v_fd, V4L2_BUF_TYPE_STILL_CAPTURE, V4L2_BUF_MODE_FIFO,
							 V4L2_PIX_FMT_JPEG, w, h, &buffers_still, STILL_BUFNUM, IMAGE_JPG_SIZE);
		if (ret != OK) {
			goto exit_this_app;
		}
	}
	...
	ret = camera_prepare(v_fd, V4L2_BUF_TYPE_VIDEO_CAPTURE, V4L2_BUF_MODE_RING, V4L2_PIX_FMT_RGB565,
						 VIDEO_HSIZE_QVGA, VIDEO_VSIZE_QVGA, &buffers_video, VIDEO_BUFNUM,
						 IMAGE_RGB_SIZE);
	if (ret != OK) {
		goto exit_this_app;
	}
	...
	if (capture_num == 0) {
		...
		printf("Take %d pictures as %s file in %s after %d seconds.\n", capture_num,
			   capture_type == V4L2_BUF_TYPE_STILL_CAPTURE ? "JPEG" : "RGB", save_dir,
			   START_CAPTURE_TIME);
	...
	while (1) {
...
#ifdef CONFIG_EXAMPLES_CAMERA_OUTPUT_LCD
			nximage_draw((FAR void *)v4l2_buf.m.userptr, VIDEO_HSIZE_QVGA, VIDEO_VSIZE_QVGA);
#endif
		...
		case APP_STATE_UNDER_CAPTURE:
			...
			while (capture_num) {
				...
				futil_writeimage((FAR uint8_t *)v4l2_buf.m.userptr, (size_t)v4l2_buf.bytesused,
