...
static uint8_t uavcan_heap[O1_HEAP_SIZE] __attribute__((aligned(O1HEAP_ALIGNMENT)));
...
void process1hztasks(CanardInstance *ins, uint64_t timestamp_usec)
{
	...
	++my_message_transfer_id; /* The transfer-ID shall be incremented after every transmission on
				 this subject. */
...
void processtxrxonce(CanardInstance *ins, canardsocketinstance *sock_ins, int timeout_msec)
{
	...
	for (const CanardFrame *txf = NULL; (txf = canardTxPeek(ins)) != NULL;) {
		if (txf->timestamp_usec >
			getmonotonictimestampusec()) /* Check if the frame has timed out. */
		{
			if (socketcantransmit(sock_ins, txf) ==
				0) /* Send the frame. Redundant interfaces may be used here. */
			{
	...
	result = canardRxAccept(ins, &received_frame, /* The CAN frame received from the bus. */
...
static int canard_daemon(int argc, char *argv[])
{
	...
	ins.node_id =
		(pub ? CONFIG_EXAMPLES_LIBOPENCYPHAL_NODE_ID : CONFIG_EXAMPLES_LIBOPENCYPHAL_NODE_ID + 1);
	...
	(void)canardRxSubscribe(
		&ins, /* Subscribe to messages uavcan.node.Heartbeat. */
...
int opencyphal_main(int argc, FAR char *argv[])
{
	...
	ret = task_create("canard_daemon", CONFIG_EXAMPLES_LIBOPENCYPHAL_DAEMON_PRIORITY,
					  CONFIG_EXAMPLES_LIBOPENCYPHAL_DAEMON_STACK_SIZE, canard_daemon, argv);
