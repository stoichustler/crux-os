...
static void feed_sighandler(int signo, FAR siginfo_t *siginfo, FAR void *context)
{
	...
	case FEED_CMD:
		...
		if (node != NULL) {
			...
			fprintf(stderr, "watcher daemon: task is not subscribed to feed dog.\n");
		}
...
static void wdt_log(int signo, FAR siginfo_t *siginfo, FAR void *context)
{
...
static int watcher_daemon(int argc, FAR char *argv[])
{
	...
	fprintf(fp, "%d %d %d %d %d\n", watcher_pid, CONFIG_EXAMPLES_WATCHER_SIGNAL, SUBSCRIBE_CMD,
