...
static struct wdog_params_s wdog = {.timeout = CONFIG_EXAMPLES_WATCHER_TIMEOUT,
									.handlers.oldhandler = NULL,
									.handlers.newhandler = wdt_handler};

static struct wdt_loginfo_s log_info = {.signal = CONFIG_EXAMPLES_WATCHER_SIGNAL_LOG};
...
int wdt_init(void)
{
	int fd;
	int ret;
	strlcpy(wdog.devname, CONFIG_EXAMPLES_WATCHER_DEVPATH, sizeof(wdog.devname));
	...
	ret = ioctl(fd, WDIOC_CAPTURE, (unsigned long)((uintptr_t) & (wdog.handlers)));
	if (ret < 0) {
		int errcode = errno;
