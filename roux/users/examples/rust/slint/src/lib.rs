...
impl slint::platform::Platform for ROUXPlatform {
...
impl LineBufferProvider for ROUXRenderer {
...
pub extern "C" fn slint_main() {
    ...
    // Set the platform
    slint::platform::set_platform(Box::new(ROUXPlatform {
        window: window.clone(),
    }))
    .unwrap();
    ...
    // Create the renderer for ROUX
    let nxrender = ROUXRenderer {
