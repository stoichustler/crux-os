...
static void nxdemo_redraw(NXWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool morem,
						  FAR void *arg);
...
static void nxdemo_mousein(NXWINDOW hwnd, FAR const struct nxgl_point_s *pos, uint8_t buttons,
						   FAR void *arg);
#endif

#ifdef CONFIG_NX_KBD
static void nxdemo_kbdin(NXWINDOW hwnd, uint8_t nch, FAR const uint8_t *ch, FAR void *arg);
#endif
...
static void nxdemo_redraw(NXWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool more,
						  FAR void *arg)
{
...
static void nxdemo_demo_3(NXWINDOW hwnd)
{
	...
	for (i = 0; i < g_nxdemo.yres; i++) {
		...
		color[0] = CONFIG_EXAMPLES_NXDEMO_DRAWCOLOR;
		ret = nx_drawline((NXWINDOW)hwnd, &vect, line_width, color, NX_LINECAP_NONE);
		if (ret < 0) {
...
static void nxdemo_demo_4(NXWINDOW hwnd)
{
	...
	ret = nx_bitmap((NXWINDOW)hwnd, &rect, src, &origin, 3);
	if (ret < 0) {
