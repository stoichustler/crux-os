...
static const uint16_t g_ipv6_ipaddr[8] = {
	HTONS(0xfc00), HTONS(0x0000), HTONS(0x0000), HTONS(0x0000),
	HTONS(0x0000), HTONS(0x0000), HTONS(0x0000), HTONS(0x0002),
};

static const uint16_t g_ipv6_netmask[8] = {
	HTONS(0xffff), HTONS(0xffff), HTONS(0xffff), HTONS(0xffff),
	HTONS(0xffff), HTONS(0xffff), HTONS(0x0000), HTONS(0x0000),
};

static const uint16_t g_ipv6_router[8] = {
	HTONS(0xfc00), HTONS(0x0000), HTONS(0x0000), HTONS(0x0000),
	HTONS(0x0000), HTONS(0x0000), HTONS(0x0000), HTONS(0x0001),
};
#endif
	...
	for (;;) {
		...
		/* Check for data successfully read */
			...
			while (nbyteswritten < nbytesread) {
				ssize_t n = write(1, buffer + nbyteswritten, nbytesread - nbyteswritten);
				if (n < 0) {
					ret = -errno;
...
{
	if (address != 0) {
		struct in_addr saddr = {address};

		char inetaddr[INET_ADDRSTRLEN];

		printf("%s%s\n", variable, inet_ntoa_r(saddr, inetaddr, sizeof(inetaddr)));
	}
}
#endif
...
#ifdef CONFIG_NET_IPv6
static void ipcfg_dump_ipv6addr(FAR const char *variable, FAR const struct in6_addr *address)
{
	int ret;
...
static int ipcfg_dump_ipv4config(FAR const char *netdev)
{
	...
	if (ipv4cfg.proto == IPv4PROTO_STATIC || ipv4cfg.proto == IPv4PROTO_FALLBACK) {
...
static int ipcfg_dump_ipv6config(FAR const char *netdev)
{
	...
	if (ipv6cfg.proto == IPv6PROTO_STATIC || ipv6cfg.proto == IPv6PROTO_FALLBACK) {
...
static int ipcfg_write_ipv6(FAR const char *netdev)
{
	...
	memcpy(ipv6cfg.ipaddr.s6_addr16, g_ipv6_ipaddr, sizeof(struct in6_addr));
	memcpy(ipv6cfg.netmask.s6_addr16, g_ipv6_netmask, sizeof(struct in6_addr));
	memcpy(ipv6cfg.router.s6_addr16, g_ipv6_router, sizeof(struct in6_addr));
