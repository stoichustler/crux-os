...
static const uint32_t g_rgb24[NCOLORS] = {RGB24_VIOLET, RGB24_BLUE,	  RGB24_GREEN,
										  RGB24_YELLOW, RGB24_ORANGE, RGB24_RED};

static const uint16_t g_rgb16[NCOLORS] = {RGB16_VIOLET, RGB16_BLUE,	  RGB16_GREEN,
										  RGB16_YELLOW, RGB16_ORANGE, RGB16_RED};

static const uint8_t g_rgb8[NCOLORS] = {RGB8_VIOLET, RGB8_BLUE,	  RGB8_GREEN,
...
static void sync_area(FAR struct fb_state_s *state)
{
	...
	if (state->act_fbmem == state->fbmem) {
		memcpy(state->fbmem, state->fbmem2, state->vinfo.yres * state->pinfo.stride);
	} else {
		memcpy(state->fbmem2, state->fbmem, state->vinfo.yres * state->pinfo.stride);
	}
}
...
static void pan_display(FAR struct fb_state_s *state)
{
	...
	if (ret > 0) {
		...
		ioctl(state->fd, FBIOPAN_DISPLAY, (unsigned long)(uintptr_t)&state->pinfo);

		state->act_fbmem = state->act_fbmem == state->fbmem ? state->fbmem2 : state->fbmem;
	}
}
...
static int fb_init_mem2(FAR struct fb_state_s *state)
{
	...
	if ((buf_offset % state->pinfo.stride) != 0) {
		fprintf(stderr,
				"ERROR: It is detected that buf_offset(%" PRIuPTR ") "
...
static void draw_rect32(FAR struct fb_state_s *state, FAR struct fb_area_s *area, int color)
{
	FAR uint32_t *dest;
	FAR uint8_t *row;
	int x;
...
static void draw_rect24(FAR struct fb_state_s *state, FAR struct fb_area_s *area, int color)
{
	FAR uint8_t *dest;
	FAR uint8_t *row;
	int x;
...
static void draw_rect16(FAR struct fb_state_s *state, FAR struct fb_area_s *area, int color)
{
	FAR uint16_t *dest;
	FAR uint8_t *row;
	int x;
...
static void draw_rect8(FAR struct fb_state_s *state, FAR struct fb_area_s *area, int color)
{
	FAR uint8_t *dest;
	FAR uint8_t *row;
	int x;
...
static void draw_rect1(FAR struct fb_state_s *state, FAR struct fb_area_s *area, int color)
{
	FAR uint8_t *pixel;
	FAR uint8_t *row;
...
static void draw_rect(FAR struct fb_state_s *state, FAR struct fb_area_s *area, int color)
{
...
#ifdef CONFIG_FB_UPDATE
	int yoffset = state->act_fbmem == state->fbmem ? 0 : state->mem2_yoffset;
	area->y += yoffset;
...
int main(int argc, FAR char *argv[])
{
	...
	ret = ioctl(state.fd, FBIOGET_VIDEOINFO, (unsigned long)((uintptr_t)&state.vinfo));
	if (ret < 0) {
		int errcode = errno;
	...
	ret = ioctl(state.fd, FBIOGET_OVERLAYINFO, (unsigned long)((uintptr_t)&state.oinfo));
	if (ret < 0) {
		int errcode = errno;
	...
	state.fbmem =
		mmap(NULL, state.pinfo.fblen, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FILE, state.fd, 0);
	...
	for (x = 0, y = 0, color = 0; color < NCOLORS; x += xstep, y += ystep, color++) {
		area.x = x;
		area.y = y;
		area.w = width;
		area.h = height;
