	...
	if (src) {
		...
		if (!nxgl_nullrect(&intersection)) {
			...
			ret = nxtk_bitmapwindow((NXTKWINDOW)hwnd, &intersection, (FAR const void **)&src,
									&bm->bounds.pt1, (unsigned int)bm->glyph->stride);
			if (ret < 0) {
				printf("nxeg_fillchar: nxtk_bitmapwindow failed: %d\n", errno);
			}
#else
			ret = nx_bitmap((NXWINDOW)hwnd, &intersection, &src, &bm->bounds.pt1,
...
nxeg_renderglyph(FAR struct nxeg_state_s *st, FAR const struct nx_fontbitmap_s *bm, uint8_t ch)
{
...
#error "Additional logic is needed here for 24bpp support"
			...
			ret = RENDERER((FAR nxgl_mxpixel_t *)glyph->bitmap, glyph->height, glyph->width,
						   glyph->stride, bm, CONFIG_EXAMPLES_NX_FONTCOLOR);
			if (ret < 0) {
...
static inline FAR const struct nxeg_glyph_s *nxeg_addspace(FAR struct nxeg_state_s *st, uint8_t ch)
{
	FAR struct nxeg_glyph_s *glyph = NULL;
...
static FAR const struct nxeg_glyph_s *nxeg_findglyph(FAR struct nxeg_state_s *st, uint8_t ch)
{
	int i;
...
static FAR const struct nxeg_glyph_s *nxeg_getglyph(FAR struct nxeg_state_s *st, uint8_t ch)
{
...
static FAR const struct nxeg_bitmap_s *nxeg_addchar(FAR struct nxeg_state_s *st, uint8_t ch)
{
...
static inline void nxeg_addchars(NXWINDOW hwnd, FAR struct nxeg_state_s *st, uint8_t nch,
...
void nxeg_kbdin(NXWINDOW hwnd, uint8_t nch, FAR const uint8_t *ch, FAR void *arg)
{
...
void nxeg_filltext(NXWINDOW hwnd, FAR const struct nxgl_rect_s *rect, FAR struct nxeg_state_s *st)
{
	int i;
