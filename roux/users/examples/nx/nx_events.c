...
static void nxeg_redraw(NXEGWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool morem,
						FAR void *arg);
...
static void nxeg_mousein(NXEGWINDOW hwnd, FAR const struct nxgl_point_s *pos, uint8_t buttons,
						 FAR void *arg);
#endif

#ifndef CONFIG_EXAMPLES_NX_RAWWINDOWS
static void nxeg_tbredraw(NXEGWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool morem,
						  FAR void *arg);
...
#ifdef CONFIG_NX_XYINPUT
static void nxeg_tbmousein(NXEGWINDOW hwnd, FAR const struct nxgl_point_s *pos, uint8_t buttons,
						   FAR void *arg);
#endif
#endif
...
static inline void nxeg_fillwindow(NXEGWINDOW hwnd, FAR const struct nxgl_rect_s *rect,
...
#ifndef CONFIG_EXAMPLES_NX_RAWWINDOWS
static inline void nxeg_filltoolbar(NXTKWINDOW htb, FAR const struct nxgl_rect_s *rect,
...
static void nxeg_redraw(NXEGWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool more,
						FAR void *arg)
{
	FAR struct nxeg_state_s *st = (FAR struct nxeg_state_s *)arg;
	printf("nxeg_redraw%d: hwnd=%p rect={(%d,%d),(%d,%d)} more=%s\n", st->wnum, hwnd, rect->pt1.x,
		   rect->pt1.y, rect->pt2.x, rect->pt2.y, more ? "true" : "false");
						  ...
						  FAR void *arg)
{
	...
	printf("nxeg_position%d: hwnd=%p size=(%d,%d) pos=(%d,%d) bounds={(%d,%d),(%d,%d)}\n", st->wnum,
		   hwnd, size->w, size->h, pos->x, pos->y, bounds->pt1.x, bounds->pt1.y, bounds->pt2.x,
		   bounds->pt2.y);
...
#ifndef CONFIG_EXAMPLES_NX_RAWWINDOWS
static void nxeg_tbredraw(NXEGWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool more,
						  FAR void *arg)
{
	FAR struct nxeg_state_s *st = (FAR struct nxeg_state_s *)arg;
							...
							FAR const struct nxgl_rect_s *bounds, FAR void *arg)
{
	...
	printf("nxeg_ptbosition%d: hwnd=%p size=(%d,%d) pos=(%d,%d) bounds={(%d,%d),(%d,%d)}\n",
		   st->wnum, hwnd, size->w, size->h, pos->x, pos->y, bounds->pt1.x, bounds->pt1.y,
