...
static void help(FAR struct neo_config_s *conf)
{
	...
	printf("  [-p path] selects the ws2812 device.  "
		   "Default: %s Current: %s\n",
		   CONFIG_EXAMPLES_WS2812_DEFAULT_DEV, conf->path ? conf->path : "NONE");
...
static void set_devpath(FAR struct neo_config_s *conf, FAR const char *devpath)
{
...
static void parse_args(FAR struct neo_config_s *conf, int argc, FAR char **argv)
{
	FAR char *ptr;
	FAR char *str;
	long value;
...
int main(int argc, FAR char *argv[])
{
	...
	if (fd < 0) {
		fprintf(stderr, "ws2812_main: open %s failed: %d\n", config.path, errno);
		goto errout;
	}
	...
	for (int i = 0; i < config.loops; ++i) {
		for (int j = 0; j < 256; ++j) {
			...
			for (int k = 0; k < config.leds; ++k) {
				*bp++ = ws2812_gamma_correct(ws2812_hsv_to_rgb((j + k) & 0xff, 0xff, 0xff));
			}
