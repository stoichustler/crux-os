...
static void rgb8888(uint32_t argb, uint8_t *a, uint8_t *r, uint8_t *g, uint8_t *b)
{
	*a = (uint8_t)(argb >> 24);
	*r = (uint8_t)(argb >> 16);
	*g = (uint8_t)(argb >> 8);
						...
						FAR const struct fb_area_s *area, uint32_t rgb)
{
	...
	printf("Fill area (%d,%d,%d,%d) with color: %08" PRIx32 " -> (r,g,b) = "
		   "(%02x,%02x,%02x)\n",
		   area->x, area->y, area->w, area->h, rgb, b, g, r);
						...
						FAR const struct fb_area_s *area, uint32_t argb)
{
	...
	printf("Fill area (%d,%d,%d,%d) with color: %08" PRIx32 " -> (a,r,g,b) = "
		   ...
		   area->x, area->y, area->w, area->h, argb, argb >> 24, argb >> 16, argb >> 8, argb);
...
static void print_plane_info(int fb)
{
	...
	fbmem = mmap(NULL, pinfo.fblen, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FILE, fb, 0);
	...
	printf("PlaneInfo:\n"
		   ...
		   "      bpp: %u\n"
		   "    fbmem: %p\n",
		   pinfo.fbmem, pinfo.fblen, pinfo.stride, pinfo.display, pinfo.bpp, fbmem);
}
...
static void print_overlay_info(int fb, uint8_t overlayno)
{
	...
	fbmem = mmap(NULL, oinfo.fblen, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FILE, fb, 0);
	...
	printf("OverlayInfo:\n"
		   ...
		   "     accl: %08" PRIx32 "\n"
		   "     mmap: %p\n",
		   oinfo.fbmem, oinfo.fblen, oinfo.stride, oinfo.overlay, oinfo.bpp, oinfo.blank,
		   oinfo.chromakey, oinfo.color, oinfo.transp.transp, oinfo.transp.transp_mode, oinfo.accl,
		   fbmem);
}
...
static int overlay_fill(int fb, uint8_t overlayno, uint32_t color, FAR const struct fb_area_s *area)
{
	int ret;
	FAR void *fbmem;
	struct fb_videoinfo_s vinfo;
	...
	if (fbmem == MAP_FAILED) {
		...
		uint32_t offset =
			(area->y + area->h - 1) * oinfo.stride + (area->x + area->w) * oinfo.bpp / 8;
...
static int overlay_area(int fb, FAR struct fb_overlayinfo_s *oinfo)
{
	...
	printf("Overlay: %d, set xpos: %d, ypos: %d, xres: %d, yres: %d\n", oinfo->overlay,
		   oinfo->sarea.x, oinfo->sarea.y, oinfo->sarea.w, oinfo->sarea.h);
