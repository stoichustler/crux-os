...
static inline int modbus_initialize(void)
{
	...
	mberr = eMBInit(MB_RTU, 0x0a, CONFIG_EXAMPLES_MODBUS_PORT, CONFIG_EXAMPLES_MODBUS_BAUD,
...
static void *modbus_pollthread(void *pvarg)
{
	...
	ret = ioctl(g_modbus.fd_leds, ULEDIOC_SUPPORTED, (unsigned long)((uintptr_t)&supported));
	if (ret < 0) {
		int errcode = errno;
...
eMBErrorCode eMBRegInputCB(uint8_t *buffer, uint16_t address, uint16_t nregs)
{
	...
	if ((address >= CONFIG_EXAMPLES_MODBUS_REG_INPUT_START) &&
		(address + nregs <=
		 CONFIG_EXAMPLES_MODBUS_REG_INPUT_START + CONFIG_EXAMPLES_MODBUS_REG_INPUT_NREGS)) {
							 ...
							 eMBRegisterMode mode)
{
	...
	if ((address >= CONFIG_EXAMPLES_MODBUS_REG_HOLDING_START) &&
		(address + nregs <=
		 CONFIG_EXAMPLES_MODBUS_REG_HOLDING_START + CONFIG_EXAMPLES_MODBUS_REG_HOLDING_NREGS)) {
...
eMBErrorCode eMBRegCoilsCB(uint8_t *buffer, uint16_t address, uint16_t ncoils, eMBRegisterMode mode)
{
	...
	if ((address >= CONFIG_EXAMPLES_MODBUS_REG_COILS_START) &&
		(address + ncoils <=
		 CONFIG_EXAMPLES_MODBUS_REG_COILS_START + CONFIG_EXAMPLES_MODBUS_REG_COILS_NREGS)) {
