...
#include <assert.h>
#include <fcntl.h>
#include <roux/video/fb.h>
#include <roux/nx/nxfonts.h>
#include <roux/video/fb.h>
#include <ctype.h>
#include <spawn.h>
#include <debug.h>
...
#if !defined(CONFIG_EXAMPLES_FBCON_PIPE_STDOUT) && !defined(CONFIG_EXAMPLES_FBCON_PIPE_STDOUT)
...
#define VT100_CURSOROFF                                                                            \
	{                                                                                              \
		ASCII_ESC, '[', '?', '2', '5', 'l'                                                         \
	} /* Cursor OFF                              */
#define VT100_CURSORON                                                                             \
	{                                                                                              \
		ASCII_ESC, '[', '?', '2', '5', 'h'                                                         \
	} /* Cursor ON                               */
#define VT100_CURSORL                                                                              \
	{                                                                                              \
		ASCII_ESC, '[', '*', 'D'                                                                   \
	} /* Move cursor left (* = [1...99]) columns */
...
static int fbcon_hidechar(FAR FAR struct fbcon_state_s *st, FAR const struct fbcon_bitmap_s *bm);
static enum fbcon_vt100state_e fbcon_vt100(FAR struct fbcon_state_s *st, char ch);
static void fbcon_fillchar(FAR struct fbcon_state_s *st, FAR const struct nxgl_rect_s *rect,
						   FAR const struct fbcon_bitmap_s *bm);
static int fbcon_fill(FAR struct fbcon_state_s *st, FAR struct nxgl_rect_s *rect,
					  FAR nxgl_mxpixel_t *color);
static void fbcon_fillspace(FAR struct fbcon_state_s *st, FAR const struct nxgl_rect_s *rect,
							FAR const struct fbcon_bitmap_s *bm);
static int fbcon_bitmap(FAR struct fbcon_state_s *st, FAR const struct nxgl_rect_s *dest,
						FAR const uint32_t *src, unsigned int stride);
static int fbcon_backspace(FAR struct fbcon_state_s *st);
static void fbcon_home(FAR struct fbcon_state_s *st);
static inline void fbcon_movedisplay(FAR struct fbcon_state_s *st, int bottom, int scrollheight);
static inline void fbcon_scroll(FAR struct fbcon_state_s *st, int scrollheight);
static void fbcon_freeglyph(FAR struct fbcon_glyph_s *glyph);
static inline FAR struct fbcon_glyph_s *fbcon_allocglyph(FAR struct fbcon_state_s *st);
static FAR struct fbcon_glyph_s *fbcon_findglyph(FAR struct fbcon_state_s *st, uint8_t ch);
static inline FAR struct fbcon_glyph_s *
fbcon_renderglyph(FAR struct fbcon_state_s *st, FAR const struct nx_fontbitmap_s *fbm, uint8_t ch);
static int fbcon_fontsize(FAR void *hfont, uint8_t ch, FAR struct nxgl_size_s *size);
static FAR struct fbcon_glyph_s *fbcon_getglyph(FAR struct fbcon_state_s *st, uint8_t ch);
static FAR const struct fbcon_bitmap_s *fbcon_addchar(FAR struct fbcon_state_s *st, uint8_t ch);
static void fbcon_write(FAR struct fbcon_state_s *st, FAR char *buffer, size_t buflen);
...
static enum fbcon_vt100state_e fbcon_vt100seq(FAR struct fbcon_state_s *st, int seqsize);
static FAR const struct vt100_sequence_s *fbcon_vt100part(FAR struct fbcon_state_s *st,
														  int seqsize);
static enum fbcon_vt100state_e fbcon_vt100(FAR struct fbcon_state_s *st, char ch);
...
static void fbcon_clearline(FAR struct fbcon_state_s *st)
{
	...
	while (--i > 0) {
		bm = &st->bm[i];
		if (bm->pos.y <= rect.pt1.y && bm->pos.y + st->fheight >= rect.pt2.y) {
			st->nchars--;
		}
	}
}
...
static inline void fbcon_movedisplay(struct fbcon_state_s *st, int bottom, int scrollheight)
{
	FAR struct fbcon_bitmap_s *bm;
	struct nxgl_rect_s rect;
	nxgl_coord_t row;
	...
	for (i = 0; i < st->nchars;) {
		...
		if (bm->pos.y < scrollheight + FBCON_LINESPACING) {
			...
			for (j = i; j < st->nchars - 1; j++) {
				memcpy(&st->bm[j], &st->bm[j + 1], sizeof(struct fbcon_bitmap_s));
			}
...
#error "Additional logic is needed here for 24bpp support"
		...
		ret = RENDERER((FAR nxgl_mxpixel_t *)glyph->bitmap, glyph->height, glyph->width,
					   glyph->stride, fbm, st->fcolor);
		if (ret < 0) {
...
static int fbcon_hidechar(FAR struct fbcon_state_s *st, FAR const struct fbcon_bitmap_s *bm)
{
...
void fbcon_fillchar(FAR struct fbcon_state_s *st, FAR const struct nxgl_rect_s *rect,
					FAR const struct fbcon_bitmap_s *bm)
{
	FAR struct fbcon_glyph_s *glyph;
	struct nxgl_rect_s bounds;
	...
	if (!nxgl_nullrect(&intersection)) {
		...
		ret = fbcon_bitmap(st, &intersection, src, (unsigned int)glyph->stride);
		if (ret < 0) {
...
FAR const struct vt100_sequence_s *fbcon_vt100part(FAR struct fbcon_state_s *st, int seqsize)
{
...
static int fbdev_get_pinfo(int fd, FAR struct fb_planeinfo_s *pinfo)
{
	...
	if (pinfo->bpp != 32 && pinfo->bpp != 24 && pinfo->bpp != 16 && pinfo->bpp != 8 &&
		pinfo->bpp != 1) {
...
static int fbcon_initialize(FAR struct fbcon_state_s *st)
{
	...
	st->cursor.pos.y = FBCON_LINESPACING;
	st->maxchars =
		(st->wsize.w / st->fwidth) * (st->wsize.h / (st->fheight + FBCON_LINESPACING) - 1);
...
int main(int argc, FAR char *argv[])
{
	...
	ret = ioctl(st->fd_fb, FBIOGET_VIDEOINFO, (unsigned long)((uintptr_t)st->vinfo));
	if (ret < 0) {
		int errcode = errno;
	...
	if (ret < 0) {
		...
		ret = FBCON_EXIT_FBIO_SELECT_OVERLAY goto errout;
	}

	/* Get the first overlay information */

	st->oinfo.overlay = 0;
	ret = ioctl(st->fd_fb, FBIOGET_OVERLAYINFO, (unsigned long)((uintptr_t)&st->oinfo));
	if (ret < 0) {
		int errcode = errno;
		gerr("ERROR: ioctl(FBIOGET_OVERLAYINFO) failed: %d\n", errcode);
		ret = FBCON_EXIT_FBIO_OVERLAY_INFO goto errout;
	}
	...
	st->fbmem =
		mmap(NULL, st->pinfo->fblen, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FILE, st->fd_fb, 0);
  ...
  if (st->pinfo->yres_virtual == (st->vinfo->yres * 2))
	...
	posix_spawn_file_actions_adddup2(&actions, g_nsh_stdin[READ_PIPE], STDIN_FILENO);

#endif /* CONFIG_EXAMPLES_FBCON_PIPE_STDIN */

	index = builtin_isavail(SPAWN_TASK);
	if (index < 0) {
		gerr("ERROR: requested task "
			 "\""
			 "%s"
			 "\""
			 " not available.\n",
