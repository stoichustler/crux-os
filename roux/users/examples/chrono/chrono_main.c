...
static void slcd_puts(FAR struct lib_outstream_s *outstream, FAR const char *str)
{
	for (; *str; str++) {
...
int main(int argc, FAR char *argv[])
{
	...
	ret = task_create("chrono_daemon", BUTTON_PRIORITY, BUTTON_STACKSIZE, chrono_daemon, NULL);
	if (ret < 0) {
		int errcode = errno;
	...
	if (!priv->initialized) {
		...
		printf("Attributes:\n");
		printf("  rows: %d columns: %d nbars: %d\n", priv->attr.nrows, priv->attr.ncolumns,
			   priv->attr.nbars);
		printf("  max contrast: %d max brightness: %d\n", priv->attr.maxcontrast,
			   priv->attr.maxbrightness);
	...
	for (;;) {
		...
		if (priv->state == CHRONO_RUNNING) {
			...
			snprintf(str, sizeof(str), "%02ld:%02ld:%01ld", min, sec,
