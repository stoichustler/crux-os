...
static int postproc_ponresetseq(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
								...
								unsigned long arg)
{
	...
	ponreset_seq[rarg->seq_no](dev, reply, usock, usock_result, usock_xid, ackinfo, rarg->arg);
	rarg->seq_no++;
...
static int send_getversion_onreset(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
								   FAR int32_t *usock_result)
{
	...
	set_container_postproc(container, postproc_ponresetseq, (unsigned long)&reset_arg);
...
static int recv_atreply_onreset(atreply_parser_t parse, FAR struct alt1250_s *dev, FAR void *arg)
{
	int ret;
	uint64_t bitmap;
	struct pollfd fds;
...
static int alt1250_lwm2m_ponreset(FAR struct alt1250_s *dev, FAR struct alt_container_s *container)
{
	int ret = REP_NO_ACK;
	int recv_ret;
	struct atreply_truefalse_s t_or_f;
