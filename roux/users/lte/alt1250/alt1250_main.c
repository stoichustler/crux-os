...
static int alt1250_loop(FAR struct alt1250_s *dev)
{
	...
	while (is_running) {
		...
		dbg_alt1250("ret: %u, recvfrom_processing: %d,"
					...
					ret, dev->recvfrom_processing, IS_POLLIN(fds[USOCKFDNO]), dev->is_usockrcvd);
...
int main(int argc, FAR char *argv[])
{
	...
	if (argc > 1) {
		...
		if (!(strncmp(argv[1], SYNC_CMD_PREFIX, strlen(SYNC_CMD_PREFIX)))) {
			syncsem = (FAR sem_t *)strtol(&argv[1][strlen(SYNC_CMD_PREFIX)], &endptr, 16);
			if (!syncsem || endptr == &argv[1][strlen(SYNC_CMD_PREFIX)] || *endptr != '\0') {
				return -EINVAL;
			}
		}
	}
...
int alt1250_collect_daemon_context(FAR struct alt1250_s *dev)
{
	...
	ctx.checksum = calc_checksum((FAR uint8_t *)&ctx, offsetof(struct alt1250_save_ctx, checksum));
...
int alt1250_apply_daemon_context(FAR struct alt1250_s *dev, FAR struct alt1250_save_ctx *ctx)
{
	...
	checksum = calc_checksum((FAR uint8_t *)ctx, offsetof(struct alt1250_save_ctx, checksum));
