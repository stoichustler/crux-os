...
static int send_setsockopt_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
						...
						FAR struct usock_ackinfo_s *ackinfo)
{
	...
	*usock_xid = request->head.xid;
	...
	USOCKET_SET_REQSOCKOPT(usock, request->level, request->option, request->valuelen);
	...
	case SOCKET_STATE_PREALLOC:
		...
		*usock_result =
			usockif_readreqoption(dev->usockfd, USOCKET_REQOPTVAL(usock), USOCKET_REQOPTLEN(usock));
		if (ret < 0) {
			return REP_SEND_ACK;
		}
	...
	default:
		...
		*usock_result =
		...
		ret = nextstep_setsockopt(dev, container, usock, usock_result, usock_xid, ackinfo, 0);
		if (IS_NEED_CONTAINER_FREE(ret)) {
			container_free(container);
		}
		break;
