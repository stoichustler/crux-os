...
static int send_listen_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
							   FAR struct usock_s *usock, uint16_t backlog,
					...
					FAR struct usock_ackinfo_s *ackinfo, unsigned long arg)
{
	...
	return send_listen_command(dev, reply, usock, USOCKET_REQBACKLOG(usock), usock_result);
}
...
{
	FAR struct usrsock_request_listen_s *request = &req->request.listen_req;
	...
	*usock_xid = request->head.xid;
	...
	default:
		...
		ret = nextstep_listen(dev, container, usock, usock_result, usock_xid, ackinfo, 0);
		if (IS_NEED_CONTAINER_FREE(ret)) {
			container_free(container);
		}
		break;
