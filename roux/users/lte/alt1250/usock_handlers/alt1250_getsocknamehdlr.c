...
static int postproc_getsockname(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
	...
	*usock_result = COMBINE_ERRCODE(*(FAR int *)resp[0], *(FAR int *)resp[1]);
	...
	if (*usock_result >= 0) {
		...
		*usock_xid = USOCKET_XID(usock);
		...
		ackinfo->valuelen = MIN(USOCKET_REQADDRLEN(usock), *(FAR uint16_t *)(resp[2]));
									...
									FAR int32_t *usock_result)
{
	...
	set_container_ids(container, USOCKET_USOCKID(usock), LTE_CMDID_GETSOCKNAME);
						 ...
						 FAR struct usock_ackinfo_s *ackinfo)
{
	...
	*usock_xid = request->head.xid;
	...
	default:
		...
		ret = nextstep_getsockname(dev, container, usock, usock_result, usock_xid, ackinfo, 0);
		if (IS_NEED_CONTAINER_FREE(ret)) {
			container_free(container);
		}
		break;
