...
static int postproc_getsockopt(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
	...
	*usock_result = COMBINE_ERRCODE(*(FAR int *)resp[0], *(FAR int *)resp[1]);
	...
	if (*usock_result >= 0) {
		...
		*usock_xid = USOCKET_XID(usock);
		...
		ackinfo->valuelen = MIN(USOCKET_REQOPTLEN(usock), *(FAR uint16_t *)(resp[2]));
						...
						unsigned long arg)
{
	...
	return send_getsockopt_command(dev, reply, usock, USOCKET_REQOPTLEVEL(usock),
								   USOCKET_REQOPTOPT(usock), USOCKET_REQOPTLEN(usock),
								   &USOCKET_REQOPTLEVEL(usock), &USOCKET_REQOPTOPT(usock),
						...
						FAR struct usock_ackinfo_s *ackinfo)
{
	...
	*usock_xid = request->head.xid;
	...
	USOCKET_SET_REQSOCKOPT(usock, request->level, request->option, request->max_valuelen);
	...
	default:
		...
		ret = nextstep_getsockopt(dev, container, usock, usock_result, usock_xid, ackinfo, 0);
		if (IS_NEED_CONTAINER_FREE(ret)) {
			container_free(container);
		}
		break;
