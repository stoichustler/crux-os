...
static int send_bind_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
	...
	*usock_xid = request->head.xid;
	...
	default:
		...
		*usock_result =
		...
		ret = nextstep_bind(dev, container, usock, usock_result, usock_xid, ackinfo, 0);
		if (IS_NEED_CONTAINER_FREE(ret)) {
			container_free(container);
		}
		break;
