...
static int postproc_recvfrom(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
...
static int send_recvfrom_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
								 FAR struct usock_s *usock, int32_t flags, uint16_t buflen,
								 int16_t addrlen, FAR int32_t *usock_result)
{
	int idx = 0;
	FAR void *inparam[4];
					  ...
					  FAR struct usock_ackinfo_s *ackinfo)
{
	...
	*usock_xid = request->head.xid;
	...
	if (IS_SMS_SOCKET(usock)) {
		...
		ret = send_recvfrom_command(dev, container, usock, request->flags, _rx_max_buflen, addrlen,
									usock_result);
