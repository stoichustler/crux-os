...
static int postproc_fwgetimglen(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
	...
	if (CONTAINER_RESPRES(reply) >= 0) {
		...
		if (dev->fwup_info.act_injected == -1) {
			...
			dev->fwup_info.hdr_injected =
				(dev->fwup_info.act_injected > 0) ? sizeof(struct delta_header_s) : 0;
		...
		if (dev->fwup_info.act_injected == -1) {
			...
			CONTAINER_RESPRES(reply) = dev->fwup_info.act_injected + dev->fwup_info.hdr_injected;
		}
	} else {
...
static int postproc_fwupdate_injection(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
									   FAR struct usock_s *usock, FAR int32_t *usock_result,
									   FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
									   unsigned long arg)
{
...
static int postproc_fwupdate_dummy(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
								   FAR struct usock_s *usock, FAR int32_t *usock_result,
								   FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
								   unsigned long arg)
{
...
static bool store_injection_data(FAR char *dist, FAR int *ofst, int sz, FAR uint8_t **src,
								 FAR int *len)
{
	bool ret = false;
	int rest;
...
{
	uint32_t crc_result = crc32((uint8_t *)&dev->fwup_info.hdr, sizeof(struct delta_header_s));
...
{
	uint32_t crc_result = crc32((uint8_t *)dev->fwup_info.img_pert, LTE_IMAGE_PERT_SIZE);
...
static int fwupdate_header_injection(FAR struct alt1250_s *dev, FAR struct lte_ioctl_data_s *cmd,
									 FAR bool *send_pert)
{
	...
	*send_pert = false;
	...
	if ((dev->fw_version[0] == 'R') && (dev->fw_version[1] == 'K') &&
		...
		if (dev->fwup_info.act_injected == 0) {
			filled_all =
				store_injection_data((char *)&dev->fwup_info.hdr, &dev->fwup_info.hdr_injected,
									 sizeof(struct delta_header_s), &data, &len);
			if (filled_all) {
				if (!verify_header_crc32(dev) || (dev->fwup_info.hdr.chunk_code != DELTA_MAGICNO) ||
		...
		if ((dev->fwup_info.hdr_injected >= sizeof(struct delta_header_s)) && (len > 0)) {
			filled_all = store_injection_data(dev->fwup_info.img_pert, &dev->fwup_info.act_injected,
...
int usockreq_ioctl_fwupdate(FAR struct alt1250_s *dev, FAR struct usrsock_request_buff_s *req,
		...
		*usock_result = fwupdate_header_injection(dev, ltecmd, &send_cmd);
		if (*usock_result < 0) {
			...
			if (send_cmd) {
				...
				postproc_priv = *(FAR int *)ltecmd->inparam[1] - *usock_result;
