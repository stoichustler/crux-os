...
static int postproc_getfl(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
						  FAR struct usock_s *usock, FAR int32_t *usock_result,
						  FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
						  unsigned long arg);

static int postproc_setfl(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
						  FAR struct usock_s *usock, FAR int32_t *usock_result,
						  FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
						  unsigned long arg);
...
static int send_fctl_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
							 FAR struct usock_s *usock, int32_t cmd, int32_t val,
							 FAR int32_t *usock_result)
{
	...
	set_container_postproc(container,
						   cmd == ALTCOM_SETFL	 ? postproc_setfl
						   : cmd == ALTCOM_GETFL ? postproc_getfl
												 : NULL,
						   0);
	...
	*usock_result = COMBINE_ERRCODE(*(FAR int *)resp[0], *(FAR int *)resp[1]);
	...
	if (*usock_result >= 0) {
		...
		case USRSOCK_REQUEST_CONNECT:
			ret = nextstep_connect(dev, reply, usock, usock_result, usock_xid, ackinfo, arg);
			break;

		case USRSOCK_REQUEST_BIND:
			ret = nextstep_bind(dev, reply, usock, usock_result, usock_xid, ackinfo, arg);
			break;

		case USRSOCK_REQUEST_LISTEN:
			ret = nextstep_listen(dev, reply, usock, usock_result, usock_xid, ackinfo, arg);
			break;

		case USRSOCK_REQUEST_SETSOCKOPT:
			ret = nextstep_setsockopt(dev, reply, usock, usock_result, usock_xid, ackinfo, arg);
			break;

		case USRSOCK_REQUEST_GETSOCKOPT:
			ret = nextstep_getsockopt(dev, reply, usock, usock_result, usock_xid, ackinfo, arg);
			break;

		case USRSOCK_REQUEST_GETSOCKNAME:
			ret = nextstep_getsockname(dev, reply, usock, usock_result, usock_xid, ackinfo, arg);
			break;
	...
	*usock_result = COMBINE_ERRCODE(*(FAR int *)resp[0], *(FAR int *)resp[1]);
	...
	if (*usock_result >= 0) {
		...
		*usock_result |= ALTCOM_O_NONBLOCK;
		...
		ret = send_fctl_command(dev, reply, usock, ALTCOM_SETFL, *usock_result, usock_result);
	}
...
static int postproc_socket(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
						   FAR struct usock_s *usock, FAR int32_t *usock_result,
						   FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
						   unsigned long arg)
{
	...
	*usock_result = COMBINE_ERRCODE(*(FAR int *)resp[0], *(FAR int *)resp[1]);
	...
	if (*usock_result >= 0) {
		...
		ret = send_fctl_command(dev, reply, usock, ALTCOM_GETFL, 0, usock_result);
	}
...
static int send_socket_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
							   FAR struct usock_s *usock, int16_t domain, int16_t type,
							   int16_t protocol, FAR int32_t *usock_result)
{
	int idx = 0;
	FAR void *inparam[3];
				   ...
				   FAR struct usock_s *usock, FAR int32_t *usock_result)
{
	...
	ret = send_socket_command(dev, container, usock, USOCKET_DOMAIN(usock), USOCKET_TYPE(usock),
							  USOCKET_PROTOCOL(usock), usock_result);
	if (*usock_result >= 0) {
					...
					FAR struct usock_ackinfo_s *ackinfo)
{
	...
	*usock_result = USOCKET_USOCKID(usock);
	...
	USOCKET_SET_SOCKTYPE(usock, request->domain, request->type, request->protocol);
