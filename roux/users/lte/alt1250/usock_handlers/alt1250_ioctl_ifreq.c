...
static int postproc_radiooff(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
...
static int send_actpdn_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
							   FAR struct usock_s *usock, FAR int32_t *usock_result)
{
	int idx = 0;
	FAR void *inparam[1];
...
static int send_radio_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
							  FAR struct usock_s *usock, FAR int32_t *usock_result, bool on)
{
	int idx = 0;
	...
	set_container_response(container, USOCKET_REP_RESPONSE(usock), idx);
	set_container_postproc(container, on ? postproc_radioon : postproc_radiooff, 0);
							 ...
							 unsigned long arg)
{
	...
	*usock_result = (CONTAINER_RESPRES(reply) == 0) ? altcom_result : CONTAINER_RESPRES(reply);
							...
							unsigned long arg)
{
	...
	*usock_result = (CONTAINER_RESPRES(reply) == 0) ? altcom_result : CONTAINER_RESPRES(reply);
	...
	if (*usock_result == 0) {
		ret = send_reportnet_command(dev, reply, usock, postproc_reportnet, 0, usock_result);
	}
...
static int do_ifup(FAR struct alt1250_s *dev, FAR struct usrsock_request_buff_s *req,
...
static int do_ifdown(FAR struct alt1250_s *dev, FAR struct usrsock_request_buff_s *req,
					 FAR int32_t *usock_result, FAR uint32_t *usock_xid,
					 FAR struct usock_ackinfo_s *ackinfo)
{
	FAR struct usrsock_request_ioctl_s *request = &req->request.ioctl_req;
...
int usockreq_ioctl_ifreq(FAR struct alt1250_s *dev, FAR struct usrsock_request_buff_s *req,
						 FAR int32_t *usock_result, FAR uint32_t *usock_xid,
						 FAR struct usock_ackinfo_s *ackinfo)
{
	FAR struct usrsock_request_ioctl_s *request = &req->request.ioctl_req;
