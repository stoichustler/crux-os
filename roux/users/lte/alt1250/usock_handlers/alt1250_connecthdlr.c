...
static int postproc_connect(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
							FAR struct usock_s *usock, FAR int32_t *usock_result,
							FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
							unsigned long arg)
{
...
static int send_connect_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
								FAR struct usock_s *usock, FAR int32_t *usock_result)
{
	int idx = 0;
	FAR void *inparam[3];
...
int nextstep_check_connectres(FAR struct alt1250_s *dev, FAR struct usock_s *usock)
{
	...
	if (container != NULL) {
		ret = send_getsockopt_command(dev, container, usock, g_request_level, g_request_option,
									  sizeof(int), &g_request_level, &g_request_option,
					 ...
					 FAR struct usock_ackinfo_s *ackinfo)
{
	...
	*usock_xid = request->head.xid;
	...
	case SOCKET_STATE_PREALLOC:
		...
		*usock_result =
			usockif_readreqaddr(dev->usockfd, &USOCKET_REQADDR(usock), USOCKET_REQADDRLEN(usock));
		if (*usock_result < 0) {
	...
	default:
		...
		*usock_result =
		...
		ret = nextstep_connect(dev, container, usock, usock_result, usock_xid, ackinfo, 0);
		if (IS_NEED_CONTAINER_FREE(ret)) {
			container_free(container);
		}
		break;
