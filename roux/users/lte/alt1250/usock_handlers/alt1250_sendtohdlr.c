...
static int send_sendto_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
							   FAR struct usock_s *usock, FAR struct usrsock_request_sendto_s *req,
							   uint16_t buflen, FAR struct sockaddr_storage *addr,
	...
	if (IS_SMS_SOCKET(usock)) {
		...
		if (request->buflen > 0) {
			...
			ret = send_sendto_command(dev, container, usock, request, sendlen,
									  (request->addrlen > 0 ? &addr : NULL), usock_result);
