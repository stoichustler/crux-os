...
extern int usockreq_ioctl_extend(FAR struct alt1250_s *dev, FAR struct usrsock_request_buff_s *req,
						  ...
						  FAR struct usock_ackinfo_s *ackinfo)
{
	...
	*usock_xid = request->head.xid;
	...
	if (dev->is_resuming && !IS_LTE_REPORT_EVENT(ltecmd->cmdid) &&
		ltecmd->cmdid != LTE_CMDID_SETEVTCTX && ltecmd->cmdid != LTE_CMDID_RESUME) {
