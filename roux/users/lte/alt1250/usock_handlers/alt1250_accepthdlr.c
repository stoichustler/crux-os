...
static int postproc_accepterr(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
...
static int send_close_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
							  FAR struct usock_s *usock, int altsock, FAR int32_t *usock_result)
{
	int idx = 0;
	FAR void *inparam[1];
...
static int postproc_accept(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
						   FAR struct usock_s *usock, FAR int32_t *usock_result,
						   FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
						   unsigned long arg)
{
	...
	if (altsock_res >= 0) {
		...
		if (!accept_sock) {
			ret = send_close_command(dev, reply, usock, altsock_res, usock_result);
			*usock_xid = USOCKET_XID(usock);
		} else {
			...
			*usock_xid = USOCKET_XID(usock);
			...
			ackinfo->valuelen = MIN(USOCKET_REQADDRLEN(usock), *(uint16_t *)(resp[2]));
...
static int send_accept_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
							   FAR struct usock_s *usock, uint16_t addrlen,
