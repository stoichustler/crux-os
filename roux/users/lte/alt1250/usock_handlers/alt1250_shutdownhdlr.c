...
static int send_shutdown_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
								 FAR struct usock_s *usock, int32_t how, FAR int32_t *usock_result)
{
	int idx = 0;
	FAR void *inparam[2];
...
{
	FAR struct usrsock_request_shutdown_s *request = &req->request.shutdown_req;
	...
	*usock_xid = request->head.xid;
	...
	if (request->how != SHUT_RD && request->how != SHUT_WR && request->how != SHUT_RDWR) {
	...
	default:
		...
		ret = send_shutdown_command(dev, container, usock, request->how, usock_result);
