...
#define IS_SMS_UNAVAIL_FWVERSION(d) (!strncmp(MODEM_FWVERSION(d), "RK_02_01_01", 11))
...
static int postproc_smsinit(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
							FAR struct usock_s *usock, FAR int32_t *usock_result,
							FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
							unsigned long arg);

static int postproc_smsinit_reopen(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
								   FAR struct usock_s *usock, FAR int32_t *usock_result,
								   FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
								   unsigned long arg);

static int postproc_smsfin(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
						   FAR struct usock_s *usock, FAR int32_t *usock_result,
						   FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
						   unsigned long arg);

static int postproc_smsfin_reopen(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
								  FAR struct usock_s *usock, FAR int32_t *usock_result,
								  FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
								  unsigned long arg);

static int postproc_smssend(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
							FAR struct usock_s *usock, FAR int32_t *usock_result,
							FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
							unsigned long arg);

static int postproc_smsdelete(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
							  FAR struct usock_s *usock, FAR int32_t *usock_result,
							  FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
							  unsigned long arg);
...
static int send_smsinit_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
								int usockid, FAR postproc_hdlr_t func, FAR int32_t *usock_result)
{
...
static int send_smsfin_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
							   int usockid, FAR postproc_hdlr_t func, FAR int32_t *usock_result)
{
	/* Dummy variable to receive replies by the container.
	 * No one refers to this address.
...
static int send_smssend_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
								FAR struct usock_s *usock, FAR struct sms_send_msg_s *msg,
								FAR uint16_t msg_len, FAR int32_t *usock_result)
{
	int idx = 0;
...
static int send_smsdelete_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
								  FAR struct usock_s *usock, uint16_t msg_index,
...
static int send_smsreportrecv_command(FAR struct alt1250_s *dev, FAR int32_t *usock_result)
{
	struct alt_container_s container = {0};
...
static bool fill_recv_ackinfo(FAR struct alt1250_s *dev, FAR struct usock_s *usock,
							  FAR int32_t *usock_result, FAR struct usock_ackinfo_s *ackinfo)
{
	FAR void **arg;
...
static void notify_read_ready(FAR struct alt1250_s *dev, uint16_t msg_index, uint16_t msg_len)
{
...
static void update_concat_size(FAR struct alt1250_s *dev, uint16_t msg_index, uint16_t msg_len,
							   uint8_t max_num, uint8_t seq_num,
							   FAR struct sms_recv_msg_header_s *sms_msg)
{
	...
	if (max_num == seq_num) {
		...
		send_smsfin_command(dev, &g_sms_container, 0, postproc_smsfin_reopen, &usock_result);
		if (usock_result < 0) {
			notify_abort(dev);
		} else {
...
static void handle_recvmsg(FAR struct alt1250_s *dev, uint16_t msg_index, uint16_t msg_len,
						   uint8_t max_num, uint8_t seq_num,
						   FAR struct sms_recv_msg_header_s *sms_msg)
{
	if (max_num == 0) {
								  ...
								  unsigned long arg)
{
	...
	if (SMS_STATE(&dev->sms_info) == SMS_STATE_UNINIT) {
		...
		send_smsinit_command(dev, reply, 0, postproc_smsinit_reopen, usock_result);
		if (*usock_result < 0) {
			notify_abort(dev);
		}
	}
...
static void sms_report_event(FAR struct alt1250_s *dev, uint16_t msg_index, uint16_t msg_len,
							 uint8_t max_num, uint8_t seq_num,
							 FAR struct sms_recv_msg_header_s *sms_msg)
{
	dbg_alt1250("%s start\n", __func__);
					 ...
					 FAR int32_t *usock_result, FAR struct usock_ackinfo_s *ackinfo)
{
	...
	if (SMS_STATE(&dev->sms_info) == SMS_STATE_UNINIT) {
		...
		ret = send_smsinit_command(dev, container, USOCKET_USOCKID(usock), postproc_smsinit,
								   usock_result);
...
int alt1250_sms_fin(FAR struct alt1250_s *dev, FAR struct usock_s *usock, FAR int32_t *usock_result)
{
	int ret = REP_SEND_ACK_WOFREE;
	FAR struct alt_container_s *container;
	...
	*usock_result = OK;
	...
	if (usocket_smssock_num(dev) == 1) {
		...
		ret = send_smsfin_command(dev, container, USOCKET_USOCKID(usock), postproc_smsfin,
								  usock_result);
...
int alt1250_sms_send(FAR struct alt1250_s *dev, FAR struct usrsock_request_sendto_s *req,
					 FAR struct usock_s *usock, FAR int32_t *usock_result)
{
	int ret = REP_SEND_ACK_WOFREE;
	FAR struct alt_container_s *container;
	...
	if (req->buflen > 0 && req->buflen <= sizeof(struct sms_send_msg_s) + (SMS_MAX_DATALEN * 2)) {
		...
		ret = send_smssend_command(dev, container, usock, (FAR struct sms_send_msg_s *)dev->tx_buff,
								   req->buflen, usock_result);
		if (IS_NEED_CONTAINER_FREE(ret)) {
			container_free(container);
		}
	} else {
...
int alt1250_sms_recv(FAR struct alt1250_s *dev, FAR struct usrsock_request_recvfrom_s *req,
					 FAR struct usock_s *usock, FAR int32_t *usock_result,
					 FAR struct usock_ackinfo_s *ackinfo)
{
	...
	if (fill_recv_ackinfo(dev, usock, usock_result, ackinfo)) {
		...
		dev->sms_info.total_msglen -= (dev->sms_info.msglen - sizeof(struct sms_recv_msg_header_s));
		...
		ret = send_smsdelete_command(dev, container, usock, dev->sms_info.msg_index, usock_result);
		if (IS_NEED_CONTAINER_FREE(ret)) {
			container_free(container);
		}
	} else {
...
int usockreq_ioctl_sms(FAR struct alt1250_s *dev, FAR struct usrsock_request_buff_s *req,
					   ...
					   FAR struct usock_ackinfo_s *ackinfo)
{
	...
	*usock_result = -EBADFD;
	...
	if (usock) {
		...
		case SIOCSMSSSCA:
			*usock_result = OK;
			memcpy(&dev->sms_info.dest_scaddr, &smsreq->smsru.scaddr, sizeof(struct sms_sc_addr_s));
			break;
...
uint64_t perform_sms_report_event(FAR struct alt1250_s *dev, uint64_t bitmap)
{
	...
	if (alt1250_checkcmdid(LTE_CMDID_SMS_REPORT_RECV, bitmap, &bit)) {
		...
		if (arg && arg[0] && arg[1] && arg[2] && arg[3] && arg[4]) {
			...
			sms_report_event(dev, *((FAR uint16_t *)arg[0]), *((FAR uint16_t *)arg[1]),
							 *((FAR uint8_t *)arg[2]), *((FAR uint8_t *)arg[3]),
