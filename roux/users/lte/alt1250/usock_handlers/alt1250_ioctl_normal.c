...
#define IS_LOG_UNAVAIL_FWVERSION(d) (!strncmp(MODEM_FWVERSION(d), "RK_02_01_01", 11))
...
static int send_resume_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
							   FAR struct usock_s *usock, FAR int32_t *usock_result, int index);
#endif
...
static int postproc_reportnet(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
...
static int postproc_radioon(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
							FAR struct usock_s *usock, FAR int32_t *usock_result,
							FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
							unsigned long arg)
{
	...
	*usock_result = REPLY_RETCODE(CONTAINER_RESPRES(reply), altcom_result);
	...
	if (*usock_result == 0) {
		ret = send_reportnet_command(dev, reply, usock, postproc_reportnet, arg, usock_result);
...
static int postproc_actpdn(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
						   FAR struct usock_s *usock, FAR int32_t *usock_result,
						   FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
						   unsigned long is_async)
{
...
static int postproc_setreport(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
							  FAR struct usock_s *usock, FAR int32_t *usock_result,
							  FAR uint32_t *usock_xid, FAR struct usock_ackinfo_s *ackinfo,
							  unsigned long arg)
{
...
static int send_getversion_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
								   FAR struct usock_s *usock, FAR int32_t *usock_result)
{
...
static int send_register_command(FAR struct alt1250_s *dev, FAR struct alt_container_s *container,
								 FAR struct usock_s *usock, FAR int32_t *usock_result,
								 FAR struct lte_ioctl_data_s *ltecmd, int index)
{
	set_container_ids(container, USOCKET_USOCKID(usock), ltecmd->cmdid);
	set_container_argument(container, ltecmd->inparam, ltecmd->inparamlen);
	set_container_response(container, ltecmd->outparam, ltecmd->outparamlen);
							   ...
							   FAR struct usock_s *usock, FAR int32_t *usock_result, int index)
{
	...
	if (cmdid != 0x00) {
		...
		if (ret == OK) {
			...
			ret = send_register_command(dev, container, usock, usock_result, &ltecmd, cb_index);
		} else {
			*usock_result = ret;
			ret = REP_SEND_ACK;
		}
...
static int lte_context_resume(FAR struct alt1250_s *dev, FAR struct lte_ioctl_data_s *cmd,
...
int usockreq_ioctl_normal(FAR struct alt1250_s *dev, FAR struct usrsock_request_buff_s *req,
						  ...
						  FAR struct usock_ackinfo_s *ackinfo)
{
	...
	*usock_xid = request->head.xid;
	...
	case LTE_CMDID_RADIOON: {
		...
		priv = LTE_IS_ASYNC_CMD(ltecmd->cmdid) ? RADIOON_ASYNC : RADIOON_SYNC;
	} break;
	...
	ret = send_lapi_command(dev, container, usock, ltecmd, postproc_hdlr, priv, usock_result);
	...
	} else if ((ltecmd->cmdid == LTE_CMDID_ACTPDN) || (cmdid == LTE_CMDID_TLS_SSL_HANDSHAKE)) {
