...
static int send_select_command(FAR struct alt1250_s *dev, int32_t mode, int32_t id, int32_t maxfds,
...
static void select_cancel(FAR struct alt1250_s *dev)
{
	...
	if (dev->sid != -1) {
		send_select_command(dev, SELECT_MODE_BLOCKCANCEL, dev->sid, dummy_maxfds, &dummy_readset,
							&dummy_writeset, &dummy_exceptset);
		dev->sid = -1;
	}
}
...
static void select_start(FAR struct alt1250_s *dev)
{
	...
	for (i = 0; i < SOCKET_COUNT; i++) {
		...
		if (usock && IS_STATE_SELECTABLE(usock)) {
			...
			maxfds = ((maxfds > USOCKET_ALTSOCKID(usock)) ? maxfds : USOCKET_ALTSOCKID(usock));
		}
	}

	if (maxfds != -1) {
		send_select_command(dev, SELECT_MODE_BLOCK, dev->sid, maxfds + 1, &readset, &writeset,
							&exceptset);
	}
}
...
static FAR struct alt_container_s *recv_selectcontainer(FAR struct alt1250_s *dev)
{
	g_current_container = altdevice_exchange_selcontainer(dev->altfd, g_current_container);
...
static void operate_dataavail(FAR struct alt1250_s *dev, FAR struct usock_s *usock,
							  FAR altcom_fd_set *readset, FAR altcom_fd_set *writeset,
							  FAR altcom_fd_set *exceptset)
{
...
static void handle_selectevt(FAR struct alt1250_s *dev, int32_t altcom_resp, int32_t modem_errno,
							 int32_t selectreq_id, FAR altcom_fd_set *readset,
							 FAR altcom_fd_set *writeset, FAR altcom_fd_set *exceptset)
{
	int i;
	FAR struct usock_s *usock;
...
uint64_t perform_select_event(FAR struct alt1250_s *dev, uint64_t bitmap)
{
	...
	if (alt1250_checkcmdid(LTE_CMDID_SELECT, bitmap, &bit)) {
		FAR struct alt_container_s *selectcontainer = recv_selectcontainer(dev);
		...
		handle_selectevt(dev, *((FAR int32_t *)selectcontainer->outparam[0]),
