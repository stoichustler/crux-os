...
int altdevice_send_command(FAR struct alt1250_s *dev, int fd, FAR struct alt_container_s *container,
...
{
	return (buffer) ? ioctl_wrapper(fd, ALT1250_IOC_SETEVTBUFF, (unsigned long)buffer) : -EINVAL;
}
...
int altdevice_getevent(int fd, FAR uint64_t *evtbitmap, FAR struct alt_container_s **replys)
{
	int ret = -EIO;
	struct alt_readdata_s dat;

	if (read(fd, &dat, sizeof(struct alt_readdata_s)) == sizeof(struct alt_readdata_s)) {
		ret = OK;
