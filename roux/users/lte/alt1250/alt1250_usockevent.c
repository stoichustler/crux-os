...
int usock_reply(int ufd, int action_code, int32_t result, uint32_t xid,
				FAR struct usock_ackinfo_s *ackinfo)
{
	...
	case REP_SEND_TERM:
		ret = usockif_sendack(ufd, result, xid, (action_code == REP_SEND_INPROG));
		break;
	...
	case REP_SEND_DACK_RXREADY:
		ret = usockif_senddataack(ufd, result, xid, ackinfo);
		usockif_sendevent(ufd, ackinfo->usockid, USRSOCK_EVENT_RECVFROM_AVAIL);
		break;
	}
...
int perform_usockrequest(FAR struct alt1250_s *dev)
{
	...
	if (!IS_USOCKREQ_RECEIVED(dev)) {
		...
		if (IS_IOCTLREQ(dev->usockreq)) {
			...
			if (ret < 0) {
				...
				usock_reply(dev->usockfd, REP_SEND_ACK_WOFREE, ret, USOCKREQXID(dev->usockreq),
							NULL);
				return OK;
			}
		}
	...
	if (!IS_REQID_VALID(dev->usockreq)) {
		...
		ret = handlers[USOCKREQID(dev->usockreq)](dev, &dev->usockreq, &usock_result, &usock_xid,
												  &ackinfo);
	}
