...
static int send_dataack(int fd, uint32_t ackxid, int32_t ackresult, uint16_t valuelen,
						uint16_t valuelen_nontrunc, FAR uint8_t *value_ptr, FAR uint8_t *buf_ptr)
{
	int ret;
...
{
	if ((sz != sizeof(struct sockaddr_in)) && (sz != sizeof(struct sockaddr_in6))) {
...
{
	return send_dataack(fd, usock_xid, usock_result, ackinfo->valuelen, ackinfo->valuelen_nontrunc,
						ackinfo->value_ptr, ackinfo->buf_ptr);
}
