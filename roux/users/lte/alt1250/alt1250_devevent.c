...
#define IS_PV1_FIRMWARE(d) (!strncmp(MODEM_FWVERSION(d), "RK_02_01_", 9))
/****************************************************************************
 * Private Functions
 ****************************************************************************/
...
static int handle_replypkt(FAR struct alt1250_s *dev, FAR struct alt_container_s *reply,
						   ...
						   FAR struct usock_ackinfo_s *ackinfo)
{
	...
	*usock_xid = usock ? USOCKET_XID(usock) : -1;
	...
	if (pp && pp->hdlr) {
		ret = pp->hdlr(dev, reply, usock, usock_result, usock_xid, ackinfo, pp->priv);
	}
...
static int perform_alt1250_reply(FAR struct alt1250_s *dev, FAR struct alt_container_s *container)
{
	int ret = REP_NO_ACK;
...
static int perform_alt1250_resetevt(FAR struct alt1250_s *dev, FAR struct alt_container_s *rlist)
{
	int ret;
...
uint64_t perform_netinfo_report_event(FAR struct alt1250_s *dev, uint64_t bitmap)
{
	uint64_t bit = 0ULL;
	FAR void **arg;
