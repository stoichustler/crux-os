...
void alt1250_netdev_ifup(FAR struct alt1250_s *dev, FAR lte_pdn_t *pdn)
{
	...
	for (i = 0; i < pdn->ipaddr_num; i++) {
		...
		if (LTE_IPTYPE_V4 == pdn->address[i].ip_type) {
			inet_pton(AF_INET, (FAR const char *)pdn->address[i].address,
			...
			dev->net_dev.d_draddr = htonl((ntohl(dev->net_dev.d_draddr) & ALT1250_SUBNET_MASK) | 1);
		...
		if (LTE_IPTYPE_V6 == pdn->address[i].ip_type) {
			inet_pton(AF_INET6, (FAR const char *)pdn->address[i].address,
