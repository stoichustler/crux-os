...
{
	FAR void *inarg[3] = {(FAR void *)data, &len, &init};
	...
	return lapi_req(LTE_CMDID_INJECTIMAGE, (FAR void *)inarg, nitems(inarg), (FAR void *)&dummy_arg,
					0, NULL);
}
...
int lte_get_version_sync(FAR lte_version_t *version)
{
	int ret;
	int result;
	FAR void *outarg[] = {&result, version};

	if (version == NULL) {
		return -EINVAL;
	}

	ret = lapi_req(LTE_CMDID_GETVER, NULL, 0, (FAR void *)outarg, nitems(outarg), NULL);
...
int lte_factory_reset_sync(void)
{
	...
	ret = lapi_req(LTE_CMDID_FACTORY_RESET, NULL, 0, (FAR void *)outarg, nitems(outarg), NULL);
