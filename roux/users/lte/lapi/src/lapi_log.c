...
int lte_log_collect(FAR char *output_fname, size_t len)
{
	...
	return lapi_req(LTE_CMDID_SAVE_LOG, inarg, nitems(inarg), outarg, nitems(outarg), NULL);
}
...
int lte_log_getlist(size_t listsize, size_t fnamelen, char list[listsize][fnamelen])
{
	...
	return lapi_req(LTE_CMDID_GET_LOGLIST, inarg, nitems(inarg), outarg, nitems(outarg), NULL);
}
...
int lte_log_open(FAR const char *filename)
{
	...
	FAR void *inarg[] = {(FAR void *)filename};

	if (filename == NULL) {
		return -EINVAL;
	}

	return lapi_req(LTE_CMDID_LOGOPEN, inarg, nitems(inarg), (FAR void *)&dummy_arg, 0, NULL);
}
...
{
	int dummy_arg; /* Dummy for blocking API call */
	FAR void *inarg[] = {(FAR void *)fd};

	return lapi_req(LTE_CMDID_LOGCLOSE, inarg, nitems(inarg), (FAR void *)&dummy_arg, 0, NULL);
}
...
{
	FAR void *inarg[] = {(FAR void *)fd, (FAR void *)len};

	FAR void *outarg[] = {buf, (FAR void *)len};

	if ((buf == NULL) || (len == 0)) {
		return -EINVAL;
	}

	return lapi_req(LTE_CMDID_LOGREAD, inarg, nitems(inarg), outarg, nitems(outarg), NULL);
}
...
{
	int dummy_arg; /* Dummy for blocking API call */
	FAR void *inarg[] = {(FAR void *)fd, (FAR void *)&offset, (FAR void *)whence};

	return lapi_req(LTE_CMDID_LOGLSEEK, inarg, nitems(inarg), (FAR void *)&dummy_arg, 0, NULL);
}
...
int lte_log_remove(FAR const char *filename)
{
	...
	return lapi_req(LTE_CMDID_LOGREMOVE, inarg, nitems(inarg), (FAR void *)&dummy_arg, 0, NULL);
}
