...
int lte_initialize(void)
{
	...
	if (!is_daemon_running()) {
		...
		snprintf(addr, ADDR_LEN, "%s%08lx", CMD_PREFIX, (unsigned long)&g_sync);

		argv[0] = addr;
		argv[1] = NULL; /* termination */

		ret = task_create(DAEMON_NAME, DAEMON_PRI, DAEMON_STACK_SZ, alt1250_main, argv);
		if (ret < 0) {
			ret = -errno;
...
{
	FAR void *inarg[] = {(FAR void *)res_ctx, &len};
	...
	return lapi_req(LTE_CMDID_RESUME, (FAR void *)inarg, nitems(inarg), (FAR void *)&dummy_arg, 0,
					NULL);
}
