...
static int lte_activate_pdn_inparam_check(FAR lte_apn_setting_t *apn)
{
	...
	if ((apn->ip_type < LTE_IPTYPE_V4) || (apn->ip_type > LTE_IPTYPE_NON)) {
		lapi_printf("ip type is invalid. iptype=%d\n", apn->ip_type);
		return -EINVAL;
	}

	if ((apn->auth_type < LTE_APN_AUTHTYPE_NONE) || (apn->auth_type > LTE_APN_AUTHTYPE_CHAP)) {
		lapi_printf("auth type is invalid. authtype=%d\n", apn->auth_type);
		return -EINVAL;
	}

	if (apn->user_name && apn->password) {
		if (strnlen((FAR char *)apn->user_name, LTE_APN_USER_NAME_LEN) >= LTE_APN_USER_NAME_LEN) {
			lapi_printf("username is length overflow.\n");
			return -EINVAL;
		}

		if (strnlen((FAR char *)apn->password, LTE_APN_PASSWD_LEN) >= LTE_APN_PASSWD_LEN) {
	...
	mask = (LTE_APN_TYPE_DEFAULT | LTE_APN_TYPE_MMS | LTE_APN_TYPE_SUPL | LTE_APN_TYPE_DUN |
			LTE_APN_TYPE_HIPRI | LTE_APN_TYPE_FOTA | LTE_APN_TYPE_IMS | LTE_APN_TYPE_CBS |
			LTE_APN_TYPE_IA | LTE_APN_TYPE_EMERGENCY);
...
int lte_activate_pdn_sync(FAR lte_apn_setting_t *apn, FAR lte_pdn_t *pdn)
{
	...
	FAR void *outarg[] = {&result, pdn};

	if (lte_activate_pdn_inparam_check(apn) || pdn == NULL) {
		return -EINVAL;
	}

	ret = lapi_req(LTE_CMDID_ACTPDN, (FAR void *)inarg, nitems(inarg), (FAR void *)outarg,
				   nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_deactivate_pdn_sync(uint8_t session_id)
{
	...
	ret = lapi_req(LTE_CMDID_DEACTPDN, (FAR void *)inarg, nitems(inarg), (FAR void *)outarg,
				   nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_activate_pdn(FAR lte_apn_setting_t *apn, activate_pdn_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_ACTPDN | LTE_CMDOPT_ASYNC_BIT, (FAR void *)inarg, nitems(inarg), NULL,
					0, callback);
}
...
int lte_deactivate_pdn(uint8_t session_id, deactivate_pdn_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_DEACTPDN | LTE_CMDOPT_ASYNC_BIT, (FAR void *)inarg, nitems(inarg),
					NULL, 0, callback);
}
...
int lte_activate_pdn_cancel(void)
{
	...
	ret = lapi_req(LTE_CMDID_ACTPDNCAN, NULL, 0, (FAR void *)outarg, nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
