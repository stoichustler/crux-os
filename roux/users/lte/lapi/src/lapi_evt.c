...
int lapi_evtinit(FAR const char *mqname)
{
	...
	in.mqname = mqname;
	ret = lapi_req(LTE_CMDID_SETEVTCTX, (FAR void *)inarg, nitems(inarg), (FAR void *)outarg,
				   nitems(outarg), NULL);
	if (ret < 0) {
...
int lapi_evtyield(int timeout_ms)
{
	...
	while (!is_exit) {
		if (timeout_ms >= 0) {
			sz = mq_timedreceive(g_mqd, (FAR void *)&buf, sizeof(buf), NULL, &ts);
			if (sz < 0) {
				ret = -errno;
				if (errno != ETIMEDOUT) {
