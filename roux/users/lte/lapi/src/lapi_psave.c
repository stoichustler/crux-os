...
static int lte_set_edrx_inparam_check(FAR lte_edrx_setting_t *settings)
{
	...
	if (settings->act_type != LTE_EDRX_ACTTYPE_WBS1 &&
		...
		settings->act_type != LTE_EDRX_ACTTYPE_GSM && settings->act_type != LTE_EDRX_ACTTYPE_IU &&
	...
	if (ret < 0 && ret != -ENOTSUP) {
		...
		if (!((ret == LTE_RAT_CATM && settings->act_type == LTE_EDRX_ACTTYPE_WBS1) ||
			  (ret == LTE_RAT_NBIOT && settings->act_type == LTE_EDRX_ACTTYPE_NBS1) ||
			  (settings->act_type == LTE_EDRX_ACTTYPE_NOTUSE))) {
			lapi_printf("Operation is not allowed[act_type : %d,"
						" RAT : %ld].\n",
...
int lte_get_edrx_sync(FAR lte_edrx_setting_t *settings)
{
	...
	int result;
	bool is_edrxevt;
	FAR void *outarg[] = {&result, settings, &is_edrxevt};

	if (settings == NULL) {
		return -EINVAL;
	}

	ret = lapi_req(LTE_CMDID_GETEDRX, NULL, 0, (FAR void *)outarg, nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_set_edrx_sync(FAR lte_edrx_setting_t *settings)
{
	...
	ret = lapi_req(LTE_CMDID_SETEDRX, (FAR void *)inarg, nitems(inarg), (FAR void *)outarg,
				   nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_get_psm_sync(FAR lte_psm_setting_t *settings)
{
	...
	FAR void *outarg[] = {&result, settings, &is_psmevt};

	if (settings == NULL) {
		return -EINVAL;
	}

	ret = lapi_req(LTE_CMDID_GETPSM, (FAR void *)inarg, nitems(inarg), (FAR void *)outarg,
				   nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_set_psm_sync(FAR lte_psm_setting_t *settings)
{
	...
	ret = lapi_req(LTE_CMDID_SETPSM, (FAR void *)inarg, nitems(inarg), (FAR void *)outarg,
				   nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_get_ce_sync(FAR lte_ce_setting_t *settings)
{
	int ret;
	int result;
	FAR void *outarg[] = {&result, settings};

	if (settings == NULL) {
		return -EINVAL;
	}

	ret = lapi_req(LTE_CMDID_GETCE, NULL, 0, (FAR void *)outarg, nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_set_ce_sync(FAR lte_ce_setting_t *settings)
{
	...
	ret = lapi_req(LTE_CMDID_SETCE, (FAR void *)inarg, nitems(inarg), (FAR void *)outarg,
				   nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_get_current_edrx_sync(FAR lte_edrx_setting_t *settings)
{
	...
	int result;
	bool is_getcedrxevt;
	FAR void *outarg[] = {&result, settings, &is_getcedrxevt};

	if (settings == NULL) {
		return -EINVAL;
	}

	ret = lapi_req(LTE_CMDID_GETCEDRX, NULL, 0, (FAR void *)outarg, nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_get_current_psm_sync(FAR lte_psm_setting_t *settings)
{
	...
	int result;
	bool is_getcpsmevt;
	FAR void *outarg[] = {&result, settings, &is_getcpsmevt};

	if (settings == NULL) {
		return -EINVAL;
	}

	ret = lapi_req(LTE_CMDID_GETCPSM, NULL, 0, (FAR void *)outarg, nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_get_edrx(get_edrx_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_GETEDRX | LTE_CMDOPT_ASYNC_BIT, NULL, 0, NULL, 0, callback);
}
...
int lte_set_edrx(FAR lte_edrx_setting_t *settings, set_edrx_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_SETEDRX | LTE_CMDOPT_ASYNC_BIT, (FAR void *)inarg, nitems(inarg),
					NULL, 0, callback);
}
...
int lte_get_psm(get_psm_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_GETPSM | LTE_CMDOPT_ASYNC_BIT, NULL, 0, NULL, 0, callback);
}
...
int lte_set_psm(FAR lte_psm_setting_t *settings, set_psm_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_SETPSM | LTE_CMDOPT_ASYNC_BIT, (FAR void *)inarg, nitems(inarg), NULL,
					0, callback);
}
...
int lte_get_ce(get_ce_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_GETCE | LTE_CMDOPT_ASYNC_BIT, NULL, 0, NULL, 0, callback);
}
...
int lte_set_ce(FAR lte_ce_setting_t *settings, set_ce_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_SETCE | LTE_CMDOPT_ASYNC_BIT, (FAR void *)inarg, nitems(inarg), NULL,
					0, callback);
}
...
int lte_get_current_edrx(get_current_edrx_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_GETCEDRX | LTE_CMDOPT_ASYNC_BIT, NULL, 0, NULL, 0, callback);
}
...
int lte_get_current_psm(get_current_psm_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_GETCPSM | LTE_CMDOPT_ASYNC_BIT, NULL, 0, NULL, 0, callback);
}
