...
int lapi_req(uint32_t cmdid, FAR void *inp, size_t ilen, FAR void *outp, size_t olen, FAR void *cb)
{
	int ret;
	int sock;
...
int lte_data_allow(uint8_t session_id, uint8_t allow, uint8_t roaming_allow,
...
int lte_get_errinfo(FAR lte_errinfo_t *info)
{
	...
	FAR void *outarg[] = {info};

	if (!info) {
		return -EINVAL;
	}

	ret = lapi_req(LTE_CMDID_GETERRINFO, NULL, 0, (FAR void *)outarg, nitems(outarg), NULL);

	return ret;
}

int lte_send_atcmd_sync(FAR const char *cmd, int cmdlen, FAR char *respbuff, int respbufflen,
						FAR int *resplen)
{
	int32_t ret;
	FAR void *inarg[] = {(FAR void *)cmd, (FAR void *)cmdlen};

	FAR void *outarg[] = {respbuff, (FAR void *)respbufflen, resplen};

	if (!cmd || (ATCMD_HEADER_LEN + ATCMD_FOOTER_LEN) > cmdlen || LTE_AT_COMMAND_MAX_LEN < cmdlen ||
	...
	ret = lapi_req(LTE_CMDID_SENDATCMD, (FAR void *)inarg, nitems(inarg), (FAR void *)outarg,
				   nitems(outarg), NULL);
