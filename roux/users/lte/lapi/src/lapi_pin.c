...
static int lte_change_pin_inparam_check(int8_t target_pin, FAR char *pincode, FAR char *new_pincode)
{
	uint8_t pinlen = 0;
...
static int lte_enter_pin_inparam_check(FAR char *pincode, FAR char *new_pincode)
{
	uint8_t pinlen = 0;
...
int lte_get_pinset_sync(FAR lte_getpin_t *pinset)
{
	int ret;
	int result;
	FAR void *outarg[] = {&result, pinset};

	if (pinset == NULL) {
		return -EINVAL;
	}

	ret = lapi_req(LTE_CMDID_GETPINSET, NULL, 0, (FAR void *)outarg, nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_set_pinenable_sync(bool enable, FAR char *pincode, FAR uint8_t *attemptsleft)
{
	int ret;
	int result;
	FAR void *inarg[] = {&enable, pincode};

	FAR void *outarg[] = {&result, attemptsleft};

	if (lte_set_pinenable_inparam_check(enable, pincode) || attemptsleft == NULL) {
		return -EINVAL;
	}

	ret = lapi_req(LTE_CMDID_PINENABLE, (FAR void *)inarg, nitems(inarg), (FAR void *)outarg,
				   nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_change_pin_sync(int8_t target_pin, FAR char *pincode, FAR char *new_pincode,
						FAR uint8_t *attemptsleft)
{
	int ret;
	int result;
	FAR void *inarg[] = {&target_pin, pincode, new_pincode};

	FAR void *outarg[] = {&result, attemptsleft};

	if (lte_change_pin_inparam_check(target_pin, pincode, new_pincode) || attemptsleft == NULL) {
		return -EINVAL;
	}

	ret = lapi_req(LTE_CMDID_CHANGEPIN, (FAR void *)inarg, nitems(inarg), (FAR void *)outarg,
				   nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_enter_pin_sync(FAR char *pincode, FAR char *new_pincode, FAR uint8_t *simstat,
					   FAR uint8_t *attemptsleft)
{
	int ret;
	int result;
	FAR void *inarg[] = {pincode, new_pincode};

	FAR void *outarg[] = {&result, simstat, attemptsleft};
	...
	if (pinset.enable == LTE_DISABLE) {
		lapi_printf("PIN lock is disable. Don't need to run lte_enter_pin_sync().\n");
		return -EPERM;
	} else if (pinset.status != LTE_PINSTAT_SIM_PIN) {
		if (pinset.status == LTE_PINSTAT_SIM_PUK) {
			lapi_printf("This SIM is PUK locked. lte_enter_pin_sync() can't be used.\n");
		} else {
	...
	ret = lapi_req(LTE_CMDID_ENTERPIN, (FAR void *)inarg, nitems(inarg), (FAR void *)outarg,
				   nitems(outarg), NULL);
	if (ret == 0) {
		ret = result;
	}
...
int lte_get_pinset(get_pinset_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_GETPINSET | LTE_CMDOPT_ASYNC_BIT, NULL, 0, NULL, 0, callback);
}

int lte_set_pinenable(bool enable, FAR char *pincode, set_pinenable_cb_t callback)
{
	FAR void *inarg[] = {&enable, pincode};
	...
	return lapi_req(LTE_CMDID_PINENABLE | LTE_CMDOPT_ASYNC_BIT, (FAR void *)inarg, nitems(inarg),
					NULL, 0, callback);
}

int lte_change_pin(int8_t target_pin, FAR char *pincode, FAR char *new_pincode,
				   change_pin_cb_t callback)
{
	...
	return lapi_req(LTE_CMDID_CHANGEPIN | LTE_CMDOPT_ASYNC_BIT, (FAR void *)inarg, nitems(inarg),
					NULL, 0, callback);
}

int lte_enter_pin(FAR char *pincode, FAR char *new_pincode, enter_pin_cb_t callback)
{
	FAR void *inarg[] = {pincode, new_pincode};
	...
	return lapi_req(LTE_CMDID_ENTERPIN | LTE_CMDOPT_ASYNC_BIT, (FAR void *)inarg, nitems(inarg),
					NULL, 0, callback);
}
