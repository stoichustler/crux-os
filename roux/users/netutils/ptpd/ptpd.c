...
static void timespec_to_ptp_format(FAR struct timespec *ts, FAR uint8_t *timestamp)
{
...
static void ptp_format_to_timespec(FAR const uint8_t *timestamp, FAR struct timespec *ts)
{
	ts->tv_sec = (((int64_t)timestamp[0]) << 40) | (((int64_t)timestamp[1]) << 32) |
...
static bool is_better_clock(FAR const struct ptp_announce_s *a, FAR const struct ptp_announce_s *b)
{
...
static int64_t timespec_delta_ns(FAR const struct timespec *ts1, FAR const struct timespec *ts2)
{
	int64_t delta_s;
...
static bool is_selected_source_valid(FAR struct ptp_state_s *state)
{
	...
	if ((state->selected_source.header.messagetype & PTP_MSGTYPE_MASK) != PTP_MSGTYPE_ANNOUNCE) {
...
static void ptp_increment_sequence(FAR uint16_t *sequence_num, FAR struct ptp_header_s *hdr)
{
	*sequence_num += 1;
...
static int ptp_getrxtime(FAR struct ptp_state_s *state, FAR struct msghdr *rxhdr,
...
static int ptp_initialize_state(FAR struct ptp_state_s *state, FAR const char *interface)
{
	int ret;
	struct ifreq req;
	...
	memcpy(state->own_identity.gm_identity, state->own_identity.header.sourceidentity,
	...
	ret = ipmsfilter(&state->interface_addr.sin_addr, &bind_addr.sin_addr, MCAST_INCLUDE);
	if (ret < 0) {
	...
	ret = bind(state->event_socket, (FAR struct sockaddr *)&bind_addr, sizeof(bind_addr));
	if (ret < 0) {
...
#ifdef CONFIG_NET_TIMESTAMP
	arg = 1;
	ret = setsockopt(state->event_socket, SOL_SOCKET, SO_TIMESTAMP, &arg, sizeof(arg));
	...
	ret = bind(state->info_socket, (FAR struct sockaddr *)&bind_addr, sizeof(bind_addr));
	if (ret < 0) {
		ptperr("Failed to bind to udp port %d\n", bind_addr.sin_port);
		return ERROR;
	}
	...
	ret = bind(state->tx_socket, (FAR struct sockaddr *)&bind_addr, sizeof(bind_addr));
	if (ret < 0) {
...
static int ptp_check_multicast_status(FAR struct ptp_state_s *state)
{
	...
	clock_gettime(CLOCK_MONOTONIC, &time_now);
	clock_timespec_subtract(&time_now, &state->last_received_multicast, &delta);
	...
	if (timespec_to_ms(&delta) > CONFIG_NETUTILS_PTPD_MULTICAST_TIMEOUT_MS) {
		...
		return ipmsfilter(&state->interface_addr.sin_addr, &mcast_addr, MCAST_INCLUDE);
	}
...
static int ptp_send_announce(FAR struct ptp_state_s *state)
{
	...
	ret =
		sendto(state->tx_socket, &msg, sizeof(msg), 0, (FAR struct sockaddr *)&addr, sizeof(addr));
...
static int ptp_send_delay_req(FAR struct ptp_state_s *state)
{
	...
	ret =
		sendto(state->tx_socket, &req, sizeof(req), 0, (FAR struct sockaddr *)&addr, sizeof(addr));
...
static int ptp_periodic_send(FAR struct ptp_state_s *state)
{
	...
	if (!state->selected_source_valid) {
		...
		clock_gettime(CLOCK_MONOTONIC, &time_now);
		clock_timespec_subtract(&time_now, &state->last_transmitted_announce, &delta);
		if (timespec_to_ms(&delta) > CONFIG_NETUTILS_PTPD_ANNOUNCE_INTERVAL_MSEC) {
			state->last_transmitted_announce = time_now;
			ptp_send_announce(state);
		}

		clock_timespec_subtract(&time_now, &state->last_transmitted_sync, &delta);
	...
	if (state->selected_source_valid && state->can_send_delayreq) {
		...
		clock_gettime(CLOCK_MONOTONIC, &time_now);
		clock_timespec_subtract(&time_now, &state->last_transmitted_delayreq, &delta);
...
static int ptp_process_announce(FAR struct ptp_state_s *state, FAR struct ptp_announce_s *msg)
{
	clock_gettime(CLOCK_MONOTONIC, &state->last_received_announce);

	if (is_better_clock(msg, &state->own_identity)) {
		if (!state->selected_source_valid || is_better_clock(msg, &state->selected_source)) {
								  ...
								  FAR struct timespec *local_timestamp)
{
	...
	int64_t delta_ns;
	int64_t absdelta_ns;
	const int64_t adj_limit_ns = CONFIG_NETUTILS_PTPD_SETTIME_THRESHOLD_MS * (int64_t)NSEC_PER_MSEC;

	ptpinfo("Local time: %lld.%09ld, remote time %lld.%09ld\n", (long long)local_timestamp->tv_sec,
			(long)local_timestamp->tv_nsec, (long long)remote_timestamp->tv_sec,
	...
	if (absdelta_ns > adj_limit_ns) {
		...
		clock_timespec_subtract(local_timestamp, &state->last_delta_timestamp, &interval);
		interval_ms = timespec_to_ms(&interval);

		if (interval_ms > 0 && interval_ms < CONFIG_NETUTILS_PTPD_TIMEOUT_MS) {
			drift_ppb = (delta_ns - state->last_delta_ns) * MSEC_PER_SEC / interval_ms;
		} else {
		...
		drift_ppb += state->last_adjtime_ns * MSEC_PER_SEC / CONFIG_CLOCK_ADJTIME_PERIOD_MS;
		...
		state->drift_ppb +=
			(drift_ppb - state->drift_ppb) * interval_ms / state->drift_avg_total_ms;

		/* Compute the value we need to give to adjtime() to match the
		 * drift rate.
		 */

		adjustment_ns = state->drift_ppb * CONFIG_CLOCK_ADJTIME_PERIOD_MS / MSEC_PER_SEC;
		...
		ptpinfo("Delta: %+lld ns, adjustment %+lld ns, drift rate %+lld ppb\n", (long long)delta_ns,
				(long long)state->last_adjtime_ns, (long long)state->drift_ppb);
...
static int ptp_process_sync(FAR struct ptp_state_s *state, FAR struct ptp_sync_s *msg)
{
	struct timespec remote_time;

	if (memcmp(msg->header.sourceidentity, state->selected_source.header.sourceidentity,
...
static int ptp_process_followup(FAR struct ptp_state_s *state, FAR struct ptp_follow_up_s *msg)
{
	struct timespec remote_time;

	if (memcmp(msg->header.sourceidentity, state->twostep_packet.header.sourceidentity,
			   sizeof(msg->header.sourceidentity)) != 0) {
		return OK; /* This packet wasn't from the currently selected source */
	}

	if (ptp_get_sequence(&msg->header) != ptp_get_sequence(&state->twostep_packet.header)) {
...
static int ptp_process_delay_req(FAR struct ptp_state_s *state, FAR struct ptp_delay_req_s *msg)
{
	...
	memcpy(resp.header.sequenceid, msg->header.sequenceid, sizeof(resp.header.sequenceid));
	resp.header.logmessageinterval = CONFIG_NETUTILS_PTPD_DELAYRESP_INTERVAL;

	ret = sendto(state->tx_socket, &resp, sizeof(resp), 0, (FAR struct sockaddr *)&addr,
				 sizeof(addr));
...
static int ptp_process_delay_resp(FAR struct ptp_state_s *state, FAR struct ptp_delay_resp_s *msg)
{
	int64_t path_delay;
	int64_t sync_delay;
	...
	if (!state->selected_source_valid ||
		memcmp(msg->header.sourceidentity, state->selected_source.header.sourceidentity,
			   sizeof(msg->header.sourceidentity)) != 0 ||
	...
	if (path_delay >= 0 && path_delay < CONFIG_NETUTILS_PTPD_MAX_PATH_DELAY_NS) {
		...
		state->path_delay_ns += (path_delay - state->path_delay_ns) / state->path_delay_avgcount;
...
static int ptp_process_rx_packet(FAR struct ptp_state_s *state, ssize_t length)
{
...
static void ptp_signal_handler(int signo, FAR siginfo_t *siginfo, FAR void *context)
{
	...
	if (signo == SIGHUP) {
		...
		state->status_req = *(FAR struct ptpd_statusreq_s *)siginfo->si_value.sival_ptr;
	}
}
...
static void ptp_process_statusreq(FAR struct ptp_state_s *state)
{
	...
	if (status->clock_source_valid) {
		...
		status->clock_source_info.variance = ((uint16_t)s->gm_quality[2] << 8) | s->gm_quality[3];

		memcpy(status->clock_source_info.gm_id, s->gm_identity,
...
int ptpd_start(FAR const char *interface)
{
	...
	pid = task_create("PTPD", CONFIG_NETUTILS_PTPD_SERVERPRIO, CONFIG_NETUTILS_PTPD_STACKSIZE,
					  ptp_daemon, task_argv);
