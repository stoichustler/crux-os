...
static void iperf_showusage(FAR const char *progname, FAR struct wifi_iperf_t *args, int exitcode)
{
...
static void iperf_printcfg(FAR struct iperf_cfg_t *cfg)
{
	...
	if (cfg->flag & IPERF_FLAG_LOCAL) {
		...
		printf("sip=%" PRId32 ".%" PRId32 ".%" PRId32 ".%" PRId32 ":%d,"
			   ...
			   (cfg->sip >> 24) & 0xff, cfg->sport, cfg->dip & 0xff, (cfg->dip >> 8) & 0xff,
...
int main(int argc, FAR char *argv[])
{
	...
	iperf_args.interval =
		arg_int0("i", "interval", "<interval>", "seconds between periodic bandwidth reports");
	iperf_args.time =
		arg_int0("t", "time", "<time>", "time in seconds to transmit for (default 10 secs)");
