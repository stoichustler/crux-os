...
typedef CODE int (*iperf_client_func_t)(FAR struct iperf_ctrl_t *ctrl, FAR struct sockaddr *addr,
										socklen_t addrlen);
typedef CODE int (*iperf_server_func_t)(FAR struct iperf_ctrl_t *ctrl, FAR struct sockaddr *addr,
										socklen_t addrlen, FAR struct sockaddr *remote_addr);
...
inline static bool iperf_is_udp_client(FAR struct iperf_ctrl_t *ctrl)
{
	return ((ctrl->cfg.flag & IPERF_FLAG_CLIENT) && (ctrl->cfg.flag & IPERF_FLAG_UDP));
}
...
inline static bool iperf_is_udp_server(FAR struct iperf_ctrl_t *ctrl)
{
	return ((ctrl->cfg.flag & IPERF_FLAG_SERVER) && (ctrl->cfg.flag & IPERF_FLAG_UDP));
}
...
inline static bool iperf_is_tcp_client(FAR struct iperf_ctrl_t *ctrl)
{
	return ((ctrl->cfg.flag & IPERF_FLAG_CLIENT) && (ctrl->cfg.flag & IPERF_FLAG_TCP));
}
...
inline static bool iperf_is_tcp_server(FAR struct iperf_ctrl_t *ctrl)
{
	return ((ctrl->cfg.flag & IPERF_FLAG_SERVER) && (ctrl->cfg.flag & IPERF_FLAG_TCP));
}
...
static void iperf_print_addr(FAR const char *str, FAR struct sockaddr *addr)
{
	...
	case AF_INET: {
		...
		printf("%s: %s:%d\n", str, inet_ntoa(inaddr->sin_addr), htons(inaddr->sin_port));
		return;
	}
	...
	case AF_RPMSG: {
		...
		printf("%s: cpu=%s,name=%s\n", str, rpaddr->rp_cpu, rpaddr->rp_name);
		return;
	}
...
static void iperf_report_task(FAR void *arg)
{
	...
	while (!ctrl->finish) {
		...
		printf("%7.2lf-%7.2lf sec %10ju Bytes %7.2f Mbits/sec\n", ts_diff(&last, &start),
			   ts_diff(&now, &start), now_len - last_len,
			   (((now_len - last_len) * 8) / 1000000.0) / ts_diff(&now, &last));
	...
	if (ts_diff(&now, &start) > 0) {
		printf("%7.2lf-%7.2lf sec %10ju Bytes %7.2f Mbits/sec\n", ts_diff(&start, &start),
			   ts_diff(&now, &start), now_len, ((now_len * 8) / 1000000.0) / ts_diff(&now, &start));
	}
...
static int iperf_start_report(FAR struct iperf_ctrl_t *ctrl)
{
	...
	ret = pthread_create(&thread, &attr, (FAR void *)iperf_report_task, ctrl);
	if (ret != 0) {
...
static int iperf_run_server(FAR struct iperf_ctrl_t *ctrl, iperf_server_func_t server_func)
{
...
static int iperf_run_client(FAR struct iperf_ctrl_t *ctrl, iperf_client_func_t client_func)
{
	if (ctrl->cfg.flag & IPERF_FLAG_LOCAL) {
		struct sockaddr_un addr;
...
static int iperf_tcp_server(FAR struct iperf_ctrl_t *ctrl, FAR struct sockaddr *addr,
							socklen_t addrlen, FAR struct sockaddr *remote_addr)
{
	int actual_recv = 0;
	int want_recv = 0;
...
static int iperf_udp_server(FAR struct iperf_ctrl_t *ctrl, FAR struct sockaddr *addr,
							socklen_t addrlen, FAR struct sockaddr *remote_addr)
{
	int actual_recv = 0;
	struct timeval t;
	...
	while (!ctrl->finish) {
		actual_recv = recvfrom(sockfd, buffer, want_recv, 0, remote_addr, &addrlen);
		if (actual_recv < 0) {
...
static int iperf_udp_client(FAR struct iperf_ctrl_t *ctrl, FAR struct sockaddr *addr,
							socklen_t addrlen)
{
...
static int iperf_tcp_client(FAR struct iperf_ctrl_t *ctrl, FAR struct sockaddr *addr,
							socklen_t addrlen)
{
	FAR uint8_t *buffer;
	int actual_send = 0;
...
int iperf_start(FAR struct iperf_cfg_t *cfg)
{
	...
	ret = pthread_create(&thread, &attr, (FAR void *)iperf_task_traffic, &ctrl);
