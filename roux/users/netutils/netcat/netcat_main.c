...
int do_io(int infd, int outfd, char *buf, size_t buf_size)
{
	ssize_t avail;
	ssize_t written;
...
int netcat_server(int argc, char *argv[])
{
	...
	if ((conn = accept4(id, (struct sockaddr *)&client, &addrlen, SOCK_CLOEXEC)) != -1) {
		result = do_io(conn, outfd, preallocated_iobuf, CONFIG_NETUTILS_NETCAT_BUFSIZE);
	}
...
int netcat_client(int argc, char *argv[])
{
...
#endif
	{
		...
		result = do_io(infd, id, preallocated_iobuf, CONFIG_NETUTILS_NETCAT_BUFSIZE);
	}
...
int main(int argc, FAR char *argv[])
{
	...
	if (2 > argc) {
		fprintf(stderr, "Usage: netcat <destination> [port] [file]\n"
