...
#define CONFIG_NETUTILS_NTPCLIENT_SERVER inet_ntoa(CONFIG_NETUTILS_NTPCLIENT_SERVERIP)
#else
...
static struct ntpc_daemon_s g_ntpc_daemon = {
	NTP_NOT_RUNNING, SEM_INITIALIZER(1), SEM_INITIALIZER(0), -1,
	{NULL, NULL},	 AF_UNSPEC, /* Default is both IPv4 and IPv6 */
};

static struct ntp_sample_s g_last_samples[CONFIG_NETUTILS_NTPCLIENT_NUM_SAMPLES];
...
static time_t ntpc_get_compile_timestamp(void)
{
	...
	year = (pyear[0] - '0') * 1000 + (pyear[1] - '0') * 100 + (pyear[2] - '0') * 10 +
		   (pyear[3] - '0') * 1;
...
static void ntpc_calculate_offset(FAR int64_t *offset, FAR int64_t *delay, uint64_t local_xmittime,
								  uint64_t local_recvtime, FAR const uint8_t *remote_recv,
								  FAR const uint8_t *remote_xmit)
{
	...
	*delay = (local_recvtime - local_xmittime) - (remote_xmittime - remote_recvtime);
}
...
static void ntpc_apply_offset(FAR struct timespec *tp, FAR struct timespec *src, int64_t offset)
{
						 ...
						 FAR struct timespec *start_monotonic)
{
	...
	diffms_real *= 1000;
	diffms_real += (int64_t)(curr_realtime.tv_nsec - start_realtime->tv_nsec) / (1000 * 1000);

	diffms_mono = curr_monotonic.tv_sec - start_monotonic->tv_sec;
	diffms_mono *= 1000;
	diffms_mono += (int64_t)(curr_monotonic.tv_nsec - start_monotonic->tv_nsec) / (1000 * 1000);
	...
	ninfo("Set time to %ju.%03ld seconds (offset: %s%lu.%03lu).\n", (intmax_t)tp.tv_sec,
		  tp.tv_nsec / NSEC_PER_MSEC, offset < 0 ? "-" : "",
...
static bool ntpc_verify_recvd_ntp_datagram(FAR const struct ntp_datagram_s *xmit,
										   FAR const struct ntp_datagram_s *recv, size_t nbytes,
										   FAR const union ntp_addr_u *xmitaddr,
										   FAR const union ntp_addr_u *recvaddr, size_t recvaddrlen)
{
	time_t buildtime;
	time_t seconds;
	...
	if (recvaddr->sa.sa_family == AF_INET6) {
		if (recvaddrlen != sizeof(struct sockaddr_in6) ||
			memcmp(&xmitaddr->in6.sin6_addr, &recvaddr->in6.sin6_addr, sizeof(struct in6_addr)) !=
				0 ||
	...
	if ((int64_t)(ntpc_getuint64(recv->xmittimestamp) - ntpc_getuint64(recv->recvtimestamp)) < 0) {
...
static int ntp_gethostip_multi(FAR const char *hostname, FAR union ntp_addr_u *ipaddr,
							   size_t nipaddr)
{
	...
	ret = getaddrinfo(hostname, STR(CONFIG_NETUTILS_NTPCLIENT_PORTNO), &hints, &info);
	if (ret != OK) {
...
static int ntp_get_next_hostip(FAR struct ntp_servers_s *srvs, FAR union ntp_addr_u *addr)
{
	int ret;
	...
	if (srvs->pos >= srvs->num) {
		...
		if (srvs->hostnext == NULL) {
			...
			srvs->hostnext = strtok_r(srvs->hostlist_str, ";", &srvs->hostlist_saveptr);
#endif
		}
		...
		ret = ntp_gethostip_multi(hostname, srvs->list, nitems(srvs->list));
		if (ret <= 0) {
			return ERROR;
		}
...
static int ntpc_get_ntp_sample(FAR struct ntp_servers_s *srvs, FAR struct ntp_sample_s *samples,
							   int curr_idx)
{
	...
	socklen =
		(server.sa.sa_family == AF_INET) ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6);
	ret = sendto(sd, &xmit, sizeof(struct ntp_datagram_s), 0, &server.sa, socklen);
	if (ret < 0) {
		errval = errno;
	...
	nbytes = recvfrom(sd, &recv, sizeof(struct ntp_datagram_s), 0, &recvaddr.sa, &socklen);
	...
	if (nbytes > 0 &&
		ntpc_verify_recvd_ntp_datagram(&xmit, &recv, nbytes, &server, &recvaddr, socklen)) {
		close(sd);
		sd = -1;
		...
		ntpc_calculate_offset(&sample->offset, &sample->delay, xmit_time, recv_time,
							  recv.recvtimestamp, recv.xmittimestamp);
...
static int ntpc_daemon(int argc, FAR char **argv)
{
	...
	samples = malloc(sizeof(struct ntp_sample_s) * CONFIG_NETUTILS_NTPCLIENT_NUM_SAMPLES);
	if (samples == NULL) {
		return EXIT_FAILURE;
	}
	...
	while (g_ntpc_daemon.state != NTP_STOP_REQUESTED) {
		...
		memset(samples, 0, sizeof(*samples) * CONFIG_NETUTILS_NTPCLIENT_NUM_SAMPLES);
...
{
	FAR char *task_argv[] = {(FAR char *)ntp_server_list, NULL};
	...
	if (g_ntpc_daemon.state == NTP_NOT_RUNNING || g_ntpc_daemon.state == NTP_STOPPED) {
		...
		g_ntpc_daemon.pid =
			task_create("NTP daemon", CONFIG_NETUTILS_NTPCLIENT_SERVERPRIO,
						CONFIG_NETUTILS_NTPCLIENT_STACKSIZE, ntpc_daemon, task_argv);
...
int ntpc_status(struct ntpc_status_s *statusp)
{
	...
	for (i = 0; i < g_last_nsamples; i++) {
		...
		statusp->samples[i].srv_addr =
			(FAR const struct sockaddr *)&statusp->samples[i]._srv_addr_store;
	}
