...
int ipmsfilter(FAR const struct in_addr *interface, FAR const struct in_addr *multiaddr,
			   uint32_t fmode)
{
	int ret = ERROR;

	if (interface != NULL && multiaddr != NULL) {
		ninfo("interface: %08" PRIx32 " muliaddr: %08" PRIx32 " fmode: %" PRId32 "\n",
			  (uint32_t)interface->s_addr, (uint32_t)multiaddr->s_addr, fmode);
