...
static int copy_address(FAR struct sockaddr_storage *dest, FAR void *src, unsigned int addrlen,
						unsigned int maxaddr, sa_family_t family)
{
...
ssize_t netlib_get_route(FAR struct rtentry *rtelist, unsigned int nentries, sa_family_t family)
{
	...
	ret = bind(fd, (FAR const struct sockaddr *)&addr, sizeof(struct sockaddr_nl));
	if (ret < 0) {
		int errcode = errno;
	...
	for (enddump = false; !enddump;) {
		...
		if (u.resp.hdr.nlmsg_len < sizeof(struct nlmsghdr) || u.resp.hdr.nlmsg_len != nrecvd) {
		...
		case RTM_NEWROUTE: {
			...
			attr = &u.resp.attr;
			paylen = u.resp.hdr.nlmsg_len - sizeof(struct nlmsghdr) - sizeof(struct rtmsg);
			...
			for (; RTA_OK(attr, paylen); attr = RTA_NEXT(attr, paylen)) {
				...
				case RTA_DST: /* The destination network */
					ret = copy_address(&dest->rt_dst, RTA_DATA(attr), attrlen, maxaddr, family);
					if (ret < 0) {
						goto errout_with_socket;
					}
				...
				case RTA_GENMASK: /* The network address mask */
					ret = copy_address(&dest->rt_genmask, RTA_DATA(attr), attrlen, maxaddr, family);
					if (ret < 0) {
						goto errout_with_socket;
					}
				...
				case RTA_GATEWAY: /* Route packets via this router */
					ret = copy_address(&dest->rt_gateway, RTA_DATA(attr), attrlen, maxaddr, family);
					if (ret < 0) {
						goto errout_with_socket;
					}
		...
		default:
			fprintf(stderr, "ERROR: Message type %u, length %u\n", u.resp.hdr.nlmsg_type,
					(unsigned)u.resp.hdr.nlmsg_len);
			ret = -EIO;
			goto errout_with_socket;
		}
	}
