...
#define SIZEOF_NETLIB_RECVFROM_RESPONSE_S(n) (sizeof(struct netlib_recvfrom_response_s) + (n)-1)
...
ssize_t netlib_get_nbtable(FAR struct neighbor_entry_s *nbtab, unsigned int nentries)
{
	...
	maxsize = CONFIG_NET_IPv6_NCONF_ENTRIES * sizeof(struct neighbor_entry_s);
	...
	if (resp->hdr.nlmsg_len < sizeof(struct nlmsghdr) || resp->hdr.nlmsg_len > nrecvd) {
