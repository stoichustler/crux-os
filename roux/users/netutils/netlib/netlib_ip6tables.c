...
#define IP6T_FILL_MATCH(e, match_name)                                                             \
	do {                                                                                           \
		strlcpy((e)->match.u.user.name, (match_name), sizeof((e)->match.u.user.name));             \
		(e)->match.u.match_size = offsetof(typeof(*(e)), target) - offsetof(typeof(*(e)), match);  \
	} while (0)
...
static FAR struct ip6t_entry *netlib_ip6t_entry_by_rulenum(FAR struct ip6t_replace *repl,
...
static int netlib_ip6t_insert_internal(FAR struct ip6t_replace **replace,
									   FAR const struct ip6t_entry *entry, enum nf_inet_hooks hook,
...
static void netlib_ip6t_delete_internal(FAR struct ip6t_replace *repl, FAR struct ip6t_entry *entry,
...
FAR struct ip6t_replace *netlib_ip6t_prepare(FAR const char *table)
{
	...
	if (getsockopt(sockfd, IPPROTO_IPV6, IP6T_SO_GET_ENTRIES, entries, &len) < 0) {
...
int netlib_ip6t_commit(FAR const struct ip6t_replace *repl)
{
	...
	ret = setsockopt(sockfd, IPPROTO_IPV6, IP6T_SO_SET_REPLACE, repl, sizeof(*repl) + repl->size);
	if (ret < 0) {
		ret = -errno;
...
int netlib_ip6t_policy(FAR const char *table, enum nf_inet_hooks hook, int verdict)
{
	...
	entry = (FAR struct ip6t_entry *)((uintptr_t)repl->entries + repl->underflow[hook]);
					   ...
					   enum nf_inet_hooks hook)
{
	...
	return netlib_ip6t_insert_internal(repl, entry, hook, (*repl)->underflow[hook]);
}
					   ...
					   enum nf_inet_hooks hook, int rulenum)
{
	...
	ip6t_entry_for_every(e, head, size)
	{
		if (e->next_offset == entry->next_offset && e->target_offset == entry->target_offset &&
			memcmp(&e->ipv6, &entry->ipv6, sizeof(struct ip6t_ip6)) == 0 &&
			memcmp(&e->elems, &entry->elems, e->next_offset - offsetof(struct ip6t_entry, elems)) ==
				0) {
			netlib_ip6t_delete_internal(repl, e, hook);
			return OK;
		}
												...
												uint8_t match_proto)
{
	...
	case IPPROTO_ICMP6: {
		FAR struct ip6t_filter_icmp_entry_s *entry = zalloc(sizeof(*entry));
		if (entry == NULL) {
			return NULL;
		}
