						   ...
						   sa_family_t family)
{
	...
	for (enddump = false; !enddump;) {
		...
		case RTM_NEWLINK: {
			...
			attr = &resp.attr;
			attrlen = resp.hdr.nlmsg_len - sizeof(struct nlmsghdr) - sizeof(struct ifinfomsg);
...
#endif
						strlcpy(devlist[ncopied].ifname, (FAR char *)RTA_DATA(attr), IFNAMSIZ);
						ncopied++;
					}
		...
		default:
			fprintf(stderr, "ERROR: Message type %u, length %lu\n", resp.hdr.nlmsg_type,
					(unsigned long)resp.hdr.nlmsg_len);
			ret = -EIO;
			goto errout_with_socket;
		}
	}
