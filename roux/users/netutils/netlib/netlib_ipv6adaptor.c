							   ...
							   FAR struct in6_addr *srcipaddr)
{
	...
	for (i = 0; i < nintf; i++) {
		...
		FAR struct sockaddr_in6 *addr = (FAR struct sockaddr_in6 *)&req->lifr_addr;
		...
		if (net_ipv6addr_maskcmp(addr->sin6_addr.s6_addr16, destipaddr->s6_addr16,
								 netmask->sin6_addr.s6_addr16)) {
			/* Yes.. return the IP address assigned to this adaptor */

			net_ipv6addr_copy(srcipaddr->s6_addr16, addr->sin6_addr.s6_addr16);
			ret = OK;
			break;
		}
	}
