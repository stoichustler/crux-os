...
#include <roux/net/netfilter/nf_nat.h>
...
static FAR struct ipt_entry *netlib_ipt_entry_by_rulenum(FAR struct ipt_replace *repl,
...
static int netlib_ipt_insert_internal(FAR struct ipt_replace **replace,
									  FAR const struct ipt_entry *entry, enum nf_inet_hooks hook,
...
static void netlib_ipt_delete_internal(FAR struct ipt_replace *repl, FAR struct ipt_entry *entry,
...
int netlib_ipt_commit(FAR const struct ipt_replace *repl)
{
	...
	ret = setsockopt(sockfd, IPPROTO_IP, IPT_SO_SET_REPLACE, repl, sizeof(*repl) + repl->size);
	if (ret < 0) {
		ret = -errno;
...
int netlib_ipt_policy(FAR const char *table, enum nf_inet_hooks hook, int verdict)
{
	...
	entry = (FAR struct ipt_entry *)((uintptr_t)repl->entries + repl->underflow[hook]);
					  ...
					  enum nf_inet_hooks hook)
{
	...
	return netlib_ipt_insert_internal(repl, entry, hook, (*repl)->underflow[hook]);
}
					  ...
					  enum nf_inet_hooks hook, int rulenum)
{
	...
	ipt_entry_for_every(e, head, size)
	{
		if (e->next_offset == entry->next_offset && e->target_offset == entry->target_offset &&
			...
			memcmp(&e->elems, &entry->elems, e->next_offset - offsetof(struct ipt_entry, elems)) ==
				0) {
			netlib_ipt_delete_internal(repl, e, hook);
			return OK;
		}
