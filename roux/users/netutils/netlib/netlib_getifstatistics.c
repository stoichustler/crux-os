...
int netlib_getifstatistics(FAR const char *ifname, FAR struct netdev_statistics_s *stat)
{
	...
	while (fgets(line, sizeof(line), stream) != NULL) {
		...
		if (strstr(line, "RX:") != NULL) {
			...
			if (sscanf(line, "%" SCNx32 "%" SCNx32 "%" SCNx32 "%" SCNx64, &stat->rx_packets,
					   &stat->rx_fragments, &stat->rx_errors, &stat->rx_bytes) <= 0) {
				ret = -EINVAL;
				break;
			}
		}
		...
		if (strstr(line, "TX:") != NULL) {
			...
			if (sscanf(line, "%" SCNx32 "%" SCNx32 "%" SCNx32 "%" SCNx32 "%" SCNx64,
					   &stat->tx_packets, &stat->tx_done, &stat->tx_errors, &stat->tx_timeouts,
					   &stat->tx_bytes) <= 0) {
				ret = -EINVAL;
				break;
			}
