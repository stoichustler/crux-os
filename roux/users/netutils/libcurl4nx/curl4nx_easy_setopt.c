...
int curl4nx_easy_setopt(FAR struct curl4nx_s *handle, int option, FAR void *param)
{
	int ret;
	...
	case CURL4NXOPT_BUFFERSIZE: {
		long len = (long)param;
		if (len < CONFIG_LIBCURL4NX_MINRXBUFLEN || len > CONFIG_LIBCURL4NX_MAXRXBUFLEN) {
			cret = CURL4NXE_BAD_FUNCTION_ARGUMENT;
			break;
		}
	...
	case CURL4NXOPT_MAXREDIRS: {
		...
		if (redirs < -1 || redirs > CONFIG_LIBCURL4NX_MAXREDIRS) {
			cret = CURL4NXE_BAD_FUNCTION_ARGUMENT;
			break;
		}
