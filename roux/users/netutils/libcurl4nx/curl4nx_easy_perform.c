...
static int curl4nx_resolve(FAR struct curl4nx_s *handle, FAR char *hostname, FAR struct in_addr *ip)
{
...
static int curl4nx_is_header(FAR char *buf, int len, FAR const char *header, FAR int *off)
{
...
int curl4nx_easy_perform(FAR struct curl4nx_s *handle)
{
	...
	do {
		...
		ret = connect(handle->sockfd, (struct sockaddr *)&server, sizeof(struct sockaddr_in));
		if (ret < 0) {
		...
		fprintf(stream, "%s %s HTTP/%d.%d\r\n", handle->method, handle->path, handle->version >> 4,
				handle->version & 0x0f);
		...
		while (1) {
			...
			if (state == CURL4NX_STATE_STATUSREASON) {
				...
				while (rxoff < ret) {
					if (handle->rxbuf[rxoff] == 0x0d || handle->rxbuf[rxoff] == 0x0a) {
						...
						if (tmplen == 2) {
							if (tmpbuf[0] == 0x0d && tmpbuf[1] == 0x0a) {
								if (headerlen == 0) /* Found an empty header */
								{
									...
									state = chunked ? CURL4NX_STATE_DATA_CHUNKED
									...
									func(headerbuf, headerlen, 1, handle->headerdata);
									...
									if (curl4nx_is_header(headerbuf, headerlen,
										...
										chunked = !strncasecmp(headerbuf + off, "chunked", 7);
										if (chunked) {
									...
									if (curl4nx_is_header(headerbuf, headerlen,
										...
										if (handle->status >= 300 && handle->status < 400) {
											if (handle->flags & CURL4NX_FLAGS_FOLLOWLOCATION) {
												if ((handle->max_redirs > 0) &&
													(redircount >= handle->max_redirs)) {
													curl4nx_info("Too many redirections\n");
													cret = CURL4NXE_TOO_MANY_REDIRECTS;
													goto close;
												}

												cret = curl4nx_easy_setopt(handle, CURL4NXOPT_URL,
																		   headerbuf + off);
												if (cret != CURL4NXE_OK) {
													goto close;
												}
			...
			if (state == CURL4NX_STATE_DATA_NORMAL) {
				if ((handle->flags & CURL4NX_FLAGS_FAILONERROR) && handle->status >= 400) {
				...
				handle->writefunc(handle->rxbuf + rxoff, ret - rxoff, 1, handle->writedata);
				if (handle->content_length != 0) {
					curl4nx_info("Done %llu of %llu\n", done, handle->content_length);

					if (handle->progressfunc) {
						handle->progressfunc(handle->progressdata, handle->content_length, done, 0,
											 0);
					}
