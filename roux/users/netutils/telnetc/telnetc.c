...
static enum telnet_error_e _error(FAR struct telnet_s *telnet, unsigned line, FAR const char *func,
								  enum telnet_error_e err, int fatal, FAR const char *fmt, ...)
{
...
#if defined(HAVE_ZLIB)
enum telnet_error_e _init_zlib(FAR struct telnet_s *telnet, int deflate, int err_fatal)
{
	FAR z_stream *z;
	int rs;
	...
	if (telnet->z != 0) {
		return _error(telnet, __LINE__, __func__, TELNET_EBADVAL, err_fatal,
					  "cannot initialize compression twice");
	}

	/* Allocate zstream box */

	if ((z = (z_stream *)calloc(1, sizeof(z_stream))) == 0) {
		return _error(telnet, __LINE__, __func__, TELNET_ENOMEM, err_fatal, "malloc() failed: %d",
					  errno);
	}
	...
	if (deflate) {
		if ((rs = deflateInit(z, Z_DEFAULT_COMPRESSION)) != Z_OK) {
			free(z);
			return _error(telnet, __LINE__, __func__, TELNET_ECOMPRESS, err_fatal,
...
static void _send(FAR struct telnet_s *telnet, FAR const char *buffer, size_t size)
{
	union telnet_event_u ev;
	...
	if (telnet->z != 0 && telnet->flags & TELNET_PFLAG_DEFLATE) {
		...
		while (telnet->z->avail_in > 0 || telnet->z->avail_out == 0) {
			...
			if ((rs = deflate(telnet->z, Z_SYNC_FLUSH)) != Z_OK) {
				_error(telnet, __LINE__, __func__, TELNET_ECOMPRESS, 1, "deflate() failed: %s",
					   zError(rs));
				deflateEnd(telnet->z);
				free(telnet->z);
				telnet->z = 0;
...
static inline int _check_telopt(FAR struct telnet_s *telnet, unsigned char telopt, int us)
{
	int i;
...
static inline void _set_rfc1143(FAR struct telnet_s *telnet, unsigned char telopt, char us,
								char him)
{
	...
	qtmp = (struct telnet_rfc1143_s *)realloc(telnet->q, sizeof(struct telnet_rfc1143_s) *
															 (telnet->q_size + 4));
	if (qtmp == 0) {
		_error(telnet, __LINE__, __func__, TELNET_ENOMEM, 0, "realloc() failed: %d", errno);
		return;
	}
...
static inline void _send_negotiate(FAR struct telnet_s *telnet, unsigned char cmd,
								   unsigned char telopt)
{
...
static int _environ_telnet(FAR struct telnet_s *telnet, unsigned char type, FAR char *buffer,
						   size_t size)
{
	union telnet_event_u ev;
	...
	if ((unsigned)buffer[size - 1] == TELNET_ENVIRON_ESC) {
		_error(telnet, __LINE__, __func__, TELNET_EPROTOCOL, 0, "telopt %d subneg ends with ESC",
			   type);
		return 0;
	}
	...
	values = (struct telnet_environ_s *)calloc(count, sizeof(struct telnet_environ_s));
	if (values == 0) {
		_error(telnet, __LINE__, __func__, TELNET_ENOMEM, 0, "calloc() failed: %d", errno);
		return 0;
	}
...
static int _mssp_telnet(FAR struct telnet_s *telnet, FAR char *buffer, size_t size)
{
	union telnet_event_u ev;
...
static int _zmp_telnet(FAR struct telnet_s *telnet, FAR const char *buffer, size_t size)
{
	union telnet_event_u ev;
	FAR const char **argv;
	FAR const char *c;
	...
	if (size == 0 || buffer[size - 1] != 0) {
		_error(telnet, __LINE__, __func__, TELNET_EPROTOCOL, 0, "incomplete ZMP frame");
		return 0;
	}
...
static int _ttype_telnet(FAR struct telnet_s *telnet, FAR const char *buffer, size_t size)
{
	union telnet_event_u ev;
	...
	if (size == 0) {
		_error(telnet, __LINE__, __func__, TELNET_EPROTOCOL, 0, "incomplete TERMINAL-TYPE request");
		return 0;
	}
	...
	if (buffer[0] == TELNET_TTYPE_IS) {
		...
		if ((name = (char *)malloc(size)) == 0) {
			_error(telnet, __LINE__, __func__, TELNET_ENOMEM, 0, "malloc() failed: %d", errno);
			return 0;
		}
								 ...
								 FAR void *user_data)
{
	...
	FAR struct telnet_s *telnet = (FAR struct telnet_s *)calloc(1, sizeof(struct telnet_s));
	if (telnet == 0) {
		return 0;
	}
...
static enum telnet_error_e _buffer_byte(FAR struct telnet_s *telnet, unsigned char byte)
{
	char *new_buffer;
	size_t i;
	...
	if (telnet->buffer_pos == telnet->buffer_size) {
		...
		if (new_buffer == 0) {
			_error(telnet, __LINE__, __func__, TELNET_ENOMEM, 0, "realloc() failed");
...
static void _process(FAR struct telnet_s *telnet, FAR const char *buffer, size_t size)
{
	union telnet_event_u ev;
	unsigned char byte;
	size_t start;
...
void telnet_recv(FAR struct telnet_s *telnet, FAR const char *buffer, size_t size)
{
#if defined(HAVE_ZLIB)
	...
	if (telnet->z != 0 && !(telnet->flags & TELNET_PFLAG_DEFLATE)) {
		...
		while (telnet->z->avail_in > 0 || telnet->z->avail_out == 0) {
			...
			if (rs == Z_OK || rs == Z_STREAM_END) {
				...
				_error(telnet, __LINE__, __func__, TELNET_ECOMPRESS, 1, "inflate() failed: %s",
					   zError(rs));
			}
...
void telnet_negotiate(FAR struct telnet_s *telnet, unsigned char cmd, unsigned char telopt)
{
	struct telnet_rfc1143_s q;
...
void telnet_send(FAR struct telnet_s *telnet, FAR const char *buffer, size_t size)
{
	size_t l;
	size_t i;
...
{
#if defined(HAVE_ZLIB)
	static const unsigned char compress2[] = {TELNET_IAC, TELNET_SB, TELNET_TELOPT_COMPRESS2,
...
int telnet_vprintf(FAR struct telnet_s *telnet, FAR const char *fmt, va_list va)
{
...
int telnet_raw_vprintf(FAR struct telnet_s *telnet, FAR const char *fmt, va_list va)
{
	char buffer[1024];
	FAR char *output = buffer;
	int rs;
...
void telnet_newenviron_value(FAR struct telnet_s *telnet, unsigned char type, const char *string)
{
...
void telnet_send_zmp(FAR struct telnet_s *telnet, size_t argc, FAR const char **argv)
{
	size_t i;
