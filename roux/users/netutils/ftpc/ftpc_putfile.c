...
static int ftpc_sendbinary(FAR struct ftpc_session_s *session, FAR FILE *linstream)
{
...
static int ftpc_sendtext(FAR struct ftpc_session_s *session, FAR FILE *linstream)
{
	int ch;
	int ret = OK;
	FILE *routstream = session->data.outstream;
...
static int ftpc_sendfile(struct ftpc_session_s *session, const char *path, FILE *stream,
