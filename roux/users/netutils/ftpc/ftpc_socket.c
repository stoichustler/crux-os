...
int ftpc_sockaccept(FAR struct ftpc_socket_s *acceptor, FAR struct ftpc_socket_s *sock)
{
	...
	addrlen = sizeof(addr);
	sock->sd = accept4(acceptor->sd, (struct sockaddr *)&addr, &addrlen, SOCK_CLOEXEC);
	if (sock->sd == -1) {
...
int ftpc_socklisten(FAR struct ftpc_socket_s *sock)
{
	...
	if (sock->laddr.sa.sa_family == AF_INET6) {
		...
		ret = bind(sock->sd, (struct sockaddr *)&sock->laddr, sizeof(struct sockaddr_in6));
	} else
#endif
#ifdef CONFIG_NET_IPv4
		if (sock->laddr.sa.sa_family == AF_INET) {
		sock->laddr.in4.sin_port = 0;
		ret = bind(sock->sd, (struct sockaddr *)&sock->laddr, sizeof(struct sockaddr_in));
	} else
#endif
	{
	...
	/* Then get the local address selected by ROUX */
...
int ftpc_sockgetsockname(FAR struct ftpc_socket_s *sock, FAR union ftpc_sockaddr_u *addr)
{
	socklen_t len;
	int ret;
