...
static int ftp_cmd_epsv(FAR struct ftpc_session_s *session, FAR union ftpc_sockaddr_u *addr)
{
	char *ptr;
	int nscan;
	int ret;
...
static int ftp_cmd_pasv(FAR struct ftpc_session_s *session, FAR union ftpc_sockaddr_u *addr)
{
	int tmpap[6];
	char *ptr;
	int nscan;
	...
	nscan = sscanf(ptr, "%d,%d,%d,%d,%d,%d", &tmpap[0], &tmpap[1], &tmpap[2], &tmpap[3], &tmpap[4],
				   &tmpap[5]);
	if (nscan != 6) {
...
static FAR char *ftpc_abspath(FAR struct ftpc_session_s *session, FAR const char *relpath,
							  FAR const char *homedir, FAR const char *curdir)
{
	FAR char *ptr = NULL;
...
int ftpc_xfrinit(FAR struct ftpc_session_s *session)
{
	...
	if (FTPC_IS_PASSIVE(session)) {
		...
		ret = ftpc_cmd(session, "PORT %d,%d,%d,%d,%d,%d", paddr[0], paddr[1], paddr[2], paddr[3],
					   pport[0], pport[1]);
#else
#ifdef CONFIG_NET_IPv6
		if (session->dacceptor.laddr.sa.sa_family == AF_INET6) {
			...
			ret =
				ftpc_cmd(session, "EPRT |2|%s|%d|", ipstr, session->dacceptor.laddr.in6.sin6_port);
		} else
			...
			ret = ftpc_cmd(session, "EPRT |1|%s|%d|", ipstr, session->dacceptor.laddr.in4.sin_port);
		} else
...
{
	FAR char *absrpath = ftpc_abspath(session, relpath, session->homerdir, session->currdir);
	ninfo("%s -> %s\n", relpath, absrpath);
	return absrpath;
}
...
{
	FAR char *abslpath = ftpc_abspath(session, relpath, session->homeldir, session->curldir);
	ninfo("%s -> %s\n", relpath, abslpath);
	return abslpath;
}
