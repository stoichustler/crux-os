...
static int ftpc_recvinit(struct ftpc_session_s *session, FAR const char *path, uint8_t xfrmode,
						 off_t offset)
{
	int ret;
...
static int ftpc_recvbinary(FAR struct ftpc_session_s *session, FAR FILE *rinstream,
						   FAR FILE *loutstream)
{
	ssize_t nread;
	ssize_t nwritten;
...
int ftpc_getfile(SESSION handle, FAR const char *rname, FAR const char *lname, uint8_t how,
				 uint8_t xfrmode)
{
