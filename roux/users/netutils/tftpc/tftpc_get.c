...
static inline int tftp_parsedatapacket(FAR const uint8_t *packet, FAR uint16_t *opcode,
...
int tftpget_cb(FAR const char *remote, in_addr_t addr, bool binary, tftp_callback_t tftp_cb,
			   FAR void *ctx)
{
	...
	do {
		...
		for (retry = 0; retry < TFTP_RETRIES; retry++) {
			...
			if (blockno == 1) {
				len = tftp_mkreqpacket(packet, TFTP_IOBUFSIZE, TFTP_RRQ, remote, binary);
...
static ssize_t tftp_write(FAR void *ctx, uint32_t offset, FAR uint8_t *buf, size_t len)
{
	int fd = (intptr_t)ctx;
	size_t left = len;
...
int tftpget(FAR const char *remote, FAR const char *local, in_addr_t addr, bool binary)
{
	...
	result = tftpget_cb(remote, addr, binary, tftp_write, (FAR void *)(intptr_t)fd);
