...
int tftp_mkdatapacket(off_t offset, FAR uint8_t *packet, uint16_t blockno, tftp_callback_t tftp_cb,
					  FAR void *ctx)
{
	int nbytesread;
	...
	nbytesread = tftp_cb(ctx, offset, &packet[TFTP_DATAHEADERSIZE], TFTP_DATASIZE);
	if (nbytesread < 0) {
		return ERROR;
	}
...
static int tftp_rcvack(int sd, FAR uint8_t *packet, FAR struct sockaddr_in *server,
					   FAR uint16_t *port, FAR uint16_t *blockno)
{
			...
			if (nbytes < TFTP_ACKHEADERSIZE) {
				...
				if (opcode != TFTP_ACK) {
					...
					if (opcode == TFTP_ERR) {
						...
						packetlen =
							tftp_mkerrpacket(packet, TFTP_ERR_ILLEGALOP, TFTP_ERRST_ILLEGALOP);
						tftp_sendto(sd, packet, packetlen, server);
					}
...
int tftpput_cb(FAR const char *remote, in_addr_t addr, bool binary, tftp_callback_t cb,
			   FAR void *ctx)
{
	...
	retry = 0;
	for (;;) {
		packetlen = tftp_mkreqpacket(packet, TFTP_IOBUFSIZE, TFTP_WRQ, remote, binary);
...
static ssize_t tftp_read(FAR void *ctx, uint32_t offset, FAR uint8_t *buf, size_t buflen)
{
	int fd = (intptr_t)ctx;
	off_t tmp;
	ssize_t nbytesread;
...
int tftpput(FAR const char *local, FAR const char *remote, in_addr_t addr, bool binary)
{
	...
	result = tftpput_cb(remote, addr, binary, tftp_read, (FAR void *)(intptr_t)fd);
