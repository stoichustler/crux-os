...
struct discover_state_s g_state = {{CONFIG_DISCOVER_DEVICE_CLASS, CONFIG_DISCOVER_DESCR}};
...
static inline void discover_initresponse(void)
{
	...
	strlcpy((char *)&g_state.response[2], g_state.info.description, DISCOVER_RESPONSE_SIZE - 2);
...
static inline int discover_respond(in_addr_t *ipaddr)
{
	...
	ret = sendto(sockfd, &g_state.response, sizeof(g_state.response), 0, (struct sockaddr *)&addr,
...
int discover_start(struct discover_info_s *info)
{
	...
	pid = task_create("Discover daemon", CONFIG_DISCOVER_PRIORITY, CONFIG_DISCOVER_STACK_SIZE,
					  discover_daemon, NULL);
	if (pid < 0) {
		int errval = errno;
