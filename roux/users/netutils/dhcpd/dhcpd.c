...
#define OK (0)
#else
#include <roux/config.h>	/* ROUX configuration */
#include <debug.h>			/* For nerr, info */
#include <roux/compiler.h>	/* For CONFIG_CPP_HAVE_WARNING */
...
static struct dhcpd_daemon_s g_dhcpd_daemon = {DHCPD_NOT_RUNNING, SEM_INITIALIZER(1),
											   SEM_INITIALIZER(0), -1, NULL};

static struct dhcpd_config_s g_dhcpd_config = {CONFIG_NETUTILS_DHCPD_STARTIP,
...
struct lease_s *dhcpd_setlease(const uint8_t *mac, in_addr_t ipaddr, time_t expiry)
{
	...
	ninfo("ipaddr: %08" PRIx32 " ipaddr: %08" PRIx32 " ndx: %d MAX: %d\n", (uint32_t)ipaddr,
		  (uint32_t)g_dhcpd_config.ds_startip, ndx, CONFIG_NETUTILS_DHCPD_MAXLEASES);
...
static inline in_addr_t dhcp_leaseipaddr(FAR struct lease_s *lease)
{
	...
	return (in_addr_t)(lease - g_state.ds_leases) + g_dhcpd_config.ds_startip;
}
...
{
	if (ipaddr >= g_dhcpd_config.ds_startip && ipaddr <= g_dhcpd_config.ds_endip) {
...
static inline int dhcpd_socket(FAR const char *interface)
{
	...
	optval = 1;
	ret = setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (FAR void *)&optval, sizeof(int));
	if (ret < 0) {
	...
	optval = 1;
	ret = setsockopt(sockfd, SOL_SOCKET, SO_BROADCAST, (FAR void *)&optval, sizeof(int));
	if (ret < 0) {
...
static int dhcpd_sendpacket(int sockfd, int bbroadcast)
{
	...
	return sendto(sockfd, &g_state.ds_outpacket, len, 0, (struct sockaddr *)&addr,
...
static inline int dhcpd_sendoffer(int sockfd, in_addr_t ipaddr, uint32_t leasetime)
{
	in_addr_t netaddr;
#ifdef HAVE_DNSIP
	uint32_t dnsaddr;
...
#ifdef HAVE_NETMASK
	dhcpd_addoption32(DHCP_OPTION_SUBNET_MASK, htonl(g_dhcpd_config.ds_netmask));
#endif
#ifdef HAVE_ROUTERIP
	dhcpd_addoption32(DHCP_OPTION_ROUTER, htonl(g_dhcpd_config.ds_routerip));
#endif
#ifdef HAVE_DNSIP
...
static inline int dhcpd_request(int sockfd)
{
	...
	if (lease) {
		...
		ipaddr = dhcp_leaseipaddr(lease);
		ninfo("Lease ipaddr: %08" PRIx32 " Server IP: %08" PRIx32 " Requested IP: %08" PRIx32 "\n",
			  (uint32_t)ipaddr, (uint32_t)g_state.ds_optserverip, (uint32_t)g_state.ds_optreqip);
	 ...
	 */
		...
		ninfo("Server IP: %08" PRIx32 " Requested IP: %08" PRIx32 "\n",
			  (uint32_t)g_state.ds_optserverip, (uint32_t)g_state.ds_optreqip);
...
static inline int dhcpd_openlistener(FAR const char *interface)
{
	...
	g_state.ds_serverip = ((FAR struct sockaddr_in *)&req.ifr_addr)->sin_addr.s_addr;
	...
	if (ret < 0) {
		nerr("ERROR: bind failed, port=%d addr=%08lx: %d\n", addr.sin_port,
			 (long)addr.sin_addr.s_addr, errno);
		close(sockfd);
		return ERROR;
	}
...
int dhcpd_start(FAR const char *interface)
{
	...
	if (g_dhcpd_daemon.ds_state == DHCPD_NOT_RUNNING || g_dhcpd_daemon.ds_state == DHCPD_STOPPED) {
		/* Start the DHCPD daemon */

		g_dhcpd_daemon.ds_state = DHCPD_STARTED;
		pid = task_create("DHCPD daemon", CONFIG_NETUTILS_DHCPD_PRIORITY,
						  CONFIG_NETUTILS_DHCPD_STACKSIZE, dhcpd_task_run, argv);
...
int dhcpd_stop(void)
{
	...
	sem_wait(&g_dhcpd_daemon.ds_lock);
	if (g_dhcpd_daemon.ds_state == DHCPD_STARTED || g_dhcpd_daemon.ds_state == DHCPD_RUNNING) {
		...
		do {
			...
			ret = kill(g_dhcpd_daemon.ds_pid, CONFIG_NETUTILS_DHCPD_SIGWAKEUP);
