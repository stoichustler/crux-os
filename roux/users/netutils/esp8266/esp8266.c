...
#error                                                                                             \
	"CONFIG_NETUTILS_ESP8266_WORKER_BUF_LEN would be bigger than CONFIG_NETUTILS_ESP8266_MAXRXLEN"
#endif
...
static int lesp_read(int timeout_ms)
{
	...
	ninfo("lesp_read %d=>%s and and = %d\n", ret, g_lesp_state.bufans, g_lesp_state.and);
...
int lesp_read_ans_ok(int timeout_ms)
{
	...
	end = time(NULL) + (timeout_ms / 1000) + LESP_TIMEOUT_FLOODING_OFFSET_S;

	while (g_lesp_state.and != LESP_OK) {
		ret = lesp_read(timeout_ms);

		if ((ret < 0) || (g_lesp_state.and == LESP_ERR) || (time(NULL) > end)) {
			ret = -1;
			break;
		}
			...
			if (c == '\n') {
				...
				if (rxlen != 0) {
					if (strcmp(worker->rxbuf, "OK") == 0) {
						...
						worker->and = LESP_ERR;
					} else if ((rxlen == 8) && (memcmp(worker->rxbuf + 1, ",CLOSED", 7) == 0)) {
...
static inline int lesp_create_worker(int priority)
{
	...
	if (ret < 0) {
		...
		ret = pthread_create(&g_lesp_state.worker.thread, (ret < 0) ? NULL : &thread_attr,
							 lesp_worker, NULL);
		if (ret < 0) {
...
int lesp_set_net(lesp_mode_t mode, in_addr_t ip, in_addr_t mask, in_addr_t gateway)
{
	...
	if (ret >= 0) {
		ret = lesp_ask_ans_ok(LESP_TIMEOUT_MS,
							  "AT+CIP%s_CUR=\"%u.%u.%u.%u\","
							  "\"%u.%u.%u.%u\",\"%u.%u.%u.%u\"\r\n",
							  (mode == LESP_MODE_STATION) ? "STA" : "AP", ip4_addr1(ip),
							  ip4_addr2(ip), ip4_addr3(ip), ip4_addr4(ip), ip4_addr1(gateway),
							  ip4_addr2(gateway), ip4_addr3(gateway), ip4_addr4(gateway),
	...
	if (!g_lesp_state.is_initialized) {
		...
		for (i = 0; i < CON_NBR; i++) {
			if ((g_lesp_state.sockets[i].flags & FLAGS_SOCK_USED) == 0) {
				...
				g_lesp_state.sockets[i].rcv_timeo.tv_sec = LESP_TIMEOUT_MS_RECV_S;
...
int lesp_connect(int sockfd, FAR const struct sockaddr *addr, socklen_t addrlen)
{
	...
	if (ret >= 0) {
		ret = lesp_ask_ans_ok(LESP_TIMEOUT_MS,
							  "AT+CIPSTART=%d,\"%s\","
							  "\"%u.%u.%u.%u\",%d\r\n",
							  sockfd, proto_str, ip4_addr1(ip), ip4_addr2(ip), ip4_addr3(ip),
							  ip4_addr4(ip), port);
		if (ret < 0) {
			errno = EIO;
		}
...
int lesp_setsockopt(int sockfd, int level, int option, FAR const void *value, socklen_t value_len)
{
	int ret = 0;
	lesp_socket_t *sock;
...
int lesp_getsockopt(int sockfd, int level, int option, FAR void *value, FAR socklen_t *value_len)
{
	int ret = 0;
	pthread_mutex_lock(&g_lesp_state.mutex);
...
FAR struct hostent *lesp_gethostbyname(FAR const char *hostname)
{
	...
	if (ret >= 0) {
		...
		ret = lesp_parse_cwdomain_ans_line(g_lesp_state.bufans, &g_lesp_state.in_addr);
