...
static int send_wol(FAR struct ether_addr *dest, FAR struct sockaddr_in *s_in)
{
	int ret;
	unsigned int i;
	int sockfd;
	int optval = 1;
	uint8_t buffer[WOL_FRAME_SIZE] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
	...
	ret = sendto(sockfd, buffer, WOL_FRAME_SIZE, 0, (FAR struct sockaddr *)s_in, sizeof(*s_in));
	if (ret < 0) {
...
int main(int argc, FAR char *argv[])
{
	...
	if (optind >= argc || ether_aton_r(argv[optind], &destmac) == NULL) {
		goto errout_with_usage;
	}
