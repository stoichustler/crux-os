...
static int ping_gethostip(FAR const char *hostname, FAR struct in_addr *dest)
{
	...
	static const struct addrinfo s_hint = {.ai_family = AF_INET};
...
static void icmp_callback(FAR struct ping_result_s *result, int code, long extra)
{
	result->code = code;
	result->extra = extra;
...
void icmp_ping(FAR const struct ping_info_s *info)
{
	...
	while (result.nrequests < info->count) {
		...
		priv->nsent = sendto(priv->sockfd, iobuffer, result.outsize, 0,
							 (FAR struct sockaddr *)&priv->destaddr, sizeof(struct sockaddr_in));
		if (priv->nsent < 0) {
		...
		do {
			...
			ret = poll(&priv->recvfd, 1, info->timeout - priv->elapsed / USEC_PER_MSEC);
			if (ret < 0) {
			...
			priv->nrecvd = recvfrom(priv->sockfd, iobuffer, result.outsize, 0,
									(FAR struct sockaddr *)&priv->fromaddr, &priv->addrlen);
			if (priv->nrecvd < 0) {
