...
static void icmp6_callback(FAR struct ping6_result_s *result, int code, long extra)
{
	result->code = code;
	result->extra = extra;
...
void icmp6_ping(FAR const struct ping6_info_s *info)
{
	...
	while (result.nrequests < info->count) {
		...
		start = clock();
		nsent = sendto(sockfd, iobuffer, result.outsize, 0, (FAR struct sockaddr *)&destaddr,
		...
		do {
			...
			nrecvd = recvfrom(sockfd, iobuffer, result.outsize, 0, (FAR struct sockaddr *)&fromaddr,
							  &addrlen);
			if (nrecvd < 0) {
