...
bool httpd_is_file(FAR const char *filename)
{
	...
	snprintf(path, CONFIG_NAME_MAX, "%s/%s", CONFIG_NETUTILS_HTTPD_PATH, filename);
...
ssize_t httpd_dirlist(int outfd, FAR struct httpd_fs_file *file)
{
	...
	if ('\0' == *path) {
		...
		snprintf(tmp, BUF_SIZE, HEADER TITLE TABLE PARENT, path, path);
	}
	...
	while (true) {
		...
		if (dent->d_type == DTYPE_DIRECTORY) {
			snprintf(tmp, BUF_SIZE, ENTRY, dent->d_name, "/", dent->d_name, "/", tm.tm_year + 1900,
					 tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, "-");
		} else {
			snprintf(size, sizeof(size), "%jd", (uintmax_t)buf.st_size);

			snprintf(tmp, BUF_SIZE, ENTRY, dent->d_name, "", dent->d_name, "", tm.tm_year + 1900,
					 tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, size);
		}
...
errout_with_hdr:
	...
	snprintf(tmp, BUF_SIZE, FOOTER, info.sysname, info.release, info.version, info.machine,
			 BOARD_NAME);
