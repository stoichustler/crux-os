...
#define dhcpv6_for_each_option(_o, start, end, otype, olen, odata)                                 \
	for ((_o) = (FAR uint8_t *)(start);                                                            \
		 (_o) + 4 <= (FAR uint8_t *)(end) && ((otype) = (_o)[0] << 8 | (_o)[1]) &&                 \
		 ((odata) = (FAR void *)&(_o)[4]) &&                                                       \
...
struct dhcp6c_retx_s {
	...
	uint16_t max_timeo;
	char name[8];
	int (*handler_reply)(FAR void *handle, enum dhcpv6_msg_e orig, FAR const void *opt,
						 FAR const void *end, uint32_t elapsed);
...
static int dhcp6c_handle_reconfigure(FAR void *handle, enum dhcpv6_msg_e orig, FAR const void *opt,
									 FAR const void *end, uint32_t elapsed);
static int dhcp6c_handle_advert(FAR void *handle, enum dhcpv6_msg_e orig, FAR const void *opt,
								FAR const void *end, uint32_t elapsed);
static int dhcp6c_commit_advert(FAR void *handle, uint32_t elapsed);
static int dhcp6c_handle_reply(FAR void *handle, enum dhcpv6_msg_e orig, FAR const void *opt,
							   FAR const void *end, uint32_t elapsed);
static int dhcp6c_handle_rebind_reply(FAR void *handle, enum dhcpv6_msg_e orig, FAR const void *opt,
									  FAR const void *end, uint32_t elapsed);
...
static FAR uint8_t *dhcp6c_resize_state(FAR void *handle, enum dhcpv6_state_e state, ssize_t len)
{
...
static void dhcp6c_add_state(FAR void *handle, enum dhcpv6_state_e state, FAR const void *data,
							 size_t len)
{
...
static size_t dhcp6c_remove_state(FAR void *handle, enum dhcpv6_state_e state, size_t offset,
								  size_t len)
{
	FAR struct dhcp6c_state_s *pdhcp6c = (FAR struct dhcp6c_state_s *)handle;
...
static bool dhcp6c_commit_state(FAR void *handle, enum dhcpv6_state_e state, size_t old_len)
{
	FAR struct dhcp6c_state_s *pdhcp6c = (FAR struct dhcp6c_state_s *)handle;
	...
	if (new_len != 0 || old_len != 0) {
		upd = (new_len != old_len) || (memcmp(old_data, new_data, new_len) != 0);
...
static FAR void *dhcp6c_get_state(FAR void *handle, enum dhcpv6_state_e state, FAR size_t *len)
{
	FAR struct dhcp6c_state_s *pdhcp6c = (FAR struct dhcp6c_state_s *)handle;
...
static void dhcp6c_get_result(FAR void *handle, FAR struct dhcp6c_state *presult)
{
	FAR struct dhcp6c_state_s *pdhcp6c = (FAR struct dhcp6c_state_s *)handle;
	size_t s_len;
	FAR uint8_t *s_data;
...
static void dhcp6c_set_iov(FAR struct iovec *piov, FAR void *base, size_t len)
{
	piov->iov_base = base;
	piov->iov_len = len;
}

static void dhcp6c_send(FAR void *handle, enum dhcpv6_msg_e type, uint8_t trid[3], uint32_t ecs)
{
	FAR struct dhcp6c_state_s *pdhcp6c = (FAR struct dhcp6c_state_s *)handle;
	char fqdn_buf[256];
	struct {
	...
	struct iovec iov[11];
	size_t cnt;
	struct sockaddr_in6 dest = {AF_INET6, htons(DHCPV6_SERVER_PORT), 0, DHCPV6_ALL_RELAYS,
								pdhcp6c->ifindex};
...
static bool dhcp6c_response_is_valid(FAR void *handle, FAR const void *buf, ssize_t len,
									 const uint8_t transaction[3], enum dhcpv6_msg_e type)
{
	FAR struct dhcp6c_state_s *pdhcp6c = (FAR struct dhcp6c_state_s *)handle;
	...
	if (len < sizeof(*rep) || memcmp(rep->tr_id, transaction, sizeof(rep->tr_id)) != 0) {
		return false;
	}

	if (type == DHCPV6_MSG_SOLICIT) {
		if (rep->msg_type != DHCPV6_MSG_ADVERT && rep->msg_type != DHCPV6_MSG_REPLY) {
			return false;
		}
	} else if (type == DHCPV6_MSG_UNKNOWN) {
		if (!pdhcp6c->accept_reconfig || rep->msg_type != DHCPV6_MSG_RECONF) {
			return false;
		}
...
static int dhcp6c_command(FAR void *handle, enum dhcpv6_msg_e type)
{
	...
	uint32_t timeout =
		CONFIG_NETUTILS_DHCP6C_REQUEST_TIMEOUT < 3 ? 3 : CONFIG_NETUTILS_DHCP6C_REQUEST_TIMEOUT;
	...
	do {
		...
		for (; len < 0 && round_start < round_end; round_start = dhcp6c_get_milli_time()) {
			/* Set timeout for receiving */

			uint64_t t = round_end - round_start;
			struct timeval retime = {t / MSEC_PER_SEC, (t % MSEC_PER_SEC) * MSEC_PER_SEC};
			...
			setsockopt(pdhcp6c->sockfd, SOL_SOCKET, SO_RCVTIMEO, &retime, sizeof(retime));
			...
			if (len > 0) {
				...
				if (retx->handler_reply != NULL) {
					len = retx->handler_reply(handle, type, opt, opt_end, elapsed / MSEC_PER_SEC);
				}
			}
		}
								...
								FAR const void *end, uint32_t elapsed)
{
	...
	dhcpv6_for_each_option(ite0, opt, end, otype, olen, odata)
	{
		if (otype == DHCPV6_OPT_SERVERID && olen <= 130) {
		...
		} else if (otype == DHCPV6_OPT_PREF && olen >= 1 && cand.preference >= 0) {
			...
			dhcpv6_for_each_option(ite1, &h[1], oend, otype, olen, d)
			{
				if (otype == DHCPV6_OPT_IA_PREFIX) {
					cand.preference += 2000;
				} else if (otype == DHCPV6_OPT_STATUS && olen >= 2 && d[0] == 0 &&
...
static int dhcp6c_commit_advert(FAR void *handle, uint32_t elapsed)
{
	...
	FAR struct dhcpv6_server_cand_s *cand = dhcp6c_get_state(handle, STATE_SERVER_CAND, &cand_len);
	bool retry = false;
	...
	if (c != NULL) {
		uint16_t hdr[2] = {htons(DHCPV6_OPT_SERVERID), htons(c->duid_len)};
...
static time_t dhcp6c_parse_ia(FAR void *handle, FAR void *opt, FAR void *end)
{
	...
	dhcpv6_for_each_option(ite0, opt, end, otype, olen, odata)
	{
		if (otype == DHCPV6_OPT_IA_PREFIX) {
			...
			dhcpv6_for_each_option(ite1, pd, pd + pd_len, stype, slen, sdata)
			{
				if (memcmp(sdata + 8, odata + 8, sizeof(local->addr) + 1) == 0) {
			...
			dhcpv6_for_each_option(ite1, na, na + na_len, stype, slen, sdata)
			{
							   ...
							   FAR const void *end, uint32_t elapsed)
{
	...
	dhcpv6_for_each_option(ite0, opt, end, otype, olen, odata)
	{
		if ((otype == DHCPV6_OPT_IA_PD || otype == DHCPV6_OPT_IA_NA) &&
			...
			dhcpv6_for_each_option(ite1, &ia_hdr[1], odata + olen, stype, slen, sdata)
			{
				if (stype == DHCPV6_OPT_STATUS && slen >= 2 && (sdata[0] || sdata[1])) {
					continue;
				}
			}
			...
			dhcpv6_for_each_option(ite1, odata, odata + olen, stype, slen, sdata)
			{
	...
	if (opt != NULL) {
		...
		have_update |= dhcp6c_commit_state(handle, STATE_SNTP_IP, sntp_ip_len);
		have_update |= dhcp6c_commit_state(handle, STATE_SNTP_FQDN, sntp_dns_len);
		have_update |= dhcp6c_commit_state(handle, STATE_SIP_IP, sip_ip_len);
		have_update |= dhcp6c_commit_state(handle, STATE_SIP_FQDN, sip_fqdn_len);
		dhcp6c_get_state(handle, STATE_IA_PD, &new_ia_pd_len);
		dhcp6c_get_state(handle, STATE_IA_NA, &new_ia_na_len);
		have_update |= (new_ia_pd_len != ia_pd_len) || (new_ia_na_len != ia_na_len);
	}
	...
	dhcpv6_for_each_option(ite0, ia_pd, ia_end, otype, olen, odata)
	{
		...
		while (!p->valid) {
			ia_end = ia_pd +
					 dhcp6c_remove_state(handle, STATE_IA_PD, (FAR uint8_t *)p - ia_pd, olen + 4);
			have_update = true;
		}
	}
	...
	dhcpv6_for_each_option(ite0, ia_na, ia_end, otype, olen, odata)
	{
		...
		while (!p->valid) {
			ia_end = ia_na +
					 dhcp6c_remove_state(handle, STATE_IA_NA, (FAR uint8_t *)p - ia_na, olen + 4);
			have_update = true;
		}
	}
									 ...
									 FAR const void *end, uint32_t elapsed)
{
	...
	dhcpv6_for_each_option(ite, opt, end, otype, olen, odata)
	{
		if (otype == DHCPV6_OPT_RECONF_MESSAGE && olen == 1 &&
			(odata[0] == DHCPV6_MSG_RENEW || odata[0] == DHCPV6_MSG_INFO_REQ)) {
			msg = odata[0];
		}
	}
	...
	if (mode == DHCPV6_STATELESS) {
		...
		while (!pdhcp6c->cancel) {
			...
			if (ret >= 0) {
				...
				if (ret < 0 || (ia_pd_new == 0 && ia_pd_len) || (ia_na_new == 0 && ia_na_len)) {
					break;
				} else if (ret >= 0) {
...
static FAR void *dhcp6c_precise_open(FAR const char *ifname, enum dhcp6c_ia_mode_e ia_mode,
									 bool request_pd, uint16_t opt[], int cnt)
{
	...
	size_t client_id_len;
	int val = 1;
	uint16_t oro[] = {htons(DHCPV6_OPT_DNS_SERVERS), htons(DHCPV6_OPT_DNS_DOMAIN),
					  htons(DHCPV6_OPT_NTP_SERVER), htons(DHCPV6_OPT_SIP_SERVER_A),
	...
	if (client_id_len == 0) {
		uint8_t duid[14] = {0, DHCPV6_OPT_CLIENTID, 0, 10, 0, DHCPV6_DUID_LLADDR, 0, 1};
		...
		if (memcmp(&duid[8], zero, ETHER_ADDR_LEN) == 0 &&
			...
			for (ifp = ifc.ifc_req; ifp < ifend && memcmp(&duid[8], zero, 6) == 0; ifp++) {
				memcpy(ifr.ifr_name, ifp->ifr_name, sizeof(ifr.ifr_name));
				ioctl(pdhcp6c->sockfd, SIOCGIFHWADDR, &ifr);
				memcpy(&duid[8], ifr.ifr_hwaddr.sa_data, ETHER_ADDR_LEN);
			}
		}
	...
	setsockopt(pdhcp6c->sockfd, SOL_SOCKET, SO_BINDTODEVICE, ifname, strlen(ifname));

	memset(&client_addr, 0, sizeof(client_addr));
	client_addr.sin6_family = AF_INET6;
	client_addr.sin6_port = htons(DHCPV6_CLIENT_PORT);

	if (bind(pdhcp6c->sockfd, (struct sockaddr *)&client_addr, sizeof(client_addr)) != 0) {
		close(pdhcp6c->urandom_fd);
		close(pdhcp6c->sockfd);
		free(pdhcp6c);
		return NULL;
