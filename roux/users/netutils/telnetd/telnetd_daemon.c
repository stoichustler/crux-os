...
int telnetd_daemon(FAR const struct telnetd_config_s *config)
{
	 ...
	 * generated when a child process terminates.  On both Linux and ROUX, a
	...
	if (config->d_family == AF_INET) {
		...
		memset(addr.ipv6.sin6_addr.s6_addr, 0, sizeof(addr.ipv6.sin6_addr.s6_addr));
	} else
#endif
	{
	...
	for (;;) {
		...
		if (ioctl(drvrfd, SIOCTELNET, (unsigned long)((uintptr_t)&session)) < 0) {
			nerr("ERROR: open(/dev/telnet) failed: %d\n", errno);
			close(drvrfd);
			goto errout_with_acceptsd;
		}
		...
		if (config->t_entry) {
			pid_t pid = task_create("Telnet session", config->t_priority, config->t_stacksize,
									config->t_entry, NULL);
			if (pid >= 0) {
				continue;
			}
		...
		if (config->t_path) {
			...
			ret = posix_spawnp(&pid, config->t_path, NULL, &attr, config->t_argv, NULL);
			if (ret > 0) {
