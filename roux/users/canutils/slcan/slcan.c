...
static int caninit(char *candev, int *s, struct sockaddr_can *addr, char *ctrlmsg,
				   struct canfd_frame *frame, struct msghdr *msg, struct iovec *iov)
{
	struct ifreq ifr;
...
int main(int argc, char *argv[])
{
	...
	debug_print("Starting slcan on ROUX\n");
			...
			if (FD_ISSET(s, &rdfs)) {
				...
				if (nbytes == CAN_MTU) {
					...
					if (frame.can_id & CAN_EFF_FLAG) {
						...
						snprintf(sbuf, sizeof(sbuf), "T%08" PRIx32 "%d", frame.can_id, frame.len);
						sbp = &sbuf[10];
					} else {
						/* 11 bit address */

						snprintf(sbuf, sizeof(sbuf), "t%03" PRIx32 "%d", frame.can_id, frame.len);
						sbp = &sbuf[5];
					}

					for (i = 0; i < frame.len; i++) {
						snprintf(sbp, sizeof(sbuf) - (sbp - sbuf), "%02X", frame.data[i]);
						sbp += 2;
					}
