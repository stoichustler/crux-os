...
#define LINEAR_MAP(x, in_min, in_max, out_min, out_max)                                            \
	(b16divb16(b16mulb16((x - in_min), (out_max - out_min)), (in_max - in_min)) + out_min)
...
static int foc_angle_onfo_run_b16(FAR foc_angle_b16_t *h, FAR struct foc_angle_in_b16_s *in,
								  ...
								  FAR struct foc_angle_out_b16_s *out)
{
	...
	duty_now = b16mulb16(b16sign(in->state->vdq.q), vector2d_mag_b16(v_dq_mod.d, v_dq_mod.q));

	/* Update and the observer gain. */

	dyn_gain = b16mulb16(LINEAR_MAP(b16abs(duty_now), 0, b16ONE,
									b16mulb16(ob->cfg.gain, ob->cfg.gain_slow), ob->cfg.gain),
						 b16HALF);

	/* Update observer */

	motor_aobserver_nfo_b16(&ob->o, &in->state->iab, &in->state->vab, &ob->cfg.phy, dyn_gain);

	/* Copy data */

	out->type = FOC_ANGLE_TYPE_ELE;
	out->angle = b16mulb16(ob->sensor_dir, motor_aobserver_angle_get_b16(&ob->o));
