...
static void foc_model_pmsm_elerun_b16(FAR foc_model_b16_t *h, FAR ab_frame_b16_t *v_ab);
...
static int foc_model_pmsm_cfg_b16(FAR foc_model_b16_t *h, FAR void *cfg)
{
	...
	pmsm_phy_params_init_b16(&phy, model->cfg.poles, model->cfg.res, model->cfg.ind,
							 model->cfg.iner, model->cfg.flux_link, model->cfg.ind_q,
							 model->cfg.ind_d);
									 ...
									 FAR struct foc_model_state_b16_s *state)
{
	...
	state->curr_raw[0] = b16toi(b16mulb16(state->curr[0], model->one_by_iphadc));
	state->curr_raw[1] = b16toi(b16mulb16(state->curr[1], model->one_by_iphadc));
	state->curr_raw[2] = b16toi(b16mulb16(state->curr[2], model->one_by_iphadc));
}
