...
int foc_routine_ident_run_b16(FAR foc_routine_b16_t *r, FAR struct foc_routine_in_b16_s *in,
...
int foc_ident_idle_run_b16(FAR struct foc_ident_b16_s *ident, FAR struct foc_routine_in_b16_s *in,
						   FAR struct foc_routine_out_b16_s *out)
{
...
int foc_ident_res_run_b16(FAR struct foc_ident_b16_s *ident, FAR struct foc_routine_in_b16_s *in,
						  FAR struct foc_routine_out_b16_s *out)
{
	int ret = FOC_ROUTINE_RUN_NOTDONE;
	b16_t err = 0;
	...
	if (ident->cntr > (ident->cfg.res_steps / 3)) {
		ident->volt_sum += vector2d_mag_b16(in->foc_state->vdq.q, in->foc_state->vdq.d);
		ident->curr_sum += vector2d_mag_b16(in->foc_state->idq.q, in->foc_state->idq.d);
	}
	...
	if (ident->cntr > ident->cfg.res_steps) {
		...
		ident->final.res =
			b16divb16(b16mulb16(ftob16(2.0f / 3.0f), ident->volt_sum), ident->curr_sum);
...
int foc_ident_ind_run_b16(FAR struct foc_ident_b16_s *ident, FAR struct foc_routine_in_b16_s *in,
						  FAR struct foc_routine_out_b16_s *out)
{
	int ret = FOC_ROUTINE_RUN_NOTDONE;
	b16_t vref = 0;
...
int foc_routine_ident_cfg_b16(FAR foc_routine_b16_t *r, FAR void *cfg)
{
	...
	if (i->cfg.res_steps <= 0 || i->cfg.ind_steps <= 0 || i->cfg.idle_steps <= 0) {
		ret = -EINVAL;
		goto errout;
	}
