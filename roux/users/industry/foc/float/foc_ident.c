...
int foc_routine_ident_run_f32(FAR foc_routine_f32_t *r, FAR struct foc_routine_in_f32_s *in,
...
int foc_ident_idle_run_f32(FAR struct foc_ident_f32_s *ident, FAR struct foc_routine_in_f32_s *in,
						   FAR struct foc_routine_out_f32_s *out)
{
...
int foc_ident_res_run_f32(FAR struct foc_ident_f32_s *ident, FAR struct foc_routine_in_f32_s *in,
						  FAR struct foc_routine_out_f32_s *out)
{
	int ret = FOC_ROUTINE_RUN_NOTDONE;
	float err = 0.0f;
	...
	if (ident->cntr > (ident->cfg.res_steps / 3)) {
		ident->volt_sum += vector2d_mag(in->foc_state->vdq.q, in->foc_state->vdq.d);
		ident->curr_sum += vector2d_mag(in->foc_state->idq.q, in->foc_state->idq.d);
	}
...
int foc_ident_ind_run_f32(FAR struct foc_ident_f32_s *ident, FAR struct foc_routine_in_f32_s *in,
						  FAR struct foc_routine_out_f32_s *out)
{
	int ret = FOC_ROUTINE_RUN_NOTDONE;
	float vref = 0.0f;
							   ...
							   FAR struct foc_routine_out_f32_s *out)
{
	...
	if (ident->cfg.cb.angle != NULL) {
		...
		out->angle = ident->cfg.cb.angle(ident->cfg.cb.priv_angle, ident->vel, DIR_CW);
	}

	if (ident->cfg.cb.speed != NULL) {
		/* Update openloop speed by speed callbacks */

		ident->vel = ident->cfg.cb.speed(ident->cfg.cb.priv_speed, ident->cfg.flux_vel, ident->vel);
	}
	...
	if (ident->cntr > ident->cfg.flux_steps) {
		volt_avg = vector2d_mag(ident->vq_sum / ident->aveg_sum, ident->vd_sum / ident->aveg_sum);
		curr_avg = vector2d_mag(ident->iq_sum / ident->aveg_sum, ident->id_sum / ident->aveg_sum);

		/* Get flux linkage */

		ident->final.flux =
			(volt_avg - ident->final.res * curr_avg) / ident->vel - ident->final.ind * curr_avg;
...
#ifdef CONFIG_INDUSTRY_FOC_IDENT_FLUX
		if (i->cntr == 0) {
			ret = i->cfg.cb.kpki(i->cfg.cb.priv_kpki, i->final.res, i->final.ind, i->cfg.per);
			if (ret < 0) {
