...
static int foc_angle_qe_run_f32(FAR foc_angle_f32_t *h, FAR struct foc_angle_in_f32_s *in,
...
static int foc_angle_qe_zero_f32(FAR foc_angle_f32_t *h)
{
	...
	ret = ioctl(qe->fd, QEIOC_POSITION, (unsigned long)((uintptr_t)&qe->offset));
	if (ret < 0) {
								...
								FAR struct foc_angle_out_f32_s *out)
{
	...
	qe->angle = (qe->sensor_dir * (qe->pos - qe->offset) * qe->one_by_posmax * MOTOR_ANGLE_M_MAX);
