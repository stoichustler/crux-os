						...
						FAR struct foc_handler_output_f32_s *out)
{
	...
	case FOC_HANDLER_MODE_CURRENT: {
		...
		h->ops.ctrl->current_run(h, in->dq_ref, in->vdq_comp, &v_ab_mod);
	...
	case FOC_HANDLER_MODE_VOLTAGE: {
		...
		h->ops.ctrl->voltage_run(h, in->dq_ref, &v_ab_mod);
...
void foc_handler_state_print_f32(FAR struct foc_state_f32_s *state)
{
	...
	FOCLIBLOG("curr = [%.2f %.2f %.2f]\n", state->curr[0], state->curr[1], state->curr[2]);
	FOCLIBLOG("volt = [%.2f %.2f %.2f]\n", state->volt[0], state->volt[1], state->volt[2]);
#else
#error TODO
#endif

	FOCLIBLOG("iab = [%.2f %.2f]\n", state->iab.a, state->iab.b);
	FOCLIBLOG("vab = [%.2f %.2f]\n", state->vab.a, state->vab.b);
	FOCLIBLOG("idq = [%.2f %.2f]\n", state->idq.d, state->idq.q);
	FOCLIBLOG("vdq = [%.2f %.2f]\n", state->vdq.d, state->vdq.q);
}
#endif
