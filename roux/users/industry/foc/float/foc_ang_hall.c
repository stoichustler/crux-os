...
static int foc_angle_hl_run_f32(FAR foc_angle_f32_t *h, FAR struct foc_angle_in_f32_s *in,
...
static float g_sector_angle[7] = {0 * HALL_ANGLE_STEP, 1 * HALL_ANGLE_STEP, 2 * HALL_ANGLE_STEP,
								  3 * HALL_ANGLE_STEP, 4 * HALL_ANGLE_STEP, 5 * HALL_ANGLE_STEP,
								  6 * HALL_ANGLE_STEP};
		...
		if (hl->per_acc > 0.0f) {
			...
			if (hl->sector * hl->sector_last != 0) {
				...
				if (hl->sector_diff * hl->sector_diff_last > 0) {
					...
					hl->vel_est = (hl->sector_diff * HALL_ANGLE_STEP / hl->per_acc);
				} else {
