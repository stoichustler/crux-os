...
static int foc_angle_onfo_run_f32(FAR foc_angle_f32_t *h, FAR struct foc_angle_in_f32_s *in,
								  ...
								  FAR struct foc_angle_out_f32_s *out)
{
	...
	dyn_gain =
		LINEAR_MAP(fabsf(duty_now), 0.0f, 1.0f, ob->cfg.gain * ob->cfg.gain_slow, ob->cfg.gain) *
		0.5f;

	/* Update observer */

	motor_aobserver_nfo(&ob->o, &in->state->iab, &in->state->vab, &ob->cfg.phy, dyn_gain);
