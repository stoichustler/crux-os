...
bool abnt_codi_parse(FAR const uint8_t *data, FAR struct abnt_codi_proto_s *proto)
{
	...
	proto->bill_indicator = (data[1] & BILL_INDICADOR) == BILL_INDICADOR ? true : false;

	/* Reactive indicator, it is invered at end of reactive interval */

	proto->react_interval = (data[1] & REACT_INTERVAL) == REACT_INTERVAL ? true : false;

	/* Verify if capacitive pulses are used to calculate consumption */

	proto->react_cap_pulse = (data[1] & REACT_CAP_PULSE) == REACT_CAP_PULSE ? true : false;

	/* Verify if inductive pulses are used to calculate consumption */

	proto->react_ind_pulse = (data[1] & REACT_IND_PULSE) == REACT_IND_PULSE ? true : false;
	...
	proto->react_charge_en = (data[2] & CHARGES_REACT_EN) == CHARGES_REACT_EN ? true : false;
