...
static int socket_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int close_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int connect_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int sendto_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int recvfrom_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int setsockopt_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int getsockopt_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int getsockname_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int getpeername_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int ioctl_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int bind_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int listen_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
static int accept_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf);
...
static int _send_ack_common(int fd, uint16_t events, uint32_t xid,
...
static FAR struct usock_s *gs2200m_socket_get(FAR struct gs2200m_s *priv, int sockid)
{
...
static FAR struct usock_s *gs2200m_find_socket_by_cid(FAR struct gs2200m_s *priv, char cid)
{
...
static ssize_t read_req(int fd, FAR const struct usrsock_request_common_s *com_hdr, FAR void *req,
						size_t reqsize)
{
	ssize_t rlen;

	rlen = read(fd, (uint8_t *)req + sizeof(*com_hdr), reqsize - sizeof(*com_hdr));
...
static int usrsock_handle_request(int fd, FAR struct gs2200m_s *priv)
{
	...
	rlen = read_req(fd, com_hdr, &req, handlers[com_hdr->reqid].hdrlen);
...
static int usock_send_event(int fd, FAR struct gs2200m_s *priv, FAR struct usock_s *usock,
							int events)
{
...
static void usock_sendevent_toall(FAR struct gs2200m_s *priv, int fd)
{
	...
	for (i = 0; i < SOCKET_COUNT; i++) {
		if (priv->sockets[i].state != CLOSED) {
			usock_send_event(fd, priv, &priv->sockets[i], USRSOCK_EVENT_RECVFROM_AVAIL);
		}
	}
}
...
static int socket_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf)
{
	...
	} else if (!priv->usock_enable && req->domain == AF_INET && req->type != SOCK_CTRL) {
...
static int connect_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf)
{
	...
	snprintf(cmsg.addr, sizeof(cmsg.addr), "%s", inet_ntoa(addr.sin_addr));
	snprintf(cmsg.port, sizeof(cmsg.port), "%d", ntohs(addr.sin_port));

	cmsg.cid = 'z'; /* set to invalid */
	cmsg.type = usock->type;

	ret = ioctl(priv->gsfd, GS2200M_IOC_CONNECT, (unsigned long)&cmsg);
...
static int sendto_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf)
{
	...
	if (usock->type == SOCK_DGRAM) {
		...
		gs2200m_printf("%s: addr: %s:%d", __func__, inet_ntoa(smsg.addr.sin_addr),
	...
	if (req->buflen > 0) {
		...
		nret = ioctl(priv->gsfd, GS2200M_IOC_SEND, (unsigned long)&smsg);
...
prepare:
	...
	wlen = usock_send_event(fd, priv, usock, USRSOCK_EVENT_SENDTO_READY);
...
static int recvfrom_request(int fd, FAR struct gs2200m_s *priv, FAR void *hdrbuf)
{
	...
	if (0 < req->max_buflen) {
		...
		ret = ioctl(priv->gsfd, GS2200M_IOC_RECV, (unsigned long)&rmsg);
	}
	...
	if (!rmsg.is_tcp) {
		gs2200m_printf("%s: from (%s:%d)\n", __func__, inet_ntoa(rmsg.addr.sin_addr),
...
prepare:
	...
	if (0 <= ret) {
		...
		resp.valuelen = MIN(resp.valuelen_nontrunc, req->max_addrlen);

		if ((0 == rmsg.len) && (0 != rmsg.reqlen)) {
			usock_send_event(fd, priv, usock, USRSOCK_EVENT_REMOTE_CLOSED);
...
prepare:
	...
	if (resp.valuelen > 0) {
		...
		usock_send_event(fd, priv, new_usock, USRSOCK_EVENT_SENDTO_READY);
	}
		...
		if (fds[1].revents & POLLIN) {
			...
			if (cid == DISASSOCIATION_CID) {
				...
				if (NULL == usock) {
					...
					usock_send_event(fd[0], priv, usock, USRSOCK_EVENT_RECVFROM_AVAIL);
				}
			}
		}
	}
...
{
	fprintf(stderr, "Usage: %s [-a [ch]] ssid passphrase(key) \n\n", cmd);
	fprintf(stderr, "AP mode : specify -a option (optionally with channel) with ssid\n"
					"          and 8 to 63 ascii passphrase for WPA2-PSK\n"
					"          or 10 hex digits key for WEP\n");
	fprintf(stderr, "STA mode: specify ssid and passphrase for WPA/WPA2 PSK\n");
}
...
int main(int argc, FAR char *argv[])
{
	...
	if ((ap_mode && (4 != argc) && (5 != argc)) || (!ap_mode && 3 != argc)) {
		_show_usage(argv[0]);
		ret = ERROR;
		goto errout;
	}
