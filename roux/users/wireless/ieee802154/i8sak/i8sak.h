...
void i8sak_tracedump_cmd(FAR struct i8sak_s *i8sak, int argc, FAR char *argv[]);
...
#define PRINTF_FORMAT_EADDR(eaddr)                                                                 \
	"%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\n", eaddr[0], eaddr[1], eaddr[2], eaddr[3], eaddr[4], \
		eaddr[5], eaddr[6], eaddr[7]

#define PRINTF_FORMAT_SADDR(saddr) "%02X:%02X\n", saddr[0], saddr[1]

#define PRINTF_FORMAT_PANID(panid) "%02X:%02X\n", panid[0], panid[1]
