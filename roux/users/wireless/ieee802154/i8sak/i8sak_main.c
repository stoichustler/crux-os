...
int i8sak_requestdaemon(FAR struct i8sak_s *i8sak)
{
	...
	if (++i8sak->daemonusers == 1) {
		...
		if (i8sak->mode == I8SAK_MODE_CHAR) {
			...
			snprintf(daemonname, I8SAK_DAEMONNAME_PREFIX_LEN + (len - 5), I8SAK_DAEMONNAME_FMT,
					 &i8sak->ifname[5]);
		}
#ifdef CONFIG_NET_6LOWPAN
		else if (i8sak->mode == I8SAK_MODE_NETIF) {
			len = strlen(i8sak->ifname);
			snprintf(daemonname, I8SAK_DAEMONNAME_PREFIX_LEN + len, I8SAK_DAEMONNAME_FMT,
					 i8sak->ifname);
		}
#endif
		...
		i8sak->daemon_pid = task_create(daemonname, CONFIG_IEEE802154_I8SAK_PRIORITY,
										CONFIG_IEEE802154_I8SAK_STACKSIZE, i8sak_daemon, NULL);
...
static int i8sak_setup(FAR struct i8sak_s *i8sak, FAR const char *ifname)
{
	...
	for (i = 0; i < IEEE802154_EADDRSIZE; i++) {
		i8sak->ep_addr.eaddr[i] = (CONFIG_IEEE802154_I8SAK_DEFAULT_EP_EADDR >> (i * 8)) & 0xff;
	}

	for (i = 0; i < IEEE802154_SADDRSIZE; i++) {
		i8sak->ep_addr.saddr[i] = (CONFIG_IEEE802154_I8SAK_DEFAULT_EP_SADDR >> (i * 8)) & 0xff;
	}

	for (i = 0; i < IEEE802154_PANIDSIZE; i++) {
		i8sak->ep_addr.panid[i] = (CONFIG_IEEE802154_I8SAK_DEFAULT_EP_PANID >> (i * 8)) & 0xff;
	}
	...
	for (i = 0; i < IEEE802154_SADDRSIZE; i++) {
		i8sak->next_saddr[i] = ((CONFIG_IEEE802154_I8SAK_DEFAULT_EP_SADDR + 1) >> (i * 8)) & 0xff;
	}
...
static int i8sak_daemon(int argc, FAR char *argv[])
{
	...
	while (!i8sak->daemon_shutdown) {
		...
		if (i8sak->startblaster) {
			...
			ret = pthread_create(&i8sak->blaster_threadid, NULL, i8sak_blaster_thread, i8sak);
			if (ret != 0) {
		...
		if (i8sak->startsniffer) {
			...
			ret = pthread_create(&i8sak->sniffer_threadid, NULL, i8sak_sniffer_thread, i8sak);
			if (ret != 0) {
...
int main(int argc, FAR char *argv[])
{
	...
	if (argc > 1) {
		...
		if ((strncmp(argv[argind], "/dev/", 5) == 0) || (strncmp(argv[argind], "wpan", 4) == 0)) {
