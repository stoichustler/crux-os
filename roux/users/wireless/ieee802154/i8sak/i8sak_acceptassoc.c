...
static void acceptassoc_eventcb(FAR struct ieee802154_primitive_s *primitive, FAR void *arg);
...
void i8sak_acceptassoc_cmd(FAR struct i8sak_s *i8sak, int argc, FAR char *argv[])
{
	...
	while ((option = getopt(argc, argv, ":he:")) != ERROR) {
		...
		case 'h':
			fprintf(stderr,
					"Starts accepting association requests\n"
	...
	i8sak_eventlistener_addreceiver(i8sak, acceptassoc_eventcb, &filter, !i8sak->acceptall);
}
