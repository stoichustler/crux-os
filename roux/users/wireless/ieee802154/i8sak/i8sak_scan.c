...
void i8sak_scan_cmd(FAR struct i8sak_s *i8sak, int argc, FAR char *argv[])
{
	...
	while ((option = getopt(argc, argv, ":hpae")) != ERROR) {
		...
		case 'h':
			fprintf(stderr,
					"Requests association with endpoint\n"
...
static void scan_eventcb(FAR struct ieee802154_primitive_s *primitive, FAR void *arg)
{
	...
	if (scan->type == IEEE802154_SCANTYPE_ACTIVE || scan->type == IEEE802154_SCANTYPE_PASSIVE) {
		...
		for (i = 0; i < scan->numresults; i++) {
			...
			printf("    PAN ID: " PRINTF_FORMAT_PANID(scan->pandescs[i].coordaddr.panid));

			if (scan->pandescs[i].coordaddr.mode == IEEE802154_ADDRMODE_SHORT) {
				printf("    Coordinator Short Address: " PRINTF_FORMAT_SADDR(
					scan->pandescs[i].coordaddr.saddr));
			} else {
				printf("    Coordinator Extended Address: " PRINTF_FORMAT_EADDR(
					scan->pandescs[i].coordaddr.eaddr));
			}
		}
	}
