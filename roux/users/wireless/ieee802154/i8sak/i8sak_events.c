...
static pthread_addr_t i8sak_eventthread(pthread_addr_t arg)
{
	...
	if (i8sak->mode == I8SAK_MODE_CHAR) {
		macarg.enable = true;
		ioctl(i8sak->fd, MAC802154IOC_ENABLE_EVENTS, (unsigned long)((uintptr_t)&macarg));
	}
#ifdef CONFIG_NET_6LOWPAN
	else if (i8sak->mode == I8SAK_MODE_NETIF) {
		netarg.u.enable = true;
		strlcpy(netarg.ifr_name, i8sak->ifname, IFNAMSIZ);
		ioctl(i8sak->fd, MAC802154IOC_ENABLE_EVENTS, (unsigned long)((uintptr_t)&netarg));
	}
#endif

	while (i8sak->eventlistener_run) {
		if (i8sak->mode == I8SAK_MODE_CHAR) {
			ret = ioctl(i8sak->fd, MAC802154IOC_GET_EVENT, (unsigned long)((uintptr_t)&macarg));
			primitive = &macarg.primitive;
		}
#ifdef CONFIG_NET_6LOWPAN
		else if (i8sak->mode == I8SAK_MODE_NETIF) {
			ret = ioctl(i8sak->fd, MAC802154IOC_GET_EVENT, (unsigned long)((uintptr_t)&netarg));
		...
		receiver = (FAR struct i8sak_eventreceiver_s *)sq_peek(&i8sak->eventreceivers);
		...
		while (receiver != NULL) {
			...
			receiver = (FAR struct i8sak_eventreceiver_s *)sq_next((FAR sq_entry_t *)receiver);
		}
...
int i8sak_eventlistener_start(FAR struct i8sak_s *i8sak)
{
	...
	ret = pthread_create(&i8sak->eventlistener_threadid, NULL, i8sak_eventthread, i8sak);
	if (ret != 0) {
...
int i8sak_eventlistener_addreceiver(FAR struct i8sak_s *i8sak, i8sak_eventcallback_t cb,
									FAR struct i8sak_eventfilter_s *filter, bool oneshot)
{
	FAR struct i8sak_eventreceiver_s *receiver;
	int ret;
	...
	receiver = (FAR struct i8sak_eventreceiver_s *)sq_remfirst(&i8sak->eventreceivers_free);
	if (receiver == NULL) {
...
int i8sak_eventlistener_removereceiver(FAR struct i8sak_s *i8sak, i8sak_eventcallback_t cb)
{
	FAR struct i8sak_eventreceiver_s *receiver;
	int ret;
