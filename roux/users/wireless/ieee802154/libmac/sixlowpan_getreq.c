...
int sixlowpan_get_req(int sock, FAR const char *ifname, FAR struct ieee802154_get_req_s *req)
{
	...
	ret = ioctl(sock, MAC802154IOC_MLME_GET_REQUEST, (unsigned long)((uintptr_t)&arg));
	if (ret < 0) {
		ret = -errno;
...
int sixlowpan_getdevmode(int sock, FAR const char *ifname, FAR enum ieee802154_devmode_e *devmode)
{
	struct ieee802154_get_req_s req;
	int ret;
