...
int sixlowpan_assoc_resp(int sock, FAR const char *ifname, FAR struct ieee802154_assoc_resp_s *resp)
{
	...
	ret = ioctl(sock, MAC802154IOC_MLME_ASSOC_RESPONSE, (unsigned long)((uintptr_t)&arg));
	if (ret < 0) {
		ret = -errno;
