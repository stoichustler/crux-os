...
static int i8shark_init(FAR struct i8shark_state_s *i8shark)
{
	...
	strlcpy(i8shark->devpath, CONFIG_IEEE802154_I8SHARK_DEVPATH, sizeof(i8shark->devpath));
...
static int i8shark_daemon(int argc, FAR char *argv[])
{
	...
	while (!g_i8shark.daemon_shutdown) {
		...
		memcpy(&zepframe[i], (frame.payload + frame.offset + 2), (frame.length - frame.offset - 2));
...
int main(int argc, FAR char *argv[])
{
	...
	g_i8shark.daemon_pid =
		task_create("i8shark", CONFIG_IEEE802154_I8SHARK_DAEMON_PRIORITY,
					CONFIG_IEEE802154_I8SHARK_DAEMON_STACKSIZE, i8shark_daemon, NULL);
