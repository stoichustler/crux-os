...
static void btsak_info_showusage(FAR const char *progname, FAR const char *cmd, int exitcode)
{
...
void btsak_cmd_info(FAR struct btsak_s *btsak, int argc, FAR char *argv[])
{
	...
	if (sockfd >= 0) {
		...
		if (ret < 0) {
			...
			printf("Device: %s\n", btsak->ifname);
			printf("BDAddr: %02x:%02x:%02x:%02x:%02x:%02x\n", btreq.btr_bdaddr.val[5],
				   btreq.btr_bdaddr.val[4], btreq.btr_bdaddr.val[3], btreq.btr_bdaddr.val[2],
