...
static void btsak_scan_showusage(FAR const char *progname, FAR const char *cmd, int exitcode)
{
...
static void btsak_cmd_scanstart(FAR struct btsak_s *btsak, FAR char *cmd, int argc,
								FAR char *argv[])
{
	...
	if (sockfd >= 0) {
		ret = ioctl(sockfd, SIOCBTSCANSTART, (unsigned long)((uintptr_t)&btreq));
		if (ret < 0) {
...
static void btsak_cmd_scanget(FAR struct btsak_s *btsak, FAR char *cmd, int argc, FAR char *argv[])
{
	struct btreq_s btreq;
	...
	sockfd = btsak_socket(btsak);
	if (sockfd >= 0) {
		ret = ioctl(sockfd, SIOCBTSCANGET, (unsigned long)((uintptr_t)&btreq));
		if (ret < 0) {
		...
		/* Show scan results */
			...
			for (i = 0; i < btreq.btr_nrsp; i++) {
				...
				printf("%2d.\taddr:            "
					   ...
					   i + 1, rsp->sr_addr.val[5], rsp->sr_addr.val[4], rsp->sr_addr.val[3],
					   rsp->sr_addr.val[2], rsp->sr_addr.val[1], rsp->sr_addr.val[0],
					   rsp->sr_addr.type);
				printf("\trssi:            %d\n", rsp->sr_rssi);
...
static void btsak_cmd_scanstop(FAR struct btsak_s *btsak, FAR char *cmd, int argc, FAR char *argv[])
{
	struct btreq_s btreq;
	int sockfd;
	int ret;
	...
	sockfd = btsak_socket(btsak);
	if (sockfd >= 0) {
		ret = ioctl(sockfd, SIOCBTSCANSTOP, (unsigned long)((uintptr_t)&btreq));
		if (ret < 0) {
