...
ifeq ($(CONFIG_INTERPRETERS_WAMR_BUILD_MODULES_FOR_ROUX),y)
include $(APPDIR)/tools/WASI-SDK.defs
include $(APPDIR)/interpreters/wamr/Toolchain.defs
...
define LINK_WASM
	$(if $(wildcard $(APPDIR)/wasm/*), \
	  $(foreach bin,$(wildcard $(APPDIR)/wasm/*.wo), \
	    ...
	    $(eval WLDFLAGS=$(shell cat $(APPDIR)/wasm/$(PROGNAME).ldflags)) \
	    $(eval RETVAL=$(shell $(WCC) $(bin) $(WBIN) $(WCFLAGS) $(WLDFLAGS) $(WCC_COMPILER_RT_LIB) \
	        -Wl,--Map=$(APPDIR)/wasm/$(PROGNAME).map \
	        -o $(BINDIR)/wasm/$(PROGNAME).wasm || echo 1;)) \
	    $(if $(RETVAL), \
