...
int mbedtls_aes_setkey_enc(FAR mbedtls_aes_context *ctx, FAR const unsigned char *key,
...
int mbedtls_aes_setkey_dec(FAR mbedtls_aes_context *ctx, FAR const unsigned char *key,
						   unsigned int keybits)
{
...
int mbedtls_aes_crypt_ecb(FAR mbedtls_aes_context *ctx, int mode, const unsigned char input[16],
						  unsigned char output[16])
{
	...
	ctx->dev.crypt.op = mode == MBEDTLS_AES_ENCRYPT ? COP_ENCRYPT : COP_DECRYPT;
...
int mbedtls_aes_crypt_ctr(FAR mbedtls_aes_context *ctx, size_t length, FAR size_t *nc_off,
						  unsigned char nonce_counter[16], unsigned char stream_block[16],
						  FAR const unsigned char *input, FAR unsigned char *output)
{
	int ret;
	...
	memcpy(ctx->key + ctx->dev.session.keylen, nonce_counter, NONCE_LENGTH);
...
int mbedtls_aes_xts_setkey_enc(FAR mbedtls_aes_xts_context *ctx, FAR const unsigned char *key,
							   unsigned int keybits)
{
...
int mbedtls_aes_xts_setkey_dec(FAR mbedtls_aes_xts_context *ctx, FAR const unsigned char *key,
							   unsigned int keybits)
{
	return mbedtls_aes_xts_setkey_enc(ctx, key, keybits);
}

int mbedtls_aes_crypt_xts(FAR mbedtls_aes_xts_context *ctx, int mode, size_t length,
						  const unsigned char data_unit[16], FAR const unsigned char *input,
...
int mbedtls_aes_crypt_cfb128(FAR mbedtls_aes_context *ctx, int mode, size_t length, size_t *iv_off,
...
int mbedtls_aes_crypt_cfb8(FAR mbedtls_aes_context *ctx, int mode, size_t length,
...
int mbedtls_aes_crypt_ofb(FAR mbedtls_aes_context *ctx, size_t length, size_t *iv_off,
						  unsigned char iv[16], const unsigned char *input, unsigned char *output)
{
	int ret;
