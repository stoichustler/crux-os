...
#include <sys/param.h>
#include <sys/types.h>
#include <roux/math/math_ioctl.h>
#include <roux/math/mpi.h>
...
static inline void mbedtls_mpi_to_mpiparam(FAR struct mpiparam *a, FAR const mbedtls_mpi *A)
{
...
static inline void mpiparam_to_mbedtls_mpi(FAR mbedtls_mpi *A, FAR const struct mpiparam *a)
{
	A->n = a->n / sizeof(mbedtls_mpi_uint);
	A->s = a->s;
...
int mbedtls_mpi_add_mpi(FAR mbedtls_mpi *X, FAR const mbedtls_mpi *A, FAR const mbedtls_mpi *B)
{
	int ret;
	int fd;
	...
	mbedtls_mpi_grow(X, MBEDTLS_ROUNDUP(MAX(A->n, B->n) + 1, sizeof(mbedtls_mpi_uint)));
...
int mbedtls_mpi_sub_mpi(FAR mbedtls_mpi *X, FAR const mbedtls_mpi *A, FAR const mbedtls_mpi *B)
{
	int ret;
	int fd;
	struct mpi_calc_s mpi;
...
int mbedtls_mpi_mul_mpi(FAR mbedtls_mpi *X, FAR const mbedtls_mpi *A, FAR const mbedtls_mpi *B)
{
	int ret;
	int fd;
	struct mpi_calc_s mpi;
	...
	mbedtls_mpi_grow(X, MBEDTLS_ROUNDUP(A->n + B->n, sizeof(mbedtls_mpi_uint)));
	mbedtls_mpi_to_mpiparam(&mpi.param[2], X);
	ret = ioctl(fd, MATHIOC_MPI_CALC, (unsigned long)((uintptr_t)&mpi));
	if (ret >= 0) {
		mpiparam_to_mbedtls_mpi(X, &mpi.param[2]);
...
int mbedtls_mpi_div_mpi(FAR mbedtls_mpi *Q, FAR mbedtls_mpi *R, FAR const mbedtls_mpi *A,
...
int mbedtls_mpi_mod_mpi(FAR mbedtls_mpi *R, FAR const mbedtls_mpi *A, FAR const mbedtls_mpi *B)
{
	int ret;
	int fd;
	struct mpi_calc_s mpi;
...
int mbedtls_mpi_exp_mod(FAR mbedtls_mpi *X, FAR const mbedtls_mpi *A, FAR const mbedtls_mpi *E,
						FAR const mbedtls_mpi *N, FAR mbedtls_mpi *)
{
	int ret;
	int fd;
	struct mpi_calc_s mpi;
...
int mbedtls_mpi_gcd(FAR mbedtls_mpi *G, FAR const mbedtls_mpi *A, FAR const mbedtls_mpi *B)
{
	int ret;
	int fd;
	struct mpi_calc_s mpi;
...
int mbedtls_mpi_inv_mod(FAR mbedtls_mpi *X, FAR const mbedtls_mpi *A, FAR const mbedtls_mpi *N)
{
	int ret;
	int fd;
	struct mpi_calc_s mpi;
