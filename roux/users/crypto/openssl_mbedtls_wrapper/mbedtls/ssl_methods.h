...
#define IMPLEMENT_TLS_METHOD_FUNC(func_name, new, free, handshake, shutdown, clear, read, send,    \
								  pending, set_fd, get_fd, set_bufflen, get_verify_result,         \
								  get_state)                                                       \
	static const SSL_METHOD_FUNC func_name LOCAL_ATRR = {                                          \
		new,	free,	handshake,	 shutdown,			clear,	  read, send, pending,             \
		set_fd, get_fd, set_bufflen, get_verify_result, get_state};
...
#define IMPLEMENT_X509_METHOD(func_name, new, free, load, show_info)                               \
	const X509_METHOD *func_name(void)                                                             \
	{                                                                                              \
		static const X509_METHOD func_name##_data LOCAL_ATRR = {new, free, load, show_info};       \
		return &func_name##_data;                                                                  \
	}

#define IMPLEMENT_PKEY_METHOD(func_name, new, free, load)                                          \
	const PKEY_METHOD *func_name(void)                                                             \
	{                                                                                              \
		static const PKEY_METHOD func_name##_data LOCAL_ATRR = {new, free, load};                  \
		return &func_name##_data;                                                                  \
	}
