...
{
	size_t len = 0;
	int ret = mbedtls_cipher_update((mbedtls_cipher_context_t *)ctx, in, in_len, out, &len);
	*out_len = len;
	return !ret;
}
...
{
	size_t len = 0;
	int ret = mbedtls_cipher_finish((mbedtls_cipher_context_t *)ctx, out, &len);
	*out_len = len;
	return !ret;
}
...
int EVP_EncryptUpdate(EVP_CIPHER_CTX *ctx, uint8_t *out, int *out_len, const uint8_t *in,
					  int in_len)
{
	return !mbedtls_cipher_update((mbedtls_cipher_context_t *)ctx, in, in_len, out,
								  (size_t *)out_len);
}

int EVP_EncryptFinal_ex(EVP_CIPHER_CTX *ctx, unsigned char *out, int *out_len)
{
	return !mbedtls_cipher_finish((mbedtls_cipher_context_t *)ctx, out, (size_t *)out_len);
}
...
int EVP_CIPHER_CTX_ctrl(EVP_CIPHER_CTX *ctx, int command, int tag_len, void *tag)
{
	...
	if (command == EVP_CTRL_GCM_GET_TAG) {
		...
		ret = mbedtls_cipher_check_tag(m_ctx, (const unsigned char *)tag, tag_len);
	}
