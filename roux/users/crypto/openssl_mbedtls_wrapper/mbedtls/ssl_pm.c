...
static void ssl_platform_debug(void *ctx, int level, const char *file, int line, const char *str)
{
...
int ssl_pm_new(SSL *ssl)
{
	...
	ret = mbedtls_ctr_drbg_seed(&ssl_pm->ctr_drbg, mbedtls_entropy_func, &ssl_pm->entropy, pers,
								pers_len);
	if (ret) {
	...
	ret = mbedtls_ssl_config_defaults(&ssl_pm->conf, endpoint, MBEDTLS_SSL_TRANSPORT_STREAM,
	...
	if (TLS_ANY_VERSION != ssl->version) {
		...
		mbedtls_ssl_conf_max_version(&ssl_pm->conf, MBEDTLS_SSL_MAJOR_VERSION_3,
...
mbedtls_x509_crt *ssl_ctx_get_mbedtls_x509_crt(SSL_CTX *ssl_ctx)
{
...
mbedtls_x509_crt *ssl_get_peer_mbedtls_x509_crt(SSL *ssl)
{
	struct x509_pm *x509_pm = (struct x509_pm *)ssl->session->peer->x509_pm;
...
int pkey_pm_load(EVP_PKEY *pk, const unsigned char *buffer, int len)
{
	...
	ret = mbedtls_pk_parse_key(pkey_pm->pkey, load_buf, len + 1, NULL, 0, mbedtls_ctr_drbg_random,
							   &ctr_drbg);
	ssl_mem_free(load_buf);
...
long ssl_pm_get_verify_result(const SSL *ssl)
{
	...
	if (ret & MBEDTLS_X509_BADCERT_NOT_TRUSTED || (ret & MBEDTLS_X509_BADCRL_NOT_TRUSTED)) {
		verify_result = X509_V_ERR_INVALID_CA;
	} else if (ret & MBEDTLS_X509_BADCERT_CN_MISMATCH) {
		verify_result = X509_V_ERR_HOSTNAME_MISMATCH;
	} else if ((ret & MBEDTLS_X509_BADCERT_BAD_KEY) || (ret & MBEDTLS_X509_BADCRL_BAD_KEY)) {
		verify_result = X509_V_ERR_CA_KEY_TOO_SMALL;
	} else if ((ret & MBEDTLS_X509_BADCERT_BAD_MD) || (ret & MBEDTLS_X509_BADCRL_BAD_MD)) {
		verify_result = X509_V_ERR_CA_MD_TOO_WEAK;
	} else if ((ret & MBEDTLS_X509_BADCERT_FUTURE) || (ret & MBEDTLS_X509_BADCRL_FUTURE)) {
		verify_result = X509_V_ERR_CERT_NOT_YET_VALID;
	} else if ((ret & MBEDTLS_X509_BADCERT_EXPIRED) || (ret & MBEDTLS_X509_BADCRL_EXPIRED)) {
...
{
	if (ssl->alpn_protos) {
		if (mbedtls_ssl_conf_alpn_protocols(&((struct ssl_pm *)(ssl->ssl_pm))->conf,
											ssl->alpn_protos)) {
...
int SSL_set_sni_callback(SSL *ssl,
						 int (*cb)(void *, mbedtls_ssl_context *, const unsigned char *, size_t),
						 void *param)
{
	struct ssl_pm *ssl_pm = (struct ssl_pm *)ssl->ssl_pm;
