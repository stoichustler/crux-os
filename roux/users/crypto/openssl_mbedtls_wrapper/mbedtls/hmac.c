...
{
	if (mbedtls_md_hmac((const mbedtls_md_info_t *)evp_md, (const unsigned char *)key, key_len,
						data, data_len, out) != 0) {
		return NULL;
	}
...
{
	int ret = mbedtls_md_setup((mbedtls_md_context_t *)ctx, (const mbedtls_md_info_t *)md, 1);
	if (ret != 0) {
		goto error;
	}

	ret = mbedtls_md_hmac_starts((mbedtls_md_context_t *)ctx, (const unsigned char *)key, key_len);
	if (ret != 0) {
		goto error;
	}
...
{
	errno = mbedtls_md_hmac_update((mbedtls_md_context_t *)ctx, data, data_len);
	return !errno;
}
