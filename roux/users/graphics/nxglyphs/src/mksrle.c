...
int main(int argc, char **argv, char **envp)
{
	...
	printf("// 8-bit color lookups.  NOTE:  This is really dumb!  The lookup index is 8-bits and "
		   "it used\n");
	printf("// to lookup an 8-bit value.  There is no savings in that!  It would be better to just "
		   "put\n");
	printf("// the 8-bit color/greyscale value in the run-length encoded image and save the cost "
		   "of these\n");
	printf("// pointless lookups.  But these p;ointless lookups do make the logic compatible with "
		   "the\n");
	...
	printf("  {                      // lut    - Pointer to the beginning of the Look-Up Table "
		   "(LUT)\n");
