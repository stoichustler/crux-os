...
#define ft80x_err(format, ...) fprintf(stderr, EXTRA_FMT format EXTRA_ARG, ##__VA_ARGS__)
#else
...
#elif defined(CONFIG_CPP_HAVE_VARARGS)
#define ft80x_warn(format, ...) fprintf(stderr, EXTRA_FMT format EXTRA_ARG, ##__VA_ARGS__)
#else
...
#elif defined(CONFIG_CPP_HAVE_VARARGS)
#define ft80x_info(format, ...) printf(EXTRA_FMT format EXTRA_ARG, ##__VA_ARGS__)
#else
...
int ft80x_putregs(int fd, uint32_t addr, uint8_t nregs, FAR const uint32_t *value);
...
uint16_t ft80x_ramcmd_freespace(int fd, FAR uint16_t *offset, FAR uint16_t *avail);
