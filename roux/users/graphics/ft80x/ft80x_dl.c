...
static void ft80x_dl_dump(FAR struct ft80x_dlbuffer_s *buffer, FAR const void *data, size_t len)
{
	size_t nwords;
	int max;
	int i;
	...
	printf("  buffer: dlsize=%lu dloffset=%lu coproc=%u\n", (unsigned long)buffer->dlsize,
		   (unsigned long)buffer->dloffset, buffer->coproc);
...
static int ft80x_dl_append(int fd, FAR struct ft80x_dlbuffer_s *buffer, FAR const void *data,
						   size_t len)
{
	int ret;
...
int ft80x_dl_start(int fd, FAR struct ft80x_dlbuffer_s *buffer, bool coproc)
{
	...
	if (!coproc) {
		...
		ret = ft80x_dl_data(fd, buffer, &dlstart, sizeof(struct ft80x_cmd_dlstart_s));
		if (ret < 0) {
...
int ft80x_dl_data(int fd, FAR struct ft80x_dlbuffer_s *buffer, FAR const void *data, size_t datlen)
{
	FAR uint8_t *bufptr;
	size_t padlen;
	int ret;
...
int ft80x_dl_string(int fd, FAR struct ft80x_dlbuffer_s *buffer, FAR const char *str)
{
	FAR uint8_t *bufptr;
	int datlen;
	int padlen;
...
int ft80x_dl_create(int fd, FAR struct ft80x_dlbuffer_s *buffer, FAR const uint32_t *cmds,
					unsigned int nwords, bool coproc)
{
	int ret;
