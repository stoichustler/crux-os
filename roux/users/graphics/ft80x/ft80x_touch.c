...
int ft80x_touch_waittag(int fd, uint8_t oldtag)
{
	...
	for (;;) {
		...
		notify.enable = true;
		ret = ioctl(fd, FT80X_IOC_EVENTNOTIFY, (unsigned long)((uintptr_t)&notify));
		if (ret < 0) {
			ret = -errno;
		...
		notify.enable = false;
		ioctl(fd, FT80X_IOC_EVENTNOTIFY, (unsigned long)((uintptr_t)&notify));
...
int ft80x_touch_info(int fd, FAR struct ft80x_touchinfo_s *info)
{
	...
	ret = ft80x_getreg16(fd, FT80X_REG_TOUCH_RZ, (FAR uint16_t *)&info->pressure);
	if (ret < 0) {
	...
	ret = ft80x_getreg16(fd, FT80X_REG_CTOUCH_TOUCH4_X, (FAR uint16_t *)&info->pos[4].u.x);
	if (ret < 0) {
		ft80x_err("ERROR: ft80x_getreg16 failed: %d\n", ret);
		return ret;
	}

	ret = ft80x_getreg16(fd, FT80X_REG_CTOUCH_TOUCH4_Y, (FAR uint16_t *)&info->pos[4].u.y);
	if (ret < 0) {
		ft80x_err("ERROR: ft80x_getreg16 failed: %d\n", ret);
		return ret;
	}
