...
static int tiff_readifdentry(int fd, off_t offset, FAR struct tiff_ifdentry_s *ifdentry)
{
	off_t newoffs;
	ssize_t nbytes;
...
static int tiff_writeifdentry(int fd, off_t offset, FAR struct tiff_ifdentry_s *ifdentry)
{
	off_t newoffs;
...
int tiff_finalize(FAR struct tiff_info_s *info)
{
	...
	for (i = 0; i < info->nstrips;) {
		...
		for (j = 0, ptr = info->iobuffer; j < noffsets; j++, ptr += 4) {
