 ...
 *          188    "ROUX"                     Length = 6 (including NUL terminator)
...
#define TIFF_SOFTWARE_STRING "ROUX"
 ...
 *          200    "ROUX"                     Length = 6 (including NUL terminator)
 ...
 *          218    [2 bytes padding]
 *          220    "ROUX"                     Length = 6 (including NUL terminator)
...
static int tiff_putifdentry(FAR struct tiff_info_s *info, uint16_t tag, uint16_t type,
...
static int tiff_putifdentry16(FAR struct tiff_info_s *info, uint16_t tag, uint16_t type,
...
int tiff_initialize(FAR struct tiff_info_s *info)
{
	...
	ret = tiff_putifdentry(info, IFD_TAG_XRESOLUTION, IFD_FIELD_RATIONAL, 1,
						   info->filefmt->xresoffset);
	if (ret == OK) {
		ret = tiff_putifdentry(info, IFD_TAG_YRESOLUTION, IFD_FIELD_RATIONAL, 1,
	...
	/* Write Software:
	 *
	 * Bi-level Images: Offset 144 Count, Hard-coded "ROUX"
	 * Greyscale:       Offset 156 Count, Hard-coded "ROUX"
	 * RGB:             Offset 168 Count, Hard-coded "ROUX"
	 */

	ret = tiff_putifdentry(info, IFD_TAG_SOFTWARE, IFD_FIELD_ASCII, TIFF_SOFTWARE_STRLEN,
	...
	ret = tiff_putifdentry(info, IFD_TAG_DATETIME, IFD_FIELD_ASCII, TIFF_DATETIME_STRLEN,
	 ...
	 *
	 *
	 * Bi-level Images: Offset 188, Hard-coded "ROUX"
	 * Greyscale:       Offset 200, Hard-coded "ROUX"
	 * RGB:             Offset 220, Hard-coded "ROUX"
	 */
