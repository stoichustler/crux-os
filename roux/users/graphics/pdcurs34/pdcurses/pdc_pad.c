...
WINDOW *newpad(int nlines, int ncols)
{
	...
	if (!(win = PDC_makenew(nlines, ncols, -1, -1)) || !(win = PDC_makelines(win))) {
		return NULL;
	}
...
int pnoutrefresh(WINDOW *w, int py, int px, int sy1, int sx1, int sy2, int sx2)
{
	...
	if (!w->_leaveit && w->_cury >= py && w->_curx >= px && w->_cury <= py + (sy2 - sy1) &&
		w->_curx <= px + (sx2 - sx1)) {
...
int pechochar(WINDOW *pad, chtype ch)
{
	...
	return prefresh(pad, save_pminrow, save_pmincol, save_sminrow, save_smincol, save_smaxrow,
					save_smaxcol);
}
