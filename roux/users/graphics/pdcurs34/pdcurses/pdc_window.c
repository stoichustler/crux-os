...
WINDOW *PDC_makenew(int nlines, int ncols, int begy, int begx)
{
	...
	PDC_LOG(
		("PDC_makenew() - called: lines %d cols %d begy %d begx %d\n", nlines, ncols, begy, begx));
...
WINDOW *newwin(int nlines, int ncols, int begy, int begx)
{
	...
	if ((begy + nlines > SP->lines || begx + ncols > SP->cols) ||
		!(win = PDC_makenew(nlines, ncols, begy, begx)) || !(win = PDC_makelines(win))) {
		return NULL;
	}
...
WINDOW *dupwin(WINDOW *win)
{
	...
	if (!(new = PDC_makenew(nlines, ncols, begy, begx)) || !(new = PDC_makelines(new))) {
		return NULL;
	}
...
WINDOW *resize_window(WINDOW *win, int nlines, int ncols)
{
	...
	new->_bmarg =
		(win->_bmarg == win->_maxy - 1) ? new->_maxy - 1 : min(win->_bmarg, (new->_maxy - 1));
	new->_parent = win->_parent;
