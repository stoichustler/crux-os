...
int init_pair(short pair, short fg, short bg)
{
	...
	if (!pdc_color_started || pair < 1 || pair >= COLOR_PAIRS || fg < first_col || fg >= COLORS ||
		bg < first_col || bg >= COLORS) {
		return ERR;
	}
...
int init_color(short color, short red, short green, short blue)
{
	...
	if (color < 0 || color >= COLORS || !PDC_can_change_color() || red < 0 || red > 1000 ||
		green < 0 || green > 1000 || blue < 0 || blue > 1000) {
		return ERR;
	}
