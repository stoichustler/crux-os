/****************************************************************************
 * apps/graphics/pdcurs34/roux/pdcscrn.c
 *
 ...
 *   backed by the ROUX termcurses interface.)  PDC_scr_open() must allocate
...
static int PDC_scr_open_term(int argc, char **argv)
{
	...
	termscreen = (FAR struct pdc_termscreen_s *)zalloc(sizeof(struct pdc_termscreen_s));
...
#ifdef CONFIG_SYSTEM_TERMCURSES
int PDC_init_color_term(FAR SCREEN *sp, short color, short red, short green, short blue)
{
	FAR struct pdc_termscreen_s *termscreen = (FAR struct pdc_termscreen_s *)sp;
	FAR struct pdc_termstate_s *termstate;
...
#ifdef PDCURSES_MONOCHROME
	greylevel =
		(DIVROUND(red * 255, 1000) + DIVROUND(green * 255, 1000) + DIVROUND(blue * 255, 1000)) / 3;
...
#ifdef CONFIG_SYSTEM_TERMCURSES
int PDC_color_content_term(FAR SCREEN *sp, short color, FAR short *red, FAR short *green,
						   FAR short *blue)
{
	FAR struct pdc_termscreen_s *termscreen = (FAR struct pdc_termscreen_s *)sp;
	FAR struct pdc_termstate_s *termstate;
...
#ifdef CONFIG_SYSTEM_TERMCURSES
static int PDC_pair_content_term(FAR SCREEN *sp, short pair, FAR short *fg, FAR short *bg)
{
	FAR struct pdc_termscreen_s *termscreen = (FAR struct pdc_termscreen_s *)sp;
...
int PDC_scr_open(int argc, char **argv)
{
	...
	ret = ioctl(fbstate->fbfd, FBIOGET_VIDEOINFO, (unsigned long)((uintptr_t)&vinfo));
	if (ret < 0) {
	...
	ret = ioctl(fbstate->fbfd, FBIOGET_PLANEINFO, (unsigned long)((uintptr_t)&pinfo));
	if (ret < 0) {
	...
	fbstate->fbmem =
		mmap(NULL, pinfo.fblen, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FILE, fbstate->fbfd, 0);
	...
	fbstate->fstride = (fbstate->fwidth + PDCURSES_PPB_MASK) >> PDCURSES_PPB_SHIFT;
	fbstate->fbuffer = (FAR uint8_t *)malloc(fbstate->fstride * fbstate->fheight);
