...
/* Fix compilation error for Non-ROUX OS */
#ifndef FAR
#define FAR
#endif
...
static inline int64_t calc_diff(FAR const struct timespec *t1, FAR const struct timespec *t2)
{
	int64_t diff;
...
static inline int ts_greater(FAR const struct timespec *a, FAR const struct timespec *b)
{
	return (a->tv_sec > b->tv_sec) || (a->tv_sec == b->tv_sec && a->tv_nsec > b->tv_nsec);
}

static inline void calc_next(FAR struct timespec *t, FAR struct timespec *intv)
{
...
{
	struct timerjitter_param_s param = {.clockid = DEFAULT_CLOCKID,
		...
		while ((arg = argv[1]) != NULL) {
			...
			for (;;) {
				...
				case 'r':
					...
					continue;
				case 'h':
					printf("usage: timerjitter [-pmr] [interval(us)] [iteration]\n"
