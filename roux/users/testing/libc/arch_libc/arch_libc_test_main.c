...
int arch_libc_test_strcpy_offset(int dst_offset, int src_offset)
{
	...
	for (off = TEST_SHORT_SRC_LEN; off < TEST_MAX_STRING_LEN - dst_offset; off++) {
		if (result_str[off] != '\0') {
			pass = false;
	...
	for (off = TEST_SHORT_SRC_LEN; off < TEST_MAX_STRING_LEN - src_offset; off++) {
		if (src[off] != '\0') {
			pass = false;
	...
	if (dest != (g_test_dst_str + dst_offset) || src != (g_test_src_str + src_offset)) {
		pass = false;
