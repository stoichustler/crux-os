...
static void verify_memory(FAR struct ramtest_s *info, uint32_t value)
{
	...
	if (info->width == 32) {
		...
		for (i = 0; i < info->nxfrs; i++) {
			if (*ptr != value) {
				printf(RAMTEST_PREFIX "ERROR: Address %p Found: %08" PRIx32 " Expected %08" PRIx32
									  "\n",
					   ptr, *ptr, value);
			}
		...
		for (i = 0; i < info->nxfrs; i++) {
			if (*ptr != value16) {
				printf(RAMTEST_PREFIX "ERROR: Address %p Found: %04" PRIx16 " Expected %04" PRIx16
									  "\n",
					   ptr, *ptr, value16);
			}
	...
	} else /* if (info->width == 8) */
	{
		...
		for (i = 0; i < info->nxfrs; i++) {
			if (*ptr != value8) {
				printf(RAMTEST_PREFIX "ERROR: Address %p Found: %02" PRIx8 " Expected %02" PRIx8
									  "\n",
					   ptr, *ptr, value8);
			}
	...
	if (info->width == 32) {
		...
		for (i = 0; i < even_nxfrs; i += 2) {
			if (ptr[0] != value_1 || ptr[1] != value_2) {
				printf(RAMTEST_PREFIX "ERROR: Address %p Found: %08" PRIx32 " and %08" PRIx32 "\n",
					   ptr, ptr[0], ptr[1]);
				printf(RAMTEST_PREFIX "               Expected: %08" PRIx32 " and %08" PRIx32 "\n",
					   value_1, value_2);
			}
		...
		for (i = 0; i < even_nxfrs; i += 2) {
			if (ptr[0] != value16_1 || ptr[1] != value16_2) {
				printf(RAMTEST_PREFIX "ERROR: Address %p Found: %04" PRIx16 " and %04" PRIx16 "\n",
					   ptr, ptr[0], ptr[1]);
				printf(RAMTEST_PREFIX "               Expected: %04" PRIx16 " and %04" PRIx16 "\n",
					   value16_1, value16_2);
			}
	...
	} else /* if (info->width == 8) */
	{
		...
		for (i = 0; i < even_nxfrs; i += 2) {
			if (ptr[0] != value8_1 || ptr[1] != value8_2) {
				printf(RAMTEST_PREFIX "ERROR: Address %p Found: %02" PRIx8 " and %02" PRIx8 "\n",
					   ptr, ptr[0], ptr[1]);
				printf(RAMTEST_PREFIX "               Expected: %02" PRIx8 " and %02" PRIx8 "\n",
					   value8_1, value8_2);
			}
...
{
	printf(RAMTEST_PREFIX "Pattern test: %08" PRIxPTR " %zu %08" PRIx32 " %08" PRIx32 "\n",
