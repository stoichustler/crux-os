...
static void error_result(struct memorystress_error_s error)
{
	syslog(LOG_ERR,
		   MEMSTRESS_PREFIX "%s ERROR!, "
							...
							"readValue = 0x%x, "
							"writeValue = 0x%x\n",
		   error.rwerror == MEMORY_STRESS_READ_ERROR ? "READ" : "WRITE", error.buf,
		   (uintptr_t *)error.node, error.size, error.offset, error.buf + error.offset, error.cnt,
		   error.index, error.readvalue, error.writevalue);
}
...
static void checknode(FAR struct memorystress_thread_context_s *context,
					  FAR struct memorystress_node_s *node, enum memorystress_rwerror_e rwerror)
{
...
static bool memorystress_iter(FAR struct memorystress_thread_context_s *context)
{
...
static FAR void *debug_aligned_alloc(size_t align, size_t nbytes)
{
	...
	syslog(LOG_INFO,
		   MEMSTRESS_PREFIX "aligned_alloc: %zu bytes, align: %zu,"
							" ptr: %p\n",
		   nbytes, align, ptr);
	return ptr;
}
...
static void global_init(FAR struct memorystress_global_s *global, int argc, FAR char *argv[])
{
	int ch;
	...
	global->threads = (FAR pthread_t *)malloc(sizeof(pthread_t) * global->nthreads);
	...
	syslog(LOG_INFO,
		   MEMSTRESS_PREFIX "\n max_allocsize: %zu\n"
							" nodelen: %zu\n sleep_us: %" PRIu32 "\n nthreads: %zu\n "
							"debug: %s\n",
		   global->max_allocsize, global->nodelen, global->sleep_us, global->nthreads,
						...
						FAR struct memorystress_global_s *global)
{
	...
	context->node_array = zalloc(sizeof(struct memorystress_node_s) * global->nodelen);
...
int main(int argc, FAR char *argv[])
{
	...
	for (i = 0; i < global.nthreads; i++) {
		if (pthread_create(&global.threads[i], NULL, memorystress_thread, (FAR void *)&global) !=
			0) {
