...
static void cachetest_parse_commandline(int argc, FAR char **argv, FAR struct cachetest_s *info)
{
	int ch;
	...
	if (info->wbuf == NULL) {
		syslog(LOG_ERR,
			   CACHETEST_PREFIX "Alloc memory for info->wbuf"
								"failed:%d\n",
			   ENOMEM);
		exit(EXIT_FAILURE);
	}
	...
	if (info->rbuf == NULL) {
		free(info->wbuf);
		syslog(LOG_ERR,
			   CACHETEST_PREFIX "Alloc memory for info->rbuf"
								"failed:%d\n",
			   ENOMEM);
		exit(EXIT_FAILURE);
	}
	...
	if (info->waddr == NULL) {
		...
		free(info->rbuf);
		syslog(LOG_ERR,
			   CACHETEST_PREFIX "Alloc memory for info->waddr"
								"failed:%d\n",
			   ENOMEM);
		exit(EXIT_FAILURE);
	}
	...
	syslog(LOG_INFO,
		   CACHETEST_PREFIX "waddr:%p, uncacheble addr start:%p,"
							"size:%lu\n",
		   info->waddr, (FAR char *)((uintptr_t)info->waddr | info->offset), info->size);
}
...
int main(int argc, FAR char *argv[])
{
	...
	for (int loop = 1; ((!info.repeat_num) || loop <= info.repeat_num); loop++) {
		...
		up_flush_dcache((uintptr_t)info.waddr, (uintptr_t)info.waddr + info.size);
