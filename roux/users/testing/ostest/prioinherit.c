...
static FAR void *lowpri_thread(FAR void *parameter)
{
	...
	if (ret != 0) {
		...
		printf("lowpri_thread-%d: %s priority before sem_post: %d\n", threadno,
			   sparam.sched_priority != expected ? "ERROR" : "SUCCESS", sparam.sched_priority);
	...
	if (ret != 0) {
		...
		printf("lowpri_thread-%d: %s final priority: %d\n", threadno,
			   sparam.sched_priority != g_lowpri ? "ERROR" : "SUCCESS", sparam.sched_priority);
...
void priority_inheritance(void)
{
	...
	for (i = 0; i < NLOWPRI_THREADS; i++) {
		...
		status =
			pthread_create(&lowpri[i], &attr, lowpri_thread, (FAR void *)((uintptr_t)threadno));
		if (status != 0) {
			printf("priority_inheritance: "
	...
	for (i = 0; i < NHIGHPRI_THREADS; i++) {
		...
		status =
			pthread_create(&highpri[i], &attr, highpri_thread, (FAR void *)((uintptr_t)threadno));
		if (status != 0) {
			printf("priority_inheritance: "
	...
	for (i = 0; i < NUMBER_OF_COMPETING_THREADS; i++) {
		...
		pids[i] = task_create(name, priority, CONFIG_DEFAULT_TASK_STACKSIZE, adversary, argv);
