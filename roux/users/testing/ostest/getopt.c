...
static int getopt_short_test(int noptions, int argc, FAR char **argv, FAR const char *optstring,
							 FAR const struct result_s *expected)
{
	...
	for (ndx = 0; (ret = getopt(argc, argv, optstring)) != ERROR && ndx < noptions; ndx++) {
		...
		if (ndx < noptions) {
			...
			if ((expected[ndx].arg == NULL && optarg != NULL) ||
				(expected[ndx].arg != NULL && strcmp(expected[ndx].arg, optarg) != 0)) {
				printf("ERROR: arg %d:  optarg=%s (expected %s)\n", ndx + 1,
					   optarg == NULL ? "null" : optarg,
					   expected[ndx].arg == NULL ? "null" : expected[ndx].arg);
				ASSERT(false);
			}
		}
...
static int getopt_long_test(int noptions, int argc, FAR char **argv, FAR const char *optstring,
							FAR const struct option *longopts, FAR int *longindex,
							FAR const struct result_s *expected)
{
	int ndx;
	int ret;
	...
	for (ndx = 0; (ret = getopt_long(argc, argv, optstring, longopts, longindex)) != ERROR; ndx++) {
		/* optind may index through argc (to the NULL argv entry) since it is
		 * required to always point to the next command line argument.
		 */
...
static int getopt_longonly_test(int noptions, int argc, FAR char **argv, FAR const char *optstring,
								FAR const struct option *longopts, FAR int *longindex,
								FAR const struct result_s *expected)
{
	int ndx;
	...
	for (ndx = 0; (ret = getopt_long_only(argc, argv, optstring, longopts, longindex)) != ERROR;
		 ndx++) {
		/* optind may index through argc (to the NULL argv entry) since it is
		 * required to always point to the next command line argument.
		 */
...
int getopt_test(void)
{
	...
	getopt_long_test(4, 8, argv, g_optstring, long_options, NULL, results);

	printf("getopt_long():  No short options\n");

	getopt_long_test(4, 8, argv, NULL, long_options, NULL, results);
	...
	getopt_long_test(4, 6, argv, g_optstring, long_options, NULL, results);
	...
	getopt_long_test(5, 9, argv, g_optstring, long_options, NULL, results);

	argv[8] = NULL;
	getopt_long_test(5, 8, argv, g_optstring, long_options, NULL, results);
	...
	getopt_longonly_test(4, 8, argv, g_optstring, long_options, NULL, results);
