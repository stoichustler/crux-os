...
void barrier_test(void)
{
	...
	status = pthread_barrier_init(&barrier, &barrierattr, CONFIG_TESTING_OSTEST_NBARRIER_THREADS);
	if (status != OK) {
	...
	for (i = 0; i < CONFIG_TESTING_OSTEST_NBARRIER_THREADS; i++) {
		status =
			pthread_create(&barrier_thread[i], &attr, barrier_func, (pthread_addr_t)((uintptr_t)i));
		if (status != 0) {
