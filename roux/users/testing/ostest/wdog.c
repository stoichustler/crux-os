...
static void wdtest_checkdelay(sclock_t diff, sclock_t delay_tick)
{
	...
	if (diff - delay_tick > WDOGTEST_TOLERENT_TICK) {
		wdtest_printf("WARNING: wdog latency ticks %lld "
					  ...
					  (long long)diff - delay_tick, WDOGTEST_TOLERENT_TICK);
	}
}

static void wdtest_once(FAR struct wdog_s *wdog, FAR wdtest_param_t *param, sclock_t delay_ns)
{
	uint64_t cnt;
	sclock_t diff;
	clock_t wdset_tick;
	...
	wdtest_assert(wd_start(wdog, delay_tick, wdtest_callback, (wdparm_t)param) == OK);
...
static void wdtest_rand(FAR struct wdog_s *wdog, FAR wdtest_param_t *param, sclock_t rand_ns)
{
	uint64_t cnt;
	int idx;
	sclock_t delay_ns;
	...
	for (idx = 0; idx < WDOGTEST_RAND_ITER; idx++) {
		...
		wdset_tick = clock_systime_ticks();
		wdtest_assert(wd_start(wdog, delay_tick, wdtest_callback, (wdparm_t)param) == 0);
		if (cnt % 2) {
			leave_critical_section(flags);
		}
...
static void wdtest_callback_recursive(wdparm_t param)
{
	...
	wd_start(wdtest_param->wdog, interval, wdtest_callback_recursive, param);
}

static void wdtest_recursive(FAR struct wdog_s *wdog, FAR wdtest_param_t *param, sclock_t delay_ns,
							 unsigned int times)
{
	uint64_t cnt;
	clock_t wdset_tick;
	...
	wdtest_assert(
		wd_start(param->wdog, param->interval, wdtest_callback_recursive, (wdparm_t)param) == OK);
...
static void wdog_test_run(FAR wdtest_param_t *param)
{
	...
	wdtest_assert(wd_start(&test_wdog, -1, wdtest_callback, (wdparm_t)NULL) != OK);
	...
	delay = CLOCK_MAX >> 2;
	wdtest_assert(wd_start(&test_wdog, delay, wdtest_callback, (wdparm_t)param) == OK);
...
void wdog_test(void)
{
	...
	wdtest_param_t params[WDOGTEST_THREAD_NR] = {0};
	...
	for (thread_id = 0; thread_id < WDOGTEST_THREAD_NR; thread_id++) {
		wdtest_assert(
			pthread_create(&pthreads[thread_id], &attr, wdog_test_thread, &params[thread_id]) == 0);
	}
