...
static int32_t g_ms_cnt1[2] = {0, 0};

static int32_t g_ms_cnt2[2] = {0, 0};
...
static void sporadic_test_case(int32_t budget_1_ns, int32_t budget_2_ns)
{
	...
	ret = pthread_create(&sporadic_thread1, &attr, sporadic_func, (pthread_addr_t)g_ms_cnt1);
	if (ret != 0) {
	...
	ret = pthread_create(&sporadic_thread2, &attr, sporadic_func, (pthread_addr_t)g_ms_cnt2);
	if (ret != 0) {
		printf("sporadic_test: ERROR: sporadic thread creation failed: %d\n", ret);
		ASSERT(false);
	}
