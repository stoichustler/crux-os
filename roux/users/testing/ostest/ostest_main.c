...
#if !defined(CONFIG_DISABLE_PTHREAD) && defined(__KERNEL__) && defined(CONFIG_SCHED_WORKQUEUE)
		/* Check work queues */
...
#if defined(CONFIG_ARCH_HAVE_FORK) && defined(CONFIG_SCHED_WAITPID) && !defined(CONFIG_ARCH_SIM)
...
int main(int argc, FAR char **argv)
{
	...
	FAR const char *const arg[7] = {argv[0], "user_main", arg1, arg2, arg3, arg4, NULL};
	...
	status = posix_spawn(&result, arg[0], NULL, &attr, (char *const *)arg, NULL);
	if (status != 0)
#else
	result = task_create("ostest", PRIORITY, STACKSIZE, user_main, (FAR char *const *)g_argv);
	if (result == ERROR)
#endif
	{
	 ...
	 * interesting case when used with the ROUX simulator.  In that case,
