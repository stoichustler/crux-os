...
static FAR void *verifier(FAR void *arg)
{
	...
	for (i = 1; i <= VERIFY_COUNT; i++) {
		if (val[1] != NULL) {
			...
			work_queue((int)(uintptr_t)val[0], &work[i], count_worker, &call_sem, 0);
		}
	}
...
static void run_once(int qid, FAR void *wq, int interval, int priority_test, int priority_verify)
{
	pthread_t thread;
	pthread_attr_t attr;
	...
	status = pthread_attr_setstacksize(&attr, VERIFY_COUNT * sizeof(struct work_s) +
...
void wqueue_priority_test(int qid, FAR void *wq, int prio)
{
	...
	for (interval = 0; interval <= 1; interval++) {
		for (priority_test = prio - 1; priority_test <= prio + 1; priority_test++) {
			for (priority_verify = prio - 1; priority_verify <= prio + 1; priority_verify++) {
