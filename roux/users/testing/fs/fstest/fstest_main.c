...
static void fstest_showmemusage(struct mallinfo *mmbefore, struct mallinfo *mmafter)
{
...
static bool fstest_checkexist(FAR struct fstest_ctx_s *ctx, FAR struct fstest_filedesc_s *file)
{
	int i;
	bool ret = false;
...
static inline void fstest_randname(FAR struct fstest_ctx_s *ctx, FAR struct fstest_filedesc_s *file)
{
	int dirlen;
	int maxname;
	int namelen;
...
static inline void fstest_randfile(FAR struct fstest_ctx_s *ctx, FAR struct fstest_filedesc_s *file)
{
	int i;
...
static inline int fstest_wrfile(FAR struct fstest_ctx_s *ctx, FAR struct fstest_filedesc_s *file)
{
	size_t offset;
	int fd;
	int ret;
...
static ssize_t fstest_rdblock(FAR struct fstest_ctx_s *ctx, int fd,
							  FAR struct fstest_filedesc_s *file, size_t offset, size_t len)
{
	size_t maxio = (rand() % CONFIG_TESTING_FSTEST_MAXIO) + 1;
	ssize_t nbytesread;
...
static inline int fstest_rdfile(FAR struct fstest_ctx_s *ctx, FAR struct fstest_filedesc_s *file)
{
	size_t ntotalread;
	ssize_t nbytesread;
	uint32_t crc;
	...
	for (ntotalread = 0; ntotalread < file->len;) {
		nbytesread = fstest_rdblock(ctx, fd, file, ntotalread, file->len - ntotalread);
		if (nbytesread < 0) {
			close(fd);
			return ERROR;
		}
	...
	do {
		...
		if (entryp) {
			printf("%2d. Type[%d]: %s Name: %s\n", number, entryp->d_type,
				   entryp->d_type == DTYPE_FILE ? "File " : "Error", entryp->d_name);
		}
