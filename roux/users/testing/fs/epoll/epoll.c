...
#define TIME_ASSERT_RANGE(e, t) assert_in_range(e, (t)-TIME_ERROR_MS, (t) + TIME_ERROR_MS)
...
static struct sockaddr_un g_epoll03_addr = {.sun_family = AF_UNIX, .sun_path = "epoll_test03"};
...
static void epoll02(FAR void **state)
{
	...
	TIME_ASSERT_RANGE(get_elapse(args->start), 1000);
	assert_true(ret == 2);
	assert_true((evs[0].data.fd == args->fd[0] && evs[1].data.fd == args->fd[1]) ||
				(evs[0].data.fd == args->fd[1] && evs[1].data.fd == args->fd[0]));
...
static void *epoll03_server(FAR void *arg)
{
	...
	ret = bind(fd, (const struct sockaddr *)&g_epoll03_addr, sizeof(g_epoll03_addr));
	assert_true(ret >= 0);
...
static void epoll03(FAR void **state)
{
	...
	ret = connect(args->fd[0], (const struct sockaddr *)&g_epoll03_addr, sizeof(g_epoll03_addr));
...
int main(int argc, FAR char *argv[])
{
	...
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_prestate_setup_teardown(epoll01, epoll01_setup, epoll01_teardown,
												 &epoll_args),
		cmocka_unit_test_prestate_setup_teardown(epoll02, epoll02_setup, epoll02_teardown,
												 &epoll_args),
		cmocka_unit_test_prestate_setup_teardown(epoll03, epoll03_setup, epoll03_teardown,
												 &epoll_args),
	};
