...
static void fill_corrupted_close_ate(FAR struct mtdnvs_ctx_s *ctx, FAR struct nvs_ate *close_ate)
{
	memset((FAR void *)close_ate, ctx->erasestate, sizeof(struct nvs_ate));
...
static void fill_close_ate(FAR struct mtdnvs_ctx_s *ctx, FAR struct nvs_ate *close_ate, int offset)
{
	memset((FAR void *)close_ate, ctx->erasestate, sizeof(struct nvs_ate));
	close_ate->id = 0xffffffff;
	close_ate->len = 0U;
...
static void fill_gc_done_ate(FAR struct mtdnvs_ctx_s *ctx, FAR struct nvs_ate *gc_done_ate)
{
	memset((FAR void *)gc_done_ate, ctx->erasestate, sizeof(struct nvs_ate));
...
static void fill_ate(FAR struct mtdnvs_ctx_s *ctx, FAR struct nvs_ate *ate, FAR const char *key,
					 uint16_t len, uint16_t offset, bool expired)
{
	memset((FAR void *)ate, ctx->erasestate, sizeof(struct nvs_ate));
	ate->id = nvs_fnv_hash((FAR const uint8_t *)key, strlen(key) + 1) % 0xfffffffd + 1;
	ate->len = len;
	ate->offset = offset;
...
static void fill_corrupted_ate(FAR struct mtdnvs_ctx_s *ctx, FAR struct nvs_ate *ate,
...
static void mtdnvs_showmemusage(struct mallinfo *mmbefore, struct mallinfo *mmafter)
{
...
extern int find_mtddriver(FAR const char *pathname, FAR struct inode **ppinode);
...
static int setup(struct mtdnvs_ctx_s *ctx)
{
	...
	ret = find_mtddriver(CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_MOUNTPT_NAME, &sys_node);
	if (ret < 0) {
...
static void test_nvs_corrupt_expire(struct mtdnvs_ctx_s *ctx)
{
	...
	padding_size = NVS_ALIGN_UP(sizeof(TEST_KEY1) + sizeof(TEST_DATA1)) - sizeof(TEST_KEY1) -
				   sizeof(TEST_DATA1);
	...
	for (i = 2 * (strlen(TEST_KEY1) + strlen(TEST_DATA1) + 2) + padding_size;
		 i < CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE - 4 * sizeof(struct nvs_ate);
		 i++) {
		ret = write(mtd_fd, &erase_value, sizeof(erase_value));
		if (ret != sizeof(erase_value)) {
...
static void test_nvs_corrupted_write(struct mtdnvs_ctx_s *ctx)
{
	...
	padding_size = NVS_ALIGN_UP(sizeof(key1) + sizeof(wr_buf_1)) - sizeof(key1) - sizeof(wr_buf_1);
	for (i = 0; i < padding_size; i++) {
		ret = write(mtd_fd, &erase_value, sizeof(erase_value));
		if (ret != sizeof(erase_value)) {
			printf("%s:write data1 padding failed, ret=%d\n", __func__, ret);
	...
	for (i = 2 * (sizeof(key1) + sizeof(wr_buf_1)) + padding_size;
		 i < CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE - 3 * sizeof(struct nvs_ate);
		 i++) {
		ret = write(mtd_fd, &erase_value, sizeof(erase_value));
		if (ret != sizeof(erase_value)) {
			printf("%s:erase failed, ret=%d\n", __func__, ret);
...
static void test_nvs_gc(struct mtdnvs_ctx_s *ctx)
{
	...
	size_t block_max_write_size =
		CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE - sizeof(struct nvs_ate);
...
static void test_nvs_corrupted_sector_close(struct mtdnvs_ctx_s *ctx)
{
	...
	for (loop_section = 0;
		 loop_section < CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_COUNT - 1;
		 loop_section++) {
		/* write valid data */
		...
		for (i = 2 * (sizeof(key1) + sizeof(wr_buf));
			 i <
			 CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE - 4 * sizeof(struct nvs_ate);
			 i++) {
			ret = write(mtd_fd, &erase_value, sizeof(erase_value));
			if (ret != sizeof(erase_value)) {
				printf("%s:erase failed, ret=%d\n", __func__, ret);
		...
		fill_ate(ctx, &ate, key1, sizeof(wr_buf), sizeof(wr_buf) + sizeof(key1),
				 (loop_section == CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_COUNT - 2)
					 ? false
					 : true);
		ret = write(mtd_fd, &ate, sizeof(ate));
		if (ret != sizeof(ate)) {
		...
		if (loop_section == CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_COUNT - 2) {
...
static void test_nvs_gc_corrupt_close_ate(struct mtdnvs_ctx_s *ctx)
{
	...
	for (i = strlen(TEST_KEY1) + strlen(TEST_DATA1) + 2;
		 i < CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE - 6 * sizeof(struct nvs_ate);
		 i++) {
		ret = write(mtd_fd, &erase_value, sizeof(erase_value));
		if (ret != sizeof(erase_value)) {
			printf("%s:erase failed, ret=%d\n", __func__, ret);
	...
	for (i = CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE - 5 * sizeof(struct nvs_ate);
		 i < CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE - 2 * sizeof(struct nvs_ate);
		 i++) {
		ret = write(mtd_fd, &erase_value, sizeof(erase_value));
		if (ret != sizeof(erase_value)) {
			printf("%s:erase failed, ret=%d\n", __func__, ret);
	...
	for (i = CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE;
		 i < CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE - sizeof(struct nvs_ate); i++) {
		ret = write(mtd_fd, &erase_value, sizeof(erase_value));
		if (ret != sizeof(erase_value)) {
			printf("%s:erase failed, ret=%d\n", __func__, ret);
			goto test_fail;
...
static void test_nvs_gc_corrupt_ate(struct mtdnvs_ctx_s *ctx)
{
	...
	for (i = 0; i < CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE / 2; i++) {
		ret = write(mtd_fd, &erase_value, sizeof(erase_value));
		if (ret != sizeof(erase_value)) {
			printf("%s:erase failed, ret=%d\n", __func__, ret);
			goto test_fail;
	...
	for (i = CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE / 2 + sizeof(struct nvs_ate);
		 i < CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE - 2 * sizeof(struct nvs_ate);
		 i++) {
		ret = write(mtd_fd, &erase_value, sizeof(erase_value));
		if (ret != sizeof(erase_value)) {
	...
	fill_close_ate(ctx, &close_ate, CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_FLASH_SECTION_SIZE / 2);
	ret = write(mtd_fd, &close_ate, sizeof(close_ate));
	if (ret != sizeof(ate)) {
		printf("%s:write gc_done ate failed, ret=%d\n", __func__, ret);
		goto test_fail;
...
int main(int argc, FAR char *argv[])
{
	...
	strlcpy(ctx->mountdir, CONFIG_TESTING_MTD_CONFIG_FAIL_SAFE_MOUNTPT_NAME, sizeof(ctx->mountdir));
