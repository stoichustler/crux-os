...
#define TESTING_NXFFS_BUFSIZE (CONFIG_RAMMTD_ERASESIZE * CONFIG_TESTING_NXFFS_NEBLOCKS)
#endif
...
static void nxffs_showmemusage(struct mallinfo *mmbefore, struct mallinfo *mmafter)
{
...
static ssize_t nxffs_rdblock(int fd, FAR struct nxffs_filedesc_s *file, size_t offset, size_t len)
{
	size_t maxio = (rand() % CONFIG_TESTING_NXFFS_MAXIO) + 1;
	ssize_t nbytesread;
