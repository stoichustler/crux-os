...
{
	char *mesg = "an elegant unit testing framework for C "
				 ...
				 "--test Test* --skip TestROUXMm0[123]\n\n";
	printf("%s", mesg);
}
...
int main(int argc, FAR char *argv[])
{
	...
	if (strlen(argv[0]) < prefix_len - 1 || strncmp(argv[0], prefix, prefix_len - 1)) {
		return 0;
	}
	...
	for (i = 1; i < argc; i++) {
		if (strcmp("--help", argv[i]) == 0 || strcmp("-?", argv[i]) == 0) {
			...
			list_tests = 1;
		} else if (strcmp("--output-path", argv[i]) == 0 || strcmp("-f", argv[i]) == 0) {
			xml_path = argv[++i];
		} else if (strcmp("--shuffle-seed", argv[i]) == 0 || strcmp("-d", argv[i]) == 0) {
			shuffle_seed = argv[++i];
	...
	for (i = 0; (builtin = builtin_for_index(i)) != NULL; i++) {
		if (builtin->main == NULL || strlen(builtin->name) < prefix_len ||
			strncmp(builtin->name, prefix, prefix_len)) {
			continue;
		}

		if (suite != NULL && !cm_regexmatch(suite, builtin->name)) {
			continue;
		}
