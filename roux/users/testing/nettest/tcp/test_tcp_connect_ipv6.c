...
int test_tcp_connect_ipv6_setup(FAR void **state)
{
	...
	tv.tv_sec = 0;
	tv.tv_usec = 10000;
	ret = setsockopt(tcp_state->client_fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
	assert_return_code(ret, errno);

	ret = setsockopt(tcp_state->client_fd, SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv));
	assert_return_code(ret, errno);
...
void test_tcp_connect_ipv6(FAR void **state)
{
	...
	ret = connect(tcp_state->client_fd, (FAR struct sockaddr *)&myaddr, addrlen);
	assert_return_code(ret, errno);
