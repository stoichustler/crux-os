...
#include "../../roux/net/utils/utils.h"
...
static void test_net_bufpool_alloc_to(FAR struct net_bufpool_s *pool, FAR sq_queue_t *allocated)
{
	FAR sq_entry_t *node;
...
static void test_net_bufpool_free_from(FAR struct net_bufpool_s *pool, FAR sq_queue_t *allocated)
{
...
void test_others_bufpool(FAR void **state)
{
	...
	NET_BUFPOOL_DECLARE(limited, sizeof(sq_entry_t), TEST_PREALLOC, TEST_DYNALLOC, TEST_MAXALLOC);
	NET_BUFPOOL_DECLARE(unlimited, sizeof(sq_entry_t), TEST_PREALLOC, TEST_DYNALLOC, 0);
