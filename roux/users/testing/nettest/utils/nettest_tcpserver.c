...
static inline bool nettest_incomingdata(FAR struct nettest_listener_s *nls, int sd)
{
	FAR char *ptr;
	int nbytes;
	int ret;
...
static inline void nettest_connection(FAR struct nettest_listener_s *nls, int lsd)
{
...
static int nettest_tcpsocket(FAR struct sockaddr *addr, int addrlen)
{
	...
	listensd = socket(addr->sa_family, SOCK_STREAM | SOCK_CLOEXEC | SOCK_NONBLOCK, 0);
	if (listensd < 0) {
		return listensd;
	}
...
#ifdef CONFIG_NET_IPv4
	socklen_t addrlen = nettest_lo_addr((FAR struct sockaddr *)&myaddr, AF_INET);
#else
	socklen_t addrlen = nettest_lo_addr((FAR struct sockaddr *)&myaddr, AF_INET6);
#endif
