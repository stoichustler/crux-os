...
/****************************************************************************
 * Name: test_roux_syscall_testgroupsetup
 ****************************************************************************/
...
/****************************************************************************
 * Name: test_roux_syscall_test_group_teardown
 ****************************************************************************/
...
static int cmtestfillfilewithfd(int fd, char pattern, size_t bs, size_t bcount)
{
	size_t i;
	char *buf;
...
int check_and_report_ftruncatetest(int fd, off_t offset, char data, off_t trunc_len)
{
	...
	for (i = 0; i < 256; i++) {
		if (buf[i] != data) {
			syslog(LOG_ERR, "FAIL, ftruncate() got incorrect data %i, expected %i\n", buf[i], data);
			return -1;
		}
	}
	...
	for (i = 0; i < 120; i++) {
		...
		if (errno != EADDRINUSE) {
			syslog(LOG_ERR, "bind(%d, %s, %d) failed\n", socket,
				   sock_addr(address, address_len, buf, sizeof(buf)), address_len);
			fail_msg("test fail !");
			return -1;
		}
