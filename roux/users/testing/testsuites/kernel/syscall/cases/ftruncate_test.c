...
static int checkandreportftruncatetest(int fd, off_t offset, char data, off_t trunc_len)
{
	int i;
	int file_length;
	int ret;
	...
	for (i = 0; i < 256; i++) {
		if (buf[i] != data) {
			syslog(LOG_ERR, "FAIL, ftruncate() got incorrect data %d, expected %d\n", buf[i], data);
			return -1;
		}
	}
...
/****************************************************************************
 * Name: test_roux_syscall_ftruncate01
 ****************************************************************************/
