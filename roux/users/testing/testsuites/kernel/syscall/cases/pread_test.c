...
void pread01_l_seek(int, off_t, int, off_t); /* function to call lseek() */
void pread01_init_buffers(void);			 /* function to initialize/allocate buffers */
int pread01_compare_bufers(void);			 /* function to compare o/p of pread/pwrite */
...
void pread01_init_buffers(void)
{
	...
	for (count = 0; count < 4; count++) {
		...
		if ((pread01_write_buf[count] == NULL) || (pread01_read_buf[count] == NULL)) {
...
void pread01_setup(void)
{
	...
	if ((pread01_fildes = open(pread01_filename, O_RDWR | O_CREAT, 0666)) < 0) {
		syslog(LOG_ERR, "FAIL, open() on %s failed, errno=%d : %s\n", pread01_filename, errno,
			   strerror(errno));
	}
	...
	if (pread01_fildes > 0) {
		...
		if ((nwrite = pwrite(pread01_fildes, pread01_write_buf[0], K1, 0)) != K1) {
			syslog(LOG_ERR,
		...
		if ((nwrite = pwrite(pread01_fildes, pread01_write_buf[2], K1, K2)) != K1) {
			syslog(LOG_ERR,
		...
		if ((nwrite = write(pread01_fildes, pread01_write_buf[3], K1)) != K1) {
			syslog(LOG_ERR,
		...
		if ((nwrite = pwrite(pread01_fildes, pread01_write_buf[1], K1, K1)) != K1) {
			syslog(LOG_ERR,
...
int pread01_compare_bufers(void)
{
	...
	for (count = 0; count < 4; count++) {
		if (memcmp(pread01_write_buf[count], pread01_read_buf[count], K1) != 0) {
...
/****************************************************************************
 * Name: test_roux_syscall_pread01
 ****************************************************************************/
