...
/****************************************************************************
 * Name: test_roux_syscall_truncate01
 ****************************************************************************/
...
void test_roux_syscall_truncate01(FAR void **state)
{
	...
	snprintf(truncate01_fileneme, sizeof(truncate01_fileneme), "%s_file", __func__);
	...
	fd = open(truncate01_fileneme, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
	if (fd < 0) {
	...
	for (lc = 0; lc < 2; lc++) {
		...
		if (ret == -1) {
			syslog(LOG_ERR, "FAIL, truncate(%s, %d) Failed, errno=%d : %s\n", truncate01_fileneme,
				   TRUNC_LEN, errno, strerror(errno));
			fail_msg("test fail !");
		} else {
			...
			if (file_length != TRUNC_LEN) {
				syslog(LOG_ERR,
					   ...
					   truncate01_fileneme, (int64_t)file_length, TRUNC_LEN);
				fail_msg("test fail !");
			}
		}
	}
