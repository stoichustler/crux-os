...
/****************************************************************************
 * Name: test_roux_syscall_socketpair01
 ****************************************************************************/
...
void test_roux_syscall_socketpair01(FAR void **state)
{
	...
	tdat[] = {{0, SOCK_STREAM, 0, fds, -1, EAFNOSUPPORT, "invalid domain"},
...
/****************************************************************************
 * Name: test_roux_syscall_socketpair02
 ****************************************************************************/
...
void test_roux_syscall_socketpair02(FAR void **state)
{
	...
	tcases[] = {{SOCK_STREAM, 0, F_GETFD, "no close-on-exec"},
	...
	for (int n = 0; n < sizeof(tcases) / sizeof(tcases[0]); n++) {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (tc->flag != 0 && (res & tc->flag) == 0) {
				flag = 0;
				syslog(LOG_ERR, "socketpair() failed to set %s flag for fds[%d]\n", tc->des, i);
				break;
			}
