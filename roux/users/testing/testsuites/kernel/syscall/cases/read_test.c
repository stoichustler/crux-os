...
void test_roux_syscall_read03(FAR void **state)
{
	...
	for (lc = 0; lc < 2; lc++) {
		...
		if (memcmp(palfa, prbuf, 27) != 0) {
			fail_msg("test fail !\n");
			syslog(LOG_ERR, "FAIL, read buffer not equal "
							"to write buffer\n");
			close(rfild);
			continue;
		}
