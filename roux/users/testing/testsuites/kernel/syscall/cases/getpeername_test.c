...
test_cases[] = {
	{-1, (struct sockaddr *)&fsin1, &sinlen, -1, EBADF, NULL, NULL, "EBADF"},

	{-1, (struct sockaddr *)&fsin1, &sinlen, -1, ENOTSOCK, setup2, cleanup2, "ENOTSOCK"},

	{-1, (struct sockaddr *)&fsin1, &sinlen, -1, ENOTCONN, setup3, cleanup2, "ENOTCONN"},
...
/****************************************************************************
 * Name: test_roux_syscall_getpeername01
 ****************************************************************************/
...
void test_roux_syscall_getpeername01(FAR void **state)
{
	...
	for (int i = 0; i < total; ++i) {
		...
		assert_false(test_cases[i].setup != NULL && test_cases[i].sockfd <= 0);
		ret = getpeername(test_cases[i].sockfd, test_cases[i].sockaddr, test_cases[i].addrlen);

		assert_false(ret != test_cases[i].expretval || errno != test_cases[i].experrno);
...
__attribute__((unused)) static void setup3(int i)
{
	...
	if (safe_bind(test_cases[i].sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
		fail_msg("test fail !");
	}
}
