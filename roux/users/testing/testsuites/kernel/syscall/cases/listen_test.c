...
/****************************************************************************
 * Name: test_roux_syscall_listen01
 ****************************************************************************/
...
void test_roux_syscall_listen01(FAR void **state)
{
	...
	tdat[] = {
		...
		{PF_INET, SOCK_DGRAM, 0, 0, -1, EOPNOTSUPP, "setup1", "cleanup1", "UDP listen"},
	};
	...
	for (testno = 0; testno < sizeof(tdat) / sizeof(tdat[0]); ++testno) {
		...
		if (rec != tdat[testno].retval || (rec < 0 && errno != tdat[testno].experrno)) {
			syslog(LOG_ERR,
				   "%s ; returned"
				   " %d (expected %d), errno %d (expected"
				   " %d)\n",
				   tdat[testno].desc, rec, tdat[testno].retval, errno, tdat[testno].experrno);
			flag = 0;
		}
