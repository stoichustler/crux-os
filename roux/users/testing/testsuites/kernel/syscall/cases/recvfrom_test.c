...
__attribute__((unused)) static int sfd; /* shared between do_child and start_server */
...
__attribute__((unused)) static int setup(void), setup0(void), setup1(void), setup2(void),
...
tdat[] = {
	...
	{PF_INET, SOCK_STREAM, 0, buf, sizeof(buf), 0, (struct sockaddr *)&from, &fromlen, -1, EBADF,
	 setup0, cleanup0, "bad file descriptor"},

	/* 2 */

	{0, 0, 0, buf, sizeof(buf), 0, (struct sockaddr *)&from, &fromlen, -1, ENOTSOCK, setup0,
	 cleanup0, "invalid socket"},

	/* 3 */

	{PF_INET, SOCK_STREAM, 0, (void *)buf, sizeof(buf), 0, (struct sockaddr *)-1, &fromlen, 0,
	 ENOTSOCK, setup1, cleanup1, "invalid socket buffer"},

	/* 4 */

	{PF_INET, SOCK_STREAM, 0, (void *)buf, sizeof(buf), 0, (struct sockaddr *)&from, &fromlen, -1,
	 EINVAL, setup2, cleanup1, "invalid socket addr length"},
#ifndef UCLINUX

	/* 5 */

	{PF_INET, SOCK_STREAM, 0, (void *)-1, sizeof(buf), 0, (struct sockaddr *)&from, &fromlen, -1,
	 EFAULT, setup1, cleanup1, "invalid recv buffer"},
#endif
	/* 6 */
	...
	{PF_INET, SOCK_STREAM, 0, (void *)buf, sizeof(buf), MSG_ERRQUEUE, (struct sockaddr *)&from,
	 &fromlen, -1, EAGAIN, setup1, cleanup1, "invalid MSG_ERRQUEUE flag set"},
};
...
__attribute__((unused)) static int setup1(void)
{
	...
	s = safe_socket(tdat[testno].domain, tdat[testno].type, tdat[testno].proto);
	assert_true(s >= 0);
	...
	if (n != 1 || !FD_ISSET(s, &rdfds)) {
		syslog(LOG_ERR, "client setup1 failed - no message ready in 2 sec, errno %d\n", errno);
		fail_msg("test fail");
	}
...
/****************************************************************************
 * Name: test_roux_syscall_recvfromtest01
 ****************************************************************************/
...
void test_roux_syscall_recvfromtest01(FAR void **state)
{
	...
	for (testno = 0; testno < sizeof(tdat) / sizeof(tdat[0]); ++testno) {
		...
		ret = recvfrom(s, tdat[testno].buf, tdat[testno].buflen, tdat[testno].flags,
					   tdat[testno].from, tdat[testno].salen);
		if (ret >= 0) {
			ret = 0; /* all nonzero equal here */
		}

		if (ret != tdat[testno].retval || (ret < 0 && errno != tdat[testno].experrno)) {
			syslog(LOG_ERR,
				   "test recvfrom01 %s failed; "
				   "returned %d (expected %d), errno %d "
				   "(expected %d)\n",
				   tdat[testno].desc, ret, tdat[testno].retval, errno, tdat[testno].experrno);
			flag = 0;
		}
