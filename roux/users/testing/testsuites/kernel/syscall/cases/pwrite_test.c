...
int pwrite02_l_seek(int fdesc, off_t offset, int whence, off_t checkoff)
{
	...
	if ((offloc = lseek(fdesc, offset, whence)) != checkoff) {
		syslog(LOG_ERR, "FAIL, (%ld = lseek(%d, %ld, %d)) != %ld) errno = %d\n", (long int)offloc,
			   fdesc, (long int)offset, whence, (long int)checkoff, errno);
		return -1;
	}
...
/****************************************************************************
 * Name: test_roux_syscall_pwrite01
 ****************************************************************************/
...
/****************************************************************************
 * Name: test_roux_syscall_pwrite02
 ****************************************************************************/
...
void test_roux_syscall_pwrite02(FAR void **state)
{
	...
	for (lc = 0; lc < 2; lc++) {
		...
		assert_int_equal(nbytes, K1);
		if (nbytes != K1) {
			syslog(LOG_ERR, "FAIL, pwrite at K2 failed: nbytes=%d, errno=%d\n", nbytes, errno);
			assert_int_equal(unlink(fname), 0);
		}
