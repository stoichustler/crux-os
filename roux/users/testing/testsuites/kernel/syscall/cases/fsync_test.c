...
/****************************************************************************
 * Name: test_roux_syscall_fsync01
 ****************************************************************************/
...
/****************************************************************************
 * Name: test_roux_syscall_fsync02
 ****************************************************************************/
...
void test_roux_syscall_fsync02(FAR void **state)
{
	...
	fifo_rfd = open("/var/Test_Fifo_SyscallFsync02", O_RDONLY | O_NONBLOCK);
	...
	for (int i = 0; i < sizeof(tcases) / sizeof(tcases[0]); i++) {
		...
		if (ret != -1) {
			...
			syslog(LOG_ERR, "fsync(): unexpected error. exp=%d   errno=%d", tcases[i].error, errno);
		} else {
			/* test PASS */
...
/****************************************************************************
 * Name: test_roux_syscall_fsync03
 ****************************************************************************/
...
void test_roux_syscall_fsync03(FAR void **state)
{
	...
	assert_false((time_delta = difftime(time_end, time_start)) > TIME_LIMIT);
