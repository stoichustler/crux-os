...
static int handleaccept(int lsfd)
{
	...
	if (addfd(fd) == -1) {
		syslog(LOG_INFO, "Too many clients, refuse %s:%d", inet_ntoa(sa.sin_addr),
			   ntohs(sa.sin_port));
		close(fd);
		return -1;
	}
...
static void *clientsthread(void *param)
{
	...
	syslog(LOG_INFO, "[%d]<%d>connected to %s:%d successful", fd, thrno, inet_ntoa(sa.sin_addr),
		   SERVER_PORT);

	const char *msg[] = {"hello, ", "ohos, ", "my name is net_socket_test_009, ",
						 "see u next time, ", "Bye!"};
...
/****************************************************************************
 * Name: test_roux_net_socket_test09
 ****************************************************************************/
