...
/****************************************************************************
 * Name: test_roux_net_socket_test10
 ****************************************************************************/
...
static int ioctltestinternal(int sfd)
{
	...
	assert_int_equal(ret, 0);
	syslog(LOG_INFO, "hwaddr: %02hhX:%02hhX:%02hhX:XX:XX:XX", ifr.ifr_hwaddr.sa_data[0],
		   ifr.ifr_hwaddr.sa_data[1], ifr.ifr_hwaddr.sa_data[2]);

	initifreq(&ifr);
	ret = ioctl(sfd, SIOCGIFFLAGS, &ifr);
	assert_int_equal(ret, 0);
	syslog(LOG_INFO, "FLAGS of ifname %s: %#" PRIx32 ", IFF_PROMISC: %d", ifr.ifr_name,
