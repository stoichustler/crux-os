...
/****************************************************************************
 * Name: test_roux_clock_test_smoke01
 ****************************************************************************/
...
void test_roux_clock_test_smoke01(FAR void **state)
{
	...
	struct timespec setts = {0, 0};

	struct timespec oldtp = {0, 0};
	...
	passflag = (ts.tv_sec >= setts.tv_sec) &&
			   (ts.tv_sec <= setts.tv_sec + 1); /* 1, means obtaining time's errno is 1 second. */
	assert_int_equal(ret, 0);
