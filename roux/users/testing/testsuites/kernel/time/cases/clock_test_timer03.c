...
/****************************************************************************
 * Name: test_roux_clock_test_timer03
 ****************************************************************************/
...
void test_roux_clock_test_timer03(FAR void **state)
{
	...
	for (int i = 0; i < sizeof(testcases) / sizeof(testcases[0]); ++i) {
		...
		escaped = end.tv_sec * (int64_t)(1e9) + end.tv_nsec - start.tv_sec * (int64_t)(1e9) -
				  start.tv_nsec;

		failed += (escaped < expected || (escaped - expected) >= 20000000); /* 20000000, 2 ticks. */
		syslog(LOG_INFO, "expected = %" PRId64 " escaped = %" PRId64 "failed = %d", expected,
			   escaped, failed);
	}
