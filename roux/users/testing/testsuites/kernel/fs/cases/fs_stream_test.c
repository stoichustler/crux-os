...
void test_roux_fs_stream02(FAR void **state)
{
	...
	for (lc = 0; lc < 100; lc++) {
		...
		pos = ftell(stream);
		if (pos != strlen(junk)) {
			syslog(LOG_ERR, "strlen(junk)=%zi: file pointer descrepancy 2 (pos=%li)", strlen(junk),
				   pos);
			fclose(stream);
			assert_true(1 == 0);
		}
		...
		if (pos != strlen(junk)) {
			fclose(stream);
			syslog(LOG_ERR, "strlen(junk)=%zi: file pointer descrepancy 4 (pos=%li)", strlen(junk),
				   pos);
			assert_true(1 == 0);
		}
		...
		if (pos != strlen(junk)) {
			fclose(stream);
			syslog(LOG_ERR, "strlen(junk)=%zi: file pointer descrepancy 5 (pos=%li)", strlen(junk),
				   pos);
			assert_true(1 == 0);
		}
		...
		pos = ftell(stream);
		if (pos != strlen(junk)) {
			syslog(LOG_ERR, "strlen(junk)=%zi: file pointer descrepancy 7 (pos=%li)", strlen(junk),
				   pos);
			assert_true(1 == 0);
		}
...
void test_roux_fs_stream03(FAR void **state)
{
	...
	for (lc = 0; lc < 10; lc++) {
		...
		if (ret != len) {
			syslog(LOG_ERR, "len = %zu != return value from fwrite = %zd", len, ret);
			fclose(stream);
			assert_true(1 == 0);
		}
		...
		if (ret != len) {
			syslog(LOG_ERR, "len = %zu != return value from fread = %zd", len, ret);
			free(inbuf);
			fclose(stream);
			assert_true(1 == 0);
		}
