...
/****************************************************************************
 * Name: test_roux_fs_verifylseek
 ****************************************************************************/

static int test_roux_fs_verifylseek(unsigned int n)
{
	char read_buf[64];
	...
	if (ret != tc->exp_off) {
		syslog(LOG_ERR, "lseek(%s, %lld, %s) returned %d, expected %lld\n", TFILE,
			   (long long)tc->off, tc->wname, ret, (long long)tc->exp_off);
		return -1;
	}
...
/****************************************************************************
 * Name: test_roux_fs_set
 ****************************************************************************/

static void test_roux_fs_set(void)
{
...
/****************************************************************************
 * Name: test_roux_fs_clean
 ****************************************************************************/

static void test_roux_fs_clean(void)
{
	if (fd > 0)
		close(fd);
}
...
void test_roux_fs_seek02(FAR void **state)
{
	...
	ret = test_roux_fs_verifylseek(0);
	assert_int_equal(ret, 0);

	/* do verify lseek */

	ret = test_roux_fs_verifylseek(1);
	assert_int_equal(ret, 0);

	/* do verify lseek */

	ret = test_roux_fs_verifylseek(2);
	assert_int_equal(ret, 0);

	/* do verify lseek */

	ret = test_roux_fs_verifylseek(3);
	assert_int_equal(ret, 0);
