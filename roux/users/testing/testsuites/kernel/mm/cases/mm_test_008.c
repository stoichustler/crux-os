...
static int test_roux_08_routine_1(int argc, char *argv[])
{
	char *ptr = NULL;
	char *tmp_ptr = NULL;
	int malloc_size;
...
static int test_roux_08_routine_2(int argc, char *argv[])
{
	char *temp_ptr = NULL;
	int flag = 0;
...
static int test_roux_08_routine_3(int argc, char *argv[])
{
	char *pm;
	...
	for (int i = 0; i < 500; i++) {
		...
		if (mem_info.mxordblk - 16 < 0) {
			syslog(LOG_INFO, "TEST END because of the mem_info.mxordblk is:%d", mem_info.mxordblk);
			break;
		}
...
void test_roux_mm08(FAR void **state)
{
	pid_t pid;
	int status;

	pid = task_create("test_roux_08_routine_1", TASK_PRIORITY, DEFAULT_STACKSIZE,
					  test_roux_08_routine_1, NULL);
	assert_true(pid > 0);
	pid = task_create("test_roux_08_routine_2", TASK_PRIORITY, DEFAULT_STACKSIZE,
					  test_roux_08_routine_2, NULL);
	assert_true(pid > 0);
	pid = task_create("test_roux_08_routine_3", TASK_PRIORITY, DEFAULT_STACKSIZE,
					  test_roux_08_routine_3, NULL);
	assert_true(pid > 0);
