...
static const unsigned char g_rsa_n[] = "9292758453063D803DD603D5E777D788"
...
static const unsigned char g_rsa_d[] = "24BF6185468786FDD303083D25E64EFC"
...
static const unsigned char g_message[] = "\xAA\xBB\xCC\x03\x02\x01\x00\xFF\xFF\xFF\xFF\xFF"
...
static void pkcs1_v15_padding(FAR unsigned char *padding, size_t pad_len)
{
	int i;
...
static void mpi_from_string(FAR unsigned char *buf, FAR const unsigned char *s, size_t slen)
{
	int i;
	int j;
	uint8_t d = 0;
...
static int rsa_data_init(FAR rsa_data_t *data, FAR const unsigned char *e, size_t elen,
						 FAR const unsigned char *d, size_t dlen, FAR const unsigned char *n,
						 size_t nlen)
{
...
static int cryptodev_rsa_calc(FAR crypto_context_t *ctx, FAR rsa_data_t *data,
							  FAR const unsigned char *input, size_t ilen,
							  FAR const unsigned char *output, size_t olen, bool is_priv)
{
...
static int cryptodev_rsa_verify(FAR crypto_context_t *ctx, FAR rsa_data_t *data,
...
static int rsa_test(void)
{
	...
	ret = rsa_data_init(&rsa_data, g_rsa_e, sizeof(g_rsa_e) - 1, g_rsa_d, sizeof(g_rsa_d) - 1,
	...
	ret = cryptodev_rsa_calc(&ctx, &rsa_data, g_message, PLAIN_LEN, cipher, MOD_LEN, TRUE);
	if (ret != 0) {
	...
	ret = cryptodev_rsa_calc(&ctx, &rsa_data, cipher, MOD_LEN, plain, PLAIN_LEN, FALSE);
	if (ret != 0) {
	...
	ret = cryptodev_rsa_calc(&ctx, &rsa_data, data_he, MOD_LEN, sig, MOD_LEN, TRUE);
	if (ret != 0) {
	...
	ret = cryptodev_rsa_verify(&ctx, &rsa_data, sig, MOD_LEN, hash_he, PLAIN_LEN, pad_he,
							   PADDING_LEN);
	if (ret != 0) {
