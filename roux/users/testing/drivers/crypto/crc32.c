...
static int syscrc32_update(FAR crypto_context *ctx, FAR const char *s, size_t len)
{
...
int main(void)
{
	...
	for (i = 0; i < sizeof(g_crc32_testcase) / sizeof(tb) - 1; i++) {
		...
		ret = syscrc32_update(&crc32_ctx, g_crc32_testcase[i].data, g_crc32_testcase[i].datalen);
		if (ret) {
	...
	for (i = 0; i < 8; i++) {
		...
		ret = syscrc32_update(&crc32_ctx, g_crc32_testcase[7].data, g_crc32_testcase[7].datalen);
		if (ret) {
			printf("syscrc32 update failed\n");
			goto err;
		}
