...
static int syscrypt(FAR const char *key, size_t klen, FAR const char *iv, FAR const char *in,
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < nitems(testcase); i++) {
		ret = syscrypt(testcase[i].key, 16, testcase[i].iv, testcase[i].plain, out, testcase[i].len,
					   1);
		if (ret) {
		...
		ret = match(out, (FAR unsigned char *)testcase[i].cipher, testcase[i].len);
		if (ret) {
			printf("aescbc encrypt field in testcase:%d\n", i);
			return -1;
		}

		ret = syscrypt(testcase[i].key, 16, testcase[i].iv, testcase[i].cipher, out,
					   testcase[i].len, 0);
		if (ret) {
		...
		ret = match(out, (FAR unsigned char *)testcase[i].plain, testcase[i].len);
		if (ret) {
			printf("aescbc decrypt field in testcase:%d\n", i);
			return -1;
		}
