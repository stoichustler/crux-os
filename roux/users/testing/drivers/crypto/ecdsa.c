...
static int ecdsa_data_init(FAR ecdsa_data_t *data, size_t xlen, size_t ylen, size_t zlen,
...
static int ecdsa_sign(int op, FAR struct pollfd *fds, FAR crypto_context_t *ctx,
					  FAR ecdsa_data_t *data, FAR const unsigned char *buf, size_t buflen)
{
...
static int ecdsa_verify(int op, FAR struct pollfd *fds, FAR crypto_context_t *ctx,
						FAR ecdsa_data_t *data, FAR const unsigned char *buf, size_t buflen)
{
	struct crypt_kop cryptk;
...
static int ecdsa_genkey(int op, FAR struct pollfd *fds, FAR crypto_context_t *ctx,
						FAR ecdsa_data_t *data)
{
	struct crypt_kop cryptk;
...
static int test_p256(int mode)
{
	...
	ret = ecdsa_genkey(CRK_ECDSA_SECP256R1_GENKEY, mode == SYNC ? NULL : &fds, &ctx, &data);
	if (ret != 0) {
	...
	ret = ecdsa_sign(CRK_ECDSA_SECP256R1_SIGN, mode == SYNC ? NULL : &fds, &ctx, &data,
					 sha256_message, len);
	if (ret != 0) {
	...
	ret = ecdsa_verify(CRK_ECDSA_SECP256R1_VERIFY, mode == SYNC ? NULL : &fds, &ctx, &data,
					   sha256_message, len);
	if (ret != 0) {
