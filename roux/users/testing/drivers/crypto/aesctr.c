...
struct {
	...
	{{"ae 68 52 f8 12 10 67 cc 4b f7 a5 76 55 77 f3 9e "
	  "00 00 00 30",
	  "00 00 00 00 00 00 00 00 00 00 00 01", "53 69 6e 67 6c 65 20 62 6c 6f 63 6b 20 6d 73 67",
	...
	{{"77 6b ef f2 85 1d b0 6f 4c 8a 05 42 c8 69 6f 6c "
	  ...
	  "00 00 00 60",
	  "db 56 72 c9 7a a8 f0 b2 00 00 00 01", "53 69 6e 67 6c 65 20 62 6c 6f 63 6b 20 6d 73 67",
...
static int syscrypt(FAR const unsigned char *key, size_t klen, FAR const unsigned char *iv,
					FAR const unsigned char *in, FAR unsigned char *out, size_t len, int encrypt)
{
...
static int run(int num)
{
	...
	if (syscrypt(data[TST_KEY], length[TST_KEY], data[TST_IV], data[TST_PLAIN], p,
