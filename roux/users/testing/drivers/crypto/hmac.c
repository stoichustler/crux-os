...
FAR char *md5_result[] = {"\x92\x94\x72\x7a\x36\x38\xbb\x1c\x13\xf4\x8e\xf8\x15\x8b\xfc\x9d",
						  "\x75\x0c\x78\x3e\x6a\xb0\xb5\x03\xea\xa8\x6e\x31\x0a\x5d\xb7\x38",
						  "\x56\xbe\x34\x52\x1d\x14\x4c\x88\xdb\xb8\xc7\x33\xf0\xe8\xb3\xf6"};

FAR char *sha1_result[] = {"\x67\x5b\x0b\x3a\x1b\x4d\xdf\x4e\x12\x48\x72\xda\x6c\x2f\x63\x2b"
						   "\xfe\xd9\x57\xe9",
...
FAR char *sha256_result[] = {"\x49\x2c\xe0\x20\xfe\x25\x34\xa5\x78\x9d\xc3\x84\x88\x06\xc7\x8f"
...
int syshmac(int mac, FAR const char *key, size_t keylen, FAR const char *s, size_t len,
			FAR char *out)
{
...
int main(void)
{
	...
	for (int i = 0; i < nitems(testcase); i++) {
		ret += syshmac(CRYPTO_MD5_HMAC, testcase[i].key, testcase[i].keylen, testcase[i].data,
					   testcase[i].datalen, output);
		if (ret) {
			printf("syshamc md5 failed\n");
		}

		ret += match((unsigned char *)md5_result[i], (unsigned char *)output, MD5_DIGEST_LENGTH);
		if (ret) {
	...
	for (int i = 0; i < nitems(testcase); i++) {
		ret = syshmac(CRYPTO_SHA1_HMAC, testcase[i].key, testcase[i].keylen, testcase[i].data,
					  testcase[i].datalen, output);
		if (ret) {
	...
	for (int i = 0; i < nitems(testcase); i++) {
		ret = syshmac(CRYPTO_SHA2_256_HMAC, testcase[i].key, testcase[i].keylen, testcase[i].data,
					  testcase[i].datalen, output);
		if (ret) {
