...
#include <fcntl.h>
#include <poll.h>
#if defined(__ROUX__) && (defined(CONFIG_DEV_RPMSG) || defined(CONFIG_DEV_RPMSG_SERVER))
...
static ssize_t testdev_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t testdev_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int testdev_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int testdev_open(FAR struct file *filep)
{
	if ((filep->f_oflags & (OPEN_FLAG | O_NONBLOCK)) == (OPEN_FLAG | O_NONBLOCK)) {
		filep->f_pos = 0;
...
static off_t testdev_seek(FAR struct file *filep, off_t offset, int whence)
{
	if (offset != OFFSET || whence != WHENCE) {
		syslog(LOG_ERR,
			   "test seek fail, offset %ld(except %ld), "
			   "whence %d(except %d)\n",
...
static int testdev_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case IOCTL:
		if (arg == 0) {
			syslog(LOG_ERR,
				   "test ioctl fail, cmd %d(except %d), "
...
static void _register_driver(int mode, char *cpu, char *remotepath, char *localpath)
{
	int ret = -EINVAL;
	...
	while ((finish - start) < hour * 3600) {
		...
		if (cnt % 50000) {
			...
			syslog(LOG_INFO, "[%s] time start: %ld finish: %ld now: %ld\n", path, (long)start,
				   (long)(start + hour * 3600), (long)finish);
		}
	}
...
{
	syslog(LOG_WARNING, "Usage: CMD [-d <regist driver>] [-c <remote cpu>] "
