...
static const char *test_pm_name[PM_COUNT] = {"normal", "idle", "standby", "sleep"};
...
static int test_pm_fake_driver_init(void)
{
	...
	for (int i = 0; i < CONFIG_SMP_NCPUS; i++) {
		...
		for (int state = 0; state < PM_COUNT; state++) {
			...
			pm_wakelock_init(&entry->wakelock[state], name, entry->domain, state);
		}
	}
...
int main(int argc, FAR char *argv[])
{
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_prestate_setup_teardown(drivertest_pm_smp, setup, teardown, NULL),
	};
