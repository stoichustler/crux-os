...
static void lcd_help(FAR const char *progname, FAR struct lcd_state_s *lcd_state, int exitcode)
{
	printf("Usage: %s"
	...
	printf("  [-p devpath] selects the lcd device.  "
		   "Default: %s Current: %s\n",
		   LCD_DEFAULT_DEVPATH, lcd_state->devpath);
...
static void parse_commandline(FAR struct lcd_state_s *lcd_state, int argc, FAR char **argv)
{
	int ch;
	int converted;
	...
	while ((ch = getopt(argc, argv, "p:d:n:f:t:h")) != ERROR) {
		switch (ch) {
		case 'p':
			snprintf(lcd_state->devpath, sizeof(lcd_state->devpath), "%s", optarg);
			break;
		case 't':
	...
	printf("devname = %s\n"
		   "testcase = %d\n",
		   lcd_state->devpath, lcd_state->test_case_id);
}
...
static int lcd_setup(FAR void **state)
{
	...
	ret = ioctl(lcd_state->lcd_info.fd, LCDDEVIO_GETPLANEINFO, &lcd_state->lcd_info.plane_info);
	assert_int_equal(ret, 0);

	ret = ioctl(lcd_state->lcd_info.fd, LCDDEVIO_GETVIDEOINFO, &lcd_state->lcd_info.video_info);
	assert_int_equal(ret, 0);

	ret = ioctl(lcd_state->lcd_info.fd, LCDDEVIO_GETAREAALIGN, &lcd_state->lcd_info.align_info);
	assert_int_equal(ret, 0);
...
static void draw_rect_buf_alloc(FAR struct lcd_info_s *lcd_info, FAR struct lcddev_area_s *area)
{
...
static void draw_rect(FAR struct lcd_info_s *lcd_info, int x, int y, int w, int h, uint32_t color)
{
	int i = 0;
	int j = 0;
	int ret = 0;
...
static void drivertest_lcd_cross(FAR void **state)
{
	...
	int step_width = 0;
	int step_num = 0;
	uint32_t colors_to_show[] = {0x00000000, 0x00ff0000, 0x0000ff00, 0x000000ff, 0x00ffffff};
	...
	for (i = 0; i < step_num; i++) {
		...
		draw_rect(&lcd_state->lcd_info, start_x, 0, step_width, yres, colors_to_show[i]);
	}
}
...
int main(int argc, FAR char *argv[])
{
	...
	snprintf(lcd_state.devpath, sizeof(lcd_state.devpath), "%s", LCD_DEFAULT_DEVPATH);

	parse_commandline(&lcd_state, argc, argv);

	const struct CMUnitTest tests[] = {
		cmocka_unit_test_prestate_setup_teardown(drivertest_lcd_black, lcd_setup, lcd_teardown,
												 &lcd_state),
		cmocka_unit_test_prestate_setup_teardown(drivertest_lcd_white, lcd_setup, lcd_teardown,
												 &lcd_state),
		cmocka_unit_test_prestate_setup_teardown(drivertest_lcd_cross, lcd_setup, lcd_teardown,
												 &lcd_state),
		cmocka_unit_test_prestate_setup_teardown(drivertest_lcd_vertical, lcd_setup, lcd_teardown,
												 &lcd_state),
	};
