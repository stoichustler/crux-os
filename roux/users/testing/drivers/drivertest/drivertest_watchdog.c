...
static void show_usage(FAR const char *progname, FAR struct wdg_state_s *wdg_state, int exitcode)
{
	...
	printf("  [-d devpath] selects the WATCHDOG device.\n"
		   "  Default: %s Current: %s\n",
		   WDG_DEFAULT_DEV_PATH, wdg_state->devpath);
	printf("  [-r test_case] selects the testcase.\n"
		   "  Default: %d Current: %d\n",
		   WDG_DEFAULT_TESTCASE, wdg_state->test_case);
...
static void parse_commandline(FAR struct wdg_state_s *wdg_state, int argc, FAR char **argv)
{
	int ch;
	int converted;
	...
	while ((ch = getopt(argc, argv, "d:r:t:l:o:a:gh")) != ERROR) {
		...
		case 'r':
			...
			if (converted < WDG_DEFAULT_TESTCASE || converted >= WDG_COUNT_TESTCASE) {
...
static void drivertest_watchdog_api(FAR void **state)
{
	...
	while (get_time_elaps(start_ms) < wdg_state->pingtime) {
		...
		if (wdg_state->test_getstatus) {
			...
			assert_in_range(status.timeout - status.timeleft,
...
{
	struct wdg_state_s wdg_state = {.devpath = WDG_DEFAULT_DEV_PATH,
