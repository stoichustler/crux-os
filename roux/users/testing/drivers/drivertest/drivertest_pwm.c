...
static void pwm_help(FAR const char *progname, FAR struct pwm_state_s *pwm_state, int exitcode)
{
	printf("Usage: %s"
	...
	printf("  [-p devpath] selects the PWM device.  "
		   "Default: %s Current: %s\n",
		   PWM_DEFAULT_DEVPATH, pwm_state->devpath);
...
static void parse_commandline(FAR struct pwm_state_s *pwm_state, int argc, FAR char **argv)
{
	int ch;
	int converted;
	...
	printf("devname = %s\n"
		   ...
		   "frenquency = %" PRIu32 "\n"
		   "duration = %d\n",
		   pwm_state->devpath, pwm_state->duty, pwm_state->freq, pwm_state->duration);
...
int main(int argc, FAR char *argv[])
{
	...
	const struct CMUnitTest tests[] = {cmocka_unit_test_prestate(drivertest_pwm, &pwm_state)};
