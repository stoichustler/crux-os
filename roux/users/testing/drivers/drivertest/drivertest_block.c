...
static void parse_commandline(int argc, FAR char **argv, FAR struct pre_build_s *pre)
{
	int option;
...
static int setup_driver(FAR void **state)
{
	...
	if (ret != 0) {
		...
		ret = MTD_IOCTL(pre->driver->u.i_mtd, MTDIOC_GEOMETRY, (unsigned long)&pre->geo);
		assert_false(ret < 0);
	}
...
int main(int argc, FAR char *argv[])
{
	...
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_prestate_setup_teardown(drivertest_block_stress, setup_bch, teardown_bch,
												 pre),
		cmocka_unit_test_prestate_setup_teardown(drivertest_block_single_write, setup_driver,
												 teardown_driver, pre),
		cmocka_unit_test_prestate_setup_teardown(drivertest_block_cache_write, setup_driver,
												 teardown_driver, pre),
	};
