...
static void parse_commandline(int argc, FAR char **argv, FAR struct pre_build_s *pre_build)
{
	int option;
...
int main(int argc, FAR char *argv[])
{
	...
	FAR struct pre_build_s pre_build = {
		.gpio_a = "dev/gpio0", .gpio_b = "dev/gpio1", .loop = false};
	...
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_prestate_setup_teardown(drivertest_gpio, setup, teardown, &pre_build),
	};
