...
static int test_pm_callback_prepare(FAR struct pm_callback_s *cb, int domain,
									enum pm_state_e pmstate);
static void test_pm_callback_notify(FAR struct pm_callback_s *cb, int domain,
									enum pm_state_e pmstate);
...
static struct pm_callback_s g_test_pm_callback = {.prepare = test_pm_callback_prepare,
												  .notify = test_pm_callback_notify};

static struct pm_wakelock_s g_test_wakelock[PM_COUNT];
static char test_wakelock_name[PM_COUNT][32] = {"test_pm_wakelock_normal", "test_pm_wakelock_idle",
...
static void drivertest_pm(FAR void **argv)
{
	...
	if (CONFIG_PM_GOVERNOR_EXPLICIT_RELAX < 0) {
		...
		check = true;
		init_delay = MAX(CONFIG_PM_GOVERNOR_EXPLICIT_RELAX, CONFIG_SERIAL_PM_ACTIVITY_PRIORITY);
	}
...
static int setup(FAR void **argv)
{
	...
	for (int state = 0; state < PM_COUNT; state++) {
		pm_wakelock_init(&g_test_wakelock[state], test_wakelock_name[state], domain, state);
	}
...
int main(int argc, FAR char *argv[])
{
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_prestate_setup_teardown(drivertest_pm, setup, teardown, NULL),
	};
