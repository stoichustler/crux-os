...
static void parse_commandline(FAR struct posix_timer_state_s *posix_timer_state, int argc,
							  FAR char **argv)
{
	int ch;
	int converted;
...
{
	FAR struct posix_timer_state_s *sigev_para = (FAR struct posix_timer_state_s *)arg.sival_ptr;
	int range = get_timestamp() - (*sigev_para).tim;

	assert_in_range(range, sigev_para->it.it_interval.tv_sec * 1000 - sigev_para->deviation,
...
static void drivertest_posix_timer(FAR void **state)
{
	...
	assert_return_code(ret, OK);
	assert_in_range(it.it_value.tv_sec, 0, posix_timer_state->it.it_value.tv_sec);
	assert_return_code(it.it_interval.tv_sec, posix_timer_state->it.it_interval.tv_sec);
...
{
	struct posix_timer_state_s posix_timer_state = {.it.it_value.tv_sec = DEFAULT_TIME_OUT,
													.it.it_value.tv_nsec = 0,
													.it.it_interval.tv_sec = DEFAULT_TIME_OUT,
													.it.it_interval.tv_nsec = 0,
													.deviation = RTC_DEFAULT_DEVIATION};

	const struct CMUnitTest tests[] = {cmocka_unit_test_prestate_setup_teardown(
		drivertest_posix_timer, NULL, NULL, &posix_timer_state)};
