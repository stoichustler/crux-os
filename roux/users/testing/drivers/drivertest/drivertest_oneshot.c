...
static void parse_commandline(FAR struct oneshot_state_s *oneshot_state, int argc, FAR char **argv)
{
	int ch;
	time_t converted;
	...
	while ((ch = getopt(argc, argv, "s:d:")) != ERROR) {
		...
		case 'd':
			strlcpy(oneshot_state->devpath, optarg, sizeof(oneshot_state->devpath));
			break;
...
static void drivertest_oneshot(FAR void **state)
{
	...
	sigset_t set;
	FAR struct oneshot_state_s *oneshot_state = (FAR struct oneshot_state_s *)*state;
	...
	for (i = 0; i < ONESHOT_DEFAULT_NSAMPLES; i++) {
		...
		assert_in_range(timespec_sub.tv_sec, oneshot_state->oneshot.ts.tv_sec - 1,
	...
	clock_timespec_subtract(&oneshot_state->oneshot.ts, &ts, &timespec_sub);
...
int main(int argc, FAR char *argv[])
{
	struct oneshot_state_s oneshot_state = {
		.devpath = "/dev/oneshot", .oneshot.ts.tv_sec = DEFAULT_TIME_OUT, .oneshot.ts.tv_nsec = 0};
