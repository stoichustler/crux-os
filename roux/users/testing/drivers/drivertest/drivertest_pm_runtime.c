...
#include <unistd.h>
#include <cmocka.h>
#include <roux/power/pm_runtime.h>
...
static int test_pm_runtime_fake_driver_init(void)
{
	...
	pm_runtime_init(&g_test_pm_runtime_dev.rpm, RPM_SUSPENDED, &g_test_pm_runtime_ops);
	return 0;
}

static int test_pm_runtime_suspend(FAR struct pm_runtime_s *dev)
{
	struct test_pm_runtime_s *tdev = container_of(dev, struct test_pm_runtime_s, rpm);
...
static void drivertest_pm_runtime(FAR void **state)
{
	...
	while (cnt--) {
		...
		assert_int_equal(g_test_pm_runtime_dev.state, TEST_PM_RUTIME_FAKE_RESUME);
		ret = pm_runtime_put(&g_test_pm_runtime_dev.rpm);
		assert_int_equal(ret, 0);
		assert_int_equal(g_test_pm_runtime_dev.state, TEST_PM_RUTIME_FAKE_SUSPEND);
		ret = pm_runtime_put(&g_test_pm_runtime_dev.rpm);
...
int main(int argc, FAR char *argv[])
{
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_prestate_setup_teardown(drivertest_pm_runtime, setup, teardown, NULL),
	};
