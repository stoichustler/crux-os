...
#define CONFIG_TESTING_DRIVER_TEST_UART_DEFAULT_CONTENT                                            \
	"0123456789abcdefg"                                                                            \
...
static uint32_t read_until(int fd, FAR char *buffer, uint32_t buffer_len, char terminator)
{
	uint32_t cnt = 0;
	ssize_t len = 0;
	char tmp_char = 0;
...
static uint32_t read_length(int fd, FAR char *buffer, uint32_t max_size)
{
	uint32_t cnt = 0;
	ssize_t len = 0;
...
static uint32_t write_length(int fd, FAR const char *buffer, uint32_t max_size)
{
	uint32_t cnt = 0;
	ssize_t len = 0;
...
{
	FAR struct test_state_s *test_state = (FAR struct test_state_s *)*state;
	int res = write(test_state->fd, DEFAULT_CONTENT, sizeof(DEFAULT_CONTENT) - 1);
...
static void drivertest_uart_burst(FAR void **state)
{
	...
	while (true) {
		...
		res = read_until(test_state->fd, ret_msg_buffer, sizeof(ret_msg_buffer), '#');
...
static void parse_args(int argc, FAR char **argv, FAR struct test_confs_s *conf)
{
	int option;
...
int main(int argc, FAR char *argv[])
{
	...
	struct test_confs_s confs = {.dev_path = CONFIG_TESTING_DRIVER_TEST_UART_DEVICE,
								 .test_case_id = 0};
	...
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_prestate_setup_teardown(drivertest_uart, setup, teardown, &confs),
	};
