...
static int audio_test_writebuffer(FAR struct audio_state_s *state, FAR struct ap_buffer_s *apb)
{
	int ret = 0;
...
static int audio_test_readbuffer(FAR struct audio_state_s *state, FAR struct ap_buffer_s *apb)
{
	if (state->out_fd == -1) {
		return -ENODATA;
	}
...
static int audio_test_enqueuebuffer(FAR struct audio_state_s *state, FAR struct ap_buffer_s *apb,
									int direction)
{
	...
	fd = direction == AUDIO_TYPE_OUTPUT ? state->outdev_fd : state->indev_fd;
...
static int audio_test_alloc_buffer(FAR struct audio_state_s *state, FAR struct ap_buffer_s **bufs,
								   FAR struct ap_buffer_info_s *buf_info, int direction)
{
...
static int audio_test_free_buffer(FAR struct audio_state_s *state, FAR struct ap_buffer_s **bufs,
								  FAR struct ap_buffer_info_s *buf_info, int direction)
{
	struct audio_buf_desc_s buf_desc;
	int ret = 0;
...
static void parse_commandline(FAR struct audio_state_s *state, int argc, FAR char **argv)
{
	int option;
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
	ret = ioctl(fd, AUDIOIOC_START, (unsigned long)state->session);
#else
...
static bool audio_test_timeout(FAR struct audio_state_s *state, int direction, struct timeval start)
{
	struct timeval now;
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
	ret = ioctl(fd, AUDIOIOC_STOP, (unsigned long)state->session);
#else
...
static void drivertest_audio(void **audio_state)
{
	...
	int directions[3] = {AUDIO_TYPE_INPUT, AUDIO_TYPE_OUTPUT, -1};
	...
	while (directions[x] != -1) {
		...
		bufs = (FAR struct ap_buffer_s **)calloc(buf_info.nbuffers, sizeof(FAR void *));
		...
		printf("Start %s. \n", direct == AUDIO_TYPE_OUTPUT ? "Playback" : "Capture");
			...
			case AUDIO_MSG_DEQUEUE:
				...
				if (streaming) {
					...
					if (ret != OK) {
						...
						ret = audio_test_enqueuebuffer(state, msg.u.ptr, direct);
						if (ret != OK) {
							close(state->out_fd);
							state->out_fd = -1;
							streaming = false;
...
static int audio_test_setup(FAR void **audio_state)
{
	...
	if (state->direction & AUDIO_TYPE_OUTPUT) {
		...
		ret = ioctl(state->outdev_fd, AUDIOIOC_GETBUFFERINFO, (unsigned long)&buf_info);
		maxmsg = MAX(maxmsg, ret >= 0 ? buf_info.nbuffers : CONFIG_AUDIO_NUM_BUFFERS);
	}
	...
	if (state->direction & AUDIO_TYPE_INPUT) {
		...
		ret = ioctl(state->indev_fd, AUDIOIOC_GETBUFFERINFO, (unsigned long)&buf_info);
		maxmsg = MAX(maxmsg, ret >= 0 ? buf_info.nbuffers : CONFIG_AUDIO_NUM_BUFFERS);
	}
	...
	snprintf(state->mqname, sizeof(state->mqname), "/tmp/%p", ((void *)state));
...
int main(int argc, FAR char *argv[])
{
	...
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_prestate_setup_teardown(drivertest_audio, audio_test_setup,
												 audio_test_teardown, &state),
	};
