...
static void fb_help(FAR const char *progname, FAR struct fb_state_s *fb_state, int exitcode)
{
	printf("Usage: %s"
	...
	printf("  [-p devpath] selects the framebuffer device.  "
		   "Default: %s Current: %s\n",
		   FB_DEFAULT_DEVPATH, fb_state->devpath);
...
static void parse_commandline(FAR struct fb_state_s *fb_state, int argc, FAR char **argv)
{
	int ch;
	int converted;
	...
	while ((ch = getopt(argc, argv, "p:d:n:f:t:h")) != ERROR) {
		switch (ch) {
		case 'p':
			snprintf(fb_state->devpath, sizeof(fb_state->devpath), "%s", optarg);
			break;
		case 't':
	...
	printf("devname = %s\n"
		   "testcase = %d\n",
		   fb_state->devpath, fb_state->test_case_id);
}
...
static int fb_init_mem2(FAR struct fb_state_s *state, FAR struct fb_planeinfo_s *pinfo)
{
	int ret;
	...
	ret = ioctl(state->fb_device, FBIOGET_PLANEINFO, (unsigned long)(uintptr_t)pinfo);
	if (ret < 0) {
		int errcode = errno;
	...
	if (buf_offset == 0) {
		...
		state->fb_info.fb_mem2 = pinfo->fbmem + state->fb_info.mem2_yoffset * pinfo->stride;
	} else {
		/* Use non-consecutive fbmem2. */

		state->fb_info.mem2_yoffset = buf_offset / state->fb_info.plane_info.stride;
...
static int fb_setup(FAR void **state)
{
	...
	ret = ioctl(fb_state->fb_device, FBIOGET_PLANEINFO, &fb_state->fb_info.plane_info);
	assert_int_equal(ret, 0);

	ret = ioctl(fb_state->fb_device, FBIOGET_VIDEOINFO, &fb_state->fb_info.video_info);
	assert_int_equal(ret, 0);

	fb_state->fb_info.fb_mem =
		mmap(NULL, fb_state->fb_info.plane_info.fblen, PROT_READ | PROT_WRITE,
			 MAP_SHARED | MAP_FILE, fb_state->fb_device, 0);

	assert_ptr_not_equal(fb_state->fb_info.fb_mem, MAP_FAILED);

	/* double buffer mode */

	if (fb_state->fb_info.plane_info.yres_virtual == (fb_state->fb_info.video_info.yres * 2)) {
...
static void pan_display(FAR struct fb_state_s *state)
{
	...
	if (ret > 0) {
		...
		state->fb_info.act_fbmem = state->fb_info.act_fbmem == state->fb_info.fb_mem
									   ? state->fb_info.fb_mem2
									   : state->fb_info.fb_mem;
	}
}
...
static void sync_area(FAR struct fb_state_s *state)
{
	...
	if (state->fb_info.act_fbmem == state->fb_info.fb_mem) {
		memcpy(state->fb_info.fb_mem, state->fb_info.fb_mem2,
			   state->fb_info.video_info.yres * state->fb_info.plane_info.stride);
	} else {
...
static void draw_rect(FAR struct fb_state_s *fb_state, int x, int y, int w, int h, uint32_t color)
{
	int i = 0;
	int j = 0;
	uint8_t *fb_tmp;
...
#ifdef CONFIG_FB_UPDATE
	ret = ioctl(fb_state->fb_device, FBIO_UPDATE, (unsigned long)((uintptr_t)&area));
	assert_int_equal(ret, 0);
#endif
...
static void drivertest_framebuffer_cross(FAR void **state)
{
	...
	int step_width = 0;
	int step_num = 0;
	uint32_t colors_to_show[] = {0xff000000, 0xffff0000, 0xff00ff00, 0xff0000ff, 0xffffffff};
...
int main(int argc, FAR char *argv[])
{
	...
	snprintf(fb_state.devpath, sizeof(fb_state.devpath), "%s", FB_DEFAULT_DEVPATH);

	parse_commandline(&fb_state, argc, argv);

	const struct CMUnitTest tests[] = {
		cmocka_unit_test_prestate_setup_teardown(drivertest_framebuffer_black, fb_setup,
												 fb_teardown, &fb_state),
		cmocka_unit_test_prestate_setup_teardown(drivertest_framebuffer_white, fb_setup,
												 fb_teardown, &fb_state),
		cmocka_unit_test_prestate_setup_teardown(drivertest_framebuffer_cross, fb_setup,
												 fb_teardown, &fb_state),
		cmocka_unit_test_prestate_setup_teardown(drivertest_framebuffer_vertical, fb_setup,
												 fb_teardown, &fb_state),
	};
