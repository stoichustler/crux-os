...
static void show_usage(FAR const char *progname, FAR struct rtc_state_s *rtc_state, int exitcode)
{
	printf("Usage: %s -d <devpath> -a <deviation> -v <delay>\n", progname);
	printf("  [-d devpath] selects the rtc device.\n"
		   "  Default: %s Current: %s\n",
		   RTC_DEFAULT_DEVPATH, rtc_state->devpath);
	printf("  [-a deviation] input rtc alarm .\n"
		   "  Default: %d Current: %d\n",
		   RTC_DEFAULT_DEVIATION, rtc_state->deviation);
	printf("  [-v delay] delay after set rtc.\n"
		   "  Default: %d Current: %d\n",
		   VENDOR_DELAY, rtc_state->vendor_delay);
	exit(exitcode);
}
...
static void parse_commandline(FAR struct rtc_state_s *rtc_state, int argc, FAR char **argv)
{
	int ch;
	int converted;
...
static void drivertest_rtc_api(FAR void **state)
{
	...
	ret = ioctl(fd, RTC_HAVE_SET_TIME, (unsigned long)((uintptr_t)&have_set_time));
	assert_return_code(ret, OK);
	...
	ret = strftime(timbuf, sizeof(timbuf), "%a, %b %d %H:%M:%S %Y", (FAR struct tm *)&rd_time);
	assert_return_code(ret, OK);
