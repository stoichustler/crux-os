...
#include <roux/input/touchscreen.h>
...
#ifdef CONFIG_LV_USE_ROUX_LIBUV
#include <uv.h>
#endif
...
void darw_canvas_frame(touchpad_s *touchpad)
{
	...
	touchpad->line_k = (LV_VER_RES - SQUARE_Y / 2) / (float)(LV_HOR_RES - SQUARE_X / 2);
	touchpad->line_b_up_1 =
		(SQUARE_Y / 2 - LV_VER_RES) / (float)(LV_HOR_RES - SQUARE_X / 2) * (SQUARE_X / 2);
	...
	touchpad->line_b_down_2 =
		(LV_HOR_RES * LV_VER_RES - (SQUARE_Y * SQUARE_Y / 4)) / (float)(LV_HOR_RES - SQUARE_X / 2);
}
...
void draw_track_line(lv_obj_t *canvas, lv_point_t pos)
{
	...
	for (index_x = pos.x - 1; index_x < pos.x + 2; index_x++) {
		for (index_y = pos.y - 1; index_y < pos.y + 2; index_y++) {
			lv_canvas_set_px(canvas, index_x, index_y, lv_palette_main(LV_PALETTE_RED),
							 LV_OPA_COVER);
		}
	}
}
...
void draw_touch_fill(touchpad_s *touchpad, lv_point_t pos)
{
	...
	if (x_index == 0 || x_index == LV_HOR_RES / SQUARE_X - 1 || y_index == 0 ||
		y_index == LV_VER_RES / SQUARE_Y - 1) {
	...
	if (touchpad->left_top || touchpad->right_bottom) {
		if (pos.y < (touchpad->line_k * pos.x + touchpad->line_b_up_1) ||
			pos.y > (touchpad->line_k * pos.x + touchpad->line_b_down_1)) {
	...
	if (touchpad->right_top || touchpad->left_bottom) {
		if (pos.y < (-touchpad->line_k * pos.x + touchpad->line_b_up_2) ||
			pos.y > (-touchpad->line_k * pos.x + touchpad->line_b_down_2)) {
...
void touchpad_read(lv_indev_t *indev, lv_indev_data_t *data)
{
	...
	nbytes = read(touchpad->fd, &sample, sizeof(struct touch_sample_s));
	...
	if (touch_flags & TOUCH_DOWN || touch_flags & TOUCH_MOVE) {
		...
		if (touchpad->is_square == true) {
			...
			if (touchpad->slider_v == NULL && pos.x - lv_slider_get_value(touchpad->slider_h) > 0 &&
				pos.x - lv_slider_get_value(touchpad->slider_h) < DISTANCE_PIX &&
				...
				lv_slider_set_value(touchpad->slider_h, pos.x, LV_ANIM_ON);
					   ...
					   LV_HOR_RES - lv_slider_get_value(touchpad->slider_h) < SQUARE_PIX) {
				if (touchpad->slider_v == NULL) {
					touchpad->slider_v = create_slider(false);
				}

				if (LV_VER_RES - pos.y - lv_slider_get_value(touchpad->slider_v) > 0 &&
					LV_VER_RES - pos.y - lv_slider_get_value(touchpad->slider_v) < DISTANCE_PIX &&
					abs(pos.x - LV_HOR_RES / 2 - DISTANCE_PIX) < SQUARE_PIX) {
					lv_slider_set_value(touchpad->slider_v, LV_VER_RES - pos.y, LV_ANIM_ON);
				}
			}

			if (touchpad->slider_v &&
				LV_VER_RES - lv_slider_get_value(touchpad->slider_v) < SQUARE_PIX) {
			...
			if (touchpad->arc_slider &&
				touchpad->diameter * 2 - lv_arc_get_value(touchpad->arc_slider) < DISTANCE_PIX) {
				if (touchpad->label_pass == NULL) {
					...
					lv_obj_set_size(touchpad->label_pass, LV_HOR_RES, LV_VER_RES);
					lv_label_set_text(touchpad->label_pass, "PASS");
					lv_obj_set_style_text_align(touchpad->label_pass, LV_TEXT_ALIGN_CENTER, 0);
...
bool touchpad_init(touchpad_s **touchpad, const char *input_path, uint32_t screen_shape)
{
	...
	*touchpad = tmp_touchpad;
	...
	if (tmp_touchpad->fd < 0) {
		LV_LOG_ERROR("touchpad %s open failed: %d", input_path, errno);
		return false;
	}
	...
	if (tmp_touchpad->is_square == true) {
		...
		lv_draw_buf_t *draw_buf =
			lv_draw_buf_create(LV_HOR_RES, LV_VER_RES, LV_COLOR_FORMAT_ARGB8888, LV_STRIDE_AUTO);
		if (draw_buf == NULL) {
		...
		lv_canvas_fill_bg(tmp_touchpad->canvas, lv_palette_lighten(LV_PALETTE_GREY, 3),
						  LV_OPA_COVER);
		...
		tmp_touchpad->diameter = LV_HOR_RES > LV_VER_RES ? LV_VER_RES : LV_HOR_RES;
		tmp_touchpad->diameter = tmp_touchpad->diameter * 3 / 4;
		tmp_touchpad->arc_indic_pos.x = (LV_HOR_RES - tmp_touchpad->diameter) / 2;
...
/****************************************************************************
 * Name: lv_roux_uv_loop
 ****************************************************************************/

#ifdef CONFIG_LV_USE_ROUX_LIBUV
static void lv_roux_uv_loop(uv_loop_t *loop, lv_roux_result_t *result)
{
	lv_roux_uv_t uv_info;
	void *data;
	...
	data = lv_roux_uv_init(&uv_info);
	uv_run(loop, UV_RUN_DEFAULT);
	lv_roux_uv_deinit(&data);
}
#endif
...
static void parse_commandline(uint32_t *screen_shape, int argc, FAR char **argv)
{
	int ch;
...
static void drivertest_touchpanel(FAR void **state)
{
	...
	lv_init();
	lv_roux_dsc_init(&info);
	lv_roux_init(&info, &result);
...
#ifdef CONFIG_LV_USE_ROUX_LIBUV
	lv_roux_uv_loop(&ui_loop, &result);
#else
	while (1) {
		lv_timer_handler();
		usleep(10 * 1000);
