...
static void show_usage(FAR const char *progname, FAR struct timer_state_s *timer_state,
					   int exitcode)
{
	printf("Usage: %s"
...
static void parse_commandline(FAR struct timer_state_s *timer_state, int argc, FAR char **argv)
{
	int ch;
	int converted;
	...
	while ((ch = getopt(argc, argv, "d:i:n:r:s:h")) != ERROR) {
		switch (ch) {
		case 'd':
			strlcpy(timer_state->devpath, optarg, sizeof(timer_state->devpath));
			break;
...
static void drivertest_timer(FAR void **state)
{
	...
	assert_in_range(timer_status.timeleft, 0, timer_state->interval);
...
{
	struct timer_state_s timer_state = {.devpath = TIMER_DEFAULT_DEVPATH,
	...
	const struct CMUnitTest tests[] = {cmocka_unit_test_prestate(drivertest_timer, &timer_state)};
