...
static void show_usage(FAR const char *progname, FAR const char *path, int exitcode)
{
...
static void drivertest_i2c_spi(FAR void **state)
{
	...
	for (times = 0; times < READ_TIMES; times++) {
		...
		printf("[%" PRIu32 "] %d, %d, %d / %d, %d, %d\n", data.sensor_time, data.gyro.x,
			   data.gyro.y, data.gyro.z, data.accel.x, data.accel.y, data.accel.z);
		fflush(stdout);
	}
}
...
int main(int argc, FAR char *argv[])
{
	...
	snprintf(test_state.dev_path, sizeof(test_state.dev_path), "%s", ACC_DEVPATH);
	while ((ch = getopt(argc, argv, "d:h")) != ERROR) {
		switch (ch) {
		case 'd':
			snprintf(test_state.dev_path, sizeof(test_state.dev_path), "%s", optarg);
			break;
		case 'h':
		case '?':
	...
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_prestate_setup_teardown(drivertest_i2c_spi, setup, teardown, &test_state),
	};
