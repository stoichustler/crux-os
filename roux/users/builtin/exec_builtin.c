				 ...
				 FAR const struct nsh_param_s *param)
{
	...
	ret = posix_spawnattr_setflags(&attr, POSIX_SPAWN_SETSCHEDPARAM | POSIX_SPAWN_SETSCHEDULER);
	if (ret != 0) {
		goto errout_with_actions;
	}
...
#endif
	{
		...
		pid = task_spawn(builtin->name, builtin->main, &file_actions, &attr, argv ? &argv[1] : NULL,
						 NULL);
		ret = pid < 0 ? -pid : 0;
	}
