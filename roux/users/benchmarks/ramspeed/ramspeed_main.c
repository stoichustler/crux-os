...
#define HAS_IRQ_CONTROL !defined(CONFIG_BUILD_KERNEL) && !defined(CONFIG_BUILD_PROTECTED)
...
static void parse_commandline(int argc, FAR char **argv, FAR struct ramspeed_s *info)
{
	int ch;
...
static void print_rate(FAR const char *name, uint64_t bytes, uint32_t cost_time)
{
	double rate;
	if (cost_time == 0) {
		printf(RAMSPEED_PREFIX "Time-consuming is too short,"
							   " please increase the <repeat number>\n");
		return;
	}

	rate = (double)bytes / 1024 / (cost_time / 1000000.0);
	printf(RAMSPEED_PREFIX "%s Rate = %.3f KB/s\t[cost: %.3f ms]\n", name, rate,
		   cost_time / 1000.0f);
}
...
static void memcpy_speed_test(FAR void *dest, FAR const void *src, size_t size, uint32_t repeat_cnt,
							  bool irq_disable)
{
	uint32_t start_time;
...
static void memset_speed_test(FAR void *dest, uint8_t value, size_t size, uint32_t repeat_num,
							  bool irq_disable)
{
	uint32_t start_time;
	uint32_t cost_time_system;
...
int main(int argc, FAR char *argv[])
{
	...
	if (ramspeed.src != NULL) {
		memcpy_speed_test(ramspeed.dest, ramspeed.src, ramspeed.size, ramspeed.repeat_num,
						  ramspeed.irq_disable);
	}

	memset_speed_test(ramspeed.dest, ramspeed.value, ramspeed.size, ramspeed.repeat_num,
					  ramspeed.irq_disable);
