...
static int performance_thread_create(FAR void *(*entry)(FAR void *), FAR void *arg, int priority)
{
...
static size_t pthread_switch_performance(void)
{
	...
	tid =
		performance_thread_create(pthread_switch_task, &perf, CONFIG_BENCHMARK_OSPERF_PRIORITY + 1);
...
static size_t context_switch_performance(void)
{
	...
	tid = performance_thread_create(context_switch_task, &time, CONFIG_INIT_PRIORITY);
	sched_yield();
...
static size_t hpwork_performance(void)
{
	...
	FAR void *args = (void *[]){(FAR void *)&work, (FAR void *)&sem, (FAR void *)&result};
...
static void performance_run(const FAR struct performance_entry_s *item, size_t count, bool detail)
{
	size_t total = 0;
	size_t max = 0;
	size_t min = 0;
	...
	printf("%-*s %10zu %10zu %10zu\n", NAME_MAX, item->name, max, min, total / count);
}
...
static const FAR struct performance_entry_s *find_entry(FAR const char *name)
{
	size_t i;
