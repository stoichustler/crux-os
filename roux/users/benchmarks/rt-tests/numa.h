...
static inline unsigned int numa_bitmask_isbitset(const struct bitmask *mask, unsigned int idx)
{
	return mask->maskp[idx / sizeof(unsigned long)] &
		   ((unsigned long)1 << (idx % sizeof(unsigned long)));
}

static inline struct bitmask *numa_bitmask_clearbit(struct bitmask *mask, unsigned int idx)
{
...
static inline unsigned int numa_bitmask_weight(const struct bitmask *bitmap)
{
	unsigned int idx;
	unsigned int ret = 0;
