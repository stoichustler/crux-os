 ...
 * under the License.
 *
 * A ROUX port of the cyclictest rt-tests utility for Linux.
 ...
 *   - ROUX Timer API: waiting for the timer to expire.
 ...
 *  - ROUX Timer API
 *
 * Authors of the ROUX port: Stepan Pressl <pressl.stepan@gmail.com>
...
static const struct option optargs[] = {
	{"clock", optional_argument, 0, 'c'},		{"distance", optional_argument, 0, 'd'},
	{"duration", optional_argument, 0, 'D'},	{"help", optional_argument, 0, 'e'},
	{"histogram", optional_argument, 0, 'h'},	{"histofall", optional_argument, 0, 'H'},
	{"interval", optional_argument, 0, 'i'},	{"loops", optional_argument, 0, 'l'},
	{"measurement", optional_argument, 0, 'm'}, {"nanosleep", optional_argument, 0, 'n'},
	{"prio", optional_argument, 0, 'p'},		{"quiet", optional_argument, 0, 'q'},
	{"threads", optional_argument, 0, 't'},		{"timer-device", optional_argument, 0, 'T'},
...
{
	puts("The Cyclictest Benchmark Utility\n"
		 "Usage:\n"
		 ...
		 "     0 selects clock_gettime, 1 uses the ROUX timer API.\n"
		 "     WARNING:\n"
...
static bool parse_args(int argc, char *const argv[])
{
	...
	int opt;
	long decimal;
	while ((opt = getopt_long(argc, argv, "c:d:D:h:Hi:l:m:n:p:qt:T:", optargs, &longindex)) != -1) {
		switch (opt) {
		case 'c':
...
static inline int64_t timediff_us_timer(struct timer_status_s after, struct timer_status_s before)
{
	int64_t ret = 0;
	uint32_t t1;
	uint32_t t2;
...
static inline void init_thread_param(struct thread_param_s *param, unsigned long interval,
									 unsigned long max_cycles, int policy, int prio,
									 struct thread_stats_s *stats, int clock)
{
	stats->avg = 0.0;
	stats->cycles = 0;
	stats->max = -LONG_MAX;
...
static void print_hist(struct thread_param_s *par[], int nthreads)
{
	...
	for (j = 0; j < nthreads; j++) {
		printf(" %05lu",
			   par[j]->stats->cycles ? (long)(par[j]->stats->avg / par[j]->stats->cycles) : 0);
	}
...
int main(int argc, char *argv[])
{
	...
	if (config.wait_method == W_DEVTIMER || config.meas_method == M_TIMER_API) {
		...
		if (config.wait_method == W_DEVTIMER) {
			...
			reqtimeout_timer = 3 * (config.interval + (config.threads - 1) * config.distance);
		}
	...
	for (i = 0; i < config.threads; ++i) {
		...
		init_thread_param(params[i], config.interval, config.loops, config.policy, config.prio,
						  stats[i], config.clock);
