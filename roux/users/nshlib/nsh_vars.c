...
FAR char *nsh_findvar(FAR struct console_stdio_s *pstate, FAR const char *name)
{
	FAR char *ptr;
	FAR char *end;
	...
	for (ptr = pstate->varp; ptr < end && !nsh_cmpname(name, ptr); ptr += (strlen(ptr) + 1))
		;
...
int nsh_setvar(FAR struct nsh_vtbl_s *vtbl, FAR const char *name, FAR const char *value)
{
	...
	if (pstate->varp != NULL && (pair = nsh_findvar(pstate, name)) != NULL) {
