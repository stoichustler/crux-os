					...
					ssize_t nbytes)
{
	...
	for (i = 0; i < nbytes; i += 16) {
		...
		for (j = 0; j < 16; j++) {
			if (i + j < nbytes) {
				snprintf(&line[size], sizeof(line) - size, "%02x ", buffer[i + j]);
			} else {
		...
		for (j = 0; j < 16; j++) {
			if (i + j < nbytes) {
				ch = buffer[i + j];
				snprintf(&line[size], sizeof(line) - size, "%c",
						 ch >= 0x20 && ch <= 0x7e ? ch : '.');
				size += strlen(&line[size]);
			}
		}
...
int cmd_hexdump(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	for (;;) {
		...
		if (nbytesread < 0) {
			nsh_error(vtbl, g_fmtcmdfailed, "hexdump", "read", NSH_ERRNO_OF(errno));
			ret = ERROR;
			break;
			...
			if (position < skip) {
				...
				if (position > skip) {
					...
					snprintf(msg, sizeof(msg), "%s at %08jx", argv[1], (uintmax_t)skip);
					nsh_dumpbuffer(vtbl, msg, &buffer[nbytesread - (position - skip)], dumpbytes);
			...
			snprintf(msg, sizeof(msg), "%s at %08jx", argv[1], (uintmax_t)position);
