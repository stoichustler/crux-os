...
int nsh_session(FAR struct console_stdio_s *pstate, int login, int argc, FAR char *argv[])
{
	...
	for (;;) {
		...
		ret = cle_fd(pstate->cn_line, nsh_prompt(), LINE_MAX, INFD(pstate), OUTFD(pstate));
		if (ret < 0) {
			dprintf(ERRFD(pstate), g_fmtcmdfailed, "nsh_session", "cle", NSH_ERRNO_OF(-ret));
			continue;
		}
#else
		...
		if (ret == EOF) {
			...
			dprintf(ERRFD(pstate), g_fmtcmdfailed, "nsh_session", "readline", NSH_ERRNO);
			ret = EXIT_SUCCESS;
			break;
		}
#endif
