...
static int cmd_unrecognized(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv);

static int cmd_clear(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv);

static int cmd_banner(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv);
...
static const struct cmdmap_s g_cmdmap[] = {
	...
	CMD_MAP("[", cmd_lbracket, 4, CONFIG_NSH_MAXARGUMENTS, "<expression> ]"),
#endif
	...
	CMD_MAP("alias", cmd_alias, 1, CONFIG_NSH_MAXARGUMENTS, "[name[=value] ... ]"),
	CMD_MAP("unalias", cmd_unalias, 1, CONFIG_NSH_MAXARGUMENTS, "[-a] name [name ... ]"),
#endif

#if defined(CONFIG_NET) && defined(CONFIG_NET_ARP) && !defined(CONFIG_NSH_DISABLE_ARP)
	CMD_MAP("arp", cmd_arp, 1, 6, "[-i <ifname>] [-a <ipaddr>|-d <ipaddr>|-s <ipaddr> <hwaddr>]"),
#endif

#if defined(CONFIG_NETUTILS_CODECS) && defined(CONFIG_CODECS_BASE64)
#ifndef CONFIG_NSH_DISABLE_BASE64DEC
	CMD_MAP("base64dec", cmd_base64decode, 2, 4, "[-w] [-f] <string or filepath>"),
#endif
#ifndef CONFIG_NSH_DISABLE_BASE64ENC
	CMD_MAP("base64enc", cmd_base64encode, 2, 4, "[-w] [-f] <string or filepath>"),
#endif
#endif
	...
	CMD_MAP("cat", cmd_cat, 1, CONFIG_NSH_MAXARGUMENTS, "[<path> [<path> [<path> ...]]]"),
#endif
	...
	CMD_MAP("clear", cmd_clear, 1, 1, NULL),

	CMD_MAP("banner", cmd_banner, 1, 1, NULL),
	...
	CMD_MAP("date", cmd_date, 1, 4, "[-s \"MMM DD HH:MM:SS YYYY\"] [-u] [+format]"),
#endif
...
#else
	CMD_MAP("echo", cmd_echo, 1, CONFIG_NSH_MAXARGUMENTS, "[-n] [<string> [<string>...]]"),
#endif
#endif
...
#ifndef CONFIG_NSH_DISABLE_EXPR
	CMD_MAP("expr", cmd_expr, 4, 4, "<operand1> <operator> <operand2>"),
#endif
	...
	CMD_MAP("get", cmd_get, 4, 7, "[-b|-n] [-f <local-path>] -h <ip-address> <remote-path>"),
#endif
#endif
...
#else
	CMD_MAP("hexdump", cmd_hexdump, 2, 4, "<file or device> [skip=<bytes>] [count=<bytes>]"),
#endif
#endif
	...
	CMD_MAP("mb", cmd_mb, 2, 3, "<hex-address>[=<hex-value>] [<hex-byte-count>]"),
#endif

#if defined(CONFIG_NETUTILS_CODECS) && defined(CONFIG_CODECS_HASH_MD5)
#ifndef CONFIG_NSH_DISABLE_MD5
	CMD_MAP("md5", cmd_md5, 1, 3, "[string] or [-f <filepath>] or read stdin"),
#endif
#endif
	...
	CMD_MAP("mkfatfs", cmd_mkfatfs, 2, 6, "[-F <fatsize>] [-r <rootdirentries>] <block-driver>"),
#endif
#endif

#ifndef CONFIG_DISABLE_MOUNTPOINT
#if defined(CONFIG_PIPES) && CONFIG_DEV_FIFO_SIZE > 0 && !defined(CONFIG_NSH_DISABLE_MKFIFO)
	CMD_MAP("mkfifo", cmd_mkfifo, 2, 2, "<path>"),
#endif
#endif

#ifndef CONFIG_NSH_DISABLE_MKRD
	CMD_MAP("mkrd", cmd_mkrd, 2, 6, "[-m <minor>] [-s <sector-size>] <nsectors>"),
#endif
...
#else
	CMD_MAP("mksmartfs", cmd_mksmartfs, 2, 5, "[-s <sector-size>] [-f] <path>"),
#endif
#endif
#endif

#ifndef CONFIG_NSH_DISABLE_MH
	CMD_MAP("mh", cmd_mh, 2, 3, "<hex-address>[=<hex-value>] [<hex-byte-count>]"),
#endif
...
#else
	CMD_MAP("mount", cmd_mount, 4, 7, "-t <fstype> [-o <options>] [<block-device>] <mount-point>"),
#endif
#endif
#endif
	...
	CMD_MAP("mw", cmd_mw, 2, 3, "<hex-address>[=<hex-value>] [<hex-byte-count>]"),
#endif

#if !defined(CONFIG_DISABLE_MOUNTPOINT) && defined(CONFIG_NET) && defined(CONFIG_NFS)
#ifndef CONFIG_NSH_DISABLE_NFSMOUNT
	CMD_MAP("nfsmount", cmd_nfsmount, 4, 5, "<server-address> <mount-point> <remote-path> [udp]"),
#endif
#endif
	...
	CMD_MAP("pmconfig", cmd_pmconfig, 1, 4, "[stay|relax] [normal|idle|standby|sleep] [domain]"),
#endif
	...
	CMD_MAP("ps", cmd_ps, 1, CONFIG_NSH_MAXARGUMENTS, "<-heap> <pid1 pid2 ...>"),
#endif

#ifdef CONFIG_NET_UDP
#ifndef CONFIG_NSH_DISABLE_PUT
	CMD_MAP("put", cmd_put, 4, 7, "[-b|-n] [-f <remote-path>] -h <ip-address> <local-path>"),
#endif
#endif
	...
	CMD_MAP("irqaff", cmd_irq_affinity, 3, 3, "irqaff [IRQ Number] [Core Mask]"),
#endif
	...
	CMD_MAP("set", cmd_set, 1, 4, "[{+|-}{e|x|xe|ex}] [<name> <value>]"),
...
#else
#if !defined(CONFIG_DISABLE_ENVIRON) && !defined(CONFIG_NSH_DISABLESCRIPT)
	CMD_MAP("set", cmd_set, 2, 4, "[{+|-}{e|x|xe|ex}] [<name> <value>]"),
#elif !defined(CONFIG_DISABLE_ENVIRON) && defined(CONFIG_NSH_DISABLESCRIPT)
...
#if !defined(CONFIG_NSH_DISABLESCRIPT) && !defined(CONFIG_NSH_DISABLE_TEST)
	CMD_MAP("test", cmd_test, 3, CONFIG_NSH_MAXARGUMENTS, "<expression>"),
#endif

#if !defined(CONFIG_NSH_DISABLE_TOP) && defined(NSH_HAVE_CPULOAD)
	CMD_MAP("top", cmd_top, 1, 5, "[ -n <num> ][ -d <delay>] [ -p <pidlist>] [-h]"),
#endif
	...
	CMD_MAP("watch", cmd_watch, 2, 6, "[-n] interval [-c] count <command>"),
#endif
	...
	CMD_MAP("wait", cmd_wait, 1, CONFIG_NSH_MAXARGUMENTS, "pid1 [pid2 [pid3] ...]"),
#endif
...
static inline void help_cmdlist(FAR struct nsh_vtbl_s *vtbl)
{
	...
	for (i = 0; i < ncmdrows; i++) {
		...
		for (j = 0, k = i; j < cmdsperline && k < NUM_CMDS; j++, k += ncmdrows) {
			/* Copy the cmd name to line buffer */

			offset += strlcpy(line + offset, g_cmdmap[k].cmd, sizeof(line) - offset);

			/* Add space between commands */

			for (cmdwidth = strlen(g_cmdmap[k].cmd); cmdwidth < colwidth; cmdwidth++) {
				line[offset++] = ' ';
			}
		}
...
#ifndef CONFIG_NSH_DISABLE_HELP
static void help_showcmd(FAR struct nsh_vtbl_s *vtbl, FAR const struct cmdmap_s *cmdmap)
{
	if (cmdmap->usage) {
...
static inline void help_builtins(FAR struct nsh_vtbl_s *vtbl)
{
	...
	num_builtin_rows = ((num_builtins + (builtins_per_line - 1)) / builtins_per_line);
	...
	for (i = 0; i < num_builtin_rows; i++) {
		...
		for (j = 0, k = i; j < builtins_per_line && (builtin = builtin_for_index(k));
			 j++, k += num_builtin_rows) {
			if (builtin->main == NULL) {
				continue;
			}

			offset += strlcpy(line + offset, builtin->name, sizeof(line) - offset);

			for (builtin_width = strlen(builtin->name); builtin_width < column_width;
				 builtin_width++) {
				line[offset++] = ' ';
			}
		}
...
static int cmd_clear(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	nsh_output(vtbl, "\x1b[3J\x1b[2J\x1b[H");
