				...
				FAR const struct nsh_param_s *param)
{
	...
	if (ret != 0) {
		...
		nsh_error(vtbl, g_fmtcmdfailed, cmd, "posix_spawn_file_actions_init", NSH_ERRNO_OF(ret));
		goto errout;
	}
	...
	if (ret != 0) {
		...
		nsh_error(vtbl, g_fmtcmdfailed, cmd, "posix_spawnattr_init", NSH_ERRNO);
	...
	if (param) {
		...
		if (param->file_in) {
			...
			ret = posix_spawn_file_actions_addopen(&file_actions, 0, param->file_in,
												   param->oflags_in, 0);
			if (ret != 0) {
				nsh_error(vtbl, g_fmtcmdfailed, cmd, "posix_spawn_file_actions_addopen", NSH_ERRNO);
				goto errout_with_actions;
			}
		}
#ifdef CONFIG_NSH_PIPELINE
		else if (param->fd_in != -1) {
			ret = posix_spawn_file_actions_adddup2(&file_actions, param->fd_in, 0);
			if (ret != 0) {
				nsh_error(vtbl, g_fmtcmdfailed, cmd, "posix_spawn_file_actions_adddup2", NSH_ERRNO);
				goto errout_with_actions;
			}
		}
#endif
		...
		if (param->file_out) {
			ret = posix_spawn_file_actions_addopen(&file_actions, 1, param->file_out,
			...
			ret = posix_spawn_file_actions_adddup2(&file_actions, param->fd_out, 1);
			if (ret != 0) {
				nsh_error(vtbl, g_fmtcmdfailed, cmd, "posix_spawn_file_actions_adddup2", NSH_ERRNO);
				goto errout_with_actions;
			}
	...
	if (ret == OK) {
...
#endif /* CONFIG_NSH_DISABLEBG */
		{
			...
			if (ret < 0) {
				 ...
				 * (because ROUX does not retain exit status of child tasks).
				...
				if (errcode == ECHILD) {
					...
					nsh_error(vtbl, g_fmtcmdfailed, cmd, "waitpid", NSH_ERRNO_OF(errcode));
				}
			}
