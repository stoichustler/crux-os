...
int cmd_df(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	{
		return nsh_catfile(vtbl, argv[0], CONFIG_NSH_PROC_MOUNTPOINT "/fs/usage");
	}
#endif
#if defined(HAVE_DF_HUMANREADBLE) && defined(HAVE_DF_BLOCKOUTPUT)
	else
	...
	{
		return nsh_catfile(vtbl, argv[0], CONFIG_NSH_PROC_MOUNTPOINT "/fs/blocks");
	}
#endif
}
#endif
...
int cmd_mount(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (argc < 2) {
		return nsh_catfile(vtbl, argv[0], CONFIG_NSH_PROC_MOUNTPOINT "/fs/mount");
	}
#endif
 ...
 ****************************************************************************/
	...
	!defined(CONFIG_NSH_DISABLE_NFSMOUNT)
