					  ...
					  FAR const char *destpath)
{
	...
	if (stat(destpath, &buf) == 0) {
		...
		if (S_ISDIR(buf.st_mode)) {
			...
			allocpath = nsh_getdirpath(vtbl, destpath, basename((FAR char *)srcpath));
			if (!allocpath) {
	...
	for (;;) {
		...
		if (nbytesread == 0) {
			...
			if (errno == EINTR) {
				...
				nsh_error(vtbl, g_fmtcmdfailed, "cp", "read", NSH_ERRNO);
			}
		...
		do {
			...
			if (nbyteswritten >= 0) {
				...
				if (errno == EINTR) {
					...
					nsh_error(vtbl, g_fmtcmdfailed, "cp", "write", NSH_ERRNO);
				}
						...
						FAR const char *destpath)
{
	...
	while ((entry = readdir(dp)) != NULL && ret == OK) {
		...
		if (S_ISDIR(buf.st_mode)) {
			...
			ret = mkdir(allocdestpath, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
			if (ret != OK) {
...
static int fdinfo_callback(FAR struct nsh_vtbl_s *vtbl, FAR const char *dirpath,
						   FAR struct dirent *entryp, FAR void *pvarg)
{
	FAR char *filepath;
	int ret;
...
int cmd_lomtd(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (teardown) {
		...
		ret = ioctl(fd, MTD_LOOPIOC_TEARDOWN, (unsigned long)((uintptr_t)loopdev));
		if (ret < 0) {
			nsh_error(vtbl, g_fmtcmdfailed, argv[0], "ioctl", NSH_ERRNO);
			goto errout_with_fd;
		}
		...
		ret = ioctl(fd, MTD_LOOPIOC_SETUP, (unsigned long)((uintptr_t)&setup));
		if (ret < 0) {
			nsh_error(vtbl, g_fmtcmdfailed, argv[0], "ioctl", NSH_ERRNO);
			goto errout_with_fd;
		}
	...
	if (fullpath != NULL) {
		...
		for (;;) {
			...
			if (ret < 0 && (errno != EEXIST || !parent)) {
				nsh_error(vtbl, g_fmtcmdfailed, fullpath, "mkdir", NSH_ERRNO);
				break;
			}
...
int cmd_mkfatfs(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	while ((option = getopt(argc, argv, ":F:r:")) != ERROR) {
		...
		case 'F':
			...
			if (fmt.ff_fattype != 0 && fmt.ff_fattype != 12 && fmt.ff_fattype != 16 &&
				fmt.ff_fattype != 32) {
...
int cmd_mkrd(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (ret < 0) {
		nsh_error(vtbl, g_fmtcmdfailed, argv[0], "boardctl(BOARDIOC_MKRD)", NSH_ERRNO_OF(-ret));
		return ERROR;
	}
		...
		if (errval == ENOENT) {
			...
			if (fd < 0) {
				...
				if (length > 0) {
					...
					if (ret < 0) {
						nsh_error(vtbl, g_fmtcmdfailed, argv[0], "ftruncate", NSH_ERRNO);
					}
				}
			...
			nsh_error(vtbl, g_fmtcmdfailed, argv[0], "stat", NSH_ERRNO_OF(errval));
			ret = ERROR;
		}
	} else if (!S_ISREG(buf.st_mode)) {
		nsh_error(vtbl, g_fmtarginvalid, argv[0]);
...
int cmd_fdinfo(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (argv[1] != NULL) {
		...
		if (!isdigit(argv[1][0])) {
			nsh_error(vtbl, g_fmtcmdfailed, "fdinfo", "not process id", NSH_ERRNO);
			return ERROR;
		}

		ret = asprintf(&fdpath, "%s/%s/group/fd", CONFIG_NSH_PROC_MOUNTPOINT, argv[1]);
		if (ret < 0) {
			return ret;
		}
	...
	return nsh_foreach_direntry(vtbl, "fdinfo", CONFIG_NSH_PROC_MOUNTPOINT, fdinfo_callback, NULL);
}
#endif
