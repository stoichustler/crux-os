...
#undef HAVE_MEMLIST
#if defined(CONFIG_NSH_CMDPARMS) || defined(CONFIG_NSH_ALIAS) || defined(CONFIG_NSH_ARGCAT)
...
#ifdef HAVE_MEMLIST
static void nsh_memlist_add(FAR struct nsh_memlist_s *memlist, FAR char *allocation);
static void nsh_memlist_free(FAR struct nsh_memlist_s *memlist);
#endif

#ifdef CONFIG_NSH_ALIAS
static void nsh_alist_add(FAR struct nsh_alist_s *alist, FAR struct nsh_alias_s *alias);
static void nsh_alist_free(FAR struct nsh_vtbl_s *vtbl, FAR struct nsh_alist_s *alist);
#endif

static int nsh_saveresult(FAR struct nsh_vtbl_s *vtbl, bool result);
static int nsh_execute(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char *argv[],
					   FAR const struct nsh_param_s *param);

#ifdef CONFIG_NSH_CMDPARMS
static FAR char *nsh_filecat(FAR struct nsh_vtbl_s *vtbl, FAR char *s1, FAR const char *filename);
static FAR char *nsh_cmdparm(FAR struct nsh_vtbl_s *vtbl, FAR char *cmdline, FAR char **allocation);
#endif

#if defined(CONFIG_NSH_ARGCAT) || defined(CONFIG_NSH_ALIAS)
static FAR char *nsh_strcat(FAR struct nsh_vtbl_s *vtbl, FAR char *s1, FAR const char *s2);
#endif
...
#ifdef CONFIG_NSH_ALIAS
static FAR char *nsh_aliasexpand(FAR struct nsh_vtbl_s *vtbl, FAR char *cmdline, FAR char **saveptr,
								 FAR NSH_MEMLIST_TYPE *memlist, FAR NSH_ALIASLIST_TYPE *alist);
#endif
...
#ifndef CONFIG_NSH_DISABLE_LOOPS
static int nsh_loop(FAR struct nsh_vtbl_s *vtbl, FAR char **ppcmd, FAR char **saveptr,
					FAR NSH_MEMLIST_TYPE *memlist, FAR NSH_ALIASLIST_TYPE *alist);
#endif
#ifndef CONFIG_NSH_DISABLE_ITEF
static int nsh_itef(FAR struct nsh_vtbl_s *vtbl, FAR char **ppcmd, FAR char **saveptr,
					FAR NSH_MEMLIST_TYPE *memlist, FAR NSH_ALIASLIST_TYPE *alist);
#endif
#endif

#ifndef CONFIG_NSH_DISABLEBG
static int nsh_nice(FAR struct nsh_vtbl_s *vtbl, FAR char **ppcmd, FAR char **saveptr,
					FAR NSH_MEMLIST_TYPE *memlist, FAR NSH_ALIASLIST_TYPE *alist);
#endif
...
/* If ROUX versioning information is available, Include that information
 * in the NSH greeting.
 */

#if CONFIG_VERSION_MAJOR != 0 || CONFIG_VERSION_MINOR != 0
const char g_nshgreeting[] = "\n[<console>] roux-" CONFIG_VERSION_STRING "\n";
#else
const char g_nshgreeting[] = "\n[<console>]\n";
#endif
...
const char g_fmtsyntax[] = "note: %s: syntax error\n";
const char g_fmtargrequired[] = "note: %s: missing required argument(s)\n";
const char g_fmtnomatching[] = "note: %s: no matching %s\n";
const char g_fmtarginvalid[] = "note: %s: argument invalid\n";
const char g_fmtargrange[] = "note: %s: value out of range\n";
const char g_fmtcmdnotfound[] = "note: %s: command not found\n";
const char g_fmtnosuch[] = "note: %s: no such %s: %s\n";
const char g_fmttoomanyargs[] = "note: %s: too many arguments\n";
const char g_fmtdeepnesting[] = "note: %s: nesting too deep\n";
const char g_fmtcontext[] = "note: %s: not valid in this context\n";
#ifdef CONFIG_NSH_STRERROR
const char g_fmtcmdfailed[] = "note: %s: %s failed: %s\n";
#else
const char g_fmtcmdfailed[] = "note: %s: %s failed: %d\n";
#endif
const char g_fmtcmdoutofmemory[] = "note: %s: out of memory\n";
const char g_fmtinternalerror[] = "note: %s: Internal error\n";
const char g_fmtsignalrecvd[] = "note: %s: Interrupted by signal\n";
...
static FAR char *nsh_filecat(FAR struct nsh_vtbl_s *vtbl, FAR char *s1, FAR const char *filename)
{
	...
	for (; index > s1size && strchr(g_token_separator, argument[index - 1]) != NULL; index--)
		;
...
static FAR char *nsh_cmdparm(FAR struct nsh_vtbl_s *vtbl, FAR char *cmdline, FAR char **allocation)
{
	struct nsh_param_s param = {.fd_in = -1,
								.fd_out = -1,
								.oflags_in = 0,
		...
		if (whilematch || untilmatch) {
			...
			*ppcmd = nsh_argument(vtbl, saveptr, memlist, alist, 0);
			...
			if (
				...
				np->np_lpstate[np->np_lpndx].lp_state == NSH_LOOP_UNTIL || np->np_fd < 0 ||
				np->np_foffs < 0) {
		...
		/* Check if the token is "done" */
			...
			*ppcmd = nsh_argument(vtbl, saveptr, memlist, alist, NULL);
			...
			if (np->np_lpstate[np->np_lpndx].lp_enable) {
				...
				ret = lseek(np->np_fd, np->np_lpstate[np->np_lpndx].lp_topoffs, SEEK_SET);
				if (ret < 0) {
					nsh_error(vtbl, g_fmtcmdfailed, "done", "lseek", NSH_ERRNO);
				}
	...
	if (cmd) {
		...
		if (strcmp(cmd, "nice") == 0) {
			...
			if (cmd && strcmp(cmd, "-d") == 0) {
				FAR char *val = nsh_argument(vtbl, saveptr, memlist, alist, NULL);
				if (val) {
					FAR char *endptr;
					vtbl->np.np_nice = (int)strtol(val, &endptr, 0);
					if (vtbl->np.np_nice > 19 || vtbl->np.np_nice < -20 || endptr == val ||
						*endptr != '\0') {
