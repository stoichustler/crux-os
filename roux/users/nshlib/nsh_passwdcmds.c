...
int cmd_useradd(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (ret < 0) {
		nsh_error(vtbl, g_fmtcmdfailed, argv[0], "passwd_adduser", NSH_ERRNO_OF(-ret));
		return ERROR;
	}
...
int cmd_userdel(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (ret < 0) {
		nsh_error(vtbl, g_fmtcmdfailed, argv[0], "passwd_deluser", NSH_ERRNO_OF(-ret));
		return ERROR;
	}
...
int cmd_passwd(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (ret < 0) {
		nsh_error(vtbl, g_fmtcmdfailed, argv[0], "passwd_update", NSH_ERRNO_OF(-ret));
		return ERROR;
	}
