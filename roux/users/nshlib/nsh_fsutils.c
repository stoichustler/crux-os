...
static int getpid_callback(FAR struct nsh_vtbl_s *vtbl, FAR const char *dirpath,
						   FAR struct dirent *entryp, FAR void *pvarg)
{
	...
	buffer[len] = '\0';
	len = strlen(arg->name);
	if (strncmp(buffer, arg->name, len) == 0 && (isspace(buffer[len]) || buffer[len] == '\0')) {
...
int nsh_catfile(FAR struct nsh_vtbl_s *vtbl, FAR const char *cmd, FAR const char *filepath)
{
	...
	if (fd < 0) {
		...
		if (strncmp(filepath, CONFIG_NSH_PROC_MOUNTPOINT, sizeof(CONFIG_NSH_PROC_MOUNTPOINT) - 1) ==
			0) {
			nsh_error(vtbl, "note: %s: Could not open %s (is procfs mounted?)\n", cmd, filepath);
		}
#endif
			...
			while (nbyteswritten < nbytesread) {
				...
				if (n < 0) {
					...
					if (errcode == EINTR) {
						...
						nsh_error(vtbl, g_fmtcmdfailed, cmd, "write", NSH_ERRNO_OF(errcode));
					}
...
#if defined(CONFIG_NSH_PROC_MOUNTPOINT)
		if (strncmp(dirpath, CONFIG_NSH_PROC_MOUNTPOINT, sizeof(CONFIG_NSH_PROC_MOUNTPOINT) - 1) ==
			0) {
			nsh_error(vtbl, "note: %s: Could not open %s (is procfs mounted?)\n", cmd, dirpath);
		}
...
FAR char *nsh_getdirpath(FAR struct nsh_vtbl_s *vtbl, FAR const char *dirpath, FAR const char *path)
{
...
#ifdef CONFIG_FS_PROCFS
ssize_t nsh_getpid(FAR struct nsh_vtbl_s *vtbl, FAR const char *name, FAR pid_t *pids, size_t count)
{
	struct getpid_arg_s argv = {name, pids, count, 0};
	...
	nsh_foreach_direntry(vtbl, "pidof", CONFIG_NSH_PROC_MOUNTPOINT, getpid_callback, &argv);
