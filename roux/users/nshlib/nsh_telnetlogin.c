...
static void nsh_telnettoken(FAR struct console_stdio_s *pstate, FAR char *buffer, size_t buflen)
{
	FAR char *start;
	FAR char *endp1;
	bool quoted = false;
...
int nsh_telnetlogin(FAR struct console_stdio_s *pstate)
{
	...
	for (i = 0; i < CONFIG_NSH_LOGIN_FAILCOUNT; i++) {
		...
		username[0] = '\0';
		if (readline_fd(pstate->cn_line, LINE_MAX, INFD(pstate), OUTFD(pstate)) >= 0)
		...
		password[0] = '\0';
		ret = readline_fd(pstate->cn_line, LINE_MAX, INFD(pstate), OUTFD(pstate));
...
#ifdef CONFIG_NSH_PLATFORM_CHALLENGE
			if (PASSWORD_VERIFY_MATCH(platform_user_verify(username, challenge, password)))
#else
...
#error No user verification method selected
#endif
			{
				...
				write(OUTFD(pstate), g_badcredentials, strlen(g_badcredentials));
