...
int cmd_addroute(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	ret = addroute(sockfd, &target.ipx, &netmask.ipx, &router.ipx, sizeof(router));
	if (ret < 0) {
...
#endif
	{
		return nsh_catfile(vtbl, argv[0], CONFIG_NSH_PROC_MOUNTPOINT "/net/route/ipv4");
	}
#endif
...
#endif
	{
		return nsh_catfile(vtbl, argv[0], CONFIG_NSH_PROC_MOUNTPOINT "/net/route/ipv6");
	}
#endif
