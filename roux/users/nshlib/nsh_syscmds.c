...
#else
#define UNAME_ALL (UNAME_KERNEL | UNAME_RELEASE | UNAME_VERSION | UNAME_MACHINE | UNAME_PLATFORM)
#endif
...
/* Keep update with roux kernel definition */

static FAR const char *const g_resetcause[] = {"none",
											   "power_on",
											   "rtc_watchdog",
											   "brown_out",
											   "core_soft_reset",
...
static FAR const char *const g_resetflag[] = {"reboot",	  "assert",	 "panic",	"bootloader",
											  "recovery", "restore", "factory", NULL};
#endif
...
typedef CODE int (*cmd_rpmsg_cb_t)(FAR int *cmd, FAR unsigned long *val, FAR char **argv);
#endif
...
static int cmd_pmconfig_recursive(FAR struct nsh_vtbl_s *vtbl, FAR const char *dirpath,
...
int cmd_pmconfig(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (argc <= 2) {
		...
		return nsh_foreach_direntry(vtbl, argv[0], CONFIG_NSH_PROC_MOUNTPOINT "/pm",
...
#if defined(CONFIG_RPMSG) && !defined(CONFIG_NSH_DISABLE_RPMSG)
static int cmd_rpmsg_once(FAR struct nsh_vtbl_s *vtbl, FAR const char *path, FAR char **argv,
						  cmd_rpmsg_cb_t rpmsg_cb)
{
	...
	if (strcmp(argv[1], "panic") == 0) {
		...
		if (argv[3] == 0 || argv[4] == 0 || argv[5] == 0 || argv[6] == 0) {
...
static int cmd_rpmsg_recursive(FAR struct nsh_vtbl_s *vtbl, FAR const char *dirpath,
							   FAR struct dirent *entryp, FAR void *pvarg)
{
	FAR char *path;
	int ret = ERROR;
...
static int cmd_rpmsg_help(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	nsh_output(vtbl, "%s <panic|dump> <path>\n", argv[0]);
#ifdef CONFIG_RPMSG_PING
	nsh_output(vtbl,
			   "%s ping <path> <times> <length> <cmd> "
			   "<period(ms)>\n\n",
			   argv[0]);
...
int cmd_rpmsg(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (strcmp(argv[2], "all") == 0) {
		return nsh_foreach_direntry(vtbl, "rpmsg", "/dev/rpmsg", cmd_rpmsg_recursive, argv);
	}
...
static int cmd_rptun_cb(FAR int *cmd, FAR unsigned long *val, FAR char **argv)
{
...
static int cmd_rptun_recursive(FAR struct nsh_vtbl_s *vtbl, FAR const char *dirpath,
							   FAR struct dirent *entryp, FAR void *pvarg)
{
	FAR char *path;
	int ret = ERROR;
...
int cmd_rptun(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (strcmp(argv[2], "all") == 0) {
		return nsh_foreach_direntry(vtbl, "rptun", "/dev/rptun", cmd_rptun_recursive, argv);
	}
