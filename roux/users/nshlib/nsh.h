...
#if !defined(CONFIG_USBHOST) || !defined(CONFIG_USBHOST_HIDKBD) || defined(HAVE_USB_CONSOLE)
...
#define TRACE_TRANSFER_BITS                                                                        \
	(TRACE_OUTREQQUEUED_BIT | TRACE_INREQQUEUED_BIT | TRACE_READ_BIT | TRACE_WRITE_BIT |           \
	 TRACE_COMPLETE_BIT)
#else
...
#define TRACE_BITSET                                                                               \
	(TRACE_INIT_BITS | TRACE_ERROR_BITS | TRACE_CLASS_BITS | TRACE_TRANSFER_BITS |                 \
...
#define CONFIG_NSH_LOGIN_PASSWORD "roux"
#endif
...
#undef NSH_HAVE_CPULOAD
	...
	(defined(CONFIG_FS_PROCFS_EXCLUDE_BLOCKS) && defined(CONFIG_FS_PROCFS_EXCLUDE_USAGE))
...
typedef CODE int (*nsh_cmd_t)(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv);

/* This is the form of a callback from nsh_foreach_direntry() */

struct dirent; /* Defined in dirent.h */
typedef CODE int (*nsh_direntry_handler_t)(FAR struct nsh_vtbl_s *vtbl, FAR const char *dirpath,
										   FAR struct dirent *entryp, FAR void *pvarg);

#if defined(CONFIG_NSH_VARS) && !defined(CONFIG_NSH_DISABLE_SET)
/* Used with nsh_foreach_var() */

typedef int (*nsh_foreach_var_t)(FAR struct nsh_vtbl_s *vtbl, FAR void *arg, FAR const char *pair);
#endif
...
#ifdef CONFIG_NSH_ALIAS
FAR struct nsh_alias_s *nsh_aliasfind(FAR struct nsh_vtbl_s *vtbl, FAR const char *token);
void nsh_aliasfree(FAR struct nsh_vtbl_s *vtbl, FAR struct nsh_alias_s *alias);
#endif

#ifndef CONFIG_NSH_DISABLESCRIPT
int nsh_script(FAR struct nsh_vtbl_s *vtbl, FAR const char *cmd, FAR const char *path, bool log);
#ifdef CONFIG_ETC_ROMFS
...
int nsh_session(FAR struct console_stdio_s *pstate, int login, int argc, FAR char *argv[]);
...
int nsh_builtin(FAR struct nsh_vtbl_s *vtbl, FAR const char *cmd, FAR char **argv,
				FAR const struct nsh_param_s *param);
#endif

#ifdef CONFIG_NSH_FILE_APPS
int nsh_fileapp(FAR struct nsh_vtbl_s *vtbl, FAR const char *cmd, FAR char **argv,
				FAR const struct nsh_param_s *param);
#endif
...
FAR char *nsh_getfullpath(FAR struct nsh_vtbl_s *vtbl, FAR const char *relpath);
void nsh_freefullpath(FAR char *fullpath);
#endif

/* Debug */

void nsh_dumpbuffer(FAR struct nsh_vtbl_s *vtbl, FAR const char *msg, FAR const uint8_t *buffer,
					ssize_t nbytes);
...
#endif
#if defined(CONFIG_NSH_LOGIN_PASSWD) && !defined(CONFIG_FSUTILS_PASSWD_READONLY)
...
#ifdef NSH_HAVE_CATFILE
int nsh_catfile(FAR struct nsh_vtbl_s *vtbl, FAR const char *cmd, FAR const char *filepath);
#endif
...
int nsh_readfile(FAR struct nsh_vtbl_s *vtbl, FAR const char *cmd, FAR const char *filepath,
...
int nsh_writefile(FAR struct nsh_vtbl_s *vtbl, FAR const char *cmd, FAR const char *buffer,
				  size_t len, FAR const char *filepath);
#endif
...
int nsh_foreach_direntry(FAR struct nsh_vtbl_s *vtbl, FAR const char *cmd, FAR const char *dirpath,
...
ssize_t nsh_getpid(FAR struct nsh_vtbl_s *vtbl, FAR const char *name, FAR pid_t *pids,
				   size_t count);
#endif
...
FAR char *nsh_getdirpath(FAR struct nsh_vtbl_s *vtbl, FAR const char *dirpath,
						 FAR const char *path);
...
#ifndef CONFIG_NSH_DISABLE_SET
int nsh_setvar(FAR struct nsh_vtbl_s *vtbl, FAR const char *name, FAR const char *value);
#endif
...
#if defined(CONFIG_NSH_VARS) && !defined(CONFIG_NSH_DISABLE_SET)
int nsh_foreach_var(FAR struct nsh_vtbl_s *vtbl, nsh_foreach_var_t cb, FAR void *arg);
#endif
