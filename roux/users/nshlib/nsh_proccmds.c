...
#ifndef CONFIG_NSH_DISABLE_PS
static void nsh_parse_statusline(FAR char *line, FAR struct nsh_taskstatus_s *status)
{
...
ssize_t ps_readprocfs(FAR struct nsh_vtbl_s *vtbl, FAR const char *basepath,
					  FAR const char *dirpath, FAR const struct dirent *entryp,
					  FAR struct nsh_taskstatus_s *status)
{
	...
	if (ret < 0 || filepath == NULL) {
		...
		ret = nsh_readfile(vtbl, "ps", filepath, status->td_buf + status->td_bufpos,
					 ...
					 FAR struct nsh_taskstatus_s *status)
{
	...
	if (status->td_stack_size > 0 && status->td_stack_used > 0) {
		...
		status->td_stack_filled = 10 * 100 * status->td_stack_used / status->td_stack_size;
	}
#endif
#endif
	...
	if (ret >= 0) {
		status->td_cpuload = nsh_trimspaces(status->td_buf + status->td_bufpos);
		status->td_bufpos += ret;
	}
#endif
	...
	if (ret >= 0) {
		status->td_cmdline = nsh_trimspaces(status->td_buf + status->td_bufpos);
		status->td_bufpos += ret;
	}
...
#endif
			   ,
			   status->td_priority, status->td_policy, status->td_type, status->td_flags,
			   status->td_state, status->td_event
#ifndef CONFIG_NSH_DISABLE_PSSIGMASK
			   ,
			   status->td_sigmask
#endif
...
#ifdef PS_SHOW_STACKUSAGE
			   ,
			   status->td_stack_used, status->td_stack_filled / 10, status->td_stack_filled % 10,
						...
						FAR struct dirent *entryp, FAR void *pvarg)
{
	...
	if (topstatus->size == 0) {
		...
		topstatus->status =
			realloc(topstatus->status, sizeof(topstatus->status[0]) * topstatus->size * 2);
		if (topstatus->status == NULL) {
...
int cmd_ps(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (argc - heap > 1) {
		...
		return nsh_foreach_direntry(vtbl, "ps", CONFIG_NSH_PROC_MOUNTPOINT, ps_callback, &heap);
	}
}
#endif
...
int cmd_uptime(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (argc < 2) {
		...
		nsh_output(vtbl, "%02u:%02u:%02u ", current_time->tm_hour, current_time->tm_min,
				   current_time->tm_sec);
	...
	if (updays) {
		nsh_output(vtbl, "%" PRIu32 " day%s, ", updays, (updays > 1) ? "s" : "");
	}

	if (pretty_format_opt) {
		if (uphours) {
			nsh_output(vtbl, "%" PRIu32 " hour%s, ", uphours, (uphours > 1) ? "s" : "");
		}

		nsh_output(vtbl, "%" PRIu32 " minute%s", upminutes, (upminutes > 1) ? "s" : "");
	} else {
		nsh_output(vtbl,
				   "%2" PRIu32 ":"
				   "%02" PRIu32,
...
int cmd_top(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	int tc = 0;
	int opt;
	struct nsh_topstatus_s topstatus = {0};
	...
	while (!quit) {
		...
		if (pidlist) {
			...
			while (pid != NULL && ret >= 0) {
				...
				ret = top_callback(vtbl, CONFIG_NSH_PROC_MOUNTPOINT, &entry, &topstatus);
				*--pid = ',';
				pid = strtok_r(NULL, ",", &save);
			}
		} else {
			ret = nsh_foreach_direntry(vtbl, "top", CONFIG_NSH_PROC_MOUNTPOINT, top_callback,
									   &topstatus);
		}

		if (ret < 0) {
			nsh_error(vtbl, g_fmtcmdfailed, "top", "nsh_foreach_direntry", NSH_ERRNO);
			break;
		}

		qsort(topstatus.status, topstatus.index, sizeof(topstatus.status[0]), top_cmpcpuload);
