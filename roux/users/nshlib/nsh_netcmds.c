...
typedef int (*nsh_netdev_callback_t)(FAR struct nsh_vtbl_s *vtbl, FAR char *devname);
 ...
 ****************************************************************************/
	...
	!defined(CONFIG_FS_PROCFS_EXCLUDE_NET) && !defined(CONFIG_NSH_DISABLE_IFCONFIG)
...
static int ifconfig_callback(FAR struct nsh_vtbl_s *vtbl, FAR char *devname)
{
	...
	snprintf(buffer, NAME_MAX + 12, CONFIG_NSH_PROC_MOUNTPOINT "/net/%s", devname);
...
#ifdef CONFIG_NET_TCP
#ifndef CONFIG_NSH_DISABLE_WGET
static int wget_callback(FAR char **buffer, int offset, int datend, FAR int *buflen, FAR void *arg)
{
	ssize_t written = write((int)((intptr_t)arg), &((*buffer)[offset]), datend - offset);
	if (written == -1) {
		return -errno;
	}
...
#if !defined(CONFIG_NSH_DISABLE_IFUPDOWN) || !defined(CONFIG_NSH_DISABLE_IFCONFIG)
static int nsh_foreach_netdev(nsh_netdev_callback_t callback, FAR struct nsh_vtbl_s *vtbl,
							  FAR char *cmd)
{
	...
	if (dir == NULL) {
		nsh_error(vtbl, "nsh: %s: Could not open %s/net (is procfs mounted?)\n", cmd,
	...
	while ((entry = readdir(dir)) != NULL) {
		...
		if (entry->d_type == DTYPE_FILE && netlib_getifstatus(entry->d_name, &flags) >= 0) {
...
int cmd_nslookup(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (ret != OK) {
		nsh_error(vtbl, g_fmtcmdfailed, argv[0], "getaddrinfo", NSH_HERRNO_OF(ret));
		return ERROR;
	}
	...
	for (next = info; next != NULL; next = next->ai_next) {
		...
		ret = getnameinfo(next->ai_addr, next->ai_addrlen, buffer, sizeof(buffer), NULL, 0,
						  NI_NUMERICHOST);
		if (ret != OK) {
			freeaddrinfo(info);
			nsh_error(vtbl, g_fmtcmdfailed, argv[0], "getnameinfo", NSH_HERRNO_OF(ret));
			return ERROR;
		}
...
int cmd_arp(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (opt_type == OPT_TYPE_ARP_LIST) {
		...
		if (nentries < 0) {
			nsh_error(vtbl, g_fmtcmdfailed, argv[0], netlib_get_arptable, NSH_ERRNO_OF(-nentries));
			free(arptab);
			return ERROR;
		}
		...
		for (i = 0; i < nentries; i++) {
			...
			if (ifname != NULL && strcmp(ifname, (FAR char *)arptab[i].arp_dev) != 0) {
				continue;
			}
			...
			snprintf(ipaddr, 16, "%u.%u.%u.%u", ptr[0], ptr[1], ptr[2], ptr[3]);

			/* Convert the MAC address string to a binary */

			snprintf(ethaddr, 24, "%02x:%02x:%02x:%02x:%02x:%02x",
					 (uint8_t)arptab[i].arp_ha.sa_data[0], (uint8_t)arptab[i].arp_ha.sa_data[1],
					 (uint8_t)arptab[i].arp_ha.sa_data[2], (uint8_t)arptab[i].arp_ha.sa_data[3],
					 (uint8_t)arptab[i].arp_ha.sa_data[4], (uint8_t)arptab[i].arp_ha.sa_data[5]);
