...
/* Are we using the ROUX console for I/O?  Or some other character device? */
...
struct nsh_vtbl_s {
	...
	ssize_t (*read)(FAR struct nsh_vtbl_s *vtbl, FAR void *buffer, size_t nbytes);
	int (*ioctl)(FAR struct nsh_vtbl_s *vtbl, int cmd, unsigned long arg);
#ifndef CONFIG_NSH_DISABLE_ERROR_PRINT
	int (*error)(FAR struct nsh_vtbl_s *vtbl, FAR const char *fmt, ...) printf_like(2, 3);
#endif
	int (*output)(FAR struct nsh_vtbl_s *vtbl, FAR const char *fmt, ...) printf_like(2, 3);
	FAR char *(*linebuffer)(FAR struct nsh_vtbl_s *vtbl);
	void (*redirect)(FAR struct nsh_vtbl_s *vtbl, int fd_in, int fd_out, FAR uint8_t *save);
