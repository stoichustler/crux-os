...
static int nsh_dumpvar(FAR struct nsh_vtbl_s *vtbl, FAR void *arg, FAR const char *pair)
{
	UNUSED(arg);
...
int cmd_env(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	return nsh_catfile(vtbl, argv[0], CONFIG_NSH_PROC_MOUNTPOINT "/self/group/env");
}
#endif
...
int cmd_set(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
...
#ifdef NSH_HAVE_VARS
	{
...
#endif
		{
			...
			if (ret < 0) {
				nsh_error(vtbl, g_fmtcmdfailed, argv[0], "nsh_setvar", NSH_ERRNO_OF(-ret));
			}
		}
...
int cmd_unset(FAR struct nsh_vtbl_s *vtbl, int argc, FAR char **argv)
{
	...
	if (status < 0 && status != -ENOENT) {
		nsh_error(vtbl, g_fmtcmdfailed, argv[0], "nsh_unsetvar", NSH_ERRNO_OF(-status));
		ret = ERROR;
	}
#endif
