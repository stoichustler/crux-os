...
static int nsh_consoleioctl(FAR struct nsh_vtbl_s *vtbl, int cmd, unsigned long arg);
static int nsh_consoleoutput(FAR struct nsh_vtbl_s *vtbl, FAR const char *fmt, ...)
	printf_like(2, 3);
#ifndef CONFIG_NSH_DISABLE_ERROR_PRINT
static int nsh_erroroutput(FAR struct nsh_vtbl_s *vtbl, FAR const char *fmt, ...) printf_like(2, 3);
#endif
static FAR char *nsh_consolelinebuffer(FAR struct nsh_vtbl_s *vtbl);
static void nsh_consoleredirect(FAR struct nsh_vtbl_s *vtbl, int fd_in, int fd_out,
								FAR uint8_t *save);
static void nsh_consoleundirect(FAR struct nsh_vtbl_s *vtbl, FAR uint8_t *save);
static void nsh_consoleexit(FAR struct nsh_vtbl_s *vtbl, int exitstatus) noreturn_function;
...
static void nsh_closeifnotclosed(struct console_stdio_s *pstate)
{
	...
	if (ERRFD(pstate) >= 0 && ERRFD(pstate) != STDERR_FILENO && ERRFD(pstate) != OUTFD(pstate)) {
		close(ERRFD(pstate));
	}
...
static ssize_t nsh_consoleread(FAR struct nsh_vtbl_s *vtbl, FAR void *buffer, size_t nbytes)
{
	FAR struct console_stdio_s *pstate = (FAR struct console_stdio_s *)vtbl;
	ssize_t ret;
...
#ifndef CONFIG_NSH_DISABLE_ERROR_PRINT
static int nsh_erroroutput(FAR struct nsh_vtbl_s *vtbl, FAR const char *fmt, ...)
{
	FAR struct console_stdio_s *pstate = (FAR struct console_stdio_s *)vtbl;
	va_list ap;
	int ret;
...
FAR struct console_stdio_s *nsh_newconsole(bool isctty)
{
	...
	if (pstate) {
		...
		strlcpy(pstate->cn_vtbl.cwd, CONFIG_LIBC_HOMEDIR, sizeof(pstate->cn_vtbl.cwd));
#endif
	}
