...
struct nxscope_proto_ops_s {
	...
	CODE int (*frame_get)(FAR struct nxscope_proto_s *p, FAR uint8_t *buff, size_t len,
						  FAR struct nxscope_frame_s *frame);

	/* Finalize a frame in a given buffer */

	CODE int (*frame_final)(FAR struct nxscope_proto_s *p, uint8_t id, FAR uint8_t *buff,
							FAR size_t *len);
};
