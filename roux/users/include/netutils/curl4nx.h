...
typedef size_t (*curl4nx_iofunc_f)(FAR char *ptr, size_t size, size_t nmemb, FAR void *userdata);
...
typedef int (*curl4nx_xferinfofunc_f)(FAR void *clientp, uint64_t dltotal, uint64_t dlnow,
...
FAR char *curl4nx_easy_escape(FAR struct curl4nx_s *handle, FAR const char *string, int length);
...
FAR char *curl4nx_easy_unescape(FAR struct curl4nx_s *handle, FAR const char *url, int inlength,
								FAR int *outlength);
...
int curl4nx_easy_setopt(FAR struct curl4nx_s *handle, int option, FAR void *optionptr);
...
int curl4nx_easy_getinfo(FAR struct curl4nx_s *handle, int info, FAR void *infoptr);
