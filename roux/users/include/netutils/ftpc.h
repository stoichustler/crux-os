...
#define SIZEOF_FTPC_DIRLIST(n) (sizeof(struct ftpc_dirlist_s) + ((n)-1) * sizeof(FAR char *))
...
int ftpc_getfile(SESSION handle, FAR const char *rname, FAR const char *lname, uint8_t how,
				 uint8_t xfrmode);
int ftp_putfile(SESSION handle, FAR const char *lname, FAR const char *rname, uint8_t how,
				uint8_t xfrmode);
