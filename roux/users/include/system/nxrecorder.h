...
struct nxrecorder_enc_ops_s {
	int format;
	CODE int (*pre_write)(int fd, uint32_t samplerate, uint8_t chans, uint8_t bps);
...
int nxrecorder_setdevice(FAR struct nxrecorder_s *precorder, FAR const char *device);
...
int nxrecorder_recordinternal(FAR struct nxrecorder_s *precorder, FAR const char *filename,
...
int nxrecorder_write_amr(int fd, uint32_t samplerate, uint8_t chans, uint8_t bps);
