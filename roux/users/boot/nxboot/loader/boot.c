...
#define IS_INTERNAL_MAGIC(magic) ((magic & NXBOOT_HEADER_MAGIC_INT_MASK) == NXBOOT_HEADER_MAGIC_INT)
...
{
	return (header->magic == NXBOOT_HEADER_MAGIC || IS_INTERNAL_MAGIC(header->magic)) &&
...
static uint32_t calculate_crc(int fd, struct nxboot_img_header *header)
{
	...
	while (remain > 0) {
		...
		if ((remain % 25) == 0) {
#ifdef CONFIG_NXBOOT_PRINTF_PROGRESS_PERCENT
			nxboot_progress(nxboot_progress_percent, ((total_size - remain) * 100) / total_size);
#else
...
static int copy_partition(int from, int where, struct nxboot_state *state, bool update)
{
	...
	magic = IS_INTERNAL_MAGIC(header.magic) ? NXBOOT_HEADER_MAGIC : NXBOOT_HEADER_MAGIC_INT;
	...
	memcpy(buf + offsetof(struct nxboot_img_header, magic), &magic, sizeof magic);
...
static bool compare_versions(struct nxboot_img_version *v1, struct nxboot_img_version *v2)
{
...
static enum nxboot_update_type get_update_type(struct nxboot_state *state, int primary, int update,
											   int recovery,
											   ...
											   struct nxboot_img_header *recovery_header)
{
	...
	if (update_header->magic == NXBOOT_HEADER_MAGIC && validate_image(update)) {
		if (primary_header->crc != update_header->crc ||
			!compare_versions(&primary_header->img_version, &update_header->img_version) ||
			!primary_valid) {
			nxboot_progress(nxboot_progress_end);
	...
	if (IS_INTERNAL_MAGIC(recovery_header->magic) && state->recovery_valid &&
		((IS_INTERNAL_MAGIC(primary_header->magic) && !state->primary_confirmed) ||
		 !primary_valid)) {
...
static int perform_update(struct nxboot_state *state, bool check_only)
{
	...
	if (state->next_boot == NXBOOT_UPDATE_TYPE_REVERT &&
		...
		if ((!state->recovery_present || !state->recovery_valid) && state->primary_confirmed &&
			primary_valid) {
...
int nxboot_get_state(struct nxboot_state *state)
{
	...
	if (primary_header.magic == NXBOOT_HEADER_MAGIC) {
		...
		recovery_pointer = primary_header.magic & NXBOOT_RECOVERY_PTR_MASK;
		if (recovery_pointer == NXBOOT_SECONDARY_SLOT_NUM) {
			state->primary_confirmed = primary_header.crc == secondary_header.crc;
		} else if (recovery_pointer == NXBOOT_TERTIARY_SLOT_NUM) {
			state->primary_confirmed = primary_header.crc == tertiary_header.crc;
		}
	}

	state->next_boot = get_update_type(state, primary, update, recovery, &primary_header,
									   update_header, recovery_header);
...
int nxboot_open_update_partition(void)
{
	...
	path = state.update == NXBOOT_SECONDARY_SLOT_NUM ? CONFIG_NXBOOT_SECONDARY_SLOT_PATH
...
int nxboot_get_confirm(void)
{
	...
	if (primary_header.magic == NXBOOT_HEADER_MAGIC) {
		...
		if (recovery_pointer != 0) {
			path = recovery_pointer == NXBOOT_SECONDARY_SLOT_NUM ? CONFIG_NXBOOT_SECONDARY_SLOT_PATH
																 : CONFIG_NXBOOT_TERTIARY_SLOT_PATH;
...
int nxboot_confirm(void)
{
	...
	while (remain > 0) {
		readsiz = remain > info_update.blocksize ? info_update.blocksize : remain;
