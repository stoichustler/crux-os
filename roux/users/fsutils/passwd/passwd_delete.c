...
static int passwd_copyfile(FAR char *iobuffer, FILE *instream, FILE *outstream, size_t copysize)
{
	FAR char *buffer;
	ssize_t nxfrd;
	size_t nwritten;
	...
	for (ncopied = 0, remaining = copysize, eof = false; ncopied < copysize && !eof;
		 ncopied += nwritten) {
...
errout_with_tmpfile:
	if (ret < 0) {
		...
		rename(CONFIG_FSUTILS_PASSWD_PATH ".tmp", CONFIG_FSUTILS_PASSWD_PATH);
	} else {
