...
int add_ptn(FAR struct efi_ptable_s *ptbl, uint64_t first, uint64_t last, FAR const char *name,
			FAR const uint8_t *type)
{
...
{
	fprintf(stderr, "usage: mkgpt write <disk> [ <partition> ]*\n"
					"       mkgpt read <disk>\n"
					"\n"
...
static void show(FAR struct efi_ptable_s *ptbl)
{
	...
	for (n = 0; n < ptbl->header.entries_count; n++) {
		...
		name[m] = 0;
		fprintf(stderr, "#%03d %13" PRIu64 " %13" PRIu64 " %s\n", n + 1, entry[n].first_lba,
				entry[n].last_lba, name);
	}
}
...
static int parse_ptn(FAR struct efi_ptable_s *ptbl, FAR char *x, FAR uint64_t *lba)
{
...
static void update_crc32(FAR struct efi_ptable_s *ptbl)
{
	...
	n = crc32part((FAR const uint8_t *)ptbl->entry,
				  sizeof(ptbl->entry[0]) * ptbl->header.entries_count, ~0l) ^
		~0l;
	ptbl->header.entries_crc32 = n;

	ptbl->header.crc32 = 0;
	n = crc32part((FAR const uint8_t *)&ptbl->header, sizeof(ptbl->header), ~0l) ^ ~0l;
	ptbl->header.crc32 = n;
}
...
static int verify_gpt_pratition(FAR struct efi_ptable_s *ptbl)
{
	...
	if ((ptbl->mbr[0x1c2] != 0xee && ptbl->mbr[0x1c2] != 0x00) || ptbl->mbr[0x1fe] != 0x55 ||
		ptbl->mbr[0x1ff] != 0xaa) {
	...
	orgcrc = hdr->crc32;
	hdr->crc32 = 0;
	if (orgcrc != (crc32part((FAR const uint8_t *)hdr, sizeof(*hdr), ~0l) ^ ~0l)) {
...
int main(int argc, FAR char **argv)
{
	...
	if (verify) {
		...
		if (ret < 0) {
			lib_dumpbuffer("Dump GPT partition:", (FAR uint8_t *)ptbl, sizeof(*ptbl));
		}
	...
	if (write(fd, &ptbl->entry, sizeof(ptbl->entry)) != sizeof(ptbl->entry)) {
