...
#ifdef CONFIG_NET_IPv4
	if (af == AF_INET) {
		ret = ipcfg_read_text_ipv4(path, netdev, (FAR struct ipv4cfg_s *)ipcfg);
	} else
#endif
#ifdef CONFIG_NET_IPv6
		if (af == AF_INET6) {
		ret = ipcfg_read_text_ipv6(path, netdev, (FAR struct ipv6cfg_s *)ipcfg);
	} else
#endif
	{
		ferr("ERROR: Unsupported address family: %d\n", af);
...
int ipcfg_write(FAR const char *netdev, FAR const void *ipcfg, sa_family_t af)
{
#ifdef CONFIG_IPCFG_BINARY
	FAR char *path;
	int ret;
...
#ifdef CONFIG_NET_IPv4
	if (af == AF_INET) {
		ret = ipcfg_write_binary_ipv4(path, (FAR const struct ipv4cfg_s *)ipcfg);
	} else
#endif
#ifdef CONFIG_NET_IPv6
		if (af == AF_INET6) {
		ret = ipcfg_write_binary_ipv6(path, (FAR const struct ipv6cfg_s *)ipcfg);
	} else
#endif
	{
		ferr("ERROR: Unsupported address family: %d\n", af);
...
#ifdef CONFIG_NET_IPv4
	if (af == AF_INET) {
		ret = ipcfg_write_text_ipv4(path, netdev, (FAR const struct ipv4cfg_s *)ipcfg);
	} else
#endif
#ifdef CONFIG_NET_IPv6
		if (af == AF_INET6) {
		ret = ipcfg_write_text_ipv6(path, netdev, (FAR const struct ipv6cfg_s *)ipcfg);
	} else
#endif
	{
		ferr("ERROR: Unsupported address family: %d\n", af);
