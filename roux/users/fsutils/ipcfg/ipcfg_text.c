...
#if defined(CONFIG_IPCFG_WRITABLE) && defined(CONFIG_NET_IPv4)
static int ipcfg_put_ipv4addr(FAR FILE *stream, FAR const char *variable, in_addr_t address)
{
	if (address != 0) {
...
#if defined(CONFIG_IPCFG_WRITABLE) && defined(CONFIG_NET_IPv4)
static int ipcfg_write_ipv4(FAR FILE *stream, FAR const struct ipv4cfg_s *ipv4cfg)
{
...
#if defined(CONFIG_IPCFG_WRITABLE) && defined(CONFIG_NET_IPv6)
static int ipcfg_write_ipv6(FAR FILE *stream, FAR const struct ipv6cfg_s *ipv6cfg)
{
	/* Format and write the file */
