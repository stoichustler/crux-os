...
static const uint8_t g_bootcodeblob[] = {
	0x0e, 0x1f, 0xbe, 0x00, 0x7c, 0xac, 0x22, 0xc0, 0x74, 0x0b, 0x56, 0xb4, 0x0e, 0xbb, 0x07,
	...
	0x69, 0x6e, 0x20, 0x2e, 0x2e, 0x2e, 0x0d, 0x0a, 0x00};
...
static inline uint32_t mkfatfs_nfatsect12(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var,
										  uint32_t navailsects)
{
	...
	if (navailsects <= (0x55555555 - (1 << (fmt->ff_clustshift + 1)))) {
		...
		denom =
			(fmt->ff_nfats << 1) + fmt->ff_nfats + (var->fv_sectorsize << (fmt->ff_clustshift + 1));
		number = (navailsects << 1) + navailsects + (1 << (fmt->ff_clustshift + 2)) +
...
static inline uint32_t mkfatfs_nfatsect16(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var,
										  uint32_t navailsects)
{
#ifdef CONFIG_HAVE_LONG_LONG
	uint64_t denom;
...
static inline uint32_t mkfatfs_nfatsect32(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var,
										  uint32_t navailsects)
{
#ifdef CONFIG_HAVE_LONG_LONG
	uint64_t denom;
...
static inline uint8_t mkfatfs_clustersearchlimits(FAR struct fat_format_s *fmt,
...
static inline int mkfatfs_tryfat12(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var,
								   FAR struct fat_config_s *config)
{
	...
	if (config->fc_nfatsects > 0) {
		...
		config->fc_nclusters =
			(config->fc_navailsects - fmt->ff_nfats * config->fc_nfatsects) >> fmt->ff_clustshift;
		...
		if (config->fc_nclusters + 2 > maxnclusters) {
			fwarn("WARNING:  Too many clusters for FAT12: %" PRId32 " > %" PRId32 "\n",
...
static inline int mkfatfs_tryfat16(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var,
								   FAR struct fat_config_s *config)
{
	uint32_t maxnclusters;
	...
	if (config->fc_nfatsects > 0) {
		...
		finfo("nfatsects=%" PRIu32 " nclusters=%" PRIu32 " (min=%u max=%" PRIu32 ")\n",
		...
		if ((config->fc_nclusters + 2 > maxnclusters) || (config->fc_nclusters < FAT_MINCLUST16)) {
...
static inline int mkfatfs_tryfat32(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var,
								   FAR struct fat_config_s *config)
{
	uint32_t maxnclusters;
	...
	if (config->fc_nfatsects > 0) {
		...
		if ((config->fc_nclusters + 3 > maxnclusters) || (config->fc_nclusters < FAT_MINCLUST32)) {
...
static inline void mkfatfs_selectfat(int fattype, FAR struct fat_format_s *fmt,
...
static inline int mkfatfs_clustersearch(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var)
{
	...
	if (var->fv_fattype != 32) {
		...
		fatconfig12.fc_navailsects = fatconfig16.fc_navailsects =
	...
	do {
      ...
      if (var->fv_fattype == 0 || var->fv_fattype == 32)
...
#endif
		{
			...
			fatconfig32.fc_navailsects =
				fmt->ff_nsectors - (1 << fmt->ff_clustshift) - fatconfig32.fc_rsvdseccount;
...
int mkfatfs_configfatfs(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var)
{
	int ret;
	...
	finfo("Volume Label:         \"%c%c%c%c%c%c%c%c%c%c%c\"\n", fmt->ff_volumelabel[0],
		  fmt->ff_volumelabel[1], fmt->ff_volumelabel[2], fmt->ff_volumelabel[3],
		  fmt->ff_volumelabel[4], fmt->ff_volumelabel[5], fmt->ff_volumelabel[6],
		  fmt->ff_volumelabel[7], fmt->ff_volumelabel[8], fmt->ff_volumelabel[9],
		  fmt->ff_volumelabel[10]);
#endif
	return OK;
}
