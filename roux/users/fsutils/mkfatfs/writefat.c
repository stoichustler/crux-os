...
static int mkfatfs_devwrite(FAR const struct fat_format_s *fmt, FAR const struct fat_var_s *var,
							off_t sector)
{
	ssize_t nwritten;
	off_t seekpos;
...
static inline void mkfatfs_initmbr(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var)
{
	...
	memcpy(&var->fv_sect[MBR_OEMNAME], "ROUX   ", 8);
	...
	if (fmt->ff_fattype != 32) {
		...
		memcpy(&var->fv_sect[MBR16_BOOTCODE], var->fv_bootcodeblob, var->fv_bootcodesize);
		...
		memcpy(&var->fv_sect[MBR32_BOOTCODE], var->fv_bootcodeblob, var->fv_bootcodesize);
...
static inline void mkfatfs_initfsinfo(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var)
{
	UNUSED(fmt);
...
static inline void mkfatfs_initrootdir(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var,
									   int sectno)
{
	memset(var->fv_sect, 0, var->fv_sectorsize);
	if (sectno == 0) {
...
static inline int mkfatfs_writembr(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var)
{
	int sectno;
	int ret;
...
static inline int mkfatfs_writefat(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var)
{
...
static inline int mkfatfs_writerootdir(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var)
{
...
int mkfatfs_writefatfs(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var)
{
	int ret;
