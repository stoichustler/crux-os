...
#if defined(CONFIG_MKFATFS_BUFFER_ALIGNMENT) && CONFIG_MKFATFS_BUFFER_ALIGNMENT > 0
...
static inline int mkfatfs_getgeometry(FAR struct fat_format_s *fmt, FAR struct fat_var_s *var)
{
	...
	ret = ioctl(var->fv_fd, BIOC_GEOMETRY, (unsigned long)((uintptr_t)&geometry));
	if (ret < 0) {
	...
	if (fmt->ff_nsectors != 0) {
		if (fmt->ff_nsectors > geometry.geo_nsectors) {
			ferr("ERROR: User maxblocks (%" PRId32 ") exceeds blocks on device (%ju)\n",
