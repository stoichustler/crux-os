...
#elif defined(CONFIG_CPP_HAVE_VARARGS)
#define iniinfo(format, ...) printf(EXTRA_FMT format EXTRA_ARG, ##__VA_ARGS__)
#else
...
static FAR char *inifile_find_section_variable(FAR struct inifile_state_s *priv,
											   FAR const char *variable);
static FAR char *inifile_find_variable(FAR struct inifile_state_s *priv, FAR const char *section,
									   FAR const char *variable);
...
static int inifile_read_line(FAR struct inifile_state_s *priv)
{
	...
	nbytes = 0;
	while ((nbytes < CONFIG_FSUTILS_INIFILE_MAXLINE) && (priv->nextch != EOF) &&
		   (priv->nextch != '\n')) {
...
FAR char *inifile_read_string(INIHANDLE handle, FAR const char *section, FAR const char *variable,
...
long inifile_read_integer(INIHANDLE handle, FAR const char *section, FAR const char *variable,
						  FAR long defvalue)
{
	FAR struct inifile_state_s *priv = (FAR struct inifile_state_s *)handle;
	FAR char *value;
