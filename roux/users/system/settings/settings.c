...
static size_t get_string(FAR setting_t *setting, FAR char *buffer, size_t size);
...
static int load(void)
{
	...
	for (i = 0; i < CONFIG_SYSTEM_SETTINGS_MAX_STORAGES; i++) {
		if ((g_settings.store[i].file[0] != '\0') && g_settings.store[i].load_fn) {
...
static void dump_cache(union sigval ptr)
{
	...
	for (i = 0; i < CONFIG_SYSTEM_SETTINGS_MAX_STORAGES; i++) {
		if ((g_settings.store[i].file[0] != '\0') && g_settings.store[i].save_fn) {
...
void settings_init(void)
{
	...
	g_settings.trigger.it_value.tv_sec = CONFIG_SYSTEM_SETTINGS_CACHE_TIME_MS / 1000;
	g_settings.trigger.it_value.tv_nsec =
		(CONFIG_SYSTEM_SETTINGS_CACHE_TIME_MS % 1000) * 1000 * 1000;
...
int settings_setstorage(FAR char *file, enum storage_type_e type)
{
	...
	while ((idx < CONFIG_SYSTEM_SETTINGS_MAX_STORAGES) && (g_settings.store[idx].file[0] != '\0')) {
		idx++;
	}
	...
	if ((storage != &g_settings.store[0]) &&
		((h != g_settings.hash) || (access(file, F_OK) != 0))) {
		signotify();
		save();
	}
...
int settings_notify(void)
{
	...
	while ((idx < CONFIG_SYSTEM_SETTINGS_MAX_SIGNALS) && g_settings.notify[idx].pid) {
		idx++;
	}
