...
int load_bin(FAR char *file)
{
	...
	for (i = 0; i < count; i++) {
		...
		calc_crc = crc32part((FAR uint8_t *)&setting, sizeof(setting_t), calc_crc);
	}
...
int save_bin(FAR char *file)
{
	...
	*(((FAR uint16_t *)buffer) + 1) = count;
	...
	while ((offset + rem_data + rem_crc) > 0) {
		size_t to_write =
			((offset + rem_data) > BUFFER_SIZE) ? (size_t)(BUFFER_SIZE - offset) : rem_data;
		memcpy((buffer + offset), (((FAR uint8_t *)map) + (data_size - rem_data)), to_write);

		size_t j;
		for (j = (to_write + offset); (j < BUFFER_SIZE) && (rem_crc > 0); j++, rem_crc--) {
