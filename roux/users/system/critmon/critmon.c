...
static int critmon_process_directory(FAR struct dirent *entryp)
{
	...
	filepath = NULL;
	ret = asprintf(&filepath, CONFIG_SYSTEM_CRITMONITOR_MOUNTPOINT "/%s/status", entryp->d_name);
	if (ret < 0) {
		fprintf(stderr, "Csection Monitor: Failed to create path to status file\n");
		return -ENOMEM;
	}
	...
	ret = asprintf(&filepath, CONFIG_SYSTEM_CRITMONITOR_MOUNTPOINT "/%s/critmon", entryp->d_name);
	if (ret < 0) {
...
static int critmon_list_once(void)
{
	...
	for (;;) {
		...
		if (DIRENT_ISDIRECTORY(entryp->d_type) && critmon_check_name(entryp->d_name)) {
...
int critmon_start_main(int argc, char **argv)
{
	...
	if (!g_critmon.started) {
		...
		ret = task_create("Csection Monitor", CONFIG_SYSTEM_CRITMONITOR_DAEMON_PRIORITY,
						  CONFIG_SYSTEM_CRITMONITOR_DAEMON_STACKSIZE, critmon_daemon, NULL);
		if (ret < 0) {
			int errcode = errno;
