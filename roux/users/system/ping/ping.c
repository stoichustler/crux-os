...
static void ping_result(FAR const struct ping_result_s *result)
{
	...
	case ICMP_I_BEGIN:
		printf("PING %u.%u.%u.%u %u bytes of data\n", ip4_addr1(result->dest.s_addr),
			   ip4_addr2(result->dest.s_addr), ip4_addr3(result->dest.s_addr),
			   ip4_addr4(result->dest.s_addr), result->info->datalen);
		break;
	...
	case ICMP_W_TIMEOUT:
		printf("No response from %u.%u.%u.%u: icmp_seq=%u time=%ld ms\n",
			   ip4_addr1(result->dest.s_addr), ip4_addr2(result->dest.s_addr),
			   ip4_addr3(result->dest.s_addr), ip4_addr4(result->dest.s_addr), result->seqno,
			   result->extra);
		break;
	...
	case ICMP_I_ROUNDTRIP:
		...
		printf("%u bytes from %u.%u.%u.%u: icmp_seq=%u time=%ld.%ld ms\n", result->info->datalen,
			   ip4_addr1(result->dest.s_addr), ip4_addr2(result->dest.s_addr),
			   ip4_addr3(result->dest.s_addr), ip4_addr4(result->dest.s_addr), result->seqno,
			   result->extra / USEC_PER_MSEC, result->extra % USEC_PER_MSEC / MSEC_PER_DSEC);
		break;
	...
	case ICMP_I_FINISH:
		if (result->nrequests > 0) {
			...
			printf("%u packets transmitted, %u received, %u%% packet loss, "
				   "time %ld ms\n",
				   result->nrequests, result->nreplies, tmp, result->extra / USEC_PER_MSEC);
			if (result->nreplies > 0) {
				...
				if (priv->tsum > 0) {
					...
					tempnum = priv->tsum2 / result->nreplies - (long long)avg * avg;
				...
				printf("rtt min/avg/max/mdev = %ld.%03ld/%ld.%03ld/"
					   ...
					   avg % USEC_PER_MSEC, priv->tmax / USEC_PER_MSEC, priv->tmax % USEC_PER_MSEC,
