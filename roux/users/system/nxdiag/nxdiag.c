...
static const char *g_valid_args[] = {"-h",	 "--help",
									 "-n",	 "--roux",
...
bool are_valid_args(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++) {
		if (!search_str_in_arr(nitems(g_valid_args), (char **)g_valid_args, argv[i])) {
			return false;
		}
	}
...
static void print_usage(char *prg)
{
	...
	fprintf(stderr, "         -h                       \
          Show this message\n");
	fprintf(stderr, "         -n, --roux              \
          Output the ROUX operational system information.\n");
#ifdef CONFIG_SYSTEM_NXDIAG_COMP_FLAGS
	fprintf(stderr, "         -f, --flags              \
          Output the ROUX compilation and linker flags used.\n");
#endif
#ifdef CONFIG_SYSTEM_NXDIAG_CONF
	fprintf(stderr, "         -c, --config             \
          Output the ROUX configuration options used.\n");
#endif
...
int main(int argc, char *argv[])
{
	...
	if (search_str_in_arr(argc, argv, "-h") || search_str_in_arr(argc, argv, "--help")) {
		print_usage(argv[0]);
		exit(EXIT_SUCCESS);
	}
	...
	if (search_str_in_arr(argc, argv, "-n") || search_str_in_arr(argc, argv, "--roux") ||
		search_str_in_arr(argc, argv, "--all")) {
		char hostname[HOST_NAME_MAX];
		gethostname(hostname, HOST_NAME_MAX);
		hostname[HOST_NAME_MAX - 1] = '\0';

		printf("ROUX RTOS info:\n");
		printf("\tHostname: %s\n", hostname);
...
#ifdef CONFIG_SYSTEM_NXDIAG_COMP_FLAGS
	if (search_str_in_arr(argc, argv, "-f") || search_str_in_arr(argc, argv, "--flags") ||
		search_str_in_arr(argc, argv, "--all")) {
		printf("ROUX CFLAGS:\n");
		print_array(ROUX_CFLAGS, ROUX_CFLAGS_ARRAY_SIZE);
		printf("ROUX CXXFLAGS:\n");
		print_array(ROUX_CXXFLAGS, ROUX_CXXFLAGS_ARRAY_SIZE);
		printf("ROUX LDFLAGS:\n");
		print_array(ROUX_LDFLAGS, ROUX_LDFLAGS_ARRAY_SIZE);
	}
#endif

#ifdef CONFIG_SYSTEM_NXDIAG_CONF
	if (search_str_in_arr(argc, argv, "-c") || search_str_in_arr(argc, argv, "--config") ||
		search_str_in_arr(argc, argv, "--all")) {
		printf("ROUX configuration options:\n");
		print_array(ROUX_CONFIG, ROUX_CONFIG_ARRAY_SIZE);
	}
#endif

	/* Host Info */

	if (search_str_in_arr(argc, argv, "-o") || search_str_in_arr(argc, argv, "--host-os") ||
		search_str_in_arr(argc, argv, "--all")) {
...
#ifdef CONFIG_SYSTEM_NXDIAG_HOST_PATH
	if (search_str_in_arr(argc, argv, "-p") || search_str_in_arr(argc, argv, "--host-path") ||
		search_str_in_arr(argc, argv, "--all")) {
...
#ifdef CONFIG_SYSTEM_NXDIAG_HOST_PACKAGES
	if (search_str_in_arr(argc, argv, "-k") || search_str_in_arr(argc, argv, "--host-packages") ||
		search_str_in_arr(argc, argv, "--all")) {
...
#ifdef CONFIG_SYSTEM_NXDIAG_HOST_MODULES
	if (search_str_in_arr(argc, argv, "-m") || search_str_in_arr(argc, argv, "--host-modules") ||
		search_str_in_arr(argc, argv, "--all")) {
	...
	if (search_str_in_arr(argc, argv, "-v") || search_str_in_arr(argc, argv, "--vendor-specific") ||
		search_str_in_arr(argc, argv, "--all")) {
