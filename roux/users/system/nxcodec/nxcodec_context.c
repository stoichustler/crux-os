...
static inline FAR nxcodec_t *nxcodec_context_to_nxcodec(FAR nxcodec_context_t *ctx)
{
	return V4L2_TYPE_IS_OUTPUT(ctx->type) ? container_of(ctx, nxcodec_t, output)
										  : container_of(ctx, nxcodec_t, capture);
}

static FAR nxcodec_context_buf_t *nxcodec_context_dequeue_buf(FAR nxcodec_context_t *ctx)
{
	...
	if (ret < 0) {
		printf("nxcodec %s VIDIOC_DQBUF - %s\n",
			   V4L2_TYPE_IS_OUTPUT(ctx->type) ? "output" : "capture", strerror(errno));
		return NULL;
	}
...
static FAR nxcodec_context_buf_t *nxcodec_context_get_freebuf(FAR nxcodec_context_t *ctx)
{
	int i;
...
static int nxcodec_context_write_data(FAR nxcodec_context_t *ctx, FAR const char *buf, int size)
{
	return write(ctx->fd, buf, size) < 0 ? -errno : 0;
}

static int nxcodec_context_read_yuv_data(FAR nxcodec_context_t *ctx, FAR char *buf,
										 FAR uint32_t *bytesused)
{
	size_t buflen = ctx->format.fmt.pix.width * ctx->format.fmt.pix.height * 3 / 2;
	ssize_t ret;
...
static int nxcodec_context_read_h264_data(FAR nxcodec_context_t *ctx, FAR char *buf,
										  FAR uint32_t *bytesused)
{
	char start_code[4];
	ssize_t ret;
	...
	if (buf[0] == 0x00 && buf[1] == 0x00 && buf[2] == 0x00 && buf[3] == 0x01) {
		...
		while (1) {
			...
			if (start_code[0] == 0x00 && start_code[1] == 0x00 && start_code[2] == 0x00 &&
				start_code[3] == 0x01) {
				size -= 4;
...
int nxcodec_context_enqueue_frame(FAR nxcodec_context_t *ctx)
{
	...
	if (ctx->format.fmt.pix.pixelformat == V4L2_PIX_FMT_H264) {
		ret = nxcodec_context_read_h264_data(ctx, buf->addr, &buf->buf.bytesused);
		if (ret < 0) {
			return ret;
		}
	} else if (ctx->format.fmt.pix.pixelformat == V4L2_PIX_FMT_YUV420) {
		ret = nxcodec_context_read_yuv_data(ctx, buf->addr, &buf->buf.bytesused);
		if (ret < 0) {
			return ret;
		}
	}
	...
	while (true) {
		...
		if (ret < 0) {
			printf("nxcodec %s enum_fmt error: %d\n",
				   V4L2_TYPE_IS_OUTPUT(ctx->type) ? "output" : "capture", errno);
			return -errno;
		}
...
int nxcodec_context_init(FAR nxcodec_context_t *ctx)
{
	...
	for (i = 0; i < ctx->nbuffers; i++) {
		...
		buf->addr = mmap(NULL, buf->buf.length, PROT_READ | PROT_WRITE, MAP_SHARED, codec->fd,
						 buf->buf.m.offset);
