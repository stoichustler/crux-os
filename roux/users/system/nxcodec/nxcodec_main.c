...
static const struct option g_long_options[] = {{"device", required_argument, NULL, 'd'},
...
int main(int argc, FAR char **argv)
{
	...
	codec.output.format.fmt.pix.width = codec.capture.format.fmt.pix.width = NXCODEC_WIDTH;

	codec.output.format.fmt.pix.height = codec.capture.format.fmt.pix.height = NXCODEC_HEIGHT;
	...
	while (1) {
		...
		case 's':
			sscanf(optarg, "%" SCNu32 "x%" SCNu32 "", &codec.capture.format.fmt.pix.width,
				   &codec.capture.format.fmt.pix.height);

			codec.output.format.fmt.pix.width = codec.capture.format.fmt.pix.width;
			codec.output.format.fmt.pix.height = codec.capture.format.fmt.pix.height;
		...
		case 'i':
			...
			snprintf(codec.output.filename, sizeof(codec.output.filename), "%s", optarg);

			if (cc[0]) {
				codec.output.format.fmt.pix.pixelformat = v4l2_fourcc(cc[0], cc[1], cc[2], cc[3]);
			}

			printf("nxcodec input  file: %s, format %c%c%c%c - %" PRIu32 "\n",
				   codec.output.filename, cc[0], cc[1], cc[2], cc[3],
				   codec.output.format.fmt.pix.pixelformat);

			break;

		case 'o':
			memset(codec.capture.filename, 0, sizeof(codec.capture.filename));
			snprintf(codec.capture.filename, sizeof(codec.capture.filename), "%s", optarg);

			if (cc[0]) {
				codec.capture.format.fmt.pix.pixelformat = v4l2_fourcc(cc[0], cc[1], cc[2], cc[3]);
			}

			printf("nxcodec output file: %s, format %c%c%c%c - %" PRIu32 "\n",
				   codec.capture.filename, cc[0], cc[1], cc[2], cc[3],
