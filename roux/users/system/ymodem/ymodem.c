...
static int ymodem_recv_buffer(FAR struct ymodem_ctx_s *ctx, FAR uint8_t *buf, size_t size)
{
	size_t i = 0;
...
static int ymodem_send_buffer(FAR struct ymodem_ctx_s *ctx, FAR const uint8_t *buf, size_t size)
{
	size_t i = 0;
...
static int ymodem_recv_packet(FAR struct ymodem_ctx_s *ctx)
{
	...
	ret = ymodem_recv_buffer(ctx, &ctx->header[1], 2 + ctx->packet_size + 2);
	if (ret < 0) {
	...
	recv_crc = (ctx->data[ctx->packet_size] << 8) + ctx->data[ctx->packet_size + 1];
...
send_start:
	...
	sprintf((FAR char *)ctx->data, "%s%c%zu", ctx->file_name, '\0', ctx->file_length);
	ctx->header[0] = SOH;
	ctx->header[1] = 0x00;
	ctx->header[2] = 0xff;
	ctx->packet_size = YMODEM_PACKET_SIZE;
...
#ifdef CONFIG_SYSTEM_YMODEM_DEBUG_FILEPATH
	ctx->debug_fd = open(CONFIG_SYSTEM_YMODEM_DEBUG_FILEPATH, O_CREAT | O_TRUNC | O_WRONLY, 0666);
	if (ctx->debug_fd < 0) {
		free(ctx->header);
		return -errno;
	}
