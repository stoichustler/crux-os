...
static int write_data(FAR struct ymodem_priv_s *priv, FAR const uint8_t *data, size_t size)
{
	size_t i = 0;
...
static int handler(FAR struct ymodem_ctx_s *ctx)
{
	...
	if (ctx->packet_type == YMODEM_FILENAME_PACKET) {
		...
		if (priv->skip_perfix != NULL) {
			...
			if (strncmp(ctx->file_name, priv->skip_perfix, len) == 0) {
				filename += len;
			}
		}
		...
		if (priv->skip_suffix != NULL) {
			...
			if (len > len2 && strcmp(filename + len - len2, priv->skip_suffix) == 0) {
				filename[len - len2] = '\0';
			}
		}

		if (priv->foldname != NULL) {
			snprintf(temp, sizeof(temp), "%s/%s", priv->foldname, filename);
			filename = temp;
		}
...
static void show_usage(FAR const char *progname)
{
	...
	fprintf(stderr, "\t-d <device>: Communication device to use."
					"Default: stdin & stdout\n");
	fprintf(stderr, "\t-f <foldname>: Save remote file fold. Default: Current fold\n");
	fprintf(stderr, "\t-p|--skip_prefix <prefix>: Remove file name prefix\n");
	fprintf(stderr, "\t-s|--skip_suffix <suffix>: Remove file name suffix\n");
	fprintf(stderr, "\t-b|--buffersize <size>: Asynchronously receive buffer size."
					"If greater than 0, accept data asynchronously, Default: 0kB\n");
	fprintf(stderr, "\t-t|--threshold <size>: Threshold for writing asynchronously."
					"Threshold must be less than or equal buffersize, Default: 0kB\n");
	fprintf(stderr, "\t-i|--interval <time>: Waiting interval for transmitting data."
					"Max:255 Min:1 Default:15 unit: 100 milliseconds\n");
	fprintf(stderr, "\t-r|--retry <retry>: Number of retries."
					"Will try <retry> times to transmitting, Default:100\n");
	fprintf(stderr, "\t-k <size>: Use a custom size to tansfer, Default: 1kB\n");
...
int main(int argc, FAR char *argv[])
{
	...
	struct option options[] = {
		{"buffersize", 1, NULL, 'b'}, {"skip_prefix", 1, NULL, 'p'}, {"skip_suffix", 1, NULL, 's'},
		{"threshold", 1, NULL, 't'},  {"interval", 1, NULL, 'i'},	 {"retry", 1, NULL, 'r'},
	};
	...
	ctx.interval = 15;
	ctx.retry = 100;
	while ((ret = getopt_long(argc, argv, "b:d:f:hk:p:s:t:i:r:", options, NULL)) != ERROR) {
		switch (ret) {
		case 'b':
	...
	if (priv.buffersize &&
		(priv.threshold > priv.buffersize || ctx.custom_size > priv.buffersize)) {
		show_usage(argv[0]);
	}
