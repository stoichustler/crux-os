...
static int read_data(FAR struct ymodem_priv_s *priv, FAR uint8_t *data, size_t size)
{
	ssize_t i = 0;
...
static int handler(FAR struct ymodem_ctx_s *ctx)
{
	...
	if (ctx->packet_type == YMODEM_FILENAME_PACKET) {
		...
		if (ctx->file_length > ctx->packet_size) {
			...
			memset(ctx->data + size, 0x1a, ctx->packet_size - ctx->file_length);
		}
...
static void show_usage(FAR const char *progname)
{
	...
	fprintf(stderr, "\t-d <device>: Communication device to use. Default: stdin & stdout\n");
	fprintf(stderr, "\t-b|--buffersize <size>: Asynchronously send buffer size."
...
int main(int argc, FAR char *argv[])
{
	...
	ctx.interval = 15;
	ctx.retry = 100;
	while ((ret = getopt_long(argc, argv, "b:d:k:h", options, NULL)) != ERROR) {
		switch (ret) {
		case 'b':
