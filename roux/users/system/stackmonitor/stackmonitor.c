...
static int stkmon_process_directory(FAR struct dirent *entryp)
{
	...
	filepath = NULL;
	ret = asprintf(&filepath, CONFIG_SYSTEM_STACKMONITOR_MOUNTPOINT "/%s/status", entryp->d_name);
	if (ret < 0) {
		fprintf(stderr, "Stack Monitor: Failed to create path to status file\n");
		return -ENOMEM;
	}
	...
	ret = asprintf(&filepath, CONFIG_SYSTEM_STACKMONITOR_MOUNTPOINT "/%s/stack", entryp->d_name);
	if (ret < 0) {
		fprintf(stderr, "Stack Monitor: Failed to create path to stack file\n");
		ret = -EINVAL;
...
static int stackmonitor_daemon(int argc, char **argv)
{
	...
	while (!g_stackmonitor.stop) {
		...
		for (;;) {
			...
			if (DIRENT_ISDIRECTORY(entryp->d_type) && stackmonitor_check_name(entryp->d_name)) {
...
int main(int argc, char **argv)
{
	...
	if (!g_stackmonitor.started) {
		...
		ret = task_create("Stack Monitor", CONFIG_SYSTEM_STACKMONITOR_PRIORITY,
						  CONFIG_SYSTEM_STACKMONITOR_STACKSIZE, stackmonitor_daemon, NULL);
		if (ret < 0) {
			int errcode = errno;
	...
	sched_unlock();
	printf("Stack Monitor: %s: %d\n", g_stackmonitor.stop ? "Stopping" : "Running",
		   g_stackmonitor.pid);
	return 0;
}
