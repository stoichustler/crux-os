...
#ifdef CONFIG_NXLOOPER_INCLUDE_PREFERRED_DEVICE
	{"device", "devfile", nxlooper_cmd_device,
	 NXLOOPER_HELP_TEXT("Specify a preferred play/record device")},
#endif
#ifdef CONFIG_NXLOOPER_INCLUDE_HELP
	{"h", "", nxlooper_cmd_help, NXLOOPER_HELP_TEXT("Display help for commands")},
	{"help", "", nxlooper_cmd_help, NXLOOPER_HELP_TEXT("Display help for commands")},
#endif
	{"loopback", "channels bpsamp samprate format chmap", nxlooper_cmd_loopback,
	 NXLOOPER_HELP_TEXT("Audio loopback test")},
#ifndef CONFIG_AUDIO_EXCLUDE_PAUSE_RESUME
	{"pause", "", nxlooper_cmd_pause, NXLOOPER_HELP_TEXT("Pause loopback")},
#endif
#ifdef CONFIG_NXLOOPER_INCLUDE_SYSTEM_RESET
	{"reset", "", nxlooper_cmd_reset, NXLOOPER_HELP_TEXT("Perform a HW reset")},
#endif
#ifndef CONFIG_AUDIO_EXCLUDE_PAUSE_RESUME
	{"resume", "", nxlooper_cmd_resume, NXLOOPER_HELP_TEXT("Resume loopback")},
#endif
#ifndef CONFIG_AUDIO_EXCLUDE_STOP
	{"stop", "", nxlooper_cmd_stop, NXLOOPER_HELP_TEXT("Stop loopback")},
#endif
	{"q", "", nxlooper_cmd_quit, NXLOOPER_HELP_TEXT("Exit NxLooper")},
	{"quit", "", nxlooper_cmd_quit, NXLOOPER_HELP_TEXT("Exit NxLooper")},
#ifndef CONFIG_AUDIO_EXCLUDE_VOLUME
	{"volume", "d%", nxlooper_cmd_volume, NXLOOPER_HELP_TEXT("Set volume to level specified")}
#endif
};

static const int g_nxlooper_cmd_count = sizeof(g_nxlooper_cmds) / sizeof(struct mp_cmd_s);
...
static int nxlooper_cmd_loopback(FAR struct nxlooper_s *plooper, char *parg)
{
	...
	sscanf(parg, "%d %d %d %d %d", &channels, &bpsamp, &samprate, &chmap, &format);
	...
	ret = nxlooper_loopback(plooper, format, channels, bpsamp, samprate, chmap);
...
static int nxlooper_cmd_help(FAR struct nxlooper_s *plooper, char *parg)
{
	...
	for (x = 0; x < g_nxlooper_cmd_count; x++) {
		len = strlen(g_nxlooper_cmds[x].cmd) + strlen(g_nxlooper_cmds[x].arghelp);
		if (len > maxlen) {
			maxlen = len;
		}
	}
	...
	for (x = 0; x < g_nxlooper_cmd_count; x++) {
		...
		len = maxlen - (strlen(g_nxlooper_cmds[x].cmd) + strlen(g_nxlooper_cmds[x].arghelp));
		for (c = 0; c < len; c++) {
			printf(" ");
		}
 ...
 *   tester that uses the ROUX audio system to record and then play
