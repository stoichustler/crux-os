...
#ifdef CONFIG_NXLOOPER_INCLUDE_DEVICE_SEARCH
	{
		...
		while ((pdevice = readdir(dirp)) != NULL) {
			...
			if ((dev_fd = open(path, O_RDWR | O_CLOEXEC)) != -1) {
				...
				if (ioctl(dev_fd, AUDIOIOC_GETCAPS, (unsigned long)&caps) == caps.ac_len) {
					if ((plooper->playdev_fd == -1) && caps.ac_controls.b[0] & AUDIO_TYPE_OUTPUT) {
					...
					if (plooper->recorddev_fd != -1 && plooper->playdev_fd != -1) {
						closedir(dirp);
						return OK;
					}
				} else {
...
static int nxlooper_enqueuerecordbuffer(FAR struct nxlooper_s *plooper, FAR struct ap_buffer_s *apb)
{
	...
	ret = ioctl(plooper->recorddev_fd, AUDIOIOC_ENQUEUEBUFFER, (unsigned long)&bufdesc);
	if (ret < 0) {
		int errcode = errno;
		DEBUGASSERT(errcode > 0);
...
static int nxlooper_enqueueplaybuffer(FAR struct nxlooper_s *plooper, FAR struct ap_buffer_s *apb)
{
	struct audio_buf_desc_s bufdesc;
	int ret;
	...
	ret = ioctl(plooper->playdev_fd, AUDIOIOC_ENQUEUEBUFFER, (unsigned long)&bufdesc);
	if (ret < 0) {
		int errcode = errno;
		DEBUGASSERT(errcode > 0);
...
static void *nxlooper_loopthread(pthread_addr_t pvarg)
{
	...
	recordbufs = (FAR struct ap_buffer_s **)calloc(recordbuf_info.nbuffers, sizeof(FAR void *));
	if (recordbufs == NULL) {
	...
	for (x = 0; x < recordbuf_info.nbuffers; x++) {
		...
		ret = ioctl(plooper->recorddev_fd, AUDIOIOC_ALLOCBUFFER, (unsigned long)&buf_desc);
	...
	if ((ret = ioctl(plooper->playdev_fd, AUDIOIOC_GETBUFFERINFO, (unsigned long)&playbuf_info)) !=
		OK) {
		/* Driver doesn't report it's buffer size.  Use our default. */
	...
	playbufs = (FAR struct ap_buffer_s **)calloc(playbuf_info.nbuffers, sizeof(FAR void *));
	if (playbufs == NULL) {
		/* Error allocating memory for buffer storage! */
	...
	for (x = 0; x < playbuf_info.nbuffers; x++) {
		...
		ret = ioctl(plooper->playdev_fd, AUDIOIOC_ALLOCBUFFER, (unsigned long)&buf_desc);
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
	ret = ioctl(plooper->recorddev_fd, AUDIOIOC_START, (unsigned long)plooper->precordses);
#else
	...
	while (running) {
		...
		case AUDIO_MSG_DEQUEUE:
			...
			if (dq_count(&playdq) != 0 && dq_count(&recorddq) != 0) {
				...
				copy = MIN(apbrec->nbytes - apbrec->curbyte, apb->nmaxbytes - apb->curbyte);
				...
				if (apbrec->curbyte == apbrec->nbytes) {
					apbrec = (FAR struct ap_buffer_s *)dq_remfirst(&recorddq);
					apbrec->curbyte = 0;
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
				ret = ioctl(plooper->playdev_fd, AUDIOIOC_START, (unsigned long)plooper->pplayses);
#else
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
			ioctl(plooper->playdev_fd, AUDIOIOC_STOP, (unsigned long)plooper->pplayses);
			ioctl(plooper->recorddev_fd, AUDIOIOC_STOP, (unsigned long)plooper->precordses);
#else
...
err_out:
	...
	if (playbufs != NULL) {
		...
		for (x = 0; x < playbuf_info.nbuffers; x++) {
			...
			if (playbufs[x] != NULL) {
				...
				ioctl(plooper->playdev_fd, AUDIOIOC_FREEBUFFER, (unsigned long)&buf_desc);
			}
		}
	...
	if (recordbufs != NULL) {
		...
		for (x = 0; x < recordbuf_info.nbuffers; x++) {
			...
			if (recordbufs[x] != NULL) {
				...
				ioctl(plooper->recorddev_fd, AUDIOIOC_FREEBUFFER, (unsigned long)&buf_desc);
			}
		}
	...
	ioctl(plooper->playdev_fd, AUDIOIOC_UNREGISTERMQ, (unsigned long)plooper->mq);
	ioctl(plooper->recorddev_fd, AUDIOIOC_UNREGISTERMQ, (unsigned long)plooper->mq);
#ifdef CONFIG_AUDIO_MULTI_SESSION
	ioctl(plooper->playdev_fd, AUDIOIOC_RELEASE, (unsigned long)plooper->pplayses);
	ioctl(plooper->recorddev_fd, AUDIOIOC_RELEASE, (unsigned long)plooper->precordses);
#else
...
int nxlooper_setvolume(FAR struct nxlooper_s *plooper, uint16_t volume)
{
	...
	if (plooper->loopstate == NXLOOPER_STATE_LOOPING) {
		...
		ret = ioctl(plooper->playdev_fd, AUDIOIOC_CONFIGURE, (unsigned long)&cap_desc);
		if (ret < 0) {
			int errcode = errno;
			DEBUGASSERT(errcode > 0);
...
int nxlooper_pause(FAR struct nxlooper_s *plooper)
{
	...
	if (plooper->loopstate == NXLOOPER_STATE_LOOPING) {
#ifdef CONFIG_AUDIO_MULTI_SESSION
		ret = ioctl(plooper->playdev_fd, AUDIOIOC_PAUSE, (unsigned long)plooper->pplayses);

		if (ret == OK) {
			ret = ioctl(plooper->recorddev_fd, AUDIOIOC_PAUSE, (unsigned long)plooper->precordses);
		}
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
		ret = ioctl(plooper->recorddev_fd, AUDIOIOC_RESUME, (unsigned long)plooper->pplayses);

		if (ret == OK) {
			ret = ioctl(plooper->playdev_fd, AUDIOIOC_RESUME, (unsigned long)plooper->precordses);
		}
...
#ifdef CONFIG_NXLOOPER_INCLUDE_PREFERRED_DEVICE
int nxlooper_setdevice(FAR struct nxlooper_s *plooper, FAR const char *pdevice)
{
	int temp_fd;
	struct audio_caps_s caps;
...
int nxlooper_stop(FAR struct nxlooper_s *plooper)
{
	...
	term_msg.u.data = 0;
	mq_send(plooper->mq, (FAR const char *)&term_msg, sizeof(term_msg), CONFIG_NXLOOPER_MSG_PRIO);
...
int nxlooper_loopback(FAR struct nxlooper_s *plooper, int format, uint8_t nchannels, uint8_t bpsamp,
					  uint32_t samprate, uint8_t chmap)
{
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
	ret = ioctl(plooper->recorddev_fd, AUDIOIOC_RESERVE, (unsigned long)&plooper->precordses);
#else
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
	ret = ioctl(plooper->playdev_fd, AUDIOIOC_RESERVE, (unsigned long)&plooper->pplayses);
#else
	...
	if (ioctl(plooper->recorddev_fd, AUDIOIOC_GETCAPS, (unsigned long)&caps) == caps.ac_len) {
	...
	ret = ioctl(plooper->recorddev_fd, AUDIOIOC_CONFIGURE, (unsigned long)&cap_desc);
	if (ret < 0) {
		ret = -errno;
		goto err_out;
	}
	...
	if ((ioctl(plooper->playdev_fd, AUDIOIOC_GETBUFFERINFO, (unsigned long)&buf_info)) != OK) {
	...
	ioctl(plooper->recorddev_fd, AUDIOIOC_REGISTERMQ, (unsigned long)plooper->mq);

	ioctl(plooper->playdev_fd, AUDIOIOC_REGISTERMQ, (unsigned long)plooper->mq);
	...
	ret = pthread_create(&plooper->loop_id, &tattr, nxlooper_loopthread, (pthread_addr_t)plooper);
