...
FILE *popen(FAR const char *command, FAR const char *mode)
{
	...
	if (strcmp(mode, "r") == 0 && (result = pipe2(fd, O_CLOEXEC)) >= 0) {
	...
	} else if (strcmp(mode, "w") == 0 && (result = pipe2(fd, O_CLOEXEC)) >= 0) {
	...
	/* Create a socketpair. Using fd[0] as the input and output to the shell */
			 ...
			 (result = socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0, fd)) >= 0) {
	...
	errcode = posix_spawnattr_setstacksize(&attr, CONFIG_SYSTEM_POPEN_STACKSIZE);
	if (errcode != 0) {
		goto errout_with_actions;
	}
#endif
	...
	errcode = posix_spawnattr_setflags(&attr, POSIX_SPAWN_SETSCHEDPARAM | POSIX_SPAWN_SETSCHEDULER);
	if (errcode != 0) {
		goto errout_with_actions;
	}
	...
	errcode = posix_spawn_file_actions_adddup2(&file_actions, newfd[0], oldfd[0]);
	if (errcode != 0) {
		goto errout_with_actions;
	}

	if (rw) {
		errcode = posix_spawn_file_actions_adddup2(&file_actions, newfd[1], oldfd[1]);
		if (errcode != 0) {
			goto errout_with_actions;
		}
	}
	...
	errcode = posix_spawn(&container->shell, argv[0], &file_actions, &attr, argv, NULL);
#else
	container->shell = task_spawn("popen", nsh_system, &file_actions, &attr, argv + 1, NULL);
