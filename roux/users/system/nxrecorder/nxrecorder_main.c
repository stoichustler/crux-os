...
static int nxrecorder_cmd_quit(FAR struct nxrecorder_s *precorder, FAR char *parg);
static int nxrecorder_cmd_recordraw(FAR struct nxrecorder_s *precorder, FAR char *parg);
static int nxrecorder_cmd_record(FAR struct nxrecorder_s *precorder, FAR char *parg);
static int nxrecorder_cmd_device(FAR struct nxrecorder_s *precorder, FAR char *parg);

#ifndef CONFIG_AUDIO_EXCLUDE_PAUSE_RESUME
static int nxrecorder_cmd_pause(FAR struct nxrecorder_s *precorder, FAR char *parg);
static int nxrecorder_cmd_resume(FAR struct nxrecorder_s *precorder, FAR char *parg);
#endif

#ifndef CONFIG_AUDIO_EXCLUDE_STOP
static int nxrecorder_cmd_stop(FAR struct nxrecorder_s *precorder, FAR char *parg);
#endif

#ifdef CONFIG_NXRECORDER_INCLUDE_HELP
static int nxrecorder_cmd_help(FAR struct nxrecorder_s *precorder, FAR char *parg);
#endif
...
static const struct mp_cmd_s g_nxrecorder_cmds[] = {
	{"device", "devfile", nxrecorder_cmd_device,
...
#endif
	{"recordraw", "filename", nxrecorder_cmd_recordraw,
	 NXRECORDER_HELP_TEXT("Record a pcm raw file")},
	{"record", "filename", nxrecorder_cmd_record, NXRECORDER_HELP_TEXT("Record a media file")},
...
static int nxrecorder_cmd_recordraw(FAR struct nxrecorder_s *precorder, FAR char *parg)
{
	...
	sscanf(parg, "%s %d %d %d %d", filename, &channels, &bpsamp, &samprate, &chmap);

	/* Try to record the file specified */

	ret = nxrecorder_recordinternal(precorder, filename, AUDIO_FMT_PCM, channels, bpsamp, samprate,
									chmap);
...
static int nxrecorder_cmd_record(FAR struct nxrecorder_s *precorder, FAR char *parg)
{
	...
	ret = nxrecorder_recordinternal(precorder, filename, AUDIO_FMT_UNDEF, channels, bpsamp,
									samprate, chmap);
 ...
 *   that uses the ROUX audio system to record pcm raw data files read in
