...
static const int g_known_ext_count = sizeof(g_known_ext) / sizeof(struct nxrecorder_ext_fmt_s);
#endif
...
static int nxrecorder_opendevice(FAR struct nxrecorder_s *precorder, int format, int subfmt)
{
	...
	if (precorder->device[0] != '\0') {
		...
		if (ioctl(precorder->dev_fd, AUDIOIOC_GETCAPS, (uintptr_t)&cap) == cap.ac_len) {
			if (((cap.ac_format.hw & (1 << (format - 1))) ||
				...
				if (subfmt != AUDIO_FMT_UNDEF && supported) {
					...
					while (ioctl(precorder->dev_fd, AUDIOIOC_GETCAPS, (uintptr_t)&cap) ==
						   cap.ac_len) {
...
static int nxrecorder_writebuffer(FAR struct nxrecorder_s *precorder, FAR struct ap_buffer_s *apb)
{
	int ret;
...
static int nxrecorder_enqueuebuffer(FAR struct nxrecorder_s *precorder, FAR struct ap_buffer_s *apb)
{
	...
	ret = ioctl(precorder->dev_fd, AUDIOIOC_ENQUEUEBUFFER, (uintptr_t)&bufdesc);
	if (ret < 0) {
		int errcode = errno;
		DEBUGASSERT(errcode > 0);
...
static FAR void *nxrecorder_recordthread(pthread_addr_t pvarg)
{
	...
	if ((ret = ioctl(precorder->dev_fd, AUDIOIOC_GETBUFFERINFO, (uintptr_t)&buf_info)) != OK) {
	...
	for (x = 0; x < buf_info.nbuffers; x++) {
		...
		ret = ioctl(precorder->dev_fd, AUDIOIOC_ALLOCBUFFER, (uintptr_t)&buf_desc);
	...
	if (running && !failed) {
#ifdef CONFIG_AUDIO_MULTI_SESSION
		ret = ioctl(precorder->dev_fd, AUDIOIOC_START, (uintptr_t)precorder->session);
#else
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
			ioctl(precorder->dev_fd, AUDIOIOC_STOP, (uintptr_t)precorder->session);
#else
...
err_out:
	...
	for (x = 0; x < buf_info.nbuffers; x++) {
		...
		if (pbuffers[x] != NULL) {
			...
			ioctl(precorder->dev_fd, AUDIOIOC_FREEBUFFER, (uintptr_t)&buf_desc);
		}
	}

	/* Unregister the message queue and release the session */

	ioctl(precorder->dev_fd, AUDIOIOC_UNREGISTERMQ, (uintptr_t)precorder->mq);
#ifdef CONFIG_AUDIO_MULTI_SESSION
	ioctl(precorder->dev_fd, AUDIOIOC_RELEASE, (uintptr_t)precorder->session);
#else
	ioctl(precorder->dev_fd, AUDIOIOC_RELEASE, 0);
#endif
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
		ret = ioctl(precorder->dev_fd, AUDIOIOC_PAUSE, (uintptr_t)precorder->session);
#else
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
		ret = ioctl(precorder->dev_fd, AUDIOIOC_RESUME, (uintptr_t)precorder->session);
#else
...
int nxrecorder_recordinternal(FAR struct nxrecorder_s *precorder, FAR const char *pfilename,
							  int filefmt, uint8_t nchannels, uint8_t bpsamp, uint32_t samprate,
							  uint8_t chmap)
{
	struct mq_attr attr;
	...
	if ((precorder->fd = open(pfilename, O_WRONLY | O_CREAT | O_TRUNC, 0666)) == -1) {
	...
	if (precorder->ops->pre_write) {
		ret = precorder->ops->pre_write(precorder->fd, samprate, nchannels, bpsamp);
		if (ret < 0) {
			goto err_out;
		}
	}
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
	ret = ioctl(precorder->dev_fd, AUDIOIOC_RESERVE, (uintptr_t)&precorder->session);
#else
	...
	if (ioctl(precorder->dev_fd, AUDIOIOC_GETCAPS, (unsigned long)&caps) == caps.ac_len) {
	...
	ret = ioctl(precorder->dev_fd, AUDIOIOC_CONFIGURE, (uintptr_t)&cap_desc);
	if (ret < 0) {
		ret = -errno;
		goto err_out;
	}
	...
	if (ioctl(precorder->dev_fd, AUDIOIOC_GETBUFFERINFO, (uintptr_t)&buf_info) != OK) {
		/* Driver doesn't report its buffer size.  Use our default. */
	...
	ioctl(precorder->dev_fd, AUDIOIOC_REGISTERMQ, (uintptr_t)precorder->mq);
	...
	pthread_attr_setstacksize(&tattr, CONFIG_NXRECORDER_RECORDTHREAD_STACKSIZE);
	...
	ret = pthread_create(&precorder->record_id, &tattr, nxrecorder_recordthread,
