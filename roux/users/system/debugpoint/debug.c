...
static void debug_callback_cmd(int type, FAR void *addr, size_t size, FAR void *arg)
{
...
static void debug_callback(int type, FAR void *addr, size_t size, FAR void *arg)
{
	*(bool *)arg = true;
	gdb_debugpoint_remove(type, addr, size);
}
...
nooptimiziation_function static void test_function(void (*func)(void))
{
	...
	ret = gdb_debugpoint_add(GDB_STOPREASON_BREAKPOINT, func, 0, debug_callback, &triggered);
...
nooptimiziation_function static void test_data(uint8_t *addr, size_t size, bool r, bool w)
{
	int ret = 0;
	int tmp;
	int test_type;
	...
	ret = gdb_debugpoint_add(test_type, addr, size, debug_callback, &triggered);
...
static bool parse_options(int argc, FAR char *argv[], struct debug_option *opt)
{
	int cmd;
...
static int handle_cmd(int argc, FAR char *argv[])
{
	...
	if (parse_options(argc, argv, &opt)) {
		...
		if (opt.cancel && opt.type != GDB_STOPREASON_NONE && opt.addr) {
			...
			ret = gdb_debugpoint_add(opt.type, opt.addr, opt.width, debug_callback_cmd, NULL);
			if (ret < 0) {
