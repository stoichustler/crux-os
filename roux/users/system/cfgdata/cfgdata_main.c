...
static void cfgdatacmd_parse_byte_array(struct config_data_s *cfg, int argc, char *argv[])
{
	int x;
	int c;
	int count;
...
static void cfgdatacmd_set(int argc, char *argv[])
{
	...
	if (cfg.configdata != (FAR uint8_t *)argv[3] && cfg.configdata != data) {
		free(cfg.configdata);
	}
}
...
#ifdef CONFIG_MTD_CONFIG_NAMED
	snprintf(fmtstr, sizeof(fmtstr), "%%-%ds%%-6sData\n", CONFIG_MTD_CONFIG_NAME_LEN);
	printf(fmtstr, "Name", "Len");
	snprintf(fmtstr, sizeof(fmtstr), "%%-%ds%%-6d", CONFIG_MTD_CONFIG_NAME_LEN);
#else
