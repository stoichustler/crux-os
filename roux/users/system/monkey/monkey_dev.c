...
static void utouch_write(int fd, int x, int y, int touch_down)
{
	...
	MONKEY_LOG_INFO("%s at x = %d, y = %d", touch_down ? "PRESS  " : "RELEASE", x, y);
}
...
FAR struct monkey_dev_s *monkey_dev_create(FAR const char *dev_path, enum monkey_dev_type_e type)
{
	...
	MONKEY_LOG_NOTICE("open %s success, fd = %d, type: %s", dev_path, fd,
...
void monkey_dev_set_state(FAR struct monkey_dev_s *dev, FAR const struct monkey_dev_state_s *state)
{
	MONKEY_ASSERT_NULL(dev);

	switch (MONKEY_GET_DEV_TYPE(dev->type)) {
	case MONKEY_DEV_TYPE_TOUCH:
		utouch_write(dev->fd, state->data.touch.x, state->data.touch.y,
...
bool monkey_dev_get_state(FAR struct monkey_dev_s *dev, FAR struct monkey_dev_state_s *state)
{
	bool retval;
	MONKEY_ASSERT_NULL(dev);
	...
	switch (dev->type) {
	case MONKEY_DEV_TYPE_TOUCH:
		retval = touch_read(dev->fd, &state->data.touch.x, &state->data.touch.y,
							&state->data.touch.is_pressed);
		break;
