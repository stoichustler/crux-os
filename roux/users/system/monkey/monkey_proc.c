...
static FAR struct monkey_dev_s *monkey_search_dev(FAR struct monkey_s *monkey,
...
static int monkey_recorder_get_next(FAR struct monkey_s *monkey, FAR uint32_t *cur_time_stamp_p,
									...
									FAR struct monkey_dev_state_s *next_state_p)
{
	...
	if (monkey->playback_ctx.time_stamp == 0) {
		res = monkey_recorder_read(monkey->recorder, cur_state_p, cur_time_stamp_p);
	...
	res = monkey_recorder_read(monkey->recorder, next_state_p, next_time_stamp_p);
...
static bool monkey_update_uinput_playback(FAR struct monkey_s *monkey)
{
	...
	num_of_get = monkey_recorder_get_next(monkey, &cur_time_stamp, &cur_state, &next_time_stamp,
										  &next_state);
	...
	case 2:
		dev = monkey_search_dev(monkey, MONKEY_UINPUT_TYPE_MASK | cur_state.type);
...
int monkey_update(FAR struct monkey_s *monkey)
{
	...
	wait_time = monkey_random(monkey->config.period.min, monkey->config.period.max);
