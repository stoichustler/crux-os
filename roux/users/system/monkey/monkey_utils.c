...
void monkey_get_localtime_str(FAR char *str_buf, size_t buf_size)
{
	...
	snprintf(str_buf, buf_size, "%04d%02d%02d_%02d%02d%02d", 1900 + timeinfo->tm_year,
...
{
	int i;
	const int grp_len = sizeof(g_type_name_grp) / sizeof(struct monkey_port_dev_type_name_s);
	for (i = 0; i < grp_len; i++) {
