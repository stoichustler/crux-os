...
static void monkey_recorder_gen_file_path(FAR char *path_buf, size_t buf_size,
										  FAR const char *dir_path)
{
	...
	snprintf(path_buf, buf_size, "%s/" MONKEY_REC_FILE_HEAD "%s" MONKEY_REC_FILE_EXT, dir_path,
			 localtime_str_buf);
}
													 ...
													 enum monkey_recorder_mode_e mode)
{
	...
	if (mode == MONKEY_RECORDER_MODE_RECORD) {
		...
		monkey_recorder_gen_file_path(file_path, sizeof(file_path), path);
		path_ptr = file_path;
	...
	MONKEY_LOG_NOTICE("open %s success, fd = %d, mode = %d", path_ptr, fd, mode);
...
enum monkey_recorder_res_e monkey_recorder_write(FAR struct monkey_recorder_s *recorder,
												 FAR const struct monkey_dev_state_s *state)
{
	...
	case MONKEY_DEV_TYPE_TOUCH:
		len = snprintf(buffer, sizeof(buffer), MONKEY_REC_TOUCH_FMT, cur_tick, state->type,
					   state->data.touch.x, state->data.touch.y, state->data.touch.is_pressed);
		break;

	case MONKEY_DEV_TYPE_BUTTON:
		len = snprintf(buffer, sizeof(buffer), MONKEY_REC_BTN_FMT, cur_tick, state->type,
...
enum monkey_recorder_res_e monkey_recorder_read(FAR struct monkey_recorder_s *recorder,
												...
												FAR uint32_t *time_stamp)
{
	...
	case MONKEY_DEV_TYPE_TOUCH:
		converted =
			sscanf(buffer, MONKEY_REC_TOUCH_FMT, time_stamp, &dev_type, &state->data.touch.x,
				   &state->data.touch.y, &state->data.touch.is_pressed);
		if (converted != 5) {
			return MONKEY_RECORDER_RES_PARSER_ERROR;
		}
		break;
	...
	case MONKEY_DEV_TYPE_BUTTON:
		converted =
			sscanf(buffer, MONKEY_REC_BTN_FMT, time_stamp, &dev_type, &state->data.button.value);
		if (converted != 3) {
			return MONKEY_RECORDER_RES_PARSER_ERROR;
		}
		break;
...
enum monkey_recorder_res_e monkey_recorder_reset(FAR struct monkey_recorder_s *recorder)
{
	MONKEY_ASSERT_NULL(recorder);
