...
#define MONKEY_EVENT_PARAM_INIT(name)                                                              \
	do {                                                                                           \
		param->event[MONKEY_EVENT_##name].weight = MONKEY_EVENT_##name##_WEIGHT_DEFAULT;           \
...
static void show_usage(FAR const char *progname, int exitcode)
{
	...
	printf("  -t <hex-value> Device type mask: uinput = 0x%02X;"
		   ...
		   MONKEY_UINPUT_TYPE_MASK, MONKEY_DEV_TYPE_TOUCH, MONKEY_DEV_TYPE_BUTTON);
...
static int monkey_get_screen_resolution(FAR int *hor_res, FAR int *ver_res)
{
	...
	*ver_res = MONKEY_SCREEN_VER_RES_DEFAULT;
	...
	ret = ioctl(fd, MONKEY_SCREEN_GETVIDEOINFO, (unsigned long)((uintptr_t)&vinfo));
...
static FAR struct monkey_s *monkey_init(FAR const struct monkey_param_s *param)
{
	...
	MONKEY_LOG_NOTICE("Screen: %dx%d, offset: %d,%d", config.screen.hor_res, config.screen.ver_res,
					  config.screen.x_offset, config.screen.y_offset);
	MONKEY_LOG_NOTICE("Period: %" PRIu32 " ~ %" PRIu32 "ms", config.period.min, config.period.max);
	...
	for (i = 0; i < MONKEY_EVENT_LAST; i++) {
		MONKEY_LOG_NOTICE("Event %d(%s): weight=%d"
						  " duration %d ~ %dms",
						  i, monkey_event_type2name(i), config.event[i].weight,
						  config.event[i].duration_min, config.event[i].duration_max);
	}
	...
	if (MONKEY_IS_UINPUT_TYPE(param->dev_type_mask)) {
		...
		if (!monkey_set_recorder_path(monkey, CONFIG_SYSTEM_MONKEY_REC_DIR_PATH)) {
			goto failed;
		}
...
static void parse_long_commandline(int argc, FAR char **argv, int ch,
								   FAR const struct option *longopts, int longindex,
								   FAR struct monkey_param_s *param)
{
	...
	case 6:
		OPTARG_TO_RANGE(param->x_offset, param->y_offset, ",");
		break;
	...
	case 7:
		...
		if (param->running_minutes > MONKEY_RUNNING_MINUTES_MAX) {
			MONKEY_LOG_WARN("Running minutes must be less than %d", MONKEY_RUNNING_MINUTES_MAX);
...
static void parse_commandline(int argc, FAR char **argv, FAR struct monkey_param_s *param)
{
	int ch;
	int longindex = 0;
	FAR const char *optstring = "t:f:p:s:b:l:";
	const struct option longopts[] = {{"weight-click", required_argument, NULL, 0},
	...
	while ((ch = getopt_long(argc, argv, optstring, longopts, &longindex)) != ERROR) {
		switch (ch) {
		case 0:
		...
		case 'l':
			...
			param->log_level = CONSTRAIN(param->log_level, 0, MONKEY_LOG_LEVEL_LAST - 1);
			break;
	...
	while (1) {
		...
		if (param.running_minutes > 0) {
			...
			if (elaps > param.running_minutes * 60 * 1000) {
				MONKEY_LOG_WARN("Running time is over: %" PRIu32 " minutes, exit...",
