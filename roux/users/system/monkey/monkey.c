...
#define MONKEY_DEV_CREATE_MATCH(monkey, type_mask, type)                                           \
	do {                                                                                           \
		if (((type_mask)&MONKEY_DEV_TYPE_##type) == MONKEY_DEV_TYPE_##type) {                      \
			...
			dev = monkey_dev_create(MONKEY_DEV_PATH_##type, MONKEY_DEV_TYPE_##type);               \
			if (!dev) {                                                                            \
				goto failed;                                                                       \
			}                                                                                      \
...
void monkey_set_config(FAR struct monkey_s *monkey, FAR const struct monkey_config_s *config)
{
	MONKEY_ASSERT_NULL(monkey);
	MONKEY_ASSERT_NULL(config);
...
bool monkey_set_recorder_path(FAR struct monkey_s *monkey, FAR const char *path)
{
	MONKEY_ASSERT_NULL(monkey);

	if (monkey->mode == MONKEY_MODE_RECORD) {
		monkey->recorder = monkey_recorder_create(path, MONKEY_RECORDER_MODE_RECORD);
	} else if (monkey->mode == MONKEY_MODE_PLAYBACK) {
		monkey->recorder = monkey_recorder_create(path, MONKEY_RECORDER_MODE_PLAYBACK);
	} else {
