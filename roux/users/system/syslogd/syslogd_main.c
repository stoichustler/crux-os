...
int main(int argc, FAR char **argv)
{
	...
	if (background) {
		...
		if (argc > MAX_ARGS) {
			fprintf(stderr, "Cannot spawn syslogd daemon: arg count %d exceeds %d", argc, MAX_ARGS);
			return EXIT_FAILURE;
		}
	...
	for (;;) {
		...
		if (!skiplog) {
			bsent =
				sendto(sock, buffer, endpos, 0, (const struct sockaddr *)&server, sizeof(server));
			if (bsent < 0) {
