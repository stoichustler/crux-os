...
#include <fnmatch.h>
#include <sys/ioctl.h>
#include <roux/note/notectl_driver.h>
...
static int trace_cmd_start(FAR const char *name, int index, int argc, FAR char **argv,
						   int notectlfd)
{
	FAR char *endptr;
	int duration = 0;
	...
	if (index < argc) {
		...
		if (!duration || endptr == argv[index] || *endptr != '\0') {
			fprintf(stderr, "trace start: invalid argument '%s'\n", argv[index]);
			return ERROR;
		}
...
static int trace_cmd_dump(FAR const char *name, int index, int argc, FAR char **argv, int notectlfd)
{
	FAR FILE *out = stdout;
	bool changed = false;
	bool cont = false;
	...
	if (index < argc) {
		if (strcmp(argv[index], "-") != 0) {
			...
			if (out == NULL) {
				fprintf(stderr, "trace dump: cannot open '%s'\n", argv[index]);
				return ERROR;
			}
		}
	...
	if (ret < 0) {
		fprintf(stderr, "trace dump: dump failed\n");
		return ERROR;
	}
...
static int trace_cmd_cmd(FAR const char *name, int index, int argc, FAR char **argv, int notectlfd)
{
	...
	if (index >= argc) {
		...
		fprintf(stderr, "trace cmd: no argument\n");
		return ERROR;
	}
...
static int trace_cmd_mode(FAR const char *name, int index, int argc, FAR char **argv, int notectlfd)
{
	struct note_filter_named_mode_s mode;
	bool owmode;
	bool enable;
	...
	while (index < argc) {
		...
		default:
			fprintf(stderr, "trace mode: invalid option '%s'\n", argv[index]);
			return ERROR;
		}
	...
	printf(" Trace                   : %s\n",
		   (mode.mode.flag & NOTE_FILTER_MODE_FLAG_ENABLE) ? "enabled" : "disabled");
	...
	ioctl(notectlfd, NOTECTL_GETSYSCALLFILTER, (unsigned long)&filter_syscall);
	...
	printf(" Syscall trace           : %s\n",
		   mode.mode.flag & NOTE_FILTER_MODE_FLAG_SYSCALL ? "on  (+s)" : "off (-s)");
	if (mode.mode.flag & NOTE_FILTER_MODE_FLAG_SYSCALL) {
		printf("  Filtered Syscalls      : %d\n", count);
	}

	printf(" Syscall trace with args : %s\n",
		   mode.mode.flag & NOTE_FILTER_MODE_FLAG_SYSCALL_ARGS ? "on  (+a)" : "off (-a)");
#endif

#ifdef CONFIG_SCHED_INSTRUMENTATION_IRQHANDLER
	strlcpy(filter_irq.name, name, NAME_MAX);
	ioctl(notectlfd, NOTECTL_GETIRQFILTER, (unsigned long)&filter_irq.irq_mask);
	...
	printf(" IRQ trace               : %s\n",
		   mode.mode.flag & NOTE_FILTER_MODE_FLAG_IRQ ? "on  (+i)" : "off (-i)");
...
#ifdef CONFIG_SCHED_INSTRUMENTATION_SWITCH
static int trace_cmd_switch(FAR const char *name, int index, int argc, FAR char **argv,
							int notectlfd)
{
	bool enable;
	struct note_filter_named_mode_s mode;
	...
	if (index < argc) {
		if (argv[index][0] == '-' || argv[index][0] == '+') {
			enable = (argv[index++][0] == '+');
			if (enable == ((mode.mode.flag & NOTE_FILTER_MODE_FLAG_SWITCH) != 0)) {
				/* Already set */
...
#ifdef CONFIG_SCHED_INSTRUMENTATION_SYSCALL
static int trace_cmd_syscall(FAR const char *name, int index, int argc, FAR char **argv,
							 int notectlfd)
{
	bool enable;
	bool modified = false;
	...
	while (index < argc) {
		...
		for (syscallno = 0; syscallno < SYS_nsyscalls; syscallno++) {
			...
			if (enable) {
				NOTE_FILTER_SYSCALLMASK_SET(syscallno, &filter_syscall.syscall_mask);
			} else {
				NOTE_FILTER_SYSCALLMASK_CLR(syscallno, &filter_syscall.syscall_mask);
			}
		}
	...
	if (modified) {
		...
		ioctl(notectlfd, NOTECTL_SETSYSCALLFILTER, (unsigned long)&filter_syscall);
	} else {
...
#ifdef CONFIG_SCHED_INSTRUMENTATION_IRQHANDLER
static int trace_cmd_irq(FAR const char *name, int index, int argc, FAR char **argv, int notectlfd)
{
	bool enable;
	bool modified = false;
	int irqno;
	...
	while (index < argc) {
		...
		if (argv[index][1] == '*') {
			...
			if (endptr == &argv[index][1] || *endptr != '\0' || irqno >= NR_IRQS) {
				fprintf(stderr, "trace irq: invalid argument '%s'\n", argv[index]);
				return ERROR;
			}
...
#ifdef CONFIG_SCHED_INSTRUMENTATION_DUMP
static int trace_cmd_print(FAR const char *name, int index, int argc, FAR char **argv,
						   int notectlfd)
{
	bool enable;
	struct note_filter_named_mode_s mode;
	...
	if (index < argc) {
		if (argv[index][0] == '-' || argv[index][0] == '+') {
			enable = (argv[index++][0] == '+');
			if (enable == ((mode.mode.flag & NOTE_FILTER_MODE_FLAG_DUMP) != 0)) {
				/* Already set */
