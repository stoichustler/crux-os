...
static int spicmd_unrecognized(FAR struct spitool_s *spitool, int argc, FAR char **argv);
...
static int spicmd_help(FAR struct spitool_s *spitool, int argc, FAR char **argv)
{
	...
	spitool_printf(spitool,
				   "  [-b bus] is the SPI bus number (decimal).  "
				   "Default: %d Current: %d\n",
				   CONFIG_SPITOOL_MINBUS, spitool->bus);
#ifdef CONFIG_SPI_CMDDATA
	spitool_printf(spitool,
				   "  [-c 0|1] Send in command mode.  "
				   "Default: %d Current: %d\n",
	...
	spitool_printf(spitool,
				   "  [-m mode] Mode for transfer.  "
				   "Default: %d Current: %d\n",
				   CONFIG_SPITOOL_DEFMODE, spitool->mode);

	spitool_printf(spitool,
				   "  [-n CSn] chip select number.  "
				   "Default: %d Current: %" PRIu32 "\n",
				   0, spitool->csn);

	spitool_printf(spitool,
				   "  [-t devtype] Chip Select type "
				   "(see spi_devtype_e).  "
				   "Default: %d Current: %" PRIu32 "\n",
				   SPIDEVTYPE_USER, spitool->devtype);

	spitool_printf(spitool,
				   "  [-u udelay] Delay after transfer in uS.  "
				   "Default: 0 Current: %" PRIu32 "\n",
				   spitool->udelay);

	spitool_printf(spitool,
				   "  [-w width] Width of bus.  "
				   "Default: %d Current: %d\n",
				   CONFIG_SPITOOL_DEFWIDTH, spitool->width);

	spitool_printf(spitool,
				   "  [-x count] Words to exchange  "
...
static int spi_execute(FAR struct spitool_s *spitool, int argc, FAR char *argv[])
{
...
static FAR char *spi_argument(FAR struct spitool_s *spitool, int argc, FAR char *argv[],
							  FAR int *pindex)
{
	FAR char *arg;
	int index = *pindex;
...
static int spi_parse(FAR struct spitool_s *spitool, int argc, FAR char *argv[])
{
...
int main(int argc, FAR char *argv[])
{
	...
	if (g_spitool.bus < CONFIG_SPITOOL_MINBUS || g_spitool.bus > CONFIG_SPITOOL_MAXBUS) {
...
ssize_t spitool_write(FAR struct spitool_s *spitool, FAR const void *buffer, size_t nbytes)
{
	ssize_t ret;
