								  ...
								  FAR const struct netlib_conntrack_tuple_s *tuple)
{
	...
	case IPPROTO_TCP:
	case IPPROTO_UDP:
		printf("sport=%" PRIu16 " dport=%" PRIu16 " ", tuple->l4.tcp.sport, tuple->l4.tcp.dport);
		break;
	case IPPROTO_ICMP:
	case IPPROTO_ICMP6:
		printf("type=%" PRIu8 " code=%" PRIu8 " id=%" PRIu16 " ", tuple->l4.icmp.type,
			   tuple->l4.icmp.code, tuple->l4.icmp.id);
		break;
	}
}
...
static void conntrack_monitor_event(void)
{
	...
	fd = conntrack_monitor_socket(NF_NETLINK_CONNTRACK_NEW | NF_NETLINK_CONNTRACK_DESTROY);
	if (fd < 0) {
		return;
	}
...
int main(int argc, FAR char *argv[])
{
	...
	args.family = arg_str0("f", "family", "PROTO",
						   "Specify L3 (ipv4, ipv6) "
