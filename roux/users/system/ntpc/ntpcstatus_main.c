...
static void format_ntptimestamp(int64_t ts, FAR char *buf, size_t len)
{
	FAR const char *sign;
	uint64_t absts;
	...
	snprintf(buf, len, "%s%" PRIu64 ".%09" PRIu64, sign, absts >> 32,
...
int main(int argc, FAR char *argv[])
{
	...
	for (i = 0; i < status.nsamples; i++) {
		...
		ret = getnameinfo(sa, salen, hbuf, sizeof(hbuf), NULL, 0, NI_NUMERICHOST);
		if (ret == 0) {
			name = hbuf;
		} else {
		...
		format_ntptimestamp(status.samples[i].offset, offset_buf, sizeof(offset_buf));
		format_ntptimestamp(status.samples[i].delay, delay_buf, sizeof(delay_buf));
