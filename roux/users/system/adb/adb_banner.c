...
int adb_fill_connect_data(char *buf, size_t bufsize)
{
	...
	if (ret) {
		...
		len = snprintf(buf, remaining, "device:%" PRIx32 ":", *(uint32_t *)board_id);
	}
#else
	...
	remaining -= len;
	buf += len;
	len = snprintf(buf, remaining, "ro.product.name=" CONFIG_ADBD_PRODUCT_NAME ";");
	...
	remaining -= len;
	buf += len;
	len = snprintf(buf, remaining, "ro.product.model=" CONFIG_ADBD_PRODUCT_MODEL ";");
	...
	remaining -= len;
	buf += len;
	len = snprintf(buf, remaining, "ro.product.device=" CONFIG_ADBD_PRODUCT_DEVICE ";");
