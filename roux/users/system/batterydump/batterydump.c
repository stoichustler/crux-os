...
static void dump_gauge(int fd, uint32_t mask)
{
	...
	printf("mask:%" PRIx32 ", state:%d, online:%d, vol:%f, capacity:%" PRIi32
		   "%%, current:%f, temperature:%f\n",
		   mask, state, online, b16tof(vol), b16toi(cap), b16tof(current), b8tof(temp));
	return;
}
...
static void dump_monitor(int fd, uint32_t mask)
{
	...
	printf("mask:%" PRIx32 ", state:%d, online:%d, vol:%d, coulombs:%d, "
		   ...
		   mask, status, online, vol, coulomb, current.current, current.time);
	return;
}
