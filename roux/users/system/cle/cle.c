...
static char g_cmd_history[CONFIG_SYSTEM_CLE_CMD_HISTORY_LEN][CONFIG_SYSTEM_CLE_CMD_HISTORY_LINELEN];
...
static void cle_setcursor(FAR struct cle_s *priv, int16_t column)
{
	...
	len = snprintf(buffer, sizeof(buffer), off < 0 ? VT100_FMT_CURSORLF : VT100_FMT_CURSORRT,
				   off < 0 ? -off : off);
 ...
 *   than is the ROUX readline!
 *
 ****************************************************************************/

int cle_fd(FAR char *line, FAR const char *prompt, uint16_t linelen, int infd, int outfd)
{
	...
	if (priv.nchars > 1) {
		...
		g_cmd_history_head = (g_cmd_history_head + 1) % CONFIG_SYSTEM_CLE_CMD_HISTORY_LEN;

		for (i = 0; (i < priv.nchars - 1) && i < (CONFIG_SYSTEM_CLE_CMD_HISTORY_LINELEN - 1); i++) {
...
int cle(FAR char *line, FAR const char *prompt, uint16_t linelen, FAR FILE *instream,
		FAR FILE *outstream)
{
	int instream_fd;
	int outstream_fd;
