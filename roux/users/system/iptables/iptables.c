...
typedef CODE int (*iptables_command_func_t)(FAR const struct iptables_args_s *args);
...
static FAR char *iptables_addr2str(struct in_addr addr, struct in_addr msk, FAR char *buf,
								   size_t bufflen)
{
...
static void iptables_print_chain(FAR const struct ipt_replace *repl, enum nf_inet_hooks hook)
{
	...
	printf(fmt, "target", "", "prot", "", "idev", "", "odev", "", "source", "", "destination", "");

	ipt_entry_for_every(entry, head, size)
	{
		target = IPT_TARGET(entry);
		match = entry->target_offset >= sizeof(struct xt_entry_match) ? IPT_MATCH(entry) : NULL;
		printf(fmt, iptables_target2str(target), INV_FLAG_STR(entry->ip.invflags & IPT_INV_PROTO),
...
static int iptables_finish_command(FAR const struct iptables_args_s *args,
								   FAR struct ipt_replace **repl, FAR struct ipt_entry *entry)
{
	int ret;
...
static int iptables_nat_command(FAR const struct iptables_args_s *args)
{
	...
	if (args->target != NULL && strcmp(args->target, XT_MASQUERADE_TARGET)) {
		printf("Only target '" XT_MASQUERADE_TARGET "' is supported for table '" TABLE_NAME_NAT
			   "'!\n");
		return -EINVAL;
	}
...
static int iptables_filter_command(FAR const struct iptables_args_s *args)
{
	...
	if (repl == NULL) {
		printf("Failed to read table '" XT_TABLE_NAME_FILTER "' from kernel!\n");
		return -EIO;
	}
	...
	if (args->sport != NULL || args->dport != NULL) {
		...
		entry = netlib_ipt_filter_entry(args->target, args->verdict, args->protocol);
		if (entry == NULL) {
	...
	if (args->saddr != NULL) {
		ret = iptables_parse_ip(args->saddr, &entry->ip.src, &entry->ip.smsk, AF_INET);
		if (ret < 0) {
	...
	if (args->daddr != NULL) {
		ret = iptables_parse_ip(args->daddr, &entry->ip.dst, &entry->ip.dmsk, AF_INET);
		if (ret < 0) {
