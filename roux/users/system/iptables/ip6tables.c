...
typedef CODE int (*ip6tables_command_func_t)(FAR const struct iptables_args_s *args);
...
static FAR char *ip6tables_addr2str(net_ipv6addr_t addr, net_ipv6addr_t msk, FAR char *buf,
									size_t bufflen)
{
...
static void ip6tables_print_chain(FAR const struct ip6t_replace *repl, enum nf_inet_hooks hook)
{
	...
	{
		target = IP6T_TARGET(entry);
		match = entry->target_offset >= sizeof(struct xt_entry_match) ? IP6T_MATCH(entry) : NULL;
		printf(fmt, iptables_target2str(target),
			   ...
			   ip6tables_addr2str(entry->ipv6.src.s6_addr16, entry->ipv6.smsk.s6_addr16, src,
								  sizeof(src)),
			   INV_FLAG_STR(entry->ipv6.invflags & IP6T_INV_DSTIP),
			   ip6tables_addr2str(entry->ipv6.dst.s6_addr16, entry->ipv6.dmsk.s6_addr16, dst,
								  sizeof(dst)),
...
static int ip6tables_finish_command(FAR const struct iptables_args_s *args,
									FAR struct ip6t_replace **repl, FAR struct ip6t_entry *entry)
{
	int ret;
...
static int ip6tables_filter_command(FAR const struct iptables_args_s *args)
{
	...
	if (args->sport != NULL || args->dport != NULL) {
		...
		entry = netlib_ip6t_filter_entry(args->target, args->verdict, args->protocol);
		if (entry == NULL) {
	...
	if (args->saddr != NULL) {
		ret = iptables_parse_ip(args->saddr, &entry->ipv6.src, &entry->ipv6.smsk, AF_INET6);
		if (ret < 0) {
	...
	if (args->daddr != NULL) {
		ret = iptables_parse_ip(args->daddr, &entry->ipv6.dst, &entry->ipv6.dmsk, AF_INET6);
		if (ret < 0) {
