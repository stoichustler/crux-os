...
#include <roux/net/netfilter/ip_tables.h>
#include <roux/net/netfilter/ip6_tables.h>
...
static FAR const char *g_hooknames[] = {"PREROUTING", "INPUT", "FORWARD", "OUTPUT", "POSTROUTING"};
...
static bool iptables_parse_cmd(FAR const char *str, FAR enum iptables_command_e *command)
{
...
static FAR const char *iptables_parse_target(FAR const char *str, FAR int8_t *verdict)
{
...
int iptables_parse(FAR struct iptables_args_s *args, int argc, FAR char *argv[])
{
	bool inv = false;
	int i;
	...
	for (i = 1; i < argc; i++) {
		...
		if (strcmp(argv[i], "--sport") == 0 || strcmp(argv[i], "--source-port") == 0) {
			if (++i >= argc) {
		...
		if (strcmp(argv[i], "--dport") == 0 || strcmp(argv[i], "--destination-port") == 0) {
			if (++i >= argc) {
		...
		if (strcmp(argv[i], "-i") == 0 || strcmp(argv[i], "--in-interface") == 0) {
			if (++i >= argc) {
		...
		if (strcmp(argv[i], "-o") == 0 || strcmp(argv[i], "--out-interface") == 0) {
			if (++i >= argc) {
		...
		if (strcmp(argv[i], "--icmp-type") == 0 || strcmp(argv[i], "--icmpv6-type") == 0) {
			if (++i >= argc) {
...
int iptables_parse_ip(FAR const char *str, FAR void *addr, FAR void *mask, uint8_t family)
{
...
void iptables_showusage(FAR const char *progname)
{
	...
	printf(fmt, "", "--delete", "-D", "chain [rulenum]", "Delete matching rule from chain");
	printf(fmt, "", "--policy", "-P", "chain target", "Set policy for chain to target");
	printf(fmt, "", "--flush", "-F", "[chain]", "Delete all rules in chain or all chains");
	printf(fmt, "", "--list", "-L", "[chain]", "List all rules in chain or all chains");

	printf("\nOptions:\n");
	printf(fmt, "   ", "--table", "-t", "table", "Table to manipulate (default: filter)");
	printf(fmt, "   ", "--jump", "-j", "target", "Target for rule");
	printf(fmt, "[!]", "--in-interface", "-i", "dev", "Input network interface name");
	printf(fmt, "[!]", "--out-interface", "-o", "dev", "Output network interface name");
	printf(fmt, "[!]", "--source", "-s", "address[/mask]", "Source address");
	printf(fmt, "[!]", "--destination", "-d", "address[/mask]", "Destination address");
	printf(fmt, "[!]", "--protocol", "-p", "proto", "Protocol (tcp, udp, icmp, esp, all)");
...
FAR const char *iptables_match2str(FAR const struct xt_entry_match *match)
{
	...
	if (strcmp(match->u.user.name, XT_MATCH_NAME_TCP) == 0) {
		...
		snprintf(s_buf, MATCH_BUFSIZE, "tcp spts:%s%u:%u dpts:%s%u:%u",
				 INV_FLAG_STR(tcp->invflags & XT_TCP_INV_SRCPT), tcp->spts[0], tcp->spts[1],
				 INV_FLAG_STR(tcp->invflags & XT_TCP_INV_DSTPT), tcp->dpts[0], tcp->dpts[1]);
	} else if (strcmp(match->u.user.name, XT_MATCH_NAME_UDP) == 0) {
		FAR struct xt_udp *udp = (FAR struct xt_udp *)(match + 1);

		snprintf(s_buf, MATCH_BUFSIZE, "udp spts:%s%u:%u dpts:%s%u:%u",
				 INV_FLAG_STR(udp->invflags & XT_UDP_INV_SRCPT), udp->spts[0], udp->spts[1],
				 INV_FLAG_STR(udp->invflags & XT_UDP_INV_DSTPT), udp->dpts[0], udp->dpts[1]);
	} else if (strcmp(match->u.user.name, XT_MATCH_NAME_ICMP) == 0) {
		FAR struct ipt_icmp *icmp = (FAR struct ipt_icmp *)(match + 1);

		snprintf(s_buf, MATCH_BUFSIZE, "icmp %stype %u",
				 INV_FLAG_STR(icmp->invflags & IPT_ICMP_INV), icmp->type);
	} else if (strcmp(match->u.user.name, XT_MATCH_NAME_ICMP6) == 0) {
		FAR struct ip6t_icmp *icmp6 = (FAR struct ip6t_icmp *)(match + 1);

		snprintf(s_buf, MATCH_BUFSIZE, "ipv6-icmp %stype %u",
				 INV_FLAG_STR(icmp6->invflags & IP6T_ICMP_INV), icmp6->type);
	} else {
