...
static int show_image(FAR struct nxcamera_s *pcam, FAR v4l2_buffer_t *buf)
{
	...
	if (pcam->display_vinfo.fmt == FB_FMT_RGB32) {
		return ConvertToARGB(
			pcam->bufs[buf->index], pcam->buf_sizes[buf->index], pcam->display_pinfo.fbmem,
			pcam->display_pinfo.stride, 0, 0, pcam->fmt.fmt.pix.width, pcam->fmt.fmt.pix.height,
			pcam->fmt.fmt.pix.width, pcam->fmt.fmt.pix.height, 0, pcam->fmt.fmt.pix.pixelformat);
	} else if (pcam->display_vinfo.fmt == FB_FMT_RGB16_565) {
		if (pcam->fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_YUV420) {
			return ConvertFromI420(
				pcam->bufs[buf->index], pcam->fmt.fmt.pix.width,
				&pcam->bufs[buf->index][pcam->fmt.fmt.pix.width * pcam->fmt.fmt.pix.height],
				pcam->fmt.fmt.pix.width / 2,
				&pcam->bufs[buf->index][pcam->fmt.fmt.pix.width * pcam->fmt.fmt.pix.height * 5 / 4],
				pcam->fmt.fmt.pix.width / 2, pcam->display_pinfo.fbmem, pcam->display_pinfo.stride,
				pcam->fmt.fmt.pix.width, pcam->fmt.fmt.pix.height, V4L2_PIX_FMT_RGB565);
		} else {
			int ret;
			FAR uint8_t *dst = malloc(pcam->fmt.fmt.pix.width * pcam->fmt.fmt.pix.height * 3 / 2);
			if (!dst) {
				return -ENOMEM;
			}

			ret = ConvertToI420(pcam->bufs[buf->index], pcam->buf_sizes[buf->index], dst,
								pcam->fmt.fmt.pix.width,
								&dst[pcam->fmt.fmt.pix.width * pcam->fmt.fmt.pix.height],
								pcam->fmt.fmt.pix.width / 2,
								&dst[pcam->fmt.fmt.pix.width * pcam->fmt.fmt.pix.height * 5 / 4],
								pcam->fmt.fmt.pix.width / 2, 0, 0, pcam->fmt.fmt.pix.width,
								pcam->fmt.fmt.pix.height, pcam->fmt.fmt.pix.width,
								pcam->fmt.fmt.pix.height, 0, pcam->fmt.fmt.pix.pixelformat);
			if (ret < 0) {
				goto err;
			}

			ret = ConvertFromI420(dst, pcam->fmt.fmt.pix.width,
								  &dst[pcam->fmt.fmt.pix.width * pcam->fmt.fmt.pix.height],
								  pcam->fmt.fmt.pix.width / 2,
								  &dst[pcam->fmt.fmt.pix.width * pcam->fmt.fmt.pix.height * 5 / 4],
								  pcam->fmt.fmt.pix.width / 2, pcam->display_pinfo.fbmem,
								  pcam->display_pinfo.stride, pcam->fmt.fmt.pix.width,
								  pcam->fmt.fmt.pix.height, V4L2_PIX_FMT_RGB565);
			if (ret < 0) {
				goto err;
			}
...
static int nxcamera_opendevice(FAR struct nxcamera_s *pcam)
{
	...
	if (pcam->capturedev[0] != '\0') {
		...
		if (pcam->displaydev[0] != '\0') {
			...
			errcode = ioctl(pcam->display_fd, FBIOGET_PLANEINFO, ((uintptr_t)&pcam->display_pinfo));

			if (errcode == OK) {
				pcam->display_pinfo.fbmem =
					mmap(NULL, pcam->display_pinfo.fblen, PROT_READ | PROT_WRITE,
						 MAP_SHARED | MAP_FILE, pcam->display_fd, 0);
			}
...
int nxcamera_setdevice(FAR struct nxcamera_s *pcam, FAR const char *device)
{
	int temp_fd;
...
int nxcamera_setfb(FAR struct nxcamera_s *pcam, FAR const char *device)
{
	...
	if (ioctl(temp_fd, FBIOGET_VIDEOINFO, (uintptr_t)&pcam->display_vinfo) != OK) {
		/* Not an Video device! */
...
int nxcamera_setfile(FAR struct nxcamera_s *pcam, FAR const char *pfile, bool isimage)
{
	int temp_fd;
...
int nxcamera_stop(FAR struct nxcamera_s *pcam)
{
	...
	term_msg.u.data = 0;
	mq_send(pcam->mq, (FAR const char *)&term_msg, sizeof(term_msg), CONFIG_NXCAMERA_MSG_PRIO);
...
int nxcamera_stream(FAR struct nxcamera_s *pcam, uint16_t width, uint16_t height,
					uint32_t framerate, uint32_t format)
{
	...
	for (i = 0; i < req.count; i++) {
		...
		pcam->bufs[i] = mmap(NULL, buf.length, PROT_READ | PROT_WRITE, MAP_SHARED, pcam->capture_fd,
							 buf.m.offset);
	...
	pcam->mq = mq_open(pcam->mqname, O_RDWR | O_CREAT | O_NONBLOCK, 0644, &attr);
	if (pcam->mq == (mqd_t)-1) {
	...
	ret = pthread_create(&pcam->loop_id, &tattr, nxcamera_loopthread, (pthread_addr_t)pcam);
