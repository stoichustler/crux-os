...
static const struct nxcamera_cmd_s g_nxcamera_cmds[] = {
	{"input", "videodev", nxcamera_cmd_input,
	 NXCAMERA_HELP_TEXT("Specify a preferred capture device")},
	{"output", "fbdev|filename", nxcamera_cmd_output,
	 NXCAMERA_HELP_TEXT("Specify a display device or filename")},
#ifdef CONFIG_NXCAMERA_INCLUDE_HELP
	{"h", "", nxcamera_cmd_help, NXCAMERA_HELP_TEXT("Display help for commands")},
	{"help", "", nxcamera_cmd_help, NXCAMERA_HELP_TEXT("Display help for commands")},
#endif
	{"stream", "width height framerate format", nxcamera_cmd_stream,
	 NXCAMERA_HELP_TEXT("Video stream test (format is fourcc)")},
	{"stop", "", nxcamera_cmd_stop, NXCAMERA_HELP_TEXT("Stop stream")},
	{"q", "", nxcamera_cmd_quit, NXCAMERA_HELP_TEXT("Exit NxCamera")},
	{"quit", "", nxcamera_cmd_quit, NXCAMERA_HELP_TEXT("Exit NxCamera")}};

static const int g_nxcamera_cmd_count = sizeof(g_nxcamera_cmds) / sizeof(struct nxcamera_cmd_s);
...
static int nxcamera_cmd_stream(FAR struct nxcamera_s *pcam, FAR char *parg)
{
	...
	sscanf(parg, "%" SCNu16 " %" SCNu16 " %" SCNu32 " %4s", &width, &height, &framerate, cc);
...
static int nxcamera_cmd_help(FAR struct nxcamera_s *pcam, FAR char *parg)
{
	...
	for (x = 0; x < g_nxcamera_cmd_count; x++) {
		len = strlen(g_nxcamera_cmds[x].cmd) + strlen(g_nxcamera_cmds[x].arghelp);
		if (len > maxlen) {
			maxlen = len;
		}
	}
	...
	for (x = 0; x < g_nxcamera_cmd_count; x++) {
		...
		len = maxlen - (strlen(g_nxcamera_cmds[x].cmd) + strlen(g_nxcamera_cmds[x].arghelp));
		for (c = 0; c < len; c++) {
			printf(" ");
		}
 ...
 *   tester that uses the ROUX video system to capture and then display
