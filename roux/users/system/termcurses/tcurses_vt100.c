...
static int tcurses_vt100_move(FAR struct termcurses_s *dev, int type, int col, int row);
static int tcurses_vt100_getwinsize(FAR struct termcurses_s *dev, FAR struct winsize *winsz);
static int tcurses_vt100_setcolors(FAR struct termcurses_s *dev,
								   FAR struct termcurses_colors_s *colors);
static int tcurses_vt100_setattributes(FAR struct termcurses_s *dev, unsigned long attrib);
...
static const struct termcurses_ops_s g_vt100_ops = {
	...
	tcurses_vt100_clear,	  tcurses_vt100_move,		   tcurses_vt100_getwinsize,
	tcurses_vt100_setcolors,  tcurses_vt100_setattributes, tcurses_vt100_getkeycode,
	tcurses_vt100_checkkey,	  tcurses_vt100_terminate};
...
static const int g_key_modifier_count = sizeof(g_key_modifiers) / (sizeof(uint16_t) * 2);
								   ...
								   FAR struct termcurses_colors_s *colors)
{
	...
	if ((colors->color_mask & TCURS_COLOR_FG) != 0) {
		snprintf(str, sizeof(str), g_setfgcolor,
				 tcurses_vt100_getcolorindex(colors->fg_red, colors->fg_green, colors->fg_blue));
		ret = write(fd, str, strlen(str));
	}
	...
	if ((colors->color_mask & TCURS_COLOR_BG) != 0) {
		...
		snprintf(str, sizeof(str), g_setbgcolor,
				 tcurses_vt100_getcolorindex(colors->bg_red, colors->bg_green, colors->bg_blue));
		ret = write(fd, str, strlen(str));
	}
									...
									FAR int *keymodifiers)
{
	...
	char buildkey[16];
	int keybuildcount;
	static const char modtable[7] = {PDC_KEY_MODIFIER_SHIFT,
...
static int tcurses_vt100_terminate(FAR struct termcurses_s *dev)
{
	...
	if (isatty(priv->in_fd)) {
		if ((priv->lflag & (ECHO | ICANON)) && tcgetattr(priv->in_fd, &cfg) == 0) {
