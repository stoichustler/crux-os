...
static int i2ctool_dump(FAR struct i2ctool_s *i2ctool, int fd, uint8_t regaddr, FAR uint8_t *buf,
						int nbytes)
{
...
int i2ccmd_dump(FAR struct i2ctool_s *i2ctool, int argc, FAR char **argv)
{
	...
	if (ret == OK) {
		i2ctool_printf(i2ctool, "DUMP Bus: %d Addr: %02x Subaddr: ", i2ctool->bus, i2ctool->addr);
