...
static FAR struct listen_object_s *listener_add_object(FAR struct list_node *objlist,
													   FAR struct nxsensor_info_s *info, int fd,
													   int chanid, FAR char *chname)
{
	FAR struct listen_object_s *tmp;
...
static int sensorscope_chinfo(FAR char *path, FAR struct nxsensor_info_s **info)
{
	int i;
	...
	while ((entry = readdir(dir)) != NULL) {
		...
		if (ret != OK) {
			...
			if (fd < 0) {
				...
				obj =
					listener_add_object(&envp->objlist, info, fd, chanid, basename(entry->d_name));
				if (obj == NULL) {
				...
				nxscope_chan_init(&envp->nxs, obj->chanid, obj->chname, u.u8, obj->info->dim, 0);
