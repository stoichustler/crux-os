...
#define viinfo(format, ...) syslog(LOG_DEBUG, EXTRA_FMT format EXTRA_ARG, ##__VA_ARGS__)
#else
#define viinfo(x...)
#endif
...
static void vi_shrinkpos(FAR struct vi_s *vi, off_t delpos, size_t delsize, FAR off_t *pos);
...
static void vi_windowpos(FAR struct vi_s *vi, off_t start, off_t end, uint16_t *pcolumn,
						 off_t *ppos);
...
static void vi_yanktext(FAR struct vi_s *vi, off_t start, off_t end, bool yankcharmode,
						bool del_after_yank);
...
static void vi_printf(FAR struct vi_s *vi, FAR const char *prefix, FAR const char *fmt, ...)
{
	...
	len = prefix ? snprintf(vi->scratch, sizeof(vi->scratch), "%s", prefix) : 0;
...
static void vi_shrinkpos(FAR struct vi_s *vi, off_t delpos, size_t delsize, FAR off_t *pos)
{
	...
	if (*pos >= vi->textsize && vi->mode != MODE_INSERT && vi->mode != MODE_REPLACE) {
		*pos = vi->textsize - 1;
	}
...
static bool vi_savetext(FAR struct vi_s *vi, FAR const char *filename, off_t pos, size_t size)
{
	...
	len = snprintf(vi->scratch, sizeof(vi->scratch), "%ldC written", nwritten);
						 ...
						 off_t *ppos)
{
	...
	for (pos = start, column = 0; pos < end && (column < vi->reqcolumn || vi->updatereqcol);
		 pos++) {
		...
		if (vi->text[pos] == '\n') {
			...
			if (vi->mode != MODE_INSERT && vi->mode != MODE_REPLACE && pos != start) {
				pos--;
				column--;
			}
...
static void vi_scrollcheck(FAR struct vi_s *vi)
{
	...
	if (vi->winpos > vi->prevpos) {
		...
		for (nlines = 0, pos = vi->prevpos; pos != vi->winpos && nlines < vi->display.row - 1;
			 nlines++) {
...
static void vi_gotofirstnonwhite(FAR struct vi_s *vi)
{
	...
	while (vi->curpos <= vi->textsize &&
		   (vi->text[vi->curpos] == ' ' || vi->text[vi->curpos] == '\t')) {
		vi->curpos++;
	}
}
						...
						bool del_after_yank)
{
	...
	if (vi->yank) {
		...
		if (alloc > vi->yankalloc || (alloc == CONFIG_SYSTEM_VI_YANK_THRESHOLD &&
...
static void vi_yank(FAR struct vi_s *vi, bool del_after_yank)
{
	...
	yank_end = end;
	if (del_after_yank && end == textsize - 1 && start != end && vi->text[end] == '\n') {
		yank_end--;
		pos_increment = 1;
	}
	...
	if ((end > 0 && start == end && end == vi->textsize - 1 && vi->text[end - 1] == '\n') ||
...
static void vi_paste(FAR struct vi_s *vi, bool paste_before)
{
	...
	while (count > 0) {
		if (vi->yankcharmode) {
			...
			if (vi->text[vi->curpos] == '\n' || vi->curpos == vi->textsize || paste_before) {
				pos = vi->curpos;
			} else {
				pos = vi->curpos + 1;
			}
...
static void vi_join(FAR struct vi_s *vi)
{
	...
	while ((vi->text[end] == ' ' || vi->text[end] == '\t') && end < vi->textsize) {
		end++;
	}
...
static off_t vi_findnextword(FAR struct vi_s *vi)
{
	...
	for (; pos < vi->textsize; pos++) {
		...
		if (srch_type == VI_CHAR_SPACE && pos_type != VI_CHAR_SPACE) {
			break;
		}

		/* Test for next punctuation if search type is alpha */

		if (srch_type == VI_CHAR_ALPHA && pos_type == VI_CHAR_PUNCT) {
			break;
		}

		/* Test for next alpha if search type is punctuation */

		if (srch_type == VI_CHAR_PUNCT && pos_type == VI_CHAR_ALPHA) {
			break;
		}
...
static void vi_gotonextword(FAR struct vi_s *vi)
{
	...
	if (vi->yankarm || vi->delarm || vi->chgarm) {
		...
		while (
			(vi->text[pos - 1] == ' ' || vi->text[pos - 1] == '\t' || vi->text[pos - 1] == '\n') &&
			pos > start) {
...
static off_t vi_findprevword(FAR struct vi_s *vi)
{
	...
	if (srch_type == pos_type) {
		...
		if ((srch_type != VI_CHAR_SPACE && srch_type != VI_CHAR_CRLF) || pos == 0) {
			return pos;
		}
	...
	while (vi->mode == MODE_COMMAND) {
		...
		case KEY_CMDMODE_REPEAT: /* Repeat the last command */
		{
			if (vi->cmdcount < CMD_BUFSIZE) {
				vi->cmdindex = 0;
				vi->cmdrepeat = true;
				vi->value = vi->value > 0 ? vi->value : vi->repeatvalue > 0 ? vi->repeatvalue : 1;
				preserve = true;
			} else {
				VI_BEL(vi);
			}
	...
	while (vi->mode == MODE_INSERT || vi->mode == MODE_REPLACE) {
		...
		if (isprint(ch) || ch == '\t') {
			...
			if (vi->cursor.column + 1 < vi->display.column && ch != '\t' &&
				(vi->curpos + 1 == vi->textsize || vi->text[vi->curpos + 1] == '\n')) {
				vi_putch(vi, ch);
			} else {
				vi->redrawline = true;
			}
