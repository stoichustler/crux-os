...
static int zmr_parsefilename(FAR struct zmr_state_s *pzmr, FAR const uint8_t *namptr);
static int zmr_openfile(FAR struct zmr_state_s *pzmr, uint32_t crc);
static int zmr_fileerror(FAR struct zmr_state_s *pzmr, uint8_t type, uint32_t data);
...
static const struct zm_transition_s g_zmr_start[] = {
	{ZME_SINIT, false, ZMR_INITWAIT, zmr_zsinit},	{ZME_FILE, false, ZMR_FILEINFO, zmr_zfile},
	{ZME_RQINIT, false, ZMR_START, zmr_zrinit},		{ZME_FIN, true, ZMR_FINISH, zmr_zfin},
	{ZME_NAK, true, ZMR_START, zmr_zrinit},			{ZME_FREECNT, false, ZMR_START, zmr_freecnt},
	{ZME_COMMAND, false, ZMR_COMMAND, zmr_cmddata}, {ZME_STDERR, false, ZMR_MESSAGE, zmr_message},
	{ZME_TIMEOUT, false, ZMR_START, zmr_startto},	{ZME_ERROR, false, ZMR_START, zmr_error}};
...
static const struct zm_transition_s g_zmr_crcwait[] = {
	{ZME_CRC, false, ZMR_READREADY, zmr_zcrc},	   {ZME_NAK, false, ZMR_CRCWAIT, zmr_nakcrc},
	{ZME_RQINIT, true, ZMR_START, zmr_zrinit},	   {ZME_FIN, true, ZMR_FINISH, zmr_zfin},
	{ZME_TIMEOUT, false, ZMR_CRCWAIT, zmr_fileto}, {ZME_ERROR, false, ZMR_CRCWAIT, zmr_error}};
...
static const struct zm_transition_s g_zmr_readready[] = {
	{ZME_DATA, false, ZMR_READING, zmr_zdata},		 {ZME_NAK, false, ZMR_READREADY, zmr_badrpos},
	{ZME_EOF, false, ZMR_START, zmr_zeof},			 {ZME_RQINIT, true, ZMR_START, zmr_zrinit},
	{ZME_FILE, false, ZMR_READREADY, zmr_badrpos},	 {ZME_FIN, true, ZMR_FINISH, zmr_zfin},
	{ZME_TIMEOUT, false, ZMR_READREADY, zmr_fileto}, {ZME_ERROR, false, ZMR_READREADY, zmr_error}};

/* Events handled in the state ZMR_READING - Reading data */

static const struct zm_transition_s g_zmr_reading[] = {
	{ZME_RQINIT, true, ZMR_START, zmr_zrinit},		  {ZME_FILE, false, ZMR_FILEINFO, zmr_zfile},
	{ZME_NAK, true, ZMR_READREADY, zmr_badrpos},	  {ZME_FIN, true, ZMR_FINISH, zmr_zfin},
	{ZME_DATA, false, ZMR_READING, zmr_zdata},		  {ZME_EOF, true, ZMR_START, zmr_zeof},
	{ZME_DATARCVD, false, ZMR_READING, zmr_filedata}, {ZME_TIMEOUT, false, ZMR_READING, zmr_fileto},
...
static const struct zm_transition_s g_zmr_finish[] = {
	{ZME_RQINIT, true, ZMR_START, zmr_zrinit},	  {ZME_FILE, true, ZMR_FILEINFO, zmr_zfile},
	{ZME_NAK, true, ZMR_FINISH, zmr_zfin},		  {ZME_FIN, true, ZMR_FINISH, zmr_zfin},
	{ZME_TIMEOUT, false, ZMR_READING, zmr_finto}, {ZME_OO, false, ZMR_READING, zmr_oo},
...
static struct zm_transition_s g_zmr_message[] = {{ZME_DATARCVD, false, ZMR_MESSAGE, zmr_zstderr},
...
static struct zm_transition_s g_zmr_done[] = {{ZME_TIMEOUT, false, ZMR_DONE, zmr_doneto},
...
static int zmr_zsrintdata(FAR struct zm_state_s *pzm)
{
	...
	zmdbg("PSTATE %d:%d->%d:%d. Received the rest of the ZSINIT packet\n", pzm->pstate,
		  pzm->psubstate, PSTATE_IDLE, PIDLE_ZPAD);
...
static int zmr_filename(FAR struct zm_state_s *pzm)
{
	...
	sscanf((FAR char *)pktptr, "%lu %lo %o %o %d %lu %d", &filesize, &timestamp, &mode, &serialno,
		   &fremaining, &bremaining, &filetype);
...
static int zmr_filedata(FAR struct zm_state_s *pzm)
{
	...
	if (pzm->pkttype == ZCRCE || pzm->pkttype == ZCRCW) {
		zmdbg("PSTATE %d:%d->%d:%d: ZCRCE|ZCRCW\n", pzm->pstate, pzm->psubstate, PSTATE_IDLE,
			  PIDLE_ZPAD);
		zmdbg("ZMR_STATE %d->%d\n", pzm->state, ZMR_READREADY);
...
static int zmr_parsefilename(FAR struct zmr_state_s *pzmr, FAR const uint8_t *namptr)
{
	...
	zmdbg("File %s f0=%02x, f1=%-2x, exists=%d, size=%ld/%ld\n", pzmr->filename, f0, f1, exists,
		  (long)buf.st_size, (long)pzmr->filesize);
	...
	case ZMCRC: /* Transfer if different CRC or length */
	{
		...
		if (exists && pzmr->filesize == buf.st_size && pzmr->crc == crc) {
	...
	case ZMNEW: /* Transfer if source is newer */
	{
		...
		if (exists && pzmr->timestamp <= buf.st_mtime) {
	...
	case ZMCHNG: /* Change filename if destination exists */
	{
		...
		if (exists) {
			...
			while (exists) {
				...
				ret = asprintf(&candidate, "%s_%" PRId32, pzmr->filename, ++uniqno);
				if (ret < 0) {
...
static int zmr_openfile(FAR struct zmr_state_s *pzmr, uint32_t crc)
{
	...
	if (pzmr->outfd < 0) {
		...
		pzmr->outfd = open((FAR char *)pzmr->filename, O_WRONLY | O_CREAT | O_TRUNC, 0644);
		if (pzmr->outfd < 0) {
