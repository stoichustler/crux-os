...
const uint8_t g_canistr[CANISTR_SIZE] = {
	...
	ASCII_CAN, ASCII_CAN, ASCII_CAN, ASCII_CAN, ASCII_CAN, ASCII_CAN, ASCII_CAN, ASCII_CAN,

	/* Ten backspace characters */

	ASCII_BS, ASCII_BS, ASCII_BS, ASCII_BS, ASCII_BS, ASCII_BS, ASCII_BS, ASCII_BS, ASCII_BS,
	ASCII_BS};
...
FAR uint8_t *zm_putzdle(FAR struct zm_state_s *pzm, FAR uint8_t *buffer, uint8_t ch)
{
	uint8_t ch7 = ch & 0x7f;
	...
	if (ch == ZDLE || ch7 == ASCII_DLE || ch7 == ASCII_DC1 || ch7 == ASCII_DC3 || ch7 == ASCII_GS ||
		(ch7 == '\r' && (pzm->flags & ZM_FLAG_ATSIGN) != 0) ||
		(ch7 < ' ' && (pzm->flags & ZM_FLAG_ESCCTRL) != 0) || ch7 == ASCII_DEL || ch == 0xff) {
...
int zm_senddata(FAR struct zm_state_s *pzm, FAR const uint8_t *buffer, size_t buflen)
{
...
int zm_sendhexhdr(FAR struct zm_state_s *pzm, int type, FAR const uint8_t *buffer)
{
	FAR uint8_t *ptr;
	ssize_t nwritten;
	uint16_t crc;
...
int zm_sendbin16hdr(FAR struct zm_state_s *pzm, int type, FAR const uint8_t *buffer)
{
	FAR uint8_t *ptr;
	ssize_t nwritten;
	uint16_t crc;
...
int zm_sendbin32hdr(FAR struct zm_state_s *pzm, int type, FAR const uint8_t *buffer)
{
	FAR uint8_t *ptr;
	ssize_t nwritten;
	uint32_t crc;
...
int zm_sendbinhdr(FAR struct zm_state_s *pzm, int type, FAR const uint8_t *buffer)
{
	if ((pzm->flags & ZM_FLAG_CRC32) == 0) {
