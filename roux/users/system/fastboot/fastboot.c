...
static void fastboot_getvar(FAR struct fastboot_ctx_s *ctx, FAR const char *arg);
static void fastboot_download(FAR struct fastboot_ctx_s *ctx, FAR const char *arg);
static void fastboot_erase(FAR struct fastboot_ctx_s *ctx, FAR const char *arg);
static void fastboot_flash(FAR struct fastboot_ctx_s *ctx, FAR const char *arg);
static void fastboot_reboot(FAR struct fastboot_ctx_s *ctx, FAR const char *arg);
static void fastboot_reboot_bootloader(FAR struct fastboot_ctx_s *ctx, FAR const char *arg);
static void fastboot_oem(FAR struct fastboot_ctx_s *ctx, FAR const char *arg);
static void fastboot_upload(FAR struct fastboot_ctx_s *ctx, FAR const char *arg);

static void fastboot_memdump(FAR struct fastboot_ctx_s *ctx, FAR const char *arg);
static void fastboot_filedump(FAR struct fastboot_ctx_s *ctx, FAR const char *arg);
#ifdef CONFIG_SYSTEM_FASTBOOTD_SHELL
static void fastboot_shell(FAR struct fastboot_ctx_s *ctx, FAR const char *arg);
#endif
...
static ssize_t fastboot_usbdev_read(FAR struct fastboot_ctx_s *ctx, FAR void *buf, size_t len);
static int fastboot_usbdev_write(FAR struct fastboot_ctx_s *ctx, FAR const void *buf, size_t len);
#endif
...
static ssize_t fastboot_tcp_read(FAR struct fastboot_ctx_s *ctx, FAR void *buf, size_t len);
static int fastboot_tcp_write(FAR struct fastboot_ctx_s *ctx, FAR const void *buf, size_t len);
#endif
...
static void fastboot_ack(FAR struct fastboot_ctx_s *ctx, FAR const char *code,
...
static void fastboot_fail(FAR struct fastboot_ctx_s *ctx, FAR const char *fmt, ...)
{
...
static void fastboot_okay(FAR struct fastboot_ctx_s *ctx, FAR const char *info)
{
...
static int fastboot_flash_write(int fd, off_t offset, FAR void *data, size_t size)
{
	int ret;
...
static int ffastboot_flash_fill(int fd, off_t offset, uint32_t fill_data, uint32_t blk_sz,
								uint32_t blk_num)
{
	FAR void *buffer;
	int ret = OK;
...
static int fastboot_flash_program(FAR struct fastboot_ctx_s *ctx, int fd)
{
	...
	if (sparse->magic != FASTBOOT_SPARSE_MAGIC) {
		ret = fastboot_flash_write(fd, 0, ctx->download_buffer, ctx->download_size);
		goto end;
	}
	...
	while (chunk_ptr < end_ptr && chunk_num--) {
		FAR struct fastboot_chunk_header_s *chunk = (FAR struct fastboot_chunk_header_s *)chunk_ptr;
		...
		case FASTBOOT_CHUNK_RAW: {
			...
			ret = fastboot_flash_write(fd, ctx->download_offset, chunk_ptr, chunk_size);
			if (ret < 0) {
				goto end;
			}
		...
		case FASTBOOT_CHUNK_FILL: {
			...
			uint32_t chunk_size = chunk->chunk_sz * sparse->blk_sz;
			ret = ffastboot_flash_fill(fd, ctx->download_offset, fill_data, sparse->blk_sz,
									   chunk->chunk_sz);
			if (ret < 0) {
				goto end;
			}
...
{
	return ctx->ops->write(ctx, ctx->upload_param.u.mem.addr, ctx->upload_param.size);
}
...
static void fastboot_memdump_region(memdump_print_t memprint, FAR void *priv)
{
	...
	for (index = 0; index < nitems(g_memory_region); index++) {
		snprintf(response, sizeof(response), "fastboot oem memdump 0x%" PRIxPTR " 0x%" PRIxPTR "\n",
				 g_memory_region[index].start,
				 g_memory_region[index].end - g_memory_region[index].start);
		memprint(priv, response);
		snprintf(response, sizeof(response), "fastboot get_staged 0x%" PRIxPTR ".bin\n",
				 g_memory_region[index].start);
		memprint(priv, response);
	}
#endif
...
static void fastboot_memdump_response(FAR void *priv, FAR const char *response)
{
...
{
	if (!arg ||
		sscanf(arg, "%p %zx", &ctx->upload_param.u.mem.addr, &ctx->upload_param.size) != 2) {
...
static int fastboot_filedump_upload(FAR struct fastboot_ctx_s *ctx)
{
	...
	while (size > 0) {
		ssize_t nread = fastboot_read(fd, ctx->download_buffer, MIN(size, ctx->download_max));
		if (nread == 0) {
			break;
		} else if (nread < 0 || ctx->ops->write(ctx, ctx->download_buffer, nread) < 0) {
...
static void fastboot_command_loop(FAR struct fastboot_ctx_s *ctx, size_t nctx)
{
...
static void fastboot_publish(FAR struct fastboot_ctx_s *ctx, FAR const char *name,
							 FAR const char *string, int data)
{
	FAR struct fastboot_var_s *var;
...
static void fastboot_create_publish(FAR struct fastboot_ctx_s *ctx, size_t nctx)
{
	for (; nctx-- > 0; ctx++) {
		fastboot_publish(ctx, "product", "ROUX", 0);
		fastboot_publish(ctx, "kernel", "ROUX", 0);
		fastboot_publish(ctx, "version", CONFIG_VERSION_STRING, 0);
		fastboot_publish(ctx, "slot-count", "1", 0);
		fastboot_publish(ctx, "max-download-size", NULL, CONFIG_SYSTEM_FASTBOOTD_DOWNLOAD_MAX);
	}
}

static void fastboot_free_publish(FAR struct fastboot_ctx_s *ctx, size_t nctx)
{
	FAR struct fastboot_var_s *next;
...
static int fastboot_open_usb(int index, int flags)
{
	...
	snprintf(usbdev, sizeof(usbdev), "%s/ep%d", FASTBOOT_USBDEV, index);
	do {
...
static int fastboot_usbdev_initialize(FAR struct fastboot_ctx_s *ctx)
{
	...
	ctx->tran_fd[0] = fastboot_open_usb(FASTBOOT_EP_BULKOUT_IDX, O_RDONLY | O_CLOEXEC | O_NONBLOCK);
	if (ctx->tran_fd[0] < 0) {
		return ctx->tran_fd[0];
	}

	ctx->tran_fd[1] = fastboot_open_usb(FASTBOOT_EP_BULKIN_IDX, O_WRONLY | O_CLOEXEC | O_NONBLOCK);
	if (ctx->tran_fd[1] < 0) {
		close(ctx->tran_fd[0]);
		ctx->tran_fd[0] = -1;
...
static int fastboot_tcp_initialize(FAR struct fastboot_ctx_s *ctx)
{
	...
	ctx->tran_fd[0] = socket(AF_INET, SOCK_STREAM, SOCK_CLOEXEC | SOCK_NONBLOCK);
	if (ctx->tran_fd[0] < 0) {
...
static ssize_t fastboot_tcp_read(FAR struct fastboot_ctx_s *ctx, FAR void *buf, size_t len)
{
	...
	if (ctx->tran_fd[1] == -1) {
		while (1) {
			...
			if (fastboot_read_all(ctx->tran_fd[1], handshake, sizeof(handshake)) !=
					sizeof(handshake) ||
				strncmp(handshake, FASTBOOT_TCP_HANDSHAKE, sizeof(handshake)) != 0 ||
				fastboot_write(ctx->tran_fd[1], handshake, sizeof(handshake)) < 0) {
	...
	if (ctx->left == 0) {
		nread = fastboot_read_all(ctx->tran_fd[1], &data_size, sizeof(data_size));
...
static int fastboot_context_initialize(FAR struct fastboot_ctx_s *ctx, size_t nctx)
{
	int ret;
...
static void fastboot_context_deinit(FAR struct fastboot_ctx_s *ctx, size_t nctx)
{
	for (; nctx-- > 0; ctx++) {
