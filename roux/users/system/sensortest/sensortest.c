...
static void print_gnss_satellite(FAR const char *buffer, FAR const char *name);
...
static const struct sensor_info g_sensor_info[] = {
	...
	{print_gnss_satellite, sizeof(struct sensor_gnss_satellite), "gnss_satellite"},
	...
	{print_velocity, sizeof(struct sensor_velocity), "velocity"},
...
static void print_vec3(const char *buffer, const char *name)
{
	...
	printf("%s: timestamp:%" PRIu64 " x:%.2f y:%.2f z:%.2f, "
		   "temperature:%.2f\n",
		   name, event->timestamp, event->x, event->y, event->z, event->temperature);
}
...
static void print_vali2(const char *buffer, const char *name)
{
	...
	printf("%s: timestamp:%" PRIu64 " value1:% " PRIi32 " value2:% " PRIi32 "\n", name,
		   event->timestamp, event->x, event->y);
}
...
static void print_valf2(const char *buffer, const char *name)
{
	...
	printf("%s: timestamp:%" PRIu64 " value1:%.2f value2:%.2f\n", name, event->timestamp,
		   event->pressure, event->temperature);
}
...
static void print_valf3(const char *buffer, const char *name)
{
	...
	printf("%s: timestamp:%" PRIu64 " value1:%.2f value2:%.2f, value3:%.2f\n", name,
		   event->timestamp, event->r, event->g, event->b);
}

static void print_ecg(const char *buffer, const char *name)
{
	struct sensor_ecg *event = (struct sensor_ecg *)buffer;
	printf("%s: timestamp:%" PRIu64 " ecg:%.4f status:%" PRIu32, name, event->timestamp, event->ecg,
		   event->status);
}
...
static void print_ppgd(const char *buffer, const char *name)
{
	...
	printf("%s: timestamp:%" PRIu64 " ppg1:%" PRIu32 " ppg2:%" PRIu32 " "
		   ...
		   name, event->timestamp, event->ppg[0], event->ppg[1], event->current, event->gain[0],
		   event->gain[1]);
}
...
static void print_ppgq(const char *buffer, const char *name)
{
	...
	printf("%s: timestamp:%" PRIu64 " ppg1:%" PRIu32 " ppg2:%" PRIu32 " "
		   ...
		   "gain4:%" PRIu16 "\n",
		   name, event->timestamp, event->ppg[0], event->ppg[1], event->ppg[2], event->ppg[3],
		   event->current, event->gain[0], event->gain[1], event->gain[2], event->gain[3]);
}
...
static void print_cap(FAR const char *buffer, FAR const char *name)
{
	...
	printf("%s: timestamp:%" PRIu64 " status:%" PRIu32 " "
		   ...
		   "rawdata3:%" PRIu32 "\n",
		   name, event->timestamp, event->status, event->rawdata[0], event->rawdata[1],
		   event->rawdata[2], event->rawdata[3]);
}
...
static void print_gnss(const char *buffer, const char *name)
{
	...
	printf("%s: timestamp:%" PRIu64 " time_utc: %" PRIu64 " latitude: %f "
		   ...
		   "hdop: %f vdop: %f ground_speed: %f course: %f satellites_used: %" PRIu32 "\n",
		   name, event->timestamp, event->time_utc, event->latitude, event->longitude,
		   event->altitude, event->altitude_ellipsoid, event->eph, event->epv, event->hdop,
		   event->vdop, event->ground_speed, event->course, event->satellites_used);
}

static void print_gnss_satellite(FAR const char *buffer, FAR const char *name)
{
	FAR FAR struct sensor_gnss_satellite *event = (FAR struct sensor_gnss_satellite *)buffer;

	printf("%s: timestamp: %" PRIu64 " count: %" PRIu32 " satellites: %" PRIu32 "\n", name,
		   event->timestamp, event->count, event->satellites);
}

static void print_velocity(const char *buffer, const char *name)
{
	FAR struct sensor_velocity *event = (FAR struct sensor_velocity *)buffer;
...
int main(int argc, FAR char *argv[])
{
	...
	if (optind < argc) {
		...
		for (idx = 0; idx < nitems(g_sensor_info); idx++) {
			if (!strncmp(name, g_sensor_info[idx].name, strlen(g_sensor_info[idx].name))) {
