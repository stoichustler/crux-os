...
typedef CODE void (*dumpfile_cb_t)(FAR char *path, FAR const char *filename, FAR void *arg);
...
static void dumpfile_count(FAR char *path, FAR const char *filename, FAR void *arg)
{
...
static void dumpfile_delete(FAR char *path, FAR const char *filename, FAR void *arg)
{
...
static int dumpfile_get_info(int fd, FAR struct coredump_info_s *info)
{
	...
	/* The last program header is for ROUX core info note. */

	if (lseek(fd, ehdr.e_phoff + ehdr.e_phentsize * (ehdr.e_phnum - 1), SEEK_SET) < 0) {
		return -errno;
	}

	if (read(fd, &phdr, sizeof(phdr)) != sizeof(phdr) || lseek(fd, phdr.p_offset, SEEK_SET) < 0) {
		return -errno;
	}

	/* The note header must match exactly. */

	if (read(fd, &nhdr, sizeof(nhdr)) != sizeof(nhdr) || nhdr.n_type != COREDUMP_MAGIC ||
		nhdr.n_namesz != COREDUMP_INFONAME_SIZE ||
		nhdr.n_descsz != sizeof(struct coredump_info_s)) {
		return -EINVAL;
	}

	if (read(fd, name, nhdr.n_namesz) != nhdr.n_namesz || strcmp(name, "ROUX") != 0) {
		return -EINVAL;
	}
...
static void coredump_restore(FAR char *savepath, size_t maxfile)
{
	...
	ret = snprintf(dumppath, sizeof(dumppath), "%s/%.16s-%llx" COREDUMP_FILE_SUFFIX, savepath,
				   info.name.version, (unsigned long long)info.time.tv_sec);
...
#ifdef CONFIG_SYSTEM_COREDUMP_RESTORE
	fprintf(stderr,
			"Second usage, will restore coredump"
			"from %s to savepath\n",
			CONFIG_SYSTEM_COREDUMP_DEVPATH);
...
int main(int argc, FAR char *argv[])
{
	...
	struct option options[] = {{"pid", 1, NULL, 'p'},
							   {"filename", 1, NULL, 'f'},
#ifdef CONFIG_SYSTEM_COREDUMP_RESTORE
							   {"savepath", 1, NULL, 's'},
							   {"maxfile", 1, NULL, 'm'},
	...
	while ((ret = getopt_long(argc, argv, "p:f:s:m:h", options, NULL)) != ERROR) {
		switch (ret) {
		case 'p':
			pid = atoi(optarg);
			break;
