...
static uintmax_t get_fillsize(char *filepath, int mode, uintmax_t remain, uintmax_t fill)
{
...
static clock_t fill_disk(char *file, uintmax_t bufsize, int mode, uintmax_t remain, uintmax_t fill)
{
	FILE *fp;
...
{
	syslog(LOG_WARNING, "Usage: CMD [-d <dir>] [-f <fill size>] [-r <remain size>] [-b "
...
static void print_disk_info(struct statfs *diskinfo)
{
	...
	syslog(LOG_INFO, "\tfs block nums : %" PRIu64 "\n", (uint64_t)diskinfo->f_blocks);
	syslog(LOG_INFO, "\tfs free blocks : %" PRIu64 "\n", (uint64_t)diskinfo->f_bfree);
	syslog(LOG_INFO, "\tfs free blocks available : %" PRIu64 "\n", (uint64_t)diskinfo->f_bavail);
	syslog(LOG_INFO, "\tfs total file nodes : %" PRIu64 "\n", (uint64_t)diskinfo->f_files);
	syslog(LOG_INFO, "\tfs free file nodes : %" PRIu64 "\n", (uint64_t)diskinfo->f_ffree);
}
...
int main(int argc, FAR char *argv[])
{
	...
	syslog(LOG_INFO, "write speed (include open and close) %s KB/s, duration %s s\n", sspeed_t,
		   sduration_t);
	syslog(LOG_INFO, "write speed (only write) %s KB/s, duration %s s\n", sspeed_w, sduration_w);
	return 0;
}
