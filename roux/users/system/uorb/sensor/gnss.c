...
#define UORB_DEBUG_FORMAT_SENSOR_GNSS                                                              \
	"timestamp:%" PRIu64 ",time_utc:%" PRIu64 ",latitude:%hf"                                      \
	",longitude:%hf"                                                                               \
	",altitude:%hf"                                                                                \
...
#define SENSOR_GNSS_SATELLITE_INFO_FORMAT(idx)                                                     \
	",svid" #idx ":%" PRIu32 ",elevation" #idx ":%" PRIu32 ",azimuth" #idx ":%" PRIu32 ",snr" #idx \
	":%" PRIu32 ""
...
static const char sensor_gnss_geofence_event_format[] =
	"type:%" PRId32 ",geofence_id:%" PRId32 "," UORB_DEBUG_FORMAT_SENSOR_GNSS ","
...
static const char sensor_gnss_satellite_format[] =
	...
	",cf:%hf" SENSOR_GNSS_SATELLITE_INFO_FORMAT(0) SENSOR_GNSS_SATELLITE_INFO_FORMAT(1)
		SENSOR_GNSS_SATELLITE_INFO_FORMAT(2) SENSOR_GNSS_SATELLITE_INFO_FORMAT(3);
#endif
...
ORB_DEFINE(sensor_gnss_clock, struct sensor_gnss_clock, sensor_gnss_clock_format);
ORB_DEFINE(sensor_gnss_geofence_event, struct sensor_gnss_geofence_event,
		   sensor_gnss_geofence_event_format);
ORB_DEFINE(sensor_gnss_measurement, struct sensor_gnss_measurement, sensor_gnss_measurement_format);
ORB_DEFINE(sensor_gnss_satellite, struct sensor_gnss_satellite, sensor_gnss_satellite_format);
