...
static FAR void *pubsubtest_thread_entry(FAR void *arg)
{
	...
	if (g_pubsubtest_print) {
		...
		snprintf(fname, sizeof(fname), UORB_STORAGE_DIR "/uorb_timings%u.txt", timingsgroup);
...
static int latency_test(bool print)
{
	...
	fd = orb_advertise_multi_queue_persist(ORB_ID(orb_test_medium), &sample, &instance, 1);
	if (fd < 0) {
...
static int test_single(void)
{
	...
	if (sample.val != sub_sample.val) {
		test_fail("copy(1) mismatch: %d expected %d", sub_sample.val, sample.val);
		goto out;
	}
	...
	if (sample.val != sub_sample.val) {
		test_fail("copy(2) mismatch: %d expected %d", sub_sample.val, sample.val);
		goto out;
	}
...
static int test_multi_inst10(void)
{
	...
	for (i = 0; i < max_inst; i++) {
		afd[i] = orb_advertise_multi_queue_persist(ORB_ID(orb_test), NULL, &i, 1);
	}
				...
				if (!updated) {
					...
					if (orb_copy(ORB_ID(orb_test), sfd[sub_instance], &sample)) {
						...
						if (sample.val != (j * sub_instance + i)) {
							test_fail("sub %d invalid value %d", sub_instance, sample.val);
							goto out;
						}
					}
				}
...
static int test_multi(int *afds, int *sfds)
{
	...
	afds[0] = orb_advertise_multi_queue_persist(ORB_ID(orb_multitest), &sample, &instance0, 1);

	if (instance0 != 0) {
		return test_fail("mult. id0: %d", instance0);
	}

	afds[1] = orb_advertise_multi_queue_persist(ORB_ID(orb_multitest), &sample, &instance1, 1);
...
static int test_multi_reversed(int *afds, int *sfds)
{
	...
	afds[2] = orb_advertise_multi_queue_persist(ORB_ID(orb_multitest), &sample, &instance2, 1);

	if (instance2 != 2) {
		return test_fail("mult. id2: %d", instance2);
	}

	afds[3] = orb_advertise_multi_queue_persist(ORB_ID(orb_multitest), &sample, &instance3, 1);
...
static FAR void *pub_test_multi2_entry(FAR void *arg)
{
	...
	for (i = 0; i < num_instances; ++i) {
		orb_pub[i] =
			orb_advertise_multi_queue_persist(ORB_ID(orb_test_medium_multi), &data_topic, &i, 1);
	}
...
static int test_multi2(void)
{
	...
	while (!g_thread_should_exit) {
		...
		if (updated) {
			...
			if (OK != orb_copy(ORB_ID(orb_test_medium_multi), orb_data_cur_fd, &msg)) {
				test_fail("copy failed: %d", errno);
				goto out;
			}

			if (last_time >= msg.timestamp && last_time != 0) {
				test_fail("Timestamp not increasing! (%" PRIu64 " >= %" PRIu64 ")", last_time,
						  msg.timestamp);
				goto out;
			}
...
int test_queue(void)
{
	...
	ptopic = orb_advertise_multi_queue_persist(ORB_ID(orb_test_medium_queue), &sample, &instance,
											   queue_size);
	if (ptopic < 0) {
		orb_unsubscribe(sfd);
		return test_fail("advertise failed: %d", errno);
...
static FAR void *pub_test_queue_entry(FAR void *arg)
{
	...
	(void)arg;
	memset(&t, '\0', sizeof(t));
	ptopic = orb_advertise_multi_queue_persist(ORB_ID(orb_test_medium_queue_poll), &t, &instance,
											   queue_size);
	if (ptopic < 0) {
		g_thread_should_exit = true;
