...
static int listener_get_state(FAR struct orb_object *object, FAR struct orb_state *state);
static int listener_add_object(FAR struct listen_list_s *objlist, FAR struct orb_object *object);
static void listener_delete_object_list(FAR struct listen_list_s *objlist);
static int listener_generate_object_list(FAR struct listen_list_s *objlist, FAR const char *filter);
static int listener_print(FAR const struct orb_metadata *meta, int fd);
static void listener_monitor(FAR struct listen_list_s *objlist, int nb_objects, float topic_rate,
							 int topic_latency, int nb_msgs, int timeout, bool record);
static int listener_update(FAR struct listen_list_s *objlist, FAR struct orb_object *object);
static void listener_top(FAR struct listen_list_s *objlist, FAR const char *filter, bool only_once);
static int listener_create_dir(FAR char *dir, size_t size);
static int listener_record(FAR const struct orb_metadata *meta, int fd, FAR FILE *file);
...
static int listener_update(FAR struct listen_list_s *objlist, FAR struct orb_object *object)
{
	...
	{
		if (tmp->object.meta == object->meta && tmp->object.instance == object->instance) {
			old = tmp;
			break;
		}
	}
	...
	if (old) {
		...
		if (delta_generation && delta_time) {
			...
			frequency = (state.max_frequency ? state.max_frequency : 1000000) * delta_generation /
						delta_time;
			uorbinfo_raw("\033[K"
						 "%-*s %2u %4" PRIu32 " %4lu "
						 "%2" PRIu32 " %4u",
						 ORB_MAX_PRINT_NAME, object->meta->o_name, object->instance,
						 state.nsubscribers, frequency, state.queue_size, object->meta->o_size);
...
static int listener_generate_object_list(FAR struct listen_list_s *objlist, FAR const char *filter)
{
	...
	while ((entry = readdir(dir))) {
		...
		if (filter) {
			...
			if (!str || (str[len] && str[len] != ',' && str[len] != object.instance + '0')) {
				continue;
			}
		}
							 ...
							 int topic_latency, int nb_msgs, int timeout, bool record)
{
	...
	if (record) {
		...
		SLIST_FOREACH(tmp, objlist, node)
		{
			sprintf(dir, "%s%d.csv", tmp->object.meta->o_name, tmp->object.instance);
			tmp->file = fopen(path, "w");
			if (tmp->file != NULL) {
#ifdef CONFIG_DEBUG_UORB
				fprintf(tmp->file, "%s,%d,%d,%s\n", tmp->object.meta->o_format,
						tmp->object.meta->o_size, tmp->object.instance, tmp->object.meta->o_name);
#endif

				uorbinfo_raw("creat file:[%s]", path);
			} else {
				uorbinfo_raw("file creat failed!meta name:%s,instance:%d", tmp->object.meta->o_name,
							 tmp->object.instance);
			}
		}
	}
		...
		if (poll(&fds[0], nb_objects, timeout * 1000) > 0) {
			...
			SLIST_FOREACH(tmp, objlist, node)
			{
				if (fds[i].revents & POLLIN) {
					...
					if (tmp->file != NULL) {
						if (listener_record(tmp->object.meta, fds[i].fd, tmp->file) < 0) {
							uorberr("Listener record %s data failed!", tmp->object.meta->o_name);
						}
					} else {
	...
	{
		if (fds[i].fd < 0) {
			uorbinfo_raw("Object name:%s%d, subscribe fail", tmp->object.meta->o_name,
						 tmp->object.instance);
		} else {
			if (topic_latency) {
			...
			uorbinfo_raw("Object name:%s%d, recieved:%d", tmp->object.meta->o_name,
						 tmp->object.instance, recv_msgs[i]);
		}
	...
	uorbinfo_raw("Total number of received Message:%d/%d", nb_recv_msgs,
				 nb_msgs ? nb_msgs : nb_recv_msgs);
	free(fds);
	free(recv_msgs);
}
...
static void listener_top(FAR struct listen_list_s *objlist, FAR const char *filter, bool only_once)
{
	...
	do {
		...
		uorbinfo_raw("\033[K"
					 "current objects: %zu",
					 listen_length(objlist));
		uorbinfo_raw("\033[K"
					 "%-*s INST #SUB RATE #Q SIZE",
...
int main(int argc, FAR char *argv[])
{
	...
	if (top) {
		...
		SLIST_FOREACH(tmp, &objlist, node)
		{
			uorbinfo_raw("object_name:%s, object_instance:%d", tmp->object.meta->o_name,
						 tmp->object.instance);
		}

		listener_monitor(&objlist, ret, topic_rate, topic_latency, nb_msgs, timeout, record);
	}
