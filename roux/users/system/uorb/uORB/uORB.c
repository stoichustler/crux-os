...
static int orb_advsub_open(FAR const struct orb_metadata *meta, int flags, int instance,
...
static int orb_advertise_multi_queue_flags(FAR const struct orb_metadata *meta,
										   ...
										   unsigned int queue_size, int flags)
{
	...
	if (data != NULL) {
		...
		if (ret != meta->o_size) {
			uorberr("%s publish %d, expect %d", meta->o_name, ret, meta->o_size);
			close(fd);
			return -1;
		}
	}
...
{
	return orb_advertise_multi_queue_flags(meta, data, instance, queue_size, O_WRONLY);
}
...
int orb_get_state(int fd, FAR struct orb_state *state)
{
	...
	state->max_frequency = tmp.min_interval ? 1000000 / tmp.min_interval : 0;
