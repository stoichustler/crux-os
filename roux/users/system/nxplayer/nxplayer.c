...
static const struct nxplayer_dec_ops_s g_dec_ops[] = {
	{AUDIO_FMT_MP3, nxplayer_parse_mp3, nxplayer_fill_common},
	{AUDIO_FMT_SBC, nxplayer_parse_sbc, nxplayer_fill_common},
...
static int _open_with_http(const char *fullurl)
{
	...
	n = netlib_parsehttpurl(fullurl, &port, hostname, sizeof(hostname) - 1, relurl,
							sizeof(relurl) - 1);
	...
	memcpy(&server.sin_addr.s_addr, he->h_addr, sizeof(in_addr_t));

	n = connect(s, (struct sockaddr *)&server, sizeof(struct sockaddr_in));
...
#ifdef CONFIG_NXPLAYER_INCLUDE_DEVICE_SEARCH
	{
		...
		while ((pdevice = readdir(dirp)) != NULL) {
			...
			if ((pplayer->dev_fd = open(path, O_RDWR | O_CLOEXEC)) != -1) {
				...
				if (ioctl(pplayer->dev_fd, AUDIOIOC_GETCAPS, (unsigned long)&cap) == cap.ac_len) {
					...
					if (((cap.ac_format.hw & (1 << (format - 1))) != 0) &&
						...
						if (subfmt != AUDIO_FMT_UNDEF) {
							...
							while (ioctl(pplayer->dev_fd, AUDIOIOC_GETCAPS, (unsigned long)&cap) ==
								   cap.ac_len) {
								...
								for (x = 0; x < sizeof(cap.ac_controls.b); x++) {
									if (cap.ac_controls.b[x] == subfmt) {
										...
										break;
									} else if (cap.ac_controls.b[x] == AUDIO_SUBFMT_END) {
...
static inline int nxplayer_fmtfromextension(FAR struct nxplayer_s *pplayer,
											FAR const char *pfilename, FAR int *subfmt)
{
	FAR const char *pext;
	uint8_t x;
	uint8_t c;
...
static int nxplayer_mediasearch(FAR struct nxplayer_s *pplayer, FAR const char *pfilename,
...
static int nxplayer_enqueuebuffer(FAR struct nxplayer_s *pplayer, FAR struct ap_buffer_s *apb)
{
	...
	ret = ioctl(pplayer->dev_fd, AUDIOIOC_ENQUEUEBUFFER, (unsigned long)&bufdesc);
	if (ret < 0) {
		int errcode = errno;
		DEBUGASSERT(errcode > 0);
...
static FAR void *nxplayer_playthread(pthread_addr_t pvarg)
{
	...
	if ((ret = ioctl(pplayer->dev_fd, AUDIOIOC_GETBUFFERINFO, (unsigned long)&buf_info)) != OK) {
	...
	for (x = 0; x < buf_info.nbuffers; x++) {
		...
		ret = ioctl(pplayer->dev_fd, AUDIOIOC_ALLOCBUFFER, (unsigned long)&buf_desc);
	...
	if (running && !failed) {
#ifdef CONFIG_AUDIO_MULTI_SESSION
		ret = ioctl(pplayer->dev_fd, AUDIOIOC_START, (unsigned long)pplayer->session);
#else
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
			ioctl(pplayer->dev_fd, AUDIOIOC_STOP, (unsigned long)pplayer->session);
#else
...
err_out:
	...
	for (x = 0; x < buf_info.nbuffers; x++) {
		...
		if (buffers[x] != NULL) {
			...
			ioctl(pplayer->dev_fd, AUDIOIOC_FREEBUFFER, (unsigned long)&buf_desc);
		}
	}
...
int nxplayer_pause(FAR struct nxplayer_s *pplayer)
{
	...
	if (pplayer->state == NXPLAYER_STATE_PLAYING) {
#ifdef CONFIG_AUDIO_MULTI_SESSION
		ret = ioctl(pplayer->dev_fd, AUDIOIOC_PAUSE, (unsigned long)pplayer->session);
#else
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
		ret = ioctl(pplayer->dev_fd, AUDIOIOC_RESUME, (unsigned long)pplayer->session);
#else
...
static int nxplayer_playinternal(FAR struct nxplayer_s *pplayer, FAR const char *pfilename,
								 int filefmt, int subfmt, uint8_t nchannels, uint8_t bpsamp,
								 uint32_t samprate, uint8_t chmap)
{
	...
	if (pplayer->ops->pre_parse) {
		ret = pplayer->ops->pre_parse(pplayer->fd, &samprate, &nchannels, &bpsamp);
	}

	/* Try to reserve the device */

#ifdef CONFIG_AUDIO_MULTI_SESSION
	ret = ioctl(pplayer->dev_fd, AUDIOIOC_RESERVE, (unsigned long)&pplayer->session);
#else
	...
	if (ioctl(pplayer->dev_fd, AUDIOIOC_GETCAPS, (unsigned long)&caps) == caps.ac_len) {
	...
	if (ioctl(pplayer->dev_fd, AUDIOIOC_GETBUFFERINFO, (unsigned long)&buf_info) != OK) {
		/* Driver doesn't report its buffer size.  Use our default. */
	...
	pthread_attr_setstacksize(&tattr, CONFIG_NXPLAYER_PLAYTHREAD_STACKSIZE);
	...
	ret = pthread_create(&pplayer->play_id, &tattr, nxplayer_playthread, (pthread_addr_t)pplayer);
	if (ret != OK) {
...
int nxplayer_playfile(FAR struct nxplayer_s *pplayer, FAR const char *pfilename, int filefmt,
					  int subfmt)
{
	return nxplayer_playinternal(pplayer, pfilename, filefmt, subfmt, 0, 0, 0, 0);
}
...
int nxplayer_playraw(FAR struct nxplayer_s *pplayer, FAR const char *pfilename, uint8_t nchannels,
					 uint8_t bpsamp, uint32_t samprate, uint8_t chmap)
{
	...
	return nxplayer_playinternal(pplayer, pfilename, AUDIO_FMT_PCM, 0, nchannels, bpsamp, samprate,
								 chmap);
}
...
#ifdef CONFIG_NXPLAYER_INCLUDE_MEDIADIR
	strlcpy(pplayer->mediadir, CONFIG_NXPLAYER_DEFAULT_MEDIADIR, sizeof(pplayer->mediadir));
#endif
