...
const static uint16_t g_mpa_freq_tab[3] = {44100, 48000, 32000};
...
static int nxplayer_parse_mpeg(uint32_t header, FAR uint32_t *samplerate, FAR uint8_t *chans,
							   FAR uint8_t *bps)
{
	int sample_rate;
	int frame_size;
...
int nxplayer_parse_mp3(int fd, FAR uint32_t *samplerate, FAR uint8_t *chans, FAR uint8_t *bps)
{
	uint32_t mpa_header;
	uint8_t buffer[10];
	off_t position;
	...
	if (!memcmp(buffer, "ID3", 3)) {
		position = (buffer[6] & ID3V2_BIT_MASK) * 0x200000 + (buffer[7] & ID3V2_BIT_MASK) * 0x4000 +
				   (buffer[8] & ID3V2_BIT_MASK) * 0x80 + (buffer[9] & ID3V2_BIT_MASK) +
				   sizeof(buffer);
	...
	mpa_header = buffer[0] << 24 | buffer[1] << 16 | buffer[2] << 8 | buffer[3];
...
int nxplayer_fill_mp3(int fd, FAR struct ap_buffer_s *apb)
{
	...
	mpa_header = header[0] << 24 | header[1] << 16 | header[2] << 8 | header[3];
