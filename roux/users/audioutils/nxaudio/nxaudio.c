...
static void free_audio_buffers(FAR struct nxaudio_s *nxaudio)
{
	...
	for (x = 0; x < nxaudio->abufnum; x++) {
		if (nxaudio->abufs[x] != NULL) {
			...
			ioctl(nxaudio->fd, AUDIOIOC_FREEBUFFER, (unsigned long)(uintptr_t)&desc);
		}
	}
...
int init_nxaudio(FAR struct nxaudio_s *nxaudio, int fs, int bps, int chnum)
{
	return init_nxaudio_devname(nxaudio, fs, bps, chnum, CONFIG_AUDIOUTILS_NXAUDIO_DEVPATH,
...
int init_nxaudio_devname(FAR struct nxaudio_s *nxaudio, int fs, int bps, int chnum,
						 const char *devname, const char *mqname)
{
	...
	if (nxaudio->fd >= 0) {
		...
		nxaudio->abufs = create_audio_buffers(nxaudio->fd, buf_info.nbuffers, buf_info.buffer_size);
...
int nxaudio_enqbuffer(FAR struct nxaudio_s *nxaudio, FAR struct ap_buffer_s *apb)
{
	struct audio_buf_desc_s desc;
...
int nxaudio_msgloop(FAR struct nxaudio_s *nxaudio, FAR struct nxaudio_callbacks_s *cbs,
					unsigned long arg)
{
	bool running = true;
