...
static int set_egparams(int fs, FAR fmsynth_egparam_t *param,
						...
						FAR struct fmsynth_eglevel_s *last_level)
{
	...
	param->diff2next = CONVERT_INITVAL(target_level->level) - CONVERT_INITVAL(last_level->level);

	if (param->initval < -FMSYNTH_MAX_EGLEVEL || param->initval > FMSYNTH_MAX_EGLEVEL ||
		param->period < 0) {
		return -1;
	}
...
int fmsyntheg_set_param(FAR fmsynth_eg_t *eg, int fs, FAR fmsynth_eglevels_t *levels)
{
	int errcnt = 0;
	...
	errcnt +=
		set_egparams(fs, &eg->state_params[EGSTATE_ATTACK], &levels->attack, &levels->release);

	errcnt +=
		set_egparams(fs, &eg->state_params[EGSTATE_DECAYBREAK], &levels->decaybrk, &levels->attack);

	errcnt += set_egparams(fs, &eg->state_params[EGSTATE_DECAY], &levels->decay, &levels->decaybrk);

	errcnt +=
		set_egparams(fs, &eg->state_params[EGSTATE_SUSTAIN], &levels->sustain, &levels->decay);

	errcnt +=
		set_egparams(fs, &eg->state_params[EGSTATE_RELEASE], &levels->release, &levels->sustain);

	eg->state_params[EGSTATE_RELEASED].initval = CONVERT_INITVAL(levels->release.level);
...
int fmsyntheg_operate(FAR fmsynth_eg_t *eg)
{
	...
	if (eg->state != EGSTATE_RELEASED) {
		if (eg->state_counter >= eg->state_params[eg->state].period) {
			...
			do {
				eg->state++;
			} while (eg->state < EGSTATE_RELEASED && eg->state_params[eg->state].period == 0);
