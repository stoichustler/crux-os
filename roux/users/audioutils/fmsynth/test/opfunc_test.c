...
static void wavegen_dump(void)
{
	...
	for (t = 0; t < DUMP_PERIOD; t++) {
		printf("%d, %d, %d, %d\n", func_sin((int)(deltaact * t)), func_tri((int)(deltaact * t)),
			   func_saw((int)(deltaact * t)), func_sqa((int)(deltaact * t)));
	}
...
static void sin_accuracy_test(void)
{
	...
	for (t = 0; t < ACCURACY_TEST_PERIOD; t++) {
		...
		printf("t=%d, operator-sin(%d)=%d, norm_sin=%f, sinf(%f)=%f ", t, (int)(deltaact * t),
			   sin_val, norm_sin, delta * t, ref_sin);
