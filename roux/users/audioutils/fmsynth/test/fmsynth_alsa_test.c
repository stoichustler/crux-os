...
static snd_pcm_t *init_alsa(int fs)
{
	...
	ret = snd_pcm_set_params(hndl, SND_PCM_FORMAT_S16, SND_PCM_ACCESS_RW_INTERLEAVED, CHANNEL_NUM,
							 fs, RESAMPLING_ALSA, LATENCY_ALSA);
	if (ret != 0) {
...
static fmsynth_eglevels_t *set_levels(fmsynth_eglevels_t *level, float atk_lvl, int atk_peri,
									  float decbrk_lvl, int decbrk_peri, float dec_lvl,
									  int dec_peri, float sus_lvl, int sus_peri, float rel_lvl,
									  int rel_peri)
{
