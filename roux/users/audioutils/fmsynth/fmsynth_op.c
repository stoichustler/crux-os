...
#define PHASE_ADJUST(th) (((th) < 0 ? (FMSYNTH_PI) - (th) : (th)) % (FMSYNTH_PI * 2))
...
static int pseudo_sin256(int theta)
{
	...
	if (phase & 0x01) {
		...
		short_sin = short_sin + (((s_sintbl[tblidx - 1] - short_sin) * rest) >> 7);
	} else {
		short_sin = s_sintbl[tblidx + 1];
		short_sin = short_sin + (((s_sintbl[tblidx + 2] - short_sin) * rest) >> 7);
	}
...
{
	if (local_fs != 0) {
		op->delta_phase = 2 * FMSYNTH_PI * op->sound_freq * op->freq_rate / (float)local_fs;
	} else {
		op->delta_phase = 0.f;
	}
}
...
FAR fmsynth_op_t *create_fmsynthop(FAR fmsynth_op_t *op, FAR fmsynth_eg_t *eg)
{
	if (op) {
		op->eg = eg;
...
int fmsynthop_set_envelope(FAR fmsynth_op_t *op, FAR fmsynth_eglevels_t *levels)
{
...
int fmsynthop_cascade_subop(FAR fmsynth_op_t *op, FAR fmsynth_op_t *subop)
{
...
int fmsynthop_parallel_subop(FAR fmsynth_op_t *op, FAR fmsynth_op_t *subop)
{
	FAR fmsynth_op_t *tmp;
...
int fmsynthop_bind_feedback(FAR fmsynth_op_t *op, FAR fmsynth_op_t *subop, float ratio)
{
	if (!op || !subop) {
		return ERROR;
	}
...
int fmsynthop_update_feedback(FAR fmsynth_op_t *op)
{
	...
	if (op->feedback_ref) {
		op->feedback_val = *op->feedback_ref * op->feedbackrate / FMSYNTH_MAX_EGLEVEL;
	}
...
int fmsynthop_operate(FAR fmsynth_op_t *op, int phase_time)
{
	...
	op->last_sigval = fmsyntheg_operate(op->eg) * op->wavegen(phase) / FMSYNTH_MAX_EGLEVEL;
