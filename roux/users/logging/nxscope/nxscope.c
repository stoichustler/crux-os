...
#define CHINFO_DATA_SIZE_MAX                                                                       \
	(sizeof(struct nxscope_chinfo_s) - sizeof(char *) + CHAN_NAMELEN_MAX + 1)
...
static int nxscope_frame_send(FAR struct nxscope_s *s, uint8_t id, FAR uint8_t *data, size_t dlen)
{
	size_t tx_i = 0;
	int ret = OK;
...
static int nxscope_cmninfo_send(FAR struct nxscope_s *s)
{
	...
	return nxscope_frame_send(s, NXSCOPE_HDRID_CMNINFO, (FAR uint8_t *)&s->cmninfo,
...
static int nxscope_enable_req(FAR struct nxscope_s *s, FAR struct nxscope_set_frame_s *set,
							  uint16_t dlen)
{
	...
	case NXSCOPE_SET_REQ_SINGLE: {
		...
		if (dlen != (sizeof(struct nxscope_set_frame_s) + NXSCOPE_ENABLE_LEN - 1)) {
	...
	case NXSCOPE_SET_REQ_BULK: {
		...
		if (dlen !=
			(sizeof(struct nxscope_set_frame_s) + NXSCOPE_ENABLE_LEN * s->cmninfo.chmax - 1)) {
...
static int nxscope_div_req(FAR struct nxscope_s *s, FAR struct nxscope_set_frame_s *set,
						   uint16_t dlen)
{
	...
	case NXSCOPE_SET_REQ_SINGLE: {
		...
		if (dlen != (sizeof(struct nxscope_set_frame_s) + NXSCOPE_DIV_LEN - 1)) {
	...
	case NXSCOPE_SET_REQ_BULK: {
		...
		if (dlen != (sizeof(struct nxscope_set_frame_s) + NXSCOPE_DIV_LEN * s->cmninfo.chmax - 1)) {
...
static int nxscope_start_req(FAR struct nxscope_s *s, FAR struct nxscope_start_data_s *data)
{
	int ret = -EINVAL;
...
static int nxscope_recv_handle(FAR struct nxscope_s *s, uint8_t id, uint16_t dlen, FAR uint8_t *buf)
{
	int ret = OK;
	...
	case NXSCOPE_HDRID_START: {
		FAR struct nxscope_start_data_s *data = (FAR struct nxscope_start_data_s *)buf;
	...
	case NXSCOPE_HDRID_ENABLE: {
		FAR struct nxscope_set_frame_s *data = (FAR struct nxscope_set_frame_s *)buf;
...
int nxscope_init(FAR struct nxscope_s *s, FAR struct nxscope_cfg_s *cfg)
{
	...
	s->txbuf_len = (CHINFO_DATA_SIZE_MAX + s->proto_cmd->footlen + s->proto_cmd->hdrlen);
