...
#include <roux/binfmt/symtab.h>
#include <roux/drivers/ramdisk.h>
#include <roux/reboot_notifier.h>

#ifdef CONFIG_NX
#include <roux/nx/nxmu.h>
#endif

#ifdef CONFIG_VNCSERVER
#include <roux/video/vnc.h>
#endif

#ifdef CONFIG_BOARDCTL_USBDEVCTRL
#include <roux/usb/adb.h>
#include <roux/usb/cdcacm.h>
#include <roux/usb/pl2303.h>
#include <roux/usb/usbmsc.h>
...
static inline int boardctl_usbdevctrl(FAR struct boardioc_usbdev_ctrl_s *ctrl)
{
	...
	case BOARDIOC_USBDEV_COMPOSITE: /* Composite device */
		...
		case BOARDIOC_USBDEV_CONNECT: /* Connect the Composite device */
		{
			...
			*ctrl->handle = board_composite_connect(ctrl->instance, ctrl->config);
...
int boardctl(unsigned int cmd, uintptr_t arg)
{
	...
	switch (cmd) {
		 ...
		 *                modification.  The argument has no meaning to ROUX;
	...
	case BOARDIOC_PM_CONTROL: {
		FAR struct boardioc_pm_ctrl_s *ctrl = (FAR struct boardioc_pm_ctrl_s *)arg;
	...
	case BOARDIOC_BOOT_IMAGE: {
		FAR const struct boardioc_boot_info_s *info = (FAR const struct boardioc_boot_info_s *)arg;
	...
	case BOARDIOC_MKRD: {
		FAR const struct boardioc_mkrd_s *desc = (FAR const struct boardioc_mkrd_s *)arg;

		if (desc == NULL) {
			ret = -EINVAL;
		} else {
			ret = mkrd((int)desc->minor, desc->nsectors, desc->sectsize, desc->rdflags);
		}
	} break;
#endif
	...
	case BOARDIOC_ROMDISK: {
		FAR const struct boardioc_romdisk_s *desc = (FAR const struct boardioc_romdisk_s *)arg;

		if (desc == NULL) {
			ret = -EINVAL;
		} else {
			ret = romdisk_register((int)desc->minor, desc->image, desc->nsectors, desc->sectsize);
		}
	} break;
#endif
	...
	case BOARDIOC_APP_SYMTAB: {
		FAR const struct boardioc_symtab_s *symdesc = (FAR const struct boardioc_symtab_s *)arg;
	...
	case BOARDIOC_BUILTINS: {
		...
		FAR const struct boardioc_builtin_s *builtin = (FAR const struct boardioc_builtin_s *)arg;
	...
	case BOARDIOC_USBDEV_CONTROL: {
		FAR struct boardioc_usbdev_ctrl_s *ctrl = (FAR struct boardioc_usbdev_ctrl_s *)arg;
	...
	case BOARDIOC_VNC_START: {
		FAR struct boardioc_vncstart_s *vnc = (FAR struct boardioc_vncstart_s *)arg;
	...
	case BOARDIOC_NXTERM: {
		FAR struct boardioc_nxterm_create_s *nxterm = (FAR struct boardioc_nxterm_create_s *)arg;

		if (nxterm == NULL) {
			ret = -EINVAL;
		} else if (nxterm->type == BOARDIOC_XTERM_RAW) {
			nxterm->nxterm = nx_register((NXWINDOW)nxterm->hwnd, &nxterm->wndo, (int)nxterm->minor);

			ret = nxterm->nxterm == NULL ? -ENODEV : OK;
		} else if (nxterm->type == BOARDIOC_XTERM_FRAMED) {
			nxterm->nxterm =
				nxtk_register((NXTKWINDOW)nxterm->hwnd, &nxterm->wndo, (int)nxterm->minor);

			ret = nxterm->nxterm == NULL ? -ENODEV : OK;
		} else if (nxterm->type == BOARDIOC_XTERM_TOOLBAR) {
			nxterm->nxterm =
				nxtool_register((NXTKWINDOW)nxterm->hwnd, &nxterm->wndo, (int)nxterm->minor);
	...
	case BOARDIOC_NXTERM_IOCTL: {
		FAR struct boardioc_nxterm_ioctl_s *nxterm = (FAR struct boardioc_nxterm_ioctl_s *)arg;
	...
	case BOARDIOC_SPINLOCK: {
		FAR struct boardioc_spinlock_s *spinlock = (FAR struct boardioc_spinlock_s *)arg;
	...
	case BOARDIOC_RESET_CAUSE: {
		FAR struct boardioc_reset_cause_s *cause = (FAR struct boardioc_reset_cause_s *)arg;
