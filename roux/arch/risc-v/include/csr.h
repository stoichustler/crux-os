...
#define WRITE_CSR(reg, val) ({ __asm__ __volatile__("csrw " __STR(reg) ", %0" ::"rK"(val)); })

/* Set the specified bits in a CSR register */

#define SET_CSR(reg, bits) ({ __asm__ __volatile__("csrs " __STR(reg) ", %0" ::"rK"(bits)); })

/* Clear the specified bits in a CSR register */

#define CLEAR_CSR(reg, bits) ({ __asm__ __volatile__("csrc " __STR(reg) ", %0" ::"rK"(bits)); })
...
#define SWAP_CSR(reg, val)                                                                         \
	({                                                                                             \
		uintptr_t regval;                                                                          \
		__asm__ __volatile__("csrrw %0, " __STR(reg) ", %1" : "=r"(regval) : "rK"(val));           \
		regval;                                                                                    \
	})
