...
/****************************************************************************
 * Map RISC-V exception code to ROUX IRQ,
...
noinstrument_function static inline_function irqstate_t up_irq_save(void)
{
	...
	__asm__ __volatile__("csrrc %0, " __XSTR(CSR_STATUS) ", %1\n"
						 : "=r"(flags)
						 : "r"(STATUS_IE)
						 : "memory");
...
noinstrument_function static inline_function void up_irq_restore(irqstate_t flags)
{
	__asm__ __volatile__("csrw " __XSTR(CSR_STATUS) ", %0\n"
						 : /* no output */
						 : "r"(flags)
						 : "memory");
}
...
noinstrument_function static inline_function void up_set_interrupt_context(bool flag)
{
#ifdef CONFIG_SMP
...
#define up_getusrpc(regs) (((uintptr_t *)((regs) ? (regs) : running_regs()))[REG_EPC])

/****************************************************************************
 * Name: up_getusrsp
 ****************************************************************************/

#define up_getusrsp(regs) (((uintptr_t *)(regs))[REG_SP])
