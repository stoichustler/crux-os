...
static void config_baud_divisors(void)
{
	...
	if (baud_value <= (uint32_t)UINT16_MAX) {
		...
		if (baud_value > 1) {
			...
			putreg32(fractional_baud_value, MPFS_CONSOLE_BASE + MPFS_UART_DFR_OFFSET);
		}
	}
}
#endif
...
void riscv_lowputc(char ch)
{
	...
	while ((getreg32(MPFS_CONSOLE_BASE + MPFS_UART_LSR_OFFSET) & UART_LSR_THRE) == 0)
		;
...
void mpfs_lowsetup(void)
{
	...
	modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_SOFT_RESET_CR_OFFSET, 0, MPFS_CONSOLE_RESETBIT);

	/* reset off */

	modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_SUBBLK_CLOCK_CR_OFFSET, 0, MPFS_CONSOLE_CLOCKBIT);

	/* clock on */

	modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_SOFT_RESET_CR_OFFSET, MPFS_CONSOLE_RESETBIT, 0);
