...
static uint32_t mpfs_ihc_sbi_parse_incoming_hartid(uint32_t mhartid, bool *is_ack, bool *is_mp)
{
	uint32_t hart_id = 0;
...
static void mpfs_ihc_sbi_message_present_handler(uint32_t *message, uint32_t mhartid,
												 uint32_t rhartid, bool is_ack, bool is_mp)
{
...
static void mpfs_ihc_sbi_rx_message(uint32_t rhartid, uint32_t mhartid, bool is_ack, bool is_mp,
									uint32_t *msg)
{
	...
	if (is_ack && !is_mp) {
		if (mhartid == CONTEXTB_HARTID) {
			...
			mpfs_ihc_sbi_message_present_handler(msg, mhartid, rhartid, is_ack, is_mp);
		...
		mpfs_modifyreg32(MPFS_IHC_CTRL(mhartid, rhartid), ACK_CLR | MP_MASK, ACK_INT);
	}
}
...
void mpfs_ihc_sbi_message_present_indirect_isr(ihc_channel_t channel, uint32_t *msg)
{
	bool is_ack;
	bool is_mp = false;
	uint32_t mhartid = mpfs_ihc_sbi_context_to_local_hart_id(channel);
	uint32_t origin_hart = mpfs_ihc_sbi_parse_incoming_hartid(mhartid, &is_ack, &is_mp);
...
static void mpfs_ihc_sbi_local_remote_config(uint32_t hart_to_configure, uint32_t rhartid)
{
...
#ifdef CONFIG_MPFS_IHC_TWO_RPMSG_CHANNELS
	if ((hart_to_configure + 1) < MPFS_NUM_HARTS) {
		putreg32(IHCIA_CONTEXTA2_INTS, MPFS_IHC_INT_EN(hart_to_configure + 1));
	}
#endif

	mpfs_modifyreg32(MPFS_IHC_CTRL(hart_to_configure, rhartid), 0, MPIE_EN | ACKIE_EN);

	/* OpenSBI extension may configure 2x consecutive harts */

#ifdef CONFIG_MPFS_IHC_TWO_RPMSG_CHANNELS
	if ((hart_to_configure + 1) < MPFS_NUM_HARTS) {
		mpfs_modifyreg32(MPFS_IHC_CTRL(hart_to_configure + 1, rhartid + 1), 0, MPIE_EN | ACKIE_EN);
	}
#endif
}
...
static int mpfs_ihc_sbi_tx_message(ihc_channel_t channel, uint32_t *message)
{
	...
	if (RMP_MESSAGE_PRESENT == (ctrl_reg & RMP_MASK)) {
		...
		mpfs_modifyreg32(MPFS_IHC_CTRL(mhartid, rhartid), 0, RMP_MESSAGE_PRESENT);
	}
...
int mpfs_ihc_sbi_ecall_handler(unsigned long funcid, uint32_t remote_channel, uint32_t *message_ptr)
{
	uint32_t mhartid;
	uint32_t rhartid;
	int result = OK;
...
#ifdef CONFIG_MPFS_IHC_TWO_RPMSG_CHANNELS
	if ((remote_channel < IHC_CHANNEL_TO_CONTEXTA) || (remote_channel > IHC_CHANNEL_TO_CONTEXTC)) {
		return -EINVAL;
	}
#else
	...
	case SBI_EXT_IHC_CTX_INIT:
		...
		/* mhartid = Linux hart id, rhartid = ROUX hart id */
	...
	case SBI_EXT_IHC_RECEIVE:
		mpfs_ihc_sbi_message_present_indirect_isr(remote_channel, message_ptr);
		break;
