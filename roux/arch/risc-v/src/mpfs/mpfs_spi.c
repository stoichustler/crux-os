...
static void mpfs_spi_select(struct spi_dev_s *dev, uint32_t devid, bool selected);
#endif

static uint32_t mpfs_spi_setfrequency(struct spi_dev_s *dev, uint32_t frequency);
...
static int mpfs_spi_hwfeatures(struct spi_dev_s *dev, spi_hwfeatures_t features);
#endif
static uint8_t mpfs_spi_status(struct spi_dev_s *dev, uint32_t devid);
#ifdef CONFIG_SPI_CMDDATA
static int mpfs_spi_cmddata(struct spi_dev_s *dev, uint32_t devid, bool cmd);
#endif
static uint32_t mpfs_spi_send(struct spi_dev_s *dev, uint32_t wd);
static void mpfs_spi_exchange(struct spi_dev_s *dev, const void *txbuffer, void *rxbuffer,
							  size_t nwords);
#ifndef CONFIG_SPI_EXCHANGE
static void mpfs_spi_sndblock(struct spi_dev_s *dev, const void *txbuffer, size_t nwords);
static void mpfs_spi_recvblock(struct spi_dev_s *dev, void *rxbuffer, size_t nwords);
#endif
...
#ifdef CONFIG_MPFS_SPI0
void weak_function mpfs_spi0_select(struct spi_dev_s *dev, uint32_t devid, bool selected)
{
#ifdef CONFIG_SPI_CS_CONTROL
...
#ifdef CONFIG_MPFS_SPI1
void weak_function mpfs_spi1_select(struct spi_dev_s *dev, uint32_t devid, bool selected)
{
#ifdef CONFIG_SPI_CS_CONTROL
	mpfs_spi_select(dev, devid, selected);
#else
...
static void mpfs_spi_load_tx_fifo(struct mpfs_spi_priv_s *priv, const void *txbuffer,
								  uint32_t nwords)
{
...
static void mpfs_spi_unload_rx_fifo(struct mpfs_spi_priv_s *priv, void *rxbuffer, uint32_t nwords)
{
...
static void mpfs_spi_irq_exchange(struct mpfs_spi_priv_s *priv, const void *txbuffer,
								  void *rxbuffer, uint32_t nwords)
{
	...
	putreg32(MPFS_SPI_TXCHUNDRUN | MPFS_SPI_RXCHOVRFLW | MPFS_SPI_RXRDONECLR | MPFS_SPI_TXDONECLR,
			 MPFS_SPI_INT_CLEAR);
	...
	if (nwords > priv->fifosize) {
		...
		modifyreg32(MPFS_SPI_CONTROL, MPFS_SPI_FRAMECNT, MPFS_SPI_FRAMECNT & (nwords << 8));
		mpfs_spi_load_tx_fifo(priv, txbuffer, nwords);
	}

	/* Enable TX, RX, underrun and overflow interrupts */

	modifyreg32(MPFS_SPI_CONTROL, 0,
				MPFS_SPI_INTTXTURUN | MPFS_SPI_INTRXOVRFLOW | MPFS_SPI_INTRXDATA |
					MPFS_SPI_INTTXDATA);
	...
	modifyreg32(
		MPFS_SPI_CONTROL,
		MPFS_SPI_INTTXTURUN | MPFS_SPI_INTRXOVRFLOW | MPFS_SPI_INTRXDATA | MPFS_SPI_INTTXDATA, 0);
...
static uint32_t mpfs_spi_poll_send(struct mpfs_spi_priv_s *priv, uint32_t wd)
{
...
static void mpfs_spi_poll_exchange(struct mpfs_spi_priv_s *priv, const void *txbuffer,
...
static void mpfs_spi_set_master_mode(struct mpfs_spi_priv_s *priv, uint8_t master)
{
	if (priv->enabled) {
		modifyreg32(MPFS_SPI_CONTROL, MPFS_SPI_ENABLE, 0);
	}
...
static void mpfs_spi_init(struct spi_dev_s *dev)
{
	...
	if (priv->id == 0) {
		...
		modifyreg32(MPFS_SYSREG_SUBBLK_CLOCK_CR, 0, SYSREG_SUBBLK_CLOCK_CR_SPI0);
	} else {
		...
		modifyreg32(MPFS_SYSREG_SUBBLK_CLOCK_CR, 0, SYSREG_SUBBLK_CLOCK_CR_SPI1);
	}
...
static void mpfs_spi_deinit(struct spi_dev_s *dev)
{
	...
	if (priv->id == 0) {
		modifyreg32(MPFS_SYSREG_SUBBLK_CLOCK_CR, SYSREG_SUBBLK_CLOCK_CR_SPI0, 0);
	} else {
		modifyreg32(MPFS_SYSREG_SUBBLK_CLOCK_CR, SYSREG_SUBBLK_CLOCK_CR_SPI1, 0);
	}
