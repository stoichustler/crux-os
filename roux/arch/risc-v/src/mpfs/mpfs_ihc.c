...
#include <roux/rptun/rptun.h>
#include <roux/drivers/addrenv.h>
#include <roux/list.h>
...
static int mpfs_rptun_register_callback(struct rptun_dev_s *dev, rptun_callback_t callback,
										void *arg);
 ...
 * combination with u-boot / Linux kernel will not boot if this ROUX image
 ...
 * has been found out.  This is not needed if ROUX is the bootloader, not
 * HSS.
 */
...
const uint32_t ihcia_remote_harts[MPFS_NUM_HARTS] = {IHCIA_H0_REMOTE_HARTS, IHCIA_H1_REMOTE_HARTS,
													 IHCIA_H2_REMOTE_HARTS, IHCIA_H3_REMOTE_HARTS,
													 IHCIA_H4_REMOTE_HARTS};

const uint32_t ihcia_remote_hart_ints[MPFS_NUM_HARTS] = {
	IHCIA_H0_REMOTE_HARTS_INTS, IHCIA_H1_REMOTE_HARTS_INTS, IHCIA_H2_REMOTE_HARTS_INTS,
	IHCIA_H3_REMOTE_HARTS_INTS, IHCIA_H4_REMOTE_HARTS_INTS};
...
static uint32_t mpfs_ihc_parse_incoming_hartid(uint32_t mhartid, bool *is_ack, bool *is_msg)
{
	uint32_t hart_id = 0;
...
static void mpfs_ihc_worker(void *arg)
{
	...
	do {
		ctrl_reg = getreg32(MPFS_IHC_CTRL(g_work_arg.mhartid, g_work_arg.rhartid));
	...
	modifyreg32(MPFS_IHC_CTRL(g_work_arg.mhartid, g_work_arg.rhartid), 0, ACK_INT);
}
#endif
...
static void mpfs_ihc_rx_message(ihc_channel_t channel, uint32_t mhartid, uint32_t *msg)
{
...
static void mpfs_ihc_message_present_isr(void)
{
	...
	uint32_t origin_hart = mpfs_ihc_parse_incoming_hartid(mhartid, &is_ack, &is_msg);
...
static void mpfs_ihc_local_remote_config(uint32_t hart_to_configure, uint32_t rhartid)
{
	/* Set-up enables in concentrator */

	putreg32(ihcia_remote_hart_ints[hart_to_configure], MPFS_IHC_INT_EN(hart_to_configure));

	modifyreg32(MPFS_IHC_CTRL(hart_to_configure, rhartid), 0, MPIE_EN | ACKIE_EN);
}
...
static const char *mpfs_rptun_get_cpuname(struct rptun_dev_s *dev)
{
	struct mpfs_rptun_dev_s *priv = container_of(dev, struct mpfs_rptun_dev_s, rptun);
	return priv->cpuname;
}
...
static struct rptun_rsc_s *mpfs_rptun_get_resource(struct rptun_dev_s *dev)
{
	...
	if (priv->shmem != NULL) {
		...
		rsc->rpmsg_vdev.dfeatures = 1 << VIRTIO_RPMSG_F_NS | 1 << VIRTIO_RPMSG_F_ACK;

		rsc->rpmsg_vdev.gfeatures = 1 << VIRTIO_RPMSG_F_NS | 1 << VIRTIO_RPMSG_F_ACK;
...
static int mpfs_echo_ping_ept_cb(struct rpmsg_endpoint *ept, void *data, size_t len, uint32_t src,
								 void *priv)
{
...
static int mpfs_echo_ping_init(struct rpmsg_device *rdev, struct rpmsg_endpoint *ept)
{
	return rpmsg_create_ept(ept, rdev, MPFS_RPTUN_PING_EPT_NAME, RPMSG_ADDR_ANY, 0,
...
{
	struct rpmsg_virtio_device *vdev = container_of(rdev, struct rpmsg_virtio_device, rdev);
...
#error Misconfiguration
	...
	if ((getreg32(MPFS_IHC_CTRL(CONTEXTA_HARTID, CONTEXTB_HARTID)) & (MPIE_EN | ACKIE_EN)) != 0) {
		g_shmem.master_up = true;
		g_shmem.rsc.rpmsg_vdev.status |= VIRTIO_CONFIG_STATUS_DRIVER_OK;
	}
	...
	ret = rpmsg_register_callback(NULL, mpfs_rpmsg_device_created, NULL, NULL, NULL);
	if (ret < 0) {
	...
	snprintf(arg1, sizeof(arg1), "%p", g_mpfs_virtqueue_table);
	argv[0] = "mpfs_ihc_thread";
	argv[1] = arg1;
	argv[2] = NULL;

	ret = kthread_create("mpfs-rptun", CONFIG_RPTUN_PRIORITY, CONFIG_RPTUN_STACKSIZE,
						 mpfs_rptun_thread, argv);
	if (ret < 0) {
		ihcerr("ERROR: Not able to create a thread!\n");
		rpmsg_unregister_callback(NULL, mpfs_rpmsg_device_created, NULL, NULL, NULL);
		goto init_error;
	}
#endif
