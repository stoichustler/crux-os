...
static void mpfs_set_rtc_divisor(void)
{
	...
	putreg32((LIBERO_SETTING_MSS_EXT_SGMII_REF_CLK / LIBERO_SETTING_MSS_RTC_TOGGLE_CLK),
...
static void mpfs_set_early_late_thresholds(uint8_t n_late_threshold, uint8_t p_early_threshold)
{
	...
	putreg32(((LIBERO_SETTING_CH0_CNTL & REG_RX0_EYEWIDTH_P_MASK) | p_eye_value) |
				 REG_RX0_EN_FLAG_N,
			 MPFS_CFG_DDR_SGMII_PHY_CH0_CNTL);
	putreg32(((LIBERO_SETTING_CH1_CNTL & REG_RX1_EYEWIDTH_P_MASK) | p_eye_value) |
				 REG_RX1_EN_FLAG_N,
...
__attribute__((section(".ram"))) void mpfs_mux_post_mss_pll_config(void)
{
	...
	while ((getreg32(MPFS_SYSREG_ENVM_CR) & ENVM_CR_CLOCK_OKAY_MASK) != ENVM_CR_CLOCK_OKAY_MASK)
		;
...
static int mpfs_pll_config(void)
{
	...
	while (!(getreg32(MPFS_IOSCB_MSS_PLL_CTRL) & PLL_CTRL_LOCK_BIT) && --retries)
		;
...
static void mpfs_sgmii_setup(void)
{
	...
	putreg32(LIBERO_SETTING_SGMII_MODE & ~(1 << 22), MPFS_CFG_DDR_SGMII_PHY_SGMII_MODE);
	...
	putreg32(SGMII_REG_PVT_SOFT_RESET_PERIPH | 0x7f, MPFS_CFG_DDR_SGMII_PHY_DYN_CNTL);
	...
	modifyreg32(MPFS_IOSCB_CALIB_SGMII_IOC_REG0, 0, SGMII_IOC_REG0_REG_CALIB_LOCK);
	...
	while (!((getreg32(MPFS_CFG_DDR_SGMII_PHY_PLL_CNTL) & SGMII_ARO_PLL0_LOCK)))
		;
	...
	modifyreg32(MPFS_SYSREG_SOFT_RESET_CR, SYSREG_SOFT_RESET_CR_MAC0 | SYSREG_SOFT_RESET_CR_MAC1,
				0);
	...
	uint32_t sro_dll_90_code = ((getreg32(MPFS_CFG_DDR_SGMII_PHY_RECAL_CNTL) >> 16) & 0x7f);
	...
	if (getreg32(MPFS_CFG_DDR_SGMII_PHY_SPARE_STAT) & (1 << 31)) {
		...
		mpfs_set_early_late_thresholds(LATE_EYE_WIDTH_PART_REVC_OR_LATER_PRE_TEST,
		...
		if (sro_dll_90_code < MIN_DLL_90_CODE_VALUE_INDICATING_TT_PART_REVB) {
			...
			mpfs_set_early_late_thresholds(LATE_TT_PART_REVB, EARLY_TT_PART_REVB);
		}
	}
	...
	putreg32(SGMII_REG_LANE1_SOFT_RESET_PERIPH | SGMII_REG_LANE0_SOFT_RESET_PERIPH | 0x7f,
