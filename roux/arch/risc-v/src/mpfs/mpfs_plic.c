...
uintptr_t mpfs_plic_get_iebase(uintptr_t hartid)
{
	...
	if (hartid == 0) {
		...
		iebase = MPFS_PLIC_H1_MIE0 + MPFS_PLIC_IEPRIV_OFFSET + (hartid - 1) * MPFS_HART_MIE_OFFSET;
	}
...
uintptr_t get_thresholdbase(uintptr_t hartid)
{
	...
	if (hartid == 0) {
		...
		threshold_address = MPFS_PLIC_H1_MTHRESHOLD + MPFS_PLIC_THRESHOLDPRIV_OFFSET +
							(hartid - 1) * MPFS_PLIC_NEXTHART_OFFSET;
	}
