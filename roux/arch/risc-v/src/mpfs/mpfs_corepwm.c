...
static int pwm_timer(struct mpfs_pwmtimer_s *priv, const struct pwm_info_s *info);
...
static void pwm_dumpregs(struct mpfs_pwmtimer_s *priv, const char *msg)
{
	...
	for (int i = 0; i < priv->nchannels; i++) {
		pwminfo("  PWM%d_POSEDGE: %s%08x PWM%d_NEGEDGE: %s%08x\n", i + 1, (i < 9) ? " " : "",
				pwm_getreg(priv, MPFS_COREPWM_PWM1_POS_EDGE_OFFSET + i * MPFS_PWMREG_STEP), i + 1,
				(i < 9) ? " " : "",
				pwm_getreg(priv, MPFS_COREPWM_PWM1_NEG_EDGE_OFFSET + i * MPFS_PWMREG_STEP));
	}
}
#endif
...
static int pwm_timer(struct mpfs_pwmtimer_s *priv, const struct pwm_info_s *info)
{
	...
	pwminfo("PWM%u frequency: %u PWMCLK: %lu prescaler: %u\n", priv->pwmid, info->frequency,
			MPFS_FPGA_PERIPHERAL_CLK, prescaler);
	...
	for (i = 0; i < CONFIG_PWM_NCHANNELS; i++) {
		...
		const int neg_edge_reg_offset =
			...
			(MPFS_COREPWM_PWM2_NEG_EDGE_OFFSET - MPFS_COREPWM_PWM1_NEG_EDGE_OFFSET) * (channel - 1);
		...
		if (channel <= 8) {
			...
			pwm_putreg(priv, MPFS_COREPWM_PWM_ENABLE_0_7_OFFSET, reg | (1 << (channel - 1)));
		} else {
			uint32_t reg = pwm_getreg(priv, MPFS_COREPWM_PWM_ENABLE_8_15_OFFSET);
			pwm_putreg(priv, MPFS_COREPWM_PWM_ENABLE_8_15_OFFSET, reg | (1 << (channel - 9)));
		}
	}
...
static int pwm_update_duty(struct mpfs_pwmtimer_s *priv, uint8_t channel, ub16_t duty16)
{
	uint32_t period;
	uint32_t neg_edge;
	...
	if (channel == 0 || channel > priv->nchannels || channel > MPFS_MAX_PWM_CHANNELS) {
...
static int pwm_init(struct mpfs_pwmtimer_s *priv)
{
	...
	modifyreg32(MPFS_SYSREG_SOFT_RESET_CR, SYSREG_SOFT_RESET_CR_FIC3 | SYSREG_SOFT_RESET_CR_FPGA,
				0);
