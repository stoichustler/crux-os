...
extern const uint8_t __mpfs_roux_start[];
extern const uint8_t __mpfs_roux_end[];
...
static int mpfs_opensbi_ecall_handler(long funcid, const struct sbi_trap_regs *regs,
...
static const struct sbi_platform platform = {.opensbi_version = OPENSBI_VERSION,
...
sbi_scratch_holder_t g_scratches[MPFS_MAX_NUM_HARTS] __attribute__((section(".l2_scratchpad")));
...
static void mpfs_opensbi_scratch_setup(uint32_t hartid)
{
	...
	g_scratches[hartid].scratch.hartid_to_scratch = (unsigned long)mpfs_hart_to_scratch;
	...
	g_scratches[hartid].scratch.fw_size = (unsigned long)_esbi_ram - (unsigned long)_ssbi_ram;

	g_scratches[hartid].scratch.fw_rw_offset = (unsigned long)g_scratches[hartid].scratch.fw_size;
	...
	g_scratches[hartid].scratch.fw_size = g_scratches[hartid].scratch.fw_rw_offset;

	g_scratches[hartid].scratch.fw_heap_offset = (unsigned long)g_scratches[hartid].scratch.fw_size;
	...
	g_scratches[hartid].scratch.fw_size =
		g_scratches[hartid].scratch.fw_heap_offset + g_scratches[hartid].scratch.fw_heap_size;
}
									  ...
									  unsigned long *out_val, struct sbi_trap_info *out_trap)
{
	...
	case SBI_EXT_IHC_RECEIVE:
		result = mpfs_ihc_sbi_ecall_handler(funcid, remote_channel, message_ptr);
		break;
...
void __attribute__((noreturn)) mpfs_opensbi_setup(uintptr_t hartid, uintptr_t next_addr)
{
	size_t i;
