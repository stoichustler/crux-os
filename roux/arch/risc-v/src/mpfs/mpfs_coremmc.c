...
#define COREMMC_ALL_ICR (COREMMC_ICR_CLRCSI | COREMMC_ICR_CLRRRI | COREMMC_ICR_ERROR)

/* Event wait interrupt mask bits */

#define COREMMC_RECV_MASK (COREMMC_IMR_ERROR)

#define COREMMC_RECV_SB_MASK (COREMMC_SBIMR_ERROR | COREMMC_SBIMR_RDONE)

#define COREMMC_RECV_MB_MASK (COREMMC_MBIMR_ERROR | COREMMC_MBIMR_RDONE)

#define COREMMC_SEND_MASK (COREMMC_IMR_ERROR)

#define COREMMC_SEND_SB_MASK (COREMMC_SBIMR_ERROR | COREMMC_SBIMR_WDONE)

#define COREMMC_SEND_MB_MASK (COREMMC_MBIMR_ERROR | COREMMC_MBIMR_WDONE)
...
static int mpfs_coremmc_wrcomplete_interrupt(int irq, void *context, void *arg);
#endif
...
struct mpfs_dev_s g_coremmc_dev = {
	...
	.lock = SP_UNLOCKED,
#ifdef CONFIG_MMCSD_SDIOWAIT_WRCOMPLETE
	.wrcomplete_irq = MPFS_IRQ_FABRIC_F2H_0 + CONFIG_MPFS_COREMMC_WRCOMPLETE_IRQNUM,
#endif
	.blocksize = 512,
	.fifo_depth = 0,
	.onebit = false,
...
static inline void mpfs_putreg32(struct mpfs_dev_s *priv, uint32_t regval, uintptr_t regaddr)
{
	DEBUGASSERT((regaddr >= priv->hw_base) && regaddr < (priv->hw_base + MPFS_COREMMC_MAX_OFFSET));
...
static inline void mpfs_putreg8(struct mpfs_dev_s *priv, uint8_t regval, uintptr_t regaddr)
{
	DEBUGASSERT((regaddr >= priv->hw_base) && regaddr < (priv->hw_base + MPFS_COREMMC_MAX_OFFSET));
...
static void mpfs_configxfrints(struct mpfs_dev_s *priv, uint32_t xfrmask, uint32_t xfr_blkmask)
{
	irqstate_t flags;
...
static void mpfs_endtransfer(struct mpfs_dev_s *priv, sdio_eventset_t wkupevent)
{
	...
	if (priv->multiblock) {
		mpfs_putreg8(priv, COREMMC_MBICR_RDONE | COREMMC_MBICR_WDONE, MPFS_COREMMC_MBICR);
	} else {
		mpfs_putreg8(priv, COREMMC_SBICR_RDONE | COREMMC_SBICR_WDONE, MPFS_COREMMC_SBICR);
	}
...
static bool mpfs_device_reset(struct sdio_dev_s *dev)
{
	...
	modifyreg32(MPFS_SYSREG_SOFT_RESET_CR, SYSREG_SOFT_RESET_CR_FPGA | SYSREG_SOFT_RESET_CR_FIC0,
				0);
	...
	mpfs_modifyreg8(MPFS_COREMMC_CTRL, 0, COREMMC_CTRL_SLRST | COREMMC_CTRL_SWRST);

	nxsig_usleep(100);

	mpfs_modifyreg8(MPFS_COREMMC_CTRL, COREMMC_CTRL_SLRST | COREMMC_CTRL_SWRST, 0);
	...
	mpfs_modifyreg8(MPFS_COREMMC_DCTRL, COREMMC_DCTRL_DSIZE, COREMMC_DCTRL_DSIZE_1BIT);
...
static void mpfs_set_4bit_mode(struct sdio_dev_s *dev)
{
	...
	if (ret < 0) {
		...
		mpfs_modifyreg8(MPFS_COREMMC_DCTRL, COREMMC_DCTRL_DSIZE, COREMMC_DCTRL_DSIZE_4BIT);
	}
}
...
#ifdef CONFIG_MMCSD_SDIOWAIT_WRCOMPLETE
		ret = irq_attach(priv->wrcomplete_irq, mpfs_coremmc_wrcomplete_interrupt, priv);
...
static int mpfs_recvsetup(struct sdio_dev_s *dev, uint8_t *buffer, size_t nbytes)
{
	...
	if (priv->multiblock) {
		mpfs_putreg8(priv, COREMMC_MBICR_RDONE | COREMMC_MBICR_ERROR, MPFS_COREMMC_MBICR);
	} else {
		mpfs_putreg8(priv, COREMMC_SBICR_RDONE | COREMMC_SBICR_ERROR, MPFS_COREMMC_SBICR);
	}
...
static int mpfs_sendsetup(struct sdio_dev_s *dev, const uint8_t *buffer, size_t nbytes)
{
	...
	if (priv->multiblock) {
		mpfs_putreg8(priv, COREMMC_MBICR_WDONE | COREMMC_MBICR_ERROR, MPFS_COREMMC_MBICR);
	} else {
		mpfs_putreg8(priv, COREMMC_SBICR_WDONE | COREMMC_SBICR_ERROR, MPFS_COREMMC_SBICR);
	}
...
static int mpfs_cancel(struct sdio_dev_s *dev)
{
	...
	mpfs_putreg8(priv, COREMMC_ALL_ICR, MPFS_COREMMC_ICR);
	mpfs_putreg8(priv, MPFS_COREMMC_SR_EBOD | MPFS_COREMMC_SR_EBUD | MPFS_COREMMC_SR_ECRD,
				 MPFS_COREMMC_SR);

	if (priv->multiblock) {
		mpfs_putreg8(priv, COREMMC_MBICR_ERROR | COREMMC_MBICR_RDONE | COREMMC_MBICR_WDONE,
					 MPFS_COREMMC_MBICR);
	} else {
		mpfs_putreg8(priv, COREMMC_SBICR_ERROR | COREMMC_SBICR_RDONE | COREMMC_SBICR_WDONE,
					 MPFS_COREMMC_SBICR);
	}
...
static int mpfs_waitresponse(struct sdio_dev_s *dev, uint32_t cmd)
{
	...
	do {
		status = getreg8(MPFS_COREMMC_ISR);
	} while (!(status & (COREMMC_ISR_RRI | COREMMC_ISR_ERROR)) && --timeout);
...
static int mpfs_check_recverror(struct mpfs_dev_s *priv)
{
	...
	} while (!(regval & (MPFS_COREMMC_SR_RDRE)) && --timeout);
...
static int mpfs_recvlong(struct sdio_dev_s *dev, uint32_t cmd, uint32_t rlong[4])
{
	...
	if (rlong) {
		rlong[0] = getreg8(MPFS_COREMMC_RR0) << 24 | getreg8(MPFS_COREMMC_RR1) << 16 |
				   getreg8(MPFS_COREMMC_RR2) << 8 | getreg8(MPFS_COREMMC_RR3);
		rlong[1] = getreg8(MPFS_COREMMC_RR4) << 24 | getreg8(MPFS_COREMMC_RR5) << 16 |
				   getreg8(MPFS_COREMMC_RR6) << 8 | getreg8(MPFS_COREMMC_RR7);
		rlong[2] = getreg8(MPFS_COREMMC_RR8) << 24 | getreg8(MPFS_COREMMC_RR9) << 16 |
				   getreg8(MPFS_COREMMC_RR10) << 8 | getreg8(MPFS_COREMMC_RR11);
		rlong[3] = getreg8(MPFS_COREMMC_RR12) << 24 | getreg8(MPFS_COREMMC_RR13) << 16 |
				   getreg8(MPFS_COREMMC_RR14) << 8 | getreg8(MPFS_COREMMC_RR15);
