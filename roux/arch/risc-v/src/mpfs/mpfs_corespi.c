...
#define MPFS_CORESPI_IRQNUM (MPFS_IRQ_FABRIC_F2H_0 + CONFIG_MPFS_CORESPI_IRQNUM)
...
static inline void mpfs_spi_write_tx8(struct mpfs_spi_priv_s *priv, unsigned nframes, bool last)
{
...
static void mpfs_spi_load_tx_fifo(struct mpfs_spi_priv_s *priv, uint32_t nwords)
{
	uint16_t *data16;
	int i;
...
static inline void mpfs_spi_read_rx8(struct mpfs_spi_priv_s *priv, unsigned nframes)
{
...
static void mpfs_spi_unload_rx_fifo(struct mpfs_spi_priv_s *priv, uint32_t nwords)
{
	uint16_t *data16;
	int i;
	unsigned frames_per_word;
								  ...
								  void *rxbuffer, uint32_t nwords)
{
	...
	putreg32(MPFS_SPI_TXCHUNDRUN | MPFS_SPI_RXCHOVRFLW | MPFS_SPI_DATA_RX | MPFS_SPI_TXDONE,
			 MPFS_SPI_INT_CLEAR);
	...
	modifyreg32(MPFS_SPI_CONTROL, 0,
				MPFS_SPI_INTTXTURUN | MPFS_SPI_INTRXOVRFLOW | MPFS_SPI_INTTXDONE);
	...
	modifyreg32(MPFS_SPI_CONTROL, MPFS_SPI_INTTXTURUN | MPFS_SPI_INTRXOVRFLOW | MPFS_SPI_INTTXDONE,
				0);
...
void weak_function mpfs_corespi_select(struct spi_dev_s *dev, uint32_t devid, bool selected)
{
#ifdef CONFIG_SPI_CS_CONTROL
