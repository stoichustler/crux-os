...
static uint8_t g_channel_nextcfg_wsize[MPFS_DMA_NUM_CHANNELS] = {
	DMA_MAX_TRANSACTION_SIZE, DMA_MAX_TRANSACTION_SIZE, DMA_MAX_TRANSACTION_SIZE,
...
int mpfs_dma_setup_transfer(unsigned int channel, struct mpfs_dma_channel_config *config)
{
	...
	if (config->enable_done_int) {
		...
		modifyreg32(MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_CONTROL_OFFSET, 0, DMA_CONTROL_DONEIE);
	} else {
		dmainfo("disable done irq\n");
		modifyreg32(MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_CONTROL_OFFSET, DMA_CONTROL_DONEIE_MASK,
					0);
	}

	if (config->enable_err_int) {
		dmainfo("enable err irq\n");
		modifyreg32(MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_CONTROL_OFFSET, 0, DMA_CONTROL_ERRORIE);
	} else {
	...
	modifyreg32(MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_CONTROL_OFFSET, 0, DMA_CONTROL_CLAIM);
	...
	putreg64(config->dest_addr, MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_NEXT_DESTINATION_OFFSET);
	putreg64(config->src_addr, MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_NEXT_SOURCE_OFFSET);

	/* Set the transfer size. */

	putreg64(config->num_bytes, MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_NEXT_BYTES_OFFSET);

	/* Setup repeat and force order requirements. */

	if (config->repeat) {
		modifyreg32(MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_NEXT_CONFIG_OFFSET, 0,
					DMA_NEXT_CONFIG_REPEAT);
	} else {
...
int mpfs_dma_set_transaction_size(unsigned int channel, uint8_t write_size, uint8_t read_size)
{
	if (channel >= MPFS_DMA_NUM_CHANNELS) {
		dmawarn("Illegal channel\n");
		return -EINVAL;
...
int mpfs_dma_start(unsigned int channel)
{
	...
	modifyreg32(MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_CONTROL_OFFSET, 0, DMA_CONTROL_RUN);
...
uint64_t mpfs_dma_get_current_destination(unsigned int channel)
{
	...
	return getreg64(MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_EXEC_DESTINATION_OFFSET);
}
...
uint64_t mpfs_dma_get_current_source(unsigned int channel)
{
	...
	return getreg64(MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_EXEC_SOURCE_OFFSET);
}
...
int mpfs_dma_clear_complete_status(unsigned int channel)
{
	...
	modifyreg32(MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_CONTROL_OFFSET, DMA_CONTROL_DONE_MASK, 0);
...
int mpfs_dma_clear_error_status(unsigned int channel)
{
	...
	modifyreg32(MPFS_DMA_REG_OFFSET(channel) + MPFS_DMA_CONTROL_OFFSET, DMA_CONTROL_ERROR, 0);
