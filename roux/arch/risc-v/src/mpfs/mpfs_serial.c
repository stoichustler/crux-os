...
static void up_enable_uart(struct up_dev_s *priv, bool enable)
{
	...
	modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_SOFT_RESET_CR_OFFSET, 0, reset_bit);

	if (enable) {
		/* reset off */

		modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_SUBBLK_CLOCK_CR_OFFSET, 0, reset_bit);

		/* clock on */

		modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_SOFT_RESET_CR_OFFSET, clock_bit, 0);
	} else {
		/* clock off */

		modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_SUBBLK_CLOCK_CR_OFFSET, clock_bit, 0);
	}
}
...
static int up_setup(struct uart_dev_s *dev)
{
	...
	up_serialout(priv, MPFS_UART_FCR_OFFSET, (UART_FCR_RFIFOR | UART_FCR_XFIFOR));
...
static int up_ioctl(struct file *filep, int cmd, unsigned long arg)
{
	...
	case TCGETS: {
		...
		termiosp->c_cflag = ((priv->parity != 0) ? PARENB : 0) |
							((priv->parity == 1) ? PARODD : 0) | ((priv->stopbits2) ? CSTOPB : 0) |
							((priv->bits == 5) ? CS5 : 0) | ((priv->bits == 6) ? CS6 : 0) |
							((priv->bits == 7) ? CS7 : 0) | ((priv->bits == 8) ? CS8 : 0);
...
static void up_send(struct uart_dev_s *dev, int ch)
{
	...
	while ((up_serialin(priv, MPFS_UART_LSR_OFFSET) & UART_LSR_THRE) == 0)
		;
