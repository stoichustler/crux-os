...
#include <roux/net/mii.h>
#include <roux/net/gmii.h>
...
#if defined(CONFIG_MPFS_ETH0_PHY_KSZ9477) || defined(CONFIG_MPFS_ETH1_PHY_KSZ9477)
...
#error "CONFIG_MPFS_PHYADDR must be defined in the ROUX configuration"
#endif
...
#define INT_ERRORS                                                                                 \
	(GEM_INT_TRANSMIT_BUFFER_UNDERRUN | GEM_INT_RECEIVE_OVERRUN | GEM_INT_AMBA_ERROR |             \
	 GEM_INT_RESP_NOT_OK)
...
static uint8_t g_pktbuf[MPFS_NETHERNET][MAX_NETDEV_PKTSIZE + CONFIG_NET_GUARDSIZE];

#if defined(CONFIG_MPFS_GMAC_PREALLOCATE)
static uint8_t g_txbuffer[CONFIG_MPFS_ETHMAC_NTXBUFFERS * GMAC_TX_UNITSIZE] aligned_data(8);
static uint8_t g_rxbuffer[CONFIG_MPFS_ETHMAC_NRXBUFFERS * GMAC_RX_UNITSIZE] aligned_data(8);
#endif
...
#endif
#ifdef CONFIG_MPFS_ETHMAC_1
	{MPFS_IRQ_MAC1_INT, MPFS_IRQ_MAC1_QUEUE1, MPFS_IRQ_MAC1_QUEUE2, MPFS_IRQ_MAC1_QUEUE3},
#endif
};
...
static int mpfs_buffer_initialize(struct mpfs_ethmac_s *priv, unsigned int queue);
...
static int mpfs_phyread(struct mpfs_ethmac_s *priv, uint8_t phyaddr, uint8_t regaddr,
						uint16_t *phyval);
static int mpfs_phywrite(struct mpfs_ethmac_s *priv, uint8_t phyaddr, uint8_t regaddr,
						 uint16_t phyval);
...
#if defined(CONFIG_DEBUG_NET) && defined(CONFIG_DEBUG_INFO) && defined(ETH_HAS_MDIO_PHY)
...
static inline void mac_putreg(struct mpfs_ethmac_s *priv, uint32_t offset, uint32_t val)
{
...
static inline uint32_t mac_getreg(struct mpfs_ethmac_s *priv, uint32_t offset)
{
	uintptr_t addr = (uintptr_t)(priv->regbase + offset);
...
static int mpfs_interrupt_0(int irq, void *context, void *arg)
{
	...
	*priv->queue[0].int_status = isr;
	...
	if ((isr & INT_RX) != 0) {
		...
		wd_start(&priv->rxtimeout, MPFS_RXTIMEOUT, mpfs_txtimeout_expiry, (wdparm_t)priv);
	}
#endif
...
static void mpfs_txreset(struct mpfs_ethmac_s *priv)
{
	...
	for (qi = 0; qi < MPFS_MAC_QUEUE_COUNT; qi++) {
		...
		txdesc[CONFIG_MPFS_ETHMAC_NTXBUFFERS - 1].status = GEM_TX_DMA_USED | GEM_TX_DMA_WRAP;
	...
	*priv->queue[0].tx_q_ptr = lower_32_bits((uintptr_t)priv->queue[0].tx_desc_tab);
}
...
static void mpfs_rxreset(struct mpfs_ethmac_s *priv)
{
	...
	*priv->queue[0].rx_q_ptr = lower_32_bits((uintptr_t)priv->queue[0].rx_desc_tab);
...
static int mpfs_rmmac(struct net_driver_s *dev, const uint8_t *mac)
{
	...
	if (regval == 0 && mac_getreg(priv, regaddr2) == 0) {
		...
		regval = mac_getreg(priv, NETWORK_CONFIG);
		regval &= ~(NETWORK_CONFIG_UNICAST_HASH_ENABLE | NETWORK_CONFIG_MULTICAST_HASH_ENABLE);
		mac_putreg(priv, NETWORK_CONFIG, regval);
	}
...
static void mpfs_enablemdio(struct mpfs_ethmac_s *priv)
{
	...
	regval = mac_getreg(priv, NETWORK_CONTROL);
	enables = regval & (NETWORK_CONTROL_ENABLE_RECEIVE | NETWORK_CONTROL_ENABLE_TRANSMIT);

	regval &= ~(NETWORK_CONTROL_ENABLE_RECEIVE | NETWORK_CONTROL_ENABLE_TRANSMIT);
						...
						uint16_t *phyval)
{
	...
	regval = PHY_MANAGEMENT_PHY_DATA(0) | PHY_MANAGEMENT_WRITE10 |
			 PHY_MANAGEMENT_REG_ADDRESS(regaddr) | PHY_MANAGEMENT_PHY_ADDRESS(phyaddr) |
			 PHY_MANAGEMENT_OPERATION_READ | PHY_MANAGEMENT_WRITE_1;
	...
	*phyval = (uint16_t)(mac_getreg(priv, PHY_MANAGEMENT) & PHY_MANAGEMENT_PHY_DATA_MASK);
	return OK;
}
						 ...
						 uint16_t phyval)
{
	...
	regval = PHY_MANAGEMENT_PHY_DATA(phyval) | PHY_MANAGEMENT_WRITE10 |
			 PHY_MANAGEMENT_REG_ADDRESS(regaddr) | PHY_MANAGEMENT_PHY_ADDRESS(phyaddr) |
			 PHY_MANAGEMENT_OPERATION_WRITE | PHY_MANAGEMENT_WRITE_1;
	mac_putreg(priv, PHY_MANAGEMENT, regval);
...
static int mpfs_autonegotiate(struct mpfs_ethmac_s *priv)
{
	...
	advertise = GMII_ADVERTISE_100BASETXFULL | GMII_ADVERTISE_100BASETXHALF |
				GMII_ADVERTISE_10BASETXFULL | GMII_ADVERTISE_10BASETXHALF | GMII_ADVERTISE_8023;
	...
	for (;;) {
		...
		if ((btsr & GMII_1000BTSR_LP1000BASETFULL) != 0 &&
			...
			ninfo("Link: FD - 1000\n");
			linkmode = NETWORK_CONFIG_FULL_DUPLEX | NETWORK_CONFIG_GIGABIT_MODE_ENABLE;
			break;
	...
	mac_putreg(priv, NETWORK_CONTROL,
			   control & ~(NETWORK_CONTROL_ENABLE_RECEIVE | NETWORK_CONTROL_ENABLE_TRANSMIT));
	...
	regval = mac_getreg(priv, NETWORK_CONFIG);
	regval &=
		~(NETWORK_CONFIG_SPEED | NETWORK_CONFIG_FULL_DUPLEX | NETWORK_CONFIG_GIGABIT_MODE_ENABLE);
	regval |= linkmode;
	mac_putreg(priv, NETWORK_CONFIG, regval);
...
static void mpfs_linkspeed(struct mpfs_ethmac_s *priv)
{
	...
	mac_putreg(priv, NETWORK_CONTROL,
			   ncr & ~(NETWORK_CONTROL_ENABLE_RECEIVE | NETWORK_CONTROL_ENABLE_TRANSMIT));
...
static int mpfs_buffer_initialize(struct mpfs_ethmac_s *priv, unsigned int queue)
{
	...
	priv->queue[queue].tx_desc_tab = (struct gmac_txdesc_s *)kmm_memalign(8, allocsize);
...
static int mpfs_transmit(struct mpfs_ethmac_s *priv, unsigned int queue)
{
	...
	ninfo("d_len: %d txhead: %d txtail: %d\n", dev->d_len, priv->queue[queue].txhead,
		  priv->queue[queue].txtail);
	...
	wd_start(&priv->txtimeout, MPFS_TXTIMEOUT, mpfs_txtimeout_expiry, (wdparm_t)priv);
...
static void mpfs_ethreset(struct mpfs_ethmac_s *priv)
{
	...
	if (priv->regbase == MPFS_GEM0_LO_BASE) {
		...
		modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_SOFT_RESET_CR_OFFSET, SYSREG_SOFT_RESET_CR_MAC0,
					0);
	}
	...
	if (priv->regbase == MPFS_GEM1_LO_BASE) {
		...
		modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_SOFT_RESET_CR_OFFSET, 0,
					SYSREG_SOFT_RESET_CR_MAC1);
		modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_SOFT_RESET_CR_OFFSET, SYSREG_SOFT_RESET_CR_MAC1,
					0);
	}
...
static int mpfs_macconfig(struct mpfs_ethmac_s *priv)
{
	...
	net_config =
		(((uint32_t)1) << NETWORK_CONFIG_DATA_BUS_WIDTH_SHIFT) |
	...
	dma_config = (MPFS_MAC_RX_BUF_VALUE << DMA_CONFIG_RX_BUF_SIZE_SHIFT) | DMA_CONFIG_TX_PBUF_SIZE |
	...
	mac_putreg(priv, TRANSMIT_Q1_PTR, ((uint32_t)(uint64_t)priv->queue[0].tx_desc_tab) | 1U);
	mac_putreg(priv, TRANSMIT_Q2_PTR, ((uint32_t)(uint64_t)priv->queue[0].tx_desc_tab) | 1U);
	mac_putreg(priv, TRANSMIT_Q3_PTR, ((uint32_t)(uint64_t)priv->queue[0].tx_desc_tab) | 1U);
	mac_putreg(priv, RECEIVE_Q1_PTR, ((uint32_t)(uint64_t)priv->queue[0].rx_desc_tab) | 1U);
	mac_putreg(priv, RECEIVE_Q2_PTR, ((uint32_t)(uint64_t)priv->queue[0].rx_desc_tab) | 1U);
	mac_putreg(priv, RECEIVE_Q3_PTR, ((uint32_t)(uint64_t)priv->queue[0].rx_desc_tab) | 1U);
...
static void mpfs_mdcclock(struct mpfs_ethmac_s *priv)
{
	...
	ncfgr &= ~(NETWORK_CONFIG_MDC_CLOCK_DIVISOR_MASK << NETWORK_CONFIG_MDC_CLOCK_DIVISOR_SHIFT);
...
static int mpfs_phyreset(struct mpfs_ethmac_s *priv)
{
	...
	ret = mpfs_phywrite(priv, priv->phyaddr, GMII_MCR, GMII_MCR_RESET);
	if (ret < 0) {
	...
	for (timeout = 0; timeout < PHY_RESET_WAIT_COUNT; timeout++) {
		mcr = GMII_MCR_RESET;
		int result = mpfs_phyread(priv, priv->phyaddr, GMII_MCR, &mcr);
		if (result < 0) {
	...
	(!defined(CONFIG_MPFS_MAC_AUTONEG) && !defined(CONFIG_MPFS_MAC_ETH1000MBPS))
