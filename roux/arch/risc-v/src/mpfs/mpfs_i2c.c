...
static const uint32_t mpfs_i2c_freqs[MPFS_I2C_NUMBER_OF_DIVIDERS] = {
	MPFS_MSS_APB_AHB_CLK / 256, MPFS_MSS_APB_AHB_CLK / 224, MPFS_MSS_APB_AHB_CLK / 192,
	MPFS_MSS_APB_AHB_CLK / 160, MPFS_MSS_APB_AHB_CLK / 960, MPFS_MSS_APB_AHB_CLK / 120,
	MPFS_MSS_APB_AHB_CLK / 60,	MPFS_FPGA_BCLK / 8};

static const uint32_t mpfs_i2c_freqs_fpga[MPFS_I2C_NUMBER_OF_DIVIDERS] = {
	MPFS_FPGA_PERIPHERAL_CLK / 256, MPFS_FPGA_PERIPHERAL_CLK / 224,
	MPFS_FPGA_PERIPHERAL_CLK / 192, MPFS_FPGA_PERIPHERAL_CLK / 160,
	MPFS_FPGA_PERIPHERAL_CLK / 960, MPFS_FPGA_PERIPHERAL_CLK / 120,
	MPFS_FPGA_PERIPHERAL_CLK / 60,	MPFS_FPGA_BCLK / 8};

static int mpfs_i2c_irq(int cpuint, void *context, void *arg);

static int mpfs_i2c_transfer(struct i2c_master_s *dev, struct i2c_msg_s *msgs, int count);
...
static struct mpfs_i2c_priv_s g_mpfs_i2c0_lo_priv = {.ops = &mpfs_i2c_ops,
													 .id = 0,
...
static struct mpfs_i2c_priv_s g_mpfs_i2c1_lo_priv = {.ops = &mpfs_i2c_ops,
													 .id = 1,
...
static struct mpfs_i2c_priv_s g_mpfs_corei2c_priv[CONFIG_MPFS_COREI2C_INSTANCES] = {
	{
		.lock = NXMUTEX_INITIALIZER,
	},
...
static int mpfs_i2c_setfrequency(struct mpfs_i2c_priv_s *priv, uint32_t frequency);
...
static int mpfs_i2c_init(struct mpfs_i2c_priv_s *priv)
{
	...
	if (!priv->initialized) {
		...
		if (priv->fpga) {
			...
			modifyreg32(MPFS_SYSREG_SUBBLK_CLOCK_CR, 0, SYSREG_SUBBLK_CLOCK_CR_I2C0);
		} else if (priv->id == 1) {
			...
			modifyreg32(MPFS_SYSREG_SUBBLK_CLOCK_CR, 0, SYSREG_SUBBLK_CLOCK_CR_I2C1);
		} else {
...
static int mpfs_i2c_irq(int cpuint, void *context, void *arg)
{
	...
	case MPFS_I2C_ST_RESTART:
		...
		putreg32((priv->target_addr << 1) | (msg->flags & I2C_M_READ), MPFS_I2C_DATA);
	...
	if (priv->frequency != frequency) {
		...
		if (priv->fpga) {
			...
			for (uint8_t i = 0; i < MPFS_I2C_NUMBER_OF_DIVIDERS; i++) {
				if (frequency >= mpfs_i2c_freqs_fpga[i] && mpfs_i2c_freqs_fpga[i] > new_freq) {
			...
			for (uint8_t i = 0; i < MPFS_I2C_NUMBER_OF_DIVIDERS; i++) {
				if (frequency >= mpfs_i2c_freqs[i] && mpfs_i2c_freqs[i] > new_freq) {
		...
		i2cinfo("Changing I2Cbus %" PRIu8 " clock speed to %" PRIu32 " (div %" PRIx32 ")\n",
				priv->id, new_freq, clock_div);

		/* Update the clock divider */

		modifyreg32(MPFS_I2C_CTRL,
					MPFS_I2C_CTRL_CR2_MASK | MPFS_I2C_CTRL_CR1_MASK | MPFS_I2C_CTRL_CR0_MASK,
...
struct i2c_master_s *mpfs_i2cbus_initialize(int port)
{
	...
	priv->id = port;
	priv->hw_base = CONFIG_MPFS_COREI2C_BASE + port * CONFIG_MPFS_COREI2C_INST_OFFSET;
