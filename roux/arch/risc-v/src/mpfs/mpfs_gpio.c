...
static struct gpio_callback_s g_mss_gpio_callbacks[GPIO_BANK0_NUM_PINS + GPIO_BANK1_NUM_PINS];
...
int mpfs_configgpio(gpio_pinset_t cfgset)
{
	...
	if (bank == 0 || bank == 1) {
		...
		modifyreg32(baseaddr, MSSIO_MUX_MASK(pin), mux << MSSIO_MUX_SHIFT(pin));

		/* Set EC configuration for MSSIO pin */

		baseaddr = MSSIO_IO_CFG_CR(bank, pin);
		modifyreg32(baseaddr, MSSIO_IO_CFG_CR_MASK(pin), ec << MSSIO_IO_CFG_CR_SHIFT(pin));
	} else {
...
int mpfs_gpiosetevent(gpio_pinset_t pinset, bool risingedge, bool fallingedge, bool high, bool low,
					  bool event, xcpt_t func, void *arg)
{
	...
	cb = bank == 2 ? &g_fab_gpio_callbacks[irq_n] : &g_mss_gpio_callbacks[irq_n];
	...
	if (bank == 2) {
		if (event) {
			modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_GPIO_INTERRUPT_FAB_CR_OFFSET, 0,
						SYSREG_GPIO_INTERRUPT_FAB_CR(pin));
		} else {
			modifyreg32(MPFS_SYSREG_BASE + MPFS_SYSREG_GPIO_INTERRUPT_FAB_CR_OFFSET,
