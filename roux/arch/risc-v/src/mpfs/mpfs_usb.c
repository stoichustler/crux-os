...
static int mpfs_ep_configure(struct usbdev_ep_s *ep, const struct usb_epdesc_s *desc, bool last);
...
#endif
static void mpfs_ep_freereq(struct usbdev_ep_s *ep, struct usbdev_req_s *);
...
static struct usbdev_ep_s *mpfs_allocep(struct usbdev_s *dev, uint8_t epno, bool in,
										uint8_t eptype);
...
static void mpfs_ep0_wrstatus(struct mpfs_usbdev_s *priv, const uint8_t *buffer, size_t buflen);
...
static const struct usb_epdesc_s g_ep0desc = {.len = USB_SIZEOF_EPDESC,
...
static void mpfs_modifyreg16(uintptr_t addr, uint16_t clearbits, uint16_t setbits)
{
	irqstate_t flags;
	uint16_t regval;

	DEBUGASSERT((addr >= MPFS_USB_BASE) && addr < (MPFS_USB_BASE + MPFS_USB_REG_MAX));
...
static void mpfs_modifyreg8(uintptr_t addr, uint8_t clearbits, uint8_t setbits)
{
	irqstate_t flags;
	uint8_t regval;
...
{
	DEBUGASSERT((regaddr >= MPFS_USB_BASE) && regaddr < (MPFS_USB_BASE + MPFS_USB_REG_MAX));
...
static void mpfs_req_enqueue(struct mpfs_rqhead_s *queue, struct mpfs_req_s *req)
{
	irqstate_t flags = spin_lock_irqsave(&queue->qlock);
...
static int mpfs_write_tx_fifo(const void *in_data, uint32_t length, uint8_t epno)
{
	uint32_t i;
	uint32_t *temp;
	uint8_t *temp_8bit;
...
static int mpfs_req_wrsetup(struct mpfs_usbdev_s *priv, struct mpfs_ep_s *privep,
							struct mpfs_req_s *privreq)
{
	const uint8_t *buf;
	uint32_t packetsize;
	...
	if (nbytes > packetsize) {
		...
		if (epno == EP0) {
			mpfs_modifyreg16(MPFS_USB_INDEXED_CSR_EP0_CSR0, 0, CSR0L_DEV_TX_PKT_RDY_MASK);
		} else {
			mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_CSR_OFFSET,
							 TXCSRL_REG_EPN_UNDERRUN_MASK, TXCSRL_REG_EPN_TX_PKT_RDY_MASK);
		}
...
static int mpfs_ep_stall(struct mpfs_ep_s *privep)
{
	...
	if ((privep->epstate != USB_EPSTATE_DISABLED) && (privep->epstate != USB_EPSTATE_STALLED)) {
		epno = USB_EPNO(privep->ep.eplog);
		...
		if (epno == EP0) {
			mpfs_putreg16(CSR0L_DEV_SEND_STALL_MASK | CSR0L_DEV_SERVICED_RX_PKT_RDY_MASK,
						  MPFS_USB_INDEXED_CSR_EP0_CSR0);
		} else if (USB_ISEPIN(privep->ep.eplog)) {
			mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_CSR_OFFSET, 0,
							 TXCSRL_REG_EPN_SEND_STALL_MASK);
		} else {
			mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_CSR_OFFSET, 0,
							 RXCSRL_REG_EPN_SEND_STALL_MASK | RXCSRL_REG_EPN_RX_PKT_RDY_MASK);
		}
	}
...
static int mpfs_req_write(struct mpfs_usbdev_s *priv, struct mpfs_ep_s *privep)
{
	struct mpfs_req_s *privreq;
	uint8_t epno;
	int bytesleft;
...
static int mpfs_req_read(struct mpfs_usbdev_s *priv, struct mpfs_ep_s *privep, uint16_t recvsize)
{
	struct mpfs_req_s *privreq;
	uint16_t reg;
	int epno;
	...
	uint16_t count = getreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_COUNT_OFFSET);
	...
	do {
		...
		if (privreq->req.len == 0) {
			...
			if (epno == EP0) {
				mpfs_putreg16(CSR0L_DEV_SERVICED_RX_PKT_RDY_MASK | CSR0L_DEV_DATA_END_MASK,
							  MPFS_USB_INDEXED_CSR_EP0_CSR0);
			} else {
				mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_CSR_OFFSET,
		...
		if ((privreq != NULL) && (privreq->inflight > 0) && (count != 0) &&
			...
			privreq->req.xfrd += mpfs_read_rx_fifo(privreq->req.buf, recvsize, epno);
...
static void mpfs_ep_set_fifo_size(uint8_t epno, uint8_t in, uint16_t fifo_size)
{
	uint16_t temp;
	uint8_t i = 0;
	...
	DEBUGASSERT(fifo_size == 8 || fifo_size == 16 || fifo_size == 32 || fifo_size == 64 ||
				fifo_size == 128 || fifo_size == 512 || fifo_size == 1024 || fifo_size == 2048 ||
				fifo_size == 4096);
...
static int mpfs_ep_configure_internal(struct mpfs_ep_s *privep, const struct usb_epdesc_s *desc)
{
	uint16_t maxpacket;
	uint8_t epno;
	uint8_t eptype;
	...
	uinfo("len: 0x%02x type: 0x%02x addr: 0x%02x attr: 0x%02x "
		  ...
		  desc->len, desc->type, desc->addr, desc->attr, desc->mxpacketsize[1],
		  desc->mxpacketsize[0], desc->interval);
	...
	if (dirin && epno) {
		...
		mpfs_putreg16(0, MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_CSR_OFFSET);

		mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_CSR_OFFSET,
						 TXCSRL_REG_EPN_UNDERRUN_MASK | TXCSRL_REG_EPN_SEND_STALL_MASK |
		...
		mpfs_putreg16(0, MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_MAX_P_OFFSET);
		mpfs_putreg16(MPFS_NUM_USB_PKT - 1,
					  MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_MAX_P_OFFSET);
		mpfs_putreg16((MPFS_NUM_USB_PKT - 1) << TX_MAX_P_REG_NUM_USB_PKT_SHIFT,
					  MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_MAX_P_OFFSET);

		mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_MAX_P_OFFSET, 0, maxpacket);
		...
		mpfs_putreg16(0, MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_CSR_OFFSET);

		mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_CSR_OFFSET,
						 RXCSRL_REG_EPN_OVERRUN_MASK | RXCSRL_REG_EPN_STALL_SENT_MASK |
		...
		mpfs_putreg16(0, MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_MAX_P_OFFSET);
		mpfs_putreg16(MPFS_NUM_USB_PKT - 1,
					  MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_MAX_P_OFFSET);
		mpfs_putreg16((MPFS_NUM_USB_PKT - 1) << RX_MAX_P_REG_NUM_USB_PKT_SHIFT,
					  MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_MAX_P_OFFSET);
		mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_MAX_P_OFFSET, 0, maxpacket);

		mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_CSR_OFFSET, 0,
						 RXCSRL_REG_EPN_CLR_DAT_TOG_MASK | RXCSRL_REG_EPN_RX_PKT_RDY_MASK);
	...
	case USB_EP_ATTR_XFER_BULK:
		if (dirin) {
			mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_CSR_OFFSET,
							 TXCSRH_REG_EPN_ENABLE_ISO_MASK, 0);
		} else {
			/* RXCSRL_REG_EPN_BI_DIS_NYET_MASK is enabled when cleared */

			mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_CSR_OFFSET,
							 RXCSRL_REG_EPN_ENABLE_ISO_MASK,
							 RXCSRL_REG_EPN_ENABLE_AUTOCLR_MASK | RXCSRL_REG_EPN_BI_DIS_NYET_MASK |
								 RXCSRL_REG_EPN_RX_PKT_RDY_MASK);
		}
		break;
	...
	case USB_EP_ATTR_XFER_INT:
		if (dirin) {
			mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_CSR_OFFSET,
							 TXCSRH_REG_EPN_ENABLE_ISO_MASK | TXCSRH_REG_EPN_ENABLE_AUTOSET_MASK,
							 0);
		} else {
			/* RXCSRL_REG_EPN_BI_DIS_NYET_MASK disabled when set */

			mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_CSR_OFFSET,
							 RXCSRL_REG_EPN_ENABLE_ISO_MASK | RXCSRL_REG_EPN_BI_DIS_NYET_MASK,
							 RXCSRL_REG_EPN_ENABLE_AUTOCLR_MASK | RXCSRL_REG_EPN_RX_PKT_RDY_MASK);
		}
		break;
...
static int mpfs_ep_resume(struct mpfs_ep_s *privep)
{
	...
	if (privep->epstate == USB_EPSTATE_STALLED) {
		...
		if (epno == EP0) {
			mpfs_modifyreg16(MPFS_USB_INDEXED_CSR_EP0_CSR0, CSR0L_DEV_STALL_SENT_MASK, 0);
		} else if (USB_ISEPIN(privep->ep.eplog)) {
			mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_CSR_OFFSET,
							 TXCSRL_REG_EPN_SEND_STALL_MASK, 0);
		} else {
			mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_CSR_OFFSET,
							 RXCSRL_REG_EPN_SEND_STALL_MASK, RXCSRL_REG_EPN_RX_PKT_RDY_MASK);
			mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_CSR_OFFSET, 0,
...
static inline struct mpfs_ep_s *mpfs_ep_reserve(struct mpfs_usbdev_s *priv, uint16_t epset, bool in)
{
	...
	if (epset != 0) {
		...
		for (epndx = 1 + (MPFS_USB_NENDPOINTS / 2) * in; epndx < max; epndx++) {
...
static struct usbdev_ep_s *mpfs_allocep(struct usbdev_s *dev, uint8_t epno, bool in, uint8_t eptype)
{
...
static inline void mpfs_ep_unreserve(struct mpfs_usbdev_s *priv, struct mpfs_ep_s *privep)
{
...
static void mpfs_ep0_dispatch(struct mpfs_usbdev_s *priv)
{
	...
	if (priv && priv->driver) {
		...
		ret = CLASS_SETUP(priv->driver, &priv->usbdev, &priv->ctrl, dataout, outlen);
		if (ret < 0) {
			/* Stall on failure */
...
static void mpfs_ep0_setup(struct mpfs_usbdev_s *priv)
{
	...
	uinfo("SETUP: type=%02x req=%02x value=%04x index=%04x len=%04x\n", priv->ctrl.type,
		  priv->ctrl.req, value.w, index.w, len.w);
	...
	case USB_REQ_GETSTATUS: {
		...
		usbtrace(TRACE_INTDECODE(MPFS_TRACEINTID_GETSTATUS), priv->ctrl.type);
		if (len.w != 2 || (priv->ctrl.type & USB_REQ_DIR_IN) == 0 || index.b[MSB] != 0 ||
			value.w != 0) {
			...
			case USB_REQ_RECIPIENT_ENDPOINT: {
				...
				if (epno >= MPFS_USB_NENDPOINTS) {
					usbtrace(TRACE_DEVERROR(MPFS_TRACEERR_BADEPGETSTATUS), epno);
					ep0result = USB_EP0SETUP_STALL;
				} else {
			...
			case USB_REQ_RECIPIENT_DEVICE: {
				if (index.w == 0) {
					usbtrace(TRACE_INTDECODE(MPFS_TRACEINTID_DEVGETSTATUS), 0);
					...
					usbtrace(TRACE_DEVERROR(MPFS_TRACEERR_BADDEVGETSTATUS), 0);
					ep0result = USB_EP0SETUP_STALL;
				}
			} break;
	...
	case USB_REQ_CLEARFEATURE: {
		...
		usbtrace(TRACE_INTDECODE(MPFS_TRACEINTID_CLEARFEATURE), priv->ctrl.type);
	...
	case USB_REQ_SETFEATURE: {
		...
		usbtrace(TRACE_INTDECODE(MPFS_TRACEINTID_SETFEATURE), priv->ctrl.type);
	...
	case USB_REQ_SETADDRESS: {
		...
		if ((priv->ctrl.type & USB_REQ_RECIPIENT_MASK) != USB_REQ_RECIPIENT_DEVICE ||
			...
			usbtrace(TRACE_INTDECODE(MPFS_TRACEINTID_EP0SETUPSETADDRESS), value.w);
	...
	case USB_REQ_GETCONFIGURATION:
		...
		{
			usbtrace(TRACE_INTDECODE(MPFS_TRACEINTID_GETCONFIG), priv->ctrl.type);
	...
	case USB_REQ_SETCONFIGURATION:
		...
		{
			usbtrace(TRACE_INTDECODE(MPFS_TRACEINTID_SETCONFIG), priv->ctrl.type);
	...
	default: {
		usbtrace(TRACE_DEVERROR(MPFS_TRACEERR_INVALIDCTRLREQ), priv->ctrl.req);
		ep0result = USB_EP0SETUP_STALL;
	} break;
	}
	...
	case USB_EP0SETUP_STALL: {
		...
		usbtrace(TRACE_DEVERROR(MPFS_TRACEERR_EP0SETUPSTALLED), priv->ctrl.req);
...
static void mpfs_ep_rx_interrupt(struct mpfs_usbdev_s *priv, int epno)
{
	...
	count = getreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_RX_COUNT_OFFSET);
...
static void mpfs_ep_tx_interrupt(struct mpfs_usbdev_s *priv, int epno)
{
	...
	uint16_t tx_csr = getreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_CSR_OFFSET);
	...
	if ((tx_csr & TXCSRL_REG_EPN_UNDERRUN_MASK) != 0) {
		...
		mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_CSR_OFFSET,
						 TXCSRL_REG_EPN_UNDERRUN_MASK, 0);
	}

	if ((tx_csr & TXCSRL_REG_EPN_STALL_SENT_MASK) != 0) {
		mpfs_modifyreg16(MPFS_USB_ENDPOINT(epno) + MPFS_USB_ENDPOINT_TX_CSR_OFFSET,
						 TXCSRL_REG_EPN_STALL_SENT_MASK, 0);
	}

	if (privep->epstate == USB_EPSTATE_SENDING || privep->epstate == USB_EPSTATE_EP0STATUSIN) {
...
static void mpfs_ctrl_ep_interrupt(struct mpfs_usbdev_s *priv)
{
	...
	if ((csr0 & CSR0L_DEV_SETUP_END_MASK) != 0) {
		...
		mpfs_modifyreg16(MPFS_USB_INDEXED_CSR_EP0_CSR0, 0, CSR0L_DEV_SERVICED_SETUP_END_MASK);
	}
	...
	if ((csr0 & CSR0L_DEV_RX_PKT_RDY_MASK) != 0) {
		...
		if (count0 > 0) {
			if (privep->epstate == USB_EPSTATE_EP0DATAOUT) {
				...
				mpfs_putreg16(CSR0L_DEV_DATA_END_MASK | CSR0L_DEV_SERVICED_RX_PKT_RDY_MASK,
							  MPFS_USB_INDEXED_CSR_EP0_CSR0);
			} else {
				...
				mpfs_putreg16(CSR0L_DEV_SERVICED_RX_PKT_RDY_MASK, MPFS_USB_INDEXED_CSR_EP0_CSR0);
			}
		}
		...
		if (privep->epstate == USB_EPSTATE_EP0DATAOUT) {
			...
			if (rlen == count0) {
				...
				usbtrace(TRACE_DEVERROR(MPFS_TRACEERR_EP0SETUPOUTSIZE), count0);
...
static void mpfs_epset_reset(struct mpfs_usbdev_s *priv, uint16_t epset)
{
	...
	for (epno = 0, bit = 1, epset &= MPFS_EPSET_ALL; epno < MPFS_USB_NENDPOINTS && epset != 0;
		 epno++, bit <<= 1) {
...
static void mpfs_hw_setup(struct mpfs_usbdev_s *priv)
{
	...
	modifyreg32(MPFS_SYSREG_SOFT_RESET_CR, SYSREG_SOFT_RESET_CR_USB | SYSREG_SOFT_RESET_CR_FPGA, 0);
	...
	memset((uint8_t *)(&priv->ep_descriptors[0]), 0, sizeof(priv->ep_descriptors));

	/* Enable interrupts */

	mpfs_putreg8(SUSPEND_IRQ_MASK | RESUME_IRQ_MASK | RESET_IRQ_MASK | CONNECT_IRQ_MASK |
					 DISCONNECT_IRQ_MASK,
				 MPFS_USB_ENABLE);
...
#ifdef CONFIG_USBDEV_DUALSPEED
	mpfs_putreg8(POWER_REG_SOFT_CONN_MASK | POWER_REG_ENABLE_HS_MASK, MPFS_USB_POWER);
#else
...
void mpfs_usbinitialize(void)
{
	...
	if (irq_attach(MPFS_IRQ_USB_MC, mpfs_usb_interrupt, priv) != 0) {
		usbtrace(TRACE_DEVERROR(MPFS_TRACEERR_IRQREGISTRATION), MPFS_IRQ_USB_MC);
		goto errout;
	}

#ifdef CONFIG_USBDEV_DMA
	if (irq_attach(MPFS_IRQ_USB_DMA, mpfs_usb_dma_interrupt, priv) != 0) {
		usbtrace(TRACE_DEVERROR(MPFS_TRACEERR_IRQREGISTRATION), MPFS_IRQ_USB_DMA);
		goto errout;
	}
#endif
