...
void up_task_start(main_t taskentry, int argc, char *argv[])
{
	...
	riscv_jump_to_user((uintptr_t)USERSPACE->task_startup, (uintptr_t)taskentry, (uintreg_t)argc,
					   (uintreg_t)argv, sp, rtcb->xcp.initregs);
#else
	riscv_jump_to_user((uintptr_t)taskentry, (uintreg_t)argc, (uintreg_t)argv, 0, sp,
					   rtcb->xcp.initregs);
#endif
}
