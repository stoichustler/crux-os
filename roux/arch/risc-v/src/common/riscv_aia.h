...
#define RISCV_APLIC_IDC(base, i) ((base) + RISCV_APLIC_IDC_BASE + RISCV_APLIC_IDC_SIZE * (i))
...
void riscv_imsic_local_eix_update(unsigned long base_id, unsigned long num_id, bool pend, bool val);

static inline void riscv_imsic_local_eie_update(unsigned long base_id, unsigned long num_id,
												bool val)
{
	return riscv_imsic_local_eix_update(base_id, num_id, false, val);
}

static inline void riscv_imsic_local_eip_update(unsigned long base_id, unsigned long num_id,
												bool val)
{
...
static inline int riscv_aplic_set_delegate(uintptr_t base, uint32_t first_irq, uint32_t last_irq,
										   uint32_t child_id)
{
	uint32_t j;
	...
	for (j = first_irq; j <= last_irq; j++) {
		putreg32(RISCV_APLIC_SOURCECFG_D | child_id, sourcecfg_base + (j - 1) * sizeof(uint32_t));
	}
...
static inline void riscv_aplic_configure_irq(uintptr_t base, uint32_t irq, uint32_t mode,
											 uint32_t hartid)
{
	uint32_t val = (hartid << RISCV_APLIC_TARGET_HART_IDX_SHIFT) | irq;
	putreg32(mode, base + RISCV_APLIC_SOURCECFG_BASE + (irq - 1) * sizeof(uint32_t));
	putreg32(val, base + RISCV_APLIC_TARGET_BASE + (irq - 1) * sizeof(uint32_t));
}

static inline void riscv_aplic_disable_irqs(uintptr_t base, uint32_t num_source)
{
	uint32_t i;
	...
	for (i = 0; i < num_source; i += 32) {
		putreg32(-1U, base + RISCV_APLIC_CLRIE_BASE + (i / 32) * sizeof(uint32_t));
	}
}
...
void riscv_aplic_init(uintptr_t base, uint32_t idelivery, uint32_t ithreshold);
...
void riscv_aplic_init_msi(uintptr_t base, uint64_t imsic_addr, uint32_t lhxs, uint32_t lhxw,
