...
size_t riscv_stack_check(uintptr_t alloc, size_t size)
{
	...
	for (ptr = (uint32_t *)start, mark = (size >> 2); *ptr == STACK_COLOR && mark > 0;
		 ptr++, mark--)
		;
...
size_t up_check_tcbstack(struct tcb_s *tcb)
{
	...
	size = riscv_stack_check((uintptr_t)tcb->stack_base_ptr, tcb->adj_stack_size);
...
{
	return riscv_stack_check((uintptr_t)g_intstackalloc, (CONFIG_ARCH_INTERRUPTSTACK & ~15));
}
#endif
