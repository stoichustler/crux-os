...
static bool decode_insn_compressed(uintptr_t *regs, riscv_insn_ctx_t *ctx)
{
	...
	case INSN_C_LWSP:
		imm = insn.lwsp.imm42 << 2 | insn.lwsp.imm5 << 5 | insn.lwsp.imm76 << 6;
	...
	case INSN_C_LDSP:
		imm = insn.ldsp.imm43 << 3 | insn.ldsp.imm5 << 5 | insn.ldsp.imm86 << 6;
...
static bool decode_insn(uintptr_t *regs, riscv_insn_ctx_t *ctx)
{
	...
	case INSN_LWU:
		...
		ctx->src = (uint8_t *)regs[insn.l.rs1] + sext(insn.l.imm, 12);
...
int riscv_misaligned(int irq, void *context, void *arg)
{
	...
	riscv_insn_ctx_t ctx = {NULL, NULL, 0, false};
