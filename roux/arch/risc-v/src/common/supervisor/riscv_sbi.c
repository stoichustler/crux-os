...
static inline sbiret_t sbi_ecall(unsigned int extid, unsigned int fid, uintreg_t parm0,
								 uintreg_t parm1, uintreg_t parm2, uintreg_t parm3, uintreg_t parm4,
								 uintreg_t parm5)
{
...
#else
	sbi_ecall(SBI_EXT_TIME, SBI_EXT_TIME_SET_TIMER, stime_value, stime_value >> 32, 0, 0, 0, 0);
#endif
}
...
{
#ifdef CONFIG_NUTTSBI
	sbiret_t ret = sbi_ecall(SBI_EXT_FIRMWARE, SBI_EXT_FIRMWARE_GET_MTIME, 0, 0, 0, 0, 0, 0);
...
{
	sbi_ecall(SBI_EXT_IPI, SBI_EXT_IPI_SEND_IPI, hmask, hbase, 0, 0, 0, 0);
}

#ifndef CONFIG_NUTTSBI
int riscv_sbi_boot_secondary(uintreg_t hartid, uintreg_t addr, uintreg_t a1)
{
	sbiret_t ret = sbi_ecall(SBI_EXT_HSM, SBI_EXT_HSM_HART_START, hartid, addr, a1, 0, 0, 0);
...
{
	sbiret_t ret = sbi_ecall(SBI_EXT_SRST, SBI_EXT_SRST_SYS_RESET, type, reason, 0, 0, 0, 0);
