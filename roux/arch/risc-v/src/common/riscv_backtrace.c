...
nosanitize_address static int backtrace(uintptr_t *base, uintptr_t *limit, uintptr_t *fp,
										uintptr_t *ra, void **buffer, int size, int *skip)
{
	int i = 0;
#if CONFIG_ARCH_INTERRUPTSTACK > 15
	int cpu = up_cpu_index();
	uintptr_t *intstack_limit =
		(uintptr_t *)((uintptr_t)g_intstacktop - (CONFIG_ARCH_INTERRUPTSTACK * cpu));
	...
	if (tcb == NULL || tcb == rtcb) {
		if (up_interrupt_context()) {
			ret = backtrace(rtcb->stack_base_ptr,
							(uintptr_t *)((uintptr_t)rtcb->stack_base_ptr + rtcb->adj_stack_size),
							(void *)getfp(), NULL, buffer, size, &skip);
			if (ret < size) {
				ret +=
					backtrace(rtcb->stack_base_ptr,
							  (uintptr_t *)((uintptr_t)rtcb->stack_base_ptr + rtcb->adj_stack_size),
							  running_regs()[REG_FP], running_regs()[REG_EPC], &buffer[ret],
							  size - ret, &skip);
			}
		} else {
			...
			if (rtcb->xcp.ustkptr != NULL) {
				ret =
					backtrace(rtcb->stack_base_ptr,
							  (uintptr_t *)((uintptr_t)rtcb->stack_base_ptr + rtcb->adj_stack_size),
							  (void *)*(rtcb->xcp.ustkptr + 1), NULL, buffer, size, &skip);
			} else
#endif
			{
				ret =
		...
		if (tcb->xcp.ustkptr != NULL) {
			ret = backtrace(tcb->stack_base_ptr,
							...
							(void *)*(tcb->xcp.ustkptr + 1), NULL, buffer, size, &skip);
		} else
#endif
		{
			ret = backtrace(tcb->stack_base_ptr,
