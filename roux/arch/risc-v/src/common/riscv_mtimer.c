...
static int riscv_mtimer_max_delay(struct oneshot_lowerhalf_s *lower, struct timespec *ts);
static int riscv_mtimer_start(struct oneshot_lowerhalf_s *lower, oneshot_callback_t callback,
							  void *arg, const struct timespec *ts);
static int riscv_mtimer_cancel(struct oneshot_lowerhalf_s *lower, struct timespec *ts);
static int riscv_mtimer_current(struct oneshot_lowerhalf_s *lower, struct timespec *ts);

static int riscv_mtimer_tick_max_delay(struct oneshot_lowerhalf_s *lower, clock_t *ticks);
static int riscv_mtimer_tick_start(struct oneshot_lowerhalf_s *lower, oneshot_callback_t callback,
								   void *arg, clock_t ticks);
static int riscv_mtimer_tick_cancel(struct oneshot_lowerhalf_s *lower, clock_t *ticks);
static int riscv_mtimer_tick_current(struct oneshot_lowerhalf_s *lower, clock_t *ticks);
...
static void riscv_mtimer_set_mtimecmp(struct riscv_mtimer_lowerhalf_s *priv, uint64_t value)
{
#if CONFIG_ARCH_RV_MMIO_BITS == 64
	if (-1 != priv->mtime) {
...
static int riscv_mtimer_start(struct oneshot_lowerhalf_s *lower, oneshot_callback_t callback,
							  void *arg, const struct timespec *ts)
{
	struct riscv_mtimer_lowerhalf_s *priv = (struct riscv_mtimer_lowerhalf_s *)lower;
	irqstate_t flags;
	uint64_t mtime;
	uint64_t alarm;
	...
	alarm = mtime + ts->tv_sec * priv->freq + ts->tv_nsec * priv->freq / NSEC_PER_SEC;
...
static int riscv_mtimer_tick_cancel(struct oneshot_lowerhalf_s *lower, clock_t *ticks)
{
	...
	*ticks = alarm * TICK_PER_SEC / priv->freq - mtime * TICK_PER_SEC / priv->freq;
...
struct oneshot_lowerhalf_s *riscv_mtimer_initialize(uintreg_t mtime, uintreg_t mtimecmp, int irq,
													uint64_t freq)
{
