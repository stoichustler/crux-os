...
#include <roux/irq.h>
#include <roux/arch.h>
#include <roux/rmt/rmt.h>
#include <roux/spinlock.h>
#include <roux/circbuf.h>
...
#define RMT_DEFAULT_CONFIG_TX(gpio, channel_id)                                                    \
	{                                                                                              \
		.rmt_mode = RMT_MODE_TX, .channel = channel_id, .gpio_num = gpio,                          \
		.clk_div = RMT_DEFAULT_CLK_DIV, .mem_block_num = 1, .flags = 0, .tx_config = {             \
...
#define RMT_DEFAULT_CONFIG_RX(gpio, channel_id)                                                    \
	{                                                                                              \
		.rmt_mode = RMT_MODE_RX, .channel = channel_id, .gpio_num = gpio,                          \
		.clk_div = RMT_DEFAULT_CLK_DIV, .mem_block_num = 1, .flags = 0, .rx_config = {             \
...
struct rmt_tx_config_s {
	...
	uint32_t loop_count; /* Maximum loop count, only take effect for chips that is capable of
...
static int rmt_set_tx_thr_intr_en(rmt_channel_t channel, bool en, uint16_t evt_thresh);
static int rmt_set_gpio(rmt_channel_t channel, rmt_mode_t mode, gpio_num_t gpio_num,
						bool invert_signal);
static bool rmt_is_channel_number_valid(rmt_channel_t channel, uint8_t mode);
static int rmt_internal_config(rmt_dev_t *dev, const struct rmt_channel_config_s *rmt_param);
static int rmt_config(const struct rmt_channel_config_s *rmt_param);
static void rmt_fill_memory(rmt_channel_t channel, const rmt_item32_t *item, uint16_t item_num,
							uint16_t mem_offset);
static int rmt_isr_register(int (*fn)(int, void *, void *), void *arg, int intr_alloc_flags);
static int rmt_driver_isr_default(int irq, void *context, void *arg);
static int rmt_driver_install(rmt_channel_t channel, size_t rx_buf_size, int intr_alloc_flags);
static int rmt_write_items(rmt_channel_t channel, const rmt_item32_t *rmt_item, int item_num,
						   bool wait_tx_done);
static ssize_t esp_rmt_read(struct rmt_dev_s *dev, char *buffer, size_t buflen);
static ssize_t esp_rmt_write(struct rmt_dev_s *dev, const char *buffer, size_t buflen);
static struct rmt_dev_s *esp_rmtinitialize(struct rmt_channel_config_s config);
...
static struct rmt_dev_common_s g_rmtdev_common = {.hal.regs = &RMT,
...
#ifdef SOC_RMT_SUPPORT_RX_PINGPONG
static int rmt_set_rx_thr_intr_en(rmt_channel_t channel, bool en, uint16_t evt_thresh)
{
	irqstate_t flags;
	uint32_t mask;
	...
	if (en) {
		uint32_t item_block_len =
			rmt_ll_rx_get_mem_blocks(g_rmtdev_common.hal.regs, RMT_DECODE_RX_CHANNEL(channel)) *
			RMT_MEM_ITEM_NUM;
		...
		flags = spin_lock_irqsave(&g_rmtdev_common.rmt_spinlock);
		rmt_ll_rx_set_limit(g_rmtdev_common.hal.regs, RMT_DECODE_RX_CHANNEL(channel), evt_thresh);
...
static int rmt_rx_start(rmt_channel_t channel, bool rx_idx_rst)
{
	...
	const uint32_t item_block_len =
		rmt_ll_rx_get_mem_blocks(g_rmtdev_common.hal.regs, ch) * RMT_MEM_ITEM_NUM;
#endif
	...
	rmt_ll_clear_interrupt_status(g_rmtdev_common.hal.regs, RMT_LL_EVENT_RX_DONE(ch));
	rmt_ll_enable_interrupt(g_rmtdev_common.hal.regs, RMT_LL_EVENT_RX_DONE(ch), true);
...
static int rmt_tx_start(rmt_channel_t channel, bool tx_idx_rst)
{
	...
	rmt_ll_clear_interrupt_status(g_rmtdev_common.hal.regs, RMT_LL_EVENT_TX_DONE(channel));

	/* Enable tx end interrupt in non-loop mode */

	if (!rmt_ll_tx_is_loop_enabled(g_rmtdev_common.hal.regs, channel)) {
		rmt_ll_enable_interrupt(g_rmtdev_common.hal.regs, RMT_LL_EVENT_TX_DONE(channel), true);
	} else {
		...
		rmt_ll_clear_interrupt_status(g_rmtdev_common.hal.regs, RMT_LL_EVENT_TX_LOOP_END(channel));
		rmt_ll_enable_interrupt(g_rmtdev_common.hal.regs, RMT_LL_EVENT_TX_LOOP_END(channel), true);
#endif
	}
...
static int rmt_set_tx_thr_intr_en(rmt_channel_t channel, bool en, uint16_t evt_thresh)
{
	...
	if (en) {
		uint32_t item_block_len =
			rmt_ll_tx_get_mem_blocks(g_rmtdev_common.hal.regs, channel) * RMT_MEM_ITEM_NUM;

		DEBUGASSERT(evt_thresh <= item_block_len);

		flags = spin_lock_irqsave(&g_rmtdev_common.rmt_spinlock);
		rmt_ll_tx_set_limit(g_rmtdev_common.hal.regs, channel, evt_thresh);
		rmt_ll_enable_interrupt(g_rmtdev_common.hal.regs, RMT_LL_EVENT_TX_THRES(channel), true);
		spin_unlock_irqrestore(&g_rmtdev_common.rmt_spinlock, flags);
	} else {
		flags = spin_lock_irqsave(&g_rmtdev_common.rmt_spinlock);
		rmt_ll_enable_interrupt(g_rmtdev_common.hal.regs, RMT_LL_EVENT_TX_THRES(channel), false);
		spin_unlock_irqrestore(&g_rmtdev_common.rmt_spinlock, flags);
	}
						...
						bool invert_signal)
{
	...
	if (mode == RMT_MODE_TX) {
		...
		esp_gpio_matrix_out(gpio_num, rmt_periph_signals.groups[0].channels[channel].tx_sig,
							invert_signal, 0);
	} else {
		DEBUGASSERT(RMT_IS_RX_CHANNEL(channel));
		esp_configgpio(gpio_num, INPUT);
		esp_gpio_matrix_in(gpio_num, rmt_periph_signals.groups[0].channels[channel].rx_sig,
						   invert_signal);
	}
...
static int rmt_internal_config(rmt_dev_t *dev, const struct rmt_channel_config_s *rmt_param)
{
	...
	if (!rmt_is_channel_number_valid(channel, mode)) {
		rmterr("Invalid channel number %u for %s mode!", channel,
			   mode == RMT_MODE_TX ? "transmitter" : "receiver");
		return -EINVAL;
	}

	DEBUGASSERT(mem_cnt + channel <= SOC_RMT_CHANNELS_PER_GROUP && mem_cnt > 0);
	...
	if (rmt_param->flags & RMT_CHANNEL_FLAGS_AWARE_DFS) {
		...
		esp_clk_tree_src_get_freq_hz((soc_module_clk_t)RMT_BASECLK_XTAL,
									 ESP_CLK_TREE_SRC_FREQ_PRECISION_CACHED, &rmt_source_clk_hz);
...
#error "No clock source is aware of DFS"
	...
	if (mode == RMT_MODE_TX) {
		...
		rmtinfo("Rmt Tx Channel %u|Gpio %u|Sclk_Hz %" PRIu32 "|Div %u|Carrier_Hz"
				" %" PRIu32 "|Duty %u",
				channel, gpio_num, rmt_source_clk_hz, clk_div, carrier_freq_hz,
...
#if SOC_RMT_SUPPORT_RX_DEMODULATION
		rmt_ll_rx_enable_carrier_demodulation(dev, RMT_DECODE_RX_CHANNEL(channel),
											  rmt_param->rx_config.rm_carrier);
		if (rmt_param->rx_config.rm_carrier) {
			uint32_t duty_total;
			uint32_t duty_high;
			uint32_t ch_clk_div =
				rmt_ll_rx_get_channel_clock_div(dev, RMT_DECODE_RX_CHANNEL(channel));
			duty_total = rmt_source_clk_hz / ch_clk_div / rmt_param->rx_config.carrier_freq_hz;
			duty_high = duty_total * rmt_param->rx_config.carrier_duty_percent / 100;

			/* there could be residual in timing the carrier pulse, so double
			 * enlarge the theoretical value.
			 */

			rmt_ll_rx_set_carrier_high_low_ticks(dev, RMT_DECODE_RX_CHANNEL(channel), duty_high * 2,
...
static int rmt_isr_register(int (*fn)(int, void *, void *), void *arg, int intr_alloc_flags)
{
	...
	cpuint = esp_setup_irq(rmt_periph_signals.groups[0].irq, ESP_IRQ_PRIORITY_DEFAULT,
	...
	ret = irq_attach(ESP_SOURCE2IRQ(rmt_periph_signals.groups[0].irq), fn, &g_rmtdev_common.hal);
	if (ret < 0) {
...
static int IRAM_ATTR rmt_driver_isr_default(int irq, void *context, void *arg)
{
	uint32_t status = 0;
	...
	while (status) {
		...
		if (p_rmt) {
			...
			item_len = rmt_ll_rx_get_memory_writer_offset(g_rmtdev_common.hal.regs, channel);
			rmt_ll_rx_set_mem_owner(g_rmtdev_common.hal.regs, channel, RMT_LL_MEM_OWNER_SW);
			if (circbuf_is_init(&p_rmt->rx_buf)) {
				...
				if ((p_rmt->rx_item_len + item_len) > (p_rmt->rx_item_buf_size / 4)) {
					int remaining_len = (p_rmt->rx_item_buf_size / 4) - p_rmt->rx_item_len;
				...
				memcpy((void *)(p_rmt->rx_item_buf + p_rmt->rx_item_len),
					   (void *)(addr + p_rmt->rx_item_start_idx), item_len * 4);
				p_rmt->rx_item_len += item_len;
				bytes = circbuf_write(&p_rmt->rx_buf, (void *)(p_rmt->rx_item_buf),
									  p_rmt->rx_item_len * 4);
#else
				bytes = circbuf_write(&p_rmt->rx_buf, (void *)addr, item_len * 4);
#endif
			...
			rmt_ll_rx_set_mem_owner(g_rmtdev_common.hal.regs, channel, RMT_LL_MEM_OWNER_HW);
	...
	while (status) {
		...
		rmt_obj_t *p_rmt = p_rmt_obj[RMT_ENCODE_RX_CHANNEL(channel)];
		mem_item_size =
			rmt_ll_rx_get_mem_blocks(g_rmtdev_common.hal.regs, channel) * RMT_MEM_ITEM_NUM;
		rx_thres_lim = rmt_ll_rx_get_limit(g_rmtdev_common.hal.regs, channel);
		item_len = (p_rmt->rx_item_start_idx == 0) ? rx_thres_lim : (mem_item_size - rx_thres_lim);
		if ((p_rmt->rx_item_len + item_len) > (p_rmt->rx_item_buf_size / 4)) {
			int remaining_len = (p_rmt->rx_item_buf_size / 4) - p_rmt->rx_item_len;
		...
		memcpy(
			...
			(void *)(RMTMEM.chan[RMT_ENCODE_RX_CHANNEL(channel)].data32 + p_rmt->rx_item_start_idx),
			item_len * 4);
		rmt_ll_rx_set_mem_owner(g_rmtdev_common.hal.regs, channel, RMT_LL_MEM_OWNER_HW);
		p_rmt->rx_item_len += item_len;
	...
	while (status) {
		...
		if (p_rmt) {
			...
			rmtinfo("status: 0x%08lx",
					rmt_ll_rx_get_status_word(g_rmtdev_common.hal.regs, channel));
		}
	...
	while (status) {
		...
		if (p_rmt) {
			...
			rmtinfo("status: 0x%08lx",
					rmt_ll_tx_get_status_word(g_rmtdev_common.hal.regs, channel));
		}
...
static int rmt_driver_install(rmt_channel_t channel, size_t rx_buf_size, int intr_alloc_flags)
{
	...
	if (g_rmtdev_common.rmt_driver_channels == 0) {
		...
		ret = rmt_isr_register(rmt_driver_isr_default, &g_rmtdev_common.hal, intr_alloc_flags);
	}
	...
	if (RMT_IS_RX_CHANNEL(channel)) {
		rmt_hal_rx_channel_reset(&g_rmtdev_common.hal, RMT_DECODE_RX_CHANNEL(channel));
	} else {
						   ...
						   bool wait_tx_done)
{
	...
	uint32_t mem_blocks = rmt_ll_tx_get_mem_blocks(g_rmtdev_common.hal.regs, channel);
	...
	if (item_num >= item_block_len) {
		...
		idle_level = rmt_ll_tx_get_idle_level(g_rmtdev_common.hal.regs, channel);
		rmt_item32_t stop_data = (rmt_item32_t){
			.level0 = idle_level,
			.duration0 = 0,
		};
...
static ssize_t esp_rmt_read(struct rmt_dev_s *dev, char *buffer, size_t buflen)
{
	...
	if ((buflen / 4) > (CONFIG_RMT_DEFAULT_RX_BUFFER_SIZE / 4)) {
		rmtwarn("WARN: RMT RX buffer (%d bytes) is smaller than requested "
				...
				CONFIG_RMT_DEFAULT_RX_BUFFER_SIZE, buflen);
	}
...
static ssize_t esp_rmt_write(struct rmt_dev_s *dev, const char *buffer, size_t buflen)
{
	...
	ret = rmt_write_items(channel, (const rmt_item32_t *)buffer, (buflen / 4), true);
...
#error "CONFIG_RMT_DEFAULT_RX_BUFFER_SIZE must be a multiple of 4"
	...
	if (priv) {
		...
		if (g_rx_channel != RMT_CHANNEL_MAX && g_tx_channel != RMT_CHANNEL_MAX) {
			...
			esp_gpio_matrix_out(config.gpio_num, RMT_SIG_OUT0_IDX + g_tx_channel, 0, 0);
			esp_gpio_matrix_in(config.gpio_num, RMT_SIG_IN0_IDX + g_rx_channel, 0);
		...
		ret = rmt_driver_install(
			config.channel, config.rmt_mode == RMT_MODE_RX ? CONFIG_RMT_DEFAULT_RX_BUFFER_SIZE : 0,
			0);
		if (ret < 0) {
