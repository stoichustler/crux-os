...
static int esp_i2c_slave_setownaddress(struct i2c_slave_s *dev, int addr, int nbits);
static int esp_i2c_slave_write(struct i2c_slave_s *dev, const uint8_t *buffer, int buflen);
static int esp_i2c_slave_read(struct i2c_slave_s *dev, uint8_t *buffer, int buflen);
static int esp_i2c_slave_registercallback(struct i2c_slave_s *dev, i2c_slave_callback_t *callback,
										  void *arg);
...
static const struct esp_i2c_config_s esp_i2c0_config = {.scl_pin = CONFIG_ESPRESSIF_I2C0_SCLPIN,
...
static const struct esp_i2c_config_s esp_i2c1_config = {.scl_pin = CONFIG_ESPRESSIF_I2C1_SCLPIN,
...
static void esp_i2c_slave_init(struct esp_i2c_priv_s *priv)
{
	...
	i2c_ll_set_sda_timing(priv->ctx->dev, I2C_SLAVE_SDA_SAMPLE_DEFAULT, I2C_SLAVE_SDA_HOLD_DEFAULT);
...
{
	struct esp_i2c_priv_s *priv = (struct esp_i2c_priv_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	int ret;
...
static void esp_i2c_process(struct esp_i2c_priv_s *priv, uint32_t irq_status)
{
	...
	if (evt_type == I2C_INTR_EVENT_TRANS_DONE || evt_type == I2C_INTR_EVENT_RXFIFO_FULL) {
		...
		i2c_ll_read_rxfifo(priv->ctx->dev, priv->rx_buffer + priv->rx_length, rx_fifo_cnt);
      ...
      if (priv->cb != NULL)
		...
		if (tx_fifo_rem != 0) {
			...
			memmove(priv->tx_buffer, priv->tx_buffer + tx_fifo_rem, tx_fifo_rem);
		} else {
...
struct i2c_slave_s *esp_i2cbus_slave_initialize(int port, int addr)
{
	...
	ret = kthread_create("esp_i2c_slave_thread", SCHED_PRIORITY_DEFAULT, 128, esp_i2c_slave_thread,
						 argv);
	if (ret < 0) {
		kmm_free(priv);
		return NULL;
