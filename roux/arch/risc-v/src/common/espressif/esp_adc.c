...
static int esp_adc_ioctl(struct adc_dev_s *dev, int cmd, unsigned long arg);
static int esp_adc_bind(struct adc_dev_s *dev, const struct adc_callback_s *callback);
...
static int esp_adc_oneshot_read(struct adc_dev_s *dev)
{
	...
	for (i = 0; i < priv->channels; i++) {
		...
		if (ch->calibrated) {
			ret = adc_cali_raw_to_voltage(ch->cali_handle, raw_value, &voltage);
			if (ret != OK) {
				voltage = 0;
		...
		ainfo("read adc %d ch %d (cali: %d): value %ld (raw %d)\n", priv->unit, ch->channel,
			  ch->calibrated, (int32_t)voltage, raw_value);
		usleep(1000);
	}
...
static int esp_adc_oneshot_new_unit(struct adc_dev_s *dev)
{
	...
	esp_clk_tree_src_get_freq_hz(ADC_DIGI_CLK_SRC_DEFAULT, ESP_CLK_TREE_SRC_FREQ_PRECISION_CACHED,
								 &clk_src_freq_hz);
...
static int esp_adc_oneshot_config_channel(struct adc_dev_s *dev, uint8_t channel)
{
	struct esp_adc_dev_s *priv = (struct esp_adc_dev_s *)dev->ad_priv;
	adc_oneshot_hal_ctx_t *hal = &priv->os_dev.hal;
	...
	ainfo("init adc unit %u, ch %u (gpio %d), atten %d, bitwidth %d", priv->unit, channel, gpio,
...
static int esp_adc_calibrate(struct adc_dev_s *dev)
{
	...
	for (i = 0; i < priv->channels; i++) {
		...
		adc_load_hw_calibration_chan_compens(priv->unit, cali_config.chan, cali_config.atten);
#endif

		ainfo("curve fitting unit %d, chan %u, atten %u, bitwidth %u\n", cali_config.unit_id,
			  cali_config.chan, cali_config.atten, cali_config.bitwidth);
...
struct adc_dev_s *esp_adc_initialize(int adc_num, const uint8_t *channel_list)
{
