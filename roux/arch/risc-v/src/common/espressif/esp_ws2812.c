...
static uint32_t map_byte_to_words(struct esp_ws2812_dev_s *dev, uint8_t byte, uint32_t *dst);
static int map_leds_to_words(struct esp_ws2812_dev_s *dev, struct rgbw_led_s *leds, uint32_t n_leds,
							 uint32_t *dst, bool has_white);
							 ...
							 uint32_t *dst, bool has_white)
{
	...
	for (uint32_t led_idx = 0; led_idx < n_leds; led_idx++) {
		dst_offset += map_byte_to_words(dev, leds[led_idx].g, dst + dst_offset);
		dst_offset += map_byte_to_words(dev, leds[led_idx].r, dst + dst_offset);
		dst_offset += map_byte_to_words(dev, leds[led_idx].b, dst + dst_offset);
		if (has_white) {
			dst_offset += map_byte_to_words(dev, leds[led_idx].w, dst + dst_offset);
		}
	}
...
static int esp_open(struct file *filep)
{
	...
	if (priv->open_count == 0) {
		...
		for (i = 0; i < dev_data->nleds; i++) {
			map_leds_to_words(priv, led, 1, ((uint32_t *)priv->buf + i * colors * 8),
							  dev_data->has_white);
		}
...
static ssize_t esp_write(struct file *filep, const char *data, size_t len)
{
	...
	struct esp_ws2812_dev_s *priv = (struct esp_ws2812_dev_s *)dev->private;
	...
	if (len > 0) {
		...
		if (((position + len) / WS2812_RW_PIXEL_SIZE) > dev->nleds) {
			ledwarn("esp_ws2812 write truncated:\n\t\tLED position: %d\n"
					...
					"\t\tLED being written: %d\n",
					position / WS2812_RW_PIXEL_SIZE, n_leds, dev->nleds,
					dev->nleds - (position / WS2812_RW_PIXEL_SIZE));
			n_leds = dev->nleds - (position / WS2812_RW_PIXEL_SIZE);
		}

		ret = map_leds_to_words(priv, (struct rgbw_led_s *)data, n_leds, (uint32_t *)bp,
								dev->has_white);
		if (ret < 0) {
	...
	rmt_bytes = priv->rmt->ops->write(priv->rmt, (const char *)priv->buf, priv->buflen);
...
struct ws2812_dev_s *esp_ws2812_setup(const char *path, struct rmt_dev_s *rmt, uint16_t pixel_count,
									  bool has_white)
{
