...
static int esp_oneshot_maxdelay(struct oneshot_lowerhalf_s *lower, struct timespec *ts);
static int esp_oneshot_start(struct oneshot_lowerhalf_s *lower, oneshot_callback_t callback,
							 void *arg, const struct timespec *ts);
static int esp_oneshot_cancel(struct oneshot_lowerhalf_s *lower, struct timespec *ts);
static int esp_oneshot_current(struct oneshot_lowerhalf_s *lower, struct timespec *ts);
...
static const struct oneshot_operations_s g_oneshot_ops = {.max_delay = esp_oneshot_maxdelay,
...
static struct esp_oneshot_lowerhalf_s g_oneshot_lowerhalf = {.lh =
																 {
																	 .ops = &g_oneshot_ops,
																 },
															 .callback = NULL,
...
static int esp_oneshot_start(struct oneshot_lowerhalf_s *lower, oneshot_callback_t callback,
							 void *arg, const struct timespec *ts)
{
	struct esp_oneshot_lowerhalf_s *priv = (struct esp_oneshot_lowerhalf_s *)lower;
	uint64_t timeout_us;
	...
	tmrinfo("callback=%p arg=%p, ts=(%lu, %ld)\n", callback, arg, (unsigned long)ts->tv_sec,
			ts->tv_nsec);
	...
	timeout_us = (uint64_t)ts->tv_sec * USEC_PER_SEC + (uint64_t)(ts->tv_nsec / NSEC_PER_USEC);
	...
	timer_ll_enable_intr(hal->dev, TIMER_LL_EVENT_ALARM(hal->timer_id), false);
	...
	timer_ll_set_alarm_value(hal->dev, hal->timer_id, timeout_us / priv->resolution);
...
static int esp_oneshot_cancel(struct oneshot_lowerhalf_s *lower, struct timespec *ts)
{
	...
	if (ts != NULL) {
		...
		volatile timg_hwtimer_reg_t *hw_timer = &(hal->dev->hw_timer[hal->timer_id]);
		alarm_value =
			((uint64_t)hw_timer->alarmhi.tx_alarm_hi << 32) | (hw_timer->alarmlo.tx_alarm_lo);
...
{
	struct esp_oneshot_lowerhalf_s *priv = (struct esp_oneshot_lowerhalf_s *)arg;
...
struct oneshot_lowerhalf_s *oneshot_initialize(int chan, uint16_t resolution)
{
	...
	timer_ll_set_clock_source(ONESHOT_TIMERGROUP_ID, lower->hal.timer_id, GPTIMER_CLK_SRC_DEFAULT);
	...
	esp_setup_irq(TG1_T0_LEVEL_INTR_SOURCE, ESP_IRQ_PRIORITY_DEFAULT, ESP_IRQ_TRIGGER_LEVEL);
...
{
	struct oneshot_lowerhalf_s *lower = oneshot_initialize(0, ONESHOT_RESOLUTION);
	if (lower == NULL) {
