...
uint32_t up_get_idletime(void)
{
	...
	alarm_value = systimer_hal_get_alarm_value(&systimer_hal, SYSTIMER_ALARM_OS_TICK_CORE0);
	counter = systimer_hal_get_counter_value(&systimer_hal, SYSTIMER_COUNTER_OS_TICK);
...
void up_step_idletime(uint32_t idletime_us)
{
	...
	systimer_hal_counter_value_advance(&systimer_hal, SYSTIMER_COUNTER_OS_TICK, idletime_us);
...
int IRAM_ATTR up_timer_cancel(struct timespec *ts)
{
	...
	if (ts != NULL) {
		if (!g_timer_started) {
			...
			counter = systimer_hal_get_counter_value(&systimer_hal, SYSTIMER_COUNTER_OS_TICK);
			alarm_ticks = systimer_hal_get_alarm_value(&systimer_hal, SYSTIMER_ALARM_OS_TICK_CORE0);
...
int IRAM_ATTR up_timer_start(const struct timespec *ts)
{
	...
	target_us = (uint64_t)ts->tv_sec * USEC_PER_SEC + (uint64_t)(ts->tv_nsec / NSEC_PER_USEC);

	alarm_ticks = systimer_hal_get_counter_value(&systimer_hal, SYSTIMER_COUNTER_OS_TICK);
	...
	systimer_hal_set_alarm_target(&systimer_hal, SYSTIMER_ALARM_OS_TICK_CORE0,
...
void up_timer_initialize(void)
{
	...
	esp_setup_irq(SYSTIMER_TARGET0_EDGE_INTR_SOURCE, ESP_IRQ_PRIORITY_DEFAULT,
