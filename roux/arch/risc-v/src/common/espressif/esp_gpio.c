...
static gpio_hal_context_t g_gpio_hal = {.dev = GPIO_HAL_GET_HW(GPIO_PORT_0)};
...
int esp_configgpio(int pin, gpio_pinattr_t attr)
{
	...
	if ((attr & DRIVE_MASK) != 0) {
		...
		gpio_hal_set_drive_capability(&g_gpio_hal, pin, GPIO_DRIVE_CAP_DEFAULT);
	}
...
void esp_gpio_matrix_out(uint32_t pin, uint32_t signal_idx, bool out_inv, bool oen_inv)
{
...
void esp_gpioirqinitialize(void)
{
	...
	g_gpio_cpuint =
		esp_setup_irq(GPIO_INTR_SOURCE, ESP_IRQ_PRIORITY_DEFAULT, ESP_IRQ_TRIGGER_LEVEL);
