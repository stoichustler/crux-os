...
#error                                                                                             \
	"WiFi configuration check: WARNING, WIFI_RX_BA_WIN should not be larger than double of the WIFI_STATIC_RX_BUFFER_NUM!"
#endif

#if SOC_PM_SUPPORT_PMU_MODEM_STATE
#define WIFI_BEACON_MONITOR_CONFIG_DEFAULT(ena)                                                    \
	{                                                                                              \
		.enable = (ena), .loss_timeout = CONFIG_ESP_WIFI_SLP_BEACON_LOST_TIMEOUT,                  \
		.loss_threshold = CONFIG_ESP_WIFI_SLP_BEACON_LOST_THRESHOLD, .delta_intr_early = 0,        \
		.delta_loss_timeout = 0, .beacon_abort = 1, .broadcast_wakeup = 1,                         \
		.tsf_time_sync_deviation = 5, .modem_state_consecutive = 10, .rf_ctrl_wait_cycle = 20      \
	}
#else
#define WIFI_BEACON_MONITOR_CONFIG_DEFAULT(ena)                                                    \
	{                                                                                              \
