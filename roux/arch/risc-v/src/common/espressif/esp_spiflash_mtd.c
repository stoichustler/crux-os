...
static int esp_erase(struct mtd_dev_s *dev, off_t startblock, size_t nblocks);
static ssize_t esp_read(struct mtd_dev_s *dev, off_t offset, size_t nbytes, uint8_t *buffer);
static ssize_t esp_bread(struct mtd_dev_s *dev, off_t startblock, size_t nblocks, uint8_t *buffer);
static ssize_t esp_write(struct mtd_dev_s *dev, off_t offset, size_t nbytes, const uint8_t *buffer);
static ssize_t esp_bwrite(struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
						  const uint8_t *buffer);
static int esp_ioctl(struct mtd_dev_s *dev, int cmd, unsigned long arg);
...
struct mtd_dev_s *esp_spiflash_alloc_mtdpart(uint32_t mtd_offset, uint32_t mtd_size)
{
	...
	mtd_part = mtd_partition((struct mtd_dev_s *)&priv->mtd, startblock, blocks);
	if (!mtd_part) {
...
{
	struct esp_mtd_dev_s *priv = (struct esp_mtd_dev_s *)&g_esp_spiflash;
