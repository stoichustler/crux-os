...
#endif
	const temperature_sensor_attribute_t
		*tsens_attribute;				  /* Attribute struct of the common layer */
...
static int inline temperature_sensor_accuracy_compare(const void *p1, const void *p2);
...
static float temperature_sensor_parse_raw_value(uint32_t tsens_raw, const int dac_offset);
...
static int esp_temperature_sensor_get_celsius(struct esp_temp_priv_s *priv, int *buffer);
#ifdef CONFIG_ESPRESSIF_TEMP_UORB
static int esp_temperature_sensor_set_interval(struct sensor_lowerhalf_s *lower, struct file *filep,
											   uint32_t *period_us);
static int esp_temperature_sensor_activate(struct sensor_lowerhalf_s *lower, struct file *filep,
										   bool enable);
#ifndef CONFIG_ESPRESSIF_TEMP_UORB_POLL
static int esp_temperature_sensor_fetch(struct sensor_lowerhalf_s *lower, struct file *filep,
										char *buffer, size_t buflen);
#endif
#else
static int esp_temperature_sensor_read(struct file *filep, char *buffer, size_t buflen);
...
static temperature_sensor_attribute_t g_tsens_attributes[TEMPERATURE_SENSOR_ATTR_RANGE_NUM];
...
static int inline temperature_sensor_accuracy_compare(const void *p1, const void *p2)
{
	return ((*(temperature_sensor_attribute_t *)p1).error_max <
			(*(temperature_sensor_attribute_t *)p2).error_max)
			   ? -1
			   : 1;
}
...
static int temperature_sensor_attribute_table_sort(void)
{
	...
	qsort(g_tsens_attributes, TEMPERATURE_SENSOR_ATTR_RANGE_NUM, sizeof(g_tsens_attributes[0]),
		  temperature_sensor_accuracy_compare);
	return OK;
}
...
static float temperature_sensor_parse_raw_value(uint32_t tsens_raw, const int dac_offset)
{
	...
	return (TEMPERATURE_SENSOR_LL_ADC_FACTOR * (float)tsens_raw -
			TEMPERATURE_SENSOR_LL_DAC_FACTOR * dac_offset - TEMPERATURE_SENSOR_LL_OFFSET_FACTOR) -
		   g_delta_t / 10.0;
}
...
{
#ifndef CONFIG_ESPRESSIF_TEMP_UORB
	register_driver(CONFIG_ESPRESSIF_TEMP_PATH, &g_esp_temp_sensor_fops, 0666, priv);
#else
...
static int esp_temperature_sensor_get_celsius(struct esp_temp_priv_s *priv, int *buffer)
{
	...
	*out = temperature_sensor_parse_raw_value(tsens_out, priv->tsens_attribute->offset);
	esp_temperature_sensor_disable(priv);

	if (*out < priv->tsens_attribute->range_min || *out > priv->tsens_attribute->range_max) {
...
static int esp_temperature_sensor_fetch(struct sensor_lowerhalf_s *lower, struct file *filep,
										char *buffer, size_t buflen)
{
	struct esp_temp_priv_s *priv = container_of(lower, struct esp_temp_priv_s, lower);
...
int esp_temperature_sensor_initialize(struct esp_temp_sensor_config_t cfg)
{
	...
	ret =
		kthread_create("esp_temperature_sensor_thread", SCHED_PRIORITY_DEFAULT,
					   CONFIG_ESPRESSIF_TEMP_THREAD_STACKSIZE, esp_temperature_sensor_thread, argv);
	if (ret < 0) {
		kmm_free(priv);
		return ret;
	}
