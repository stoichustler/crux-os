...
#if defined(CONFIG_ESPRESSIF_BOOTLOADER_MCUBOOT) || defined(CONFIG_ESPRESSIF_SIMPLE_BOOT)
...
#define CACHE_MASK (EXTMEM_ICACHE_SHUT_IBUS_M | EXTMEM_ICACHE_SHUT_DBUS_M)
...
#define IS_RTC_FAST_IRAM(addr) ((addr) >= SOC_RTC_IRAM_LOW && (addr) < SOC_RTC_IRAM_HIGH)
#define IS_RTC_FAST_DRAM(addr) ((addr) >= SOC_RTC_DRAM_LOW && (addr) < SOC_RTC_DRAM_HIGH)
#else
#define IS_RTC_FAST_IRAM(addr) false
#define IS_RTC_FAST_DRAM(addr) false
#endif
#ifdef SOC_RTC_SLOW_MEM_SUPPORTED
#define IS_RTC_SLOW_DRAM(addr) ((addr) >= SOC_RTC_DATA_LOW && (addr) < SOC_RTC_DATA_HIGH)
#else
#define IS_RTC_SLOW_DRAM(addr) false
#endif
#define IS_NONE(addr)                                                                              \
	(!IS_IROM(addr) && !IS_DROM(addr) && !IS_IRAM(addr) && !IS_DRAM(addr) &&                       \
	 !IS_RTC_FAST_IRAM(addr) && !IS_RTC_FAST_DRAM(addr) && !IS_RTC_SLOW_DRAM(addr) &&              \
	 !IS_PADD(addr))
							...
							uint32_t app_irom_vaddr, uint32_t app_irom_size)
{
	...
	if (bootloader_flash_read(offset, &image_header, sizeof(esp_image_header_t), true) != ESP_OK) {
	...
	while (segments++ < 16 && rom_segments < 2) {
		...
		if (bootloader_flash_read(offset, &segment_hdr, sizeof(esp_image_segment_header_t), true) !=
			ESP_OK) {
		...
		if (IS_RTC_FAST_IRAM(segment_hdr.load_addr) || IS_RTC_FAST_DRAM(segment_hdr.load_addr) ||
		...
		ets_printf(
			"%s: lma 0x%08x vma 0x%08lx len 0x%-6lx (%lu)\n",
			IS_NONE(segment_hdr.load_addr) ? "???"
			: IS_RTC_FAST_IRAM(segment_hdr.load_addr) || IS_RTC_FAST_DRAM(segment_hdr.load_addr) ||
					IS_RTC_SLOW_DRAM(segment_hdr.load_addr)
				? "rtc"
			: IS_MMAP(segment_hdr.load_addr) ? IS_IROM(segment_hdr.load_addr) ? "imap" : "dmap"
		...
		if (IS_DROM(segment_hdr.load_addr) && segment_hdr.load_addr == (uint32_t)_image_drom_vma) {
...
void __esp_start(void)
{
	...
	if (map_rom_segments(app_drom_start, app_drom_vaddr, app_drom_size, app_irom_start,
						 app_irom_vaddr, app_irom_size) != 0) {
	...
	cache_mmu_irom_size =
		((_instruction_size + SPI_FLASH_MMU_PAGE_SIZE - 1) / SPI_FLASH_MMU_PAGE_SIZE) *
		sizeof(uint32_t);
