...
static bool esp_rxflowcontrol(uart_dev_t *dev, unsigned int nbuffered, bool upper);
#endif
#endif
...
static struct uart_ops_s g_uart_ops = {.setup = esp_setup,
...
#ifdef HAVE_RS485
	if ((int_status & UART_INTR_TX_BRK_IDLE) != 0 && esp_txempty(dev)) {
...
static int esp_attach(uart_dev_t *dev)
{
	...
	priv->cpuint = esp_setup_irq(priv->source, priv->int_pri, ESP_IRQ_TRIGGER_LEVEL);
...
static int esp_ioctl(struct file *filep, int cmd, unsigned long arg)
{
	...
	case TCSETS: {
		...
		if (ret == OK) {
			 ...
			 * See roux/libs/libc/termios/lib_tcsetattr.c
			 */
...
static bool esp_rxflowcontrol(uart_dev_t *dev, unsigned int nbuffered, bool upper)
{
	...
	if (priv->iflow) {
		...
		if (nbuffered == 0 || !upper) {
			...
			uart_hal_set_hw_flow_ctrl(priv->hal, flow_ctrl, (uint8_t)(SOC_UART_FIFO_LEN / 2));
			esp_rxint(dev, true);
			ret = false;
		} else {
 ...
 *   For more details, take a look at: roux/arch/xtensa/src/common/xtensa.h
 *
 ****************************************************************************/
