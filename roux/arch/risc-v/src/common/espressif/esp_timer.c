...
static int esp_timer_getstatus(struct timer_lowerhalf_s *lower, struct timer_status_s *status);
static int esp_timer_settimeout(struct timer_lowerhalf_s *lower, uint32_t timeout);
static void esp_timer_setcallback(struct timer_lowerhalf_s *lower, tccb_t callback, void *arg);
static int esp_timer_maxtimeout(struct timer_lowerhalf_s *lower, uint32_t *timeout);
...
static const struct timer_ops_s g_timer_ops = {.start = esp_timer_start,
											   .stop = esp_timer_stop,
...
static struct esp_timer_lowerhalf_s g_timer0_lowerhalf = {.ops = &g_timer_ops,
...
static struct esp_timer_lowerhalf_s g_timer1_lowerhalf = {.ops = &g_timer_ops,
...
static int esp_timer_start(struct timer_lowerhalf_s *lower)
{
	...
	timer_ll_set_clock_source(priv->group_id, hal->timer_id, GPTIMER_CLK_SRC_DEFAULT);
...
static int esp_timer_getstatus(struct timer_lowerhalf_s *lower, struct timer_status_s *status)
{
	...
	alarm_value = ((uint64_t)hw_timer->alarmhi.tx_alarm_hi << 32) | (hw_timer->alarmlo.tx_alarm_lo);
...
static void esp_timer_setcallback(struct timer_lowerhalf_s *lower, tccb_t callback, void *arg)
{
	...
	timer_ll_enable_intr(hal->dev, TIMER_LL_EVENT_ALARM(hal->timer_id), enable);
}
...
static int esp_timer_maxtimeout(struct timer_lowerhalf_s *lower, uint32_t *max_timeout)
{
...
IRAM_ATTR static int esp_timer_isr(int irq, void *context, void *arg)
{
	...
	if (priv->callback(&next_interval_us, priv->arg)) {
		if (next_interval_us > 0) {
			...
			timer_ll_set_alarm_value(hal->dev, hal->timer_id, (uint64_t)next_interval_us);
		}
	} else {
...
int esp_timer_initialize(int group_id)
{
	...
	lib_put_pathbuffer(devpath);
	esp_setup_irq(lower->source, ESP_IRQ_PRIORITY_DEFAULT, ESP_IRQ_TRIGGER_LEVEL);
