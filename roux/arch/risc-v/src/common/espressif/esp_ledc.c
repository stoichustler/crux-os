...
#if CONFIG_ESPRESSIF_LEDC_TIMER0_CHANNELS + CONFIG_ESPRESSIF_LEDC_TIMER1_CHANNELS +                \
		CONFIG_ESPRESSIF_LEDC_TIMER2_CHANNELS + CONFIG_ESPRESSIF_LEDC_TIMER3_CHANNELS >            \
	LEDC_CHANNELS
...
struct ledc_obj_s {
	...
	bool glb_clk_is_acquired[LEDC_TIMER_MAX]; /* Tracks whether the global clock is being acquired
						 by each timer */
};
...
static uint32_t ledc_calculate_divisor(uint32_t src_clk_freq, int freq_hz, uint32_t precision);

static uint32_t ledc_auto_global_clk_div(int freq_hz, uint32_t precision,
										 ledc_slow_clk_sel_t *clk_target);

static uint32_t ledc_auto_clk_divisor(int freq_hz, uint32_t precision, ledc_clk_src_t *clk_source,
									  ledc_slow_clk_sel_t *clk_target);

static int ledc_timer_set(ledc_timer_t timer_sel, uint32_t clock_divider, uint32_t duty_resolution,
						  ledc_clk_src_t clk_src);

static int ledc_set_timer_div(ledc_timer_t timer_num, ledc_clk_cfg_t clk_cfg, int freq_hz,
							  int duty_resolution);
...
static int ledc_duty_config(ledc_channel_t channel, int hpoint_val, int duty_val,
							ledc_duty_direction_t duty_direction, uint32_t duty_num,
							uint32_t duty_cycle, uint32_t duty_scale);

static int ledc_set_duty_with_hpoint(ledc_channel_t channel, uint32_t duty, uint32_t hpoint);
...
static uint32_t ledc_duty_bin_conversion(uint16_t roux_duty, uint32_t duty_resolution);

static int ledc_bind_channel_timer(ledc_channel_t channel, ledc_timer_t timer_sel);
...
static const struct pwm_ops_s g_pwmops = {.setup = pwm_setup,
...
static struct esp_ledc_chan_s g_ledc_chans[LEDC_CHANNELS] = {
	{.num = 0, .pin = CONFIG_ESPRESSIF_LEDC_CHANNEL0_PIN},
	{.num = 1, .pin = CONFIG_ESPRESSIF_LEDC_CHANNEL1_PIN},
	{.num = 2, .pin = CONFIG_ESPRESSIF_LEDC_CHANNEL2_PIN},
	{.num = 3, .pin = CONFIG_ESPRESSIF_LEDC_CHANNEL3_PIN},
#if LEDC_CHANNELS > 4
	{.num = 4, .pin = CONFIG_ESPRESSIF_LEDC_CHANNEL4_PIN},
	{.num = 5, .pin = CONFIG_ESPRESSIF_LEDC_CHANNEL5_PIN}
#endif
};
...
static struct esp_ledc_s g_pwm0dev = {.ops = &g_pwmops,
									  .timer = 0,
...
static struct esp_ledc_s g_pwm1dev = {.ops = &g_pwmops,
									  .timer = 1,
...
static struct esp_ledc_s g_pwm2dev = {.ops = &g_pwmops,
									  .timer = 2,
...
static struct esp_ledc_s g_pwm3dev = {.ops = &g_pwmops,
									  .timer = 3,
...
static bool ledc_ctx_create(void)
{
	...
	if (!p_ledc_obj) {
		...
		ledc_new_mode_obj = (struct ledc_obj_s *)kmm_calloc(1, sizeof(struct ledc_obj_s));
...
#else
		pwminfo("Calibrate RC_FAST_CLK : %" PRIu32 " Hz", s_ledc_slow_clk_rc_fast_freq);
#endif
		return true;
	}
...
static inline uint32_t ledc_calculate_divisor(uint32_t src_clk_freq, int freq_hz,
											  uint32_t precision)
{
										 ...
										 ledc_slow_clk_sel_t *clk_target)
{
	...
	for (int i = 0; i < nitems(s_glb_clks); i++) {
		...
		if (s_glb_clks[i] == LEDC_SLOW_CLK_RC_FAST && s_ledc_slow_clk_rc_fast_freq == 0 &&
			!ledc_slow_clk_calibrate()) {
			pwminfo("Unable to retrieve RC_FAST clock frequency, skipping it");
			continue;
		}

		esp_clk_tree_src_get_freq_hz((soc_module_clk_t)s_glb_clks[i], LEDC_CLK_SRC_FREQ_PRECISION,
									 &clk_freq);
									  ...
									  ledc_slow_clk_sel_t *clk_target)
{
	...
	uint32_t div_param_global = ledc_auto_global_clk_div(freq_hz, precision, clk_target);
						  ...
						  ledc_clk_src_t clk_src)
{
	...
	ledc_hal_set_clock_divider(&(p_ledc_obj->ledc_hal), timer_sel, clock_divider);

	ledc_hal_set_duty_resolution(&(p_ledc_obj->ledc_hal), timer_sel, duty_resolution);
							  ...
							  int duty_resolution)
{
	...
	if (clk_cfg == LEDC_AUTO_CLK) {
		...
		div_param = ledc_auto_clk_divisor(freq_hz, precision, &timer_clk_src, &glb_clk);
		...
		esp_clk_tree_src_get_freq_hz((soc_module_clk_t)clk_cfg, LEDC_CLK_SRC_FREQ_PRECISION,
									 &src_clk_freq);
	...
	pwminfo("Using clock source %d (in slow mode). Divisor: 0x%" PRIx32, timer_clk_src, div_param);
	...
	if (p_ledc_obj->glb_clk != LEDC_SLOW_CLK_UNINIT && p_ledc_obj->glb_clk != glb_clk) {
		for (i = 0; i < LEDC_TIMER_MAX; i++) {
			if (i != timer_num && p_ledc_obj->glb_clk_is_acquired[i]) {
				leave_critical_section(flags);
				pwmerr("Timer clock conflict. Already is %d but attempt to %d", p_ledc_obj->glb_clk,
					   glb_clk);
			}
		}
	}

	if (timer_num == LEDC_TIMER_MAX - 1 && p_ledc_obj->glb_clk_is_acquired[timer_num - 1]) {
		return -EINVAL;
	}
...
static int setup_timer(struct esp_ledc_s *priv)
{
	...
	DEBUGASSERT(priv->duty_resolution < LEDC_TIMER_BIT_MAX && priv->duty_resolution > 0);
	...
	ret = ledc_set_timer_div(priv->timer, priv->clk_cfg, priv->frequency, priv->duty_resolution);
...
static IRAM_ATTR int ledc_duty_config(ledc_channel_t channel, int hpoint_val, int duty_val,
									  ledc_duty_direction_t duty_direction, uint32_t duty_num,
									  uint32_t duty_cycle, uint32_t duty_scale)
{
	if (hpoint_val >= 0) {
	...
	pwminfo("ledc_duty_config: channel: %d, hpoint_val: %d, duty_val: %d, "
			...
			channel, hpoint_val, duty_val, duty_direction, duty_num, duty_cycle, duty_scale);
	ledc_hal_set_fade_param(&(p_ledc_obj->ledc_hal), channel, 0, duty_direction, duty_cycle,
							duty_scale, duty_num);
 ...
 *
 * Input Parameters:
 *   roux_duty - Duty cycle value from roux.
...
static uint32_t ledc_duty_bin_conversion(uint16_t roux_duty, uint32_t duty_resolution)
{
	...
	uint32_t mapped_value = (roux_duty * max_value) / UINT16_MAX;
...
static void setup_channel(struct esp_ledc_s *priv, int cn)
{
	 ...
	 */
		...
		if (p_ledc_obj->glb_clk == LEDC_SLOW_CLK_UNINIT) {
			ledc_hal_set_slow_clk_sel(&(p_ledc_obj->ledc_hal), LEDC_LL_GLOBAL_CLK_DEFAULT);
		}
	...
	ledc_set_duty_with_hpoint(chan->num, chan->duty, CONFIG_ESPRESSIF_LEDC_HPOINT);
...
static int pwm_setup(struct pwm_lowerhalf_s *dev)
{
	...
	for (int i = 0; i < priv->channels; i++) {
		...
		pwminfo("Channel %d mapped to pin %d\n", priv->chans[i].num, priv->chans[i].pin);

		esp_configgpio(priv->chans[i].pin, OUTPUT | PULLUP);
		esp_gpio_matrix_out(priv->chans[i].pin, LEDC_LS_SIG_OUT0_IDX + priv->chans[i].num, 0, 0);
	}
...
static int pwm_start(struct pwm_lowerhalf_s *dev, const struct pwm_info_s *info)
{
	...
	if (priv->frequency != info->frequency) {
		pwminfo("Setting Timer %" PRIu8 " to frequency %" PRIu32 " Hz using %" PRIu8
				" bits of resolution\n",
				priv->timer, info->frequency, priv->duty_resolution);

		ret |=
			ledc_set_timer_div(priv->timer, priv->clk_cfg, info->frequency, priv->duty_resolution);
	...
	for (int i = 0; i < channels; i++) {
#ifdef CONFIG_PWM_NCHANNELS
		duty = ledc_duty_bin_conversion(info->channels[i].duty, priv->duty_resolution);
#else
		duty = ledc_duty_bin_conversion(info[i].duty, priv->duty_resolution);
#endif
		if (priv->chans[i].duty != duty) {
			uint32_t max_value = (1 << priv->duty_resolution) - 1;
			pwminfo("Setting PWM channel %" PRIu8 " to duty cycle %" PRIu32 "(%0.4f)\n",
					priv->chans[i].num, duty, (float)duty / max_value);

			flags = enter_critical_section();

			ret |= ledc_duty_config(priv->chans[i].num, LEDC_VAL_NO_CHANGE, duty,
									LEDC_DUTY_DIR_INCREASE, 1, 1, 0);
