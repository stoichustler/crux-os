...
#define SPI_VIA_IOMUX                                                                              \
	(SPI_IS_CS_IOMUX || SPI_HAVE_SWCS) && (SPI_IS_CLK_IOMUX) && (SPI_IS_MOSI_IOMUX) &&             \
		(SPI_IS_MISO_IOMUX)
...
static void esp_spi_select(struct spi_dev_s *dev, uint32_t devid, bool selected);
#endif
static uint32_t esp_spi_setfrequency(struct spi_dev_s *dev, uint32_t frequency);
...
static int esp_spi_hwfeatures(struct spi_dev_s *dev, spi_hwfeatures_t features);
#endif
static uint32_t esp_spi_send(struct spi_dev_s *dev, uint32_t wd);
static void esp_spi_exchange(struct spi_dev_s *dev, const void *txbuffer, void *rxbuffer,
							 size_t nwords);
#ifdef CONFIG_ESPRESSIF_SPI2_DMA
static int esp_spi_interrupt(int irq, void *context, void *arg);
static int esp_spi_sem_waitdone(struct esp_spi_priv_s *priv);
static void esp_spi_dma_exchange(struct esp_spi_priv_s *priv, const void *txbuffer, void *rxbuffer,
								 uint32_t nwords);
#else
static void esp_spi_poll_exchange(struct esp_spi_priv_s *priv, const void *txbuffer, void *rxbuffer,
								  size_t nwords);
#endif
#ifndef CONFIG_SPI_EXCHANGE
static void esp_spi_sndblock(struct spi_dev_s *dev, const void *txbuffer, size_t nwords);
static void esp_spi_recvblock(struct spi_dev_s *dev, void *rxbuffer, size_t nwords);
#endif
...
static spi_hal_context_t ctx = {0};

static spi_hal_dev_config_t dev_cfg = {
	.mode = SPI_DEFAULT_MODE, .cs_setup = 0, .cs_hold = 0, .cs_pin_id = 0, .timing_conf = {0}, {0}};
...
static const struct esp_spi_config_s esp_spi2_config = {.width = SPI_DEFAULT_WIDTH,
...
static uint32_t esp_spi_setfrequency(struct spi_dev_s *dev, uint32_t frequency)
{
	...
	{
		spi_ll_set_clk_source(priv->ctx->hw, priv->dev_cfg->timing_conf.clock_source);
	}

	esp_clk_tree_enable_src(SPI_CLK_SRC_DEFAULT, true);
	esp_clk_tree_src_get_freq_hz(SPI_CLK_SRC_DEFAULT, ESP_CLK_TREE_SRC_FREQ_PRECISION_CACHED,
								 ...
								 uint32_t nwords)
{
	...
	spi_hal_trans_config_t trans = {0};
	...
	while (bytes != 0) {
		...
		n = esp_dma_setup(channel, true, dma_txdesc, SPI_DMA_DESC_NUM, tp, bytes);
		...
		if (rp != NULL) {
			...
			esp_dma_setup(channel, false, dma_rxdesc, SPI_DMA_DESC_NUM, rp, bytes);
...
static uint32_t esp_spi_poll_send(struct esp_spi_priv_s *priv, uint32_t wd)
{
	...
	spi_hal_enable_data_line(priv->ctx->hw,
							 (!hal_dev->half_duplex && trans.rcv_buffer) || trans.send_buffer,
							 !!trans.rcv_buffer);
								  ...
								  size_t nwords)
{
	...
	while (bytes_remaining != 0) {
		...
		spi_hal_user_start(priv->ctx);
		spi_hal_enable_data_line(priv->ctx->hw,
								 (!priv->timing_param->half_duplex && trans.rcv_buffer) ||
									 trans.send_buffer,
								 !!trans.rcv_buffer);
...
struct spi_dev_s *esp_spibus_initialize(int port)
{
	...
	priv->cpuint =
		esp_setup_irq(priv->config->periph, ESP_IRQ_PRIORITY_DEFAULT, ESP_IRQ_TRIGGER_LEVEL);
	if (priv->cpuint < 0) {
