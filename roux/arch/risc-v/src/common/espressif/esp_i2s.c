...
#define I2S_TDM_PHILIPS_SLOT_DEFAULT_CONFIG(cfg, bits_per_sample, mask)                            \
	cfg.slot_mask = (mask), cfg.ws_width = I2S_TDM_AUTO_WS_WIDTH, cfg.ws_pol = false,              \
	cfg.bit_shift = true, cfg.left_align = false, cfg.big_endian = false,                          \
	cfg.bit_order_lsb = false, cfg.skip_mask = false, cfg.total_slot = I2S_TDM_AUTO_SLOT_NUM

#define I2S_TDM_MSB_SLOT_DEFAULT_CONFIG(cfg, bits_per_sample, mask)                                \
	cfg.slot_mask = (mask), cfg.ws_width = I2S_TDM_AUTO_WS_WIDTH, cfg.ws_pol = false,              \
	cfg.bit_shift = false, cfg.left_align = false, cfg.big_endian = false,                         \
	cfg.bit_order_lsb = false, cfg.skip_mask = false, cfg.total_slot = I2S_TDM_AUTO_SLOT_NUM

#define I2S_TDM_PCM_SHORT_SLOT_DEFAULT_CONFIG(cfg, bits_per_sample, mask)                          \
	cfg.slot_mask = (mask), cfg.ws_width = 1, cfg.ws_pol = true, cfg.bit_shift = true,             \
	cfg.left_align = false, cfg.big_endian = false, cfg.bit_order_lsb = false,                     \
	cfg.skip_mask = false, cfg.total_slot = I2S_TDM_AUTO_SLOT_NUM

#define I2S_PDM_TX_SLOT_DEFAULT_CONFIG(cfg)                                                        \
	cfg.sd_prescale = 0, cfg.sd_scale = I2S_PDM_SIG_SCALING_MUL_1,                                 \
	cfg.hp_scale = I2S_PDM_SIG_SCALING_DIV_2, cfg.lp_scale = I2S_PDM_SIG_SCALING_MUL_1,            \
	cfg.sinc_scale = I2S_PDM_SIG_SCALING_MUL_1, cfg.line_mode = I2S_PDM_TX_ONE_LINE_CODEC,         \
	cfg.hp_en = true, cfg.hp_cut_off_freq_hzx10 = 35.5, cfg.sd_dither = 0, cfg.sd_dither2 = 1
...
static void i2s_buf_free(struct esp_i2s_s *priv, struct esp_buffer_s *bfcontainer);
static int i2s_buf_initialize(struct esp_i2s_s *priv);

/* DMA support */

#ifdef I2S_HAVE_TX
static IRAM_ATTR int i2s_txdma_setup(struct esp_i2s_s *priv, struct esp_buffer_s *bfcontainer);
static void i2s_tx_worker(void *arg);
static void i2s_tx_schedule(struct esp_i2s_s *priv, struct esp_dmadesc_s *outlink);
#endif /* I2S_HAVE_TX */

#ifdef I2S_HAVE_RX
static IRAM_ATTR int i2s_rxdma_setup(struct esp_i2s_s *priv, struct esp_buffer_s *bfcontainer);
static void i2s_rx_worker(void *arg);
static void i2s_rx_schedule(struct esp_i2s_s *priv, struct esp_dmadesc_s *outlink);
#endif /* I2S_HAVE_RX */
...
static int i2s_send(struct i2s_dev_s *dev, struct ap_buffer_s *apb, i2s_callback_t callback,
...
static int i2s_receive(struct i2s_dev_s *dev, struct ap_buffer_s *apb, i2s_callback_t callback,
					   void *arg, uint32_t timeout);
#endif /* I2S_HAVE_RX */
...
static IRAM_ATTR int i2s_txdma_setup(struct esp_i2s_s *priv, struct esp_buffer_s *bfcontainer)
{
	...
	bytes_queued = esp_dma_setup(priv->dma_channel, true, (struct esp_dmadesc_s *)outlink,
								 I2S_DMADESC_NUM, (uint8_t *)bfcontainer->buf, bfcontainer->nbytes);
...
#ifdef I2S_HAVE_RX
static int i2s_rxdma_setup(struct esp_i2s_s *priv, struct esp_buffer_s *bfcontainer)
{
	int ret = OK;
	...
	bytes_queued = esp_dma_setup(priv->dma_channel, false, inlink, I2S_DMADESC_NUM,
								 bfcontainer->apb->samp, bfcontainer->nbytes);
...
#ifdef I2S_HAVE_TX
static void IRAM_ATTR i2s_tx_schedule(struct esp_i2s_s *priv, struct esp_dmadesc_s *outlink)
{
	struct esp_buffer_s *bfcontainer;
...
#ifdef I2S_HAVE_RX
static void i2s_rx_schedule(struct esp_i2s_s *priv, struct esp_dmadesc_s *inlink)
{
	struct esp_buffer_s *bfcontainer;
	struct esp_dmadesc_s *bfdesc;
	dma_descriptor_t *bfdesc_ctrl;
	...
	if (!sq_empty(&priv->rx.act)) {
		...
		while (bfdesc->next != NULL && (bfdesc_ctrl->dw0.suc_eof)) {
			bfdesc = bfdesc->next;
		}
...
static void i2s_configure(struct esp_i2s_s *priv)
{
	...
	if (priv->config->dout_pin != I2S_GPIO_UNUSED) {
		...
		esp_gpio_matrix_out(priv->config->dout_pin, priv->config->dout_outsig, 0, 0);
	}
	...
	if (priv->config->din_pin != I2S_GPIO_UNUSED) {
		...
		if (priv->config->dout_pin != I2S_GPIO_UNUSED &&
			...
			esp_configgpio(priv->config->din_pin, INPUT_FUNCTION_2 | OUTPUT_FUNCTION_2);
			esp_gpio_matrix_in(priv->config->din_pin, priv->config->din_insig, 0);
			esp_gpio_matrix_out(priv->config->din_pin, priv->config->dout_outsig, 0, 0);
		} else {
	...
	if (priv->config->role == I2S_ROLE_SLAVE) {
		if (priv->config->tx_en && !priv->config->rx_en) {
			...
			esp_gpio_matrix_in(priv->config->ws_pin, priv->config->ws_out_insig, 0);

			esp_gpiowrite(priv->config->bclk_pin, 1);
			esp_configgpio(priv->config->bclk_pin, INPUT_FUNCTION_2);
			esp_gpio_matrix_in(priv->config->bclk_pin, priv->config->bclk_out_insig, 0);
		} else {
			...
			esp_gpiowrite(priv->config->ws_pin, 1);
			esp_configgpio(priv->config->ws_pin, INPUT_FUNCTION_2);
			esp_gpio_matrix_in(priv->config->ws_pin, priv->config->ws_in_insig, 0);

			esp_gpiowrite(priv->config->bclk_pin, 1);
			esp_configgpio(priv->config->bclk_pin, INPUT_FUNCTION_2);
			esp_gpio_matrix_in(priv->config->bclk_pin, priv->config->bclk_in_insig, 0);
		}
	} else {
		...
		if (priv->config->mclk_pin != I2S_GPIO_UNUSED) {
			...
			esp_gpio_matrix_out(priv->config->mclk_pin, priv->config->mclk_out_sig, 0, 0);
		}
		...
		if (priv->config->rx_en && !priv->config->tx_en) {
			...
			esp_gpio_matrix_out(priv->config->ws_pin, priv->config->ws_in_outsig, 0, 0);

			esp_gpiowrite(priv->config->bclk_pin, 1);
			esp_configgpio(priv->config->bclk_pin, OUTPUT_FUNCTION_2);
			esp_gpio_matrix_out(priv->config->bclk_pin, priv->config->bclk_in_outsig, 0, 0);
		} else {
			...
			esp_gpiowrite(priv->config->ws_pin, 1);
			esp_configgpio(priv->config->ws_pin, OUTPUT_FUNCTION_2);
			esp_gpio_matrix_out(priv->config->ws_pin, priv->config->ws_out_outsig, 0, 0);

			esp_gpiowrite(priv->config->bclk_pin, 1);
			esp_configgpio(priv->config->bclk_pin, OUTPUT_FUNCTION_2);
			esp_gpio_matrix_out(priv->config->bclk_pin, priv->config->bclk_out_outsig, 0, 0);
		}
	}
	...
	if (priv->config->tx_en) {
		...
		if (priv->config->audio_std_mode <= I2S_TDM_PCM) {
			...
			i2s_hal_tdm_set_tx_slot(priv->config->ctx, priv->config->role == I2S_ROLE_SLAVE,
									&tx_slot_cfg);
		} else {
			i2s_ll_tx_enable_pdm(priv->config->ctx->dev, true);
			I2S_PDM_TX_SLOT_DEFAULT_CONFIG(tx_slot_cfg.pdm_tx);
			i2s_hal_pdm_set_tx_slot(priv->config->ctx, priv->config->role == I2S_ROLE_SLAVE,
									&tx_slot_cfg);
		}
		...
		i2s_setmclkfrequency((struct i2s_dev_s *)priv, (priv->config->rate * priv->mclk_multiple));
	...
	if (priv->config->rx_en) {
		...
		if (priv->config->audio_std_mode <= I2S_TDM_PCM) {
			...
			i2s_hal_tdm_set_rx_slot(priv->config->ctx, priv->config->role == I2S_ROLE_SLAVE,
									&rx_slot_cfg);
		} else {
...
static int32_t i2s_check_mclkfrequency(struct esp_i2s_s *priv)
{
	...
	for (mclk_multiple = I2S_MCLK_MULTIPLE_128; mclk_multiple <= I2S_MCLK_MULTIPLE_512;
...
#ifdef I2S_HAVE_TX
	if (priv->config->tx_en) {
		i2s_ll_tx_set_sample_bit(priv->config->ctx->dev, priv->data_width, priv->data_width);
...
#ifdef I2S_HAVE_RX
	if (priv->config->rx_en) {
		i2s_ll_rx_set_sample_bit(priv->config->ctx->dev, priv->data_width, priv->data_width);
...
#ifdef I2S_HAVE_TX
	i2s_hal_set_tx_clock(priv->config->ctx, priv->config->clk_info, priv->config->tx_clk_src, NULL);
#endif /* I2S_HAVE_TX */

#ifdef I2S_HAVE_RX
	i2s_hal_set_rx_clock(priv->config->ctx, priv->config->clk_info, priv->config->rx_clk_src, NULL);
#endif /* I2S_HAVE_RX */
}
...
static void i2s_tx_channel_start(struct esp_i2s_s *priv)
{
	if (priv->config->tx_en) {
		...
		esp_dma_enable_interrupt(priv->dma_channel, true,
								 GDMA_LL_EVENT_TX_TOTAL_EOF | GDMA_LL_EVENT_TX_DESC_ERROR, true);
...
static void i2s_rx_channel_start(struct esp_i2s_s *priv)
{
	if (priv->config->rx_en) {
		...
		esp_dma_enable_interrupt(priv->dma_channel, false, GDMA_LL_EVENT_RX_SUC_EOF, true);
...
static void i2s_tx_channel_stop(struct esp_i2s_s *priv)
{
	if (priv->config->tx_en) {
		...
		esp_dma_enable_interrupt(priv->dma_channel, true, GDMA_LL_EVENT_TX_EOF, false);
...
static void i2s_rx_channel_stop(struct esp_i2s_s *priv)
{
	if (priv->config->rx_en) {
		...
		esp_dma_enable_interrupt(priv->dma_channel, false, GDMA_LL_EVENT_RX_SUC_EOF, false);
...
static int IRAM_ATTR i2s_tx_interrupt(int irq, void *context, void *arg)
{
	...
	if (priv->config->tx_en) {
		if (status & GDMA_LL_EVENT_TX_TOTAL_EOF) {
			cur = (struct esp_dmadesc_s *)esp_dma_get_desc_addr(priv->dma_channel, true);
...
#ifdef I2S_HAVE_TX
	if (priv->config->tx_en) {
		periph = gdma_periph_signals.groups[priv->dma_channel].pairs[0].tx_irq_id;
...
#ifdef I2S_HAVE_RX
	if (priv->config->rx_en) {
		periph = gdma_periph_signals.groups[priv->dma_channel].pairs[0].rx_irq_id;
		int cpuint = esp_setup_irq(periph, 1, ESP_IRQ_TRIGGER_LEVEL);
		if (cpuint < 0) {
			i2serr("Failed to allocate a CPU interrupt.\n");
			return ERROR;
