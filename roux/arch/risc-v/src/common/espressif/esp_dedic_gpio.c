...
static int esp_dedic_gpio_bundle_read(struct file *filep, char *buffer, size_t buflen);
static int esp_dedic_gpio_bundle_write(struct file *filep, const char *buffer, size_t buflen);
...
void esp_dedic_gpio_write(struct file *dev, uint32_t mask, uint32_t value)
{
	struct esp_dedic_gpio_bundle_priv_s *priv = (struct esp_dedic_gpio_bundle_priv_s *)dev;
...
struct file *esp_dedic_gpio_new_bundle(struct esp_dedic_gpio_config_s *config)
{
	...
	DEBUGASSERT(config->flags->input_enable || config->flags->output_enable > 0);
	...
	if (priv) {
		...
		if (config->flags->output_enable) {
			...
			for (int i = 0; i <= SOC_DEDIC_GPIO_OUT_CHANNELS_NUM - config->array_size; i++) {
				if ((dedic_gpio_common.out_occupied_mask & (pattern << i)) == 0) {
					out_mask = pattern << i;
					out_offset = i;
					break;
				}
			...
			attr |= OUTPUT;
			gpioinfo("New out bundle(%p), offset=%" PRIu32 ", mask(%" PRIx32 ")", priv,
					 priv->out_offset, priv->out_mask);
		}
		...
		if (config->flags->input_enable) {
			...
			for (int i = 0; i <= SOC_DEDIC_GPIO_IN_CHANNELS_NUM - config->array_size; i++) {
			...
			attr |= INPUT;
			gpioinfo("New in bundle(%p), offset=%" PRIu32 ", mask(%" PRIx32 ")", priv,
					 priv->in_offset, priv->in_mask);
		}
	...
	register_driver(config->path, &dedic_gpio_ops, 0666, priv);
