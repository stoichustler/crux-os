...
static uart_hal_context_t g_uart0_hal = {.dev = &UART0};

struct esp_uart_s g_uart0_config = {.source = UART0_INTR_SOURCE,
									.cpuint = -ENOMEM,
...
static uart_hal_context_t g_uart1_hal = {.dev = &UART1};

struct esp_uart_s g_uart1_config = {.source = UART1_INTR_SOURCE,
									.cpuint = -ENOMEM,
									.int_pri = ESP_IRQ_PRIORITY_DEFAULT,
									.id = 1,
									.irq = ESP_IRQ_UART1,
...
static uart_hal_context_t g_lp_uart0_hal = {.dev = (hal_uart_dev_t *)&LP_UART};

struct esp_uart_s g_lp_uart0_config = {.source = LP_UART_INTR_SOURCE,
									   .cpuint = -ENOMEM,
									   .int_pri = ESP_IRQ_PRIORITY_DEFAULT,
									   .id = ESP_LP_UART0_ID,
									   .irq = ESP_IRQ_LP_UART,
...
static void esp_lowputc_lp_uart_config_io(const struct esp_uart_s *priv, int8_t pin,
										  rtc_gpio_mode_t direction, uint32_t idx)
{
...
void esp_lowputc_disable_all_uart_int(struct esp_uart_s *priv, uint32_t *current_status)
{
	irqstate_t flags;
...
void esp_lowputc_restore_all_uart_int(const struct esp_uart_s *priv, uint32_t *last_status)
{
...
void esp_lowputc_config_pins(const struct esp_uart_s *priv)
{
	...
	if (priv->id < ESP_LP_UART0_ID) {
		...
		esp_lowputc_lp_uart_config_io(priv, priv->rxpin, RTC_GPIO_MODE_INPUT_ONLY,
									  SOC_UART_RX_PIN_IDX);

		esp_lowputc_lp_uart_config_io(priv, priv->txpin, RTC_GPIO_MODE_OUTPUT_ONLY,
									  SOC_UART_TX_PIN_IDX);

#ifdef CONFIG_SERIAL_IFLOWCONTROL
		if (priv->iflow) {
			esp_lowputc_lp_uart_config_io(priv, priv->rtspin, RTC_GPIO_MODE_OUTPUT_ONLY,
...
#ifdef CONFIG_SERIAL_OFLOWCONTROL
		if (priv->oflow) {
			esp_lowputc_lp_uart_config_io(priv, priv->ctspin, RTC_GPIO_MODE_INPUT_ONLY,
