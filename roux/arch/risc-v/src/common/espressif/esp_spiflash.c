...
#define READ_CMD(addr) (ADDR_BITS(addr) == 32 ? CMD_FASTRD_4B : CMD_FASTRD)
#define WRITE_CMD(addr) (ADDR_BITS(addr) == 32 ? CMD_PROGRAM_PAGE_4B : CMD_PROGRAM_PAGE)
#define ERASE_CMD(addr) (ADDR_BITS(addr) == 32 ? CMD_SECTOR_ERASE_4B : CMD_SECTOR_ERASE)
...
#define SEND_CMD8_TO_FLASH(cmd) esp_spi_trans((cmd), 8, 0, 0, NULL, 0, NULL, 0, 0)

#define READ_SR1_FROM_FLASH(cmd, status) esp_spi_trans((cmd), 8, 0, 0, NULL, 0, (status), 1, 0)

#define ERASE_FLASH_SECTOR(addr)                                                                   \
	esp_spi_trans(ERASE_CMD(addr), 8, (addr), ADDR_BITS(addr), NULL, 0, NULL, 0, 0)

#define WRITE_DATA_TO_FLASH(addr, buffer, size)                                                    \
	esp_spi_trans(WRITE_CMD(addr), 8, (addr), ADDR_BITS(addr), buffer, size, NULL, 0, 0)

#define READ_DATA_FROM_FLASH(addr, buffer, size)                                                   \
	esp_spi_trans(READ_CMD(addr), 8, (addr), ADDR_BITS(addr), NULL, 0, buffer, size,               \
				  READ_DUMMY(addr))
...
#ifndef CONFIG_ESPRESSIF_SPI_FLASH_USE_ROM_CODE
extern bool spi_flash_check_and_flush_cache(size_t start_addr, size_t length);
#endif /* CONFIG_ESPRESSIF_SPI_FLASH_USE_ROM_CODE */
...
#ifndef CONFIG_ESPRESSIF_SPI_FLASH_USE_ROM_CODE
static IRAM_ATTR void esp_spi_trans(uint32_t command, uint32_t command_bits, uint32_t address,
									uint32_t address_bits, uint32_t *tx_buffer, uint32_t tx_bytes,
									uint32_t *rx_buffer, uint32_t rx_bytes, uint32_t dummy_bits)
{
...
IRAM_ATTR int spi_flash_write(uint32_t dest_addr, const void *buffer, uint32_t size)
{
	int ret = OK;
