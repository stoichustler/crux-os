...
#include <roux/timers/capture.h>
#endif
#ifdef CONFIG_ESP_MCPWM_MOTOR
#include <roux/motor/motor.h>
#endif
...
#ifdef CONFIG_ESP_MCPWM_CAPTURE
static int esp_mcpwm_capture_set_gpio(struct mcpwm_cap_channel_lowerhalf_s *lower);
...
static int esp_capture_getduty(struct cap_lowerhalf_s *lower, uint8_t *duty);
static int esp_capture_getfreq(struct cap_lowerhalf_s *lower, uint32_t *freq);
#endif
...
static int esp_motor_params_set(struct motor_lowerhalf_s *dev, struct motor_params_s *param);
static int esp_motor_state_get(struct motor_lowerhalf_s *dev, struct motor_state_s *state);
static int esp_motor_limits_set(struct motor_lowerhalf_s *dev, struct motor_limits_s *limits);
static int esp_motor_fault_clear(struct motor_lowerhalf_s *dev, uint8_t fault);
static int esp_motor_ioctl(struct motor_lowerhalf_s *dev, int cmd, unsigned long arg);

/* Lower-half motor functions */

static int esp_motor_pwm_config(struct mcpwm_motor_lowerhalf_s *lower);
static int esp_mcpwm_motor_set_gpio(struct mcpwm_motor_lowerhalf_s *lower, bool enable);
static int esp_motor_set_duty_cycle(struct mcpwm_motor_lowerhalf_s *lower, float duty);
...
#ifdef ESP_MCPMW_MOTOR_FAULT
static int esp_mcpwm_fault_gpio_config(struct mcpwm_motor_lowerhalf_s *lower, bool enable);
static int esp_motor_fault_configure(struct mcpwm_motor_lowerhalf_s *lower, bool enable);
#endif
...
static int IRAM_ATTR mcpwm_driver_isr_default(int irq, void *context, void *arg);
#endif
...
#if defined(CONFIG_ESP_MCPWM_MOTOR_CH0) && defined(CONFIG_ESP_MCPWM_MOTOR_BDC)
...
static int esp_motor_setup(struct motor_lowerhalf_s *dev)
{
	...
	struct mcpwm_motor_lowerhalf_s *priv = (struct mcpwm_motor_lowerhalf_s *)dev;
	...
	if ((priv->state.state == MOTOR_STATE_FAULT) || (priv->state.state == MOTOR_STATE_CRITICAL)) {
	...
	esp_clk_tree_src_get_freq_hz(MCPWM_DEV_CLK_SOURCE, ESP_CLK_TREE_SRC_FREQ_PRECISION_CACHED,
								 &base_clock);
	...
	priv->timer_prescale = base_clock / ((2 * priv->counter_peak + 1) * priv->pwm_frequency);

	mcpwm_ll_timer_set_peak(hal->dev, priv->timer_id, priv->counter_peak, true);
	mcpwm_ll_timer_set_clock_prescale(hal->dev, priv->timer_id, priv->timer_prescale);
	...
	mcpwm_ll_timer_set_count_mode(hal->dev, priv->timer_id, MCPWM_TIMER_COUNT_MODE_UP_DOWN);

	mcpwm_ll_operator_flush_shadow(hal->dev, priv->operator_id);
	mcpwm_ll_operator_connect_timer(hal->dev, priv->operator_id, priv->timer_id);
	mcpwm_ll_operator_enable_update_compare_on_tez(hal->dev, priv->operator_id, MCPWM_GENERATOR_0,
												   true);
	...
	mtrinfo("Channel %d starts: prescale %d | freq: %" PRIu32 "\n", priv->channel_id,
			priv->timer_prescale - 1, priv->pwm_frequency);
	return OK;
}
#endif
...
static int esp_motor_shutdown(struct motor_lowerhalf_s *dev)
{
	...
	mcpwm_ll_timer_set_count_mode(hal->dev, priv->timer_id, MCPWM_TIMER_COUNT_MODE_PAUSE);
	mcpwm_ll_timer_set_start_stop_command(hal->dev, priv->timer_id, MCPWM_TIMER_STOP_EMPTY);
...
static int esp_motor_stop(struct motor_lowerhalf_s *dev)
{
	...
	mcpwm_ll_timer_set_start_stop_command(hal->dev, priv->timer_id, MCPWM_TIMER_STOP_EMPTY);
	mcpwm_ll_gen_set_continue_force_level(hal->dev, priv->operator_id, MCPWM_GENERATOR_0, 0);
	mcpwm_ll_gen_set_continue_force_level(hal->dev, priv->operator_id, MCPWM_GENERATOR_1, 0);
...
static int esp_motor_start(struct motor_lowerhalf_s *dev)
{
	...
	if ((priv->state.state == MOTOR_STATE_CRITICAL) || (priv->state.state == MOTOR_STATE_FAULT)) {
		spin_unlock_irqrestore(&g_mcpwm_common.mcpwm_spinlock, flags);
	...
	mcpwm_ll_timer_set_start_stop_command(hal->dev, priv->timer_id, MCPWM_TIMER_START_NO_STOP);
...
static int esp_motor_fault_configure(struct mcpwm_motor_lowerhalf_s *lower, bool enable)
{
	...
	if (!enable) {
		...
		mcpwm_ll_intr_enable(hal->dev, MCPWM_LL_EVENT_FAULT_ENTER(lower->fault_id), false);
		mcpwm_ll_intr_enable(hal->dev, MCPWM_LL_EVENT_FAULT_EXIT(lower->fault_id), false);
		return OK;
	}
	...
	mcpwm_ll_brake_enable_oneshot_mode(hal->dev, lower->operator_id, lower->fault_id, true);
	...
	mcpwm_ll_generator_set_action_on_brake_event(hal->dev, lower->operator_id, MCPWM_GENERATOR_0,
												 MCPWM_TIMER_DIRECTION_UP,
												 MCPWM_OPER_BRAKE_MODE_OST, 1);
	mcpwm_ll_generator_set_action_on_brake_event(hal->dev, lower->operator_id, MCPWM_GENERATOR_0,
												 MCPWM_TIMER_DIRECTION_DOWN,
												 MCPWM_OPER_BRAKE_MODE_OST, 1);
	mcpwm_ll_generator_set_action_on_brake_event(hal->dev, lower->operator_id, MCPWM_GENERATOR_1,
												 MCPWM_TIMER_DIRECTION_UP,
												 MCPWM_OPER_BRAKE_MODE_OST, 1);
	mcpwm_ll_generator_set_action_on_brake_event(hal->dev, lower->operator_id, MCPWM_GENERATOR_1,
												 MCPWM_TIMER_DIRECTION_DOWN,
	...
	mcpwm_ll_intr_enable(hal->dev, MCPWM_LL_EVENT_FAULT_ENTER(lower->fault_id), true);
	mcpwm_ll_intr_enable(hal->dev, MCPWM_LL_EVENT_FAULT_EXIT(lower->fault_id), true);
	mcpwm_ll_intr_clear_status(hal->dev, MCPWM_LL_EVENT_FAULT_ENTER(lower->fault_id));
	mcpwm_ll_intr_clear_status(hal->dev, MCPWM_LL_EVENT_FAULT_EXIT(lower->fault_id));

	spin_unlock_irqrestore(&g_mcpwm_common.mcpwm_spinlock, flags);
	mtrinfo("Brake configured for motor channel %d on fault id %d", lower->channel_id,
			lower->fault_id);
	return OK;
}
#endif
...
static int esp_motor_set_duty_cycle(struct mcpwm_motor_lowerhalf_s *lower, float duty)
{
	...
	mcpwm_ll_operator_set_compare_value(hal->dev, lower->operator_id, MCPWM_GENERATOR_0, pwm_count);
...
static int esp_motor_bdc_set_direction(struct mcpwm_motor_lowerhalf_s *lower)
{
	...
	if (lower->opmode == MOTOR_OPMODE_SPEED) {
		...
		if (lower->param.direction == MOTOR_DIR_CW) {
			mcpwm_ll_gen_disable_continue_force_action(hal->dev, lower->operator_id,
													   MCPWM_GENERATOR_0);
			mcpwm_ll_gen_set_continue_force_level(hal->dev, lower->operator_id, MCPWM_GENERATOR_1,
												  0);
		} else {
			mcpwm_ll_gen_disable_continue_force_action(hal->dev, lower->operator_id,
													   MCPWM_GENERATOR_1);
			mcpwm_ll_gen_set_continue_force_level(hal->dev, lower->operator_id, MCPWM_GENERATOR_0,
												  0);
		}

		direction = lower->param.direction;
#else
		mcpwm_ll_gen_disable_continue_force_action(hal->dev, lower->operator_id, MCPWM_GENERATOR_0);
		mcpwm_ll_gen_set_continue_force_level(hal->dev, lower->operator_id, MCPWM_GENERATOR_1, 0);
...
static int esp_motor_pwm_config(struct mcpwm_motor_lowerhalf_s *lower)
{
	...
	if (lower->opmode == MOTOR_OPMODE_SPEED) {
		...
		mcpwm_ll_generator_set_action_on_compare_event(hal->dev, lower->operator_id,
													   MCPWM_GENERATOR_0, MCPWM_TIMER_DIRECTION_UP,
													   MCPWM_GENERATOR_0, MCPWM_GEN_ACTION_HIGH);
		mcpwm_ll_generator_set_action_on_compare_event(
			hal->dev, lower->operator_id, MCPWM_GENERATOR_0, MCPWM_TIMER_DIRECTION_DOWN,
			MCPWM_GENERATOR_0, MCPWM_GEN_ACTION_LOW);

		/* PWM_B Output */

		mcpwm_ll_generator_set_action_on_compare_event(hal->dev, lower->operator_id,
													   MCPWM_GENERATOR_1, MCPWM_TIMER_DIRECTION_UP,
													   MCPWM_GENERATOR_0, MCPWM_GEN_ACTION_HIGH);
		mcpwm_ll_generator_set_action_on_compare_event(
			hal->dev, lower->operator_id, MCPWM_GENERATOR_1, MCPWM_TIMER_DIRECTION_DOWN,
			MCPWM_GENERATOR_0, MCPWM_GEN_ACTION_LOW);
	} else {
...
static int esp_mcpwm_fault_gpio_config(struct mcpwm_motor_lowerhalf_s *lower, bool enable)
{
	...
	esp_gpio_matrix_in(lower->fault_pin,
					   mcpwm_periph_signals.groups[MCPWM_CAPTURE_DEFAULT_GROUP]
						   .gpio_faults[lower->fault_id]
						   .fault_sig,
					   false);
...
static int esp_mcpwm_motor_set_gpio(struct mcpwm_motor_lowerhalf_s *lower, bool enable)
{
	...
	if (!enable) {
		esp_gpio_matrix_out(lower->generator_pins[MCPWM_GENERATOR_0], 0x100, false, false);

		esp_gpio_matrix_out(lower->generator_pins[MCPWM_GENERATOR_1], 0x100, false, false);
		return OK;
	}

	ret = esp_configgpio(lower->generator_pins[MCPWM_GENERATOR_0], GPIO_OUT_FUNCTION);
	if (ret < 0) {
		mtrerr("Failed configuring PWM_A GPIO\n");
		return ret;
	}

	ret = esp_configgpio(lower->generator_pins[MCPWM_GENERATOR_1], GPIO_OUT_FUNCTION);
	if (ret < 0) {
		mtrerr("Failed configuring PWM_B GPIO\n");
		return ret;
	}

	esp_gpio_matrix_out(lower->generator_pins[MCPWM_GENERATOR_0],
						mcpwm_periph_signals.groups[MCPWM_CAPTURE_DEFAULT_GROUP]
							.operators[lower->channel_id]
							.generators[MCPWM_GENERATOR_0]
							.pwm_sig,
						false, false);

	esp_gpio_matrix_out(lower->generator_pins[MCPWM_GENERATOR_1],
						mcpwm_periph_signals.groups[MCPWM_CAPTURE_DEFAULT_GROUP]
							.operators[lower->channel_id]
							.generators[MCPWM_GENERATOR_1]
							.pwm_sig,
						false, false);
...
static int esp_capture_start(struct cap_lowerhalf_s *lower)
{
	...
	struct mcpwm_cap_channel_lowerhalf_s *priv = (struct mcpwm_cap_channel_lowerhalf_s *)lower;
	irqstate_t flags;
	mcpwm_hal_context_t *hal = &priv->common->hal;
	...
	mcpwm_ll_intr_enable(hal->dev, MCPWM_LL_EVENT_CAPTURE(priv->channel_id), true);
	mcpwm_ll_intr_clear_status(hal->dev, MCPWM_LL_EVENT_CAPTURE(priv->channel_id));
...
static int esp_capture_stop(struct cap_lowerhalf_s *lower)
{
	...
	mcpwm_ll_intr_enable(hal->dev, MCPWM_LL_EVENT_CAPTURE(priv->channel_id), false);
	priv->enabled = false;
...
static void esp_mcpwm_group_start(void)
{
	...
	mcpwm_ll_group_set_clock_source(g_mcpwm_common.group.group_id, MCPWM_DEV_CLK_SOURCE);
	mcpwm_ll_group_set_clock_prescale(g_mcpwm_common.group.group_id, g_mcpwm_common.group_prescale);
...
static int esp_mcpwm_capture_set_gpio(struct mcpwm_cap_channel_lowerhalf_s *lower)
{
	...
	esp_gpio_matrix_in(lower->gpio_pin,
					   mcpwm_periph_signals.groups[0].captures[lower->channel_id].cap_sig, false);

	cpinfo("GPIO: %d configured for channel %d\n", lower->gpio_pin, lower->channel_id);
	return ret;
}
#endif
...
static int esp_mcpwm_isr_register(int (*fn)(int, void *, void *), void *arg)
{
	...
	cpuint = esp_setup_irq(mcpwm_periph_signals.groups[0].irq_id, ESP_IRQ_PRIORITY_DEFAULT,
	...
	ret = irq_attach(ESP_IRQ_MCPWM0, fn, &g_mcpwm_common);
	if (ret < 0) {
...
static int IRAM_ATTR mcpwm_driver_isr_default(int irq, void *context, void *arg)
{
	...
	if (status & MCPWM_LL_EVENT_FAULT_ENTER(MCPWM_FAULT_0)) {
		...
		mcpwm_ll_intr_clear_status(common->hal.dev,
								   status & MCPWM_LL_EVENT_FAULT_ENTER(MCPWM_FAULT_0));
		esp_motor_fault_set((struct motor_lowerhalf_s *)priv, MOTOR_FAULT_OTHER);
	...
	if (status & MCPWM_LL_EVENT_FAULT_EXIT(MCPWM_FAULT_0)) {
		priv = &mcpwm_bdc_ch0_lowerhalf;
		mcpwm_ll_intr_clear_status(common->hal.dev,
								   status & MCPWM_LL_EVENT_FAULT_EXIT(MCPWM_FAULT_0));
		esp_motor_fault_clear((struct motor_lowerhalf_s *)priv, MOTOR_FAULT_OTHER);
	...
	mcpwm_ll_intr_clear_status(common->hal.dev, status & MCPWM_LL_EVENT_CAPTURE(lower->channel_id));
	...
	if (cap_edge == MCPWM_CAP_EDGE_POS) {
		...
		mcpwm_ll_capture_enable_negedge(common->hal.dev, lower->channel_id, true);
		mcpwm_ll_capture_enable_posedge(common->hal.dev, lower->channel_id, false);
	} else {
		...
		mcpwm_ll_capture_enable_negedge(common->hal.dev, lower->channel_id, false);
		mcpwm_ll_capture_enable_posedge(common->hal.dev, lower->channel_id, true);
	}
#endif
...
#ifdef CONFIG_ESP_MCPWM_MOTOR_BDC
struct motor_lowerhalf_s *esp_motor_bdc_initialize(int channel, uint32_t frequency, int pwm_a_pin,
												   int pwm_b_pin, int fault_pin)
{
	struct mcpwm_motor_lowerhalf_s *lower = NULL;
	uint32_t ref_clock;
	int ret;

	if ((frequency > MCPWM_MAX_PWM_OUT_FREQ) || (frequency < MCPWM_MIN_PWM_OUT_FREQ)) {
		mtrerr("Invalid frequency set. Must be between %d and %d Hz\n", MCPWM_MIN_PWM_OUT_FREQ,
			   MCPWM_MAX_PWM_OUT_FREQ);
		return NULL;
	}
	...
	mtrinfo("Ch %d initialized. GPIO: PWM_A: %d | PWM_B: %d | Freq: %lu\n", lower->channel_id,
			lower->generator_pins[MCPWM_GENERATOR_0], lower->generator_pins[MCPWM_GENERATOR_1],
			lower->pwm_frequency);
