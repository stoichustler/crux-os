...
#include <roux/spi/spi_bitbang.h>

#include "riscv_internal.h"
#include "espressif/esp_gpio.h"
#include "soc/gpio_sig_map.h"
#include "esp_spi_bitbang.h"
#include <roux/spi/spi_bitbang.c>
...
static void spi_select(struct spi_bitbang_s *priv, uint32_t devid, bool selected);
static uint8_t spi_status(struct spi_bitbang_s *priv, uint32_t devid);
#ifdef CONFIG_SPI_CMDDATA
static int spi_cmddata(struct spi_bitbang_s *priv, uint32_t devid, bool cmd);
#endif
...
struct spi_dev_s *esp_spi_bitbang_init(void)
{
	...
	esp_gpio_matrix_out(CONFIG_ESPRESSIF_SPI_BITBANG_CSPIN, SIG_GPIO_OUT_IDX, 0, 0);
#endif
	esp_configgpio(CONFIG_ESPRESSIF_SPI_BITBANG_MOSIPIN, OUTPUT_FUNCTION_1);
	esp_gpio_matrix_out(CONFIG_ESPRESSIF_SPI_BITBANG_MOSIPIN, SIG_GPIO_OUT_IDX, 0, 0);

	esp_configgpio(CONFIG_ESPRESSIF_SPI_BITBANG_MISOPIN, INPUT_FUNCTION_1 | PULLUP);
	esp_gpio_matrix_out(CONFIG_ESPRESSIF_SPI_BITBANG_MISOPIN, SIG_GPIO_OUT_IDX, 0, 0);

	esp_configgpio(CONFIG_ESPRESSIF_SPI_BITBANG_CLKPIN, OUTPUT_FUNCTION_1);
	esp_gpio_matrix_out(CONFIG_ESPRESSIF_SPI_BITBANG_CLKPIN, SIG_GPIO_OUT_IDX, 0, 0);
