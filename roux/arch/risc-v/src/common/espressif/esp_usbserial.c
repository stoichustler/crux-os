...
static void esp_txint(struct uart_dev_s *dev, bool enable)
{
	...
	if (enable) {
		usb_serial_jtag_ll_ena_intr_mask(USB_SERIAL_JTAG_SERIAL_IN_EMPTY_INT_ENA);
	} else {
		usb_serial_jtag_ll_disable_intr_mask(USB_SERIAL_JTAG_SERIAL_IN_EMPTY_INT_ENA);
	}
}
...
{
	if (enable) {
		usb_serial_jtag_ll_ena_intr_mask(USB_SERIAL_JTAG_SERIAL_OUT_RECV_PKT_INT_ENA);
	} else {
		usb_serial_jtag_ll_disable_intr_mask(USB_SERIAL_JTAG_SERIAL_OUT_RECV_PKT_INT_ENA);
	}
}
...
static int esp_attach(struct uart_dev_s *dev)
{
	...
	priv->cpuint = esp_setup_irq(priv->source, ESP_IRQ_PRIORITY_DEFAULT, ESP_IRQ_TRIGGER_LEVEL);
	if (priv->cpuint < 0) {
		return priv->cpuint;
	}
