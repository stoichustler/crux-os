...
#define GET_CHAN_ID_FROM_RET_CHAN(unit_id, chan_id)                                                \
	(chan_id - (SOC_PCNT_CHANNELS_PER_UNIT * unit_id))
...
struct esp_pcnt_priv_s {
	...
	struct esp_pcnt_watch_point_priv_s
		watchers[PCNT_LL_WATCH_EVENT_MAX]; /* array of PCNT watchers */
};
...
static int esp_pcnt_isr_register(int (*fn)(int, void *, void *), int intr_alloc_flags);
static int esp_pcnt_ioctl(struct cap_lowerhalf_s *dev, int cmd, unsigned long arg);
...
static struct esp_pcnt_priv_s pcnt_units[PCNT_UNIT_COUNT] = {0};
	...
	if (unit_id < SOC_PCNT_UNITS_PER_GROUP) {
		...
		while (event_status) {
			...
			if (unit->cb) {
				...
				data.watch_point_value = unit->watchers[event_id].watch_point_value;
				data.zero_cross_mode = pcnt_ll_get_zero_cross_mode(ctx.dev, unit_id);
...
static int esp_pcnt_isr_register(int (*fn)(int, void *, void *), int intr_alloc_flags)
{
	...
	cpuint = esp_setup_irq(pcnt_periph_signals.groups[0].irq, ESP_IRQ_PRIORITY_DEFAULT,
	...
	ret = irq_attach(ESP_SOURCE2IRQ(pcnt_periph_signals.groups[0].irq), fn, 0);
	if (ret < 0) {
...
static int esp_pcnt_unit_enable(struct cap_lowerhalf_s *dev)
{
	...
	if (priv->intr) {
		pcnt_ll_enable_intr(ctx.dev, PCNT_LL_UNIT_WATCH_EVENT(priv->unit_id), true);
	}
...
static int esp_pcnt_unit_disable(struct cap_lowerhalf_s *dev)
{
	...
	if (priv->intr) {
		pcnt_ll_enable_intr(ctx.dev, PCNT_LL_UNIT_WATCH_EVENT(priv->unit_id), false);
	}
										   ...
										   const struct esp_pcnt_glitch_filter_config_s *config)
{
	...
	flags = spin_lock_irqsave(&priv->lock);
	if (config != NULL) {
		pcnt_ll_set_glitch_filter_thres(ctx.dev, priv->unit_id, glitch_filter_thres);
												 ...
												 int (*fn)(int, void *, void *))
{
	...
	priv->intr = intr_flag;
	flags = spin_lock_irqsave(&priv->lock);
	pcnt_ll_enable_intr(ctx.dev, PCNT_LL_UNIT_WATCH_EVENT(priv->unit_id), intr_flag);
	spin_unlock_irqrestore(&priv->lock, flags);
	priv->cb = fn;
...
struct cap_lowerhalf_s *esp_pcnt_new_unit(struct esp_pcnt_unit_config_s *config)
{
	int unit_id;
	int cpuint;
	int cpu = this_cpu();
	...
	pcnt_ll_enable_intr(ctx.dev, PCNT_LL_UNIT_WATCH_EVENT(unit_id), config->accum_count);
	pcnt_ll_clear_intr_status(ctx.dev, PCNT_LL_UNIT_WATCH_EVENT(unit_id));
	spin_unlock_irqrestore(&pcnt_units[unit_id].lock, flags);
	...
	cpinfo("Allocated pcnt unit: %" PRId16 " count range:[%d %d]\n", unit_id,
		   pcnt_units[unit_id].config.low_limit, pcnt_units[unit_id].config.high_limit);
...
int esp_pcnt_del_unit(struct cap_lowerhalf_s *dev)
{
	...
	for (i = 0; i < SOC_PCNT_CHANNELS_PER_UNIT; i++) {
		if (!priv->channels[i]) {
			cperr("Channel %" PRId8 " still in working!\n", i);
			return ERROR;
		}
	}
...
int esp_pcnt_unit_add_watch_point(struct cap_lowerhalf_s *dev, int watch_point)
{
	...
	if (watch_point > unit->config.high_limit || watch_point < unit->config.low_limit) {
	...
	if (watch_point == 0) {
		if (unit->watchers[PCNT_LL_WATCH_EVENT_ZERO_CROSS].event_id !=
			...
			pcnt_ll_enable_zero_cross_event(ctx.dev, unit->unit_id, true);
		}
	}
	...
	/* high limit watch point */
		...
		if (unit->watchers[PCNT_LL_WATCH_EVENT_HIGH_LIMIT].event_id !=
			...
			pcnt_ll_enable_high_limit_event(ctx.dev, unit->unit_id, true);
		}
	}
	...
	/* low limit watch point */
		...
		if (unit->watchers[PCNT_LL_WATCH_EVENT_LOW_LIMIT].event_id != PCNT_LL_WATCH_EVENT_INVALID) {
			...
			pcnt_ll_enable_low_limit_event(ctx.dev, unit->unit_id, true);
		}
	}
	...
	/* other threshold watch point */
		...
		case 1:
			...
			if (wp->event_id == PCNT_LL_WATCH_EVENT_INVALID) {
				...
				pcnt_ll_set_thres_value(ctx.dev, unit->unit_id, 1, watch_point);
				pcnt_ll_enable_thres_event(ctx.dev, unit->unit_id, 1, true);
				break;
		...
		case 0:
			...
			if (wp->event_id == PCNT_LL_WATCH_EVENT_INVALID) {
				...
				wp->watch_point_value = watch_point;
				pcnt_ll_set_thres_value(ctx.dev, unit->unit_id, 0, watch_point);
				pcnt_ll_enable_thres_event(ctx.dev, unit->unit_id, 0, true);
				break;
			} else if (wp->watch_point_value == watch_point) {
				cperr("Watcher has been installed already!\n");
				ret = ERROR;
...
int esp_pcnt_unit_remove_watch_point(struct cap_lowerhalf_s *dev, int watch_point)
{
	struct esp_pcnt_priv_s *unit = (struct esp_pcnt_priv_s *)dev;
	int ret = OK;
	int i;
...
int esp_pcnt_new_channel(struct cap_lowerhalf_s *dev, struct esp_pcnt_chan_config_s *config)
{
	struct esp_pcnt_priv_s *priv = (struct esp_pcnt_priv_s *)dev;
	int channel_id = -1;
	...
	virt_gpio = (config->flags && ESP_PCNT_CHAN_VIRT_LVL_IO_LVL) ? GPIO_MATRIX_CONST_ONE_INPUT
	...
	if (config->edge_gpio_num >= 0) {
		...
		esp_gpio_matrix_in(virt_gpio, chan->groups[0].units[unit_id].channels[channel_id].pulse_sig,
						   (config->flags && ESP_PCNT_CHAN_INVERT_EDGE_IN));
	}
...
void esp_pcnt_channel_set_edge_action(int channel, enum esp_pcnt_chan_edge_action_e pos_act,
									  enum esp_pcnt_chan_edge_action_e neg_act)
{
	...
	flags = spin_lock_irqsave(&pcnt_units[unit_id].lock);
	pcnt_ll_set_edge_action(ctx.dev, unit_id, chan_id, pos_act, neg_act);
	spin_unlock_irqrestore(&pcnt_units[unit_id].lock, flags);
}
...
void esp_pcnt_channel_set_level_action(int channel, enum esp_pcnt_chan_level_action_e pos_act,
									   enum esp_pcnt_chan_level_action_e neg_act)
{
	...
	flags = spin_lock_irqsave(&pcnt_units[unit_id].lock);
	pcnt_ll_set_level_action(ctx.dev, unit_id, chan_id, pos_act, neg_act);
	spin_unlock_irqrestore(&pcnt_units[unit_id].lock, flags);
}
