...
#define WDT_INTR_ENABLE(timer, ctx, en)                                                            \
	(timer == RTC ? rwdt_ll_set_intr_enable(ctx->rwdt_dev, en)                                     \
				  : mwdt_ll_set_intr_enable(ctx->mwdt_dev, en))

#define WDT_WP_DISABLE(dev)                                                                        \
	do {                                                                                           \
		if (IS_XTWDT(dev) != true)                                                                 \
			wdt_hal_write_protect_disable(dev->ctx);                                               \
	} while (0);

#define WDT_WP_ENABLE(dev)                                                                         \
	do {                                                                                           \
		if (IS_XTWDT(dev) != true)                                                                 \
...
static int wdt_getstatus(struct watchdog_lowerhalf_s *lower, struct watchdog_status_s *status);
static int wdt_settimeout(struct watchdog_lowerhalf_s *lower, uint32_t timeout);
static xcpt_t wdt_capture(struct watchdog_lowerhalf_s *lower, xcpt_t handler);
static int wdt_ioctl(struct watchdog_lowerhalf_s *lower, int cmd, unsigned long arg);
/****************************************************************************
 * Private Data
 ****************************************************************************/
...
static const struct watchdog_ops_s g_esp_wdg_ops = {.start = wdt_start,
													.stop = wdt_stop,
...
static struct esp_wdt_lowerhalf_s g_esp_mwdt0_lowerhalf = {.ops = &g_esp_wdg_ops,
...
static struct esp_wdt_lowerhalf_s g_esp_mwdt1_lowerhalf = {.ops = &g_esp_wdg_ops,
														   .timeout = MWDT_MAX_TIMEOUT_MS,
...
static struct esp_wdt_lowerhalf_s g_esp_rwdt_lowerhalf = {.ops = &g_esp_wdg_ops,
...
static esp_xt_wdt_config_t cfg = {.timeout = CONFIG_ESPRESSIF_XTWDT_TIMEOUT,
...
struct esp_wdt_lowerhalf_s g_esp_xtwdt_lowerhalf = {.ops = &g_esp_wdg_ops,
													.periph = RTC_CORE_INTR_SOURCE,
													.peripheral = XTAL32K,
...
static int wdt_start(struct watchdog_lowerhalf_s *lower)
{
	...
	if (priv->handler == NULL) {
		...
		if (IS_XTWDT(priv) != true) {
			...
			wdt_hal_config_stage(priv->ctx, WDT_STAGE0, timeout, priv->action);
		}
	} else {
		...
		if (IS_XTWDT(priv) != true) {
			...
			xt_wdt_ll_intr_enable(((xt_wdt_hal_context_t *)priv->ctx)->dev, true);
		}
#endif
	}
...
static int wdt_settimeout(struct watchdog_lowerhalf_s *lower, uint32_t timeout)
{
	...
	if (IS_XTWDT(priv) != true) {
		...
		xtal32k_ms_max = (XTWDT_FULL_STAGE * NSEC_PER_USEC / (uint32_t)xtal32k_cycles);
		...
		xt_wdt_ll_set_timeout(((xt_wdt_hal_context_t *)priv->ctx)->dev, timeout);
		return OK;
	}
#endif
...
int esp_wdt_initialize(const char *devpath, enum esp_wdt_inst_e wdt_id)
{
	...
	case ESP_WDT_MWDT0: {
		...
		wdt_hal_init(lower->ctx, WDT_MWDT0, MWDT_LL_DEFAULT_CLK_PRESCALER, true);
	...
	case ESP_WDT_MWDT1: {
		...
		wdt_hal_init(lower->ctx, WDT_MWDT1, MWDT_LL_DEFAULT_CLK_PRESCALER, true);
	...
	case ESP_WDT_XTAL32K: {
		...
		esp_rtcioirqenable(lower->irq);
		xt_wdt_hal_init((xt_wdt_hal_context_t *)lower->ctx, (xt_wdt_hal_config_t *)&cfg);
		if (cfg.auto_backup_clk_enable) {
			uint32_t rtc_clk_frequency_khz =
				rtc_clk_freq_cal(rtc_clk_cal(RTC_CAL_INTERNAL_OSC, 500)) / 1000;
			xt_wdt_hal_enable_backup_clk((xt_wdt_hal_context_t *)lower->ctx, rtc_clk_frequency_khz);
		}
		break;
	}
#endif
	...
	lower->upper = watchdog_register(devpath, (struct watchdog_lowerhalf_s *)lower);
	if (lower->upper == NULL) {
	...
	esp_setup_irq(lower->periph, ESP_IRQ_PRIORITY_DEFAULT, ESP_IRQ_TRIGGER_LEVEL);
