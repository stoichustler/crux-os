...
#define WLAN_BUF_SIZE (CONFIG_NET_ETH_PKTSIZE + CONFIG_NET_LL_GUARDSIZE + CONFIG_NET_GUARDSIZE)
...
#ifdef ESP_WLAN_HAS_SOFTAP
static const struct wlan_ops g_softap_ops = {.start = esp_wifi_softap_start,
...
#ifdef ESP_WLAN_HAS_STA
static int esp_wifi_sta_register_recv_cb(int (*recv_cb)(void *buffer, uint16_t len, void *eb));
static void esp_wifi_sta_register_txdone_cb(wifi_tx_done_cb_t cb);
#endif /* ESP_WLAN_HAS_STA */
#ifdef ESP_WLAN_HAS_SOFTAP
static int esp_wifi_softap_register_recv_cb(int (*recv_cb)(void *buffer, uint16_t len, void *eb));
...
static void wlan_transmit(struct wlan_priv_s *priv)
{
	...
	while ((iob = iob_peek_queue(&priv->txb)) != NULL) {
		...
		if (ret == -ENOMEM) {
			wd_start(&priv->txtimeout, WLAN_TXTOUT, wlan_txtimeout_expiry, (uint32_t)priv);
			break;
		} else {
			if (ret < 0) {
...
static int wlan_rx_done(struct wlan_priv_s *priv, void *buffer, uint16_t len, void *eb)
{
...
static void wlan_rxpoll(void *arg)
{
	...
	while ((iob = wlan_recvframe(priv)) != NULL) {
		...
		eth_hdr =
			(struct eth_hdr_s *)&dev->d_iob->io_data[CONFIG_NET_LL_GUARDSIZE - NET_LL_HDRLEN(dev)];
 ...
 *
 * Description:
 *   ROUX Callback: Remove the specified MAC address from the
...
static int esp_net_initialize(int devno, uint8_t *mac_addr, const struct wlan_ops *ops)
{
	int ret;
...
static void wlan_sta_tx_done(uint8_t ifidx, uint8_t *data, uint16_t *len, bool status)
{
	struct wlan_priv_s *priv = &g_wlan_priv[ESP_WLAN_STA_DEVNO];
...
static void wlan_softap_tx_done(uint8_t ifidx, uint8_t *data, uint16_t *len, bool status)
{
	struct wlan_priv_s *priv = &g_wlan_priv[ESP_WLAN_SOFTAP_DEVNO];
...
int esp_wlan_softap_initialize(void)
{
	...
	ret = esp_net_initialize(ESP_WLAN_SOFTAP_DEVNO, mac, &g_softap_ops);
	if (ret < 0) {
		nerr("ERROR: Failed to initialize net\n");
		return ret;
	}
...
void IRAM_ATTR esp_wifi_tx_done_cb(uint8_t ifidx, uint8_t *data, uint16_t *len, bool txstatus)
{
#ifdef ESP_WLAN_HAS_STA
