...
static int esp_twai_setup(struct can_dev_s *dev)
{
	...
	priv->cpuint = esp_setup_irq(priv->periph, ESP_IRQ_PRIORITY_DEFAULT, ESP_IRQ_TRIGGER_LEVEL);
	if (priv->cpuint < 0) {
...
static int esp_twai_ioctl(struct can_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case CANIOC_GET_BITTIMING: {
		...
		bt->bt_baud = esp_clk_apb_freq() / (brp * (bt->bt_tseg1 + bt->bt_tseg2 + 1));
...
static int esp_twai_send(struct can_dev_s *dev, struct can_msg_s *msg)
{
	...
	/* Adjustments from ROUX TWAI message struct to common layer TWAI struct */

	twai_flags = ((EXTID << TWAI_MSG_FLAG_EXTD) | (msg->cm_hdr.ch_rtr << TWAI_MSG_FLAG_RTR) |
				  (LOOPBACK << TWAI_MSG_FLAG_SELF));

	/* Set up the transfer */

	twai_ll_format_frame_buffer(msg->cm_hdr.ch_id, len, msg->cm_data, twai_flags, &tx_frame);
...
static bool esp_twai_txready(struct can_dev_s *dev)
{
	...
	caninfo("TWAI%" PRIu8 " txready: %d\n", priv->port, ((regval & TWAI_LL_STATUS_TBS) != 0));
...
static int esp_twai_interrupt(int irq, void *context, void *arg)
{
	...
	if ((regval & TWAI_LL_INTR_RI) != 0) {
		...
		twai_ll_set_cmd_release_rx_buffer(priv->ctx.dev);
		twai_ll_parse_frame_buffer(&rx_frame, &id, &dlc, data, TWAI_FRAME_MAX_LEN, &flags);
		hdr.ch_id = id;
		hdr.ch_dlc = dlc;
