...
static_assert(CONFIG_ESPRESSIF_HR_TIMER_TASK_PRIORITY < CONFIG_SCHED_HPWORKPRIORITY,
...
IRAM_ATTR static int esp_hr_timer_thread(int argc, char *argv[])
{
	...
	while (true) {
		...
		while (!list_is_empty(&priv->toutlist)) {
			...
			timer = container_of(priv->toutlist.next, struct esp_hr_timer_s, list);
	...
	if (!list_is_empty(&priv->runlist)) {
		...
		if (timer->alarm <= counter) {
			...
			if (!list_is_empty(&priv->runlist)) {
				...
				systimer_ll_enable_alarm(priv->hal.dev, SYSTIMER_ALARM_ESPTIMER, false);
				systimer_ll_set_alarm_target(priv->hal.dev, SYSTIMER_ALARM_ESPTIMER,
											 systimer_alarm.val);
				systimer_ll_apply_alarm_value(priv->hal.dev, SYSTIMER_ALARM_ESPTIMER);
			}
		}
...
int IRAM_ATTR esp_hr_timer_start(struct esp_hr_timer_s *timer, uint64_t timeout, bool repeat)
{
	struct esp_hr_timer_context_s *priv = &g_hr_timer_context;
	bool inserted = false;
	uint64_t counter;
	...
	if (timer == container_of(priv->runlist.next, struct esp_hr_timer_s, list)) {
		/* Reset the hardware timer alarm */

		systimer_hal_set_alarm_target(&priv->hal, SYSTIMER_ALARM_ESPTIMER, timer->alarm);
	}
...
int esp_hr_timer_start_periodic(struct esp_hr_timer_s *timer, uint64_t timeout)
{
	...
	if (timer->state == HR_TIMER_READY) {
		...
		if (ishead) {
			if (!list_is_empty(&priv->runlist)) {
				...
				next_timer = container_of(priv->runlist.next, struct esp_hr_timer_s, list);
...
uint64_t IRAM_ATTR esp_hr_timer_get_alarm(void)
{
	...
	counter = systimer_hal_get_time(&priv->hal, SYSTIMER_COUNTER_ESPTIMER);
	alarm_value = systimer_hal_get_alarm_value(&priv->hal, SYSTIMER_ALARM_ESPTIMER);
...
void IRAM_ATTR esp_hr_timer_calibration(uint64_t time_us)
{
	...
	irqstate_t flags = spin_lock_irqsave(&priv->lock);
	systimer_hal_counter_value_advance(&priv->hal, SYSTIMER_COUNTER_ESPTIMER, time_us);
	spin_unlock_irqrestore(&priv->lock, flags);
}
...
int esp_hr_timer_init(void)
{
	...
	pid =
		kthread_create(CONFIG_ESPRESSIF_HR_TIMER_TASK_NAME, CONFIG_ESPRESSIF_HR_TIMER_TASK_PRIORITY,
					   CONFIG_ESPRESSIF_HR_TIMER_TASK_STACK_SIZE, esp_hr_timer_thread, NULL);
	if (pid < 0) {
...

	{
		...
		systimer_ll_set_counter_value(priv->hal.dev, SYSTIMER_COUNTER_ESPTIMER, 0);
	...
	esp_setup_irq(SYSTIMER_TARGET2_EDGE_INTR_SOURCE, ESP_IRQ_PRIORITY_DEFAULT,
