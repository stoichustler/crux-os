...
static ssize_t esp_nxdiag_read(struct file *filep, char *buffer, size_t buflen);
void esp_nxdiag_read_flash_id(int *mfg_id_out, int *flash_id_out);
void esp_nxdiag_read_security_info(bool *crypt_cnt, bool *secure_boot, bool *flash_crpyt_en,
								   int *chip_id, int *api_version, uint32_t *key_purposes);
...
static ssize_t esp_nxdiag_read(struct file *filep, char *buffer, size_t buflen)
{
	...
	snprintf(tmp, tmp_arr_size, "Flash ID:\n\t Manufacturer: %d\n\t Device: %d\n\n", mfg_id,
			 flash_id);
	strcat(buffer, tmp);
	...
	if (ret != OK) {
		...
		snprintf(tmp, tmp_arr_size, "MAC address: %02x:%02x:%02x:%02x:%02x:%02x\n\n", mac[0],
	...
	esp_nxdiag_read_security_info(&crypt_cnt, &secure_boot, &flash_crpyt_en, &chip_id, &api_version,
								  key_purposes);
	snprintf(tmp, tmp_arr_size, "Security information:\n\
           Key Purposes:(%ld, %ld, %ld, %ld, %ld, %ld, %ld)\n",
			 key_purposes[0], key_purposes[1], key_purposes[2], key_purposes[3], key_purposes[4],
			 key_purposes[5], key_purposes[6]);
	strcat(buffer, tmp);
	...
	snprintf(tmp, tmp_arr_size, "\t SPI Boot Crypt Count (SPI_BOOT_CRYPT_CNT): %x\n\n", crypt_cnt);
	strcat(buffer, tmp);
