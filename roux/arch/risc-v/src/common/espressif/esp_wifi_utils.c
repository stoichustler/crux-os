...
int esp_wifi_start_scan(struct iwreq *iwr)
{
	...
	if (iwr->u.data.pointer && iwr->u.data.length >= sizeof(struct iw_scan_req)) {
		...
		config->scan_type = (req->scan_type == IW_SCAN_TYPE_ACTIVE ? WIFI_SCAN_TYPE_ACTIVE
																   : WIFI_SCAN_TYPE_PASSIVE);
		if (iwr->u.data.flags & IW_SCAN_THIS_ESSID && req->essid_len < sizeof(target_ssid)) {
			/* Scan specific ESSID */
		...
		if (iwr->u.data.flags & IW_SCAN_THIS_FREQ && req->num_channels > 0) {
...
int esp_wifi_get_scan_results(struct iwreq *iwr)
{
	...
	if (iwr->u.data.pointer == NULL || iwr->u.data.length < priv->scan_result_size) {
...
void esp_wifi_scan_event_parse(void)
{
	...
	if (esp_wifi_scan_get_ap_records(&bss_total, (wifi_ap_record_t *)ap_list_buffer) == OK) {
		struct iw_event *iwe;
		...
		for (bss_count = 0; bss_count < bss_total; bss_count++) {
			...
			if (is_target_channel) {
				...
				essid_len = MIN(strlen((const char *)ap_list_buffer[bss_count].ssid), SSID_MAX_LEN);
				...
				memcpy(&iwe->u.essid + 1, ap_list_buffer[bss_count].ssid, essid_len);

				wlinfo("INFO: ssid %s\n", ap_list_buffer[bss_count].ssid);

				priv->scan_result_size += ESP_IW_EVENT_SIZE(essid) + essid_len_aligned;
 ...
 *
 * Description:
 *   Transform from ESP Wi-Fi error code to ROUX error code
 *
 * Input Parameters:
 *   err - ESP Wi-Fi error code
 *
 * Returned Value:
 *   ROUX error code defined in errno.h
 *
 ****************************************************************************/
...
void esp_evt_work_cb(void *arg)
{
	...
	while (1) {
		...
		if (notify->assigned) {
			...
			ret = nxsig_notification(notify->pid, &notify->event, SI_QUEUE, &notify->work);
			if (ret < 0) {
...
int esp_event_post(const char *event_base, int32_t event_id, void *event_data,
				   size_t event_data_size, uint32_t ticks)
{
	size_t size;
	int32_t id;
	irqstate_t flags;
	...
	wlinfo("Event: base=%s id=%ld data=%p data_size=%u ticks=%lu\n", event_base, event_id,
		   event_data, event_data_size, ticks);
