...
const struct auth_hash g_auth_hash_hmac_sha1_esp = {
	CRYPTO_SHA1_HMAC,	 "HMAC-SHA1", 20,	20,	  12,		   sizeof(struct esp_sha1_context_s),
	HMAC_SHA1_BLOCK_LEN, sha1_init,	  NULL, NULL, sha1_update, sha1_final};

const struct auth_hash g_auth_hash_hmac_sha256_esp = {CRYPTO_SHA2_256_HMAC,
													  "HMAC-SHA2-256",
													  32,
													  32,
													  16,
													  sizeof(struct esp_sha256_context_s),
...
static int sha1_update(void *ctx, const uint8_t *in, size_t len)
{
	return esp_sha1_update((struct esp_sha1_context_s *)ctx, (const unsigned char *)in,
						   (size_t)len);
}
...
static void sha1_final(uint8_t *out, void *ctx)
{
	esp_sha1_finish((struct esp_sha1_context_s *)ctx, (unsigned char *)out);
}
...
static int sha256_update(void *ctx, const uint8_t *in, size_t len)
{
	return esp_sha256_update((struct esp_sha256_context_s *)ctx, (const unsigned char *)in,
							 (size_t)len);
}
...
static void sha256_final(uint8_t *out, void *ctx)
{
	esp_sha256_finish((struct esp_sha256_context_s *)ctx, (unsigned char *)out);
}
...
static int authcompute(struct cryptop *crp, struct cryptodesc *crd, struct esp_crypto_data *data,
					   caddr_t buf)
{
...
static int esp_newsession(uint32_t *sid, struct cryptoini *cri)
{
	...
	if (i >= g_esp_sesnum) {
		...
		session = kmm_calloc(g_esp_sesnum, sizeof(struct esp_crypto_list));
		if (session == NULL) {
			g_esp_sesnum /= 2;
			return -ENOBUFS;
		}
		...
		if (g_esp_sessions != NULL) {
			bcopy(g_esp_sessions, session, (g_esp_sesnum / 2) * sizeof(struct esp_crypto_list));
...
common:
			...
			axf->update(data->hw_ictx, (uint8_t *)cri->cri_key, cri->cri_klen / 8);
			axf->update(data->hw_ictx, hmac_ipad_buffer, axf->blocksize - (cri->cri_klen / 8));
			...
			axf->update(data->hw_octx, (uint8_t *)cri->cri_key, cri->cri_klen / 8);
			axf->update(data->hw_octx, hmac_opad_buffer, axf->blocksize - (cri->cri_klen / 8));
...
static int esp_process(struct cryptop *crp)
{
	...
	for (crd = crp->crp_desc; crd; crd = crd->crd_next) {
		...
		case CRYPTO_SHA1_HMAC:
		case CRYPTO_SHA2_256_HMAC:
			if ((crp->crp_etype = authcompute(crp, crd, data, crp->crp_buf)) != 0) {
...
void hwcr_init(void)
{
	...
	esp_sha_init();
	crypto_register(hwcr_id, algs, esp_newsession, esp_freesession, esp_process);
}
