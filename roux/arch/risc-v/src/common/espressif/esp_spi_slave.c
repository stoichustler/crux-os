...
#define SPI_VIA_IOMUX                                                                              \
	(SPI_IS_CS_IOMUX) && (SPI_IS_CLK_IOMUX) && (SPI_IS_MOSI_IOMUX) && (SPI_IS_MISO_IOMUX)
...
static void spislave_evict_sent_data(struct spislave_priv_s *priv, uint32_t sent_bytes);
...
static void spislave_bind(struct spi_slave_ctrlr_s *ctrlr, struct spi_slave_dev_s *dev,
						  enum spi_slave_mode_e mode, int nbits);
static void spislave_unbind(struct spi_slave_ctrlr_s *ctrlr);
static int spislave_enqueue(struct spi_slave_ctrlr_s *ctrlr, const void *data, size_t nwords);
...
static const struct spi_slave_ctrlrops_s esp_spi2slave_ops = {.bind = spislave_bind,
...
static struct spislave_priv_s esp_spi2slave_priv = {.ctrlr = {.ops = &esp_spi2slave_ops},
													.dev = NULL,
...
static void spislave_evict_sent_data(struct spislave_priv_s *priv, uint32_t sent_bytes)
{
	if (sent_bytes < priv->tx_length) {
		...
		memmove(priv->tx_buffer, priv->tx_buffer + sent_bytes, priv->tx_length);
...
static void spislave_setup_tx_dma(struct spislave_priv_s *priv)
{
	esp_dma_setup(priv->dma_channel, true, dma_txdesc, SPI_DMA_DESC_NUM, priv->tx_buffer,
				  SPI_SLAVE_BUFSIZE);
...
static inline void spi_slave_prepare_data(struct spislave_priv_s *priv, ssize_t nbits_to_send)
{
	spi_ll_slave_reset(priv->ctx.hw);
...
static int spislave_enqueue(struct spi_slave_ctrlr_s *ctrlr, const void *data, size_t len)
{
	struct spislave_priv_s *priv = (struct spislave_priv_s *)ctrlr;
...
static size_t spislave_qpoll(struct spi_slave_ctrlr_s *ctrlr)
{
	...
	tmp = SPIS_DEV_RECEIVE(priv->dev, priv->rx_buffer, BYTES2WORDS(priv, priv->rx_length));
...
struct spi_slave_ctrlr_s *esp_spislave_ctrlr_initialize(int port)
{
	...
	DEBUGVERIFY(irq_attach(ESP_PIN2IRQ(priv->config->cs_pin), spislave_cs_interrupt, priv));
