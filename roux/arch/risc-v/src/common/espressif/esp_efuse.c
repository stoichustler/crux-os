...
static int esp_efuse_lowerhalf_read(struct efuse_lowerhalf_s *lower, const efuse_desc_t *field[],
									uint8_t *data, size_t bits_len);
static int esp_efuse_lowerhalf_write(struct efuse_lowerhalf_s *lower, const efuse_desc_t *field[],
									 const uint8_t *data, size_t bits_len);
static int esp_efuse_lowerhalf_ioctl(struct efuse_lowerhalf_s *lower, int cmd, unsigned long arg);
									...
									uint8_t *data, size_t bits_len)
{
	...
	ret = esp_efuse_read_field_blob((const esp_efuse_desc_t **)field, data, bits_len);
									 ...
									 const uint8_t *data, size_t bits_len)
{
	...
	ret = esp_efuse_write_field_blob((const esp_efuse_desc_t **)field, data, bits_len);
...
int esp_efuse_initialize(const char *devpath)
{
	...
	lower->upper = efuse_register(devpath, (struct efuse_lowerhalf_s *)lower);
