...
#define I2C_INT_ERR_MASK                                                                           \
	(I2C_NACK_INT_ENA_M | I2C_TIME_OUT_INT_ENA_M | I2C_ARBITRATION_LOST_INT_ENA_M)
...
static void esp_i2c_init_clock(struct esp_i2c_priv_s *priv, uint32_t bus_freq);
...
static int esp_i2c_transfer(struct i2c_master_s *dev, struct i2c_msg_s *msgs, int count);
...
static void esp_i2c_tracenew(struct esp_i2c_priv_s *priv, uint32_t status);
static void esp_i2c_traceevent(struct esp_i2c_priv_s *priv, enum esp_trace_e event, uint32_t parm,
							   uint32_t status);
...
#endif /* CONFIG_I2C_POLLED */
static inline void esp_i2c_process(struct esp_i2c_priv_s *priv, uint32_t status);
...
static const struct i2c_ops_s esp_i2c_ops = {.transfer = esp_i2c_transfer
#ifdef CONFIG_I2C_RESET
											 ,
											 .reset = esp_i2c_reset
#endif
...
i2c_hal_context_t i2c0_ctx = {0};

/* I2C device structure */

static const struct esp_i2c_config_s esp_i2c0_config = {.clk_freq = I2C_CLK_FREQ_DEF,
...
static struct esp_i2c_priv_s esp_i2c0_priv = {.ops = &esp_i2c_ops,
											  .id = 0,
...
i2c_hal_context_t i2c1_ctx = {0};

/* I2C device structure */

static const struct esp_i2c_config_s esp_i2c1_config = {.clk_freq = I2C_CLK_FREQ_DEF,
...
static struct esp_i2c_priv_s esp_i2c1_priv = {.ops = &esp_i2c_ops,
											  .id = 1,
...
i2c_hal_context_t lp_i2c0_ctx = {0};

/* I2C device structure */

static const struct esp_i2c_config_s esp_lp_i2c0_config = {.clk_freq = I2C_CLK_FREQ_DEF,
														   .scl_pin = 7,
														   .sda_pin = 6,
#ifndef CONFIG_I2C_POLLED
...
static struct esp_i2c_priv_s esp_lp_i2c0_priv = {.ops = &esp_i2c_ops,
												 .id = LP_I2C_NUM_0,
...
#ifdef CONFIG_I2C_TRACE
static const char *g_trace_names[] = {"NONE      ", "SENDADDR  ", "SENDBYTE  ", "RCVMODEEN ",
									  "RCVBYTE   ", "STOP      ", "ERROR     "};
#endif
...
static void esp_i2c_senddata(struct esp_i2c_priv_s *priv)
{
	...
	i2c_ll_hw_cmd_t write_cmd = {.ack_en = 1, .op_code = I2C_LL_CMD_WRITE};

	i2c_ll_hw_cmd_t end_cmd = {.op_code = I2C_LL_CMD_END};
...
static void esp_i2c_sendstop(struct esp_i2c_priv_s *priv)
{
	i2c_ll_hw_cmd_t stop_cmd = {.op_code = I2C_LL_CMD_STOP};
...
static void esp_i2c_init_clock(struct esp_i2c_priv_s *priv, uint32_t bus_freq)
{
	...
	esp_clk_tree_src_get_freq_hz(priv->clk_src, ESP_CLK_TREE_SRC_FREQ_PRECISION_APPROX,
								 &src_clk_frequency);
	i2c_hal_set_bus_timing(priv->ctx, priv->config->clk_freq, priv->clk_src, src_clk_frequency);
	i2c_hal_master_set_scl_timeout_val(priv->ctx, SCL_TIMEOUT_VAL_US, src_clk_frequency);
...
static void esp_lp_i2c_config_io(const struct esp_i2c_priv_s *priv, int8_t pin)
{
...
static int esp_i2c_sem_waitdone(struct esp_i2c_priv_s *priv)
{
	return nxsem_tickwait_uninterruptible(&priv->sem_isr, ESPRESSIF_I2CTIMEOTICKS);
}
#endif
...
static void esp_i2c_clear_bus(struct esp_i2c_priv_s *priv)
{
	i2c_ll_master_clr_bus(priv->ctx->dev, I2C_LL_RESET_SLV_SCL_PULSE_NUM_DEFAULT, true);
...
static inline void esp_i2c_process(struct esp_i2c_priv_s *priv, uint32_t irq_status)
{
	struct i2c_msg_s *msg = &priv->msgv[priv->msgid];
#ifdef CONFIG_I2C_TRACE
	uint32_t status = 0;
	...
	if (I2C_INT_ERR_MASK & irq_status) {
		...
		if (priv->i2cstate == I2CSTATE_PROC) {
			if (msg->flags & I2C_M_READ) {
				if (priv->ready_read) {
					esp_i2c_traceevent(priv, I2CEVENT_RCVBYTE, priv->bytes, status);
				...
				if (priv->bytes == msg->length) {
					esp_i2c_traceevent(priv, I2CEVENT_STOP, msg->length, status);
					esp_i2c_sendstop(priv);
#ifndef CONFIG_I2C_POLLED
					priv->i2cstate = I2CSTATE_FINISH;
#endif
				} else {
					esp_i2c_traceevent(priv, I2CEVENT_RCVMODEEN, 0, status);
...
struct i2c_master_s *esp_i2cbus_initialize(int port)
{
	...
	priv->cpuint = esp_setup_irq(config->periph, ESP_IRQ_PRIORITY_DEFAULT, ESP_IRQ_TRIGGER_LEVEL);
	if (priv->cpuint < 0) {
