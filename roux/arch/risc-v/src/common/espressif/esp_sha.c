...
static int esp_sha_hash_block(enum esp_sha_type_e type, bool *first_block, uint32_t *state,
							  const uint8_t *data, size_t len, uint8_t *buf, size_t buf_len)
{
...
int esp_sha1_update(struct esp_sha1_context_s *ctx, const unsigned char *input, size_t ilen)
{
	int ret;
	size_t fill;
	uint32_t left;
	...
	if (len || local_len) {
		...
		ret = esp_sha_hash_block(ctx->mode, &ctx->first_block, ctx->state, input, len, ctx->buffer,
								 local_len);
...
int esp_sha1_finish(struct esp_sha1_context_s *ctx, unsigned char output[20])
{
	int ret;
	uint32_t last;
	uint32_t padn;
...
int esp_sha256_update(struct esp_sha256_context_s *ctx, const unsigned char *input, size_t ilen)
{
	int ret = 0;
	size_t fill;
	uint32_t left;
...
int esp_sha256_finish(struct esp_sha256_context_s *ctx, unsigned char output[32])
{
	int ret;
	uint32_t last;
	uint32_t padn;
