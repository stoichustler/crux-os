...
struct esp_sdm_channel_priv_s {
	...
	uint32_t sample_rate_hz; /* Over sample rate in Hz, it determines the frequency of the carrier
				pulses */
...
static int esp_sdm_channel_set_pulse_density(struct esp_sdm_channel_priv_s *chan, int8_t density);
...
static void dac_reset(struct dac_dev_s *dev)
{
	struct esp_sdm_group_priv_s *group = (struct esp_sdm_group_priv_s *)dev->ad_priv;
...
static int dac_send(struct dac_dev_s *dev, struct dac_msg_s *msg)
{
	...
	if (group->channels[channel] != NULL) {
		ret = esp_sdm_channel_set_pulse_density(group->channels[channel], (int8_t)msg->am_data);
	} else {
...
struct esp_sdm_channel_priv_s *esp_sdm_create_config_channel(struct esp_sdm_chan_config_s config,
															 int group_id, int chan_id)
{
	int attr;
	uint32_t prescale;
	uint32_t src_clk_hz;
	struct esp_sdm_channel_priv_s *ret = kmm_zalloc(sizeof(struct esp_sdm_channel_priv_s));
	if (ret == NULL) {
	...
	esp_gpio_matrix_out(config.gpio_num, sigma_delta_periph_signals.channels[ret->chan_id].sd_sig,
	...
	sdm_ll_set_prescale(g_esp_sdm.groups[ret->group_id]->hal.dev, ret->chan_id, prescale);
	ret->sample_rate_hz = prescale;
	sdm_ll_set_pulse_density(g_esp_sdm.groups[ret->group_id]->hal.dev, ret->chan_id, 0);

	ainfo("New SDM channel (%d,%d) at %p, gpio=%d,sample rate=%" PRIu32 "Hz\n", ret->group_id,
		  ret->chan_id, ret, config.gpio_num, ret->sample_rate_hz);
	return ret;
}
...
static struct esp_sdm_group_priv_s *esp_sdm_init(struct esp_sdm_chan_config_s config)
{
	...
	for (i = 0; i < SOC_SDM_GROUPS; i++) {
		...
		if (g_esp_sdm.groups[i] == NULL) {
			g_esp_sdm.groups[i] = kmm_zalloc(sizeof(struct esp_sdm_group_priv_s));
			if (g_esp_sdm.groups[i] == NULL) {
	...
	if (g_esp_sdm.group_ref_counts[group_id] >= SOC_SDM_CHANNELS_PER_GROUP) {
...
int esp_sdm_create_channel(struct esp_sdm_chan_config_s config, struct dac_dev_s *dac_dev)
{
	...
	group->channels[chan_num] = esp_sdm_create_config_channel(config, group->group_id, chan_num);
	if (group->channels[chan_num] == NULL) {
		aerr("ERROR! Failed to create channel(%d) on group(%d)\n", chan_num, group->group_id);
		return ERROR;
	}
