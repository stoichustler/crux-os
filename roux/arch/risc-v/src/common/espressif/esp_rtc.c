...
#ifdef CONFIG_RTC_DRIVER
static int esp_rtc_rdtime(struct rtc_lowerhalf_s *lower, struct rtc_time *rtctime);
static int esp_rtc_settime(struct rtc_lowerhalf_s *lower, const struct rtc_time *rtctime);
...
static int esp_rtc_rdalarm(struct rtc_lowerhalf_s *lower, struct lower_rdalarm_s *alarminfo);
#endif /* CONFIG_RTC_ALARM */
#endif /* CONFIG_RTC_DRIVER */
...
static struct esp_rtc_lowerhalf_s g_rtc_lowerhalf = {.ops = &g_rtc_ops};
...
static int esp_rtc_setalarm(struct rtc_lowerhalf_s *lower, const struct lower_setalarm_s *alarminfo)
{
	struct esp_rtc_lowerhalf_s *priv = (struct esp_rtc_lowerhalf_s *)lower;
	irqstate_t flags;
	int ret;
...
static int esp_rtc_rdalarm(struct rtc_lowerhalf_s *lower, struct lower_rdalarm_s *alarminfo)
{
	...
	ts.tv_sec = (esp_hr_timer_time_us() + g_rtc_save->offset + cbinfo->deadline_us) / USEC_PER_SEC;
	ts.tv_nsec =
		((esp_hr_timer_time_us() + g_rtc_save->offset + cbinfo->deadline_us) % USEC_PER_SEC) *
		NSEC_PER_USEC;

	localtime_r((const time_t *)&ts.tv_sec, (struct tm *)alarminfo->time);
...
time_t up_rtc_time(void)
{
	...
	if (g_hr_timer_enabled) {
		...
		time_us = esp_hr_timer_time_us() + g_rtc_save->offset + esp_rtc_get_boot_time();
	} else
#endif
	{
...
int up_rtc_settime(const struct timespec *ts)
{
	...
	now_us = ((uint64_t)ts->tv_sec) * USEC_PER_SEC + ts->tv_nsec / NSEC_PER_USEC;
