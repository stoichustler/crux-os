...
#define CPUINT_ASSIGN(cpuint, in_irq)                                                              \
	do {                                                                                           \
		(cpuint).assigned = 1;                                                                     \
		(cpuint).cpuint_en = 1;                                                                    \
		(cpuint).irq = (in_irq);                                                                   \
...
static int esp_cpuint_alloc(int irq)
{
	...
	if (intset != 0) {
		...
		for (cpuint = 0, bitmask = 0xff; cpuint < ESP_NCPUINTS && (intset & bitmask) == 0;
			 cpuint += 8, bitmask <<= 8)
			;
...
void esp_route_intr(int source, int cpuint, irq_priority_t priority, irq_trigger_t type)
{
