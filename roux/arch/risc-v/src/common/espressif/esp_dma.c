...
static gdma_hal_config_t cfg = {.group_id = 0};
...
int32_t esp_dma_request(enum esp_dma_periph_e periph, uint32_t tx_prio, uint32_t rx_prio,
						bool burst_en)
{
	int chan;
	...
	gdma_hal_connect_peri(&ctx, chan, GDMA_CHANNEL_DIRECTION_TX, periph, periph);
	gdma_hal_connect_peri(&ctx, chan, GDMA_CHANNEL_DIRECTION_RX, periph, periph);
	...
	if (burst_en) {
		...
		gdma_hal_enable_burst(&ctx, chan, GDMA_CHANNEL_DIRECTION_TX, true, true);
		gdma_hal_enable_burst(&ctx, chan, GDMA_CHANNEL_DIRECTION_RX, true, true);
	}
...
uint32_t esp_dma_setup(int chan, bool tx, struct esp_dmadesc_s *dmadesc, uint32_t num,
					   uint8_t *pbuf, uint32_t len)
{
	...
	gdma_channel_direction_t dir =
		tx == true ? GDMA_CHANNEL_DIRECTION_TX : GDMA_CHANNEL_DIRECTION_RX;
