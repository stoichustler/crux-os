...
#include <roux/sensors/qencoder.h>
...
static int esp_ioctl(struct qe_lowerhalf_s *lower, int cmd, unsigned long arg);
...
static int esp_position(struct qe_lowerhalf_s *lower, int32_t *pos)
{
	...
	ret = (priv->pcnt->ops->ioctl(priv->pcnt, CAPIOC_PULSES, (unsigned long)&count) & 0xffff);
...
int esp_qeinitialize(const char *devpath, struct cap_lowerhalf_s *pcnt, int pcnt_num)
{
