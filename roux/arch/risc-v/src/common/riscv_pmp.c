...
static bool pmp_check_region_attrs(uintptr_t base, uintptr_t size, uintptr_t type)
{
	switch (type) {
	case PMPCFG_A_TOR:
...
int riscv_config_pmp_region(uintptr_t region, uintptr_t attr, uintptr_t base, uintptr_t size)
{
	uintptr_t addr = 0;
	uintptr_t cfg = 0;
...
int riscv_check_pmp_access(uintptr_t attr, uintptr_t base, uintptr_t size)
{
	...
	for (region = 0; region < 16 && size > 0; region++) {
		...
		if ((base >= entry.base && end <= entry.end) || (base < entry.base && end > entry.end) ||
			(base >= entry.base && base < entry.end) || (end > entry.base && end <= entry.end)) {
