...
static const char *g_reasons_str[RISCV_MAX_EXCEPTION + 1] = {"Instruction address misaligned",
...
int riscv_exception(int mcause, void *regs, void *args)
{
	...
	_alert("EXCEPTION: %s. MCAUSE: %" PRIxREG ", EPC: %" PRIxREG ", MTVAL: %" PRIxREG "\n",
		   mcause > RISCV_MAX_EXCEPTION ? "Unknown" : g_reasons_str[cause], cause,
		   READ_CSR(CSR_EPC), READ_CSR(CSR_TVAL));
...
int riscv_fillpage(int mcause, void *regs, void *args)
{
	...
	_info("EXCEPTION: %s. MCAUSE: %" PRIxREG ", EPC: %" PRIxREG ", MTVAL: %" PRIxREG "\n",
		  mcause > RISCV_MAX_EXCEPTION ? "Unknown" : g_reasons_str[cause], cause, READ_CSR(CSR_EPC),
		  READ_CSR(CSR_TVAL));
	...
	if (vaddr >= CONFIG_ARCH_TEXT_VBASE && vaddr <= ARCH_TEXT_VEND) {
		 ...
		 * https://github.com/apache/roux/pull/6193.
		 */
