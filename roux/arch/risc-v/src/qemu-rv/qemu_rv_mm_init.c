...
#define PGT_L2_SIZE                                                                                \
	(4096) /* Enough to map continuous 12 MiB,                                                     \
			*  even unaligned */
...
static void map_region(uintptr_t paddr, uintptr_t vaddr, size_t size, uint32_t mmuflags)
{
	...
	while (vaddr < endaddr) {
		...
		pbase = mmu_pte_to_paddr(mmu_ln_getentry(KMM_SPBASE_IDX, KMM_SPBASE, vaddr));
		if (!pbase) {
			...
			mmu_ln_setentry(KMM_SPBASE_IDX, KMM_SPBASE, pbase, vaddr, MMU_UPGT_FLAGS);
		}
