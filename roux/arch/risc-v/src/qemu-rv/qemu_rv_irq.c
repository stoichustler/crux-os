...
void up_irqinitialize(void)
{
	...
	for (id = 0; id < QEMU_RV_APLIC_NR_IRQ; id++) {
		riscv_aplic_configure_irq(QEMU_RV_APLIC_BASE, id + 1, RISCV_APLIC_DEFAULT_PRIORITY, 0);
	}
...
void up_disable_irq(int irq)
{
	...
	if (irq == RISCV_IRQ_SOFT) {
		...
		riscv_imsic_csr_write(ISELECT_EITHRESHOLD, RISCV_IMSIC_DISABLE_EITHRESHOLD);

		/* Enable irq delivery for IMSIC */

		riscv_imsic_csr_write(ISELECT_EIDELIVERY, RISCV_IMSIC_DISABLE_EIDELIVERY);
#endif
	} else if (irq > RISCV_IRQ_EXT) {
...
#ifndef CONFIG_ARCH_RV_HAVE_APLIC
			modifyreg32(QEMU_RV_PLIC_ENABLE1 + (4 * (extirq / 32)), 1 << (extirq % 32), 0);
#else
...
void up_enable_irq(int irq)
{
	...
	if (irq == RISCV_IRQ_SOFT) {
		...
		riscv_imsic_csr_write(ISELECT_EITHRESHOLD, RISCV_IMSIC_ENABLE_EITHRESHOLD);

		/* Enable irq delivery for IMSIC */

		riscv_imsic_csr_write(ISELECT_EIDELIVERY, RISCV_IMSIC_ENABLE_EIDELIVERY);
#endif
	} else if (irq > RISCV_IRQ_EXT) {
		extirq = irq - RISCV_IRQ_EXT;
		...
		if (0 <= extirq && extirq <= 63) {
#ifndef CONFIG_ARCH_RV_HAVE_APLIC
			modifyreg32(QEMU_RV_PLIC_ENABLE1 + (4 * (extirq / 32)), 0, 1 << (extirq % 32));
#else
			riscv_aplic_configure_irq(QEMU_RV_APLIC_BASE, extirq,
									  RISCV_APLIC_SOURCECFG_SM_EDGE_RISE, up_cpu_index());
#ifdef CONFIG_ARCH_RV_HAVE_IMSIC
...
#elif defined(CONFIG_ARCH_RV_HAVE_APLIC)
	modifyreg32(QEMU_RV_APLIC_BASE + RISCV_APLIC_DOMAINCFG, 0x0, RISCV_APLIC_DOMAINCFG_IE);
#endif
