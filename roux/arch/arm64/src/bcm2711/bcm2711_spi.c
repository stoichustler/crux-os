...
static void spi_exchange(FAR struct spi_dev_s *dev, FAR const void *txbuffer, void *rxbuffer,
						 size_t nwords);
static void spi_select(FAR struct spi_dev_s *dev, uint32_t devid, bool selected);
#ifndef CONFIG_SPI_EXCHANGE
static void spi_sndblock(FAR struct spi_dev_s *dev, const void *buffer, size_t nwords);
...
static void spi_interrupt_en(struct bcm2711_spidev_s *priv, bool en)
{
	...
	modreg32(regval, (BCM_SPI_CS_INTR | BCM_SPI_CS_INTD), BCM_SPI_CS(priv->base));
}
...
static void spi_fill_txfifo(struct bcm2711_spidev_s *dev)
{
	...
	while ((getreg32(BCM_SPI_CS(dev->base)) & BCM_SPI_CS_TXD) &&
		   !(getreg32(BCM_SPI_CS(dev->base)) & BCM_SPI_CS_RXR) && dev->nwords) {
		if (dev->txbuffer) {
...
static void spi_setmode(struct spi_dev_s *dev, enum spi_mode_e mode)
{
	...
	modreg32(regval, (BCM_SPI_CS_CPOL | BCM_SPI_CS_CPHA), BCM_SPI_CS(priv->base));
	priv->mode = mode;
}
