...
static int bcm2711_i2c_semtimedwait(struct bcm2711_i2cdev_s *priv, uint32_t ms);
static void bcm2711_i2c_clearfifos(struct bcm2711_i2cdev_s *priv);
static void bcm2711_i2c_setfrequency(struct bcm2711_i2cdev_s *priv, uint32_t frequency);
...
static int bcm2711_i2c_transfer(struct i2c_master_s *dev, struct i2c_msg_s *msgs, int count);
...
static int bcm2711_i2c_interrupted(struct bcm2711_i2cdev_s *priv)
{
	return getreg32(BCM_BSC_S(priv->base)) & (BCM_BSC_S_DONE | BCM_BSC_S_RXR | BCM_BSC_S_TXW);
}
...
static int bcm2711_i2c_secondary_handler(struct bcm2711_i2cdev_s *priv)
{
	...
	if (status & BCM_BSC_S_ERR) {
		modreg32(BCM_BSC_S_ERR, BCM_BSC_S_ERR, status_addr); /* Acknowledge err */
		priv->err = -EIO;
		ret = -EIO;
		post_sem = true;
	}
	...
	if (status & BCM_BSC_S_CLKT) {
		modreg32(BCM_BSC_S_CLKT, BCM_BSC_S_CLKT, status_addr); /* Acknowledge err */
		priv->err = -EIO;
		ret = -EIO;
		post_sem = true;
	}
