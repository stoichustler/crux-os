/****************************************************************************
 * arch/arm64/src/rk356x/rk356x_textheap.c
 *
...
static struct arm_mmu_region g_mmu_load = MMU_REGION_FLAT_ENTRY(
	"nx_load", (uint64_t)_sload, (uint64_t)_szload, MT_NORMAL | MT_RW | MT_EXECUTE | MT_SECURE);
...
void *up_textheap_memalign(size_t align, size_t size)
{
	if (g_textheap == NULL) {
		...
		g_textheap = mm_initialize("textheap", (void *)&_sload, (size_t)_szload);
	}
