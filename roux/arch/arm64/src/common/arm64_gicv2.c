...
#define GIC_ICDICTR_LSPI_SHIFT                                                                     \
	(11) /* Bits 11-15: Number of Lockable Shared Peripheral Interrupts */
...
#define GIC_ICDSGIR_TGTFILTER_LIST                                                                 \
	(0 << GIC_ICDSGIR_TGTFILTER_SHIFT) /* Interrupt sent to CPUs CPU target list */
#define GIC_ICDSGIR_TGTFILTER_OTHER                                                                \
	(1 << GIC_ICDSGIR_TGTFILTER_SHIFT) /* Interrupt is sent to all but requesting CPU */
#define GIC_ICDSGIR_TGTFILTER_THIS                                                                 \
	(2 << GIC_ICDSGIR_TGTFILTER_SHIFT) /* Interrupt is sent to requesting CPU only */
									   /* Bits 26-31: Reserved */
...
static void arm_gic_initialize(void)
{
	...
	iccicr &= ~(GIC_ICCICRS_FIQEN | GIC_ICCICRS_ACKTCTL | GIC_ICCICRS_CBPR | GIC_ICCICRS_EOIMODES |
				GIC_ICCICRS_EOIMODENS | GIC_ICCICRS_ENABLEGRP0 | GIC_ICCICRS_ENABLEGRP1 |
				GIC_ICCICRS_FIQBYPDISGRP0 | GIC_ICCICRS_IRQBYPDISGRP0 | GIC_ICCICRS_FIQBYPDISGRP1 |
				GIC_ICCICRS_IRQBYPDISGRP1);

#else
	/* Clear non-secure state ICCICR bits to be configured below */

	iccicr &= ~(GIC_ICCICRU_EOIMODENS | GIC_ICCICRU_ENABLEGRP1 | GIC_ICCICRU_FIQBYPDISGRP1 |
	...
	iccicr |= (GIC_ICCICRS_ENABLEGRP0 | GIC_ICCICRS_ENABLEGRP1 | GIC_ICCICRS_FIQBYPDISGRP0 |
			   GIC_ICCICRS_IRQBYPDISGRP0 | GIC_ICCICRS_FIQBYPDISGRP1 | GIC_ICCICRS_IRQBYPDISGRP1);
...
void arm64_gic_irq_set_priority(unsigned int intid, unsigned int prio, uint32_t flags)
{
	int ret;
