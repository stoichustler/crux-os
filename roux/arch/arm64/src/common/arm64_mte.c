...
{
	int supported;
	__asm__ volatile("mrs %0, ID_AA64PFR1_EL1\n"
					 "ubfx %0, %0, #8, #3\n"
					 : "=r"(supported)
					 :
					 : "memory");
...
{
	return (FAR void *)((((uint64_t)addr) & ~((uint64_t)0xff << MTE_TAG_SHIFT)) |
...
void arm64_mte_init(void)
{
	...
	val = (read_sysreg(CNTVCT_EL0) & RGSR_EL1_SEED_MASK) << RGSR_EL1_SEED_SHIFT;
