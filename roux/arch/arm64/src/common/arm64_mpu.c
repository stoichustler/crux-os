...
void mpu_modify_region(unsigned int region, const struct arm64_mpu_region *table)
{
	...
	rbar |= table->attr.rbar & (MPU_RBAR_XN_MSK | MPU_RBAR_AP_MSK | MPU_RBAR_SH_MSK);
	rlar |= (table->attr.mair_idx << MPU_RLAR_ATTRINDX_POS) & MPU_RLAR_ATTRINDX_MSK;
...
unsigned int mpu_configure_region(const struct arm64_mpu_region *table)
{
...
void arm64_mpu_init(bool is_primary_core)
{
	...
	__MPU_ASSERT(GET_EL(val) == MODE_EL1, "Exception level not EL1, MPU not enabled!\n");
