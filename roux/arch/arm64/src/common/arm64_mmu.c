...
#define XLAT_TABLE_LEVEL_SPACE(level)                                                              \
	(((level) == 0) ? L0_SPACE : ((level) == 1) ? L1_SPACE : ((level) == 2) ? L2_SPACE : L3_SPACE)
#endif
...
#define XLAT_TABLE_ENTRIES_SHIFT (XLAT_TABLE_SIZE_SHIFT - XLAT_TABLE_ENTRY_SIZE_SHIFT)
...
#define LEVEL_TO_VA_SIZE_SHIFT(level)                                                              \
	(PAGE_SIZE_SHIFT + (XLAT_TABLE_ENTRIES_SHIFT * (XLAT_TABLE_LEVEL_MAX - (level))))
...
static uint64_t base_xlat_table[BASE_XLAT_TABLE_SIZE] aligned_data(BASE_XLAT_TABLE_ALIGN);

static uint64_t xlat_tables[CONFIG_MAX_XLAT_TABLES][XLAT_TABLE_ENTRIES] aligned_data(
	XLAT_TABLE_ENTRIES * sizeof(uint64_t));

/* ROUX RTOS execution regions with appropriate attributes */

static const struct arm_mmu_region g_mmu_nxrt_regions[] = {
	/* Mark text segment cacheable,read only and executable */

	MMU_REGION_FLAT_ENTRY("nx_code", (uint64_t)_stext, (uint64_t)_sztext, MT_CODE | MT_SECURE),

	/* Mark rodata segment cacheable, read only and execute-never */

	MMU_REGION_FLAT_ENTRY("nx_rodata", (uint64_t)_srodata, (uint64_t)_szrodata,
						  MT_RODATA | MT_SECURE),
	...
	MMU_REGION_FLAT_ENTRY("nx_data", (uint64_t)_sdata, (uint64_t)_szdata,
						  MT_NORMAL | MT_RW | MT_SECURE),

#ifdef CONFIG_BUILD_KERNEL
	MMU_REGION_FLAT_ENTRY("nx_pgpool", (uint64_t)CONFIG_ARCH_PGPOOL_PBASE,
						  (uint64_t)CONFIG_ARCH_PGPOOL_SIZE, MT_NORMAL | MT_RW | MT_SECURE),
#endif
};
...
static const size_t g_pgt_sizes[] = {MMU_L0_PAGE_SIZE, MMU_L1_PAGE_SIZE, MMU_L2_PAGE_SIZE,
									 MMU_L3_PAGE_SIZE};
...
static uint64_t get_tcr(int el)
{
	...
	tcr |= TCR_TG0_4K | TCR_SHARED_INNER | TCR_ORGN_WBWA | TCR_IRGN_WBWA | TCR_TBI_FLAGS;
...
static void set_pte_block_desc(uint64_t *pte, uint64_t addr_pa, unsigned int attrs,
							   unsigned int level)
{
	...
	sinfo("%s ", XLAT_TABLE_LEVEL_SPACE(level));
	sinfo("%p: ", pte);
	sinfo("%s ", (mem_type == MT_NORMAL) ? "MEM" : ((mem_type == MT_NORMAL_NC) ? "NC" : "DEV"));
...
static uint64_t *new_prealloc_table(void)
{
	...
	__MMU_ASSERT(table_idx < CONFIG_MAX_XLAT_TABLES, "Enough xlat tables not allocated");
...
static void init_xlat_tables(const struct arm_mmu_region *region)
{
	...
	__MMU_ASSERT(((virt & (PAGE_SIZE - 1)) == 0) && ((size & (PAGE_SIZE - 1)) == 0),
				 "address/size are not page aligned\n");

	while (size) {
		__MMU_ASSERT(level <= XLAT_TABLE_LEVEL_MAX, "max translation table level exceeded\n");
...
static void setup_page_tables(void)
{
	...
	__MMU_ASSERT(max_va <= (1ULL << CONFIG_ARM64_VA_BITS), "Maximum VA not supported\n");
...
int arm64_mmu_set_memregion(const struct arm_mmu_region *region)
{
	...
	if (((virt & (PAGE_SIZE - 1)) == 0) && ((size & (PAGE_SIZE - 1)) == 0)) {
		init_xlat_tables(region);
	} else {
...
int arm64_mmu_init(bool is_primary_core)
{
...
#if CONFIG_ARCH_ARM64_EXCEPTION_LEVEL == 3
	__MMU_ASSERT(GET_EL(el) == MODE_EL3, "Exception level not EL3, MMU not enabled!\n");
...
#else
	__MMU_ASSERT(GET_EL(el) == MODE_EL1, "Exception level not EL1, MMU not enabled!\n");
	...
	/* @HUSTLER
	 *
	 * Currently, roux (NuttX) ain't work properly on crux (xen) hypervisor yet,
	 * now 'arm64_lowputs' can work as a debug method.
	 */
#ifdef CONFIG_MMU_DEBUG
	sinfo("xlat tables:\n");
	...
	arm64_lowputs("MMU enabled\n");

	return 0;
}

void mmu_ln_setentry(uint32_t ptlevel, uintptr_t lnvaddr, uintptr_t paddr, uintptr_t vaddr,
					 uint64_t mmuflags)
{
	uintptr_t *lntable = (uintptr_t *)lnvaddr;
	uint32_t index;

	DEBUGASSERT(ptlevel >= XLAT_TABLE_BASE_LEVEL && ptlevel <= XLAT_TABLE_LEVEL_MAX);
	...
	up_flush_dcache((uintptr_t)&lntable[index], (uintptr_t)&lntable[index] + sizeof(uintptr_t));
...
uintptr_t mmu_ln_getentry(uint32_t ptlevel, uintptr_t lnvaddr, uintptr_t vaddr)
{
	uintptr_t *lntable = (uintptr_t *)lnvaddr;
	uint32_t index;
...
void mmu_ln_restore(uint32_t ptlevel, uintptr_t lnvaddr, uintptr_t vaddr, uintptr_t entry)
{
	uintptr_t *lntable = (uintptr_t *)lnvaddr;
	uint32_t index;
