...
#define LINE_ALIGN_UP(a, line) (((a) + LINE_MASK(line)) & ~LINE_MASK(line))

#define dc_ops(op, val) ({ __asm__ volatile("dc " op ", %x0" : : "r"(val) : "memory"); })

#define ic_ops(op, val) ({ __asm__ volatile("ic " op ", %0" : : "r"(val) : "memory"); })
...
static inline uint32_t arm64_cache_get_info(uint32_t *sets, uint32_t *ways, bool icache)
{
	uint32_t csselr;
	uint32_t ccsidr;

	csselr = read_sysreg(csselr_el1);
	write_sysreg((csselr & ~CSSELR_EL1_IND_MASK) | (icache << CSSELR_EL1_IND_SHIFT), csselr_el1);
...
static inline int arm64_dcache_range(uintptr_t start_addr, uintptr_t end_addr, int op)
{
