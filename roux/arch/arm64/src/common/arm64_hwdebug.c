...
#define ARM64_DBGBWCR_VAL(type, len)                                                               \
	(arm64_convert_type(type) | arm64_convert_size(len) | ARM64_DBGBCR_PAC_ALL | ARM64_DBGBWCR_E)

#define ARM64_MASK_ADDR(addr) ((uint64_t)kasan_clear_tag((void *)addr) & ~0x3)
...
static uint32_t arm64_convert_type(int type)
{
	...
	case DEBUGPOINT_WATCHPOINT_RW:
		return (ARM64_DBGBWCR_LSC_LOAD | ARM64_DBGBWCR_LSC_STORE) << ARM64_DBGBWCR_LSC_OFFSET;
...
static int arm64_breakpoint_add(uintptr_t addr)
{
	...
	for (i = 0; i < num; i++) {
		if (!(ARM64_DBG_GETN(bcr, i) & ARM64_DBGBWCR_E)) {
			...
			ARM64_DBG_SETN(bcr, i, ARM64_DBGBWCR_VAL(DEBUGPOINT_BREAKPOINT, 8));
			return i;
		}
	}
...
static int arm64_watchpoint_match(uint64_t *regs, uint64_t far, uint64_t esr)
{
	...
	for (i = 0; i < num; i++) {
		if (ARM64_MASK_ADDR(dp[i].addr) == ARM64_MASK_ADDR(far)) {
			dp[i].callback(dp[i].type, dp[i].addr, dp[i].size, dp[i].arg);
			return OK;
		}
	}
...
int up_debugpoint_add(int type, void *addr, size_t size, debug_callback_t callback, void *arg)
{
	...
	} else if (type == DEBUGPOINT_WATCHPOINT_RO || type == DEBUGPOINT_WATCHPOINT_WO ||
