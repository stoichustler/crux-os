...
unused_code static inline void sys_set_bit(unsigned long addr, unsigned int bit)
{
	uint32_t temp;
...
unused_code static inline int sys_test_bit(unsigned long addr, unsigned int bit)
{
	uint32_t temp;
...
static int arm64_gic_send_sgi(unsigned int sgi_id, uint64_t target_aff, uint16_t target_list)
{
	uint32_t aff3;
	uint32_t aff2;
	uint32_t aff1;
...
void arm64_gic_raise_sgi(unsigned int sgi_id, uint16_t target_list)
{
	...
	while ((i = ffs(target_list))) {
		...
		if (pre_cluster_id != UINT64_MAX && pre_cluster_id != curr_cluster_id) {
...
static void gicv3_cpuif_init(void)
{
	...
	if (!(icc_sre & ICC_SRE_ELX_SRE_BIT)) {
		icc_sre = (icc_sre | ICC_SRE_ELX_SRE_BIT | ICC_SRE_ELX_DIB_BIT | ICC_SRE_ELX_DFB_BIT);
...
static void gicv3_dist_init(void)
{
	...
	for (intid = GIC_SPI_INT_BASE; intid < num_ints; intid += GIC_NUM_INTR_PER_REG) {
		...
		putreg32(BIT64_MASK(GIC_NUM_INTR_PER_REG), IGROUPMODR(base, idx));
	}
...
static int gic_validate_redist_version(void)
{
	...
	ppi_nr = (~0U);
	reg = getreg32(redist_base + GICR_PIDR2) & GICR_PIDR2_ARCH_MASK;
...
static void arm64_gic_init(void)
{
	...
	cpu = this_cpu();
	g_gic_rdists[cpu] = CONFIG_GICR_BASE + up_cpu_index() * CONFIG_GICR_OFFSET;
