...
static int arm64_fpu_procfs_open(struct file *filep, const char *relpath, int oflags, mode_t mode);
static int arm64_fpu_procfs_close(struct file *filep);
static ssize_t arm64_fpu_procfs_read(struct file *filep, char *buffer, size_t buflen);
...
static const struct procfs_entry_s g_procfs_arm64_fpu = {"fpu", &arm64_fpu_procfs_operations};
...
static ssize_t arm64_fpu_procfs_read(struct file *filep, char *buffer, size_t buflen)
{
	...
	for (i = 0; i < CONFIG_SMP_NCPUS; i++) {
		ctx = &g_cpu_fpu_ctx[i];
		linesize += snprintf(attr->line + linesize, FPU_PROC_LINELEN,
							 "CPU%d: save: %d restore: %d "
							 "switch: %d exedepth: %d\n",
							 i, ctx->save_count, ctx->restore_count, ctx->switch_count,
							 ctx->exe_depth_count);
	}
...
{
	const uint64_t *regs1 = (uint64_t *)((uintptr_t)saveregs1 + ARM64_CONTEXT_SIZE);
	const uint64_t *regs2 = (uint64_t *)((uintptr_t)saveregs2 + ARM64_CONTEXT_SIZE);

	/* Only compare callee-saved registers, caller-saved registers do not
	 * need to be preserved.
	 */

	return memcmp(&regs1[REG_Q4], &regs2[REG_Q4], 8 * FPU_CALLEE_REGS) == 0;
}
