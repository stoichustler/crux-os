...
static int modify_region(uintptr_t vstart, uintptr_t vend, uintptr_t setmask)
{
	...
	for (vaddr = vstart; vaddr < vend; vaddr += MM_PGSIZE) {
		for (ptlevel = mmu_get_base_pgt_level(), lnvaddr = l1vaddr; ptlevel < MMU_PGT_LEVEL_MAX;
			 ptlevel++) {
...
int up_addrenv_mprot(arch_addrenv_t *addrenv, uintptr_t addr, size_t len, int prot)
{
	uintptr_t setmask;
