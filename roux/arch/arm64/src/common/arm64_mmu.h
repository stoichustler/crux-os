...
#define MMU_REGION_ENTRY(_name, _base_pa, _base_va, _size, _attrs)                                 \
	{                                                                                              \
		.name = (_name), .base_pa = (_base_pa), .base_va = (_base_va), .size = (_size),            \
		.attrs = (_attrs),                                                                         \
	}
...
#define MMU_MT_NORMAL_FLAGS                                                                        \
	(PTE_PAGE_DESC | PTE_BLOCK_DESC_AF | PTE_BLOCK_DESC_INNER_SHARE |                              \
	 PTE_BLOCK_DESC_MEMTYPE(MT_NORMAL))

/* Flags for user FLASH (RX) and user RAM (RW) */

#define MMU_UTEXT_FLAGS                                                                            \
	(PTE_BLOCK_DESC_AP_RO | PTE_BLOCK_DESC_PXN | PTE_BLOCK_DESC_AP_USER | PTE_BLOCK_DESC_NG |      \
	 MMU_MT_NORMAL_FLAGS)
#define MMU_UDATA_FLAGS                                                                            \
	(PTE_BLOCK_DESC_AP_RW | PTE_BLOCK_DESC_PXN | PTE_BLOCK_DESC_AP_USER | PTE_BLOCK_DESC_NG |      \
	 PTE_BLOCK_DESC_UXN | MMU_MT_NORMAL_FLAGS)
...
{
	__asm__ __volatile__("dsb nshst\n"
						 "tlbi vmalle1\n"
						 "dsb nsh\n"
						 "isb"
						 :
...
#define mmu_ln_clear(ptlevel, lnvaddr, vaddr) mmu_ln_restore(ptlevel, lnvaddr, vaddr, 0)
