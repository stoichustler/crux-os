...
#include <roux/timers/arch_alarm.h>
...
static inline uint64_t arm64_arch_cnt2tick(uint64_t count, uint64_t freq)
{
	...
	if (count > multiply_safe_count) {
		...
		result_ticks += count / multiply_safe_count * (multiply_safe_count * TICK_PER_SEC / freq);
...
static inline uint64_t arm64_arch_tick2cnt(uint64_t ticks, uint64_t freq)
{
	...
	if (ticks > multiply_safe_ticks) {
		...
		result_count += ticks / multiply_safe_ticks * (multiply_safe_ticks * freq / TICK_PER_SEC);
...
{
	struct arm64_oneshot_lowerhalf_s *priv = (struct arm64_oneshot_lowerhalf_s *)arg;
...
static int arm64_tick_max_delay(struct oneshot_lowerhalf_s *lower, clock_t *ticks)
{
...
static int arm64_tick_cancel(struct oneshot_lowerhalf_s *lower, clock_t *ticks)
{
	struct arm64_oneshot_lowerhalf_s *priv = (struct arm64_oneshot_lowerhalf_s *)lower;
...
static int arm64_tick_start(struct oneshot_lowerhalf_s *lower, oneshot_callback_t callback,
...
static int arm64_tick_current(struct oneshot_lowerhalf_s *lower, clock_t *ticks)
{
	uint64_t count;
	struct arm64_oneshot_lowerhalf_s *priv = (struct arm64_oneshot_lowerhalf_s *)lower;
...
struct oneshot_lowerhalf_s *arm64_oneshot_initialize(void)
{
	...
	priv = (struct arm64_oneshot_lowerhalf_s *)kmm_zalloc(sizeof(struct arm64_oneshot_lowerhalf_s));
	...
	irq_attach(ARM_ARCH_TIMER_IRQ, arm64_arch_timer_compare_isr, priv);
