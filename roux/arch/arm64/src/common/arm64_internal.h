...
#define INIT_STACK_DEFINE(sym, size)                                                               \
	char locate_data(".initstack") aligned_data(STACK_ALIGNMENT) sym[size]

#define INIT_STACK_ARRAY_DEFINE(sym, n, size)                                                      \
	char locate_data(".initstack") aligned_data(STACK_ALIGNMENT) sym[n][size]

#define INIT_STACK_DEFINE_EXTERN(sym, size) EXTERN char sym[size]

#define INIT_STACK_ARRAY_DEFINE_EXTERN(sym, n, size) EXTERN char sym[n][size]

#define STACK_PTR_TO_FRAME(type, ptr) (type *)STACK_ALIGN_DOWN((uintptr_t)(ptr) - sizeof(type))

#define INTSTACK_SIZE (CONFIG_ARCH_INTERRUPTSTACK & ~STACK_ALIGN_MASK)

#ifdef CONFIG_SMP

INIT_STACK_ARRAY_DEFINE_EXTERN(g_cpu_idlestackalloc, CONFIG_SMP_NCPUS, SMP_STACK_SIZE);
INIT_STACK_ARRAY_DEFINE_EXTERN(g_interrupt_stacks, CONFIG_SMP_NCPUS, INTSTACK_SIZE);

#ifdef CONFIG_ARM64_DECODEFIQ
INIT_STACK_ARRAY_DEFINE_EXTERN(g_interrupt_fiq_stacks, CONFIG_SMP_NCPUS, INTSTACK_SIZE);
#endif
#else
 ...
 *
 * Memory layout for ROUX at arm64 for FLAT Build
 *
 *    +-------------------------+ <- RAM BASE
 *    |                         |
 *    |<<<<<<<<<<<<<<<<<<<<<<<<<| <- LOAD_BASE
...
void arm64_jump_to_user(uint64_t entry, uint64_t x0, uint64_t x1, uint64_t sp_el0,
