...
nosanitize_address static int backtrace(uintptr_t *base, uintptr_t *limit, uintptr_t *fp,
										uintptr_t *pc, void **buffer, int size, int *skip)
{
	int i = 0;
...
int up_backtrace(struct tcb_s *tcb, void **buffer, int size, int skip)
{
	...
	if (tcb == NULL || tcb == rtcb) {
		if (up_interrupt_context()) {
			...
			ret = backtrace(istackbase, istackbase + INTSTACK_SIZE,
							(void *)__builtin_frame_address(0), NULL, buffer, size, &skip);
#else
			ret = backtrace(rtcb->stack_base_ptr, rtcb->stack_base_ptr + rtcb->adj_stack_size,
							(void *)__builtin_frame_address(0), NULL, buffer, size, &skip);
#endif /* CONFIG_ARCH_INTERRUPTSTACK > 7 */
			if (ret < size) {
				ret += backtrace(rtcb->stack_base_ptr, rtcb->stack_base_ptr + rtcb->adj_stack_size,
								 running_regs()[REG_X29], running_regs()[REG_ELR], &buffer[ret],
								 size - ret, &skip);
			}
		} else {
			ret = backtrace(rtcb->stack_base_ptr, rtcb->stack_base_ptr + rtcb->adj_stack_size,
		...
		ret = backtrace(tcb->stack_base_ptr, tcb->stack_base_ptr + tcb->adj_stack_size,
						running_regs()[REG_X29], running_regs()[REG_ELR], buffer, size, &skip);
	}
