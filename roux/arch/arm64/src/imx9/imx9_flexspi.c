...
static void imx9_flexspi_update_lut(struct flexspi_dev_s *dev, uint32_t index, const uint32_t *cmd,
									uint32_t count);
...
enum flexspi_flags_e {
	...
	FLEXSPI_SEQUENCE_EXECUTION_TIMEOUT_FLAG =
		FLEXSPI_INTEN_SEQTIMEOUTEN_MASK, /* Sequence execution timeout */
	...
	FLEXSPI_IP_TX_FIFO_WATERMARK_EMPTY_FLAG =
		FLEXSPI_INTEN_IPTXWEEN_MASK, /* IP TX FIFO WaterMark empty */

	FLEXSPI_IP_RX_FIFO_WATERMARK_AVAILABLE_FLAG =
		FLEXSPI_INTEN_IPRXWAEN_MASK, /* IP RX FIFO WaterMark available */
	...
	FLEXSPI_IP_COMMAND_SEQUENCE_ERROR_FLAG =
		FLEXSPI_INTEN_IPCMDERREN_MASK, /* IP triggered Command Sequences Error */
...
enum flexspi_flag_constants_e {
	...
	ERROR_FLAGS = FLEXSPI_SEQUENCE_EXECUTION_TIMEOUT_FLAG | FLEXSPI_IP_COMMAND_SEQUENCE_ERROR_FLAG |
				  FLEXSPI_IP_COMMAND_GRANT_TIMEOUT_FLAG,
};
...
struct flexspi_ahb_buffer_config_s {
	...
	bool enable_prefetch; /* AHB Read Prefetch Enable for current AHB RX Buffer corresponding
						   * Master, allows prefetch disable/enable separately for each master */
};
...
struct flexspi_config_s {
	enum flexspi_read_sample_clock_e
		rx_sample_clock; /* Sample Clock source selection for Flash Reading */
	...
	bool enable_same_config_for_all; /* Enable/disable same configuration for all connected devices
									  * when enabled, same configuration in FLASHA1CRx is applied to
									  * all */

	uint16_t
		seq_timeout_cycle; /* Timeout wait cycle for command sequence execution,
	...
	struct {
		...
		uint8_t ahb_grant_timeout_cycle; /* Timeout wait cycle for AHB command grant,
										  * timeout after ahb_grant_timeout_cyle*1024 AHB clock
										  * cycles */

		uint16_t
			ahb_bus_timeout_cycle; /* Timeout wait cycle for AHB read/write access,
									* timeout after ahb_bus_timeout_cycle*1024 AHB clock cycles */

		uint8_t
			resume_wait_cycle; /* Wait cycle for idle state before suspended command sequence
		...
		bool enable_clear_ahb_buffer_opt; /* Enable/disable automatically clean AHB RX Buffer and TX
										   * Buffer when FLEXSPI returns STOP mode ACK */

		bool enable_read_address_opt; /* Enable/disable remove AHB read burst start address
									   * alignment limitation. when enable, there is no AHB read
		...
		bool enable_ahb_bufferable; /* Enable/disable AHB bufferable write access support, when
									 * enabled, FLEXSPI return before waiting for command execution
									 * finished */
...
static int imx9_flexspi_check_and_clear_error(struct flexspi_type_s *base, uint32_t status);
...
static inline void imx9_flexspi_software_reset_private(struct flexspi_type_s *base)
{
...
static inline bool imx9_flexspi_get_bus_idle_status(struct flexspi_type_s *base)
{
										   ...
										   struct flexspi_device_config_s *config)
{
	...
	uint32_t rx_sample_clock =
		(base->MCR0 & FLEXSPI_MCR0_RXCLKSRC_MASK) >> FLEXSPI_MCR0_RXCLKSRC_SHIFT;
	...
	if (is_unified_config) {
		...
		if (config->flexspi_root_clk >= 100u * FREQ_1MHz) {
			...
			temp =
				(uint32_t)config->data_valid_time * 1000u; /* Convert data valid time in ns to ps */
...
static int imx9_flexspi_check_and_clear_error(struct flexspi_type_s *base, uint32_t status)
{
	...
	if (0u != status) {
		...
		} else if (0u != (status & (uint32_t)FLEXSPI_IP_COMMAND_SEQUENCE_ERROR_FLAG)) {
			result = STATUS_FLEXSPI_IP_COMMAND_SEQUENCE_ERROR;
		} else if (0u != (status & (uint32_t)FLEXSPI_IP_COMMAND_GRANT_TIMEOUT_FLAG)) {
...
void imx9_flexspi_init(struct flexspi_type_s *base, const struct flexspi_config_s *config)
{
	...
	config_value = FLEXSPI_MCR0_RXCLKSRC(config->rx_sample_clock) |
				   ...
				   FLEXSPI_MCR0_AHBGRANTWAIT(config->ahb_config.ahb_grant_timeout_cycle) |
				   ...
				   FLEXSPI_MCR0_ATDFEN(config->ahb_config.enable_ahb_write_ip_tx_fifo) |
				   FLEXSPI_MCR0_ARDFEN(config->ahb_config.enable_ahb_write_ip_rx_fifo) |
				   FLEXSPI_MCR0_MDIS_MASK;
	base->MCR0 = config_value;

	/* Configure MCR1 configurations */

	config_value = FLEXSPI_MCR1_SEQWAIT(config->seq_timeout_cycle) |
	...
	config_value &= ~(FLEXSPI_MCR2_RESUMEWAIT_MASK | FLEXSPI_MCR2_SCKBDIFFOPT_MASK |
					  FLEXSPI_MCR2_SAMEDEVICEEN_MASK | FLEXSPI_MCR2_CLRAHBBUFOPT_MASK);

	config_value |= FLEXSPI_MCR2_RESUMEWAIT(config->ahb_config.resume_wait_cycle) |
					FLEXSPI_MCR2_SCKBDIFFOPT(config->enable_sckb_diff_opt) |
					FLEXSPI_MCR2_SAMEDEVICEEN(config->enable_same_config_for_all) |
					FLEXSPI_MCR2_CLRAHBBUFOPT(config->ahb_config.enable_clear_ahb_buffer_opt);
	...
	config_value &= ~(FLEXSPI_AHBCR_READADDROPT_MASK | FLEXSPI_AHBCR_PREFETCHEN_MASK |
	...
	config_value |= FLEXSPI_AHBCR_READADDROPT(config->ahb_config.enable_read_address_opt) |
					FLEXSPI_AHBCR_PREFETCHEN(config->ahb_config.enable_ahb_prefetch) |
					FLEXSPI_AHBCR_BUFFERABLEEN(config->ahb_config.enable_ahb_bufferable) |
					FLEXSPI_AHBCR_CACHABLEEN(config->ahb_config.enable_ahb_cachable);
	...
	for (i = 0; i < (uint32_t)FLEXSPI_AHB_BUFFER_COUNT; i++) {
		...
		config_value &= ~(FLEXSPI_AHBRXBUFCR0_PREFETCHEN_MASK | FLEXSPI_AHBRXBUFCR0_PRIORITY_MASK |
						  FLEXSPI_AHBRXBUFCR0_MSTRID_MASK | FLEXSPI_AHBRXBUFCR0_BUFSZ_MASK);

		config_value |=
			FLEXSPI_AHBRXBUFCR0_PREFETCHEN(config->ahb_config.buffer[i].enable_prefetch) |
			FLEXSPI_AHBRXBUFCR0_PRIORITY(config->ahb_config.buffer[i].priority) |
			FLEXSPI_AHBRXBUFCR0_MSTRID(config->ahb_config.buffer[i].master_index) |
			FLEXSPI_AHBRXBUFCR0_BUFSZ((uint32_t)config->ahb_config.buffer[i].buffer_size / 8u);
	...
	base->IPRXFCR |= FLEXSPI_IPRXFCR_RXWMRK((uint32_t)config->rx_watermark / 8u - 1u);
	base->IPTXFCR &= ~FLEXSPI_IPTXFCR_TXWMRK_MASK;
	base->IPTXFCR |= FLEXSPI_IPTXFCR_TXWMRK((uint32_t)config->tx_watermark / 8u - 1u);
...
void imx9_flexspi_get_default_config(struct flexspi_config_s *config)
{
	...
	memset(config->ahb_config.buffer, 0, sizeof(config->ahb_config.buffer));
											...
											enum flexspi_port_e port)
{
	...
	base->FLSHCR1[port] =
		...
		FLEXSPI_FLSHCR1_TCSH(config->cs_hold_time) | FLEXSPI_FLSHCR1_TCSS(config->cs_setup_time) |
		FLEXSPI_FLSHCR1_CAS(config->columnspace) | FLEXSPI_FLSHCR1_WA(config->enable_word_address);

	/* Configure AHB operation items */

	config_value = base->FLSHCR2[port];

	config_value &= ~(FLEXSPI_FLSHCR2_AWRWAITUNIT_MASK | FLEXSPI_FLSHCR2_AWRWAIT_MASK |
					  FLEXSPI_FLSHCR2_AWRSEQNUM_MASK | FLEXSPI_FLSHCR2_AWRSEQID_MASK |
					  FLEXSPI_FLSHCR2_ARDSEQNUM_MASK | FLEXSPI_FLSHCR2_ARDSEQID_MASK);

	config_value |= FLEXSPI_FLSHCR2_AWRWAITUNIT(config->ahb_write_wait_unit) |
					FLEXSPI_FLSHCR2_AWRWAIT(config->ahb_write_wait_interval);

	if (config->awr_seq_number > 0u) {
		config_value |= FLEXSPI_FLSHCR2_AWRSEQID((uint32_t)config->awr_seq_index) |
						FLEXSPI_FLSHCR2_AWRSEQNUM((uint32_t)config->awr_seq_number - 1u);
	}

	if (config->ard_seq_number > 0u) {
		config_value |= FLEXSPI_FLSHCR2_ARDSEQID((uint32_t)config->ard_seq_index) |
						FLEXSPI_FLSHCR2_ARDSEQNUM((uint32_t)config->ard_seq_number - 1u);
	}
	...
	status_value = (index == 0u) ? ((uint32_t)FLEXSPI_FLASH_A_SAMPLE_CLOCK_SLAVE_DELAY_LOCKED |
...
void imx9_flexspi_update_lut_private(struct flexspi_type_s *base, uint32_t index,
...
static int imx9_flexspi_write_blocking(struct flexspi_type_s *base, uint32_t *buffer, size_t size)
{
	uint32_t tx_watermark =
		((base->IPTXFCR & FLEXSPI_IPTXFCR_TXWMRK_MASK) >> FLEXSPI_IPTXFCR_TXWMRK_SHIFT) + 1u;
	uint32_t status;
	int result = 0;
	uint32_t i = 0;
	...
	while (0u != size) {
		...
		while (0u == ((status = base->INTR) & (uint32_t)FLEXSPI_IP_TX_FIFO_WATERMARK_EMPTY_FLAG)) {
		}
...
static int imx9_flexspi_read_blocking(struct flexspi_type_s *base, uint32_t *buffer, size_t size)
{
	uint32_t rx_watermark =
		((base->IPRXFCR & FLEXSPI_IPRXFCR_RXWMRK_MASK) >> FLEXSPI_IPRXFCR_RXWMRK_SHIFT) + 1u;
	uint32_t status;
	int result = 0;
	uint32_t i = 0;
										   ...
										   struct flexspi_transfer_s *xfer)
{
	...
	base->INTR |= FLEXSPI_INTR_AHBCMDERR_MASK | FLEXSPI_INTR_IPCMDERR_MASK |
	...
	if ((xfer->cmd_type == FLEXSPI_READ) || (xfer->cmd_type == FLEXSPI_WRITE) ||
	...
	config_value |= FLEXSPI_IPCR1_ISEQID((uint32_t)xfer->seq_index) |
	...
	if ((xfer->cmd_type == FLEXSPI_WRITE) || (xfer->cmd_type == FLEXSPI_CONFIG)) {
		result = imx9_flexspi_write_blocking(base, xfer->data, xfer->data_size);
	} else if (xfer->cmd_type == FLEXSPI_READ) {
		result = imx9_flexspi_read_blocking(base, xfer->data, xfer->data_size);
	}
