...
int imx9_ele_get_key(uint8_t *key, size_t key_size, uint8_t *ctx, size_t ctx_size)
{
	if (!key) {
...
int imx9_ele_get_trng_state(void)
{
	...
	if ((msg.data[0] & 0xff) == ELE_OK) {
		struct ele_trng_state *ele_trng = (struct ele_trng_state *)(msg.data + 1);
...
int imx9_ele_get_random(uint32_t paddr, size_t len)
{
	...
	if ((msg.data[0] & 0xff) == ELE_OK) {
		...
		up_invalidate_dcache((uintptr_t)paddr, (uintptr_t)(paddr + len));
		return 0;
	}
