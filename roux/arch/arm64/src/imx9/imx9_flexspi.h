...
enum flexspi_read_sample_clock_e {
	FLEXSPI_READ_SAMPLE_CLK_LOOPBACK_INTERNALLY = 0x0u, /* Dummy Read strobe generated by FlexSPI
														 * Controller and loopback internally */

	FLEXSPI_READ_SAMPLE_CLK_LOOPBACK_FROM_DQS_PAD = 0x1u, /* Dummy Read strobe generated by FlexSPI
														   * Controller and loopback from DQS pad */

	FLEXSPI_READ_SAMPLE_CLK_LOOPBACK_FROM_SCK_PAD =
		0x2u, /* SCK output clock and loopback from SCK pad */

	FLEXSPI_READ_SAMPLE_CLK_EXTERNAL_INPUT_FROM_DQS_PAD =
		0x3u, /* Flash provided Read strobe and input from DQS pad */
};
...
struct flexspi_ops_s {
	...
	int (*transfer_blocking)(struct flexspi_dev_s *dev, struct flexspi_transfer_s *xfer);
	void (*software_reset)(struct flexspi_dev_s *dev);
	void (*update_lut)(struct flexspi_dev_s *dev, uint32_t index, const uint32_t *cmd,
					   uint32_t count);
	void (*set_device_config)(struct flexspi_dev_s *dev, struct flexspi_device_config_s *config,
