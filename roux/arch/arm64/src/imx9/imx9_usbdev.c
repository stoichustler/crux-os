...
#define IS_CACHE_ALIGNED(x, y) (((uintptr_t)(x)&DCACHE_LINEMASK) == 0 && ((y)&DCACHE_LINEMASK) == 0)
...
#define DTD_NEXTDESC_INVALID                                                                       \
	(1 << 0) /* Bit 0     : Next Descriptor Invalid. The "Terminate" bit. */
...
#define DTD_CONFIG_MULT_VARIABLE                                                                   \
	(0 << 10) /* Bits 10-11 : Number of packets executed per transacation descriptor (override) */
...
#define DQH_CAPABILITY_MULT_VARIABLE                                                               \
	(0 << 30) /* Bits 30-31 : Number of packets executed per transaction descriptor */
#define DQH_CAPABILITY_MULT_NUM(n) ((n) << 30)
#define DQH_CAPABILITY_ZLT (1 << 29) /* Bit 29     : Zero Length Termination Select */
#define DQH_CAPABILITY_MAX_PACKET(n)                                                               \
	((n) << 16) /* Bits 16-29 : Maximum packet size of associated endpoint (<1024) */
...
static inline void imx9_modifyreg(struct imx9_usb_s *priv, off_t offset, uint32_t clear,
								  uint32_t set);

/* Request queue operations *************************************************/

static struct imx9_req_s *imx9_rqdequeue(struct imx9_ep_s *privep);
static bool imx9_rqenqueue(struct imx9_ep_s *privep, struct imx9_req_s *req);

/* Low level data transfers and request operations **************************/

static inline void imx9_writedtd(struct imx9_dtd_s *dtd, const uint8_t *data, uint32_t nbytes);
static inline void imx9_queuedtd(struct imx9_usb_s *priv, uint8_t epphy, struct imx9_dtd_s *dtd);
static inline void imx9_ep0xfer(struct imx9_usb_s *priv, uint8_t epphy, uint8_t *data,
								uint32_t nbytes);
static void imx9_readsetup(struct imx9_usb_s *priv, uint8_t epphy, struct usb_ctrlreq_s *ctrl);
...
static void imx9_reqcomplete(struct imx9_ep_s *privep, struct imx9_req_s *privreq, int16_t result);

static void imx9_cancelrequests(struct imx9_ep_s *privep, int16_t status);

/* Interrupt handling *******************************************************/

static struct imx9_ep_s *imx9_epfindbyaddr(struct imx9_usb_s *priv, uint16_t eplog);
static void imx9_dispatchrequest(struct imx9_usb_s *priv, const struct usb_ctrlreq_s *ctrl);
static void imx9_ep0configure(struct imx9_usb_s *priv);
static void imx9_usbreset(struct imx9_usb_s *priv);
static inline void imx9_ep0state(struct imx9_usb_s *priv, uint16_t state);
...
static bool imx9_epcomplete(struct imx9_usb_s *priv, uint8_t epphy);
...
static int imx9_epconfigure(struct usbdev_ep_s *ep, const struct usb_epdesc_s *desc, bool last);
static int imx9_epdisable(struct usbdev_ep_s *ep);
static struct usbdev_req_s *imx9_epallocreq(struct usbdev_ep_s *ep);
static void imx9_epfreereq(struct usbdev_ep_s *ep, struct usbdev_req_s *);
...
#endif
static int imx9_epsubmit(struct usbdev_ep_s *ep, struct usbdev_req_s *req);
static int imx9_epcancel(struct usbdev_ep_s *ep, struct usbdev_req_s *req);
static int imx9_epstall(struct usbdev_ep_s *ep, bool resume);

static struct usbdev_ep_s *imx9_allocep(struct usbdev_s *dev, uint8_t epno, bool in,
										uint8_t eptype);
static void imx9_freeep(struct usbdev_s *dev, struct usbdev_ep_s *ep);
...
static inline void imx9_writedtd(struct imx9_dtd_s *dtd, const uint8_t *data, uint32_t nbytes)
{
	...
	up_clean_dcache((uintptr_t)dtd, (uintptr_t)dtd + sizeof(struct imx9_dtd_s));
...
static void imx9_queuedtd(struct imx9_usb_s *priv, uint8_t epphy, struct imx9_dtd_s *dtd)
{
	...
	up_clean_dcache((uintptr_t)dqh, (uintptr_t)dqh + sizeof(struct imx9_dqh_s));
...
static inline void imx9_ep0xfer(struct imx9_usb_s *priv, uint8_t epphy, uint8_t *buf,
								uint32_t nbytes)
{
...
static void imx9_readsetup(struct imx9_usb_s *priv, uint8_t epphy, struct usb_ctrlreq_s *ctrl)
{
	...
	up_invalidate_dcache((uintptr_t)dqh, (uintptr_t)dqh + sizeof(struct imx9_dqh_s));
	...
	while (!(imx9_getreg(priv, IMX9_USBDEV_USBCMD_OFFSET) & USBDEV_USBCMD_SUTW))
		;
	...
	imx9_putreg(priv, IMX9_USBDEV_ENDPTSETUPSTAT_OFFSET, IMX9_ENDPTMASK(IMX9_EP0_OUT));
	UP_DSB();
}
...
static int imx9_progressep(struct imx9_ep_s *privep)
{
	...
	if (privreq->req.len == 0) {
		...
		if (privep->epphy == IMX9_EP0_IN && privep->dev->ep0.state == EP0STATE_SETUP_OUT) {
...
static void imx9_cancelrequests(struct imx9_ep_s *privep, int16_t status)
{
	...
	while (!imx9_rqempty(privep)) {
		...
		usbtrace(TRACE_COMPLETE(privep->epphy), (imx9_rqpeek(privep))->req.xfrd);
...
static void imx9_dispatchrequest(struct imx9_usb_s *priv, const struct usb_ctrlreq_s *ctrl)
{
	...
	if (priv->driver) {
		...
		up_invalidate_dcache((uintptr_t)priv->ep0.buf, (uintptr_t)priv->ep0.buf + EP0_MAXBUFLEN);

		/* Forward to the control request to the class driver implementation */

		ret = CLASS_SETUP(priv->driver, &priv->usbdev, ctrl, priv->ep0.buf, priv->ep0.buf_len);
	}
...
static void imx9_ep0configure(struct imx9_usb_s *priv)
{
	...
	priv->qh[IMX9_EP0_OUT].capability = (DQH_CAPABILITY_MAX_PACKET(CONFIG_USBDEV_EP0_MAXSIZE) |
										 DQH_CAPABILITY_IOS | DQH_CAPABILITY_ZLT);

	priv->qh[IMX9_EP0_IN].capability = (DQH_CAPABILITY_MAX_PACKET(CONFIG_USBDEV_EP0_MAXSIZE) |
										DQH_CAPABILITY_IOS | DQH_CAPABILITY_ZLT);

	priv->qh[IMX9_EP0_OUT].currdesc = DTD_NEXTDESC_INVALID;
	priv->qh[IMX9_EP0_IN].currdesc = DTD_NEXTDESC_INVALID;

	up_clean_dcache((uintptr_t)priv->qh, (uintptr_t)priv->qh + (sizeof(struct imx9_dqh_s)));
...
static void imx9_usbreset(struct imx9_usb_s *priv)
{
	...
	imx9_modifyreg(priv, IMX9_USBDEV_ENDPTCTRL1_OFFSET, USBDEV_ENDPTCTRL_RXE | USBDEV_ENDPTCTRL_TXE,
				   0);
	imx9_modifyreg(priv, IMX9_USBDEV_ENDPTCTRL2_OFFSET, USBDEV_ENDPTCTRL_RXE | USBDEV_ENDPTCTRL_TXE,
				   0);
	imx9_modifyreg(priv, IMX9_USBDEV_ENDPTCTRL3_OFFSET, USBDEV_ENDPTCTRL_RXE | USBDEV_ENDPTCTRL_TXE,
				   0);
	imx9_modifyreg(priv, IMX9_USBDEV_ENDPTCTRL4_OFFSET, USBDEV_ENDPTCTRL_RXE | USBDEV_ENDPTCTRL_TXE,
				   0);
	imx9_modifyreg(priv, IMX9_USBDEV_ENDPTCTRL5_OFFSET, USBDEV_ENDPTCTRL_RXE | USBDEV_ENDPTCTRL_TXE,
				   0);

	/* Clear all pending interrupts */

	imx9_putreg(priv, IMX9_USBDEV_ENDPTNAK_OFFSET, imx9_getreg(priv, IMX9_USBDEV_ENDPTNAK_OFFSET));
	...
	imx9_modifyreg(priv, IMX9_USBDEV_USBCMD_OFFSET, USBDEV_USBCMD_ITC_MASK, USBDEV_USBCMD_ITCIMME);
	...
	up_clean_dcache((uintptr_t)priv->qh, (uintptr_t)priv->qh + sizeof(priv->qh));
	up_clean_dcache((uintptr_t)priv->td, (uintptr_t)priv->td + sizeof(priv->td));
	...
	imx9_putreg(priv, IMX9_USBDEV_ENDPOINTLIST_OFFSET, (uint32_t)(uintptr_t)priv->qh);
	...
	imx9_putreg(priv, IMX9_USBDEV_USBINTR_OFFSET,
				USB_FRAME_INT | USB_ERROR_INT | USBDEV_USBINTR_NAKE | USBDEV_USBINTR_SLE |
...
static inline void imx9_ep0state(struct imx9_usb_s *priv, uint16_t state)
{
	...
	case EP0STATE_WAIT_NAK_IN:
		imx9_putreg(priv, IMX9_USBDEV_ENDPTNAKEN_OFFSET, IMX9_ENDPTMASK(IMX9_EP0_IN));
		break;

	case EP0STATE_WAIT_NAK_OUT:
		imx9_putreg(priv, IMX9_USBDEV_ENDPTNAKEN_OFFSET, IMX9_ENDPTMASK(IMX9_EP0_OUT));
		break;
...
static inline void imx9_ep0setup(struct imx9_usb_s *priv)
{
	...
	if ((ctrl->type & USB_REQ_TYPE_MASK) != USB_REQ_TYPE_STANDARD) {
		...
		case USB_REQ_GETSTATUS: {
			...
			if (!priv->paddrset || len != 2 || (ctrl->type & USB_REQ_DIR_IN) == 0 || value != 0) {
				priv->stalled = true;
			} else {
				switch (ctrl->type & USB_REQ_RECIPIENT_MASK) {
				case USB_REQ_RECIPIENT_ENDPOINT: {
					usbtrace(TRACE_INTDECODE(IMX9_TRACEINTID_EPGETSTATUS), 0);
					privep = imx9_epfindbyaddr(priv, index);
					if (!privep) {
						usbtrace(TRACE_DEVERROR(IMX9_TRACEERR_BADEPGETSTATUS), 0);
						priv->stalled = true;
					} else {
						if (privep->stalled) {
				...
				case USB_REQ_RECIPIENT_DEVICE: {
					if (index == 0) {
						usbtrace(TRACE_INTDECODE(IMX9_TRACEINTID_DEVGETSTATUS), 0);

						/* Features:  Remote Wakeup=YES; selfpowered=? */

						priv->ep0.buf[0] = (priv->selfpowered << USB_FEATURE_SELFPOWERED) |
						...
						usbtrace(TRACE_DEVERROR(IMX9_TRACEERR_BADDEVGETSTATUS), 0);
						priv->stalled = true;
					}
				} break;

				case USB_REQ_RECIPIENT_INTERFACE: {
					usbtrace(TRACE_INTDECODE(IMX9_TRACEINTID_IFGETSTATUS), 0);
					priv->ep0.buf[0] = 0;
					priv->ep0.buf[1] = 0;
				...
				default: {
					usbtrace(TRACE_DEVERROR(IMX9_TRACEERR_BADGETSTATUS), 0);
					priv->stalled = true;
				} break;
				}
			}
		...
		case USB_REQ_SETADDRESS: {
			...
			usbtrace(TRACE_INTDECODE(IMX9_TRACEINTID_EP0SETUPSETADDRESS), value);
			if (((ctrl->type & USB_REQ_RECIPIENT_MASK) == USB_REQ_RECIPIENT_DEVICE) && index == 0 &&
				len == 0 && value < 128) {
	...
	if (priv->stalled) {
		usbtrace(TRACE_DEVERROR(IMX9_TRACEERR_EP0SETUPSTALLED), priv->ep0.state);
...
static void imx9_ep0complete(struct imx9_usb_s *priv, uint8_t epphy)
{
	...
	usbtrace(TRACE_INTDECODE(IMX9_TRACEINTID_EP0COMPLETE), (uint16_t)priv->ep0.state);
	...
	case EP0STATE_WAIT_STATUS_IN:
		...
		if (!priv->paddrset && priv->paddr != 0) {
			usbtrace(TRACE_INTDECODE(IMX9_TRACEINTID_EP0INSETADDRESS), (uint16_t)priv->paddr);
	...
	default:
#ifdef CONFIG_DEBUG_FEATURES
		DEBUGASSERT(priv->ep0.state != EP0STATE_DATA_IN && priv->ep0.state != EP0STATE_DATA_OUT &&
...
static void imx9_ep0nak(struct imx9_usb_s *priv, uint8_t epphy)
{
	usbtrace(TRACE_INTDECODE(IMX9_TRACEINTID_EP0NAK), (uint16_t)priv->ep0.state);
...
bool imx9_epcomplete(struct imx9_usb_s *priv, uint8_t epphy)
{
	...
	up_invalidate_dcache((uintptr_t)dtd, (uintptr_t)dtd + sizeof(struct imx9_dtd_s));
	...
	if (IMX9_EPPHYOUT(privep->epphy)) {
		...
		DEBUGASSERT(IS_CACHE_ALIGNED(privreq->req.buf, DCACHE_ALIGN_UP(privreq->req.xfrd)));
		up_invalidate_dcache((uintptr_t)privreq->req.buf,
							 (uintptr_t)privreq->req.buf + DCACHE_ALIGN_UP(privreq->req.xfrd));
	} else {
		...
		bool need_zlp =
			(xfrd == privep->ep.maxpacket) && ((privreq->req.flags & USBDEV_REQFLAGS_NULLPKT) != 0);
...
static int imx9_usbinterrupt(int irq, void *context, void *arg)
{
	...
	if (disr & USBDEV_USBSTS_UI) {
		...
		uint32_t setupstat = imx9_getreg(priv, IMX9_USBDEV_ENDPTSETUPSTAT_OFFSET);
		if (setupstat) {
			...
			imx9_putreg(priv, IMX9_USBDEV_ENDPTCOMPLETE_OFFSET,
						IMX9_ENDPTMASK(IMX9_EP0_IN) | IMX9_ENDPTMASK(IMX9_EP0_OUT));

			if (setupstat & IMX9_ENDPTMASK(IMX9_EP0_OUT)) {
				usbtrace(TRACE_INTDECODE(IMX9_TRACEINTID_EP0SETUP), setupstat);
				imx9_ep0setup(priv);
			}
		}
	}
...
static int imx9_epconfigure(struct usbdev_ep_s *ep, const struct usb_epdesc_s *desc, bool last)
{
	...
	if ((desc->attr & USB_EP_ATTR_XFERTYPE_MASK) == USB_EP_ATTR_XFER_ISOC) {
		dqh->capability =
			(DQH_CAPABILITY_MAX_PACKET(maxsize) | DQH_CAPABILITY_IOS | DQH_CAPABILITY_ZLT);
	} else {
		dqh->capability = (DQH_CAPABILITY_MAX_PACKET(maxsize) | DQH_CAPABILITY_ZLT);
	}
	...
	if (IMX9_EPPHYIN(privep->epphy)) {
		...
		imx9_modifyreg(priv, IMX9_USBDEV_ENDPTCTRL_OFFSET(privep->epphy >> 1), 0xffff0000, cfg);
	} else {
		/* Reset the data toggles */
	...
	if (IMX9_EPPHYIN(privep->epphy)) {
		imx9_modifyreg(priv, IMX9_USBDEV_ENDPTCTRL_OFFSET(privep->epphy >> 1), 0,
					   USBDEV_ENDPTCTRL_TXE);
	} else {
		imx9_modifyreg(priv, IMX9_USBDEV_ENDPTCTRL_OFFSET(privep->epphy >> 1), 0,
...
static int imx9_epdisable(struct usbdev_ep_s *ep)
{
	...
	if (IMX9_EPPHYIN(privep->epphy)) {
		imx9_modifyreg(priv, IMX9_USBDEV_ENDPTCTRL_OFFSET(privep->epphy >> 1), USBDEV_ENDPTCTRL_TXE,
					   0);
	} else {
		imx9_modifyreg(priv, IMX9_USBDEV_ENDPTCTRL_OFFSET(privep->epphy >> 1), USBDEV_ENDPTCTRL_RXE,
					   0);
	}
...
static void imx9_epfreereq(struct usbdev_ep_s *ep, struct usbdev_req_s *req)
{
...
static int imx9_epsubmit(struct usbdev_ep_s *ep, struct usbdev_req_s *req)
{
	...
	if (!priv->driver || priv->usbdev.speed == USB_SPEED_UNKNOWN) {
		usbtrace(TRACE_DEVERROR(IMX9_TRACEERR_NOTCONFIGURED), priv->usbdev.speed);
		return -ESHUTDOWN;
	}
...
static int imx9_epstall(struct usbdev_ep_s *ep, bool resume)
{
	...
	uint32_t ctrl_xs = IMX9_EPPHYIN(privep->epphy) ? USBDEV_ENDPTCTRL_TXS : USBDEV_ENDPTCTRL_RXS;
	uint32_t ctrl_xr = IMX9_EPPHYIN(privep->epphy) ? USBDEV_ENDPTCTRL_TXR : USBDEV_ENDPTCTRL_RXR;
...
static struct usbdev_ep_s *imx9_allocep(struct usbdev_s *dev, uint8_t eplog, bool in,
										uint8_t eptype)
{
...
void arm64_usbinitialize(void)
{
	...
	imx9_putreg(priv, IMX9_USBDEV_USBMODE_OFFSET,
				USBDEV_USBMODE_SDIS | USBDEV_USBMODE_SLOM | USBDEV_USBMODE_CM_DEVICE);
...
#ifdef CONFIG_DEBUG_FEATURES
	if (!driver || !driver->ops->bind || !driver->ops->unbind || !driver->ops->disconnect ||
		!driver->ops->setup) {
		usbtrace(TRACE_DEVERROR(IMX9_TRACEERR_INVALIDPARMS), 0);
		return -EINVAL;
	}
