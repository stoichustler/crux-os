 ...
 * include/roux/spi/spi.h). All other methods (including
...
#define SPI_SR_CLEAR                                                                               \
	(LPSPI_SR_WCF | LPSPI_SR_FCF | LPSPI_SR_TCF | LPSPI_SR_TEF | LPSPI_SR_REF | LPSPI_SR_DMF)
...
static inline uint32_t imx9_lpspi_getreg32(struct imx9_lpspidev_s *priv, uint8_t offset);
static inline void imx9_lpspi_putreg32(struct imx9_lpspidev_s *priv, uint8_t offset,
									   uint32_t value);
static inline uint32_t imx9_lpspi_readword(struct imx9_lpspidev_s *priv);
static inline void imx9_lpspi_writeword(struct imx9_lpspidev_s *priv, uint16_t byte);
static inline bool imx9_lpspi_9to16bitmode(struct imx9_lpspidev_s *priv);
static inline void imx9_lpspi_master_set_delays(struct imx9_lpspidev_s *priv, uint32_t delay_ns,
												enum imx9_delay_e type);
static inline void imx9_lpspi_master_set_delay_scaler(struct imx9_lpspidev_s *priv, uint32_t scaler,
													  enum imx9_delay_e type);
...
static uint32_t imx9_lpspi_setfrequency(struct spi_dev_s *dev, uint32_t frequency);
...
static int imx9_lpspi_hwfeatures(struct spi_dev_s *dev, imx9_lpspi_hwfeatures_t features);
#endif
static uint32_t imx9_lpspi_send(struct spi_dev_s *dev, uint32_t wd);
static void imx9_lpspi_exchange(struct spi_dev_s *dev, const void *txbuffer, void *rxbuffer,
								size_t nwords);
#ifndef CONFIG_SPI_EXCHANGE
static void imx9_lpspi_sndblock(struct spi_dev_s *dev, const void *txbuffer, size_t nwords);
static void imx9_lpspi_recvblock(struct spi_dev_s *dev, void *rxbuffer, size_t nwords);
#endif
...
static inline void imx9_lpspi_putreg32(struct imx9_lpspidev_s *priv, uint8_t offset, uint32_t value)
{
...
static inline uint32_t imx9_lpspi_readword(struct imx9_lpspidev_s *priv)
{
	...
	while ((imx9_lpspi_getreg32(priv, IMX9_LPSPI_SR_OFFSET) & LPSPI_SR_RDF) == 0)
		;
...
static inline void imx9_lpspi_writeword(struct imx9_lpspidev_s *priv, uint16_t word)
{
	/* Wait until the transmit buffer is empty */

	while ((imx9_lpspi_getreg32(priv, IMX9_LPSPI_SR_OFFSET) & LPSPI_SR_TDF) == 0)
		;
...
static inline bool imx9_lpspi_9to16bitmode(struct imx9_lpspidev_s *priv)
{
	...
	if (((imx9_lpspi_getreg32(priv, IMX9_LPSPI_TCR_OFFSET) & LPSPI_TCR_FRAMESZ_MASK) + 1) < 9) {
		ret = false;
	} else {
		ret = true;
	}
...
static void imx9_lpspi_modifyreg32(struct imx9_lpspidev_s *priv, uint8_t offset, uint32_t clrbits,
								   uint32_t setbits)
{
...
static void imx9_lpspi_modifytcr(struct imx9_lpspidev_s *priv, uint32_t clrbits, uint32_t setbits)
{
	uint32_t men;
													  ...
													  enum imx9_delay_e type)
{
	...
	case LPSPI_PCS_TO_SCK:
		imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_CCR_OFFSET, LPSPI_CCR_PCSSCK_MASK, 0);
		imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_CCR_OFFSET, 0,
							   LPSPI_CCR_DBT(dbt) | LPSPI_CCR_PCSSCK(scaler) |
								   LPSPI_CCR_SCKDIV(sckdiv));
		break;

	case LPSPI_LAST_SCK_TO_PCS:
		imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_CCR_OFFSET, LPSPI_CCR_SCKPCS_MASK, 0);
		imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_CCR_OFFSET, 0,
							   LPSPI_CCR_DBT(dbt) | LPSPI_CCR_PCSSCK(scaler) |
								   LPSPI_CCR_SCKDIV(sckdiv));
		break;
	...
	case LPSPI_BETWEEN_TRANSFER:
		imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_CCR_OFFSET, LPSPI_CCR_DBT_MASK, 0);
		imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_CCR_OFFSET, 0,
							   LPSPI_CCR_DBT(dbt) | LPSPI_CCR_PCSSCK(scaler) |
								   LPSPI_CCR_SCKDIV(sckdiv));
		break;
												...
												enum imx9_delay_e type)
{
	...
	clock_div_prescaler =
		src_freq /
		(1 << ((imx9_lpspi_getreg32(priv, IMX9_LPSPI_TCR_OFFSET) & LPSPI_TCR_PRESCALE_MASK) >>
...
static uint32_t imx9_lpspi_setfrequency(struct spi_dev_s *dev, uint32_t frequency)
{
	...
	if (frequency != priv->frequency) {
		...
		imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_CCR_OFFSET, LPSPI_CCR_SCKDIV_MASK,
							   LPSPI_CCR_SCKDIV(best_scaler));

		/* Update TCR */

		imx9_lpspi_modifytcr(priv, LPSPI_TCR_PRESCALE_MASK, LPSPI_TCR_PRESCALE(best_prescaler));

		priv->frequency = frequency;
		priv->actual = best_frequency;
		imx9_lpspi_master_set_delays(priv, 1000000000 / best_frequency, LPSPI_PCS_TO_SCK);
		imx9_lpspi_master_set_delays(priv, 1000000000 / best_frequency, LPSPI_LAST_SCK_TO_PCS);
		imx9_lpspi_master_set_delays(priv, 1000000000 / best_frequency, LPSPI_BETWEEN_TRANSFER);
...
static void imx9_lpspi_setmode(struct spi_dev_s *dev, enum spi_mode_e mode)
{
	...
	if (mode != priv->mode) {
		...
		imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_CR_OFFSET, 0, LPSPI_CR_RRF);
...
static void imx9_lpspi_setbits(struct spi_dev_s *dev, int nbits)
{
	...
	if (nbits != priv->nbits) {
		...
		imx9_lpspi_modifytcr(priv, LPSPI_TCR_FRAMESZ_MASK, LPSPI_TCR_FRAMESZ(nbits - 1));
...
static uint32_t imx9_lpspi_send(struct spi_dev_s *dev, uint32_t wd)
{
	...
	while ((imx9_lpspi_getreg32(priv, IMX9_LPSPI_SR_OFFSET) & LPSPI_SR_RDF) != LPSPI_SR_RDF)
		;
...
static void imx9_lpspi_exchange(struct spi_dev_s *dev, const void *txbuffer, void *rxbuffer,
								size_t nwords)
#else
static void imx9_lpspi_exchange_nodma(struct spi_dev_s *dev, const void *txbuffer, void *rxbuffer,
									  size_t nwords)
#endif
{
	struct imx9_lpspidev_s *priv = (struct imx9_lpspidev_s *)dev;
								...
								size_t nwords)
{
	...
	imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_CR_OFFSET, LPSPI_CR_RTF | LPSPI_CR_RRF,
	...
	if (txbuffer) {
		...
		up_clean_dcache((uintptr_t)priv->txbuf, (uintptr_t)priv->txbuf + nbytes);
	}
	...
	imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_DER_OFFSET, 0, LPSPI_DER_TDDE | LPSPI_DER_RDDE);
	...
	if (rxbuffer && ret >= 0) {
		...
		up_invalidate_dcache((uintptr_t)priv->rxbuf, (uintptr_t)priv->rxbuf + nbytes);
...
static void imx9_lpspi_bus_initialize(struct imx9_lpspidev_s *priv)
{
	...
	imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_CR_OFFSET, 0, LPSPI_CR_RTF | LPSPI_CR_RRF);
	imx9_lpspi_putreg32(priv, IMX9_LPSPI_CR_OFFSET, 0x00);

	/* Set LPSPI to master */

	imx9_lpspi_modifyreg32(priv, IMX9_LPSPI_CFGR1_OFFSET, 0, LPSPI_CFGR1_MASTER);
	...
	imx9_lpspi_putreg32(priv, IMX9_LPSPI_FCR_OFFSET, LPSPI_FCR_TXWATER(0) | LPSPI_FCR_RXWATER(0));
	...
	if (priv->rxch && priv->txch) {
		...
		if (priv->txbuf == NULL && priv->rxbuf == NULL) {
			...
			up_invalidate_dcache((uintptr_t)priv->rxbuf,
								 (uintptr_t)priv->rxbuf + CONFIG_IMX9_LPSPI_DMA_BUFFER_SIZE);
		}
	} else {
		priv->rxdma = NULL;
		priv->txdma = NULL;
