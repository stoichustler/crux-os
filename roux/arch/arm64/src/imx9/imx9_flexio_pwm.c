...
static inline uint32_t flexio_getreg(struct imx9_pwmtimer_s *priv, int offset)
{
...
static inline void flexio_putreg(struct imx9_pwmtimer_s *priv, int offset, uint32_t value)
{
...
static int pwm_update_frequency(struct imx9_pwmtimer_s *priv, int freq)
{
	...
	flexio_putreg(priv, IMX9_FLEXIO_TIMCMP_OFFSET(priv->trigger_ch), priv->period / 2);

	/* Enable / disable timer */

	flexio_putreg(priv, IMX9_FLEXIO_TIMCTL_OFFSET(priv->trigger_ch),
				  freq > 0 ? FLEXIO_TIMCTL_TIMOD(0x3) : FLEXIO_TIMCTL_TIMOD(0x0));
...
static int pwm_update_duty(struct imx9_pwmtimer_s *priv, int pwm_ch, ub16_t duty16)
{
	...
	if (priv->frequency == 0) {
		flexio_putreg(priv, IMX9_FLEXIO_TIMCFG_OFFSET(timer),
					  FLEXIO_TIMCFG_TIMDIS(0x2) | FLEXIO_TIMCFG_TIMENA(0x6));
		...
		regval = (FLEXIO_TIMCTL_TIMOD(0x3) | FLEXIO_TIMCTL_TRGSEL(4 * priv->trigger_ch + 3) |
				  FLEXIO_TIMCTL_TRGSRC(priv->int_trigger) |
				  FLEXIO_TIMCTL_PINSEL(PIN_FOR_TIMER(priv, timer)));

		flexio_putreg(priv, IMX9_FLEXIO_TIMCTL_OFFSET(timer), regval | FLEXIO_TIMCTL_PINCFG(0x2));

		flexio_putreg(priv, IMX9_FLEXIO_TIMCTL_OFFSET(timer), regval | FLEXIO_TIMCTL_PINCFG(0x3));
	}

	pwminfo("PWM%d channel %d, p: %d e: %" PRIu32 "\n", priv->id, pwm_ch, priv->period, edge);
...
static int pwm_start(struct pwm_lowerhalf_s *dev, const struct pwm_info_s *info)
{
	...
	for (i = 0; i < CONFIG_PWM_NCHANNELS; i++) {
		...
		ret = pwm_update_duty(priv, info->channels[i].channel, info->channels[i].duty);
	}
...
static int pwm_stop(struct pwm_lowerhalf_s *dev)
{
	...
	for (i = 0; i < priv->nchannels; i++) {
		flexio_putreg(priv, IMX9_FLEXIO_TIMCTL_OFFSET(i), FLEXIO_TIMCTL_PINCFG(0x2));
...
struct pwm_lowerhalf_s *imx9_flexio_pwm_init(flexio_pwm_id_t pwmid)
{
	...
	if (lower) {
		...
		while (flexio_getreg(lower, IMX9_FLEXIO_CTRL_OFFSET) != FLEXIO_CTRL_FLEXEN_MASK)
			;
