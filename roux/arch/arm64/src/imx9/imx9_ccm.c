...
static int imx9_ccm_clk_src_tz_access(uint32_t oscpll, bool non_secure, bool user_mode,
									  bool lock_tz)
{
	...
	modifyreg32(IMX9_CCM_OSCPLL_AUTH(oscpll), 0,
				CCM_AUTH_TZ_USER(user_mode) | CCM_AUTH_TZ_NS(non_secure) |
...
static int imx9_ccm_clk_root_tz_access(uint32_t clk_root_id, bool non_secure, bool user_mode,
									   bool lock_tz)
{
...
static int imx9_ccm_lpcg_tz_access(uint32_t lpcg, bool non_secure, bool user_mode, bool lock_tz)
{
...
static int imx9_ccm_shared_gpr_tz_access(uint32_t gpr, bool non_secure, bool user_mode,
										 bool lock_tz)
{
...
int imx9_ccm_clock_init(void)
{
	...
	ret = imx9_ccm_configure_root_clock(CCM_ARM_A55_PERIPH_CLK_ROOT, SYS_PLL1PFD0, 3);
	if (ret != 0) {
		return ret;
	}
	...
	ret = imx9_ccm_configure_root_clock(CCM_ARM_A55_MTR_BUS_CLK_ROOT, SYS_PLL1PFD1DIV2, 3);
	if (ret != 0) {
		return ret;
	}
	...
	ret = imx9_ccm_configure_root_clock(CCM_BUS_WAKEUP_CLK_ROOT, SYS_PLL1PFD1DIV2, 3);
	if (ret != 0) {
		return ret;
	}
	...
	ret = imx9_ccm_configure_root_clock(CCM_BUS_AON_CLK_ROOT, SYS_PLL1PFD1DIV2, 3);
	if (ret != 0) {
		return ret;
	}
	...
	ret = imx9_ccm_configure_root_clock(CCM_WAKEUP_AXI_CLK_ROOT, SYS_PLL1PFD2, 2);
	if (ret != 0) {
		return ret;
	}
	...
	ret = imx9_ccm_configure_root_clock(CCM_SWO_TRACE_CLK_ROOT, SYS_PLL1PFD1DIV2, 3);
	if (ret != 0) {
		return ret;
	}
	...
	ret = imx9_ccm_configure_root_clock(CCM_NIC_APB_CLK_ROOT, SYS_PLL1PFD1DIV2, 3);
	if (ret != 0) {
		return ret;
	}
