...
#define IMX9_LPSPI_TDBR_OFFSET(n)                                                                  \
	(0x0400 + ((n) << 2)) /* Transmit Data Burst Register n=0..127 (TDBRn) */
#define IMX9_LPSPI_RDBR_OFFSET(n)                                                                  \
	(0x0600 + ((n) << 2)) /* Receive Data Burst Register n=0..127 (RDBRn) */
...
#define LPSPI_CFGR0_RDMO_FIFO                                                                      \
	(0 << 9) /*        Received data is stored in the receive FIFO as in normal operations */
#define LPSPI_CFGR0_RDMO_DMF                                                                       \
	(1 << 9) /*        Received data is discarded unless the Data Match Flag (DMF) is set */
			 /* Bits 10-31: Reserved */
...
#define LPSPI_CFGR1_PCSPOL_LOW(n)                                                                  \
	(0 << (LPSPI_CFGR1_PCSPOL_SHIFT +                                                              \
		   (n))) /* The Peripheral Chip Select PCS[n] pin is active low */
#define LPSPI_CFGR1_PCSPOL_HIGH(n)                                                                 \
	(1 << (LPSPI_CFGR1_PCSPOL_SHIFT +                                                              \
		   (n))) /* The Peripheral Chip Select PCS[n] pin is active high */
...
#define LPSPI_CFGR1_PINCFG_SIN_SOUT                                                                \
	(0x00 << LPSPI_CFGR1_PINCFG_SHIFT) /* SIN is used for input data and SOUT is used for output   \
					  data */
#define LPSPI_CFGR1_PINCFG_SIN_SIN                                                                 \
	(0x01 << LPSPI_CFGR1_PINCFG_SHIFT) /* SIN is used for both input and output data */
#define LPSPI_CFGR1_PINCFG_SOUT_SOUT                                                               \
	(0x02 << LPSPI_CFGR1_PINCFG_SHIFT) /* SOUT is used for both input and output data */
#define LPSPI_CFGR1_PINCFG_SOUT_SIN                                                                \
	(0x03 << LPSPI_CFGR1_PINCFG_SHIFT) /* SOUT is used for input data and SIN is used for output   \
					  data */
#define LPSPI_CFGR1_PINCFG(n) ((n) << LPSPI_CFGR1_PINCFG_SHIFT)

#define LPSPI_CFGR1_OUTCFG (1 << 26) /* Bit 26: Output Config (OUTCFG) */
#define LPSPI_CFGR1_OUTCFG_RETAIN                                                                  \
	(0 << 26) /*         Output data retains last value when chip select is negated */
#define LPSPI_CFGR1_OUTCFG_TRISTATE                                                                \
	(1 << 26) /*         Output data is tristated when chip select is negated */
#define LPSPI_CFGR1_PCSCFG_SHIFT                                                                   \
	(27) /* Bits 27-28: Peripheral Chip Select Configuration (PCSCFG) */
#define LPSPI_CFGR1_PCSCFG_MASK (0x03 << LPSPI_CFGR1_PCSCFG_SHIFT)
#define LPSPI_CFGR1_PCSCFG_PCS                                                                     \
	(0x00 << LPSPI_CFGR1_PCSCFG_SHIFT) /* PCS[2:7] are configured for chip select function */
#define LPSPI_CFGR1_PCSCFG_4BIT                                                                    \
	(0x01 << LPSPI_CFGR1_PCSCFG_SHIFT) /* PCS[2:3] are configured for half-duplex 4-bit transfers  \
										*/
#define LPSPI_CFGR1_PCSCFG_8BIT                                                                    \
	(0x03 << LPSPI_CFGR1_PCSCFG_SHIFT) /* PCS[2:7] are configured for half-duplex 4-bit and 8-bit  \
					  transfers */
...
#define LPSPI_TCR_CPHA_CAPTURED                                                                    \
	(0 << 30) /*         Data is captured on the leading edge of SCK and changed on the following  \
		 edge of SCK */
#define LPSPI_TCR_CPHA_CHANGED                                                                     \
