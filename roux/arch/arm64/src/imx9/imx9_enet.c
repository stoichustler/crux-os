...
#define ALIGNED_BUFSIZE ENET_ALIGN_UP(CONFIG_NET_ETH_PKTSIZE + CONFIG_NET_GUARDSIZE)
...
#define ERROR_INTERRUPTS                                                                           \
	(ENET_INT_UN | ENET_INT_RL | ENET_INT_LC | ENET_INT_EBERR | ENET_INT_BABT | ENET_INT_BABR)
...
static inline uint32_t imx9_enet_getreg32(struct imx9_driver_s *priv, uint32_t offset);
static inline void imx9_enet_putreg32(struct imx9_driver_s *priv, uint32_t value, uint32_t offset);

static inline void imx9_enet_modifyreg32(struct imx9_driver_s *priv, unsigned int offset,
...
static int imx9_writemii(struct imx9_driver_s *priv, uint8_t regaddr, uint16_t data);
static int imx9_readmii(struct imx9_driver_s *priv, uint8_t regaddr, uint16_t *data);
...
static int imx9_readmmd(struct imx9_driver_s *priv, uint8_t mmd, uint16_t regaddr, uint16_t *data);
#if 0
...
static struct enet_txdesc_s g_enet1_tx_desc_pool[IMX9_ENET_NTXBUFFERS] aligned_data(ENET_ALIGN);
static struct enet_desc_s g_enet1_rx_desc_pool[IMX9_ENET_NRXBUFFERS] aligned_data(ENET_ALIGN);

/* The DMA buffers */

static uint8_t g_enet_1_buffer_pool[IMX9_ENET_NTXBUFFERS + IMX9_ENET_NRXBUFFERS]
								   [ALIGNED_BUFSIZE] aligned_data(ENET_ALIGN);
#endif
...
static bool imx9_txringfull(struct imx9_driver_s *priv)
{
	...
	up_invalidate_dcache((uintptr_t)txdesc, (uintptr_t)txdesc + sizeof(struct enet_txdesc_s));
...
static int imx9_transmit(struct imx9_driver_s *priv, uint32_t *buf_swap)
{
	...
	txdesc2->status1 = (txdesc2->status1 & TXDESC_W) | (TXDESC_TC | TXDESC_L | TXDESC_R);
	...
	up_clean_dcache((uintptr_t)txdesc, (uintptr_t)txdesc + sizeof(struct enet_txdesc_s));
	...
	wd_start(&priv->txtimeout, IMX9_TXTIMEOUT, imx9_txtimeout_expiry, (wdparm_t)priv);
...
{
	up_invalidate_dcache((uintptr_t)rxdesc, (uintptr_t)rxdesc + sizeof(struct enet_desc_s));
	...
	do {
		...
		if (received) {
			...
			if ((rxdesc->status1 & RXDESC_STATUS1_ERRORS) != 0 ||
				...
				nerr("status1 %x, status2 %x", rxdesc->status1 & RXDESC_STATUS1_ERRORS,
					 rxdesc->status2 & RXDESC_STATUS2_ERRORS);
			}

			DEBUGASSERT(rxdesc->length > 0 && ((uintptr_t)rxdesc->data & ENET_ALIGN_MASK) == 0);
			...
			if (swap_d_n == 1) {
				...
				up_clean_dcache((uintptr_t)&rxdesc[(-1)],
								(uintptr_t)&rxdesc[(-1)] + 2 * sizeof(rxdesc[0]));
...
static int imx9_ifup_action(struct net_driver_s *dev, bool resetphy)
{
	...
	imx9_enet_putreg32(priv, (mac[0] << 24) | (mac[1] << 16) | (mac[2] << 8) | mac[3],
					   IMX9_ENET_PALR_OFFSET);
	imx9_enet_putreg32(priv, (mac[4] << 24) | (mac[5] << 16), IMX9_ENET_PAUR_OFFSET);
	...
	imx9_enet_putreg32(priv, (uint32_t)(uintptr_t)priv->rxdesc, IMX9_ENET_RDSR_OFFSET);

	/* Point to the start of the circular TX buffer descriptor queue */

	imx9_enet_putreg32(priv, (uint32_t)(uintptr_t)priv->txdesc, IMX9_ENET_TDSR_OFFSET);
...
static int imx9_ioctl(struct net_driver_s *dev, int cmd, unsigned long arg)
{
	...
	{
		struct mii_ioctl_data_s *req = (struct mii_ioctl_data_s *)((uintptr_t)arg);
		if (priv->cur_phy && priv->cur_phy->clause == 45 && MII_MSR == req->reg_num) {
			ret = imx9_readmmd(priv, MMD1, MMD1_PMA_STATUS1, &req->val_out);
		} else {
...
static int imx9_phyintenable(struct imx9_driver_s *priv)
{
	...
	if (imx9_phy_is(priv, MII_YT8512_NAME)) {
		...
		rreg = MII_YT8512_ISR;
		wreg = MII_YT8512_IMR;
	} else if (imx9_phy_is(priv, MII_KSZ8051_NAME) || imx9_phy_is(priv, MII_KSZ8061_NAME) ||
			   imx9_phy_is(priv, MII_KSZ8081_NAME) || imx9_phy_is(priv, MII_DP83825I_NAME)) {
...
static void imx9_initmii(struct imx9_driver_s *priv)
{
	...
	imx9_enet_putreg32(priv,
					   ENET_MSCR_HOLDTIME_2CYCLES | IMX9_MII_SPEED << ENET_MSCR_MII_SPEED_SHIFT,
...
static int imx9_writemii(struct imx9_driver_s *priv, uint8_t regaddr, uint16_t data)
{
	...
	imx9_enet_putreg32(priv,
					   data | 2 << ENET_MMFR_TA_SHIFT | (uint32_t)regaddr << ENET_MMFR_RA_SHIFT |
						   (uint32_t)priv->phyaddr << ENET_MMFR_PA_SHIFT | ENET_MMFR_OP_WRMII |
	...
	for (timeout = 0; timeout < MII_MAXPOLLS; timeout++) {
		if ((imx9_enet_getreg32(priv, IMX9_ENET_EIR_OFFSET) & ENET_INT_MII) != 0) {
			break;
		}
	}
...
static int imx9_readmii(struct imx9_driver_s *priv, uint8_t regaddr, uint16_t *data)
{
	...
	imx9_enet_putreg32(priv,
					   2 << ENET_MMFR_TA_SHIFT | (uint32_t)regaddr << ENET_MMFR_RA_SHIFT |
						   (uint32_t)priv->phyaddr << ENET_MMFR_PA_SHIFT | ENET_MMFR_OP_RDMII |
						   1 << ENET_MMFR_ST_SHIFT,
					   IMX9_ENET_MMFR_OFFSET);
	...
	*data = (uint16_t)(imx9_enet_getreg32(priv, IMX9_ENET_MMFR_OFFSET) & ENET_MMFR_DATA_MASK);
	return OK;
}
...
int imx9_read_phy_status(struct imx9_driver_s *priv)
{
	...
	if (status != 0xffff) {
		ninfo("%s: PHY status %x: %04x\n", priv->cur_phy->name, priv->cur_phy->status, status);
...
static int imx9_determine_phy(struct imx9_driver_s *priv)
{
	...
	for (i = 0; i < priv->n_phys; i++) {
		...
		for (; priv->phyaddr <= last_phyaddr; priv->phyaddr++) {
			...
			if (retries <= 3 && ret == 0 && phydata == priv->phy_list[i].id1) {
				do {
					nxsig_usleep(100);
					phydata = 0xffff;
					ret = imx9_readmii(priv, MII_PHYID2, &phydata);
					ninfo("phy %s addr %d received PHYID2 %x\n", priv->phy_list[i].name,
						  priv->phyaddr, phydata);
				} while ((ret < 0 || phydata == 0xffff) && ++retries < 3);
				if (retries <= 3 && ret == 0 &&
					(phydata & 0xfff0) == (priv->phy_list[i].id2 & 0xfff0)) {
...
static int imx9_readmmd(struct imx9_driver_s *priv, uint8_t mmd, uint16_t regaddr, uint16_t *data)
{
	...
	imx9_enet_putreg32(
		priv,
		0 << ENET_MMFR_ST_SHIFT | ENET_MMFR_OP_WRNOTMII | (uint32_t)mmd << ENET_MMFR_RA_SHIFT |
			(uint32_t)priv->phyaddr << ENET_MMFR_PA_SHIFT | 2 << ENET_MMFR_TA_SHIFT | regaddr,
		IMX9_ENET_MMFR_OFFSET);
	...
	imx9_enet_putreg32(priv,
					   0 << ENET_MMFR_ST_SHIFT | ENET_MMFR_OP_RDNOTMII |
						   (uint32_t)mmd << ENET_MMFR_RA_SHIFT |
						   (uint32_t)priv->phyaddr << ENET_MMFR_PA_SHIFT | 2 << ENET_MMFR_TA_SHIFT,
					   IMX9_ENET_MMFR_OFFSET);
...
static int imx9_phy_set_speed(struct imx9_driver_s *priv, bool autonegotiate)
{
	...
	if (!autonegotiate) {
		...
		if (priv->full_duplex) {
			advertise &= ~(MII_ADVERTISE_1000XHALF | MII_ADVERTISE_100BASETXHALF |
						   MII_ADVERTISE_10BASETXHALF);
			btcr &= ~GMII_1000BTCR_1000BASETHALF;
			force_mcr |= MII_MCR_FULLDPLX;
		} else {
			advertise &= ~(MII_ADVERTISE_1000XFULL | MII_ADVERTISE_100BASETXFULL |
		...
		if (priv->s_10mbps) {
			advertise &= ~(MII_ADVERTISE_100BASETXFULL | MII_ADVERTISE_100BASETXHALF |
						   MII_ADVERTISE_1000XFULL | MII_ADVERTISE_1000XHALF);
			btcr &= ~(GMII_1000BTCR_1000BASETHALF | GMII_1000BTCR_1000BASETFULL);
...
static inline int imx9_initphy(struct imx9_driver_s *priv, bool renogphy)
{
	...
	if (renogphy) {
		...
		if (priv->cur_phy && (phydata & 0xfff0) != (priv->cur_phy->id2 & 0xfff0)) {
			nerr("ERROR: PHYID2=%04x incorrect for %s.  Expected %04x\n", (phydata & 0xfff0),
				 phy_name, (priv->cur_phy->id2 & 0xfff0));
			return -ENXIO;
		}

		if (imx9_phy_is(priv, MII_LAN8720_NAME) || imx9_phy_is(priv, MII_LAN8742A_NAME)) {
	...
	rcr = (ENET_RCR_CRCFWD |
		   ((CONFIG_NET_ETH_PKTSIZE + CONFIG_NET_GUARDSIZE) << ENET_RCR_MAX_FL_SHIFT) |
...
static void imx9_initbuffers(struct imx9_driver_s *priv)
{
	...
	up_clean_dcache((uintptr_t)priv->txdesc,
					(uintptr_t)priv->txdesc + IMX9_ENET_NTXBUFFERS * sizeof(priv->txdesc[0]));
	up_clean_dcache((uintptr_t)priv->rxdesc,
					(uintptr_t)priv->rxdesc + IMX9_ENET_NRXBUFFERS * sizeof(priv->rxdesc[0]));
