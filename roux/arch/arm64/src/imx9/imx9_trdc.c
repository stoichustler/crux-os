...
static const struct trdc_config_info g_trdc_cfg_info[] = {
	{0x44270000, trdc_a_mbc_glbac, nitems(trdc_a_mbc_glbac), trdc_a_mbc, nitems(trdc_a_mbc),
	 trdc_a_mrc_glbac, nitems(trdc_a_mrc_glbac), trdc_a_mrc, nitems(trdc_a_mrc)}, /* TRDC_A */
	{0x42460000, trdc_w_mbc_glbac, nitems(trdc_w_mbc_glbac), trdc_w_mbc, nitems(trdc_w_mbc),
	 trdc_w_mrc_glbac, nitems(trdc_w_mrc_glbac), trdc_w_mrc, nitems(trdc_w_mrc)}, /* TRDC_W */
	{0x49010000, trdc_n_mbc_glbac, nitems(trdc_n_mbc_glbac), trdc_n_mbc, nitems(trdc_n_mbc),
	 trdc_n_mrc_glbac, nitems(trdc_n_mrc_glbac), trdc_n_mrc, nitems(trdc_n_mrc)}, /* TRDC_N */
};

static const struct trdc_config_info g_trdc_boot_cfg_info[] = {
	{0x49010000, trdc_boot_mbc_glbac, nitems(trdc_boot_mbc_glbac), trdc_boot_ocram_mbc,
	 nitems(trdc_boot_ocram_mbc), trdc_n_mrc_glbac, nitems(trdc_n_mrc_glbac), trdc_n_mrc,
	 nitems(trdc_n_mrc)},
};
...
static int trdc_mda_set_noncpu(unsigned long trdc_reg, uint32_t mda_inst, uint32_t mda_reg,
							   bool did_bypass, uint8_t sa, uint8_t pa, uint8_t did, bool lock)
{
...
static uint32_t trdc_mbc_blk_num(unsigned long trdc_reg, uint32_t mbc_x, uint32_t mem_x)
{
...
static int trdc_mbc_set_control(unsigned long trdc_reg, uint32_t mbc_x, uint32_t glbac_id,
								uint32_t glbac_val)
{
	struct trdc_mbc *mbc_base = (struct trdc_mbc *)trdc_get_mbc_base(trdc_reg, mbc_x);
	struct mbc_mem_dom *mbc_dom;
	...
	if (glbac_id == 7 && glbac_val != 0x6000) {
		for (i = 0; i < nitems(g_trdc_mgr_blks); i++) {
			if (trdc_reg == g_trdc_mgr_blks[i].trdc_base && mbc_x == g_trdc_mgr_blks[i].mbc_id) {
				return -EPERM;
			}
		}
	}
...
static int trdc_mbc_blk_config(unsigned long trdc_reg, uint32_t mbc_x, uint32_t dom_x,
							   uint32_t mem_x, uint32_t blk_x, bool sec_access, uint32_t glbac_id)
{
	struct trdc_mbc *mbc_base = (struct trdc_mbc *)trdc_get_mbc_base(trdc_reg, mbc_x);
	struct mbc_mem_dom *mbc_dom;
	uint32_t *cfg_w;
...
static int trdc_mrc_set_control(unsigned long trdc_reg, uint32_t mrc_x, uint32_t glbac_id,
								uint32_t glbac_val)
{
...
static int trdc_mrc_rgn_config(unsigned long trdc_reg, uint32_t mrc_x, uint32_t dom_x,
							   uint32_t rgn_id, uint32_t addr_start, uint32_t addr_size,
...
static bool is_trdc_mgr_slot(unsigned long trdc_base, uint8_t mbc_id, uint8_t mem_id,
							 uint16_t blk_id)
{
	uint32_t i;
	...
	for (i = 0; i < nitems(g_trdc_mgr_blks); i++) {
		if (g_trdc_mgr_blks[i].trdc_base == trdc_base) {
			if (mbc_id == g_trdc_mgr_blks[i].mbc_id && mem_id == g_trdc_mgr_blks[i].mbc_mem_id &&
				(blk_id == g_trdc_mgr_blks[i].blk_mgr || blk_id == g_trdc_mgr_blks[i].blk_mc))
				return true;
		}
	}
	return false;
...
static void trdc_mgr_mbc_setup(const struct trdc_mgr_info *mgr)
{
	...
	if (trdc_mbc_enabled(mgr->trdc_base)) {
		trdc_mbc_set_control(mgr->trdc_base, mgr->mbc_id, 7,
							 0x6000); /* ONLY secure privilege can access */
		for (i = 0; i < 16; i++) {
			trdc_mbc_blk_config(mgr->trdc_base, mgr->mbc_id, i, mgr->mbc_mem_id, mgr->blk_mgr, true,
								7);
			trdc_mbc_blk_config(mgr->trdc_base, mgr->mbc_id, i, mgr->mbc_mem_id, mgr->blk_mc, true,
								7);
		}
...
static void trdc_fuse_init(void)
{
	...
	for (i = 0; i < nitems(g_fuse_data); i++) {
		...
		{
			g_fuse_data[i].value = imx9_ele_read_common_fuse(g_fuse_data[i].fsb_index);
		} else {
			g_fuse_data[i].value =
				mmio_read_32(FSB_BASE + FSB_SHADOW_OFF + (g_fuse_data[i].fsb_index << 2));
		}
	}
}
...
static void trdc_mgr_fused_slot_setup(const struct trdc_fused_module_info *fused_slot)
{
	uint32_t i;
	uint32_t val;
	uint32_t did;
	...
	if (trdc_mbc_enabled(fused_slot->trdc_base)) {
		trdc_mbc_set_control(fused_slot->trdc_base, fused_slot->mbc_id, 6,
		...
		for (i = 0; i < cfg->num_mbc_cfg; i++) {
			if (cfg->mbc_cfg[i].blk_id == MBC_BLK_ALL) {
				...
				for (j = 0; j < num; j++) {
					...
					is_mgr = is_trdc_mgr_slot(cfg->trdc_base, cfg->mbc_cfg[i].mbc_id,
											  cfg->mbc_cfg[i].mem_id, j);
					if (is_mgr)
						continue;

					trdc_mbc_blk_config(cfg->trdc_base, cfg->mbc_cfg[i].mbc_id,
										cfg->mbc_cfg[i].dom_id, cfg->mbc_cfg[i].mem_id, j,
										cfg->mbc_cfg[i].secure, cfg->mbc_cfg[i].glbac_id);
				}
			} else {
				trdc_mbc_blk_config(cfg->trdc_base, cfg->mbc_cfg[i].mbc_id, cfg->mbc_cfg[i].dom_id,
									cfg->mbc_cfg[i].mem_id, cfg->mbc_cfg[i].blk_id,
									cfg->mbc_cfg[i].secure, cfg->mbc_cfg[i].glbac_id);
			}
		}
	}
...
static void trdc_try_lockup(const struct trdc_config_info *cfg)
{
	...
	if (trdc_mrc_enabled(cfg->trdc_base)) {
		for (i = 0; i < cfg->num_mrc_glbac; i++) {
			trdc_mrc_set_control(
				cfg->trdc_base, cfg->mrc_glbac[i].mbc_mrc_id, cfg->mrc_glbac[i].glbac_id,
	...
	if (trdc_mbc_enabled(cfg->trdc_base)) {
		for (i = 0; i < cfg->num_mbc_glbac; i++) {
			trdc_mbc_set_control(
				cfg->trdc_base, cfg->mbc_glbac[i].mbc_mrc_id, cfg->mbc_glbac[i].glbac_id,
