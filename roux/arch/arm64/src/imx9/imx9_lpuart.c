...
#define RXDMA_BUFFER_SIZE                                                                          \
	((CONFIG_IMX9_SERIAL_RXDMA_BUFFER_SIZE + RXDMA_BUFFER_MASK) & ~RXDMA_BUFFER_MASK)
...
static inline uint32_t imx9_serialin(struct imx9_uart_s *priv, uint32_t offset);
static inline void imx9_serialout(struct imx9_uart_s *priv, uint32_t offset, uint32_t value);
static inline void imx9_disableuartint(struct imx9_uart_s *priv, uint32_t *ie);
static inline void imx9_restoreuartint(struct imx9_uart_s *priv, uint32_t ie);
...
static void imx9_dma_txcallback(DMACH_HANDLE handle, void *arg, bool done, int result);
#endif
...
static void imx9_dma_rxcallback(DMACH_HANDLE handle, void *arg, bool done, int result);
#endif
...
static char g_lpuart1rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV8A_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_LPUART2_RXDMA
static char g_lpuart2rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV8A_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_LPUART3_RXDMA
static char g_lpuart3rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV8A_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_LPUART4_RXDMA
static char g_lpuart4rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV8A_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_LPUART5_RXDMA
static char g_lpuart5rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV8A_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_LPUART6_RXDMA
static char g_lpuart6rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV8A_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_LPUART7_RXDMA
static char g_lpuart7rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV8A_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_LPUART8_RXDMA
static char g_lpuart8rxfifo[RXDMA_BUFFER_SIZE] aligned_data(ARMV8A_DCACHE_LINESIZE);
#endif

#ifdef CONFIG_IMX9_LPUART1
static char g_lpuart1rxbuffer[CONFIG_LPUART1_RXBUFSIZE];
static char g_lpuart1txbuffer[LPUART1_TXBUFSIZE_ADJUSTED] LPUART1_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_IMX9_LPUART2
static char g_lpuart2rxbuffer[CONFIG_LPUART2_RXBUFSIZE];
static char g_lpuart2txbuffer[LPUART2_TXBUFSIZE_ADJUSTED] LPUART2_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_IMX9_LPUART3
static char g_lpuart3rxbuffer[CONFIG_LPUART3_RXBUFSIZE];
static char g_lpuart3txbuffer[LPUART3_TXBUFSIZE_ADJUSTED] LPUART3_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_IMX9_LPUART4
static char g_lpuart4rxbuffer[CONFIG_LPUART4_RXBUFSIZE];
static char g_lpuart4txbuffer[LPUART4_TXBUFSIZE_ADJUSTED] LPUART4_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_IMX9_LPUART5
static char g_lpuart5rxbuffer[CONFIG_LPUART5_RXBUFSIZE];
static char g_lpuart5txbuffer[LPUART5_TXBUFSIZE_ADJUSTED] LPUART5_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_IMX9_LPUART6
static char g_lpuart6rxbuffer[CONFIG_LPUART6_RXBUFSIZE];
static char g_lpuart6txbuffer[LPUART6_TXBUFSIZE_ADJUSTED] LPUART6_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_IMX9_LPUART7
static char g_lpuart7rxbuffer[CONFIG_LPUART7_RXBUFSIZE];
static char g_lpuart7txbuffer[LPUART7_TXBUFSIZE_ADJUSTED] LPUART7_TXBUFSIZE_ALGN;
#endif

#ifdef CONFIG_IMX9_LPUART8
static char g_lpuart8rxbuffer[CONFIG_LPUART8_RXBUFSIZE];
static char g_lpuart8txbuffer[LPUART8_TXBUFSIZE_ADJUSTED] LPUART8_TXBUFSIZE_ALGN;
#endif
...
static inline void imx9_disableuartint_nolock(struct imx9_uart_s *priv, uint32_t *ie)
{
	uint32_t regval;
...
static inline void imx9_restoreuartint_nolock(struct imx9_uart_s *priv, uint32_t ie)
{
	uint32_t regval;
...
static int imx9_dma_setup(struct uart_dev_s *dev)
{
	...
	if (priv->txch != 0) {
		...
		modifyreg32(priv->uartbase + IMX9_LPUART_BAUD_OFFSET, 0, LPUART_BAUD_TDMAE);
	}
#endif
	...
	if (priv->rxch != 0) {
		...
		config.flags = EDMA_CONFIG_LINKTYPE_LINKNONE | EDMA_CONFIG_LOOPDEST | EDMA_CONFIG_INTHALF |
		...
		up_invalidate_dcache((uintptr_t)priv->rxfifo, (uintptr_t)priv->rxfifo + RXDMA_BUFFER_SIZE);
		priv->rxdmaavail = 0;
#endif

		/* Enable receive Rx DMA for the UART */

		modifyreg32(priv->uartbase + IMX9_LPUART_BAUD_OFFSET, 0, LPUART_BAUD_RDMAE);
...
static int imx9_setup(struct uart_dev_s *dev)
{
	...
	priv->ie = imx9_serialin(priv, IMX9_LPUART_CTRL_OFFSET) & LPUART_ALL_INTS;
	return ret;
...
static int imx9_interrupt(int irq, void *context, void *arg)
{
	...
	lsr = usr &
		  ~(LPUART_STAT_LBKDIF | LPUART_STAT_RXEDGIF | LPUART_STAT_IDLE | LPUART_STAT_OR |
			LPUART_STAT_NF | LPUART_STAT_FE | LPUART_STAT_PF | LPUART_STAT_MA1F | LPUART_STAT_MA2F);
	...
	if ((usr & LPUART_STAT_OR) != 0) {
		imx9_serialout(priv, IMX9_LPUART_STAT_OFFSET, LPUART_STAT_OR | lsr);
	}

	if ((usr & LPUART_STAT_NF) != 0) {
		imx9_serialout(priv, IMX9_LPUART_STAT_OFFSET, LPUART_STAT_NF | lsr);
	}

	if ((usr & LPUART_STAT_PF) != 0) {
		imx9_serialout(priv, IMX9_LPUART_STAT_OFFSET, LPUART_STAT_PF | lsr);
	}

	if ((usr & LPUART_STAT_FE) != 0) {
		imx9_serialout(priv, IMX9_LPUART_STAT_OFFSET, LPUART_STAT_FE | lsr);
	}
	...
	if ((usr & LPUART_STAT_IDLE) != 0) {
		imx9_serialout(priv, IMX9_LPUART_STAT_OFFSET, LPUART_STAT_IDLE | lsr);
...
static int imx9_ioctl(struct file *filep, int cmd, unsigned long arg)
{
	...
	defined(CONFIG_IMX9_LPUART_SINGLEWIRE) || defined(CONFIG_IMX9_LPUART_INVERT)
	...
	case TIOCSINVERT: {
		...
		if ((arg & SER_INVERT_ENABLED_TX) && ((ctrl & LPUART_CTRL_LOOPS) != LPUART_CTRL_LOOPS)) {
...
static bool imx9_txready(struct uart_dev_s *dev)
{
	...
	fifo_size = (regval & LPUART_FIFO_TXFIFOSIZE_MASK) >> LPUART_FIFO_TXFIFOSIZE_SHIFT;
	fifo_size = fifo_size == 0 ? 1 : (1 << (fifo_size + 1));
	regval = imx9_serialin(priv, IMX9_LPUART_WATER_OFFSET);
	fifo_count = (regval & LPUART_WATER_TXCOUNT_MASK) >> LPUART_WATER_TXCOUNT_SHIFT;
...
static void imx9_dma_rxcallback(DMACH_HANDLE handle, void *arg, bool done, int result)
{
	...
	if ((sr & (LPUART_STAT_OR | LPUART_STAT_NF | LPUART_STAT_FE)) != 0) {
		imx9_serialout(priv, IMX9_LPUART_STAT_OFFSET,
					   sr & (LPUART_STAT_OR | LPUART_STAT_NF | LPUART_STAT_FE));
	}
}
#endif
