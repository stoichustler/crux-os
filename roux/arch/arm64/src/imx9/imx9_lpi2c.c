...
#if !defined(CONFIG_IMX9_LPI2C_TIMEOSEC) && !defined(CONFIG_IMX9_LPI2C_TIMEOMS)
...
#define CONFIG_IMX9_LPI2C_TIMEOTICKS                                                               \
	(SEC2TICK(CONFIG_IMX9_LPI2C_TIMEOSEC) + MSEC2TICK(CONFIG_IMX9_LPI2C_TIMEOMS))
#endif

#ifndef CONFIG_IMX9_LPI2C_DYNTIMEO_STARTSTOP
#define CONFIG_IMX9_LPI2C_DYNTIMEO_STARTSTOP TICK2USEC(CONFIG_IMX9_LPI2C_TIMEOTICKS)
#endif
...
static inline uint32_t imx9_lpi2c_getreg(struct imx9_lpi2c_priv_s *priv, uint16_t offset);
static inline void imx9_lpi2c_putreg(struct imx9_lpi2c_priv_s *priv, uint16_t offset,
									 uint32_t value);
static inline void imx9_lpi2c_modifyreg(struct imx9_lpi2c_priv_s *priv, uint16_t offset,
...
static inline int imx9_lpi2c_sem_waitdone(struct imx9_lpi2c_priv_s *priv);

#ifdef CONFIG_I2C_TRACE
static void imx9_lpi2c_tracereset(struct imx9_lpi2c_priv_s *priv);
static void imx9_lpi2c_tracenew(struct imx9_lpi2c_priv_s *priv, uint32_t status);
static void imx9_lpi2c_traceevent(struct imx9_lpi2c_priv_s *priv, enum imx9_trace_e event,
								  uint32_t parm);
static void imx9_lpi2c_tracedump(struct imx9_lpi2c_priv_s *priv);
#endif /* CONFIG_I2C_TRACE */

static void imx9_lpi2c_setclock(struct imx9_lpi2c_priv_s *priv, uint32_t frequency);
static inline void imx9_lpi2c_sendstart(struct imx9_lpi2c_priv_s *priv, uint8_t address);
static inline void imx9_lpi2c_sendstop(struct imx9_lpi2c_priv_s *priv);
static inline uint32_t imx9_lpi2c_getstatus(struct imx9_lpi2c_priv_s *priv);
...
static int imx9_lpi2c_transfer(struct i2c_master_s *dev, struct i2c_msg_s *msgs, int count);
#ifdef CONFIG_I2C_RESET
static int imx9_lpi2c_reset(struct i2c_master_s *dev);
#endif

#ifdef CONFIG_IMX9_LPI2C_DMA
static void imx9_rxdma_callback(DMACH_HANDLE handle, void *arg, bool done, int result);
static void imx9_txdma_callback(DMACH_HANDLE handle, void *arg, bool done, int result);
#endif
...
#ifdef CONFIG_I2C_TRACE
static const char *g_trace_names[] = {"NONE           ", "SENDADDR       ", "SENDBYTE       ",
									  "RCVBYTE        ", "NOSTART        ", "START/RESTART  ",
									  "STOP           ", "ERROR          "};
#endif
...
static struct imx9_lpi2c_priv_s imx9_lpi2c1_priv = {.ops = &imx9_lpi2c_ops,
...
static struct imx9_lpi2c_priv_s imx9_lpi2c2_priv = {.ops = &imx9_lpi2c_ops,
...
static struct imx9_lpi2c_priv_s imx9_lpi2c3_priv = {.ops = &imx9_lpi2c_ops,
...
static struct imx9_lpi2c_priv_s imx9_lpi2c4_priv = {.ops = &imx9_lpi2c_ops,
...
static struct imx9_lpi2c_priv_s imx9_lpi2c5_priv = {.ops = &imx9_lpi2c_ops,
...
static struct imx9_lpi2c_priv_s imx9_lpi2c6_priv = {.ops = &imx9_lpi2c_ops,
...
static struct imx9_lpi2c_priv_s imx9_lpi2c7_priv = {.ops = &imx9_lpi2c_ops,
...
static struct imx9_lpi2c_priv_s imx9_lpi2c8_priv = {.ops = &imx9_lpi2c_ops,
...
#ifdef CONFIG_IMX9_LPI2C_DYNTIMEO
	ret =
		nxsem_tickwait_uninterruptible(&priv->sem_isr, imx9_lpi2c_toticks(priv->msgc, priv->msgv));
#else
	ret = nxsem_tickwait_uninterruptible(&priv->sem_isr, CONFIG_IMX9_LPI2C_TIMEOTICKS);
#endif
...
static inline int imx9_lpi2c_sem_waitdone(struct imx9_lpi2c_priv_s *priv)
{
	...
	i2cinfo("intstate: %d elapsed: %ld threshold: %ld status: %08x\n", priv->intstate,
			(long)elapsed, (long)timeout, priv->status);
...
static void imx9_rxdma_callback(DMACH_HANDLE handle, void *arg, bool done, int result)
{
	...
	imx9_lpi2c_modifyreg(priv, IMX9_LPI2C_MIER_OFFSET, 0, LPI2C_MIER_SDIE);
...
static void imx9_lpi2c_setclock(struct imx9_lpi2c_priv_s *priv, uint32_t frequency)
{
	...
	if (priv->config->mode == LPI2C_MASTER) {
		if (frequency != priv->frequency) {
			...
			men = imx9_lpi2c_getreg(priv, IMX9_LPI2C_MCR_OFFSET) & LPI2C_MCR_MEN;
			if (men) {
			...
			for (prescale = 1; (prescale <= 128) && (best_error != 0); prescale *= 2) {
				for (clk_hi_cycle = 1; clk_hi_cycle < 32; clk_hi_cycle++) {
					if (clk_hi_cycle == 1) {
						...
						computed_rate =
							(src_freq / prescale) / ((3 * clk_hi_cycle + 2) + (2 / prescale));
					}
			...
			imx9_lpi2c_modifyreg(priv, IMX9_LPI2C_MCFGR1_OFFSET, LPI2C_MCFGR1_PRESCALE_MASK,
...
static inline void imx9_lpi2c_sendstart(struct imx9_lpi2c_priv_s *priv, uint8_t address)
{
	...
	do {
		txcount = (imx9_lpi2c_getreg(priv, IMX9_LPI2C_MFSR_OFFSET) & LPI2C_MFSR_TXCOUNT_MASK) >>
	...
	imx9_lpi2c_putreg(priv, IMX9_LPI2C_MTDR_OFFSET, (LPI2C_MTDR_CMD_START | LPI2C_MTDR_DATA(addr)));
}
...
static inline uint32_t imx9_lpi2c_getenabledints(struct imx9_lpi2c_priv_s *priv)
{
...
{
	uint32_t irq_config =
		(LPI2C_MIER_EPIE | LPI2C_MIER_SDIE | LPI2C_MIER_NDIE | LPI2C_MIER_ALIE | LPI2C_MIER_FEIE);
	...
	imx9_lpi2c_modifyreg(priv, IMX9_LPI2C_MCFGR0_OFFSET, LPI2C_MCFG0_ABORT, 0);

	/* Send start + address unless M_NOSTART is defined */

	if ((priv->flags & I2C_M_NOSTART) == 0) {
		imx9_lpi2c_traceevent(priv, I2CEVENT_STARTRESTART, priv->msgc);
...
static int imx9_lpi2c_stop_transfer(struct imx9_lpi2c_priv_s *priv)
{
	...
	imx9_lpi2c_modifyreg(priv, IMX9_LPI2C_MIER_OFFSET,
						 LPI2C_MIER_TDIE | LPI2C_MIER_RDIE | LPI2C_MIER_NDIE | LPI2C_MIER_ALIE |
							 LPI2C_MIER_SDIE | LPI2C_MIER_EPIE,
						 0);
...
static int imx9_lpi2c_isr_process(struct imx9_lpi2c_priv_s *priv)
{
	...
	if (priv->rxdma != NULL || priv->txdma != NULL) {
		...
		if ((status & (LPI2C_MSR_SDF | LPI2C_MSR_EPF)) != 0) {
			...
			imx9_lpi2c_putreg(priv, IMX9_LPI2C_MSR_OFFSET,
							  status & (LPI2C_MSR_SDF | LPI2C_MSR_EPF));
	...
	if ((status & (LPI2C_MSR_RDF | LPI2C_MSR_NDF)) == (LPI2C_MSR_RDF | LPI2C_MSR_NDF) &&
		priv->dcnt == 1) {
	...
	if ((status & LPI2C_MSR_ERROR_MASK) != 0) {
		...
		imx9_lpi2c_modifyreg(priv, IMX9_LPI2C_MCR_OFFSET, 0, LPI2C_MCR_RTF | LPI2C_MCR_RRF);
		...
		if ((status & LPI2C_MSR_SDF) == 0) {
			...
			imx9_lpi2c_modifyreg(priv, IMX9_LPI2C_MIER_OFFSET, LPI2C_MIER_TDIE | LPI2C_MIER_TDIE,
								 0);

			/* Abort any ongoing transfer, this also sends stop on the bus */

			imx9_lpi2c_modifyreg(priv, IMX9_LPI2C_MCFGR0_OFFSET, 0, LPI2C_MCFG0_ABORT);
		} else {
	...
	if (status & (LPI2C_MSR_EPF | LPI2C_MSR_SDF)) {
		...
		imx9_lpi2c_putreg(priv, IMX9_LPI2C_MSR_OFFSET, status & (LPI2C_MSR_EPF | LPI2C_MSR_SDF));
	...
	/* Check if there are received bytes */
		...
		*priv->ptr++ = imx9_lpi2c_getreg(priv, IMX9_LPI2C_MRDR_OFFSET) & LPI2C_MRDR_DATA_MASK;
...
static int imx9_lpi2c_init(struct imx9_lpi2c_priv_s *priv)
{
	...
	imx9_lpi2c_modifyreg(priv, IMX9_LPI2C_MCFGR0_OFFSET, LPI2C_MCFG0_HREN | LPI2C_MCFG0_HRSEL,
						 LPI2C_MCFG0_HRPOL);
	...
	imx9_lpi2c_putreg(priv, IMX9_LPI2C_MFCR_OFFSET, LPI2C_MFCR_TXWATER(0) | LPI2C_MFCR_RXWATER(0));
	...
	imx9_lpi2c_putreg(priv, IMX9_LPI2C_MCFGR3_OFFSET, LPI2C_MCFG3_PINLOW_CYCLES(0));
...
#ifdef CONFIG_IMX9_LPI2C_DMA
static int imx9_lpi2c_dma_command_configure(struct imx9_lpi2c_priv_s *priv, uint16_t *ccmd,
											uint32_t ncmd)
{
...
#ifdef CONFIG_IMX9_LPI2C_DMA
static int imx9_lpi2c_dma_data_configure(struct imx9_lpi2c_priv_s *priv, struct i2c_msg_s *msg)
{
	DMACH_HANDLE dma;
	struct imx9_edma_xfrconfig_s config;
	memset(&config, 0, sizeof(config));
...
#ifdef CONFIG_IMX9_LPI2C_DMA
static int imx9_lpi2c_form_command_list(struct imx9_lpi2c_priv_s *priv, struct i2c_msg_s *msg,
										int ncmds)
{
	ssize_t length = 0;
	...
	if (priv->flags & I2C_M_NOSTART) {
		...
		uint16_t badd =
			(priv->flags & I2C_M_READ) ? I2C_READADDR8(msg->addr) : I2C_WRITEADDR8(msg->addr);
...
static int imx9_lpi2c_dma_transfer(struct imx9_lpi2c_priv_s *priv)
{
	...
	imx9_lpi2c_modifyreg(priv, IMX9_LPI2C_MDER_OFFSET, LPI2C_MDER_TDDE | LPI2C_MDER_RDDE, 0);
	...
	imx9_lpi2c_putreg(priv, IMX9_LPI2C_MSR_OFFSET, LPI2C_MSR_NDF | LPI2C_MSR_ALF | LPI2C_MSR_FEF);

	/* Enable the Interrupts */

	imx9_lpi2c_putreg(priv, IMX9_LPI2C_MIER_OFFSET,
					  LPI2C_MIER_NDIE | LPI2C_MIER_ALIE | LPI2C_MIER_PLTIE);
	...
	imx9_lpi2c_modifyreg(priv, IMX9_LPI2C_MDER_OFFSET, 0, LPI2C_MDER_TDDE | LPI2C_MDER_RDDE);
	return OK;
}
#endif
