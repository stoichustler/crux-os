...
static inline void pwm_putreg(struct imx9_pwmtimer_s *priv, int offset, uint32_t value)
{
	...
	if (freq != priv->frequency) {
		...
		if (freq > 0 && priv->frequency == 0) {
			...
			for (i = 0; i < priv->n_channels; i++) {
				pwm_putreg(priv, IMX9_TPM_CXSC_OFFSET(CHMUX(priv, i)), TPM_CXSC_MSB_MASK);
			}
		}

		priv->period = freq > 0 ? priv->clk / freq : 0;

		pwminfo("PWM%d frequency: %" PRIu32 " period: %" PRIu32 "\n", priv->id, freq, priv->period);
...
static int pwm_update_duty(struct imx9_pwmtimer_s *priv, int pwm_ch, ub16_t duty16)
{
	...
	pwminfo("PWM%d channel %d, p: %d e: %" PRIu32 "\n", priv->id, CHMUX(priv, timer), priv->period,
			edge);
...
static int pwm_start(struct pwm_lowerhalf_s *dev, const struct pwm_info_s *info)
{
	...
	if (pwm_update_frequency(priv, info->frequency) == OK) {
		...
		for (i = 0; i < CONFIG_PWM_NCHANNELS; i++) {
			...
			pwm_update_duty(priv, info->channels[i].channel, info->channels[i].duty);
		}
	}
