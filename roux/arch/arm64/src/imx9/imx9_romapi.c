...
struct romapi_s {
	...
	uint16_t tag;
	uint32_t reserved1;
	uint32_t (*load_image)(uint8_t *dest, uint32_t offset, uint32_t size, uint32_t xor);
	uint32_t (*query_boot_infor)(uint32_t infor_type, uint32_t *infor, uint32_t xor);
};
...
uint32_t imx9_romapi_load_image(uint8_t *dest, uint32_t offset, uint32_t size)
{
	uint32_t xor ;
	uint32_t ret;
...
int64_t imx9_romapi_get_seq_cntr_base(uint32_t image_offset, uint32_t pagesize, uint8_t cntr_idx)
{
	uint32_t size;
	uint32_t ret;
	uint16_t cntr_size;
	...
	for (uint8_t i = 0; i < cntr_idx; i++) {
		ret = imx9_romapi_load_image((uint8_t *)CONFIG_IMX_AHAB_CNTR_ADDR, image_offset, size);
		if (ret != ROMAPI_OK) {
		...
		image_offset = ALIGN_UP((cntr_size + image_offset), CONTAINER_HDR_ALIGNMENT);
	}
