...
static int pll_init(uintptr_t reg, bool frac, struct pll_parms_s *parm)
{
	...
	val = PLL_DIV_RDIV(parm->rdiv) | PLL_DIV_MFI(parm->mfi) | PLL_DIV_ODIV(parm->odiv);
...
static int pll_pfd_init(uintptr_t reg, int pfd, struct pfd_parms_s *pfdparm)
{
	...
	putreg32(PLL_DFS_CLKOUT_EN | PLL_DFS_CLKOUT_DIVBY2_EN | PLL_DFS_ENABLE, PLL_CLR(ctrl));
...
static uint32_t calculate_vco_freq(const struct pll_parms_s *parm, bool frac)
{
	...
	if (frac) {
		return (uint64_t)XTAL_FREQ * (parm->mfi * parm->mfd + parm->mfn) / parm->mfd / parm->rdiv;
	} else {
...
static void imx9_dram_disable_bypass(void)
{
	...
	imx9_ccm_shared_gpr_set(CCM_SHARED_GPR_DRAM_CLK, CCM_SHARED_GPR_DRAM_CLK_SEL_PLL);
}
#endif
