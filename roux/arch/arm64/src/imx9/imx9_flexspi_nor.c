...
static const uint32_t g_flexspi_nor_lut[][4] = {
	[READ_ID] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_RDID, FLEXSPI_COMMAND_READ_SDR,
							FLEXSPI_1PAD, 0x04),
		},

	[READ_STATUS_REG] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_RDSR, FLEXSPI_COMMAND_READ_SDR,
							FLEXSPI_1PAD, 0x04),
		},

	[READ_FLAG_STATUS_REG] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_RFSR, FLEXSPI_COMMAND_READ_SDR,
							FLEXSPI_1PAD, 0x04),
		},

	[WRITE_STATUS_REG] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_WRSR, FLEXSPI_COMMAND_WRITE_SDR,
							FLEXSPI_1PAD, 0x04),
		},

	[WRITE_ENABLE] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_WREN, FLEXSPI_COMMAND_STOP,
							FLEXSPI_1PAD, 0),
		},

	[ERASE_SECTOR] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD,
							M25P_SSE, /* note: sub-sector erase */
							FLEXSPI_COMMAND_RADDR_SDR, FLEXSPI_1PAD, ADDRESS_32BIT),
		},

	[ERASE_CHIP] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_BE, FLEXSPI_COMMAND_STOP,
							FLEXSPI_1PAD, 0),
		},
	...
	[READ_FAST_QUAD_OUTPUT] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_Q_FAST_RD,
							FLEXSPI_COMMAND_RADDR_SDR, FLEXSPI_1PAD, ADDRESS_32BIT),
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_DUMMY_SDR, FLEXSPI_4PAD, 0x08, FLEXSPI_COMMAND_READ_SDR,
							FLEXSPI_4PAD, 0x04),
		},
	...
	[PAGE_PROGRAM_QUAD_INPUT] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_Q_FAST_PP,
							FLEXSPI_COMMAND_RADDR_SDR, FLEXSPI_1PAD, ADDRESS_32BIT),
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_WRITE_SDR, FLEXSPI_4PAD, 0x04, FLEXSPI_COMMAND_STOP,
							FLEXSPI_1PAD, 0),
		},

	[ENTER_4BYTE] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_4B_ENTER, FLEXSPI_COMMAND_STOP,
							FLEXSPI_1PAD, 0),
		},

	[ENTER_QPI] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_Q_ENTER, FLEXSPI_COMMAND_STOP,
							FLEXSPI_1PAD, 0),
		},

	[ENTER_DDR] =
		{
			FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_WECR, FLEXSPI_COMMAND_WRITE_SDR,
							FLEXSPI_1PAD, 0x8),
		},

	[READ_FAST] = {FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR, FLEXSPI_1PAD, M25P_FAST_READ,
								   FLEXSPI_COMMAND_RADDR_SDR, FLEXSPI_1PAD, ADDRESS_32BIT),
...
static int imx9_flexspi_nor_erase(struct mtd_dev_s *dev, off_t startblock, size_t nblocks);
static ssize_t imx9_flexspi_nor_read(struct mtd_dev_s *dev, off_t offset, size_t nbytes,
									 uint8_t *buffer);
static ssize_t imx9_flexspi_nor_bread(struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
									  uint8_t *buffer);
static ssize_t imx9_flexspi_nor_bwrite(struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
									   const uint8_t *buffer);
static int imx9_flexspi_nor_ioctl(struct mtd_dev_s *dev, int cmd, unsigned long arg);
...
static struct imx9_flexspi_nor_dev_s g_flexspi_nor = {.mtd = {.erase = imx9_flexspi_nor_erase,
													  ...
													  .flexspi = (void *)0,
													  .ahb_base =
														  (uint8_t *)CONFIG_FSPI_PER_BASEADDR,
...
static int imx9_flexspi_nor_get_id(struct imx9_flexspi_nor_dev_s *priv)
{
...
static int imx9_flexspi_nor_read_register(const struct imx9_flexspi_nor_dev_s *dev, uint32_t *value,
										  uint32_t len, int seq)
{
	int stat;
...
static int imx9_flexspi_nor_write_cmd(const struct imx9_flexspi_nor_dev_s *dev, int cmd)
{
	int stat;
...
static int imx9_flexspi_nor_erase_sector(const struct imx9_flexspi_nor_dev_s *dev, off_t offset)
{
	int stat;
...
static int imx9_flexspi_nor_page_program(const struct imx9_flexspi_nor_dev_s *dev, off_t offset,
...
static int imx9_flexspi_nor_wait_bus_busy(const struct imx9_flexspi_nor_dev_s *dev)
{
	uint32_t status = 0;
	int ret;
...
static ssize_t imx9_flexspi_nor_read(struct mtd_dev_s *dev, off_t offset, size_t nbytes,
									 uint8_t *buffer)
{
	struct imx9_flexspi_nor_dev_s *priv = (struct imx9_flexspi_nor_dev_s *)dev;
	uint8_t *src;
									   ...
									   const uint8_t *src)
{
	...
	up_clean_dcache((uintptr_t)src, (uintptr_t)src + ALIGN_UP(len, ARMV8A_DCACHE_LINESIZE));
...
static int imx9_flexspi_nor_erase(struct mtd_dev_s *dev, off_t startblock, size_t nblocks)
{
	...
	while (blocksleft-- > 0) {
		...
		imx9_flexspi_nor_write_cmd(priv, WRITE_ENABLE);
		imx9_flexspi_nor_erase_sector(priv, startblock << priv->subsectorshift);
		imx9_flexspi_nor_wait_bus_busy(priv);
		FLEXSPI_SOFTWARE_RESET(priv->flexspi);
		startblock++;
	}
	...
	case MTDIOC_GEOMETRY: {
		...
		if (geo) {
			...
			if (priv->subsectorshift > 0) {
				...
				geo->neraseblocks =
					priv->nsectors * (1 << (priv->sectorshift - priv->subsectorshift));
			} else
#endif
			{
...
struct mtd_dev_s *imx9_flexspi_nor_initialize(int intf)
{
	...
	FLEXSPI_SET_DEVICE_CONFIG(g_flexspi_nor.flexspi, g_flexspi_nor.config, g_flexspi_nor.port);
	FLEXSPI_UPDATE_LUT(g_flexspi_nor.flexspi, 0, (const uint32_t *)g_flexspi_nor_lut,
	...
	imx9_flexspi_nor_read_register(&g_flexspi_nor, &val, 1, READ_FLAG_STATUS_REG);
