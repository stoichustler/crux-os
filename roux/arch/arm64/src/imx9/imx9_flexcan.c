...
static uint32_t imx9_bitratetotimeseg(struct imx9_driver_s *priv, struct flexcan_timeseg *timeseg,
									  bool can_fd_data)
{
#if defined(CONFIG_NET_CAN_CANFD)
	...
	const uint32_t PRESDIVMAX = can_fd_data ? CAN_FDCBT_FPRESDIV_MASK >> CAN_FDCBT_FPRESDIV_SHIFT
											: CAN_CBT_EPRESDIV_MASK >> CAN_CBT_EPRESDIV_SHIFT;

	/* Max register field values from PSEG and PROPSEG */

	const int32_t PSEGMAX = can_fd_data ? CAN_FDCBT_FPSEG1_MASK >> CAN_FDCBT_FPSEG1_SHIFT
...
static bool imx9_waitmcr_change(uint32_t base, uint32_t mask, bool target_state);
static volatile struct mb_s *flexcan_get_mb(struct imx9_driver_s *priv, int mbi);
...
static int imx9_flexcan_interrupt(int irq, void *context, void *arg);
...
static int imx9_transmit(struct imx9_driver_s *priv)
{
	...
	if (priv->dev.d_sndlen > priv->dev.d_len) {
		...
		if (CONFIG_NET_CAN_RAW_DEFAULT_TX_DEADLINE > 0) {
			...
			priv->txmb[txmb].deadline.tv_sec =
				ts.tv_sec + CONFIG_NET_CAN_RAW_DEFAULT_TX_DEADLINE / 1000000;
			priv->txmb[txmb].deadline.tv_usec =
				(ts.tv_nsec / 1000) + CONFIG_NET_CAN_RAW_DEFAULT_TX_DEADLINE % 1000000;
		} else {
	...
	if (can_id & CAN_EFF_FLAG) {
		...
		mb->id = ((can_id & CAN_SFF_MASK) << CAN_MB_ID_ID_STD_SHIFT) & CAN_MB_ID_ID_STD_MASK;
	}
	...
	if (timeout > 0) {
		wd_start(&priv->txtimeout[txmb], timeout + 1, imx9_txtimeout_expiry, (wdparm_t)priv);
	}
#endif
...
static int imx9_flexcan_interrupt(int irq, void *context, void *arg)
{
	...
	if (irq == priv->irq) {
		...
		if (flags & IFLAG1_RX) {
			...
			modifyreg32(priv->base + IMX9_CAN_IMASK1_OFFSET, flags & IFLAG1_RX, 0);
		}
		...
		putreg32(esr1 & (CAN_ESR1_PTA | CAN_ESR1_ATP | CAN_ESR1_ERROVR | CAN_ESR1_ERRINTFAST |
						 CAN_ESR1_BOFFDONEINT | CAN_ESR1_TWRNINT | CAN_ESR1_RWRNINT |
...
static int imx9_initialize(struct imx9_driver_s *priv)
{
	...
	modifyreg32(priv->base + IMX9_CAN_MCR_OFFSET, CAN_MCR_MAXMB_MASK,
				CAN_MCR_SLFWAK | CAN_MCR_WRNEN | CAN_MCR_WAKSRC | CAN_MCR_IRMQ | CAN_MCR_LPRIOEN |
					CAN_MCR_AEN |
	...
	if (!priv->canfd_capable) {
		modifyreg32(priv->base + IMX9_CAN_CTRL1_OFFSET,
					CAN_CTRL1_PRESDIV_MASK | CAN_CTRL1_PROPSEG_MASK | CAN_CTRL1_PSEG1_MASK |
						CAN_CTRL1_PSEG2_MASK | CAN_CTRL1_RJW_MASK,
		...
		modifyreg32(priv->base + IMX9_CAN_CBT_OFFSET,
					CAN_CBT_EPRESDIV_MASK | CAN_CBT_EPROPSEG_MASK | CAN_CBT_EPSEG1_MASK |
						CAN_CBT_EPSEG2_MASK | CAN_CBT_ERJW_MASK,
		...
		modifyreg32(
			priv->base + IMX9_CAN_FDCBT_OFFSET,
			CAN_FDCBT_FPRESDIV_MASK | CAN_FDCBT_FPROPSEG_MASK | CAN_FDCBT_FPSEG1_MASK |
				CAN_FDCBT_FPSEG2_MASK | CAN_FDCBT_FRJW_MASK,
			CAN_FDCBT_FPRESDIV(priv->data_timing.presdiv) | /* Prescaler divisor factor of 1 */
				CAN_FDCBT_FPROPSEG(
					priv->data_timing.propseg) |			/* Propagation
				...
				CAN_FDCBT_FRJW(
					priv->data_timing.pseg2)); /* Resynchorinzation jump width same as PSEG2 */
		...
		modifyreg32(
			priv->base + IMX9_CAN_FDCTRL_OFFSET, 0,
		...
		modifyreg32(priv->base + IMX9_CAN_CTRL2_OFFSET, 0, CAN_CTRL2_ISOCANFDEN);
	}
	...
	modifyreg32(priv->base + IMX9_CAN_CTRL2_OFFSET, CAN_CTRL2_RETRY_MASK,
				CAN_CTRL2_RRS | CAN_CTRL2_EACEN | 3 << CAN_CTRL2_RETRY_SHIFT);
