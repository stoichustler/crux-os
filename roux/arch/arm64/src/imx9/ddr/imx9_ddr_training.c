...
#define imx9_ddrphy_write(data, addr) putreg32(data, ddrphy_to_axi_addr_remap(addr))
#define imx9_ddrphy_read(addr) getreg32(ddrphy_to_axi_addr_remap(addr))
...
static uint32_t array_max(uint32_t data[], uint32_t addr_start, uint32_t addr_end)
{
	uint32_t imax = 0;
	uint32_t i;
...
static void imx9_save_trained_ccd(void)
{
	...
	g_ccd_rr_max = ccd_ch_r_max[0] > ccd_ch_r_max[1] ? ccd_ch_r_max[0] : ccd_ch_r_max[1];
	g_ccd_rw_max = ccd_ch_r_max[2] > ccd_ch_r_max[3] ? ccd_ch_r_max[2] : ccd_ch_r_max[3];

	g_ccd_wr_max = ccd_ch_w_max[0] > ccd_ch_w_max[1] ? ccd_ch_w_max[0] : ccd_ch_w_max[1];
	g_ccd_ww_max = ccd_ch_w_max[2] > ccd_ch_w_max[3] ? ccd_ch_w_max[2] : ccd_ch_w_max[3];
}
...
static uint32_t ddrc_get_fsp_reg(struct dram_cfg_param *ddrc_cfg, uint32_t cfg_num, uint32_t reg)
{
	uint32_t i;
...
static void ddrc_update_fsp_reg(struct dram_cfg_param *ddrc_cfg, int cfg_num, uint32_t reg,
								uint32_t val)
{
	uint32_t i;
...
static void imx9_update_rank_space(struct dram_timing_info *dram_timing, uint32_t num_cfgs)
{
	uint32_t extra_wwt;
	uint32_t extra_rrt;
	uint32_t extra_wrt;
	...
	for (i = 0; i < num_cfgs; i++) {
		...
		tmp_0 = ddrc_get_fsp_reg(dram_timing->fsp_cfg[i].ddrc_cfg,
								 nitems(dram_timing->fsp_cfg[i].ddrc_cfg), IMX9_DDRC_TIMING_CFG_0);
		...
		tmp_4 = ddrc_get_fsp_reg(dram_timing->fsp_cfg[i].ddrc_cfg,
								 nitems(dram_timing->fsp_cfg[i].ddrc_cfg), IMX9_DDRC_TIMING_CFG_4);
		...
		tmp_4 &= 0xffff00ff;
		tmp_4 |= (extra_rwt << 14) | (extra_wrt << 12) | (extra_rrt << 10) | (extra_wwt << 8);
