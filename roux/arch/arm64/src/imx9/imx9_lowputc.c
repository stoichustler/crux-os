...
void imx9_lowsetup(void)
{
	...
	imx9_lpuart_configure(IMX9_CONSOLE_BASE, IMX9_CONSOLE_DEVNUM, &g_console_config);
#endif
...
int imx9_lpuart_configure(uint32_t base, int uartnum, const struct uart_config_s *config)
{
	...
	for (temp_osr = 4; temp_osr <= 32; temp_osr++) {
		...
		if (temp_diff > abs(calculated_baud - configured_baud)) {
			temp_diff = abs(calculated_baud - configured_baud);
			temp_sbr++;
		}
	...
	putreg32(LPUART_FIFO_RXFE | LPUART_FIFO_TXFE, base + IMX9_LPUART_FIFO_OFFSET);
...
{
#ifdef IMX9_CONSOLE_BASE
	while ((getreg32(IMX9_CONSOLE_BASE + IMX9_LPUART_STAT_OFFSET) & LPUART_STAT_TDRE) == 0) {
	}
	...
	if (ch == '\n') {
		...
		putreg32((uint32_t)'\r', IMX9_CONSOLE_BASE + IMX9_LPUART_DATA_OFFSET);
