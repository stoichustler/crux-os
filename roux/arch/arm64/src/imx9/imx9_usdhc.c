...
struct imx9_dev_s {
	...
	uint8_t rxbuffer[SDMMC_MAX_BLOCK_SIZE] __attribute__((aligned(ARMV8A_DCACHE_LINESIZE)));
...
static void imx9_dumpsample(struct imx9_dev_s *priv, struct imx9_sdhcregs_s *regs, const char *msg);
...
static void imx9_dataconfig(struct imx9_dev_s *priv, bool bwrite, unsigned int datalen,
...
static void imx9_endwait(struct imx9_dev_s *priv, sdio_eventset_t wkupevent);
static void imx9_endtransfer(struct imx9_dev_s *priv, sdio_eventset_t wkupevent);
...
static int imx9_sendcmd(struct sdio_dev_s *dev, uint32_t cmd, uint32_t arg);

#ifdef CONFIG_SDIO_BLOCKSETUP
static void imx9_blocksetup(struct sdio_dev_s *dev, unsigned int blocklen, unsigned int nblocks);
#endif

#ifndef CONFIG_IMX9_USDHC_DMA
static int imx9_recvsetup(struct sdio_dev_s *dev, uint8_t *buffer, size_t nbytes);
static int imx9_sendsetup(struct sdio_dev_s *dev, const uint8_t *buffer, size_t nbytes);
#endif

static int imx9_cancel(struct sdio_dev_s *dev);
static int imx9_waitresponse(struct sdio_dev_s *dev, uint32_t cmd);
static int imx9_recvshortcrc(struct sdio_dev_s *dev, uint32_t cmd, uint32_t *rshort);
static int imx9_recvlong(struct sdio_dev_s *dev, uint32_t cmd, uint32_t rlong[4]);
static int imx9_recvshort(struct sdio_dev_s *dev, uint32_t cmd, uint32_t *rshort);

/* EVENT handler */

static void imx9_waitenable(struct sdio_dev_s *dev, sdio_eventset_t eventset, uint32_t timeout);
static sdio_eventset_t imx9_eventwait(struct sdio_dev_s *dev);
static void imx9_callbackenable(struct sdio_dev_s *dev, sdio_eventset_t eventset);
static int imx9_registercallback(struct sdio_dev_s *dev, worker_t callback, void *arg);

/* DMA */

#ifdef CONFIG_IMX9_USDHC_DMA
#if defined(CONFIG_ARCH_HAVE_SDIO_PREFLIGHT)
static int imx9_dmapreflight(struct sdio_dev_s *dev, const uint8_t *buffer, size_t buflen);
#endif
static int imx9_dmarecvsetup(struct sdio_dev_s *dev, uint8_t *buffer, size_t buflen);
static int imx9_dmasendsetup(struct sdio_dev_s *dev, const uint8_t *buffer, size_t buflen);
#endif
...
static void imx9_sampleinit(void)
{
	memset(g_sampleregs, 0xff, DEBUG_NSAMPLES * sizeof(struct imx9_sdhcregs_s));
}
#endif
...
static void imx9_dumpsamples(struct imx9_dev_s *priv)
{
	imx9_dumpsample(priv, &g_sampleregs[SAMPLENDX_BEFORE_SETUP], "Before setup");
	imx9_dumpsample(priv, &g_sampleregs[SAMPLENDX_AFTER_SETUP], "After setup");
	imx9_dumpsample(priv, &g_sampleregs[SAMPLENDX_END_TRANSFER], "End of transfer");
}
#endif
							...
							unsigned int timeout)
{
	...
	if (bwrite) {
		...
		putreg32(watermark << USDHC_WML_WR_SHIFT, priv->addr + IMX9_USDHC_WML_OFFSET);
	} else {
		...
		putreg32(watermark << USDHC_WML_RD_SHIFT, priv->addr + IMX9_USDHC_WML_OFFSET);
	}
}
...
static void imx9_transmit(struct imx9_dev_s *priv)
{
	...
	while (priv->remaining > 0 &&
		   (getreg32(priv->addr + IMX9_USDHC_PRSSTAT_OFFSET) & USDHC_PRSSTAT_BWEN) != 0) {
...
static void imx9_receive(struct imx9_dev_s *priv)
{
	...
	while (priv->remaining > 0 &&
		   (getreg32(priv->addr + IMX9_USDHC_IRQSTAT_OFFSET) & USDHC_INT_BRR) != 0) {
...
static void imx9_recvdma(struct imx9_dev_s *priv)
{
	...
	if (priv->unaligned_rx) {
		...
		up_invalidate_dcache((uintptr_t)priv->buffer, (uintptr_t)priv->buffer + priv->remaining);
		priv->remaining = 0;
	}
...
static void imx9_endtransfer(struct imx9_dev_s *priv, sdio_eventset_t wkupevent)
{
	...
	putreg32(USDHC_XFRDONE_INTS | USDHC_DMADONE_INTS, priv->addr + IMX9_USDHC_IRQSTAT_OFFSET);
...
static int imx9_interrupt(int irq, void *context, void *arg)
{
	...
	mcinfo("IRQSTAT: %08" PRIx32 " IRQSIGEN %08" PRIx32 " enabled: %08" PRIx32 "\n",
...
static void imx9_reset(struct sdio_dev_s *dev)
{
	...
	modifyreg32(priv->addr + IMX9_USDHC_VENDOR2_OFFSET, USDHC_VS2_BUSRESET | USDHC_VS2_ACMD23ARGU2,
				0);
	...
	while ((getreg32(priv->addr + IMX9_USDHC_SYSCTL_OFFSET) & USDHC_SYSCTL_RSTA) != 0) {
	}
	...
	mcinfo("SYSCTL: %08" PRIx32 " PRSSTAT: %08" PRIx32 " IRQSTATEN: %08" PRIx32 "\n",
...
static sdio_statset_t imx9_status(struct sdio_dev_s *dev)
{
	...
	if (priv->sw_cd_gpio != 0) {
		...
		present = ((getreg32(priv->addr + IMX9_USDHC_PRSSTAT_OFFSET) & USDHC_PRSSTAT_CINS) != 0) ^
				  priv->cd_invert;
	}
...
static void imx9_frequency(struct sdio_dev_s *dev, uint32_t frequency)
{
	...
	regval |= (USDHC_SYSCTL_SDCLKEN | USDHC_SYSCTL_PEREN | USDHC_SYSCTL_HCKEN | USDHC_SYSCTL_IPGEN);
...
static void imx9_clock(struct sdio_dev_s *dev, enum sdio_clock_e rate)
{
	...
	case CLOCK_IDMODE: {
		...
		regval |= (BOARD_USDHC_IDMODE_PRESCALER | BOARD_USDHC_IDMODE_DIVISOR | USDHC_SYSCTL_INITA);
	} break;
	...
	while ((getreg32(priv->addr + IMX9_USDHC_PRSSTAT_OFFSET) & USDHC_PRSSTAT_SDSTB) == 0) {
	}
...
static int imx9_attach(struct sdio_dev_s *dev)
{
	...
	if (priv->addr == IMX9_USDHC1_BASE) {
		...
		if (priv->addr == IMX9_USDHC2_BASE) {
			ret = irq_attach(IMX9_IRQ_USDHC2, imx9_interrupt, &g_sdhcdev[usdhc2_dev]);
		} else {
			PANIC();
		}
	}
...
static int imx9_sendcmd(struct sdio_dev_s *dev, uint32_t cmd, uint32_t arg)
{
	...
	if ((cmd & (MMCSD_MULTIBLOCK | SDIO_MULTIBLOCK)) != 0) {
		...
		if (((cmd & MMCSD_MULTIBLOCK) != 0) && ((cmd & MMCSD_STOPXFR) != 0)) {
	...
	case MMCSD_R1B_RESPONSE: {
		...
		regval |= (USDHC_XFERTYP_RSPTYP_LEN48BSY | USDHC_XFERTYP_CICEN | USDHC_XFERTYP_CCCEN);
	} break;

	case MMCSD_R1_RESPONSE: /* Response length 48, check cmdindex */
	case MMCSD_R5_RESPONSE:
	case MMCSD_R6_RESPONSE: {
		regval |= (USDHC_XFERTYP_RSPTYP_LEN48 | USDHC_XFERTYP_CICEN | USDHC_XFERTYP_CCCEN);
	} break;
	...
	mcinfo("cmd: %08" PRIx32 " arg: %08" PRIx32 " regval: %08" PRIx32 " mcrval: %08" PRIx32 "\n",
	...
	if ((getreg32(priv->addr + IMX9_USDHC_IRQSTAT_OFFSET) & USDHC_RESPERR_INTS) != 0) {
		modifyreg32(priv->addr + IMX9_USDHC_SYSCTL_OFFSET, 0, USDHC_SYSCTL_RSTC);
		while ((getreg32(priv->addr + IMX9_USDHC_SYSCTL_OFFSET) & USDHC_SYSCTL_RSTC) != 0) {
		}
	}
	...
	while ((getreg32(priv->addr + IMX9_USDHC_PRSSTAT_OFFSET) & USDHC_PRSSTAT_CIHB) != 0) {
		/* Calculate the elapsed time */

		elapsed = clock_systime_ticks() - start;
		if (elapsed >= timeout) {
			mcerr("ERROR: Timeout (waiting CIHB) cmd: %08" PRIx32 " PRSSTAT: %08" PRIx32 "\n", cmd,
				  getreg32(priv->addr + IMX9_USDHC_PRSSTAT_OFFSET));
			return -EBUSY;
		}
	}
...
static int imx9_waitresponse(struct sdio_dev_s *dev, uint32_t cmd)
{
	...
	start = clock_systime_ticks();
	while ((getreg32(priv->addr + IMX9_USDHC_IRQSTAT_OFFSET) & USDHC_INT_CC) == 0) {
		/* Calculate the elapsed time */
	...
	if (enerrors != 0) {
		mcerr("ERROR: cmd: %08" PRIx32 " errors: %08" PRIx32 ", "
			  ...
			  cmd, errors, enerrors, getreg32(priv->addr + IMX9_USDHC_IRQSTAT_OFFSET));
...
static void imx9_waitenable(struct sdio_dev_s *dev, sdio_eventset_t eventset, uint32_t timeout)
{
	...
	if ((priv->waitevents & SDIOWAIT_TIMEOUT) != 0) {
		...
		ret = wd_start(&priv->waitwdog, delay, imx9_eventtimeout, (wdparm_t)priv);
...
static sdio_eventset_t imx9_eventwait(struct sdio_dev_s *dev)
{
	...
	if (ret < 0 || priv->wkupevent == SDIOWAIT_TIMEOUT) {
...
static int imx9_dmarecvsetup(struct sdio_dev_s *dev, uint8_t *buffer, size_t buflen)
{
...
#if defined(CONFIG_ARCH_HAVE_SDIO_PREFLIGHT) && !defined(CONFIG_ARM64_DCACHE_DISABLE)
	...
	DEBUGASSERT(buflen <= sizeof(priv->rxbuffer) || imx9_dmapreflight(dev, buffer, buflen) == 0);
#endif
...
#if !defined(CONFIG_ARM64_DCACHE_DISABLE)
	if (priv->unaligned_rx) {
		putreg32((uint64_t)priv->rxbuffer, priv->addr + IMX9_USDHC_DSADDR_OFFSET);
	} else
#endif
	{
		putreg32((uint64_t)priv->buffer, priv->addr + IMX9_USDHC_DSADDR_OFFSET);
	}
...
void imx9_usdhc_set_sdio_card_isr(struct sdio_dev_s *dev, int (*func)(void *), void *arg)
{
	irqstate_t flags;
	uint32_t regval;
	struct imx9_dev_s *priv = (struct imx9_dev_s *)dev;
