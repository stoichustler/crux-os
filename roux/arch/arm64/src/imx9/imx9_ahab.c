...
struct boot_img_hdr *ahab_read_auth_image(struct container_hdr *container, int image_index,
										  unsigned long container_offset)
{
	...
	images = (struct boot_img_hdr *)((uint8_t *)container + sizeof(struct container_hdr));
	...
	imx9_romapi_load_image((uint8_t *)images[image_index].dst, offset, size);

	/* Flush the cache so ELE can read from RAM the updated values. */

	up_flush_dcache((unsigned long)images[image_index].dst, images[image_index].dst + size);
