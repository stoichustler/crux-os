...
static inline uint32_t imx8_read(struct imx8_uart_port_s *port, uint32_t off)
{
	return getreg32(port->iobase + off);
}

static inline void imx8_write(struct imx8_uart_port_s *port, uint32_t val, uint32_t off)
{
...
static void imx8_shutdown(struct uart_dev_s *dev)
{
	...
	temp = imx8_read(sport, UARTCTRL);
	temp &= ~(UARTCTRL_TE | UARTCTRL_RE | UARTCTRL_TIE | UARTCTRL_TCIE | UARTCTRL_RIE |
			  UARTCTRL_ILIE | UARTCTRL_LOOPS);
	imx8_write(sport, temp, UARTCTRL);
...
static void imx8_setup_watermark(struct imx8_uart_port_s *sport)
{
	...
	ctrl_saved = ctrl;
	ctrl &= ~(UARTCTRL_TIE | UARTCTRL_TCIE | UARTCTRL_TE | UARTCTRL_RIE | UARTCTRL_RE);
...
static void imx8_hw_disable(struct imx8_uart_port_s *sport)
{
	...
	temp = imx8_read(sport, UARTCTRL);
	temp &= ~(UARTCTRL_RIE | UARTCTRL_ILIE | UARTCTRL_RE | UARTCTRL_TIE | UARTCTRL_TE);
	imx8_write(sport, temp, UARTCTRL);
}
...
static int imx8_attach(struct uart_dev_s *dev)
{
	...
	sport->txfifo_size = UARTFIFO_DEPTH((temp >> UARTFIFO_TXSIZE_OFF) & UARTFIFO_FIFOSIZE_MASK);
	sport->rxfifo_size = UARTFIFO_DEPTH((temp >> UARTFIFO_RXSIZE_OFF) & UARTFIFO_FIFOSIZE_MASK);
