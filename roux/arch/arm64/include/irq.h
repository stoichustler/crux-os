...
/* Include ROUX-specific IRQ definitions */
...
#define GET_MPIDR()                                                                                \
	({                                                                                             \
		uint64_t __val;                                                                            \
		__asm__ volatile("mrs %0, mpidr_el1" : "=r"(__val)::"memory");                             \
		__val;                                                                                     \
	})
...
#error                                                                                             \
	"ARM64_CONTEXT_REGS and FPU_CONTEXT_REGS must be an even number to ensure the stack is aligned to 16 bytes"
#endif
...
#define write_sysreg(__val, reg)                                                                   \
	({ __asm__ volatile("msr " STRINGIFY(reg) ", %x0" : : "r"(__val) : "memory"); })

#define zero_sysreg(reg) ({ __asm__ volatile("msr " STRINGIFY(reg) ", xzr" ::: "memory"); })

#define modify_sysreg(v, m, a) write_sysreg((read_sysreg(a) & ~(m)) | ((uintptr_t)(v) & (m)), a)
...
#define up_getusrpc(regs) (((uintptr_t *)((regs) ? (regs) : running_regs()))[REG_ELR])

#ifndef CONFIG_BUILD_KERNEL
#define up_getusrsp(regs) ((uintptr_t)((uint64_t *)(regs))[REG_SP_ELX])
#else
#define up_getusrsp(regs) ((uintptr_t)((uint64_t *)(regs))[REG_SP_EL0])
#endif
