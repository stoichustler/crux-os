...
static inline void videomode_swap(FAR struct videomode_s *left, FAR struct videomode_s *right)
{
...
void sort_videomodes(FAR struct videomode_s *modes, FAR struct videomode_s **preferred,
					 unsigned int nmodes)
{
	...
	if (*preferred != NULL) {
		...
		refresh = DIVIDE(DIVIDE((*preferred)->dotclock * 1000, (*preferred)->htotal),
						 (*preferred)->vtotal);
		if (*preferred != modes) {
		...
		refresh = DIVIDE(DIVIDE(tmpmode->dotclock * 1000, tmpmode->htotal), tmpmode->vtotal);
		if (tmpmode != modes) {
	...
	for (j = 1; j < nmodes - 1; j++) {
		...
		for (i = j; i < nmodes; i++) {
			rtemp = _abs(refresh - DIVIDE(DIVIDE(modes[i].dotclock * 1000, modes[i].htotal),
										  modes[i].vtotal));
