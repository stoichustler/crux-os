...
{
	return DIVIDE(DIVIDE(videomode->dotclock * 1000, videomode->htotal), videomode->vtotal);
}
...
void videomode_dump(FAR const char *prefix, FAR const struct videomode_s *videomode, bool terse)
{
	if (videomode != NULL) {
		if (prefix != NULL) {
		...
		syslog(LOG_INFO, "%ux%u @ %luHz", videomode->hdisplay, videomode->vdisplay,
			   (unsigned long)videomode_refresh(videomode));

		if (!terse) {
			syslog(LOG_INFO, " (%lu %u %u %u %u %u %u", (unsigned long)videomode->dotclock,
