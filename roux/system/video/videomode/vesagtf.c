...
void vesagtf_mode_params(unsigned int x, unsigned int y, unsigned int refresh,
						 FAR struct vesagtf_params *params, unsigned int flags,
						 FAR struct videomode_s *videomode)
{
  ...
  unsigned int h_freq;
	...
	top_margin = (flags & VESAGTF_FLAG_MARGINS) ? DIVIDE(v_lines * params->margin_ppt, 1000) : 0;
	...
	h_period_est = DIVIDE(((DIVIDE(2000000000000ULL, v_field_rqd)) - (2000000 * params->min_vsbp)),
  ...
  v_back_porch = vsync_plus_bp - params->vsync_rqd;
	...
	total_v_lines =
		v_lines + top_margin + bottom_margin + vsync_plus_bp + interlace + params->min_porch;
	...
	v_field_est = DIVIDE(DIVIDE(1000000000000000ULL, h_period_est), total_v_lines);
  ...
  v_frame_rate = (flags & VESAGTF_FLAG_ILACE) ?
	...
	left_margin = right_margin =
		(flags & VESAGTF_FLAG_MARGINS)
			? DIVIDE(DIVIDE(h_pixels * params->margin_ppt, 1000), CELL_GRAN) * CELL_GRAN
			: 0;
	...
	ideal_duty_cycle = ((C_PRIME256(params) * 1000) - (M_PRIME256(params) * h_period / 1000000));
	...
	h_blank =
		DIVIDE(DIVIDE(total_active_pixels * ideal_duty_cycle, (256000 * 100ULL) - ideal_duty_cycle),
			   2 * CELL_GRAN) *
		(2 * CELL_GRAN);
  ...
  h_freq = 1000000000 / h_period;
	...
	h_sync = DIVIDE(((params->hsync_pct * total_pixels) / 100), CELL_GRAN) * CELL_GRAN;
