...
static void btnet_scan_callback(FAR const bt_addr_le_t *addr, int8_t rssi, uint8_t adv_type,
...
static void btnet_read_callback(FAR struct bt_conn_s *conn, int result, FAR const void *data,
								uint16_t length)
{
...
int btnet_ioctl(FAR struct net_driver_s *netdev, int cmd, unsigned long arg)
{
	...
	case SIOCBTADVSTART: {
		ret = bt_start_advertising(btreq->btr_advtype, btreq->btr_advad, btreq->btr_advsd);
	...
	case SIOCBTGATTRD: {
		...
		if (conn == NULL) {
			...
			if (btreq->btr_rdnhandles == 1) {
				...
				ret = bt_gatt_read(conn, btreq->btr_rdhandles[0], btreq->btr_rdoffset,
								   btnet_read_callback);
				...
				ret = bt_gatt_read_multiple(conn, btreq->btr_rdhandles, btreq->btr_rdnhandles,
											btnet_read_callback);
			} else {
				ret = -ENFILE;
			}
	...
	case SIOCBTGATTWR: {
		...
		if (conn == NULL) {
			...
			ret = bt_gatt_write(conn, btreq->btr_wrhandle, btreq->btr_wrdata, btreq->btr_wrnbytes,
								bnet_write_callback);
			if (ret < 0) {
