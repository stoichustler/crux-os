...
int bt_queue_open(FAR const char *name, int oflags, int nmsgs, FAR struct file *mqd)
{
	struct mq_attr attr;
	int ret = OK;
...
int bt_queue_send(struct file *mqd, FAR struct bt_buf_s *buf, unsigned int priority)
{
	struct bt_bufmsg_s msg;
	int ret;
	...
	msg.buf = buf;
	ret = file_mq_send(mqd, (FAR const char *)&msg, sizeof(struct bt_bufmsg_s), priority);
	if (ret < 0) {
