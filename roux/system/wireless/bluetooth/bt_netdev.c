...
static void btnet_l2cap_receive(FAR struct bt_conn_s *conn, FAR struct bt_buf_s *buf,
...
static int btnet_req_data(FAR struct radio_driver_s *netdev, FAR const void *meta,
...
static int btnet_req_hci_data(FAR struct btnet_driver_s *priv,
							  FAR struct bluetooth_frame_meta_s *meta, FAR struct iob_s *framelist);
								...
								FAR void *context, uint16_t cid)
{
	...
	DEBUGASSERT(conn != NULL && buf != NULL && buf->frame != NULL && context != NULL &&
				cid < UINT8_MAX);
	...
	frame->io_offset = (unsigned int)((uintptr_t)buf->data - (uintptr_t)frame->io_data);
	...
	if (ret < 0) {
		...
		if ((frame->io_data[frame->io_offset] & SIXLOWPAN_DISPATCH_NALP_MASK) ==
 ...
 *
 * Input Parameters:
 *   netdev - Reference to the ROUX driver state structure
 *
 * Returned Value:
 ...
 *
 * Description:
 *   ROUX Callback: Bring up the Bluetooth interface when an IP address
 *   is provided
 *
 * Input Parameters:
 *   netdev - Reference to the ROUX driver state structure
...
static int btnet_ifup(FAR struct net_driver_s *netdev)
{
	...
	if (ret >= 0) {
		...
		wlinfo("Bringing up: IP   %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", netdev->d_ipv6addr[0],
			   netdev->d_ipv6addr[1], netdev->d_ipv6addr[2], netdev->d_ipv6addr[3],
			   netdev->d_ipv6addr[4], netdev->d_ipv6addr[5], netdev->d_ipv6addr[6],
			   netdev->d_ipv6addr[7]);
		wlinfo("             ADDR %02x:%02x:%02x:%02x:%02x:%02x\n", netdev->d_mac.radio.nv_addr[0],
			   netdev->d_mac.radio.nv_addr[1], netdev->d_mac.radio.nv_addr[2],
			   netdev->d_mac.radio.nv_addr[3], netdev->d_mac.radio.nv_addr[4],
			   netdev->d_mac.radio.nv_addr[5]);
							  ...
							  FAR struct bluetooth_frame_meta_s *meta, FAR struct iob_s *framelist)
{
	...
	for (iob = framelist; iob != NULL; iob = framelist) {
		...
		DEBUGASSERT(iob->io_offset == BLUETOOTH_H4_HDRLEN && iob->io_len >= BLUETOOTH_H4_HDRLEN);
...
int bt_netdev_register(FAR struct bt_driver_s *btdev)
{
	...
	btdev->bt_net = priv = (FAR struct btnet_driver_s *)kmm_zalloc(sizeof(struct btnet_driver_s));
