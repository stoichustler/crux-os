...
#include <roux/wireless/ieee802154/ieee802154_device.h>
...
static ssize_t mac802154dev_write(FAR struct file *filep, FAR const char *buffer, size_t len);
static int mac802154dev_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
...
static int mac802154dev_open(FAR struct file *filep)
{
	...
	opriv = (FAR struct mac802154dev_open_s *)kmm_zalloc(sizeof(struct mac802154dev_open_s));
...
static int mac802154dev_close(FAR struct file *filep)
{
	...
	for (prev = NULL, curr = dev->md_open; curr && curr != opriv;
	...
	if (dev->md_open) {
		...
		primitive = (FAR struct ieee802154_primitive_s *)sq_remfirst(&dev->primitive_queue);
...
static ssize_t mac802154dev_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	...
	ret = mac802154_ioctl(dev->md_mac, MAC802154IOC_MLME_GET_REQUEST, (unsigned long)&req);
	...
	if (ret == 0 && req.attrval.mac.promisc_mode) {
		...
		memcpy(&rx->payload[0], &ind->frame->io_data[ind->frame->io_offset], rx->length);
	}
...
static int mac802154dev_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	FAR struct inode *inode;
	FAR struct mac802154_chardevice_s *dev;
	FAR union ieee802154_macarg_u *macarg = (FAR union ieee802154_macarg_u *)((uintptr_t)arg);
	int ret;
...
static int mac802154dev_notify(FAR struct mac802154_maccb_s *maccb,
							   FAR struct ieee802154_primitive_s *primitive)
{
	FAR struct mac802154dev_callback_s *cb = (FAR struct mac802154dev_callback_s *)maccb;
	FAR struct mac802154_chardevice_s *dev;
