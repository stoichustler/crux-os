...
#include <roux/wireless/ieee802154/ieee802154_radio.h>
...
static int mac802154_radiopoll(FAR const struct ieee802154_radiocb_s *radiocb, bool gts,
...
static void mac802154_edresult(FAR const struct ieee802154_radiocb_s *radiocb, uint8_t edval);
					  ...
					  FAR struct ieee802154_primitive_s *primitive)
{
	...
	if (work_available(&priv->notifwork)) {
		work_queue(LPWORK, &priv->notifwork, mac802154_notify_worker, (FAR void *)priv, 0);
	}
}
...
static void mac802154_notify_worker(FAR void *arg)
{
	...
	primitive = (FAR struct ieee802154_primitive_s *)sq_remfirst(&priv->primitive_queue);
	nxmutex_unlock(&priv->lock);
...
void mac802154_updatebeacon(FAR struct ieee802154_privmac_s *priv)
{
	...
	IEEE802154_SETBEACONORDER(beacon->bf_data, beacon->bf_len, priv->sfspec.beaconorder);
	IEEE802154_SETSFORDER(beacon->bf_data, beacon->bf_len, priv->sfspec.sforder);
	IEEE802154_SETFINCAPSLOT(beacon->bf_data, beacon->bf_len, priv->sfspec.final_capslot);
	if (priv->sfspec.ble) {
	...
	memcpy(&beacon->bf_data[beacon->bf_len], priv->beaconpayload, priv->beaconpayloadlength);
							 ...
							 FAR struct ieee802154_txdesc_s *txdesc)
{
	...
	if (work_available(&priv->purge_work)) {
		work_queue(HPWORK, &priv->purge_work, mac802154_purge_worker, (FAR void *)priv, ticks);
	}
}
...
static void mac802154_purge_worker(FAR void *arg)
{
	...
	while (1) {
		...
		if (clock_systime_ticks() >= txdesc->purgetime) {
			...
			work_queue(HPWORK, &priv->purge_work, mac802154_purge_worker, priv,
					   txdesc->purgetime - clock_systime_ticks());
			break;
		}
	}
...
static int mac802154_radiopoll(FAR const struct ieee802154_radiocb_s *radiocb, bool gts,
							   FAR struct ieee802154_txdesc_s **txdesc)
{
	FAR struct mac802154_radiocb_s *cb = (FAR struct mac802154_radiocb_s *)radiocb;
							 ...
							 FAR struct ieee802154_txdesc_s *txdesc)
{
	...
	if (work_available(&priv->txdone_work)) {
		work_queue(HPWORK, &priv->txdone_work, mac802154_txdone_worker, (FAR void *)priv, 0);
	}
}
							  ...
							  FAR struct ieee802154_data_ind_s *ind)
{
	...
	if (work_available(&priv->rx_work)) {
		work_queue(HPWORK, &priv->rx_work, mac802154_rxframe_worker, (FAR void *)priv, 0);
	}
}
...
static void mac802154_rxframe_worker(FAR void *arg)
{
	...
	while (1) {
		...
		ind->src.mode =
			(*frame_ctrl & IEEE802154_FRAMECTRL_SADDR) >> IEEE802154_FRAMECTRL_SHIFT_SADDR;

		ind->dest.mode =
			(*frame_ctrl & IEEE802154_FRAMECTRL_DADDR) >> IEEE802154_FRAMECTRL_SHIFT_DADDR;

		panid_comp =
			(*frame_ctrl & IEEE802154_FRAMECTRL_PANIDCOMP) >> IEEE802154_FRAMECTRL_SHIFT_PANIDCOMP;
		...
		ftype = (*frame_ctrl & IEEE802154_FRAMECTRL_FTYPE) >> IEEE802154_FRAMECTRL_SHIFT_FTYPE;
									...
									FAR struct ieee802154_data_ind_s *ind)
{
	...
	beacon->pandesc.sfspec.beaconorder = IEEE802154_GETBEACONORDER(iob->io_data, iob->io_offset);

	beacon->pandesc.sfspec.sforder = IEEE802154_GETSFORDER(iob->io_data, iob->io_offset);

	beacon->pandesc.sfspec.final_capslot = IEEE802154_GETFINCAPSLOT(iob->io_data, iob->io_offset);

	beacon->pandesc.sfspec.ble = IEEE802154_GETBLE(iob->io_data, iob->io_offset);

	beacon->pandesc.sfspec.pancoord = IEEE802154_GETPANCOORD(iob->io_data, iob->io_offset);

	beacon->pandesc.sfspec.assocpermit = IEEE802154_GETASSOCPERMIT(iob->io_data, iob->io_offset);
	...
	if (iob->io_len < (iob->io_offset + (IEEE802154_SADDRSIZE * beacon->pendaddr.nsaddr) +
	...
	for (i = beacon->pendaddr.nsaddr; i < (beacon->pendaddr.nsaddr + beacon->pendaddr.neaddr);
		 i++) {
	...
	if (priv->curr_op == MAC802154_OP_SCAN) {
		...
		for (i = 0; i < priv->npandesc; i++) {
			...
			if (memcmp(&ind->src, &priv->pandescs[i].coordaddr, sizeof(struct ieee802154_addr_s)) !=
				0) {
				continue;
			}
	...
	/* If we are not performing a SCAN operation */
		...
		if (priv->curr_op == MAC802154_OP_ASSOC && pending_eaddr) {
			...
			mac802154_createdatareq(priv, &priv->pandesc.coordaddr, IEEE802154_ADDRMODE_EXTENDED,
									respdesc);
			...
			if (priv->autoreq || priv->curr_op == MAC802154_OP_POLL) {
				...
				if (pending_saddr | pending_eaddr) {
					...
					if (priv->curr_op == MAC802154_OP_POLL) {
						...
						int sval;
						DEBUGASSERT(nxsem_get_value(&priv->opsem, &sval) == 0 && sval == 1);
