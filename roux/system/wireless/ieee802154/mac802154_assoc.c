...
int mac802154_req_associate(MACHANDLE mac, FAR struct ieee802154_assoc_req_s *req)
{
	...
	iob->io_data[iob->io_len] |= (req->capabilities.devtype << IEEE802154_CAPABILITY_SHIFT_DEVTYPE);
	iob->io_data[iob->io_len] |=
		(req->capabilities.powersource << IEEE802154_CAPABILITY_SHIFT_PWRSRC);
	iob->io_data[iob->io_len] |=
		(req->capabilities.rxonidle << IEEE802154_CAPABILITY_SHIFT_RXONIDLE);
	iob->io_data[iob->io_len] |=
		(req->capabilities.security << IEEE802154_CAPABILITY_SHIFT_SECURITY);
	iob->io_data[iob->io_len] |=
		(req->capabilities.allocaddr << IEEE802154_CAPABILITY_SHIFT_ALLOCADDR);
...
int mac802154_resp_associate(MACHANDLE mac, FAR struct ieee802154_assoc_resp_s *resp)
{
	FAR struct ieee802154_privmac_s *priv = (FAR struct ieee802154_privmac_s *)mac;
	FAR struct ieee802154_txdesc_s *txdesc;
	FAR struct iob_s *iob;
							   ...
							   FAR struct ieee802154_txdesc_s *txdesc)
{
	...
	if (txdesc->conf->status != IEEE802154_STATUS_SUCCESS) {
		...
		if (priv->sfspec.beaconorder < 15) {
			...
			work_queue(LPWORK, &priv->macop_work, mac802154_extract_assocresp, priv, 0);
		}
									...
									FAR struct ieee802154_txdesc_s *txdesc)
{
	...
	if (primitive->u.dataconf.status != IEEE802154_STATUS_SUCCESS || txdesc->framepending == 0) {
		...
		if (priv->sfspec.beaconorder == 15) {
			...
			wlinfo("Starting timeout timer\n");
			mac802154_timerstart(priv, priv->max_frame_waittime, mac802154_assoctimeout);
		}
...
static void mac802154_extract_assocresp(FAR void *arg)
{
	...
	mac802154_createdatareq(priv, &priv->pandesc.coordaddr, IEEE802154_ADDRMODE_EXTENDED, respdesc);
