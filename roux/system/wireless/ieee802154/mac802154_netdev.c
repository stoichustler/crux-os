...
static int macnet_rxframe(FAR struct macnet_driver_s *maccb, FAR struct ieee802154_data_ind_s *ind);
...
#ifdef CONFIG_NET_6LOWPAN_EXTENDEDADDR
static int macnet_coord_eaddr(FAR struct radio_driver_s *radio, FAR uint8_t *eaddr);
#else
static int macnet_coord_saddr(FAR struct radio_driver_s *radio, FAR uint8_t *saddr);
#endif
#endif
...
static int macnet_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
static int macnet_get_mhrlen(FAR struct radio_driver_s *netdev, FAR const void *meta);
static int macnet_req_data(FAR struct radio_driver_s *netdev, FAR const void *meta,
...
static int macnet_notify(FAR struct mac802154_maccb_s *maccb,
						 FAR struct ieee802154_primitive_s *primitive)
{
	FAR struct macnet_callback_s *cb = (FAR struct macnet_callback_s *)maccb;
	FAR struct macnet_driver_s *priv;
...
static int macnet_rxframe(FAR struct macnet_driver_s *priv, FAR struct ieee802154_data_ind_s *ind)
{
...
static int macnet_get_mhrlen(FAR struct radio_driver_s *netdev, FAR const void *meta)
{
	FAR struct macnet_driver_s *priv = (FAR struct macnet_driver_s *)netdev;
...
int mac802154netdev_register(MACHANDLE mac)
{
	...
	priv = (FAR struct macnet_driver_s *)kmm_zalloc(sizeof(struct macnet_driver_s));
