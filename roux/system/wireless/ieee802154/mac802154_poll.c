...
int mac802154_req_poll(MACHANDLE mac, FAR struct ieee802154_poll_req_s *req)
{
	...
	if (priv->addr.saddr[0] >= 0xfe && priv->addr.saddr[1] == 0xff) {
		mac802154_createdatareq(priv, &req->coordaddr, IEEE802154_ADDRMODE_EXTENDED, txdesc);
	} else {
		mac802154_createdatareq(priv, &req->coordaddr, IEEE802154_ADDRMODE_SHORT, txdesc);
	}
								   ...
								   FAR struct ieee802154_txdesc_s *txdesc)
{
	...
	if (txdesc->conf->status != IEEE802154_STATUS_SUCCESS || txdesc->framepending == 0) {
		...
		mac802154_timerstart(priv, priv->max_frame_waittime, mac802154_polltimeout);
