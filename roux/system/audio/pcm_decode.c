...
static ssize_t pcm_parsewav(FAR struct pcm_decode_s *priv, uint8_t *data, apb_samp_t len);
#endif
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int pcm_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
static void pcm_callback(FAR void *arg, uint16_t reason, FAR struct ap_buffer_s *apb,
...
static inline bool pcm_validwav(FAR const struct wav_header_s *wav)
{
	return (wav->hdr.chunkid == WAV_HDR_CHUNKID && wav->hdr.format == WAV_HDR_FORMAT &&
			wav->fmt.chunkid == WAV_FMT_CHUNKID && wav->fmt.chunklen == WAV_FMT_CHUNKLEN &&
			wav->fmt.format == WAV_FMT_FORMAT && wav->fmt.nchannels < 256 && wav->fmt.align < 256 &&
			wav->fmt.bpsamp < 256);
}
...
static ssize_t pcm_parsewav(FAR struct pcm_decode_s *priv, uint8_t *data, apb_samp_t len)
{
	...
	for (;;) {
		...
		dchunk =
			(FAR const struct wav_datachunk_s *)((uintptr_t)dchunk + localwav.data.chunklen + 8);
	}
...
static int pcm_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb)
{
	...
	audinfo("curbyte=%d nbytes=%d nmaxbytes=%d bytesleft=%d\n", apb->curbyte, apb->nbytes,
			apb->nmaxbytes, bytesleft);

	/* Parse and verify the candidate PCM WAV file header */

#ifndef CONFIG_AUDIO_FORMAT_RAW
	ssize_t headersize = pcm_parsewav(priv, &apb->samp[apb->curbyte], bytesleft);
	if (headersize > 0) {
