...
static struct nxbe_window_s *nxmu_revalidate_g_mwnd(FAR struct nxbe_window_s *wnd)
{
	if (!g_mwnd) {
		return NULL;
	}
...
int nxmu_mousein(FAR struct nxmu_state_s *nxmu, FAR const struct nxgl_point_s *pos, int buttons)
{
