...
int nxmu_sendclient(FAR struct nxmu_conn_s *conn, FAR const void *msg, size_t msglen);
...
int nxmu_sendclientwindow(FAR struct nxbe_window_s *wnd, FAR const void *msg, size_t msglen);
...
void nxmu_requestbkgd(FAR struct nxmu_conn_s *conn, FAR struct nxbe_state_s *be,
...
void nxmu_redraw(FAR struct nxbe_window_s *wnd, FAR const struct nxgl_rect_s *rect);
...
#ifdef CONFIG_NX_XYINPUT
int nxmu_mousein(FAR struct nxmu_state_s *nxmu, FAR const struct nxgl_point_s *pos, int button);
#endif
