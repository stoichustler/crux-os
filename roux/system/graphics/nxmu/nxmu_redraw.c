									...
									FAR const struct nxgl_rect_s *rect)
{
	...
	nxgl_rectoffset(&wndrect, rect, -wnd->bounds.pt1.x, -wnd->bounds.pt1.y);

	/* Get the source of address of the rectangle in the framebuffer. */

	bpp = wnd->be->plane[0].pinfo.bpp;
	src[0] = (FAR const void *)((FAR uint8_t *)wnd->fbmem + wndrect.pt1.y * wnd->stride +
								((bpp * wndrect.pt1.x) >> 3));
...
void nxmu_redrawreq(FAR struct nxbe_window_s *wnd, FAR const struct nxgl_rect_s *rect)
{
	...
	nxmu_sendclientwindow(wnd, &outmsg, sizeof(struct nxclimsg_redraw_s));
}
