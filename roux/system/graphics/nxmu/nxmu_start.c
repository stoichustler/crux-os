...
{
	DEBUGASSERT((unsigned)display < CONFIG_NX_NDISPLAYS && (unsigned)plane < CONFIG_NX_NPLANES);
	...
	if (!g_nxserver_started[display]) {
		...
		int server;
		FAR char *const argv[3] = {(FAR char *const)display_str, (FAR char *const)plane_str, NULL};
		...
		server = kthread_create("NX Server", CONFIG_NXSTART_SERVERPRIO, CONFIG_NXSTART_SERVERSTACK,
								nx_server, argv);
		if (server < 0) {
