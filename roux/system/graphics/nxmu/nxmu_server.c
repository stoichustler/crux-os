...
static void nxmu_event(FAR struct nxbe_window_s *wnd, enum nx_event_e event, FAR void *arg)
{
	...
	ret = nxmu_sendclient(wnd->conn, &outmsg, sizeof(struct nxclimsg_event_s));
	if (ret < 0) {
		gerr("ERROR: nxmu_sendclient failed: %d\n", ret);
	}
}
...
int nx_runinstance(FAR const char *mqname, FAR NX_DRIVERTYPE *dev)
{
	...
	for (;;) {
		...
		case NX_SVRMSG_GETRECTANGLE: /* Get a rectangular region from the window */
		{
			...
			nxbe_getrectangle(getmsg->wnd, &getmsg->rect, getmsg->plane, getmsg->dest,
							  getmsg->deststride);
		...
		case NX_CLIMSG_REDRAW: /* Re-draw the background window */
		{
			...
			ginfo("Re-draw background rect={(%d,%d),(%d,%d)}\n", redraw->rect.pt1.x,
				  redraw->rect.pt1.y, redraw->rect.pt2.x, redraw->rect.pt2.y);
