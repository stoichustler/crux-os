...
static void nxbe_clipmovesrc(FAR struct nxbe_clipops_s *cops, FAR struct nxbe_plane_s *plane,
							 FAR const struct nxgl_rect_s *rect)
{
	...
	if (info->offset.x != 0 || info->offset.y != 0) {
		...
		wnd = info->wnd;
		nxgl_rectoffset(&update, rect, offset.x - wnd->bounds.pt1.x, offset.y - wnd->bounds.pt1.y);
...
static void nxbe_clipmoveobscured(FAR struct nxbe_clipops_s *cops, FAR struct nxbe_plane_s *plane,
								  FAR const struct nxgl_rect_s *rect)
{
	struct nxbe_move_s *info = (struct nxbe_move_s *)cops;
...
static void nxbe_clipmovedest(FAR struct nxbe_clipops_s *cops, FAR struct nxbe_plane_s *plane,
							  FAR const struct nxgl_rect_s *rect)
{
	...
	if (!nxgl_nullrect(&src)) {
		...
		nxbe_clipper(dstdata->wnd->above, &src, dstdata->order, &srcinfo.cops, plane);
	}
}
...
static inline void nxbe_move_dev(FAR struct nxbe_window_s *wnd, FAR const struct nxgl_rect_s *rect,
								 FAR const struct nxgl_point_s *offset)
{
...
#endif
	{
		...
		nxbe_clipper(wnd->above, &info.srcrect, info.order, &info.cops, &wnd->be->plane[i]);
...
static inline void nxbe_move_pwfb(FAR struct nxbe_window_s *wnd, FAR const struct nxgl_rect_s *rect,
								  FAR const struct nxgl_point_s *offset)
{
	...
	src[0] = (FAR const void *)((FAR uint8_t *)wnd->fbmem + destrect.pt1.y * wnd->stride +
