...
#ifdef CONFIG_NX_RAMBACKED
static void nxbe_realloc(FAR struct nxbe_window_s *wnd, FAR struct nxgl_rect_s *oldbounds)
{
	...
	if (NXBE_ISRAMBACKED(wnd)) {
		...
		for (src = (FAR uint8_t *)wnd->fbmem, dest = (FAR uint8_t *)newfb, row = 0; row < minheight;
