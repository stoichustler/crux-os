...
static void nxbe_clipredraw(FAR struct nxbe_clipops_s *cops, FAR struct nxbe_plane_s *plane,
				 ...
				 FAR const struct nxgl_rect_s *rect)
{
	...
	if (!nxgl_nullrect(&remaining)) {
		...
		for (i = 0; i < be->vinfo.nplanes; i++) {
			nxbe_clipper(wnd->above, &remaining, NX_CLIPORDER_DEFAULT, &info.cops, &be->plane[i]);
		}
#else
		nxbe_clipper(wnd->above, &remaining, NX_CLIPORDER_DEFAULT, &info.cops, &be->plane[0]);
#endif
	}
}
