...
static void nxbe_clipvisible(FAR struct nxbe_clipops_s *cops, FAR struct nxbe_plane_s *plane,
...
bool nxbe_isvisible(FAR struct nxbe_window_s *wnd, FAR const struct nxgl_point_s *pos)
{
	...
	nxbe_clipper(wnd->above, &wnd->bounds, NX_CLIPORDER_DEFAULT, &info.cops, &wnd->be->plane[0]);
