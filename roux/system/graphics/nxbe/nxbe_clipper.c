									  ...
									  FAR const struct nxgl_rect_s *rect)
{
	...
	if ((stack->npushed + 1) > stack->mxrects) {
		...
		newstack = kmm_realloc(stack->stack, sizeof(struct nxbe_cliprect_s) * mxrects);
		if (!newstack) {
...
static inline bool nxbe_poprectangle(struct nxbe_clipstack_s *stack, FAR struct nxbe_window_s **wnd,
									 struct nxgl_rect_s *rect)
{
	if (stack->npushed > 0) {
		stack->npushed--;
