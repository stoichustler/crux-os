...
static void bitmap_clipcopy(FAR struct nxbe_clipops_s *cops, FAR struct nxbe_plane_s *plane,
							FAR const struct nxgl_rect_s *rect)
{
	...
	plane->dev.copyrectangle(&plane->pinfo, rect, bminfo->src, &bminfo->origin, bminfo->stride);
									...
									FAR const struct nxgl_point_s *origin, unsigned int stride)
{
	...
	if (!nxgl_nullrect(&destrect)) {
		...
		nxgl_rectoffset(&destrect, &destrect, -wnd->bounds.pt1.x, -wnd->bounds.pt1.y);
		...
		wnd->be->plane[0].pwfb.copyrectangle(wnd, &destrect, src[0], origin, stride);
	}
}
#endif
					 ...
					 unsigned int stride)
{
...
#endif
	{
		...
		nxbe_clipper(wnd->above, &remaining, NX_CLIPORDER_DEFAULT, &info.cops, &wnd->be->plane[i]);
	}
}
