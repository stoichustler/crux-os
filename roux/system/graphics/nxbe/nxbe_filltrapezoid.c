...
static void nxbe_clipfilltrapezoid(FAR struct nxbe_clipops_s *cops, FAR struct nxbe_plane_s *plane,
								   FAR const struct nxgl_rect_s *rect)
{
	...
	plane->dev.filltrapezoid(&plane->pinfo, &fillinfo->trap, rect, fillinfo->color);

#ifdef CONFIG_NX_UPDATE
	/* Notify external logic that the display has been updated */

	update.pt1.x =
		MIN(MAX(fillinfo->trap.top.x1, rect->pt1.x), MAX(fillinfo->trap.bot.x1, rect->pt1.x));
	update.pt1.y = MAX(fillinfo->trap.top.y, rect->pt1.y);
	update.pt2.x =
		MAX(MIN(fillinfo->trap.top.x2, rect->pt2.x), MIN(fillinfo->trap.bot.x2, rect->pt2.x));
										  ...
										  nxgl_mxpixel_t color[CONFIG_NX_NPLANES])
{
...
#endif
	{
		...
		info.color = color[i];
		nxbe_clipper(wnd->above, bounds, NX_CLIPORDER_DEFAULT, &info.cops, &wnd->be->plane[i]);
										   ...
										   nxgl_mxpixel_t color[CONFIG_NX_NPLANES])
{
	...
	nxgl_trapoffset(&reltrap, trap, -wnd->bounds.pt1.x, -wnd->bounds.pt1.y);
	nxgl_rectoffset(&relbounds, bounds, -wnd->bounds.pt1.x, -wnd->bounds.pt1.y);
	...
	wnd->be->plane[0].pwfb.filltrapezoid(wnd, &reltrap, &relbounds, color[0]);
	...
	src[0] = (FAR const void *)((FAR uint8_t *)wnd->fbmem + relbounds.pt1.y * wnd->stride +
