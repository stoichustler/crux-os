...
void NXGL_FUNCNAME(nxglib_cursor_backup, NXGLIB_SUFFIX)(FAR struct nxbe_state_s *be,
														...
														int planeno)
{
	...
	if (!nxgl_nullrect(&intersection)) {
		...
		sline =
			(FAR uint8_t *)fbmem + sstride * intersection.pt1.y + NXGL_SCALEX(intersection.pt1.x);
		dline = (FAR uint8_t *)be->cursor.bkgd + dstride * origin.y + NXGL_SCALEX(origin.x);
