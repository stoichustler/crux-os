...
static NXGL_PIXEL_T nxbe_map_color(FAR struct nxbe_state_s *be, int plane, uint8_t pixel)
{
	switch (pixel) {
	case 0:
	default:
...
void NXGL_FUNCNAME(nxglib_cursor_draw, NXGLIB_SUFFIX)(FAR struct nxbe_state_s *be,
													  ...
													  int planeno)
{
	...
	if (!nxgl_nullrect(&intersection)) {
		...
		dline =
			(FAR uint8_t *)fbmem + dstride * intersection.pt1.y + NXGL_SCALEX(intersection.pt1.x);
