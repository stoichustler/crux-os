...
void NXGL_FUNCNAME(nxgl_moverectangle, NXGLIB_SUFFIX)(FAR struct lcd_planeinfo_s *pinfo,
													  ...
													  FAR struct nxgl_point_s *offset)
{
	...
	if (offset->y < rect->pt1.y) {
		...
		for (srcrow = rect->pt1.y, destrow = offset->y; srcrow <= rect->pt2.y;
			 srcrow++, destrow++) {
			pinfo->getrun(pinfo->dev, srcrow, rect->pt1.x, pinfo->buffer, ncols);
			pinfo->putrun(pinfo->dev, destrow, offset->x, pinfo->buffer, ncols);
		}
	}
	 ...
	 */
		...
		for (srcrow = rect->pt2.y, destrow = offset->y + dy; srcrow >= rect->pt1.y;
			 srcrow--, destrow--) {
			pinfo->getrun(pinfo->dev, srcrow, rect->pt1.x, pinfo->buffer, ncols);
			pinfo->putrun(pinfo->dev, destrow, offset->x, pinfo->buffer, ncols);
		}
