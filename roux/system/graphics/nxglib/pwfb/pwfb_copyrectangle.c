...
void NXGL_FUNCNAME(pwfb_copyrectangle,
				   NXGLIB_SUFFIX)(FAR struct nxbe_window_s *bwnd,
								  FAR const struct nxgl_rect_s *dest, FAR const void *src,
								  FAR const struct nxgl_point_s *origin, unsigned int srcstride)
{
	...
	sline = (FAR const uint8_t *)src + NXGL_SCALEX(dest->pt1.x - origin->x) +
			(dest->pt1.y - origin->y) * srcstride;
	dline = (FAR uint8_t *)bwnd->fbmem + dest->pt1.y * deststride + NXGL_SCALEX(dest->pt1.x);
