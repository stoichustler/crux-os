...
static inline void pwfb_lowresmemcpy(FAR uint8_t *dline, FAR const uint8_t *sline,
									 unsigned int width, uint8_t leadmask, uint8_t tailmask)
{
...
void NXGL_FUNCNAME(pwfb_moverectangle, NXGLIB_SUFFIX)(FAR struct nxbe_window_s *bwnd,
													  ...
													  FAR struct nxgl_point_s *offset)
{
	...
	sline = (FAR uint8_t *)bwnd->fbmem + rect->pt1.y * stride + NXGL_SCALEX(rect->pt1.x);
	...
	dline = (FAR uint8_t *)bwnd->fbmem + offset->y * stride + NXGL_SCALEX(offset->x);
	...
	if (offset->y < rect->pt1.y || (offset->y < rect->pt1.y && offset->x <= rect->pt1.x)) {
