...
static ssize_t nxterm_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
int nxterm_ioctl_tap(int cmd, uintptr_t arg)
{
	...
	case NXTERMIOC_NXTERM_REDRAW: {
		FAR struct nxtermioc_redraw_s *redraw = (FAR struct nxtermioc_redraw_s *)((uintptr_t)arg);
...
#ifdef CONFIG_NXTERM_NXKBDIN
		FAR struct nxtermioc_kbdin_s *kbdin = (FAR struct nxtermioc_kbdin_s *)((uintptr_t)arg);
	...
	case NXTERMIOC_NXTERM_RESIZE: {
		FAR struct nxtermioc_resize_s *resize = (FAR struct nxtermioc_resize_s *)((uintptr_t)arg);
