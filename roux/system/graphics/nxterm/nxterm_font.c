...
static void nxterm_fillspace(FAR struct nxterm_state_s *priv, FAR const struct nxgl_rect_s *rect,
					 ...
					 FAR const struct nxterm_bitmap_s *bm)
{
	...
	if (!nxgl_nullrect(&intersection)) {
		...
		ret = priv->ops->bitmap(priv, &intersection, &src, &bm->pos, (unsigned int)glyph->stride);
		DEBUGASSERT(ret >= 0);
	}
}
