...
static const struct nibble_info_s g_0bit_info[1] = {{/*  0 xxxx */
													 0, 0, 0, 0}};

static const struct nibble_info_s g_1bit_info[2] = {{/*  0 xxx0 */
													 1, 1, 0, 0},
													{/*  1 xxx1 */
													 0, 0, 0, 0}};

static const struct nibble_info_s g_2bit_info[4] = {{/*  0 xx00 */
													 2, 2, 0, 0},
													{/*  1 xx01 */
													 1, 0, 1, 0},
													{/*  2 xx10 */
...
static const struct nibble_info_s g_3bit_info[8] = {{/*  0 x000 */
													 3, 3, 0, 0},
													{/*  1 x001 */
													 2, 0, 2, 0},
													{/*  2 x010 */
...
static const struct nibble_info_s g_4bit_info[16] = {{/*  0 0000 */
													  4, 4, 0, 0},
													 {/*  1 0001 */
													  3, 0, 3, 0},
													 {/*  2 0010 */
...
static const struct nibble_info_s *g_info_table[5] = {g_0bit_info, g_1bit_info, g_2bit_info,
													  g_3bit_info, g_4bit_info};
...
static void gran_nibble_info(uint8_t value, FAR struct valinfo_s *info, unsigned int nbits)
{
...
static void gran_byte_info(uint8_t value, FAR struct valinfo_s *info, unsigned int nbits)
{
	uint8_t mask;
...
static void gran_hword_info(uint16_t value, FAR struct valinfo_s *info, unsigned int nbits)
{
	uint16_t mask;
...
void gran_info(GRAN_HANDLE handle, FAR struct graninfo_s *info)
{
	...
	for (granidx = 0; granidx < priv->ngranules; granidx += 32) {
		...
		if (value == mask) {
			...
			gran_hword_info((uint16_t)(value & 0xffff), &hwinfo, nbits > 16 ? 16 : nbits);
			if (nbits > 16) {
				struct valinfo_s msinfo;
