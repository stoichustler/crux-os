...
static bool in_range(FAR const void *start, size_t length, FAR const void *range_start,
					 size_t range_length)
{
...
void mm_map_initialize(FAR struct mm_map_s *mm, bool kernel)
{
	...
	if (!kernel) {
		mm->mm_map_vpages = gran_initialize((FAR void *)CONFIG_ARCH_SHM_VBASE, ARCH_SHM_SIZE,
									   ...
									   FAR const struct mm_map_entry_s *entry)
{
	...
	if (nxrmutex_lock(&mm->mm_map_mutex) == OK) {
		if (entry == NULL) {
			...
			next_entry = (struct mm_map_entry_s *)sq_next(((sq_entry_t *)entry));
		}
...
FAR struct mm_map_entry_s *mm_map_find(FAR struct mm_map_s *mm, FAR const void *vaddr,
									   size_t length)
{
	...
	if (nxrmutex_lock(&mm->mm_map_mutex) == OK) {
		...
		while (found_entry && !in_range(vaddr, length, found_entry->vaddr, found_entry->length)) {
			found_entry = (struct mm_map_entry_s *)sq_next(((sq_entry_t *)found_entry));
		}
...
int mm_map_remove(FAR struct mm_map_s *mm, FAR struct mm_map_entry_s *entry)
{
	...
	if (entry == prev_entry) {
		...
		while ((removed_entry = (struct mm_map_entry_s *)sq_next(((sq_entry_t *)prev_entry)))) {
