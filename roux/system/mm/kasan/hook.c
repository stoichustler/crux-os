...
static void kasan_show_memory(FAR const uint8_t *addr, size_t size, size_t dumpsize)
{
	...
	for (p = start; p < end; p += 16) {
		...
		for (i = 0; i < 16; i++) {
			if (kasan_is_poisoned(p + i, 1)) {
				if (p + i == addr) {
					ret += sprintf(buffer + ret, "\b[\033[31m%02x\033[0m ", p[i]);
...
static void kasan_report(FAR const void *addr, size_t size, bool is_write, FAR void *return_address)
{
	...
	if ((is_write && MM_KASAN_DISABLE_WRITE_PANIC) || (!is_write && MM_KASAN_DISABLE_READ_PANIC)) {
		dump_stack();
	} else {
		PANIC();
	}
...
#if MM_KASAN_WATCHPOINT > 0
static void kasan_check_watchpoint(FAR const void *addr, size_t size, bool is_write,
								   FAR void *return_address)
{
	...
	for (i = 0; i < MM_KASAN_WATCHPOINT; i++) {
		...
		if (addr + size <= watchpoint->addr || addr >= watchpoint->addr + watchpoint->size) {
			continue;
		}
...
static inline void kasan_check_report(FAR const void *addr, size_t size, bool is_write,
									  FAR void *return_address)
{
#ifdef CONFIG_MM_KASAN
...
void __sanitizer_annotate_contiguous_container(FAR const void *beg, FAR const void *end,
											   FAR const void *old_mid, FAR const void *new_mid)
{
	/* Shut up compiler complaints */
}
