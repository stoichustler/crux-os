...
#define KASAN_GLOBAL_FIRST_WORD_MASK(start) (UINTPTR_MAX << ((start) & (KASAN_BITS_PER_WORD - 1)))
#define KASAN_GLOBAL_LAST_WORD_MASK(end) (UINTPTR_MAX >> (-(end) & (KASAN_BITS_PER_WORD - 1)))
...
static inline_function FAR uintptr_t *kasan_global_mem_to_shadow(FAR const void *ptr, size_t size,
																 FAR unsigned int *bit)
{
	...
	for (i = 0; g_global_region[i]; i++) {
		if (addr >= g_global_region[i]->begin && addr < g_global_region[i]->end) {
			...
			*bit = addr % KASAN_BITS_PER_WORD;
			return (FAR uintptr_t *)&g_global_region[i]->shadow[addr / KASAN_BITS_PER_WORD];
		}
	}
...
static inline_function bool kasan_global_is_poisoned(FAR const void *addr, size_t size)
{
	FAR uintptr_t *p;
	unsigned int bit;
	unsigned int nbit;
