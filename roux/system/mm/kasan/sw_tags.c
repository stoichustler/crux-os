...
#define KASAN_SHADOW_SIZE(size) ((size) + KASAN_SHADOW_SCALE - 1) / KASAN_SHADOW_SCALE
#define KASAN_REGION_SIZE(size) (sizeof(struct kasan_region_s) + KASAN_SHADOW_SIZE(size))
...
static inline_function FAR uint8_t *kasan_mem_to_shadow(FAR const void *ptr, size_t size)
{
	uintptr_t addr;
	int i;
...
static inline_function bool kasan_is_poisoned(FAR const void *addr, size_t size)
{
	FAR uint8_t *p;
	uint8_t tag;
...
static void kasan_set_poison(FAR const void *addr, size_t size, uint8_t value)
{
	irqstate_t flags;
	FAR uint8_t *p;
...
{
	return (FAR void *)((uint64_t)kasan_clear_tag(addr) | (((uint64_t)tag) << KASAN_TAG_SHIFT));
}
...
{
	return (FAR void *)(((uint64_t)addr) & ~((uint64_t)0xff << KASAN_TAG_SHIFT));
}
