...
typedef void (*mempool_callback_t)(FAR struct mempool_s *pool, FAR struct mempool_backtrace_s *buf,
...
static inline FAR sq_entry_t *mempool_remove_queue(FAR struct mempool_s *pool,
...
static inline void mempool_add_queue(FAR struct mempool_s *pool, FAR sq_queue_t *queue,
									 FAR char *base, size_t nblks, size_t blocksize)
{
	while (nblks-- > 0) {
#if CONFIG_MM_BACKTRACE >= 0
		FAR struct mempool_backtrace_s *buf =
			(FAR struct mempool_backtrace_s *)(base + nblks * blocksize + pool->blocksize);
										 ...
										 FAR struct mempool_backtrace_s *buf)
{
	...
	if (pool->procfs.backtrace) {
		int result = sched_backtrace(buf->pid, buf->backtrace, CONFIG_MM_BACKTRACE,
...
static void mempool_foreach(FAR struct mempool_s *pool, mempool_callback_t callback,
							FAR const void *input, FAR void *output)
{
	...
	if (pool->ibase != NULL) {
		...
		while (nblks--) {
			buf =
				(FAR struct mempool_backtrace_s *)pool->ibase + nblks * blocksize + pool->blocksize;
	...
	sq_for_every(&pool->equeue, entry)
	{
		...
		while (nblks--) {
			buf = (FAR struct mempool_backtrace_s *)(base + nblks * blocksize + pool->blocksize);
			callback(pool, buf, input, output);
		}
	}
}
...
static void mempool_info_task_callback(FAR struct mempool_s *pool,
									   FAR struct mempool_backtrace_s *buf, FAR const void *input,
									   FAR void *output)
{
	size_t blocksize = MEMPOOL_REALBLOCKSIZE(pool);
									 ...
									 FAR void *output)
{
	...
	if ((MM_DUMP_ASSIGN(dump, buf) || MM_DUMP_ALLOC(dump, buf) || MM_DUMP_LEAK(dump, buf)) &&
		...
		syslog(LOG_INFO, "%6d%12zu%9zu%12lu%*p %s\n", buf->pid, blocksize, overhead, buf->seqno,
			   BACKTRACE_PTR_FMT_WIDTH, ((FAR char *)buf - pool->blocksize), tmp);
	}
}

static void mempool_memdump_free_callback(FAR struct mempool_s *pool,
										  FAR struct mempool_backtrace_s *buf,
										  FAR const void *input, FAR void *output)
{
	size_t blocksize = MEMPOOL_REALBLOCKSIZE(pool);
...
int mempool_init(FAR struct mempool_s *pool, FAR const char *name)
{
	...
	if (pool->interruptsize >= blocksize) {
		...
		mempool_add_queue(pool, &pool->iqueue, pool->ibase, ninterrupt, blocksize);
	...
	if (pool->initialsize >= blocksize + MEMPOOL_HEADER_SIZE) {
		size_t ninitial = (pool->initialsize - MEMPOOL_HEADER_SIZE) / blocksize;
		...
		sq_addlast((FAR sq_entry_t *)(base + ninitial * blocksize), &pool->equeue);
	...
	if (blk == NULL) {
		if (up_interrupt_context()) {
			...
			if (pool->expandsize >= blocksize + MEMPOOL_HEADER_SIZE) {
				size_t nexpand = (pool->expandsize - MEMPOOL_HEADER_SIZE) / blocksize;
				...
				kasan_poison(base, size);
				flags = spin_lock_irqsave(&pool->lock);
				mempool_add_queue(pool, &pool->queue, base, nexpand, blocksize);
				sq_addlast((FAR sq_entry_t *)(base + nexpand * blocksize), &pool->equeue);
				blk = mempool_remove_queue(pool, &pool->queue);
			} else if (!pool->wait || nxsem_wait_uninterruptible(&pool->waitsem) < 0) {
				return NULL;
			} else {
				goto retry;
			}
...
struct mallinfo_task mempool_info_task(FAR struct mempool_s *pool, FAR const struct malltask *task)
{
	size_t blocksize = MEMPOOL_REALBLOCKSIZE(pool);
	...
	if (task->pid == PID_MM_FREE) {
		irqstate_t flags = spin_lock_irqsave(&pool->lock);
		size_t count = sq_count(&pool->queue) + sq_count(&pool->iqueue);
...
void mempool_memdump(FAR struct mempool_s *pool, FAR const struct mm_memdump_s *dump)
{
#if CONFIG_MM_BACKTRACE >= 0
...
int mempool_deinit(FAR struct mempool_s *pool)
{
	...
	if (pool->ibase) {
		pool->ibase = kasan_unpoison(pool->ibase, pool->interruptsize / blocksize * blocksize);
		pool->free(pool, pool->ibase);
	}
