...
static inline FAR struct mempool_s *mempool_multiple_find(FAR struct mempool_multiple_s *mpool,
														  size_t size)
{
	size_t right;
	size_t left = 0;
...
static FAR void *mempool_multiple_alloc_chunk(FAR struct mempool_multiple_s *mpool, size_t align,
											  size_t size)
{
...
retry:
		tmp = mpool->alloc(mpool->arg, mpool->expandsize,
						   mpool->chunk_size + sizeof(struct mpool_chunk_s));
...
static void mempool_multiple_free_chunk(FAR struct mempool_multiple_s *mpool, FAR void *ptr)
{
	FAR struct mpool_chunk_s *chunk;
...
static FAR void *mempool_multiple_alloc_callback(FAR struct mempool_s *pool, size_t size)
{
	...
	nxrmutex_lock(&mpool->lock);
	ret = mempool_multiple_alloc_chunk(mpool, mpool->expandsize, mpool->minpoolsize + size);
	if (ret == NULL) {
		nxrmutex_unlock(&mpool->lock);
		return NULL;
	}
	...
	if (mpool->dict[row] == NULL) {
		mpool->dict[row] = mempool_multiple_alloc_chunk(mpool, sizeof(uintptr_t),
...
static void mempool_multiple_free_callback(FAR struct mempool_s *pool, FAR void *addr)
{
	FAR struct mempool_multiple_s *mpool = pool->priv;

	mempool_multiple_free_chunk(mpool, (FAR char *)addr - mpool->minpoolsize);
}
...
static FAR struct mpool_dict_s *mempool_multiple_get_dict(FAR struct mempool_multiple_s *mpool,
														  FAR void *blk)
{
	FAR void *addr;
	size_t index;
	...
	if (kasan_clear_tag(mpool->dict[row]) == NULL ||
		...
		((FAR char *)kasan_clear_tag(blk) - (FAR char *)addr >= mpool->dict[row][col].size)) {
		return NULL;
	}
...
static void mempool_multiple_check(FAR struct mempool_s *pool, FAR void *blk)
{
...
mempool_multiple_init(FAR const char *name, FAR const size_t *poolsize, size_t npools,
					  mempool_multiple_alloc_t alloc, mempool_multiple_alloc_size_t alloc_size,
					  mempool_multiple_free_t free, FAR void *arg, size_t chunksize,
					  size_t expandsize, size_t dict_expendsize)
{
	...
	mpool = alloc(arg, sizeof(uintptr_t),
				  sizeof(struct mempool_multiple_s) + npools * sizeof(struct mempool_s));

	if (mpool == NULL) {
		return NULL;
	}

	pools = (FAR struct mempool_s *)((uintptr_t)mpool + sizeof(struct mempool_multiple_s));
	...
	mpool->dict_used = 0;
	mpool->dict_col_num_log2 = fls(dict_expendsize / sizeof(struct mpool_dict_s));

	mpool->dict_row_num = dict_expendsize / sizeof(FAR struct mpool_dict_s *);
	mpool->dict =
		mempool_multiple_alloc_chunk(mpool, sizeof(FAR struct mpool_dict_s *),
									 sizeof(FAR struct mpool_dict_s *) * mpool->dict_row_num);
	if (mpool->dict == NULL) {
		goto err_with_pools;
	}

	memset(mpool->dict, 0, mpool->dict_row_num * sizeof(FAR struct mpool_dict_s *));
...
FAR void *mempool_multiple_alloc(FAR struct mempool_multiple_s *mpool, size_t size)
{
...
FAR void *mempool_multiple_realloc(FAR struct mempool_multiple_s *mpool, FAR void *oldblk,
								   size_t size)
{
	FAR struct mpool_dict_s *dict;
	FAR void *blk;
...
int mempool_multiple_free(FAR struct mempool_multiple_s *mpool, FAR void *blk)
{
	FAR struct mpool_dict_s *dict;
	...
	blk = (FAR char *)blk - (((FAR char *)kasan_clear_tag(blk) -
							  ((FAR char *)kasan_clear_tag(dict->addr) + mpool->minpoolsize)) %
...
ssize_t mempool_multiple_alloc_size(FAR struct mempool_multiple_s *mpool, FAR void *blk)
{
	FAR struct mpool_dict_s *dict;
...
FAR void *mempool_multiple_memalign(FAR struct mempool_multiple_s *mpool, size_t alignment,
									size_t size)
{
	FAR struct mempool_s *end;
	FAR struct mempool_s *pool;
...
void mempool_multiple_foreach(FAR struct mempool_multiple_s *mpool,
							  mempool_multiple_foreach_t handle, FAR void *arg)
{
	size_t i;
...
struct mallinfo mempool_multiple_mallinfo(FAR struct mempool_multiple_s *mpool)
{
	...
	for (i = 0; i < mpool->npools; i++) {
		...
		info.fordblks += (poolinfo.ordblks + poolinfo.iordblks) * poolinfo.sizeblks;
...
struct mallinfo_task mempool_multiple_info_task(FAR struct mempool_multiple_s *mpool,
												FAR const struct malltask *task)
{
	int i;
	struct mallinfo_task info;
	struct mallinfo_task ret = {0, 0};
