...
static int mempool_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
...
static ssize_t mempool_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static ssize_t mempool_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	linesize = procfs_snprintf(procfile->line, MEMPOOLINFO_LINELEN, "%13s%11s%9s%9s%9s%9s%9s\n", "",
	...
	for (entry = g_mempool_procfs; entry != NULL; entry = entry->next) {
		if (totalsize < buflen) {
			FAR struct mempool_s *pool = container_of(entry, struct mempool_s, procfs);
			...
			linesize = procfs_snprintf(procfile->line, MEMPOOLINFO_LINELEN,
									   ...
									   minfo.sizeblks, minfo.aordblks, minfo.ordblks,
									   minfo.iordblks, minfo.nwaiter);
			copysize = procfs_memcpy(procfile->line, linesize, buffer, buflen, &offset);
			totalsize += copysize;
		}
	}
