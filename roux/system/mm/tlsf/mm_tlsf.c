...
#include <roux/arch.h>
#include <roux/fs/procfs.h>
#include <roux/mutex.h>
#include <roux/mm/mm.h>
#include <roux/mm/kasan.h>
#include <roux/mm/mempool.h>
#include <roux/sched_note.h>
...
#ifdef CONFIG_MM_HEAP_MEMPOOL
static inline_function void memdump_info_pool(FAR struct mm_memdump_priv_s *priv,
											  FAR struct mm_heap_s *heap)
{
	priv->info = mempool_multiple_info_task(heap->mm_mpool, priv->dump);
}

static inline_function void memdump_dump_pool(FAR struct mm_memdump_priv_s *priv,
											  FAR struct mm_heap_s *heap)
{
...
static void memdump_allocnode(FAR void *ptr, size_t size)
{
	...
	FAR struct memdump_backtrace_s *buf = ptr + size - sizeof(struct memdump_backtrace_s);
	...
	backtrace_format(tmp, sizeof(tmp), buf->backtrace, CONFIG_MM_BACKTRACE);
...
static void memdump_record_biggest(FAR struct mm_memdump_priv_s *priv, FAR void *ptr, size_t size)
{
	...
	if (priv->filled > 1) {
		qsort(priv->node, priv->filled, sizeof(struct mm_tlsf_node_s), memdump_record_comare);
	}
}
...
static void memdump_backtrace(FAR struct mm_heap_s *heap, FAR struct memdump_backtrace_s *buf)
{
#if CONFIG_MM_BACKTRACE > 0
	...
	if (heap->mm_procfs.backtrace || (tcb && tcb->flags & TCB_FLAG_HEAP_DUMP)) {
		int ret = sched_backtrace(buf->pid, buf->backtrace, CONFIG_MM_BACKTRACE,
...
#if CONFIG_MM_FREE_DELAYCOUNT_MAX > 0
	if (tmp == NULL ||
		(!force && heap->mm_delaycount[this_cpu()] < CONFIG_MM_FREE_DELAYCOUNT_MAX)) {
		mm_unlock_irq(heap, flags);
		return false;
	}
...
static FAR void *mempool_memalign(FAR void *arg, size_t alignment, size_t size)
{
...
static void mallinfo_handler(FAR void *ptr, size_t size, int used, FAR void *user)
{
...
static void mallinfo_task_handler(FAR void *ptr, size_t size, int used, FAR void *user)
{
...
#define buf NULL
		...
		if ((MM_DUMP_ASSIGN(task, buf) || MM_DUMP_ALLOC(task, buf) || MM_DUMP_LEAK(task, buf)) &&
			MM_DUMP_SEQNO(task, buf)) {
			info->aordblks++;
			info->uordblks += size;
		}
#undef buf
...
static void memdump_handler(FAR void *ptr, size_t size, int used, FAR void *user)
{
...
#define buf NULL
#endif
		if ((MM_DUMP_ASSIGN(dump, buf) || MM_DUMP_ALLOC(dump, buf) || MM_DUMP_LEAK(dump, buf)) &&
			MM_DUMP_SEQNO(dump, buf)) {
			priv->info.aordblks++;
...
static void mm_delayfree(FAR struct mm_heap_s *heap, FAR void *mem, bool delay)
{
	if (mm_lock(heap) == 0) {
...
#undef idx
	...
	sched_note_heap(NOTE_HEAP_ADD, heap, heapstart, heapsize, heap->mm_curused);
	mm_unlock(heap);
}
...
bool mm_heapmember(FAR struct mm_heap_s *heap, FAR void *mem)
{
	...
	for (i = 0; i < heap->mm_nregions; i++) {
		if (mem >= heap->mm_heapstart[i] && mem < heap->mm_heapend[i]) {
			return true;
		}
	}
	...
	if (mem >= heap->mm_heapstart[0] && mem < heap->mm_heapend[0]) {
		return true;
	}
...
#ifdef CONFIG_MM_HEAP_MEMPOOL
FAR struct mm_heap_s *mm_initialize_pool(FAR const char *name, FAR void *heap_start,
										 size_t heap_size, FAR const struct mempool_init_s *init)
{
	FAR struct mm_heap_s *heap;
	...
	if (init != NULL && init->poolsize != NULL && init->npools != 0) {
		...
		heap->mm_mpool = mempool_multiple_init(
			name, init->poolsize, init->npools, (mempool_multiple_alloc_t)mempool_memalign,
			(mempool_multiple_alloc_size_t)mm_malloc_size, (mempool_multiple_free_t)mm_free, heap,
			init->chunksize, init->expandsize, init->dict_expendsize);
	}
...
#define region 0
...
#endif
	{
		...
		DEBUGVERIFY(mm_lock(heap));
		tlsf_walk_pool(heap->mm_heapstart[region], mallinfo_handler, &info);
		mm_unlock(heap);
	}
#undef region
...
struct mallinfo_task mm_mallinfo_task(FAR struct mm_heap_s *heap, FAR const struct malltask *task)
{
	struct mm_mallinfo_handler_s handle;
	struct mallinfo_task info = {0, 0};
...
#define region 0
...
#endif
	{
		...
		DEBUGVERIFY(mm_lock(heap));
		tlsf_walk_pool(heap->mm_heapstart[region], mallinfo_task_handler, &handle);
		mm_unlock(heap);
	}
#undef region
...
void mm_memdump(FAR struct mm_heap_s *heap, FAR const struct mm_memdump_s *dump)
{
#if CONFIG_MM_REGIONS > 1
	int region;
#else
#define region 0
	...
	if (pid == PID_MM_MEMPOOL) {
		...
		if (tcb == NULL) {
			...
			syslog(LOG_INFO,
				   "Memdump task stack_alloc_ptr: %p,"
...
#else
	syslog(LOG_INFO, "%6s%12s%12s%*s %s\n", "PID", "Size", "Sequence", BACKTRACE_PTR_FMT_WIDTH,
		   "Address", "Backtrace");
#endif
	...
	{
		DEBUGVERIFY(mm_lock(heap));
		tlsf_walk_pool(heap->mm_heapstart[region], memdump_handler, &priv);
		mm_unlock(heap);
	}
#undef region
...
FAR void *mm_malloc(FAR struct mm_heap_s *heap, size_t size)
{
	...
	DEBUGVERIFY(mm_lock(heap));
#if CONFIG_MM_BACKTRACE >= 0
	ret = tlsf_malloc(heap->mm_tlsf, size + sizeof(struct memdump_backtrace_s));
#else
	...
	if (ret) {
		sched_note_heap(NOTE_HEAP_ALLOC, heap, ret, nodesize, heap->mm_curused);
	}
...
FAR void *mm_memalign(FAR struct mm_heap_s *heap, size_t alignment, size_t size)
{
	...
	DEBUGVERIFY(mm_lock(heap));
#if CONFIG_MM_BACKTRACE >= 0
	ret = tlsf_memalign(heap->mm_tlsf, alignment, size + sizeof(struct memdump_backtrace_s));
#else
...
FAR void *mm_realloc(FAR struct mm_heap_s *heap, FAR void *oldmem, size_t size)
{
...
#if CONFIG_MM_BACKTRACE >= 0
	newmem = tlsf_realloc(heap->mm_tlsf, oldmem, size + sizeof(struct memdump_backtrace_s));
#else
	...
	if (newmem) {
		sched_note_heap(NOTE_HEAP_FREE, heap, oldmem, oldsize, heap->mm_curused - newsize);
		sched_note_heap(NOTE_HEAP_ALLOC, heap, newmem, newsize, heap->mm_curused);
	}
...
void mm_uninitialize(FAR struct mm_heap_s *heap)
{
	...
	for (i = 0; i < CONFIG_MM_REGIONS; i++) {
		...
		sched_note_heap(NOTE_HEAP_REMOVE, heap, heap->mm_heapstart[i],
						(uintptr_t)heap->mm_heapend[i] - (uintptr_t)heap->mm_heapstart[i],
						heap->mm_curused);
	}
