...
static FAR const char *const g_type_check_kinds[] = {"load of",
													 "store to",
...
static void ubsan_prologue_epilogue(FAR struct source_location *loc, FAR const char *reason)
{
...
static void handle_misaligned_access(FAR struct type_mismatch_data_common *data, uintptr_t ptr)
{
	ubsan_prologue(data->location, "misaligned-access");

	_alert("%s misaligned address %p for type %s\n", g_type_check_kinds[data->type_check_kind],
		   (FAR void *)ptr, data->type->type_name);
	_alert("which requires %ld byte alignment\n", data->alignment);

	ubsan_epilogue();
}

static void handle_object_size_mismatch(FAR struct type_mismatch_data_common *data, uintptr_t ptr)
{
...
static void ubsan_type_mismatch_common(FAR struct type_mismatch_data_common *data, uintptr_t ptr)
{
	if (!ptr) {
...
static int64_t get_signed_val(FAR struct type_descriptor *type, FAR void *val)
{
	if (is_inline_int(type)) {
		...
		return (int64_t)(((ret & (1llu << (bits - 1))) != 0) ? ret | ~mask : ret);
	}
...
static uint64_t get_unsigned_val(FAR struct type_descriptor *type, FAR void *val)
{
	if (is_inline_int(type)) {
...
static void val_to_string(FAR char *str, size_t size, FAR struct type_descriptor *type,
						  FAR void *value)
{
	if (type_is_int(type)) {
...
void __ubsan_handle_divrem_overflow(FAR void *data, FAR void *lhs, FAR void *rhs)
{
...
void __ubsan_handle_alignment_assumption(FAR void *data, uintptr_t ptr, uintptr_t align,
										 uintptr_t offset)
{
	...
	real_ptr = ptr - offset;
	_alert("%saddress is %lu aligned, misalignment offset is %zu bytes", offset ? "offset " : "",
		   1ul << (real_ptr ? ffsl(real_ptr) : 0), real_ptr & (align - 1));

	ubsan_epilogue();
}

void __ubsan_handle_type_mismatch(FAR struct type_mismatch_data *data, FAR void *ptr)
{
...
void __ubsan_handle_add_overflow(FAR void *data, FAR void *lhs, FAR void *rhs)
{
	ubsan_prologue_epilogue(data, "add-overflow");
}

void __ubsan_handle_sub_overflow(FAR void *data, FAR void *lhs, FAR void *rhs)
{
	ubsan_prologue_epilogue(data, "sub-overflow");
}

void __ubsan_handle_mul_overflow(FAR void *data, FAR void *lhs, FAR void *rhs)
{
...
void __ubsan_handle_pointer_overflow(FAR void *data, FAR void *ptr, FAR void *result)
{
...
void __ubsan_handle_dynamic_type_cache_miss(FAR void *data, FAR void *ptr, FAR void *hash)
{
