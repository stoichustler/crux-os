...
static FAR struct iob_s *iob_tryalloc_internal(bool throttled)
{
	...
	int16_t count = (throttled ? g_iob_count - CONFIG_IOB_THROTTLE : g_iob_count);
...
static FAR struct iob_s *iob_allocwait(bool throttled, unsigned int timeout)
{
	...
	if (iob == NULL) {
		...
		if (timeout == UINT_MAX) {
			...
			ret = nxsem_tickwait_uninterruptible(sem, iob_allocwait_gettimeout(start, timeout));
		}
...
FAR struct iob_s *iob_alloc_dynamic(uint16_t size)
{
	...
	if (iob) {
		...
		iob->io_data = (FAR uint8_t *)ALIGN_UP((uintptr_t)(iob + 1), CONFIG_IOB_ALIGNMENT);
	}
