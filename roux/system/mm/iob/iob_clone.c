					  ...
					  int offset2, bool throttled, bool block)
{
	...
	while (iob1 && len > 0) {
		...
		if ((int)(offset2 + iob2->io_offset - IOB_BUFSIZE(iob2)) >= 0 && iob1 != NULL) {
			ret = iob_next(iob2, throttled, block);
			if (ret < 0) {
				return ret;
			}
...
{
	DEBUGASSERT(iob2->io_len == 0 && iob2->io_offset == 0 && iob2->io_pktlen == 0 &&
				iob2->io_flink == NULL);

	return iob_clone_partial(iob1, iob1->io_pktlen, 0, iob2, 0, throttled, block);
}
