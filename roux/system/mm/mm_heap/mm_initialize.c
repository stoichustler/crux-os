...
static FAR void *mempool_memalign(FAR void *arg, size_t alignment, size_t size)
{
	...
	if (ret) {
		node = (FAR struct mm_allocnode_s *)((uintptr_t)ret - MM_SIZEOF_ALLOCNODE);
...
#define idx 0
	...
	heapbase =
		MM_ALIGN_UP((uintptr_t)heapstart + 2 * MM_SIZEOF_ALLOCNODE) - 2 * MM_SIZEOF_ALLOCNODE;
	...
	heap->mm_heapend[idx] = (FAR struct mm_allocnode_s *)(heapend - MM_SIZEOF_ALLOCNODE);
	heap->mm_heapend[idx]->size = MM_SIZEOF_ALLOCNODE | MM_ALLOC_BIT | MM_PREVFREE_BIT;
