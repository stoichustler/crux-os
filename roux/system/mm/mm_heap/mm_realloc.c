...
FAR void *mm_realloc(FAR struct mm_heap_s *heap, FAR void *oldmem, size_t size)
{
	...
	oldnode =
		(FAR struct mm_allocnode_s *)((FAR char *)kasan_clear_tag(oldmem) - MM_SIZEOF_ALLOCNODE);
	...
	if (newsize <= oldsize) {
		...
		if (newsize < oldsize) {
			...
			kasan_poison((FAR char *)oldnode + MM_SIZEOF_NODE(oldnode) + sizeof(mmsize_t) -
	...
	if (MM_PREVNODE_IS_FREE(oldnode)) {
		prev = (FAR struct mm_freenode_s *)((FAR char *)oldnode - oldnode->preceding);
	...
	if (nextsize + prevsize + oldsize >= newsize) {
		...
		if (takeprev) {
			...
			if (takeprev < prevsize) {
				...
				nodesize += prevsize;
				newnode->size = nodesize | MM_ALLOC_BIT | (newnode->size & MM_MASK_BIT);
			}
		...
		if (takenext) {
			...
			if (takenext < nextsize) {
				...
				newnode = (FAR struct mm_freenode_s *)((FAR char *)oldnode + nodesize);
