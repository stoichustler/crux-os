...
#define region 0
...
#endif
	{
		...
		for (node = heap->mm_heapstart[region]; node < heap->mm_heapend[region];
			...
			minfo("region=%d node=%p size=%zu preceding=%u (%c %c)\n", region, node, nodesize,
				  (unsigned int)node->preceding, MM_PREVNODE_IS_FREE(node) ? 'F' : 'A',
				  MM_NODE_IS_ALLOC(node) ? 'A' : 'F');

			handler(node, arg);

			DEBUGASSERT(MM_PREVNODE_IS_ALLOC(node) || MM_SIZEOF_NODE(prev) == node->preceding);
			prev = node;
		}
