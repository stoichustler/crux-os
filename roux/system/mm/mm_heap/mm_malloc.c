...
void mm_mempool_dump_handle(FAR struct mempool_s *pool, FAR void *arg)
{
	...
	mwarn("%9lu%11lu%9lu%9lu%9lu%9lu\n", info.sizeblks, info.arena, info.aordblks, info.ordblks,
		  info.iordblks, info.nwaiter);
}
#endif
...
FAR void *mm_malloc(FAR struct mm_heap_s *heap, size_t size)
{
	...
	if (node) {
		...
		if (remaining >= MM_MIN_CHUNK) {
			...
			remainder = (FAR struct mm_freenode_s *)(((FAR char *)node) + alignsize);
	...
	/* Try again after free delay list */
		...
		mwarn("Total:%d, used:%d, free:%d, largest:%d, nused:%d, nfree:%d\n", minfo.arena,
			  minfo.uordblks, minfo.fordblks, minfo.mxordblk, minfo.aordblks, minfo.ordblks);
#if CONFIG_MM_BACKTRACE >= 0
		nxsched_foreach(mm_dump_handler, heap);
		mm_dump_handler(NULL, heap);
#endif
#ifdef CONFIG_MM_HEAP_MEMPOOL
		mwarn("%11s%9s%9s%9s%9s%9s\n", "bsize", "total", "nused", "nfree", "nifree", "nwaiter");
		mempool_multiple_foreach(heap->mm_mpool, mm_mempool_dump_handle, NULL);
#endif
#ifdef CONFIG_MM_DUMP_DETAILS_ON_FAILURE
