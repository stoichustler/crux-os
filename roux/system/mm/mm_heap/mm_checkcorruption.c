...
static void checkcorruption_handler(FAR struct mm_allocnode_s *node, FAR void *arg)
{
	...
	if (MM_NODE_IS_ALLOC(node)) {
		...
		ASSERT(fnode->flink == NULL || fnode->flink->blink == fnode);
		ASSERT(fnode->flink == NULL || MM_SIZEOF_NODE(fnode->flink) == 0 ||
