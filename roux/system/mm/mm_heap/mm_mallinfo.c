...
static void mallinfo_task_handler(FAR struct mm_allocnode_s *node, FAR void *arg)
{
	...
	if (MM_NODE_IS_ALLOC(node)) {
		DEBUGASSERT(nodesize >= MM_SIZEOF_ALLOCNODE);
		if ((MM_DUMP_ASSIGN(task, node) || MM_DUMP_ALLOC(task, node) || MM_DUMP_LEAK(task, node)) &&
			MM_DUMP_SEQNO(task, node)) {
			info->aordblks++;
			info->uordblks += nodesize;
		}
...
size_t mm_heapfree_largest(FAR struct mm_heap_s *heap)
{
	...
	for (node = heap->mm_nodelist[MM_NNODES - 1].blink; node; node = node->blink) {
		size_t nodesize = MM_SIZEOF_NODE(node);
		if (nodesize != 0) {
			return nodesize;
		}
