...
#define MM_ALLOCNODE_OVERHEAD (CONFIG_MM_NODE_GUARDSIZE + MM_SIZEOF_ALLOCNODE - sizeof(mmsize_t))
...
static_assert(MM_SIZEOF_ALLOCNODE <= MM_MIN_CHUNK, "Error size for struct mm_allocnode_s\n");

static_assert(MM_ALIGN >= sizeof(uintptr_t) && (MM_ALIGN & MM_GRAN_MASK) == 0,
...
typedef CODE void (*mm_node_handler_t)(FAR struct mm_allocnode_s *node, FAR void *arg);
...
void mm_foreach(FAR struct mm_heap_s *heap, mm_node_handler_t handler, FAR void *arg);
											...
											FAR struct mm_freenode_s *node)
{
	...
	for (prev = &heap->mm_nodelist[ndx], next = heap->mm_nodelist[ndx].flink;
		 next && next->size && MM_SIZEOF_NODE(next) < nodesize; prev = next, next = next->flink)
		;
