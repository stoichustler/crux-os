...
static void memdump_allocnode(FAR struct mm_allocnode_s *node)
{
	...
	syslog(LOG_INFO, "%6d%12zu%9zu%12lu%*p\n", node->pid, nodesize, overhead, node->seqno,
		   BACKTRACE_PTR_FMT_WIDTH, (FAR const char *)node + MM_SIZEOF_ALLOCNODE);
#else
	char buf[BACKTRACE_BUFFER_SIZE(CONFIG_MM_BACKTRACE)];

	backtrace_format(buf, sizeof(buf), node->backtrace, CONFIG_MM_BACKTRACE);

	syslog(LOG_INFO, "%6d%12zu%9zu%12lu%*p %s\n", node->pid, nodesize, overhead, node->seqno,
		   BACKTRACE_PTR_FMT_WIDTH, (FAR const char *)node + MM_SIZEOF_ALLOCNODE, buf);
#endif
}
								   ...
								   FAR struct mm_allocnode_s *node)
{
	...
	if (priv->filled > 1) {
		qsort(priv->node, priv->filled, sizeof(struct mm_allocnode_s *), memdump_record_comare);
	}
}
...
static void memdump_handler(FAR struct mm_allocnode_s *node, FAR void *arg)
{
	...
	if (MM_NODE_IS_ALLOC(node)) {
		...
		if ((MM_DUMP_ASSIGN(dump, node) || MM_DUMP_ALLOC(dump, node) || MM_DUMP_LEAK(dump, node)) &&
			MM_DUMP_SEQNO(dump, node)) {
			priv->info.aordblks++;
			...
			FAR struct mm_allocnode_s *next =
				(FAR struct mm_allocnode_s *)((FAR char *)node + nodesize);
		...
		DEBUGASSERT(fnode->flink == NULL || fnode->flink->blink == fnode);
		DEBUGASSERT(fnode->flink == NULL || MM_SIZEOF_NODE(fnode->flink) == 0 ||
