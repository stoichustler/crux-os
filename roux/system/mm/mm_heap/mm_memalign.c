...
FAR void *mm_memalign(FAR struct mm_heap_s *heap, size_t alignment, size_t size)
{
	...
	if (alignedchunk != rawchunk) {
		...
		next = (FAR struct mm_allocnode_s *)((FAR char *)node + MM_SIZEOF_NODE(node));

		newnode = (FAR struct mm_allocnode_s *)(alignedchunk - MM_SIZEOF_ALLOCNODE);
	...
	sched_note_heap(NOTE_HEAP_ALLOC, heap, (FAR void *)alignedchunk, size, heap->mm_curused);
	...
	alignedchunk =
		(uintptr_t)kasan_unpoison((FAR const void *)alignedchunk, size - MM_ALLOCNODE_OVERHEAD);
