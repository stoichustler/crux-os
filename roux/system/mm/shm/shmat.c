...
static int munmap_shm(FAR struct task_group_s *group, FAR struct mm_map_entry_s *entry,
					  FAR void *start, size_t length)
{
	...
	if (group) {
		...
		vm_release_region(get_group_mm(group), shmaddr, region->sr_ds.shm_segsz);
...
FAR void *shmat(int shmid, FAR const void *shmaddr, int shmflg)
{
	...
	vaddr = vm_alloc_region(get_group_mm(group), NULL, region->sr_ds.shm_segsz);
	if (vaddr == NULL) {
