...
static int do_test_aes(FAR struct cipher_testvec *test, int mode, int encrypt)
{
	...
	int res =
		aes_cypher(out, test->input, test->ilen, test->iv, test->key, test->klen, mode, encrypt);
	if (res == OK) {
...
static int test_aes(void)
{
	...
	AES_CYPHER_TEST(AES_MODE_ECB, "ECB", nitems(aes_enc_tv_template), nitems(aes_dec_tv_template),
					aes_enc_tv_template, aes_dec_tv_template)
	AES_CYPHER_TEST(AES_MODE_CBC, "CBC", nitems(aes_cbc_enc_tv_template),
					nitems(aes_cbc_dec_tv_template), aes_cbc_enc_tv_template,
					aes_cbc_dec_tv_template)
	AES_CYPHER_TEST(AES_MODE_CTR, "CTR", nitems(aes_ctr_enc_tv_template),
					nitems(aes_ctr_dec_tv_template), aes_ctr_enc_tv_template,
