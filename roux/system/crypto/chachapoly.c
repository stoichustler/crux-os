...
void chacha20_crypt(caddr_t key, FAR uint8_t *data)
{
	...
	chacha_encrypt_bytes((FAR chacha_ctx *)ctx->block, data, data, CHACHA20_BLOCK_LEN);
}
...
void chacha20_poly1305_setkey(FAR void *xctx, FAR const uint8_t *key, uint16_t klen)
{
	FAR CHACHA20_POLY1305_CTX *ctx = xctx;
...
void chacha20_poly1305_reinit(FAR void *xctx, FAR const uint8_t *iv, uint16_t ivlen)
{
	FAR CHACHA20_POLY1305_CTX *ctx = xctx;
	...
	chacha_encrypt_bytes((FAR chacha_ctx *)&ctx->chacha, ctx->key, ctx->key, POLY1305_KEYLEN);
	poly1305_begin((FAR poly1305_state *)&ctx->poly, ctx->key);
}

int chacha20_poly1305_update(FAR void *xctx, FAR const uint8_t *data, size_t len)
{
	...
	if (rem > 0) {
		poly1305_update((FAR poly1305_state *)&ctx->poly, zeroes, POLY1305_BLOCK_LEN - rem);
	}
...
void chacha20poly1305_encrypt(FAR uint8_t *dst, FAR const uint8_t *src, const size_t src_len,
							  FAR const uint8_t *ad, const size_t ad_len, const uint64_t nonce,
...
int chacha20poly1305_decrypt(FAR uint8_t *dst, FAR const uint8_t *src, const size_t src_len,
							 FAR const uint8_t *ad, const size_t ad_len, const uint64_t nonce,
							 FAR const uint8_t *key)
{
	poly1305_state poly1305_ctx;
...
void xchacha20poly1305_encrypt(FAR uint8_t *dst, FAR const uint8_t *src, const size_t src_len,
							   FAR const uint8_t *ad, const size_t ad_len, FAR const uint8_t *nonce,
							   FAR const uint8_t *key)
{
	int i;
	...
	chacha20poly1305_encrypt(dst, src, src_len, ad, ad_len, h_nonce, (FAR uint8_t *)derived_key);
	explicit_bzero(derived_key, CHACHA20POLY1305_KEY_SIZE);
}

int xchacha20poly1305_decrypt(FAR uint8_t *dst, FAR const uint8_t *src, const size_t src_len,
							  FAR const uint8_t *ad, const size_t ad_len, FAR const uint8_t *nonce,
							  FAR const uint8_t *key)
{
	int ret;
	...
	ret = chacha20poly1305_decrypt(dst, src, src_len, ad, ad_len, h_nonce,
								   (FAR uint8_t *)derived_key);
	explicit_bzero(derived_key, CHACHA20POLY1305_KEY_SIZE);
