...
const struct enc_xform enc_xform_3des = {
	CRYPTO_3DES_CBC, "3DES", 8, 8, 24, 24, 384, des3_encrypt, des3_decrypt, des3_setkey, NULL};

const struct enc_xform enc_xform_blf = {
	CRYPTO_BLF_CBC,	 "Blowfish",  8,		   8,		   5,	56 /* 448 bits, max key */,
	sizeof(blf_ctx), blf_encrypt, blf_decrypt, blf_setkey, NULL};

const struct enc_xform enc_xform_cast5 = {
	CRYPTO_CAST_CBC, "CAST-128",   8,	8, 5, 16, sizeof(cast_key), cast5_encrypt,
	cast5_decrypt,	 cast5_setkey, NULL};

const struct enc_xform enc_xform_aes = {
	CRYPTO_AES_CBC,	  "AES", 16, 16, 16, 32, sizeof(AES_CTX), aes_encrypt_xform, aes_decrypt_xform,
	aes_setkey_xform, NULL};

const struct enc_xform enc_xform_aes_ctr = {CRYPTO_AES_CTR,
											"AES-CTR",
											16,
											8,
											16 + 4,
...
const struct enc_xform enc_xform_aes_gcm = {CRYPTO_AES_GCM_16,
											"AES-GCM",
											1,
											8,
											16 + 4,
...
const struct enc_xform enc_xform_aes_gmac = {
	CRYPTO_AES_GMAC, "AES-GMAC", 1, 8, 16 + 4, 32 + 4, 0, NULL, NULL, NULL, NULL};

const struct enc_xform enc_xform_aes_cmac = {
	CRYPTO_AES_CMAC, "AES-CMAC", 1, 0, 16, 32, 0, null_encrypt, null_decrypt, null_setkey, NULL};

const struct enc_xform enc_xform_aes_xts = {CRYPTO_AES_XTS,
											"AES-XTS",
											16,
											8,
											32,
...
const struct enc_xform enc_xform_aes_ofb = {CRYPTO_AES_OFB,
											"AES-OFB",
											16,
											16,
											16,
...
const struct enc_xform enc_xform_aes_cfb_8 = {CRYPTO_AES_CFB_8,
											  "AES-CFB-8",
											  16,
											  16,
											  16,
...
const struct enc_xform enc_xform_aes_cfb_128 = {CRYPTO_AES_CFB_128,
												"AES-CFB-128",
												16,
												16,
												16,
...
const struct enc_xform enc_xform_chacha20_poly1305 = {CRYPTO_CHACHA20_POLY1305,
													  "CHACHA20-POLY1305",
													  1,
													  8,
													  32 + 4,
...
const struct enc_xform enc_xform_null = {CRYPTO_NULL,  "NULL",		 4,			  0,   0, 256, 0,
										 null_encrypt, null_decrypt, null_setkey, NULL};

/* Authentication instances */

const struct auth_hash auth_hash_hmac_md5_96 = {CRYPTO_MD5_HMAC,
												"HMAC-MD5",
												16,
												16,
												12,
												sizeof(MD5_CTX),
												HMAC_MD5_BLOCK_LEN,
												(void (*)(FAR void *))md5init,
												NULL,
												NULL,
												md5update_int,
												(void (*)(FAR uint8_t *, FAR void *))md5final};

const struct auth_hash auth_hash_hmac_sha1_96 = {CRYPTO_SHA1_HMAC,
												 "HMAC-SHA1",
												 20,
												 20,
												 12,
...
const struct auth_hash auth_hash_gmac_aes_128 = {
	CRYPTO_AES_128_GMAC, "GMAC-AES-128",	   16 + 4,			 GMAC_BLOCK_LEN,
	GMAC_DIGEST_LEN,	 sizeof(AES_GMAC_CTX), AESCTR_BLOCKSIZE, aes_gmac_init,
	aes_gmac_setkey,	 aes_gmac_reinit,	   aes_gmac_update,	 aes_gmac_final};

const struct auth_hash auth_hash_gmac_aes_192 = {
	CRYPTO_AES_192_GMAC, "GMAC-AES-192",	   24 + 4,			 GMAC_BLOCK_LEN,
	GMAC_DIGEST_LEN,	 sizeof(AES_GMAC_CTX), AESCTR_BLOCKSIZE, aes_gmac_init,
	aes_gmac_setkey,	 aes_gmac_reinit,	   aes_gmac_update,	 aes_gmac_final};

const struct auth_hash auth_hash_gmac_aes_256 = {
	CRYPTO_AES_256_GMAC, "GMAC-AES-256",	   32 + 4,			 GMAC_BLOCK_LEN,
	GMAC_DIGEST_LEN,	 sizeof(AES_GMAC_CTX), AESCTR_BLOCKSIZE, aes_gmac_init,
	aes_gmac_setkey,	 aes_gmac_reinit,	   aes_gmac_update,	 aes_gmac_final};

const struct auth_hash auth_hash_chacha20_poly1305 = {
	CRYPTO_CHACHA20_POLY1305_MAC, "CHACHA20-POLY1305",		CHACHA20_KEYSIZE + CHACHA20_SALT,
	POLY1305_BLOCK_LEN,			  POLY1305_TAGLEN,			sizeof(CHACHA20_POLY1305_CTX),
	CHACHA20_BLOCK_LEN,			  chacha20_poly1305_init,	chacha20_poly1305_setkey,
	chacha20_poly1305_reinit,	  chacha20_poly1305_update, chacha20_poly1305_final};
...
const struct auth_hash auth_hash_cmac_aes_128 = {
	...
	sizeof(AES_CMAC_CTX),
	AESCTR_BLOCKSIZE,
	(void (*)(FAR void *))aes_cmac_init,
	(void (*)(FAR void *, FAR const uint8_t *, uint16_t))aes_cmac_setkey,
	NULL,
	(int (*)(FAR void *, FAR const uint8_t *, size_t))aes_cmac_update,
	(void (*)(FAR uint8_t *, FAR void *))aes_cmac_final};

const struct auth_hash auth_hash_md5 = {CRYPTO_MD5,
										"MD5",
										0,
										16,
										16,
...
const struct auth_hash auth_hash_poly1305 = {CRYPTO_POLY1305,
											 "POLY1305",
											 0,
											 16,
											 16,
											 ...
											 poly1305_block_size,
											 (void (*)(FAR void *))null_init,
											 poly1305_setkey,
											 NULL,
											 poly1305update_int,
											 (void (*)(FAR uint8_t *, FAR void *))poly1305_final};

const struct auth_hash auth_hash_ripemd_160 = {CRYPTO_RIPEMD160,
											   "RIPEMD160",
											   0,
											   20,
											   20,
...
const struct auth_hash auth_hash_sha1 = {CRYPTO_SHA1,
										 "SHA1",
										 0,
										 20,
										 20,
...
const struct auth_hash auth_hash_sha2_224 = {CRYPTO_SHA2_224,
											 "SHA2-224",
											 0,
											 28,
											 16,
											 sizeof(SHA2_CTX),
											 SHA224_BLOCK_LENGTH,
											 (void (*)(FAR void *))sha224init,
											 NULL,
											 NULL,
											 sha224update_int,
											 (void (*)(FAR uint8_t *, FAR void *))sha224final};

const struct auth_hash auth_hash_sha2_256 = {CRYPTO_SHA2_256,
											 "SHA2-256",
											 0,
											 32,
											 16,
...
const struct auth_hash auth_hash_sha2_384 = {CRYPTO_SHA2_384,
											 "SHA2-384",
											 0,
											 48,
											 24,
...
const struct auth_hash auth_hash_sha2_512 = {CRYPTO_SHA2_512,
											 "SHA2-512",
											 0,
											 64,
											 32,
...
const struct auth_hash auth_hash_crc32 = {CRYPTO_CRC32,		"CRC32",   0,		  32,		   0,
										  sizeof(uint32_t), 1,		   null_init, crc32setkey, NULL,
										  crc32update,		crc32final};
...
{
	if (des_set_key(key, sched) < 0 || des_set_key(key + 8, sched + 128) < 0 ||
		des_set_key(key + 16, sched + 256) < 0) {
		return -1;
	}
...
void aes_xts_crypt(FAR struct aes_xts_ctx *ctx, FAR uint8_t *data, u_int do_encrypt)
{
