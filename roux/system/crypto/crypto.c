...
int crypto_newsession(FAR uint64_t *sid, FAR struct cryptoini *cri, int hard)
{
	uint32_t hid;
	uint32_t lid;
	uint32_t hid2 = -1;
	...
	do {
		for (hid = 0; hid < crypto_drivers_num; hid++) {
			...
			if (cpc->cc_newsession == NULL || (cpc->cc_flags & CRYPTOCAP_F_CLEANUP)) {
				continue;
			}
			...
			if (hid2 != -1) {
				...
				if (crypto_drivers[hid].cc_sessions <= crypto_drivers[hid2].cc_sessions) {
					hid2 = hid;
				}
			} else {
...
int crypto_get_driverid(uint8_t flags)
{
	...
	if (crypto_drivers_num == 0) {
		...
		crypto_drivers = kmm_calloc(crypto_drivers_num, sizeof(struct cryptocap));
		if (crypto_drivers == NULL) {
	...
	if (i == crypto_drivers_num) {
		...
		newdrv = kmm_calloc(crypto_drivers_num * 2, sizeof(struct cryptocap));
		if (newdrv == NULL) {
			nxmutex_unlock(&g_crypto_lock);
			return -1;
		}

		bcopy(crypto_drivers, newdrv, crypto_drivers_num * sizeof(struct cryptocap));
		bzero(&newdrv[crypto_drivers_num], crypto_drivers_num * sizeof(struct cryptocap));
...
int crypto_kregister(uint32_t driverid, FAR int *kalg, CODE int (*kprocess)(FAR struct cryptkop *))
{
	int i;

	if (driverid >= crypto_drivers_num || kalg == NULL || crypto_drivers == NULL) {
		return -EINVAL;
	}
...
int crypto_register(uint32_t driverid, FAR int *alg,
					CODE int (*newses)(FAR uint32_t *, FAR struct cryptoini *),
					CODE int (*freeses)(uint64_t), CODE int (*process)(FAR struct cryptop *))
{
	int i;

	if (driverid >= crypto_drivers_num || alg == NULL || crypto_drivers == NULL) {
		return -EINVAL;
	}
...
int crypto_unregister(uint32_t driverid, int alg)
{
	...
	if (driverid >= crypto_drivers_num || crypto_drivers == NULL || alg <= 0 ||
...
int crypto_kinvoke(FAR struct cryptkop *krp)
{
	...
	nxmutex_lock(&g_crypto_lock);
	for (hid = 0; hid < crypto_drivers_num; hid++) {
		if ((crypto_drivers[hid].cc_flags & CRYPTOCAP_F_SOFTWARE) && cryptodevallowsoft == 0) {
			continue;
		}
		...
		if ((crypto_drivers[hid].cc_kalg[krp->krp_op] & CRYPTO_ALG_FLAG_SUPPORTED) == 0) {
			continue;
		}
...
int crypto_getfeat(FAR int *featp)
{
	...
	for (hid = 0; hid < crypto_drivers_num; hid++) {
		...
		for (kalg = 0; kalg <= CRK_ALGORITHM_MAX; kalg++) {
			if ((crypto_drivers[hid].cc_kalg[kalg] & CRYPTO_ALG_FLAG_SUPPORTED) != 0) {
				feat |= 1 << kalg;
			}
		}
	}
