...
CODE void (*ghash_update)(FAR GHASH_CTX *, FAR uint8_t *, size_t) = ghash_update_mi;
...
void ghash_update_mi(FAR GHASH_CTX *ctx, FAR uint8_t *X, size_t len)
{
	...
	for (i = 0; i < len / GMAC_BLOCK_LEN; i++) {
		...
		ghash_gfmul((FAR uint32_t *)ctx->S, (FAR uint32_t *)ctx->H, (FAR uint32_t *)ctx->S);
...
int aes_gmac_update(FAR void *xctx, FAR const uint8_t *data, size_t len)
{
	...
	if (len > 0) {
		...
		if (len >= GMAC_BLOCK_LEN) {
			(*ghash_update)(&ctx->ghash, (FAR uint8_t *)data, len - plen);
		}

		if (plen) {
			memcpy((FAR uint8_t *)blk, (FAR uint8_t *)data + (len - plen), plen);
			(*ghash_update)(&ctx->ghash, (FAR uint8_t *)blk, GMAC_BLOCK_LEN);
		}
	}
