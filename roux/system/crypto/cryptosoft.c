...
int swcr_encdec(FAR struct cryptop *crp, FAR struct cryptodesc *crd, FAR struct swcr_data *sw,
				caddr_t buf)
{
...
int swcr_authcompute(FAR struct cryptop *crp, FAR struct cryptodesc *crd, FAR struct swcr_data *sw,
					 caddr_t buf)
{
...
int swcr_hash(FAR struct cryptop *crp, FAR struct cryptodesc *crd, FAR struct swcr_data *sw,
			  caddr_t buf)
{
	FAR const struct auth_hash *axf = sw->sw_axf;

	if (crd->crd_flags & CRD_F_UPDATE) {
		return axf->update(&sw->sw_ctx, (FAR uint8_t *)buf + crd->crd_skip, crd->crd_len);
	} else {
		axf->final((FAR uint8_t *)crp->crp_mac, &sw->sw_ctx);
	}
...
int swcr_authenc(FAR struct cryptop *crp)
{
	...
	for (crd = crp->crp_desc; crd; crd = crd->crd_next) {
		for (sw = swcr_sessions[crp->crp_sid & 0xffffffff]; sw && sw->sw_alg != crd->crd_alg;
			 sw = sw->sw_next)
			;
...
int swcr_compdec(FAR struct cryptodesc *crd, FAR struct swcr_data *sw, caddr_t buf, int outtype)
{
	FAR uint8_t *data;
	FAR uint8_t *out;
...
int swcr_newsession(FAR uint32_t *sid, FAR struct cryptoini *cri)
{
	...
	if (swcr_sessions == NULL || i == swcr_sesnum) {
		...
		if (swcr_sessions) {
			bcopy(swcr_sessions, swd, (swcr_sesnum / 2) * sizeof(struct swcr_data *));
...
enccommon:
			...
			if (cri->cri_klen / 8 > txf->maxkey || cri->cri_klen / 8 < txf->minkey) {
				swcr_freesession(i);
				return -EINVAL;
			}

			if (txf->setkey((*swd)->sw_kschedule, (FAR uint8_t *)cri->cri_key, cri->cri_klen / 8) <
				0) {
				swcr_freesession(i);
				return -EINVAL;
			}
...
authcommon:
			...
			axf->update((*swd)->sw_ictx, (FAR uint8_t *)cri->cri_key, cri->cri_klen / 8);
			axf->update((*swd)->sw_ictx, hmac_ipad_buffer, axf->blocksize - (cri->cri_klen / 8));
			...
			axf->update((*swd)->sw_octx, (FAR uint8_t *)cri->cri_key, cri->cri_klen / 8);
			axf->update((*swd)->sw_octx, hmac_opad_buffer, axf->blocksize - (cri->cri_klen / 8));
...
auth3common:
			...
			if (cri->cri_sid != -1) {
				...
				bcopy(&swcr_sessions[cri->cri_sid]->sw_ctx, &(*swd)->sw_ctx, axf->ctxsize);
			}
			break;
...
auth4common:
			...
			axf->init((*swd)->sw_ictx);
			axf->setkey((*swd)->sw_ictx, (FAR uint8_t *)cri->cri_key, cri->cri_klen / 8);
			bcopy((*swd)->sw_ictx, &(*swd)->sw_ctx, axf->ctxsize);
			(*swd)->sw_axf = axf;
			break;
...
int swcr_freesession(uint64_t tid)
{
	...
	if (sid > swcr_sesnum || swcr_sessions == NULL || swcr_sessions[sid] == NULL) {
		return -EINVAL;
	}
...
int swcr_process(struct cryptop *crp)
{
	...
	for (crd = crp->crp_desc; crd; crd = crd->crd_next) {
		...
		for (sw = swcr_sessions[lid]; sw && sw->sw_alg != crd->crd_alg; sw = sw->sw_next)
			;
		...
		case CRYPTO_AES_CFB_128:
			...
			if ((crp->crp_etype = swcr_encdec(crp, crd, sw, crp->crp_buf)) != 0) {
				goto done;
			}
		...
		case CRYPTO_SHA2_384_HMAC:
		case CRYPTO_SHA2_512_HMAC:
			if ((crp->crp_etype = swcr_authcompute(crp, crd, sw, crp->crp_buf)) != 0) {
				goto done;
			}
		...
		case CRYPTO_SHA2_512:
		case CRYPTO_CRC32:
			if ((crp->crp_etype = swcr_hash(crp, crd, sw, crp->crp_buf)) != 0) {
				goto done;
			}
...
int swcr_rsa_verify(struct cryptkop *krp)
{
	...
	pow_mod_faster(&a, &e, &n, &r);
	return !!memcmp(r.array, hash, hash_len) + !!memcmp(r.array + hash_len, padding, padding_len);
}
...
void swcr_init(void)
{
	...
	int flags = CRYPTOCAP_F_SOFTWARE | CRYPTOCAP_F_ENCRYPT_MAC | CRYPTOCAP_F_MAC_ENCRYPT;
