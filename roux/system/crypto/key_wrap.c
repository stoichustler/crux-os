...
void aes_key_wrap_set_key(FAR aes_key_wrap_ctx *ctx, FAR const uint8_t *K, size_t k_len)
{
	aes_setkey(&ctx->ctx, K, k_len);
}

void aes_key_wrap_set_key_wrap_only(FAR aes_key_wrap_ctx *ctx, FAR const uint8_t *K, size_t k_len)
{
	aes_setkey(&ctx->ctx, K, k_len);
}

void aes_key_wrap(FAR aes_key_wrap_ctx *ctx, FAR const uint8_t *P, size_t n, FAR uint8_t *C)
{
	uint64_t B[2];
	uint64_t t;
	FAR uint8_t *A;
...
int aes_key_unwrap(FAR aes_key_wrap_ctx *ctx, FAR const uint8_t *C, FAR uint8_t *P, size_t n)
{
	uint64_t B[2];
	uint64_t t;
	uint8_t A[8];
