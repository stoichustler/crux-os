...
void aes_cmac_setkey(FAR AES_CMAC_CTX *ctx, FAR const uint8_t *key)
{
	aes_setkey(&ctx->aesctx, key, 16);
}

void aes_cmac_update(FAR AES_CMAC_CTX *ctx, FAR const uint8_t *data, u_int len)
{
	u_int mlen;
...
void aes_cmac_final(FAR uint8_t *digest, FAR AES_CMAC_CTX *ctx)
{
	uint8_t K[16];
