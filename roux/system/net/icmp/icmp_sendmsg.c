...
static void sendto_request(FAR struct net_driver_s *dev, FAR struct icmp_sendto_s *pstate)
{
	...
	ipv4_build_header(IPv4BUF, dev->d_len, IP_PROTO_ICMP, &dev->d_ipaddr, &pstate->snd_toaddr,
...
ssize_t icmp_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
	...
	if (psock->s_type != SOCK_RAW &&
		(icmp->type != ICMP_ECHO_REQUEST || icmp->id != conn->id || dev != conn->dev)) {
		conn->id = 0;
		conn->dev = NULL;
	...
	if (state.snd_cb != NULL) {
		...
		if (ret < 0) {
			if (ret == -ETIMEDOUT) {
				...
				if (!net_ipv4addr_maskcmp(state.snd_toaddr, dev->d_ipaddr, dev->d_netmask)) {
