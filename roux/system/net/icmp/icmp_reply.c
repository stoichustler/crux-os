...
void icmp_reply(FAR struct net_driver_s *dev, int type, int code)
{
	...
	if (net_ipv4addr_hdrcmp(ipv4->destipaddr, &any)
		...
		net_ipv4addr_broadcast(net_ip4addr_conv32(ipv4->destipaddr), dev->d_netmask)
#endif /* CONFIG_NET_BROADCAST */
	) {
		dev->d_len = 0;
		return;
	...
	if (CONFIG_IOB_BUFSIZE >= datalen + ipicmplen + CONFIG_NET_LL_GUARDSIZE) {
		/* Reuse current iob */
		...
		if (iob_trycopyin(dev->d_iob, (FAR void *)ipv4, IPv4_HDRLEN, 0, false) != IPv4_HDRLEN) {
			dev->d_len = 0;
			netdev_iob_release(dev);
			iob_free_chain(iob);
			return;
		...
		iob_update_pktlen(dev->d_iob, dev->d_iob->io_pktlen + sizeof(struct icmp_hdr_s), false);
	...
	ipv4_build_header(IPv4BUF, dev->d_len, IP_PROTO_ICMP, &dev->d_ipaddr,
					  (FAR in_addr_t *)ipv4->srcipaddr, IP_TTL_DEFAULT, 0, NULL);
