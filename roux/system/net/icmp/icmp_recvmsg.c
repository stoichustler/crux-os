...
static inline ssize_t icmp_readahead(FAR struct icmp_conn_s *conn, FAR void *buf, size_t buflen,
									 FAR struct sockaddr_in *from, FAR socklen_t *fromlen, bool raw)
{
	...
	if ((iob = iob_peek_queue(&conn->readahead)) != NULL) {
		...
		if (!raw) {
			FAR struct ipv4_hdr_s *ipv4 = (FAR struct ipv4_hdr_s *)IOB_DATA(iob);
...
ssize_t icmp_recvmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
	...
	if (!IOB_QEMPTY(&conn->readahead)) {
		ret = icmp_readahead(conn, buf, len, (FAR struct sockaddr_in *)from, fromlen,
