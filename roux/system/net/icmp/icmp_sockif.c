...
static int icmp_netpoll(FAR struct socket *psock, FAR struct pollfd *fds, bool setup);
static int icmp_close(FAR struct socket *psock);
#ifdef CONFIG_NET_SOCKOPTS
static int icmp_getsockopt(FAR struct socket *psock, int level, int option, FAR void *value,
						   FAR socklen_t *value_len);
static int icmp_setsockopt(FAR struct socket *psock, int level, int option, FAR const void *value,
						   socklen_t value_len);
#endif
...
static int icmp_setup(FAR struct socket *psock)
{
	...
	if ((psock->s_type == SOCK_DGRAM || psock->s_type == SOCK_CTRL || psock->s_type == SOCK_RAW) &&
...
static int icmp_getsockopt_internal(FAR struct socket *psock, int option, FAR void *value,
									FAR socklen_t *value_len)
{
	int ret;
...
static int icmp_setsockopt_internal(FAR struct socket *psock, int option, FAR const void *value,
									socklen_t value_len)
{
	int ret;
