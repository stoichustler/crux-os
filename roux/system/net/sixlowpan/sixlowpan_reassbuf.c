									  ...
									  FAR const struct netdev_varaddr_s *fragsrc)
{
	...
	if (fragsrc->nv_addrlen == reass->rb_fragsrc.nv_addrlen) {
		...
		return (memcmp(fragsrc->nv_addr, reass->rb_fragsrc.nv_addr, fragsrc->nv_addrlen) == 0);
	}
...
static void sixlowpan_remove_active(FAR struct sixlowpan_reassbuf_s *reass)
{
	...
	for (prev = NULL, curr = g_active_reass; curr != NULL && curr != reass;
...
void sixlowpan_reass_initialize(void)
{
	...
	g_free_reass = NULL;
	for (i = 0, reass = g_metadata_pool; i < CONFIG_NET_6LOWPAN_NREASSBUF; i++, reass++) {
...
sixlowpan_reass_allocate(uint16_t reasstag, FAR const struct netdev_varaddr_s *fragsrc)
{
	...
	if (g_free_reass != NULL) {
		...
		reass =
			(FAR struct sixlowpan_reassbuf_s *)kmm_malloc((sizeof(struct sixlowpan_reassbuf_s)));
													  ...
													  FAR const struct netdev_varaddr_s *fragsrc)
{
	...
	for (reass = g_active_reass; reass != NULL; reass = reass->rb_flink) {
		...
		if (reass->rb_reasstag == reasstag && sixlowpan_compare_fragsrc(reass, fragsrc)) {
			return reass;
		}
	}
