									 ...
									 FAR const uint8_t *buf, uint16_t buflen)
{
	...
	sum = chksum(sum, (FAR uint8_t *)ipv6udp->ipv6.srcipaddr, 2 * sizeof(net_ipv6addr_t));
...
ssize_t psock_6lowpan_udp_sendto(FAR struct socket *psock, FAR const void *buf, size_t buflen,
								 int flags, FAR const struct sockaddr *to, socklen_t tolen)
{
	...
	dev = netdev_findby_ripv6addr(conn->u.ipv6.laddr, to6->sin6_addr.in6_u.u6_addr16);
	if (dev == NULL) {
	...
	ret = sixlowpan_nexthopaddr((FAR struct radio_driver_s *)dev, to6->sin6_addr.in6_u.u6_addr16,
								&destmac);
	if (ret < 0) {
	...
	ret = sixlowpan_send(dev, &conn->sconn.list, &conn->sconn.list_tail,
						 (FAR const struct ipv6_hdr_s *)&ipv6udp, buf, buflen, &destmac,
...
ssize_t psock_6lowpan_udp_send(FAR struct socket *psock, FAR const void *buf, size_t buflen)
{
	FAR struct udp_conn_s *conn;
	...
	return psock_6lowpan_udp_sendto(psock, buf, buflen, 0, (FAR const struct sockaddr *)&to,
...
void sixlowpan_udp_send(FAR struct net_driver_s *dev, FAR struct net_driver_s *fwddev,
						FAR struct ipv6_hdr_s *ipv6)
{
	...
	if (dev != NULL && dev->d_len > 0 && fwddev != NULL) {
		sixlowpan_dumpbuffer("Outgoing UDP packet", (FAR const uint8_t *)ipv6udp, dev->d_len);
		...
		if (ipv6udp->ipv6.proto != IP_PROTO_UDP) {
			...
			if (hdrlen > dev->d_len) {
				...
				sixlowpan_queue_frames((FAR struct radio_driver_s *)fwddev, &ipv6udp->ipv6, buf,
									   buflen, &destmac);
			}
		}
	}
