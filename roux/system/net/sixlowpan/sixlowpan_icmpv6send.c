						   ...
						   FAR struct ipv6_hdr_s *ipv6)
{
	...
	if (dev != NULL && dev->d_len > 0) {
		sixlowpan_dumpbuffer("Outgoing ICMPv6 packet", (FAR const uint8_t *)ipv6icmpv6, dev->d_len);
		...
		if (ipv6icmpv6->ipv6.proto != IP_PROTO_ICMP6) {
			...
			if (hdrlen > dev->d_len) {
				...
				sixlowpan_queue_frames((FAR struct radio_driver_s *)fwddev, &ipv6icmpv6->ipv6, buf,
									   buflen, &destmac);
			}
		}
	}
