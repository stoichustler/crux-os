...
static uint16_t send_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
	...
	if ((flags & WPAN_NEWDATA) == 0) {
		...
		sinfo->s_result = sixlowpan_queue_frames((FAR struct radio_driver_s *)dev, sinfo->s_ipv6hdr,
												 sinfo->s_buf, sinfo->s_len, sinfo->s_destmac);
