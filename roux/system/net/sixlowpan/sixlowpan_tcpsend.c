									 ...
									 FAR const uint8_t *buf, uint16_t buflen)
{
	...
	sum = chksum(sum, (FAR uint8_t *)ipv6tcp->ipv6.srcipaddr, 2 * sizeof(net_ipv6addr_t));
...
static int sixlowpan_tcp_header(FAR struct tcp_conn_s *conn, FAR struct net_driver_s *dev,
								...
								FAR struct ipv6tcp_hdr_s *ipv6tcp)
{
	...
	if (!net_ipv6addr_cmp(conn->u.ipv6.laddr, g_ipv6_unspecaddr)) {
		...
		net_ipv6addr_hdrcopy(ipv6tcp->ipv6.srcipaddr, netdev_ipv6_srcaddr(dev, conn->u.ipv6.raddr));
	}
...
static uint16_t tcp_send_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
									  uint16_t flags)
{
	...
	if ((flags & WPAN_NEWDATA) == 0 && sinfo->s_sent < sinfo->s_buflen) {
		...
		ninfo("s_buflen=%zu s_sent=%zu mss=%u snd_wnd=%u sndlen=%d\n", sinfo->s_buflen,
			  sinfo->s_sent, conn->mss, conn->snd_wnd, sndlen);
		...
		if (sndlen > 0) {
			...
			ret = sixlowpan_tcp_header(conn, dev, &sinfo->s_buf[sinfo->s_sent], sndlen, &ipv6tcp);
			if (ret < 0) {
			...
			ret = sixlowpan_queue_frames((FAR struct radio_driver_s *)dev, &ipv6tcp.ipv6,
										 &sinfo->s_buf[sinfo->s_sent], sndlen, sinfo->s_destmac);
			if (ret < 0) {
			...
			ninfo("Sent: acked=%" PRId32 " sent=%zd "
				  ...
				  sinfo->s_acked, sinfo->s_sent, sinfo->s_buflen, (uint32_t)conn->tx_unacked);
		}
	}
...
static int sixlowpan_send_packet(FAR struct socket *psock, FAR struct net_driver_s *dev,
								 FAR struct tcp_conn_s *conn, FAR const uint8_t *buf, size_t len,
								 FAR const struct netdev_varaddr_s *destmac, unsigned int timeout)
{
	...
	if (len > 0) {
		...
		if (sinfo.s_cb != NULL) {
			...
			sinfo.s_cb->flags =
				(NETDEV_DOWN | TCP_ACKDATA | TCP_REXMIT | TCP_DISCONN_EVENTS | WPAN_POLL);
...
ssize_t psock_6lowpan_tcp_send(FAR struct socket *psock, FAR const void *buf, size_t buflen)
{
	...
	ret = sixlowpan_nexthopaddr((FAR struct radio_driver_s *)dev, conn->u.ipv6.raddr, &destmac);
	if (ret < 0) {
...
void sixlowpan_tcp_send(FAR struct net_driver_s *dev, FAR struct net_driver_s *fwddev,
						FAR struct ipv6_hdr_s *ipv6)
{
	...
	ninfo("d_len %u\n", dev->d_len);
	sixlowpan_dumpbuffer("Outgoing TCP packet", (FAR const uint8_t *)ipv6, dev->d_len);
	...
	if (dev != NULL && dev->d_len > 0 && fwddev != NULL) {
		...
		if (ipv6hdr->ipv6.proto != IP_PROTO_TCP) {
			...
			if (hdrlen > dev->d_len) {
				...
				sixlowpan_queue_frames((FAR struct radio_driver_s *)fwddev, &ipv6hdr->ipv6, buf,
									   buflen, &destmac);
			}
		}
	}
