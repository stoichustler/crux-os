	...
	if (ifaddr != NULL) {
		...
		if (net_ipv6_mask2pref(ifaddr->mask) == preflen) {
			nwarn("WARNING: Trying to add same IPv6 address on net device! "
				  ...
				  NTOHS(addr[0]), NTOHS(addr[1]), NTOHS(addr[2]), NTOHS(addr[3]), NTOHS(addr[4]),
				  NTOHS(addr[5]), NTOHS(addr[6]), NTOHS(addr[7]), preflen);
			return -EEXIST;
		}
...
void netdev_ipv6_addmcastmac(FAR struct net_driver_s *dev, const net_ipv6addr_t addr)
{
	...
	if (dev->d_addmac != NULL) {
		...
		ninfo("Add IPv6 Multicast: %02x:%02x:%02x:%02x:%02x:%02x\n", mcastmac[0], mcastmac[1],
			  mcastmac[2], mcastmac[3], mcastmac[4], mcastmac[5]);
