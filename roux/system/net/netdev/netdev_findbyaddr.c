...
static FAR struct net_driver_s *netdev_prefixlen_findby_lipv4addr(in_addr_t lipaddr,
																  FAR int8_t *prefixlen)
{
	...
	for (dev = g_netdevices; dev; dev = dev->flink) {
		...
		if ((dev->d_flags & IFF_UP) != 0 && !net_ipv4addr_cmp(dev->d_ipaddr, INADDR_ANY)) {
#ifndef CONFIG_ROUTE_LONGEST_MATCH
			/* Yes.. check for an address match (under the netmask) */

			if (net_ipv4addr_maskcmp(dev->d_ipaddr, lipaddr, dev->d_netmask)) {
				/* Its a match */
			...
			if (net_ipv4addr_maskcmp(dev->d_ipaddr, lipaddr, dev->d_netmask)) {
				...
				if (len > bestpref
					...
					|| (len == bestpref && arp_find(lipaddr, NULL, dev, true) == OK)
#endif
				) {
					bestdev = dev;
					bestpref = len;
...
static FAR struct net_driver_s *netdev_prefixlen_findby_lipv6addr(const net_ipv6addr_t lipaddr,
