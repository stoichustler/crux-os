...
#include <roux/net/radiodev.h>

#ifdef CONFIG_NET_6LOWPAN
#include <roux/net/sixlowpan.h>
#endif

#include <sys/sockio.h>
#include <roux/net/igmp.h>
...
#include <roux/wireless/bluetooth/bt_ioctl.h>
#endif

#ifdef CONFIG_WIRELESS_IEEE802154
#include <roux/wireless/ieee802154/ieee802154_mac.h>
#endif

#ifdef CONFIG_WIRELESS_PKTRADIO
#include <roux/wireless/pktradio.h>
#endif

#ifdef CONFIG_NET_CELLULAR
#include <roux/wireless/cellular/cellular.h>
#endif
...
#if defined(CONFIG_NET_IEEE802154) ||                                                              \
	(defined(CONFIG_NET_6LOWPAN) && defined(CONFIG_WIRELESS_IEEE802154))
...
static void ioctl_get_ipv4addr(FAR struct sockaddr *outaddr, in_addr_t inaddr)
{
...
#ifdef CONFIG_NET_IPv4
static void ioctl_get_ipv4broadcast(FAR struct sockaddr *outaddr, in_addr_t inaddr,
									in_addr_t netmask)
{
	FAR struct sockaddr_in *dest = (FAR struct sockaddr_in *)outaddr;
	dest->sin_family = AF_INET;
...
#ifdef CONFIG_NET_IPv4
static void ioctl_set_ipv4addr(FAR in_addr_t *outaddr, FAR const struct sockaddr *inaddr)
{
...
static int netdev_bluetooth_ioctl(FAR struct socket *psock, int cmd, unsigned long arg)
{
...
static int netdev_iee802154_ioctl(FAR struct socket *psock, int cmd, unsigned long arg)
{
	FAR struct net_driver_s *dev;
	FAR char *ifname;
	int ret = -ENOTTY;
...
static int netdev_pktradio_ioctl(FAR struct socket *psock, int cmd, unsigned long arg)
{
	FAR struct net_driver_s *dev;
	FAR char *ifname;
	int ret = -ENOTTY;
	...
	if (arg != 0ul) {
		if (_PKRADIOIOCVALID(cmd)) {
			...
			FAR struct pktradio_ifreq_s *cmddata = (FAR struct pktradio_ifreq_s *)((uintptr_t)arg);
...
static int netdev_cell_ioctl(FAR struct socket *psock, int cmd, FAR struct icellreq *req)
{
...
static int netdev_wifr_ioctl(FAR struct socket *psock, int cmd, FAR struct iwreq *req)
{
	FAR struct net_driver_s *dev;
	int ret = -ENOTTY;
...
static ssize_t net_ioctl_ifreq_arglen(uint8_t domain, int cmd)
{
	...
	case SIOCSIFADDR:
	case SIOCDIFADDR:
		return domain == PF_INET6 ? sizeof(struct in6_ifreq) : sizeof(struct ifreq);
...
static int netdev_ifr_ioctl(FAR struct socket *psock, int cmd, FAR struct ifreq *req)
{
	FAR struct net_driver_s *dev = NULL;
	unsigned int idx = 0;
	int ret = OK;
	...
	default:
		...
		if (net_ioctl_ifreq_arglen(psock->s_domain, cmd) >= (ssize_t)sizeof(struct ifreq)) {
	...
	case SIOCGIFBRDADDR: /* Get broadcast IP address */
		ioctl_get_ipv4broadcast(&req->ifr_broadaddr, dev->d_ipaddr, dev->d_netmask);
		break;
	...
	case SIOCSLIFADDR: /* Set IP address */
	{
		...
		netlink_device_notify_ipaddr(dev, RTM_NEWADDR, AF_INET6, dev->d_ipv6[idx].addr,
									 net_ipv6_mask2pref(dev->d_ipv6[idx].mask));
	} break;
	...
	case SIOCGIFHWADDR: /* Get hardware address */
		...
		if (dev->d_lltype == NET_LL_ETHERNET || dev->d_lltype == NET_LL_IEEE80211) {
			...
			memcpy(req->ifr_hwaddr.sa_data, dev->d_mac.ether.ether_addr_octet, IFHWADDRLEN);
		} else
#endif
#ifdef CONFIG_NET_6LOWPAN
			if (dev->d_lltype == NET_LL_IEEE802154 || dev->d_lltype == NET_LL_PKTRADIO) {
			req->ifr_hwaddr.sa_family = ARPHRD_IEEE802154;
			memcpy(req->ifr_hwaddr.sa_data, dev->d_mac.radio.nv_addr, dev->d_mac.radio.nv_addrlen);
		} else
#endif
		{
...
#ifdef CONFIG_NET_ETHERNET
		if (dev->d_lltype == NET_LL_ETHERNET || dev->d_lltype == NET_LL_IEEE80211) {
			memcpy(dev->d_mac.ether.ether_addr_octet, req->ifr_hwaddr.sa_data, IFHWADDRLEN);
		} else
#endif
#ifdef CONFIG_NET_6LOWPAN
			if (dev->d_lltype == NET_LL_IEEE802154 || dev->d_lltype == NET_LL_PKTRADIO) {
			...
			if (ret >= 0) {
				...
				DEBUGASSERT(dev->d_mac.radio.nv_addrlen <= sizeof(dev->d_mac.radio.nv_addr));
				DEBUGASSERT(dev->d_mac.radio.nv_addrlen <= sizeof(req->ifr_hwaddr.sa_data));
	...
	case SIOCSIFADDR: /* Set IP address */
		...
		if (psock->s_domain == PF_INET6) {
			FAR struct in6_ifreq *ifr6 = (FAR struct in6_ifreq *)req;
			ret = netdev_ipv6_add(dev, ifr6->ifr6_addr.in6_u.u6_addr16, ifr6->ifr6_prefixlen);
			if (ret == OK) {
	...
	case SIOCDIFADDR: /* Delete IP address */
		...
		if (psock->s_domain == PF_INET6) {
			FAR struct in6_ifreq *ifr6 = (FAR struct in6_ifreq *)req;
			ret = netdev_ipv6_del(dev, ifr6->ifr6_addr.in6_u.u6_addr16, ifr6->ifr6_prefixlen);
			if (ret == OK) {
	...
	case SIOCMIINOTIFY: /* Set up for PHY event notifications */
		if (dev->d_ioctl) {
			FAR struct mii_ioctl_notify_s *notify = &req->ifr_ifru.ifru_mii_notify;
	...
	case SIOCSMIIREG: /* Set MII register via MDIO */
		if (dev->d_ioctl) {
			FAR struct mii_ioctl_data_s *mii_data = &req->ifr_ifru.ifru_mii_data;
	...
	case SIOCGCANBITRATE: /* Get bitrate from a CAN controller */
		if (dev->d_ioctl) {
			FAR struct can_ioctl_data_s *can_bitrate_data = &req->ifr_ifru.ifru_can_data;
			ret = dev->d_ioctl(dev, cmd, (unsigned long)(uintptr_t)can_bitrate_data);
		} else {
			ret = -ENOSYS;
		}
		break;
	...
	case SIOCCANRECOVERY:	/* Recovery can controller when bus-off */
		if (dev->d_ioctl) {
			FAR struct can_ioctl_filter_s *can_filter = &req->ifr_ifru.ifru_can_filter;
			ret = dev->d_ioctl(dev, cmd, (unsigned long)(uintptr_t)can_filter);
		} else {
			ret = -ENOSYS;
		}
		break;
	...
	case SIOCSCANSTATE: /* Set the LIN/CAN controller state */
		if (dev->d_ioctl) {
			FAR struct can_ioctl_state_s *can_state = &req->ifr_ifru.ifru_can_state;
			ret = dev->d_ioctl(dev, cmd, (unsigned long)(uintptr_t)can_state);
		} else {
			ret = -ENOSYS;
		}
		break;
...
#ifdef CONFIG_NET_IGMP
static int netdev_imsf_ioctl(FAR struct socket *psock, int cmd, FAR struct ip_msfilter *imsf)
{
	FAR struct net_driver_s *dev;
	int ret = -EINVAL;
...
static bool ioctl_arpreq_parse(FAR struct arpreq *req, FAR struct net_driver_s **dev,
							   FAR struct sockaddr_in **addr)
{
	if (req != NULL) {
...
#ifdef CONFIG_NET_ARP
static int netdev_arp_ioctl(FAR struct socket *psock, int cmd, FAR struct arpreq *req)
{
	FAR struct net_driver_s *dev;
	FAR struct sockaddr_in *addr;
	int ret;
	...
	case SIOCSARP: /* Set an ARP mapping */
	{
		if (ioctl_arpreq_parse(req, &dev, &addr) && dev != NULL &&
			req->arp_pa.sa_family == AF_INET && req->arp_ha.sa_family == ARPHRD_ETHER) {
			/* Update any existing ARP table entry for this protocol
			 * address -OR- add a new ARP table entry if there is not.
			 */

			ret = arp_update(dev, addr->sin_addr.s_addr, (FAR const uint8_t *)req->arp_ha.sa_data);
		} else {
			ret = -EINVAL;
		}
	} break;
	...
	{
		if (ioctl_arpreq_parse(req, &dev, &addr) && req->arp_pa.sa_family == AF_INET) {
			ret = arp_find(addr->sin_addr.s_addr, (FAR uint8_t *)req->arp_ha.sa_data, dev, true);
			if (ret >= 0) {
...
static int netdev_ioctl(FAR struct socket *psock, int cmd, unsigned long arg)
{
	int ret = -ENOTTY;
	...
	case FIONBIO: {
		...
		DEBUGASSERT(psock->s_sockif != NULL && psock->s_sockif->si_sockcaps != NULL);
 ...
 *
 * Input Parameters:
 *   psock    A pointer to a ROUX-specific, internal socket structure
 *   cmd      The ioctl command
 *   arg      The argument of the ioctl cmd
 *
 * Returned Value:
...
int psock_vioctl(FAR struct socket *psock, int cmd, va_list ap)
{
	...
	if (ret == -ENOTTY) {
		ret = netdev_cell_ioctl(psock, cmd, (FAR struct icellreq *)(uintptr_t)arg);
	}
#endif
	...
	if (ret == -ENOTTY) {
		ret = netdev_imsf_ioctl(psock, cmd, (FAR struct ip_msfilter *)(uintptr_t)arg);
	}
#endif
	...
	if (ret == -ENOTTY) {
		ret = netdev_arp_ioctl(psock, cmd, (FAR struct arpreq *)(uintptr_t)arg);
	}
#endif
	...
	if (ret == -ENOTTY) {
		ret = netdev_rt_ioctl(psock, cmd, (FAR struct rtentry *)(uintptr_t)arg);
	}
#endif
