...
static int ifconf_ipv4_callback(FAR struct net_driver_s *dev, FAR void *arg)
{
	...
	if (!net_ipv4addr_cmp(dev->d_ipaddr, INADDR_ANY) && (dev->d_flags & IFF_UP) != 0) {
		...
		if (ifc->ifc_len + sizeof(struct ifreq) <= info->bufsize) {
			FAR struct ifreq *req = (FAR struct ifreq *)&ifc->ifc_buf[ifc->ifc_len];
			FAR struct sockaddr_in *inaddr = (FAR struct sockaddr_in *)&req->ifr_addr;
									 ...
									 FAR struct netdev_ifaddr6_s *addr, FAR void *arg)
{
	...
	if (lifc->lifc_len + sizeof(struct lifreq) <= info->bufsize) {
		FAR struct lifreq *req = (FAR struct lifreq *)&lifc->lifc_buf[lifc->lifc_len];
		FAR struct sockaddr_in6 *inaddr = (FAR struct sockaddr_in6 *)&req->lifr_addr;
		...
		if (addr_idx > 0) {
			...
			if (snprintf(ifname, sizeof(ifname), "%s:%d", dev->d_ifname, addr_idx - 1) >=
				IFNAMSIZ) {
