...
void netdev_statistics_log(FAR void *arg)
{
	...
	stats_log("%s:T%" PRIu32 "/%" PRIu32 "(%" PRIu64 "B)"
			  ",R"
...
#endif
			  "\n",
			  dev->d_ifname, stats->tx_done, stats->tx_packets, stats->tx_bytes
#if defined(CONFIG_NET_IPv4) && defined(CONFIG_NET_IPv6)
			  ,
...
#ifdef CONFIG_NET_ICMP
			  ,
			  g_netstats.icmp.sent, g_netstats.icmp.recv, g_netstats.icmp.drop
#endif
#ifdef CONFIG_NET_ICMPv6
			  ,
			  g_netstats.icmpv6.sent, g_netstats.icmpv6.recv, g_netstats.icmpv6.drop
#endif
	);
}
#endif
