...
static const uint32_t g_crc32c_table[256] = {
	...
	0xeb1fcbad, 0x197448ae, 0x0a24bb5a, 0xf84f3859, 0x2c855cb2, 0xdeeedfb1, 0xcdb52c45, 0x3fd5af46,
...
static uint32_t compute_toeplitz_hash(FAR const uint8_t *packet, uint32_t len)
{
	...
	for (i = 0; i < len; i++) {
		for (j = 0; j < 8; j++) {
			...
			key <<= 1;
			if ((i + 4) < RANDOM_KEY_SIZE && (g_random_key.u8[i + 4] & (1 << (7 - j)))) {
				key |= 1;
			}
		}
	}
...
static uint32_t create_binary(hashcal_type_e hash_type, uint8_t domain,
							  ...
							  FAR const uint32_t *dst_addr, uint16_t dst_port, FAR uint8_t *packet)
{
	uint32_t iter = 0;
...
static uint32_t compute_hash(hashcal_algo_e hash_algo, hashcal_type_e hash_type, uint8_t domain,
							 FAR const uint32_t *src_addr, uint16_t src_port,
							 FAR const uint32_t *dst_addr, uint16_t dst_port)
{
	...
	cal_len = create_binary(hash_type, domain, src_addr, src_port, dst_addr, dst_port, packet);
...
void netdev_notify_recvcpu(FAR struct net_driver_s *dev, int cpu, uint8_t domain,
						   FAR const void *src_addr, uint16_t src_port, FAR const void *dst_addr,
						   uint16_t dst_port)
{
	if (dev != NULL && dev->d_ioctl != NULL) {
		uint32_t hash = compute_hash(HASHCAL_ALGO_CRC32, HASHCAL_TYPE_4TUPLE, domain, src_addr,
		...
		ret = dev->d_ioctl(dev, SIOCNOTIFYRECVCPU, (unsigned long)(uintptr_t)&arg);
		if (ret < 0) {
