...
static int psock_socketlevel_option(FAR struct socket *psock, int option, FAR void *value,
					 ...
					 FAR socklen_t *value_len)
{
	...
	if (psock->s_sockif->si_getsockopt != NULL) {
		ret = psock->s_sockif->si_getsockopt(psock, level, option, value, value_len);
	}
