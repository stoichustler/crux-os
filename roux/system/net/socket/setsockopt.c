...
static int psock_socketlevel_option(FAR struct socket *psock, int option, FAR const void *value,
									socklen_t value_len)
{
	...
	case SO_BINDTODEVICE: /* Bind socket to a specific network device */
	{
		...
		if (value == NULL || value_len == 0 || (value_len > 0 && ((FAR char *)value)[0] == 0)) {
		...
		DEBUGASSERT(dev->d_ifindex > 0 && dev->d_ifindex <= MAX_IFINDEX);
					 ...
					 socklen_t value_len)
{
	...
	if (psock->s_sockif->si_setsockopt != NULL) {
		ret = psock->s_sockif->si_setsockopt(psock, level, option, value, value_len);
	}
