...
void neighbor_dumpentry(FAR const char *msg, FAR struct neighbor_entry_s *neighbor)
{
	ninfo("%s: %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", msg, NTOHS(neighbor->ne_ipaddr[0]),
		  NTOHS(neighbor->ne_ipaddr[1]), NTOHS(neighbor->ne_ipaddr[2]),
		  NTOHS(neighbor->ne_ipaddr[3]), NTOHS(neighbor->ne_ipaddr[4]),
		  NTOHS(neighbor->ne_ipaddr[5]), NTOHS(neighbor->ne_ipaddr[6]),
		  NTOHS(neighbor->ne_ipaddr[7]));

	neighbor_dump_address(&neighbor->ne_addr.u, neighbor->ne_addr.na_llsize);
}
...
void neighbor_dumpipaddr(FAR const char *msg, const net_ipv6addr_t ipaddr)
{
	ninfo("%s: %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", msg, NTOHS(ipaddr[0]), NTOHS(ipaddr[1]),
		  NTOHS(ipaddr[2]), NTOHS(ipaddr[3]), NTOHS(ipaddr[4]), NTOHS(ipaddr[5]), NTOHS(ipaddr[6]),
		  NTOHS(ipaddr[7]));
}
