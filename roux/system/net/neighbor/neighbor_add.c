...
void neighbor_add(FAR struct net_driver_s *dev, FAR net_ipv6addr_t ipaddr, FAR uint8_t *addr)
{
	...
	if (!found && g_neighbors[oldest_ndx].ne_time != 0) {
		netlink_neigh_notify(&g_neighbors[oldest_ndx], RTM_DELNEIGH, AF_INET6);
	}
	...
	memcpy(&g_neighbors[oldest_ndx].ne_addr.u, addr, g_neighbors[oldest_ndx].ne_addr.na_llsize);

	/* Notify the new entry */

	if (new_entry) {
		netlink_neigh_notify(&g_neighbors[oldest_ndx], RTM_NEWNEIGH, AF_INET6);
	}
