...
static uint16_t poll_event(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
...
static int usrsock_pollsetup(FAR struct socket *psock, FAR struct pollfd *fds)
{
	...
	cb->flags = USRSOCK_EVENT_ABORT | USRSOCK_EVENT_CONNECT_READY | USRSOCK_EVENT_SENDTO_READY |
				USRSOCK_EVENT_RECVFROM_AVAIL | USRSOCK_EVENT_REMOTE_CLOSED;
	cb->priv = info;
	cb->event = poll_event;
	...
	if (conn->state == USRSOCK_CONN_STATE_UNINITIALIZED ||
		...
		ninfo("socket %s.\n",
			  conn->state == USRSOCK_CONN_STATE_UNINITIALIZED ? "uninitialized" : "aborted");
	...
	else if ((psock->s_type == SOCK_STREAM || psock->s_type == SOCK_SEQPACKET) &&
...
static int usrsock_pollteardown(FAR struct socket *psock, FAR struct pollfd *fds)
{
	FAR struct usrsock_conn_s *conn = psock->s_conn;
	FAR struct usrsock_poll_s *info;
	...
	if (info) {
		...
		devif_conn_callback_free(NULL, info->cb, &conn->sconn.list, &conn->sconn.list_tail);
