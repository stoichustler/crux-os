...
static uint16_t connect_event(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
...
static int do_connect_request(FAR struct usrsock_conn_s *conn, FAR const struct sockaddr *addr,
							  socklen_t addrlen)
{
...
int usrsock_connect(FAR struct socket *psock, FAR const struct sockaddr *addr, socklen_t addrlen)
{
	...
	if (conn->connected && (psock->s_type == SOCK_STREAM || psock->s_type == SOCK_SEQPACKET)) {
		/* Already connected. */
	...
	ret =
		usrsock_setup_request_callback(conn, &state, connect_event,
									   USRSOCK_EVENT_ABORT | USRSOCK_EVENT_REMOTE_CLOSED |
										   USRSOCK_EVENT_REQ_COMPLETE | USRSOCK_EVENT_SENDTO_READY);
	if (ret < 0) {
