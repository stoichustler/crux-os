...
static uint16_t ioctl_event(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
...
static int do_ioctl_request(FAR struct usrsock_conn_s *conn, int cmd, FAR void *arg, size_t arglen)
{
...
int usrsock_ioctl(FAR struct socket *psock, int cmd, unsigned long arg_)
{
	...
	ret = usrsock_setup_data_request_callback(conn, &state, ioctl_event,
