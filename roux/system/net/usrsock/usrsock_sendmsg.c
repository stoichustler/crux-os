...
static uint16_t sendto_event(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
...
static int do_sendto_request(FAR struct usrsock_conn_s *conn, FAR struct msghdr *msg, int flags)
{
	...
	do {
		...
		if (!(conn->flags & USRSOCK_EVENT_SENDTO_READY)) {
			...
			ret = usrsock_setup_request_callback(conn, &state, sendto_event,
												 USRSOCK_EVENT_ABORT | USRSOCK_EVENT_SENDTO_READY |
			...
			ret = net_sem_timedwait(&state.recvsem, _SO_TIMEOUT(conn->sconn.s_sndtimeo));
