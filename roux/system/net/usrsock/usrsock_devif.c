...
static struct usrsock_req_s g_usrsock_req = {
	NXMUTEX_INITIALIZER, SEM_INITIALIZER(0), 0, 0, 0, NULL};
...
static ssize_t usrsock_iovec_do(FAR void *srcdst, size_t srcdstlen, FAR struct iovec *iov,
								int iovcnt, size_t pos, bool from_iov, FAR bool *done)
{
...
static ssize_t usrsock_handle_response(FAR struct usrsock_conn_s *conn, FAR const void *buffer,
									   size_t len)
{
...
static ssize_t usrsock_handle_datareq_response(FAR struct usrsock_conn_s *conn,
	...
	if (hdr->result > 0) {
		...
		if (conn->resp.datain.iov[iovpos].iov_len < hdr->result) {
			nwarn("%dth buffer not large enough "
				  "(need: %" PRId32 ", have: %zu).\n",
				  iovpos, hdr->result, conn->resp.datain.iov[iovpos].iov_len);
			return -EINVAL;
		}
...
static ssize_t usrsock_handle_req_response(FAR const void *buffer, size_t len, FAR bool *req_done)
{
	FAR const struct usrsock_message_req_ack_s *hdr = buffer;
	FAR struct usrsock_conn_s *conn = NULL;
	FAR struct usrsock_req_s *req = &g_usrsock_req;
	ssize_t (*handle_response)(FAR struct usrsock_conn_s * conn, FAR const void *buffer,
							   size_t len);
	size_t hdrlen;
	ssize_t ret;
	...
	while ((conn = usrsock_nextconn(conn)) != NULL && conn->resp.xid != hdr->xid)
		;
	if (!conn) {
...
static ssize_t usrsock_handle_message(FAR const void *buffer, size_t len, FAR bool *req_done)
{
	FAR const struct usrsock_message_common_s *common = buffer;
...
ssize_t usrsock_response(FAR const char *buffer, size_t len, FAR bool *req_done)
{
	FAR struct usrsock_req_s *req = &g_usrsock_req;
	FAR struct usrsock_conn_s *conn;
	size_t origlen = len;
	...
	if (!req->datain_conn) {
		...
		if (len < sizeof(struct usrsock_message_common_s)) {
			nerr("message too short, %zu < %zu.\n", len, sizeof(struct usrsock_message_common_s));
			return -EINVAL;
		}
	...
	if (req->datain_conn) {
		...
		if (len != 0) {
			ret = usrsock_iovec_put(conn->resp.datain.iov, conn->resp.datain.iovcnt,
...
ssize_t usrsock_iovec_get(FAR void *dst, size_t dstlen, FAR const struct iovec *iov, int iovcnt,
						  size_t pos, FAR bool *done)
{
	return usrsock_iovec_do(dst, dstlen, (FAR struct iovec *)iov, iovcnt, pos, true, done);
}
...
ssize_t usrsock_iovec_put(FAR struct iovec *iov, int iovcnt, size_t pos, FAR const void *src,
						  size_t srclen)
{
	return usrsock_iovec_do((FAR void *)src, srclen, iov, iovcnt, pos, false, NULL);
}
...
int usrsock_do_request(FAR struct usrsock_conn_s *conn, FAR struct iovec *iov, unsigned int iovcnt)
{
