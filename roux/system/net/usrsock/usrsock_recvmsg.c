...
static uint16_t recvfrom_event(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
	...
	if (flags & USRSOCK_EVENT_ABORT) {
		...
		if (!(flags & USRSOCK_EVENT_RECVFROM_AVAIL) &&
			(pstate->reqstate.result >= 0 || pstate->reqstate.result == -EAGAIN)) {
...
static int do_recvfrom_request(FAR struct usrsock_conn_s *conn, size_t buflen, socklen_t addrlen,
							   int32_t flags)
{
	...
	do {
		...
		if (!(conn->flags & USRSOCK_EVENT_RECVFROM_AVAIL)) {
			...
			ret = usrsock_setup_data_request_callback(
				...
				USRSOCK_EVENT_ABORT | USRSOCK_EVENT_RECVFROM_AVAIL | USRSOCK_EVENT_REMOTE_CLOSED);
			if (ret < 0) {
			...
			ret = net_sem_timedwait(&state.reqstate.recvsem, _SO_TIMEOUT(conn->sconn.s_rcvtimeo));
		...
		ret = usrsock_setup_data_request_callback(conn, &state, recvfrom_event,
