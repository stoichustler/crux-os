...
NET_BUFPOOL_DECLARE(g_usrsock_connections, sizeof(struct usrsock_conn_s),
					CONFIG_NET_USRSOCK_PREALLOC_CONNS, CONFIG_NET_USRSOCK_ALLOC_CONNS,
								   ...
								   FAR devif_callback_event_t event, uint16_t flags)
{
	...
	pstate->cb = devif_callback_alloc(NULL, &conn->sconn.list, &conn->sconn.list_tail);
	if (pstate->cb) {
...
void usrsock_teardown_request_callback(FAR struct usrsock_reqstate_s *pstate)
{
	...
	devif_conn_callback_free(NULL, pstate->cb, &conn->sconn.list, &conn->sconn.list_tail);
