...
static uint16_t accept_event(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
	...
	if (flags & USRSOCK_EVENT_ABORT) {
		...
		if (pstate->reqstate.result >= 0 || pstate->reqstate.result == -EAGAIN) {
				   ...
				   FAR struct socket *newsock, int flags)
{
	...
	do {
		...
		ret = usrsock_setup_data_request_callback(conn, &state, accept_event,
