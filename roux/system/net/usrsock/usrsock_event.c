...
int usrsock_event(FAR struct usrsock_conn_s *conn)
{
	...
	if (conn->state == USRSOCK_CONN_STATE_CONNECTING) {
		if ((events & USRSOCK_EVENT_REQ_COMPLETE) || (events & USRSOCK_EVENT_SENDTO_READY)) {
			conn->state = USRSOCK_CONN_STATE_READY;
			events |= USRSOCK_EVENT_CONNECT_READY;

			if ((conn->resp.result == 0) || (events & USRSOCK_EVENT_SENDTO_READY)) {
				conn->connected = true;
			}
		}
	}
	...
	if ((conn->state == USRSOCK_CONN_STATE_READY || conn->state == USRSOCK_CONN_STATE_CONNECTING) &&
