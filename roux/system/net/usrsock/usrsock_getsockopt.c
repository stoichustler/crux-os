...
#if defined(CONFIG_NET) && defined(CONFIG_NET_USRSOCK) && defined(CONFIG_NET_SOCKOPTS)
...
static uint16_t getsockopt_event(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
...
static int do_getsockopt_request(FAR struct usrsock_conn_s *conn, int level, int option,
								 socklen_t value_len)
{
					   ...
					   FAR socklen_t *value_len)
{
	...
	if (level == SOL_SOCKET &&
		(option == SO_TYPE || option == SO_RCVTIMEO || option == SO_SNDTIMEO)) {
		return -ENOPROTOOPT;
	}
	...
	ret = usrsock_setup_data_request_callback(conn, &state, getsockopt_event,
