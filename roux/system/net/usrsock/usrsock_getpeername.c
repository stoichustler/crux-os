...
static uint16_t getpeername_event(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
...
static int do_getpeername_request(FAR struct usrsock_conn_s *conn, socklen_t addrlen)
{
...
int usrsock_getpeername(FAR struct socket *psock, FAR struct sockaddr *addr, FAR socklen_t *addrlen)
{
	...
	ret = usrsock_setup_data_request_callback(conn, &state, getpeername_event,
