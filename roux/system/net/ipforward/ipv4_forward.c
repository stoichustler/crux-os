...
static int ipv4_hdrsize(FAR struct ipv4_hdr_s *ipv4)
{
	...
	case IP_PROTO_TCP: {
		FAR struct tcp_hdr_s *tcp = (FAR struct tcp_hdr_s *)((FAR uint8_t *)ipv4 + iphdrlen);
...
static int ipv4_dev_forward(FAR struct net_driver_s *dev, FAR struct net_driver_s *fwddev,
...
static int ipv4_forward_callback(FAR struct net_driver_s *fwddev, FAR void *arg)
{
...
#ifdef CONFIG_NET_IPFORWARD_BROADCAST
void ipv4_forward_broadcast(FAR struct net_driver_s *dev, FAR struct ipv4_hdr_s *ipv4)
{
