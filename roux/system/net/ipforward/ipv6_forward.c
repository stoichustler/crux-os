...
static int ipv6_hdrsize(FAR struct ipv6_hdr_s *ipv6)
{
	...
	case IP_PROTO_TCP: {
		FAR struct tcp_hdr_s *tcp = (FAR struct tcp_hdr_s *)((FAR uint8_t *)ipv6 + IPv6_HDRLEN);
...
static int ipv6_packet_conversion(FAR struct net_driver_s *dev, FAR struct net_driver_s *fwddev,
								  FAR struct ipv6_hdr_s *ipv6)
{
	...
	if (dev->d_len > 0) {
		...
		if (fwddev->d_lltype != NET_LL_IEEE802154 && fwddev->d_lltype != NET_LL_PKTRADIO) {
...
static int ipv6_dev_forward(FAR struct net_driver_s *dev, FAR struct net_driver_s *fwddev,
							FAR struct ipv6_hdr_s *ipv6)
{
...
static int ipv6_forward_callback(FAR struct net_driver_s *fwddev, FAR void *arg)
{
...
#ifdef CONFIG_NET_IPFORWARD_BROADCAST
void ipv6_forward_broadcast(FAR struct net_driver_s *dev, FAR struct ipv6_hdr_s *ipv6)
{
