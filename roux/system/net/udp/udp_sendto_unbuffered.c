...
static inline void sendto_ipselect(FAR struct net_driver_s *dev, FAR struct sendto_s *pstate)
{
						 ...
						 FAR const struct sockaddr *to, socklen_t tolen)
{
	...
	if (psock == NULL || psock->s_type != SOCK_DGRAM || psock->s_conn == NULL) {
	...
	if (state.st_cb) {
		...
		ret = net_sem_timedwait(&state.st_sem, _SO_TIMEOUT(conn->sconn.s_sndtimeo));
		if (ret >= 0) {
