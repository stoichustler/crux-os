...
static inline void sendto_ipselect(FAR struct net_driver_s *dev, FAR struct udp_conn_s *conn);
#endif
static int sendto_next_transfer(FAR struct udp_conn_s *conn);
static uint16_t sendto_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags);
...
static inline void sendto_ipselect(FAR struct net_driver_s *dev, FAR struct udp_conn_s *conn)
{
	...
	if (conn->domain == PF_INET ||
		(conn->domain == PF_INET6 && ip6_is_ipv4addr((FAR struct in6_addr *)conn->u.ipv6.raddr))) {
...
static uint16_t sendto_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
	...
	if (dev->d_sndlen <= 0 && (flags & UDP_NEWDATA) == 0 && (flags & UDP_POLL) != 0 &&
						 ...
						 FAR const struct sockaddr *to, socklen_t tolen)
{
	...
	if (len > 0) {
		...
		wrb = udp_wrbuffer_tryalloc(len + udpip_hdrsize(conn) + CONFIG_NET_LL_GUARDSIZE);
#else
		if (nonblock) {
...
#endif
			{
				FAR struct sockaddr_in *addr4 = (FAR struct sockaddr_in *)&wrb->wb_dest;
...
#endif
			{
				FAR struct sockaddr_in6 *addr6 = (FAR struct sockaddr_in6 *)&wrb->wb_dest;
		...
		if (nonblock) {
			ret = iob_trycopyin(wrb->wb_iob, (FAR uint8_t *)buf, len, udpiplen, false);
		} else {
			unsigned int count;
			int blresult;
			...
			ret = iob_copyin(wrb->wb_iob, (FAR uint8_t *)buf, len, udpiplen, false);
			if (blresult >= 0) {
