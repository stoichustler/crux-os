...
#define UDP_WBDUMP(msg, wrb, len, offset) udp_wrbuffer_dump(msg, wrb, len, offset)
#else
...
int udp_setsockopt(FAR struct socket *psock, int option, FAR const void *value,
				   socklen_t value_len);
#endif
...
#ifdef CONFIG_NET_UDP_WRITE_BUFFERS
#ifdef CONFIG_DEBUG_FEATURES
void udp_wrbuffer_dump(FAR const char *msg, FAR struct udp_wrbuffer_s *wrb, unsigned int len,
					   unsigned int offset);
#else
...
uint16_t udp_callback(FAR struct net_driver_s *dev, FAR struct udp_conn_s *conn, uint16_t flags);
...
ssize_t psock_udp_sendto(FAR struct socket *psock, FAR const void *buf, size_t len, int flags,
...
#ifdef CONFIG_NET_UDP_NOTIFIER
int udp_readahead_notifier_setup(worker_t worker, FAR struct udp_conn_s *conn, FAR void *arg);
#endif
...
#ifdef CONFIG_NET_UDP_NOTIFIER
int udp_writebuffer_notifier_setup(worker_t worker, FAR struct udp_conn_s *conn, FAR void *arg);
#endif
