...
FAR struct net_driver_s *udp_find_laddr_device(FAR struct udp_conn_s *conn)
{
...
#endif
	{
		...
		if (conn->u.ipv4.laddr == 0) {
			...
			return netdev_findby_ripv4addr(conn->u.ipv4.laddr, conn->u.ipv4.laddr);
		}
	}
#endif
...
#endif
	{
		...
		if (net_ipv6addr_cmp(conn->u.ipv6.laddr, g_ipv6_unspecaddr)) {
			...
			return netdev_findby_ripv6addr(conn->u.ipv6.laddr, conn->u.ipv6.laddr);
		}
	}
#endif
}
											   ...
											   FAR struct sockaddr_storage *remote)
{
...
#endif
	{
		...
		if (remote) {
			FAR const struct sockaddr_in *inaddr = (FAR const struct sockaddr_in *)remote;
		...
		if (IN_MULTICAST(NTOHL(raddr))) {
			if ((conn->sconn.s_boundto == 0) && (conn->mreq.imr_ifindex != 0)) {
...
#endif
	{
		...
		if (remote) {
			FAR const struct sockaddr_in6 *inaddr = (FAR const struct sockaddr_in6 *)remote;
			net_ipv6addr_copy(raddr.in6_u.u6_addr16, inaddr->sin6_addr.s6_addr16);
		} else {
		...
		return netdev_findby_ripv6addr(conn->u.ipv6.laddr, raddr.in6_u.u6_addr16);
	}
#endif
}
