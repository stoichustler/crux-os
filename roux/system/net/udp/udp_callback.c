								...
								FAR uint8_t *buffer, uint16_t buflen)
{
...
#endif
	{
...
#endif
		{
			...
			net_ipv4addr_copy(src_addr4.sin_addr.s_addr, net_ip4addr_conv32(ipv4->srcipaddr));
	...
	ret = iob_trycopyin(iob, (FAR const uint8_t *)&dev->d_rxtime, sizeof(struct timespec), offset,
						true);
	if (ret < 0) {
		goto errout;
	}
	...
	ret = iob_trycopyin(iob, &src_addr_size, sizeof(src_addr_size), offset, true);
	if (ret < 0) {
		goto errout;
	}
	...
	ret = iob_trycopyin(iob, &dev->d_ifindex, sizeof(dev->d_ifindex), offset, true);
	if (ret < 0) {
		goto errout;
	}
#endif
	...
	ret = iob_trycopyin(iob, (FAR const uint8_t *)&buflen, sizeof(buflen), offset, true);
	if (ret < 0) {
		goto errout;
	}
...
static inline uint16_t net_dataevent(FAR struct net_driver_s *dev, FAR struct udp_conn_s *conn,
									 uint16_t flags)
{
	uint16_t ret;
