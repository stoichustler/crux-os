									 ...
									 FAR struct timespec *timestamp)
{
	...
	cmsg_append(msg, SOL_SOCKET, SO_TIMESTAMP, &tv, sizeof(struct timeval));
}
#endif

#ifdef CONFIG_NET_SOCKOPTS
static void udp_recvpktinfo(FAR struct udp_recvfrom_s *pstate, FAR void *srcaddr, uint8_t ifindex)
{
	FAR struct msghdr *msg = pstate->ir_msg;
	FAR struct udp_conn_s *conn = pstate->ir_conn;

#ifdef CONFIG_NET_IPv4
	if (conn->domain == PF_INET && _SO_GETOPT(conn->sconn.s_options, IP_PKTINFO)) {
	...
	if (conn->domain == PF_INET6 && _SO_GETOPT(conn->sconn.s_options, IPV6_RECVPKTINFO)) {
		...
		cmsg_append(msg, IPPROTO_IPV6, IPV6_PKTINFO, &pktinfo, sizeof(pktinfo));
	}
#endif
}
#else
#define udp_recvpktinfo(p, s, i)                                                                   \
	{                                                                                              \
		(void)(p);                                                                                 \
		(void)(s);                                                                                 \
		(void)(i);                                                                                 \
										  ...
										  FAR struct udp_recvfrom_s *pstate)
{
	...
	recvlen = iob_copyout(pstate->ir_msg->msg_iov->iov_base, dev->d_iob, recvlen,
						  dev->d_appdata - dev->d_iob->io_data - dev->d_iob->io_offset);
...
static inline void udp_readahead(struct udp_recvfrom_s *pstate)
{
	...
	if ((iob = conn->readahead) != NULL) {
		...
		recvlen = iob_copyout((FAR uint8_t *)&datalen, iob, sizeof(datalen), offset);
		...
		ifindex = 1;
#endif
		recvlen = iob_copyout(&src_addr_size, iob, sizeof(src_addr_size), offset);
		...
		if (conn->timestamp) {
			...
			recvlen = iob_copyout((FAR uint8_t *)&timestamp, iob, sizeof(struct timespec), offset);
		...
		recvlen = iob_copyout(pstate->ir_msg->msg_iov->iov_base, iob,
							  MIN(pstate->ir_msg->msg_iov->iov_len, datalen), offset);
		...
		if (pstate->ir_msg->msg_name) {
			pstate->ir_msg->msg_namelen = src_addr_size > pstate->ir_msg->msg_namelen
...
static inline void udp_sender(FAR struct net_driver_s *dev, FAR struct udp_recvfrom_s *pstate)
{
#ifdef CONFIG_NET_IPv6
	uint8_t srcaddr[sizeof(struct sockaddr_in6)];
#else
...
#endif
	{
		...
		if (conn->domain == PF_INET6) {
			...
			FAR struct sockaddr_in6 *infrom6 = (FAR struct sockaddr_in6 *)srcaddr;
...
#endif
		{
			...
			net_ipv4addr_copy(infrom->sin_addr.s_addr, net_ip4addr_conv32(ipv4->srcipaddr));
	...
	if (pstate->ir_msg->msg_name) {
		pstate->ir_msg->msg_namelen =
			fromlen > pstate->ir_msg->msg_namelen ? pstate->ir_msg->msg_namelen : fromlen;
		memcpy(pstate->ir_msg->msg_name, srcaddr, pstate->ir_msg->msg_namelen);
	}
...
static uint16_t udp_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
...
static void udp_recvfrom_initialize(FAR struct udp_conn_s *conn, FAR struct msghdr *msg,
									FAR struct udp_recvfrom_s *pstate, int flags)
{
	/* Initialize the state structure. */
...
static void udp_notify_recvcpu(FAR struct udp_conn_s *conn)
{
	...
	if (cpu != conn->rcvcpu) {
		if (conn->domain == PF_INET) {
			netdev_notify_recvcpu(conn->dev, cpu, conn->domain, &(conn->u.ipv4.laddr), conn->lport,
								  &(conn->u.ipv4.raddr), conn->rport);
		} else {
			netdev_notify_recvcpu(conn->dev, cpu, conn->domain, &(conn->u.ipv6.laddr), conn->lport,
...
ssize_t psock_udp_recvfrom(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
	 ...
	 */
		...
		if (state.ir_cb) {
			...
			ret = net_sem_timedwait(&state.ir_sem, _SO_TIMEOUT(conn->sconn.s_rcvtimeo));
