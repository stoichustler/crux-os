...
#define SNOOP_EPOCH_USEC(tv)                                                                       \
	(((tv).tv_sec - 0x386d4380ll) * 1000000ll + (tv).tv_usec + 0x00e03ab44a676000ll)
...
static void snoop_fill_packet_header(FAR struct snoop_s *snoop, uint32_t bytes, uint32_t drops,
									 uint32_t flags, FAR struct snoop_packet_header_s *header)
{
	struct timeval tv;
...
int snoop_open(FAR struct snoop_s *snoop, FAR const char *filename, uint32_t datalink,
			   bool autosync)
{
	...
	case SNOOP_DATALINK_TYPE_FDDI:
	case SNOOP_DATALINK_TYPE_OTHER: {
		static const uint8_t snoop_magic[] = {'s', 'n', 'o', 'o', 'p', '\0', '\0', '\0'};
	...
	ret = file_open(&snoop->filep, filename, O_RDWR | O_CREAT | O_CLOEXEC, 0666);
	if (ret < 0) {
		return ret;
	}
...
int snoop_dump(FAR struct snoop_s *snoop, FAR const void *buf, uint32_t nbytes, uint32_t drops,
			   uint32_t flags)
{
	...
	if (up_interrupt_context()) {
		if (sizeof(snoop->buf) - snoop->next < nbytes + sizeof(struct snoop_packet_header_s)) {
			ret = -ENOMEM;
			goto out_leave;
		}
