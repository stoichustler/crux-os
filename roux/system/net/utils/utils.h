...
#define NET_PORT_RANDOM_INIT(port)                                                                 \
	do {                                                                                           \
		...
		(port) = (port) % (CONFIG_NET_DEFAULT_MAX_PORT - CONFIG_NET_DEFAULT_MIN_PORT + 1);         \
...
#define NET_PORT_NEXT_H(hport)                                                                     \
	do {                                                                                           \
		++(hport);                                                                                 \
		if ((hport) > CONFIG_NET_DEFAULT_MAX_PORT || (hport) < CONFIG_NET_DEFAULT_MIN_PORT) {      \
...
unsigned int net_timeval2dsec(FAR struct timeval *tv, enum tv2ds_remainder_e remainder);

/****************************************************************************
 * Name: net_ipv4_mask2pref
 *
 * Description:
 *   Convert a 32-bit netmask to a prefix length.  The ROUX IPv4
 ...
 *
 * Description:
 *   Convert a 128-bit netmask to a prefix length.  The ROUX IPv6
...
FAR void *net_bufpool_timedalloc(FAR struct net_bufpool_s *pool, unsigned int timeout);
...
void net_chksum_adjust(FAR uint16_t *chksum, FAR const uint16_t *optr, ssize_t olen,
...
FAR void *cmsg_append(FAR struct msghdr *msg, int level, int type, FAR void *value, int value_len);
