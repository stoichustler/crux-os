...
static int netlink_bind(FAR struct socket *psock, FAR const struct sockaddr *addr,
						socklen_t addrlen);
static int netlink_getsockname(FAR struct socket *psock, FAR struct sockaddr *addr,
							   FAR socklen_t *addrlen);
static int netlink_getpeername(FAR struct socket *psock, FAR struct sockaddr *addr,
							   FAR socklen_t *addrlen);
static int netlink_connect(FAR struct socket *psock, FAR const struct sockaddr *addr,
						   socklen_t addrlen);
static int netlink_poll(FAR struct socket *psock, FAR struct pollfd *fds, bool setup);
static ssize_t netlink_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags);
static ssize_t netlink_recvmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags);
...
static int netlink_setup(FAR struct socket *psock)
{
	...
	if (domain == PF_NETLINK && (type == SOCK_RAW || type == SOCK_DGRAM || type == SOCK_CTRL)) {
...
#ifdef CONFIG_NETLINK_ROUTE
	case NETLINK_ROUTE:
		ret = netlink_route_sendto(conn, nlmsg, msg->msg_iov->iov_len, flags,
								   (FAR const struct sockaddr_nl *)to, tolen);
		break;
#endif

#ifdef CONFIG_NETLINK_NETFILTER
	case NETLINK_NETFILTER:
		ret = netlink_netfilter_sendto(conn, nlmsg, msg->msg_iov->iov_len, flags,
									   (FAR const struct sockaddr_nl *)to, tolen);
		break;
#endif
...
static ssize_t netlink_recvmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
	...
	DEBUGASSERT(from == NULL || (fromlen != NULL && *fromlen >= sizeof(struct sockaddr_nl)));
