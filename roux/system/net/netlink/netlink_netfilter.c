...
#define SIZEOF_CTNL_RECVFROM_RESPONSE_S(n) (sizeof(struct conntrack_recvfrom_response_s) + (n)-1)
...
#define SIZEOF_CTNL_RECVFROM_RSPLIST_S(n) (sizeof(struct conntrack_recvfrom_rsplist_s) + (n)-1)
...
static size_t netlink_conntrack_fill_ip(FAR void *buf, uint8_t domain, FAR const void *src,
										FAR const void *dst)
{
...
static size_t netlink_conntrack_fill_proto(FAR void *buf, uint8_t proto, uint16_t sport,
										   uint16_t dport, bool reply)
{
	switch (proto) {
...
#if defined(CONFIG_NET_TCP) || defined(CONFIG_NET_UDP)
	{
		...
		tuple_tcpudp->attr.nfa_len = sizeof(struct conntrack_tuple_tcpudp_s);
...
#if defined(CONFIG_NET_ICMP) || defined(CONFIG_NET_ICMPv6)
	{
		...
		if (proto == IPPROTO_ICMP) {
			...
			tuple_icmp->type.value = reply ? ICMP_ECHO_REPLY : ICMP_ECHO_REQUEST;
		...
		if (proto == IPPROTO_ICMP6) {
			...
			tuple_icmp->type.value = reply ? ICMPv6_ECHO_REPLY : ICMPv6_ECHO_REQUEST;
...
static FAR struct netlink_response_s *netlink_get_conntrack(FAR const struct nlmsghdr *req,
															uint16_t flags, uint8_t type,
															uint8_t domain, uint8_t proto,
															...
															FAR const void *ripaddr, uint16_t rport)
{
	...
	offset += netlink_conntrack_fill_ip(&entry->payload.data[offset], domain, lipaddr, ripaddr);
	offset +=
		netlink_conntrack_fill_proto(&entry->payload.data[offset], proto, lport, rport, false);
	...
	offset += netlink_conntrack_fill_ip(&entry->payload.data[offset], domain, ripaddr, eipaddr);
	offset += netlink_conntrack_fill_proto(&entry->payload.data[offset], proto, rport, eport, true);
...
#endif
	return netlink_get_conntrack(req, flags, type, PF_INET, entry->protocol, &entry->local_ip,
								 entry->local_port, &entry->external_ip, entry->external_port,
...
#ifdef CONFIG_NET_NAT66
static FAR struct netlink_response_s *netlink_get_ipv6_conntrack(FAR const struct nlmsghdr *req,
																 FAR const ipv6_nat_entry_t *entry,
																 uint16_t flags, uint8_t type)
{
	return netlink_get_conntrack(req, flags, type, PF_INET6, entry->protocol, entry->local_ip,
								 entry->local_port, entry->external_ip, entry->external_port,
...
#ifdef CONFIG_NET_NAT44
static void netlink_add_ipv4_conntrack(FAR ipv4_nat_entry_t *entry, FAR void *arg)
{
	FAR struct nfnl_info_s *info = arg;
	FAR struct netlink_response_s *resp;
	uint16_t flags = NLM_F_MULTI | NLM_F_DUMP_FILTERED;

	resp = netlink_get_ipv4_conntrack(&info->req->hdr, entry, flags, IPCTNL_MSG_CT_NEW);
	if (resp != NULL) {
...
#ifdef CONFIG_NET_NAT66
static void netlink_add_ipv6_conntrack(FAR ipv6_nat_entry_t *entry, FAR void *arg)
{
	FAR struct nfnl_info_s *info = arg;
	FAR struct netlink_response_s *resp;
	uint16_t flags = NLM_F_MULTI | NLM_F_DUMP_FILTERED;

	resp = netlink_get_ipv6_conntrack(&info->req->hdr, entry, flags, IPCTNL_MSG_CT_NEW);
...
ssize_t netlink_netfilter_sendto(NETLINK_HANDLE handle, FAR const struct nlmsghdr *nlmsg,
								 size_t len, int flags, FAR const struct sockaddr_nl *to,
								 socklen_t tolen)
{
	FAR const struct nfnl_sendto_request_s *req = (FAR const struct nfnl_sendto_request_s *)nlmsg;
	ssize_t ret = -ENOSYS;
	uint8_t subsys;
...
void netlink_conntrack_notify(uint8_t type, uint8_t domain, FAR const void *nat_entry)
{
	FAR struct netlink_response_s *resp;
	uint16_t flags;
	int group;
