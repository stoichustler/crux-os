...
#define nla_ok(nla, remaining)                                                                     \
	((remaining) >= sizeof(*(nla)) && (nla)->nla_len >= sizeof(*(nla)) &&                          \
...
#define nlmsg_parse(nlh, hdrlen, tb, maxtype, policy, extack)                                      \
	((nlh)->nlmsg_len < nlmsg_msg_size(hdrlen)                                                     \
		 ? -EINVAL                                                                                 \
		 : nla_parse(tb, maxtype, nlmsg_attrdata(nlh, hdrlen), nlmsg_attrlen(nlh, hdrlen), policy, \
					 extack))
...
int netlink_notifier_setup(worker_t worker, FAR struct netlink_conn_s *conn, FAR void *arg);
...
int netlink_add_terminator(NETLINK_HANDLE handle, FAR const struct nlmsghdr *req, int group);
...
FAR struct netlink_response_s *netlink_tryget_response(FAR struct netlink_conn_s *conn);
...
int netlink_get_response(FAR struct netlink_conn_s *conn, FAR struct netlink_response_s **response);
...
FAR struct nlattr *nla_next(FAR const struct nlattr *nla, FAR int *remaining);
...
int nla_parse(FAR struct nlattr **tb, int maxtype, FAR const struct nlattr *head, int len,
			  FAR const struct nla_policy *policy, FAR struct netlink_ext_ack *extack);
