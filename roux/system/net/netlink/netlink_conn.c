...
NET_BUFPOOL_DECLARE(g_netlink_connections, sizeof(struct netlink_conn_s),
					CONFIG_NETLINK_PREALLOC_CONNS, CONFIG_NETLINK_ALLOC_CONNS,
					CONFIG_NETLINK_MAX_CONNS);
...
static FAR struct netlink_response_s *netlink_get_terminator(FAR const struct nlmsghdr *req)
{
...
void netlink_add_response(NETLINK_HANDLE handle, FAR struct netlink_response_s *resp)
{
	FAR struct netlink_conn_s *conn;
...
int netlink_get_response(FAR struct netlink_conn_s *conn, FAR struct netlink_response_s **response)
{
	...
	while ((*response = netlink_tryget_response(conn)) == NULL) {
		...
		ret = netlink_notifier_setup(netlink_response_available, conn, &waitsem);
		if (ret < 0) {
