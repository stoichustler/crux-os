...
#if !defined(CONFIG_NET_ROUTE) || (!defined(CONFIG_NET_IPv4) && !defined(CONFIG_NET_IPv6))
...
#define SIZEOF_NLROUTE_RECVFROM_RESPONSE_S(n) (sizeof(struct getneigh_recvfrom_response_s) + (n)-1)
...
#define SIZEOF_NLROUTE_RECVFROM_RSPLIST_S(n) (sizeof(struct getneigh_recvfrom_rsplist_s) + (n)-1)
...
static_assert(sizeof(g_ifa_ipv4_policy) / sizeof(g_ifa_ipv4_policy[0]) == IFA_MAX + 1,
			  "The policy definition has changed,"
			  " please check it");
#endif
...
static_assert(sizeof(g_ifa_ipv6_policy) / sizeof(g_ifa_ipv6_policy[0]) == IFA_MAX + 1,
			  "The policy definition has changed,"
			  " please check it");
#endif
#else
...
netlink_get_device(FAR struct net_driver_s *dev, FAR const struct nlroute_sendto_request_s *req)
{
	...
	alloc = (FAR struct getlink_recvfrom_rsplist_s *)kmm_zalloc(
		sizeof(struct getlink_recvfrom_rsplist_s));
	if (alloc == NULL) {
	...
	if (dev->d_lltype == NET_LL_ETHERNET || dev->d_lltype == NET_LL_IEEE80211 ||
		dev->d_lltype == NET_LL_LOOPBACK || dev->d_lltype == NET_LL_TUN) {
...
#ifndef CONFIG_NETLINK_DISABLE_GETLINK
static FAR struct netlink_response_s *
netlink_get_ifaddr(FAR struct net_driver_s *dev, int domain, int type, FAR const void *local_addr,
				   uint8_t prefixlen, FAR const struct nlroute_sendto_request_s *req)
{
	...
	alloc = (FAR struct getaddr_recvfrom_rsplist_s *)kmm_zalloc(
		sizeof(struct getaddr_recvfrom_rsplist_s));
	if (alloc == NULL) {
		nerr("ERROR: Failed to allocate response buffer.\n");
		return NULL;
	}
...
#ifndef CONFIG_NETLINK_DISABLE_GETLINK
static int netlink_device_callback(FAR struct net_driver_s *dev, FAR void *arg)
{
...
static size_t netlink_fill_arptable(FAR struct getneigh_recvfrom_rsplist_s **entry)
{
	...
	net_lock();
	ncopied = arp_snapshot((FAR struct arpreq *)(*entry)->payload.data, CONFIG_NET_ARPTAB_SIZE);
	net_unlock();
...
static size_t netlink_fill_nbtable(FAR struct getneigh_recvfrom_rsplist_s **entry)
{
	unsigned int ncopied;
	size_t allocsize;
	size_t tabsize;
					   ...
					   FAR const struct nlroute_sendto_request_s *req)
{
	...
	alloc = (FAR struct getroute_recvfrom_ipv4resplist_s *)kmm_zalloc(
		sizeof(struct getroute_recvfrom_ipv4resplist_s));
	if (alloc == NULL) {
		return NULL;
	}
...
static int netlink_ipv4route_callback(FAR struct net_route_ipv4_s *route, FAR void *arg)
{
	FAR struct nlroute_info_s *info = arg;
	FAR struct netlink_response_s *resp;
					   ...
					   FAR const struct nlroute_sendto_request_s *req)
{
	...
	alloc = (FAR struct getroute_recvfrom_ipv6resplist_s *)kmm_zalloc(
		sizeof(struct getroute_recvfrom_ipv6resplist_s));
	if (alloc == NULL) {
		return NULL;
	}
...
static int netlink_ipv6route_callback(FAR struct net_route_ipv6_s *route, FAR void *arg)
{
	FAR struct nlroute_info_s *info = arg;
	FAR struct netlink_response_s *resp;
...
static int netlink_new_ipv4addr(NETLINK_HANDLE handle, FAR const struct nlmsghdr *nlmsg)
{
	...
	ret = nlmsg_parse(nlmsg, sizeof(*ifm), tb, IFA_MAX, g_ifa_ipv4_policy, &extack);
	if (ret < 0) {
		return ret;
	}
	...
	netlink_device_notify_ipaddr(dev, RTM_NEWADDR, AF_INET, &dev->d_ipaddr, ifm->ifa_prefixlen);
	net_unlock();
...
static int netlink_new_ipv6addr(NETLINK_HANDLE handle, FAR const struct nlmsghdr *nlmsg)
{
	FAR struct net_driver_s *dev;
	FAR struct ifaddrmsg *ifm = NLMSG_DATA(nlmsg);
	FAR struct nlattr *tb[IFA_MAX + 1];
	...
	ret = nlmsg_parse(nlmsg, sizeof(*ifm), tb, IFA_MAX, g_ifa_ipv6_policy, &extack);
	if (ret < 0) {
		return ret;
	}
	...
	if (ret == OK) {
		netlink_device_notify_ipaddr(dev, RTM_NEWADDR, AF_INET6, nla_data(tb[IFA_LOCAL]),
									 ifm->ifa_prefixlen);
	}
...
static int netlink_del_ipv4addr(NETLINK_HANDLE handle, FAR const struct nlmsghdr *nlmsg)
{
	FAR struct net_driver_s *dev;
	FAR struct ifaddrmsg *ifm = NLMSG_DATA(nlmsg);
	FAR struct nlattr *tb[IFA_MAX + 1];
...
static int netlink_del_ipv6addr(NETLINK_HANDLE handle, FAR const struct nlmsghdr *nlmsg)
{
	FAR struct net_driver_s *dev;
	FAR struct ifaddrmsg *ifm = NLMSG_DATA(nlmsg);
	FAR struct nlattr *tb[IFA_MAX + 1];
	...
	if (ret == OK) {
		netlink_device_notify_ipaddr(dev, RTM_DELADDR, AF_INET6, nla_data(tb[IFA_LOCAL]),
									 ifm->ifa_prefixlen);
	}
...
static int netlink_ipv6_addr_callback(FAR struct net_driver_s *dev,
									  FAR struct netdev_ifaddr6_s *addr, FAR void *arg)
{
	FAR struct nlroute_info_s *info = arg;
	FAR struct netlink_response_s *resp;
...
static int netlink_addr_callback(FAR struct net_driver_s *dev, FAR void *arg)
{
	...
	if (info->req->gen.rtgen_family == AF_INET) {
		resp = netlink_get_ifaddr(dev, AF_INET, RTM_NEWADDR, &dev->d_ipaddr,
								  net_ipv4_mask2pref(dev->d_netmask), info->req);
		if (resp == NULL) {
			return -ENOMEM;
		}
...
static int netlink_get_addr(NETLINK_HANDLE handle, FAR const struct nlroute_sendto_request_s *req)
{
	struct nlroute_info_s info;
	int ret;
...
ssize_t netlink_route_sendto(NETLINK_HANDLE handle, FAR const struct nlmsghdr *nlmsg, size_t len,
							 int flags, FAR const struct sockaddr_nl *to, socklen_t tolen)
{
	FAR const struct nlroute_sendto_request_s *req =
		(FAR const struct nlroute_sendto_request_s *)nlmsg;
	int ret;

	DEBUGASSERT(handle != NULL && nlmsg != NULL && nlmsg->nlmsg_len >= sizeof(struct nlmsghdr) &&
				len >= sizeof(struct nlmsghdr) && len >= nlmsg->nlmsg_len && to != NULL &&
...
#if !defined(CONFIG_NETLINK_DISABLE_NEWADDR) || !defined(CONFIG_NETLINK_DISABLE_DELADDR) ||        \
	!defined(CONFIG_NETLINK_DISABLE_GETADDR)
void netlink_device_notify_ipaddr(FAR struct net_driver_s *dev, int type, int domain,
...
#ifdef CONFIG_NET_IPv4
	if (domain == AF_INET) {
		resp = netlink_get_ipv4_route((FAR struct net_route_ipv4_s *)route, type, NULL);
		group = RTNLGRP_IPV4_ROUTE;
	} else
#endif
#ifdef CONFIG_NET_IPv6
		if (domain == AF_INET6) {
		resp = netlink_get_ipv6_route((FAR struct net_route_ipv6_s *)route, type, NULL);
