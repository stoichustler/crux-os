					...
					socklen_t value_len)
{
				  ...
				  * packets sent by the network stack */
	{
		...
		ttl = (value_len >= sizeof(int)) ? *(FAR int *)value : (int)*(FAR unsigned char *)value;
...
#ifdef NET_UDP_HAVE_STACK
	{
		...
		if (value_len >= sizeof(struct ip_mreqn)) {
			...
			if (value_len >= sizeof(struct ip_mreq)) {
				...
				memcpy(&mreq.imr_multiaddr, value, sizeof(struct in_addr));
			}
		}
		...
		if (conn->sconn.s_boundto && mreq.imr_ifindex != conn->sconn.s_boundto) {
			ret = -EINVAL;
			break;
		}
#endif
	...
	case IP_PKTINFO: {
		...
		enable = (value_len >= sizeof(int)) ? *(FAR int *)value : (int)*(FAR unsigned char *)value;
	...
	case IP_TOS: {
		...
		tos = (value_len >= sizeof(int)) ? *(FAR int *)value : (int)*(FAR unsigned char *)value;
		if (tos < 0 || tos > 0xff) {
