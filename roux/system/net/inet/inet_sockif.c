...
static int inet_bind(FAR struct socket *psock, FAR const struct sockaddr *addr, socklen_t addrlen);
static int inet_getsockname(FAR struct socket *psock, FAR struct sockaddr *addr,
							FAR socklen_t *addrlen);
static int inet_getpeername(FAR struct socket *psock, FAR struct sockaddr *addr,
							FAR socklen_t *addrlen);
static int inet_listen(FAR struct socket *psock, int backlog);
static int inet_connect(FAR struct socket *psock, FAR const struct sockaddr *addr,
						socklen_t addrlen);
static int inet_accept(FAR struct socket *psock, FAR struct sockaddr *addr, FAR socklen_t *addrlen,
					   FAR struct socket *newsock, int flags);
static int inet_poll(FAR struct socket *psock, FAR struct pollfd *fds, bool setup);
static ssize_t inet_send(FAR struct socket *psock, FAR const void *buf, size_t len, int flags);
static ssize_t inet_sendto(FAR struct socket *psock, FAR const void *buf, size_t len, int flags,
						   FAR const struct sockaddr *to, socklen_t tolen);
static ssize_t inet_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags);
static ssize_t inet_recvmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags);
...
static int inet_getsockopt(FAR struct socket *psock, int level, int option, FAR void *value,
						   FAR socklen_t *value_len);
static int inet_setsockopt(FAR struct socket *psock, int level, int option, FAR const void *value,
						   socklen_t value_len);
#endif
#ifdef CONFIG_NET_SENDFILE
static ssize_t inet_sendfile(FAR struct socket *psock, FAR struct file *infile, FAR off_t *offset,
							 size_t count);
#endif
...
static void inet_addref(FAR struct socket *psock)
{
	...
	if (psock->s_type == SOCK_STREAM ||
		(psock->s_type == SOCK_CTRL && (psock->s_proto == 0 || psock->s_proto == IPPROTO_TCP))) {
...
static int inet_get_socketlevel_option(FAR struct socket *psock, int option, FAR void *value,
									   FAR socklen_t *value_len)
{
	switch (option) {
...
static int inet_set_socketlevel_option(FAR struct socket *psock, int option, FAR const void *value,
									   socklen_t value_len)
{
	switch (option) {
#ifdef CONFIG_NET_TCPPROTO_OPTIONS
...
static int inet_listen(FAR struct socket *psock, int backlog)
{
	...
	if (conn->lport == 0) {
...
#endif
		{
			...
			conn->lport =
				tcp_selectport(PF_INET, (FAR const union ip_addr_u *)&conn->u.ipv4.laddr, 0);
		}
...
#endif
		{
			...
			conn->lport =
				tcp_selectport(PF_INET6, (FAR const union ip_addr_u *)conn->u.ipv6.laddr, 0);
		}
...
#if defined(NET_TCP_HAVE_STACK) || defined(NET_UDP_HAVE_STACK)
static inline int inet_pollsetup(FAR struct socket *psock, FAR struct pollfd *fds)
{
#ifdef NET_TCP_HAVE_STACK
	if (psock->s_type == SOCK_STREAM) {
...
#if defined(NET_TCP_HAVE_STACK) || defined(NET_UDP_HAVE_STACK)
static inline int inet_pollteardown(FAR struct socket *psock, FAR struct pollfd *fds)
{
#ifdef NET_TCP_HAVE_STACK
	if (psock->s_type == SOCK_STREAM) {
...
static ssize_t inet_send(FAR struct socket *psock, FAR const void *buf, size_t len, int flags)
{
	...
	case SOCK_DGRAM: {
		...
		if (ret < 0) {
			...
			ret = _SS_ISCONNECTED(conn->s_flags) ? psock_udp_sendto(psock, buf, len, 0, NULL, 0)
												 : -ENOTCONN;
		}
#endif /* NET_UDP_HAVE_STACK */
...
static ssize_t inet_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
	...
	ret = to ? inet_sendto(psock, buf, len, flags, to, tolen) : inet_send(psock, buf, len, flags);
...
#if defined(CONFIG_NET_UDP) && defined(NET_UDP_HAVE_STACK)
	if (psock->s_type == SOCK_DGRAM ||
		(psock->s_type == SOCK_CTRL && (psock->s_proto == 0 || psock->s_proto == IPPROTO_UDP))) {
...
static int inet_socketpair(FAR struct socket *psocks[2])
{
	...
	if (psocks[0]->s_type == SOCK_STREAM) {
		ret = psock_socket(psocks[1]->s_domain, psocks[1]->s_type, psocks[1]->s_proto, &server);
		if (ret < 0) {
			return ret;
		}
...
FAR const struct sock_intf_s *inet_sockif(sa_family_t family, int type, int protocol)
{
	...
	if (family == PF_INET && (type == SOCK_DGRAM || type == SOCK_CTRL || type == SOCK_RAW) &&
		...
		if (family == PF_INET6 && (type == SOCK_DGRAM || type == SOCK_CTRL || type == SOCK_RAW) &&
