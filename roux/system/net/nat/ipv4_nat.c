...
#define L4_HDR(ipv4) (FAR void *)((FAR uint8_t *)(ipv4) + (((ipv4)->vhl & IPv4_HLMASK) << 2))
...
static FAR ipv4_nat_entry_t *ipv4_nat_inbound_internal(FAR struct ipv4_hdr_s *ipv4,
													   enum nat_manip_type_e manip_type);

static FAR ipv4_nat_entry_t *ipv4_nat_outbound_internal(FAR struct net_driver_s *dev,
...
static void ipv4_nat_ip_adjust(FAR struct ipv4_hdr_s *ipv4, FAR uint16_t *l4chksum,
							   FAR uint16_t *old_ip, in_addr_t new_ip)
{
	if (l4chksum != NULL) {
...
static void ipv4_nat_port_adjust(FAR uint16_t *l4chksum, FAR uint16_t *old_port, uint16_t new_port)
{
	if (l4chksum != NULL) {
...
#ifdef CONFIG_NET_TCP
static FAR ipv4_nat_entry_t *ipv4_nat_inbound_tcp(FAR struct ipv4_hdr_s *ipv4,
												  enum nat_manip_type_e manip_type)
{
	...
	FAR ipv4_nat_entry_t *entry =
		ipv4_nat_inbound_entry_find(IP_PROTO_TCP, net_ip4addr_conv32(external_ip), *external_port,
									net_ip4addr_conv32(peer_ip), *peer_port, true);
	if (!entry) {
		return NULL;
	}
...
#ifdef CONFIG_NET_UDP
static FAR ipv4_nat_entry_t *ipv4_nat_inbound_udp(FAR struct ipv4_hdr_s *ipv4,
												  enum nat_manip_type_e manip_type)
{
...
#ifdef CONFIG_NET_ICMP
static FAR ipv4_nat_entry_t *ipv4_nat_inbound_icmp(FAR struct ipv4_hdr_s *ipv4,
												   enum nat_manip_type_e manip_type)
{
	...
	case ICMP_ECHO_REPLY:
		entry = ipv4_nat_inbound_entry_find(IP_PROTO_ICMP, net_ip4addr_conv32(external_ip),
											icmp->id, net_ip4addr_conv32(peer_ip), icmp->id, true);
		if (!entry) {
			return NULL;
		}

		ipv4_nat_port_adjust(&icmp->icmpchksum, &icmp->id, entry->local_port);
	...
	case ICMP_PARAMETER_PROBLEM:
		...
		if (manip_type == NAT_MANIP_DST) {
			...
			FAR struct ipv4_hdr_s *inner = (FAR struct ipv4_hdr_s *)(icmp + 1);
			FAR void *inner_l4;
			...
			DEBUGASSERT((intptr_t)inner_l4 - (intptr_t)ipv4 + inner_l4hdrlen <= CONFIG_IOB_BUFSIZE);
			...
			nat_chksum_adjust(&icmp->icmpchksum, inner_l4hdrbak, inner_l4, inner_l4hdrlen);
...
#ifdef CONFIG_NET_TCP
static FAR ipv4_nat_entry_t *ipv4_nat_outbound_tcp(FAR struct net_driver_s *dev,
												   FAR struct ipv4_hdr_s *ipv4,
												   enum nat_manip_type_e manip_type)
{
	FAR struct tcp_hdr_s *tcp = L4_HDR(ipv4);
	...
	entry = ipv4_nat_outbound_entry_find(dev, IP_PROTO_TCP, net_ip4addr_conv32(local_ip),
										 *local_port, net_ip4addr_conv32(peer_ip), *peer_port,
...
#ifdef CONFIG_NET_UDP
static FAR ipv4_nat_entry_t *ipv4_nat_outbound_udp(FAR struct net_driver_s *dev,
												   FAR struct ipv4_hdr_s *ipv4,
												   enum nat_manip_type_e manip_type)
{
	FAR struct udp_hdr_s *udp = L4_HDR(ipv4);
	...
	entry = ipv4_nat_outbound_entry_find(dev, IP_PROTO_UDP, net_ip4addr_conv32(local_ip),
										 *local_port, net_ip4addr_conv32(peer_ip), *peer_port,
										 manip_type == NAT_MANIP_SRC);
	if (!entry) {
		return NULL;
...
#ifdef CONFIG_NET_ICMP
static FAR ipv4_nat_entry_t *ipv4_nat_outbound_icmp(FAR struct net_driver_s *dev,
													FAR struct ipv4_hdr_s *ipv4,
													enum nat_manip_type_e manip_type)
{
	FAR struct icmp_hdr_s *icmp = L4_HDR(ipv4);
	...
	case ICMP_ECHO_REPLY:
		...
		entry = ipv4_nat_outbound_entry_find(dev, IP_PROTO_ICMP, net_ip4addr_conv32(local_ip),
											 icmp->id, net_ip4addr_conv32(peer_ip), icmp->id,
											 manip_type == NAT_MANIP_SRC);
		if (!entry) {
			return NULL;
		}

		ipv4_nat_port_adjust(&icmp->icmpchksum, &icmp->id, entry->external_port);
...
void ipv4_nat_inbound(FAR struct net_driver_s *dev, FAR struct ipv4_hdr_s *ipv4)
{
	/* We only process packets from NAT device and targeting at the address
	 * assigned to the device.
	 */

	if (IFF_IS_NAT(dev->d_flags) && net_ipv4addr_hdrcmp(ipv4->destipaddr, &dev->d_ipaddr)) {
...
int ipv4_nat_outbound(FAR struct net_driver_s *dev, FAR struct ipv4_hdr_s *ipv4,
					  enum nat_manip_type_e manip_type)
{
	...
	if (IFF_IS_NAT(dev->d_flags) && !net_ipv4addr_hdrcmp(ipv4->srcipaddr, &dev->d_ipaddr) &&
		!net_ipv4addr_hdrcmp(ipv4->destipaddr, &dev->d_ipaddr)) {
		/* TODO: Skip broadcast? */

		FAR ipv4_nat_entry_t *entry = ipv4_nat_outbound_internal(dev, ipv4, manip_type);
