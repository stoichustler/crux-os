...
static inline uint32_t ipv4_nat_inbound_key(in_addr_t external_ip, uint16_t external_port,
											uint8_t protocol)
{
...
static inline uint32_t ipv4_nat_outbound_key(in_addr_t local_ip, uint16_t local_port,
											 uint8_t protocol)
{
...
static FAR ipv4_nat_entry_t *ipv4_nat_entry_create(uint8_t protocol, in_addr_t external_ip,
												   uint16_t external_port, in_addr_t local_ip,
												   uint16_t local_port, in_addr_t peer_ip,
												   uint16_t peer_port)
{
...
{
	ninfo("INFO: Removing NAT44 entry proto=%" PRIu8 ", local=%" PRIx32 ":%" PRIu16
		  ", external=:%" PRIu16 "\n",
		  entry->protocol, entry->local_ip, entry->local_port, entry->external_port);

	hashtable_delete(
		g_nat44_inbound, &entry->hash_inbound,
		ipv4_nat_inbound_key(entry->external_ip, entry->external_port, entry->protocol));
	hashtable_delete(g_nat44_outbound, &entry->hash_outbound,
					 ipv4_nat_outbound_key(entry->local_ip, entry->local_port, entry->protocol));
...
static void ipv4_nat_reclaim_entry_cb(FAR ipv4_nat_entry_t *entry, FAR void *arg)
{
...
static void ipv4_nat_entry_clear_cb(FAR ipv4_nat_entry_t *entry, FAR void *arg)
{
...
void ipv4_nat_entry_foreach(ipv4_nat_entry_cb_t cb, FAR void *arg)
{
	...
	{
		FAR ipv4_nat_entry_t *entry = container_of(p, ipv4_nat_entry_t, hash_inbound);
...
FAR ipv4_nat_entry_t *ipv4_nat_inbound_entry_find(uint8_t protocol, in_addr_t external_ip,
...
#ifdef CONFIG_NET_NAT44_SYMMETRIC
			&& (skip_peer ||
				(net_ipv4addr_cmp(entry->peer_ip, peer_ip) && entry->peer_port == peer_port))
#endif
		) {
			if (refresh) {
				ipv4_nat_entry_refresh(entry);
...
FAR ipv4_nat_entry_t *ipv4_nat_outbound_entry_find(FAR struct net_driver_s *dev, uint8_t protocol,
												   ...
												   bool try_create)
{
	...
	hashtable_for_every_possible_safe(g_nat44_outbound, p, tmp,
									  ipv4_nat_outbound_key(local_ip, local_port, protocol))
	{
		FAR ipv4_nat_entry_t *entry = container_of(p, ipv4_nat_entry_t, hash_outbound);
		...
		if (entry->protocol == protocol && net_ipv4addr_cmp(entry->external_ip, dev->d_ipaddr) &&
			net_ipv4addr_cmp(entry->local_ip, local_ip) && entry->local_port == local_port
#ifdef CONFIG_NET_NAT44_SYMMETRIC
			&& net_ipv4addr_cmp(entry->peer_ip, peer_ip) && entry->peer_port == peer_port
#endif
		) {
	...
	external_port =
		nat_port_select(dev, PF_INET, protocol, (FAR union ip_addr_u *)&dev->d_ipaddr, local_port);
	if (!external_port) {
		nwarn("WARNING: Failed to find an available port!\n");
		return NULL;
	}

	return ipv4_nat_entry_create(protocol, dev->d_ipaddr, external_port, local_ip, local_port,
								 peer_ip, peer_port);
}
