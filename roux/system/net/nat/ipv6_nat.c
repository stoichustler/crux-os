...
static FAR ipv6_nat_entry_t *ipv6_nat_inbound_internal(FAR struct ipv6_hdr_s *ipv6,
													   enum nat_manip_type_e manip_type);

static FAR ipv6_nat_entry_t *ipv6_nat_outbound_internal(FAR struct net_driver_s *dev,
...
static void ipv6_nat_ip_adjust(FAR uint16_t *l4chksum, FAR uint16_t *old_ip, net_ipv6addr_t new_ip)
{
...
static void ipv6_nat_port_adjust(FAR uint16_t *l4chksum, FAR uint16_t *old_port, uint16_t new_port)
{
	if (l4chksum != NULL) {
...
static FAR ipv6_nat_entry_t *ipv6_nat_inbound_tcp(FAR struct ipv6_hdr_s *ipv6,
												  ...
												  enum nat_manip_type_e manip_type)
{
	...
	FAR ipv6_nat_entry_t *entry = ipv6_nat_inbound_entry_find(
		IP_PROTO_TCP, external_ip, *external_port, peer_ip, *peer_port, true);
	if (!entry) {
		return NULL;
	}
...
static FAR ipv6_nat_entry_t *ipv6_nat_inbound_udp(FAR struct ipv6_hdr_s *ipv6,
												  ...
												  enum nat_manip_type_e manip_type)
{
	...
	FAR ipv6_nat_entry_t *entry = ipv6_nat_inbound_entry_find(
		IP_PROTO_UDP, external_ip, *external_port, peer_ip, *peer_port, true);
...
static FAR ipv6_nat_entry_t *ipv6_nat_inbound_icmpv6(FAR struct ipv6_hdr_s *ipv6,
													 ...
													 enum nat_manip_type_e manip_type)
{
	...
	case ICMPv6_ECHO_REPLY:
		entry = ipv6_nat_inbound_entry_find(IP_PROTO_ICMP6, external_ip, icmpv6->data[0], peer_ip,
											icmpv6->data[0], true);
		if (!entry) {
			return NULL;
		}

		ipv6_nat_port_adjust(&icmpv6->chksum, &icmpv6->data[0], entry->local_port);
	...
	case ICMPv6_PACKET_PARAM_PROBLEM:
		...
		if (manip_type == NAT_MANIP_DST) {
			...
			FAR struct ipv6_hdr_s *inner = (FAR struct ipv6_hdr_s *)(icmpv6 + 1);
...
#ifdef CONFIG_NET_TCP
static FAR ipv6_nat_entry_t *ipv6_nat_outbound_tcp(FAR struct net_driver_s *dev,
												   FAR struct ipv6_hdr_s *ipv6,
												   FAR struct tcp_hdr_s *tcp,
												   enum nat_manip_type_e manip_type)
{
	...
	entry = ipv6_nat_outbound_entry_find(dev, IP_PROTO_TCP, local_ip, *local_port, peer_ip,
										 *peer_port, manip_type == NAT_MANIP_SRC);
	if (!entry) {
		return NULL;
	}
...
#ifdef CONFIG_NET_UDP
static FAR ipv6_nat_entry_t *ipv6_nat_outbound_udp(FAR struct net_driver_s *dev,
												   FAR struct ipv6_hdr_s *ipv6,
												   FAR struct udp_hdr_s *udp,
												   enum nat_manip_type_e manip_type)
{
	...
	entry = ipv6_nat_outbound_entry_find(dev, IP_PROTO_UDP, local_ip, *local_port, peer_ip,
										 *peer_port, manip_type == NAT_MANIP_SRC);
	if (!entry) {
		return NULL;
	}
...
#ifdef CONFIG_NET_ICMPv6
static FAR ipv6_nat_entry_t *ipv6_nat_outbound_icmpv6(FAR struct net_driver_s *dev,
													  FAR struct ipv6_hdr_s *ipv6,
													  FAR struct icmpv6_hdr_s *icmpv6,
													  enum nat_manip_type_e manip_type)
{
	...
	case ICMPv6_ECHO_REPLY:
		...
		entry = ipv6_nat_outbound_entry_find(dev, IP_PROTO_ICMP6, local_ip, icmpv6->data[0],
											 peer_ip, icmpv6->data[0], manip_type == NAT_MANIP_SRC);
		if (!entry) {
			return NULL;
		}

		ipv6_nat_port_adjust(&icmpv6->chksum, &icmpv6->data[0], entry->external_port);
...
void ipv6_nat_inbound(FAR struct net_driver_s *dev, FAR struct ipv6_hdr_s *ipv6)
{
	/* We only process packets from NAT device and targeting at the address
	 * assigned to the device.
	 */

	if (IFF_IS_NAT(dev->d_flags) && NETDEV_IS_MY_V6ADDR(dev, ipv6->destipaddr)) {
...
int ipv6_nat_outbound(FAR struct net_driver_s *dev, FAR struct ipv6_hdr_s *ipv6,
					  enum nat_manip_type_e manip_type)
{
	...
	if (IFF_IS_NAT(dev->d_flags) && !NETDEV_IS_MY_V6ADDR(dev, ipv6->srcipaddr) &&
		!NETDEV_IS_MY_V6ADDR(dev, ipv6->destipaddr)) {
		FAR ipv6_nat_entry_t *entry = ipv6_nat_outbound_internal(dev, ipv6, manip_type);
