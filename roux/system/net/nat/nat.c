...
static uint16_t nat_port_select_without_stack(uint8_t domain, uint8_t protocol,
											  FAR const union ip_addr_u *ip, uint16_t local_port)
{
...
bool nat_port_inuse(uint8_t domain, uint8_t protocol, FAR const union ip_addr_u *ip, uint16_t port)
{
#ifdef CONFIG_NET_NAT44
	if (domain == PF_INET) {
		return !!ipv4_nat_inbound_entry_find(protocol, ip->ipv4, port, INADDR_ANY, 0, false);
	}
#endif

#ifdef CONFIG_NET_NAT66
	if (domain == PF_INET6) {
		return !!ipv6_nat_inbound_entry_find(protocol, ip->ipv6, port, g_ipv6_unspecaddr, 0, false);
	}
#endif
...
uint16_t nat_port_select(FAR struct net_driver_s *dev, uint8_t domain, uint8_t protocol,
						 FAR const union ip_addr_u *external_ip, uint16_t local_port)
{
	switch (protocol) {
	...
	case IP_PROTO_TCP: {
		...
		return ret > 0 ? ret : 0;
#else
		return nat_port_select_without_stack(domain, IP_PROTO_TCP, external_ip, local_port);
#endif
	}
#endif
...
#else
		return nat_port_select_without_stack(domain, IP_PROTO_UDP, external_ip, local_port);
#endif
	}
#endif
	...
	case IP_PROTO_ICMP: {
		...
		while (icmp_findconn(dev, id) || nat_port_inuse(domain, IP_PROTO_ICMP, external_ip, id)) {
		...
		return id;
#else
		return nat_port_select_without_stack(domain, IP_PROTO_ICMP, external_ip, local_port);
#endif
	}
#endif
	...
	case IP_PROTO_ICMP6: {
		...
		uint16_t id = local_port;
		uint16_t hid = NTOHS(id);
		while (icmpv6_active(id) || nat_port_inuse(domain, IP_PROTO_ICMP6, external_ip, id)) {
			NET_PORT_NEXT_NH(id, hid);
			if (id == local_port) {
				/* We have looped back, failed. */
		...
		return id;
#else
		return nat_port_select_without_stack(domain, IP_PROTO_ICMP6, external_ip, local_port);
#endif
	}
#endif
	}
...
uint32_t nat_expire_time(uint8_t protocol)
{
	...
	case IP_PROTO_TCP:
		...
		return TICK2SEC(clock_systime_ticks()) + CONFIG_NET_NAT_TCP_EXPIRE_SEC;
#endif

#ifdef CONFIG_NET_UDP
	case IP_PROTO_UDP:
		return TICK2SEC(clock_systime_ticks()) + CONFIG_NET_NAT_UDP_EXPIRE_SEC;
#endif

#ifdef CONFIG_NET_ICMP
	case IP_PROTO_ICMP:
		return TICK2SEC(clock_systime_ticks()) + CONFIG_NET_NAT_ICMP_EXPIRE_SEC;
#endif

#ifdef CONFIG_NET_ICMPv6
	case IP_PROTO_ICMP6:
		return TICK2SEC(clock_systime_ticks()) + CONFIG_NET_NAT_ICMPv6_EXPIRE_SEC;
#endif
