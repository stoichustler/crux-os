...
NET_BUFPOOL_DECLARE(g_bluetooth_connections, sizeof(struct bluetooth_conn_s),
					CONFIG_NET_BLUETOOTH_PREALLOC_CONNS, CONFIG_NET_BLUETOOTH_ALLOC_CONNS,
...
static const bt_addr_t g_any_addr = {BT_ADDR_ANY};
...
FAR struct bluetooth_conn_s *bluetooth_conn_active(FAR const struct bluetooth_frame_meta_s *meta)
{
	...
	for (conn = (FAR struct bluetooth_conn_s *)g_active_bluetooth_connections.head; conn != NULL;
		 conn = (FAR struct bluetooth_conn_s *)conn->bc_conn.node.flink) {
		/* match protocol and channel first */

		if (meta->bm_proto != conn->bc_proto || meta->bm_channel != conn->bc_channel) {
			continue;
		}
...
{
	if (!conn) {
		return (FAR struct bluetooth_conn_s *)g_active_bluetooth_connections.head;
	} else {
