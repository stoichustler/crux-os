...
static uint16_t bluetooth_sendto_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
											  uint16_t flags)
{
	...
	if (pstate != NULL && (flags & BLUETOOTH_POLL) != 0) {
		...
		if (pstate->is_buflen + hdrlen > BLUETOOTH_MAX_FRAMELEN) {
			nerr("ERROR: User buffer will not fit into the frame: %u > %u\n",
				 (unsigned int)(pstate->is_buflen + hdrlen), (unsigned int)CONFIG_IOB_BUFSIZE);
			ret = -E2BIG;
			goto errout;
		}
...
static ssize_t bluetooth_sendto(FAR struct socket *psock, FAR const void *buf, size_t len,
								int flags, FAR const struct sockaddr *to, socklen_t tolen)
{
	...
	if (psock->s_proto == BTPROTO_L2CAP) {
		...
		radio = (FAR struct radio_driver_s *)netdev_findbyindex(conn->bc_ldev + 1);
	...
	if (psock->s_proto == BTPROTO_L2CAP) {
		...
		memcpy(&state.is_destaddr, &destaddr->l2_bdaddr, sizeof(bt_addr_t));
	} else if (psock->s_proto == BTPROTO_HCI) {
...
static ssize_t bluetooth_l2cap_send(FAR struct socket *psock, FAR const void *buf, size_t len,
									int flags)
{
	...
	if (!_SS_ISCONNECTED(conn->bc_conn.s_flags)) {
		...
		ret = bluetooth_sendto(psock, buf, len, flags, (FAR const struct sockaddr *)&to,
...
static ssize_t bluetooth_hci_send(FAR struct socket *psock, FAR const void *buf, size_t len,
								  int flags)
{
...
static ssize_t bluetooth_send(FAR struct socket *psock, FAR const void *buf, size_t len, int flags)
{
	ssize_t ret;
...
ssize_t bluetooth_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
