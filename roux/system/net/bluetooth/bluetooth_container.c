...
static struct bluetooth_container_s g_container_pool[CONFIG_NET_BLUETOOTH_NCONTAINERS];
...
void bluetooth_container_initialize(void)
{
	...
	for (i = 0, container = g_container_pool; i < CONFIG_NET_BLUETOOTH_NCONTAINERS;
		 i++, container++) {
...
FAR struct bluetooth_container_s *bluetooth_container_allocate(void)
{
	...
	if (g_free_container != NULL) {
		...
		net_unlock();
		container =
			(FAR struct bluetooth_container_s *)kmm_malloc((sizeof(struct bluetooth_container_s)));
