...
#endif
#ifdef CONFIG_NET_IPv6
#if defined(CONFIG_NETDEV_MULTIPLE_IPv6) && defined(CONFIG_DESIGNATED_INITIALIZERS)
	,
...
static int netprocfs_radio_linklayer(FAR struct netprocfs_file_s *netfile, int len)
{
	...
	len += snprintf(&netfile->line[len], NET_LINELEN - len, "%s\tLink encap:6LoWPAN HWaddr ",
					dev->d_ifname);
#else
	len += snprintf(&netfile->line[len], NET_LINELEN - len, "%s\tLink encap:Raw HWaddr ",
					dev->d_ifname);
#endif
	...
	if (addr->nv_addrlen < 1 || addr->nv_addrlen > RADIO_MAX_ADDRLEN) {
		...
		len += snprintf(&netfile->line[len], NET_LINELEN - len, "%02x", addr->nv_addr[0]);

#if RADIO_MAX_ADDRLEN > 1 /* Avoids a warning */
		for (i = 1; i < addr->nv_addrlen; i++) {
			len += snprintf(&netfile->line[len], NET_LINELEN - len, ":%02x", addr->nv_addr[i]);
		}
#endif
	}
...
static int netprocfs_linklayer(FAR struct netprocfs_file_s *netfile)
{
	...
	case NET_LL_IEEE80211: {
		char hwaddr[20];
		len += snprintf(&netfile->line[len], NET_LINELEN - len, "%s\tLink encap:Ethernet HWaddr %s",
						dev->d_ifname, ether_ntoa_r(&dev->d_mac.ether, hwaddr));
	} break;
#endif
	...
	case NET_LL_LOOPBACK:
		len += snprintf(&netfile->line[len], NET_LINELEN - len, "%s\tLink encap:Local Loopback",
						dev->d_ifname);
		break;
#endif
	...
	case NET_LL_SLIP:
		len +=
			snprintf(&netfile->line[len], NET_LINELEN - len, "%s\tLink encap:SLIP", dev->d_ifname);
		break;
#endif
	...
	case NET_LL_PPP:
		len +=
			snprintf(&netfile->line[len], NET_LINELEN - len, "%s\tLink encap:P-t-P", dev->d_ifname);
		break;
#endif
	...
	case NET_LL_TUN:
		len +=
			snprintf(&netfile->line[len], NET_LINELEN - len, "%s\tLink encap:TUN", dev->d_ifname);
		break;
#endif

	default:
		len += snprintf(&netfile->line[len], NET_LINELEN - len, "%s\tLink encap:UNSPEC",
						dev->d_ifname);
	}

	len += snprintf(&netfile->line[len], NET_LINELEN - len, " at %s", status);

	len += snprintf(&netfile->line[len], NET_LINELEN - len, " mtu %d\n",
...
static int netprocfs_inet4addresses(FAR struct netprocfs_file_s *netfile)
{
	...
	len += snprintf(&netfile->line[len], NET_LINELEN - len, "DRaddr:%s ",
					inet_ntoa_r(addr, inetaddr, sizeof(inetaddr)));
...
static int netprocfs_inet6draddress(FAR struct netprocfs_file_s *netfile)
{
	...
	if (inet_ntop(AF_INET6, dev->d_ipv6draddr, addrstr, INET6_ADDRSTRLEN)) {
		len += snprintf(&netfile->line[len], NET_LINELEN - len, "\tinet6 DRaddr: %s\n\n", addrstr);
	}
...
static int netprocfs_rxstatistics(FAR struct netprocfs_file_s *netfile)
{
	...
	return snprintf(netfile->line, NET_LINELEN, "\t    %08lx %08lx %08lx %-16llx\n",
					(unsigned long)stats->rx_packets, (unsigned long)stats->rx_fragments,
					(unsigned long)stats->rx_errors, (unsigned long long)stats->rx_bytes);
}
#endif /* CONFIG_NETDEV_STATISTICS */
...
static int netprocfs_txstatistics_header(FAR struct netprocfs_file_s *netfile)
{
	...
	return snprintf(netfile->line, NET_LINELEN, "\tTX: %-8s %-8s %-8s %-8s %-8s\n", "Queued",
					"Sent", "Errors", "Timeouts", "Bytes");
}
#endif /* CONFIG_NETDEV_STATISTICS */
...
static int netprocfs_txstatistics(FAR struct netprocfs_file_s *netfile)
{
	...
	return snprintf(netfile->line, NET_LINELEN, "\t    %08lx %08lx %08lx %08lx %-16llx \n",
					(unsigned long)stats->tx_packets, (unsigned long)stats->tx_done,
					(unsigned long)stats->tx_errors, (unsigned long)stats->tx_timeouts,
...
ssize_t netprocfs_read_devstats(FAR struct netprocfs_file_s *priv, FAR char *buffer, size_t buflen)
{
	return netprocfs_read_linegen(priv, buffer, buflen, g_netstat_linegen, NSTAT_LINES);
}
