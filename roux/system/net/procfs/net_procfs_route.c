...
static void route_sprintf(FAR struct route_info_s *info, FAR const char *fmt, ...)
	printf_like(2, 3);
#ifdef CONFIG_NET_IPv4
...
#endif
#ifdef CONFIG_NET_IPv4
static ssize_t route_ipv4_table(FAR struct route_file_s *procfile, FAR char *buffer, size_t buflen,
								off_t offset);
#endif
#ifdef CONFIG_NET_IPv6
static ssize_t route_ipv6_table(FAR struct route_file_s *procfile, FAR char *buffer, size_t buflen,
								off_t offset);
#endif

/* File system methods */

static int route_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int route_close(FAR struct file *filep);
static ssize_t route_read(FAR struct file *filep, FAR char *buffer, size_t buflen);

static int route_dup(FAR const struct file *oldp, FAR struct file *newp);

static int route_opendir(const char *relpath, FAR struct fs_dirent_s **dir);
static int route_closedir(FAR struct fs_dirent_s *dir);
static int route_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry);
...
static int route_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	procfile = (FAR struct route_file_s *)kmm_zalloc(sizeof(struct route_file_s));
	if (!procfile) {
...
static int route_dup(FAR const struct file *oldp, FAR struct file *newp)
{
	...
	newfile = (FAR struct route_file_s *)kmm_malloc(sizeof(struct route_file_s));
	if (!newfile) {
		ferr("ERROR: Failed to allocate file container\n");
		return -ENOMEM;
	}
...
static int route_opendir(FAR const char *relpath, FAR struct fs_dirent_s **dir)
{
	...
	level2 = (FAR struct route_dir_s *)kmm_zalloc(sizeof(struct route_dir_s));
	if (!level2) {
