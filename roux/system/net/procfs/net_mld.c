...
static const linegen_t g_mld_linegen[] = {
	...
	netprocfs_done_sent,		netprocfs_queries_received_1, netprocfs_queries_received_2,
	netprocfs_reports_received, netprocfs_done_received};
...
static int netprocfs_joinleave(FAR struct netprocfs_file_s *netfile)
{
	...
	len = snprintf(netfile->line, NET_LINELEN, "Joins: %04x ", g_netstats.mld.njoins);
	len +=
		snprintf(&netfile->line[len], NET_LINELEN - len, "Leaves: %04x\n", g_netstats.mld.nleaves);
	return len;
}
...
static int netprocfs_reports_sent(FAR struct netprocfs_file_s *netfile)
{
	...
	len += snprintf(&netfile->line[len], NET_LINELEN - len, "    Ver 2: %04x  %04x\n",
...
static int netprocfs_queries_received_1(FAR struct netprocfs_file_s *netfile)
{
	...
	len += snprintf(&netfile->line[len], NET_LINELEN - len, "    MAS:   %04x\n",
...
static int netprocfs_queries_received_2(FAR struct netprocfs_file_s *netfile)
{
	...
	len = snprintf(netfile->line, NET_LINELEN, "    MASS:  %04x\n",
				   g_netstats.mld.mass_query_received);
	len += snprintf(&netfile->line[len], NET_LINELEN - len, "    Ucast: %04x\n",
					g_netstats.mld.ucast_query_received);
	len += snprintf(&netfile->line[len], NET_LINELEN - len, "    Bad:   %04x\n",
...
static int netprocfs_reports_received(FAR struct netprocfs_file_s *netfile)
{
	...
	len += snprintf(&netfile->line[len], NET_LINELEN - len, "    Ver 2: %04x\n",
...
ssize_t netprocfs_read_mldstats(FAR struct netprocfs_file_s *priv, FAR char *buffer, size_t buflen)
{
