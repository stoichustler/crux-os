...
static ssize_t netprocfs_udpstats(FAR struct netprocfs_file_s *priv, FAR char *buffer,
...
#endif
						" %6u",
						priv->offset++, conn->sconn.s_flags,
#if CONFIG_NET_SEND_BUFSIZE > 0
...
ssize_t netprocfs_read_udpstats(FAR struct netprocfs_file_s *priv, FAR char *buffer, size_t buflen)
{
	int skip = 1;
	int len = 0;
	...
	if (udp_nextconn(NULL) != NULL) {
		...
		len += netprocfs_udpstats(priv, buffer + len, buflen - len, PF_INET, &skip);
#endif /* CONFIG_NET_IPv4 */

#ifdef CONFIG_NET_IPv6
		len += netprocfs_udpstats(priv, buffer + len, buflen - len, PF_INET6, &skip);
