...
typedef CODE ssize_t (*read_stat_t)(FAR struct netprocfs_file_s *priv, FAR char *buffer,
									size_t buflen);
...
static int netprocfs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int netprocfs_close(FAR struct file *filep);
static ssize_t netprocfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen);

static int netprocfs_dup(FAR const struct file *oldp, FAR struct file *newp);

static int netprocfs_opendir(FAR const char *relpath, FAR struct fs_dirent_s **dir);
static int netprocfs_closedir(FAR struct fs_dirent_s *dir);
static int netprocfs_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry);
...
static const struct netprocfs_entry_s g_net_entries[] = {
	...
	{DTYPE_FILE, "stat", {netprocfs_read_netstats}},
#ifdef CONFIG_NET_MLD
	{DTYPE_FILE, "mld", {netprocfs_read_mldstats}},
#endif
#ifdef NET_TCP_HAVE_STACK
	{DTYPE_FILE, "tcp", {netprocfs_read_tcpstats}},
#endif
#ifdef NET_UDP_HAVE_STACK
	{DTYPE_FILE, "udp", {netprocfs_read_udpstats}},
#endif
#endif
#ifdef CONFIG_NET_ROUTE
	{DTYPE_DIRECTORY, "route", {(FAR void *)&g_netroute_operations}},
#endif
	{DTYPE_FILE, "", {netprocfs_read_devstats}}};
...
static int netprocfs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	for (i = 0; i < nitems(g_net_entries); i++) {
		if (strncmp(relpath + 4, g_net_entries[i].name, strlen(g_net_entries[i].name))) {
			continue;
		}
...
static ssize_t netprocfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	nreturned = g_net_entries[priv->entry].u.stat(priv, buffer, buflen);
...
static int netprocfs_opendir(FAR const char *relpath, FAR struct fs_dirent_s **dir)
{
	...
	level1 = (FAR struct netprocfs_level1_s *)kmm_zalloc(sizeof(struct netprocfs_level1_s));
...
static int netprocfs_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry)
{
	...
	if (level1->base.level == 1) {
		...
		if (index < nitems(g_net_entries) - 1) {
			...
			strlcpy(entry->d_name, g_net_entries[index].name, sizeof(entry->d_name));
		}
...
ssize_t netprocfs_read_linegen(FAR struct netprocfs_file_s *priv, FAR char *buffer, size_t buflen,
