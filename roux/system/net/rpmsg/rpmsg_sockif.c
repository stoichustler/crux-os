...
static int rpmsg_socket_bind(FAR struct socket *psock, FAR const struct sockaddr *addr,
							 socklen_t addrlen);
static int rpmsg_socket_getsockname(FAR struct socket *psock, FAR struct sockaddr *addr,
									FAR socklen_t *addrlen);
static int rpmsg_socket_getpeername(FAR struct socket *psock, FAR struct sockaddr *addr,
									FAR socklen_t *addrlen);
static int rpmsg_socket_listen(FAR struct socket *psock, int backlog);
static int rpmsg_socket_connect(FAR struct socket *psock, FAR const struct sockaddr *addr,
								socklen_t addrlen);
static int rpmsg_socket_accept(FAR struct socket *psock, FAR struct sockaddr *addr,
							   FAR socklen_t *addrlen, FAR struct socket *newsock, int flags);
static int rpmsg_socket_poll(FAR struct socket *psock, FAR struct pollfd *fds, bool setup);
static ssize_t rpmsg_socket_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags);
static ssize_t rpmsg_socket_recvmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags);
static int rpmsg_socket_close(FAR struct socket *psock);
static int rpmsg_socket_ioctl(FAR struct socket *psock, int cmd, unsigned long arg);
static int rpmsg_socket_shutdown(FAR struct socket *psock, int how);
#ifdef CONFIG_NET_SOCKOPTS
static int rpmsg_socket_getsockopt(FAR struct socket *psock, int level, int option, FAR void *value,
								   FAR socklen_t *value_len);
static int rpmsg_socket_setsockopt(FAR struct socket *psock, int level, int option,
...
static inline void rpmsg_socket_poll_notify(FAR struct rpmsg_socket_conn_s *conn,
...
static inline uint32_t rpmsg_socket_get_space(FAR struct rpmsg_socket_conn_s *conn)
{
	return conn->sendsize - (conn->sendpos - conn->ackpos);
}

static int rpmsg_socket_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
		...
		if (len > sizeof(*msg)) {
			...
			if (conn->recvdata) {
				...
				if (len == msg->len) {
					...
					conn->recvpos += len;
					memcpy(conn->recvdata, buf + sizeof(uint32_t), conn->recvlen);
					len = 0;
				}
...
static inline void rpmsg_socket_destroy_ept(FAR struct rpmsg_socket_conn_s *conn)
{
	if (!conn) {
		return;
	}
...
static void rpmsg_socket_format_name(FAR struct rpmsg_socket_conn_s *conn, FAR char *namebuf)
{
	if (strlen(conn->rpaddr.rp_name) > RPMSG_SOCKET_NAME_LEN) {
		uint32_t crc = crc32((const uint8_t *)conn->rpaddr.rp_name, strlen(conn->rpaddr.rp_name));
		snprintf(namebuf, RPMSG_NAME_SIZE, "%s%.2s%08" PRIx32 "%s", RPMSG_SOCKET_NAME_PREFIX,
				 conn->rpaddr.rp_name, crc, conn->nameid);
	} else {
		snprintf(namebuf, RPMSG_NAME_SIZE, "%s%.*s%s", RPMSG_SOCKET_NAME_PREFIX,
				 RPMSG_SOCKET_NAME_LEN, conn->rpaddr.rp_name, conn->nameid);
	}
}

static void rpmsg_socket_device_created(FAR struct rpmsg_device *rdev, FAR void *priv)
{
	...
	if (strcmp(conn->rpaddr.rp_cpu, rpmsg_get_cpuname(rdev)) == 0) {
		...
		rpmsg_create_ept(&conn->ept, rdev, namebuf, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
						 rpmsg_socket_ept_cb, rpmsg_socket_ns_unbind);
	}
}

static bool rpmsg_socket_ns_match(FAR struct rpmsg_device *rdev, FAR void *priv,
								  FAR const char *name, uint32_t dest)
{
	...
	if (server->rpaddr.rp_cpu[0] && strcmp(server->rpaddr.rp_cpu, rpmsg_get_cpuname(rdev))) {
...
static void rpmsg_socket_ns_bind(FAR struct rpmsg_device *rdev, FAR void *priv,
								 FAR const char *name, uint32_t dest)
{
	FAR struct rpmsg_socket_conn_s *server = priv;
	...
	ret = rpmsg_create_ept(&new->ept, rdev, name, RPMSG_ADDR_ANY, dest, rpmsg_socket_ept_cb,
						   rpmsg_socket_ns_unbind);
	if (ret < 0) {
		rpmsg_socket_free(new);
		return;
	}
...
static int rpmsg_socket_getaddr(FAR struct rpmsg_socket_conn_s *conn, FAR struct sockaddr *addr,
								FAR socklen_t *addrlen)
{
...
static int rpmsg_socket_setaddr(FAR struct rpmsg_socket_conn_s *conn,
								FAR const struct sockaddr *addr, socklen_t addrlen, bool suffix)
{
	FAR struct sockaddr_rpmsg *rpaddr = (FAR struct sockaddr_rpmsg *)addr;

	if (rpaddr->rp_family != AF_RPMSG || addrlen < sizeof(struct sockaddr_rpmsg)) {
		return -EINVAL;
	}
	...
	if (suffix) {
		snprintf(conn->nameid, sizeof(conn->nameid), ":%" PRIx32, g_rpmsg_id++);
	} else {
		conn->nameid[0] = 0;
	}
									...
									FAR socklen_t *addrlen)
{
	...
	if (ret >= 0) {
		strlcpy(((struct sockaddr_rpmsg *)addr)->rp_cpu, CONFIG_RPMSG_LOCAL_CPUNAME,
				RPMSG_SOCKET_CPU_SIZE);
	}
...
static int rpmsg_socket_listen(FAR struct socket *psock, int backlog)
{
	...
	return rpmsg_register_callback(server, NULL, NULL, rpmsg_socket_ns_match, rpmsg_socket_ns_bind);
}
...
static int rpmsg_socket_connect_internal(FAR struct socket *psock)
{
	...
	ret = rpmsg_register_callback(conn, rpmsg_socket_device_created, NULL, NULL, NULL);
	if (ret < 0) {
		return ret;
	}
	...
	if (conn->sendsize == 0) {
		...
		ret = net_sem_timedwait(&conn->sendsem, _SO_TIMEOUT(conn->sconn.s_sndtimeo));

		if (ret < 0) {
			rpmsg_unregister_callback(conn, rpmsg_socket_device_created, NULL, NULL, NULL);
								...
								socklen_t addrlen)
{
	...
	ret = rpmsg_socket_setaddr(conn, addr, addrlen, psock->s_type == SOCK_STREAM);
	if (ret < 0) {
		return ret;
	}
...
static int rpmsg_socket_poll(FAR struct socket *psock, FAR struct pollfd *fds, bool setup)
{
	...
	if (setup) {
		...
		{
			if ((!conn->ept.rdev || conn->unbind) && !_SS_INITD(conn->sconn.s_flags)) {
				eventset |= POLLHUP;
			}
		}
...
static uint32_t rpmsg_socket_get_iovlen(FAR const struct iovec *buf, size_t iovcnt)
{
	uint32_t len = 0;
	while (iovcnt--) {
		len += (buf++)->iov_len;
...
static ssize_t rpmsg_socket_send_continuous(FAR struct socket *psock, FAR const struct iovec *buf,
											size_t iovcnt, bool nonblock)
{
	...
	while (written < len) {
		...
		while (block_written < block) {
			...
			memcpy(msg->data + block_written, (FAR const uint8_t *)buf->iov_base + offset, chunk);
			offset += chunk;
		...
		ret = rpmsg_sendto_nocopy(&conn->ept, msg, block + sizeof(*msg), conn->ept.dest_addr);
		nxmutex_unlock(&conn->sendlock);
		if (ret < 0) {
...
static ssize_t rpmsg_socket_send_single(FAR struct socket *psock, FAR const struct iovec *buf,
										size_t iovcnt, bool nonblock)
{
	FAR struct rpmsg_socket_conn_s *conn = psock->s_conn;
	FAR struct rpmsg_socket_data_s *msg;
	...
	ret = rpmsg_sendto_nocopy(&conn->ept, msg, total + sizeof(*msg), conn->ept.dest_addr);
	nxmutex_unlock(&conn->sendlock);
	if (ret < 0) {
		rpmsg_release_tx_buffer(&conn->ept, msg);
	}
...
static ssize_t rpmsg_socket_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
	...
	nonblock = _SS_ISNONBLOCK(conn->sconn.s_flags) || (flags & MSG_DONTWAIT) != 0;
...
static ssize_t rpmsg_socket_recvmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
	...
	ret = net_sem_timedwait(&conn->recvsem, _SO_TIMEOUT(conn->sconn.s_rcvtimeo));
	if (!conn->ept.rdev || conn->unbind) {
		ret = 0;
	}
...
static int rpmsg_socket_close(FAR struct socket *psock)
{
	...
	if (conn->backlog > 0) {
		rpmsg_unregister_callback(conn, NULL, NULL, rpmsg_socket_ns_match, rpmsg_socket_ns_bind);
	} else {
		rpmsg_unregister_callback(conn, rpmsg_socket_device_created, NULL, NULL, NULL);
	}
...
static void rpmsg_socket_path(FAR struct rpmsg_socket_conn_s *conn, FAR char *buf, size_t len)
{
	if (conn->backlog) /* Server */
	{
		snprintf(buf, len, "rpmsg:[%s:[%s%s]<->%s]", CONFIG_RPMSG_LOCAL_CPUNAME,
				 conn->rpaddr.rp_name, conn->nameid, conn->rpaddr.rp_cpu);
	} else /* Client */
	{
		snprintf(buf, len, "rpmsg:[%s<->%s:[%s%s]]", CONFIG_RPMSG_LOCAL_CPUNAME,
				 conn->rpaddr.rp_cpu, conn->rpaddr.rp_name, conn->nameid);
	}
}
