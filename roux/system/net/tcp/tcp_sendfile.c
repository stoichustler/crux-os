...
#if defined(CONFIG_NET_SENDFILE) && defined(CONFIG_NET_TCP) && defined(NET_TCP_HAVE_STACK)
...
static uint16_t sendfile_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
									  uint16_t flags)
{
	...
	if ((flags & TCP_REXMIT) != 0) {
		...
		ret = devif_file_send(dev, pstate->snd_file, sndlen,
							  pstate->snd_foffset + pstate->snd_acked, tcpip_hdrsize(conn));
		if (ret < 0) {
	...
	if ((flags & TCP_NEWDATA) == 0 && pstate->snd_sent < pstate->snd_flen) {
		...
		if ((pstate->snd_sent - pstate->snd_acked + sndlen) < conn->snd_wnd) {
			...
			ret = devif_file_send(dev, pstate->snd_file, sndlen,
								  pstate->snd_foffset + pstate->snd_sent, tcpip_hdrsize(conn));
			if (ret < 0) {
				nerr("ERROR: Failed to read from input file: %d\n", (int)ret);
				pstate->snd_sent = ret;
				goto end_wait;
					 ...
					 size_t count)
{
	...
	for (;;) {
		...
		ret =
			net_sem_timedwait_uninterruptible(&state.snd_sem, _SO_TIMEOUT(conn->sconn.s_sndtimeo));
