...
static void psock_insert_segment(FAR struct tcp_wrbuffer_s *wrb, FAR sq_queue_t *q)
{
...
static void retransmit_segment(FAR struct tcp_conn_s *conn, FAR struct tcp_wrbuffer_s *wrb)
{
...
static inline void psock_lost_connection(FAR struct tcp_conn_s *conn, bool abort)
{
					  ...
					  FAR struct tcp_ofoseg_s *segs)
{
	...
	for (i = 0; i < ((tcp->tcpoffset >> 4) - 5) << 2;) {
		...
		if (opt == TCP_OPT_END) {
			...
			nsack = (*(tcp->optdata + 1 + i) - TCP_OPT_SACK_PERM_LEN) / (sizeof(uint32_t) * 2);
			sacks = (FAR struct tcp_sack_s *)(tcp->optdata + i + TCP_OPT_SACK_PERM_LEN);
...
static uint16_t psock_send_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
										uint16_t flags)
{
	...
	DEBUGASSERT((flags & TCP_ACKDATA) == 0 || (flags & TCP_REXMIT) == 0);
	DEBUGASSERT((flags & TCP_DISCONN_EVENTS) == 0 || (flags & TCP_REXMIT) == 0);
	...
	if ((flags & TCP_ACKDATA) != 0) {
		...
		for (entry = sq_peek(&conn->unacked_q); entry; entry = next) {
			...
			if (TCP_SEQ_GT(ackno, TCP_WBSEQNO(wrb))) {
				...
				ninfo("ACK: wrb=%p seqno=%" PRIu32 " lastseq=%" PRIu32 " pktlen=%u ackno=%" PRIu32
					  "\n",
					  wrb, TCP_WBSEQNO(wrb), lastseq, TCP_WBPKTLEN(wrb), ackno);
...
#ifdef CONFIG_NET_TCP_SELECTIVE_ACK
					if ((conn->flags & TCP_SACK) && (tcp->tcpoffset & 0xf0) > 0x50) {
		...
		if (wrb && TCP_WBSENT(wrb) > 0 && TCP_SEQ_GT(ackno, TCP_WBSEQNO(wrb))) {
			...
			ninfo("ACK: wrb=%p seqno=%" PRIu32 " nacked=%" PRIu32 " sent=%u ackno=%" PRIu32 "\n",
			...
			ninfo("ACK: wrb=%p seqno=%" PRIu32 " pktlen=%u sent=%u\n", wrb, TCP_WBSEQNO(wrb),
				  TCP_WBPKTLEN(wrb), TCP_WBSENT(wrb));
		}
	}
	...
	if (rexmitno != 0) {
		...
		for (entry = sq_peek(&conn->unacked_q); entry; entry = next) {
			...
			ret = devif_iob_send(dev, TCP_WBIOB(wrb), sndlen, 0, tcpip_hdrsize(conn));
			if (ret <= 0) {
				return flags;
			}
		...
		for (entry = sq_peek(&conn->unacked_q); entry; entry = next) {
			...
			for (i = 0, right = 0; i < nsacks; i++) {
				...
				if (TCP_SEQ_LT(TCP_WBSEQNO(wrb), ofosegs[i].left) &&
					...
					ninfo("TCP REXMIT "
						  "[%" PRIu32 " : %" PRIu32 " : %d]\n",
						  TCP_WBSEQNO(wrb), TCP_SEQ_ADD(TCP_WBSEQNO(wrb), TCP_WBPKTLEN(wrb)),
						  TCP_WBPKTLEN(wrb));
					sq_rem(entry, &conn->unacked_q);
	...
	if ((conn->tcpstateflags & TCP_ESTABLISHED) && ((flags & TCP_NEWDATA) == 0) &&
		(flags & (TCP_POLL | TCP_REXMIT | TCP_ACKDATA)) && !(sq_empty(&conn->write_q)) &&
		conn->snd_wnd > 0) {
		FAR struct tcp_wrbuffer_s *wrb;
		...
		if (TCP_SEQ_LT(seq, snd_wnd_edge)) {
			...
			ninfo("SEND: wrb=%p seq=%" PRIu32 " pktlen=%u sent=%u sndlen=%zu "
				  "mss=%u snd_wnd=%" PRIu32 " seq=%" PRIu32 " remaining_snd_wnd=%" PRIu32 "\n",
				  wrb, TCP_WBSEQNO(wrb), TCP_WBPKTLEN(wrb), TCP_WBSENT(wrb), sndlen, conn->mss,
			...
			ret = devif_iob_send(dev, TCP_WBIOB(wrb), sndlen, TCP_WBSENT(wrb), tcpip_hdrsize(conn));
			if (ret <= 0) {
				return flags;
			}
			...
			ninfo("SEND: wrb=%p nrtx=%u tx_unacked=%" PRIu32 " sent=%" PRIu32 "\n", wrb,
				  TCP_WBNRTX(wrb), conn->tx_unacked, conn->sent);
...
ssize_t psock_tcp_send(FAR struct socket *psock, FAR const void *buf, size_t len, int flags)
{
	...
	if (psock == NULL || psock->s_type != SOCK_STREAM || psock->s_conn == NULL) {
	...
	while (len > 0) {
		...
		conn->sndcb->flags = (TCP_ACKDATA | TCP_REXMIT | TCP_POLL | TCP_DISCONN_EVENTS);
		...
		while (true) {
			...
			if (nonblock || (timeout != UINT_MAX && tcp_send_gettimeout(start, timeout) == 0)) {
