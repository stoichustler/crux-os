...
static uint32_t tcp_calc_rcvsize(FAR struct tcp_conn_s *conn, uint32_t recvwndo)
{
...
static uint32_t tcp_maxrcvwin(FAR struct tcp_conn_s *conn)
{
	...
	recvwndo =
		tcp_calc_rcvsize(conn, (CONFIG_IOB_NBUFFERS - CONFIG_IOB_THROTTLE) * CONFIG_IOB_BUFSIZE);
...
uint32_t tcp_get_recvwindow(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn)
{
	...
	if (conn->nofosegs > 0) {
		uint32_t desire = conn->ofosegs[0].left - tcp_getsequence(conn->rcvseq);
