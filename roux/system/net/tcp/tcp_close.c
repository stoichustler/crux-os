...
static uint16_t tcp_close_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
									   uint16_t flags)
{
...
static inline int tcp_close_disconnect(FAR struct socket *psock)
{
	...
	if ((conn->tcpstateflags == TCP_ESTABLISHED || conn->tcpstateflags == TCP_LAST_ACK) &&
		...
		conn->clscb->flags = TCP_NEWDATA | TCP_ACKDATA | TCP_POLL | TCP_DISCONN_EVENTS;
		...
		if (_SO_GETOPT(conn->sconn.s_options, SO_LINGER)) {
			conn->ltimeout = clock_systime_ticks() + DSEC2TICK(conn->sconn.s_linger);
