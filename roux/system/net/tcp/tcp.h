...
#define TCP_WBCOPYIN(wrb, src, n, off) (iob_copyin((wrb)->wb_iob, src, (n), (off), true))
#define TCP_WBTRYCOPYIN(wrb, src, n, off) (iob_trycopyin((wrb)->wb_iob, src, (n), (off), true))

#define TCP_WBTRIM(wrb, n)                                                                         \
	do {                                                                                           \
		(wrb)->wb_iob = iob_trimhead((wrb)->wb_iob, (n));                                          \
	} while (0)

#ifdef CONFIG_DEBUG_FEATURES
#define TCP_WBDUMP(msg, wrb, len, offset) tcp_wrbuffer_dump(msg, wrb, len, offset)
#else
...
struct tcp_conn_s {
...
#endif
#if defined(CONFIG_NET_TCP_WRITE_BUFFERS) || defined(CONFIG_NET_TCP_WINDOW_SCALE)
	...
	CODE int (*accept)(FAR struct tcp_conn_s *listener, FAR struct tcp_conn_s *conn);
...
FAR struct tcp_conn_s *tcp_active(FAR struct net_driver_s *dev, FAR struct tcp_hdr_s *tcp);
...
FAR struct tcp_conn_s *tcp_alloc_accept(FAR struct net_driver_s *dev, FAR struct tcp_hdr_s *tcp,
...
int tcp_selectport(uint8_t domain, FAR const union ip_addr_u *ipaddr, uint16_t portno);
...
int psock_tcp_connect(FAR struct socket *psock, FAR const struct sockaddr *addr);
...
void tcp_lost_connection(FAR struct tcp_conn_s *conn, FAR struct devif_callback_s *cb,
						 uint16_t flags);
...
#if defined(CONFIG_NET_IPv4) && defined(CONFIG_NET_IPv6)
FAR struct tcp_conn_s *tcp_findlistener(FAR union ip_binding_u *uaddr, uint16_t portno,
										uint8_t domain);
#else
FAR struct tcp_conn_s *tcp_findlistener(FAR union ip_binding_u *uaddr, uint16_t portno);
#endif
...
#if defined(CONFIG_NET_IPv4) && defined(CONFIG_NET_IPv6)
bool tcp_islistener(FAR union ip_binding_u *uaddr, uint16_t portno, uint8_t domain);
#else
...
int tcp_accept_connection(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn,
						  uint16_t portno);
...
void tcp_send(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn, uint16_t flags,
			  uint16_t len);
...
ssize_t tcp_sendfile(FAR struct socket *psock, FAR struct file *infile, FAR off_t *offset,
					 size_t count);
#endif
...
void tcp_synack(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn, uint8_t ack);
...
void tcp_appsend(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn, uint16_t result);
...
void tcp_rexmit(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn, uint16_t result);
...
void tcp_send_txnotify(FAR struct socket *psock, FAR struct tcp_conn_s *conn);
...
uint16_t tcp_callback(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn, uint16_t flags);
...
uint16_t tcp_datahandler(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn,
						 uint16_t offset);
...
int psock_tcp_accept(FAR struct socket *psock, FAR struct sockaddr *addr, FAR socklen_t *addrlen,
					 FAR void **newconn);
...
ssize_t psock_tcp_recvfrom(FAR struct socket *psock, FAR struct msghdr *msg, int flags);
...
ssize_t psock_tcp_send(FAR struct socket *psock, FAR const void *buf, size_t len, int flags);
...
#ifdef CONFIG_NET_TCPPROTO_OPTIONS
int tcp_setsockopt(FAR struct socket *psock, int option, FAR const void *value,
				   socklen_t value_len);
#endif
...
#ifdef CONFIG_NET_TCPPROTO_OPTIONS
int tcp_getsockopt(FAR struct socket *psock, int option, FAR void *value, FAR socklen_t *value_len);
#endif
...
uint32_t tcp_get_recvwindow(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn);
...
#ifdef CONFIG_DEBUG_FEATURES
void tcp_event_handler_dump(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags,
...
int tcp_readahead_notifier_setup(worker_t worker, FAR struct tcp_conn_s *conn, FAR void *arg);
#endif
...
#ifdef CONFIG_NET_TCP_NOTIFIER
int tcp_writebuffer_notifier_setup(worker_t worker, FAR struct tcp_conn_s *conn, FAR void *arg);
#endif
...
#ifdef CONFIG_NET_TCP_NOTIFIER
int tcp_disconnect_notifier_setup(worker_t worker, FAR struct tcp_conn_s *conn, FAR void *arg);
#endif
