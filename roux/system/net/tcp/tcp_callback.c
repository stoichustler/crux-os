...
static inline uint16_t tcp_data_event(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn,
									  uint16_t flags)
{
	uint16_t recvlen;
...
static uint16_t tcp_ofoseg_data_event(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn,
									  uint16_t flags)
{
	...
	while (i < conn->nofosegs) {
		...
		if (rcvseq == seg->left) {
			ninfo("TCP OFOSEG input [%" PRIu32 " : %" PRIu32 " : %u]\n", seg->left, seg->right,
				  seg->data->io_pktlen);
			rcvseq = TCP_SEQ_ADD(rcvseq, seg->data->io_pktlen);
			 ...
			 */
				...
				seg->data = iob_trimhead(seg->data, TCP_SEQ_SUB(rcvseq, seg->left));
				seg->left = rcvseq;
				if (seg->data != NULL) {
...
uint16_t tcp_datahandler(FAR struct net_driver_s *dev, FAR struct tcp_conn_s *conn, uint16_t offset)
{
