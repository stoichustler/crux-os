...
static uint16_t tcp_poll_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
									  uint16_t flags)
{
...
int tcp_pollsetup(FAR struct socket *psock, FAR struct pollfd *fds)
{
	...
	nonblock_conn =
		((conn->tcpstateflags == TCP_ALLOCATED || conn->tcpstateflags == TCP_SYN_SENT) &&
	...
	if (!nonblock_conn && !_SS_ISCONNECTED(conn->sconn.s_flags) &&
		...
		eventset |= POLLERR | POLLHUP;
	} else if (_SS_ISCONNECTED(conn->sconn.s_flags) && psock_tcp_cansend(conn) >= 0) {
		eventset |= POLLWRNORM;
	}
