...
static uint16_t tcpsend_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
	...
	if ((flags & TCP_ACKDATA) != 0) {
		...
		if (pstate->snd_acked < pstate->snd_sent && (flags & TCP_NEWDATA) == 0 &&
			(tcp->flags & (TCP_SYN | TCP_FIN)) == 0 && ackno == pstate->snd_prev_ack &&
	...
	if ((flags & TCP_REXMIT) != 0) {
		...
		ret = devif_send(dev, &pstate->snd_buffer[pstate->snd_acked], sndlen, tcpip_hdrsize(conn));
		if (ret <= 0) {
			pstate->snd_sent = ret;
			goto end_wait;
		}
	...
	if ((flags & TCP_NEWDATA) == 0 && pstate->snd_sent < pstate->snd_buflen) {
		...
		if ((pstate->snd_sent - pstate->snd_acked + sndlen) < conn->snd_wnd) {
			...
			ret =
				devif_send(dev, &pstate->snd_buffer[pstate->snd_sent], sndlen, tcpip_hdrsize(conn));
			if (ret <= 0) {
				pstate->snd_sent = ret;
				goto end_wait;
			}
...
ssize_t psock_tcp_send(FAR struct socket *psock, FAR const void *buf, size_t len, int flags)
{
	...
	if (len > 0) {
		...
		if (state.snd_cb) {
			...
			state.snd_cb->flags = (TCP_ACKDATA | TCP_REXMIT | TCP_POLL | TCP_DISCONN_EVENTS);
			...
			for (;;) {
				...
				ret = net_sem_timedwait(&state.snd_sem, _SO_TIMEOUT(conn->sconn.s_sndtimeo));
