...
static inline void tcp_update_recvlen(FAR struct tcp_recvfrom_s *pstate, size_t recvlen)
{
...
static size_t tcp_recvfrom_newdata(FAR struct net_driver_s *dev, FAR struct tcp_recvfrom_s *pstate)
{
	unsigned int offset;
	size_t recvlen;
	...
	dev->d_iob = iob_trimhead(dev->d_iob, pstate->ir_flags & MSG_PEEK ? offset : recvlen + offset);
...
static inline uint16_t tcp_newdata(FAR struct net_driver_s *dev, FAR struct tcp_recvfrom_s *pstate,
								   uint16_t flags)
{
	...
	if (recvlen < dev->d_len) {
		...
		if (nsaved < buflen) {
			nwarn("WARNING: packet data not fully saved "
				  "(%d/%u/%zu/%u bytes)\n",
				  buflen - nsaved, (unsigned int)nsaved, recvlen, (unsigned int)dev->d_len);
		}
...
static inline void tcp_readahead(struct tcp_recvfrom_s *pstate)
{
	...
	while ((iob = conn->readahead) != NULL && pstate->ir_buflen > 0) {
...
static inline void tcp_sender(FAR struct net_driver_s *dev, FAR struct tcp_recvfrom_s *pstate)
{
...
#endif
	{
		FAR struct sockaddr_in6 *infrom = (FAR struct sockaddr_in6 *)pstate->ir_from;
...
#endif
	{
		FAR struct sockaddr_in *infrom = (FAR struct sockaddr_in *)pstate->ir_from;
...
static uint16_t tcp_recvhandler(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags)
{
	...
	if (pstate) {
		...
		if ((flags & TCP_NEWDATA) != 0) {
			...
			if ((flags & TCP_ACKDATA) != 0) {
				...
				int ret =
					iob_clone_partial(dev->d_iob, dev->d_iob->io_pktlen, 0, iob, 0, false, false);
				if (ret < 0) {
					iob_free_chain(iob);
			...
			if ((pstate->ir_recvlen > 0 && (pstate->ir_cb->flags & TCP_WAITALL) == 0) ||
				pstate->ir_buflen == 0) {
				ninfo("TCP resume\n");
...
static void tcp_recvfrom_initialize(FAR struct tcp_conn_s *conn, FAR void *buf, size_t len,
									FAR struct sockaddr *infrom, FAR socklen_t *fromlen,
									FAR struct tcp_recvfrom_s *pstate, int flags)
{
	/* Initialize the state structure. */
...
static ssize_t tcp_recvfrom_one(FAR struct tcp_conn_s *conn, FAR void *buf, size_t len,
								FAR struct sockaddr *from, FAR socklen_t *fromlen, int flags)
{
	...
	else if (_SS_ISNONBLOCK(conn->sconn.s_flags) || (flags & MSG_DONTWAIT) != 0) {
	 ...
	 */
		...
		if (((flags & MSG_WAITALL) != 0 || state.ir_recvlen == 0) && state.ir_buflen > 0) {
