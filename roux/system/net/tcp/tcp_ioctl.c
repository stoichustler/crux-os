...
#endif
#endif
			 ", st %02" PRIx8 ", flg %" PRIx8 "]",
			 inet_ntop(domain, laddr, local, sizeof(local)), ntohs(conn->lport),
			 inet_ntop(domain, raddr, remote, sizeof(remote)), ntohs(conn->rport),
#if CONFIG_NET_SEND_BUFSIZE > 0
			 tcp_wrbuffer_inqueue_size(conn), conn->snd_bufs,
#endif
#if CONFIG_NET_RECV_BUFSIZE > 0
			 (conn->readahead) ? conn->readahead->io_pktlen : 0, conn->rcv_bufs,
#ifdef CONFIG_NET_TCP_OUT_OF_ORDER
			 tcp_ofoseg_bufsize(conn),
#endif
#endif
			 conn->tcpstateflags, conn->sconn.s_flags);
}
...
#else
		*(FAR int *)((uintptr_t)arg) = conn->snd_bufs - tcp_wrbuffer_inqueue_size(conn);
#endif
#else
