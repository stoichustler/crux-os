...
static uint16_t tcp_shutdown_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
										  uint16_t flags)
{
...
static inline int tcp_send_fin(FAR struct socket *psock)
{
	...
	if ((conn->tcpstateflags == TCP_ESTABLISHED || conn->tcpstateflags == TCP_SYN_SENT ||
