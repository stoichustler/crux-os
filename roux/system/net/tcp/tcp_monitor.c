...
static uint16_t tcp_monitor_event(FAR struct net_driver_s *dev, FAR void *pvpriv, uint16_t flags);
...
static void tcp_shutdown_monitor(FAR struct tcp_conn_s *conn, uint16_t flags)
{
	...
	while (conn->connevents != NULL) {
		devif_conn_callback_free(conn->dev, conn->connevents, &conn->connevents,
...
int tcp_start_monitor(FAR struct socket *psock)
{
	...
	nonblock_conn = (conn->tcpstateflags == TCP_SYN_SENT && _SS_ISNONBLOCK(conn->sconn.s_flags));
	...
	if (!(conn->tcpstateflags == TCP_ESTABLISHED || conn->tcpstateflags == TCP_SYN_RCVD ||
		...
		if (conn->tcpstateflags == TCP_CLOSED || conn->tcpstateflags == TCP_LAST_ACK) {
			net_unlock();
			return OK;
		}
	...
	cb = devif_callback_alloc(conn->dev, &conn->connevents, &conn->connevents_tail);
	if (cb != NULL) {
