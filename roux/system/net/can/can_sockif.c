...
static uint16_t can_poll_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
									  uint16_t flags)
{
...
static int can_setup(FAR struct socket *psock)
{
	...
	if (domain == PF_CAN && (type == SOCK_RAW || type == SOCK_DGRAM || type == SOCK_CTRL)) {
		...
		conn->recv_buffnum =
			(CONFIG_NET_RECV_BUFSIZE + CONFIG_IOB_BUFSIZE - 1) / CONFIG_IOB_BUFSIZE;
#endif
...
static int can_bind(FAR struct socket *psock, FAR const struct sockaddr *addr, socklen_t addrlen)
{
	...
	FAR struct can_conn_s *conn;
	DEBUGASSERT(addr != NULL && addrlen >= sizeof(struct sockaddr_can));
