...
static inline void can_add_recvlen(FAR struct can_recvfrom_s *pstate, size_t recvlen)
{
...
static size_t can_recvfrom_newdata(FAR struct net_driver_s *dev, FAR struct can_recvfrom_s *pstate)
{
	unsigned int offset;
	size_t recvlen;
#ifdef CONFIG_NET_TIMESTAMP
...
static inline void can_newdata(FAR struct net_driver_s *dev, FAR struct can_recvfrom_s *pstate)
{
...
static inline int can_readahead(struct can_recvfrom_s *pstate)
{
	...
	if ((iob = iob_peek_queue(&conn->readahead)) != NULL && pstate->pr_buflen > 0) {
		...
		if (_SO_GETOPT(conn->sconn.s_options, SO_TIMESTAMP) &&
			pstate->pr_msglen == sizeof(struct timeval)) {
			iob_copyout(pstate->pr_msgbuf, iob, sizeof(struct timeval), -CONFIG_NET_LL_GUARDSIZE);
		}
#endif
	...
	for (i = 0; i < conn->filter_count; i++) {
		if (conn->filters[i].can_id & CAN_INV_FILTER) {
			if ((id & conn->filters[i].can_mask) !=
				((conn->filters[i].can_id & ~CAN_INV_FILTER) & conn->filters[i].can_mask)) {
				return 1;
			}
		} else {
			if ((id & conn->filters[i].can_mask) ==
...
static uint16_t can_recvfrom_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
										  uint16_t flags)
{
...
#ifdef CONFIG_NET_TIMESTAMP
				if ((_SO_GETOPT(conn->sconn.s_options, SO_TIMESTAMP) &&
					 dev->d_len > sizeof(struct can_frame) + sizeof(struct timeval)) ||
...
static ssize_t can_recvfrom_result(int result, FAR struct can_recvfrom_s *pstate)
{
...
ssize_t can_recvmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
...
#ifdef CONFIG_NET_TIMESTAMP
	if (_SO_GETOPT(conn->sconn.s_options, SO_TIMESTAMP)) {
		state.pr_msgbuf = cmsg_append(msg, SOL_SOCKET, SO_TIMESTAMP, NULL, sizeof(struct timeval));
