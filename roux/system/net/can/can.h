...
FAR struct can_conn_s *can_active(FAR struct net_driver_s *dev, FAR struct can_conn_s *conn);
...
uint16_t can_callback(FAR struct net_driver_s *dev, FAR struct can_conn_s *conn, uint16_t flags);
...
uint16_t can_datahandler(FAR struct net_driver_s *dev, FAR struct can_conn_s *conn);
...
ssize_t can_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags);
 ...
 *
 *   See <roux/can.h> for the a complete list of values of CAN protocol
 *   options.
 *
 * Input Parameters:
...
#ifdef CONFIG_NET_CANPROTO_OPTIONS
int can_setsockopt(FAR struct socket *psock, int level, int option, FAR const void *value,
				   socklen_t value_len);
#endif
 ...
 *   header files (such as roux/can.h for the case of the CAN protocol).
 *
 * Input Parameters:
...
#ifdef CONFIG_NET_CANPROTO_OPTIONS
int can_getsockopt(FAR struct socket *psock, int level, int option, FAR void *value,
