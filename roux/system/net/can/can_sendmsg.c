										...
										uint16_t flags)
{
	...
	if (pstate) {
		...
		/* It looks like we are good to send the data */
			...
			int ret =
				devif_send(dev, pstate->snd_buffer, pstate->snd_buflen + pstate->pr_msglen, 0);
			...
			{
				memcpy(dev->d_buf + pstate->snd_buflen, pstate->pr_msgbuf, pstate->pr_msglen);
			}
		}
...
ssize_t can_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
	...
	if (_SO_GETOPT(conn->sconn.s_options, CAN_RAW_FD_FRAMES)) {
		if (msg->msg_iov->iov_len != CANFD_MTU && msg->msg_iov->iov_len != CAN_MTU) {
			return -EINVAL;
		}
	} else
#endif
	...
	if (msg->msg_controllen > sizeof(struct cmsghdr)) {
		...
		if (_SO_GETOPT(conn->sconn.s_options, CAN_RAW_TX_DEADLINE) &&
			cmsg->cmsg_level == SOL_CAN_RAW && cmsg->cmsg_type == CAN_RAW_TX_DEADLINE &&
