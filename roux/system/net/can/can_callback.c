...
static inline uint16_t can_data_event(FAR struct net_driver_s *dev, FAR struct can_conn_s *conn,
									  uint16_t flags)
{
...
uint16_t can_callback(FAR struct net_driver_s *dev, FAR struct can_conn_s *conn, uint16_t flags)
{
	...
	if (conn) {
		...
		if (_SO_GETOPT(conn->sconn.s_options, SO_TIMESTAMP) && (dev->d_iob != NULL)) {
			struct timeval tv;
			...
			len = iob_trycopyin(dev->d_iob, (FAR uint8_t *)&tv, sizeof(struct timeval),
