...
#include <roux/net/netfilter/x_tables.h>
...
static void convert_invflags(FAR struct ipfilter_entry_s *entry, uint8_t invflags)
{
...
static void convert_tcpudp(FAR struct ipfilter_entry_s *entry, uint16_t spts[2], uint16_t dpts[2],
						   uint8_t invflags)
{
...
static void convert_icmp(FAR struct ipfilter_entry_s *entry, uint8_t type, uint8_t invflags)
{
...
static FAR struct ipv4_filter_entry_s *convert_ipv4entry(FAR const struct ipt_entry *entry)
{
	...
	case IPPROTO_TCP:
		if (strcmp(match->u.user.name, XT_MATCH_NAME_TCP) == 0) {
			...
			convert_tcpudp(&filter->common, tcp->spts, tcp->dpts, tcp->invflags);
		}
		break;
	...
	case IPPROTO_UDP:
		if (strcmp(match->u.user.name, XT_MATCH_NAME_TCP) == 0) {
			...
			convert_tcpudp(&filter->common, udp->spts, udp->dpts, udp->invflags);
		}
		break;
...
static FAR struct ipv6_filter_entry_s *convert_ipv6entry(FAR const struct ip6t_entry *entry)
{
	FAR const struct xt_entry_match *match;
	FAR const struct xt_entry_target *target;
