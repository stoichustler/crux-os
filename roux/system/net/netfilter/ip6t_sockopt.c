...
static FAR struct ip6t_table_s *ip6t_table(FAR const char *name)
{
	...
	for (i = 0; i < nitems(g_tables); i++) {
		...
		if (g_tables[i].repl != NULL && strcmp(g_tables[i].repl->name, name) == 0) {
			return &g_tables[i];
		}
	}
...
static int replace_entries(FAR const struct ip6t_replace *repl, socklen_t len)
{
	int ret;
...
FAR struct ip6t_replace *ip6t_alloc_table(FAR const char *table, unsigned int valid_hooks)
{
	...
	strlcpy(error_entry->target.errorname, XT_ERROR_TARGET, sizeof(error_entry->target.errorname));
...
int ip6t_getsockopt(FAR struct socket *psock, int option, FAR void *value, FAR socklen_t *value_len)
{
	switch (option) {
