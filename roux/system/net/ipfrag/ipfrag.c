...
#if (defined(CONFIG_NET_IPv4) || defined(CONFIG_NET_IPv6)) && defined(CONFIG_NET_IPFRAG)
...
#define IOBUF_CNT(ptr) (((ptr)->io_pktlen + CONFIG_IOB_BUFSIZE - 1) / CONFIG_IOB_BUFSIZE)
...
static inline FAR struct ip_fraglink_s *ip_fragin_freelink(FAR struct ip_fraglink_s *fraglink);
...
static inline FAR struct iob_s *ip_fragout_allocfragbuf(FAR struct iob_queue_s *fragq);
...
static void ip_fragin_timerwork(FAR void *arg)
{
	...
	while (entry != NULL) {
		...
		node = (FAR struct ip_fragsnode_s *)container_of(entry, FAR struct ip_fragsnode_s, flinkat);
		...
		if (interval >= REASSEMBLY_TIMEOUT_TICKS) {
			...
			if ((node->verifyflag & IP_FRAGVERIFY_RECVDZEROFRAG) != 0) {
				...
				if (node->frags->isipv4) {
					icmp_reply(dev, ICMP_TIME_EXCEEDED, ICMP_EXC_FRAGTIME);
				}
#endif

#ifdef CONFIG_NET_IPv6
				if (!node->frags->isipv4) {
					icmpv6_reply(dev, ICMPv6_PACKET_TIME_EXCEEDED, ICMPV6_EXC_FRAGTIME, 0);
				}
#endif
	...
	if (sq_peek(&g_assemblyhead_time) != NULL) {
		...
		wd_start(&g_wdfragtimeout, delay, ip_fragin_timerout_expiry, (wdparm_t)NULL);
	} else {
...
static void ip_fragin_cachemonitor(FAR struct ip_fragsnode_s *curnode)
{
	...
	if (g_bufoccupy > REASSEMBLY_MAXOCCUPYIOB) {
		...
		while (entry != NULL && cleancnt > 0) {
			...
			node = (FAR struct ip_fragsnode_s *)container_of(entry, FAR struct ip_fragsnode_s,
															 flinkat);
...
bool ip_fragin_enqueue(FAR struct net_driver_s *dev, FAR struct ip_fraglink_s *curfraglink)
{
	FAR struct ip_fragsnode_s *node;
	FAR sq_entry_t *entry;
	...
	if (node != NULL && curfraglink->ipid == node->ipid) {
		...
		if (sq_peek(&g_assemblyhead_ipid) == NULL || entrylast == NULL) {
			...
			sq_addafter(entrylast, (FAR sq_entry_t *)node, &g_assemblyhead_ipid);
		}
...
int32_t ip_fragout_slice(FAR struct iob_s *iob, uint8_t domain, uint16_t mtu, uint16_t unfraglen,
						 FAR struct iob_queue_s *fragq)
{
	...
	if (domain == PF_INET) {
		...
		if (leftlen == 0 && orig != NULL) {
			...
			while (leftlen > 0) {
				...
				GOTO_IF(iob_trycopyin(reorg, leftstart, ncopy, reorg->io_pktlen, false) < 0,
						allocfail);
		...
		orig = iob;
		memcpy(reorg->io_data + reorg->io_offset, orig->io_data + orig->io_offset,
			   unfraglen - EXTHDR_FRAG_LEN);
	...
	while (orig != NULL) {
		...
		while (leftlen > 0) {
			...
			if (navail > 0) {
				...
				if (iob_trycopyin(reorg, leftstart, ncopy, reorg->io_pktlen, false) < 0) {
					goto allocfail;
				}
...
void ip_frag_startwdog(void)
{
	if (g_wdfragtimeout.func == NULL) {
		wd_start(&g_wdfragtimeout, REASSEMBLY_TIMEOUT_TICKS, ip_fragin_timerout_expiry,
				 (wdparm_t)NULL);
	}
}
