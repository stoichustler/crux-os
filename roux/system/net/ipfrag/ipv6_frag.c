...
static int32_t ipv6_fragin_getinfo(FAR struct iob_s *iob, FAR struct ip_fraglink_s *fraglink);
static uint32_t ipv6_fragin_reassemble(FAR struct ip_fragsnode_s *node);
static inline void ipv6_fragout_buildipv6header(FAR struct ipv6_hdr_s *ref,
												FAR struct ipv6_hdr_s *ipv6, uint16_t hdrlen,
												uint16_t datalen, uint16_t nxthdroff,
												uint16_t nxtprot);
static inline void ipv6_fragout_buildipv6fragheader(FAR struct ipv6_fragment_extension_s *frag,
													uint8_t nxthdr, uint16_t ipoff, uint32_t ipid);
static uint16_t ipv6_fragout_getunfraginfo(FAR struct iob_s *iob, uint16_t *hdroff,
										   uint16_t *hdrtype);
...
static int32_t ipv6_fragin_getinfo(FAR struct iob_s *iob, FAR struct ip_fraglink_s *fraglink)
{
	FAR struct ipv6_hdr_s *ipv6 = (FAR struct ipv6_hdr_s *)(iob->io_data + iob->io_offset);
	...
	if (nxthdr == NEXT_FRAGMENT_EH) {
		...
		fraglink->ipid = NTOHL(((uint32_t)(*(FAR uint16_t *)(&fraghdr->id[0])) << 16) +
...
static uint32_t ipv6_fragin_reassemble(FAR struct ip_fragsnode_s *node)
{
	...
	while (fraglink != NULL) {
		...
		if (fraglink->fragoff == 0) {
			...
			memmove(fraghdr, payload, iob->io_len - (payload - (iob->io_data + iob->io_offset)));
...
int32_t ipv6_fragout(FAR struct net_driver_s *dev, uint16_t mtu)
{
	...
	FAR struct ipv6_fragment_extension_s *fraghdr;
	struct iob_queue_s fragq = {NULL, NULL};
	...
	for (iter = 0; iter < nfrags; iter++) {
		...
		if (iter == 0) {
			...
			ipv6_fragout_buildipv6header(ref, ref, unfraglen, frag->io_pktlen - IPv6_HDRLEN, hdroff,
										 NEXT_FRAGMENT_EH);

			/* Build the fragment header for the zero fragment */

			fraghdr = (FAR struct ipv6_fragment_extension_s *)(frag->io_data + frag->io_offset +
															   unfraglen);
			ipv6_fragout_buildipv6fragheader(fraghdr, hdrtype, FRAGHDR_FRAG_MOREFRAGS, ipid);
		} else {
			...
			ipv6_fragout_buildipv6header(
				ref, (FAR struct ipv6_hdr_s *)(frag->io_data + frag->io_offset), unfraglen,
