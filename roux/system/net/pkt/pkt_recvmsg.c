...
static inline void pkt_add_recvlen(FAR struct pkt_recvfrom_s *pstate, size_t recvlen)
{
...
static void pkt_recvfrom_newdata(FAR struct net_driver_s *dev, FAR struct pkt_recvfrom_s *pstate)
{
	unsigned int offset;
	size_t recvlen;
...
static uint16_t pkt_recvfrom_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
										  uint16_t flags)
{
...
static void pkt_recvfrom_initialize(FAR struct socket *psock, FAR void *buf, size_t len,
...
static ssize_t pkt_recvfrom_result(int result, FAR struct pkt_recvfrom_s *pstate)
{
...
static inline ssize_t pkt_readahead(FAR struct pkt_conn_s *conn, FAR void *buf, size_t buflen)
{
...
ssize_t pkt_recvmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
