...
static uint16_t pkt_datahandler(FAR struct net_driver_s *dev, FAR struct pkt_conn_s *conn)
{
	...
	ret = iob_clone_partial(dev->d_iob, dev->d_len, -NET_LL_HDRLEN(dev), iob, 0, true, false);
	if (ret < 0) {
