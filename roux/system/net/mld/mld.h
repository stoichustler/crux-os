...
#define SET_MLD_QUERIER(f)                                                                         \
	do {                                                                                           \
		(f) |= MLD_QUERIER;                                                                        \
	} while (0)
#define SET_MLD_V1COMPAT(f)                                                                        \
	do {                                                                                           \
		(f) |= MLD_V1COMPAT;                                                                       \
	} while (0)
#define SET_MLD_GENPEND(f)                                                                         \
	do {                                                                                           \
		(f) |= MLD_GENPEND;                                                                        \
	} while (0)

#define CLR_MLD_QUERIER(f)                                                                         \
	do {                                                                                           \
		(f) &= ~MLD_QUERIER;                                                                       \
	} while (0)
#define CLR_MLD_V1COMPAT(f)                                                                        \
	do {                                                                                           \
		(f) &= ~MLD_V1COMPAT;                                                                      \
	} while (0)
#define CLR_MLD_GENPEND(f)                                                                         \
	do {                                                                                           \
		(f) &= ~MLD_GENPEND;                                                                       \
	} while (0)
...
#define SET_MLD_STARTUP(f)                                                                         \
	do {                                                                                           \
		(f) |= MLD_STARTUP;                                                                        \
	} while (0)
#define SET_MLD_LASTREPORT(f)                                                                      \
	do {                                                                                           \
		(f) |= MLD_LASTREPORT;                                                                     \
	} while (0)
#define SET_MLD_SCHEDMSG(f)                                                                        \
	do {                                                                                           \
		(f) |= MLD_SCHEDMSG;                                                                       \
	} while (0)
#define SET_MLD_WAITMSG(f)                                                                         \
	do {                                                                                           \
		(f) |= MLD_WAITMSG;                                                                        \
	} while (0)
#define SET_MLD_RPTPEND(f)                                                                         \
	do {                                                                                           \
		(f) |= MLD_RPTPEND;                                                                        \
	} while (0)

#define CLR_MLD_STARTUP(f)                                                                         \
	do {                                                                                           \
		(f) &= ~MLD_STARTUP;                                                                       \
	} while (0)
#define CLR_MLD_LASTREPORT(f)                                                                      \
	do {                                                                                           \
		(f) &= ~MLD_LASTREPORT;                                                                    \
	} while (0)
#define CLR_MLD_SCHEDMSG(f)                                                                        \
	do {                                                                                           \
		(f) &= ~MLD_SCHEDMSG;                                                                      \
	} while (0)
#define CLR_MLD_WAITMSG(f)                                                                         \
	do {                                                                                           \
		(f) &= ~MLD_WAITMSG;                                                                       \
	} while (0)
#define CLR_MLD_RPTPEND(f)                                                                         \
	do {                                                                                           \
		(f) &= ~MLD_RPTPEND;                                                                       \
	} while (0)
...
int mld_query(FAR struct net_driver_s *dev, FAR const struct mld_mcast_listen_query_s *query);
...
int mld_done(FAR struct net_driver_s *dev, FAR const struct mld_mcast_listen_done_s *done);
...
FAR struct mld_group_s *mld_grpalloc(FAR struct net_driver_s *dev, FAR const net_ipv6addr_t addr);
...
FAR struct mld_group_s *mld_grpfind(FAR struct net_driver_s *dev, FAR const net_ipv6addr_t addr);
...
void mld_send(FAR struct net_driver_s *dev, FAR struct mld_group_s *group, uint8_t msgtype);
...
void mld_addmcastmac(FAR struct net_driver_s *dev, FAR const net_ipv6addr_t ipaddr);
...
void mld_removemcastmac(FAR struct net_driver_s *dev, FAR const net_ipv6addr_t ipaddr);
