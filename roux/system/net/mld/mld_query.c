...
static inline void mld_check_v1compat(FAR struct net_driver_s *dev, bool mldv1)
{
	if (mldv1) {
		...
		mld_start_v1timer(dev, MSEC2TICK(MLD_V1PRESENT_MSEC((clock_t)MLD_QUERY_MSEC)));
	}
}
...
static bool mld_cmpaddr(FAR struct net_driver_s *dev, const net_ipv6addr_t srcaddr)
{
...
static void mld_check_querier(FAR struct net_driver_s *dev, FAR struct ipv6_hdr_s *ipv6)
{
...
int mld_query(FAR struct net_driver_s *dev, FAR const struct mld_mcast_listen_query_s *query)
{
  ...
  mrc = NTOHS(query->mrc);
	...
	if (net_ipv6addr_cmp(ipv6->destipaddr, g_ipv6_allnodes) &&
		net_ipv6addr_cmp(query->grpaddr, g_ipv6_unspecaddr) && query->nsources == 0) {
		...
		for (member = (FAR struct mld_group_s *)dev->d_mld.grplist.head; member != NULL;
			 member = member->next) {
