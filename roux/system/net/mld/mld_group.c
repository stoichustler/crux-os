...
static int mld_ngroups(FAR struct net_driver_s *dev)
{
	...
	for (group = (FAR struct mld_group_s *)dev->d_mld.grplist.head; group != NULL;
		 group = group->next) {
		ngroups++;
	}
...
FAR struct mld_group_s *mld_grpfind(FAR struct net_driver_s *dev, FAR const net_ipv6addr_t addr)
{
	...
	mldinfo("Searching for group: %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", NTOHS(addr[0]),
			NTOHS(addr[1]), NTOHS(addr[2]), NTOHS(addr[3]), NTOHS(addr[4]), NTOHS(addr[5]),
			NTOHS(addr[6]), NTOHS(addr[7]));

	for (group = (FAR struct mld_group_s *)dev->d_mld.grplist.head; group; group = group->next) {
		mldinfo("Compare: %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", NTOHS(group->grpaddr[0]),
				NTOHS(group->grpaddr[1]), NTOHS(group->grpaddr[2]), NTOHS(group->grpaddr[3]),
				NTOHS(group->grpaddr[4]), NTOHS(group->grpaddr[5]), NTOHS(group->grpaddr[6]),
				NTOHS(group->grpaddr[7]));
...
void mld_new_pollcycle(FAR struct net_driver_s *dev)
{
	...
	for (member = (FAR struct mld_group_s *)dev->d_mld.grplist.head; member;
		 member = member->next) {
