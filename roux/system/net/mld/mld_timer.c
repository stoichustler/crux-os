...
void mld_start_gentimer(FAR struct net_driver_s *dev, clock_t ticks)
{
	...
	ret = wd_start(&dev->d_mld.gendog, ticks, mld_gendog_timout, dev->d_ifindex);
...
void mld_start_v1timer(FAR struct net_driver_s *dev, clock_t ticks)
{
	...
	ret = wd_start(&dev->d_mld.v1dog, ticks, mld_v1dog_timout, dev->d_ifindex);
...
void mld_start_polltimer(FAR struct mld_group_s *group, clock_t ticks)
{
	...
	ret = wd_start(&group->polldog, ticks, mld_polldog_timout, (wdparm_t)group);
