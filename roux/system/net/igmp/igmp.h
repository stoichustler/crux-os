...
#define SET_IDLEMEMBER(f)                                                                          \
	do {                                                                                           \
		(f) |= IGMP_IDLEMEMBER;                                                                    \
	} while (0)
#define SET_LASTREPORT(f)                                                                          \
	do {                                                                                           \
		(f) |= IGMP_LASTREPORT;                                                                    \
	} while (0)
#define SET_SCHEDMSG(f)                                                                            \
	do {                                                                                           \
		(f) |= IGMP_SCHEDMSG;                                                                      \
	} while (0)
#define SET_WAITMSG(f)                                                                             \
	do {                                                                                           \
		(f) |= IGMP_WAITMSG;                                                                       \
	} while (0)

#define CLR_IDLEMEMBER(f)                                                                          \
	do {                                                                                           \
		(f) &= ~IGMP_IDLEMEMBER;                                                                   \
	} while (0)
#define CLR_LASTREPORT(f)                                                                          \
	do {                                                                                           \
		(f) &= ~IGMP_LASTREPORT;                                                                   \
	} while (0)
#define CLR_SCHEDMSG(f)                                                                            \
	do {                                                                                           \
		(f) &= ~IGMP_SCHEDMSG;                                                                     \
	} while (0)
#define CLR_WAITMSG(f)                                                                             \
	do {                                                                                           \
		(f) &= ~IGMP_WAITMSG;                                                                      \
	} while (0)
...
FAR struct igmp_group_s *igmp_grpalloc(FAR struct net_driver_s *dev, FAR const in_addr_t *addr);
...
FAR struct igmp_group_s *igmp_grpfind(FAR struct net_driver_s *dev, FAR const in_addr_t *addr);
...
FAR struct igmp_group_s *igmp_grpallocfind(FAR struct net_driver_s *dev, FAR const in_addr_t *addr);
...
int igmp_joingroup(FAR struct net_driver_s *dev, FAR const struct in_addr *grpaddr);
...
int igmp_leavegroup(FAR struct net_driver_s *dev, FAR const struct in_addr *grpaddr);
...
void igmp_removemcastmac(FAR struct net_driver_s *dev, FAR const in_addr_t *ip);
