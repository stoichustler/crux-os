								 ...
								 FAR struct ieee802154_frame_meta_s *meta)
{
	...
	DEBUGASSERT(radio != NULL && pstate != NULL && pstate->is_sock != NULL && meta != NULL);
...
static uint16_t ieee802154_sendto_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
											   uint16_t flags)
{
...
static ssize_t ieee802154_sendto(FAR struct socket *psock, FAR const void *buf, size_t len,
								 int flags, FAR const struct sockaddr *to, socklen_t tolen)
{
	...
	memcpy(&state.is_destaddr, &destaddr->sa_addr, sizeof(struct ieee802154_saddr_s));
...
static ssize_t ieee802154_send(FAR struct socket *psock, FAR const void *buf, size_t len, int flags)
{
	...
	if (psock->s_type == SOCK_DGRAM) {
		...
		if (!_SS_ISCONNECTED(conn->sconn.s_flags) ||
			...
			memcpy(&to.sa_addr, &conn->raddr, sizeof(struct ieee802154_saddr_s));

			/* Then perform the send() as sendto() */

			ret = ieee802154_sendto(psock, buf, len, flags, (FAR const struct sockaddr *)&to,
...
ssize_t ieee802154_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
