...
static int ieee802154_bind(FAR struct socket *psock, FAR const struct sockaddr *addr,
						   socklen_t addrlen);
static int ieee802154_getsockname(FAR struct socket *psock, FAR struct sockaddr *addr,
								  FAR socklen_t *addrlen);
static int ieee802154_getpeername(FAR struct socket *psock, FAR struct sockaddr *addr,
								  FAR socklen_t *addrlen);
static int ieee802154_connect(FAR struct socket *psock, FAR const struct sockaddr *addr,
							  socklen_t addrlen);
							  ...
							  socklen_t addrlen)
{
	...
	if (addr->sa_family == AF_IEEE802154) {
		...
		memcpy(&conn->raddr, &ieeeaddr->sa_addr, sizeof(struct ieee802154_saddr_s));
	} else {
						   ...
						   socklen_t addrlen)
{
	...
	if (addr->sa_family != AF_IEEE802154 || addrlen < sizeof(struct sockaddr_ieee802154_s)) {
		nerr("ERROR: Invalid family: %u or address length: %zu < %zu\n", addr->sa_family,
			 (size_t)addrlen, sizeof(struct sockaddr_ieee802154_s));
		return -EBADF;
	}
	...
	if (conn == NULL || (psock->s_type != SOCK_DGRAM && psock->s_type != SOCK_CTRL)) {
