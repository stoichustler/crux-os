...
static struct ieee802154_container_s g_container_pool[CONFIG_NET_IEEE802154_NCONTAINERS];
...
void ieee802154_container_initialize(void)
{
	...
	for (i = 0, container = g_container_pool; i < CONFIG_NET_IEEE802154_NCONTAINERS;
		 i++, container++) {
...
FAR struct ieee802154_container_s *ieee802154_container_allocate(void)
{
	...
	if (g_free_container != NULL) {
		...
		net_unlock();
		container = (FAR struct ieee802154_container_s *)kmm_malloc(
			(sizeof(struct ieee802154_container_s)));
