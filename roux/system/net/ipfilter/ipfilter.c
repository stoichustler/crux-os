...
#define SPORT_MATCH(entry, sport)                                                                  \
	(((sport) >= (entry)->match.tcpudp.sports[0] && (sport) <= (entry)->match.tcpudp.sports[1]) ^  \
	 (entry)->inv_sport)

#define DPORT_MATCH(entry, dport)                                                                  \
	(((dport) >= (entry)->match.tcpudp.dports[0] && (dport) <= (entry)->match.tcpudp.dports[1]) ^  \
	 (entry)->inv_dport)

#define ICMP_MATCH(entry, icmphdr)                                                                 \
	(((entry)->match.icmp.type == 0xFF || (entry)->match.icmp.type == (icmphdr)->type) ^           \
	 (entry)->inv_icmp)

/* Getting L4 header from IPv4/IPv6 header. */

#define IPv4_L4HDR(ipv4) ((FAR void *)((FAR uint8_t *)(ipv4) + (((ipv4)->vhl & IPv4_HLMASK) << 2)))
...
static bool ipfilter_match_proto(FAR const struct ipfilter_entry_s *entry, FAR const void *l4hdr,
								 uint8_t proto)
{
	bool matched;
...
static int ipv4_filter_match(FAR const struct net_driver_s *indev,
							 FAR const struct net_driver_s *outdev,
							 FAR const struct ipv4_hdr_s *ipv4, enum ipfilter_chain_e chain)
{
	...
	sq_for_every(queue, entry)
	{
		...
		matched =
			net_ipv4addr_maskcmp(filter->sip, ipaddr, filter->smsk) ^ filter->common.inv_srcip;
		if (!matched) {
			continue;
		}
		...
		matched =
			net_ipv4addr_maskcmp(filter->dip, ipaddr, filter->dmsk) ^ filter->common.inv_dstip;
		if (!matched) {
			continue;
		}
...
static int ipv6_filter_match(FAR const struct net_driver_s *indev,
							 FAR const struct net_driver_s *outdev,
							 FAR const struct ipv6_hdr_s *ipv6, enum ipfilter_chain_e chain)
{
	...
	sq_for_every(queue, entry)
	{
		...
		matched = net_ipv6addr_maskcmp(filter->sip, ipv6->srcipaddr, filter->smsk) ^
				  filter->common.inv_srcip;
		if (!matched) {
			continue;
		}

		matched = net_ipv6addr_maskcmp(filter->dip, ipv6->destipaddr, filter->dmsk) ^
...
void ipfilter_cfg_add(FAR struct ipfilter_entry_s *entry, sa_family_t family,
...
int ipv4_filter_fwd(FAR struct net_driver_s *indev, FAR struct net_driver_s *outdev,
...
int ipv6_filter_fwd(FAR struct net_driver_s *indev, FAR struct net_driver_s *outdev,
