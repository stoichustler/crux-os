...
static struct net_cache_ipv4_entry_s g_prealloc_ipv4cache[CONFIG_ROUTE_MAX_IPv4_CACHEROUTES];
...
static struct net_cache_ipv6_entry_s g_prealloc_ipv6cache[CONFIG_ROUTE_MAX_IPv6_CACHEROUTES];
...
FAR struct net_cache_ipv4_entry_s *net_remove_oldest_ipv4(void)
{
	...
	if (cache != NULL) {
		if (g_ipv4_cache.head == g_ipv4_cache.tail) {
			...
			for (prev = g_ipv4_cache.head; prev != NULL && prev->flink != cache; prev = prev->flink)
				;
...
FAR struct net_cache_ipv6_entry_s *net_remove_oldest_ipv6(void)
{
	...
	if (cache != NULL) {
		if (g_ipv6_cache.head == g_ipv6_cache.tail) {
			...
			for (prev = g_ipv6_cache.head; prev != NULL && prev->flink != cache; prev = prev->flink)
				;
...
int net_addcache_ipv4(FAR struct net_route_ipv4_s *route)
{
	...
	for (prev = NULL, cache = g_ipv4_cache.head; cache != NULL;
...
int net_addcache_ipv6(FAR struct net_route_ipv6_s *route)
{
	...
	for (prev = NULL, cache = g_ipv6_cache.head; cache != NULL;
		 prev = cache, cache = cache->flink) {
		/* To match, the masked target addresses and netmasks must be the
		 * same.
		 */
