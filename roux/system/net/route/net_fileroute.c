...
int net_openroute_detached(FAR const char *pathname, int oflags, FAR struct file *filep)
{
	int ret;
	...
	if (S_ISREG(buf.st_mode)) {
		...
		if (nentries * entrysize != buf.st_size) {
			...
			nwarn("         %lu != %lu / %lu\n", (unsigned long)nentries,
				  (unsigned long)buf.st_size, (unsigned long)entrysize);
		}
#endif
...
#ifdef CONFIG_ROUTE_IPv4_FILEROUTE
ssize_t net_readroute_ipv4(FAR struct file *filep, FAR struct net_route_ipv4_s *route)
{
	FAR uint8_t *dest;
	ssize_t ntotal;
	ssize_t ret;
...
#ifdef CONFIG_ROUTE_IPv6_FILEROUTE
ssize_t net_readroute_ipv6(FAR struct file *filep, FAR struct net_route_ipv6_s *route)
{
	FAR uint8_t *dest;
	ssize_t ntotal;
	ssize_t ret;
...
#ifdef CONFIG_ROUTE_IPv4_FILEROUTE
ssize_t net_writeroute_ipv4(FAR struct file *filep, FAR const struct net_route_ipv4_s *route)
{
...
#ifdef CONFIG_ROUTE_IPv6_FILEROUTE
ssize_t net_writeroute_ipv6(FAR struct file *filep, FAR const struct net_route_ipv6_s *route)
{
	FAR const uint8_t *src;
	ssize_t ntotal;
	ssize_t ret;
