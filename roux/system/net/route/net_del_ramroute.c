...
#ifdef CONFIG_ROUTE_IPv4_RAMROUTE
static int net_del_ipv4route(FAR struct net_route_ipv4_s *route, FAR void *arg)
{
...
#ifdef CONFIG_ROUTE_IPv6_RAMROUTE
static int net_del_ipv6route(FAR struct net_route_ipv6_s *route, FAR void *arg)
{
	...
	ninfo("With:\n");
	ninfo("  target:  %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", HTONS(match->target[0]),
		  HTONS(match->target[1]), HTONS(match->target[2]), HTONS(match->target[3]),
		  HTONS(match->target[4]), HTONS(match->target[5]), HTONS(match->target[6]),
		  HTONS(match->target[7]));
	ninfo("  netmask: %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", HTONS(match->netmask[0]),
		  HTONS(match->netmask[1]), HTONS(match->netmask[2]), HTONS(match->netmask[3]),
		  HTONS(match->netmask[4]), HTONS(match->netmask[5]), HTONS(match->netmask[6]),
		  HTONS(match->netmask[7]));
