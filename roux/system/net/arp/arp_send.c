...
static void arp_send_terminate(FAR struct net_driver_s *dev, FAR struct arp_send_s *state,
							   int result)
{
...
static uint16_t arp_send_eventhandler(FAR struct net_driver_s *dev, FAR void *priv, uint16_t flags)
{
...
int arp_send(in_addr_t ipaddr)
{
	...
	do {
		...
		do {
			ret = net_sem_timedwait_uninterruptible(&state.snd_sem, CONFIG_ARP_SEND_DELAYMSEC);
			if (ret == -ETIMEDOUT) {
