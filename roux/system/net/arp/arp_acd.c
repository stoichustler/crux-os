...
static void arp_acd_arrange_announce(FAR struct net_driver_s *dev)
{
	...
	int ret = work_queue(LPWORK, &dev->d_acd.work, arp_acd_try_announce, (FAR void *)dev,
						 DSEC2TICK(dev->d_acd.ttw));
	if (ret != OK) {
		nerr("ERROR ret %d \n", ret);
	}
}
...
void arp_acd_update(FAR struct net_driver_s *dev)
{
	...
	if (!net_ipv4addr_hdrcmp(arp->ah_sipaddr, &dev->d_ipaddr) ||
		(memcmp(arp->ah_shwaddr, dev->d_mac.ether.ether_addr_octet, sizeof(arp->ah_shwaddr)) ==
		 0)) {
		return;
	}

	if ((dev->d_acd.lastconflict > 0) &&
		(now - dev->d_acd.lastconflict) < DSEC2TICK(DEFEND_INTERVAL * ARP_ACD_TICKS_PER_SECOND)) {
		...
		if (dev->d_acd.state != ARP_ACD_STATE_ANNOUNCING) {
			...
			dev->d_acd.sendnum = 0;
			dev->d_acd.ttw = ANNOUNCE_WAIT * ARP_ACD_TICKS_PER_SECOND;
		}
	}
}
