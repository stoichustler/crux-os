...
NET_BUFPOOL_DECLARE(g_cbprealloc, sizeof(struct devif_callback_s),
					CONFIG_NET_PREALLOC_DEVIF_CALLBACKS, CONFIG_NET_ALLOC_DEVIF_CALLBACKS, 0);
...
static void devif_callback_free(FAR struct net_driver_s *dev, FAR struct devif_callback_s *cb,
								...
								FAR struct devif_callback_s **list_tail)
{
	...
	if (cb) {
		...
		if (dev != NULL) {
			...
			for (prev = NULL, curr = dev->d_devcb; curr != NULL && curr != cb;
