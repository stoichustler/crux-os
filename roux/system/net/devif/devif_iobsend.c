...
int devif_iob_send(FAR struct net_driver_s *dev, FAR struct iob_s *iob, unsigned int len,
				   unsigned int offset, unsigned int target_offset)
{
	...
	ret = iob_clone_partial(iob, len, offset, dev->d_iob, target_offset, false, false);
	if (ret != OK) {
