					...
					unsigned int offset, unsigned int target_offset)
{
	...
	if (len > iob_navail(false) * CONFIG_IOB_BUFSIZE || netdev_iob_prepare(dev, false, 0) != OK) {
		ret = -ENOMEM;
		goto errout;
	}
	...
	while (remain > 0) {
		...
		copying = CONFIG_IOB_BUFSIZE - (iob->io_len + iob->io_offset);
		if (copying > remain) {
			copying = remain;
		}

		if (copying > 0) {
			ret = file_read(file, iob->io_data + (iob->io_len + iob->io_offset), copying);
			if (ret < 0) {
				goto errout;
			}
