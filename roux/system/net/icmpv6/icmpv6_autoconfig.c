...
static void icmpv6_router_terminate(FAR struct icmpv6_router_s *state, int result)
{
...
static uint16_t icmpv6_router_eventhandler(FAR struct net_driver_s *dev, FAR void *priv,
										   uint16_t flags)
{
...
int icmpv6_autoconfig(FAR struct net_driver_s *dev)
{
	...
	ninfo("lladdr=%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", NTOHS(lladdr[0]), NTOHS(lladdr[1]),
		  NTOHS(lladdr[2]), NTOHS(lladdr[3]), NTOHS(lladdr[4]), NTOHS(lladdr[5]), NTOHS(lladdr[6]),
		  NTOHS(lladdr[7]));
