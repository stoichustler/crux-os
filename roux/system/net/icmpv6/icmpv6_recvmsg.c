...
#define ICMPv6_SIZE ((dev)->d_len - IPv6_HDRLEN)
...
static uint16_t recvfrom_eventhandler(FAR struct net_driver_s *dev, FAR void *pvpriv,
									  uint16_t flags)
{
	...
	if (pstate != NULL) {
		...
		if ((flags & ICMPv6_NEWDATA) != 0) /* No incoming data */
		{
			...
			if (_SO_GETOPT(conn->sconn.s_options, IPV6_RECVHOPLIMIT)) {
				...
				cmsg_append(pstate->msg, SOL_IPV6, IPV6_HOPLIMIT, &hoplimit, sizeof(hoplimit));
			}
#endif
...
static inline ssize_t icmpv6_readahead(FAR struct icmpv6_conn_s *conn, FAR void *buf, size_t buflen,
									   FAR struct msghdr *msg)
{
	...
	if ((iob = iob_peek_queue(&conn->readahead)) != NULL) {
		...
		if (_SO_GETOPT(conn->sconn.s_options, IPV6_RECVHOPLIMIT)) {
			...
			cmsg_append(msg, SOL_IPV6, IPV6_HOPLIMIT, &hoplimit, sizeof(hoplimit));
		}
#endif
...
ssize_t icmpv6_recvmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
	...
	if (!IOB_QEMPTY(&conn->readahead)) {
		...
		if (state.recv_cb) {
			...
			ret = net_sem_timedwait(&state.recv_sem, _SO_TIMEOUT(conn->sconn.s_rcvtimeo));
			if (ret < 0) {
		...
		if (from != NULL) {
			...
			net_ipv6addr_copy(inaddr->sin6_addr.s6_addr16, state.recv_from.s6_addr16);
		}
