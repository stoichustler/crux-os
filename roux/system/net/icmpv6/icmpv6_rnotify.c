						 ...
						 const net_ipv6addr_t prefix, unsigned int preflen)
{
	...
	ninfo("preflen=%d netmask=%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", preflen, NTOHS(mask[0]),
		  NTOHS(mask[1]), NTOHS(mask[2]), NTOHS(mask[3]), NTOHS(mask[4]), NTOHS(mask[5]),
		  NTOHS(mask[6]), NTOHS(mask[7]));
	...
	ninfo("prefix=%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", NTOHS(prefix[0]), NTOHS(prefix[1]),
		  NTOHS(prefix[2]), NTOHS(prefix[3]), NTOHS(prefix[4]), NTOHS(prefix[5]), NTOHS(prefix[6]),
		  NTOHS(prefix[7]));
	ninfo("IP address=%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", NTOHS(addr[0]), NTOHS(addr[1]),
		  NTOHS(addr[2]), NTOHS(addr[3]), NTOHS(addr[4]), NTOHS(addr[5]), NTOHS(addr[6]),
		  NTOHS(addr[7]));
	...
	ninfo("DR address=%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", NTOHS(dev->d_ipv6draddr[0]),
		  NTOHS(dev->d_ipv6draddr[1]), NTOHS(dev->d_ipv6draddr[2]), NTOHS(dev->d_ipv6draddr[3]),
		  NTOHS(dev->d_ipv6draddr[4]), NTOHS(dev->d_ipv6draddr[5]), NTOHS(dev->d_ipv6draddr[6]),
		  NTOHS(dev->d_ipv6draddr[7]));
...
int icmpv6_rwait_cancel(FAR struct icmpv6_rnotify_s *notify)
{
	...
	flags = enter_critical_section();
	for (prev = NULL, curr = g_icmpv6_rwaiters; curr && curr != notify;
...
void icmpv6_rnotify(FAR struct net_driver_s *dev, int result)
{
	...
	for (curr = g_icmpv6_rwaiters; curr; curr = curr->rn_flink) {
		...
		if (curr->rn_result != OK && strncmp(curr->rn_ifname, dev->d_ifname, IFNAMSIZ) == 0) {
