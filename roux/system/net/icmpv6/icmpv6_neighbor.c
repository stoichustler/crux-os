...
static uint16_t icmpv6_neighbor_eventhandler(FAR struct net_driver_s *dev, FAR void *priv,
											 uint16_t flags)
{
	...
	if (state) {
		...
		if (strncmp((FAR const char *)dev->d_ifname, (FAR const char *)state->snd_ifname,
					IFNAMSIZ) != 0) {
...
int icmpv6_neighbor(FAR struct net_driver_s *dev, const net_ipv6addr_t ipaddr)
{
	...
	if (net_ipv6addr_cmp(ipaddr, g_ipv6_unspecaddr) || net_is_addr_mcast(ipaddr)) {
	...
	net_lock();
	state.snd_cb = devif_callback_alloc((dev), &(dev)->d_conncb, &(dev)->d_conncb_tail);
	if (!state.snd_cb) {
	...
	strlcpy((FAR char *)state.snd_ifname, (FAR const char *)dev->d_ifname, IFNAMSIZ);
