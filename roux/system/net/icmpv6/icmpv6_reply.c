...
void icmpv6_reply(FAR struct net_driver_s *dev, int type, int code, int data)
{
	...
	if (CONFIG_IOB_BUFSIZE >= datalen + ipicmplen + CONFIG_NET_LL_GUARDSIZE) {
		...
		if (iob_trycopyin(dev->d_iob, (FAR void *)ipv6, IPv6_HDRLEN, 0, false) != IPv6_HDRLEN) {
			dev->d_len = 0;
			netdev_iob_release(dev);
			iob_free_chain(iob);
			return;
		...
		iob_update_pktlen(dev->d_iob, dev->d_iob->io_pktlen + sizeof(struct icmpv6_hdr_s), false);
	...
	ipv6_build_header(IPv6BUF, dev->d_len - IPv6_HDRLEN, IP_PROTO_ICMP6,
					  netdev_ipv6_srcaddr(dev, ipv6->destipaddr), ipv6->srcipaddr, 255, 0);
