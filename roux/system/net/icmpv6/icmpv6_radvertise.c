...
static const net_ipv6addr_t g_ipv6_prefix = {
	HTONS(CONFIG_NET_ICMPv6_PREFIX_1), HTONS(CONFIG_NET_ICMPv6_PREFIX_2),
	HTONS(CONFIG_NET_ICMPv6_PREFIX_3), HTONS(CONFIG_NET_ICMPv6_PREFIX_4),
	HTONS(CONFIG_NET_ICMPv6_PREFIX_5), HTONS(CONFIG_NET_ICMPv6_PREFIX_6),
	HTONS(CONFIG_NET_ICMPv6_PREFIX_7), HTONS(CONFIG_NET_ICMPv6_PREFIX_8)};
...
static int icmpv6_radvertise_fill_rndss(FAR void *arg, FAR struct sockaddr *addr,
										FAR socklen_t addrlen)
{
	...
	if (addr->sa_family == AF_INET6) {
		...
		net_ipv6addr_copy(server + 16 * rdnss_context->nservers, &addr6->sin6_addr);
...
void icmpv6_radvertise(FAR struct net_driver_s *dev)
{
	...
	l3size = sizeof(struct icmpv6_router_advertise_s) + SIZEOF_ICMPV6_SRCLLADDR_S(lladdrsize) +
	...
	adv->lifetime = HTONS(CONFIG_NET_ICMPv6_ROUTER_LIFETIME); /* Router lifetime */
	...
	srcaddr = (FAR struct icmpv6_srclladdr_s *)((FAR uint8_t *)adv +
	...
	mtu =
		(FAR struct icmpv6_mtu_s *)((FAR uint8_t *)srcaddr + SIZEOF_ICMPV6_SRCLLADDR_S(lladdrsize));
	...
	prefix = (FAR struct icmpv6_prefixinfo_s *)((FAR uint8_t *)mtu + sizeof(struct icmpv6_mtu_s));
...
skip_prefix:
...
#ifdef CONFIG_NET_ICMPv6_ROUTER_RDNSS
	rdnss = (FAR struct icmpv6_rdnss_s *)((FAR uint8_t *)adv + l3size);
	...
	ipv6_build_header(IPv6BUF, l3size, IP_PROTO_ICMP6, srcv6addr, g_ipv6_allnodes, 255, 0);
