...
static uint16_t icmpv6_datahandler(FAR struct net_driver_s *dev, FAR struct icmpv6_conn_s *conn,
								   unsigned int iplen)
{
	...
	ret = iob_clone_partial(dev->d_iob, dev->d_iob->io_pktlen, iplen, iob, 0, true, false);
	if (ret < 0) {
		iob_free_chain(iob);
		return ret;
	}
...
static int icmpv6_delivery_callback(FAR struct icmpv6_conn_s *conn, FAR void *arg)
{
	...
	if (icmpv6_filter(conn->filter.icmp6_filt, icmpv6->type) &&
		(icmpv6->type != ICMPv6_ECHO_REPLY || conn->id != ICMPv6REPLY->id || conn->dev != dev)) {
		return 0;
	}

	info->delivered = true;
	if (devif_conn_event(dev, ICMPv6_NEWDATA, conn->sconn.list) != ICMPv6_NEWDATA) {
...
void icmpv6_input(FAR struct net_driver_s *dev, unsigned int iplen)
{
	...
	case ICMPV6_ROUTER_ADVERTISE: {
		...
		for (ndx = 0; ndx < optlen;) {
			FAR struct icmpv6_generic_s *opt = (FAR struct icmpv6_generic_s *)&options[ndx];
			...
			case ICMPv6_OPT_PREFIX: {
				FAR struct icmpv6_prefixinfo_s *prefixopt = (FAR struct icmpv6_prefixinfo_s *)opt;
				...
				if ((prefixopt->flags & ICMPv6_PRFX_FLAG_A) != 0) {
					...
					icmpv6_setaddresses(dev, ipv6->srcipaddr, prefixopt->prefix,
										prefixopt->preflen);
					netlink_ipv6_prefix_notify(dev, RTM_NEWPREFIX, prefixopt);
				} else if ((adv->flags & ICMPv6_RADV_FLAG_M) != 0) {
					net_ipv6addr_copy(dev->d_ipv6draddr, ipv6->srcipaddr);
				}

				/* Notify any waiting threads */

				icmpv6_rnotify(dev, (adv->flags & ICMPv6_RADV_FLAG_M) ? -EADDRNOTAVAIL : OK);
				prefix = true;
			} break;

			case ICMPv6_OPT_MTU: {
				FAR struct icmpv6_mtu_s *mtuopt = (FAR struct icmpv6_mtu_s *)opt;
				dev->d_pktsize = NTOHS(mtuopt->mtu[1]) + dev->d_llhdrlen;
			} break;

#ifdef CONFIG_ICMPv6_AUTOCONF_RDNSS
			case ICMPv6_OPT_RDNSS: {
				FAR struct icmpv6_rdnss_s *rdnss = (FAR struct icmpv6_rdnss_s *)opt;
