...
static void sendto_request(FAR struct net_driver_s *dev, FAR struct icmpv6_sendto_s *pstate)
{
...
ssize_t icmpv6_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
	...
	if (psock->s_type != SOCK_RAW &&
		(icmpv6->type != ICMPv6_ECHO_REQUEST || icmpv6->id != conn->id || dev != conn->dev)) {
		conn->id = 0;
		conn->dev = NULL;
