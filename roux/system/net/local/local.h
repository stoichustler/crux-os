...
struct local_conn_s {
	...
	FAR struct local_conn_s *lc_peer; /* Peer connection instance */
#ifdef CONFIG_NET_LOCAL_SCM
	uint16_t lc_cfpcount;						 /* Control file pointer counter */
	FAR struct file *lc_cfps[LOCAL_NCONTROLFDS]; /* Socket message control filep */
...
int local_alloc_accept(FAR struct local_conn_s *server, FAR struct local_conn_s *client,
...
FAR struct local_conn_s *local_findconn(FAR const struct local_conn_s *conn,
...
int psock_local_bind(FAR struct socket *psock, FAR const struct sockaddr *addr, socklen_t addrlen);
...
int psock_local_connect(FAR struct socket *psock, FAR const struct sockaddr *addr);
...
#ifdef CONFIG_NET_LOCAL_STREAM
int local_accept(FAR struct socket *psock, FAR struct sockaddr *addr, FAR socklen_t *addrlen,
...
ssize_t local_sendmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags);
...
int local_send_preamble(FAR struct local_conn_s *conn, FAR struct file *filep,
						FAR const struct iovec *buf, size_t len, size_t rcvsize);
...
int local_send_packet(FAR struct file *filep, FAR const struct iovec *buf, size_t len);
...
ssize_t local_recvmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags);
...
int local_fifo_read(FAR struct file *filep, FAR uint8_t *buf, size_t *len, bool once);
...
int local_getaddr(FAR struct local_conn_s *conn, FAR struct sockaddr *addr, FAR socklen_t *addrlen);
...
int local_create_fifos(FAR struct local_conn_s *conn, uint32_t cssize, uint32_t scsize);
...
int local_create_halfduplex(FAR struct local_conn_s *conn, FAR const char *path, uint32_t bufsize);
#endif
...
int local_open_client_rx(FAR struct local_conn_s *client, FAR struct local_conn_s *server,
						 bool nonblock);
...
int local_open_client_tx(FAR struct local_conn_s *client, FAR struct local_conn_s *server,
						 bool nonblock);
...
#ifdef CONFIG_NET_LOCAL_DGRAM
int local_open_sender(FAR struct local_conn_s *conn, FAR const char *path, bool nonblock);
#endif
...
void local_event_pollnotify(FAR struct local_conn_s *conn, pollevent_t eventset);
...
#ifdef CONFIG_NET_LOCAL_DGRAM
int local_set_pollthreshold(FAR struct local_conn_s *conn, unsigned long threshold);
#endif
