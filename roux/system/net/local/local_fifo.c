...
#define LOCAL_FULLPATH_LEN                                                                         \
	(sizeof(CONFIG_NET_LOCAL_VFS_PATH) + UNIX_PATH_MAX + LOCAL_SUFFIX_LEN + 2 + 8)
...
static void local_format_name(FAR const char *inpath, FAR char *outpath, FAR const char *suffix,
							  int32_t id)
{
	if (strncmp(inpath, CONFIG_NET_LOCAL_VFS_PATH, sizeof(CONFIG_NET_LOCAL_VFS_PATH) - 1) == 0) {
		inpath += sizeof(CONFIG_NET_LOCAL_VFS_PATH) - 1;
	}

	if (id < 0) {
		snprintf(outpath, LOCAL_FULLPATH_LEN - 1, CONFIG_NET_LOCAL_VFS_PATH "/%s%s", inpath,
				 suffix);
	} else {
		snprintf(outpath, LOCAL_FULLPATH_LEN - 1, CONFIG_NET_LOCAL_VFS_PATH "/%s%s%" PRIx32, inpath,
				 suffix, id);
	}
...
{
	local_format_name(conn->lc_path, path, LOCAL_CS_SUFFIX, conn->lc_instance_id);
}
...
{
	local_format_name(conn->lc_path, path, LOCAL_SC_SUFFIX, conn->lc_instance_id);
}
...
static int local_rx_open(FAR struct local_conn_s *conn, FAR const char *path, bool nonblock)
{
	int ret;

	ret = file_open(&conn->lc_infile, path, O_RDONLY | O_NONBLOCK | O_CLOEXEC);
	if (ret < 0) {
...
static int local_tx_open(FAR struct local_conn_s *conn, FAR const char *path, bool nonblock)
{
	int ret;

	ret = file_open(&conn->lc_outfile, path, O_WRONLY | O_NONBLOCK | O_CLOEXEC);
	if (ret < 0) {
...
#ifdef CONFIG_NET_LOCAL_DGRAM
static int local_set_pollinthreshold(FAR struct file *filep, unsigned long threshold)
{
	int ret;
...
#ifdef CONFIG_NET_LOCAL_DGRAM
static int local_set_polloutthreshold(FAR struct file *filep, unsigned long threshold)
{
	int ret;
...
int local_open_receiver(FAR struct local_conn_s *conn, bool nonblock)
{
	...
	if (ret == OK) {
		...
		if (ret == 0) {
			...
			ret = local_set_pollinthreshold(&conn->lc_infile, 2 * sizeof(lc_size_t));
		}
	}
...
int local_open_sender(FAR struct local_conn_s *conn, FAR const char *path, bool nonblock)
{
	...
	if (ret == OK) {
		...
		if (ret == 0) {
			...
			ret = local_set_polloutthreshold(&conn->lc_outfile, 2 * sizeof(lc_size_t));
		}
	}
