...
static int local_event_pollsetup(FAR struct local_conn_s *conn, FAR struct pollfd *fds, bool setup)
{
	int i;
...
int local_pollsetup(FAR struct socket *psock, FAR struct pollfd *fds)
{
	...
	case (POLLIN | POLLOUT): {
		...
		if (conn->lc_infile.f_inode == NULL || conn->lc_outfile.f_inode == NULL) {
			fds->priv = NULL;
			goto pollerr;
		}
