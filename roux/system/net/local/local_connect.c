...
int psock_local_connect(FAR struct socket *psock, FAR const struct sockaddr *addr)
{
	...
	if (client->lc_state == LOCAL_STATE_ACCEPT || client->lc_state == LOCAL_STATE_CONNECTED) {
		return -EISCONN;
	}
	...
	while ((conn = local_nextconn(conn)) != NULL) {
		...
		case LOCAL_TYPE_PATHNAME: /* lc_path holds a null terminated string */
			...
			if (conn->lc_state == LOCAL_STATE_LISTENING && conn->lc_type == type &&
				...
				ret = local_stream_connect(client, conn, _SS_ISNONBLOCK(client->lc_conn.s_flags));
