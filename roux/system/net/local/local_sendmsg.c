...
static int local_sendctl(FAR struct local_conn_s *conn, FAR struct msghdr *msg)
{
	...
	{
		if (!CMSG_OK(msg, cmsg) || cmsg->cmsg_level != SOL_SOCKET ||
...
static ssize_t local_send(FAR struct socket *psock, FAR const struct iovec *buf, size_t len,
						  int flags)
{
	ssize_t ret;
...
static ssize_t local_sendto(FAR struct socket *psock, FAR const struct iovec *buf, size_t len,
							int flags, FAR const struct sockaddr *to, socklen_t tolen)
{
	...
	if (conn->lc_state != LOCAL_STATE_UNBOUND && conn->lc_state != LOCAL_STATE_BOUND) {
	...
	ret = local_open_sender(conn, unaddr->sun_path,
							_SS_ISNONBLOCK(conn->lc_conn.s_flags) || (flags & MSG_DONTWAIT) != 0);
	if (ret < 0) {
	...
	ret = local_send_preamble(conn, &conn->lc_outfile, buf, len, server->lc_rcvsize);
	if (ret < 0) {
