...
static int local_fifo_write(FAR struct file *filep, FAR const uint8_t *buf, size_t len)
{
	ssize_t nwritten = 0;
	ssize_t ret = 0;
						...
						FAR const struct iovec *buf, size_t len, size_t rcvsize)
{
	...
	ret = local_fifo_write(&conn->lc_outfile, (FAR const uint8_t *)&pathlen, sizeof(lc_size_t));
	if (ret != sizeof(lc_size_t)) {
		nerr("ERROR: local send path length failed ret: %d\n", ret);
		return ret;
	}

	ret = local_fifo_write(filep, (FAR const uint8_t *)&pktlen, sizeof(lc_size_t));
	if (ret != sizeof(lc_size_t)) {
		return ret;
	}

	return local_fifo_write(&conn->lc_outfile, (uint8_t *)conn->lc_path, pathlen);
}
