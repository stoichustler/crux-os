...
static int psock_fifo_read(FAR struct socket *psock, FAR void *buf, size_t offset,
						   FAR size_t *readlen, int flags, bool once)
{
	...
	if (flags & MSG_PEEK) {
		struct pipe_peek_s peek = {buf, offset, *readlen};

		ret = file_ioctl(&conn->lc_infile, PIPEIOC_PEEK, (unsigned long)((uintptr_t)&peek));
	} else {
...
static void local_recvctl(FAR struct local_conn_s *conn, FAR struct msghdr *msg, int flags)
{
	...
	fds = cmsg_append(msg, SOL_SOCKET, SCM_RIGHTS, NULL, sizeof(int) * peer->lc_cfpcount);
	if (fds == NULL) {
		goto out;
	}
	...
	for (i = 0; i < count; i++) {
		fds[i] = file_dup(peer->lc_cfps[i], 0, flags & MSG_CMSG_CLOEXEC ? O_CLOEXEC : 0);
	...
	if (i > 0) {
		if (peer->lc_cfpcount) {
			memmove(peer->lc_cfps[0], peer->lc_cfps[i], sizeof(FAR void *) * peer->lc_cfpcount);
		}
	}
...
static inline ssize_t psock_stream_recvfrom(FAR struct socket *psock, FAR void *buf, size_t len,
...
static int psock_fifo_discard(FAR struct socket *psock, size_t len, int flags)
{
...
static inline ssize_t psock_dgram_recvfrom(FAR struct socket *psock, FAR void *buf, size_t len,
										   int flags, FAR struct sockaddr *from,
										   FAR socklen_t *fromlen)
{
	FAR struct local_conn_s *conn = psock->s_conn;
	...
	if (conn->lc_state != LOCAL_STATE_BOUND && conn->lc_state != LOCAL_STATE_CONNECTED) {
	...
	if (from && fromlen && *fromlen) {
		...
		ret = psock_fifo_read(psock, from->sa_data, offset, &pathlen, flags, false);
		if (ret < 0) {
...
ssize_t local_recvmsg(FAR struct socket *psock, FAR struct msghdr *msg, int flags)
{
	...
	if (len >= 0 && msg->msg_control && msg->msg_controllen > sizeof(struct cmsghdr)) {
