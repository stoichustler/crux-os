...
static int local_bind(FAR struct socket *psock, FAR const struct sockaddr *addr, socklen_t addrlen);
static int local_getsockname(FAR struct socket *psock, FAR struct sockaddr *addr,
							 FAR socklen_t *addrlen);
static int local_getpeername(FAR struct socket *psock, FAR struct sockaddr *addr,
							 FAR socklen_t *addrlen);
static int local_connect(FAR struct socket *psock, FAR const struct sockaddr *addr,
						 socklen_t addrlen);
static int local_poll(FAR struct socket *psock, FAR struct pollfd *fds, bool setup);
...
static int local_getsockopt(FAR struct socket *psock, int level, int option, FAR void *value,
							FAR socklen_t *value_len);
static int local_setsockopt(FAR struct socket *psock, int level, int option, FAR const void *value,
							socklen_t value_len);
#endif
...
static int local_setup(FAR struct socket *psock)
{
	...
	case SOCK_CTRL:
		if (psock->s_proto == 0 || psock->s_proto == IPPROTO_TCP || psock->s_proto == IPPROTO_UDP) {
			/* Allocate and attach the local connection structure */
	...
	if (*addrlen > sizeof(sa_family_t)) {
		...
		if (conn->lc_type == LOCAL_TYPE_UNNAMED) {
			...
			*addrlen = sizeof(sa_family_t);
			...
			size_t namelen = strlen(conn->lc_path) + 1 + (conn->lc_type == LOCAL_TYPE_ABSTRACT);
	...
	if (*addrlen > sizeof(sa_family_t)) {
		...
		if (peer->lc_type == LOCAL_TYPE_UNNAMED) {
			...
			*addrlen = sizeof(sa_family_t);
			...
			size_t namelen = strlen(peer->lc_path) + 1 + (peer->lc_type == LOCAL_TYPE_ABSTRACT);
							...
							socklen_t value_len)
{
	...
	if (level == SOL_SOCKET) {
		...
		case SO_SNDBUF: {
			...
			if (conn->lc_peer) {
				rcvsize = MIN(*(FAR const int *)value, CONFIG_DEV_PIPE_MAXSIZE);
				if (conn->lc_peer->lc_infile.f_inode != NULL) {
					ret = file_ioctl(&conn->lc_peer->lc_infile, PIPEIOC_SETSIZE, rcvsize);
				}
		...
		case SO_RCVBUF: {
			...
			if (conn->lc_infile.f_inode != NULL) {
				ret = file_ioctl(&conn->lc_infile, PIPEIOC_SETSIZE, rcvsize);
			}
#ifdef CONFIG_NET_LOCAL_DGRAM
			else if (psock->s_type == SOCK_DGRAM && conn->lc_state == LOCAL_STATE_BOUND) {
				ret = local_create_halfduplex(conn, conn->lc_path, rcvsize);
				if (ret >= 0) {
...
static int local_ioctl(FAR struct socket *psock, int cmd, unsigned long arg)
{
	...
	case PIPEIOC_POLLOUTTHRD:
		...
		break;
	case FIOC_FILEPATH:
		snprintf((FAR char *)(uintptr_t)arg, PATH_MAX, "local:[%s]", conn->lc_path);
		break;
	case BIOC_FLUSH:
		ret = -EINVAL;
		break;
...
static int local_socketpair(FAR struct socket *psocks[2])
{
	...
	for (i = 0; i < 2; i++) {
		...
		snprintf(conns[i]->lc_path, sizeof(conns[i]->lc_path), "socketpair%p", psocks[0]);
	...
	ret = local_create_fifos(conns[0], conns[0]->lc_rcvsize, conns[1]->lc_rcvsize);
	if (ret < 0) {
		goto errout;
	}
	...
	conns[0]->lc_state = conns[1]->lc_state = LOCAL_STATE_CONNECTED;
