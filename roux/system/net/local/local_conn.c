...
FAR struct local_conn_s *local_findconn(FAR const struct local_conn_s *local_conn,
										FAR const struct sockaddr_un *unaddr)
{
	...
	while ((conn = local_nextconn(conn)) != NULL) {
		if (local_conn->lc_proto == conn->lc_proto &&
			strncmp(conn->lc_path, &unaddr->sun_path[index], UNIX_PATH_MAX - 1) == 0) {
			return conn;
		}
	}
...
{
	FAR struct local_conn_s *conn = kmm_zalloc(sizeof(struct local_conn_s));
