...
#endif
static inline int fat_parsesfname(FAR const char **path, FAR struct fat_dirinfo_s *dirinfo,
								  FAR char *terminator);
#ifdef CONFIG_FAT_LFN
static inline int fat_parselfname(FAR const char **path, FAR struct fat_dirinfo_s *dirinfo,
								  FAR char *terminator);
static inline int fat_createalias(FAR struct fat_dirinfo_s *dirinfo);
static inline int fat_findalias(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo);
static inline int fat_uniquealias(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo);
#endif
static int fat_path2dirname(FAR const char **path, FAR struct fat_dirinfo_s *dirinfo,
							FAR char *terminator);
static int fat_findsfnentry(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo);
#ifdef CONFIG_FAT_LFN
static bool fat_cmplfnchunk(FAR uint8_t *chunk, FAR const lfnchar *substr, int nchunk);
static bool fat_cmplfname(FAR const uint8_t *direntry, FAR const lfnchar *substr);
static inline int fat_findlfnentry(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo);
...
static inline int fat_allocatelfnentry(FAR struct fat_mountpt_s *fs,
									   FAR struct fat_dirinfo_s *dirinfo);
#endif
static inline int fat_getsfname(FAR uint8_t *direntry, FAR char *buffer, unsigned int buflen);
#ifdef CONFIG_FAT_LFN
static void fat_getlfnchunk(FAR uint8_t *chunk, FAR lfnchar *dest, int nchunk);
static inline int fat_getlfname(FAR struct fat_mountpt_s *fs, FAR struct fs_dirent_s *dir,
								FAR struct dirent *entry);
#endif
static int fat_putsfname(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo);
#ifdef CONFIG_FAT_LFN
static void fat_initlfname(FAR uint8_t *chunk, int nchunk);
static void fat_putlfnchunk(FAR uint8_t *chunk, FAR const lfnchar *src, int nchunk);
static int fat_putlfname(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo);
#endif
static int fat_putsfdirentry(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo,
...
static int fat_findsfnentry(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo)
{
	...
	for (;;) {
		...
		if (direntry[DIR_NAME] != DIR0_EMPTY && !(DIR_GETATTRIBUTES(direntry) & FATATTR_VOLUMEID) &&
								...
								FAR struct dirent *entry)
{
	...
	for (;;) {
		...
		if ((seqno & LDIR0_LAST) != 0) {
			...
			for (; nsrc > 0 && (lfname[nsrc - 1] == ' ' || lfname[nsrc - 1] == '\0' ||
								lfname[nsrc - 1] == 0xffff);
				 nsrc--)
				;
		...
		for (i = nsrc - 1; i >= 0; i--) {
			offset = fat_ucstoutf8((FAR uint8_t *)entry->d_name, offset, lfname[i]);
		}
#else
		/* Get the string offset associated with the "last" entry. */
		...
		if (seqno < 1) {
			...
			if (offset > 0) {
				memmove(entry->d_name, &entry->d_name[offset], (NAME_MAX + 1) - offset);
			}
#endif
	...
	for (;;) {
		...
		if ((seqno & LDIR0_LAST) != 0 && remainder != 0) {
			...
			if (remainder > 0) {
				...
				fat_putlfnchunk(LDIR_PTRWCHAR6_11(direntry), &dirinfo->fd_lfname[offset + 5],
								nbytes);
				remainder -= nbytes;
			}
			...
			if (remainder > 0) {
				...
				fat_putlfnchunk(LDIR_PTRWCHAR12_13(direntry), &dirinfo->fd_lfname[offset + 11],
								nbytes);
				remainder -= nbytes;
			}
...
int fat_finddirentry(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo,
					 FAR const char *path)
{
	FAR uint8_t *direntry;
	off_t cluster;
...
int fat_allocatedirentry(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo)
{
	int32_t cluster;
	int32_t prevcluster;
	off_t sector;
...
int fat_dirname2path(FAR struct fat_mountpt_s *fs, FAR struct fs_dirent_s *dir,
					 FAR struct dirent *entry)
{
	FAR struct fat_dirent_s *fdir;
	uint16_t diroffset;
...
int fat_dirnamewrite(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo)
{
#ifdef CONFIG_FAT_LFN
	int ret;
...
int fat_dirwrite(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo,
				 uint8_t attributes, uint32_t fattime)
{
#ifdef CONFIG_FAT_LFN
	int ret;
...
int fat_dircreate(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo)
{
	uint32_t fattime;
	int ret;
...
int fat_remove(FAR struct fat_mountpt_s *fs, FAR const char *relpath, bool directory)
{
