...
static int fat_stat_common(FAR struct fat_mountpt_s *fs, FAR uint8_t *direntry,
						   FAR struct stat *buf);
...
static int fat_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	ff->ff_startcluster =
		((uint32_t)DIR_GETFSTCLUSTHI(direntry) << 16) | DIR_GETFSTCLUSTLO(direntry);
...
static int fat_close(FAR struct file *filep)
{
	...
	if ((ff->ff_bflags & UMOUNT_FORCED) == 0) {
		...
		for (prevff = NULL, currff = fs->fs_head; currff && currff != ff;
...
static int fat_zero_cluster(FAR struct fat_mountpt_s *fs, int cluster, int start, int end)
{
	FAR uint8_t *buf;
...
static int fat_get_sectors(FAR struct file *filep, bool read)
{
	...
	if (ff->ff_startcluster == 0) {
		...
		cluster = 0;
		num_traversed = 0;
	} else if (ff->ff_currentcluster >= 2 && ff->ff_currentcluster < fs->fs_nclusters + 2 &&
					   ...
					   FAR struct fs_dirent_s **dir)
{
	...
	if (dirinfo.fd_root) {
		...
		if ((DIR_GETATTRIBUTES(direntry) & FATATTR_DIRECTORY) == 0) {
			...
			fdir->dir.fd_currsector = fat_cluster2sector(fs, fdir->dir.fd_currcluster);
			fdir->dir.fd_index = 2;
		}
	}
...
static int fat_rewinddir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir)
{
	...
	if (fs->fs_type != FSTYPE_FAT32 && fdir->dir.fd_startcluster == 0) {
		/* Handle the FAT12/16 root directory */

		fdir->dir.fd_currcluster = 0;
		fdir->dir.fd_currsector = fs->fs_rootbase;
		fdir->dir.fd_index = 0;
	} else if (fs->fs_type == FSTYPE_FAT32 && fdir->dir.fd_startcluster == fs->fs_rootbase) {
