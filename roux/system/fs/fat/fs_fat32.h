...
#define PART_PUTSTARTSECTOR(n, p, v)                                                               \
	fat_putuint32(UBYTE_PTR(p, PART_ENTRY(n) + PART_STARTSECTOR), v)
...
EXTERN int fat_hwread(FAR struct fat_mountpt_s *fs, FAR uint8_t *buffer, off_t sector,
					  unsigned int nsectors);
EXTERN int fat_hwwrite(FAR struct fat_mountpt_s *fs, FAR uint8_t *buffer, off_t sector,
					   unsigned int nsectors);

/* Cluster / cluster chain access helpers */

EXTERN off_t fat_cluster2sector(FAR struct fat_mountpt_s *fs, uint32_t cluster);
EXTERN off_t fat_getcluster(FAR struct fat_mountpt_s *fs, uint32_t clusterno);
EXTERN int fat_putcluster(FAR struct fat_mountpt_s *fs, uint32_t clusterno, off_t startsector);
EXTERN int fat_removechain(FAR struct fat_mountpt_s *fs, uint32_t cluster);
EXTERN int32_t fat_extendchain(FAR struct fat_mountpt_s *fs, uint32_t cluster);

#define fat_createchain(fs) fat_extendchain(fs, 0)

/* Help for traversing directory trees and accessing directory entries */

EXTERN int fat_nextdirentry(FAR struct fat_mountpt_s *fs, FAR struct fs_fatdir_s *dir);
EXTERN int fat_finddirentry(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo,
							FAR const char *path);
EXTERN int fat_dirnamewrite(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo);
EXTERN int fat_dirwrite(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo,
						uint8_t attributes, uint32_t fattime);
EXTERN int fat_allocatedirentry(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo);
EXTERN int fat_freedirentry(FAR struct fat_mountpt_s *fs, FAR struct fat_dirseq_s *seq);
EXTERN int fat_dirname2path(FAR struct fat_mountpt_s *fs, FAR struct fs_dirent_s *dir,
							FAR struct dirent *entry);

/* File creation and removal helpers */

EXTERN int fat_dirtruncate(FAR struct fat_mountpt_s *fs, FAR uint8_t *direntry);
EXTERN int fat_dirshrink(FAR struct fat_mountpt_s *fs, FAR uint8_t *direntry, off_t length);
EXTERN int fat_dirextend(FAR struct fat_mountpt_s *fs, FAR struct fat_file_s *ff, off_t length);
EXTERN int fat_dircreate(FAR struct fat_mountpt_s *fs, FAR struct fat_dirinfo_s *dirinfo);
EXTERN int fat_remove(FAR struct fat_mountpt_s *fs, FAR const char *relpath, bool directory);
...
EXTERN int fat_ffcacheflush(FAR struct fat_mountpt_s *fs, FAR struct fat_file_s *ff);
EXTERN int fat_ffcacheread(FAR struct fat_mountpt_s *fs, FAR struct fat_file_s *ff, off_t sector);
EXTERN int fat_ffcacheinvalidate(FAR struct fat_mountpt_s *fs, FAR struct fat_file_s *ff);
...
EXTERN int fat_nfreeclusters(FAR struct fat_mountpt_s *fs, FAR fsblkcnt_t *pfreeclusters);
EXTERN int fat_currentsector(FAR struct fat_mountpt_s *fs, FAR struct fat_file_s *ff,
							 off_t position);
