...
static int fat_checkbootrecord(struct fat_mountpt_s *fs)
{
	...
	if (FBR_GETSIGNATURE(fs->fs_buffer) != BOOT_SIGNATURE16 ||
		...
		fwarn("WARNING: Signature: %04x FS sectorsize: %d "
			  ...
			  FBR_GETSIGNATURE(fs->fs_buffer), FBR_GETBYTESPERSEC(fs->fs_buffer),
			  fs->fs_hwsectorsize);
	...
	if (!fs->fs_fattotsec || fs->fs_fattotsec > fs->fs_hwnsectors) {
		fwarn("WARNING: fs_fattotsec %" PRId32 " fs_hwnsectors: %" PRIdOFF "\n", fs->fs_fattotsec,
			  fs->fs_hwnsectors);
	...
	if (ndatasectors > fs->fs_hwnsectors) {
		fwarn("WARNING: ndatasectors %" PRId32 " fs_hwnsectors: %" PRIdOFF "\n", ndatasectors,
			  fs->fs_hwnsectors);
	...
	if (clusterno == 0 || (clusterno >= 2 && clusterno < fs->fs_nclusters + 2)) {
		...
		case FSTYPE_FAT12: {
			...
			if ((clusterno & 1) != 0) {
				...
				value = (fs->fs_buffer[fatindex] & 0x0f) | (uint8_t)nextcluster << 4;
			} else {
...
int fat_dirextend(FAR struct fat_mountpt_s *fs, FAR struct fat_file_s *ff, off_t length)
{
	...
	while (remaining > 0) {
		...
		if (sectndx == 0 &&
			(remaining >= fs->fs_hwsectorsize || (pos + remaining) >= ff->ff_size)) {
...
int fat_currentsector(struct fat_mountpt_s *fs, struct fat_file_s *ff, off_t position)
{
	...
	if (position <= ff->ff_size) {
		...
		finfo("position=%" PRIdOFF " currentsector=%" PRIdOFF " sectorsincluster=%d\n", position,
			  ff->ff_currentsector, ff->ff_sectorsincluster);
