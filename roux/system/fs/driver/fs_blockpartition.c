...
static ssize_t part_read(FAR struct inode *inode, FAR unsigned char *buffer, blkcnt_t start_sector,
						 unsigned int nsectors);
static ssize_t part_write(FAR struct inode *inode, FAR const unsigned char *buffer,
						  blkcnt_t start_sector, unsigned int nsectors);
static int part_geometry(FAR struct inode *inode, FAR struct geometry *geometry);
...
static int part_ioctl(FAR struct inode *inode, int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case BIOC_PARTINFO: {
		FAR struct partition_info_s *info = (FAR struct partition_info_s *)ptr_arg;
		if (info != NULL) {
			info->numsectors = dev->nsectors;
			info->sectorsize = dev->sectorsize;
			info->startsector = dev->firstsector;

			strlcpy(info->parent, dev->parent->i_name, sizeof(info->parent));
	...
	default:
		if (parent->u.i_bops->ioctl) {
			if (cmd == MTDIOC_PROTECT || cmd == MTDIOC_UNPROTECT) {
				FAR struct mtd_protect_s *prot = (FAR struct mtd_protect_s *)ptr_arg;
			...
			if (ret >= 0) {
				if (cmd == BIOC_XIPBASE) {
					...
					*(FAR uint8_t *)base += dev->firstsector * dev->sectorsize;
				} else if (cmd == MTDIOC_GEOMETRY) {
					FAR struct mtd_geometry_s *mgeo = (FAR struct mtd_geometry_s *)ptr_arg;
...
int register_partition_with_inode(FAR const char *partition, mode_t mode, FAR struct inode *parent,
							...
							off_t firstsector, off_t nsectors)
{
	...
	ret = register_partition_with_inode(partition, mode, inode, firstsector, nsectors);
	inode_release(inode);
