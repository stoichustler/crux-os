...
static ssize_t merge_read(FAR struct inode *inode, FAR unsigned char *buffer, blkcnt_t startsector,
						  unsigned int nsectors);
static ssize_t merge_write(FAR struct inode *inode, FAR const unsigned char *buffer,
						   blkcnt_t startsector, unsigned int nsectors);
static int merge_geometry(FAR struct inode *inode, FAR struct geometry *geometry);
...
static int merge_open(FAR struct inode *inode)
{
	...
	for (i = 0; i < priv->npart; i++) {
		...
		if (inode->u.i_bops->geometry == NULL ||
			...
			priv->part[i].geo.geo_sectorsize <= 0 || priv->part[i].geo.geo_nsectors <= 0) {
			ferr("ERROR: Failed to get geometry for %s\n", priv->part[i].path);
			ret = -EINVAL;
			goto err_with_inode;
		}

		if (priv->part[0].geo.geo_sectorsize != priv->part[i].geo.geo_sectorsize) {
		...
		finfo("[%s] nsectors: %" PRIuOFF " sectorsize:%u\n", priv->part[i].path,
			  priv->part[i].geo.geo_nsectors, priv->part[i].geo.geo_sectorsize);
	}
						   ...
						   blkcnt_t startsector, unsigned int nsectors)
{
	...
	for (i = 0; nsectors > 0 && i < priv->npart; i++) {
		...
		nwritten = inode->u.i_bops->write(inode, buffer, startsector, nwritten);
		if (nwritten < 0) {
			break;
		}
...
int register_merge_blockdriver(FAR const char *merge, ...)
{
	...
	priv = kmm_zalloc(sizeof(struct merge_priv_s) + i * sizeof(struct merge_part_s));
	if (priv == NULL) {
