					...
					if (strncmp(entry->name, fs->fs_workbuffer, fs->fs_llformat.namesize) == 0) {
						...
						if (*ptr == '\0') {
							...
							if (direntry->name == NULL) {
								direntry->name =
									(FAR char *)fs_heap_malloc(fs->fs_llformat.namesize + 1);
							}
						...
						FAR struct smartfs_ofile_s *sf)
{
	...
	while (1) {
		...
		while (offset + entrysize < readwrite.count) {
			...
			offset += entrysize;
			entry = (FAR struct smartfs_entry_header_s *)&fs->fs_rwbuffer[offset];
		}
...
#if CONFIG_SMARTFS_ERASEDSTATE == 0xff
#ifdef CONFIG_SMARTFS_ALIGNED_ACCESS
	smartfs_wrle16(&entry->flags,
				   (uint16_t)(SMARTFS_DIRENT_ACTIVE | SMARTFS_DIRENT_DELETING |
...
#ifdef CONFIG_SMARTFS_ALIGNED_ACCESS
	smartfs_wrle16(&entry->flags,
				   (uint16_t)(SMARTFS_DIRENT_EMPTY | type | (mode & SMARTFS_DIRENT_MODE)));
#else
...
int smartfs_sync_internal(FAR struct smartfs_mountpt_s *fs, FAR struct smartfs_ofile_s *sf)
{
	...
	if (sf->bflags & SMARTFS_BFLAG_DIRTY) {
		...
		if (SMARTFS_USED(header) == SMARTFS_ERASEDSTATE_16BIT) {
			...
			SMARTFS_SET_USED(header, SMARTFS_USED(header) + sf->byteswritten);
		}
							...
							off_t offset, int whence)
{
	...
	if (newpos >= sectorstartpos && newpos < sectorstartpos + fs->fs_llformat.availbytes -
