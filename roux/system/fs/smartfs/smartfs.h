...
#define FS_IOCTL(f, c, a)                                                                          \
	(FS_BOPS(f)->ioctl ? FS_BOPS(f)->ioctl((f)->fs_blkdriver, c, a) : (-ENOSYS))
...
#define SMARTFS_ERASEDSTATE_16BIT                                                                  \
	(uint16_t)((CONFIG_SMARTFS_ERASEDSTATE << 8) | CONFIG_SMARTFS_ERASEDSTATE)
...
#define SMARTFS_SET_NEXTSECTOR(h, v) smartfs_wrle16(h->nextsector, (uint16_t)(v))

#define SMARTFS_USED(h) (smartfs_rdle16(h->used))
#define SMARTFS_SET_USED(h, v) smartfs_wrle16(h->used, (uint16_t)(v))

#else
#define SMARTFS_NEXTSECTOR(h) (*((FAR uint16_t *)h->nextsector))
#define SMARTFS_SET_NEXTSECTOR(h, v) ((*((FAR uint16_t *)h->nextsector)) = (uint16_t)(v))

#define SMARTFS_USED(h) (*((FAR uint16_t *)h->used))
#define SMARTFS_SET_USED(h, v) ((*((FAR uint16_t *)h->used)) = (uint16_t)(v))
#endif
...
int smartfs_finddirentry(FAR struct smartfs_mountpt_s *fs, FAR struct smartfs_entry_s *direntry,
						 FAR const char *relpath, FAR uint16_t *parentdirsector,
						 FAR const char **filename);

int smartfs_createentry(FAR struct smartfs_mountpt_s *fs, uint16_t parentdirsector,
						FAR const char *filename, uint16_t type, mode_t mode,
						FAR struct smartfs_entry_s *direntry, uint16_t sectorno,
...
off_t smartfs_seek_internal(FAR struct smartfs_mountpt_s *fs, FAR struct smartfs_ofile_s *sf,
							off_t offset, int whence);

int smartfs_shrinkfile(FAR struct smartfs_mountpt_s *fs, FAR struct smartfs_ofile_s *sf,
					   off_t length);

int smartfs_extendfile(FAR struct smartfs_mountpt_s *fs, FAR struct smartfs_ofile_s *sf,
					   off_t length);
