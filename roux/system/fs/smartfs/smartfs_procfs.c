...
static int smartfs_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry);
...
static int smartfs_opendir(FAR const char *relpath, FAR struct fs_dirent_s **dir)
{
	...
	level1 = (FAR struct smartfs_level1_s *)fs_heap_malloc(sizeof(struct smartfs_level1_s));
...
static int smartfs_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry)
{
	...
	/* We are traversing a subdirectory of task attributes */
		...
		if (level1->base.level == 1) {
			...
			strlcpy(entry->d_name, level1->mount->fs_blkdriver->i_name, sizeof(entry->d_name));
			...
			strlcpy(entry->d_name, g_direntry[level1->base.index++].name, sizeof(entry->d_name));
		} else if (level1->base.level == 3) {
			/* Listing the contents of a specific entry */

			entry->d_type = g_direntry[level1->base.index].type;
			strlcpy(entry->d_name, g_direntry[level1->direntry].name, sizeof(entry->d_name));
			level1->base.index++;
		}
...
static ssize_t smartfs_debug_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	priv->level1.mount->fs_blkdriver->u.i_bops->ioctl(priv->level1.mount->fs_blkdriver,
													  BIOC_DEBUGCMD, (unsigned long)&debug_data);
...
static size_t smartfs_status_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (priv->offset == 0) {
		...
		ret = priv->level1.mount->fs_blkdriver->u.i_bops->ioctl(
			priv->level1.mount->fs_blkdriver, BIOC_GETPROCFSD, (unsigned long)&procfs_data);
		...
		if (ret == OK) {
			...
			if (procfs_data.blockerases == 0) {
				...
				utilization = 100 *
							  (procfs_data.blockerases * procfs_data.sectorsperblk -
...
#endif
						   ,
						   procfs_data.formatversion, procfs_data.namelen, procfs_data.totalsectors,
						   procfs_data.sectorsize, procfs_data.formatsector, procfs_data.dirsector,
