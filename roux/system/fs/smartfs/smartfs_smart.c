...
static int smartfs_closedir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir);
static int smartfs_readdir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir,
						   FAR struct dirent *dentry);
static int smartfs_rewinddir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir);

static int smartfs_bind(FAR struct inode *blkdriver, FAR const void *data, FAR void **handle);
static int smartfs_unbind(FAR void *handle, FAR struct inode **blkdriver, unsigned int flags);
...
static int smartfs_mkdir(FAR struct inode *mountpt, FAR const char *relpath, mode_t mode);
static int smartfs_rmdir(FAR struct inode *mountpt, FAR const char *relpath);
static int smartfs_rename(FAR struct inode *mountpt, FAR const char *oldrelpath,
						  FAR const char *newrelpath);
static void smartfs_stat_common(FAR struct smartfs_mountpt_s *fs, FAR struct smartfs_entry_s *entry,
								FAR struct stat *buf);
static int smartfs_stat(FAR struct inode *mountpt, FAR const char *relpath, FAR struct stat *buf);
...
static int smartfs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	if (ret == OK) {
		...
		if (parentdirsector != 0xffff) {
			...
			ret = smartfs_createentry(fs, parentdirsector, filename, SMARTFS_DIRENT_TYPE_FILE, mode,
									  &sf->entry, 0xffff, sf);
			if (ret != OK) {
				goto errout_with_buffer;
			}
						   ...
						   FAR struct dirent *dentry)
{
	...
	while (sdir->fs_currsector != SMARTFS_ERASEDSTATE_16BIT) {
		...
		while (sdir->fs_curroffset + entrysize < ret) {
			...
			entry = (FAR struct smartfs_entry_header_s *)&fs->fs_rwbuffer[sdir->fs_curroffset];
			...
			sdir->fs_curroffset += entrysize;
			if (sdir->fs_curroffset + entrysize >= fs->fs_llformat.availbytes) {
...
static int smartfs_bind(FAR struct inode *blkdriver, FAR const void *data, FAR void **handle)
{
	...
	fs = (FAR struct smartfs_mountpt_s *)fs_heap_zalloc(sizeof(struct smartfs_mountpt_s));
	if (!fs) {
		return -ENOMEM;
	}
...
static int smartfs_mkdir(FAR struct inode *mountpt, FAR const char *relpath, mode_t mode)
{
	...
	if (ret == OK) {
		...
		ret = smartfs_createentry(fs, parentdirsector, filename, SMARTFS_DIRENT_TYPE_DIR, mode,
								  &entry, 0xffff, NULL);
		if (ret != OK) {
			goto errout_with_lock;
		}
				   ...
				   FAR const char *newrelpath)
{
	...
	if (newparentdirsector != 0xffff) {
		...
		ret = smartfs_createentry(fs, newparentdirsector, newfilename, type, mode, &newentry,
								  oldentry.firstsector, NULL);
		if (ret != OK) {
			goto errout_with_lock;
		}
		...
		direntry = (FAR struct smartfs_entry_header_s *)&fs->fs_rwbuffer[oldentry.doffset];
