...
static int littlefs_rename(FAR struct inode *mountpt, FAR const char *oldrelpath,
...
static int littlefs_fchstat(FAR const struct file *filep, FAR const struct stat *buf, int flags);
static int littlefs_chstat(FAR struct inode *mountpt, FAR const char *relpath,
...
static int littlefs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	ret = littlefs_convert_result(lfs_file_open(&fs->lfs, &priv->file, relpath, oflags));
	if (ret < 0) {
		/* Error opening file */
	...
	if (oflags & LFS_O_CREAT) {
		...
		ret = littlefs_convert_result(lfs_setattr(&fs->lfs, relpath, 0, &attr, sizeof(attr)));
		if (ret < 0) {
	...
	if (oflags & LFS_O_APPEND) {
		ret = littlefs_convert_result(lfs_file_seek(&fs->lfs, &priv->file, 0, LFS_SEEK_END));
		if (ret >= 0) {
			filep->f_pos = ret;
		} else {
			goto errout_with_file;
...
static ssize_t littlefs_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (filep->f_pos != priv->file.pos) {
		ret = littlefs_convert_result(
			lfs_file_seek(&fs->lfs, &priv->file, filep->f_pos, LFS_SEEK_SET));
		if (ret < 0) {
			goto out;
		}
	}

	ret = littlefs_convert_result(lfs_file_read(&fs->lfs, &priv->file, buffer, buflen));
	if (ret > 0) {
		filep->f_pos += ret;
	}
...
static ssize_t littlefs_write(FAR struct file *filep, const char *buffer, size_t buflen)
{
	...
	ret = littlefs_convert_result(lfs_file_write(&fs->lfs, &priv->file, buffer, buflen));
	if (ret > 0) {
		filep->f_pos += ret;
	}
...
static off_t littlefs_seek(FAR struct file *filep, off_t offset, int whence)
{
	...
	ret = littlefs_convert_result(lfs_file_seek(&fs->lfs, &priv->file, offset, whence));
	if (ret >= 0) {
		filep->f_pos = ret;
	}
...
static int littlefs_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case FIOC_FILEPATH: {
		...
		if (ret >= 0) {
			...
			ret = littlefs_convert_result(
				lfs_file_path(&fs->lfs, &priv->file, path + len, PATH_MAX - len));
		}
	} break;
#endif
...
#ifdef CONFIG_FS_LITTLEFS_ATTR_UPDATE
	ret = littlefs_convert_result(lfs_file_getattr(&fs->lfs, &priv->file, 0, &attr, sizeof(attr)));
	if (ret < 0) {
...
static int littlefs_fchstat(FAR const struct file *filep, FAR const struct stat *buf, int flags)
{
	...
	attr.at_ctim = 1000000000ull * buf->st_ctim.tv_sec + buf->st_ctim.tv_nsec;

	if ((CH_STAT_ATIME & flags) == CH_STAT_ATIME) {
		attr.at_atim = 1000000000ull * buf->st_atim.tv_sec + buf->st_atim.tv_nsec;
	}

	if ((CH_STAT_MTIME & flags) == CH_STAT_MTIME) {
		attr.at_mtim = 1000000000ull * buf->st_mtim.tv_sec + buf->st_mtim.tv_nsec;
	}

	ret = littlefs_convert_result(lfs_file_setattr(&fs->lfs, &priv->file, 0, &attr, sizeof(attr)));
	if (ret < 0) {
		goto errout;
	}
...
static int littlefs_truncate(FAR struct file *filep, off_t length)
{
	...
	ret = littlefs_convert_result(lfs_file_truncate(&fs->lfs, &priv->file, length));
	nxmutex_unlock(&fs->lock);
...
static int littlefs_read_block(FAR const struct lfs_config *c, lfs_block_t block, lfs_off_t off,
...
static int littlefs_write_block(FAR const struct lfs_config *c, lfs_block_t block, lfs_off_t off,
...
static int littlefs_bind(FAR struct inode *driver, FAR const void *data, FAR void **handle)
{
	...
	fs->cfg.read_size = fs->geo.blocksize * CONFIG_FS_LITTLEFS_READ_SIZE_FACTOR;
	fs->cfg.prog_size = fs->geo.blocksize * CONFIG_FS_LITTLEFS_PROGRAM_SIZE_FACTOR;
	fs->cfg.block_size = fs->geo.erasesize * CONFIG_FS_LITTLEFS_BLOCK_SIZE_FACTOR;
	...
	fs->cfg.cache_size = fs->geo.blocksize * CONFIG_FS_LITTLEFS_CACHE_SIZE_FACTOR;
...
static int littlefs_mkdir(FAR struct inode *mountpt, FAR const char *relpath, mode_t mode)
{
	...
	if (ret >= 0) {
		...
		attr.at_atim = attr.at_ctim;
		attr.at_mtim = attr.at_ctim;
		ret = littlefs_convert_result(lfs_setattr(&fs->lfs, path, 0, &attr, sizeof(attr)));
		if (ret < 0) {
						   ...
						   FAR const char *newrelpath)
{
	...
	ret = littlefs_convert_result(lfs_rename(&fs->lfs, oldrelpath, newrelpath));
	nxmutex_unlock(&fs->lock);
...
static int littlefs_stat(FAR struct inode *mountpt, FAR const char *relpath, FAR struct stat *buf)
{
	...
	ret = littlefs_convert_result(lfs_getattr(&fs->lfs, relpath, 0, &attr, sizeof(attr)));
	if (ret < 0) {
		if (ret != -ENODATA) {
			goto errout;
		}
