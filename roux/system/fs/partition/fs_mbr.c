...
{
	return (type == DOS_EXTENDED_PARTITION || type == WIN98_EXTENDED_PARTITION ||
...
int parse_mbr_partition(FAR struct partition_state_s *state, partition_handler_t handler,
						FAR void *arg)
{
	...
	for (i = 0; i < 4; i++) {
		pentry.firstblock = MBR_LBA_TO_BLOCK(table[i].partition_start, state->blocksize);
		pentry.nblocks = MBR_LBA_TO_BLOCK(table[i].partition_size, state->blocksize);
	...
	if (extended) {
		uint32_t ebr_block;
		ebr_block = MBR_LBA_TO_BLOCK(extended->partition_start, state->blocksize);
		while (1) {
			...
			pentry.firstblock =
				ebr_block + MBR_LBA_TO_BLOCK(table[0].partition_start, state->blocksize);
			pentry.nblocks = MBR_LBA_TO_BLOCK(table[0].partition_size, state->blocksize);
			handler(&pentry, arg);
			pentry.index++;
			...
			if (table[1].partition_start) {
				ebr_block = pentry.firstblock +
							MBR_LBA_TO_BLOCK(table[1].partition_start, state->blocksize);
			} else {
				break;
			}
		}
