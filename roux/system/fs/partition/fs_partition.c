...
typedef CODE int (*partition_parser_t)(FAR struct partition_state_s *state,
...
static int parse_partition(FAR struct partition_state_s *state, partition_handler_t handler,
						   FAR void *arg);
...
static void register_partition(FAR struct partition_s *part, FAR void *arg)
{
	if (part->name[0] != '\0') {
		...
		if (state->blk != NULL) {
			register_partition_with_inode(path, 0660, state->blk, part->firstblock, part->nblocks);
		}
#ifdef CONFIG_MTD
		else {
			register_partition_with_mtd(path, 0660, state->mtd, part->firstblock, part->nblocks);
		}
#endif
						   ...
						   FAR void *arg)
{
	int i;
	int ret = 0;
	struct partition_register_s reg = {state, arg ? arg : "/dev"};
...
int read_partition_block(FAR struct partition_state_s *state, FAR void *buffer, size_t startblock,
						 size_t nblocks)
{
	if (state->blk) {
...
int parse_block_partition(FAR const char *path, partition_handler_t handler, FAR void *arg)
{
	...
	if (state.blk->u.i_bops->ioctl != NULL &&
		state.blk->u.i_bops->ioctl(state.blk, MTDIOC_GEOMETRY, (unsigned long)(uintptr_t)&mgeo) >=
			0) {
...
int parse_mtd_partition(FAR struct mtd_dev_s *mtd, partition_handler_t handler, FAR void *arg)
{
	struct partition_state_s state;
	struct mtd_geometry_s mgeo;
	int ret;
