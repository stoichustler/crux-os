...
begin_packed_struct struct gpt_header_s {
	...
	uint64_t
		first_usable_lba; /* First usable LBA for partitions primary partition table last LBA + 1 */
	...
	uint64_t partition_entry_lba; /* Starting LBA of array of partition entries (always 2 in primary
					 copy) */
...
{
	return (((uint64_t)state->nblocks) * state->blocksize + GPT_BLOCK_SIZE - 1) / GPT_BLOCK_SIZE -
		   1;
}
														...
														FAR struct gpt_header_s *gpt)
{
	...
	size = le32toh(gpt->num_partition_entries) * le32toh(gpt->sizeof_partition_entry);
	if (!size) {
		return NULL;
	}
	...
	from = GPT_LBA_TO_BLOCK(gpt->partition_entry_lba, state->blocksize);
...
static int gpt_header_is_valid(FAR struct partition_state_s *state, FAR struct gpt_header_s *gpt,
							   blkcnt_t lba)
{
	uint32_t crc;
	uint32_t origcrc;
	...
	gpt->header_crc32 = 0;
	crc = crc32part((FAR const uint8_t *)gpt, le32toh(gpt->header_size), ~0l) ^ ~0l;
	if (crc != le32toh(origcrc)) {
		ferr("GUID Partition Table Header CRC is wrong: %" PRIx32 " != %" PRIx32 "\n", crc,
			 le32toh(origcrc));
		return -EINVAL;
	}
...
static inline int gpt_pte_is_valid(FAR const struct gpt_entry_s *pte, blkcnt_t lastlba)
{
	if (!memcmp(&pte->partition_type_guid, &g_null_guid, sizeof(g_null_guid)) ||
		le64toh(pte->starting_lba) > lastlba || le64toh(pte->ending_lba) > lastlba) {
		return 0;
	}
...
static void gpt_part_set_name(FAR struct gpt_entry_s *pte, FAR char *dest, size_t len)
{
	int i;
...
int parse_gpt_partition(FAR struct partition_state_s *state, partition_handler_t handler,
						FAR void *arg)
{
	...
	count = (sizeof(struct gpt_ptable_s) + (state->blocksize - 1)) / state->blocksize;
	...
	if (ret < 0 || !ptes) {
		...
		ret = read_partition_block(state, ptbl,
								   GPT_LBA_TO_BLOCK(gpt->alternate_lba, state->blocksize), count);
		if (ret < 0) {
			goto err;
		}

		gpt = (FAR struct gpt_header_s *)ptbl;
		ret = gpt_header_is_valid(state, gpt, le64toh(gpt->alternate_lba));
		if (ret >= 0) {
			/* Verify gpt header is valid */
	...
	for (pentry.index = 0; pentry.index < nb_part; pentry.index++) {
		...
		pentry.firstblock = GPT_LBA_TO_BLOCK(ptes[pentry.index].starting_lba, state->blocksize);
		pentry.nblocks = GPT_LBA_TO_BLOCK(ptes[pentry.index].ending_lba + 1, state->blocksize) -
						 pentry.firstblock;
		pentry.blocksize = state->blocksize;
		gpt_part_set_name(&ptes[pentry.index], pentry.name, sizeof(pentry.name));
		handler(&pentry, arg);
	}
