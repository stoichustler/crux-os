...
static FAR void *cromfs_offset2addr(FAR const struct cromfs_volume_s *fs, uint32_t offset);
static uint32_t cromfs_addr2offset(FAR const struct cromfs_volume_s *fs, FAR const void *addr);
...
static int cromfs_foreach_node(FAR const struct cromfs_volume_s *fs,
							   FAR const struct cromfs_node_s *node, bool follow,
...
static int cromfs_compare_node(FAR const struct cromfs_volume_s *fs,
							   FAR const struct cromfs_node_s *node, uint32_t offset,
							   FAR void *arg);
static int cromfs_find_node(FAR const struct cromfs_volume_s *fs, FAR const char *relpath,
							FAR struct cromfs_nodeinfo_s *info, FAR uint32_t *offset);

/* Common file system methods */

static int cromfs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int cromfs_close(FAR struct file *filep);
static ssize_t cromfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static int cromfs_closedir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir);
static int cromfs_readdir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir,
						  FAR struct dirent *entry);
static int cromfs_rewinddir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir);

static int cromfs_bind(FAR struct inode *blkdriver, FAR const void *data, FAR void **handle);
static int cromfs_unbind(FAR void *handle, FAR struct inode **blkdriver, unsigned int flags);
static int cromfs_statfs(FAR struct inode *mountpt, FAR struct statfs *buf);

static int cromfs_stat(FAR struct inode *mountpt, FAR const char *relpath, FAR struct stat *buf);
							  ...
							  FAR struct cromfs_node_s *newnode)
{
	...
	for (i = 0; i < CROMFS_MAX_LINKS; i++) {
		...
		linknode = (FAR const struct cromfs_node_s *)cromfs_offset2addr(fs, node->u.cn_link);
							   ...
							   cromfs_foreach_t callback, FAR void *arg)
{
	...
	while (pnode != NULL) {
		...
		pnode = (FAR const struct cromfs_node_s *)cromfs_offset2addr(fs, offset);
	}
...
static int cromfs_compare_node(FAR const struct cromfs_volume_s *fs,
							   FAR const struct cromfs_node_s *node, uint32_t offset, FAR void *arg)
{
	...
	if (strncmp(name, cpnode->segment, namlen) == 0) {
		...
		child = (FAR const struct cromfs_node_s *)cromfs_offset2addr(fs, node->u.cn_child);
		...
		return cromfs_foreach_node(fs, child, true, cromfs_compare_node, cpnode);
	}
							...
							FAR struct cromfs_nodeinfo_s *info, FAR uint32_t *offset)
{
	...
	root = (FAR const struct cromfs_node_s *)cromfs_offset2addr(fs, fs->cv_root);
...
static int cromfs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	ff->ff_node = (FAR const struct cromfs_node_s *)cromfs_offset2addr(fs, offset);
...
static ssize_t cromfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	blkoffs = 0;
	ulen = 0;
	nexthdr = (FAR struct lzf_header_s *)cromfs_offset2addr(fs, ff->ff_node->u.cn_blocks);
	...
	while (remaining > 0) {
		...
		do {
			...
			if (currhdr->lzf_type == LZF_TYPE0_HDR) {
				FAR struct lzf_type0_header_s *hdr0 = (FAR struct lzf_type0_header_s *)currhdr;

				ulen = (uint16_t)hdr0->lzf_len[0] << 8 | (uint16_t)hdr0->lzf_len[1];
				blksize = (uint32_t)ulen + LZF_TYPE0_HDR_SIZE;
			} else {
				FAR struct lzf_type1_header_s *hdr1 = (FAR struct lzf_type1_header_s *)currhdr;

				ulen = (uint16_t)hdr1->lzf_ulen[0] << 8 | (uint16_t)hdr1->lzf_ulen[1];
				clen = (uint16_t)hdr1->lzf_clen[0] << 8 | (uint16_t)hdr1->lzf_clen[1];
		...
		if (currhdr->lzf_type == LZF_TYPE0_HDR) {
			...
			if (filep->f_pos <= blkoffs && ulen <= remaining) {
				...
				finfo("voloffs=%" PRIu32 " blkoffs=%" PRIu32 " ulen=%" PRIu16 " ff_offset=%" PRIu32
					  " copysize=%u\n",
				...
				if (voloffs != ff->ff_offset) {
					...
					decomplen = lzf_decompress(src, clen, ff->ff_buffer, fs->cv_bsize);

					ff->ff_offset = voloffs;
					ff->ff_ulen = decomplen;
				}

				finfo("voloffs=%" PRIu32 " blkoffs=%" PRIu32 " ulen=%" PRIu16 " clen=%" PRIu16
					  " ff_offset=%" PRIu32 "  copyoffs=%u copysize=%u\n",
					  voloffs, blkoffs, ulen, clen, ff->ff_offset, copyoffs, copysize);
