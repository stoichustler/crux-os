...
static int rpmsgfs_default_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								   uint32_t src, FAR void *priv);
static int rpmsgfs_read_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								uint32_t src, FAR void *priv);
static int rpmsgfs_ioctl_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
static int rpmsgfs_readdir_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								   uint32_t src, FAR void *priv);
static int rpmsgfs_statfs_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
static int rpmsgfs_stat_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								uint32_t src, FAR void *priv);
static void rpmsgfs_device_created(struct rpmsg_device *rdev, FAR void *priv_);
static void rpmsgfs_device_destroy(struct rpmsg_device *rdev, FAR void *priv_);
static int rpmsgfs_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len, uint32_t src,
						  FAR void *priv);
static int rpmsgfs_send_recv(FAR struct rpmsgfs_s *priv, uint32_t command, bool copy,
							 FAR struct rpmsgfs_header_s *msg, int len, FAR void *data);
...
static const rpmsg_ept_cb g_rpmsgfs_handler[] = {
	[RPMSGFS_OPEN] = rpmsgfs_default_handler,	   [RPMSGFS_CLOSE] = rpmsgfs_default_handler,
	[RPMSGFS_READ] = rpmsgfs_read_handler,		   [RPMSGFS_WRITE] = rpmsgfs_default_handler,
	[RPMSGFS_LSEEK] = rpmsgfs_default_handler,	   [RPMSGFS_IOCTL] = rpmsgfs_ioctl_handler,
	[RPMSGFS_SYNC] = rpmsgfs_default_handler,	   [RPMSGFS_DUP] = rpmsgfs_default_handler,
	[RPMSGFS_FSTAT] = rpmsgfs_stat_handler,		   [RPMSGFS_FTRUNCATE] = rpmsgfs_default_handler,
	[RPMSGFS_OPENDIR] = rpmsgfs_default_handler,   [RPMSGFS_READDIR] = rpmsgfs_readdir_handler,
	[RPMSGFS_REWINDDIR] = rpmsgfs_default_handler, [RPMSGFS_CLOSEDIR] = rpmsgfs_default_handler,
	[RPMSGFS_STATFS] = rpmsgfs_statfs_handler,	   [RPMSGFS_UNLINK] = rpmsgfs_default_handler,
	[RPMSGFS_MKDIR] = rpmsgfs_default_handler,	   [RPMSGFS_RMDIR] = rpmsgfs_default_handler,
	[RPMSGFS_RENAME] = rpmsgfs_default_handler,	   [RPMSGFS_STAT] = rpmsgfs_stat_handler,
	[RPMSGFS_FCHSTAT] = rpmsgfs_default_handler,   [RPMSGFS_CHSTAT] = rpmsgfs_default_handler,
};
								   ...
								   uint32_t src, FAR void *priv)
{
	...
	FAR struct rpmsgfs_cookie_s *cookie = (struct rpmsgfs_cookie_s *)(uintptr_t)header->cookie;
...
{
	FAR struct rpmsgfs_header_s *header = data;
	FAR struct rpmsgfs_cookie_s *cookie = (FAR struct rpmsgfs_cookie_s *)(uintptr_t)header->cookie;
...
static FAR void *rpmsgfs_get_tx_payload_buffer(FAR struct rpmsgfs_s *priv, FAR uint32_t *len)
{
	int sval;
...
static void rpmsgfs_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	...
	if (strcmp(priv->cpuname, rpmsg_get_cpuname(rdev)) == 0) {
		...
		rpmsg_create_ept(&priv->ept, rdev, buf, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY, rpmsgfs_ept_cb,
						 NULL);
	}
}

static void rpmsgfs_device_destroy(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
...
int rpmsgfs_client_open(FAR void *handle, FAR const char *pathname, int flags, int mode)
{
	...
	return rpmsgfs_send_recv(priv, RPMSGFS_OPEN, false, (struct rpmsgfs_header_s *)msg, len, NULL);
}
...
int rpmsgfs_client_close(FAR void *handle, int fd)
{
	...
	return rpmsgfs_send_recv(handle, RPMSGFS_CLOSE, true, (struct rpmsgfs_header_s *)&msg,
							 sizeof(msg), NULL);
}

ssize_t rpmsgfs_client_read(FAR void *handle, int fd, FAR void *buf, size_t count)
{
	FAR struct rpmsgfs_s *priv = handle;
	struct iovec read = {
		.iov_base = buf,
...
ssize_t rpmsgfs_client_write(FAR void *handle, int fd, FAR const void *buf, size_t count)
{
	FAR struct rpmsgfs_s *priv = handle;
	struct rpmsgfs_cookie_s cookie;
	size_t written = 0;
...
off_t rpmsgfs_client_lseek(FAR void *handle, int fd, off_t offset, int whence)
{
	...
	return rpmsgfs_send_recv(handle, RPMSGFS_LSEEK, true, (struct rpmsgfs_header_s *)&msg,
							 sizeof(msg), NULL);
}

int rpmsgfs_client_ioctl(FAR void *handle, int fd, int request, unsigned long arg)
{
	...
	while (1) {
		...
		ret = rpmsgfs_send_recv(handle, RPMSGFS_IOCTL, false, (FAR struct rpmsgfs_header_s *)msg,
								len, arglen > 0 ? (FAR void *)arg : NULL);
...
void rpmsgfs_client_sync(FAR void *handle, int fd)
{
	...
	rpmsgfs_send_recv(handle, RPMSGFS_SYNC, true, (struct rpmsgfs_header_s *)&msg, sizeof(msg),
					  NULL);
}
...
int rpmsgfs_client_dup(FAR void *handle, int fd)
{
	...
	return rpmsgfs_send_recv(handle, RPMSGFS_DUP, true, (struct rpmsgfs_header_s *)&msg,
							 sizeof(msg), NULL);
}
...
int rpmsgfs_client_fstat(FAR void *handle, int fd, struct stat *buf)
{
	...
	return rpmsgfs_send_recv(handle, RPMSGFS_FSTAT, true, (struct rpmsgfs_header_s *)&msg,
							 sizeof(msg), buf);
}
...
int rpmsgfs_client_ftruncate(FAR void *handle, int fd, off_t length)
{
	...
	return rpmsgfs_send_recv(handle, RPMSGFS_FTRUNCATE, true, (struct rpmsgfs_header_s *)&msg,
							 sizeof(msg), NULL);
}
...
FAR void *rpmsgfs_client_opendir(FAR void *handle, FAR const char *name)
{
	...
	ret =
		rpmsgfs_send_recv(priv, RPMSGFS_OPENDIR, false, (struct rpmsgfs_header_s *)msg, len, NULL);

	return ret < 0 ? NULL : (FAR void *)((uintptr_t)ret);
}

int rpmsgfs_client_readdir(FAR void *handle, FAR void *dirp, FAR struct dirent *entry)
{
	struct rpmsgfs_readdir_s msg = {
		.fd = (uintptr_t)dirp,
	};

	return rpmsgfs_send_recv(handle, RPMSGFS_READDIR, true, (struct rpmsgfs_header_s *)&msg,
							 sizeof(msg), entry);
}
...
void rpmsgfs_client_rewinddir(FAR void *handle, FAR void *dirp)
{
	...
	rpmsgfs_send_recv(handle, RPMSGFS_REWINDDIR, true, (struct rpmsgfs_header_s *)&msg, sizeof(msg),
					  NULL);
}
...
int rpmsgfs_client_bind(FAR void **handle, FAR const char *cpuname)
{
	...
	ret = rpmsg_register_callback(priv, rpmsgfs_device_created, rpmsgfs_device_destroy, NULL, NULL);
	if (ret < 0) {
...
int rpmsgfs_client_unbind(FAR void *handle)
{
	...
	rpmsg_unregister_callback(priv, rpmsgfs_device_created, rpmsgfs_device_destroy, NULL, NULL);
...
int rpmsgfs_client_closedir(FAR void *handle, FAR void *dirp)
{
	...
	return rpmsgfs_send_recv(handle, RPMSGFS_CLOSEDIR, true, (struct rpmsgfs_header_s *)&msg,
							 sizeof(msg), NULL);
}

int rpmsgfs_client_statfs(FAR void *handle, FAR const char *path, FAR struct statfs *buf)
{
	struct rpmsgfs_s *priv = handle;
	...
	return rpmsgfs_send_recv(priv, RPMSGFS_STATFS, false, (struct rpmsgfs_header_s *)msg, len, buf);
}
...
int rpmsgfs_client_unlink(FAR void *handle, FAR const char *pathname)
{
	...
	return rpmsgfs_send_recv(priv, RPMSGFS_UNLINK, false, (struct rpmsgfs_header_s *)msg, len,
							 NULL);
}

int rpmsgfs_client_mkdir(FAR void *handle, FAR const char *pathname, mode_t mode)
{
	struct rpmsgfs_s *priv = handle;
	...
	return rpmsgfs_send_recv(priv, RPMSGFS_MKDIR, false, (struct rpmsgfs_header_s *)msg, len, NULL);
}
...
int rpmsgfs_client_rmdir(FAR void *handle, FAR const char *pathname)
{
	...
	return rpmsgfs_send_recv(priv, RPMSGFS_RMDIR, false, (struct rpmsgfs_header_s *)msg, len, NULL);
}

int rpmsgfs_client_rename(FAR void *handle, FAR const char *oldpath, FAR const char *newpath)
{
	struct rpmsgfs_s *priv = handle;
	...
	return rpmsgfs_send_recv(priv, RPMSGFS_RENAME, false, (struct rpmsgfs_header_s *)msg, len,
							 NULL);
}

int rpmsgfs_client_stat(FAR void *handle, FAR const char *path, FAR struct stat *buf)
{
	FAR struct rpmsgfs_s *priv = handle;
	...
	return rpmsgfs_send_recv(priv, RPMSGFS_STAT, false, (struct rpmsgfs_header_s *)msg, len, buf);
}

int rpmsgfs_client_fchstat(FAR void *handle, int fd, const struct stat *buf, int flags)
{
	...
	return rpmsgfs_send_recv(handle, RPMSGFS_FCHSTAT, true, (struct rpmsgfs_header_s *)&msg,
							 sizeof(msg), NULL);
}

int rpmsgfs_client_chstat(FAR void *handle, FAR const char *path, FAR const struct stat *buf,
						  int flags)
{
	FAR struct rpmsgfs_s *priv = handle;
	...
	return rpmsgfs_send_recv(priv, RPMSGFS_CHSTAT, false, (struct rpmsgfs_header_s *)msg, len,
							 NULL);
}
