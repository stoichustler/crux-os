...
static inline void spiffs_mtd_dump(FAR const char *msg, FAR const uint8_t *buffer,
								   unsigned int buflen)
{
...
ssize_t spiffs_mtd_write(FAR struct spiffs_s *fs, off_t offset, size_t len, FAR const uint8_t *src)
{
	size_t remaining;
	ssize_t ret;
	off_t blkmask;
...
#endif
	{
		...
		spiffs_mtdinfo("Whole blocks=%d blkstart=%lu remaining=%lu\n", nblocks,
					   (unsigned long)blkstart, (unsigned long)remaining);
		...
		if (remaining > 0) {
			...
			spiffs_mtd_dump("Read trailing partial block", fs->mtd_work, blksize);
			...
			spiffs_mtd_dump("Write trailing partial block", fs->mtd_work, blksize);
...
ssize_t spiffs_mtd_read(FAR struct spiffs_s *fs, off_t offset, size_t len, FAR uint8_t *dest)
{
#ifdef CONFIG_SPIFFS_MTDDUMP
