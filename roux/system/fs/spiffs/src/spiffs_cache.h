...
#define SPIFFS_CACHE_PAGE_SIZE(fs) (sizeof(struct spiffs_cache_page_s) + SPIFFS_GEO_PAGE_SIZE(fs))

#define spiffs_get_cache(fs) ((FAR struct spiffs_cache_s *)((fs)->cache))

#define spiffs_get_cache_page_hdr(fs, c, cpndx)                                                    \
	((FAR struct spiffs_cache_page_s *)(&((c)->cpages[(cpndx)*SPIFFS_CACHE_PAGE_SIZE(fs)])))
