...
static ssize_t shmfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t shmfs_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int shmfs_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *entry);
static int shmfs_munmap(FAR struct task_group_s *group, FAR struct mm_map_entry_s *entry,
...
static int shmfs_map_object(FAR struct shmfs_object_s *object, FAR void **vaddr)
{
	int ret = OK;
	...
	mapaddr = (uintptr_t)vm_alloc_region(get_group_mm(group), 0, object->length);
	if (mapaddr == 0) {
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		vm_release_region(get_group_mm(group), (FAR void *)mapaddr, object->length);
	} else {
...
static int shmfs_add_map(FAR struct mm_map_entry_s *entry, FAR struct inode *inode)
{
...
static int shmfs_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *entry)
{
	...
	if (ret < 0 || (ret = shmfs_add_map(entry, filep->f_inode)) < 0) {
...
static int shmfs_unmap_area(FAR struct task_group_s *group, FAR void *vaddr, size_t length)
{
	int ret = OK;
