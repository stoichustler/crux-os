...
static int meminfo_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int meminfo_close(FAR struct file *filep);
#ifndef CONFIG_FS_PROCFS_EXCLUDE_MEMDUMP
static ssize_t memdump_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t memdump_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
#endif
static ssize_t meminfo_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static ssize_t meminfo_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	linesize =
		procfs_snprintf(procfile->line, MEMINFO_LINELEN, "%11s%11s%11s%11s%11s%7s%7s%s\n", "total",
						"used", "free", "maxused", "maxfree", "nused", "nfree", " name");
	...
	for (entry = g_procfs_meminfo; entry != NULL; entry = entry->next) {
		if (buflen > 0) {
			...
			linesize = procfs_snprintf(procfile->line, MEMINFO_LINELEN,
									   ...
									   (unsigned long)info.fordblks, (unsigned long)info.usmblks,
									   (unsigned long)info.mxordblk, (unsigned long)info.aordblks,
									   (unsigned long)info.ordblks, entry->name);
			copysize = procfs_memcpy(procfile->line, linesize, buffer, buflen, &offset);
			totalsize += copysize;
		}
	}
	...
	if (buflen > 0) {
		...
		linesize =
			procfs_snprintf(procfile->line, MEMINFO_LINELEN, "%11lu%11lu%11lu%11s%11lu %18s\n",
	...
	if (buflen > 0) {
		...
		linesize = procfs_snprintf(
			...
			(unsigned long)progmem.arena, (unsigned long)progmem.uordblks,
			(unsigned long)progmem.fordblks, (unsigned long)progmem.mxordblk,
			(unsigned long)progmem.aordblks, (unsigned long)progmem.ordblks, "Prog");
		copysize = procfs_memcpy(procfile->line, linesize, buffer, buflen, &offset);
		totalsize += copysize;
	}
#endif
...
static int meminfo_dup(FAR const struct file *oldp, FAR struct file *newp)
{
	...
	newattr = (FAR struct meminfo_file_s *)fs_heap_malloc(sizeof(struct meminfo_file_s));
	if (!newattr) {
		ferr("ERROR: Failed to allocate file attributes\n");
		return -ENOMEM;
	}
