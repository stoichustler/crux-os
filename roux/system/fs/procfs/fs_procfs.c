...
#if !defined(CONFIG_SCHED_CPULOAD_NONE) && !defined(CONFIG_FS_PROCFS_EXCLUDE_CPULOAD)
...
#ifdef CONFIG_FS_PROCFS_REGISTER
static const uint8_t g_base_entrycount = sizeof(g_base_entries) / sizeof(struct procfs_entry_s);

static FAR struct procfs_entry_s *g_procfs_entries;
static uint8_t g_procfs_entrycount;
#else
static const uint8_t g_procfs_entrycount = sizeof(g_procfs_entries) / sizeof(struct procfs_entry_s);
#endif
...
static int procfs_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int procfs_unbind(FAR void *handle, FAR struct inode **blkdriver, unsigned int flags);
...
static int procfs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	for (x = 0; x < g_procfs_entrycount; x++) {
		...
		if (fnmatch(g_procfs_entries[x].pathpattern, relpath, 0) == 0) {
			...
			DEBUGASSERT(g_procfs_entries[x].ops && g_procfs_entries[x].ops->open);

			ret = g_procfs_entries[x].ops->open(filep, relpath, oflags, mode);
			if (ret == OK) {
				DEBUGASSERT(filep->f_priv);

				((FAR struct procfs_file_s *)filep->f_priv)->procfsentry = &g_procfs_entries[x];
				break;
			}
		}
	}
						  ...
						  FAR struct fs_dirent_s **dir)
{
	...
	if (!relpath || relpath[0] == '\0') {
		...
		level0 = (FAR struct procfs_level0_s *)fs_heap_zalloc(sizeof(struct procfs_level0_s) +
															  sizeof(pid_t) * num);
		if (!level0) {
		...
		*dir = (FAR struct fs_dirent_s *)level0;
		...
		for (x = 0; x < g_procfs_entrycount; x++) {
			...
			/* Test for a sub-string match (e.g. "ls /proc/fs") */
				...
				level1 =
					(FAR struct procfs_level1_s *)fs_heap_zalloc(sizeof(struct procfs_level1_s));
				if (!level1) {
...
static int procfs_closedir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir)
{
	...
	if (dirpriv->procfsentry != NULL && dirpriv->procfsentry->ops->closedir != NULL) {
						  ...
						  FAR struct dirent *entry)
{
	...
	if (priv->level == 0) {
		...
		if (index >= priv->nentries) {
			...
			if (index < g_procfs_entrycount) {
				...
				if (pentry->type == PROCFS_DIR_TYPE || level0->lastlen != strlen(name)) {
	...
	/* Are we reading an intermediate subdirectory? */
		...
		if (level1->base.index < g_procfs_entrycount && level1->firstindex < g_procfs_entrycount &&
			strncmp(g_procfs_entries[level1->base.index].pathpattern,
					g_procfs_entries[level1->firstindex].pathpattern, level1->subdirlen) == 0) {
			/* This entry matches.  Report the subdir entry */

			name = &g_procfs_entries[level1->base.index].pathpattern[level1->subdirlen + 1];
	...
	if (!relpath || relpath[0] == '\0') {
		...
		for (x = 0; x < g_procfs_entrycount; x++) {
			...
			if (fnmatch(g_procfs_entries[x].pathpattern, relpath, 0) == 0) {
				...
				DEBUGASSERT(g_procfs_entries[x].ops && g_procfs_entries[x].ops->stat);
...
int procfs_initialize(void)
{
	...
	if (g_procfs_entries == NULL) {
		...
		g_procfs_entries = (FAR struct procfs_entry_s *)fs_heap_malloc(sizeof(g_base_entries));
		if (g_procfs_entries == NULL) {
			return -ENOMEM;
		}
...
int procfs_register(FAR const struct procfs_entry_s *entry)
{
	...
	newtable = (FAR struct procfs_entry_s *)fs_heap_realloc(g_procfs_entries, newsize);
	if (newtable != NULL) {
