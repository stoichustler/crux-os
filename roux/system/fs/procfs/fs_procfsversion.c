...
static int version_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	attr = (FAR struct version_file_s *)fs_heap_zalloc(sizeof(struct version_file_s));
...
static ssize_t version_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (filep->f_pos == 0) {
		uname(&name);
		linesize = procfs_snprintf(attr->line, VERSION_LINELEN, "%s version %s %s %s\n",
								   name.sysname, name.release, name.version, CONFIG_BASE_DEFCONFIG);
...
static int version_dup(FAR const struct file *oldp, FAR struct file *newp)
{
	...
	newattr = (FAR struct version_file_s *)fs_heap_malloc(sizeof(struct version_file_s));
