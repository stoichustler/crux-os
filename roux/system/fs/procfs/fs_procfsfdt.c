...
static int fdt_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int fdt_close(FAR struct file *filep);
static ssize_t fdt_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static ssize_t fdt_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	offset = filep->f_pos;
	ret = procfs_memcpy(fdt, be32toh(fdt_header->totalsize), buffer, buflen, &offset);
