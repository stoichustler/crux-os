...
static int pressure_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int pressure_close(FAR struct file *filep);
static ssize_t pressure_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t pressure_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int pressure_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
static int pressure_dup(FAR const struct file *oldp, FAR struct file *newp);
static int pressure_opendir(FAR const char *relpath, FAR struct fs_dirent_s **dir);
static int pressure_closedir(FAR struct fs_dirent_s *dir);
static int pressure_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry);
...
static int pressure_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup)
{
	...
	if (setup) {
		if (priv->fds == NULL) {
			...
			if (g_remaining <= priv->threshold &&
				(priv->lasttick == CLOCK_MAX || current - priv->lasttick >= priv->interval)) {
...
void mm_notify_pressure(size_t remaining, size_t largest)
{
	...
	{
		FAR struct pressure_file_s *pressure = container_of(entry, struct pressure_file_s, entry);
		...
		if (pressure->lasttick != CLOCK_MAX && current - pressure->lasttick < pressure->interval) {
			continue;
		}
