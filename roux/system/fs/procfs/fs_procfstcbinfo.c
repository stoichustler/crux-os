...
static int tcbinfo_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int tcbinfo_close(FAR struct file *filep);
static ssize_t tcbinfo_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static int tcbinfo_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	attr = (FAR struct tcbinfo_file_s *)fs_heap_zalloc(sizeof(struct tcbinfo_file_s));
...
nooptimiziation_function FAR static void *tcbinfo_running_regs(void)
{
	return running_regs();
}
...
static ssize_t tcbinfo_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (filep->f_pos == 0) {
		linesize =
			procfs_snprintf(attr->line, TCBINFO_LINELEN, "pointer %p size %zu current regs %p\n",
							&g_tcbinfo, sizeof(struct tcbinfo_s), tcbinfo_running_regs());
...
static int tcbinfo_dup(FAR const struct file *oldp, FAR struct file *newp)
{
	...
	newattr = (FAR struct tcbinfo_file_s *)fs_heap_malloc(sizeof(struct tcbinfo_file_s));
