...
static FAR const char *const g_policy[4] = {"SCHED_FIFO", "SCHED_RR", "SCHED_SPORADIC"};
...
static ssize_t proc_status(FAR struct proc_file_s *procfile, FAR struct tcb_s *tcb,
						   FAR char *buffer, size_t buflen, off_t offset);
static ssize_t proc_cmdline(FAR struct proc_file_s *procfile, FAR struct tcb_s *tcb,
							FAR char *buffer, size_t buflen, off_t offset);
#ifndef CONFIG_SCHED_CPULOAD_NONE
static ssize_t proc_loadavg(FAR struct proc_file_s *procfile, FAR struct tcb_s *tcb,
							FAR char *buffer, size_t buflen, off_t offset);
#endif
#ifdef CONFIG_SCHED_CRITMONITOR
static ssize_t proc_critmon(FAR struct proc_file_s *procfile, FAR struct tcb_s *tcb,
							FAR char *buffer, size_t buflen, off_t offset);
#endif
#if CONFIG_MM_BACKTRACE >= 0
static ssize_t proc_heap(FAR struct proc_file_s *procfile, FAR struct tcb_s *tcb, FAR char *buffer,
						 size_t buflen, off_t offset);
#endif
#ifdef CONFIG_DEBUG_MM
static ssize_t proc_heapcheck(FAR struct proc_file_s *procfile, FAR struct tcb_s *tcb,
							  FAR char *buffer, size_t buflen, off_t offset);
static ssize_t proc_heapcheck_write(FAR struct proc_file_s *procfile, FAR struct tcb_s *tcb,
									FAR const char *buffer, size_t buflen, off_t offset);
#endif
static ssize_t proc_stack(FAR struct proc_file_s *procfile, FAR struct tcb_s *tcb, FAR char *buffer,
						  size_t buflen, off_t offset);
static ssize_t proc_groupstatus(FAR struct proc_file_s *procfile, FAR struct tcb_s *tcb,
								FAR char *buffer, size_t buflen, off_t offset);
static ssize_t proc_groupfd(FAR struct proc_file_s *procfile, FAR struct tcb_s *tcb,
							FAR char *buffer, size_t buflen, off_t offset);
#if !defined(CONFIG_DISABLE_ENVIRON) && !defined(CONFIG_FS_PROCFS_EXCLUDE_ENVIRON)
static int proc_groupenv_callback(FAR void *arg, FAR const char *pair);
static ssize_t proc_groupenv(FAR struct proc_file_s *procfile, FAR struct tcb_s *tcb,
							 FAR char *buffer, size_t buflen, off_t offset);
#endif
...
static ssize_t proc_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int proc_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry);
...
static FAR const struct proc_node_s *proc_findnode(FAR const char *relpath)
{
	...
	for (i = 0; i < PROC_NNODES; i++) {
		...
		if (strncmp(g_nodeinfo[i]->relpath, relpath, len) == 0 &&
			(relpath[len] == '\0' || (relpath[len] == '/' && relpath[len + 1] == '\0' &&
						   ...
						   FAR char *buffer, size_t buflen, off_t offset)
{
	...
	linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, "%-12s%.18s\n", "Name:", name);
	...
	linesize = procfs_snprintf(
		procfile->line, STATUS_LINELEN, "%-12s%s\n",
	...
	linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, "%-12s%d\n",
							   "Group:", tcb->group ? tcb->group->tg_pid : -1);
	copysize = procfs_memcpy(procfile->line, linesize, buffer, remaining, &offset);
	...
	if (tcb->task_state >= FIRST_ASSIGNED_STATE && tcb->task_state <= LAST_ASSIGNED_STATE) {
		...
		linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, "%-12s---\n", "CPU:");
	}
	...
	linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, "%-12s%c\n",
							   "Flags:", tcb->flags & TCB_FLAG_EXIT_PROCESSING ? 'P' : '-');
	...
	linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, "%-12s%d (%d)\n",
	...
	/* Show the signal mask. Note: sigset_t is uint32_t on ROUX. */

	linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, "%-12s" SIGSET_FMT "\n",
							...
							FAR char *buffer, size_t buflen, off_t offset)
{
	...
	copysize = procfs_memcpy(procfile->line, linesize, buffer, sizeof(procfile->line), &offset);
	totalsize += copysize;
	buffer += copysize;
	remaining -= copysize;
							...
							FAR char *buffer, size_t buflen, off_t offset)
{
	...
	linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, "%3" PRId32 ".%01" PRId32 "%%\n",
							   intpart, fracpart);
							...
							FAR char *buffer, size_t buflen, off_t offset)
{
	...
	linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, "%lu.%09lu %p,",
							   (unsigned long)maxtime.tv_sec, (unsigned long)maxtime.tv_nsec,
	...
	linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, "%lu.%09lu,%lu.%09lu\n",
							   (unsigned long)maxtime.tv_sec, (unsigned long)maxtime.tv_nsec,
							   (unsigned long)runtime.tv_sec, (unsigned long)(runtime.tv_nsec));
	copysize = procfs_memcpy(procfile->line, linesize, buffer, remaining, &offset);
						 ...
						 size_t buflen, off_t offset)
{
	...
	linesize =
		procfs_snprintf(procfile->line, STATUS_LINELEN, "%-12s%d\n", "AllocSize:", info.uordblks);
	copysize = procfs_memcpy(procfile->line, linesize, buffer, remaining, &offset);
	...
	totalsize += procfs_memcpy(procfile->line, linesize, buffer, remaining, &offset);
	return totalsize;
}
#endif
							  ...
							  FAR char *buffer, size_t buflen, off_t offset)
{
	...
	linesize =
		procfs_snprintf(procfile->line, STATUS_LINELEN, "%-12s%zu\n", "HeapCheck:", heapcheck);
						  ...
						  size_t buflen, off_t offset)
{
	...
	linesize =
		procfs_snprintf(procfile->line, STATUS_LINELEN, "%-12s%zu\n",
						"StackMax: ", tcb->stack_base_ptr + tcb->adj_stack_size - tcb->sp_deepest);
	copysize = procfs_memcpy(procfile->line, linesize, buffer, remaining, &offset);
	...
	for (i = tcb->level_deepest - 1; i >= 0; i--) {
		linesize = procfs_snprintf(
			procfile->line, STATUS_LINELEN, "%-12zu%-pS\n",
			(i ? tcb->stackrecord_sp_deepest[i - 1] : tcb->stack_base_ptr + tcb->adj_stack_size) -
	...
	if (tcb->caller_deepest) {
		linesize = procfs_snprintf(procfile->line, STATUS_LINELEN,
								   ...
								   "Max: %u Overflow: %zu\n",
								   CONFIG_SCHED_STACK_RECORD, tcb->caller_deepest);
		copysize = procfs_memcpy(procfile->line, linesize, buffer, remaining, &offset);
		totalsize += copysize;
		buffer += copysize;
		remaining -= copysize;
								...
								FAR char *buffer, size_t buflen, off_t offset)
{
	...
	linesize =
		procfs_snprintf(procfile->line, STATUS_LINELEN, "%-12s0x%02x\n", "Flags:", group->tg_flags);
	copysize = procfs_memcpy(procfile->line, linesize, buffer, remaining, &offset);
	...
	linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, "Member IDs:");
	copysize = procfs_memcpy(procfile->line, linesize, buffer, remaining, &offset);
	...
	sq_for_every_safe(&group->tg_members, curr, next)
	{
		...
		linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, " %d", tcb->pid);
		copysize = procfs_memcpy(procfile->line, linesize, buffer, remaining, &offset);
							...
							FAR char *buffer, size_t buflen, off_t offset)
{
	...
	for (i = 0; i < count; i++) {
		...
		linesize = procfs_snprintf(procfile->line, STATUS_LINELEN, "%-3d %-7d %-4x %-9ld %-14s ", i,
								   filep->f_oflags, INODE_GET_TYPE(filep->f_inode),
								   (long)filep->f_pos, path);
		if (linesize < STATUS_LINELEN) {
#if CONFIG_FS_BACKTRACE > 0
			linesize += backtrace_format(procfile->line + linesize, STATUS_LINELEN - linesize,
										 fdp->f_backtrace, CONFIG_FS_BACKTRACE);
#endif
...
static int proc_groupenv_callback(FAR void *arg, FAR const char *pair)
{
	...
	*dest = '\0';
	...
	linesize = procfs_snprintf(info->procfile->line, STATUS_LINELEN, "%s=%s\n", name, value);
	copysize =
		procfs_memcpy(info->procfile->line, linesize, info->buffer, info->remaining, &info->offset);
...
static int proc_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	procfile = (FAR struct proc_file_s *)fs_heap_zalloc(sizeof(struct proc_file_s));
	if (procfile == NULL) {
...
static ssize_t proc_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	case PROC_HEAP_CHECK:
		ret = proc_heapcheck_write(procfile, tcb, buffer, buflen, filep->f_pos);
		break;
#endif
