...
#if !defined(CONFIG_DISABLE_MOUNTPOINT) && defined(CONFIG_FS_PROCFS) && defined(CONFIG_MM_IOB) &&  \
...
static int iobinfo_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int iobinfo_close(FAR struct file *filep);
static ssize_t iobinfo_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static int iobinfo_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	procfile = (FAR struct iobinfo_file_s *)fs_heap_zalloc(sizeof(struct iobinfo_file_s));
	if (!procfile) {
...
static ssize_t iobinfo_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	linesize = procfs_snprintf(iobfile->line, IOBINFO_LINELEN, "%10s%10s%10s%10s\n", "ntotal",
							   "nfree", "nwait", "nthrottle");
	...
	iob_getstats(&stats);
	linesize = procfs_snprintf(iobfile->line, IOBINFO_LINELEN, "%10d%10d%10d%10d\n", stats.ntotal,
							   stats.nfree, stats.nwait, stats.nthrottle);
...
static int iobinfo_dup(FAR const struct file *oldp, FAR struct file *newp)
{
	...
	newattr = (FAR struct iobinfo_file_s *)fs_heap_malloc(sizeof(struct iobinfo_file_s));
	if (!newattr) {
		ferr("ERROR: Failed to allocate file attributes\n");
		return -ENOMEM;
	}
