...
static ssize_t uptime_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (filep->f_pos == 0) {
		...
		linesize = procfs_snprintf(attr->line, UPTIME_LINELEN, "%10.2f\n", now);
		...
		linesize = procfs_snprintf(attr->line, UPTIME_LINELEN, "%7" PRIu64 ".%02u\n", sec, csec);
#else
		linesize = procfs_snprintf(attr->line, UPTIME_LINELEN, "%7" PRIu32 ".%02u\n", sec, csec);
#endif
