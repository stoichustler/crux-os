...
#if !defined(CONFIG_FS_PROCFS_EXCLUDE_MOUNT) || !defined(CONFIG_FS_PROCFS_EXCLUDE_BLOCKS) ||       \
...
static void mount_sprintf(FAR struct mount_info_s *info, FAR const char *fmt, ...)
	printf_like(2, 3);
#ifndef CONFIG_FS_PROCFS_EXCLUDE_MOUNT
static int mount_entry(FAR const char *mountpoint, FAR struct statfs *statbuf, FAR void *arg);
#endif
#ifndef CONFIG_FS_PROCFS_EXCLUDE_BLOCKS
static int blocks_entry(FAR const char *mountpoint, FAR struct statfs *statbuf, FAR void *arg);
#endif
#ifndef CONFIG_FS_PROCFS_EXCLUDE_USAGE
static int usage_entry(FAR const char *mountpoint, FAR struct statfs *statbuf, FAR void *arg);
#endif

/* File system methods */

static int mount_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int mount_close(FAR struct file *filep);
static ssize_t mount_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static void mount_sprintf(FAR struct mount_info_s *info, FAR const char *fmt, ...)
{
	...
	copysize = procfs_memcpy(info->line, linesize, info->buffer, info->remaining, &info->offset);
...
static int blocks_entry(FAR const char *mountpoint, FAR struct statfs *statbuf, FAR void *arg)
{
	...
	mount_sprintf(info, "%6zu %10" PRIuOFF " %10" PRIuOFF "  %10" PRIuOFF " %s\n", statbuf->f_bsize,
				  statbuf->f_blocks, statbuf->f_blocks - statbuf->f_bavail, statbuf->f_bavail,
				  mountpoint);
...
static int usage_entry(FAR const char *mountpoint, FAR struct statfs *statbuf, FAR void *arg)
{
	...
	mount_sprintf(info, "  %-10s %8" PRIuOFF "%c %8" PRIuOFF "%c  %8" PRIuOFF "%c %s\n", fstype,
				  size, sizelabel, used, usedlabel, free, freelabel, mountpoint);
...
static int mount_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	procfile = (FAR struct mount_file_s *)fs_heap_zalloc(sizeof(struct mount_file_s));
	if (!procfile) {
...
static int mount_dup(FAR const struct file *oldp, FAR struct file *newp)
{
	...
	newfile = (FAR struct mount_file_s *)fs_heap_malloc(sizeof(struct mount_file_s));
	if (!newfile) {
		ferr("ERROR: Failed to allocate file container\n");
		return -ENOMEM;
	}
