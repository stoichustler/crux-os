...
static int automount_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
#endif /* CONFIG_FS_AUTOMOUNTER_DRIVER */
...
static int automount_interrupt(FAR const struct automount_lower_s *lower, FAR void *arg,
							   bool inserted);
...
static int automount_close(FAR struct file *filep)
{
	...
	for (prev = NULL, curr = priv->ao_open; curr != NULL && curr != opriv;
...
static int automount_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case FIOC_NOTIFY: {
		FAR struct automount_notify_s *notify = (FAR struct automount_notify_s *)((uintptr_t)arg);
...
static void automount_mount(FAR struct automounter_state_s *priv)
{
	...
	case OK_NOENT:
		...
		ret = nx_mount(lower->blockdev, lower->mountpoint, lower->fstype, 0, NULL);
		if (ret < 0) {
	...
	case OK_EXIST:
		...
		if (ret < 0) {
			...
			if (ret == -EBUSY) {
				...
				ret = wd_start(&priv->wdog, lower->udelay, automount_timeout, (wdparm_t)priv);
				if (ret < 0) {
...
FAR void *automount_initialize(FAR const struct automount_lower_s *lower)
{
	...
	snprintf(devpath, PATH_MAX, CONFIG_FS_AUTOMOUNTER_VFS_PATH "%s", lower->mountpoint);
