...
#if defined(CONFIG_FS_NXFFS) || defined(CONFIG_FS_BINFS) || defined(CONFIG_FS_PROCFS) ||           \
	defined(CONFIG_NFS) || defined(CONFIG_FS_TMPFS) || defined(CONFIG_FS_USERFS) ||                \
	defined(CONFIG_FS_CROMFS) || defined(CONFIG_FS_UNIONFS) || defined(CONFIG_FS_HOSTFS) ||        \
	defined(CONFIG_FS_ZIPFS) || defined(CONFIG_FS_RPMSGFS) || defined(CONFIG_FS_V9FS)
...
static FAR const struct mountpt_operations *mount_findfs(FAR const struct fsmap_t *fstab,
														 FAR const char *filesystemtype)
{
...
int mount(FAR const char *source, FAR const char *target, FAR const char *filesystemtype,
		  unsigned long mountflags, FAR const void *data)
{
	int ret;
