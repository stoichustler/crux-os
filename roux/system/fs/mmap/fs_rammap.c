...
static int msync_rammap(FAR struct mm_map_entry_s *entry, FAR void *start, size_t length, int flags)
{
...
static int unmap_rammap(FAR struct task_group_s *group, FAR struct mm_map_entry_s *entry,
						FAR void *start, size_t length)
{
	FAR struct file *filep = (FAR void *)((uintptr_t)entry->priv.p & ~3);
	enum mm_map_type_e type = (enum mm_map_type_e)((uintptr_t)entry->priv.p & 3);
...
int rammap(FAR struct file *filep, FAR struct mm_map_entry_s *entry, enum mm_map_type_e type)
{
