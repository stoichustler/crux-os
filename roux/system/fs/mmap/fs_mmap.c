...
static int file_mmap_(FAR struct file *filep, FAR void *start, size_t length, int prot, int flags,
					  off_t offset, enum mm_map_type_e type, FAR void **mapped)
{
	int ret = -ENOTTY;
	...
	struct mm_map_entry_s entry = {
		NULL,										/* sq_entry_t */
		start, length, offset, prot, flags, {NULL}, /* priv.p */
		NULL										/* munmap */
	};
	...
	if ((flags & MAP_SHARED) && (filep->f_oflags & O_WROK) == 0 && prot == PROT_WRITE) {
...
{
	return file_mmap_(filep, start, length, prot, flags, offset, MAP_KERNEL, mapped);
}
 ...
 *
 * Description:
 *   ROUX operates in a flat open address space.  Therefore, it generally
...
FAR void *mmap(FAR void *start, size_t length, int prot, int flags, int fd, off_t offset)
{
	...
	ret = file_mmap_(filep, start, length, prot, flags, offset, MAP_USER, &mapped);
	if (filep) {
		file_put(filep);
	}
