...
static int unmap_anonymous(FAR struct task_group_s *group, FAR struct mm_map_entry_s *entry,
...
int map_anonymous(FAR struct mm_map_entry_s *entry, bool kernel)
{
	...
	entry->vaddr = kernel ? fs_heap_zalloc(entry->length) : kumm_zalloc(entry->length);
	if (entry->vaddr == NULL) {
