...
static int virtio_9p_create(FAR struct v9fs_transport_s **transport, FAR const char *args);
...
static void virtio_9p_destroy(FAR struct v9fs_transport_s *transport)
{
	FAR struct virtio_9p_priv_s *priv = container_of(transport, struct virtio_9p_priv_s, transport);
							 ...
							 FAR struct v9fs_payload_s *payload)
{
	...
	ret = virtqueue_add_buffer(vq, vb, payload->wcount, payload->rcount, payload);
	if (ret < 0) {
...
static int virtio_9p_probe(FAR struct virtio_device *vdev)
{
	FAR struct virtio_9p_priv_s *priv = container_of(vdev->priv, struct virtio_9p_priv_s, vdrv);
	...
	virtio_read_config_member(vdev, struct virtio_9p_config_s, tag_len, &config.tag_len);
	virtio_read_config(vdev, offsetof(struct virtio_9p_config_s, tag), &config.tag, config.tag_len);
