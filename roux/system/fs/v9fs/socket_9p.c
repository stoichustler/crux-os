...
static int socket_9p_create(FAR struct v9fs_transport_s **transport, FAR const char *args);
...
static int socket_9p_create(FAR struct v9fs_transport_s **transport, FAR const char *args)
{
	...
	ret = psock_connect(&priv->psock, (FAR const struct sockaddr *)&sin, sizeof(sin));
	if (ret < 0) {
		goto out;
	}
...
static int socket_9p_request(FAR struct v9fs_transport_s *transport,
							 FAR struct v9fs_payload_s *payload)
{
	FAR struct socket_9p_priv_s *priv = (FAR struct socket_9p_priv_s *)transport;
	struct msghdr msg;
	FAR char *ptr;
	size_t len;
	int ret;
	...
	ret = psock_recvfrom(&priv->psock, ptr, V9FS_HEADER_OFFSET, MSG_WAITALL, NULL, NULL);
	if (ret < 0) {
		goto out;
	}
	...
	if (len > 0) {
		...
		for (index = 0; index < payload->rcount; index++) {
			payload->riov[index].iov_len = MIN(len, payload->riov[index].iov_len);
