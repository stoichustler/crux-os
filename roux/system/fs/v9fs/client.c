...
#define V9FS_IOHDRSZ                                                                               \
	24 /* Reserve enough space for                                                                 \
		* TWRITE/TREAD                                                                             \
		*/
#define V9FS_READDIRHDRSZ                                                                          \
	24				   /* Reserve enough space for                                                 \
						* READDIR                                                                  \
						*/
...
#define V9FS_DEFAULT_VERSION                                                                       \
	"9P2000.L" /* Current implementations are                                                      \
...
static uint32_t v9fs_map_setattr_flags(int flags)
{
	...
	static const struct v9fs_map_s flags_map[] = {
		{CH_STAT_MODE, V9FS_SETATTR_MODE},		{CH_STAT_UID, V9FS_SETATTR_UID},
		{CH_STAT_GID, V9FS_SETATTR_GID},		{CH_STAT_SIZE, V9FS_SETATTR_SIZE},
		{CH_STAT_ATIME, V9FS_SETATTR_ATIMESET}, {CH_STAT_MTIME, V9FS_SETATTR_MTIMESET},
	};
...
static uint32_t v9fs_map_open_flags(int flags)
{
	...
	size_t i;
	static const struct v9fs_map_s flags_map[] = {
		{O_CREAT, V9FS_CREATE},		 {O_NOCTTY, V9FS_NOCTTY},	  {O_TRUNC, V9FS_TRUNC},
		{O_APPEND, V9FS_APPEND},	 {O_NONBLOCK, V9FS_NONBLOCK}, {O_DSYNC, V9FS_DSYNC},
		{FASYNC, V9FS_FASYNC},		 {O_DIRECT, V9FS_DIRECT},	  {O_DIRECTORY, V9FS_DIRECTORY},
		{O_NOFOLLOW, V9FS_NOFOLLOW}, {O_NOATIME, V9FS_NOATIME},	  {O_CLOEXEC, V9FS_CLOEXEC},
		{O_EXCL, V9FS_EXCL},		 {O_SYNC, V9FS_SYNC},
	};
...
static int v9fs_fid_create(FAR struct v9fs_client_s *client, FAR const char *relpath)
{
...
static void v9fs_fid_destroy(FAR struct v9fs_client_s *client, uint32_t fid)
{
...
static int v9fs_client_rpc(FAR struct v9fs_transport_s *transport, FAR struct iovec *wiov,
						   size_t wcount, FAR struct iovec *riov, size_t rcount, uint16_t tag)
{
...
static int v9fs_client_clunk(FAR struct v9fs_client_s *client, uint32_t fid)
{
	...
	ret = v9fs_client_rpc(client->transport, wiov, 1, riov, 1, request.header.tag);
...
static int v9fs_client_attach(FAR struct v9fs_client_s *client, uint32_t fid, FAR const char *uname,
							  FAR const char *aname)
{
	...
	newfid = ret;
	request.header.size = V9FS_HDRSZ + V9FS_BIT32SZ * 3 + V9FS_BIT16SZ * 2 + uname_len + aname_len;
	...
	wiov[0].iov_base = &request;
	wiov[0].iov_len = V9FS_HDRSZ + V9FS_BIT32SZ * 3 + V9FS_BIT16SZ * 2 + uname_len + aname_len;
	riov[0].iov_base = &response;
...
int v9fs_client_statfs(FAR struct v9fs_client_s *client, FAR struct statfs *buf)
{
...
int v9fs_client_stat(FAR struct v9fs_client_s *client, uint32_t fid, FAR struct stat *buf)
{
...
int v9fs_client_chstat(FAR struct v9fs_client_s *client, uint32_t fid, FAR const struct stat *buf,
					   int flags)
{
	...
	return v9fs_client_rpc(client->transport, wiov, 1, riov, 1, request.header.tag);
}
...
ssize_t v9fs_client_read(FAR struct v9fs_client_s *client, uint32_t fid, FAR void *buffer,
						 off_t offset, size_t buflen)
{
	...
	while (buflen > 0) {
		request.header.size = V9FS_HDRSZ + V9FS_BIT32SZ + V9FS_BIT64SZ + V9FS_BIT32SZ;
		...
		ret = v9fs_client_rpc(client->transport, wiov, 1, riov, 2, request.header.tag);
		if (ret < 0) {
			break;
		}
...
ssize_t v9fs_client_readdir(FAR struct v9fs_client_s *client, uint32_t fid, FAR void *buffer,
							off_t offset, size_t buflen)
{
	FAR struct v9fs_fid_s *fidp;
...
ssize_t v9fs_client_convertdir(FAR const uint8_t *buffer, size_t bufsize, off_t head,
							   FAR off_t *offset, FAR struct dirent *entry)
{
	uint64_t off;
	uint16_t name_len;
	off_t next = head;
...
ssize_t v9fs_client_write(FAR struct v9fs_client_s *client, uint32_t fid, FAR const void *buffer,
						  off_t offset, size_t buflen)
{
	FAR struct v9fs_fid_s *fidp;
	...
	while (buflen > 0) {
		request.count = buflen > fidp->iounit ? fidp->iounit : buflen;
		request.header.size =
			V9FS_HDRSZ + V9FS_BIT32SZ + V9FS_BIT64SZ + V9FS_BIT32SZ + request.count;
		...
		ret = v9fs_client_rpc(client->transport, wiov, 2, riov, 1, request.header.tag);
		if (ret < 0) {
			break;
		}
...
int v9fs_client_rename(FAR struct v9fs_client_s *client, uint32_t fid, uint32_t newfid,
					   FAR const char *name)
{
	...
	request.header.size = V9FS_HDRSZ + V9FS_BIT32SZ * 2 + V9FS_BIT16SZ + request.name_len;
	request.header.type = V9FS_TRENAME;
	request.header.tag = v9fs_get_tagid(client);
	memcpy(&request.buffer, name, request.name_len);

	wiov[0].iov_base = &request;
	wiov[0].iov_len = V9FS_HDRSZ + V9FS_BIT32SZ * 2 + V9FS_BIT16SZ + request.name_len;
	riov[0].iov_base = &response;
	riov[0].iov_len = V9FS_HDRSZ + V9FS_BIT32SZ;
...
int v9fs_client_unlink(FAR struct v9fs_client_s *client, uint32_t fid, FAR const char *name,
					   bool isdir)
{
...
int v9fs_client_mkdir(FAR struct v9fs_client_s *client, uint32_t fid, FAR const char *name,
					  int mode)
{
	...
	request.fid = fid;
	request.name_len = strlen(name);
	request.header.size = V9FS_HDRSZ + V9FS_BIT32SZ * 3 + V9FS_BIT16SZ + request.name_len;
	...
	wiov[0].iov_base = &request;
	wiov[0].iov_len = V9FS_HDRSZ + V9FS_BIT32SZ * 3 + V9FS_BIT16SZ + request.name_len;
	riov[0].iov_base = &response;
	riov[0].iov_len = V9FS_HDRSZ + V9FS_QIDSZ;
...
int v9fs_client_create(FAR struct v9fs_client_s *client, uint32_t fid, FAR const char *name,
					   int oflags, int mode)
{
	FAR struct v9fs_fid_s *fidp;
	...
	request.fid = fid;
	request.name_len = strlen(name);
	request.header.size = V9FS_HDRSZ + V9FS_BIT32SZ * 4 + V9FS_BIT16SZ + request.name_len;
	...
	wiov[0].iov_base = &request;
	wiov[0].iov_len = V9FS_HDRSZ + V9FS_BIT32SZ * 4 + V9FS_BIT16SZ + request.name_len;
	riov[0].iov_base = &response;
...
int v9fs_client_open(FAR struct v9fs_client_s *client, uint32_t fid, int oflags)
{
	FAR struct v9fs_fid_s *fidp;
...
int v9fs_client_getname(FAR struct v9fs_client_s *client, uint32_t fid, FAR char *path)
{
	FAR struct v9fs_fid_s *fidp;
					 ...
					 FAR const char **childname)
{
	...
	newfid = ret;
	request.header.size = V9FS_HDRSZ + V9FS_BIT32SZ * 2 + V9FS_BIT16SZ + total_len;
	...
	ret = v9fs_client_rpc(client->transport, wiov, total_len ? 2 : 1, riov, 2, request.header.tag);
	if (ret < 0) {
		v9fs_fid_destroy(client, newfid);
		goto err;
	}
