...
static FAR const char *unionfs_offsetpath(FAR const char *relpath, FAR const char *prefix);
static bool unionfs_ispartprefix(FAR const char *partprefix, FAR const char *prefix);
static int unionfs_tryopen(FAR struct file *filep, FAR const char *relpath, FAR const char *prefix,
						   int oflags, mode_t mode);
static int unionfs_tryopendir(FAR struct inode *inode, FAR const char *relpath,
							  FAR const char *prefix, FAR struct fs_dirent_s **dir);
...
static int unionfs_tryunlink(FAR struct inode *inode, FAR const char *relpath,
							 FAR const char *prefix);
static int unionfs_tryrename(FAR struct inode *mountpt, FAR const char *oldrelpath,
							 FAR const char *newrelpath, FAR const char *prefix);
static int unionfs_trystat(FAR struct inode *inode, FAR const char *relpath, FAR const char *prefix,
						   FAR struct stat *buf);
static int unionfs_trychstat(FAR struct inode *inode, FAR const char *relpath,
							 FAR const char *prefix, FAR const struct stat *buf, int flags);
static int unionfs_trystatdir(FAR struct inode *inode, FAR const char *relpath,
							  FAR const char *prefix);
static int unionfs_trystatfile(FAR struct inode *inode, FAR const char *relpath,
							   FAR const char *prefix);
...
static int unionfs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int unionfs_close(FAR struct file *filep);
static ssize_t unionfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t unionfs_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int unionfs_fchstat(FAR const struct file *filep, FAR const struct stat *buf, int flags);
...
static int unionfs_closedir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir);
static int unionfs_readdir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir,
						   FAR struct dirent *entry);
static int unionfs_rewinddir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir);

static int unionfs_bind(FAR struct inode *blkdriver, FAR const void *data, FAR void **handle);
static int unionfs_unbind(FAR void *handle, FAR struct inode **blkdriver, unsigned int flags);
static int unionfs_statfs(FAR struct inode *mountpt, FAR struct statfs *buf);

/* Operations on paths */

static int unionfs_unlink(FAR struct inode *mountpt, FAR const char *relpath);
static int unionfs_mkdir(FAR struct inode *mountpt, FAR const char *relpath, mode_t mode);
static int unionfs_rmdir(FAR struct inode *mountpt, FAR const char *relpath);
static int unionfs_rename(FAR struct inode *mountpt, FAR const char *oldrelpath,
						  FAR const char *newrelpath);
static int unionfs_stat(FAR struct inode *mountpt, FAR const char *relpath, FAR struct stat *buf);
...
static int unionfs_dobind(FAR const char *fspath1, FAR const char *prefix1, FAR const char *fspath2,
...
static void unionfs_destroy(FAR struct unionfs_inode_s *ui)
{
	DEBUGASSERT(ui != NULL && ui->ui_fs[0].um_node != NULL && ui->ui_fs[1].um_node != NULL &&
				ui->ui_nopen == 0);
...
static int unionfs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	uf = (FAR struct unionfs_file_s *)fs_heap_zalloc(sizeof(struct unionfs_file_s));
	if (uf == NULL) {
		ret = -ENOMEM;
...
static int unionfs_dup(FAR const struct file *oldp, FAR struct file *newp)
{
	...
	newpriv = (FAR struct unionfs_file_s *)fs_heap_malloc(sizeof(struct unionfs_file_s));
	if (newpriv != NULL) {
						   ...
						   FAR struct fs_dirent_s **dir)
{
	...
	um = &ui->ui_fs[1];
	ret = unionfs_tryopendir(um->um_node, relpath, um->um_prefix, &udir->fu_lower[1]);
	if (ret >= 0) {
	...
	um = &ui->ui_fs[0];
	ret = unionfs_tryopendir(um->um_node, relpath, um->um_prefix, &udir->fu_lower[0]);
	if (ret >= 0) {
						   ...
						   FAR struct dirent *entry)
{
	...
	if (udir->fu_prefix[udir->fu_ndx]) {
		DEBUGASSERT(udir->fu_lower[udir->fu_ndx] == NULL && um->um_prefix != NULL);
		...
		if (udir->fu_ndx == 0 && (udir->fu_prefix[1] || udir->fu_lower[1] != NULL)) {
	...
	if (ops->readdir != NULL) {
		...
		do {
			...
			ret = ops->readdir(um->um_node, udir->fu_lower[udir->fu_ndx], entry);
			...
			if (ret == -ENOENT && udir->fu_ndx == 0) {
				...
				if (udir->fu_prefix[1]) {
					...
					if (relpath) {
						...
						um0 = &ui->ui_fs[0];
						tmp = unionfs_trystat(um0->um_node, relpath, um0->um_prefix, &buf);
						  ...
						  FAR const char *newrelpath)
{
	...
	if (tmp >= 0) {
		...
		ret = unionfs_tryrename(um->um_node, oldrelpath, newrelpath, um->um_prefix);
		if (ret >= 0) {
						  ...
						  FAR const char *prefix2, FAR void **handle)
{
	...
	ui = (FAR struct unionfs_inode_s *)fs_heap_zalloc(sizeof(struct unionfs_inode_s));
	if (!ui) {
