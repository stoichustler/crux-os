...
static inline int romfs_checkentry(FAR struct romfs_mountpt_s *rm, uint32_t offset,
								   ...
								   FAR struct romfs_nodeinfo_s *nodeinfo)
{
	...
	if (strlen(name) == entrylen && memcmp(entryname, name, entrylen) == 0) {
...
static int16_t romfs_devcacheread(FAR struct romfs_mountpt_s *rm, uint32_t offset)
{
	uint32_t sector;
	int ret;
...
static int romfs_followhardlinks(FAR struct romfs_mountpt_s *rm, uint32_t offset,
								 FAR uint32_t *poffset)
{
	uint32_t next;
	int16_t ndx;
	int i;
...
static inline int romfs_searchdir(FAR struct romfs_mountpt_s *rm, FAR const char *entryname,
								  int entrylen, FAR struct romfs_nodeinfo_s *nodeinfo)
{
#ifdef CONFIG_FS_ROMFS_CACHE_NODE
	...
	cnodeinfo = bsearch(&entry, nodeinfo->rn_child, nodeinfo->rn_count, sizeof(*nodeinfo->rn_child),
...
static FAR struct romfs_sparenode_s *romfs_alloc_sparenode(uint32_t start, uint32_t end)
{
...
static int romfs_init_sparelist(FAR struct romfs_mountpt_s *rm, bool rw)
{
	...
	node = romfs_alloc_sparenode(0, rm->rm_hwsectorsize * rm->rm_hwnsectors);
	if (node == NULL) {
		return -ENOMEM;
	}
...
static int romfs_alloc_spareregion(FAR struct list_node *list, uint32_t start, uint32_t end)
{
	FAR struct romfs_sparenode_s *node;
...
static void romfs_devmemcpy(FAR struct romfs_mountpt_s *rm, int ndx, FAR const void *buf,
							size_t len)
{
...
static void romfs_devstrcpy(FAR struct romfs_mountpt_s *rm, int ndx, FAR const char *buf)
{
...
static void romfs_devwrite32(FAR struct romfs_mountpt_s *rm, int ndx, uint32_t value)
{
...
static int romfs_hwwrite(FAR struct romfs_mountpt_s *rm, FAR uint8_t *buffer, uint32_t sector,
...
static int romfs_devcachewrite(FAR struct romfs_mountpt_s *rm, uint32_t sector)
{
	int ret;
...
#ifdef CONFIG_FS_ROMFS_CACHE_NODE
static int romfs_cachenode(FAR struct romfs_mountpt_s *rm, uint32_t origoffset, uint32_t offset,
						   uint32_t next, uint32_t size, FAR const char *name,
						   FAR struct romfs_nodeinfo_s **pnodeinfo)
{
	...
	*pnodeinfo = nodeinfo;
	...
	if (!list_is_empty(&rm->rm_sparelist)) {
		...
		ret = romfs_alloc_spareregion(&rm->rm_sparelist, origoffset, origoffset + totalsize);
		if (ret < 0) {
			return ret;
		}
	}
	...
	if (!IS_DIRECTORY(next) || (strcmp(name, ".") == 0) || (strcmp(name, "..") == 0)) {
		nodeinfo->rn_size = size;
		return 0;
	}
	...
	do {
		...
		ret = romfs_parsedirentry(rm, origoffset, &offset, &next, &info, &size);
		if (ret < 0) {
			return ret;
		}
		...
		if (child == NULL || nodeinfo->rn_count == count - 1) {
			...
			memset(nodeinfo->rn_child + count, 0, NODEINFO_NINCR * sizeof(*nodeinfo->rn_child));
		...
		ret = romfs_cachenode(rm, origoffset, offset, next, size, childname, child);
		if (ret < 0) {
			nodeinfo->rn_count--;
			return ret;
		}
	...
	if (nodeinfo->rn_count > 1) {
		qsort(nodeinfo->rn_child, nodeinfo->rn_count, sizeof(*nodeinfo->rn_child),
...
int romfs_hwread(FAR struct romfs_mountpt_s *rm, FAR uint8_t *buffer, uint32_t sector,
				 unsigned int nsectors)
{
	int ret = 0;
	...
	if (rm->rm_xipbase) {
		...
		memcpy(buffer, rm->rm_xipbase + sector * rm->rm_hwsectorsize,
			   nsectors * rm->rm_hwsectorsize);
	} else {
		/* In non-XIP mode, we have to read the data from the device */

		FAR struct inode *inode = rm->rm_blkdriver;
		ssize_t nsectorsread = inode->u.i_bops->read(inode, buffer, sector, nsectors);
...
int romfs_filecacheread(FAR struct romfs_mountpt_s *rm, FAR struct romfs_file_s *rf,
						uint32_t sector)
{
	int ret;
	...
	finfo("sector: %" PRId32 " cached: %" PRId32 " ncached: %" PRId32 ""
		  ...
		  sector, rf->rf_cachesector, rf->rf_ncachesector, rm->rm_hwsectorsize, rm->rm_xipbase,
		  rf->rf_buffer);
	...
	if (rf->rf_cachesector > sector || rf->rf_cachesector + rf->rf_ncachesector <= sector) {
		/* Check the access mode */
...
#ifdef CONFIG_FS_ROMFS_CACHE_NODE
	ret = romfs_cachenode(rm, 0, rootoffset, RFNEXT_DIRECTORY, 0, "", &rm->rm_root);
	if (ret < 0) {
#ifdef CONFIG_FS_ROMFS_WRITEABLE
...
int romfs_fileconfigure(FAR struct romfs_mountpt_s *rm, FAR struct romfs_file_s *rf)
{
	...
	if (rm->rm_xipbase) {
		...
		rf->rf_buffer = fs_heap_malloc(rm->rm_hwsectorsize * rf->rf_ncachesector);
		if (!rf->rf_buffer) {
			return -ENOMEM;
		}
	}
...
int romfs_finddirentry(FAR struct romfs_mountpt_s *rm, FAR struct romfs_nodeinfo_s *nodeinfo,
					   FAR const char *path)
{
...
int romfs_parsedirentry(FAR struct romfs_mountpt_s *rm, uint32_t offset, FAR uint32_t *poffset,
						uint32_t *pnext, FAR uint32_t *pinfo, FAR uint32_t *psize)
{
	uint32_t save;
	uint32_t next;
	int16_t ndx;
...
int romfs_parsefilename(FAR struct romfs_mountpt_s *rm, uint32_t offset, FAR char *pname)
{
	int16_t ndx;
	uint16_t namelen = 0;
	uint16_t chunklen;
...
int romfs_datastart(FAR struct romfs_mountpt_s *rm, FAR struct romfs_nodeinfo_s *nodeinfo,
					FAR uint32_t *start)
{
#ifdef CONFIG_FS_ROMFS_CACHE_NODE
	*start = ROMFS_ALIGNUP(nodeinfo->rn_offset + ROMFS_FHDR_NAME + nodeinfo->rn_namesize + 1);
	return 0;
#else
