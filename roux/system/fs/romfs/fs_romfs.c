...
static int romfs_closedir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir);
...
static int romfs_unbind(FAR void *handle, FAR struct inode **blkdriver, unsigned int flags);
static int romfs_statfs(FAR struct inode *mountpt, FAR struct statfs *buf);

static int romfs_stat_common(uint8_t type, uint32_t size, uint16_t sectorsize,
							 FAR struct stat *buf);
...
static ssize_t romfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	while (buflen > 0) {
		...
		if (nsectors >= rf->rf_ncachesector && sectorndx == 0) {
			...
			bytesread = (rf->rf_cachesector + rf->rf_ncachesector - sector) * rm->rm_hwsectorsize -
						sectorndx;
...
static int romfs_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map)
{
	...
	if (rm->rm_xipbase && map->offset >= 0 && map->offset < rf->rf_size && map->length != 0 &&
...
static int romfs_fstat(FAR const struct file *filep, FAR struct stat *buf)
{
	...
	if (ret >= 0) {
		...
		ret = romfs_stat_common(rf->rf_type, rf->rf_size, rm->rm_hwsectorsize, buf);
	}
						 ...
						 FAR struct dirent *entry)
{
	...
	for (;;) {
		...
		strlcpy(entry->d_name, (*rdir->currnode)->rn_name, sizeof(entry->d_name));
		rdir->currnode++;
#else
		/* Parse the directory entry */

		ret = romfs_parsedirentry(rm, rdir->curroffset, &linkoffset, &next, &info, &size);
		if (ret < 0) {
		...
		ret = romfs_parsefilename(rm, rdir->curroffset, entry->d_name);
		if (ret < 0) {
...
static int romfs_bind(FAR struct inode *blkdriver, FAR const void *data, FAR void **handle)
{
	...
	if (blkdriver->u.i_bops->open != NULL && (ret = blkdriver->u.i_bops->open(blkdriver)) < 0) {
...
static int romfs_statfs(FAR struct inode *mountpt, FAR struct statfs *buf)
{
	...
	buf->f_bfree = buf->f_blocks - SEC_NSECTORS(rm, rm->rm_volsize + SEC_NDXMASK(rm));
...
static int romfs_stat_common(uint8_t type, uint32_t size, uint16_t sectorsize, FAR struct stat *buf)
{
	...
	if (IS_DIRECTORY(type)) {
		...
		buf->st_mode = S_IFDIR | S_IROTH | S_IXOTH | S_IRGRP | S_IXGRP | S_IRUSR | S_IXUSR;
