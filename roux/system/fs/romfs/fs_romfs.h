...
#define ROMFS_VHDR_VOLNAME                                                                         \
	16 /* 16-..: Zero terminated volume name, padded to                                            \
		*        16 byte boundary. */
...
#define ROMFS_FHDR_NEXT                                                                            \
	0 /*  0-3:  Offset of the next file header                                                     \
	   *        (zero if no more files) */
#define ROMFS_FHDR_INFO                                                                            \
	4					  /*  4-7:  Info for directories/hard links/                               \
						   *        devices */
#define ROMFS_FHDR_SIZE 8 /*  8-11: Size of this file in bytes */
#define ROMFS_FHDR_CHKSUM                                                                          \
	12 /* 12-15: Checksum covering the meta data,                                                  \
		*        including the file name, and                                                      \
		*        padding. */
#define ROMFS_FHDR_NAME                                                                            \
	16 /* 16-..: Zero terminated volume name, padded                                               \
		*        to 16 byte boundary. */
 ...
 * RFNEXT_FIFO are not presently supported in ROUX.
 */
...
int romfs_parsedirentry(FAR struct romfs_mountpt_s *rm, uint32_t offset, FAR uint32_t *poffset,
						FAR uint32_t *pnext, FAR uint32_t *pinfo, FAR uint32_t *psize);
