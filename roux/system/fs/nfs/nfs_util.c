...
static inline int nfs_pathsegment(FAR const char **path, FAR char *buffer, FAR char *terminator)
{
...
int nfs_request(FAR struct nfsmount *nmp, int procnum, FAR void *request, size_t reqlen,
				FAR void *response, size_t resplen)
{
	...
	error = rpcclnt_request(clnt, procnum, NFS_PROG, NFS_VER3, request, reqlen, response, resplen);
	if (error != 0) {
		...
		error =
			rpcclnt_request(clnt, procnum, NFS_PROG, NFS_VER3, request, reqlen, response, resplen);
	...
	if (replyh.nfs_status != 0) {
		/* NFS_ERRORS are the same as ROUX errno values */
...
int nfs_lookup(FAR struct nfsmount *nmp, FAR const char *filename, FAR struct file_handle *fhandle,
			   FAR struct nfs_fattr *obj_attributes, FAR struct nfs_fattr *dir_attributes)
{
	FAR uint32_t *ptr;
	uint32_t value;
	int reqlen;
	...
	*ptr++ = txdr_unsigned(namelen);
	...
	error = nfs_request(nmp, NFSPROC_LOOKUP, (FAR void *)&nmp->nm_msgbuffer.lookup, reqlen,
...
int nfs_findnode(FAR struct nfsmount *nmp, FAR const char *relpath, FAR struct file_handle *fhandle,
				 FAR struct nfs_fattr *obj_attributes, FAR struct nfs_fattr *dir_attributes)
{
...
int nfs_finddir(FAR struct nfsmount *nmp, FAR const char *relpath, FAR struct file_handle *fhandle,
