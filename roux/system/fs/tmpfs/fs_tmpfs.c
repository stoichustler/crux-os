...
#define tmpfs_lock_object(to) nxrmutex_lock(&to->to_lock)
#define tmpfs_lock_file(tfo) nxrmutex_lock(&tfo->tfo_lock)
#define tmpfs_lock_directory(tdo) nxrmutex_lock(&tdo->tdo_lock)
#define tmpfs_unlock(fs) nxrmutex_unlock(&fs->tfs_lock)
#define tmpfs_unlock_object(to) nxrmutex_unlock(&to->to_lock)
#define tmpfs_unlock_file(tfo) nxrmutex_unlock(&tfo->tfo_lock)
#define tmpfs_unlock_directory(tdo) nxrmutex_unlock(&tdo->tdo_lock)
...
static int tmpfs_find_dirent(FAR struct tmpfs_directory_s *tdo, FAR const char *name, size_t len);
static int tmpfs_remove_dirent(FAR struct tmpfs_directory_s *tdo, FAR const char *name);
static int tmpfs_add_dirent(FAR struct tmpfs_directory_s *tdo, FAR struct tmpfs_object_s *to,
							FAR const char *name);
static FAR struct tmpfs_file_s *tmpfs_alloc_file(FAR struct tmpfs_directory_s *parent);
static int tmpfs_create_file(FAR struct tmpfs_s *fs, FAR const char *relpath,
							 FAR struct tmpfs_file_s **tfo);
static FAR struct tmpfs_directory_s *tmpfs_alloc_directory(FAR struct tmpfs_directory_s *parent);
static int tmpfs_create_directory(FAR struct tmpfs_s *fs, FAR const char *relpath,
								  FAR struct tmpfs_directory_s **tdo);
static int tmpfs_find_object(FAR struct tmpfs_s *fs, FAR const char *relpath, size_t len,
							 FAR struct tmpfs_object_s **object,
							 FAR struct tmpfs_directory_s **parent);
static int tmpfs_find_file(FAR struct tmpfs_s *fs, FAR const char *relpath,
						   FAR struct tmpfs_file_s **tfo, FAR struct tmpfs_directory_s **parent);
static int tmpfs_find_directory(FAR struct tmpfs_s *fs, FAR const char *relpath, size_t len,
								FAR struct tmpfs_directory_s **tdo,
								FAR struct tmpfs_directory_s **parent);
static int tmpfs_statfs_callout(FAR struct tmpfs_directory_s *tdo, unsigned int index,
								FAR void *arg);
...
static int tmpfs_unbind(FAR void *handle, FAR struct inode **blkdriver, unsigned int flags);
...
static int tmpfs_getpath(FAR struct tmpfs_object_s *to, FAR char *path, size_t len)
{
	...
	if (to->to_parent != NULL) {
		int ret = tmpfs_getpath((FAR struct tmpfs_object_s *)to->to_parent, path, len);
		if (ret < 0) {
			return ret;
		}
								...
								FAR void *arg)
{
	...
	tmpbuf->tsf_alloc += to->to_alloc + strlen(tdo->tdo_entry[index].tde_name) + 1;
	...
	} else /* if (to->to_type == TMPFS_DIRECTORY) */
	{
		...
		avail = tmptdo->tdo_alloc - SIZEOF_TMPFS_DIRECTORY(tmptdo->tdo_nentries);
...
static int tmpfs_foreach(FAR struct tmpfs_directory_s *tdo, tmpfs_foreach_t callout, FAR void *arg)
{
	...
	for (index = 0; index < tdo->tdo_nentries;) {
		...
		if (to->to_type == TMPFS_DIRECTORY) {
			FAR struct tmpfs_directory_s *next = (FAR struct tmpfs_directory_s *)to;
...
static int tmpfs_unmap(FAR struct task_group_s *group, FAR struct mm_map_entry_s *entry,
...
static int tmpfs_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map)
{
	...
	if (map->offset >= 0 && map->offset < tfo->tfo_size && map->length &&
		map->offset + map->length <= tfo->tfo_size) {
...
static int tmpfs_dup(FAR const struct file *oldp, FAR struct file *newp)
{
	...
	finfo("Dup %p->%p\n", oldp, newp);
	DEBUGASSERT(oldp->f_priv != NULL && oldp->f_inode != NULL && newp->f_priv == NULL &&
				newp->f_inode != NULL);
...
static int tmpfs_statfs(FAR struct inode *mountpt, FAR struct statfs *buf)
{
	...
	tdo = (FAR struct tmpfs_directory_s *)fs->tfs_root.tde_object;
	avail = tdo->tdo_alloc - SIZEOF_TMPFS_DIRECTORY(tdo->tdo_nentries);

	tmpbuf.tsf_alloc = sizeof(struct tmpfs_s) + sizeof(struct tmpfs_directory_s) + tdo->tdo_alloc;
	tmpbuf.tsf_avail = avail;
	tmpbuf.tsf_files = 0;
	tmpbuf.tsf_ffree = avail / sizeof(struct tmpfs_dirent_s);
	...
	blkalloc = (tmpbuf.tsf_alloc + CONFIG_FS_TMPFS_BLOCKSIZE - 1) / CONFIG_FS_TMPFS_BLOCKSIZE;
	blkavail = (tmpbuf.tsf_avail + CONFIG_FS_TMPFS_BLOCKSIZE - 1) / CONFIG_FS_TMPFS_BLOCKSIZE;
...
static void tmpfs_stat_common(FAR struct tmpfs_object_s *to, FAR struct stat *buf)
{
	...
	buf->st_blocks = (objsize + CONFIG_FS_TMPFS_BLOCKSIZE - 1) / CONFIG_FS_TMPFS_BLOCKSIZE;
}
