...
static ssize_t signalfd_file_read(FAR struct file *filep, FAR char *buffer, size_t len);
static int signalfd_file_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static struct inode g_signalfd_inode = {NULL,					/* i_parent */
										NULL,					/* i_peer */
										NULL,					/* i_child */
										1,						/* i_crefs */
...
static void signalfd_action(int signo, FAR siginfo_t *info, FAR void *ucontext)
{
...
int signalfd(int fd, FAR const sigset_t *mask, int flags)
{
	...
	if (fd == -1) {
		...
		fd = file_allocate_from_inode(&g_signalfd_inode, O_RDOK | flags, 0, dev, 0);
		if (fd < 0) {
			ret = -fd;
