...
static ssize_t pseudofile_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static ssize_t pseudofile_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static off_t pseudofile_seek(FAR struct file *filep, off_t offset, int whence);
static int pseudofile_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map);
static int pseudofile_munmap(FAR struct task_group_s *group, FAR struct mm_map_entry_s *map,
...
static int pseudofile_expand(FAR struct inode *node, size_t size)
{
	FAR struct fs_pseudofile_s *pf = node->i_private;
	FAR void *tmp;
...
static int pseudofile_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map)
{
	...
	inode_addref(node);
	if (map->offset >= 0 && map->offset < node->i_size && map->length != 0 &&
		map->offset + map->length <= node->i_size) {
...
int pseudofile_create(FAR struct inode **node, FAR const char *path, mode_t mode)
{
