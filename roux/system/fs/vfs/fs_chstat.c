...
static int chstat_recursive(FAR const char *path, FAR const struct stat *buf, int flags,
							int resolve)
{
...
static int chstat(FAR const char *path, FAR struct stat *buf, int flags, int resolve)
{
	int ret = -EINVAL;
