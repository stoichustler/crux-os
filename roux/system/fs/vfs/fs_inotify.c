...
static ssize_t inotify_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static int inotify_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static struct inode g_inotify_inode = {
	NULL, NULL, NULL, 1, FSNODEFLAG_TYPE_DRIVER, {&g_inotify_fops}};
...
static FAR struct inotify_event_s *inotify_alloc_event(int wd, uint32_t mask, uint32_t cookie,
													   FAR const char *name)
{
...
static void inotify_queue_event(FAR struct inotify_device_s *dev, int wd, uint32_t mask,
								uint32_t cookie, FAR const char *name)
{
	FAR struct inotify_event_s *event;
	...
	if (!list_is_empty(&dev->events)) {
		...
		last = list_last_entry(&dev->events, struct inotify_event_s, node);
		if (last->event.mask == mask && last->event.wd == wd && last->event.cookie == cookie &&
...
static void inotify_remove_watch_no_event(FAR struct inotify_watch_s *watch)
{
...
static ssize_t inotify_read(FAR struct file *filp, FAR char *buffer, size_t len)
{
...
static int inotify_close(FAR struct file *filep)
{
	...
	while (!list_is_empty(&dev->watches)) {
		FAR struct inotify_watch_s *watch;
		watch = list_first_entry(&dev->watches, struct inotify_watch_s, d_node);
		inotify_remove_watch_no_event(watch);
	}
...
static FAR struct inotify_device_s *inotify_get_device_from_fd(int fd, FAR struct file **filep)
{
...
inotify_get_watch_from_list(FAR struct inotify_device_s *dev, FAR struct inotify_watch_list_s *list)
{
	FAR struct inotify_watch_s *watch;
...
static FAR struct inotify_watch_s *inotify_alloc_watch(FAR struct inotify_device_s *dev,
...
static FAR struct inotify_watch_s *inotify_find_watch(FAR struct inotify_device_s *dev, int wd)
{
	FAR struct inotify_watch_s *watch;
...
static void inotify_free_watch_list(FAR struct inotify_watch_list_s *list)
{
	...
	do {
		...
		watch = list_first_entry(&list->watches, struct inotify_watch_s, l_node);
		dev = watch->dev;
		nxmutex_lock(&dev->lock);
...
static FAR struct inotify_watch_list_s *inotify_alloc_watch_list(FAR const char *path)
{
	FAR struct inotify_watch_list_s *list;
	FAR ENTRY *result;
	ENTRY item;
...
static FAR struct inotify_watch_list_s *inotify_get_watch_list(FAR const char *path)
{
	FAR ENTRY *result;
	ENTRY item;
...
static void inotify_queue_watch_list_event(FAR struct inotify_watch_list_s *list, uint32_t mask,
										   uint32_t cookie, FAR const char *name)
{
	FAR struct inotify_watch_s *watch;
	FAR struct inotify_watch_s *w_tmp;

	list_for_every_entry_safe(&list->watches, watch, w_tmp, struct inotify_watch_s, l_node)
	{
...
static void inotify_queue_parent_event(FAR char *path, uint32_t mask, uint32_t cookie)
{
	FAR struct inotify_watch_list_s *list;
	FAR char *name;
...
static int notify_check_inode(FAR struct file *filep)
{
	...
	if ((tcb->flags & TCB_FLAG_SIGNAL_ACTION) ||
		(!INODE_IS_MOUNTPT(filep->f_inode) && !INODE_IS_PSEUDODIR(filep->f_inode) &&
...
static inline void notify_queue_filep_event(FAR struct file *filep, uint32_t mask)
{
	FAR char *pathbuffer;
	int ret;
...
int inotify_init1(int flags)
{
	...
	ret = file_allocate_from_inode(&g_inotify_inode, O_RDOK | flags, 0, dev, 0);
	if (ret < 0) {
...
void notify_rename(FAR const char *oldpath, bool oldisdir, FAR const char *newpath, bool newisdir)
{
