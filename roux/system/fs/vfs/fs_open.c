 ...
 *
 *   inode_checkflags() is an internal ROUX interface and should not be
...
static int file_vopen(FAR struct file *filep, FAR const char *path, int oflags, mode_t umask,
					  va_list ap)
{
...
#if defined(CONFIG_BCH) && !defined(CONFIG_DISABLE_MOUNTPOINT) &&                                  \
 ...
 *
 *   nx_open() is an internal ROUX interface and should not be called from
 *   applications.
 *
 * Input Parameters:
...
static int nx_vopen(FAR struct fdlist *list, FAR const char *path, int oflags, va_list ap)
{
