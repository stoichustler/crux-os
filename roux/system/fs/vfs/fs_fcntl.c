...
static int file_vfcntl(FAR struct file *filep, int cmd, va_list ap)
{
	...
	case F_GETLK:
		...
		{
			ret = file_ioctl(filep, FIOC_GETLK, va_arg(ap, FAR struct flock *));
		}
	...
	case F_SETLK:
		...
		{
			ret = file_ioctl(filep, FIOC_SETLK, va_arg(ap, FAR struct flock *));
		}
	...
	case F_SETLKW:
		...
		{
			ret = file_ioctl(filep, FIOC_SETLKW, va_arg(ap, FAR struct flock *));
		}
