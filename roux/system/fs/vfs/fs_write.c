...
static ssize_t file_writev_compat(FAR struct file *filep, FAR const struct iovec *iov, int iovcnt)
{
	...
	for (i = 0, ntotal = 0; i < iovcnt; i++) {
		...
		nwritten = inode->u.i_ops->write(filep, iov[i].iov_base, iov[i].iov_len);
