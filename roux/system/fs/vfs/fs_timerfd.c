...
static ssize_t timerfd_read(FAR struct file *filep, FAR char *buffer, size_t len);
#ifdef CONFIG_TIMER_FD_POLL
static int timerfd_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
#endif

static int timerfd_blocking_io(FAR struct timerfd_priv_s *dev, FAR timerfd_waiter_sem_t *sem,
...
static struct inode g_timerfd_inode = {NULL,				   /* i_parent */
									   NULL,				   /* i_peer */
									   NULL,				   /* i_child */
									   1,					   /* i_crefs */
...
static FAR struct timerfd_priv_s *timerfd_allocdev(void)
{
	...
	dev = (FAR struct timerfd_priv_s *)fs_heap_zalloc(sizeof(struct timerfd_priv_s));
	if (dev) {
...
int timerfd_create(int clockid, int flags)
{
	...
	new_fd = file_allocate_from_inode(&g_timerfd_inode, O_RDONLY | flags, 0, new_dev, 0);
	if (new_fd < 0) {
		ret = new_fd;
...
int timerfd_settime(int fd, int flags, FAR const struct itimerspec *new_value,
