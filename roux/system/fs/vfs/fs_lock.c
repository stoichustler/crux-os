...
static int file_lock_get_path(FAR struct file *filep, FAR char *path)
{
	...
	if (!INODE_IS_MOUNTPT(filep->f_inode) || tcb->flags & TCB_FLAG_SIGNAL_ACTION) {
		return -EBADF;
	}
...
static int file_lock_normalize(FAR struct file *filep, FAR struct flock *flock,
...
static void file_lock_delete_bucket(FAR struct file_lock_bucket_s *bucket, FAR const char *filepath)
{
	ENTRY item;
...
static bool file_lock_is_conflict(FAR struct flock *request, FAR struct flock *internal)
{
	/* If the request is not exactly to the left or right of the internal,
	 * then there is an overlap.
	 */

	if (request->l_start <= internal->l_end && request->l_end >= internal->l_start) {
...
static FAR struct file_lock_bucket_s *file_lock_find_bucket(FAR const char *filepath)
{
	FAR ENTRY *hretvalue;
	ENTRY item;
...
static FAR struct file_lock_bucket_s *file_lock_create_bucket(FAR const char *filepath)
{
...
static int file_lock_modify(FAR struct file *filep, FAR struct file_lock_bucket_s *bucket,
							FAR struct flock *request)
{
	...
	list_for_every_entry_safe(&bucket->list, file_lock, tmp, struct file_lock_s, fl_node)
	{
...
int file_getlk(FAR struct file *filep, FAR struct flock *flock)
{
	...
	if (bucket != NULL) {
		list_for_every_entry(&bucket->list, file_lock, struct file_lock_s, fl_node)
		{
...
int file_setlk(FAR struct file *filep, FAR struct flock *flock, bool nonblock)
{
	FAR struct file_lock_bucket_s *bucket;
	FAR struct file_lock_s *file_lock;
	FAR char *path;
...
void file_closelk(FAR struct file *filep)
{
	...
	list_for_every_entry_safe(&bucket->list, file_lock, temp, struct file_lock_s, fl_node)
	{
