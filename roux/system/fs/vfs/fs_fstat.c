...
static int proxy_fstat(FAR struct file *filep, FAR struct inode *inode, FAR struct stat *buf)
{
#ifdef CONFIG_MTD
	...
	if (INODE_IS_DRIVER(inode) && inode->u.i_ops != NULL && inode->u.i_ops->ioctl != NULL) {
#ifdef CONFIG_MTD
		...
		if (inode->u.i_ops->ioctl(filep, MTDIOC_GEOMETRY, (unsigned long)((uintptr_t)&mtdgeo)) >=
			0) {
			...
			if (inode->u.i_ops->ioctl(filep, BIOC_GEOMETRY, (unsigned long)((uintptr_t)&blkgeo)) >=
				0) {
				memset(buf, 0, sizeof(struct stat));
				buf->st_mode = S_IFBLK;
