...
static inline void poll_teardown(FAR struct pollfd *fds, nfds_t nfds, FAR int *count)
{
	unsigned int i;
...
static inline int poll_setup(FAR struct pollfd *fds, nfds_t nfds, FAR sem_t *sem)
{
	unsigned int i;
	int ret = OK;
	int count = 0;
...
noinstrument_function void poll_notify(FAR struct pollfd **afds, int nfds, pollevent_t eventset)
{
	int i;
	FAR struct pollfd *fds;
	...
	for (i = 0; i < nfds && eventset; i++) {
		...
		if (fds != NULL) {
			...
			if ((fds->revents != 0 || (fds->events & POLLALWAYS) != 0) && fds->cb != NULL) {
...
int file_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup)
{
	...
	if (inode != NULL) {
		...
		if ((INODE_IS_DRIVER(inode) || INODE_IS_MQUEUE(inode) || INODE_IS_SOCKET(inode) ||
			 INODE_IS_PIPE(inode)) &&
		...
		else if (INODE_IS_MOUNTPT(inode) || INODE_IS_BLOCK(inode) || INODE_IS_MTD(inode)) {
			if (setup) {
