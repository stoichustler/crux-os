...
static ssize_t eventfd_do_read(FAR struct file *filep, FAR char *buffer, size_t len);
static ssize_t eventfd_do_write(FAR struct file *filep, FAR const char *buffer, size_t len);
#ifdef CONFIG_EVENT_FD_POLL
static int eventfd_do_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
#endif

static int eventfd_blocking_io(FAR struct eventfd_priv_s *dev, FAR eventfd_waiter_sem_t *sem,
...
static struct inode g_eventfd_inode = {NULL,				   /* i_parent */
									   NULL,				   /* i_peer */
									   NULL,				   /* i_child */
									   1,					   /* i_crefs */
...
static FAR struct eventfd_priv_s *eventfd_allocdev(void)
{
	...
	dev = (FAR struct eventfd_priv_s *)fs_heap_zalloc(sizeof(struct eventfd_priv_s));
	if (dev) {
...
static ssize_t eventfd_do_write(FAR struct file *filep, FAR const char *buffer, size_t len)
{
	...
	if (len < sizeof(eventfd_t) || buffer == NULL || (*(FAR eventfd_t *)buffer == (eventfd_t)-1) ||
	...
	if (new_counter < dev->counter) {
		...
		} while ((new_counter = dev->counter + *(FAR eventfd_t *)buffer) < dev->counter);
...
int eventfd(unsigned int count, int flags)
{
	...
	new_fd = file_allocate_from_inode(&g_eventfd_inode, O_RDWR | flags, 0, new_dev, 0);
	if (new_fd < 0) {
		ret = new_fd;
		goto exit_with_dev;
	}
