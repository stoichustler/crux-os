...
static ssize_t dir_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static struct inode g_dir_inode = {
	NULL, NULL, NULL, 1, 0, {&g_dir_fileops},
};
...
static int open_pseudodir(FAR struct inode *inode, FAR struct fs_dirent_s **dir)
{
...
static int read_pseudodir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry)
{
	...
	if (pdir->next->i_child != NULL || pdir->next->u.i_ops == NULL) {
		entry->d_type = DTYPE_DIRECTORY;
	}
...
#ifndef CONFIG_DISABLE_MOUNTPOINT
	if (INODE_IS_MOUNTPT(inode)) {
		ret = inode->u.i_mops->readdir(inode, dir, (FAR struct dirent *)buffer);
	} else
#endif
	{
