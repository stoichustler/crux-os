...
static int epoll_do_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
static int epoll_setup(FAR epoll_head_t *eph);
static int epoll_teardown(FAR epoll_head_t *eph, FAR struct epoll_event *evs, int maxevents);
...
static struct inode g_epoll_inode = {NULL,					 /* i_parent */
									 NULL,					 /* i_peer */
									 NULL,					 /* i_child */
									 1,						 /* i_crefs */
...
int epoll_ctl(int epfd, int op, int fd, FAR struct epoll_event *ev)
{
	...
	case EPOLL_CTL_ADD:
		...
		if (list_is_empty(&eph->free)) {
			...
			extend = fs_heap_zalloc(sizeof(*extend) + 2 * sizeof(epoll_node_t) * eph->size);
			if (extend == NULL) {
				ret = -ENOMEM;
				goto err;
			}
...
int epoll_pwait(int epfd, FAR struct epoll_event *evs, int maxevents, int timeout,
