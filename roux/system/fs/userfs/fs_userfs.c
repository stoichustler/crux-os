...
static int userfs_open(FAR struct file *filep, const char *relpath, int oflags, mode_t mode);
static int userfs_close(FAR struct file *filep);
static ssize_t userfs_read(FAR struct file *filep, char *buffer, size_t buflen);
static ssize_t userfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t userfs_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int userfs_closedir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir);
static int userfs_readdir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir,
						  FAR struct dirent *entry);
static int userfs_rewinddir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir);

static int userfs_bind(FAR struct inode *blkdriver, FAR const void *data, FAR void **handle);
static int userfs_unbind(FAR void *handle, FAR struct inode **blkdriver, unsigned int flags);
static int userfs_statfs(FAR struct inode *mountpt, FAR struct statfs *buf);

static int userfs_unlink(FAR struct inode *mountpt, FAR const char *relpath);
static int userfs_mkdir(FAR struct inode *mountpt, FAR const char *relpath, mode_t mode);
static int userfs_rmdir(FAR struct inode *mountpt, FAR const char *relpath);
static int userfs_rename(FAR struct inode *mountpt, FAR const char *oldrelpath,
						 FAR const char *newrelpath);
static int userfs_stat(FAR struct inode *mountpt, FAR const char *relpath, FAR struct stat *buf);
static int userfs_fchstat(FAR const struct file *filep, FAR const struct stat *buf, int flags);
...
static int userfs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, SIZEOF_USERFS_OPEN_REQUEST_S(pathlen + 1), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
	...
	nrecvd = psock_recvfrom(&priv->psock, priv->iobuffer, IOBUFFER_SIZE(priv), 0, NULL, NULL);
	nxmutex_unlock(&priv->lock);
...
static int userfs_close(FAR struct file *filep)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_close_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static ssize_t userfs_read(FAR struct file *filep, char *buffer, size_t buflen)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_read_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static ssize_t userfs_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, SIZEOF_USERFS_WRITE_REQUEST_S(buflen), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static off_t userfs_seek(FAR struct file *filep, off_t offset, int whence)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_seek_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_ioctl_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_sync(FAR struct file *filep)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_sync_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_dup(FAR const struct file *oldp, FAR struct file *newp)
{
	...
	DEBUGASSERT(oldp != NULL && oldp->f_inode != NULL && oldp->f_inode->i_private != NULL);
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_dup_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_fstat(FAR const struct file *filep, FAR struct stat *buf)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_fstat_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_fchstat(FAR const struct file *filep, FAR const struct stat *buf, int flags)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_fchstat_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
...
static int userfs_truncate(FAR struct file *filep, off_t length)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_truncate_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
						  ...
						  FAR struct fs_dirent_s **dir)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, SIZEOF_USERFS_OPENDIR_REQUEST_S(pathlen + 1),
						 0, (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
		return (int)nsent;
...
static int userfs_closedir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_closedir_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
						  ...
						  FAR struct dirent *entry)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_readdir_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_rewinddir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_rewinddir_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_bind(FAR struct inode *blkdriver, FAR const void *data, FAR void **handle)
{
	...
	ret = psock_bind(&priv->psock, (FAR struct sockaddr *)&client, sizeof(struct sockaddr_in));
	if (ret < 0) {
...
static int userfs_unbind(FAR void *handle, FAR struct inode **blkdriver, unsigned int flags)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_destroy_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_statfs(FAR struct inode *mountpt, FAR struct statfs *buf)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, sizeof(struct userfs_statfs_request_s), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_unlink(FAR struct inode *mountpt, FAR const char *relpath)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, SIZEOF_USERFS_UNLINK_REQUEST_S(pathlen + 1),
						 0, (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_mkdir(FAR struct inode *mountpt, FAR const char *relpath, mode_t mode)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, SIZEOF_USERFS_MKDIR_REQUEST_S(pathlen + 1),
						 0, (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_rmdir(FAR struct inode *mountpt, FAR const char *relpath)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, SIZEOF_USERFS_RMDIR_REQUEST_S(pathlen + 1),
						 0, (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
...
static int userfs_stat(FAR struct inode *mountpt, FAR const char *relpath, FAR struct stat *buf)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, SIZEOF_USERFS_STAT_REQUEST_S(pathlen + 1), 0,
						 (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %d\n", (int)nsent);
		nxmutex_unlock(&priv->lock);
						 ...
						 FAR const struct stat *buf, int flags)
{
	...
	nsent = psock_sendto(&priv->psock, priv->iobuffer, SIZEOF_USERFS_CHSTAT_REQUEST_S(pathlen + 1),
						 0, (FAR struct sockaddr *)&priv->server, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ferr("ERROR: psock_sendto failed: %zd\n", nsent);
		nxmutex_unlock(&priv->lock);
