...
static void fdlist_get_by_index(FAR struct fdlist *list, int l1, int l2, FAR struct file **filep,
								FAR struct fd **fdp)
{
	FAR struct fd *fdp1;
	irqstate_t flags;
...
static int fdlist_extend(FAR struct fdlist *list, size_t row)
{
	...
	i = orig_rows;
	do {
		fds[i] = fs_heap_zalloc(sizeof(struct fd) * CONFIG_NFILE_DESCRIPTORS_PER_BLOCK);
		if (fds[i] == NULL) {
			while (--i >= orig_rows) {
				fs_heap_free(fds[i]);
			}
...
static void fdlist_install(FAR struct fdlist *list, int fd, FAR struct file *filep, int oflags)
{
...
int fdlist_dup3(FAR struct fdlist *list, int fd1, int fd2, int flags)
{
	...
	if (fd2 >= fdlist_count(list)) {
		ret = fdlist_extend(list, fd2 / CONFIG_NFILE_DESCRIPTORS_PER_BLOCK + 1);
		if (ret < 0) {
			return ret;
		}
	}
...
#if CONFIG_FS_BACKTRACE > 0
		backtrace_format(buf, sizeof(buf), fdp->f_backtrace, CONFIG_FS_BACKTRACE);
#endif

		syslog(LOG_INFO,
			   "%-4d%-4d%-8d%-5x%-10ld%-14s"
#if CONFIG_FS_BACKTRACE > 0
			   " %s"
#endif
			   "\n",
			   getpid(), i, filep->f_oflags, INODE_GET_TYPE(filep->f_inode), (long)filep->f_pos,
			   path
#if CONFIG_FS_BACKTRACE > 0
			   ,
			   buf
...
int fdlist_get2(FAR struct fdlist *list, int fd, FAR struct file **filep, FAR struct fd **fdp)
{
	...
	fdlist_get_by_index(list, fd / CONFIG_NFILE_DESCRIPTORS_PER_BLOCK,
						fd % CONFIG_NFILE_DESCRIPTORS_PER_BLOCK, filep, fdp);
	if (*filep == NULL) {
		return -EBADF;
	}
...
{
	return fdlist_allocate(nxsched_get_fdlist_from_tcb(this_task()), oflags, minfd, filep);
}
