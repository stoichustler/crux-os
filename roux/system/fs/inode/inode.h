...
#define FS_ADD_BACKTRACE(fd)                                                                       \
	do {                                                                                           \
		int n = sched_backtrace(_SCHED_GETTID(), (fd)->f_backtrace, CONFIG_FS_BACKTRACE,           \
...
typedef int (*foreach_inode_t)(FAR struct inode *inode, FAR char dirpath[PATH_MAX], FAR void *arg);
...
int inode_chstat(FAR struct inode *inode, FAR const struct stat *buf, int flags, int resolve);
...
int inode_reserve(FAR const char *path, mode_t mode, FAR struct inode **inode);
