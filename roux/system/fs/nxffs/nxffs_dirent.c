...
int nxffs_opendir(FAR struct inode *mountpt, FAR const char *relpath, FAR struct fs_dirent_s **dir)
{
	...
	/* Recover the file system state from the ROUX inode instance */
...
int nxffs_readdir(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir, FAR struct dirent *dentry)
{
	FAR struct nxffs_volume_s *volume;
	FAR struct nxffs_dir_s *ndir;
