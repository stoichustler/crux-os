...
static inline int nxffs_hdrpos(FAR struct nxffs_volume_s *volume, FAR struct nxffs_wrfile_s *wrfile)
{
	int ret;
...
static inline int nxffs_nampos(FAR struct nxffs_volume_s *volume, FAR struct nxffs_wrfile_s *wrfile,
							   int namlen)
{
	int ret;
...
static inline int nxffs_namerased(FAR struct nxffs_volume_s *volume,
								  FAR struct nxffs_wrfile_s *wrfile, int namlen)
{
	int ret;
...
static inline int nxffs_wrname(FAR struct nxffs_volume_s *volume, FAR struct nxffs_entry_s *entry,
							   int namlen)
{
	int ret;
...
static inline int nxffs_wropen(FAR struct nxffs_volume_s *volume, FAR const char *name,
							   mode_t oflags, FAR struct nxffs_ofile_s **ppofile)
{
	FAR struct nxffs_wrfile_s *wrfile;
...
#else
	wrfile = (FAR struct nxffs_wrfile_s *)fs_heap_zalloc(sizeof(struct nxffs_wrfile_s));
	if (!wrfile) {
		ret = -ENOMEM;
		goto errout_with_lock;
	}
...
static inline int nxffs_rdopen(FAR struct nxffs_volume_s *volume, FAR const char *name,
							   FAR struct nxffs_ofile_s **ppofile)
{
	 ...
	 */
		...
		ofile = (FAR struct nxffs_ofile_s *)fs_heap_zalloc(sizeof(struct nxffs_ofile_s));
		if (!ofile) {
								  ...
								  FAR struct nxffs_ofile_s *ofile)
{
	...
	for (prev = NULL, curr = volume->ofiles; curr && curr != ofile; prev = curr, curr = curr->flink)
		;
...
int nxffs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
	...
	/* Get the mountpoint private data from the ROUX inode reference in the
	 * file structure
	 */
