...
#if defined(CONFIG_DEBUG_FEATURES) && defined(CONFIG_DEBUG_FS)
static inline ssize_t nxffs_analyzeinode(FAR struct nxffs_blkinfo_s *blkinfo, int offset)
{
  ...
  utc    = nxffs_rdle32(inode.utc);
	...
	if (noffs > nextblock) {
		...
		if (blkinfo->verbose) {
			syslog(LOG_NOTICE, g_format, blkinfo->block, offset, "INODE", "UNVERFD", datlen);
		}
	...
	if (crc != ecrc) {
		syslog(LOG_NOTICE, g_format, blkinfo->block, offset, "INODE", "CRC BAD", datlen);
		return ERROR;
	}
	...
	if (state == INODE_STATE_FILE) {
		if (blkinfo->verbose) {
			syslog(LOG_NOTICE, g_format, blkinfo->block, offset, "INODE", "OK     ", datlen);
		}
	} else if (state == INODE_STATE_DELETED) {
		if (blkinfo->verbose) {
			syslog(LOG_NOTICE, g_format, blkinfo->block, offset, "INODE", "DELETED", datlen);
		}
	} else {
		syslog(LOG_NOTICE, g_format, blkinfo->block, offset, "INODE", "CORRUPT", datlen);
	}
...
#if defined(CONFIG_DEBUG_FEATURES) && defined(CONFIG_DEBUG_FS)
static inline ssize_t nxffs_analyzedata(FAR struct nxffs_blkinfo_s *blkinfo, int offset)
{
	...
	if (crc != ecrc) {
		syslog(LOG_NOTICE, g_format, blkinfo->block, offset, "DATA ", "CRC BAD",
	...
	if (blkinfo->verbose) {
		syslog(LOG_NOTICE, g_format, blkinfo->block, offset, "DATA ", "OK     ",
			   (long unsigned int)datlen);
	}
...
static inline void nxffs_analyze(FAR struct nxffs_blkinfo_s *blkinfo)
{
	...
	if (memcmp(blkhdr->magic, g_blockmagic, NXFFS_MAGICSIZE) != 0) {
		syslog(LOG_NOTICE, g_format, blkinfo->block, 0, "BLOCK", "NO FRMT", blkinfo->geo.blocksize);
		...
		syslog(LOG_NOTICE, g_format, blkinfo->block, 0, "BLOCK", "BAD    ", blkinfo->geo.blocksize);
	} else {
		syslog(LOG_NOTICE, g_format, blkinfo->block, 0, "BLOCK", "CORRUPT", blkinfo->geo.blocksize);
	}
...
int nxffs_dump(FAR struct mtd_dev_s *mtd, bool verbose)
{
	...
	for (blkinfo.block = 0, blkinfo.offset = 0; blkinfo.block < blkinfo.nblocks;
