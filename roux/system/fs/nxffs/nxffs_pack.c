...
static off_t nxffs_getoffset(FAR struct nxffs_volume_s *volume, off_t offset, off_t block)
{
...
static off_t nxffs_packtell(FAR struct nxffs_volume_s *volume, FAR struct nxffs_pack_s *pack)
{
...
static inline int nxffs_startpos(FAR struct nxffs_volume_s *volume, FAR struct nxffs_pack_s *pack,
								 FAR off_t *froffset)
{
...
static int nxffs_srcsetup(FAR struct nxffs_volume_s *volume, FAR struct nxffs_pack_s *pack,
						  off_t offset)
{
...
static int nxffs_destsetup(FAR struct nxffs_volume_s *volume, FAR struct nxffs_pack_s *pack)
{
	size_t mindata;
	int namlen;
	int ret;
	...
	if (pack->dest.entry.hoffset == 0) {
		...
		memset(&pack->iobuffer[pack->iooffset], CONFIG_NXFFS_ERASEDSTATE, SIZEOF_NXFFS_INODE_HDR);
...
static int nxffs_wrinodehdr(FAR struct nxffs_volume_s *volume, FAR struct nxffs_pack_s *pack)
{
...
static void nxffs_wrdathdr(FAR struct nxffs_volume_s *volume, FAR struct nxffs_pack_s *pack)
{
...
static void nxffs_packtransfer(FAR struct nxffs_volume_s *volume, FAR struct nxffs_pack_s *pack)
{
...
static int nxffs_endsrcblock(FAR struct nxffs_volume_s *volume, FAR struct nxffs_pack_s *pack)
{
	struct nxffs_blkentry_s blkentry;
	off_t offset;
	int ret;
...
static inline int nxffs_packblock(FAR struct nxffs_volume_s *volume, FAR struct nxffs_pack_s *pack)
{
	off_t offset;
	int ret;
...
static FAR struct nxffs_wrfile_s *nxffs_setupwriter(FAR struct nxffs_volume_s *volume,
													FAR struct nxffs_pack_s *pack)
{
...
static inline int nxffs_packwriter(FAR struct nxffs_volume_s *volume, FAR struct nxffs_pack_s *pack,
								   FAR struct nxffs_wrfile_s *wrfile)
{
	int ret;
		...
		if (wrfile) {
			...
			if (ret == OK) {
				...
				if (ret == OK) {
					...
					volume->froffset = block * volume->geo.blocksize + SIZEOF_NXFFS_BLOCK_HDR;
				}
			}
...
start_pack:
	...
	for (eblock = pack.ioblock / volume->blkper; eblock < volume->geo.neraseblocks; eblock++) {
		...
		for (i = 0, block = pack.block0, pack.iobuffer = volume->pack; i < volume->blkper;
