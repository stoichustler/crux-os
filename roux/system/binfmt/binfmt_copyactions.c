					   ...
					   FAR const posix_spawn_file_actions_t *actions)
{
	...
	for (entry = (FAR struct spawn_general_file_action_s *)actions; entry != NULL;
		...
		case SPAWN_FILE_ACTION_OPEN:
			...
			size += ALIGN_UP(SIZEOF_OPEN_FILE_ACTION_S(strlen(open->path)), sizeof(FAR void *));
			break;
	...
	*copy = buffer = kmm_malloc(size);
	...
	for (entry = (FAR struct spawn_general_file_action_s *)actions, prev = NULL; entry != NULL;
		 entry = entry->flink) {
		switch (entry->action) {
		case SPAWN_FILE_ACTION_CLOSE:
			close = buffer;
		...
		case SPAWN_FILE_ACTION_OPEN:
			...
			buffer = (FAR char *)buffer +
					 ALIGN_UP(SIZEOF_OPEN_FILE_ACTION_S(strlen(tmp->path)), sizeof(FAR void *));
			break;
