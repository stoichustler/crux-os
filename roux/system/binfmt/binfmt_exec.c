...
static int exec_internal(FAR const char *filename, FAR char *const *argv, FAR char *const *envp,
...
{
	return exec_internal(filename, argv, envp, exports, nexports, actions, attr, true);
}
		 ...
		 FAR const struct symtab_s *exports, int nexports)
{
	...
	ret = exec_internal(filename, argv, envp, exports, nexports, NULL, NULL, false);
	if (ret < 0) {
		set_errno(-ret);
		ret = ERROR;
	}
