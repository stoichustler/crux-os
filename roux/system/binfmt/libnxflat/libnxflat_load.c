...
int nxflat_load(struct nxflat_loadinfo_s *loadinfo)
{
	...
	ret = file_mmap(&loadinfo->file, NULL, loadinfo->isize, PROT_READ, MAP_SHARED | MAP_FILE, 0,
					(FAR void **)&loadinfo->ispace);
	if (ret < 0) {
	...
	ret = nxflat_read(loadinfo, (FAR char *)loadinfo->dspace->region, dreadsize, doffset);
	if (ret < 0) {
	...
	binfo("TEXT: %08x Entry point offset: %08" PRIx32 " Data offset: %08jx\n", loadinfo->ispace,
		  loadinfo->entryoffs, (intmax_t)doffset);
