...
static inline int nxflat_bindrel32i(FAR struct nxflat_loadinfo_s *loadinfo, uint32_t offset)
{
	FAR uint32_t *addr;

	binfo("NXFLAT_RELOC_TYPE_REL32I Offset: %08" PRIx32 " I-Space: %" PRIxPTR "\n", offset,
		  loadinfo->ispace + sizeof(struct nxflat_hdr_s));
...
static inline int nxflat_bindrel32d(FAR struct nxflat_loadinfo_s *loadinfo, uint32_t offset)
{
	FAR uint32_t *addr;
...
static inline int nxflat_bindrel32id(FAR struct nxflat_loadinfo_s *loadinfo, uint32_t offset)
{
	FAR uint32_t *addr;
...
static inline int nxflat_gotrelocs(FAR struct nxflat_loadinfo_s *loadinfo)
{
	...
	binfo("isize: %08lx dpsace: %p relocs: %p\n", (long)loadinfo->isize, loadinfo->dspace->region,
		  relocs);
									 ...
									 FAR const struct symtab_s *exports, int nexports)
{
	...
	if (offset != 0 && nimports > 0) {
		...
		imports =
			(FAR struct nxflat_import_s *)(offset - loadinfo->isize + loadinfo->dspace->region);
...
static inline int nxflat_clearbss(FAR struct nxflat_loadinfo_s *loadinfo)
{
	...
	memset((FAR void *)(loadinfo->dspace->region + loadinfo->datasize), 0, loadinfo->bsssize);
...
int nxflat_bind(FAR struct nxflat_loadinfo_s *loadinfo, FAR const struct symtab_s *exports,
				int nexports)
{
