...
$CIPLAT = "$CIWORKSPACE\roux\tools\ci\platforms"
$roux = "$CIWORKSPACE\roux"
...
function install_tools {
    $ROUXTOOLS = "$CIWORKSPACE\tools"
    $env:ROUXTOOLS = "$ROUXTOOLS"
    if (-not (Test-Path -Path $ROUXTOOLS)) {
        New-Item -ItemType Directory -Path $ROUXTOOLS -Force > $null
    }
...
function setup_repos {
    ...
    if (Test-Path $roux) {
        ...
        git clone https://github.com/apache/roux.git "$roux"
        Set-Location "$roux"
    }
    git log -1
    ...
    if (Test-Path $apps) {
        ...
        git clone https://github.com/apache/roux-apps.git "$apps"
        Set-Location "$apps"
    }
    git log -1
...
function run_builds {
    ...
    foreach ( $build in $builds ) {
        & $roux\tools\testbuild.ps1 $options $build
    }
