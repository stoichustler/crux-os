...
core=$target
image=`find ${roux} -type f -name 'roux'`
path=${image%/*}
cd ${roux}/tools/ci/testrun/script
python3 -m pytest -m "${mark}" ./ -B ${BOARD} -P ${path} -L ${logs}/${BOARD}/${core} -R ${target} -C --json=${logs}/${BOARD}/${core}/pytest.json
ret="$?"

#clean
find ${roux}/tools/ci/testrun -name '__pycache__' |xargs rm -rf
find ${roux}/tools/ci/testrun -name '.pytest_cache' |xargs rm -rf
rm -rf ${logs}
rm -f ${roux}/fatfs.img
