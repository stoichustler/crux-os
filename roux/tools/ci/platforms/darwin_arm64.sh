...
# ROUX CI Job for macOS, runs on Apple Silicon Mac
...
arm_clang_toolchain() {
  ...
  if [ ! -f "${ROUXTOOLS}/clang-arm-none-eabi/bin/clang" ]; then
    ...
    sudo cp -R /Volumes/${basefile}/${basefile} "${ROUXTOOLS}"/${basefile}
...
binutils() {
  add_path "${ROUXTOOLS}"/bintools/bin

  if ! type objcopy > /dev/null 2>&1; then
    brew install binutils
    mkdir -p "${ROUXTOOLS}"/bintools/bin
    # It is possible we cached prebuilt but did brew install so recreate
    # symlink if it exists
    rm -f "${ROUXTOOLS}"/bintools/bin/objcopy
    ln -s /opt/homebrew/opt/binutils/bin/objcopy "${ROUXTOOLS}"/bintools/bin/objcopy
  fi
...
bloaty() {
  ...
  if [ ! -f "${ROUXTOOLS}/bloaty/bin/bloaty" ]; then
    git clone --branch main https://github.com/google/bloaty "${ROUXTOOLS}"/bloaty-src
    ...
    cmake -B build/bloaty -GNinja -D BLOATY_PREFER_SYSTEM_CAPSTONE=NO -D CMAKE_INSTALL_PREFIX="${ROUXTOOLS}"/bloaty
...
gperf() {
  add_path "${ROUXTOOLS}"/gperf/bin

  if [ ! -f "${ROUXTOOLS}/gperf/bin/gperf" ]; then
    local basefile
    basefile=gperf-3.1
    ...
    cd "${ROUXTOOLS}"/${basefile}
    ./configure --prefix="${ROUXTOOLS}"/gperf; make; make install
    cd "${ROUXTOOLS}"
...
mips_gcc_toolchain() {
  add_path "${ROUXTOOLS}"/pinguino-compilers/macosx/p32/bin
...
python_tools() {
  # Python User Env
  export PIP_USER=yes
  export PYTHONUSERBASE=${ROUXTOOLS}/pylocal
  echo "export PIP_USER=yes" >> "${ROUXTOOLS}"/env.sh
  echo "export PYTHONUSERBASE=${ROUXTOOLS}/pylocal" >> "${ROUXTOOLS}"/env.sh
...
wasi_sdk() {
  ...
  if [ ! -f "${ROUXTOOLS}/wamrc/wasi-sdk/bin/clang" ]; then
    local wasibasefile
    local wasmbasefile
...
install_build_tools() {
  ...
  mkdir -p "${ROUXTOOLS}"/homebrew
  export HOMEBREW_CACHE=${ROUXTOOLS}/homebrew
  echo "export HOMEBREW_CACHE=${ROUXTOOLS}/homebrew" >> "${ROUXTOOLS}"/env.sh
