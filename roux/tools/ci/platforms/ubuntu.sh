...
bloaty() {
  add_path "${ROUXTOOLS}"/bloaty/bin

  if [ ! -f "${ROUXTOOLS}/bloaty/bin/bloaty" ]; then
    git clone --depth 1 --branch v1.1 https://github.com/google/bloaty "${ROUXTOOLS}"/bloaty-src
    mkdir -p "${ROUXTOOLS}"/bloaty
    cd "${ROUXTOOLS}"/bloaty-src
    cmake -B build -DCMAKE_INSTALL_PREFIX="${ROUXTOOLS}"/bloaty
    cmake --build build
    cmake --build build --target install
    cd "${ROUXTOOLS}"
    rm -rf bloaty-src
    ls -a "${ROUXTOOLS}"/bloaty
  fi
...
mips_gcc_toolchain() {
  add_path "${ROUXTOOLS}"/pinguino-compilers/p32/bin

  if [ ! -d "${ROUXTOOLS}/pinguino-compilers/p32/bin/p32-gcc" ]; then
    local basefile
    basefile=pinguino-linux64-p32
    mkdir -p "${ROUXTOOLS}"/pinguino-compilers
    cd "${ROUXTOOLS}"
    ...
    mv p32 "${ROUXTOOLS}"/pinguino-compilers/p32
    rm ${basefile}.zip
  fi
...
rx_gcc_toolchain() {
  add_path "${ROUXTOOLS}"/renesas-toolchain/rx-elf-gcc/bin

  if [ ! -f "${ROUXTOOLS}/renesas-toolchain/rx-elf-gcc/bin/rx-elf-gcc" ]; then
    ...
    mkdir -p "${ROUXTOOLS}"/renesas-tools/source
    ...
    mv ${basefilebinutils} "${ROUXTOOLS}"/renesas-tools/source/binutils
    ...
    mv ${basefilegcc} "${ROUXTOOLS}"/renesas-tools/source/gcc
    ...
    mv ${basefilenewlib} "${ROUXTOOLS}"/renesas-tools/source/newlib
    rm ${basefilenewlib}.tar.gz

    # Install binutils
    cd "${ROUXTOOLS}"/renesas-tools/source/binutils; chmod +x ./configure ./mkinstalldirs
    mkdir -p "${ROUXTOOLS}"/renesas-tools/build/binutils; cd "${ROUXTOOLS}"/renesas-tools/build/binutils
    "${ROUXTOOLS}"/renesas-tools/source/binutils/configure --target=rx-elf --prefix="${ROUXTOOLS}"/renesas-toolchain/rx-elf-gcc \
      --disable-werror
    make; make install

    # Install gcc
    cd "${ROUXTOOLS}"/renesas-tools/source/gcc
    ...
    mkdir -p "${ROUXTOOLS}"/renesas-tools/build/gcc; cd "${ROUXTOOLS}"/renesas-tools/build/gcc
    "${ROUXTOOLS}"/renesas-tools/source/gcc/configure --target=rx-elf --prefix="${ROUXTOOLS}"/renesas-toolchain/rx-elf-gcc \
    ...
    # Install newlib
    cd "${ROUXTOOLS}"/renesas-tools/source/newlib; chmod +x ./configure
    mkdir -p "${ROUXTOOLS}"/renesas-tools/build/newlib; cd "${ROUXTOOLS}"/renesas-tools/build/newlib
    "${ROUXTOOLS}"/renesas-tools/source/newlib/configure --target=rx-elf --prefix="${ROUXTOOLS}"/renesas-toolchain/rx-elf-gcc
    make; make install
    rm -rf "${ROUXTOOLS}"/renesas-tools/
  fi
...
wasi_sdk() {
  add_path "${ROUXTOOLS}"/wamrc

  if [ ! -f "${ROUXTOOLS}/wasi-sdk/bin/clang" ]; then
    local wasibasefile
    local wasmbasefile
  ...
  export WASI_SDK_PATH="${ROUXTOOLS}/wasi-sdk"
  echo "export WASI_SDK_PATH=${ROUXTOOLS}/wasi-sdk" >> "${ROUXTOOLS}"/env.sh
...
setup_links() {
  ...
  ln -sf "$(which ccache)" "${ROUXTOOLS}"/ccache/bin/rx-elf-gcc
  ln -sf "$(which ccache)" "${ROUXTOOLS}"/ccache/bin/riscv-none-elf-gcc
  ln -sf "$(which ccache)" "${ROUXTOOLS}"/ccache/bin/riscv-none-elf-g++
  ...
  ln -sf "$(which ccache)" "${ROUXTOOLS}"/ccache/bin/xtensa-esp32s2-elf-gcc
  ln -sf "$(which ccache)" "${ROUXTOOLS}"/ccache/bin/xtensa-esp32s2-elf-g++
  ln -sf "$(which ccache)" "${ROUXTOOLS}"/ccache/bin/xtensa-esp32s3-elf-gcc
  ln -sf "$(which ccache)" "${ROUXTOOLS}"/ccache/bin/xtensa-esp32s3-elf-g++
}
