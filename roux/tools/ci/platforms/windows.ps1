...
function add_path() {
  ...
  if ($envPaths -notcontains $Path) {
    Add-Content -Path "$ROUXTOOLS\env.ps1" -Value "add_path $Path"
...
function arm_clang_toolchain {
  ...
  try {
    if (run_command("clang") -ne 0) {
      ...
      if (-not (Test-Path -Path "$ROUXTOOLS\clang-arm-none-eabi\bin\clang.exe")) {
        # Download the file
        Write-Host "Download: ARM clang toolchain" -ForegroundColor Green
        $basefile = "LLVMEmbeddedToolchainForArm-17.0.1-Windows-x86_64"
        Set-Location "$ROUXTOOLS"
        # Download the latest ARM clang toolchain prebuilt by ARM
        Invoke-WebRequest -Uri "https://github.com/ARM-software/LLVM-embedded-toolchain-for-Arm/releases/download/release-17.0.1/$basefile.zip" -OutFile "$ROUXTOOLS\$basefile.zip" -ErrorAction Stop
        Expand-Archive "$ROUXTOOLS\$basefile.zip"
...
function arm_gcc_toolchain() {
  ...
  try {
    if (run_command("arm-none-eabi-gcc") -ne 0) {
      ...
      if (-not (Test-Path -Path "$ROUXTOOLS\gcc-arm-none-eabi\bin\arm-none-eabi-gcc.exe")) {
        # Download the file
        ...
        Invoke-WebRequest -Uri "https://developer.arm.com/-/media/Files/downloads/gnu/13.2.rel1/binrel/$basefile.zip" -OutFile "$ROUXTOOLS\$basefile.zip" -ErrorAction Stop
        Expand-Archive "$ROUXTOOLS\$basefile.zip"
...
function arm64_gcc_toolchain() {
  ...
  try {
    if (run_command("aarch64-none-elf-gcc") -ne 0) {
      ...
      if (-not (Test-Path -Path "$ROUXTOOLS\gcc-aarch64-none-elf\bin\aarch64-none-elf-gcc.exe")) {
        # Download the file
...
function cmake_tool {
  ...
  if (run_command("cmake") -ne 0) {
    ...
    if ($null -eq (Get-Command cmake -ErrorAction SilentlyContinue)) {
      ...
      Set-Location "$ROUXTOOLS"
      # Download tool cmake
      Invoke-WebRequest -Uri "https://github.com/Kitware/CMake/releases/download/v3.31.6/$basefile.zip" -OutFile "$ROUXTOOLS\$basefile.zip" -ErrorAction Stop
      Expand-Archive "$ROUXTOOLS\$basefile.zip"
...
function kconfig_frontends() {
  ...
  try {
    if (-not (Test-Path -Path "$ROUXTOOLS\kconfig-frontends\bin\kconfig-conf.exe")) {
      # Download the file
      ...
      Invoke-WebRequest -Uri "https://github.com/simbit18/kconfig-frontends-windows-mingw64/releases/download/kconfig-frontends-4.11.0/$basefile.zip" -OutFile "$ROUXTOOLS\$basefile.zip" -ErrorAction Stop
      Expand-Archive "$ROUXTOOLS\$basefile.zip"
...
function ninja_tool {
  ...
  if (run_command("ninja") -ne 0) {
    ...
    if ($null -eq (Get-Command ninja -ErrorAction SilentlyContinue)) {
      ...
      Set-Location "$ROUXTOOLS"
      # Download tool ninja
      Invoke-WebRequest -Uri "https://github.com/ninja-build/ninja/releases/download/v1.12.1/$basefile.zip" -OutFile "$ROUXTOOLS\$basefile.zip" -ErrorAction Stop
      Expand-Archive "$ROUXTOOLS\$basefile.zip" # -DestinationPath  "$basefile"
...
function riscv_gcc_toolchain() {
  ...
  try {
    if (run_command("riscv-none-elf-gcc") -ne 0) {
      add_path "$ROUXTOOLS\riscv-none-elf-gcc\bin"
      if (-not (Test-Path -Path "$ROUXTOOLS\riscv-none-elf-gcc\bin\riscv-none-elf-gcc.exe")) {
        ...
        Invoke-WebRequest -Uri "https://github.com/xpack-dev-tools/riscv-none-elf-gcc-xpack/releases/download/v14.2.0-3/$basefile.zip" -OutFile "$ROUXTOOLS\$basefile.zip" -ErrorAction Stop
        Expand-Archive "$ROUXTOOLS\$basefile.zip"
...
function rust() {
  ...
  if (run_command("rustc") -ne 0) {
    ...
    $env:CARGO_HOME = "$ROUXTOOLS\rust\cargo"
    $env:RUSTUP_HOME = "$ROUXTOOLS\rust\rustup"

    Add-Content -Path "$ROUXTOOLS\env.ps1" -Value "CARGO_HOME=$ROUXTOOLS\rust\cargo"
    Add-Content -Path "$ROUXTOOLS\env.ps1" -Value "RUSTUP_HOME=$ROUXTOOLS\rust\rustup"
    ...
    if ($null -eq (Get-Command rustc -ErrorAction SilentlyContinue)) {
      ...
      New-Item -ItemType Directory -Path "$ROUXTOOLS\rust" -Force
      Set-Location "$ROUXTOOLS"
...
function install_build_tools {
  if (-not (Test-Path -Path "$ROUXTOOLS\env.ps1")) {
    add_envpath "$ROUXTOOLS\env.ps1"
  }
