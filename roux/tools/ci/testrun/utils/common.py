...
class connectROUX(object):
    ...
    def setup(self):
        ...
        if self.target == "sim":
            # start roux for sim
            start.startROUX(self, self.path, self.log_path, self.board, self.sudo)
        if self.target == "qemu":
...
class start:
    ...
    def startROUX(self, path, log_path, board="sim", sudo=False):
        os.chdir(path)
        ...
        if sudo:
            if board in ["sim_rpserver", "sim_rpproxy"]:
                os.chdir(path)
                self.process = pexpect.spawn(
                    "bash", ["-c", "sudo ./rpserver/roux/roux | tee %s" % self.log]
                )
            else:
                self.process = pexpect.spawn(
                    "bash", ["-c", "sudo ./roux | tee %s" % self.log]
                )
        else:
            self.process = pexpect.spawn("bash", ["-c", "./roux | tee %s" % self.log])
    ...
    def startQemu(self, path, log_path, board="qemu", sudo=False):
        ...
        if flag1 == "imx6":
            self.process = pexpect.spawn(
                ...
                [
                    "-c",
                    "qemu-system-arm -semihosting -M sabrelite -m 1024 -smp 4 -kernel ./roux -nographic | tee %s"
                    % self.log,
                ],
            )
        if flag1 == "qemu-rv":
            ...
            self.process = pexpect.spawn(
                ...
                [
                    "-c",
                    "%s -M virt -bios ./roux -nographic %s | tee %s"
