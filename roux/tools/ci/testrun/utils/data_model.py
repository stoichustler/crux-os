...
class SuiteInfo:
    def __init__(self, test_suite_name):
        # suite name, e.g. "ROUXMmTestSuites"
        self.test_suite_name = test_suite_name
...
class CmockaSingleCoreRecord:
    def __init__(self, lines, core="", board="", log="", duration=0):
        ...
        case_pattern = r"\]\s+(?P<test_case_name>TestROUX\w+)"
    ...
    def process(self, lines, err_code):
        ...
        case_run_pattern = r"\[\s+RUN\s+\] (?P<test_case_name>TestROUX\w+)"
        case_pass_pattern = r"\[\s+OK\s+\] (?P<test_case_name>TestROUX\w+)"
        case_fail_pattern = r"\[\s+FAILED\s+\] (?P<test_case_name>TestROUX\w+)"
