...
static struct reserved_s g_reserved[] = {
	{TOKEN_COMMENT, "comment"}, {TOKEN_CONFIG, "config"},	  {TOKEN_MENUCONFIG, "menuconfig"},
	{TOKEN_BOOL, "bool"},		{TOKEN_TRISTATE, "tristate"}, {TOKEN_INT, "int"},
	{TOKEN_HEX, "hex"},			{TOKEN_STRING, "string"},	  {TOKEN_DEFAULT, "default"},
	{TOKEN_RANGE, "range"},		{TOKEN_SELECT, "select"},	  {TOKEN_DEPENDS, "depends"},
	{TOKEN_ON, "on"},			{TOKEN_OPTION, "option"},	  {TOKEN_HELP, "help"},
	{TOKEN_HELP, "---help---"}, {TOKEN_MAINMENU, "mainmenu"}, {TOKEN_MENU, "menu"},
	{TOKEN_ENDMENU, "endmenu"}, {TOKEN_CHOICE, "choice"},	  {TOKEN_ENDCHOICE, "endchoice"},
	{TOKEN_PROMPT, "prompt"},	{TOKEN_SOURCE, "source"},	  {TOKEN_IF, "if"},
	{TOKEN_ENDIF, "endif"},		{TOKEN_NOTRESERVED, NULL}};
...
static inline void process_help(FILE *stream, output_t outfunc)
{
	...
	for (;;) {
		 ...
		 * rather simply a ROUX Kconfig convention.
		 */
...
static inline char *process_config(FILE *stream, const char *varname, const char *kconfigdir,
								   const char *kconfigname)
{
	...
	if (!hidden) {
		paranum = get_paranum();
		output("<li><a href=\"#CONFIG_%s\">%s <code>CONFIG_%s</code>", config.c_name, paranum,
			   config.c_name);
		outfunc("%s ", paranum);
		incr_paranum();
	}
...
int main(int argc, char **argv, char **envp)
{
	...
	output("<html>\n");
	output("<head>\n");
	output("<title>ROUX Configuration Options</title>\n");
	output("</head>\n");
	...
	output("<h1><big><font color=\"#3c34ec\">"
		   "<i>ROUX Configuration Variables</i></font></big></h1>\n");
	...
	body("<p>\n");
	body("  <b>Overview</b>.\n");
	body("  The ROUX RTOS is highly configurable.\n");
	body("  The ROUX configuration files are maintained using the "
		 "kconfig-frontends</a> tool.\n");
	body("  That configuration tool uses <code>Kconfig</code> "
		 "files that can be found through the ROUX source tree.\n");
	...
	body("  Each configuration variable provides one configuration "
		 "option for the ROUX RTOS.\n");
	body("  This configurable options are described in this document.\n");
	body("</p>\n");
	body("<p>\n");
	body("  <b>Main Menu</b>.\n");
	body("  The normal way to start the ROUX configuration is to enter "
		 "this command line from the ROUX build directory: "
		 "<code>make menuconfig</code>.\n");
	body("  Note that ROUX must first be configured <i>before</i> "
	...
	body("  This documentation was auto-generated using the "
		 "kconfig2html tool\n");
	body("  That tool analyzes the ROUX <code>Kconfig</code> "
