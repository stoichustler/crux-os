...
static const char *convert_path(const char *path)
{
	...
	if (g_winpath) {
		...
		size = cygwin_conv_path(CCP_POSIX_TO_WIN_A | CCP_RELATIVE, g_dequoted, NULL, 0);
		if (size > (MAX_PATH - 3)) {
			fprintf(stderr, "# ERROR: POSIX path too long: %zd\n", size);
			exit(EXIT_FAILURE);
		}

		ret = cygwin_conv_path(CCP_POSIX_TO_WIN_A | CCP_RELATIVE, g_dequoted, &g_posixpath[1],
							   MAX_PATH - 3);
		if (ret < 0) {
...
static void do_dependency(const char *file)
{
	...
	if (g_objpath) {
		...
		snprintf(tmp, NAME_MAX + 6, "%s%s%c%s%s ", moption[1], g_objpath, separator, objname,
				 g_suffix);
