...
const char *class_text[] = {"info", "warning", "error", "fatal"};
...
static const char *g_white_prefix[] = {
	"ASCII_",  /* Ref:  include/roux/ascii.h */
...
static const char *g_white_suffix[] = {
	/* Ref:  include/roux/wireless/nrf24l01.h */
...
static const char *g_white_content_list[] = {
	/* Ref:
	 * roux-apps/examples/wamr_module/module_hello.c
	 * roux-apps/interpreters/wamr/wamr_custom_init.c
	 *
	 * They are from the WAMR project.
	 */

	"NativeSymbol", "RuntimeInitArgs",
	...
	"Address", "Description", "FirstUnicastAddress", "GetAdaptersAddresses", "GetProcAddress",
	"LoadLibrary", "lpSockaddr", "Next", "PhysicalAddressLength", "PhysicalAddress",
	"WideCharToMultiByte",
	...
	"DataType", "Offset", "Prio", "pU32_Value", "RangeMax", "RangeMin", "SEGGER_SYSVIEW",
	"ScalingFactor", "sName", "sUnit", "StackBase", "StackSize", "TaskID",
	...
	"CREATE3args", "CREATE3resok", "LOOKUP3args", "LOOKUP3filename", "LOOKUP3resok", "WRITE3args",
	"WRITE3resok", "READ3args", "READ3resok", "REMOVE3args", "REMOVE3resok", "RENAME3args",
	"RENAME3resok", "MKDIR3args", "MKDIR3resok", "RMDIR3args", "RMDIR3resok", "READDIR3args",
	"READDIR3resok", "SETATTR3args", "SETATTR3resok", "FS3args", "SIZEOF_rpc_reply_read",
	"SIZEOF_rpc_call_write", "SIZEOF_rpc_reply_readdir", "SIZEOF_nfsmount",

	/* Ref:
	 * mm/kasan/kasan.c
	 */

	"__asan_loadN", "__asan_storeN", "__asan_loadN_noabort", "__asan_storeN_noabort",
	...
	"RTOS_Init", "RTOS_GetVersion", "RTOS_GetSymbols", "RTOS_GetNumThreads",
	"RTOS_GetCurrentThreadId", "RTOS_GetThreadId", "RTOS_GetThreadDisplay", "RTOS_GetThreadReg",
	"RTOS_GetThreadRegList", "RTOS_GetThreadRegList", "RTOS_SetThreadReg", "RTOS_SetThreadRegList",
	"RTOS_UpdateThreads",
	...
	"Display", "Button1Mask", "Button2Mask", "Button3Mask", "Button1", "Button2", "Button3",
	"XEvent", "XPending", "XNextEvent", "KeyPress", "KeyRelease", "MotionNotify", "ButtonPress",
	"ButtonRelease", "XLookupKeysym",

	/* Ref:
	 * sim/posix/sim_x11framebuffer.c
	 */

	"Window", "XShmSegmentInfo", "XImage", "XGCValues", "XTextProperty", "XSizeHints",
	"XOpenDisplay", "XCreateSimpleWindow", "DefaultRootWindow", "XStringListToTextProperty",
	"XSetWMProperties", "XMapWindow", "XSelectInput", "XAllowEvents", "XGrabButton", "XCreateGC",
	"XSetErrorHandler", "XSync", "XShmDetach", "XDestroyImage", "XUngrabButton", "XCloseDisplay",
	"XShmQueryExtension", "XShmCreateImage", "XShmAttach", "DefaultVisual", "XCreateImage",
	"XGetWindowAttributes", "DefaultColormap", "XAllocColor", "XShmPutImage", "XPutImage",
	"Colormap", "DefaultScreen", "BlackPixel", "PSize", "PMinSize", "PMaxSize", "ButtonPressMask",
	"ButtonReleaseMask", "PointerMotionMask", "KeyPressMask", "KeyReleaseMask", "ButtonMotionMask",
	"GrabModeAsync", "GCGraphicsExposures", "XErrorEvent", "AnyModifier", "None", "Status",
	"DoGreen", "DoRed", "DoBlue", "ZPixmap", "readOnly", "XWindowAttributes", "XColor", "AsyncBoth",
	"CurrentTime", "XUnmapWindow", "XFree",

	/* Ref:
	 * roux/arch/sim/src/sim_hostdecoder.*
	 */

	"ISVCDecoder", "SBufferInfo", "SDecodingParam", "eEcActiveIdc", "sVideoProperty",
	"eVideoBsType", "cmResultSuccess", "uiInBsTimeStamp", "dsErrorFree", "iBufferStatus", "UsrData",
	"sSystemBuffer", "iWidth", "iHeight", "iStride", "uiOutYuvTimeStamp", "WelsCreateDecoder",
	"WelsDestroyDecoder", "Initialize", "Uninitialize", "DecodeFrame2", "FlushFrame", "SetOption",
	"GetOption",
	...
	"inflateInit", "inflateEnd", "Bytef",

	/* Ref:
	 * sim/posix/sim_hostmemory.c
	 */

	"CreateFileMapping", "MapViewOfFile", "CloseHandle", "UnmapViewOfFile",
	...
	"CreateWaitableTimer", "SetWaitableTimer", "WaitForSingleObject", "dwHighDateTime",
	"dwLowDateTime", "QuadPart",

	/* Ref:
	 * sim/posix/sim_hostuart.c
	 */

	"GetStdHandle", "GetConsoleMode", "SetConsoleMode", "WriteConsole", "ReadConsole",
	...
	"bRequestType", "bRequest", "wValue", "wIndex", "wLength", "bLength", "bDescriptorType",
	...
	"bNumConfigurations", "bDeviceClass", "idVendor", "idProduct",
	...
	"unzFile", "uLong", "unzOpen2_64", "unzLocateFile", "unzOpenCurrentFile", "unzClose",
	"unzReadCurrentFile", "unzGetCurrentFileInfo64", "unzGoToNextFile", "unzGoToFirstFile",

	/* Ref:
	 * apps/netutils/telnetc/telnetc.c
	 */

	"deflateInit", "deflateEnd", "inflateInit", "inflateEnd", "zError",

	/* Ref:
	 * apps/tee/libteec/optee_client/libteec/include/tee_client_api.h
	 */

	"clockSeqAndNode", "paramTypes", "timeLow", "timeMid", "timeHiAndVersion",

	NULL};

static const char *g_white_headers[] = {"windows.h",

										/* Ref:
										 * arch/tricore/src/common/tricore_serial.c
										 */

										"IfxAsclin_Asc.h", NULL};
...
static int message(enum class_e class, const char *text, int lineno, int ndx)
{
	...
	if (g_verbose == 2) {
		if (lineno == -1 && ndx == -1) {
			...
			fprintf(out, "%s:%d:%d: %s: %s\n", g_file_name, lineno, ndx, class_text[class], text);
		}
	}
...
static void check_spaces_left(char *line, int lineno, int ndx)
{
	...
	if (ndx-- > 0 && line[ndx] != ' ' && line[ndx] != '(' && line[ndx] != ')') {
		ERROR("Operator/assignment must be preceded with whitespace", lineno, ndx);
	}
}
...
static void check_spaces_leftright(char *line, int lineno, int ndx1, int ndx2)
{
	if (ndx1 > 0 && line[ndx1 - 1] != ' ') {
		ERROR("Operator/assignment must be preceded with whitespace", lineno, ndx1);
	}

	if (line[ndx2 + 1] != '\0' && line[ndx2 + 1] != '\n' && line[ndx2 + 1] != ' ') {
		ERROR("Operator/assignment must be followed with whitespace", lineno, ndx2);
	}
}
...
{
	if (ndx1 > 0 && line[ndx1 - 1] == ' ') {
		ERROR("There should be no spaces before the operator/assignment", lineno, ndx1);
	}

	if (line[ndx2 + 1] == ' ') {
		ERROR("There should be no spaces after the operator/assignment", lineno, ndx2);
	}
}
...
static void check_operand_leftright(char *line, int lineno, int ndx1, int ndx2)
{
	...
	if (ndx1 > 0 && (line[ndx1 - 1] == ' ' || line[ndx1 - 1] == '(' || line[ndx1 - 1] == '[') &&
		(line[ndx2 + 1] == ' ' || line[ndx2 + 1] == '\0' || line[ndx2 + 1] == ';' ||
...
static int block_comment_width(char *line)
{
	...
	if (strncmp(&line[b], "/***", 4) == 0 && strncmp(&line[e - 2], "***", 3) == 0) {
	...
	if (strncmp(&line[b], "***", 3) == 0 && strncmp(&line[e - 3], "***/", 4) == 0) {
		/* Return the the length of the line up to the final '*' */
	...
	if (strncmp(&line[b], "/*", 2) == 0 && strncmp(&line[e - 3], "***/", 4) == 0) {
		/* Return the the length of the line up to the final '*' */
...
static bool white_file_list(const char *filename)
{
	...
	for (pptr = g_white_files; (str = *pptr) != NULL; pptr++) {
...
static bool white_content_list(const char *ident, int lineno)
{
	...
	for (pptr = g_white_prefix; (str = *pptr) != NULL; pptr++) {
		len = strlen(str);
	...
	for (pptr = g_white_headers; (str = *pptr) != NULL; pptr++) {
	...
	for (pptr = g_white_suffix; (str = *pptr) != NULL; pptr++) {
		len = strlen(str);
	...
	for (pptr = g_white_content_list; (str = *pptr) != NULL; pptr++) {
		len = strlen(str);
		if (strncmp(ident, str, len) == 0 && isalnum(ident[len]) == 0) {
			return true;
		}
	}
...
int main(int argc, char **argv, char **envp)
{
	...
	while (fgets(line, LINE_SIZE, instream)) {
		...
		} else /* This line is non-blank */
		{
			...
			if (lineno == comment_lineno + 1) {
				...
				if (line[n] != '}' && line[n] != '#' && prevrhcmt == 0) {
					ERROR("Missing blank line after comment", comment_lineno, 1);
				}
			}
			...
			if (lineno == 2) {
				if (line[n] == '*' && line[n + 1] == '\n') {
					ERROR("Missing relative file path in file header", lineno, n);
				} else if (isspace(line[n + 2])) {
					ERROR("Too many whitespaces before relative file path", lineno, n);
				} else {
...
#ifdef TOPDIR
					/* TOPDIR macro contains the absolute path to the "roux"
					...
					if (basedir != NULL) {
						...
						char *line_dup = strndup(&line[n + 2], strlen(&line[n + 2]) - 1);

						if (strcmp(line_dup, basedir + offset) != 0) {
							ERROR("Relative file path does not match actual file", lineno, n);
						}

						free(line_dup);
					} else if (strncmp(&line[n + 2], apps_dir, apps_len) != 0) {
						/* g_file_name neither belongs to "roux" repository
						 * nor begins with the root dir of the other
						 * repository (e.g. "apps/")
						 */

						ERROR("Path relative to repository other than \"roux\" "
							  "must begin with the root directory",
							  lineno, n);
					} else {
#endif
						...
						ssize_t base = strlen(g_file_name) - strlen(&line_dup[offset]);
			...
			if (bfunctions && lineno == rbrace_lineno + 1) {
				...
				if (dnest == 0 && strchr(line, '}') == NULL && line[n] != '#' &&
					strncmp(&line[n], "else", 4) != 0 && strncmp(&line[n], "while", 5) != 0 &&
					strncmp(&line[n], "break", 5) != 0) {
					ERROR("Right brace must be followed by a blank line", rbrace_lineno, n + 1);
				}
				...
				if (line[n] == '#') {
					...
					if (strncmp(&line[ii], "else", 4) != 0 && strncmp(&line[ii], "elif", 4) != 0) {
		...
		for (n = 0; line[n] != '\n' && isspace((int)line[n]); n++) {
			...
			default: {
				snprintf(buffer, sizeof(buffer), "Unexpected white space character %02x found",
						 line[n]);
		...
		if (line[indent] == '#' || ppline != PPLINE_NONE) {
				...
				if (line[ii] != '\0') {
					...
					if (strncmp(&line[ii], "define", 6) == 0) {
						...
						if (g_section != PRE_PROCESSOR_DEFINITIONS) {
							...
							if (g_section == NO_SECTION && g_file_type != C_HEADER) {
			...
			if (ppline == PPLINE_IF || ppline == PPLINE_ELIF) {
				...
				if (bdef) {
					while (line[ii] != '\0' && (isspace(line[ii]) || line[ii] == '(')) {
						ii++;
					}
			...
			if (lptr != NULL) {
				...
				if (ppline == PPLINE_DEFINE) {
					...
					if (prevrhcmt > 0 && n != prevrhcmt) {
						rhcomment = prevrhcmt;
						WARN("Wrong column position of comment right of code", lineno, n);
					}
				} else {
					...
					if (ncomment > 0 &&
						(ppline == PPLINE_IF || ppline == PPLINE_ELSE || ppline == PPLINE_ELIF)) {
						/* in #if...  and #el... */

						ERROR("No multiline comment right of code allowed here", lineno, n);
					}
				}
			}
		...
		if (linelen >= 5) {
			lptr = strstr(line, "*/");
			if (line[indent] == '/' && line[indent + 1] == '*' && lptr - line == linelen - 3) {
				...
				if (lineno > 1 && comment_lineno != lineno - 1 && blank_lineno != lineno - 1 &&
					noblank_lineno != lineno - 1 && rhcomment == 0) {
		...
		for (; line[n] != '\n' && line[n] != '\0'; n++) {
			...
			if (ncomment == 0 && !bstring && (line[n] == '_' || isalpha(line[n]))) {
				...
				do {
					...
					if (!have_lower && islower(line[n])) {
						...
						case 'v':
							if (n > 1 && line[n - 2] == 'I' && line[n - 1] == 'P' &&
								(line[n + 1] == '4' || line[n + 1] == '6')) {
							} else if (n > 3 && line[n - 4] == 'I' && line[n - 3] == 'C' &&
									   line[n - 2] == 'M' && line[n - 1] == 'P' &&
									   line[n + 1] == '6') {
							} else if (n > 3 && line[n - 4] == 'I' && line[n - 3] == 'G' &&
									   line[n - 2] == 'M' && line[n - 1] == 'P' &&
									   line[n + 1] == '2') {
							} else {
								have_lower = true;
						...
						case 'p':
							if (!have_upper || n < 1 || !isdigit(line[n - 1]) ||
								!isdigit(line[n + 1])) {
								have_lower = true;
							}
							break;
				...
				if (have_upper && have_lower) {
					...
					if (g_skipmixedcase || white_content_list(&line[ident_index], lineno)) {
						/* No error */
					}
			...
			if (line[n] == '/' && !bstring) {
				...
				/* Check for end of a C comment */
					...
					if (n < 2) {
						...
						ERROR("Missing space before closing C comment", lineno, n);
					}
				...
				/* Check for C++ style comments */
					...
					if ((ncomment == 0) && (n == 0 || strncmp(&line[n - 1], "://", 3) != 0)) {
			...
			if (ncomment == 0 && !bstring && inasm == 0) {
				...
				case '{': {
					if (n > indent) {
						...
						if (dnest == 0 || !bfunctions || lineno == rbrace_lineno) {
							ERROR("Left bracket not on separate line", lineno, n);
						}
					} else if (line[n + 1] != '\n') {
						if (dnest == 0) {
					...
					if (lineno == externc_lineno || lineno - 1 == externc_lineno) {
						bexternc = true;
					}
				...
				case '}': {
					...
					if (n > indent) {
						if (dnest == 0) {
							ERROR("Right bracket not on separate line", lineno, n);
						}
					}
					...
					if (line[n + 1] != '\n' && line[n + 1] != ',' && line[n + 1] != ';') {
						...
						if (line[sndx] == '_' || isalpha(line[sndx])) {
							...
							while (line[endx] == '_' || isalnum(line[endx])) {
								endx++;
							}
							...
							if (strncmp(&line[sndx], "while", 5) == 0) {
								ERROR("'while' must be on a separate line", lineno, sndx);
							} else if (line[endx] == ',') {
								ERROR("Multiple data definitions on line", lineno, endx);
							} else if (line[endx] == ';') {
								if (whitespace) {
									ERROR("Space precedes semi-colon", lineno, endx);
								}
							} else if (line[endx] == '=') {
					...
					if (lineno == blank_lineno + 1) {
						ERROR("Blank line precedes right brace at line", lineno, 1);
					}
				...
				case ':': {
					if (bcase == true) {
						...
						if (*ndx != '\0' && *ndx != '/') {
							ERROR("Case statement should be on a new line", lineno, n);
						}
				...
				case '-':
					...
					else if ((line[n - 1] == 'e' || line[n - 1] == 'E') && isdigit(line[n + 1]) &&
							 isdigit(line[n - 2])) {
						n++;
					} else {
				...
				case '&':
					...
					if (isalpha((int)line[n + 1]) || line[n + 1] == '_' || line[n + 1] == '(') {
					}
				...
				case '*':
					...
					else if (isalpha((int)line[n + 1]) || line[n + 1] == '_' ||
							 line[n + 1] == '(') {
						break;
					}
		...
		if (line[n] == '\n' || line[n] == '\0') {
			...
			if (m > 1 && isspace((int)line[m - 1]) && line[m - 1] != '\n' && line[m - 1] != '\r') {
			 ...
			 */
				...
				for (bslash = false, a = m; a > 2 && strchr("\n\r/", line[a]) != NULL; a--) {
					if (line[a] == '/') {
						bslash = true;
					}
				}
		...
		if ((ncomment > 0 || prevncomment > 0) && !bstring) {
			...
			if (indent == 0 && line[0] != '/' && !bexternc) {
				...
				if (line[indent] == '/') {
					...
					if ((indent & 3) != 3 && bfunctions && dnest == 0 && rhcomment == 0) {
	...
	fclose(instream);
	if (g_verbose == 1) {
		fprintf(stderr, "%s: %s nxstyle check\n", g_file_name, g_status == 0 ? "PASSED" : "FAILED");
	}
