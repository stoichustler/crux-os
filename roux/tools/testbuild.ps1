...
# Check if roux directory exists
if (-Not (Test-Path -Path $roux)) {
  Write-Host "ERROR: Expected to find roux/ at $roux" -ForegroundColor Red
  showusage
}
...
function distclean {
  ...
  if ((Test-Path ".config") -or (Test-Path "build\.config")) {
    if (($GITCLEAN -eq 1) -or ($cmake)) {
      ...
      if ($CHECKCLEAN -ne 0) {
        if ((Test-Path -Path "$roux\.git") -or (Test-Path -Path "$APPSDIR\.git")) {
          try {
            if (git -C $roux status --ignored -s 2>$null) {
              git -C $roux status --ignored -s
              $global:fail = 1
            }
...
function configure_cmake {
  ...
  if ($toolchain) {
    ...
    $contentconfig = Get-Content "$roux\build\.config"
    ...
    if ($original_toolchain) {
      ...
      kconfig-tweak.ps1 --file "$roux\build\.config" -d $original_toolchain
    }

    Write-Host "  Enabling $toolchain"
    kconfig-tweak.ps1 --file "$roux\build\.config" -e $toolchain
  }
}
...
function build_cmake {
  ...
  if ($SAVEARTIFACTS -eq 1) {
    ...
    try {
      ...
      $contentmanifest = Get-Content "$roux\build\roux.manifest"
      $tmpconfig = $config -replace '\\', ':'
      if (($tmpconfig -match "windows") -and ($MSVC -eq 1)) {
        Copy-Item -Path "$roux\build\Debug\roux.exe" -Destination $artifactconfigdir -Force -ErrorAction Stop
        Copy-Item -Path "$roux\build\Debug\roux.pdb" -Destination $artifactconfigdir -Force -ErrorAction Stop
      }
      else {
        foreach ($ma in $contentmanifest) {
          Copy-Item -Path "$roux\build\$ma" -Destination $artifactconfigdir -Force -ErrorAction Stop
        }
      }
    }
    catch {
...
function refresh_cmake {
  ...
  if ($toolchain) {
    if ($original_toolchain) {
      kconfig-tweak.ps1 --file "$roux\build\.config" -e $original_toolchain
    } 
    kconfig-tweak.ps1 --file "$roux\build\.config" -d $toolchain
  }
  ...
  if ($CHECKCLEAN -ne 0) {
    if ((Test-Path -Path "$roux\.git") -or (Test-Path -Path "$APPSDIR\.git")) {
      try {
        if (git -C $roux status -s 2>$null) {
          Write-Host "Git $roux status " -ForegroundColor Yellow
          git -C $roux status -s
          $global:fail = 1
        }
