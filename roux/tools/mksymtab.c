...
int main(int argc, char **argv, char **envp)
{
	...
	fprintf(outstream, "#include <roux/config.h>\n");
	fprintf(outstream, "#include <roux/compiler.h>\n");
	fprintf(outstream, "#include <roux/symtab.h>\n\n");
	...
	while ((ptr = read_line(instream)) != NULL) {
		...
		if (parm1) {
			fprintf(outstream, "%s  { \"%s\", (FAR const void *)%s }", nextterm, g_parm[NAME_INDEX],
					g_parm[NAME_INDEX]);
		} else {
			fprintf(outstream, "%s  { \"%s\", (FAR const void *)&%s }", nextterm,
