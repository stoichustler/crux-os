...
while [ ! -z "$1" ]; do
  ...
  -v )
    ...
    ;;
  -h )
    echo "$0 is a tool for generation of proper version files for the ROUX build"
    echo ""
    echo $USAGE
    echo ""
    echo "Where:"
    ...
    echo "    The ROUX version number expressed as a major, minor and patch"
...
if [ -z ${VERSION} ] ; then
  VERSION=`git -C ${WD} describe --match "roux-*" 2>/dev/null | tail -1 | cut -d'-' -f2`

  # If the VERSION does not match X.Y.Z, retrieve version from the tag

  if [[ ! ${VERSION} =~ ([0-9]+)\.([0-9]+)\.([0-9]+) ]] ; then
    VERSION=`git -C ${WD} -c 'versionsort.suffix=-' tag --sort=v:refname 2>/dev/null | grep -E "roux-[0-9]+\.[0-9]+\.[0-9]+" | tail -1 | cut -d'-' -f2-`
  fi
