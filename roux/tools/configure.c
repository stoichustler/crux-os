...
typedef void (*config_callback)(const char *boarddir, const char *archname, const char *chipname,
								const char *boardname, const char *configname, void *data);
static void config_search(const char *boarddir, config_callback callback, void *data);
static void archname_callback(const char *boarddir, const char *archname, const char *chipname,
							  const char *boardname, const char *configname, void *data);
static void find_archname(void);
static void enumerate_callback(const char *boarddir, const char *archname, const char *chipname,
							   const char *boardname, const char *configname, void *data);
...
static char *g_topdir = NULL;	   /* Full path to top-level ROUX build directory */
...
static const char *g_optfiles[] = {".gdbinit", ".cproject", ".project"};
...
static void show_usage(const char *progname, int exitcode)
{
	...
	fprintf(stderr, "    path from the top-level ROUX build directory.\n");
	...
	fprintf(stderr, "    board directory under roux%cboards%c.\n", g_delim, g_delim);
	...
	fprintf(stderr, "    under roux%cboards%c<board-name>%cconfigs%c.\n", g_delim, g_delim, g_delim,
			g_delim);
...
static void find_topdir(void)
{
	...
	/* Get and verify the top-level ROUX directory */

	/* First get the current directory.  We expect this to be either
	 * the roux root directory or the tools subdirectory.
	 */
	...
	/* Check if the current directory is the roux root directory.
	...
	if (check_directory(g_buffer)) {
		 ...
		 * We must have already been in the roux root directory.  We
...
static void archname_callback(const char *boarddir, const char *archname, const char *chipname,
							  const char *boardname, const char *configname, void *data)
{
	if (strcmp(g_boarddir, boardname) == 0 && strcmp(g_configdir, configname) == 0) {
...
static void check_configdir(void)
{
	if (g_configpath && verify_optiondir(g_configpath)) {
		...
		snprintf(g_buffer, BUFFER_SIZE, "%s%c..%c..%cscripts", g_configpath, g_delim, g_delim,
				 g_delim);
		...
		snprintf(g_buffer, BUFFER_SIZE, "%s%cboards%c%s%c%s%c%s%cconfigs%c%s", g_topdir, g_delim,
				 g_delim, g_archdir, g_delim, g_chipdir, g_delim, g_boarddir, g_delim, g_delim,
				 g_configdir);
		...
		snprintf(g_buffer, BUFFER_SIZE, "%s%cboards%c%s%c%s%c%s%cscripts", g_topdir, g_delim,
				 g_delim, g_archdir, g_delim, g_chipdir, g_delim, g_boarddir, g_delim);
...
static void check_appdir(void)
{
	...
	if (!g_appdir) {
		...
		if (g_winnative == g_oldnative) {
			...
			struct variable_s *var = find_variable("CONFIG_APPS_DIR", g_configvars);
		...
		/* Try ../apps-xx.yy where xx.yy are the ROUX version number */
	...
	if (!verify_file(g_buffer)) {
		...
		if (g_scriptspath != NULL) {
			...
			if (!verify_file(g_buffer)) {
				...
				snprintf(g_buffer, BUFFER_SIZE, "%s%c..%c..%c..%ccommon%cscripts%cMake.defs",
						 g_configpath, g_delim, g_delim, g_delim, g_delim, g_delim, g_delim);
				if (!verify_file(g_buffer)) {
...
static void copy_optional(void)
{
	...
	for (i = 0; i < N_OPTFILES; i++) {
		snprintf(g_buffer, BUFFER_SIZE, "%s%c%s", g_configpath, g_delim, g_optfiles[i]);
...
static void enable_feature(const char *destconfig, const char *varname)
{
	...
	snprintf(g_buffer, BUFFER_SIZE, "kconfig-tweak --file %s --enable %s", destconfig, varname);
...
static void disable_feature(const char *destconfig, const char *varname)
{
	...
	snprintf(g_buffer, BUFFER_SIZE, "kconfig-tweak --file %s --disable %s", destconfig, varname);
...
static void configure(void)
{
	...
	if (g_needapppath) {
		...
		if (!stream) {
			fprintf(stderr, "ERROR: Failed to open %s for append mode: %s\n", destconfig,
					strerror(errno));
			exit(EXIT_FAILURE);
		}
...
int main(int argc, char **argv, char **envp)
{
	...
	debug("main: Checking ROUX Directories\n");
	find_topdir();
	check_configdir();
	check_configured();
