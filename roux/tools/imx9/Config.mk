...
# These are the macros that will be used in the ROUX make system to compile
...
define POSTBUILD
	...
	$(Q) $(OBJCOPY) -O binary -R .bss -R .initstack $(BIN) roux.bin
	$(Q) ([ $$? -eq 0 ] && echo "Done.")
	...
	+$(Q) $(MAKE) -C $(TOPDIR)/tools/imx9 -f Makefile.host
	$(Q) tools/imx9/mkimage_imx9$(HOSTEXEEXT) -soc IMX9 -append $(BASE_PATH)$(AHAB)/mx93a1-ahab-container.img -c -ap roux.bin a55 0x2049a000 -out flash.bin 1>/dev/null 2>&1
	$(Q) dd if=/dev/zero of=imx9-sdimage.img bs=1k count=32 1>/dev/null 2>&1
	$(Q) cat flash.bin >> imx9-sdimage.img
	$(Q) rm flash.bin
	$(Q) echo "imx9-sdimage.img" >> roux.manifest
	$(Q) echo "Created imx9-sdimage.img"


	$(Q) sh tools/imx9/fspi_fcb_gen.sh tools/imx9//fspi_header
	$(Q) tools/imx9/mkimage_imx9$(HOSTEXEEXT) -soc IMX9 -dev flexspi -append $(BASE_PATH)$(AHAB)/mx93a1-ahab-container.img -c -ap roux.bin a55 0x2049a000 -fcb fcb.bin 0x204F0000 -out flash.bin 1>/dev/null 2>&1
	...
	$(Q) rm flash.bin
	$(Q) echo "imx9-norimage.img" >> roux.manifest
	$(Q) echo "Created imx9-norimage.img"
	$(Q) $(MAKE) -C $(TOPDIR)/tools/imx9 -f Makefile.host clean
endef
endif
