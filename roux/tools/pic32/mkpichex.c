...
{
	fprintf(stderr, "USAGE: %s <abs path to roux.hex>\n", progname);
	exit(1);
}
...
int main(int argc, char **argv, char **envp)
{
	...
	srcfile = getfilepath(argv[1], "roux", "hex");
	if (!srcfile) {
		fprintf(stderr, "getfilepath failed\n");
		exit(2);
	}

	destfile = getfilepath(argv[1], "roux", "tmp");
	if (!destfile) {
		fprintf(stderr, "getfilepath failed\n");
		errcode = 2;
	...
	/* Remove the original roux.hex file */

	if (remove(srcfile) != 0) {
		fprintf(stderr, "Failed to remove the old '%s'\n", srcfile);
	}

	/* Rename the new roux.tmp file to roux.hex */
