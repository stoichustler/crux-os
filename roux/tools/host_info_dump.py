...
def verbose(info, logs):
    ...
    if args.verbose:
        ...
        out_str += "ROUX RTOS info: {}\n\n".format(info["OS_VERSION"])
        if args.flags:
            out_str += "ROUX CFLAGS: {}\n\n".format(info["ROUX_CFLAGS"])
            out_str += "ROUX CXXFLAGS: {}\n\n".format(info["ROUX_CXXFLAGS"])
            out_str += "ROUX LDFLAGS: {}\n\n".format(info["ROUX_LDFLAGS"])

        if args.config:
            out_str += "ROUX configuration options: \n"
            for i in range(len(info["ROUX_CONFIG"])):
                out_str += "  " + info["ROUX_CONFIG"][i].replace('"', '\\"') + "\n"
            out_str += "\n\n"
...
def get_compilation_flags(flags):
    """
    Gets the compilation flags used to compile the ROUX source code and splits
    them into a list.
...
def generate_header(args):
    ...
    and ROUX configuration that can be used by ROUX applications.
    ...
    output += " * can be used by ROUX applications.\n"
    output += " *\n"
    output += " ****************************************************************************/\n\n"
    ...
    if args.flags:
        ...
        info["ROUX_CFLAGS"] = get_compilation_flags(cflags)
        info["ROUX_CXXFLAGS"] = get_compilation_flags(cxxflags)
        info["ROUX_LDFLAGS"] = get_compilation_flags(ldflags)

        output += "#define ROUX_CFLAGS_ARRAY_SIZE {}\n".format(
            len(info["ROUX_CFLAGS"])
        )
        output += "static const char *ROUX_CFLAGS[ROUX_CFLAGS_ARRAY_SIZE] =\n{\n"
        for i in range(len(info["ROUX_CFLAGS"])):
            output += '  "' + info["ROUX_CFLAGS"][i] + '",\n'
        output += "};\n\n"

        output += "#define ROUX_CXXFLAGS_ARRAY_SIZE {}\n".format(
            len(info["ROUX_CXXFLAGS"])
        )
        output += "static const char *ROUX_CXXFLAGS[ROUX_CXXFLAGS_ARRAY_SIZE] =\n{\n"
        for i in range(len(info["ROUX_CXXFLAGS"])):
            output += '  "' + info["ROUX_CXXFLAGS"][i] + '",\n'
        output += "};\n\n"

        output += "#define ROUX_LDFLAGS_ARRAY_SIZE {}\n".format(
            len(info["ROUX_LDFLAGS"])
        )
        output += "static const char *ROUX_LDFLAGS[ROUX_LDFLAGS_ARRAY_SIZE] =\n{\n"
        for i in range(len(info["ROUX_LDFLAGS"])):
            output += '  "' + info["ROUX_LDFLAGS"][i] + '",\n'
        output += "};\n\n"
        ...
        with open(config_path, "r") as f:
            for line in f:
                ...
                if not line or line.startswith("#"):
                    continue
                info["ROUX_CONFIG"].append(line)
    except FileNotFoundError:
        print("Error: ROUX configuration file not found: {}".format(config_path))
        sys.exit(1)

    if args.config:
        output += "#define ROUX_CONFIG_ARRAY_SIZE {}\n".format(
            len(info["ROUX_CONFIG"])
        )
        output += "static const char *ROUX_CONFIG[ROUX_CONFIG_ARRAY_SIZE] =\n{\n"
        for i in range(len(info["ROUX_CONFIG"])):
            output += '  "' + info["ROUX_CONFIG"][i].replace('"', '\\"') + '",\n'
        output += "};\n\n"
    ...
    if args.target_info:
        ...
        arch_chip = [s for s in info["ROUX_CONFIG"] if arch_chip_key in s][0].split(
            arch_chip_key
        )[-1]
        if "esp" in arch_chip:
            ...
            vendor_output, vendor_build_output = get_vendor_info(info["ROUX_CONFIG"])
...
if __name__ == "__main__":
    ...
    Required arguments:
        roux_path: The path to the ROUX source directory.
    ...
    Optional arguments:
        The command line arguments. The available arguments are:
            ...
            -c, --config:
                        Get the ROUX compilation configurations used.
            -f, --flags <CFLAGS> <CXXFLAGS> <LDFLAGS>:
                        Provide the ROUX compilation flags used.
            --verbose:
    ...
    parser.add_argument("roux_path", help="ROUX source directory path.")
    parser.add_argument(
        "-h",
        "--help",
        action="help",
    ...
    parser.add_argument(
        ...
        "--config",
        action="store_true",
        help="Get the ROUX compilation configurations used.",
    )
    parser.add_argument(
        "-f",
        "--flags",
        ...
        help="Provide the ROUX compilation and linker flags used.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
