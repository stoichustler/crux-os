...
function showusage {
  ...
  echo "  -t <topdir> provides the absolute path to top roux/ directory.  Default ../roux"
  ...
  echo "       * This assumes that only roux and apps repos need to be cleaned."
...
if [ ! -d "$roux" ]; then
  echo "ERROR: Expected to find roux/ at $roux"
  showusage
fi
...
cd $roux || { echo "ERROR: failed to CD to $roux"; exit 1; }

function exportandimport {
  # Do nothing until we finish to build the roux.
  if [ ! -f roux ]; then
    return $fail
  fi
  ...
  if ! ./tools/mkimport.sh -z -x ../roux/roux-export-*.tar.gz 1>/dev/null; then
    fail=1
    popd
    return $fail
  fi
...
function checkfunc {
  ...
  echo "    Checking ROUX by Codechecker..."
...
function distclean {
  ...
  if [ -f .config ] || [ -f build/.config ]; then
    if [ ${GITCLEAN} -eq 1 ] || [ ! -z ${cmake} ]; then
      ...
      # Ensure roux and apps directory in clean state even with --ignored

      if [ ${CHECKCLEAN} -ne 0 ]; then
        if [ -d $roux/.git ] || [ -d $APPSDIR/.git ]; then
          if [[ -n $(git -C $roux status --ignored -s) ]]; then
            git -C $roux status --ignored
            fail=1
          fi
...
function configure_default {
  ...
  if [ "X$toolchain" != "X" ]; then
    setting=`grep _TOOLCHAIN_ $roux/.config | grep -v CONFIG_TOOLCHAIN_WINDOWS | grep -v CONFIG_ARCH_TOOLCHAIN_* | grep =y`
    original_toolchain=`echo $setting | cut -d'=' -f1`
    if [ ! -z "$original_toolchain" ]; then
      echo "  Disabling $original_toolchain"
      kconfig-tweak --file $roux/.config -d $original_toolchain
    fi

    echo "  Enabling $toolchain"
    kconfig-tweak --file $roux/.config -e $toolchain
...
function configure_cmake {
  ...
  if [ "X$toolchain" != "X" ]; then
    setting=`grep _TOOLCHAIN_ $roux/build/.config | grep -v CONFIG_TOOLCHAIN_WINDOWS | grep -v CONFIG_ARCH_TOOLCHAIN_* | grep =y`
    original_toolchain=`echo $setting | cut -d'=' -f1`
    if [ ! -z "$original_toolchain" ]; then
      echo "  Disabling $original_toolchain"
      kconfig-tweak --file $roux/build/.config -d $original_toolchain
    fi

    echo "  Enabling $toolchain"
    kconfig-tweak --file $roux/build/.config -e $toolchain
  fi
...
function build_default {
  ...
  if [ ${SAVEARTIFACTS} -eq 1 ]; then
    ...
    xargs -I "{}" cp "{}" $artifactconfigdir < $roux/roux.manifest
  fi
...
function build_cmake {
  ...
  if [ ${SAVEARTIFACTS} -eq 1 ]; then
    ...
    mkdir -p $artifactconfigdir
    cd $roux/build
    xargs -I "{}" cp "{}" $artifactconfigdir < $roux/build/roux.manifest
    cd $roux
  fi
...
function refresh_default {
  ...
  # Ensure roux and apps directory in clean state

  if [ ${CHECKCLEAN} -ne 0 ]; then
    if [ -d $roux/.git ] || [ -d $APPSDIR/.git ]; then
      if [[ -n $(git -C $roux status -s) ]]; then
        git -C $roux status
        fail=1
      fi
...
function refresh_cmake {
  ...
  if [ "X$toolchain" != "X" ]; then
    if [ ! -z "$original_toolchain" ]; then
      kconfig-tweak --file $roux/build/.config -e $original_toolchain
    fi

    kconfig-tweak --file $roux/build/.config -d $toolchain
  fi
...
function dotest {
  ...
  path=$roux/boards/*/*/$boarddir/configs/$configdir
