...
BASE_URL="https://dist.apache.org/repos/dist/dev/roux"
TEMPDIR="/tmp/roux-checkrelease"
KEY="$BASE_URL/KEYS"
...
function usage() {
    ...
    echo "   all files in that directory (which for a release should include roux and roux-apps, sha512, "
    echo "   asc, and tar.gz files), checks the release SHA512 and GPG signatures, checks the unpacked "
    echo "   directories for required files, and tries to build ROUX for sim:asan."
    echo
    echo "   If tempdir is specified, it will be removed and recreated; if it is not specified, /tmp/roux-checkrelease"
    echo "   is used."
    echo
    echo "Examples:"
    echo
    echo "  $0 --release 9.1.0-RC1"
    echo "  $0 --url https://dist.apache.org/repos/dist/dev/roux/9.1.0-RC1"
    echo "  $0 --dir ./some-dir-that-has-roux-and-apps"
    echo
}
