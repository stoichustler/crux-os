...
if [ -z "$topdir" -o ! -d "$topdir" ]; then
  echo "The full path to the ROUX base directory must be provided on the command line"
  echo $usage
  exit 1
fi
...
# Extract all values from the .config in the $topdir that contains all of the ROUX
...
if [ "X$romfs" != "Xy" ]; then
  echo "ROMFS support is disabled in the ROUX configuration"
...
if [ "$usefat" = true -a "X$fatfs" != "Xy" ]; then
  echo "FAT FS support is disabled in the ROUX configuration"
...
echo '#include <roux/compiler.h>' >${headerfile}
