...
if [ -x tools/sethost.sh ]; then
  ...
  echo "This script must be executed in roux/ or roux/tools directories"
  exit 1
fi

if [ ! -r $roux/.config ]; then
  echo "There is no .config at $roux"
  exit 1
fi

if [ ! -r $roux/Make.defs ]; then
  echo "ERROR: No readable Make.defs file exists at $roux"
  exit 1
fi
...
if [ "X$host" == "Xlinux" -o "X$host" == "Xmacos" -o "X$host" == "Xbsd" ]; then
  ...
  kconfig-tweak --file $roux/.config --disable CONFIG_HOST_WINDOWS

  # Enable Linux or macOS or BSD

  if [ "X$host" == "Xlinux" ]; then
    echo "  Select CONFIG_HOST_LINUX=y"
    kconfig-tweak --file $roux/.config --disable CONFIG_HOST_MACOS
    kconfig-tweak --file $roux/.config --disable CONFIG_HOST_BSD
    kconfig-tweak --file $roux/.config --enable CONFIG_HOST_LINUX

    if [ "X$cpu" == "Xarm64" ]; then
      echo "  Select CONFIG_HOST_ARM64=y"
      kconfig-tweak --file $roux/.config --enable CONFIG_HOST_ARM64
    fi

  elif [ "X$host" == "Xbsd" ]; then
    echo "  Select CONFIG_HOST_BSD=y"
    kconfig-tweak --file $roux/.config --disable CONFIG_HOST_MACOS
    kconfig-tweak --file $roux/.config --disable CONFIG_HOST_LINUX
    kconfig-tweak --file $roux/.config --enable CONFIG_HOST_BSD

  else
    echo "  Select CONFIG_HOST_MACOS=y"
    kconfig-tweak --file $roux/.config --disable CONFIG_HOST_LINUX
    kconfig-tweak --file $roux/.config --disable CONFIG_HOST_BSD
    kconfig-tweak --file $roux/.config --enable CONFIG_HOST_MACOS
  ...
  kconfig-tweak --file $roux/.config --enable CONFIG_SIM_X8664_SYSTEMV
else
  ...
  kconfig-tweak --file $roux/.config --enable CONFIG_HOST_WINDOWS
  kconfig-tweak --file $roux/.config --enable CONFIG_SIM_X8664_MICROSOFT

  # Enable Windows environment

  if [ "X$wenv" == "Xcygwin" ]; then
    echo "  Select CONFIG_WINDOWS_CYGWIN=y"
    kconfig-tweak --file $roux/.config --enable CONFIG_WINDOWS_CYGWIN
  else
    if [ "X$wenv" == "Xmsys" ]; then
      echo "  Select CONFIG_WINDOWS_MSYS=y"
      kconfig-tweak --file $roux/.config --enable CONFIG_WINDOWS_MSYS
    else
      echo "  Select CONFIG_WINDOWS_NATIVE=y"
      kconfig-tweak --file $roux/.config --enable CONFIG_EXPERIMENTAL
      kconfig-tweak --file $roux/.config --enable CONFIG_WINDOWS_NATIVE
    fi
  fi
fi
