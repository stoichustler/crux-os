...
static void trimline(char *line)
{
	...
	str = line;
	char *strend;
	for (strend = str + strlen(str) - 1; strend >= str && isspace((int)(*strend)); strend--)
		;
	*(strend + 1) = 0;
}
...
static void bdf_printoutput(FILE *out, glyphinfo_t *ginfo, nx_fontmetric_t *nxmetric)
{
	int i;
	int j;
	...
	if ((ginfo->encoding >= NXFONT_MIN7BIT && ginfo->encoding <= NXFONT_MAX7BIT) ||
		(ginfo->encoding >= NXFONT_MIN8BIT && ginfo->encoding <= NXFONT_MAX8BIT)) {
		/* Glyph general info */

		if (ginfo->bb_x_off < 0) {
			fprintf(out, "/* %s (%d) -- NOTE: Xoffset should be %d, not 0. */\n", ginfo->name,
					ginfo->encoding, ginfo->bb_x_off);
		} else {
		...
		fprintf(out, "#define NXFONT_METRICS_%d {%d, %d, %d, %d, %d, 0}\n", ginfo->encoding,
				nxmetric->stride, nxmetric->width, nxmetric->height, nxmetric->xoffset,
				nxmetric->yoffset);
		...
		while (fgets(line, BDF_MAX_LINE_LENGTH, file) != NULL) {
			...
			while ((token = (char *)strtok_r(str, " ", &saveptr1))) {
				...
				if (strcmp(token, "STARTCHAR") == 0) {
					...
					/* The ROUX font format does not support
					 * negative X offsets.
					 */
					...
					if (ginfo.bb_x_off < 0) {
						...
						printf("%s: ignoring negative x offset for "
							   "glyph '%s' (%d)\n",
							   argv[0], ginfo.name, ginfo.encoding);
					} else {
						nxmetric.xoffset = ginfo.bb_x_off;
					}

					nxmetric.yoffset = fbb_y + fbb_y_off - ginfo.bb_y_off - ginfo.bb_h;
