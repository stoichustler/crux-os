...
static bool find_variable(const char *configpath, const char *varname, char **varvalue)
{
	FILE *stream;
	char *tmpname;
	char *tmpvalue;
...
static bool check_variable(const char *configpath, const char *varname, const char *varvalue)
{
	char *tmpvalue;
...
static int foreach_dirent(const char *dirpath, direntcb_t cb, void *arg)
{
	...
	for (;;) {
		...
		if (entry == NULL && errno != 0) {
			fprintf(stderr, "ERROR: Failed to read directory '%s' entry: %s\n", dirpath,
					strerror(errno));
			closedir(dirp);
			exit(EXIT_FAILURE);
		}
...
static int enum_architectures(const char *dirpath, struct dirent *entry, void *arg)
{
	...
	if (ret < 0) {
		fprintf(stderr, "ERROR: asprintf() failed to archpath\n");
		return ret;
	}

	ret = asprintf(&testpath, "%s%cKconfig", archpath, g_delim);
	if (ret < 0) {
		fprintf(stderr, "ERROR: asprintf() failed to testpath\n");
		return ret;
	}
	...
	if (test_filepath(testpath)) {
		...
		if (ret < 0) {
			fprintf(stderr, "ERROR: asprintf() failed to testpath/include\n");
			return ret;
		}
		...
		if (test_dirpath(testpath)) {
			...
			if (ret < 0) {
				fprintf(stderr, "ERROR: asprintf() failed to testpath/src\n");
				return ret;
			}

			if (test_dirpath(testpath)) {
				if (g_narch >= MAX_ARCHITECTURES) {
					fprintf(stderr, "ERROR: Too many architecture directories found\n");
					exit(EXIT_FAILURE);
				}
...
static int enum_mcus(const char *dirpath, struct dirent *entry, void *arg)
{
	...
	if (ret < 0) {
		fprintf(stderr, "ERROR: asprintf() failed to mcupath\n");
		return ret;
	}

	ret = asprintf(&testpath, "%s%cKconfig", mcupath, g_delim);
	if (ret < 0) {
		fprintf(stderr, "ERROR: asprintf() failed to archpath/Kconfig\n");
		return ret;
	}
	...
	if (test_filepath(testpath)) {
		...
		if (ret < 0) {
			fprintf(stderr, "ERROR: asprintf() failed to testpath/Make.defs\n");
			return ret;
		}

		if (test_filepath(testpath)) {
			if (g_nmcu >= MAX_MCUS) {
				fprintf(stderr, "ERROR: Too many MCU directories found\n");
				exit(EXIT_FAILURE);
			}
...
static int enum_board_configurations(const char *dirpath, struct dirent *entry, void *arg)
{
	char *configpath;
	char *varvalue;
	int ret = 0;
	...
	ret = asprintf(&configpath, "%s%c%s%cdefconfig", dirpath, g_delim, entry->d_name, g_delim);
	if (ret < 0) {
		fprintf(stderr, "ERROR: asprintf() failed to configpath\n");
		return ret;
	}
	...
	if (test_filepath(configpath)) {
		...
		if (ret < 0) {
			fprintf(stderr, "ERROR: asprintf() failed to varvalue\n");
			return ret;
		}
		...
		if (check_variable(configpath, "CONFIG_ARCH_CHIP", varvalue)) {
			...
			if (g_nboard >= MAX_BOARDS) {
				fprintf(stderr, "ERROR: Too many board configurations found\n");
				exit(EXIT_FAILURE);
			}
...
static int enum_boards(const char *dirpath, struct dirent *entry, void *arg)
{
	...
	if (ret < 0) {
		fprintf(stderr, "ERROR: asprintf() failed to boardpath\n");
		return ret;
	}
	...
	if (test_filepath(testpath)) {
		...
		if (test_dirpath(testpath)) {
			...
			if (test_dirpath(testpath)) {
				...
				foreach_dirent(boardpath, enum_board_configurations, entry->d_name);
			}
		}
	}
...
int main(int argc, char **argv)
{
	...
	g_nmcu = 0;
	ret = asprintf(&archpath, "%s%c%s%csrc", g_archdir, g_delim, g_selected_arch, g_delim);
	if (ret < 0) {
		fprintf(stderr, "ERROR: asprintf() failed to archpath/src\n");
		return ret;
	}
