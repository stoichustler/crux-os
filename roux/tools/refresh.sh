...
while [ ! -z "$1" ]; do
  ...
  --help )
    ...
    echo "  --nocopy"
    echo "     Do not copy defconfig from roux/boards/<board>/configs to roux/.config"
    echo "  --help"
    echo "     Show this help message and exit"
    echo "  <board>"
    echo "     The board directory under roux/boards/arch/chip/"
    echo "  <config>"
    echo "     The board configuration directory under roux/boards/arch/chip/<board>/configs"
    echo "  <archname>"
    echo "     The architecture directory under roux/boards/"
    echo "  <chipname>"
    echo "     The chip family directory under roux/boards/<arch>/"
    echo " "
...
if [ ! -x tools/${MYNAME} ] ; then
  echo "ERROR:  This file must be executed from the top-level ROUX directory: $PWD"
  exit 1
fi
