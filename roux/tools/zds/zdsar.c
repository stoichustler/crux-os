...
static const char *quote_backslash(const char *argument)
{
	...
	if (*src) {
		fprintf(stderr,
				"ERROR: Truncated during expansion string "
				"is too long [%lu/%u]\n",
...
static const char *convert_path(const char *path, cygwin_conv_path_t what)
{
	...
	ret = cygwin_conv_path(what | CCP_RELATIVE, g_dequoted, &g_hostpath[1], MAX_PATH - 3);
	if (ret < 0) {
...
static void show_usage(const char *progname, const char *msg, int exitcode)
{
	...
	fprintf(stderr, "\n");
	fprintf(stderr,
			"%s [OPTIONS] --ar \"<AR>\" --library \"<LIBRARY>\" "
			"obj [obj [obj...]]\n",
			progname);
	fprintf(stderr, "\n");
...
static void parse_args(int argc, char **argv)
{
	...
	if (objpath != NULL) {
...
#ifdef HOST_NATIVE
		if (objpath[0] != SEPARATOR || (isalpha(objpath[0]) && objpath[1] != ':'))
#else
...
#endif
		{
			...
			if (pathlen >= MAX_PATH) {
				fprintf(stderr,
						"ERROR: Working directory path is "
						"too long [%d/%d]: %s\n",
	...
	for (;;) {
		...
		while ((object = strtok_r(objects, " ", &lasts)) != NULL) {
			...
			if (!S_ISREG(buf.st_mode)) {
				fprintf(stderr,
						"ERROR: Object %s exists but is not a regular "
						"file\n",
						g_path);
				exit(EXIT_FAILURE);
			}
			...
			if (totallen >= MAX_BUFFER) {
				fprintf(stderr,
						"ERROR: object argument is too long [%d/%d]: "
						"%s=-+%s\n",
			...
			pathlen =
				snprintf(&g_command[cmdlen], MAX_BUFFER - cmdlen, "%s=-+%s", g_libname, quoted);
			cmdlen += pathlen;
