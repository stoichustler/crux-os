...
#define ROUX_IXOTH (1 << 0) /* Must match ROUX's S_IXOTH */
#define ROUX_IWOTH (1 << 1) /* Must match ROUX's S_IWOTH */
#define ROUX_IROTH (1 << 2) /* Must match ROUX's S_IROTH */

#define ROUX_IRXOTH (ROUX_IROTH | ROUX_IXOTH)

#define ROUX_IXGRP (1 << 3) /* Must match ROUX's S_IXGRP */
#define ROUX_IWGRP (1 << 4) /* Must match ROUX's S_IWGRP */
#define ROUX_IRGRP (1 << 5) /* Must match ROUX's S_IRGRP */

#define ROUX_IRXGRP (ROUX_IRGRP | ROUX_IXGRP)

#define ROUX_IXUSR (1 << 6) /* Must match ROUX's S_IXUSR */
#define ROUX_IWUSR (1 << 7) /* Must match ROUX's S_IWUSR */
#define ROUX_IRUSR (1 << 8) /* Must match ROUX's S_IRUSR */

#define ROUX_IRXUSR (ROUX_IRUSR | ROUX_IXUSR)

#define ROUX_IFDIR (4 << 12)  /* Must match ROUX's S_IFDIR */
#define ROUX_IFREG (8 << 12)  /* Must match ROUX's S_IFREG */
#define ROUX_IFLNK (10 << 12) /* Must match ROUX's S_IFLNK */

#define DIR_MODEFLAGS (ROUX_IFDIR | ROUX_IRXUSR | ROUX_IRXGRP | ROUX_IRXOTH)
#define DIRLINK_MODEFLAGS (ROUX_IFLNK | ROUX_IRXUSR | ROUX_IRXGRP | ROUX_IRXOTH)
#define FILE_MODEFLAGS (ROUX_IFREG | ROUX_IRUSR | ROUX_IRGRP | ROUX_IROTH)
 ...
 *
 * Similarly, the ROUX mode_t is only 16-bits so uint16_t is explicitly used
 * for ROUX file modes.
 */
...
typedef int (*traversal_callback_t)(const char *dirpath, const char *name, void *arg,
									bool lastentry);
...
static size_t lzf_compress(const uint8_t *inbuffer, unsigned int inlen, union lzf_result_u *result);
...
static void gen_directory(const char *path, const char *name, mode_t mode, bool lastentry);
static void gen_file(const char *path, const char *name, mode_t mode, bool lastentry);
static int dir_notempty(const char *dirpath, const char *name, void *arg, bool lastentry);
static int process_direntry(const char *dirpath, const char *name, void *arg, bool lastentry);
static int traverse_directory(const char *dirpath, traversal_callback_t callback, void *arg);
	...
	while (inptr < inend - 2) {
		...
		*hslot = (uint8_t *)inptr;
		...
		if (ref < inptr && /* the next test will actually take care of this, but this is faster */
			(off = inptr - ref - 1) < LZF_MAX_OFF && ref > (uint8_t *)inbuffer &&
			ref[2] == inptr[2] && ((ref[1] << 8) | ref[0]) == ((inptr[1] << 8) | inptr[0])) {
			/* Match found at *ref++ */
...
static uint16_t get_mode(mode_t mode)
{
	...
	/* Convert mode to CROMFS ROUX read-only mode */
...
static void gen_directory(const char *path, const char *name, mode_t mode, bool lastentry)
{
	...
	node.cn_mode = TGT_UINT16(ROUX_IFDIR | get_mode(mode));
	node.cn_pad = 0;
...
static void gen_file(const char *path, const char *name, mode_t mode, bool lastentry)
{
	...
	do {
		...
		if (nread > 0) {
			...
			fprintf(g_tmpstream,
					...
					"*/\n\n",
					(unsigned long)g_offset, blkno, (long)blklen, (long)nread, clen);
	...
	fprintf(g_tmpstream,
			...
			(unsigned long)nodeoffs, path, (unsigned long)ntotal, (unsigned long)blktotal);

	node.cn_mode = TGT_UINT16(ROUX_IFREG | get_mode(mode));
	node.cn_pad = 0;
