...
class ROUXDictExporter(DictExporter):
...
def get_die_filename(die, lineprog):
    ...
    # Change path to relative to ROUX base
    try:
        path = path.resolve()
    except OSError as e:
...
def generate_any_tree(symbol_dict, total_size, path_prefix):
    ...
    if Path(path_prefix) == Path(args.rouxbase):
        # All source files are under roux_base so there is
        ...
        node_roux_base = TreeNode("roux_base", args.rouxbase)
    ...
    path_node_map = [
        [Path(args.rouxbase), node_roux_base],
    ...
    if node_roux_base is not root:
        # roux_base and OUTPUT_DIR nodes don't have sum of symbol size
        # so calculate them here.
        node_roux_base.size = sum_node_children_size(node_roux_base)
        node_output_dir.size = sum_node_children_size(node_output_dir)

        # Find out which nodes need to be in the tree.
        # "(no path)", roux_base nodes are essential.
        children = [node_no_paths, node_roux_base]
        if node_output_dir.height != 0:
            # OUTPUT_DIR may be under roux_base.
...
def parse_args():
    ...
    parser.add_argument("-k", "--kernel", required=True, help="ROUX ELF binary")
    parser.add_argument("-z", "--rouxbase", required=True, help="ROUX base path")
    parser.add_argument(
        "-q",
        "--quiet",
        action="store_true",
