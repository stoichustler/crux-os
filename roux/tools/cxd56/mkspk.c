...
static uint8_t vmk[16] = "\x27\xc0\xaf\x1b\x5d\xcb\xc6\xc5\x58\x22\x1c\xdd\xaf\xf3\x20\x21";
...
static struct args *parse_args(int argc, char **argv)
{
	...
	if (show_help == 1) {
		fprintf(stderr, "mkspk [-c <number>] <filename> <save name> [<output file>]\n");
		exit(EXIT_FAILURE);
	}
...
static void *create_image(struct elf_file *elf, int core, char *savename, int *image_size)
{
	char *img;
