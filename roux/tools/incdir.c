...
{
	fprintf(stderr,
			"%s is a tool for flexible generation of include path "
			"arguments for a\n",
			progname);
...
static enum compiler_e get_compiler(char *ccname)
{
	...
	if (strstr(ccname, "gcc") != NULL || strstr(ccname, "g++") != NULL ||
	...
	} else if (strstr(ccname, "ccarm") != NULL || strstr(ccname, "cxarm") != NULL) {
		return COMPILER_GHS;
	} else if (strstr(ccname, "mingw") != NULL) {
		return COMPILER_MINGW;
	} else if (strstr(ccname, "ez8cc") != NULL || strstr(ccname, "zneocc") != NULL ||
...
int main(int argc, char **argv, char **envp)
{
	...
	for (i = 0; i < ndirs; i++) {
		...
		if (os == OS_CYGWIN && wintool) {
			...
			cygwin_conv_path(CCP_POSIX_TO_WIN_A, dirname, convpath, bufsize);
			incpath = convpath;
		} else
#endif
		{
