...
GIT_DIR = $(if $(wildcard $(TOPDIR)/.git),y,)
...
# If system/external/ contains a Kconfig, we define the EXTERNALDIR variable to 'external'
...
EXTERNALDIR := $(shell if [ -r $(TOPDIR)/system/external/Kconfig ]; then \
                 echo 'system/external'; else echo 'system/dummy'; fi)
...
# LINKLIBS derives from ROUXLIBS and is simply the same list with the
#   subdirectory removed

LINKLIBS = $(patsubst staging/%,%,$(ROUXLIBS))
...
# Target used to copy include/roux/lib/math.h.  If CONFIG_ARCH_MATH_H is
# defined, then there is an architecture specific math.h header file
# that will be included indirectly from include/math.h.  But first, we
# have to copy math.h from include/roux/. to include/.  Logic within
# include/roux/lib/math.h will hand the redirection to the architecture-
# specific math.h header file.
#
# If the CONFIG_LIBM is defined, the Rhombus libm will be built at libm/libm.
# Definitions and prototypes for the Rhombus libm are also contained in
# include/roux/lib/math.h and so the file must also be copied in that case.
#
...
ifeq ($(NEED_MATH_H),y)
include/math.h: include/roux/lib/math.h
	$(Q) cp -f include/roux/lib/math.h include/math.h
endif
...
include/float.h: include/roux/lib/float.h
	$(Q) cp -f include/roux/lib/float.h include/float.h
endif

# Target used to copy include/roux/lib/stdarg.h.  If CONFIG_ARCH_STDARG_H is
# defined, then there is an architecture specific stdarg.h header file
# that will be included indirectly from include/lib/stdarg.h.  But first, we
# have to copy stdarg.h from include/roux/. to include/.

ifeq ($(CONFIG_ARCH_STDARG_H),y)
include/stdarg.h: include/roux/lib/stdarg.h
	$(Q) cp -f include/roux/lib/stdarg.h include/stdarg.h
endif

# Target used to copy include/roux/lib/setjmp.h.  If CONFIG_ARCH_SETJMP_H is
...
# have to copy setjmp.h from include/roux/. to include/.

ifeq ($(CONFIG_ARCH_SETJMP_H),y)
include/setjmp.h: include/roux/lib/setjmp.h
	$(Q) cp -f include/roux/lib/setjmp.h include/setjmp.h
endif
...
# Targets used to build include/roux/version.h.  Creation of version.h is
# part of the overall ROUX configuration sequence. Notice that the
# tools/mkversion tool is built and used to create include/roux/version.h
...
include/roux/version.h: $(TOPDIR)/.version tools/mkversion$(HOSTEXEEXT)
	$(Q) echo "Create version.h"
	$(Q) tools/mkversion $(TOPDIR) > $@

# Targets used to build include/roux/config.h.  Creation of config.h is
# part of the overall ROUX configuration sequence. Notice that the
# tools/mkconfig tool is built and used to create include/roux/config.h

tools/mkconfig$(HOSTEXEEXT): prebuild
	$(Q) $(MAKE) -C tools -f Makefile.host mkconfig$(HOSTEXEEXT)

include/roux/config.h: $(TOPDIR)/.config tools/mkconfig$(HOSTEXEEXT)
...
#
# Directories links.  Most of establishing the ROUX configuration involves
...
# context
#
# The context target is invoked on each target build to assure that ROUX is
# properly configured.  The basic configuration steps include creation of the
# the config.h and version.h header files in the include/roux directory and
...
context: tools/incdir$(HOSTEXEEXT) include/roux/config.h include/roux/version.h .dirlinks $(CONTEXTDIRS_DEPS) | staging
...
%.context: include/roux/config.h .dirlinks
...
clean_context: clean_dirlinks
	...
	$(call DELFILE, include/roux/config.h)
	$(call DELFILE, include/roux/version.h)
...
# $(BIN)
#
# Create the final ROUX executable in a two pass build process.  In the
...
ifeq ($(CONFIG_BUILD_2PASS),y)
	...
	$(Q) echo $(BIN) > roux.manifest
	$(Q) printf "%s\n" *.map >> roux.manifest
ifeq ($(CONFIG_INTELHEX_BINARY),y)
	@echo "(cp)    roux.hex"
	$(Q) $(OBJCOPY) $(OBJCOPYARGS) -O ihex $(BIN) roux.hex
	$(Q) echo roux.hex >> roux.manifest
endif
ifeq ($(CONFIG_MOTOROLA_SREC),y)
	@echo "(cp)    roux.srec"
	$(Q) $(OBJCOPY) $(OBJCOPYARGS) -O srec $(BIN) roux.srec
	$(Q) echo roux.srec >> roux.manifest
endif
ifeq ($(CONFIG_RAW_BINARY),y)
	@echo "(cp)    roux.bin"
	$(Q) $(OBJCOPY) $(OBJCOPYARGS) -O binary $(BIN) roux.bin
	$(Q) echo roux.bin >> roux.manifest
endif
ifeq ($(CONFIG_UBOOT_UIMAGE),y)
	@echo "(ui)    uImage"
	$(Q) mkimage -A $(CONFIG_ARCH) -O linux -C none -T kernel -a $(CONFIG_UIMAGE_LOAD_ADDRESS) \
		-e $(CONFIG_UIMAGE_ENTRY_POINT) -n $(BIN) -d roux.bin uImage
	$(Q) if [ -w /tftpboot ] ; then \
		cp -f uImage /tftpboot/uImage; \
	fi
	$(Q) echo "uImage" >> roux.manifest
endif
ifeq ($(CONFIG_RAW_DISASSEMBLY),y)
	@echo "(cp)    roux.asm"
	$(Q) $(OBJDUMP) -d $(BIN) > roux.asm
	$(Q) echo roux.bin >> roux.asm
endif
	$(call POSTBUILD, $(TOPDIR))

# qemu quick debug for qemu-armv8a:nsh_smp
qemu:
	$(Q) qemu-system-aarch64 -cpu cortex-a53 -smp 4 -nographic \
		-machine virt,virtualization=on,gic-version=3 \
		-net none -chardev stdio,id=con,mux=on -serial chardev:con \
		-mon chardev=con,mode=readline -kernel ./roux

tags:
	$(Q) ctags --languages=Asm,c,c++ -R

# make scp IP=xxxx TO=xxxx
scp:
	$(Q) scp roux.bin root@$(IP):$(TO)

# flash (or download : DEPRECATED)
#
# This is a helper target that will rebuild ROUX and flash it to the target
...
# functions required for properly executing the ROUX binary.
...
host_info: checkpython3
	@if [[ ! -f "include/sysinfo.h" ]]; then \
		...
		python3 $(TOPDIR)/tools/host_info_dump.py $(SYSINFO_FLAGS) \
		 $(realpath $(TOPDIR)) > include/sysinfo.h; \
	fi
	@python3 $(TOPDIR)/tools/host_info_parse.py $(SYSINFO_PARSE_FLAGS)
...
# location: https://bitbucket.org/roux/tools/downloads/.  See README.txt
# file in the ROUX tools GIT repository for additional information.
...
# export
#
# The export target will package the ROUX libraries and header files into
...
clean: subdir_clean
	...
	$(call DELFILE, roux-export*.zip)
	$(call DELDIR, roux-export*)
	$(call DELFILE, roux_user*)
	$(call DELDIR, staging)
	$(call DELFILE, uImage)
	$(call CLEAN)
...
# application directory.  A sample apps/ directory is included with ROUX,
# however, this is not treated as part of ROUX and may be replaced with a
...
# functions in the user application directory from the ROUX build directory.
#
