...
struct jlink_ops_s {
	...
	int (*readmem)(uint32_t addr, char *pdata, unsigned int numbytes);
	...
	int (*writemem)(uint32_t addr, const char *pdata, unsigned numbytes);
...
static struct symbols_s g_symbols[] = {{"g_pidhash", 0, 0},
									   {"g_npidhash", 0, 0},
									   {"g_tcbinfo", 0, 0},
									   {"g_readytorun", 0, 0},
									   {NULL, 0, 0}};
...
static inline int encode_hex(char *line, uint32_t value)
{
	...
	return snprintf(line, 9, "%02x%02x%02x%02x", value & 0xff, (value & 0xff00) >> 8,
...
static int get_pid(struct plugin_priv_s *priv, uint32_t idx, uint32_t *pid)
{
	int ret;
...
static int get_idx_from_pid(struct plugin_priv_s *priv, uint32_t pid)
{
	int idx;
...
static int setget_reg(struct plugin_priv_s *priv, uint32_t idx, uint32_t regidx, uint32_t *regval,
					  bool write)
{
	uint32_t regaddr;
...
static int update_tcbinfo(struct plugin_priv_s *priv)
{
	if (!priv->tcbinfo) {
		...
		ret = READU16(g_symbols[TCBINFO].address + offsetof(struct tcbinfo_s, regs_num), &regs_num);
		if (ret != 0) {
		...
		ret = READU32(g_symbols[TCBINFO].address + offsetof(struct tcbinfo_s, reg_off), &reg_off);
		if (ret != 0) {
			PERROR("error in read regoffs address ret %d\n", ret);
			return ret;
		}

		priv->tcbinfo = ALLOC(sizeof(struct tcbinfo_s) + regs_num * sizeof(uint16_t));
		...
		ret = READMEM(g_symbols[TCBINFO].address, (char *)priv->tcbinfo, sizeof(struct tcbinfo_s));
		if (ret != sizeof(struct tcbinfo_s)) {
			PERROR("error in read tcbinfo_s ret %d\n", ret);
			return ret;
		}

		ret = READMEM(reg_off, (char *)&priv->tcbinfo->reg_offs[0], regs_num * sizeof(uint16_t));
...
static int update_pidhash(struct plugin_priv_s *priv)
{
	...
	if (npidhash != priv->npidhash) {
		priv->pidhash = REALLOC(priv->pidhash, npidhash * sizeof(uint32_t *));
		if (!priv->pidhash) {
	...
	ret = READMEM(pidhashaddr, (char *)priv->pidhash, priv->npidhash * sizeof(uint32_t *));
	...
	for (i = 0; i < priv->npidhash; i++) {
		if (priv->pidhash[i]) {
			ret = READU32(priv->pidhash[i] + priv->tcbinfo->regs_off, &priv->regsaddr[i]);
			if (ret != 0) {
...
int RTOS_GetThreadDisplay(char *display, uint32_t threadid)
{
	...
	len += snprintf(display + len, DISPLAY_LENGTH - len, "[PID:%03d]", threadid);

	ret = READMEM(g_plugin_priv.pidhash[idx] + g_plugin_priv.tcbinfo->name_off, name, TCB_NAMESIZE);
	if (ret != TCB_NAMESIZE) {
	...
	if ((name[0] > 'a' && name[0] < 'z') || (name[0] > 'A' && name[0] < 'Z')) {
		len += snprintf(display + len, DISPLAY_LENGTH - len, "%s", name);
	} else {
		len += snprintf(display + len, DISPLAY_LENGTH - len, "thread-%d", threadid);
	}

	ret = READU8(g_plugin_priv.pidhash[idx] + g_plugin_priv.tcbinfo->state_off, &readval);
	if (ret != 0) {
		PERROR("error in read tcb state return %d\n", ret);
	}

	len += snprintf(display + len, DISPLAY_LENGTH - len, ":%04d", readval);

	ret = READU8(g_plugin_priv.pidhash[idx] + g_plugin_priv.tcbinfo->pri_off, &readval);
	if (ret != 0) {
		PERROR("error in read tcb pri return %d\n", ret);
	}

	len += snprintf(display + len, DISPLAY_LENGTH - len, "[PRI:%03d]", readval);
...
int RTOS_SetThreadReg(char *hexregval, uint32_t regindex, uint32_t threadid)
{
	int idx;
	uint32_t regval;
