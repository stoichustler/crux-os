...
static void generate_proxy(int nfixed, int nparms)
{
	...
	fprintf(stream, "#include <roux/config.h>\n");
...
static void generate_wrapper(int nfixed, int nparms)
{
	...
	fprintf(stream, "#include <roux/config.h>\n");
	fprintf(stream, "#include <roux/sched_note.h>\n");
	fprintf(stream, "#include <stdint.h>\n");

	/* Suppress "'noreturn' function does return" warnings. */

	fprintf(stream, "#include <roux/compiler.h>\n");
	...
	fprintf(stream, "#include <roux/arch.h>\n\n");
	...
	fprintf(stream, "\n  sched_note_syscall_enter(SYS_%s, %d", g_parm[NAME_INDEX], nparms);
		...
		for (i = PARM1_INDEX; i < nargs; i++) {
			if (is_vararg(g_parm[i])) {
				...
				if (i == --nargs) {
					...
					memmove(g_parm[i], g_parm[i + 1], sizeof(g_parm[i]) * (nargs - i));
				}
