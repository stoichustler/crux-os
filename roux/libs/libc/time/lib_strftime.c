...
static const char *const g_abbrev_wdayname[7] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};

static const char *const g_wdayname[7] = {"Sunday",	  "Monday", "Tuesday", "Wednesday",
										  "Thursday", "Friday", "Saturday"};

static const char *const g_abbrev_monthname[12] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
												   "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};

static const char *const g_monthname[12] = {"January",	 "February", "March",	 "April",
...
static int get_week_num(FAR const struct tm *time)
{
	...
	int week = (time->tm_yday + DAYSPERWEEK - (time->tm_wday + 6) % DAYSPERWEEK) / DAYSPERWEEK;
	...
	if (week == 0) {
		...
		int dec31 = (time->tm_wday + DAYSPERWEEK - time->tm_yday - 1) % DAYSPERWEEK;
		if (dec31 == TM_THURSDAY || (dec31 == TM_FRIDAY && is_leap(time->tm_year % 400 - 1))) {
			week++;
		}
	} else if (week == 53) {
		...
		if (jan1 != TM_THURSDAY && (jan1 != TM_WEDNESDAY || !is_leap(time->tm_year))) {
			week = 1;
		}
	}
...
process_next:
		...
		case 'F': {
			len = snprintf(dest, chleft, "%04d-%02d-%02d", tm->tm_year + TM_YEAR_BASE,
		...
		case 'I': {
			len = snprintf(dest, chleft, "%02d", (tm->tm_hour % 12) != 0 ? (tm->tm_hour % 12) : 12);
		} break;
		...
		case 'j': {
			if (tm->tm_mon < 12) {
				value =
					clock_daysbeforemonth(tm->tm_mon, clock_isleapyear(tm->tm_year)) + tm->tm_mday;
		...
		case 'l': {
			len = snprintf(dest, chleft, "%2d", (tm->tm_hour % 12) != 0 ? (tm->tm_hour % 12) : 12);
		} break;
		...
		case 'r': {
			...
			value = tm->tm_hour == 12 ? tm->tm_hour == 12 : tm->tm_hour % (HOURSPERDAY / 2);

			len = snprintf(dest, chleft, "%02d:%02d:%02d %s", value, tm->tm_min, tm->tm_sec, str);
		} break;

			/* %R: Shortcut for %H:%M. */

		case 'R': {
			len = snprintf(dest, chleft, "%02d:%02d", tm->tm_hour, tm->tm_min);
		} break;
		...
		case 'T': {
			len = snprintf(dest, chleft, "%02d:%02d:%02d", tm->tm_hour, tm->tm_min, tm->tm_sec);
		} break;
		...
		case 'U': {
			value = (tm->tm_yday + DAYSPERWEEK - tm->tm_wday) / DAYSPERWEEK;
			len = snprintf(dest, chleft, "%02d", value);
		} break;
		...
		case 'W': {
			value = (tm->tm_yday + DAYSPERWEEK - (tm->tm_wday + 6) % DAYSPERWEEK) / DAYSPERWEEK;
			len = snprintf(dest, chleft, "%02d", value);
		} break;
		...
		case 'x': {
			len = snprintf(dest, chleft, "%02d/%02d/%04d", tm->tm_mon, tm->tm_mday,
