...
static FAR struct myifaddrs *getmutil_ipv6addr(int fd, FAR struct lifreq *req,
											   FAR struct myifaddrs *ifaddr)
{
	...
	for (index = 0; index < CONFIG_NETDEV_MAX_IPv6_ADDR - 1; index++) {
		int len = snprintf(req->lifr_name, sizeof(req->lifr_name), "%s:%d", ifaddr->name, index);
		...
		ifaddr->addrs.ifa_broadaddr = (FAR struct sockaddr *)&ifaddr->broadaddr;
	...
	for (i = 1, *addrs = NULL; i <= MAX_IFINDEX; i++) {
		...
		if (ioctl(sockfd, SIOCGIFADDR, (unsigned long)&req) >= 0) {
			...
			if (ioctl(sockfd, SIOCGIFNETMASK, (unsigned long)&req) >= 0) {
				myaddrs->addrs.ifa_netmask = (FAR struct sockaddr *)&myaddrs->netmask;
				memcpy(&myaddrs->netmask, &req.lifr_netmask, sizeof(req.lifr_netmask));
			}

			if (ioctl(sockfd, SIOCGIFDSTADDR, (unsigned long)&req) >= 0) {
				myaddrs->addrs.ifa_dstaddr = (FAR struct sockaddr *)&myaddrs->dstaddr;
				memcpy(&myaddrs->dstaddr, &req.lifr_dstaddr, sizeof(req.lifr_dstaddr));
			} else if (ioctl(sockfd, SIOCGIFBRDADDR, (unsigned long)&req) >= 0) {
				myaddrs->addrs.ifa_broadaddr = (FAR struct sockaddr *)&myaddrs->broadaddr;
				memcpy(&myaddrs->broadaddr, &req.lifr_broadaddr, sizeof(req.lifr_broadaddr));
			}

			if (ioctl(sockfd, SIOCGIFHWADDR, (unsigned long)&req) >= 0) {
				myaddrs->addrs.ifa_data = &myaddrs->hwaddr;
				memcpy(&myaddrs->hwaddr, &req.lifr_hwaddr, sizeof(req.lifr_hwaddr));
			}
		}
#endif
