...
static ssize_t memstream_write(FAR void *c, FAR const char *buf, size_t size)
{
	FAR struct memstream_cookie_s *memstream_cookie = (FAR struct memstream_cookie_s *)c;
	FAR char *buf_grow;
	size_t new_size;
	...
	if (memstream_cookie->pos + size + 1 > memstream_cookie->size) {
		...
		memset(buf_grow + memstream_cookie->end, 0, new_size - memstream_cookie->size);
	...
	if (memstream_cookie->pos > memstream_cookie->end) {
		...
		memstream_cookie->saved = *(*memstream_cookie->buf + memstream_cookie->pos);
	}
...
static off_t memstream_seek(FAR void *c, FAR off_t *offset, int whence)
{
	...
	if (memstream_cookie->pos < memstream_cookie->end) {
		...
		*(*memstream_cookie->buf + memstream_cookie->pos) = memstream_cookie->saved;
	}
