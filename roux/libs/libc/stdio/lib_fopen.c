...
#define MODE_W                                                                                     \
	(1 << 1) /* Bit 1: "w{b|x|+}" open for writing, truncating,                                    \
			  * or creating file */
#define MODE_A                                                                                     \
	(1 << 2) /* Bit 2: "a{b|x|+}" open for writing, appending                                      \
			  * the to file */
...
FAR FILE *fdopen(int fd, FAR const char *mode)
{
	...
	if (fd >= 3) {
		...
		android_fdsan_exchange_owner_tag(
			fd, 0, android_fdsan_create_owner_tag(ANDROID_FDSAN_OWNER_TYPE_FILE, (uintptr_t)filep));
#endif
	} else {
