				...
				if (remaining > 0) {
					...
					if (remaining > buffer_available) {
						if (stream->fs_iofunc.read != NULL) {
							bytes_read = stream->fs_iofunc.read(stream->fs_cookie, dest, remaining);
						} else {
						...
						if (stream->fs_iofunc.read != NULL) {
							bytes_read = stream->fs_iofunc.read(
								stream->fs_cookie, stream->fs_bufread, buffer_available);
						} else {
							bytes_read = _NX_READ(fd, stream->fs_bufread, buffer_available);
						}
