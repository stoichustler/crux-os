...
#include <roux/roux.h>
#include <roux/sched.h>
#include <roux/ascii.h>
#include <roux/gdbstub.h>
#include <roux/memoryregion.h>
#include <roux/streams.h>
...
#define GDB_DEBUG(stat, ...)                                                                       \
	lib_sprintf((FAR struct lib_outstream_s *)&state->stream, ##__VA_ARGS__)
...
#define IS_SPECIAL_CHARACTERS(c) ((c) == '#' || (c) == '$' || (c) == '}' || (c) == '*')
...
typedef CODE ssize_t (*gdb_format_func_t)(FAR void *buf, size_t buf_len, FAR const void *data,
										  size_t data_len);

/* Calculate remaining space in packet from ptr_next position. */

#define gdb_remaining_len(state) ((state)->pkt_len - ((state)->pkt_next - (state)->pkt_buf))
...
static const struct memory_region_s g_memory_region[] = {CONFIG_BOARD_MEMORY_RANGE};
#endif
...
static int gdb_putchar(FAR struct gdb_state_s *state, int ch, FAR char *csum);
static void gdb_escapechar(FAR struct gdb_state_s *state, char c, FAR char *csum);
...
static ssize_t gdb_bin2hex(FAR void *buf, size_t buf_len, FAR const void *data, size_t data_len);
static ssize_t gdb_hex2bin(FAR void *buf, size_t buf_len, FAR const void *data, size_t data_len);

/* Packet creation helpers */

static int gdb_send_ok_packet(FAR struct gdb_state_s *state);
static int gdb_send_error_packet(FAR struct gdb_state_s *state, unsigned char error);

/* Command functions */

static ssize_t gdb_get_memory(FAR struct gdb_state_s *state, FAR void *buf, size_t buf_len,
							  uintptr_t addr, size_t len, gdb_format_func_t format);
static ssize_t gdb_put_memory(FAR struct gdb_state_s *state, FAR const void *buf, size_t buf_len,
							  uintptr_t addr, size_t len, gdb_format_func_t format);
...
static int gdb_expect_integer(FAR struct gdb_state_s *state, FAR uintptr_t *arg)
{
...
static int gdb_expect_addr_length(FAR struct gdb_state_s *state, FAR uintptr_t *addr,
								  FAR size_t *length)
{
	int ret;
...
static bool gdb_is_valid_region(FAR struct gdb_state_s *state, uintptr_t addr, size_t len,
								uint32_t flags)
{
#ifdef CONFIG_BOARD_MEMORY_RANGE
	...
	while (region->start < region->end) {
		if (addr >= region->start && addr <= region->end - len &&
...
static int gdb_send_signal_packet(FAR struct gdb_state_s *state, unsigned char signal)
{
	int ret;

	state->pkt_buf[0] = 'S';
	ret = gdb_bin2hex(&state->pkt_buf[1], sizeof(state->pkt_buf) - 1, &signal, 1);
	if (ret < 0) {
		return ret;
	}
...
static int gdb_send_error_packet(FAR struct gdb_state_s *state, unsigned char error)
{
	...
	state->pkt_buf[0] = 'E';
	ret = gdb_bin2hex(&state->pkt_buf[1], sizeof(state->pkt_buf) - 1, &error, 1);
	if (ret < 0) {
		return ret;
	}
...
static void gdb_get_registers(FAR struct gdb_state_s *state)
{
	...
	for (i = 0; i < state->size / sizeof(uintptr_t); i++) {
		if (g_tcbinfo.reg_off.p[i] == UINT16_MAX) {
			...
			state->registers[i] = *(FAR uintptr_t *)(reg + g_tcbinfo.reg_off.p[i]);
		}
	}
}
...
static int gdb_read_registers(FAR struct gdb_state_s *state)
{
	...
	ret = gdb_bin2hex(state->pkt_buf, sizeof(state->pkt_buf), state->registers, state->size);
	if (ret < 0) {
		return ret;
	}
...
static int gdb_write_registers(FAR struct gdb_state_s *state)
{
	...
	ret = gdb_hex2bin(state->registers, sizeof(state->registers), state->pkt_buf + 1,
					  state->pkt_len - 1);
	if (ret < 0) {
		return ret;
	}
...
static int gdb_read_register(FAR struct gdb_state_s *state)
{
	...
	ret = gdb_bin2hex(state->pkt_buf, sizeof(state->pkt_buf), &state->registers[addr],
...
static int gdb_read_memory(FAR struct gdb_state_s *state)
{
	...
	ret = gdb_get_memory(state, state->pkt_buf, sizeof(state->pkt_buf), addr, length, gdb_bin2hex);
	if (ret < 0) {
		return ret;
	}
...
static int gdb_read_bin_memory(FAR struct gdb_state_s *state)
{
	...
	state->pkt_buf[0] = 'b';
	ret = gdb_get_memory(state, state->pkt_buf + 1, sizeof(state->pkt_buf) - 1, addr, length, NULL);
	if (ret < 0) {
		return ret;
	}
...
static int gdb_write_memory(FAR struct gdb_state_s *state)
{
	...
	ret =
		gdb_put_memory(state, state->pkt_next, gdb_remaining_len(state), addr, length, gdb_hex2bin);
	if (ret < 0) {
		return ret;
	}
...
static int gdb_write_bin_memory(FAR struct gdb_state_s *state)
{
	...
	ret = gdb_put_memory(state, state->pkt_next, gdb_remaining_len(state), addr, length, NULL);
	if (ret < 0) {
		return ret;
	}
...
static int gdb_query(FAR struct gdb_state_s *state)
{
	...
	if (memcmp(&state->pkt_buf[1], fthread_info, sizeof(fthread_info) - 1) == 0) {
		state->pkt_buf[0] = 'm';
		state->pkt_len = 1;
		...
		gdb_send_packet(state);
	} else if (memcmp(&state->pkt_buf[1], sthread_info, sizeof(sthread_info) - 1) == 0) {
		state->pkt_buf[0] = 'l';
		state->pkt_len = 1;
		gdb_send_packet(state);
	} else if (memcmp(&state->pkt_buf[1], supported_info, sizeof(supported_info) - 1) == 0) {
#ifdef CONFIG_ARCH_HAVE_DEBUG
		state->pkt_len = sprintf(state->pkt_buf, "binary-upload+;hwbreak+;PacketSize=%zx",
								 sizeof(state->pkt_buf));
#else
		state->pkt_len =
			sprintf(state->pkt_buf, "binary-upload+;PacketSize=%zx", sizeof(state->pkt_buf));
#endif
		gdb_send_packet(state);
	} else if (memcmp(&state->pkt_buf[1], thread_extra_info, sizeof(thread_extra_info) - 1) == 0) {
		...
		snprintf(thread_info, sizeof(thread_info), "Name: %s, State: %s, Priority: %d, Stack: %zu",
				 get_task_name(tcb), thread_state, tcb->sched_priority, tcb->adj_stack_size);

		ret = gdb_bin2hex(state->pkt_buf, sizeof(state->pkt_buf), thread_info, strlen(thread_info));
		...
		len = gdb_hex2bin(cmd, len / 2, &state->pkt_buf[sizeof(r_cmd) + 1], len);
		if (len < 0) {
			return len;
		}
...
static int gdb_send_stop(FAR struct gdb_state_s *state, int stopreason, FAR void *stopaddr)
{
	int ret;
...
retry:
	...
	case GDB_STOPREASON_WATCHPOINT_RO:
		ret = sprintf(state->pkt_buf, "T05thread:%x;rwatch:%" PRIxPTR ";", state->pid + 1,
					  (uintptr_t)stopaddr);
		break;
	case GDB_STOPREASON_WATCHPOINT_WO:
		ret = sprintf(state->pkt_buf, "T05thread:%x;awatch:%" PRIxPTR ";", state->pid + 1,
					  (uintptr_t)stopaddr);
		break;
	case GDB_STOPREASON_WATCHPOINT_RW:
		ret = sprintf(state->pkt_buf, "T05thread:%x;watch:%" PRIxPTR ";", state->pid + 1,
					  (uintptr_t)stopaddr);
		break;
	case GDB_STOPREASON_BREAKPOINT:
		ret = sprintf(state->pkt_buf, "T05thread:%x;hwbreak:;", state->pid + 1);
		break;
...
static int gdb_smp_debugpoint_add(FAR void *arg)
{
	...
	return up_debugpoint_add(point->type, point->addr, point->size, point->callback, point->arg);
}
...
static void gdb_debugpoint_callback(int type, FAR void *addr, size_t size, FAR void *arg)
{
	int stopreason;
...
static int gdb_debugpoint(FAR struct gdb_state_s *state, bool enable)
{
	...
	if (enable) {
		ret = gdb_debugpoint_add(type, (FAR void *)addr, size, gdb_debugpoint_callback, state);
	} else {
...
{
	int ret = gdb_debugpoint_add(GDB_STOPREASON_STEPPOINT, NULL, 0, gdb_debugpoint_callback, state);
...
int gdb_debugpoint_add(int type, FAR void *addr, size_t size, debug_callback_t callback,
					   FAR void *arg)
{
#ifdef CONFIG_SMP
	...
	point.arg = arg;
	return nxsched_smp_call((1 << CONFIG_SMP_NCPUS) - 1, gdb_smp_debugpoint_add, &point);
#else
...
int gdb_debugpoint_remove(int type, FAR void *addr, size_t size)
{
	...
	return nxsched_smp_call((1 << CONFIG_SMP_NCPUS) - 1, gdb_smp_debugpoint_remove, &point);
#else
...
FAR struct gdb_state_s *gdb_state_init(gdb_send_func_t send, gdb_recv_func_t recv,
									   gdb_monitor_func_t monitor, FAR void *priv)
{
...
int gdb_console_message(FAR struct gdb_state_s *state, FAR const char *msg)
{
	...
	state->pkt_buf[0] = 'O';
	ret = gdb_bin2hex(&state->pkt_buf[1], sizeof(state->pkt_buf) - 1, msg, strlen(msg));
	if (ret < 0) {
		return ret;
	}
...
int gdb_process(FAR struct gdb_state_s *state, int stopreason, FAR void *stopaddr)
{
	int ret;
