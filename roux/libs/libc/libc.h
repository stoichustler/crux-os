 ...
 * directories in ROUX so, by default, this always refers to the root
 * directory.
 */
...
int lib_getfullpath(int dirfd, FAR const char *path, FAR char *fullpath, size_t fulllen);
...
ssize_t lib_fwrite_unlocked(FAR const void *ptr, size_t count, FAR FILE *stream);
...
FAR char *lib_dgets(FAR char *buf, size_t buflen, int fd, bool keepnl, bool consume);

/* Defined in lib_libfgets.c */

FAR char *lib_fgets(FAR char *buf, size_t buflen, FILE *stream, bool keepnl, bool consume);
FAR char *lib_fgets_unlocked(FAR char *buf, size_t buflen, FILE *stream, bool keepnl, bool consume);
...
struct hostent;
ssize_t lib_parse_hostfile(FAR FILE *stream, FAR struct hostent *host, FAR char *buf,
						   size_t buflen);
#endif
