...
static bool lib_lo_ipv4match(FAR const void *addr, socklen_t len, int type)
{
	...
	if (type == AF_INET && len >= sizeof(struct in_addr)) {
		...
		return net_ipv4addr_maskcmp(ipv4addr->s_addr, g_lo_ipv4addr, g_lo_ipv4mask);
	}
...
static bool lib_lo_ipv6match(FAR const void *addr, socklen_t len, int type)
{
	...
	if (type == AF_INET6 && len >= sizeof(struct in6_addr)) {
		...
		return net_ipv6addr_maskcmp(ipv6addr->s6_addr16, g_lo_ipv6addr, g_lo_ipv6mask);
	}
...
static int lib_localhost(FAR const void *addr, socklen_t len, int type, FAR struct hostent_s *host,
...
int lib_hostfile_lookup(FAR const void *addr, socklen_t len, int type, FAR struct hostent_s *host,
						FAR char *buf, size_t buflen, FAR int *h_errnop)
{
	FAR FILE *stream;
	int herrnocode;
	int nread;
 ...
 *   is used internally by ROUX to implement the POSIX gethostbyaddr().
 *
 * Input Parameters:
 *   addr - The address of the host to find.
 *   len - The length of the address
