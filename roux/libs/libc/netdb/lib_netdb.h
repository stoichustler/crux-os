...
#define CONFIG_NETDB_MAX_IPADDR (CONFIG_NETDB_MAX_IPv4ADDR + CONFIG_NETDB_MAX_IPv6ADDR)
...
bool convert_hostent(FAR const struct hostent_s *in, int type, FAR struct hostent *out);

ssize_t parse_hostfile(FAR FILE *stream, FAR struct hostent_s *host, FAR char *buf, size_t buflen);
