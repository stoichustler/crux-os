...
FAR static struct ai_s *alloc_ai(int family, int socktype, int protocol, int port,
								 FAR const void *addr)
{
	FAR struct ai_s *ai;
...
int getaddrinfo(FAR const char *hostname, FAR const char *servname, FAR const struct addrinfo *hint,
				FAR struct addrinfo **res)
{
	...
	const int valid_flags = AI_PASSIVE | AI_CANONNAME | AI_NUMERICHOST | AI_NUMERICSERV |
	...
	if (hint) {
		...
		if (family != AF_INET && family != AF_INET6 && family != AF_LOCAL && family != AF_RPMSG &&
			family != AF_VSOCK && family != AF_UNSPEC) {
			return EAI_FAMILY;
		}
	}
	...
	*res = NULL;
...
#ifdef CONFIG_NET_IPv4
		if (family == AF_INET || family == AF_UNSPEC) {
			ai = alloc_ai(AF_INET, socktype, proto, port, &g_lo_ipv4addr);
			if (ai != NULL) {
				*res = (FAR struct addrinfo *)ai;
			}
		}
...
#ifdef CONFIG_NET_IPv6
		if (family == AF_INET6 || family == AF_UNSPEC) {
			ai = alloc_ai(AF_INET6, socktype, proto, port, &g_lo_ipv6addr);
			if (ai != NULL) {
				/* Can return both IPv4 and IPv6 loopback. */
	...
	gethostentbyname_r(hostname, &host, hostbuffer, CONFIG_NETDB_BUFSIZE, &ret, flags);
	if (ret != OK) {
		lib_free(hostbuffer);
		return ret;
	}
	...
	for (i = 0; host.h_addr_list[i]; i++) {
		...
		ai = alloc_ai(host.h_addrtypes[i], socktype, proto, port, host.h_addr_list[i]);
		if (ai == NULL) {
			if (*res) {
				freeaddrinfo(*res);
			}
