...
int getnameinfo(FAR const struct sockaddr *addr, socklen_t addrlen, FAR char *host,
				socklen_t hostlen, FAR char *serv, socklen_t servlen, int flags)
{
	...
	if (host && !(flags & NI_NUMERICHOST)) {
		...
		ret = gethostbyaddr_r(saddr, saddr_len, addr->sa_family, &hostent, host, hostlen, &res,
							  &error);
	...
	if (serv && !(flags & NI_NUMERICSERV)) {
		...
		ret = getservbyport_r(port, flags & NI_DGRAM ? "udp" : "tcp", &servent, serv, servlen,
							  &result);
