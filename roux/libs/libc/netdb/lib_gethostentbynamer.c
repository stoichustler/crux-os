...
static int lib_numeric_address(FAR const char *name, FAR struct hostent_s *host, FAR char *buf,
							   size_t buflen)
{
...
static int lib_localhost(FAR const char *name, FAR struct hostent_s *host, FAR char *buf,
						 size_t buflen)
{
	FAR struct hostent_info_s *info;
	FAR char *dest;
...
static int lib_find_answer(FAR const char *name, FAR struct hostent_s *host, FAR char *buf,
						   size_t buflen)
{
	FAR struct hostent_info_s *info;
	FAR char *ptr;
...
#ifdef CONFIG_NETDB_DNSCLIENT
static int lib_dns_query(FAR const char *hostname, FAR union dns_addr_u *addr, FAR int *naddr)
{
...
#ifdef CONFIG_NETDB_DNSCLIENT
static int lib_dns_lookup(FAR const char *name, FAR struct hostent_s *host, FAR char *buf,
						  size_t buflen)
{
	FAR struct hostent_info_s *info;
	FAR char *ptr;
	...
	namelen = strlen(name);
	if (buflen < sizeof(struct hostent_info_s) - 1 + sizeof(union dns_addr_u) + namelen + 1) {
		return -ERANGE;
	}
...
static int lib_hostfile_lookup(FAR const char *name, FAR struct hostent_s *host, FAR char *buf,
							   size_t buflen)
{
	FAR FILE *stream;
	int nread;
...
int gethostentbyname_r(FAR const char *name, FAR struct hostent_s *host, FAR char *buf,
