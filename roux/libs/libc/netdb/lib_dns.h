...
void dns_save_answer(FAR const char *hostname, FAR const union dns_addr_u *addr, int naddr,
					 uint32_t ttl);
#endif
...
#if CONFIG_NETDB_DNSCLIENT_ENTRIES > 0
int dns_find_answer(FAR const char *hostname, FAR union dns_addr_u *addr, FAR int *naddr);
#endif
