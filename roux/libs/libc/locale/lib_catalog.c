...
begin_packed_struct struct cathdr_s {
	uint32_t magic;
	uint32_t nsets;
	uint32_t size;
	uint32_t msg_offset;
...
begin_packed_struct struct catset_s {
	uint32_t setno;
	uint32_t nmsgs;
	uint32_t index;
} end_packed_struct;

begin_packed_struct struct catmsg_s {
	uint32_t msgno;
	uint32_t msglen;
	uint32_t offset;
} end_packed_struct;
...
FAR char *catgets(nl_catd catd, int set_id, int msg_id, FAR const char *s)
{
	...
	FAR const struct catmsg_s *msg =
		(FAR const struct catmsg_s *)((FAR const char *)(hdr + 1) + be32toh(hdr->msg_offset));
	FAR const char *string = ((FAR const char *)(hdr + 1) + be32toh(hdr->txt_offset));
