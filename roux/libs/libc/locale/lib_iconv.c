...
static const unsigned char g_charmaps[] = {"utf8\0char\0\0\310"
										   "wchart\0\0\306"
										   "ucs2be\0\0\304"
										   "ucs2le\0\0\305"
										   "utf16be\0\0\302"
...
iconv_t iconv_open(FAR const char *to, FAR const char *from)
{
	...
	if ((t = find_charmap(to)) == -1 || (f = find_charmap(from)) == -1 || (g_charmaps[t] >= 0330)) {
		set_errno(EINVAL);
		return (iconv_t)-1;
	}
...
size_t iconv(iconv_t cd, FAR char **in, FAR size_t *inb, FAR char **out, FAR size_t *outb)
{
	FAR struct stateful_cd *scd;
	...
	for (; *inb; *in += l, *inb -= l) {
		...
		case BIG5: {
			...
			if (c - 0xa1 >= 0xfa - 0xa1) {
				...
				c = (g_hkscs[4867 + (c * 157 + d) / 16] >> (c * 157 + d) % 16) % 2 << 17 |
					g_hkscs[c * 157 + d];
				...
				if (c / 256 == 0xdc) {
					...
					ptmp = tmp.c;
					out_len = sizeof(tmp);
					tmpx = iconv(combine_to_from(to, find_charmap("utf8")), &in_buf, &in_len, &ptmp,
								 &out_len);
					tmplen = ptmp - tmp.c;
					if (tmplen > *outb) {
						goto toobig;
			...
			c -= 0xa1;
			c = g_big5[c][d] | (c == 0x27 && (d == 0x3a || d == 0x3c || d == 0x42)) << 17;
			if (!c) {
				goto ilseq;
			}
		} break;
