...
static FAR const char *evalexpr(FAR struct eval_s *ev, FAR const char *s, int d);
...
static FAR void *momap(FAR const char *path, FAR size_t *size)
{
	...
	if (fstat(fd, &st) >= 0) {
		*size = st.st_size;
		...
		if (map != MAP_FAILED && map[0] != MO_MAGIC && map[0] != __swap_uint32(MO_MAGIC)) {
			munmap(map, *size);
			map = MAP_FAILED;
		}
	}
...
static FAR char *molookup(FAR char *p, size_t size, FAR const char *s)
{
	...
	if (n > size / 8 || o > size - 8 * n || t > size - 8 * n || (o | t) % 4 != 0) {
		return NULL;
	}
...
static FAR const char *evalprim(FAR struct eval_s *ev, FAR const char *s, int d)
{
	FAR char *e;
...
static FAR const char *evalbinop(FAR struct eval_s *ev, FAR const char *s, int minprec, int d)
{
...
FAR char *dgettext(FAR const char *domainname, FAR const char *msgid)
{
	return dcgettext(domainname, msgid, LC_MESSAGES);
}

FAR char *dcgettext(FAR const char *domainname, FAR const char *msgid, int category)
{
	return dcngettext(domainname, msgid, NULL, 1, category);
}

FAR char *ngettext(FAR const char *msgid1, FAR const char *msgid2, unsigned long int n)
{
	return dngettext(NULL, msgid1, msgid2, n);
}

FAR char *dngettext(FAR const char *domainname, FAR const char *msgid1, FAR const char *msgid2,
					unsigned long int n)
{
	return dcngettext(domainname, msgid1, msgid2, n, LC_MESSAGES);
}

FAR char *dcngettext(FAR const char *domainname, FAR const char *msgid1, FAR const char *msgid2,
					 unsigned long int n, int category)
{
	...
	snprintf(path, PATH_MAX, CONFIG_LIBC_LOCALE_PATH "/%s/%s/%s.mo", lang, g_catname[category],
			 domainname);
...
FAR char *bindtextdomain(FAR const char *domainname, FAR const char *dirname)
{
...
FAR char *bind_textdomain_codeset(FAR const char *domainname, FAR const char *codeset)
{
