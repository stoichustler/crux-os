...
static void _add_hi20(void *arch_data, uintptr_t hi20_rel, uintptr_t hi20_offset)
{
...
int up_relocate(const Elf_Rel *rel, const Elf_Sym *sym, uintptr_t addr, void *arch_data)
{
	berr("Not implemented\n");
	return -ENOSYS;
}

int up_relocateadd(const Elf_Rela *rel, const Elf_Sym *sym, uintptr_t addr, void *arch_data)
{
	long offset;
	...
	if (relotype == R_RISCV_RELAX) {
		...
		binfo("%s at %08" PRIxPTR " [%08" PRIx32 "]\n", _get_rname(relotype), addr,
			  _get_val((uint16_t *)addr));
	...
	case R_RISCV_64: {
		binfo("%s at %08" PRIxPTR " [%08" PRIx32 "] "
			  ...
			  _get_rname(relotype), addr, _get_val((uint16_t *)addr), sym,
	...
	case R_RISCV_PCREL_LO12_S: {
		...
		val = (((int32_t)imm_lo >> 5) << 25) + (((int32_t)imm_lo & 0x1f) << 7);
	...
	case R_RISCV_LO12_S: {
		...
		uint32_t val = (((int32_t)imm_lo >> 5) << 25) + (((int32_t)imm_lo & 0x1f) << 7);
