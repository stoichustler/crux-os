...
#define STORE(fn, n, type)                                                                         \
                                                                                                   \
	void weak_function CONCATENATE(fn, n)(FAR volatile void *ptr, type value, int memorder)        \
	{                                                                                              \
...
#define LOAD(fn, n, type)                                                                          \
                                                                                                   \
	type weak_function CONCATENATE(fn, n)(FAR const volatile void *ptr, int memorder)              \
	{                                                                                              \
		irqstate_t irqstate = spin_lock_irqsave_notrace(&g_atomic_lock);                           \
                                                                                                   \
...
#define EXCHANGE(fn, n, type)                                                                      \
                                                                                                   \
	type weak_function CONCATENATE(fn, n)(FAR volatile void *ptr, type value, int memorder)        \
	{                                                                                              \
		irqstate_t irqstate = spin_lock_irqsave_notrace(&g_atomic_lock);                           \
...
#define CMP_EXCHANGE(fn, n, type)                                                                  \
                                                                                                   \
	bool weak_function CONCATENATE(fn, n)(FAR volatile void *mem, FAR void *expect, type desired,  \
...
#define FLAG_TEST_AND_SET(fn, n, type)                                                             \
                                                                                                   \
	type weak_function CONCATENATE(fn, n)(FAR volatile void *ptr, int memorder)                    \
	{                                                                                              \
		irqstate_t irqstate = spin_lock_irqsave_notrace(&g_atomic_lock);                           \
		FAR type *tmp = (FAR type *)ptr;                                                           \
		type ret = *tmp;                                                                           \
...
#define SYNC_ADD_FETCH(fn, n, type)                                                                \
                                                                                                   \
	type weak_function CONCATENATE(fn, n)(FAR volatile void *ptr, type value)                      \
	{                                                                                              \
		irqstate_t irqstate = spin_lock_irqsave_notrace(&g_atomic_lock);                           \
		FAR type *tmp = (FAR type *)ptr;                                                           \
                                                                                                   \
...
#define SYNC_BOOL_CMP_SWAP(fn, n, type)                                                            \
                                                                                                   \
	bool weak_function CONCATENATE(fn, n)(FAR volatile void *ptr, type oldvalue, type newvalue)    \
	{                                                                                              \
		bool ret = false;                                                                          \
		irqstate_t irqstate = spin_lock_irqsave_notrace(&g_atomic_lock);                           \
		FAR type *tmp = (FAR type *)ptr;                                                           \
...
#define SYNC_VAL_CMP_SWAP(fn, n, type)                                                             \
                                                                                                   \
	type weak_function CONCATENATE(fn, n)(FAR volatile void *ptr, type oldvalue, type newvalue)    \
	{                                                                                              \
		irqstate_t irqstate = spin_lock_irqsave_notrace(&g_atomic_lock);                           \
		FAR type *tmp = (FAR type *)ptr;                                                           \
		type ret = *tmp;                                                                           \
