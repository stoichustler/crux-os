...
static enum slcdret_e slcd_reget(FAR struct slcdstate_s *state, FAR uint8_t *pch, FAR uint8_t *parg)
{
...
enum slcdret_e slcd_decode(FAR struct lib_instream_s *stream, FAR struct slcdstate_s *state,
						   FAR uint8_t *pch, FAR uint8_t *parg)
{
	...
	if (!IS_HEX(ch)) {
		...
		code = CODE_RETURN(ch);
		count = slcd_nibble(state->buf[NDX_COUNTH]) << 4 | slcd_nibble(state->buf[NDX_COUNTL]);
