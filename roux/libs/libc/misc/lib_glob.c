...
static int append(FAR struct match_s **tail, FAR const char *name, size_t len, int mark);
static int do_glob(FAR char *buf, size_t pos, int type, FAR char *pat, int flags,
				   CODE int (*errfunc)(FAR const char *path, int err), FAR struct match_s **tail);
				   ...
				   CODE int (*errfunc)(FAR const char *path, int err), FAR struct match_s **tail)
{
	...
	if (!*pat) {
		...
		if (!type && lstat(buf, &st)) {
			if (get_errno() != ENOENT && (errfunc(buf, get_errno()) || (flags & GLOB_ERR) != 0)) {
				return GLOB_ABORTED;
			}
	...
	while (get_errno() = 0, de = readdir(dir)) {
		...
		fnm_flags = ((flags & GLOB_NOESCAPE) ? FNM_NOESCAPE : 0) | FNM_PERIOD;
		...
		r = do_glob(buf, pos + l, de->d_type, p2 ? p2 : "", flags, errfunc, tail);
		if (r) {
			closedir(dir);
			return r;
		}
...
int glob(FAR const char *pat, int flags, CODE int (*errfunc)(FAR const char *path, int err),
		 FAR glob_t *g)
{
	struct match_s head;
	...
	if (flags & GLOB_APPEND) {
		FAR char **pathv =
			lib_realloc(g->gl_pathv, (offs + g->gl_pathc + cnt + 1) * sizeof(FAR char *));
		if (!pathv) {
			freelist(&head);
			return GLOB_NOSPACE;
		}
