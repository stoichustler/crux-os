...
/* References:
 *
 *   crc16xmodem_tab calculated by Mark G. Mendel, Network Systems Corporation.
...
static const uint16_t crc16xmodem_tab[256] = {
...
uint16_t crc16xmodempart(FAR const uint8_t *src, size_t len, uint16_t crc16val)
{
	size_t i;

	for (i = 0; i < len; i++) {
		crc16val = crc16xmodem_tab[((crc16val >> 8) & 0xff) ^ src[i]] ^ (crc16val << 8);
	}
