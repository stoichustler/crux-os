...
#define BACKTRACE_NUMBER                                                                           \
	(CONFIG_LIBC_BACKTRACE_BUFFSIZE / (sizeof(struct backtrace_entry_s) + sizeof(int)))
...
static int backtrace_hash(FAR struct backtrace_pool_s *bp, FAR const void *backtrace, int depth)
{
...
static int backtrace_exist(FAR struct backtrace_pool_s *bp, int slot, FAR void *backtrace,
						   int depth)
{
...
int backtrace_record(int skip)
{
	...
	depth = sched_backtrace(_SCHED_GETTID(), buffer, CONFIG_LIBC_BACKTRACE_DEPTH, skip);
	if (depth <= 0) {
		return -ENOENT;
	}
...
nosanitize_address int backtrace_format(FAR char *buffer, int size, FAR void *backtrace[],
										int depth)
{
	...
	for (i = 0; i < depth && backtrace[i]; i++) {
		...
		len += snprintf(buffer + i * BACKTRACE_PTR_FMT_WIDTH, size - i * BACKTRACE_PTR_FMT_WIDTH,
