...
key_t ftok(FAR const char *pathname, int proj_id)
{
	...
	snprintf(fullpath, PATH_MAX, "%s/", CONFIG_LIBC_FTOK_VFS_PATH);
	...
	if (stat(fullpath, &st) < 0 && get_errno() == ENOENT) {
		...
		if (mkdir(fullpath, S_IRWXU) < 0 || stat(fullpath, &st) < 0) {
	...
	lib_put_pathbuffer(fullpath);
	return ((key_t)proj_id << 24 | (key_t)(st.st_dev & 0xff) << 16 | (key_t)(st.st_ino & 0xffff));
}
