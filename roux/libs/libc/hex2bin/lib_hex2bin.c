...
static int readstream(FAR struct lib_instream_s *instream, FAR uint8_t *line, unsigned int lineno)
{
	int nbytes = 0;
	int ch;
...
static inline void writedata(FAR struct lib_sostream_s *outstream, FAR uint8_t *data, int bytecount)
{
...
int hex2bin(FAR struct lib_instream_s *instream, FAR struct lib_sostream_s *outstream,
			unsigned long baseaddr, unsigned long endpaddr, enum hex2bin_swap_e swap)
{
	FAR uint8_t *alloc;
	FAR uint8_t *line;
	FAR uint8_t *bin;
	...
	while ((nbytes = readstream(instream, line, lineno)) != EOF) {
		...
		address16 = (uint16_t)bin[ADDRESS_BINNDX] << 8 | (uint16_t)bin[ADDRESS_BINNDX + 1];
		...
		case RECORD_DATA: /* Data */
		{
			...
			if (address != expected) {
				off_t pos = lib_stream_seek(outstream, address - baseaddr, SEEK_SET);
				if (pos == (off_t)-1) {
		...
		case RECORD_EXT_SEGADDR: /* Extended segment address record */
			...
			if (bytecount != 2 || address16 != 0 || bin[DATA_BINNDX + 1] != 0) {
				...
				lerr("  bytecount=%d address=%04x segment=%02x%02x\n", bytecount, address16,
					 bin[DATA_BINNDX], bin[DATA_BINNDX + 1]);
				goto errout_with_einval;
			}
		...
		case RECORD_EXT_LINADDR: /* Extended linear address record */
			...
			extension = (unsigned long)bin[DATA_BINNDX] << 24 | (unsigned long)bin[DATA_BINNDX + 1]
																	<< 16;
			break;
