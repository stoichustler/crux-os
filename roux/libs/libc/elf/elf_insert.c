...
void libelf_dumploadinfo(FAR struct mod_loadinfo_s *loadinfo)
{
	...
	binfo("ELF Header:\n");
	binfo("  e_ident:      %02x %02x %02x %02x\n", loadinfo->ehdr.e_ident[0],
		  loadinfo->ehdr.e_ident[1], loadinfo->ehdr.e_ident[2], loadinfo->ehdr.e_ident[3]);
...
void libelf_dumpentrypt(FAR struct mod_loadinfo_s *loadinfo)
{
	...
	if (loadinfo->ehdr.e_type == ET_REL) {
		entry = (FAR const uint8_t *)((uintptr_t)loadinfo->textalloc + loadinfo->ehdr.e_entry);
	...
	libelf_dumpbuffer("Entry code", entry, MIN(loadinfo->textsize - loadinfo->ehdr.e_entry, 512));
...
static int libelf_loadsymtab(FAR struct module_s *modp, FAR struct mod_loadinfo_s *loadinfo)
{
	...
	for (i = 0; i < symhdr->sh_size / sizeof(Elf_Sym); i++) {
		if (sym[i].st_shndx != SHN_UNDEF && sym[i].st_shndx < loadinfo->ehdr.e_shnum) {
