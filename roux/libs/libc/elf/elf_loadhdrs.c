...
int libelf_loadhdrs(FAR struct mod_loadinfo_s *loadinfo)
{
	...
	ret = libelf_read(loadinfo, (FAR uint8_t *)loadinfo->shdr, shdrsize, loadinfo->ehdr.e_shoff);
	if (ret < 0) {
	...
	if (loadinfo->ehdr.e_phnum > 0) {
		...
		ret =
			libelf_read(loadinfo, (FAR uint8_t *)loadinfo->phdr, phdrsize, loadinfo->ehdr.e_phoff);
		if (ret < 0) {
