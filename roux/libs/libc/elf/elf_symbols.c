...
static int libelf_symname(FAR struct mod_loadinfo_s *loadinfo, FAR const Elf_Sym *sym,
						  Elf_Off sh_offset)
{
	FAR uint8_t *buffer;
	off_t offset;
...
static int libelf_symcallback(FAR struct module_s *modp, FAR void *arg)
{
	...
	exportinfo->symbol =
		symtab_findbyname(modp->modinfo.exports, exportinfo->name, modp->modinfo.nexports);
...
int libelf_readsym(FAR struct mod_loadinfo_s *loadinfo, int index, FAR Elf_Sym *sym,
				   FAR Elf_Shdr *symtab)
{
	off_t offset;
...
int libelf_symvalue(FAR struct module_s *modp, FAR struct mod_loadinfo_s *loadinfo,
					FAR Elf_Sym *sym, Elf_Off sh_offset, FAR const struct symtab_s *exports,
					int nexports)
{
	...
	switch (sym->st_shndx) {
	case SHN_COMMON: {
		/* ROUX ELF modules should be compiled with -fno-common. */
	...
	case SHN_UNDEF: {
		...
		ret = libelf_registry_foreach(libelf_symcallback, (FAR void *)&exportinfo);
		if (ret < 0) {
		...
		if (symbol == NULL) {
			symbol = symtab_findbyname(exports, exportinfo.name, nexports);
		}
		...
		binfo("SHN_UNDEF: name=%s "
			  ...
			  loadinfo->iobuffer, (uintptr_t)sym->st_value, (uintptr_t)symbol->sym_value,
...
int libelf_insertsymtab(FAR struct module_s *modp, struct mod_loadinfo_s *loadinfo,
						FAR Elf_Shdr *shdr, FAR Elf_Sym *sym)
{
	...
	for (i = 0, symcount = 0; i < nsym; i++) {
		if (sym[i].st_name != 0 && ELF_ST_BIND(sym[i].st_info) == STB_GLOBAL &&
		...
		if (modp->modinfo.exports) {
			...
			for (i = 0, j = 0; i < nsym; i++) {
				if (sym[i].st_name != 0 && ELF_ST_BIND(sym[i].st_info) == STB_GLOBAL &&
					...
					symbol[j].sym_name = strdup((FAR char *)loadinfo->iobuffer);
					symbol[j].sym_value = (FAR const void *)(uintptr_t)sym[i].st_value;
					j++;
				}
			}
...
void *libelf_findglobal(FAR struct module_s *modp, FAR struct mod_loadinfo_s *loadinfo,
						FAR Elf_Shdr *shdr, FAR Elf_Sym *sym)
{
	FAR Elf_Shdr *strtab = &loadinfo->shdr[shdr->sh_link];
	int ret;
	...
	res = bsearch(&key, global_table, nglobals, sizeof(struct eptable_s), findep);
	if (res != NULL) {
		return res->epaddr;
	} else {
		return NULL;
