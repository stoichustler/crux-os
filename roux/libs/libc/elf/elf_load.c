...
#define buffer_data_address(p) (FAR uint8_t *)up_textheap_data_address((FAR void *)p)
#else
...
static int libelf_section_alloc(FAR struct mod_loadinfo_s *loadinfo, FAR Elf_Shdr *shdr,
								uint8_t idx)
{
	...
	if (loadinfo->sectalloc == NULL) {
		...
		loadinfo->sectalloc = lib_zalloc(sizeof(uintptr_t) * loadinfo->ehdr.e_shnum);
		if (loadinfo->sectalloc == NULL) {
			return -ENOMEM;
		}
	}
	...
	if ((shdr->sh_flags & SHF_WRITE) != 0) {
		...
		loadinfo->sectalloc[idx] = (uintptr_t)up_dataheap_memalign(
			(FAR const char *)loadinfo->iobuffer, shdr->sh_addralign, shdr->sh_size);
#else
		loadinfo->sectalloc[idx] = (uintptr_t)lib_memalign(shdr->sh_addralign, shdr->sh_size);
#endif
...
static int libelf_vma2lma(FAR struct mod_loadinfo_s *loadinfo, FAR Elf_Shdr *shdr,
						  FAR Elf_Addr *lma)
{
	int i;
...
static void libelf_set_emptysect_vma(FAR struct mod_loadinfo_s *loadinfo, int section)
{
...
static inline int libelf_loadfile(FAR struct mod_loadinfo_s *loadinfo)
{
	...
	if (loadinfo->ehdr.e_type == ET_DYN) {
		for (i = 0; i < loadinfo->ehdr.e_phnum; i++) {
			...
			if (phdr->p_type == PT_LOAD) {
				if (phdr->p_flags & PF_X) {
					ret = libelf_read(loadinfo, buffer_data_address(text), phdr->p_filesz,
									  phdr->p_offset);
				} else {
					size_t bsssize = phdr->p_memsz - phdr->p_filesz;
					ret = libelf_read(loadinfo, data, phdr->p_filesz, phdr->p_offset);
		...
		for (i = 0; i < loadinfo->ehdr.e_shnum; i++) {
			...
			if (shdr->sh_type != SHT_NOBITS) {
				...
				ret = libelf_read(loadinfo, buffer_data_address(*pptr), shdr->sh_size,
								  shdr->sh_offset);
				if (ret < 0) {
					berr("ERROR: Failed to read section %d: %d\n", i, ret);
					return ret;
	...
	if (loadinfo->gotindex >= 0) {
		...
		for (; got < end; got++) {
			for (i = 0; i < loadinfo->ehdr.e_shnum; i++) {
				...
				if (*got >= shdr->sh_offset && *got < shdr->sh_offset + shdr->sh_size) {
...
int libelf_load(FAR struct mod_loadinfo_s *loadinfo)
{
	...
	if (loadinfo->gotindex >= 0) {
		...
		if (ioctl(loadinfo->filfd, FIOC_XIPBASE, (unsigned long)&loadinfo->xipbase) >= 0) {
	...
	if (loadinfo->ehdr.e_type == ET_REL || loadinfo->ehdr.e_type == ET_EXEC) {
		...
		if (loadinfo->xipbase != 0) {
			loadinfo->textalloc = loadinfo->xipbase + loadinfo->shdr[1].sh_offset;
...
#else
			loadinfo->textalloc =
				(uintptr_t)lib_memalign(loadinfo->textalign, loadinfo->textsize + loadinfo->segpad);
#endif
...
#ifdef CONFIG_ARCH_USE_DATA_HEAP
			loadinfo->datastart =
				(uintptr_t)up_dataheap_memalign(loadinfo->dataalign, loadinfo->datasize);
#else
			loadinfo->datastart = (uintptr_t)lib_memalign(loadinfo->dataalign, loadinfo->datasize);
#endif
		...
		loadinfo->textalloc = (uintptr_t)lib_memalign(
			loadinfo->textalign, loadinfo->textsize + loadinfo->datasize + loadinfo->segpad);
	...
	if (ret < 0) {
		...
		up_init_exidx(loadinfo->shdr[ret].sh_addr, loadinfo->shdr[ret].sh_size);
	}
#endif
...
int libelf_load_with_addrenv(FAR struct mod_loadinfo_s *loadinfo)
{
	...
	ret = libelf_addrenv_alloc(loadinfo, loadinfo->textsize, loadinfo->datasize);
	if (ret < 0) {
