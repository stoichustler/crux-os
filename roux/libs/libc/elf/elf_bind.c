...
/* REVISIT:  This naming breaks the ROUX coding standard, but is consistent
...
static inline int libelf_readrels(FAR struct mod_loadinfo_s *loadinfo, FAR const Elf_Shdr *relsec,
								  int index, FAR Elf_Rel *rels, int count)
{
	off_t offset;
	int size;
	...
	return libelf_read(loadinfo, (FAR uint8_t *)rels, size, relsec->sh_offset + offset);
}
...
static inline int libelf_readrelas(FAR struct mod_loadinfo_s *loadinfo, FAR const Elf_Shdr *relsec,
								   int index, FAR Elf_Rela *relas, int count)
{
	off_t offset;
	int size;
	...
	return libelf_read(loadinfo, (FAR uint8_t *)relas, size, relsec->sh_offset + offset);
}
...
static int libelf_relocate(FAR struct module_s *modp, FAR struct mod_loadinfo_s *loadinfo,
						   int relidx, FAR const struct symtab_s *exports, int nexports)
{
	...
	rels = lib_malloc(CONFIG_LIBC_ELF_RELOCATION_BUFFERCOUNT * sizeof(Elf_Rel));
	if (!rels) {
	...
	for (i = j = 0; i < relsec->sh_size / sizeof(Elf_Rel); i++) {
		...
		if (!(i % CONFIG_LIBC_ELF_RELOCATION_BUFFERCOUNT)) {
			ret =
				libelf_readrels(loadinfo, relsec, i, rels, CONFIG_LIBC_ELF_RELOCATION_BUFFERCOUNT);
			if (ret < 0) {
		...
		if (sym == NULL) {
			...
			ret = libelf_readsym(loadinfo, symidx, sym, &loadinfo->shdr[loadinfo->symtabidx]);
			if (ret < 0) {
				berr("ERROR: Section %d reloc %d: "
			...
			ret = libelf_symvalue(modp, loadinfo, sym,
								  loadinfo->shdr[loadinfo->strtabidx].sh_offset, exports, nexports);
			if (ret < 0) {
		...
		if (loadinfo->gotindex >= 0) {
			if (sym->st_shndx == SHN_UNDEF) {
				...
				FAR uintptr_t *gotaddr =
					(FAR uintptr_t *)(gotsec->sh_addr +
			...
			if (rel->r_offset - dstsec->sh_offset > dstsec->sh_size) {
				berr("ERROR: Section %d reloc %d: "
					 "Relocation address out of range, "
					 "offset %" PRIuPTR " size %ju\n",
					 relidx, i, (uintptr_t)rel->r_offset, (uintmax_t)dstsec->sh_size);
				ret = -EINVAL;
				break;
			}
			...
			if (ELF_ST_TYPE(sym->st_info) == STT_SECTION) {
				...
				*(FAR uintptr_t *)addr -= loadinfo->shdr[sym->st_shndx].sh_offset;
			} else {
...
static int libelf_relocateadd(FAR struct module_s *modp, FAR struct mod_loadinfo_s *loadinfo,
							  int relidx, FAR const struct symtab_s *exports, int nexports)
{
	FAR Elf_Shdr *relsec = &loadinfo->shdr[relidx];
	FAR Elf_Shdr *dstsec = &loadinfo->shdr[relsec->sh_info];
	...
	relas = lib_malloc(CONFIG_LIBC_ELF_RELOCATION_BUFFERCOUNT * sizeof(Elf_Rela));
	if (!relas) {
	...
	for (i = j = 0; i < relsec->sh_size / sizeof(Elf_Rela); i++) {
		...
		if (rela->r_offset < 0 || rela->r_offset > dstsec->sh_size) {
			berr("ERROR: Section %d reloc %d: "
				 "Relocation address out of range, "
				 "offset %" PRIuPTR " size %ju\n",
				 relidx, i, (uintptr_t)rela->r_offset, (uintmax_t)dstsec->sh_size);
			ret = -EINVAL;
			break;
		}
...
static int libelf_relocatedyn(FAR struct module_s *modp, FAR struct mod_loadinfo_s *loadinfo,
							  int relidx)
{
	...
	ret = libelf_read(loadinfo, (FAR uint8_t *)dyn, shdr->sh_size, shdr->sh_offset);
	if (ret < 0) {
	...
	rels = lib_zalloc(CONFIG_LIBC_ELF_RELOCATION_BUFFERCOUNT * sizeof(Elf_Rela));
	if (!rels) {
		berr("Failed to allocate memory for elf relocation rels\n");
		lib_free(dyn);
		return -ENOMEM;
	...
	ret = libelf_read(loadinfo, (FAR uint8_t *)sym, symhdr->sh_size, symhdr->sh_offset);
	if (ret < 0) {
	...
	for (idx_rel = 0; idx_rel < N_RELS; idx_rel++) {
		...
		for (i = 0; i < lrelent; i++) {
			...
			if (!(i % CONFIG_LIBC_ELF_RELOCATION_BUFFERCOUNT)) {
				size_t relsize = (sizeof(Elf_Rela) * CONFIG_LIBC_ELF_RELOCATION_BUFFERCOUNT);

				if (reldata.relsz[idx_rel] < relsize) {
					relsize = reldata.relsz[idx_rel];
				}

				ret = libelf_read(loadinfo, (FAR uint8_t *)rels, relsize,
								  reldata.reloff[idx_rel] + i * sizeof(Elf_Rel));
			...
			if ((idx_sym = ELF_R_SYM(rel->r_info)) != 0) {
				...
				if (sym[idx_sym].st_shndx == SHN_UNDEF) {
					...
					ep = libelf_findglobal(modp, loadinfo, symhdr, &sym[idx_sym]);
					if ((ep == NULL) && (ELF_ST_BIND(sym[idx_sym].st_info) != STB_WEAK)) {
				...
				if ((*(FAR uint32_t *)addr) < loadinfo->datasec) {
					dynsym.st_value = *(FAR uint32_t *)addr + loadinfo->textalloc;
				} else {
					dynsym.st_value =
						*(FAR uint32_t *)addr - loadinfo->datasec + loadinfo->datastart;
				}
...
int libelf_bind(FAR struct module_s *modp, FAR struct mod_loadinfo_s *loadinfo,
				FAR const struct symtab_s *exports, int nexports)
{
	...
	for (i = 1; i < loadinfo->ehdr.e_shnum; i++) {
		...
		if (loadinfo->ehdr.e_type == ET_DYN) {
			...
			case SHT_INIT_ARRAY:
				loadinfo->initarr =
					loadinfo->shdr[i].sh_addr - loadinfo->datasec + loadinfo->datastart;
			...
			case SHT_RELA:
				...
				ret = libelf_relocateadd(modp, loadinfo, i, exports, nexports);
				break;
			case SHT_INIT_ARRAY:
	...
	for (i = 0; loadinfo->ehdr.e_type == ET_REL && i < loadinfo->ehdr.e_shnum; i++) {
