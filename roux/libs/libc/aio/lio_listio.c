...
static void lio_sighandler(int signo, siginfo_t *info, void *ucontext)
{
	...
	if (ret != -EINPROGRESS) {
		...
		DEBUGVERIFY(
			nxsig_notification(sighand->pid, &sighand->sig, SI_ASYNCIO, &aiocbp->aio_sigwork));
	...
	for (i = 0; i < nent; i++) {
		...
		if (aiocbp) {
			...
			if (aiocbp->aio_result == -EINPROGRESS) {
				...
				memcpy(priv, (FAR void *)&sighand, sizeof(struct lio_sighand_s));
			}
...
int lio_listio(int mode, FAR struct aiocb *const list[], int nent, FAR struct sigevent *sig)
{
	 ...
	 */
		...
		if (nqueued > 0) {
			...
			status = nxsig_notification(_SCHED_GETPID(), sig, SI_ASYNCIO, &aiocbp->aio_sigwork);
			if (status < 0 && ret == OK) {
