 ...
 *   ROUX.  daemon() is only described in the Linux man pages like:
 *   http://man7.org/linux/man-pages/man3/daemon.3.html
 *
 * Limitations:
 *   ROUX does not implement controlling terminals so the primary purpose
 ...
 *   be translated to ROUX.  The above mentioned web page states:
 *
 *     (This function forks, and if the fork(2) succeeds, the parent calls
 *     _exit(2), so that further errors are seen by the child only.)
 *
 *   So the basic operation of the ROUX daemon() is different.  The GNU C
