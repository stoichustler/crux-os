...
static int md5_update(FAR cryptodev_context_t *ctx, FAR const void *input, size_t ilen)
{
	int ret;
...
static int md5_finish(FAR cryptodev_context_t *ctx, unsigned char output[16])
{
	int ret;
...
static FAR char *md5_crypt(FAR const char *key, FAR const char *setting, FAR char *output)
{
	...
	*p++ = '$';
	...
	for (i = 0; i < 5; i++) {
		p = md5_to64(p, (md[perm[i][0]] << 16) | (md[perm[i][1]] << 8) | md[perm[i][2]], 4);
	}
