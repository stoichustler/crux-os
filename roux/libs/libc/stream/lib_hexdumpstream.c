...
static size_t bin2hex(FAR const uint8_t *buf, size_t buflen, FAR char *hex, size_t hexlen)
{
	size_t i;
...
static void hexdumpstream_putc(FAR struct lib_outstream_s *self, int ch)
{
	...
	bin2hex(&byte, 1, stream->buffer + stream->pending, (outlen - stream->pending) / 2);
...
static ssize_t hexdumpstream_puts(FAR struct lib_outstream_s *self, FAR const void *buf, size_t len)
{
	FAR struct lib_hexdumpstream_s *stream = (FAR void *)self;
	...
	while (remain > 0) {
		...
		ret = bin2hex(p, ret, stream->buffer + stream->pending, (outlen - stream->pending) / 2);
