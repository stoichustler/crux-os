...
#define buf_arg(buf, type)                                                                         \
	((buf) = (FAR char *)(buf) + sizeof(*(type)0), (type)((FAR char *)(buf) - sizeof(*(type)0)))

#define next_arg(varg, vabuf, type) (varg) ? va_arg((vabuf).ap, type) : buf_arg((vabuf).buf, type)
...
static FAR const char *findscanset(FAR const char *fmt, FAR unsigned char set[32])
{
	int c;
	int n;
	int v;
...
static int vscanf_internal(FAR struct lib_instream_s *stream, FAR int *lastc,
						   FAR const IPTR char *fmt, bool varg, union vabuf_u vabuf)
{
	int c;
	FAR char *tv;
	FAR const char *tc;
		...
		if (fmt_char(fmt) == '%') {
			...
			if (fmt_char(fmt) == 's') {
				...
				if (c > 0) {
					...
					fwidth = 0;
					while ((!width || (fwidth < width)) && (c > 0) && !isspace(c)) {
						if (!noassign) {
							tv[fwidth] = c;
						}
...
#ifdef CONFIG_HAVE_LONG_LONG
					case LL_MOD:
						plonglong = next_arg(varg, vabuf, FAR unsigned long long *);
						*plonglong = 0;
						break;
#endif
					}
