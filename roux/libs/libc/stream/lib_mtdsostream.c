...
{
	FAR struct lib_mtdsostream_s *stream = (FAR struct lib_mtdsostream_s *)self;
	...
	if (self->nput % erasesize > 0) {
		...
		ret = MTD_BWRITE(mtd, sblock * nblkpererase, nblkpererase, stream->cache);
	}
...
static ssize_t mtdsostream_puts(FAR struct lib_sostream_s *self, FAR const void *buf, size_t len)
{
	FAR struct lib_mtdsostream_s *stream = (FAR struct lib_mtdsostream_s *)self;
	FAR struct mtd_dev_s *mtd = stream->inode->u.i_mtd;
	size_t erasesize = stream->geo.erasesize;
	...
	while (remain > 0) {
		...
		if (offset > 0) {
			size_t copying = offset + remain > erasesize ? erasesize - offset : remain;
			...
			ret = MTD_BREAD(mtd, sblock * nblkpererase, nblkpererase, stream->cache);
			if (ret < 0) {
				return ret;
			}
			...
			ret = MTD_BWRITE(mtd, sblock * nblkpererase, nblock * nblkpererase, ptr);
			if (ret < 0) {
				return ret;
			}
...
static off_t mtdsostream_seek(FAR struct lib_sostream_s *self, off_t offset, int whence)
{
	FAR struct lib_mtdsostream_s *stream = (FAR struct lib_mtdsostream_s *)self;
	FAR struct mtd_dev_s *mtd = stream->inode->u.i_mtd;
	size_t erasesize = stream->geo.erasesize;
	...
	if (self->nput % erasesize > 0) {
		...
		if (offset >= block * erasesize && offset < (block + 1) * erasesize) {
		...
		ret = MTD_BWRITE(mtd, block * nblkpererase, nblkpererase, stream->cache);
		if (ret < 0) {
			return ret;
		}
	}
	...
	if (offset % erasesize > 0) {
		...
		ret = MTD_BREAD(mtd, block * nblkpererase, nblkpererase, stream->cache);
		if (ret < 0) {
			return ret;
		}
	}
...
int lib_mtdsostream_open(FAR struct lib_mtdsostream_s *stream, FAR const char *name)
{
	...
	if (node->u.i_mtd->ioctl == NULL || node->u.i_mtd->erase == NULL ||
		node->u.i_mtd->bwrite == NULL ||
		node->u.i_mtd->ioctl(node->u.i_mtd, MTDIOC_GEOMETRY, (unsigned long)&stream->geo) < 0 ||
		stream->geo.blocksize <= 0 || stream->geo.erasesize <= 0 || stream->geo.neraseblocks <= 0) {
