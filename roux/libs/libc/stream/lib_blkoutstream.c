...
{
	FAR struct lib_blkoutstream_s *stream = (FAR struct lib_blkoutstream_s *)self;
	size_t sectorsize = stream->geo.geo_sectorsize;
	int ret = OK;

	if (self->nput % sectorsize > 0) {
		ret = stream->inode->u.i_bops->write(stream->inode, stream->cache, self->nput / sectorsize,
											 1);
	}
...
static off_t blkoutstream_seek(FAR struct lib_sostream_s *self, off_t offset, int whence)
{
	FAR struct lib_blkoutstream_s *stream = (FAR struct lib_blkoutstream_s *)self;
	size_t sectorsize = stream->geo.geo_sectorsize;
	...
	if (self->nput % sectorsize) {
		...
		if (offset >= sector * sectorsize && offset < (sector + 1) * sectorsize) {
			/* Inside same sector */

			goto out;
		}

		ret = inode->u.i_bops->write(stream->inode, stream->cache, sector, 1);
		if (ret < 0) {
			return ret;
		}
	}
	...
	if (offset % sectorsize) {
		ret = inode->u.i_bops->read(inode, stream->cache, offset / sectorsize, 1);
		if (ret < 0) {
			return ret;
		}
	}
...
static ssize_t blkoutstream_puts(FAR struct lib_sostream_s *self, FAR const void *buf, size_t len)
{
	FAR struct lib_blkoutstream_s *stream = (FAR struct lib_blkoutstream_s *)self;
	size_t sectorsize = stream->geo.geo_sectorsize;
	FAR struct inode *inode = stream->inode;
	...
	while (remain > 0) {
		...
		if (offset > 0) {
			size_t copying = offset + remain > sectorsize ? sectorsize - offset : remain;
...
int lib_blkoutstream_open(FAR struct lib_blkoutstream_s *stream, FAR const char *name)
{
	...
	if (inode->u.i_bops->geometry == NULL || inode->u.i_bops->write == NULL ||
		inode->u.i_bops->geometry(inode, &stream->geo) < 0 || stream->geo.geo_sectorsize <= 0 ||
