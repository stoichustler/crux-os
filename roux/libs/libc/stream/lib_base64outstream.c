...
static int base64stream_flush(FAR struct lib_outstream_s *self)
{
	...
	if (stream->nbytes > 0) {
		b64_ntop(stream->bytes, stream->nbytes, stream->buffer + stream->pending, 4 + 1);
...
static void base64stream_putc(FAR struct lib_outstream_s *self, int ch)
{
	...
	if (stream->nbytes == 3) {
		b64_ntop(stream->bytes, 3, stream->buffer + stream->pending, 4 + 1);
		stream->pending += 4;
		stream->nbytes = 0;
...
static ssize_t base64stream_puts(FAR struct lib_outstream_s *self, FAR const void *buf, size_t len)
{
	FAR struct lib_base64outstream_s *stream = (FAR void *)self;
	...
	while (remaining >= 3) {
		...
		outlen = n / 3 * 4;
		b64_ntop(input, n, stream->buffer + stream->pending, outlen + 1);
