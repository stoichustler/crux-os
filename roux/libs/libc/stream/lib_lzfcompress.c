...
{
	FAR struct lib_lzfoutstream_s *stream = (FAR struct lib_lzfoutstream_s *)self;
	FAR struct lzf_header_s *header;
	size_t outlen;

	if (stream->offset > 0) {
		outlen = lzf_compress(stream->in, stream->offset, &stream->out[LZF_MAX_HDR_SIZE],
...
static ssize_t lzfoutstream_puts(FAR struct lib_outstream_s *self, FAR const void *buf, size_t len)
{
	FAR struct lib_lzfoutstream_s *stream = (FAR struct lib_lzfoutstream_s *)self;
	FAR struct lzf_header_s *header;
	...
	while (total > 0) {
		copying = stream->offset + total > LZF_STREAM_BLOCKSIZE
					  ? LZF_STREAM_BLOCKSIZE - stream->offset
					  : total;
