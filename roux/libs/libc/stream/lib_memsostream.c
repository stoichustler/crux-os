...
{
	FAR struct lib_memsostream_s *stream = (FAR struct lib_memsostream_s *)self;
...
static ssize_t memsostream_puts(FAR struct lib_sostream_s *self, FAR const void *buf, size_t len)
{
	ssize_t ncopy;
	FAR struct lib_memsostream_s *stream = (FAR struct lib_memsostream_s *)self;
	...
	ncopy = stream->offset + len + 1 < stream->buflen ? len : stream->buflen - stream->offset - 1;
	if (ncopy > 0) {
...
static off_t memsostream_seek(FAR struct lib_sostream_s *self, off_t offset, int whence)
{
	FAR struct lib_memsostream_s *stream = (FAR struct lib_memsostream_s *)self;
	off_t newpos;
...
void lib_memsostream(FAR struct lib_memsostream_s *outstream, FAR char *bufstart, size_t buflen)
{
