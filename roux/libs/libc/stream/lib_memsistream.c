...
{
	FAR struct lib_memsistream_s *stream = (FAR struct lib_memsistream_s *)self;
	int ret = -EINVAL;
...
static ssize_t memsistream_gets(FAR struct lib_instream_s *self, FAR void *buffer, size_t len)
{
	FAR struct lib_memsistream_s *stream = (FAR struct lib_memsistream_s *)self;
	ssize_t ret = -EINVAL;
...
static off_t memsistream_seek(FAR struct lib_sistream_s *self, off_t offset, int whence)
{
	FAR struct lib_memsistream_s *stream = (FAR struct lib_memsistream_s *)self;
	off_t newpos;
...
void lib_memsistream(FAR struct lib_memsistream_s *instream, FAR const char *bufstart,
					 size_t buflen)
{
