...
static ssize_t mtdoutstream_bwrite_cache(FAR struct lib_mtdoutstream_s *stream,
...
static ssize_t mtdoutstream_bwrite(FAR struct lib_mtdoutstream_s *stream, FAR struct mtd_dev_s *mtd,
								   size_t sblock, size_t nblock, FAR const unsigned char *buf)
{
	size_t nblkpererase = stream->geo.erasesize / stream->geo.blocksize;
...
{
	FAR struct lib_mtdoutstream_s *stream = (FAR struct lib_mtdoutstream_s *)self;
...
#endif
		{
			ret = mtdoutstream_bwrite_cache(stream, mtd, self->nput / blocksize);
		}
	}
...
static ssize_t mtdoutstream_puts(FAR struct lib_outstream_s *self, FAR const void *buf, size_t len)
{
	FAR struct lib_mtdoutstream_s *stream = (FAR struct lib_mtdoutstream_s *)self;
	FAR struct mtd_dev_s *mtd = stream->inode->u.i_mtd;
...
#endif
	{
		while (remain > 0) {
			...
			if (offset > 0) {
				size_t copying = offset + remain > blocksize ? blocksize - offset : remain;
