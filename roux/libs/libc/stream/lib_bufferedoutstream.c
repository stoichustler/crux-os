...
{
	FAR struct lib_bufferedoutstream_s *stream = (FAR struct lib_bufferedoutstream_s *)self;
	int ret = OK;

	ret = lib_stream_puts(stream->backend, stream->buffer, stream->pending);
...
static ssize_t bufferedoutstream_puts(FAR struct lib_outstream_s *self, FAR const void *buf,
									  size_t len)
{
	FAR struct lib_bufferedoutstream_s *stream = (FAR struct lib_bufferedoutstream_s *)self;
	size_t ret = len;
