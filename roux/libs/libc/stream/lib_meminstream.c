...
{
	FAR struct lib_meminstream_s *stream = (FAR struct lib_meminstream_s *)self;
	int ret = -EINVAL;
...
static ssize_t meminstream_gets(FAR struct lib_instream_s *self, FAR void *buffer, size_t len)
{
	FAR struct lib_meminstream_s *stream = (FAR struct lib_meminstream_s *)self;
	ssize_t ret = -EINVAL;
	...
	if (self->nget < stream->buflen) {
		ret = stream->buflen - self->nget < len ? stream->buflen - self->nget : len;
		self->nget += ret;
