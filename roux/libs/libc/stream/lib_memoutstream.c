...
static ssize_t memoutstream_puts(FAR struct lib_outstream_s *self, FAR const void *buf, size_t len)
{
	FAR struct lib_memoutstream_s *stream = (FAR struct lib_memoutstream_s *)self;
	size_t ncopy;
	...
	ncopy = stream->buflen - self->nput >= len ? len : stream->buflen - self->nput;
	if (ncopy > 0) {
...
void lib_memoutstream(FAR struct lib_memoutstream_s *outstream, FAR char *bufstart, size_t buflen)
{
