...
typedef CODE int(grp_foreach_match_t)(FAR const struct group *entry, uintptr_t arg);
...
static int grp_foreach(grp_foreach_match_t match, uintptr_t arg, FAR struct group *entry,
...
int grp_findby_name(FAR const char *gname, FAR struct group *entry, FAR char *buffer, size_t buflen)
{
