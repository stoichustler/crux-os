...
static int call_nftw(FAR char *path, nftw_cb_t fn, int flags, int base, int level,
					 FAR const struct stat *buf, int info)
{
	struct FTW ftw = {base, level};
...
static int do_nftw(FAR char *path, nftw_cb_t fn, int fdlimit, int flags, int level)
{
	...
	if (r < 0) {
		if (!(flags & FTW_PHYS) && get_errno() == ENOENT && !lstat(path, &buf)) {
			info = FTW_SLN;
