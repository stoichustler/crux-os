...
FAR struct passwd *getpwent(void)
{
	...
	getpwent_r(&g_passwd, g_passwd_buffer, CONFIG_LIBC_PASSWD_LINESIZE, &pwd);
	return pwd;
}
...
int getpwent_r(FAR struct passwd *pwd, FAR char *buf, size_t buflen, FAR struct passwd **result)
{
	...
	*result = pwd;
	...
	ret = getpwbuf_r(ROOT_UID, ROOT_GID, ROOT_NAME, ROOT_NAME, ROOT_DIR, ROOT_SHELL, ROOT_PASSWD,
