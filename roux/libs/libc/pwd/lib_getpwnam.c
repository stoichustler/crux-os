...
FAR struct passwd *getpwnam(FAR const char *name)
{
	...
	ret = pwd_findby_name(name, &g_passwd, g_passwd_buffer, CONFIG_LIBC_PASSWD_LINESIZE);
	if (ret != 1) {
		return NULL;
	}
	...
	return getpwbuf(ROOT_UID, ROOT_GID, ROOT_NAME, ROOT_GEOCS, ROOT_DIR, ROOT_SHELL, ROOT_PASSWD);
#endif
}
