									   ...
									   FAR struct userfs_open_request_s *req, size_t reqlen)
{
	...
	resp.ret =
		info->userops->open(info->volinfo, req->relpath, req->oflags, req->mode, &resp.openinfo);

	/* Send the response */

	resp.resp = USERFS_RESP_OPEN;
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_open_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		ret = -get_errno();
										...
										FAR struct userfs_close_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_close_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
									   ...
									   FAR struct userfs_read_request_s *req, size_t reqlen)
{
	...
	resp->nread = info->userops->read(info->volinfo, req->openinfo, resp->rddata, readlen);

	/* Send the response */

	resp->resp = USERFS_RESP_READ;
	resplen = SIZEOF_USERFS_READ_RESPONSE_S(resp->nread);
	nsent = sendto(info->sockfd, resp, resplen, 0, (FAR struct sockaddr *)&info->client,
										...
										FAR struct userfs_write_request_s *req, size_t reqlen)
{
	...
	resp.nwritten = info->userops->write(info->volinfo, req->openinfo, req->wrdata, writelen);

	/* Send the response */

	resp.resp = USERFS_RESP_WRITE;
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_write_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
									   ...
									   FAR struct userfs_seek_request_s *req, size_t reqlen)
{
	...
	resp.ret = info->userops->seek(info->volinfo, req->openinfo, req->offset, req->whence);

	/* Send the response */

	resp.resp = USERFS_RESP_SEEK;
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_seek_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										...
										FAR struct userfs_ioctl_request_s *req, size_t reqlen)
{
	...
	resp.ret = info->userops->ioctl(info->volinfo, req->openinfo, req->cmd, req->arg);

	/* Send the response */

	resp.resp = USERFS_RESP_IOCTL;
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_ioctl_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
									   ...
									   FAR struct userfs_sync_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_sync_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
									  ...
									  FAR struct userfs_dup_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_dup_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										...
										FAR struct userfs_fstat_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_fstat_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										   ...
										   FAR struct userfs_truncate_request_s *req, size_t reqlen)
{
	...
	resp.resp = USERFS_RESP_FSTAT;
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_truncate_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										  ...
										  FAR struct userfs_opendir_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_opendir_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										   ...
										   FAR struct userfs_closedir_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_closedir_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										  ...
										  FAR struct userfs_readdir_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_readdir_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
											...
											size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_rewinddir_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										 ...
										 FAR struct userfs_statfs_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_statfs_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										 ...
										 FAR struct userfs_unlink_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_unlink_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										...
										FAR struct userfs_mkdir_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_mkdir_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										...
										FAR struct userfs_rmdir_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_rmdir_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										 ...
										 FAR struct userfs_rename_request_s *req, size_t reqlen)
{
	...
	resp.ret = info->userops->rename(info->volinfo, req->oldrelpath, newrelpath);

	/* Send the response */

	resp.resp = USERFS_RESP_RENAME;
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_rename_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
									   ...
									   FAR struct userfs_stat_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_stat_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										  ...
										  FAR struct userfs_destroy_request_s *req, size_t reqlen)
{
	...
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_destroy_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	if (nsent < 0) {
		int ret = -get_errno();
										  ...
										  FAR struct userfs_fchstat_request_s *req, size_t reqlen)
{
	...
	resp.ret = info->userops->fchstat(info->volinfo, req->openinfo, &req->buf, req->flags);

	/* Send the response */

	resp.resp = USERFS_RESP_FCHSTAT;
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_fchstat_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
										 ...
										 FAR struct userfs_chstat_request_s *req, size_t reqlen)
{
	...
	resp.ret = info->userops->chstat(info->volinfo, req->relpath, &req->buf, req->flags);

	/* Send the response */

	resp.resp = USERFS_RESP_CHSTAT;
	nsent = sendto(info->sockfd, &resp, sizeof(struct userfs_chstat_response_s), 0,
				   (FAR struct sockaddr *)&info->client, sizeof(struct sockaddr_in));
	return nsent < 0 ? nsent : OK;
}
 ...
 *   ROUX C library and is intended to be called by application logic.
 *
 * Input Parameters:
...
int userfs_run(FAR const char *mountpt, FAR const struct userfs_operations_s *userops,
			   FAR void *volinfo, size_t mxwrite)
{
	...
	ret = bind(info->sockfd, (struct sockaddr *)&server, sizeof(struct sockaddr_in));
	if (ret < 0) {
		ret = -get_errno();
	...
	do {
		...
		nread = recvfrom(info->sockfd, info->iobuffer, info->iolen, 0,
						 (FAR struct sockaddr *)&info->client, &addrlen);
		if (nread < 0) {
			ret = -get_errno();
		...
		case USERFS_REQ_OPEN:
			ret = userfs_open_dispatch(info, (FAR struct userfs_open_request_s *)info->iobuffer,
									   nread);
			break;

		case USERFS_REQ_CLOSE:
			ret = userfs_close_dispatch(info, (FAR struct userfs_close_request_s *)info->iobuffer,
										nread);
			break;

		case USERFS_REQ_READ:
			ret = userfs_read_dispatch(info, (FAR struct userfs_read_request_s *)info->iobuffer,
									   nread);
			break;

		case USERFS_REQ_WRITE:
			ret = userfs_write_dispatch(info, (FAR struct userfs_write_request_s *)info->iobuffer,
										nread);
			break;

		case USERFS_REQ_SEEK:
			ret = userfs_seek_dispatch(info, (FAR struct userfs_seek_request_s *)info->iobuffer,
									   nread);
			break;

		case USERFS_REQ_IOCTL:
			ret = userfs_ioctl_dispatch(info, (FAR struct userfs_ioctl_request_s *)info->iobuffer,
										nread);
			break;

		case USERFS_REQ_SYNC:
			ret = userfs_sync_dispatch(info, (FAR struct userfs_sync_request_s *)info->iobuffer,
									   nread);
			break;

		case USERFS_REQ_DUP:
			ret =
				userfs_dup_dispatch(info, (FAR struct userfs_dup_request_s *)info->iobuffer, nread);
			break;

		case USERFS_REQ_FSTAT:
			ret = userfs_fstat_dispatch(info, (FAR struct userfs_fstat_request_s *)info->iobuffer,
										nread);
			break;

		case USERFS_REQ_TRUNCATE:
			ret = userfs_truncate_dispatch(
				info, (FAR struct userfs_truncate_request_s *)info->iobuffer, nread);
			break;

		case USERFS_REQ_OPENDIR:
			ret = userfs_opendir_dispatch(
				info, (FAR struct userfs_opendir_request_s *)info->iobuffer, nread);
			break;

		case USERFS_REQ_CLOSEDIR:
			ret = userfs_closedir_dispatch(
				info, (FAR struct userfs_closedir_request_s *)info->iobuffer, nread);
			break;

		case USERFS_REQ_READDIR:
			ret = userfs_readdir_dispatch(
				info, (FAR struct userfs_readdir_request_s *)info->iobuffer, nread);
			break;

		case USERFS_REQ_REWINDDIR:
			ret = userfs_rewinddir_dispatch(
				info, (FAR struct userfs_rewinddir_request_s *)info->iobuffer, nread);
			break;

		case USERFS_REQ_STATFS:
			ret = userfs_statfs_dispatch(info, (FAR struct userfs_statfs_request_s *)info->iobuffer,
										 nread);
			break;

		case USERFS_REQ_UNLINK:
			ret = userfs_unlink_dispatch(info, (FAR struct userfs_unlink_request_s *)info->iobuffer,
										 nread);
			break;

		case USERFS_REQ_MKDIR:
			ret = userfs_mkdir_dispatch(info, (FAR struct userfs_mkdir_request_s *)info->iobuffer,
										nread);
			break;

		case USERFS_REQ_RMDIR:
			ret = userfs_rmdir_dispatch(info, (FAR struct userfs_rmdir_request_s *)info->iobuffer,
										nread);
			break;

		case USERFS_REQ_RENAME:
			ret = userfs_rename_dispatch(info, (FAR struct userfs_rename_request_s *)info->iobuffer,
										 nread);
			break;

		case USERFS_REQ_STAT:
			ret = userfs_stat_dispatch(info, (FAR struct userfs_stat_request_s *)info->iobuffer,
									   nread);
			break;

		case USERFS_REQ_DESTROY:
			ret = userfs_destroy_dispatch(
				info, (FAR struct userfs_destroy_request_s *)info->iobuffer, nread);
			break;

		case USERFS_REQ_FCHSTAT:
			ret = userfs_fchstat_dispatch(
				info, (FAR struct userfs_fchstat_request_s *)info->iobuffer, nread);
			break;

		case USERFS_REQ_CHSTAT:
			ret = userfs_chstat_dispatch(info, (FAR struct userfs_chstat_request_s *)info->iobuffer,
										 nread);
			break;
