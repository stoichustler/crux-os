				   ...
				   FAR struct nxgl_rect_s *rect, nxgl_coord_t linewidth)
{
	...
	ginfo("vector: (%d,%d)->(%d,%d) linewidth: %d\n", vector->pt1.x, vector->pt1.y, vector->pt2.x,
		  vector->pt2.y, linewidth);
	...
	} else /* if (vector->pt1.y == vector->pt2.y) */
	{
		...
		ginfo("Horizontal rect: (%d,%d),(%d,%d)\n", rect->pt1.x, rect->pt1.y, rect->pt2.x,
			  rect->pt2.y);
	...
	if (linewidth == 1 && abs(line.pt2.x - line.pt1.x) < (line.pt2.y - line.pt1.y)) {
		...
		ginfo("Vertical traps[1]: (%08" PRIx32 ",%08" PRIx32 ",%d),"
			  ...
			  traps[1].top.x1, traps[1].top.x2, traps[1].top.y, traps[1].bot.x1, traps[1].bot.x2,
			  traps[1].bot.y);
		...
		pixels_per_row = itob16(line.pt2.x - line.pt1.x) / (line.pt2.y - line.pt1.y);
	...
	if (iyoffset > 0) {
		...
		ginfo("traps[0]: (%08" PRIx32 ",%08" PRIx32 ",%d),"
			  "(%08" PRIx32 ",%08" PRIx32 ",%d)\n",
			  traps[0].top.x1, traps[0].top.x2, traps[0].top.y, traps[0].bot.x1, traps[0].bot.x2,
			  traps[0].bot.y);
		...
		ginfo("traps[2]: (%08" PRIx32 ",%08" PRIx32 ",%d),"
			  "(%08" PRIx32 ",%08" PRIx32 ",%d)\n",
			  traps[2].top.x1, traps[2].top.x2, traps[2].top.y, traps[2].bot.x1, traps[2].bot.x2,
			  traps[2].bot.y);
