...
static void nxtk_redraw(NXWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool morem,
						FAR void *arg);
...
static void nxtk_mousein(NXWINDOW hwnd, FAR const struct nxgl_point_s *pos, uint8_t buttons,
						 FAR void *arg);
#endif
#ifdef CONFIG_NX_KBD
static void nxtk_kbdin(NXWINDOW hwnd, uint8_t nch, const uint8_t *ch, FAR void *arg);
#endif
static void nxtk_event(NXWINDOW hwnd, enum nx_event_e event, FAR void *arg1, FAR void *arg2);
...
static void nxtk_redraw(NXWINDOW hwnd, FAR const struct nxgl_rect_s *rect, bool more, FAR void *arg)
{
	...
	ginfo("hwnd=%p rect={(%d,%d),(%d,%d)} more=%d\n", hwnd, rect->pt1.x, rect->pt1.y, rect->pt2.x,
		  rect->pt2.y, more);
						  ...
						  FAR void *arg)
{
	...
	ginfo("hwnd=%p size=(%d,%d) pos=(%d,%d) bounds={(%d,%d),(%d,%d)}\n", hwnd, size->w, size->h,
		  pos->x, pos->y, bounds->pt1.x, bounds->pt1.y, bounds->pt2.x, bounds->pt2.y);
	...
	if (fwnd->fwcb->position) {
		...
		fwnd->fwcb->position((NXTKWINDOW)fwnd, &subwindowsize, &fwnd->fwrect.pt1, bounds,
							 fwnd->fwarg);
	}
	...
	if (fwnd->tbcb && fwnd->tbcb->position) {
		...
		fwnd->tbcb->position((NXTKWINDOW)fwnd, &subwindowsize, &fwnd->tbrect.pt1, bounds,
							 fwnd->tbarg);
	}
}
						 ...
						 FAR void *arg)
{
	...
	if (fwnd->fwcb->mousein != NULL && nxgl_rectinside(&fwnd->fwrect, &fwnd->mpos)) {
	...
	else if (fwnd->tbcb->mousein != NULL && nxgl_rectinside(&fwnd->tbrect, &fwnd->mpos)) {
	...
	else if (fwnd->fwcb->mousein != NULL && pos->y >= fwnd->tbheight + CONFIG_NXTK_BORDERWIDTH) {
		nxgl_vectsubtract(&relpos, &abspos, &fwnd->fwrect.pt1);
		fwnd->fwcb->mousein((NXTKWINDOW)fwnd, &relpos, buttons, fwnd->fwarg);
	}
	...
	else if (fwnd->tbcb->mousein != NULL && pos->y < fwnd->tbheight + CONFIG_NXTK_BORDERWIDTH) {
		nxgl_vectsubtract(&relpos, &abspos, &fwnd->tbrect.pt1);
		fwnd->tbcb->mousein((NXTKWINDOW)fwnd, &relpos, buttons, fwnd->tbarg);
	}
}
