...
typedef CODE int (*nxf_renderer_t)(FAR nxgl_mxpixel_t *dest, uint16_t height, uint16_t width,
								   uint16_t stride, FAR const struct nx_fontbitmap_s *bm,
...
static FAR struct nxfonts_glyph_s *nxf_findglyph(FAR struct nxfonts_fcache_s *priv, uint8_t ch)
{
	FAR struct nxfonts_glyph_s *glyph;
	FAR struct nxfonts_glyph_s *prev;
	...
	for (prev = NULL, glyph = priv->head; glyph != NULL; prev = glyph, glyph = glyph->flink) {
														  ...
														  uint8_t ch)
{
	...
	if (glyph != NULL) {
		...
		ret = priv->renderer((FAR nxgl_mxpixel_t *)glyph->bitmap, glyph->height, glyph->width,
							 glyph->stride, fbm, priv->fgcolor);
		if (ret < 0) {
...
static FAR struct nxfonts_fcache_s *nxf_findcache(enum nx_fontid_e fontid, nxgl_mxpixel_t fgcolor,
												  nxgl_mxpixel_t bgcolor, int bpp)
{
	FAR struct nxfonts_fcache_s *fcache;

	ginfo("fontid=%d fgcolor=%ju bgcolor=%ju bpp=%d\n", fontid, (uintmax_t)fgcolor,
		  (uintmax_t)bgcolor, bpp);
	...
	for (fcache = g_fcaches; fcache != NULL; fcache = fcache->flink) {
		...
		if (fcache->fontid == fontid && fcache->fgcolor == fgcolor && fcache->bgcolor == bgcolor &&
			fcache->bpp == bpp) {
			/* Yes... return it */
						 ...
						 int bpp, int maxglyphs)
{
	...
	ginfo("fontid=%d fgcolor=%ju bgcolor=%ju bpp=%d maxglyphs=%d\n", fontid, (uintmax_t)fgcolor,
		  (uintmax_t)bgcolor, bpp, maxglyphs);
	...
	if (priv == NULL) {
		...
		priv = (FAR struct nxfonts_fcache_s *)lib_zalloc(sizeof(struct nxfonts_fcache_s));
...
void nxf_cache_disconnect(FCACHE fhandle)
{
	...
	if (priv->fclients <= 1) {
		...
		for (prev = NULL, fcache = g_fcaches; fcache != priv && fcache != NULL;
			 fcache = fcache->flink) {
		}
