...
int pmsm_model_elec(FAR struct pmsm_model_f32_s *model, FAR ab_frame_f32_t *vab)
{
	...
	inv_clarke_transform(&model->state.v_ab, &model->state.v_abc);

	/* Park transform - get DQ voltage */

	park_transform(&model->state.angle.angle_el, &model->state.v_ab, &model->state.v_dq);
	...
	inv_park_transform(&model->state.angle.angle_el, &model->state.i_dq, &model->state.i_ab);

	/* Inverse Clarke transform - get abc current */

	inv_clarke_transform(&model->state.i_ab, &model->state.i_abc);
