							 ...
							 b16_t dir, b16_t speed)
{
	...
	FAR struct motor_aobserver_smo_b16_s *smo = (FAR struct motor_aobserver_smo_b16_s *)o->ao;
	...
	smo->F = (b16ONE - b16mulb16(b16mulb16(o->per, phy->res), phy->one_by_ind));
	...
	i_est->a = (b16mulb16(smo->F, i_est->a) + b16mulb16(smo->G, (v_err->a - z->a)));
	i_est->b = (b16mulb16(smo->F, i_est->b) + b16mulb16(smo->G, (v_err->b - z->b)));
	...
	if (i_err_a_abs < smo->err_max) {
		...
		z->a = b16mulb16(b16mulb16(i_err->a, smo->k_slide), smo->one_by_err_max);
	} else {
		/* Non-linear region */
	...
	if (i_err_b_abs < smo->err_max) {
		...
		z->b = b16mulb16(b16mulb16(i_err->b, smo->k_slide), smo->one_by_err_max);
	} else {
		/* Non-linear region */
...
void motor_sobserver_div_b16(FAR struct motor_sobserver_b16_s *o, b16_t angle)
{
	...
	FAR struct motor_sobserver_div_b16_s *so = (FAR struct motor_sobserver_div_b16_s *)o->so;
...
void motor_aobserver_nfo_b16(FAR struct motor_aobserver_b16_s *o, FAR ab_frame_b16_t *i_ab,
							 ...
							 b16_t gain)
{
	FAR struct motor_aobserver_nfo_b16_s *nfo = (FAR struct motor_aobserver_nfo_b16_s *)o->ao;
	b16_t angle;
	b16_t err;
	b16_t x1_dot;
	b16_t x2_dot;
	...
	err = SQ_B16(phy->flux_link) - (SQ_B16(nfo->x1 - l_ia) + SQ_B16(nfo->x2 - l_ib));
	...
	x1_dot = -r_ia + v_ab->a + b16mulb16(b16mulb16(gain, (nfo->x1 - l_ia)), err);
	x2_dot = -r_ib + v_ab->b + b16mulb16(b16mulb16(gain, (nfo->x2 - l_ib)), err);
	...
	if (vector2d_mag_b16(nfo->x1, nfo->x2) < (b16mulb16(phy->flux_link, b16HALF))) {
...
{
	FAR struct motor_sobserver_pll_b16_s *so = (FAR struct motor_sobserver_pll_b16_s *)o->so;
	b16_t delta_theta = 0;
	...
	so->pll_phase += b16mulb16((o->speed + b16mulb16(so->pll_kp, delta_theta)), o->per);
