...
int pmsm_model_elec_b16(FAR struct pmsm_model_b16_s *model, FAR ab_frame_b16_t *vab)
{
	...
	inv_clarke_transform_b16(&model->state.v_ab, &model->state.v_abc);

	/* Park transform - get DQ voltage */

	park_transform_b16(&model->state.angle.angle_el, &model->state.v_ab, &model->state.v_dq);
	...
	inv_park_transform_b16(&model->state.angle.angle_el, &model->state.i_dq, &model->state.i_ab);

	/* Inverse Clarke transform - get abc current */

	inv_clarke_transform_b16(&model->state.i_ab, &model->state.i_abc);
...
int pmsm_model_mech_b16(FAR struct pmsm_model_b16_s *model, b16_t load)
{
	...
	model->state.omega_e = b16mulb16(model->state.omega_m, itob16(model->phy.motor.p));
