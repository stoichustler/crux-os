...
#define INSTR_PROF_RAW_MAGIC_64                                                                    \
	(uint64_t)255 << 56 | (uint64_t)'l' << 48 | (uint64_t)'p' << 40 | (uint64_t)'r' << 32 |        \
...
{
	return (((intptr_t)end + __llvm_profile_counter_entry_size() - 1) - (intptr_t)begin) /
...
{
	return (((intptr_t)end + sizeof(__llvm_profile_data) - 1) - (intptr_t)begin) /
...
void __llvm_profile_register_names_function(void *names_start, uint64_t names_size)
{
	(void)names_start;
	(void)names_size;
}
...
size_t __llvm_profile_dump(FAR struct lib_outstream_s *stream)
{
	...
	const uint64_t num_data = __llvm_profile_get_num_data(data_begin, data_end);

	const char *counters_begin = __llvm_profile_begin_counters();
	const char *counters_end = __llvm_profile_end_counters();
	const uint64_t num_counters = __llvm_profile_get_num_counters(counters_begin, counters_end);
	...
	uint64_t padding_bytes_after_names = __llvm_profile_get_num_padding_bytes(names_size);
	...
	if (sizeof(__llvm_profile_data) * num_data !=
		stream->puts(stream, data_begin, sizeof(__llvm_profile_data) * num_data)) {
		goto exit;
	}

	size += sizeof(uint64_t) * num_counters;
	if (sizeof(uint64_t) * num_counters !=
		stream->puts(stream, counters_begin, sizeof(uint64_t) * num_counters)) {
		goto exit;
	}
