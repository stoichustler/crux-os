...
typedef CODE int (*dump_fn_t)(FAR struct dump_args *, FAR char *, FAR uint8_t *, size_t);
...
static int get_path_tokens(FAR char *path, FAR char **tokens, size_t max_tokens)
{
...
static size_t gcov_convert(FAR uint8_t *buffer, FAR const struct gcov_info *info)
{
...
static int gcov_reboot_notify(FAR struct notifier_block *nb, unsigned long action, FAR void *data)
{
	__gcov_dump();
	return OK;
}
...
static int gcov_standard_dump(FAR struct dump_args *args, FAR char *path, FAR uint8_t *data,
							  size_t size)
{
	int written;
	int fd;
...
static int gcov_onefile_dump(FAR struct dump_args *args, FAR char *path, FAR uint8_t *data,
							 size_t size)
{
	...
	lib_sprintf(stream, "gcov start filename:%s size: %zuByte\n", path, size);

	lib_stream_puts(&hexstream, data, size);
	lib_stream_flush(&hexstream);

	lib_sprintf(stream, "gcov end filename:%s checksum: %#0x\n", path, checksum);
...
static int gcov_dump_foreach(dump_fn_t handler, FAR struct dump_args *args)
{
	...
	for (info = __gcov_info_start; info; info = info->next) {
		...
		token_count = args->token_cnt + get_path_tokens(filename, &args->tokens[args->token_cnt],
														GCOV_PATH_MAX_TOKENS);
		if (token_count < args->token_cnt) {
			syslog(LOG_ERR, "gcov get path tokens failed! skip %s", info->filename);
			goto exit;
		}
...
void __gcov_init(FAR struct gcov_info *info)
{
	...
	if (!inited) {
		...
		strftime(path + strlen(path), PATH_MAX, "/gcov_%Y%m%d_%H%M%S", tm_info);
...
void __gcov_dump(void)
{
	...
	if (stat(prefix, &state) || S_ISREG(state.st_mode) || S_ISCHR(state.st_mode)) {
		...
		args.token_cnt = get_path_tokens(prefix, args.tokens, GCOV_PATH_MAX_TOKENS);
