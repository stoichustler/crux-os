...
noinstrument_function static int write_gmon(FAR struct gmonparam *p, FAR const char *output)
{
...
noinstrument_function void moncontrol(int mode)
{
	...
	if (mode) {
		uintptr_t lowpc = ALIGN_DOWN((uintptr_t)&_stext, HISTFRACTION * sizeof(HISTCOUNTER));
		uintptr_t highpc = ALIGN_UP((uintptr_t)&_etext, HISTFRACTION * sizeof(HISTCOUNTER));
		size_t textsize = highpc - lowpc;
		size_t kcountsize = ALIGN_UP(textsize / HISTFRACTION, sizeof(*p->kcount));
		int scale =
			kcountsize >= textsize ? SCALE_1_TO_1 : (float)kcountsize / textsize * SCALE_1_TO_1;
...
noinstrument_function void monstartup(unsigned long lowpc, unsigned long highpc)
{
	FAR struct gmonparam *p = &g_monparam;
	irqstate_t flags;
	FAR char *buffer;
...
noinstrument_function void _mcleanup(void)
{
	FAR struct gmonparam *p = &g_monparam;
...
noinstrument_function void mcount_internal(uintptr_t frompc, uintptr_t selfpc)
{
	FAR struct gmonparam *p = &g_monparam;
