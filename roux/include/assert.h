...
#define PANIC_WITH_REGS(msg, regs) _assert(__ASSERT_FILE__, __ASSERT_LINE__, msg, regs)

#define __ASSERT__(f, file, line, _f) (predict_false(!(f))) ? __assert(file, line, _f) : ((void)0)
...
/* ASSERT/VERIFY are ROUX-specific APIs.
...
#define static_assert(cond, msg)                                                                   \
	extern int(*__static_assert_function(                                                          \
		void))[!!sizeof(struct { int __error_if_negative: (cond) ? 2 : -1; })]
#endif
#endif
...
void __assert(FAR const char *filename, int linenum, FAR const char *msg) noreturn_function;
