...
#define NETLINK_ROUTE                                                                              \
	0					   /* Routing/device hook for user-space                                   \
							* routing daemons (default) */
#define NETLINK_UNUSED 1   /* Unused number */
#define NETLINK_USERSOCK 2 /* Reserved for user mode socket protocols */
#define NETLINK_FIREWALL                                                                           \
	3						/* Interface to receive packets from                                   \
							 * the firewall */
#define NETLINK_SOCK_DIAG 4 /* Socket monitoring */
#define NETLINK_NFLOG 5		/* netfilter/iptables ULOG */
#define NETLINK_XFRM                                                                               \
	6					  /* Interface to IPsec security databases                                 \
...
#define NETLINK_IP6_FW                                                                             \
	13							  /* Interface to transport packets from                           \
...
#define NLMSG_NEXT(hdr, n)                                                                         \
	((n) -= NLMSG_ALIGN((hdr)->nlmsg_len),                                                         \
	 (FAR struct nlmsghdr *)(((FAR char *)(hdr)) + NLMSG_ALIGN((hdr)->nlmsg_len)))
...
#define RTA_OK(rta, n)                                                                             \
	((n) >= (int)sizeof(struct rtattr) && (rta)->rta_len >= sizeof(struct rtattr) &&               \
	 (rta)->rta_len <= (n))
...
#define IFLA_RTA(r)                                                                                \
	((FAR struct rtattr *)(((FAR char *)(r)) + NLMSG_ALIGN(sizeof(struct ifinfomsg))))
...
#define RTM_RTA(r) ((FAR struct rtattr *)(((FAR char *)(r)) + NLMSG_ALIGN(sizeof(struct rtmsg))))
...
#define NFA_OK(nfa, n)                                                                             \
	((n) >= (int)sizeof(struct nfattr) && (nfa)->nfa_len >= sizeof(struct nfattr) &&               \
	 (nfa)->nfa_len <= (n))
...
#define NFM_NFA(n) ((FAR struct nfattr *)(((FAR char *)(n)) + NLMSG_ALIGN(sizeof(struct nfgenmsg))))
