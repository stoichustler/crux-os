...
#define GLOB_APPEND                                                                                \
	0x01 /* Append generated pathnames to                                                          \
		  * those previously obtained. */
#define GLOB_DOOFFS                                                                                \
	0x02			  /* Specify how many null pointers                                            \
					   * to add to the beginning of gl_pathv. */
#define GLOB_ERR 0x04 /* Cause glob() to return on error. */
#define GLOB_MARK                                                                                  \
	0x08 /* Each pathname that is a directory that                                                 \
		  * matches pattern has a slash appended. */
#define GLOB_NOCHECK                                                                               \
	0x10				   /* If pattern does not match any pathname, then                         \
...
#define GLOB_ABORTED                                                                               \
	1 /* The scan was stopped because GLOB_ERR                                                     \
	   * was set or (*errfunc)() returned non-zero. */
#define GLOB_NOMATCH                                                                               \
	2				   /* The pattern does not match any existing pathname,                        \
