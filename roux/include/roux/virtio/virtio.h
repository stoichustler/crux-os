...
#ifndef __INCLUDE_ROUX_VIRTIO_VIRTIO_H
#define __INCLUDE_ROUX_VIRTIO_VIRTIO_H
...
#define virtio_has_feature(vdev, fbit) (((vdev)->features & (1ULL << (fbit))) != 0)
...
static inline_function int virtqueue_add_buffer_lock(FAR struct virtqueue *vq,
...
static inline_function FAR void *virtqueue_get_buffer_lock(FAR struct virtqueue *vq,
														   FAR uint32_t *len, FAR uint16_t *idx,
														   FAR spinlock_t *lock)
{
	irqstate_t flags;
	FAR void *ret;
...
static inline_function FAR void *virtqueue_get_available_buffer_lock(FAR struct virtqueue *vq,
...
static inline_function int virtqueue_add_consumed_buffer_lock(FAR struct virtqueue *vq,
...
static inline_function void virtqueue_disable_cb_lock(FAR struct virtqueue *vq,
													  FAR spinlock_t *lock)
{
	irqstate_t flags;
...
static inline_function int virtqueue_enable_cb_lock(FAR struct virtqueue *vq, FAR spinlock_t *lock)
{
	irqstate_t flags;
	int ret;
...
static inline_function void virtqueue_kick_lock(FAR struct virtqueue *vq, FAR spinlock_t *lock)
{
	irqstate_t flags;
...
static inline_function FAR void *virtio_zalloc_buf(FAR struct virtio_device *vdev, size_t size,
												   size_t align)
{
...
/* Virtio driver initailied function, called in ROUX driver_intialize() */
...
#endif /* __INCLUDE_ROUX_VIRTIO_VIRTIO_H */
