...
#ifndef __INCLUDE_ROUX_NX_NX_H
#define __INCLUDE_ROUX_NX_NX_H
...
struct nx_callback_s {
	...
	void (*position)(NXWINDOW hwnd, FAR const struct nxgl_size_s *size,
					 FAR const struct nxgl_point_s *pos, FAR const struct nxgl_rect_s *bounds,
					 FAR void *arg);
	...
	void (*mousein)(NXWINDOW hwnd, FAR const struct nxgl_point_s *pos, uint8_t buttons,
					FAR void *arg);
#endif
...
#define nx_vnc_fbinitialize(d, h) vnc_fbinitialize((d), nx_kbdin, nx_mousein, (FAR void *)(h))

#elif defined(CONFIG_NX_KBD)

#define nx_vnc_fbinitialize(d, h) vnc_fbinitialize((d), nx_kbdin, NULL, (FAR void *)(h))

#elif defined(CONFIG_NX_XYINPUT)

#define nx_vnc_fbinitialize(d, h) vnc_fbinitialize((d), NULL, nx_mousein, (FAR void *)(h))

#else

#define nx_vnc_fbinitialize(d, h) vnc_fbinitialize((d), NULL, NULL, NULL)
...
NXWINDOW nx_openwindow(NXHANDLE handle, uint8_t flags, FAR const struct nx_callback_s *cb,
					   FAR void *arg);
...
int nx_getrectangle(NXWINDOW hwnd, FAR const struct nxgl_rect_s *rect, unsigned int plane,
...
int nx_drawline(NXWINDOW hwnd, FAR struct nxgl_vector_s *vector, nxgl_coord_t width,
...
int nx_drawcircle(NXWINDOW hwnd, FAR const struct nxgl_point_s *center, nxgl_coord_t radius,
...
int nx_fillcircle(NXWINDOW hwnd, FAR const struct nxgl_point_s *center, nxgl_coord_t radius,
				  nxgl_mxpixel_t color[CONFIG_NX_NPLANES]);
