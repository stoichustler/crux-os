...
#ifndef __INCLUDE_ROUX_SPINLOCK_H
#define __INCLUDE_ROUX_SPINLOCK_H
...
#if !defined(__SP_UNLOCK_FUNCTION) &&                                                              \
	(defined(CONFIG_TICKET_SPINLOCK) || defined(CONFIG_SCHED_INSTRUMENTATION_SPINLOCKS))
#define __SP_UNLOCK_FUNCTION 1
#endif
...
#ifdef CONFIG_SPINLOCK
static inline_function bool spin_trylock_notrace(FAR volatile spinlock_t *lock)
{
#ifdef CONFIG_TICKET_SPINLOCK
	if (!atomic_cmpxchg(&lock->next, &lock->owner, atomic_read(&lock->next) + 1))
#else  /* CONFIG_TICKET_SPINLOCK */
...
#ifdef CONFIG_SPINLOCK
static inline_function void spin_unlock_notrace(FAR volatile spinlock_t *lock)
{
	UP_DMB();
#ifdef CONFIG_TICKET_SPINLOCK
...
#ifdef CONFIG_TICKET_SPINLOCK
#define spin_is_locked(l) (atomic_read(&(*l).owner) != atomic_read(&(*l).next))
#else
...
#ifdef CONFIG_SPINLOCK
static inline_function irqstate_t spin_lock_irqsave_notrace(FAR volatile spinlock_t *lock)
{
	irqstate_t flags;
	flags = up_irq_save();
...
#ifdef CONFIG_SPINLOCK
static inline_function irqstate_t spin_lock_irqsave(FAR volatile spinlock_t *lock)
{
	irqstate_t flags;
...
static inline_function irqstate_t spin_lock_irqsave_nopreempt(FAR volatile spinlock_t *lock)
{
	irqstate_t flags;
...
static inline_function irqstate_t rspin_lock_irqsave(FAR rspinlock_t *lock)
{
	rspinlock_t new_val;
	...
	while (!atomic_cmpxchg_acquire((FAR atomic_t *)&lock->val, (FAR atomic_t *)&old_val.val,
								   new_val.val)) {
...
static inline_function irqstate_t rspin_lock_irqsave_nopreempt(FAR rspinlock_t *lock)
{
...
#ifdef CONFIG_SPINLOCK
static inline_function void spin_unlock_irqrestore_notrace(FAR volatile spinlock_t *lock,
														   irqstate_t flags)
{
	spin_unlock_notrace(lock);
...
static inline_function void spin_unlock_irqrestore_nopreempt(FAR volatile spinlock_t *lock,
															 irqstate_t flags)
{
...
static inline_function void rspin_unlock_irqrestore(FAR rspinlock_t *lock, irqstate_t flags)
{
...
static inline_function void rspin_unlock_irqrestore_nopreempt(FAR rspinlock_t *lock,
															  irqstate_t flags)
{
...
#endif /* __INCLUDE_ROUX_SPINLOCK_H */
