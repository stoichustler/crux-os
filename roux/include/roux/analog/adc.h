...
#ifndef __INCLUDE_ROUX_ANALOG_ADC_H
#define __INCLUDE_ROUX_ANALOG_ADC_H
...
/* Default configuration settings that may be overridden in the ROUX
...
struct adc_callback_s {
	...
	CODE int (*au_receive_batch)(FAR struct adc_dev_s *dev, FAR const uint8_t *channel,
								 FAR const uint32_t *data, size_t count);
...
struct adc_ops_s {
	...
	CODE int (*ao_bind)(FAR struct adc_dev_s *dev, FAR const struct adc_callback_s *callback);
...
FAR struct adc_dev_s *ads1255_initialize(FAR struct spi_dev_s *spi, unsigned int devno);
...
FAR struct adc_dev_s *ads7828_initialize(FAR struct i2c_master_s *i2c, uint8_t addr);
...
#endif /* __INCLUDE_ROUX_ANALOG_ADC_H */
