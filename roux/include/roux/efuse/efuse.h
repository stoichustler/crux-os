...
#ifndef __INCLUDE_ROUX_EFUSE_EFUSE_H
#define __INCLUDE_ROUX_EFUSE_EFUSE_H
 ...
 *           in a header file inside include/roux/efuse/ directory.
 ...
 *              supported by ROUX. So, the first efuse bit starting at
...
struct efuse_ops_s {
	...
	CODE int (*read_field)(FAR struct efuse_lowerhalf_s *lower, FAR const efuse_desc_t *field[],
						   FAR uint8_t *data, size_t bit_size);

	/* Write an EFUSE bit */

	CODE int (*write_field)(FAR struct efuse_lowerhalf_s *lower, FAR const efuse_desc_t *field[],
	...
	CODE int (*ioctl)(FAR struct efuse_lowerhalf_s *lower, int cmd, unsigned long arg);
};
...
#endif /* CONFIG_EFUSE */
#endif /* __INCLUDE_ROUX_EFUSE_EFUSE_H */
