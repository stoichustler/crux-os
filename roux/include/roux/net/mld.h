...
#ifndef __INCLUDE_ROUX_NET_MLD_H
#define __INCLUDE_ROUX_NET_MLD_H
...
#define MLD_MRD_VALUE(mrc) ((MLD_MRC_GETMANT(mrc) | 0x1000) << (MLD_MRC_GETEXP(mrc) + 3))
...
#define MLD_QQI_VALUE(qqic) ((MLD_QQIC_GETMANT(qqic) | 0x1000) << (MLD_QQIC_GETEXP(qqic) + 3))
...
#define MODE_IS_INCLUDE                                                                            \
	(1) /* Indicates that the interface has a                                                      \
		 * filter mode of INCLUDE . */
#define MODE_IS_EXCLUDE                                                                            \
	(2) /* Indicates that the interface has a                                                      \
...
#define CHANGE_TO_INCLUDE_MODE                                                                     \
	(3) /* Iindicates that the interface has                                                       \
		 * changed to INCLUDE filter mode. */
#define CHANGE_TO_EXCLUDE_MODE                                                                     \
	(4) /* Indicates that the interface has                                                        \
...
#define ALLOW_NEW_SOURCES                                                                          \
	(5) /* Indicates that the Source Addresses                                                     \
		 * contain a list of additional sources                                                    \
		 * that the node wishes to listen to. */
#define BLOCK_OLD_SOURCES                                                                          \
	(6) /* Indicates that the Source Addresses                                                     \
...
#define SIZEOF_MLD_MCAST_LISTEN_QUERY_S(nsources)                                                  \
	(sizeof(struct mld_mcast_listen_query_s) + sizeof(net_ipv6addr_t) * ((nsources)-1))
...
#define SIZEOF_MLD_MCAST_ADDREC_V2_S(nsources, auxdatlen)                                          \
	(sizeof(struct mld_mcast_addrec_v2_s) + sizeof(net_ipv6addr_t) * ((nsources)-1) + (auxdatlen))
...
#define SIZEOF_MLD_MCAST_LISTEN_REPORT_V2_S(addreclen)                                             \
	(sizeof(struct mld_mcast_listen_report_v2_s) - sizeof(struct mld_mcast_addrec_v2_s) +          \
	 (addreclen))
...
#endif /* __INCLUDE_ROUX_NET_MLD_H */
