...
#ifndef __INCLUDE_ROUX_RPTUN_RPTUN_H
#define __INCLUDE_ROUX_RPTUN_RPTUN_H
...
#define RPTUN_GET_LOCAL_CPUNAME(d)                                                                 \
...
#define RPTUN_GET_CPUNAME(d) ((d)->ops->get_cpuname ? (d)->ops->get_cpuname(d) : "")
...
#define RPTUN_GET_FIRMWARE(d) ((d)->ops->get_firmware ? (d)->ops->get_firmware(d) : NULL)
...
#define RPTUN_GET_ADDRENV(d) ((d)->ops->get_addrenv ? (d)->ops->get_addrenv(d) : NULL)
...
#define RPTUN_GET_RESOURCE(d) ((d)->ops->get_resource ? (d)->ops->get_resource(d) : NULL)
...
#define RPTUN_IS_AUTOSTART(d) ((d)->ops->is_autostart ? (d)->ops->is_autostart(d) : false)
...
#define RPTUN_IS_MASTER(d) ((d)->ops->is_master ? (d)->ops->is_master(d) : false)
 ...
 *
 ****************************************************************************/
#define RPTUN_CONFIG(d, p) ((d)->ops->config ? (d)->ops->config(d, p) : 0)
...
#define RPTUN_START(d) ((d)->ops->start ? (d)->ops->start(d) : -ENOSYS)
...
#define RPTUN_STOP(d) ((d)->ops->stop ? (d)->ops->stop(d) : -ENOSYS)
...
#define RPTUN_NOTIFY(d, v) ((d)->ops->notify ? (d)->ops->notify(d, v) : -ENOSYS)
...
#define RPTUN_REGISTER_CALLBACK(d, c, a)                                                           \
...
#define RPTUN_UNREGISTER_CALLBACK(d)                                                               \
	((d)->ops->register_callback ? (d)->ops->register_callback(d, NULL, NULL) : -ENOSYS)
...
#define RPTUN_RESET(d, v) ((d)->ops->reset ? (d)->ops->reset(d, v) : UNUSED(d))
...
#define RPTUN_PANIC(d) ((d)->ops->panic ? (d)->ops->panic(d) : UNUSED(d))
...
struct rptun_ops_s {
	...
	CODE int (*register_callback)(FAR struct rptun_dev_s *dev, rptun_callback_t callback,
								  FAR void *arg);
...
#endif /* CONFIG_RPTUN */
#endif /* __INCLUDE_ROUX_RPTUN_RPTUN_H */
