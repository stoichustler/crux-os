...
#ifndef __INCLUDE_ROUX_AIE_AI_ENGINE_H
#define __INCLUDE_ROUX_AIE_AI_ENGINE_H
...
struct aie_ops_s {
	...
	CODE int (*feed_input)(FAR struct aie_lowerhalf_s *lower, int id, uintptr_t input);

	CODE int (*get_output)(FAR struct aie_lowerhalf_s *lower, int id, uintptr_t output);

	CODE int (*control)(FAR struct aie_lowerhalf_s *lower, int id, int cmd, unsigned long arg);
};
...
int aie_register(FAR const char *path, FAR struct aie_lowerhalf_s *lower);

#endif /* __INCLUDE_ROUX_AIE_AI_ENGINE_H */
