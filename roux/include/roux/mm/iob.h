...
#ifndef __INCLUDE_ROUX_MM_IOB_H
#define __INCLUDE_ROUX_MM_IOB_H
...
FAR struct iob_s *iob_alloc_with_data(FAR void *data, uint16_t size, iob_free_cb_t free_cb);
#endif
...
#if CONFIG_IOB_NCHAINS > 0
void iob_free_queue_qentry(FAR struct iob_s *iob, FAR struct iob_queue_s *iobq);
#endif /* CONFIG_IOB_NCHAINS > 0 */
...
int iob_copyin(FAR struct iob_s *iob, FAR const uint8_t *src, unsigned int len, int offset,
			   bool throttled);
...
int iob_trycopyin(FAR struct iob_s *iob, FAR const uint8_t *src, unsigned int len, int offset,
				  bool throttled);
...
int iob_copyout(FAR uint8_t *dest, FAR const struct iob_s *iob, unsigned int len, int offset);
...
int iob_clone(FAR struct iob_s *iob1, FAR struct iob_s *iob2, bool throttled, bool block);
...
int iob_clone_partial(FAR struct iob_s *iob1, unsigned int len, int offset1, FAR struct iob_s *iob2,
...
int iob_update_pktlen(FAR struct iob_s *iob, unsigned int pktlen, bool throttled);
