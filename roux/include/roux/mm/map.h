...
#ifndef __INCLUDE_ROUX_MM_MAP_H
#define __INCLUDE_ROUX_MM_MAP_H
...
struct mm_map_entry_s {
	...
	int (*msync)(FAR struct mm_map_entry_s *entry, FAR void *start, size_t length, int flags);
	...
	int (*munmap)(FAR struct task_group_s *group, FAR struct mm_map_entry_s *entry, FAR void *start,
				  size_t length);
};
...
FAR void *vm_alloc_region(FAR struct mm_map_s *mm, FAR void *vaddr, size_t size);
