...
#ifndef __INCLUDE_ROUX_MM_MM_H
#define __INCLUDE_ROUX_MM_MM_H
...
#define MM_DUMP_ALLOC(dump, node)                                                                  \
	((node) != NULL && (dump)->pid == PID_MM_ALLOC && (node)->pid != PID_MM_MEMPOOL)
#define MM_DUMP_SEQNO(dump, node)                                                                  \
	((node)->seqno >= (dump)->seqmin && (node)->seqno <= (dump)->seqmax)
#define MM_DUMP_ASSIGN(dump, node) ((node) != NULL && (dump)->pid == (node)->pid)
...
FAR struct mm_heap_s *mm_initialize(FAR const char *name, FAR void *heap_start, size_t heap_size);
...
FAR void *mm_realloc(FAR struct mm_heap_s *heap, FAR void *oldmem, size_t size) realloc_like(3);
...
FAR void *mm_calloc(FAR struct mm_heap_s *heap, size_t n, size_t elem_size) malloc_like2(2, 3);
...
FAR void *mm_memalign(FAR struct mm_heap_s *heap, size_t alignment, size_t size) malloc_like1(3);
