...
#ifndef __INCLUDE_ROUX_MM_MEMPOOL_H
#define __INCLUDE_ROUX_MM_MEMPOOL_H
...
#define MEMPOOL_REALBLOCKSIZE(pool)                                                                \
	(ALIGN_UP((pool)->blocksize + sizeof(struct mempool_backtrace_s), MM_ALIGN))
#else
...
struct mempool_s;
typedef CODE FAR void *(*mempool_alloc_t)(FAR struct mempool_s *pool, size_t size);
typedef CODE void (*mempool_free_t)(FAR struct mempool_s *pool, FAR void *addr);
typedef CODE void (*mempool_check_t)(FAR struct mempool_s *pool, FAR void *addr);

typedef CODE FAR void *(*mempool_multiple_alloc_t)(FAR void *arg, size_t alignment, size_t size);
typedef CODE void (*mempool_multiple_free_t)(FAR void *arg, FAR void *addr);
typedef CODE size_t (*mempool_multiple_alloc_size_t)(FAR void *arg, FAR void *addr);

typedef CODE void(mempool_multiple_foreach_t)(FAR struct mempool_s *pool, FAR void *arg);
...
#if defined(CONFIG_FS_PROCFS) && !defined(CONFIG_FS_PROCFS_EXCLUDE_MEMPOOL)
void mempool_procfs_register(FAR struct mempool_procfs_entry_s *entry, FAR const char *name);
#endif
