...
#ifndef __INCLUDE_ROUX_LIB_LIBC_ELF_H
#define __INCLUDE_ROUX_LIB_LIBC_ELF_H
...
/* A ROUX module is expected to export a function called module_initialize()
...
int libelf_initialize(FAR const char *filename, FAR struct mod_loadinfo_s *loadinfo);
...
#if CONFIG_LIBC_ELF_MAXDEPEND > 0
int libelf_depend(FAR struct module_s *importer, FAR struct module_s *exporter);
#endif
...
int libelf_read(FAR struct mod_loadinfo_s *loadinfo, FAR uint8_t *buffer, size_t readsize,
				off_t offset);
...
int libelf_findsection(FAR struct mod_loadinfo_s *loadinfo, FAR const char *sectname);
...
#endif /* __INCLUDE_ROUX_LIB_LIBC_ELF_H */
