...
#ifndef __INCLUDE_ROUX_SCHED_NOTE_H
#define __INCLUDE_ROUX_SCHED_NOTE_H
...
#define NOTE_ALIGN(a) (((a) + sizeof(uintptr_t) - 1) & ~(sizeof(uintptr_t) - 1))
...
#ifdef CONFIG_SCHED_INSTRUMENTATION_SYSCALL
#define NOTE_FILTER_SYSCALLMASK_SET(nr, s) ((s)->syscall_mask[(nr) / 8] |= (1 << ((nr) % 8)))
#define NOTE_FILTER_SYSCALLMASK_CLR(nr, s) ((s)->syscall_mask[(nr) / 8] &= ~(1 << ((nr) % 8)))
#define NOTE_FILTER_SYSCALLMASK_ISSET(nr, s) ((s)->syscall_mask[(nr) / 8] & (1 << ((nr) % 8)))
#define NOTE_FILTER_SYSCALLMASK_ZERO(s) memset((s), 0, sizeof(struct note_filter_syscall_s))
#else
...
#define NOTE_FILTER_IRQMASK_SET(nr, s) ((s)->irq_mask[(nr) / 8] |= (1 << ((nr) % 8)))
#define NOTE_FILTER_IRQMASK_CLR(nr, s) ((s)->irq_mask[(nr) / 8] &= ~(1 << ((nr) % 8)))
#define NOTE_FILTER_IRQMASK_ISSET(nr, s) ((s)->irq_mask[(nr) / 8] & (1 << ((nr) % 8)))
#define NOTE_FILTER_IRQMASK_ZERO(s) memset((s), 0, sizeof(struct note_filter_irq_s))
#else
...
#define NOTE_FILTER_TAGMASK_SET(tag, s) ((s)->tag_mask[(tag) / 8] |= (1 << ((tag) % 8)))
#define NOTE_FILTER_TAGMASK_CLR(tag, s) ((s)->tag_mask[(tag) / 8] &= ~(1 << ((tag) % 8)))
#define NOTE_FILTER_TAGMASK_ISSET(tag, s) ((s)->tag_mask[(tag) / 8] & (1 << ((tag) % 8)))
#define NOTE_FILTER_TAGMASK_ZERO(s) memset((s), 0, sizeof(struct note_filter_tag_s));
#else
...
#define NOTE_PRINTF_INT_TYPE(arg)                                                                  \
	(sizeof((arg) + 0) <= sizeof(uint32_t) ? NOTE_PRINTF_UINT32 : NOTE_PRINTF_UINT64)
...
#define NOTE_PRINTF_TAG(...) ((GET_ARG_COUNT(__VA_ARGS__) << 28) + NOTE_PRINTF_TYPES(__VA_ARGS__))
...
#define sched_note_vprintf(tag, fmt, va) sched_note_vprintf_ip(tag, SCHED_NOTE_IP, fmt, 0, va)

#ifdef CONFIG_DRIVERS_NOTE_STRIP_FORMAT
#define sched_note_printf(tag, fmt, ...)                                                           \
	do {                                                                                           \
		static const locate_data(".printf_format") char __fmt__[] = fmt;                           \
		uint32_t __type__ = NOTE_PRINTF_TAG(__VA_ARGS__);                                          \
		static_assert(GET_ARG_COUNT(__VA_ARGS__) <= 14, "The number of sched_note_nprintf "        \
														"parameters needs to be less than 14");    \
		sched_note_printf_ip(tag, SCHED_NOTE_IP, __fmt__, __type__, ##__VA_ARGS__);                \
	} while (0)
#else
#define sched_note_printf(tag, fmt, ...)                                                           \
...
#define sched_note_begin(tag) sched_note_event(tag, NOTE_DUMP_BEGIN, NULL, 0)
#define sched_note_end(tag) sched_note_event(tag, NOTE_DUMP_END, NULL, 0)
#define sched_note_beginex(tag, str) sched_note_event(tag, NOTE_DUMP_BEGIN, str, strlen(str))
#define sched_note_endex(tag, str) sched_note_event(tag, NOTE_DUMP_END, str, strlen(str))
#define sched_note_mark(tag, str) sched_note_event(tag, NOTE_DUMP_MARK, str, strlen(str))
...
#define SIZEOF_NOTE_SYSCALL_ENTER(n)                                                               \
	(sizeof(struct note_common_s) + sizeof(uint8_t) + sizeof(uint8_t) + (sizeof(uintptr_t) * (n)))
...
#define SIZEOF_NOTE_PRINTF(n) (sizeof(struct note_printf_s) + ((n)-1) * sizeof(uint8_t))
...
#define SIZEOF_NOTE_EVENT(n) (sizeof(struct note_event_s) + ((n)) * sizeof(uint8_t))
...
void sched_note_heap(uint8_t event, FAR void *heap, FAR void *mem, size_t size, size_t used);
#else
#define sched_note_heap(e, h, m, s, c)
#endif

#ifdef CONFIG_SCHED_INSTRUMENTATION_DUMP
void sched_note_event_ip(uint32_t tag, uintptr_t ip, uint8_t event, FAR const void *buf,
						 size_t len);
void sched_note_vprintf_ip(uint32_t tag, uintptr_t ip, FAR const char *fmt, uint32_t type,
						   va_list va) printf_like(3, 0);
void sched_note_printf_ip(uint32_t tag, uintptr_t ip, FAR const char *fmt, uint32_t type, ...)
	printf_like(3, 5);
#else
...
#if defined(CONFIG_SCHED_INSTRUMENTATION_FILTER) && defined(CONFIG_SCHED_INSTRUMENTATION_DUMP)
...
#endif
#endif /* __INCLUDE_ROUX_SCHED_NOTE_H */
