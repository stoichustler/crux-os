/****************************************************************************
 * include/roux/sensors/lsm6dsl.h
 *
...
#ifndef __INCLUDE_ROUX_SENSORS_LSM6DSL_H
#define __INCLUDE_ROUX_SENSORS_LSM6DSL_H
...
#define LSM6DSL_SENSOR_SYNC_TIME_FRAME                                                             \
	0x04								   /* Sensor synchronization time frame register (r/w).    \
											*/
...
#define LSM6DSL_STATUS_REG                                                                         \
	0x1E						/* The STATUS_REG register is read by the SPI/I2C interface (r).   \
								 */
...
#define LSM6DSL_OUTX_L_G                                                                           \
	0x22 /* Angular rate sensor pitch axis (X) angular rate output register (r). */
#define LSM6DSL_OUTX_H_G                                                                           \
	0x23 /* Angular rate sensor pitch axis (X) angular rate output register (r). */
#define LSM6DSL_OUTY_L_G                                                                           \
	0x24 /* Angular rate sensor roll axis (Y) angular rate output register (r). */
#define LSM6DSL_OUTY_H_G                                                                           \
	0x25 /* Angular rate sensor roll axis (Y) angular rate output register (r). */
#define LSM6DSL_OUTZ_L_G                                                                           \
	0x26 /* Angular rate sensor roll axis (Z) angular rate output register (r). */
#define LSM6DSL_OUTZ_H_G                                                                           \
	0x27 /* Angular rate sensor roll axis (Z) angular rate output register (r). */
...
#define LSM6DSL_TIMESTAMP0_REG                                                                     \
	0x40 /* Timestamp first (least significant) byte data output register (r). */
#define LSM6DSL_TIMESTAMP1_REG 0x41 /* Timestamp second byte data output register (r). */
#define LSM6DSL_TIMESTAMP2_REG                                                                     \
	0x42 /* Timestamp third (most significant) byte data output register (r). */
...
#define LSM6DSL_FUNC_SRC1                                                                          \
	0x53 /* Significant motion, tilt, step detector, hard/soft-iron and sensor hub interrupt       \
		source register (r). */
...
#define LSM6DSL_TAP_CFG                                                                            \
	0x58 /* Enables interrupt and inactivity functions, configuration of filtering and tap         \
		recognition functions (r/w). */
#define LSM6DSL_TAP_THS_6D                                                                         \
	0x59 /* Portrait/landscape position and tap function threshold register (r/w). */
...
#define LSM6DSL_WAKE_UP_DUR                                                                        \
	0x5C /* Free-fall, wakeup, timestamp and sleep mode functions duration setting register (r/w). \
		  */
...
#define LSM6DSL_SLV0_ADD                                                                           \
	0x02 /* I2C slave address of the first external sensor (Sensor1) register (r/w). */
#define LSM6DSL_SLV0_SUBADD                                                                        \
	0x03 /* Address of register on the first external sensor (Sensor1) register (r/w). */
#define LSM6DSL_SLAVE0_CONFIG                                                                      \
	0x04 /* First external sensor (Sensor1) configuration and sensor hub settings register (r/w).  \
		  */
#define LSM6DSL_SLV1_ADD                                                                           \
	0x05 /* I2C slave address of the second external sensor (Sensor2) register (r/w). */
#define LSM6DSL_SLV1_SUBADD                                                                        \
	0x06 /* Address of register on the second external sensor (Sensor2) register (r/w). */
#define LSM6DSL_SLAVE1_CONFIG                                                                      \
	0x07 /* Second external sensor (Sensor2) configuration register (r/w). */
#define LSM6DSL_SLV2_ADD                                                                           \
	0x08 /* I2C slave address of the third external sensor (Sensor3) register (r/w). */
#define LSM6DSL_SLV2_SUBADD                                                                        \
	0x09 /* Address of register on the third external sensor (Sensor3) register (r/w). */
#define LSM6DSL_SLAVE2_CONFIG                                                                      \
	0x0A /* Third external sensor (Sensor3) configuration register (r/w). */
#define LSM6DSL_SLV3_ADD                                                                           \
	0x0B /* I2C slave address of the fourth external sensor (Sensor4) register (r/w). */
#define LSM6DSL_SLV3_SUBADD                                                                        \
	0x0C /* Address of register on the fourth external sensor (Sensor4) register (r/w). */
#define LSM6DSL_SLAVE3_CONFIG                                                                      \
	0x0D /* Fourth external sensor (Sensor4) configuration register (r/w). */
#define LSM6DSL_DATAWRITE_SRC_MODE_SUB_SLV0                                                        \
	0x0E /* Data to be written into the slave device register (r/w). */
...
#define LSM6DSL_STEP_COUNT_DELTA                                                                   \
	0x15						/* Time period register for step detection on delta time (r/w). */
...
struct lsm6dsl_ops_s {
	...
	CODE int (*selftest)(FAR struct lsm6dsl_dev_s *priv, uint32_t mode);
};
...
int lsm6dsl_sensor_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr);
...
#endif /* __INCLUDE_ROUX_SENSORS_LSM6DSL_H */
