...
#ifndef __INCLUDE_ROUX_SENSORS_BMG160_H
#define __INCLUDE_ROUX_SENSORS_BMG160_H
...
#define BMG160_INT_MAP_0_REG                                                                       \
	(0x17) /* Controls which interrupt signals are mapped to the INT1 pin */
#define BMG160_INT_MAP_1_REG                                                                       \
	(0x18) /* Controls which interrupt signals are mapped to the INT1 pin and INT2 pin */
#define BMG160_INT_MAP_2_REG                                                                       \
	(0x19) /* Controls which interrupt signals are mapped to the INT2 pin */
#define BMG160_INT_ZERO_REG                                                                        \
	(0x1A) /* Contains the data source definition of those interrupts with selectable data source  \
			*/
#define BMG160_INT_ONE_REG                                                                         \
	(0x1B) /* Contains the data source definition of fast offset compensation and the any motion   \
		  threshold */
#define BMG160_INT_TWO_REG                                                                         \
	(0x1C) /* Contains the any motion configuration for x-, y- and z-axis                          \
			*/
#define BMG160_INT_FOUR_REG (0x1E)
#define BMG160_INT_RST_LATCH_REG                                                                   \
	(0x21) /* Contains the interrupt reset bit and the interrupt mode selection */

/* Interrupt High Rate Configuration Register */

#define BMG160_HIGH_TH_X_REG                                                                       \
	(0x22) /* Contains the high rate threshold and high rate hysteresis setting for the x-axis */
#define BMG160_HIGH_DUR_X_REG (0x23) /* Contains high rate duration setting for the x-axis */
#define BMG160_HIGH_TH_Y_REG                                                                       \
	(0x24) /* Contains the high rate threshold and high rate hysteresis setting for the y-axis */
#define BMG160_HIGH_DUR_Y_REG (0x25) /* Contains high rate duration setting for the y-axis */
#define BMG160_HIGH_TH_Z_REG                                                                       \
	(0x26) /* Contains the high rate threshold and high rate hysteresis setting for the z-axis */
...
#define BMG160_TRIM_NVM_CTRL_REG                                                                   \
	(0x33) /* Contains the control settings for the few-time programmable non-volatile memory      \
		  (NVM) */
...
#define BMG160_FIFO_CONFIG_1_REG                                                                   \
	(0x3E) /* Contains FIFO configuration settings. The FIFO buffer memory is cleared and          \
...
#define BMG160_HBW_REG_SHW_DIS_BM                                                                  \
	(1 << 6) /* Disable shadow mechanism for the rate data output register */
...
#define BMG160_INT_MAP_1_REG_INT2_AUTO_OFF_BM                                                      \
	(1 << 4) /* Map Auto Offset tap interrupt to INT2 pin */
#define BMG160_INT_MAP_1_REG_INT1_AUTO_OFF_BM                                                      \
	(1 << 3)									   /* Map Auto Offset tap interrupt to INT1 pin */
...
#define BMG160_INT_ZERO_REG_SLOW_OFF_UN_BM                                                         \
	(1 << 5) /* Selects unfiltered data for slow offset compensation */
#define BMG160_INT_ZERO_REG_HIGH_UN_D_BM                                                           \
	(1 << 3) /* Selects unfiltered data for high rate interrupt */
#define BMG160_INT_ZERO_REG_ANY_UN_D_BM                                                            \
	(1 << 1) /* Selects unfiltered data for any motion interrupt */

/* BMG160 INT_ONE_REG Definitions */

#define BMG160_INT_ONE_REG_FAST_OFF_UN_BM                                                          \
	(1 << 7) /* Selects unfiltered data for fast offset compensation */
...
#define BMG160_INT_RST_LATCH_REG_OFF_RST_BM                                                        \
...
#define BMG160_FOC_REG_FAST_OFF_EN_BM                                                              \
	(1 << 2) /* Triggers the fast offset compensation for the enabled axes */
...
#define BMG160_TRIM_NVM_CTRL_REG_NVM_LOAD_BM                                                       \
	(1 << 3) /* Triggers an update of all config registers form NVM,                               \
			  * the NVM_RDY flag must be '1' prior to triggering the update */
#define BMG160_TRIM_NVM_CTRL_REG_NVM_PROG_TRIG_BM                                                  \
	(1 << 1) /* Triggers an NVM write operation; (see page 59, data sheet)                         \
			  * the NVM_RDY flag must be '1' prior to triggering the update */
...
#define BMG160_BGW_SPI3_WDT_REG_I2C_WDT_EN_BM                                                      \
	(1 << 2) /* Enables watchdog at the SDA pin if I2C mode is selected */
#define BMG160_BGW_SPI3_WDT_REG_I2C_WDT_SEL_BM                                                     \
	(1 << 1)									 /* Select an I2C watchdog timer period of 50ms */
...
struct bmg160_config_s {
	 ...
	 * to use multiple spi device ids which are employed by ROUX to select/
...
#endif /* __INCLUDE_ROUX_SENSORS_BMG160_H */
