/****************************************************************************
 * include/roux/sensors/adxl345.h
 *
...
#ifndef __INCLUDE_ROUX_SENSORS_ADXL345_H
#define __INCLUDE_ROUX_SENSORS_ADXL345_H
...
#define POWER_CTL_MEASURE                                                                          \
	(1 << 3) /* Bit 3: Writing 0 put part in standby mode, 1 measurement mode */
#define POWER_CTL_AUTO_SLEEP                                                                       \
	(1 << 4) /* Bit 4: If set and link bit is set then device sleep if no activity */
...
#define INT_SRC_DATA_READY                                                                         \
	(1 << 7) /* Bit 7: Data Ready happened, even if INT_ENABLE is disabled                         \
			  */
...
#define DATA_FMT_JUSTIFY                                                                           \
	(1 << 2) /* Bit 2: Value 1 selects left justified mode, 0 selects right just. */
#define DATA_FMT_FULL_RES (1 << 3) /* Bit 3: Value 0 sets 10-bit mode, value 1 sets 13-bit mode */
#define DATA_FMT_INT_INVERT                                                                        \
	(1 << 5) /* Bit 5: Value 0 interrupt active high, 1 sets interrupt active low */
...
#define FIFO_CTL_SAMPLES_SHIFT                                                                     \
	0 /* Bit 0-4: Numbers of samples needed to trigger a watermark event */
#define FIFO_CTL_SAMPLES_MASK (31 << FIFO_CTL_SAMPLES_SHIFT)
#define FIFO_CTL_TRIGGER                                                                           \
	(1 << 5)				  /* Bit 5: Value 0 links trigger event to INT1, value 1 to INT2       \
							   */
...
#define FIFO_STATUS_TRIG                                                                           \
	(1 << 7) /* Bit 7: A 1 reports a trigger event occurred, 0 means no event */
...
struct adxl345_config_s {
	...
	int (*attach)(FAR struct adxl345_config_s *state, adxl345_handler_t handler, FAR void *arg);
...
ADXL345_HANDLE adxl345_instantiate(FAR struct spi_dev_s *dev, FAR struct adxl345_config_s *config);
#else
...
#endif /* __INCLUDE_ROUX_SENSORS_ADXL345_H */
