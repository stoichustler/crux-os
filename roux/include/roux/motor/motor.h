...
#ifndef __INCLUDE_ROUX_MOTOR_MOTOR_H
#define __INCLUDE_ROUX_MOTOR_MOTOR_H
...
#include <roux/motor/motor_ioctl.h>
...
struct motor_ops_s {
	...
	CODE int (*params_set)(FAR struct motor_lowerhalf_s *dev, FAR struct motor_params_s *param);
	...
	CODE int (*limits_set)(FAR struct motor_lowerhalf_s *dev, FAR struct motor_limits_s *limits);
	...
	CODE int (*state_get)(FAR struct motor_lowerhalf_s *dev, FAR struct motor_state_s *state);

	/* Get current fault state */

	CODE int (*fault_get)(FAR struct motor_lowerhalf_s *dev, FAR uint8_t *fault);
	...
	CODE int (*ioctl)(FAR struct motor_lowerhalf_s *dev, int cmd, unsigned long arg);
};
...
int motor_register(FAR const char *path, FAR struct motor_lowerhalf_s *lower);
...
#endif /* __INCLUDE_ROUX_DRIVERS_MOTOR_MOTOR_H */
