...
#ifndef __INCLUDE_ROUX_MOTOR_STEPPER_H
#define __INCLUDE_ROUX_MOTOR_STEPPER_H
...
#include <roux/motor/stepper_ioctl.h>
...
struct stepper_ops_s {
	...
	CODE int (*work)(FAR struct stepper_lowerhalf_s *dev, FAR struct stepper_job_s const *param);
	...
	CODE int (*microstepping)(FAR struct stepper_lowerhalf_s *dev, uint16_t resolution);

	/* Lower-half logic may support platform-specific ioctl commands */

	CODE int (*ioctl)(FAR struct stepper_lowerhalf_s *dev, int cmd, unsigned long arg);
};
...
int stepper_register(FAR const char *path, FAR struct stepper_lowerhalf_s *lower);
...
#endif /* __INCLUDE_ROUX_DRIVERS_MOTOR_STEPPER_H */
