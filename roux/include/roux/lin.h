...
#ifndef __INCLUDE_ROUX_LIN_H
#define __INCLUDE_ROUX_LIN_H
...
#define LIN_ERR_BUS                                                                                \
	(1 << 2) /* Bit 2: Bus reasons make frame error (see LIN_ERR_BUS_* definitions) */
...
#define LIN_ERR_TX_BREAK_TMO                                                                       \
	(1 << 0) /* Bit 0: Master send break field, but detect break event timeout */
...
#define LIN_ERR_TX_DATA_TMO                                                                        \
	(1 << 3) /* Bit 3: Master/slave send data timeout (receive back timeout) */
#define LIN_ERR_TX_CHECKSUM_TMO                                                                    \
	(1 << 4) /* Bit 4: Master/slave send checksum timeout(receive back timeout) */

/* Data[2] rx error */

#define LIN_ERR_RX_UNSPEC 0x00 /* Unspecified error */
#define LIN_ERR_RX_NO_RESPONSE                                                                     \
	(1 << 0) /* Bit 0: Prepared to receive response, but no response received */
#define LIN_ERR_RX_RESPONSE (1 << 1) /* Bit 1: Receive incomplete response */
#define LIN_ERR_RX_CKSUM_TMO                                                                       \
	(1 << 2)						   /* Bit 2: Response data received, receive checksum timeout  \
										*/
...
#define LIN_ERR_BUS_TXCKSUM                                                                        \
	(1 << 1)					  /* Bit 1: Checksum received back is not equal to checksum sent */
...
#define LIN_EVT_WAKEUP_PASSIVE                                                                     \
	(1 << 1) /* Wake up when detected low level signal of bus a for a period of time (such as      \
		250us-5ms) */
...
#define LIN_EVT_SLEEP_IDLE                                                                         \
	(1 << 4) /* Go to sleep when bus keep in the inactive for a period of time(such as 4s) */
