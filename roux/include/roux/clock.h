...
#ifndef __INCLUDE_ROUX_CLOCK_H
#define __INCLUDE_ROUX_CLOCK_H
...
#if defined(CONFIG_DEBUG_SCHED) && defined(CONFIG_SYSTEM_TIME64) && !defined(CONFIG_SCHED_TICKLESS)
...
#define INITIAL_SYSTEM_TIMER_TICKS ((uint64_t)(UINT32_MAX - (TICK_PER_SEC * 5)))
#else
...
#define clock_time2ticks_floor(ts)                                                                 \
	((clock_t)(ts)->tv_sec * TICK_PER_SEC + div_const((uint32_t)(ts)->tv_nsec, NSEC_PER_TICK))
...
#define clock_time2usec(ts)                                                                        \
	((uint64_t)(ts)->tv_sec * USEC_PER_SEC + div_const((uint32_t)(ts)->tv_nsec, NSEC_PER_USEC))
...
#define clock_time2nsec(ts) ((uint64_t)(ts)->tv_sec * NSEC_PER_SEC + (uint64_t)(ts)->tv_nsec)
...
int clock_realtime2absticks(FAR const struct timespec *reltime, FAR clock_t *absticks);
 ...
 *   include/roux/timers/oneshot.h to provide external clocking to assess
 *   the CPU load.
 *
 * Input Parameters:
 ...
 *   include/roux/timers/timer.h to provide external clocking to assess
 *   the CPU load.
 *
 * Input Parameters:
