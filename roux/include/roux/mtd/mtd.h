...
#ifndef __INCLUDE_ROUX_MTD_MTD_H
#define __INCLUDE_ROUX_MTD_MTD_H
...
struct mtd_dev_s {
	...
	CODE int (*erase)(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks);

	/* Read/write from the specified read/write blocks */

	CODE ssize_t (*bread)(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
						  FAR uint8_t *buffer);
	CODE ssize_t (*bwrite)(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
	...
	CODE ssize_t (*read)(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
						 FAR uint8_t *buffer);
#ifdef CONFIG_MTD_BYTE_WRITE
	CODE ssize_t (*write)(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
						  FAR const uint8_t *buffer);
#endif
...
int ftl_initialize_by_path(FAR const char *path, FAR struct mtd_dev_s *mtd, int oflags);
 ...
 *   devtype    - device type, from include/roux/eeprom/spi_xx25xx.h
...
FAR struct mtd_dev_s *at25ee_initialize(FAR struct spi_dev_s *dev, int devtype, int readonly);
...
FAR struct mtd_dev_s *is25xp_initialize(FAR struct spi_dev_s *dev, uint16_t spi_devid);
...
FAR struct mtd_dev_s *ramtron_initialize(FAR struct spi_dev_s *dev, uint16_t spi_devid);
...
FAR struct mtd_dev_s *sst26_initialize_spi(FAR struct spi_dev_s *dev, uint16_t spi_devid);
...
FAR struct mtd_dev_s *gd55_initialize(FAR struct qspi_dev_s *dev, bool unprotect);
...
FAR struct mtd_dev_s *nullmtd_initialize(size_t mtdlen, int16_t sectsize, int32_t erasesize);
...
int register_cfi_driver(volatile uintptr_t addr_base, volatile uintptr_t addr_end,
...
#endif /* __ASSEMBLY__ */
#endif /* __INCLUDE_ROUX_MTD_MTD_H */
