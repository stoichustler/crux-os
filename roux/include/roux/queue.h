...
#ifndef __INCLUDE_ROUX_QUEUE_H
#define __INCLUDE_ROUX_QUEUE_H
...
#define sq_for_every(q, p) for ((p) = (q)->head; (p) != NULL; (p) = (p)->flink)

#define sq_for_every_safe(q, p, tmp)                                                               \
	for ((p) = (q)->head, (tmp) = (p) ? (p)->flink : NULL; (p) != NULL;                            \
		 (p) = (tmp), (tmp) = (p) ? (p)->flink : NULL)
...
#define sq_inqueue(p, q) ((p)->flink || sq_tail(q) == (p))
#define dq_inqueue(p, q) ((p)->flink || dq_tail(q) == (p))

/* sq/dq_is_singular - tests whether a list has just one entry.  */

#define sq_is_singular(q) (!sq_empty(q) && (q)->head->flink == NULL)

#define dq_is_singular(q) (!dq_empty(q) && (q)->head->flink == NULL)
...
#else
STATIC_INLINE void sq_addafter(FAR sq_entry_t *prev, FAR sq_entry_t *node, FAR sq_queue_t *queue)
{
...
#else
STATIC_INLINE void dq_addafter(FAR dq_entry_t *prev, FAR dq_entry_t *node, FAR dq_queue_t *queue)
{
	if (!queue->head || prev == queue->tail) {
...
#else
STATIC_INLINE FAR sq_entry_t *sq_remafter(FAR sq_entry_t *node, FAR sq_queue_t *queue)
{
...
#else
STATIC_INLINE FAR dq_entry_t *dq_remafter(FAR dq_entry_t *node, FAR dq_queue_t *queue)
{
...
STATIC_INLINE size_t sq_count(FAR sq_queue_t *queue)
{
	...
	for (node = queue->head, count = 0; node != NULL; node = node->flink, count++)
		;
