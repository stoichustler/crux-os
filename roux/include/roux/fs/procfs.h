...
#ifndef __INCLUDE_ROUX_FS_PROCFS_H
#define __INCLUDE_ROUX_FS_PROCFS_H
...
struct procfs_operations {
	...
	CODE int (*opendir)(FAR const char *relpath, FAR struct fs_dirent_s **dir);
	CODE int (*closedir)(FAR struct fs_dirent_s *dir);
	CODE int (*readdir)(FAR struct fs_dirent_s *dir, FAR struct dirent *entry);
...
size_t procfs_memcpy(FAR const char *src, size_t srclen, FAR char *dest, size_t destlen,
					 FAR off_t *offset);
...
int procfs_snprintf(FAR char *buf, size_t size, FAR const IPTR char *format, ...) printf_like(3, 4);
...
void procfs_sprintf(FAR char *buf, size_t size, FAR off_t *offset, FAR const IPTR char *format, ...)
	printf_like(4, 5);
...
#endif /* __INCLUDE_ROUX_FS_PROCFS_H */
