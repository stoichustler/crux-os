...
#ifndef __INCLUDE_ROUX_FS_FS_H
#define __INCLUDE_ROUX_FS_FS_H
...
#include <roux/atomic.h>
#include <roux/mutex.h>
#include <roux/semaphore.h>
#include <roux/mm/map.h>
#include <roux/spawn.h>
#include <roux/queue.h>
#include <roux/irq.h>
#include <roux/spinlock_type.h>
...
struct block_operations {
	...
	CODE ssize_t (*read)(FAR struct inode *inode, FAR unsigned char *buffer, blkcnt_t start_sector,
						 unsigned int nsectors);
	CODE ssize_t (*write)(FAR struct inode *inode, FAR const unsigned char *buffer,
						  blkcnt_t start_sector, unsigned int nsectors);
	CODE int (*geometry)(FAR struct inode *inode, FAR struct geometry *geometry);
...
struct mountpt_operations {
	...
	CODE int (*open)(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
	...
	CODE int (*fchstat)(FAR const struct file *filep, FAR const struct stat *buf, int flags);

	/* Directory operations */

	CODE int (*opendir)(FAR struct inode *mountpt, FAR const char *relpath,
						FAR struct fs_dirent_s **dir);
	CODE int (*closedir)(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir);
	CODE int (*readdir)(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir,
						FAR struct dirent *entry);
	CODE int (*rewinddir)(FAR struct inode *mountpt, FAR struct fs_dirent_s *dir);

	/* General volume-related mountpoint operations: */

	CODE int (*bind)(FAR struct inode *blkdriver, FAR const void *data, FAR void **handle);
	CODE int (*unbind)(FAR void *handle, FAR struct inode **blkdriver, unsigned int flags);
	...
	CODE int (*rename)(FAR struct inode *mountpt, FAR const char *oldrelpath,
					   FAR const char *newrelpath);
	CODE int (*stat)(FAR struct inode *mountpt, FAR const char *relpath, FAR struct stat *buf);
...
/* This structure represents one inode in the ROUX pseudo-file system */
...
typedef CODE ssize_t cookie_read_function_t(FAR void *cookie, FAR char *buf, size_t size);
typedef CODE ssize_t cookie_write_function_t(FAR void *cookie, FAR const char *buf, size_t size);
typedef CODE off_t cookie_seek_function_t(FAR void *cookie, FAR off_t *offset, int whence);
 ...
 * Note that ROUX can support the standard C APIs with or without buffering
 *
...
int register_driver(FAR const char *path, FAR const struct file_operations *fops, mode_t mode,
					FAR void *priv);
...
#ifndef CONFIG_DISABLE_MOUNTPOINT
int register_blockdriver(FAR const char *path, FAR const struct block_operations *bops, mode_t mode,
						 FAR void *priv);
#endif
...
#ifndef CONFIG_DISABLE_MOUNTPOINT
int register_blockpartition(FAR const char *partition, mode_t mode, FAR const char *parent,
...
#ifdef CONFIG_MTD
int register_mtddriver(FAR const char *path, FAR struct mtd_dev_s *mtd, mode_t mode,
					   FAR void *priv);
#endif
...
#ifdef CONFIG_MTD
int register_mtdpartition(FAR const char *partition, mode_t mode, FAR const char *parent,
...
int register_pipedriver(FAR const char *path, FAR const struct file_operations *fops, mode_t mode,
						FAR void *priv);
 ...
 *
 *   nx_mount() is an internal ROUX interface and should not be called from
 *   applications.
 *
 * Returned Value:
...
#ifndef CONFIG_DISABLE_MOUNTPOINT
int nx_mount(FAR const char *source, FAR const char *target, FAR const char *filesystemtype,
			 unsigned long mountflags, FAR const void *data);
#endif
 ...
 *
 *   nx_umount2() is an internal ROUX interface and should not be called
 *   from applications.
 *
 * Returned Value:
 *   Zero is returned on success; a negated value is returned on any failure.
...
int fdlist_copy(FAR struct fdlist *plist, FAR struct fdlist *clist,
				FAR const posix_spawn_file_actions_t *actions, bool cloexec);
...
int fdlist_get2(FAR struct fdlist *list, int fd, FAR struct file **filep, FAR struct fd **fdp);
...
int fdlist_dupfile(FAR struct fdlist *list, int oflags, int minfd, FAR struct file *filep);
...
int fdlist_allocate(FAR struct fdlist *list, int oflags, int minfd, FAR struct file **filep);
...
int file_allocate_from_inode(FAR struct inode *inode, int oflags, off_t pos, FAR void *priv,
							 int minfd);
 ...
 *   errno variable.
 *
 *   fdlist_dup3() is an internal ROUX interface and should not be
 ...
 *   errno variable.
 *
 *   fdlist_dup2() is an internal ROUX interface and should not be
 *   called from applications.
 *
 *   Clone a file descriptor to a specific descriptor number.
 *
 ...
 *   not a cancellation point and it does not modify the errno variable.
 *
 *   nx_dup2() is an internal ROUX interface and should not be called from
 *   applications.
 *
 * Returned Value:
 *   fd2 is returned on success; a negated errno value is return on
 ...
 *   variable.
 *
 *   fdlist_open() is an internal ROUX interface and should not be
 *   called from applications.
 *
 * Input Parameters:
 *   list   - Pointer to the file descriptor list structure.
...
int fdlist_open(FAR struct fdlist *list, FAR const char *path, int oflags, ...);
 ...
 *   not a cancellation point and it does not modify the errno variable.
 *
 *   nx_open() is an internal ROUX interface and should not be called
 *   from applications.
 *
 * Returned Value:
 *   The new file descriptor is returned on success; a negated errno value is
 ...
 *   errno variable.
 *
 *   fdlist_close() is an internal ROUX interface and should not
 ...
 *   not a cancellation point and it does not modify the errno variable.
 *
 *   nx_close() is an internal ROUX interface and should not be called from
 *   applications.
 *
 * Returned Value:
 *   The new file descriptor is returned on success; a negated errno value is
...
ssize_t file_readv(FAR struct file *filep, FAR const struct iovec *iov, int iovcnt);
...
ssize_t file_sendfile(FAR struct file *outfile, FAR struct file *infile, FAR off_t *offset,
					  size_t count);
...
int file_mmap(FAR struct file *filep, FAR void *start, size_t length, int prot, int flags,
			  off_t offset, FAR void **mapped);
 ...
 *   not a cancellation point and it does not modify the errno variable.
 *
 *   nx_fstat() is an internal ROUX interface and should not be called from
 *   applications.
 *
 * Returned Value:
 *   Zero is returned on success; a negated value is returned on any failure.
 ...
 *   not a cancellation point and it does not modify the errno variable.
 *
 *   nx_stat() is an internal ROUX interface and should not be called from
 *   applications.
 *
 * Returned Value:
 *   Zero is returned on success; a negated value is returned on any failure.
 ...
 *   is not a cancellation point and it does not modify the errno variable.
 *
 *   nx_unlink() is an internal ROUX interface and should not be called
 *   from applications.
 *
 * Returned Value:
 *   Zero is returned on success; a negated value is returned on any failure.
 ...
 *   Linux, a ROUX FIFO is not a special file type but simply a device
 ...
 *
 *   NOTE: nx_mkfifo is a special, non-standard, ROUX-only interface.  Since
 *   the ROUX FIFOs are based in in-memory, circular buffers, the ability
