...
#ifndef __INCLUDE_ROUX_VIDEO_IMGDATA_H
#define __INCLUDE_ROUX_VIDEO_IMGDATA_H
...
#define IMGDATA_INIT(d) ((d)->ops->init ? (d)->ops->init(d) : -ENOTTY)
#define IMGDATA_UNINIT(d) ((d)->ops->uninit ? (d)->ops->uninit(d) : -ENOTTY)
#define IMGDATA_SET_BUF(d, n, f, a, s) ((d)->ops->set_buf ? (d)->ops->set_buf(d, n, f, a, s) : NULL)
#define IMGDATA_VALIDATE_FRAME_SETTING(d, n, f, i)                                                 \
	((d)->ops->validate_frame_setting ? (d)->ops->validate_frame_setting(d, n, f, i) : -ENOTTY)
#define IMGDATA_START_CAPTURE(d, n, f, i, c, a)                                                    \
	((d)->ops->start_capture ? (d)->ops->start_capture(d, n, f, i, c, a) : -ENOTTY)
#define IMGDATA_STOP_CAPTURE(d) ((d)->ops->stop_capture ? (d)->ops->stop_capture(d) : -ENOTTY)
...
typedef int (*imgdata_capture_t)(uint8_t result, uint32_t size, FAR const struct timeval *ts,
								 FAR void *arg);
...
struct imgdata_ops_s {
	...
	CODE int (*set_buf)(FAR struct imgdata_s *data, uint8_t nr_datafmts,
						FAR imgdata_format_t *datafmts, uint8_t *addr, uint32_t size);

	CODE int (*validate_frame_setting)(FAR struct imgdata_s *data, uint8_t nr_datafmts,
									   FAR imgdata_format_t *datafmts,
									   FAR imgdata_interval_t *interval);
	CODE int (*start_capture)(FAR struct imgdata_s *data, uint8_t nr_datafmts,
...
#endif /* __INCLUDE_ROUX_VIDEO_IMGDATA_H */
