...
#ifndef __INCLUDE_ROUX_VIDEO_FB_H
#define __INCLUDE_ROUX_VIDEO_FB_H
...
#include <roux/video/rgbcolors.h>
#endif
...
#define FB_FMT_CLJR                                                                                \
	25 /* BPP=8   4 pixels packed into a uint32_t.                                                 \
		*         YUV 4:1:1 with l< 8 bits                                                         \
		*         per YUV sample */
#define FB_FMT_CYUV                                                                                \
	26				   /* BPP=16  UYVY except that height is                                       \
						*         reversed */
#define FB_FMT_IRAW 27 /* BPP=?   Intel uncompressed YUV */
#define FB_FMT_IUYV                                                                                \
	28 /* BPP=16  Interlaced UYVY (line order                                                      \
		*         0,2,4,.., 1,3,5...) */
#define FB_FMT_IY41                                                                                \
	29				   /* BPP=12  Interlaced Y41P (line order                                      \
						*         0,2,4,.., 1,3,5...) */
#define FB_FMT_IYU2 30 /* BPP=24 */
#define FB_FMT_HDYC                                                                                \
	31 /* BPP=16  UYVY except uses the BT709                                                       \
		*         color space  */
#define FB_FMT_UYVP                                                                                \
	32							/* BPP=24? YCbCr 4:2:2, 10-bits per                                \
...
#define FB_FMT_YVU9                                                                                \
	46				   /* BPP=9   8-bit Y followed by 8-bit                                        \
						*         4x4 VU */
#define FB_FMT_YUV9 47 /* BPP=9? */
#define FB_FMT_IF09                                                                                \
	48 /* BPP=9.5 YVU9 + 4x4 plane of delta                                                        \
		*         relative to tframe. */
#define FB_FMT_YV16                                                                                \
	49 /* BPP=16  8-bit Y followed by 8-bit                                                        \
		*         2x1 VU */
#define FB_FMT_YV12                                                                                \
	50 /* BPP=12  8-bit Y followed by 8-bit                                                        \
		*         2x2 VU */
#define FB_FMT_I420                                                                                \
	51							/* BPP=12  8-bit Y followed by 8-bit                               \
								 *         2x2 UV */
#define FB_FMT_IYUV FB_FMT_I420 /* BPP=12 */
#define FB_FMT_NV12                                                                                \
	52				   /* BPP=12  8-bit Y followed by an                                           \
						*         interleaved 2x2 UV */
#define FB_FMT_NV21 53 /* BPP=12  NV12 with UV reversed */
#define FB_FMT_IMC1                                                                                \
	54 /* BPP=12  YV12 except UV planes same                                                       \
		*         stride as Y */
#define FB_FMT_IMC2                                                                                \
	55 /* BPP=12  IMC1 except UV lines                                                             \
		*         interleaved at half stride                                                       \
		*         boundaries */
#define FB_FMT_IMC3                                                                                \
	56 /* BPP=12  As IMC1 except that UV                                                           \
		*         swapped */
#define FB_FMT_IMC4                                                                                \
	57 /* BPP=12  As IMC2  except that UV                                                          \
		*         swapped */
#define FB_FMT_CLPL                                                                                \
	58				   /* BPP=12  YV12 but including a level                                       \
						*         of indirection. */
...
#define FBIO_UPDATE                                                                                \
	_FBIOC(0x0007) /* Update a rectangular region in                                               \
					* the framebuffer                                                              \
					* Argument: read-only struct                                                   \
					*           fb_area_s */
#endif
...
#define FBIOSET_TRANSP                                                                             \
	_FBIOC(0x000b) /* Set opacity or transparency                                                  \
					* Argument: read-only struct                                                   \
					*           fb_overlayinfo_s */
#define FBIOSET_CHROMAKEY                                                                          \
	_FBIOC(0x000c) /* Set chroma key                                                               \
					* Argument: read-only struct                                                   \
					*           fb_overlayinfo_s */
#define FBIOSET_COLOR                                                                              \
	_FBIOC(0x000d) /* Set color                                                                    \
					* Aŕgument: read-only struct                                                  \
					*           fb_overlayinfo_s */
#define FBIOSET_BLANK                                                                              \
	_FBIOC(0x000e) /* Blank or unblank                                                             \
					* Argument: read-only struct                                                   \
					*           fb_overlayinfo_s */
#define FBIOSET_AREA                                                                               \
	_FBIOC(0x000f) /* Set active overlay area                                                      \
					* Argument: read-only struct                                                   \
					*           fb_overlayinfo_s */
#define FBIOSET_DESTAREA                                                                           \
	_FBIOC(0x0010) /* Set destination area on                                                      \
					* primary FB.                                                                  \
					* Argument: read-only struct                                                   \
					*           fb_overlayinfo_s */
...
#ifdef CONFIG_FB_OVERLAY_BLIT
#define FBIOSET_BLIT                                                                               \
	_FBIOC(0x0011) /* Blit area between overlays                                                   \
					* Argument: read-only struct                                                   \
					*           fb_overlayblit_s */
#define FBIOSET_BLEND                                                                              \
	_FBIOC(0x0012) /* Blend area between overlays                                                  \
					* Argument: read-only struct                                                   \
					*           fb_overlayblend_s */
#endif

#define FBIOPAN_OVERLAY                                                                            \
	_FBIOC(0x0013) /* Pan display for overlay                                                      \
					* Argument: read-only struct                                                   \
					*           fb_overlayinfo_s */
...
#define FBIOSET_POWER                                                                              \
	_FBIOC(0x0014) /* Set panel power                                                              \
					* Argument:             int */
#define FBIOGET_POWER                                                                              \
	_FBIOC(0x0015) /* Get panel current power                                                      \
					* Argument:            int* */
#define FBIOSET_FRAMERATE                                                                          \
	_FBIOC(0x0016) /* Set frame rate                                                               \
					* Argument:             int */
#define FBIOGET_FRAMERATE                                                                          \
	_FBIOC(0x0017) /* Get frame rate                                                               \
					* Argument:            int* */

#define FBIOPAN_DISPLAY                                                                            \
	_FBIOC(0x0018) /* Pan display                                                                  \
					* Argument: read-only struct                                                   \
					*           fb_planeinfo_s* */
...
#define FBIOSET_VSYNCOFFSET                                                                        \
	_FBIOC(0x001a) /* Set VSync offset in usec                                                     \
					* Argument:             int */
...
struct fb_vtable_s {
	...
	int (*updatearea)(FAR struct fb_vtable_s *vtable, FAR const struct fb_area_s *area);
#endif
	...
	int (*setdestarea)(FAR struct fb_vtable_s *vtable, FAR const struct fb_overlayinfo_s *oinfo);
	...
	int (*panoverlay)(FAR struct fb_vtable_s *vtable, FAR const struct fb_overlayinfo_s *oinfo);
...
#ifdef CONFIG_VIDEO_FB_SPLASHSCREEN
#if defined(CONFIG_VIDEO_FB_SPLASHSCREEN_BPP8) || defined(CONFIG_VIDEO_FB_SPLASHSCREEN_MONO) ||    \
...
struct palette_bitmap_s {
	...
	FAR const struct splscr_bitmap_s *data; /* The RLE data                              */
};
#endif
...
#endif /* __INCLUDE_ROUX_VIDEO_FB_H */
