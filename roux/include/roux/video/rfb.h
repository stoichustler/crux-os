...
#ifndef __INCLUDE_ROUX_VIDEO_RFB_H
#define __INCLUDE_ROUX_VIDEO_RFB_H
...
#define SIZEOF_RFB_SUPPORTED_SECTYPES_S(n) (sizeof(struct rfb_supported_sectypes_s) + (n)-1)
...
#define SIZEOF_RFB_SECTYPE_CONNFAIL_S(n) (sizeof(struct rfb_sectype_connfail_s) + (n)-1)
...
#define SIZEOF_RFB_SECTYPE_FAIL_S(n) (sizeof(struct rfb_sectype_fail_s) + (n)-1)
...
#define SIZEOF_RFB_SERVERINIT_S(n) (sizeof(struct rfb_serverinit_s) + (n)-1)
...
#define SIZEOF_RFB_CLIENTCUTTEXT_S(n) (sizeof(struct rfb_clientcuttext_s) + (n)-1)
...
#define SIZEOF_RFB_RECTANGE_S(d) (sizeof(struct rfb_rectangle_s) + (d)-1)
...
#define RFB_RGB_SIZE(n) ((n) * sizeof(struct rfb_rgb_s))
...
#define SIZEOF_RFB_SERVERCUTTEXT_S(n) (sizeof(struct rfb_servercuttext_s) + (n)-1)
...
#define SIZEOF_RFB_SRLE_S(n, r) (sizeof(struct rfb_srle_s) + (n)-1)
...
#define SIZEOF_RFB_RAWPIXEL8_S(n, r) (sizeof(struct rfb_rawpixel8_s) + (n)-1)

struct rfb_rawpixel16_s {
	uint8_t pixels[2]; /* Actual size is 2*w*h */
};

#define SIZEOF_RFB_RAWPIXEL16_S(n, r) (sizeof(struct rfb_rawpixel16_s) + (((n)-1) << 1))

struct rfb_rawpixel32_s {
	uint8_t pixels[4]; /* Actual size is 4*w*h */
};

#define SIZEOF_RFB_RAWPIXEL32_S(n, r) (sizeof(struct rfb_rawpixel32_s) + (((n)-1) << 2))
...
#define rfb_getbe16(s) (((uint16_t)((s)[0]) << 8) | (uint16_t)((s)[1]))
...
#define rfb_getle16(s) (((uint16_t)((s)[1]) << 8) | (uint16_t)((s)[0]))
...
#define rfb_getle32(s)                                                                             \
	(((uint32_t)((s)[3]) << 24) | ((uint32_t)((s)[2]) << 16) | ((uint32_t)((s)[1]) << 8) |         \
	 (uint32_t)((s)[0]))
...
#endif /* __INCLUDE_ROUX_VIDEO_RFB_H */
