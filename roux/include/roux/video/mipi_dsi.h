...
#ifndef __INCLUDE_ROUX_VIDEO_MIPI_DSI_H
#define __INCLUDE_ROUX_VIDEO_MIPI_DSI_H
...
#define MIPI_DSI_MSG_USE_LPM                                                                       \
	(1 << 1)							  /* Use Low Power Mode to                                 \
										   * transmit message */
#define MIPI_DSI_MSG_AFTER_FRAME (1 << 2) /* Transmit message after frame */

/* Tearing Effect Output Line mode */

#define MIPI_DSI_DCS_TEAR_MODE_VBLANK                                                              \
	0 /* The TE output line consists of                                                            \
	   * V-Blanking information only */
#define MIPI_DSI_DCS_TEAR_MODE_VHBLANK                                                             \
	1 /* The TE output line consists of                                                            \
...
#define MIPI_DSI_MODE_VIDEO_AUTO_VERT                                                              \
	(1 << 3) /* Enable auto vertical                                                               \
			  * count mode */
#define MIPI_DSI_MODE_VIDEO_HSE                                                                    \
	(1 << 4)								/* Enable hsync-end packets                            \
											 * in vsync-pulse and                                  \
											 * v-porch area */
...
#define MIPI_DSI_MODE_VSYNC_FLUSH                                                                  \
	(1 << 8) /* Flush display FIFO on vsync                                                        \
			  * pulse */
#define MIPI_DSI_MODE_NO_EOT_PACKET                                                                \
	(1 << 9) /* Disable EoT packets in HS                                                          \
			  * mode */
#define MIPI_DSI_CLOCK_NON_CONTINUOUS                                                              \
	(1 << 10) /* Device supports                                                                   \
			   * non-continuous clock                                                              \
			   * behavior (DSI spec 5.6.1) */
#define MIPI_DSI_MODE_LPM                                                                          \
	(1 << 11) /* Transmit data in low                                                              \
			   * power */
#define MIPI_DSI_MODE_AFTER_FRAME                                                                  \
	(1 << 12) /* Transmit data after frame                                                         \
			   * transfer done */
#define MIPI_DSI_HS_PKT_END_ALIGNED                                                                \
	(1 << 13) /* Transmit data ending at                                                           \
...
FAR struct mipi_dsi_device *mipi_dsi_device_register(FAR struct mipi_dsi_host *host,
...
int mipi_dsi_generic_write(FAR struct mipi_dsi_device *device, FAR const void *payload,
						   size_t size);
...
ssize_t mipi_dsi_generic_read(FAR struct mipi_dsi_device *device, FAR const void *params,
							  size_t num_params, FAR void *data, size_t size);
...
ssize_t mipi_dsi_dcs_write_buffer(FAR struct mipi_dsi_device *device, FAR const void *data,
								  size_t len);
...
ssize_t mipi_dsi_dcs_write(FAR struct mipi_dsi_device *device, uint8_t cmd, FAR const void *data,
						   size_t len);
...
ssize_t mipi_dsi_dcs_read(FAR struct mipi_dsi_device *device, uint8_t cmd, FAR void *data,
						  size_t len);
...
int mipi_dsi_dcs_set_column_address(FAR struct mipi_dsi_device *device, uint16_t start,
									uint16_t end);
...
#endif
#endif /* __INCLUDE_ROUX_VIDEO_MIPI_DSI_H */
