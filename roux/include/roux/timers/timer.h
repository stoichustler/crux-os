...
#ifndef __INCLUDE_ROUX_TIMERS_TIMER_H
#define __INCLUDE_ROUX_TIMERS_TIMER_H
...
#define TCFLAGS_HANDLER                                                                            \
	(1 << 1) /* 1=Call the user function when the                                                  \
			  *   timer expires */

/* Method access helper macros **********************************************/

#define TIMER_START(l) ((l)->ops->start ? (l)->ops->start(l) : -ENOTSUP)

#define TIMER_STOP(l) ((l)->ops->stop ? (l)->ops->stop(l) : -ENOTSUP)
...
#define TIMER_IOCTL(l, c, a) ((l)->ops->ioctl ? (l)->ops->ioctl(l, c, a) : -ENOTTY)
...
struct timer_ops_s {
	...
	/* Call the ROUX INTERNAL timeout callback on timeout.
	 * NOTE:  Providing callback==NULL disable.
	 * NOT to call back into applications.
	 */

	CODE void (*setcallback)(FAR struct timer_lowerhalf_s *lower, CODE tccb_t callback,
							 FAR void *arg);
	...
	CODE int (*maxtimeout)(FAR struct timer_lowerhalf_s *lower, FAR uint32_t *maxtimeout);
	...
	CODE int (*tick_setttimeout)(FAR struct timer_lowerhalf_s *lower, uint32_t timeout);

	/* Get the maximum supported tick timeout value */

	CODE int (*tick_maxtimeout)(FAR struct timer_lowerhalf_s *lower, FAR uint32_t *maxtimeout);
};
...
static inline int timer_getstatus(FAR struct timer_lowerhalf_s *lower,
								  FAR struct timer_status_s *status)
{
	int ret;
...
static inline int timer_settimeout(FAR struct timer_lowerhalf_s *lower, uint32_t timeout)
{
...
static inline int timer_maxtimeout(FAR struct timer_lowerhalf_s *lower, FAR uint32_t *maxtimeout)
{
	int ret;
...
static inline int timer_tick_getstatus(FAR struct timer_lowerhalf_s *lower,
									   FAR struct timer_status_s *status)
{
	int ret;
...
static inline int timer_tick_settimeout(FAR struct timer_lowerhalf_s *lower, uint32_t timeout)
{
...
static inline int timer_tick_maxtimeout(FAR struct timer_lowerhalf_s *lower,
 ...
 *
 * Input Parameters:
 *   dev path - The full path to the driver to be registered in the ROUX
...
#endif /* CONFIG_TIMER */
#endif /* __INCLUDE_ROUX_TIMERS_TIMER_H */
