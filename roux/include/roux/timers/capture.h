...
#ifndef __INCLUDE_ROUX_TIMERS_CAPTURE_H
#define __INCLUDE_ROUX_TIMERS_CAPTURE_H
...
struct cap_ops_s {
	...
	CODE int (*getduty)(FAR struct cap_lowerhalf_s *lower, FAR uint8_t *duty);

	/* Get the result pwm capture frequency value */

	CODE int (*getfreq)(FAR struct cap_lowerhalf_s *lower, FAR uint32_t *freq);

	/* Get the result pwm capture edges value */

	CODE int (*getedges)(FAR struct cap_lowerhalf_s *lower, FAR uint32_t *edges);

	/* Lower-half logic may support platform-specific ioctl commands */

	CODE int (*ioctl)(FAR struct cap_lowerhalf_s *dev, int cmd, unsigned long arg);
};
...
int cap_register_multiple(FAR const char *devpath, FAR struct cap_lowerhalf_s **lower, int n);
...
#endif /* __INCLUDE_ROUX_TIMERS_CAPTURE_H */
