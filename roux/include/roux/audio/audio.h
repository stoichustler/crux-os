...
#ifndef __INCLUDE_ROUX_AUDIO_AUDIO_H
#define __INCLUDE_ROUX_AUDIO_AUDIO_H
...
/* The ROUX audio interface support different types of audio devices for
...
/* The following defines the audio data format types in ROUX.  During a
...
#define AUDIO_SAMP_RATE_DEF_ALL                                                                    \
	(AUDIO_SAMP_RATE_8K | AUDIO_SAMP_RATE_11K | AUDIO_SAMP_RATE_12K | AUDIO_SAMP_RATE_16K |        \
	 AUDIO_SAMP_RATE_22K | AUDIO_SAMP_RATE_24K | AUDIO_SAMP_RATE_32K | AUDIO_SAMP_RATE_44K |       \
	 AUDIO_SAMP_RATE_48K | AUDIO_SAMP_RATE_88K | AUDIO_SAMP_RATE_96K | AUDIO_SAMP_RATE_128K |      \
	 AUDIO_SAMP_RATE_160K | AUDIO_SAMP_RATE_172K | AUDIO_SAMP_RATE_192K)
...
#define AUDIO_ABP_ACTIVE                                                                           \
	0x0040 /* Set if this buffer is still active.                                                  \
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
typedef CODE void (*audio_callback_t)(FAR void *priv, uint16_t reason, FAR struct ap_buffer_s *apb,
...
struct audio_ops_s {
	...
	CODE int (*getcaps)(FAR struct audio_lowerhalf_s *dev, int type, FAR struct audio_caps_s *caps);
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
	CODE int (*configure)(FAR struct audio_lowerhalf_s *dev, FAR void *session,
						  FAR const struct audio_caps_s *caps);
#else
	CODE int (*configure)(FAR struct audio_lowerhalf_s *dev, FAR const struct audio_caps_s *caps);
#endif
 ...
 *     ROUX filesystem (i.e. the path "/dev/audio" will be prepended to the
...
#endif /* CONFIG_AUDIO */
#endif /* __INCLUDE_ROUX_AUDIO_AUDIO_H */
