...
#ifndef __INCLUDE_ROUX_PCI_PCI_H
#define __INCLUDE_ROUX_PCI_PCI_H
...
#include <roux/list.h>
#include <roux/pci/pci_ids.h>
#include <roux/pci/pci_regs.h>
...
#define PCI_DEFINE_DEVICE_TABLE(table) static const struct pci_device_id_s table[]
...
#define pci_resource_size(res)                                                                     \
	(((res)->start == 0 && (res)->end == 0) ? 0 : ((res)->end - (res)->start + 1))
...
#define pci_read_io_byte(dev, addr, val) pci_bus_read_io_byte((dev)->bus, addr, val)

#define pci_read_io_word(dev, addr, val) pci_bus_read_io_word((dev)->bus, addr, val)

#define pci_read_io_dword(dev, addr, val) pci_bus_read_io_dword((dev)->bus, addr, val)
...
#define pci_write_io_byte(dev, addr, val) pci_bus_write_io_byte((dev)->bus, addr, val)

#define pci_write_io_word(dev, addr, val) pci_bus_write_io_word((dev)->bus, addr, val)

#define pci_write_io_dword(dev, addr, val) pci_bus_write_io_dword((dev)->bus, addr, val)
...
#define pci_write_mmio_byte(dev, addr, val) *((FAR volatile uint8_t *)(addr)) = val

#define pci_write_mmio_word(dev, addr, val) *((FAR volatile uint16_t *)(addr)) = val

#define pci_write_mmio_dword(dev, addr, val) *((FAR volatile uint32_t *)(addr)) = val
...
#define pci_read_mmio_byte(dev, addr, val) *(val) = *((FAR volatile uint8_t *)(addr))

#define pci_read_mmio_word(dev, addr, val) *(val) = *((FAR volatile uint16_t *)(addr))

#define pci_read_mmio_dword(dev, addr, val) *(val) = *((FAR volatile uint32_t *)(addr))
...
#define pci_is_bridge(dev)                                                                         \
	((dev)->hdr_type == PCI_HEADER_TYPE_BRIDGE || (dev)->hdr_type == PCI_HEADER_TYPE_CARDBUS)
...
struct pci_ops_s {
	CODE int (*read)(FAR struct pci_bus_s *bus, uint32_t devfn, int where, int size,
					 FAR uint32_t *val);
	CODE int (*write)(FAR struct pci_bus_s *bus, uint32_t devfn, int where, int size, uint32_t val);

	/* Return memory address for pci resource */

	CODE uintptr_t (*map)(FAR struct pci_bus_s *bus, uintptr_t start, uintptr_t end);
	CODE int (*read_io)(FAR struct pci_bus_s *bus, uintptr_t addr, int size, FAR uint32_t *val);
	CODE int (*write_io)(FAR struct pci_bus_s *bus, uintptr_t addr, int size, uint32_t val);

	/* Get interrupt number associated with a given INTx line */

	CODE int (*get_irq)(FAR struct pci_bus_s *bus, uint32_t devfn, uint8_t line, uint8_t pin);

	/* Allocate interrupt for MSI/MSI-X */

	CODE int (*alloc_irq)(FAR struct pci_bus_s *bus, uint32_t devfn, FAR int *irq, int num);

	CODE void (*release_irq)(FAR struct pci_bus_s *bus, FAR int *irq, int num);

	/* Connect interrupt for MSI/MSI-X */

	CODE int (*connect_irq)(FAR struct pci_bus_s *bus, FAR int *irq, int num, FAR uintptr_t *mar,
							FAR uint32_t *mdr);
};
...
int pci_bus_read_config_byte(FAR struct pci_bus_s *bus, unsigned int devfn, int where,
							 FAR uint8_t *val);
int pci_bus_read_config_word(FAR struct pci_bus_s *bus, unsigned int devfn, int where,
							 FAR uint16_t *val);
int pci_bus_read_config_dword(FAR struct pci_bus_s *bus, unsigned int devfn, int where,
							  FAR uint32_t *val);
...
int pci_bus_write_config_byte(FAR struct pci_bus_s *bus, unsigned int devfn, int where,
							  uint8_t val);
int pci_bus_write_config_word(FAR struct pci_bus_s *bus, unsigned int devfn, int where,
							  uint16_t val);
int pci_bus_write_config_dword(FAR struct pci_bus_s *bus, unsigned int devfn, int where,
							   uint32_t val);
...
int pci_bus_read_io_byte(FAR struct pci_bus_s *bus, uintptr_t where, FAR uint8_t *val);
int pci_bus_read_io_word(FAR struct pci_bus_s *bus, uintptr_t where, FAR uint16_t *val);
int pci_bus_read_io_dword(FAR struct pci_bus_s *bus, uintptr_t where, FAR uint32_t *val);
...
int pci_bus_write_io_byte(FAR struct pci_bus_s *bus, uintptr_t where, uint8_t value);
int pci_bus_write_io_word(FAR struct pci_bus_s *bus, uintptr_t where, uint16_t value);
int pci_bus_write_io_dword(FAR struct pci_bus_s *bus, uintptr_t where, uint32_t value);
...
FAR void *pci_map_bar_region(FAR struct pci_device_s *dev, int bar, uintptr_t offset,
							 size_t length);
...
#endif /* __INCLUDE_ROUX_PCI_PCI_H */
