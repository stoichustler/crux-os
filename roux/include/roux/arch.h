...
#ifndef __INCLUDE_ROUX_ARCH_H
#define __INCLUDE_ROUX_ARCH_H

/* This header file contains function prototypes for the interfaces between
 * (1) the roux core-code, (2) the microprocessor specific logic that
 ...
 *    definitions provide the common interface between ROUX and the
...
typedef CODE void (*debug_callback_t)(int type, FAR void *addr, size_t size, FAR void *arg);
 ...
 * in the configuration.  See declaration in include/roux/irq.h
 */
 ...
 *
 *   This function is called only from the ROUX scheduling
...
void up_pthread_start(pthread_trampoline_t startup, pthread_startroutine_t entrypt,
...
#if !defined(CONFIG_BUILD_FLAT) && defined(__KERNEL__)
void up_signal_dispatch(_sa_sigaction_t sighand, int signo, FAR siginfo_t *info,
						FAR void *ucontext);
#endif
...
void up_signal_handler(_sa_sigaction_t sighand, int signo, FAR siginfo_t *info,
					   FAR void *ucontext) noreturn_function;
#endif
...
FAR void *up_textheap_memalign(FAR const char *sectname, size_t align, size_t size);
#else
...
#if defined(CONFIG_ARCH_USE_SEPARATED_SECTION)
FAR void *up_dataheap_memalign(FAR const char *sectname, size_t align, size_t size);
#else
...
#ifdef CONFIG_ARCH_ADDRENV
int up_addrenv_vdata(FAR arch_addrenv_t *addrenv, uintptr_t textsize, FAR void **vdata);
#endif
...
#ifdef CONFIG_ARCH_ADDRENV
int up_addrenv_clone(FAR const arch_addrenv_t *src, FAR arch_addrenv_t *dest);
#endif
...
#ifdef CONFIG_ARCH_ADDRENV
int up_addrenv_mprot(FAR arch_addrenv_t *addrenv, uintptr_t addr, size_t len, int prot);
#endif
...
#if defined(CONFIG_ARCH_ADDRENV) && defined(CONFIG_MM_KMAP)
int up_addrenv_kmap_pages(FAR void **pages, unsigned int npages, uintptr_t vaddr, int prot);
#endif
...
/* See prototype in include/roux/elf.h */
...
/* See prototype in include/roux/spinlock.h */
...
#if defined(CONFIG_RTC) && defined(CONFIG_RTC_DATETIME) && defined(CONFIG_ARCH_HAVE_RTC_SUBSECONDS)
...
int up_debugpoint_add(int type, FAR void *addr, size_t size, debug_callback_t callback,
					  FAR void *arg);
...
int up_connect_irq(FAR const int *irq, int num, FAR uintptr_t *mar, FAR uint32_t *mdr);
