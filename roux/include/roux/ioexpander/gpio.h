...
#ifndef __INCLUDE_ROUX_IOEXPANDER_GPIO_H
#define __INCLUDE_ROUX_IOEXPANDER_GPIO_H
...
struct gpio_operations_s {
	...
	CODE int (*go_attach)(FAR struct gpio_dev_s *dev, pin_interrupt_t callback);
...
int gpio_pin_unregister_byname(FAR struct gpio_dev_s *dev, FAR const char *pinname);
...
int gpio_lower_half_byname(FAR struct ioexpander_dev_s *ioe, unsigned int pin,
						   enum gpio_pintype_e pintype, FAR char *name);
#endif
...
#ifdef CONFIG_GPIO_LOWER_HALF
struct ioexpander_dev_s;
int gpio_lower_half(FAR struct ioexpander_dev_s *ioe, unsigned int pin, enum gpio_pintype_e pintype,
					int minor);
#endif
...
#endif /* __INCLUDE_ROUX_IOEXPANDER_GPIO_H */
