...
#ifndef __INCLUDE_ROUX_USB_USBDEV_H
#define __INCLUDE_ROUX_USB_USBDEV_H
...
struct composite_devdesc_s {
	...
	CODE int (*classobject)(int minor, FAR struct usbdev_devinfo_s *devinfo,
...
struct usbdev_epops_s {
	...
	CODE int (*configure)(FAR struct usbdev_ep_s *ep, FAR const struct usb_epdesc_s *desc,
						  bool last);
	...
	CODE void (*freereq)(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req);
	...
	CODE int (*submit)(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req);
	CODE int (*cancel)(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req);
...
struct usbdev_ops_s {
	...
	CODE FAR struct usbdev_ep_s *(*allocep)(FAR struct usbdev_s *dev, uint8_t epphy, bool in,
											uint8_t eptype);
	...
	CODE int (*ioctl)(FAR struct usbdev_s *dev, unsigned code, unsigned long param);
};
...
struct usbdevclass_driverops_s {
	CODE int (*bind)(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);
	CODE void (*unbind)(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);
	CODE int (*setup)(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev,
					  FAR const struct usb_ctrlreq_s *ctrl, FAR uint8_t *dataout, size_t outlen);
	CODE void (*disconnect)(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);
	CODE void (*suspend)(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);
	CODE void (*resume)(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);
};
...
FAR struct usbdev_req_s *usbdev_allocreq(FAR struct usbdev_ep_s *ep, size_t len);
...
void usbdev_freereq(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req);
...
int usbdev_copy_devdesc(FAR void *dest, FAR const struct usb_devdesc_s *src, uint8_t speed);
...
int usbdev_copy_epdesc(FAR struct usb_epdesc_s *epdesc, uint8_t epno, uint8_t speed,
 ...
 *
 *   If CONFIG_USBDEV_DMAMEMORY is also defined in the ROUX configuration,
 ...
 *   gran_free() (See roux/mm/gran.h).  Note that the gran_free() function
...
#endif /* __INCLUDE_ROUX_USB_USBDEV_H */
