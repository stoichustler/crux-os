...
#ifndef __INCLUDE_ROUX_USB_HID_PARSER_H
#define __INCLUDE_ROUX_USB_HID_PARSER_H
 ...
 *   CONFIG_HID_STATEDEPTH in the ROUX config file.
 *
 ...
 *   overridden by defining CONFIG_HID_MAXCOLLECTIONS in the ROUX config
 *   file.
 *
 * CONFIG_HID_MAXITEMS
 ...
 *   CONFIG_HID_MAXITEMS in the ROUX config file.
 *
 * CONFIG_HID_MAXIDS
 ...
 *   CONFIG_HID_MAXIDS in the ROUX config file. Note that IN, OUT and
...
struct hid_rptinfo_s {
	...
	struct hid_rptsizeinfo_s
		rptsize[CONFIG_HID_MAXIDS]; /* Report sizes for each report in the interface */
...
int hid_parsereport(FAR const uint8_t *report, int rptlen, hid_rptfilter_t filter,
...
#endif /* __INCLUDE_ROUX_USB_HID_PARSER_H */
