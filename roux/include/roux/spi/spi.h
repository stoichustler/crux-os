...
#ifndef __INCLUDE_ROUX_SPI_SPI_H
#define __INCLUDE_ROUX_SPI_SPI_H
...
#define SPI_STATUS(d, id) ((d)->ops->status ? (d)->ops->status(d, id) : SPI_STATUS_PRESENT)
 ...
 *   method is required if CONFIG_SPI_CMDDATA is selected in the ROUX
 *   configuration
 *
 * Input Parameters:
 *   dev - Device-specific state data
...
#define SPI_TRIGGER(d) (((d)->ops->trigger) ? ((d)->ops->trigger(d)) : -ENOSYS)
...
#ifdef CONFIG_SPI_DELAY_CONTROL
	CODE int (*setdelay)(FAR struct spi_dev_s *dev, uint32_t a, uint32_t b, uint32_t c, uint32_t i);
#endif
...
#ifdef CONFIG_SPI_EXCHANGE
	CODE void (*exchange)(FAR struct spi_dev_s *dev, FAR const void *txbuffer, FAR void *rxbuffer,
						  size_t nwords);
#else
...
#endif
#endif /* __INCLUDE_ROUX_SPI_SPI_H */
