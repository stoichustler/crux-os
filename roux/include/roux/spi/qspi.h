...
#ifndef __INCLUDE_ROUX_SPI_QSPI_H
#define __INCLUDE_ROUX_SPI_QSPI_H
...
#define QSPIMEM_ISRANDOM(f)                                                                        \
	(((f) & (QSPIMEM_SCRAMBLE | QSPIMEM_RANDOM)) == (QSPIMEM_SCRAMBLE | QSPIMEM_RANDOM))
...
#ifdef CONFIG_QSPI_HWFEATURES
	CODE int (*hwfeatures)(FAR struct qspi_dev_s *dev, qspi_hwfeatures_t features);
#endif
	CODE int (*command)(FAR struct qspi_dev_s *dev, FAR struct qspi_cmdinfo_s *cmdinfo);
	CODE int (*memory)(FAR struct qspi_dev_s *dev, FAR struct qspi_meminfo_s *meminfo);
...
#endif
#endif /* __INCLUDE_ROUX_SPI_QSPI_H */
