...
#ifndef __INCLUDE_ROUX_SERIAL_SERIAL_H
#define __INCLUDE_ROUX_SERIAL_SERIAL_H
...
#define uart_release(dev) ((dev)->ops->release ? (dev)->ops->release(dev) : -ENOSYS)

#ifdef CONFIG_SERIAL_TXDMA
#define uart_dmasend(dev) ((dev)->ops->dmasend ? (dev)->ops->dmasend(dev) : -ENOSYS)

#define uart_dmatxavail(dev) ((dev)->ops->dmatxavail ? (dev)->ops->dmatxavail(dev) : -ENOSYS)
#endif

#ifdef CONFIG_SERIAL_RXDMA
#define uart_dmareceive(dev) ((dev)->ops->dmareceive ? (dev)->ops->dmareceive(dev) : -ENOSYS)

#define uart_dmarxfree(dev) ((dev)->ops->dmarxfree ? (dev)->ops->dmarxfree(dev) : -ENOSYS)
#endif
...
struct uart_ops_s {
	...
	CODE ssize_t (*recvbuf)(FAR struct uart_dev_s *dev, FAR void *buf, size_t len);

	/* This method will send multiple bytes.
	 * Returns the actual number of characters sent.
	 */

	CODE ssize_t (*sendbuf)(FAR struct uart_dev_s *dev, FAR const void *buf, size_t len);
};
...
#endif /* __INCLUDE_ROUX_SERIAL_SERIAL_H */
