...
#ifndef __INCLUDE_ROUX_SCHED_H
#define __INCLUDE_ROUX_SCHED_H
...
#define SMP_CALL_INITIALIZER(func, arg)                                                            \
	{                                                                                              \
		(func), (arg)                                                                              \
	}
...
struct binary_s; /* Forward reference                        */
				 /* Defined in include/roux/binfmt/binfmt.h */
#endif
 ...
 * The TCB is the heart of the ROUX task-control logic.
...
begin_packed_struct struct tcbinfo_s {
	 ...
	 *
	 * value UINT16_MAX: This register was not provided by ROUX
	 */
...
int nxtask_init(FAR struct task_tcb_s *tcb, const char *name, int priority, FAR void *stack,
				uint32_t stack_size, main_t entry, FAR char *const argv[], FAR char *const envp[],
...
int nxtask_create(FAR const char *name, int priority, FAR void *stack_addr, int stack_size,
 ...
 *
 * Returned Value:
 *   This is a ROUX internal function so it follows the convention that
...
int nxsched_set_scheduler(pid_t pid, int policy, FAR const struct sched_param *param);
...
#ifdef CONFIG_SMP
int nxsched_set_affinity(pid_t pid, size_t cpusetsize, FAR const cpu_set_t *mask);
#endif
...
void nxsched_get_stateinfo(FAR struct tcb_s *tcb, FAR char *state, size_t length);
...
int nxsched_smp_call_handler(int irq, FAR void *context, FAR void *arg);
...
void nxsched_smp_call_init(FAR struct smp_call_data_s *data, nxsched_smp_call_t func,
						   FAR void *arg);
...
int nxsched_smp_call_single(int cpuid, nxsched_smp_call_t func, FAR void *arg);
...
int nxsched_smp_call(cpu_set_t cpuset, nxsched_smp_call_t func, FAR void *arg);
...
int nxsched_smp_call_single_async(int cpuid, FAR struct smp_call_data_s *data);
...
int nxsched_smp_call_async(cpu_set_t cpuset, FAR struct smp_call_data_s *data);
#endif
