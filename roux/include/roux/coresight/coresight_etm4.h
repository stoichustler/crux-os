/****************************************************************************
 * include/roux/coresight/coresight_etm4.h
 *
...
#ifndef __INCLUDE_ROUX_CORESIGHT_CORESIGHT_ETM4_H
#define __INCLUDE_ROUX_CORESIGHT_CORESIGHT_ETM4_H

/****************************************************************************
 * Included Files
 ****************************************************************************/

#include <roux/coresight/coresight.h>
...
struct etm4_config_s {
	...
	uint32_t
		stall_ctrl; /* Enables trace unit functionality that prevents trace unit buffer overflows */
	uint32_t ts_ctrl;	 /* Controls the insertion of global timestamps in the trace streams */
	uint32_t syncfreq;	 /* Controls how often trace synchronization requests occur. the TRCCCCTLR
				register */
	uint32_t ccctlr;	 /* Sets the threshold value for cycle counting */
	uint32_t bb_ctrl;	 /* Controls which regions in the memory map are enabled to use branch
				broadcasting */
	uint32_t vinst_ctrl; /* Controls instruction trace filtering */
	uint32_t viiectlr;	 /* Set or read, the address range comparators */
	uint32_t vissctlr;	 /* Set, or read, the single address comparators that control the ViewInst
				start-stop logic */
	uint32_t vipcssctlr; /* Set, or read, which PE comparator inputs can control the ViewInst
				start-stop logic */
	uint32_t seq_ctrl[ETM4_MAX_SEQ_STATES]; /* Control for the sequencer state transition control
						   register */
	...
	uint32_t
		res_ctrl[ETM4_MAX_RES_SEL]; /* Controls the selection of the resources in the trace unit */
	uint32_t
		ss_ctrl[ETM4_MAX_SS_CMP]; /* Controls the corresponding single-shot comparator resource */
	uint32_t
		ss_status[ETM4_MAX_SS_CMP]; /* The status of the corresponding single-shot comparator */
	uint32_t
		ss_pe_cmp[ETM4_MAX_SS_CMP]; /* Selects the PE comparator inputs for Single-shot control */
...
struct coresight_etm4_dev_s {
	...
	uint8_t nr_pe_cmp; /* he number of processing entity comparator inputs that are available for
			  tracing */
	uint8_t
		nr_addr_cmp; /* Number of pairs of address comparators available as found in ETMIDR4 0-3 */
	...
	uint8_t s_ex_level;	 /* In secure state, indicates whether instruction tracing is supported for
				the corresponding Exception level */
	uint8_t ns_ex_level; /* In non-secure state, indicates whether instruction tracing is supported
	...
	uint64_t trfcr;	 /* If the CPU supports FEAT_TRF, set TRFCR_ELx to enable tracing at all levels;
...
FAR struct coresight_etm4_dev_s *etm4_register(FAR const struct coresight_desc_s *desc);
...
int etm4_config(FAR struct coresight_etm4_dev_s *etmdev, FAR const struct etm4_config_s *config);

#endif /* __INCLUDE_ROUX_CORESIGHT_CORESIGHT_ETM4_H */
