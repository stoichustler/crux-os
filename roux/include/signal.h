...
/* ROUX does not support all standard signal actions.  ROUX supports what
 * are referred to as "real time" signals.  The default action of all ROUX
...
#define SA_NOCLDSTOP                                                                               \
	(1 << 0) /* Do not generate SIGCHLD when                                                       \
...
#define SA_ONSTACK                                                                                 \
	(1 << 3) /* Indicates that a registered stack_t                                                \
			  * will be used */
#define SA_RESTART                                                                                 \
	(1 << 4) /* Flag to get restarting signals                                                     \
			  * (which were the default long ago) */
#define SA_NODEFER                                                                                 \
	(1 << 5) /* Prevents the current signal from                                                   \
			  * being masked in the handler */
#define SA_RESETHAND                                                                               \
	(1 << 6)				   /* Clears the handler when the signal                               \
								* is delivered */
 ...
 * in ROUX, the default action for all signals is to ignore them.
 */
 ...
 * These should be used only internally within the ROUX signal logic.
 */

typedef CODE void (*_sa_handler_t)(int signo);
typedef CODE void (*_sa_sigaction_t)(int signo, FAR siginfo_t *siginfo, FAR void *context);
...
int sigandset(FAR sigset_t *dest, FAR const sigset_t *left, FAR const sigset_t *right);
...
int sigorset(FAR sigset_t *dest, FAR const sigset_t *left, FAR const sigset_t *right);
