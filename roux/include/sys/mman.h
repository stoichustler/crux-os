...
/* Protections are chosen from these bits, OR'd together.  ROUX does not
 ...
 * specified by OpenGroup.org.  The rest are unique to ROUX or are
...
int posix_mem_offset(FAR const void *addr, size_t len, FAR off_t *off, FAR size_t *contig_len,
					 FAR int *fildes);
