...
#define FD_CLR(fd, set)                                                                            \
	((((fd_set *)(set))->arr)[_FD_NDX(fdcheck_restore(fd))] &=                                     \
	 ~(UINT32_C(1) << _FD_BIT(fdcheck_restore(fd))))
#define FD_SET(fd, set)                                                                            \
	((((fd_set *)(set))->arr)[_FD_NDX(fdcheck_restore(fd))] |=                                     \
	 (UINT32_C(1) << _FD_BIT(fdcheck_restore(fd))))
#define FD_ISSET(fd, set)                                                                          \
	(((((fd_set *)(set))->arr)[_FD_NDX(fdcheck_restore(fd))] &                                     \
	  (UINT32_C(1) << _FD_BIT(fdcheck_restore(fd)))) != 0)
#else
#define FD_CLR(fd, set) ((((fd_set *)(set))->arr)[_FD_NDX(fd)] &= ~(UINT32_C(1) << _FD_BIT(fd)))
#define FD_SET(fd, set) ((((fd_set *)(set))->arr)[_FD_NDX(fd)] |= (UINT32_C(1) << _FD_BIT(fd)))
#define FD_ISSET(fd, set)                                                                          \
...
struct timeval;
int select(int nfds, FAR fd_set *readfds, FAR fd_set *writefds, FAR fd_set *exceptfds,
		   FAR struct timeval *timeout);

int pselect(int nfds, FAR fd_set *readfds, FAR fd_set *writefds, FAR fd_set *exceptfds,
			FAR const struct timespec *timeout, FAR const sigset_t *sigmask);
