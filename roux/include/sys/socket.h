 ...
 * recognized by Linux, not all are supported by ROUX.
 */
...
#define CMSG_DATA(cmsg) ((FAR void *)((FAR char *)(cmsg) + CMSG_ALIGN(sizeof(struct cmsghdr))))
...
static inline FAR struct cmsghdr *__cmsg_nxthdr(FAR void *__ctl, unsigned long __size,
												FAR struct cmsghdr *__cmsg)
{
	size_t len = CMSG_ALIGN(__cmsg->cmsg_len);
	FAR struct cmsghdr *__ptr = (FAR struct cmsghdr *)(((FAR char *)__cmsg) + len);
...
static inline FAR struct cmsghdr *cmsg_nxthdr(FAR struct msghdr *__msg, FAR struct cmsghdr *__cmsg)
{
...
int accept4(int sockfd, FAR struct sockaddr *addr, FAR socklen_t *addrlen, int flags);
...
fortify_function(send) ssize_t send(int sockfd, FAR const void *buf, size_t len, int flags)
{
	fortify_assert(len <= fortify_size(buf, 0));
	return __real_send(sockfd, buf, len, flags);
}

fortify_function(sendto) ssize_t sendto(int sockfd, FAR const void *buf, size_t len, int flags,
										FAR const struct sockaddr *to, socklen_t tolen)
{
	fortify_assert(len <= fortify_size(buf, 0));
...
fortify_function(recv) ssize_t recv(int sockfd, FAR void *buf, size_t len, int flags)
{
	fortify_assert(len <= fortify_size(buf, 0));
	return __real_recv(sockfd, buf, len, flags);
}

fortify_function(recvfrom) ssize_t recvfrom(int sockfd, FAR void *buf, size_t len, int flags,
