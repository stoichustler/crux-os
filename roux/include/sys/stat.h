...
/* mode_t bit settings (most of these do not apply to ROUX). This assumes
...
int fstatat(int dirfd, FAR const char *path, FAR struct stat *buf, int flags);
...
int utimensat(int dirfd, FAR const char *path, const struct timespec times[2], int flags);
