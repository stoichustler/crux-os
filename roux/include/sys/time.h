...
#define ITIMER_PROF                                                                                \
	2 /* Timers run when the process is executing and when                                         \
 ...
 *
 *   ROUX implements gettimeofday() as a thin layer around clock_gettime();
 *
 * Input Parameters:
 ...
 *   not settimeofday().
 *
 *   ROUX implements settimeofday() as a thin layer around clock_settime();
 *
 * Input Parameters:
...
int setitimer(int which, FAR const struct itimerval *value, FAR struct itimerval *ovalue);
...
int futimesat(int dirfd, FAR const char *path, const struct timeval times[2]);
