...
#define _alert(format, ...) __arch_syslog(LOG_ALERT, EXTRA_FMT format EXTRA_ARG, ##__VA_ARGS__)
#endif
 ...
 *
 *   The ROUX implementation does not support any special formatting
...
typedef CODE void (*lib_dump_handler_t)(FAR void *arg, FAR const char *fmt, ...) printf_like(2, 3);

/* Dump a buffer of data with handler */

void lib_dumphandler(FAR const char *msg, FAR const uint8_t *buffer, unsigned int buflen,
					 lib_dump_handler_t handler, FAR void *arg);

/* Do a pretty buffer dump from multiple buffers with handler. */

void lib_dumpvhandler(FAR const char *msg, FAR const struct iovec *iov, int iovcnt,
					  lib_dump_handler_t handler, FAR void *arg);
...
void lib_dumpvbuffer(FAR const char *msg, FAR const struct iovec *iov, int iovcnt);

/* Dump a buffer of data with fd */

void lib_dumpfile(int fd, FAR const char *msg, FAR const uint8_t *buffer, unsigned int buflen);

/* Do a pretty buffer dump from multiple buffers with fd. */

void lib_dumpvfile(int fd, FAR const char *msg, FAR const struct iovec *iov, int iovcnt);
