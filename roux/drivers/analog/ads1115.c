...
#include <roux/analog/ads1115.h>
#include <roux/analog/ioctl.h>
#include <roux/arch.h>
...
#define ADS1115_ONESHOT_MODE(priv) (((priv->cmdbyte & ADS1115_MODE_MASK) == ADS1115_MODE_MASK))
...
static int ads1115_bind(FAR struct adc_dev_s *dev, FAR const struct adc_callback_s *callback);
...
static int ads1115_write_register(FAR struct ads1115_dev_s *priv, enum ads1115_registers_e reg,
								  uint16_t value)
{
	int ret = OK;
...
static int ads1115_read_register(FAR struct ads1115_dev_s *priv, uint8_t reg, uint16_t *buf)
{
	int ret = OK;
	struct i2c_msg_s i2cmsg[2];
...
static int ads1115_read_current_register(FAR struct ads1115_dev_s *priv, uint16_t *buf)
{
	int ret = OK;
...
static int ads1115_trigger_conversion(FAR struct ads1115_dev_s *priv, FAR struct adc_msg_s *msg)
{
	int ret = OK;
	...
	if (new_channel || ADS1115_ONESHOT_MODE(priv)) {
		...
		ret = ads1115_write_register(priv, ADS1115_CONFIG_REGISTER, htobe16(priv->cmdbyte));
	}
...
static int ads1115_read_conversion(FAR struct ads1115_dev_s *priv, FAR struct adc_msg_s *msg)
{
	int ret = OK;
	uint16_t buf;
	int count = 0;
...
static int ads1115_readchannel(FAR struct ads1115_dev_s *priv, FAR struct adc_msg_s *msg)
{
	int ret = OK;
...
{
	FAR struct ads1115_dev_s *priv = (FAR struct ads1115_dev_s *)dev->ad_priv;
	return ads1115_write_register(priv, ADS1115_CONFIG_REGISTER, htobe16(priv->cmdbyte));
}
...
static int ads1115_ioctl(FAR struct adc_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case ANIOC_ADS1115_SET_HI_THRESH: {
		...
		ret = ads1115_write_register(priv, ADS1115_HI_THRESH_REGISTER, htobe16(arg));
	} break;

	case ANIOC_ADS1115_SET_LO_THRESH: {
		DEBUGASSERT(arg >= 0 && arg <= UINT16_MAX);
		ret = ads1115_write_register(priv, ADS1115_LO_THRESH_REGISTER, htobe16(arg));
	} break;
...
FAR struct adc_dev_s *ads1115_initialize(FAR struct i2c_master_s *i2c, uint8_t addr)
{
