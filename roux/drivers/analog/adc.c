...
static ssize_t adc_read(FAR struct file *fielp, FAR char *buffer, size_t buflen);
...
static int adc_receive_batch(FAR struct adc_dev_s *dev, FAR const uint8_t *channel,
...
static ssize_t adc_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (buflen >= msglen) {
		...
		if (msglen == 4) {
			...
			used = (fifo->af_tail - fifo->af_head + fifo->af_fifosize) % fifo->af_fifosize;
							 ...
							 FAR const uint32_t *data, size_t count)
{
	...
	memcpy(&fifo->af_data[fifo->af_tail], data, first * sizeof(uint32_t));
	...
	if (second > 0) {
		memcpy(&fifo->af_data[0], &data[first], second * sizeof(uint32_t));
...
int adc_register(FAR const char *path, FAR struct adc_dev_s *dev)
{
	...
	if (fifo->af_data == NULL) {
		fifo->af_data = kmm_malloc(fifo->af_fifosize * sizeof(*(fifo->af_data)));
		if (fifo->af_data == NULL) {
			if (alloc_channel) {
