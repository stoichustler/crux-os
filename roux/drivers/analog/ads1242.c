...
static void ads1242_perform_systemoffset_calibration(FAR struct ads1242_dev_s *dev);
static void ads1242_read_conversion_result(FAR struct ads1242_dev_s *dev,
										   FAR uint32_t *conversion_result);

static void ads1242_write_reg(FAR struct ads1242_dev_s *dev, uint8_t const reg_addr,
							  uint8_t const reg_value);
static void ads1242_read_reg(FAR struct ads1242_dev_s *dev, uint8_t const reg_addr,
							 FAR uint8_t *reg_value);
...
static ssize_t ads1242_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
							 ...
							 ADS1242_GAIN_SELECTION const gain_selection)
{
	...
	setup_reg_value &=
		~(ADS1242_REG_SETUP_BIT_PGA2 | ADS1242_REG_SETUP_BIT_PGA1 | ADS1242_REG_SETUP_BIT_PGA0);
...
static int ads1242_open(FAR struct file *filep)
{
	...
	ads1242_write_reg(priv, ADS1242_REG_ACR, ADS1242_REG_ACR_BIT_SPEED | ADS1242_REG_ACR_BIT_BUFEN);
...
static ssize_t ads1242_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	return -ENOSYS;
}
...
static int ads1242_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case ANIOC_ADS2142_SET_POSITIVE_INPUT: {
		ads1242_set_positive_input(priv, (ADS1242_POSITIVE_INPUT_SELECTION)(arg));
	} break;

		/* Set the negative input of the ADC */

	case ANIOC_ADS2142_SET_NEGATIVE_INPUT: {
		ads1242_set_negative_input(priv, (ADS1242_NEGATIVE_INPUT_SELECTION)(arg));
	} break;
...
int ads1242_register(FAR const char *devpath, FAR struct spi_dev_s *spi, uint32_t const osc_freq_hz)
{
