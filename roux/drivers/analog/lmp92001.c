...
#include <roux/arch.h>
#include <roux/analog/adc.h>
#include <roux/analog/dac.h>
#include <roux/ioexpander/ioexpander.h>
#include <roux/i2c/i2c_master.h>

#include <roux/analog/lmp92001.h>
...
static int lmp92001_i2c_write(FAR struct lmp92001_dev_s *priv, FAR const uint8_t *buffer,
							  int buflen);
static int lmp92001_i2c_read(FAR struct lmp92001_dev_s *priv, uint8_t reg, FAR uint8_t *buffer,
							 int buflen);

static int lmp92001_dac_setref(FAR struct lmp92001_dev_s *priv, enum lmp92001_ref_e ref);
static int lmp92001_dac_updateall(FAR struct lmp92001_dev_s *priv, uint16_t value);

static int lmp92001_adc_setref(FAR struct lmp92001_dev_s *priv, enum lmp92001_ref_e ref);
...
static int lmp92001_adc_readchannel(FAR struct lmp92001_dev_s *priv, FAR struct adc_msg_s *msg);
...
static int lmp92001_dac_send(FAR struct dac_dev_s *dev, FAR struct dac_msg_s *msg);
static int lmp92001_dac_ioctl(FAR struct dac_dev_s *dev, int cmd, unsigned long arg);
#endif

/* ADC Interface. */

#ifdef CONFIG_ADC
static int lmp92001_adc_bind(FAR struct adc_dev_s *dev, FAR const struct adc_callback_s *callback);
...
static int lmp92001_adc_ioctl(FAR struct adc_dev_s *dev, int cmd, unsigned long arg);
#endif

/* I/O Expander Interface. */

#ifdef CONFIG_IOEXPANDER
static int lmp92001_gpio_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int dir);
static int lmp92001_gpio_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt,
								void *regval);
static int lmp92001_gpio_writepin(FAR struct ioexpander_dev_s *dev, uint8_t pin, bool value);
static int lmp92001_gpio_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
#ifdef CONFIG_IOEXPANDER_MULTIPIN
static int lmp92001_gpio_multiwritepin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
									   FAR const bool *values, int count);
static int lmp92001_gpio_multireadpin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
									  FAR bool *values, int count);
#endif
#ifdef CONFIG_IOEXPANDER_INT_ENABLE
static FAR void *lmp92001_gpio_attach(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset,
									  ioe_callback_t callback, FAR void *arg);
static int lmp92001_gpio_detach(FAR struct ioexpander_dev_s *dev, FAR void *handle);
#endif
#endif
...
#ifdef CONFIG_IOEXPANDER
static const struct ioexpander_ops_s g_gpio_ops = {lmp92001_gpio_direction,
							  ...
							  int buflen)
{
	...
	msg.frequency = CONFIG_LMP92001_I2C_FREQUENCY, msg.addr = priv->addr;
	msg.flags = 0;
							 ...
							 int buflen)
{
	...
	msg[0].frequency = CONFIG_LMP92001_I2C_FREQUENCY, msg[0].addr = priv->addr,
	msg[0].flags = I2C_M_NOSTOP;
	msg[0].buffer = &reg;
	msg[0].length = 1;

	msg[1].frequency = CONFIG_LMP92001_I2C_FREQUENCY, msg[1].addr = priv->addr,
...
static int lmp92001_adc_ioctl(FAR struct adc_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case ANIOC_LMP92001_ADC_ENABLE: {
		ret = lmp92001_adc_enablechannel(priv, (enum lmp92001_adc_enable_e)arg);
	} break;
	...
	case ANIOC_LMP92001_ADC_SINGLESHOT_CONV: {
		...
		for (i = 0; i < priv->adc_channels_enabled; i++) {
			...
			priv->cb->au_receive(&g_adcdev, priv->adc_channels[i], msg.am_data);
		}
	} break;

	case ANIOC_LMP92001_ADC_READ_CHANNEL: {
		FAR struct adc_msg_s *msg = (FAR struct adc_msg_s *)((uintptr_t)arg);
...
#ifdef CONFIG_IOEXPANDER
static int lmp92001_gpio_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int direction)
{
...
static int lmp92001_gpio_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value)
{
	...
	DEBUGASSERT(priv != NULL && pin < LMP92001_GPIO_MAX_PINS && value != NULL);
...
#ifdef CONFIG_DAC
FAR struct dac_dev_s *lmp92001_dac_initialize(FAR struct i2c_master_s *i2c, uint8_t addr)
{
...
#ifdef CONFIG_ADC
FAR struct adc_dev_s *lmp92001_adc_initialize(FAR struct i2c_master_s *i2c, uint8_t addr)
{
	FAR struct lmp92001_dev_s *priv;
...
#ifdef CONFIG_IOEXPANDER
FAR struct ioexpander_dev_s *lmp92001_gpio_initialize(FAR struct i2c_master_s *i2c, uint8_t addr)
{
	FAR struct lmp92001_dev_s *priv;
