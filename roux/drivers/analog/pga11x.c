...
int pga11x_select(PGA11X_HANDLE handle, FAR const struct pga11x_settings_s *settings)
{
#ifndef CONFIG_PGA11X_DAISYCHAIN
	...
	cmd = PGA11X_CMD_WRITE | ((uint16_t)settings->channel << PGA11X_CHAN_SHIFT) |
	...
	u1cmd = PGA11X_CMD_WRITE | ((uint16_t)settings->u1.channel << PGA11X_CHAN_SHIFT) |
			((uint16_t)settings->u1.gain << PGA11X_GAIN_SHIFT);

	u2cmd = PGA11X_DCCMD_WRITE | ((uint16_t)settings->u2.channel << PGA11X_CHAN_SHIFT) |
...
int pga11x_uselect(PGA11X_HANDLE handle, int pos, FAR const struct pga11x_usettings_s *settings)
{
	FAR struct spi_dev_s *spi = (FAR struct spi_dev_s *)handle;
	uint16_t u1cmd;
	uint16_t u2cmd;
	...
	if (pos == 0) {
		u1cmd = PGA11X_CMD_WRITE | ((uint16_t)settings->channel << PGA11X_CHAN_SHIFT) |
				((uint16_t)settings->gain << PGA11X_GAIN_SHIFT);
		u2cmd = PGA11X_DCCMD_NOOP;
	} else /* if (pos == 1) */
	{
		u1cmd = PGA11X_CMD_NOOP;
		u2cmd = PGA11X_DCCMD_WRITE | ((uint16_t)settings->channel << PGA11X_CHAN_SHIFT) |
				((uint16_t)settings->gain << PGA11X_GAIN_SHIFT);
	}
...
#ifdef CONFIG_PGA11X_DAISYCHAIN
int pga11x_uread(PGA11X_HANDLE handle, int pos, FAR struct pga11x_usettings_s *settings)
{
