...
static int mcp47x6_ioctl(FAR struct dac_dev_s *dev, int cmd, unsigned long arg);
...
static int mcp47x6_i2c_write(FAR struct mcp47x6_dev_s *priv, uint8_t const *source, size_t size)
{
...
static int mcp47x6_i2c_read(FAR struct mcp47x6_dev_s *priv, uint8_t *destination, size_t size)
{
	struct i2c_msg_s msg;
	int ret;
...
static int mcp47x6_setup(FAR struct dac_dev_s *dev)
{
	...
	uint8_t default_settings =
		MCP47X6_REFERENCE_VDD_UNBUFFERED | MCP47X6_POWER_DOWN_DISABLED | MCP47X6_GAIN_1X;
	...
	priv->cmd = response & (MCP47X6_REFERENCE_MASK | MCP47X6_POWER_DOWN_MASK | MCP47X6_GAIN_MASK);
...
FAR struct dac_dev_s *mcp47x6_initialize(FAR struct i2c_master_s *i2c, uint8_t addr)
{
