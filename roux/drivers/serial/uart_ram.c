...
#define UART_RAMBUF_SECTION(n) locate_data(CONFIG_RAM_UART_BUFFER_SECTION "." #n)
...
static size_t uart_rambuf_txready(FAR struct uart_rambuf_s *buf)
{
	...
	return rdoff > wroff ? rdoff - wroff - 1 : sizeof(buf->buffer) - wroff + rdoff - 1;
}
...
static size_t uart_rambuf_rxavailable(FAR struct uart_rambuf_s *buf)
{
	...
	return wroff >= rdoff ? wroff - rdoff : sizeof(buf->buffer) - rdoff + wroff;
}
...
static int uart_ram_attach(FAR struct uart_dev_s *dev)
{
	...
	wd_start(&priv->wdog, USEC2TICK(CONFIG_RAM_UART_POLLING_INTERVAL), uart_ram_wdog,
			 (wdparm_t)dev);
	return OK;
}
...
int uart_ram_register(FAR const char *devname, struct uart_rambuf_s rambuf[2], bool slave)
{
