...
static int uart_gdbstub_ctrlc(FAR struct uart_dev_s *dev, FAR unsigned int *status);

/****************************************************************************
 * Private Functions
 ****************************************************************************/

static void uart_gdbstub_attach(FAR struct uart_gdbstub_s *uart_gdbstub, bool replace)
{
...
static int uart_gdbstub_panic_callback(FAR struct notifier_block *nb, unsigned long action,
									   FAR void *data)
{
	FAR struct uart_gdbstub_s *uart_gdbstub = container_of(nb, struct uart_gdbstub_s, nb);
	...
	while (true) {
		if (uart_gdbstub->console == uart_gdbstub->dev && uart_gdbstub->org_ops != NULL) {
			if (uart_gdbstub->org_ops->recvbuf) {
				if (uart_gdbstub->org_ops->rxavailable(uart_gdbstub->console)) {
					uart_gdbstub->org_ops->recvbuf(uart_gdbstub->console, &ch, 1);
				}
			} else {
				ch = uart_gdbstub->org_ops->receive(uart_gdbstub->console, &status);
			}
		} else {
			if (uart_gdbstub->console->ops->recvbuf) {
				if (uart_gdbstub->console->ops->rxavailable(uart_gdbstub->console)) {
					uart_gdbstub->console->ops->recvbuf(uart_gdbstub->console, &ch, 1);
				}
			} else {
				ch = uart_gdbstub->console->ops->receive(uart_gdbstub->console, &status);
			}
		}
		...
		if ((clock_systime_ticks()) - base >= SEC2TICK(CONFIG_SERIAL_GDBSTUB_PANIC_TIMEOUT)) {
	...
	_alert("Please use gdb of the corresponding architecture to "
		   "connect to roux");
	_alert("such as: arm-none-eabi-gdb roux -ex \"set "
...
static ssize_t uart_gdbstub_receive(FAR void *priv, FAR void *buf, size_t len)
{
...
static ssize_t uart_gdbstub_send(FAR void *priv, FAR const char *buf, size_t len)
{
	FAR struct uart_gdbstub_s *uart_gdbstub = priv;
	FAR uart_dev_t *dev = uart_gdbstub->dev;
	size_t i = 0;
...
int uart_gdbstub_register(FAR uart_dev_t *dev, FAR const char *path)
{
	...
	uart_gdbstub->state =
		gdb_state_init(uart_gdbstub_send, uart_gdbstub_receive, NULL, uart_gdbstub);
