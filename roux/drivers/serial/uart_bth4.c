...
#include <roux/wireless/bluetooth/bt_uart.h>
...
static ssize_t uart_bth4_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t uart_bth4_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int uart_bth4_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
...
static void uart_bth4_pollnotify(FAR struct uart_bth4_s *dev, pollevent_t eventset)
{
...
static int uart_bth4_receive(FAR struct bt_driver_s *drv, enum bt_buf_type_e type, FAR void *buffer,
							 size_t buflen)
{
	...
	if (circbuf_space(&dev->circbuf) >= buflen + H4_HEADER_SIZE) {
		if (type == BT_EVT) {
			htype = H4_EVT;
...
static ssize_t uart_bth4_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	memcpy(data - H4_HEADER_SIZE + dev->sendlen, buffer, buflen);
	dev->sendlen += buflen;
	...
	for (;;) {
		...
		ret = dev->drv->send(dev->drv, type, data, pktlen - H4_HEADER_SIZE);
		if (ret < 0) {
			goto err;
		}
		...
		memmove(data - H4_HEADER_SIZE, dev->sendbuf + pktlen, dev->sendlen);
	}
...
int uart_bth4_register(FAR const char *path, FAR struct bt_driver_s *drv)
{
	...
	dev = (FAR struct uart_bth4_s *)kmm_zalloc(sizeof(struct uart_bth4_s));
	if (dev == NULL) {
		return -ENOMEM;
	}

	ret = circbuf_init(&dev->circbuf, NULL, CONFIG_UART_BTH4_RXBUFSIZE);
	if (ret < 0) {
		kmm_free(dev);
		return -ENOMEM;
	}
