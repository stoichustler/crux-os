...
#include <roux/serial/uart_rpmsg.h>
...
static bool uart_rpmsg_rxflowcontrol(FAR struct uart_dev_s *dev, unsigned int nbuffered,
									 bool upper);
...
static void uart_rpmsg_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_);
static void uart_rpmsg_device_destroy(FAR struct rpmsg_device *rdev, FAR void *priv_);
static int uart_rpmsg_ept_cb(struct rpmsg_endpoint *ept, FAR void *data, size_t len, uint32_t src,
							 FAR void *priv_);
...
static bool uart_rpmsg_rxflowcontrol(FAR struct uart_dev_s *dev, unsigned int nbuffered, bool upper)
{
...
static void uart_rpmsg_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	...
	if (strcmp(priv->cpuname, rpmsg_get_cpuname(rdev)) == 0) {
		priv->ept.priv = dev;
		snprintf(eptname, sizeof(eptname), "%s%s", UART_RPMSG_EPT_PREFIX, priv->devname);
...
static void uart_rpmsg_device_destroy(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	...
	if (priv->ept.priv != NULL && strcmp(priv->cpuname, rpmsg_get_cpuname(rdev)) == 0) {
...
static int uart_rpmsg_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
...
int uart_rpmsg_init(FAR const char *cpuname, FAR const char *devname, int buf_size, bool isconsole)
{
	...
	dev = kmm_zalloc(sizeof(struct uart_dev_s) + sizeof(struct uart_rpmsg_priv_s) + buf_size * 2);
	if (dev == NULL) {
		return -ENOMEM;
	}
	...
	dev->priv = dev + 1;
	dev->recv.buffer = (FAR char *)dev->priv + sizeof(struct uart_rpmsg_priv_s);
	...
	ret = rpmsg_register_callback(dev, uart_rpmsg_device_created, uart_rpmsg_device_destroy, NULL,
								  NULL);
	if (ret < 0) {
	...
	snprintf(name, sizeof(name), "%s%s", UART_RPMSG_DEV_PREFIX, devname);
