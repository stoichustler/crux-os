...
static void uart_poll_notify(FAR uart_dev_t *dev, unsigned int min, unsigned int max,
...
static inline ssize_t uart_irqwritev(FAR uart_dev_t *dev, FAR struct uio *uio);
...
static bool uart_is_termios_hw_change(FAR struct file *filep, FAR const struct termios *new)
{
...
static inline void uart_putchars(FAR uart_dev_t *dev, FAR const void *buf, size_t len)
{
...
static ssize_t uart_readv(FAR struct file *filep, FAR struct uio *uio)
{
	...
	while (recvd < buflen) {
		...
		if (rxbuf->head != tail) {
			...
			if ((dev->tc_lflag & ICANON) && (ch == ASCII_BS || ch == ASCII_DEL)) {
				if (recvd > 0) {
...
#endif
				{
					/* Now wait with the Rx interrupt enabled.  ROUX will
...
static int uart_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	if (dev->ops->ioctl && (cmd != TCSETS || uart_is_termios_hw_change(filep, termiosp))) {
