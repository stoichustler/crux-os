...
#include <roux/serial/uart_cmsdk.h>
...
static int uart_cmsdk_receive(FAR struct uart_dev_s *dev, FAR unsigned int *status);
...
static uint32_t uart_cmsdk_serialmodify(FAR struct uart_cmsdk_s *priv, uint32_t offset,
...
static inline void uart_cmsdk_restoreuartint(FAR struct uart_cmsdk_s *priv, uint32_t ier)
{
...
static int uart_cmsdk_setup(FAR struct uart_dev_s *dev)
{
	...
	uart_cmsdk_serialmodify(priv, UART_CTRL_OFFSET, 0, UART_CTRL_TX_ENABLE | UART_CTRL_RX_ENABLE);
...
static int uart_cmsdk_receive(struct uart_dev_s *dev, FAR unsigned int *status)
{
	FAR struct uart_cmsdk_s *priv = dev->priv;
...
static void uart_cmsdk_rxint(struct uart_dev_s *dev, bool enable)
{
	...
	if (enable) {
		uart_cmsdk_serialmodify(priv, UART_CTRL_OFFSET, 0,
								UART_CTRL_RX_INT_ENABLE | UART_CTRL_RX_OVERRUN_INT_ENABLE);
	} else {
...
static void uart_cmsdk_txint(struct uart_dev_s *dev, bool enable)
{
	...
	if (enable) {
		uart_cmsdk_serialmodify(priv, UART_CTRL_OFFSET, 0,
								UART_CTRL_TX_INT_ENABLE | UART_CTRL_TX_OVERRUN_INT_ENABLE);
