...
#include <roux/serial/uart_16550.h>
#include <roux/serial/uart_pci_16550.h>
...
static uart_datawidth_t pci_u16550_getreg_mem(FAR struct u16550_s *priv, unsigned int offset);
static void pci_u16550_putreg_mem(FAR struct u16550_s *priv, unsigned int offset,
								  uart_datawidth_t value);
static uart_datawidth_t pci_u16550_getreg_io(FAR struct u16550_s *priv, unsigned int offset);
static void pci_u16550_putreg_io(FAR struct u16550_s *priv, unsigned int offset,
								 uart_datawidth_t value);
static int pci_u16550_ioctl(FAR struct u16550_s *priv, int cmd, unsigned long arg);

static FAR struct dma_chan_s *pci_u16550_dmachan(FAR struct u16550_s *priv, unsigned int ident);
...
static int pci_u16550_initialize(FAR struct pci_u16550_priv_s *priv,
								 FAR const struct pci_u16550_type_s *type, uintptr_t base,
								 FAR struct pci_device_s *dev, bool mmio);
...
static const struct pci_device_id_s g_pci_u16550_id_table[] = {
	{PCI_DEVICE(0x1b36, 0x0002), .driver_data = (uintptr_t)&g_pci_u16550_qemu_x1},
	{PCI_DEVICE(0x1b36, 0x0003), .driver_data = (uintptr_t)&g_pci_u16550_qemu_x2},
	{PCI_DEVICE(0x1b36, 0x0004), .driver_data = (uintptr_t)&g_pci_u16550_qemu_x4},
	{PCI_DEVICE(0x125b, 0x9100), .driver_data = (uintptr_t)&g_pci_u16550_ax99100_x2},
	{}};
...
static struct pci_u16550_priv_s g_pci_u16550_priv0 = {
	...
	.common = {.baud = CONFIG_16550_PCI_UART0_BAUD,
...
#if defined(CONFIG_16550_PCI_UART0_IFLOWCONTROL) || defined(CONFIG_16550_PCI_UART0_OFLOWCONTROL)
			   .flow = true,
#endif
			   .rxtrigger = 2,
			   .lock = SP_UNLOCKED},
...
static struct pci_u16550_priv_s g_pci_u16550_priv1 = {
	...
	.common = {.baud = CONFIG_16550_PCI_UART1_BAUD,
...
#if defined(CONFIG_16550_PCI_UART1_IFLOWCONTROL) || defined(CONFIG_16550_PCI_UART1_OFLOWCONTROL)
			   .flow = true,
#endif
			   .rxtrigger = 2,
			   .lock = SP_UNLOCKED},
...
static struct pci_u16550_priv_s g_pci_u16550_priv2 = {
	...
	.common = {.baud = CONFIG_16550_PCI_UART2_BAUD,
...
#if defined(CONFIG_16550_PCI_UART2_IFLOWCONTROL) || defined(CONFIG_16550_PCI_UART2_OFLOWCONTROL)
			   .flow = true,
#endif
			   .rxtrigger = 2,
			   .lock = SP_UNLOCKED},
...
static struct pci_u16550_priv_s g_pci_u16550_priv3 = {
	...
	.common = {.baud = CONFIG_16550_PCI_UART3_BAUD,
...
#if defined(CONFIG_16550_PCI_UART3_IFLOWCONTROL) || defined(CONFIG_16550_PCI_UART3_OFLOWCONTROL)
			   .flow = true,
#endif
			   .rxtrigger = 2,
			   .lock = SP_UNLOCKED},
								 ...
								 FAR struct pci_device_s *dev, bool mmio)
{
	...
	offset = (priv->common.regincr * sizeof(uart_datawidth_t) * UART_IER_OFFSET);
...
static int pci_u16550_register(FAR uart_dev_t *dev)
{
	FAR struct pci_u16550_priv_s *priv = (FAR struct pci_u16550_priv_s *)dev->priv;
	int ret = OK;
...
static int pci_u16550_probe(FAR struct pci_device_s *dev)
{
	...
	for (; port < ports; port++) {
		...
		for (i = 0; i < nitems(g_pci_u16550_dev); i++) {
			...
			if (priv->vendor == dev->vendor && priv->device == dev->device && priv->port == port) {
				break;
			}
		}
