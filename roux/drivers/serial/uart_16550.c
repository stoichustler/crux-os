...
#if defined(CONFIG_16550_UART0) || defined(CONFIG_16550_UART1) || defined(CONFIG_16550_UART2) ||   \
	defined(CONFIG_16550_UART3)
...
#ifdef CONFIG_SERIAL_UART_ARCH_MMIO
static uart_datawidth_t u16550_mmio_getreg(FAR struct u16550_s *priv, unsigned int offset);
static void u16550_mmio_putreg(FAR struct u16550_s *priv, unsigned int offset,
...
static bool u16550_rxflowcontrol(struct uart_dev_s *dev, unsigned int nbuffered, bool upper);
#endif
...
static inline void u16550_disableuartint(FAR struct u16550_s *priv, FAR uart_datawidth_t *ier)
{
	if (ier) {
...
static int u16550_setup(FAR struct uart_dev_s *dev)
{
	...
	u16550_serialout(priv, UART_FCR_OFFSET,
					 (priv->rxtrigger << UART_FCR_RXTRIGGER_SHIFT | UART_FCR_TXRST |
...
#ifdef HAVE_16550_UART_DMA
static void u16550_dmasend_done(FAR struct dma_chan_s *chan, FAR void *arg, ssize_t len)
{
...
static void u16550_dmasend(FAR struct uart_dev_s *dev)
{
	...
	DMA_START(priv->chantx, u16550_dmasend_done, dev,
			  up_addrenv_va_to_pa((FAR void *)priv->uartbase), up_addrenv_va_to_pa(buffer), length);
}

static void u16550_dmareceive_done(FAR struct dma_chan_s *chan, FAR void *arg, ssize_t len)
{
	FAR struct uart_dev_s *dev = arg;
	FAR struct u16550_s *priv = dev->priv;
...
static void u16550_dmarxfree(FAR struct uart_dev_s *dev)
{
	...
	if (priv->chanrx == NULL) {
		priv->chanrx = priv->ops->dmachan(priv, priv->dmarx) if (priv->chanrx == NULL)
		{
		...
		DMA_START_CYCLIC(
			priv->chanrx, u16550_dmareceive_done, dev, up_addrenv_va_to_pa(priv->dmarxbuf),
			up_addrenv_va_to_pa((FAR void *)priv->uartbase), priv->dmarxsize, priv->dmarxsize / 4);
	}
