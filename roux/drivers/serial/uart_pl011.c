...
#include <roux/serial/uart_pl011.h>
...
static int pl011_receive(FAR struct uart_dev_s *dev, FAR unsigned int *status);
...
static int pl011_set_baudrate(FAR const struct pl011_uart_port_s *sport, uint32_t clk,
							  uint32_t baudrate)
{
	FAR const struct pl011_config *config = &sport->config;
...
static int pl011_irq_rx_ready(FAR const struct pl011_uart_port_s *sport)
{
	...
	return (config->uart->imsc & PL011_IMSC_RXIM) && (!(config->uart->fr & PL011_FR_RXFE));
}
...
static bool pl011_txready(FAR struct uart_dev_s *dev)
{
	...
	return (config->uart->imsc & PL011_IMSC_TXIM) && pl011_irq_tx_ready(sport);
}
...
static bool pl011_rxavailable(FAR struct uart_dev_s *dev)
{
	...
	if (!data->sbsa &&
		(!(config->uart->cr & PL011_CR_UARTEN) || !(config->uart->cr & PL011_CR_RXE))) {
		return false;
	}
