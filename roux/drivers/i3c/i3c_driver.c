...
static ssize_t i3cdrvr_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t i3cdrvr_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static ssize_t i3cdrvr_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	return 0; /* Return EOF */
}
...
static ssize_t i3cdrvr_write(FAR struct file *filep, FAR const char *buffer, size_t len)
{
...
static int i3cdrvr_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case I3CIOC_PRIV_XFERS:
		...
		ret = i3c_device_do_priv_xfers(desc->dev, transfer->xfers, transfer->nxfers);
		break;
