													 ...
													 FAR const struct i3c_device_id *id)
{
	...
	i3c_bus_for_each_i3cdev(&master->bus, desc)
	{
		...
		if ((id->match_flags & I3C_MATCH_DCR) && id->dcr != desc->info.dcr) {
			continue;
		}

		if ((id->match_flags & I3C_MATCH_MANUF) && id->manuf_id != manuf) {
			continue;
		}

		if ((id->match_flags & I3C_MATCH_PART) && (rndpid || id->part_id != part)) {
			continue;
		}

		if ((id->match_flags & I3C_MATCH_EXTRA_INFO) && (rndpid || id->extra_info != ext_info)) {
			continue;
		}
