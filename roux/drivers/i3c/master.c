...
#include <roux/i3c/master.h>
#include <roux/i2c/i2c_master.h>
#include <roux/i3c/i3c_driver.h>
#include <roux/i3c/device.h>
#include "internals.h"
...
static FAR struct i3c_master_controller *i3c_bus_to_i3c_master(FAR struct i3c_bus *i3cbus)
{
...
static int i3c_master_to_i2c_bus_number(FAR struct i3c_master_controller *master)
{
...
static enum i3c_addr_slot_status i3c_bus_get_addr_slot_status(FAR struct i3c_bus *bus,
															  uint16_t addr)
{
	int32_t status;
	int bitpos = addr * 2;
...
static void i3c_bus_set_addr_slot_status(FAR struct i3c_bus *bus, uint16_t addr,
										 enum i3c_addr_slot_status status)
{
	...
	*ptr &= ~((unsigned long)I3C_ADDR_SLOT_STATUS_MASK << (bitpos % I3C_BITS_PER_LONG));
...
static bool i3c_bus_dev_addr_is_avail(FAR struct i3c_bus *bus, unsigned char addr)
{
	enum i3c_addr_slot_status status;
...
static int i3c_bus_get_free_addr(FAR struct i3c_bus *bus, unsigned char start_addr)
{
	enum i3c_addr_slot_status status;
	unsigned char addr;
...
static void i3c_bus_init_addrslots(FAR struct i3c_bus *bus)
{
	...
	i3c_bus_set_addr_slot_status(bus, I3C_BROADCAST_ADDR, I3C_ADDR_SLOT_RSVD);
	for (i = 0; i < 7; i++) {
		i3c_bus_set_addr_slot_status(bus, I3C_BROADCAST_ADDR ^ I3C_BIT(i), I3C_ADDR_SLOT_RSVD);
	}
}
...
static int i3c_bus_set_mode(FAR struct i3c_bus *i3cbus, enum i3c_bus_mode mode,
							unsigned long max_i2c_scl_rate)
{
	...
	case I3C_BUS_MODE_MIXED_SLOW: {
		...
		if (!i3cbus->scl_rate.i3c || i3cbus->scl_rate.i3c > i3cbus->scl_rate.i2c) {
...
static int i3c_master_to_i3c_bus_number(FAR struct i3c_master_controller *master)
{
...
static FAR void *i3c_ccc_cmd_dest_init(FAR struct i3c_ccc_cmd_dest *dest, unsigned char addr,
									   uint16_t payloadlen)
{
	dest->addr = addr;
...
static void i3c_ccc_cmd_init(FAR struct i3c_ccc_cmd *cmd, bool rnw, unsigned char id,
							 FAR struct i3c_ccc_cmd_dest *dests, unsigned int ndests)
{
	cmd->rnw = rnw ? 1 : 0;
	cmd->id = id;
	cmd->dests = dests;
...
static FAR struct i3c_dev_desc *i3c_master_alloc_i3c_dev(FAR struct i3c_master_controller *master,
...
static int i3c_master_rstdaa_locked(FAR struct i3c_master_controller *master, unsigned char addr)
{
	...
	i3c_ccc_cmd_init(&cmd, false, I3C_CCC_RSTDAA(addr == I3C_BROADCAST_ADDR), &dest, 1);
...
static int i3c_master_enec_disec_locked(FAR struct i3c_master_controller *master,
...
static int i3c_master_setda_locked(FAR struct i3c_master_controller *master, unsigned char oldaddr,
								   unsigned char newaddr, bool setdasa)
{
	struct i3c_ccc_cmd_dest dest;
	...
	i3c_ccc_cmd_init(&cmd, false, setdasa ? I3C_CCC_SETDASA : I3C_CCC_SETNEWDA, &dest, 1);
	ret = i3c_master_send_ccc_cmd_locked(master, &cmd);
	i3c_ccc_cmd_dest_cleanup(&dest);
...
static int i3c_master_setnewda_locked(FAR struct i3c_master_controller *master,
...
static int i3c_master_getmxds_locked(FAR struct i3c_master_controller *master,
									 FAR struct i3c_device_info *info)
{
	...
	getmaxds = i3c_ccc_cmd_dest_init(&dest, info->dyn_addr, sizeof(*getmaxds));
	if (getmaxds == NULL) {
		return -ENOMEM;
	}
...
static int i3c_master_gethdrcap_locked(FAR struct i3c_master_controller *master,
									   FAR struct i3c_device_info *info)
{
	...
	gethdrcap = i3c_ccc_cmd_dest_init(&dest, info->dyn_addr, sizeof(*gethdrcap));
	if (gethdrcap == NULL) {
		return -ENOMEM;
	}
...
static int i3c_master_retrieve_dev_info(FAR struct i3c_dev_desc *dev)
{
	...
	slot_status = i3c_bus_get_addr_slot_status(&master->bus, dev->info.dyn_addr);
...
static int i3c_master_get_i3c_addrs(FAR struct i3c_dev_desc *dev)
{
	...
	if (dev->info.dyn_addr) {
		status = i3c_bus_get_addr_slot_status(&master->bus, dev->info.dyn_addr);
		if (status != I3C_ADDR_SLOT_FREE) {
			return -EBUSY;
		}

		i3c_bus_set_addr_slot_status(&master->bus, dev->info.dyn_addr, I3C_ADDR_SLOT_I3C_DEV);
	}

	return 0;
}

static int i3c_master_attach_i3c_dev(FAR struct i3c_master_controller *master,
									 FAR struct i3c_dev_desc *dev)
{
	int ret;
...
static int i3c_master_reattach_i3c_dev(FAR struct i3c_dev_desc *dev, unsigned char old_dyn_addr)
{
	FAR struct i3c_master_controller *master = i3c_dev_get_master(dev);
	enum i3c_addr_slot_status status;
	int ret;
...
static void i3c_master_register_new_i3c_devs(FAR struct i3c_master_controller *master)
{
...
static void i3c_master_detach_free_devs(FAR struct i3c_master_controller *master)
{
	FAR struct i3c_dev_desc *i3cdev;
	FAR struct i3c_dev_desc *i3ctmp;

	list_for_every_entry_safe(&master->bus.devs.i3c, i3cdev, i3ctmp, struct i3c_dev_desc,
							  common.node)
	{
...
static int i3c_master_bus_init(FAR struct i3c_master_controller *master)
{
	...
	ret = i3c_master_disec_locked(master, I3C_BROADCAST_ADDR,
								  I3C_CCC_EVENT_SIR | I3C_CCC_EVENT_MR | I3C_CCC_EVENT_HJ);
	if (ret && ret != I3C_ERROR_M2) {
...
static FAR struct i3c_dev_desc *i3c_master_search_i3c_dev_duplicate(FAR struct i3c_dev_desc *refdev)
{
...
static int i3c_master_i2c_adapter_xfer(FAR struct i2c_master_s *i2c, FAR struct i2c_msg_s *xfers,
									   int nxfers)
{
...
static void i3c_master_unregister_i3c_devs(FAR struct i3c_master_controller *master)
{
	FAR struct i3c_dev_desc *i3cdev;
...
static void i3c_master_init_ibi_slot(FAR struct i3c_dev_desc *dev, FAR struct i3c_ibi_slot *slot)
{
	slot->dev = dev;
}

static int i3c_master_check_ops(FAR const struct i3c_master_controller_ops *ops)
{
	if (ops == NULL || ops->bus_init == NULL || ops->priv_xfers == NULL ||
		ops->send_ccc_cmd == NULL || ops->do_daa == NULL || ops->i2c_xfers == NULL) {
		return -EINVAL;
	}

	if (ops->request_ibi && (ops->enable_ibi == NULL || ops->disable_ibi == NULL ||
...
static void i3c_unregister_driver(FAR struct i3c_master_controller *master)
{
	...
	snprintf(devname, sizeof(devname), "/dev/i3c%d", i3c_master_to_i3c_bus_number(master));
	nx_unlink(devname);
}
...
int i3c_master_send_ccc_cmd_locked(FAR struct i3c_master_controller *master,
								   FAR struct i3c_ccc_cmd *cmd)
{
	...
	if (master->init_done && !nxmutex_is_locked(&master->bus.lock)) {
		return -EINVAL;
	}
	...
	if (master->ops->supports_ccc_cmd && !master->ops->supports_ccc_cmd(master, cmd)) {
		return -ENOTSUP;
	}
...
int i3c_master_get_free_addr(FAR struct i3c_master_controller *master, unsigned char start_addr)
{
...
int i3c_master_enec_locked(FAR struct i3c_master_controller *master, unsigned char addr,
						   unsigned char evts)
{
...
int i3c_master_disec_locked(FAR struct i3c_master_controller *master, unsigned char addr,
							unsigned char evts)
{
						...
						FAR const struct i3c_device_info *info)
{
	...
	if (I3C_BCR_DEVICE_ROLE(info->bcr) == I3C_BCR_I3C_MASTER && master->secondary) {
		return -EINVAL;
	}
...
int i3c_master_add_i3c_dev_locked(FAR struct i3c_master_controller *master, unsigned char addr)
{
	...
	if (newdev->info.dyn_addr != expected_dyn_addr) {
		...
		ret = i3c_master_setnewda_locked(master, newdev->info.dyn_addr, expected_dyn_addr);
		if (!ret) {
...
void i3c_master_queue_ibi(FAR struct i3c_dev_desc *dev, FAR struct i3c_ibi_slot *slot)
{
...
void i3c_generic_ibi_free_pool(FAR struct i3c_generic_ibi_pool *pool)
{
	...
	while (!list_is_empty(&pool->free_slots)) {
		slot = list_first_entry(&pool->free_slots, struct i3c_generic_ibi_slot, node);
...
FAR struct i3c_generic_ibi_pool *i3c_generic_ibi_alloc_pool(FAR struct i3c_dev_desc *dev,
															FAR const struct i3c_ibi_setup *req)
{
	...
	if (req->max_payload_len) {
		pool->payload_buf = kmm_calloc(req->num_slots, req->max_payload_len);
	...
	for (i = 0; i < req->num_slots; i++) {
		...
		if (req->max_payload_len) {
			slot->base.data = pool->payload_buf + (i * req->max_payload_len);
		}
...
FAR struct i3c_ibi_slot *i3c_generic_ibi_get_free_slot(FAR struct i3c_generic_ibi_pool *pool)
{
...
void i3c_generic_ibi_recycle_slot(FAR struct i3c_generic_ibi_pool *pool, FAR struct i3c_ibi_slot *s)
{
	FAR struct i3c_generic_ibi_slot *slot;
	unsigned long flags;
...
int i3c_dev_do_priv_xfers_locked(FAR struct i3c_dev_desc *dev, FAR struct i3c_priv_xfer *xfers,
								 int nxfers)
{
	FAR struct i3c_master_controller *master;
...
int i3c_dev_request_ibi_locked(FAR struct i3c_dev_desc *dev, FAR const struct i3c_ibi_setup *req)
{
	FAR struct i3c_master_controller *master = i3c_dev_get_master(dev);
...
int i3c_master_i2c_attach(FAR struct i3c_master_controller *master, FAR struct i2c_config_s *config)
{
	int ret;
...
int i3c_master_register(FAR struct i3c_master_controller *master,
						FAR const struct i3c_master_controller_ops *ops, bool secondary)
{
