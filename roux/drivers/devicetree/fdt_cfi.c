...
int fdt_cfi_register(FAR const void *fdt)
{
	...
	while ((offset = fdt_node_offset_by_compatible(fdt, offset, "cfi-flash")) >= 0) {
		...
		for (i = 0; i < num; i++) {
			...
			uintptr_t flash_end = fdt_get_reg_size_by_index(fdt, offset, i) + flash_base;

			if ((CONFIG_FLASH_START > flash_end ||
				 CONFIG_FLASH_START + CONFIG_FLASH_SIZE < flash_base)) {
				ret = register_cfi_driver(flash_base, flash_end, bankwidth, i);
				if (ret < 0) {
					break;
				}
			} else {
				ferr("cfi flash%d has been used to store text:"
					 "[%x,%x], flash:[%x,%x]\n",
					 i, flash_base, flash_end, CONFIG_FLASH_START,
					 CONFIG_FLASH_START + CONFIG_FLASH_SIZE);
			}
		}
	}
