...
int fdt_pci_ecam_register(FAR const void *fdt)
{
	...
	offset = fdt_node_offset_by_compatible(fdt, -1, "pci-host-ecam-generic");
	if (offset < 0) {
		return offset;
	}
	...
	for (rlen /= 4; (rlen -= na + pna + ns) >= 0; ranges += na + pna + ns) {
		...
		if ((type & FDT_PCI_TYPE_MASK) == FDT_PCI_TYPE_IO) {
			...
			prefetch.end = prefetch.start + fdt_ld_by_cells(ranges + na + pna, ns);
			prefetch.offset = fdt_ld_by_cells(ranges + na, pna) - prefetch.start;
