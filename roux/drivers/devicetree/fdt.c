...
int fdt_get_irq(FAR const void *fdt, int nodeoffset, int offset, int irqbase)
{
...
int fdt_get_irq_by_path(FAR const void *fdt, int offset, const char *path, int irqbase)
{
...
uintptr_t fdt_get_reg_base_by_name(FAR const void *fdt, int offset, FAR const char *reg_name)
{
	uintptr_t addr = 0;

	int reg_index = fdt_stringlist_search(fdt, offset, "reg-names", reg_name);
	if (reg_index < 0) {
		return addr;
	}
...
uintptr_t fdt_get_reg_base_by_index(FAR const void *fdt, int offset, int index)
{
	FAR const void *reg;
	uintptr_t addr = 0;
	...
	if (reg != NULL) {
		...
		addr =
			fdt_ld_by_cells((FAR fdt32_t *)reg + (address_cell + size_cell) * index, address_cell);
	}
...
size_t fdt_get_reg_size(FAR const void *fdt, int offset)
{
	...
	reg = fdt_getprop(fdt, offset, "reg", NULL);
	if (reg != NULL) {
		size = fdt_ld_by_cells((FAR fdt32_t *)reg + fdt_get_parent_address_cells(fdt, offset),
...
size_t fdt_get_reg_size_by_index(FAR const void *fdt, int offset, int index)
{
	...
	if (reg != NULL) {
		...
		address_cell = fdt_get_parent_address_cells(fdt, offset);
		size_cell = fdt_get_parent_size_cells(fdt, offset);
		size = fdt_ld_by_cells(
			(FAR fdt32_t *)reg + (address_cell + size_cell) * index + address_cell, size_cell);
	}
...
FAR const char *fdt_get_node_label(FAR const void *fdt, int node)
{
	...
	{
		property_name = fdt_getprop_by_offset(fdt, property_offset, &label_name, NULL);
...
uintptr_t fdt_get_clock_frequency(FAR const void *fdt, int offset)
{
	...
	clock_frequency = fdt_ld_by_cells(pv, fdt_get_parent_address_cells(fdt, offset));

	return clock_frequency;
}

uintptr_t fdt_get_clock_frequency_from_clocks(FAR const void *fdt, int offset, int index)
{
	const fdt32_t *pv;
	fdt32_t clk_phandle;
	int pv_offset;
...
void fdt_node_from_compat(FAR const void *fdt, FAR const char **compatible_ids,
						  FAR void (*driver_callback)(FAR const void *fdt, int offset))
{
	int offset = 0;
	...
	while (*compatible_ids) {
		while (true) {
			offset = fdt_node_offset_by_compatible(fdt, offset, *compatible_ids);
...
int fdt_load_prop_u32(FAR const void *fdt, int offset, FAR const char *property, int index,
					  FAR uint32_t *value)
{
