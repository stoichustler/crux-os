...
ssize_t ipcc_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	for (nwritten = 0;;) {
		...
		if (circbuf_used(&priv->ipcc->txbuf) == 0) {
			...
			nwritten += priv->ipcc->ops.write(priv->ipcc, buffer + nwritten, buflen - nwritten);
		...
		nwritten +=
			circbuf_write(&priv->ipcc->txbuf, buffer + nwritten, (ssize_t)buflen - nwritten);
