...
static struct list_node g_reset_controller_list = LIST_INITIAL_VALUE(g_reset_controller_list);
...
static inline FAR struct reset_control_array *rstc_to_array(FAR struct reset_control *rstc)
{
...
static int reset_control_array_deassert(FAR struct reset_control_array *resets)
{
	int ret = 0;
	unsigned int i;
...
static int reset_control_array_acquire(FAR struct reset_control_array *resets)
{
	unsigned int i;
	unsigned int ret = 0;
...
static void reset_control_array_release(FAR struct reset_control_array *resets)
{
	unsigned int i;
...
static FAR struct reset_control *reset_control_get_internal(FAR struct reset_controller_dev *rcdev,
															...
															bool acquired)
{
	...
	list_for_every_entry(&rcdev->reset_control_head, rstc, struct reset_control, list)
	{
		if (rstc->id == index) {
...
static FAR struct reset_controller_dev *reset_controller_get_by_name(FAR const char *name)
{
	FAR struct reset_controller_dev *rcdev;

	nxmutex_lock(&g_reset_list_mutex);
	list_for_every_entry(&g_reset_controller_list, rcdev, struct reset_controller_dev, list)
	{
...
FAR struct reset_control *reset_control_get(FAR const char *name, int index, bool shared,
											bool acquired)
{
	FAR struct reset_control *rstc;
	FAR struct reset_controller_dev *rcdev;
...
int reset_control_acquire(FAR struct reset_control *rstc)
{
	...
	list_for_every_entry(&rstc->rcdev->reset_control_head, rc, struct reset_control, list)
	{
...
FAR struct reset_control *reset_control_array_get(FAR const char *name, const int id[],
												  const unsigned int num, bool shared,
												  bool acquired)
{
	...
	resets = kmm_zalloc(sizeof(struct reset_control_array) + sizeof(struct reset_control) * num);
	if (!resets) {
		return NULL;
	}
