...
static int reset_rpmsg_acquire_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									   uint32_t src, FAR void *priv);

static int reset_rpmsg_release_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									   uint32_t src, FAR void *priv);

static int reset_rpmsg_reset_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									 uint32_t src, FAR void *priv);

static int reset_rpmsg_assert_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									  uint32_t src, FAR void *priv);

static int reset_rpmsg_deassert_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
										uint32_t src, FAR void *priv);

static int reset_rpmsg_status_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									  uint32_t src, FAR void *priv);

static int reset_rpmsg_acquire(FAR struct reset_controller_dev *rcdev, unsigned int id, bool shared,
							   bool acquired);

static int reset_rpmsg_release(FAR struct reset_controller_dev *rcdev, unsigned int id);

static int reset_rpmsg_reset(FAR struct reset_controller_dev *rcdev, unsigned int id);

static int reset_rpmsg_assert(FAR struct reset_controller_dev *rcdev, unsigned int id);

static int reset_rpmsg_deassert(FAR struct reset_controller_dev *rcdev, unsigned int id);

static int reset_rpmsg_status(FAR struct reset_controller_dev *rcdev, unsigned int id);

static int reset_rpmsg_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
							  uint32_t src, FAR void *priv);
...
static struct list_node g_reset_rpmsg_client = LIST_INITIAL_VALUE(g_reset_rpmsg_client);
...
static void reset_rpmsg_client_created(FAR struct rpmsg_device *rdev, FAR void *priv)
{
	...
	if (strcmp(client->cpuname, rpmsg_get_cpuname(rdev)) == 0) {
		rpmsg_create_ept(&client->ept, rdev, RESET_RPMSG_EPT_NAME, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
...
static void reset_rpmsg_client_destroy(FAR struct rpmsg_device *rdev, FAR void *priv)
{
	FAR struct reset_rpmsg_client_s *client = priv;
...
static FAR struct reset_rpmsg_client_s *reset_rpmsg_get_client(FAR const char *name)
{
	...
	list_for_every_entry(&g_reset_rpmsg_client, client, struct reset_rpmsg_client_s, node)
	{
	...
	rpmsg_register_callback(client, reset_rpmsg_client_created, reset_rpmsg_client_destroy, NULL,
							NULL);
	return client;
...
static int reset_rpmsg_sendrecv(FAR struct rpmsg_endpoint *ept, uint32_t command,
								FAR struct reset_rpmsg_header_s *msg, int len)
{
	struct reset_rpmsg_cookie_s cookie = {0};
							   ...
							   bool acquired)
{
	...
	return reset_rpmsg_sendrecv(ept, RESET_RPMSG_ACQUIRE, (struct reset_rpmsg_header_s *)msg, len);
}
...
static int reset_rpmsg_release(FAR struct reset_controller_dev *rcdev, unsigned int id)
{
	...
	strlcpy(msg->name, name, len - sizeof(*msg));
	msg->id = id;
	return reset_rpmsg_sendrecv(ept, RESET_RPMSG_RELEASE, (struct reset_rpmsg_header_s *)msg, len);
}
...
static int reset_rpmsg_reset(FAR struct reset_controller_dev *rcdev, unsigned int id)
{
	...
	strlcpy(msg->name, name, len - sizeof(*msg));
	msg->id = id;
	return reset_rpmsg_sendrecv(ept, RESET_RPMSG_RESET, (struct reset_rpmsg_header_s *)msg, len);
}
...
static int reset_rpmsg_assert(FAR struct reset_controller_dev *rcdev, unsigned int id)
{
	...
	strlcpy(msg->name, name, len - sizeof(*msg));
	msg->id = id;
	return reset_rpmsg_sendrecv(ept, RESET_RPMSG_ASSERT, (struct reset_rpmsg_header_s *)msg, len);
}
...
static int reset_rpmsg_deassert(FAR struct reset_controller_dev *rcdev, unsigned int id)
{
	...
	strlcpy(msg->name, name, len - sizeof(*msg));
	msg->id = id;
	return reset_rpmsg_sendrecv(ept, RESET_RPMSG_DEASSERT, (struct reset_rpmsg_header_s *)msg, len);
}
...
static int reset_rpmsg_status(FAR struct reset_controller_dev *rcdev, unsigned int id)
{
	...
	strlcpy(msg->name, name, len - sizeof(*msg));
	msg->id = id;
	return reset_rpmsg_sendrecv(ept, RESET_RPMSG_STATUS, (struct reset_rpmsg_header_s *)msg, len);
}

static FAR struct reset_control *reset_rpmsg_find(FAR struct rpmsg_endpoint *ept,
												  FAR const char *name, unsigned int id)
{
	...
	list_for_every_entry(&server->list, reset, struct reset_rpmsg_s, node)
	{
									   ...
									   uint32_t src, FAR void *priv)
{
	...
	reset->rstc = reset_control_get(msg->name, msg->id, msg->shared, msg->acquired);
	if (reset->rstc == NULL) {
		kmm_free(reset);
		nxmutex_unlock(&server->lock);
		goto out;
							  ...
							  uint32_t src, FAR void *priv)
{
	...
	struct reset_rpmsg_cookie_s *cookie = (struct reset_rpmsg_cookie_s *)(uintptr_t)header->cookie;

	if (cookie && header->response) {
		cookie->result = header->result;
		nxsem_post(&cookie->sem);
		ret = 0;
	} else if (cmd < nitems(g_reset_rpmsg_handler) && g_reset_rpmsg_handler[cmd]) {
		header->response = 1;
...
static bool reset_rpmsg_server_match(FAR struct rpmsg_device *rdev, FAR void *priv,
									 FAR const char *name, uint32_t dest)
{
	return strcmp(name, RESET_RPMSG_EPT_NAME) == 0;
}

static void reset_rpmsg_server_ept_release(FAR struct rpmsg_endpoint *ept, FAR void *priv)
{
	FAR struct reset_rpmsg_server_s *server = ept->priv;
	FAR struct reset_rpmsg_s *reset;
	FAR struct reset_rpmsg_s *tmp;

	list_for_every_entry_safe(&server->list, reset, tmp, struct reset_rpmsg_s, node)
	{
		reset_control_put(reset->rstc);
		list_delete(&reset->node);
		kmm_free(reset);
...
static void reset_rpmsg_server_bind(FAR struct rpmsg_device *rdev, FAR void *priv,
									FAR const char *name, uint32_t dest)
{
	...
	rpmsg_create_ept(&server->ept, rdev, name, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY, reset_rpmsg_ept_cb,
					 rpmsg_destroy_ept);
}
...
{
	return rpmsg_register_callback(NULL, NULL, NULL, reset_rpmsg_server_match,
