...
#include <roux/note/noteram_driver.h>
 ...
 *  In ROUX, PID number less than NCPUS are idle tasks.
 *  In Linux, there is only one idle task of PID 0.
 */

#define get_pid(pid) ((pid) < NCPUS ? 0 : (pid))

#define get_task_state(s) ((s) == 0 ? 'X' : ((s) <= LAST_READY_TO_RUN_STATE ? 'R' : 'S'))
...
static ssize_t noteram_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static int noteram_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int noteram_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
static void noteram_add(FAR struct note_driver_s *drv, FAR const void *note, size_t len);
...
static const struct note_driver_ops_s g_noteram_ops = {noteram_add};
...
static inline unsigned int noteram_next(FAR struct noteram_driver_s *drv, unsigned int ndx,
										unsigned int offset)
{
	ndx += offset;
...
static ssize_t noteram_get(FAR struct noteram_driver_s *drv, FAR uint8_t *buffer, size_t buflen)
{
...
{
	FAR struct noteram_dump_context_s *ctx;
	FAR struct noteram_driver_s *drv = (FAR struct noteram_driver_s *)filep->f_inode->i_private;
...
static ssize_t noteram_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (ctx->mode == NOTERAM_MODE_READ_BINARY) {
		...
		do {
			...
			ret = noteram_dump_one(note, (FAR struct lib_outstream_s *)&stream, ctx);
		} while (ret == 0);
	}
...
static void noteram_add(FAR struct note_driver_s *driver, FAR const void *note, size_t notelen)
{
...
static int noteram_dump_header(FAR struct lib_outstream_s *s, FAR struct note_common_s *note,
							   FAR struct noteram_dump_context_s *ctx)
{
	struct timespec ts;
	pid_t pid;
	...
	ret = lib_sprintf(s, "%8s-%-3u [%d] %3" PRIu64 ".%09lu: ", get_taskname(pid), get_pid(pid), cpu,
...
static int noteram_dump_sched_switch(FAR struct lib_outstream_s *s, FAR struct note_common_s *note,
									 FAR struct noteram_dump_context_s *ctx)
{
	...
	ret = lib_sprintf(s,
					  ...
					  get_taskname(current_pid), get_pid(current_pid), current_priority,
					  get_task_state(cctx->current_state), get_taskname(next_pid),
...
static int noteram_dump_printf(FAR struct lib_outstream_s *s, FAR struct note_printf_s *note)
{
	size_t ret = 0;
							...
							FAR struct noteram_dump_context_s *ctx)
{
	...
	case NOTE_RESUME: {
		...
		if (cctx->intr_nest == 0) {
			...
			ret += lib_sprintf(s,
							   "sched_waking: comm=%s "
							   "pid=%d target_cpu=%d\n",
							   get_taskname(cctx->next_pid), get_pid(cctx->next_pid), cpu);
	...
	case NOTE_SYSCALL_ENTER: {
		...
		if (nsc->nsc_nr < CONFIG_SYS_RESERVED || nsc->nsc_nr >= SYS_maxsyscall) {
			break;
		}

		ret += noteram_dump_header(s, note, ctx);
		ret += lib_sprintf(s, "sys_%s(", g_funcnames[nsc->nsc_nr - CONFIG_SYS_RESERVED]);
	...
	case NOTE_SYSCALL_LEAVE: {
		...
		ret += lib_sprintf(s, "sys_%s -> 0x%" PRIxPTR "\n",
						   g_funcnames[nsc->nsc_nr - CONFIG_SYS_RESERVED], result);
	} break;
#endif
	...
	case NOTE_WDOG_LEAVE: {
		...
		ret += lib_sprintf(s, "tracing_mark_write: I|%d|wdog: %s-%pS %p\n", pid,
						   name[note->nc_type - NOTE_WDOG_START], (FAR void *)nw->handler,
						   (FAR void *)nw->arg);
	} break;
#endif
	...
	case NOTE_DUMP_COUNTER: {
		...
		ret += noteram_dump_header(s, &nbi->nev_cmn, ctx);
		ret +=
			lib_sprintf(s, "tracing_mark_write: C|%d|%s|%ld\n", pid, counter->name, counter->value);
	} break;
#endif
	...
	case NOTE_HEAP_FREE: {
		...
		FAR const char *name[] = {"add", "remove", "malloc", "free"};

		ret += noteram_dump_header(s, &nmm->nhp_cmn, ctx);
		ret += lib_sprintf(s,
						   "tracing_mark_write: C|%d|Heap Usage|%d|%s"
						   ": heap: %p size:%" PRIiPTR ", address: %p\n",
						   pid, nmm->used, name[note->nc_type - NOTE_HEAP_ADD], nmm->heap,
						   nmm->size, nmm->mem);
	} break;
#endif
	default:
...
static int noteram_crash_dump(FAR struct notifier_block *nb, unsigned long action, FAR void *data)
{
...
#endif
	return register_driver("/dev/note/ram", &g_noteram_fops, 0666, &g_noteram_driver);
}
...
FAR struct note_driver_s *noteram_initialize(FAR const char *devpath, size_t bufsize,
											 bool overwrite)
{
	FAR struct noteram_driver_s *drv;
#ifdef CONFIG_SCHED_INSTRUMENTATION_FILTER
	...
	drv->driver.filter.mode.flag = CONFIG_SCHED_INSTRUMENTATION_FILTER_DEFAULT_MODE;

#ifdef CONFIG_SMP
	drv->driver.filter.mode.cpuset = CONFIG_SCHED_INSTRUMENTATION_CPUSET;
#endif
