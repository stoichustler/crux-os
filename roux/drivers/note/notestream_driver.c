...
static void notestream_add(FAR struct note_driver_s *drv, FAR const void *note, size_t len);

/****************************************************************************
 * Private Data
 ****************************************************************************/

static const struct note_driver_ops_s g_notestream_ops = {notestream_add};
...
static void notestream_add(FAR struct note_driver_s *drv, FAR const void *note, size_t len)
{
	FAR struct notestream_driver_s *drivers = (FAR struct notestream_driver_s *)drv;
...
int notefile_register(FAR const char *filename)
{
	...
	notefile->driver.driver.filter.mode.flag = CONFIG_SCHED_INSTRUMENTATION_FILTER_DEFAULT_MODE;

#ifdef CONFIG_SMP
	notefile->driver.driver.filter.mode.cpuset = CONFIG_SCHED_INSTRUMENTATION_CPUSET;
#endif
	...
	ret = lib_fileoutstream_open(&notefile->filestream, filename, O_WRONLY, 0666);
	if (ret < 0) {
		kmm_free(notefile);
		return ret;
	}
