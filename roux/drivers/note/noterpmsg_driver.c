...
static void noterpmsg_add(FAR struct note_driver_s *driver, FAR const void *note, size_t notelen);

/****************************************************************************
 * Private Data
 ****************************************************************************/

static const struct note_driver_ops_s g_noterpmsg_ops = {noterpmsg_add};
...
static inline size_t noterpmsg_next(FAR struct noterpmsg_driver_s *drv, size_t pos, size_t offset)
{
	pos += offset;
...
static bool noterpmsg_transfer(FAR struct noterpmsg_driver_s *drv, bool wait)
{
	for (;;) {
		FAR uint8_t *buffer;
		uint32_t space;
...
static void noterpmsg_work(FAR void *priv)
{
	...
	if (!noterpmsg_transfer(drv, false)) {
		work_queue(HPWORK, &drv->work, noterpmsg_work, drv, NOTE_RPMSG_WORK_DELAY);
	}
...
static void noterpmsg_add(FAR struct note_driver_s *driver, FAR const void *note, size_t notelen)
{
	FAR struct noterpmsg_driver_s *drv = (FAR struct noterpmsg_driver_s *)driver;
	irqstate_t flags;
	size_t space;
...
static int noterpmsg_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
							uint32_t src, FAR void *priv)
{
	return 0;
}

static void noterpmsg_device_created(FAR struct rpmsg_device *rdev, FAR void *priv)
{
	FAR struct noterpmsg_driver_s *drv = priv;
	int ret;

	if (strcmp(CONFIG_DRIVERS_NOTERPMSG_SERVER_NAME, rpmsg_get_cpuname(rdev)) == 0) {
		drv->ept.priv = drv;
		spin_lock_init(&drv->lock);

		ret = rpmsg_create_ept(&drv->ept, rdev, NOTERPMSG_EPT_NAME, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
...
static void noterpmsg_device_destroy(FAR struct rpmsg_device *rdev, FAR void *priv)
{
	FAR struct noterpmsg_driver_s *drv = priv;
...
{
	return rpmsg_register_callback(&g_noterpmsg_driver, noterpmsg_device_created,
								   noterpmsg_device_destroy, NULL, NULL);
}
