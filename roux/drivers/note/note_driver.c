...
#include <roux/note/notelog_driver.h>
#include <roux/note/notestream_driver.h>
 ...
 ****************************************************************************/
		...
		defined(CONFIG_SEGGER_SYSVIEW) >                                                           \
	CONFIG_DRIVERS_NOTE_MAX
#error "Maximum channel number exceeds. "
#endif

#define note_add(drv, note, notelen) ((drv)->ops->add(drv, note, notelen))
#define note_start(drv, tcb) ((drv)->ops->start && ((drv)->ops->start(drv, tcb), true))
#define note_stop(drv, tcb) ((drv)->ops->stop && ((drv)->ops->stop(drv, tcb), true))
#define note_suspend(drv, tcb) ((drv)->ops->suspend && ((drv)->ops->suspend(drv, tcb), true))
#define note_resume(drv, tcb) ((drv)->ops->resume && ((drv)->ops->resume(drv, tcb), true))
...
#define note_spinlock(drv, tcb, spinlock, type)                                                    \
	((drv)->ops->spinlock && ((drv)->ops->spinlock(drv, tcb, spinlock, type), true))
#define note_syscall_enter(drv, nr, argc, ap)                                                      \
	((drv)->ops->syscall_enter && ((drv)->ops->syscall_enter(drv, nr, argc, ap), true))
#define note_syscall_leave(drv, nr, result)                                                        \
	((drv)->ops->syscall_leave && ((drv)->ops->syscall_leave(drv, nr, result), true))
#define note_irqhandler(drv, irq, handler, enter)                                                  \
	((drv)->ops->irqhandler && ((drv)->ops->irqhandler(drv, irq, handler, enter), true))
...
FAR static struct note_driver_s *g_note_drivers[CONFIG_DRIVERS_NOTE_MAX + 1] = {
...
#if CONFIG_DRIVERS_NOTE_TASKNAME_BUFSIZE > 0 && defined(CONFIG_SCHED_INSTRUMENTATION_SWITCH)
...
static void note_common(FAR struct tcb_s *tcb, FAR struct note_common_s *note, uint8_t length,
						uint8_t type)
{
...
static inline int note_isenabled_syscall(FAR struct note_driver_s *driver, int nr)
{
#ifdef CONFIG_SCHED_INSTRUMENTATION_FILTER
	if (!note_isenabled(driver)) {
		return false;
	...
	if (!(driver->filter.mode.flag & NOTE_FILTER_MODE_FLAG_SYSCALL) ||
		NOTE_FILTER_SYSCALLMASK_ISSET(nr - CONFIG_SYS_RESERVED, &driver->filter.syscall_mask)) {
		return false;
	}
#endif
...
#ifdef CONFIG_SCHED_INSTRUMENTATION_IRQHANDLER
static inline int note_isenabled_irq(FAR struct note_driver_s *driver, int irq, bool enter)
{
#ifdef CONFIG_SCHED_INSTRUMENTATION_FILTER
	if (!note_isenabled(driver)) {
		return false;
...
static inline int note_isenabled_dump(FAR struct note_driver_s *driver, uint32_t tag)
{
#ifdef CONFIG_SCHED_INSTRUMENTATION_FILTER
	if (!note_isenabled(driver)) {
		return false;
...
static FAR struct note_taskname_info_s *note_find_taskname(pid_t pid)
{
	...
	while (n != g_note_taskname.head) {
		ti = (FAR struct note_taskname_info_s *)&g_note_taskname.buffer[n];
		if (ti->pid == pid) {
			return ti;
		}
...
static void note_record_taskname(pid_t pid, FAR const char *name)
{
	...
	if (g_note_taskname.head >= g_note_taskname.tail) {
		remain =
			CONFIG_DRIVERS_NOTE_TASKNAME_BUFSIZE - (g_note_taskname.head - g_note_taskname.tail);
	} else {
	...
	while (skiplen + tilen >= remain) {
		...
		ti = (FAR struct note_taskname_info_s *)&g_note_taskname.buffer[g_note_taskname.tail];
		g_note_taskname.tail =
			(g_note_taskname.tail + ti->size) % CONFIG_DRIVERS_NOTE_TASKNAME_BUFSIZE;
		remain += ti->size;
	}
	...
	if (skiplen) {
		...
		ti = (FAR struct note_taskname_info_s *)&g_note_taskname.buffer[g_note_taskname.head];
		ti->size = skiplen;
...
void sched_note_add(FAR const void *data, size_t len)
{
	...
	while (len >= sizeof(struct note_common_s)) {
		...
		DEBUGASSERT(notelen >= sizeof(struct note_common_s) && len >= notelen);
