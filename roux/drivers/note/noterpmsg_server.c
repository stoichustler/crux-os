...
static bool noterpmsg_ns_match(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							   uint32_t dest)
{
	return !strcmp(name, NOTERPMSG_EPT_NAME);
}

static void noterpmsg_ns_bind(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							  uint32_t dest)
{
	...
	ret = rpmsg_create_ept(&srv->ept, rdev, NOTERPMSG_EPT_NAME, RPMSG_ADDR_ANY, dest,
...
{
	return rpmsg_register_callback(NULL, NULL, NULL, noterpmsg_ns_match, noterpmsg_ns_bind);
}
