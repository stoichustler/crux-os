...
static ssize_t se05x_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t se05x_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static const FAR struct file_operations g_fops = {se05x_open, se05x_close, se05x_read, se05x_write,
...
static ssize_t se05x_read(FAR struct file *filep, char *buffer, size_t buflen)
{
	return -ENOSYS;
}

static ssize_t se05x_write(FAR struct file *filep, const char *buffer, size_t buflen)
{
	return -ENOSYS;
}
...
static int se05x_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SEIOC_DERIVE_SYMM_KEY: {
		FAR struct se05x_derive_key_s *derive_key_args = (FAR struct se05x_derive_key_s *)arg;
		ret = pnt_se05x_derive_key(priv, derive_key_args);
	} break;

	case SEIOC_CREATE_SIGNATURE: {
		FAR struct se05x_signature_s *create_signature_args = (FAR struct se05x_signature_s *)arg;
		ret = pnt_se05x_create_signature(priv, create_signature_args);
	} break;

	case SEIOC_VERIFY_SIGNATURE: {
		FAR struct se05x_signature_s *verify_signature_args = (FAR struct se05x_signature_s *)arg;
