...
static int se05x_i2c_write(FAR struct se05x_dev_s *priv, FAR const uint8_t *buffer, ssize_t buflen)
{
...
static int se05x_i2c_read(FAR struct se05x_dev_s *priv, FAR uint8_t *buffer, ssize_t buflen)
{
	struct i2c_msg_s msg;
	int ret;
...
unsigned int sm_i2c_write(FAR void *conn_ctx, unsigned char bus, unsigned char addr,
						  FAR unsigned char *tx, unsigned short tx_len)
{
	(void)bus;
...
unsigned int sm_i2c_read(FAR void *conn_ctx, uint8_t bus, uint8_t addr, FAR uint8_t *rx,
