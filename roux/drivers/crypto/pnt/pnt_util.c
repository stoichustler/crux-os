...
static smStatus_t t1oi2c_transceive(pSe05xSession_t session_ctx, FAR uint8_t *tx, size_t tx_len,
									FAR uint8_t *rx, size_t *rx_len)
{
	memcpy(session_ctx->apdu_buffer, tx, tx_len);
	smStatus_t status =
		smComT1oI2C_TransceiveRaw(session_ctx->conn_context, session_ctx->apdu_buffer, tx_len,
...
bool select_card_manager(pSe05xSession_t session_ctx)
{
	...
	smStatus_t status =
		t1oi2c_transceive(session_ctx, tx_buf, sizeof(tx_buf), response_data, &response_data_size);
...
bool se05x_identify(pSe05xSession_t session_ctx, FAR identify_rsp_t *identify_response)
{
	const uint8_t cmd[] = {
	...
	smStatus_t status =
		t1oi2c_transceive(session_ctx, (FAR uint8_t *)cmd, sizeof(cmd),
						  (FAR uint8_t *)identify_response, &identify_response_size);

	return (status == SM_OK) && (identify_response_size == sizeof(identify_rsp_t));
}
