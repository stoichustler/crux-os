...
static_assert((CONFIG_NET_LL_GUARDSIZE % 4) == 2, "CONFIG_NET_LL_GUARDSIZE - ETH_HDRLEN "
...
struct rndis_dev_s {
	...
	FAR struct usbdev_req_s
		*epintin_req;				/* Pointer to preallocated interrupt in endpoint request */
...
static int usbclass_setup(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev,
...
const static struct usbdevclass_driverops_s g_driverops = {
	&usbclass_bind, &usbclass_unbind, &usbclass_setup, &usbclass_disconnect, NULL, NULL};
...
static const struct usb_cfgdesc_s g_rndis_cfgdesc = {.len = USB_SIZEOF_CFGDESC,
...
static const struct usb_iaddesc_s g_rndis_assoc_desc = {.len = USB_SIZEOF_IADDESC,
...
static const struct usb_ifdesc_s g_rndis_comm_ifdesc = {.len = USB_SIZEOF_IFDESC,
	...
	.compdesc =
		{
			...
			.attr = 0,
			.wbytes[0] = LSBYTE((CONFIG_RNDIS_EPINTIN_MAXBURST + 1) * CONFIG_RNDIS_EPINTIN_HSSIZE),
...
static const struct usb_ifdesc_s g_rndis_data_ifdesc = {.len = USB_SIZEOF_IFDESC,
														.type = USB_DESC_TYPE_INTERFACE,
														.ifno = 1,
														.alt = 0,
														.neps = 2,
...
static uint8_t g_rndis_default_mac_addr[6] = {0x02, 0x00, 0x00, 0x11, 0x22, 0x33};
...
static int rndis_submit_rdreq(FAR struct rndis_dev_s *priv)
{
	...
	if (!priv->rdreq_submitted && !priv->rx_blocked) {
		...
		if (ret != OK) {
			usbtrace(TRACE_CLSERROR(USBSER_TRACEERR_RDSUBMIT), (uint16_t)-priv->rdreq->result);
		} else {
...
static void rndis_freewrreq(FAR struct rndis_dev_s *priv, FAR struct rndis_req_s *req)
{
...
static void rndis_iob2buf(FAR struct rndis_dev_s *priv, FAR struct rndis_req_s *req)
{
	uint16_t llhdrlen = NET_LL_HDRLEN(&priv->netdev);
	uint32_t offset = CONFIG_NET_LL_GUARDSIZE - llhdrlen - RNDIS_PACKET_HDR_SIZE;
	...
	if (req->iob->io_flink == NULL) {
		...
		req->req->buf = req->buf;
		iob_copyout(&req->req->buf[RNDIS_PACKET_HDR_SIZE], req->iob, req->iob->io_pktlen + llhdrlen,
					-llhdrlen);
		iob_free_chain(req->iob);
		req->iob = NULL;
	}
...
static uint16_t rndis_fillrequest(FAR struct rndis_dev_s *priv, FAR struct rndis_req_s *req)
{
	size_t datalen;

	req->req->len = 0;

	datalen = MIN(priv->netdev.d_len, CONFIG_RNDIS_BULKIN_REQLEN - RNDIS_PACKET_HDR_SIZE);
	if (datalen > 0) {
		...
		FAR struct rndis_packet_msg *msg = (FAR struct rndis_packet_msg *)req->req->buf;
...
static void rndis_rxdispatch(FAR void *arg)
{
	...
	hdr = (FAR struct eth_hdr_s *)&priv->netdev.d_iob
			  ->io_data[CONFIG_NET_LL_GUARDSIZE - NET_LL_HDRLEN(&priv->netdev)];
...
static inline int rndis_recvpacket(FAR struct rndis_dev_s *priv, FAR uint8_t *reqbuf,
								   uint16_t reqlen)
{
	...
	if (!priv->current_rx_datagram_size) {
		if (reqlen < 16) {
			...
			if (msg->msgtype == RNDIS_PACKET_MSG) {
				...
				if (priv->current_rx_datagram_offset < reqlen) {
					iob_trycopyin(priv->rx_req->iob, &reqbuf[priv->current_rx_datagram_offset],
		...
		if (priv->current_rx_received >= priv->current_rx_datagram_offset &&
			...
			if ((index + copysize) <= CONFIG_NET_ETH_PKTSIZE) {
				iob_trycopyin(priv->rx_req->iob, reqbuf, copysize, priv->rx_req->iob->io_pktlen,
							  false);
			} else {
...
static FAR void *rndis_prepare_response(FAR struct rndis_dev_s *priv, size_t size,
										FAR struct rndis_command_header *request_hdr)
{
	...
	FAR struct rndis_response_header *hdr = (FAR struct rndis_response_header *)buf;
...
{
	size_t size_words = size / sizeof(uint32_t);
	FAR struct rndis_notification *notif = (FAR struct rndis_notification *)priv->epintin_req->buf;
...
static int rndis_handle_control_message(FAR struct rndis_dev_s *priv, FAR uint8_t *dataout,
										uint16_t outlen)
{
	FAR struct rndis_command_header *cmd_hdr = (FAR struct rndis_command_header *)dataout;
	...
	case RNDIS_QUERY_MSG: {
		...
		FAR struct rndis_query_msg *req = (FAR struct rndis_query_msg *)dataout;
		...
		for (i = 0; i < sizeof(g_rndis_oid_values) / sizeof(g_rndis_oid_values[0]); i++) {
			...
			if (match) {
				...
				if (req->objid == RNDIS_OID_GEN_CURRENT_PACKET_FILTER) {
					...
					memcpy(resp->buffer, g_rndis_oid_values[i].data, resp->buflen);
				} else {
					memcpy(resp->buffer, &g_rndis_oid_values[i].value, resp->buflen);
				}
				break;
			}
		}

		uinfo("RNDIS Query RID=%08x OID=%08x LEN=%d DAT=%08x", (unsigned)req->hdr.reqid,
			  (unsigned)req->objid, (int)resp->buflen, (unsigned)resp->buffer[0]);
	...
	case RNDIS_SET_MSG: {
		...
		uinfo("RNDIS SET RID=%08x OID=%08x LEN=%d DAT=%08x", (unsigned)req->hdr.reqid,
			  (unsigned)req->objid, (int)req->buflen, (unsigned)req->buffer[0]);
...
static void rndis_rdcomplete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req)
{
...
static void rndis_wrcomplete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req)
{
	FAR struct rndis_dev_s *priv;
...
static void usbclass_epintin_complete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req)
{
	if (req->result || req->xfrd != req->len) {
		usbtrace(TRACE_CLSERROR(USBSER_TRACEERR_REQRESULT), (uint16_t)-req->result);
	}
...
static int16_t usbclass_mkcfgdesc(FAR uint8_t *buf, FAR struct usbdev_devinfo_s *devinfo,
								  uint8_t speed, uint8_t type)
{
	...
	ret = usbdev_copy_epdesc((struct usb_epdesc_s *)buf, epno, speed, &g_rndis_epintindesc);
	...
	ret = usbdev_copy_epdesc((struct usb_epdesc_s *)buf, epno, speed, &g_rndis_epbulkindesc);
	if (buf != NULL) {
		buf += ret;
	}
	...
	ret = usbdev_copy_epdesc((struct usb_epdesc_s *)buf, epno, speed, &g_rndis_epbulkoutdesc);
	if (buf != NULL) {
		buf += ret;
	}
...
static int usbclass_bind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev)
{
	...
	priv->epintin = DEV_ALLOCEP(dev, USB_EPIN(priv->devinfo.epno[RNDIS_EP_INTIN_IDX]), true,
	...
	priv->epintin_req = usbdev_allocreq(priv->epintin, sizeof(struct rndis_notification));
	...
	priv->epbulkin = DEV_ALLOCEP(dev, USB_EPIN(priv->devinfo.epno[RNDIS_EP_BULKIN_IDX]), true,
	...
	priv->epbulkout = DEV_ALLOCEP(dev, USB_EPOUT(priv->devinfo.epno[RNDIS_EP_BULKOUT_IDX]), false,
								  USB_EP_ATTR_XFER_BULK);
	if (!priv->epbulkout) {
	...
	if (dev->speed == USB_SPEED_SUPER || dev->speed == USB_SPEED_SUPER_PLUS) {
		if (CONFIG_RNDIS_EPBULKOUT_MAXBURST < USB_SS_BULK_EP_MAXBURST) {
			reqlen = CONFIG_RNDIS_EPBULKOUT_SSSIZE * (CONFIG_RNDIS_EPBULKOUT_MAXBURST + 1);
		} else {
	...
	if (CONFIG_IOB_BUFSIZE >= CONFIG_RNDIS_BULKIN_REQLEN) {
		...
		if (dev->speed == USB_SPEED_SUPER || dev->speed == USB_SPEED_SUPER_PLUS) {
			if (CONFIG_RNDIS_EPBULKIN_MAXBURST < USB_SS_BULK_EP_MAXBURST) {
				reqlen = CONFIG_RNDIS_EPBULKIN_SSSIZE * (CONFIG_RNDIS_EPBULKIN_MAXBURST + 1);
			} else {
				reqlen = CONFIG_RNDIS_EPBULKIN_SSSIZE * USB_SS_BULK_EP_MAXBURST;
			}
		} else
#endif
#if defined(CONFIG_USBDEV_DUALSPEED)
	...
	case USB_REQ_TYPE_STANDARD: {
		...
		case USB_REQ_GETDESCRIPTOR: {
			...
			case USB_DESC_TYPE_DEVICE: {
				ret = usbdev_copy_devdesc(ctrlreq->buf, &g_devdesc, dev->speed);
			} break;
#endif
			...
			case USB_DESC_TYPE_CONFIG: {
				ret = usbclass_mkcfgdesc(ctrlreq->buf, &priv->devinfo, dev->speed, ctrl->value[1]);
			} break;
#endif
			...
			case USB_DESC_TYPE_STRING: {
				...
				ret = usbclass_mkstrdesc(ctrl->value[0], (FAR struct usb_strdesc_s *)ctrlreq->buf);
			} break;
#endif
...
static int usbclass_setconfig(FAR struct rndis_dev_s *priv, uint8_t config)
{
	...
	usbdev_copy_epdesc(&epdesc.epdesc, priv->devinfo.epno[RNDIS_EP_INTIN_IDX], priv->usbdev->speed,
					   &g_rndis_epintindesc);
	...
	usbdev_copy_epdesc(&epdesc.epdesc, priv->devinfo.epno[RNDIS_EP_BULKIN_IDX], priv->usbdev->speed,
	...
	usbdev_copy_epdesc(&epdesc.epdesc, priv->devinfo.epno[RNDIS_EP_BULKOUT_IDX],
					   priv->usbdev->speed, &g_rndis_epbulkoutdesc);
...
static int usbclass_classobject(int minor, FAR struct usbdev_devinfo_s *devinfo,
