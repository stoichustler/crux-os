...
static int usbmsc_setup(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev,
...
static int usbmsc_bind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev)
{
	...
	priv->epbulkin =
		DEV_ALLOCEP(dev, USBMSC_MKEPBULKIN(&priv->devinfo), true, USB_EP_ATTR_XFER_BULK);
	if (!priv->epbulkin) {
	...
	priv->epbulkout =
		DEV_ALLOCEP(dev, USBMSC_MKEPBULKOUT(&priv->devinfo), false, USB_EP_ATTR_XFER_BULK);
	if (!priv->epbulkout) {
	...
	for (i = 0; i < CONFIG_USBMSC_NRDREQS; i++) {
		...
		if (dev->speed == USB_SPEED_SUPER || dev->speed == USB_SPEED_SUPER_PLUS) {
			reqcontainer->req = usbdev_allocreq(priv->epbulkout, USBMSC_SSBULKMAXPACKET *
																	 (USBMSC_SSBULKMAXBURST + 1));
		} else
#endif
		{
			reqcontainer->req = usbdev_allocreq(priv->epbulkout, CONFIG_USBMSC_BULKOUTREQLEN);
		}
...
#ifdef CONFIG_USBDEV_SUPERSPEED
		if (dev->speed == USB_SPEED_SUPER || dev->speed == USB_SPEED_SUPER_PLUS) {
			reqcontainer->req = usbdev_allocreq(priv->epbulkin, USBMSC_SSBULKMAXPACKET *
																	(USBMSC_SSBULKMAXBURST + 1));
		} else
#endif
		{
			reqcontainer->req = usbdev_allocreq(priv->epbulkin, CONFIG_USBMSC_BULKINREQLEN);
		}
	...
	if ((ctrl->type & USB_REQ_TYPE_MASK) == USB_REQ_TYPE_STANDARD) {
		...
		case USB_REQ_GETDESCRIPTOR: {
			...
			case USB_DESC_TYPE_DEVICE: {
				ret = usbdev_copy_devdesc(ctrlreq->buf, usbmsc_getdevdesc(), dev->speed);
			} break;
#endif
			...
			case USB_DESC_TYPE_CONFIG: {
				ret = usbmsc_mkcfgdesc(ctrlreq->buf, &priv->devinfo, dev->speed, ctrl->value[1]);
			} break;
#endif
			...
			case USB_DESC_TYPE_STRING: {
				...
				ret = usbmsc_mkstrdesc(ctrl->value[0], (FAR struct usb_strdesc_s *)ctrlreq->buf);
			} break;
#endif
		...
		case USB_REQ_SETINTERFACE: {
			if (ctrl->type == USB_REQ_RECIPIENT_INTERFACE) {
				if (priv->config == USBMSC_CONFIGID && index == USBMSC_INTERFACEID &&
...
int usbmsc_setconfig(FAR struct usbmsc_dev_s *priv, uint8_t config)
{
	...
	usbmsc_copy_epdesc(USBMSC_EPBULKIN, &epdesc.epdesc, &priv->devinfo, priv->usbdev->speed);
	...
	usbmsc_copy_epdesc(USBMSC_EPBULKOUT, &epdesc.epdesc, &priv->devinfo, priv->usbdev->speed);
				   ...
				   off_t startsector, size_t nsectors, bool readonly)
{
	...
	if (!priv->iobuffer) {
		...
		priv->iobuffer = kmm_malloc(geo.geo_sectorsize * CONFIG_USBMSC_NWRREQS);
#else
...
#endif
	int
	usbmsc_exportluns(FAR void *handle)
{
	FAR struct usbmsc_alloc_s *alloc = (FAR struct usbmsc_alloc_s *)handle;
	FAR struct usbmsc_dev_s *priv;
#ifndef CONFIG_USBMSC_COMPOSITE
	...
	ret = kthread_create("scsid", CONFIG_USBMSC_SCSI_PRIO, CONFIG_USBMSC_SCSI_STACKSIZE,
