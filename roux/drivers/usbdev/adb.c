...
static const struct usb_devdesc_s g_adb_devdesc = {
	...
	.vendor =									/* Vendor ID */
	{LSBYTE(CONFIG_USBADB_VENDORID), MSBYTE(CONFIG_USBADB_VENDORID)},
	.product = /* Product ID */
	{LSBYTE(CONFIG_USBADB_PRODUCTID), MSBYTE(CONFIG_USBADB_PRODUCTID)},
	.device = /* Device ID */
	{LSBYTE(USBADB_VERSIONNO), MSBYTE(USBADB_VERSIONNO)},
...
static const struct usb_cfgdesc_s g_adb_cfgdesc = {
	...
	.attr = USB_CONFIG_ATTR_ONE | USBADB_SELFPOWERED | USBADB_REMOTEWAKEUP, /* Attributes */
...
static const struct usb_ifdesc_s g_adb_ifdesc = {.len = USB_SIZEOF_IFDESC,
								  ...
								  uint8_t speed, uint8_t type)
{
	...
	ret = usbdev_copy_epdesc((FAR struct usb_epdesc_s *)epdesc, devinfo->epno[USBADB_EP_BULKIN_IDX],
							 speed, &g_adb_epbulkin);
	totallen += ret;
	epdesc += ret;

	ret = usbdev_copy_epdesc((FAR struct usb_epdesc_s *)epdesc,
							 devinfo->epno[USBADB_EP_BULKOUT_IDX], speed, &g_adb_epbulkout);
	totallen += ret;
	epdesc += ret;
...
FAR void *usbdev_adb_initialize(void)
{
	...
	devdesc.devinfo.epno[USBADB_EP_BULKIN_IDX] = USB_EPNO(CONFIG_USBADB_EPBULKIN);
	devdesc.devinfo.epno[USBADB_EP_BULKOUT_IDX] = USB_EPNO(CONFIG_USBADB_EPBULKOUT);
...
void usbdev_adb_get_composite_devdesc(FAR struct composite_devdesc_s *dev)
{
	...
	dev->mkconfdesc = usbclass_mkcfgdesc, dev->mkstrdesc = usbclass_mkstrdesc,
	dev->nconfigs = USBADB_NCONFIGS;
	dev->configid = 1;
#ifdef CONFIG_USBDEV_SUPERSPEED
	dev->cfgdescsize = sizeof(g_adb_ifdesc) + USB_SIZEOF_EPDESC * 2 + USB_SIZEOF_SS_EPCOMPDESC * 2;
#else
