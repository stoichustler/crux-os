...
static void cdcacm_copy_epcompdesc(enum cdcacm_epdesc_e epid,
								   FAR struct usb_ss_epcompdesc_s *epcompdesc)
{
	...
	case CDCACM_EPINTIN: /* Interrupt IN endpoint */
	{
		...
		epcompdesc->attr = 0;
		epcompdesc->wbytes[0] = LSBYTE((epcompdesc->mxburst + 1) * CONFIG_CDCACM_EPINTIN_SSSIZE);
		epcompdesc->wbytes[1] = MSBYTE((epcompdesc->mxburst + 1) * CONFIG_CDCACM_EPINTIN_SSSIZE);
	} break;
#endif
...
int cdcacm_copy_epdesc(enum cdcacm_epdesc_e epid, FAR struct usb_epdesc_s *epdesc,
					   FAR struct usbdev_devinfo_s *devinfo, uint8_t speed)
{
	...
	case CDCACM_EPINTIN: /* Interrupt IN endpoint */
	{
		...
		if (speed == USB_SPEED_SUPER || speed == USB_SPEED_SUPER_PLUS) {
			...
			epdesc++;
			cdcacm_copy_epcompdesc(epid, (FAR struct usb_ss_epcompdesc_s *)epdesc);
		} else
#endif
...
int16_t cdcacm_mkcfgdesc(FAR uint8_t *buf, FAR struct usbdev_devinfo_s *devinfo, uint8_t speed,
						 uint8_t type)
{
	int length = 0;
	int ret;
	...
	ret = cdcacm_copy_epdesc(CDCACM_EPINTIN, (FAR struct usb_epdesc_s *)buf, devinfo, speed);
	if (buf != NULL) {
		buf += ret;
	}
	...
	ret = cdcacm_copy_epdesc(CDCACM_EPBULKOUT, (FAR struct usb_epdesc_s *)buf, devinfo, speed);
	if (buf != NULL) {
		buf += ret;
	}
	...
	ret = cdcacm_copy_epdesc(CDCACM_EPBULKIN, (FAR struct usb_epdesc_s *)buf, devinfo, speed);
	if (buf != NULL) {
		buf += ret;
	}
