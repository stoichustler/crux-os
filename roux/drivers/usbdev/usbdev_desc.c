...
int usbdev_copy_devdesc(FAR void *dest, FAR const struct usb_devdesc_s *src, uint8_t speed)
{
	...
	FAR struct usb_devdesc_s *p_desc = (struct usb_devdesc_s *)dest;
#endif
	...
	if (speed >= USB_SPEED_SUPER) {
		p_desc->usb[0] = LSBYTE(0x0320), p_desc->usb[1] = MSBYTE(0x0320);
...
#ifdef CONFIG_USBDEV_SUPERSPEED
static void usbdev_copy_epcompdesc(FAR struct usb_ss_epcompdesc_s *epcompdesc,
								   FAR const struct usbdev_epinfo_s *epinfo)
{
	...
	if (transtpye == USB_EP_ATTR_XFER_BULK) {
		...
		if (epcompdesc->mxburst >= USB_SS_INT_EP_MAXBURST) {
			...
			epcompdesc->wbytes[0] = LSBYTE((epcompdesc->mxburst + 1) * epinfo->sssize);
			epcompdesc->wbytes[1] = MSBYTE((epcompdesc->mxburst + 1) * epinfo->sssize);
		}
					   ...
					   FAR const struct usbdev_epinfo_s *epinfo)
{
	...
	if (speed == USB_SPEED_SUPER || speed == USB_SPEED_SUPER_PLUS) {
		...
		epdesc++;
		usbdev_copy_epcompdesc((FAR struct usb_ss_epcompdesc_s *)epdesc, epinfo);
	} else
#endif
