...
static void usbmsc_dumpdata(FAR const char *msg, const uint8_t *buf, int buflen);
#else
...
static int usbmsc_modepage(FAR struct usbmsc_dev_s *priv, FAR uint8_t *buf, uint8_t pcpgcode,
						   FAR int *mdlen);
...
#if defined(CONFIG_DEBUG_INFO) && defined(CONFIG_DEBUG_USB)
static void usbmsc_dumpdata(FAR const char *msg, FAR const uint8_t *buf, int buflen)
{
	int i;
...
static inline int usbmsc_cmdread10(FAR struct usbmsc_dev_s *priv)
{
	FAR struct scsicmd_read10_s *read10 = (FAR struct scsicmd_read10_s *)priv->cdb;
	FAR struct usbmsc_lun_s *lun;
	int ret;
...
static inline int usbmsc_cmdwrite10(FAR struct usbmsc_dev_s *priv)
{
	FAR struct scsicmd_write10_s *write10 = (FAR struct scsicmd_write10_s *)priv->cdb;
	FAR struct usbmsc_lun_s *lun;
	int ret;
...
static inline int usbmsc_cmdmodeselect10(FAR struct usbmsc_dev_s *priv)
{
	...
	usbmsc_setupcmd(priv, SCSICMD_MODESELECT10_SIZEOF, USBMSC_FLAGS_DIRHOST2DEVICE);
...
static inline int usbmsc_cmdread12(FAR struct usbmsc_dev_s *priv)
{
	FAR struct scsicmd_read12_s *read12 = (FAR struct scsicmd_read12_s *)priv->cdb;
	FAR struct usbmsc_lun_s *lun;
	int ret;
...
static inline int usbmsc_cmdwrite12(FAR struct usbmsc_dev_s *priv)
{
	FAR struct scsicmd_write12_s *write12 = (FAR struct scsicmd_write12_s *)priv->cdb;
	FAR struct usbmsc_lun_s *lun;
	int ret;
...
static int usbmsc_idlestate(FAR struct usbmsc_dev_s *priv)
{
	...
	if (req->xfrd != USBMSC_CBW_SIZEOF || cbw->signature[0] != 'U' || cbw->signature[1] != 'S' ||
		cbw->signature[2] != 'B' || cbw->signature[3] != 'C') {
	...
	else if (cbw->lun >= priv->nluns || (cbw->flags & ~USBMSC_CBWFLAG_IN) != 0 || cbw->cdblen < 6 ||
	...
	while (priv->u.xfrlen > 0) {
		...
		while (priv->nreqbytes > 0 && priv->u.xfrlen > 0) {
			...
			if ((priv->nsectbytes >= lun->sectorsize * priv->u.xfrlen) ||
				...
				nwritten = USBMSC_DRVR_WRITE(lun, priv->iobuffer, priv->sector, nrbufs);
				if (nwritten < 0) {
...
int usbmsc_scsi_main(int argc, FAR char *argv[])
{
	...
	while ((eventset & USBMSC_EVENT_TERMINATEREQUEST) == 0) {
		...
		if ((eventset & (USBMSC_EVENT_DISCONNECT | USBMSC_EVENT_RESET | USBMSC_EVENT_CFGCHANGE |
			...
			if ((eventset &
				 (USBMSC_EVENT_RESET | USBMSC_EVENT_CFGCHANGE | USBMSC_EVENT_IFCHANGE)) != 0) {
