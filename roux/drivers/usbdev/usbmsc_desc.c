...
static const struct usb_devdesc_s g_devdesc = {
	...
	{								  /* vendor */
	 LSBYTE(CONFIG_USBMSC_VENDORID), MSBYTE(CONFIG_USBMSC_VENDORID)},
	{/* product */
	 LSBYTE(CONFIG_USBMSC_PRODUCTID), MSBYTE(CONFIG_USBMSC_PRODUCTID)},
	{/* device */
	 LSBYTE(CONFIG_USBMSC_VERSIONNO), MSBYTE(CONFIG_USBMSC_VERSIONNO)},
...
static void usbmsc_copy_epcompdesc(enum usbmsc_epdesc_e epid,
...
int usbmsc_copy_epdesc(enum usbmsc_epdesc_e epid, FAR struct usb_epdesc_s *epdesc,
					   FAR struct usbdev_devinfo_s *devinfo, uint8_t speed)
{
	...
	case USBMSC_EPBULKOUT: /* Bulk OUT endpoint */
	{
		...
		if (speed == USB_SPEED_SUPER || speed == USB_SPEED_SUPER_PLUS) {
			...
			epdesc++;
			usbmsc_copy_epcompdesc(epid, (FAR struct usb_ss_epcompdesc_s *)epdesc);
		} else
#endif
#ifdef CONFIG_USBDEV_DUALSPEED
...
int16_t usbmsc_mkcfgdesc(uint8_t *buf, FAR struct usbdev_devinfo_s *devinfo, uint8_t speed,
						 uint8_t type)
{
#ifndef CONFIG_USBMSC_COMPOSITE
	...
	ret = usbmsc_copy_epdesc(USBMSC_EPBULKIN, (FAR struct usb_epdesc_s *)buf, devinfo, speed);
	...
	ret = usbmsc_copy_epdesc(USBMSC_EPBULKOUT, (FAR struct usb_epdesc_s *)buf, devinfo, speed);
