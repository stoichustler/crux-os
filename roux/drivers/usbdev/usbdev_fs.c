...
static int usbdev_fs_classbind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);
static void usbdev_fs_classunbind(FAR struct usbdevclass_driver_s *driver,
								  FAR struct usbdev_s *dev);
static int usbdev_fs_classsetup(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev,
								FAR const struct usb_ctrlreq_s *ctrl, FAR uint8_t *dataout,
								size_t outlen);
static void usbdev_fs_classdisconnect(FAR struct usbdevclass_driver_s *driver,
									  FAR struct usbdev_s *dev);
...
static ssize_t usbdev_fs_read(FAR struct file *filep, FAR char *buffer, size_t len);
static ssize_t usbdev_fs_write(FAR struct file *filep, FAR const char *buffer, size_t len);
static int usbdev_fs_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static void usbdev_fs_notify(FAR struct usbdev_fs_ep_s *fs_ep, pollevent_t eventset)
{
...
static int usbdev_fs_submit_wrreq(FAR struct usbdev_ep_s *ep, FAR struct usbdev_fs_req_s *container,
								  uint16_t len)
{
...
static int usbdev_fs_submit_rdreq(FAR struct usbdev_ep_s *ep, FAR struct usbdev_fs_req_s *container)
{
	FAR struct usbdev_req_s *req = container->req;
...
static void usbdev_fs_rdcomplete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req)
{
...
static void usbdev_fs_wrcomplete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req)
{
	FAR struct usbdev_fs_req_s *container;
	FAR struct usbdev_fs_ep_s *fs_ep;
	irqstate_t flags;
	...
	case -ESHUTDOWN: /* Disconnection */
	{
		usbtrace(TRACE_CLSERROR(USBSER_TRACEERR_WRSHUTDOWN), sq_count(&fs_ep->reqq));
	} break;
...
static int usbdev_fs_blocking_io(FAR struct usbdev_fs_ep_s *fs_ep,
								 FAR usbdev_fs_waiter_sem_t **list, FAR struct sq_queue_s *queue)
{
	...
	if (fs_ep->unlinked && fs_ep->crefs == 0) {
		...
		if (do_free && fs->uninitialized) {
			FAR struct usbdev_fs_driver_s *alloc =
				container_of(fs, FAR struct usbdev_fs_driver_s, dev);
...
static ssize_t usbdev_fs_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	...
	if (sq_empty(queue)) {
		...
		do {
			...
			ret = usbdev_fs_blocking_io(fs_ep, &fs_ep->sems, queue);
			if (ret < 0) {
				return ret;
			}
	...
	while (!sq_empty(queue)) {
		...
		if (is_ep0) {
			...
			ctrl_container = container_of(sq_peek(queue), struct usbdev_ctrlreq_s, node);
		...
		container = container_of(sq_peek(queue), struct usbdev_fs_req_s, node);
		...
		if (reqlen > len) {
			...
			if (buffer != NULL) {
				memcpy(&buffer[retlen], &container->req->buf[container->offset], len);
			}
		...
		if (buffer != NULL) {
			memcpy(&buffer[retlen], &container->req->buf[container->offset], reqlen);
		}
...
static ssize_t usbdev_fs_write(FAR struct file *filep, FAR const char *buffer, size_t len)
{
	...
	if (sq_empty(&fs_ep->reqq)) {
		...
		do {
			...
			ret = usbdev_fs_blocking_io(fs_ep, &fs_ep->sems, &fs_ep->reqq);
			if (ret < 0) {
				return ret;
			}
	...
	while (!sq_empty(&fs_ep->reqq)) {
		...
		container = container_of(sq_remfirst(&fs_ep->reqq), struct usbdev_fs_req_s, node);
							 ...
							 FAR struct usbdev_fs_ep_s *fs_ep)
{
	...
	if (epno != 0) {
		fs_ep->ep = DEV_ALLOCEP(dev, epno, USB_ISEPIN(epinfo->desc.addr),
	...
	fs_ep->reqbuffer = kmm_zalloc(sizeof(struct usbdev_fs_req_s) * epinfo->reqnum);
	if (!fs_ep->reqbuffer) {
	...
	if (epno == 0) {
		fs_ep->ctrlreqbuffer = kmm_zalloc(sizeof(struct usbdev_ctrlreq_s) * epinfo->reqnum);
...
static void usbdev_fs_ep_unbind(FAR const char *devname, FAR struct usbdev_s *dev,
								FAR const struct usbdev_epinfo_s *epinfo,
								FAR struct usbdev_fs_ep_s *fs_ep)
{
	uint16_t i;
	...
	if (fs_ep->reqbuffer) {
		for (i = 0; i < epinfo->reqnum; i++) {
			FAR struct usbdev_fs_req_s *container = &fs_ep->reqbuffer[i];
			if (container->req) {
...
static void usbdev_fs_register_driver(FAR void *arg)
{
	...
	for (i = 0; i < devinfo->nendpoints + 1; i++) {
		snprintf(devname, sizeof(devname), "%s/ep%d", devinfo->name, i);
...
static int usbdev_fs_classsetconfig(FAR struct usbdev_fs_dev_s *fs, uint8_t config)
{
	FAR struct usbdev_devinfo_s *devinfo = &fs->devinfo;
	...
	for (i = 0; i < devinfo->nendpoints; i++) {
		...
		usbdev_copy_epdesc(&epdesc.epdesc, devinfo->epno[i], fs->cdev->usbdev->speed,
						   devinfo->epinfos[i]);
		ret = EP_CONFIGURE(fs_ep->ep, &epdesc.epdesc, (i == (devinfo->nendpoints - 1)));
		if (ret < 0) {
...
static int usbdev_fs_classbind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev)
{
	FAR struct usbdev_fs_driver_s *fs_drvr =
		container_of(driver, FAR struct usbdev_fs_driver_s, drvr);
	...
	fs->eps = kmm_zalloc((devinfo->nendpoints + 1) * sizeof(struct usbdev_fs_ep_s));
	if (fs->eps == NULL) {
	...
	for (i = 0; i < devinfo->nendpoints; i++) {
		fs->eps[i + 1].dev = fs;
		ret = usbdev_fs_ep_bind(dev, devinfo->epno[i], devinfo->epinfos[i], &fs->eps[i + 1]);
		if (ret < 0) {
...
static void usbdev_fs_classunbind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev)
{
	FAR struct usbdev_fs_driver_s *fs_drvr =
		container_of(driver, FAR struct usbdev_fs_driver_s, drvr);
	FAR struct usbdev_fs_dev_s *fs = &fs_drvr->dev;
								...
								size_t outlen)
{
	...
	if ((ctrl->type & USB_REQ_TYPE_MASK) == USB_REQ_TYPE_STANDARD &&
		ctrl->req == USB_REQ_SETCONFIGURATION && ctrl->type == 0) {
		...
		if (!sq_empty(&ep0->ctrlreqq_free)) {
			...
			container =
				container_of(sq_remfirst(&ep0->ctrlreqq_free), struct usbdev_ctrlreq_s, node);
...
static void usbdev_fs_classresume(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev)
{
	FAR struct usbdev_fs_driver_s *fs_drvr =
		container_of(driver, FAR struct usbdev_fs_driver_s, drvr);
	FAR struct usbdev_fs_dev_s *fs = &fs_drvr->dev;
...
int usbdev_fs_classobject(int minor, FAR struct usbdev_devinfo_s *devinfo,
...
{
	FAR struct usbdev_fs_driver_s *alloc =
		container_of(classdev, FAR struct usbdev_fs_driver_s, drvr);
