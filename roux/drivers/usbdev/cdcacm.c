...
static int cdcacm_epconfigure(FAR struct usbdev_ep_s *ep, enum cdcacm_epdesc_e epid, bool last,
...
static int cdcacm_setup(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev,
...
static ssize_t cdcuart_recvbuf(FAR struct uart_dev_s *dev, FAR void *buf, size_t len);
static bool cdcuart_txready(FAR struct uart_dev_s *dev);
static ssize_t cdcuart_sendbuf(FAR struct uart_dev_s *dev, FAR const void *buf, size_t len);
...
static int cdcacm_sndpacket(FAR struct cdcacm_dev_s *priv)
{
	...
	if (!sq_empty(&priv->txfree)) {
		priv->wrcontainer = (FAR struct cdcacm_wrreq_s *)sq_remfirst(&priv->txfree);
...
static int cdcacm_release_rxpending(FAR struct cdcacm_dev_s *priv)
{
	...
	if (!sq_empty(&priv->rxpending)) {
		wd_start(&priv->rxfailsafe, CDCACM_RXDELAY, cdcacm_rxtimeout, (wdparm_t)priv);
	}
...
static int cdcacm_serialstate(FAR struct cdcacm_dev_s *priv)
{
	...
	notify->type = (USB_REQ_DIR_IN | USB_REQ_TYPE_CLASS | USB_REQ_RECIPIENT_INTERFACE);
...
static int cdcacm_setconfig(FAR struct cdcacm_dev_s *priv, uint8_t config)
{
	...
	ret = cdcacm_epconfigure(priv->epintin, CDCACM_EPINTIN, false, &priv->devinfo,
							 priv->usbdev->speed);
	...
	ret = cdcacm_epconfigure(priv->epbulkin, CDCACM_EPBULKIN, false, &priv->devinfo,
							 priv->usbdev->speed);
	...
	ret = cdcacm_epconfigure(priv->epbulkout, CDCACM_EPBULKOUT, true, &priv->devinfo,
							 priv->usbdev->speed);
...
static int cdcacm_bind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev)
{
	...
	priv->epintin = DEV_ALLOCEP(dev, CDCACM_MKEPINTIN(&priv->devinfo), true, USB_EP_ATTR_XFER_INT);
	if (!priv->epintin) {
	...
	priv->epbulkin =
		DEV_ALLOCEP(dev, CDCACM_MKEPBULKIN(&priv->devinfo), true, USB_EP_ATTR_XFER_BULK);
	if (!priv->epbulkin) {
	...
	priv->epbulkout =
		DEV_ALLOCEP(dev, CDCACM_MKEPBULKOUT(&priv->devinfo), false, USB_EP_ATTR_XFER_BULK);
	if (!priv->epbulkout) {
	...
	if (dev->speed == USB_SPEED_SUPER || dev->speed == USB_SPEED_SUPER_PLUS) {
		if (CONFIG_CDCACM_EPBULKOUT_MAXBURST < USB_SS_BULK_EP_MAXBURST) {
			reqlen = CONFIG_CDCACM_EPBULKOUT_SSSIZE * (CONFIG_CDCACM_EPBULKOUT_MAXBURST + 1);
		} else {
			reqlen = CONFIG_CDCACM_EPBULKOUT_SSSIZE * USB_SS_BULK_EP_MAXBURST;
		}
	} else
#endif
	...
	if (dev->speed == USB_SPEED_SUPER || dev->speed == USB_SPEED_SUPER_PLUS) {
		if (CONFIG_CDCACM_EPBULKIN_MAXBURST < USB_SS_BULK_EP_MAXBURST) {
			reqlen = CONFIG_CDCACM_EPBULKOUT_SSSIZE * (CONFIG_CDCACM_EPBULKIN_MAXBURST + 1);
		} else {
			reqlen = CONFIG_CDCACM_EPBULKOUT_SSSIZE * USB_SS_BULK_EP_MAXBURST;
		}
	} else
	...
	if ((ctrl->type & USB_REQ_TYPE_MASK) == USB_REQ_TYPE_STANDARD) {
		...
		case USB_REQ_GETDESCRIPTOR: {
			...
			case USB_DESC_TYPE_DEVICE: {
				ret = usbdev_copy_devdesc(ctrlreq->buf, cdcacm_getdevdesc(), dev->speed);
			} break;
#endif
			...
			case USB_DESC_TYPE_CONFIG: {
				ret = cdcacm_mkcfgdesc(ctrlreq->buf, &priv->devinfo, dev->speed, ctrl->value[1]);
			} break;
#endif
			...
			case USB_DESC_TYPE_STRING: {
				...
				ret = cdcacm_mkstrdesc(ctrl->value[0], (FAR struct usb_strdesc_s *)ctrlreq->buf);
			} break;
#endif
		...
		case USB_REQ_SETINTERFACE: {
			if (ctrl->type == USB_REQ_RECIPIENT_INTERFACE && priv->config == CDCACM_CONFIGID) {
				if ((index == priv->devinfo.ifnobase && value == CDCACM_NOTALTIFID) ||
					(index == (priv->devinfo.ifnobase + 1) && value == CDCACM_DATAALTIFID)) {
					cdcacm_resetconfig(priv);
...
static int cdcuart_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case TCFLSH: {
		...
		if (arg == TCIFLUSH || arg == TCIOFLUSH) {
			...
			if (priv->rdcontainer) {
				sq_addlast((FAR sq_entry_t *)priv->rdcontainer, &priv->rxpending);
				priv->rdcontainer = NULL;
			}

			while (!sq_empty(&priv->rxpending)) {
				rdcontainer = (FAR struct cdcacm_rdreq_s *)sq_remfirst(&priv->rxpending);
				ret = cdcacm_requeue_rdrequest(priv, rdcontainer);
			}
		...
		if (arg == TCOFLUSH || arg == TCIOFLUSH) {
			...
			if (priv->wrcontainer) {
				...
				priv->wrcontainer = (FAR struct cdcacm_wrreq_s *)sq_remfirst(&priv->txfree);
				serdev->xmit.buffer = (FAR char *)priv->wrcontainer->req->buf;
				priv->nwrq--;
				serdev->xmit.head = 0;
				serdev->xmit.tail = 0;
	...
	case CAIOC_REGISTERCB: {
		 ...
		 *   See include/roux/usb/cdc.h for structure definition.  This IOCTL
	...
	case CAIOC_GETLINECODING: {
		 ...
		 *   include/roux/usb/cdc.h for bit definitions.  This IOCTL should be
	...
	case CAIOC_GETCTRLLINE: {
		 ...
		 *   Bitmap Values" and CDC_UART_definitions in include/roux/usb/cdc.h.
		 */
	...
	case TCGETS: {
		...
		termiosp->c_cflag = ((priv->linecoding.parity != CDC_PARITY_NONE) ? PARENB : 0) |
							...
							((priv->linecoding.stop == CDC_CHFMT_STOP2) ? CSTOPB : 0) | CS8;
...
static void cdcacm_rcvpacket(FAR struct cdcacm_dev_s *priv)
{
	...
	if (!priv->rdcontainer && !sq_empty(&priv->rxpending)) {
		priv->rdcontainer = (FAR struct cdcacm_rdreq_s *)sq_remfirst(&priv->rxpending);
...
ssize_t cdcacm_write(FAR const char *buffer, size_t buflen)
{
	...
	while (len < buflen) {
		...
		if (cdcuart_txready(&priv->serdev)) {
			ssize_t ret = cdcuart_sendbuf(&priv->serdev, buffer + len, buflen - len);
			if (ret < 0) {
				leave_critical_section(flags);
				return ret;
			}
...
#endif
	int
	cdcacm_classobject(int minor, FAR struct usbdev_devinfo_s *devinfo,
