...
struct cdcecm_driver_s {
	...
	uint16_t pktbuf[(CONFIG_NET_ETH_PKTSIZE + CONFIG_NET_GUARDSIZE + 1) / 2];
...
static int cdcecm_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
...
static int cdcecm_bind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);

static void cdcecm_unbind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);

static int cdcecm_setup(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev,
						FAR const struct usb_ctrlreq_s *ctrl, FAR uint8_t *dataout, size_t outlen);

static void cdcecm_disconnect(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);

/* USB Device Class helpers */

static void cdcecm_ep0incomplete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req);
static void cdcecm_rdcomplete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req);
static void cdcecm_wrcomplete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req);

static int cdcecm_mkepdesc(int epidx, FAR struct usb_epdesc_s *epdesc,
...
static const struct usbdevclass_driverops_s g_usbdevops = {
	cdcecm_bind, cdcecm_unbind, cdcecm_setup, cdcecm_disconnect, NULL, NULL};
...
static const struct usb_devdesc_s g_devdesc = {
	...
	{LSBYTE(CONFIG_CDCECM_VENDORID), MSBYTE(CONFIG_CDCECM_VENDORID)},
	{LSBYTE(CONFIG_CDCECM_PRODUCTID), MSBYTE(CONFIG_CDCECM_PRODUCTID)},
...
static int cdcecm_txpoll(FAR struct net_driver_s *dev)
{
	FAR struct cdcecm_driver_s *priv = (FAR struct cdcecm_driver_s *)dev->d_private;
...
static int cdcecm_setconfig(FAR struct cdcecm_driver_s *self, uint8_t config)
{
	...
	cdcecm_mkepdesc(CDCECM_EP_INTIN_IDX, &epdesc.epdesc, &self->devinfo, self->usbdev.speed);
	...
	cdcecm_mkepdesc(CDCECM_EP_BULKIN_IDX, &epdesc.epdesc, &self->devinfo, self->usbdev.speed);
	...
	cdcecm_mkepdesc(CDCECM_EP_BULKOUT_IDX, &epdesc.epdesc, &self->devinfo, self->usbdev.speed);
	...
	self->rdreq->callback = cdcecm_rdcomplete, ret = EP_SUBMIT(self->epbulkout, self->rdreq);
	if (ret != OK) {
	...
	memcpy(self->dev.d_mac.ether.ether_addr_octet, "\x00\xe0\xde\xad\xbe\xef", IFHWADDRLEN);
...
static int cdcecm_setinterface(FAR struct cdcecm_driver_s *self, uint16_t interface,
							   uint16_t altsetting)
{
...
static void cdcecm_mkepcompdesc(int epidx, FAR struct usb_ss_epcompdesc_s *epcompdesc)
{
	switch (epidx) {
	case CDCECM_EP_INTIN_IDX: /* Interrupt IN endpoint */
	{
		...
		epcompdesc->attr = 0;
		epcompdesc->wbytes[0] = LSBYTE((epcompdesc->mxburst + 1) * CONFIG_CDCECM_EPINTIN_SSSIZE);
		epcompdesc->wbytes[1] = MSBYTE((epcompdesc->mxburst + 1) * CONFIG_CDCECM_EPINTIN_SSSIZE);
	} break;
...
static int16_t cdcecm_mkcfgdesc(FAR uint8_t *desc, FAR struct usbdev_devinfo_s *devinfo,
								uint8_t speed, uint8_t type)
{
	...
	ret = cdcecm_mkepdesc(CDCECM_EP_INTIN_IDX, (FAR struct usb_epdesc_s *)desc, devinfo, speed);
	if (desc) {
		desc += ret;
	}
	...
	ret = cdcecm_mkepdesc(CDCECM_EP_BULKIN_IDX, (FAR struct usb_epdesc_s *)desc, devinfo, speed);
	if (desc) {
		desc += ret;
	}
	...
	ret = cdcecm_mkepdesc(CDCECM_EP_BULKOUT_IDX, (FAR struct usb_epdesc_s *)desc, devinfo, speed);
	if (desc) {
		desc += ret;
	}
...
static int cdcecm_getdescriptor(FAR struct cdcecm_driver_s *self, uint8_t type, uint8_t index,
								FAR void *desc)
{
	...
	case USB_DESC_TYPE_CONFIG: {
		return cdcecm_mkcfgdesc((FAR uint8_t *)desc, &self->devinfo, self->usbdev.speed, type);
	} break;
...
static int cdcecm_bind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev)
{
	...
	self->epint = DEV_ALLOCEP(dev, USB_DIR_IN | self->devinfo.epno[CDCECM_EP_INTIN_IDX], true,
							  USB_EP_ATTR_XFER_INT);
	self->epbulkin = DEV_ALLOCEP(dev, USB_DIR_IN | self->devinfo.epno[CDCECM_EP_BULKIN_IDX], true,
	...
	self->rdreq = usbdev_allocreq(self->epbulkout, CONFIG_NET_ETH_PKTSIZE + CONFIG_NET_GUARDSIZE);
	if (self->rdreq == NULL) {
		uerr("Out of memory\n");
		ret = -ENOMEM;
		goto error;
	...
	self->wrreq = usbdev_allocreq(self->epbulkin, CONFIG_NET_ETH_PKTSIZE + CONFIG_NET_GUARDSIZE);
	if (self->wrreq == NULL) {
		uerr("Out of memory\n");
		ret = -ENOMEM;
		goto error;
