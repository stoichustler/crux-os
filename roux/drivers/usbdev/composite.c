...
static int composite_classsetup(FAR struct composite_dev_s *priv, FAR struct usbdev_s *dev,
...
static int composite_setup(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev,
						   FAR const struct usb_ctrlreq_s *ctrl, FAR uint8_t *dataout,
						   size_t outlen);
								...
								size_t outlen)
{
	...
	for (i = 0; i < priv->ndevices; i++) {
		if (interface >= priv->device[i].compdesc.devinfo.ifnobase &&
			...
			return CLASS_SETUP(priv->device[i].dev, dev, ctrl, dataout, outlen);
		}
	}
...
static int composite_msftdescriptor(FAR struct composite_dev_s *priv, FAR struct usbdev_s *dev,
									FAR const struct usb_ctrlreq_s *ctrl,
									FAR struct usbdev_req_s *ctrl_rsp, FAR bool *dispatched)
{
	if (ctrl->index[0] == MSFTOSDESC_INDEX_FUNCTION) {
		...
		if (response->count > 0) {
			size_t total_len = sizeof(struct usb_msft_os_feature_desc_s) +
							   (response->count - 1) * sizeof(struct usb_msft_os_function_desc_s);
...
static int16_t composite_mkcfgdesc(FAR struct usbdevclass_driver_s *driver, FAR uint8_t *buf,
								   uint8_t speed, uint8_t type)
{
	...
	for (i = 0; i < priv->ndevices; i++) {
		...
		len = devobj->compdesc.mkconfdesc(buf, &devobj->compdesc.devinfo, speed, type);
		total += len;
		buf += len;
	}
...
static int composite_mkstrdesc(FAR struct usbdevclass_driver_s *driver, uint8_t id,
							   FAR struct usb_strdesc_s *outdesc)
{
	FAR struct composite_dev_s *priv = ((FAR struct composite_driver_s *)driver)->dev;
	...
	if (id == USB_REQ_GETMSFTOSDESCRIPTOR) {
		...
		static const uint8_t msft_response[16] = {
			'M', 0, 'S', 0, 'F', 0, 'T', 0, '1', 0, '0', 0, '0', 0, USB_REQ_GETMSFTOSDESCRIPTOR, 0};
	...
	for (strdesc = strdescs->strdesc; strdesc != NULL && strdesc->string != NULL; strdesc++) {
		if (strdesc->id == id) {
	...
	for (i = 0; i < priv->ndevices; i++) {
		if (id > priv->device[i].compdesc.devinfo.strbase &&
			id <= priv->device[i].compdesc.devinfo.strbase +
					  priv->device[i].compdesc.devinfo.nstrings) {
			return priv->device[i].compdesc.mkstrdesc(id - priv->device[i].compdesc.devinfo.strbase,
													  outdesc);
		}
	}
	...
	if ((ctrl->type & USB_REQ_TYPE_MASK) == USB_REQ_TYPE_STANDARD &&
		...
		case USB_REQ_GETDESCRIPTOR: {
			...
			case USB_DESC_TYPE_DEVICE: {
				ret = usbdev_copy_devdesc(ctrlreq->buf, priv->descs->devdesc, dev->speed);

#ifdef CONFIG_BOARD_USBDEV_PIDVID
				{
					uint16_t pid = board_usbdev_pid();
					uint16_t vid = board_usbdev_vid();
					FAR struct usb_devdesc_s *p_desc = (FAR struct usb_devdesc_s *)ctrlreq->buf;
			...
			case USB_DESC_TYPE_CONFIG: {
				ret = composite_mkcfgdesc(driver, ctrlreq->buf, dev->speed, ctrl->value[1]);
			} break;
...
#endif
	else if (recipient == USB_REQ_RECIPIENT_INTERFACE || recipient == USB_REQ_RECIPIENT_ENDPOINT) {
		/**********************************************************************
...
FAR void *composite_initialize(FAR const struct usbdev_devdescs_s *devdescs,
							   FAR struct composite_devdesc_s *pdevices, uint8_t ndevices)
{
	FAR const struct usbdev_strdesc_s *strdesc;
	...
	for (i = 0; i < ndevices; i++) {
		...
		ret = devobj->compdesc.classobject(devobj->compdesc.minor, &devobj->compdesc.devinfo,
										   &devobj->dev);
		if (ret < 0) {
...
int composite_ep0submit(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev,
						FAR struct usbdev_req_s *ctrlreq, FAR const struct usb_ctrlreq_s *ctrl)
{
	bool ep0submit = true;
