...
static const struct usb_devdesc_s g_mtp_devdesc = {
	...
	.vendor =									/* Vendor ID */
	{LSBYTE(CONFIG_USBMTP_VENDORID), MSBYTE(CONFIG_USBMTP_VENDORID)},
	.product = /* Product ID */
	{LSBYTE(CONFIG_USBMTP_PRODUCTID), MSBYTE(CONFIG_USBMTP_PRODUCTID)},
	.device = /* Device ID */
	{LSBYTE(USBMTP_VERSIONNO), MSBYTE(USBMTP_VERSIONNO)},
...
static const struct usb_cfgdesc_s g_mtp_cfgdesc = {
	...
	.attr = USB_CONFIG_ATTR_ONE | USBMTP_SELFPOWERED | USBMTP_REMOTEWAKEUP, /* Attributes */
...
static const struct usb_ifdesc_s g_mtp_ifdesc = {.len = USB_SIZEOF_IFDESC,
	...
	.desc =
		{
			...
			.addr = USB_DIR_IN,
			.attr = USB_EP_ATTR_XFER_BULK | USB_EP_ATTR_NO_SYNC | USB_EP_ATTR_USAGE_DATA,
			.interval = 0,
		},
	...
	.desc =
		{
			...
			.type = USB_DESC_TYPE_ENDPOINT,
			.addr = USB_DIR_IN,
			.attr = USB_EP_ATTR_XFER_INT | USB_EP_ATTR_NO_SYNC | USB_EP_ATTR_USAGE_DATA,
	...
	.compdesc =
		{
			...
			.attr = 0,
			.wbytes[0] = LSBYTE((CONFIG_USBMTP_EPINTIN_MAXBURST + 1) * CONFIG_USBMTP_EPINTIN_SIZE),
			.wbytes[1] = MSBYTE((CONFIG_USBMTP_EPINTIN_MAXBURST + 1) * CONFIG_USBMTP_EPINTIN_SIZE),
		},
#endif
};
								  ...
								  uint8_t speed, uint8_t type)
{
	...
	ret = usbdev_copy_epdesc((FAR struct usb_epdesc_s *)epdesc, devinfo->epno[USBMTP_EP_BULKIN_IDX],
							 speed, &g_mtp_epbulkin);
	totallen += ret;
	epdesc += ret;

	ret = usbdev_copy_epdesc((FAR struct usb_epdesc_s *)epdesc,
							 devinfo->epno[USBMTP_EP_BULKOUT_IDX], speed, &g_mtp_epbulkout);
	totallen += ret;
	epdesc += ret;

	ret = usbdev_copy_epdesc((FAR struct usb_epdesc_s *)epdesc, devinfo->epno[USBMTP_EP_INTIN_IDX],
							 speed, &g_mtp_epintin);
	totallen += ret;
	epdesc += ret;
...
FAR void *usbdev_mtp_initialize(void)
{
	...
	devdesc.devinfo.epno[USBMTP_EP_BULKIN_IDX] = USB_EPNO(CONFIG_USBMTP_EPBULKIN);
	devdesc.devinfo.epno[USBMTP_EP_BULKOUT_IDX] = USB_EPNO(CONFIG_USBMTP_EPBULKOUT);
	devdesc.devinfo.epno[USBMTP_EP_INTIN_IDX] = USB_EPNO(CONFIG_USBMTP_EPINTIN);
...
void usbdev_mtp_get_composite_devdesc(FAR struct composite_devdesc_s *dev)
{
	...
	dev->configid = 1;
#ifdef CONFIG_USBDEV_SUPERSPEED
	dev->cfgdescsize = sizeof(g_mtp_ifdesc) + USB_SIZEOF_EPDESC * 3 + USB_SIZEOF_SS_EPCOMPDESC * 3;
#else
