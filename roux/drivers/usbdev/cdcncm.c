...
#include <roux/net/netdev_lowerhalf.h>
#include <roux/usb/usbdev.h>
#include <roux/usb/cdc.h>
#include <roux/usb/cdcncm.h>
#include <roux/usb/usbdev_trace.h>
...
#define FORMATS_SUPPORTED (CDC_NCM_NTB16_SUPPORTED | CDC_NCM_NTB32_SUPPORTED)
...
#define INIT_NDP16_OPTS                                                                            \
	{                                                                                              \
		.nthsign = CDC_NCM_NTH16_SIGN, .ndpsign = CDC_NCM_NDP16_NOCRC_SIGN,                        \
		.nthsize = sizeof(struct cdc_ncm_nth16_s), .ndpsize = sizeof(struct usb_cdc_ncm_ndp16_s),  \
		.dpesize = sizeof(struct usb_cdc_ncm_dpe16_s), .ndpalign = 4, .dgramitemlen = 2,           \
		.blocklen = 2, .ndpindex = 2, .reserved1 = 0, .reserved2 = 0, .nextndpindex = 2,           \
	}

#define INIT_NDP32_OPTS                                                                            \
	{                                                                                              \
		.nthsign = CDC_NCM_NTH32_SIGN, .ndpsign = CDC_NCM_NDP32_NOCRC_SIGN,                        \
		.nthsize = sizeof(struct cdc_ncm_nth32_s), .ndpsize = sizeof(struct usb_cdc_ncm_ndp32_s),  \
		.dpesize = sizeof(struct usb_cdc_ncm_dpe32_s), .ndpalign = 8, .dgramitemlen = 4,           \
		.blocklen = 4, .ndpindex = 4, .reserved1 = 2, .reserved2 = 4, .nextndpindex = 4,           \
	}
 ...
 * +------------+       +------------+
 *
 * The layout of the NTB(NCM Transfer Block) structure in the ROUX system
 * is as follows:
 * +--------------------------+
 * |NTH :       nth sign      |
 * |            nth len       |
...
struct cdcncm_driver_s {
	...
	FAR const struct ndp_parser_opts_s *parseropts; /* Options currently used to parse NTB */
	...
	/* This holds the information visible to the ROUX network */
...
static ssize_t cdcmbim_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t cdcmbim_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int cdcmbim_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
#endif
...
static int cdcncm_addmac(FAR struct netdev_lowerhalf_s *dev, FAR const uint8_t *mac);
#ifdef CONFIG_NET_MCASTGROUP
static int cdcncm_rmmac(FAR struct netdev_lowerhalf_s *dev, FAR const uint8_t *mac);
#endif
#endif
#ifdef CONFIG_NETDEV_IOCTL
static int cdcncm_ioctl(FAR struct netdev_lowerhalf_s *dev, int cmd, unsigned long arg);
#endif
...
static int cdcncm_bind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);

static void cdcncm_unbind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);

static int cdcncm_setup(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev,
						FAR const struct usb_ctrlreq_s *ctrl, FAR uint8_t *dataout, size_t outlen);

static void cdcncm_disconnect(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev);

/* USB Device Class helpers */

static void cdcncm_ep0incomplete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req);
static void cdcncm_intcomplete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req);
static void cdcncm_rdcomplete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req);
static void cdcncm_wrcomplete(FAR struct usbdev_ep_s *ep, FAR struct usbdev_req_s *req);
...
static const struct usbdevclass_driverops_s g_usbdevops = {
	cdcncm_bind, cdcncm_unbind, cdcncm_setup, cdcncm_disconnect, NULL, NULL};
...
static const struct usb_devdesc_s g_ncmdevdesc = {
	...
	{LSBYTE(CONFIG_CDCNCM_VENDORID), MSBYTE(CONFIG_CDCNCM_VENDORID)},
	{LSBYTE(CONFIG_CDCNCM_PRODUCTID), MSBYTE(CONFIG_CDCNCM_PRODUCTID)},
	{LSBYTE(CDCECM_VERSIONNO), MSBYTE(CDCECM_VERSIONNO)},
...
static inline void cdcncm_put(FAR uint8_t **address, size_t size, uint32_t value)
{
	switch (size) {
	case 2:
...
static ssize_t cdcmbim_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	work_queue(ETHWORK, &self->ncmdriver.notifywork, cdcncm_notify_worker, self, 0);
...
static void cdcncm_transmit_format(FAR struct cdcncm_driver_s *self, FAR netpkt_t *pkt)
{
	...
	if (self->dgramcount == 0) {
		...
		self->dgramaddr =
			self->wrreq->buf + ndpindex + opts->ndpsize + (TX_MAX_NUM_DPE + 1) * dgramidxlen;
		self->dgramaddr = (FAR uint8_t *)NCM_ALIGN((uintptr_t)self->dgramaddr, div) + rem;
	...
	tmp = self->wrreq->buf + ndpindex + opts->ndpsize + self->dgramcount * dgramidxlen;
...
static void cdcncm_transmit_work(FAR void *arg)
{
	...
	tmp += opts->reserved1 + opts->nextndpindex + opts->reserved2 + self->dgramcount * dgramidxlen;
	self->dgramcount = 0;
...
static int cdcncm_packet_handler(FAR struct cdcncm_driver_s *self, FAR uint8_t *dgram,
								 uint32_t dglen)
{
...
/* ROUX netdev callback functions */

/****************************************************************************
 * Name: cdcncm_ifup
 *
 * Description:
 *   ROUX Callback: Bring up the Ethernet interface when an IP address is
 *   provided
 *
 * Input Parameters:
 *   dev - Reference to the ROUX driver state structure
 *
 * Returned Value:
 *   None
 *
...
static int cdcncm_ifup(FAR struct netdev_lowerhalf_s *dev)
{
	FAR struct cdcncm_driver_s *priv = container_of(dev, struct cdcncm_driver_s, dev);

#ifdef CONFIG_NET_IPv4
	ninfo("Bringing up: %u.%u.%u.%u\n", ip4_addr1(dev->netdev.d_ipaddr),
		  ip4_addr2(dev->netdev.d_ipaddr), ip4_addr3(dev->netdev.d_ipaddr),
		  ip4_addr4(dev->netdev.d_ipaddr));
#endif
#ifdef CONFIG_NET_IPv6
	ninfo("Bringing up: %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", dev->netdev.d_ipv6addr[0],
		  dev->netdev.d_ipv6addr[1], dev->netdev.d_ipv6addr[2], dev->netdev.d_ipv6addr[3],
		  dev->netdev.d_ipv6addr[4], dev->netdev.d_ipv6addr[5], dev->netdev.d_ipv6addr[6],
		  dev->netdev.d_ipv6addr[7]);
#endif
 ...
 *
 * Description:
 *   ROUX Callback: Stop the interface.
 *
 * Input Parameters:
 *   dev - Reference to the ROUX driver state structure
 *
 ...
 *
 * Input Parameters:
 *   dev - Reference to the ROUX netdev lowerhalf driver structure
...
static int cdcncm_send(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt)
{
	...
	if ((self->wrreq->buf + NTB_OUT_SIZE - self->dgramaddr < self->dev.netdev.d_pktsize) ||
		self->dgramcount >= TX_MAX_NUM_DPE) {
 ...
 *
 * Description:
 *   ROUX Callback: Add the specified MAC address to the hardware multicast
 *   address filtering
 *
 * Input Parameters:
 *   dev  - Reference to the ROUX driver state structure
 ...
 *
 * Description:
 *   ROUX Callback: Remove the specified MAC address from the hardware
...
static void cdcncm_resetconfig(FAR struct cdcncm_driver_s *self)
{
	...
	self->ndpsign = self->isncm ? self->parseropts->ndpsign : CDC_MBIM_NDP16_NOCRC_SIGN;
}
...
static int cdcncm_setconfig(FAR struct cdcncm_driver_s *self, uint8_t config)
{
	...
	cdcncm_mkepdesc(CDCNCM_EP_INTIN_IDX, &epdesc.epdesc, &self->devinfo, self->usbdev.speed);
	...
	cdcncm_mkepdesc(CDCNCM_EP_BULKIN_IDX, &epdesc.epdesc, &self->devinfo, self->usbdev.speed);
	...
	cdcncm_mkepdesc(CDCNCM_EP_BULKOUT_IDX, &epdesc.epdesc, &self->devinfo, self->usbdev.speed);
	...
	self->rdreq->callback = cdcncm_rdcomplete, ret = EP_SUBMIT(self->epbulkout, self->rdreq);
	if (ret != OK) {
	...
	memcpy(self->dev.netdev.d_mac.ether.ether_addr_octet, "\x00\xe0\xde\xad\xbe\xef", IFHWADDRLEN);
...
{
	FAR struct usb_ctrlreq_s *req = (FAR struct usb_ctrlreq_s *)self->notifyreq->buf;
	int ret = 0;
	...
	case NCM_NOTIFY_SPEED: {
		...
		data[0] = self->usbdev.speed == USB_SPEED_HIGH ? CDCECM_HIGH_BITRATE : CDCECM_LOW_BITRATE;
		data[1] = data[0];
		ret = sizeof(*req) + 8;
...
static int cdcncm_setinterface(FAR struct cdcncm_driver_s *self, uint16_t interface,
							   uint16_t altsetting)
{
	if (interface == self->devinfo.ifnobase + 1) {
		...
		work_queue(ETHWORK, &self->notifywork, cdcncm_notify_worker, self, MSEC2TICK(100));
	} else {
...
static int cdcnm_mkstrdesc(uint8_t id, FAR struct usb_strdesc_s *strdesc, bool isncm)
{
...
static void cdcncm_mkepcompdesc(int epidx, FAR struct usb_ss_epcompdesc_s *epcompdesc)
{
	switch (epidx) {
	case CDCNCM_EP_INTIN_IDX: /* Interrupt IN endpoint */
	{
		...
		epcompdesc->attr = 0;
		epcompdesc->wbytes[0] = LSBYTE((epcompdesc->mxburst + 1) * CONFIG_CDCNCM_EPINTIN_SSSIZE);
		epcompdesc->wbytes[1] = MSBYTE((epcompdesc->mxburst + 1) * CONFIG_CDCNCM_EPINTIN_SSSIZE);
	} break;
						   ...
						   FAR struct usbdev_devinfo_s *devinfo, uint8_t speed)
{
...
#ifdef CONFIG_USBDEV_SUPERSPEED
	if (speed == USB_SPEED_SUPER || speed == USB_SPEED_SUPER_PLUS || speed == USB_SPEED_UNKNOWN) {
	...
	case CDCNCM_EP_INTIN_IDX: /* Interrupt IN endpoint */
	{
		epdesc->addr = USB_DIR_IN | devinfo->epno[CDCNCM_EP_INTIN_IDX];
	...
	case CDCNCM_EP_BULKIN_IDX: {
		epdesc->addr = USB_DIR_IN | devinfo->epno[CDCNCM_EP_BULKIN_IDX];
	...
	case CDCNCM_EP_BULKOUT_IDX: {
		epdesc->addr = USB_DIR_OUT | devinfo->epno[CDCNCM_EP_BULKOUT_IDX];
		epdesc->attr = USB_EP_ATTR_XFER_BULK;
...
static int16_t cdcnm_mkcfgdesc(FAR uint8_t *desc, FAR struct usbdev_devinfo_s *devinfo,
							   uint8_t speed, uint8_t type, bool isncm)
{
	...
	if (desc) {
		...
		cfgdesc->attr = USB_CONFIG_ATTR_ONE | CDCECM_SELFPOWERED | CDCECM_REMOTEWAKEUP;
	...
	if (desc) {
		...
		iaddesc->nifs =
			devinfo->ninterfaces;		  /* Number of interfaces associated with the function */
		iaddesc->classid = USB_CLASS_CDC; /* Class code */
		iaddesc->subclass = isncm ? CDC_SUBCLASS_NCM : CDC_SUBCLASS_MBIM; /* Sub-class code */
	...
	ret = cdcncm_mkepdesc(CDCNCM_EP_INTIN_IDX, (FAR struct usb_epdesc_s *)desc, devinfo, speed);
	if (desc) {
		desc += ret;
	}
	...
	ret = cdcncm_mkepdesc(CDCNCM_EP_BULKIN_IDX, (FAR struct usb_epdesc_s *)desc, devinfo, speed);
	if (desc) {
		desc += ret;
	}
	...
	ret = cdcncm_mkepdesc(CDCNCM_EP_BULKOUT_IDX, (FAR struct usb_epdesc_s *)desc, devinfo, speed);
	if (desc) {
		desc += ret;
	}
...
static int16_t cdcncm_mkcfgdesc(FAR uint8_t *desc, FAR struct usbdev_devinfo_s *devinfo,
...
#ifdef CONFIG_NET_CDCMBIM
static int16_t cdcmbim_mkcfgdesc(FAR uint8_t *desc, FAR struct usbdev_devinfo_s *devinfo,
								 uint8_t speed, uint8_t type)
{
...
static int cdcncm_getdescriptor(FAR struct cdcncm_driver_s *self, uint8_t type, uint8_t index,
								FAR void *desc)
{
	switch (type) {
#ifndef CONFIG_CDCNCM_COMPOSITE
	case USB_DESC_TYPE_DEVICE:
		if (self->isncm) {
			return usbdev_copy_devdesc(desc, &g_ncmdevdesc, self->usbdev.speed);
		}
#ifdef CONFIG_NET_CDCMBIM
		else {
	...
	case USB_DESC_TYPE_CONFIG:
		return cdcncm_mkcfgdesc((FAR uint8_t *)desc, &self->devinfo, self->usbdev.speed, type);
...
static int cdcncm_bind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev)
{
	...
	self->epint = DEV_ALLOCEP(dev, USB_DIR_IN | self->devinfo.epno[CDCNCM_EP_INTIN_IDX], true,
							  USB_EP_ATTR_XFER_INT);
	self->epbulkin = DEV_ALLOCEP(dev, USB_DIR_IN | self->devinfo.epno[CDCNCM_EP_BULKIN_IDX], true,
								 USB_EP_ATTR_XFER_BULK);
	self->epbulkout = DEV_ALLOCEP(dev, USB_DIR_OUT | self->devinfo.epno[CDCNCM_EP_BULKOUT_IDX],
						...
						FAR const struct usb_ctrlreq_s *ctrl, FAR uint8_t *dataout, size_t outlen)
{
	...
	if ((ctrl->type & USB_REQ_TYPE_MASK) == USB_REQ_TYPE_STANDARD) {
		...
		case USB_REQ_GETDESCRIPTOR: {
			...
			ret = cdcncm_getdescriptor(self, desctype, descindex, self->ctrlreq->buf);
		} break;
		...
		case NCM_GET_NTB_PARAMETERS:
			if (len >= sizeof(g_ntbparameters)) {
				memcpy(self->ctrlreq->buf, &g_ntbparameters, sizeof(g_ntbparameters));
		...
		case NCM_SET_NTB_FORMAT:
			...
			case 0x0001:
				...
				self->ndpsign = self->isncm ? self->parseropts->ndpsign : CDC_MBIM_NDP32_NOCRC_SIGN;
		...
		case MBIM_SEND_COMMAND: {
			FAR struct cdcmbim_driver_s *mbim = (FAR struct cdcmbim_driver_s *)self;
...
static int cdcncm_classobject(int minor, FAR struct usbdev_devinfo_s *devinfo,
...
#ifdef CONFIG_NET_CDCMBIM
static int cdcmbim_classobject(int minor, FAR struct usbdev_devinfo_s *devinfo,
							   FAR struct usbdevclass_driver_s **classdev)
{
	FAR struct cdcmbim_driver_s *self;
...
{
	FAR struct cdcmbim_driver_s *self = (FAR struct cdcmbim_driver_s *)classdev;
	int ret;
...
static void cdcnm_get_composite_devdesc(FAR struct composite_devdesc_s *dev, bool isncm)
{
	...
	dev->cfgdescsize = isncm ? cdcncm_mkcfgdesc(NULL, NULL, USB_SPEED_UNKNOWN, 0)
 ...
 *   network driver to ROUX and bring up the network.
 *
 * Input Parameters:
