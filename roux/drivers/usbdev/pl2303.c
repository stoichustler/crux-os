...
#define CONFIG_PL2303_VENDORSTR "ROUX"
#endif
...
/* These settings are not modifiable via the ROUX configuration */
...
static inline int usbclass_recvpacket(FAR struct pl2303_dev_s *priv, uint8_t *reqbuf,
									  uint16_t reqlen);

/* Configuration ************************************************************/

static int usbclass_mkstrdesc(uint8_t id, struct usb_strdesc_s *strdesc);
static int usbclass_mkepbulkdesc(FAR const struct usb_epdesc_s *indesc,
#ifdef CONFIG_USBDEV_SUPERSPEED
								 FAR const struct usb_ss_epcompdesc_s *incompdesc,
#endif
								 uint8_t speed, FAR struct usb_epdesc_s *outdesc);
...
static int usbclass_mkepintdesc(FAR const struct usb_epdesc_s *indesc,
#ifdef CONFIG_USBDEV_SUPERSPEED
								FAR const struct usb_ss_epcompdesc_s *incompdesc,
#endif
								uint8_t speed, FAR struct usb_epdesc_s *outdesc)
{
	...
	if (speed >= USB_SPEED_SUPER) {
		FAR struct usb_ss_epcompdesc_s *outcompdesc = (FAR struct usb_ss_epcompdesc_s *)(outdesc++);
...
static int16_t usbclass_mkcfgdesc(uint8_t *buf, uint8_t speed, uint8_t type)
{
	...
	ret = usbclass_mkepintdesc(&g_epintindesc,
							   ...
							   &g_epintincompdesc,
#endif
							   speed, (FAR struct usb_epdesc_s *)buf);
...
static int usbclass_setconfig(FAR struct pl2303_dev_s *priv, uint8_t config)
{
	...
	usbclass_mkepintdesc(&g_epintindesc,
						 ...
						 &g_epintincompdesc,
#endif
						 priv->usbdev->speed, &epdesc.epdesc);
...
static int usbclass_bind(FAR struct usbdevclass_driver_s *driver, FAR struct usbdev_s *dev)
{
	...
	if (dev->speed == USB_SPEED_SUPER || dev->speed == USB_SPEED_SUPER_PLUS) {
		if (CONFIG_PL2303_EPBULK_MAXBURST < USB_SS_BULK_EP_MAXBURST) {
			reqlen = CONFIG_PL2303_EPBULK_SSSIZE * (CONFIG_PL2303_EPBULK_MAXBURST + 1);
		} else {
	...
	case USB_REQ_TYPE_STANDARD: {
		...
		case USB_REQ_GETDESCRIPTOR: {
			...
			case USB_DESC_TYPE_CONFIG: {
				ret = usbclass_mkcfgdesc(ctrlreq->buf, dev->speed, ctrl->value[1]);
			} break;
		...
		case USB_REQ_SETINTERFACE: {
			if (ctrl->type == USB_REQ_RECIPIENT_INTERFACE) {
				if (priv->config == PL2303_CONFIGID && index == PL2303_INTERFACEID &&
