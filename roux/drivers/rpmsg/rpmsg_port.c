...
static FAR const char *rpmsg_port_get_local_cpuname(FAR struct rpmsg_s *rpmsg);
...
static const struct rpmsg_ops_s g_rpmsg_port_ops = {
	NULL, NULL, NULL, NULL, rpmsg_port_dump, rpmsg_port_get_local_cpuname, rpmsg_port_get_cpuname,
};
...
static void rpmsg_port_add_node(FAR struct rpmsg_port_list_s *list, FAR struct list_node *node)
{
	irqstate_t flags;
...
static FAR struct list_node *rpmsg_port_remove_node(FAR struct rpmsg_port_list_s *list)
{
...
static int rpmsg_port_create_queue(FAR struct rpmsg_port_queue_s *queue, uint16_t count,
								   uint16_t len, FAR void *buf)
{
	FAR struct list_node *node;
...
static int rpmsg_port_create_queues(FAR struct rpmsg_port_s *port,
									FAR const struct rpmsg_port_config_s *cfg)
{
	int ret;

	ret = rpmsg_port_create_queue(&port->txq, cfg->txnum, cfg->txlen, cfg->txbuf);
	if (ret < 0) {
		return ret;
	}

	ret = rpmsg_port_create_queue(&port->rxq, cfg->rxnum, cfg->rxlen, cfg->rxbuf);
	if (ret < 0) {
...
static FAR void *rpmsg_port_get_tx_payload_buffer(FAR struct rpmsg_device *rdev, FAR uint32_t *len,
												  int wait)
{
	FAR struct rpmsg_port_s *port = metal_container_of(rdev, struct rpmsg_port_s, rdev);
	FAR struct rpmsg_port_header_s *hdr = rpmsg_port_queue_get_available_buffer(&port->txq, wait);

	if (hdr == NULL) {
		return NULL;
	}

	*len = hdr->len - sizeof(struct rpmsg_port_header_s) - sizeof(struct rpmsg_hdr);
...
static int rpmsg_port_send_offchannel_nocopy(FAR struct rpmsg_device *rdev, uint32_t src,
											 uint32_t dst, FAR const void *data, int len)
{
	FAR struct rpmsg_port_s *port = metal_container_of(rdev, struct rpmsg_port_s, rdev);
	...
	hdr->len = sizeof(struct rpmsg_port_header_s) + sizeof(struct rpmsg_hdr) + len;
...
static int rpmsg_port_send_offchannel_raw(FAR struct rpmsg_device *rdev, uint32_t src, uint32_t dst,
										  FAR const void *data, int len, int wait)
{
	uint32_t buflen;
	FAR void *buf;
...
static void rpmsg_port_hold_rx_buffer(FAR struct rpmsg_device *rdev, FAR void *rxbuf)
{
...
static void rpmsg_port_release_rx_buffer(FAR struct rpmsg_device *rdev, FAR void *rxbuf)
{
	FAR struct rpmsg_port_s *port = metal_container_of(rdev, struct rpmsg_port_s, rdev);
	FAR struct rpmsg_hdr *rphdr = RPMSG_LOCATE_HDR(rxbuf);
	FAR struct rpmsg_port_header_s *hdr =
		metal_container_of(rphdr, struct rpmsg_port_header_s, buf);
	uint32_t reserved = atomic_fetch_sub(&rphdr->reserved, 1 << RPMSG_BUF_HELD_SHIFT);
...
static int rpmsg_port_release_tx_buffer(FAR struct rpmsg_device *rdev, FAR void *txbuf)
{
	FAR struct rpmsg_port_s *port = metal_container_of(rdev, struct rpmsg_port_s, rdev);
...
static int rpmsg_port_get_tx_buffer_size(FAR struct rpmsg_device *rdev)
{
	...
	return port->txq.len - sizeof(struct rpmsg_port_header_s) - sizeof(struct rpmsg_hdr);
}
...
static int rpmsg_port_get_rx_buffer_size(FAR struct rpmsg_device *rdev)
{
	...
	return port->rxq.len - sizeof(struct rpmsg_port_header_s) - sizeof(struct rpmsg_hdr);
}
...
static int rpmsg_port_ns_callback(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
...
int rpmsg_port_initialize(FAR struct rpmsg_port_s *port, FAR const struct rpmsg_port_config_s *cfg,
						  FAR const struct rpmsg_port_ops_s *ops)
{
	...
	rpmsg_register_endpoint(rdev, &rdev->ns_ept, "NS", RPMSG_NS_EPT_ADDR, RPMSG_NS_EPT_ADDR,
...
rpmsg_port_queue_get_available_buffer(FAR struct rpmsg_port_queue_s *queue, bool wait)
{
	FAR struct list_node *node;
	FAR struct rpmsg_port_header_s *hdr;
...
FAR struct rpmsg_port_header_s *rpmsg_port_queue_get_buffer(FAR struct rpmsg_port_queue_s *queue,
															bool wait)
{
	FAR struct list_node *node;
...
int rpmsg_port_register(FAR struct rpmsg_port_s *port, FAR const char *local_cpuname)
{
	char name[64];
	int ret;
...
static void rpmsg_port_dump_buffer(FAR struct rpmsg_device *rdev,
								   FAR struct rpmsg_port_queue_s *queue, bool rx)
{
	FAR struct list_node *node;
	irqstate_t flags = spin_lock_irqsave(&queue->ready.lock);

	metal_log(METAL_LOG_EMERGENCY, "rpmsg_port queue %s: {used: %u, avail: %u}\n", rx ? "RX" : "TX",
			  rpmsg_port_queue_nused(queue), rpmsg_port_queue_navail(queue));
	metal_log(METAL_LOG_EMERGENCY, "rpmsg buffer list:\n");
	list_for_every(&queue->ready.head, node)
	{
		FAR struct rpmsg_port_header_s *hdr = RPMSG_PORT_NODE_TO_BUF(queue, node);
		FAR struct rpmsg_hdr *rphdr = (FAR struct rpmsg_hdr *)hdr->buf;
		FAR struct rpmsg_endpoint *ept;
