...
static int rpmsg_dev_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
...
static inline FAR struct rpmsg_s *rpmsg_get_by_rdev(FAR struct rpmsg_device *rdev)
{
	if (!rdev) {
		return NULL;
	}
...
static int rpmsg_dev_ioctl_(FAR struct rpmsg_s *rpmsg, int cmd, unsigned long arg)
{
	int ret = -ENOTTY;
...
static int rpmsg_dev_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	FAR struct rpmsg_s *rpmsg = (FAR struct rpmsg_s *)filep->f_inode->i_private;
...
int rpmsg_register_callback(FAR void *priv, rpmsg_dev_cb_t device_created,
							rpmsg_dev_cb_t device_destroy, rpmsg_match_cb_t ns_match,
							rpmsg_bind_cb_t ns_bind)
{
	...
	{
		FAR struct rpmsg_s *rpmsg = metal_container_of(node, struct rpmsg_s, node);
...
again:
		...
		{
			FAR struct rpmsg_bind_s *bind = metal_container_of(bnode, struct rpmsg_bind_s, node);
...
void rpmsg_unregister_callback(FAR void *priv, rpmsg_dev_cb_t device_created,
							   rpmsg_dev_cb_t device_destroy, rpmsg_match_cb_t ns_match,
							   rpmsg_bind_cb_t ns_bind)
{
	FAR struct metal_list *node;
	...
	{
		FAR struct rpmsg_cb_s *cb = metal_container_of(node, struct rpmsg_cb_s, node);

		if (cb->priv == priv && cb->device_created == device_created &&
			cb->device_destroy == device_destroy && cb->ns_match == ns_match &&
			cb->ns_bind == ns_bind) {
	...
	if (device_destroy) {
		...
		{
			FAR struct rpmsg_s *rpmsg = metal_container_of(pnode, struct rpmsg_s, node);
...
void rpmsg_ns_bind(FAR struct rpmsg_device *rdev, FAR const char *name, uint32_t dest)
{
	FAR struct rpmsg_s *rpmsg = rpmsg_get_by_rdev(rdev);
...
void rpmsg_ns_unbind(FAR struct rpmsg_device *rdev, FAR const char *name, uint32_t dest)
{
	FAR struct rpmsg_s *rpmsg = rpmsg_get_by_rdev(rdev);
	FAR struct metal_list *node;
	...
	{
		FAR struct rpmsg_bind_s *bind = metal_container_of(node, struct rpmsg_bind_s, node);
