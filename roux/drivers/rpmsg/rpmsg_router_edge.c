...
#define rpmsg_router_edge_from_rdev(d) metal_container_of(d, struct rpmsg_router_edge_s, rdev)
...
static FAR const char *rpmsg_router_edge_get_local_cpuname(FAR struct rpmsg_s *rpmsg);
static FAR const char *rpmsg_router_edge_get_cpuname(FAR struct rpmsg_s *rpmsg);

static int rpmsg_router_edge_get_rx_buffer_size(FAR struct rpmsg_device *rdev);
static int rpmsg_router_edge_get_tx_buffer_size(FAR struct rpmsg_device *rdev);
...
static FAR const char *rpmsg_router_edge_get_local_cpuname(FAR struct rpmsg_s *rpmsg)
{
	FAR struct rpmsg_router_edge_s *edge = (FAR struct rpmsg_router_edge_s *)rpmsg;
...
static FAR void *rpmsg_router_edge_get_tx_payload_buffer(FAR struct rpmsg_device *rdev,
...
static void rpmsg_router_edge_hold_rx_buffer(FAR struct rpmsg_device *rdev, FAR void *rxbuf)
{
	FAR struct rpmsg_router_edge_s *edge = rpmsg_router_edge_from_rdev(rdev);
	FAR struct rpmsg_device *hubdev = edge->hubdev;
...
static void rpmsg_router_edge_release_rx_buffer(FAR struct rpmsg_device *rdev, FAR void *rxbuf)
{
...
static int rpmsg_router_edge_release_tx_buffer(FAR struct rpmsg_device *rdev, FAR void *txbuf)
{
	struct rpmsg_router_edge_s *edge = rpmsg_router_edge_from_rdev(rdev);
	struct rpmsg_device *hubdev = edge->hubdev;
...
static int rpmsg_router_edge_send_nocopy(FAR struct rpmsg_device *rdev, uint32_t src, uint32_t dst,
...
static int rpmsg_router_edge_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
...
static int rpmsg_router_edge_send_offchannel_raw(FAR struct rpmsg_device *rdev, uint32_t src,
												 ...
												 int wait)
{
	...
	if (dst != RPMSG_NS_EPT_ADDR) {
		...
		return hubdev->ops.send_offchannel_raw(hubdev, src, dst, data, len, wait);
	}
	...
	snprintf(ns_msg->name, sizeof(ns_msg->name), RPMSG_ROUTER_NAME_PREFIX "%s:%s", edge->remotecpu,
			 name);
	...
	if (ns_msg->flags == RPMSG_NS_DESTROY) {
		...
		ept = rpmsg_get_endpoint(hubdev, ns_msg->name, RPMSG_ADDR_ANY, usr_ept->dest_addr);
		...
		ret = rpmsg_create_ept(ept, hubdev, ns_msg->name, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
							   rpmsg_router_edge_cb, rpmsg_router_edge_unbind);
		if (ret < 0) {
...
static bool rpmsg_router_edge_match(FAR struct rpmsg_device *rdev, FAR void *priv,
									FAR const char *name, uint32_t dest)
{
	...
	return !strncmp(name + RPMSG_ROUTER_NAME_PREFIX_LEN, edge->remotecpu, strlen(edge->remotecpu));
}
...
static void rpmsg_router_edge_bind(FAR struct rpmsg_device *rdev, FAR void *priv,
								   FAR const char *name, uint32_t dest)
{
	FAR struct rpmsg_router_edge_s *edge = priv;
	FAR struct rpmsg_device *edgedev = &edge->rdev;

	edgedev->ns_bind_cb(edgedev, name + RPMSG_ROUTER_NAME_PREFIX_LEN + strlen(edge->remotecpu) + 1,
						dest);
}
...
{
	rpmsg_unregister_callback(edge, NULL, NULL, rpmsg_router_edge_match, rpmsg_router_edge_bind);
...
static FAR struct rpmsg_router_edge_s *rpmsg_router_edge_create(FAR struct rpmsg_device *hubdev,
																...
																FAR const char *remotecpu)
{
	...
	ret =
		rpmsg_register_callback(edge, NULL, NULL, rpmsg_router_edge_match, rpmsg_router_edge_bind);
...
static int rpmsg_router_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len, uint32_t src,
						   FAR void *priv)
{
	...
	edge = rpmsg_router_edge_create(ept->rdev, msg, ept->name + RPMSG_ROUTER_NAME_LEN);
	if (!edge) {
		return -ENODEV;
	}
...
static bool rpmsg_router_match(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							   uint32_t dest)
{
...
static void rpmsg_router_bind(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							  uint32_t dest)
{
	FAR struct rpmsg_endpoint *ept;
	int ret;
	...
	ret = rpmsg_create_ept(ept, rdev, name, RPMSG_ADDR_ANY, dest, rpmsg_router_cb,
						   rpmsg_router_unbind);
	if (ret < 0) {
		rpmsgerr("Create router endpoint failed: %d\n", ret);
		kmm_free(ept);
...
int rpmsg_router_edge_init(void)
{
	...
	return rpmsg_register_callback(NULL, NULL, NULL, rpmsg_router_match, rpmsg_router_bind);
}
