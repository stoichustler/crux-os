...
#include <roux/rpmsg/rpmsg_virtio_lite.h>
...
static FAR const char *rpmsg_virtio_lite_get_local_cpuname(FAR struct rpmsg_s *rpmsg);
static FAR const char *rpmsg_virtio_lite_get_cpuname(FAR struct rpmsg_s *rpmsg);

static int rpmsg_virtio_lite_create_virtqueues_(FAR struct virtio_device *vdev, unsigned int flags,
												unsigned int nvqs, FAR const char *names[],
												vq_callback callbacks[], FAR void *callback_args[]);
static uint8_t rpmsg_virtio_lite_get_status_(FAR struct virtio_device *dev);
static void rpmsg_virtio_lite_set_status_(FAR struct virtio_device *dev, uint8_t status);
static uint64_t rpmsg_virtio_lite_get_features_(FAR struct virtio_device *dev);
static void rpmsg_virtio_lite_set_features(FAR struct virtio_device *dev, uint64_t feature);
...
#if defined(CONFIG_OPENAMP_DEBUG) || defined(CONFIG_RPMSG_VIRTIO_LITE_PM_AUTORELAX)
static int rpmsg_virtio_lite_buffer_nused(FAR struct rpmsg_virtio_device *rvdev, bool rx)
{
...
static void rpmsg_virtio_lite_wakeup_tx(FAR struct rpmsg_virtio_lite_priv_s *priv)
{
	int semcount;
...
{
	FAR struct rpmsg_virtio_lite_priv_s *priv = (FAR struct rpmsg_virtio_lite_priv_s *)arg;
...
static inline void rpmsg_virtio_lite_pm_action(FAR struct rpmsg_virtio_lite_priv_s *priv, bool stay)
{
	irqstate_t flags;
	int count;
	...
	if (!stay && count > 0 && rpmsg_virtio_lite_buffer_nused(&priv->rvdev, false) == 0) {
		pm_wakelock_relax(&priv->wakelock);
	}
#endif
...
static inline bool rpmsg_virtio_lite_available_rx(FAR struct rpmsg_virtio_lite_priv_s *priv)
{
...
static inline void rpmsg_virtio_lite_update_rx(FAR struct rpmsg_virtio_lite_priv_s *priv)
{
	FAR struct rpmsg_virtio_device *rvdev = &priv->rvdev;
	FAR struct virtqueue *rvq = rvdev->rvq;
												...
												vq_callback callbacks[], FAR void *callback_args[])
{
	...
	for (i = 0; i < nvqs; i++) {
		...
		if (vdev->role == VIRTIO_DEV_DRIVER) {
			...
			metal_io_block_set(vinfo->io, offset, 0, vring_size(valloc->num_descs, valloc->align));
		}
#endif

		ret = virtqueue_create(vdev, i, names[i], valloc, callbacks[i], vdev->func->notify,
							   vinfo->vq);
		if (ret < 0) {
			return ret;
		}
...
{
	FAR struct rpmsg_virtio_lite_priv_s *priv = rpmsg_virtio_lite_get_priv(vdev);

	return priv->rsc->rpmsg_vdev.status;
}

static void rpmsg_virtio_lite_set_status_(FAR struct virtio_device *vdev, uint8_t status)
{
	FAR struct rpmsg_virtio_lite_priv_s *priv = rpmsg_virtio_lite_get_priv(vdev);
...
static uint64_t rpmsg_virtio_lite_get_features_(FAR struct virtio_device *vdev)
{
	FAR struct rpmsg_virtio_lite_priv_s *priv = rpmsg_virtio_lite_get_priv(vdev);
...
static void rpmsg_virtio_lite_set_features(FAR struct virtio_device *vdev, uint64_t features)
{
	FAR struct rpmsg_virtio_lite_priv_s *priv = rpmsg_virtio_lite_get_priv(vdev);
...
static void rpmsg_virtio_lite_notify(FAR struct virtqueue *vq)
{
	...
	if (vdev->vrings_info->notifyid == vdev->vrings_info[svq->vq_queue_index].notifyid) {
...
static bool rpmsg_virtio_lite_is_recursive(FAR struct rpmsg_virtio_lite_priv_s *priv)
{
...
static int rpmsg_virtio_lite_wait(FAR struct rpmsg_s *rpmsg, FAR sem_t *sem)
{
	FAR struct rpmsg_virtio_lite_priv_s *priv = (FAR struct rpmsg_virtio_lite_priv_s *)rpmsg;
	int ret;
...
{
	FAR struct rpmsg_virtio_lite_priv_s *priv = (FAR struct rpmsg_virtio_lite_priv_s *)rpmsg;
	FAR struct rpmsg_virtio_lite_cmd_s *cmd = RPMSG_VIRTIO_LITE_RSC2CMD(priv->rsc);

	if (RPMSG_VIRTIO_LITE_IS_MASTER(priv->dev)) {
		cmd->cmd_master = RPMSG_VIRTIO_LITE_CMD(RPMSG_VIRTIO_LITE_CMD_PANIC, 0);
	} else {
		cmd->cmd_slave = RPMSG_VIRTIO_LITE_CMD(RPMSG_VIRTIO_LITE_CMD_PANIC, 0);
	}
...
static void rpmsg_virtio_lite_dump_buffer(FAR struct rpmsg_virtio_device *rvdev, bool rx)
{
	FAR struct virtqueue *vq = rx ? rvdev->rvq : rvdev->svq;
	int num;
	int i;
	...
	metal_log(METAL_LOG_EMERGENCY, "    %s buffer, total %d, pending %d\n", rx ? "RX" : "TX",
			  vq->vq_nentries, num);
	...
	for (i = 0; i < num; i++) {
		...
		addr = metal_io_phys_to_virt(vq->shm_io, vq->vq_ring.desc[desc_idx].addr);
		if (addr) {
			FAR struct rpmsg_hdr *hdr = addr;
			FAR struct rpmsg_endpoint *ept;

			ept = rpmsg_get_ept_from_addr(&rvdev->rdev, rx ? hdr->dst : hdr->src);
			if (ept) {
...
static void rpmsg_virtio_lite_dump(FAR struct rpmsg_s *rpmsg)
{
	...
	if (up_interrupt_context() || sched_idletask() || nxmutex_is_hold(&rdev->lock)) {
		needlock = false;
	}
	...
	metal_log(METAL_LOG_EMERGENCY, "Dump rpmsg info between cpu (master: %s)%s <==> %s:\n",
			  rpmsg_virtio_get_role(rvdev) == RPMSG_HOST ? "yes" : "no", CONFIG_RPMSG_LOCAL_CPUNAME,
...
static void rpmsg_virtio_lite_wakeup_rx(FAR struct rpmsg_virtio_lite_priv_s *priv)
{
	int semcount;
...
{
	FAR struct rpmsg_virtio_lite_cmd_s *rpmsg_virtio_cmd = RPMSG_VIRTIO_LITE_RSC2CMD(priv->rsc);
	uint32_t cmd;
...
static int rpmsg_virtio_lite_notify_wait(FAR struct rpmsg_device *rdev, uint32_t id)
{
...
static int rpmsg_virtio_lite_start(FAR struct rpmsg_virtio_lite_priv_s *priv)
{
	...
	vdev->role = RPMSG_VIRTIO_LITE_IS_MASTER(priv->dev) ? RPMSG_HOST : RPMSG_REMOTE;
	...
	ret = rpmsg_init_vdev_with_config(&priv->rvdev, vdev, rpmsg_ns_bind, metal_io_get_region(),
									  priv->pool, &config);
	if (ret != 0) {
	...
	RPMSG_VIRTIO_LITE_REGISTER_CALLBACK(priv->dev, rpmsg_virtio_lite_callback, priv);
...
{
	FAR struct rpmsg_virtio_lite_priv_s *priv =
		(FAR struct rpmsg_virtio_lite_priv_s *)((uintptr_t)strtoul(argv[2], NULL, 16));
	int ret;
...
int rpmsg_virtio_lite_initialize(FAR struct rpmsg_virtio_lite_s *dev)
{
	...
	snprintf(name, sizeof(name), "/dev/rpmsg/%s", RPMSG_VIRTIO_LITE_GET_CPUNAME(dev));
	...
	ret = kthread_create("rpmsg_virtio", CONFIG_RPMSG_VIRTIO_LITE_PRIORITY,
						 CONFIG_RPMSG_VIRTIO_LITE_STACKSIZE, rpmsg_virtio_lite_thread, argv);
	if (ret < 0) {
		goto err_thread;
	}
