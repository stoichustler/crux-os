...
static void rpmsg_port_uart_send_packet(FAR struct rpmsg_port_uart_s *rpuart, FAR const void *data,
										size_t datalen)
{
...
static void rpmsg_port_uart_send_frame(FAR struct rpmsg_port_uart_s *rpuart, FAR const void *data,
									   size_t datalen)
{
...
static void rpmsg_port_uart_send_connect_req(FAR struct rpmsg_port_uart_s *rpuart)
{
...
static void rpmsg_port_uart_send_connect_ack(FAR struct rpmsg_port_uart_s *rpuart)
{
...
static void rpmsg_port_uart_register_callback(FAR struct rpmsg_port_s *port,
											  rpmsg_port_rx_cb_t callback)
{
	FAR struct rpmsg_port_uart_s *rpuart = (FAR struct rpmsg_port_uart_s *)port;
...
static void rpmsg_port_uart_process_rx_conn(FAR struct rpmsg_port_uart_s *rpuart, uint8_t ch)
{
...
static int rpmsg_port_uart_rx_thread(int argc, FAR char *argv[])
{
	...
	for (;;) {
		...
		for (i = 0; i < ret; i++) {
			if (buf[i] == RPMSG_PORT_UART_CONNREQ || buf[i] == RPMSG_PORT_UART_CONNACK) {
				if (hdr != NULL) {
			...
			case RPMSG_PORT_UART_RX_WAIT_START:
				if (buf[i] == RPMSG_PORT_UART_START) {
					if (hdr == NULL) {
						hdr = rpmsg_port_queue_get_available_buffer(rxq, true);
						next = 0;
					}
			...
			case RPMSG_PORT_UART_RX_RECV_NORMAL:
				if (buf[i] == RPMSG_PORT_UART_START) {
					...
					DEBUGASSERT(hdr->crc == 0 || hdr->crc == rpmsg_port_uart_crc16(hdr));
			...
			case RPMSG_PORT_UART_RX_RECV_ESCAPE:
				*((FAR char *)hdr + next++) = buf[i] ^ RPMSG_PORT_UART_ESCAPE_MASK;
				state = RPMSG_PORT_UART_RX_RECV_NORMAL;
				break;
...
int rpmsg_port_uart_initialize(FAR const struct rpmsg_port_config_s *cfg, FAR const char *uartpath,
							   FAR const char *localcpu)
{
	...
	argv[2] = NULL;
	ret =
		kthread_create("rpmsg-uart-rx", CONFIG_RPMSG_PORT_UART_RX_THREAD_PRIORITY,
					   CONFIG_RPMSG_PORT_UART_RX_THREAD_STACKSIZE, rpmsg_port_uart_rx_thread, argv);
	if (ret < 0) {
	...
	argv[2] = NULL;
	ret =
		kthread_create("rpmsg-uart-tx", CONFIG_RPMSG_PORT_UART_TX_THREAD_PRIORITY,
					   CONFIG_RPMSG_PORT_UART_TX_THREAD_STACKSIZE, rpmsg_port_uart_tx_thread, argv);
	if (ret < 0) {
