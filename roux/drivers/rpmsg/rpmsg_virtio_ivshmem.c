...
#include <roux/pci/pci_ivshmem.h>
#include <roux/rpmsg/rpmsg_virtio_ivshmem.h>
...
static const FAR char *rpmsg_virtio_ivshmem_get_cpuname(FAR struct rpmsg_virtio_lite_s *dev);
static FAR struct rpmsg_virtio_lite_rsc_s *
rpmsg_virtio_ivshmem_get_resource(FAR struct rpmsg_virtio_lite_s *dev);
static int rpmsg_virtio_ivshmem_is_master(FAR struct rpmsg_virtio_lite_s *dev);
static int rpmsg_virtio_ivshmem_notify(FAR struct rpmsg_virtio_lite_s *dev, uint32_t notifyid);
static int rpmsg_virtio_ivshmem_register_callback(FAR struct rpmsg_virtio_lite_s *dev,
												  rpmsg_virtio_callback_t callback, FAR void *arg);
...
static const FAR char *rpmsg_virtio_ivshmem_get_cpuname(FAR struct rpmsg_virtio_lite_s *dev)
{
	FAR struct rpmsg_virtio_ivshmem_dev_s *priv = (FAR struct rpmsg_virtio_ivshmem_dev_s *)dev;
	return priv->cpuname;
}
...
static int rpmsg_virtio_ivshmem_notify(FAR struct rpmsg_virtio_lite_s *dev, uint32_t vqid)
{
	FAR struct rpmsg_virtio_ivshmem_dev_s *priv = (FAR struct rpmsg_virtio_ivshmem_dev_s *)dev;
...
static int rpmsg_virtio_ivshmem_interrupt(int irq, FAR void *context, FAR void *arg)
{
...
{
	FAR struct rpmsg_virtio_ivshmem_dev_s *priv = (FAR struct rpmsg_virtio_ivshmem_dev_s *)arg;
	...
	wd_start(&priv->wdog, RPMSG_VIRTIO_IVSHMEM_WDOG_DELAY, rpmsg_virtio_ivshmem_wdog,
			 (wdparm_t)priv);
}
...
static int rpmsg_virtio_ivshmem_probe(FAR struct ivshmem_device_s *ivdev)
{
	FAR struct rpmsg_virtio_ivshmem_dev_s *priv = rpmsg_virtio_ivshmem_from(ivdev);
	int ret;
	...
	if (!ivshmem_support_irq(ivdev)) {
		ret = wd_start(&priv->wdog, RPMSG_VIRTIO_IVSHMEM_WDOG_DELAY, rpmsg_virtio_ivshmem_wdog,
					   (wdparm_t)priv);
		if (ret < 0) {
...
int pci_register_rpmsg_virtio_ivshmem_driver(void)
{
	...
	while (name != NULL) {
		...
		FAR struct rpmsg_virtio_ivshmem_dev_s *priv = kmm_zalloc(sizeof(*priv));
		if (priv == NULL) {
			return -ENOMEM;
		}
		...
		str = strchr(++name, ':');
		snprintf(priv->cpuname, RPMSG_NAME_SIZE, "%.*s", (int)(str - name), name);
		priv->master = *(str + 1) == 'm';

		pciinfo("Register ivshmem driver, id=%d, cpuname=%s, master=%d\n", priv->drv.id,
				priv->cpuname, priv->master);
