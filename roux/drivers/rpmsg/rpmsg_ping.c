...
static int rpmsg_ping_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
...
static int rpmsg_ping_once(FAR struct rpmsg_endpoint *ept, int len, int cmd, uint32_t *buf_len,
						   char i)
{
...
int rpmsg_ping(FAR struct rpmsg_endpoint *ept, FAR const struct rpmsg_ping_s *ping)
{
...
int rpmsg_ping_init(FAR struct rpmsg_device *rdev, FAR struct rpmsg_endpoint *ept)
{
	return rpmsg_create_ept(ept, rdev, RPMSG_PING_EPT_NAME, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
