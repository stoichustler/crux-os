...
#define rpmsg_port_spi_crc16(hdr)                                                                  \
...
static void rpmsg_port_spi_register_cb(FAR struct rpmsg_port_s *port, rpmsg_port_rx_cb_t callback);
static void rpmsg_port_spi_slave_select(FAR struct spi_slave_dev_s *dev, bool selected);
static void rpmsg_port_spi_slave_cmddata(FAR struct spi_slave_dev_s *dev, bool data);
static size_t rpmsg_port_spi_slave_getdata(FAR struct spi_slave_dev_s *dev, FAR const void **data);
static size_t rpmsg_port_spi_slave_receive(FAR struct spi_slave_dev_s *dev, FAR const void *data,
										   size_t nwords);
static void rpmsg_port_spi_slave_notify(FAR struct spi_slave_dev_s *dev, spi_slave_state_t state);
...
static void rpmsg_port_spi_exchange(FAR struct rpmsg_port_spi_s *rpspi)
{
	...
	} else if (rpspi->txavail > 0 && rpmsg_port_queue_nused(&rpspi->port.txq) > 0) {
	...
	SPIS_CTRLR_ENQUEUE(rpspi->spictrlr, txhdr, BYTES2WORDS(rpspi, rpspi->cmdhdr->len));
...
static void rpmsg_port_spi_notify_tx_ready(FAR struct rpmsg_port_s *port)
{
	FAR struct rpmsg_port_spi_s *rpspi = container_of(port, struct rpmsg_port_spi_s, port);
...
static size_t rpmsg_port_spi_slave_getdata(FAR struct spi_slave_dev_s *dev, FAR const void **data)
{
	FAR struct rpmsg_port_spi_s *rpspi = container_of(dev, struct rpmsg_port_spi_s, spislv);
...
static void rpmsg_port_spi_slave_notify(FAR struct spi_slave_dev_s *dev, spi_slave_state_t state)
{
	...
	if (rpspi->rxhdr->cmd != RPMSG_PORT_SPI_CMD_AVAIL) {
		...
		rpspi->rxhdr = rpmsg_port_queue_get_available_buffer(&rpspi->port.rxq, false);
...
static int rpmsg_port_spi_mreq_handler(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset,
									   FAR void *arg)
{
...
static void rpmsg_port_spi_process_packet(FAR struct rpmsg_port_spi_s *rpspi,
										  FAR struct rpmsg_port_header_s *rxhdr)
{
...
static int rpmsg_port_spi_init_gpio(FAR struct ioexpander_dev_s *ioe, FAR uint8_t *gpio,
									uint8_t pin, int invert, ioe_callback_t callback,
									FAR void *args)
{
	int direction = callback ? IOEXPANDER_DIRECTION_IN_PULLDOWN : IOEXPANDER_DIRECTION_OUT;
	int ret;

	ret = IOEXP_SETOPTION(ioe, pin, IOEXPANDER_OPTION_INVERT, (FAR void *)invert);
	if (ret < 0) {
	...
	if (direction == IOEXPANDER_DIRECTION_IN_PULLDOWN) {
		int intcfg =
			invert == IOEXPANDER_VAL_INVERT ? IOEXPANDER_VAL_FALLING : IOEXPANDER_VAL_RISING;
		FAR void *ptr;

		ret = IOEXP_SETOPTION(ioe, pin, IOEXPANDER_OPTION_INTCFG, (FAR void *)intcfg);
		if (ret < 0) {
...
static int rpmsg_port_spi_init_hardware(FAR struct rpmsg_port_spi_s *rpspi,
										...
										FAR struct ioexpander_dev_s *ioe)
{
	...
	ret = rpmsg_port_spi_init_gpio(ioe, &rpspi->mreq, spicfg->mreq_pin, spicfg->mreq_invert,
	...
	ret = rpmsg_port_spi_init_gpio(ioe, &rpspi->sreq, spicfg->sreq_pin, spicfg->sreq_invert, NULL,
								   NULL);
	if (ret < 0) {
...
int rpmsg_port_spi_slave_initialize(FAR const struct rpmsg_port_config_s *cfg,
									FAR const struct rpmsg_port_spi_config_s *spicfg,
									FAR struct spi_slave_ctrlr_s *spictrlr,
									FAR struct ioexpander_dev_s *ioe)
{
	...
	rpspi->cmdhdr = rpmsg_port_queue_get_available_buffer(&rpspi->port.txq, true);
	rpspi->rxhdr = rpmsg_port_queue_get_available_buffer(&rpspi->port.rxq, true);
	DEBUGASSERT(rpspi->cmdhdr != NULL && rpspi->rxhdr != NULL);

	rpspi->rxthres =
		rpmsg_port_queue_navail(&rpspi->port.rxq) * CONFIG_RPMSG_PORT_SPI_RX_THRESHOLD / 100;
	...
	argv[1] = arg1;
	argv[2] = NULL;
	ret = kthread_create("rpmsg-spi-slv", CONFIG_RPMSG_PORT_SPI_THREAD_PRIORITY,
						 CONFIG_RPMSG_PORT_SPI_THREAD_STACKSIZE, rpmsg_port_spi_thread, argv);
	if (ret < 0) {
