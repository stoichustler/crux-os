...
static void rpmsg_port_spi_exchange(FAR struct rpmsg_port_spi_s *rpspi)
{
	...
	SPI_EXCHANGE(rpspi->spi, txhdr, rpspi->rxhdr, BYTES2WORDS(rpspi, rpspi->cmdhdr->len));
...
out:
	...
	} else if (rpspi->connected && rpspi->txavail > 0 &&
...
static int rpmsg_port_spi_sreq_handler(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset,
									   FAR void *arg)
{
	FAR struct rpmsg_port_spi_s *rpspi = arg;
										...
										FAR struct spi_dev_s *spi, FAR struct ioexpander_dev_s *ioe)
{
	...
	ret = rpmsg_port_spi_init_gpio(ioe, &rpspi->mreq, spicfg->mreq_pin, spicfg->mreq_invert, NULL,
								   NULL);
	if (ret < 0) {
	...
	ret = rpmsg_port_spi_init_gpio(ioe, &rpspi->sreq, spicfg->sreq_pin, spicfg->sreq_invert,
...
int rpmsg_port_spi_initialize(FAR const struct rpmsg_port_config_s *cfg,
							  FAR const struct rpmsg_port_spi_config_s *spicfg,
							  FAR struct spi_dev_s *spi, FAR struct ioexpander_dev_s *ioe)
{
	FAR struct rpmsg_port_spi_s *rpspi;
	...
	argv[1] = arg1;
	argv[2] = NULL;
	ret = kthread_create("rpmsg-spi", CONFIG_RPMSG_PORT_SPI_THREAD_PRIORITY,
