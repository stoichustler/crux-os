...
static int rpmsg_router_hub_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
...
static void rpmsg_router_hub_bound(FAR struct rpmsg_endpoint *ept)
{
	...
	ret = rpmsg_create_ept(src_ept, src_ept->rdev, src_ept->name, RPMSG_ADDR_ANY,
						   src_ept->dest_addr, rpmsg_router_hub_cb, rpmsg_router_hub_unbind);
...
static bool rpmsg_router_hub_match(FAR struct rpmsg_device *rdev, FAR void *priv,
								   FAR const char *name, uint32_t dest)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (strncmp(name + RPMSG_ROUTER_NAME_PREFIX_LEN, hub->cpuname[1 - i],
...
static void rpmsg_router_hub_bind(FAR struct rpmsg_device *rdev, FAR void *priv,
								  FAR const char *name, uint32_t dest)
{
	FAR struct rpmsg_router_hub_s *hub = priv;
	...
	snprintf(dst_name, RPMSG_NAME_SIZE, RPMSG_ROUTER_NAME_PREFIX "%s%s", hub->cpuname[i],
			 name + RPMSG_ROUTER_NAME_PREFIX_LEN + strlen(hub->cpuname[1 - i]));
	...
	ret = rpmsg_create_ept(dst_ept, dst_rdev, dst_name, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
...
static void rpmsg_router_bound(FAR struct rpmsg_endpoint *ept)
{
	...
	if (!is_rpmsg_ept_ready(&hub->ept[0]) || !is_rpmsg_ept_ready(&hub->ept[1])) {
		return;
	}
	...
	for (i = 0; i < 2; i++) {
		...
		msg.tx_len =
			MIN(rpmsg_get_rx_buffer_size(&hub->ept[i]), rpmsg_get_tx_buffer_size(&hub->ept[1 - i]));
		msg.rx_len =
			MIN(rpmsg_get_tx_buffer_size(&hub->ept[i]), rpmsg_get_rx_buffer_size(&hub->ept[1 - i]));
...
static void rpmsg_router_created(FAR struct rpmsg_device *rdev, FAR void *priv)
{
	FAR struct rpmsg_router_hub_s *hub = priv;
	...
	for (i = 0; i < 2; i++) {
		...
		snprintf(name, RPMSG_NAME_SIZE, RPMSG_ROUTER_NAME "%s", hub->cpuname[1 - i]);

		ret = rpmsg_create_ept(&hub->ept[i], rdev, name, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
...
static void rpmsg_router_destroy(FAR struct rpmsg_device *rdev, FAR void *priv)
{
	FAR struct rpmsg_router_hub_s *hub = priv;
	struct rpmsg_router_s msg;
	int i;
...
int rpmsg_router_hub_init(FAR const char *edge0, FAR const char *edge1)
{
	...
	ret = rpmsg_register_callback(hub, rpmsg_router_created, rpmsg_router_destroy,
								  rpmsg_router_hub_match, rpmsg_router_hub_bind);
	if (ret < 0) {
