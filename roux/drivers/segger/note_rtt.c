...
static void notertt_add(FAR struct note_driver_s *drv, FAR const void *note, size_t len);
...
static void notertt_add(FAR struct note_driver_s *drv, FAR const void *buf, size_t notelen)
{
...
{
	lib_rttoutstream_open(&g_notertt.stream, CONFIG_NOTE_RTT_CHANNEL,
