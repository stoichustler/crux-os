...
static void note_sysview_start(FAR struct note_driver_s *drv, FAR struct tcb_s *tcb);
static void note_sysview_stop(FAR struct note_driver_s *drv, FAR struct tcb_s *tcb);
#ifdef CONFIG_SCHED_INSTRUMENTATION_SWITCH
static void note_sysview_suspend(FAR struct note_driver_s *drv, FAR struct tcb_s *tcb);
static void note_sysview_resume(FAR struct note_driver_s *drv, FAR struct tcb_s *tcb);
#endif
#ifdef CONFIG_SCHED_INSTRUMENTATION_IRQHANDLER
static void note_sysview_irqhandler(FAR struct note_driver_s *drv, int irq, FAR void *handler,
									bool enter);
#endif
#ifdef CONFIG_SCHED_INSTRUMENTATION_SYSCALL
static void note_sysview_syscall_enter(FAR struct note_driver_s *drv, int nr, int argc,
									   va_list *ap);
static void note_sysview_syscall_leave(FAR struct note_driver_s *drv, int nr, uintptr_t result);
#endif
#ifdef CONFIG_SCHED_INSTRUMENTATION_HEAP
static void note_sysview_heap(FAR struct note_driver_s *drv, uint8_t event, FAR void *heap,
							  FAR void *mem, size_t size, size_t curused);
#endif
#ifdef CONFIG_SCHED_INSTRUMENTATION_WDOG
static void note_sysview_wdog(FAR struct note_driver_s *drv, uint8_t event, FAR void *handler,
							  FAR const void *arg);
#endif

#ifdef CONFIG_SCHED_INSTRUMENTATION_DUMP
static void note_sysview_vprintf(FAR struct note_driver_s *drv, uintptr_t ip, FAR const char *fmt,
								 va_list va);
#endif
...
static void note_sysview_send_description(void)
{
	...
	SEGGER_SYSVIEW_SendSysDesc("O=ROUX");
}
...
static void note_sysview_irqhandler(FAR struct note_driver_s *drv, int irq, FAR void *handler,
									bool enter)
{
	FAR struct note_sysview_driver_s *driver = (FAR struct note_sysview_driver_s *)drv;
...
#ifdef CONFIG_SCHED_INSTRUMENTATION_SYSCALL
static void note_sysview_syscall_enter(FAR struct note_driver_s *drv, int nr, int argc, va_list *ap)
{
	FAR struct note_sysview_driver_s *driver = (FAR struct note_sysview_driver_s *)drv;
	...
	if (NOTE_FILTER_SYSCALLMASK_ISSET(nr, &driver->driver.filter.syscall_mask) == 0) {
		/* Set the name marker */
		...
		if (NOTE_FILTER_SYSCALLMASK_ISSET(0, &driver->driver.filter.syscall_mask) == 0) {
			NOTE_FILTER_SYSCALLMASK_SET(0, &driver->driver.filter.syscall_mask);
		}
	}
...
int note_sysview_initialize(void)
{
	...
	SEGGER_SYSVIEW_Init(freq, freq, &g_sysview_trace_api, note_sysview_send_description);
	...
	syslog(LOG_NOTICE, "SEGGER RTT Control Block Address: %#" PRIxPTR "\n",
		   (uintptr_t)&_SEGGER_RTT + CONFIG_SEGGER_RTT_UNCACHED_OFF);
	return ret;
}
