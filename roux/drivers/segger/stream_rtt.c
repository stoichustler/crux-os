...
{
	FAR struct lib_rttoutstream_s *stream = (FAR struct lib_rttoutstream_s *)self;
...
static ssize_t rttstream_puts(FAR struct lib_outstream_s *self, FAR const void *buf, size_t len)
{
	FAR struct lib_rttoutstream_s *stream = (FAR struct lib_rttoutstream_s *)self;
	ssize_t ret;
...
{
	FAR struct lib_rttinstream_s *stream = (FAR struct lib_rttinstream_s *)self;
	int ch = -1;
...
static ssize_t rttstream_gets(FAR struct lib_instream_s *self, FAR void *buffer, size_t size)
{
	FAR struct lib_rttinstream_s *stream = (FAR struct lib_rttinstream_s *)self;
	ssize_t ret;
...
void lib_rttoutstream_open(FAR struct lib_rttoutstream_s *stream, int channel, size_t bufsize)
{
	if (channel) {
		...
		SEGGER_RTT_ConfigUpBuffer(channel, stream->name, stream->buffer, bufsize,
...
void lib_rttoutstream_close(FAR struct lib_rttoutstream_s *stream)
{
	if (stream->channel) {
		...
		SEGGER_RTT_ConfigUpBuffer(stream->channel, NULL, NULL, 0, SEGGER_RTT_MODE_DEFAULT);
	}
}
...
void lib_rttinstream_open(FAR struct lib_rttinstream_s *stream, int channel, size_t bufsize)
{
	if (channel) {
		...
		DEBUGASSERT(stream->buffer);
		snprintf(stream->name, sizeof(stream->name), "rtt%d", channel);
		SEGGER_RTT_ConfigDownBuffer(channel, stream->name, stream->buffer, bufsize,
									SEGGER_RTT_MODE_DEFAULT);
	}
...
void lib_rttinstream_close(FAR struct lib_rttinstream_s *stream)
{
	if (stream->channel) {
		kmm_free(stream->buffer);
		SEGGER_RTT_ConfigDownBuffer(stream->channel, NULL, NULL, 0, SEGGER_RTT_MODE_DEFAULT);
	}
}
