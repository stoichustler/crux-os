...
static int serial_rtt_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int serial_rtt_receive(FAR struct uart_dev_s *dev, FAR unsigned int *status);
...
static char SERIAL_RTT_BUFFER_SECTION g_rtt1_up_buffer[CONFIG_SEGGER_RTT1_BUFFER_SIZE_UP];
static char SERIAL_RTT_BUFFER_SECTION g_rtt1_down_buffer[CONFIG_SEGGER_RTT1_BUFFER_SIZE_DOWN];
...
static char SERIAL_RTT_BUFFER_SECTION g_rtt2_up_buffer[CONFIG_SEGGER_RTT2_BUFFER_SIZE_UP];
static char SERIAL_RTT_BUFFER_SECTION g_rtt2_down_buffer[CONFIG_SEGGER_RTT2_BUFFER_SIZE_DOWN];
...
static int serial_rtt_attach(FAR struct uart_dev_s *dev)
{
	...
	wd_start(&rtt->wdog, USEC2TICK(CONFIG_SERIAL_RTT_POLLING_INTERVAL), serial_rtt_timeout,
			 (wdparm_t)dev);
	return OK;
}
...
static void serial_rtt_dmasend(FAR struct uart_dev_s *dev)
{
	...
	if (len == xfer->length && xfer->nlength) {
		len += SEGGER_RTT_WriteNoLock(rtt->channel, xfer->nbuffer, xfer->nlength);
	}
...
static void serial_rtt_dmareceive(FAR struct uart_dev_s *dev)
{
	...
	if (len == xfer->length && xfer->nbuffer && SEGGER_RTT_HasData(rtt->channel)) {
		len += SEGGER_RTT_ReadNoLock(rtt->channel, xfer->nbuffer, xfer->nlength);
	}
...
static void serial_rtt_timeout(wdparm_t arg)
{
	...
	wd_start(&rtt->wdog, USEC2TICK(CONFIG_SERIAL_RTT_POLLING_INTERVAL), serial_rtt_timeout, arg);
}
...
static void serial_rtt_register(FAR const char *name, FAR struct serial_rtt_s *rtt)
{
	SEGGER_RTT_ConfigUpBuffer(rtt->channel, name, rtt->up_buffer, rtt->uart.xmit.size,
							  SEGGER_RTT_MODE_NO_BLOCK_TRIM);
	SEGGER_RTT_ConfigDownBuffer(rtt->channel, name, rtt->down_buffer, rtt->uart.recv.size,
								SEGGER_RTT_MODE_NO_BLOCK_TRIM);
