...
#define to_clk_fd(_clk) (FAR struct clk_fractional_divider_s *)(_clk->private_data)

/****************************************************************************
 * Private Functions
 ****************************************************************************/

static uint32_t clk_fd_recalc_rate(FAR struct clk_s *clk, uint32_t parent_rate)
{
...
static uint32_t clk_fd_round_rate(FAR struct clk_s *clk, uint32_t rate, FAR uint32_t *prate)
{
	FAR struct clk_fractional_divider_s *fd = to_clk_fd(clk);
	...
	do {
		...
		if ((m % 2 != 0) && (fd->flags & CLK_FRAC_MUL_NEED_EVEN)) {
			m *= 2;
			n *= 2;
		}
...
static int clk_fd_set_rate(FAR struct clk_s *clk, uint32_t rate, uint32_t parent_rate)
{
	FAR struct clk_fractional_divider_s *fd = to_clk_fd(clk);
	uint32_t mmask = MASK(fd->mwidth) << fd->mshift;
	uint32_t nmask = MASK(fd->nwidth) << fd->nshift;
												  ...
												  uint8_t clk_divider_flags)
{
	...
	return clk_register(name, parent_names, num_parents, flags, &g_clk_fractional_divider_ops, &fd,
						sizeof(fd));
}
