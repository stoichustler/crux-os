...
static bool mux_is_better_rate(uint32_t rate, uint32_t now, uint32_t best, uint8_t flags)
{
...
static uint32_t clk_mux_determine_rate(FAR struct clk_s *clk, uint32_t rate,
								   ...
								   uint8_t width, uint8_t clk_mux_flags)
{
	...
	if (clk_mux_flags & CLK_MUX_READ_ONLY) {
		return clk_register(name, parent_names, num_parents, flags, &g_clk_mux_ro_ops, &mux,
							sizeof(mux));
	} else {
		return clk_register(name, parent_names, num_parents, flags, &g_clk_mux_ops, &mux,
							sizeof(mux));
	}
}
