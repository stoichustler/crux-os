...
#define to_clk_divider(_clk) (FAR struct clk_divider_s *)(_clk->private_data)
...
static uint32_t divider_recalc_rate(uint32_t parent_rate, uint32_t val, uint16_t flags)
{
	uint32_t div;
...
static uint32_t clk_divider_recalc_rate(FAR struct clk_s *clk, uint32_t parent_rate)
{
...
static uint32_t _div_round_up(uint32_t parent_rate, uint32_t rate, uint16_t flags)
{
...
static uint32_t _div_round_closest(uint32_t parent_rate, uint32_t rate, uint16_t flags)
{
	uint32_t up;
	uint32_t down;
	uint32_t up_rate;
...
static uint32_t _div_round(uint32_t parent_rate, uint32_t rate, uint16_t flags)
{
...
static bool _is_best_div(uint32_t rate, uint32_t now, uint32_t best, uint16_t flags)
{
	if (flags & CLK_DIVIDER_ROUND_CLOSEST) {
...
static uint32_t clk_divider_bestdiv(FAR struct clk_s *clk, uint32_t rate,
									uint32_t *best_parent_rate, uint8_t width)
{
	uint32_t i;
	uint32_t bestdiv = 0;
	uint32_t maxdiv;
	...
	for (i = _next_div(mindiv, divider->flags); i <= maxdiv; i = _next_div(i, divider->flags)) {
...
static uint32_t divider_round_rate(FAR struct clk_s *clk, uint32_t rate, uint32_t *prate,
								   uint8_t width)
{
	uint32_t div;
...
static uint32_t clk_divider_round_rate(FAR struct clk_s *clk, uint32_t rate, uint32_t *prate)
{
	FAR struct clk_divider_s *divider = to_clk_divider(clk);
...
static int32_t divider_get_val(uint32_t rate, uint32_t parent_rate, uint8_t width, uint16_t flags)
{
	uint32_t div;
	uint32_t value;
...
static int clk_divider_set_rate(FAR struct clk_s *clk, uint32_t rate, uint32_t parent_rate)
{
	FAR struct clk_divider_s *divider = to_clk_divider(clk);
	int32_t value;
	uint32_t val;
...
FAR struct clk_s *clk_register_divider(FAR const char *name, FAR const char *parent_name,
									   ...
									   uint16_t clk_divider_flags)
{
	...
	return clk_register(name, parent_names, num_parents, flags, &g_clk_divider_ops, &div,
						sizeof(div));
}
