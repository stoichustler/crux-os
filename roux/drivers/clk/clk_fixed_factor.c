...
#define to_clk_fixed_factor(_clk) (FAR struct clk_fixed_factor_s *)(_clk->private_data)

/****************************************************************************
 * Private Functions
 ****************************************************************************/

static uint32_t clk_factor_recalc_rate(FAR struct clk_s *clk, uint32_t parent_rate)
{
...
static uint32_t clk_factor_round_rate(FAR struct clk_s *clk, uint32_t rate, FAR uint32_t *prate)
{
	FAR struct clk_fixed_factor_s *fixed = to_clk_fixed_factor(clk);
											...
											uint8_t flags, uint8_t mult, uint8_t div)
{
	...
	return clk_register(name, parent_names, num_parents, flags, &g_clk_fixed_factor_ops, &fixed,
						sizeof(fixed));
}
