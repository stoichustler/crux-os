...
static int clk_fetch_parent_index(FAR struct clk_s *clk, FAR struct clk_s *parent);
...
static void clk_calc_subtree(FAR struct clk_s *clk, uint32_t new_rate, FAR struct clk_s *new_parent,
							 uint8_t p_index);
static FAR struct clk_s *clk_calc_new_rates(FAR struct clk_s *clk, uint32_t rate);
...
static FAR struct clk_s *__clk_lookup(FAR const char *name, FAR struct clk_s *clk);
...
static int clk_procfs_open(FAR struct file *filep, FAR const char *relpath, int oflags,
						   mode_t mode);
static int clk_procfs_close(FAR struct file *filep);
static ssize_t clk_procfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static int clk_procfs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode)
{
...
static size_t clk_procfs_printf(FAR char *buffer, size_t buflen, FAR off_t *pos,
								FAR const char *fmt, ...)
{
...
static size_t clk_procfs_show_subtree(FAR struct clk_s *clk, int level, FAR char *buffer,
									  size_t buflen, FAR off_t *pos, FAR irqstate_t *flags)
{
	...
	ret = clk_procfs_printf(buffer, buflen, pos, "%*s%-*s %11d %11u %11d\n", level * 2, "",
							40 - level * 2, clk_get_name(clk), clk_is_enabled(clk),
							clk_get_rate(clk), clk_get_phase(clk));
	buffer += ret;
	buflen -= ret;
	...
	if (buflen > 0) {
		...
		{
			ret = clk_procfs_show_subtree(child, level + 1, buffer, buflen, pos, flags);
			buffer += ret;
			buflen -= ret;
...
static size_t clk_procfs_showtree(FAR char *buffer, size_t buflen, FAR off_t *pos)
{
...
static ssize_t clk_procfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	ret = clk_procfs_printf(buffer, buflen, &pos, "%8s%44s%12s%12s\n", "clock", "enable_cnt",
							"rate", "phase");
	buffer += ret;
	buflen -= ret;
...
static int __clk_register(FAR struct clk_s *clk)
{
	...
	if (clk->ops->set_rate_and_parent && !(clk->ops->set_parent && clk->ops->set_rate)) {
		return -EINVAL;
	}
	...
	list_for_every_entry_safe(&g_clk_orphan_list, orphan, temp, struct clk_s, node)
	{
...
FAR struct clk_s *clk_get_parent_by_index(FAR struct clk_s *clk, uint8_t index)
{
...
FAR struct clk_s *clk_register(FAR const char *name, FAR const char *const *parent_names,
							   uint8_t num_parents, uint8_t flags, FAR const struct clk_ops_s *ops,
							   FAR void *private_data, size_t private_size)
{
	...
	for (i = 0; i < num_parents; i++) {
		if (flags & CLK_PARENT_NAME_IS_STATIC) {
			...
			strlcpy((FAR char *)clk->parent_names[i], parent_names[i], len - off);
