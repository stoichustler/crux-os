...
#define to_clk_multiplier(_clk) (FAR struct clk_multiplier_s *)(_clk->private_data)
...
static uint32_t _get_mult(FAR struct clk_multiplier_s *multiplier, uint32_t val)
{
	if (multiplier->flags & CLK_MULT_ONE_BASED) {
		return val;
	}
...
static uint32_t _get_val(FAR struct clk_multiplier_s *multiplier, uint32_t mult)
{
	if (multiplier->flags & CLK_MULT_ONE_BASED) {
		return mult;
	}
...
static uint32_t clk_multiplier_recalc_rate(FAR struct clk_s *clk, uint32_t parent_rate)
{
...
static bool __is_best_rate(uint32_t rate, uint32_t new, uint32_t best, uint16_t flags)
{
...
static uint32_t clk_multiplier_round_rate(FAR struct clk_s *clk, uint32_t rate, FAR uint32_t *prate)
{
	return *prate * clk_multiplier_bestmult(clk, rate, prate);
}

static int clk_multiplier_set_rate(FAR struct clk_s *clk, uint32_t rate, uint32_t parent_rate)
{
	FAR struct clk_multiplier_s *multiplier = to_clk_multiplier(clk);
	uint32_t mult;
	uint32_t value;
										  ...
										  uint8_t clk_multiplier_flags)
{
	...
	return clk_register(name, parent_names, num_parents, flags, &g_clk_multiplier_ops, &mult,
						sizeof(mult));
}
