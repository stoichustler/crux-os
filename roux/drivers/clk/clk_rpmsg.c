...
static FAR struct clk_rpmsg_s *clk_rpmsg_get_clk(FAR struct rpmsg_endpoint *ept,
												 FAR const char *name);

static int clk_rpmsg_enable_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									uint32_t src, FAR void *priv);
static int clk_rpmsg_disable_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									 uint32_t src, FAR void *priv);
static int clk_rpmsg_getrate_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									 uint32_t src, FAR void *priv);
static int clk_rpmsg_roundrate_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									   uint32_t src, FAR void *priv);
static int clk_rpmsg_setrate_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									 uint32_t src, FAR void *priv);
static int clk_rpmsg_setphase_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									  uint32_t src, FAR void *priv);
static int clk_rpmsg_getphase_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									  uint32_t src, FAR void *priv);
static int clk_rpmsg_isenabled_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									   uint32_t src, FAR void *priv);

static void clk_rpmsg_client_created(FAR struct rpmsg_device *rdev, FAR void *priv_);
static void clk_rpmsg_client_destroy(FAR struct rpmsg_device *rdev, FAR void *priv_);

static bool clk_rpmsg_server_match(FAR struct rpmsg_device *rdev, FAR void *priv_,
								   FAR const char *name, uint32_t dest);
static void clk_rpmsg_server_bind(FAR struct rpmsg_device *rdev, FAR void *priv_,
								  FAR const char *name, uint32_t dest);

static int clk_rpmsg_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
							uint32_t src, FAR void *priv);

static int64_t clk_rpmsg_sendrecv(FAR struct rpmsg_endpoint *ept, uint32_t command,
								  FAR struct clk_rpmsg_header_s *msg, int32_t len);
...
static int clk_rpmsg_set_rate(FAR struct clk_s *clk, uint32_t rate, uint32_t parent_rate);
static uint32_t clk_rpmsg_recalc_rate(FAR struct clk_s *clk, uint32_t parent_rate);
...
static FAR struct clk_rpmsg_client_s *clk_rpmsg_get_priv(FAR const char *name)
{
	...
	list_for_every_entry(&g_clk_rpmsg_priv, priv, struct clk_rpmsg_client_s, node)
	{
	...
	rpmsg_register_callback(priv, clk_rpmsg_client_created, clk_rpmsg_client_destroy, NULL, NULL);
	return priv;
...
static void clk_rpmsg_server_ept_release(FAR struct rpmsg_endpoint *ept)
{
	...
	list_for_every_entry_safe(&priv->clk_list, clkrp, clkrp_tmp, struct clk_rpmsg_s, node)
	{
		if (clkrp->enable) {
								  ...
								  FAR const char *name, uint32_t dest)
{
	...
	rpmsg_create_ept(&priv->ept, rdev, name, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY, clk_rpmsg_ept_cb,
					 rpmsg_destroy_ept);
}
...
static void clk_rpmsg_client_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	...
	if (!strcmp(priv->cpuname, rpmsg_get_cpuname(rdev))) {
		...
		rpmsg_create_ept(&priv->ept, rdev, CLK_RPMSG_EPT_NAME, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
							...
							uint32_t src, FAR void *priv)
{
	...
	if (hdr->response) {
		FAR struct clk_rpmsg_cookie_s *cookie = (struct clk_rpmsg_cookie_s *)(uintptr_t)hdr->cookie;
		if (cookie) {
			cookie->result = hdr->result;
			nxsem_post(&cookie->sem);
			ret = 0;
		}
	} else if (cmd < nitems(g_clk_rpmsg_handler) && g_clk_rpmsg_handler[cmd]) {
		hdr->response = 1;
...
static int clk_rpmsg_enable(FAR struct clk_s *clk)
{
	...
	return clk_rpmsg_sendrecv(ept, CLK_RPMSG_ENABLE, (struct clk_rpmsg_header_s *)msg, len);
}
...
static void clk_rpmsg_disable(FAR struct clk_s *clk)
{
	...
	clk_rpmsg_sendrecv(ept, CLK_RPMSG_DISABLE, (struct clk_rpmsg_header_s *)msg, len);
}
...
static int clk_rpmsg_is_enabled(FAR struct clk_s *clk)
{
	...
	return clk_rpmsg_sendrecv(ept, CLK_RPMSG_ISENABLED, (struct clk_rpmsg_header_s *)msg, len);
}
									 ...
									 FAR uint32_t *parent_rate)
{
	...
	ret = clk_rpmsg_sendrecv(ept, CLK_RPMSG_ROUNDRATE, (struct clk_rpmsg_header_s *)msg, len);
	if (ret < 0) {
		return 0;
	}
...
static int clk_rpmsg_set_rate(FAR struct clk_s *clk, uint32_t rate, uint32_t parent_rate)
{
	...
	return clk_rpmsg_sendrecv(ept, CLK_RPMSG_SETRATE, (struct clk_rpmsg_header_s *)msg, len);
}
...
static uint32_t clk_rpmsg_recalc_rate(FAR struct clk_s *clk, uint32_t parent_rate)
{
	...
	ret = clk_rpmsg_sendrecv(ept, CLK_RPMSG_GETRATE, (struct clk_rpmsg_header_s *)msg, len);
	if (ret < 0) {
		return 0;
	}
...
static int clk_rpmsg_get_phase(FAR struct clk_s *clk)
{
	...
	return clk_rpmsg_sendrecv(ept, CLK_RPMSG_GETPHASE, (struct clk_rpmsg_header_s *)msg, len);
}
...
static int clk_rpmsg_set_phase(FAR struct clk_s *clk, int degrees)
{
	...
	return clk_rpmsg_sendrecv(ept, CLK_RPMSG_SETPHASE, (struct clk_rpmsg_header_s *)msg, len);
}
...
{
	return rpmsg_register_callback(NULL, NULL, NULL, clk_rpmsg_server_match, clk_rpmsg_server_bind);
}
