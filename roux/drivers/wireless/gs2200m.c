...
#include <roux/ascii.h>
#include <roux/arch.h>
#include <roux/spi/spi.h>
#include <roux/kmalloc.h>
#include <roux/wqueue.h>
#include <roux/mutex.h>
#include <roux/signal.h>
#include <roux/wireless/wireless.h>
#include <roux/wireless/gs2200m.h>
#include <roux/net/netdev.h>
...
static ssize_t gs2200m_read(FAR struct file *filep, FAR char *buff, size_t len);
static ssize_t gs2200m_write(FAR struct file *filep, FAR const char *buff, size_t len);
static int gs2200m_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int gs2200m_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static struct evt_code_s _evt_table[] = {{"OK", TYPE_OK},
...
static void _release_pkt_dat(FAR struct gs2200m_dev_s *dev, FAR struct pkt_dat_s *pkt_dat)
{
	int i;
	...
	if (pkt_dat->len) {
		...
		if (pkt_dat->type == TYPE_BULK_DATA_TCP || pkt_dat->type == TYPE_BULK_DATA_UDP) {
...
static bool _copy_data_from_pkt(FAR struct gs2200m_dev_s *dev, struct gs2200m_recv_msg *msg)
{
	FAR struct pkt_dat_s *pkt_dat;
...
static ssize_t gs2200m_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
...
static ssize_t gs2200m_write(FAR struct file *filep, FAR const char *buffer, size_t len)
{
...
static void _write_data(FAR struct gs2200m_dev_s *dev, FAR uint8_t *buf, FAR uint16_t len)
{
...
static void _read_data(FAR struct gs2200m_dev_s *dev, FAR uint8_t *buff, FAR uint16_t len)
{
	memset(buff, 0, len);
...
enum spi_status_e gs2200m_hal_write(FAR struct gs2200m_dev_s *dev, FAR const void *data,
									uint16_t txlen)
{
...
retry:
	...
	if (dev->lower->dready(NULL)) {
		...
		if (!work_available(&dev->irq_work)) {
			...
			work_queue(GS2200MWORK, &dev->irq_work, gs2200m_irq_worker, (FAR void *)dev, 0);
		}
...
enum spi_status_e gs2200m_hal_read(FAR struct gs2200m_dev_s *dev, FAR uint8_t *data,
								   FAR uint16_t *len)
{
	uint8_t hdr[8];
	int i;
...
static void _parse_pkt_in_s0(FAR struct pkt_ctx_s *pkt_ctx, FAR struct pkt_dat_s *pkt_dat)
{
	switch (*(pkt_ctx->ptr)) {
	case ASCII_CR:
	case ASCII_LF:
...
static void _parse_pkt_in_s1(FAR struct pkt_ctx_s *pkt_ctx, FAR struct pkt_dat_s *pkt_dat)
{
	FAR char *msg;
	int msize;
	int n;
...
static void _parse_pkt_in_s2(FAR struct pkt_ctx_s *pkt_ctx, FAR struct pkt_dat_s *pkt_dat)
{
...
static void _parse_pkt_in_s3(FAR struct pkt_ctx_s *pkt_ctx, FAR struct pkt_dat_s *pkt_dat)
{
	ASSERT(pkt_dat);
...
static void _parse_pkt_in_s4(FAR struct pkt_ctx_s *pkt_ctx, FAR struct pkt_dat_s *pkt_dat)
{
	char addr[17];
	char port[6];
	int n;
...
static enum pkt_type_e _parse_pkt(FAR uint8_t *p, uint16_t len, FAR struct pkt_dat_s *pkt_dat)
{
...
static void _dup_pkt_dat_and_notify(FAR struct gs2200m_dev_s *dev, FAR struct pkt_dat_s *pkt_dat0)
{
	FAR struct pkt_dat_s *pkt_dat;
	uint8_t c;
										...
										FAR struct pkt_dat_s *pkt_dat)
{
	...
	if (pkt_dat) {
		...
		if (t == TYPE_BULK_DATA_TCP || t == TYPE_BULK_DATA_UDP) {
			/* Update total bulk data size */
...
static enum pkt_type_e gs2200m_send_cmd(FAR struct gs2200m_dev_s *dev, FAR char *cmd,
										FAR struct pkt_dat_s *pkt_dat)
{
	enum spi_status_e s;
...
static enum pkt_type_e gs2200m_send_cmd2(FAR struct gs2200m_dev_s *dev, FAR char *cmd)
{
...
static enum pkt_type_e gs2200m_set_opmode(FAR struct gs2200m_dev_s *dev, uint8_t mode)
{
	enum pkt_type_e t;
	char cmd[20];
...
static enum pkt_type_e gs2200m_get_mac(FAR struct gs2200m_dev_s *dev)
{
	...
	n = sscanf(pkt_dat.msg[0],
			   "%2" PRIx32 ":%2" PRIx32 ":%2" PRIx32 ":%2" PRIx32 ":%2" PRIx32 ":%2" PRIx32,
...
static enum pkt_type_e gs2200m_enable_dhcpc(FAR struct gs2200m_dev_s *dev, uint8_t on)
{
	char cmd[16];
...
static enum pkt_type_e gs2200m_calc_key(FAR struct gs2200m_dev_s *dev, FAR char *ssid,
										FAR char *psk)
{
	char cmd[80];
...
static enum pkt_type_e gs2200m_set_security(FAR struct gs2200m_dev_s *dev, uint8_t mode)
{
	char cmd[16];
...
static enum pkt_type_e gs2200m_join_network(FAR struct gs2200m_dev_s *dev, FAR char *ssid,
											uint8_t ch)
{
	struct pkt_dat_s pkt_dat;
	enum pkt_type_e r;
	...
	n = sscanf(pkt_dat.msg[1] + 1, " %[^:]:%[^:]:%[^ ]", addr[0], addr[1], addr[2]);
	ASSERT(3 == n);
...
static enum pkt_type_e gs2200m_set_addresses(FAR struct gs2200m_dev_s *dev, FAR const char *address,
											 FAR const char *netmask, FAR const char *gateway)
{
	char cmd[100];
...
static enum pkt_type_e gs2200m_enable_dhcps(FAR struct gs2200m_dev_s *dev, uint8_t on)
{
	char cmd[20];
...
static enum pkt_type_e gs2200m_set_auth(FAR struct gs2200m_dev_s *dev, int mode)
{
	char cmd[16];
...
static enum pkt_type_e gs2200m_set_wepkey(FAR struct gs2200m_dev_s *dev, FAR char *key)
{
	char cmd[32];
...
static enum pkt_type_e gs2200m_set_wpa2pf(FAR struct gs2200m_dev_s *dev, FAR char *key)
{
	char cmd[64];
...
static enum pkt_type_e gs2200m_create_clnt(FAR struct gs2200m_dev_s *dev,
										   FAR struct gs2200m_connect_msg *msg, FAR char *cid)
{
	struct pkt_dat_s pkt_dat;
	enum pkt_type_e r;
	char cmd[40];
...
static enum pkt_type_e gs2200m_start_server(FAR struct gs2200m_dev_s *dev, FAR char *port,
											bool is_tcp, FAR char *cid)
{
	struct pkt_dat_s pkt_dat;
	enum pkt_type_e r;
	char cmd[40];
										 ...
										 FAR struct gs2200m_send_msg *msg)
{
	...
	if (msg->is_tcp) {
		...
		snprintf(cmd, sizeof(cmd), "%cY%c%s:%d:%s", ASCII_ESC, msg->cid,
				 inet_ntoa_r(msg->addr.sin_addr, inetaddr, sizeof(inetaddr)),
				 NTOHS(msg->addr.sin_port), digits);
	}
	...
	s = gs2200m_hal_write(dev, (FAR char *)dev->tx_buff, msg->len + bulk_hdr_size);
...
static enum pkt_type_e gs2200m_close_conn(FAR struct gs2200m_dev_s *dev, char cid)
{
	char cmd[15];
...
static enum pkt_type_e gs2200m_enable_bulk(FAR struct gs2200m_dev_s *dev, uint8_t on)
{
	char cmd[20];
...
static enum pkt_type_e gs2200m_enable_echo(FAR struct gs2200m_dev_s *dev, uint8_t on)
{
	char cmd[9];
...
static enum pkt_type_e gs2200m_activate_wrx(FAR struct gs2200m_dev_s *dev, uint8_t on)
{
	char cmd[30];
...
static enum pkt_type_e gs2200m_powersave_wrx(FAR struct gs2200m_dev_s *dev, uint8_t on)
{
	char cmd[30];
...
static enum pkt_type_e gs2200m_syncloss(FAR struct gs2200m_dev_s *dev, int val)
{
	char cmd[30];
...
#ifdef USE_LED
static enum pkt_type_e gs2200m_set_gpio(FAR struct gs2200m_dev_s *dev, int n, int val)
{
	char cmd[24];
...
static enum pkt_type_e gs2200m_set_loglevel(FAR struct gs2200m_dev_s *dev, int level)
{
	char cmd[16];
										   ...
										   FAR struct gs2200m_name_msg *msg)
{
	...
	for (i = 1; i < pkt_dat.n - 2; i++) {
		...
		n = sscanf(pkt_dat.msg[i], "%c %7s %6s %d %d %d.%d.%d.%d", &c, type, mode, &p[0], &p[1],
				   &a[0], &a[1], &a[2], &a[3]);
		ASSERT(9 == n);

		wlinfo("[%d]: %c %s %s %d %d %d.%d.%d.%d\n", i, c, type, mode, p[0], p[1], a[0], a[1], a[2],
			   a[3]);
		...
		if (c == msg->cid) {
			...
			if (msg->local) {
				...
				snprintf(addr, sizeof(addr), "%d.%d.%d.%d", a[0], a[1], a[2], a[3]);
...
static int gs2200m_ioctl_bind(FAR struct gs2200m_dev_s *dev, FAR struct gs2200m_bind_msg *msg)
{
...
static int gs2200m_ioctl_connect(FAR struct gs2200m_dev_s *dev, FAR struct gs2200m_connect_msg *msg)
{
	enum pkt_type_e type;
	char cid = 'z';
	int ret = OK;
...
static int gs2200m_ioctl_send(FAR struct gs2200m_dev_s *dev, FAR struct gs2200m_send_msg *msg)
{
...
static int gs2200m_ioctl_recv(FAR struct gs2200m_dev_s *dev, FAR struct gs2200m_recv_msg *msg)
{
	bool cont = true;
	int ret = OK;
	uint8_t c = _cid_to_uint8(msg->cid);
...
static int gs2200m_ioctl_close(FAR struct gs2200m_dev_s *dev, FAR struct gs2200m_close_msg *msg)
{
	enum pkt_type_e type = TYPE_OK;
	int ret = OK;
...
static int gs2200m_ioctl_accept(FAR struct gs2200m_dev_s *dev, FAR struct gs2200m_accept_msg *msg)
{
	FAR struct pkt_dat_s *pkt_dat;
...
static int gs2200m_ioctl_assoc_sta(FAR struct gs2200m_dev_s *dev, FAR struct gs2200m_assoc_msg *msg)
{
	enum pkt_type_e t;
	...
	t = gs2200m_set_addresses(dev, "10.0.0.2", "255.255.255.0", "10.0.0.1");
	ASSERT(TYPE_OK == t);
#else
	/* Enable DHCP Client */
...
static int gs2200m_ioctl_assoc_ap(FAR struct gs2200m_dev_s *dev, FAR struct gs2200m_assoc_msg *msg)
{
	enum pkt_type_e t;
	...
	t = gs2200m_set_addresses(dev, "192.168.11.1", "255.255.255.0", "192.168.11.1");
	ASSERT(TYPE_OK == t);
...
static int gs2200m_ioctl_iwreq(FAR struct gs2200m_dev_s *dev, FAR struct gs2200m_ifreq_msg *msg)
{
	...
	if (msg->cmd == SIOCGIWNWID) {
		...
		n = sscanf(pkt_dat.msg[2], "BSSID=%c:%c:%c:%c:%c:%c %s", &res->u.ap_addr.sa_data[0],
				   &res->u.ap_addr.sa_data[1], &res->u.ap_addr.sa_data[2],
				   &res->u.ap_addr.sa_data[3], &res->u.ap_addr.sa_data[4],
				   &res->u.ap_addr.sa_data[5], cmd);
		ASSERT(7 == n);
		wlinfo("BSSID:%02X:%02X:%02X:%02X:%02X:%02X\n", res->u.ap_addr.sa_data[0],
			   res->u.ap_addr.sa_data[1], res->u.ap_addr.sa_data[2], res->u.ap_addr.sa_data[3],
		...
		n = sscanf(pkt_dat.msg[2], "%s CHANNEL=%" SCNd32 " %s", cmd, &res->u.freq.m, cmd2);
		ASSERT(3 == n);
...
static int gs2200m_ioctl_ifreq(FAR struct gs2200m_dev_s *dev, FAR struct gs2200m_ifreq_msg *msg)
{
	...
	case SIOCGIFHWADDR:
		getreq = true;
		memcpy(&msg->ifr.ifr_hwaddr.sa_data, dev->net_dev.d_mac.ether.ether_addr_octet, 6);
		break;

	case SIOCGIFADDR:
		getreq = true;
		memcpy(&inaddr->sin_addr, &dev->net_dev.d_ipaddr, sizeof(dev->net_dev.d_ipaddr));
		break;

	case SIOCSIFADDR:
		memcpy(&dev->net_dev.d_ipaddr, &inaddr->sin_addr, sizeof(inaddr->sin_addr));
		break;

	case SIOCSIFDSTADDR:
		memcpy(&dev->net_dev.d_draddr, &inaddr->sin_addr, sizeof(inaddr->sin_addr));
		break;

	case SIOCSIFNETMASK:
		memcpy(&dev->net_dev.d_netmask, &inaddr->sin_addr, sizeof(inaddr->sin_addr));
		break;
	...
	if (false == getreq && OK == ret) {
		...
		strlcpy(addr[0], inet_ntoa_r(in[0], inetaddr, sizeof(inetaddr)), sizeof(addr[0]));
		strlcpy(addr[1], inet_ntoa_r(in[1], inetaddr, sizeof(inetaddr)), sizeof(addr[1]));
		strlcpy(addr[2], inet_ntoa_r(in[2], inetaddr, sizeof(inetaddr)), sizeof(addr[2]));
...
static int gs2200m_ioctl_name(FAR struct gs2200m_dev_s *dev, FAR struct gs2200m_name_msg *msg)
{
	enum pkt_type_e r;
	...
	if (msg->local) {
		...
		memcpy(&msg->addr.sin_addr, &dev->net_dev.d_ipaddr, sizeof(msg->addr.sin_addr));
	}
...
static int gs2200m_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case GS2200M_IOC_CONNECT: {
		struct gs2200m_connect_msg *msg = (struct gs2200m_connect_msg *)arg;

		ret = gs2200m_ioctl_connect(dev, msg);
	} break;

	case GS2200M_IOC_SEND: {
		struct gs2200m_send_msg *msg = (struct gs2200m_send_msg *)arg;

		ret = gs2200m_ioctl_send(dev, msg);
	} break;

	case GS2200M_IOC_RECV: {
		struct gs2200m_recv_msg *msg = (struct gs2200m_recv_msg *)arg;
	...
	case GS2200M_IOC_CLOSE: {
		struct gs2200m_close_msg *msg = (struct gs2200m_close_msg *)arg;
	...
	case GS2200M_IOC_BIND: {
		struct gs2200m_bind_msg *msg = (struct gs2200m_bind_msg *)arg;
	...
	case GS2200M_IOC_ACCEPT: {
		struct gs2200m_accept_msg *msg = (struct gs2200m_accept_msg *)arg;
	...
	case GS2200M_IOC_ASSOC: {
		struct gs2200m_assoc_msg *msg = (struct gs2200m_assoc_msg *)arg;
	...
	case GS2200M_IOC_IFREQ: {
		struct gs2200m_ifreq_msg *msg = (struct gs2200m_ifreq_msg *)arg;
	...
	case GS2200M_IOC_NAME: {
		struct gs2200m_name_msg *msg = (struct gs2200m_name_msg *)arg;
...
static int gs2200m_irq(int irq, FAR void *context, FAR void *arg)
{
	...
	return work_queue(GS2200MWORK, &dev->irq_work, gs2200m_irq_worker, (FAR void *)dev, 0);
}
...
FAR void *gs2200m_register(FAR const char *devpath, FAR struct spi_dev_s *spi,
						   FAR const struct gs2200m_lower_s *lower)
{
	FAR struct gs2200m_dev_s *dev;
	int ret;
