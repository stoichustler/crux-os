 ...
 * network driver and the ROUX network.  The LP work queue performs (1)
...
#include <roux/wireless/spirit.h>
...
#if CONFIG_SPIRIT_PKTLEN > SPIRIT_MAX_FIFO_LEN && !defined(CONFIG_SPIRIT_FIFOS)
...
static int spirit_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif

static int spirit_get_mhrlen(FAR struct radio_driver_s *netdev, FAR const void *meta);
static int spirit_req_data(FAR struct radio_driver_s *netdev, FAR const void *meta,
...
int spirit_hw_initialize(FAR struct spirit_driver_s *dev, FAR struct spi_dev_s *spi);
...
static void spirit_transmit_work(FAR void *arg)
{
	...
	while (priv->txhead != NULL && priv->state == DRIVER_STATE_IDLE) {
		...
		ret = spirit_pktcommon_set_txdestaddr(spirit, pktmeta->pm_dest.pa_addr[0]);
		if (ret < 0) {
      ...
      if (pktmeta->pm_dest.pa_addr[0] == SPIRIT_BCAST_ADDRESS ||
		...
		ret = spirit_timer_setup_rxtimeout(spirit, priv->counter, priv->prescaler);
		if (ret < 0) {
			wlerr("ERROR: spirit_timer_setup_rxtimeout failed: %d\n", ret);
			goto errout_with_csma;
		}
		...
		wd_start(&priv->txtimeout, SPIRIT_TXTIMEOUT, spirit_txtimeout_expiry, (wdparm_t)priv);
	}
...
static void spirit_schedule_transmit_work(FAR struct spirit_driver_s *priv)
{
	if (priv->txhead != NULL && priv->state == DRIVER_STATE_IDLE && work_available(&priv->txwork)) {
...
static void spirit_interrupt_work(FAR void *arg)
{
	...
	if (irqstatus.IRQ_TX_FIFO_ERROR != 0 || irqstatus.IRQ_MAX_RE_TX_REACH != 0 ||
...
#warning Missing logic
		...
		if ((offset + count) > CONFIG_IOB_BUFSIZE) {
			...
			if (iob == NULL) {
				...
				if (pktmeta == NULL) {
					...
					pktmeta->pm_src.pa_addr[0] = spirit_pktcommon_get_rxsrcaddr(spirit);

					pktmeta->pm_dest.pa_addrlen = 1;
					pktmeta->pm_dest.pa_addr[0] = spirit_pktcommon_get_nodeaddress(spirit);
	...
	if (irqstatus.IRQ_RX_DATA_DISC != 0) {
		...
		wlinfo("Data discarded: Node addr=%02x RX dest addr=%02x\n",
			   spirit_pktcommon_get_nodeaddress(spirit), spirit_pktcommon_get_rxdestaddr(spirit));
		wlinfo("                CRC error=%u RX timeout=%u\n", irqstatus.IRQ_CRC_ERROR,
			   irqstatus.IRQ_RX_TIMEOUT);
...
static int spirit_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	return work_queue(HPWORK, &priv->irqwork, spirit_interrupt_work, (FAR void *)priv, 0);
}
 ...
 *
 * Description:
 *   ROUX Callback: Bring up the Spirit interface when an IP address is
 *   provided
 *
 * Input Parameters:
 *   dev - Reference to the ROUX driver state structure
...
static int spirit_ifup(FAR struct net_driver_s *dev)
{
	...
	if (!priv->ifup) {
		...
		ret = spirit_pktcommon_set_nodeaddress(spirit, dev->d_mac.radio.nv_addr[0]);
		if (ret < 0) {
...
static int spirit_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg)
{
	...
	case SIOCPKTRADIOGNODE: {
		FAR struct pktradio_addr_s *retaddr = (FAR struct pktradio_addr_s *)&cmddata->pifr_hwaddr;
		FAR struct netdev_varaddr_s *devaddr = &dev->d_mac.radio;
						   ...
						   FAR struct iob_s *framelist)
{
	...
	for (iob = framelist; iob != NULL; iob = framelist) {
		...
		memcpy(&pktmeta->pm_src, &metain->pm_src, sizeof(struct pktradio_addr_s));
		memcpy(&pktmeta->pm_dest, &metain->pm_dest, sizeof(struct pktradio_addr_s));
		pktmeta->pm_iob = iob;
...
int spirit_hw_initialize(FAR struct spirit_driver_s *priv, FAR struct spi_dev_s *spi)
{
	FAR struct spirit_library_s *spirit = &priv->spirit;
	int ret;
  ...
  if (ret < 0)
	...
	spirit_timer_calc_rxtimeout_values(spirit, SPIRIT_RXTIMEOUT, &priv->counter, &priv->prescaler);

	ret = spirit_timer_set_rxtimeout_stopcondition(spirit, SQI_ABOVE_THRESHOLD);
	if (ret < 0) {
...
int spirit_netdev_initialize(FAR struct spi_dev_s *spi, FAR const struct spirit_lower_s *lower)
{
	FAR struct spirit_driver_s *priv;
