...
static void spirit_regdebug(FAR const char *msg, FAR uint8_t *header, FAR const uint8_t *buffer,
							unsigned int buflen)
{
	wlinfo("%-8s: %02x %02x\n", header[0], header[1]);
...
static void spirit_fifodebug(FAR const char *msg, FAR uint8_t *header, FAR const uint8_t *buffer,
							 unsigned int buflen)
{
	wlinfo("%-8s: %02x %02x\n", header[0], header[1]);
	spirit_dump_buffer(buffer, buflen);
...
int spirit_reg_read(FAR struct spirit_library_s *spirit, uint8_t regaddr, FAR uint8_t *buffer,
					unsigned int buflen)
{
	uint8_t header[2];
	uint8_t status[2];
...
int spirit_reg_modify(FAR struct spirit_library_s *spirit, uint8_t regaddr, uint8_t setbits,
					  uint8_t clrbits)
{
	uint8_t header[2];
	uint8_t status[2];
...
int spirit_fifo_read(FAR struct spirit_library_s *spirit, FAR uint8_t *buffer, unsigned int buflen)
{
	uint8_t header[2];
	uint8_t status[2];
...
int spirit_fifo_write(FAR struct spirit_library_s *spirit, FAR const uint8_t *buffer,
					  unsigned int buflen)
{
	uint8_t header[2];
	uint8_t status[2];
...
int spirit_waitstatus(FAR struct spirit_library_s *spirit, enum spirit_state_e state,
					  unsigned int msec)
{
	clock_t start;
	clock_t ticks;
...
#else
	ticks = ((clock_t)msec * USEC_PER_MSEC + (USEC_PER_TICK - 1)) / USEC_PER_TICK;
#endif
