...
#include <roux/wireless/ioctl.h>
#include <sched.h>
#include <stdint.h>
#include <stdio.h>
#include <sys/endian.h>
#include <unistd.h>
#include <roux/wireless/lpwan/sx126x.h>
...
static ssize_t sx126x_read(FAR struct file *filep, FAR char *buffer, size_t buflen);

static ssize_t sx126x_write(FAR struct file *filep, FAR const char *buf, size_t buflen);
...
enum sx126x_cmd_status {
	SX126X_STATUS_RESERVED,
	SX126X_STATUS_RFU,
	SX126X_STATUS_DATA_AVAILABLE,
	SX126X_STATUS_TIMEOUT,
	SX126X_STATUS_ERROR,
	SX126X_STATUS_EXECUTE_FAIL,
...
static const struct file_operations sx126x_ops = {
	sx126x_open, sx126x_close, sx126x_read, sx126x_write, NULL, sx126x_ioctl, NULL, NULL};
...
static void sx126x_command(FAR struct sx126x_dev_s *dev, uint8_t cmd, FAR const uint8_t *params,
						   size_t paramslen, FAR uint8_t *returns);

static void sx126x_reset(FAR struct sx126x_dev_s *dev);

static void sx126x_get_status(FAR struct sx126x_dev_s *dev, FAR struct sx126x_status_s *status);

static void sx126x_spi_lock(FAR struct sx126x_dev_s *dev);

static void sx126x_spi_unlock(FAR struct sx126x_dev_s *dev);

static void sx126x_write_register(FAR struct sx126x_dev_s *dev, uint16_t address, uint8_t *data,
								  size_t data_length);

/* Operational modes functions **********************************************/

static void sx126x_set_standby(FAR struct sx126x_dev_s *dev, enum sx126x_standby_mode_e mode);
...
static void sx126x_set_pa_config(FAR struct sx126x_dev_s *dev, enum sx126x_device_e model,
...
static void sx126x_set_dio_irq_params(FAR struct sx126x_dev_s *dev, uint16_t irq_mask,
									  uint16_t dio1_mask, uint16_t dio2_mask, uint16_t dio3_mask);

static void sx126x_set_dio2_as_rf_switch(FAR struct sx126x_dev_s *dev, bool enable);

static void sx126x_set_dio3_as_tcxo(FAR struct sx126x_dev_s *dev,
									enum sx126x_tcxo_voltage_e voltage, uint32_t delay);

static void sx126x_get_irq_status(FAR struct sx126x_dev_s *dev, FAR uint16_t *irqstatus);

static void sx126x_clear_irq_status(FAR struct sx126x_dev_s *dev, uint16_t clearbits);

/* RF Modulation and Packet-Related Functions *******************************/

static void sx126x_set_packet_params_lora(FAR struct sx126x_dev_s *dev,
										  FAR struct sx126x_packetparams_lora_s *pktparams);

static void sx126x_set_modulation_params_lora(FAR struct sx126x_dev_s *dev,
											  FAR struct sx126x_modparams_lora_s *modparams);

static void sx126x_set_buffer_base_address(FAR struct sx126x_dev_s *dev, uint8_t tx, uint8_t rx);

static void sx126x_set_tx_params(FAR struct sx126x_dev_s *dev, uint8_t power,
								 enum sx126x_ramp_time_e ramp_time);

static void sx126x_set_packet_type(FAR struct sx126x_dev_s *dev, enum sx126x_packet_type_e type);

static void sx126x_set_rf_frequency(FAR struct sx126x_dev_s *dev, uint32_t frequency_hz);

/* Communication status information *****************************************/

static void sx126x_get_rssi_inst(FAR struct sx126x_dev_s *dev, FAR int32_t *dbm);

static void sx126x_get_rx_buffer_status(FAR struct sx126x_dev_s *dev, uint8_t *status,
										uint8_t *payload_len, uint8_t *rx_buff_offset);
...
static void sx126x_read_buffer(FAR struct sx126x_dev_s *dev, uint8_t offset, FAR uint8_t *payload,
							   uint8_t len);

/* Register settings ********************************************************/

static void sx126x_set_syncword(FAR struct sx126x_dev_s *dev, uint8_t *syncword,
...
static inline int sx126x_attachirq0(FAR struct sx126x_dev_s *dev, xcpt_t isr, FAR void *arg);
...
static ssize_t sx126x_read(FAR struct file *filep, FAR char *buf, size_t buflen)
{
	int ret = 0;
	...
	sx126x_spi_lock(dev);
	sx126x_get_rx_buffer_status(dev, &status, &header->payload_length, &offset);
	sx126x_read_buffer(dev, offset, header->payload, header->payload_length);
	sx126x_spi_unlock(dev);
...
static int sx126x_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	struct sx126x_dev_s *dev;
	dev = filep->f_inode->i_private;
	wlinfo("IOCTL cmd %d arg %u SX126x dev_number %d", cmd, *(FAR uint32_t *)((uintptr_t)arg),
	...
	case SX126XIOC_LORACONFIGSET: {
		FAR struct sx126x_lora_config_s *ptr = (FAR struct sx126x_lora_config_s *)((uintptr_t)arg);
		DEBUGASSERT(ptr != NULL);
		...
		dev->low_datarate_optimization = ptr->modulation.low_datarate_optimization;
...
static void sx126x_set_standby(FAR struct sx126x_dev_s *dev, enum sx126x_standby_mode_e mode)
{
	sx126x_command(dev, SX126X_SETSTANDBY, (uint8_t *)&mode, SX126X_SETSTANDBY_PARAMS, NULL);
}
...
static void sx126x_set_tx(FAR struct sx126x_dev_s *dev, uint32_t timeout)
{
	...
	sx126x_command(dev, SX126X_SETTX, (uint8_t *)&timeout, SX126X_SETTX_PARAMS, NULL);
}
...
static void sx126x_set_rx(FAR struct sx126x_dev_s *dev, uint32_t timeout)
{
	...
	sx126x_command(dev, SX126X_SETRX, (uint8_t *)&timeout, SX126X_SETRX_PARAMS, NULL);
}

static void sx126x_stop_timer_on_preamble(FAR struct sx126x_dev_s *dev, bool enable)
{
	sx126x_command(dev, SX126X_STOPTIMERONPREAMBLE, (uint8_t *)&enable,
				   SX126X_STOPTIMERONPREAMBLE_PARAMS, NULL);
}

static void sx126x_set_rx_duty_cycle(FAR struct sx126x_dev_s *dev, uint32_t rx_period,
									 uint32_t sleep_period)
{
	...
	memcpy(params + SX126X_SETRXDUTYCYCLE_RXPERIOD_PARAM, (uint8_t *)&rx_period,
		   SX126X_SETRXDUTYCYCLE_RXPERIOD_PARAMS);
	memcpy(params + SX126X_SETRXDUTYCYCLE_SLEEPPERIOD_PARAM, (uint8_t *)&sleep_period,
		   SX126X_SETRXDUTYCYCLE_SLEEPPERIOD_PARAMS);

	sx126x_command(dev, SX126X_SETRXDUTYCYCLE, params, SX126X_SETRXDUTYCYCLE_PARAMS, NULL);
}
...
static void sx126x_set_regulator_mode(FAR struct sx126x_dev_s *dev,
									  enum sx126x_regulator_mode_e mode)
{
	sx126x_command(dev, SX126X_SETREGULATORMODE, (uint8_t *)&mode, SX126X_SETREGULATORMODE_PARAMS,
				   NULL);
}
								 ...
								 uint8_t hpmax, uint8_t padutycycle)
{
	...
	sx126x_command(dev, SX126X_SETPACONFIG, params, SX126X_SETPACONFIG_PARMS, NULL);
}
									  ...
									  uint16_t dio1_mask, uint16_t dio2_mask, uint16_t dio3_mask)
{
	...
	sx126x_command(dev, SX126X_SETDIOIRQPARAMS, params, SX126X_SETDIOIRQPARAMS_PARAMS, NULL);
}

static void sx126x_set_dio2_as_rf_switch(FAR struct sx126x_dev_s *dev, bool enable)
{
	sx126x_command(dev, SX126X_SETDIO2RFSWCTRL, (uint8_t *)&enable, SX126X_SETDIO2RFSWCTRL_PARAMS,
				   NULL);
}
									...
									enum sx126x_tcxo_voltage_e voltage, uint32_t delay)
{
	...
	sx126x_command(dev, SX126X_SETDIO3TCXOCTRL, params, SX126X_SETDIO3TCXOCTRL_PARAMS, NULL);
}
...
static void sx126x_get_irq_status(FAR struct sx126x_dev_s *dev, FAR uint16_t *irqstatus)
{
	...
	sx126x_command(dev, SX126X_GETIRQSTATUS, NULL, SX126X_GETIRQSTATUS_RETURNS, returns);

	uint16_t bits;
	memcpy(&bits, returns + SX126X_GETIRQSTATUS_IRQSTATUS_RETURN,
...
static void sx126x_clear_irq_status(FAR struct sx126x_dev_s *dev, uint16_t clearbits)
{
	...
	memcpy(params + SX126X_CLEARIRQSTATUS_CLEAR_PARAM, &clearbits,
		   SX126X_CLEARIRQSTATUS_CLEAR_PARAMS);

	sx126x_command(dev, SX126X_CLEARIRQSTATUS, params, SX126X_CLEARIRQSTATUS_PARAMS, NULL);
}
										  ...
										  FAR struct sx126x_packetparams_lora_s *pktparams)
{
	...
	params[SX126X_PKTPARAM3_LORA_HEADERTYPE_PARAM] = pktparams->fixed_length_header;
	params[SX126X_PKTPARAM4_LORA_PAYLOADLEN_PARAM] = pktparams->payload_length;
	...
	sx126x_command(dev, SX126X_SETPACKETPARMS, params, SX126X_SETPACKETPARMS_PARAMS, NULL);
}
											  ...
											  FAR struct sx126x_modparams_lora_s *modparams)
{
	...
	params[SX126X_MODPARAM1_LORA_SF_PARAM] = modparams->spreading_factor;
	params[SX126X_MODPARAM2_LORA_BW_PARAM] = modparams->bandwidth;
	params[SX126X_MODPARAM3_LORA_CR_PARAM] = modparams->coding_rate;
	params[SX126X_MODPARAM4_LORA_LOWDATRATE_OPTI_PARAM] = modparams->low_datarate_optimization;

	sx126x_command(dev, SX126X_SETMODULATIONPARAMS, params, SX126X_SETMODULATIONPARAMS_PARAMS,
				   NULL);
}
...
static void sx126x_set_buffer_base_address(FAR struct sx126x_dev_s *dev, uint8_t tx, uint8_t rx)
{
	...
	sx126x_command(dev, SX126X_SETBUFFERBASEADDRESS, params, SX126X_SETBUFFERBASEADDRESS_PARAMS,
				   NULL);
}
								 ...
								 enum sx126x_ramp_time_e ramp_time)
{
	...
	sx126x_command(dev, SX126X_SETTXPARMS, params, SX126X_SETTXPARMS_PARAMS, NULL);
}

static void sx126x_set_packet_type(FAR struct sx126x_dev_s *dev, enum sx126x_packet_type_e type)
{
	sx126x_command(dev, SX126X_SETPACKETTYPE, (uint8_t *)&type, SX126X_SETPACKETTYPE_PARAMS, NULL);
}

static void sx126x_set_rf_frequency(FAR struct sx126x_dev_s *dev, uint32_t frequency_hz)
{
	uint32_t corrected_freq = sx126x_convert_freq_in_hz_to_pll_step(frequency_hz);
...
static void sx126x_set_lora_symb_num_timout(FAR struct sx126x_dev_s *dev, uint8_t symbnum)
{
	sx126x_command(dev, SX126X_SETLORASYMBNUMTIMEOUT, &symbnum, SX126X_SETLORASYMBNUMTIMEOUT_PARAMS,
				   NULL);
}
...
static void sx126x_get_status(FAR struct sx126x_dev_s *dev, FAR struct sx126x_status_s *status)
{
	...
	status->mode = (rets[0] & SX126X_STATUS_CHIPMODE_MASK) >> SX126X_STATUS_CHIPMODE_SHIFT;
	status->cmd = (rets[0] & SX126X_STATUS_CMD_MASK) >> SX126X_STATUS_CMD_SHIFT;
}
...
static void sx126x_get_rssi_inst(FAR struct sx126x_dev_s *dev, FAR int32_t *dbm)
{
	...
	sx126x_command(dev, SX126X_GETRSSIINST, NULL, SX126X_GETRSSIINST_RETURNS, rets);
										...
										uint8_t *payload_len, uint8_t *rx_buff_offset)
{
	...
	sx126x_command(dev, SX126X_GETRXBUFFERSTATUS, NULL, SX126X_GETRXBUFFERSTATUS_RETURNS, returns);
...
static void sx126x_command(FAR struct sx126x_dev_s *dev, uint8_t cmd, const FAR uint8_t *params,
						   size_t paramslen, FAR uint8_t *returns)
{
	sx126x_select(dev);
...
static void sx126x_read_register(FAR struct sx126x_dev_s *dev, uint16_t address, uint8_t *data,
								 size_t data_length)
{
	sx126x_select(dev);
...
static int sx126x_setup_radio(FAR struct sx126x_dev_s *dev)
{
	...
	case (SX126X_PACKETTYPE_LORA): {
		...
		struct sx126x_modparams_lora_s modparams = {.spreading_factor = dev->lora_sf,
		...
		struct sx126x_packetparams_lora_s pktparams = {.crc_enable = dev->lora_crc,
													   .fixed_length_header =
	...
	sx126x_set_dio_irq_params(dev, dev->irq_mask, dev->lower->masks.dio1_mask,
							  dev->lower->masks.dio2_mask, dev->lower->masks.dio3_mask);
	...
	sx126x_set_dio3_as_tcxo(dev, dev->lower->dio3_voltage, dev->lower->dio3_delay);
	return 0;
}
...
void sx126x_register(FAR struct spi_dev_s *spi, FAR const struct sx126x_lower_s *lower,
					 const char *path)
{
	...
	(void)register_driver(path, &sx126x_ops, 0666, dev);
}
