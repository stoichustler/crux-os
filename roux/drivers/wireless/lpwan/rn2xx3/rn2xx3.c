...
#include <roux/wireless/lpwan/rn2xx3.h>
...
#endif
static ssize_t rn2xx3_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static ssize_t rn2xx3_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static const struct txlvl_s RN2903_TXLVLS[] = {
	{.lvl = 2, .power = 300},	{.lvl = 3, .power = 400},	{.lvl = 4, .power = 500},
	{.lvl = 5, .power = 600},	{.lvl = 6, .power = 700},	{.lvl = 7, .power = 800},
	{.lvl = 8, .power = 900},	{.lvl = 9, .power = 1000},	{.lvl = 10, .power = 1100},
	{.lvl = 11, .power = 1200}, {.lvl = 12, .power = 1300}, {.lvl = 14, .power = 1470},
	{.lvl = 15, .power = 1550}, {.lvl = 16, .power = 1630}, {.lvl = 17, .power = 1700},
	{.lvl = 20, .power = 1850},
};
...
static const struct txlvl_s RN2483_TXLVLS[] = {
	{.lvl = -3, .power = -400}, {.lvl = -2, .power = -290}, {.lvl = -1, .power = -190},
	{.lvl = 0, .power = -170},	{.lvl = 1, .power = -60},	{.lvl = 2, .power = 40},
	{.lvl = 3, .power = 140},	{.lvl = 4, .power = 250},	{.lvl = 5, .power = 360},
	{.lvl = 6, .power = 470},	{.lvl = 7, .power = 580},	{.lvl = 8, .power = 690},
	{.lvl = 9, .power = 810},	{.lvl = 10, .power = 930},	{.lvl = 11, .power = 1040},
	{.lvl = 12, .power = 1160}, {.lvl = 13, .power = 1250}, {.lvl = 14, .power = 1350},
	{.lvl = 15, .power = 1410},
};
...
static ssize_t read_line(FAR struct rn2xx3_dev_s *priv, FAR char *buf, size_t nbytes)
{
	ssize_t length = 0;
	ssize_t i = 0;
...
static int mac_pause(FAR struct rn2xx3_dev_s *priv)
{
	...
	length = file_write(&priv->uart, "mac pause\r\n", sizeof("mac pause\r\n") - 1);
	if (length < 0) {
		return length;
	}
...
static int8_t rn2xx3_txpwrlevel(FAR int32_t *dbm, FAR const struct txlvl_s *arr, uint8_t len)
{
...
static int32_t rn2xx3_txpwrdbm(int8_t lvl, FAR const struct txlvl_s *arr, uint8_t len)
{
...
static ssize_t rn2xx3_getparam(FAR struct rn2xx3_dev_s *priv, FAR char *cmd, FAR char *resp,
							   size_t nbytes)
{
	ssize_t length;
...
static int rn2xx3_getmodel(FAR struct rn2xx3_dev_s *priv, FAR enum rn2xx3_type_e *model)
{
	ssize_t length;
	char response[40];
	...
	length = rn2xx3_getparam(priv, "sys get ver\r\n", response, sizeof(response));
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_getsnr(FAR struct rn2xx3_dev_s *priv, FAR int8_t *snr)
{
	...
	length = rn2xx3_getparam(priv, "radio get snr\r\n", response, sizeof(response));
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_crc_en(FAR struct rn2xx3_dev_s *priv, bool enable)
{
	...
	length = file_write(&priv->uart, "radio set crc ", sizeof("radio set crc ") - 1);
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_iqi_en(FAR struct rn2xx3_dev_s *priv, bool enable)
{
	...
	length = file_write(&priv->uart, "radio set iqi ", sizeof("radio set iqi ") - 1);
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_setfreq(FAR struct rn2xx3_dev_s *priv, uint32_t freq)
{
	...
	length = file_write(&priv->uart, "radio set freq ", sizeof("radio set freq ") - 1);
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_settxpwr(FAR struct rn2xx3_dev_s *priv, FAR int32_t *pwr)
{
	...
	if (priv->model == RN2903) {
		txpwr = rn2xx3_txpwrlevel(pwr, RN2903_TXLVLS, array_len(RN2903_TXLVLS));
	} else {
		txpwr = rn2xx3_txpwrlevel(pwr, RN2483_TXLVLS, array_len(RN2483_TXLVLS));
	}

	length = file_write(&priv->uart, "radio set pwr ", sizeof("radio set pwr ") - 1);
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_gettxpwr(FAR struct rn2xx3_dev_s *priv, FAR int32_t *txpwr)
{
	...
	length = rn2xx3_getparam(priv, "radio get pwr\r\n", response, sizeof(response));
	if (length < 0) {
		return length;
	}
	...
	if (priv->model == RN2903) {
		*txpwr = rn2xx3_txpwrdbm(txlevel, RN2903_TXLVLS, array_len(RN2903_TXLVLS));
	} else {
		*txpwr = rn2xx3_txpwrdbm(txlevel, RN2483_TXLVLS, array_len(RN2483_TXLVLS));
	}
...
static int rn2xx3_setbw(FAR struct rn2xx3_dev_s *priv, uint32_t bw)
{
	char bwstr[16];
	ssize_t length;
	length = file_write(&priv->uart, "radio set bw ", sizeof("radio set bw ") - 1);
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_getbw(FAR struct rn2xx3_dev_s *priv, FAR uint32_t *bw)
{
	...
	length = rn2xx3_getparam(priv, "radio get bw\r\n", response, sizeof(response));
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_setsf(FAR struct rn2xx3_dev_s *priv, uint8_t sf)
{
	...
	length = file_write(&priv->uart, "radio set sf ", sizeof("radio set sf ") - 1);
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_getsf(FAR struct rn2xx3_dev_s *priv, FAR uint8_t *sf)
{
	...
	length = rn2xx3_getparam(priv, "radio get sf\r\n", response, sizeof(response));
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_setprlen(FAR struct rn2xx3_dev_s *priv, uint16_t prlen)
{
	...
	length = file_write(&priv->uart, "radio set prlen ", sizeof("radio set prlen ") - 1);
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_getprlen(FAR struct rn2xx3_dev_s *priv, FAR uint16_t *prlen)
{
	ssize_t length;
	char response[20];
...
static int rn2xx3_setmod(FAR struct rn2xx3_dev_s *priv, enum rn2xx3_mod_e mod)
{
	int err;
	char modstr[16];
	ssize_t length;
	...
	length = file_write(&priv->uart, "radio set mod ", sizeof("radio set mod ") - 1);
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_getmod(FAR struct rn2xx3_dev_s *priv, FAR enum rn2xx3_mod_e *mod)
{
	ssize_t length;
	char response[12];
...
static int rn2xx3_setcr(FAR struct rn2xx3_dev_s *priv, enum rn2xx3_cr_e cr)
{
	...
	length = file_write(&priv->uart, "radio set cr ", sizeof("radio set cr ") - 1);
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_getcr(FAR struct rn2xx3_dev_s *priv, FAR enum rn2xx3_cr_e *cr)
{
	ssize_t length;
	char response[10];

	length = rn2xx3_getparam(priv, "radio get cr\r\n", response, sizeof(response));
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_setsync(FAR struct rn2xx3_dev_s *priv, uint64_t sync)
{
	...
	length = file_write(&priv->uart, "radio set sync ", sizeof("radio set sync ") - 1);
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_getsync(FAR struct rn2xx3_dev_s *priv, FAR uint64_t *sync)
{
	ssize_t length;
	char response[20];
...
static int rn2xx3_setbitrate(FAR struct rn2xx3_dev_s *priv, uint32_t bitrate)
{
	...
	length = file_write(&priv->uart, "radio set bitrate ", sizeof("radio set bitrate ") - 1);
	if (length < 0) {
		return length;
	}
...
static int rn2xx3_getbitrate(FAR struct rn2xx3_dev_s *priv, FAR uint32_t *bitrate)
{
	ssize_t length;
	char response[20];
