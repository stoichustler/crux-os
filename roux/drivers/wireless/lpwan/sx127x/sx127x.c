...
#include <roux/wireless/lpwan/sx127x.h>
#include "sx127x.h"
...
struct sx127x_priv_ops_s {
	...
	CODE int (*syncword_set)(FAR struct sx127x_dev_s *dev, FAR uint8_t *sw, uint8_t len);

	/* Get sync word */

	CODE void (*syncword_get)(FAR struct sx127x_dev_s *dev, FAR uint8_t *sw, FAR uint8_t *len);

#ifdef CONFIG_LPWAN_SX127X_TXSUPPORT
	/* Send packet */

	CODE int (*send)(FAR struct sx127x_dev_s *dev, FAR const uint8_t *data, size_t datalen);
#endif
...
static void sx127x_writeregbyte(FAR struct sx127x_dev_s *dev, uint8_t reg, uint8_t value);
static uint8_t sx127x_modregbyte(FAR struct sx127x_dev_s *dev, uint8_t reg, uint8_t setbits,
								 uint8_t clrbits);
...
static int16_t sx127x_lora_rssi_correct(FAR struct sx127x_dev_s *dev, uint32_t freq, int8_t snr,
										uint8_t regval);
...
static int sx127x_lora_syncword_set(FAR struct sx127x_dev_s *dev, FAR uint8_t *sw, uint8_t len);
static void sx127x_lora_syncword_get(FAR struct sx127x_dev_s *dev, FAR uint8_t *sw, uint8_t *len);
...
#endif
#ifdef CONFIG_LPWAN_SX127X_TXSUPPORT
static int sx127x_lora_send(FAR struct sx127x_dev_s *dev, FAR const uint8_t *data, size_t datalen);
#endif
#ifdef CONFIG_DEBUG_WIRELESS_INFO
...
static int sx127x_fskook_bitrate_set(FAR struct sx127x_dev_s *dev, uint32_t bitrate);
...
static int sx127x_fskook_syncword_set(FAR struct sx127x_dev_s *dev, FAR uint8_t *sw, uint8_t len);
static void sx127x_fskook_syncword_get(FAR struct sx127x_dev_s *dev, FAR uint8_t *sw,
									   FAR uint8_t *len);
#ifdef CONFIG_LPWAN_SX127X_RXSUPPORT
...
static inline int sx127x_attachirq0(FAR struct sx127x_dev_s *dev, xcpt_t isr, FAR void *arg);
static int sx127x_irq0handler(int irq, FAR void *context, FAR void *arg);

static int sx127x_modulation_set(FAR struct sx127x_dev_s *dev, uint8_t modulation);
...
static int sx127x_syncword_set(FAR struct sx127x_dev_s *dev, FAR uint8_t *sw, uint8_t len);
static void sx127x_syncword_get(FAR struct sx127x_dev_s *dev, FAR uint8_t *sw, FAR uint8_t *len);
#ifdef CONFIG_DEBUG_WIRELESS_INFO
...
#ifdef CONFIG_LPWAN_SX127X_TXSUPPORT
static int sx127x_txfifo_write(FAR struct sx127x_dev_s *dev, FAR const uint8_t *data,
							   size_t datalen);
#endif
#ifdef CONFIG_LPWAN_SX127X_RXSUPPORT
static ssize_t sx127x_rxfifo_get(FAR struct sx127x_dev_s *dev, FAR uint8_t *buffer, size_t buflen);
static void sx127x_rxfifo_put(FAR struct sx127x_dev_s *dev, FAR uint8_t *buffer, size_t buflen);
#endif
...
static ssize_t sx127x_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t sx127x_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static uint8_t sx127x_access(FAR struct sx127x_dev_s *dev, sx127x_access_mode_t mode, uint8_t cmd,
...
static inline uint8_t sx127x_readreg(FAR struct sx127x_dev_s *dev, uint8_t reg, FAR uint8_t *value,
									 int len)
{
...
static inline uint8_t sx127x_readregbyte(FAR struct sx127x_dev_s *dev, uint8_t reg)
{
	uint8_t val = 0;
...
{
	return sx127x_access(dev, MODE_WRITE, reg | SX127X_W_REGISTER, (FAR uint8_t *)value, len);
}
...
static inline void sx127x_writeregbyte(FAR struct sx127x_dev_s *dev, uint8_t reg, uint8_t value)
{
...
static inline int sx127x_attachirq1(FAR struct sx127x_dev_s *dev, xcpt_t isr, FAR void *arg)
{
...
static inline int sx127x_attachirq2(FAR struct sx127x_dev_s *dev, xcpt_t isr, FAR void *arg)
{
...
static inline int sx127x_attachirq3(FAR struct sx127x_dev_s *dev, xcpt_t isr, FAR void *arg)
{
...
static inline int sx127x_attachirq4(FAR struct sx127x_dev_s *dev, xcpt_t isr, FAR void *arg)
{
...
static inline int sx127x_attachirq5(FAR struct sx127x_dev_s *dev, xcpt_t isr, FAR void *arg)
{
...
static int sx127x_fskook_isr0_process(FAR struct sx127x_dev_s *dev)
{
	...
	irq1 = (SX127X_FOM_IRQ1_RSSI | SX127X_FOM_IRQ1_PREAMBE | SX127X_FOM_IRQ1_SYNCADDRMATCH);
...
static ssize_t sx127x_rxfifo_get(FAR struct sx127x_dev_s *dev, FAR uint8_t *buffer, size_t buflen)
{
	...
	pkt = (struct sx127x_read_hdr_s *)(dev->rx_buffer + dev->nxt_read * SX127X_RXFIFO_ITEM_SIZE);
...
#ifdef CONFIG_LPWAN_SX127X_FSKOOK
static int sx127x_fskook_send(FAR struct sx127x_dev_s *dev, FAR const uint8_t *data, size_t datalen)
{
	DEBUGASSERT(dev->modulation == SX127X_MODULATION_FSK ||
				dev->modulation == SX127X_MODULATION_OOK);
...
static int sx127x_fskook_opmode_init(FAR struct sx127x_dev_s *dev, uint8_t opmode)
{
	...
	case SX127X_OPMODE_TX: {
		...
		sx127x_writeregbyte(dev, SX127X_FOM_FIFOTHR, SX127X_FOM_FIFOTHR_TXSTARTCOND);
	...
	case SX127X_OPMODE_RX: {
		...
		setbits =
			(SX127X_FOM_RXCFG_AGCAUTOON | SX127X_FOM_RXCFG_AFCAUTOON | SX127X_FOM_RXCFG_TRG_PREDET);
...
static int sx127x_fskook_bitrate_set(FAR struct sx127x_dev_s *dev, uint32_t bitrate)
{
	...
	sx127x_writeregbyte(dev, SX127X_FOM_BITRATEMSB, SX127X_FOM_BITRATE_MSB(br));

	/* Write LSB */

	sx127x_writeregbyte(dev, SX127X_FOM_BITRATELSB, SX127X_FOM_BITRATE_LSB(br));
...
static void sx127x_fskook_preamble_set(FAR struct sx127x_dev_s *dev, uint32_t len)
{
	...
	if (len == 0) {
		...
		regval = (SX127X_FOM_PREDET_ON | SX127X_FOM_PREDET_SIZE_2B | SX127X_FOM_PREDET_TOL(10));
		sx127x_writeregbyte(dev, SX127X_FOM_PREDET, regval);
	}
...
static int sx127x_lora_opmode_set(FAR struct sx127x_dev_s *dev, uint8_t opmode)
{
	...
	sx127x_modregbyte(dev, SX127X_CMN_OPMODE, ((opmode - 1) << SX127X_CMN_OPMODE_MODE_SHIFT),
...
static void sx127x_lora_syncword_get(FAR struct sx127x_dev_s *dev, FAR uint8_t *sw,
									 FAR uint8_t *len)
{
	DEBUGASSERT(dev->modulation == SX127X_MODULATION_FSK ||
				dev->modulation == SX127X_MODULATION_OOK);
										...
										uint8_t regval)
{
	...
	offset =
		(freq > SX127X_HFBAND_THR ? SX127X_LRM_RSSIVAL_HF_OFFSET : SX127X_LRM_RSSIVAL_LF_OFFSET);
...
static uint8_t sx127x_modulation_get(FAR struct sx127x_dev_s *dev)
{
	...
	if (regval & SX127X_CMN_OPMODE_LRMODE) {
		...
		ret = (regval & SX127X_CMN_OPMODE_MODTYPE_FSK ? SX127X_MODULATION_FSK
...
static int sx127x_init(FAR struct sx127x_dev_s *dev)
{
	...
	dev->opmode = (((regval >> SX127X_CMN_OPMODE_MODE_SHIFT) & SX127X_CMN_OPMODE_MODE_MASK) + 1);
...
int sx127x_register(FAR struct spi_dev_s *spi, FAR const struct sx127x_lower_s *lower)
{
	FAR struct sx127x_dev_s *dev = NULL;
	int ret = OK;
