...
#include <roux/wireless/nrf24l01.h>
#include "nrf24l01.h"
...
static uint8_t nrf24l01_access(FAR struct nrf24l01_dev_s *dev, nrf24l01_access_mode_t mode,
							   uint8_t cmd, FAR uint8_t *buf, int length);
...
static uint8_t nrf24l01_readreg(FAR struct nrf24l01_dev_s *dev, uint8_t reg, FAR uint8_t *value,
								int len);

/* Read single byte value from a register of nrf24 */

static uint8_t nrf24l01_readregbyte(FAR struct nrf24l01_dev_s *dev, uint8_t reg);
static void nrf24l01_writeregbyte(FAR struct nrf24l01_dev_s *dev, uint8_t reg, uint8_t value);
static uint8_t nrf24l01_setregbit(FAR struct nrf24l01_dev_s *dev, uint8_t reg, uint8_t value,
								  bool set);
...
static inline int nrf24l01_attachirq(FAR struct nrf24l01_dev_s *dev, xcpt_t isr, FAR void *arg);
...
#ifdef CONFIG_WL_NRF24L01_RXSUPPORT
static void fifoput(FAR struct nrf24l01_dev_s *dev, uint8_t pipeno, FAR uint8_t *buffer,
					uint8_t buflen);
static uint8_t fifoget(FAR struct nrf24l01_dev_s *dev, FAR uint8_t *buffer, uint8_t buflen,
					   FAR uint8_t *pipeno);
static void nrf24l01_worker(FAR void *arg);
#endif

#ifdef CONFIG_DEBUG_WIRELESS
static void binarycvt(FAR char *deststr, size_t destlen, FAR const uint8_t *srcbin, size_t srclen);
#endif
...
static int nrf24l01_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static inline uint8_t nrf24l01_readreg(FAR struct nrf24l01_dev_s *dev, uint8_t reg,
...
static inline uint8_t nrf24l01_readregbyte(FAR struct nrf24l01_dev_s *dev, uint8_t reg)
{
	uint8_t val;
...
static inline void nrf24l01_writeregbyte(FAR struct nrf24l01_dev_s *dev, uint8_t reg, uint8_t value)
{
...
static int dosend(FAR struct nrf24l01_dev_s *dev, FAR const uint8_t *data, size_t datalen)
{
	...
	cmd = dev->tx_payload_noack ? NRF24L01_W_TX_PAYLOAD_NOACK : NRF24L01_W_TX_PAYLOAD;
	...
	if (status & NRF24L01_TX_DS) {
		...
		ret = OK;
		dev->lastxmitcount = (obsvalue & NRF24L01_ARC_CNT_MASK) >> NRF24L01_ARC_CNT_SHIFT;
...
static ssize_t nrf24l01_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	ret = nrf24l01_recv(dev, (FAR uint8_t *)buffer, buflen, &dev->last_recvpipeno);
...
int nrf24l01_setretransmit(FAR struct nrf24l01_dev_s *dev, nrf24l01_retransmit_delay_t retrdelay,
						   uint8_t retrcount)
{
	uint8_t val;
...
int nrf24l01_sendto(FAR struct nrf24l01_dev_s *dev, FAR const uint8_t *data, size_t datalen,
					FAR const uint8_t *destaddr)
{
...
#ifdef CONFIG_WL_NRF24L01_RXSUPPORT
ssize_t nrf24l01_recv(FAR struct nrf24l01_dev_s *dev, FAR uint8_t *buffer, size_t buflen,
					  FAR uint8_t *recvpipe)
{
