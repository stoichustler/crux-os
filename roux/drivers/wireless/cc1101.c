...
#include <roux/wireless/cc1101.h>
...
#define CC1101_SRX                                                                                 \
	0x34 /* Enable RX. Perform calibration first if switching from IDLE and MCSM0.FS_AUTOCAL=1. */
...
static ssize_t cc1101_file_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t cc1101_file_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int cc1101_file_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static void fifo_put(FAR struct cc1101_dev_s *dev, FAR uint8_t *buffer, uint8_t buflen)
{
	int ret;
	int i;
...
static uint8_t fifo_get(FAR struct cc1101_dev_s *dev, FAR uint8_t *buffer, uint8_t buflen)
{
	uint8_t pktlen;
	uint8_t i;
	int ret;
	...
	for (i = 0; i < pktlen && i < CC1101_PACKET_MAXTOTALLEN; i++) {
		*(buffer++) = dev->rx_buffer[dev->nxt_read * CC1101_FIFO_SIZE + i + 1];
	}
...
int cc1101_access(FAR struct cc1101_dev_s *dev, uint8_t addr, FAR uint8_t *buf, int length)
{
	int stabyte;
...
FAR struct cc1101_dev_s *cc1101_init(FAR struct spi_dev_s *spi, uint32_t isr_pin, uint32_t miso_pin,
									 FAR const struct c1101_rfsettings_s *rfsettings,
									 wait_cc1101_ready wait)
{
	FAR struct cc1101_dev_s *dev;
...
int cc1101_setrf(FAR struct cc1101_dev_s *dev, FAR const struct c1101_rfsettings_s *settings)
{
	int ret;
	...
	ret = cc1101_access(dev, CC1101_FSCTRL1, (FAR uint8_t *)&settings->FSCTRL1, -11);
	if (ret < 0) {
		return -EIO;
	}

	ret = cc1101_access(dev, CC1101_FOCCFG, (FAR uint8_t *)&settings->FOCCFG, -5);
	if (ret < 0) {
		return -EIO;
	}

	ret = cc1101_access(dev, CC1101_FREND1, (FAR uint8_t *)&settings->FREND1, -6);
	if (ret < 0) {
		return -EIO;
	}
...
int cc1101_write(FAR struct cc1101_dev_s *dev, FAR const uint8_t *buf, size_t size)
{
	uint8_t packetlen;
...
int cc1101_register(FAR const char *path, FAR struct cc1101_dev_s *dev)
{
	...
	dev->rx_buffer = kmm_malloc(CC1101_FIFO_SIZE * CONFIG_WL_CC1101_RXFIFO_LEN);
