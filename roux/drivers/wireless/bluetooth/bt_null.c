...
#include <roux/wireless/bluetooth/bt_null.h>
...
static int btnull_send(FAR struct bt_driver_s *dev, enum bt_buf_type_e type, FAR void *data,
					   size_t len);
...
static const bt_addr_t g_bt_addr = {{0xfe, 0xed, 0xb0, 0x0b, 0xfa, 0xce}};
					   ...
					   size_t len)
{
	...
	if (type == BT_CMD) {
		...
		bt_netdev_receive(dev, outbuf->type, outbuf->data, outbuf->len);
