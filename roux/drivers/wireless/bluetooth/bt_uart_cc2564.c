...
static int cc2564_send(FAR const struct btuart_lowerhalf_s *lower, FAR const uint8_t *buf,
					   ssize_t count)
{
...
static void cc2564_recv(FAR const struct btuart_lowerhalf_s *lower, FAR uint8_t *buf, ssize_t count)
{
	FAR uint8_t *ptr = buf;
	ssize_t nread;
...
static int cc2564_load(FAR const struct btuart_lowerhalf_s *lower, FAR const uint8_t *chipdata)
{
	uint8_t buffer[32];
