...
static int rpmsghci_recv_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
...
static FAR void *rpmsghci_get_tx_payload_buffer(FAR struct rpmsghci_s *priv, FAR uint32_t *len)
{
...
static int rpmsghci_bt_open(struct bt_driver_s *btdev)
{
	...
	return rpmsghci_send(priv, RPMSGHCI_OPEN, &msg->header, sizeof(struct rpmsghci_open_s));
}
...
static void rpmsghci_bt_close(struct bt_driver_s *btdev)
{
	...
	rpmsghci_send(priv, RPMSGHCI_CLOSE, &msg->header, sizeof(struct rpmsghci_close_s));
}
...
static int rpmsghci_bt_send(struct bt_driver_s *btdev, uint8_t type, void *data, size_t len)
{
	FAR struct rpmsghci_s *priv = NULL;
...
static int rpmsghci_bt_ioctl(FAR struct bt_driver_s *btdev, int cmd, unsigned long arg)
{
	/* TODO */
...
static int rpmsghci_client_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv)
{
	FAR struct rpmsghci_header_s *header = data;
...
static void rpmsghci_client_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	FAR struct rpmsghci_s *priv = priv_;
	char eptname[RPMSG_NAME_SIZE];

	if (strcmp(priv->cpuname, rpmsg_get_cpuname(rdev)) == 0) {
		snprintf(eptname, RPMSG_NAME_SIZE, "%s%s", RPMSGHCI_NAME_PREFIX, priv->name);
		priv->ept.priv = priv;
...
static void rpmsghci_client_destroy(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	FAR struct rpmsghci_s *priv = priv_;
...
FAR struct bt_driver_s *rpmsghci_register(FAR const char *cpuname, FAR const char *name)
{
	FAR struct rpmsghci_s *priv = NULL;
	...
	rpmsg_register_callback(priv, rpmsghci_client_created, rpmsghci_client_destroy, NULL, NULL);
