...
#include <roux/wireless/bluetooth/bt_rpmsghci.h>
...
static int rpmsghci_open_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
static int rpmsghci_close_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
static int rpmsghci_send_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
static int rpmsghci_ioctl_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
static int rpmsghci_default_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									uint32_t src, FAR void *priv);
...
static FAR void *rpmsghci_get_tx_payload_buffer(FAR struct rpmsghci_server_s *priv,
												FAR uint32_t *len)
{
...
static int rpmsghci_send(FAR struct rpmsghci_server_s *priv, uint32_t command,
								 ...
								 uint32_t src, FAR void *priv)
{
	...
	ret = btdev->send(btdev, msg->type, msg->data, len - sizeof(struct rpmsghci_data_s) + 1);
								  ...
								  uint32_t src, FAR void *priv)
{
	...
	ret = btdev->ioctl(btdev, msg->request, msg->arglen > 0 ? (unsigned long)msg->buf : msg->arg);
...
static bool rpmsghci_ns_match(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							  uint32_t dest)
{
...
static int rpmsghci_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len, uint32_t src,
						   FAR void *priv)
{
	FAR struct rpmsghci_header_s *header = data;
...
static void rpmsghci_ns_bind(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							 uint32_t dest)
{
	...
	rpmsg_create_ept(&server->ept, rdev, name, RPMSG_ADDR_ANY, dest, rpmsghci_ept_cb,
					 rpmsg_destroy_ept);
}
...
int rpmsghci_bt_receive(FAR struct bt_driver_s *btdev, uint8_t type, FAR void *data, size_t len)
{
...
int rpmshci_server_init(FAR const char *name, FAR struct bt_driver_s *btdev)
{
	...
	ret = rpmsg_register_callback(priv, NULL, NULL, rpmsghci_ns_match, rpmsghci_ns_bind);
	if (ret < 0) {
		kmm_free(priv);
	}
