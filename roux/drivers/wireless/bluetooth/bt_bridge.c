...
#include <roux/net/snoop.h>

#include <roux/wireless/bluetooth/bt_bridge.h>
#include <roux/wireless/bluetooth/bt_driver.h>
...
static bool bt_filter_set(FAR uint16_t *array, int size, uint16_t old, uint16_t new)
{
	int i;
...
static bool bt_filter_set_handle(FAR uint16_t *handle, int size, uint16_t old, uint16_t new)
{
	return bt_filter_set(handle, size, old, new);
}

static bool bt_filter_alloc_handle(FAR struct bt_filter_s *filter, uint16_t handle)
{
	return bt_filter_set_handle(filter->handle, BT_FILTER_CONN_COUNT, 0, handle);
}

static bool bt_filter_free_handle(FAR struct bt_filter_s *filter, uint16_t handle)
{
	return bt_filter_set_handle(filter->handle, BT_FILTER_CONN_COUNT, handle, 0);
}

static bool bt_filter_has_handle(FAR struct bt_filter_s *filter, uint16_t handle)
{
	return bt_filter_set_handle(filter->handle, BT_FILTER_CONN_COUNT, handle, handle);
}

static bool bt_filter_set_opcode(FAR uint16_t *opcode, int size, uint16_t old, uint16_t new)
{
	return bt_filter_set(opcode, size, old, new);
}

static bool bt_filter_alloc_opcode(FAR struct bt_filter_s *filter, uint16_t opcode)
{
	return bt_filter_set_opcode(filter->opcode, BT_FILTER_OPCODE_COUNT, 0, opcode);
}

static bool bt_filter_free_opcode(FAR struct bt_filter_s *filter, uint16_t opcode)
{
	return bt_filter_set_opcode(filter->opcode, BT_FILTER_OPCODE_COUNT, opcode, 0);
}

static bool bt_filter_can_recv(FAR struct bt_filter_s *filter, enum bt_buf_type_e type,
...
static bool bt_filter_can_send(FAR struct bt_filter_s *filter, enum bt_buf_type_e type,
...
{
	FAR struct bt_bridge_device_s *device = (FAR struct bt_bridge_device_s *)drv;
...
static int bt_bridge_send_reset_response(FAR struct bt_driver_s *driver)
{
	...
	return bt_netdev_receive(driver, BT_EVT, reset_response, sizeof(reset_response));
}

static bool bt_bridge_filter_command(FAR struct bt_bridge_s *bridge, FAR struct bt_driver_s *driver,
									 FAR uint8_t *data)
{
...
static int bt_bridge_send(FAR struct bt_driver_s *drv, enum bt_buf_type_e type, FAR void *data,
						  size_t len)
{
	FAR struct bt_bridge_device_s *device = (FAR struct bt_bridge_device_s *)drv;
	FAR struct bt_bridge_s *bridge = device->bridge;
	...
	if (bt_filter_can_send(&device->filter, type, data, len)) {
		leave_critical_section(flags);
#ifdef CONFIG_BLUETOOTH_BRIDGE_BTSNOOP
		snoop_dump(bridge->snoop, data - drv->head_reserve, len + drv->head_reserve, 0,
...
static int bt_bridge_receive(FAR struct bt_driver_s *drv, enum bt_buf_type_e type, FAR void *data,
							 size_t len)
{
...
static int bt_bridge_ioctl(FAR struct bt_driver_s *drv, int cmd, unsigned long arg)
{
	FAR struct bt_bridge_device_s *device = (FAR struct bt_bridge_device_s *)drv;
	FAR struct bt_bridge_s *bridge = device->bridge;
	FAR struct bt_driver_s *driver = bridge->driver;
	...
	case SIOCBTSNOOPOPEN: {
		...
		FAR struct snoop_s *snoop = (FAR struct snoop_s *)kmm_zalloc(sizeof(struct snoop_s));
		if (!snoop) {
			return -ENOMEM;
		}
...
static void bt_device_init(FAR struct bt_bridge_s *bridge, FAR struct bt_driver_s **drv,
						   uint8_t type)
{
...
int bt_bridge_register(FAR struct bt_driver_s *hcidrv, FAR struct bt_driver_s **btdrv,
