...
#include <roux/wireless/bluetooth/bt_slip.h>
...
static int bt_slip_send(FAR struct bt_driver_s *dev, enum bt_buf_type_e type, FAR void *data,
						size_t len);

static int bt_slip_ioctl(FAR struct bt_driver_s *dev, int cmd, unsigned long arg);
...
static const uint8_t g_sync_req[] = {0x01, 0x7e};

static const uint8_t g_sync_rsp[] = {0x02, 0x7d};

static const uint8_t g_conf_req[] = {0x03, 0xfc, 0x10 | CONFIG_BLUETOOTH_SLIP_TXWIN};

static const uint8_t g_conf_rsp[] = {0x04, 0x7b};
...
{
	return (bt_slip_bit8_reverse(x & 0xff) << 8) | bt_slip_bit8_reverse(x >> 8);
}
...
static FAR uint8_t *bt_slip_unslip_byte(FAR uint8_t *packet, FAR uint8_t *byte)
{
	uint8_t offset = 0;
...
static int bt_slip_send_packet(FAR struct sliphci_s *priv, uint8_t type, FAR const uint8_t *payload,
							   size_t len)
{
	...
	*pend++ = SLIP_DELIMITER;
	...
	wlinfo("tx t:%d l:%d s:%d a:%d checksum:0x%04x \n", BT_SLIP_GET_PKT_TYPE(header),
		   BT_SLIP_GET_LEN(header), BT_SLIP_GET_SEQ(header), BT_SLIP_GET_ACK(header), checksum);

	return priv->drv->send(priv->drv, type, packet, pend - packet);
}

static void bt_slip_send_link_control(FAR struct sliphci_s *priv, FAR const void *data, size_t len)
{
...
static int bt_slip_link_packet_hanlde(FAR struct sliphci_s *priv, FAR const uint8_t *header,
									  size_t len)
{
	...
	if (memcmp(payload, g_sync_req, ARRAY_SIZE(g_sync_req)) == 0) {
		...
		if (priv->txwin > CONFIG_BLUETOOTH_SLIP_TXWIN) {
			wlerr("err: txwin(%d) overflow(%d)", priv->txwin, CONFIG_BLUETOOTH_SLIP_TXWIN);
			return ret;
		}
...
static int bt_slip_hci_packet_handle(FAR struct bt_driver_s *dev, uint8_t type, FAR void *data,
									 size_t len)
{
...
static int bt_slip_packet_receive(FAR struct sliphci_s *priv, FAR uint8_t *header, size_t len)
{
	...
	case HCI_ACLDATA_PKT:
	case HCI_ISODATA_PKT: {
		ret = bt_slip_hci_packet_handle(&priv->dev, BT_SLIP_GET_PKT_TYPE(header), payload,
										BT_SLIP_GET_LEN(header));
	} break;
						...
						size_t len)
{
	...
	if (work_available(&priv->retxworker)) {
		work_queue(HPWORK, &priv->retxworker, bt_slip_retx_work, priv, BT_SLIP_RTX_TIMEOUT);
	}
...
static int bt_slip_receive(FAR struct bt_driver_s *drv, enum bt_buf_type_e type, FAR void *data,
						   size_t len)
{
	...
	for (packet = data, cursor = data, header = data, state = PACKET_START;
		...
		case PACKET_HEADER: {
			...
			*cursor++ = byte;
			...
			wlinfo("rx t:%d l:%d s:%d a:%d", BT_SLIP_GET_PKT_TYPE(header), BT_SLIP_GET_LEN(header),
				   BT_SLIP_GET_SEQ(header), BT_SLIP_GET_ACK(header));
			...
			if (BT_SLIP_IS_RELIABLE(header) && BT_SLIP_GET_SEQ(header) != priv->txack) {
				wlerr("err: out of order packet arrived(%u != %u)", BT_SLIP_GET_SEQ(header),
					  priv->txack);
				state = PACKET_START;
				break;
			}
		...
		case PACKET_DICHECK: {
			...
			*cursor++ = byte;
			...
			if (bt_slip_bit16_reverse(checksum) != bt_slip_get_crc(cursor)) {
				wlerr("err: checksum(0x%04x) not match(0x%04x)", checksum, bt_slip_get_crc(cursor));
				state = PACKET_START;
				break;
			}
