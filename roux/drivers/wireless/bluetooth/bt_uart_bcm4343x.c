...
static void hciuart_cb(FAR const struct btuart_lowerhalf_s *lower, FAR void *param)
{
...
static int uartwriteconf(FAR const struct btuart_lowerhalf_s *lower, FAR sem_t *rxsem,
						 FAR const uint8_t *dout, uint32_t doutl, FAR const uint8_t *cmp,
						 uint32_t maxl)
{
	int ret;
	int gotlen = 0;
...
static int set_baudrate(FAR const struct btuart_lowerhalf_s *lower, FAR sem_t *rxsem,
						int targetspeed)
{
	int ret;
	uint8_t baudrate_cmd[] = {0x01,
							  0x18,
							  g_hcd_command_byte2,
							  0x06,
							  0x00,
	...
	const uint8_t baudrate_conf[] = {0x04, 0x0e, 0x04, 0x01, 0x18, g_hcd_command_byte2, 0x00};

	ret = uartwriteconf(lower, rxsem, baudrate_cmd, sizeof(baudrate_cmd), baudrate_conf,
						sizeof(baudrate_conf));
...
static int load_bcm4343x_firmware(FAR const struct btuart_lowerhalf_s *lower)
{
	...
	const uint8_t enter_download_mode[] = {0x01, g_hcd_patchram_command, g_hcd_command_byte2, 0x00};
	...
	ret = uartwriteconf(lower, &rxsem, enter_download_mode, sizeof(enter_download_mode),
	...
	while (rp < (g_bt_firmware_hcd + g_bt_firmware_len)) {
		...
		do {
			...
			ret = uartwriteconf(lower, &rxsem, rp, 3 + txlen, command_resp, sizeof(command_resp));
			if (ret) {
	...
	if (command == g_hcd_launch_command) {
		lower->write(lower, &istx, 1);
		ret = uartwriteconf(lower, &rxsem, rp, 3 + txlen, launch_resp, sizeof(launch_resp));
		if (ret != 0) {
...
int btuart_create(FAR const struct btuart_lowerhalf_s *lower, FAR struct bt_driver_s **driver)
{
	...
	upper = (FAR struct btuart_upperhalf_s *)kmm_zalloc(sizeof(struct btuart_upperhalf_s));
