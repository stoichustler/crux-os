...
#include <roux/wireless/bluetooth/bt_uart_shim.h>
...
static void hciuart_rxenable(FAR const struct btuart_lowerhalf_s *lower, bool enable);
static int hciuart_setbaud(FAR const struct btuart_lowerhalf_s *lower, uint32_t baud);
static ssize_t hciuart_read(FAR const struct btuart_lowerhalf_s *lower, FAR void *buffer,
							size_t buflen);
static ssize_t hciuart_write(FAR const struct btuart_lowerhalf_s *lower, FAR const void *buffer,
							 size_t buflen);
static ssize_t hciuart_rxdrain(FAR const struct btuart_lowerhalf_s *lower);
static int hciuart_ioctl(FAR const struct btuart_lowerhalf_s *lower, int cmd, unsigned long arg);
...
FAR struct btuart_lowerhalf_s *btuart_shim_getdevice(FAR const char *path)
{
	...
	n = (FAR struct hciuart_config_s *)kmm_zalloc(sizeof(struct hciuart_config_s));
