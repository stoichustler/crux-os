...
#include <roux/wireless/ieee802154/at86rf23x.h>
...
static int at86rf23x_writeframe(FAR struct spi_dev_s *spi, FAR uint8_t *frame, uint8_t len);
static uint8_t at86rf23x_readframe(FAR struct spi_dev_s *spi, FAR uint8_t *frame_rx);
static int at86rf23x_set_trxstate(FAR struct at86rf23x_dev_s *dev, uint8_t state, uint8_t force);
...
static const struct ieee802154_radioops_s at86rf23x_devops = {
	.ioctl = at86rf23x_ioctl, .rxenable = at86rf23x_rxenable, .transmit = at86rf23x_transmit};
...
static void at86rf23x_setregbits(FAR struct spi_dev_s *spi, uint8_t addr, uint8_t pos, uint8_t mask,
								 uint8_t val)
{
	uint8_t reg;
...
static uint8_t at86rf23x_getregbits(FAR struct spi_dev_s *spi, uint8_t addr, uint8_t pos,
									uint8_t mask)
{
	uint8_t val;
...
static int at86rf23x_set_trxstate(FAR struct at86rf23x_dev_s *dev, uint8_t state, uint8_t force)
{
	...
	if ((status != TRX_STATUS_TRXOFF) && (status != TRX_STATUS_RXON) &&
		(status != TRX_STATUS_PLLON) && (status != TRX_STATUS_RXAACKON) &&
		(status != TRX_STATUS_TXARETON) && (status != TRX_STATUS_PON)) {
	...
	case TRX_CMD_TRXOFF:
		...
		if ((status == TRX_STATUS_RXON) || (status == TRX_STATUS_PLLON) ||
			(status == TRX_STATUS_RXAACKON) || (status == TRX_STATUS_TXARETON)) {
	...
	case TRX_CMD_RX_ON:
		...
		} else if ((status == TRX_STATUS_RXAACKON) || (status == TRX_STATUS_RXON) ||
				   (status == TRX_STATUS_PLLON) || (status == TRX_STATUS_TXARETON)) {
			at86rf23x_setregbits(dev->spi, RF23X_TRXCMD_STATE, TRX_CMD_RX_ON);
			up_udelay(RF23X_TIME_TRANSITION_PLL_ACTIVE);
		}
...
static int at86rf23x_regdump(FAR struct at86rf23x_dev_s *dev)
{
	...
	for (i = 0; i < 0x30; i++) {
		...
		snprintf(buf + len, sizeof(buf) - len, "%02x ", at86rf23x_getreg(dev->spi, i));
		len += strlen(buf + len);
