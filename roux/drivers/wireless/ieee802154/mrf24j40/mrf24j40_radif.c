...
static void mrf24j40_setorder(FAR struct mrf24j40_radio_s *dev, uint8_t bo, uint8_t so);
...
static const uint8_t g_allones[8] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
...
static void mrf24j40_slpclkcal(FAR struct mrf24j40_radio_s *dev)
{
	...
	mrf24j40_setreg(dev->spi, MRF24J40_SLPCON1, 0x01 | MRF24J40_SLPCON1_CLKOUT_DISABLED);
	...
	while (!(mrf24j40_getreg(dev->spi, MRF24J40_SLPCAL2) & MRF24J40_SLPCAL2_SLPCALRDY)) {
		up_udelay(1);
	}
...
int mrf24j40_bind(FAR struct ieee802154_radio_s *radio, FAR struct ieee802154_radiocb_s *radiocb)
{
...
int mrf24j40_txdelayed(FAR struct ieee802154_radio_s *radio, FAR struct ieee802154_txdesc_s *txdesc,
...
int mrf24j40_reset(FAR struct ieee802154_radio_s *radio)
{
	...
	mrf24j40_setreg(
		dev->spi, MRF24J40_PACON2,
		0x98); /* 10011000 Enable FIFO (default), TXONTS=6 (recommended), TXONT<8:7>=0 */
	mrf24j40_setreg(
		dev->spi, MRF24J40_TXSTBL,
		0x95); /* 10010101 set the SIFS period. RFSTBL=9, MSIFS=5, aMinSIFSPeriod=14 (min 12) */
	mrf24j40_setreg(
		dev->spi, MRF24J40_TXPEND,
		0x7c); /* 01111100 set the LIFS period, MLIFS=1Fh=31 aMinLIFSPeriod=40 (min 40) */
	mrf24j40_setreg(dev->spi, MRF24J40_TXTIME,
					0x30); /* 00110000 set the turnaround time, TURNTIME=3 aTurnAroundTime=12 */
	mrf24j40_setreg(dev->spi, MRF24J40_RFCON1,
					0x02); /* 00000010 VCO optimization, recommended value */
	mrf24j40_setreg(dev->spi, MRF24J40_RFCON2, 0x80); /* 10000000 Enable PLL */
	mrf24j40_setreg(dev->spi, MRF24J40_RFCON6,
					0x90); /* 10010000 TX filter enable, fast 20M recovery, No bat monitor */
...
int mrf24j40_getattr(FAR struct ieee802154_radio_s *radio, enum ieee802154_attr_e attr,
...
int mrf24j40_setattr(FAR struct ieee802154_radio_s *radio, enum ieee802154_attr_e attr,
