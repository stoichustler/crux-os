...
static void xbee_assocworker(FAR void *arg)
{
	...
	if (priv->associating) {
		...
		wd_start(&priv->assocwd, XBEE_ASSOC_POLLDELAY, xbee_assoctimer, (wdparm_t)arg);
	}
}
...
int xbee_bind(XBEEHANDLE xbee, FAR struct xbee_maccb_s *cb)
{
	...
	for (prev = NULL, next = priv->cb; (next != NULL && cb->prio <= next->prio);
				  ...
				  FAR struct iob_s *frame)
{
	...
	frame->io_len++;
	xbee_insert_checksum(&frame->io_data[frame->io_offset], (frame->io_len - frame->io_offset));
	...
	do {
		...
		wd_start(&priv->reqdata_wd, XBEE_RESPONSE_TIMEOUT, xbee_reqdata_timeout, (wdparm_t)priv);
...
int xbee_req_get(XBEEHANDLE xbee, enum ieee802154_attr_e attr, FAR union ieee802154_attr_u *attrval)
{
	FAR struct xbee_priv_s *priv = (FAR struct xbee_priv_s *)xbee;
...
int xbee_req_associate(XBEEHANDLE xbee, FAR struct ieee802154_assoc_req_s *req)
{
	...
	return wd_start(&priv->assocwd, XBEE_ASSOC_POLLDELAY, xbee_assoctimer, (wdparm_t)priv);
}
