...
static void xbee_process_apiframes(FAR struct xbee_priv_s *priv, FAR struct iob_s *iob);
static void xbee_process_txstatus(FAR struct xbee_priv_s *priv, uint8_t frameid, uint8_t status);
static void xbee_process_rxframe(FAR struct xbee_priv_s *priv, FAR struct iob_s *frame,
								 enum ieee802154_addrmode_e addrmode);
static void xbee_notify(FAR struct xbee_priv_s *priv, FAR struct ieee802154_primitive_s *primitive);
...
static void xbee_process_apiframes(FAR struct xbee_priv_s *priv, FAR struct iob_s *framelist)
{
	...
	while (frame) {
		...
		case XBEE_APIFRAME_MODEMSTATUS: {
			...
			if (frame->io_data[frame->io_offset] == 0x06) {
				if (work_available(&priv->backupwork)) {
					work_queue(LPWORK, &priv->backupwork, xbee_backup_worker, (FAR void *)priv, 0);
				}
			}
#endif
		...
		case XBEE_APIFRAME_ATRESPONSE: {
			...
			if (frame->io_data[frame->io_offset]) {
				...
				if ((priv->querycmd[0] == *command) && (priv->querycmd[1] == *(command + 1))) {
...
static void xbee_notify(FAR struct xbee_priv_s *priv, FAR struct ieee802154_primitive_s *primitive)
{
	...
	if (work_available(&priv->notifwork)) {
		work_queue(LPWORK, &priv->notifwork, xbee_notify_worker, (FAR void *)priv, 0);
	}
}
...
static void xbee_lockupcheck_timeout(wdparm_t arg)
{
	...
	if (work_available(&priv->lockupwork)) {
		work_queue(LPWORK, &priv->lockupwork, xbee_lockupcheck_worker, (FAR void *)priv, 0);
	}
}
...
static void xbee_lockupcheck_reschedule(FAR struct xbee_priv_s *priv)
{
	...
	wd_start(&priv->lockup_wd, XBEE_LOCKUP_QUERYTIME, xbee_lockupcheck_timeout, (wdparm_t)priv);
}
...
XBEEHANDLE xbee_init(FAR struct spi_dev_s *spi, FAR const struct xbee_lower_s *lower)
{
...
void xbee_send_apiframe(FAR struct xbee_priv_s *priv, FAR const uint8_t *frame, uint16_t framelen)
{
...
int xbee_atquery(FAR struct xbee_priv_s *priv, FAR const char *atcommand)
{
	...
	do {
		...
		if (memcmp(atcommand, "WR", 2) != 0) {
			...
			wd_start(&priv->atquery_wd, XBEE_ATQUERY_TIMEOUT, xbee_atquery_timeout, (wdparm_t)priv);
		}
