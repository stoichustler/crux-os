...
#ifdef CONFIG_NET_6LOWPAN_EXTENDEDADDR
static int xbeenet_coord_eaddr(FAR struct radio_driver_s *radio, FAR uint8_t *eaddr);
#else
static int xbeenet_coord_saddr(FAR struct radio_driver_s *radio, FAR uint8_t *saddr);
#endif
#endif
...
static int xbeenet_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
static int xbeenet_get_mhrlen(FAR struct radio_driver_s *netdev, FAR const void *meta);
static int xbeenet_req_data(FAR struct radio_driver_s *netdev, FAR const void *meta,
...
static int xbeenet_set_ipaddress(FAR struct net_driver_s *dev)
{
	...
	ret = xbee_ioctl(priv->xd_mac, MAC802154IOC_MLME_GET_REQUEST, (unsigned long)((uintptr_t)&arg));
	if (ret < 0) {
...
static int xbeenet_notify(FAR struct xbee_maccb_s *maccb,
						  FAR struct ieee802154_primitive_s *primitive)
{
	FAR struct xbeenet_callback_s *cb = (FAR struct xbeenet_callback_s *)maccb;
	FAR struct xbeenet_driver_s *priv;
...
static int xbeenet_rxframe(FAR struct xbeenet_driver_s *priv, FAR struct ieee802154_data_ind_s *ind)
{
 ...
 *
 * Description:
 *   ROUX Callback: Bring up the IEEE 802.15.4 interface when an IP address
 *   is provided
 *
 * Input Parameters:
 *   dev - Reference to the ROUX driver state structure
...
static int xbeenet_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg)
{
	...
	if (_MAC802154IOCVALID(cmd)) {
		FAR struct ieee802154_netmac_s *netmac = (FAR struct ieee802154_netmac_s *)arg;
...
static int xbeenet_get_mhrlen(FAR struct radio_driver_s *netdev, FAR const void *meta)
{
	FAR struct xbeenet_driver_s *priv = (FAR struct xbeenet_driver_s *)netdev;
...
int xbee_netdev_register(XBEEHANDLE xbee)
{
	...
	priv = (FAR struct xbeenet_driver_s *)kmm_zalloc(sizeof(struct xbeenet_driver_s));
