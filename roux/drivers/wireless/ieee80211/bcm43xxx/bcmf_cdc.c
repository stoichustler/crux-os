...
static FAR struct bcmf_frame_s *bcmf_cdc_allocate_frame(FAR struct bcmf_dev_s *priv, FAR char *name,
														FAR uint8_t *data, uint32_t len);

static int bcmf_cdc_sendframe(FAR struct bcmf_dev_s *priv, uint32_t cmd, int ifidx, bool set,
							  FAR struct bcmf_frame_s *frame);

static int bcmf_cdc_control_request(FAR struct bcmf_dev_s *priv, uint32_t ifidx, bool set,
									uint32_t cmd, FAR char *name, FAR uint8_t *data,
									FAR uint32_t *len);

static int bcmf_cdc_control_request_unsafe(FAR struct bcmf_dev_s *priv, uint32_t ifidx, bool set,
										   uint32_t cmd, FAR char *name, FAR uint8_t *data,
										   FAR uint32_t *len);
														...
														FAR uint8_t *data, uint32_t len)
{
	...
	frame = priv->bus->allocate_frame(priv, sizeof(struct bcmf_cdc_header) + data_len + name_len,
									  true, true);
	...
	memcpy(frame->data + sizeof(struct bcmf_cdc_header) + name_len, data, data_len);

	return frame;
}

int bcmf_cdc_sendframe(FAR struct bcmf_dev_s *priv, uint32_t cmd, int ifidx, bool set,
					   FAR struct bcmf_frame_s *frame)
{
	FAR struct bcmf_cdc_header *header = (FAR struct bcmf_cdc_header *)frame->data;
...
int bcmf_cdc_control_request(FAR struct bcmf_dev_s *priv, uint32_t ifidx, bool set, uint32_t cmd,
							 FAR char *name, FAR uint8_t *data, FAR uint32_t *len)
{
	int ret;
	...
	ret = bcmf_cdc_control_request_unsafe(priv, ifidx, set, cmd, name, data, len);

	nxsem_post(&priv->control_mutex);
	return ret;
}

int bcmf_cdc_control_request_unsafe(FAR struct bcmf_dev_s *priv, uint32_t ifidx, bool set,
									uint32_t cmd, FAR char *name, FAR uint8_t *data,
									FAR uint32_t *len)
{
	int ret;
	...
	*len = 0;
	...
	ret =
		nxsem_tickwait_uninterruptible(&priv->control_timeout, MSEC2TICK(BCMF_CONTROL_TIMEOUT_MS));
	if (ret < 0) {
...
int bcmf_cdc_iovar_request(FAR struct bcmf_dev_s *priv, uint32_t ifidx, bool set, FAR char *name,
						   FAR uint8_t *data, FAR uint32_t *len)
{
	return bcmf_cdc_control_request(priv, ifidx, set, set ? WLC_SET_VAR : WLC_GET_VAR, name, data,
									len);
}

int bcmf_cdc_iovar_request_unsafe(FAR struct bcmf_dev_s *priv, uint32_t ifidx, bool set,
								  FAR char *name, FAR uint8_t *data, FAR uint32_t *len)
{
	return bcmf_cdc_control_request_unsafe(priv, ifidx, set, set ? WLC_SET_VAR : WLC_GET_VAR, name,
										   data, len);
}

int bcmf_cdc_ioctl(FAR struct bcmf_dev_s *priv, uint32_t ifidx, bool set, uint32_t cmd,
				   FAR uint8_t *data, FAR uint32_t *len)
{
...
int bcmf_cdc_process_control_frame(FAR struct bcmf_dev_s *priv, FAR struct bcmf_frame_s *frame)
{
	...
	if (cdc_header->flags >> BCMF_CONTROL_REQID_SHIFT == priv->control_reqid) {
		...
		if (priv->control_rxdata) {
			...
			memcpy(priv->control_rxdata, (FAR uint8_t *)&cdc_header[1], priv->control_rxdata_len);
		}
