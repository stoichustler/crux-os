...
#include <roux/wireless/ieee80211/bcmf_board.h>
...
#define REV16(x)                                                                                   \
	(((x & 0x000000ff) << 8) | ((x & 0x0000ff00) >> 8) | ((x & 0x00ff0000) << 8) |                 \
	 ((x & 0xff000000) >> 8))
...
static inline uint32_t bcmf_gspi_read_reg_32(FAR gspi_dev_t *gspi, enum gspi_cmd_func_e function,
											 uint32_t address)
{
	uint32_t buffer;
...
static inline uint16_t bcmf_gspi_read_reg_16(FAR gspi_dev_t *gspi, enum gspi_cmd_func_e function,
											 uint32_t address)
{
	uint32_t buffer;
...
static inline uint8_t bcmf_gspi_read_reg_8(FAR gspi_dev_t *gspi, enum gspi_cmd_func_e function,
										   uint32_t address)
{
	uint32_t buffer;
...
static inline void bcmf_gspi_write_reg_16(FAR gspi_dev_t *gspi, enum gspi_cmd_func_e function,
...
static inline void bcmf_gspi_write_reg_8(FAR gspi_dev_t *gspi, enum gspi_cmd_func_e function,
										 uint32_t address, uint32_t value)
{
...
static int bcmf_gspi_kso_enable(FAR bcmf_gspi_dev_t *gbus, bool enable)
{
	...
	if (enable) {
		...
		loops = 200;
		while (--loops > 0) {
			bcmf_gspi_write_reg_8(gspi, gspi_f1_backplane, SBSDIO_FUNC1_SLEEPCSR,
			...
			if ((value & (SBSDIO_FUNC1_SLEEPCSR_KSO_MASK | SBSDIO_FUNC1_SLEEPCSR_DEVON_MASK)) !=
				0) {
				break;
			}
		}
...
static int bcmf_gspi_bus_sleep(FAR bcmf_gspi_dev_t *gbus, bool sleep)
{
	...
	if (sleep) {
		...
		gbus->sleeping = true;
		bcmf_gspi_write_reg_8(gspi, gspi_f1_backplane, SBSDIO_FUNC1_CHIPCLKCSR, 0);
		...
		while (--loops > 0) {
			...
			bcmf_gspi_write_reg_8(gspi, gspi_f1_backplane, SBSDIO_FUNC1_CHIPCLKCSR,
...
static int bcmf_gspi_thread(int argc, FAR char **argv)
{
	...
	while (gbus->thread_run) {
		...
		if (gbus->irq_pending) {
			...
			intr_flags = bcmf_gspi_read_reg_16(gspi, gspi_f0_bus, CYW_REG_INTERRUPT);

			if (intr_flags != 0) {
				bcmf_gspi_write_reg_16(gspi, gspi_f0_bus, CYW_REG_INTERRUPT, intr_flags);
			}
		}
		...
		if (wait_for_event) {
			...
			if (enter_low_power) {
				...
				ret = nxsem_tickwait_uninterruptible(&gbus->thread_signal,
...
static int bcmf_gspi_init_device(FAR bcmf_gspi_dev_t *gbus)
{
	...
	for (i = 0; i < BCMF_GSPI_READY_TRYS; ++i) {
		ret = gspi->read(gspi, true, gspi_f0_bus_rev16, CYW_REG_TEST_RO, 4, buffer);
	...
	data = CYW_REG_SETUP_WORD_LEN_32 | CYW_REG_SETUP_BIG_ENDIAN | CYW_REG_SETUP_HIGH_SPEED |
		   CYW_REG_SETUP_INT_POLARITY | CYW_REG_SETUP_WAKE_UP | CYW_REG_STAT_ENA_INTR_STAT;
...
static int bcmf_gspi_setup_interrupts(FAR bcmf_gspi_dev_t *gbus)
{
	...
	bcmf_gspi_write_reg_16(gspi, gspi_f0_bus, CYW_REG_INTERRUPT,
						   CYW_REG_INTERRUPT_DATA_NOT_AVAIL | CYW_REG_INTERRUPT_COMMAND_ERROR |
							   CYW_REG_INTERRUPT_DATA_ERROR | CYW_REG_INTERRUPT_F1_OVERFLOW);

	/* We only want an interrupt if an F2 packet is available */

	bcmf_gspi_write_reg_16(gspi, gspi_f0_bus, CYW_REG_INTR_ENA, CYW_REG_INTR_ENA_F2_PKT_AVAIL);
...
static int bcmf_gspi_init_alp_clock(FAR bcmf_gspi_dev_t *gbus)
{
	...
	while (--loops > 0) {
		...
		value = bcmf_gspi_read_reg_8(gspi, gspi_f1_backplane, SBSDIO_FUNC1_CHIPCLKCSR);
...
static int bcmf_gspi_init_save_restore(FAR bcmf_gspi_dev_t *gbus)
{
	...
	if (srctrl != 0) {
		...
		bcmf_write_reg(gbus, 0, SDIO_CCCR_BRCM_CARDCAP, SDIO_CCCR_BRCM_CARDCAP_CMD_NODEC);
		bcmf_write_reg(gbus, 1, SBSDIO_FUNC1_CHIPCLKCSR, SBSDIO_FORCE_HT);
...
static int bcmf_bus_gspi_initialize(FAR struct bcmf_dev_s *priv, FAR struct gspi_dev_s *gspi)
{
	FAR bcmf_gspi_dev_t *gbus;
...
int bcmf_bus_gspi_active(FAR struct bcmf_dev_s *priv, bool active)
{
	...
	argv[0] = arg1;
	argv[1] = NULL;
	ret = kthread_create(BCMF_GSPI_THREAD_NAME, CONFIG_IEEE80211_BROADCOM_SCHED_PRIORITY,
						 BCMF_GSPI_THREAD_STACK_SIZE, bcmf_gspi_thread, argv);
	if (ret <= 0) {
...
int bcmf_transfer_bytes(FAR bcmf_gspi_dev_t *gbus, bool write, uint8_t function, uint32_t address,
						uint8_t *buf, unsigned int len)
{
	...
	if (write) {
		ret = gspi->write(gspi, true, function, address, len, (FAR uint32_t *)buf);
	...
	ret = gspi->read(gspi, true, function, address, len, (FAR uint32_t *)buf);
...
int bcmf_read_reg(FAR bcmf_gspi_dev_t *gbus, uint8_t function, uint32_t address, uint8_t *reg)
{
...
int bcmf_write_reg(FAR bcmf_gspi_dev_t *gbus, uint8_t function, uint32_t address, uint8_t reg)
{
