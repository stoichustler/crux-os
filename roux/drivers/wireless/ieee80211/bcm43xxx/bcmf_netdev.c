...
static int bcmf_transmit(FAR struct bcmf_dev_s *priv, FAR struct bcmf_frame_s *frame);
...
static int bcmf_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
...
int bcmf_netdev_alloc_tx_frame(FAR struct bcmf_dev_s *priv)
{
	...
	priv->cur_tx_frame = bcmf_bdc_allocate_frame(priv, MAX_NETDEV_PKTSIZE, false);
...
static int bcmf_transmit(FAR struct bcmf_dev_s *priv, struct bcmf_frame_s *frame)
{
	int ret;

	frame->len = priv->bc_dev.d_len + (unsigned int)(frame->data - frame->base);
...
void bcmf_netdev_notify_tx(FAR struct bcmf_dev_s *priv)
{
	...
	if (work_available(&priv->bc_pollwork)) {
		work_queue(BCMFWORK, &priv->bc_pollwork, bcmf_tx_poll_work, priv, 0);
	}
}
...
static int bcmf_ifup(FAR struct net_driver_s *dev)
{
	...
	if (memcmp(&priv->bc_dev.d_mac.ether, &zmac, sizeof(zmac)) != 0) {
		...
		bcmf_cdc_iovar_request(priv, CHIP_STA_INTERFACE, true, IOVAR_STR_CUR_ETHERADDR,
							   priv->bc_dev.d_mac.ether.ether_addr_octet, &out_len);
	}

	/* Query MAC address */

	out_len = ETHER_ADDR_LEN;
	ret = bcmf_cdc_iovar_request(priv, CHIP_STA_INTERFACE, false, IOVAR_STR_CUR_ETHERADDR,
								 priv->bc_dev.d_mac.ether.ether_addr_octet, &out_len);
	if (ret != OK) {
		goto errout_in_wl_active;
	}
...
#ifdef CONFIG_IEEE80211_BROADCOM_LOWPOWER
static bool bcmf_lowpower_expiration(FAR struct bcmf_dev_s *priv, FAR struct work_s *work,
...
static void bcmf_lowpower_ifdown_work(FAR void *arg)
{
	...
	if (bcmf_lowpower_expiration(arg, &priv->lp_work_ifdown, bcmf_lowpower_ifdown_work,
...
static void bcmf_lowpower_poll(FAR struct bcmf_dev_s *priv)
{
	if (priv->bc_bifup) {
		...
		if (work_available(&priv->lp_work_dtim) && priv->lp_dtim != LP_DTIM_INTERVAL) {
			work_queue(LPWORK, &priv->lp_work_dtim, bcmf_lowpower_work, priv,
					   SEC2TICK(LP_DTIM_TIMEOUT));
		}

		if (work_available(&priv->lp_work_ifdown)) {
			work_queue(LPWORK, &priv->lp_work_ifdown, bcmf_lowpower_ifdown_work, priv,
					   SEC2TICK(LP_IFDOWN_TIMEOUT));
		}
	}
}
