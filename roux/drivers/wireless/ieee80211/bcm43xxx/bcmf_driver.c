...
#include <roux/wdog.h>
#include <roux/sdio.h>
#include <roux/wireless/ieee80211/ieee80211.h>
...
#define MAX_CHUNK_LEN (CONFIG_NET_ETH_PKTSIZE > 1514 ? 1400 : CONFIG_NET_ETH_PKTSIZE - 114)
...
static void bcmf_wl_radio_event_handler(FAR struct bcmf_dev_s *priv, FAR struct bcmf_event_s *event,
										unsigned int len);

static void bcmf_wl_scan_event_handler(FAR struct bcmf_dev_s *priv, FAR struct bcmf_event_s *event,
									   unsigned int len);

static void bcmf_wl_auth_event_handler(FAR struct bcmf_dev_s *priv, FAR struct bcmf_event_s *event,
									   unsigned int len);

static int bcmf_wl_get_interface(FAR struct bcmf_dev_s *priv, FAR struct iwreq *iwr);
...
int bcmf_wl_set_mac_address(FAR struct bcmf_dev_s *priv, struct ifreq *req)
{
	...
	ret = bcmf_cdc_iovar_request(priv, CHIP_STA_INTERFACE, true, IOVAR_STR_CUR_ETHERADDR,
								 (FAR uint8_t *)req->ifr_hwaddr.sa_data, &out_len);
	if (ret != OK) {
		return ret;
	}

	wlinfo("MAC address updated %02X:%02X:%02X:%02X:%02X:%02X\n", req->ifr_hwaddr.sa_data[0],
		   req->ifr_hwaddr.sa_data[1], req->ifr_hwaddr.sa_data[2], req->ifr_hwaddr.sa_data[3],
		   req->ifr_hwaddr.sa_data[4], req->ifr_hwaddr.sa_data[5]);

	memcpy(priv->bc_dev.d_mac.ether.ether_addr_octet, req->ifr_hwaddr.sa_data, ETHER_ADDR_LEN);
...
int bcmf_driver_download_clm(FAR struct bcmf_dev_s *priv)
{
	...
	ret = file_open(&finfo, CONFIG_IEEE80211_BROADCOM_FWCLMNAME, O_RDONLY | O_CLOEXEC);
	if (ret < 0) {
	...
	do {
		...
		nread = file_read(&finfo, downloadbuff + sizeof(struct wl_dload_data), chunk_len);
		if (nread < 0) {
			ret = (int)nread;
		...
		ret = bcmf_cdc_iovar_request(priv, CHIP_STA_INTERFACE, true, IOVAR_STR_CLMLOAD,
									 downloadbuff, &out_len);
...
int bcmf_wl_active(FAR struct bcmf_dev_s *priv, bool active)
{
	...
	*(FAR uint32_t *)tmp_buf = 0;
	ret = bcmf_cdc_iovar_request(priv, interface, true, IOVAR_STR_TX_GLOM, tmp_buf, &out_len);
	if (ret != OK) {
		goto errout_in_sdio_active;
	}
	...
	out_len = 4;
	value = PM_OFF;
	ret = bcmf_cdc_ioctl(priv, interface, true, WLC_SET_PM, (FAR uint8_t *)&value, &out_len);
	if (ret != OK) {
		goto errout_in_sdio_active;
	}
	...
	out_len = 4;
	value = GMODE_AUTO;
	ret = bcmf_cdc_ioctl(priv, interface, true, WLC_SET_GMODE, (FAR uint8_t *)&value, &out_len);
	if (ret != OK) {
		goto errout_in_sdio_active;
	}
	...
	out_len = 4;
	value = 1;
	ret = bcmf_cdc_iovar_request(priv, interface, true, IOVAR_STR_ROAM_OFF, (FAR uint8_t *)&value,
								 &out_len);
	if (ret != OK) {
		goto errout_in_sdio_active;
	}
	...
	ret = bcmf_cdc_iovar_request(priv, interface, true, "bsscfg:" IOVAR_STR_SUP_WPA2_EAPVER,
								 tmp_buf, &out_len);
	if (ret != OK) {
		goto errout_in_sdio_active;
	}
	...
	ret = bcmf_cdc_iovar_request(priv, interface, false, IOVAR_STR_VERSION, tmp_buf, &out_len);
	if (ret != OK) {
		goto errout_in_sdio_active;
	}
...
int bcmf_driver_initialize(FAR struct bcmf_dev_s *priv)
{
	...
	bcmf_event_register(priv, bcmf_wl_auth_event_handler, WLC_E_DEAUTH);
...
void bcmf_wl_radio_event_handler(FAR struct bcmf_dev_s *priv, FAR struct bcmf_event_s *event,
								 unsigned int len)
{
...
void bcmf_wl_auth_event_handler(FAR struct bcmf_dev_s *priv, FAR struct bcmf_event_s *event,
								unsigned int len)
{
	bool auth = false;
	int carrier = -1;
	...
	} else if (type == WLC_E_DEAUTH || type == WLC_E_DEAUTH_IND || type == WLC_E_DISASSOC ||
			   type == WLC_E_DISASSOC_IND || (type == WLC_E_LINK && reason != 0)) {
		carrier = 0;
	}
...
void bcmf_wl_scan_event_handler(FAR struct bcmf_dev_s *priv, FAR struct bcmf_event_s *event,
								unsigned int len)
{
	...
	for (i = 0; i < result->bss_count; i++) {
		...
		wlinfo("Scan result: <%.32s> "
			   ...
			   "signal %d %d %d\n",
			   bss->SSID, bss->BSSID.ether_addr_octet[0], bss->BSSID.ether_addr_octet[1],
			   bss->BSSID.ether_addr_octet[2], bss->BSSID.ether_addr_octet[3],
			   bss->BSSID.ether_addr_octet[4], bss->BSSID.ether_addr_octet[5], bss->RSSI,
			   bss->phy_noise, bss->SNR);

		if (strnlen((FAR const char *)bss->SSID, sizeof(bss->SSID)) == 0) {
			continue;
		}
		...
		while (1) {
			...
			case WLAN_EID_RSN: {
				...
				suitelen = sizeof(*rsn) + rsn->scount * sizeof(wpa_cipher_suite_t);
			...
			case WLAN_EID_VENDOR_SPECIFIC: {
				FAR wpa_ie_fixed_t *ie = (FAR wpa_ie_fixed_t *)&ie_buffer[ie_offset];
				FAR wpa_akm_t *akm;
				FAR wpa_rsn_t *rsn;
				...
				rsn = (FAR wpa_rsn_t *)&ie_buffer[ie_offset + sizeof(wpa_ie_fixed_t) - 2];
				suitelen = sizeof(wpa_ie_fixed_t) + sizeof(*rsn) +
						   rsn->scount * sizeof(wpa_cipher_suite_t) - 2;
...
vaild_bss:
		...
		for (j = 0; j < priv->scan_result_entries; j++) {
			...
			if (memcmp(&curr->BSSID, &bss[i].BSSID, sizeof(curr->BSSID)) == 0 ||
				memcmp(&curr->SSID, &bss[i].SSID, sizeof(curr->SSID)) == 0) {
...
static int bcmf_wl_scan_format_results(FAR struct bcmf_dev_s *priv, FAR struct iwreq *iwr)
{
	...
	len =
		IW_EV_LEN(ap_addr) + IW_EV_LEN(qual) + IW_EV_LEN(freq) + IW_EV_LEN(data) + IW_EV_LEN(essid);

	len *= priv->scan_result_entries;

	for (i = 0; i < priv->scan_result_entries; i++) {
		scan_result[i] = &priv->scan_result[i];
		len += (MIN(strlen((FAR const char *)scan_result[i]->SSID), 32) + 3) & ~3;
	}
	...
	for (i = 0; i < priv->scan_result_entries; i++) {
		...
		memcpy(&iwe->u.ap_addr.sa_data, info->BSSID.ether_addr_octet, IFHWADDRLEN);
...
int bcmf_wl_enable(FAR struct bcmf_dev_s *priv, bool enable)
{
	...
	out_len = 0;
	ret =
		bcmf_cdc_ioctl(priv, CHIP_STA_INTERFACE, true, enable ? WLC_UP : WLC_DOWN, NULL, &out_len);
...
int bcmf_wl_start_scan(FAR struct bcmf_dev_s *priv, FAR struct iwreq *iwr)
{
	...
	memset(&scan_params.params.bssid, 0xff, sizeof(scan_params.params.bssid));
	...
	if (iwr->u.data.pointer && iwr->u.data.length >= sizeof(*req)) {
		...
		memcpy(&scan_params.params.bssid, req->bssid.sa_data, sizeof(scan_params.params.bssid));

		scan_params.params.scan_type = req->scan_type == IW_SCAN_TYPE_ACTIVE ? 0 : 1;
	...
	out_len = 4;
	if (bcmf_cdc_ioctl(priv, CHIP_STA_INTERFACE, true, WLC_SET_PASSIVE_SCAN, (FAR uint8_t *)&value,
					   &out_len)) {
		ret = -EIO;
		goto exit_failed;
	}
	...
	if (priv->scan_result == NULL) {
		priv->scan_result = kmm_malloc(sizeof(wl_bss_info_t) * BCMF_SCAN_RESULT_ENTRIES);
		if (priv->scan_result == NULL) {
	...
	if (bcmf_cdc_iovar_request_unsafe(priv, CHIP_STA_INTERFACE, true, IOVAR_STR_ESCAN,
									  (FAR uint8_t *)&scan_params, &out_len)) {
		ret = -EIO;
		goto exit_sem_post;
	}
	...
	wd_start(&priv->scan_timeout, BCMF_SCAN_TIMEOUT_TICK, bcmf_wl_scan_timeout, (wdparm_t)priv);
...
int bcmf_wl_set_auth_param(FAR struct bcmf_dev_s *priv, struct iwreq *iwr)
{
	...
	case IW_AUTH_WPA_VERSION: {
		...
		if (bcmf_cdc_iovar_request(priv, interface, true, "bsscfg:" IOVAR_STR_SUP_WPA,
								   (FAR uint8_t *)wpa_version, &out_len)) {
			return -EIO;
		}

		out_len = 4;
		if (bcmf_cdc_ioctl(priv, interface, true, WLC_SET_WPA_AUTH, (FAR uint8_t *)&auth_mode,
						   &out_len)) {
			return -EIO;
		}
	}
	...
	case IW_AUTH_CIPHER_GROUP: {
		...
		out_len = 4;
		if (bcmf_cdc_ioctl(priv, interface, true, WLC_SET_WSEC, (FAR uint8_t *)&cipher_mode,
						   &out_len)) {
			return -EIO;
		}
		...
		out_len = 4;
		if (bcmf_cdc_ioctl(priv, interface, true, WLC_SET_AUTH, (FAR uint8_t *)&wep_auth,
						   &out_len)) {
			return -EIO;
		}
	}
...
int bcmf_wl_set_mode(FAR struct bcmf_dev_s *priv, struct iwreq *iwr)
{
	...
	return bcmf_cdc_ioctl(priv, interface, true, WLC_SET_INFRA, (FAR uint8_t *)&value, &out_len);
}
...
int bcmf_wl_get_mode(FAR struct bcmf_dev_s *priv, struct iwreq *iwr)
{
	...
	ret = bcmf_cdc_ioctl(priv, interface, false, WLC_GET_INFRA, (FAR uint8_t *)&infra, &out_len);
	if (ret == OK) {
		out_len = sizeof(ap);
		ret = bcmf_cdc_ioctl(priv, interface, false, WLC_GET_AP, (FAR uint8_t *)&ap, &out_len);
	}
...
int bcmf_wl_set_bssid(FAR struct bcmf_dev_s *priv, struct iwreq *iwr)
{
	...
	if (ret == OK) {
		out_len = sizeof(infra);
		ret =
			bcmf_cdc_ioctl(priv, interface, false, WLC_GET_INFRA, (FAR uint8_t *)&infra, &out_len);

		if (ret == OK) {
			out_len = sizeof(struct ether_addr);
			ret = bcmf_cdc_ioctl(priv, interface, true,
								 ((ap || !infra) ? WLC_SET_BSSID : WLC_REASSOC),
...
int bcmf_wl_get_channel(FAR struct bcmf_dev_s *priv, int interface)
{
	...
	out_len = sizeof(ci);
	ret = bcmf_cdc_ioctl(priv, interface, false, WLC_GET_CHANNEL, (FAR uint8_t *)&ci, &out_len);
...
int bcmf_wl_get_rate(FAR struct bcmf_dev_s *priv, struct iwreq *iwr)
{
	...
	out_len = sizeof(rate);
	ret = bcmf_cdc_ioctl(priv, interface, false, WLC_GET_RATE, (FAR uint8_t *)&rate, &out_len);
	if (ret == OK) {
...
int bcmf_wl_get_txpower(FAR struct bcmf_dev_s *priv, struct iwreq *iwr)
{
	...
	ret = bcmf_cdc_ioctl(priv, interface, false, WLC_GET_RADIO, (FAR uint8_t *)&radio, &out_len);
	if (ret == OK) {
		out_len = sizeof(iwr->u.txpower.value);
		ret = bcmf_cdc_iovar_request(priv, interface, false, IOVAR_STR_QTXPOWER,
									 (FAR uint8_t *)&(iwr->u.txpower.value), &out_len);
		if (ret == OK) {
...
int bcmf_wl_get_rssi(FAR struct bcmf_dev_s *priv, struct iwreq *iwr)
{
	...
	out_len = sizeof(rssi);
	ret = bcmf_cdc_ioctl(priv, interface, false, WLC_GET_RSSI, (FAR uint8_t *)&rssi, &out_len);
	if (ret == OK) {
...
int bcmf_wl_set_ssid(FAR struct bcmf_dev_s *priv, struct iwreq *iwr)
{
	...
	ret = bcmf_cdc_ioctl(priv, interface, true, WLC_DISASSOC, (FAR uint8_t *)&scbval, &out_len);
	...
	if (priv->auth_pending) {
		...
		ret = bcmf_cdc_ioctl(priv, interface, true, WLC_SET_WSEC_PMK,
	...
	out_len = sizeof(ssid);
	ret = bcmf_cdc_ioctl(priv, interface, true, WLC_SET_SSID, (FAR uint8_t *)&ssid, &out_len);
	if (ret == OK) {
		ret = nxsem_tickwait_uninterruptible(priv->auth_signal, MSEC2TICK(BCMF_AUTH_TIMEOUT_MS));
	}
...
int bcmf_wl_get_ssid(FAR struct bcmf_dev_s *priv, struct iwreq *iwr)
{
	...
	out_len = sizeof(ssid);
	ret = bcmf_cdc_ioctl(priv, interface, false, WLC_GET_SSID, (FAR uint8_t *)&ssid, &out_len);
	if (ret == OK) {
...
int bcmf_wl_set_country_code(FAR struct bcmf_dev_s *priv, int interface, FAR void *code)
{
	uint8_t country[4] = {0};
	...
	return bcmf_cdc_iovar_request(priv, interface, true, IOVAR_STR_COUNTRY, country, &out_len);
}
...
int bcmf_wl_get_country(FAR struct bcmf_dev_s *priv, FAR struct iwreq *iwr)
{
	...
	out_len = sizeof(country);
	ret = bcmf_cdc_iovar_request(priv, interface, false, IOVAR_STR_COUNTRY, country, &out_len);
	if (ret == OK) {
...
int bcmf_wl_set_dtim(FAR struct bcmf_dev_s *priv, uint32_t interval_ms)
{
	...
	ret = bcmf_cdc_iovar_request(priv, CHIP_STA_INTERFACE, true, IOVAR_STR_LISTEN_INTERVAL_DTIM,
...
int bcmf_wl_set_pta_priority(FAR struct bcmf_dev_s *priv, uint32_t prio)
{
	...
	ret = bcmf_cdc_iovar_request(priv, CHIP_STA_INTERFACE, true, IOVAR_STR_COEX_PARA,
								 (FAR uint8_t *)&pta_prio_map[prio], &out_len);
	if (ret == OK) {
