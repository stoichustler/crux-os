...
static int f2_frame_rx_fail(FAR bcmf_gspi_dev_t *gbus, bool retry)
{
	...
	if (retry) {
		...
		bcmf_write_sbregb(gbus, CORE_BUS_REG(gbus->chip->core_base[SDIOD_CORE_ID], tosbmailbox),
						  SMB_NAK);
	}
...
int process_f2_frame_header(FAR bcmf_interface_dev_t *gbus, FAR f2_frame_header_t *header)
{
	if (header->data_offset < sizeof(f2_frame_header_t) || header->data_offset > header->size) {
...
int bcmf_gspi_read_f2_frame(FAR struct bcmf_dev_s *priv, int frame_length)
{
	...
	ret = gspi->read(gspi, true, gspi_f2_dma, 0, frame_length, (FAR uint32_t *)iframe->data);
...
int bcmf_gspi_send_f2_frame(FAR struct bcmf_dev_s *priv)
{
	...
	iframe = list_remove_head_type(&gbus->tx_queue, bcmf_interface_frame_t, list_entry);
  ...
  bcmf_hexdump(iframe->header.base, iframe->header.len,
	...
	ret = gspi->write(gspi, true, gspi_f2_dma, 0, iframe->header.len,
