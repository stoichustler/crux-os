...
static int bcmf_core_set_backplane_window(FAR bcmf_interface_dev_t *ibus, uint32_t address);
static int bcmf_upload_binary(FAR bcmf_interface_dev_t *ibus, uint32_t address, uint8_t *buf,
							  unsigned int len);
...
int bcmf_core_set_backplane_window(FAR bcmf_interface_dev_t *ibus, uint32_t address)
{
	int ret;
	int i;
	...
	for (i = 1; i < 4; i++) {
		...
		if (addr_part != cur_addr_part) {
			...
			ret = bcmf_write_reg(ibus, 1, SBSDIO_FUNC1_SBADDRLOW + i - 1, addr_part);
...
int bcmf_upload_binary(FAR bcmf_interface_dev_t *ibus, uint32_t address, uint8_t *buf,
					   unsigned int len)
{
	unsigned int size;
	...
	while (len > 0) {
		...
		ret = bcmf_transfer_bytes(ibus, true, 1, address & SBSDIO_SB_OFT_ADDR_MASK, buf, size);
		if (ret != OK) {
	...
	while (validate_len > 0) {
		...
		ret = bcmf_transfer_bytes(ibus, false, 1, validate_address & SBSDIO_SB_OFT_ADDR_MASK,
								  compare_buffer, size);
		if (ret != OK) {
			wlerr("validate transfer failed %d %x %d\n", ret, validate_address, size);
			return ret;
		}
...
int bcmf_upload_file(FAR bcmf_interface_dev_t *ibus, uint32_t address, FAR const char *path)
{
	struct file finfo;
	FAR uint8_t *buf;
	size_t total_read;
	...
	do {
		...
		ret =
			bcmf_transfer_bytes(ibus, true, 1, address & SBSDIO_SB_OFT_ADDR_MASK, buf, total_read);
		if (ret < 0) {
			wlerr("ERROR: Transfer failed address=%lx total_read=%lu: %d\n", (unsigned long)address,
				  (unsigned long)total_read, ret);
			goto errout_with_buf;
		}
...
out:
	...
	ret = bcmf_upload_binary(ibus, ibus->chip->ram_size - 4 - nvram_sz + ibus->chip->ram_base,
							 nvram_buf, nvram_sz);
	...
	return bcmf_write_sbreg(ibus, ibus->chip->ram_size - 4 + ibus->chip->ram_base,
							(FAR uint8_t *)&token, 4);
}
...
int bcmf_read_sbreg(FAR bcmf_interface_dev_t *ibus, uint32_t address, FAR uint8_t *reg,
					unsigned int len)
{
	int ret = bcmf_core_set_backplane_window(ibus, address);
	if (ret != OK) {
...
int bcmf_write_sbreg(FAR bcmf_interface_dev_t *ibus, uint32_t address, FAR uint8_t *reg,
					 unsigned int len)
{
	int ret = bcmf_core_set_backplane_window(ibus, address);
	if (ret != OK) {
...
int bcmf_core_upload_firmware(FAR bcmf_interface_dev_t *ibus)
{
...
#if defined(CONFIG_IEEE80211_BROADCOM_BCM4301X) || defined(CONFIG_IEEE80211_BROADCOM_BCM43362) ||  \
	defined(CONFIG_IEEE80211_BROADCOM_BCM43438) || defined(CONFIG_IEEE80211_INFINEON_CYW43439)
...
#if defined(CONFIG_IEEE80211_BROADCOM_BCM43438) || defined(CONFIG_IEEE80211_INFINEON_CYW43439)
	...
	case SDIO_DEVICE_ID_BROADCOM_43455:
		...
		bcmf_core_reset(ibus, WLAN_ARMCR4_CORE_ID, value, ARMCR4_BCMA_IOCTL_CPUHALT,
...
#ifdef CONFIG_IEEE80211_BROADCOM_FWFILES
	ret = bcmf_upload_file(ibus, ibus->chip->ram_base, CONFIG_IEEE80211_BROADCOM_FWFILENAME);
#else
	wlinfo("firmware size is %d bytes\n", *ibus->chip->firmware_image_size);

	ret = bcmf_upload_binary(ibus, ibus->chip->ram_base, ibus->chip->firmware_image,
							 *ibus->chip->firmware_image_size);
#endif
	...
	case SDIO_DEVICE_ID_BROADCOM_43455:
		...
		bcmf_write_sbregw(ibus, CORE_BUS_REG(ibus->chip->core_base[SDIOD_CORE_ID], intstatus),
						  0xffffffff);

		/* Write reset vector to address 0 */

		ret = bcmf_upload_binary(ibus, 0, ibus->chip->firmware_image, 4);
		if (ret < 0) {
			return ret;
		}

		bcmf_core_reset(ibus, WLAN_ARMCR4_CORE_ID, ARMCR4_BCMA_IOCTL_CPUHALT, 0, 0);
...
void bcmf_core_disable(FAR bcmf_interface_dev_t *ibus, unsigned int core, uint32_t prereset,
					   uint32_t reset)
{
	uint32_t value;
	...
	if ((value & BCMA_RESET_CTL_RESET) == 0) {
		...
		bcmf_write_sbregw(ibus, base + BCMA_IOCTL, prereset | BCMA_IOCTL_FGC | BCMA_IOCTL_CLK);
		bcmf_read_sbregw(ibus, base + BCMA_IOCTL, &value);
	...
	bcmf_write_sbregw(ibus, base + BCMA_IOCTL, reset | BCMA_IOCTL_FGC | BCMA_IOCTL_CLK);
	bcmf_read_sbregw(ibus, base + BCMA_IOCTL, &value);
	up_udelay(10);
}

void bcmf_core_reset(FAR bcmf_interface_dev_t *ibus, unsigned int core, uint32_t prereset,
