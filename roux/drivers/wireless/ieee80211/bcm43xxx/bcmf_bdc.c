...
struct bcmf_frame_s *bcmf_bdc_allocate_frame(FAR struct bcmf_dev_s *priv, uint32_t len, bool block)
{
	...
	frame = priv->bus->allocate_frame(priv, sizeof(struct bcmf_bdc_header) + len, block, false);
...
int bcmf_bdc_process_event_frame(FAR struct bcmf_dev_s *priv, struct bcmf_frame_s *frame)
{
	int data_size;
	...
	event_msg = (struct bcmf_event_msg *)(frame->data + sizeof(struct bcmf_bdc_header) +
...
int bcmf_event_register(FAR struct bcmf_dev_s *priv, event_handler_t handler, unsigned int event_id)
{
	if (event_id >= BCMF_EVENT_COUNT) {
		/* Invalid event id */
...
int bcmf_event_unregister(FAR struct bcmf_dev_s *priv, unsigned int event_id)
{
...
int bcmf_event_push_config(FAR struct bcmf_dev_s *priv)
{
	...
	if (bcmf_cdc_iovar_request(priv, CHIP_STA_INTERFACE, true, IOVAR_STR_EVENT_MSGS, event_mask,
							   &out_len)) {
		return -EIO;
	}
...
int bcmf_bdc_transmit_frame(FAR struct bcmf_dev_s *priv, struct bcmf_frame_s *frame)
{
	struct bcmf_bdc_header *header;
