...
#include <roux/wireless/ieee80211/bcmf_gpio.h>
...
int bcmf_set_gpio(FAR struct bcmf_dev_s *priv, int pin, bool value)
{
	...
	return bcmf_cdc_iovar_request(priv, CHIP_STA_INTERFACE, true, IOVAR_STR_GPIOOUT,
								  (FAR uint8_t *)&buffer, &buf_len);
}
...
int bcmf_get_gpio(FAR struct bcmf_dev_s *priv, int pin, bool *value)
{
	...
	ret = bcmf_cdc_iovar_request(priv, CHIP_STA_INTERFACE, false, IOVAR_STR_CCGPIOIN,
								 (FAR uint8_t *)&buffer, &buf_len);
	if (ret != OK) {
		return ret;
	}
