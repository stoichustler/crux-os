...
#include <roux/wireless/ieee80211/bcmf_sdio.h>
...
int bcmf_probe(FAR struct bcmf_sdio_dev_s *sbus)
{
	...
	ret = bcmf_write_reg(sbus, 0, SDIO_CCCR_INTEN, (1 << 0) | (1 << 1) | (1 << 2));
	if (ret != OK) {
		goto exit_error;
	}
	...
	if (value & SDIO_CCCR_HIGHSPEED_SHS) {
		...
		ret = bcmf_write_reg(sbus, 0, SDIO_CCCR_HIGHSPEED, SDIO_CCCR_HIGHSPEED_EHS);
		if (ret != OK) {
			goto exit_error;
		}
	} else {
...
int bcmf_bus_setup_interrupts(FAR struct bcmf_sdio_dev_s *sbus)
{
	...
	bcmf_write_sbregw(sbus, CORE_BUS_REG(sbus->chip->core_base[SDIOD_CORE_ID], hostintmask),
					  I_HMB_SW_MASK);

	bcmf_write_sbregb(sbus, CORE_BUS_REG(sbus->chip->core_base[SDIOD_CORE_ID], funcintmask), 2);
...
static int bcmf_bus_sdio_initialize(FAR struct bcmf_dev_s *priv, int minor,
									FAR struct sdio_dev_s *dev)
{
	...
	argv[0] = arg1;
	argv[1] = NULL;
	ret = kthread_create(BCMF_THREAD_NAME, CONFIG_IEEE80211_BROADCOM_SCHED_PRIORITY,
						 BCMF_THREAD_STACK_SIZE, bcmf_sdio_thread, argv);
	if (ret <= 0) {
...
int bcmf_transfer_bytes(FAR struct bcmf_sdio_dev_s *sbus, bool write, uint8_t function,
						uint32_t address, uint8_t *buf, unsigned int len)
{
	...
	if (len == 1) {
		if (write) {
			return sdio_io_rw_direct(sbus->sdio_dev, write, function, address, *buf, NULL);
		}

		return sdio_io_rw_direct(sbus->sdio_dev, write, function, address, 0, buf);
	}
	...
	return sdio_io_rw_extended(sbus->sdio_dev, write, function, address, true, buf, blocklen,
							   nblocks);
}
...
int bcmf_read_reg(FAR struct bcmf_sdio_dev_s *sbus, uint8_t function, uint32_t address,
				  uint8_t *reg)
{
	*reg = 0;
...
int bcmf_write_reg(FAR struct bcmf_sdio_dev_s *sbus, uint8_t function, uint32_t address,
				   uint8_t reg)
{
...
{
	FAR struct bcmf_dev_s *priv = (FAR struct bcmf_dev_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	FAR struct bcmf_sdio_dev_s *sbus = (FAR struct bcmf_sdio_dev_s *)priv->bus;
	...
	while (true) {
		...
		if ((sbus->intstatus & I_HMB_FRAME_IND) == 0 && list_is_empty(&sbus->tx_queue) &&
			!sbus->irq_pending) {
			...
			if (timeout == UINT_MAX) {
				...
				ret = nxsem_tickwait_uninterruptible(&sbus->thread_signal, timeout);
			}
		...
		if (sbus->irq_pending) {
			...
			bcmf_read_sbregw(sbus, CORE_BUS_REG(sbus->chip->core_base[SDIOD_CORE_ID], intstatus),
							 &sbus->intstatus);

			/* Clear interrupts */

			bcmf_write_sbregw(sbus, CORE_BUS_REG(sbus->chip->core_base[SDIOD_CORE_ID], intstatus),
							  sbus->intstatus);
		}
