...
static bcmf_interface_frame_t g_pktframes[CONFIG_IEEE80211_BROADCOM_FRAME_POOL_SIZE];
...
void bcmf_interface_free_frame(FAR struct bcmf_dev_s *priv, bcmf_interface_frame_t *iframe)
{
...
bcmf_interface_frame_t *bcmf_interface_allocate_frame(FAR struct bcmf_dev_s *priv, bool block,
													  bool tx)
{
	FAR bcmf_interface_dev_t *ibus = (FAR bcmf_interface_dev_t *)priv->bus;
	...
	while (1) {
		...
		if (!tx || ibus->tx_queue_count < CONFIG_IEEE80211_BROADCOM_FRAME_POOL_SIZE / 2) {
			if ((iframe = list_remove_head_type(&free_interface_frames, bcmf_interface_frame_t,
												list_entry)) != NULL) {
				if (tx) {
					ibus->tx_queue_count++;
				}
...
#else
	iframe->header.len = HEADER_SIZE + MAX_NETDEV_PKTSIZE + CONFIG_NET_GUARDSIZE;
#endif
