		...
		if ((filep->f_oflags & O_RDOK) != 0) {
			if (--dev->d_nreaders <= 0) {
				if (PIPE_IS_POLICY_0(dev->d_flags)) {
					...
					poll_notify(dev->d_fds, CONFIG_DEV_PIPE_NPOLLWAITERS, POLLERR);
					pipecommon_wakeup(&dev->d_wrsem);
				}
			}
		}
...
int pipecommon_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup)
{
	...
	if (setup) {
		...
		eventset = 0;
		if ((filep->f_oflags & O_WROK) && nbytes < (dev->d_bufsize - dev->d_polloutthrd)) {
			eventset |= POLLOUT;
		}
...
int pipecommon_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case PIPEIOC_PEEK: {
		...
		ret = circbuf_peekat(&dev->d_buffer, dev->d_buffer.tail + peek->offset, peek->buf,
							 peek->size);
	} break;
