...
static int lirc_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
static ssize_t lirc_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static ssize_t lirc_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static int lirc_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case LIRC_SET_REC_CARRIER:
		if (!lower->ops->s_rx_carrier_range) {
			...
			ret = lower->ops->s_rx_carrier_range(lower, fh->carrier_low, arg);
		}
		break;
...
static ssize_t lirc_write_pulse(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	FAR struct lirc_fh_s *fh = filep->f_priv;
	FAR struct lirc_lowerhalf_s *lower = fh->lower;
	size_t count;
...
static ssize_t lirc_write_scancode(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	FAR struct lirc_fh_s *fh = filep->f_priv;
	FAR struct lirc_lowerhalf_s *lower = fh->lower;
	...
	return lower->ops->tx_scancode(lower, (FAR struct lirc_scancode *)buffer);
}
...
static ssize_t lirc_read_scancode(FAR struct file *filep, FAR char *buffer, size_t length)
{
	FAR struct lirc_fh_s *fh = filep->f_priv;
	irqstate_t flags;
	ssize_t ret;

	if (length < sizeof(struct lirc_scancode) || length % sizeof(struct lirc_scancode)) {
		return -EINVAL;
	}
...
static ssize_t lirc_read_mode2(FAR struct file *filep, FAR char *buffer, size_t length)
{
	FAR struct lirc_fh_s *fh = filep->f_priv;
	irqstate_t flags;
	ssize_t ret = 0;
...
static ssize_t lirc_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	FAR struct lirc_fh_s *fh = filep->f_priv;
...
void lirc_raw_event(FAR struct lirc_lowerhalf_s *lower, struct lirc_raw_event_s ev)
{
	FAR struct lirc_upperhalf_s *upper = lower->priv;
	...
	if (ev.reset) {
		...
		if (upper->gap) {
			upper->gap_duration += (lirc_get_timestamp() / 1000) - upper->gap_start;
...
void lirc_scancode_event(FAR struct lirc_lowerhalf_s *lower, FAR struct lirc_scancode *lsc)
{
	FAR struct lirc_upperhalf_s *upper = lower->priv;
	FAR struct list_node *node;
	FAR struct list_node *tmp;
...
void lirc_sample_event(FAR struct lirc_lowerhalf_s *lower, unsigned int sample)
{
	FAR struct lirc_upperhalf_s *upper = lower->priv;
	FAR struct list_node *node;
	FAR struct list_node *tmp;
