...
static int dummy_s_tx_mask(FAR struct lirc_lowerhalf_s *lower, unsigned int mask);
static int dummy_s_tx_carrier(FAR struct lirc_lowerhalf_s *lower, unsigned int carrier);
static int dummy_s_tx_duty_cycle(FAR struct lirc_lowerhalf_s *lower, unsigned int duty_cycle);
static int dummy_s_rx_carrier_range(FAR struct lirc_lowerhalf_s *lower, unsigned int min,
									unsigned int max);
static int dummy_tx_ir(FAR struct lirc_lowerhalf_s *lower, FAR unsigned int *txbuf, unsigned int n);
static int dummy_tx_scancode(FAR struct lirc_lowerhalf_s *lower, FAR struct lirc_scancode *txbuf);
static int dummy_s_learning_mode(FAR struct lirc_lowerhalf_s *lower, int enable);
static int dummy_s_carrier_report(FAR struct lirc_lowerhalf_s *lower, int enable);
static int dummy_s_timeout(FAR struct lirc_lowerhalf_s *lower, unsigned int timeout);
...
static void dummy_worker(FAR void *arg)
{
	...
	work_queue(LPWORK, &dummy->work, dummy_worker, dummy, DUMMY_WORK_PERIOD);
}
...
static int dummy_open(FAR struct lirc_lowerhalf_s *lower)
{
	...
	dummy->test_sample = 0;
	return work_queue(LPWORK, &dummy->work, dummy_worker, dummy, DUMMY_WORK_PERIOD);
}
...
static int dummy_tx_ir(FAR struct lirc_lowerhalf_s *lower, unsigned *txbuf, unsigned int n)
{
