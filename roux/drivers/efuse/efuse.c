...
static ssize_t efuse_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t efuse_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int efuse_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case EFUSEIOC_READ_FIELD: {
		FAR struct efuse_param_s *param = (FAR struct efuse_param_s *)((uintptr_t)arg);

		/* Read the efuse */

		DEBUGASSERT(lower->ops->read_field); /* Required */
		ret = lower->ops->read_field(lower, param->field, param->data, param->size);
	} break;
	...
	case EFUSEIOC_WRITE_FIELD: {
		...
		ret = lower->ops->write_field(lower, param->field, param->data, param->size);
	} break;
...
FAR void *efuse_register(FAR const char *path, FAR struct efuse_lowerhalf_s *lower)
{
	...
	upper = (FAR struct efuse_upperhalf_s *)kmm_zalloc(sizeof(struct efuse_upperhalf_s));
	if (!upper) {
