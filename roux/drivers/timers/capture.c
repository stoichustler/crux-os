...
static ssize_t cap_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int cap_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case CAPIOC_ALL: {
		FAR struct cap_all_s *ptr = (FAR struct cap_all_s *)((uintptr_t)arg);
		DEBUGASSERT(ptr);
		for (i = 0; i < upper->nchan; i++) {
	...
	default: {
		...
		FAR unsigned long int *ptr = (FAR unsigned long int *)arg;
		for (i = 0; i < upper->nchan; i++) {
			ret = lower[i]->ops->ioctl(lower[i], cmd, (unsigned long)&ptr[i]);
			if (ret < 0) {
				break;
			}
		}
