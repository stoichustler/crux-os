...
int up_rtc_getdatetime(FAR struct tm *tp)
{
	...
	} while ((buffer[0] & DSXXXX_TIME_SEC_BCDMASK) > (seconds & DSXXXX_TIME_SEC_BCDMASK));
...
int up_rtc_settime(FAR const struct timespec *tp)
{
	...
	} while ((buffer[1] & DSXXXX_TIME_SEC_BCDMASK) > (seconds & DSXXXX_TIME_SEC_BCDMASK));
