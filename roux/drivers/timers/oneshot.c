...
static ssize_t oneshot_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static void oneshot_callback(FAR struct oneshot_lowerhalf_s *lower, FAR void *arg)
{
	...
	nxsig_notification(priv->od_pid, &priv->od_event, SI_QUEUE, &priv->od_work);
}
...
static int oneshot_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case OSIOC_START: {
		...
		ret = ONESHOT_START(priv->od_lower, oneshot_callback, priv, &start->ts);
	} break;
...
int oneshot_register(FAR const char *devname, FAR struct oneshot_lowerhalf_s *lower)
{
	...
	priv = (FAR struct oneshot_dev_s *)kmm_zalloc(sizeof(struct oneshot_dev_s));
