...
#include <roux/timers/rpmsg_rtc.h>
#include <roux/timers/arch_rtc.h>
...
#ifndef CONFIG_RTC_RPMSG_SERVER
static void rpmsg_rtc_device_created(FAR struct rpmsg_device *rdev, FAR void *priv);
static void rpmsg_rtc_device_destroy(FAR struct rpmsg_device *rdev, FAR void *priv);
static void rpmsg_rtc_alarm_fire_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
										 uint32_t src, FAR void *priv);
static int rpmsg_rtc_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
							uint32_t src, FAR void *priv);

static int rpmsg_rtc_send_recv(FAR struct rpmsg_rtc_lowerhalf_s *lower, uint32_t command,
							   FAR struct rpmsg_rtc_header_s *msg, int len);
static int rpmsg_rtc_rdtime(FAR struct rtc_lowerhalf_s *lower, FAR struct rtc_time *rtctime);
static int rpmsg_rtc_settime(FAR struct rtc_lowerhalf_s *lower, FAR const struct rtc_time *rtctime);
...
static int rpmsg_rtc_cancelalarm(FAR struct rtc_lowerhalf_s *lower, int alarmid);
static int rpmsg_rtc_rdalarm(FAR struct rtc_lowerhalf_s *lower_,
							 FAR struct lower_rdalarm_s *alarminfo);
#endif
#else
static int rpmsg_rtc_server_rdtime(FAR struct rtc_lowerhalf_s *lower, FAR struct rtc_time *rtctime);
...
static int rpmsg_rtc_server_setrelative(FAR struct rtc_lowerhalf_s *lower,
										FAR const struct lower_setrelative_s *relinfo);
static int rpmsg_rtc_server_cancelalarm(FAR struct rtc_lowerhalf_s *lower, int alarmid);
...
static int rpmsg_rtc_server_cancelperiodic(FAR struct rtc_lowerhalf_s *lower, int alarmid);
#endif
#ifdef CONFIG_RTC_IOCTL
static int rpmsg_rtc_server_ioctl(FAR struct rtc_lowerhalf_s *lower, int cmd, unsigned long arg);
#endif
...
static const struct rtc_ops_s g_rpmsg_rtc_ops = {
	rpmsg_rtc_rdtime,	rpmsg_rtc_settime,	   rpmsg_rtc_havesettime,
#ifdef CONFIG_RTC_ALARM
	rpmsg_rtc_setalarm, rpmsg_rtc_setrelative, rpmsg_rtc_cancelalarm, rpmsg_rtc_rdalarm,
#endif
};
#else
static struct rtc_ops_s g_rpmsg_rtc_server_ops = {
	rpmsg_rtc_server_rdtime,	  rpmsg_rtc_server_settime,		   rpmsg_rtc_server_havesettime,
#ifdef CONFIG_RTC_ALARM
	rpmsg_rtc_server_setalarm,	  rpmsg_rtc_server_setrelative,	   rpmsg_rtc_server_cancelalarm,
	rpmsg_rtc_server_rdalarm,
#endif
#ifdef CONFIG_RTC_PERIODIC
	rpmsg_rtc_server_setperiodic, rpmsg_rtc_server_cancelperiodic,
#endif
#ifdef CONFIG_RTC_IOCTL
...
static void rpmsg_rtc_device_created(FAR struct rpmsg_device *rdev, FAR void *priv)
{
	...
	if (strcmp(CONFIG_RTC_RPMSG_SERVER_NAME, rpmsg_get_cpuname(rdev)) == 0) {
		lower->ept.priv = lower;

		rpmsg_create_ept(&lower->ept, rdev, RPMSG_RTC_EPT_NAME, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
...
static int rpmsg_rtc_rdtime(FAR struct rtc_lowerhalf_s *lower, FAR struct rtc_time *rtctime)
{
	...
	ret = rpmsg_rtc_send_recv((FAR struct rpmsg_rtc_lowerhalf_s *)lower, RPMSG_RTC_GET,
...
static int rpmsg_rtc_settime(FAR struct rtc_lowerhalf_s *lower, FAR const struct rtc_time *rtctime)
{
	...
	return rpmsg_rtc_send_recv((FAR struct rpmsg_rtc_lowerhalf_s *)lower, RPMSG_RTC_SET,
							   (struct rpmsg_rtc_header_s *)&msg, sizeof(msg));
}
							  ...
							  FAR const struct lower_setalarm_s *alarminfo)
{
	...
	ret = rpmsg_rtc_send_recv(lower, RPMSG_RTC_ALARM_SET, (struct rpmsg_rtc_header_s *)&msg,
							  sizeof(msg));
	if (ret >= 0) {
...
static int rpmsg_rtc_cancelalarm(FAR struct rtc_lowerhalf_s *lower, int alarmid)
{
	...
	msg.id = alarmid;
	return rpmsg_rtc_send_recv((FAR struct rpmsg_rtc_lowerhalf_s *)lower, RPMSG_RTC_ALARM_CANCEL,
							   (struct rpmsg_rtc_header_s *)&msg, sizeof(msg));
}
...
static int rpmsg_rtc_server_rdtime(FAR struct rtc_lowerhalf_s *lower, FAR struct rtc_time *rtctime)
{
	FAR struct rpmsg_rtc_server_s *server = (FAR struct rpmsg_rtc_server_s *)lower;
...
static int rpmsg_rtc_server_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								   uint32_t src, FAR void *priv)
{
	FAR struct rpmsg_rtc_header_s *header = data;
	...
	case RPMSG_RTC_ALARM_SET: {
		FAR struct rpmsg_rtc_client_s *client = container_of(ept, struct rpmsg_rtc_client_s, ept);
...
static bool rpmsg_rtc_server_ns_match(FAR struct rpmsg_device *rdev, FAR void *priv,
...
static void rpmsg_rtc_server_ns_bind(FAR struct rpmsg_device *rdev, FAR void *priv,
									 FAR const char *name, uint32_t dest)
{
	...
	if (rpmsg_create_ept(&client->ept, rdev, RPMSG_RTC_EPT_NAME, RPMSG_ADDR_ANY, dest,
						 rpmsg_rtc_server_ept_cb, rpmsg_destroy_ept) < 0) {
		kmm_free(client);
		return;
	}
...
FAR struct rtc_lowerhalf_s *rpmsg_rtc_initialize(void)
{
	...
	if (lower != NULL) {
		...
		rpmsg_register_callback(lower, rpmsg_rtc_device_created, rpmsg_rtc_device_destroy, NULL,
								NULL);
	}
...
FAR struct rtc_lowerhalf_s *rpmsg_rtc_server_initialize(FAR struct rtc_lowerhalf_s *lower)
{
	...
	if (server) {
		...
		if (rpmsg_register_callback(server, NULL, NULL, rpmsg_rtc_server_ns_match,
