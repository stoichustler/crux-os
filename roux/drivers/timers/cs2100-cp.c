...
static int cs2100_write_reg(FAR const struct cs2100_config_s *config, uint8_t regaddr,
							uint8_t regval)
{
...
#ifdef CONFIG_CS2100CP_DEBUG
static int cs2100_read_reg(FAR const struct cs2100_config_s *config, uint8_t regaddr,
						   uint8_t *regval)
{
...
static int cs2100_write_ratio(FAR const struct cs2100_config_s *config, uint32_t ratio)
{
	struct i2c_msg_s msg;
	uint8_t buffer[5];
	int ret;
...
#ifdef CONFIG_CS2100CP_DEBUG
static int cs2100_read_ratio(FAR const struct cs2100_config_s *config, uint32_t *ratio)
{
	struct i2c_msg_s msg;
	uint8_t buffer[4];
	int ret;
	...
	if (ret >= 0) {
		...
		if (ret >= 0) {
			*ratio = ((uint32_t)buffer[0] << 24) | ((uint32_t)buffer[1] << 16) |
...
int cs2100_disable(FAR const struct cs2100_config_s *config)
{
	...
	regval = CS2100_DEVCTL_UNLOCK | CS2100_DEVCTL_AUXOUTDIS | CS2100_DEVCTL_CLKOUTDIS;
	ret = cs2100_write_reg(config, CS2100_DEVCTL, regval);
	if (ret < 0) {
		cserr("ERROR: Failed to set CS2100_DEVCTL: %d\n", ret);
		return ret;
