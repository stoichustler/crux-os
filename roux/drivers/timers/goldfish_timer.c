...
static int goldfish_timer_maxdelay(FAR struct oneshot_lowerhalf_s *lower, FAR struct timespec *ts);
static int goldfish_timer_start(FAR struct oneshot_lowerhalf_s *lower,
								FAR oneshot_callback_t callback, FAR void *arg,
								FAR const struct timespec *ts);
static int goldfish_timer_cancel(FAR struct oneshot_lowerhalf_s *lower, FAR struct timespec *ts);
static int goldfish_timer_current(FAR struct oneshot_lowerhalf_s *lower, FAR struct timespec *ts);
...
static int goldfish_timer_maxdelay(FAR struct oneshot_lowerhalf_s *lower_, FAR struct timespec *ts)
{
...
static int goldfish_timer_start(FAR struct oneshot_lowerhalf_s *lower_,
								...
								FAR const struct timespec *ts)
{
	FAR struct goldfish_timer_lowerhalf_s *lower = (FAR struct goldfish_timer_lowerhalf_s *)lower_;
	irqstate_t flags;
	uint64_t nsec;
	uint32_t l32;
	uint32_t h32;
...
static int goldfish_timer_cancel(FAR struct oneshot_lowerhalf_s *lower_, FAR struct timespec *ts)
{
...
static int goldfish_timer_current(FAR struct oneshot_lowerhalf_s *lower_, FAR struct timespec *ts)
{
	FAR struct goldfish_timer_lowerhalf_s *lower = (FAR struct goldfish_timer_lowerhalf_s *)lower_;
	irqstate_t flags;
	uint32_t l32;
...
static int goldfish_timer_interrupt(int irq, FAR void *context, FAR void *arg)
{
...
FAR struct oneshot_lowerhalf_s *goldfish_timer_initialize(uintptr_t base, int irq)
{
