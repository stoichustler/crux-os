...
#define WATCHDOG_AUTOMONITOR_TIMEOUT (CONFIG_WATCHDOG_AUTOMONITOR_TIMEOUT)
...
#define WATCHDOG_AUTOMONITOR_PING_INTERVAL (CONFIG_WATCHDOG_AUTOMONITOR_TIMEOUT / 2)
#else
#define WATCHDOG_AUTOMONITOR_PING_INTERVAL CONFIG_WATCHDOG_AUTOMONITOR_PING_INTERVAL
#endif
#define WATCHDOG_AUTOMONITOR_PING_INTERVAL_MSEC (WATCHDOG_AUTOMONITOR_PING_INTERVAL)
#define WATCHDOG_AUTOMONITOR_PING_INTERVAL_TICK MSEC2TICK(WATCHDOG_AUTOMONITOR_PING_INTERVAL)
#endif
#endif
...
static void watchdog_automonitor_oneshot(FAR struct oneshot_lowerhalf_s *oneshot, FAR void *arg)
{
	FAR struct watchdog_upperhalf_s *upper = arg;
	FAR struct watchdog_lowerhalf_s *lower = upper->lower;

	if (upper->monitor) {
		struct timespec ts = {WATCHDOG_AUTOMONITOR_PING_INTERVAL, 0};
...
#elif defined(CONFIG_WATCHDOG_AUTOMONITOR_BY_TIMER)
static bool watchdog_automonitor_timer(FAR uint32_t *next_interval_us, FAR void *arg)
{
	FAR struct watchdog_upperhalf_s *upper = arg;
	FAR struct watchdog_lowerhalf_s *lower = upper->lower;
...
static void watchdog_automonitor_wdog(wdparm_t arg)
{
	...
	if (upper->monitor) {
		lower->ops->keepalive(lower);
		wd_start(&upper->wdog, WATCHDOG_AUTOMONITOR_PING_INTERVAL_TICK, watchdog_automonitor_wdog,
				 (wdparm_t)upper);
	}
}
#elif defined(CONFIG_WATCHDOG_AUTOMONITOR_BY_WORKER)
static void watchdog_automonitor_worker(FAR void *arg)
{
	...
	if (upper->monitor) {
		lower->ops->keepalive(lower);
		work_queue(LPWORK, &upper->work, watchdog_automonitor_worker, upper,
				   WATCHDOG_AUTOMONITOR_PING_INTERVAL_TICK);
	}
}
#elif defined(CONFIG_WATCHDOG_AUTOMONITOR_BY_IDLE)
static void watchdog_automonitor_idle(FAR struct pm_callback_s *cb, int domain,
									  enum pm_state_e pmstate)
{
	...
	if (domain == PM_IDLE_DOMAIN && pmstate != PM_RESTORE && upper->monitor) {
		lower->ops->keepalive(lower);
	}
}
#endif
...
#ifdef CONFIG_WATCHDOG_PANIC_NOTIFIER
static int wdog_notifier(FAR struct notifier_block *nb, unsigned long action, FAR void *data)
{
	FAR struct watchdog_upperhalf_s *upper = (FAR struct watchdog_upperhalf_s *)nb;
 ...
 *
 * Input Parameters:
 *   dev path - The full path to the driver to be registers in the ROUX
...
#endif
{
	...
	upper = (FAR struct watchdog_upperhalf_s *)kmm_zalloc(sizeof(struct watchdog_upperhalf_s));
	if (!upper) {
