...
static ssize_t lwlconsole_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t lwlconsole_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static struct lwl_entry_s g_d = {LWL_ID_SIG};
...
static bool writeword(uint32_t newupword)
{
	...
	g_d.upword =
		LWL_DNSENSE(g_d.upword) | newupword | (LWL_UPSENSE(g_d.upword) ? 0 : LWL_UPSENSEBIT);
...
static bool write8bits(uint8_t port, uint8_t val)
{
	...
	uint32_t newupword = LWL_ACTIVE(true) | LWL_OCTVAL(1) | LWL_PORT(port) | (val & 0xff);
...
static bool write16bits(uint8_t port, uint32_t val)
{
	...
	uint32_t newupword = LWL_ACTIVE(true) | LWL_OCTVAL(2) | LWL_PORT(port) | (val & 0xffff);
...
static bool write24bits(uint8_t port, uint32_t val)
{
	...
	uint32_t newupword = LWL_ACTIVE(true) | LWL_OCTVAL(3) | LWL_PORT(port) | (val & 0xffffff);
...
static ssize_t lwlconsole_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	while (buflen) {
		...
		default:
			if (write24bits(LWL_PORT_CONSOLE, buffer[0] | (buffer[1] << 8) | (buffer[2] << 16))) {
				oc += 3;
				buffer += 3;
				buflen -= 3;
			}
