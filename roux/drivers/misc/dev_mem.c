...
static ssize_t devmem_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t devmem_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int devmem_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map);
...
{
	return register_driver("/dev/mem", &g_devmem_fops, 0666, (FAR void *)g_memory_region);
}
