...
static ssize_t rd_read(FAR struct inode *inode, FAR unsigned char *buffer, blkcnt_t start_sector,
					   unsigned int nsectors);
static ssize_t rd_write(FAR struct inode *inode, FAR const unsigned char *buffer,
...
static ssize_t rd_read(FAR struct inode *inode, unsigned char *buffer, blkcnt_t start_sector,
					   unsigned int nsectors)
{
	FAR struct rd_struct_s *dev;
	...
	if (start_sector < dev->rd_nsectors && start_sector + nsectors <= dev->rd_nsectors) {
		...
		memcpy(buffer, &dev->rd_buffer[start_sector * dev->rd_sectsize],
			   nsectors * dev->rd_sectsize);
		return nsectors;
	}
						...
						blkcnt_t start_sector, unsigned int nsectors)
{
	...
	if (!RDFLAG_IS_WRENABLED(dev->rd_flags)) {
		...
		memcpy(&dev->rd_buffer[start_sector * dev->rd_sectsize], buffer,
			   nsectors * dev->rd_sectsize);
		return nsectors;
	}
...
int ramdisk_register(int minor, FAR uint8_t *buffer, uint32_t nsectors, uint16_t sectsize,
					 uint8_t rdflags)
{
	FAR struct rd_struct_s *dev;
	char devname[16];
