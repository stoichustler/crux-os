...
typedef void (*optee_smc_fn)(unsigned long, unsigned long, unsigned long, unsigned long,
...
static inline void reg_pair_from_64(uint64_t val, uint64_t *reg0, uint64_t *reg1)
{
	*reg0 = val >> 32;
	*reg1 = val & UINT32_MAX;
}
...
static bool optee_smc_is_compatible(optee_smc_fn smc_fn)
{
	...
	if (osrev.result.build_id) {
		syslog(LOG_INFO, "OP-TEE: OS revision %lu.%lu (%08lx)\n", osrev.result.major,
			   osrev.result.minor, osrev.result.build_id);
	} else {
	...
	smc_fn(OPTEE_SMC_EXCHANGE_CAPABILITIES, OPTEE_SMC_NSEC_CAP_UNIPROCESSOR, 0, 0, 0, 0, 0, 0,
		   &xchgcaps.smccc);
...
static void optee_smc_handle_rpc(FAR struct optee_priv_data *priv_, FAR smccc_res_t *par)
{
...
int optee_transport_call(FAR struct optee_priv_data *priv_, FAR struct optee_msg_arg *arg)
{
	FAR struct optee_smc_priv_data *priv = (FAR struct optee_smc_priv_data *)priv_;
	smccc_res_t res;
	smccc_res_t par;
	uintptr_t arg_pa;
	...
	for (;;) {
		...
		priv->smc_fn(par.a0, par.a1, par.a2, par.a3, par.a4, par.a5, par.a6, par.a7, &res);
