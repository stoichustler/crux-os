...
static ssize_t rpmsgblk_read(FAR struct inode *inode, FAR unsigned char *buffer,
							 blkcnt_t start_sector, unsigned int nsectors);
static ssize_t rpmsgblk_write(FAR struct inode *inode, FAR const unsigned char *buffer,
							  blkcnt_t start_sector, unsigned int nsectors);
static int rpmsgblk_geometry(FAR struct inode *inode, FAR struct geometry *geometry);
static ssize_t rpmsgblk_ioctl_arglen(int cmd);
static int rpmsgblk_ioctl(FAR struct inode *inode, int cmd, unsigned long arg);
...
static int rpmsgblk_send_recv(FAR struct rpmsgblk_s *priv, uint32_t command, bool copy,
							  FAR struct rpmsgblk_header_s *msg, int len, FAR void *data);
static FAR void *rpmsgblk_get_tx_payload_buffer(FAR struct rpmsgblk_s *priv, FAR uint32_t *len);

/* Functions handle the responses from the remote cpu */

static int rpmsgblk_default_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									uint32_t src, FAR void *priv);
static int rpmsgblk_read_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
static int rpmsgblk_geometry_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									 uint32_t src, FAR void *priv);
static int rpmsgblk_ioctl_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);

/* Functions for creating communication with remote cpu */

static void rpmsgblk_device_created(struct rpmsg_device *rdev, FAR void *priv_);
static void rpmsgblk_device_destroy(struct rpmsg_device *rdev, FAR void *priv_);
static int rpmsgblk_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len, uint32_t src,
						   FAR void *priv);
...
static const rpmsg_ept_cb g_rpmsgblk_handler[] = {
	[RPMSGBLK_OPEN] = rpmsgblk_default_handler,		 [RPMSGBLK_CLOSE] = rpmsgblk_default_handler,
	[RPMSGBLK_READ] = rpmsgblk_read_handler,		 [RPMSGBLK_WRITE] = rpmsgblk_default_handler,
	[RPMSGBLK_GEOMETRY] = rpmsgblk_geometry_handler, [RPMSGBLK_IOCTL] = rpmsgblk_ioctl_handler,
};
...
static int rpmsgblk_open(FAR struct inode *inode)
{
	...
	if (priv->refs++ == 0) {
		...
		ret = rpmsgblk_send_recv(priv, RPMSGBLK_OPEN, true, &msg.header, sizeof(msg), NULL);
		if (ret < 0) {
...
static int rpmsgblk_close(FAR struct inode *inode)
{
	...
	if (--priv->refs == 0) {
		ret = rpmsgblk_send_recv(priv, RPMSGBLK_CLOSE, true, &msg.header, sizeof(msg), NULL);
		if (ret < 0) {
							 ...
							 blkcnt_t start_sector, unsigned int nsectors)
{
	...
	ret = rpmsgblk_send_recv(priv, RPMSGBLK_READ, true, &msg.header, sizeof(msg) - 1, &iov);
							  ...
							  blkcnt_t start_sector, unsigned int nsectors)
{
	...
	while (written < nsectors) {
		...
		ret = rpmsg_send_nocopy(&priv->ept, msg, sizeof(*msg) - 1 + msg->nsectors * sectorsize);
		if (ret < 0) {
...
static int rpmsgblk_geometry(FAR struct inode *inode, FAR struct geometry *geometry)
{
	...
	ret = rpmsgblk_send_recv(priv, RPMSGBLK_GEOMETRY, true, &msg.header, sizeof(msg), NULL);
	if (ret >= 0) {
...
static int rpmsgblk_mmc_cmd_ioctl(FAR struct inode *inode, unsigned long arg)
{
	...
	return rpmsgblk_send_recv(priv, RPMSGBLK_IOCTL, false, &msg->header, msglen, ioc);
}
...
static int rpmsgblk_mmc_multi_cmd_ioctl(FAR struct inode *inode, unsigned long arg)
{
	FAR struct rpmsgblk_s *priv = inode->i_private;
	FAR struct mmc_ioc_multi_cmd *mioc = (FAR struct mmc_ioc_multi_cmd *)(uintptr_t)arg;
	FAR struct rpmsgblk_ioctl_s *msg;
	size_t arglen;
	size_t msglen;
	size_t rsplen;
	...
	arglen = sizeof(struct mmc_ioc_multi_cmd) + mioc->num_of_cmds * sizeof(struct mmc_ioc_cmd);
	rsplen = arglen;
	...
	if (MAX(msglen, rsplen) > rpmsg_get_tx_buffer_size(&priv->ept)) {
		...
		for (i = 0; i < mioc->num_of_cmds; i++) {
			ret = rpmsgblk_mmc_cmd_ioctl(inode, (unsigned long)(uintptr_t)&mioc->cmds[i]);
			if (ret < 0) {
				return ret;
			}
		}
		...
		size_t off =
			sizeof(struct mmc_ioc_multi_cmd) + mioc->num_of_cmds * sizeof(struct mmc_ioc_cmd);
		uint32_t space;
		...
		for (i = 0; i < mioc->num_of_cmds; i++) {
			if (mioc->cmds[i].data_ptr && mioc->cmds[i].write_flag) {
				memcpy(msg->buf + off, (FAR void *)(uintptr_t)mioc->cmds[i].data_ptr,
					   mioc->cmds[i].blksz * mioc->cmds[i].blocks);
				off += mioc->cmds[i].blksz * mioc->cmds[i].blocks;
			}
		}

		return rpmsgblk_send_recv(priv, RPMSGBLK_IOCTL, false, &msg->header, msglen,
								  (FAR void *)arg);
	}
}
...
static int rpmsgblk_default_ioctl(FAR struct inode *inode, int cmd, unsigned long arg)
{
	FAR struct rpmsgblk_s *priv = inode->i_private;
	FAR struct rpmsgblk_ioctl_s *msg;
	uint32_t space;
...
{
	FAR struct mmc_ioc_cmd *ioc = (FAR struct mmc_ioc_cmd *)(uintptr_t)cookie->data;
...
static void rpmsgblk_mmc_multi_cmd_handler(FAR struct rpmsgblk_cookie_s *cookie,
										   FAR struct rpmsgblk_ioctl_s *rsp)
{
	FAR struct mmc_ioc_multi_cmd *mioc = (FAR struct mmc_ioc_multi_cmd *)(uintptr_t)cookie->data;
	FAR struct mmc_ioc_multi_cmd *mioc_rsp = (FAR struct mmc_ioc_multi_cmd *)(uintptr_t)rsp->buf;
	size_t off = sizeof(struct mmc_ioc_multi_cmd) + mioc->num_of_cmds * sizeof(struct mmc_ioc_cmd);
	uint64_t i;
	...
	for (i = 0; i < mioc->num_of_cmds; i++) {
		...
		if (mioc->cmds[i].data_ptr && !mioc->cmds[i].write_flag) {
			memcpy((FAR void *)(uintptr_t)mioc->cmds[i].data_ptr, rsp->buf + off,
				   mioc->cmds[i].blksz * mioc->cmds[i].blocks);
			off += mioc->cmds[i].blksz * mioc->cmds[i].blocks;
		}
	}
...
static void rpmsgblk_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	...
	if (strcmp(priv->remotecpu, rpmsg_get_cpuname(rdev)) == 0) {
		...
		snprintf(buf, sizeof(buf), "%s%s", RPMSGBLK_NAME_PREFIX, priv->remotepath);
		rpmsg_create_ept(&priv->ept, rdev, buf, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY, rpmsgblk_ept_cb,
						 NULL);
	}
}
...
static void rpmsgblk_device_destroy(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	FAR struct rpmsgblk_s *priv = priv_;
					  ...
					  FAR const char *localpath)
{
	...
	ret =
		rpmsg_register_callback(dev, rpmsgblk_device_created, rpmsgblk_device_destroy, NULL, NULL);
	if (ret < 0) {
...
fail_with_rpmsg:
	rpmsg_unregister_callback(dev, rpmsgblk_device_created, rpmsgblk_device_destroy, NULL, NULL);
