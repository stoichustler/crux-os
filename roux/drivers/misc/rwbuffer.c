...
static ssize_t rwb_read_(FAR struct rwbuffer_s *rwb, off_t startblock, size_t nblocks,
...
static inline bool rwb_overlap(off_t blockstart1, size_t nblocks1, off_t blockstart2,
							   size_t nblocks2)
{
...
#ifdef CONFIG_DRVR_WRITEBUFFER
static ssize_t rwb_writebuffer(FAR struct rwbuffer_s *rwb, off_t startblock, uint32_t nblocks,
							   FAR const uint8_t *wrbuffer)
{
	...
	while (nblocks > 0) {
		...
		memcpy(rwb->wrbuffer + padblocks * rwb->blocksize, wrbuffer, remain * rwb->blocksize);
...
#ifdef CONFIG_DRVR_READAHEAD
static inline void rwb_bufferread(FAR struct rwbuffer_s *rwb, off_t startblock, size_t nblocks,
								  FAR uint8_t **rdbuffer)
{
...
ssize_t rwb_read(FAR struct rwbuffer_s *rwb, off_t startblock, size_t nblocks,
				 FAR uint8_t *rdbuffer)
{
	int ret = OK;
	size_t readblocks = 0;
	...
	if (rwb->wrmaxblocks > 0) {
		...
		if (rwb_overlap(rwb->wrblockstart, rwb->wrnblocks, startblock, nblocks)) {
			...
			rdblocks = nblocks > (rwb->wrnblocks - wrnpass) ? (rwb->wrnblocks - wrnpass) : nblocks;
			memcpy(rdbuffer, &rwb->wrbuffer[wrnpass * rwb->blocksize], rdblocks * rwb->blocksize);
...
ssize_t rwb_write(FAR struct rwbuffer_s *rwb, off_t startblock, size_t nblocks,
				  FAR const uint8_t *wrbuffer)
{
	int ret = OK;
