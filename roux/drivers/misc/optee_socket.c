...
static int optee_socket_recv(FAR struct socket *psock, FAR void *msg, size_t size)
{
	while (size > 0) {
...
int optee_transport_open(FAR struct optee_priv_data **priv_)
{
	...
	ret = psock_connect(&priv->socket, (FAR const struct sockaddr *)&addr, sizeof(addr));
	if (ret < 0) {
...
{
	FAR struct optee_socket_priv_data *priv = (FAR struct optee_socket_priv_data *)priv_;
...
int optee_transport_call(FAR struct optee_priv_data *priv_, FAR struct optee_msg_arg *arg)
{
	...
	for (i = 0; i < arg->num_params; i++) {
		if (arg->params[i].attr == OPTEE_MSG_ATTR_TYPE_RMEM_INPUT ||
			...
			iov[iovlen].iov_base = (FAR void *)(uintptr_t)arg->params[i].u.rmem.shm_ref;
	...
	for (i = 0; i < arg->num_params; i++) {
		if (arg->params[i].attr == OPTEE_MSG_ATTR_TYPE_RMEM_OUTPUT ||
			...
			ret = optee_socket_recv(&priv->socket,
									(FAR void *)(uintptr_t)arg->params[i].u.rmem.shm_ref, size);
			if (ret < 0) {
				return ret;
			}
		}
