...
static ssize_t rpmsgdev_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t rpmsgdev_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int rpmsgdev_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);

/* Functions for sending data to the remote cpu */

static int rpmsgdev_send_recv(FAR struct rpmsgdev_s *priv, uint32_t command, bool copy,
							  FAR struct rpmsgdev_header_s *msg, int len, FAR void *data);
static FAR void *rpmsgdev_get_tx_payload_buffer(FAR struct rpmsgdev_s *priv, FAR uint32_t *len);

/* Functions handle the responses from the remote cpu */

static int rpmsgdev_default_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									uint32_t src, FAR void *priv);
static int rpmsgdev_read_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
static int rpmsgdev_ioctl_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
static int rpmsgdev_notify_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								   uint32_t src, FAR void *priv);

/* Functions for creating communication with remote cpu */

static void rpmsgdev_device_created(struct rpmsg_device *rdev, FAR void *priv_);
static void rpmsgdev_device_destroy(struct rpmsg_device *rdev, FAR void *priv_);
static int rpmsgdev_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len, uint32_t src,
						   FAR void *priv);
...
static const rpmsg_ept_cb g_rpmsgdev_handler[] = {
	[RPMSGDEV_OPEN] = rpmsgdev_default_handler,	 [RPMSGDEV_CLOSE] = rpmsgdev_default_handler,
	[RPMSGDEV_READ] = rpmsgdev_read_handler,	 [RPMSGDEV_READ_NOFRAG] = rpmsgdev_read_handler,
	[RPMSGDEV_WRITE] = rpmsgdev_default_handler, [RPMSGDEV_LSEEK] = rpmsgdev_default_handler,
	[RPMSGDEV_IOCTL] = rpmsgdev_ioctl_handler,	 [RPMSGDEV_POLL] = rpmsgdev_default_handler,
...
static int rpmsgdev_open(FAR struct file *filep)
{
	...
	ret = rpmsgdev_send_recv(dev, RPMSGDEV_OPEN, true, &msg.header, sizeof(msg), NULL);
	if (ret < 0) {
...
static int rpmsgdev_close(FAR struct file *filep)
{
	...
	ret = rpmsgdev_send_recv(dev, RPMSGDEV_CLOSE, true, &msg.header, sizeof(msg), NULL);
	if (ret < 0) {
...
static ssize_t rpmsgdev_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	msg.filep = priv->filep;
	msg.count = buflen;
	command = dev->flags & RPMSGDEV_NOFRAG_READ ? RPMSGDEV_READ_NOFRAG : RPMSGDEV_READ;

	for (;;) {
		ret = rpmsgdev_send_recv(dev, command, true, &msg.header, sizeof(msg) - 1, &read);
...
static ssize_t rpmsgdev_write(FAR struct file *filep, const char *buffer, size_t buflen)
{
	FAR struct rpmsgdev_s *dev;
	FAR struct rpmsgdev_priv_s *priv;
	...
	while (written < buflen) {
		...
		ret = rpmsgdev_send_recv(dev, RPMSGDEV_WRITE, false, &msg->header, sizeof(*msg) - 1 + space,
								 NULL);
		if (ret >= 0) {
			written += ret;
			continue;
...
static off_t rpmsgdev_seek(FAR struct file *filep, off_t offset, int whence)
{
	...
	ret = rpmsgdev_send_recv(dev, RPMSGDEV_LSEEK, true, &msg.header, sizeof(msg), NULL);
	if (ret >= 0) {
...
static int rpmsgdev_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	ret = rpmsgdev_send_recv(dev, RPMSGDEV_IOCTL, false, &msg->header, msglen,
...
static int rpmsgdev_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup)
{
	...
	return rpmsgdev_send_recv(dev, RPMSGDEV_POLL, true, &msg.header, sizeof(msg), NULL);
}
								 ...
								 uint32_t src, FAR void *priv)
{
	...
	if (header->command == RPMSGDEV_READ_NOFRAG || cookie->result <= 0 ||
		read->iov_len >= rsp->count) {
		rpmsg_post(ept, &cookie->sem);
	}
...
static void rpmsgdev_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	...
	if (strcmp(priv->remotecpu, rpmsg_get_cpuname(rdev)) == 0) {
		...
		snprintf(buf, sizeof(buf), "%s%s", RPMSGDEV_NAME_PREFIX, priv->remotepath);
		rpmsg_create_ept(&priv->ept, rdev, buf, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY, rpmsgdev_ept_cb,
						 NULL);
	}
}
...
static void rpmsgdev_device_destroy(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	FAR struct rpmsgdev_s *priv = priv_;
					  ...
					  FAR const char *localpath, uint32_t flags)
{
	...
	DEBUGASSERT(strlen(remotepath) + RPMSGDEV_NAME_PREFIX_LEN <= RPMSG_NAME_SIZE);
	...
	ret =
		rpmsg_register_callback(dev, rpmsgdev_device_created, rpmsgdev_device_destroy, NULL, NULL);
	if (ret < 0) {
...
fail_with_rpmsg:
	rpmsg_unregister_callback(dev, rpmsgdev_device_created, rpmsgdev_device_destroy, NULL, NULL);
