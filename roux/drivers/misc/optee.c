...
#include <roux/lib/math32.h>
#include <sys/mman.h>
#include <sys/param.h>

#ifdef CONFIG_ARCH_ADDRENV
#include <roux/pgalloc.h>
 ...
 * component optee_client (https://github.com/OP-TEE/optee_client) to ROUX.
...
#define OPTEE_PAGES_ARRAY_LEN ((OPTEE_MSG_NONCONTIG_PAGE_SIZE / sizeof(uint64_t)) - 1)
...
#define optee_msg_alloc(priv, numparams, msg)                                                      \
	do {                                                                                           \
		if ((priv)->alignment > sizeof(uintptr_t)) {                                               \
			(msg) = kmm_memalign((priv)->alignment, OPTEE_MSG_GET_ARG_SIZE(numparams));            \
		} else {                                                                                   \
...
static bool optee_is_valid_range(FAR const void *va, size_t size)
{
	...
	if (up_addrenv_user_vaddr((uintptr_t)va) && up_addrenv_user_vaddr((uintptr_t)va + size - 1)) {
		return true;
	}
 ...
 *   know about the address environment mapping of ROUX, so physical
...
static FAR void *optee_shm_to_page_list(FAR struct optee_shm *shm, FAR uintptr_t *list_pa)
{
	...
	list_size =
		div_round_up(total_pages, OPTEE_PAGES_ARRAY_LEN) * sizeof(struct optee_page_list_entry);
...
static int optee_shm_register(FAR struct optee_priv_data *priv, FAR struct optee_shm *shm)
{
	...
	msg->params[0].attr = OPTEE_MSG_ATTR_TYPE_TMEM_OUTPUT | OPTEE_MSG_ATTR_NONCONTIG;
...
static int optee_shm_unregister(FAR struct optee_priv_data *priv, FAR struct optee_shm *shm)
{
	FAR struct optee_msg_arg *msg;
	int ret = 0;
									 ...
									 FAR const struct tee_ioctl_param *p)
{
	...
	if (p->c == TEE_MEMREF_NULL) {
		mp->attr =
			OPTEE_MSG_ATTR_TYPE_TMEM_INPUT + p->attr - TEE_IOCTL_PARAM_ATTR_TYPE_MEMREF_INPUT;
		mp->u.tmem.buf_ptr = 0;
		mp->u.tmem.shm_ref = 0;
		mp->u.tmem.size = p->b;
		return 0;
	...
	if (shm->flags & TEE_SHM_REGISTER) {
		...
		mp->attr =
			OPTEE_MSG_ATTR_TYPE_RMEM_INPUT + p->attr - TEE_IOCTL_PARAM_ATTR_TYPE_MEMREF_INPUT;
		mp->u.rmem.offs = p->a;
		mp->u.rmem.size = p->b;
...
static int optee_to_msg_param(FAR struct optee_priv_data *priv, FAR struct optee_msg_param *mparams,
							  size_t num_params, FAR const struct tee_ioctl_param *params)
{
	size_t n;
	int ret;
	...
	for (n = 0; n < num_params; n++) {
		...
		case TEE_IOCTL_PARAM_ATTR_TYPE_VALUE_INOUT:
			mp->attr =
				OPTEE_MSG_ATTR_TYPE_VALUE_INPUT + p->attr - TEE_IOCTL_PARAM_ATTR_TYPE_VALUE_INPUT;
			mp->u.value.a = p->a;
			mp->u.value.b = p->b;
			mp->u.value.c = p->c;
			break;
...
static int optee_from_msg_param(FAR struct tee_ioctl_param *params, size_t num_params,
								FAR const struct optee_msg_param *mparams)
{
	...
	for (n = 0; n < num_params; n++) {
		...
		case OPTEE_MSG_ATTR_TYPE_VALUE_INOUT:
			p->attr =
				TEE_IOCTL_PARAM_ATTR_TYPE_VALUE_INPUT + mp->attr - OPTEE_MSG_ATTR_TYPE_VALUE_INPUT;
			p->a = mp->u.value.a;
			p->b = mp->u.value.b;
			p->c = mp->u.value.c;
			break;
		...
		case OPTEE_MSG_ATTR_TYPE_TMEM_INOUT:
			p->attr =
				TEE_IOCTL_PARAM_ATTR_TYPE_MEMREF_INPUT + mp->attr - OPTEE_MSG_ATTR_TYPE_TMEM_INPUT;
			p->b = mp->u.tmem.size;
		...
		case OPTEE_MSG_ATTR_TYPE_RMEM_INOUT:
			p->attr =
				TEE_IOCTL_PARAM_ATTR_TYPE_MEMREF_INPUT + mp->attr - OPTEE_MSG_ATTR_TYPE_RMEM_INPUT;
			p->b = mp->u.rmem.size;
			shm = (FAR struct optee_shm *)(uintptr_t)mp->u.tmem.shm_ref;
			break;
		default:
...
static int optee_close_session(FAR struct optee_priv_data *priv, uint32_t session)
{
	FAR struct optee_msg_arg *msg;
	int ret;
									...
									FAR struct tee_ioctl_buf_data *buf)
{
	...
	msg->params[0].attr = OPTEE_MSG_ATTR_TYPE_VALUE_INPUT | OPTEE_MSG_ATTR_META;
	msg->params[1].attr = OPTEE_MSG_ATTR_TYPE_VALUE_INPUT | OPTEE_MSG_ATTR_META;
	memcpy(&msg->params[0].u.value, arg->uuid, sizeof(arg->uuid));
	msg->params[1].u.value.c = arg->clnt_login;

	ret = optee_to_msg_param(priv, msg->params + 2, arg->num_params, arg->params);
	if (ret < 0) {
		goto errout_with_msg;
	}
...
static int optee_ioctl_invoke(FAR struct optee_priv_data *priv, FAR struct tee_ioctl_buf_data *buf)
{
	...
	if (buf->buf_len > TEE_MAX_ARG_SIZE || buf->buf_len < sizeof(struct tee_ioctl_invoke_arg)) {
		return -EINVAL;
	}
...
static int optee_ioctl_close_session(FAR struct optee_priv_data *priv,
...
static int optee_ioctl_version(FAR struct tee_ioctl_version_data *vers)
{
	...
	vers->gen_caps = TEE_GEN_CAP_GP | TEE_GEN_CAP_MEMREF_NULL | TEE_GEN_CAP_REG_MEM;
	return 0;
}
...
static int optee_ioctl_shm_alloc(FAR struct optee_priv_data *priv,
								 FAR struct tee_ioctl_shm_alloc_data *data)
{
	...
	addr = mmap(NULL, data->size, PROT_READ | PROT_WRITE, MAP_SHARED, memfd, 0);
	if (addr == MAP_FAILED) {
		ret = get_errno();
		goto err;
	}
...
static int optee_ioctl_shm_register(FAR struct optee_priv_data *priv,
									FAR struct tee_ioctl_shm_register_data *rdata)
{
	...
	if (!optee_is_valid_range((FAR void *)(uintptr_t)rdata->addr, rdata->length)) {
		return -EFAULT;
	}
	...
	ret = optee_shm_alloc(priv, (FAR void *)(uintptr_t)rdata->addr, rdata->length, TEE_SHM_REGISTER,
						  &shm);
	if (ret < 0) {
		return ret;
	}
...
int optee_shm_alloc(FAR struct optee_priv_data *priv, FAR void *addr, size_t size, uint32_t flags,
