...
static int rpmsgdev_open_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
static int rpmsgdev_close_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
static int rpmsgdev_read_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
static int rpmsgdev_write_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
static int rpmsgdev_lseek_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
...
static int rpmsgdev_poll_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);

/* Functions for creating communication with client cpu */

static bool rpmsgdev_ns_match(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							  uint32_t dest);
static void rpmsgdev_ns_bind(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							 uint32_t dest);
...
static const rpmsg_ept_cb g_rpmsgdev_handler[] = {
	[RPMSGDEV_OPEN] = rpmsgdev_open_handler,   [RPMSGDEV_CLOSE] = rpmsgdev_close_handler,
	[RPMSGDEV_READ] = rpmsgdev_read_handler,   [RPMSGDEV_READ_NOFRAG] = rpmsgdev_read_handler,
	[RPMSGDEV_WRITE] = rpmsgdev_write_handler, [RPMSGDEV_LSEEK] = rpmsgdev_lseek_handler,
	[RPMSGDEV_IOCTL] = rpmsgdev_ioctl_handler, [RPMSGDEV_POLL] = rpmsgdev_poll_handler,
};
								 ...
								 uint32_t src, FAR void *priv)
{
	...
	msg->header.result = file_open(&dev->file, &ept->name[RPMSGDEV_NAME_PREFIX_LEN], msg->flags, 0);
								  ...
								  uint32_t src, FAR void *priv)
{
	...
	FAR struct rpmsgdev_device_s *dev = (FAR struct rpmsgdev_device_s *)(uintptr_t)msg->filep;
								  ...
								  uint32_t src, FAR void *priv)
{
	...
	msg->header.result =
		file_ioctl(filep, msg->request, msg->arglen > 0 ? (unsigned long)msg->buf : msg->arg);
...
static void rpmsgdev_poll_worker(FAR void *arg)
{
	...
	FAR struct rpmsgdev_device_s *dev = container_of(fds, FAR struct rpmsgdev_device_s, fd);
...
static void rpmsgdev_ept_release(FAR struct rpmsg_endpoint *ept)
{
	...
	list_for_every_entry_safe(&server->head, dev, tmpdev, struct rpmsgdev_device_s, node)
	{
		if (dev->cfd != 0) {
							 ...
							 uint32_t dest)
{
	...
	ret = rpmsg_create_ept(&server->ept, rdev, name, RPMSG_ADDR_ANY, dest, rpmsgdev_ept_cb,
						   rpmsg_destroy_ept);
	if (ret < 0) {
...
static void rpmsgdev_server_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	struct rpmsgdev_export_s *priv = priv_;
	char buf[RPMSG_NAME_SIZE];

	if (strcmp(priv->remotecpu, rpmsg_get_cpuname(rdev)) == 0) {
		snprintf(buf, sizeof(buf), "%s%s", RPMSGDEV_NAME_PREFIX, priv->localpath);
		rpmsgdev_ns_bind(rdev, priv, buf, RPMSG_ADDR_ANY);

		rpmsg_unregister_callback(priv, rpmsgdev_server_created, NULL, NULL, NULL);
	}
}
...
int rpmsgdev_export(FAR const char *remotecpu, FAR const char *localpath)
{
	...
	return rpmsg_register_callback(priv, rpmsgdev_server_created, NULL, NULL, NULL);
}
...
{
	return rpmsg_register_callback(NULL, NULL, NULL, rpmsgdev_ns_match, rpmsgdev_ns_bind);
}
