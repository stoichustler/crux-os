...
#define GOLDFISH_PIPE_WAKE_UNLOCK_DMA_SHARED                                                       \
	(1 << 4) /* Unlock DMA buffer of the pipe shared to this pipe */
...
struct goldfish_pipe_dev_s {
	...
	struct goldfish_pipe_signalled_s pipes_signalled[GOLDFISH_PIPE_MAX_SIGNALLED];
	FAR uint8_t *base;
};
...
static ssize_t goldfish_pipe_read(FAR struct file *filp, FAR char *buffer, size_t bufflen);
static ssize_t goldfish_pipe_write(FAR struct file *filp, FAR const char *buffer, size_t bufflen);
static int goldfish_pipe_poll(FAR struct file *filp, FAR struct pollfd *fds, bool setup);
...
static int goldfish_pipe_command_locked(FAR struct goldfish_pipe_s *pipe, int cmd)
{
...
static int goldfish_pipe_wait(FAR struct goldfish_pipe_s *pipe, bool is_write)
{
	int ret;
...
static ssize_t goldfish_pipe_transfer_one(FAR struct goldfish_pipe_s *pipe, FAR char *buffer,
										  size_t buflen, bool is_write)
{
	ssize_t ret;
...
static ssize_t goldfish_pipe_transfer(FAR struct file *filp, FAR char *buffer, size_t bufflen,
									  bool is_write)
{
...
static void goldfish_pipe_wake(FAR struct goldfish_pipe_s *pipe, bool is_write)
{
...
static int goldfish_pipe_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	count = goldfish_pipe_getreg32(dev->base + GOLDFISH_PIPE_REG_GET_SIGNALLED);
...
static void goldfish_pipe_write_addr(FAR void *addr, FAR void *portl, FAR void *porth)
{
...
int goldfish_pipe_register(FAR void *base, int irq)
{
	...
	goldfish_pipe_putreg32(GOLDFISH_PIPE_DRIVER_VERSION, dev->base + GOLDFISH_PIPE_REG_VERSION);
	...
	goldfish_pipe_write_addr(&dev->pipes_signalled, dev->base + GOLDFISH_PIPE_REG_SIGNAL_BUFFER,
	...
	goldfish_pipe_write_addr(&dev->open_params, dev->base + GOLDFISH_PIPE_REG_OPEN_BUFFER,
							 dev->base + GOLDFISH_PIPE_REG_OPEN_BUFFER_HIGH);

	/* Register the pipe device */

	return register_driver("/dev/goldfish_pipe", &g_goldfish_pipe_fops, 0666, dev);
