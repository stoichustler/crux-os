...
static ssize_t devascii_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t devascii_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int devascii_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static ssize_t devascii_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	size_t i;
	for (i = 0; i < len; i++) {
...
static ssize_t devascii_write(FAR struct file *filep, FAR const char *buffer, size_t len)
{
	UNUSED(filep);
	UNUSED(buffer);
