...
static int rpmsgblk_open_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
static int rpmsgblk_close_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
static int rpmsgblk_read_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
static int rpmsgblk_write_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
...
static bool rpmsgblk_ns_match(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							  uint32_t dest);
static void rpmsgblk_ns_bind(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							 uint32_t dest);
...
static const rpmsg_ept_cb g_rpmsgblk_handler[] = {
	[RPMSGBLK_OPEN] = rpmsgblk_open_handler,		 [RPMSGBLK_CLOSE] = rpmsgblk_close_handler,
	[RPMSGBLK_READ] = rpmsgblk_read_handler,		 [RPMSGBLK_WRITE] = rpmsgblk_write_handler,
								 ...
								 uint32_t src, FAR void *priv)
{
	...
	while (read < msg->nsectors) {
		...
		*rsp = *msg;
		...
		ret = server->bops->read(server->blknode, (FAR unsigned char *)rsp->buf, msg->startsector,
								 nsectors);
		rsp->header.result = ret;
		if (rpmsg_send_nocopy(ept, rsp, (ret < 0 ? 0 : ret * msg->sectorsize) + sizeof(*rsp) - 1) <
			0) {
								  ...
								  uint32_t src, FAR void *priv)
{
	...
	ret = server->bops->write(server->blknode, (FAR unsigned char *)msg->buf, msg->startsector,
							  msg->nsectors);
	if (ret <= 0) {
...
{
	FAR struct rpmsgblk_server_s *server = ept->priv;
	FAR struct mmc_ioc_cmd *ioc = (FAR struct mmc_ioc_cmd *)(uintptr_t)msg->buf;
	...
	rsp->header.result =
		server->bops->ioctl(server->blknode, rsp->request, (unsigned long)rsp->buf);
...
{
	FAR struct rpmsgblk_server_s *server = ept->priv;
	FAR struct mmc_ioc_multi_cmd *mioc = (FAR struct mmc_ioc_multi_cmd *)(uintptr_t)msg->buf;
	FAR struct rpmsgblk_ioctl_s *rsp;
	...
	off = sizeof(struct mmc_ioc_multi_cmd) + mioc->num_of_cmds * sizeof(struct mmc_ioc_cmd);
	...
	for (i = 0; i < mioc_rsp->num_of_cmds; i++) {
		if (mioc_rsp->cmds[i].write_flag) {
			mioc_rsp->cmds[i].data_ptr = (uint64_t)(uintptr_t)(msg->buf + off);
			off += mioc_rsp->cmds[i].blksz * mioc_rsp->cmds[i].blocks;
		} else {
			mioc_rsp->cmds[i].data_ptr = (uint64_t)(uintptr_t)(rsp->buf + rsp_off);
								  ...
								  uint32_t src, FAR void *priv)
{
	...
	msg->header.result = server->bops->ioctl(server->blknode, msg->request,
											 msg->arglen > 0 ? (unsigned long)msg->buf : msg->arg);
							 ...
							 uint32_t dest)
{
	...
	ret = find_blockdriver(&name[RPMSGBLK_NAME_PREFIX_LEN], 0, &server->blknode);
	if (ret < 0) {
	...
	ret = rpmsg_create_ept(&server->ept, rdev, name, RPMSG_ADDR_ANY, dest, rpmsgblk_ept_cb,
						   rpmsg_destroy_ept);
	if (ret < 0) {
...
{
	return rpmsg_register_callback(NULL, NULL, NULL, rpmsgblk_ns_match, rpmsgblk_ns_bind);
}
