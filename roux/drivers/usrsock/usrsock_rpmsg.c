...
static int usrsock_rpmsg_send_dns_request(FAR void *arg, FAR struct sockaddr *addr,
										  socklen_t addrlen);
#endif
...
static void usrsock_rpmsg_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_);
...
static struct usrsock_rpmsg_s g_usrsock_rpmsg = {SEM_INITIALIZER(0)};
...
static void usrsock_rpmsg_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	...
	if (!strcmp(CONFIG_NET_USRSOCK_RPMSG_CPUNAME, rpmsg_get_cpuname(rdev))) {
		...
		rpmsg_create_ept(&priv->ept, rdev, USRSOCK_RPMSG_EPT_NAME, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
...
{
	rpmsg_register_callback(&g_usrsock_rpmsg, usrsock_rpmsg_device_created,
