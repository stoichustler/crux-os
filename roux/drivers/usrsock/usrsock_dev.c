...
static ssize_t usrsockdev_read(FAR struct file *filep, FAR char *buffer, size_t len);
static ssize_t usrsockdev_write(FAR struct file *filep, FAR const char *buffer, size_t len);
...
static int usrsockdev_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static struct usrsockdev_s g_usrsockdev = {NXMUTEX_INITIALIZER};
...
static ssize_t usrsockdev_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	...
	if (dev->req.iov) {
		...
		rlen = usrsock_iovec_get(buffer, len, dev->req.iov, dev->req.iovcnt, dev->req.pos, NULL);
		if (rlen < 0) {
...
static off_t usrsockdev_seek(FAR struct file *filep, off_t offset, int whence)
{
	...
	if (dev->req.iov) {
		...
		rlen = usrsock_iovec_get(NULL, 0, dev->req.iov, dev->req.iovcnt, pos, NULL);
		if (rlen < 0) {
...
static int usrsockdev_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup)
{
	...
	if (setup) {
		...
		if (dev->req.iov != NULL &&
			!(usrsock_iovec_get(NULL, 0, dev->req.iov, dev->req.iovcnt, dev->req.pos, NULL) < 0)) {
