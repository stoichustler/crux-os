...
#include <roux/net/net.h>
#include <roux/queue.h>
#include <roux/rpmsg/rpmsg.h>
...
static int usrsock_rpmsg_send_ack(FAR struct rpmsg_endpoint *ept, uint16_t events, uint32_t xid,
								  int32_t result);
static int usrsock_rpmsg_send_data_ack(FAR struct rpmsg_endpoint *ept,
									   FAR struct usrsock_message_datareq_ack_s *ack,
									   uint16_t events, uint32_t xid, int32_t result,
									   uint16_t valuelen, uint16_t valuelen_nontrunc,
									   int32_t datalen);
static int usrsock_rpmsg_send_event(FAR struct rpmsg_endpoint *ept, int16_t usockid,
									uint16_t events);

static int usrsock_rpmsg_socket_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
										uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_close_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									   uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_connect_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
										 uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_sendto_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
										uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_recvfrom_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
										  size_t len, uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_setsockopt_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
											size_t len, uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_getsockopt_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
											size_t len, uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_getsockname_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
											 size_t len, uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_getpeername_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
											 size_t len, uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_bind_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									  uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_listen_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
										uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_accept_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
										uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_ioctl_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									   uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_shutdown_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
										  size_t len, uint32_t src, FAR void *priv_);
static int usrsock_rpmsg_dns_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									 uint32_t src, FAR void *priv_);

static bool usrsock_rpmsg_ns_match(FAR struct rpmsg_device *rdev, FAR void *priv_,
								   FAR const char *name, uint32_t dest);
static void usrsock_rpmsg_ns_bind(FAR struct rpmsg_device *rdev, FAR void *priv_,
								  FAR const char *name, uint32_t dest);
static int usrsock_rpmsg_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								uint32_t src, FAR void *priv);

static void usrsock_rpmsg_poll_cb(FAR struct pollfd *pfds);
static void usrsock_rpmsg_poll_setup(FAR struct pollfd *pfds, pollevent_t events);
...
static const rpmsg_ept_cb g_usrsock_rpmsg_handler[] = {
	usrsock_rpmsg_socket_handler,	   usrsock_rpmsg_close_handler,
	usrsock_rpmsg_connect_handler,	   usrsock_rpmsg_sendto_handler,
	usrsock_rpmsg_recvfrom_handler,	   usrsock_rpmsg_setsockopt_handler,
	usrsock_rpmsg_getsockopt_handler,  usrsock_rpmsg_getsockname_handler,
	usrsock_rpmsg_getpeername_handler, usrsock_rpmsg_bind_handler,
	usrsock_rpmsg_listen_handler,	   usrsock_rpmsg_accept_handler,
	usrsock_rpmsg_ioctl_handler,	   usrsock_rpmsg_shutdown_handler,
...
static int usrsock_rpmsg_send_frag_ack(FAR struct rpmsg_endpoint *ept,
									   FAR struct usrsock_message_frag_ack_s *ack, uint16_t events,
									   uint32_t xid, int32_t result, uint32_t datalen)
{
										...
										uint32_t src, FAR void *priv_)
{
	...
	for (i = 0; i < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS; i++) {
		if (priv->epts[i] == NULL) {
			...
			ret = psock_socket(req->domain, req->type | SOCK_NONBLOCK, req->protocol,
							   &priv->socks[i]);
			if (ret >= 0) {
	...
	if (retr >= 0 && ret >= 0 && req->type != SOCK_STREAM && req->type != SOCK_SEQPACKET) {
									   ...
									   uint32_t src, FAR void *priv_)
{
	...
	if (req->usockid >= 0 && req->usockid < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS) {
										 ...
										 uint32_t src, FAR void *priv_)
{
	...
	if (req->usockid >= 0 && req->usockid < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS) {
		ret = psock_connect(&priv->socks[req->usockid], (FAR const struct sockaddr *)(req + 1),
							req->addrlen);
	}
	...
	if (retr >= 0 && ret >= 0) {
		...
		if (!inprogress) {
			retr = usrsock_rpmsg_send_event(ept, req->usockid, USRSOCK_EVENT_SENDTO_READY);
		}
	}
										...
										uint32_t src, FAR void *priv_)
{
	...
	FAR struct usrsock_rpmsg_ept_s *uept = (FAR struct usrsock_rpmsg_ept_s *)ept;
	FAR struct usrsock_rpmsg_s *priv = priv_;
	uint16_t events = 0;
	ssize_t ret = -EBADF;
	int retr;
	...
	if (uept->remain > 0) {
		...
		if (req->usockid >= 0 && req->usockid < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS) {
			...
			if (uept->remain > 0) {
				...
				ret = psock_sendto(
					&priv->socks[req->usockid], (FAR const void *)(req + 1) + req->addrlen,
					req->buflen, req->flags,
					req->addrlen ? (FAR const struct sockaddr *)(req + 1) : NULL, req->addrlen);
			}
		}
	}

out:
	if (ret > 0 && usrsock_rpmsg_available(&priv->socks[req->usockid], FIONSPACE)) {
		events |= USRSOCK_EVENT_SENDTO_READY;
	}
...
static int usrsock_rpmsg_recvfrom_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
										  size_t len_, uint32_t src, FAR void *priv_)
{
	struct iovec iov[CONFIG_NET_USRSOCK_RPMSG_SERVER_NIOVEC];
	...
	if (req->usockid >= 0 && req->usockid < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS) {
		ret = psock_recvfrom(&priv->socks[req->usockid], (FAR void *)(ack + 1) + inaddrlen,
							 len - sizeof(*ack) - inaddrlen, req->flags,
							 outaddrlen ? (FAR struct sockaddr *)(ack + 1) : NULL,
							 outaddrlen ? &outaddrlen : NULL);
		totlen = ret;
		if (ret > 0 && (priv->socks[req->usockid].s_type & SOCK_TYPE_MASK) == SOCK_STREAM) {
			if (outaddrlen < inaddrlen) {
				memcpy((FAR void *)(ack + 1) + outaddrlen, (FAR void *)(ack + 1) + inaddrlen, ret);
			}
			...
			net_lock();
			while (totlen < buflen && i < CONFIG_NET_USRSOCK_RPMSG_SERVER_NIOVEC) {
				...
				iov[i].iov_base = rpmsg_get_tx_payload_buffer(ept, &len, false);
				if (!iov[i].iov_base) {
					break;
				}
				...
				iov[i].iov_len =
					psock_recvfrom(&priv->socks[req->usockid], (FAR char *)iov[i].iov_base + offset,
								   len - offset, req->flags | MSG_DONTWAIT, NULL, NULL);
	...
	retr = usrsock_rpmsg_send_data_ack(ept, ack, events, req->head.xid, totlen, inaddrlen,
									   outaddrlen, ret);
	for (i = 0; i < CONFIG_NET_USRSOCK_RPMSG_SERVER_NIOVEC; i++) {
		if (!iov[i].iov_base) {
			break;
		...
		usrsock_rpmsg_send_frag_ack(ept, (FAR struct usrsock_message_frag_ack_s *)iov[i].iov_base,
											...
											size_t len, uint32_t src, FAR void *priv_)
{
	...
	if (req->usockid >= 0 && req->usockid < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS) {
		ret = psock_setsockopt(&priv->socks[req->usockid], req->level, req->option, req + 1,
							   req->valuelen);
	}
											...
											size_t len_, uint32_t src, FAR void *priv_)
{
	...
	if (req->usockid >= 0 && req->usockid < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS) {
		ret =
			psock_getsockopt(&priv->socks[req->usockid], req->level, req->option, ack + 1, &optlen);
	}

	return usrsock_rpmsg_send_data_ack(ept, ack, 0, req->head.xid, ret, optlen, optlen, ret);
}
											 ...
											 size_t len_, uint32_t src, FAR void *priv_)
{
	...
	ack = rpmsg_get_tx_payload_buffer(ept, &len, true);
	if (req->usockid >= 0 && req->usockid < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS) {
		ret = psock_getsockname(&priv->socks[req->usockid], (FAR struct sockaddr *)(ack + 1),
								&outaddrlen);
	}

	return usrsock_rpmsg_send_data_ack(ept, ack, 0, req->head.xid, ret, inaddrlen, outaddrlen, ret);
}
											 ...
											 size_t len_, uint32_t src, FAR void *priv_)
{
	...
	ack = rpmsg_get_tx_payload_buffer(ept, &len, true);
	if (req->usockid >= 0 && req->usockid < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS) {
		ret = psock_getpeername(&priv->socks[req->usockid], (FAR struct sockaddr *)(ack + 1),
								&outaddrlen);
	}
									  ...
									  uint32_t src, FAR void *priv_)
{
	...
	if (req->usockid >= 0 && req->usockid < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS) {
		ret = psock_bind(&priv->socks[req->usockid], (FAR const struct sockaddr *)(req + 1),
						 req->addrlen);
	}
...
static int usrsock_rpmsg_accept_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len_,
										uint32_t src, FAR void *priv_)
{
		...
		for (i = 0; i < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS; i++) {
			if (priv->epts[i] == NULL) {
				...
				ret = psock_accept(&priv->socks[req->usockid],
								   outaddrlen ? (FAR struct sockaddr *)(ack + 1) : NULL,
								   outaddrlen ? &outaddrlen : NULL, &priv->socks[i], SOCK_NONBLOCK);
				if (ret >= 0) {
					/* Append index as usockid to the payload */

					if (outaddrlen <= inaddrlen) {
						*(FAR int16_t *)((FAR void *)(ack + 1) + outaddrlen) = i;
					} else {
						*(FAR int16_t *)((FAR void *)(ack + 1) + inaddrlen) = i;
					}
	...
	retr = usrsock_rpmsg_send_data_ack(ept, ack, 0, req->head.xid, ret, inaddrlen, outaddrlen, ret);
	if (retr >= 0 && ret >= 0) {
		usrsock_rpmsg_poll_setup(&priv->pfds[req->usockid], POLLIN);
...
static int usrsock_rpmsg_ioctl_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len_,
									   uint32_t src, FAR void *priv_)
{
	...
	if (req->usockid >= 0 && req->usockid < CONFIG_NET_USRSOCK_RPMSG_SERVER_NSOCKS) {
		...
		ret = psock_ioctl(&priv->socks[req->usockid], req->cmd, (unsigned long)(ack + 1));
	...
	return usrsock_rpmsg_send_data_ack(ept, ack, 0, req->head.xid, ret, req->arglen, req->arglen,
									   ret);
}
...
#ifdef CONFIG_NETDB_DNSCLIENT
static int usrsock_rpmsg_send_dns_event(FAR void *arg, FAR struct sockaddr *addr, socklen_t addrlen)
{
								  ...
								  FAR const char *name, uint32_t dest)
{
	...
	ret = rpmsg_create_ept(&uept->ept, rdev, USRSOCK_RPMSG_EPT_NAME, RPMSG_ADDR_ANY, dest,
...
static int usrsock_rpmsg_ept_do_cb(FAR struct usrsock_rpmsg_ept_s *uept, FAR void *data, size_t len,
								   uint32_t src, FAR struct usrsock_rpmsg_s *priv)
{
	...
	if (uept->remain > 0) {
		...
		return g_usrsock_rpmsg_handler[common->reqid](&uept->ept, data, len, src, priv);
	}
								...
								uint32_t src, FAR void *priv_)
{
	...
	while ((req = (FAR struct usrsock_rpmsg_req_s *)sq_remfirst(&uept->req_pending)) != NULL) {
		data = req->data;
		len = req->len;
		src = req->src;
...
static void usrsock_rpmsg_poll_setup(FAR struct pollfd *pfds, pollevent_t events)
{
	...
	if (ret < 0) {
		nerr("psock_poll failed. ret %d domain %u type %u pfds->fd %d"
			 ...
			 ret, psock->s_domain, psock->s_type, pfds->fd, pfds->events, pfds->revents);
	}
...
int usrsock_rpmsg_server_initialize(void)
{
	...
	ret = rpmsg_register_callback(priv, NULL, NULL, usrsock_rpmsg_ns_match, usrsock_rpmsg_ns_bind);
	if (ret >= 0) {
		return ret;
	}
