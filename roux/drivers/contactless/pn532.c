...
bool pn532_rx_frame_is_valid(FAR struct pn532_frame *f, bool check_data)
{
	...
	if (check_data) {
		...
		if (chk != f->data[f->len - 1]) {
			ctlserr("ERROR: Frame data checksum failed: calc=0x%X != 0x%X", chk,
					f->data[f->len - 1]);
			return false;
		}
	}
...
int pn532_read_frame(FAR struct pn532_dev_s *dev, FAR struct pn532_frame *f, int max_size)
{
	int res = -EIO;
...
int pn532_sam_config(FAR struct pn532_dev_s *dev, FAR struct pn_sam_settings_s *settings)
{
	char cmd_buffer[4 + 7];
	FAR struct pn532_frame *f = (FAR struct pn532_frame *)cmd_buffer;
	int res;
...
int pn532_get_fw_version(FAR struct pn532_dev_s *dev, FAR struct pn_firmware_version *fv)
{
...
uint32_t pn532_write_passive_data(FAR struct pn532_dev_s *dev, uint8_t address, FAR uint8_t *data,
								  uint8_t len)
{
...
uint32_t pn532_read_passive_data(FAR struct pn532_dev_s *dev, uint8_t address, FAR uint8_t *data,
								 uint8_t len)
{
...
uint32_t pn532_read_passive_target_id(FAR struct pn532_dev_s *dev, uint8_t baudrate)
{
	uint8_t cmd_buffer[4 + 7];
	FAR struct pn532_frame *f = (FAR struct pn532_frame *)cmd_buffer;
	uint8_t resp[20];
	...
	if (dev->state == PN532_STATE_DATA_READY) {
		...
		if (pn532_read_frame(dev, (FAR struct pn532_frame *)resp, 15) == OK) {
			...
			if (f->data[0] == PN532_COMMAND_INLISTPASSIVETARGET + 1) {
				...
				if (r->nbtg == 1) {
					FAR struct pn_target_type_a *t = (FAR struct pn_target_type_a *)&r->target_data;
...
static int pn532_read_passive_target(FAR struct pn532_dev_s *dev, uint8_t baudrate)
{
	uint8_t cmd_buffer[4 + 7];
	FAR struct pn532_frame *f = (FAR struct pn532_frame *)cmd_buffer;
...
static int _ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case PN532IOC_READ_TAG_DATA: {
		FAR struct pn_mifare_tag_data_s *tag_data = (FAR struct pn_mifare_tag_data_s *)arg;
		...
		if (tag_data != 0) {
			...
			ret = pn532_read_passive_data(dev, tag_data->address, (FAR uint8_t *)&tag_data->data,
	...
	case PN532IOC_WRITE_TAG_DATA: {
		...
		if (tag_data != 0) {
			...
			ret = pn532_write_passive_data(dev, tag_data->address, (FAR uint8_t *)&tag_data->data,
										   sizeof(tag_data->data));
