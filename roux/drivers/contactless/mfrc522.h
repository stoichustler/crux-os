...
#define MFRC522_CW_GSP_REG                                                                         \
	(0x28 << 1) /* defines the conductance of p-driver during no modulation */
#define MFRC522_MOD_GSP_REG                                                                        \
	(0x29 << 1)						  /* defines the conductance of p-driver during modulation     \
									   */
...
#define MFRC522_IRQ_PUSH_PULL                                                                      \
	(1 << 7) /* 1 = IRQ pin is a standard CMOS output pin, 0 = open-drain */
...
#define MFRC522_MFIN_ACT_IRQ                                                                       \
	(1 << 4)				  /* MFIN is active, int is set on rising/falling signal edge          \
							   */
#define MFRC522_SET2 (1 << 7) /* indicates the status of the marked bits in the DivIrqReg */

/* Section 9.3.1.7: ErrorReg register */

#define MFRC522_PROTO_ERR                                                                          \
	(1 << 0) /* set if the SOF is incorrect or during MFAuthent if data is incorrect */
...
#define MFRC522_WR_ERR                                                                             \
	(1 << 7) /* data write error in the FIFO, host writing to FIFO at the wrong time */
...
#define MFRC522_MODEM_STATE_MASK                                                                   \
	(7 << 0)				   /* shows the state of the transmitter and receiver state machine */
...
#define MFRC522_TEMP_SENS_CLEAR                                                                    \
	(1 << 7) /* clears the temperature error if it is below 125C (R/W bit) */
...
#define MFRC522_FIFOLEVEL_MASK                                                                     \
	(0x7F)							  /* indicates the number of bytes stored in the FIFO buffer   \
									   */
...
#define MFRC522_RX_LAST_BITS_MASK                                                                  \
	(7 << 0) /* indicates the number of valid bits in the last received byte */
...
#define MFRC522_TX_LAST_BITS_MASK                                                                  \
	(7 << 0) /* defines the number of bits of the last byte that will be transmitted */
...
#define MFRC522_COLL_POS_NOT_VALID                                                                 \
	(1 << 5) /* no collision detected or it is out of the range of CollPos[4:0] */
#define MFRC522_VALUES_AFTER_COLL                                                                  \
	(1 << 7) /* 0 means: all received bits will be cleared after a collision */
...
#define MFRC522_TX_WAIT_RF                                                                         \
	(1 << 5)					   /* transmitter can only be started if an RF field is generated */
...
#define MFRC522_RX_MULTIPLE                                                                        \
	(1 << 2) /* enable to receive more than one data frame, only at 106kBd */
#define MFRC522_RX_NO_ERR                                                                          \
	(1 << 3) /* ignore invalid data stream error (less than 4 bits received) */
...
#define MFRC522_INV_TX1_RF_OFF                                                                     \
	(1 << 4) /* output signal on pin TX1 is inverted when driver TX1 is disabled */
#define MFRC522_INV_TX2_RF_OFF                                                                     \
	(1 << 5) /* output signal on pin TX2 is inverted when driver TX2 is disabled */
#define MFRC522_INV_TX1_RF_ON                                                                      \
	(1 << 6) /* output signal on pin TX1 is inverted when driver TX1 is enabled */
#define MFRC522_INV_TX2_RF_ON                                                                      \
	(1 << 7) /* output signal on pin TX2 is inverted when driver TX2 is enabled */

/* Section 9.3.2.6: TxASKReg register */

#define MFRC522_FORCE_100ASK                                                                       \
	(1 << 6) /* forces a 100% ASK modulation independent of the ModGsPReg setting */
...
#define MFRC522_MFOUT_TX_STREAM                                                                    \
	(5) /* serial data stream to be transmitted, data stream before Miller encoder */
		/* 6 - reserved */
#define MFRC522_MFOUT_RX_STREAM                                                                    \
	(7) /* serial data stream received, data stream after Manchester decoder */
		/* 8-15 - reserved */
#define MFRC522_DRV_SEL_MASK (3 << 4)	/* selects the input of drivers TX1 and TX2 */
#define MFRC522_DRV_SEL_3STATE (0 << 4) /* 3-state */
#define MFRC522_DRV_SEL_INT_ENV                                                                    \
	(1 << 4) /* modulation signal (envelope) from the internal encoder */
#define MFRC522_DVR_SEL_ENV_MFIN (2 << 4) /* modulation signal (envelope) from pin MFIN */
#define MFRC522_DVR_SEL_HIGH                                                                       \
	(3 << 4) /* High: depends on InvTx1RFOn/InvTx1RFOff and InvTx2RFOn/InvTx2RFOff */
...
#define MFRC522_UART_INT_MOD                                                                       \
	(2 << 6) /* modulated signal from the internal analog module, default */
...
#define MFRC522_TAU_SYNC_MASK                                                                      \
	(3 << 0) /* changes the time-constant of the internal PLL during burst */
#define MFRC522_TAU_RCV_MASK                                                                       \
	(3 << 2) /* changes the time-constant of the internal PLL during data reception */
#define MFRC522_TPRESCAL_EVEN (1 << 4) /* defines the Timer Prescaler formula to use */
#define MFRC522_FIX_IQ                                                                             \
	(1 << 5) /* defines if reception will be fixed at channel I or Q based on AddIQ[1:0] */
...
#define MFRC522_SELFTEST_MASK                                                                      \
	(0xF) /* for default operation the self test must be disabled by value 0000b */
#define MFRC522_RFT_MASK (3 << 4) /* reserved for production tests */
#define MFRC522_AMP_RCV                                                                            \
	(1 << 6) /* non-linear signal processing mode, increase range distance at 106kBd */
