...
static int sdio_sendcmdpoll(FAR struct sdio_dev_s *dev, uint32_t cmd, uint32_t arg)
{
	int ret;
	...
	if (ret == OK) {
		...
		if (ret != OK) {
			wlerr("ERROR: Wait for response to cmd: %08" PRIx32 " failed: %d\n", cmd, ret);
		}
	}
						...
						unsigned int nblocks)
{
	...
	SDIO_WAITENABLE(dev, SDIOWAIT_TRANSFERDONE | SDIOWAIT_TIMEOUT | SDIOWAIT_ERROR,
...
int sdio_set_blocksize(FAR struct sdio_dev_s *dev, uint8_t function, uint16_t blocksize)
{
	...
	ret = sdio_io_rw_direct(dev, true, 0, (function << SDIO_FBR_SHIFT) + SDIO_CCCR_FN0_BLKSIZE_0,
							blocksize & 0xff, NULL);
	if (ret != OK) {
		return ret;
	}

	ret = sdio_io_rw_direct(dev, true, 0, (function << SDIO_FBR_SHIFT) + SDIO_CCCR_FN0_BLKSIZE_1,
							(blocksize >> 8), NULL);
