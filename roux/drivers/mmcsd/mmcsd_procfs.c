...
typedef CODE ssize_t (*mmcsd_read_t)(FAR struct file *filep, FAR char *buffer, size_t buflen,
...
static int mmcsd_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int mmcsd_close(FAR struct file *filep);
static ssize_t mmcsd_read_cid(FAR struct file *filep, FAR char *buffer, size_t buflen,
							  FAR struct mmcsd_state_s *priv);
static ssize_t mmcsd_read_csd(FAR struct file *filep, FAR char *buffer, size_t buflen,
							  FAR struct mmcsd_state_s *priv);
static ssize_t mmcsd_read_type(FAR struct file *filep, FAR char *buffer, size_t buflen,
							   FAR struct mmcsd_state_s *priv);
static ssize_t mmcsd_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static int mmcsd_dup(FAR const struct file *oldp, FAR struct file *newp);

static int mmcsd_opendir(FAR const char *relpath, FAR struct fs_dirent_s **dir);
static int mmcsd_closedir(FAR struct fs_dirent_s *dir);
static int mmcsd_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry);
...
static const struct procfs_entry_s g_mmcsd_procfs1 = {"mmcsd", &g_mmcsd_operations,
													  PROCFS_DIR_TYPE};
static const struct procfs_entry_s g_mmcsd_procfs2 = {"mmcsd/**", &g_mmcsd_operations,
													  PROCFS_UNKOWN_TYPE};
...
static int mmcsd_get_file_index(FAR const char *relpath)
{
	...
	for (i = 0; i < nitems(g_mmcsd_files); i++) {
		if (strncmp(relpath, g_mmcsd_files[i].name, strlen(g_mmcsd_files[i].name)) == 0) {
			return i;
		}
	}
							  ...
							  FAR struct mmcsd_state_s *priv)
{
	...
	linesize = snprintf(mmcsdfile->line, MMCSD_LINELEN, "%08lx%08lx%08lx%08lx", priv->cid[0],
						priv->cid[1], priv->cid[2], priv->cid[3]);
	totalsize = procfs_memcpy(mmcsdfile->line, linesize, buffer, buflen, &offset);
							  ...
							  FAR struct mmcsd_state_s *priv)
{
	...
	linesize = snprintf(mmcsdfile->line, MMCSD_LINELEN, "%08lx%08lx%08lx%08lx", priv->csd[0],
						priv->csd[1], priv->csd[2], priv->csd[3]);
	totalsize = procfs_memcpy(mmcsdfile->line, linesize, buffer, buflen, &offset);
	filep->f_pos += totalsize;
	return totalsize;
}
...
static int mmcsd_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry)
{
	...
	snprintf(entry->d_name, NAME_MAX + 1, "%s%d", g_mmcsd_files[fpos].name, index);
