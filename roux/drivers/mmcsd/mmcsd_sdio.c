...
static int mmcsd_sendcmdpoll(FAR struct mmcsd_state_s *priv, uint32_t cmd, uint32_t arg);
...
static int mmcsd_verifystate(FAR struct mmcsd_state_s *priv, uint32_t status);
...
static int mmcsd_eventwait(FAR struct mmcsd_state_s *priv, sdio_eventset_t failevents);
...
#if MMCSD_MULTIBLOCK_LIMIT != 1
static int mmcsd_setblockcount(FAR struct mmcsd_state_s *priv, uint32_t nblocks);
#endif
static ssize_t mmcsd_readsingle(FAR struct mmcsd_part_s *part, FAR uint8_t *buffer,
								off_t startblock);
#if MMCSD_MULTIBLOCK_LIMIT != 1
static ssize_t mmcsd_readmultiple(FAR struct mmcsd_part_s *part, FAR uint8_t *buffer,
								  off_t startblock, size_t nblocks);
#endif
static ssize_t mmcsd_writesingle(FAR struct mmcsd_part_s *part, FAR const uint8_t *buffer,
								 off_t startblock);
#if MMCSD_MULTIBLOCK_LIMIT != 1
static ssize_t mmcsd_writemultiple(FAR struct mmcsd_part_s *part, FAR const uint8_t *buffer,
								   off_t startblock, size_t nblocks);
#endif
...
static ssize_t mmcsd_read(FAR struct inode *inode, FAR unsigned char *buffer, blkcnt_t startsector,
...
static int mmcsd_read_extcsd(FAR struct mmcsd_state_s *priv, FAR uint8_t *extcsd);
static void mmcsd_decode_extcsd(FAR struct mmcsd_state_s *priv, FAR const uint8_t *extcsd);
#endif
...
static int mmcsd_iocmd(FAR struct mmcsd_part_s *part, FAR struct mmc_ioc_cmd *ic_ptr);
static int mmcsd_multi_iocmd(FAR struct mmcsd_part_s *part, FAR struct mmc_ioc_multi_cmd *imc_ptr);
#endif
...
static FAR const char *g_partname[MMCSD_PART_COUNT] = {"",	  "boot0", "boot1", "rpmb",
													   "gp1", "gp2",   "gp3",	"gp4"};
...
static void mmcsd_decode_csd(FAR struct mmcsd_state_s *priv, uint32_t csd[4])
{
	...
	if (IS_BLOCK(priv->type)) {
		...
		if (IS_MMC(priv->type)) {
			...
			if (csize != MMCSD_CSD_CSIZE_THRESHOLD) {
				priv->part[0].nblocks = ((uint32_t)csize + 1) * (1 << (csizemult + 2));
			}
	...
	finfo("  TAAC {TIME_UNIT: %d TIME_VALUE: %d} NSAC: %d\n", decoded.taac.timeunit,
		  decoded.taac.timevalue, decoded.nsac);
	...
	finfo("  WRITE_BLK_MISALIGN: %d READ_BLK_MISALIGN: %d\n", decoded.writeblkmisalign,
		  decoded.readblkmisalign);
	finfo("  DSR_IMP: %d\n", decoded.dsrimp);
	...
	if (IS_BLOCK(priv->type)) {
		...
		if (IS_MMC(priv->type)) {
			...
			finfo("    VDD_R_CURR_MIN: %d VDD_R_CURR_MAX: %d\n", decoded.u.mmc.vddrcurrmin,
				  decoded.u.mmc.vddrcurrmax);
			finfo("    VDD_W_CURR_MIN: %d VDD_W_CURR_MAX: %d\n", decoded.u.mmc.vddwcurrmin,
				  decoded.u.mmc.vddwcurrmax);
...
#endif
		{
			...
			finfo("    C_SIZE: %d SD_ER_BLK_EN: %d\n", decoded.u.sdblock.csize,
				  decoded.u.sdblock.sderblen);
		...
		finfo("    C_SIZE: %d C_SIZE_MULT: %d\n", decoded.u.sdbyte.csize,
			  decoded.u.sdbyte.csizemult);
		finfo("    VDD_R_CURR_MIN: %d VDD_R_CURR_MAX: %d\n", decoded.u.sdbyte.vddrcurrmin,
			  decoded.u.sdbyte.vddrcurrmax);
		finfo("    VDD_W_CURR_MIN: %d VDD_W_CURR_MAX: %d\n", decoded.u.sdbyte.vddwcurrmin,
			  decoded.u.sdbyte.vddwcurrmax);
	...
	finfo("  WP_GRP_EN: %d MMC DFLT_ECC: %d (MMC) R2W_FACTOR: %d\n", decoded.wpgrpen,
		  decoded.mmcdfltecc, decoded.r2wfactor);
	...
	finfo("  PERM_WRITE_PROTECT: %d TMP_WRITE_PROTECT: %d\n", decoded.permwriteprotect,
		  decoded.tmpwriteprotect);
	finfo("  FILE_FORMAT: %d ECC: %d (MMC) CRC: %d\n", decoded.fileformat, decoded.mmcecc,
		  decoded.crc);

	finfo("Capacity: %luKb, Block size: %db, nblocks: %d wrprotect: %d\n",
		  (unsigned long)MMCSD_CAPACITY(priv->part[0].nblocks, priv->blockshift), priv->blocksize,
		  priv->part[0].nblocks, priv->wrprotect);
#endif
}
...
static void mmcsd_decode_cid(FAR struct mmcsd_state_s *priv, uint32_t cid[4])
{
	...
	finfo("mid: %02x cbx: %01x oid: %01x pnm: %s prv: %d psn: %08x mdt: %02x\
         crc: %02x\n",
		  decoded.mid, decoded.cbx, decoded.oid, decoded.pnm, decoded.prv,
...
static void mmcsd_decode_scr(FAR struct mmcsd_state_s *priv, uint32_t scr[2])
{
	...
	finfo("  DATA_STATE_AFTER_ERASE: %d SD_SECURITY: %d SD_BUS_WIDTHS: %x\n", decoded.erasestate,
		  decoded.security, decoded.buswidth);
...
static int mmcsd_transferready(FAR struct mmcsd_state_s *priv)
{
	...
	do {
		...
		else if (!IS_STATE(r1, MMCSD_R1_STATE_PRG) && !IS_STATE(r1, MMCSD_R1_STATE_RCV)) {
								...
								off_t startblock)
{
	...
	if (priv->partnum != partnum) {
		ret = mmcsd_switch(priv, MMC_CMD6_MODE(MMC_CMD6_MODE_WRITE_BYTE) |
									 MMC_CMD6_INDEX(EXT_CSD_PART_CONF) | MMC_CMD6_VALUE(partnum));
		if (ret != OK) {
								 ...
								 off_t startblock)
{
	...
	SDIO_WAITENABLE(priv->dev, SDIOWAIT_WRCOMPLETE | SDIOWAIT_TIMEOUT, MMCSD_BLOCK_WDATADELAY);
#endif
...
#ifdef CONFIG_MMCSD_MMCSUPPORT
	if (IS_MMC(priv->type)) {
		ret = mmcsd_setblockcount(priv, priv->partnum == MMCSD_PART_RPMB ? ((1 << 31) | nblocks)
																		 : nblocks);
		if (ret != OK) {
			return ret;
		}
...
static ssize_t mmcsd_read(FAR struct inode *inode, unsigned char *buffer, blkcnt_t startsector,
						  unsigned int nsectors)
{
	FAR struct mmcsd_state_s *priv;
	FAR struct mmcsd_part_s *part;
...
static int mmcsd_widebus(FAR struct mmcsd_state_s *priv)
{
	...
	if (IS_SD(priv->type) && (priv->buswidth & MMCSD_SCR_BUSWIDTH_4BIT) != 0 &&
...
#ifdef CONFIG_MMCSD_MMCSUPPORT
	else if (IS_MMC(priv->type) && ((priv->buswidth & MMCSD_SCR_BUSWIDTH_4BIT) != 0 &&
									(priv->caps & SDIO_CAPS_1BIT_ONLY) == 0)) {
		/* SD card supports 4-bit BUS and host settings is not 1-bit only.
		 * Configuring MMC - Use MMC_SWITCH access modes.
		 */

		mmcsd_sendcmdpoll(priv, MMCSD_CMD6, MMC_CMD6_BUSWIDTH(EXT_CSD_BUS_WIDTH_4));
	...
	if ((priv->caps & SDIO_CAPS_1BIT_ONLY) == 0 &&
		(IS_MMC(priv->type) || (priv->buswidth & MMCSD_SCR_BUSWIDTH_4BIT) != 0)) {
	...
	if (IS_SD(priv->type)) {
		...
		if (priv->caps & SDIO_CAPS_MMC_HS_MODE) {
			mmcsd_sendcmdpoll(priv, MMCSD_CMD6, MMC_CMD6_HS_TIMING(EXT_CSD_HS_TIMING_HS));
			ret = mmcsd_recv_r1(priv, MMCSD_CMD6);
			if (ret != OK) {
...
static void mmcsd_decode_extcsd(FAR struct mmcsd_state_s *priv, FAR const uint8_t *extcsd)
{
	...
	priv->part[0].nblocks =
		(extcsd[215] << 24) | (extcsd[214] << 16) | (extcsd[213] << 8) | extcsd[212];
...
static int mmcsd_general_cmd_write(FAR struct mmcsd_state_s *priv, FAR const uint8_t *buffer,
								   off_t startblock)
{
	int ret;
...
static int mmcsd_general_cmd_read(FAR struct mmcsd_state_s *priv, FAR uint8_t *buffer,
								  off_t startblock)
{
	int ret;
...
static int mmcsd_iocmd(FAR struct mmcsd_part_s *part, FAR struct mmc_ioc_cmd *ic_ptr)
{
	...
	{
		memcpy((FAR void *)(uintptr_t)ic_ptr->data_ptr, priv->cid, sizeof(priv->cid));
	} break;
	...
	{
		ret = mmcsd_read_extcsd(priv, (FAR uint8_t *)(uintptr_t)ic_ptr->data_ptr);
	} break;
#endif
	...
	case MMCSD_CMDIDX18: /* Read multi blocks commands */
	{
		if (ic_ptr->blocks > 0) {
			...
			ret = mmcsd_readmultiple(part, (FAR uint8_t *)(uintptr_t)ic_ptr->data_ptr, ic_ptr->arg,
									 ic_ptr->blocks);
	...
	{
		ret = mmcsd_setblockcount(priv, ic_ptr->blocks ? ic_ptr->blocks : ic_ptr->arg);
	} break;
	case MMCSD_CMDIDX25: /* Write multi blocks commands */
	{
		if (ic_ptr->blocks > 0) {
			...
			ret = mmcsd_writemultiple(part, (const FAR uint8_t *)(uintptr_t)ic_ptr->data_ptr,
	...
	case MMCSD_CMDIDX56: /* General commands */
	{
		if (ic_ptr->write_flag) {
			ret = mmcsd_general_cmd_write(priv, (FAR uint8_t *)(uintptr_t)(ic_ptr->data_ptr),
										  ic_ptr->arg);
			if (ret != OK) {
				ferr("mmcsd_iocmd MMCSD_CMDIDX56 write failed.\n");
			}
		} else {
			ret = mmcsd_general_cmd_read(priv, (FAR uint8_t *)(uintptr_t)(ic_ptr->data_ptr),
										 ic_ptr->arg);
			if (ret != OK) {
		...
		if (ret != OK) {
			...
			if (ret == OK) {
				...
				finfo("Capacity: %" PRIuOFF " Kbytes\n",
					  MMCSD_CAPACITY(priv->part[0].nblocks, priv->blockshift));
			...
			for (i = 0; i < MMCSD_PART_COUNT; i++) {
				...
				if (priv->part[i].nblocks != 0) {
					snprintf(devname, sizeof(devname), "/dev/mmcsd%d%s", priv->minor,
							 g_partname[i]);
					register_blockdriver(devname, &g_bops, 0666, &priv->part[i]);
				}
			}
		}
...
static int mmcsd_removed(FAR struct mmcsd_state_s *priv)
{
	...
	for (i = 0; i < MMCSD_PART_COUNT; i++) {
		snprintf(devname, sizeof(devname), "/dev/mmcsd%d%s", priv->minor, g_partname[i]);
...
int mmcsd_slotinitialize(int minor, FAR struct sdio_dev_s *dev)
{
	...
	finfo("MMC: %s %" PRIu64 "KB %s %s mode\n", devname,
		  ...
		  priv->widebus ? "4-bits" : "1-bit", mmc_get_mode_name(priv->mode));
