...
static uint32_t mmcsd_sendcmd(FAR struct mmcsd_slot_s *slot, FAR const struct mmcsd_cmdinfo_s *cmd,
							  uint32_t arg);
static void mmcsd_setblklen(FAR struct mmcsd_slot_s *slot, uint32_t length);
static uint32_t mmcsd_nsac(FAR struct mmcsd_slot_s *slot, FAR uint8_t *csd, uint32_t frequency);
...
static void mmcsd_checkwrprotect(FAR struct mmcsd_slot_s *slot, FAR uint8_t *csd);
static int mmcsd_getcardinfo(FAR struct mmcsd_slot_s *slot, FAR uint8_t *buffer,
...
static int mmcsd_recvblock(FAR struct mmcsd_slot_s *slot, FAR uint8_t *buffer, int nbytes);
#if !defined(CONFIG_MMCSD_READONLY)
static int mmcsd_xmitblock(FAR struct mmcsd_slot_s *slot, FAR const uint8_t *buffer, int nbytes,
						   uint8_t token);
#endif
...
static ssize_t mmcsd_read(FAR struct inode *inode, FAR unsigned char *buffer, blkcnt_t start_sector,
...
static void mmcsd_decodecsd(FAR struct mmcsd_slot_s *slot, FAR uint8_t *csd)
{
	...
	maxfrequency = g_transpeedtu[MMCSD_CSD_TRANSPEED_TIMEVALUE(csd)] *
...
static void mmcsd_checkwrprotect(FAR struct mmcsd_slot_s *slot, FAR uint8_t *csd)
{
	...
	if ((SPI_STATUS(spi, SPIDEV_MMCSD(0)) & SPI_STATUS_WRPROTECTED) != 0 ||
		MMCSD_CSD_PERMWRITEPROTECT(csd) || MMCSD_CSD_TMPWRITEPROTECT(csd)) {
...
#if !defined(CONFIG_MMCSD_READONLY)
	geometry->geo_writeenabled = ((slot->state & MMCSD_SLOTSTATUS_WRPROTECT) == 0);
#else
...
static void mmcsd_mediachanged(FAR void *arg)
{
	...
	slot->state &=
		~(MMCSD_SLOTSTATUS_NODISK | MMCSD_SLOTSTATUS_NOTREADY | MMCSD_SLOTSTATUS_MEDIACHGD);
...
#ifdef CONFIG_DEBUG_FEATURES
	if ((unsigned)slotno >= CONFIG_MMCSD_NSLOTS || (unsigned)minor > 255 || spi == NULL) {
