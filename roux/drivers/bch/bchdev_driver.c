...
static ssize_t bch_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t bch_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
ioctl_default:
	default: {
		...
		if (bchinode->u.i_bops->ioctl != NULL) {
			...
			if (ret == -ENOTTY && (cmd == BIOC_FLUSH || cmd == BIOC_DISCARD)) {
				ret = 0;
			}
		}
	} break;
