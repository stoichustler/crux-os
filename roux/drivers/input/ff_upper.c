...
static ssize_t ff_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int ff_check_effect_access(FAR struct ff_upperhalf_s *upper, int effect_id,
...
static inline bool ff_check_effects_compatible(FAR struct ff_effect *e1, FAR struct ff_effect *e2)
{
	return e1->type == e2->type &&
		   (e1->type != FF_PERIODIC || e1->u.periodic.waveform == e2->u.periodic.waveform);
}
...
static int ff_compat_effect(FAR struct ff_lowerhalf_s *lower, FAR struct ff_effect *effect)
{
	switch (effect->type) {
	case FF_RUMBLE: {
		int magnitude;
		...
		magnitude = effect->u.rumble.strong_magnitude / 3 + effect->u.rumble.weak_magnitude / 6;
...
static int ff_erase_effect(FAR struct ff_upperhalf_s *upper, int effect_id, FAR struct file *filep)
{
...
static int ff_upload(FAR struct ff_upperhalf_s *upper, FAR struct ff_effect *effect,
					 FAR struct file *filep)
{
	FAR struct ff_lowerhalf_s *lower = upper->lower;
...
static int ff_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case EVIOCSFF: {
		ret = ff_upload(upper, (FAR struct ff_effect *)(uintptr_t)arg, filep);
	} break;
...
int ff_register(FAR struct ff_lowerhalf_s *lower, FAR const char *path, int max_effects)
{
	...
	upper = kmm_zalloc(sizeof(struct ff_upperhalf_s) + sizeof(struct ff_effect_s) * max_effects);
	if (upper == NULL) {
		return -ENOMEM;
	}
