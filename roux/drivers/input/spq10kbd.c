...
#include <roux/input/spq10kbd.h>
#include <roux/input/djoystick.h>
...
#define DJOY_SUPPORTED                                                                             \
	(DJOY_UP_BIT | DJOY_DOWN_BIT | DJOY_LEFT_BIT | DJOY_RIGHT_BIT | DJOY_BUTTON_1_BIT |            \
	 DJOY_BUTTON_2_BIT | DJOY_BUTTON_3_BIT | DJOY_BUTTON_4_BIT)
...
static void spq10kbd_putreg8(FAR struct spq10kbd_dev_s *priv, uint8_t regaddr, uint8_t regval);
static uint8_t spq10kbd_getreg8(FAR struct spq10kbd_dev_s *priv, uint8_t regaddr);
static uint16_t spq10kbd_getreg16(FAR struct spq10kbd_dev_s *priv, uint8_t regaddr);
static int spq10kbd_checkver(FAR struct spq10kbd_dev_s *priv);
static int spq10kbd_reset(FAR struct spq10kbd_dev_s *priv);
static void spq10kbd_putbuffer(FAR struct spq10kbd_dev_s *priv, uint8_t keycode);
...
static void djoy_enable(FAR const struct djoy_lowerhalf_s *lower, djoy_buttonset_t press,
						djoy_buttonset_t release, djoy_interrupt_t handler, FAR void *arg);
#endif /* CONFIG_SPQ10KBD_DJOY */
...
static ssize_t spq10kbd_read(FAR struct file *filep, FAR char *buffer, size_t len);
static ssize_t spq10kbd_write(FAR struct file *filep, FAR const char *buffer, size_t len);
static int spq10kbd_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static djoy_buttonset_t djoy_sample(FAR const struct djoy_lowerhalf_s *lower)
{
	FAR struct spq10kbd_dev_s *priv = (FAR struct spq10kbd_dev_s *)(lower->config);
...
static void spq10kbd_worker(FAR void *arg)
{
	...
	if (regval & SPQ10KBD_INT_KEY) {
		...
		while (spq10kbd_getreg8(priv, SPQ10KBD_KEY) & SPQ10KBD_KEY_COUNT_MASK) {
			...
			state = (regval & SPQ10KBD_FIF_STATE_MASK) >> SPQ10KBD_FIF_STATE_SHIFT;
			key = (regval & SPQ10KBD_FIF_KEY_MASK) >> SPQ10KBD_FIF_KEY_SHIFT;
			if (key <= KEY_BUTTON_LAST &&
				!(key == KEY_BACKSPACE || key == KEY_TAB || key == KEY_NL)) {
#ifdef CONFIG_SPQ10KBD_DJOY
				...
				case KEY_PRESS:
					...
					if (priv->djoypressmask & joystick_map[key]) {
						priv->djoyhandle(&priv->djoylower, priv->djoycbarg);
					}
...
#ifdef CONFIG_SPQ10KBD_DJOY
int spq10kbd_register(FAR struct i2c_master_s *i2c, FAR const struct spq10kbd_config_s *config,
...
#endif
{
	...
	DEBUGASSERT(config->attach != NULL && config->enable != NULL && config->clear != NULL);
