...
static int tsc2007_sample(FAR struct tsc2007_dev_s *priv, FAR struct tsc2007_sample_s *sample);
static int tsc2007_waitsample(FAR struct tsc2007_dev_s *priv, FAR struct tsc2007_sample_s *sample);
...
static int tsc2007_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int tsc2007_interrupt(int irq, FAR void *context)
{
	...
	priv = &g_tsc2007;
#else
	for (priv = g_tsc2007list; priv && priv->configs->irq != irq; priv = priv->flink)
		;
...
int tsc2007_register(FAR struct i2c_master_s *dev, FAR struct tsc2007_config_s *config, int minor)
{
	FAR struct tsc2007_dev_s *priv;
