...
#include <roux/input/cypress_mbr3108.h>
...
static ssize_t mbr3108_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t mbr3108_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int mbr3108_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int mbr3108_i2c_write(FAR struct mbr3108_dev_s *dev, uint8_t reg, FAR const uint8_t *buf,
							 size_t buflen)
{
	struct i2c_msg_s msgv[2] = {
		{CONFIG_MBR3108_I2C_FREQUENCY, dev->addr, 0, &reg, sizeof(reg)},
		{CONFIG_MBR3108_I2C_FREQUENCY, dev->addr, I2C_M_NOSTART, buf, buflen}};
...
static int mbr3108_i2c_read(FAR struct mbr3108_dev_s *dev, uint8_t reg, FAR uint8_t *buf,
							size_t buflen)
{
	struct i2c_msg_s msgv[2] = {{CONFIG_MBR3108_I2C_FREQUENCY, dev->addr, 0, &reg, sizeof(reg)},
								{CONFIG_MBR3108_I2C_FREQUENCY, dev->addr, I2C_M_READ, buf, buflen}};
										...
										FAR const struct mbr3108_sensor_conf_s *conf)
{
	...
	ret = mbr3108_i2c_write(dev, start_reg, conf->conf_data, last_reg - start_reg + 1);
	if (ret < 0) {
...
static int mbr3108_get_sensor_status(FAR struct mbr3108_dev_s *dev, FAR void *buf)
{
	...
	status.latched_button = (readbuf[MBR3108_LATCHED_BUTTON_STAT + 0 - start_reg]) |
	...
	mbr3108_dbg("but: %x, prox: %x; latched[btn: %x, prox: %x]\n", status.button, status.proximity,
				status.latched_button, status.latched_button);

	return 0;
}

static int mbr3108_get_sensor_debug_data(FAR struct mbr3108_dev_s *dev, FAR void *buf)
{
	...
	data.sensor_average_counts = (readbuf[MBR3108_DEBUG_AVG_RAW_COUNT0 + 0 - start_reg]) |
								 (readbuf[MBR3108_DEBUG_AVG_RAW_COUNT0 + 1 - start_reg] << 8);
	data.sensor_baseline = (readbuf[MBR3108_DEBUG_BASELINE0 + 0 - start_reg]) |
						   (readbuf[MBR3108_DEBUG_BASELINE0 + 1 - start_reg] << 8);
	data.sensor_diff_counts = (readbuf[MBR3108_DEBUG_DIFFERENCE_COUNT0 + 0 - start_reg]) |
							  (readbuf[MBR3108_DEBUG_DIFFERENCE_COUNT0 + 1 - start_reg] << 8);
	data.sensor_raw_counts = (readbuf[MBR3108_DEBUG_RAW_COUNT0 + 0 - start_reg]) |
	...
	mbr3108_dbg("avg_cnt: %d, baseline: %d, diff_cnt: %d, raw_cnt: %d, "
				"total_cp: %d\n",
				data.sensor_average_counts, data.sensor_baseline, data.sensor_diff_counts,
				data.sensor_raw_counts, data.sensor_total_capacitance);
...
static int mbr3108_probe_device(FAR struct mbr3108_dev_s *dev)
{
	...
	if (fam_id != MBR3108_EXPECTED_FAMILY_ID || dev_id != MBR3108_EXPECTED_DEVICE_ID ||
		...
		mbr3108_dbg("  Expected: family_id: 0x%02x, device_id: "
					"0x%04x, device_rev: %d\n",
					MBR3108_EXPECTED_FAMILY_ID, MBR3108_EXPECTED_DEVICE_ID,
...
int cypress_mbr3108_register(FAR const char *devpath, FAR struct i2c_master_s *i2c_dev,
							 uint8_t i2c_devaddr, FAR struct mbr3108_board_s *board_config,
							 FAR const struct mbr3108_sensor_conf_s *sensor_conf)
{
	FAR struct mbr3108_dev_s *priv;
	int ret = 0;
