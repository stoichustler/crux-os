...
struct keyboard_upperhalf_s {
	...
	FAR struct keyboard_lowerhalf_s *lower; /* A pointer of lower half instance */
...
static ssize_t keyboard_read(FAR struct file *filep, FAR char *buffer, size_t len);
static ssize_t keyboard_write(FAR struct file *filep, FAR const char *buffer, size_t len);
static int keyboard_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int keyboard_open(FAR struct file *filep)
{
	...
	ret = circbuf_init(&opriv->circ, NULL, upper->nums * sizeof(struct keyboard_event_s));
	if (ret < 0) {
		kmm_free(opriv);
		nxmutex_unlock(&upper->lock);
		return ret;
...
static ssize_t keyboard_read(FAR struct file *filep, FAR char *buff, size_t len)
{
	FAR struct keyboard_opriv_s *opriv = filep->f_priv;
	int ret;
...
static ssize_t keyboard_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct keyboard_upperhalf_s *upper = inode->i_private;
...
int keyboard_register(FAR struct keyboard_lowerhalf_s *lower, FAR const char *path, uint8_t nums)
{
...
int keyboard_unregister(FAR struct keyboard_lowerhalf_s *lower, FAR const char *path)
{
	FAR struct keyboard_upperhalf_s *upper;
	int ret;
...
void keyboard_event(FAR struct keyboard_lowerhalf_s *lower, uint32_t keycode, uint32_t type)
{
