...
static ssize_t mouse_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static int mouse_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int mouse_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int mouse_open(FAR struct file *filep)
{
	...
	ret = circbuf_init(&openpriv->circbuf, NULL, upper->nums * sizeof(struct mouse_report_s));
	if (ret < 0) {
		kmm_free(openpriv);
		nxmutex_unlock(&upper->lock);
		return ret;
...
static ssize_t mouse_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	FAR struct mouse_openpriv_s *openpriv = filep->f_priv;
	ssize_t ret;
...
void mouse_event(FAR void *priv, FAR const struct mouse_report_s *sample)
{
	...
	{
		circbuf_overwrite(&openpriv->circbuf, sample, sizeof(struct mouse_report_s));
...
int mouse_register(FAR struct mouse_lowerhalf_s *lower, FAR const char *path, uint8_t nums)
{
...
void mouse_unregister(FAR struct mouse_lowerhalf_s *lower, FAR const char *path)
{
	FAR struct mouse_upperhalf_s *upper;
