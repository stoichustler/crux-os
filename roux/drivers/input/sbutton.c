		...
		if (state == KEY_RELEASE && priv->pressed) {
			...
			if (elapsed < MSEC2TICK(CONFIG_INPUT_SBUTTON_KEY_DEBOUNCE_MS)) {
				...
				if (elapsed < MSEC2TICK(CONFIG_INPUT_SBUTTON_KEY_THRESH_MS)) {
					iinfo("KEY_1\n");
					keyboard_event(&priv->lower, CONFIG_INPUT_SBUTTON_KEY1, KEYBOARD_PRESS);
				} else {
					iinfo("KEY_2\n");
					keyboard_event(&priv->lower, CONFIG_INPUT_SBUTTON_KEY2, KEYBOARD_PRESS);
				}
			}
		}
	}
...
int sbutton_register(FAR const struct sbutton_config_s *config, char kbdminor)
{
	...
	ret = keyboard_register(&priv->lower, kbddevname, CONFIG_INPUT_SBUTTON_BUFSIZE);
	if (ret < 0) {
