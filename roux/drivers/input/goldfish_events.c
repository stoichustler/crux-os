...
#include <roux/input/goldfish_events.h>
...
static int goldfish_events_interrupt(int irq, FAR void *dev_id, FAR void *arg);
...
static bool goldfish_events_send_keyboard_event(FAR struct goldfish_events_s *events,
												FAR struct goldfish_input_event *evt)
{
	if (evt->type == EV_KEY) {
		keyboard_event(&(events->keyboardlower), keyboard_translate_virtio_code(evt->code),
					   !evt->value);
		return true;
	}
...
static bool goldfish_events_send_mouse_event(FAR struct goldfish_events_s *events,
											 FAR struct goldfish_input_event *evt)
{
	if (evt->type == EV_REL) {
		switch (evt->code) {
		...
		case BTN_MIDDLE:
			...
			return true;
			 ...
			 ((events->mousesample.x != 0) || (events->mousesample.y != 0)
#ifdef CONFIG_INPUT_MOUSE_WHEEL
...
static bool goldfish_events_send_touch_event(FAR struct goldfish_events_s *events,
											 FAR struct goldfish_input_event *evt)
{
	if (evt->type == EV_ABS) {
		switch (evt->code) {
		...
		if (evt->code == BTN_TOUCH) {
			...
			return true;
			   ...
			   ((events->touchsample.point[0].pressure != 0) ||
				(events->touchsample.point[0].x != 0) || (events->touchsample.point[0].y != 0))) {
...
static void goldfish_events_worker(FAR void *arg)
{
	...
	putreg32(GOLDFISH_EVENTS_PAGE_ABSDATA, events->base + GOLDFISH_EVENTS_SET_PAGE);
...
static void goldfish_drivers_register(FAR struct goldfish_events_s *events)
{
	...
	if (getreg32(events->base + GOLDFISH_EVENTS_LEN)) {
		putreg32(GOLDFISH_EVENTS_PAGE_ABSDATA, events->base + GOLDFISH_EVENTS_SET_PAGE);
		if (getreg32(events->base + GOLDFISH_EVENTS_LEN)) {
			keyboard_register(&(events->keyboardlower), "/dev/kbd0", CONFIG_INPUT_GOLDFISH_NBUFFER);
		}
	}

	putreg32(GOLDFISH_EVENTS_PAGE_EVBITS | EV_REL, events->base + GOLDFISH_EVENTS_SET_PAGE);
	if (getreg32(events->base + GOLDFISH_EVENTS_LEN)) {
		putreg32(GOLDFISH_EVENTS_PAGE_ABSDATA, events->base + GOLDFISH_EVENTS_SET_PAGE);
		if (getreg32(events->base + GOLDFISH_EVENTS_LEN)) {
			mouse_register(&(events->mouselower), "/dev/mouse0", CONFIG_INPUT_GOLDFISH_NBUFFER);
		}
	}

	putreg32(GOLDFISH_EVENTS_PAGE_EVBITS | EV_ABS, events->base + GOLDFISH_EVENTS_SET_PAGE);
	if (getreg32(events->base + GOLDFISH_EVENTS_LEN)) {
		putreg32(GOLDFISH_EVENTS_PAGE_ABSDATA, events->base + GOLDFISH_EVENTS_SET_PAGE);
		if (getreg32(events->base + GOLDFISH_EVENTS_LEN)) {
			touch_register(&(events->touchlower), "/dev/input0", CONFIG_INPUT_GOLDFISH_NBUFFER);
		}
	}
}
