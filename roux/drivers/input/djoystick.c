...
static void djoy_interrupt(FAR const struct djoy_lowerhalf_s *lower, FAR void *arg);
...
static ssize_t djoy_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static int djoy_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int djoy_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static void djoy_enable(FAR struct djoy_upperhalf_s *priv)
{
	...
	if (press != 0 || release != 0) {
		...
		lower->dl_enable(lower, press, release, (djoy_interrupt_t)djoy_interrupt, priv);
	} else {
...
static void djoy_sample(FAR struct djoy_upperhalf_s *priv)
{
	...
	for (opriv = priv->du_open; opriv; opriv = opriv->do_flink) {
		...
		if ((press & opriv->do_pollevents.dp_press) != 0 ||
			...
			poll_notify(opriv->do_fds, CONFIG_INPUT_DJOYSTICK_NPOLLWAITERS, POLLIN);
		}
...
static int djoy_close(FAR struct file *filep)
{
	...
	for (prev = NULL, curr = priv->du_open; curr && curr != opriv;
...
static ssize_t djoy_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	FAR struct inode *inode;
	FAR struct djoy_open_s *opriv;
	FAR struct djoy_upperhalf_s *priv;
...
static int djoy_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case DJOYIOC_POLLEVENTS: {
		FAR struct djoy_pollevents_s *pollevents = (FAR struct djoy_pollevents_s *)((uintptr_t)arg);
	...
	case DJOYIOC_REGISTER: {
		FAR struct djoy_notify_s *notify = (FAR struct djoy_notify_s *)((uintptr_t)arg);
...
int djoy_register(FAR const char *devname, FAR const struct djoy_lowerhalf_s *lower)
{
	FAR struct djoy_upperhalf_s *priv;
	int ret;
	...
	priv = (FAR struct djoy_upperhalf_s *)kmm_zalloc(sizeof(struct djoy_upperhalf_s));
	if (!priv) {
