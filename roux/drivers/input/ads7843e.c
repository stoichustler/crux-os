...
static int ads7843e_sample(FAR struct ads7843e_dev_s *priv, FAR struct ads7843e_sample_s *sample);
...
static int ads7843e_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static void ads7843e_worker(FAR void *arg)
{
	 ...
	 */
		...
		wd_start(&priv->wdog, ADS7843E_WDOG_DELAY, ads7843e_wdog, (wdparm_t)priv);
		goto ignored;
	} else {
...
static int ads7843e_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	priv = &g_ads7843e;
#else
	for (priv = g_ads7843elist; priv && priv->configs->irq != irq; priv = priv->flink)
		;
...
int ads7843e_register(FAR struct spi_dev_s *spi, FAR struct ads7843e_config_s *config, int minor)
{
	FAR struct ads7843e_dev_s *priv;
