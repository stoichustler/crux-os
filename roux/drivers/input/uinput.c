...
#include <roux/roux.h>
#include <roux/input/buttons.h>
#include <roux/input/keyboard.h>
...
struct uinput_context_s {
	...
	ssize_t (*notify)(FAR void *uinput_lower, FAR const char *buffer, size_t buflen);
};
...
static int uinput_rpmsg_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
							   uint32_t src, FAR void *priv);

static void uinput_rpmsg_device_created(FAR struct rpmsg_device *rdev, FAR void *priv);

static void uinput_rpmsg_device_destroy(FAR struct rpmsg_device *rdev, FAR void *priv);

static int uinput_rpmsg_initialize(FAR struct uinput_context_s *ctx, FAR const char *name);

static void uinput_rpmsg_notify(FAR struct uinput_context_s *ctx, FAR const char *buffer,
								size_t buflen);
...
static ssize_t uinput_touch_notify(FAR void *uinput_lower, FAR const char *buffer, size_t buflen);

static ssize_t uinput_touch_write(FAR struct touch_lowerhalf_s *lower, FAR const char *buffer,
								  size_t buflen);
...
static ssize_t uinput_button_notify(FAR void *uinput_lower, FAR const char *buffer, size_t buflen);

static ssize_t uinput_button_write(FAR const struct btn_lowerhalf_s *lower, FAR const char *buffer,
								   size_t buflen);

static btn_buttonset_t uinput_button_supported(FAR const struct btn_lowerhalf_s *lower);

static btn_buttonset_t uinput_button_buttons(FAR const struct btn_lowerhalf_s *lower);

static void uinput_button_enable(FAR const struct btn_lowerhalf_s *lower, btn_buttonset_t press,
								 btn_buttonset_t release, btn_handler_t handler, FAR void *arg);

#endif /* CONFIG_UINPUT_BUTTONS */

#ifdef CONFIG_UINPUT_KEYBOARD

static ssize_t uinput_keyboard_notify(FAR void *uinput_lower, FAR const char *buffer,
									  size_t buflen);

static ssize_t uinput_keyboard_write(FAR struct keyboard_lowerhalf_s *lower, FAR const char *buffer,
									 size_t buflen);
...
static void uinput_rpmsg_device_created(FAR struct rpmsg_device *rdev, FAR void *priv)
{
	...
	ept->ept.priv = ctx;
	snprintf(rpmsg_ept_name, sizeof(rpmsg_ept_name), UINPUT_NAME_RPMSG, ctx->name);
	ret = rpmsg_create_ept(&ept->ept, rdev, rpmsg_ept_name, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
...
static int uinput_rpmsg_initialize(FAR struct uinput_context_s *ctx, FAR const char *name)
{
	...
	return rpmsg_register_callback(ctx, uinput_rpmsg_device_created, uinput_rpmsg_device_destroy,
								   NULL, NULL);
}
...
static ssize_t uinput_keyboard_notify(FAR void *uinput_lower, FAR const char *buffer, size_t buflen)
{
...
int uinput_touch_initialize(void)
{
	...
	ret = touch_register(&utcs_lower->lower, "/dev/" UINPUT_NAME_TOUCH,
