...
#include <roux/input/ajoystick.h>
...
static void ajoy_interrupt(FAR const struct ajoy_lowerhalf_s *lower, FAR void *arg);
...
static ssize_t ajoy_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static int ajoy_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int ajoy_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static void ajoy_enable(FAR struct ajoy_upperhalf_s *priv)
{
	...
	if (press != 0 || release != 0) {
		...
		lower->al_enable(lower, press, release, (ajoy_handler_t)ajoy_interrupt, priv);
	} else {
...
static void ajoy_sample(FAR struct ajoy_upperhalf_s *priv)
{
	...
	for (opriv = priv->au_open; opriv; opriv = opriv->ao_flink) {
		...
		if ((press & opriv->ao_pollevents.ap_press) != 0 ||
			...
			poll_notify(opriv->ao_fds, CONFIG_INPUT_AJOYSTICK_NPOLLWAITERS, POLLIN);
		}
...
static int ajoy_close(FAR struct file *filep)
{
	...
	for (prev = NULL, curr = priv->au_open; curr && curr != opriv;
...
static ssize_t ajoy_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	FAR struct inode *inode;
	FAR struct ajoy_open_s *opriv;
	FAR struct ajoy_upperhalf_s *priv;
...
static int ajoy_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case AJOYIOC_POLLEVENTS: {
		FAR struct ajoy_pollevents_s *pollevents = (FAR struct ajoy_pollevents_s *)((uintptr_t)arg);
	...
	case AJOYIOC_REGISTER: {
		FAR struct ajoy_notify_s *notify = (FAR struct ajoy_notify_s *)((uintptr_t)arg);
...
int ajoy_register(FAR const char *devname, FAR const struct ajoy_lowerhalf_s *lower)
{
	FAR struct ajoy_upperhalf_s *priv;
	int ret;
	...
	priv = (FAR struct ajoy_upperhalf_s *)kmm_zalloc(sizeof(struct ajoy_upperhalf_s));
	if (!priv) {
