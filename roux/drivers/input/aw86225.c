...
#include <roux/mm/mm.h>
#include <roux/roux.h>
#include <roux/timers/timer.h>
#include <roux/timers/watchdog.h>
#include <roux/ioexpander/ioexpander.h>
#include <roux/ioexpander/gpio.h>
#include <roux/input/aw86225.h>
#include <roux/input/ff.h>
#include <roux/irq.h>
#include <roux/lib/lib.h>
...
static int aw86225_i2c_write_cnt(FAR struct aw86225 *aw86225, uint8_t reg_addr,
...
static int aw86225_i2c_write(FAR struct aw86225 *aw86225, uint8_t reg_addr, uint8_t reg_data)
{
	return aw86225_i2c_write_cnt(aw86225, reg_addr, &reg_data, 1);
}

static int aw86225_i2c_read_cnt(FAR struct aw86225 *aw86225, uint8_t reg_addr,
								FAR const uint8_t *reg_data, uint32_t cnt)
{
	struct i2c_msg_s msg[2];
	int ret = -1;
...
static int aw86225_i2c_read(FAR struct aw86225 *aw86225, uint8_t reg_addr, FAR uint8_t *reg_data)
{
	return aw86225_i2c_read_cnt(aw86225, reg_addr, reg_data, 1);
}

#ifdef CONFIG_AW86225_RTP_FILE_SUPPORT
static int aw86225_i2c_writes(FAR struct aw86225 *aw86225, unsigned char reg_addr,
							  FAR unsigned char *buf, unsigned int len)
{
	return aw86225_i2c_write_cnt(aw86225, reg_addr, buf, len);
}
#endif

static int aw86225_i2c_write_bits(FAR struct aw86225 *aw86225, unsigned char reg_addr,
...
static int aw86225_request_firmware(FAR struct aw86225_firmware *fw, FAR const char *filename)
{
	FAR char *file_path;
	struct file file;
	size_t file_size;
	...
	snprintf(file_path, PATH_MAX, "%s/%s", CONFIG_AW86225_RTP_FILE_PATH, filename);
...
static void aw86225_haptic_upload_lra(FAR struct aw86225 *aw86225, unsigned int flag)
{
	switch (flag) {
	case AW86225_WRITE_ZERO: {
		aw86225_i2c_write_bits(aw86225, AW86225_REG_TRIMCFG3, AW86225_BIT_TRIMCFG3_TRIM_LRA_MASK,
							   0x00);
		break;
	}
...
static void aw86225_sram_size(FAR struct aw86225 *aw86225, int size_flag)
{
	if (size_flag == AW86225_HAPTIC_SRAM_2K) {
		aw86225_i2c_write_bits(aw86225, AW86225_REG_RTPCFG1, AW86225_BIT_RTPCFG1_SRAM_SIZE_2K_MASK,
							   AW86225_BIT_RTPCFG1_SRAM_SIZE_2K_EN);
		aw86225_i2c_write_bits(aw86225, AW86225_REG_RTPCFG1, AW86225_BIT_RTPCFG1_SRAM_SIZE_1K_MASK,
...
static void aw86225_haptic_stop(FAR struct aw86225 *aw86225)
{
	...
	if (force_flag) {
		...
		aw86225_i2c_write_bits(aw86225, AW86225_REG_SYSCTRL2, AW86225_BIT_SYSCTRL2_STANDBY_MASK,
...
static void aw86225_haptic_raminit(FAR struct aw86225 *aw86225, bool flag)
{
	if (flag) {
		aw86225_i2c_write_bits(aw86225, AW86225_REG_SYSCTRL1, AW86225_BIT_SYSCTRL1_RAMINIT_MASK,
...
static void aw86225_haptic_get_vbat(FAR struct aw86225 *aw86225)
{
	...
	aw86225_i2c_write_bits(aw86225, AW86225_REG_DETCFG2, AW86225_BIT_DETCFG2_VBAT_GO_MASK,
...
static int aw86225_haptic_set_gain(FAR struct aw86225 *aw86225, unsigned char gain)
{
	...
	if (aw86225->ram_vbat_compensate == AW86225_HAPTIC_RAM_VBAT_COMP_ENABLE) {
		...
		iinfo("%s: enable vbat comp, level = %x comp level = %x", __func__, gain, comp_gain);
		aw86225_i2c_write(aw86225, AW86225_REG_PLAYCFG2, comp_gain);
	} else {
		iinfo("%s: disable compsensation, vbat=%d, vbat_min=%d, vbat_ref=%d", __func__,
			  aw86225->vbat, AW86225_VBAT_MIN, AW86225_VBAT_REFER);
...
static int aw86225_haptic_ram_vbat_compensate(FAR struct aw86225 *aw86225, bool flag)
{
	if (flag) {
...
static int aw86225_haptic_play_mode(FAR struct aw86225 *aw86225, unsigned char play_mode)
{
	switch (play_mode) {
	...
	case AW86225_HAPTIC_RAM_MODE: {
		...
		aw86225_i2c_write_bits(aw86225, AW86225_REG_PLAYCFG3, AW86225_BIT_PLAYCFG3_PLAY_MODE_MASK,
...
static int aw86225_haptic_set_wav_seq(FAR struct aw86225 *aw86225, unsigned char wav,
									  unsigned char seq)
{
...
static int aw86225_haptic_set_wav_loop(FAR struct aw86225 *aw86225, unsigned char wav,
									   unsigned char loop)
{
	unsigned char tmp = 0;
...
static int aw86225_haptic_set_main_loop(FAR struct aw86225 *aw86225, uint8_t loop)
{
	aw86225_i2c_write_bits(aw86225, AW86225_REG_WAVCFG13, AW86225_BIT_WAVCFG13_MAINLOOP_MASK, loop);
	return 0;
}
...
static int aw86225_haptic_cont_get_f0(FAR struct aw86225 *aw86225)
{
	...
	aw86225_i2c_write_bits(aw86225, AW86225_REG_CONTCFG1, AW86225_BIT_CONTCFG1_EN_F0_DET_MASK,
						   AW86225_BIT_CONTCFG1_F0_DET_ENABLE);

	/* cont config */

	aw86225_i2c_write_bits(aw86225, AW86225_REG_CONTCFG6, AW86225_BIT_CONTCFG6_TRACK_EN_MASK,
	...
	aw86225_i2c_write_bits(aw86225, AW86225_REG_PLAYCFG3, AW86225_BIT_PLAYCFG3_BRK_EN_MASK,
						   AW86225_BIT_PLAYCFG3_BRK_ENABLE);

	/* f0 driver level */

	aw86225_i2c_write_bits(aw86225, AW86225_REG_CONTCFG6, AW86225_BIT_CONTCFG6_DRV1_LVL_MASK,
						   aw86225->config->cont_drv1_lvl_dt);
	aw86225_i2c_write(aw86225, AW86225_REG_CONTCFG7, aw86225->config->cont_drv2_lvl_dt);

	/* DRV1_TIME */

	aw86225_i2c_write(aw86225, AW86225_REG_CONTCFG8, aw86225->config->cont_drv1_time_dt);

	/* DRV2_TIME */

	aw86225_i2c_write(aw86225, AW86225_REG_CONTCFG9, aw86225->config->cont_drv2_time_dt);
...
static void aw86225_haptic_set_rtp_aei(FAR struct aw86225 *aw86225, bool flag)
{
	if (flag) {
		aw86225_i2c_write_bits(aw86225, AW86225_REG_SYSINTM, AW86225_BIT_SYSINTM_FF_AEM_MASK,
...
static int aw86225_haptic_rtp_init(FAR struct aw86225 *aw86225)
{
	...
	while ((!aw86225_haptic_rtp_get_fifo_afs(aw86225)) &&
		...
		if (aw86225->rtp_cnt < aw86225->ram.base_addr) {
			if ((aw86225->rtp_container->len - aw86225->rtp_cnt) < aw86225->ram.base_addr) {
		...
		aw86225_i2c_writes(aw86225, AW86225_REG_RTPDATA,
						   &aw86225->rtp_container->data[aw86225->rtp_cnt], buf_len);
		aw86225->rtp_cnt += buf_len;
		aw86225_i2c_read(aw86225, AW86225_REG_GLBRD5, &glb_state_val);
		if ((aw86225->rtp_cnt == aw86225->rtp_container->len) || ((glb_state_val & 0x0f) == 0x00)) {
	...
	nxmutex_unlock(&aw86225->rtp_lock);
	if (aw86225->play_mode == AW86225_HAPTIC_RTP_MODE && !atomic_read(&aw86225->exit_in_rtp_loop)) {
...
static int aw86225_haptic_set_repeat_wav_seq(FAR struct aw86225 *aw86225, unsigned char seq)
{
	aw86225_haptic_set_wav_seq(aw86225, 0x00, seq);
	aw86225_haptic_set_wav_loop(aw86225, 0x00, AW86225_BIT_WAVLOOP_INIFINITELY);
	return 0;
}

static void aw86225_haptic_set_pwm(FAR struct aw86225 *aw86225, unsigned char mode)
{
	switch (mode) {
	case AW86225_PWM_48K: {
		aw86225_i2c_write_bits(aw86225, AW86225_REG_SYSCTRL2, AW86225_BIT_SYSCTRL2_WAVDAT_MODE_MASK,
...
static int16_t aw86225_haptic_effect_strength(FAR struct aw86225 *aw86225)
{
...
static void aw86225_rtp_work_routine(FAR void *arg)
{
	...
	iinfo("%s: effect_id =%d state = %d\n", __func__, aw86225->effect_id, aw86225->state);
	...
	if (aw86225->state) {
		...
		aw86225->rtp_file_num = aw86225->effect_id - aw86225->config->effect_id_boundary;
		iinfo("aw86225->rtp_file_num =%d\n", aw86225->rtp_file_num);
		if (aw86225->rtp_file_num < 0) {
			aw86225->rtp_file_num = 0;
		}

		if (aw86225->rtp_file_num > ((sizeof(g_aw86225_rtp_name) / AW86225_RTP_NAME_MAX) - 1)) {
			aw86225->rtp_file_num = (sizeof(g_aw86225_rtp_name) / AW86225_RTP_NAME_MAX) - 1;
		}

		/* fw loaded */

		ret = aw86225_request_firmware(&rtp_file, g_aw86225_rtp_name[aw86225->rtp_file_num]);
		if (ret < 0) {
		...
		iinfo("%s: rtp file:[%s] size = %dbytes\n", __func__,
			  g_aw86225_rtp_name[aw86225->rtp_file_num], aw86225->rtp_container->len);
									...
									FAR struct aw86225_container *aw86225_cont)
{
	...
	aw86225->ram.base_addr =
		(unsigned int)((aw86225_cont->data[0 + shift] << 8) | (aw86225_cont->data[1 + shift]));

	/* default 3k SRAM */

	aw86225_sram_size(aw86225, AW86225_HAPTIC_SRAM_3K);

	aw86225_i2c_write_bits(aw86225, AW86225_REG_RTPCFG1, AW86225_BIT_RTPCFG1_ADDRH_MASK,
						   aw86225_cont->data[0 + shift]);

	aw86225_i2c_write(aw86225, AW86225_REG_RTPCFG2, aw86225_cont->data[1 + shift]);

	/* FIFO_AEH */

	aw86225_i2c_write_bits(aw86225, AW86225_REG_RTPCFG3, AW86225_BIT_RTPCFG3_FIFO_AEH_MASK,
						   (unsigned char)(((aw86225->ram.base_addr >> 1) >> 4) & 0xf0));

	/* FIFO AEL */

	aw86225_i2c_write(aw86225, AW86225_REG_RTPCFG4,
					  (unsigned char)(((aw86225->ram.base_addr >> 1) & 0x00ff)));

	/* FIFO_AFH */

	aw86225_i2c_write_bits(
		aw86225, AW86225_REG_RTPCFG3, AW86225_BIT_RTPCFG3_FIFO_AFH_MASK,
		(unsigned char)(((aw86225->ram.base_addr - (aw86225->ram.base_addr >> 2)) >> 8) & 0x0f));

	/* FIFO_AFL */

	aw86225_i2c_write(
		aw86225, AW86225_REG_RTPCFG5,
		(unsigned char)(((aw86225->ram.base_addr - (aw86225->ram.base_addr >> 2)) & 0x00ff)));
	...
	aw86225_i2c_write_bits(aw86225, AW86225_REG_RAMADDRH, AW86225_BIT_RAMADDRH_MASK,
						   aw86225_cont->data[0 + shift]);
	aw86225_i2c_write(aw86225, AW86225_REG_RAMADDRL, aw86225_cont->data[1 + shift]);
	shift = aw86225->ram.ram_shift;
	iinfo("%s: ram_len = %d\n", __func__, aw86225_cont->len - shift);
	for (i = shift; i < aw86225_cont->len; i++) {
		aw86225->ram_update_flag =
			aw86225_i2c_write(aw86225, AW86225_REG_RAMDATA, aw86225_cont->data[i]);
	}
...
static void aw86225_ram_loaded(FAR struct aw86225_firmware *cont, FAR struct aw86225 *aw86225)
{
...
#ifdef AW_READ_BIN_FLEXBALLY
		if (load_cont <= 20) {
			work_queue(HPWORK, &aw86225->ram_work, aw86225_ram_work_routine, aw86225,
					   MSEC2TICK(ram_timer_val));
...
static void aw86225_vibrator_timer_func(wdparm_t arg)
{
	...
	iinfo("%s enter\n", __func__);
	aw86225->state = 0;
	work_queue(HPWORK, &aw86225->long_vibrate_work, aw86225_long_vibrate_work_routine, aw86225, 0);
}

static void aw86225_haptic_play_repeat_seq(FAR struct aw86225 *aw86225, unsigned char flag)
{
	if (flag) {
...
static void aw86225_haptic_trig_config(FAR struct aw86225 *aw86225)
{
	if (aw86225->is_used_irq == false) {
		aw86225_i2c_write_bits(aw86225, AW86225_REG_SYSCTRL2, AW86225_BIT_SYSCTRL2_INTN_PIN_MASK,
							   AW86225_BIT_SYSCTRL2_TRIG1);
	}
}

static void aw86225_haptic_swicth_motor_protect_config(FAR struct aw86225 *aw86225,
													   unsigned char addr, unsigned char val)
{
	if (addr == 1) {
		aw86225_i2c_write_bits(aw86225, AW86225_REG_DETCFG1, AW86225_BIT_DETCFG1_PRCT_MODE_MASK,
							   AW86225_BIT_DETCFG1_PRCT_MODE_VALID);
		aw86225_i2c_write_bits(aw86225, AW86225_REG_PWMCFG1, AW86225_BIT_PWMCFG1_PRC_EN_MASK,
							   AW86225_BIT_PWMCFG1_PRC_ENABLE);
		aw86225_i2c_write_bits(aw86225, AW86225_REG_PWMCFG3, AW86225_BIT_PWMCFG3_PR_EN_MASK,
		...
		aw86225_i2c_write_bits(aw86225, AW86225_REG_PWMCFG1, AW86225_BIT_PWMCFG1_PRCTIME_MASK, val);
	} else if (addr == 0x3e) {
		aw86225_i2c_write_bits(aw86225, AW86225_REG_PWMCFG3, AW86225_BIT_PWMCFG3_PRLVL_MASK, val);
	} else if (addr == 0x3f) {
...
static void aw86225_haptic_f0_calibration(FAR struct aw86225 *aw86225)
{
	...
	unsigned int f0_cali_min =
		aw86225->config->f0_ref * (100 - aw86225->config->f0_cali_percent) / 100;
	...
	if (aw86225_haptic_cont_get_f0(aw86225)) {
		...
		iinfo("%s f0_ref = %d, f0_cali_min = %d, f0_cali_max = %d, f0 = %d\n", __func__,
			  aw86225->config->f0_ref, f0_cali_min, f0_cali_max, aw86225->f0);
		...
		f0_cali_step =
			100000 * ((int)f0_limit - (int)aw86225->config->f0_ref) / ((int)f0_limit * 24);
...
static int aw86225_haptic_cont_config(FAR struct aw86225 *aw86225)
{
	...
	aw86225_i2c_write(aw86225, AW86225_REG_CONTCFG7, aw86225->cont_drv2_lvl);
...
static int aw86225_haptic_activate(FAR struct aw86225 *aw86225)
{
	aw86225_i2c_write_bits(aw86225, AW86225_REG_SYSCTRL2, AW86225_BIT_SYSCTRL2_STANDBY_MASK,
						   AW86225_BIT_SYSCTRL2_STANDBY_OFF);
	aw86225_interrupt_clear(aw86225);
	aw86225_i2c_write_bits(aw86225, AW86225_REG_SYSINTM, AW86225_BIT_SYSINTM_UVLM_MASK,
...
static int aw86225_haptic_play_effect_seq(FAR struct aw86225 *aw86225, unsigned char flag)
{
	...
	if (flag) {
		if (aw86225->activate_mode == AW86225_HAPTIC_ACTIVATE_RAM_MODE) {
			...
			aw86225_haptic_set_wav_seq(aw86225, 0x00, (char)aw86225->effect_id + 1);
		...
		if (aw86225->activate_mode == AW86225_HAPTIC_ACTIVATE_RAM_LOOP_MODE) {
			aw86225_haptic_set_repeat_wav_seq(aw86225, (aw86225->config->effect_id_boundary + 1));
		...
		if (aw86225->activate_mode == AW86225_HAPTIC_ACTIVATE_RTP_MODE) {
			int budry = aw86225->effect_id - aw86225->config->effect_id_boundary;
			iinfo("budry = %d\n", budry);
			for (uint8_t i = 0; i < 8; i++) {
				iinfo("wav_seq[%d] = %d\n", i, aw86225->pattern[budry].patternid[i]);
				iinfo("wav_loop[%d] = %d\n", i, aw86225->pattern[budry].waveloop[i]);
				aw86225_haptic_set_wav_seq(aw86225, i, aw86225->pattern[budry].patternid[i] + 1);
				aw86225_haptic_set_wav_loop(aw86225, i, aw86225->pattern[budry].waveloop[i]);
			}
...
static void aw86225_haptic_misc_para_init(FAR struct aw86225 *aw86225)
{
	...
	aw86225_i2c_write(aw86225, AW86225_REG_SYSCTRL3, aw86225->config->sine_array[0]);

	/* SIN_L */

	aw86225_i2c_write(aw86225, AW86225_REG_SYSCTRL4, aw86225->config->sine_array[1]);

	/* COS_H */

	aw86225_i2c_write(aw86225, AW86225_REG_SYSCTRL5, aw86225->config->sine_array[2]);

	/* COS_L */

	aw86225_i2c_write(aw86225, AW86225_REG_SYSCTRL6, aw86225->config->sine_array[3]);
	aw86225_i2c_write_bits(aw86225, AW86225_REG_TRGCFG8, AW86225_BIT_TRGCFG8_TRG_TRIG1_MODE_MASK,
	...
	if (!aw86225->config->d2s_gain) {
		...
		aw86225_i2c_write_bits(aw86225, AW86225_REG_SYSCTRL7, AW86225_BIT_SYSCTRL7_D2S_GAIN_MASK,
	...
	if (!aw86225->config->cont_tset) {
		...
		aw86225_i2c_write_bits(aw86225, AW86225_REG_CONTCFG13, AW86225_BIT_CONTCFG13_TSET_MASK,
	...
	if (!aw86225->config->cont_bemf_set) {
		...
		aw86225_i2c_write_bits(aw86225, AW86225_REG_CONTCFG13, AW86225_BIT_CONTCFG13_BEME_SET_MASK,
	...
	if (!aw86225->cont_brk_time) {
		...
		aw86225_i2c_write(aw86225, AW86225_REG_CONTCFG10, aw86225->cont_brk_time);
	}
	...
	if (!aw86225->config->cont_brk_gain) {
		...
		aw86225_i2c_write_bits(aw86225, AW86225_REG_CONTCFG5, AW86225_BIT_CONTCFG5_BRK_GAIN_MASK,
...
static int aw86225_haptic_offset_calibration(FAR struct aw86225 *aw86225)
{
	unsigned int cont = 2000;
	unsigned char reg_val = 0;

	aw86225_haptic_raminit(aw86225, true);
	aw86225_i2c_write_bits(aw86225, AW86225_REG_DETCFG2, AW86225_BIT_DETCFG2_DIAG_GO_MASK,
...
static void aw86225_haptic_vbat_mode_config(FAR struct aw86225 *aw86225, unsigned char flag)
{
	if (flag == AW86225_HAPTIC_CONT_VBAT_HW_ADJUST_MODE) {
		aw86225_i2c_write_bits(aw86225, AW86225_REG_SYSCTRL1, AW86225_BIT_SYSCTRL1_VBAT_MODE_MASK,
...
static void aw86225_haptic_auto_bst_enable(FAR struct aw86225 *aw86225, unsigned char flag)
{
	if (flag) {
		aw86225_i2c_write_bits(aw86225, AW86225_REG_PLAYCFG3, AW86225_BIT_PLAYCFG3_BRK_EN_MASK,
							   AW86225_BIT_PLAYCFG3_BRK_ENABLE);
...
static void aw86225_long_vibrate_work_routine(FAR void *arg)
{
	...
	iinfo("effect_id = %d state=%d activate_mode = %d duration = %d\n", aw86225->effect_id,
		  aw86225->state, aw86225->activate_mode, aw86225->duration);
	...
	if (aw86225->state) {
		if (aw86225->activate_mode == AW86225_HAPTIC_ACTIVATE_RAM_MODE) {
		...
		} else if (aw86225->activate_mode == AW86225_HAPTIC_ACTIVATE_RAM_LOOP_MODE) {
			aw86225_haptic_ram_vbat_compensate(aw86225, true);
			aw86225_haptic_play_effect_seq(aw86225, true);

			/* run ms timer */

			wd_start(&aw86225->timer, MSEC2TICK(aw86225->duration), aw86225_vibrator_timer_func,
					 (wdparm_t)aw86225);
			aw86225->wk_lock_flag = 1;
			...
			wd_start(&aw86225->timer, MSEC2TICK(aw86225->pattern[budry].duration),
...
static int aw86225_haptic_init(FAR struct aw86225 *aw86225)
{
	...
	aw86225_haptic_auto_bst_enable(aw86225, aw86225->config->is_enabled_auto_bst);

	/* vbat compensation */

	aw86225_haptic_vbat_mode_config(aw86225, AW86225_HAPTIC_CONT_VBAT_HW_ADJUST_MODE);
	aw86225->ram_vbat_compensate = AW86225_HAPTIC_RAM_VBAT_COMP_ENABLE;

	/* f0 calibration LRA trim source select register */

	aw86225_i2c_write_bits(aw86225, AW86225_REG_TRIMCFG1, AW86225_BIT_TRIMCFG1_RL_TRIM_SRC_MASK,
...
static int aw86225_haptics_upload_effect(FAR struct ff_lowerhalf_s *lower,
										 FAR struct ff_effect *effect, FAR struct ff_effect *old)
{
	...
	if (aw86225->effect_type == FF_CONSTANT) {
		...
		if (aw86225->effect_id < 0 || aw86225->effect_id > aw86225->config->effect_max) {
			nxmutex_unlock(&aw86225->lock);
			return 0;
		}
		...
		if (aw86225->effect_id < aw86225->config->effect_id_boundary) {
			...
			data[1] = aw86225->predefined[aw86225->effect_id].play_rate_us / 1000000;
			data[2] = aw86225->predefined[aw86225->effect_id].play_rate_us / 1000;
			iinfo("aw86225->predefined[aw86225->effect_id] \
                 .play_rate_us/1000 = %d\n",
				  aw86225->predefined[aw86225->effect_id].play_rate_us / 1000);
		}

		if (aw86225->effect_id >= aw86225->config->effect_id_boundary) {
			aw86225->activate_mode = AW86225_HAPTIC_ACTIVATE_RTP_MODE;
			iinfo("%s: aw86225->effect_id=%d , aw86225->activate_mode = %d\n", __func__,
				  aw86225->effect_id, aw86225->activate_mode);
			int budry = aw86225->effect_id - aw86225->config->effect_id_boundary;
#ifdef CONFIG_AW86225_RTP_FILE_SUPPORT
...
static int aw86225_haptics_playback(struct ff_lowerhalf_s *lower, int effect_id, int val)
{
	FAR struct aw86225 *aw86225 = (FAR struct aw86225 *)lower;
	...
	if (aw86225->effect_type == FF_CONSTANT &&
		...
		work_queue(HPWORK, &aw86225->long_vibrate_work, aw86225_long_vibrate_work_routine, aw86225,
				   0);
	} else if (aw86225->effect_type == FF_PERIODIC &&
			   aw86225->activate_mode == AW86225_HAPTIC_ACTIVATE_RAM_MODE) {
...
#ifdef CONFIG_AW86225_RTP_FILE_SUPPORT
		work_queue(HPWORK, &aw86225->rtp_work, aw86225_rtp_work_routine, aw86225, 0);
#else
		work_queue(HPWORK, &aw86225->long_vibrate_work, aw86225_long_vibrate_work_routine, aw86225,
				   0);
#endif
...
static int aw86225_haptics_erase(FAR struct ff_lowerhalf_s *lower, int effect_id)
{
	FAR struct aw86225 *aw86225 = (FAR struct aw86225 *)lower;
...
static void aw86225_haptics_set_gain_work_routine(FAR void *arg)
{
	...
	iinfo("%s: set_gain queue work, new_gain = %x level = %x \n", __func__, aw86225->new_gain,
		  aw86225->level);

	if (aw86225->ram_vbat_compensate == AW86225_HAPTIC_RAM_VBAT_COMP_ENABLE && aw86225->vbat) {
		unsigned char comp_level = aw86225->level * AW86225_VBAT_REFER / aw86225->vbat;
		...
		iinfo("%s: enable vbat comp, level = %x comp level = %x", __func__, aw86225->level,
			  comp_level);
...
static void aw86225_haptics_set_gain(FAR struct ff_lowerhalf_s *lower, uint16_t gain)
{
	FAR struct aw86225 *aw86225 = (FAR struct aw86225 *)lower;

	aw86225->new_gain = gain;
	work_queue(HPWORK, &aw86225->set_gain_work, aw86225_haptics_set_gain_work_routine, aw86225, 0);
}
...
int aw86225_initialize(FAR struct i2c_master_s *master, FAR struct ioexpander_dev_s *ioedev,
					   FAR const struct aw86225_board_config *config)
{
	...
	ret = IOEXP_SETDIRECTION(ioedev, config->intpin, IOEXPANDER_DIRECTION_IN_PULLUP);
	if (ret < 0) {
		ierr("ioexpander set direction error: %d\n", ret);
		goto err;
	}

	ret = IOEXP_SETDIRECTION(ioedev, config->powerpin, IOEXPANDER_DIRECTION_OUT);
	if (ret < 0) {
		ierr("ioexpander set direction error: %d\n", ret);
		goto err;
	}
