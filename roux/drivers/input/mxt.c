...
#define MXT_GETUINT16(p)                                                                           \
	(((uint16_t)(((FAR uint8_t *)(p))[1]) << 8) | (uint16_t)(((FAR uint8_t *)(p))[0]))
...
static int mxt_getreg(FAR struct mxt_dev_s *priv, uint16_t regaddr, FAR uint8_t *buffer,
					  size_t buflen);
static int mxt_putreg(FAR struct mxt_dev_s *priv, uint16_t regaddr, FAR const uint8_t *buffer,
					  size_t buflen);

/* MXT object/message access */

static FAR struct mxt_object_s *mxt_object(FAR struct mxt_dev_s *priv, uint8_t type);
static int mxt_getmessage(FAR struct mxt_dev_s *priv, FAR struct mxt_msg_s *msg);
static int mxt_putobject(FAR struct mxt_dev_s *priv, uint8_t type, uint8_t offset, uint8_t value);
#if 0 /* Not used */
...
#ifdef CONFIG_MXT_BUTTONS
static void mxt_button_event(FAR struct mxt_dev_s *priv, FAR struct mxt_msg_s *msg);
#endif
static void mxt_touch_event(FAR struct mxt_dev_s *priv, FAR struct mxt_msg_s *msg, int ndx);
static void mxt_worker(FAR void *arg);
static int mxt_interrupt(FAR const struct mxt_lower_s *lower, FAR void *context);
...
static int mxt_getmessage(FAR struct mxt_dev_s *priv, FAR struct mxt_msg_s *msg)
{
	...
	return mxt_getreg(priv, regaddr, (FAR uint8_t *)msg, sizeof(struct mxt_msg_s));
}
...
static void mxt_touch_event(FAR struct mxt_dev_s *priv, FAR struct mxt_msg_s *msg, int ndx)
{
	...
	if ((status & MXT_DETECT) == 0) {
		...
		else if (sample->contact != CONTACT_NEW && sample->contact != CONTACT_MOVE) {
...
static void mxt_worker(FAR void *arg)
{
	...
	do {
		...
		if (id == priv->t6id) {
			...
			status = msg.body[0];
			chksum = (uint32_t)msg.body[1] | ((uint32_t)msg.body[2] << 8) |
			 ...
			 */
				...
				iinfo("Ignored: id=%u message="
					  ...
					  msg.id, msg.body[0], msg.body[1], msg.body[2], msg.body[3], msg.body[4],
...
static ssize_t mxt_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	...
	for (i = 0; i < priv->nslots; i++) {
		...
		if (sample->contact == CONTACT_LOST || sample->contact == CONTACT_NEW ||
			sample->contact == CONTACT_MOVE) {
		...
		for (i = 0, j = 0; i < priv->nslots && j < ncontacts; i++) {
			...
			if (sample->contact == CONTACT_LOST || sample->contact == CONTACT_NEW ||
				...
				if (sample->contact == CONTACT_LOST) {
					...
					if (sample->valid) {
						point->flags =
							TOUCH_UP | TOUCH_ID_VALID | TOUCH_POS_VALID | TOUCH_PRESSURE_VALID;
					} else {
...
static int mxt_getinfo(FAR struct mxt_dev_s *priv)
{
	...
	ret = mxt_getreg(priv, MXT_INFO, (FAR uint8_t *)&priv->info, sizeof(struct mxt_info_s));
	if (ret < 0) {
...
static int mxt_getobjtab(FAR struct mxt_dev_s *priv)
{
	...
	ret = mxt_getreg(priv, MXT_OBJECT_START, (FAR uint8_t *)priv->objtab, tabsize);
	if (ret < 0) {
	...
	for (i = 0, id = 1; i < priv->info.nobjects; i++) {
		...
		iinfo("%2d. type %2d addr %04x size: %d instances: %d IDs: %u-%u\n", i, object->type,
			  MXT_GETUINT16(object->addr), object->size + 1, object->ninstances + 1, idmin, idmax);
...
static int mxt_hwinitialize(FAR struct mxt_dev_s *priv)
{
	...
	iinfo("Family: %u variant: %u version: %u.%u.%02x\n", info->family, info->variant,
		  info->version >> 4, info->version & 0x0f, info->build);
	...
	priv->sample = (FAR struct mxt_sample_s *)kmm_zalloc(nslots * sizeof(struct mxt_sample_s));
	if (priv->sample == NULL) {
		ierr("ERROR: Failed to allocate object table\n");
		ret = -ENOMEM;
		goto errout_with_objtab;
...
int mxt_register(FAR struct i2c_master_s *i2c, FAR const struct mxt_lower_s *const lower, int minor)
{
	FAR struct mxt_dev_s *priv;
