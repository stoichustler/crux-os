...
static int stmpe811_sample(FAR struct stmpe811_dev_s *priv, FAR struct stmpe811_sample_s *sample);
...
static int stmpe811_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static ssize_t stmpe811_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	...
	if (sample.contact == CONTACT_UP) {
		...
		report->point[0].flags =
			TOUCH_DOWN | TOUCH_ID_VALID | TOUCH_POS_VALID | TOUCH_PRESSURE_VALID;
	} else /* if (sample->contact == CONTACT_MOVE) */
	{
		/* Movement of the same contact */

		report->point[0].flags =
			TOUCH_MOVE | TOUCH_ID_VALID | TOUCH_POS_VALID | TOUCH_PRESSURE_VALID;
	}
...
static void stmpe811_timeout(wdparm_t arg)
{
	...
	if (priv->sample.contact == CONTACT_DOWN || priv->sample.contact == CONTACT_MOVE) {
...
static inline void stmpe811_tscinitialize(FAR struct stmpe811_dev_s *priv)
{
	...
	stmpe811_putreg8(priv, STMPE811_TSC_CFG,
					 TSC_CFG_AVE_CTRL_4SAMPLES | TSC_CFG_TOUCH_DELAY_500US |
						 TSC_CFG_SETTLING_500US);
...
void stmpe811_tscworker(FAR struct stmpe811_dev_s *priv, uint8_t intsta)
{
	...
	if (!pendown) {
		...
		if (priv->sample.contact == CONTACT_NONE || priv->sample.contact == CONTACT_UP) {
			goto ignored;
		}
...
ignored:
	nxmutex_unlock(&priv->lock);
	if (priv->sample.contact == CONTACT_DOWN || priv->sample.contact == CONTACT_MOVE) {
		wd_start(&priv->wdog, STMPE811_PENUP_TICKS, stmpe811_timeout, (wdparm_t)priv);
	}
