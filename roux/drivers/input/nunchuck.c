...
static ssize_t nunchuck_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static int nunchuck_ioctl(FAR struct file *filep, int cmd, unsigned long arg);

/* I2C Helpers */

static int nunchuck_i2c_read(FAR struct nunchuck_dev_s *priv, FAR uint8_t *regval, int len);
static int nunchuck_i2c_write(FAR struct nunchuck_dev_s *priv, uint8_t const *data, int len);
static int nunchuck_sample(FAR struct nunchuck_dev_s *priv, FAR struct nunchuck_sample_s *buffer);
...
static int nunchuck_close(FAR struct file *filep)
{
	...
	for (prev = NULL, curr = priv->nck_open; curr && curr != opriv;
...
static ssize_t nunchuck_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	FAR struct inode *inode;
	FAR struct nunchuck_dev_s *priv;
	int ret;
...
int nunchuck_register(FAR const char *devname, FAR struct i2c_master_s *i2c)
{
	...
	priv = (FAR struct nunchuck_dev_s *)kmm_zalloc(sizeof(struct nunchuck_dev_s));
