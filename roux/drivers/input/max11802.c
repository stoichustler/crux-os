...
static int max11802_sample(FAR struct max11802_dev_s *priv, FAR struct max11802_sample_s *sample);
...
static int max11802_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static void max11802_worker(FAR void *arg)
{
	 ...
	 */
		...
		wd_start(&priv->wdog, MAX11802_WDOG_DELAY, max11802_wdog, (wdparm_t)priv);
		goto ignored;
	} else {
		/* Wait for data ready
		 *
...
static int max11802_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	priv = &g_max11802;
#else
	for (priv = g_max11802list; priv && priv->configs->irq != irq; priv = priv->flink)
		;
...
int max11802_register(FAR struct spi_dev_s *spi, FAR struct max11802_config_s *config, int minor)
{
	FAR struct max11802_dev_s *priv;
