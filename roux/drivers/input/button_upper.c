...
static void btn_interrupt(FAR const struct btn_lowerhalf_s *lower, FAR void *arg);
...
static ssize_t btn_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t btn_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int btn_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int btn_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static void btn_enable(FAR struct btn_upperhalf_s *priv)
{
	...
	if (press != 0 || release != 0) {
		...
		lower->bl_enable(lower, press, release, (btn_handler_t)btn_interrupt, priv);
	} else {
...
#if CONFIG_INPUT_BUTTONS_DEBOUNCE_DELAY
	wd_start(&priv->bu_wdog, MSEC2TICK(CONFIG_INPUT_BUTTONS_DEBOUNCE_DELAY), btn_sample,
			 (wdparm_t)priv);
#else
...
static void btn_sample(wdparm_t arg)
{
	...
	for (opriv = priv->bu_open; opriv; opriv = opriv->bo_flink) {
		...
		if ((press & opriv->bo_pollevents.bp_press) != 0 ||
			...
			poll_notify(opriv->bo_fds, CONFIG_INPUT_BUTTONS_NPOLLWAITERS, POLLIN);
		}
...
static int btn_close(FAR struct file *filep)
{
	...
	for (prev = NULL, curr = priv->bu_open; curr && curr != opriv;
...
static ssize_t btn_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	FAR struct inode *inode;
	FAR struct btn_open_s *opriv;
	FAR struct btn_upperhalf_s *priv;
...
static int btn_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case BTNIOC_POLLEVENTS: {
		FAR struct btn_pollevents_s *pollevents = (FAR struct btn_pollevents_s *)((uintptr_t)arg);
	...
	case BTNIOC_REGISTER: {
		FAR struct btn_notify_s *notify = (FAR struct btn_notify_s *)((uintptr_t)arg);
...
int btn_register(FAR const char *devname, FAR const struct btn_lowerhalf_s *lower)
{
	FAR struct btn_upperhalf_s *priv;
	int ret;
	...
	priv = (FAR struct btn_upperhalf_s *)kmm_zalloc(sizeof(struct btn_upperhalf_s));
	if (!priv) {
