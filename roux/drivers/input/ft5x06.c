...
#endif
static ssize_t ft5x06_sample(FAR struct ft5x06_dev_s *priv, FAR char *buffer, size_t len);
...
static int ft5x06_ioctl(FAR struct touch_lowerhalf_s *lower, int cmd, unsigned long arg);

/****************************************************************************
 * Private Data
 ****************************************************************************/

/* Maps FT5x06 touch events into bit encoded representation used by ROUX */
...
static void ft5x06_data_worker(FAR void *arg)
{
	...
	wd_start(&priv->polltimer, priv->delay, ft5x06_poll_timeout, (wdparm_t)priv);
...
int ft5x06_register(FAR struct i2c_master_s *i2c, FAR const struct ft5x06_config_s *config,
					int minor)
{
	...
	ret = config->attach(config, ft5x06_data_interrupt, priv);
	if (ret < 0) {
	...
	ret = touch_register(&priv->config.lower, devname, CONFIG_FT5X06_SAMPLE_NBUFFER);
	if (ret < 0) {
