...
/* Reference:
 * "ROUX RTOS for PinePhone: Touch Panel"
...
static int gt9xx_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int gt9xx_i2c_read(FAR struct gt9xx_dev_s *dev, uint16_t reg, uint8_t *buf, size_t buflen)
{
	int ret;
	...
	struct i2c_msg_s msgv[2] = {{/* Send the I2C Register Address */
...
static int gt9xx_i2c_write(FAR struct gt9xx_dev_s *dev, uint16_t reg, uint8_t val)
{
	int ret;
...
static int gt9xx_read_touch_data(FAR struct gt9xx_dev_s *dev, FAR struct touch_sample_s *sample)
{
	uint8_t status[1];
	uint8_t status_code;
...
static ssize_t gt9xx_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (priv->flags & TOUCH_DOWN) {
		...
		if (sample.npoints >= 1 && priv->x == sample.point[0].x && priv->y == sample.point[0].y) {
			memset(&sample, 0, sizeof(sample));
			sample.npoints = 0;
...
int gt9xx_register(FAR const char *devpath, FAR struct i2c_master_s *i2c_dev, uint8_t i2c_devaddr,
