...
static ssize_t touch_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	FAR struct touch_openpriv_s *openpriv = filep->f_priv;
	int ret;
							   ...
							   FAR struct touch_sample_s *sample)
{
	...
	circbuf_overwrite(&openpriv->circbuf, sample, SIZEOF_TOUCH_SAMPLE_S(sample->npoints));
