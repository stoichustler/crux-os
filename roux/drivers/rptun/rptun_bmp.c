...
#include <roux/rptun/rptun_bmp.h>
...
static struct FAR rptun_rsc_s *rptun_bmp_get_resource(FAR struct rptun_dev_s *dev);
...
static int rptun_bmp_notify(FAR struct rptun_dev_s *dev, uint32_t notifyid);
static int rptun_bmp_register_callback(FAR struct rptun_dev_s *dev, rptun_callback_t callback,
									   FAR void *arg);
...
static FAR struct rptun_rsc_s *rptun_bmp_get_resource(FAR struct rptun_dev_s *dev)
{
	FAR struct rptun_bmp_dev_s *priv = (FAR struct rptun_bmp_dev_s *)dev;
	return priv->rsc;
}
...
int rptun_bmp_init(FAR const char *cpuname, bool master, FAR struct rptun_rsc_s *rsc, int irq_event,
				   int irq_trigger, cpu_set_t remote_cpu)
{
	...
	ret = irq_attach(dev->irq_event, rprun_bmp_interrupt, dev);
	if (ret < 0) {
		kmm_free(dev);
		return ret;
	}
