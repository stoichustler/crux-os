...
#include <roux/rptun/rptun_ivshmem.h>
#include <roux/wdog.h>
...
static const FAR struct rptun_addrenv_s *rptun_ivshmem_get_addrenv(FAR struct rptun_dev_s *dev);
static FAR struct rptun_rsc_s *rptun_ivshmem_get_resource(FAR struct rptun_dev_s *dev);
...
static int rptun_ivshmem_notify(FAR struct rptun_dev_s *dev, uint32_t notifyid);
static int rptun_ivshmem_register_callback(FAR struct rptun_dev_s *dev, rptun_callback_t callback,
										   FAR void *arg);
...
{
	FAR struct rptun_ivshmem_dev_s *priv = (FAR struct rptun_ivshmem_dev_s *)dev;
	return priv->cpuname;
}
...
static FAR struct rptun_rsc_s *rptun_ivshmem_get_resource(FAR struct rptun_dev_s *dev)
{
	...
	if (priv->master) {
		...
		rsc->rpmsg_vdev.dfeatures =
			1 << VIRTIO_RPMSG_F_NS | 1 << VIRTIO_RPMSG_F_ACK | 1 << VIRTIO_RPMSG_F_BUFSZ;
...
static void rptun_ivshmem_wdog(wdparm_t arg)
{
	FAR struct rptun_ivshmem_dev_s *priv = (FAR struct rptun_ivshmem_dev_s *)arg;
	...
	wd_start(&priv->wdog, RPTUN_IVSHMEM_WDOG_DELAY, rptun_ivshmem_wdog, (wdparm_t)priv);
}
