...
static ssize_t loop_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t loop_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int loop_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case LOOPIOC_SETUP: {
		...
		if (setup == NULL) {
			...
			ret = losetup(setup->devname, setup->filename, setup->sectsize, setup->offset,
						  setup->readonly);
		}
	} break;
