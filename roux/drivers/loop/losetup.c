...
static ssize_t loop_read(FAR struct inode *inode, FAR unsigned char *buffer, blkcnt_t start_sector,
						 unsigned int nsectors);
static ssize_t loop_write(FAR struct inode *inode, FAR const unsigned char *buffer,
						 ...
						 unsigned int nsectors)
{
	...
	do {
		nbytesread = file_read(&dev->devfile, buffer, nsectors * dev->sectsize);
						  ...
						  blkcnt_t start_sector, unsigned int nsectors)
{
	...
	do {
		nbyteswritten = file_write(&dev->devfile, buffer, nsectors * dev->sectsize);
...
int losetup(FAR const char *devname, FAR const char *filename, uint16_t sectsize, off_t offset,
			bool readonly)
{
	FAR struct loop_struct_s *dev;
	struct stat sb;
