...
static int thermal_procfs_open(FAR struct file *filep, FAR const char *relpath, int oflags,
							   mode_t mode);
static int thermal_procfs_close(FAR struct file *filep);
static ssize_t thermal_procfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t thermal_procfs_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int thermal_procfs_dup(FAR const struct file *oldp, FAR struct file *newp);
static int thermal_procfs_opendir(FAR const char *relpath, FAR struct fs_dirent_s **dir);
static int thermal_procfs_closedir(FAR struct fs_dirent_s *dir);
static int thermal_procfs_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry);
static int thermal_procfs_rewinddir(FAR struct fs_dirent_s *dir);
static int thermal_procfs_stat(FAR const char *relpath, FAR struct stat *buf);

/****************************************************************************
 * Private Data
 ****************************************************************************/

static struct list_node g_thermal_procfs_list = LIST_INITIAL_VALUE(g_thermal_procfs_list);
							   ...
							   mode_t mode)
{
	...
	list_for_every_entry(&g_thermal_procfs_list, child, struct thermal_procfs_s, node)
	{
...
static ssize_t thermal_procfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	list_for_every_entry(&p->zdev->instance_list, ins, struct thermal_instance_s, zdev_node)
	{
		ins->cdev->ops->get_state(ins->cdev, &current);
		procfs_sprintf(buffer, buflen, &offset, "z:%s t:%d t:%d h:%u l:%u c:%s ", ins->zdev->name,
					   ins->zdev->temperature, ins->trip, ins->upper, ins->lower, ins->cdev->name);

		if (ins->target == THERMAL_NO_TARGET) {
			procfs_sprintf(buffer, buflen, &offset, "s:%u|%s", current, "(invalid)");
		} else {
			procfs_sprintf(buffer, buflen, &offset, "s:%u|%u", current, ins->target);
		}
...
void thermal_zone_procfs_unregister(FAR struct thermal_zone_device_s *zdev)
{
	...
	list_for_every_entry(&g_thermal_procfs_list, p, FAR struct thermal_procfs_s, node)
	{
		if (p->zdev == zdev) {
			list_delete(&p->node);
			kmm_free(p);
