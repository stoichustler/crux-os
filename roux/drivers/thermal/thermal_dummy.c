...
static int dummy_zdev_get_temp(FAR struct thermal_zone_device_s *zdev, FAR int *temp);
static int dummy_zdev_set_trips(FAR struct thermal_zone_device_s *zdev, int low, int high);

/* Cooling Device */

static int dummy_cdev_get_max_state(FAR struct thermal_cooling_device_s *cdev,
									FAR unsigned int *state);
static int dummy_cdev_get_state(FAR struct thermal_cooling_device_s *cdev, FAR unsigned int *state);
static int dummy_cdev_set_state(FAR struct thermal_cooling_device_s *cdev, unsigned int state);
...
static int dummy_cpufreq_target_index(FAR struct cpufreq_policy *driver, unsigned int index);
...
#ifdef CONFIG_THERMAL_DUMMY_CPUFREQ
static FAR const struct cpufreq_frequency_table *
dummy_cpufreq_get_table(FAR struct cpufreq_policy *policy)
{
	FAR struct dummy_cpufreq_driver_s *driver = (FAR struct dummy_cpufreq_driver_s *)policy->driver;

	return driver->table;
}

static int dummy_cpufreq_target_index(FAR struct cpufreq_policy *policy, unsigned int index)
{
	FAR struct dummy_cpufreq_driver_s *driver = (FAR struct dummy_cpufreq_driver_s *)policy->driver;
...
int thermal_dummy_init(void)
{
...
#else
	cdev = thermal_cooling_device_register("passive_dev", &g_dummy_passive, &g_dummy_cooling_ops);
	if (cdev == NULL) {
	...
	cdev = thermal_cooling_device_register("fan0", &g_dummy_fan0_data, &g_dummy_cooling_ops);
	if (cdev == NULL) {
	...
	zdev = thermal_zone_device_register("cpu-thermal", &g_dummy_zone, &g_dummy_zone_ops,
										&g_dummy_params);
	if (zdev == NULL) {
