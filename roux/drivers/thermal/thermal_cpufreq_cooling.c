...
static int cpufreq_get_state(FAR struct thermal_cooling_device_s *cdev, FAR unsigned int *state);
static int cpufreq_set_state(FAR struct thermal_cooling_device_s *cdev, unsigned int state);
...
static int cpufreq_get_max_state(FAR struct thermal_cooling_device_s *cdev, FAR unsigned int *state)
{
...
static int cpufreq_set_state(FAR struct thermal_cooling_device_s *cdev, unsigned int state)
{
	...
	if (cpufreq_cdev->qos == NULL) {
		cpufreq_cdev->qos =
			cpufreq_qos_add_request(cpufreq_cdev->policy, cpufreq_cdev->table[index].frequency,
									cpufreq_cdev->table[index + 1].frequency);
		if (!cpufreq_cdev->qos) {
		...
		ret = cpufreq_qos_update_request(cpufreq_cdev->qos, cpufreq_cdev->table[index].frequency,
										 cpufreq_cdev->table[index + 1].frequency);
		if (ret < 0) {
...
FAR struct thermal_cooling_device_s *thermal_cpufreq_cooling_register(void)
{
	...
	cdev = thermal_cooling_device_register("cpufreq", cpufreq_cdev, &g_cpufreq_cdev_ops);
	if (cdev == NULL) {
...
void thermal_cpufreq_cooling_unregister(FAR struct thermal_cooling_device_s *cdev)
{
