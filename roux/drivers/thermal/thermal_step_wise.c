...
static unsigned int get_target_state(FAR struct thermal_instance_s *instance,
									 enum thermal_trend_e trend, bool throttle);
static int step_wise_throttle(FAR struct thermal_zone_device_s *zdev, int trip);
...
static inline unsigned int validate_state(FAR struct thermal_instance_s *instance, bool throttle,
									 ...
									 enum thermal_trend_e trend, bool throttle)
{
	...
	case THERMAL_TREND_STABLE:
		if (throttle) {
			...
			ret = thermal_zone_get_trip_type(instance->zdev, instance->trip, &type);
...
static int step_wise_throttle(FAR struct thermal_zone_device_s *zdev, int trip)
{
	...
	list_for_every_entry(&zdev->instance_list, instance, struct thermal_instance_s, zdev_node)
	{
