...
static int zone_bind_cooling(FAR struct thermal_zone_device_s *zdev, int trip,
							 FAR struct thermal_cooling_device_s *cdev, unsigned int upper,
							 unsigned int lower, unsigned int weight);
static void zone_unbind_cooling(FAR struct thermal_zone_device_s *zdev, int trip,
								FAR struct thermal_cooling_device_s *cdev);

static FAR struct thermal_governor_s *find_governor_by_name(FAR const char *name);
...
#ifdef CONFIG_PM
static void thermal_pm_notify(FAR struct pm_callback_s *cb, int domain, enum pm_state_e pmstate);
#endif
...
static struct list_node g_cooling_dev_list = LIST_INITIAL_VALUE(g_cooling_dev_list);

static struct list_node g_governor_list = LIST_INITIAL_VALUE(g_governor_list);

static struct list_node g_zone_dev_list = LIST_INITIAL_VALUE(g_zone_dev_list);
							 ...
							 unsigned int lower, unsigned int weight)
{
	...
	list_for_every_entry(&zdev->instance_list, pos, struct thermal_instance_s, zdev_node)
	{
		if (zdev == pos->zdev && cdev == pos->cdev && trip == pos->trip) {
			thwarn("Instance %s %s %d already exist!", zdev->name, cdev->name, trip);
			return -EEXIST;
		}
	}
								...
								FAR struct thermal_cooling_device_s *cdev)
{
	...
	list_for_every_entry(&zdev->instance_list, ins, struct thermal_instance_s, zdev_node)
	{
						...
						FAR struct thermal_cooling_device_s *cdev)
{
	...
	for (i = 0; i < zdev->params->num_maps; i++) {
		...
		for (j = 0; j < zdev->params->num_trips; j++) {
			...
			ret = zone_bind_cooling(zdev, j, cdev, map->high, map->low, map->weight);
			if (ret < 0) {
...
static FAR struct thermal_governor_s *find_governor_by_name(FAR const char *name)
{
	...
	list_for_every_entry(&g_governor_list, gov, struct thermal_governor_s, node)
	{
...
static void thermal_pm_notify(FAR struct pm_callback_s *cb, int domain, enum pm_state_e pmstate)
{
	...
	switch (pmstate) {
	case PM_SLEEP: {
		list_for_every_entry(&g_zone_dev_list, zdev, struct thermal_zone_device_s, node)
		{
	...
	case PM_STANDBY: {
		list_for_every_entry(&g_zone_dev_list, zdev, struct thermal_zone_device_s, node)
		{
			if (zdev->enabled && work_available(&zdev->monitor)) {
				work_queue(LPWORK, &zdev->monitor, (worker_t)thermal_zone_device_update, zdev,
...
int thermal_zone_enable(FAR struct thermal_zone_device_s *zdev, bool enabled)
{
...
int thermal_zone_get_trip_temp(FAR struct thermal_zone_device_s *zdev, int trip, FAR int *temp)
{
...
int thermal_zone_get_trip_type(FAR struct thermal_zone_device_s *zdev, int trip,
...
int thermal_zone_get_trip_hyst(FAR struct thermal_zone_device_s *zdev, int trip, FAR int *hyst)
{
...
int thermal_register_governor(FAR struct thermal_governor_s *gov)
{
	...
	list_for_every_entry(&g_governor_list, pos, struct thermal_governor_s, node)
	{
								...
								FAR const struct thermal_cooling_device_ops_s *ops)
{
	...
	list_for_every_entry(&g_cooling_dev_list, cdev, struct thermal_cooling_device_s, node)
	{
...
void thermal_cooling_device_unregister(FAR struct thermal_cooling_device_s *cdev)
{
	FAR struct thermal_zone_device_s *zdev;
...
void thermal_cooling_device_update(FAR struct thermal_cooling_device_s *cdev)
{
	...
	list_for_every_entry(&cdev->instance_list, instance, struct thermal_instance_s, cdev_node)
	{
		if ((instance->target != THERMAL_NO_TARGET) &&
			(instance->target > target || target == THERMAL_NO_TARGET)) {
							 ...
							 FAR const struct thermal_zone_params_s *params)
{
	...
	list_for_every_entry(&g_zone_dev_list, pos, struct thermal_zone_device_s, node)
	{
...
delayed_work:
	...
	work_queue(LPWORK, &zdev->monitor, (worker_t)thermal_zone_device_update, zdev, delay);
