...
static void wm8776_writereg(FAR struct wm8776_dev_s *priv, uint8_t regaddr, uint16_t regval);
static int wm8776_getcaps(FAR struct audio_lowerhalf_s *dev, int type,
						  FAR struct audio_caps_s *caps);
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int wm8776_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
							FAR const struct audio_caps_s *caps);
#else
static int wm8776_configure(FAR struct audio_lowerhalf_s *dev, FAR const struct audio_caps_s *caps);
#endif
static int wm8776_shutdown(FAR struct audio_lowerhalf_s *dev);
static void wm8776_senddone(FAR struct i2s_dev_s *i2s, FAR struct ap_buffer_s *apb, FAR void *arg,
							int result);
...
#endif
#endif
static int wm8776_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int wm8776_cancelbuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int wm8776_ioctl(FAR struct audio_lowerhalf_s *dev, int cmd, unsigned long arg);
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int wm8776_reserve(FAR struct audio_lowerhalf_s *dev, FAR void **session);
#else
static int wm8776_reserve(FAR struct audio_lowerhalf_s *dev);
#endif
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int wm8776_release(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
...
static void wm8776_setvolume(FAR struct wm8776_dev_s *priv, uint16_t volume, bool mute)
{
	uint16_t regval;
	uint16_t tmp_vol;
							...
							int result)
{
	...
	ret = file_mq_send(&priv->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_WM8776_MSG_PRIO);
	if (ret < 0) {
...
static int wm8776_sendbuffer(FAR struct wm8776_dev_s *priv)
{
	...
	while (priv->inflight < CONFIG_WM8776_INFLIGHT && dq_peek(&priv->pendq) != NULL &&
		   !priv->paused) {
...
#endif
{
	...
	ret = pthread_create(&priv->threadid, &tattr, wm8776_workerthread, (pthread_addr_t)priv);
	if (ret != OK) {
...
#endif
{
	...
	term_msg.u.data = 0;
	file_mq_send(&priv->mq, (FAR const char *)&term_msg, sizeof(term_msg), CONFIG_WM8776_MSG_PRIO);
...
FAR struct audio_lowerhalf_s *wm8776_initialize(FAR struct i2c_master_s *i2c,
