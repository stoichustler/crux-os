...
static
	...
	uint8_t
	es8311_readreg(FAR struct es8311_dev_s *priv, uint8_t regaddr);
static int es8311_writereg(FAR struct es8311_dev_s *priv, uint8_t regaddr, uint16_t regval);
#ifndef CONFIG_AUDIO_EXCLUDE_VOLUME
static int es8311_setvolume(FAR struct es8311_dev_s *priv, es_module_e module, uint16_t volume);
#endif /* CONFIG_AUDIO_EXCLUDE_VOLUME */
static int es8311_getcoeff(FAR struct es8311_dev_s *priv, uint32_t samplerate);
#ifndef CONFIG_AUDIO_EXCLUDE_MUTE
static int es8311_setmute(FAR struct es8311_dev_s *priv, es_module_e module, bool enable);
...
static int es8311_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
							FAR const struct audio_caps_s *caps);
#else  /* !CONFIG_AUDIO_MULTI_SESSION */
static int es8311_configure(FAR struct audio_lowerhalf_s *dev, FAR const struct audio_caps_s *caps);
#endif /* !CONFIG_AUDIO_MULTI_SESSION */
static int es8311_shutdown(FAR struct audio_lowerhalf_s *dev);
static void es8311_processdone(FAR struct i2s_dev_s *i2s, FAR struct ap_buffer_s *apb,
...
static int es8311_resume(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else  /* !CONFIG_AUDIO_MULTI_SESSION */
static int es8311_pause(FAR struct audio_lowerhalf_s *dev);
static int es8311_resume(FAR struct audio_lowerhalf_s *dev);
#endif /* !CONFIG_AUDIO_MULTI_SESSION */
#endif /* CONFIG_AUDIO_EXCLUDE_PAUSE_RESUME */
static int es8311_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int es8311_cancelbuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int es8311_ioctl(FAR struct audio_lowerhalf_s *dev, int cmd, unsigned long arg);
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int es8311_reserve(FAR struct audio_lowerhalf_s *dev, FAR void **session);
#else  /* !CONFIG_AUDIO_MULTI_SESSION */
static int es8311_reserve(FAR struct audio_lowerhalf_s *dev);
#endif /* !CONFIG_AUDIO_MULTI_SESSION */
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int es8311_release(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else  /* !CONFIG_AUDIO_MULTI_SESSION */
...
static int es8311_setmicgain(FAR struct es8311_dev_s *priv, uint32_t gain)
{
	...
	static const es8311_mic_gain_e gain_map[] = {
		ES8311_MIC_GAIN_0DB,  ES8311_MIC_GAIN_6DB,	ES8311_MIC_GAIN_12DB, ES8311_MIC_GAIN_18DB,
		ES8311_MIC_GAIN_24DB, ES8311_MIC_GAIN_30DB, ES8311_MIC_GAIN_36DB, ES8311_MIC_GAIN_42DB};
...
static int es8311_getcoeff(FAR struct es8311_dev_s *priv, uint32_t samplerate)
{
	...
	for (i = 0; i < nitems(es8311_coeff_div); i++) {
		if (priv->mclk == es8311_coeff_div[i].mclk && samplerate == es8311_coeff_div[i].rate) {
...
static int es8311_setmute(FAR struct es8311_dev_s *priv, es_module_e module, bool enable)
{
	...
	if (module == ES_MODULE_ADC || module == ES_MODULE_ADC_DAC) {
		...
		ret |= es8311_writereg(priv, ES8311_SDPOUT_REG0A, reg | ((int)enable << 6));
	}
...
static int es8311_setbitspersample(FAR struct es8311_dev_s *priv)
{
	...
	if (priv->audio_mode == ES_MODULE_ADC || priv->audio_mode == ES_MODULE_ADC_DAC) {
		...
		ret |= es8311_writereg(priv, ES8311_SDPOUT_REG0A, reg | (bit_config << 2));
	}

	if (priv->audio_mode == ES_MODULE_DAC || priv->audio_mode == ES_MODULE_ADC_DAC) {
		reg = es8311_readreg(priv, ES8311_SDPIN_REG09) & 0xe3;
		ret |= es8311_writereg(priv, ES8311_SDPIN_REG09, reg | (bit_config << 2));
	}
							   ...
							   FAR void *arg, int result)
{
	...
	ret = file_mq_send(&priv->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_ES8311_MSG_PRIO);
	if (ret < 0) {
...
static int es8311_processbegin(FAR struct es8311_dev_s *priv)
{
	...
	while (priv->inflight < CONFIG_ES8311_INFLIGHT && dq_peek(&priv->pendq) != NULL &&
		   !priv->paused) {
		...
		if (priv->audio_mode == ES_MODULE_DAC) {
			ret = I2S_SEND(priv->i2s, apb, es8311_processdone, priv, timeout);
		} else {
			ret = I2S_RECEIVE(priv->i2s, apb, es8311_processdone, priv, timeout);
		}
...
#endif /* !CONFIG_AUDIO_MULTI_SESSION */
{
	...
	ret = pthread_create(&priv->threadid, &tattr, es8311_workerthread, (pthread_addr_t)priv);
	if (ret != OK) {
...
#endif /* !CONFIG_AUDIO_MULTI_SESSION */
{
	...
	term_msg.u.data = 0;
	file_mq_send(&priv->mq, (FAR const char *)&term_msg, sizeof(term_msg), CONFIG_ES8311_MSG_PRIO);
...
FAR struct audio_lowerhalf_s *es8311_initialize(FAR struct i2c_master_s *i2c,
