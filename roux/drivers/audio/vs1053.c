...
static int vs1053_configure(FAR struct audio_lowerhalf_s *lower, FAR void *session,
...
static int vs1053_reserve(FAR struct audio_lowerhalf_s *lower, FAR void **session);
static int vs1053_release(FAR struct audio_lowerhalf_s *lower, FAR void *session);
#else
...
static int vs1053_enqueuebuffer(FAR struct audio_lowerhalf_s *lower, FAR struct ap_buffer_s *apb);
static int vs1053_cancelbuffer(FAR struct audio_lowerhalf_s *lower, FAR struct ap_buffer_s *apb);
static int vs1053_ioctl(FAR struct audio_lowerhalf_s *lower, int cmd, unsigned long arg);
...
static void vs1053_setbass(FAR struct vs1053_struct_s *dev)
{
	...
	vs1053_writereg(dev, VS1053_SCI_BASS,
					(treble_boost << 12) | (treble_range << 8) | (bass_boost << 4) | bass_range);
	vs1053_spi_unlock(spi);
}
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
					dev->lower.upper(dev->lower.priv, AUDIO_CALLBACK_IOERR, NULL, ret, NULL);
#else
					dev->lower.upper(dev->lower.priv, AUDIO_CALLBACK_IOERR, NULL, ret);
#endif
...
static int vs1053_dreq_isr(int irq, FAR void *context, FAR void *arg)
{
	...
	if (dev->running) {
		...
		file_mq_send(&dev->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_VS1053_MSG_PRIO);
	} else {
		msg.msg_id = AUDIO_MSG_DATA_REQUEST;
	}
...
#endif
{
	...
	snprintf(dev->mqname, sizeof(dev->mqname), "/tmp/%" PRIXPTR, (uintptr_t)dev);
	attr.mq_maxmsg = 16;
	attr.mq_msgsize = sizeof(struct audio_msg_s);
	attr.mq_curmsgs = 0;
	attr.mq_flags = 0;
	ret = file_mq_open(&dev->mq, dev->mqname, O_RDWR | O_CREAT, 0644, &attr);
	if (ret < 0) {
	...
	ret = pthread_create(&dev->threadid, &tattr, vs1053_workerthread, (pthread_addr_t)dev);
	if (ret != OK) {
...
#endif
{
	...
	term_msg.u.data = 0;
	file_mq_send(&dev->mq, (FAR const char *)&term_msg, sizeof(term_msg), CONFIG_VS1053_MSG_PRIO);
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int vs1053_reserve(FAR struct audio_lowerhalf_s *lower, FAR void **psession)
#else
static int vs1053_reserve(FAR struct audio_lowerhalf_s *lower)
#endif
{
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int vs1053_release(FAR struct audio_lowerhalf_s *lower, FAR void *psession)
#else
static int vs1053_release(FAR struct audio_lowerhalf_s *lower)
#endif
{
