...
#include <roux/audio/audio_fake.h>
...
static int audio_fake_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int audio_fake_start(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
static int audio_fake_start(FAR struct audio_lowerhalf_s *dev);
#endif
#ifndef CONFIG_AUDIO_EXCLUDE_STOP
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int audio_fake_stop(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int audio_fake_pause(FAR struct audio_lowerhalf_s *dev, FAR void *session);
static int audio_fake_resume(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
...
#endif
#endif
static int audio_fake_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int audio_fake_cancelbuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int audio_fake_ioctl(FAR struct audio_lowerhalf_s *dev, int cmd, unsigned long arg);
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int audio_fake_reserve(FAR struct audio_lowerhalf_s *dev, FAR void **session);
#else
static int audio_fake_reserve(FAR struct audio_lowerhalf_s *dev);
#endif
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int audio_fake_release(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
...
static int audio_fake_file_write(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int audio_fake_file_read(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
...
audio_fake_init_device(bool playback, FAR const struct audio_fake_params *dev_params);
 ...
 *
 * Description: Convert the samplerate to ROUX audio samplerate.
 *
 ****************************************************************************/
 ...
 *
 * Description: Convert the format to ROUX audio format.
 *
 ****************************************************************************/
...
static int audio_fake_file_init(FAR struct audio_lowerhalf_s *dev)
{
	...
	snprintf(filename, sizeof(filename), "%s/%s_%d_%d_%d.pcm", CONFIG_FAKE_AUDIO_DATA_PATH,
			 priv->dev_params->dev_name, priv->sample_rate, priv->channels, priv->format);

	if (priv->playback) {
		ret = file_open(&priv->file, filename, O_RDWR | O_CREAT | O_CLOEXEC, 0666);
	} else {
...
static int audio_fake_process_buffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb)
{
	FAR struct audio_fake_s *priv = (FAR struct audio_fake_s *)dev;
	int64_t frame_time;
	int64_t diff_time;
	...
	diff_time =
		(int64_t)tv2.tv_sec * 1000000 + tv2.tv_usec - ((int64_t)tv1.tv_sec * 1000000 + tv1.tv_usec);

	if (diff_time >= frame_time) {
		audwarn("WARN: %s file time %" PRId64 " > frame time %" PRId64 ".\n",
				priv->playback ? "write" : "read", diff_time, frame_time);
		ret = OK;
		goto out;
	}
							  ...
							  FAR struct audio_caps_s *caps)
{
	...
	case AUDIO_TYPE_QUERY:
		...
		caps->ac_channels =
			(priv->dev_params->channels[0] << 4) | (priv->dev_params->channels[1] & 0x0f);
		...
		case AUDIO_TYPE_QUERY:
			...
			caps->ac_controls.b[0] = priv->playback ? AUDIO_TYPE_OUTPUT : AUDIO_TYPE_INPUT;
		...
		case AUDIO_FMT_PCM:
			...
			caps->ac_controls.b[0] = AUDIO_SUBFMT_CONVERT(priv->dev_params->format[0]);
			caps->ac_controls.b[1] = AUDIO_SUBFMT_CONVERT(priv->dev_params->format[1]);
			caps->ac_controls.b[2] = AUDIO_SUBFMT_CONVERT(priv->dev_params->format[2]);
			caps->ac_controls.b[3] = AUDIO_SUBFMT_CONVERT(priv->dev_params->format[3]);
	...
	case AUDIO_TYPE_INPUT:
		...
		case AUDIO_TYPE_QUERY:
			...
			caps->ac_controls.hw[0] = AUDIO_SAMP_RATE_CONVERT(priv->dev_params->samplerate[0]) |
...
#endif
{
	...
	if (priv->mqname[0] == '\0') {
		...
		snprintf(priv->mqname, sizeof(priv->mqname), "/tmp/%" PRIXPTR, (uintptr_t)priv);
	...
	case AUDIO_TYPE_OUTPUT:
	case AUDIO_TYPE_INPUT:
		priv->sample_rate = caps->ac_controls.hw[0] | (caps->ac_controls.b[3] << 16);
		priv->channels = caps->ac_channels;
		priv->format = caps->ac_controls.b[2];

		priv->scaler = caps->ac_channels * caps->ac_controls.hw[0] * caps->ac_controls.b[2] / 8;

		audinfo("Audio type: %s\n",
				(caps->ac_type == AUDIO_TYPE_OUTPUT) ? "AUDIO_TYPE_OUTPUT" : "AUDIO_TYPE_INPUT");
...
#endif
{
	...
	ret = pthread_create(&priv->threadid, &tattr, audio_fake_workerthread, (pthread_addr_t)priv);
	if (ret != OK) {
...
static int audio_fake_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb)
{
	...
	ret = file_mq_send(&priv->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_AUDIO_FAKE_MSG_PRIO);
	if (ret < 0) {
...
static int audio_fake_ioctl(FAR struct audio_lowerhalf_s *dev, int cmd, unsigned long arg)
{
	...
	case AUDIOIOC_GETBUFFERINFO: {
		...
		bufinfo->buffer_size = (priv->scaler * priv->dev_params->period_time) / 1000;
...
int audio_fake_initialize(void)
{
	...
	for (i = 0; i < nitems(g_dev_params); i++) {
		ret = audio_register(g_dev_params[i].dev_name,
							 audio_fake_init_device(g_dev_params[i].playback, &g_dev_params[i]));
		if (ret < 0) {
