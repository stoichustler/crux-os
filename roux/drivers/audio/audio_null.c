...
#include <roux/audio/audio_null.h>
...
static int null_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
...
#endif
static int null_sleep(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
...
static int null_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb)
{
	...
	ret = file_mq_send(&priv->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_AUDIO_NULL_MSG_PRIO);
	if (ret < 0) {
