...
static int cs4344_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
							FAR const struct audio_caps_s *caps);
#else
static int cs4344_configure(FAR struct audio_lowerhalf_s *dev, FAR const struct audio_caps_s *caps);
#endif
static int cs4344_shutdown(FAR struct audio_lowerhalf_s *dev);
static void cs4344_senddone(FAR struct i2s_dev_s *i2s, FAR struct ap_buffer_s *apb, FAR void *arg,
							int result);
static void cs4344_returnbuffers(FAR struct cs4344_dev_s *priv);
static int cs4344_sendbuffer(FAR struct cs4344_dev_s *priv);

#ifdef CONFIG_AUDIO_MULTI_SESSION
static int cs4344_start(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int cs4344_pause(FAR struct audio_lowerhalf_s *dev, FAR void *session);
static int cs4344_resume(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
static int cs4344_pause(FAR struct audio_lowerhalf_s *dev);
static int cs4344_resume(FAR struct audio_lowerhalf_s *dev);
#endif
#endif
static int cs4344_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int cs4344_cancelbuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int cs4344_ioctl(FAR struct audio_lowerhalf_s *dev, int cmd, unsigned long arg);
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int cs4344_reserve(FAR struct audio_lowerhalf_s *dev, FAR void **session);
#else
static int cs4344_reserve(FAR struct audio_lowerhalf_s *dev);
#endif
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int cs4344_release(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
						  ...
						  FAR struct audio_caps_s *caps)
{
	...
	case AUDIO_TYPE_OUTPUT:
		...
		case AUDIO_TYPE_QUERY:
			...
			caps->ac_controls.hw[0] = AUDIO_SAMP_RATE_16K | AUDIO_SAMP_RATE_22K |
							...
							int result)
{
	...
	ret = file_mq_send(&priv->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_CS4344_MSG_PRIO);
	if (ret < 0) {
...
static int cs4344_sendbuffer(FAR struct cs4344_dev_s *priv)
{
	...
	while (priv->inflight < CONFIG_CS4344_INFLIGHT && dq_peek(&priv->pendq) != NULL &&
		   !priv->paused) {
...
#endif
{
	...
	ret = pthread_create(&priv->threadid, &tattr, cs4344_workerthread, (pthread_addr_t)priv);
	if (ret != OK) {
...
#endif
{
	...
	term_msg.u.data = 0;
	file_mq_send(&priv->mq, (FAR const char *)&term_msg, sizeof(term_msg), CONFIG_CS4344_MSG_PRIO);
