...
#include <roux/arch.h>
#include <roux/irq.h>
#include <roux/audio/audio_dma.h>
...
static int audio_dma_getcaps(struct audio_lowerhalf_s *dev, int type, struct audio_caps_s *caps);
static int audio_dma_shutdown(struct audio_lowerhalf_s *dev);
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int audio_dma_configure(struct audio_lowerhalf_s *dev, void *session,
...
#else
static int audio_dma_configure(struct audio_lowerhalf_s *dev, const struct audio_caps_s *caps);
...
#endif
static int audio_dma_allocbuffer(struct audio_lowerhalf_s *dev, struct audio_buf_desc_s *bufdesc);
static int audio_dma_freebuffer(struct audio_lowerhalf_s *dev, struct audio_buf_desc_s *bufdesc);
static int audio_dma_enqueuebuffer(struct audio_lowerhalf_s *dev, struct ap_buffer_s *apb);
static int audio_dma_ioctl(struct audio_lowerhalf_s *dev, int cmd, unsigned long arg);
...
#endif
{
	...
	return DMA_START_CYCLIC(audio_dma->chan, audio_dma_callback, audio_dma, audio_dma->dst_addr,
							audio_dma->src_addr, audio_dma->buffer_num * audio_dma->buffer_size,
...
static int audio_dma_allocbuffer(struct audio_lowerhalf_s *dev, struct audio_buf_desc_s *bufdesc)
{
	...
	if (!audio_dma->alloc_addr) {
		audio_dma->alloc_addr = kumm_memalign(32, audio_dma->buffer_num * audio_dma->buffer_size);
		if (!audio_dma->alloc_addr) {
			return -ENOMEM;
		}
	...
	*bufdesc->u.pbuffer = apb;
	...
	apb->samp = audio_dma->alloc_addr + audio_dma->alloc_index * audio_dma->buffer_size;
...
static void audio_dma_callback(struct dma_chan_s *chan, void *arg, ssize_t len)
{
	...
	if (!audio_dma->playback)
		up_invalidate_dcache((uintptr_t)apb->samp, (uintptr_t)apb->samp + apb->nbytes);
...
struct audio_lowerhalf_s *audio_dma_initialize(struct dma_dev_s *dma_dev, uint8_t chan_num,
											   bool playback, uint8_t fifo_width,
											   uintptr_t fifo_addr)
{
