...
#endif
	uint16_t
	wm8994_readreg(FAR struct wm8994_dev_s *priv, uint16_t regaddr);
static void wm8994_writereg(FAR struct wm8994_dev_s *priv, uint16_t regaddr, uint16_t regval);

#ifndef CONFIG_AUDIO_EXCLUDE_VOLUME
static inline uint16_t wm8994_scalevolume(uint16_t volume, b16_t scale);
static void wm8994_setvolume(FAR struct wm8994_dev_s *priv, uint16_t volume, bool mute);
#endif
...
static int wm8994_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
							FAR const struct audio_caps_s *caps);
#else
static int wm8994_configure(FAR struct audio_lowerhalf_s *dev, FAR const struct audio_caps_s *caps);
#endif
static int wm8994_shutdown(FAR struct audio_lowerhalf_s *dev);
static void wm8994_senddone(FAR struct i2s_dev_s *i2s, FAR struct ap_buffer_s *apb, FAR void *arg,
							int result);
...
#endif
#endif
static int wm8994_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int wm8994_cancelbuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int wm8994_ioctl(FAR struct audio_lowerhalf_s *dev, int cmd, unsigned long arg);
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int wm8994_reserve(FAR struct audio_lowerhalf_s *dev, FAR void **session);
#else
static int wm8994_reserve(FAR struct audio_lowerhalf_s *dev);
#endif
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int wm8994_release(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
...
static int wm8994_interrupt(FAR const struct wm8994_lower_s *lower, FAR void *arg);
#endif
...
static void wm8994_setvolume(FAR struct wm8994_dev_s *priv, uint16_t volume, bool mute)
{
	...
	if (priv->balance <= (b16HALF - 1)) {
		...
		leftlevel = wm8994_scalevolume(2 * volume, b16ONE - (b16_t)priv->balance);
	}
	...
	if (priv->balance >= (b16HALF - 1)) {
		...
		rightlevel = wm8994_scalevolume(2 * volume, (b16_t)priv->balance);
	}
#else
...
static void wm8994_setbitrate(FAR struct wm8994_dev_s *priv)
{
	...
	audinfo("sample rate=%u nchannels=%u bpsamp=%u framelen=%d fout=%lu\n", priv->samprate,
			priv->nchannels, priv->bpsamp, framelen, (unsigned long)fout);
							...
							int result)
{
	...
	ret = file_mq_send(&priv->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_WM8994_MSG_PRIO);
	if (ret < 0) {
...
static int wm8994_sendbuffer(FAR struct wm8994_dev_s *priv)
{
	...
	while (priv->inflight < CONFIG_WM8994_INFLIGHT && dq_peek(&priv->pendq) != NULL &&
		   !priv->paused) {
		...
		timeout =
			MSEC2TICK(((uint32_t)(apb->nbytes - apb->curbyte) << shift) / (uint32_t)priv->samprate);
...
#endif
{
	...
	ret = pthread_create(&priv->threadid, &tattr, wm8994_workerthread, (pthread_addr_t)priv);
	if (ret != OK) {
...
#endif
{
	...
	term_msg.u.data = 0;
	file_mq_send(&priv->mq, (FAR const char *)&term_msg, sizeof(term_msg), CONFIG_WM8994_MSG_PRIO);
...
static void wm8994_audio_output(FAR struct wm8994_dev_s *priv)
{
	...
	regval = WM8994_STARTUP_BIAS_ENA | WM8994_VMID_BUF_ENA | WM8994_VMID_RAMP_SOFT_FAST_START;
	wm8994_writereg(priv, WM8994_ANTI_POP2, regval);

	/* VMID Control */

	regval = WM8994_BIAS_ENA | WM8994_VMID_SEL_2X40K;
	...
	regval = WM8994_AIF1DAC1L_ENA | WM8994_AIF1DAC1R_ENA | WM8994_DAC1L_ENA | WM8994_DAC1R_ENA;
	...
	regval = WM8994_AIF1ADCR_SRC | WM8994_AIF1_FMT_I2S;
	...
	regval = WM8994_SYSDSPCLK_ENA | WM8994_AIF1DSPCLK_ENA;
	...
	if (cold_startup) {
		regval = WM8994_WSEQ_ENA | WM8994_WSEQ_START |
	...
	regval = WM8994_SPKRVOL_ENA | WM8994_SPKLVOL_ENA;
	...
	if (spk_out) {
		...
		regval = WM8994_DAC2L_TO_SPKMIXL | WM8994_DAC2R_TO_SPKMIXR;
		wm8994_writereg(priv, WM8994_SPEAKER_MIXER, regval);

		/* Enable bias generator, Enable VMID, Enable SPKOUTL, Enable SPKOUTR */

		regval = WM8994_SPKOUTR_ENA | WM8994_SPKOUTL_ENA | WM8994_BIAS_ENA | WM8994_VMID_SEL_2X40K;
		wm8994_writereg(priv, WM8994_PM1, regval);
	...
	if (spk_out) {
		regval |= WM8994_SPKOUTR_ENA | WM8994_SPKOUTL_ENA;
	}
	...
	if (hp_out) {
		regval |= WM8994_HPOUT1L_ENA | WM8994_HPOUT1R_ENA;
	}
	...
	regval = WM8994_HPOUT1L_DLY | WM8994_HPOUT1R_DLY;
	...
	regval = WM8994_MIXOUTL_ENA | WM8994_MIXOUTR_ENA;
	if (spk_out) {
		regval |= WM8994_SPKLVOL_ENA | WM8994_SPKRVOL_ENA;
	}
	wm8994_writereg(priv, WM8994_PM3, regval);

	/* Enable DC Servo and trigger start-up mode on left and right channels */

	regval = WM8994_DCS_TRIG_STARTUP_1 | WM8994_DCS_TRIG_STARTUP_0 | WM8994_DCS_ENA_CHAN_1 |
	...
	regval = WM8994_HPOUT1L_RMV_SHORT | WM8994_HPOUT1L_OUTP | WM8994_HPOUT1L_DLY |
...
FAR struct audio_lowerhalf_s *wm8994_initialize(FAR struct i2c_master_s *i2c,
