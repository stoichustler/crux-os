...
#endif
	uint8_t
	cs43l22_readreg(FAR struct cs43l22_dev_s *priv, uint8_t regaddr);
static void cs43l22_writereg(FAR struct cs43l22_dev_s *priv, uint8_t regaddr, uint8_t regval);

#ifndef CONFIG_AUDIO_EXCLUDE_VOLUME
static void cs43l22_setvolume(FAR struct cs43l22_dev_s *priv, uint16_t volume, bool mute);
#endif
...
static int cs43l22_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
...
static void cs43l22_senddone(FAR struct i2s_dev_s *i2s, FAR struct ap_buffer_s *apb, FAR void *arg,
							 int result);
...
#endif
#endif
static int cs43l22_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int cs43l22_cancelbuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int cs43l22_ioctl(FAR struct audio_lowerhalf_s *dev, int cmd, unsigned long arg);
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int cs43l22_reserve(FAR struct audio_lowerhalf_s *dev, FAR void **session);
#else
...
static int cs43l22_interrupt(FAR const struct cs43l22_lower_s *lower, FAR void *arg);
#endif
...
static int cs43l22_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
							 FAR const struct audio_caps_s *caps)
#else
static int cs43l22_configure(FAR struct audio_lowerhalf_s *dev, FAR const struct audio_caps_s *caps)
#endif
{
	...
	case AUDIO_TYPE_FEATURE:
		...
		case AUDIO_FU_BALANCE: {
			...
			if (balance >= 0 && balance <= 1000) {
				...
				cs43l22_setvolume(priv, (179 * priv->volume / 1000) + 76, priv->mute);
			} else {
				ret = -EDOM;
			}
		} break;
							 ...
							 int result)
{
	...
	ret = file_mq_send(&priv->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_CS43L22_MSG_PRIO);
	if (ret < 0) {
...
static int cs43l22_sendbuffer(FAR struct cs43l22_dev_s *priv)
{
	...
	while (priv->inflight < CONFIG_CS43L22_INFLIGHT && dq_peek(&priv->pendq) != NULL &&
		   !priv->paused) {
...
#endif
{
	...
	ret = pthread_create(&priv->threadid, &tattr, cs43l22_workerthread, (pthread_addr_t)priv);
	if (ret != OK) {
...
#endif
{
	...
	term_msg.u.data = 0;
	file_mq_send(&priv->mq, (FAR const char *)&term_msg, sizeof(term_msg), CONFIG_CS43L22_MSG_PRIO);
