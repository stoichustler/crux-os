...
#endif
	uint16_t
	wm8904_readreg(FAR struct wm8904_dev_s *priv, uint8_t regaddr);
...
static void wm8904_setvolume(FAR struct wm8904_dev_s *priv, uint16_t volume, bool mute);
#endif
...
static int wm8904_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
							FAR const struct audio_caps_s *caps);
#else
static int wm8904_configure(FAR struct audio_lowerhalf_s *dev, FAR const struct audio_caps_s *caps);
#endif
static int wm8904_shutdown(FAR struct audio_lowerhalf_s *dev);
static void wm8904_senddone(FAR struct i2s_dev_s *i2s, FAR struct ap_buffer_s *apb, FAR void *arg,
							int result);
...
#endif
#endif
static int wm8904_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int wm8904_cancelbuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int wm8904_ioctl(FAR struct audio_lowerhalf_s *dev, int cmd, unsigned long arg);
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int wm8904_reserve(FAR struct audio_lowerhalf_s *dev, FAR void **session);
#else
static int wm8904_reserve(FAR struct audio_lowerhalf_s *dev);
#endif
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int wm8904_release(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
...
static int wm8904_interrupt(FAR const struct wm8904_lower_s *lower, FAR void *arg);
#endif
...
static void wm8904_setdatawidth(FAR struct wm8904_dev_s *priv)
{
	...
	if (priv->bpsamp == 16) {
		...
		regval =
			(WM8904_AIFADCR_SRC | WM8904_AIFDACR_SRC | WM8904_ADC_COMPMODE | WM8904_DAC_COMPMODE);
		wm8904_writereg(priv, WM8904_AIF0, regval);
	}
}
...
static void wm8904_setbitrate(FAR struct wm8904_dev_s *priv)
{
  ...
  wm8904_writereg(priv, WM8904_CTRLIF_TEST_1, 0);
	...
	audinfo("mclk=%lu fref=%lu fvco=%lu fout=%lu divndx=%u\n", (unsigned long)priv->lower->mclk,
			(unsigned long)fref, (unsigned long)fvco, (unsigned long)fout, divndx);
	...
	regval = WM8904_FLL_OUTDIV(outdiv) | WM8904_FLL_CTRL_RATE(1) | WM8904_FLL_FRATIO(fllndx);
	...
	do {
		nxsig_usleep(5 * 5000);
	} while ((wm8904_readreg(priv, WM8904_INT_STATUS) & WM8904_FLL_LOCK_INT) != 0 || --retries > 0);
							...
							int result)
{
	...
	ret = file_mq_send(&priv->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_WM8904_MSG_PRIO);
	if (ret < 0) {
...
static int wm8904_sendbuffer(FAR struct wm8904_dev_s *priv)
{
	...
	while (priv->inflight < CONFIG_WM8904_INFLIGHT && dq_peek(&priv->pendq) != NULL &&
		   !priv->paused) {
...
#endif
{
	...
	ret = pthread_create(&priv->threadid, &tattr, wm8904_workerthread, (pthread_addr_t)priv);
	if (ret != OK) {
...
#endif
{
	...
	term_msg.u.data = 0;
	file_mq_send(&priv->mq, (FAR const char *)&term_msg, sizeof(term_msg), CONFIG_WM8904_MSG_PRIO);
...
FAR struct audio_lowerhalf_s *wm8904_initialize(FAR struct i2c_master_s *i2c,
