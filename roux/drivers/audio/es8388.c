...
#endif
	uint8_t
	es8388_readreg(FAR struct es8388_dev_s *priv, uint8_t regaddr);
static void es8388_writereg(FAR struct es8388_dev_s *priv, uint8_t regaddr, uint16_t regval);
#ifndef CONFIG_AUDIO_EXCLUDE_VOLUME
static void es8388_setvolume(FAR struct es8388_dev_s *priv, es_module_e module, uint16_t volume);
#endif
static void es8388_setmclkfrequency(FAR struct es8388_dev_s *priv);
#ifndef CONFIG_AUDIO_EXCLUDE_MUTE
static void es8388_setmute(FAR struct es8388_dev_s *priv, es_module_e module, bool enable);
#endif
...
static int es8388_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
							FAR const struct audio_caps_s *caps);
#else
static int es8388_configure(FAR struct audio_lowerhalf_s *dev, FAR const struct audio_caps_s *caps);
#endif
static int es8388_shutdown(FAR struct audio_lowerhalf_s *dev);
static void es8388_processdone(FAR struct i2s_dev_s *i2s, FAR struct ap_buffer_s *apb,
...
static int es8388_resume(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
static int es8388_pause(FAR struct audio_lowerhalf_s *dev);
static int es8388_resume(FAR struct audio_lowerhalf_s *dev);
#endif
#endif
static int es8388_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int es8388_cancelbuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int es8388_ioctl(FAR struct audio_lowerhalf_s *dev, int cmd, unsigned long arg);
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int es8388_reserve(FAR struct audio_lowerhalf_s *dev, FAR void **session);
#else
static int es8388_reserve(FAR struct audio_lowerhalf_s *dev);
#endif
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int es8388_release(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
...
static void es8388_setmicgain(FAR struct es8388_dev_s *priv, uint32_t gain)
{
	...
	es8388_writereg(priv, ES8388_ADCCONTROL1,
					ES8388_MICAMPR(priv->mic_gain) | ES8388_MICAMPL(priv->mic_gain));
}
...
static void es8388_setvolume(FAR struct es8388_dev_s *priv, es_module_e module, uint16_t volume)
{
	...
	if (priv->balance <= AUDIO_BALANCE_CENTER) {
		...
		leftlvl =
			((((AUDIO_BALANCE_RIGHT - priv->balance) * 100) / AUDIO_BALANCE_CENTER) * volume) / 100;
	}
	...
	if (priv->balance >= AUDIO_BALANCE_CENTER) {
		...
		rightlvl = (((priv->balance * 100) / AUDIO_BALANCE_CENTER) * volume) / 100;
	}
	...
	dbleftlvl = (int16_t)(leftlvl ? (20 * log10f((float)rightlvl / AUDIO_VOLUME_MAX)) : -96);
	dbrightlvl = (int16_t)(rightlvl ? (20 * log10f((float)rightlvl / AUDIO_VOLUME_MAX)) : -96);
...
static void es8388_setmute(FAR struct es8388_dev_s *priv, es_module_e module, bool enable)
{
	...
	if (module == ES_MODULE_DAC || module == ES_MODULE_ADC_DAC) {
		reg = es8388_readreg(priv, ES8388_DACCONTROL3) & (~ES8388_DACMUTE_BITMASK);
		es8388_writereg(priv, ES8388_DACCONTROL3, reg | ES8388_DACMUTE(enable));
	}

	if (module == ES_MODULE_ADC || module == ES_MODULE_ADC_DAC) {
		reg = es8388_readreg(priv, ES8388_ADCCONTROL7) & (~ES8388_ADCMUTE_BITMASK);
		es8388_writereg(priv, ES8388_ADCCONTROL7, reg | ES8388_ADCMUTE(enable));
	}
}
#endif
...
static void es8388_setbitspersample(FAR struct es8388_dev_s *priv)
{
	...
	if (priv->audio_mode == ES_MODULE_ADC || priv->audio_mode == ES_MODULE_ADC_DAC) {
		reg = es8388_readreg(priv, ES8388_ADCCONTROL4) & (~ES8388_ADCWL_BITMASK);
		es8388_writereg(priv, ES8388_ADCCONTROL4, reg | ES8388_ADCWL(bit_config));
	}

	if (priv->audio_mode == ES_MODULE_DAC || priv->audio_mode == ES_MODULE_ADC_DAC) {
		reg = es8388_readreg(priv, ES8388_DACCONTROL1) & (~ES8388_DACWL_BITMASK);
		es8388_writereg(priv, ES8388_DACCONTROL1, reg | ES8388_DACWL(bit_config));
	}
						  ...
						  FAR struct audio_caps_s *caps)
{
	...
	case AUDIO_TYPE_QUERY:
		...
		case AUDIO_TYPE_QUERY:
			...
			caps->ac_controls.b[0] = AUDIO_TYPE_INPUT | AUDIO_TYPE_OUTPUT | AUDIO_TYPE_FEATURE;
			break;
	...
	case AUDIO_TYPE_OUTPUT:
		...
		case AUDIO_TYPE_QUERY:
			...
			caps->ac_controls.hw[0] = AUDIO_SAMP_RATE_11K | AUDIO_SAMP_RATE_16K |
	...
	case AUDIO_TYPE_FEATURE:
		...
		if (caps->ac_subtype == AUDIO_FU_UNDEF) {
			...
			caps->ac_controls.b[1] = (AUDIO_FU_BALANCE | AUDIO_FU_INP_GAIN) >> 8;
		}
...
#endif
{
	...
	case AUDIO_TYPE_OUTPUT: {
		...
		if (caps->ac_controls.b[2] != 16 && caps->ac_controls.b[2] != 18 &&
			caps->ac_controls.b[2] != 20 && caps->ac_controls.b[2] != 24 &&
							   ...
							   FAR void *arg, int result)
{
	...
	ret = file_mq_send(&priv->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_ES8388_MSG_PRIO);
	if (ret < 0) {
...
static int es8388_processbegin(FAR struct es8388_dev_s *priv)
{
	...
	while (priv->inflight < CONFIG_ES8388_INFLIGHT && dq_peek(&priv->pendq) != NULL &&
		   !priv->paused) {
		...
		if (priv->audio_mode == ES_MODULE_DAC) {
			ret = I2S_SEND(priv->i2s, apb, es8388_processdone, priv, timeout);
		} else {
			ret = I2S_RECEIVE(priv->i2s, apb, es8388_processdone, priv, timeout);
		}
...
#endif
{
	...
	if (priv->audio_mode == ES_MODULE_LINE) {
		es8388_writereg(priv, ES8388_DACCONTROL16, ES8388_RMIXSEL_RIN2 | ES8388_LMIXSEL_LIN2);

		es8388_writereg(priv, ES8388_DACCONTROL17,
						ES8388_LI2LOVOL(ES8388_MIXER_GAIN_0DB) | ES8388_LI2LO_ENABLE |
		...
		es8388_writereg(priv, ES8388_DACCONTROL21,
						ES8388_DAC_DLL_PWD_NORMAL | ES8388_ADC_DLL_PWD_NORMAL |
							ES8388_MCLK_DIS_NORMAL | ES8388_OFFSET_DIS_DISABLE |
							ES8388_LRCK_SEL_ADC | ES8388_SLRCK_SAME);
	} else {
		es8388_writereg(priv, ES8388_DACCONTROL21,
						ES8388_DAC_DLL_PWD_NORMAL | ES8388_ADC_DLL_PWD_NORMAL |
							ES8388_MCLK_DIS_NORMAL | ES8388_OFFSET_DIS_DISABLE |
							ES8388_LRCK_SEL_DAC | ES8388_SLRCK_SAME);
	}
	...
	if (regval != prev_regval) {
		es8388_writereg(priv, ES8388_CHIPPOWER,
						ES8388_DACVREF_PDN_PWRUP | ES8388_ADCVREF_PDN_PWRUP |
							ES8388_DACDLL_PDN_NORMAL | ES8388_ADCDLL_PDN_NORMAL |
							ES8388_DAC_STM_RST_RESET | ES8388_ADC_STM_RST_RESET |
							ES8388_DAC_DIGPDN_RESET | ES8388_ADC_DIGPDN_RESET);

		es8388_writereg(priv, ES8388_CHIPPOWER,
						ES8388_DACVREF_PDN_PWRUP | ES8388_ADCVREF_PDN_PWRUP |
							ES8388_DACDLL_PDN_NORMAL | ES8388_ADCDLL_PDN_NORMAL |
							ES8388_DAC_STM_RST_NORMAL | ES8388_ADC_STM_RST_NORMAL |
							ES8388_DAC_DIGPDN_NORMAL | ES8388_ADC_DIGPDN_NORMAL);
	}

	if (priv->audio_mode == ES_MODULE_LINE || priv->audio_mode == ES_MODULE_ADC_DAC ||
		priv->audio_mode == ES_MODULE_ADC) {
		es8388_writereg(priv, ES8388_ADCPOWER,
						ES8388_INT1LP_NORMAL | ES8388_FLASHLP_NORMAL | ES8388_PDNADCBIASGEN_NORMAL |
							ES8388_PDNMICB_PWRON | ES8388_PDNADCR_PWRUP | ES8388_PDNADCL_PWRUP |
	...
	if (priv->audio_mode == ES_MODULE_LINE || priv->audio_mode == ES_MODULE_ADC_DAC ||
		...
		es8388_writereg(priv, ES8388_DACPOWER,
						ES8388_ROUT2_ENABLE | ES8388_LOUT2_ENABLE | ES8388_ROUT1_ENABLE |
							ES8388_LOUT1_ENABLE | ES8388_PDNDACR_PWRUP | ES8388_PDNDACL_PWRUP);
	}
	...
	snprintf(priv->mqname, sizeof(priv->mqname), "/regconfig/%" PRIXPTR, (uintptr_t)priv);
	...
	ret = pthread_create(&priv->threadid, &tattr, es8388_workerthread, (pthread_addr_t)priv);
	if (ret != OK) {
...
#endif
{
	...
	if (priv->audio_mode == ES_MODULE_LINE) {
		...
		es8388_writereg(priv, ES8388_DACCONTROL16, ES8388_RMIXSEL_RIN1 | ES8388_LMIXSEL_LIN1);

		es8388_writereg(priv, ES8388_DACCONTROL17,
						ES8388_LI2LOVOL(ES8388_MIXER_GAIN_0DB) | ES8388_LI2LO_DISABLE |
							ES8388_LD2LO_ENABLE);

		es8388_writereg(priv, ES8388_DACCONTROL20,
						ES8388_RI2ROVOL(ES8388_MIXER_GAIN_0DB) | ES8388_RI2RO_DISABLE |
							ES8388_RD2RO_ENABLE);
	...
	if (priv->audio_mode == ES_MODULE_DAC || priv->audio_mode == ES_MODULE_ADC_DAC) {
		es8388_writereg(priv, ES8388_DACPOWER,
						ES8388_ROUT2_DISABLE | ES8388_LOUT2_DISABLE | ES8388_ROUT1_DISABLE |
							ES8388_LOUT1_DISABLE | ES8388_PDNDACR_PWRUP | ES8388_PDNDACL_PWRUP);
	}

	if (priv->audio_mode == ES_MODULE_ADC || priv->audio_mode == ES_MODULE_ADC_DAC) {
		es8388_writereg(priv, ES8388_ADCPOWER,
						ES8388_INT1LP_LP | ES8388_FLASHLP_LP | ES8388_PDNADCBIASGEN_LP |
							ES8388_PDNMICB_PWRDN | ES8388_PDNADCR_PWRDN | ES8388_PDNADCL_PWRDN |
							ES8388_PDNAINR_PWRDN | ES8388_PDNAINL_PWRDN);
	}

	if (priv->audio_mode == ES_MODULE_ADC_DAC) {
		es8388_writereg(priv, ES8388_DACCONTROL21,
						ES8388_DAC_DLL_PWD_PWRDN | ES8388_ADC_DLL_PWD_PWRDN |
							ES8388_MCLK_DIS_DISABLE | ES8388_OFFSET_DIS_DISABLE |
							ES8388_LRCK_SEL_DAC | ES8388_SLRCK_SAME);
	}
...
stop_msg:
	...
	term_msg.u.data = 0;
	file_mq_send(&priv->mq, (FAR const char *)&term_msg, sizeof(term_msg), CONFIG_ES8388_MSG_PRIO);
...
static int es8388_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb)
{
	...
	if (priv->mq.f_inode != NULL) {
		...
		ret = file_mq_send(&priv->mq, (FAR const char *)&term_msg, sizeof(term_msg),
...
static void es8388_reset(FAR struct es8388_dev_s *priv)
{
	...
	es8388_writereg(priv, ES8388_DACCONTROL3,
					ES8388_DACMUTE_MUTED | ES8388_DACLER_NORMAL | ES8388_DACSOFTRAMP_DISABLE |
						ES8388_DACRAMPRATE_4LRCK);

	es8388_writereg(priv, ES8388_CONTROL2,
					ES8388_PDNVREFBUF_NORMAL | ES8388_VREFLO_NORMAL | ES8388_PDNIBIASGEN_NORMAL |
						ES8388_PDNANA_NORMAL | ES8388_LPVREFBUF_LP | ES8388_LPVCMMOD_NORMAL |
						(1 << 6)); /* Default value of undocumented bit */

	es8388_writereg(priv, ES8388_CHIPPOWER,
					ES8388_DACVREF_PDN_PWRUP | ES8388_ADCVREF_PDN_PWRUP | ES8388_DACDLL_PDN_NORMAL |
						ES8388_ADCDLL_PDN_NORMAL | ES8388_DAC_STM_RST_NORMAL |
						ES8388_ADC_STM_RST_NORMAL | ES8388_DAC_DIGPDN_NORMAL |
	...
	es8388_writereg(priv, ES8388_MASTERMODE,
					ES8388_BCLKDIV(ES_MCLK_DIV_AUTO) | ES8388_BCLK_INV_NORMAL |
						ES8388_MCLKDIV2_NODIV | ES8388_MSC_SLAVE);

	es8388_writereg(priv, ES8388_DACPOWER,
					ES8388_ROUT2_DISABLE | ES8388_LOUT2_DISABLE | ES8388_ROUT1_DISABLE |
						ES8388_LOUT1_DISABLE | ES8388_PDNDACR_PWRDN | ES8388_PDNDACL_PWRDN);

	es8388_writereg(priv, ES8388_CONTROL1,
					ES8388_VMIDSEL_500K | ES8388_ENREF_DISABLE | ES8388_SEQEN_DISABLE |
						ES8388_SAMEFS_SAME | ES8388_DACMCLK_ADCMCLK | ES8388_LRCM_ISOLATED |
						ES8388_SCPRESET_NORMAL);

	es8388_writereg(priv, ES8388_DACCONTROL1,
					ES8388_DACFORMAT(ES_I2S_NORMAL) | ES8388_DACWL(ES_WORD_LENGTH_16BITS) |
						ES8388_DACLRP_NORM_2ND | ES8388_DACLRSWAP_NORMAL);

	es8388_writereg(priv, ES8388_DACCONTROL2,
					ES8388_DACFSRATIO(ES_LCLK_DIV_256) | ES8388_DACFSMODE_SINGLE);
	...
	es8388_writereg(priv, ES8388_DACCONTROL21,
					ES8388_DAC_DLL_PWD_NORMAL | ES8388_ADC_DLL_PWD_NORMAL | ES8388_MCLK_DIS_NORMAL |
						ES8388_OFFSET_DIS_DISABLE | ES8388_LRCK_SEL_DAC | ES8388_SLRCK_SAME);

	es8388_writereg(priv, ES8388_DACCONTROL23, ES8388_VROI_1_5K);

	es8388_writereg(priv, ES8388_DACCONTROL24, ES8388_LOUT1VOL(ES8388_DAC_CHVOL_DB(0)));

	es8388_writereg(priv, ES8388_DACCONTROL25, ES8388_ROUT1VOL(ES8388_DAC_CHVOL_DB(0)));

	es8388_writereg(priv, ES8388_DACCONTROL26, ES8388_LOUT2VOL(ES8388_DAC_CHVOL_DB(0)));

	es8388_writereg(priv, ES8388_DACCONTROL27, ES8388_ROUT2VOL(ES8388_DAC_CHVOL_DB(0)));
	...
	if (priv->dac_output == ES8388_DAC_OUTPUT_LINE2) {
		...
		regconfig = ES_DAC_CHANNEL_LOUT1 | ES_DAC_CHANNEL_ROUT1 | ES_DAC_CHANNEL_LOUT2 |
					ES_DAC_CHANNEL_ROUT2;
	}
	...
	es8388_writereg(priv, ES8388_ADCCONTROL4,
					ES8388_ADCFORMAT(ES_I2S_NORMAL) | ES8388_ADCWL(ES_WORD_LENGTH_16BITS) |
						ES8388_ADCLRP_NORM_2ND | ES8388_DATSEL_LL);

	es8388_writereg(priv, ES8388_ADCCONTROL5,
					ES8388_ADCFSRATIO(ES_LCLK_DIV_256) | ES8388_ADCFSMODE_SINGLE);
	...
	es8388_writereg(priv, ES8388_ADCPOWER,
					ES8388_INT1LP_LP | ES8388_FLASHLP_NORMAL | ES8388_PDNADCBIASGEN_NORMAL |
						ES8388_PDNMICB_PWRDN | ES8388_PDNADCR_PWRUP | ES8388_PDNADCL_PWRUP |
						ES8388_PDNAINR_NORMAL | ES8388_PDNAINL_NORMAL);
	...
	if (priv->audio_mode == ES_MODULE_LINE) {
		es8388_writereg(priv, ES8388_DACCONTROL21,
						ES8388_DAC_DLL_PWD_NORMAL | ES8388_ADC_DLL_PWD_NORMAL |
							ES8388_MCLK_DIS_NORMAL | ES8388_OFFSET_DIS_DISABLE | ES8388_SLRCK_SAME |
							ES8388_LRCK_SEL_DAC);

		es8388_writereg(priv, ES8388_DACCONTROL16, ES8388_RMIXSEL_RIN1 | ES8388_LMIXSEL_LIN1);

		es8388_writereg(priv, ES8388_DACCONTROL17,
						ES8388_LD2LO_ENABLE | ES8388_LI2LO_DISABLE |
							ES8388_LI2LOVOL(ES8388_MIXER_GAIN_0DB));

		es8388_writereg(priv, ES8388_DACCONTROL20,
						ES8388_RD2RO_ENABLE | ES8388_RI2RO_DISABLE |
...
FAR struct audio_lowerhalf_s *es8388_initialize(FAR struct i2c_master_s *i2c,
