...
static void getsiminfo_parse_response(FAR struct apicmd_cmddat_getsiminfo_res_s *resp,
...
int32_t altcom_getphone_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
...
int32_t altcom_getimsi_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								   FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_getsiminfo_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
	FAR uint32_t *option = (FAR uint32_t *)arg[0];

	FAR struct apicmd_cmddat_getsiminfo_s *out = (FAR struct apicmd_cmddat_getsiminfo_s *)pktbuf;
...
int32_t altcom_getphone_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								  ...
								  FAR uint64_t *bitmap)
{
	...
	FAR struct apicmd_cmddat_phonenores_s *in = (FAR struct apicmd_cmddat_phonenores_s *)pktbuf;
	...
	*errcause = in->errcause;
	if (0 == *ret) {
		if (arglen > 3) {
			...
			if (*len < strnlen((FAR const char *)in->phoneno, LTE_PHONENO_LEN)) {
				return -ENOBUFS;
			}
		}
...
int32_t altcom_getimsi_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								 uint8_t altver, FAR void **arg, size_t arglen,
								 FAR uint64_t *bitmap)
{
	FAR int *ret = (FAR int *)arg[0];
	...
	FAR struct apicmd_cmddat_getimsires_s *in = (FAR struct apicmd_cmddat_getimsires_s *)pktbuf;
...
int32_t altcom_getsiminfo_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf,
