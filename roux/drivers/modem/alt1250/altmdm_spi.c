...
void altmdm_set_spipkt_rxbuffer(FAR altmdm_spipkt_t *pkt, FAR void *buf, uint16_t sz)
{
	pkt->buffer = buf;
	pkt->buff_size = sz;
}

void altmdm_set_spipkt_txbuffer(FAR altmdm_spipkt_t *pkt, FAR void *buf, uint16_t sz)
{
	uint16_t total_sz;

	if (sz <= ALTSPI_MAX_PKTSIZE) {
		sz &= ALTSPI_PKT_SIZEMASK;
		total_sz = (sz + (ALTSPI_PKT_WORDSIZE - 1)) & ~(ALTSPI_PKT_WORDSIZE - 1);
...
void altmdm_overwrite_body_size(FAR altmdm_spipkt_t *pkt, uint16_t sz)
{
	...
	if (sz <= ALTSPI_MAX_PKTSIZE) {
		...
		total_sz = (sz + (ALTSPI_PKT_WORDSIZE - 1)) & ~(ALTSPI_PKT_WORDSIZE - 1);
		pkt->header =
			(pkt->header & ALTSPI_STATUSMASK) | ((total_sz << ALTSPI_PKT_TOTALSIZE_POS) + sz);
	}
}
...
bool altmdm_is_valid_spipkt_header(FAR altmdm_spipkt_t *pkt)
{
	...
	return (
		(actual_size <= ALTSPI_MAX_PKTSIZE) && (actual_size <= pkt->buff_size) &&
		(((actual_size + (ALTSPI_PKT_WORDSIZE - 1)) & ~(ALTSPI_PKT_WORDSIZE - 1)) == total_size));
}
...
void altmdm_do_body_transaction(FAR struct spi_dev_s *spidev,
								...
								FAR altmdm_spipkt_t *tx_pkt, FAR altmdm_spipkt_t *rx_pkt)
{
	size_t exchange_size;

	/* Larger size is exchange size */

	exchange_size = pkt_total_size(tx_pkt) > pkt_total_size(rx_pkt) ? pkt_total_size(tx_pkt)
																	: pkt_total_size(rx_pkt);

	SPI_EXCHANGE(spidev, tx_pkt->buffer, rx_pkt->buffer, exchange_size);

	m_info("Body TRX done. ex size: %d, rx size: %d/%d, tx size: %d/%d\n", exchange_size,
		   pkt_actual_size(rx_pkt), pkt_total_size(rx_pkt), pkt_actual_size(tx_pkt),
		   pkt_total_size(tx_pkt));
}
