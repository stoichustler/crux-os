...
int32_t altcom_getimei_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
...
int32_t altcom_sendatcmd_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									 FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_errinfo_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								 uint8_t altver, FAR void **arg, size_t arglen,
								 FAR uint64_t *bitmap)
{
	FAR lte_errinfo_t *info = (FAR lte_errinfo_t *)arg[0];
	FAR struct apicmd_cmddat_errinfo_s *in = (FAR struct apicmd_cmddat_errinfo_s *)pktbuf;
...
int32_t altcom_getimei_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								 uint8_t altver, FAR void **arg, size_t arglen,
								 FAR uint64_t *bitmap)
{
	FAR int *ret = (FAR int *)arg[0];
	FAR char *imei = (FAR char *)arg[1];
	FAR struct apicmd_cmddat_getimeires_s *in = (FAR struct apicmd_cmddat_getimeires_s *)pktbuf;
...
int32_t altcom_sendatcmd_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								   uint8_t altver, FAR void **arg, size_t arglen,
								   FAR uint64_t *bitmap)
{
...
int32_t altcom_urc_event_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								   uint8_t altver, FAR void **arg, size_t arglen,
								   FAR uint64_t *bitmap)
{
	int32_t ret = -ENOTSUP;
	...
	if (altver == ALTCOM_VER4) {
		...
		*bitmap = get_event_lapibuffer(dev, lcmdid, &inst);
		...
		if (*bitmap != 0ULL) {
			ret = lwm2m_urc_handler(pktbuf, pktsz, inst->outparam, inst->outparamlen);
		}
	}
#endif
