...
#include <roux/modem/alt1250.h> /* for ALTCOM_VERx */
...
static altmdm_state_t process_state_setsustimersleep(uint32_t, altmdm_state_t);
...
#define TABLE_CONTENT(array_name, namea, nameb, namec)                                             \
	[ALTMDM_STATE_##array_name] = {next_state_##namea, waitevt_state_##nameb, process_state_##namec}
#else
#define TABLE_CONTENT(array_name, namea, nameb, namec)                                             \
	[ALTMDM_STATE_##array_name] = {next_state_##namea, waitevt_state_##nameb,                      \
								   process_state_##namec, #array_name}

static char *g_vp_name[] = {"NO_RESET", "V1      ", "",			"",		   "V4      ",
							"UNKNOWN ", "TRYV1   ", "NOTV1   ", "TRYV4   "};
#endif
...
#ifdef CONFIG_MODEM_ALT1250_DEBUG
static void dump_current_all_status(FAR altmdm_dev_t *dev, uint32_t evt, altmdm_state_t next,
									int is_exit)
{
	m_info("state[%s => %s], vp[%s], event out[%08lx]:cur[%08lx], "
		   "wcount=%2d, txreq_buf=%s, txreq_sz=%4d, "
		   "tx_hdr=%08lx, rx_hdr=%08lx, exit=%s\n",
		   g_state_func[dev->current_state].name, g_state_func[next].name, g_vp_name[dev->vp], evt,
		   dev->event.event, dev->wcounter, dev->txreq_buff == NULL ? "No-req " : "Request",
		   dev->txreq_size, dev->tx_pkt.header, dev->rx_pkt.header, is_exit ? "Yes" : "No ");
}
#else
...
static void txsustimer_handler(int signo, FAR siginfo_t *info, FAR void *uctx)
{
	FAR struct altmdm_dev_s *priv = (FAR struct altmdm_dev_s *)(info->si_value.sival_ptr);
...
static void process_before_poweroff(void)
{
	...
	altmdm_event_clear(&g_altmdm_dev.event, (allevt & ~(EVENT_POWERON | EVENT_DESTROY)));
...
static altmdm_state_t process_state_common(uint32_t event, altmdm_state_t state)
{
	return state;
}
...
static uint32_t waitevt_state_poweroff(void)
{
	return altmdm_event_wait(&g_altmdm_dev.event,
							 EVENT_POWERON | EVENT_DESTROY | EVENT_SUSPEND | EVENT_RESUME, false,
							 0);
}

static altmdm_state_t process_state_poweroff(uint32_t event, altmdm_state_t state)
{
...
static uint32_t waitevt_state_sleep(void)
{
	...
	event = altmdm_event_wait(&g_altmdm_dev.event,
							  EVENT_TXREQ | EVENT_RXREQ | EVENT_WLOCK | EVENT_POWEROFF |
								  EVENT_RESET | EVENT_DESTROY | EVENT_SUSPEND,
							  false, 0);

	return event;
}

static altmdm_state_t process_state_sleep(uint32_t event, altmdm_state_t state)
{
...
static uint32_t waitevt_state_sleepwotx(void)
{
	...
	event = altmdm_event_wait(&g_altmdm_dev.event,
							  EVENT_RXREQ | EVENT_TXSUSTO | EVENT_WLOCK | EVENT_POWEROFF |
								  EVENT_RESET | EVENT_DESTROY,
							  false, 0);

	return event;
}

static altmdm_state_t process_state_sleepwotx(uint32_t event, altmdm_state_t state)
{
	if (event & EVENT_DESTROY) {
		altmdm_event_clear(&g_altmdm_dev.event, EVENT_DESTROY);
		state = ALTMDM_STATE_DESTROY;
...
static altmdm_state_t process_state_idle4rst(uint32_t event, altmdm_state_t state)
{
	enum version_phase_e vp;
	vp = get_vp();
...
static altmdm_state_t process_state_idlewto(uint32_t event, altmdm_state_t state)
{
	/* The order of checking the events is related to processing
	 * priority, so be careful when making changes.
	 */
...
static altmdm_state_t process_state_idlewoto(uint32_t event, altmdm_state_t state)
{
	if (event & EVENT_DESTROY) {
		altmdm_event_clear(&g_altmdm_dev.event, EVENT_DESTROY);
		state = ALTMDM_STATE_DESTROY;
...
static uint32_t waitevt_state_idlewotx(void)
{
	return altmdm_event_wait(
		&g_altmdm_dev.event,
		EVENT_RXREQ | EVENT_TXSUSTO | EVENT_POWEROFF | EVENT_RESET | EVENT_DESTROY, false, 0);
}

static altmdm_state_t process_state_idlewotx(uint32_t event, altmdm_state_t state)
{
	if (event & EVENT_DESTROY) {
		altmdm_event_clear(&g_altmdm_dev.event, EVENT_DESTROY);
		state = ALTMDM_STATE_DESTROY;
...
static altmdm_state_t process_state_v1set(uint32_t event, altmdm_state_t state)
{
	int len;
	FAR void *pkt;
...
static altmdm_state_t process_state_v4set(uint32_t event, altmdm_state_t state)
{
	int len;
	FAR void *pkt;
...
static altmdm_state_t process_state_sleepset(uint32_t event, altmdm_state_t state)
{
...
static altmdm_state_t process_state_txprepare(uint32_t event, altmdm_state_t state)
{
	FAR void *buff;
	int len;
...
static altmdm_state_t process_state_txreq(uint32_t event, altmdm_state_t state)
{
...
static uint32_t waitevt_state_hdrsreq(void)
{
	return altmdm_event_wait(&g_altmdm_dev.event, EVENT_RXREQ | EVENT_POWEROFF | EVENT_DESTROY,
							 false, TIMEOUT_HDR_TRX_STATE);
}

static altmdm_state_t process_state_hdrsreq(uint32_t event, altmdm_state_t state)
{
	if (event & EVENT_DESTROY) {
		altmdm_event_clear(&g_altmdm_dev.event, EVENT_DESTROY);
		state = ALTMDM_STATE_DESTROY;
...
static altmdm_state_t process_state_hdrtrx(uint32_t event, altmdm_state_t state)
{
	altmdm_do_hdr_transaction(g_altmdm_dev.spidev, g_altmdm_dev.lower, &g_altmdm_dev.tx_pkt,
							  &g_altmdm_dev.rx_pkt);
...
static altmdm_state_t process_state_sleeppkt(uint32_t event, altmdm_state_t state)
{
...
static altmdm_state_t process_state_bodysreq(uint32_t event, altmdm_state_t state)
{
	if (event & EVENT_DESTROY) {
		altmdm_event_clear(&g_altmdm_dev.event, EVENT_DESTROY);
		state = ALTMDM_STATE_DESTROY;
...
static altmdm_state_t process_state_bodytrx(uint32_t event, altmdm_state_t state)
{
	altmdm_do_body_transaction(g_altmdm_dev.spidev, g_altmdm_dev.lower, &g_altmdm_dev.tx_pkt,
							   &g_altmdm_dev.rx_pkt);
	...
	} else if (pkt_total_size(&g_altmdm_dev.rx_pkt) != 0 && g_altmdm_dev.retry_mode == 0) {
...
static altmdm_state_t process_state_gotrx(uint32_t event, altmdm_state_t state)
{
	enum version_phase_e vp;
	FAR void *rcv_data;
	...
	if (is_vp_valid()) {
		...
		} else if ((vp == VP_TRYV4) && altcom_is_v4pkt_ok((FAR struct altcom_cmdhdr_s *)rcv_data)) {
			set_vp(VP_V4);
...
static altmdm_state_t process_state_gotrst(uint32_t event, altmdm_state_t state)
{
	if (is_vp_noreset()) {
...
static altmdm_state_t process_state_gotsleep(uint32_t event, altmdm_state_t state)
{
...
static altmdm_state_t process_state_backtoidle(uint32_t event, altmdm_state_t state)
{
	if (is_buffer_full(&g_altmdm_dev.rx_pkt)) {
...
static altmdm_state_t process_state_retrecv(uint32_t event, altmdm_state_t state)
{
...
static altmdm_state_t process_state_forcerst(uint32_t event, altmdm_state_t state)
{
	force_reset();
...
static altmdm_state_t process_state_sleeping(uint32_t event, altmdm_state_t state)
{
	if (is_buffer_full(&g_altmdm_dev.rx_pkt)) {
		if (altmdm_timer_is_running(g_altmdm_dev.txsus_timer)) {
...
static altmdm_state_t process_state_decidedelay(uint32_t event, altmdm_state_t state)
{
	if (has_sendrequest(&g_altmdm_dev.tx_pkt) || is_sleep_pkt(&g_altmdm_dev.tx_pkt)) {
...
static altmdm_state_t process_state_delaynext(uint32_t event, altmdm_state_t state)
{
	state = ALTMDM_STATE_BACKTOIDLE;
...
static altmdm_state_t process_state_setsustimer(uint32_t event, altmdm_state_t state)
{
	altmdm_timer_restart(g_altmdm_dev.txsus_timer, TXSUS_TIMEOUT, 0);
...
static altmdm_state_t process_state_setsustimersleep(uint32_t event, altmdm_state_t state)
{
	altmdm_timer_restart(g_altmdm_dev.txsus_timer, TXSUS_TIMEOUT, 0);
...
int altmdm_init(FAR struct spi_dev_s *spidev, FAR const struct alt1250_lower_s *lower)
{
	...
	g_altmdm_dev.txsus_timer = altmdm_timer_start(0, 0, txsustimer_handler, &g_altmdm_dev);
...
int altmdm_read(FAR uint8_t *buff, int sz)
{
	...
	while (!is_exit) {
		...
		is_exit = g_state_func[next_state].goto_next(g_altmdm_dev.current_state);
...
int altmdm_write(FAR uint8_t *buff, int sz)
{
	...
	do {
		...
		if (should_wait) {
			ret = altmdm_event_wait(&g_altmdm_dev.txdone_event, TX_DONE | TX_CANCEL, true, 0);
			if (ret & TX_CANCEL) {
