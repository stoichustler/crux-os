								  ...
								  FAR lte_version_t *version)
{
	...
	strncpy(version->bb_product, (FAR const char *)resp->bb_product, LTE_VER_BB_PRODUCT_LEN - 1);
	strncpy(version->np_package, (FAR const char *)resp->np_package, LTE_VER_NP_PACKAGE_LEN - 1);
}
...
int32_t altcom_getver_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
...
int32_t altcom_injectimage_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									   ...
									   FAR uint16_t *altcid)
{
	...
	if (altver == ALTCOM_VER1) {
		...
		*altcid = APICMDID_FW_INJECTDELTAIMG;
		...
		len = (len > APICMD_FW_INJECTDATA_MAXLEN) ? APICMD_FW_INJECTDATA_MAXLEN : len;

		out->data_len = htonl(len);
		out->inject_mode = mode ? LTEFW_INJECTION_MODE_NEW : LTEFW_INJECTION_MODE_APPEND;
		...
		*altcid = APICMDID_FW_INJECTDELTAIMG_V4;
		len = (len > APICMD_FW_INJECTDATA_MAXLEN_V4) ? APICMD_FW_INJECTDATA_MAXLEN_V4 : len;
...
int32_t altcom_getimagelen_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									   FAR uint8_t *pktbuf, const size_t pktsz,
									   FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_execupdate_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_getupdateres_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
										FAR uint8_t *pktbuf, const size_t pktsz,
										FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_getver_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								uint8_t altver, FAR void **arg, size_t arglen, FAR uint64_t *bitmap)
{
	...
	FAR struct apicmd_cmddat_getverres_s *in = (FAR struct apicmd_cmddat_getverres_s *)pktbuf;
...
int32_t altcom_fwcommon_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
