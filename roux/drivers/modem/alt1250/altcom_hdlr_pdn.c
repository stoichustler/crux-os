...
static int32_t altcombs_set_pdninfo(struct apicmd_pdnset_s *cmd_pdn, lte_pdn_t *lte_pdn)
{
	int32_t i;
	...
	for (i = 0; i < lte_pdn->ipaddr_num; i++) {
		...
		strncpy(lte_pdn->address[i].address, (FAR char *)cmd_pdn->ip_address[i].address,
				LTE_IPADDR_MAX_LEN - 1);
	}

	lte_pdn->ims_register =
		cmd_pdn->imsregister == APICMD_PDN_IMS_REG ? LTE_IMS_REGISTERED : LTE_IMS_NOT_REGISTERED;
	lte_pdn->data_allow =
		cmd_pdn->dataallow == APICMD_PDN_DATAALLOW_ALLOW ? LTE_DATA_ALLOW : LTE_DATA_DISALLOW;
	lte_pdn->data_roaming_allow = cmd_pdn->dararoamingallow == APICMD_PDN_DATAROAMALLOW_ALLOW
									  ? LTE_DATA_ALLOW
									  : LTE_DATA_DISALLOW;
...
static int32_t altcombs_set_pdninfo_v4(FAR struct apicmd_pdnset_v4_s *cmd_pdn,
									   ...
									   FAR struct sockaddr_storage *dnsaddrs)
{
	...
	if (ndnsaddrs && dnsaddrs) {
		*ndnsaddrs = (cmd_pdn->dnsaddr_num > APICMD_PDN_DNSCOUNT_MAX) ? APICMD_PDN_DNSCOUNT_MAX
																	  : cmd_pdn->dnsaddr_num;
		for (i = 0; (i < APICMD_PDN_DNSCOUNT_MAX) && (i < *ndnsaddrs); i++) {
			af = (cmd_pdn->dns_address[i].iptype == LTE_IPTYPE_V4) ? AF_INET : AF_INET6;
			if (af == AF_INET) {
				FAR struct sockaddr_in *addr = (FAR struct sockaddr_in *)&dnsaddrs[i];

				addr->sin_family = AF_INET;
				addr->sin_port = 0;
				if (1 != inet_pton(af, (FAR const char *)cmd_pdn->dns_address[i].address,
				...
				FAR struct sockaddr_in6 *addr = (FAR struct sockaddr_in6 *)&dnsaddrs[i];
...
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV1
static void getnetinfo_parse_response(FAR struct apicmd_cmddat_getnetinfores_s *resp,
									  FAR lte_netinfo_t *netinfo, uint8_t pdn_num)
{
	...
	netinfo->nw_err.reject_cause.category = resp->netinfo.err_info.reject_cause.category;
	netinfo->nw_err.reject_cause.value = resp->netinfo.err_info.reject_cause.value;
	...
	if ((0 < resp->netinfo.pdn_count) && (netinfo->pdn_stat)) {
		for (i = 0; (i < resp->netinfo.pdn_count) && (i < pdn_num); i++) {
			altcombs_set_pdninfo(&resp->netinfo.pdn[i], &netinfo->pdn_stat[i]);
		}
	}
}
#endif
...
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV4
static void getnetinfo_parse_response_v4(FAR struct apicmd_cmddat_getnetinfores_v4_s *resp,
										 FAR lte_netinfo_t *netinfo, uint8_t pdn_num)
{
	uint8_t i;
	...
	if ((0 < resp->netinfo.pdn_count) && (netinfo->pdn_stat)) {
		for (i = 0; (i < resp->netinfo.pdn_count) && (i < pdn_num); i++) {
			altcombs_set_pdninfo_v4(&resp->netinfo.pdn[i], &netinfo->pdn_stat[i], NULL, NULL);
		}
	}
}
#endif
...
int32_t altcom_actpdn_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
	FAR lte_apn_setting_t *apn = (FAR lte_apn_setting_t *)arg[0];

	FAR struct apicmd_cmddat_activatepdn_s *out = (FAR struct apicmd_cmddat_activatepdn_s *)pktbuf;
	...
	strncpy((FAR char *)out->apnname, (FAR const char *)apn->apn, sizeof(out->apnname));
	if (apn->auth_type != LTE_APN_AUTHTYPE_NONE) {
		strncpy((FAR char *)out->username, (FAR const char *)apn->user_name, sizeof(out->username));
		strncpy((FAR char *)out->password, (FAR const char *)apn->password, sizeof(out->password));
	}
...
int32_t altcom_deactpdn_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_actpdncancel_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
...
int32_t altcom_getnetinfo_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_setrepnet_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									 FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_actpdn_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
...
int32_t altcom_deactpdn_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
...
int32_t altcom_actpdncancel_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf,
...
int32_t altcom_getnetinfo_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf,
									size_t pktsz, uint8_t altver, FAR void **arg, size_t arglen,
									FAR uint64_t *bitmap)
{
	FAR int *ret = (FAR int *)arg[0];
...
int32_t altcom_setrepnet_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								   uint8_t altver, FAR void **arg, size_t arglen,
								   FAR uint64_t *bitmap)
{
	FAR int *ret = (FAR int *)arg[0];
...
int32_t altcom_repnet_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								uint8_t altver, FAR void **arg, size_t arglen, FAR uint64_t *bitmap)
{
	FAR lte_netinfo_t *netinfo = (FAR lte_netinfo_t *)arg[0];
	FAR uint8_t *ndnsaddrs = (FAR uint8_t *)arg[1];
	FAR struct sockaddr_storage *dnsaddrs = (FAR struct sockaddr_storage *)arg[2];
	int i = 0;
	...
	if (altver == ALTCOM_VER1) {
		...
		netinfo->nw_err.reject_cause.category = in->netinfo.err_info.reject_cause.category;
		netinfo->nw_err.reject_cause.value = in->netinfo.err_info.reject_cause.value;
		netinfo->pdn_num = in->netinfo.pdn_count;
		if (0 < in->netinfo.pdn_count) {
			for (i = 0; i < in->netinfo.pdn_count; i++) {
				altcombs_set_pdninfo(&in->netinfo.pdn[i], &netinfo->pdn_stat[i]);
			}
		}
		...
		if (pktsz == (sizeof(struct apicmd_cmddat_rep_netinfo_s))) {
			*ndnsaddrs = 0;
			...
			if (*(FAR uint32_t *)in->dnsaddrv4 != 0) {
				FAR struct sockaddr_in *v4addr = (FAR struct sockaddr_in *)&dnsaddrs[0];

				v4addr->sin_family = AF_INET;
				v4addr->sin_port = 0;
				memcpy(&v4addr->sin_addr, in->dnsaddrv4, sizeof(v4addr->sin_addr));
				(*ndnsaddrs)++;
			}
			...
			if (!((*(FAR uint32_t *)&in->dnsaddrv6[0] == 0) &&
				...
				FAR struct sockaddr_in6 *v6addr = (FAR struct sockaddr_in6 *)&dnsaddrs[*ndnsaddrs];

				v6addr->sin6_family = AF_INET6;
				v6addr->sin6_port = 0;
				memcpy(&v6addr->sin6_addr, in->dnsaddrv6, sizeof(v6addr->sin6_addr));
				(*ndnsaddrs)++;
			}
		}
	} else
		...
		if (0 < in->netinfo.pdn_count) {
			for (i = 0; i < in->netinfo.pdn_count; i++) {
				if (i == 0) {
					altcombs_set_pdninfo_v4(&in->netinfo.pdn[i], &netinfo->pdn_stat[i], ndnsaddrs,
											dnsaddrs);
				} else {
					altcombs_set_pdninfo_v4(&in->netinfo.pdn[i], &netinfo->pdn_stat[i], NULL, NULL);
				}
			}
		}
	} else
