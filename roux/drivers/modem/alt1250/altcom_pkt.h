...
#define ALTCOM_PKT_SIZE_MAX                                                                        \
	(ALTCOM_PAYLOAD_SIZE_MAX + sizeof(struct altcom_cmdhdr_s) + sizeof(struct altcom_cmdfooter_s))
#define ALTCOM_RX_PKT_SIZE_MAX (ALTCOM_PAYLOAD_SIZE_MAX_V4 + sizeof(struct altcom_cmdhdr_s))
...
{
	FAR struct altcom_errind_s *pkt = (FAR struct altcom_errind_s *)&hdr->payload[0];
...
{
	if (ver == ALTCOM_VER1) {
		payloadlen += sizeof(struct altcom_cmdhdr_s) + sizeof(struct altcom_cmdfooter_s);
...
static inline uint16_t convert_cid2v1(uint16_t cid)
{
	...
	if ((cid >= APICMDID_DEACTIVATE_PDN_V4) && (cid <= APICMDID_REPORT_NETINFO_V4)) {
		return (cid - 0x2ed);
	}
	...
	if ((cid >= APICMDID_FW_INJECTDELTAIMG_V4) && (cid <= APICMDID_FW_GETUPDATERESULT_V4)) {
		return (cid + 0xdd0);
	}
...
void altcom_set_header_top(FAR struct altcom_cmdhdr_s *hdr, uint8_t ver, uint16_t cid);
...
uint16_t altcom_make_header(FAR struct altcom_cmdhdr_s *hdr, uint8_t ver, uint16_t cid,
							uint16_t sz);
