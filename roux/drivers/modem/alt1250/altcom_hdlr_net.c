...
#include <roux/wireless/lte/lte.h>
...
{
	data->valid = APICMD_QUALITY_ENABLE == cmd_quality->enability ? LTE_VALID : LTE_INVALID;
	if (data->valid) {
		...
		if (data->rsrp < APICMD_QUALITY_RSRP_MIN || APICMD_QUALITY_RSRP_MAX < data->rsrp) {
			m_err("data.rsrp error:%d\n", data->rsrp);
			data->valid = LTE_INVALID;
		} else if (data->rsrq < APICMD_QUALITY_RSRQ_MIN || APICMD_QUALITY_RSRQ_MAX < data->rsrq) {
			m_err("data.rsrq error:%d\n", data->rsrq);
			data->valid = LTE_INVALID;
		} else if (data->sinr < APICMD_QUALITY_SINR_MIN || APICMD_QUALITY_SINR_MAX < data->sinr) {
...
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV1
static void altcombs_set_cellinfo(FAR struct apicmd_cmddat_cellinfo_s *cmd_cellinfo,
								  FAR lte_cellinfo_t *api_cellinfo)
{
	if (cmd_cellinfo->valid == LTE_VALID) {
		if (ntohl(cmd_cellinfo->cell_id) > APICMD_CELLINFO_CELLID_MAX) {
			...
			api_cellinfo->valid = LTE_INVALID;
		} else if (cmd_cellinfo->mnc_digit < APICMD_CELLINFO_MNC_DIGIT_MIN ||
				   cmd_cellinfo->mnc_digit > LTE_MNC_DIGIT_MAX) {
			m_err("cmd_cellinfo->mnc_digit error:%d\n", cmd_cellinfo->mnc_digit);
			api_cellinfo->valid = LTE_INVALID;
		} else if (!altcombs_check_arrydigitnum(cmd_cellinfo->mnc, cmd_cellinfo->mnc_digit)) {
...
static void altcombs_set_cellinfo_v4(FAR struct apicmd_cmddat_cellinfo_v4_s *cmd_cellinfo,
									 FAR lte_cellinfo_t *api_cellinfo)
{
	int i;
	...
	if (cmd_cellinfo->enability != LTE_INVALID) {
		if (ntohl(cmd_cellinfo->cell_id) > APICMD_CELLINFO_CELLID_MAX) {
			...
			m_err("cmd_cellinfo->mnc error\n");
		} else if (strlen((FAR const char *)cmd_cellinfo->cgid) > APICMD_CELLINFO_GCID_MAX) {
			...
			api_cellinfo->gcid =
				strtoul((FAR const char *)cmd_cellinfo->cgid, NULL, ALTCOMBS_BASE_HEX);
			...
			for (i = 0; i < api_cellinfo->nr_neighbor; i++) {
				...
				api_cellinfo->neighbors[i].earfcn = ntohl(cmd_cellinfo->neighbor_cell[i].earfcn);
				if (cmd_cellinfo->neighbor_cell[i].valid & APICMD_CELLINFO_VALID_SFN) {
					api_cellinfo->neighbors[i].option |= LTE_CELLINFO_OPT_SFN;
					api_cellinfo->neighbors[i].sfn = ntohs(cmd_cellinfo->neighbor_cell[i].sfn);
				}

				if (cmd_cellinfo->neighbor_cell[i].valid & APICMD_CELLINFO_VALID_RSRP) {
					api_cellinfo->neighbors[i].option |= LTE_CELLINFO_OPT_RSRP;
					api_cellinfo->neighbors[i].rsrp = ntohs(cmd_cellinfo->neighbor_cell[i].rsrp);
				}

				if (cmd_cellinfo->neighbor_cell[i].valid & APICMD_CELLINFO_VALID_RSRQ) {
					api_cellinfo->neighbors[i].option |= LTE_CELLINFO_OPT_RSRQ;
					api_cellinfo->neighbors[i].rsrq = ntohs(cmd_cellinfo->neighbor_cell[i].rsrq);
				}
			}
		}
	}
...
static void getltime_parse_response(FAR struct apicmd_cmddat_getltimeres_s *resp,
...
static void parse_ltime(FAR struct apicmd_cmddat_ltime_s *from, FAR lte_localtime_t *to)
{
	to->year = from->year;
	to->mon = from->month;
	to->mday = from->day;
...
static int parse_simd(FAR struct apicmd_cmddat_repevt_simd_s *simd, FAR uint32_t *simstat)
{
	int ret = 0;
...
static int parse_simstate(FAR struct apicmd_cmddat_repevt_simstate_s *simstate,
...
int32_t altcom_getltime_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_getoper_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								   FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_getqual_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								   FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_getcell_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								   FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_getrat_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_setrat_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV4
...
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV4
	if (altver == ALTCOM_VER4) {
		FAR struct apicmd_cmddat_setrat_s *out = (FAR struct apicmd_cmddat_setrat_s *)pktbuf;
...
int32_t altcom_getimscap_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									 FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_setrepqual_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_setrepcell_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
	FAR cellinfo_report_cb_t *callback = (FAR cellinfo_report_cb_t *)arg[0];
...
int32_t altcom_setrepevt_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									 FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV1
	if (altver == ALTCOM_VER1) {
		FAR struct apicmd_cmddat_setrepevt_s *out = (FAR struct apicmd_cmddat_setrepevt_s *)pktbuf;

		if (*id == LTE_CMDID_REPSIMSTAT) {
			if (callback) {
				g_set_repevt |= (APICMD_SET_REP_EVT_SIMD | APICMD_SET_REP_EVT_SIMSTATE);
			} else {
				g_set_repevt &= ~(APICMD_SET_REP_EVT_SIMD | APICMD_SET_REP_EVT_SIMSTATE);
			}
		...
		*altcid = APICMDID_SET_REP_EVT;
		...
		if (*id == LTE_CMDID_REPSIMSTAT) {
			out->event = htons(APICMD_SET_REP_EVT_SIMD | APICMD_SET_REP_EVT_SIMSTATE);
		} else if (*id == LTE_CMDID_REPLTIME) {
		...
		out->enability = callback ? APICMD_SET_REP_EVT_ENABLE : APICMD_SET_REP_EVT_DISABLE;
...
int32_t altcom_getltime_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								  ...
								  FAR uint64_t *bitmap)
{
	...
	FAR struct apicmd_cmddat_getltimeres_s *in = (FAR struct apicmd_cmddat_getltimeres_s *)pktbuf;
...
int32_t altcom_getoper_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								 uint8_t altver, FAR void **arg, size_t arglen,
								 FAR uint64_t *bitmap)
{
	FAR int *ret = (FAR int *)arg[0];
		...
		*ret = (LTE_RESULT_OK == in->result) ? 0 : -EPROTO;
		if (0 == *ret) {
			if (arglen > 2) {
				...
				if (*len < strnlen((FAR const char *)in->oper, LTE_OPERATOR_LEN)) {
					return -ENOBUFS;
				}
			}
...
int32_t altcom_getqual_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								 uint8_t altver, FAR void **arg, size_t arglen,
								 FAR uint64_t *bitmap)
{
	FAR int *ret = (FAR int *)arg[0];
...
int32_t altcom_getcell_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								 uint8_t altver, FAR void **arg, size_t arglen,
								 FAR uint64_t *bitmap)
{
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV4
...
int32_t altcom_getrat_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								uint8_t altver, FAR void **arg, size_t arglen, FAR uint64_t *bitmap)
{
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV4
	FAR int *ret = (FAR int *)arg[0];
...
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV4
	if (altver == ALTCOM_VER4) {
		FAR struct apicmd_cmddat_getratres_s *in = (FAR struct apicmd_cmddat_getratres_s *)pktbuf;
...
int32_t altcom_setrat_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								uint8_t altver, FAR void **arg, size_t arglen, FAR uint64_t *bitmap)
{
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV4
	FAR int *ret = (FAR int *)arg[0];
...
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV4
	if (altver == ALTCOM_VER4) {
		FAR struct apicmd_cmddat_setratres_s *in = (FAR struct apicmd_cmddat_setratres_s *)pktbuf;
...
int32_t altcom_getimscap_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								   uint8_t altver, FAR void **arg, size_t arglen,
								   FAR uint64_t *bitmap)
{
	FAR int *ret = (FAR int *)arg[0];
	FAR bool *imscap = (FAR bool *)arg[1];
	FAR struct apicmd_cmddat_getimscapres_s *in = (FAR struct apicmd_cmddat_getimscapres_s *)pktbuf;

	*ret = (LTE_RESULT_OK == in->result) ? 0 : -EPROTO;
	if (0 == *ret) {
		*imscap = (in->ims_cap == APICMD_IMSCAP_ENABLE) ? LTE_ENABLE : LTE_DISABLE;
	}

	return 0;
}

int32_t altcom_setrepqual_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf,
									size_t pktsz, uint8_t altver, FAR void **arg, size_t arglen,
									FAR uint64_t *bitmap)
{
	FAR int *ret = (FAR int *)arg[0];
...
int32_t altcom_setrepcell_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf,
									size_t pktsz, uint8_t altver, FAR void **arg, size_t arglen,
									FAR uint64_t *bitmap)
{
	FAR int *ret = (FAR int *)arg[0];
...
int32_t altcom_repqual_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								 uint8_t altver, FAR void **arg, size_t arglen,
								 FAR uint64_t *bitmap)
{
	...
	FAR struct apicmd_cmddat_quality_s *in = (FAR struct apicmd_cmddat_quality_s *)pktbuf;
...
int32_t altcom_repcell_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								 uint8_t altver, FAR void **arg, size_t arglen,
								 FAR uint64_t *bitmap)
{
...
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV1
	if (altver == ALTCOM_VER1) {
		FAR struct apicmd_cmddat_cellinfo_s *in = (FAR struct apicmd_cmddat_cellinfo_s *)pktbuf;
...
int32_t altcom_setrepevt_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								   uint8_t altver, FAR void **arg, size_t arglen,
								   FAR uint64_t *bitmap)
{
	FAR int *ret = (FAR int *)arg[0];
...
int32_t altcom_repevt_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								uint8_t altver, FAR void **arg, size_t arglen, FAR uint64_t *bitmap)
{
	int32_t ret = -ENOTSUP;
...
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV1
	if (altver == ALTCOM_VER1) {
		FAR struct apicmd_cmddat_repevt_s *in = (FAR struct apicmd_cmddat_repevt_s *)pktbuf;
		...
		case APICMD_REPORT_EVT_TYPE_SIMSTATE: {
			*bitmap = get_event_lapibuffer(dev, LTE_CMDID_REPSIMSTAT, &inst);
			if (*bitmap != 0ULL) {
...
#ifndef CONFIG_MODEM_ALT1250_DISABLE_PV4
		if (altver == ALTCOM_VER4) {
		FAR struct apicmd_cmddat_repevt_v4_s *in = (FAR struct apicmd_cmddat_repevt_v4_s *)pktbuf;
