								...
								FAR struct altcom_sockaddr_storage *to)
{
	...
	if (from->sa_family == AF_INET) {
		...
		memcpy(&inaddr_to->sin_addr, &inaddr_from->sin_addr, sizeof(struct altcom_in_addr));
		...
		memcpy(&in6addr_to->sin6_addr, &in6addr_from->sin6_addr, sizeof(struct altcom_in6_addr));
	}
}

static void altstorage2sockaddr(FAR const struct altcom_sockaddr_storage *from,
								FAR struct sockaddr *to)
{
	...
	if (from->ss_family == ALTCOM_AF_INET) {
		...
		inaddr_to->sin_port = inaddr_from->sin_port;
		memcpy(&inaddr_to->sin_addr, &inaddr_from->sin_addr, sizeof(struct in_addr));
		...
		in6addr_to->sin6_port = in6addr_from->sin6_port;
		memcpy(&in6addr_to->sin6_addr, &in6addr_from->sin6_addr, sizeof(struct in6_addr));
...
{
	if (flags & (MSG_DONTROUTE | MSG_CTRUNC | MSG_PROXY | MSG_TRUNC | MSG_EOR | MSG_FIN | MSG_SYN |
...
static int32_t sockaddrlen_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									   FAR uint8_t *pktbuf, const size_t pktsz)
{
	int32_t size = 0;
	FAR int16_t *usockid = (FAR int16_t *)arg[0];
	FAR int16_t *addrlen = (FAR int16_t *)arg[1];

	FAR struct altmdmpkt_sockaddrlen_s *out = (FAR struct altmdmpkt_sockaddrlen_s *)pktbuf;
...
static int32_t sockaddr_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									FAR uint8_t *pktbuf, const size_t pktsz)
{
	int32_t size = 0;
	FAR int16_t *usockid = (FAR int16_t *)arg[0];
	...
	FAR struct altmdmpkt_sockaddr_s *out = (FAR struct altmdmpkt_sockaddr_s *)pktbuf;
...
int32_t altcom_socket_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	...
	FAR struct apicmd_socket_s *out = (FAR struct apicmd_socket_s *)pktbuf;
...
int32_t altcom_close_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver, FAR uint8_t *pktbuf,
								 const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
	FAR int16_t *usockid = (FAR int16_t *)arg[0];

	FAR struct apicmd_close_s *out = (FAR struct apicmd_close_s *)pktbuf;
...
int32_t altcom_accept_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_bind_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver, FAR uint8_t *pktbuf,
								const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_connect_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								   FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_fcntl_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver, FAR uint8_t *pktbuf,
								 const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
	...
	FAR struct apicmd_fcntl_s *out = (FAR struct apicmd_fcntl_s *)pktbuf;
...
int32_t altcom_getsockname_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									   FAR uint8_t *pktbuf, const size_t pktsz,
									   FAR uint16_t *altcid)
{
	int32_t size = 0;
...
int32_t altcom_getsockopt_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
	FAR int32_t *sockfd = (FAR int32_t *)arg[0];
	...
	FAR struct apicmd_getsockopt_s *out = (FAR struct apicmd_getsockopt_s *)pktbuf;
...
int32_t altcom_listen_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
	FAR int32_t *sockfd = (FAR int32_t *)arg[0];
	...
	FAR struct apicmd_listen_s *out = (FAR struct apicmd_listen_s *)pktbuf;
...
int32_t altcom_recvfrom_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
	FAR int32_t *sockfd = (FAR int32_t *)arg[0];
	...
	FAR struct apicmd_recvfrom_s *out = (FAR struct apicmd_recvfrom_s *)pktbuf;
	int32_t flg;
...
int32_t altcom_sendto_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
	FAR int32_t *sockfd = (FAR int32_t *)arg[0];
	...
	FAR struct apicmd_sendto_s *out = (FAR struct apicmd_sendto_s *)pktbuf;
	int32_t flg;
	struct altcom_sockaddr_storage altsa;
	...
	if (size == 0) {
		...
		size = sizeof(struct apicmd_sendto_s) - sizeof(out->senddata) + *buflen;
	}
...
int32_t altcom_setsockopt_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
	FAR int32_t *sockfd = (FAR int32_t *)arg[0];
	...
	FAR struct apicmd_setsockopt_s *out = (FAR struct apicmd_setsockopt_s *)pktbuf;
	...
	case ALTCOM_SO_SETMODE_32BIT:
		...
		*((FAR int32_t *)out->optval) = htonl(*((FAR int32_t *)value));
		break;
...
int32_t altcom_select_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								  FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	int32_t size = 0;
	...
	FAR struct apicmd_select_s *out = (FAR struct apicmd_select_s *)pktbuf;
...
int32_t altcom_shutdown_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	FAR int32_t sockfd = *((FAR int32_t *)arg[0]);
	FAR int32_t how = *((FAR int32_t *)arg[1]);
	FAR struct apicmd_shutdown_s *out = (FAR struct apicmd_shutdown_s *)pktbuf;
...
int32_t altcom_sockcomm_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								  ...
								  FAR uint64_t *bitmap)
{
	...
	FAR struct altmdmpktr_sockcomm_s *in = (FAR struct altmdmpktr_sockcomm_s *)pktbuf;

	*ret = ntohl(in->ret_code);
	*errcode = altcom_errno2roux(ntohl(in->err_code));

	return 0;
}

int32_t altcom_scokaddr_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								  uint8_t altver, FAR void **arg, size_t arglen,
								  FAR uint64_t *bitmap)
{
	int32_t rc = OK;
	...
	FAR struct altmdmpktr_sockaddr_s *in = (FAR struct altmdmpktr_sockaddr_s *)pktbuf;
	struct altcom_sockaddr_storage altsa;
...
int32_t altcom_getsockopt_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf,
									...
									FAR uint64_t *bitmap)
{
	...
	FAR struct apicmd_getsockoptres_s *in = (FAR struct apicmd_getsockoptres_s *)pktbuf;
		...
		*optlen = ntohl(in->optlen);
		if (*optlen > APICMD_OPTVAL_LENGTH) {
			...
			case ALTCOM_SO_SETMODE_32BIT:
				...
				*((FAR int32_t *)optval) = ntohl(*((FAR int32_t *)in->optval));
				break;
			case ALTCOM_SO_SETMODE_LINGER:
				...
				((FAR struct linger *)optval)->l_onoff = ntohl(plinger->l_onoff);
				((FAR struct linger *)optval)->l_linger = ntohl(plinger->l_linger);
				break;
			case ALTCOM_SO_SETMODE_INADDR:
				...
				((FAR struct in_addr *)optval)->s_addr = ntohl(pinaddr->s_addr);
				break;
			default:
				break;
			}
...
int32_t altcom_recvfrom_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								  uint8_t altver, FAR void **arg, size_t arglen,
								  FAR uint64_t *bitmap)
{
	int32_t rc = OK;
	...
	FAR struct apicmd_recvfromres_s *in = (FAR struct apicmd_recvfromres_s *)pktbuf;
	struct altcom_sockaddr_storage altsa;
...
int32_t altcom_select_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								uint8_t altver, FAR void **arg, size_t arglen, FAR uint64_t *bitmap)
{
	...
	FAR struct apicmd_selectres_s *in = (FAR struct apicmd_selectres_s *)pktbuf;
