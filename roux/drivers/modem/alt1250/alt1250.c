...
static ssize_t alt1250_read(FAR struct file *filep, FAR char *buffer, size_t len);
...
compose_handler_t alt1250_additional_composehdlr(uint32_t, FAR uint8_t *, size_t);

#ifdef CONFIG_PM
static int alt1250_pm_prepare(struct pm_callback_s *cb, int domain, enum pm_state_e pmstate);
static void alt1250_pm_notify(struct pm_callback_s *cb, int domain, enum pm_state_e pmstate);
#endif
...
static void add_list(FAR struct alt_queue_s *head, FAR struct alt_container_s *list)
{
...
static FAR struct alt_container_s *remove_list_all(FAR struct alt_queue_s *head)
{
...
static FAR struct alt_container_s *remove_list(FAR struct alt_queue_s *head, uint16_t cmdid,
											   uint16_t transid)
{
	FAR struct alt_container_s *list;
...
static ssize_t read_data(FAR struct alt1250_dev_s *dev, FAR struct alt_readdata_s *rdata)
{
	int idx;
...
static void write_evtbitmap(FAR struct alt1250_dev_s *dev, uint64_t bitmap,
...
static void add_evtbuff(FAR struct alt1250_dev_s *dev, FAR struct alt_evtbuffer_s *buff)
{
	dev->evtbuff = buff;
}
...
static int write_evtbuff_byidx(FAR struct alt1250_dev_s *dev, uint64_t idx,
							   CODE void (*write_func)(FAR void *outp[], FAR void *inp),
							   FAR void *inp)
{
	int ret = WRITE_NG;
...
static void lock_evtbuffinst(FAR alt_evtbuf_inst_t *inst, FAR struct alt1250_dev_s *dev)
{
	nxmutex_lock(&dev->evtmaplock);
	nxmutex_lock(&inst->stat_lock);
}
...
static void unlock_evtbufinst(FAR alt_evtbuf_inst_t *inst, FAR struct alt1250_dev_s *dev)
{
	nxmutex_unlock(&inst->stat_lock);
	nxmutex_unlock(&dev->evtmaplock);
}
...
static FAR alt_evtbuf_inst_t *search_evtbufinst(uint16_t cid, FAR uint64_t *bitmap,
...
static uint64_t get_bitmap(FAR struct alt1250_dev_s *dev, uint16_t cid, uint8_t altver)
{
...
static FAR alt_evtbuf_inst_t *get_evtbuffinst_withlock(FAR struct alt1250_dev_s *dev, uint16_t cid,
													   uint8_t altver, FAR uint64_t *bitmap)
{
...
compose_handler_t get_composehdlr(uint32_t cmdid, FAR uint8_t *payload, size_t size)
{
...
static int alt1250_power_control(FAR struct alt1250_dev_s *dev, FAR struct alt_power_s *req)
{
	int ret = OK;
...
static int make_altcomcmd_and_send(FAR struct alt1250_dev_s *dev, FAR alt_container_t *req)
{
	int ret = OK;
	...
	handler = get_composehdlr(req->cmdid & ~LTE_CMDOPT_ASYNC_BIT, payload, ALTCOM_PAYLOAD_SIZE_MAX);
	if (handler) {
		altver = altmdm_get_protoversion();
		if ((altver == ALTCOM_VERX) || is_senddisable(dev)) {
			ret = -ENETDOWN;
		} else {
			ret = handler(req->inparam, req->inparamlen, altver, payload, ALTCOM_PAYLOAD_SIZE_MAX,
						  &cid);
			...
			if (ret >= 0) {
				tid =
					altcom_make_header((FAR struct altcom_cmdhdr_s *)g_sendbuff, altver, cid, ret);
				...
				if (ret < 0) {
					...
					if ((req->outparam != NULL) &&
						(remove_list(&dev->waitlist, req->altcid, req->alttid) == NULL)) {
						ret = -ENETRESET;
					}
				} else {
					m_info("write success: size=%d, cid=0x%04x tid=0x%04x\n", ret, cid, tid);
...
static int exchange_selectcontainer(FAR struct alt1250_dev_s *dev, FAR alt_container_t **container)
{
...
static int parse_altcompkt(FAR struct alt1250_dev_s *dev, FAR uint8_t *pkt, int pktlen,
						   FAR uint64_t *bitmap, FAR struct alt_container_s **container)
{
	int ret;
	...
	*container = remove_list(&dev->waitlist, cid, tid);
	...
	ret = parser(dev, get_payload(h), get_payload_len(h), get_altver(h), outparam, outparamlen,
				 bitmap);
...
static int altcom_recvthread(int argc, FAR char *argv[])
{
	...
	while (is_running) {
		ret = altmdm_read(g_recvbuff + recvedlen, ALTCOM_RX_PKT_SIZE_MAX - recvedlen);
		...
		if (ret >= 0) {
			...
			if (ret == 0) {
				ret = parse_altcompkt(dev, g_recvbuff, recvedlen, &bitmap, &container);
				if (ret == OK) {
		...
		} else if (ret == ALTMDM_RETURN_RESET_V1 || ret == ALTMDM_RETURN_RESET_V4) {
			reason = altmdm_get_reset_reason();

			m_info("receive ALTMDM_RETURN_RESET_V%s reason: %d\n",
				   (ret == ALTMDM_RETURN_RESET_V1) ? "1" : "4", reason);

#if defined(CONFIG_MODEM_ALT1250_DISABLE_PV1) && defined(CONFIG_MODEM_ALT1250_DISABLE_PV4)
#error Unsupported configuration. Do not disable both PV1 and PV4.
			...
			ret = write_evtbuff_byidx(dev, 0, write_restart_param, (FAR void *)&reason);
...
static int alt1250_start_rxthread(FAR struct alt1250_dev_s *dev, bool senddisable)
{
	int ret = OK;
	...
	ret = kthread_create("altcom_recvthread", SCHED_PRIORITY_DEFAULT, CONFIG_DEFAULT_TASK_STACKSIZE,
						 altcom_recvthread, NULL);
...
static int alt1250_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case ALT1250_IOC_SETEVTBUFF: {
		FAR struct alt_evtbuffer_s *buff = (FAR struct alt_evtbuffer_s *)arg;
...
FAR void *alt1250_register(FAR const char *devpath, FAR struct spi_dev_s *dev,
						   FAR const struct alt1250_lower_s *lower)
{
	FAR struct alt1250_dev_s *priv;
	int ret;

	priv = (FAR struct alt1250_dev_s *)kmm_malloc(sizeof(struct alt1250_dev_s));
	if (!priv) {
...
uint64_t get_event_lapibuffer(FAR struct alt1250_dev_s *dev, uint32_t lapicmdid,
