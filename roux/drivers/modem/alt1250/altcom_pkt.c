...
static uint8_t g_poweron_cmd[sizeof(struct altcom_cmdhdr_s) + sizeof(struct altcom_cmdfooter_s)];
...
static inline void set_header_top(FAR struct altcom_cmdhdr_s *hdr, uint8_t ver, uint16_t cid)
{
...
static struct calculated_checksum convert_ntoh(FAR struct altcom_cmdhdr_s *hdr)
{
	...
	if (hdr->ver == ALTCOM_VER1) {
		...
		checksum.body_checksum = calc_checksum_v1(
			&hdr->payload[0], sizeof(struct altcom_cmdfooter_s) - 2 + hdr->datalen);
		...
		checksum.body_checksum = calc_checksum_v4(&hdr->payload[0], hdr->datalen);
...
static int check_valid_pkt(FAR struct altcom_cmdhdr_s *hdr, uint16_t body_checksum)
{
	...
	if (hdr->ver == ALTCOM_VER1) {
		...
		if (footer->checksum == body_checksum) {
			...
			m_err("[V1] cmdid: 0x%04x cmdlen: %d, "
				  ...
				  hdr->cmdid, hdr->datalen, footer->checksum, body_checksum);
		}
		...
		if (hdr->v4_data_cksum == body_checksum) {
			...
			m_err("[V4] cmdid: 0x%04x cmdlen: %d, "
				  "body checksum: 0x%04x/0x%04x\n",
				  hdr->cmdid, hdr->datalen, hdr->v4_data_cksum, body_checksum);
		}
	}
...
static int check_valid_reply(FAR struct altcom_cmdhdr_s *hdr, uint16_t body_checksum)
{
	int valid_version = ALTCOM_VERX;
	...
	if (hdr->ver == ALTCOM_VER1) {
		...
		if (ntohs(hdr->v1_checksum) == checksum) {
			...
			m_err("[V1] cmdid: 0x%04x cmdlen: %d, "
				  ...
				  ntohs(hdr->cmdid), ntohs(hdr->datalen), ntohs(hdr->v1_checksum), checksum);
		}
	} else if (hdr->ver == ALTCOM_VER4) {
		checksum = calc_checksum_v4((FAR uint8_t *)hdr, sizeof(struct altcom_cmdhdr_s) -
															sizeof(hdr->v4_hdr_cksum) -
															sizeof(hdr->v4_data_cksum));
		if (ntohs(hdr->v4_hdr_cksum) == checksum) {
			if (ntohs(hdr->datalen) + sizeof(struct altcom_cmdhdr_s) <= ALTCOM_RX_PKT_SIZE_MAX) {
				return true;
			} else {
			...
			m_err("[V4] cmdid: 0x%04x cmdlen: %d, "
				  "header checksum: 0x%04x/0x%04x\n",
				  ntohs(hdr->cmdid), ntohs(hdr->datalen), ntohs(hdr->v4_hdr_cksum), checksum);
		}
	}
...
{
	FAR struct altcom_cmdhdr_s *hdr = (FAR struct altcom_cmdhdr_s *)g_poweron_cmd;
	FAR struct altcom_cmdfooter_s *footer = (FAR struct altcom_cmdfooter_s *)&hdr->payload[0];

	set_header_top(hdr, ALTCOM_VER1, ALTCOM_CMDID_POWER_ON_V1);
	hdr->v1_options = htons(ALTCOM_CMDOPT_CHECKSUM_EN);
	hdr->v1_checksum =
		htons(calc_checksum_v1((FAR uint8_t *)hdr, sizeof(struct altcom_cmdhdr_s) - 2));

	footer->reserve = 0;
	footer->checksum =
		htons(calc_checksum_v1(&hdr->payload[0], sizeof(struct altcom_cmdfooter_s) - 2));
...
FAR void *altcom_make_poweron_cmd_v4(int *sz)
{
	...
	hdr->v4_hdr_cksum =
		htons(calc_checksum_v4((FAR uint8_t *)hdr, sizeof(struct altcom_cmdhdr_s) - 4));
...
int altcom_is_pkt_ok(FAR uint8_t *pkt, int sz)
{
	...
	ver = check_valid_pkt((FAR struct altcom_cmdhdr_s *)pkt, checksum.body_checksum);
	if (ver == ALTCOM_VERX) {
		ret = -EPROTO;
	}
...
uint16_t altcom_make_header(FAR struct altcom_cmdhdr_s *hdr, uint8_t ver, uint16_t cid, uint16_t sz)
{
	...
	if (ver == ALTCOM_VER1) {
		FAR struct altcom_cmdfooter_s *footer = (FAR struct altcom_cmdfooter_s *)&hdr->payload[sz];
		...
		footer->reserve = 0;
		footer->checksum =
			htons(calc_checksum_v1(&hdr->payload[0], sizeof(struct altcom_cmdfooter_s) - 2 + sz));
