...
begin_packed_struct struct apicmd_pdnset_s {
	...
	uint32_t apntype;
	uint8_t ipaddr_num;
	struct apicmd_ipaddr_s ip_address[LTE_PDN_IPADDR_MAX_COUNT];
	uint8_t imsregister;
	uint8_t dataallow;
...
begin_packed_struct struct apicmd_pdnset_v4_s {
	...
	uint8_t dararoamingallow;
	uint8_t dnsaddr_num;
	struct apicmd_ipaddr_s dns_address[APICMD_PDN_DNSCOUNT_MAX];
} end_packed_struct;
...
begin_packed_struct struct apicmd_cmddat_cellinfo_v4_s {
	...
	struct apicmd_cmddat_neighbor_cell_s neighbor_cell[LTE_NEIGHBOR_CELL_MAX];
} end_packed_struct;
