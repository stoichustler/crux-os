...
static int32_t read_request_hndl(FAR uint8_t *pktbuf, size_t pktsz, FAR void **cb_args,
								 size_t arglen);
static int32_t write_request_hndl(FAR uint8_t *pktbuf, size_t pktsz, FAR void **cb_args,
								  size_t arglen);
static int32_t exec_request_hndl(FAR uint8_t *pktbuf, size_t pktsz, FAR void **cb_args,
								 size_t arglen);
static int32_t start_ov_request_hndl(FAR uint8_t *pktbuf, size_t pktsz, FAR void **cb_args,
									 size_t arglen);
static int32_t stop_ov_request_hndl(FAR uint8_t *pktbuf, size_t pktsz, FAR void **cb_args,
									size_t arglen);
static int32_t fwupdate_notice_hndl(FAR uint8_t *pktbuf, size_t pktsz, FAR void **cb_args,
									size_t arglen);
static int32_t server_op_notice_hndl(FAR uint8_t *pktbuf, size_t pktsz, FAR void **cb_args,
									 size_t arglen);
...
static struct urc_hdltbl_s urc_idhandles[] = {
	{"%LWM2MOBJCMDU: \"READ\",", LTE_CMDID_LWM2M_READ_EVT, read_request_hndl},

	{"%LWM2MOBJCMDU: \"WRITE\",", LTE_CMDID_LWM2M_WRITE_EVT, write_request_hndl},

	{"%LWM2MOBJCMDU: \"EXE\",", LTE_CMDID_LWM2M_EXEC_EVT, exec_request_hndl},

	{"%LWM2MOBJCMDU: \"OBSERVE_START\",", LTE_CMDID_LWM2M_OVSTART_EVT, start_ov_request_hndl},

	{"%LWM2MOBJCMDU: \"OBSERVE_STOP\",", LTE_CMDID_LWM2M_OVSTOP_EVT, stop_ov_request_hndl},

	{"%LWM2MOPEV: ", LTE_CMDID_LWM2M_SERVEROP_EVT, server_op_notice_hndl},

	{"%LWM2MEV: ", LTE_CMDID_LWM2M_FWUP_EVT, fwupdate_notice_hndl},
...
static FAR char strcpy_until(FAR char *dst, int n, FAR char **src, FAR char *delim)
{
	FAR char *tmp = *src;
...
static FAR uint8_t *parse_instance(FAR uint8_t *pktbuf, FAR int *seq_no, FAR int *srv_id,
								  ...
								  size_t arglen)
{
	...
	pktbuf = parse_instance(pktbuf, (FAR int *)&cb_args[0], (FAR int *)&cb_args[1],
							(FAR struct lwm2mstub_instance_s *)(cb_args[2]));
...
FAR static uint8_t *parse_observe(FAR uint8_t *pktbuf, FAR int *seq_no, FAR int *srv_id, int tksize,
								  FAR char *token, FAR struct lwm2mstub_instance_s *inst)
{
	*seq_no = atoi((FAR char *)pktbuf); /* for seq_no */
	pktbuf = skip_until(pktbuf, ",");
...
static int32_t start_ov_request_hndl(FAR uint8_t *pktbuf, size_t pktsz, FAR void **cb_args,
									 size_t arglen)
{
	FAR struct lwm2mstub_ovcondition_s *cond = (FAR struct lwm2mstub_ovcondition_s *)cb_args[5];

	parse_observe(pktbuf, (FAR int *)&cb_args[0], (FAR int *)&cb_args[1], (int)cb_args[4],
				  (FAR char *)cb_args[3], (FAR struct lwm2mstub_instance_s *)cb_args[2]);
...
lwm2mstub_hndl_t lwm2mstub_get_handler(FAR uint8_t **pktbuf, FAR size_t *pktsz,
									   FAR uint32_t *lcmdid)
{
	FAR char *head_pos;
