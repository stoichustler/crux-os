...
static int copy_logfilename(FAR char *filename, size_t fnamelen, FAR char *path)
{
	int ret = OK;
...
{
	if (strlen(filename) + strlen(ALTCOM_LOGSPATH) >= ALTCOM_LOG_ACCESS_PATH_LEN_MAX) {
		return -ENAMETOOLONG;
	}

	snprintf(path, ALTCOM_LOG_ACCESS_PATH_LEN_MAX, "%s%s", ALTCOM_LOGSPATH, filename);
...
int32_t altcom_logsave_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								   FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	FAR struct apicmd_cmddat_clogs_s *out = (FAR struct apicmd_cmddat_clogs_s *)pktbuf;
...
int32_t altcom_loglist_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								   FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	FAR struct apicmddbg_getloglist_s *out = (FAR struct apicmddbg_getloglist_s *)pktbuf;
...
int32_t altcom_logopen_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								   FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
...
int32_t altcom_logclose_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
...
int32_t altcom_logread_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
								   FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	...
	out->fd = htonl(fd);
	out->readlen = (rlen > ALTCOM_LOG_READ_LEN_MAX) ? htonl(ALTCOM_LOG_READ_LEN_MAX) : htonl(rlen);
...
int32_t altcom_loglseek_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
...
int32_t altcom_logremove_pkt_compose(FAR void **arg, size_t arglen, uint8_t altver,
									 FAR uint8_t *pktbuf, const size_t pktsz, FAR uint16_t *altcid)
{
	FAR struct apicmd_logremove_s *out = (FAR struct apicmd_logremove_s *)pktbuf;
	FAR char *filename = (FAR char *)arg[0];
...
int32_t altcom_logsave_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								 uint8_t altver, FAR void **arg, size_t arglen,
								 FAR uint64_t *bitmap)
{
	FAR struct apicmd_cmddat_clogsres_s *out = (FAR struct apicmd_cmddat_clogsres_s *)pktbuf;
...
int32_t altcom_loglist_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								 uint8_t altver, FAR void **arg, size_t arglen,
								 FAR uint64_t *bitmap)
{
	FAR struct apicmddbg_getloglistres_s *out = (FAR struct apicmddbg_getloglistres_s *)pktbuf;
	...
	if (ret == 0) {
		...
		for (i = 0; i < MIN(nlists, out->listsize); i++) {
			ret = copy_logfilename(&list[i * fnamelen], fnamelen, &out->list[i * out->pathlen]);
			if (ret != OK) {
				break;
			}
		}
...
int32_t altcom_logcommon_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								   uint8_t altver, FAR void **arg, size_t arglen,
								   FAR uint64_t *bitmap)
{
	FAR struct apicmd_logcommonres_s *out = (FAR struct apicmd_logcommonres_s *)pktbuf;

	return ntohl(out->altcom_result);
}

int32_t altcom_logread_pkt_parse(FAR struct alt1250_dev_s *dev, FAR uint8_t *pktbuf, size_t pktsz,
								 int8_t altver, FAR void **arg, size_t arglen, FAR uint64_t *bitmap)
{
	FAR struct apicmd_logreadres_s *out = (FAR struct apicmd_logreadres_s *)pktbuf;
	FAR void *buf = arg[0];
