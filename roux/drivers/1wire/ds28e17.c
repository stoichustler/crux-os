...
#endif
static int ds_i2c_transfer(FAR struct i2c_master_s *i2cdev, FAR struct i2c_msg_s *msgs, int count);
...
static const struct i2c_ops_s ds_i2c_ops = {ds_i2c_transfer
#ifdef CONFIG_I2C_RESET
											,
											ds_i2c_reset
#endif
...
static int ds_i2c_read(FAR struct ds_i2c_inst_s *inst, uint16_t i2c_addr, FAR uint8_t *buffer,
					   ssize_t length)
{
	FAR struct onewire_master_s *master = inst->master;
	uint16_t crc;
...
static int ds_i2c_write_read(FAR struct ds_i2c_inst_s *inst, uint16_t i2c_addr,
							 FAR const uint8_t *wbuffer, ssize_t wlength, FAR uint8_t *rbuffer,
							 ssize_t rlength)
{
	FAR struct onewire_master_s *master = inst->master;
	uint16_t crc;
...
static int ds_i2c_process(FAR struct i2c_master_s *i2cdev, FAR struct i2c_msg_s *msgs, int count)
{
	...
	while (i < count) {
		...
		if (i < count - 1 && msgs[i].addr == msgs[i + 1].addr &&
			(msgs[i].flags & I2C_M_READ) == 0 && (msgs[i + 1].flags & I2C_M_READ)) {
			/* Write-read combined transfer. */

			ret = ds_i2c_write_read(inst, msgs[i].addr, msgs[i].buffer, msgs[i].length,
			...
			ret =
				ds_i2c_write(inst, msgs[i].addr, msgs[i].buffer, msgs[i].length, i == (count - 1));
			if (ret < 0) {
				i = ret;
				goto errout;
			}
...
int ds28e17_search(FAR struct ds28e17_dev_s *priv,
				   void (*cb_search)(int family, uint64_t romcode, void *arg), void *arg)
{
...
FAR struct i2c_master_s *ds28e17_lower_half(FAR struct ds28e17_dev_s *priv, uint64_t romcode)
{
...
int ds28e17_lower_half_unregister(FAR struct ds28e17_dev_s *priv, FAR struct i2c_master_s *i2cdev)
{
	FAR struct ds_i2c_inst_s *inst = (FAR struct ds_i2c_inst_s *)i2cdev;
	FAR struct onewire_master_s *master = inst->master;
	int ret;
