...
static inline int sx1509_write(FAR struct sx1509_dev_s *priv, FAR const uint8_t *wbuffer,
							   int wbuflen);
static inline int sx1509_writeread(FAR struct sx1509_dev_s *priv, FAR const uint8_t *wbuffer,
								   int wbuflen, FAR uint8_t *rbuffer, int rbuflen);
static int sx1509_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int dir);
static int sx1509_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, void *val);
static int sx1509_writepin(FAR struct ioexpander_dev_s *dev, uint8_t pin, bool value);
static int sx1509_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
static int sx1509_readbuf(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
#ifdef CONFIG_IOEXPANDER_INT_ENABLE
static FAR void *sx1509_attach(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset,
...
static userled_set_t sx1509_ll_supported(FAR const struct userled_lowerhalf_s *lower);
static void sx1509_ll_setled(FAR const struct userled_lowerhalf_s *lower, int led, bool ledon);
static void sx1509_ll_setall(FAR const struct userled_lowerhalf_s *lower, userled_set_t ledset);
#ifdef CONFIG_USERLED_LOWER_READSTATE
static void sx1509_ll_getall(FAR const struct userled_lowerhalf_s *lower, userled_set_t *ledset);
#endif
...
static const struct ioexpander_ops_s g_sx1509_ops = {sx1509_direction,
													 sx1509_option,
													 sx1509_writepin,
													 sx1509_readpin,
													 sx1509_readbuf
...
static FAR uint8_t g_sx1509_led_regs[SX1509_NR_GPIO_MAX][5] = {
	{SX1509_REGTON_0, SX1509_REGION_0, SX1509_REGOFF_0, 0, 0},

	{SX1509_REGTON_1, SX1509_REGION_1, SX1509_REGOFF_1, 0, 0},
	{SX1509_REGTON_2, SX1509_REGION_2, SX1509_REGOFF_2, 0, 0},
	{SX1509_REGTON_3, SX1509_REGION_3, SX1509_REGOFF_3, 0, 0},
	{SX1509_REGTON_4, SX1509_REGION_4, SX1509_REGOFF_4, SX1509_TRISE_4, SX1509_TFALL_4},
	{SX1509_REGTON_5, SX1509_REGION_5, SX1509_REGOFF_5, SX1509_TRISE_5, SX1509_TFALL_5},
	{SX1509_REGTON_6, SX1509_REGION_6, SX1509_REGOFF_6, SX1509_TRISE_6, SX1509_TFALL_6},
	{SX1509_REGTON_7, SX1509_REGION_7, SX1509_REGOFF_7, SX1509_TRISE_7, SX1509_TFALL_7},
	{SX1509_REGTON_8, SX1509_REGION_8, SX1509_REGOFF_8, 0, 0},
	{SX1509_REGTON_9, SX1509_REGION_9, SX1509_REGOFF_9, 0, 0},
	{SX1509_REGTON_10, SX1509_REGION_10, SX1509_REGOFF_10, 0, 0},
	{SX1509_REGTON_11, SX1509_REGION_11, SX1509_REGOFF_11, 0, 0},
	{SX1509_REGTON_12, SX1509_REGION_12, SX1509_REGOFF_12, SX1509_TRISE_12, SX1509_TFALL_12},
	{SX1509_REGTON_13, SX1509_REGION_13, SX1509_REGOFF_13, SX1509_TRISE_13, SX1509_TFALL_13},
	{SX1509_REGTON_14, SX1509_REGION_14, SX1509_REGOFF_14, SX1509_TRISE_14, SX1509_TFALL_14},
	{SX1509_REGTON_15, SX1509_REGION_15, SX1509_REGOFF_15, SX1509_TRISE_15, SX1509_TFALL_15}};
#endif
								   ...
								   int wbuflen, FAR uint8_t *rbuffer, int rbuflen)
{
	...
	return i2c_writeread(priv->i2c, &config, wbuffer, wbuflen, rbuffer, rbuflen);
}
...
static int sx1509_setbit(FAR struct sx1509_dev_s *priv, uint8_t addr, uint8_t pin, bool bitval)
{
	uint8_t buf[2];
	int ret;
...
static int sx1509_getbit(FAR struct sx1509_dev_s *priv, uint8_t addr, uint8_t pin, FAR bool *val)
{
	uint8_t buf;
	int ret;
...
static int sx1509_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int direction)
{
	FAR struct sx1509_dev_s *priv = (FAR struct sx1509_dev_s *)dev;
	int ret;

	if (direction != IOEXPANDER_DIRECTION_IN && direction != IOEXPANDER_DIRECTION_IN_PULLUP &&
		direction != IOEXPANDER_DIRECTION_IN_PULLDOWN && direction != IOEXPANDER_DIRECTION_OUT &&
	...
	ret =
		sx1509_setbit(priv, SX1509_REGPULLUP_A, pin, (direction == IOEXPANDER_DIRECTION_IN_PULLUP));
	if (ret < 0) {
		goto errout_unlock;
	}
...
static int sx1509_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, FAR void *value)
{
	FAR struct sx1509_dev_s *priv = (FAR struct sx1509_dev_s *)dev;
	int ret = -EINVAL;
...
{
	FAR struct sx1509_dev_s *priv = (FAR struct sx1509_dev_s *)dev;
	FAR struct sx1509_callback_s *cb = (FAR struct sx1509_callback_s *)handle;

	DEBUGASSERT(priv != NULL && cb != NULL);
	DEBUGASSERT((uintptr_t)cb >= (uintptr_t)&priv->cb[0] &&
				(uintptr_t)cb <= (uintptr_t)&priv->cb[CONFIG_SX1509_INT_NCALLBACKS - 1]);
	UNUSED(priv);
...
static int sx1509_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	if (work_available(&priv->work)) {
		work_queue(HPWORK, &priv->work, sx1509_irqworker, (FAR void *)priv, 0);
	}
...
static int sx1509_osc_config(FAR struct sx1509_dev_s *priv)
{
	...
	ret = sx1509_write(priv, buf, 2);
	if (ret == OK) {
		priv->led_freq = (SX1509_INTOSC_FREQ / (2 << (priv->config->led_pre - 1)));
...
static userled_set_t sx1509_ll_supported(FAR const struct userled_lowerhalf_s *lower)
{
	FAR struct sx1509_dev_s *priv = container_of(lower, struct sx1509_dev_s, userleds);
...
static void sx1509_ll_setall(FAR const struct userled_lowerhalf_s *lower, userled_set_t ledset)
{
	...
	for (pin = 0; pin < SX1509_NR_GPIO_MAX; pin++) {
		if ((priv->pin2led[pin] >= 0)) {
			sx1509_ll_setled(lower, priv->pin2led[pin], ledset & (1 << priv->pin2led[pin]));
		}
	}
}
...
static void sx1509_ll_getall(FAR const struct userled_lowerhalf_s *lower, userled_set_t *ledset)
{
	...
	for (pin = 0; pin < SX1509_NR_GPIO_MAX; pin++) {
		if ((priv->pin2led[pin] >= 0)) {
			ret = sx1509_readpin((FAR struct ioexpander_dev_s *)priv, pin, &val);
			if (ret < 0) {
				break;
			}
								...
								FAR struct userled_effect_set_s *set)
{
	...
	if (set->t_on <= priv->t_on_1_ms) {
		...
		tmp = ((SX1509_TON_2_MAX * set->t_on / priv->t_on_2_ms) + SX1509_TON_1_MAX);
	...
	if (set->t_off <= priv->t_off_1_ms) {
		...
		tmp = ((SX1509_TOFF_2_MAX * set->t_off / priv->t_off_2_ms) + SX1509_TOFF_1_MAX);
...
FAR struct ioexpander_dev_s *sx1509_initialize(FAR struct i2c_master_s *i2cdev,
											   FAR struct sx1509_config_s *config)
{
	...
	priv = (FAR struct sx1509_dev_s *)kmm_zalloc(sizeof(struct sx1509_dev_s));
	if (!priv) {
		return NULL;
	}
...
int sx1509_leds_initialize(FAR struct ioexpander_dev_s *ioe, FAR const char *devname)
{
