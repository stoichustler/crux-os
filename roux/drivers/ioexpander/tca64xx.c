...
#include <roux/ioexpander/tca64xx.h>
...
static int tca64_getreg(FAR struct tca64_dev_s *priv, uint8_t regaddr, FAR uint8_t *regval,
						unsigned int count);
static int tca64_putreg(struct tca64_dev_s *priv, uint8_t regaddr, FAR uint8_t *regval,
						unsigned int count);

/* I/O Expander Methods */

static int tca64_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int dir);
static int tca64_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, void *regval);
static int tca64_writepin(FAR struct ioexpander_dev_s *dev, uint8_t pin, bool value);
static int tca64_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
#ifdef CONFIG_IOEXPANDER_MULTIPIN
static int tca64_multiwritepin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
							   FAR const bool *values, int count);
static int tca64_multireadpin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
							  FAR bool *values, int count);
#endif
#ifdef CONFIG_IOEXPANDER_INT_ENABLE
static FAR void *tca64_attach(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset,
...
static void tca64_int_update(FAR struct tca64_dev_s *priv, ioe_pinset_t input, ioe_pinset_t mask);
...
static const struct ioexpander_ops_s g_tca64_ops = {tca64_direction,
													tca64_option,
													tca64_writepin,
													tca64_readpin,
													tca64_readpin
...
static FAR const struct tca64_part_s *tca64_getpart(FAR struct tca64_dev_s *priv)
{
	DEBUGASSERT(priv != NULL && priv->config != NULL && priv->config->part < TCA64_NPARTS);
						...
						unsigned int count)
{
	...
	msg[0].frequency = TCA64XX_I2C_MAXFREQUENCY, msg[0].addr = priv->config->address,
	msg[0].flags = 0, msg[0].buffer = &regaddr, msg[0].length = 1,

	msg[1].frequency = TCA64XX_I2C_MAXFREQUENCY, msg[1].addr = priv->config->address,
	msg[1].flags = I2C_M_READ, msg[1].buffer = regval, msg[1].length = count,
						...
						unsigned int count)
{
	...
	msg[0].frequency = TCA64XX_I2C_MAXFREQUENCY, msg[0].addr = priv->config->address,
	msg[0].flags = 0, msg[0].buffer = cmd, msg[0].length = count + 1,
...
static int tca64_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int direction)
{
	...
	DEBUGASSERT(priv != NULL && priv->config != NULL && pin < CONFIG_IOEXPANDER_NPINS);
...
static int tca64_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, FAR void *value)
{
	FAR struct tca64_dev_s *priv = (FAR struct tca64_dev_s *)dev;
	int ret = -ENOSYS;
...
static int tca64_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value)
{
	...
	tca64_int_update(priv, (ioe_pinset_t)regval << (pin & ~7), (ioe_pinset_t)0xff << (pin & ~7));
#endif
							  ...
							  FAR bool *values, int count)
{
	...
	DEBUGASSERT(priv != NULL && priv->config != NULL && pins != NULL && values != NULL &&
				count > 0);
...
errout_with_restart:
	...
	ret = wd_start(&priv->wdog, TCA64XX_POLLDELAY, tca64_poll_expiry, (wdparm_t)priv);
	if (ret < 0) {
...
static void tca64_interrupt(FAR void *arg)
{
	...
	if (work_available(&priv->work)) {
		...
		work_queue(HPWORK, &priv->work, tca64_irqworker, (FAR void *)priv, 0);
	}
}
#endif
