...
#include <roux/ioexpander/ioe_rpmsg.h>
...
static int ioe_rpmsg_client_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								   uint32_t src, FAR void *priv_);
static int ioe_rpmsg_server_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								   uint32_t src, FAR void *priv_);

static int ioe_rpmsg_direction_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									   uint32_t src, FAR void *priv_);
static int ioe_rpmsg_option_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									uint32_t src, FAR void *priv_);
static int ioe_rpmsg_writepin_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									  uint32_t src, FAR void *priv_);
static int ioe_rpmsg_readpin_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									 uint32_t src, FAR void *priv_);
#ifdef CONFIG_IOEXPANDER_INT_ENABLE
static int ioe_rpmsg_attach_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									uint32_t src, FAR void *priv_);
static int ioe_rpmsg_detach_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									uint32_t src, FAR void *priv_);
static int ioe_rpmsg_irq_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv_);
#endif

static int ioe_rpmsg_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int dir);
static int ioe_rpmsg_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, void *regval);
static int ioe_rpmsg_writepin(FAR struct ioexpander_dev_s *dev, uint8_t pin, bool value);
static int ioe_rpmsg_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
#ifdef CONFIG_IOEXPANDER_INT_ENABLE
static void *ioe_rpmsg_attach(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset,
...
static const struct ioexpander_ops_s g_ioe_rpmsg_ops = {ioe_rpmsg_direction,
														ioe_rpmsg_option,
														ioe_rpmsg_writepin,
														ioe_rpmsg_readpin,
														ioe_rpmsg_readpin
...
static int ioe_rpmsg_wait_ready(FAR struct rpmsg_endpoint *ept)
{
	...
	if (!is_rpmsg_ept_ready(ept)) {
		FAR struct ioe_rpmsg_client_s *priv = container_of(ept, struct ioe_rpmsg_client_s, ept);
...
static int ioe_rpmsg_sendrecv(FAR struct rpmsg_endpoint *ept, uint32_t command,
							  FAR struct ioe_rpmsg_header_s *msg, int len)
{
	struct ioe_rpmsg_cookie_s cookie = {0};
...
static int ioe_rpmsg_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int dir)
{
	...
	return ioe_rpmsg_sendrecv(&priv->ept, IOE_RPMSG_DIRECTION, (struct ioe_rpmsg_header_s *)&msg,
							  sizeof(msg));
}
...
static int ioe_rpmsg_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value)
{
	...
	ret = ioe_rpmsg_sendrecv(&priv->ept, IOE_RPMSG_READPIN, (struct ioe_rpmsg_header_s *)&msg,
							 sizeof(msg));
	if (ret >= 0) {
		*value = ret;
	}
...
static int ioe_rpmsg_writepin(FAR struct ioexpander_dev_s *dev, uint8_t pin, bool value)
{
	...
	return ioe_rpmsg_sendrecv(&priv->ept, IOE_RPMSG_WRITEPIN, (struct ioe_rpmsg_header_s *)&msg,
							  sizeof(msg));
}
...
static int ioe_rpmsg_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, void *regval)
{
	...
	return ioe_rpmsg_sendrecv(&priv->ept, IOE_RPMSG_OPTION, (struct ioe_rpmsg_header_s *)&msg,
							  sizeof(msg));
}
							  ...
							  ioe_callback_t callback, FAR void *arg)
{
	...
	ret = ioe_rpmsg_sendrecv(&priv->ept, IOE_RPMSG_ATTACH, (struct ioe_rpmsg_header_s *)&msg,
							 sizeof(msg));
	if (ret >= 0) {
...
static int ioe_rpmsg_detach(FAR struct ioexpander_dev_s *dev, FAR void *handle)
{
	...
	return ioe_rpmsg_sendrecv(&priv->ept, IOE_RPMSG_DETACH, (struct ioe_rpmsg_header_s *)&msg,
							  sizeof(msg));
}
#endif
									...
									uint32_t src, FAR void *priv_)
{
	...
	msg->header.result =
		IOEXP_SETOPTION(priv->ioe, msg->pin, msg->opt, (FAR void *)(uintptr_t)msg->val);
...
static int ioe_rpmsg_irq_cb(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset, FAR void *priv_)
{
	FAR struct ioe_rpmsg_cb_s *cb = priv_;
								   ...
								   uint32_t src, FAR void *priv_)
{
	...
	struct ioe_rpmsg_cookie_s *cookie = (struct ioe_rpmsg_cookie_s *)(uintptr_t)msg->cookie;
...
static void ioe_rpmsg_client_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	FAR struct ioe_rpmsg_client_s *priv = priv_;
	...
	if (!strcmp(priv->cpuname, rpmsg_get_cpuname(rdev))) {
		...
		priv->ept.priv = priv;
		rpmsg_create_ept(&priv->ept, rdev, eptname, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
						 ioe_rpmsg_client_ept_cb, NULL);

		rpmsg_post(&priv->ept, &priv->sem);
	}
}

static void ioe_rpmsg_client_destroy(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	FAR struct ioe_rpmsg_client_s *priv = priv_;
...
static bool ioe_rpmsg_server_match(FAR struct rpmsg_device *rdev, FAR void *priv_,
...
static void ioe_rpmsg_server_bind(FAR struct rpmsg_device *rdev, FAR void *priv_,
								  FAR const char *name, uint32_t dest)
{
	FAR struct ioe_rpmsg_server_s *priv = priv_;
	FAR struct rpmsg_endpoint *ept;
	...
	rpmsg_create_ept(ept, rdev, name, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY, ioe_rpmsg_server_ept_cb,
					 rpmsg_destroy_ept);
}
...
int ioe_rpmsg_server_initialize(FAR const char *name, FAR struct ioexpander_dev_s *ioe)
{
	...
	ret = rpmsg_register_callback(priv, NULL, NULL, ioe_rpmsg_server_match, ioe_rpmsg_server_bind);
	if (ret < 0) {
		kmm_free(priv);
	}
...
FAR struct ioexpander_dev_s *ioe_rpmsg_client_initialize(FAR const char *cpuname,
														 FAR const char *name)
{
	...
	ret = rpmsg_register_callback(priv, ioe_rpmsg_client_created, ioe_rpmsg_client_destroy, NULL,
								  NULL);
	if (ret < 0) {
		kmm_free(priv);
		return NULL;
