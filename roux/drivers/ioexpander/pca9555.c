...
static inline int pca9555_write(FAR struct pca9555_dev_s *pca, FAR const uint8_t *wbuffer,
								int wbuflen);
static inline int pca9555_writeread(FAR struct pca9555_dev_s *pca, FAR const uint8_t *wbuffer,
									int wbuflen, FAR uint8_t *rbuffer, int rbuflen);
static int pca9555_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int dir);
static int pca9555_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, void *val);
static int pca9555_writepin(FAR struct ioexpander_dev_s *dev, uint8_t pin, bool value);
static int pca9555_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
static int pca9555_readbuf(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
#ifdef CONFIG_IOEXPANDER_MULTIPIN
static int pca9555_multiwritepin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								 FAR const bool *values, int count);
static int pca9555_multireadpin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								FAR bool *values, int count);
static int pca9555_multireadbuf(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								FAR bool *values, int count);
#endif
#ifdef CONFIG_IOEXPANDER_INT_ENABLE
static FAR void *pca9555_attach(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset,
...
static const struct ioexpander_ops_s g_pca9555_ops = {pca9555_direction,
													  pca9555_option,
													  pca9555_writepin,
													  pca9555_readpin,
													  pca9555_readbuf
...
static int pca9555_setbit(FAR struct pca9555_dev_s *pca, uint8_t addr, uint8_t pin, bool bitval)
{
	uint8_t buf[2];
	int ret;
...
static int pca9555_getbit(FAR struct pca9555_dev_s *pca, uint8_t addr, uint8_t pin, FAR bool *val)
{
	uint8_t buf;
	int ret;
...
static int pca9555_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int direction)
{
	...
	ret = pca9555_setbit(pca, PCA9555_REG_CONFIG, pin, (direction == IOEXPANDER_DIRECTION_IN));
...
static int pca9555_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, FAR void *value)
{
	FAR struct pca9555_dev_s *pca = (FAR struct pca9555_dev_s *)dev;
	int ret = -EINVAL;
								...
								FAR bool *values, int count)
{
	...
	ret = pca9555_getmultibits(pca, PCA9555_REG_INPUT, pins, values, count);
	nxmutex_unlock(&pca->lock);
	return ret;
}
								...
								FAR bool *values, int count)
{
	...
	ret = pca9555_getmultibits(pca, PCA9555_REG_OUTPUT, pins, values, count);
	nxmutex_unlock(&pca->lock);
	return ret;
}
...
static int pca9555_detach(FAR struct ioexpander_dev_s *dev, FAR void *handle)
{
	...
	DEBUGASSERT((uintptr_t)cb >= (uintptr_t)&pca->cb[0] &&
				(uintptr_t)cb <= (uintptr_t)&pca->cb[CONFIG_PCA9555_INT_NCALLBACKS - 1]);
	UNUSED(pca);
...
static int pca9555_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	if (work_available(&pca->work)) {
		...
		work_queue(HPWORK, &pca->work, pca9555_irqworker, (FAR void *)pca, 0);
	}
