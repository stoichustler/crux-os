...
static inline int pca9538_write(FAR struct pca9538_dev_s *pca, FAR const uint8_t *wbuffer,
								int wbuflen);
static inline int pca9538_writeread(FAR struct pca9538_dev_s *pca, FAR const uint8_t *wbuffer,
									int wbuflen, FAR uint8_t *rbuffer, int rbuflen);
static int pca9538_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int dir);
static int pca9538_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, void *val);
static int pca9538_writepin(FAR struct ioexpander_dev_s *dev, uint8_t pin, bool value);
static int pca9538_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
static int pca9538_readbuf(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
#ifdef CONFIG_IOEXPANDER_MULTIPIN
static int pca9538_multiwritepin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								 FAR const bool *values, int count);
static int pca9538_multireadpin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								FAR bool *values, int count);
static int pca9538_multireadbuf(FAR struct ioexpander_dev_s *dev, FAR uint8_t *pins,
								FAR bool *values, int count);
#endif
#ifdef CONFIG_IOEXPANDER_INT_ENABLE
static FAR void *pca9538_attach(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset,
...
static const struct ioexpander_ops_s g_pca9538_ops = {pca9538_direction,
													  pca9538_option,
													  pca9538_writepin,
													  pca9538_readpin,
													  pca9538_readbuf
...
static int pca9538_setbit(FAR struct pca9538_dev_s *pca, uint8_t addr, uint8_t pin, bool bitval)
{
	uint8_t buf[2];
	int ret;
...
static int pca9538_getbit(FAR struct pca9538_dev_s *pca, uint8_t addr, uint8_t pin, FAR bool *val)
{
	uint8_t buf;
	int ret;
...
static int pca9538_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int direction)
{
	...
	ret = pca9538_setbit(pca, PCA9538_REG_CONFIG, pin, (direction == IOEXPANDER_DIRECTION_IN));
...
static int pca9538_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, FAR void *value)
{
	FAR struct pca9538_dev_s *pca = (FAR struct pca9538_dev_s *)dev;
	int ret = -EINVAL;
...
static int pca9538_getmultibits(FAR struct pca9538_dev_s *pca, uint8_t addr, FAR uint8_t *pins,
								FAR bool *values, int count)
{
	uint8_t buf[2];
	int ret = OK;
								...
								FAR bool *values, int count)
{
	...
	ret = pca9538_getmultibits(pca, PCA9538_REG_INPUT, pins, values, count);
	nxmutex_unlock(&pca->lock);
	return ret;
}
								...
								FAR bool *values, int count)
{
	...
	ret = pca9538_getmultibits(pca, PCA9538_REG_OUTPUT, pins, values, count);
	nxmutex_unlock(&pca->lock);
	return ret;
}
...
{
	FAR struct pca9538_dev_s *pca = (FAR struct pca9538_dev_s *)dev;
	FAR struct pca9538_callback_s *cb = (FAR struct pca9538_callback_s *)handle;
		...
		for (i = 0; i < CONFIG_PCA9538_INT_NCALLBACKS; i++) {
			...
			if (pca->cb[i].cbfunc != NULL) {
				...
				if (match != 0) {
					...
					pca->cb[i].cbfunc(&pca->dev, match, pca->cb[i].cbarg);
				}
			}
		}
	}
...
static int pca9538_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	if (work_available(&pca->work)) {
		...
		work_queue(HPWORK, &pca->work, pca9538_irqworker, (FAR void *)pca, 0);
	}
...
FAR struct ioexpander_dev_s *pca9538_initialize(FAR struct i2c_master_s *i2cdev,
												FAR struct pca9538_config_s *config)
{
	FAR struct pca9538_dev_s *pcadev;

	DEBUGASSERT(i2cdev != NULL && config != NULL && config->set_nreset_pin != NULL);
...
#ifdef CONFIG_PCA9538_INT_ENABLE
	DEBUGASSERT(pcadev->config->attach != NULL && pcadev->config->enable != NULL);
