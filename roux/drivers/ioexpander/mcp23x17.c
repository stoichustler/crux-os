...
static inline int mcp23x17_write(FAR struct mcp23x17_dev_s *priv, FAR const uint8_t *wbuffer,
								 int wbuflen);
static inline int mcp23x17_writeread(FAR struct mcp23x17_dev_s *priv, FAR const uint8_t *wbuffer,
									 int wbuflen, FAR uint8_t *rbuffer, int rbuflen);
static int mcp23x17_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int dir);
static int mcp23x17_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, void *val);
static int mcp23x17_writepin(FAR struct ioexpander_dev_s *dev, uint8_t pin, bool value);
static int mcp23x17_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
static int mcp23x17_readbuf(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
#ifdef CONFIG_IOEXPANDER_MULTIPIN
static int mcp23x17_multiwritepin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								  FAR const bool *values, int count);
static int mcp23x17_multireadpin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								 FAR bool *values, int count);
static int mcp23x17_multireadbuf(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								 FAR bool *values, int count);
#endif
#ifdef CONFIG_IOEXPANDER_INT_ENABLE
static FAR void *mcp23x17_attach(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset,
...
static const struct ioexpander_ops_s g_mcp23x17_ops = {mcp23x17_direction,
													   mcp23x17_option,
													   mcp23x17_writepin,
													   mcp23x17_readpin,
													   mcp23x17_readbuf
...
static int mcp23x17_setbit(FAR struct mcp23x17_dev_s *priv, uint8_t addr, uint8_t pin, bool bitval)
{
	uint8_t buf[2];
	int ret;
...
static int mcp23x17_getbit(FAR struct mcp23x17_dev_s *priv, uint8_t addr, uint8_t pin,
						   FAR bool *val)
{
	uint8_t buf;
	int ret;
...
static int mcp23x17_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int direction)
{
	...
	ret = mcp23x17_setbit(priv, MCP23X17_GPPUA, pin, (direction == IOEXPANDER_DIRECTION_IN_PULLUP));
...
static int mcp23x17_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, FAR void *value)
{
	FAR struct mcp23x17_dev_s *priv = (FAR struct mcp23x17_dev_s *)dev;
	int ret = -EINVAL;
	...
	if (opt == IOEXPANDER_OPTION_INVERT) {
		...
		ret =
			mcp23x17_setbit(priv, MCP23X17_IPOLA, pin, ((uintptr_t)value == IOEXPANDER_VAL_INVERT));
		nxmutex_unlock(&priv->lock);
	}
...
static int mcp23x17_getmultibits(FAR struct mcp23x17_dev_s *priv, uint8_t addr, FAR uint8_t *pins,
								 FAR bool *values, int count)
{
	uint8_t buf[2];
	int ret = OK;
								 ...
								 FAR bool *values, int count)
{
	...
	ret = mcp23x17_getmultibits(priv, MCP23X17_GPIOA, pins, values, count);
	nxmutex_unlock(&priv->lock);
	return ret;
}
...
{
	FAR struct mcp23x17_dev_s *priv = (FAR struct mcp23x17_dev_s *)dev;
	FAR struct mcp23x17_callback_s *cb = (FAR struct mcp23x17_callback_s *)handle;

	DEBUGASSERT(priv != NULL && cb != NULL);
	DEBUGASSERT((uintptr_t)cb >= (uintptr_t)&priv->cb[0] &&
				(uintptr_t)cb <= (uintptr_t)&priv->cb[CONFIG_MCP23X17_INT_NCALLBACKS - 1]);
	UNUSED(priv);
...
static int mcp23x17_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	if (work_available(&priv->work)) {
		...
		work_queue(HPWORK, &priv->work, mcp23x17_irqworker, (FAR void *)priv, 0);
	}
...
FAR struct ioexpander_dev_s *mcp23x17_initialize(FAR struct i2c_master_s *i2cdev,
												 FAR struct mcp23x17_config_s *config)
{
	...
	priv = (FAR struct mcp23x17_dev_s *)kmm_zalloc(sizeof(struct mcp23x17_dev_s));
	if (!priv) {
		return NULL;
	}
