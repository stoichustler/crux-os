...
#include <roux/ioexpander/pcf8574.h>
...
static int pcf8574_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int dir);
static int pcf8574_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, void *regval);
static int pcf8574_writepin(FAR struct ioexpander_dev_s *dev, uint8_t pin, bool value);
static int pcf8574_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
#ifdef CONFIG_IOEXPANDER_MULTIPIN
static int pcf8574_multiwritepin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								 FAR const bool *values, int count);
static int pcf8574_multireadpin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								FAR bool *values, int count);
#endif
#ifdef CONFIG_IOEXPANDER_INT_ENABLE
static FAR void *pcf8574_attach(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset,
...
static const struct ioexpander_ops_s g_pcf8574_ops = {pcf8574_direction,
													  pcf8574_option,
													  pcf8574_writepin,
													  pcf8574_readpin,
													  pcf8574_readpin
...
static int pcf8574_read(FAR struct pcf8574_dev_s *priv, FAR uint8_t *portval)
{
	...
	msg.frequency = priv->config->frequency, msg.addr = priv->config->address,
...
static int pcf8574_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int direction)
{
...
static int pcf8574_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, FAR void *value)
{
	FAR struct pcf8574_dev_s *priv = (FAR struct pcf8574_dev_s *)dev;
	int ret = -ENOSYS;
...
static void pcf8574_irqworker(void *arg)
{
	...
	ret = wd_start(&priv->wdog, PCF8574_POLLDELAY, pcf8574_poll_expiry, (wdparm_t)priv);
	if (ret < 0) {
...
static void pcf8574_interrupt(FAR void *arg)
{
	...
	if (work_available(&priv->work)) {
		...
		work_queue(HPWORK, &priv->work, pcf8574_irqworker, (FAR void *)priv, 0);
	}
}
#endif
												...
												FAR struct pcf8574_config_s *config)
{
	...
	if (wd_start(&priv->wdog, PCF8574_POLLDELAY, pcf8574_poll_expiry, (wdparm_t)priv) < 0) {
		gpioerr("ERROR: Failed to start poll timer\n");
	}
#endif
