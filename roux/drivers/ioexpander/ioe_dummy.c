...
#include <roux/ioexpander/ioe_dummy.h>

/****************************************************************************
 * Pre-processor Definitions
 ****************************************************************************/

#define IOE_DUMMY_POLLDELAY (CONFIG_IOEXPANDER_DUMMY_INT_POLLDELAY / USEC_PER_TICK)

#define IOE_DUMMY_INT_ENABLED(d, p) (((d)->intenab & ((ioe_pinset_t)1 << (p))) != 0)
#define IOE_DUMMY_INT_DISABLED(d, p) (((d)->intenab & ((ioe_pinset_t)1 << (p))) == 0)

#define IOE_DUMMY_LEVEL_SENSITIVE(d, p) (((d)->trigger & ((ioe_pinset_t)1 << (p))) == 0)
#define IOE_DUMMY_LEVEL_HIGH(d, p) (((d)->level[0] & ((ioe_pinset_t)1 << (p))) != 0)
#define IOE_DUMMY_LEVEL_LOW(d, p) (((d)->level[1] & ((ioe_pinset_t)1 << (p))) != 0)

#define IOE_DUMMY_EDGE_SENSITIVE(d, p) (((d)->trigger & ((ioe_pinset_t)1 << (p))) != 0)
#define IOE_DUMMY_EDGE_RISING(d, p) (((d)->level[0] & ((ioe_pinset_t)1 << (p))) != 0)
#define IOE_DUMMY_EDGE_FALLING(d, p) (((d)->level[1] & ((ioe_pinset_t)1 << (p))) != 0)
#define IOE_DUMMY_EDGE_BOTH(d, p) (IOE_DUMMY_LEVEL_RISING(d, p) && IOE_DUMMY_LEVEL_FALLING(d, p))
...
static int ioe_dummy_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int dir);
static int ioe_dummy_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, void *regval);
static int ioe_dummy_writepin(FAR struct ioexpander_dev_s *dev, uint8_t pin, bool value);
static int ioe_dummy_readpin(FAR struct ioexpander_dev_s *dev, uint8_t pin, FAR bool *value);
#ifdef CONFIG_IOEXPANDER_MULTIPIN
static int ioe_dummy_multiwritepin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								   FAR const bool *values, int count);
static int ioe_dummy_multireadpin(FAR struct ioexpander_dev_s *dev, FAR const uint8_t *pins,
								  FAR bool *values, int count);
#endif
#ifdef CONFIG_IOEXPANDER_INT_ENABLE
static FAR void *ioe_dummy_attach(FAR struct ioexpander_dev_s *dev, ioe_pinset_t pinset,
								  ioe_callback_t callback, FAR void *arg);
static int ioe_dummy_detach(FAR struct ioexpander_dev_s *dev, FAR void *handle);
#endif
...
static const struct ioexpander_ops_s g_ioe_dummy_ops = {ioe_dummy_direction,
														ioe_dummy_option,
														ioe_dummy_writepin,
														ioe_dummy_readpin,
														ioe_dummy_readpin
...
static int ioe_dummy_direction(FAR struct ioexpander_dev_s *dev, uint8_t pin, int direction)
{
...
static int ioe_dummy_option(FAR struct ioexpander_dev_s *dev, uint8_t pin, int opt, FAR void *value)
{
	FAR struct ioe_dummy_dev_s *priv = (FAR struct ioe_dummy_dev_s *)dev;
	int ret = -ENOSYS;
...
{
	FAR struct ioe_dummy_dev_s *priv = (FAR struct ioe_dummy_dev_s *)dev;
	FAR struct ioe_dummy_callback_s *cb = (FAR struct ioe_dummy_callback_s *)handle;

	gpioinfo("handle=%p\n", handle);

	DEBUGASSERT(priv != NULL && cb != NULL);
	DEBUGASSERT((uintptr_t)cb >= (uintptr_t)&priv->cb[0] &&
				(uintptr_t)cb <= (uintptr_t)&priv->cb[CONFIG_IOEXPANDER_DUMMY_INT_NCALLBACKS - 1]);
	UNUSED(priv);
...
static ioe_pinset_t ioe_dummy_int_update(FAR struct ioe_dummy_dev_s *priv)
{
	...
	if (diff != 0) {
		gpioinfo("toggles=%lx inval=%lx last=%lx diff=%lx\n", (unsigned long)toggles,
				 (unsigned long)priv->inval, (unsigned long)priv->last, (unsigned long)diff);
	}
...
static void ioe_dummy_interrupt_work(void *arg)
{
	...
	ret = wd_start(&priv->wdog, IOE_DUMMY_POLLDELAY, ioe_dummy_interrupt, (wdparm_t)priv);
	if (ret < 0) {
...
static void ioe_dummy_interrupt(wdparm_t arg)
{
	...
	if (work_available(&priv->work)) {
		...
		work_queue(HPWORK, &priv->work, ioe_dummy_interrupt_work, (FAR void *)priv, 0);
	}
}
