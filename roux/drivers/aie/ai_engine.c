...
static int aie_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case AIE_CMD_FEED_INPUT:
		ret = lower->ops->feed_input(lower, (int)(intptr_t)filep->f_priv, arg /* input */);
		break;

	case AIE_CMD_GET_OUTPUT:
		ret = lower->ops->get_output(lower, (int)(intptr_t)filep->f_priv, arg /* output */);
		break;

	default:
		if (lower->ops->control) {
			ret = lower->ops->control(lower, (int)(intptr_t)filep->f_priv, cmd, arg);
		} else {
			ret = -ENOSYS;
		}
		break;
...
int aie_register(FAR const char *path, FAR struct aie_lowerhalf_s *lower)
{
	...
	upper = (FAR struct aie_upperhalf_s *)kmm_malloc(sizeof(struct aie_upperhalf_s));
	if (!upper) {
