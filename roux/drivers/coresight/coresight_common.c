...
static DECLARE_BITMAP(g_coresight_trace_id_bitmap, CORESIGHT_SOURCE_BITMAP_SIZE);
static spinlock_t g_coresight_trace_id_lock = SP_UNLOCKED;

static const uint32_t g_coresight_barrier_pkt[4] = {0x7fffffff, 0x7fffffff, 0x7fffffff, 0x7fffffff};
...
int coresight_claim_device(uintptr_t addr)
{
	...
	if (coresight_get32(addr + CORESIGHT_CLAIMCLR) == CORESIGHT_CLAIM_SELF_HOSTED) {
		ret = 0;
	} else {
		/* There was a race setting the tags, clean up and fail */

		coresight_put32(CORESIGHT_CLAIM_SELF_HOSTED, addr + CORESIGHT_CLAIMCLR);
	}
