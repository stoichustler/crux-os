...
#include <roux/coresight/coresight_replicator.h>
...
static int replicator_enable(FAR struct coresight_dev_s *csdev, int iport, int oport);
static void replicator_disable(FAR struct coresight_dev_s *csdev, int iport, int oport);
...
static int replicator_hw_enable(FAR struct coresight_replicator_dev_s *repdev, int port)
{
	uint32_t id0val;
	uint32_t id1val;
	int ret = 0;
...
static void replicator_hw_disable(FAR struct coresight_replicator_dev_s *repdev, int port)
{
	uint32_t id0val;
	uint32_t id1val;
	uint32_t off;
...
static int replicator_enable(FAR struct coresight_dev_s *csdev, int iport, int oport)
{
	FAR struct coresight_replicator_dev_s *repdev = (FAR struct coresight_replicator_dev_s *)csdev;
	int ret = 0;
...
FAR struct coresight_replicator_dev_s *replicator_register(FAR const struct coresight_desc_s *desc)
{
	...
	repdev =
		kmm_zalloc(sizeof(struct coresight_replicator_dev_s) + sizeof(uint8_t) * desc->outport_num);
	if (repdev == NULL) {
