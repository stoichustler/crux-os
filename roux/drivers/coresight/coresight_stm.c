...
#include <roux/coresight/coresight_stm.h>
...
static void stm_hw_disable(FAR struct coresight_stm_dev_s *stmdev)
{
	...
	if (coresight_timeout(0x0, STM_TCSR_BUSY, stmdev->csdev.addr + STM_TCSR) < 0) {
...
static void stm_hw_enable(FAR struct coresight_stm_dev_s *stmdev)
{
	...
	if (stmdev->stmheer != 0) {
		...
		coresight_put32(STM_HE_EN | STM_HE_ERRDETECT_EN, stmdev->csdev.addr + STM_HEMCR);
	}
...
static void stm_disable(FAR struct coresight_dev_s *csdev)
{
	FAR struct coresight_stm_dev_s *stmdev = (FAR struct coresight_stm_dev_s *)csdev;
...
static void stm_init_default_data(FAR struct coresight_stm_dev_s *stmdev)
{
	...
	memset(stmdev->guaranteed, 0, sizeof(unsigned long) * BITS_TO_LONGS(stmdev->numsp));
}
...
static void stm_send(uintptr_t addr, const void *data, uint32_t size, uint8_t write_bytes)
{
	uint64_t payload[1];
...
int stm_set_channel_options(FAR struct coresight_stm_dev_s *stmdev, uint32_t channel,
							uint32_t options)
{
...
ssize_t stm_sendpacket(FAR struct coresight_stm_dev_s *stmdev, enum stp_packet_type_e type,
					   enum stp_packet_flags_e flag, uint32_t channel, FAR const void *data,
					   size_t size)
{
	uintptr_t chaddr;
	uint32_t offset;
...
FAR struct coresight_stm_dev_s *stm_register(FAR const struct coresight_desc_s *desc)
{
