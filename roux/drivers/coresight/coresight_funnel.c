...
#include <roux/coresight/coresight_funnel.h>
...
static int funnel_enable(FAR struct coresight_dev_s *csdev, int iport, int oport);
static void funnel_disable(FAR struct coresight_dev_s *csdev, int iport, int oport);
...
static int funnel_hw_enable(FAR struct coresight_funnel_dev_s *fundev, int port)
{
	uint32_t functl;
	int ret = 0;
...
static void funnel_hw_disable(FAR struct coresight_funnel_dev_s *fundev, int port)
{
	uint32_t functl;
...
static int funnel_enable(FAR struct coresight_dev_s *csdev, int iport, int oport)
{
	FAR struct coresight_funnel_dev_s *fundev = (FAR struct coresight_funnel_dev_s *)csdev;
	int ret = 0;
...
void set_funnel_priority(FAR struct coresight_funnel_dev_s *fundev, uint32_t priority)
{
...
FAR struct coresight_funnel_dev_s *funnel_register(FAR const struct coresight_desc_s *desc)
{
	FAR struct coresight_funnel_dev_s *fundev;
	FAR struct coresight_dev_s *csdev;
	int ret;

	fundev = kmm_zalloc(sizeof(struct coresight_funnel_dev_s) + sizeof(uint8_t) * desc->inport_num);
	if (fundev == NULL) {
