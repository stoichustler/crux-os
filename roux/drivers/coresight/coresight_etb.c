...
#include <roux/coresight/coresight_etb.h>
...
static ssize_t etb_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static void etb_hw_enable(FAR struct coresight_etb_dev_s *etbdev)
{
	...
	coresight_put32(ETB_FFCR_EN_FTC | ETB_FFCR_STOP_TRIGGER, etbdev->csdev.addr + ETB_FFCR);
...
static void etb_hw_read(FAR struct coresight_etb_dev_s *etbdev)
{
	...
	if ((coresight_get32(etbdev->csdev.addr + ETB_STATUS_REG) & ETB_STATUS_RAM_FULL) == 0) {
		coresight_put32(0x0, etbdev->csdev.addr + ETB_RAM_READ_POINTER);
	} else {
...
static void etb_hw_disable(FAR struct coresight_etb_dev_s *etbdev)
{
	...
	coresight_modify32(ETB_FFCR_STOP_FI, ETB_FFCR_STOP_FI, etbdev->csdev.addr + ETB_FFCR);
	coresight_modify32(ETB_FFCR_FON_MAN, ETB_FFCR_FON_MAN, etbdev->csdev.addr + ETB_FFCR);
	if (coresight_timeout(0x0, ETB_FFCR_FON_MAN, etbdev->csdev.addr + ETB_FFCR) < 0) {
	...
	if (coresight_timeout(ETB_FFSR_FT_STOPPED, ETB_FFSR_FT_STOPPED, etbdev->csdev.addr + ETB_FFSR) <
		0) {
...
static int etb_enable(FAR struct coresight_dev_s *csdev)
{
	FAR struct coresight_etb_dev_s *etbdev = (FAR struct coresight_etb_dev_s *)csdev;
	int ret;
...
FAR struct coresight_etb_dev_s *etb_register(FAR const struct coresight_desc_s *desc)
{
	FAR struct coresight_etb_dev_s *etbdev;
