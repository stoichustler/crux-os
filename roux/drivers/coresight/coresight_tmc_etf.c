...
static int tmc_etf_link_enable(FAR struct coresight_dev_s *csdev, int iport, int oport);
static void tmc_etf_link_disable(FAR struct coresight_dev_s *csdev, int iport, int oport);

static int tmc_etf_open(FAR struct file *filep);
static int tmc_etf_close(FAR struct file *filep);
static ssize_t tmc_etf_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static int tmc_etf_link_hw_enable(FAR struct coresight_tmc_dev_s *tmcdev)
{
	...
	coresight_put32(TMC_FFCR_EN_FMT | TMC_FFCR_EN_TI, tmcdev->csdev.addr + TMC_FFCR);
...
static void tmc_etf_hw_disable_and_read(FAR struct coresight_tmc_dev_s *tmcdev)
{
	coresight_unlock(tmcdev->csdev.addr);
