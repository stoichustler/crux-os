...
#include <roux/coresight/coresight_etm.h>
...
#define ETM_MODE_ALL                                                                               \
	(ETM_MODE_EXCLUDE | ETM_MODE_CYCACC | ETM_MODE_STALL | ETM_MODE_TIMESTAMP | ETM_MODE_CTXID |   \
	 ETM_MODE_BBROAD | ETM_MODE_RET_STACK | ETM_MODE_EXCL_KERN | ETM_MODE_EXCL_USER)
...
#define ETM_DEFAULT_EVENT_VAL (ETM_HARD_WIRE_RES_A | ETM_ADD_COMP_0 | ETM_EVENT_NOT_A)

#define ETM3X_SUPPORTED_OPTIONS (ETM_CR_CYC_ACC | ETM_CR_TIMESTAMP_EN | ETM_CR_RETURN_STACK)
...
static inline void etm_write_reg(FAR struct coresight_etm_dev_s *etmdev, uint32_t val, uint32_t off)
{
	coresight_put32(val, etmdev->csdev.addr + off);
}

static inline uint32_t etm_read_reg(FAR struct coresight_etm_dev_s *etmdev, uint32_t off)
{
...
static void etm_modify_reg32(FAR struct coresight_etm_dev_s *etmdev, uint32_t val, uint32_t mask,
							 uint32_t off)
{
...
{
	coresight_modify32(ETM_PDCR_PWD_UP, ETM_PDCR_PWD_UP, etmdev->csdev.addr + ETM_PDCR);
}
...
static int etm_timeout(FAR struct coresight_etm_dev_s *etmdev, uint32_t val, uint32_t mask,
					   uint32_t off)
{
	int i;
...
static int etm_enable(FAR struct coresight_dev_s *csdev)
{
	FAR struct coresight_etm_dev_s *etmdev = (FAR struct coresight_etm_dev_s *)csdev;
	int ret;
...
int etm_config(FAR struct coresight_etm_dev_s *etmdev, FAR const struct etm_config_s *config)
{
	coresight_unlock(etmdev->csdev.addr);
	etm_os_unlock(etmdev);
	etm_clr_pwrdwn(etmdev);
...
FAR struct coresight_etm_dev_s *etm_register(FAR const struct coresight_desc_s *desc)
{
