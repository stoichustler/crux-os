...
#include <roux/coresight/coresight_tmc.h>
...
static ssize_t tmc_etr_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static int tmc_etr_hw_enable(FAR struct coresight_tmc_dev_s *tmcdev)
{
	...
	if (coresight_timeout(TMC_STS_TMCREADY, TMC_STS_TMCREADY, tmcdev->csdev.addr + TMC_STS) < 0) {
	...
	coresight_put32(((uint64_t)(uintptr_t)tmcdev->buf) >> 32, tmcdev->csdev.addr + TMC_DBAHI);

	if (tmcdev->caps & TMC_ETR_SAVE_RESTORE) {
		coresight_put32((uintptr_t)tmcdev->buf, tmcdev->csdev.addr + TMC_RRP);
		coresight_put32(((uint64_t)(uintptr_t)tmcdev->buf >> 32), tmcdev->csdev.addr + TMC_RRPHI);

		coresight_put32((uintptr_t)tmcdev->buf, tmcdev->csdev.addr + TMC_RWP);
		coresight_put32(((uint64_t)(uintptr_t)tmcdev->buf >> 32), tmcdev->csdev.addr + TMC_RWPHI);

		coresight_modify32(0x0, TMC_STS_FULL, tmcdev->csdev.addr + TMC_STS);
	}

	coresight_put32(TMC_FFCR_EN_FMT | TMC_FFCR_EN_TI | TMC_FFCR_FON_FLIN | TMC_FFCR_FON_TRIG_EVT |
...
static void tmc_flush_and_stop(FAR struct coresight_tmc_dev_s *tmcdev)
{
	coresight_modify32(TMC_FFCR_STOP_ON_FLUSH, TMC_FFCR_STOP_ON_FLUSH,
					   tmcdev->csdev.addr + TMC_FFCR);
	coresight_modify32(TMC_FFCR_FON_MAN, TMC_FFCR_FON_MAN, tmcdev->csdev.addr + TMC_FFCR);
	if (coresight_timeout(0x0, TMC_FFCR_FON_MAN, tmcdev->csdev.addr + TMC_FFCR) < 0) {
...
static void tmc_etr_hw_read(FAR struct coresight_tmc_dev_s *tmcdev)
{
	...
	if (tmcdev->offset + tmcdev->len > tmcdev->size) {
		...
		up_invalidate_dcache((uintptr_t)((FAR char *)tmcdev->buf + tmcdev->offset), tmcdev->len);
	}

	if (lost == true) {
		coresight_insert_barrier_packet((FAR char *)tmcdev->buf + tmcdev->offset);
	}
}
...
static void tmc_etr_hw_disable_and_read(FAR struct coresight_tmc_dev_s *tmcdev)
{
	coresight_unlock(tmcdev->csdev.addr);
...
static int tmc_etr_enable(FAR struct coresight_dev_s *csdev)
{
	FAR struct coresight_tmc_dev_s *tmcdev = (FAR struct coresight_tmc_dev_s *)csdev;
	int ret;
					 ...
					 FAR const struct coresight_desc_s *desc)
{
	...
	if ((coresight_get32(desc->addr + TMC_AUTHSTATUS) & TMC_AUTH_NSID_MASK) != TMC_NSID_EN) {
		return -EACCES;
	}
