...
static void coresight_notify_pm(struct pm_callback_s *cb, int domain, enum pm_state_e pmstate)
{
	FAR struct coresight_dev_s *csdev = container_of(cb, struct coresight_dev_s, pmcb);
	...
	case PM_SLEEP:
		if (pmstate == PM_NORMAL || pmstate == PM_IDLE || pmstate == PM_STANDBY) {
...
static int coresight_enable_link(FAR struct coresight_dev_s *csdev,
								 FAR struct coresight_dev_s *prev, FAR struct coresight_dev_s *next)
{
	int inport = 0;
	int outport = 0;
	int ret;
...
static int coresight_build_path(FAR struct coresight_dev_s *srcdev,
								FAR struct coresight_dev_s *destdev, FAR struct list_node *path)
{
...
static void coresight_disable_dev(FAR struct coresight_node_s *node)
{
	...
	case CORESIGHT_DEV_TYPE_LINK: {
		FAR struct coresight_node_s *prev = list_prev_entry(node, struct coresight_node_s, link);
		FAR struct coresight_node_s *next = list_next_entry(node, struct coresight_node_s, link);
...
{
	coresight_disable_path_from(path, container_of(path->next, struct coresight_node_s, link));
}
...
static int coresight_enable_path(FAR struct list_node *path)
{
	...
	list_for_every_entry_reverse(path, node, struct coresight_node_s, link)
	{
		...
		case CORESIGHT_DEV_TYPE_LINK: {
			...
			ret = coresight_enable_link(node->csdev, prev->csdev, next->csdev);
			if (ret < 0) {
...
int coresight_register(FAR struct coresight_dev_s *csdev, FAR const struct coresight_desc_s *desc)
{
	FAR struct coresight_dev_s *tempdev;
	irqstate_t flags;
	int i;
	...
	if (csdev->outport_num > 0) {
		csdev->outconns = kmm_zalloc(sizeof(struct coresight_connect_s) * desc->outport_num);
		...
		for (i = 0; i < csdev->outport_num; i++) {
			...
			FAR const struct coresight_portdesc_s *portdesc = &desc->outports[i];
...
int coresight_enable(FAR struct coresight_dev_s *srcdev, FAR struct coresight_dev_s *destdev)
{
	FAR struct coresight_node_s *node;
	irqstate_t flags;
	int ret;
