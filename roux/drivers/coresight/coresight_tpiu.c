...
#include <roux/coresight/coresight_tpiu.h>
...
static void tpiu_hw_disable(FAR struct coresight_tpiu_dev_s *tpiudev)
{
	...
	coresight_modify32(TPIU_FFCR_STOP_FI, TPIU_FFCR_STOP_FI, tpiudev->csdev.addr + TPIU_FFCR);
	coresight_modify32(TPIU_FFCR_FON_MAN, TPIU_FFCR_FON_MAN, tpiudev->csdev.addr + TPIU_FFCR);
	if (coresight_timeout(0, TPIU_FFCR_FON_MAN, tpiudev->csdev.addr + TPIU_FFCR) < 0) {
...
static void tpiu_disable(FAR struct coresight_dev_s *csdev)
{
	FAR struct coresight_tpiu_dev_s *tpiudev = (FAR struct coresight_tpiu_dev_s *)csdev;
...
FAR struct coresight_tpiu_dev_s *tpiu_register(FAR const struct coresight_desc_s *desc)
{
