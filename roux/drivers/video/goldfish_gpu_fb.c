...
static int goldfish_gpu_fb_getplaneinfo(FAR struct fb_vtable_s *vtable, int planeno,
										FAR struct fb_planeinfo_s *pinfo);
static int goldfish_gpu_fb_vsync_thread(int argc, FAR char **argv);
static int goldfish_gpu_fb_init_pipe(FAR struct file *filep, FAR const char *ns,
									 FAR const char *pipe_name, int flags);
static int goldfish_gpu_fb_read_pipe(FAR struct file *pipe, FAR void *buffer, size_t size);
static int goldfish_gpu_fb_write_pipe(FAR struct file *pipe, FAR const void *buffer, size_t size);
static int goldfish_gpu_fb_get_param(FAR struct file *pipe, int type);
static int goldfish_gpu_fb_create_colorbuffer(FAR struct file *pipe, int width, int height,
											  int format);
static int goldfish_gpu_fb_update_colorbuffer(FAR struct file *pipe, int colorbuffer, int x, int y,
											  int width, int height, int format, int type,
											  FAR void *pixel, size_t size);
static int goldfish_gpu_fb_post(FAR struct file *pipe, int colorbuffer);
static int goldfish_gpu_fb_commit(FAR struct goldfish_gpu_fb_s *fb, FAR void *buf);
									 ...
									 FAR const char *pipe_name, int flags)
{
	...
	if (ret < 0) {
		gerr("Could not connect to the '%s' error: %s", buf, strerror(-ret));
		file_close(filep);
		return ret;
	}
	...
	if (ret < 0) {
		gerr("Could not write zero flag to the '%s' error: %s", buf, strerror(-ret));
		file_close(filep);
		return ret;
	}
...
static int goldfish_gpu_fb_commit(FAR struct goldfish_gpu_fb_s *fb, FAR void *buf)
{
	...
	ret = goldfish_gpu_fb_update_colorbuffer(&fb->pipe, fb->colorbuffer, 0, 0, fb->videoinfo.xres,
											 fb->videoinfo.yres, fb->colorformat, fb->colortype,
											 buf, fb->planeinfo.stride * fb->videoinfo.yres);
	if (ret < 0) {
...
static int goldfish_gpu_fb_vsync_thread(int argc, FAR char **argv)
{
	FAR struct goldfish_gpu_fb_s *fb =
		(FAR struct goldfish_gpu_fb_s *)((uintptr_t)strtoul(argv[1], NULL, 0));
	...
	while (1) {
		...
		if (now - last >= MSEC2TICK(16)) {
			...
			if (fb_peek_paninfo(&fb->vtable, &info, FB_NO_OVERLAY) == OK) {
				FAR void *buf = fb->planeinfo.fbmem + fb->planeinfo.stride * info.planeinfo.yoffset;
...
int goldfish_gpu_fb_register(int display)
{
	...
	ret = goldfish_gpu_fb_init_pipe(&fb->pipe, NULL, "opengles", O_RDWR | O_CLOEXEC);
	if (ret < 0) {
	...
	ret = goldfish_gpu_fb_create_colorbuffer(&fb->pipe, fb->videoinfo.xres, fb->videoinfo.yres,
											 fb->colorformat);
	if (ret < 0) {
	...
	argv[0] = arg1;
	argv[1] = NULL;
	pid = kthread_create("goldfish_gpu_fb_thread", CONFIG_GOLDFISH_GPU_FB_PRIORITY,
						 CONFIG_DEFAULT_TASK_STACKSIZE, goldfish_gpu_fb_vsync_thread, argv);
	if (pid < 0) {
