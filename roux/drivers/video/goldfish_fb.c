...
static int goldfish_getvideoinfo(FAR struct fb_vtable_s *vtable, FAR struct fb_videoinfo_s *vinfo);
...
static void goldfish_fb_vsync_irq(FAR struct goldfish_fb_s *fb)
{
	...
	if (fb_peek_paninfo(&fb->vtable, &info, FB_NO_OVERLAY) == OK) {
		uintptr_t buf =
			(uintptr_t)(fb->planeinfo.fbmem + fb->planeinfo.stride * info.planeinfo.yoffset);
...
int goldfish_fb_register(int display, FAR void *regs, int irq)
{
	...
	const struct goldfish_fb_format_s format_map[] = {
		[GOLDFISH_FB_FORMAT_BRGA_8888] = {FB_FMT_RGBA32, 32},
		[GOLDFISH_FB_FORMAT_RGBX_8888] = {FB_FMT_RGB32, 32},
		[GOLDFISH_FB_FORMAT_RGB_888] = {FB_FMT_RGB24, 24},
		[GOLDFISH_FB_FORMAT_RGB_565] = {FB_FMT_RGB16_565, 16},
		[GOLDFISH_FB_FORMAT_BGRA_8888] = {FB_FMT_RGBA32, 32},
		[GOLDFISH_FB_FORMAT_RGBA_5551] = {FB_FMT_RGB16_555, 16},
		[GOLDFISH_FB_FORMAT_RGBA_4444] = {FB_FMT_RGBA16, 16},
	};
	...
	fb->planeinfo.yres_virtual = fb->videoinfo.yres * CONFIG_GOLDFISH_FB_FRAME_NBUFFER;
	...
	putreg32((uintptr_t)fb->planeinfo.fbmem, fb->base + GOLDFISH_FB_SET_BASE);
