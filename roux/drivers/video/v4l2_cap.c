...
#include <roux/video/v4l2_cap.h>
#include <roux/video/video.h>
...
static FAR capture_type_inf_t *get_capture_type_inf(FAR capture_mng_t *cmng, uint8_t type);
static enum capture_state_e estimate_next_capture_state(FAR capture_mng_t *cmng,
														enum capture_state_cause_e cause);
static void change_capture_state(FAR capture_mng_t *cmng, enum capture_state_e next_state);
...
static int save_scene_param(FAR capture_mng_t *cmng, enum v4l2_scene_mode mode, uint32_t id,
							FAR struct v4l2_ext_control *control);
static int complete_capture(uint8_t err_code, uint32_t datasize, FAR const struct timeval *ts,
							FAR void *arg);
static int validate_frame_setting(FAR capture_mng_t *cmng, enum v4l2_buf_type type, uint8_t nr_fmt,
								  FAR video_format_t *vfmt, FAR struct v4l2_rect *clip,
								  FAR struct v4l2_fract *interval);
static size_t get_bufsize(FAR video_format_t *vf);

/* ioctl function for each cmds of ioctl */

static int capture_querycap(FAR struct file *filep, FAR struct v4l2_capability *cap);
static int capture_g_input(FAR int *num);
static int capture_enum_input(FAR struct file *filep, FAR struct v4l2_input *input);
static int capture_reqbufs(FAR struct file *filep, FAR struct v4l2_requestbuffers *reqbufs);
...
static int capture_cancel_dqbuf(FAR struct file *filep, enum v4l2_buf_type type);
static int capture_g_fmt(FAR struct file *filep, FAR struct v4l2_format *fmt);
static int capture_s_fmt(FAR struct file *filep, FAR struct v4l2_format *fmt);
static int capture_try_fmt(FAR struct file *filep, FAR struct v4l2_format *fmt);
static int capture_g_parm(FAR struct file *filep, FAR struct v4l2_streamparm *parm);
static int capture_s_parm(FAR struct file *filep, FAR struct v4l2_streamparm *parm);
static int capture_streamon(FAR struct file *filep, FAR enum v4l2_buf_type *type);
static int capture_streamoff(FAR struct file *filep, FAR enum v4l2_buf_type *type);
static int capture_do_halfpush(FAR struct file *filep, bool enable);
static int capture_takepict_start(FAR struct file *filep, int32_t capture_num);
static int capture_takepict_stop(FAR struct file *filep, bool halfpush);
static int capture_s_selection(FAR struct file *filep, FAR struct v4l2_selection *clip);
static int capture_g_selection(FAR struct file *filep, FAR struct v4l2_selection *clip);
static int capture_queryctrl(FAR struct file *filep, FAR struct v4l2_queryctrl *ctrl);
static int capture_query_ext_ctrl(FAR struct file *filep, FAR struct v4l2_query_ext_ctrl *ctrl);
static int capture_querymenu(FAR struct file *filep, FAR struct v4l2_querymenu *menu);
static int capture_g_ctrl(FAR struct file *filep, FAR struct v4l2_control *ctrl);
static int capture_s_ctrl(FAR struct file *filep, FAR struct v4l2_control *ctrl);
static int capture_g_ext_ctrls(FAR struct file *filep, FAR struct v4l2_ext_controls *ctrls);
static int capture_s_ext_ctrls(FAR struct file *filep, FAR struct v4l2_ext_controls *ctrls);
static int capture_query_ext_ctrl_scene(FAR struct file *filep,
										FAR struct v4s_query_ext_ctrl_scene *ctrl);
static int capture_querymenu_scene(FAR struct file *filep, FAR struct v4s_querymenu_scene *menu);
...
static int capture_s_ext_ctrls_scene(FAR struct file *filep,
									 FAR struct v4s_ext_controls_scene *ctrls);
static int capture_enum_fmt(FAR struct file *filep, FAR struct v4l2_fmtdesc *f);
static int capture_enum_frminterval(FAR struct file *filep, FAR struct v4l2_frmivalenum *f);
static int capture_enum_frmsize(FAR struct file *filep, FAR struct v4l2_frmsizeenum *f);

/* File operations function */

static int capture_open(FAR struct file *filep);
static int capture_close(FAR struct file *filep);
static int capture_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map);
static int capture_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
#ifndef CONFIG_DISABLE_PSEUDOFS_OPERATIONS
														...
														enum capture_state_cause_e cause)
{
	...
	case CAUSE_CAPTURE_DQBUF:
		if (current_state == CAPTURE_STATE_STREAMON && !is_taking_still_picture(cmng)) {
			return CAPTURE_STATE_CAPTURE;
		} else {
			return current_state;
		}
...
static void convert_to_imgdatafmt(FAR video_format_t *video, FAR imgdata_format_t *data)
{
	ASSERT(video && data);
...
static void convert_to_imgsensorfmt(FAR video_format_t *video, FAR imgsensor_format_t *sensor)
{
	ASSERT(video && sensor);
...
static void convert_to_imgdatainterval(FAR struct v4l2_fract *video, FAR imgdata_interval_t *data)
{
	ASSERT(video && data);
...
static void get_clipped_format(uint8_t nr_fmt, FAR video_format_t *fmt, FAR struct v4l2_rect *clip,
							   FAR video_format_t *c_fmt)
{
	...
	if (is_clipped(clip)) {
		...
		c_fmt[CAPTURE_FMT_MAIN].pixelformat = fmt[CAPTURE_FMT_MAIN].pixelformat;
		...
		if (nr_fmt > 1) {
			...
			memcpy(&c_fmt[CAPTURE_FMT_SUB], &fmt[CAPTURE_FMT_SUB], sizeof(video_format_t));

			c_fmt[CAPTURE_FMT_SUB].width =
				(uint32_t)c_fmt[CAPTURE_FMT_SUB].width * clip->width / fmt[CAPTURE_FMT_MAIN].width;

			c_fmt[CAPTURE_FMT_SUB].height = (uint32_t)c_fmt[CAPTURE_FMT_SUB].height * clip->height /
...
static int start_capture(FAR struct capture_mng_s *cmng, enum v4l2_buf_type type, uint8_t nr_fmt,
						 FAR video_format_t *fmt, FAR struct v4l2_rect *clip,
						 FAR struct v4l2_fract *interval, uintptr_t bufaddr, uint32_t bufsize)
{
	...
	IMGDATA_SET_BUF(cmng->imgdata, nr_fmt, df, (FAR uint8_t *)bufaddr, bufsize);
	IMGDATA_START_CAPTURE(cmng->imgdata, nr_fmt, df, &di, complete_capture, cmng);
	IMGSENSOR_START_CAPTURE(cmng->imgsensor,
							type == V4L2_BUF_TYPE_VIDEO_CAPTURE ? IMGSENSOR_STREAM_TYPE_VIDEO
																: IMGSENSOR_STREAM_TYPE_STILL,
							nr_fmt, sf, &si);
	return OK;
}

static void stop_capture(FAR struct capture_mng_s *cmng, enum v4l2_buf_type type)
{
	ASSERT(cmng->imgsensor && cmng->imgdata);

	IMGDATA_STOP_CAPTURE(cmng->imgdata);
	IMGSENSOR_STOP_CAPTURE(cmng->imgsensor, type == V4L2_BUF_TYPE_VIDEO_CAPTURE
												? IMGSENSOR_STREAM_TYPE_VIDEO
												: IMGSENSOR_STREAM_TYPE_STILL);
}
...
static void change_capture_state(FAR capture_mng_t *cmng, enum capture_state_e next_state)
{
	...
	if (current_state != CAPTURE_STATE_CAPTURE && next_state == CAPTURE_STATE_CAPTURE) {
		FAR vbuf_container_t *container =
			video_framebuff_get_vacant_container(&cmng->capture_inf.bufinf);
		if (container != NULL) {
			cmng->capture_inf.seqnum = 0;
			start_capture(cmng, V4L2_BUF_TYPE_VIDEO_CAPTURE, cmng->capture_inf.nr_fmt,
						  cmng->capture_inf.fmt, &cmng->capture_inf.clip,
						  &cmng->capture_inf.frame_interval, container->buf.m.userptr,
						  container->buf.length);
		} else {
			updated_next_state = CAPTURE_STATE_STREAMON;
		}
	} else if (current_state == CAPTURE_STATE_CAPTURE && next_state != CAPTURE_STATE_CAPTURE) {
...
static void initialize_frame_setting(FAR struct imgsensor_s *imgsensor, FAR uint8_t *nr_fmt,
									 FAR video_format_t *fmt, FAR struct v4l2_fract *interval)
{
	...
	*nr_fmt = 1;
	if (imgsensor && imgsensor->frmsizes) {
		if (imgsensor->frmsizes[0].type == V4L2_FRMSIZE_TYPE_DISCRETE) {
			fmt[CAPTURE_FMT_MAIN].width = imgsensor->frmsizes[0].discrete.width;
			fmt[CAPTURE_FMT_MAIN].height = imgsensor->frmsizes[0].discrete.height;
		} else {
			fmt[CAPTURE_FMT_MAIN].width = imgsensor->frmsizes[0].stepwise.min_width;
			fmt[CAPTURE_FMT_MAIN].height = imgsensor->frmsizes[0].stepwise.min_height;
		}
	} else {
	...
	if (imgsensor && imgsensor->frmintervals) {
		if (imgsensor->frmintervals[0].type == V4L2_FRMIVAL_TYPE_DISCRETE) {
			interval->denominator = imgsensor->frmintervals[0].discrete.denominator;
			interval->numerator = imgsensor->frmintervals[0].discrete.numerator;
		} else {
			interval->denominator = imgsensor->frmintervals[0].stepwise.min.denominator;
			interval->numerator = imgsensor->frmintervals[0].stepwise.min.numerator;
		}
	} else {
...
static void initialize_streamresources(FAR capture_type_inf_t *type_inf, FAR capture_mng_t *cmng)
{
	...
	initialize_frame_setting(cmng->imgsensor, &type_inf->nr_fmt, type_inf->fmt,
...
static int32_t initialize_scene_gamma(FAR capture_mng_t *cmng, FAR uint8_t **gamma)
{
	...
	*gamma = NULL;
	...
	ret = IMGSENSOR_GET_SUPPORTED_VALUE(cmng->imgsensor, IMGSENSOR_ID_GAMMA_CURVE, &sup_val);
	if (ret != OK) {
...
static int initialize_scene_parameter(FAR capture_mng_t *cmng, enum v4l2_scene_mode mode,
									  FAR capture_scene_params_t **vsp)
{
	FAR capture_scene_params_t *sp = kmm_malloc(sizeof(capture_scene_params_t));
	...
	sp->awb = get_default_value(cmng, IMGSENSOR_ID_AUTO_WHITE_BALANCE);
	...
	sp->exposure_time = get_default_value(cmng, IMGSENSOR_ID_EXPOSURE_ABSOLUTE);
	...
	sp->wb = get_default_value(cmng, IMGSENSOR_ID_AUTO_N_PRESET_WB);
	sp->wdr = get_default_value(cmng, IMGSENSOR_ID_WIDE_DYNAMIC_RANGE);
	sp->stabilization = get_default_value(cmng, IMGSENSOR_ID_IMG_STABILIZATION);
	sp->iso_auto = get_default_value(cmng, IMGSENSOR_ID_ISO_SENSITIVITY_AUTO);
	sp->iso = get_default_value(cmng, IMGSENSOR_ID_ISO_SENSITIVITY);
	sp->meter = get_default_value(cmng, IMGSENSOR_ID_EXPOSURE_METERING);
...
{
	memset(cmng->capture_scene_param, 0, sizeof(cmng->capture_scene_param));
...
static void cleanup_streamresources(FAR capture_type_inf_t *type_inf, FAR capture_mng_t *cmng)
{
...
{
	return type_inf->bufinf.container_size * get_bufsize(&type_inf->fmt[CAPTURE_FMT_MAIN]);
}
...
static bool validate_clip_setting(FAR struct v4l2_rect *clip, FAR video_format_t *fmt)
{
...
{
	int size = sizeof(g_capture_parameter_name) / sizeof(capture_parameter_name_t);
	int cnt;
...
static int set_intvalue(FAR struct capture_mng_s *cmng, uint32_t id, int32_t value32)
{
...
static int set_pvalue(FAR struct capture_mng_s *cmng, uint32_t id, int size, void *pval)
{
	imgsensor_value_t value;
...
static int reflect_scene_parameter(FAR capture_mng_t *cmng, enum v4l2_scene_mode mode)
{
	...
	set_pvalue(cmng, IMGSENSOR_ID_GAMMA_CURVE, sp->gamma_curve_sz, sp->gamma_curve);
	...
	if (sp->ae == V4L2_EXPOSURE_MANUAL || sp->ae == V4L2_EXPOSURE_SHUTTER_PRIORITY) {
...
static int read_scene_param(FAR struct capture_mng_s *cmng, enum v4l2_scene_mode mode, uint32_t id,
							FAR struct v4l2_ext_control *control)
{
	imgsensor_supported_value_t value;
	capture_scene_params_t *sp;
	...
	case IMGSENSOR_ID_GAMMA_CURVE:
		memcpy(control->p_u8, sp->gamma_curve, sp->gamma_curve_sz);
		break;
...
static int check_range(int64_t value, int64_t min, int64_t max, uint64_t step)
{
							...
							FAR struct v4l2_ext_control *control)
{
	...
	case IMGSENSOR_CTRL_TYPE_U8:
		...
		for (i = 0; i < elem->nr_elems; i++) {
			ret = check_range(control->p_u8[i], elem->minimum, elem->maximum, elem->step);
			if (ret != OK) {
				return ret;
			}
		}
	...
	case IMGSENSOR_CTRL_TYPE_U16:
		...
		for (i = 0; i < elem->nr_elems; i++) {
			ret = check_range(control->p_u16[i], elem->minimum, elem->maximum, elem->step);
			if (ret != OK) {
				return ret;
			}
		}
	...
	case IMGSENSOR_CTRL_TYPE_U32:
		...
		for (i = 0; i < elem->nr_elems; i++) {
			ret = check_range(control->p_u32[i], elem->minimum, elem->maximum, elem->step);
			if (ret != OK) {
				return ret;
			}
		}
	...
	default:
		ret = check_range(control->value, range->minimum, range->maximum, range->step);
		if (ret != OK) {
			return ret;
		}
	...
	case IMGSENSOR_ID_GAMMA_CURVE:
		memcpy(sp->gamma_curve, control->p_u8, sp->gamma_curve_sz);
		break;
							...
							FAR void *arg)
{
	...
	buf_type = cmng->still_inf.state == CAPTURE_STATE_CAPTURE ? V4L2_BUF_TYPE_STILL_CAPTURE
	...
	if (type_inf->remaining_capnum == 0) {
		...
		if (buf_type == V4L2_BUF_TYPE_STILL_CAPTURE &&
			...
			cmng->capture_inf.wait_capture.waitend_cause = WAITEND_CAUSE_STILLSTOP;
		...
		if (container == NULL) {
			...
			get_clipped_format(type_inf->nr_fmt, type_inf->fmt, &type_inf->clip, c_fmt);

			convert_to_imgdatafmt(&c_fmt[CAPTURE_FMT_MAIN], &df[IMGDATA_FMT_MAIN]);
			convert_to_imgdatafmt(&c_fmt[CAPTURE_FMT_SUB], &df[IMGDATA_FMT_SUB]);

			IMGDATA_SET_BUF(cmng->imgdata, type_inf->nr_fmt, df,
							(FAR uint8_t *)container->buf.m.userptr, container->buf.length);
...
static FAR struct imgsensor_s *get_connected_imgsensor(FAR struct imgsensor_s **sensors,
													   size_t sensor_num)
{
	...
	for (i = 0; i < sensor_num; i++) {
		if (sensors[i] && IMGSENSOR_IS_AVAILABLE(sensors[i])) {
			sensor = sensors[i];
			break;
		}
	}
	...
	if (type_inf->state == CAPTURE_STATE_CAPTURE) {
		...
		if (ret == OK && reqbufs->memory == V4L2_MEMORY_MMAP) {
			...
			if (imgdata->ops->alloc) {
				type_inf->bufheap = imgdata->ops->alloc(
					imgdata, 32, reqbufs->count * get_bufsize(&type_inf->fmt[CAPTURE_FMT_MAIN]));
			} else {
				type_inf->bufheap = kumm_memalign(
					32, reqbufs->count * get_bufsize(&type_inf->fmt[CAPTURE_FMT_MAIN]));
			}
...
static int capture_qbuf(FAR struct file *filep, FAR struct v4l2_buffer *buf)
{
	...
	if (buf->memory == V4L2_MEMORY_MMAP) {
		...
		container->buf.m.userptr =
			(unsigned long)(type_inf->bufheap + container->buf.length * buf->index);
	}
	...
	if (type_inf->state == CAPTURE_STATE_STREAMON) {
		...
		if (buf->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
			...
			next_capture_state = estimate_next_capture_state(cmng, CAUSE_CAPTURE_START);
			...
			if (container != NULL) {
				type_inf->seqnum = 0;
				start_capture(cmng, buf->type, type_inf->nr_fmt, type_inf->fmt, &type_inf->clip,
							  &type_inf->frame_interval, container->buf.m.userptr,
							  container->buf.length);
	...
	if (container == NULL) {
		...
		do {
			if (buf->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
				...
				flags = enter_critical_section();
				next_capture_state = estimate_next_capture_state(cmng, CAUSE_CAPTURE_DQBUF);
				change_capture_state(cmng, next_capture_state);
				leave_critical_section(flags);
			}

			nxsem_wait_uninterruptible(dqbuf_wait_flg);
		} while (type_inf->wait_capture.waitend_cause == WAITEND_CAUSE_STILLSTOP);
		...
		if (container == NULL) {
			...
			DEBUGASSERT(type_inf->wait_capture.waitend_cause == WAITEND_CAUSE_DQCANCEL);
			return -ECANCELED;
		}
...
static int capture_s_fmt(FAR struct file *filep, FAR struct v4l2_format *fmt)
{
	...
	case V4L2_PIX_FMT_SUBIMG_RGB565:
		if (type_inf->fmt[CAPTURE_FMT_MAIN].pixelformat != V4L2_PIX_FMT_JPEG_WITH_SUBIMG) {
			return -EPERM;
		}
		...
		type_inf->fmt[CAPTURE_FMT_SUB].pixelformat =
			fmt->fmt.pix.pixelformat == V4L2_PIX_FMT_SUBIMG_UYVY ? V4L2_PIX_FMT_UYVY
																 : V4L2_PIX_FMT_RGB565;
		type_inf->nr_fmt = 2;
		break;
	...
	default:
		...
		type_inf->fmt[CAPTURE_FMT_MAIN].pixelformat = fmt->fmt.pix.pixelformat;
		type_inf->nr_fmt = 1;
		break;
	}
...
static int capture_try_fmt(FAR struct file *filep, FAR struct v4l2_format *fmt)
{
	...
	case V4L2_PIX_FMT_SUBIMG_RGB565:
		...
		nr_fmt = 2;
		memcpy(&vf[CAPTURE_FMT_MAIN], &type_inf->fmt[CAPTURE_FMT_MAIN], sizeof(video_format_t));
		vf[CAPTURE_FMT_SUB].width = fmt->fmt.pix.width;
		vf[CAPTURE_FMT_SUB].height = fmt->fmt.pix.height;
		vf[CAPTURE_FMT_SUB].pixelformat = fmt->fmt.pix.pixelformat == V4L2_PIX_FMT_SUBIMG_UYVY
											  ? V4L2_PIX_FMT_UYVY
											  : V4L2_PIX_FMT_RGB565;
		break;
	case V4L2_PIX_FMT_NV12:
	...
	return validate_frame_setting(cmng, fmt->type, nr_fmt, vf, &type_inf->clip,
								  &type_inf->frame_interval);
}
...
static int capture_g_parm(FAR struct file *filep, FAR struct v4l2_streamparm *parm)
{
	...
	if (type_inf->state == CAPTURE_STATE_CAPTURE) {
		...
		ret = IMGSENSOR_GET_FRAME_INTERVAL(
			cmng->imgsensor, parm->type, (imgsensor_interval_t *)&parm->parm.capture.timeperframe);
	}

	if (ret != OK) {
		/* In no capture state or error case, return stored value. */

		memcpy(&parm->parm.capture.timeperframe, &type_inf->frame_interval,
...
static int capture_s_parm(FAR struct file *filep, FAR struct v4l2_streamparm *parm)
{
	...
	ret = validate_frame_setting(cmng, parm->type, type_inf->nr_fmt, type_inf->fmt, &type_inf->clip,
								 &parm->parm.capture.timeperframe);
	if (ret != OK) {
		return ret;
	}

	memcpy(&type_inf->frame_interval, &parm->parm.capture.timeperframe, sizeof(struct v4l2_fract));
...
static int capture_streamoff(FAR struct file *filep, FAR enum v4l2_buf_type *type)
{
	...
	if (type_inf->state == CAPTURE_STATE_STREAMOFF) {
		...
		next_capture_state = estimate_next_capture_state(cmng, CAUSE_CAPTURE_STOP);
		change_capture_state(cmng, next_capture_state);
	}
...
static int capture_do_halfpush(FAR struct file *filep, bool enable)
{
	...
	control[0].value = enable ? V4L2_LOCK_EXPOSURE | V4L2_LOCK_WHITE_BALANCE : 0;
...
static int capture_takepict_start(FAR struct file *filep, int32_t capture_num)
{
	...
	if (cmng->still_inf.state != CAPTURE_STATE_STREAMOFF) {
		...
		next_capture_state = estimate_next_capture_state(cmng, CAUSE_STILL_START);
		change_capture_state(cmng, next_capture_state);

		leave_critical_section(flags);

		container = video_framebuff_get_vacant_container(&cmng->still_inf.bufinf);
		if (container != NULL) {
			/* Start still stream capture */

			start_capture(cmng, V4L2_BUF_TYPE_STILL_CAPTURE, cmng->still_inf.nr_fmt,
						  cmng->still_inf.fmt, &cmng->still_inf.clip,
						  &cmng->still_inf.frame_interval, container->buf.m.userptr,
						  container->buf.length);
...
static int capture_takepict_stop(FAR struct file *filep, bool halfpush)
{
	...
	if (cmng->still_inf.state == CAPTURE_STATE_STREAMOFF &&
		...
		next_capture_state = estimate_next_capture_state(cmng, CAUSE_STILL_STOP);
		change_capture_state(cmng, next_capture_state);
...
static int capture_s_selection(FAR struct file *filep, FAR struct v4l2_selection *clip)
{
	...
	ret = validate_frame_setting(cmng, clip->type, type_inf->nr_fmt, type_inf->fmt, &clip->r,
								 &type_inf->frame_interval);
	if (ret != OK) {
		return ret;
	}

	id = clip->type == V4L2_BUF_TYPE_VIDEO_CAPTURE ? IMGSENSOR_ID_CLIP_VIDEO
												   : IMGSENSOR_ID_CLIP_STILL;
...
static int capture_queryctrl(FAR struct file *filep, FAR struct v4l2_queryctrl *ctrl)
{
	...
	if (ext_ctrl.type == V4L2_CTRL_TYPE_INTEGER64 || ext_ctrl.type == V4L2_CTRL_TYPE_U8 ||
		ext_ctrl.type == V4L2_CTRL_TYPE_U16 || ext_ctrl.type == V4L2_CTRL_TYPE_U32) {
...
static int capture_query_ext_ctrl(FAR struct file *filep, FAR struct v4l2_query_ext_ctrl *attr)
{
	FAR struct inode *inode = filep->f_inode;
	FAR capture_mng_t *cmng = inode->i_private;
	imgsensor_supported_value_t value;
...
static int capture_querymenu(FAR struct file *filep, FAR struct v4l2_querymenu *menu)
{
	...
	if (menu->id == V4L2_CID_SCENE_MODE) {
		...
		ret = IMGSENSOR_GET_SUPPORTED_VALUE(cmng->imgsensor, menu->id, &value);
		if (ret < 0) {
			return ret;
		}
...
static int capture_g_ext_ctrls(FAR struct file *filep, FAR struct v4l2_ext_controls *ctrls)
{
	...
	for (cnt = 0, control = ctrls->controls; cnt < ctrls->count; cnt++, control++) {
		if (control->id == V4L2_CID_SCENE_MODE) {
			...
			ret = IMGSENSOR_GET_VALUE(cmng->imgsensor, control->id, control->size,
...
static int capture_s_ext_ctrls(FAR struct file *filep, FAR struct v4l2_ext_controls *ctrls)
{
	...
	for (cnt = 0, control = ctrls->controls; cnt < ctrls->count; cnt++, control++) {
		if (control->id == V4L2_CID_SCENE_MODE) {
			...
			ret = IMGSENSOR_SET_VALUE(cmng->imgsensor, control->id, control->size,
									  (imgsensor_value_t)control->value64);
			if (ret == 0) {
				if (cmng->capture_scene_mode == V4L2_SCENE_MODE_NONE) {
					save_scene_param(cmng, V4L2_SCENE_MODE_NONE, control->id, control);
				}
			}
		}
									 ...
									 FAR struct v4s_ext_controls_scene *ctrls)
{
	...
	for (cnt = 0, control = ctrls->control.controls; cnt < ctrls->control.count; cnt++, control++) {
									 ...
									 FAR struct v4s_ext_controls_scene *ctrls)
{
	...
	for (cnt = 0, control = ctrls->control.controls; cnt < ctrls->control.count; cnt++, control++) {
		ret = read_scene_param(cmng, ctrls->mode, control->id, control);
		if (ret != OK) {
			ctrls->control.error_idx = cnt;
			return ret;
		}
...
static int capture_enum_fmt(FAR struct file *filep, FAR struct v4l2_fmtdesc *f)
{
	...
	if (cmng->imgsensor && cmng->imgsensor->fmtdescs) {
		if (f->index > cmng->imgsensor->fmtdescs_num) {
			...
			strlcpy(f->description, cmng->imgsensor->fmtdescs[f->index].description,
...
static int capture_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map)
{
	...
	if (map->offset >= 0 && map->offset < heapsize && map->length &&
		map->offset + map->length <= heapsize) {
...
{
	return capture_register(devpath, g_capture_data, g_capture_registered_sensor,
...
int capture_register(FAR const char *devpath, FAR struct imgdata_s *data,
					 FAR struct imgsensor_s **sensors, size_t sensor_num)
{
...
int imgsensor_register(FAR struct imgsensor_s *sensor)
{
	...
	new_addr = kmm_realloc(g_capture_registered_sensor,
						   sizeof(sensor) * (g_capture_registered_sensor_num + 1));
	if (new_addr != NULL) {
