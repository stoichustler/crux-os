...
ssize_t vnc_rre8(FAR struct vnc_session_s *session, FAR struct rre_encode8_s *dest,
				 FAR struct fb_area_s *rect, uint8_t bgcolor)
{
...
ssize_t vnc_rre16(FAR struct vnc_session_s *session, FAR struct rre_encode16_s *dest,
				  FAR struct fb_area_s *rect, uint16_t bgcolor)
{
	rfb_putbe32(dest->hdr.nsubrects, 1);
...
ssize_t vnc_rre32(FAR struct vnc_session_s *session, FAR struct rre_encode32_s *dest,
				  FAR struct fb_area_s *rect, uint32_t bgcolor)
{
	rfb_putbe32(dest->hdr.nsubrects, 1);
	...
	if (session->rre) {
		...
		if (ret == 1) {
			...
			case FB_FMT_RGB8_222:
				nbytes += vnc_rre8(session, (FAR struct rre_encode8_s *)rrect->data, rect,
								   vnc_convert_rgb8_222(bgcolor));
				break;

			case FB_FMT_RGB8_332:
				nbytes += vnc_rre8(session, (FAR struct rre_encode8_s *)rrect->data, rect,
								   vnc_convert_rgb8_332(bgcolor));
				break;

			case FB_FMT_RGB16_555:
				nbytes += vnc_rre16(session, (FAR struct rre_encode16_s *)rrect->data, rect,
									vnc_convert_rgb16_555(bgcolor));
				break;

			case FB_FMT_RGB16_565:
				nbytes += vnc_rre16(session, (FAR struct rre_encode16_s *)rrect->data, rect,
									vnc_convert_rgb16_565(bgcolor));
				break;

			case FB_FMT_RGB32:
				nbytes += vnc_rre32(session, (FAR struct rre_encode32_s *)rrect->data, rect,
									vnc_convert_rgb32_888(bgcolor));
				break;
