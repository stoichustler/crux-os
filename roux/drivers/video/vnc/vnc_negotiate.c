...
int vnc_negotiate(FAR struct vnc_session_s *session)
{
	...
	nsent = psock_send(&session->connect, sectype, sizeof(struct rfb_sectype_s), 0);
	if (nsent < 0) {
	...
	nsent = psock_send(&session->connect, sectypes, SIZEOF_RFB_SUPPORTED_SECTYPES_S(1), 0);
	if (nsent < 0) {
	...
	nrecvd = psock_recv(&session->connect, sectype, sizeof(struct rfb_selected_sectype_s), 0);
	if (nrecvd < 0) {
	...
	if (sectype->type != RFB_SECTYPE_NONE) {
		...
		nsent = psock_send(&session->connect, secresult, sizeof(struct rfb_sectype_result_s), 0);
		if (nsent < 0) {
		...
		nsent = psock_send(&session->connect, secfail, SIZEOF_RFB_SECTYPE_FAIL_S(len), 0);
		if (nsent < 0) {
	...
	nrecvd = psock_recv(&session->connect, session->inbuf, sizeof(struct rfb_clientinit_s), 0);
	if (nrecvd < 0) {
	...
	nsent = psock_send(&session->connect, serverinit, SIZEOF_RFB_SERVERINIT_S(len), 0);
	if (nsent < 0) {
	...
	nrecvd = psock_recv(&session->connect, setformat, sizeof(struct rfb_setpixelformat_s), 0);
	if (nrecvd < 0) {
	...
	nrecvd = psock_recv(&session->connect, encodings, CONFIG_VNCSERVER_INBUFFER_SIZE, 0);
	if (nrecvd < 0) {
