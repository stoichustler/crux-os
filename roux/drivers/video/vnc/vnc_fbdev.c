...
static int up_setcursor(FAR struct fb_vtable_s *vtable, FAR struct fb_setcursor_s *settings);
#endif

/* Update the host window when there is a change to the framebuffer */

static int up_updateearea(FAR struct fb_vtable_s *vtable, FAR const struct fb_area_s *area);
...
static int vnc_start_server(int display)
{
	...
	ret = kthread_create("vnc_server", CONFIG_VNCSERVER_PRIO, CONFIG_VNCSERVER_STACKSIZE,
						 vnc_server, argv);
	if (ret < 0) {
 ...
 *     CONFIG_VNCSERVER_KBDENCODE.  See include/roux/input/kbd_codec.h.
...
int vnc_fbinitialize(int display, vnc_kbdout_t kbdout, vnc_mouseout_t mouseout, FAR void *arg)
{
	FAR struct vnc_session_s *session;
	int ret;
...
int vnc_fb_register(int display)
{
	...
	ret = snprintf(devname, sizeof(devname), CONFIG_VNCSERVER_TOUCH_DEVNAME "%d", display);
	...
	ret = snprintf(devname, sizeof(devname), CONFIG_VNCSERVER_KBD_DEVNAME "%d", display);
	if (ret < 0) {
...
#ifdef CONFIG_FB_CMAP
		fbinfo->vtable.getcmap = up_getcmap, fbinfo->vtable.putcmap = up_putcmap,
#endif
#ifdef CONFIG_FB_HWCURSOR
		fbinfo->vtable.getcursor = up_getcursor, fbinfo->vtable.setcursor = up_setcursor,
#endif
#ifdef CONFIG_FB_SYNC
		fbinfo->vtable.waitforvsync = up_waitforsync;
#endif
		fbinfo->vtable.updatearea = up_updateearea, fbinfo->display = display;
