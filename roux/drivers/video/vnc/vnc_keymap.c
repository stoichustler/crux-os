...
#ifdef CONFIG_VNCSERVER_KBDENCODE
static inline int vnc_kbd_encode(FAR uint8_t *buffer, uint8_t keycode, uint8_t terminator)
{
	*buffer++ = ASCII_ESC;
...
static int vnc_kbd_lookup(FAR const struct vnc_keymap_s *table, unsigned int nelem, uint16_t keysym)
{
	int i;
