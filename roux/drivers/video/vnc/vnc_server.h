...
#define CONFIG_VNCSERVER_NAME "ROUX"
#endif
...
struct vnc_session_s {
	...
	volatile uint8_t colorfmt; /* Remote color format (See include/roux/fb.h) */
...
int vnc_client_pixelformat(FAR struct vnc_session_s *session, FAR struct rfb_pixelfmt_s *pixelfmt);
...
int vnc_update_rectangle(FAR struct vnc_session_s *session, FAR const struct fb_area_s *rect,
						 bool change);
 ...
 *
 * Description:
 *   Map the receive X11 keysym into something understood by ROUX and route
 *   that through NX to the appropriate window.
 *
 * Input Parameters:
 *   session - An instance of the session structure.
 *   keysym  - The X11 keysym value (see include/roux/inputx11_keysymdef)
...
void vnc_key_map(FAR struct vnc_session_s *session, uint16_t keysym, bool keydown);
...
int vnc_touch_register(FAR const char *devpath, FAR struct vnc_session_s *session);
...
void vnc_touch_unregister(FAR struct vnc_session_s *session, FAR const char *devpath);
...
int vnc_kbd_register(FAR const char *devpath, FAR struct vnc_session_s *session);
...
void vnc_kbd_unregister(FAR struct vnc_session_s *session, FAR const char *devpath);
...
int vnc_colors(FAR struct vnc_session_s *session, FAR struct fb_area_s *rect,
