...
int vnc_read_remainder(FAR struct vnc_session_s *session, size_t msglen, size_t offset)
{
	ssize_t nrecvd;
	size_t ntotal;
	...
	for (ntotal = 0; ntotal < msglen; offset += nrecvd, ntotal += nrecvd) {
		...
		nrecvd = psock_recv(&session->connect, &session->inbuf[offset], msglen - ntotal, 0);
		if (nrecvd < 0) {
...
int vnc_receiver(FAR struct vnc_session_s *session)
{
	...
	tv.tv_sec = 0;
	tv.tv_usec = 0;
	ret = psock_setsockopt(&session->connect, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(struct timeval));
	if (ret < 0) {
	...
	for (;;) {
		...
		case RFB_SETPIXELFMT_MSG: /* SetPixelFormat */
		{
			...
			ret = vnc_read_remainder(session, sizeof(struct rfb_setpixelformat_s) - 1, 1);
			if (ret < 0) {
		...
		case RFB_SETENCODINGS_MSG: /* SetEncodings */
		{
			...
			ret = vnc_read_remainder(session, SIZEOF_RFB_SERVERINIT_S(0) - 1, 1);
			if (ret < 0) {
				...
				ret = vnc_read_remainder(session, nencodings * sizeof(uint32_t),
		...
		case RFB_FBUPDATEREQ_MSG: /* FramebufferUpdateRequest */
		{
			...
			ret = vnc_read_remainder(session, sizeof(struct rfb_fbupdatereq_s) - 1, 1);
			if (ret < 0) {
				gerr("ERROR: "
		...
		case RFB_KEYEVENT_MSG: /* KeyEvent */
		{
			...
			ret = vnc_read_remainder(session, sizeof(struct rfb_keyevent_s) - 1, 1);
			if (ret < 0) {
				...
				session->kbdout(session->arg, keyevent->down, (FAR const uint8_t *)keyevent->key);

#else
				vnc_key_map(session, rfb_getbe32(keyevent->key), (bool)keyevent->down);
#endif
			}
		} break;
		...
		case RFB_POINTEREVENT_MSG: /* PointerEvent */
		{
			...
			ret = vnc_read_remainder(session, sizeof(struct rfb_pointerevent_s) - 1, 1);
			if (ret < 0) {
				...
				session->mouseout(session->arg, (fb_coord_t)rfb_getbe16(event->xpos),
								  (fb_coord_t)rfb_getbe16(event->ypos), buttons);
			}
		} break;
		...
		case RFB_CLIENTCUTTEXT_MSG: /* ClientCutText */
		{
			...
			ret = vnc_read_remainder(session, SIZEOF_RFB_CLIENTCUTTEXT_S(0) - 1, 1);
			if (ret < 0) {
				...
				ret = vnc_read_remainder(session, length, SIZEOF_RFB_CLIENTCUTTEXT_S(0));
				if (ret < 0) {
