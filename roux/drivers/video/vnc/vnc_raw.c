...
static size_t vnc_copy8(FAR struct vnc_session_s *session, fb_coord_t row, fb_coord_t col,
						fb_coord_t height, fb_coord_t width, vnc_convert8_t convert)
{
...
static size_t vnc_copy16(FAR struct vnc_session_s *session, fb_coord_t row, fb_coord_t col,
						 fb_coord_t height, fb_coord_t width, vnc_convert16_t convert)
{
	FAR struct rfb_framebufferupdate_s *update;
	FAR const lfb_color_t *srcleft;
	FAR const lfb_color_t *src;
...
static size_t vnc_copy32(FAR struct vnc_session_s *session, fb_coord_t row, fb_coord_t col,
						 fb_coord_t height, fb_coord_t width, vnc_convert32_t convert)
{
	FAR struct rfb_framebufferupdate_s *update;
	FAR const lfb_color_t *srcleft;
	FAR const lfb_color_t *src;
...
int vnc_raw(FAR struct vnc_session_s *session, FAR struct fb_area_s *rect)
{
	...
	for (y = rect->y; srcheight > 0 && colorfmt == session->colorfmt;
		...
		for (width = srcwidth, x = rect->x; width > 0 && colorfmt == session->colorfmt;
			...
			{
				size = vnc_copy32(session, y, x, updheight, updwidth, convert.bpp32);
			}
