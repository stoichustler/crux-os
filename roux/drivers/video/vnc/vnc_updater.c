...
static const struct fb_area_s g_wholescreen = {0, 0, CONFIG_VNCSERVER_SCREENWIDTH,
...
#ifdef VNCSERVER_SEM_DEBUG
static void vnc_sem_debug(FAR struct vnc_session_s *session, FAR const char *msg,
						  unsigned int unattached)
{
...
static FAR struct vnc_fbupdate_s *vnc_alloc_update(FAR struct vnc_session_s *session)
{
	FAR struct vnc_fbupdate_s *update;
	irqstate_t flags;
...
static void vnc_free_update(FAR struct vnc_session_s *session, FAR struct vnc_fbupdate_s *update)
{
	irqstate_t flags;
	int sval;
...
static FAR struct vnc_fbupdate_s *vnc_remove_queue(FAR struct vnc_session_s *session)
{
...
static void vnc_add_queue(FAR struct vnc_session_s *session, FAR struct vnc_fbupdate_s *rect)
{
	irqstate_t flags;
	int sval;
...
static FAR void *vnc_updater(FAR void *arg)
{
	...
	while (session->state == VNCSERVER_RUNNING) {
		...
		updinfo("Dequeued {(%d, %d),(%d, %d)}\n", srcrect->rect.x, srcrect->rect.y, srcrect->rect.w,
				srcrect->rect.h);
						 ...
						 bool change)
{
	...
	if (intersection.w && intersection.h) {
		...
		whupd = (memcmp(&intersection, &g_wholescreen, sizeof(struct fb_area_s)) == 0);
