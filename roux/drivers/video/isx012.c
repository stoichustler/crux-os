...
#define VALIDATE_VALUE(val, min, max, step)                                                        \
	(((val >= min) && (val <= max) && (((val - min) % step) == 0) ? OK : -ERANGE))
...
static uint16_t isx012_getreg(FAR isx012_dev_t *priv, uint16_t regaddr, uint16_t regsize);
static int isx012_putreg(FAR isx012_dev_t *priv, uint16_t regaddr, uint32_t regval,
						 uint16_t regsize);
static int isx012_putreglist(FAR isx012_dev_t *priv, FAR const isx012_reg_t *reglist,
							 size_t nentries);
#ifdef ISX012_CHECK_IN_DETAIL
static int isx012_chipid(FAR struct i2c_master_s *i2c);
#endif

static int isx012_chk_int_state(FAR isx012_dev_t *priv, uint8_t sts, uint32_t delay_time,
								uint32_t wait_time, uint32_t timeout);
static int isx012_set_mode_param(FAR isx012_dev_t *priv, imgsensor_stream_type_t type,
								 uint8_t nr_fmt, FAR imgsensor_format_t *fmt,
								 FAR imgsensor_interval_t *interval);
static int isx012_change_camera_mode(FAR isx012_dev_t *priv, uint8_t mode);
static int isx012_change_device_state(FAR isx012_dev_t *priv, isx012_state_t state);
static int isx012_replace_frameinterval_to_regval(FAR imgsensor_interval_t *interval);
static int8_t isx012_get_maximum_fps(uint8_t nr_datafmt, FAR imgsensor_format_t *datafmt);
...
static int isx012_validate_frame_setting(FAR struct imgsensor_s *sensor,
										 ...
										 FAR imgsensor_interval_t *interval);
static int isx012_start_capture(FAR struct imgsensor_s *sensor, imgsensor_stream_type_t type,
								uint8_t nr_datafmt, FAR imgsensor_format_t *datafmts,
								FAR imgsensor_interval_t *interval);
static int isx012_stop_capture(FAR struct imgsensor_s *sensor, imgsensor_stream_type_t type);
static int isx012_get_supported_value(FAR struct imgsensor_s *sensor, uint32_t id,
									  FAR imgsensor_supported_value_t *value);
static int isx012_get_value(FAR struct imgsensor_s *sensor, uint32_t id, uint32_t size,
							FAR imgsensor_value_t *value);
static int isx012_set_value(FAR struct imgsensor_s *sensor, uint32_t id, uint32_t size,
...
static const isx012_reg_t g_isx012_def_init[] = {
	...
	{PLL_CKSEL, 0x00, 0x01},		{SRCCK_DIV, 0x00, 0x01},
#endif
	{DRIVABILITY, 0xaa, 0x01},		{VIFCONFIG, 0x0200, 0x02},
	{YUVCONFIG_TN, 0xff0a, 0x02},	{ILCODELEN, 0x00, 0x01},
	{AFMODE_MONI, 0x01, 0x01},		{YUVCONFIG, 0xff6a, 0x02},
	{VIF_REC601_Y, 0x10fe, 0x02},	{VIF_REC601_C, 0x10f0, 0x02},
	{HSENS_MODE_SEL, 0x11, 0x01},	{VIF_CLKCONFIG1, 0x30, 0x01},
	{VIF_CLKCONFIG2, 0x30, 0x01},	{VIF_CLKCONFIG3, 0x30, 0x01},
	{VIF_CLKCONFIG4, 0x30, 0x01},	{VIF_CLKCONFIG5, 0x30, 0x01},
	{VIF_CLKCONFIG6, 0x30, 0x01},	{VIF_CLKCONFIG7, 0x30, 0x01},
	{VIF_CLKCONFIG8, 0x30, 0x01},	{VIF_CLKCONFIG9, 0x30, 0x01},
	{VIF_CLKCONFIG10, 0x30, 0x01},	{VIF_CLKCONFIG11, 0x30, 0x01},
	{VIF_CLKCONFIG12, 0x30, 0x01},	{VIF_CLKCONFIG13, 0x11, 0x01},
	{VIF_CLKCONFIG14, 0x11, 0x01},	{VIF_CLKCONFIG15, 0x11, 0x01},
	{VIF_CLKCONFIG16, 0x11, 0x01},
#ifdef ISX012_NOT_USE_NSTBY
	{INCK_SET, 0x17, 0x01}, /* INCK_SET */
#endif
	{FRM_FIX_SN1_2, 0xff, 0x01}, /* Fix framerate */
	{FAST_MODECHG_EN, 0x01, 0x01},	{FAST_SHT_MODE_SEL, 0x01, 0x01},
	{CAP_HALF_AE_CTRL, 0x07, 0x01}, /* HAFREL=HIGHSPEED, CAP=Auto  */
	{HALF_AWB_CTRL, 0x01, 0x01},	{AESPEED_FAST, 0x0f, 0x01},
	{FASTMOVE_TIMEOUT, 0x2d, 0x01}, {YGAMMA_MODE, 0x01, 0x01},
	{INT_QLTY2, 0x50, 0x01},		{JPEG_PRED_MODE, 0x00, 0x01},
};
...
static const isx012_reg_t g_isx012_shd_thresholds[] = {
	{SHD_INP_TH_HB_H_R2, 0x1478, 2}, {SHD_INP_TH_HB_L_R2, 0x1380, 2},
	{SHD_INP_TH_LB_H_R2, 0x10cc, 2}, {SHD_INP_TH_LB_L_R2, 0x1004, 2},
	{SHD_INP_TH_HB_H_RB, 0x10cc, 2}, {SHD_INP_TH_HB_L_RB, 0x1004, 2},
	{SHD_INP_TH_LB_H_RB, 0x0000, 2}, {SHD_INP_TH_LB_L_RB, 0x0000, 2},
};
...
static const int32_t g_isx012_colorfx_actual[] = {
	IMGSENSOR_COLORFX_NONE,		IMGSENSOR_COLORFX_BW,	  IMGSENSOR_COLORFX_SEPIA,
	IMGSENSOR_COLORFX_NEGATIVE, IMGSENSOR_COLORFX_SKETCH, IMGSENSOR_COLORFX_SOLARIZATION,
	IMGSENSOR_COLORFX_PASTEL};

static const uint8_t g_isx012_colorfx_regval[] = {
	REGVAL_EFFECT_NONE,	  REGVAL_EFFECT_MONOTONE,	  REGVAL_EFFECT_SEPIA, REGVAL_EFFECT_NEGPOS,
...
static const uint8_t g_isx012_presetwb_regval[] = {
	REGVAL_AWB_ATM,			 REGVAL_AWB_LIGHTBULB,	   REGVAL_AWB_FLUORESCENTLIGHT,
	REGVAL_AWB_CLEARWEATHER, REGVAL_AWB_CLOUDYWEATHER, REGVAL_AWB_SHADE};

static const int32_t g_isx012_photometry_actual[] = {
	IMGSENSOR_EXPOSURE_METERING_AVERAGE, IMGSENSOR_EXPOSURE_METERING_CENTER_WEIGHTED,
	IMGSENSOR_EXPOSURE_METERING_SPOT, IMGSENSOR_EXPOSURE_METERING_MATRIX};

static const uint8_t g_isx012_photometry_regval[] = {
	REGVAL_PHOTOMETRY_AVERAGE, REGVAL_PHOTOMETRY_CENTERWEIGHT, REGVAL_PHOTOMETRY_SPOT,
	REGVAL_PHOTOMETRY_MULTIPATTERN};

static const int32_t g_isx012_iso_actual[] = {
	25 * 1000,	32 * 1000,	40 * 1000,	 50 * 1000,	  64 * 1000,  80 * 1000,  100 * 1000,
	125 * 1000, 160 * 1000, 200 * 1000,	 250 * 1000,  320 * 1000, 400 * 1000, 500 * 1000,
	640 * 1000, 800 * 1000, 1000 * 1000, 1250 * 1000, 1600 * 1000};

static const uint8_t g_isx012_iso_regval[] = {
	REGVAL_ISO_25,	REGVAL_ISO_32,	 REGVAL_ISO_40,	  REGVAL_ISO_50,  REGVAL_ISO_64,
	REGVAL_ISO_80,	REGVAL_ISO_100,	 REGVAL_ISO_125,  REGVAL_ISO_160, REGVAL_ISO_200,
	REGVAL_ISO_250, REGVAL_ISO_320,	 REGVAL_ISO_400,  REGVAL_ISO_500, REGVAL_ISO_640,
	REGVAL_ISO_800, REGVAL_ISO_1000, REGVAL_ISO_1250, REGVAL_ISO_1600};
						 ...
						 uint16_t regsize)
{
	...
	ret = i2c_write(priv->i2c, &priv->i2c_cfg, (FAR uint8_t *)buffer, regsize + 2);
	if (ret < 0) {
		verr("i2c_write failed: %d\n", ret);
	}
...
static int isx012_replace_fmt_to_regval(uint8_t nr_fmt, FAR imgsensor_format_t *fmt)
{
	int ret;
...
static bool is_movie_needed(uint8_t fmt, uint8_t fps)
{
	...
	if ((fmt == IMGSENSOR_PIX_FMT_UYVY) || (fmt == IMGSENSOR_PIX_FMT_RGB565)) {
...
static void resize_for_clip(uint8_t nr_fmt, FAR imgsensor_format_t *fmt, FAR isx012_rect_t *clip,
							FAR uint16_t *w, FAR uint16_t *h, FAR uint16_t *s_w, FAR uint16_t *s_h)
{
	...
	*h = (clip->height == 0) ? fmt[IMGSENSOR_FMT_MAIN].height : clip->height;
	...
	if (nr_fmt > 1) {
		*s_w = fmt[IMGSENSOR_FMT_SUB].width;
		if (clip->width > 0) {
			*s_w = (uint32_t)*s_w * clip->width / fmt[IMGSENSOR_FMT_MAIN].width;
		}

		*s_h = fmt[IMGSENSOR_FMT_SUB].height;
		if (clip->height > 0) {
			*s_h = (uint32_t)*s_h * clip->height / fmt[IMGSENSOR_FMT_MAIN].height;
		}
	}
}

static void calc_clip_regval(uint16_t pos, uint16_t clip_sz, uint16_t frm_sz, uint16_t basis_sz,
							 FAR uint32_t *ratio, FAR int32_t *offset)
{
...
static bool is_clipped(FAR isx012_rect_t *clip)
{
	...
	if ((clip->left == 0) && (clip->top == 0) && (clip->width == 0) && (clip->height == 0)) {
		return false;
	}
...
static void activate_clip(FAR isx012_dev_t *priv, uint16_t w, uint16_t h, FAR isx012_rect_t *clip)
{
	uint8_t hvfree = 0;
	...
	if (is_clipped(clip)) {
		...
		calc_clip_regval(clip->left, clip->width, w, BASE_HSIZE_FOR_CLIP_OFFSET, &r_x, &x);
		calc_clip_regval(clip->top, clip->height, h, BASE_VSIZE_FOR_CLIP_OFFSET, &r_y, &y);
								 ...
								 FAR imgsensor_interval_t *interval)
{
	...
	activate_clip(priv, fmt[IMGSENSOR_FMT_MAIN].width, fmt[IMGSENSOR_FMT_MAIN].height, clip);
...
static int isx012_change_camera_mode(FAR isx012_dev_t *priv, uint8_t mode)
{
	...
	if (mode != isx012_getreg(priv, MODESEL, sizeof(mode))) {
		...
		ret = isx012_chk_int_state(priv, CM_CHANGED_STS, CAMERA_MODE_DELAY_TIME,
								   CAMERA_MODE_WAIT_TIME, CAMERA_MODE_TIMEOUT);
		if (ret != 0) {
			return ret;
		}
	}
	...
	if (mode != REGVAL_MODESEL_HREL) {
		...
		for (i = 0; i < mask_num; i++) {
			...
			ret =
				isx012_chk_int_state(priv, VINT_STS, VINT_DELAY_TIME, VINT_WAIT_TIME, VINT_TIMEOUT);
			if (ret != 0) {
				return ret;
			}
		}
...
static int8_t isx012_get_maximum_fps(uint8_t nr_fmt, FAR imgsensor_format_t *fmt)
{
	...
	case IMGSENSOR_PIX_FMT_RGB565: /* RGB565 */
		...
		if ((main_w < OUT_YUV_HSIZE_MIN) || (main_h < OUT_YUV_VSIZE_MIN) ||
			(main_w > OUT_YUV_15FPS_HSIZE_MAX) || (main_h > OUT_YUV_15FPS_VSIZE_MAX)) {
			/* IN frame size is out of range */

			return -EINVAL;
		} else if ((main_w <= OUT_YUV_120FPS_HSIZE_MAX) && (main_h <= OUT_YUV_120FPS_VSIZE_MAX)) {
	...
	case IMGSENSOR_PIX_FMT_JPEG: /* JPEG */
		...
		if ((main_w < OUT_JPG_HSIZE_MIN) || (main_h < OUT_JPG_VSIZE_MIN) ||
			(main_w > OUT_JPG_15FPS_HSIZE_MAX) || (main_h > OUT_JPG_15FPS_VSIZE_MAX)) {
			/* IN frame size is out of range */

			return -EINVAL;
		} else if ((main_w <= OUT_JPG_120FPS_HSIZE_MAX) && (main_h <= OUT_JPG_120FPS_VSIZE_MAX)) {
			max_fps = REGVAL_FPSTYPE_120FPS;
		} else if ((main_w <= OUT_JPG_60FPS_HSIZE_MAX) && (main_h <= OUT_JPG_60FPS_VSIZE_MAX)) {
			max_fps = REGVAL_FPSTYPE_60FPS;
		} else if ((main_w <= OUT_JPG_30FPS_HSIZE_MAX) && (main_h <= OUT_JPG_30FPS_VSIZE_MAX)) {
	...
	case IMGSENSOR_PIX_FMT_JPEG_WITH_SUBIMG: /* JPEG + sub image */
		...
		if (nr_fmt == 1) {
			...
			if (fmt[IMGSENSOR_FMT_SUB].pixelformat != IMGSENSOR_PIX_FMT_UYVY) {
		...
		if ((main_w < OUT_JPG_HSIZE_MIN) || (main_h < OUT_JPG_VSIZE_MIN) ||
			(main_w > OUT_JPGINT_15FPS_HSIZE_MAX) || (main_h > OUT_JPGINT_15FPS_VSIZE_MAX) ||
			(sub_w < OUT_YUV_HSIZE_MIN) || (sub_h < OUT_YUV_VSIZE_MIN) ||
			(sub_w > OUT_YUVINT_30FPS_HSIZE_MAX) || (sub_h > OUT_YUVINT_30FPS_VSIZE_MAX)) {
			/* IN frame size is out of range */
									  ...
									  FAR imgsensor_supported_value_t *value)
{
	...
	case IMGSENSOR_ID_COLORFX:
		...
		discrete->nr_values = ARRAY_NENTRIES(g_isx012_colorfx_actual);
	...
	case IMGSENSOR_ID_EXPOSURE_METERING:
		value->type = IMGSENSOR_CTRL_TYPE_INTEGER_MENU;
		discrete->nr_values = ARRAY_NENTRIES(g_isx012_photometry_actual);
		discrete->values = g_isx012_photometry_actual;
		discrete->default_value = IMGSENSOR_EXPOSURE_METERING_AVERAGE;
...
static void get_current_framesize(FAR struct isx012_dev_s *priv, FAR uint16_t *w, FAR uint16_t *h)
{
...
static uint32_t restore_spot_position(uint8_t regval, uint16_t w, uint16_t split)
{
							...
							FAR imgsensor_value_t *value)
{
	...
	switch (id) {
	case IMGSENSOR_ID_BRIGHTNESS:
		readvalue = isx012_getreg(priv, ISX012_REG_BRIGHTNESS, ISX012_SIZE_BRIGHTNESS);

		value->value32 = (int32_t)(int8_t)(0x00ff & readvalue);
		break;

	case IMGSENSOR_ID_CONTRAST:
		value->value32 = isx012_getreg(priv, ISX012_REG_CONTRAST, ISX012_SIZE_CONTRAST);
		break;

	case IMGSENSOR_ID_SATURATION:
		value->value32 = isx012_getreg(priv, ISX012_REG_SATURATION, ISX012_SIZE_SATURATION);
		break;

	case IMGSENSOR_ID_HUE:
		value->value32 = isx012_getreg(priv, ISX012_REG_HUE, ISX012_SIZE_HUE);
		break;

	case IMGSENSOR_ID_AUTO_WHITE_BALANCE:
		readvalue = isx012_getreg(priv, ISX012_REG_AUTOWB, ISX012_SIZE_AUTOWB);
	...
	case IMGSENSOR_ID_GAMMA_CURVE:
		...
		for (cnt = 0; cnt < ISX012_ELEMS_GAMMACURVE; cnt++) {
			*read_dst = isx012_getreg(priv, read_src, ISX012_SIZE_GAMMACURVE);
	...
	case IMGSENSOR_ID_EXPOSURE:
		readvalue = isx012_getreg(priv, ISX012_REG_EXPOSURE, ISX012_SIZE_EXPOSURE);
	...
	case IMGSENSOR_ID_HFLIP_VIDEO:
		readvalue = isx012_getreg(priv, ISX012_REG_HFLIP, ISX012_SIZE_HFLIP);
	...
	case IMGSENSOR_ID_VFLIP_VIDEO:
		readvalue = isx012_getreg(priv, ISX012_REG_VFLIP, ISX012_SIZE_VFLIP);
	...
	case IMGSENSOR_ID_HFLIP_STILL:
		readvalue = isx012_getreg(priv, ISX012_REG_HFLIP_STILL, ISX012_SIZE_HFLIP_STILL);
	...
	case IMGSENSOR_ID_VFLIP_STILL:
		readvalue = isx012_getreg(priv, ISX012_REG_VFLIP_STILL, ISX012_SIZE_VFLIP_STILL);
	...
	case IMGSENSOR_ID_SHARPNESS:
		value->value32 = isx012_getreg(priv, ISX012_REG_SHARPNESS, ISX012_SIZE_SHARPNESS);
		break;

	case IMGSENSOR_ID_COLOR_KILLER:
		readvalue = isx012_getreg(priv, ISX012_REG_COLORKILLER, ISX012_SIZE_COLORKILLER);
	...
	case IMGSENSOR_ID_COLORFX:
		readvalue = isx012_getreg(priv, ISX012_REG_COLOREFFECT, ISX012_SIZE_COLOREFFECT);
	...
	case IMGSENSOR_ID_EXPOSURE_AUTO:
		readvalue = isx012_getreg(priv, ISX012_REG_EXPOSURETIME, ISX012_SIZE_EXPOSURETIME);

		value->value32 = readvalue ? IMGSENSOR_EXPOSURE_MANUAL : IMGSENSOR_EXPOSURE_AUTO;

		break;

	case IMGSENSOR_ID_EXPOSURE_ABSOLUTE:
		value->value32 = isx012_getreg(priv, ISX012_REG_EXPOSURETIME, ISX012_SIZE_EXPOSURETIME);

		if (value->value32 == REGVAL_EXPOSURETIME_AUTO) {
			exposure_time_lsb = isx012_getreg(priv, ISX012_REG_EXPOSUREAUTOVALUE_LSB,
											  ISX012_SIZE_EXPOSUREAUTOVALUE);
			exposure_time_msb = isx012_getreg(priv, ISX012_REG_EXPOSUREAUTOVALUE_MSB,
											  ISX012_SIZE_EXPOSUREAUTOVALUE);

			value->value32 = (uint16_t)(((exposure_time_msb << 16) | exposure_time_lsb) /
	...
	case IMGSENSOR_ID_WIDE_DYNAMIC_RANGE:
		readvalue = isx012_getreg(priv, ISX012_REG_YGAMMA, ISX012_SIZE_YGAMMA);
		value->value32 = readvalue ? 0 : 1;

		break;

	case IMGSENSOR_ID_ISO_SENSITIVITY:
		readvalue = isx012_getreg(priv, ISX012_REG_ISOAUTOVALUE, ISX012_SIZE_ISOAUTOVALUE);
	...
	case IMGSENSOR_ID_ISO_SENSITIVITY_AUTO:
		readvalue = isx012_getreg(priv, ISX012_REG_ISOAUTO, ISX012_SIZE_ISOAUTO);

		value->value32 = (readvalue == REGVAL_ISO_AUTO) ? IMGSENSOR_ISO_SENSITIVITY_AUTO
														: IMGSENSOR_ISO_SENSITIVITY_MANUAL;

		break;

	case IMGSENSOR_ID_EXPOSURE_METERING:
		readvalue = isx012_getreg(priv, ISX012_REG_PHOTOMETRY, ISX012_SIZE_PHOTOMETRY);

		ret = -EINVAL;
		for (cnt = 0; cnt < ARRAY_NENTRIES(g_isx012_photometry_regval); cnt++) {
	...
	case IMGSENSOR_ID_JPEG_QUALITY:
		value->value32 = isx012_getreg(priv, ISX012_REG_JPGQUALITY, ISX012_SIZE_JPGQUALITY);
		break;
...
static int set_clip(uint32_t size, FAR uint32_t *val, FAR isx012_rect_t *target)
{
	DEBUGASSERT(target);
							...
							imgsensor_value_t value)
{
	...
	switch (id) {
	case IMGSENSOR_ID_BRIGHTNESS:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_BRIGHTNESS, ISX012_MAX_BRIGHTNESS,
							 ISX012_STEP_BRIGHTNESS);
		if (ret != OK) {
			break;
		}

		ret = isx012_putreg(priv, ISX012_REG_BRIGHTNESS, value.value32, ISX012_SIZE_BRIGHTNESS);
		break;

	case IMGSENSOR_ID_CONTRAST:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_CONTRAST, ISX012_MAX_CONTRAST,
							 ISX012_STEP_CONTRAST);
		if (ret != OK) {
			break;
		}

		ret = isx012_putreg(priv, ISX012_REG_CONTRAST, value.value32, ISX012_SIZE_CONTRAST);
		break;

	case IMGSENSOR_ID_SATURATION:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_SATURATION, ISX012_MAX_SATURATION,
							 ISX012_STEP_SATURATION);
		if (ret != OK) {
			break;
		}

		ret = isx012_putreg(priv, ISX012_REG_SATURATION, value.value32, ISX012_SIZE_SATURATION);
		break;

	case IMGSENSOR_ID_HUE:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_HUE, ISX012_MAX_HUE, ISX012_STEP_HUE);
		if (ret != OK) {
			break;
		}

		ret = isx012_putreg(priv, ISX012_REG_HUE, value.value32, ISX012_SIZE_HUE);
		break;

	case IMGSENSOR_ID_AUTO_WHITE_BALANCE:
		ret =
			VALIDATE_VALUE(value.value32, ISX012_MIN_AUTOWB, ISX012_MAX_AUTOWB, ISX012_STEP_AUTOWB);
		if (ret != OK) {
			break;
		}

		regval = isx012_getreg(priv, ISX012_REG_AUTOWB, ISX012_SIZE_AUTOWB);
		...
		ret = isx012_putreg(priv, ISX012_REG_AUTOWB, regval, ISX012_SIZE_AUTOWB);
		break;
	...
	case IMGSENSOR_ID_GAMMA_CURVE:
		...
		for (cnt = 0; cnt < ISX012_ELEMS_GAMMACURVE; cnt++) {
			ret = VALIDATE_VALUE(*write_src, ISX012_MIN_GAMMACURVE, ISX012_MAX_GAMMACURVE,
								 ISX012_STEP_GAMMACURVE);
			if (ret != OK) {
				break;
			}

			ret = isx012_putreg(priv, write_dst, *write_src, ISX012_SIZE_GAMMACURVE);
	...
	case IMGSENSOR_ID_EXPOSURE:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_EXPOSURE, ISX012_MAX_EXPOSURE,
							 ISX012_STEP_EXPOSURE);
		if (ret != OK) {
			break;
		}

		ret = isx012_putreg(priv, ISX012_REG_EXPOSURE, value.value32, ISX012_SIZE_EXPOSURE);

		break;

	case IMGSENSOR_ID_HFLIP_VIDEO:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_HFLIP, ISX012_MAX_HFLIP, ISX012_STEP_HFLIP);
		if (ret != OK) {
			break;
		}

		regval = isx012_getreg(priv, ISX012_REG_HFLIP, ISX012_SIZE_HFLIP);
		...
		ret = isx012_putreg(priv, ISX012_REG_HFLIP, regval, ISX012_SIZE_HFLIP);
		break;

	case IMGSENSOR_ID_VFLIP_VIDEO:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_VFLIP, ISX012_MAX_VFLIP, ISX012_STEP_VFLIP);
		if (ret != OK) {
			break;
		}

		regval = isx012_getreg(priv, ISX012_REG_VFLIP, ISX012_SIZE_VFLIP);
		...
		ret = isx012_putreg(priv, ISX012_REG_VFLIP, regval, ISX012_SIZE_VFLIP);
		break;

	case IMGSENSOR_ID_HFLIP_STILL:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_HFLIP_STILL, ISX012_MAX_HFLIP_STILL,
							 ISX012_STEP_HFLIP_STILL);
		if (ret != OK) {
			break;
		}

		regval = isx012_getreg(priv, ISX012_REG_HFLIP_STILL, ISX012_SIZE_HFLIP_STILL);
		...
		ret = isx012_putreg(priv, ISX012_REG_HFLIP_STILL, regval, ISX012_SIZE_HFLIP_STILL);

		break;

	case IMGSENSOR_ID_VFLIP_STILL:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_VFLIP_STILL, ISX012_MAX_VFLIP_STILL,
							 ISX012_STEP_VFLIP_STILL);
		if (ret != OK) {
			break;
		}

		regval = isx012_getreg(priv, ISX012_REG_VFLIP_STILL, ISX012_SIZE_VFLIP_STILL);
		...
		ret = isx012_putreg(priv, ISX012_REG_VFLIP_STILL, regval, ISX012_SIZE_VFLIP_STILL);
		break;

	case IMGSENSOR_ID_SHARPNESS:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_SHARPNESS, ISX012_MAX_SHARPNESS,
							 ISX012_STEP_SHARPNESS);
		if (ret != OK) {
			break;
		}

		ret = isx012_putreg(priv, ISX012_REG_SHARPNESS, value.value32, ISX012_SIZE_SHARPNESS);
		break;

	case IMGSENSOR_ID_COLOR_KILLER:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_COLORKILLER, ISX012_MAX_COLORKILLER,
							 ISX012_STEP_COLORKILLER);
		if (ret != OK) {
			break;
		}

		ret = isx012_putreg(priv, ISX012_REG_COLORKILLER,
	...
	case IMGSENSOR_ID_COLORFX:
		for (cnt = 0; cnt < ARRAY_NENTRIES(g_isx012_colorfx_actual); cnt++) {
			if (g_isx012_colorfx_actual[cnt] == value.value32) {
				ret = isx012_putreg(priv, ISX012_REG_COLOREFFECT, g_isx012_colorfx_regval[cnt],
	...
	case IMGSENSOR_ID_EXPOSURE_AUTO:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_EXPOSUREAUTO, ISX012_MAX_EXPOSUREAUTO,
		...
		if (value.value32 == IMGSENSOR_EXPOSURE_AUTO) {
			...
			ret = isx012_putreg(priv, ISX012_REG_EXPOSURETIME, REGVAL_EXPOSURETIME_AUTO,
			...
			regval = (uint16_t)(((exposure_time_msb << 16) | exposure_time_lsb) /
								ISX012_UNIT_EXPOSURETIME_US);
			ret = isx012_putreg(priv, ISX012_REG_EXPOSURETIME, regval, ISX012_SIZE_EXPOSURETIME);
		}

		break;

	case IMGSENSOR_ID_EXPOSURE_ABSOLUTE:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_EXPOSURETIME, ISX012_MAX_EXPOSURETIME,
							 ISX012_STEP_EXPOSURETIME);
		if (ret != OK) {
			break;
		}

		ret = isx012_putreg(priv, ISX012_REG_EXPOSURETIME, value.value32, ISX012_SIZE_EXPOSURETIME);
		break;

	case IMGSENSOR_ID_WIDE_DYNAMIC_RANGE:
		ret =
			VALIDATE_VALUE(value.value32, ISX012_MIN_YGAMMA, ISX012_MAX_YGAMMA, ISX012_STEP_YGAMMA);
		if (ret != OK) {
			break;
		}
		...
		ret = isx012_putreg(priv, ISX012_REG_YGAMMA,
	...
	case IMGSENSOR_ID_ISO_SENSITIVITY:
		for (cnt = 0; cnt < ARRAY_NENTRIES(g_isx012_iso_actual); cnt++) {
			if (g_isx012_iso_actual[cnt] == value.value32) {
				ret =
					isx012_putreg(priv, ISX012_REG_ISO, g_isx012_iso_regval[cnt], ISX012_SIZE_ISO);
				break;
			}
		}
	...
	case IMGSENSOR_ID_ISO_SENSITIVITY_AUTO:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_ISOAUTO, ISX012_MAX_ISOAUTO,
							 ISX012_STEP_ISOAUTO);
		if (ret != OK) {
			break;
		}
		...
		if (value.value32 == IMGSENSOR_ISO_SENSITIVITY_AUTO) {
			ret = isx012_putreg(priv, ISX012_REG_ISOAUTO, REGVAL_ISO_AUTO, ISX012_SIZE_ISOAUTO);
		} else {
			/* In manual case, read auto adjust value and set it */

			regval = isx012_getreg(priv, ISX012_REG_ISOAUTOVALUE, ISX012_SIZE_ISOAUTOVALUE);
			ret = isx012_putreg(priv, ISX012_REG_ISO, regval, ISX012_SIZE_ISO);
		}

		break;

	case IMGSENSOR_ID_EXPOSURE_METERING:
		for (cnt = 0; cnt < ARRAY_NENTRIES(g_isx012_photometry_actual); cnt++) {
			if (g_isx012_photometry_actual[cnt] == value.value32) {
				ret = isx012_putreg(priv, ISX012_REG_PHOTOMETRY, g_isx012_photometry_regval[cnt],
	...
	case IMGSENSOR_ID_SPOT_POSITION:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_SPOTPOS, ISX012_MAX_SPOTPOS,
							 ISX012_STEP_SPOTPOS);
		if (ret != OK) {
			break;
		}
	...
	case IMGSENSOR_ID_AUTO_N_PRESET_WB:
		for (cnt = 0; cnt < ARRAY_NENTRIES(g_isx012_presetwb_actual); cnt++) {
			if (g_isx012_presetwb_actual[cnt] == value.value32) {
				ret = isx012_putreg(priv, ISX012_REG_PRESETWB, g_isx012_presetwb_regval[cnt],
	...
	case IMGSENSOR_ID_3A_LOCK:
		ret =
			VALIDATE_VALUE(value.value32, ISX012_MIN_3ALOCK, ISX012_MAX_3ALOCK, ISX012_STEP_3ALOCK);
		if (ret != OK) {
			break;
		}
		...
		if ((value.value32 & IMGSENSOR_LOCK_EXPOSURE) == IMGSENSOR_LOCK_EXPOSURE) {
			regval |= REGVAL_CPUEXT_BIT_AESTOP;
		}

		if ((value.value32 & IMGSENSOR_LOCK_WHITE_BALANCE) == IMGSENSOR_LOCK_WHITE_BALANCE) {
			regval |= REGVAL_CPUEXT_BIT_AWBSTOP;
		}

		ret = isx012_putreg(priv, ISX012_REG_3ALOCK, regval, ISX012_SIZE_3ALOCK);
	...
	case IMGSENSOR_ID_JPEG_QUALITY:
		ret = VALIDATE_VALUE(value.value32, ISX012_MIN_JPGQUALITY, ISX012_MAX_JPGQUALITY,
							 ISX012_STEP_JPGQUALITY);
		if (ret != OK) {
			break;
		}

		ret = isx012_putreg(priv, ISX012_REG_JPGQUALITY, value.value32, ISX012_SIZE_JPGQUALITY);
		break;
...
static int isx012_set_shd(FAR isx012_dev_t *priv)
{
	...
	for (unit_cnt = 0; unit_cnt < CXC_RGB_DATA_UNIT_NUM; unit_cnt++) {
		for (size_cnt = 0; size_cnt < CXC_RGB_DATA_UNIT_SIZE; size_cnt++) {
			ret = isx012_putreg(priv, CXC_RGB_UNIT(unit_cnt, size_cnt),
								g_isx012_cxc_rgb_data[unit_cnt][size_cnt], 1);
			if (ret < 0) {
	...
	for (unit_cnt = 0; unit_cnt < CXC_GRB_DATA_UNIT_NUM; unit_cnt++) {
		for (size_cnt = 0; size_cnt < CXC_GRB_DATA_UNIT_SIZE; size_cnt++) {
			ret = isx012_putreg(priv, CXC_GRB_UNIT(unit_cnt, size_cnt),
								g_isx012_cxc_grb_data[unit_cnt][size_cnt], 1);
			if (ret < 0) {
	...
	for (unit_cnt = 0; unit_cnt < SHD_RGB_DATA_UNIT_NUM; unit_cnt++) {
		for (size_cnt = 0; size_cnt < SHD_RGB_DATA_UNIT_SIZE; size_cnt++) {
			ret = isx012_putreg(priv, SHD_RGB_UNIT(unit_cnt, size_cnt),
								g_isx012_shd_rgb_data[unit_cnt][size_cnt], 1);
			if (ret < 0) {
	...
	for (unit_cnt = 0; unit_cnt < SHD_GRB_DATA_UNIT_NUM; unit_cnt++) {
		for (size_cnt = 0; size_cnt < SHD_GRB_DATA_UNIT_SIZE; size_cnt++) {
			ret = isx012_putreg(priv, SHD_GRB_UNIT(unit_cnt, size_cnt),
								g_isx012_shd_grb_data[unit_cnt][size_cnt], 1);
			if (ret < 0) {
	...
	for (unit_cnt = 0; unit_cnt < SHD_R1_DATA_UNIT_NUM; unit_cnt++) {
		for (size_cnt = 0; size_cnt < SHD_R1_DATA_UNIT_SIZE; size_cnt++) {
			ret = isx012_putreg(priv, SHD_R1_UNIT(unit_cnt, size_cnt),
								g_isx012_shd_r1_data[unit_cnt][size_cnt], 1);
			if (ret < 0) {
	...
	for (unit_cnt = 0; unit_cnt < SHD_R2_DATA_UNIT_NUM; unit_cnt++) {
		for (size_cnt = 0; size_cnt < SHD_R2_DATA_UNIT_SIZE; size_cnt++) {
			ret = isx012_putreg(priv, SHD_R2_UNIT(unit_cnt, size_cnt),
								g_isx012_shd_r2_data[unit_cnt][size_cnt], 1);
			if (ret < 0) {
	...
	for (unit_cnt = 0; unit_cnt < SHD_B2_DATA_UNIT_NUM; unit_cnt++) {
		for (size_cnt = 0; size_cnt < SHD_B2_DATA_UNIT_SIZE; size_cnt++) {
			ret = isx012_putreg(priv, SHD_B2_UNIT(unit_cnt, size_cnt),
								g_isx012_shd_b2_data[unit_cnt][size_cnt], 1);
			if (ret < 0) {
