...
static FAR struct circbuf_s *fb_get_panbuf(FAR struct fb_chardev_s *fb, int overlay);
static int fb_add_paninfo(FAR struct fb_chardev_s *fb, FAR const union fb_paninfo_u *info,
						  int overlay);
static int fb_clear_paninfo(FAR struct fb_chardev_s *fb, int overlay);
static int fb_open(FAR struct file *filep);
static int fb_close(FAR struct file *filep);
static ssize_t fb_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t fb_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int fb_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
static int fb_get_panelinfo(FAR struct fb_chardev_s *fb, FAR struct fb_panelinfo_s *panelinfo,
							int overlay);
static int fb_get_planeinfo(FAR struct fb_chardev_s *fb, FAR struct fb_planeinfo_s *pinfo,
							uint8_t display);
#ifdef CONFIG_FB_SYNC
static int fb_sem_wait(FAR struct fb_chardev_s *fb, FAR struct fb_priv_s *priv, int overlay);
static void fb_sem_post(FAR struct fb_chardev_s *fb, int overlay);
#endif

#ifdef CONFIG_BUILD_KERNEL
static int fb_munmap(FAR struct task_group_s *group, FAR struct mm_map_entry_s *entry,
					 FAR void *start, size_t length);
#endif

#ifdef CONFIG_VIDEO_FB_SPLASHSCREEN
static int fb_splashscreen(FAR struct fb_videoinfo_s *vinfo, FAR struct fb_planeinfo_s *pinfo);
static int fb_splash_fill(FAR struct fb_videoinfo_s *vinfo, FAR struct fb_planeinfo_s *pinfo,
						  uint32_t colour);
#endif
						  ...
						  uint32_t colour)
{
	...
	int rgb_colour;
	int y;
#if !defined(CONFIG_VIDEO_FB_SPLASHSCREEN_MONO) && !defined(CONFIG_VIDEO_FB_SPLASHSCREEN_GREY) &&  \
...
#elif defined(CONFIG_VIDEO_FB_SPLASHSCREEN_BPP8)
	rgb_colour = MKRGB(r, g, b);
#elif defined(CONFIG_VIDEO_FB_SPLASHSCREEN_MONO) || defined(CONFIG_VIDEO_FB_SPLASHSCREEN_GREY)
...
static int fb_splashscreen(FAR struct fb_videoinfo_s *vinfo, FAR struct fb_planeinfo_s *pinfo)
{
	...
	dst = pinfo->fbmem + (pinfo->stride * ((vinfo->yres - g_splscr.height) / 2)) +
		  (((vinfo->xres - g_splscr.width) / 2) * sizeof(fb_pixel_t));
...
static int fb_open(FAR struct file *filep)
{
	...
	if (fb->head == NULL) {
		if (fb->vtable->open != NULL && (ret = fb->vtable->open(fb->vtable)) < 0) {
			goto err_fb;
		}
	}
...
static int fb_close(FAR struct file *filep)
{
	...
	for (prev = NULL, curr = fb->head; curr && curr != priv; prev = curr, curr = curr->flink)
		;
...
static int fb_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case FBIOSET_FRAMERATE: {
		DEBUGASSERT(fb->vtable != NULL && fb->vtable->setframerate != NULL);
	...
	case FBIOGET_VSCREENINFO: {
		...
		FAR struct fb_var_screeninfo *varinfo = (FAR struct fb_var_screeninfo *)((uintptr_t)arg);
	...
	case FBIOGET_FSCREENINFO: {
		struct fb_videoinfo_s vinfo;
		struct fb_planeinfo_s pinfo;
		FAR struct fb_fix_screeninfo *fixinfo = (FAR struct fb_fix_screeninfo *)((uintptr_t)arg);
		...
		strlcpy(fixinfo->id, (FAR const char *)vinfo.moduleinfo, sizeof(fixinfo->id));
#endif
		fixinfo->smem_start = (unsigned long)pinfo.fbmem;
		fixinfo->smem_len = pinfo.fblen;
		fixinfo->type =
			FB_ISYUVPLANAR(vinfo.fmt) ? FB_TYPE_INTERLEAVED_PLANES : FB_TYPE_PACKED_PIXELS;
		fixinfo->visual = FB_ISMONO(vinfo.fmt) ? FB_VISUAL_MONO10 : FB_VISUAL_TRUECOLOR;
...
static int fb_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map)
{
	...
	if (map->offset >= 0 && map->offset < panelinfo.fblen && map->length &&
		map->offset + map->length <= panelinfo.fblen) {
#ifdef CONFIG_BUILD_KERNEL
		map->vaddr = vm_map_region((uintptr_t)panelinfo.fbmem + map->offset, panelinfo.fblen);
							...
							int overlay)
{
	...
	if (overlay != FB_NO_OVERLAY) {
		...
		panelinfo->fbcount = oinfo.yres_virtual == 0 ? 1 : (oinfo.yres_virtual / oinfo.yres);
	...
	panelinfo->fbcount = pinfo.yres_virtual == 0 ? 1 : (pinfo.yres_virtual / vinfo.yres);
...
int fb_register_device(int display, int plane, FAR struct fb_vtable_s *vtable)
{
	...
	for (i = 0; i < fb->paninfo_count; i++) {
		...
		ret = circbuf_init(&(fb->paninfo[i].buf), NULL,
						   panelinfo.fbcount * sizeof(union fb_paninfo_u));
