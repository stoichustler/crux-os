...
static FAR const char *goldfish_camera_get_driver_name(FAR struct imgsensor_s *sensor);
static int goldfish_camera_validate_frame_setting(FAR struct imgsensor_s *sensor,
...
static int goldfish_camera_data_validate_frame_setting(FAR struct imgdata_s *data,
													   uint8_t nr_datafmt,
													   FAR imgdata_format_t *datafmt,
													   FAR imgdata_interval_t *interval);
static int goldfish_camera_data_start_capture(FAR struct imgdata_s *data, uint8_t nr_datafmt,
											  FAR imgdata_format_t *datafmt,
											  FAR imgdata_interval_t *interval,
											  imgdata_capture_t callback, FAR void *arg);
static int goldfish_camera_data_stop_capture(FAR struct imgdata_s *data);
static int goldfish_camera_data_set_buf(FAR struct imgdata_s *data, uint8_t nr_datafmts,
...
static int goldfish_camera_read_fully(FAR struct file *file, FAR void *buffer, size_t size)
{
...
static int goldfish_camera_write_fully(FAR struct file *file, FAR const void *buffer, size_t size)
{
...
static int goldfish_camera_send(FAR struct file *file, FAR const char *format, ...)
{
	char buf[256];
	va_list ap;
	int len;
...
static ssize_t goldfish_camera_recv(FAR struct file *file, FAR void **data)
{
	char buf[9];
	size_t payload_size;
	bool empty = !*data;
...
static ssize_t goldfish_camera_get_list(FAR goldfish_camera_priv_t **priv, size_t size)
{
	...
	ret = file_open(&file, CONFIG_GOLDFISH_CAMERA_PIPE_PATH, O_RDWR | O_CLOEXEC);
	if (ret < 0) {
		verr("Failed to open: %s: %d\n", CONFIG_GOLDFISH_CAMERA_PIPE_PATH, ret);
		return ret;
	}

	ret = goldfish_camera_send(&file, "pipe:%s:%s", g_qemudservice, g_cameraservice);
	if (ret < 0) {
		goto out;
	}
	...
	while ((data = strstr(data, "name")) != NULL) {
		...
		sscanf(data, "name=%s channel=%" SCNu32 " pix=%" SCNu32 " dir=%s framedims=%s",
			   priv[count]->info.name, &priv[count]->info.channel, &priv[count]->info.pix,
			   priv[count]->info.dir, sizelist);

		sscanf(sizedata, "%" SCNu16 "x%" SCNu16 "", &priv[count]->info.size[sizecount].width,
...
{
	FAR goldfish_camera_priv_t *priv =
		(FAR goldfish_camera_priv_t *)((uintptr_t)strtoul(argv[1], NULL, 16));
	int ret;
...
reload:
		ret = goldfish_camera_recv(&priv->file, (FAR void **)&priv->next_buf);
		if (ret < 0) {
			return ret;
		} else if (ret == 0) {
			goto reload;
...
static FAR const char *goldfish_camera_get_driver_name(FAR struct imgsensor_s *sensor)
{
	FAR goldfish_camera_priv_t *priv = container_of(sensor, goldfish_camera_priv_t, sensor);
...
static int goldfish_camera_data_init(FAR struct imgdata_s *data)
{
	...
	ret = file_open(&priv->file, CONFIG_GOLDFISH_CAMERA_PIPE_PATH, O_RDWR | O_CLOEXEC);
	if (ret < 0) {
		verr("Failed to open: %s: %d\n", CONFIG_GOLDFISH_CAMERA_PIPE_PATH, ret);
		return ret;
	}
	...
	ret = goldfish_camera_send(&priv->file, "pipe:%s:%s:name=%s", g_qemudservice, g_cameraservice,
							   priv->info.name);
	if (ret < 0) {
		goto err;
	}
	...
	ret = kthread_create("goldfish_camera_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_DEFAULT_TASK_STACKSIZE, goldfish_camera_thread, argv);
	if (ret < 0) {
		goto err;
	}
...
static int goldfish_camera_data_validate_buf(FAR uint8_t *addr, uint32_t size)
{
													   ...
													   FAR imgdata_interval_t *interval)
{
	...
	for (i = 0; i < GOLDFISH_CAMERA_SIZE_MAX_NUMBER; i++) {
		if (datafmt[IMGDATA_FMT_MAIN].width == priv->info.size[i].width &&
			...
			imgdata_fmt_to_v4l2(datafmt[IMGDATA_FMT_MAIN].pixelformat) == priv->info.pix) {
			return 0;
		}
	}
											  ...
											  imgdata_capture_t callback, FAR void *arg)
{
	...
	ret = goldfish_camera_send(&priv->file, "start dim=%dx%d pix=%d",
							   datafmt[IMGDATA_FMT_MAIN].width, datafmt[IMGDATA_FMT_MAIN].height,
