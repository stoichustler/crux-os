...
static int codec_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map);
static int codec_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);

/* Common function */

static FAR codec_type_inf_t *codec_get_type_inf(FAR struct codec_file_s *cfile, int type);

/* ioctl function for each cmds of ioctl */

static int codec_querycap(FAR struct file *filep, FAR struct v4l2_capability *cap);
static int codec_reqbufs(FAR struct file *filep, FAR struct v4l2_requestbuffers *reqbufs);
...
static int codec_g_fmt(FAR struct file *filep, FAR struct v4l2_format *fmt);
static int codec_s_fmt(FAR struct file *filep, FAR struct v4l2_format *fmt);
static int codec_try_fmt(FAR struct file *filep, FAR struct v4l2_format *fmt);
static int codec_g_parm(FAR struct file *filep, FAR struct v4l2_streamparm *parm);
static int codec_s_parm(FAR struct file *filep, FAR struct v4l2_streamparm *parm);
static int codec_streamon(FAR struct file *filep, FAR enum v4l2_buf_type *type);
static int codec_streamoff(FAR struct file *filep, FAR enum v4l2_buf_type *type);
static int codec_g_selection(FAR struct file *filep, FAR struct v4l2_selection *clip);
static int codec_s_selection(FAR struct file *filep, FAR struct v4l2_selection *clip);
static int codec_g_ext_ctrls(FAR struct file *filep, FAR struct v4l2_ext_controls *ctrls);
static int codec_s_ext_ctrls(FAR struct file *filep, FAR struct v4l2_ext_controls *ctrls);
static int codec_enum_fmt(FAR struct file *filep, FAR struct v4l2_fmtdesc *fmt);
static int codec_cropcap(FAR struct file *filep, FAR struct v4l2_cropcap *cropcap);
static int codec_dqevent(FAR struct file *filep, FAR struct v4l2_event *event);
static int codec_subscribe_event(FAR struct file *filep, FAR struct v4l2_event_subscription *sub);
static int codec_decoder_cmd(FAR struct file *filep, FAR struct v4l2_decoder_cmd *cmd);
static int codec_encoder_cmd(FAR struct file *filep, FAR struct v4l2_encoder_cmd *cmd);
...
static int codec_dqbuf(FAR struct file *filep, FAR struct v4l2_buffer *buf)
{
	...
	vinfo("%s dequeue done\n", V4L2_TYPE_IS_OUTPUT(buf->type) ? "output" : "capture");
...
int codec_dqevent(FAR struct file *filep, FAR struct v4l2_event *event)
{
	FAR codec_file_t *cfile = filep->f_priv;
...
int codec_subscribe_event(FAR struct file *filep, FAR struct v4l2_event_subscription *sub)
{
	FAR struct inode *inode = filep->f_inode;
	FAR codec_mng_t *cmng = inode->i_private;
	FAR codec_file_t *cfile = filep->f_priv;
...
int codec_decoder_cmd(FAR struct file *filep, FAR struct v4l2_decoder_cmd *cmd)
{
	FAR struct inode *inode = filep->f_inode;
	FAR codec_mng_t *cmng = inode->i_private;
	FAR codec_file_t *cfile = filep->f_priv;
...
int codec_encoder_cmd(FAR struct file *filep, FAR struct v4l2_encoder_cmd *cmd)
{
	FAR struct inode *inode = filep->f_inode;
	FAR codec_mng_t *cmng = inode->i_private;
	FAR codec_file_t *cfile = filep->f_priv;
...
static int codec_open(FAR struct file *filep)
{
	...
	for (i = 0; i < CODEC_EVENT_COUNT; i++) {
		sq_addlast((FAR sq_entry_t *)&cfile->event_pool[i], &cfile->event_free);
	}
...
static int codec_munmap(FAR struct task_group_s *group, FAR struct mm_map_entry_s *entry,
...
static int codec_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map)
{
	...
	if (map->offset >= 0 && map->offset < total_size && map->length &&
		map->offset + map->length <= total_size) {
...
static int codec_put_buf(FAR codec_file_t *cfile, FAR codec_type_inf_t *type_inf,
						 FAR struct v4l2_buffer *buf)
{
	...
	poll_notify(&cfile->fds, 1, V4L2_TYPE_IS_OUTPUT(buf->type) ? POLLOUT : POLLIN);
