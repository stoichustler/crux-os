 ...
 *
 * Note: Although we use the term "frame buffer", we cannot use the ROUX
...
static struct path_name_map_s reg_name_map[] = {
	PATH_MAP_ENTRY(VM0),   PATH_MAP_ENTRY(VM1),	 PATH_MAP_ENTRY(HOS),  PATH_MAP_ENTRY(VOS),
	PATH_MAP_ENTRY(DMM),   PATH_MAP_ENTRY(DMAH), PATH_MAP_ENTRY(DMAL), PATH_MAP_ENTRY(DMDI),
	PATH_MAP_ENTRY(CMM),   PATH_MAP_ENTRY(CMAH), PATH_MAP_ENTRY(CMAL), PATH_MAP_ENTRY(CMDI),
	PATH_MAP_ENTRY(OSDM),  PATH_MAP_ENTRY(RB0),	 PATH_MAP_ENTRY(RB1),  PATH_MAP_ENTRY(RB2),
	PATH_MAP_ENTRY(RB3),   PATH_MAP_ENTRY(RB4),	 PATH_MAP_ENTRY(RB6),  PATH_MAP_ENTRY(RB7),
	PATH_MAP_ENTRY(RB8),   PATH_MAP_ENTRY(RB9),	 PATH_MAP_ENTRY(RB10), PATH_MAP_ENTRY(RB11),
	PATH_MAP_ENTRY(RB12),  PATH_MAP_ENTRY(RB13), PATH_MAP_ENTRY(RB14), PATH_MAP_ENTRY(RB15),
	PATH_MAP_ENTRY(OSDBL), PATH_MAP_ENTRY(STAT), PATH_MAP_ENTRY(DMDO), PATH_MAP_ENTRY(CMDO)};
#endif
...
static ssize_t mx7_write(FAR struct file *filep, FAR const char *buf, size_t len);

#if defined(DEBUG)
static ssize_t mx7_debug_read(FAR struct file *filep, FAR char *buf, size_t len);
static ssize_t mx7_debug_write(FAR struct file *filep, FAR const char *buf, size_t len);
#endif
...
static int __mx7_read_reg(FAR struct mx7_dev_s *dev, enum mx7_regaddr_e addr, FAR uint8_t *buf,
						  uint8_t len)
{
	int ret;
...
static int __mx7_write_reg(FAR struct mx7_dev_s *dev, enum mx7_regaddr_e addr,
...
static inline int __mx7_write_reg__cmah(FAR struct mx7_dev_s *dev, uint8_t val)
{
...
static inline int __mx7_write_reg__cmal(FAR struct mx7_dev_s *dev, uint8_t val)
{
...
static inline int __mx7_write_reg__osdbl(FAR struct mx7_dev_s *dev, uint8_t val)
{
...
static ssize_t __write_fb(FAR struct mx7_dev_s *dev, FAR const uint8_t *buf, size_t len, uint8_t ca,
						  size_t pos)
{
	ssize_t ret = len;
	FAR struct spi_dev_s *spi = dev->config.spi;
...
static ssize_t __read_cm(FAR struct mx7_dev_s *dev, size_t pos, FAR uint8_t *buf, size_t len)
{
...
static ssize_t mx7_write_fb(FAR struct file *filep, FAR const char *buf, size_t len)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct mx7_dev_s *dev = inode->i_private;
	ssize_t ret;
...
static int add_interface(FAR const char *path, FAR const char *name,
						 FAR const struct file_operations *fops, mode_t mode, FAR void *private)
{
	char buf[128];
	...
	/* Register the interface in the usual way. ROUX will build the
...
int max7456_register(FAR const char *path, FAR struct mx7_config_s *config)
{
	...
	for (n = 0; ret >= 0 && n < REG_NAME_MAP_LEN; n++) {
		ret = add_interface(path, reg_name_map[n].path, &g_mx7_debug_fops, 0666, dev);
	}
#endif
