...
static FAR const char *const g_priority_str[] = {"EMERG", "ALERT",	"CRIT", "ERROR",
...
#if defined(CONFIG_SYSLOG_TIMESTAMP_FORMATTED)
	date_buf[0] = '\0';
	strftime(date_buf, CONFIG_SYSLOG_TIMESTAMP_BUFFER, CONFIG_SYSLOG_TIMESTAMP_FORMAT, &tm);
#endif
#endif

#if defined(CONFIG_SYSLOG_COLOR_OUTPUT) || defined(CONFIG_SYSLOG_TIMESTAMP) ||                     \
	defined(CONFIG_SMP) || defined(CONFIG_SYSLOG_PROCESSID) || defined(CONFIG_SYSLOG_PRIORITY) ||  \
	defined(CONFIG_SYSLOG_PREFIX) || defined(CONFIG_SYSLOG_PROCESS_NAME)
