...
static ssize_t syslog_chardev_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
#ifdef CONFIG_SYSLOG_IOCTL
static int syslog_chardev_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
#endif
...
static ssize_t syslog_chardev_write(FAR struct file *filep, FAR const char *buffer, size_t len)
{
...
static int syslog_chardev_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	if (cmd == SYSLOGIOC_GETCHANNELS) {
		...
		for (i = 0; i < CONFIG_SYSLOG_MAX_CHANNELS; i++) {
			...
			strlcpy(info[i].sc_name, channel->sc_name, sizeof(info[i].sc_name));
			info[i].sc_disable = channel->sc_state & SYSLOG_CHANNEL_DISABLE;
		}
	} else if (cmd == SYSLOGIOC_SETFILTER) {
		info = (FAR struct syslog_channel_info_s *)arg;

		for (i = 0; i < CONFIG_SYSLOG_MAX_CHANNELS; i++) {
			if (strncmp(g_syslog_channel[i]->sc_name, info->sc_name, sizeof(info->sc_name)) == 0) {
				channel = g_syslog_channel[i];
				break;
			}
		}
		...
		channel->sc_state = info->sc_disable ? channel->sc_state | SYSLOG_CHANNEL_DISABLE
