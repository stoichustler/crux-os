...
static ssize_t syslog_dev_write(FAR syslog_channel_t *channel, FAR const char *buffer,
								size_t buflen);
...
static const struct syslog_channel_ops_s g_syslog_dev_ops = {
	syslog_dev_putc,		syslog_dev_force, syslog_dev_flush, syslog_dev_write, NULL,
...
static int syslog_dev_open(FAR struct syslog_dev_s *syslog_dev, FAR const char *devpath, int oflags,
						   int mode)
{
	int ret;
...
static int syslog_dev_outputready(FAR struct syslog_dev_s *syslog_dev)
{
	...
	if (syslog_dev->sl_state != SYSLOG_OPENED) {
		...
		if (syslog_dev->sl_state == SYSLOG_REOPEN) {
			...
			DEBUGASSERT(syslog_dev->sl_devpath != NULL);
			ret = syslog_dev_open(syslog_dev, syslog_dev->sl_devpath, (int)syslog_dev->sl_oflags,
								...
								size_t buflen)
{
	...
	for (endptr = buffer, remaining = buflen; remaining > 0; endptr++, remaining--) {
		...
		if (*endptr == '\r' || *endptr == '\n') {
			...
			if (remaining > 1 && ((endptr[0] == '\r' && endptr[1] == '\n') ||
								  (endptr[0] == '\n' && endptr[1] == '\r'))) {
			...
			if (writelen > 0) {
				nwritten = file_write(&syslog_dev->sl_file, g_syscrlf, writelen);
...
FAR syslog_channel_t *syslog_dev_initialize(FAR const char *devpath, int oflags, int mode)
{
...
void syslog_dev_uninitialize(FAR syslog_channel_t *channel)
{
	...
	if (syslog_dev->sl_state == SYSLOG_OPENED || syslog_dev->sl_state == SYSLOG_FAILURE) {
		file_close(&syslog_dev->sl_file);
		nxrmutex_destroy(&syslog_dev->sl_lock);
	}
