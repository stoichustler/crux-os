...
static ssize_t ramlog_file_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t ramlog_file_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static uint32_t g_sysbuffer[CONFIG_RAMLOG_BUFSIZE / 4] locate_data(RAMLOG_BUFFER_SECTION);
#else
...
static uint32_t ramlog_bufferused(FAR struct ramlog_dev_s *priv, FAR struct ramlog_user_s *upriv)
{
...
static void ramlog_copybuf(FAR struct ramlog_dev_s *priv, FAR const char *buffer, size_t len)
{
...
static ssize_t ramlog_addbuf(FAR struct ramlog_dev_s *priv, FAR const char *buffer, size_t len)
{
#ifdef CONFIG_RAMLOG_SYSLOG
	FAR struct ramlog_header_s *header = priv->rl_header;
#endif
...
static ssize_t ramlog_file_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
...
static ssize_t ramlog_file_write(FAR struct file *filep, FAR const char *buffer, size_t len)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct ramlog_dev_s *priv = inode->i_private;
...
static int ramlog_file_open(FAR struct file *filep)
{
	...
	upriv->rl_tail = header->rl_head > priv->rl_bufsize ? header->rl_head - priv->rl_bufsize : 0;
	leave_critical_section(flags);
...
ssize_t ramlog_write(FAR syslog_channel_t *channel, FAR const char *buffer, size_t buflen)
{
