...
static ssize_t syslog_stream_write(FAR syslog_channel_t *channel, FAR const char *buffer,
								   size_t buflen);
...
static const struct syslog_channel_ops_s g_syslog_stream_ops = {
	syslog_stream_putc,	 NULL, syslog_stream_flush, syslog_stream_write, syslog_stream_write,
...
static ssize_t syslog_stream_write(FAR syslog_channel_t *channel, FAR const char *buffer,
								   size_t buflen)
{
	FAR struct syslog_stream_s *chan = (FAR struct syslog_stream_s *)channel;
	ssize_t nwritten;
	irqstate_t flags;
...
FAR syslog_channel_t *syslog_stream_channel(FAR struct lib_outstream_s *stream)
{
	...
	chan = (FAR struct syslog_stream_s *)kmm_zalloc(sizeof(struct syslog_stream_s));
