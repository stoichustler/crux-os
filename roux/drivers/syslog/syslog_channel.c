...
#include <roux/syslog/ramlog.h>
#endif

#ifdef CONFIG_SYSLOG_RPMSG
#include <roux/syslog/syslog_rpmsg.h>
#endif
...
static ssize_t syslog_default_write(FAR syslog_channel_t *channel, FAR const char *buffer,
									size_t buflen);
#endif

#ifdef CONFIG_SYSLOG_CDCACM
static int syslog_cdcacm_putc(FAR struct syslog_channel_s *channel, int ch);
static ssize_t syslog_cdcacm_write(FAR struct syslog_channel_s *channel, FAR const char *buffer,
								   size_t buflen);
#endif
...
#ifdef CONFIG_RAMLOG_SYSLOG
static const struct syslog_channel_ops_s g_ramlog_channel_ops = {ramlog_putc, ramlog_putc, NULL,
																 ramlog_write, ramlog_write};

static syslog_channel_t g_ramlog_channel = {&g_ramlog_channel_ops
...
static const struct syslog_channel_ops_s g_rpmsg_channel_ops = {
	syslog_rpmsg_putc, syslog_rpmsg_putc, syslog_rpmsg_flush, syslog_rpmsg_write,
	syslog_rpmsg_write};

static syslog_channel_t g_rpmsg_channel = {&g_rpmsg_channel_ops
#ifdef CONFIG_SYSLOG_IOCTL
										   ,
										   "rpmsg"
#endif
...
static const struct syslog_channel_ops_s g_rtt_channel_ops = {
	syslog_rtt_putc, syslog_rtt_putc, NULL, syslog_rtt_write, syslog_rtt_write};

static syslog_channel_t g_rtt_channel = {&g_rtt_channel_ops
#ifdef CONFIG_SYSLOG_IOCTL
										 ,
										 "rtt"
#endif
...
static const struct syslog_channel_ops_s g_cdcacm_channel_ops = {
	syslog_cdcacm_putc, syslog_cdcacm_putc, NULL, syslog_cdcacm_write, syslog_cdcacm_write};

static struct syslog_channel_s g_cdcacm_channel = {&g_cdcacm_channel_ops
#ifdef CONFIG_SYSLOG_IOCTL
												   ,
												   "cdcacm"
#endif
...
static const struct syslog_channel_ops_s g_default_channel_ops = {
	syslog_default_putc, syslog_default_putc, NULL, syslog_default_write};

static syslog_channel_t g_default_channel = {&g_default_channel_ops
#ifdef CONFIG_SYSLOG_IOCTL
											 ,
											 "default"
#endif
...
#define SYSLOG_NCHANNELS                                                                           \
	(SYSLOG_DEFAULT_AVAILABLE + RAMLOG_SYSLOG_AVAILABLE + SYSLOG_RPMSG_AVAILABLE +                 \
...
static int syslog_default_putc(FAR syslog_channel_t *channel, int ch)
{
	...
	if (csection_available()) {
		/* See https://github.com/apache/roux/issues/14662
		...
		for (i = 0; i < CONFIG_SYSLOG_MAX_CHANNELS; i++) {
			if (g_syslog_channel[i] == NULL) {
				...
				if (channel->sc_name[0] == '\0') {
					snprintf(channel->sc_name, sizeof(channel->sc_name), "channel-%p",
							 channel->sc_ops);
				}
#endif
	...
	if (channel != NULL) {
		for (i = 0; i < CONFIG_SYSLOG_MAX_CHANNELS; i++) {
			if (g_syslog_channel[i] == channel) {
				...
				while (i < (CONFIG_SYSLOG_MAX_CHANNELS - 1) && g_syslog_channel[i + 1] != NULL) {
