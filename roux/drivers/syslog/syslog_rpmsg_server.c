...
static void syslog_rpmsg_write_internal(FAR const char *buf1, size_t len1, FAR const char *buf2,
										size_t len2);
static bool syslog_rpmsg_ns_match(FAR struct rpmsg_device *rdev, FAR void *priv_,
								  FAR const char *name, uint32_t dest);
static void syslog_rpmsg_ns_bind(FAR struct rpmsg_device *rdev, FAR void *priv_,
								 FAR const char *name, uint32_t dest);
static int syslog_rpmsg_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
							   uint32_t src, FAR void *priv_);
#ifdef CONFIG_SYSLOG_RPMSG_SERVER_CHARDEV
static int syslog_rpmsg_file_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
#endif
								 ...
								 FAR const char *name, uint32_t dest)
{
	...
	ret = rpmsg_create_ept(&priv->ept, rdev, SYSLOG_RPMSG_EPT_NAME, RPMSG_ADDR_ANY, dest,
							   ...
							   uint32_t src, FAR void *priv_)
{
	...
	if (header->command == SYSLOG_RPMSG_TRANSFER) {
		...
		if (nl != NULL) {
			...
			if (priv->nextpos) {
				syslog_rpmsg_write_internal(priv->tmpbuf, priv->nextpos, msg->data, printed);
				priv->nextpos = 0;
			} else {
...
int syslog_rpmsg_server_init(void)
{
	...
	return rpmsg_register_callback(NULL, NULL, NULL, syslog_rpmsg_ns_match, syslog_rpmsg_ns_bind);
}
