...
static void syslog_rpmsg_addbuf(FAR struct syslog_rpmsg_s *priv, FAR const char *buffer,
								size_t len);
static void syslog_rpmsg_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_);
static void syslog_rpmsg_device_destroy(FAR struct rpmsg_device *rdev, FAR void *priv_);
static int syslog_rpmsg_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
							   uint32_t src, FAR void *priv_);
#ifdef CONFIG_SYSLOG_RPMSG_CHARDEV
static ssize_t syslog_rpmsg_file_read(FAR struct file *filep, FAR char *buffer, size_t len);
static ssize_t syslog_rpmsg_file_write(FAR struct file *filep, FAR const char *buffer, size_t len);
#endif
...
static void syslog_rpmsg_addbuf(FAR struct syslog_rpmsg_s *priv, FAR const char *buffer, size_t len)
{
...
static void syslog_rpmsg_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	...
	if (priv->buffer && strcmp(CONFIG_SYSLOG_RPMSG_SERVER_NAME, rpmsg_get_cpuname(rdev)) == 0) {
		priv->ept.priv = priv;

		ret = rpmsg_create_ept(&priv->ept, rdev, SYSLOG_RPMSG_EPT_NAME, RPMSG_ADDR_ANY,
							   RPMSG_ADDR_ANY, syslog_rpmsg_ept_cb, NULL);
		if (ret == 0) {
...
int syslog_rpmsg_flush(FAR syslog_channel_t *channel)
{
	...
	if (priv->head > priv->flush && priv->head - priv->flush > priv->size) {
...
ssize_t syslog_rpmsg_write(FAR syslog_channel_t *channel, FAR const char *buffer, size_t buflen)
{
	FAR struct syslog_rpmsg_s *priv = &g_syslog_rpmsg;
...
int syslog_rpmsg_init(void)
{
	...
	ret = register_driver(CONFIG_SYSLOG_DEVPATH, &g_syslog_rpmsgfops, 0666, &g_syslog_rpmsg);
	if (ret < 0) {
		return ret;
	}
#endif

	return rpmsg_register_callback(&g_syslog_rpmsg, syslog_rpmsg_device_created,
								   syslog_rpmsg_device_destroy, NULL, NULL);
}
