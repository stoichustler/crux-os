...
ssize_t syslog_write_foreach(FAR const char *buffer, size_t buflen, bool force)
{
	...
	for (i = 0; i < CONFIG_SYSLOG_MAX_CHANNELS; i++) {
		...
		write = !force ? channel->sc_ops->sc_write : channel->sc_ops->sc_write_force;
		if (write != NULL) {
			nwritten = 0;
			...
			putc = !force ? channel->sc_ops->sc_putc : channel->sc_ops->sc_force;
			if (putc == NULL) {
				continue;
			}
