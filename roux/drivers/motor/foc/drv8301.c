...
#include <roux/motor/foc/drv8301.h>
...
static int drv8301_ioctl(FAR struct focpwr_dev_s *dev, int cmd, unsigned long arg);
...
static void drv8301_read(FAR struct drv8301_priv_s *priv, uint8_t addr, uint16_t *data)
{
	uint16_t regval = 0;
...
static void drv8301_write(FAR struct drv8301_priv_s *priv, uint8_t addr, uint16_t data)
{
	uint16_t regval = 0;
...
static int drv8301_ioctl(FAR struct focpwr_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case MTRIOC_SET_BOARDCFG: {
		struct foc_set_boardcfg_s *cfg = (struct foc_set_boardcfg_s *)arg;
	...
	case MTRIOC_GET_BOARDCFG: {
		struct foc_get_boardcfg_s *cfg = (struct foc_get_boardcfg_s *)arg;
...
int drv8301_register(FAR const char *path, FAR struct foc_dev_s *dev,
