...
#include <roux/motor/foc/foc_dummy.h>
#include <roux/motor/foc/foc_lower.h>
...
#define FOC_DUMMY_DATA_FROM_DEV_GET(d) ((FAR struct foc_dummy_data_s *)(d)->lower->data)
...
static int foc_dummy_configure(FAR struct foc_dev_s *dev, FAR struct foc_cfg_s *cfg);
...
static int foc_dummy_pwm_duty_set(FAR struct foc_dev_s *dev, FAR foc_duty_t *duty);
static int foc_dummy_pwm_off(FAR struct foc_dev_s *dev, bool off);
static int foc_dummy_info_get(FAR struct foc_dev_s *dev, FAR struct foc_info_s *info);
static int foc_dummy_ioctl(FAR struct foc_dev_s *dev, int cmd, unsigned long arg);
static int foc_dummy_bind(FAR struct foc_dev_s *dev, FAR struct foc_callbacks_s *cb);
...
static struct foc_lower_ops_s g_foc_dummy_ops = {
	foc_dummy_configure, foc_dummy_setup,		foc_dummy_shutdown, foc_dummy_pwm_duty_set,
	foc_dummy_pwm_off,	 foc_dummy_start,		foc_dummy_info_get, foc_dummy_ioctl,
	foc_dummy_bind,		 foc_dummy_fault_clear,
#ifdef CONFIG_MOTOR_FOC_TRACE
	foc_dummy_trace
#endif
};
