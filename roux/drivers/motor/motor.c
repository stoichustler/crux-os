...
static int motor_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case MTRIOC_SET_LIMITS: {
		FAR struct motor_limits_s *limits = (FAR struct motor_limits_s *)((uintptr_t)arg);
	...
	case MTRIOC_GET_STATE: {
		FAR struct motor_state_s *state = (FAR struct motor_state_s *)((uintptr_t)arg);
	...
	case MTRIOC_SET_PARAMS: {
		FAR struct motor_params_s *params = (FAR struct motor_params_s *)((uintptr_t)arg);
		...
		if (params->direction != MOTOR_DIR_CCW && params->direction != MOTOR_DIR_CW) {
		...
		if (params->position < 0.0 || params->position > lower->limits.position) {
		...
		if (lower->limits.speed > 0.0 && params->speed > lower->limits.speed) {
		...
		if (lower->limits.torque > 0.0 && params->torque > lower->limits.torque) {
		...
		if (lower->limits.force > 0.0 && params->force > lower->limits.force) {
