...
#include <roux/sensors/wtgahrs2.h>
...
static int wtgahrs2_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, bool sw);
static int wtgahrs2_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static const uint8_t g_wtgahrs2_unlock[] = {0xff, 0xaa, 0x69, 0x88, 0xb5};

static const uint8_t g_wtgahrs2_odr_200hz[] = {0xff, 0xaa, 0x03, 0x0b, 0x00};

static const uint8_t g_wtgahrs2_enable_sensor[] = {0xff, 0xaa, 0x02, 0xd7, 0x05};
...
static void wtgahrs2_sendcmd(FAR struct wtgahrs2_dev_s *rtdata, const void *cmd)
{
...
static void wtgahrs2_accel_data(FAR struct wtgahrs2_dev_s *rtdata, FAR unsigned char *buffer)
{
...
static void wtgahrs2_gyro_data(FAR struct wtgahrs2_dev_s *rtdata, FAR unsigned char *buffer)
{
...
static void wtgahrs2_mag_data(FAR struct wtgahrs2_dev_s *rtdata, FAR unsigned char *buffer)
{
...
static void wtgahrs2_baro_data(FAR struct wtgahrs2_dev_s *rtdata, FAR unsigned char *buffer)
{
	...
	baro.timestamp = now;
	baro.pressure = (long)(buffer[3] << 24 | buffer[2] << 16 | buffer[1] << 8 | buffer[0]) / 100.0f;
...
static void wtgahrs2_gnss_data(FAR struct wtgahrs2_dev_s *rtdata, FAR unsigned char *buffer,
							   int info_type)
{
	...
	case WTGAHRS2_GNSS1_INFO:
		...
		rtdata->gnss.longitude =
			(buffer[3] << 8 | buffer[2] << 8 | buffer[1] << 8 | buffer[0]) / 10000000.0f;
		rtdata->gnss.latitude =
			(buffer[7] << 8 | buffer[6] << 8 | buffer[5] << 8 | buffer[4]) / 10000000.0f;
		break;

	case WTGAHRS2_GNSS2_INFO:
		rtdata->gnss_mask |= WTGAHRS2_GNSS2_MASK;
		rtdata->gnss.altitude = (float)(buffer[1] << 8 | buffer[0]) / 10.0f;
		rtdata->gnss.ground_speed =
			(float)(buffer[7] << 8 | buffer[6] << 8 | buffer[5] << 8 | buffer[4]) / 3600.0f;
		break;
	}
	...
	if (rtdata->gnss_mask == WTGAHRS2_GNSS_MASK) {
		...
		sninfo("Time : %" PRIu64 " utc_time: %" PRIu64 "\n", rtdata->gnss.timestamp,
			   rtdata->gnss.time_utc);
		sninfo("GNSS longitude : %fdegree, latitude:%fdegree\n", rtdata->gnss.longitude,
			   rtdata->gnss.latitude);
		sninfo("GNSS speed: %fm/s, altitude: %fm\n", rtdata->gnss.ground_speed,
			   rtdata->gnss.altitude);
	}
}

static bool wtgahrs2_process_data(FAR struct wtgahrs2_dev_s *rtdata, FAR unsigned char *buffer)
{
	unsigned char sum = 0;
	int i;
...
static int wtgahrs2_thread(int argc, FAR char *argv[])
{
	FAR struct wtgahrs2_dev_s *rtdata =
		(FAR struct wtgahrs2_dev_s *)(uintptr_t)strtoul(argv[1], NULL, 16);
...
int wtgahrs2_initialize(FAR const char *path, int devno)
{
	...
	ret = kthread_create("wtgahrs2_thread", SCHED_PRIORITY_DEFAULT, CONFIG_DEFAULT_TASK_STACKSIZE,
