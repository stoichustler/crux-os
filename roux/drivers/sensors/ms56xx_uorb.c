...
#include <roux/sensors/ms56xx.h>
#include <roux/sensors/msxxxx_crc4.h>

#if defined(CONFIG_SENSORS_MS56XX) && (defined(CONFIG_I2C) || defined(CONFIG_SPI))
...
static int ms56xx_read24(FAR struct ms56xx_dev_s *priv, FAR uint8_t *regval);

static int32_t ms56xx_compensate_temp(FAR struct ms56xx_dev_s *priv, uint32_t temp_raw,
									  int32_t *deltat);
static uint32_t ms56xx_compensate_press(FAR struct ms56xx_dev_s *priv, uint32_t press, uint32_t dt,
										int32_t *temp);
...
static int ms56xx_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							   FAR uint32_t *period_us);
static int ms56xx_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						   bool enable);
...
static inline void baro_measure_read(FAR struct ms56xx_dev_s *priv, FAR struct sensor_baro *baro)
{
	uint32_t press;
	uint32_t temp_raw;
	int32_t temp;
	...
	press = (uint32_t)buffer[0] << 16 | (uint32_t)buffer[1] << 8 | (uint32_t)buffer[2];
	...
	temp_raw = (uint32_t)buffer[0] << 16 | (uint32_t)buffer[1] << 8 | (uint32_t)buffer[2];
...
static int ms56xx_thread(int argc, char **argv)
{
	FAR struct ms56xx_dev_s *priv =
		(FAR struct ms56xx_dev_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
...
static int ms56xx_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							   FAR uint32_t *period_us)
{
	FAR struct ms56xx_dev_s *priv = container_of(lower, FAR struct ms56xx_dev_s, sensor_lower);
					...
					enum ms56xx_model_e model)
{
	...
	argv[0] = arg1;
	argv[1] = NULL;
	ret = kthread_create("ms56xx_thread", SCHED_PRIORITY_DEFAULT, CONFIG_MS56XX_THREAD_STACKSIZE,
						 ms56xx_thread, argv);
	if (ret < 0) {
