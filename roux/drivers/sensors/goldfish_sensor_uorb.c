...
#include <roux/roux.h>
#include <roux/sensors/goldfish_sensor.h>
...
static int goldfish_sensor_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
									bool enabled);
static int goldfish_sensor_set_interval(FAR struct sensor_lowerhalf_s *lower,
										FAR struct file *filep, FAR uint32_t *period_us);
static int goldfish_sensor_get_info(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static inline int goldfish_sensor_read_pipe(FAR struct file *pipe, FAR void *buffer, size_t size)
{
...
static inline int goldfish_sensor_write_pipe(FAR struct file *pipe, FAR const void *buffer,
											 size_t size)
{
...
static inline int goldfish_sensor_send(FAR struct file *pipe, FAR const void *msg, size_t size)
{
	char header[5];
	int ret = 0;
...
static inline ssize_t goldfish_sensor_recv(FAR struct file *pipe, FAR void *msg, size_t maxsize)
{
	char header[5];
	size_t size;
	int ret;
...
static inline int goldfish_sensor_open_pipe(FAR struct file *filep, FAR const char *ns,
											FAR const char *pipe_name, int flags)
{
	char buf[256];
	int len;
	int ret;
...
static FAR const char *goldfish_sensor_match(FAR const char *s, FAR const char *p)
{
	size_t l = strlen(p);
	return strncmp(s, p, l) ? NULL : s + l;
}

static int64_t goldfish_sensor_weigthed_average(int64_t a, int64_t aw, int64_t b, int64_t bw)
{
	return (a * aw + b * bw) / (aw + bw);
}

static int goldfish_sensor_do_activate(FAR struct file *pipe, int handle, bool enabled)
{
	char buffer[64];
	int len;

	len = snprintf(buffer, sizeof(buffer), "set:%s:%d", goldfish_sensor_get_name(handle), enabled);
...
static void goldfish_sensor_parse_event(FAR struct goldfish_sensor_s *sensor)
{
	...
	if ((value = goldfish_sensor_match(buf, "acceleration:")) != NULL) {
		...
		if (sscanf(value, "%f:%f:%f", &accel.x, &accel.y, &accel.z) == 3) {
			accel.temperature = NAN;
			accel.timestamp = now_ns + sensor->time_bias_ns;
			sensor->lower_accel.push_event(sensor->lower_accel.priv, &accel,
										   sizeof(struct sensor_accel));
		}
	} else if ((value = goldfish_sensor_match(buf, "gyroscope:")) != NULL) {
		struct sensor_gyro gyro;
		if (sscanf(value, "%f:%f:%f", &gyro.x, &gyro.y, &gyro.z) == 3) {
			gyro.temperature = NAN;
			gyro.timestamp = now_ns + sensor->time_bias_ns;
			sensor->lower_gyro.push_event(sensor->lower_gyro.priv, &gyro,
										  sizeof(struct sensor_gyro));
		}
	} else if ((value = goldfish_sensor_match(buf, "magnetic:")) != NULL) {
		struct sensor_mag mag;
		if (sscanf(value, "%f:%f:%f", &mag.x, &mag.y, &mag.z) == 3) {
			mag.temperature = NAN;
			mag.timestamp = now_ns + sensor->time_bias_ns;
			sensor->lower_mag.push_event(sensor->lower_mag.priv, &mag, sizeof(struct sensor_mag));
		}
	} else if ((value = goldfish_sensor_match(buf, "gyroscope-uncalibrated:")) != NULL) {
		struct sensor_gyro gyro;
		if (sscanf(value, "%f:%f:%f", &gyro.x, &gyro.y, &gyro.z) == 3) {
			gyro.temperature = NAN;
			gyro.timestamp = now_ns + sensor->time_bias_ns;
			sensor->lower_gyro_uncalibrated.push_event(sensor->lower_gyro_uncalibrated.priv, &gyro,
													   sizeof(struct sensor_gyro));
		}
	} else if ((value = goldfish_sensor_match(buf, "acceleration-uncalibrated:")) != NULL) {
		struct sensor_accel accel;
		if (sscanf(value, "%f:%f:%f", &accel.x, &accel.y, &accel.z) == 3) {
			accel.temperature = NAN;
			accel.timestamp = now_ns + sensor->time_bias_ns;
			sensor->lower_accel_uncalibrated.push_event(sensor->lower_accel_uncalibrated.priv,
														&accel, sizeof(struct sensor_accel));
		}
	} else if ((value = goldfish_sensor_match(buf, "magnetic-uncalibrated:")) != NULL) {
		struct sensor_mag mag;
		if (sscanf(value, "%f:%f:%f", &mag.x, &mag.y, &mag.z) == 3) {
			mag.temperature = NAN;
			mag.timestamp = now_ns + sensor->time_bias_ns;
			sensor->lower_mag_uncalibrated.push_event(sensor->lower_mag_uncalibrated.priv, &mag,
		...
		if (sscanf(value, "%f", &temp.temperature) == 1) {
			...
			sensor->lower_temp.push_event(sensor->lower_temp.priv, &temp,
		...
		if (sscanf(value, "%f", &prox.proximity) == 1) {
			...
			sensor->lower_prox.push_event(sensor->lower_prox.priv, &prox,
		...
		if (sscanf(value, "%f", &light.light) == 1) {
			...
			light.ir = NAN;
			sensor->lower_light.push_event(sensor->lower_light.priv, &light,
		...
		if (sscanf(value, "%f", &baro.pressure) == 1) {
			...
			sensor->lower_baro.push_event(sensor->lower_baro.priv, &baro,
		...
		if (sscanf(value, "%f", &humi.humidity) == 1) {
			...
			sensor->lower_humi.push_event(sensor->lower_humi.priv, &humi,
		...
		if (sscanf(value, "%f", &hrate.bpm) == 1) {
			...
			sensor->lower_hrate.push_event(sensor->lower_hrate.priv, &hrate,
		...
		if ((sscanf(value, "%" PRId64, &guest_ms) == 1) && (guest_ms >= 0)) {
			...
			sensor->time_bias_ns =
				MIN(0, goldfish_sensor_weigthed_average(sensor->time_bias_ns, 3, time_bias_ns, 1));
		}
							 ...
							 FAR struct goldfish_sensor_s **priv)
{
	...
	case SENSOR_TYPE_ACCELEROMETER_UNCALIBRATED:
		priv = container_of(lower, struct goldfish_sensor_s, lower_accel_uncalibrated);
	...
	case SENSOR_TYPE_MAGNETIC_FIELD_UNCALIBRATED:
		priv = container_of(lower, struct goldfish_sensor_s, lower_mag_uncalibrated);
	...
	case SENSOR_TYPE_GYROSCOPE_UNCALIBRATED:
		priv = container_of(lower, struct goldfish_sensor_s, lower_gyro_uncalibrated);
										...
										FAR struct file *filep, FAR uint32_t *period_us)
{
	...
	len = snprintf(buffer, sizeof(buffer), "set-delay: %d", (int)(*period_us / 1000));
									...
									FAR struct sensor_device_info_s *info)
{
	...
	for (i = 0; i < sizeof(g_goldfish_sensor_info); i++) {
		if (!strncmp(goldfish_sensor_get_name(handle), g_goldfish_sensor_info[i].name,
					 strlen(g_goldfish_sensor_info[i].name))) {
			memcpy(info, &g_goldfish_sensor_info[i], sizeof(struct sensor_device_info_s));
			return OK;
		}
	}
...
int goldfish_sensor_init(int devno, uint32_t batch_number)
{
	...
	len = snprintf(buffer, sizeof(buffer), "time:%" PRId64, sensor_get_timestamp());
	...
	ret = goldfish_sensor_send(&sensor->pipe, GOLDFISH_LIST_SENSOR_CMD,
	...
	argv[0] = arg1;
	argv[1] = NULL;
	ret = kthread_create("goldfish_sensor_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_DEFAULT_TASK_STACKSIZE, goldfish_sensor_thread, argv);
	if (ret < 0) {
	...
	sensor->lower_accel_uncalibrated.type = SENSOR_TYPE_ACCELEROMETER_UNCALIBRATED;
	sensor->lower_accel_uncalibrated.ops = &g_goldfish_sensor_ops;
	sensor->lower_accel_uncalibrated.nbuffer = batch_number;

	sensor->lower_mag_uncalibrated.type = SENSOR_TYPE_MAGNETIC_FIELD_UNCALIBRATED;
	sensor->lower_mag_uncalibrated.ops = &g_goldfish_sensor_ops;
	sensor->lower_mag_uncalibrated.nbuffer = batch_number;

	sensor->lower_gyro_uncalibrated.type = SENSOR_TYPE_GYROSCOPE_UNCALIBRATED;
