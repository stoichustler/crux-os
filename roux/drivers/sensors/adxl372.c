...
#if defined(CONFIG_SPI) && defined(CONFIG_SENSORS_ADXL372) && defined(CONFIG_SPI_EXCHANGE)
...
#include <roux/sensors/adxl372.h>
...
static uint8_t adxl372_read_register(FAR struct adxl372_dev_s *dev, uint8_t reg_addr);
static void adxl372_read_registerblk(FAR struct adxl372_dev_s *dev, uint8_t reg_addr,
									 FAR uint8_t *reg_data, uint8_t xfercnt);
static void adxl372_write_register(FAR struct adxl372_dev_s *dev, uint8_t reg_addr,
								   uint8_t reg_data);
static void adxl372_write_registerblk(FAR struct adxl372_dev_s *dev, uint8_t reg_addr,
									  FAR uint8_t *reg_data, uint8_t xfercnt);
...
static ssize_t adxl372_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static ssize_t adxl372_dvr_read(FAR void *instance, FAR char *buffer, size_t buflen);
static ssize_t adxl372_dvr_write(FAR void *instance, FAR const char *buffer, size_t buflen);
static off_t adxl372_dvr_seek(FAR void *instance, off_t offset, int whence);
static int adxl372_dvr_ioctl(FAR void *instance, int cmd, unsigned long arg);
static void adxl372_dvr_exchange(FAR void *instance, FAR const void *txbuffer, FAR void *rxbuffer,
								 size_t nwords);
...
static struct adxl372_reg_pair_s g_initial_adxl372_cr_values[] = {
	...
	{.addr = ADXL372_POWER_CTL, .value = 0},
	{.addr = ADXL372_FIFO_CTL, .value = ADXL372_FIFO_BYPASSED},

	/* Interrupts disabled. */

	{.addr = ADXL372_INT1_MAP, .value = 0},
	{.addr = ADXL372_TIMING, .value = ADXL372_TIMING_ODR1600},
	{.addr = ADXL372_MEASURE, .value = ADXL372_MEAS_BW800},
	{.addr = ADXL372_POWER_CTL, .value = ADXL372_POWER_HPF_DISABLE | ADXL372_POWER_MODE_MEASURE}};
...
static int adxl372_dvr_open(FAR void *instance, int32_t arg)
{
	...
	if ((pnpid & 0xffffff00) != (ADXL372_DEVID_AD_VALUE << 24 | ADXL372_DEVID_MST_VALUE << 16 |
...
static ssize_t adxl372_dvr_read(FAR void *instance, FAR char *buffer, size_t buflen)
{
	...
	adxl372_read_registerblk(priv, priv->seek_address, (FAR uint8_t *)buffer, buflen);
...
static ssize_t adxl372_dvr_write(FAR void *instance, FAR const char *buffer, size_t buflen)
{
	...
	adxl372_write_registerblk(priv, priv->seek_address, (FAR uint8_t *)buffer, buflen);
...
static ssize_t adxl372_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct adxl372_dev_s *priv = inode->i_private;
...
int adxl372_register(FAR const char *devpath, FAR struct spi_dev_s *spi,
