...
#include <roux/sensors/hdc1008.h>
...
#define HDC1008_CONFIGURATION_HRES_14BIT                                                           \
	(CONFIGURATION_RES_14BIT << HDC1008_CONFIGURATION_HRES_SHIFT)
#define HDC1008_CONFIGURATION_HRES_11BIT                                                           \
	(CONFIGURATION_RES_11BIT << HDC1008_CONFIGURATION_HRES_SHIFT)
...
#define HDC1008_CONFIGURATION_TRES_14BIT                                                           \
	(CONFIGURATION_RES_14BIT << HDC1008_CONFIGURATION_TRES_SHIFT)
#define HDC1008_CONFIGURATION_TRES_11BIT                                                           \
	(CONFIGURATION_RES_11BIT << HDC1008_CONFIGURATION_TRES_SHIFT)
...
#endif
static ssize_t hdc1008_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t hdc1008_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int hdc1008_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
#ifndef CONFIG_DISABLE_PSEUDOFS_OPERATIONS
static int hdc1008_unlink(FAR struct inode *inode);
#endif
static int hdc1008_getreg(FAR struct hdc1008_dev_s *priv, uint8_t regaddr, FAR uint16_t *regval);
static int hdc1008_putreg(FAR struct hdc1008_dev_s *priv, uint8_t regaddr, FAR uint16_t regval);
static int hdc1008_reset(FAR struct hdc1008_dev_s *priv);
static int hdc1008_measure_trh(FAR struct hdc1008_dev_s *priv, int *t, int *h);
...
static int hdc1008_measure_t_or_rh(FAR struct hdc1008_dev_s *priv, bool temperature, int *val)
{
	struct i2c_config_s config;
	uint8_t buf[4];
	uint8_t reg;
...
static int hdc1008_set_operational_mode(struct hdc1008_dev_s *priv, uint8_t mode)
{
	int ret;
	uint16_t reg;
...
static int hdc1008_set_temperature_resolution(struct hdc1008_dev_s *priv, uint8_t resolution)
{
	int ret;
	uint16_t reg;
...
static int hdc1008_set_humidity_resolution(struct hdc1008_dev_s *priv, uint8_t resolution)
{
	int ret;
	uint16_t reg;
...
static int hdc1008_set_heater_mode(struct hdc1008_dev_s *priv, uint8_t mode)
{
	int ret;
	uint16_t reg;
...
static ssize_t hdc1008_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (priv->mode == HDC1008_MEAS_TEMPERATURE) {
		len = snprintf(buffer, buflen, "%d.%02d", data.temperature / 100, data.temperature % 100);
	} else if (priv->mode == HDC1008_MEAS_HUMIDITY) {
		len = snprintf(buffer, buflen, "%d.%d", data.humidity / 10, data.humidity % 10);
	} else if (priv->mode == HDC1008_MEAS_T_AND_RH) {
		len = snprintf(buffer, buflen, "%d.%02d %d.%d", data.temperature / 100,
					   data.temperature % 100, data.humidity / 10, data.humidity % 10);
	} else {
		nxmutex_unlock(&priv->devlock);
		return -EINVAL;
	}
...
static int hdc1008_reset(FAR struct hdc1008_dev_s *priv)
{
	...
	ret = hdc1008_putreg(priv, HDC1008_REG_CONFIGURATION, reg | HDC1008_CONFIGURATION_RST);
	if (ret < 0) {
		return ret;
	}
...
int hdc1008_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	FAR struct hdc1008_dev_s *priv;
	uint16_t reg;
	int ret;
