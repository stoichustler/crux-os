...
#include <roux/sensors/cxd5602pwbimu.h>
...
static uint8_t cxd5602pwbimu_spigetreg8(FAR struct cxd5602pwbimu_dev_s *priv, uint8_t regaddr);
static void cxd5602pwbimu_spiputreg8(FAR struct cxd5602pwbimu_dev_s *priv, uint8_t regaddr,
									 uint8_t regval);
static void cxd5602pwbimu_recv(FAR struct cxd5602pwbimu_dev_s *priv, FAR uint8_t *buffer, int len);

static int cxd5602pwbimu_getregsn(FAR struct cxd5602pwbimu_dev_s *priv, int slaveid,
								  uint8_t regaddr, FAR uint8_t *buffer, uint8_t len);
static int cxd5602pwbimu_getregsn_woadr(FAR struct cxd5602pwbimu_dev_s *priv, int slaveid,
										FAR uint8_t *buffer, uint8_t len);
static int cxd5602pwbimu_putregsn(FAR struct cxd5602pwbimu_dev_s *priv, int slaveid,
								  uint8_t regaddr, FAR uint8_t *buffer, uint8_t len);
static int cxd5602pwbimu_putregs(FAR struct cxd5602pwbimu_dev_s *priv, uint8_t regaddr,
								 FAR uint8_t *buffer, uint8_t len);
static int cxd5602pwbimu_putreg8(FAR struct cxd5602pwbimu_dev_s *priv, uint8_t regaddr,
								 uint8_t regval);
static int cxd5602pwbimu_putreg8n(FAR struct cxd5602pwbimu_dev_s *priv, int slaveid,
								  uint8_t regaddr, uint8_t regval);

/* Device control methods */

static int cxd5602pwbimu_enable(FAR struct cxd5602pwbimu_dev_s *priv, bool enable);
static int cxd5602pwbimu_setodr(FAR struct cxd5602pwbimu_dev_s *priv, uint32_t rate);
static int cxd5602pwbimu_setcalib(FAR struct cxd5602pwbimu_dev_s *priv,
								  FAR cxd5602pwbimu_calib_t *calib);
static int cxd5602pwbimu_setdrange(FAR struct cxd5602pwbimu_dev_s *priv, int accel, int gyro);

/* Firmware update related functions */

static int cxd5602pwbimu_updatemode(FAR struct cxd5602pwbimu_dev_s *priv);
static int cxd5602pwbimu_waitforready(FAR struct cxd5602pwbimu_dev_s *priv, int slaveid);
static int cxd5602pwbimu_sendfwchunk(FAR struct cxd5602pwbimu_dev_s *priv, int slaveid,
									 FAR uint8_t *buf, int len);
static int cxd5602pwbimu_verifyfw(FAR struct cxd5602pwbimu_dev_s *priv, int slaveid);
...
static ssize_t cxd5602pwbimu_read(FAR struct file *filep, FAR char *buffer, size_t len);
static int cxd5602pwbimu_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int cxd5602pwbimu_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int cxd5602pwbimu_int_handler(int irq, FAR void *context, FAR void *arg);
...
static int cxd5602pwbimu_enable(FAR struct cxd5602pwbimu_dev_s *priv, bool enable)
{
	...
	if (enable) {
		...
		ret = cxd5602pwbimu_putreg8(priv, CXD5602PWBIMU_OUTPUT_ENABLE, OUTPUT_ENABLE);
		if (!ret) {
		...
		cxd5602pwbimu_spiputreg8(priv, CXD5602PWBIMU_OUTPUT_ENABLE, OUTPUT_DISABLE);
								  ...
								  FAR cxd5602pwbimu_calib_t *calib)
{
	...
	ret = cxd5602pwbimu_putregs(priv, CXD5602PWBIMU_USER_CALIB_COEF, (FAR uint8_t *)calib,
...
static int cxd5602pwbimu_setfifothresh(FAR struct cxd5602pwbimu_dev_s *priv, int thresh)
{
	size_t size;
	int ret;
...
static int cxd5602pwbimu_updatemode(FAR struct cxd5602pwbimu_dev_s *priv)
{
	...
	for (i = 0; i < priv->nslaves; i++) {
		ret = cxd5602pwbimu_putreg8n(priv, i, CXD5602PWBIMU_CHANGE_TO_UPDATEMODE, 1);
		if (ret) {
			return ret;
		}
	}
...
static int cxd5602pwbimu_waitforready(FAR struct cxd5602pwbimu_dev_s *priv, int slaveid)
{
	...
	retry = 1000;
	do {
		ret = cxd5602pwbimu_getregsn(priv, slaveid, CXD5602PWBIMU_INJECTION_READY, &val, 1);
		if (ret) {
			return ret;
		}
	} while (val == 0 && --retry);
									 ...
									 FAR uint8_t *buf, int len)
{
	...
	ret = cxd5602pwbimu_putregsn(priv, slaveid, CXD5602PWBIMU_INJECT_BINARY, buf, len);
...
static int cxd5602pwbimu_verifyfw(FAR struct cxd5602pwbimu_dev_s *priv, int slaveid)
{
	...
	val = 1;
	ret = cxd5602pwbimu_putregsn(priv, slaveid, CXD5602PWBIMU_VERIFY_BINARY, &val, 1);
	if (ret) {
		return ret;
	}
	...
	retry = 1000;
	do {
		ret = cxd5602pwbimu_getregsn(priv, slaveid, CXD5602PWBIMU_UPDATE_RESULT, &val, 1);
		if (ret) {
			return ret;
		}
...
static int cxd5602pwbimu_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SNIOC_WREGS: {
		...
		ret = cxd5602pwbimu_putregsn(priv, p->slaveid, p->addr, p->value, p->len);
	} break;

	case SNIOC_RREGS: {
		FAR cxd5602pwbimu_regs_t *p = (FAR cxd5602pwbimu_regs_t *)arg;

		ret = cxd5602pwbimu_getregsn(priv, p->slaveid, p->addr, p->value, p->len);
	} break;
	...
	case SNIOC_GETBNUM: {
		ret = priv->nslaves == 2 ? 1 : priv->nslaves == 4 ? 2 : 0;
	} break;
...
int cxd5602pwbimu_register(FAR const char *devpath, FAR struct spi_dev_s *dev_spi,
						   FAR struct i2c_master_s *dev_i2c, FAR cxd5602pwbimu_config_t *config)
{
	FAR struct cxd5602pwbimu_dev_s *priv;
