...
static uint8_t adxl372_getreg8(FAR struct adxl372_sensor_s *priv, uint8_t regaddr);
static void adxl372_putreg8(FAR struct adxl372_sensor_s *priv, uint8_t regaddr, uint8_t regval);
static void adxl372_getregs(FAR struct adxl372_sensor_s *priv, uint8_t regaddr, FAR uint8_t *regval,
							int len);
...
static int adxl372_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							bool enable);
static int adxl372_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								FAR uint32_t *period_us);
#ifndef CONFIG_SENSORS_ADXL372_POLL
static int adxl372_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static void adxl372_start(FAR struct adxl372_sensor_s *priv)
{
	adxl372_putreg8(priv, ADXL372_POWER_CTL,
					ADXL372_POWER_HPF_DISABLE | ADXL372_POWER_MODE_MEASURE);
}
...
static int adxl372_thread(int argc, FAR char **argv)
{
	FAR struct adxl372_sensor_s *priv =
		(FAR struct adxl372_sensor_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	struct sensor_accel accel;
	uint8_t data[6];
	int ret;
...
int adxl372_register_uorb(int devno, FAR struct spi_dev_s *spi)
{
	...
	ret = kthread_create("adxl372_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_ADXL372_THREAD_STACKSIZE, adxl372_thread, argv);
	if (ret < 0) {
		kmm_free(priv);
	}
#endif
