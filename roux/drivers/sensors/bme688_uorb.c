...
#define CHECK_OS_BOUNDS(type) ((type) >= BME688_OS_SKIPPED && (type) <= BME688_OS_16X)
...
typedef int (*push_data_func)(FAR struct bme688_dev_s *priv, FAR struct bme688_data_s *data);
...
static int bme688_putreg8(FAR struct bme688_dev_s *priv, uint8_t regaddr, uint8_t regval);
static int bme688_getregs(FAR struct bme688_dev_s *priv, uint8_t regaddr, uint8_t *rxbuffer,
						  uint8_t length);

#ifndef CONFIG_BME688_DISABLE_TEMP_MEAS
static int bme688_push_temp_data(FAR struct bme688_dev_s *priv, FAR struct bme688_data_s *data);
#endif

#ifndef CONFIG_BME688_DISABLE_PRESS_MEAS
static int bme688_push_press_data(FAR struct bme688_dev_s *priv, FAR struct bme688_data_s *data);
#endif

#ifndef CONFIG_BME688_DISABLE_HUM_MEAS
static int bme688_push_hum_data(FAR struct bme688_dev_s *priv, FAR struct bme688_data_s *data);
#endif

#ifndef CONFIG_BME688_DISABLE_GAS_MEAS
static int bme688_push_gas_data(FAR struct bme688_dev_s *priv, FAR struct bme688_data_s *data);
#endif

/* Sensor methods */

static int bme688_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						   bool enable);
static int bme688_calibrate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							unsigned long arg);
static int bme688_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
						  unsigned long arg);
...
static int bme688_get_calib_data(FAR struct bme688_dev_s *priv)
{
	...
	ret = bme688_getregs(priv, BME688_COEFF_ADDR1, coeff, BME688_COEFF_ADDR1_LEN);
	if (ret < 0) {
		return ret;
	}
	...
	ret = bme688_getregs(priv, BME688_COEFF_ADDR2, &coeff[BME688_COEFF_ADDR1_LEN],
	...
	priv->dev.calib.t1 = coeff[BME688_T1_MSB_REG] << 8 | coeff[BME688_T1_LSB_REG];
	priv->dev.calib.t2 = coeff[BME688_T2_MSB_REG] << 8 | coeff[BME688_T2_LSB_REG];
	priv->dev.calib.t3 = coeff[BME688_T3_REG];

#ifndef CONFIG_BME688_DISABLE_PRESS_MEAS
	priv->dev.calib.p1 = coeff[BME688_P1_MSB_REG] << 8 | coeff[BME688_P1_LSB_REG];
	priv->dev.calib.p2 = coeff[BME688_P2_MSB_REG] << 8 | coeff[BME688_P2_LSB_REG];
	priv->dev.calib.p3 = coeff[BME688_P3_REG];
	priv->dev.calib.p4 = coeff[BME688_P4_MSB_REG] << 8 | coeff[BME688_P4_LSB_REG];
	priv->dev.calib.p5 = coeff[BME688_P5_MSB_REG] << 8 | coeff[BME688_P5_LSB_REG];
	priv->dev.calib.p6 = coeff[BME688_P6_REG];
	priv->dev.calib.p7 = coeff[BME688_P7_REG];
	priv->dev.calib.p8 = coeff[BME688_P8_MSB_REG] << 8 | coeff[BME688_P8_LSB_REG];
	priv->dev.calib.p9 = coeff[BME688_P9_MSB_REG] << 8 | coeff[BME688_P9_LSB_REG];
	...
	priv->dev.calib.h2 =
		(uint16_t)(((uint16_t)coeff[BME688_H2_MSB_REG] << 4) | ((coeff[BME688_H2_LSB_REG]) >> 4));
	...
	priv->dev.calib.gh2 = coeff[BME688_GH2_MSB_REG] << 8 | coeff[BME688_GH2_LSB_REG];
	...
	priv->dev.calib.range_sw_err = ((int8_t)temp_val & (int8_t)BME688_RSERROR_MSK) / 16;
...
static int bme688_push_press_data(FAR struct bme688_dev_s *priv, FAR struct bme688_data_s *data)
{
	...
	ret = lower.push_event(lower.priv, &press_data, sizeof(struct sensor_baro));
...
static uint8_t calc_heater_res(FAR const struct bme688_dev_s *priv)
{
	...
	var2 = (dev.calib.gh1 + 784) * (((((dev.calib.gh2 + 154009) * temp * 5) / 100) + 3276880) / 10);
	var3 = var1 + (var2 / 2);
...
static int bme688_configure(FAR struct bme688_dev_s *priv, FAR struct bme688_config_s *config)
{
	int ret;
...
#ifdef CONFIG_BME688_ENABLE_IIR_FILTER
	if (config->filter_coef < BME688_FILTER_COEF0 || config->filter_coef > BME688_FILTER_COEF127) {
		return -EINVAL;
	}
#endif

#ifndef CONFIG_BME688_DISABLE_GAS_MEAS
	if (config->target_temp < MIN_HOT_PLATE_TEMP || config->target_temp > MAX_HOT_PLATE_TEMP) {
		return -EINVAL;
	}
...
static float bme688_comp_temp(FAR struct bme688_dev_s *priv, uint32_t adc_temp)
{
	float var1 = 0.0f;
	float var2 = 0.0f;
	...
	var1 =
		((((float)adc_temp / 16384.0f) - ((float)dev.calib.t1 / 1024.0f)) * ((float)dev.calib.t2));
...
static float bme688_comp_press(FAR struct bme688_dev_s *priv, uint32_t adc_press)
{
	float var1 = 0.0f;
	float var2 = 0.0f;
	float var3 = 0.0f;
	...
	var1 = (((((float)dev.calib.p3 * var1 * var1) / 16384.0f) + ((float)dev.calib.p2 * var1)) /
			524288.0f);
	...
	if ((int)var1 != 0) {
		...
		var3 = ((calc_pres / 256.0f) * (calc_pres / 256.0f) * (calc_pres / 256.0f) *
				(dev.calib.p10 / 131072.0f));
		calc_pres = (calc_pres + (var1 + var2 + var3 + ((float)dev.calib.p7 * 128.0f)) / 16.0f);
	}
...
static float bme688_comp_hum(FAR struct bme688_dev_s *priv, uint16_t adc_hum)
{
	float calc_hum = 0.0f;
	float var1 = 0.0f;
	float var2 = 0.0f;
	...
	var1 = (float)((float)adc_hum) -
		   (((float)dev.calib.h1 * 16.0f) + (((float)dev.calib.h3 / 2.0f) * temp_comp));
...
static float bme688_calc_gas_res(FAR struct bme688_dev_s *priv, uint16_t adc_gas_res,
								 uint8_t gas_range)
{
...
static int bme688_read_measurements(FAR struct bme688_dev_s *priv, FAR struct bme688_data_s *data)
{
	uint8_t status;
	uint32_t adc_temp;
	...
	adc_temp = (uint32_t)(((uint32_t)data_regs[5] << 12) | ((uint32_t)data_regs[6] << 4) |
						  ((uint32_t)data_regs[7] >> 4));

	data->temperature = bme688_comp_temp(priv, adc_temp);

#ifndef CONFIG_BME688_DISABLE_PRESS_MEAS
	adc_press = (uint32_t)(((uint32_t)data_regs[2] << 12) | ((uint32_t)data_regs[3] << 4) |
...
#ifndef CONFIG_BME688_DISABLE_HUM_MEAS
	adc_hum = (uint16_t)(((uint32_t)data_regs[8] << 8) | (uint32_t)data_regs[9]);

	data->humidity = bme688_comp_hum(priv, adc_hum);
#endif /* !CONFIG_BME688_DISABLE_HUM_MEAS */

#ifndef CONFIG_BME688_DISABLE_GAS_MEAS
	adc_gas_res = (uint16_t)((uint32_t)data_regs[15] << 2 | (((uint32_t)data_regs[16]) >> 6));
						   ...
						   bool enable)
{
	...
	dev = (FAR struct bme688_sensor_s *)((uintptr_t)lower - offset * sizeof(*lower));
...
static int bme688_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
						  unsigned long arg)
{
	FAR struct bme688_sensor_s *dev = container_of(lower, FAR struct bme688_sensor_s, lower);
	FAR struct bme688_dev_s *priv = container_of(dev, FAR struct bme688_dev_s, dev);
	int ret;
...
int bme688_register(int devno, FAR struct i2c_master_s *i2c, FAR struct bme688_config_s *config)
{
	...
	ret = kthread_create("bme688_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_BME688_THREAD_STACKSIZE, bme688_thread, argv);
	if (ret < 0) {
