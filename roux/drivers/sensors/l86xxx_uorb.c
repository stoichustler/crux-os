...
#if CONFIG_L86_XXX_BAUD == 4800 || CONFIG_L86_XXX_BAUD == 9600 || CONFIG_L86_XXX_BAUD == 14400 ||  \
	CONFIG_L86_XXX_BAUD == 19200 || CONFIG_L86_XXX_BAUD == 38400 ||                                \
	CONFIG_L86_XXX_BAUD == 57600 || CONFIG_L86_XXX_BAUD == 115200
...
static int l86xxx_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
						  unsigned long arg);
static int l86xxx_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						   bool enable);
static int l86xxx_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
{
	FAR l86xxx_dev_s *dev = (FAR l86xxx_dev_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
		...
		if (bw > 0) {
			...
			case MINMEA_SENTENCE_VTG: {
				...
				if (minmea_parse_vtg(&frame, dev->buffer)) {
					gps.ground_speed =
						minmea_tofloat(&frame.speed_kph) * 3.6; /* Convert speed in kph to mps */
...
int l86xxx_register(FAR const char *uartpath, int devno)
{
	...
	err = kthread_create("l86xxx_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_L86_XXX_THREAD_STACKSIZE, l86xxx_thread, argv);
	...
	sninfo("Registered L86-XXX driver with kernel polling thread"
		   ...
		   L86_XXX_BAUD_RATE, CONFIG_L86_XXX_FIX_INT);
