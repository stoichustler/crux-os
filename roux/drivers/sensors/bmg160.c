...
static void bmg160_read_register(FAR struct bmg160_dev_s *dev, uint8_t const reg_addr,
								 uint8_t *reg_data);
static void bmg160_write_register(FAR struct bmg160_dev_s *dev, uint8_t const reg_addr,
...
static void bmg160_read_gyroscope_data(FAR struct bmg160_dev_s *dev, uint16_t *x_gyr,
...
static ssize_t bmg160_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int bmg160_interrupt_handler(int irq, FAR void *context)
{
	...
	for (priv = g_bmg160_list; priv && priv->config->irq != irq; priv = priv->flink)
		;
...
static int bmg160_open(FAR struct file *filep)
{
	...
	bmg160_write_register(priv, BMG160_RANGE_REG,
	...
	bmg160_write_register(priv, BMG160_INT_EN_0_REG, BMG160_INT_EN_0_REG_DATA_EN_BM);

	/* Enable - active high level interrupt 1 - push-pull interrupt */

	bmg160_write_register(priv, BMG160_INT_EN_1_REG, BMG160_INT_EN_1_REG_INT1_LVL_BM);

	/* Enable - map new data interrupt to INT1 */

	bmg160_write_register(priv, BMG160_INT_MAP_1_REG, BMG160_INT_MAP_1_REG_INT1_DATA_BM);
...
static ssize_t bmg160_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct bmg160_dev_s *priv = inode->i_private;
