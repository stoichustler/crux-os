...
static int bmp180_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							   FAR uint32_t *period_us);
static int bmp180_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						   bool enable);
						   ...
						   bool enable)
{
	...
	if (enable) {
		work_queue(HPWORK, &priv->work, bmp180_worker, priv, priv->interval / USEC_PER_TICK);
	} else {
...
int bmp180_register_uorb(int devno, FAR struct i2c_master_s *i2c)
{
	...
	priv = (FAR struct bmp180_dev_uorb_s *)kmm_zalloc(sizeof(struct bmp180_dev_uorb_s));
