...
#include <roux/sensors/lis3dsh.h>
...
static void lis3dsh_read_register(FAR struct lis3dsh_dev_s *dev, uint8_t const reg_addr,
								  uint8_t *reg_data);
#endif

static void lis3dsh_write_register(FAR struct lis3dsh_dev_s *dev, uint8_t const reg_addr,
...
static void lis3dsh_read_acclerometer_data(FAR struct lis3dsh_dev_s *dev, uint16_t *x_acc,
										   uint16_t *y_acc, uint16_t *z_acc);
static int lis3dsh_interrupt_handler(int irq, FAR void *context, FAR void *arg);
...
static ssize_t lis3dsh_read(FAR struct file *, FAR char *buffer, size_t buflen);
static ssize_t lis3dsh_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int lis3dsh_open(FAR struct file *filep)
{
	...
	lis3dsh_write_register(priv, LIS3DSH_CTRL_REG_5, LIS3DSH_CTRL_REG_5_FSCALE_2_BM);

	/* Enable - Auto increment of address when reading multiple bytes */

	lis3dsh_write_register(priv, LIS3DSH_CTRL_REG_6, LIS3DSH_CTRL_REG_6_ADD_INC_BM);
	...
	lis3dsh_write_register(priv, LIS3DSH_CTRL_REG_4,
	...
	lis3dsh_write_register(priv, LIS3DSH_CTRL_REG_3,
...
static ssize_t lis3dsh_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct lis3dsh_dev_s *priv = inode->i_private;
					 ...
					 FAR struct lis3dsh_config_s *config)
{
	...
	priv = kmm_malloc(sizeof(struct lis3dsh_dev_s));
	if (priv == NULL) {
