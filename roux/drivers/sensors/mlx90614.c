...
static int mlx90614_read_word(FAR struct mlx90614_dev_s *priv, uint8_t cmd, FAR uint16_t *regval);
static int mlx90614_write_word(FAR struct mlx90614_dev_s *priv, uint8_t cmd, uint16_t regval);

/* Character driver methods */

static ssize_t mlx90614_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t mlx90614_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int mlx90614_read_word(FAR struct mlx90614_dev_s *priv, uint8_t cmd, FAR uint16_t *regval)
{
	...
	ret = i2c_writeread(priv->i2c, &config, (FAR const uint8_t *)&cmd, 1, buffer, 3);
	if (ret < 0) {
...
int mlx90614_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	...
	FAR struct mlx90614_dev_s *priv = kmm_malloc(sizeof(struct mlx90614_dev_s));
