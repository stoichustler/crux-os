...
#include <roux/sensors/goldfish_gnss.h>
...
static int goldfish_gnss_activate(FAR struct gnss_lowerhalf_s *lower, FAR struct file *filep,
								  bool enabled);
...
static inline int goldfish_gnss_write_pipe(FAR struct file *pipe, FAR const void *buffer,
										   size_t size)
{
...
static inline int goldfish_gnss_open_pipe(FAR struct file *filep, FAR const char *ns,
										  FAR const char *pipe_name, int flags)
{
	...
	if (ret < 0) {
		snerr("Could not open '%s': %s", "/dev/goldfish_pipe", strerror(-ret));
		return ret;
	}
	...
	if (ret < 0) {
		snerr("Could not connect to the '%s' service: %s", buf, strerror(-ret));
		file_close(filep);
		return ret;
	}
...
static int goldfish_gnss_activate(FAR struct gnss_lowerhalf_s *gnss, FAR struct file *filep,
								  bool enabled)
{
	FAR struct goldfish_gnss_s *priv = container_of(gnss, struct goldfish_gnss_s, gnss);
...
static int goldfish_gnss_thread(int argc, FAR char **argv)
{
	FAR struct goldfish_gnss_s *priv =
		(FAR struct goldfish_gnss_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	ssize_t len;
	char buf[256];
...
int goldfish_gnss_init(int devno, uint32_t batch_number)
{
	...
	ret = goldfish_gnss_open_pipe(&gnss->pipe, "qemud", "gps", O_RDWR | O_CLOEXEC);
	if (ret < 0) {
		kmm_free(gnss);
		return ret;
	}
	...
	ret = kthread_create("goldfish_gnss_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_DEFAULT_TASK_STACKSIZE, goldfish_gnss_thread, argv);
	if (ret < 0) {
