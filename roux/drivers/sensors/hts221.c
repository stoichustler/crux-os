...
static ssize_t hts221_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t hts221_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int hts221_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int hts221_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int hts221_do_transfer(FAR struct hts221_dev_s *priv, FAR struct i2c_msg_s *msgv,
							  size_t nmsg)
{
	int ret = -EIO;
	int retries;
...
static int32_t hts221_write_reg8(FAR struct hts221_dev_s *priv, const uint8_t *command)
{
	struct i2c_msg_s msgv[2] = {{.frequency = CONFIG_HTS221_I2C_FREQUENCY,
								 .addr = priv->addr,
								 .flags = 0,
...
static int hts221_read_reg(FAR struct hts221_dev_s *priv, FAR const uint8_t *command,
						   FAR uint8_t *value)
{
	struct i2c_msg_s msgv[2] = {{.frequency = CONFIG_HTS221_I2C_FREQUENCY,
								 .addr = priv->addr,
...
static int hts221_cfgr_resolution(FAR struct hts221_dev_s *priv, FAR hts221_settings_t *settings)
{
	int ret;
	uint8_t value;
	...
	hts221_dbg("Resolution changed: temp=%d humid=%d ret=%d\n", settings->temp_resol,
			   settings->humid_resol, ret);

	return ret;
}

static int hts221_config_ctrl_reg3(FAR struct hts221_dev_s *priv, FAR hts221_settings_t *settings)
{
	int ret = OK;
	uint8_t regval = 0;
...
static int hts221_config_ctrl_reg2(FAR struct hts221_dev_s *priv, FAR hts221_settings_t *settings)
{
	int ret = OK;
	uint8_t regval = 0;
...
static int hts221_config_ctrl_reg1(FAR struct hts221_dev_s *priv, FAR hts221_settings_t *settings)
{
	int ret = OK;
	uint8_t regval = 0;
...
static int hts221_config(FAR struct hts221_dev_s *priv, FAR hts221_settings_t *cfgr)
{
	int ret = OK;
...
static int hts221_check_status(FAR struct hts221_dev_s *priv, FAR hts221_status_t *status)
{
	int ret = OK;
...
static int hts221_read_raw_data(FAR struct hts221_dev_s *priv, FAR hts221_raw_data_t *data)
{
	int ret = OK;
	...
	hts221_dbg("Humid: 0x%02X, 0x%02X Temper: 0x%02X 0x%02X\n", data->humid_high_bits,
			   data->humid_low_bits, data->temp_high_bits, data->temp_low_bits);
...
static int hts221_calculate_temperature(FAR struct hts221_dev_s *priv, FAR int *temperature,
...
static int hts221_calculate_humidity(FAR struct hts221_dev_s *priv, FAR unsigned int *humidity,
									 FAR hts221_raw_data_t *raw_data)
{
	int16_t h_out;
...
static int hts221_read_convert_data(FAR struct hts221_dev_s *priv, FAR hts221_conv_data_t *data)
{
	int ret = OK;
...
{
	int ret;
	hts221_status_t status = {.is_humid_ready = false, .is_temp_ready = false};
...
int hts221_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
					FAR hts221_config_t *config)
{
	int ret = 0;
	FAR struct hts221_dev_s *priv;
