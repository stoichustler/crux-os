...
#include <roux/sensors/mlx90393.h>
...
static void mlx90393_read_register(FAR struct mlx90393_dev_s *dev, uint8_t const reg_addr,
								   uint16_t *reg_data);
static void mlx90393_write_register(FAR struct mlx90393_dev_s *dev, uint8_t const reg_addr,
...
static ssize_t mlx90393_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static void mlx90393_read_measurement_data(FAR struct mlx90393_dev_s *dev)
{
	...
	add_sensor_randomness((x_mag << 17) ^ (y_mag << 9) ^ (z_mag << 1) ^ temperature);
}
...
static int mlx90393_interrupt_handler(int irq, FAR void *context)
{
	...
	for (priv = g_mlx90393_list; priv && priv->config->irq != irq; priv = priv->flink)
		;
...
static ssize_t mlx90393_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct mlx90393_dev_s *priv = inode->i_private;
