...
#include <roux/sensors/apds9922.h>
#include <sys/ioctl.h>
...
#define APDS9922_PACK_TO_UINT32(a) (((a)[0] >> 24) | ((a)[1] >> 16) | ((a)[2] >> 8) | ((a)[3]))
#define APDS9922_PACK_TO_UINT16(a) (((a)[0] >> 0) | ((a)[1]))
...
#else
#define APDS9922_PACK_TO_UINT32(a) (((a)[3] << 24) | ((a)[2] << 16) | ((a)[1] << 8) | ((a)[0]))
#define APDS9922_PACK_TO_UINT16(a) (((a)[1] << 8) | ((a)[0]))
#define APDS9922_UNPACK_FROM_UINT32(w, b)                                                          \
	do {                                                                                           \
		(b)[3] = ((w) >> 24) & 0xff;                                                               \
		(b)[2] = ((w) >> 16) & 0xff;                                                               \
...
static const struct als_data als_data[] = {
	{1, 400, 1048575}, {3, 200, 524287}, {6, 100, 262143}, {9, 50, 131071}, {18, 25, 65535},
};
...
static int apds9922_i2c_read(FAR struct apds9922_dev_s *priv, uint8_t const regaddr,
							 FAR uint8_t *regval, int len);
static int apds9922_i2c_write(FAR struct apds9922_dev_s *priv, uint8_t const regaddr,
...
static int apds9922_als_persistance(FAR struct apds9922_dev_s *priv, uint8_t persistance);
static int apds9922_als_variance(FAR struct apds9922_dev_s *priv, uint8_t variance);
...
static int apds9922_als_factor(FAR struct apds9922_dev_s *priv, uint32_t factor);
static int apds9922_als_limit(FAR struct apds9922_dev_s *priv, uint32_t limit);
...
static int apds9922_ps_canc_lev(FAR struct apds9922_dev_s *priv, uint16_t lev);
static int apds9922_ps_int_mode(FAR struct apds9922_dev_s *priv, int mode);
static int apds9922_ps_persistance(FAR struct apds9922_dev_s *priv, uint8_t persistance);
static int apds9922_ps_notify_mode(FAR struct apds9922_dev_s *priv, int notify);

/* Character driver methods */

static int apds9922_open(FAR struct file *filep);
static int apds9922_close(FAR struct file *filep);
static ssize_t apds9922_als_read(FAR struct file *filep, FAR char *, size_t buflen);
static ssize_t apds9922_als_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int apds9922_als_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int apds9922_als_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
static ssize_t apds9922_ps_read(FAR struct file *filep, FAR char *, size_t buflen);
static ssize_t apds9922_ps_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int apds9922_ps_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int apds9922_ps_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
							   ...
							   FAR struct adps9922_als_thresh thresholds)
{
	...
	if (thresholds.upper > threshmax) {
		snerr("ALS upper threshold out of range: %" PRIu32 ", max: %" PRIu32 "\n", thresholds.upper,
			  threshmax);
		return -EINVAL;
	}

	if (thresholds.lower > threshmax) {
		snerr("ALS lower threshold out of range: %" PRIu32 ", max: %" PRIu32 "\n", thresholds.lower,
			  threshmax);
		return -EINVAL;
	}
...
static ssize_t apds9922_als_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
...
static ssize_t apds9922_ps_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct apds9922_dev_s *priv = inode->i_private;
...
int apds9922_register(FAR const char *devpath_als, FAR const char *devpath_ps,
