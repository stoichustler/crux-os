...
static int sht21_access(FAR struct sht21_dev_s *priv, uint8_t reg_addr, bool read,
						FAR uint8_t *reg_value, uint8_t len);
#ifdef CONFIG_SHT21_DEBUG
static int sht21_read8(FAR struct sht21_dev_s *priv, uint8_t regaddr, FAR uint8_t *regvalue);
#endif
static int sht21_read16(FAR struct sht21_dev_s *priv, uint8_t regaddr, FAR uint16_t *regvalue);
...
static ssize_t sht21_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int sht21_read_values(FAR struct sht21_dev_s *priv, FAR int *temp, FAR int *rh)
{
	uint16_t temp16;
	uint16_t rh16;
	struct timespec ts;
...
static int sht21_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SNIOC_READ_CONVERT_DATA: {
		...
		if (ret < 0) {
			...
			FAR struct sht21_conv_data_s *data = (FAR struct sht21_conv_data_s *)arg;
...
int sht21_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	FAR struct sht21_dev_s *priv;
	int ret;
