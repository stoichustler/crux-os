...
#include <roux/sensors/ina3221.h>
...
static int ina3221_write16(FAR struct ina3221_dev_s *priv, uint8_t regaddr, FAR uint16_t regvalue);
static int ina3221_read16(FAR struct ina3221_dev_s *priv, uint8_t regaddr, FAR uint16_t *regvalue);
static int ina3221_readpower(FAR struct ina3221_dev_s *priv, FAR struct ina3221_s *buffer);

/* Character driver methods */

static ssize_t ina3221_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t ina3221_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int ina3221_access(FAR struct ina3221_dev_s *priv, uint8_t start_register_address,
...
static int ina3221_readpower(FAR struct ina3221_dev_s *priv, FAR struct ina3221_s *buffer)
{
	...
	for (i = 0; i < 3; i++) {
		if (priv->config & (INA3221_CONFIG_CH1_EN >> i)) {
			...
			ret = ina3221_read16(priv, (INA3221_REG_CH1_BUS_VOLTAGE + i * 2), &reg);
			if (ret < 0) {
			...
			ret = ina3221_read16(priv, (INA3221_REG_CH1_SHUNT_VOLTAGE + i * 2), &reg);
			if (ret < 0) {
				snerr("ERROR: ina3221_read16 failed: %d\n", ret);
				return ret;
			}
