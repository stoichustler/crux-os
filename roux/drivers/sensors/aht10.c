...
static int aht10_writereg(FAR struct aht10_dev_s *priv, uint8_t reg, FAR uint8_t *data);
static int aht10_readregs(FAR struct aht10_dev_s *priv, FAR uint8_t *buf, uint8_t len);
...
static ssize_t aht10_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int aht10_read_values(FAR struct aht10_dev_s *priv, FAR int *temp, FAR int *rh)
{
	uint8_t buf[6];
	uint32_t temp20;
	uint32_t rh20;
...
static int aht10_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SNIOC_READ_CONVERT_DATA: {
		...
		if (ret < 0) {
			...
			FAR struct aht10_conv_data_s *data = (FAR struct aht10_conv_data_s *)arg;
...
int aht10_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	FAR struct aht10_dev_s *priv;
	int ret;
