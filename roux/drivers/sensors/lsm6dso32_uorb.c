...
#include <roux/random.h>
#include <roux/semaphore.h>
#include <roux/sensors/lsm6dso32.h>
...
static int lsm6dso32_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
							 unsigned long arg);
static int lsm6dso32_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							  bool enable);
static int lsm6dso32_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								  FAR uint32_t *period_us);
static int lsm6dso32_selftest(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							  unsigned long arg);
static int lsm6dso32_set_calibvalue(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
									unsigned long arg);
static int lsm6dso32_get_info(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static int lsm6dso32_write_bytes(FAR struct lsm6dso32_dev_s *priv, uint8_t addr, void *buf,
								 size_t nbytes)
{
...
static int lsm6dso32_read_bytes(FAR struct lsm6dso32_dev_s *priv, uint8_t addr, void *buf,
								size_t nbytes)
{
	struct i2c_msg_s cmd[2];
...
static int lsm6dso32_set_bits(FAR struct lsm6dso32_dev_s *priv, uint8_t addr, uint8_t set_bits,
							  uint8_t clear_bits)
{
	int err;
	uint8_t reg;
...
static int accel_set_odr(FAR struct lsm6dso32_dev_s *dev, enum lsm6dso32_odr_e odr)
{
	int err;
...
static int gyro_set_odr(FAR struct lsm6dso32_dev_s *dev, enum lsm6dso32_odr_e odr)
{
	int err;
...
static int accel_set_fsr(FAR struct lsm6dso32_dev_s *dev, enum lsm6dso32_fsr_xl_e fsr)
{
	int err;
...
static int gyro_set_fsr(FAR struct lsm6dso32_dev_s *dev, enum lsm6dso32_fsr_gyro_e fsr)
{
	int err;
...
static int gyro_int_enable(FAR struct lsm6dso32_dev_s *dev, bool enable)
{
	...
	err = lsm6dso32_set_bits(dev, INT_CTRL[dev->gyro.intpin], enable_bits, disable_bits);
	if (err < 0) {
		return err;
	}
...
static int accel_int_enable(FAR struct lsm6dso32_dev_s *dev, bool enable)
{
	...
	err = lsm6dso32_set_bits(dev, INT_CTRL[dev->accel.intpin], enable_bits, disable_bits);
	if (err < 0) {
		return err;
	}
...
static int lsm6dso32_read_gyro(FAR struct lsm6dso32_dev_s *dev, FAR struct sensor_gyro *data)
{
	...
	data->x = (float)(raw_data[1]) * FSR_GYRO_SENS[dev->gyro.fsr] - dev->gy_off[0];
	data->y = (float)(raw_data[2]) * FSR_GYRO_SENS[dev->gyro.fsr] - dev->gy_off[1];
	data->z = (float)(raw_data[3]) * FSR_GYRO_SENS[dev->gyro.fsr] - dev->gy_off[2];
...
static int lsm6dso32_read_accel(FAR struct lsm6dso32_dev_s *dev, FAR struct sensor_accel *data)
{
...
static int lsm6dso32_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							  bool enable)
{
	FAR struct lsm6dso32_sens_s *sens = container_of(lower, FAR struct lsm6dso32_sens_s, lower);
...
static int gyro_selftest(FAR struct lsm6dso32_dev_s *dev)
{
	...
	uint8_t test_control_regs[10] = {0x0, 0x5c, 0x44, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
	...
	err = lsm6dso32_read_bytes(dev, CTRL1_XL, prev_control_regs, sizeof(prev_control_regs));
	if (err < 0) {
	...
	if (!(GYRO_ST_MIN <= gyro_st[0] && gyro_st[0] <= GYRO_ST_MAX && GYRO_ST_MIN <= gyro_st[1] &&
		  gyro_st[1] <= GYRO_ST_MAX && GYRO_ST_MIN <= gyro_st[2] && gyro_st[2] <= GYRO_ST_MAX)) {
		snerr("LSM6DSO32 failed gyro self-test (%d X, %d Y, %d Z).", gyro_st[0], gyro_st[1],
			  gyro_st[2]);
...
early_restore:
	...
	restore_err =
		lsm6dso32_write_bytes(dev, CTRL1_XL, prev_control_regs, sizeof(prev_control_regs));
...
static int accel_selftest(FAR struct lsm6dso32_dev_s *dev)
{
	...
	uint8_t prev_control_regs[10];
	uint8_t test_control_regs[10] = {0x30, 0x00, 0x44, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
	...
	if (!(XL_ST_MIN <= accel_st[0] && accel_st[0] <= XL_ST_MAX && XL_ST_MIN <= accel_st[1] &&
		  accel_st[1] <= XL_ST_MAX && XL_ST_MIN <= accel_st[2] && accel_st[2] <= XL_ST_MAX)) {
		snerr("LSM6DSO32 failed accel self-test (%d X, %d Y, %d Z)\n", accel_st[0], accel_st[1],
			  accel_st[2]);
		err = -EAGAIN; /* Failed self-test */
	}
...
int lsm6dso32_register(FAR struct i2c_master_s *i2c, uint8_t addr, uint8_t devno,
					   struct lsm6dso32_config_s *config)
{
	...
	DEBUGASSERT(config->gy_int == LSM6DSO32_INT1 || config->gy_int == LSM6DSO32_INT2);
	DEBUGASSERT(config->xl_int == LSM6DSO32_INT1 || config->xl_int == LSM6DSO32_INT2);
	...
	if (config->gy_attach != NULL) {
		...
		err = kthread_create("lsm6dso32_gy_thread", SCHED_PRIORITY_DEFAULT,
							 CONFIG_SENSORS_LSM6DSO32_THREAD_STACKSIZE, gyro_thread, argv);
		if (err < 0) {
	...
	if (config->xl_attach != NULL) {
		...
		err = kthread_create("lsm6dso32_xl_thread", SCHED_PRIORITY_DEFAULT,
							 CONFIG_SENSORS_LSM6DSO32_THREAD_STACKSIZE, accel_thread, argv);
		if (err < 0) {
