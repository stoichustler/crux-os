...
static int ltr308_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						   bool enabled);
static int ltr308_calibrate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							unsigned long arg);
...
static int ltr308_set_reg8(FAR struct ltr308_dev_s *priv, uint8_t regaddr, uint8_t regval)
{
...
static int ltr308_get_reg8(FAR struct ltr308_dev_s *priv, uint8_t regaddr, FAR uint8_t *regval)
{
...
static int ltr308_get_reg24(FAR struct ltr308_dev_s *priv, uint8_t regaddr, FAR uint32_t *val)
{
	int ret;
	int i;
...
static int ltr308_get_status(FAR struct ltr308_dev_s *priv, FAR bool *power_on,
							 FAR bool *interrupt_pending, FAR bool *data_pending)
{
	uint8_t status;
	uint8_t ret;
...
static int ltr308_get_lux(FAR struct ltr308_dev_s *priv, uint8_t gain, uint8_t integration_time,
						  uint32_t data, FAR float *lux)
{
	float d0;
...
static int ltr308_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						   bool enabled)
{
	FAR struct ltr308_sensor_s *dev = container_of(lower, FAR struct ltr308_sensor_s, lower);
	FAR struct ltr308_dev_s *priv = container_of(dev, FAR struct ltr308_dev_s, dev);
	int ret;
							...
							unsigned long arg)
{
	FAR struct ltr308_sensor_s *dev = container_of(lower, FAR struct ltr308_sensor_s, lower);
	FAR struct ltr308_dev_s *priv = container_of(dev, FAR struct ltr308_dev_s, dev);
	FAR struct ltr308_calibval *calibval = (FAR struct ltr308_calibval *)arg;
...
static int ltr308_thread(int argc, char **argv)
{
	FAR struct ltr308_dev_s *priv =
		(FAR struct ltr308_dev_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	...
	while (true) {
		...
		ret = ltr308_get_lux(priv, priv->dev.gain, priv->dev.integration_time, data, &lux);
		if (ret < 0) {
			goto thread_sleep;
		}
		...
		light.light = lux;
		priv->dev.lower.push_event(priv->dev.lower.priv, &light, sizeof(struct sensor_light));
...
int ltr308_register(int devno, FAR struct i2c_master_s *i2c)
{
	...
	ret = kthread_create("ltr308_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_LTR308_THREAD_STACKSIZE, ltr308_thread, argv);
	if (ret < 0) {
