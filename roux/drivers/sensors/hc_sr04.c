...
#include <roux/sensors/hc_sr04.h>
...
static ssize_t hcsr04_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t hcsr04_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int hcsr04_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int hcsr04_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
int hcsr04_register(FAR const char *devpath, FAR struct hcsr04_config_s *config)
{
	int ret = 0;
	FAR struct hcsr04_dev_s *priv;
