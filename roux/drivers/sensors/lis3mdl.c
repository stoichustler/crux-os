...
#include <roux/sensors/lis3mdl.h>
...
static void lis3mdl_read_register(FAR struct lis3mdl_dev_s *dev, uint8_t const reg_addr,
								  uint8_t *reg_data);
static void lis3mdl_write_register(FAR struct lis3mdl_dev_s *dev, uint8_t const reg_addr,
...
static void lis3mdl_read_magnetic_data(FAR struct lis3mdl_dev_s *dev, uint16_t *x_mag,
									   uint16_t *y_mag, uint16_t *z_mag);
static void lis3mdl_read_temperature(FAR struct lis3mdl_dev_s *dev, uint16_t *temperature);
...
static ssize_t lis3mdl_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t lis3mdl_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static void lis3mdl_reset(FAR struct lis3mdl_dev_s *dev)
{
	lis3mdl_write_register(dev, LIS3MDL_CTRL_REG_2, LIS3MDL_CTRL_REG_2_SOFT_RST_BM);
...
static void lis3mdl_read_measurement_data(FAR struct lis3mdl_dev_s *dev)
{
	...
	add_sensor_randomness((x_mag << 16) ^ (y_mag << 10) ^ (z_mag << 2) ^ temperature);
}
...
static int lis3mdl_interrupt_handler(int irq, FAR void *context)
{
	...
	for (priv = g_lis3mdl_list; priv && priv->config->irq != irq; priv = priv->flink)
		;
...
static int lis3mdl_open(FAR struct file *filep)
{
	...
	lis3mdl_write_register(priv, LIS3MDL_CTRL_REG_2,
						   LIS3MDL_CTRL_REG_2_FS_1_BM | LIS3MDL_CTRL_REG_2_FS_0_BM);
	...
	lis3mdl_write_register(priv, LIS3MDL_CTRL_REG_1,
						   LIS3MDL_CTRL_REG_1_TEMP_EN_BM | LIS3MDL_CTRL_REG_1_OM_1_BM |
	...
	lis3mdl_write_register(priv, LIS3MDL_CTRL_REG_4,
						   LIS3MDL_CTRL_REG_4_OMZ_1_BM | LIS3MDL_CTRL_REG_4_OMZ_0_BM);
	...
	reg_content = 0;
	for (reg_addr = LIS3MDL_CTRL_REG_1; reg_addr <= LIS3MDL_CTRL_REG_5; reg_addr++) {
