...
#include <roux/sensors/apds9960.h>
...
static int apds9960_i2c_read(FAR struct apds9960_dev_s *priv, uint8_t const regaddr,
							 FAR uint8_t *regval, int len);
static int apds9960_i2c_read8(FAR struct apds9960_dev_s *priv, uint8_t const regaddr,
							  FAR uint8_t *regval);
static int apds9960_i2c_write(FAR struct apds9960_dev_s *priv, uint8_t const *data, int len);
static int apds9960_i2c_write8(FAR struct apds9960_dev_s *priv, uint8_t const regaddr,
							   uint8_t regval);

/* Character driver methods */

static ssize_t apds9960_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t apds9960_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static bool apds9960_processgesture(FAR struct apds9960_dev_s *priv)
{
	...
	if ((priv->gesture_data.total_gestures <= 32) && (priv->gesture_data.total_gestures > 0)) {
	...
	if ((priv->gesture_ud_count == 0) && (priv->gesture_lr_count == 0)) {
		if ((abs(ud_delta) < GESTURE_SENSITIVITY_2) && (abs(lr_delta) < GESTURE_SENSITIVITY_2)) {
			...
			if ((priv->gesture_near_count >= 10) && (priv->gesture_far_count >= 2)) {
				if ((ud_delta == 0) && (lr_delta == 0)) {
		...
		if ((priv->gesture_ud_count == 1) && (priv->gesture_lr_count == 0)) {
			...
			if ((priv->gesture_ud_count == 0) && (priv->gesture_lr_count == 1)) {
				...
				if ((priv->gesture_ud_count == 0) && (priv->gesture_lr_count == -1)) {
					...
					if ((priv->gesture_ud_count == -1) && (priv->gesture_lr_count == 1)) {
						if (abs(priv->gesture_ud_delta) > abs(priv->gesture_lr_delta)) {
							priv->gesture_motion = DIR_UP;
						} else {
							priv->gesture_motion = DIR_RIGHT;
						}
	...
	while (1) {
		...
		if ((gstatus & GVALID) == GVALID) {
			...
			if (fifo_level > 0) {
				...
				ret =
					apds9960_i2c_read(priv, APDS9960_GFIFO_U, (FAR uint8_t *)fifo_data, bytes_read);
				if (ret < 0) {
				...
				for (i = 0; i < fifo_level; i++) {
					sninfo("U: %03d | D: %03d | L: %03d | R: %03d\n", fifo_data[i],
						   fifo_data[i + 1], fifo_data[i + 2], fifo_data[i + 3]);
				}
...
int apds9960_register(FAR const char *devpath, FAR struct apds9960_config_s *config)
{
	FAR struct apds9960_dev_s *priv;
	int ret;
