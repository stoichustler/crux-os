...
static int sensor_update_interval(FAR struct file *filep, FAR struct sensor_upperhalf_s *upper,
								  FAR struct sensor_user_s *user, uint32_t interval)
{
	FAR struct sensor_lowerhalf_s *lower = upper->lower;
	FAR struct sensor_user_s *tmp;
	uint32_t min_interval = interval;
	uint32_t min_latency = interval != UINT32_MAX ? user->state.latency : UINT32_MAX;
	int ret = 0;
	...
	if (lower->ops->set_interval) {
		if (min_interval != UINT32_MAX && min_interval != upper->state.min_interval) {
		...
		if (lower->ops->batch && (min_latency != upper->state.min_latency ||
...
static int sensor_update_latency(FAR struct file *filep, FAR struct sensor_upperhalf_s *upper,
								 FAR struct sensor_user_s *user, uint32_t latency)
{
	FAR struct sensor_lowerhalf_s *lower = upper->lower;
	FAR struct sensor_user_s *tmp;
...
static void sensor_generate_timing(FAR struct sensor_upperhalf_s *upper, unsigned long nums)
{
	uint32_t interval = upper->state.min_interval != UINT32_MAX ? upper->state.min_interval : 1;
	while (nums-- > 0) {
		upper->state.generation += interval;
		circbuf_overwrite(&upper->timing, &upper->state.generation, TIMING_BUF_ESIZE);
	}
}

static bool sensor_is_updated(FAR struct sensor_upperhalf_s *upper, FAR struct sensor_user_s *user)
{
	...
	if (delta <= 0) {
		...
		return delta >= user->state.interval - (upper->state.min_interval >> 1);
	}
}

static void sensor_catch_up(FAR struct sensor_upperhalf_s *upper, FAR struct sensor_user_s *user)
{
	uint32_t generation;
	long delta;
	...
	if (delta > 0) {
		...
		if (user->state.interval == UINT32_MAX) {
			...
			user->state.generation += ROUND_DOWN(delta, user->state.interval);
		}
	}
}

static ssize_t sensor_do_samples(FAR struct sensor_upperhalf_s *upper,
								 FAR struct sensor_user_s *user, FAR char *buffer, size_t len)
{
	uint32_t generation;
	ssize_t ret = 0;
	size_t nums;
	...
	if (user->state.interval == UINT32_MAX) {
		if (buffer != NULL) {
			ret = circbuf_peekat(&upper->buffer, user->bufferpos * upper->state.esize, buffer, len);
		} else {
			ret = len;
		}

		user->bufferpos += nums;
		circbuf_peekat(&upper->timing, (user->bufferpos - 1) * TIMING_BUF_ESIZE,
	...
	circbuf_peekat(&upper->timing, pos * TIMING_BUF_ESIZE, &generation, TIMING_BUF_ESIZE);
	while (pos++ != end) {
		uint32_t next_generation;
		long delta;

		if (pos * TIMING_BUF_ESIZE == upper->timing.head) {
			next_generation = upper->state.generation + upper->state.min_interval;
		} else {
			circbuf_peekat(&upper->timing, pos * TIMING_BUF_ESIZE, &next_generation,
						   TIMING_BUF_ESIZE);
		}

		delta =
			next_generation + generation - ((user->state.generation + user->state.interval) << 1);
		if (delta >= 0) {
			if (buffer != NULL) {
				ret += circbuf_peekat(&upper->buffer, (pos - 1) * upper->state.esize, buffer + ret,
									  upper->state.esize);
			} else {
	...
	if (pos - 1 == end && sensor_is_updated(upper, user)) {
		generation =
			upper->state.generation - user->state.generation + (upper->state.min_interval >> 1);
		user->state.generation += ROUND_DOWN(generation, user->state.interval);
	}

	return ret;
}

static void sensor_pollnotify_one(FAR struct sensor_user_s *user, pollevent_t eventset,
								  sensor_role_t role)
{
	if (!(user->role & role)) {
		return;
...
static ssize_t sensor_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	...
	if (lower->ops->fetch) {
		...
		if (buffer == NULL) {
			...
			ret = circbuf_peekat(&upper->buffer, (user->bufferpos - 1) * upper->state.esize, buffer,
								 upper->state.esize);
		}
	} else {
		ret = -ENODATA;
...
static int sensor_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SNIOC_GET_STATE: {
		nxrmutex_lock(&upper->lock);
		memcpy((FAR void *)(uintptr_t)arg, &upper->state, sizeof(upper->state));
		user->changed = false;
		nxrmutex_unlock(&upper->lock);
	} break;
	...
	case SNIOC_GET_USTATE: {
		nxrmutex_lock(&upper->lock);
		memcpy((FAR void *)(uintptr_t)arg, &user->state, sizeof(user->state));
		nxrmutex_unlock(&upper->lock);
	} break;

	case SNIOC_SET_INTERVAL: {
		nxrmutex_lock(&upper->lock);
		ret = sensor_update_interval(filep, upper, user, arg1 ? arg1 : UINT32_MAX);
		nxrmutex_unlock(&upper->lock);
	} break;
	...
	case SNIOC_GET_INFO: {
		...
		ret = lower->ops->get_info(lower, filep, (FAR struct sensor_device_info_s *)(uintptr_t)arg);
	} break;
...
static ssize_t sensor_push_event(FAR void *priv, FAR const void *data, size_t bytes)
{
	...
	if (!circbuf_is_init(&upper->buffer)) {
		...
		ret = circbuf_init(&upper->buffer, NULL, lower->nbuffer * upper->state.esize);
		if (ret < 0) {
			nxrmutex_unlock(&upper->lock);
			return ret;
		}

		ret = circbuf_init(&upper->timing, NULL, lower->nbuffer * TIMING_BUF_ESIZE);
		if (ret < 0) {
...
void sensor_remap_vector_raw16(FAR const int16_t *in, FAR int16_t *out, int place)
{
...
int sensor_register(FAR struct sensor_lowerhalf_s *lower, int devno)
{
	...
	snprintf(path, PATH_MAX, DEVNAME_FMT, g_sensor_meta[lower->type].name, devno);
	ret = sensor_custom_register(lower, path, g_sensor_meta[lower->type].esize);
...
int sensor_custom_register(FAR struct sensor_lowerhalf_s *lower, FAR const char *path, size_t esize)
{
...
void sensor_custom_unregister(FAR struct sensor_lowerhalf_s *lower, FAR const char *path)
{
	FAR struct sensor_upperhalf_s *upper;
