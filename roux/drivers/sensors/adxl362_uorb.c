...
#include <roux/sensors/adxl362.h>
...
static uint8_t adxl362_getreg8(FAR struct adxl362_sensor_s *priv, uint8_t regaddr);
static void adxl362_putreg8(FAR struct adxl362_sensor_s *priv, uint8_t regaddr, uint8_t regval);
static void adxl362_getregs(FAR struct adxl362_sensor_s *priv, uint8_t regaddr, FAR uint8_t *regval,
							int len);
...
static int adxl362_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							bool enable);
static int adxl362_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								FAR uint32_t *period_us);
#ifndef CONFIG_SENSORS_ADXL362_POLL
static int adxl362_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static int adxl362_thread(int argc, FAR char **argv)
{
	FAR struct adxl362_sensor_s *priv =
		(FAR struct adxl362_sensor_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	struct sensor_accel accel;
	uint8_t data[8];
	int ret;
...
int adxl362_register(int devno, FAR struct spi_dev_s *spi)
{
	...
	ret = kthread_create("adxl362_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_ADXL362_THREAD_STACKSIZE, adxl362_thread, argv);
	if (ret < 0) {
		kmm_free(priv);
	}
#endif
