...
#include <roux/sensors/ak09912.h>
#include <roux/wdog.h>
#include <roux/irq.h>
...
#define MERGE_BYTE(low, high) ((low & 0xff) | ((high << 8) & ~0xff))
...
static ssize_t ak09912_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t ak09912_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int32_t ak09912_getreg(FAR struct ak09912_dev_s *priv, uint8_t regaddr, FAR uint8_t *buffer,
							  uint32_t cnt)
{
	struct i2c_msg_s msg[2];
	int ret;
...
static int ak09912_set_noise_suppr_flt(FAR struct ak09912_dev_s *priv, uint32_t nsf)
{
	int ret = 0;
	uint8_t ctrl1 = 0;
										...
										FAR struct mag_data_s *mag_data)
{
	...
	wd_start(&priv->wd, AK09912_POLLING_TIMEOUT, ak09912_wd_timeout, (wdparm_t)priv);
...
static int ak09912_read_mag_data(FAR struct ak09912_dev_s *priv, FAR struct mag_data_s *mag_data)
{
	...
	mag_data->x = (int16_t)(mag_data->x * ((int32_t)priv->asa_data.x + AK09912_SENSITIVITY) /
							AK09912_SENSITIVITY_DIV);
	mag_data->y = (int16_t)(mag_data->y * ((int32_t)priv->asa_data.y + AK09912_SENSITIVITY) /
							AK09912_SENSITIVITY_DIV);
	mag_data->z = (int16_t)(mag_data->z * ((int32_t)priv->asa_data.z + AK09912_SENSITIVITY) /
							AK09912_SENSITIVITY_DIV);
