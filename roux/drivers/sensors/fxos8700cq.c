...
#include <roux/sensors/fxos8700cq.h>
...
static uint8_t fxos8700cq_getreg8(FAR struct fxos8700cq_dev_s *priv, uint8_t regaddr);

static int fxos8700cq_putreg8(FAR struct fxos8700cq_dev_s *priv, uint8_t regaddr, uint8_t regval);

static int fxos8700cq_getregs(FAR struct fxos8700cq_dev_s *priv, uint8_t regaddr, uint8_t *regval,
							  int len);
...
static ssize_t fxos8700cq_read(FAR struct file *filep, FAR char *buffer, size_t len);
...
static int fxos8700cq_open(FAR struct file *filep)
{
	...
	regval = 0 | (0b11 << 0) | (0b11 << 3) | (0b1 << 5) | (0b0 << 6) | (0b0 << 7);
...
static ssize_t fxos8700cq_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	...
	int status =
		fxos8700cq_getregs(priv, FXOS8700CQ_STATUS, (FAR uint8_t *)data, FXOS8700CQ_READ_LEN);
	if (status < 0) {
...
int fxos8700cq_register(FAR const char *devpath, FAR struct i2c_master_s *dev)
{
	FAR struct fxos8700cq_dev_s *priv;
	int ret;

	priv = (FAR struct fxos8700cq_dev_s *)kmm_malloc(sizeof(struct fxos8700cq_dev_s));
