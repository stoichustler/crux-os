...
static int bh1749nuc_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							  bool enable);
#ifndef CONFIG_SENSORS_BH1749NUC_POLL
static int bh1749nuc_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						   FAR char *buffer, size_t buflen);
#endif
static int bh1749nuc_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								  FAR uint32_t *period_us);
static int bh1749nuc_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
							 unsigned long arg);
...
static const struct sensor_ops_s g_bh1749nuc_sensor_ops = {NULL, /* open */
														   NULL, /* close */
														   bh1749nuc_activate,
							  ...
							  bool enable)
{
	...
	if (enable) {
		...
		val = (BH1749NUC_MODE_CONTROL1_MEAS_TIME160MS | BH1749NUC_MODE_CONTROL1_IR_GAIN_X1 |
...
{
	FAR struct bh1749nuc_sensor_dev_s *dev =
		(FAR struct bh1749nuc_sensor_dev_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	...
	while (true) {
		...
		if (rgb->enabled) {
			...
			rgb->lower.push_event(rgb->lower.priv, &rgb_data, sizeof(rgb_data));
		}
		...
		if (ir->enabled) {
			...
			ir->lower.push_event(ir->lower.priv, &ir_data, sizeof(ir_data));
		}
...
int bh1749nuc_register_uorb(int devno, FAR struct bh1749nuc_config_s *config)
{
	...
	bh1749nuc_putreg8(&dev->dev, BH1749NUC_SYSTEM_CONTROL, BH1749NUC_SYSTEM_CONTROL_SW_RESET);
	...
	ret = kthread_create("bh1749nuc_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_BH1749NUC_THREAD_STACKSIZE, bh1749nuc_thread, argv);
	if (ret < 0) {
		goto thr_err;
	}
#endif
