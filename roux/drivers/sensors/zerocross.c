...
#include <roux/sensors/zerocross.h>
...
static void zerocross_interrupt(FAR const struct zc_lowerhalf_s *lower, FAR void *arg);
...
static void zerocross_interrupt(FAR const struct zc_lowerhalf_s *lower, FAR void *arg)
{
	...
	for (opriv = priv->zu_open; opriv; opriv = opriv->do_flink) {
		...
		nxsig_notification(opriv->do_pid, &opriv->do_event, SI_QUEUE, &opriv->do_work);
	}
...
static int zc_close(FAR struct file *filep)
{
	...
	for (prev = NULL, curr = priv->zu_open; curr && curr != opriv;
...
static int zc_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case ZCIOC_REGISTER: {
		FAR struct sigevent *event = (FAR struct sigevent *)((uintptr_t)arg);
...
int zc_register(FAR const char *devname, FAR struct zc_lowerhalf_s *lower)
{
	...
	priv = (FAR struct zc_upperhalf_s *)kmm_zalloc(sizeof(struct zc_upperhalf_s));
