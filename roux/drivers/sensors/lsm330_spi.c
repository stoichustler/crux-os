...
#if defined(CONFIG_SPI) && defined(CONFIG_SENSORS_LSM330SPI) && defined(CONFIG_SPI_EXCHANGE)
...
static uint8_t lsm330_read_register(FAR struct lsm330_dev_s *dev, uint8_t reg_addr);
static void lsm330_read_acl_registerblk(FAR struct lsm330_dev_s *dev, uint8_t reg_addr,
										FAR uint8_t *reg_data, uint8_t xfercnt);
static void lsm330_read_gyro_registerblk(FAR struct lsm330_dev_s *dev, uint8_t reg_addr,
										 FAR uint8_t *reg_data, uint8_t xfercnt);
static void lsm330_write_register(FAR struct lsm330_dev_s *dev, uint8_t reg_addr, uint8_t reg_data);
static void lsm330_write_acl_registerblk(FAR struct lsm330_dev_s *dev, uint8_t reg_addr,
										 FAR uint8_t *reg_data, uint8_t xfercnt);
static void lsm330_write_gyro_registerblk(FAR struct lsm330_dev_s *dev, uint8_t reg_addr,
										  FAR uint8_t *reg_data, uint8_t xfercnt);
...
static ssize_t lsm330acl_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static ssize_t lsm330gyro_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static ssize_t lsm330acl_dvr_read(FAR void *instance_handle, FAR char *buffer, size_t buflen);
static ssize_t lsm330gyro_dvr_read(FAR void *instance_handle, FAR char *buffer, size_t buflen);
static ssize_t lsm330acl_dvr_write(FAR void *instance_handle, FAR const char *buffer,
								   size_t buflen);
static ssize_t lsm330gyro_dvr_write(FAR void *instance_handle, FAR const char *buffer,
									size_t buflen);
static off_t lsm330acl_dvr_seek(FAR void *instance_handle, off_t offset, int whence);
static off_t lsm330gyro_dvr_seek(FAR void *instance_handle, off_t offset, int whence);
static int lsm330_dvr_ioctl(FAR void *instance_handle, int cmd, unsigned long arg);
static void lsm330_dvr_exchange(FAR void *instance_handle, FAR const void *txbuffer,
...
static struct lsm330_reg_pair_s g_default_lsm330_aclcr_values[] = {
	...
	{.addr = LSM330_ACL_CTRL_REG5, .value = 0x00},

	/* CR6 BW[2:1]   FSCALE[2:0] - -  SIM
	 *     00=800Hz   10 0=16g   0 0  0=4-wire
	 */

	{.addr = LSM330_ACL_CTRL_REG6, .value = 0x20},

	/* CR7 BOOT FIFO_EN WTM_EN ADD_INC P1_MTY P1_WTM P1_OVR WTM_EN
	 *      0      0      0      1       0      0      0       0
	 */

	{.addr = LSM330_ACL_CTRL_REG7, .value = 0x10},

	/* CR2 HYST1 -  SM1_PIN - - SM1_EN
	 *     000   0     0    0 0    0
	 */

	{.addr = LSM330_ACL_CTRL_REG2, .value = 0x00},

	/* CR3 HYST2 -  SM2_PIN - - SM2_EN
	 *     000   0     0    0 0    0
	 */

	{.addr = LSM330_ACL_CTRL_REG3, .value = 0x00},

	/* CR4 DR_EN IEA IEL INT2_EN INT1_EN  VFILT STRT
	 *       1    1   0     0       0       0    0
	 */

	{.addr = LSM330_ACL_CTRL_REG4, .value = 0xc0},

	/* CR5 ODR[3:0]        BDU ZEN YEN XEN
	 *     1001=1600Hz      1   1   1   1=all enabled
	 */

	{.addr = LSM330_ACL_CTRL_REG5, .value = 0x9f}};
...
static int lsm330acl_dvr_open(FAR void *instance_handle, int32_t arg)
{
	...
	} else /* ID matches */
	{
		...
		if (priv->config->initial_cr_values_size == 0 || priv->config->initial_cr_values == NULL) {
...
static ssize_t lsm330acl_dvr_read(FAR void *instance_handle, FAR char *buffer, size_t buflen)
{
	...
	lsm330_read_acl_registerblk(priv, priv->seek_address, (FAR uint8_t *)buffer, buflen);
	return buflen;
}
...
static ssize_t lsm330gyro_dvr_read(FAR void *instance_handle, FAR char *buffer, size_t buflen)
{
	...
	lsm330_read_gyro_registerblk(priv, priv->seek_address, (FAR uint8_t *)buffer, buflen);
	return buflen;
}
...
static ssize_t lsm330acl_dvr_write(FAR void *instance_handle, FAR const char *buffer, size_t buflen)
{
	FAR struct lsm330_dev_s *priv = (FAR struct lsm330_dev_s *)instance_handle;
	...
	lsm330_write_acl_registerblk(priv, priv->seek_address, (FAR uint8_t *)buffer, buflen);
	return buflen;
}
									...
									size_t buflen)
{
	...
	lsm330_write_gyro_registerblk(priv, priv->seek_address, (FAR uint8_t *)buffer, buflen);
	return buflen;
}
...
static ssize_t lsm330acl_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct lsm330_dev_s *priv = inode->i_private;
...
static ssize_t lsm330gyro_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct lsm330_dev_s *priv = inode->i_private;
...
int lsm330_register(FAR const char *devpath_acl, FAR const char *devpath_gyro,
					FAR struct spi_dev_s *spi, FAR struct lsm330_config_s *config_acl,
