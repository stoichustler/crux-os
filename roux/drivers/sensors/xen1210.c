...
#include <roux/sensors/xen1210.h>
...
static ssize_t xen1210_read(FAR struct file *filep, FAR char *buffer, size_t len);
...
static void xen1210_interrupt(FAR struct xen1210_config_s *config, FAR void *arg)
{
	FAR struct xen1210_dev_s *priv = (FAR struct xen1210_dev_s *)arg;
	int ret;
...
XEN1210_HANDLE xen1210_instantiate(FAR struct spi_dev_s *dev, FAR struct xen1210_config_s *config)
{
	FAR struct xen1210_dev_s *priv;
	uint32_t regval;
	...
	config->attach(config, (xen1210_handler_t)xen1210_interrupt, (FAR void *)priv);
...
void xen1210_getdata(FAR struct xen1210_dev_s *priv)
{
	...
	add_sensor_randomness((priv->sample.data_x << 8) ^ (priv->sample.data_y << 4) ^
