...
#ifdef CONFIG_SCD41_I2C
static int scd41_do_transfer(FAR struct i2c_master_s *i2c, FAR struct i2c_msg_s *msgv, size_t nmsg);
#endif
static int scd41_write_cmd(FAR struct scd41_dev_s *priv, uint16_t cmd,
						   FAR struct scd41_word_s *params, unsigned int num_params);
static int scd41_read_words(FAR struct scd41_dev_s *priv, FAR struct scd41_word_s *words,
							unsigned int num_words);

/* Data conversion */

static uint8_t scd41_crc_word(uint16_t word);
static void scd41_set_command_param(FAR struct scd41_word_s *param, uint16_t value);
static int scd41_check_data_crc(FAR const struct scd41_word_s *words, unsigned int num_words);
static uint16_t scd41_data_word2uint16(FAR const struct scd41_word_s *word);

/* Driver features */

static int scd41_read_values(FAR struct scd41_dev_s *priv, FAR float *temp, FAR float *rh,
							 FAR float *co2, bool wait);
...
#endif
static ssize_t scd41_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t scd41_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
							 ...
							 FAR float *co2, bool wait)
{
	...
	if (wait || !priv->valid ||
		...
		add_sensor_randomness((data[0].crc << 16) ^ (data[1].crc << 8) ^ data[2].crc);
...
static ssize_t scd41_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	ret = scd41_read_values(priv, &temp, &rh, &co2, !(filep->f_oflags & O_NONBLOCK));
	if (ret < 0) {
		...
		length = snprintf(buffer, buflen, "%s%u.%02u %s%u.%02u %s%u.%02u\n", co2 < 0 ? "-" : "",
						  co2_100 / 100, co2_100 % 100, temp < 0 ? "-" : "", temp100 / 100,
						  temp100 % 100, rh < 0 ? "-" : "", rh100 / 100, rh100 % 100);
		if (length > buflen) {
			length = buflen;
		}
	}
...
static int scd41_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SNIOC_SET_PRESSURE_COMP: {
		...
		scd41_set_command_param(&param, priv->pressure_comp);
		ret = scd41_write_cmd(priv, SCD41_CMD_SET_PRESSURE_COMP, &param, 1);
	} break;
	...
	case SNIOC_SET_ALTITUDE_COMP: {
		...
		scd41_set_command_param(&param, priv->altitude_comp);
		ret = scd41_write_cmd(priv, SCD41_CMD_SET_ALT_COMPENSATION, &param, 1);
	} break;
	...
	case SNIOC_READ_CONVERT_DATA: {
		...
		if (ret < 0) {
			...
			FAR struct scd41_conv_data_s *data = (FAR struct scd41_conv_data_s *)arg;
