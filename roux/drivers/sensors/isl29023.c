...
#include <roux/sensors/isl29023.h>
...
static int isl29023_i2c_write(FAR struct isl29023_dev_s *dev, FAR const uint8_t *buffer,
							  ssize_t buflen);
static int isl29023_i2c_read(FAR struct isl29023_dev_s *dev, FAR uint8_t *buffer, ssize_t buflen);
static int isl29023_read_reg(FAR struct isl29023_dev_s *dev, const uint8_t regaddr, uint8_t *buffer,
							 size_t buflen);
static int isl29023_read_lux(FAR struct isl29023_dev_s *dev, FAR struct isl29023_data_s *data);
static int isl29023_set_op_mode(FAR struct isl29023_dev_s *dev, uint8_t mode);
static int isl29023_set_resolution(FAR struct isl29023_dev_s *dev, uint8_t res_mode);
static int isl29023_set_range(FAR struct isl29023_dev_s *dev, uint8_t range_mode);

/* Driver methods */

static ssize_t isl29023_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t isl29023_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
							  ...
							  ssize_t buflen)
{
	...
	msg.frequency = CONFIG_ISL29023_I2C_FREQUENCY, msg.addr = dev->addr;
	msg.flags = 0;
...
static int isl29023_i2c_read(FAR struct isl29023_dev_s *dev, FAR uint8_t *buffer, ssize_t buflen)
{
	...
	msg.frequency = CONFIG_ISL29023_I2C_FREQUENCY, msg.addr = dev->addr, msg.flags = I2C_M_READ;
	msg.buffer = buffer;
	msg.length = buflen;
...
int isl29023_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
