...
static void bmi160_accel_enable(FAR struct bmi160_dev_uorb_s *priv, bool enable);
static void bmi160_gyro_enable(FAR struct bmi160_dev_uorb_s *priv, bool enable);

/* Sensor ops functions */

static int bmi160_set_accel_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
									 FAR uint32_t *period_us);
static int bmi160_set_gyro_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
									FAR uint32_t *period_us);
static int bmi160_accel_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								 bool enable);
static int bmi160_gyro_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								bool enable);
...
static int bmi160_findodr(uint32_t time, FAR const struct bmi160_odr_s *odr_s, int len);
...
static void bmi160_accel_enable(FAR struct bmi160_dev_uorb_s *priv, bool enable)
{
	...
	if (enable) {
		...
		idx = bmi160_findodr(priv->interval, g_bmi160_accel_odr, nitems(g_bmi160_accel_odr));
		bmi160_putreg8(&priv->dev, BMI160_ACCEL_CONFIG,
					   ACCEL_NORMAL_AVG4 | g_bmi160_accel_odr[idx].regval);

		work_queue(HPWORK, &priv->work, bmi160_accel_worker, priv, priv->interval / USEC_PER_TICK);
	} else {
...
static void bmi160_gyro_enable(FAR struct bmi160_dev_uorb_s *priv, bool enable)
{
	...
	if (enable) {
		...
		idx = bmi160_findodr(priv->interval, g_bmi160_gyro_odr, nitems(g_bmi160_gyro_odr));
		bmi160_putreg8(&priv->dev, BMI160_GYRO_CONFIG,
					   GYRO_NORMAL_MODE | g_bmi160_gyro_odr[idx].regval);

		work_queue(HPWORK, &priv->work, bmi160_gyro_worker, priv, priv->interval / USEC_PER_TICK);
	} else {
		work_cancel(HPWORK, &priv->work);
									 ...
									 FAR uint32_t *period_us)
{
	...
	num = bmi160_findodr(*period_us, g_bmi160_accel_odr, nitems(g_bmi160_accel_odr));
	bmi160_putreg8(&priv->dev, BMI160_ACCEL_CONFIG,
									...
									FAR uint32_t *period_us)
{
	...
	num = bmi160_findodr(*period_us, g_bmi160_gyro_odr, nitems(g_bmi160_gyro_odr));
	bmi160_putreg8(&priv->dev, BMI160_GYRO_CONFIG,
...
static int bmi160_register_accel(int devno, FAR struct i2c_master_s *dev)
#else /* CONFIG_BMI160_SPI */
static int bmi160_register_accel(int devno, FAR struct spi_dev_s *dev)
#endif
{
...
#ifdef CONFIG_SENSORS_BMI160_I2C
static int bmi160_register_gyro(int devno, FAR struct i2c_master_s *dev)
#else /* CONFIG_BMI160_SPI */
static int bmi160_register_gyro(int devno, FAR struct spi_dev_s *dev)
#endif
{
	FAR struct bmi160_dev_uorb_s *priv;
	int ret;
