...
static void bmi088_accel_enable(FAR struct bmi088_acc_dev_uorb_s *priv, bool enable);
static void bmi088_gyro_enable(FAR struct bmi088_gyro_dev_uorb_s *priv, bool enable);

/* Sensor ops functions */

static int bmi088_set_accel_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
									 FAR uint32_t *period_us);
static int bmi088_set_gyro_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
									FAR uint32_t *period_us);
static int bmi088_accel_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								 bool enable);
static int bmi088_gyro_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								bool enable);
...
static int bmi088_find_parameter(uint32_t parameter, FAR const struct bmi088_para_s *parameter_s,
								 int len);
...
static void bmi088_accel_enable(FAR struct bmi088_acc_dev_uorb_s *priv, bool enable)
{
	...
	if (enable) {
		...
		up_mdelay(1);
		bmi088_put_acc_reg8(&priv->dev, BMI088_ACC_PWR_CTRL, BMI088_ACC_PWR_CTRL_ACC_ENABLE);
		up_mdelay(5);

		idx_odr =
			bmi088_find_parameter(priv->interval, g_bmi088_accel_odr, nitems(g_bmi088_accel_odr));

		idx_bwp = bmi088_find_parameter(priv->bwp, g_bmi088_acc_bwp, nitems(g_bmi088_acc_bwp));

		idx_range =
			bmi088_find_parameter(priv->range, g_bmi088_accel_range, nitems(g_bmi088_accel_range));

		bmi088_put_acc_reg8(&priv->dev, BMI088_ACC_CONF,
							g_bmi088_acc_bwp[idx_bwp].regval | g_bmi088_accel_odr[idx_odr].regval);

		bmi088_put_acc_reg8(&priv->dev, BMI088_ACC_RANGE, g_bmi088_accel_range[idx_range].regval);

		up_mdelay(5);
		bmi088_put_acc_reg8(&priv->dev, BMI088_ACC_PWR_CONF, BMI088_ACC_PWR_CONF_ACTIVE_MODE);

		work_queue(HPWORK, &priv->work, bmi088_accel_worker, priv, priv->interval / USEC_PER_TICK);
	} else {
		/* Set suspend mode to sensors. */

		work_cancel(HPWORK, &priv->work);
		bmi088_put_acc_reg8(&priv->dev, BMI088_ACC_PWR_CONF, BMI088_ACC_PWR_CONF_SUSPEND_MODE);
		up_mdelay(5);
		bmi088_put_acc_reg8(&priv->dev, BMI088_ACC_PWR_CTRL, BMI088_ACC_PWR_CTRL_ACC_DISABLE);
		up_mdelay(5);
	}
}
...
static void bmi088_gyro_enable(FAR struct bmi088_gyro_dev_uorb_s *priv, bool enable)
{
	...
	if (enable) {
		...
		bmi088_put_gyro_reg8(&priv->dev, BMI088_GYRO_LPM1, BMI088_GYRO_PM_NORMAL);

		nxsig_usleep(30000);

		idx_odr =
			bmi088_find_parameter(priv->interval, g_bmi088_gyro_odr, nitems(g_bmi088_gyro_odr));

		idx_range =
			bmi088_find_parameter(priv->range, g_bmi088_gyro_range, nitems(g_bmi088_gyro_range));

		bmi088_put_gyro_reg8(&priv->dev, BMI088_GYRO_RANGE, g_bmi088_gyro_odr[idx_range].regval);

		bmi088_put_gyro_reg8(&priv->dev, BMI088_GYRO_BANDWIDTH,
							 g_bmi088_gyro_range[idx_odr].regval);

		bmi088_put_gyro_reg8(&priv->dev, BMI088_GYRO_LPM1, BMI088_GYRO_LPM1_NORMAL_MODE);

		work_queue(HPWORK, &priv->work, bmi088_gyro_worker, priv, priv->interval / USEC_PER_TICK);
	} else {
		work_cancel(HPWORK, &priv->work);

		/* Set suspend mode to sensors. */

		bmi088_put_gyro_reg8(&priv->dev, BMI088_GYRO_LPM1, BMI088_GYRO_LPM1_SUSPEND_MODE);
	}
}
...
static int bmi088_set_accel_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
									 FAR uint32_t *period_us)
{
	FAR struct bmi088_acc_dev_uorb_s *priv = (FAR struct bmi088_acc_dev_uorb_s *)lower;
	int num;
	...
	num = bmi088_find_parameter(*period_us, g_bmi088_accel_odr, nitems(g_bmi088_accel_odr));

	bmi088_put_gyro_reg8(&priv->dev, BMI088_ACC_CONF,
						 BMI088_ACC_CONF_BWP_NORMAL | g_bmi088_accel_odr[num].regval);
...
static int bmi088_set_gyro_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
									FAR uint32_t *period_us)
{
	FAR struct bmi088_gyro_dev_uorb_s *priv = (FAR struct bmi088_gyro_dev_uorb_s *)lower;
	int num;
	...
	num = bmi088_find_parameter(*period_us, g_bmi088_gyro_odr, nitems(g_bmi088_gyro_odr));
	bmi088_put_gyro_reg8(&priv->dev, BMI088_GYRO_BANDWIDTH, g_bmi088_gyro_odr[num].regval);
...
static void bmi088_accel_worker(FAR void *arg)
{
	...
	bmi088_get_acc_regs(&priv->dev, BMI088_ACC_X_LSB, (FAR uint8_t *)&p, 6);

	bmi088_get_acc_regs(&priv->dev, BMI088_SENSORTIME_0, (FAR uint8_t *)&time, 3);

	bmi088_get_acc_regs(&priv->dev, BMI088_TEMP_MSB, temp, 2);
...
static void bmi088_gyro_worker(FAR void *arg)
{
	...
	bmi088_get_gyro_regs(&priv->dev, BMI088_GYRO_X_LSB, (FAR uint8_t *)&p, 6);
...
static int bmi088_register_accel(int devno, FAR struct i2c_master_s *dev)
#else /* CONFIG_BMI088_SPI */
static int bmi088_register_accel(int devno, FAR struct spi_dev_s *dev)
#endif
{
...
#ifdef CONFIG_SENSORS_BMI088_I2C
static int bmi088_register_gyro(int devno, FAR struct i2c_master_s *dev)
#else /* CONFIG_BMI088_SPI */
static int bmi088_register_gyro(int devno, FAR struct spi_dev_s *dev)
#endif
{
