...
#include <roux/sensors/ms58xx.h>
#include <roux/sensors/msxxxx_crc4.h>
...
static int ms58xx_i2c_write(FAR struct ms58xx_dev_s *priv, FAR const uint8_t *buffer, int buflen);
static int ms58xx_i2c_read(FAR struct ms58xx_dev_s *priv, FAR uint8_t *buffer, int buflen);
static int ms58xx_readu16(FAR struct ms58xx_dev_s *priv, uint8_t regaddr, FAR uint16_t *regval);
...
static int ms58xx_convert(FAR struct ms58xx_dev_s *priv, uint8_t regaddr, FAR uint32_t *regval);
static int ms58xx_measure(FAR struct ms58xx_dev_s *priv);

/* Character Driver Methods */

static ssize_t ms58xx_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t ms58xx_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int ms58xx_i2c_write(FAR struct ms58xx_dev_s *priv, FAR const uint8_t *buffer, int buflen)
{
	...
	msg.frequency = CONFIG_MS58XX_I2C_FREQUENCY, msg.addr = priv->addr;
	msg.flags = 0;
...
static int ms58xx_i2c_read(FAR struct ms58xx_dev_s *priv, FAR uint8_t *buffer, int buflen)
{
	...
	msg.frequency = CONFIG_MS58XX_I2C_FREQUENCY, msg.addr = priv->addr, msg.flags = I2C_M_READ;
	msg.buffer = buffer;
	msg.length = buflen;
...
static int ms58xx_measure(FAR struct ms58xx_dev_s *priv)
{
	...
	temp = (int32_t)((int64_t)2000 + (int64_t)diff * (int64_t)priv->c6 / ((int64_t)1 << 23));
	...
	case MS58XX_MODEL_MS5803_07:
		if (press > 110000) {
			press += (int32_t)((((int64_t)priv->c7 - ((int64_t)1 << 5)) * (int64_t)100 *
									((int64_t)1 << 2) -
								((int64_t)priv->c8 - ((int64_t)1 << 5)) *
									((int64_t)temp - (int64_t)2000) / ((int64_t)1 << 4)) *
							   ((int64_t)press - (int64_t)110000) / (int64_t)49000000);
		}
		break;

	case MS58XX_MODEL_MS5806_02:
#if CONFIG_MS58XX_VDD >= 22 && CONFIG_MS58XX_VDD <= 30
		press += (int32_t)(((int64_t)30 - (int64_t)CONFIG_MS58XX_VDD) * (int64_t)priv->c7 /
...
int ms58xx_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
					uint16_t osr, enum ms58xx_model_e model)
{
	...
	DEBUGASSERT(((model == MS58XX_MODEL_MS5803_02 || model == MS58XX_MODEL_MS5803_05 ||
				  model == MS58XX_MODEL_MS5803_07 || model == MS58XX_MODEL_MS5803_14 ||
				  model == MS58XX_MODEL_MS5803_30 || model == MS58XX_MODEL_MS5806_02) &&
				 (addr == MS58XX_ADDR0 || addr == MS58XX_ADDR1)) ||
				((model == MS58XX_MODEL_MS5805_02 || model == MS58XX_MODEL_MS5837_30) &&
				 addr == MS58XX_ADDR0));
