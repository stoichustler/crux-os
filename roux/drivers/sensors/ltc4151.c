...
#include <roux/sensors/ltc4151.h>
...
static int ltc4151_readpower(FAR struct ltc4151_dev_s *priv, FAR struct ltc4151_s *buffer);

/* Character driver methods */

static ssize_t ltc4151_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t ltc4151_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int ltc4151_read_reg(FAR struct ltc4151_dev_s *priv, uint8_t start_register_address,
...
static int ltc4151_read16(FAR struct ltc4151_dev_s *priv, uint8_t regaddr, FAR uint16_t *regvalue)
{
	int ret = ltc4151_read_reg(priv, regaddr, (FAR uint8_t *)regvalue, sizeof(*regvalue));

	/* Bytes are 8bit_msb.4bit_0.4bit_lsb = 16bit */

	*regvalue =
		((*regvalue & LTC4151_VALUE_MSB_MASK) << 4) | ((*regvalue & LTC4151_VALUE_LSB_MASK) >> 8);
	return ret;
}
...
static int ltc4151_readpower(FAR struct ltc4151_dev_s *priv, FAR struct ltc4151_s *buffer)
{
	...
	float_current = (((float)current_reg) * 20.0 / (1000.0 * priv->shunt_resistor_value));
...
int ltc4151_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
