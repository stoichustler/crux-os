...
#include <roux/sensors/fakesensor.h>
#include <roux/sensors/sensor.h>
#include <roux/sensors/gnss.h>
...
static int fakesensor_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							   bool enable);
static int fakesensor_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								   FAR uint32_t *period_us);
static int fakesensor_batch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							FAR uint32_t *latency_us);
static int fakegnss_activate(FAR struct gnss_lowerhalf_s *lower, FAR struct file *filep, bool sw);
static int fakegnss_set_interval(FAR struct gnss_lowerhalf_s *lower, FAR struct file *filep,
								 FAR uint32_t *period_us);
static void fakesensor_push_event(FAR struct fakesensor_s *sensor, uint64_t event_timestamp);
...
static int fakesensor_read_csv_line(FAR struct file *file, FAR char *buffer, int len, int start)
{
	int i;
...
static int fakesensor_read_csv_header(FAR struct fakesensor_s *sensor)
{
	...
	sensor->raw_start = fakesensor_read_csv_line(&sensor->data, buffer, sizeof(buffer), 0);
...
static inline void fakesensor_read_accel(FAR struct fakesensor_s *sensor, uint64_t event_timestamp)
{
	struct sensor_accel accel;
	char raw[50];
	fakesensor_read_csv_line(&sensor->data, raw, sizeof(raw), sensor->raw_start);
	sscanf(raw, "%f,%f,%f\n", &accel.x, &accel.y, &accel.z);
	accel.temperature = NAN;
	accel.timestamp = event_timestamp;
	sensor->lower.push_event(sensor->lower.priv, &accel, sizeof(struct sensor_accel));
}

static inline void fakesensor_read_mag(FAR struct fakesensor_s *sensor, uint64_t event_timestamp)
{
	...
	sensor->lower.push_event(sensor->lower.priv, &mag, sizeof(struct sensor_mag));
}

static inline void fakesensor_read_gyro(FAR struct fakesensor_s *sensor, uint64_t event_timestamp)
{
	...
	sensor->lower.push_event(sensor->lower.priv, &gyro, sizeof(struct sensor_gyro));
}

static inline void fakesensor_read_baro(FAR struct fakesensor_s *sensor, uint64_t event_timestamp)
{
	...
	sensor->lower.push_event(sensor->lower.priv, &baro, sizeof(struct sensor_baro));
}
...
static inline void fakesensor_read_gnss(FAR struct fakesensor_s *sensor)
{
	...
	while (1) {
		fakesensor_read_csv_line(&sensor->data, raw, sizeof(raw), sensor->raw_start);
		sensor->gnss.push_data(sensor->gnss.priv, raw, strlen(raw), true);
...
static int fakesensor_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							   bool enable)
{
	FAR struct fakesensor_s *sensor = container_of(lower, struct fakesensor_s, lower);
	if (enable) {
		sensor->running = true;
...
static int fakegnss_activate(FAR struct gnss_lowerhalf_s *lower, FAR struct file *filep,
							 bool enable)
{
...
void fakesensor_push_event(FAR struct fakesensor_s *sensor, uint64_t event_timestamp)
{
	switch (sensor->type) {
...
static int fakesensor_thread(int argc, char **argv)
{
	FAR struct fakesensor_s *sensor =
		(FAR struct fakesensor_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	int ret;
	...
	while (true) {
		...
		ret = file_open(&sensor->data, sensor->file_path, O_RDONLY | O_CLOEXEC);
		if (ret < 0) {
		...
		while (sensor->running) {
			...
			if (sensor->batch) {
				...
				uint64_t event_timestamp = sensor_get_timestamp() - sensor->interval * batch_num;
				int i;
 ...
 *
 * Input Parameters:
 *   type        - The type of sensor and defined in <roux/sensors/sensor.h>
...
int fakesensor_init(int type, FAR const char *file_name, int devno, uint32_t batch_number)
{
	FAR struct fakesensor_s *sensor;
	FAR char *argv[2];
	char arg1[32];
	...
	argv[0] = arg1;
	argv[1] = NULL;
	ret = kthread_create("fakesensor_thread", SCHED_PRIORITY_DEFAULT, CONFIG_DEFAULT_TASK_STACKSIZE,
