...
static int msa301_readreg8(FAR struct msa301_dev_s *priv, uint8_t regaddr, FAR uint8_t *regval);
static int msa301_writereg8(FAR struct msa301_dev_s *priv, uint8_t regaddr, uint8_t regval);
static int msa301_readreg(FAR struct msa301_dev_s *priv, uint8_t regaddr, FAR uint8_t *regval,
						  uint8_t len);
...
static ssize_t msa301_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t msa301_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int msa301_ioctl(FAR struct file *filep, int cmd, unsigned long arg);

/* Common Register Function */

static int msa301_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
						   FAR const struct msa301_ops_s *ops);
...
int msa301_sensor_register(FAR const char *devpath, FAR struct i2c_master_s *i2c)
{
