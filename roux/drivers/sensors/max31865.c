...
#include <roux/sensors/max31865.h>
...
static ssize_t max31865_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int max31865_open(FAR struct file *filep)
{
	...
	regval = MAX31865_50HZ_FILTER | MAX31865_3WIRE | MAX31865_AUTO_CONVERSION | MAX31865_BIAS_ON;
	SPI_SEND(priv->spi, regval);
...
static ssize_t max31865_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (*regval !=
		(MAX31865_50HZ_FILTER | MAX31865_3WIRE | MAX31865_AUTO_CONVERSION | MAX31865_BIAS_ON)) {
	...
	temp = (-RTD_A + sqrt(RTD_A * RTD_A - 4 * RTD_B * (1 - rt / 100.0))) / (2 * RTD_B);
...
int max31865_register(FAR const char *devpath, FAR struct spi_dev_s *spi, uint16_t devid)
{
