...
static bool dht_check_data(FAR struct dhtxx_sensor_data_s *data, float min_hum, float max_hum,
						   float min_temp, float max_temp);
static int dht_parse_data(FAR struct dhtxx_dev_s *priv, FAR struct dhtxx_sensor_data_s *data);

/* Character driver methods */

static int dhtxx_open(FAR struct file *filep);
static ssize_t dhtxx_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t dhtxx_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static bool dht_verify_checksum(FAR struct dhtxx_dev_s *priv)
{
	...
	sum = (priv->raw_data[0] + priv->raw_data[1] + priv->raw_data[2] + priv->raw_data[3]) & 0xffu;
...
static int dht_parse_data(FAR struct dhtxx_dev_s *priv, FAR struct dhtxx_sensor_data_s *data)
{
	...
	case DHTXX_DHT11:
		...
		if (!dht_check_data(data, DHT11_MIN_HUM, DHT11_MAX_HUM, DHT11_MIN_TEMP, DHT11_MAX_TEMP)) {
			ret = -1;
		}
	...
	case DHTXX_DHT12:
		...
		if (!dht_check_data(data, DHT12_MIN_HUM, DHT12_MAX_HUM, DHT12_MIN_TEMP, DHT12_MAX_TEMP)) {
			ret = -1;
		}
	...
	case DHTXX_DHT44:
		...
		data->temp = (((priv->raw_data[2] & 0x7fu) << 8u) | priv->raw_data[3]) * 0.1F;
		if (priv->raw_data[2] & 0x80u) {
			data->temp *= -1;
		}

		if (!dht_check_data(data, DHT22_MIN_HUM, DHT22_MAX_HUM, DHT22_MIN_TEMP, DHT22_MAX_TEMP)) {
			ret = -1;
		}
...
static ssize_t dhtxx_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	FAR struct inode *inode = filep->f_inode;
	FAR struct dhtxx_dev_s *priv = inode->i_private;
	FAR struct dhtxx_sensor_data_s *data = (FAR struct dhtxx_sensor_data_s *)buffer;
