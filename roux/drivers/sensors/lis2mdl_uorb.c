...
#include <roux/sensors/lis2mdl.h>
...
static int lis2mdl_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							bool enable);
static int lis2mdl_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								FAR uint32_t *period_us);
static int lis2mdl_selftest(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							unsigned long arg);
static int lis2mdl_get_info(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							FAR struct sensor_device_info_s *info);
static int lis2mdl_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
						   unsigned long arg);
static int lis2mdl_set_calibvalue(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								  unsigned long arg);
#ifdef CONFIG_SENSORS_LIS2MDL_FETCH
static int lis2mdl_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static int lis2mdl_read_reg(FAR struct lis2mdl_dev_s *dev, uint8_t addr, void *buf, uint8_t nbytes)
{
...
static int lis2mdl_write_reg(FAR struct lis2mdl_dev_s *dev, uint8_t addr, void *buf, uint8_t nbytes)
{
...
static int lis2mdl_read_data(FAR struct lis2mdl_dev_s *dev, FAR struct sensor_mag *data)
{
...
static int lis2mdl_set_bit(FAR struct lis2mdl_dev_s *dev, uint8_t addr, uint8_t bit, bool val)
{
	int err;
	uint8_t regval;
...
static int lis2mdl_set_odr(FAR struct lis2mdl_dev_s *dev, enum lis2mdl_odr_e odr)
{
	int err;
	uint8_t regval;
...
static int lis2mdl_set_mode(FAR struct lis2mdl_dev_s *dev, enum lis2mdl_mode_e mode)
{
	int err;
	uint8_t regval;
...
static int lis2mdl_enable_interrupts(FAR struct lis2mdl_dev_s *dev, bool enable)
{
	int err;
...
static int lis2mdl_temp_compensation(FAR struct lis2mdl_dev_s *dev, bool enable)
{
...
static int lis2mdl_get_info(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							FAR struct sensor_device_info_s *info)
{
	FAR struct lis2mdl_dev_s *dev = container_of(lower, FAR struct lis2mdl_dev_s, lower);
							...
							unsigned long arg)
{
	...
	if (data.x < 0.015f * GAUSS_TO_UTESLA || data.x > 0.5f * GAUSS_TO_UTESLA ||
		...
		snerr("LIS2MDL failed self-test (%.2fX, %.2fY, %.2fZ)", data.x, data.y, data.z);
		err = -EAGAIN;
	}
								  ...
								  unsigned long arg)
{
	...
	err = lis2mdl_write_reg(dev, REG_OFFSET_X_REG_L, hardirons, sizeof(hardirons));
...
int lis2mdl_register(FAR struct i2c_master_s *i2c, int devno, uint8_t addr, lis2mdl_attach attach)
{
	...
	if (attach != NULL) {
		...
		err = kthread_create("lis2mdl_thread", SCHED_PRIORITY_DEFAULT,
							 CONFIG_SENSORS_LIS2MDL_THREAD_STACKSIZE, lis2mdl_thread, argv);
		if (err < 0) {
