...
static int lm75_i2c_write(FAR struct lm75_dev_s *priv, FAR const uint8_t *buffer, int buflen);
static int lm75_i2c_read(FAR struct lm75_dev_s *priv, FAR uint8_t *buffer, int buflen);
static int lm75_readb16(FAR struct lm75_dev_s *priv, uint8_t regaddr, FAR b16_t *regvalue);
static int lm75_writeb16(FAR struct lm75_dev_s *priv, uint8_t regaddr, b16_t regval);
...
static int lm75_i2c_write(FAR struct lm75_dev_s *priv, FAR const uint8_t *buffer, int buflen)
{
	...
	msg.frequency = CONFIG_LM75_I2C_FREQUENCY, msg.addr = priv->addr;
	msg.flags = 0;
...
static int lm75_i2c_read(FAR struct lm75_dev_s *priv, FAR uint8_t *buffer, int buflen)
{
	...
	msg.frequency = CONFIG_LM75_I2C_FREQUENCY, msg.addr = priv->addr, msg.flags = I2C_M_READ;
	msg.buffer = buffer;
	msg.length = buflen;
