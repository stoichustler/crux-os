...
static int lm92_i2c_write(FAR struct lm92_dev_s *priv, FAR const uint8_t *buffer, int buflen);
static int lm92_i2c_read(FAR struct lm92_dev_s *priv, FAR uint8_t *buffer, int buflen);
static int lm92_readb16(FAR struct lm92_dev_s *priv, uint8_t regaddr, FAR b16_t *regvalue);
static int lm92_writeb16(FAR struct lm92_dev_s *priv, uint8_t regaddr, b16_t regval);
...
static ssize_t lm92_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t lm92_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int lm92_i2c_write(FAR struct lm92_dev_s *priv, FAR const uint8_t *buffer, int buflen)
{
	...
	msg.frequency = CONFIG_LM92_I2C_FREQUENCY, msg.addr = priv->addr;
	msg.flags = 0;
...
static int lm92_i2c_read(FAR struct lm92_dev_s *priv, FAR uint8_t *buffer, int buflen)
{
	...
	msg.frequency = CONFIG_LM92_I2C_FREQUENCY, msg.addr = priv->addr, msg.flags = I2C_M_READ;
	msg.buffer = buffer;
	msg.length = buflen;
...
static int lm92_readb16(FAR struct lm92_dev_s *priv, uint8_t regaddr, FAR b16_t *regvalue)
{
	...
	*regvalue = (b16_t)((uint32_t)(buffer[0] & (1 << 7)) << 24 |
						(uint32_t)(buffer[0] & ~(1 << 7)) << 17 | (uint32_t)(buffer[1] & ~7) << 9);
...
static int lm92_writeb16(FAR struct lm92_dev_s *priv, uint8_t regaddr, b16_t regval)
{
	...
	buffer[0] = regaddr;
	buffer[1] =
		(uint8_t)(((uint32_t)regval & (1 << 31)) >> 24 | ((uint32_t)regval & ~(1 << 31)) >> 17);
...
int lm92_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
