...
static int ina219_write16(FAR struct ina219_dev_s *priv, uint8_t regaddr, FAR uint16_t regvalue);
static int ina219_read16(FAR struct ina219_dev_s *priv, uint8_t regaddr, FAR uint16_t *regvalue);
static int ina219_readpower(FAR struct ina219_dev_s *priv, FAR struct ina219_s *buffer);

/* Character driver methods */

static int ina219_open(FAR struct file *filep);
static int ina219_close(FAR struct file *filep);
static ssize_t ina219_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t ina219_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int ina219_access(FAR struct ina219_dev_s *priv, uint8_t start_register_address,
...
static int ina219_open(FAR struct file *filep)
{
	...
	return ina219_write16(priv, INA219_REG_CONFIG, priv->config | INA219_CONFIG_OPMODE_SBCONT);
}
...
static int ina219_close(FAR struct file *filep)
{
	...
	return ina219_write16(priv, INA219_REG_CONFIG, priv->config | INA219_CONFIG_OPMODE_OFF);
}
...
int ina219_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
					int32_t shuntval, uint16_t config)
{
	...
	ret = ina219_write16(priv, INA219_REG_CONFIG, priv->config | INA219_CONFIG_OPMODE_OFF);
	if (ret < 0) {
