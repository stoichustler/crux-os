...
static int lsm9ds1_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							bool enable);
static int lsm9ds1_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								FAR uint32_t *period_us);
#ifndef CONFIG_SENSORS_LSM9DS1_POLL
static int lsm9ds1_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						 FAR char *buffer, size_t buflen);
#endif
static int lsm9ds1_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
						   unsigned long arg);

/* Helpers */

static int lsm9ds1_mag_scale(FAR struct lsm9ds1_sensor_s *priv, uint8_t scale);
static int lsm9ds1_accel_scale(FAR struct lsm9ds1_sensor_s *priv, uint8_t scale);
static int lsm9ds1_gyro_scale(FAR struct lsm9ds1_sensor_s *priv, uint8_t scale);

/****************************************************************************
 * Private Data
 ****************************************************************************/

static const struct sensor_ops_s g_sensor_ops = {NULL, /* open */
												 NULL, /* close */
						 ...
						 FAR char *buffer, size_t buflen)
{
	...
	case SENSOR_TYPE_ACCELEROMETER: {
		...
		ret = lsm9ds1_readreg(&priv->base, LSM9DS1_OUT_X_L_XL, (FAR uint8_t *)data, 6);
	...
	case SENSOR_TYPE_GYROSCOPE: {
		...
		ret = lsm9ds1_readreg(&priv->base, LSM9DS1_OUT_X_L_G, (FAR uint8_t *)data, 6);
	...
	case SENSOR_TYPE_MAGNETIC_FIELD: {
		...
		ret = lsm9ds1_readreg(&priv->base, LSM9DS1_OUT_X_L_M, (FAR uint8_t *)data, 6);
...
static void lsm9ds1_accel_data(FAR struct lsm9ds1_sensor_s *priv, FAR int16_t *buf)
{
...
static void lsm9ds1_gyro_data(FAR struct lsm9ds1_sensor_s *priv, FAR int16_t *buf)
{
	FAR struct sensor_lowerhalf_s *lower = &priv->lower;
	struct sensor_gyro gyro;
	uint64_t now = sensor_get_timestamp();
...
static void lsm9ds1_mag_data(FAR struct lsm9ds1_sensor_s *priv, FAR int16_t *buf)
{
	FAR struct sensor_lowerhalf_s *lower = &priv->lower;
	struct sensor_mag mag;
	uint64_t now = sensor_get_timestamp();
...
static int lsm9ds1_thread(int argc, FAR char **argv)
{
	FAR struct lsm9ds1_sensor_dev_s *dev =
		(FAR struct lsm9ds1_sensor_dev_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	...
	while (true) {
		...
		if (accel->enabled) {
			ret = lsm9ds1_readreg(&accel->base, LSM9DS1_OUT_X_L_XL, (FAR uint8_t *)adata, 6);
		...
		if (gyro->enabled) {
			ret = lsm9ds1_readreg(&gyro->base, LSM9DS1_OUT_X_L_G, (FAR uint8_t *)gdata, 6);
		...
		if (mag->enabled) {
			ret = lsm9ds1_readreg(&mag->base, LSM9DS1_OUT_X_L_M, (FAR uint8_t *)mdata, 6);
...
int lsm9ds1_register_uorb(int devno, FAR struct lsm9ds1_config_s *config)
{
	...
	ret = kthread_create("lsm9ds1_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_LSM9DS1_THREAD_STACKSIZE, lsm9ds1_thread, argv);
	if (ret < 0) {
		goto thr_err;
	}
#endif
