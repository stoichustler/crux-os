...
static int gnss_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, bool enable);
static int gnss_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							 FAR uint32_t *interval);
static int gnss_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
						unsigned long arg);

static int gnss_open(FAR struct file *filep);
static int gnss_close(FAR struct file *filep);
static ssize_t gnss_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t gnss_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static const struct gnss_constellation_s g_gnss_constellation[] = {
	{"GP", SENSOR_GNSS_CONSTELLATION_GPS},	  {"GL", SENSOR_GNSS_CONSTELLATION_GLONASS},
	{"GQ", SENSOR_GNSS_CONSTELLATION_QZSS},	  {"GB", SENSOR_GNSS_CONSTELLATION_BEIDOU},
	{"BD", SENSOR_GNSS_CONSTELLATION_BEIDOU}, {"GA", SENSOR_GNSS_CONSTELLATION_GALILEO},
};
...
static int gnss_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, bool enable)
{
	...
	if ((upper->crefs == UINT8_MAX && enable) || (upper->crefs == 0 && !enable)) {
		ret = -EINVAL;
	} else {
...
check:
	...
	ret = circbuf_peekat(&upper->buffer, user->pos, buffer, buflen);
	user->pos += ret;
...
static ssize_t gnss_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	if (upper->lower->ops->inject_data != NULL) {
		ret = upper->lower->ops->inject_data(upper->lower, filep, buffer, buflen);
	}
...
static void gnss_parse_nmea(FAR struct gnss_upperhalf_s *upper, FAR const char *nmea)
{
	...
	case MINMEA_SENTENCE_GGA: {
		...
		if (minmea_parse_gga(&frame, nmea)) {
			...
			upper->gnss.altitude_ellipsoid = minmea_tofloat(&frame.height);
	...
	case MINMEA_SENTENCE_RMC: {
		...
		if (minmea_parse_rmc(&frame, nmea)) {
			...
			upper->gnss.ground_speed =
				minmea_tofloat(&frame.speed) * GNSS_KNOT_TO_KMH / GNSS_KMH_TO_MPS;
	...
	case MINMEA_SENTENCE_GST: {
		...
		if (minmea_parse_gst(&frame, nmea)) {
			lat_err = minmea_tofloat(&frame.latitude_error_deviation);
			lat_err *= lat_err;
			lon_err = minmea_tofloat(&frame.longitude_error_deviation);
			lon_err *= lon_err;
			upper->gnss.eph = sqrtf(lat_err + lon_err);
			upper->gnss.epv = minmea_tofloat(&frame.altitude_error_deviation);
		}
	...
	case MINMEA_SENTENCE_GSV: {
		...
		if (minmea_parse_gsv(&frame, nmea)) {
			...
			memcpy(satellite.info, frame.sats, sizeof(satellite.info[0]) * 4);
			...
			for (i = 0; i < nitems(g_gnss_constellation); i++) {
				if (!strncmp(g_gnss_constellation[i].prefix, nmea,
					...
					if ((satellite.constellation == SENSOR_GNSS_CONSTELLATION_GPS) &&
						(frame.msg_nr > 32)) {
						satellite.constellation = SENSOR_GNSS_CONSTELLATION_SBAS;
					}
			...
			lower->push_event(lower->priv, &satellite, sizeof(satellite));
		}
...
static void gnss_parse(FAR struct gnss_upperhalf_s *upper, FAR const char *buffer, size_t bytes)
{
...
static void gnss_push_data(FAR void *priv, FAR const void *data, size_t bytes, bool is_nmea)
{
...
static void gnss_push_event(FAR void *priv, FAR const void *data, size_t bytes, int type)
{
	FAR struct gnss_upperhalf_s *upper = priv;
	FAR struct sensor_lowerhalf_s *lower;
...
int gnss_register(FAR struct gnss_lowerhalf_s *lower, int devno, uint32_t nbuffer[], size_t count)
{
	FAR struct gnss_upperhalf_s *upper;
	...
	ret = circbuf_init(&upper->buffer, NULL, CONFIG_SENSORS_GNSS_RECV_BUFFERSIZE);
	if (ret < 0) {
		goto circ_err;
	}
...
gnss_clock_err:
	sensor_unregister(&upper->dev[SENSOR_GNSS_IDX_GNSS_MEASUREMENT].lower, devno);
gnss_measurement_err:
	sensor_unregister(&upper->dev[SENSOR_GNSS_IDX_GNSS_SATELLITE].lower, devno);
satellite_err:
