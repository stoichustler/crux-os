...
static ssize_t bh1749nuc_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t bh1749nuc_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int bh1749nuc_close(FAR struct file *filep)
{
	...
	val = (BH1749NUC_SYSTEM_CONTROL_SW_RESET | BH1749NUC_SYSTEM_CONTROL_INT_RESET);
...
static ssize_t bh1749nuc_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct bh1749nuc_dev_s *priv = inode->i_private;
	FAR struct bh1749nuc_data_s *data = (FAR struct bh1749nuc_data_s *)buffer;
...
int bh1749nuc_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	...
	priv = (FAR struct bh1749nuc_dev_s *)kmm_malloc(sizeof(struct bh1749nuc_dev_s));
	if (!priv) {
