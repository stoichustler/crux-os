...
#include <roux/sensors/nau7802.h>
...
static const uint32_t ODR_TO_INTERVAL[] = {[NAU7802_ODR_10HZ] = 100000,
...
static int nau7802_set_bits(FAR nau7802_dev_s *dev, uint8_t addr, uint8_t n_bits,
							uint8_t n_bit_shifts, uint8_t value)
{
	int err = 0;
	uint8_t reg_val;
...
static int nau7802_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								uint32_t *period_us)
{
...
static int nau7802_set_calibvalue(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								  unsigned long arg)
{
	FAR nau7802_dev_s *dev = container_of(lower, FAR nau7802_dev_s, lower);
	uint32_t calibvalue = (uint32_t)arg;
...
static int nau7802_calibrate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static int nau7802_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
						   unsigned long arg)
{
	FAR nau7802_dev_s *dev = container_of(lower, FAR nau7802_dev_s, lower);
	int err = 0;
...
static int nau7802_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							bool enable)
{
	FAR nau7802_dev_s *dev = container_of(lower, FAR nau7802_dev_s, lower);
...
static int nau7802_get_info(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static const struct sensor_ops_s g_sensor_ops = {.activate = nau7802_activate,
...
int nau7802_register(FAR struct i2c_master_s *i2c, int devno, uint8_t addr)
{
	...
	err = kthread_create("nau7802_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_NAU7802_THREAD_STACKSIZE, nau7802_thread, argv);
	if (err < 0) {
