...
static int ina226_write16(FAR struct ina226_dev_s *priv, uint8_t regaddr, FAR uint16_t regvalue);
static int ina226_read16(FAR struct ina226_dev_s *priv, uint8_t regaddr, FAR uint16_t *regvalue);
static int ina226_readpower(FAR struct ina226_dev_s *priv, FAR struct ina226_s *buffer);

/* Character driver methods */

static int ina226_open(FAR struct file *filep);
static int ina226_close(FAR struct file *filep);
static ssize_t ina226_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t ina226_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int ina226_access(FAR struct ina226_dev_s *priv, uint8_t start_register_address,
						 bool reading, FAR uint8_t *register_value, uint8_t data_length)
{
	...
	sninfo("start_register_address: "
		   ...
		   start_register_address, data_length, *register_value, *((FAR uint16_t *)register_value),
		   ret);
...
static int ina226_open(FAR struct file *filep)
{
	...
	return ina226_write16(priv, INA226_REG_CONFIG, priv->config | INA226_CONFIG_MODE_SBCONT);
}
...
static int ina226_close(FAR struct file *filep)
{
	...
	return ina226_write16(priv, INA226_REG_CONFIG, priv->config | INA226_CONFIG_MODE_PWRDOWN);
}
...
int ina226_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
					int32_t shuntval, uint16_t config)
{
	...
	ret = ina226_write16(priv, INA226_REG_CONFIG, priv->config | INA226_CONFIG_MODE_PWRDOWN);
	if (ret < 0) {
