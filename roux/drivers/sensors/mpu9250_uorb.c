...
#include <roux/sensors/mpu9250.h>
...
static int mpu9250_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								FAR uint32_t *period_us);
static int mpu9250_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							bool enable);
static int mpu9250_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
						   unsigned long arg);

static inline int mpu9250_write_gyro_range(FAR struct mpu9250_dev_s *dev, uint8_t fs_sel);
static void mpu9250_gyro_scale(FAR struct mpu9250_sensor_s *priv, enum gyro_config_bit scale);
static inline int mpu9250_write_accel_range(FAR struct mpu9250_dev_s *dev, uint8_t afs_sel);
static void mpu9250_accel_scale(FAR struct mpu9250_sensor_s *priv, enum accel_config_bit scale);
static int ak8963_initialize(FAR struct mpu9250_dev_s *dev, int measure_freq);
static int read_ak8963_reg(FAR struct mpu9250_dev_s *dev, enum mpu9250_regaddr_e reg, uint8_t *val);
static int write_ak8963_reg(FAR struct mpu9250_dev_s *dev, enum mpu9250_regaddr_e reg, uint8_t val);
...
static int mpu9250_read_reg_spi(FAR struct mpu9250_dev_s *dev, enum mpu9250_regaddr_e reg_addr,
...
static int mpu9250_write_reg_spi(FAR struct mpu9250_dev_s *dev, enum mpu9250_regaddr_e reg_addr,
...
static int mpu9250_read_reg_i2c(FAR struct mpu9250_dev_s *dev, uint8_t reg_addr, FAR uint8_t *buf,
								uint8_t len)
{
	int ret;
...
static int mpu9250_write_reg_i2c(FAR struct mpu9250_dev_s *dev, uint8_t reg_addr,
								 FAR const uint8_t *buf, uint8_t len)
{
	int ret;
	struct i2c_msg_s msg[2];
...
static inline int mpu9250_read_reg(FAR struct mpu9250_dev_s *dev, enum mpu9250_regaddr_e reg_addr,
								   FAR uint8_t *buf, uint8_t len)
{
#ifdef CONFIG_MPU9250_SPI
...
static inline int mpu9250_write_reg(FAR struct mpu9250_dev_s *dev, enum mpu9250_regaddr_e reg_addr,
									FAR const uint8_t *buf, uint8_t len)
{
#ifdef CONFIG_MPU9250_SPI
...
static int mpu9250_write_reg_verify(FAR struct mpu9250_dev_s *dev, enum mpu9250_regaddr_e reg_addr,
...
static inline int mpu9250_modify_reg(FAR struct mpu9250_dev_s *dev, enum mpu9250_regaddr_e reg_addr,
...
static inline int mpu9250_read_imu(FAR struct mpu9250_dev_s *dev, FAR struct sensor_data_s *buf)
{
	return mpu9250_read_reg(dev, ACCEL_XOUT_H, (FAR uint8_t *)buf, sizeof(*buf));
}
...
static inline int mpu9250_write_signal_reset(FAR struct mpu9250_dev_s *dev, uint8_t val)
{
	return mpu9250_write_reg(dev, SIGNAL_PATH_RESET, &val, sizeof(val));
}

static inline int mpu9250_write_int_pin_cfg(FAR struct mpu9250_dev_s *dev, uint8_t val)
{
	return mpu9250_write_reg(dev, INT_PIN_CFG, &val, sizeof(val));
}

static inline int mpu9250_write_pwr_mgmt_1(FAR struct mpu9250_dev_s *dev, uint8_t val)
{
	return mpu9250_write_reg(dev, PWR_MGMT_1, &val, sizeof(val));
}

static inline int mpu9250_write_pwr_mgmt_2(FAR struct mpu9250_dev_s *dev, uint8_t val)
{
	return mpu9250_write_reg(dev, PWR_MGMT_2, &val, sizeof(val));
}

static inline int mpu9250_write_user_ctrl(FAR struct mpu9250_dev_s *dev, uint8_t val)
{
	return mpu9250_write_reg(dev, USER_CTRL, &val, sizeof(val));
}

static inline int mpu9250_write_fifo_en(FAR struct mpu9250_dev_s *dev, uint8_t val)
{
...
static inline int mpu9250_write_config(FAR struct mpu9250_dev_s *dev, uint8_t ext_sync_set,
									   uint8_t dlpf_cfg)
{
	uint8_t val =
		TO_BITFIELD(CONFIG_EXT_SYNC_SET, ext_sync_set) | TO_BITFIELD(CONFIG_DLPF_CFG, dlpf_cfg);
...
static inline int mpu9250_write_config2(FAR struct mpu9250_dev_s *dev, uint8_t acce_fchoice_b,
										uint8_t a_dlpf_cfg)
{
...
static int mpu9250_initialize(FAR struct mpu9250_dev_s *dev)
{
	...
	ret = mpu9250_write_user_ctrl(dev, USER_CTRL_FIFO_RST | USER_CTRL_I2C_MST_RST |
...
#ifdef CONFIG_MPU9250_SPI
	if (dev->config.spi) {
		ret = mpu9250_write_user_ctrl(dev, USER_CTRL_I2C_MST_EN | USER_CTRL_I2C_IF_DIS);
	}
#else
	if (dev->config.i2c) {
	...
	ret = mpu9250_write_config(dev, CONFIG_MPU9250_EXT_SYNC_SET, CONFIG_MPU9250_DLPF_CFG);
	if (ret < 0) {
	...
	ret = mpu9250_write_config2(dev, CONFIG_MPU9250_ACCEL_FCHOICE_B, CONFIG_MPU9250_A_DLPF_CFG);
	if (ret < 0) {
...
static int ak8963_initialize(FAR struct mpu9250_dev_s *dev, int measure_freq)
{
	...
	ret = mpu9250_modify_reg(dev, USER_CTRL, USER_CTRL_I2C_MST_EN, USER_CTRL_I2C_MST_EN);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		mpu9250_modify_reg(dev, USER_CTRL, USER_CTRL_I2C_MST_RST, USER_CTRL_I2C_MST_RST);
		return ret;
	}
...
static int get_mag_adjustment(FAR struct mpu9250_dev_s *dev)
{
	...
	ret = write_ak8963_reg(dev, MPU9250_MAG_REG_CNTL1, BIT_MAG_CNTL1_MODE_POWER_DOWN);
	if (ret < 0) {
		return ret;
	}
	...
	ret = write_ak8963_reg(dev, MPU9250_MAG_REG_CNTL1,
						   BIT_MAG_CNTL1_16_BITS | BIT_MAG_CNTL1_FUSE_ROM_ACCESS_MODE);
	if (ret < 0) {
		return ret;
	}
...
static int read_ak8963_reg(FAR struct mpu9250_dev_s *dev, enum mpu9250_regaddr_e reg,
						   FAR uint8_t *val)
{
	int ret = 0;
	uint8_t b = 0;
...
static int write_ak8963_reg(FAR struct mpu9250_dev_s *dev, enum mpu9250_regaddr_e reg, uint8_t val)
{
	...
	ret = mpu9250_write_reg_verify(dev, I2C_SLV4_ADDR, MPU9250_AK8963_I2C_ADDR, 0xff);
	if (ret < 0) {
		return ret;
	}
...
static void mpu9250_accel_data(FAR struct mpu9250_sensor_s *priv, FAR struct sensor_data_s *buf)
{
...
static void mpu9250_gyro_data(FAR struct mpu9250_sensor_s *priv, FAR struct sensor_data_s *buf)
{
	FAR struct sensor_lowerhalf_s *lower = &priv->lower;
	uint64_t now = sensor_get_timestamp();
...
static void mpu9250_mag_data(FAR struct mpu9250_sensor_s *priv, FAR struct sensor_data_s *buf)
{
	FAR struct sensor_lowerhalf_s *lower = &priv->lower;
	uint64_t now = sensor_get_timestamp();
...
static int mpu9250_thread(int argc, FAR char **argv)
{
	FAR struct mpu9250_dev_s *dev =
		(FAR struct mpu9250_dev_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
...
int mpu9250_register(int devno, FAR struct mpu9250_config_s *config)
{
	...
	ret = kthread_create("mpu9250_thread", SCHED_PRIORITY_DEFAULT, CONFIG_MPU9250_THREAD_STACKSIZE,
						 mpu9250_thread, argv);
	if (ret < 0) {
		goto thr_err;
	}
