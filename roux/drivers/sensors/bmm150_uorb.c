...
static int bmm150_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						   bool enable);
#ifndef CONFIG_SENSORS_BMM150_POLL
static int bmm150_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						FAR char *buffer, size_t buflen);
#endif
static int bmm150_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							   FAR uint32_t *period_us);

/* Helpers */

static float bmm150_getx(FAR struct bmm150_sensor_dev_s *dev, FAR int16_t *data);
static float bmm150_gety(FAR struct bmm150_sensor_dev_s *dev, FAR int16_t *data);
static float bmm150_getz(FAR struct bmm150_sensor_dev_s *dev, FAR int16_t *data);
...
static uint8_t bmm150_getreg8(FAR struct bmm150_sensor_dev_s *priv, uint8_t regaddr)
{
...
void bmm150_getregs(FAR struct bmm150_sensor_dev_s *priv, uint8_t regaddr, FAR uint8_t *regval,
					int len)
{
	struct i2c_msg_s msg[2];
	int ret;
...
static void bmm150_putreg8(FAR struct bmm150_sensor_dev_s *priv, uint8_t regaddr, uint8_t regval)
{
	struct i2c_msg_s msg[2];
	uint8_t txbuffer[2];
	int ret;
...
static int bmm150_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						FAR char *buffer, size_t buflen)
{
	FAR struct bmm150_sensor_dev_s *dev = (FAR struct bmm150_sensor_dev_s *)lower;
...
static int bmm150_thread(int argc, FAR char **argv)
{
	FAR struct bmm150_sensor_dev_s *dev =
		(FAR struct bmm150_sensor_dev_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	struct sensor_mag mag_data;
	int16_t data[4];
	int ret = 0;
	...
	while (true) {
		...
		if (dev->enabled) {
			...
			mag_data.y = bmm150_gety(dev, data);
			mag_data.z = bmm150_getz(dev, data);
			dev->lower.push_event(dev->lower.priv, &mag_data, sizeof(mag_data));
		}
...
int bmm150_register_uorb(int devno, FAR struct bmm150_config_s *config)
{
	...
	ret = kthread_create("bmm150_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_BMM150_THREAD_STACKSIZE, bmm150_thread, argv);
	if (ret < 0) {
		goto thr_err;
	}
#endif
