...
#include <roux/sensors/amg88xx.h>
...
static inline int amg88xx_i2c_write_addr(FAR struct amg88xx_dev_s *priv, uint8_t regaddr,
										 uint8_t regval);

static inline int amg88xx_i2c_read_addr(FAR struct amg88xx_dev_s *priv, uint8_t regaddr,
										uint8_t *regval);
...
static ssize_t amg88xx_read(FAR struct file *filep, FAR char *buffer, size_t buflen);

static int amg88xx_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
...
static int amg88xx_open(FAR struct file *filep)
{
	...
	amg88xx_i2c_read_addr(priv, (uint8_t)AMG88XX_POWER_CONTROL_REGISTER, (uint8_t *)&sleep);

	/* Set the sensor to normal operation mode */

	ret = amg88xx_i2c_write_addr(priv, (uint8_t)AMG88XX_POWER_CONTROL_REGISTER,
	...
	if (sleep == AMG88XX_SLEEP_MODE_CMD) {
		...
		ret = amg88xx_i2c_write_addr(priv, (uint8_t)AMG88XX_RESET_REGISTER,
...
static int amg88xx_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SNIOC_SET_FRAMERATE:
		...
		case fps_one:
			...
			ret = amg88xx_i2c_write_addr(priv, (uint8_t)AMG88XX_FRAME_RATE_REGISTER, regval);
			break;
		case fps_ten:
	...
	case SNIOC_SET_MOVING_AVG:
		ret = amg88xx_i2c_write_addr(priv, (uint8_t)AMG88XX_AVERAGE_MODE_REGISTER,
		...
		regval = (bool)arg ? (AMG88XX_MAMOD_ENABLE << AMG88XX_MAMOD_BIT)
						   : (AMG88XX_MAMOD_DISABLE << AMG88XX_MAMOD_BIT);

		ret = amg88xx_i2c_write_addr(priv, (uint8_t)AMG88XX_AVERAGE_REGISTER, regval);
...
int amg88xx_register(FAR const char *devpath, FAR struct i2c_master_s *i2c,
					 FAR struct amg88xx_config_s *config)
{
	...
	priv = (FAR struct amg88xx_dev_s *)kmm_malloc(sizeof(struct amg88xx_dev_s));
