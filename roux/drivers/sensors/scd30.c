...
#ifdef CONFIG_SCD30_I2C
static int scd30_do_transfer(FAR struct i2c_master_s *i2c, FAR struct i2c_msg_s *msgv, size_t nmsg);
#endif
static int scd30_write_cmd(FAR struct scd30_dev_s *priv, uint16_t cmd,
						   FAR struct scd30_word_s *params, unsigned int num_params);
static int scd30_read_words(FAR struct scd30_dev_s *priv, FAR struct scd30_word_s *words,
							unsigned int num_words);

/* Data conversion */

static uint8_t scd30_crc_word(uint16_t word);
static void scd30_set_command_param(FAR struct scd30_word_s *param, uint16_t value);
static int scd30_check_data_crc(FAR const struct scd30_word_s *words, unsigned int num_words);
...
static int scd30_read_values(FAR struct scd30_dev_s *priv, FAR float *temp, FAR float *rh,
...
#endif
static ssize_t scd30_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t scd30_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static ssize_t scd30_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	ret = scd30_read_values(priv, &temp, &rh, &co2, !(filep->f_oflags & O_NONBLOCK));
	if (ret < 0) {
...
static int scd30_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SNIOC_SET_ALTITUDE_COMP: {
		...
		scd30_set_command_param(&param, priv->altitude_comp);
		ret = scd30_write_cmd(priv, SCD30_CMD_SET_ALT_COMPENSATION, &param, 1);
	} break;
	...
	case SNIOC_READ_CONVERT_DATA: {
		...
		if (ret < 0) {
			...
			FAR struct scd30_conv_data_s *data = (FAR struct scd30_conv_data_s *)arg;
...
int scd30_register_i2c(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	FAR struct scd30_dev_s *priv;
	int ret;
