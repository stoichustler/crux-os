...
static int kxtj9_reg_read(FAR struct kxtj9_dev_s *priv, uint8_t regaddr, FAR uint8_t *regval,
						  unsigned int len);
static int kxtj9_reg_write(FAR struct kxtj9_dev_s *priv, uint8_t regaddr, uint8_t regval);
...
static ssize_t kxtj9_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t kxtj9_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
						  ...
						  unsigned int len)
{
	...
	do {
		...
		msg[0].frequency = CONFIG_KXTJ9_I2C_BUS_SPEED, msg[0].addr = priv->address;
		msg[0].flags = 0;
		msg[0].buffer = buf;
		msg[0].length = 1;
		...
		msg[1].frequency = CONFIG_KXTJ9_I2C_BUS_SPEED, msg[1].addr = priv->address;
...
static int kxtj9_reg_write(FAR struct kxtj9_dev_s *priv, uint8_t regaddr, uint8_t regval)
{
	...
	do {
		...
		msg.frequency = CONFIG_KXTJ9_I2C_BUS_SPEED, msg.addr = priv->address;
		msg.flags = 0;
		msg.buffer = buf;
		msg.length = 2;
								  ...
								  FAR struct kxtj9_sensor_data *sensor_data)
{
	...
	add_sensor_randomness((acc_data[0] << 16) ^ (acc_data[1] << 8) ^ acc_data[2]);
...
int kxtj9_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t address)
{
	FAR struct kxtj9_dev_s *priv;
	int ret;
