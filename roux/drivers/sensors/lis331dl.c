...
#include <roux/sensors/lis331dl.h>
...
static int lis331dl_access(FAR struct lis331dl_dev_s *dev, uint8_t subaddr, FAR uint8_t *buf,
						   int length)
{
	uint16_t flags;
	int retval;
	...
	struct i2c_msg_s msgv[2] = {{.frequency = CONFIG_LIS331DL_I2C_FREQUENCY,
								 .addr = dev->address,
								 .flags = 0,
								 .buffer = &subaddr,
								 .length = 1},
...
FAR struct lis331dl_dev_s *lis331dl_init(FAR struct i2c_master_s *i2c, uint16_t address)
{
...
{
	dev->cr1 = ST_LIS331DL_CR1_PD | ST_LIS331DL_CR1_ZEN | ST_LIS331DL_CR1_YEN | ST_LIS331DL_CR1_XEN;
	dev->cr2 = 0;
	dev->cr3 = 0;
...
{
	dev->cr1 = ST_LIS331DL_CR1_PD | (full ? ST_LIS331DL_CR1_FS : 0) |
			   (fast ? ST_LIS331DL_CR1_DR : 0) | ST_LIS331DL_CR1_ZEN | ST_LIS331DL_CR1_YEN |
			   ST_LIS331DL_CR1_XEN;
...
FAR const struct lis331dl_vector_s *lis331dl_getreadings(FAR struct lis331dl_dev_s *dev)
{
	uint8_t retval[7];
	...
	if (lis331dl_access(dev, ST_LIS331DL_STATUS_REG, retval, 7) == 7) {
		...
		add_sensor_randomness((retval[2] << 16) ^ (retval[4] << 8) ^ (retval[6] << 0));
