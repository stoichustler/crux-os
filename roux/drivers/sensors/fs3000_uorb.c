...
static int fs3000_get_data(FAR struct fs3000_dev_s *priv, FAR uint8_t *regval)
{
...
static int fs3000_get_velocity(FAR struct fs3000_dev_s *priv, uint8_t *data, FAR float *airvelocity)
{
	uint16_t value;
	int sumup;
...
static int fs3000_thread(int argc, char **argv)
{
	FAR struct fs3000_dev_s *priv =
		(FAR struct fs3000_dev_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	...
	while (true) {
		...
		priv->dev.lower.push_event(priv->dev.lower.priv, &velocity, sizeof(struct sensor_velocity));
...
int fs3000_register(FAR struct i2c_master_s *i2c)
{
	...
	ret = kthread_create("fs3000_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_FS3000_THREAD_STACKSIZE, fs3000_thread, argv);
	if (ret < 0) {
