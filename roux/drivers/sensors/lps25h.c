...
static ssize_t lps25h_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t lps25h_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int lps25h_read_pressure(FAR struct lps25h_dev_s *dev, FAR lps25h_pressure_data_t *pres);
static int lps25h_read_temper(FAR struct lps25h_dev_s *dev, FAR lps25h_temper_data_t *temper);
...
static int lps25h_do_transfer(FAR struct lps25h_dev_s *dev, FAR struct i2c_msg_s *msgv, size_t nmsg)
{
	int ret = -EIO;
	int retries;
...
static int lps25h_write_reg8(FAR struct lps25h_dev_s *dev, uint8_t reg_addr, const uint8_t value)
{
	struct i2c_msg_s msgv[2] = {{.frequency = CONFIG_LPS25H_I2C_FREQUENCY,
								 .addr = dev->addr,
								 .flags = 0,
								 .buffer = &reg_addr,
								 .length = 1},
...
static int lps25h_read_reg8(FAR struct lps25h_dev_s *dev, FAR uint8_t *reg_addr, FAR uint8_t *value)
{
	struct i2c_msg_s msgv[2] = {{.frequency = CONFIG_LPS25H_I2C_FREQUENCY,
								 .addr = dev->addr,
								 .flags = 0,
...
static int lps25h_configure_dev(FAR struct lps25h_dev_s *dev)
{
	...
	ret = lps25h_write_reg8(dev, LPS25H_CTRL_REG1, LPS25H_PD | (CTRL_REG1_ODR_1HZ << 4));
...
static int lps25h_one_shot(FAR struct lps25h_dev_s *dev)
{
	...
	for (retries = 0; retries < LPS25H_MAX_RETRIES; retries++) {
		...
		ret = nxsem_tickwait_uninterruptible(&dev->waitsem, MSEC2TICK(LPS25H_RETRY_TIMEOUT_MSECS));
		if (ret == OK) {
			break;
...
static int lps25h_read_pressure(FAR struct lps25h_dev_s *dev, FAR lps25h_pressure_data_t *pres)
{
	...
	pres_res = ((int32_t)pres_value_h << 16) | ((int16_t)pres_value_l << 8) | pres_value_xl;
...
static int lps25h_who_am_i(FAR struct lps25h_dev_s *dev, lps25h_who_am_i_data *who_am_i_data)
{
...
int lps25h_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
					FAR lps25h_config_t *config)
{
	int ret = 0;
