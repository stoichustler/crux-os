...
#include <roux/1wire/1wire.h>
#include <roux/1wire/1wire_master.h>
#include <roux/1wire/1wire_crc.h>
#include <roux/sensors/ds18b20.h>
...
#define DS18B20_TEMP_DEC(lsb, res) (((lsb)&0x0f) & DS18B20_TEMP_RESMASK(lsb, res))
#define DS18B20_TEMP(lsb, msb, res)                                                                \
	(DS18B20_TEMP_PREDEC(lsb, msb) + DS18B20_TEMP_DEC(lsb, res) / 16.0)
...
static int ds18b20_active(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						  bool enabled);

static int ds18b20_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						 FAR char *buffer, size_t buflen);

static int ds18b20_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
						   unsigned long arg);

#ifdef CONFIG_SENSORS_DS18B20_POLL
static int ds18b20_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static const uint32_t g_res_timeout[DS18B20_NRES] = {93750, 187500, 375000, 750000};
...
{
	return (spad[DS18B20_SPAD_LSB_OFFSET] | (spad[DS18B20_SPAD_MSB_OFFSET] << 8));
}
...
{
	int8_t predec =
		DS18B20_TEMP_PREDEC(spad[DS18B20_SPAD_LSB_OFFSET], spad[DS18B20_SPAD_MSB_OFFSET]);
	uint8_t dec = DS18B20_TEMP_DEC(spad[DS18B20_SPAD_LSB_OFFSET], spad[DS18B20_SPAD_RES_OFFSET]);
						   ...
						   FAR const struct onewire_config_s *config)
{
	...
	ret = onewire_search(master, DS18B20_DEVICE_FAMILY, true, ds18b20_alarm_cb, &alarm);
	if (ret > 0) {
...
static int ds18b20_write_spad(FAR struct ds18b20_dev_s *dev, int8_t th, int8_t tl, uint8_t res)
{
	int ret;
...
static int ds18b20_read_spad(FAR struct ds18b20_dev_s *dev, FAR uint8_t *spad)
{
	int ret;
	uint8_t crc;
	...
	ret = onewire_writeread(dev->master, &dev->config, &wbuf, 1, spad, DS18B20_SPAD_SIZE);
	if (ret < 0) {
	...
	if (reg->res < DS18B20_RES_CONV(DS18B20_RESMIN) || reg->res > DS18B20_RES_CONV(reg->res)) {
...
static int ds18b20_set_res(FAR struct ds18b20_dev_s *dev, uint8_t res)
{
	...
	ret = ds18b20_write_spad(dev, dev->reg.alarm.thigh, dev->reg.alarm.tlow, res);
	if (ret < 0) {
		return ret;
	}
...
static int ds18b20_set_alarm(FAR struct ds18b20_dev_s *dev, FAR const struct ds18b20_alarm_s *alarm)
{
	int ret;
	uint8_t spad[DS18B20_SPAD_SIZE];
	...
	if (spad[DS18B20_SPAD_TH_OFFSET] != alarm->thigh ||
		...
		snerr("ERROR: Expected alarm trigger does not match, "
			  "received TH: %d, TL: %d\n",
			  spad[DS18B20_SPAD_TH_OFFSET], spad[DS18B20_SPAD_TL_OFFSET]);
		return -EIO;
	}
...
#ifdef CONFIG_SENSORS_DS18B20_POLL
static void ds18b20_notify(FAR struct ds18b20_dev_s *dev, FAR struct ds18b20_sensor_data_s *data)
{
	...
	sensor->lower.push_event(sensor->lower.priv, &temp, sizeof(struct sensor_temp));
}
#endif
						 ...
						 FAR char *buffer, size_t buflen)
{
	...
	if (!ret) {
		FAR struct sensor_temp *temp = (FAR struct sensor_temp *)buffer;
		temp->temperature = ds18b20_temp(data.spad);
		temp->timestamp = data.timestamp;
	} else {
		return ret;
						   ...
						   unsigned long arg)
{
	...
	case SNIOC_SETALARM: {
		FAR struct ds18b20_alarm_s *ptr = (FAR struct ds18b20_alarm_s *)((uintptr_t)arg);
		DEBUGASSERT(ptr != NULL);
...
{
	uint16_t orawdata;
	FAR struct ds18b20_dev_s *priv =
		(FAR struct ds18b20_dev_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
...
int ds18b20_register(int devno, FAR struct onewire_master_s *onewire, uint64_t romcode)
{
	int ret;
	...
	FAR struct ds18b20_dev_s *priv =
		(FAR struct ds18b20_dev_s *)kmm_malloc(sizeof(struct ds18b20_dev_s));
	...
	ret = kthread_create("ds18b20_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_DS18B20_THREAD_STACKSIZE, ds18b20_thread, argv);
	if (ret > 0)
#endif
	{
		return OK;
