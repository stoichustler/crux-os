...
static int sgp30_do_transfer(FAR struct i2c_master_s *i2c, FAR struct i2c_msg_s *msgv, size_t nmsg);
static int sgp30_write_cmd(FAR struct sgp30_dev_s *priv, struct sgp30_cmd_s cmd,
						   FAR struct sgp30_word_s *params, unsigned int num_params);
static int sgp30_read_cmd(FAR struct sgp30_dev_s *priv, struct sgp30_cmd_s cmd,
						  FAR struct sgp30_word_s *words, unsigned int num_words);

/* Data conversion */

static uint8_t sgp30_crc_word(uint16_t word);
static void sgp30_set_command_param(FAR struct sgp30_word_s *param, uint16_t value);
static int sgp30_check_data_crc(FAR const struct sgp30_word_s *words, unsigned int num_words);
static uint16_t sgp30_data_word_to_uint16(FAR const struct sgp30_word_s *word);

/* Driver features */

static int sgp30_measure_airq(FAR struct sgp30_dev_s *priv, FAR struct sgp30_conv_data_s *data);
static int sgp30_measure_raw(FAR struct sgp30_dev_s *priv, FAR struct sgp30_raw_data_s *data);
...
#endif
static ssize_t sgp30_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t sgp30_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static const struct sgp30_cmd_s SGP30_CMD_INIT_AIR_QUALITY = {0x2003, 0};

static const struct sgp30_cmd_s SGP30_CMD_MEASURE_AIR_QUALITY = {0x2008, 12000};

static const struct sgp30_cmd_s SGP30_CMD_GET_BASELINE = {0x2015, 10000};

static const struct sgp30_cmd_s SGP30_CMD_SET_BASELINE = {0x201e, 10000};

static const struct sgp30_cmd_s SGP30_CMD_SET_HUMIDITY = {0x2061, 10000};

static const struct sgp30_cmd_s SGP30_CMD_MEASURE_TEST = {0x2032, 220000};

static const struct sgp30_cmd_s SGP30_CMD_GET_FEATURE_SET_VERSION = {0x202f, 2000};

static const struct sgp30_cmd_s SGP30_CMD_MEASURE_RAW_SIGNALS = {0x2050, 25000};

static const struct sgp30_cmd_s SGP30_CMD_GET_SERIAL_ID = {0x3682, 500};
...
static bool has_time_passed(FAR struct timespec *curr, FAR struct timespec *start,
...
static bool time_has_passed_ms(FAR struct timespec *curr, FAR struct timespec *start,
	...
	if (priv->crefs == 1) {
		...
		if (sgp30_read_cmd(priv, SGP30_CMD_GET_SERIAL_ID, serial, 3) >= 0 &&
			...
			sgp30_dbg("serial id: %04x-%04x-%04x\n", sgp30_data_word_to_uint16(serial + 0),
					  sgp30_data_word_to_uint16(serial + 1), sgp30_data_word_to_uint16(serial + 2));
...
static int sgp30_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SNIOC_GET_BASELINE: {
		FAR struct sgp30_baseline_s *baseline = (FAR struct sgp30_baseline_s *)arg;
	...
	case SNIOC_SET_BASELINE: {
		FAR const struct sgp30_baseline_s *baseline = (FAR const struct sgp30_baseline_s *)arg;
	...
	case SNIOC_READ_CONVERT_DATA: {
		FAR struct sgp30_conv_data_s *data = (FAR struct sgp30_conv_data_s *)arg;
	...
	case SNIOC_READ_RAW_DATA: {
		FAR struct sgp30_raw_data_s *data = (FAR struct sgp30_raw_data_s *)arg;
...
int sgp30_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	FAR struct sgp30_dev_s *priv;
	int ret;
