...
#ifdef CONFIG_SPS30_I2C
static int sps30_do_transfer(FAR struct i2c_master_s *i2c, FAR struct i2c_msg_s *msgv, size_t nmsg);
#endif
static int sps30_write_cmd(FAR struct sps30_dev_s *priv, uint16_t cmd,
						   FAR struct sps30_word_s *params, unsigned int num_params);
static int sps30_read_cmd(FAR struct sps30_dev_s *priv, uint16_t cmd,
						  FAR struct sps30_word_s *words, unsigned int num_words);

/* Data conversion */

static uint8_t sps30_crc_word(uint16_t word);
static void sps30_set_command_param(FAR struct sps30_word_s *param, uint16_t value);
static int sps30_check_data_crc(FAR const struct sps30_word_s *words, unsigned int num_words);
...
static int sps30_read_values(FAR struct sps30_dev_s *priv, FAR struct sps30_conv_data_s *out,
							 bool wait);
static int sps30_configure(FAR struct sps30_dev_s *priv, bool start);
static int sps30_read_dev_info(FAR struct sps30_dev_s *priv, uint16_t cmd, char *out,
							   size_t outlen);
...
#endif
static ssize_t sps30_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t sps30_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static uint8_t sps30_crc_word(uint16_t word)
{
	static const uint8_t crc_table[16] = {0x00, 0x31, 0x62, 0x53, 0xc4, 0xf5, 0xa6, 0x97,
							 ...
							 bool wait)
{
	...
	if (wait || !priv->valid ||
		...
		add_sensor_randomness(
			((data[0].crc ^ data[1].crc) << 0) ^ ((data[2].crc ^ data[3].crc) << 8) ^
			((data[4].crc ^ data[5].crc) << 16) ^ ((data[6].crc ^ data[7].crc) << 24) ^
			((data[8].crc ^ data[9].crc) << 0) ^ ((data[10].crc ^ data[11].crc) << 8) ^
			((data[12].crc ^ data[13].crc) << 16) ^ ((data[14].crc ^ data[15].crc) << 24) ^
			((data[16].crc ^ data[17].crc) << 0) ^ ((data[18].crc ^ data[19].crc) << 8));

		priv->data.mass_concenration_pm1_0 = sps30_data_words2float(data + 0);
		priv->data.mass_concenration_pm2_5 = sps30_data_words2float(data + 2);
		priv->data.mass_concenration_pm4_0 = sps30_data_words2float(data + 4);
		priv->data.mass_concenration_pm10 = sps30_data_words2float(data + 6);
		priv->data.number_concenration_pm0_5 = sps30_data_words2float(data + 8);
		priv->data.number_concenration_pm1_0 = sps30_data_words2float(data + 10);
		priv->data.number_concenration_pm2_5 = sps30_data_words2float(data + 12);
		priv->data.number_concenration_pm4_0 = sps30_data_words2float(data + 14);
		priv->data.number_concenration_pm10 = sps30_data_words2float(data + 16);
		priv->data.typical_particle_size = sps30_data_words2float(data + 18);
		priv->last_update = ts;
		priv->valid = true;
...
static int sps30_read_dev_info(FAR struct sps30_dev_s *priv, uint16_t cmd, char *out, size_t outlen)
{
...
static int sps30_open(FAR struct file *filep)
{
	...
	if (priv->crefs == 1) {
		...
		ret = sps30_read_dev_info(priv, SPS30_CMD_READ_ARTICLE_CODE, code.c, sizeof(code.c));
		if (ret >= 0) {
			ret = sps30_read_dev_info(priv, SPS30_CMD_READ_SERIAL_NUMBER, sn.c, sizeof(sn.c));
			if (ret >= 0) {
				static int once;

				if (!once) {
					once = 1;
					up_rngaddentropy(RND_SRC_SENSOR, code.u32, (strlen(code.c) + 3) / 4);
					up_rngaddentropy(RND_SRC_SENSOR, sn.u32, (strlen(sn.c) + 3) / 4);
				}
...
static ssize_t sps30_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (ret < 0) {
		...
		length = snprintf(buffer, buflen,
						  ...
						  "%u.%02u %u.%02u %u.%02u %u.%02u %u.%02u\n",
						  data100[0] / 100, data100[0] % 100, data100[1] / 100, data100[1] % 100,
						  data100[2] / 100, data100[2] % 100, data100[3] / 100, data100[3] % 100,
						  data100[4] / 100, data100[4] % 100, data100[5] / 100, data100[5] % 100,
						  data100[6] / 100, data100[6] % 100, data100[7] / 100, data100[7] % 100,
						  data100[8] / 100, data100[8] % 100, data100[9] / 100, data100[9] % 100);
		if (length > buflen) {
			length = buflen;
		}
	}
...
static int sps30_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SNIOC_SET_CLEAN_INTERVAL: {
		...
		ret = sps30_write_cmd(priv, SPS30_CMD_SET_AUTO_CLEANING_INTERVAL, &param, 1);
	} break;
	...
	case SNIOC_READ_CONVERT_DATA: {
		FAR struct sps30_conv_data_s *data = (FAR struct sps30_conv_data_s *)arg;
...
int sps30_register_i2c(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	FAR struct sps30_dev_s *priv;
	int ret;
