...
#include <roux/sensors/max44009.h>
...
static ssize_t max44009_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t max44009_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int max44009_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int max44009_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);

static int max44009_read_data(FAR struct max44009_dev_s *priv, FAR struct max44009_data_s *data);
...
static int max44009_do_transfer(FAR struct max44009_dev_s *dev, FAR struct i2c_msg_s *msgv,
								size_t nmsg)
{
	int ret = -EIO;
	int retries;
...
static int max44009_write_reg8(FAR struct max44009_dev_s *dev, FAR const uint8_t *command)
{
	struct i2c_msg_s msgv[2] = {{.frequency = CONFIG_MAX44009_I2C_FREQUENCY,
								 .addr = dev->addr,
								 .flags = 0,
...
static int max44009_read_reg8(FAR struct max44009_dev_s *dev, FAR uint8_t *command,
							  FAR uint8_t *value)
{
	struct i2c_msg_s msgv[2] = {{.frequency = CONFIG_MAX44009_I2C_FREQUENCY,
								 .addr = dev->addr,
...
static int max44009_set_interrupt_bit(FAR struct max44009_dev_s *priv, bool is_activated)
{
	int ret;
	uint8_t cmd[2];
...
static int max44009_set_manual_mode(FAR struct max44009_dev_s *priv, bool is_manual)
{
	int ret;
	uint8_t value = 0;
...
static int max44009_set_continuous_mode(FAR struct max44009_dev_s *priv, bool is_cont)
{
	int ret;
	uint8_t value = 0;
...
static int max44009_set_current_div_ratio(FAR struct max44009_dev_s *priv, bool is_cdr)
{
	int ret;
	uint8_t value = 0;
...
static int max44009_set_integration_time(FAR struct max44009_dev_s *priv,
										 enum max44009_integration_time_e integration_time)
{
	int ret;
	uint8_t value = 0;
...
static int max44009_set_threshold_timer(FAR struct max44009_dev_s *priv, uint8_t threshold_timer)
{
	int ret;
	uint8_t cmd[2];
...
static int max44009_selftest(FAR struct max44009_dev_s *priv, FAR struct max44009_data_s *data)
{
	int ret;
...
static int max44009_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SNIOC_THRESHOLD:
		ret = max44009_set_threshold(priv, (FAR struct max44009_threshold_s *)arg);
		break;
...
int max44009_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
					  FAR struct max44009_config_s *config)
{
