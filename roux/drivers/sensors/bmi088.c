...
static ssize_t bmi088_acc_read(FAR struct file *filep, FAR char *buffer, size_t len);
static ssize_t bmi088_gyro_read(FAR struct file *filep, FAR char *buffer, size_t len);
static int bmi088_acc_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int bmi088_gyro_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
...
static int bmi088_acc_open(FAR struct file *filep)
{
	...
	up_mdelay(1);
	bmi088_put_acc_reg8(priv, BMI088_ACC_PWR_CTRL, BMI088_ACC_PWR_CTRL_ACC_ENABLE);
	up_mdelay(5);
	bmi088_put_acc_reg8(priv, BMI088_ACC_RANGE, acc_range);
	up_mdelay(50);
	bmi088_put_acc_reg8(priv, BMI088_ACC_PWR_CONF, BMI088_ACC_PWR_CONF_ACTIVE_MODE);
...
static int bmi088_acc_close(FAR struct file *filep)
{
	...
	bmi088_put_acc_reg8(priv, BMI088_ACC_PWR_CONF, BMI088_ACC_PWR_CONF_SUSPEND_MODE);
	up_mdelay(5);
	bmi088_put_acc_reg8(priv, BMI088_ACC_PWR_CTRL, BMI088_ACC_PWR_CTRL_ACC_DISABLE);
	up_mdelay(5);
...
static ssize_t bmi088_gyro_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	...
	bmi088_get_gyro_regs(priv, BMI088_GYRO_X_LSB, (uint8_t *)&p->gyro_source, 6);
...
int bmi088_acc_register(FAR const char *devpath, FAR struct i2c_master_s *dev)
...
#ifdef CONFIG_SENSORS_BMI088_I2C
int bmi088_gyro_register(FAR const char *devpath, FAR struct i2c_master_s *dev)
#else /* CONFIG_SENSORS_BMI088_SPI */
