...
static int bmp280_putreg8(FAR struct bmp280_dev_s *priv, uint8_t regaddr, uint8_t regval);

/* Sensor methods */

static int bmp280_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							   FAR uint32_t *period_us);
static int bmp280_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						   bool enable);
static int bmp280_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static int bmp280_getregs(FAR struct bmp280_dev_s *priv, uint8_t regaddr, uint8_t *rxbuffer,
						  uint8_t length)
{
	struct i2c_msg_s msg[2];
	int ret;
...
static int32_t bmp280_compensate_temp(FAR struct bmp280_dev_s *priv, int32_t temp)
{
	...
	var2 = (((((temp >> 4) - ((int32_t)c->t1)) * ((temp >> 4) - ((int32_t)c->t1))) >> 12) *
			((int32_t)c->t3)) >>
		   14;
...
static uint32_t bmp280_compensate_press(FAR struct bmp280_dev_s *priv, uint32_t press)
{
	struct bmp280_calib_s *c = &priv->calib;
	int32_t var1;
	int32_t var2;
	...
	var1 =
		(((c->p3 * (((var1 >> 2) * (var1 >> 2)) >> 13)) >> 3) + ((((int32_t)c->p2) * var1) >> 1)) >>
		18;
...
static int bmp280_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							   FAR uint32_t *period_us)
{
	FAR struct bmp280_dev_s *priv = container_of(lower, FAR struct bmp280_dev_s, sensor_lower);
	int ret = 0;
						   ...
						   bool enable)
{
	...
	if (enable) {
		...
		ret = bmp280_putreg8(priv, BMP280_CTRL_MEAS, BMP280_NORMAL_MODE | BMP280_OS_STANDARD_RES);
	} else {
		/* Set to sleep mode */
						...
						FAR char *buffer, size_t buflen)
{
	...
	if (!priv->activated) {
		...
		ret =
			bmp280_putreg8(priv, BMP280_CTRL_MEAS, BMP280_FORCED_MODE | BMP280_OS_ULTRA_LOW_POWER);
