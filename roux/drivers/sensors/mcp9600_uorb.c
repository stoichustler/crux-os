...
#include <roux/sensors/mcp9600.h>
...
static int mcp9600_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
						   unsigned long arg);
static int mcp9600_get_info(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							FAR struct sensor_device_info_s *info);
static int mcp9600_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								FAR uint32_t *period_us);
static int mcp9600_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							bool enable);
#ifndef CONFIG_SENSORS_MCP9600_POLL
static int mcp9600_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static const uint8_t g_thermo_types[] = {
	[SENSOR_THERMO_TYPE_K] = 0x0, [SENSOR_THERMO_TYPE_J] = 0x1, [SENSOR_THERMO_TYPE_T] = 0x2,
	[SENSOR_THERMO_TYPE_N] = 0x3, [SENSOR_THERMO_TYPE_S] = 0x4, [SENSOR_THERMO_TYPE_E] = 0x5,
...
static int mcp9600_read_reg(FAR struct mcp9600_dev_s *priv, uint8_t reg, FAR void *buf,
							size_t nbytes)
{
	...
	return I2C_TRANSFER(priv->i2c, read_cmd, sizeof(read_cmd) / sizeof(struct i2c_msg_s));
}
...
static int mcp9600_write_reg(FAR struct mcp9600_dev_s *priv, uint8_t reg, FAR void *buf,
							 size_t nbytes)
{
	struct i2c_msg_s read_cmd[] = {
		{
...
static int mcp9600_read(FAR struct mcp9600_dev_s *priv, FAR struct sensor_temp *hot,
						FAR struct sensor_temp *cold, FAR struct sensor_temp *delta)
{
	int err;
								...
								FAR struct mcp9600_alert_conf_s *config)
{
	...
	err =
		mcp9600_write_reg(priv, g_alert_hysts[config->alert], &config->temp, sizeof(config->temp));
	if (err < 0) {
		return err;
	}
	...
	err = mcp9600_write_reg(priv, g_alert_limits[config->alert], &limit, sizeof(limit));
	if (err < 0) {
		return err;
	}
	...
	return mcp9600_write_reg(priv, g_alert_configs[config->alert], &config_reg, sizeof(config_reg));
}
...
static int mcp9600_validate_conf(FAR struct mcp9600_devconf_s *conf)
{
	...
	if (conf->resolution < MCP9600_ADC_RES_18 || conf->resolution > MCP9600_ADC_RES_12) {
		return -EINVAL;
	}

	if (conf->num_samples < MCP9600_SAMPLE_1 || conf->num_samples > MCP9600_SAMPLE_128) {
		return -EINVAL;
	}
	...
	if (conf->cold_res != MCP9600_COLDRES_0625 || conf->cold_res != MCP9600_COLDRES_25) {
		return -EINVAL;
	}
...
static int mcp9600_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								FAR uint32_t *period_us)
{
	FAR struct mcp9600_sens_s *sens = container_of(lower, FAR struct mcp9600_sens_s, lower);
							...
							bool enable)
{
	...
	int err = 0;
	FAR struct mcp9600_sens_s *priv = container_of(lower, FAR struct mcp9600_sens_s, lower);
	...
	if (enable && !priv->enabled) {
		...
		if ((dev->conf.mode != MCP9600_MODE_SHUTDOWN) &&
			(!dev->hot_junc.enabled && !dev->cold_junc.enabled && !dev->delta.enabled)) {
						   ...
						   unsigned long arg)
{
	...
	case SNIOC_WHO_AM_I: {
		struct mcp9600_devinfo_s *devinfo = (struct mcp9600_devinfo_s *)(arg);
		if (devinfo == NULL) {
			err = -EINVAL;
			break;
		}
	...
	case SNIOC_READ_RAW_DATA: {
		...
		err = mcp9600_read_reg(dev, REG_RAW_ADC, raw_data, 3); /* Only read 24 bits */
	...
	case SNIOC_CHECK_STATUS_REG: {
		...
		err = mcp9600_read_reg(dev, REG_STATUS, &status_reg, sizeof(status_reg));
		if (err < 0) {
			break;
		}
		...
		status_reg &= 0x3f;
		err = mcp9600_write_reg(dev, REG_STATUS, &status_reg, sizeof(status_reg));
	} break;

		/* Configure the MCP9600 */

	case SNIOC_CONFIGURE: {
		uint8_t registers[2] = {0, 0};
		...
		err = mcp9600_write_reg(dev, REG_THERMO_SEN_CONF, registers, sizeof(registers));
		if (err < 0) {
			break;
		};
	...
	case SNIOC_WRITECONF: {
		struct mcp9600_alert_conf_s *conf = (struct mcp9600_alert_conf_s *)(arg);
		if (conf == NULL) {
			err = -EINVAL;
			break;
		}
...
static int mcp9600_thread(int argc, char **argv)
{
	...
	while (true) {
		if (!dev->hot_junc.enabled && !dev->cold_junc.enabled && !dev->delta.enabled) {
		...
		if (dev->hot_junc.enabled) {
			dev->hot_junc.lower.push_event(dev->hot_junc.lower.priv, &hot_junc, sizeof(hot_junc));
		}

		if (dev->cold_junc.enabled) {
			dev->cold_junc.lower.push_event(dev->cold_junc.lower.priv, &cold_junc,
											sizeof(cold_junc));
		}

		if (dev->delta.enabled) {
			dev->delta.lower.push_event(dev->delta.lower.priv, &delta, sizeof(delta));
		}
...
int mcp9600_register(FAR struct i2c_master_s *i2c, uint8_t addr, uint8_t h_devno, uint8_t c_devno,
					 uint8_t d_devno)
{
	...
	argv[0] = arg1;
	argv[1] = NULL;
	err = kthread_create("mcp9600_thread", SCHED_PRIORITY_DEFAULT, CONFIG_MCP9600_THREAD_STACKSIZE,
						 mcp9600_thread, argv);
	if (err < 0) {
