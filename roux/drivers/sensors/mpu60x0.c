...
#include <roux/sensors/mpu60x0.h>
...
static int __mpu_read_reg_spi(FAR struct mpu_dev_s *dev, enum mpu_regaddr_e reg_addr,
...
static int __mpu_write_reg_spi(FAR struct mpu_dev_s *dev, enum mpu_regaddr_e reg_addr,
...
static int __mpu_read_reg_i2c(FAR struct mpu_dev_s *dev, uint8_t reg_addr, FAR uint8_t *buf,
							  uint8_t len)
{
	int ret;
...
static int __mpu_write_reg_i2c(FAR struct mpu_dev_s *dev, uint8_t reg_addr, FAR const uint8_t *buf,
							   uint8_t len)
{
	int ret;
	struct i2c_msg_s msg[2];
...
static inline int __mpu_read_reg(FAR struct mpu_dev_s *dev, enum mpu_regaddr_e reg_addr,
								 FAR uint8_t *buf, uint8_t len)
{
#ifdef CONFIG_MPU60X0_SPI
...
static inline int __mpu_write_reg(FAR struct mpu_dev_s *dev, enum mpu_regaddr_e reg_addr,
								  FAR const uint8_t *buf, uint8_t len)
{
#ifdef CONFIG_MPU60X0_SPI
...
static inline int __mpu_read_imu(FAR struct mpu_dev_s *dev, FAR struct sensor_data_s *buf)
{
	if (dev->fifo_enabled) {
...
static inline int __mpu_write_config(FAR struct mpu_dev_s *dev, uint8_t ext_sync_set,
									 uint8_t dlpf_cfg)
{
	uint8_t val =
		TO_BITFIELD(CONFIG__EXT_SYNC_SET, ext_sync_set) | TO_BITFIELD(CONFIG__DLPF_CFG, dlpf_cfg);
	return __mpu_write_reg(dev, CONFIG, &val, sizeof(val));
}

/* Sets the SMPLRT_DIV that controls the sample rate. */

static inline int __mpu_set_sample_rate_divider(FAR struct mpu_dev_s *dev, uint8_t val)
{
...
static inline int __mpu_read_sample_rate(FAR struct mpu_dev_s *dev)
{
	...
	ret = __mpu_read_reg(dev, SMPLRT_DIV, &dev->smplrt_div, sizeof(dev->smplrt_div));
	if (ret < 0) {
		return ret;
	}

	ret = __mpu_read_reg(dev, CONFIG, &dev->dlpf_config, sizeof(dev->dlpf_config));
	if (ret < 0) {
		return ret;
	}
...
static inline int __mpu_read_fifo_count(FAR struct mpu_dev_s *dev, uint16_t *buf)
{
	int ret;
...
static inline int __mpu_set_fifo(FAR struct mpu_dev_s *dev, uint8_t val)
{
...
static inline int __mpu_user_control(FAR struct mpu_dev_s *dev, uint8_t val)
{
	return __mpu_write_reg(dev, USER_CTRL, &val, sizeof(val));
}
...
static int mpu_reset(FAR struct mpu_dev_s *dev)
{
	...
	__mpu_write_config(dev, CONFIG_MPU60X0_EXT_SYNC_SET, CONFIG_MPU60X0_DLPF_CFG);
...
static int mpu_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SNIOC_ENABLE_FIFO:
		if (!write_data) {
			...
			ret = __mpu_set_fifo(priv, FIFO_EN__TEMP | FIFO_EN__XG | FIFO_EN__YG | FIFO_EN__ZG |
										   FIFO_EN__ACCEL);
