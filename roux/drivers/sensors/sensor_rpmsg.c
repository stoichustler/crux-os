...
#define SENSOR_RPMSG_FUNCTION(name, cmd, arg1, arg2, size, wait, type)                             \
	static int sensor_rpmsg_##name(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,   \
								   type arg1)                                                      \
	{                                                                                              \
...
static int sensor_rpmsg_open(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep);
static int sensor_rpmsg_close(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep);
static int sensor_rpmsg_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								 bool enable);
static int sensor_rpmsg_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
									 FAR uint32_t *period_us);
static int sensor_rpmsg_batch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							  FAR uint32_t *latency_us);
static int sensor_rpmsg_flush(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep);
static int sensor_rpmsg_selftest(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								 unsigned long arg);
static int sensor_rpmsg_set_calibvalue(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
									   unsigned long arg);
static int sensor_rpmsg_calibrate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								  unsigned long arg);
static int sensor_rpmsg_get_info(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								 FAR struct sensor_device_info_s *info);
static int sensor_rpmsg_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
								int cmd, unsigned long arg);
static int sensor_rpmsg_adv_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									uint32_t src, FAR void *priv);
static int sensor_rpmsg_advack_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									   uint32_t src, FAR void *priv);
static int sensor_rpmsg_unadv_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									  uint32_t src, FAR void *priv);
static int sensor_rpmsg_sub_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									uint32_t src, FAR void *priv);
static int sensor_rpmsg_suback_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									   uint32_t src, FAR void *priv);
static int sensor_rpmsg_unsub_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									  uint32_t src, FAR void *priv);
static int sensor_rpmsg_publish_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
										uint32_t src, FAR void *priv);
static int sensor_rpmsg_ioctl_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									  uint32_t src, FAR void *priv);
static int sensor_rpmsg_ioctlack_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
										 uint32_t src, FAR void *priv);
...
static const struct sensor_ops_s g_sensor_rpmsg_ops = {.open = sensor_rpmsg_open,
...
static void sensor_rpmsg_advsub_one(FAR struct sensor_rpmsg_dev_s *dev,
									FAR struct rpmsg_endpoint *ept, int command)
{
...
static void sensor_rpmsg_advsub(FAR struct sensor_rpmsg_dev_s *dev, int command)
{
	...
	list_for_every_entry(&g_eptlist, sre, struct sensor_rpmsg_ept_s, node)
	{
...
static int sensor_rpmsg_ioctl(FAR struct sensor_rpmsg_dev_s *dev, int cmd, unsigned long arg,
							  size_t len, bool wait)
{
	...
	list_for_every_entry_safe(&dev->proxylist, proxy, ptmp, struct sensor_rpmsg_proxy_s, node)
	{
		ept = proxy->ept;
...
sensor_rpmsg_alloc_proxy(FAR struct sensor_rpmsg_dev_s *dev, FAR struct rpmsg_endpoint *ept,
						 FAR struct sensor_rpmsg_advsub_s *msg)
{
	FAR struct sensor_rpmsg_proxy_s *proxy;
	...
	sensor_rpmsg_lock(dev);
	list_for_every_entry(&dev->proxylist, proxy, struct sensor_rpmsg_proxy_s, node)
	{
	...
	if (state.min_interval != UINT32_MAX) {
		sensor_rpmsg_ioctl(dev, SNIOC_SET_INTERVAL, state.min_interval, 0, false);
	}
...
static void sensor_rpmsg_push_event_persist(FAR struct sensor_rpmsg_dev_s *dev,
											FAR struct sensor_rpmsg_stub_s *stub)
{
	...
	if (ret > 0) {
		...
		rpmsg_send_nocopy(&sre->ept, msg, sizeof(*msg) + ((sizeof(*cell) + ret + 0x7) & ~0x7));
	} else {
		rpmsg_release_tx_buffer(&sre->ept, msg);
	}
}

static FAR struct sensor_rpmsg_stub_s *sensor_rpmsg_alloc_stub(FAR struct sensor_rpmsg_dev_s *dev,
															   FAR struct rpmsg_endpoint *ept,
															   uint64_t cookie)
{
	FAR struct sensor_rpmsg_stub_s *stub;
	...
	sensor_rpmsg_lock(dev);
	list_for_every_entry(&dev->stublist, stub, struct sensor_rpmsg_stub_s, node)
	{
	...
	stub->ept = ept;
	stub->cookie = cookie;
	ret = file_open(&stub->file, dev->path, O_RDOK | O_NONBLOCK | O_CLOEXEC | SENSOR_REMOTE);
	if (ret < 0) {
		kmm_free(stub);
		return NULL;
	}
...
static int sensor_rpmsg_close(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep)
{
	...
	if (filep->f_oflags & O_WROK) {
		if (dev->nadvertisers == 1) {
			...
			list_for_every_entry_safe(&dev->stublist, stub, stmp, struct sensor_rpmsg_stub_s, node)
			{
	...
	if (filep->f_oflags & O_RDOK) {
		if (dev->nsubscribers == 1) {
			...
			list_for_every_entry_safe(&dev->proxylist, proxy, ptmp, struct sensor_rpmsg_proxy_s,
									  node)
			{
...
SENSOR_RPMSG_FUNCTION(set_interval, SNIOC_SET_INTERVAL, *interval, interval, 0, false, uint32_t)
SENSOR_RPMSG_FUNCTION(batch, SNIOC_BATCH, *latency, latency, 0, false, uint32_t)
SENSOR_RPMSG_FUNCTION(selftest, SNIOC_SELFTEST, arg, arg, 0, true, unsigned long)
SENSOR_RPMSG_FUNCTION(set_calibvalue, SNIOC_SET_CALIBVALUE, arg, arg, 256, true, unsigned long)
SENSOR_RPMSG_FUNCTION(calibrate, SNIOC_CALIBRATE, arg, arg, 256, true, unsigned long)
								 ...
								 FAR struct sensor_device_info_s *info)
{
	...
	if (drv->ops->get_info) {
		...
		ret = sensor_rpmsg_ioctl(dev, SNIOC_GET_INFO, (unsigned long)(uintptr_t)info,
								...
								int cmd, unsigned long arg)
{
	...
	if (drv->ops->control) {
		...
		return sensor_rpmsg_ioctl(dev, cmd, arg, sizeof(*ioctl) + ioctl->len, true);
	}
...
static void sensor_rpmsg_push_event_one(FAR struct sensor_rpmsg_dev_s *dev,
										FAR struct sensor_rpmsg_stub_s *stub, bool flushed)
{
	FAR struct sensor_rpmsg_cell_s *cell;
	FAR struct sensor_rpmsg_ept_s *sre;
	FAR struct sensor_rpmsg_data_s *msg;
	...
	for (;;) {
		...
		if (!sre->buffer || sre->written + sizeof(*cell) + state.esize > sre->space) {
			if (sre->buffer) {
		...
		if (flushed) {
			...
			ret = file_read(&stub->file, cell->data, sre->space - sre->written - sizeof(*cell));
			if (ret <= 0) {
				break;
			}
		}
	...
	if (sre->expire <= now && sre->buffer) {
		...
		if (sre->expire == UINT64_MAX || sre->expire - now > state.interval / 2) {
...
static ssize_t sensor_rpmsg_push_event(FAR void *priv, FAR const void *data, size_t bytes)
{
	...
	list_for_every_entry_safe(&dev->stublist, stub, stmp, struct sensor_rpmsg_stub_s, node)
	{
		sensor_rpmsg_push_event_one(dev, stub, stub->flushing && bytes == 0);
	}
...
static FAR struct sensor_rpmsg_dev_s *sensor_rpmsg_find_dev(FAR const char *path)
{
									   ...
									   uint32_t src, FAR void *priv)
{
	...
	dev = sensor_rpmsg_find_dev(msg->path);
	if (dev && (!dev->nsubscribers || !sensor_rpmsg_alloc_proxy(dev, ept, msg))) {
										...
										uint32_t src, FAR void *priv)
{
	...
	while (written < len) {
		cell = (FAR struct sensor_rpmsg_cell_s *)((FAR char *)data + written);
		...
		if (cell->nbuffer > dev->lower.nbuffer) {
			...
			ret = file_open(&file, dev->path, SENSOR_REMOTE | O_CLOEXEC);
			if (ret >= 0) {
				ret = file_ioctl(&file, SNIOC_SET_BUFFER_NUMBER, cell->nbuffer);
				if (ret >= 0) {
									  ...
									  uint32_t src, FAR void *priv)
{
	...
	arg = msg->arglen > 0 ? (unsigned long)(uintptr_t)msg->argbuf : msg->arg;
										 ...
										 uint32_t src, FAR void *priv)
{
	...
	cookie = (FAR struct sensor_rpmsg_ioctl_cookie_s *)(uintptr_t)msg->cookie;
...
static int sensor_rpmsg_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
							   uint32_t src, FAR void *priv)
{
	FAR struct sensor_rpmsg_advsub_s *msg = data;

	if (msg->command < sizeof(g_sensor_rpmsg_handler) / sizeof(g_sensor_rpmsg_handler[0])) {
...
static void sensor_rpmsg_device_created(FAR struct rpmsg_device *rdev, FAR void *priv)
{
	FAR struct sensor_rpmsg_ept_s *sre;
	...
	if (rpmsg_create_ept(&sre->ept, rdev, SENSOR_RPMSG_EPT_NAME, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
						 sensor_rpmsg_ept_cb, rpmsg_destroy_ept) < 0) {
		nxrmutex_destroy(&sre->lock);
		kmm_free(sre);
	}
}
...
FAR struct sensor_lowerhalf_s *sensor_rpmsg_register(FAR struct sensor_lowerhalf_s *lower,
													 FAR const char *path)
{
	FAR struct sensor_rpmsg_ept_s *sre;
	FAR struct sensor_rpmsg_dev_s *dev;
...
{
	return rpmsg_register_callback(NULL, sensor_rpmsg_device_created, NULL, NULL, NULL);
}
