...
static ssize_t lsm9ds1_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t lsm9ds1_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int lsm9ds1_ioctl(FAR struct file *filep, int cmd, unsigned long arg);

/* Common Register Function */

static int lsm9ds1_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
							FAR const struct lsm9ds1_ops_s *ops, uint8_t datareg);
...
static ssize_t lsm9ds1_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	DEBUGASSERT(priv->datareg == LSM9DS1_OUT_X_L_G || priv->datareg == LSM9DS1_OUT_X_L_XL ||
							...
							FAR const struct lsm9ds1_ops_s *ops, uint8_t datareg)
{
	...
	DEBUGASSERT(datareg == LSM9DS1_OUT_X_L_XL || datareg == LSM9DS1_OUT_X_L_G ||
...
int lsm9ds1accel_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	/* Sanity check */

	DEBUGASSERT(addr == LSM9DS1ACCEL_ADDR0 || addr == LSM9DS1ACCEL_ADDR1);

	return lsm9ds1_register(devpath, i2c, addr, &g_lsm9ds1accel_ops, LSM9DS1_OUT_X_L_XL);
}
...
int lsm9ds1gyro_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	/* Sanity check */

	DEBUGASSERT(addr == LSM9DS1GYRO_ADDR0 || addr == LSM9DS1GYRO_ADDR1);

	return lsm9ds1_register(devpath, i2c, addr, &g_lsm9ds1gyro_ops, LSM9DS1_OUT_X_L_G);
}
...
int lsm9ds1mag_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	/* Sanity check */

	DEBUGASSERT(addr == LSM9DS1MAG_ADDR0 || addr == LSM9DS1MAG_ADDR1);

	return lsm9ds1_register(devpath, i2c, addr, &g_lsm9ds1mag_ops, LSM9DS1_OUT_X_L_M);
}
