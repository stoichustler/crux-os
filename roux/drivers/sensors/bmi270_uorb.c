...
static int bmi270_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						   bool enable);
static int bmi270_set_interval(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
							   FAR uint32_t *period_us);
#ifndef CONFIG_SENSORS_BMI270_POLL
static int bmi270_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						FAR char *buffer, size_t buflen);
#endif
static int bmi270_control(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep, int cmd,
						  unsigned long arg);

/* Helpers */

static int bmi270_accel_scale(FAR struct bmi270_sensor_s *priv, uint8_t scale);
static int bmi270_gyro_scale(FAR struct bmi270_sensor_s *priv, uint16_t scale);
						   ...
						   bool enable)
{
	...
	tmp = (dev->priv[BMI270_ACCEL_IDX].enabled + dev->priv[BMI270_GYRO_IDX].enabled);
						...
						FAR char *buffer, size_t buflen)
{
	...
	case SENSOR_TYPE_ACCELEROMETER: {
		...
		bmi270_getregs(&priv->base, BMI270_DATA_8, (FAR uint8_t *)data, 6);
	...
	case SENSOR_TYPE_GYROSCOPE: {
		...
		bmi270_getregs(&priv->base, BMI270_DATA_14, (FAR uint8_t *)data, 6);
...
static void bmi270_accel_data(FAR struct bmi270_sensor_s *priv, FAR int16_t *buf)
{
...
static void bmi270_gyro_data(FAR struct bmi270_sensor_s *priv, FAR int16_t *buf)
{
	FAR struct sensor_lowerhalf_s *lower = &priv->lower;
	struct sensor_gyro gyro;
	uint64_t now = sensor_get_timestamp();
...
static int bmi270_thread(int argc, FAR char **argv)
{
	FAR struct bmi270_sensor_dev_s *dev =
		(FAR struct bmi270_sensor_dev_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
...
int bmi270_register_uorb(int devno, FAR struct i2c_master_s *i2c, uint8_t addr)
...
#endif
{
	...
	ret = kthread_create("bmi270_thread", SCHED_PRIORITY_DEFAULT,
						 CONFIG_SENSORS_BMI270_THREAD_STACKSIZE, bmi270_thread, argv);
	if (ret < 0) {
		goto thr_err;
	}
#endif
