...
static void l3gd20_read_register(FAR struct l3gd20_dev_s *dev, uint8_t const reg_addr,
								 uint8_t *reg_data);
#endif
static void l3gd20_write_register(FAR struct l3gd20_dev_s *dev, uint8_t const reg_addr,
...
static void l3gd20_read_gyroscope_data(FAR struct l3gd20_dev_s *dev, uint16_t *x_gyr,
									   uint16_t *y_gyr, uint16_t *z_gyr);
static void l3gd20_read_temperature(FAR struct l3gd20_dev_s *dev, uint8_t *temperature);
static int l3gd20_interrupt_handler(int irq, FAR void *context, FAR void *arg);
static int l3gd20_activate(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						   bool enable);
#if CONFIG_SENSORS_L3GD20_BUFFER_SIZE > 0
static void l3gd20_worker(FAR void *arg);
#else
static int l3gd20_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
...
static const struct sensor_ops_s g_l2gd20_ops = {.activate = l3gd20_activate,
												 .set_interval = NULL,
												 .batch = NULL,
#if CONFIG_SENSORS_L3GD20_BUFFER_SIZE > 0
												 .fetch = NULL,
...
static void l3gd20_read_measurement_data(FAR struct l3gd20_dev_s *dev, FAR struct sensor_gyro *data)
{
	uint16_t x_gyr = 0;
	uint16_t y_gyr = 0;
	uint16_t z_gyr = 0;
...
static void l3gd20_read_temperature(FAR struct l3gd20_dev_s *dev, FAR uint8_t *temperature)
{
	/* Lock the SPI bus so that only one device can access it at the same
	 * time
	 */
...
static int l3gd20_interrupt_handler(int irq, FAR void *context, FAR void *arg)
{
	...
	for (priv = g_l3gd20_list; priv && priv->config->irq != irq; priv = priv->flink) {
...
static void l3gd20_worker(FAR void *arg)
{
	...
	priv->lower.push_event(priv->lower.priv, &temp, sizeof(struct sensor_gyro));
}
...
static int l3gd20_fetch(FAR struct sensor_lowerhalf_s *lower, FAR struct file *filep,
						FAR char *buffer, size_t buflen)
{
	FAR struct l3gd20_dev_s *priv = container_of(lower, FAR struct l3gd20_dev_s, lower);
						   ...
						   bool enable)
{
	...
	if (enable == true) {
		...
		l3gd20_write_register(priv, L3GD20_CTRL_REG_3, L3GD20_CTRL_REG_3_I2_DRDY_BM);
		...
		l3gd20_write_register(priv, L3GD20_CTRL_REG_4,
							  L3GD20_CTRL_REG_4_BDU_BM | L3GD20_CTRL_REG_4_FS_1_BM |
		...
		l3gd20_write_register(priv, L3GD20_CTRL_REG_1,
							  L3GD20_CTRL_REG_1_POWERDOWN_BM | L3GD20_CTRL_REG_1_X_EN_BM |
								  L3GD20_CTRL_REG_1_Y_EN_BM | L3GD20_CTRL_REG_1_Z_EN_BM);
		...
		for (reg_addr = L3GD20_CTRL_REG_1; reg_addr <= L3GD20_CTRL_REG_5; reg_addr++) {
...
int l3gd20_register(int devno, FAR struct spi_dev_s *spi, FAR struct l3gd20_config_s *config)
{
