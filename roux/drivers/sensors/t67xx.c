...
#define T67XX_I2C_DELAY_MSEC                                                                       \
	10 /* 5-10 ms recommended, 10 ms                                                               \
		* is always safe. */
...
static int t67xx_read16(FAR struct t67xx_dev_s *priv, uint16_t regaddr, FAR uint16_t *regvalue);
static int t67xx_write16(FAR struct t67xx_dev_s *priv, uint16_t regaddr, FAR uint16_t regvalue,
						 bool read_reply);

/* Driver features */

static int t67xx_check_status(FAR struct t67xx_dev_s *priv, bool *warming_up, bool *calibrating);
static int t67xx_read_fwrev(FAR struct t67xx_dev_s *priv, uint16_t *rev);
static int t67xx_read_gas_ppm(FAR struct t67xx_dev_s *priv, FAR struct t67xx_value_s *buffer);
...
static ssize_t t67xx_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t t67xx_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int t67xx_check_status(FAR struct t67xx_dev_s *priv, bool *warming_up, bool *calibrating)
{
	...
	if (status & (T67XX_STATUS_ERROR | T67XX_STATUS_FLASH_ERROR | T67XX_STATUS_CALIB_ERROR)) {
...
static int t67xx_spcal(FAR struct t67xx_dev_s *priv, bool start)
{
	...
	ret = t67xx_write16(priv, T67XX_REG_SPCAL, start ? SPCAL_START : SPCAL_STOP, true);
	if (ret < 0) {
...
static int t67xx_abclogic(FAR struct t67xx_dev_s *priv, bool enable)
{
	...
	ret =
		t67xx_write16(priv, T67XX_REG_ABCLOGIC, enable ? ABCLOGIC_ENABLE : ABCLOGIC_DISABLE, true);
	if (ret < 0) {
...
int t67xx_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
