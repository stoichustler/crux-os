...
static int lis2dh_access(FAR struct lis2dh_dev_s *dev, uint8_t subaddr, FAR uint8_t *buf,
						 int length);
static int lis2dh_get_reading(FAR struct lis2dh_dev_s *dev, FAR struct lis2dh_vector_s *res,
							  bool force_read);
...
static int lis2dh_setup(FAR struct lis2dh_dev_s *dev, FAR struct lis2dh_setup *new_setup);
...
static FAR const struct lis2dh_vector_s *lis2dh_get_raw_readings(FAR struct lis2dh_dev_s *dev,
																 FAR int *err);
#endif
...
static int lis2dh_fifo_start(FAR struct lis2dh_dev_s *priv)
{
	...
	buf = 0x00 | priv->setup->trigger_selection | priv->setup->fifo_trigger_threshold;
...
static ssize_t lis2dh_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	} else /* FIFO modes */
	{
		...
		do {
			...
			ptr->header.meas_count += lis2dh_get_fifo_readings(priv, ptr, fifo_num_samples, &ret);
	...
	return sizeof(ptr->header) + ptr->header.meas_count * sizeof(struct lis2dh_vector_s);
}
...
static int lis2dh_handle_selftest(FAR struct lis2dh_dev_s *priv)
{
	...
	for (i = SELFTEST_0; i <= SELFTEST_1; i++) {
		...
		if (abs_st_x_value < SELFTEST_ABS_DIFF_MIN_12BIT ||
			...
			syslog(LOG_NOTICE,
				   "Selftest %d fail! Limits (%d <= value <= %d). "
				   "Results: x: %d, y: %d, z: %d ",
				   i, SELFTEST_ABS_DIFF_MIN_12BIT, SELFTEST_ABS_DIFF_MAX_12BIT, abs_st_x_value,
				   abs_st_y_value, abs_st_z_value);
			ret = -ERANGE;
			goto out;
		}
	...
	*err = 0;
	...
	while (--retries_left > 0) {
		...
		if (lis2dh_data_available(dev)) {
			if (lis2dh_access(dev, ST_LIS2DH_OUT_X_L_REG, retval, sizeof(retval)) ==
				sizeof(retval)) {
							  ...
							  bool force_read)
{
	...
	if (lis2dh_access(dev, ST_LIS2DH_STATUS_REG, retval, 7) == 7) {
		...
		add_sensor_randomness((((uint32_t)retval[6] << 25) | ((uint32_t)retval[6] >> 7)) ^
							  ((uint32_t)retval[5] << 20) ^ ((uint32_t)retval[4] << 15) ^
							  ((uint32_t)retval[3] << 10) ^ ((uint32_t)retval[2] << 5) ^
							  ((uint32_t)retval[1] << 0));
											 ...
											 FAR int *perr)
{
	...
	if (lis2dh_access(priv, ST_LIS2DH_OUT_X_L_REG, (FAR void *)buf, buflen) != buflen) {
						 ...
						 int length)
{
	...
	for (retries = 0; retries < LIS2DH_I2C_RETRIES; retries++) {
		...
		struct i2c_msg_s msgv[2] = {{.frequency = CONFIG_LIS2DH_I2C_FREQUENCY,
									 .addr = dev->addr,
									 .flags = 0,
									 .buffer = &subaddr,
									 .length = 1},
...
static int lis2dh_setup(FAR struct lis2dh_dev_s *dev, FAR struct lis2dh_setup *new_setup)
{
	...
	value = dev->setup->hpmode | dev->setup->hpcf | dev->setup->fds | dev->setup->hpclick |
			dev->setup->hpis2 | dev->setup->hpis1;
	...
	value = dev->setup->bdu | dev->setup->endian | dev->setup->fullscale |
			dev->setup->high_resolution_enable | dev->setup->selftest | dev->setup->spi_mode;
	...
	value = dev->setup->reboot | dev->setup->fifo_enable | dev->setup->int1_latch |
			dev->setup->int1_4d_enable | dev->setup->int2_latch | dev->setup->int2_4d_enable;
	if (lis2dh_access(dev, ST_LIS2DH_CTRL_REG5, &value, -1) != 1) {
		goto error;
	}
	...
	value = dev->setup->int2_click_enable | dev->setup->int_enable | dev->setup->boot_int1_enable |
			dev->setup->high_low_active;
	...
	value =
		dev->setup->fifo_mode | dev->setup->trigger_selection | dev->setup->fifo_trigger_threshold;
	...
	value = dev->setup->int1_interrupt_mode | dev->setup->int1_enable_6d |
			dev->setup->int1_int_z_high_enable | dev->setup->int1_int_z_low_enable |
			dev->setup->int1_int_y_high_enable | dev->setup->int1_int_y_low_enable |
			dev->setup->int1_int_x_high_enable | dev->setup->int1_int_x_low_enable;
	...
	value = dev->setup->int2_interrupt_mode | dev->setup->int2_enable_6d |
			dev->setup->int2_int_z_high_enable | dev->setup->int2_int_z_low_enable |
			dev->setup->int2_int_y_high_enable | dev->setup->int2_int_y_low_enable |
			dev->setup->int2_int_x_high_enable | dev->setup->int2_int_x_low_enable;
	...
	value = dev->setup->data_rate | dev->setup->low_power_mode_enable | dev->setup->zen |
			dev->setup->yen | dev->setup->xen;
...
int lis2dh_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
					FAR struct lis2dh_config_s *config)
{
	FAR struct lis2dh_dev_s *priv;
	int ret;
