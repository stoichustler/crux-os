...
#include <roux/spi/spi_transfer.h>
...
static int regmap_spi_write(FAR struct regmap_bus_s *bus, FAR const void *data, unsigned int count);

static int regmap_spi_read(FAR struct regmap_bus_s *bus, FAR const void *reg, unsigned int reg_size,
						   FAR void *val, unsigned int val_size);

static int regmap_spi_reg_write(FAR struct regmap_bus_s *bus, unsigned int regaddr,
								unsigned int value);

static int regmap_spi_reg_read(FAR struct regmap_bus_s *bus, unsigned int regaddr, FAR void *value);
...
FAR struct regmap_s *regmap_init_spi(FAR struct spi_dev_s *spi, uint32_t freq, uint32_t devid,
									 enum spi_mode_e mode, FAR const struct regmap_config_s *config)
{
