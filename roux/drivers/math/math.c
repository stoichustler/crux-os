...
static ssize_t math_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t math_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int math_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case MATHIOC_CORDIC_CALC: {
		FAR struct cordic_calc_s *calc = (FAR struct cordic_calc_s *)((uintptr_t)arg);
	...
	case MATHIOC_FFT_CALC: {
		FAR struct fft_calc_s *calc = (FAR struct fft_calc_s *)((uintptr_t)arg);
	...
	case MATHIOC_MPI_CALC: {
		FAR struct mpi_calc_s *calc = (FAR struct mpi_calc_s *)((uintptr_t)arg);
...
int math_register(FAR const char *path, FAR const struct math_config_s *config)
{
	...
	upper = (FAR math_upperhalf_s *)kmm_malloc(sizeof(math_upperhalf_s));
	if (!upper) {
...
#ifdef CONFIG_MATH_CORDIC
int cordic_register(FAR const char *path, FAR struct cordic_lowerhalf_s *lower)
{
...
#ifdef CONFIG_MATH_FFT
int fft_register(FAR const char *path, FAR struct fft_lowerhalf_s *lower)
{
	struct math_config_s config;
...
#ifdef CONFIG_MATH_MPI
int mpi_register(FAR const char *path, FAR struct mpi_lowerhalf_s *lower)
{
	struct math_config_s config;
