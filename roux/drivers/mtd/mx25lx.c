...
#define SET_VALID(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= MX25L_CACHE_VALID;                                                           \
	} while (0)
#define SET_DIRTY(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= MX25L_CACHE_DIRTY;                                                           \
	} while (0)
#define SET_ERASED(p)                                                                              \
	do {                                                                                           \
		(p)->flags |= MX25L_CACHE_ERASED;                                                          \
	} while (0)
...
static void mx25l_byteread(FAR struct mx25l_dev_s *priv, FAR uint8_t *buffer, off_t address,
						   size_t nbytes);
static inline void mx25l_pagewrite(FAR struct mx25l_dev_s *priv, FAR const uint8_t *buffer,
...
static void mx25l_cachewrite(FAR struct mx25l_dev_s *priv, FAR const uint8_t *buffer, off_t sector,
							 size_t nsectors);
#endif
...
static ssize_t mx25l_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
						   FAR uint8_t *buf);
static ssize_t mx25l_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
...
#else
	nbytes = mx25l_read(dev, startblock << priv->pageshift, nblocks << priv->pageshift, buffer);
	if (nbytes > 0) {
...
#else
	mx25l_pagewrite(priv, buffer, startblock << priv->pageshift, nblocks << priv->pageshift);
#endif
	mx25l_unlock(priv->dev);
...
static int mx25l_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
#ifdef CONFIG_MX25L_SECTOR512
			info->numsectors = priv->nsectors << (priv->sectorshift - MX25L_SECTOR512_SHIFT);
