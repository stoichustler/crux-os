...
static int at25ee_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks);
static ssize_t at25ee_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							FAR uint8_t *buf);
static ssize_t at25ee_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							 FAR const uint8_t *buf);
static ssize_t at25ee_read(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
						   FAR uint8_t *buf);
static ssize_t at25ee_write(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
							FAR const uint8_t *buf);
static int at25ee_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg);
...
static void at25ee_sendcmd(FAR struct spi_dev_s *spi, uint8_t cmd, uint8_t addrlen, uint32_t addr);
...
static void at25ee_lock(FAR struct spi_dev_s *dev)
{
	...
	SPI_SETDELAY(dev, CONFIG_AT25EE_START_DELAY, CONFIG_AT25EE_STOP_DELAY, CONFIG_AT25EE_CS_DELAY,
				 CONFIG_AT25EE_IFDELAY);
#endif
}
...
static ssize_t at25ee_read(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes, FAR uint8_t *buf)
{
	int ret;
	FAR struct at25ee_dev_s *priv = (FAR struct at25ee_dev_s *)dev;
...
FAR struct mtd_dev_s *at25ee_initialize(FAR struct spi_dev_s *dev, int devtype, int readonly)
{
	...
	if ((devtype < 0) || (devtype >= sizeof(g_at25ee_devices) / sizeof(g_at25ee_devices[0]))) {
		return NULL;
	}
	...
	finfo("EEPROM device, %" PRIu32 " bytes, "
		  ...
		  priv->size, priv->pgsize, priv->addrlen, priv->readonly);
