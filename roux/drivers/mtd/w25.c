...
#define SET_VALID(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= W25_CACHE_VALID;                                                             \
	} while (0)
#define SET_DIRTY(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= W25_CACHE_DIRTY;                                                             \
	} while (0)
#define SET_ERASED(p)                                                                              \
	do {                                                                                           \
		(p)->flags |= W25_CACHE_ERASED;                                                            \
	} while (0)
...
static void w25_byteread(FAR struct w25_dev_s *priv, FAR uint8_t *buffer, off_t address,
						 size_t nbytes);
#ifndef CONFIG_W25_READONLY
static void w25_pagewrite(FAR struct w25_dev_s *priv, FAR const uint8_t *buffer, off_t address,
						  size_t nbytes);
#endif
...
static void w25_cachewrite(FAR struct w25_dev_s *priv, FAR const uint8_t *buffer, off_t sector,
						   size_t nsectors);
#endif
...
static inline int w25_readid(struct w25_dev_s *priv)
{
	...
	if ((manufacturer == W25_JEDEC_WINBOND || manufacturer == W25_JEDEC_AMIC) &&
		(memory == W25X_JEDEC_MEMORY_TYPE || memory == W25Q_JEDEC_MEMORY_TYPE_A ||
		 memory == W25Q_JEDEC_MEMORY_TYPE_B || memory == W25Q_JEDEC_MEMORY_TYPE_C ||
...
#ifndef CONFIG_W25_READONLY
static void w25_pagewrite(struct w25_dev_s *priv, FAR const uint8_t *buffer, off_t address,
						  size_t nbytes)
{
	uint8_t status;
...
#if defined(CONFIG_MTD_BYTE_WRITE) && !defined(CONFIG_W25_READONLY)
static inline void w25_bytewrite(struct w25_dev_s *priv, FAR const uint8_t *buffer, off_t offset,
								 uint16_t count)
{
...
#ifdef CONFIG_W25_SECTOR512
	nbytes =
		w25_read(dev, startblock << W25_SECTOR512_SHIFT, nblocks << W25_SECTOR512_SHIFT, buffer);
	if (nbytes > 0) {
...
static int w25_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case MTDIOC_GEOMETRY: {
		...
		if (geo) {
			...
			w25_finfo("blocksize: %" PRIu32 " erasesize: %" PRIu32 " neraseblocks: %" PRIu32 "\n",
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
#ifdef CONFIG_W25_SECTOR512
			info->numsectors = priv->nsectors << (W25_SECTOR_SHIFT - W25_SECTOR512_SHIFT);
			info->sectorsize = 1 << W25_SECTOR512_SHIFT;
#else
			info->numsectors = priv->nsectors * W25_SECTOR_SIZE / W25_PAGE_SIZE;
