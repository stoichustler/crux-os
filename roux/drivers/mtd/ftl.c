...
#include <roux/drivers/rwbuffer.h>
...
static ssize_t ftl_read(FAR struct inode *inode, FAR unsigned char *buffer, blkcnt_t start_sector,
						unsigned int nsectors);
static ssize_t ftl_flush(FAR void *priv, FAR const uint8_t *buffer, off_t startblock,
						 size_t nblocks);
static ssize_t ftl_flush_direct(FAR struct ftl_struct_s *dev, FAR const uint8_t *buffer,
								off_t startblock, size_t nblocks);
static ssize_t ftl_write(FAR struct inode *inode, FAR const unsigned char *buffer,
						 blkcnt_t start_sector, unsigned int nsectors);
static int ftl_geometry(FAR struct inode *inode, FAR struct geometry *geometry);
...
static size_t ftl_get_cblock(FAR struct ftl_struct_s *dev, off_t start, size_t count)
{
	off_t i;
...
static ssize_t ftl_mtd_bread(FAR struct ftl_struct_s *dev, off_t startblock, size_t nblocks,
							 FAR uint8_t *buffer)
{
	...
	if (dev->lptable == NULL) {
		...
		if (ret != nblocks) {
			ferr("ERROR: Read %zu blocks starting at block %" PRIdOFF " failed: %zd\n", nblocks,
				 startblock, ret);
		}
	...
	while (nblocks > 0) {
		...
		count = ftl_get_cblock(dev, starteraseblock, (offset + nblocks + mask) / dev->blkper);
		count = MIN(count * dev->blkper - offset, nblocks);
		startphysicalblock = dev->lptable[starteraseblock] * dev->blkper + offset;
							  ...
							  FAR const uint8_t *buffer)
{
	...
	while (1) {
		...
		ret =
			MTD_BWRITE(dev->mtd, dev->lptable[starteraseblock] * dev->blkper, dev->blkper, buffer);
		if (ret == dev->blkper) {
			return ret;
		}
...
static ssize_t ftl_read(FAR struct inode *inode, unsigned char *buffer, blkcnt_t start_sector,
						unsigned int nsectors)
{
	FAR struct ftl_struct_s *dev;
								...
								off_t startblock, size_t nblocks)
{
	...
	while (nblocks) {
		...
		if (dev->lptable == NULL) {
			...
			ret = MTD_BWRITE(dev->mtd, dev->lptable[starteraseblock] * dev->blkper + offset, count,
							 buffer);
			if (ret != count) {
				MTD_MARKBAD(dev->mtd, dev->lptable[starteraseblock]);
				ftl_update_map(dev, starteraseblock);
						 ...
						 size_t nblocks)
{
	...
	if (alignedblock > startblock) {
		...
		finfo("Copy %d bytes into erase block=%" PRIdOFF " at offset=%" PRIdOFF "\n", nbytes,
			  eraseblock, offset);
	...
	while (remaining >= dev->blkper) {
		...
		finfo("Write %" PRId32 " bytes into erase block=%" PRIdOFF " at offset=0\n",
