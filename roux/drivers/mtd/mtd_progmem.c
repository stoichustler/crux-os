...
static ssize_t progmem_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							 FAR uint8_t *buf);
static ssize_t progmem_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							  FAR const uint8_t *buf);
static ssize_t progmem_read(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
							FAR uint8_t *buffer);
#ifdef CONFIG_MTD_BYTE_WRITE
static ssize_t progmem_write(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
							...
							FAR uint8_t *buffer)
{
	...
	result = up_progmem_read(
		up_progmem_getaddress(startblock) + (offset & ((1 << priv->blkshift) - 1)), buffer, nbytes);
...
static int progmem_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
			info->numsectors = up_progmem_neraseblocks() << (priv->ersshift - priv->blkshift);
