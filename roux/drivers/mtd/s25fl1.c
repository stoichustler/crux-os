...
#define S25FL1_READ_STATUS1                                                                        \
	0x05 /* Read status register 1:         *                                                      \
		  * 0x05 | SR1                      */
#define S25FL1_READ_STATUS2                                                                        \
	0x35 /* Read status register 2:         *                                                      \
		  * 0x35 | SR2                      */
#define S25FL1_READ_STATUS3                                                                        \
	0x33 /* Read status register 3:         *                                                      \
		  * 0x33 | SR3                      */
#define S25FL1_WRITE_ENABLE                                                                        \
	0x06 /* Write enable:                   *                                                      \
		  * 0x06                            */
#define S25FL1_VWRITE_ENABLE                                                                       \
	0x50 /* Write enable for volatile status: *                                                    \
		  * 0x50                            */
#define S25FL1_WRITE_DISABLE                                                                       \
	0x04 /* Write disable command code:     *                                                      \
		  * 0x04                            */
#define S25FL1_WRITE_STATUS                                                                        \
	0x01 /* Write status register:          *                                                      \
		  * 0x01 | SR1 | SR2 | SR3          */
#define S25FL1_WRAP_ENABLE                                                                         \
	0x77 /* Set Burst with Wrap:            *                                                      \
		  * 0x77 | xx | xx | xx | SR3       */
#define S25FL1_UNPROTECT_SECTOR                                                                    \
	0x39 /* Set Block / Pointer Protection: *                                                      \
		  * 0x39 | ADDR(MS) | ADDR(MID) | xx */
#define S25FL1_PAGE_PROGRAM                                                                        \
	0x02 /* Page Program:                   *                                                      \
		  * 0x02 | ADDR(MS) | ADDR(MID) |   *                                                      \
		  * ADDR(LS) | data                 */
#define S25FL1_SECTOR_ERASE                                                                        \
	0x20 /* Sector Erase (4 kB)             *                                                      \
		  * 0x02 | ADDR(MS) | ADDR(MID) |   *                                                      \
		  * ADDR(LS)                        */
#define S25FL1_BLOCK_ERASE                                                                         \
	0xd8 /* Block Erase (64 kB):            *                                                      \
		  * 0x02 | ADDR(MS) | ADDR(MID) |   *                                                      \
		  * ADDR(LS)                        */
#define S25FL1_CHIP_ERASE_1                                                                        \
	0x60 /* Chip Erase 1:                   *                                                      \
		  * 0x60                            */
#define S25FL1_CHIP_ERASE_2                                                                        \
	0xc7 /* Chip Erase 2:                   *                                                      \
		  * 0xc7                            */
#define S25FL1_ERASE_PROG_SUSPEND                                                                  \
	0x75 /* Erase / Program Suspend:        *                                                      \
		  * 0x75                            */
#define S25FL1_ERASE_PROG_RESUME                                                                   \
...
#define S25FL1_SOFT_RESET_ENABLE                                                                   \
	0x66 /* Software Reset Enable:          *                                                      \
		  *   0x66                          */
#define S25FL1_SOFT_RESET                                                                          \
	0x99 /* Software Reset:                 *                                                      \
		  *   0x99                          */
...
#define S25FL1_DEEP_PWRDOWN                                                                        \
	0xb9 /* Deep Power-down:                *                                                      \
		  *   0xb9                          */
#define S25FL1_RELEASE_PWRDOWN                                                                     \
	0xab /* Release Power down / Device ID: *                                                      \
		  *   0xab | dummy | dummy | dummy | *                                                     \
		  *   DeviceID  */
#define S25FL1_MANUFACTURER                                                                        \
	0x90 /* Manufacturer / Device ID:       *                                                      \
		  *   0x90 | dummy | dummy | 0x00 | *                                                      \
		  *   Manufacturer | DeviceID       */
#define S25FL1_JEDEC_ID                                                                            \
	0x9f /* JEDEC ID:                       *                                                      \
		  *   0x9f | Manufacturer | MemoryType | *                                                 \
		  *   Capacity                      */
#define S25FL1_READ_SFDP                                                                           \
	0x5a /* Read SFDP Register / Read Unique ID  *                                                 \
		  * Number:                          *                                                     \
		  *   0x5a | 0x00 | 0x00 | ADDR | dummy | *                                                \
		  *   data...                        */
#define S25FL1_READ_SECURITY                                                                       \
	0x48 /* Read Security Registers:         *                                                     \
		  *   0x48 | ADDR(MS) | ADDR(MID) |  *                                                     \
		  *   ADDR(LS) | dummy | data...     */
#define S25FL1_ERASE_SECURITY                                                                      \
	0x44 /* Erase Security Registers:        *                                                     \
		  *   0x48 | ADDR(MS) | ADDR(MID) |  *                                                     \
		  *   ADDR(LS)                       */
#define S25FL1_PROgRAM_SECURITY                                                                    \
...
static int s25fl1_command_address(FAR struct qspi_dev_s *qspi, uint8_t cmd, off_t addr,
								  uint8_t addrlen);
static int s25fl1_command_read(FAR struct qspi_dev_s *qspi, uint8_t cmd, FAR void *buffer,
							   size_t buflen);
static int s25fl1_command_write(FAR struct qspi_dev_s *qspi, uint8_t cmd, FAR const void *buffer,
								size_t buflen);
...
static int s25fl1_protect(FAR struct s25fl1_dev_s *priv, off_t startblock, size_t nblocks);
static int s25fl1_unprotect(FAR struct s25fl1_dev_s *priv, off_t startblock, size_t nblocks);
static bool s25fl1_isprotected(FAR struct s25fl1_dev_s *priv, uint8_t status, off_t address);
...
static int s25fl1_read_byte(FAR struct s25fl1_dev_s *priv, FAR uint8_t *buffer, off_t address,
							size_t nbytes);
static int s25fl1_write_page(FAR struct s25fl1_dev_s *priv, FAR const uint8_t *buffer,
							 off_t address, size_t nbytes);
#ifdef CONFIG_S25FL1_SECTOR512
static int s25fl1_flush_cache(FAR struct s25fl1_dev_s *priv);
static FAR uint8_t *s25fl1_read_cache(FAR struct s25fl1_dev_s *priv, off_t sector);
static void s25fl1_erase_cache(FAR struct s25fl1_dev_s *priv, off_t sector);
static int s25fl1_write_cache(FAR struct s25fl1_dev_s *priv, FAR const uint8_t *buffer,
...
static ssize_t s25fl1_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							FAR uint8_t *buf);
static ssize_t s25fl1_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
...
{
	DEBUGVERIFY(
		s25fl1_command_read(priv->qspi, S25FL1_READ_STATUS1, (FAR void *)&priv->readbuf[0], 1));
...
{
	DEBUGVERIFY(
		s25fl1_command_read(priv->qspi, S25FL1_READ_STATUS2, (FAR void *)&priv->readbuf[0], 1));
	return priv->readbuf[0];
}
...
{
	DEBUGVERIFY(
		s25fl1_command_read(priv->qspi, S25FL1_READ_STATUS3, (FAR void *)&priv->readbuf[0], 1));
	return priv->readbuf[0];
}
...
static void s25fl1_write_status(FAR struct s25fl1_dev_s *priv)
{
	...
	s25fl1_command_write(priv->qspi, S25FL1_WRITE_STATUS, (FAR const void *)priv->cmdbuf, 3);
...
static inline int s25fl1_readid(FAR struct s25fl1_dev_s *priv)
{
	...
	finfo("Manufacturer: %02x Device Type %02x, Capacity: %02x", priv->cmdbuf[0], priv->cmdbuf[1],
		  priv->cmdbuf[2]);
...
static int s25fl1_erase_sector(FAR struct s25fl1_dev_s *priv, off_t sector)
{
	...
	if ((status & STATUS1_BP_MASK) != 0 && s25fl1_isprotected(priv, status, address)) {
...
static FAR uint8_t *s25fl1_read_cache(FAR struct s25fl1_dev_s *priv, off_t sector)
{
	...
	if (!IS_VALID(priv) || esectno != priv->esectno) {
		...
		ret = s25fl1_read_byte(priv, priv->sector, (esectno << priv->sectorshift),
							   (1 << priv->sectorshift));
		if (ret < 0) {
...
#else
	nbytes = s25fl1_read(dev, startblock << priv->pageshift, nblocks << priv->pageshift, buffer);
	if (nbytes > 0) {
...
#else
	ret =
		s25fl1_write_page(priv, buffer, startblock << priv->pageshift, nblocks << priv->pageshift);
	if (ret < 0) {
		ferr("ERROR: s25fl1_write_page failed: %d\n", ret);
	}
#endif
...
static int s25fl1_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
#ifdef CONFIG_S25FL1_SECTOR512
			info->numsectors = priv->nsectors << (priv->sectorshift - S25FL1_SECTOR512_SHIFT);
