...
#define SET_VALID(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= MX25RXX_CACHE_VALID;                                                         \
	} while (0)
#define SET_DIRTY(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= MX25RXX_CACHE_DIRTY;                                                         \
	} while (0)
#define SET_ERASED(p)                                                                              \
	do {                                                                                           \
		(p)->flags |= MX25RXX_CACHE_ERASED;                                                        \
	} while (0)
...
static int mx25rxx_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks);
static ssize_t mx25rxx_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							 FAR uint8_t *buf);
static ssize_t mx25rxx_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							  FAR const uint8_t *buf);
static ssize_t mx25rxx_read(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
							FAR uint8_t *buffer);
...
static int mx25rxx_command_read(FAR struct qspi_dev_s *qspi, uint8_t cmd, FAR void *buffer,
								size_t buflen);
static int mx25rxx_command_write(FAR struct qspi_dev_s *qspi, uint8_t cmd, FAR const void *buffer,
								 size_t buflen);
static int mx25rxx_command(FAR struct qspi_dev_s *qspi, uint8_t cmd);
static int mx25rxx_command_address(FAR struct qspi_dev_s *qspi, uint8_t cmd, off_t addr,
								   uint8_t addrlen);

static int mx25rxx_readid(FAR struct mx25rxx_dev_s *dev);
static int mx25rxx_read_byte(FAR struct mx25rxx_dev_s *dev, FAR uint8_t *buffer, off_t address,
							 size_t buflen);
...
static void mx25rxx_write_status_config(FAR struct mx25rxx_dev_s *dev, uint8_t status,
										uint16_t config);
static void mx25rxx_write_enable(FAR struct mx25rxx_dev_s *dev, bool enable);

static int mx25rxx_write_page(FAR struct mx25rxx_dev_s *priv, FAR const uint8_t *buffer,
							  off_t address, size_t buflen);
static int mx25rxx_erase_sector(FAR struct mx25rxx_dev_s *priv, off_t sector);
#if 0 /* FIXME:  Not used */
...
static FAR uint8_t *mx25rxx_read_cache(FAR struct mx25rxx_dev_s *priv, off_t sector);
static void mx25rxx_erase_cache(FAR struct mx25rxx_dev_s *priv, off_t sector);
static int mx25rxx_write_cache(FAR struct mx25rxx_dev_s *priv, FAR const uint8_t *buffer,
...
int mx25rxx_command_read(FAR struct qspi_dev_s *qspi, uint8_t cmd, FAR void *buffer, size_t buflen)
{
...
int mx25rxx_command_write(FAR struct qspi_dev_s *qspi, uint8_t cmd, FAR const void *buffer,
						  size_t buflen)
{
	struct qspi_cmdinfo_s cmdinfo;
...
int mx25rxx_command_address(FAR struct qspi_dev_s *qspi, uint8_t cmd, off_t addr, uint8_t addrlen)
{
	struct qspi_cmdinfo_s cmdinfo;
...
int mx25rxx_read_byte(FAR struct mx25rxx_dev_s *dev, FAR uint8_t *buffer, off_t address,
					  size_t buflen)
{
...
int mx25rxx_write_page(FAR struct mx25rxx_dev_s *priv, FAR const uint8_t *buffer, off_t address,
					   size_t buflen)
{
	struct qspi_meminfo_s meminfo;
...
void mx25rxx_write_status_config(FAR struct mx25rxx_dev_s *dev, uint8_t status, uint16_t config)
{
...
ssize_t mx25rxx_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks, FAR uint8_t *buf)
{
...
#else
	nbytes = mx25rxx_read(dev, startblock << priv->pageshift, nblocks << priv->pageshift, buf);
	if (nbytes > 0) {
...
ssize_t mx25rxx_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
					   FAR const uint8_t *buf)
{
	FAR struct mx25rxx_dev_s *priv = (FAR struct mx25rxx_dev_s *)dev;
	int ret;
...
#else
	ret = mx25rxx_write_page(priv, buf, startblock << priv->pageshift, nblocks << priv->pageshift);
	if (ret < 0) {
...
ssize_t mx25rxx_read(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes, FAR uint8_t *buffer)
{
	int ret;
	FAR struct mx25rxx_dev_s *priv = (FAR struct mx25rxx_dev_s *)dev;
...
int mx25rxx_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case MTDIOC_GEOMETRY: {
		...
		if (geo) {
			...
			geo->neraseblocks = priv->nsectors << (priv->sectorshift - MX25RXX_SECTOR512_SHIFT);
#else
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
#ifdef CONFIG_MX25RXX_SECTOR512
			info->numsectors = priv->nsectors << (priv->sectorshift - MX25RXX_SECTOR512_SHIFT);
...
int mx25rxx_readid(FAR struct mx25rxx_dev_s *dev)
{
	...
	if (dev->cmdbuf[0] != MX25R_JEDEC_MANUFACTURER || dev->cmdbuf[1] != MX25R_JEDEC_MEMORY_TYPE) {
...
static FAR uint8_t *mx25rxx_read_cache(FAR struct mx25rxx_dev_s *priv, off_t sector)
{
	...
	if (!IS_VALID(priv) || esectno != priv->esectno) {
		...
		ret = mx25rxx_read_byte(priv, priv->sector, (esectno << priv->sectorshift),
								(1 << priv->sectorshift));
		if (ret < 0) {
							   ...
							   off_t sector, size_t nsectors)
{
	...
	for (; nsectors > 0; nsectors--) {
		...
		finfo("address: %08jx nbytes: %d 0x%04" PRIx32 "\n",
			  (intmax_t)(sector << MX25RXX_SECTOR512_SHIFT), MX25RXX_SECTOR512_SIZE,
			  *(FAR uint32_t *)buffer);
