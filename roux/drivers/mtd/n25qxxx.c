...
#define N25QXXX_READ_STATUS                                                                        \
	0x05 /* Read status register:              *                                                   \
		  *   0x05 | SR                        */
#define N25QXXX_WRITE_STATUS                                                                       \
	0x01 /* Write status register:             *                                                   \
		  *   0x01 | SR                        */
#define N25QXXX_READ_VOLCFG                                                                        \
	0x85 /* Read volatile configuration register:   *                                              \
		  *   0x85 | VCR                       */
#define N25QXXX_WRITE_VOLCFG                                                                       \
	0x81 /* Write svolatile configuration register: *                                              \
		  *   0x81 | VCR                       */
...
/*                                            Data sequence                 */
#define N25QXXX_FAST_READ_QUADIO                                                                   \
...
static int n25qxxx_command_address(FAR struct qspi_dev_s *qspi, uint8_t cmd, off_t addr,
								   uint8_t addrlen);
static int n25qxxx_command_read(FAR struct qspi_dev_s *qspi, uint8_t cmd, FAR void *buffer,
								size_t buflen);
static int n25qxxx_command_write(FAR struct qspi_dev_s *qspi, uint8_t cmd, FAR const void *buffer,
								 size_t buflen);
...
static int n25qxxx_protect(FAR struct n25qxxx_dev_s *priv, off_t startblock, size_t nblocks);
static int n25qxxx_unprotect(FAR struct n25qxxx_dev_s *priv, off_t startblock, size_t nblocks);
static bool n25qxxx_isprotected(FAR struct n25qxxx_dev_s *priv, uint8_t status, off_t address);
...
static int n25qxxx_read_byte(FAR struct n25qxxx_dev_s *priv, FAR uint8_t *buffer, off_t address,
							 size_t nbytes);
static int n25qxxx_write_page(FAR struct n25qxxx_dev_s *priv, FAR const uint8_t *buffer,
							  off_t address, size_t nbytes);
#ifdef CONFIG_N25QXXX_SECTOR512
static int n25qxxx_flush_cache(FAR struct n25qxxx_dev_s *priv);
static FAR uint8_t *n25qxxx_read_cache(FAR struct n25qxxx_dev_s *priv, off_t sector);
static void n25qxxx_erase_cache(FAR struct n25qxxx_dev_s *priv, off_t sector);
static int n25qxxx_write_cache(FAR struct n25qxxx_dev_s *priv, FAR const uint8_t *buffer,
...
static ssize_t n25qxxx_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							 FAR uint8_t *buf);
static ssize_t n25qxxx_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
...
{
	DEBUGVERIFY(
		n25qxxx_command_read(priv->qspi, N25QXXX_READ_STATUS, (FAR void *)&priv->readbuf[0], 1));
...
static void n25qxxx_write_status(FAR struct n25qxxx_dev_s *priv)
{
	...
	n25qxxx_command_write(priv->qspi, N25QXXX_WRITE_STATUS, (FAR const void *)priv->cmdbuf, 1);
...
{
	DEBUGVERIFY(
		n25qxxx_command_read(priv->qspi, N25QXXX_READ_VOLCFG, (FAR void *)&priv->readbuf[0], 1));
	return priv->readbuf[0];
}
...
{
	n25qxxx_write_enable(priv);
	n25qxxx_command_write(priv->qspi, N25QXXX_WRITE_VOLCFG, (FAR const void *)priv->cmdbuf, 1);
	n25qxxx_write_disable(priv);
}
...
static FAR uint8_t *n25qxxx_read_cache(FAR struct n25qxxx_dev_s *priv, off_t sector)
{
	...
	if (!IS_VALID(priv) || esectno != priv->esectno) {
		...
		ret = n25qxxx_read_byte(priv, priv->sector, (esectno << priv->sectorshift),
								(1 << priv->sectorshift));
		if (ret < 0) {
...
#else
	nbytes = n25qxxx_read(dev, startblock << priv->pageshift, nblocks << priv->pageshift, buffer);
	if (nbytes > 0) {
...
#else
	ret =
		n25qxxx_write_page(priv, buffer, startblock << priv->pageshift, nblocks << priv->pageshift);
	if (ret < 0) {
		ferr("ERROR: n25qxxx_write_page failed: %d\n", ret);
	}
#endif
...
static int n25qxxx_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
#ifdef CONFIG_N25QXXX_SECTOR512
			info->numsectors = priv->nsectors << (priv->sectorshift - N25QXXX_SECTOR512_SHIFT);
