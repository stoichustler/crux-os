...
#include <roux/mtd/nand_wrapper.h>
...
int nand_wrapper_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks)
{
	int ret;
	...
	nand_wrapper_ins_i++;
	NAND_WRAPPER_LOG("[UPPER %" PRIu64 " | %s] Startblock: %" PRIi32 ", N Blocks: %zu\n",
...
ssize_t nand_wrapper_bread(FAR struct mtd_dev_s *dev, off_t startpage, size_t npages,
						   FAR uint8_t *buffer)
{
	int ret;
	FAR struct nand_wrapper_dev_s *nand_dev;
...
ssize_t nand_wrapper_bwrite(FAR struct mtd_dev_s *dev, off_t startpage, size_t npages,
...
int nand_wrapper_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	int ret;
	FAR struct nand_wrapper_dev_s *nand_dev;
	...
	nxmutex_lock(&nand_wrapper_dev_mut);
	nand_wrapper_ins_i++;
	NAND_WRAPPER_LOG("[UPPER %" PRIu64 " | %s] Command: %" PRIi32 ", Arg : %zu\n",
