...
static void gd25_byteread(FAR struct gd25_dev_s *priv, FAR uint8_t *buffer, off_t address,
						  size_t nbytes);
#ifndef CONFIG_GD25_READONLY
static void gd25_pagewrite(FAR struct gd25_dev_s *priv, FAR const uint8_t *buffer, off_t address,
						   size_t nbytes);
#endif
...
static int gd25_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks);
static ssize_t gd25_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
						  FAR uint8_t *buf);
static ssize_t gd25_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
...
static ssize_t gd25_write(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
...
static void gd25_lock(FAR struct spi_dev_s *spi)
{
	...
	SPI_SETDELAY(spi, CONFIG_GD25_START_DELAY, CONFIG_GD25_STOP_DELAY, CONFIG_GD25_CS_DELAY,
				 CONFIG_GD25_IFDELAY);
#endif
}
...
static inline int gd25_readid(FAR struct gd25_dev_s *priv)
{
	...
	if ((manufacturer == GD25_JEDEC_MANUFACTURER || manufacturer == P25_JEDEC_MANUFACTURER) &&
		(memory == GD25L_JEDEC_MEMORY_TYPE || memory == GD25Q_JEDEC_MEMORY_TYPE)) {
...
static inline void gd25_bytewrite(FAR struct gd25_dev_s *priv, FAR const uint8_t *buffer,
						   ...
						   FAR const uint8_t *buffer)
{
	...
	gd25_lock(priv->spi);
	gd25_purdid(priv);
	gd25_pagewrite(priv, buffer, startblock << GD25_PAGE_SHIFT, nblocks << GD25_PAGE_SHIFT);
	gd25_pd(priv);
	gd25_unlock(priv->spi);
...
static int gd25_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
			info->numsectors = priv->nsectors * GD25_SECTOR_SIZE / GD25_PAGE_SIZE;
