...
#define NAND_RAM_STATUS_LOG(str, ...) syslog(LOG_DEBUG, "nand_ram_status: " str, __VA_ARGS__);
...
static int g_nand_ram_rand_bad_blk_indx[] = {4,	  14,  19,	21,	 28,  30,  107, 108,
											 164, 173, 179, 229, 268, 362, 377, 382,
											 396, 410, 412, 419, 428, 456, 500, 0};
...
static void nand_ram_storage_status(void)
{
	...
	for (i = 0; i < NAND_RAM_N_PAGES; i++) {
		...
		NAND_RAM_STATUS_LOG("Block %3" PRIi32 ", Page %6" PRIi32 ", Bad: %1" PRIi32 " |"
							" Reads: %6" PRIi32 ", Writes: %6" PRIi32 ", Erases: %6" PRIi32 "\n",
							i >> NAND_RAM_LOG_PAGES_PER_BLOCK, i, bad, reads, writes, erases);
	}
...
static void nand_ram_storage_init(void)
{
	...
	memset(nand_ram_flash_data, 0xff, sizeof(struct nand_ram_data_s) * NAND_RAM_N_PAGES);
	memset(nand_ram_flash_spare, 0, sizeof(struct nand_ram_spare_s) * NAND_RAM_N_PAGES);
	...
	for (i = 0; g_nand_ram_rand_bad_blk_indx[i] != 0 &&
				g_nand_ram_rand_bad_blk_indx[i] < NAND_RAM_N_BLOCKS;
		 i++) {
		int j;

		for (j = 0; j < NAND_RAM_PAGES_PER_BLOCK; j++) {
			int page = (g_nand_ram_rand_bad_blk_indx[i] << NAND_RAM_LOG_PAGES_PER_BLOCK) + j;
...
int nand_ram_eraseblock(FAR struct nand_raw_s *raw, off_t block)
{
	...
	NAND_RAM_LOG("[LOWER %" PRIu64 " | %s] Block %" PRIi32 ", Start Page: %" PRIi32
				 ", Last Page: %" PRIi32,
...
int nand_ram_rawread(FAR struct nand_raw_s *raw, off_t block, unsigned int page, FAR void *data,
					 FAR void *spare)
{
	int ret;
	uint32_t read_page;
	...
	if (data != NULL) {
		if (nand_ram_flash_spare[read_page].free == NAND_RAM_PAGE_FREE) {
			...
			memcpy(data, (const void *)read_page_data->page, NAND_RAM_PAGE_SIZE);
		}
	}
...
int nand_ram_rawwrite(FAR struct nand_raw_s *raw, off_t block, unsigned int page,
					  FAR const void *data, FAR const void *spare)
{
	int ret;
	uint32_t write_page;
