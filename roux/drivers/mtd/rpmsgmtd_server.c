...
static int rpmsgmtd_erase_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
static int rpmsgmtd_bread_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
static int rpmsgmtd_bwrite_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								   uint32_t src, FAR void *priv);
static int rpmsgmtd_read_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
static int rpmsgmtd_write_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
...
static bool rpmsgmtd_ns_match(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							  uint32_t dest);
static void rpmsgmtd_ns_bind(FAR struct rpmsg_device *rdev, FAR void *priv, FAR const char *name,
							 uint32_t dest);
...
static const rpmsg_ept_cb g_rpmsgmtd_handler[] = {
	[RPMSGMTD_ERASE] = rpmsgmtd_erase_handler,	 [RPMSGMTD_BREAD] = rpmsgmtd_bread_handler,
	[RPMSGMTD_BWRITE] = rpmsgmtd_bwrite_handler, [RPMSGMTD_READ] = rpmsgmtd_read_handler,
	[RPMSGMTD_WRITE] = rpmsgmtd_write_handler,	 [RPMSGMTD_GEOMETRY] = rpmsgmtd_geometry_handler,
								  ...
								  uint32_t src, FAR void *priv)
{
	...
	while (read < msg->nblocks) {
		...
		*rsp = *msg;
		...
		ret = MTD_BREAD(server->dev, msg->startblock + read, nblocks, rsp->buf);

		rsp->header.result = ret;
		rpmsg_send_nocopy(ept, rsp, (ret < 0 ? 0 : ret * msg->blocksize) + sizeof(*rsp) - 1);
		if (ret < 0) {
								 ...
								 uint32_t src, FAR void *priv)
{
	...
	while (read < msg->nbytes) {
		...
		*rsp = *msg;
		...
		ret = MTD_READ(server->dev, msg->offset + read, space, (FAR uint8_t *)rsp->buf);
									 ...
									 uint32_t src, FAR void *priv)
{
	...
	msg->header.result = MTD_IOCTL(server->dev, MTDIOC_GEOMETRY, (unsigned long)&geo);
								  ...
								  uint32_t src, FAR void *priv)
{
	...
	msg->header.result =
		MTD_IOCTL(server->dev, msg->request, msg->arglen > 0 ? (unsigned long)msg->buf : msg->arg);
							 ...
							 uint32_t dest)
{
	...
	ret = rpmsg_create_ept(&server->ept, rdev, name, RPMSG_ADDR_ANY, dest, rpmsgmtd_ept_cb,
						   rpmsg_destroy_ept);
	if (ret < 0) {
...
{
	return rpmsg_register_callback(NULL, NULL, NULL, rpmsgmtd_ns_match, rpmsgmtd_ns_bind);
}
