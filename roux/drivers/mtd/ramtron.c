...
static ssize_t ramtron_bwrite_chunked(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
...
static inline int ramtron_readid(struct ramtron_dev_s *priv)
{
	...
	if (priv->part->name != NULL) {
		...
		finfo("RAMTRON %s of size %" PRIu32 " bytes (mf:%02" PRIx16 " mem:%02" PRIx16
			  " cap:%02" PRIx16 " part:%02" PRIx16 ")\n",
