...
static ssize_t sst26_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
						   FAR uint8_t *buf);
...
static inline void sst26_pagewrite(struct sst26_dev_s *priv, FAR const uint8_t *buffer, off_t page)
{
...
#ifdef CONFIG_MTD_BYTE_WRITE
static inline void sst26_bytewrite(struct sst26_dev_s *priv, FAR const uint8_t *buffer,
						   ...
						   FAR uint8_t *buffer)
{
	...
	nbytes = sst26_read(dev, startblock << priv->pageshift, nblocks << priv->pageshift, buffer);
	if (nbytes > 0) {
