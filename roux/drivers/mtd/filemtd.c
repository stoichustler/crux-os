...
static ssize_t filemtd_read(FAR struct file_dev_s *priv, FAR unsigned char *buffer,
							size_t offsetbytes, unsigned int nbytes);
static ssize_t filemtd_write(FAR struct file_dev_s *priv, size_t offset, FAR const void *src,
							 size_t len);

/* MTD driver methods */

static int filemtd_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks);
static ssize_t filemtd_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							 FAR uint8_t *buf);
static ssize_t filemtd_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							  FAR const uint8_t *buf);
static ssize_t filemtd_byteread(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
								FAR uint8_t *buf);
#ifdef CONFIG_MTD_BYTE_WRITE
static ssize_t file_bytewrite(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
							  FAR const uint8_t *buf);
#endif
...
static ssize_t mtd_loop_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t mtd_loop_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int mtd_loop_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
...
static int filemtd_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
			info->numsectors = priv->nblocks * priv->erasesize / priv->blocksize;
...
#ifdef CONFIG_MTD_LOOP
static int mtd_loop_setup(FAR const char *devname, FAR const char *filename, int sectsize,
						  int erasesize, off_t offset)
{
...
#ifdef CONFIG_MTD_LOOP
static ssize_t mtd_loop_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	return 0; /* Return EOF */
}
#endif /* CONFIG_MTD_LOOP */
...
#ifdef CONFIG_MTD_LOOP
static ssize_t mtd_loop_write(FAR struct file *filep, FAR const char *buffer, size_t len)
{
...
static int mtd_loop_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case MTD_LOOPIOC_SETUP: {
		FAR struct mtd_losetup_s *setup = (FAR struct mtd_losetup_s *)((uintptr_t)arg);

		if (setup == NULL) {
			ret = -EINVAL;
		} else {
			ret = mtd_loop_setup(setup->devname, setup->filename, setup->sectsize, setup->erasesize,
								 setup->offset);
		}
	} break;
...
FAR struct mtd_dev_s *filemtd_initialize(FAR const char *path, off_t offset, int16_t sectsize,
										 int32_t erasesize)
{
	...
	if ((priv->erasesize / priv->blocksize) * priv->blocksize != priv->erasesize) {
