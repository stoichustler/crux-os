...
#define SET_VALID(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= SST25_CACHE_VALID;                                                           \
	} while (0)
#define SET_DIRTY(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= SST25_CACHE_DIRTY;                                                           \
	} while (0)
#define SET_ERASED(p)                                                                              \
	do {                                                                                           \
		(p)->flags |= SST25_CACHE_ERASED;                                                          \
	} while (0)
...
static void sst25_byteread(FAR struct sst25_dev_s *priv, FAR uint8_t *buffer, off_t address,
						   size_t nbytes);
#ifndef CONFIG_SST25_READONLY
#ifdef CONFIG_SST25_SLOWWRITE
static void sst25_bytewrite(FAR struct sst25_dev_s *priv, FAR const uint8_t *buffer, off_t address,
							size_t nbytes);
#else
static void sst25_wordwrite(FAR struct sst25_dev_s *priv, FAR const uint8_t *buffer, off_t address,
							size_t nbytes);
#endif
...
static void sst25_cachewrite(FAR struct sst25_dev_s *priv, FAR const uint8_t *buffer, off_t sector,
							 size_t nsectors);
#endif
#endif
...
#if defined(CONFIG_SST25_SLOWWRITE) && !defined(CONFIG_SST25_READONLY)
static void sst25_bytewrite(struct sst25_dev_s *priv, FAR const uint8_t *buffer, off_t address,
							size_t nbytes)
{
	uint8_t status;
...
#if !defined(CONFIG_SST25_SLOWWRITE) && !defined(CONFIG_SST25_READONLY)
static void sst25_wordwrite(struct sst25_dev_s *priv, FAR const uint8_t *buffer, off_t address,
							size_t nbytes)
{
	...
	while (nwords > 0) {
		...
		while (nwords > 0 && buffer[0] == SST25_ERASED_STATE && buffer[1] == SST25_ERASED_STATE) {
						   ...
						   FAR uint8_t *buffer)
{
	...
	nbytes =
		sst25_read(dev, startblock << SST25_SECTOR_SHIFT, nblocks << SST25_SECTOR_SHIFT, buffer);
	if (nbytes > 0) {
							...
							FAR const uint8_t *buffer)
{
	...
	sst25_bytewrite(priv, buffer, startblock << priv->sectorshift, nblocks << priv->sectorshift);
#else
	sst25_wordwrite(priv, buffer, startblock << priv->sectorshift, nblocks << priv->sectorshift);
#endif
	sst25_unlock(priv->dev);
...
static int sst25_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
#ifdef CONFIG_SST25_SECTOR512
			info->numsectors = priv->nsectors << (priv->sectorshift - SST25_SECTOR_SHIFT);
