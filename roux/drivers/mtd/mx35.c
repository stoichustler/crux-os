...
#define MX35_READ_FROM_CACHE                                                                       \
	0x03 /* Output cache data                                                                      \
		  * on SO                2  1  1-2112 */
#define MX35_READ_FROM_CACHE_X1                                                                    \
	0x0B /* Output cache data                                                                      \
		  * on SO                2  1  1-2112 */
#define MX35_READ_FROM_CACHE_X2                                                                    \
	0x3B /* Output cache data                                                                      \
		  * on SI and SO         2  1  1-2112 */
#define MX35_READ_FROM_CACHE_X4                                                                    \
	0x6B				  /* Output cache data                                                     \
						   * on SI, SO, WP, HOLD  2  1  1-2112 */
#define MX35_READ_ID 0x9F /* Read device ID       0  1  2      */
#define MX35_ECC_STATUS_READ                                                                       \
	0x7C					  /* Internal ECC status                                               \
							   * output               0  1  1      */
#define MX35_BLOCK_ERASE 0xD8 /* Block erase          3  0  0      */
#define MX35_PROGRAM_EXECUTE                                                                       \
	0x10 /* Enter block/page                                                                       \
		  * address, execute     3  0  0      */
#define MX35_PROGRAM_LOAD                                                                          \
	0x02 /* Load program data with                                                                 \
		  * cache reset first    2  0  1-2112 */
#define MX35_PROGRAM_LOAD_RANDOM                                                                   \
	0x84 /* Load program data                                                                      \
		  * without cache reset  2  0  1-2112 */
#define MX35_PROGRAM_LOAD_X4                                                                       \
	0x32 /* Program load operation                                                                 \
		  * with x4 data input   2  0  1-2112 */
#define MX35_PROGRAM_LOAD_RANDOM_X4                                                                \
	0x34						/* Load random operation                                           \
								 * with x4 data input   2  0  1-2112 */
...
#define MX35_BP_BPRWD                                                                              \
	(1 << 7) /* Bit 7: Block Protection Register                                                   \
			  *        Write Disable */
...
static inline uint32_t mx35_addresstorow(FAR struct mx35_dev_s *priv, uint32_t address);
static inline uint32_t mx35_addresstocolumn(FAR struct mx35_dev_s *priv, uint32_t address);
...
static void mx35_readbuffer(FAR struct mx35_dev_s *priv, uint32_t address, uint8_t *buffer,
							size_t length);
...
static int mx35_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case MTDIOC_ECCSTATUS: {
		...
		*result = (priv->eccstatus & MX35_FEATURE_ECC_MASK) >> MX35_FEATURE_ECC_OFFSET;
