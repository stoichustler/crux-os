...
static ssize_t ram_byteread(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
							FAR uint8_t *buf);
#ifdef CONFIG_MTD_BYTE_WRITE
static ssize_t ram_bytewrite(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
							 FAR const uint8_t *buf);
#endif
...
static int ram_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
			info->numsectors = priv->nblocks * CONFIG_RAMMTD_ERASESIZE / CONFIG_RAMMTD_BLOCKSIZE;
