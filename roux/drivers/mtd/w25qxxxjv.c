...
#define W25QXXXJV_FAST_READ_QUADIO                                                                 \
	0xeb /* Fast Read Quad I/O:        *                                                           \
		  *   0xeb | ADDR | data...    */
#define W25QXXXJV_FAST_READ_QUADIO_4BT                                                             \
	0xec /* Fast Read Quad I/O 4 bytes *                                                           \
		  *   0xec | ADDR | data...    */
...
#define W25QXXXJV_SW_DIE_SELECT                                                                    \
	0xc2 /* SW_DIE_SELECT ID:               *                                                      \
		  * 0xc2 | Die index                 */
...
#define W25QXXXJVQ_JEDEC_DEVICE_TYPE                                                               \
	0x40 /* with QE = 1 (fixed) in Status register-2.                                              \
		  * Backward compatible to FV family. */

#define W25QXXXJVM_JEDEC_DEVICE_TYPE                                                               \
	0x70 /* with QE = 0 (programmable) in Status register-2.                                       \
...
#define SET_VALID(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= W25QXXXJV_CACHE_VALID;                                                       \
	} while (0)
#define SET_DIRTY(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= W25QXXXJV_CACHE_DIRTY;                                                       \
	} while (0)
#define SET_ERASED(p)                                                                              \
	do {                                                                                           \
		(p)->flags |= W25QXXXJV_CACHE_ERASED;                                                      \
	} while (0)
...
static int w25qxxxjv_command_address(FAR struct qspi_dev_s *qspi, uint8_t cmd, off_t addr,
									 uint8_t addrlen);
static int w25qxxxjv_command_read(FAR struct qspi_dev_s *qspi, uint8_t cmd, FAR void *buffer,
								  size_t buflen);
static int w25qxxxjv_command_write(FAR struct qspi_dev_s *qspi, uint8_t cmd, FAR const void *buffer,
								   size_t buflen);
...
static int w25qxxxjv_get_die_from_addr(FAR struct w25qxxxjv_dev_s *priv, off_t addr);
static int w25qxxxjv_readid(FAR struct w25qxxxjv_dev_s *priv);
static int w25qxxxjv_protect(FAR struct w25qxxxjv_dev_s *priv, off_t startblock, size_t nblocks);
static int w25qxxxjv_unprotect(FAR struct w25qxxxjv_dev_s *priv, off_t startblock, size_t nblocks);
static bool w25qxxxjv_isprotected(FAR struct w25qxxxjv_dev_s *priv, uint8_t status, off_t address);
...
static int w25qxxxjv_read_byte(FAR struct w25qxxxjv_dev_s *priv, FAR uint8_t *buffer, off_t address,
							   size_t nbytes);
static int w25qxxxjv_write_page(FAR struct w25qxxxjv_dev_s *priv, FAR const uint8_t *buffer,
								off_t address, size_t nbytes);
#ifdef CONFIG_W25QXXXJV_SECTOR512
static int w25qxxxjv_flush_cache(FAR struct w25qxxxjv_dev_s *priv);
static FAR uint8_t *w25qxxxjv_read_cache(FAR struct w25qxxxjv_dev_s *priv, off_t sector);
static void w25qxxxjv_erase_cache(FAR struct w25qxxxjv_dev_s *priv, off_t sector);
static int w25qxxxjv_write_cache(FAR struct w25qxxxjv_dev_s *priv, FAR const uint8_t *buffer,
								 off_t sector, size_t nsectors);
#endif

/* MTD driver methods */

static int w25qxxxjv_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks);
static ssize_t w25qxxxjv_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							   FAR uint8_t *buf);
static ssize_t w25qxxxjv_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
...
static void w25qxxxjv_write_status(FAR struct w25qxxxjv_dev_s *priv)
{
	...
	w25qxxxjv_command_write(priv->qspi, W25QXXXJV_WRITE_STATUS_1, (FAR const void *)priv->cmdbuf,
							1);
...
{
	w25qxxxjv_write_enable(priv);
	w25qxxxjv_command_write(priv->qspi, W25QXXXJV_SW_DIE_SELECT, (FAR const void *)&die, 1);
	w25qxxxjv_write_disable(priv);
...
static void w25qxxxjv_quad_enable(FAR struct w25qxxxjv_dev_s *priv)
{
	w25qxxxjv_command_read(priv->qspi, W25QXXXJV_READ_STATUS_2, (FAR void *)priv->cmdbuf, 1);
...
static int w25qxxxjv_protect(FAR struct w25qxxxjv_dev_s *priv, off_t startblock, size_t nblocks)
{
	...
	if ((priv->cmdbuf[0] & priv->protectmask) == (STATUS_BP_ALL & priv->protectmask)) {
...
static int w25qxxxjv_erase_sector(FAR struct w25qxxxjv_dev_s *priv, off_t sector)
{
	...
	if ((status & priv->protectmask) != 0 && w25qxxxjv_isprotected(priv, status, address)) {
	...
	w25qxxxjv_write_enable(priv);
	w25qxxxjv_command_address(priv->qspi, W25QXXXJV_SECTOR_ERASE, address, priv->addresslen);
							   ...
							   size_t buflen)
{
	...
	meminfo.cmd =
		(priv->addresslen == 4) ? W25QXXXJV_FAST_READ_QUADIO_4BT : W25QXXXJV_FAST_READ_QUADIO;
...
static FAR uint8_t *w25qxxxjv_read_cache(FAR struct w25qxxxjv_dev_s *priv, off_t sector)
{
	...
	if (!IS_VALID(priv) || esectno != priv->esectno) {
		...
		ret = w25qxxxjv_read_byte(priv, priv->sector, (esectno << priv->sectorshift),
								  (1 << priv->sectorshift));
		if (ret < 0) {
							   ...
							   FAR uint8_t *buffer)
{
	...
	if (priv->numofdies != 0) {
		priv->currentdie = w25qxxxjv_get_die_from_addr(priv, startblock << priv->pageshift);
	}
...
#else
	nbytes = w25qxxxjv_read(dev, startblock << priv->pageshift, nblocks << priv->pageshift, buffer);
	if (nbytes > 0) {
...
static int w25qxxxjv_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case MTDIOC_GEOMETRY: {
		...
		if (geo) {
			...
			geo->neraseblocks = priv->nsectors << (priv->sectorshift - W25QXXXJV_SECTOR512_SHIFT);
#else
	...
	case BIOC_PARTINFO: {
		FAR struct partition_info_s *info = (FAR struct partition_info_s *)arg;
		if (info != NULL) {
#ifdef CONFIG_W25QXXXJV_SECTOR512
			info->numsectors = priv->nsectors << (priv->sectorshift - W25QXXXJV_SECTOR512_SHIFT);
			info->sectorsize = 1 << W25QXXXJV_SECTOR512_SHIFT;
#else
			info->numsectors = priv->nsectors << (priv->sectorshift - priv->pageshift);
	...
	case MTDIOC_PROTECT: {
		FAR const struct mtd_protect_s *prot = (FAR const struct mtd_protect_s *)((uintptr_t)arg);
...
FAR struct mtd_dev_s *w25qxxxjv_initialize(FAR struct qspi_dev_s *qspi, bool unprotect)
{
