...
#if defined(CONFIG_FS_PROCFS) && !defined(CONFIG_PROCFS_EXCLUDE_PARTITIONS)
static int part_procfs_open(FAR struct file *filep, FAR const char *relpath, int oflags,
							mode_t mode);
...
static int part_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
			...
			strlcpy(info->parent, priv->parent->name, sizeof(info->parent));
	...
	case BIOC_XIPBASE: {
		...
		if (ppv) {
			...
			ret =
				priv->parent->ioctl(priv->parent, BIOC_XIPBASE, (unsigned long)((uintptr_t)&base));
			if (ret == OK) {
				/* Add the offset of this partition to the XIP base and
				 * return the sum to the caller.
				 */

				*ppv = (FAR void *)(uintptr_t)(base + priv->firstblock * priv->geo.blocksize);
			}
		}
	} break;
	...
	case MTDIOC_BULKERASE: {
		...
		ret = priv->parent->erase(priv->parent, priv->firstblock / priv->blkpererase,
	...
	case MTDIOC_ERASESECTORS: {
		...
		ret = priv->parent->erase(
			priv->parent, priv->firstblock / priv->blkpererase + erase->startblock, erase->nblocks);
	} break;
...
static int part_isbad(FAR struct mtd_dev_s *dev, off_t block)
{
	...
	if (priv->parent->isbad) {
		return priv->parent->isbad(priv->parent, block + priv->firstblock / priv->blkpererase);
	}
...
static int part_markbad(FAR struct mtd_dev_s *dev, off_t block)
{
	...
	if (priv->parent->markbad) {
		return priv->parent->markbad(priv->parent, block + priv->firstblock / priv->blkpererase);
	}
...
static ssize_t part_procfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (attr->nextpart) {
		...
		do {
			...
			ret =
				snprintf(&buffer[total], buflen - total, "%s%7ju %7ju   %s\n", partname,
						 (uintmax_t)attr->nextpart->firstblock / blkpererase,
						 (uintmax_t)attr->nextpart->geo.neraseblocks, attr->nextpart->parent->name);
#else
			ret =
				snprintf(&buffer[total], buflen - total, "%7ju %7ju   %s\n",
						 (uintmax_t)attr->nextpart->firstblock / blkpererase,
						 (uintmax_t)attr->nextpart->geo.neraseblocks, attr->nextpart->parent->name);
#endif
...
FAR struct mtd_dev_s *mtd_partition(FAR struct mtd_dev_s *mtd, off_t firstblock, off_t nblocks)
{
	...
	ret = mtd->ioctl(mtd, MTDIOC_GEOMETRY, (unsigned long)((uintptr_t)&part->geo));
	if (ret < 0) {
	...
	DEBUGASSERT(part->blkpererase * part->geo.blocksize == part->geo.erasesize);
