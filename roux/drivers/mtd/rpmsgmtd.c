...
static int rpmsgmtd_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks);
static int rpmsgmtd_get_geometry(FAR struct rpmsgmtd_s *dev, FAR struct mtd_geometry_s *geometry);
static ssize_t rpmsgmtd_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							  FAR uint8_t *buffer);
static ssize_t rpmsgmtd_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							   FAR const uint8_t *buffer);
static ssize_t rpmsgmtd_read(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
							 FAR uint8_t *buffer);
#ifdef CONFIG_MTD_BYTE_WRITE
static ssize_t rpmsgmtd_write(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
							  FAR const uint8_t *buffer);
#endif
static ssize_t rpmsgmtd_ioctl_arglen(int cmd);
static int rpmsgmtd_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg);

/* Functions for sending data to the remote cpu */

static int rpmsgmtd_send_recv(FAR struct rpmsgmtd_s *priv, uint32_t command, bool copy,
							  FAR struct rpmsgmtd_header_s *msg, int len, FAR void *data);
static FAR void *rpmsgmtd_get_tx_payload_buffer(FAR struct rpmsgmtd_s *priv, FAR uint32_t *len);

/* Functions handle the responses from the remote cpu */

static int rpmsgmtd_default_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									uint32_t src, FAR void *priv);
static int rpmsgmtd_bread_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);
static int rpmsgmtd_read_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								 uint32_t src, FAR void *priv);
static int rpmsgmtd_geometry_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
									 uint32_t src, FAR void *priv);
static int rpmsgmtd_ioctl_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);

/* Functions for creating communication with remote cpu */

static void rpmsgmtd_device_created(struct rpmsg_device *rdev, FAR void *priv_);
static void rpmsgmtd_device_destroy(struct rpmsg_device *rdev, FAR void *priv_);
static int rpmsgmtd_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len, uint32_t src,
						   FAR void *priv);
...
static const rpmsg_ept_cb g_rpmsgmtd_handler[] = {
	[RPMSGMTD_ERASE] = rpmsgmtd_default_handler,  [RPMSGMTD_BREAD] = rpmsgmtd_bread_handler,
	[RPMSGMTD_BWRITE] = rpmsgmtd_default_handler, [RPMSGMTD_READ] = rpmsgmtd_read_handler,
	[RPMSGMTD_WRITE] = rpmsgmtd_default_handler,  [RPMSGMTD_GEOMETRY] = rpmsgmtd_geometry_handler,
...
static int rpmsgmtd_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks)
{
	...
	return rpmsgmtd_send_recv(priv, RPMSGMTD_ERASE, true, &msg.header, sizeof(msg), NULL);
}
...
static int rpmsgmtd_get_geometry(FAR struct rpmsgmtd_s *dev, FAR struct mtd_geometry_s *geometry)
{
	...
	ret = rpmsgmtd_send_recv(priv, RPMSGMTD_GEOMETRY, true, &msg.header, sizeof(msg), NULL);
							  ...
							  FAR uint8_t *buffer)
{
	...
	ret = rpmsgmtd_send_recv(priv, RPMSGMTD_BREAD, true, &msg.header, sizeof(msg) - 1, &iov);
							   ...
							   FAR const uint8_t *buffer)
{
	...
	while (written < nblocks) {
		...
		ret = rpmsg_send_nocopy(&priv->ept, msg, sizeof(*msg) - 1 + msg->nblocks * blocksize);
		if (ret < 0) {
			goto out;
		}
	}
							 ...
							 FAR uint8_t *buffer)
{
	...
	ret = rpmsgmtd_send_recv(priv, RPMSGMTD_READ, true, &msg.header, sizeof(msg) - 1, &iov);
...
static int rpmsgmtd_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	return rpmsgmtd_send_recv(priv, RPMSGMTD_IOCTL, false, &msg->header, msglen,
...
static void rpmsgmtd_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	...
	if (strcmp(priv->remotecpu, rpmsg_get_cpuname(rdev)) == 0) {
		...
		snprintf(buf, sizeof(buf), "%s%s", RPMSGMTD_NAME_PREFIX, priv->remotepath);
		rpmsg_create_ept(&priv->ept, rdev, buf, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY, rpmsgmtd_ept_cb,
						 NULL);
	}
}
...
static void rpmsgmtd_device_destroy(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	FAR struct rpmsgmtd_s *priv = priv_;
					  ...
					  FAR const char *localpath)
{
	...
	ret =
		rpmsg_register_callback(dev, rpmsgmtd_device_created, rpmsgmtd_device_destroy, NULL, NULL);
	if (ret < 0) {
...
fail_with_rpmsg:
	rpmsg_unregister_callback(dev, rpmsgmtd_device_created, rpmsgmtd_device_destroy, NULL, NULL);
