...
static ssize_t s512_bread(FAR struct mtd_dev_s *dev, off_t sector512, size_t nsectors,
						  FAR uint8_t *buf);
...
static FAR uint8_t *s512_cacheread(struct s512_dev_s *priv, off_t sector512)
{
	...
	if (!IS_VALID(priv) || eblockno != priv->eblockno) {
		...
		result = priv->dev->bread(priv->dev, sector, priv->sectperblock, priv->eblock);
		if (result < 0) {
			ferr("ERROR: bread(%lu, %lu) returned %ld\n", (unsigned long)sector,
				 (unsigned long)priv->eblocksize, (long)result);
...
FAR struct mtd_dev_s *s512_initialize(FAR struct mtd_dev_s *mtd)
{
	...
	if (ret < 0 || geo.erasesize <= SECTOR_512 || (geo.erasesize & ~MASK_512) != geo.erasesize) {
