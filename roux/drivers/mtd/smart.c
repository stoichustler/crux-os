...
#define SMART_FIRST_ALLOC_SECTOR                                                                   \
	12 /* First logical sector number                                                              \
...
#if defined(CONFIG_MTD_SMART_READAHEAD) ||                                                         \
	(defined(CONFIG_DRVR_WRITABLE) && defined(CONFIG_MTD_SMART_WRITEBUFFER))
...
#define SET_BITMAP(m, n)                                                                           \
	do {                                                                                           \
		(m)[(n) / 8] |= 1 << ((n) % 8);                                                            \
	} while (0)
#define CLR_BITMAP(m, n)                                                                           \
	do {                                                                                           \
		(m)[(n) / 8] &= ~(1 << ((n) % 8));                                                         \
	} while (0)
...
static const uint8_t g_wearbit_to_levelmap4[] = {7, 13, 10, 14, 6, 15, 5, 4,
												 3, 12, 9,	8,	2, 11, 1, 0};
#endif
...
static ssize_t smart_reload(struct smart_struct_s *dev, FAR uint8_t *buffer, off_t startblock,
							size_t nblocks);
static ssize_t smart_read(FAR struct inode *inode, FAR unsigned char *buffer, blkcnt_t start_sector,
						  unsigned int nsectors);
static ssize_t smart_write(FAR struct inode *inode, FAR const unsigned char *buffer,
						   blkcnt_t start_sector, unsigned int nsectors);
static int smart_geometry(FAR struct inode *inode, FAR struct geometry *geometry);
static int smart_ioctl(FAR struct inode *inode, int cmd, unsigned long arg);

static int smart_findfreephyssector(FAR struct smart_struct_s *dev, bool canrelocate);
...
static int smart_relocate_sector(FAR struct smart_struct_s *dev, uint16_t oldsector,
								 uint16_t newsector);
...
static ssize_t smart_loop_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t smart_loop_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
#ifdef CONFIG_MTD_SMART_ALLOC_DEBUG
FAR static void *smart_malloc(FAR struct smart_struct_s *dev, size_t bytes, FAR const char *name)
{
...
#ifdef CONFIG_MTD_SMART_ALLOC_DEBUG
FAR static void *smart_zalloc(FAR struct smart_struct_s *dev, size_t bytes, FAR const char *name)
{
	FAR void *mem;
...
static void smart_set_count(FAR struct smart_struct_s *dev, FAR uint8_t *pcount, uint16_t block,
							uint8_t count)
{
...
#ifdef CONFIG_MTD_SMART_PACK_COUNTS
static void smart_add_count(FAR struct smart_struct_s *dev, FAR uint8_t *pcount, uint16_t block,
							int adder)
{
	int16_t value;
							...
							size_t nblocks)
{
	...
	if (nread != mtdblocks) {
		ferr("ERROR: Read %zd blocks starting at block %" PRIdOFF " failed: %zd\n", nblocks,
			 startblock, nread);
	}
...
static int smart_setsectorsize(FAR struct smart_struct_s *dev, uint16_t size)
{
	...
	dev->smap = (FAR uint16_t *)smart_malloc(dev, totalsectors * sizeof(uint16_t) + allocsize,
											 "Sector map");
	if (!dev->smap) {
	...
	if (dev->scache == NULL) {
		dev->scache = (FAR struct smart_cache_s *)smart_malloc(
			dev, CONFIG_MTD_SMART_SECTOR_CACHE_SIZE * sizeof(struct smart_cache_s) + allocsize,
			"Sector Cache");
	}
	...
	dev->wearstatus =
		(FAR uint8_t *)smart_malloc(dev, dev->neraseblocks >> SMART_WEAR_BIT_DIVIDE, "Wear status");
	if (!dev->wearstatus) {
...
static ssize_t smart_bytewrite(FAR struct smart_struct_s *dev, size_t offset, int nbytes,
							   FAR const uint8_t *buffer)
{
...
#endif
	{
		...
		nblocks = (offset - startblock * dev->geo.blocksize + nbytes + dev->geo.blocksize - 1) /
				  dev->geo.blocksize;
...
#ifdef CONFIG_MTD_SMART_MINIMIZE_RAM
static int smart_add_sector_to_cache(FAR struct smart_struct_s *dev, uint16_t logical,
									 uint16_t physical, int line)
{
	uint16_t index;
	uint16_t x;
	uint16_t oldest;
...
static int smart_scan(FAR struct smart_struct_s *dev)
{
	...
	static const uint16_t sizetbl[8] = {
		CONFIG_MTD_SMART_SECTOR_SIZE, 512, 1024, 4096, 2048, 8192, 16384, 32768};
	...
	for (sector = 0; sector < totalsectors; sector++) {
		...
		if (logicalsector == 0) {
			...
			for (x = 1; x < dev->rootdirentries; x++) {
				if (dev->partname[0] != '\0') {
					snprintf(devname, sizeof(devname), "/dev/smart%d%sd%d", dev->minor,
							 dev->partname, x + 1);
				} else {
				...
				rootdirdev = (FAR struct smart_multiroot_device_s *)smart_malloc(
					dev, sizeof(*rootdirdev), "Root Dir");
				if (rootdirdev == NULL) {
...
#endif
		{
			...
			ret = MTD_BREAD(dev->mtd, winner * dev->mtdblkspersector, dev->mtdblkspersector,
	...
	if (sector != 0xffff) {
		...
		ret = MTD_BREAD(dev->mtd, sector * dev->mtdblkspersector, dev->mtdblkspersector,
						(FAR uint8_t *)dev->rwbuffer);
		if (ret != dev->mtdblkspersector) {
			ferr("ERROR: Error reading physical sector %d.\n", sector);
			goto err_out;
		...
		if (dev->rwbuffer[SMART_WEAR_LEVEL_FORMAT_SIG] == 0) {
			...
			memset(&dev->rwbuffer[SMART_WEAR_LEVEL_FORMAT_SIG], 0xff,
				   dev->mtdblkspersector * dev->geo.blocksize - SMART_WEAR_LEVEL_FORMAT_SIG);
	...
	if (dev->mtdblkspersector == 0 || dev->sectorsperblk == 0 || dev->sectorsperblk == 0 ||
		dev->sectorsize == 0) {
...
static void smart_erase_block_if_empty(FAR struct smart_struct_s *dev, uint16_t block,
									   bool forceerase)
{
	uint16_t freecount;
...
static int smart_relocate_static_data(FAR struct smart_struct_s *dev, uint16_t block)
{
	...
	if (smart_get_wear_level(dev, block) >= SMART_WEAR_FULL_RELOCATE_THRESHOLD) {
		...
		finfo("Moving block %d, wear %d, free %d, "
			  ...
			  x, smart_get_wear_level(dev, x), nextsector, newsector, block,
			  smart_get_wear_level(dev, block));

		nextsector = block * dev->sectorsperblk;
		for (sector = x * dev->sectorsperblk;
			 sector < x * dev->sectorsperblk + dev->availsectperblk; sector++) {
...
static crc_t smart_calc_sector_crc(FAR struct smart_struct_s *dev)
{
	...
	crc = crc8((FAR uint8_t *)&dev->rwbuffer[sizeof(struct smart_sect_header_s)],
	...
	crc = crc8part((FAR uint8_t *)&dev->rwbuffer[offsetof(struct smart_sect_header_s, status)], 1,
				   crc);

#elif defined(CONFIG_SMART_CRC_16)
	/* Calculate CRC on data region of the sector */

	crc = crc16((FAR uint8_t *)&dev->rwbuffer[sizeof(struct smart_sect_header_s)],
				dev->mtdblkspersector * dev->geo.blocksize - sizeof(struct smart_sect_header_s));
	...
	crc = crc16part((FAR uint8_t *)&dev->rwbuffer[offsetof(struct smart_sect_header_s, status)], 2,
					crc);

#elif defined(CONFIG_SMART_CRC_32)
	/* Calculate CRC on data region of the sector */

	crc = crc32((FAR uint8_t *)&dev->rwbuffer[sizeof(struct smart_sect_header_s)],
				dev->mtdblkspersector * dev->geo.blocksize - sizeof(struct smart_sect_header_s));
...
static inline int smart_llformat(FAR struct smart_struct_s *dev, unsigned long arg)
{
	...
	*((FAR uint16_t *)sectorheader->logicalsector) = 0;
	sectorheader->status =
		(uint8_t) ~(SMART_STATUS_COMMITTED | SMART_STATUS_VERBITS | SMART_STATUS_SIZEBITS) |
		SMART_STATUS_VERSION | sectsize;
#ifdef CONFIG_MTD_SMART_ENABLE_CRC
								 ...
								 uint16_t newsector)
{
	...
	*((FAR uint32_t *)header->crc32) = smart_calc_sector_crc(dev);
	...
	ret = MTD_BWRITE(dev->mtd, newsector * dev->mtdblkspersector, dev->mtdblkspersector,
					 (FAR uint8_t *)dev->rwbuffer);
	if (ret != dev->mtdblkspersector) {
...
static int smart_relocate_block(FAR struct smart_struct_s *dev, uint16_t block)
{
	...
	for (x = block * dev->sectorsperblk; x < block * dev->sectorsperblk + dev->availsectperblk;
		 x++) {
		/* Read the next sector from this erase block */

		ret = MTD_BREAD(dev->mtd, x * dev->mtdblkspersector, dev->mtdblkspersector,
						(FAR uint8_t *)dev->rwbuffer);
		if (ret != dev->mtdblkspersector) {
			ferr("ERROR: Error reading sector %d\n", x);
			ret = -EIO;
...
retry:
	...
	if (allocblock == 0xffff) {
...

		{
			...
			for (i = 0, remaining = dev->neraseblocks; remaining > 0; i += 8, remaining -= 8) {
				for (j = 0, k = 0; j < 8 && j < remaining; j++) {
					snprintf(&buffer[k], sizeof(buffer) - k, "%12d", dev->freecount[i + j]);
					k += strlen(&buffer[k]);
				}
		...
		if (collect) {
          ...
          releasemax = smart_get_count(dev, dev->releasecount, collectblock);
			...
			finfo("Collecting block %d, free=%d released=%d, "
				  ...
				  smart_get_count(dev, dev->releasecount, collectblock), dev->freesectors,
				  dev->releasesectors);
#else
			finfo("Collecting block %d, free=%d released=%d\n", collectblock,
				  dev->freecount[collectblock], dev->releasecount[collectblock]);
#endif
...
static int smart_write_wearstatus(struct smart_struct_s *dev)
{
	...
	while (remaining) {
		...
		towrite = remaining;
		if (towrite >
			dev->sectorsize - (SMARTFS_FMT_WEAR_POS + sizeof(struct smart_sect_header_s))) {
...
static int smart_write_alloc_sector(FAR struct smart_struct_s *dev, uint16_t logical,
									uint16_t physical)
{
	int ret = 1;
	uint8_t sectsize;
...
#if CONFIG_SMARTFS_ERASEDSTATE == 0xff
	header->status = ~(SMART_STATUS_COMMITTED | SMART_STATUS_SIZEBITS | SMART_STATUS_VERBITS) |
					 SMART_STATUS_VERSION | sectsize;
#ifdef CONFIG_MTD_SMART_ENABLE_CRC
...
static int smart_writesector(FAR struct smart_struct_s *dev, unsigned long arg)
{
	...
	ret = MTD_BREAD(dev->mtd, mtdblock, dev->mtdblkspersector, (FAR uint8_t *)dev->rwbuffer);
	if (ret != dev->mtdblkspersector) {
		ferr("ERROR: Error reading phys sector %d\n", physsector);
		ret = -EIO;
		goto errout;
	...
	memcpy(&dev->rwbuffer[sizeof(struct smart_sect_header_s) + req->offset], req->buffer,
		   req->count);
	...
	*((FAR uint32_t *)header->crc32) = smart_calc_sector_crc(dev);
	...
	if (needsrelocate) {
		...
		ret = MTD_BWRITE(dev->mtd, physsector * dev->mtdblkspersector, dev->mtdblkspersector,
						 (FAR uint8_t *)dev->rwbuffer);
		if (ret != dev->mtdblkspersector) {
	...
	} else /* needsrelocate */
	{
		...
		ret = MTD_BREAD(dev->mtd, physsector * dev->mtdblkspersector, dev->mtdblkspersector,
						(FAR uint8_t *)dev->rwbuffer);
		if (ret == dev->mtdblkspersector) {
			/* Validate the CRC of the read-back data */
...
static int smart_readsector(FAR struct smart_struct_s *dev, unsigned long arg)
{
	...
	req = (FAR struct smart_read_write_s *)arg;
	DEBUGASSERT(req->offset < dev->sectorsize);
	DEBUGASSERT(req->offset + req->count + sizeof(struct smart_sect_header_s) <= dev->sectorsize);
	...
	memmove((FAR char *)req->buffer,
			&dev->rwbuffer[req->offset + sizeof(struct smart_sect_header_s)], req->count);
	ret = req->count;
	...
	readaddr = (uint32_t)physsector * dev->mtdblkspersector * dev->geo.blocksize + req->offset +
...
static inline int smart_allocsector(FAR struct smart_struct_s *dev, unsigned long requested)
{
	...
	finfo("Alloc: log=%d, phys=%d, erase block=%d, free=%d, released=%d\n", logsector,
		  physicalsector, physicalsector / dev->sectorsperblk, dev->freesectors,
		  dev->releasesectors);
	...
	{
		FAR struct smart_allocsector_s *allocsect =
			(FAR struct smart_allocsector_s *)kmm_malloc(sizeof(struct smart_allocsector_s));
...
#ifdef CONFIG_SMARTFS_MULTI_ROOT_DIRS
		ret =
			smart_getformat(dev, (FAR struct smart_format_s *)arg,
...
static int smart_fsck_file(FAR struct smart_struct_s *dev, FAR uint8_t *checkmap,
						   uint16_t logsector)
{
	int ret = OK;
	ssize_t size;
	...
	do {
		...
		chain =
			(FAR struct smart_chain_header_s *)&dev->rwbuffer[sizeof(struct smart_sect_header_s)];
...
static int smart_fsck_directory(FAR struct smart_struct_s *dev, FAR uint8_t *checkmap,
								uint16_t logsector)
{
	int ret = OK;
	int relocate = 0;
	...
	size = MTD_BREAD(dev->mtd, physsector * dev->mtdblkspersector, dev->mtdblkspersector, rwbuffer);
	...
	chain = (FAR struct smart_chain_header_s *)&rwbuffer[sizeof(struct smart_sect_header_s)];
	entry = (FAR struct smart_entry_header_s *)&rwbuffer[sizeof(struct smart_sect_header_s) +
...
#ifdef CONFIG_DEBUG_FS_INFO
		strlcpy(entryname, (FAR const char *)(cur + sizeof(struct smart_entry_header_s)),
				sizeof(entryname));
...
static int smart_fsck(FAR struct smart_struct_s *dev)
{
	...
	for (logsector = SMART_FIRST_ALLOC_SECTOR; logsector < dev->totalsectors; logsector++) {
...
#ifdef CONFIG_SMARTFS_MULTI_ROOT_DIRS
		if (partname != NULL) {
			snprintf(devname, sizeof(devname), "/dev/smart%d%sd1", minor, partname);
		} else {
...
#else
		if (partname != NULL) {
			snprintf(devname, sizeof(devname), "/dev/smart%d%s", minor, partname);
		} else {
...
#ifdef CONFIG_SMART_DEV_LOOP
static int smart_losetup(int minor, FAR const char *filename, int sectsize, int erasesize,
...
static int smart_loop_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case SMART_LOOPIOC_SETUP: {
		...
		if (setup == NULL) {
			...
			ret = smart_losetup(setup->minor, setup->filename, setup->sectsize, setup->erasesize,
