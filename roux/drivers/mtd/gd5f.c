...
static void gd5f_readbuffer(FAR struct gd5f_dev_s *priv, uint32_t address, uint8_t *buffer,
							size_t length);
						   ...
						   FAR const uint8_t *buffer)
{
	...
	nbytes = gd5f_write(dev, startblock << priv->pageshift, nblocks << priv->pageshift, buffer);
	if (nbytes > 0) {
		nbytes >>= priv->pageshift;
	}
...
static int gd5f_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case MTDIOC_ECCSTATUS: {
		...
		*result = (priv->eccstatus & GD5F_FEATURE_ECC_MASK) >> GD5F_FEATURE_ECC_OFFSET;
