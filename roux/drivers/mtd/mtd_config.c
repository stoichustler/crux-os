...
static int mtdconfig_readbytes(FAR struct mtdconfig_struct_s *dev, int offset, FAR uint8_t *pdata,
							   int readlen)
{
...
static int mtdconfig_findnextentry(FAR struct mtdconfig_struct_s *dev, off_t offset,
								   FAR struct mtdconfig_header_s *phdr, uint16_t size)
{
	bool found = false;
	int ret;
	uint16_t block;
...
read_next:
		ret = mtdconfig_readbytes(dev, offset, (FAR uint8_t *)phdr, sizeof(*phdr));
		if (ret != OK) {
			return 0;
		}
	...
	while (src_block < dev->neraseblocks) {
		...
		while (src_offset < dev->erasesize) {
			...
			if (phdr->flags == MTD_ERASED_FLAGS(dev)) {
				...
				ret = mtdconfig_writebytes(dev, dst_offset, (FAR uint8_t *)phdr, sizeof(hdr));
				if (ret != sizeof(hdr)) {
					/* I/O Error! */
				...
				ret = mtdconfig_writebytes(dev, dst_offset, &pbuf[src_offset + sizeof(hdr)],
										   phdr->len);
				if (ret != phdr->len) {
					/* I/O Error! */
...
retry_relocate:
		...
		if (hdr.flags == MTD_ERASED_FLAGS(dev)) {
...
#endif
			{
				...
				ret = mtdconfig_writebytes(dev, dst_offset, (FAR uint8_t *)&hdr, sizeof(hdr));
				if (ret != sizeof(hdr)) {
					/* I/O Error! */
...
static int mtdconfig_findentry(FAR struct mtdconfig_struct_s *dev, off_t offset,
							   FAR struct config_data_s *pdata, FAR struct mtdconfig_header_s *phdr)
{
	uint16_t endblock;
	int ret;
...
retry_find:
	...
	if (offset > 0) {
		...
		if (bytes != pdata->len) {
			...
			hdr.flags = MTD_ERASED_FLAGS(dev);
			mtdconfig_writebytes(dev, offset, (FAR uint8_t *)&hdr, sizeof(hdr.flags));
			ret = -EIO;
			goto errout;
		}
								 ...
								 FAR struct config_data_s *pdata)
{
...
#ifdef CONFIG_MTD_CONFIG_NAMED
	if (dev->readoff != 0 && hdr.name[0] != MTD_ERASED_ID(dev))
#else
...
#endif
	{
		...
		ret =
			mtdconfig_readbytes(dev, dev->readoff + sizeof(hdr), pdata->configdata, bytes_to_read);
		if (ret < 0) {
			goto errout;
		}
...
static int mtdconfig_nextconfig(FAR struct mtdconfig_struct_s *dev, FAR struct config_data_s *pdata)
{
	int ret = -ENOENT;
	off_t bytes_to_read;
	struct mtdconfig_header_s hdr;
	...
	ret = mtdconfig_readbytes(dev, dev->readoff, (FAR uint8_t *)&hdr, sizeof(hdr));
	if (ret < 0) {
		goto errout;
	}
...
int mtdconfig_register(FAR struct mtd_dev_s *mtd)
{
	...
	if (dev != NULL) {
		...
		ret = MTD_IOCTL(mtd, MTDIOC_ERASESTATE, (unsigned long)((uintptr_t)&dev->erasestate));
		if (ret < 0) {
