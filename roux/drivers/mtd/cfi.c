...
#define CFI_INTEL_STATUS_ECLBS                                                                     \
	0x20 /* clear block lock bit                                                                   \
		  * or erase error */
#define CFI_INTEL_STATUS_PSLBS                                                                     \
	0X10							/* write or block lock bit                                     \
									 * set error */
...
#define CFI_AMD_STATUS_TOGGLE                                                                      \
	0x40 /* DQ6 will toggle after a                                                                \
...
static const uint32_t g_cfi_query_address[2] = {0x55, 0x555};
...
static uintptr_t cfi_get_block_addr(FAR struct cfi_dev_s *cfi, blkcnt_t block)
{
	uint8_t i;
...
static uintptr_t cfi_addr_map(FAR struct cfi_dev_s *cfi, blkcnt_t block, off_t offset)
{
...
static void cfi_add_byte(FAR struct cfi_dev_s *cfi, FAR cfiword_t *word, uint8_t c)
{
...
static inline void cfi_write_data(FAR struct cfi_dev_s *cfi, uintptr_t addr, FAR const void *buffer)
{
	switch (cfi->bankwidth) {
	case CFI_PORT_8BIT:
...
static void cfi_write_block_cmd(FAR struct cfi_dev_s *cfi, blkcnt_t block, uint32_t cmd)
{
...
static void cfi_write_address_cmd(FAR struct cfi_dev_s *cfi, uintptr_t addr, uint8_t cmd)
{
	cfiword_t val = cfi_make_cmd(cfi, cmd);
...
static inline cfiword_t cfi_read_data(FAR struct cfi_dev_s *cfi, uintptr_t addr)
{
	switch (cfi->bankwidth) {
	case CFI_PORT_8BIT:
...
static bool cfi_is_equal(FAR struct cfi_dev_s *cfi, uintptr_t addr, uint8_t cmd)
{
	cfiword_t val = cfi_make_cmd(cfi, cmd);
...
static int cfi_intel_status_check(FAR struct cfi_dev_s *cfi, blkcnt_t block, uint32_t tout)
{
	uint32_t start;
	...
	while (cfi_intel_is_busy(cfi, block)) {
		...
		if (!cfi_is_equal(cfi, 0, CFI_INTEL_STATUS_DONE)) {
			...
			if (cfi_is_set(cfi, CFI_INTEL_STATUS_ECLBS | CFI_INTEL_STATUS_PSLBS)) {
...
static int cfi_intel_write_word(FAR struct cfi_dev_s *cfi, uintptr_t addr, cfiword_t word)
{
...
static int cfi_intel_write_buffer(FAR struct cfi_dev_s *cfi, uintptr_t addr, size_t nbytes,
...
static int cfi_intel_erase(FAR struct cfi_dev_s *cfi, blkcnt_t block)
{
	...
	return cfi_intel_status_check(cfi, block,
								  1000 * (1 << cfi->info.block_erase_timeout_typ) *
...
static bool cfi_amd_is_busy(FAR struct cfi_dev_s *cfi, blkcnt_t block)
{
	...
	return ((cfi_read_byte(addr) ^ cfi_read_byte(addr)) & CFI_AMD_STATUS_TOGGLE) != 0;
}
...
static int cfi_amd_status_check(FAR struct cfi_dev_s *cfi, blkcnt_t block, uint32_t tout)
{
	uint32_t start;
...
static int cfi_amd_write_word(FAR struct cfi_dev_s *cfi, uintptr_t addr, cfiword_t word)
{
	off_t offset = addr - cfi->base_addr;
...
static int cfi_amd_write_buffer(FAR struct cfi_dev_s *cfi, uintptr_t addr, size_t nbytes,
								FAR const uint8_t *buffer)
{
	blkcnt_t block;
	off_t offset = addr - cfi->base_addr;
...
static int cfi_write_word(FAR struct cfi_dev_s *cfi, uintptr_t addr, cfiword_t word)
{
...
static int cfi_write_buffer(FAR struct cfi_dev_s *cfi, uintptr_t addr, size_t nbytes,
							FAR const uint8_t *buffer)
{
	switch (cfi->info.p_id) {
	case CFI_CMDSET_INTEL_EXTENDED:
...
static ssize_t cfi_write_unalign(FAR struct cfi_dev_s *cfi, off_t offset, size_t nbytes,
								 FAR const uint8_t *buffer)
{
	uint8_t i;
	int ret;
...
blkcnt_t cfi_get_total_blocknum(FAR struct cfi_dev_s *cfi)
{
	...
	for (i = 0; i < cfi->info.erase_region_num; i++) {
		blocknum +=
			cfi_get_blocknum(cfi, i) * cfi_get_blocksize(cfi, i) / cfi_get_blocksize(cfi, 0);
	}
...
int cfi_check(FAR struct cfi_dev_s *cfi)
{
	...
	for (cfi->dev_width = 1; cfi->dev_width <= cfi->bankwidth; cfi->dev_width <<= 1) {
		cfi_reset(cfi);
		for (i = 0; i < nitems(g_cfi_query_address); i++) {
			...
			if (cfi_is_equal(cfi, CFI_QRY_OFFSET, 'Q') &&
				...
				for (j = CFI_QRY_OFFSET; j < CFI_ERASE_BLOCK_REGION_INFO_OFFSET; j++) {
					*ptr++ = cfi_read_byte(cfi_addr_map(cfi, 0, j));
				}

				while (j < CFI_ERASE_BLOCK_REGION_INFO_OFFSET + 4 * info->erase_region_num) {
					*ptr++ = cfi_read_byte(cfi_addr_map(cfi, 0, j++));
				}

				cfi->cfi_offset = g_cfi_query_address[i];
				cfi->dev_num = cfi->bankwidth / cfi->dev_width;
				cfi->page_size = (1 << info->max_write_bytes_num) * cfi->dev_num;
...
int cfi_erase(FAR struct cfi_dev_s *cfi, blkcnt_t startblock, blkcnt_t blockcnt)
{
	blkcnt_t i;
	int ret = 0;
...
int cfi_read(FAR struct cfi_dev_s *cfi, off_t offset, size_t nbytes, FAR uint8_t *buffer)
{
	uintptr_t base = cfi->base_addr;
...
int cfi_write(FAR struct cfi_dev_s *cfi, off_t offset, size_t nbytes, FAR const uint8_t *buffer)
{
	ssize_t ret;
