...
static int cfi_mtd_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks);
static ssize_t cfi_mtd_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							 FAR uint8_t *buf);
static ssize_t cfi_mtd_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
							  FAR const uint8_t *buf);
static ssize_t cfi_mtd_read(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
							FAR uint8_t *buffer);
#ifdef CONFIG_MTD_BYTE_WRITE
static ssize_t cfi_mtd_write(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
							 FAR const uint8_t *buffer);
#endif
static int cfi_mtd_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg);
...
static int cfi_mtd_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks)
{
	...
	endblock = cfi_find_block(priv, (startblock + nblocks) * cfi_get_blocksize(priv, 0));
	startblock = cfi_find_block(priv, startblock * cfi_get_blocksize(priv, 0));
...
static ssize_t cfi_mtd_bread(FAR struct mtd_dev_s *dev, off_t startpage, size_t npages,
							 FAR uint8_t *buf)
{
	FAR struct cfi_dev_s *priv = (FAR struct cfi_dev_s *)dev;
...
static ssize_t cfi_mtd_bwrite(FAR struct mtd_dev_s *dev, off_t startpage, size_t npages,
							  FAR const uint8_t *buf)
{
	FAR struct cfi_dev_s *priv = (FAR struct cfi_dev_s *)dev;
	off_t offset = startpage * priv->page_size;
...
static FAR struct mtd_dev_s *cfi_initialize(uintptr_t addr_base, uintptr_t addr_end,
											uint32_t bankwidth)
{
...
int register_cfi_driver(uintptr_t addr_base, uintptr_t addr_end, uint32_t bankwidth, int id)
{
	int ret = -ENXIO;
