...
static int nvs_flash_wrt(FAR struct nvs_fs *fs, uint32_t addr, FAR const void *data, size_t len)
{
	off_t offset;
	int ret;
...
static int nvs_flash_rd(FAR struct nvs_fs *fs, uint32_t addr, FAR void *data, size_t len)
{
	off_t offset;
	int ret;
...
static int nvs_flash_ate_wrt(FAR struct nvs_fs *fs, FAR const struct nvs_ate *entry)
{
	int rc;
...
static int nvs_flash_data_wrt(FAR struct nvs_fs *fs, FAR const void *data, size_t len)
{
	int rc;
...
static int nvs_flash_ate_rd(FAR struct nvs_fs *fs, uint32_t addr, FAR struct nvs_ate *entry)
{
...
static int nvs_flash_block_cmp(FAR struct nvs_fs *fs, uint32_t addr, FAR const void *data,
							   size_t len)
{
...
static int nvs_flash_direct_cmp(FAR struct nvs_fs *fs, uint32_t addr1, uint32_t addr2, size_t len)
{
	int rc;
	size_t bytes_to_cmp;
...
static int nvs_flash_cmp_const(FAR struct nvs_fs *fs, uint32_t addr, uint8_t value, size_t len)
{
	int rc;
	size_t bytes_to_cmp;
...
static int nvs_flash_block_move(FAR struct nvs_fs *fs, uint32_t addr, size_t len)
{
	int rc;
...
static int nvs_flash_erase_block(FAR struct nvs_fs *fs, uint32_t addr)
{
	...
	rc = MTD_ERASE(fs->mtd, CONFIG_MTD_BLOCKSIZE_MULTIPLE * (addr >> ADDR_BLOCK_SHIFT),
...
static void nvs_ate_crc8_update(FAR struct nvs_ate *entry)
{
	...
	ate_crc = crc8part((FAR const uint8_t *)entry, offsetof(struct nvs_ate, crc8), 0xff);
	entry->crc8 = ate_crc;
}
...
static int nvs_ate_cmp_const(FAR const struct nvs_ate *entry, uint8_t value)
{
...
static int nvs_ate_valid(FAR struct nvs_fs *fs, FAR const struct nvs_ate *entry)
{
	if (nvs_ate_crc8_check(entry) || entry->offset >= (fs->blocksize - sizeof(struct nvs_ate))) {
		return 0;
	}
...
static int nvs_close_ate_valid(FAR struct nvs_fs *fs, FAR const struct nvs_ate *entry)
{
	if (!nvs_ate_valid(fs, entry) || entry->len != 0 || entry->id != NVS_SPECIAL_ATE_ID) {
		return 0;
	}
...
int nvs_flash_write_multi_blk(FAR struct nvs_fs *fs, const uint8_t *addr, size_t size)
{
	size_t blk_cnt;
	int left;
	int rc;
...
static int nvs_flash_wrt_entry(FAR struct nvs_fs *fs, uint32_t id, FAR const uint8_t *key,
							   size_t key_size, FAR const void *data, size_t len)
{
	int rc;
	...
	if (rc) {
		...
		copy_len = (left + len) <= NVS_ALIGN_SIZE ? len : (NVS_ALIGN_SIZE - left);
...
static int nvs_recover_last_ate(FAR struct nvs_fs *fs, FAR uint32_t *addr)
{
	...
	*addr -= sizeof(struct nvs_ate);
	...
	while (ate_end_addr > data_end_addr) {
		...
		if (nvs_ate_valid(fs, &end_ate)) {
			...
			data_end_addr += end_ate.offset + NVS_ALIGN_UP(end_ate.key_len + end_ate.len);
			*addr = ate_end_addr;
		}
...
static int nvs_prev_ate(FAR struct nvs_fs *fs, FAR uint32_t *addr, FAR struct nvs_ate *ate)
{
	int rc;
	...
	*addr += sizeof(struct nvs_ate);
	if (((*addr) & ADDR_OFFS_MASK) != (fs->blocksize - sizeof(struct nvs_ate))) {
		return 0;
	}
...
static int nvs_block_close(FAR struct nvs_fs *fs)
{
	...
	close_ate.len = 0;
	close_ate.key_len = 0;
	close_ate.offset = (fs->ate_wra + sizeof(struct nvs_ate)) & ADDR_OFFS_MASK;
...
static int nvs_expire_ate(FAR struct nvs_fs *fs, uint32_t addr)
{
	...
	return nvs_flash_wrt(fs, addr + offsetof(struct nvs_ate, expired), expired, sizeof(expired));
}
...
static int nvs_gc(FAR struct nvs_fs *fs)
{
	...
	do {
		...
		if (gc_ate.id != NVS_SPECIAL_ATE_ID) {
			...
			rc = nvs_flash_block_move(fs, data_addr, NVS_ALIGN_UP(gc_ate.key_len + gc_ate.len));
			if (rc) {
				return rc;
			}
...
static int nvs_startup(FAR struct nvs_fs *fs)
{
	...
	rc = MTD_IOCTL(fs->mtd, MTDIOC_ERASESTATE, (unsigned long)((uintptr_t)&fs->erasestate));
	if (rc < 0) {
	...
	for (i = 0; i < fs->nblocks; i++) {
		addr = (i << ADDR_BLOCK_SHIFT) + (uint16_t)(fs->blocksize - sizeof(struct nvs_ate));
		rc = nvs_flash_cmp_const(fs, addr, fs->erasestate, sizeof(struct nvs_ate));
		fwarn("rc=%d\n", rc);
		if (rc) {
			/* Closed block */
	...
	if (i == fs->nblocks) {
		...
		rc = nvs_flash_cmp_const(fs, addr - sizeof(struct nvs_ate), fs->erasestate,
	...
	while (fs->ate_wra >= fs->data_wra) {
		...
		if (nvs_ate_valid(fs, &last_ate)) {
			...
			fs->data_wra = addr & ADDR_BLOCK_MASK;
			fs->data_wra += last_ate.offset + NVS_ALIGN_UP(last_ate.key_len + last_ate.len);
	...
	if (rc) {
		...
		while ((addr & ADDR_OFFS_MASK) < (fs->blocksize - sizeof(struct nvs_ate))) {
			rc = nvs_flash_ate_rd(fs, addr, &gc_done_ate);
			if (rc) {
				return rc;
			}

			if (nvs_ate_valid(fs, &gc_done_ate) && (gc_done_ate.id == NVS_SPECIAL_ATE_ID) &&
				(gc_done_ate.len == 0)) {
				gc_done_marker = true;
				break;
			}
	...
	while (1) {
		...
		if (nvs_ate_valid(fs, &last_ate) && (last_ate.id != NVS_SPECIAL_ATE_ID)) {
			...
			while (1) {
				...
				if (nvs_ate_valid(fs, &second_ate) && second_ate.id == last_ate.id &&
					...
					if ((second_ate.key_len == last_ate.key_len) &&
						!nvs_flash_direct_cmp(fs, (last_addr & ADDR_BLOCK_MASK) + last_ate.offset,
...
static ssize_t nvs_read_entry(FAR struct nvs_fs *fs, FAR const uint8_t *key, size_t key_size,
							  FAR void *data, size_t len, FAR uint32_t *ate_addr)
{
	int rc;
	uint32_t wlk_addr;
	uint32_t rd_addr;
	...
	do {
		...
		if ((wlk_ate.id == hash_id) && (nvs_ate_valid(fs, &wlk_ate))) {
			if ((wlk_ate.key_len == key_size) &&
				(!nvs_flash_block_cmp(fs, (rd_addr & ADDR_BLOCK_MASK) + wlk_ate.offset, key,
									  key_size))) {
...
static ssize_t nvs_write(FAR struct nvs_fs *fs, FAR struct config_data_s *pdata)
{
	int rc;
	int gc_count;
	size_t data_size;
	...
	if (prev_found) {
		...
		if (pdata->len == 0) {
		...
		} else if (pdata->len == wlk_ate.len && wlk_ate.expired[0] == fs->erasestate) {
			...
			rd_addr += wlk_ate.offset + wlk_ate.key_len;
			rc = nvs_flash_block_cmp(fs, rd_addr, pdata->configdata, pdata->len);
			if (rc <= 0) {
				return rc;
			}
		}
	...
	while (1) {
		...
		if (fs->ate_wra >= fs->data_wra + required_space) {
			...
			if (prev_found && wlk_ate.expired[0] == fs->erasestate) {
				...
				if (gc_count >=
					fs->nblocks - 1 -
						(block_to_write_befor_gc + fs->nblocks - (hist_addr >> ADDR_BLOCK_SHIFT)) %
							fs->nblocks) {
					rc = nvs_read_entry(fs, key, key_size, NULL, 0, &hist_addr);
			...
			rc = nvs_flash_wrt_entry(fs, hash_id, key, key_size, pdata->configdata, pdata->len);
			if (rc) {
...
static ssize_t nvs_read(FAR struct nvs_fs *fs, FAR struct config_data_s *pdata)
{
	size_t key_size;
	ssize_t ret;
	...
	ret = nvs_read_entry(fs, key, key_size, pdata->configdata, pdata->len, NULL);
	if (ret > 0) {
		pdata->len = ret;
		return 0;
	}
...
static int nvs_next(FAR struct nvs_fs *fs, FAR struct config_data_s *pdata, bool first)
{
	int rc;
	...
	do {
		...
		if (nvs_ate_valid(fs, &step_ate) && step_ate.id != NVS_SPECIAL_ATE_ID &&
...
#ifdef CONFIG_MTD_CONFIG_NAMED
	rc = nvs_flash_rd(fs, (rd_addr & ADDR_BLOCK_MASK) + step_ate.offset, key,
...
#else
	rc = nvs_flash_rd(fs, (rd_addr & ADDR_BLOCK_MASK) + step_ate.offset, key,
					  MIN(sizeof(key), step_ate.key_len));
	if (rc) {
		ferr("Key read failed, rc=%d\n", rc);
		return rc;
	}
	...
	rc = nvs_flash_rd(fs, (rd_addr & ADDR_BLOCK_MASK) + step_ate.offset + step_ate.key_len,
					  pdata->configdata, MIN(pdata->len, step_ate.len));
	if (rc) {
...
int mtdconfig_register_by_path(FAR struct mtd_dev_s *mtd, FAR const char *path)
{
	int ret;
