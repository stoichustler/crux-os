...
#define GD55_SR_BP_TOP(b) (((b + 1) << GD55_SR_BP_SHIFT) | GD55_STATUS_TB_TOP)
#define GD55_SR_BP_BOTTOM(b) (((b + 1) << GD55_SR_BP_SHIFT) | GD55_STATUS_TB_BOTTOM)
...
#define GD55_ODT_DS_REG                                                                            \
	3								 /* On-die termination and driver                              \
...
#define SET_VALID(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= GD55_CACHE_VALID;                                                            \
	} while (0)
#define SET_DIRTY(p)                                                                               \
	do {                                                                                           \
		(p)->flags |= GD55_CACHE_DIRTY;                                                            \
	} while (0)
#define SET_ERASED(p)                                                                              \
	do {                                                                                           \
		(p)->flags |= GD55_CACHE_ERASED;                                                           \
	} while (0)
...
static int gd55_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks);
static ssize_t gd55_bread(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
						  FAR uint8_t *buf);
static ssize_t gd55_bwrite(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks,
						   FAR const uint8_t *buf);
static ssize_t gd55_read(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes,
						 FAR uint8_t *buffer);
...
static int gd55_command_read(FAR struct gd55_dev_s *priv, uint8_t cmd, FAR void *buffer,
							 size_t buflen);
static int gd55_command(FAR struct gd55_dev_s *priv, uint8_t cmd);
static int gd55_command_address(FAR struct gd55_dev_s *priv, uint8_t cmd, off_t addr,
								uint8_t addrlen);
static int gd55_readid(FAR struct gd55_dev_s *priv);
static int gd55_protect(FAR struct gd55_dev_s *priv, off_t startblock, size_t nblocks);
static int gd55_unprotect(FAR struct gd55_dev_s *priv, off_t startblock, size_t nblocks);
static bool gd55_isprotected(FAR struct gd55_dev_s *priv, off_t addr, uint8_t status);
static int gd55_read_bytes(FAR struct gd55_dev_s *priv, FAR uint8_t *buffer, off_t address,
						   size_t buflen);
...
static int gd55_command_write(FAR struct gd55_dev_s *priv, uint8_t cmd, FAR const void *buffer,
							  size_t buflen);
static void gd55_write_enable(FAR struct gd55_dev_s *priv);
static int gd55_write_page(FAR struct gd55_dev_s *priv, FAR const uint8_t *buffer, off_t address,
						   size_t buflen);
...
static FAR uint8_t *gd55_read_cache(FAR struct gd55_dev_s *priv, off_t sector);
static void gd55_erase_cache(FAR struct gd55_dev_s *priv, off_t sector);
static int gd55_write_cache(FAR struct gd55_dev_s *priv, FAR const uint8_t *buffer, off_t sector,
							size_t nsectors);
#else
...
static int gd55_erase_64kblock(FAR struct gd55_dev_s *priv, off_t sector)
{
	...
	gd55_write_enable(priv);
	gd55_command_address(priv, GD55_BE64, addr, mode_4byte_addr ? 4 : 3);
...
static void gd55_write_status1(FAR struct gd55_dev_s *priv)
{
	...
	gd55_command_write(priv, GD55_WRSR1, (FAR const void *)priv->cmdbuf, 1);
...
static int gd55_erase(FAR struct mtd_dev_s *dev, off_t startblock, size_t nblocks)
{
	...
	while (blocksleft > 0) {
		...
		if (((startblock & (sectorsper64kblock - 1)) == 0) && (blocksleft >= sectorsper64kblock)) {
			/* Erase 64k block */
...
#ifdef CONFIG_MTD_GD55_SECTOR512
	nbytes =
		gd55_read(dev, startblock << GD55_SECTOR512_SHIFT, nblocks << GD55_SECTOR512_SHIFT, buf);
	if (nbytes > 0) {
		nbytes >>= GD55_SECTOR512_SHIFT;
	}
#else
	nbytes = gd55_read(dev, startblock << GD55_PAGE_SHIFT, nblocks << GD55_PAGE_SHIFT, buf);
	if (nbytes > 0) {
...
#else
	ret = gd55_write_page(priv, buf, startblock << GD55_PAGE_SHIFT, nblocks << GD55_PAGE_SHIFT);
	if (ret < 0) {
...
static int gd55_ioctl(FAR struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case MTDIOC_GEOMETRY: {
		...
		if (geo) {
			...
			geo->neraseblocks = priv->nsectors << (GD55_SECTOR_SHIFT - GD55_SECTOR512_SHIFT);
#else
	...
	case BIOC_PARTINFO: {
		...
		if (info != NULL) {
#ifdef CONFIG_MTD_GD55_SECTOR512
			info->numsectors = priv->nsectors << (GD55_SECTOR_SHIFT - GD55_SECTOR512_SHIFT);
			info->sectorsize = GD55_SECTOR512_SIZE;
#else
			info->numsectors = priv->nsectors << (GD55_SECTOR_SHIFT - GD55_PAGE_SHIFT);
...
static int gd55_protect(FAR struct gd55_dev_s *priv, off_t startblock, size_t nblocks)
{
	...
	if (nblocks == (priv->nsectors * GD55_SECTORS_PER_BP_BLK)) {
		...
		blkmask = (startblock == 0) ? GD55_SR_BP_BOTTOM(blkmask) : GD55_SR_BP_TOP(blkmask);
	}

	/* startblock must be first block, or (memory top - nblocks) */

	if ((startblock != 0) &&
		(startblock != (((priv->nsectors << GD55_SECTOR_SHIFT) / GD55_MIN_BP_BLKS) - nblocks))) {
		return -EINVAL;
	}
...
static FAR uint8_t *gd55_read_cache(FAR struct gd55_dev_s *priv, off_t sector)
{
	...
	if (!IS_VALID(priv) || esectno != priv->esectno) {
		...
		ret = gd55_read_bytes(priv, priv->sector, (esectno << GD55_SECTOR_SHIFT), GD55_SECTOR_SIZE);
		if (ret < 0) {
...
static void gd55_erase_cache(FAR struct gd55_dev_s *priv, off_t sector)
{
	...
	if (!IS_ERASED(priv)) {
		off_t esectno = sector >> (GD55_SECTOR_SHIFT - GD55_SECTOR512_SHIFT);
							...
							size_t nsectors)
{
	...
	for (; nsectors > 0; nsectors--) {
		...
		finfo("address: %08jx nbytes: %d 0x%04" PRIx32 "\n",
			  (intmax_t)(sector << GD55_SECTOR512_SHIFT), GD55_SECTOR512_SIZE,
			  *(FAR uint32_t *)buffer);
...
FAR struct mtd_dev_s *gd55_initialize(FAR struct qspi_dev_s *qspi, bool unprotect)
{
