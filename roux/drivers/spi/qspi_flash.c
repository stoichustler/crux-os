...
static uint32_t qspi_flash_setfrequency(FAR struct qspi_dev_s *dev, uint32_t frequency);
static void qspi_flash_setmode(FAR struct qspi_dev_s *dev, enum qspi_mode_e mode);
static void qspi_flash_setbits(FAR struct qspi_dev_s *dev, int nbits);
static int qspi_flash_command(FAR struct qspi_dev_s *dev, FAR struct qspi_cmdinfo_s *cmd);
static int qspi_flash_memory(FAR struct qspi_dev_s *dev, FAR struct qspi_meminfo_s *mem);
...
static void qspi_flash_writeword(FAR struct qspi_flashdev_s *priv, uint16_t data,
...
struct qspi_flashdev_s g_qspidev = {{&g_qspiops}};
...
int qspi_flash_memory(FAR struct qspi_dev_s *dev, FAR struct qspi_meminfo_s *mem)
{
...
static int qspi_flash_command(FAR struct qspi_dev_s *dev, FAR struct qspi_cmdinfo_s *cmdinfo)
{
	FAR uint8_t *p_buf;
	FAR struct qspi_flashdev_s *priv = (FAR struct qspi_flashdev_s *)dev;
