...
static void spi_flash_select(FAR struct spi_dev_s *dev, uint32_t devid, bool selected);
...
static void spi_flash_sndblock(FAR struct spi_dev_s *dev, FAR const void *txbuffer, size_t nwords);
static void spi_flash_recvblock(FAR struct spi_dev_s *dev, FAR void *rxbuffer, size_t nwords);
#endif
...
static void spi_flash_writeword(FAR struct spi_flash_dev_s *priv, uint16_t data)
{
	...
	case SPI_FLASH_STATE_PP4:
		...
		if ((priv->address & CONFIG_SPI_FLASH_PAGESIZE_MASK) == CONFIG_SPI_FLASH_PAGESIZE_MASK) {
