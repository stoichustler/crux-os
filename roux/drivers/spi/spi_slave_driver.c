...
static ssize_t spi_slave_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static size_t spi_slave_getdata(FAR struct spi_slave_dev_s *dev, FAR const void **data);
static size_t spi_slave_receive(FAR struct spi_slave_dev_s *dev, FAR const void *data,
								size_t nwords);
static void spi_slave_notify(FAR struct spi_slave_dev_s *dev, spi_slave_state_t state);
...
static int spi_slave_open(FAR struct file *filep)
{
	...
	if (priv->crefs == 0) {
		SPIS_CTRLR_BIND(priv->ctrlr, (FAR struct spi_slave_dev_s *)priv,
						CONFIG_SPI_SLAVE_DRIVER_MODE, CONFIG_SPI_SLAVE_DRIVER_WIDTH);
	}
...
static ssize_t spi_slave_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	enqueued_bytes = WORDS2BYTES(SPIS_CTRLR_ENQUEUE(priv->ctrlr, buffer, BYTES2WORDS(buflen)));
...
#ifdef CONFIG_SPI_SLAVE_DRIVER_COLORIZE_TX_BUFFER
	memset(priv->tx_buffer, CONFIG_SPI_SLAVE_DRIVER_COLORIZE_PATTERN,
