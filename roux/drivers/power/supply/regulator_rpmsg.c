...
static int regulator_rpmsg_ept_cb(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
								  uint32_t src, FAR void *priv);

static int regulator_rpmsg_enable_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
										  size_t len, uint32_t src, FAR void *priv);
static int regulator_rpmsg_disable_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
										   size_t len, uint32_t src, FAR void *priv);
static int regulator_rpmsg_getvol_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
										  size_t len, uint32_t src, FAR void *priv);
static int regulator_rpmsg_setvol_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
										  size_t len, uint32_t src, FAR void *priv);
static int regulator_rpmsg_isenabled_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
											 size_t len, uint32_t src, FAR void *priv);

static void regulator_rpmsg_client_created(struct rpmsg_device *rdev, FAR void *priv);
static void regulator_rpmsg_client_destroy(struct rpmsg_device *rdev, FAR void *priv);

static bool regulator_rpmsg_server_match(FAR struct rpmsg_device *rdev, FAR void *priv,
										 FAR const char *name, uint32_t dest);
static void regulator_rpmsg_server_bind(FAR struct rpmsg_device *rdev, FAR void *priv,
										FAR const char *name, uint32_t dest);

static int regulator_rpmsg_set_voltage(FAR struct regulator_dev_s *rdev, int min_uv, int max_uv,
...
static struct list_node g_regulator_rpmsg_client = LIST_INITIAL_VALUE(g_regulator_rpmsg_client);
...
static FAR struct regulator_rpmsg_client_s *regulator_rpmsg_get_client(FAR const char *name)
{
	...
	list_for_every_entry(&g_regulator_rpmsg_client, client, struct regulator_rpmsg_client_s, node)
	{
	...
	rpmsg_register_callback(client, regulator_rpmsg_client_created, regulator_rpmsg_client_destroy,
							NULL, NULL);
	return client;
...
static FAR struct rpmsg_endpoint *regulator_rpmsg_get_ept(FAR const char **name)
{
	FAR struct regulator_rpmsg_client_s *client;
	int ret = 0;
...
static FAR struct regulator_rpmsg_s *regulator_rpmsg_get_reg(FAR struct rpmsg_endpoint *ept,
															 FAR const char *name)
{
	...
	list_for_every_entry(regulator_list, reg, struct regulator_rpmsg_s, node)
	{
...
static void regulator_rpmsg_client_created(struct rpmsg_device *rdev, FAR void *priv)
{
	...
	if (strcmp(client->cpuname, rpmsg_get_cpuname(rdev)) == 0) {
		...
		rpmsg_create_ept(&client->ept, rdev, REGULATOR_RPMSG_EPT_NAME, RPMSG_ADDR_ANY,
						 RPMSG_ADDR_ANY, regulator_rpmsg_ept_cb, NULL);
		nxsem_post(&client->sem);
	}
}
...
static void regulator_rpmsg_server_ept_release(FAR struct rpmsg_endpoint *ept)
{
	FAR struct regulator_rpmsg_server_s *server = ept->priv;
	FAR struct regulator_rpmsg_s *reg;
	FAR struct regulator_rpmsg_s *tmp;

	list_for_every_entry_safe(&server->regulator_list, reg, tmp, struct regulator_rpmsg_s, node)
	{
		if (reg->enable) {
										...
										FAR const char *name, uint32_t dest)
{
	...
	rpmsg_create_ept(&server->ept, rdev, name, RPMSG_ADDR_ANY, RPMSG_ADDR_ANY,
					 regulator_rpmsg_ept_cb, rpmsg_destroy_ept);
}
								  ...
								  uint32_t src, FAR void *priv)
{
	...
	if (cookie && header->response) {
		...
		ret = 0;
	} else if (cmd < nitems(g_regulator_rpmsg_handler) && g_regulator_rpmsg_handler[cmd]) {
		header->response = 1;
										  ...
										  size_t len, uint32_t src, FAR void *priv)
{
	...
	FAR struct regulator_rpmsg_s *reg = regulator_rpmsg_get_reg(ept, msg->name);
	int ret = -ENOENT;
										  ...
										  size_t len, uint32_t src, FAR void *priv)
{
	...
	if (reg) {
		ret = regulator_set_voltage(reg->regulator, msg->min_uv, msg->max_uv);
	}
...
static int regulator_rpmsg_sendrecv(FAR struct rpmsg_endpoint *ept, uint32_t command,
									FAR struct regulator_rpmsg_header_s *msg, int len)
{
...
static int regulator_rpmsg_enable(FAR struct regulator_dev_s *rdev)
{
	...
	return regulator_rpmsg_sendrecv(ept, REGULATOR_RPMSG_ENABLE,
									(FAR struct regulator_rpmsg_header_s *)msg, len);
}
...
static int regulator_rpmsg_is_enabled(FAR struct regulator_dev_s *rdev)
{
	...
	return regulator_rpmsg_sendrecv(ept, REGULATOR_RPMSG_IS_ENABLED,
									(struct regulator_rpmsg_header_s *)msg, len);
}
...
{
	return rpmsg_register_callback(NULL, NULL, NULL, regulator_rpmsg_server_match,
