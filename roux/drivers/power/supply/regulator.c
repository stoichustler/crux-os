...
static int regulator_check_consumers(FAR struct regulator_dev_s *rdev, FAR int *min_uv,
									 FAR int *max_uv);
static FAR struct regulator_dev_s *regulator_dev_lookup(const char *supply);
static int regulator_map_voltage_iterate(FAR struct regulator_dev_s *rdev, int min_uv, int max_uv);
static int _regulator_get_voltage(FAR struct regulator_dev_s *rdev);
static int _regulator_do_set_voltage(FAR struct regulator_dev_s *rdev, int min_uv, int max_uv);
static int _regulator_set_voltage_unlocked(FAR struct regulator_s *regulator, int min_uv,
										   int max_uv);
									 ...
									 FAR int *max_uv)
{
	...
	list_for_every_entry(&rdev->consumer_list, regulator, struct regulator_s, list)
	{
...
#ifdef CONFIG_PM
static void regulator_pm_notify(struct pm_callback_s *cb, int domain, enum pm_state_e pmstate)
{
	...
	if (state) {
		...
		if (rdev->ops->set_suspend_mode && state->mode != REGULATOR_MODE_INVALID) {
...
int regulator_disable_deferred(FAR struct regulator_s *regulator, int ms)
{
	...
	return work_queue(LPWORK, (FAR struct work_s *)&regulator, (worker_t)regulator_disable,
					  regulator, MSEC2TICK(ms));
}
...
int regulator_set_voltage(FAR struct regulator_s *regulator, int min_uv, int max_uv)
{
	FAR struct regulator_dev_s *rdev;
	irqstate_t flags;
	int ret = 0;
...
int regulator_set_mode(FAR struct regulator_s *regulator, enum regulator_mode_e mode)
{
	FAR struct regulator_dev_s *rdev = regulator->rdev;
...
FAR struct regulator_dev_s *regulator_register(FAR const struct regulator_desc_s *regulator_desc,
											   ...
											   FAR void *priv)
{
	...
	} else if (!rdev->desc->boot_on && !rdev->desc->always_on && _regulator_is_enabled(rdev)) {
		_regulator_do_disable(rdev);
	}

bypass:
	if (rdev->desc->apply_uv) {
		_regulator_do_set_voltage(rdev, rdev->desc->min_uv, rdev->desc->max_uv);
	}
