...
static int powerled_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case PWRIOC_START: {
		...
		if (powerled->limits.lock == false || powerled->limits.current <= 0) {
	...
	case PWRIOC_SET_LIMITS: {
		FAR struct powerled_limits_s *limits = (FAR struct powerled_limits_s *)((uintptr_t)arg);
	...
	case PWRIOC_GET_STATE: {
		FAR struct powerled_state_s *state = (FAR struct powerled_state_s *)((uintptr_t)arg);
	...
	case PWRIOC_SET_PARAMS: {
		FAR struct powerled_params_s *params = (FAR struct powerled_params_s *)((uintptr_t)arg);
		...
		if (params->brightness < 0.0 || params->brightness > 100.0 || params->frequency < 0.0 ||
			params->duty < 0.0 || params->duty > 100.0) {
			pwrerr("ERROR: powerled invalid parameters %f %f %f\n", params->brightness,
				   params->frequency, params->duty);
