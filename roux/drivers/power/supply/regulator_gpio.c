...
static int regulator_gpio_enable(FAR struct regulator_dev_s *rdev)
{
	...
	return IOEXP_WRITEPIN(priv->iodev, rdev->desc->enable_reg, !!rdev->desc->enable_mask);
}
...
int regulator_gpio_init(FAR struct ioexpander_dev_s *iodev, FAR const struct regulator_desc_s *desc)
{
	...
	ret = IOEXP_SETDIRECTION(priv->iodev, desc->enable_reg, IOEXPANDER_DIRECTION_OUT);
	if (ret >= 0) {
		priv->rdev = regulator_register(desc, &g_regulator_gpio_ops, priv);
		if (priv->rdev == NULL) {
			ret = -EINVAL;
		}
	}
