...
static int smps_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case PWRIOC_START: {
		...
		if ((smps->limits.lock == false) ||
			(smps->limits.v_in <= 0 && smps->limits.v_out <= 0 && smps->limits.i_in <= 0 &&
			 smps->limits.i_out <= 0 && smps->limits.p_in <= 0 && smps->limits.p_out <= 0)) {
	...
	case PWRIOC_SET_LIMITS: {
		FAR struct smps_limits_s *limits = (FAR struct smps_limits_s *)((uintptr_t)arg);
	...
	case PWRIOC_GET_STATE: {
		FAR struct smps_state_s *state = (FAR struct smps_state_s *)((uintptr_t)arg);
	...
	case PWRIOC_SET_PARAMS: {
		FAR struct smps_params_s *params = (FAR struct smps_params_s *)((uintptr_t)arg);
