...
struct pm_activity_governor_s g_pm_activity_governor = {
	{CONFIG_PM_GOVERNOR_IDLEENTER_COUNT, CONFIG_PM_GOVERNOR_STANDBYENTER_COUNT,
	 CONFIG_PM_GOVERNOR_SLEEPENTER_COUNT},
	{CONFIG_PM_GOVERNOR_IDLEENTER_THRESH, CONFIG_PM_GOVERNOR_STANDBYENTER_THRESH,
	 CONFIG_PM_GOVERNOR_SLEEPENTER_THRESH},
	{CONFIG_PM_GOVERNOR_IDLEEXIT_THRESH, CONFIG_PM_GOVERNOR_STANDBYEXIT_THRESH,
...
static void governor_timer(int domain, enum pm_state_e newstate)
{
	...
	static const int pmtick[3] = {TIME_SLICE_TICKS * CONFIG_PM_GOVERNOR_IDLEENTER_COUNT,
	...
	if (newstate < PM_SLEEP && dq_empty(&pdom->wakelock[newstate])) {
		sclock_t delay = pmtick[newstate] + pdomstate->btime - clock_systime_ticks();
		...
		if (!WDOG_ISACTIVE(&pdomstate->wdog) || abs(delay - left) > PM_TIMER_GAP) {
			wd_start(&pdomstate->wdog, delay, governor_timer_cb, (wdparm_t)domain);
		}
	} else {
