...
static void pm_stats_preparefail(int domain, FAR struct pm_callback_s *callback, int newstate,
								 int ret)
{
	struct timespec ts;
	...
	if (pf->state != PM_RESTORE) {
		...
		clock_timespec_add(&ts, &pf->duration[pf->state], &pf->duration[pf->state]);
		pf->state = PM_RESTORE;
	}
...
static int pm_prepall(int domain, enum pm_state_e newstate, bool restore)
{
	...
	if (newstate <= pdom->state) {
		...
		for (entry = dq_peek(&pdom->registry); entry && ret == OK; entry = dq_next(entry)) {
		...
		for (entry = dq_tail(&pdom->registry); entry && ret == OK; entry = dq_prev(entry)) {
			/* Is the prepare callback supported? */
...
static inline void pm_changeall(int domain, enum pm_state_e newstate)
{
	...
	if (newstate <= pdom->state) {
		...
		for (entry = dq_peek(&pdom->registry); entry; entry = dq_next(entry)) {
		...
		for (entry = dq_tail(&pdom->registry); entry; entry = dq_prev(entry)) {
			/* Is the notification callback supported? */
...
int pm_changestate(int domain, enum pm_state_e newstate)
{
	...
	pm_stats(&g_pmdomains[domain], g_pmdomains[domain].state, newstate);
