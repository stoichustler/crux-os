...
#define STFMT                                                                                      \
	"%-18s %8" PRIu64 "s %3" PRIu64 "%% %8" PRIu64 "s %3" PRIu64 "%% %8" PRIu64 "s %3" PRIu64 "%%" \
	"\n"
#define PFFMT                                                                                      \
	"%-18p %8" PRIu64 "s %3" PRIu64 "%% %8" PRIu64 "s %3" PRIu64 "%% %8" PRIu64 "s %3" PRIu64 "%%" \
	"\n"
#define WAFMT "%-25s %-14s %-14" PRIu32 " %" PRIu64 "s\n"
#else
#define STFMT                                                                                      \
	"%-18s %8" PRIu32 "s %3" PRIu32 "%% %8" PRIu32 "s %3" PRIu32 "%% %8" PRIu32 "s %3" PRIu32 "%%" \
	"\n"
#define PFFMT                                                                                      \
	"%-18p %8" PRIu32 "s %3" PRIu32 "%% %8" PRIu32 "s %3" PRIu32 "%% %8" PRIu32 "s %3" PRIu32 "%%" \
	"\n"
...
typedef ssize_t (*pm_read_t)(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static int pm_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int pm_close(FAR struct file *filep);
static ssize_t pm_read_state(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t pm_read_wakelock(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t pm_read_preparefail(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t pm_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static int pm_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry);
...
static FAR const char *g_pm_state[PM_COUNT] = {"normal", "idle", "standby", "sleep"};
...
static int pm_get_file_index(FAR const char *relpath)
{
	...
	for (i = 0; i < nitems(g_pm_files); i++) {
		if (strncmp(relpath, g_pm_files[i].name, strlen(g_pm_files[i].name)) == 0) {
			return i;
		}
	}
...
static ssize_t pm_read_state(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	copysize = procfs_memcpy(pmfile->line, linesize, buffer, buflen, &offset);
	...
	for (state = 0; state < PM_COUNT && totalsize < buflen; state++) {
		...
		linesize = snprintf(pmfile->line, PM_LINELEN, STFMT, g_pm_state[state], wake[state],
							100 * wake[state] / sum, sleep[state], 100 * sleep[state] / sum, total,
							100 * total / sum);
		buffer += copysize;
		buflen -= copysize;
...
static ssize_t pm_read_wakelock(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	linesize = snprintf(pmfile->line, PM_LINELEN, WAHDR, pmfile->domain);
	copysize = procfs_memcpy(pmfile->line, linesize, buffer, buflen, &offset);
	...
	for (; entry && totalsize < buflen; entry = dq_next(entry)) {
		FAR struct pm_wakelock_s *wakelock = container_of(entry, struct pm_wakelock_s, fsnode);
		...
		linesize = snprintf(pmfile->line, PM_LINELEN, WAFMT, wakelock->name,
							g_pm_state[wakelock->state], wakelock->count, time);
...
static ssize_t pm_read_preparefail(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	for (entry = dq_peek(&dom->registry); entry; entry = dq_next(entry)) {
	...
	for (entry = dq_peek(&dom->registry); entry; entry = dq_next(entry)) {
		...
		linesize =
			snprintf(pmfile->line, PM_LINELEN, PFFMT, cb->prepare, pf->duration[PM_IDLE].tv_sec,
					 100 * pf->duration[PM_IDLE].tv_sec / sum, pf->duration[PM_STANDBY].tv_sec,
					 100 * pf->duration[PM_STANDBY].tv_sec / sum, pf->duration[PM_SLEEP].tv_sec,
...
static int pm_readdir(FAR struct fs_dirent_s *dir, FAR struct dirent *entry)
{
	...
	snprintf(entry->d_name, NAME_MAX + 1, "%s%d", g_pm_files[fpos].name, domain);
