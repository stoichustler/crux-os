...
static void stability_governor_statechanged(int domain, enum pm_state_e newstate);
...
static void stability_governor_statechanged(int domain, enum pm_state_e newstate)
{
	if (newstate == PM_RESTORE) {
		...
		if (thresh > 0 && state != newstate) {
			wd_start(&g_stability_governor.domain[domain].wdog, thresh, stability_governor_timer_cb,
					 0);
		}
	}
}
...
static enum pm_state_e stability_governor_checkstate(int domain)
{
	...
	if (g_stability_governor_thresh[state] > 0 && (!wdog_wakeup || state_pending != state)) {
		state = pdom->state;
