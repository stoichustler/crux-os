...
#define GOLDFISH_BATTERY_READ(data, addr) (*(FAR volatile uint32_t *)(data->reg_base + addr))
...
static int goldfish_charger_online(FAR struct battery_gauge_dev_s *dev, FAR bool *status);
static int goldfish_battery_state(FAR struct battery_gauge_dev_s *dev, FAR int *status);
static int goldfish_battery_voltage(FAR struct battery_gauge_dev_s *dev, FAR b16_t *value);
static int goldfish_battery_capacity(FAR struct battery_gauge_dev_s *dev, FAR b16_t *value);
static int goldfish_battery_current(FAR struct battery_gauge_dev_s *dev, FAR b16_t *value);
static int goldfish_battery_temp(FAR struct battery_gauge_dev_s *dev, FAR b8_t *value);
...
static void goldfish_battery_work(FAR void *arg)
{
	...
	uint32_t mask = BATTERY_STATE_CHANGED | BATTERY_VOLTAGE_CHANGED | BATTERY_CURRENT_CHANGED |
					BATTERY_CAPACITY_CHANGED | BATTERY_TEMPERATURE_CHANGED | BATTERY_ONLINE_CHANGED;
	int ret;
...
static int goldfish_battery_interrupt(int irq, FAR void *context, void *arg)
{
	...
	if (status) {
		...
		if (ret < 0) {
			baterr("battery interrupt %" PRIu32 "work_battery ret:%d\n", status, ret);
		}
	}
