...
static ssize_t bat_monitor_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t bat_monitor_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int bat_monitor_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int bat_monitor_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int battery_monitor_notify(FAR struct battery_monitor_priv_s *priv, uint32_t mask)
{
...
int battery_monitor_changed(FAR struct battery_monitor_dev_s *dev, uint32_t mask)
{
	...
	dev->mask |= mask;
	list_for_every_entry(&dev->flist, priv, struct battery_monitor_priv_s, node)
	{
