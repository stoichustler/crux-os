...
static int bat_gauge_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int battery_gauge_notify(FAR struct battery_gauge_priv_s *priv, uint32_t mask)
{
...
int battery_gauge_changed(FAR struct battery_gauge_dev_s *dev, uint32_t mask)
{
	...
	dev->mask |= mask;
	list_for_every_entry(&dev->flist, priv, struct battery_gauge_priv_s, node)
	{
