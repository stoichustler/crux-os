...
static int bat_charger_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int battery_charger_notify(FAR struct battery_charger_priv_s *priv, uint32_t mask)
{
...
int battery_charger_changed(FAR struct battery_charger_dev_s *dev, uint32_t mask)
{
	...
	dev->mask |= mask;
	list_for_every_entry(&dev->flist, priv, struct battery_charger_priv_s, node)
	{
