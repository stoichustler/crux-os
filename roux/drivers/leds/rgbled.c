...
static ssize_t rgbled_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t rgbled_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static unsigned short rgbled_lightness(unsigned char color_level)
{
	...
	static const unsigned char lut_color_in[9] = {0x00, 0x20, 0x40, 0x60, 0x80,
												  0xa0, 0xc0, 0xe0, 0xff};

	static const unsigned short lut_pwm_out[9] = {0x0000, 0x03d1, 0x0b62, 0x1952, 0x2f93,
	...
	if (lut_index < sizeof(lut_color_in)) {
		if (lut_color_in[lut_index] == color_level) {
			...
			pwm_cmd =
				(unsigned short)(lut_pwm_out[lut_index - 1] +
...
int rgbled_register(FAR const char *path, FAR struct pwm_lowerhalf_s *ledr,
					FAR struct pwm_lowerhalf_s *ledg, FAR struct pwm_lowerhalf_s *ledb
#ifdef CONFIG_PWM_MULTICHAN
					,
...
)
{
	...
	upper = (FAR struct rgbled_upperhalf_s *)kmm_zalloc(sizeof(struct rgbled_upperhalf_s));
