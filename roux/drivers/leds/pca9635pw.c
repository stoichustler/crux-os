...
static int pca9635pw_i2c_write_byte(FAR struct pca9635pw_dev_s *priv, uint8_t const reg_addr,
									uint8_t const reg_val);
static int pca9635pw_set_led_mode(FAR struct pca9635pw_dev_s *priv, uint8_t const led_out_x_mode);
...
static int pca9635pw_set_led_mode(FAR struct pca9635pw_dev_s *priv, uint8_t const led_out_x_mode)
{
	...
	for (; current_ledout_reg <= PCA9635PW_LED_OUT_3; current_ledout_reg++) {
		int const ret = pca9635pw_i2c_write_byte(priv, current_ledout_reg, led_out_x_mode);
		if (ret < 0) {
			return ret;
		}
	}
...
static int pca9635pw_open(FAR struct file *filep)
{
	...
	ret = pca9635pw_i2c_write_byte(priv, PCA9635PW_MODE_1, PCA9635PW_MODE_1_INITIAL_VALUE);
	if (ret < 0) {
	...
	uint8_t const PCA9635PW_MODE_2_INITIAL_VALUE = PCA9635PW_MODE_2_INVRT | PCA9635PW_MODE_2_OUTDRV;

	ret = pca9635pw_i2c_write_byte(priv, PCA9635PW_MODE_2, PCA9635PW_MODE_2_INITIAL_VALUE);
	if (ret < 0) {
...
static int pca9635pw_close(FAR struct file *filep)
{
	...
	ret = pca9635pw_i2c_write_byte(priv, PCA9635PW_MODE_1, PCA9635PW_MODE_1_FINAL_VALUE);
	if (ret < 0) {
		return ret;
	}
					   ...
					   uint8_t const pca9635pw_i2c_addr)
{
	...
	FAR struct pca9635pw_dev_s *priv = kmm_malloc(sizeof(struct pca9635pw_dev_s));
