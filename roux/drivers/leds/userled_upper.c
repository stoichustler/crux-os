...
static ssize_t userled_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int userled_close(FAR struct file *filep)
{
	...
	for (prev = NULL, curr = priv->lu_open; curr && curr != opriv;
...
static ssize_t userled_write(FAR struct file *filep, FAR const char *buffer, size_t len)
{
	FAR struct inode *inode;
	FAR struct userled_upperhalf_s *priv;
...
static int userled_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case ULEDIOC_SETLED: {
		...
		if (userled) {
			...
			if ((size_t)led < 8 * sizeof(userled_set_t) && (priv->lu_supported & (1 << led)) != 0) {
...
int userled_register(FAR const char *devname, FAR const struct userled_lowerhalf_s *lower)
{
	FAR struct userled_upperhalf_s *priv;
	int ret;
	...
	priv = (FAR struct userled_upperhalf_s *)kmm_zalloc(sizeof(struct userled_upperhalf_s));
