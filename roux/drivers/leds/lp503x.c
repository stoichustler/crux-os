...
static int lp503x_i2c_write_reg(struct lp503x_dev_s *priv, uint8_t const reg_addr,
								uint8_t const reg_val);
static int lp503x_i2c_read_reg(struct lp503x_dev_s *priv, uint8_t const reg_addr, uint8_t *regval);
static int lp503x_open(struct file *filep);
static int lp503x_close(struct file *filep);
static int lp503x_ioctl(struct file *filep, int cmd, unsigned long arg);
#ifdef CONFIG_DEBUG_LP503X
static int lp503x_dump_registers(struct lp503x_dev_s *priv, const char *msg);
#else
...
static int lp503x_set_rgbbrightness(struct lp503x_dev_s *priv, int led, int brightness);
static int lp503x_set_outcolour(struct lp503x_dev_s *priv, int led, int colour);
static int lp503x_set_config(struct lp503x_dev_s *priv);
static int lp503x_set_bank_mode(struct lp503x_dev_s *priv);
static int lp503x_set_bank_colour(struct lp503x_dev_s *priv, char bank, int brightness);
static int lp503x_set_bank_brightness(struct lp503x_dev_s *priv, int brightness);
...
static int lp503x_dump_registers(struct lp503x_dev_s *priv, const char *msg)
{
	...
	lp503x_info("Dev Config0:\t%02x Dev Conf1:\t%02x \
	...
	lp503x_info("Bank Bright:\t%02x BankA Col:\t%02x \
	...
	lp503x_info("LED0 Bright:\t%02x LED1 Col:\t%02x \
	...
	lp503x_info("LED4 Bright:\t%02x LED5 Bright:\t%02x \
	...
	lp503x_info("LED8 Bright:\t%02x LED9 Bright:\t%02x \
	...
	lp503x_info("Out0 Col:\t%02x Out1 Col:\t%02x \
	...
	lp503x_info("Out4 Col:\t%02x Out5 Col:\t%02x \
	...
	lp503x_info("Out8 Col:\t%02x Out9 Col:\t%02x \
	...
	lp503x_info("Out12 Col:\t%02x Out13 Col:\t%02x \
	...
	lp503x_info("Out16 Col:\t%02x Out17 Col:\t%02x \
	...
	lp503x_info("Out20 Col:\t%02x Out21 Col:\t%02x \
	...
	lp503x_info("Out24 Col:\t%02x Out25 Col:\t%02x \
	...
	lp503x_info("Out28 Col:\t%02x Out29 Col:\t%02x \
...
static int lp503x_set_rgbled_colour(struct lp503x_dev_s *priv, int led, int colour)
{
	int ret;
	int regaddr;
...
static int lp503x_set_outcolour(struct lp503x_dev_s *priv, int led, int brightness)
{
	int ret;
...
int lp503x_register(const char *devpath, struct i2c_master_s *i2c, uint8_t const lp503x_i2c_addr,
					int const i2c_frequency)
{
