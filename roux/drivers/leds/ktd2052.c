...
static int ktd2052_read_reg(FAR struct ktd2052_dev_s *priv, uint8_t reg_addr, uint8_t *reg_val);
...
static ssize_t ktd2052_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t ktd2052_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static FAR struct ktd2052_dev_s *ktd2052_initialize(FAR struct i2c_master_s *i2c, uint8_t addr,
													uint32_t freq)
{
...
static int ktd2052_set_rgb(FAR struct ktd2052_dev_s *priv, uint8_t module, const uint8_t *rgb)
{
	int ret;
	uint8_t base_reg;
...
static int ktd2052_set_mode(FAR struct ktd2052_dev_s *priv, uint8_t mode, bool bright_extend,
							uint8_t temp_limit, uint8_t fade_rate)
{
	uint8_t reg_val = 0;
	...
	reg_val = (mode << 6) | (bright_extend ? CONTROL_BE_EN : 0) | (temp_limit << 3) | fade_rate;
...
static int ktd2052_setup_pattern(FAR struct ktd2052_dev_s *priv, uint8_t slots, uint8_t duration,
								 uint8_t fade_rate1, uint8_t watchdog)
{
	uint8_t mode;
	uint8_t reg_val;
	int ret;
	...
	ret = ktd2052_write_regs(priv, KTD2052_PG_CNTL_REG, &reg_val, 1);
	if (ret < 0) {
		return ret;
	}
...
static int ktd2052_set_pattern_slots(FAR struct ktd2052_dev_s *priv, uint8_t module, uint8_t slots)
{
	if (module < 1 || module > 4) {
		return -EINVAL;
	}
...
static int ktd2052_open(FAR struct file *filep)
{
	...
	ret = ktd2052_set_mode(priv, KTD2052_MODE_NORMAL, true, KTD2052_TEMP_LIMIT_90, 0);
	if (ret < 0) {
		return ret;
	}
...
static ssize_t ktd2052_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (buflen >= 1) {
		...
		if (buflen >= 2) {
			...
			ret = ktd2052_read_reg(priv, KTD2052_CONTROL_REG, (uint8_t *)&buffer[1]);
			if (ret < 0) {
				return ret;
			}
...
static ssize_t ktd2052_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	ret = ktd2052_write_regs(priv, KTD2052_IRED1_REG, (const uint8_t *)&buffer[0], buflen);
	if (ret < 0) {
		return ret;
	}
...
static int ktd2052_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case KTDIOSETMODE: {
		...
		ret = ktd2052_set_mode(priv, mode->mode, mode->bright_extend, mode->temp_limit,
							   mode->fade_rate);
	} break;
	...
	case KTDIOSETPATTERN: {
		...
		FAR struct ktd2052_pattern_s *pattern = (FAR struct ktd2052_pattern_s *)arg;
		if (pattern == NULL) {
			ret = -EINVAL;
			break;
		}

		ret = ktd2052_setup_pattern(priv, pattern->slots, pattern->duration, pattern->fade_rate1,
									pattern->watchdog);
	} break;
...
int ktd2052_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
					 uint32_t freq)
{
	FAR struct ktd2052_dev_s *priv;
	int ret;
