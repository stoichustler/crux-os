...
static inline void max7219_write16(FAR struct max7219_dev_s *priv, uint16_t value);

/* Character driver methods */

static ssize_t max7219_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t max7219_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
int max7219_leds_register(FAR const char *devpath, FAR struct spi_dev_s *spi)
{
	...
	data = (MAX7219_INTENSITY) | (DISPLAY_INTENSITY(CONFIG_MAX7219_INTENSITY) << 8);
	max7219_write16(priv, data);
