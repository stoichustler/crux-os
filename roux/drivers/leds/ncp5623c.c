...
static int ncp5623c_i2c_write_byte(FAR struct ncp5623c_dev_s *priv, uint8_t const reg_addr,
...
static ssize_t ncp5623c_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t ncp5623c_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static ssize_t ncp5623c_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	ret = ncp5623c_i2c_write_byte(priv, NCP5623C_PWM1, red);
	if (ret != OK) {
	...
	ret = ncp5623c_i2c_write_byte(priv, NCP5623C_PWM2, green);
	if (ret != OK) {
	...
	ret = ncp5623c_i2c_write_byte(priv, NCP5623C_PWM3, blue);
	if (ret != OK) {
