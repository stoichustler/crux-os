...
static uint32_t virtio_mmio_get_queue_len(FAR struct metal_io_region *io, int idx);
static int virtio_mmio_config_virtqueue(FAR struct metal_io_region *io, FAR struct virtqueue *vq);
static FAR void *virtio_mmio_alloc_buf(FAR struct virtio_device *vdev, size_t size, size_t align);
static void virtio_mmio_free_buf(FAR struct virtio_device *vdev, FAR void *buf);
static int virtio_mmio_init_device(FAR struct virtio_mmio_device_s *vmdev, FAR void *regs, int irq);

/* Virtio mmio dispatch functions */

static int virtio_mmio_create_virtqueue(FAR struct virtio_mmio_device_s *vmdev, unsigned int i,
										FAR const char *name, vq_callback callback);
static int virtio_mmio_create_virtqueues(FAR struct virtio_device *vdev, unsigned int flags,
...
static void virtio_mmio_set_status(FAR struct virtio_device *vdev, uint8_t status);
static uint8_t virtio_mmio_get_status(FAR struct virtio_device *vdev);
static void virtio_mmio_write_config(FAR struct virtio_device *vdev, uint32_t offset, void *dst,
									 int length);
static void virtio_mmio_read_config(FAR struct virtio_device *vdev, uint32_t offset, FAR void *dst,
									int length);
static uint64_t virtio_mmio_get_features(FAR struct virtio_device *vdev);
static void virtio_mmio_set_features(FAR struct virtio_device *vdev, uint64_t features);
...
static int virtio_mmio_config_virtqueue(FAR struct metal_io_region *io, FAR struct virtqueue *vq)
{
	...
	if (metal_io_read32(io, version == VIRTIO_MMIO_VERSION_1 ? VIRTIO_MMIO_QUEUE_PFN
...
static int virtio_mmio_create_virtqueues(FAR struct virtio_device *vdev, unsigned int flags,
										 ...
										 vq_callback callbacks[], FAR void *callback_args[])
{
	FAR struct virtio_mmio_device_s *vmdev = (FAR struct virtio_mmio_device_s *)vdev;
	unsigned int i;
	int ret = OK;
...
static void virtio_mmio_write_config(FAR struct virtio_device *vdev, uint32_t offset, void *src,
									 int length)
{
	FAR struct virtio_mmio_device_s *vmdev = (FAR struct virtio_mmio_device_s *)vdev;
	...
	case 8:
		...
		metal_io_write32(&vmdev->cfg_io, write_offset + sizeof(u32data), u32data);
		break;
	default:
byte_write: {
	FAR char *s = src;
									...
									int length)
{
	...
	case 8:
		u32data = metal_io_read32(&vmdev->cfg_io, read_offset);
		memcpy(dst, &u32data, sizeof(u32data));
		u32data = metal_io_read32(&vmdev->cfg_io, read_offset + sizeof(u32data));
...
static void virtio_mmio_set_features(FAR struct virtio_device *vdev, uint64_t features)
{
	...
	metal_io_write32(&vmdev->cfg_io, VIRTIO_MMIO_DRIVER_FEATURES_SEL, 1);
	metal_io_write32(&vmdev->cfg_io, VIRTIO_MMIO_DRIVER_FEATURES, features >> 32);
...
static void virtio_mmio_notify(FAR struct virtqueue *vq)
{
	FAR struct virtio_mmio_device_s *vmdev = (FAR struct virtio_mmio_device_s *)vq->vq_dev;

	/* VIRTIO_F_NOTIFICATION_DATA is not supported for now */

	metal_io_write32(&vmdev->cfg_io, VIRTIO_MMIO_QUEUE_NOTIFY, vq->vq_queue_index);
}
...
static int virtio_mmio_init_device(FAR struct virtio_mmio_device_s *vmdev, FAR void *regs, int irq)
{
	...
	metal_io_init(&vmdev->shm_io, NULL, &vmdev->shm_phy, SIZE_MAX, UINT_MAX, 0, metal_io_get_ops());
	metal_io_init(&vmdev->cfg_io, regs, &vmdev->cfg_phy, SIZE_MAX, UINT_MAX, 0, metal_io_get_ops());
	...
	if (vdev->id.version == VIRTIO_MMIO_VERSION_1) {
		metal_io_write32(&vmdev->cfg_io, VIRTIO_MMIO_PAGE_SIZE, VIRTIO_PAGE_SIZE);
	}
