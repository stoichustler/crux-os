...
#define VIRTIO_NET_MAX_PKT_SIZE ((CONFIG_NET_LL_GUARDSIZE - ETH_HDRLEN) + VIRTIO_NET_BUFSIZE)
...
static int virtio_net_send(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt);
static netpkt_t *virtio_net_recv(FAR struct netdev_lowerhalf_s *dev);
#ifdef CONFIG_NET_MCASTGROUP
static int virtio_net_addmac(FAR struct netdev_lowerhalf_s *dev, FAR const uint8_t *mac);
static int virtio_net_rmmac(FAR struct netdev_lowerhalf_s *dev, FAR const uint8_t *mac);
#endif
#ifdef CONFIG_NETDEV_IOCTL
static int virtio_net_ioctl(FAR struct netdev_lowerhalf_s *dev, int cmd, unsigned long arg);
#endif
...
static const struct netdev_ops_s g_virtio_net_ops = {virtio_net_ifup,	virtio_net_ifdown,
													 virtio_net_send,	virtio_net_recv,
#ifdef CONFIG_NET_MCASTGROUP
													 virtio_net_addmac, virtio_net_rmmac,
#endif
#ifdef CONFIG_NETDEV_IOCTL
													 virtio_net_ioctl,
#endif
...
static int virtio_net_addbuffer(FAR struct netdev_lowerhalf_s *dev, FAR struct virtqueue *vq,
								FAR netpkt_t *pkt, unsigned int vq_id)
{
	...
	hdr = (FAR struct virtio_net_llhdr_s *)((FAR uint8_t *)iov[0].iov_base - VIRTIO_NET_LLHDRSIZE);
	...
	if (vq_id == VIRTIO_NET_RX) {
		return virtqueue_add_buffer_lock(vq, vb, 0, iov_cnt, hdr, &priv->lock[vq_id]);
	} else {
		return virtqueue_add_buffer_lock(vq, vb, iov_cnt, 0, hdr, &priv->lock[vq_id]);
	}
}
...
static void virtio_net_rxfill(FAR struct netdev_lowerhalf_s *dev)
{
	...
	for (i = 0; i < priv->bufnum; i++) {
		...
		if (netpkt_setdatalen(dev, pkt, VIRTIO_NET_BUFSIZE) < VIRTIO_NET_BUFSIZE) {
...
static void virtio_net_txfree(FAR struct netdev_lowerhalf_s *dev)
{
	...
	while (1) {
		...
		hdr = virtqueue_get_buffer_lock(vq, NULL, NULL, &priv->lock[VIRTIO_NET_TX]);
		if (hdr == NULL) {
			break;
		}
...
static int virtio_net_ifup(FAR struct netdev_lowerhalf_s *dev)
{
	...
	virtqueue_enable_cb_lock(priv->vdev->vrings_info[VIRTIO_NET_RX].vq, &priv->lock[VIRTIO_NET_RX]);
...
static int virtio_net_ifdown(FAR struct netdev_lowerhalf_s *dev)
{
	...
	for (i = 0; i < VIRTIO_NET_NUM; i++) {
		virtqueue_disable_cb_lock(priv->vdev->vrings_info[i].vq, &priv->lock[i]);
	}
...
static int virtio_net_init(FAR struct virtio_net_priv_s *priv, FAR struct virtio_device *vdev)
{
	...
	virtio_negotiate_features(vdev, (1UL << VIRTIO_NET_F_MAC) | (1UL << VIRTIO_F_ANY_LAYOUT), NULL);
	...
	ret = virtio_create_virtqueues(vdev, 0, VIRTIO_NET_NUM, vqnames, callbacks, NULL);
	if (ret < 0) {
...
#endif
	priv->bufnum = MIN(vdev->vrings_info[VIRTIO_NET_RX].info.num_descs / (VIRTIO_NET_MAX_NIOB + 1),
					   priv->bufnum);
	priv->bufnum = MIN(vdev->vrings_info[VIRTIO_NET_TX].info.num_descs / (VIRTIO_NET_MAX_NIOB + 1),
					   priv->bufnum);
	return OK;
}
...
{
	FAR struct net_driver_s *dev = &((FAR struct netdev_lowerhalf_s *)&priv->lower)->netdev;
	FAR struct virtio_device *vdev = priv->vdev;
	FAR uint8_t *mac = dev->d_mac.ether.ether_addr_octet;

	if (virtio_has_feature(vdev, VIRTIO_NET_F_MAC)) {
		virtio_read_config(vdev, offsetof(struct virtio_net_config_s, mac), mac, IFHWADDRLEN);
	} else {
...
#ifdef CONFIG_DRIVERS_WIFI_SIM
								g_netdev_num++ < CONFIG_WIFI_SIM_NUMBER ? NET_LL_IEEE80211
																		: NET_LL_ETHERNET
#else
								NET_LL_ETHERNET
#endif
