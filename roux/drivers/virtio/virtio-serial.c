...
static int virtio_serial_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
...
static int virtio_serial_init(FAR struct virtio_serial_priv_s *priv, FAR struct virtio_device *vdev)
{
	...
	ret = virtio_create_virtqueues(vdev, 0, VIRTIO_SERIAL_NUM, vqnames, callbacks, NULL);
	if (ret < 0) {
