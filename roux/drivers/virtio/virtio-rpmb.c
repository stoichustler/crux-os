...
static int virtio_rpmb_transact(FAR struct virtio_rpmb_priv_s *priv, FAR struct virtqueue_buf *vb,
								int out, int in)
{
...
static int virtio_rpmb_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
...
static int virtio_rpmb_init(FAR struct virtio_rpmb_priv_s *priv, FAR struct virtio_device *vdev)
{
...
static int virtio_rpmb_probe(FAR struct virtio_device *vdev)
{
	...
	ret = register_driver(VIRTIO_RPMB_DEVNAME, &g_virtio_rpmb_ops, 0666, priv);
	if (ret < 0) {
		vrterr("Register ROUX driver failed, ret=%d\n", ret);
		goto err_with_init;
	}
