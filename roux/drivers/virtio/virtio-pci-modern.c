...
static uint16_t virtio_pci_modern_get_queue_len(FAR struct virtio_pci_device_s *vpdev, int idx);
static int virtio_pci_modern_config_vector(FAR struct virtio_pci_device_s *vpdev, bool enable);
static int virtio_pci_modern_create_virtqueue(FAR struct virtio_pci_device_s *vpdev,
											  FAR struct virtqueue *vq);
static void virtio_pci_modern_delete_virtqueue(FAR struct virtio_device *vdev, int index);
static void virtio_pci_modern_set_status(FAR struct virtio_device *vdev, uint8_t status);
static uint8_t virtio_pci_modern_get_status(FAR struct virtio_device *vdev);
static void virtio_pci_modern_write_config(FAR struct virtio_device *vdev, uint32_t offset,
										   FAR void *dst, int length);
static void virtio_pci_modern_read_config(FAR struct virtio_device *vdev, uint32_t offset,
										  FAR void *dst, int length);
static uint64_t virtio_pci_modern_get_features(FAR struct virtio_device *vdev);
static void virtio_pci_modern_set_features(FAR struct virtio_device *vdev, uint64_t features);
...
static uint8_t virtio_pci_find_capability(FAR struct pci_device_s *dev, uint8_t cfg_type,
										  unsigned int flags)
{
	uint8_t type;
	uint8_t bar;
...
static FAR void *virtio_pci_map_capability(FAR struct virtio_pci_device_s *vpdev, uint8_t pos,
										   size_t *len)
{
...
void virtio_pci_modern_delete_virtqueue(FAR struct virtio_device *vdev, int index)
{
	FAR struct virtio_pci_device_s *vpdev = (FAR struct virtio_pci_device_s *)vdev;
	FAR struct virtio_pci_common_cfg_s *cfg = vpdev->common;
...
static void virtio_pci_modern_notify(FAR struct virtqueue *vq)
{
	FAR struct virtio_pci_device_s *vpdev = (FAR struct virtio_pci_device_s *)vq->vq_dev;
	FAR struct virtio_vring_info *vrinfo;
...
static int virtio_pci_init_device(FAR struct virtio_pci_device_s *vpdev)
{
	...
	common = virtio_pci_find_capability(dev, VIRTIO_PCI_CAP_COMMON_CFG, PCI_RESOURCE_MEM);
	if (common == 0) {
		vrtinfo("Leaving for legacy driver\n");
		return -ENODEV;
	}

	notify = virtio_pci_find_capability(dev, VIRTIO_PCI_CAP_NOTIFY_CFG, PCI_RESOURCE_MEM);
	if (notify == 0) {
		vrterr("Missing capabilities %d %d\n", common, notify);
		return -EINVAL;
	}

	device = virtio_pci_find_capability(dev, VIRTIO_PCI_CAP_DEVICE_CFG, PCI_RESOURCE_MEM);
	...
	pci_read_config_dword(dev,
						  notify + offsetof(struct virtio_pci_notify_cap_s, notify_off_multiplier),
