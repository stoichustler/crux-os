...
#define VIRTIO_GPU_MAP_ERR(e) ((e) == VIRTIO_GPU_RESP_ERR_OUT_OF_MEMORY ? -ENOMEM : -EINVAL)
...
static int virtio_gpu_send_cmd(FAR struct virtqueue *vq, FAR struct virtqueue_buf *buf_list,
							   int readable, int writable, FAR void *cookie);
static void virtio_gpu_done(FAR struct virtqueue *vq);
static int virtio_gpu_init(FAR struct virtio_gpu_priv_s *priv, FAR struct virtio_device *vdev);
static int virtio_gpu_get_display_info(FAR struct virtio_gpu_priv_s *priv);
static int virtio_gpu_create_2d(FAR struct virtio_gpu_priv_s *priv, int resource_id, int width,
								int height);
static int virtio_gpu_attach_backing(FAR struct virtio_gpu_priv_s *priv, int resource_id,
									 FAR struct virtio_gpu_mem_entry *ents, uint32_t nents);
static int virtio_gpu_set_scanout(FAR struct virtio_gpu_priv_s *priv, int scanout_id,
								  int resource_id, int width, int height);
static int virtio_gpu_transfer_to_host_2d(FAR struct virtio_gpu_priv_s *priv, int resource_id,
										  int x, int y, int width, int height);
static int virtio_gpu_flush_resource(FAR struct virtio_gpu_priv_s *priv, int resource_id, int x,
...
static int virtio_gpu_getplaneinfo(FAR struct fb_vtable_s *vtable, int planeno,
								   FAR struct fb_planeinfo_s *pinfo);
static int virtio_gpu_updatearea(FAR struct fb_vtable_s *vtable, FAR const struct fb_area_s *area);
...
static void virtio_gpu_done(FAR struct virtqueue *vq)
{
	...
	while ((cookie = virtqueue_get_buffer_lock(vq, NULL, NULL, &priv->lock)) != NULL) {
		if (cookie->blocking) {
...
static int virtio_gpu_init(FAR struct virtio_gpu_priv_s *priv, FAR struct virtio_device *vdev)
{
	...
	ret = virtio_create_virtqueues(vdev, 0, VIRTIO_GPU_NUM, vqnames, callbacks, NULL);
	if (ret < 0) {
									 ...
									 FAR struct virtio_gpu_mem_entry *ents, uint32_t nents)
{
	...
	if (nents > VIRTIO_GPU_MAX_NENTS) {
		vrterr("ERROR: Backing memory entries count %" PRId32 "exceeds %d", nents,
			   VIRTIO_GPU_MAX_NENTS);
		return -E2BIG;
	}
...
static int virtio_gpu_probe(FAR struct virtio_device *vdev)
{
	...
	ret = virtio_gpu_transfer_to_host_2d(priv, 1, 0, 0, priv->xres, priv->yres);
	if (ret < 0) {
	...
	if (ret < 0) {
		vrterr("ERROR: Failed to initialize framebuffer driver, ret=%d", ret);
...
static int virtio_gpu_getvideoinfo(FAR struct fb_vtable_s *vtable, FAR struct fb_videoinfo_s *vinfo)
{
	FAR struct virtio_gpu_priv_s *priv = (FAR struct virtio_gpu_priv_s *)vtable;
								   ...
								   FAR struct fb_planeinfo_s *pinfo)
{
	...
	if (planeno >= VIRTIO_GPU_MAX_PLANE) {
		vrterr("ERROR: plane number %d exceeds %d", planeno, VIRTIO_GPU_MAX_PLANE - 1);
		return -EINVAL;
	}
...
static int virtio_gpu_updatearea(FAR struct fb_vtable_s *vtable, FAR const struct fb_area_s *area)
{
	...
	vrtinfo("update disp %d:(%d %d)[%d %d]", priv->display, area->x, area->y, area->w, area->h);
	ret = virtio_gpu_transfer_to_host_2d(priv, 1, area->x, area->y, area->w, area->h);
	if (ret < 0) {
		vrterr("virtio_gpu_transfer_to_host_2d failed: %d", ret);
		return ret;
	}

	ret = virtio_gpu_flush_resource(priv, 1, area->x, area->y, area->w, area->h);
	return ret;
}
...
int virtio_gpu_fb_register(int display)
{
	...
	if (display < 0 || display >= VIRTIO_GPU_MAX_DISP || !g_virtio_gpu[display]) {
		vrterr("ERROR: display number %d is out of range [%d, %d]", display, 0,
			   VIRTIO_GPU_MAX_DISP - 1);
		return -EINVAL;
	}
