...
static ssize_t virtio_blk_rdwr(FAR struct virtio_blk_priv_s *priv, FAR void *buffer,
							   blkcnt_t startsector, unsigned int nsectors, bool write);
static int virtio_blk_open(FAR struct inode *inode);
static int virtio_blk_close(FAR struct inode *inode);
static ssize_t virtio_blk_read(FAR struct inode *inode, FAR unsigned char *buffer,
							   blkcnt_t startsector, unsigned int nsectors);
static ssize_t virtio_blk_write(FAR struct inode *inode, FAR const unsigned char *buffer,
								blkcnt_t startsector, unsigned int nsectors);
static int virtio_blk_geometry(FAR struct inode *inode, FAR struct geometry *geometry);
static int virtio_blk_ioctl(FAR struct inode *inode, int cmd, unsigned long arg);
static int virtio_blk_flush(FAR struct virtio_blk_priv_s *priv);

/* Other functions */

static int virtio_blk_init(FAR struct virtio_blk_priv_s *priv, FAR struct virtio_device *vdev);
...
static void virtio_blk_wait_complete(FAR struct virtqueue *vq, FAR sem_t *respsem)
{
								...
								blkcnt_t startsector, unsigned int nsectors)
{
	...
	return virtio_blk_rdwr(priv, (FAR void *)buffer, startsector, nsectors, true);
}
...
static int virtio_blk_init(FAR struct virtio_blk_priv_s *priv, FAR struct virtio_device *vdev)
{
	...
	virtio_negotiate_features(vdev,
							  (1UL << VIRTIO_BLK_F_RO) | (1UL << VIRTIO_BLK_F_BLK_SIZE) |
...
static int virtio_blk_probe(FAR struct virtio_device *vdev)
{
	...
	if (virtio_has_feature(vdev, VIRTIO_BLK_F_BLK_SIZE)) {
		virtio_read_config_member(priv->vdev, struct virtio_blk_config_s, blk_size,
								  &priv->block_size);
