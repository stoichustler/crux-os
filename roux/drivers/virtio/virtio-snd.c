...
/* Map for converting VirtIO frame rate to roux frame rate */
...
/* Map for roux bps to virtio pcm subformat */
...
static unsigned int virtio_snd_get_period_bytes(unsigned int rate, unsigned int ch,
												unsigned int bps, unsigned int period_time);
static unsigned int virtio_snd_get_support_rates(FAR const struct virtio_snd_pcm_info *info);
static void virtio_snd_get_support_formats(FAR const struct virtio_snd_pcm_info *info,
										   FAR struct audio_caps_s *caps);

static int virtio_snd_send_pcm(FAR struct virtio_snd_dev_s *sdev,
							   FAR struct virtio_snd_buffer_s *buf);
static int virtio_snd_send_ctl(FAR struct virtio_snd_s *priv, FAR struct virtqueue_buf *vb,
							   int readable, int writable);
static int virtio_snd_query_info(FAR struct virtio_snd_s *priv, int cmd, size_t size, size_t count,
								 FAR struct virtio_snd_pcm_info *info);
static int virtio_snd_set_params(FAR struct virtio_snd_dev_s *sdev, unsigned int ch,
								 unsigned int rate, unsigned int bps);
static int virtio_snd_send_cmd(FAR struct virtio_snd_dev_s *sdev, int cmd);

static int virtio_snd_getcaps(FAR struct audio_lowerhalf_s *dev, int type,
							  FAR struct audio_caps_s *caps);
#ifdef CONFIG_AUDIO_MULTI_SESSION
static int virtio_snd_configure(FAR struct audio_lowerhalf_s *dev, FAR void *session,
								FAR const struct audio_caps_s *caps);
static int virtio_snd_start(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#ifndef CONFIG_AUDIO_EXCLUDE_STOP
static int virtio_snd_stop(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#endif
#ifndef CONFIG_AUDIO_EXCLUDE_PAUSE_RESUME
static int virtio_snd_pause(FAR struct audio_lowerhalf_s *dev, FAR void *session);
static int virtio_snd_resume(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#endif
static int virtio_snd_reserve(FAR struct audio_lowerhalf_s *dev, FAR void **session);
static int virtio_snd_release(FAR struct audio_lowerhalf_s *dev, FAR void *session);
#else
...
static int virtio_snd_freebuffer(FAR struct audio_lowerhalf_s *dev,
								 FAR struct audio_buf_desc_s *apb);
static int virtio_snd_enqueuebuffer(FAR struct audio_lowerhalf_s *dev, FAR struct ap_buffer_s *apb);
static int virtio_snd_ioctl(FAR struct audio_lowerhalf_s *dev, int cmd, unsigned long arg);
...
static const struct virtio_snd_format_map_s g_format_map[] = {
	[VIRTIO_SND_PCM_FMT_S8] = {AUDIO_SUBFMT_PCM_S8, 8},
	[VIRTIO_SND_PCM_FMT_S16] = {AUDIO_SUBFMT_PCM_S16_LE, 16},
	[VIRTIO_SND_PCM_FMT_S32] = {AUDIO_SUBFMT_PCM_S32_LE, 32}};
...
#ifdef CONFIG_AUDIO_MULTI_SESSION
		buf->dev->upper(buf->dev->priv, AUDIO_CALLBACK_DEQUEUE, &buf->apb, OK, NULL);
#else
		buf->dev->upper(buf->dev->priv, AUDIO_CALLBACK_DEQUEUE, &buf->apb, OK);
#endif
							   ...
							   FAR struct virtio_snd_buffer_s *buf)
{
	...
	int idx = info->direction == VIRTIO_SND_D_INPUT ? VIRTIO_SND_VQ_RX : VIRTIO_SND_VQ_TX;
...
static int virtio_snd_send_ctl(FAR struct virtio_snd_s *priv, FAR struct virtqueue_buf *vb,
							   int readable, int writable)
{
	FAR struct virtqueue *vq = priv->vdev->vrings_info[VIRTIO_SND_VQ_CONTROL].vq;
	irqstate_t flags;
	sem_t ctl_sem;
	int ret;
								 ...
								 unsigned int rate, unsigned int bps)
{
	...
	req->buffer_bytes = req->period_bytes * CONFIG_DRIVERS_VIRTIO_SND_BUFFER_COUNT;
							  ...
							  FAR struct audio_caps_s *caps)
{
	...
	case AUDIO_TYPE_QUERY:
		...
		case AUDIO_TYPE_QUERY:
			caps->ac_controls.b[0] =
				info->direction == VIRTIO_SND_D_INPUT ? AUDIO_TYPE_INPUT : AUDIO_TYPE_OUTPUT;
	...
	case AUDIO_TYPE_INPUT: {
		caps->ac_channels = (info->channels_min << 4) | (info->channels_max & 0x0f);
		switch (caps->ac_subtype) {
		case AUDIO_TYPE_QUERY:
...
#endif
{
	...
	case AUDIO_TYPE_INPUT:
		...
		sdev->period_bytes =
			virtio_snd_get_period_bytes(rate, ch, bps, CONFIG_DRIVERS_VIRTIO_SOUND_PERIOD_TIME);
...
{
	FAR struct virtio_snd_dev_s *sdev = (FAR struct virtio_snd_dev_s *)dev;
	FAR struct virtio_snd_buffer_s *buf = (FAR struct virtio_snd_buffer_s *)apb;
...
static int virtio_snd_ioctl(FAR struct audio_lowerhalf_s *dev, int cmd, unsigned long arg)
{
	...
	case AUDIOIOC_GETLATENCY:
		*latency = sdev->cache_buffers * sdev->period_bytes / sdev->frame_size;
		break;
...
static int virtio_snd_init(FAR struct virtio_snd_s *priv)
{
	...
	ret = virtio_create_virtqueues(priv->vdev, 0, VIRTIO_SND_VQ_MAX, vqnames, callbacks, NULL);
	if (ret < 0) {
	...
	virtio_read_config(priv->vdev, 0, &priv->config, sizeof(struct virtio_snd_config));
	vrtinfo("jacks:%" PRIu32 " streams:%" PRIu32 " chmap:%" PRIu32 "\n", priv->config.jacks,
			priv->config.streams, priv->config.chmaps);

	priv->info =
		virtio_alloc_buf(priv->vdev, priv->config.streams * sizeof(struct virtio_snd_pcm_info), 16);
	if (priv->info == NULL) {
	...
	ret = virtio_snd_query_info(priv, VIRTIO_SND_R_PCM_INFO, sizeof(struct virtio_snd_pcm_info),
								priv->config.streams, priv->info);
	if (ret < 0) {
...
static int virtio_snd_register_audio_driver(FAR struct virtio_snd_s *priv)
{
	char devname[32];
	int tx_minor = 0;
	int rx_minor = 0;
	...
	priv->dev = kmm_zalloc(priv->config.streams * sizeof(struct virtio_snd_dev_s));
	if (priv->dev == NULL) {
