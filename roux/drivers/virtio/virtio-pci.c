...
#include <roux/virtio/virtio-pci.h>
...
static FAR void *virtio_pci_alloc_buf(FAR struct virtio_device *vdev, size_t size, size_t align);
static void virtio_pci_free_buf(FAR struct virtio_device *vdev, FAR void *buf);

static int virtio_pci_probe(FAR struct pci_device_s *dev);
static void virtio_pci_remove(FAR struct pci_device_s *dev);

static int virtio_pci_create_virtqueue(FAR struct virtio_pci_device_s *vpdev, unsigned int i,
...
static struct pci_driver_s g_virtio_pci_drv = {g_virtio_pci_id_table, virtio_pci_probe,
											   virtio_pci_remove};
...
static int virtio_pci_config_changed(int irq, FAR void *context, FAR void *arg)
{
...
{
	FAR struct virtio_pci_device_s *vpdev = (FAR struct virtio_pci_device_s *)arg;
...
static int virtio_pci_probe(FAR struct pci_device_s *dev)
{
	...
	metal_io_init(&vpdev->shm_io, NULL, &vpdev->shm_phy, SIZE_MAX, UINT_MAX, 0, metal_io_get_ops());
	...
	vrtinfo("Interrupt mode: attaching MSI %d to %p, %d to %p\n", vpdev->irq[VIRTIO_PCI_INT_CFG],
			virtio_pci_config_changed, vpdev->irq[VIRTIO_PCI_INT_VQ], virtio_pci_interrupt);
	...
	irq_attach(vpdev->irq[VIRTIO_PCI_INT_CFG], virtio_pci_config_changed, vpdev);
...
int virtio_pci_create_virtqueues(FAR struct virtio_device *vdev, unsigned int flags,
...
uint64_t virtio_pci_negotiate_features(FAR struct virtio_device *vdev, uint64_t features)
{
