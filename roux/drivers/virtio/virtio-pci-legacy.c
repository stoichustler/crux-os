...
static uint16_t virtio_pci_legacy_get_queue_len(FAR struct virtio_pci_device_s *vpdev, int idx);
static int virtio_pci_legacy_create_virtqueue(FAR struct virtio_pci_device_s *vpdev,
											  FAR struct virtqueue *vq);
static int virtio_pci_legacy_config_vector(FAR struct virtio_pci_device_s *vpdev, bool enable);

static void virtio_pci_legacy_delete_virtqueue(FAR struct virtio_device *vdev, int index);
static void virtio_pci_legacy_set_status(FAR struct virtio_device *vdev, uint8_t status);
static uint8_t virtio_pci_legacy_get_status(FAR struct virtio_device *vdev);
static void virtio_pci_legacy_write_config(FAR struct virtio_device *vdev, uint32_t offset,
										   FAR void *dst, int length);
static void virtio_pci_legacy_read_config(FAR struct virtio_device *vdev, uint32_t offset,
										  FAR void *dst, int length);
static uint64_t virtio_pci_legacy_get_features(FAR struct virtio_device *vdev);
static void virtio_pci_legacy_set_features(FAR struct virtio_device *vdev, uint64_t features);
											  ...
											  FAR struct virtqueue *vq)
{
	...
	pci_write_io_dword(vpdev->dev, (uintptr_t)(vpdev->ioaddr + VIRTIO_PCI_QUEUE_PFN),
					   up_addrenv_va_to_pa(vq->vq_ring.desc) >> VIRTIO_PCI_QUEUE_ADDR_SHIFT);
...
void virtio_pci_legacy_delete_virtqueue(FAR struct virtio_device *vdev, int index)
{
	...
	pci_write_io_word(vpdev->dev, (uintptr_t)(vpdev->ioaddr + VIRTIO_PCI_QUEUE_SEL), index);
	...
	pci_read_io_byte(vpdev->dev, (uintptr_t)(vpdev->ioaddr + VIRTIO_PCI_ISR), &isr);
#endif
...
static int virtio_pci_legacy_init_device(FAR struct virtio_pci_device_s *vpdev)
{
