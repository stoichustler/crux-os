...
static void vhost_defered_probe_work(FAR void *arg)
{
	...
	list_for_every_entry_safe(&g_vhost_bus.defered_device, item, tmp, struct vhost_device_item_s,
							  node)
	{
		...
		list_for_every_entry(&g_vhost_bus.driver, driver, struct vhost_driver, node)
		{
	...
	if (!list_is_empty(&g_vhost_bus.defered_device)) {
		work_queue(LPWORK, &g_vhost_bus.defered_work, vhost_defered_probe_work, NULL,
...
int vhost_register_driver(FAR struct vhost_driver *driver)
{
	...
	DEBUGASSERT(driver != NULL && driver->probe != NULL && driver->remove != NULL);
	...
	list_for_every_entry(&g_vhost_bus.device, item, struct vhost_device_item_s, node)
	{
...
int vhost_unregister_device(FAR struct vhost_device *device)
{
	...
	list_for_every_entry(&g_vhost_bus.defered_device, item, struct vhost_device_item_s, node)
	{
		if (item->device == device) {
			list_delete(&item->node);
			kmm_free(item);
