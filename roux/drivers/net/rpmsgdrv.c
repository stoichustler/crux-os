...
static int net_rpmsg_drv_default_handler(FAR struct rpmsg_endpoint *ept, FAR void *data, size_t len,
										 uint32_t src, FAR void *priv);
static int net_rpmsg_drv_sockioctl_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
										   size_t len, uint32_t src, FAR void *priv);
static int net_rpmsg_drv_transfer_handler(FAR struct rpmsg_endpoint *ept, FAR void *data,
										  size_t len, uint32_t src, FAR void *priv);

static void net_rpmsg_drv_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_);
static void net_rpmsg_drv_device_destroy(FAR struct rpmsg_device *rdev, FAR void *priv_);
static int net_rpmsg_drv_ept_cb(FAR struct rpmsg_endpoint *ept, void *data, size_t len,
								uint32_t src, FAR void *priv);

static int net_rpmsg_drv_send_recv(struct net_driver_s *dev, void *header_, uint32_t command,
								   int len);
...
static int net_rpmsg_drv_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
										   ...
										   size_t len, uint32_t src, FAR void *priv)
{
	...
	kthread_create("rpmsg-net", CONFIG_NET_RPMSG_PRIORITY, CONFIG_NET_RPMSG_STACKSIZE,
				   net_rpmsg_drv_sockioctl_task, argv);
...
{
	FAR struct ipv4_hdr_s *ip = (FAR struct ipv4_hdr_s *)(dev->d_buf + dev->d_llhdrlen);
...
{
	FAR struct ipv6_hdr_s *ip = (FAR struct ipv6_hdr_s *)(dev->d_buf + dev->d_llhdrlen);
	FAR struct eth_hdr_s *eth = (FAR struct eth_hdr_s *)dev->d_buf;
...
static void net_rpmsg_drv_device_created(FAR struct rpmsg_device *rdev, FAR void *priv_)
{
	...
	if (!strcmp(priv->cpuname, rpmsg_get_cpuname(rdev))) {
		priv->ept.priv = dev;
		snprintf(eptname, sizeof(eptname), NET_RPMSG_EPT_NAME, priv->devname);
...
static int net_rpmsg_drv_send_recv(FAR struct net_driver_s *dev, FAR void *header_,
								   uint32_t command, int len)
{
	FAR struct net_rpmsg_drv_s *priv = dev->d_private;
 ...
 *
 * Description:
 *   ROUX Callback: Bring up the link interface when an IP address is
 *   provided
 *
 * Parameters:
 *   dev - Reference to the ROUX driver state structure
...
static int net_rpmsg_drv_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg)
{
	...
	if (len >= 0) {
		...
		ret = net_rpmsg_drv_send_recv(dev, msg, NET_RPMSG_DEVIOCTL, sizeof(*msg) + len);
		if (ret >= 0) {
...
int net_rpmsg_drv_init(FAR const char *cpuname, FAR const char *devname, enum net_lltype_e lltype)
{
	...
	ret = rpmsg_register_callback(dev, net_rpmsg_drv_device_created, net_rpmsg_drv_device_destroy,
								  NULL, NULL);
	...
	if (ret < 0) {
		rpmsg_unregister_callback(dev, net_rpmsg_drv_device_created, net_rpmsg_drv_device_destroy,
								  NULL, NULL);
		kmm_free(priv);
	}
