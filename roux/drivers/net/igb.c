...
#define IGB_GPIE_MSIX_SINGLE (IGB_GPIE_NSICR | IGB_GPIE_EIAME | IGB_GPIE_PBASUPPORT)
#define IGB_MSIX_IMS (IGB_IC_TXDW | IGB_IC_LSC | IGB_IC_RXMISS | IGB_IC_RXDW)
#define IGB_MSIX_EIMS (IGB_EIMS_NOMSIX_OTHER | IGB_EIMS_NOMSIX_RXTX0)
...
static uint32_t igb_getreg_mem(FAR struct igb_driver_s *priv, unsigned int offset);
static void igb_putreg_mem(FAR struct igb_driver_s *priv, unsigned int offset, uint32_t value);
#ifdef CONFIG_DEBUG_NET_INFO
static void igb_dump_reg(FAR struct igb_driver_s *priv, FAR const char *msg, unsigned int offset);
...
static int igb_transmit(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt);
...
#ifdef CONFIG_NET_MCASTGROUP
static uint32_t igb_hashmta(FAR struct igb_driver_s *priv, FAR const uint8_t *mac);
static int igb_addmac(FAR struct netdev_lowerhalf_s *dev, FAR const uint8_t *mac);
static int igb_rmmac(FAR struct netdev_lowerhalf_s *dev, FAR const uint8_t *mac);
#endif
...
static const struct igb_type_s g_igb_82576 = {.desc_align = 128, .mta_regs = 128};

/* Intel I211 */

static const struct igb_type_s g_igb_i211 = {.desc_align = 128, .mta_regs = 128};
...
static int igb_transmit(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt)
{
	...
	priv->tx[desc].cmd = (IGB_TDESC_CMD_EOP | IGB_TDESC_CMD_IFCS | IGB_TDESC_CMD_RS);
	priv->tx[desc].cso = 0;
...
static void igb_enable(FAR struct igb_driver_s *priv)
{
	...
	regval = (IGB_RCTL_EN | IGB_RCTL_MPE | (IGB_RCTL_BSIZE << IGB_RCTL_BSIZE_SHIFT));
...
static int igb_probe(FAR struct pci_device_s *dev)
{
	...
	priv->tx = kmm_memalign(type->desc_align, IGB_TX_DESC * sizeof(struct igb_tx_leg_s));
	if (priv->tx == NULL) {
	...
	priv->rx = kmm_memalign(type->desc_align, IGB_RX_DESC * sizeof(struct igb_rx_leg_s));
	if (priv->rx == NULL) {
