...
static uint8_t g_port_vlan_config[] = {
	CONFIG_NET_KSZ9477_PORT_VLAN_PHY1, CONFIG_NET_KSZ9477_PORT_VLAN_PHY2,
	CONFIG_NET_KSZ9477_PORT_VLAN_PHY3, CONFIG_NET_KSZ9477_PORT_VLAN_PHY4,
	CONFIG_NET_KSZ9477_PORT_VLAN_PHY5, CONFIG_NET_KSZ9477_PORT_VLAN_RMII,
...
{
	return (data << 24) | (data >> 24) | ((data << 8) & 0xff0000) | ((data >> 8) & 0xff00);
}
...
static int ksz9477_sgmii_read_indirect(uint32_t address, uint16_t *value, unsigned len)
{
	int ret;
	uint32_t data;
...
static int ksz9477_sgmii_write_indirect(uint32_t address, uint16_t *value, unsigned len)
{
	int ret;
	uint32_t data;
...
int ksz9477_configure_port_vlan(ksz9477_port_t port, uint8_t disable, uint8_t enable)
{
...
int ksz9477_init(ksz9477_port_t master_port)
{
	...
	ret = ksz9477_sgmii_read_indirect(KSZ9477_SGMII_ID1, (FAR uint16_t *)&regval32, 2);
	...
	if (master_port == KSZ9477_PORT_SGMII) {
		...
		regval16 = (SGMII_AUTONEG_CONTROL_PCS_SGMII | SGMII_AUTONEG_CONTROL_TC_MASTER |
					SGMII_AUTONEG_CONTROL_LINK_STATUS);
		ret = ksz9477_sgmii_write_indirect(KSZ9477_SGMII_AUTONEG_CONTROL, &regval16, 1);
		...
		ret = ksz9477_sgmii_write_indirect(KSZ9477_SGMII_AUTONEG_ADVERTISE, &regval16, 1);
	...
	for (i = 0; ret == OK && i < 7; i++) {
		ret = ksz9477_reg_write32(KSZ9477_Q_MGMT_PORT_CONTROL1(KSZ9477_PORT_PHY1 + i),
	...
	for (i = 0; ret == OK && i < 7; i++) {
		ret = ksz9477_reg_write8(KSZ9477_PORT_MIRROR_CONTROL(KSZ9477_PORT_PHY1 + i),
