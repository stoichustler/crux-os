...
static void setup_i2c_transfer(FAR struct i2c_msg_s *msg, uint8_t *data, size_t len, bool read)
{
...
int ksz9477_read(FAR struct ksz9477_transfer_s *read_msg)
{
	...
	setup_i2c_transfer(&msg[0], (FAR uint8_t *)&read_msg->reg, sizeof(read_msg->reg), false);
...
int ksz9477_write(FAR struct ksz9477_transfer_s *write_msg)
{
	...
	setup_i2c_transfer(&msg, (FAR uint8_t *)&write_msg->reg, write_msg->len, false);
...
int ksz9477_i2c_init(FAR struct i2c_master_s *i2c_bus, ksz9477_port_t master_port)
{
	if (!i2c_bus) {
		return -EINVAL;
	}
