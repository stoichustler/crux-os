...
#include <roux/net/lan9250.h>

#ifdef CONFIG_NET_PKT
#include <roux/net/pkt.h>
#endif

#ifdef CONFIG_LAN9250_SPI
#include <roux/spi/spi.h>
#else
#include <roux/spi/qspi.h>
#endif
...
#define LAN9250_PKTBUF_SIZE LAN9250_ALIGN(MAX_NETDEV_PKTSIZE + CONFIG_NET_GUARDSIZE + 4)
...
#define lan9250_setreg_dump(a, v) syslog(LOG_DEBUG, "LAN9250 REG: 0x%04x<-0x%08x\n", a, v)
#define lan9250_getreg_dump(a, v) syslog(LOG_DEBUG, "LAN9250 REG: 0x%04x->0x%08x\n", a, v)
#define lan9250_setmacreg_dump(a, v) syslog(LOG_DEBUG, "LAN9250 MAC: 0x%02x<-0x%08x\n", a, v)
#define lan9250_getmacreg_dump(a, v) syslog(LOG_DEBUG, "LAN9250 MAC: 0x%02x->0x%08x\n", a, v)
#define lan9250_setphyreg_dump(a, v) syslog(LOG_DEBUG, "LAN9250 PHY: 0x%02x<-0x%08x\n", a, v)
#define lan9250_getphyreg_dump(a, v) syslog(LOG_DEBUG, "LAN9250 PHY: 0x%02x->0x%08x\n", a, v)
...
static uint32_t lan9250_get_reg(FAR struct lan9250_driver_s *priv, uint16_t address);
static void lan9250_set_reg(FAR struct lan9250_driver_s *priv, uint16_t address, uint32_t data);
static void lan9250_wait_ready(FAR struct lan9250_driver_s *priv, uint16_t address, uint32_t mask,
							   uint32_t expected, uint32_t timeout);

/* MAC register access */

static uint32_t lan9250_get_macreg(FAR struct lan9250_driver_s *priv, uint8_t address);
static void lan9250_set_macreg(FAR struct lan9250_driver_s *priv, uint8_t address, uint32_t data);
static void lan9250_wait_mac_ready(FAR struct lan9250_driver_s *priv, uint8_t address,
								   uint32_t mask, uint32_t expected, uint32_t timeout);

/* PHY register access */

static uint16_t lan9250_get_phyreg(FAR struct lan9250_driver_s *priv, uint8_t phyaddr);
static void lan9250_set_phyreg(FAR struct lan9250_driver_s *priv, uint8_t phyaddr,
							   uint16_t phydata);

/* SPI buffer transfers */

static void lan9250_recv_buffer(FAR struct lan9250_driver_s *priv, FAR uint8_t *buffer,
								size_t buflen);
static inline void lan9250_send_buffer(FAR struct lan9250_driver_s *priv, FAR const uint8_t *buffer,
									   size_t buflen);
...
static void lan9250_set_txavailabe(FAR struct lan9250_driver_s *priv, bool enable);
...
static int lan9250_addmac(FAR struct net_driver_s *dev, FAR const uint8_t *mac);
static int lan9250_rmmac(FAR struct net_driver_s *dev, FAR const uint8_t *mac);
#endif

#ifdef CONFIG_NETDEV_IOCTL
static int lan9250_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
							   ...
							   uint32_t expected, uint32_t second)
{
	...
	if (timeout) {
		nerr("ERROR: wait register:0x%04" PRIx16 ", mask:0x%08" PRIx32 ", expected:0x%08" PRIx32
			 "\n",
...
static uint32_t lan9250_get_macreg(FAR struct lan9250_driver_s *priv, uint8_t address)
{
	...
	lan9250_wait_ready(priv, LAN9250_HMCSRICR, HMCSRICR_CSRB, 0, LAN9250_MAC_TIMEOUT);
...
static void lan9250_wait_mac_ready(FAR struct lan9250_driver_s *priv, uint8_t address,
								   uint32_t mask, uint32_t expected, uint32_t second)
{
	clock_t tout_ticks = clock_systime_ticks() + SEC2TICK(second);
	bool timeout = false;
	...
	if (timeout) {
		nerr("ERROR: wait MAC register:0x%02" PRIx8 ", mask:0x%08" PRIx32 ", expect:0x%08" PRIx32
			 "\n",
			 address, mask, expected);
	}
}
...
static uint16_t lan9250_get_phyreg(FAR struct lan9250_driver_s *priv, uint8_t address)
{
	uint32_t regval = (1 << HMACMIIAR_PHYA_S) | (address << HMACMIIAR_MIIRX_S);

	/* Wait PHY to be ready and send reading register command */

	lan9250_wait_mac_ready(priv, LAN9250_HMACMIIAR, HMACMIIAR_MIIB, 0, LAN9250_PHY_TIMEOUT);
...
static void lan9250_set_phyreg(FAR struct lan9250_driver_s *priv, uint8_t address, uint16_t data)
{
	uint32_t regval = (1 << HMACMIIAR_PHYA_S) | (address << HMACMIIAR_MIIRX_S) | HMACMIIAR_MIIW;

	DEBUGASSERT((address & (HMACMIIAR_MIIRX_M >> HMACMIIAR_MIIRX_S)) == address);
									   ...
									   size_t buflen)
{
	...
	regval = SPI_CMD_WRITE_A_4BA | SPI_CMD_WRITE_A_FS | SPI_CMD_WRITE_A_LS | buflen;
...
static void lan9250_enable_sqi(FAR struct lan9250_driver_s *priv)
{
	clock_t tout_ticks = clock_systime_ticks() + MSEC2TICK(LAN9250_SQI_TIMEOUT);
	struct qspi_cmdinfo_s cmdinfo;
	int ret;
...
static inline void lan9250_sw_reset(FAR struct lan9250_driver_s *priv)
{
	...
	lan9250_wait_ready(priv, LAN9250_BOTR, BOTR_MASK, BOTR_VAL, LAN9250_RESET_TIMEOUT);
	...
	lan9250_wait_ready(priv, LAN9250_HWCFGR, HWCFGR_READY, HWCFGR_READY, LAN9250_RESET_TIMEOUT);
...
static void lan9250_set_txavailabe(FAR struct lan9250_driver_s *priv, bool enable)
{
	...
	if (enable) {
		...
		regval = (((LAN9250_PKTBUF_SIZE + 63) / 64) << FLIR_FITXDAL_S) | FLIR_FITXSL_M;
...
static int lan9250_reset(FAR struct lan9250_driver_s *priv)
{
	...
	regval = (110 << HMAFCCFGR_AFCHL_S) | (55 << HMAFCCFGR_AFCLL_S) | (4 << HMAFCCFGR_BPD_S) |
			 HMAFCCFGR_FCOAF;
	...
	regval = (10 << ICFGR_IDAI_S) | ICFGR_IRQE | ICFGR_IRQBT;
	...
	regval = PMCR_PMWU | PMCR_1588CLKD | PMCR_1588TSUCLKD | PMCR_EDE | PMCR_WOE | PMCR_PMEBT |
	...
	regval = PHYANAR_AP | PHYANAR_SP | PHYANAR_100BXHD | PHYANAR_10BXHD | PHYANAR_SF;
...
static void lan9250_set_macaddr(FAR struct lan9250_driver_s *priv)
{
	...
	low_addr = (((uint32_t)mac[3] << 24) | ((uint32_t)mac[2] << 16) | ((uint32_t)mac[1] << 8) |
				((uint32_t)mac[0] << 0));
...
static int lan9250_transmit(FAR struct lan9250_driver_s *priv)
{
	...
	if (free_size > priv->dev.d_len) {
		...
		lan9250_dump_buf("Transmit Packet", priv->dev.d_buf, priv->dev.d_len);
		...
		if ((free_size - priv->dev.d_len) < LAN9250_PKTBUF_SIZE) {
			...
			wd_start(&priv->txtout_timer, LAN9250_TX_TIMEOUT, lan9250_txtout_timercb,
					 (wdparm_t)priv);
		}
	} else {
...
static int lan9250_txpoll(FAR struct net_driver_s *dev)
{
	FAR struct lan9250_driver_s *priv = (FAR struct lan9250_driver_s *)dev->d_private;
...
static void lan9250_rxdone_isr(FAR struct lan9250_driver_s *priv)
{
	...
	while (pktcnt--) {
		...
		if (pktlen) {
			...
			lan9250_dump_buf("Received Packet", priv->dev.d_buf, priv->dev.d_len);
...
static int lan9250_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	return work_queue(LAN9250_WORK, &priv->irq_work, lan9250_int_worker, (FAR void *)priv, 0);
}
...
static void lan9250_txtout_timercb(wdparm_t arg)
{
	...
	ret = work_queue(LAN9250_WORK, &priv->txtout_work, lan9250_txtout_worker, priv, 0);
	DEBUGASSERT(ret == OK);
	UNUSED(ret);
}
...
static int lan9250_ifup(FAR struct net_driver_s *dev)
{
	...
	if (ret == OK) {
		...
		ninfo("MAC: %02x:%02x:%02x:%02x:%02x:%02x\n", (uint8_t)(mac_addr[1] >> 0),
			  (uint8_t)(mac_addr[1] >> 8), (uint8_t)(mac_addr[0] >> 24),
			  (uint8_t)(mac_addr[0] >> 16), (uint8_t)(mac_addr[0] >> 8),
			  (uint8_t)(mac_addr[0] >> 0));
#endif
	}
...
static int lan9250_txavail(FAR struct net_driver_s *dev)
{
	...
	if (work_available(&priv->txpoll_work)) {
		...
		work_queue(LAN9250_WORK, &priv->txpoll_work, lan9250_txavail_work, priv, 0);
	}
