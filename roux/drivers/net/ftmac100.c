...
#define MACCR_ENABLE_ALL                                                                           \
	(FTMAC100_MACCR_XMT_EN | FTMAC100_MACCR_RCV_EN | FTMAC100_MACCR_XDMA_EN |                      \
	 FTMAC100_MACCR_RDMA_EN | FTMAC100_MACCR_CRC_APD | FTMAC100_MACCR_FULLDUP |                    \
	 FTMAC100_MACCR_RX_RUNT | FTMAC100_MACCR_HT_MULTI_EN | FTMAC100_MACCR_RX_BROADPKT)
#else
#define MACCR_ENABLE_ALL                                                                           \
	(FTMAC100_MACCR_XMT_EN | FTMAC100_MACCR_RCV_EN | FTMAC100_MACCR_XDMA_EN |                      \
	 FTMAC100_MACCR_RDMA_EN | FTMAC100_MACCR_CRC_APD | FTMAC100_MACCR_FULLDUP |                    \
	 FTMAC100_MACCR_RX_RUNT | FTMAC100_MACCR_RX_BROADPKT)
#endif

#define MACCR_DISABLE_ALL 0

#define INT_MASK_ALL_ENABLED                                                                       \
	(FTMAC100_INT_RPKT_FINISH | FTMAC100_INT_NORXBUF | FTMAC100_INT_XPKT_OK |                      \
	 FTMAC100_INT_XPKT_LOST | FTMAC100_INT_RPKT_LOST | FTMAC100_INT_AHB_ERR |                      \
...
static uint16_t g_pktbuf[CONFIG_FTMAC100_NINTERFACES][(PKTBUF_SIZE + 1) / 2];

/* Driver state structure. */

static struct ftmac100_driver_s g_ftmac100[CONFIG_FTMAC100_NINTERFACES] aligned_data(16);
...
static FAR struct ftmac100_rxdes_s *ftmac100_current_rxdes(FAR struct ftmac100_driver_s *priv)
{
	return &priv->rxdes[priv->rx_pointer];
}

static FAR struct ftmac100_txdes_s *ftmac100_current_txdes(FAR struct ftmac100_driver_s *priv)
{
	return &priv->txdes[priv->tx_pointer];
}

static FAR struct ftmac100_txdes_s *ftmac100_current_clean_txdes(FAR struct ftmac100_driver_s *priv)
{
...
static int ftmac100_transmit(FAR struct ftmac100_driver_s *priv)
{
	...
	txdes->txdes1 |= (FTMAC100_TXDES1_FTS | FTMAC100_TXDES1_LTS | FTMAC100_TXDES1_TXIC |
	...
	wd_start(&priv->ft_txtimeout, FTMAC100_TXTIMEOUT, ftmac100_txtimeout_expiry, (wdparm_t)priv);
 ...
 *
 * Input Parameters:
 *   priv - Reference to the ROUX driver state structure
 *
 * Returned Value:
 *   None
 *
...
static void ftmac100_init(FAR struct ftmac100_driver_s *priv)
{
  ...
  putreg32(FTMAC100_ITC_RXINT_THR(1) | FTMAC100_ITC_TXINT_THR(1),
	...
	putreg32(FTMAC100_DBLAC_RXFIFO_LTHR(2) | FTMAC100_DBLAC_RXFIFO_HTHR(6) |
				 FTMAC100_DBLAC_RX_THR_EN,
			 &iobase->dblac);
...
{
	int i;
	uint32_t phycr = FTMAC100_PHYCR_PHYAD(1) | FTMAC100_PHYCR_REGAD(reg) | FTMAC100_PHYCR_MIIRD;
...
static void ftmac100_set_mac(FAR struct ftmac100_driver_s *priv, FAR const unsigned char *mac)
{
	FAR struct ftmac100_register_s *iobase = (FAR struct ftmac100_register_s *)priv->iobase;
...
static void ftmac100_interrupt_work(FAR void *arg)
{
	...
	ninfo("status=%08x(%08x) BASE=%p ISR=%p PHYCR=%p\n", status, getreg32(&iobase->isr), iobase,
		  &iobase->isr, &iobase->phycr);
...
int ftmac100_initialize(int intf)
{
	...
	memcpy(priv->ft_dev.d_mac.ether.ether_addr_octet, (FAR void *)(CONFIG_FTMAC100_MAC0_ENV_ADDR),
		   6);
