...
#define PKTMEM_RX_END                                                                              \
	(PKTMEM_END - ALIGNED_BUFSIZE)			/* RX buffer length is total SRAM minus TX buffer */
...
#define enc_rdgreg(priv, ctrlreg) enc_rdgreg2(priv, ENC_RCR | GETADDR(ctrlreg))
#define enc_wrgreg(priv, ctrlreg, wrdata) enc_wrgreg2(priv, ENC_WCR | GETADDR(ctrlreg), wrdata)
#define enc_bfcgreg(priv, ctrlreg, clrbits) enc_wrgreg2(priv, ENC_BFC | GETADDR(ctrlreg), clrbits)
#define enc_bfsgreg(priv, ctrlreg, setbits) enc_wrgreg2(priv, ENC_BFS | GETADDR(ctrlreg), setbits)
...
#define enc_wrdump(a, v) syslog(LOG_DEBUG, "ENC28J60: %02x<-%02x\n", a, v);
#define enc_rddump(a, v) syslog(LOG_DEBUG, "ENC28J60: %02x->%02x\n", a, v);
#define enc_cmddump(c) syslog(LOG_DEBUG, "ENC28J60: CMD: %02x\n", c);
...
static int enc_waitbreg(FAR struct enc_driver_s *priv, uint8_t ctrlreg, uint8_t bits,
						uint8_t value);
...
static inline void enc_wrbuffer(FAR struct enc_driver_s *priv, FAR const uint8_t *buffer,
								size_t buflen);
								...
								size_t buflen)
{
	...
	SPI_SEND(priv->spi, (PKTCTRL_PCRCEN | PKTCTRL_PPADEN | PKTCTRL_PHUGEEN));
...
static int enc_transmit(FAR struct enc_driver_s *priv)
{
	...
	wd_start(&priv->txtimeout, ENC_TXTIMEOUT, enc_txtimeout, (wdparm_t)priv);
	return OK;
}
...
static int enc_ifup(struct net_driver_s *dev)
{
	...
	if (ret == OK) {
		...
		enc_wrgreg(priv, ENC_EIE,
				   EIE_INTIE | EIE_PKTIE | EIE_LINKIE | EIE_TXIE | EIE_TXERIE | EIE_RXERIE);
...
int enc_initialize(FAR struct spi_dev_s *spi, FAR const struct enc_lower_s *lower,
				   unsigned int devno)
{
	FAR struct enc_driver_s *priv;
