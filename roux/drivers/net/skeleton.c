...
static uint16_t g_pktbuf[CONFIG_NET_SKELETON_NINTERFACES][(PKTBUF_SIZE + 1) / 2];
...
static int skel_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
...
static int skel_transmit(FAR struct skel_driver_s *priv)
{
	...
	wd_start(&priv->sk_txtimeout, SKELETON_TXTIMEOUT, skel_txtimeout_expiry, (wdparm_t)priv);
	return OK;
}
