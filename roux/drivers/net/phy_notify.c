...
static FAR struct phy_notify_s *phy_find_assigned(FAR const char *intf, pid_t pid)
{
	...
	for (i = 0; i < CONFIG_PHY_NOTIFICATION_NCLIENTS; i++) {
		client = &g_notify_clients[i];
		if (client->assigned && client->pid == pid && strncmp(client->intf, intf, IFNAMSIZ) == 0) {
...
static int phy_handler(int irq, FAR void *context, FAR void *arg)
{
	...
	ret = nxsig_notification(client->pid, &client->event, SI_QUEUE, &client->work);
	if (ret < 0) {
...
int phy_notify_subscribe(FAR const char *intf, pid_t pid, FAR struct sigevent *event)
{
	FAR struct phy_notify_s *client;
	int ret = OK;
