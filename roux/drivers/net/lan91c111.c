...
static int lan91c111_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
...
static void putreg8(FAR struct lan91c111_driver_s *priv, uint16_t offset, uint8_t value)
{
	offset = updatebank(priv, offset);
	*(FAR volatile uint8_t *)(priv->base + offset) = value;
}

static void putreg16(FAR struct lan91c111_driver_s *priv, uint16_t offset, uint16_t value)
{
	offset = updatebank(priv, offset);
	*(FAR volatile uint16_t *)(priv->base + offset) = value;
}

static void modifyreg16(FAR struct lan91c111_driver_s *priv, uint16_t offset, uint16_t clearbits,
						uint16_t setbits)
{
	uint16_t value;
...
static void getregs32(FAR struct lan91c111_driver_s *priv, uint16_t offset, void *value_,
					  size_t length)
{
...
static void putregs32(FAR struct lan91c111_driver_s *priv, uint16_t offset, const void *value_,
					  size_t length)
{
...
static void copyfrom16(FAR struct lan91c111_driver_s *priv, uint16_t offset, FAR void *value_,
					   size_t length)
{
...
static void copyto16(FAR struct lan91c111_driver_s *priv, uint16_t offset, FAR const void *value_,
					 size_t length)
{
...
static void outmii(FAR struct lan91c111_driver_s *priv, uint32_t value, size_t bits)
{
	uint32_t mask;
	uint16_t mii;
...
static void putphy(FAR struct lan91c111_driver_s *priv, uint8_t offset, uint16_t value)
{
	/* Idle - 32 ones */
...
static int lan91c111_ifup(FAR struct net_driver_s *dev)
{
	...
	putreg8(priv, IM_REG, IM_MDINT | IM_RX_OVRN_INT | IM_RCV_INT | IM_TX_INT);

	putphy(priv, PHY_MASK_REG, /* Interrupts listed here are disabled */
		   PHY_INT_LOSSSYNC | PHY_INT_CWRD | PHY_INT_SSD | PHY_INT_ESD | PHY_INT_RPOL |
			   PHY_INT_JAB | PHY_INT_SPDDET | PHY_INT_DPLXDET);
