...
#define IGC_GPIE_MSIX_SINGLE (IGC_GPIE_NSICR | IGC_GPIE_EIAME | IGC_GPIE_PBASUPPORT)
#define IGC_MSIX_IMS (IGC_IC_TXDW | IGC_IC_LSC | IGC_IC_RXMISS | IGC_IC_RXDW)
#define IGC_MSIX_EIMS (IGC_EIMS_NOMSIX_OTHER | IGC_EIMS_NOMSIX_RXTX0)
...
static uint32_t igc_getreg_mem(FAR struct igc_driver_s *priv, unsigned int offset);
static void igc_putreg_mem(FAR struct igc_driver_s *priv, unsigned int offset, uint32_t value);
#ifdef CONFIG_DEBUG_NET_INFO
static void igc_dump_reg(FAR struct igc_driver_s *priv, FAR const char *msg, unsigned int offset);
...
static int igc_transmit(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt);
...
#ifdef CONFIG_NET_MCASTGROUP
static uint32_t igc_hashmta(FAR struct igc_driver_s *priv, FAR const uint8_t *mac);
static int igc_addmac(FAR struct netdev_lowerhalf_s *dev, FAR const uint8_t *mac);
static int igc_rmmac(FAR struct netdev_lowerhalf_s *dev, FAR const uint8_t *mac);
#endif
...
static const struct igc_type_s g_igc_i225lm = {.desc_align = 128, .mta_regs = 128};

static const struct igc_type_s g_igc_i226v = {.desc_align = 128, .mta_regs = 128};

static const struct pci_device_id_s g_igc_id_table[] = {
	{PCI_DEVICE(0x8086, 0x15f2), .driver_data = (uintptr_t)&g_igc_i225lm},
	{PCI_DEVICE(0x8086, 0x125c), .driver_data = (uintptr_t)&g_igc_i226v},
	{}};
...
static int igc_transmit(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt)
{
	...
	priv->tx[desc].cmd = (IGC_TDESC_CMD_EOP | IGC_TDESC_CMD_IFCS | IGC_TDESC_CMD_RS);
	priv->tx[desc].cso = 0;
...
static void igc_enable(FAR struct igc_driver_s *priv)
{
	...
	regval = (IGC_RCTL_EN | IGC_RCTL_MPE | (IGC_RCTL_BSIZE << IGC_RCTL_BSIZE_SHIFT));
...
static int igc_probe(FAR struct pci_device_s *dev)
{
	...
	priv->tx = kmm_memalign(type->desc_align, IGC_TX_DESC * sizeof(struct igc_tx_leg_s));
	if (priv->tx == NULL) {
	...
	priv->rx = kmm_memalign(type->desc_align, IGC_RX_DESC * sizeof(struct igc_rx_leg_s));
	if (priv->rx == NULL) {
