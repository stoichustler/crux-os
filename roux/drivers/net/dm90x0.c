 ...
 * in the ROUX configuration file.
 */
...
static void dm9x_phywrite(FAR struct dm9x_driver_s *priv, int reg, uint16_t value);
...
static int dm9x_transmit(FAR struct dm9x_driver_s *priv)
{
	...
	if (priv->dm_ntxpending < 1 || (priv->dm_b100m && priv->dm_ntxpending < 2)) {
		...
		wd_start(&priv->dm_txtimeout, DM6X_TXTIMEOUT, dm9x_txtimeout_expiry, (wdparm_t)priv);
		return OK;
	}
 ...
 *
 * Description:
 *   ROUX Callback: Bring up the DM90x0 interface when an IP address is
 *   provided
 *
 * Input Parameters:
 *   dev  - Reference to the ROUX driver state structure
