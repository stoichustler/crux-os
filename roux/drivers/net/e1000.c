...
#else
#define E1000_INTERRUPTS                                                                           \
	(E1000_IC_RXO | E1000_IC_RXT0 | E1000_IC_RXDMT0 | E1000_IC_LSC | E1000_IC_TXDW)
#endif
...
#else
#define E1000_MSIX_INTERRUPTS (E1000_IC_RXQ0 | E1000_IC_TXQ0 | E1000_IC_OTHER)
#define E1000_MSIX_IVAR (E1000_IVAR_RXQ0_EN | E1000_IVAR_TXQ0_EN | E1000_IVAR_OTHER_EN)
#endif
...
static uint32_t e1000_getreg_mem(FAR struct e1000_driver_s *priv, unsigned int offset);
static void e1000_putreg_mem(FAR struct e1000_driver_s *priv, unsigned int offset, uint32_t value);
#ifdef CONFIG_DEBUG_NET_INFO
static void e1000_dump_reg(FAR struct e1000_driver_s *priv, FAR const char *msg,
						   unsigned int offset);
static void e1000_dump_mem(FAR struct e1000_driver_s *priv, FAR const char *msg);
#endif
...
static int e1000_transmit(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt);
...
#ifdef CONFIG_NET_MCASTGROUP
static uint32_t e1000_hashmta(FAR struct e1000_driver_s *priv, FAR const uint8_t *mac);
static int e1000_addmac(FAR struct netdev_lowerhalf_s *dev, FAR const uint8_t *mac);
static int e1000_rmmac(FAR struct netdev_lowerhalf_s *dev, FAR const uint8_t *mac);
#endif
...
static const struct e1000_type_s g_e1000_i219 = {
	.desc_align = 128, .mta_regs = 32, .flags = E1000_RESET_BROKEN};

/* Intel 82801IB (QEMU -device e1000) */

static const struct e1000_type_s g_e1000_82540em = {.desc_align = 16, .mta_regs = 128, .flags = 0};

/* Intel 82574L (QEMU -device e1000e) */

static const struct e1000_type_s g_e1000_82574l = {
	.desc_align = 128, .mta_regs = 128, .flags = E1000_HAS_MSIX};

static const struct pci_device_id_s g_e1000_id_table[] = {
	{PCI_DEVICE(0x8086, 0x1a1e), .driver_data = (uintptr_t)&g_e1000_i219},
	{PCI_DEVICE(0x8086, 0x100e), .driver_data = (uintptr_t)&g_e1000_82540em},
	{PCI_DEVICE(0x8086, 0x10d3), .driver_data = (uintptr_t)&g_e1000_82574l},
	{}};
...
static int e1000_transmit(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt)
{
	...
	priv->tx[desc].cmd =
		(E1000_TDESC_CMD_EOP | E1000_TDESC_CMD_IFCS | E1000_TDESC_CMD_RS | E1000_TDESC_CMD_RPS);
	priv->tx[desc].cso = 0;
...
static FAR netpkt_t *e1000_receive(FAR struct netdev_lowerhalf_s *dev)
{
	...
	rx->addr = up_addrenv_va_to_pa(netpkt_getdata(dev, priv->rx_pkt[desc]));
	rx->len = 0;
	rx->status = 0;
...
static void e1000_enable(FAR struct e1000_driver_s *priv)
{
	...
	for (i = 0; i < E1000_RX_DESC; i += 1) {
		...
		priv->rx[i].addr = up_addrenv_va_to_pa(netpkt_getdata(dev, priv->rx_pkt[i]));
		priv->rx[i].len = 0;
		priv->rx[i].status = 0;
	}
	...
	regval = E1000_RCTL_EN | E1000_RCTL_MPE | (E1000_RCTL_BSIZE << E1000_RCTL_BSIZE_SHIFT) |
...
static int e1000_probe(FAR struct pci_device_s *dev)
{
	...
	priv->tx = kmm_memalign(type->desc_align, E1000_TX_DESC * sizeof(struct e1000_tx_leg_s));
	if (priv->tx == NULL) {
	...
	priv->rx = kmm_memalign(type->desc_align, E1000_RX_DESC * sizeof(struct e1000_rx_leg_s));
	if (priv->rx == NULL) {
