...
static void slip_set_pollfd_events(FAR struct slip_driver_s *self, short events);
...
static void slip_transmit(FAR struct slip_driver_s *self)
{
	...
	if (self->txlen > 0) {
		...
		for (i = 0; (i < 10) && (self->txsent != self->txlen);) {
			ssz = file_write(&self->tty, &self->txbuf[self->txsent], self->txlen - self->txsent);
			if (ssz <= 0) {
				nxsig_usleep(10000);
				i++;
				continue;
	...
	for (unsigned int bytesread = 0; bytesread < self->dev.d_len;) {
		...
		copied = iob_copyout(&self->txbuf[self->txlen], self->dev.d_iob, chunk_sz, bytesread);
		if (copied <= 0) {
			goto error;
		}
	...
	for (p = memchr(&self->txbuf[1], SLIP_ESC, self->txlen - 1); p != NULL;
	...
	for (p = memchr(&self->txbuf[1], SLIP_END, self->txlen - 1); p != NULL;
...
static int slip_txpoll(FAR struct net_driver_s *dev)
{
	FAR struct slip_driver_s *self = (FAR struct slip_driver_s *)dev->d_private;
...
static void slip_interrupt_work(FAR void *arg)
{
	...
	if (self->rxlen < sizeof(self->rxbuf)) {
		ssz = file_read(&self->tty, &self->rxbuf[self->rxlen], sizeof(self->rxbuf) - self->rxlen);
		if (ssz > 0) {
	...
	if (self->rxlen == sizeof(self->rxbuf) || memchr(self->rxbuf, SLIP_END, self->rxlen)) {
		slip_receive(self);
	}
