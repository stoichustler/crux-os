...
#define enc_wrdump(a, v) syslog(LOG_DEBUG, "ENCX24J600: %02x<-%04x\n", a, v);
#define enc_rddump(a, v) syslog(LOG_DEBUG, "ENCX24J600: %02x->%04x\n", a, v);
#define enc_bfsdump(a, m) syslog(LOG_DEBUG, "ENCX24J600: %02x|=%04x\n", a, m);
#define enc_bfcdump(a, m) syslog(LOG_DEBUG, "ENCX24J600: %02x&=~%04x\n", a, m);
#define enc_cmddump(c) syslog(LOG_DEBUG, "ENCX24J600: CMD: %02x\n", c);
...
static uint16_t g_pktbuf[CONFIG_ENCX24J600_NINTERFACES][(PKTBUF_SIZE + 1) / 2];
...
static int enc_waitreg(FAR struct enc_driver_s *priv, uint16_t ctrlreg, uint16_t bits,
					   uint16_t value);
...
static void enc_rxrmpkt(FAR struct enc_driver_s *priv, FAR struct enc_descr_s *descr)
{
	...
	if (descr != NULL) {
		if (descr == (FAR struct enc_descr_s *)sq_peek(&priv->rxqueue)) {
			...
			addr =
				(descr->addr - PKTMEM_RX_START + descr->len - 2 + PKTMEM_RX_SIZE) % PKTMEM_RX_SIZE +
				PKTMEM_RX_START;
...
static void enc_pktif(FAR struct enc_driver_s *priv)
{
	...
	while (pktcnt > 0) {
		...
		rxstat = (uint32_t)rsv[7] << 24 | (uint32_t)rsv[6] << 16 | (uint32_t)rsv[5] << 8 |
				 (uint32_t)rsv[4];
