...
#define CHAN2G(freq)                                                                               \
	{                                                                                              \
		.band = WLAN_80211_BAND_2GHZ, .center_freq = (freq),                                       \
	}

#define CHAN5G(freq)                                                                               \
	{                                                                                              \
		.band = WLAN_80211_BAND_5GHZ, .center_freq = (freq),                                       \
	}
...
static int wifidriver_essid(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr, bool set);
static int wifidriver_bssid(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr, bool set);
static int wifidriver_passwd(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr, bool set);
static int wifidriver_mode(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr, bool set);
static int wifidriver_auth(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr, bool set);
static int wifidriver_freq(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr, bool set);
static int wifidriver_bitrate(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr, bool set);
static int wifidriver_txpower(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr, bool set);
static int wifidriver_country(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr, bool set);
static int wifidriver_sensitivity(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr,
								  bool set);
static int wifidriver_scan(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr, bool set);
static int wifidriver_range(FAR struct netdev_lowerhalf_s *dev, FAR struct iwreq *iwr);

static int wifidriver_get_bssinfo(FAR struct wifi_sim_bss_s *bss_info, FAR char *buf, int len);
...
static const struct wireless_ops_s g_iw_ops = {
	wifidriver_connect, wifidriver_disconnect, wifidriver_essid,   wifidriver_bssid,
	wifidriver_passwd,	wifidriver_mode,	   wifidriver_auth,	   wifidriver_freq,
	wifidriver_bitrate, wifidriver_txpower,	   wifidriver_country, wifidriver_sensitivity,
	wifidriver_scan,	wifidriver_range};
...
static int wifi_send_event(FAR struct wifi_sim_s *wifidev, unsigned int cmd,
						   FAR union iwreq_data *wrqu)
{
	...
	alloc = (FAR struct wireless_event_list_s *)kmm_zalloc(
	...
	netlink_add_broadcast(RTNLGRP_LINK, (FAR struct netlink_response_s *)alloc);
	return OK;
}
...
static int freq_to_channel(uint16_t freq)
{
	...
	if ((freq >= 1 && freq <= 14) || (freq >= 36 && freq <= 165) || (freq >= 182 && freq <= 196)) {
		channel = freq;
		return channel;
	}
...
static FAR struct wifi_sim_bss_s *select_bss(FAR struct wifi_sim_s *sta, FAR char *bss_buf)
{
	char bss[256];
	FAR char *s;
	FAR char *p;
	...
	for (p = bss_buf; *p != '\0'; p++) {
		...
		if ((sta->ssid_flag == 2 && !memcmp(sta->bssid, bss_info.bssid, 6)) ||
			...
			if (sta->psk_flag == 0) {
				if (!strstr(bss_info.security, "WPA") && !strstr(bss_info.security, "WEP")) {
					bss_found = true;
				} else {
				...
				break;
					   ...
					   strstr(bss_info.security, get_cipherstr(sta->pairwise_chiper))) {
				bss_found = true;
				break;
			} else {
				nerr("security doesn't match");
...
static int verify_password(FAR struct wifi_sim_s *sta, FAR struct wifi_sim_bss_s *bss)
{
	int ret = ERROR;
	...
	case IW_MODE_INFRA: {
		...
		if (ret == OK) {
			...
			if (wifidev->ssid_flag == 2) {
				memcpy(wifidev->ssid, bss_info->ssid, strlen(bss_info->ssid));
			}
		} else {
...
static int wifidriver_get_mode(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	switch (wifidev->mode) {
	case IW_MODE_INFRA:
	case IW_MODE_MASTER: {
...
static int wifidriver_set_mode(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	switch (pwrq->u.mode) {
	case IW_MODE_INFRA:
	case IW_MODE_MASTER: {
...
static int wifidriver_set_country(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	switch (wifidev->mode) {
	case IW_MODE_INFRA:
	case IW_MODE_MASTER: {
		memcpy(wifidev->country, pwrq->u.data.pointer, pwrq->u.data.length);
...
static int wifidriver_get_country(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	switch (wifidev->mode) {
	case IW_MODE_INFRA:
	case IW_MODE_MASTER: {
		pwrq->u.data.length = strlen(wifidev->country);
		memcpy(pwrq->u.data.pointer, wifidev->country, pwrq->u.data.length);
		ninfo("set country is %s\n", wifidev->country);
	} break;
...
static int wifidriver_get_sensitivity(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	switch (wifidev->mode) {
	case IW_MODE_INFRA:
	case IW_MODE_MASTER: {
...
int wifidriver_set_freq(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	int ret;
...
static int wifidriver_get_freq(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	switch (wifidev->mode) {
	case IW_MODE_INFRA: {
		if (wifidriver_sta_is_connected(wifidev)) {
...
int wifidriver_set_txpower(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	int ret = OK;
...
static int wifidriver_get_txpower(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	switch (wifidev->mode) {
	case IW_MODE_INFRA: {
		pwrq->u.txpower.value = wifidev->txpower ? wifidev->txpower : WLAN_DEFAULT_TXPOWER;
...
static int wifidriver_get_bitrate(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	switch (wifidev->mode) {
	case IW_MODE_INFRA:
		if (wifidev->state != WLAN_STA_STATE_CONNECTED) {
...
static int wifidriver_get_range(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	int k;
	int i;
...
static int wifidriver_start_scan(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	switch (wifidev->mode) {
	case IW_MODE_INFRA: {
...
static int copy_scan_results(FAR struct iw_scan_result_s *scan_req, FAR struct wifi_sim_bss_s *info)
{
	int need_len;
	FAR char *pointer;
...
static int wifidriver_get_bssinfo(FAR struct wifi_sim_bss_s *bss_info, FAR char *buf, int len)
{
	...
	for (i = 0, p = buf; p - buf < len; p++, i++) {
		...
		case 3: {
			...
			bss_info->capability |= strlen(str) > strlen("[ESS]") ? IEEE80211_CAP_PRIVACY : 0x01;
...
static int get_scan_results(FAR struct wifi_sim_s *wifidev, FAR struct iw_scan_result_s *scan_reqs)
{
	int ret;
	FAR char *rbuf;
	FAR char *p;
...
static int wifidriver_scan_result(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
...
static int wifidriver_set_essid(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	switch (wifidev->mode) {
	case IW_MODE_INFRA:
	case IW_MODE_MASTER: {
...
static int wifidriver_get_essid(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
...
static int wifidriver_set_bssid(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
...
static int wifidriver_get_bssid(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	FAR struct sockaddr *sockaddr = &pwrq->u.ap_addr;
	FAR unsigned char *pdata = (FAR unsigned char *)sockaddr->sa_data;
	int ret;
...
static int wifidriver_set_auth(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
...
static int wifidriver_get_auth(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	switch (wifidev->mode) {
	case IW_MODE_INFRA:
		break;
...
static int wifidriver_set_psk(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	...
	case IW_MODE_INFRA: {
		...
		ninfo("psk=%s, key_len= %d, alg=%u\n", wifidev->password, ext->key_len, ext->alg);
...
static int wifidriver_get_psk(FAR struct wifi_sim_s *wifidev, FAR struct iwreq *pwrq)
{
	FAR struct iw_encode_ext *ext;
	int ret = OK;
	int len;
