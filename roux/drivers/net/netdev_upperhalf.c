...
static FAR netpkt_t *netpkt_get(FAR struct net_driver_s *dev, enum netpkt_type_e type)
{
...
static void netpkt_put(FAR struct net_driver_s *dev, FAR netpkt_t *pkt, enum netpkt_type_e type)
{
...
static FAR struct netdev_upperhalf_s *netdev_upper_alloc(FAR struct netdev_lowerhalf_s *dev)
{
...
static void netdev_upper_txavail_work(FAR struct netdev_upperhalf_s *upper)
{
	...
	if (IFF_IS_UP(dev->d_flags)) {
		...
		while (netdev_upper_can_tx(upper) && netdev_upper_tx(dev) == NETDEV_TX_CONTINUE)
			;
	}
}
 ...
 *
 * Input Parameters:
 *   dev - Reference to the ROUX network driver state structure
 *
 * Assumptions:
 *   Called with the network locked.
 *
...
static void eth_input(FAR struct net_driver_s *dev)
{
	...
	if (eth_hdr->type == HTONS(TPID_8021QVLAN)) {
		...
		memmove((FAR uint8_t *)eth_hdr + 4, eth_hdr, offsetof(struct eth_hdr_s, type));
...
static int netdev_upper_wait(FAR sem_t *sem)
{
	...
	int ret = nxsem_tickwait(sem, USEC2TICK(CONFIG_NETDEV_WORK_THREAD_POLLING_PERIOD));
...
static int netdev_upper_loop(int argc, FAR char *argv[])
{
	...
	while (netdev_upper_wait(&upper->sem[cpu]) == OK && upper->tid[cpu] != INVALID_PROCESS_ID) {
...
static inline void netdev_upper_queue_work(FAR struct net_driver_s *dev)
{
	...
	if (nxsem_get_value(&upper->sem[cpu], &semcount) == OK && semcount <= 0) {
...
int netdev_upper_wireless_ioctl(FAR struct netdev_lowerhalf_s *lower, int cmd, unsigned long arg)
{
	...
	case SIOCSIWESSID: /* Set ESSID */
		if (ops->essid) {
			if ((iwr->u.essid.flags == IW_ESSID_ON) || (iwr->u.essid.flags == IW_ESSID_DELAY_ON)) {
...
static int netdev_upper_ifup(FAR struct net_driver_s *dev)
{
	...
	for (i = 0; i < NETDEV_THREAD_COUNT; i++) {
		if (upper->tid[i] <= 0) {
			...
			upper->tid[i] = kthread_create(name, CONFIG_NETDEV_WORK_THREAD_PRIORITY,
										   CONFIG_DEFAULT_TASK_STACKSIZE, netdev_upper_loop, argv);
			if (upper->tid[i] < 0) {
				return upper->tid[i];
			}
		}
...
static int netdev_upper_addmac(FAR struct net_driver_s *dev, FAR const uint8_t *mac)
{
	FAR struct netdev_upperhalf_s *upper = dev->d_private;
...
#ifdef CONFIG_NET_MCASTGROUP
static int netdev_upper_rmmac(FAR struct net_driver_s *dev, FAR const uint8_t *mac)
{
	FAR struct netdev_upperhalf_s *upper = dev->d_private;
...
static int netdev_upper_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg)
{
	FAR struct netdev_upperhalf_s *upper = dev->d_private;
	FAR struct netdev_lowerhalf_s *lower = upper->lower;
...
void netpkt_free(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt, enum netpkt_type_e type)
{
	atomic_fetch_add(&dev->quota[type], 1);
	iob_free_chain(pkt);
}
...
int netpkt_copyin(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt, FAR const uint8_t *src,
				  unsigned int len, int offset)
{
	return iob_trycopyin(pkt, src, len, offset - NET_LL_HDRLEN(&dev->netdev), false);
}
...
int netpkt_copyout(FAR struct netdev_lowerhalf_s *dev, FAR uint8_t *dest, FAR const netpkt_t *pkt,
				   unsigned int len, int offset)
{
...
int netpkt_setdatalen(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt, unsigned int len)
{
...
unsigned int netpkt_getdatalen(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt)
{
...
int netpkt_to_iov(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt, FAR struct iovec *iov,
				  int iovcnt)
{
	int i;
