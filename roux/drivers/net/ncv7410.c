...
static int ncv_write_reg(FAR struct ncv7410_driver_s *priv, oa_regid_t regid, uint32_t word);

static int ncv_read_reg(FAR struct ncv7410_driver_s *priv, oa_regid_t regid, FAR uint32_t *word);

static int ncv_set_clear_bits(FAR struct ncv7410_driver_s *priv, oa_regid_t regid, uint32_t setbits,
							  uint32_t clearbits);

static int ncv_poll_footer(FAR struct ncv7410_driver_s *priv, FAR uint32_t *footer);

static int ncv_exchange_chunk(FAR struct ncv7410_driver_s *priv, FAR uint8_t *txbuf,
							  FAR uint8_t *rxbuf, uint32_t header, uint32_t *footer);
...
static uint32_t ncv_prepare_chunk_exchange(FAR struct ncv7410_driver_s *priv, FAR uint8_t *txbuf);
...
static void ncv_handle_rx_chunk(FAR struct ncv7410_driver_s *priv, uint32_t footer,
								FAR uint8_t *rxbuf);
...
static int ncv7410_transmit(FAR struct netdev_lowerhalf_s *dev, FAR netpkt_t *pkt);
...
static void ncv_finalize_rx_packet(FAR struct ncv7410_driver_s *priv)
{
	netpkt_setdatalen(&priv->dev, priv->rx_pkt, netpkt_getdatalen(&priv->dev, priv->rx_pkt) - 4);
...
static int ncv_write_reg(FAR struct ncv7410_driver_s *priv, oa_regid_t regid, uint32_t word)
{
	...
	uint32_t header = (1 << OA_WNR_POS) /* Write Not Read */
					  | (mms << OA_MMS_POS) | (addr << OA_ADDR_POS);
...
static int ncv_read_reg(FAR struct ncv7410_driver_s *priv, oa_regid_t regid, FAR uint32_t *word)
{
	...
	header = (mms << OA_MMS_POS) | (addr << OA_ADDR_POS);
							  ...
							  FAR uint8_t *rxbuf, uint32_t header, uint32_t *footer)
{
	...
	SPI_EXCHANGE(priv->spi, &txbuf[NCV_CHUNK_DEFAULT_PAYLOAD_SIZE - 4], (uint8_t *)footer, 4);
	ncv_deselect_spi(priv);
...
static int ncv_config(FAR struct ncv7410_driver_s *priv)
{
	...
	regval = (NCV_DIO_TXRX_FUNC << NCV_DIO0_FUNC_POS) |
			 (NCV_DIO_LINK_CTRL_FUNC << NCV_DIO1_FUNC_POS) | (1 << NCV_DIO0_OUT_VAL_POS) |
	...
	regval = (1 << NCV_MAC_CONTROL0_FCSA_POS) | (1 << NCV_MAC_CONTROL0_TXEN_POS) |
			 (1 << NCV_MAC_CONTROL0_RXEN_POS) | (1 << NCV_MAC_CONTROL0_ADRF_POS);
	...
	regval = (mac[2] << 24) | (mac[3] << 16) | (mac[4] << 8) | (mac[5]);
	...
	regval = (1 << OA_CONFIG0_SYNC_POS) | (1 << OA_CONFIG0_CSARFE_POS) |
			 (1 << OA_CONFIG0_ZARFE_POS) | (1 << OA_CONFIG0_RXCTE_POS) /* a bit lower latency */
			 | (3 << OA_CONFIG0_TXCTHRESH_POS) | (6 << OA_CONFIG0_CPS_POS);
 ...
 *
 * Description:
 *   ROUX callback: Bring up the Ethernet interface
 *
 * Input Parameters:
 ...
 *
 * Description:
 *   ROUX callback: Shut down the Ethernet interface.
 *
 * Input Parameters:
 *   dev - reference to the ROUX driver state structure
 *
 ...
 *
 * Description:
 *   ROUX callback: Transmit the given packet.
 *
 * Input Parameters:
 *   dev - reference to the ROUX driver state structure
 ...
 *
 * Description:
 *   ROUX callback: Claims an rx packet if available.
 *
 * Input Parameters:
 *   dev - reference to the ROUX driver state structure
 *
...
int ncv7410_initialize(FAR struct spi_dev_s *spi, int irq, struct ncv7410_config_s *config)
{
