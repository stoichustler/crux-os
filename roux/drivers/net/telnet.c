...
static inline void telnet_dumpbuffer(FAR const char *msg, FAR const char *buffer,
									 unsigned int nbytes);
#else
...
static ssize_t telnet_receive(FAR struct telnet_dev_s *priv, FAR const char *src, size_t srclen,
...
static int telnet_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
#ifdef HAVE_SIGNALS
static void telnet_check_ctrlchar(FAR struct telnet_dev_s *priv, FAR char *buffer, size_t len)
{
	int signo = 0;
...
static ssize_t telnet_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	...
	do {
		...
		if (priv->td_pending == 0) {
			nread = psock_recv(&priv->td_psock, priv->td_rxbuffer, CONFIG_TELNET_RXBUFFER_SIZE, 0);
			if (nread <= 0) {
				return nread;
			}
...
static int telnet_session(FAR struct telnet_session_s *session)
{
	...
	while (priv->td_minor < CONFIG_TELNET_MAXLCLIENTS) {
		if (g_telnet_clients[priv->td_minor] == NULL) {
			snprintf(session->ts_devpath, TELNET_DEVPATH_MAX, TELNET_DEVFMT, priv->td_minor);
			break;
		}
