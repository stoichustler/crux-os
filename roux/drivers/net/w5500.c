...
#define PHYCFGR_OPMDC_10BT_FD_NAN                                                                  \
	(3 << PHYCFGR_OPMDC_SHIFT) /* 100BT Full-duplex,                                               \
								* Auto-negotiation */
#define PHYCFGR_OPMDC_100BT_HD_AN                                                                  \
	(4 << PHYCFGR_OPMDC_SHIFT)								/* 100BT Half-duplex,                  \
															 * Auto-negotiation */
#define PHYCFGR_OPMDC_POWER_DOWN (6 << PHYCFGR_OPMDC_SHIFT) /* Power Down mode */
#define PHYCFGR_OPMDC_ALLCAP_AN                                                                    \
	(7 << PHYCFGR_OPMDC_SHIFT) /* All capable,                                                     \
								* Auto-negotiation */
...
#define SN_MR_BCASTB                                                                               \
	(1 << 6)				 /* Bit 6:  Broadcast Blocking in MACRAW and                           \
							  *         UDP mode */
...
#define SN_CR_OPEN                                                                                 \
	0x01 /* Socket n is initialized and opened according                                           \
		  * to the protocol selected in SN_MR */
#define SN_CR_LISTEN                                                                               \
	0x02					 /* Socket n operates as a 'TCP server' and waits                      \
...
static uint16_t g_pktbuf[CONFIG_NET_W5500_NINTERFACES][(PKTBUF_SIZE + 1) / 2];
...
static int w5500_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
...
static void w5500_read(FAR struct w5500_driver_s *self, uint8_t block_select_bits, uint16_t offset,
					   FAR void *buffer, uint16_t len)
{
	uint8_t addr_cntl[3];
...
static void w5500_write(FAR struct w5500_driver_s *self, uint8_t block_select_bits, off_t offset,
...
static uint8_t w5500_read8(FAR struct w5500_driver_s *self, uint8_t block_select_bits,
						   uint16_t offset)
{
	uint8_t value;

	w5500_read(self, block_select_bits, offset, &value, sizeof(value));
...
static void w5500_write8(FAR struct w5500_driver_s *self, uint8_t block_select_bits,
						 uint16_t offset, uint8_t value)
{
	w5500_write(self, block_select_bits, offset, &value, sizeof(value));
}
...
static uint16_t w5500_read16(FAR struct w5500_driver_s *self, uint8_t block_select_bits,
							 uint16_t offset)
{
	uint16_t value;
...
static void w5500_write16(FAR struct w5500_driver_s *self, uint8_t block_select_bits,
...
static int w5500_read16_atomic(FAR struct w5500_driver_s *self, uint8_t block_select_bits,
							   uint16_t offset, FAR uint16_t *value)
{
	int i;
...
static uint16_t w5500_txbuf_copy(FAR struct w5500_driver_s *self)
{
	...
	w5500_write(self, W5500_BSB_SOCKET_TX_BUFFER(0), offset, self->w_dev.d_buf, self->w_dev.d_len);
...
static bool w5500_txbuf_next(FAR struct w5500_driver_s *self)
{
	...
	offset = w5500_read16(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_TX_RD0);
	...
	w5500_write16(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_TX_WR0, offset);

	w5500_write8(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_CR, SN_CR_SEND);

	/* (Re-)start the TX timeout watchdog timer */

	wd_start(&self->w_txtimeout, W5500_TXTIMEOUT, w5500_txtimeout_expiry, (wdparm_t)self);
...
static int w5500_unfence(FAR struct w5500_driver_s *self)
{
	...
	w5500_write(self, W5500_BSB_COMMON_REGS, W5500_SHAR0, /* Source Hardware Address Register */
	...
	w5500_write8(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_MR, SM_MR_MACRAW | SN_MR_MFEN);

	/* Allocate all TX and RX buffer space to socket 0 ... */

	w5500_write8(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_RXBUF_SIZE, SN_RXBUF_16KB);

	w5500_write8(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_TXBUF_SIZE, SN_TXBUF_16KB);

	/* ... and none to sockets 1 to 7. */

	for (i = 1; i < 8; i++) {
		w5500_write8(self, W5500_BSB_SOCKET_REGS(i), W5500_SN_RXBUF_SIZE, SN_RXBUF_0KB);

		w5500_write8(self, W5500_BSB_SOCKET_REGS(i), W5500_SN_TXBUF_SIZE, SN_TXBUF_0KB);
	}
	...
	w5500_write8(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_IMR, SN_INT_RECV);

	/* Enable interrupts on socket 0 */

	w5500_write8(self, W5500_BSB_COMMON_REGS, W5500_SIMR, /* Socket Interrupt Mask Register */
				 SIMR(0));

	/* Open socket 0 */

	w5500_write8(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_CR, /* Control Register */
				 SN_CR_OPEN);

	/* Check whether socket 0 is open in MACRAW mode. */

	value = w5500_read8(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_SR);
	...
	value = w5500_read8(self, W5500_BSB_COMMON_REGS, W5500_PHYCFGR);
...
static void w5500_transmit(FAR struct w5500_driver_s *self)
{
	...
	if (w5500_txbuf_numpending(self) == 1) {
		...
		w5500_write8(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_IMR, SN_INT_RECV | SN_INT_SEND_OK);
...
static int w5500_txpoll(FAR struct net_driver_s *dev)
{
	FAR struct w5500_driver_s *self = (FAR struct w5500_driver_s *)dev->d_private;
...
static void w5500_receive(FAR struct w5500_driver_s *self)
{
	do {
		...
		ret = w5500_read16_atomic(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_RX_RSR0, &s0_rx_rsr);
		if (ret != OK) {
			goto error;
		}
		...
		s0_rx_rd = w5500_read16(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_RX_RD0);

		/* Read 16-bit length field. */

		pktlen = w5500_read16(self, W5500_BSB_SOCKET_RX_BUFFER(0), s0_rx_rd);
		...
		if (self->w_dev.d_len <= CONFIG_NET_ETH_PKTSIZE) {
			w5500_read(self, W5500_BSB_SOCKET_RX_BUFFER(0), s0_rx_rd + sizeof(pktlen),
					   self->w_dev.d_buf, self->w_dev.d_len);
		}

		/* Acknowledge data reception to W5500 */

		w5500_write16(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_RX_RD0, s0_rx_rd + pktlen);

		w5500_write8(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_CR, SN_CR_RECV);

		/* Check for errors and update statistics */

		if (self->w_dev.d_len > CONFIG_NET_ETH_PKTSIZE || self->w_dev.d_len < ETH_HDRLEN) {
...
#ifdef CONFIG_DEBUG_NET_INFO
		ninfodumpbuffer("Received Packet:", self->w_dev.d_buf, self->w_dev.d_len);
#endif
...
static void w5500_interrupt_work(FAR void *arg)
{
	...
	w5500_read(self, W5500_BSB_COMMON_REGS, W5500_IR, ir, sizeof(ir));
	...
	if (ir[0] != 0) {
		...
		w5500_write8(self, W5500_BSB_COMMON_REGS, W5500_IR, ir[0]);
	}
	...
	ir[0] = w5500_read8(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_IR);
	...
	w5500_write8(self, W5500_BSB_SOCKET_REGS(0), W5500_SN_IR, ir[0]);
...
static int w5500_ifup(FAR struct net_driver_s *dev)
{
	...
	ninfo("Bringing up: %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", dev->d_ipv6addr[0],
		  dev->d_ipv6addr[1], dev->d_ipv6addr[2], dev->d_ipv6addr[3], dev->d_ipv6addr[4],
		  dev->d_ipv6addr[5], dev->d_ipv6addr[6], dev->d_ipv6addr[7]);
#endif
 ...
 *
 * Input Parameters:
 *   arg - Reference to the ROUX driver state structure (cast to void*)
 *
 * Returned Value:
 *   None
 *
...
static int w5500_txavail(FAR struct net_driver_s *dev)
{
	FAR struct w5500_driver_s *priv = (FAR struct w5500_driver_s *)dev->d_private;
...
int w5500_initialize(FAR struct spi_dev_s *spi_dev, FAR const struct w5500_lower_s *lower,
					 unsigned int devno)
{
