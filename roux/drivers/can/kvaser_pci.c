...
#define KVASER_INT_ERR                                                                             \
	(SJA1000_OVERRUN_INT_ENA | SJA1000_ERR_PASSIVE_INT_ENA | SJA1000_ARB_LOST_INT_ENA |            \
...
static uint8_t kvaser_getreg_sja(FAR struct kvaser_sja_s *priv, unsigned int offset);
static void kvaser_putreg_sja(FAR struct kvaser_sja_s *priv, unsigned int offset, uint8_t value);

static uint32_t kvaser_getreg_s5920(FAR struct kvaser_driver_s *priv, unsigned int offset);
static void kvaser_putreg_s5920(FAR struct kvaser_driver_s *priv, unsigned int offset,
								uint32_t value);
...
static int kvaser_chrdev_ioctl(FAR struct can_dev_s *dev, int cmd, unsigned long arg);
static int kvaser_chrdev_remoterequest(FAR struct can_dev_s *dev, uint16_t id);
static int kvaser_chrdev_send(FAR struct can_dev_s *dev, FAR struct can_msg_s *msg);
...
static int kvaser_sock_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
...
static const struct pci_device_id_s g_kvaser_id_table[] = {
	{PCI_DEVICE(0x10e8, 0x8406), .driver_data = 0}, {}};
...
static int kvaser_chrdev_send(FAR struct can_dev_s *dev, FAR struct can_msg_s *msg)
{
	...
	if (bytes > 8 || msg->cm_hdr.ch_edl || msg->cm_hdr.ch_brs || msg->cm_hdr.ch_esi) {
...
static void kvaser_chardev_error(FAR struct kvaser_sja_s *priv, uint8_t st)
{
	...
	if (st & SJA1000_ARB_LOST_INT_ST) {
		...
		regval = (kvaser_getreg_sja(priv, SJA1000_ARB_LOST_CAP_REG) & SJA1000_ARB_LOST_CAP_M);
...
static void kvaser_sock_receive(FAR struct kvaser_sja_s *priv)
{
	...
	while (kvaser_getreg_sja(priv, SJA1000_STATUS_REG) & SJA1000_RX_BUF_ST) {
		...
		if (!(regval & SJA1000_FI_FF)) {
			frame->can_id = ((kvaser_getreg_sja(priv, SJA1000_DATA_1_REG) << 3) |
							 (kvaser_getreg_sja(priv, SJA1000_DATA_2_REG) >> 5));

			dreg = SJA1000_DATA_3_REG;
		} else {
#ifdef CONFIG_NET_CAN_EXTID
			frame->can_id = ((kvaser_getreg_sja(priv, SJA1000_DATA_1_REG) << 21) |
							 (kvaser_getreg_sja(priv, SJA1000_DATA_2_REG) << 13) |
							 (kvaser_getreg_sja(priv, SJA1000_DATA_3_REG) << 5) |
							 (kvaser_getreg_sja(priv, SJA1000_DATA_4_REG) >> 3));
...
static int kvaser_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	if (kvaser_getreg_s5920(priv, KVASER_S5920_INTCSR) & KVASER_S5920_INTCSR_INT_ASSERT) {
#ifdef CONFIG_CAN_KVASER_CHARDEV
		...
		work_queue(KVASER_CANWORK, &priv->irqwork, kvaser_sock_interrupt_work, priv, 0);
#endif
	}
