...
static inline ssize_t can_rtrread(FAR struct file *filep, FAR struct canioc_rtr_s *rtr);
	...
	if (dev->cd_crefs >= 255) {
		...
		if (ret == OK) {
			...
			if ((filep->f_oflags & O_RDOK) != 0) {
				list_add_head(&dev->cd_readers, (FAR struct list_node *)init_can_reader(filep));
			}
		}
...
static inline ssize_t can_rtrread(FAR struct file *filep, FAR struct canioc_rtr_s *request)
{
	...
	if (wait) {
		...
		if (dev->cd_ops->co_remoterequest != NULL) {
			if (request->ci_msg->cm_hdr.ch_id < CAN_MAX_STDMSGID
				...
				ret = dev_remoterequest(dev, (uint16_t)(request->ci_msg->cm_hdr.ch_id));
			} else {
				ret = -EINVAL;
			}
		} else {
			...
			ret = can_write(filep, (FAR const char *)request->ci_msg,
		...
		if (ret >= 0) {
			...
			ret = nxsem_tickwait(&wait->cr_sem, SEC2TICK(request->ci_timeout.tv_sec) +
...
static int can_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case CANIOC_RTR: {
		ret = can_rtrread(filep, (FAR struct canioc_rtr_s *)((uintptr_t)arg));
	} break;
	...
	case FIONWRITE: {
		*(FAR int *)arg = PENDING_COUNT(&dev->cd_sender) - SENDING_COUNT(&dev->cd_sender);
	} break;
	...
	case CANIOC_SET_TRANSVSTATE: {
		...
		if (dev->cd_transv && dev->cd_transv->ct_ops && dev->cd_transv->ct_ops->ct_setstate) {
			FAR const struct can_transv_ops_s *ct_ops = dev->cd_transv->ct_ops;
	...
	case CANIOC_GET_TRANSVSTATE: {
		...
		if (dev->cd_transv && dev->cd_transv->ct_ops && dev->cd_transv->ct_ops->ct_getstate) {
