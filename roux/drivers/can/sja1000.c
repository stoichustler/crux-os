...
static void sja1000_set_acc_filter(struct sja1000_dev_s *priv, uint32_t code, uint32_t mask,
								   bool single_filter);

/* SJA1000 bit-timing initialization */

static int sja1000_baud_rate(struct sja1000_dev_s *priv, int rate, int clock, int sjw, int sampl_pt,
							 int flags);
...
static void sja1000_reset(struct can_dev_s *dev)
{
	...
	sja1000_putreg(priv, SJA1000_MODE_REG, mode_value); /* Enter Reset Mode */

	sja1000_modifyreg32(priv, SJA1000_CLOCK_DIVIDER_REG, 0, SJA1000_EXT_MODE_M);

	sja1000_putreg(priv, SJA1000_INT_ENA_REG, 0); /* Disable interrupts */
	sja1000_getreg(priv, SJA1000_STATUS_REG);	  /* Clear status bits */

	sja1000_putreg(priv, SJA1000_TX_ERR_CNT_REG, SJA1000_INIT_TEC);		   /* TEC */
	sja1000_putreg(priv, SJA1000_RX_ERR_CNT_REG, SJA1000_INIT_REC);		   /* REC */
	sja1000_putreg(priv, SJA1000_ERR_WARNING_LIMIT_REG, SJA1000_INIT_EWL); /* EWL */

	sja1000_set_acc_filter(priv, SJA1000_ACCEPTANCE_CODE, SJA1000_ACCEPTANCE_MASK, true);

	/* Set bit timing */

	ret =
		sja1000_baud_rate(priv, config->bitrate, config->clk_freq, config->sjw, config->samplep, 0);
	...
	sja1000_putreg(priv, SJA1000_CMD_REG, SJA1000_ABORT_TX_M | SJA1000_CLR_OVERRUN_M);
...
static int sja1000_setup(struct can_dev_s *dev)
{
	...
	ret = config->attach(config, (sja1000_handler_t)sja1000_interrupt, (FAR void *)dev);
	if (ret < 0) {
...
static int sja1000_ioctl(struct can_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case CANIOC_GET_BITTIMING: {
		...
		bt->bt_sjw = ((timing0 & SJA1000_SYNC_JUMP_WIDTH_M) >> SJA1000_SYNC_JUMP_WIDTH_S) + 1;

		bt->bt_tseg1 = ((timing1 & SJA1000_TIME_SEG1_M) >> SJA1000_TIME_SEG1_S) + 1;
		bt->bt_tseg2 = ((timing1 & SJA1000_TIME_SEG2_M) >> SJA1000_TIME_SEG2_S) + 1;
		bt->bt_baud = config->clk_freq / (brp * (bt->bt_tseg1 + bt->bt_tseg2 + 1));

		cantrace("Retrieved baud rate. TS1: %" PRId8 " TS2: %" PRId8 " BRP: %" PRId32 "\n",
...
static int sja1000_send(struct can_dev_s *dev, struct can_msg_s *msg)
{
	...
	if (msg->cm_hdr.ch_extid) {
		...
		for (i = 0; i < len; i++) {
			sja1000_putreg(priv, (SJA1000_DATA_5_REG + i), msg->cm_data[i]);
		}
	} else
#endif
	{
		...
		sja1000_putreg(priv, SJA1000_DATA_2_REG, id & 0xff);
		for (i = 0; i < len; i++) {
			sja1000_putreg(priv, (SJA1000_DATA_3_REG + i), msg->cm_data[i]);
		}
	}
	...
	if (config->loopback) {
		sja1000_putreg(priv, SJA1000_CMD_REG, SJA1000_SELF_RX_REQ_M | SJA1000_ABORT_TX_M);
	} else {
...
static bool sja1000_txready(struct can_dev_s *dev)
{
	...
	caninfo("SJA1000 (%" PRIu8 ") txready: %d\n", port, ((regval & SJA1000_TX_BUF_ST_M) != 0));
								   ...
								   bool single_filter)
{
	...
	for (int i = 0; i < 4; i++) {
		sja1000_putreg(priv, (SJA1000_DATA_0_REG + i), ((code >> ((3 - i) * 8)) & 0xff));
		sja1000_putreg(priv, (SJA1000_DATA_4_REG + i), ((mask >> ((3 - i) * 8)) & 0xff));
	}
}
							 ...
							 int flags)
{
	...
	for (tseg = (0 + 0 + 2) * 2; tseg <= (timing->tseg2_max + timing->tseg1_max + 2) * 2 + 1;
		 tseg++) {
	...
	timing0 |= ((sjw - 1) << SJA1000_SYNC_JUMP_WIDTH_S) & SJA1000_SYNC_JUMP_WIDTH_M;
	...
	if (config->triple_sample) {
		...
		timing1 |= (config->triple_sample << SJA1000_TIME_SAMP_S) & SJA1000_TIME_SAMP_M;
	}
