...
#define CTUCANFD_INT_ERR (CTUCANFD_INT_DOI | CTUCANFD_INT_ALI | CTUCANFD_INT_BEI)
...
static uint32_t ctucanfd_getreg(FAR struct ctucanfd_can_s *priv, unsigned int offset);
static void ctucanfd_putreg(FAR struct ctucanfd_can_s *priv, unsigned int offset, uint32_t value);
...
static int ctucanfd_chrdev_ioctl(FAR struct can_dev_s *dev, int cmd, unsigned long arg);
static int ctucanfd_chrdev_remoterequest(FAR struct can_dev_s *dev, uint16_t id);
static int ctucanfd_chrdev_send(FAR struct can_dev_s *dev, struct can_msg_s *msg);
...
static int ctucanfd_sock_ioctl(FAR struct net_driver_s *dev, int cmd, unsigned long arg);
#endif
...
static const struct pci_device_id_s g_ctucanfd_id_table[] = {
	{PCI_DEVICE(0x1760, 0xff00), .driver_data = 0}, {}};
...
static int ctucanfd_chrdev_send(FAR struct can_dev_s *dev, FAR struct can_msg_s *msg)
{
	FAR struct ctucanfd_can_s *priv = (FAR struct ctucanfd_can_s *)dev;
	...
	for (i = 0; i < bytes; i++) {
		ctucanfd_putreg(priv, offset + CTUCANFD_TXBUF_DATA + i, msg->cm_data[i]);
	}
...
static void ctucanfd_chardev_error(FAR struct ctucanfd_can_s *priv, uint32_t stat)
{
	struct can_hdr_s hdr;
	uint16_t errbits = 0;
	...
	if (stat & CTUCANFD_INT_DOI) {
		...
		ctucanfd_putreg(priv, CTUCANFD_CMD, CTUCANFD_CMD_RRB | CTUCANFD_CMD_CDO);
	}
	...
	for (i = 0; i < priv->count; i++) {
		...
		if (stat & CTUCANFD_INT_TXI) {
			...
			for (txidx = 0; txidx < priv->devs[i].txbufcnt; txidx++) {
				if (CTUCANFD_TXSTAT_GET(regval, txidx) == CTUCANFD_TXSTAT_TOK) {
					can_txdone(&priv->devs[i].dev);

					/* Mark buffer as empty */

					regval = (CTUCANFD_TXCMD_TXCE + (1 << (CTUCANFD_TXCMD_TXB_SHIFT + txidx)));
...
static int ctucanfd_sock_ifup(FAR struct net_driver_s *dev)
{
	FAR struct ctucanfd_can_s *priv = (FAR struct ctucanfd_can_s *)dev->d_private;
...
static int ctucanfd_sock_send(FAR struct ctucanfd_can_s *priv)
{
	...
	if (priv->dev.d_len == sizeof(struct can_frame)) {
		FAR struct can_frame *frame = frame = (FAR struct can_frame *)priv->dev.d_buf;
		...
		for (i = 0; i < frame->can_dlc; i++) {
			ctucanfd_putreg(priv, offset + CTUCANFD_TXBUF_DATA + i, frame->data[i]);
		}
	}
#ifdef CONFIG_NET_CAN_CANFD
	else /* CAN FD frame */
	{
		FAR struct canfd_frame *frame = (FAR struct canfd_frame *)priv->dev.d_buf;
...
static void ctucanfd_sock_error(FAR struct ctucanfd_can_s *priv, uint32_t stat)
{
...
static int ctucanfd_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	work_queue(CTUCANFD_CANWORK, &priv->irqwork, ctucanfd_sock_interrupt_work, priv, 0);
#endif
