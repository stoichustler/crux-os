...
#define MCP2515_BRP                                                                                \
	((uint8_t)(((float)(MCP2515_CANCLK_FREQUENCY) / ((float)(MCP2515_TSEG1 + MCP2515_TSEG2 + 1) *  \
													 (float)(2 * CONFIG_MCP2515_BITRATE))) -       \
			   1))
...
#define MCP2515_STDFILTER_BYTES MCP2515_ALIGN_UP(CONFIG_MCP2515_NSTDFILTERS << 2)
#define MCP2515_STDFILTER_WORDS (MCP2515_STDFILTER_BYTES >> 2)

#define MCP2515_EXTFILTER_BYTES MCP2515_ALIGN_UP(CONFIG_MCP2515_NEXTFILTERS << 3)
...
static void mcp2515_readregs(FAR struct mcp2515_can_s *priv, uint8_t regaddr, FAR uint8_t *buffer,
							 uint8_t len);
static void mcp2515_writeregs(FAR struct mcp2515_can_s *priv, uint8_t regaddr,
							  FAR const uint8_t *buffer, uint8_t len);
static void mcp2515_modifyreg(FAR struct mcp2515_can_s *priv, uint8_t regaddr, uint8_t mask,
							  uint8_t value);
#ifdef CONFIG_MCP2515_REGDEBUG
...
#ifdef CONFIG_CAN_ERRORS
static void mcp2515_error(FAR struct can_dev_s *dev, uint8_t status, uint8_t oldstatus);
#endif
...
static void mcp2515_read_2regs(FAR struct mcp2515_can_s *priv, uint8_t reg, FAR uint8_t *v1,
							   FAR uint8_t *v2)
{
...
{
	FAR struct mcp2515_config_s *config = priv->config;
	uint8_t wr[4] = {MCP2515_BITMOD, regaddr, mask, value};
		...
		if ((priv->filters & (1 << ndx)) == 0) {
          ...
          if (extconfig->sf_prio == CAN_MSGPRIO_LOW)
			...
			if (mode == CAN_FILTER_DUAL) {
				...
				mcp2515_writeregs(priv, MCP2515_RXF0EID0 + offset + ((priv->nalloc - 1) * 4),
								  &regval, 1);

				/* EID8 - EID15 */

				regval = (uint8_t)((extconfig->xf_id1 & 0xff00) >> 8);
				mcp2515_writeregs(priv, MCP2515_RXF0EID8 + offset + ((priv->nalloc - 1) * 4),
								  &regval, 1);
				...
				mcp2515_writeregs(priv, MCP2515_RXF0SIDL + offset + ((priv->nalloc - 1) * 4),
								  &regval, 1);
		...
		if ((priv->filters & (1 << ndx)) == 0) {
          ...
          if (stdconfig->sf_prio == CAN_MSGPRIO_LOW)
			...
			if (mode == CAN_FILTER_DUAL) {
				...
				mcp2515_writeregs(priv, MCP2515_RXF0SIDH + offset + ((priv->nalloc - 1) * 4),
								  &regval, 1);
			...
			regval = 0;
			mcp2515_writeregs(priv, MCP2515_RXF0EID0 + offset + ((priv->nalloc - 1) * 4), &regval,
							  1);
			mcp2515_writeregs(priv, MCP2515_RXF0EID8 + offset + ((priv->nalloc - 1) * 4), &regval,
							  1);
			mcp2515_writeregs(priv, MCP2515_RXM0EID0 + offset, &regval, 1);
			mcp2515_writeregs(priv, MCP2515_RXM0EID8 + offset, &regval, 1);
...
static void mcp2515_rxint(FAR struct can_dev_s *dev, bool enable)
{
	...
	if (enable) {
		mcp2515_modifyreg(priv, MCP2515_CANINTE, priv->rxints | MCP2515_ERROR_INTS,
...
static void mcp2515_txint(FAR struct can_dev_s *dev, bool enable)
{
	...
	if (enable) {
		mcp2515_modifyreg(priv, MCP2515_CANINTE, priv->txints | MCP2515_ERROR_INTS,
...
static int mcp2515_ioctl(FAR struct can_dev_s *dev, int cmd, unsigned long arg)
{
	...
	case CANIOC_GET_BITTIMING: {
		...
		bt->bt_baud = MCP2515_CANCLK_FREQUENCY / brp / (bt->bt_tseg1 + bt->bt_tseg2 + 1);
		ret = OK;
	} break;
	...
	case CANIOC_SET_BITTIMING: {
		FAR const struct canioc_bittiming_s *bt = (FAR const struct canioc_bittiming_s *)arg;
		irqstate_t flags;
		uint8_t brp;
		uint8_t sjw;
		uint8_t tseg1;
		...
		brp = (uint32_t)(((float)MCP2515_CANCLK_FREQUENCY /
						  ((float)(tseg1 + tseg2 + 1) * (float)(2 * bt->bt_baud))) -
						 1);
...
static int mcp2515_send(FAR struct can_dev_s *dev, FAR struct can_msg_s *msg)
{
	...
	caninfo("CAN%d\n", config->devid);
	caninfo("CAN%d ID: %" PRId32 " DLC: %d\n", config->devid, (uint32_t)msg->cm_hdr.ch_id,
			msg->cm_hdr.ch_dlc);
	UNUSED(config);
...
static void mcp2515_receive(FAR struct can_dev_s *dev, uint8_t offset)
{
	...
	mcp2515_readregs(priv, MCP2515_RXB0CTRL + offset, priv->spi_rxbuf, SPI_TRANSFER_BUF_LEN);
