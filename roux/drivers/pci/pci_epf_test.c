...
static int pci_epf_test_copy(FAR struct pci_epf_test_s *test)
{
	...
	ret = pci_epc_map_addr(epc, epf->funcno, src_addr, reg->src_addr, reg->size);
	if (ret < 0) {
	...
	ret = pci_epc_map_addr(epc, epf->funcno, dst_addr, reg->dst_addr, reg->size);
	if (ret < 0) {
...
static void pci_epf_test_raise_irq(FAR struct pci_epf_test_s *test, uint8_t irq_type, uint16_t irq)
{
	FAR struct pci_epf_device_s *epf = test->epf;
	FAR struct pci_epc_ctrl_s *epc = epf->epc;
	FAR struct pci_epf_test_reg_s *reg;
...
static void pci_epf_test_cmd_handler(FAR void *arg)
{
	...
	if (command & PCI_EPF_TEST_COMMAND_RAISE_LEGACY_IRQ) {
		...
		reg->status = PCI_EPF_TEST_STATUS_IRQ_RAISED;
		pci_epc_raise_irq(epc, epf->funcno, PCI_EPC_IRQ_MSI, reg->irq_number);
		...
		reg->status = PCI_EPF_TEST_STATUS_IRQ_RAISED;
		pci_epc_raise_irq(epc, epf->funcno, PCI_EPC_IRQ_MSIX, reg->irq_number);
	}

reset_handler:
	work_queue(HPWORK, &test->work, pci_epf_test_cmd_handler, test, PCI_EPF_TEST_WORK_PERIOD);
}
...
static int pci_epf_test_core_init(FAR struct pci_epf_device_s *epf)
{
	...
	if (msi_capable) {
		ret = pci_epc_set_msi(epc, epf->funcno, epf->msi_interrupts);
		if (ret < 0) {
	...
	if (msix_capable) {
		ret = pci_epc_set_msix(epc, epf->funcno, epf->msix_interrupts, test->test_reg_bar,
...
static int pci_epf_test_link_up(FAR struct pci_epf_device_s *epf)
{
	...
	return work_queue(HPWORK, &test->work, pci_epf_test_cmd_handler, test,
					  PCI_EPF_TEST_WORK_PERIOD);
}

static int pci_epf_test_alloc_space(FAR struct pci_epf_device_s *epf)
{
	FAR const struct pci_epc_features_s *features;
	FAR struct pci_epf_test_s *test = epf->priv;
	FAR struct pci_epc_ctrl_s *epc = epf->epc;
	...
	for (bar = 0; bar < PCI_STD_NUM_BARS; bar += add) {
		...
		test->reg[bar] = pci_epf_alloc_space(epf, bar, test->bar_size[bar], features->align);
