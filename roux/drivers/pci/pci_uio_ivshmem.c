...
#define dev_to_udev(dev) ((FAR struct uio_ivshmem_dev_s *)ivshmem_get_driver(dev))
...
typedef CODE void (*uio_ivshmem_notify_t)(FAR struct uio_ivshmem_notify_s *notify,
										  int32_t newevent);
...
static ssize_t uio_ivshmem_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t uio_ivshmem_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int uio_ivshmem_unmap(FAR struct task_group_s *group, FAR struct mm_map_entry_s *entry,
							 FAR void *start, size_t length);
static int uio_ivshmem_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map);
static int uio_ivshmem_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static inline void uio_ivshmem_add_notify(FAR struct uio_ivshmem_dev_s *dev,
...
static inline void uio_ivshmem_remove_notify(FAR struct uio_ivshmem_dev_s *dev,
											 FAR struct uio_ivshmem_notify_s *notify)
{
	irqstate_t flags;
...
static void uio_ivshmem_notify_read(FAR struct uio_ivshmem_notify_s *notify, int32_t newevent)
{
	FAR struct uio_ivshmem_read_s *read = (FAR struct uio_ivshmem_read_s *)notify;
...
static int uio_ivshmem_mmap(FAR struct file *filep, FAR struct mm_map_entry_s *map)
{
	...
	if (map->offset < 0 || map->offset >= shmem_size || map->length == 0 ||
		map->offset + map->length > shmem_size) {
		return -EINVAL;
	}
...
void uio_ivshmem_notify_poll(FAR struct uio_ivshmem_notify_s *notify, int32_t newevent)
{
	FAR struct uio_ivshmem_poll_s *poll = (FAR struct uio_ivshmem_poll_s *)notify;
...
static int uio_ivshmem_interrupt(int irq, FAR void *context, FAR void *arg)
{
	...
	flags = spin_lock_irqsave(&dev->lock);
	dev->event_count++;
	list_for_every_entry(&dev->notify_list, notify, struct uio_ivshmem_notify_s, node)
	{
