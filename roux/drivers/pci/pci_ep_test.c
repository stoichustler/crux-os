...
#include <roux/pci/pci_ep_test.h>
...
static int pci_ep_test_ioctl(FAR struct file *filep, int cmd, unsigned long arge);
...
static inline uint32_t pci_ep_test_read_dword(FAR struct pci_ep_test_s *test, uint32_t offset)
{
	uint32_t value;
...
static inline void pci_ep_test_write_dword(FAR struct pci_ep_test_s *test, uint32_t offset,
										   uint32_t value)
{
	pci_write_mmio_dword(test->pdev, test->base + offset, value);
}

static inline uint32_t pci_ep_test_bar_read_dword(FAR struct pci_ep_test_s *test, int bar,
												  uint32_t offset)
{
	uint32_t value;
...
static inline void pci_ep_test_bar_write_dword(FAR struct pci_ep_test_s *test, int bar,
...
static bool pci_ep_test_bar(FAR struct pci_ep_test_s *test, int barno)
{
...
static bool pci_ep_test_legacy_irq(FAR struct pci_ep_test_s *test)
{
	pci_ep_test_write_dword(test, PCI_EP_TEST_IRQ_TYPE, PCI_EP_TEST_IRQ_TYPE_LEGACY);
	pci_ep_test_write_dword(test, PCI_EP_TEST_IRQ_NUMBER, 0);
	pci_ep_test_write_dword(test, PCI_EP_TEST_COMMAND, PCI_EP_TEST_COMMAND_LEGACY_IRQ);
...
static bool pci_ep_test_msi_irq(FAR struct pci_ep_test_s *test, uint16_t msi_num, bool msix)
{
	pci_ep_test_write_dword(test, PCI_EP_TEST_IRQ_TYPE,
							msix ? PCI_EP_TEST_IRQ_TYPE_MSIX : PCI_EP_TEST_IRQ_TYPE_MSI);
	pci_ep_test_write_dword(test, PCI_EP_TEST_IRQ_NUMBER, msi_num);
	pci_ep_test_write_dword(test, PCI_EP_TEST_COMMAND,
							msix ? PCI_EP_TEST_COMMAND_MSIX_IRQ : PCI_EP_TEST_COMMAND_MSI_IRQ);
	nxsem_wait(&test->irq_raise);
...
static bool pci_ep_test_write(FAR struct pci_ep_test_s *test, unsigned long arg)
{
	...
	pci_ep_test_write_dword(test, PCI_EP_TEST_LOWER_SRC_ADDR, phys_addr);
	pci_ep_test_write_dword(test, PCI_EP_TEST_UPPER_SRC_ADDR, phys_addr >> 32);
	...
	pci_ep_test_write_dword(test, PCI_EP_TEST_COMMAND, PCI_EP_TEST_COMMAND_READ);

	nxsem_wait(&test->irq_raise);
	kmm_free(addr);

	return !!(pci_ep_test_read_dword(test, PCI_EP_TEST_STATUS) & PCI_EP_TEST_STATUS_READ_SUCCESS);
}
...
static bool pci_ep_test_read(FAR struct pci_ep_test_s *test, unsigned long arg)
{
	FAR struct pci_ep_test_param_s *param;
	FAR void *addr;
	uint64_t phys_addr;
	...
	pci_ep_test_write_dword(test, PCI_EP_TEST_LOWER_DST_ADDR, phys_addr);
	pci_ep_test_write_dword(test, PCI_EP_TEST_UPPER_DST_ADDR, phys_addr >> 32);
	...
	pci_ep_test_write_dword(test, PCI_EP_TEST_IRQ_TYPE, test->irq_type);
	pci_ep_test_write_dword(test, PCI_EP_TEST_IRQ_NUMBER, 1);
	pci_ep_test_write_dword(test, PCI_EP_TEST_COMMAND, PCI_EP_TEST_COMMAND_WRITE);
...
static bool pci_ep_test_copy(struct pci_ep_test_s *test, unsigned long arg)
{
	FAR struct pci_ep_test_param_s *param;
	FAR void *src_addr;
	FAR void *dst_addr;
	...
	pci_ep_test_write_dword(test, PCI_EP_TEST_LOWER_SRC_ADDR, src_phys_addr);
	pci_ep_test_write_dword(test, PCI_EP_TEST_UPPER_SRC_ADDR, src_phys_addr >> 32);
	...
	pci_ep_test_write_dword(test, PCI_EP_TEST_LOWER_DST_ADDR, dst_phys_addr);
	pci_ep_test_write_dword(test, PCI_EP_TEST_UPPER_DST_ADDR, dst_phys_addr >> 32);
	...
	pci_ep_test_write_dword(test, PCI_EP_TEST_IRQ_TYPE, test->irq_type);
	pci_ep_test_write_dword(test, PCI_EP_TEST_IRQ_NUMBER, 1);
	pci_ep_test_write_dword(test, PCI_EP_TEST_COMMAND, PCI_EP_TEST_COMMAND_COPY);
...
static int pci_ep_test_handler(int irq, FAR void *context, FAR void *args)
{
	FAR struct pci_ep_test_s *test = (FAR struct pci_ep_test_s *)args;
	uint32_t reg;
...
static int pci_ep_test_alloc_irq(FAR struct pci_ep_test_s *test, int irq_type)
{
	FAR struct pci_device_s *pdev = test->pdev;
	int ret = -EINVAL;
...
static bool pci_ep_test_set_irq(struct pci_ep_test_s *test, int req_irq_type)
{
	int ret;
...
static int pci_ep_test_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	FAR struct pci_ep_test_s *test;
	int ret = -EINVAL;
	int bar;
...
static int pci_ep_test_probe(FAR struct pci_device_s *dev)
{
	...
	snprintf(test->name, sizeof(test->name), PCI_EP_TEST_DEVICE_NAME ".%d", g_pci_ep_idr++);
