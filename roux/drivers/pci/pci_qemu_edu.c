...
static uint32_t pci_qemu_edu_read_reg32(FAR struct pci_qemu_edu_priv_s *priv, int reg);
static void pci_qemu_edu_write_reg32(FAR struct pci_qemu_edu_priv_s *priv, int reg, uint32_t val);
static void pci_qemu_edu_write_reg64(FAR struct pci_qemu_edu_priv_s *priv, int reg, uint64_t val);
...
static void pci_qemu_edu_test_poll(FAR struct pci_qemu_edu_priv_s *priv)
{
	...
	pciinfo("Live Check: Wrote: 0x%08" PRIx32 " Read: 0x%08" PRIx32 " Error Bits 0x%08" PRIx32 "\n",
...
static void pci_qemu_edu_test_dma(FAR struct pci_qemu_edu_priv_s *priv)
{
	...
	pci_qemu_edu_write_reg64(priv, PCI_QEMU_EDU_REG_DMA_SOURCE, (uint64_t)test_block);
	pci_qemu_edu_write_reg64(priv, PCI_QEMU_EDU_REG_DMA_DEST, dev_addr);
	pci_qemu_edu_write_reg64(priv, PCI_QEMU_EDU_REG_DMA_COUNT, (uint64_t)block_size);
	...
	pci_qemu_edu_write_reg64(priv, PCI_QEMU_EDU_REG_DMA_DEST, (uint64_t)test_block);
	pci_qemu_edu_write_reg64(priv, PCI_QEMU_EDU_REG_DMA_SOURCE, dev_addr);
	pci_qemu_edu_write_reg64(priv, PCI_QEMU_EDU_REG_DMA_COUNT, (uint64_t)block_size);
	pci_qemu_edu_write_reg32(priv, PCI_QEMU_EDU_REG_STATUS, 0x00);
	pci_qemu_edu_write_reg64(priv, PCI_QEMU_EDU_REG_DMA_CMD, 0x01 | 0x02 | 0x04);
	nxsem_wait(&priv->isr_done);
...
static int pci_qemu_edu_interrupt(int irq, FAR void *context, FAR void *arg)
{
	FAR struct pci_qemu_edu_priv_s *priv = (FAR struct pci_qemu_edu_priv_s *)arg;
	uint32_t status;
	...
	case 0x1: {
		priv->test_result = pci_qemu_edu_read_reg32(priv, PCI_QEMU_EDU_REG_FAC);
