...
static struct list_node g_pci_epc_device_list = LIST_INITIAL_VALUE(g_pci_epc_device_list);
...
FAR struct pci_epc_ctrl_s *pci_get_epc(FAR const char *epc_name)
{
	...
	list_for_every_entry(&g_pci_epc_device_list, epc, struct pci_epc_ctrl_s, node)
	{
...
int pci_epc_get_next_free_bar(FAR const struct pci_epc_features_s *epc_features, int barno)
{
...
FAR const struct pci_epc_features_s *pci_epc_get_features(FAR struct pci_epc_ctrl_s *epc,
														  uint8_t funcno)
{
	FAR const struct pci_epc_features_s *epc_features;

	if (epc == NULL || epc->ops->get_features == NULL || funcno >= epc->max_functions) {
		return NULL;
	}
...
int pci_epc_raise_irq(FAR struct pci_epc_ctrl_s *epc, uint8_t funcno, enum pci_epc_irq_type_e type,
					  uint16_t interrupt_num)
{
	int ret;

	if (epc == NULL || epc->ops->raise_irq == NULL || funcno >= epc->max_functions) {
		return -EINVAL;
	}
...
int pci_epc_map_msi_irq(FAR struct pci_epc_ctrl_s *epc, uint8_t funcno, uintptr_t phys_addr,
						uint8_t interrupt_num, uint32_t entry_size, FAR uint32_t *msi_data,
						FAR uint32_t *msi_addr_offset)
{
	...
	nxmutex_lock(&epc->lock);
	ret = epc->ops->map_msi_irq(epc, funcno, phys_addr, interrupt_num, entry_size, msi_data,
								msi_addr_offset);
	nxmutex_unlock(&epc->lock);
...
int pci_epc_get_msi(FAR struct pci_epc_ctrl_s *epc, uint8_t funcno)
{
	...
	if (epc == NULL || funcno >= epc->max_functions || epc->ops->get_msi == NULL) {
		return -EINVAL;
	}
...
int pci_epc_set_msi(FAR struct pci_epc_ctrl_s *epc, uint8_t funcno, uint8_t interrupts)
{
	int ret;

	if (epc == NULL || funcno >= epc->max_functions || interrupts < 1 || interrupts > 32 ||
		epc->ops->set_msi == NULL) {
		return -EINVAL;
	}
...
int pci_epc_get_msix(FAR struct pci_epc_ctrl_s *epc, uint8_t funcno)
{
	...
	if (epc == NULL || funcno >= epc->max_functions || epc->ops->get_msix == NULL) {
		return -EINVAL;
	}
...
int pci_epc_set_msix(FAR struct pci_epc_ctrl_s *epc, uint8_t funcno, uint16_t interrupts, int barno,
					 uint32_t offset)
{
	int ret;

	if (epc == NULL || funcno >= epc->max_functions || interrupts < 1 || interrupts > 2048 ||
		epc->ops->set_msix == NULL) {
		return -EINVAL;
	}
...
void pci_epc_unmap_addr(FAR struct pci_epc_ctrl_s *epc, uint8_t funcno, uintptr_t phys_addr)
{
	if (epc == NULL || funcno >= epc->max_functions || epc->ops->unmap_addr == NULL) {
		return;
	}
...
int pci_epc_map_addr(FAR struct pci_epc_ctrl_s *epc, uint8_t funcno, uintptr_t phys_addr,
					 uint64_t pci_addr, size_t size)
{
	int ret;

	if (epc == NULL || funcno >= epc->max_functions || epc->ops->map_addr == NULL) {
		return -EINVAL;
	}
...
{
	if (epc == NULL || funcno >= epc->max_functions || epc->ops->clear_bar == NULL ||
		(bar->barno == PCI_STD_NUM_BARS - 1 && (bar->flags & PCI_BASE_ADDRESS_MEM_TYPE_64))) {
		return;
	}
...
int pci_epc_set_bar(FAR struct pci_epc_ctrl_s *epc, uint8_t funcno, FAR struct pci_epf_bar_s *bar)
{
	int flags = bar->flags;
	int ret;

	if (epc == NULL || funcno >= epc->max_functions ||
		(bar->barno == PCI_STD_NUM_BARS - 1 && flags & PCI_BASE_ADDRESS_MEM_TYPE_64) ||
		(bar->size > UINT32_MAX && !(flags & PCI_BASE_ADDRESS_MEM_TYPE_64)) ||
						 ...
						 FAR struct pci_epf_header_s *header)
{
	...
	if (epc == NULL || epc->ops->write_header == NULL || funcno >= epc->max_functions) {
		return -EINVAL;
	}
...
int pci_epc_add_epf(FAR struct pci_epc_ctrl_s *epc, FAR struct pci_epf_device_s *epf)
{
	uint32_t funcno;
	int ret = 0;
...
void pci_epc_remove_epf(FAR struct pci_epc_ctrl_s *epc, FAR struct pci_epf_device_s *epf)
{
...
FAR struct pci_epc_ctrl_s *pci_epc_create(FAR const char *name, FAR void *priv,
