...
static const struct pci_device_id_s g_ivshmem_id_table[] = {{PCI_DEVICE(0x1af4, 0x1110)},
															{
																0,
															}};
...
static int ivshmem_register_device(FAR struct ivshmem_device_s *dev)
{
	...
	list_for_every_entry(&g_ivshmem_bus.drv, drv, struct ivshmem_driver_s, node)
	{
		if (drv->id == dev->id) {
...
static int ivshmem_probe(FAR struct pci_device_s *dev)
{
	...
	if (pci_resource_flags(dev, IVSHMEM_MSIX_BAR) != 0) {
		pci_read_mmio_dword(dev, (FAR char *)ivdev->reg + IVSHMEM_REG_IVPOSITION, &ivdev->vmid);
...
FAR struct ivshmem_driver_s *ivshmem_get_driver(FAR struct ivshmem_device_s *dev)
{
	return dev->drv;
}
...
FAR void *ivshmem_get_shmem(FAR struct ivshmem_device_s *dev, FAR size_t *size)
{
	*size = dev->shmem_size;
	return dev->shmem;
}
...
int ivshmem_attach_irq(FAR struct ivshmem_device_s *dev, xcpt_t isr, FAR void *arg)
{
...
int ivshmem_kick_peer(FAR struct ivshmem_device_s *dev)
{
	...
	pci_write_mmio_dword(dev->dev, dev->reg + IVSHMEM_REG_DOORBELL, peer_id << 16);
	return 0;
}
...
int ivshmem_register_driver(FAR struct ivshmem_driver_s *drv)
{
	...
	DEBUGASSERT(drv != NULL && drv->probe != NULL && drv->remove != NULL);
	...
	list_for_every_entry(&g_ivshmem_bus.dev, dev, struct ivshmem_device_s, node)
	{
		if (drv->id == dev->id) {
