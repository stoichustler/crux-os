...
#define PCI_BUS_READ_CONFIG(len, type, size)                                                       \
	int pci_bus_read_config_##len(FAR struct pci_bus_s *bus, unsigned int devfn, int where,        \
								  FAR type *value)                                                 \
	{                                                                                              \
		int ret = -EINVAL;                                                                         \
		uint32_t data = 0;                                                                         \
...
#define PCI_BUS_WRITE_CONFIG(len, type, size)                                                      \
	int pci_bus_write_config_##len(FAR struct pci_bus_s *bus, unsigned int devfn, int where,       \
								   type value)                                                     \
	{                                                                                              \
		int ret = -EINVAL;                                                                         \
                                                                                                   \
...
#define PCI_BUS_READ_IO(len, type, size)                                                           \
	int pci_bus_read_io_##len(FAR struct pci_bus_s *bus, uintptr_t where, FAR type *value)         \
	{                                                                                              \
		int ret = -EINVAL;                                                                         \
		uint32_t data = 0;                                                                         \
                                                                                                   \
...
#define PCI_BUS_WRITE_IO(len, type, size)                                                          \
	int pci_bus_write_io_##len(FAR struct pci_bus_s *bus, uintptr_t where, type value)             \
	{                                                                                              \
		int ret = -EINVAL;                                                                         \
                                                                                                   \
		if (!PCI_##len##_BAD) {                                                                    \
...
static struct list_node g_pci_device_list = LIST_INITIAL_VALUE(g_pci_device_list);
static struct list_node g_pci_driver_list = LIST_INITIAL_VALUE(g_pci_driver_list);
static struct list_node g_pci_ctrl_list = LIST_INITIAL_VALUE(g_pci_ctrl_list);
...
static FAR struct pci_device_s *pci_do_find_device_from_bus(FAR struct pci_bus_s *bus,
															uint8_t busno, unsigned int devfn)
{
	...
	list_for_every_entry(&bus->children, bus_tmp, struct pci_bus_s, node)
	{
...
static int pci_vpd_read(FAR struct pci_bus_s *bus, uint32_t devfn, int offset, int count,
						FAR uint32_t *data)
{
	uint8_t pos;
	int i;
...
static int pci_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case PCIOCREAD: {
		...
		ret = pci_bus_read_config(ctrl->bus, devfn, io->pi_reg, io->pi_width, &io->pi_data);
		break;
	}

	case PCIOCWRITE: {
		FAR struct pci_io *io = (FAR struct pci_io *)arg;
		ret = pci_bus_write_config(ctrl->bus, devfn, io->pi_reg, io->pi_width, io->pi_data);
		break;
	}

	case PCIOCGETROMLEN: {
		FAR struct pci_rom *rom = (FAR struct pci_rom *)arg;
		FAR struct pci_device_s *dev = pci_find_device_from_bus(ctrl->bus, sel->pc_bus, devfn);
		if (dev == NULL) {
			return -ENODEV;
		}
	...
	case PCIOCGETROM: {
		...
		pci_bus_write_config_dword(ctrl->bus, devfn, PCI_ROM_ADDRESS, addr);
		break;
	}
	...
	case PCIOCREADMASK: {
		...
		FAR struct pci_io *io = (FAR struct pci_io *)arg;
		if (io->pi_width != 4 || (io->pi_reg & 0x3) || io->pi_reg < PCI_BASE_ADDRESS_0 ||
		...
		pci_bus_write_config_dword(ctrl->bus, devfn, io->pi_reg, 0xffffffff);
		pci_bus_read_config_dword(ctrl->bus, devfn, io->pi_reg, &io->pi_data);
	...
	case PCIOCGETVPD: {
		...
		ret = pci_vpd_read(ctrl->bus, devfn, pv->pv_offset, pv->pv_count, pv->pv_data);
		break;
	}
...
static uint8_t pci_bus_find_start_cap(FAR struct pci_bus_s *bus, unsigned int devfn,
									  uint8_t hdr_type)
{
	uint16_t status;
...
static uint8_t pci_find_next_cap_ttl(FAR struct pci_bus_s *bus, unsigned int devfn, uint8_t pos,
									 int cap, FAR int *ttl)
{
	uint16_t ent;
	uint8_t id;
...
static uint8_t pci_find_next_cap(FAR struct pci_bus_s *bus, unsigned int devfn, uint8_t pos,
								 int cap)
{
	int ttl = 48;
...
static void pci_setup_device(FAR struct pci_device_s *dev, int max_bar, uint8_t rom_addr,
							 FAR struct pci_resource_s *io, FAR struct pci_resource_s *mem,
							 FAR struct pci_resource_s *mem_pref)
{
	...
	pci_write_config_byte(dev, PCI_COMMAND, cmd & ~PCI_COMMAND_IO & ~PCI_COMMAND_MEMORY);
#else
	uint32_t tmp;
#endif
	...
	for (bar = 0; bar < max_bar; bar++) {
		...
		if (mask & PCI_BASE_ADDRESS_SPACE_IO) {
			...
			if ((mask & PCI_BASE_ADDRESS_MEM_TYPE_MASK) == PCI_BASE_ADDRESS_MEM_TYPE_64) {
				/* Prefetchable MEM */

				mask64 = 0xfffffff0;
				flags = PCI_RESOURCE_MEM_64 | PCI_RESOURCE_PREFETCH;
				res = mem_pref;
			} else if (((mask & PCI_BASE_ADDRESS_MEM_TYPE_MASK) == PCI_BASE_ADDRESS_MEM_TYPE_32) &&
	...
	pci_write_config_dword(dev, rom_addr, ~PCI_ROM_ADDRESS_ENABLE);
	...
	if (start != 0 && size64 != 0) {
		dev->resource[PCI_ROM_RESOURCE].flags |= PCI_RESOURCE_MEM | PCI_RESOURCE_PREFETCH;
...
static void pci_presetup_bridge(FAR struct pci_device_s *dev)
{
	...
	if (pci_resource_size(&ctrl->mem)) {
		...
		pci_write_config_word(dev, PCI_MEMORY_BASE, (ctrl->mem.start & 0xfff00000) >> 16);
	...
	if (pci_resource_size(&ctrl->mem_pref)) {
		...
		pci_write_config_word(dev, PCI_PREF_MEMORY_BASE, (ctrl->mem_pref.start & 0xfff00000) >> 16);
		if (base & PCI_PREF_RANGE_TYPE_64) {
			pci_write_config_dword(dev, PCI_PREF_BASE_UPPER32, ctrl->mem_pref.start >> 32);
		}
	...
	if (pci_resource_size(&ctrl->io)) {
		...
		pci_write_config_byte(dev, PCI_IO_BASE, (ctrl->io.start & 0x0000f000) >> 8);
		if (base & PCI_IO_RANGE_TYPE_32) {
			pci_write_config_word(dev, PCI_IO_BASE_UPPER16, (ctrl->io.start & 0xffff0000) >> 16);
		}
...
static void pci_postsetup_bridge(FAR struct pci_device_s *dev)
{
	...
	if (pci_resource_size(&ctrl->mem)) {
		...
		pci_write_config_word(dev, PCI_MEMORY_LIMIT, ((ctrl->mem.start - 1) & 0xfff00000) >> 16);
	}
	...
	if (pci_resource_size(&ctrl->io)) {
		...
		pci_write_config_byte(dev, PCI_IO_LIMIT, ((ctrl->io.start - 1) & 0x0000f000) >> 8);
		pci_write_config_word(dev, PCI_IO_LIMIT_UPPER16, ((ctrl->io.start - 1) & 0xffff0000) >> 16);
	}
#endif
}
...
static void pci_scan_bus(FAR struct pci_bus_s *bus)
{
	...
	for (devfn = 0; devfn < 0xff; ++devfn) {
		...
		if (pci_bus_read_config_dword(bus, devfn, PCI_VENDOR_ID, &l) || l == 0xffffffff ||
			l == 0x00000000 || l == 0x0000ffff || l == 0xffff0000) {
			continue;
		}
...
static void pci_get_msi_base(FAR struct pci_device_s *dev, FAR uint8_t *msi, FAR uint8_t *msix)
{
	if (msi != NULL) {
...
static int pci_enable_msi(FAR struct pci_device_s *dev, FAR int *irq, int num, uint8_t msi)
{
	uint32_t mdr = 0;
	uint16_t flags = 0;
	uintptr_t mar = 0;
	...
	if ((flags & PCI_MSI_FLAGS_64BIT) != 0) {
		pci_write_config_dword(dev, msi + PCI_MSI_ADDRESS_HI, ((uint64_t)mar >> 32));
...
static void pci_disable_msix(FAR struct pci_device_s *dev, uint8_t msix, int num)
{
	uint16_t flags = 0;
	uintptr_t tbladdr = 0;
...
static int pci_enable_msix(FAR struct pci_device_s *dev, FAR int *irq, int num, uint8_t msix)
{
	uint32_t mdr = 0;
	uint16_t flags = 0;
	uintptr_t mar = 0;
	...
	for (i = 0; i < num; i++) {
		...
		ret = dev->bus->ctrl->ops->connect_irq(dev->bus, &irq[i], 1, &mar, &mdr);
		if (ret < 0) {
			return ret;
		}
		...
		pci_write_mmio_dword(dev, tbladdr + PCI_MSIX_ENTRY_UPPER_ADDR, ((uint64_t)mar >> 32));
...
FAR struct pci_device_s *pci_find_device_from_bus(FAR struct pci_bus_s *bus, uint8_t busno,
												  unsigned int devfn)
{
	FAR struct pci_device_s *dev;
	int ret;
...
int pci_bus_read_config(FAR struct pci_bus_s *bus, unsigned int devfn, int where, int size,
						FAR uint32_t *val)
{
...
int pci_bus_write_config(FAR struct pci_bus_s *bus, unsigned int devfn, int where, int size,
						 uint32_t val)
{
	if (size != 1 && size != 2 && size != 4) {
		return -EINVAL;
...
int pci_bus_read_io(FAR struct pci_bus_s *bus, uintptr_t addr, int size, FAR uint32_t *val)
{
	if (size != 1 && size != 2 && size != 4) {
		return -EINVAL;
	}
...
int pci_bus_write_io(FAR struct pci_bus_s *bus, uintptr_t addr, int size, uint32_t val)
{
	if (size != 1 && size != 2 && size != 4) {
		return -EINVAL;
	}
...
int pci_enable_device(FAR struct pci_device_s *dev)
{
	...
	return pci_write_config_dword(dev, PCI_COMMAND, cmd | PCI_COMMAND_IO | PCI_COMMAND_MEMORY);
}
...
FAR void *pci_bus_map_region(FAR struct pci_bus_s *bus, uintptr_t start, size_t size)
{
	return bus->ctrl->ops->map ? (FAR void *)bus->ctrl->ops->map(bus, start, start + size)
							   : (FAR void *)start;
}
...
FAR void *pci_map_bar_region(FAR struct pci_device_s *dev, int bar, uintptr_t offset, size_t length)
{
...
uint8_t pci_find_next_capability(FAR struct pci_device_s *dev, uint8_t pos, int cap)
{
	return pci_find_next_cap(dev->bus, dev->devfn, pos + PCI_CAP_LIST_NEXT, cap);
}
...
bool pci_stat_line(FAR struct pci_device_s *dev)
{
	...
	return (!(tmp1 & PCI_COMMAND_INTX_DISABLE) && (tmp2 & PCI_STATUS_INTERRUPT));
}
...
uint8_t pci_bus_find_capability(FAR struct pci_bus_s *bus, unsigned int devfn, int cap)
{
	uint8_t type = 0;
	uint8_t pos;
