...
struct pci_qemu_test_ops_s {
	...
	CODE int (*write)(FAR struct pci_bus_s *bus, FAR void *addr, uint32_t val, int size);
};
...
static uint32_t pci_qemu_test_read_mem(FAR struct pci_bus_s *bus, FAR void *addr, int size);
static int pci_qemu_test_write_mem(FAR struct pci_bus_s *bus, FAR void *addr, uint32_t val,
								   int size);
static uint32_t pci_qemu_test_read_io(FAR struct pci_bus_s *bus, FAR void *addr, int size);
static int pci_qemu_test_write_io(FAR struct pci_bus_s *bus, FAR void *addr, uint32_t val,
								  int size);
...
static int pci_qemu_test_write_io(FAR struct pci_bus_s *bus, FAR void *addr, uint32_t val, int size)
{
...
static bool pci_qemu_test_bar(FAR struct pci_device_s *dev,
							  ...
							  FAR struct pci_qemu_test_hdr_s *hdr, uint8_t num)
{
	...
	for (write_cnt = 0; write_cnt < write_limit; write_cnt++) {
		pciinfo("[%" PRIu32 "]Issuing WRITE to %p Data: 0x%04" PRIx32 " Width: %u\n", write_cnt,
				(FAR char *)hdr + offset, data, width);
