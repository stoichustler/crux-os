...
static struct list_node g_pci_epf_device_list = LIST_INITIAL_VALUE(g_pci_epf_device_list);
static struct list_node g_pci_epf_driver_list = LIST_INITIAL_VALUE(g_pci_epf_driver_list);
...
pci_epf_match_id(FAR const struct pci_epf_device_s *dev, FAR const struct pci_epf_device_id_s *id)
{
	while (id->name[0]) {
...
static FAR const struct pci_epf_device_id_s *pci_epf_match_device(FAR struct pci_epf_device_s *dev,
...
void pci_epf_free_space(FAR struct pci_epf_device_s *epf, int barno, FAR void *addr)
{
...
FAR void *pci_epf_alloc_space(FAR struct pci_epf_device_s *epf, int barno, size_t size,
							  size_t align)
{
	FAR struct pci_epf_bar_s *bar;
	FAR void *space;
	...
	bar[barno].flags |=
		(size > UINT32_MAX) ? PCI_BASE_ADDRESS_MEM_TYPE_64 : PCI_BASE_ADDRESS_MEM_TYPE_32;
...
int pci_epf_device_register(FAR struct pci_epf_device_s *epf)
{
	...
	list_for_every_entry(&g_pci_epf_driver_list, drv, struct pci_epf_driver_s, node)
	{
...
int pci_epf_register_driver(FAR struct pci_epf_driver_s *drv)
{
	...
	if (drv == NULL || drv->ops == NULL || drv->ops->bind == NULL || drv->id_table == NULL) {
		return -EINVAL;
	}
	...
	list_for_every_entry(&g_pci_epf_device_list, epf, struct pci_epf_device_s, node)
	{
		if (epf->driver != NULL) {
			continue;
		}
