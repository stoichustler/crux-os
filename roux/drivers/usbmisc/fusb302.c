...
static ssize_t fusb302_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int fusb302_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int fusb302_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
static int fusb302_putreg(FAR struct fusb302_dev_s *priv, uint8_t regaddr, uint8_t regval);
...
static int fusb302_set_mode(FAR struct fusb302_dev_s *priv, enum fusb302_mode_e mode);
...
void enableccmeas(FAR struct fusb302_dev_s *priv, enum cc_meas_e pullup, bool enable);
void set_switches(FAR struct fusb302_dev_s *priv, uint8_t toggsval);
...
static int set_int_mask(struct fusb302_dev_s *priv)
{
	...
	case WAITING_FOR_SRC_TOGG_I:
		ret = fusb302_putreg(priv, FUSB302_MASK_REG, (uint8_t)~MASKA_TOGDONE);
		ret = fusb302_putreg(priv, FUSB302_MASKA_REG, (uint8_t)MASKA_HARDRST);
		ret = fusb302_putreg(priv, FUSB302_MASKB_REG, (uint8_t)MASKB_GCRCSENT);
		break;
	case WAITING_FOR_VBUS:
		ret = fusb302_putreg(priv, FUSB302_MASK_REG,
							 ((uint8_t)~MASK_VBUS_OK) | ((uint8_t)~MASK_BC_LVL) |
								 ((uint8_t)~MASK_COMP_CHNG));
		break;
	case WAITING_FOR_COMP_CHNG:
		ret = fusb302_putreg(priv, FUSB302_MASK_REG, (uint8_t)~MASK_COMP_CHNG);
		break;
	case WAITING_FOR_SRC_DISCONNECT:
		ret = fusb302_putreg(priv, FUSB302_MASK_REG, (uint8_t)~MASK_COMP_CHNG);
		break;
	case WAITING_FOR_SNK_DISCONNECT:
		ret = fusb302_putreg(priv, FUSB302_MASK_REG, (uint8_t)~MASK_VBUS_OK);
		break;
...
void enableccmeas(struct fusb302_dev_s *priv, enum cc_meas_e measure, bool enable)
{
	int regval;
	...
	if (!priv->int_pending) {
		...
		case WAITING_FOR_DRP_TOGG_I:
			if (interrupta & INTERRUPTA_M_TOGDONE) {
				...
				fusb302_putreg(priv, FUSB302_POWER_REG, POWER_MODE_ALL);

				fusb302_dump_registers(priv, "After DRP toggle detect handling");
				...
				case TOGGS_SNK_CC2:
					regval = fusb302_getreg(priv, FUSB302_STATUS0_REG);
					if ((interrupt & INTERRUPT_VBUS_OK) || (regval & STATUS0_VBUS_OK) ||
		...
		case WAITING_FOR_SNK_TOGG_I:
			if (interrupta & INTERRUPTA_M_TOGDONE) {
				...
				fusb302_dump_registers(priv, "After SNK toggle detect handling");
		...
		case WAITING_FOR_SRC_TOGG_I:
			if (interrupta & INTERRUPTA_M_TOGDONE) {
				...
				fusb302_dump_registers(priv, "After SRC toggle detect handling");
...
static void fusb302_dump_registers(FAR struct fusb302_dev_s *priv, FAR const char *msg)
{
	...
	fusb302_info(
		...
		fusb302_getreg(priv, FUSB302_DEV_ID_REG), fusb302_getreg(priv, FUSB302_SWITCHES0_REG),
		fusb302_getreg(priv, FUSB302_SWITCHES1_REG), fusb302_getreg(priv, FUSB302_MEASURE_REG));
	fusb302_info(
		"     Slice: %02x  Control0: %02x    Control1:%02x   Control2: %02x\n",
		fusb302_getreg(priv, FUSB302_SLICE_REG), fusb302_getreg(priv, FUSB302_CONTROL0_REG),
		fusb302_getreg(priv, FUSB302_CONTROL1_REG), fusb302_getreg(priv, FUSB302_CONTROL2_REG));
	fusb302_info(" Control3:  %02x      Mask: %02x       Power:%02x      Reset: %02x\n",
				 fusb302_getreg(priv, FUSB302_CONTROL3_REG), fusb302_getreg(priv, FUSB302_MASK_REG),
				 fusb302_getreg(priv, FUSB302_POWER_REG), fusb302_getreg(priv, FUSB302_RESET_REG));
	fusb302_info(" OCPreg:    %02x     MaskA: %02x       MaskB:%02x   Control4: %02x\n",
				 fusb302_getreg(priv, FUSB302_OCPREG_REG), fusb302_getreg(priv, FUSB302_MASKA_REG),
	...
	fusb302_info(
		...
		fusb302_getreg(priv, FUSB302_STATUS0A_REG), fusb302_getreg(priv, FUSB302_STATUS1A_REG),
		fusb302_getreg(priv, FUSB302_INTERRUPTA_REG), fusb302_getreg(priv, FUSB302_INTERRUPTB_REG));
	fusb302_info(
		" Status0:   %02x   Status1: %02x   Interrupt:%02x      FIFOs: %02x\n",
		fusb302_getreg(priv, FUSB302_STATUS0_REG), fusb302_getreg(priv, FUSB302_STATUS1_REG),
		fusb302_getreg(priv, FUSB302_INTERRUPT_REG), fusb302_getreg(priv, FUSB302_FIFOS_REG));
}
#endif
...
static int fusb302_read_device_id(FAR struct fusb302_dev_s *priv, FAR uint8_t *arg)
{
	int ret;
...
static int fusb302_setup(FAR struct fusb302_dev_s *priv, struct fusb302_setup_s *setup)
{
	/* Not implemented yet */
...
static int fusb302_set_mode(FAR struct fusb302_dev_s *priv, enum fusb302_mode_e mode)
{
	...
	case MODE_SNK_POLL_AUTO: {
		...
		ret = fusb302_putreg(priv, FUSB302_CONTROL2_REG,
							 (SET_POLL_MODE(SNK_POLLING) | CONTROL2_TOGGLE));
	...
	case MODE_SRC_POLL_AUTO: {
		...
		ret = fusb302_putreg(priv, FUSB302_CONTROL2_REG,
							 (SET_POLL_MODE(SRC_POLLING) | CONTROL2_TOGGLE));
...
static int fusb302_read_status(FAR struct fusb302_dev_s *priv, FAR uint8_t *arg)
{
	int ret;
...
static ssize_t fusb302_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct fusb302_dev_s *priv = inode->i_private;
...
int fusb302_register(FAR const char *devpath, FAR struct fusb302_config_s *config)
{
	...
	priv = (FAR struct fusb302_dev_s *)kmm_zalloc(sizeof(struct fusb302_dev_s));
