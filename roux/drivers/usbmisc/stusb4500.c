...
static ssize_t stusb4500_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int stusb4500_getreg(FAR struct stusb4500_dev_s *priv, uint8_t reg, uint8_t *rxbuf,
							uint8_t len)
{
	int ret = -EIO;
	int retries;
	...
	for (retries = 0; retries < STUSB4500_I2C_RETRIES; retries++) {
		...
		if (ret >= 0) {
			...
			int offset =
				snprintf(buffer, STUSB4500_BUFFER_SIZE, "reg:%02X value:%02X", reg, rxbuf[0]);

			for (int rxbytes = 1; rxbytes < len && offset < STUSB4500_BUFFER_SIZE; rxbytes++) {
				offset += snprintf(buffer + offset, STUSB4500_BUFFER_SIZE - offset, " %02X",
								   rxbuf[rxbytes]);
			}
...
static int stusb4500_putreg(FAR struct stusb4500_dev_s *priv, uint8_t regaddr, uint8_t regval)
{
	int ret = -EIO;
	int retries;
...
static int stusb4500_read_device_id(FAR struct stusb4500_dev_s *priv, FAR uint8_t *dev_id)
{
	int ret = -EIO;
...
static ssize_t stusb4500_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	return 0;
}
...
static int stusb4500_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case USBCIOC_SET_PWR: {
		...
		stusb4500_putreg(priv, STUSB4500_REG_DPM_PDO_NUMB, 2U);

		/* Download negotiation contract */

		ret = stusb4500_putreg(priv, STUSB4500_REG_DPM_SNK_PDO2_0, pdo->bytes[0]);
		ret = stusb4500_putreg(priv, STUSB4500_REG_DPM_SNK_PDO2_1, pdo->bytes[1]);
		ret = stusb4500_putreg(priv, STUSB4500_REG_DPM_SNK_PDO2_2, pdo->bytes[2]);
		ret = stusb4500_putreg(priv, STUSB4500_REG_DPM_SNK_PDO2_3, pdo->bytes[3]);

		/* Trigger negotiation by soft reset */

		stusb4500_putreg(priv, STUSB4500_REG_TX_HEADER_LOW, 0x0d);
		stusb4500_putreg(priv, STUSB4500_REG_PD_COMMAND_CTRL, 0x26);
		break;
	}
...
int stusb4500_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr)
{
	...
	priv = (FAR struct stusb4500_dev_s *)kmm_zalloc(sizeof(struct stusb4500_dev_s));
	if (!priv) {
