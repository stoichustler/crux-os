...
#define FUSB303_ALL_INTR                                                                           \
	(INTERRUPT_ATTACH | INTERRUPT_DETACH | INTERRUPT_BC_LVL | INTERRUPT_AUTOSNK |                  \
	 INTERRUPT_VBUS_CHG | INTERRUPT_FAULT | INTERRUPT_ORIENT)

#define FUSB303_ALL_INTR1                                                                          \
	(INTERRUPT1_REMEDY | INTERRUPT1_FRC_SUCC | INTERRUPT1_FRC_FAIL | INTERRUPT1_REM_FAIL |         \
...
#ifdef CONFIG_DEBUG_FUSB303
static int fusb303_dumpregs(FAR const char *funcname, FAR struct fusb303_dev_s *priv);
#endif
...
static ssize_t fusb303_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int fusb303_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
static int fusb303_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int fusb303_putreg(FAR struct fusb303_dev_s *priv, uint8_t regaddr, uint8_t regval)
{
	int ret = -EIO;
	int retries;
...
static int fusb303_read_device_id(FAR struct fusb303_dev_s *priv, FAR uint8_t *dev_id,
								  FAR uint8_t *dev_type)
{
	int ret;
...
static int fusb303_setup(FAR struct fusb303_dev_s *priv, struct fusb303_setup_s *setup)
{
	int ret = OK;
	uint8_t regval;
	...
	fusb303_info("drp_tgl:%02X, host_curr:%02X\n"
				 ...
				 setup->drp_toggle_timing, setup->host_current, (int)setup->dcable_en,
				 (int)setup->remedy_en, (int)setup->auto_snk_en, (int)setup->global_int_mask,
	...
	regval = setup->drp_toggle_timing | setup->host_current | setup->global_int_mask;
	if (setup->dcable_en) {
...
static int fusb303_set_mode(FAR struct fusb303_dev_s *priv, enum fusb303_mode_e mode)
{
	int ret;
...
static int fusb303_set_state(FAR struct fusb303_dev_s *priv, enum fusb303_manual_e state)
{
	int ret;
...
static int fusb303_read_status(FAR struct fusb303_dev_s *priv, FAR uint8_t *arg)
{
	int ret;
...
static int fusb303_read_devtype(FAR struct fusb303_dev_s *priv, FAR uint8_t *arg)
{
	int ret;
...
static ssize_t fusb303_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	FAR struct inode *inode = filep->f_inode;
	FAR struct fusb303_dev_s *priv = inode->i_private;
...
int fusb303_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
					 FAR struct fusb303_config_s *config)
{
	FAR struct fusb303_dev_s *priv;
	int ret;
