...
static int fusb301_putreg(FAR struct fusb301_dev_s *priv, uint8_t regaddr, uint8_t regval)
{
	int ret = -EIO;
	int retries;
...
static int fusb301_read_device_id(FAR struct fusb301_dev_s *priv, FAR uint8_t *arg)
{
	int ret;
...
static int fusb301_setup(FAR struct fusb301_dev_s *priv, struct fusb301_setup_s *setup)
{
	int ret = OK;
	...
	ret = fusb301_putreg(priv, FUSB301_CONTROL_REG,
						 setup->drp_toggle_timing | setup->host_current | setup->global_int_mask);
...
static int fusb301_set_mode(FAR struct fusb301_dev_s *priv, enum fusb301_mode_e mode)
{
	int ret = OK;
...
static int fusb301_set_state(FAR struct fusb301_dev_s *priv, enum fusb301_manual_e state)
{
	int ret = OK;
...
static int fusb301_read_status(FAR struct fusb301_dev_s *priv, FAR uint8_t *arg)
{
	int ret;
...
static int fusb301_read_devtype(FAR struct fusb301_dev_s *priv, FAR uint8_t *arg)
{
	int ret;
...
int fusb301_register(FAR const char *devpath, FAR struct i2c_master_s *i2c, uint8_t addr,
					 FAR struct fusb301_config_s *config)
{
	FAR struct fusb301_dev_s *priv;
	int ret;
