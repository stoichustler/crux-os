...
static void jd9851_cmddata(FAR struct jd9851_dev_s *dev, uint8_t cmd, const uint8_t *data, int len);
...
static void jd9851_setcursor(FAR struct jd9851_dev_s *dev, uint16_t x0, uint16_t y0, uint16_t x1,
							 uint16_t y1);
static void jd9851_bpp(FAR struct jd9851_dev_s *dev, int bpp);
static void jd9851_wrram(FAR struct jd9851_dev_s *dev, FAR const uint8_t *buff, size_t size,
						 size_t skip, size_t count);
#ifndef CONFIG_LCD_NOGETRUN
static void jd9851_rdram(FAR struct jd9851_dev_s *dev, FAR uint16_t *buff, size_t size);
#endif
static void jd9851_fill(FAR struct jd9851_dev_s *dev, uint16_t color);

/* LCD Data Transfer Methods */

static int jd9851_putrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						 FAR const uint8_t *buffer, size_t npixels);
static int jd9851_putarea(FAR struct lcd_dev_s *dev, fb_coord_t row_start, fb_coord_t row_end,
						  fb_coord_t col_start, fb_coord_t col_end, FAR const uint8_t *buffer,
						  fb_coord_t stride);
#ifndef CONFIG_LCD_NOGETRUN
static int jd9851_getrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						 FAR uint8_t *buffer, size_t npixels);
#endif

/* LCD Configuration */

static int jd9851_getvideoinfo(FAR struct lcd_dev_s *dev, FAR struct fb_videoinfo_s *vinfo);
static int jd9851_getplaneinfo(FAR struct lcd_dev_s *dev, unsigned int planeno,
...
static int jd9851_setcontrast(FAR struct lcd_dev_s *dev, unsigned int contrast);
...
static void jd9851_init(FAR struct jd9851_dev_s *dev)
{
	...
	jd9851_cmddata(dev, JD9851_GAMMA_SET_CMD, (const uint8_t *)"\x1b\x7a\x17\x32", 4);
	jd9851_cmddata(dev, JD9851_R_GAMMA_SET_CMD,
				   (const uint8_t *)"\x3c\x32\x2b\x2c\x2f\x32\x2c\x2b\x28\x27\x22\x16\x10\x0b\
                  \x05\x0e\x3c\x32\x2b\x2c\x2f\x32\x2c\x2b\x28\x27\x22\x16\
                  \x10\x0b\x05\x0e",
				   32);
	jd9851_cmddata(dev, JD9851_POWER_CTRL, (const uint8_t *)"\x33\x28\xcc", 3);
	jd9851_cmddata(dev, JD9851_DCDC_SET, (const uint8_t *)"\x47\x7a\x30\x30\x6c\x60\x50\x70", 8);
	...
	jd9851_cmddata(dev, JD9851_SETRGBCYC, (const uint8_t *)"\x08\x00\x0a\x10\x08\x54\x45\x71\x2c",
				   9);
	jd9851_cmddata(dev, JD9851_SETTCON,
				   (const uint8_t *)"\x00\xa0\x79\x0e\x0a\x16\x79\x0e\x0a\x16\x79\x0e\x0a\x16\
                  \x82\x00\x03",
				   17);
	jd9851_cmddata(dev, JD9851_SETGD, (const uint8_t *)"\x04\x0c\x6b\x0f\x07\x03", 6);
	...
	jd9851_cmddata(dev, JD9851_DCDC_SET2, (const uint8_t *)"\x19\xa0\x2f\x04\x33", 5);
	jd9851_cmddata(dev, JD9851_SETPANEL, (const uint8_t *)"\x10\x66\x66\x01", 4);
