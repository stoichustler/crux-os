...
static int ft80x_fade(FAR struct ft80x_dev_s *priv, FAR const struct ft80x_fade_s *fade);

static void ft80x_notify(FAR struct ft80x_dev_s *priv, enum ft80x_notify_e id, int value);
...
static ssize_t ft80x_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t ft80x_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static ssize_t ft80x_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
...
static ssize_t ft80x_write(FAR struct file *filep, FAR const char *buffer, size_t len)
{
	FAR struct inode *inode;
	FAR struct ft80x_dev_s *priv;
	int ret;
	...
	lcdinfo("buffer: %p len %lu\n", buffer, (unsigned long)len);
	DEBUGASSERT(buffer != NULL && ((uintptr_t)buffer & 3) == 0 && len > 0 && (len & 3) == 0 &&
				len <= FT80X_RAM_DL_SIZE);

	inode = filep->f_inode;
	DEBUGASSERT(inode->i_private != NULL);
	priv = inode->i_private;

	if (buffer == NULL || ((uintptr_t)buffer & 3) != 0 || len == 0 || (len & 3) != 0 ||
		(len + filep->f_pos) > FT80X_RAM_DL_SIZE) {
		return -EINVAL;
	}
...
static int ft80x_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case FT80X_IOC_APPENDDL: {
		FAR struct ft80x_displaylist_s *dl = (FAR struct ft80x_displaylist_s *)((uintptr_t)arg);

		if (dl == NULL || ((uintptr_t)&dl->cmd & 3) != 0 || (dl->dlsize & 3) != 0 ||
			...
			if (dl->dlsize > 0) {
				...
				ft80x_write_memory(priv, FT80X_RAM_DL + filep->f_pos, &dl->cmd, dl->dlsize);
	...
	case FT80X_IOC_GETRAMDL: {
		FAR struct ft80x_relmem_s *ramdl = (FAR struct ft80x_relmem_s *)((uintptr_t)arg);

		if (ramdl == NULL || ((uintptr_t)ramdl->offset & 3) != 0 ||
			ramdl->offset >= FT80X_RAM_DL_SIZE) {
			ret = -EINVAL;
		} else {
			ft80x_read_memory(priv, FT80X_RAM_DL + ramdl->offset, ramdl->value, ramdl->nbytes);
			ret = OK;
		}
	} break;
	...
	case FT80X_IOC_PUTRAMG: {
		FAR struct ft80x_relmem_s *ramg = (FAR struct ft80x_relmem_s *)((uintptr_t)arg);

		if (ramg == NULL || (ramg->offset + ramg->nbytes) >= FT80X_RAM_G_SIZE) {
			ret = -EINVAL;
		} else {
			ft80x_write_memory(priv, FT80X_RAM_G + ramg->offset, ramg->value, ramg->nbytes);
			ret = OK;
		}
	} break;
	...
	case FT80X_IOC_PUTRAMCMD: {
		FAR struct ft80x_relmem_s *ramcmd = (FAR struct ft80x_relmem_s *)((uintptr_t)arg);

		if (ramcmd == NULL || ((uintptr_t)ramcmd->offset & 3) != 0) {
			ret = -EINVAL;
		} else {
			ft80x_write_memory(priv, FT80X_RAM_CMD + ramcmd->offset, ramcmd->value, ramcmd->nbytes);
			ret = OK;
		}
	} break;
	...
	case FT80X_IOC_GETREG8: {
		FAR struct ft80x_register_s *reg = (FAR struct ft80x_register_s *)((uintptr_t)arg);
	...
	case FT80X_IOC_GETREGS: {
		FAR struct ft80x_registers_s *regs = (FAR struct ft80x_registers_s *)((uintptr_t)arg);

		if (regs == NULL || ((uintptr_t)regs->addr & 3) != 0) {
			ret = -EINVAL;
		} else {
			ft80x_read_memory(priv, regs->addr, regs->value, (size_t)regs->nregs << 2);
			ret = OK;
		}
	} break;
	...
	case FT80X_IOC_PUTREGS: {
		...
		if (regs == NULL || ((uintptr_t)regs->addr & 3) != 0) {
			...
			ft80x_write_memory(priv, regs->addr, regs->value, (size_t)regs->nregs << 2);
			ret = OK;
		}
	} break;
	...
	case FT80X_IOC_EVENTNOTIFY: {
		FAR struct ft80x_notify_s *notify = (FAR struct ft80x_notify_s *)((uintptr_t)arg);

		if (notify == NULL || notify->pid < 0 || (unsigned int)notify->id >= FT80X_INT_NEVENTS) {
			ret = -EINVAL;
		} else {
	...
	case FT80X_IOC_FADE: {
		FAR const struct ft80x_fade_s *fade = (FAR const struct ft80x_fade_s *)((uintptr_t)arg);

		if (fade == NULL || fade->duty > 100 || fade->delay < MIN_FADE_DELAY ||
			fade->delay > MAX_FADE_DELAY) {
			ret = -EINVAL;
		} else {
