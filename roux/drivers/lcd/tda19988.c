...
#include <roux/video/edid.h>
#include <roux/lcd/tda19988.h>
...
static int tda19988_getregs(FAR const struct tda19988_i2c_s *dev, uint8_t regaddr,
							FAR uint8_t *regval, int nregs);
static int tda19988_putreg(FAR const struct tda19988_i2c_s *dev, uint8_t regaddr, uint8_t regval);
static int tda19988_putreg16(FAR const struct tda19988_i2c_s *dev, uint8_t regaddr,
							 uint16_t regval);
static int tda19988_modifyreg(FAR const struct tda19988_i2c_s *dev, uint8_t regaddr,
							  uint8_t clrbits, uint8_t setbits);

/* CEC I2C Helpers */

static inline int tda19988_cec_getregs(FAR struct tda1988_dev_s *priv, uint8_t regaddr,
									   FAR uint8_t *regval, int nregs);
static inline int tda19988_cec_putreg(FAR struct tda1988_dev_s *priv, uint8_t regaddr,
									  uint8_t regval);
static inline int tda19988_cec_modifyreg(FAR struct tda1988_dev_s *priv, uint8_t regaddr,
										 uint8_t clrbits, uint8_t setbits);

/* HDMI I2C Helpers */

static int tda19988_select_page(FAR struct tda1988_dev_s *priv, uint8_t page);
static int tda19988_hdmi_getregs(FAR struct tda1988_dev_s *priv, uint16_t reginfo,
								 FAR uint8_t *regval, int nregs);
static int tda19988_hdmi_putreg(FAR struct tda1988_dev_s *priv, uint16_t reginfo, uint8_t regval);
static int tda19988_hdmi_putreg16(FAR struct tda1988_dev_s *priv, uint16_t reginfo,
								  uint16_t regval);
static int tda19988_hdmi_modifyreg(FAR struct tda1988_dev_s *priv, uint16_t reginfo,
								   uint8_t clrbits, uint8_t setbits);
...
static int tda19988_fetch_edid_block(FAR struct tda1988_dev_s *priv, FAR uint8_t *buf, int block);
static int tda19988_fetch_edid(struct tda1988_dev_s *priv);
static ssize_t tda19988_read_internal(FAR struct tda1988_dev_s *priv, off_t offset,
...
static ssize_t tda19988_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t tda19988_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int tda19988_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
#ifndef CONFIG_DISABLE_PSEUDOFS_OPERATIONS
...
static int tda19988_putreg16(FAR const struct tda19988_i2c_s *dev, uint8_t regaddr, uint16_t regval)
{
	uint8_t buffer[3];
	int ret;
...
static int tda19988_select_page(FAR struct tda1988_dev_s *priv, uint8_t page)
{
	...
	if (page != HDMI_NO_PAGE && page != priv->page) {
		ret = tda19988_putreg(&priv->lower->hdmi, REGADDR(HDMI_PAGE_SELECT_REG), page);
	}
...
static int tda19988_hdmi_putreg16(FAR struct tda1988_dev_s *priv, uint16_t reginfo, uint16_t regval)
{
	uint8_t page = REGPAGE(reginfo);
	uint8_t regaddr = REGADDR(reginfo);
	int ret;
...
static int tda19988_fetch_edid(struct tda1988_dev_s *priv)
{
	...
	if (priv->version == HDMI_CTRL_REV_TDA19988) {
		tda19988_hdmi_modifyreg(priv, HDMI_HDCPOTP_TX4_REG, HDMI_HDCPOTP_TX4_PDRAM, 0);
	}
	...
	if (priv->version == HDMI_CTRL_REV_TDA19988) {
		tda19988_hdmi_modifyreg(priv, HDMI_HDCPOTP_TX4_REG, 0, HDMI_HDCPOTP_TX4_PDRAM);
	}
...
static ssize_t tda19988_read(FAR struct file *filep, FAR char *buffer, size_t len)
{
	FAR struct inode *inode;
	FAR struct tda1988_dev_s *priv;
	ssize_t nread;
	...
	nread = tda19988_read_internal(priv, filep->f_pos, (FAR uint8_t *)buffer, len);
	if (nread > 0) {
		filep->f_pos += nread;
	}
...
static ssize_t tda19988_write(FAR struct file *filep, FAR const char *buffer, size_t len)
{
...
static int tda19988_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case TDA19988_IOC_VIDEOMODE: {
		FAR const struct videomode_s *mode = (FAR const struct videomode_s *)((uintptr_t)arg);
...
static int tda19988_hwinitialize(FAR struct tda1988_dev_s *priv)
{
	...
	tda19988_cec_putreg(priv, CEC_ENAMODS_REG, CEC_ENAMODS_RXSENS | CEC_ENAMODS_HDMI);
	up_udelay(1000);
	...
	tda19988_hdmi_modifyreg(priv, HDMI_CTRL_MAIN_CNTRL0_REG, 0, HDMI_CTRL_MAIN_CNTRL0_SR);
	tda19988_hdmi_modifyreg(priv, HDMI_CTRL_MAIN_CNTRL0_REG, HDMI_CTRL_MAIN_CNTRL0_SR, 0);

	/* PLL registers common configuration */

	tda19988_hdmi_putreg(priv, HDMI_PLL_SERIAL_1_REG, 0x00);
	tda19988_hdmi_putreg(priv, HDMI_PLL_SERIAL_2_REG, HDMI_PLL_SERIAL_2_SRL_NOSC(1));
	...
	tda19988_hdmi_putreg(priv, HDMI_PLL_SEL_CLK_REG,
						 HDMI_PLL_SEL_CLK_SEL_CLK1 | HDMI_PLL_SEL_CLK_ENA_SC_CLK);
									   ...
									   FAR const struct videomode_s *mode)
{
	...
	tda19988_hdmi_modifyreg(priv, HDMI_CTRL_TBG_CNTRL_1_REG, 0, HDMI_CTRL_TBG_CNTRL_1_DWIN_DIS);
	tda19988_hdmi_modifyreg(priv, HDMI_HDCPOTP_TX33_REG, HDMI_HDCPOTP_TX33_HDMI, 0);
	tda19988_hdmi_putreg(priv, HDMI_AUDIO_ENC_CTRL_REG, HDMI_AUDIO_ENC_CNTRL_DVI_MODE);

	/* No pre-filter or interpreter */

	tda19988_hdmi_putreg(priv, HDMI_CTRL_HVF_CNTRL_0_REG,
						 HDMI_CTRL_HVF_CNTRL_0_INTPOL_BYPASS | HDMI_CTRL_HVF_CNTRL_0_PREFIL_NONE);
	tda19988_hdmi_putreg(priv, HDMI_CTRL_VIPCTRL_5_REG, HDMI_CTRL_VIPCTRL_5_SP_CNT(0));
	tda19988_hdmi_putreg(priv, HDMI_CTRL_VIPCTRL_4_REG,
						 HDMI_CTRL_VIPCTRL_4_BLANKIT_NDE | HDMI_CTRL_VIPCTRL_4_BLC_NONE);

	tda19988_hdmi_modifyreg(priv, HDMI_PLL_SERIAL_3_REG, HDMI_PLL_SERIAL_3_SRL_CCIR, 0);
	tda19988_hdmi_modifyreg(priv, HDMI_PLL_SERIAL_1_REG, HDMI_PLL_SERIAL_1_SRL_MAN_IP, 0);
	tda19988_hdmi_modifyreg(priv, HDMI_PLL_SERIAL_3_REG, HDMI_PLL_SERIAL_3_SRL_DE, 0);
	tda19988_hdmi_putreg(priv, HDMI_PLL_SERIALIZER_REG, 0);
	tda19988_hdmi_putreg(priv, HDMI_CTRL_HVF_CNTRL_1_REG, HDMI_CTRL_HVF_CNTRL_1_VQR_FULL);

	tda19988_hdmi_putreg(priv, HDMI_CTRL_RPT_CNTRL_REG, 0);
	tda19988_hdmi_putreg(priv, HDMI_PLL_SEL_CLK_REG,
						 HDMI_PLL_SEL_CLK_SEL_VRF_CLK(0) | HDMI_PLL_SEL_CLK_SEL_CLK1 |
							 HDMI_PLL_SEL_CLK_ENA_SC_CLK);

	tda19988_hdmi_putreg(priv, HDMI_PLL_SERIAL_2_REG,
						 HDMI_PLL_SERIAL_2_SRL_NOSC(div) | HDMI_PLL_SERIAL_2_SRL_PR(0));

	tda19988_hdmi_modifyreg(priv, HDMI_CTRL_MATCTRL_REG, 0, HDMI_CTRL_MAT_CONTRL_MAT_BP);

	tda19988_hdmi_putreg(priv, HDMI_PLL_ANA_GENERAL_REG, 0x09);

	tda19988_hdmi_modifyreg(priv, HDMI_CTRL_TBG_CNTRL_0_REG, HDMI_CTRL_TBG_CNTRL_0_SYNC_MTHD, 0);
	...
	tda19988_hdmi_putreg16(priv, HDMI_CTRL_MUX_VS_LINE_STRT_1_MSB_REG, vs1_line_start);
	...
	tda19988_hdmi_putreg16(priv, HDMI_CTRL_VS_LINE_STRT_2_MSB_REG, vs2_line_start);
	...
	tda19988_hdmi_putreg16(priv, HDMI_CTRL_VWIN_START_2_MSB_REG, vwin2_line_start);
	...
	tda19988_hdmi_modifyreg(priv, HDMI_CTRL_TBG_CNTRL_0_REG, HDMI_CTRL_TBG_CNTRL_0_SYNC_ONCE, 0);
	return OK;
}
...
static void tda19988_shutdown(FAR struct tda1988_dev_s *priv)
{
	...
	{
		DEBUGASSERT(priv->lower->attach != NULL && priv->lower->enable != NULL);
...
TDA19988_HANDLE tda19988_register(FAR const char *devpath, FAR const struct tda19988_lower_s *lower)
{
	FAR struct tda1988_dev_s *priv;
	int ret;
...
int tda19988_videomode(TDA19988_HANDLE handle, FAR const struct videomode_s *mode)
{
...
ssize_t tda19988_read_edid(TDA19988_HANDLE handle, off_t offset, FAR uint8_t *buffer, size_t buflen)
{
	FAR struct tda1988_dev_s *priv = (FAR struct tda1988_dev_s *)handle;
	size_t nread;
	int ret;
