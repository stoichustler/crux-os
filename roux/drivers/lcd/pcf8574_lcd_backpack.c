...
#include <roux/lcd/pcf8574_lcd_backpack.h>
...
static ssize_t pcf8574_lcd_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static void lcd_backlight(FAR struct pcf8574_lcd_dev_s *priv, bool blon)
{
	...
	data = ((blon && priv->cfg.bl_active_high) || (!blon && !priv->cfg.bl_active_high))
			   ? (1 << priv->cfg.bl)
			   : 0;
...
static inline uint8_t rc2addr(FAR struct pcf8574_lcd_dev_s *priv, uint8_t row, uint8_t col)
{
	if (row < 2) {
...
static inline void addr2rc(FAR struct pcf8574_lcd_dev_s *priv, uint8_t addr, FAR uint8_t *row,
						   FAR uint8_t *col)
{
	*row = addr / 0x40;
	*col = addr % 0x40;
...
static inline void lcd_putdata(FAR struct pcf8574_lcd_dev_s *priv, uint8_t data)
{
...
static bool lcd_read_busy_addr(FAR struct pcf8574_lcd_dev_s *priv, FAR uint8_t *addr)
{
...
static void lcd_create_char(FAR struct pcf8574_lcd_dev_s *priv, uint8_t idxchar,
							FAR const uint8_t *chardata)
{
	int nidx;
	uint8_t addr;
...
static void lcd_get_curpos(FAR struct pcf8574_lcd_dev_s *priv, FAR uint8_t *row, FAR uint8_t *col)
{
	uint8_t addr;
...
static void lcd_codec_action(FAR struct pcf8574_lcd_dev_s *priv, enum slcdcode_e code,
							 uint8_t count)
{
	switch (code) {
		/* Erasure */
...
static void lcd_fpos_to_curpos(FAR struct pcf8574_lcd_dev_s *priv, off_t fpos, FAR uint8_t *row,
...
static void lcd_curpos_to_fpos(FAR struct pcf8574_lcd_dev_s *priv, uint8_t row, uint8_t col,
							   FAR off_t *fpos)
{
...
int pcf8574_lcd_backpack_register(FAR const char *devpath, FAR struct i2c_master_s *i2c,
