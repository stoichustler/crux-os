...
#define SSD1351_VPRECHG(n) (100 * (SSD1351_CLAMP(n, 20, 60) - 20) / (100 * (60 - 20) / 31))
...
static void ssd1351_read(FAR struct ssd1351_dev_s *priv, uint8_t cmd, FAR uint8_t *data,
						 size_t datlen);
#endif
static void ssd1351_write(FAR struct ssd1351_dev_s *priv, uint8_t cmd, FAR const uint8_t *data,
						  size_t datlen);

/* LCD Data Transfer Methods */

static int ssd1351_putrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						  FAR const uint8_t *buffer, size_t npixels);
static int ssd1351_getrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						  FAR uint8_t *buffer, size_t npixels);

/* LCD Configuration */

static int ssd1351_getvideoinfo(FAR struct lcd_dev_s *dev, FAR struct fb_videoinfo_s *vinfo);
static int ssd1351_getplaneinfo(FAR struct lcd_dev_s *dev, unsigned int planeno,
...
/* This is the standard, ROUX LCD driver object */
...
static void ssd1351_setcursor(FAR struct ssd1351_dev_s *priv, uint8_t col, uint8_t row)
{
	uint8_t buf[2];
						  ...
						  FAR const uint8_t *buffer, size_t npixels)
{
	...
	DEBUGASSERT(buffer != NULL && ((uintptr_t)buffer & 1) == 0 && col >= 0 &&
				col + npixels <= SSD1351_XRES && row >= 0 && row < SSD1351_YRES);
	...
	ssd1351_write(priv, SSD1351_CMD_RAMWRITE, buffer, SSD1351_PIX2BYTES(npixels));
						  ...
						  FAR uint8_t *buffer, size_t npixels)
{
	...
	ssd1351_read(priv, SSD1351_CMD_RAMREAD, buffer, SSD1351_PIX2BYTES(npixels));
...
static int ssd1351_setcontrast(FAR struct lcd_dev_s *dev, unsigned int contrast)
{
	return -ENOSYS;
}
...
static inline void ssd1351_hwinitialize(FAR struct ssd1351_dev_s *priv)
{
	...
	buf[0] = SSD1351_CLKDIV(CONFIG_SSD1351_CLKDIV) | SSD1351_OSCFREQ(CONFIG_SSD1351_OSCFREQ);
...
#ifdef CONFIG_SSD1351_PARALLEL8BIT
FAR struct lcd_dev_s *ssd1351_initialize(FAR struct ssd1351_lcd_s *lcd, unsigned int devno)
#elif defined(CONFIG_SSD1351_SPI3WIRE) || defined(CONFIG_SSD1351_SPI4WIRE)
FAR struct lcd_dev_s *ssd1351_initialize(FAR struct spi_dev_s *spi, unsigned int devno)
#endif
{
