...
static void gc9a01_cmddata(FAR struct gc9a01_dev_s *dev, uint8_t cmd, const uint8_t *data, int len);
...
static void gc9a01_setarea(FAR struct gc9a01_dev_s *dev, uint16_t x0, uint16_t y0, uint16_t x1,
						   uint16_t y1);
static void gc9a01_bpp(FAR struct gc9a01_dev_s *dev, int bpp);
static void gc9a01_wrram(FAR struct gc9a01_dev_s *dev, FAR const uint8_t *buff, size_t size,
						 size_t skip, size_t count);
#ifndef CONFIG_LCD_NOGETRUN
static void gc9a01_rdram(FAR struct gc9a01_dev_s *dev, FAR uint16_t *buff, size_t size);
#endif
static void gc9a01_fill(FAR struct gc9a01_dev_s *dev, uint16_t color);

/* LCD Data Transfer Methods */

static int gc9a01_putrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						 FAR const uint8_t *buffer, size_t npixels);
static int gc9a01_putarea(FAR struct lcd_dev_s *dev, fb_coord_t row_start, fb_coord_t row_end,
						  fb_coord_t col_start, fb_coord_t col_end, FAR const uint8_t *buffer,
						  fb_coord_t stride);
#ifndef CONFIG_LCD_NOGETRUN
static int gc9a01_getrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						 FAR uint8_t *buffer, size_t npixels);
#endif

/* LCD Configuration */

static int gc9a01_getvideoinfo(FAR struct lcd_dev_s *dev, FAR struct fb_videoinfo_s *vinfo);
static int gc9a01_getplaneinfo(FAR struct lcd_dev_s *dev, unsigned int planeno,
...
static int gc9a01_setcontrast(FAR struct lcd_dev_s *dev, unsigned int contrast);
...
static void gc9a01_init(FAR struct gc9a01_dev_s *dev)
{
	...
	gc9a01_cmddata(dev, 0x70, (const uint8_t *)"\x07\x07\x04\x0E\x0F\x09\x07\x08\x03", 9);
	gc9a01_cmddata(dev, 0xe8, (const uint8_t *)"\x34", 1);
	gc9a01_cmddata(dev, 0x62, (const uint8_t *)"\x18\x0D\x71\xED\x70\x70\x18\x0F\x71\xEF\x70\x70",
				   12);
	gc9a01_cmddata(dev, 0x63, (const uint8_t *)"\x18\x11\x71\xF1\x70\x70\x18\x13\x71\xF3\x70\x70",
				   12);
	gc9a01_cmddata(dev, 0x64, (const uint8_t *)"\x28\x29\xF1\x01\xF1\x00\x07", 7);
	gc9a01_cmddata(dev, 0x66, (const uint8_t *)"\x3C\x00\xCD\x67\x45\x45\x10\x00\x00\x00", 10);
	gc9a01_cmddata(dev, 0x67, (const uint8_t *)"\x00\x3C\x00\x00\x00\x01\x54\x10\x32\x98", 10);
	gc9a01_cmddata(dev, 0x74, (const uint8_t *)"\x10\x85\x80\x00\x00\x4E\x00", 7);
