...
#define ST7565_XSTRIDE                                                                             \
	(ST7565_XRES >> 3) /* Pixels arrange "horizontally                                             \
						* for 'user' */
#define ST7565_YSTRIDE                                                                             \
	(ST7565_YRES >> 3) /* But actual device                                                        \
...
static inline int st7565_send_data_buf(FAR struct st7565_dev_s *priv, FAR const uint8_t *buf,
									   int size);
static inline int st7565_backlight(FAR struct st7565_dev_s *priv, int level);

/* LCD Data Transfer Methods */

static int st7565_putrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						 FAR const uint8_t *buffer, size_t npixels);
static int st7565_getrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						 FAR uint8_t *buffer, size_t npixels);

/* LCD Configuration */

static int st7565_getvideoinfo(FAR struct lcd_dev_s *dev, FAR struct fb_videoinfo_s *vinfo);
...
static inline void up_clear(FAR struct st7565_dev_s *priv)
{
	...
	for (page = 0, i = 0; i < 8; i++) {
		...
		st7565_send_data_buf(priv, &priv->fb[page * ST7565_XRES], ST7565_XRES);
	}
...
FAR struct lcd_dev_s *st7565_initialize(FAR struct st7565_lcd_s *lcd, unsigned int devno)
{
