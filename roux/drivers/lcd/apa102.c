...
#include <roux/lcd/apa102.h>
#include <roux/leds/apa102.h>
...
static int apa102_putrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						 FAR const uint8_t *buffer, size_t npixels);
static int apa102_putarea(FAR struct lcd_dev_s *dev, fb_coord_t row_start, fb_coord_t row_end,
						  fb_coord_t col_start, fb_coord_t col_end, FAR const uint8_t *buffer,
						  fb_coord_t stride);
static int apa102_getrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						 FAR uint8_t *buffer, size_t npixels);

/* LCD Configuration */

static int apa102_getvideoinfo(FAR struct lcd_dev_s *dev, FAR struct fb_videoinfo_s *vinfo);
static int apa102_getplaneinfo(FAR struct lcd_dev_s *dev, unsigned int planeno,
...
static inline void apa102_write32(FAR struct apa102_dev_s *priv, uint32_t value)
{
						 ...
						 FAR const uint8_t *buffer, size_t npixels)
{
	...
	for (i = 0; i < pixlen; i++) {
		...
		if (row % 2 == 0) {
			...
			priv->fb[(row * APA102_XRES) + APA102_XRES - col - i - 1] = rgb565_apa102(*ptr++);
		}
	}
		...
		for (j = col_start; j <= col_end; j++) {
			if (i % 2 == 0) {
				priv->fb[(i * APA102_XRES) + j] =
					rgb565_apa102(*(src + ((i - row_start) * APA102_XRES) + (j - col_start)));
			} else {
...
FAR struct lcd_dev_s *apa102_initialize(FAR struct spi_dev_s *spi, unsigned int devno)
{
