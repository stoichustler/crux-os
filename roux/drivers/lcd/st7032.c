...
#include <roux/lcd/slcd_codec.h>
#include <roux/lcd/slcd_ioctl.h>
#include <roux/i2c/i2c_master.h>
#include <roux/lcd/st7032.h>
...
static inline void st7032_write_inst(FAR struct st7032_dev_s *priv, uint8_t cmd);

static inline void st7032_write_data(FAR struct st7032_dev_s *priv, uint8_t value);

static inline void st7032_setcontrast(FAR struct st7032_dev_s *priv, int8_t contrast);

static void lcd_scroll_up(FAR struct st7032_dev_s *priv);

/* Character driver methods */

static ssize_t st7032_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t st7032_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static off_t st7032_seek(FAR struct file *filep, off_t offset, int whence);
static int st7032_ioctl(FAR struct file *filep, int cmd, unsigned long arg);
...
static inline void st7032_setcontrast(FAR struct st7032_dev_s *priv, int8_t contrast)
{
	...
	st7032_write_inst(priv,
					  (contrast >> 4) | ST7032_POWER_ICON_CTRL_SET | POWER_ICON_BOST_CTRL_BON);
}
...
static inline void addr2rc(FAR struct st7032_dev_s *priv, uint8_t addr, FAR uint8_t *row,
						   FAR uint8_t *col)
{
	*row = addr / 0x40;
	*col = addr % 0x40;
...
static void lcd_codec_action(FAR struct st7032_dev_s *priv, enum slcdcode_e code, uint8_t count)
{
	switch (code) {
		/* Erasure */
	...
	case SLCDCODE_BLINKSTART: /* Start blinking with current cursor position */
		st7032_write_inst(priv, ST7032_DISPLAY_ON_OFF | DISPLAY_ON_OFF_D | DISPLAY_ON_OFF_C |
									DISPLAY_ON_OFF_B);
		break;
...
static void lcd_init(FAR struct st7032_dev_s *priv)
{
	...
	data = ST7032_FUNCTION_SET | FUNCTION_SET_DL | FUNCTION_SET_N | FUNCTION_SET_IS;
	...
	data = ST7032_DISPLAY_ON_OFF | DISPLAY_ON_OFF_D | DISPLAY_ON_OFF_C | DISPLAY_ON_OFF_B;
...
static void lcd_curpos_to_fpos(FAR struct st7032_dev_s *priv, uint8_t row, uint8_t col,
							   FAR off_t *fpos)
{
...
static ssize_t st7032_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	while ((result = slcd_decode(&instream.common, &state, &ch, &count)) != SLCDRET_EOF) {
