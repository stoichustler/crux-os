...
static inline void ili9225_sendcmd(FAR struct ili9225_dev_s *dev, uint16_t cmd);
static inline void ili9225_writereg(FAR struct ili9225_dev_s *dev, uint16_t reg, uint16_t data);
static void ili9225_display(FAR struct ili9225_dev_s *dev, bool on);
static void ili9225_setarea(FAR struct ili9225_dev_s *dev, uint16_t x0, uint16_t y0, uint16_t x1,
							uint16_t y1);
static void ili9225_bpp(FAR struct ili9225_dev_s *dev, int bpp);
static void ili9225_wrram(FAR struct ili9225_dev_s *dev, FAR const uint16_t *buff, size_t size);
#ifndef CONFIG_LCD_NOGETRUN
static void ili9225_rdram(FAR struct ili9225_dev_s *dev, FAR uint16_t *buff, size_t size);
#endif
static void ili9225_fill(FAR struct ili9225_dev_s *dev, uint16_t color);

/* LCD Data Transfer Methods */

static int ili9225_putrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						  FAR const uint8_t *buffer, size_t npixels);
#ifndef CONFIG_LCD_NOGETRUN
static int ili9225_getrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						  FAR uint8_t *buffer, size_t npixels);
#endif

/* LCD Configuration */

static int ili9225_getvideoinfo(FAR struct lcd_dev_s *dev, FAR struct fb_videoinfo_s *vinfo);
static int ili9225_getplaneinfo(FAR struct lcd_dev_s *dev, unsigned int planeno,
...
static void ili9225_display(FAR struct ili9225_dev_s *dev, bool on)
{
	if (!on) {
		...
		ili9225_writereg(dev, ILI9225_DISP_CTRL1, ILI9225_DISP_CTRL1_GON | ILI9225_DISP_CTRL1_D(2));
		up_mdelay(20);
		...
		ili9225_writereg(dev, ILI9225_POWER_CTRL2,
						 ILI9225_POWER_CTRL2_VC(8) | ILI9225_POWER_CTRL2_VC_EN);
		ili9225_writereg(dev, ILI9225_POWER_CTRL3,
						 ILI9225_POWER_CTRL3_DC3(1) | ILI9225_POWER_CTRL3_DC2(2) |
							 ILI9225_POWER_CTRL3_DC1(1) | ILI9225_POWER_CTRL3_BT(6));
		ili9225_writereg(dev, ILI9225_POWER_CTRL4, ILI9225_POWER_CTRL4_GVD(111));
		ili9225_writereg(dev, ILI9225_POWER_CTRL5,
						 ILI9225_POWER_CTRL5_VML(95) | ILI9225_POWER_CTRL5_VCM(9) |
							 ILI9225_POWER_CTRL5_VCOMG);
		ili9225_writereg(dev, ILI9225_POWER_CTRL1, ILI9225_POWER_CTRL1_SAP(8));
		ili9225_writereg(dev, ILI9225_POWER_CTRL2,
						 ILI9225_POWER_CTRL2_VC(11) | ILI9225_POWER_CTRL2_VC_EN |
							 ILI9225_POWER_CTRL2_AON | ILI9225_POWER_CTRL2_APON);
		ili9225_writereg(dev, ILI9225_DRIVER_OUTPUT_CTRL,
						 ILI9225_DRIVER_OUTPUT_CTRL_NL(28) | ILI9225_DRIVER_OUTPUT_CTRL_SS);
		ili9225_writereg(dev, ILI9225_LCD_DRIVING_CTRL, ILI9225_LCD_DRIVING_CTRL_INV0);
		ili9225_writereg(dev, ILI9225_ENTRY_MODE,
						 ILI9225_ENTRY_MODE_AM | ILI9225_ENTRY_MODE_ID(3) | ILI9225_ENTRY_MODE_BGR);
		ili9225_writereg(dev, ILI9225_DISP_CTRL2,
						 ILI9225_DISP_CTRL2_BP(8) | ILI9225_DISP_CTRL2_FP(8));
		ili9225_writereg(dev, ILI9225_FRAME_CYCLE_CTRL,
						 ILI9225_FRAME_CYCLE_CTRL_STD(1) | ILI9225_FRAME_CYCLE_CTRL_NO(1));
		ili9225_writereg(dev, ILI9225_RGB_DISP_INT_CTRL1, ILI9225_RGB_DISP_INT_CTRL1_RIM(1));
		ili9225_writereg(dev, ILI9225_OSC_CTRL, ILI9225_OSC_CTRL_EN | ILI9225_OSC_CTRL_FOSC(13));
		ili9225_writereg(dev, ILI9225_VCI_REC, ILI9225_VCI_REC_VCIR(2));

		ili9225_writereg(dev, ILI9225_VER_SCROLL_CTRL1, ILI9225_VER_SCROLL_CTRL1_SEA(219));
		ili9225_writereg(dev, ILI9225_PART_SCR_DRIV_POS1, ILI9225_PART_SCR_DRIV_POS1_SE(219));
		ili9225_writereg(dev, ILI9225_HORIZONTAL_ADDR_END, ILI9225_HORIZONTAL_ADDR_END_HEA(175));
		ili9225_writereg(dev, ILI9225_VERTICAL_ADDR_END, ILI9225_VERTICAL_ADDR_END_VEA(219));

		ili9225_writereg(dev, ILI9225_GAMMA_CTRL2,
						 ILI9225_GAMMA_CTRL2_KP2(8) | ILI9225_GAMMA_CTRL2_KP3(8));
		ili9225_writereg(dev, ILI9225_GAMMA_CTRL3,
						 ILI9225_GAMMA_CTRL3_KP4(10) | ILI9225_GAMMA_CTRL3_KP5(8));
		ili9225_writereg(dev, ILI9225_GAMMA_CTRL4, ILI9225_GAMMA_CTRL4_RP0(10));
		ili9225_writereg(dev, ILI9225_GAMMA_CTRL5,
						 ILI9225_GAMMA_CTRL5_KN0(8) | ILI9225_GAMMA_CTRL5_KN1(10));
		ili9225_writereg(dev, ILI9225_GAMMA_CTRL6,
						 ILI9225_GAMMA_CTRL6_KN2(8) | ILI9225_GAMMA_CTRL6_KN3(8));
		ili9225_writereg(dev, ILI9225_GAMMA_CTRL8, ILI9225_GAMMA_CTRL8_RN1(10));
		ili9225_writereg(dev, ILI9225_GAMMA_CTRL9,
						 ILI9225_GAMMA_CTRL9_VRP0(16) | ILI9225_GAMMA_CTRL9_VRP1(7));
		ili9225_writereg(dev, ILI9225_GAMMA_CTRL10,
						 ILI9225_GAMMA_CTRL10_VRN0(10) | ILI9225_GAMMA_CTRL10_VRN1(7));
		ili9225_writereg(dev, ILI9225_DISP_CTRL1, ILI9225_DISP_CTRL1_D(2) | ILI9225_DISP_CTRL1_GON);

		ili9225_writereg(dev, ILI9225_DISP_CTRL1,
						 ILI9225_DISP_CTRL1_D(3) | ILI9225_DISP_CTRL1_REV | ILI9225_DISP_CTRL1_GON |
							...
							uint16_t y1)
{
	...
	ili9225_writereg(dev, ILI9225_HORIZONTAL_ADDR_START, ILI9225_HORIZONTAL_ADDR_START_HSA(x0));
	ili9225_writereg(dev, ILI9225_HORIZONTAL_ADDR_END, ILI9225_HORIZONTAL_ADDR_END_HEA(x1));

	/* Set column address */

	ili9225_writereg(dev, ILI9225_VERTICAL_ADDR_START, ILI9225_VERTICAL_ADDR_START_VSA(y0));
	ili9225_writereg(dev, ILI9225_VERTICAL_ADDR_END, ILI9225_VERTICAL_ADDR_END_VEA(y1));
}
