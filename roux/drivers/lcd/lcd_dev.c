...
static int lcddev_open(FAR struct file *filep)
{
	...
	if (priv->crefs == 0) {
		if (priv->lcd_ptr->open != NULL && (ret = priv->lcd_ptr->open(priv->lcd_ptr)) < 0) {
			goto err_lcd;
		}
	}
...
static int lcddev_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case LCDDEVIO_GETRUN: {
		...
		ret = priv->planeinfo.getrun(priv->lcd_ptr, lcd_run->row, lcd_run->col, lcd_run->data,
									 lcd_run->npixels);
	} break;
	case LCDDEVIO_PUTRUN: {
		FAR const struct lcddev_run_s *lcd_run = (FAR const struct lcddev_run_s *)arg;

		ret = priv->planeinfo.putrun(priv->lcd_ptr, lcd_run->row, lcd_run->col, lcd_run->data,
									 lcd_run->npixels);
	} break;
	case LCDDEVIO_GETAREA: {
		...
		size_t pixel_size = priv->planeinfo.bpp > 1 ? priv->planeinfo.bpp >> 3 : 1;
		size_t row_size = lcd_area->stride > 0 ? lcd_area->stride : cols * pixel_size;

		if (priv->planeinfo.getarea) {
			ret = priv->planeinfo.getarea(priv->lcd_ptr, lcd_area->row_start, lcd_area->row_end,
										  lcd_area->col_start, lcd_area->col_end, lcd_area->data,
										  row_size);
		} else {
			...
			for (row = lcd_area->row_start; row <= lcd_area->row_end; row++) {
				ret = priv->planeinfo.getrun(priv->lcd_ptr, row, lcd_area->col_start, buf, cols);
				if (ret < 0) {
					break;
				}
	...
	case LCDDEVIO_PUTAREA: {
		...
		if (priv->planeinfo.putarea) {
			ret = priv->planeinfo.putarea(priv->lcd_ptr, lcd_area->row_start, lcd_area->row_end,
										  lcd_area->col_start, lcd_area->col_end, lcd_area->data,
										  row_size);
		} else {
			...
			for (row = lcd_area->row_start; row <= lcd_area->row_end; row++) {
				ret = priv->planeinfo.putrun(priv->lcd_ptr, row, lcd_area->col_start, buf, cols);
				if (ret < 0) {
					break;
				}
	...
	case LCDDEVIO_GETVIDEOINFO: {
		ret = priv->lcd_ptr->getvideoinfo(priv->lcd_ptr, (FAR struct fb_videoinfo_s *)arg);
	} break;
	case LCDDEVIO_SETPLANENO: {
		ret = priv->lcd_ptr->getplaneinfo(priv->lcd_ptr, (int)arg, &priv->planeinfo);
	} break;
	...
	case LCDDEVIO_GETCURSOR: {
		FAR struct fb_cursorattrib_s *attrib = (FAR struct fb_cursorattrib_s *)arg;

		ret = priv->lcd_ptr->getcursor(priv->lcd_ptr, attrib);
	} break;
	case LCDDEVIO_SETCURSOR: {
		FAR struct fb_setcursor_s *settings = (FAR struct fb_setcursor_s *)arg;
	...
	case LCDDEVIO_GETAREAALIGN: {
		FAR struct lcddev_area_align_s *area_align = (FAR struct lcddev_area_align_s *)arg;
