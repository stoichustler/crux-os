...
static inline void ht16k33_write_cmd(FAR struct ht16k33_dev_s *priv, int dev_id, uint8_t cmd);

static inline void ht16k33_write_data(FAR struct ht16k33_dev_s *priv, int dev_id, uint8_t cmd,
									  uint8_t *values, int nbytes);

static inline void ht16k33_setcontrast(FAR struct ht16k33_dev_s *priv, int dev_id, int8_t contrast);
...
static ssize_t ht16k33_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static inline void addr2rc(FAR struct ht16k33_dev_s *priv, uint8_t addr, FAR uint8_t *row,
						   FAR uint8_t *col)
{
	*row = 0;
	*col = addr / 2;
...
static void lcd_codec_action(FAR struct ht16k33_dev_s *priv, enum slcdcode_e code, uint8_t count)
{
	switch (code) {
		/* Erasure */
	...
	case SLCDCODE_BLINKSTART: /* Start blinking with current cursor
							   * position
							   */
		ht16k33_write_cmd(priv, 0, HT16K33_DISPLAY_SETUP | DISPLAY_SETUP_BLINK_2HZ);
		break;
	...
	case SLCDCODE_BLINKOFF: /* Turn blinking off */
		ht16k33_write_cmd(priv, 0, HT16K33_DISPLAY_SETUP | DISPLAY_SETUP_BLINK_OFF);
...
static void lcd_curpos_to_fpos(FAR struct ht16k33_dev_s *priv, uint8_t row, uint8_t col,
							   FAR off_t *fpos)
{
...
int ht16k33_register(int devno, FAR struct i2c_master_s *i2c)
{
	...
	priv = (FAR struct ht16k33_dev_s *)kmm_malloc(sizeof(struct ht16k33_dev_s));
	if (!priv) {
