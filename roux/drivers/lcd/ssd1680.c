...
#include <roux/lcd/lcd.h>
#include <roux/lcd/ssd1680.h>
...
static FAR void *bitscpy_ds(FAR void *dest, int dest_offset, FAR const void *src, size_t nbits);

static FAR void *bitscpy_ss(FAR void *dest, FAR const void *src, int src_offset, size_t nbits);

/* LCD Data Transfer Methods */

static int ssd1680_busy_wait(FAR struct ssd1680_dev_s *priv);

static void ssd1680_snd_cmd_with_data0(FAR struct ssd1680_dev_s *priv, uint8_t cmd);

static void ssd1680_snd_cmd_with_data1(FAR struct ssd1680_dev_s *priv, uint8_t cmd, uint8_t dta1);

static void ssd1680_snd_cmd_with_data2(FAR struct ssd1680_dev_s *priv, uint8_t cmd, uint8_t dta1,
									   uint8_t dta2);

static void ssd1680_snd_cmd_with_data3(FAR struct ssd1680_dev_s *priv, uint8_t cmd, uint8_t dta1,
									   uint8_t dta2, uint8_t dta3);

static void ssd1680_snd_cmd_with_data4(FAR struct ssd1680_dev_s *priv, uint8_t cmd, uint8_t dta1,
									   uint8_t dta2, uint8_t dta3, uint8_t dta4);

#ifndef CONFIG_LCD_SSD1681_1_54
static void ssd1680_snd_cmd_with_data(FAR struct ssd1680_dev_s *priv, uint8_t cmd,
									  const uint8_t *dta, int dta_len);
#endif

#if !defined(CONFIG_LCD_PORTRAIT) && !defined(CONFIG_LCD_RPORTRAIT)
#if SSD1680_DEV_BPP == 1
static void ssd1680_snd_cmd_with_data_bitstrip(FAR struct ssd1680_dev_s *priv, uint8_t cmd,
											   const uint8_t *dta, int dta_len, int strip_len);
#else
...
static int ssd1680_putrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						  FAR const uint8_t *buffer, size_t npixels);

static int ssd1680_getrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
...
static int ssd1680_getvideoinfo(FAR struct lcd_dev_s *dev, FAR struct fb_videoinfo_s *vinfo);
static int ssd1680_getplaneinfo(FAR struct lcd_dev_s *dev, unsigned int planeno,
...
/* This is the standard, ROUX Plane information object */
						  ...
						  FAR const uint8_t *buffer, size_t npixels)
{
	...
	uint8_t *dst = priv->shadow_fb + row * SSD1680_DEV_ROWSIZE + (col >> SSD1680_PDF);
						  ...
						  FAR uint8_t *buffer, size_t npixels)
{
	...
	bitscpy_ss(buffer, priv->shadow_fb + row * SSD1680_DEV_FBSIZE + (col >> SSD1680_PDF),
			   col % SSD1680_PDV, npixels);
...
static int ssd1680_configuredisplay(struct ssd1680_dev_s *priv)
{
	...
	lcdinfo("Set boost soft start (0x%02x): 0x%02x, 0x%02x, 0x%02x\n", SSD1680_BOOST_SOFTSTART,
			0xd7, 0xd6, 0x9d);
	ssd1680_snd_cmd_with_data3(priv, SSD1680_BOOST_SOFTSTART, 0xd7, 0xd6, 0x9d);

	/* Step 3: Vcom Voltage SSD1680_WRITE_VCOM */

	lcdinfo("Set Vcom voltage (0x%02x): 0x%02x\n", SSD1680_WRITE_VCOM, SSD1680_VALUE_VCOM);
	ssd1680_snd_cmd_with_data1(priv, SSD1680_WRITE_VCOM, SSD1680_VALUE_VCOM);

	lcdinfo("Set Gate voltage (0x%02x): 0x%02x\n", SSD1680_GATE_VOLTAGE, SSD1680_VALUE_G_VOLTAGE);
	ssd1680_snd_cmd_with_data1(priv, SSD1680_GATE_VOLTAGE, SSD1680_VALUE_G_VOLTAGE);

	lcdinfo("Set Source voltage (0x%02x): 0x%02x, 0x%02x, 0x%02x\n", SSD1680_SOURCE_VOLTAGE,
			SSD1680_VALUE_S_VOLTAGE_1, SSD1680_VALUE_S_VOLTAGE_2, SSD1680_VALUE_S_VOLTAGE_3);
	ssd1680_snd_cmd_with_data3(priv, SSD1680_SOURCE_VOLTAGE, SSD1680_VALUE_S_VOLTAGE_1,
							   SSD1680_VALUE_S_VOLTAGE_2, SSD1680_VALUE_S_VOLTAGE_3);

	/* Step 4: Sending undocumented command: 0x3a with data 1A */

	lcdinfo("Set dummy line (0x%02x): 0x%02x\n", SSD1680_DUMMY_LINE, SSD1680_VALUE_DUMMY_LINE);
	ssd1680_snd_cmd_with_data1(priv, SSD1680_DUMMY_LINE, SSD1680_VALUE_DUMMY_LINE);

	/* Step 5: Sending undocumented command: 0x3b with data 08 */

	lcdinfo("Set gate time (0x%02x): 0x%02x\n", SSD1680_GATE_TIME, SSD1680_VALUE_GATE_TIME);
	ssd1680_snd_cmd_with_data1(priv, SSD1680_GATE_TIME, SSD1680_VALUE_GATE_TIME);
#endif
	...
	ssd1680_snd_cmd_with_data1(priv, SSD1680_DATA_MODE, SSD1680_VAL_DATA_MODE);

	/* Step 7: Set ram X start/end position 00 FF */

	lcdinfo("Set ram X start/end position (0x%02x): 0, %d\n", SSD1680_SET_RAMXPOS,
			(SSD1680_DEV_X_ROUND_UP >> 3) - 1);
	ssd1680_snd_cmd_with_data2(priv, SSD1680_SET_RAMXPOS, 0x00, (SSD1680_DEV_X_ROUND_UP >> 3) - 1);

	/* Step 8: Set ram Y start/end position */

	lcdinfo("Set ram Y start/end position (0x%02x): 0, %d\n", SSD1680_SET_RAMYPOS,
			SSD1680_DEV_NATIVE_YRES - 1);
	ssd1680_snd_cmd_with_data4(priv, SSD1680_SET_RAMYPOS, 0x00, 0x00,
							   (uint8_t)((SSD1680_DEV_NATIVE_YRES - 1) & 0xff),
...
#ifndef CONFIG_LCD_SSD1681_1_54
	lcdinfo("Write lookup table (0x%02x): (%d bytes)\n", SSD1680_WRITE_LUT, sizeof(ssd1680_lut));
	ssd1680_snd_cmd_with_data(priv, SSD1680_WRITE_LUT, ssd1680_lut, sizeof(ssd1680_lut));
#endif
	...
	ssd1680_snd_cmd_with_data1(priv, SSD1680_DISP_CTRL2, SSD1680_VALUE_UPDATE_CTRL);
...
#if SSD1680_DEV_BPP == 1
	ssd1680_snd_cmd_with_data(priv, SSD1680_WRITE_RAM1, src, SSD1680_DEV_ROWSIZE);
#else
	ssd1680_snd_cmd_with_data_even_bits(priv, SSD1680_WRITE_RAM1, src, SSD1680_DEV_ROWSIZE);

	ssd1680_snd_cmd_with_data1(priv, SSD1680_SET_RAMXCOUNT, 1);
	ssd1680_snd_cmd_with_data2(priv, SSD1680_SET_RAMYCOUNT, row, row >> 8);
	ssd1680_snd_cmd_with_data_odd_bits(priv, SSD1680_WRITE_RAM2, src, SSD1680_DEV_FBSIZE);
#endif
...
#if SSD1680_DEV_BPP == 1
	ssd1680_snd_cmd_with_data_bitstrip(priv, SSD1680_WRITE_RAM1, src, SSD1680_DEV_NATIVE_YRES,
									   SSD1680_DEV_ROWSIZE);
#else
...
static void ssd1680_snd_cmd_with_data_bitstrip(FAR struct ssd1680_dev_s *priv, uint8_t cmd,
											   const uint8_t *dta, int nopix, int strip_len)
#if SSD1680_DEV_BPP == 1
{
	int i;
	int j;
	...
	const int offset = (SSD1680_DEV_ROWSIZE * SSD1680_PDV - SSD1680_DEV_NATIVE_YRES) % SSD1680_PDV;
	...
	while (nopix > 0) {
		for (j = 0; j < 8; j++) {
			...
			if (offset == 0) {
				...
				bytes[j] |= ((*(dta + j * strip_len - 1)) << (8 - offset) & (0xff << (8 - offset)));
			}
#else
			bytes[j] = *(dta + j * strip_len);
#endif
...
#else
{
	...
	while (nopix > 0) {
		for (j = 0; j < 8; j++) {
#if defined(CONFIG_LCD_LANDSCAPE)
			tmp = *(dta + j * strip_len) + ((*(dta + j * strip_len + 1)) << 8);
#else
			tmp = *(dta + j * strip_len + 1) + ((*(dta + j * strip_len)) << 8);
#endif
			rows[j] = (tmp & 0x01) | ((tmp >> 1) & 0x02) | ((tmp >> 2) & 0x04) |
					  ((tmp >> 3) & 0x08) | ((tmp >> 4) & 0x10) | ((tmp >> 5) & 0x20) |
...
static void ssd1680_snd_cmd_with_data_odd_bits_bitstrip(FAR struct ssd1680_dev_s *priv, uint8_t cmd,
														const uint8_t *dta, int nopix,
														int strip_len)
{
	int i;
	int j;
	...
	while (nopix > 0) {
		for (j = 0; j < 8; j++) {
#if defined(CONFIG_LCD_LANDSCAPE)
			rows[j] = *(dta + j * strip_len) + ((*(dta + j * strip_len + 1)) << 8);
#else
			rows[j] = *(dta + j * strip_len + 1) + ((*(dta + j * strip_len)) << 8);
#endif
		}
...
static void ssd1680_snd_cmd_with_data_even_bits(FAR struct ssd1680_dev_s *priv, uint8_t cmd,
												const uint8_t *dta, int dta_len)
{
	int i;
	uint8_t dta_byte;
	...
	for (i = 0; i < dta_len; i++) {
		src1 = *(dta++);
		src2 = *(dta++);
		dta_byte = (src1 & 0x01) | ((src1 >> 1) & 0x02) | ((src1 >> 2) & 0x04) |
				   ((src1 >> 3) & 0x08) | ((src2 << 4) & 0x10) | ((src2 << 3) & 0x20) |
...
static void ssd1680_snd_cmd_with_data_odd_bits(FAR struct ssd1680_dev_s *priv, uint8_t cmd,
											   const uint8_t *dta, int dta_len)
{
	int i;
	uint8_t dta_byte;
	...
	for (i = 0; i < dta_len; i++) {
		src1 = *(dta++);
		src2 = *(dta++);
		dta_byte = ((src1 >> 1 & 0x01)) | ((src1 >> 2) & 0x02) | ((src1 >> 3) & 0x04) |
				   ((src1 >> 4) & 0x08) | ((src2 << 3) & 0x10) | ((src2 << 2) & 0x20) |
										 ...
										 FAR const struct ssd1680_priv_s *board_priv)
{
	...
	memset(priv->shadow_fb, SSD1680_Y1_BLACK & 1 ? 0xff : 0x00, SSD1680_DEV_FBSIZE);
...
static FAR void *bitscpy_ss(FAR void *dest, FAR const void *src, int src_offset, size_t nbits)
{
	...
	if (nbits > 0) {
		...
		if (nbits + src_offset > 8) {
			pin++;
			val |= ((*pin & (~(0xff >> (nbits + src_offset - 8)) >> (8 - src_offset))));
		}
