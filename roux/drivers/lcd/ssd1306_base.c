...
static int ssd1306_putrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						  FAR const uint8_t *buffer, size_t npixels);
static int ssd1306_getrun(FAR struct lcd_dev_s *dev, fb_coord_t row, fb_coord_t col,
						  FAR uint8_t *buffer, size_t npixels);

/* LCD Configuration */

static int ssd1306_getvideoinfo(FAR struct lcd_dev_s *dev, FAR struct fb_videoinfo_s *vinfo);
...
static int ssd1306_setcontrast(struct lcd_dev_s *dev, unsigned int contrast)
{
	...
	ret =
		ssd1306_sendbyte(priv, SSD1306_CONTRAST(scaled)); /* Data 1: Set 1 of 256 contrast steps */
	if (ret < 0) {
		return ret;
	}
...
static int ssd1306_redrawfb(struct ssd1306_dev_s *priv)
{
	...
	for (page = 0; page < SSD1306_DEV_PAGES; page++) {
		...
		ret = ssd1306_sendblk(priv, &priv->fb[page * SSD1306_DEV_XRES], SSD1306_DEV_XRES);
		if (ret < 0) {
			return ret;
		}
	}
