...
static int lcdfb_updateearea(FAR struct fb_vtable_s *vtable, FAR const struct fb_area_s *area);

/* Get information about the video controller configuration and the
 * configuration of each color plane.
 */

static int lcdfb_getvideoinfo(FAR struct fb_vtable_s *vtable, FAR struct fb_videoinfo_s *vinfo);
...
static int lcdfb_getcmap(FAR struct fb_vtable_s *vtable, FAR struct fb_cmap_s *cmap);
static int lcdfb_putcmap(FAR struct fb_vtable_s *vtable, FAR const struct fb_cmap_s *cmap);
#endif
...
static int lcdfb_getcursor(FAR struct fb_vtable_s *vtable, FAR struct fb_cursorattrib_s *attrib);
static int lcdfb_setcursor(FAR struct fb_vtable_s *vtable, FAR struct fb_setcursor_s *settings);
#endif
...
static int lcdfb_updateearea(FAR struct fb_vtable_s *vtable, FAR const struct fb_area_s *area)
{
	...
	if (pinfo->putarea != NULL) {
		...
		ret = pinfo->putarea(pinfo->dev, starty, endy, startx, endx, run, priv->stride);
		if (ret < 0) {
...
static int lcdfb_getcmap(FAR struct fb_vtable_s *vtable, FAR struct fb_cmap_s *cmap)
{
	...
	if (priv != NULL && cmap != NULL) {
		...
		lcd = priv->lcd DEBUGASSERT(lcd->getcmap != NULL);
...
static int lcdfb_putcmap(FAR struct fb_vtable_s *vtable, FAR const struct fb_cmap_s *cmap)
{
	...
	if (priv != NULL && cmap != NULL) {
		...
		lcd = priv->lcd DEBUGASSERT(lcd->putcmap != NULL);
...
static int lcdfb_getcursor(FAR struct fb_vtable_s *vtable, FAR struct fb_cursorattrib_s *attrib)
{
	...
	if (priv != NULL && attrib != NULL) {
		...
		lcd = priv->lcd DEBUGASSERT(lcd->getcursor != NULL);
...
static int lcdfb_setcursor(FAR struct fb_vtable_s *vtable, FAR struct fb_setcursor_s *settings)
{
	...
	if (priv != NULL && settings != NULL) {
		...
		lcd = priv->lcd DEBUGASSERT(lcd->setcursor != NULL);
...
static int lcdfb_ioctl(FAR struct fb_vtable_s *vtable, int cmd, unsigned long arg)
{
	int ret = -EINVAL;
	FAR struct lcdfb_dev_s *priv;
	FAR struct lcd_dev_s *lcd;
...
int up_fbinitialize(int display)
{
	...
	priv->vtable.getvideoinfo = lcdfb_getvideoinfo, priv->vtable.getplaneinfo = lcdfb_getplaneinfo,
#ifdef CONFIG_FB_CMAP
	priv->vtable.getcmap = lcdfb_getcmap, priv->vtable.putcmap = lcdfb_putcmap,
#endif
#ifdef CONFIG_FB_HWCURSOR
	priv->vtable.getcursor = lcdfb_getcursor, priv->vtable.setcursor = lcdfb_setcursor,
#endif
	priv->vtable.updatearea = lcdfb_updateearea, priv->vtable.setpower = lcdfb_setpower,
	priv->vtable.ioctl = lcdfb_ioctl, priv->vtable.open = lcdfb_open,
...
void up_fbuninitialize(int display)
{
	...
	for (prev = NULL, priv = g_lcdfb; priv != NULL; prev = priv, priv = priv->flink) {
		if (priv->display == display) {
