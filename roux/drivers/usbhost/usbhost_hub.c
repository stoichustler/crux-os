...
static inline int usbhost_hubpwr(FAR struct usbhost_hubpriv_s *priv,
								 FAR struct usbhost_hubport_s *hport, bool on);
...
static int usbhost_connect(FAR struct usbhost_class_s *hubclass, FAR const uint8_t *configdesc,
						   int desclen);
			...
			if ((epdesc->attr & USB_EP_ATTR_XFERTYPE_MASK) == USB_EP_ATTR_XFER_INT) {
				...
				if (USB_ISEPOUT(epdesc->addr)) {
					...
					uinfo(" addr=%d interval=%d mxpacketsize=%d\n", intindesc.addr,
						  intindesc.interval, intindesc.mxpacketsize);
				}
			}
		} break;
...
static int usbhost_hubpwr(FAR struct usbhost_hubpriv_s *priv, FAR struct usbhost_hubport_s *hport,
						  bool on)
{
	FAR struct usb_ctrlreq_s *ctrlreq;
	uint16_t req;
...
static void usbhost_hub_event(FAR void *arg)
{
	...
	if (!priv->disconnected) {
		...
		ret = DRVR_ASYNCH(hport->drvr, priv->intin, (FAR uint8_t *)priv->buffer, INTIN_BUFSIZE,
...
static void usbhost_callback(FAR void *arg, ssize_t nbytes)
{
	...
	if (work_available(&priv->work) && !priv->disconnected) {
		work_queue(LPWORK, &priv->work, usbhost_hub_event, hubclass, delay);
	}
}
...
static int usbhost_disconnected(struct usbhost_class_s *hubclass)
{
	...
	ret = work_queue(LPWORK, &priv->work, usbhost_disconnect_event, hubclass, 0);
	leave_critical_section(flags);
	return ret;
}
