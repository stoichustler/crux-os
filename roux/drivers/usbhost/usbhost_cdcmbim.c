...
static inline void usbhost_mkdevname(FAR struct usbhost_cdcmbim_s *priv, FAR char *devname);
...
static int usbhost_cfgdesc(FAR struct usbhost_cdcmbim_s *priv, FAR const uint8_t *configdesc,
						   int desclen);
...
static ssize_t cdcwdm_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t cdcwdm_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int cdcwdm_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);

/* ROUX network callback functions */
...
static void cdcmbim_receive(FAR struct usbhost_cdcmbim_s *priv, FAR uint8_t *buf, size_t len);
...
static int usbhost_ctrl_cmd(FAR struct usbhost_cdcmbim_s *priv, uint8_t type, uint8_t req,
							uint16_t value, uint16_t iface, FAR uint8_t *payload, uint16_t len)
{
...
static ssize_t usbhost_readmessage(FAR struct usbhost_cdcmbim_s *priv, FAR char *buffer,
								   size_t buflen)
{
	irqstate_t flags;
	ssize_t ret = -EAGAIN;
...
static ssize_t cdcwdm_read(FAR struct file *filep, FAR char *buffer, size_t buflen)
{
	...
	if (priv->comm_rxlen == 0 && priv->comm_rxmsgs > 0) {
		...
		if (work_available(&priv->comm_rxwork)) {
			work_queue(LPWORK, &priv->comm_rxwork, (worker_t)usbhost_rxdata_work, priv, 0);
		}
	}
...
static ssize_t cdcwdm_write(FAR struct file *filep, FAR const char *buffer, size_t buflen)
{
	...
	ret = usbhost_ctrl_cmd(priv, USB_REQ_DIR_OUT | USB_REQ_TYPE_CLASS | USB_REQ_RECIPIENT_INTERFACE,
						   USB_CDC_SEND_ENCAPSULATED_COMMAND, 0, priv->ctrlif,
...
static void usbhost_bulkin_work(FAR void *arg)
{
	...
	if (priv->bulkinbytes <
		(int16_t)(sizeof(struct usb_cdc_ncm_nth16_s) + sizeof(struct usb_cdc_ncm_ndp16_s))) {
		goto out;
	}
...
out:
	DRVR_ASYNCH(hport->drvr, priv->bulkin, (FAR uint8_t *)priv->rxnetbuf, CDCMBIM_NETBUF_SIZE,
...
static void usbhost_rxdata_work(FAR void *arg)
{
	...
	ret = usbhost_ctrl_cmd(priv, USB_REQ_DIR_IN | USB_REQ_TYPE_CLASS | USB_REQ_RECIPIENT_INTERFACE,
						   USB_CDC_GET_ENCAPSULATED_RESPONSE, 0, priv->ctrlif, priv->comm_rxbuf,
						   priv->maxctrlsize);
	if (ret) {
...
static void usbhost_notification_work(FAR void *arg)
{
	...
	if (!priv->disconnected && priv->intin) {
		...
		if (priv->nnbytes >= 0) {
			...
			if ((inmsg->type ==
				...
				if (priv->comm_rxmsgs == 1) {
					if (work_available(&priv->comm_rxwork)) {
						work_queue(LPWORK, &priv->comm_rxwork, usbhost_rxdata_work, priv, 0);
					}
				}
			}
		}
		...
		ret = DRVR_ASYNCH(hport->drvr, priv->intin, (FAR uint8_t *)priv->notification,
						  SIZEOF_NOTIFICATION_S(0), usbhost_notification_callback, priv);
		if (ret < 0) {
						   ...
						   int desclen)
{
	...
	DEBUGASSERT(priv != NULL && priv->usbclass.hport && configdesc != NULL &&
	...
	while (remaining >= sizeof(struct usb_desc_s)) {
		...
		case USB_DESC_TYPE_CSINTERFACE: {
			FAR struct usb_csifdesc_s *csdesc = (FAR struct usb_csifdesc_s *)desc;

			/* MBIM functional descriptor */

			if (csdesc->subtype == CDC_DSUBTYPE_MBIM) {
				FAR struct usb_mbim_desc_s *mbim = (FAR struct usb_mbim_desc_s *)desc;
		...
		case USB_DESC_TYPE_INTERFACE: {
			...
			if (ifdesc->classid == CDC_CLASS_COMM && ifdesc->subclass == CDC_SUBCLASS_MBIM &&
		...
		case USB_DESC_TYPE_ENDPOINT: {
			...
			else if ((epdesc->attr & USB_EP_ATTR_XFERTYPE_MASK) == USB_EP_ATTR_XFER_BULK) {
...
static int usbhost_setinterface(FAR struct usbhost_cdcmbim_s *priv, uint16_t iface,
								uint16_t setting)
{
	return usbhost_ctrl_cmd(priv, USB_REQ_DIR_OUT | USB_REQ_RECIPIENT_INTERFACE,
							USB_REQ_SETINTERFACE, setting, iface, NULL, 0);
}

static int cdc_ncm_set_ntb_input_size(FAR struct usbhost_cdcmbim_s *priv, uint32_t size)
{
	uint8_t buf[4];
	...
	return usbhost_ctrl_cmd(priv,
							...
							USB_CDC_SET_NTB_INPUT_SIZE, 0, priv->ctrlif, buf, 4);
}
...
static int cdc_ncm_read_parameters(FAR struct usbhost_cdcmbim_s *priv)
{
	...
	ret = usbhost_ctrl_cmd(priv, USB_REQ_DIR_IN | USB_REQ_TYPE_CLASS | USB_REQ_RECIPIENT_INTERFACE,
						   USB_CDC_GET_NTB_PARAMETERS, 0, priv->ctrlif, (FAR uint8_t *)&params,
						   sizeof(params));
	if (ret == OK) {
...
static int usbhost_connect(FAR struct usbhost_class_s *usbclass, FAR const uint8_t *configdesc,
						   int desclen)
{
	FAR struct usbhost_cdcmbim_s *priv = (FAR struct usbhost_cdcmbim_s *)usbclass;
	int ret;
...
static int usbhost_disconnected(FAR struct usbhost_class_s *usbclass)
{
	...
	if (priv->crefs == 1) {
		...
		if (up_interrupt_context()) {
			...
			work_queue(LPWORK, &priv->destroywork, usbhost_destroy, priv, 0);
		} else {
			/* Do the work now */
...
static int cdcmbim_transmit(FAR struct usbhost_cdcmbim_s *priv)
{
	...
	if ((len % priv->bulkmxpacket) == 0) {
		ret = DRVR_TRANSFER(hport->drvr, priv->bulkout, priv->txnetbuf, 0);
		if (ret < 0) {
...
static int cdcmbim_txpoll(FAR struct net_driver_s *dev)
{
	FAR struct usbhost_cdcmbim_s *priv = (FAR struct usbhost_cdcmbim_s *)dev->d_private;
 ...
 *
 * Description:
 *   ROUX Callback: Bring up the MBIM interface when an IP address is
 *   provided
 *
 * Input Parameters:
 *   dev - Reference to the ROUX driver state structure
...
static int cdcmbim_ifup(FAR struct net_driver_s *dev)
{
	...
	if (priv->bulkin) {
		ret = DRVR_ASYNCH(hport->drvr, priv->bulkin, priv->rxnetbuf, CDCMBIM_NETBUF_SIZE,
						  usbhost_bulkin_callback, priv);
		if (ret < 0) {
