...
static inline void usbhost_mkdevname(FAR struct usbhost_ft232r_s *priv, FAR char *devname);
...
static int usbhost_cfgdesc(FAR struct usbhost_ft232r_s *priv, FAR const uint8_t *configdesc,
						   int desclen);
...
#ifdef CONFIG_SERIAL_IFLOWCONTROL
static bool usbhost_rxflowcontrol(FAR struct uart_dev_s *uartdev, unsigned int nbuffered,
								  bool upper);
#endif
...
static int ft232r_ctrlxfer(FAR struct usbhost_ft232r_s *priv, uint8_t req, uint16_t value,
						   uint16_t index);
...
static FAR struct usbhost_ft232r_s *usbhost_allocclass(void)
{
	...
	priv = (FAR struct usbhost_ft232r_s *)kmm_malloc(sizeof(struct usbhost_ft232r_s));
						   ...
						   uint16_t index)
{
	...
	ctrlreq->type = USB_DIR_OUT | USB_REQ_TYPE_VENDOR | USB_REQ_RECIPIENT_DEVICE;
	ctrlreq->req = req;
...
static int ft232r_reset(FAR struct usbhost_ft232r_s *priv, bool purgerxtx)
{
	int ret = ft232r_ctrlxfer(priv, USBHOST_FT232R_CTRLREQ_RESET, purgerxtx, 0);
	if (ret < 0) {
...
static int ft232r_modemctrl(FAR struct usbhost_ft232r_s *priv)
{
	uint16_t value = USBHOST_FT232R_MODEMCTRL_VAL_RTS | USBHOST_FT232R_MODEMCTRL_VAL_DTR |
					 USBHOST_FT232R_MODEMCTRL_VAL_RTS_EN | USBHOST_FT232R_MODEMCTRL_VAL_DTR_EN;

	int ret = ft232r_ctrlxfer(priv, USBHOST_FT232R_CTRLREQ_MODEMCTRL, value, 0);
	if (ret < 0) {
		uerr("ERROR: ft232r_ctrlxfer failed: %d\n", ret);
	}
...
static int ft232r_setflowctrl(FAR struct usbhost_ft232r_s *priv)
{
	...
	int ret = ft232r_ctrlxfer(priv, USBHOST_FT232R_CTRLREQ_SETFLOWCTRL, value, index);
	if (ret < 0) {
		uerr("ERROR: ft232r_ctrlxfer failed: %d\n", ret);
	}
...
static int ft232r_setdivisor(uint32_t *divisor, uint32_t baud)
{
	...
	if (baud == USBHOST_FT232R_BAUD_3MHZ) {
		...
		*divisor = 1;
		...
		int divfrac[9] = {0, 500, 250, 125, 375, 625, 750, 875, 1000};
		...
		*divisor = (uint32_t)frac;
		...
		if (rem > 0) {
			...
			err = (frac - (double)(*divisor + (double)divfrac[j] / 1000.0)) / frac;
...
static int ft232r_setbaud(FAR struct usbhost_ft232r_s *priv)
{
	...
	if (ret < 0) {
		...
		ret = ft232r_ctrlxfer(priv, USBHOST_FT232R_CTRLREQ_SETBAUD, divisor & 0xffff,
							  divisor && 0x10000);
		if (ret < 0) {
			uerr("ERROR: ft232r_ctrlxfer failed: %d\n", ret);
		}
...
static int ft232r_setdata(FAR struct usbhost_ft232r_s *priv)
{
	...
	ret = ft232r_ctrlxfer(priv, USBHOST_FT232R_CTRLREQ_SETDATA, linecoding, 0);
	if (ret < 0) {
		uerr("ERROR: ft232r_ctrlxfer failed: %d\n", ret);
	}
...
{
	int ret =
		ft232r_ctrlxfer(priv, USBHOST_FT232R_CTRLREQ_SETLATTIMER, CONFIG_USBHOST_FT232R_LATENCY, 0);
	if (ret < 0) {
		uerr("ERROR: ft232r_ctrlxfer failed: %d\n", ret);
	}
...
static void usbhost_txdata_work(FAR void *arg)
{
...
#ifdef CONFIG_USBHOST_FT232R_HWFLOWCTRL
	while (txtail != txbuf->head && priv->txena && !priv->disconnected && priv->cts)
#else
	...
	if (priv->txena && !priv->disconnected) {
		...
		ret = work_queue(LPWORK, &priv->txwork, usbhost_txdata_work, priv, USBHOST_FT232R_TXDELAY);
		DEBUGASSERT(ret >= 0);
		UNUSED(ret);
	}
}
...
static void usbhost_rxdata_work(FAR void *arg)
{
...
#endif
	{
		...
		ret = work_queue(LPWORK, &priv->rxwork, usbhost_rxdata_work, priv, USBHOST_FT232R_RXDELAY);
		DEBUGASSERT(ret >= 0);
		UNUSED(ret);
	}
	...
	while (remaining >= sizeof(struct usb_desc_s)) {
		...
		case USB_DESC_TYPE_INTERFACE: {
			...
			uinfo("Interface descriptor: class: %d subclass: %d proto: %d\n", ifdesc->classid,
				  ifdesc->subclass, ifdesc->protocol);
			DEBUGASSERT(remaining >= USB_SIZEOF_IFDESC);

			/* Check for the FTDI data interface */

			if (ifdesc->classid == USB_CLASS_VENDOR_SPEC && (found & USBHOST_DATAIF_FOUND) == 0) {
...
static int usbhost_connect(FAR struct usbhost_class_s *usbclass, FAR const uint8_t *configdesc,
						   int desclen)
{
	FAR struct usbhost_ft232r_s *priv = (FAR struct usbhost_ft232r_s *)usbclass;
	char devname[DEV_NAMELEN];
	int ret;
...
static int usbhost_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	if (priv->disconnected) {
		...
		case TIOCSERGSTRUCT: {
			FAR struct usbhost_ft232r_s *user = (FAR struct usbhost_ft232r_s *)arg;
			if (!user) {
				ret = -EINVAL;
			} else {
		...
		case TCGETS: {
			...
			termiosp->c_cflag = ((priv->parity != 0) ? PARENB : 0) |
								((priv->parity == 1) ? PARODD : 0) | ((priv->stop2) ? CSTOPB : 0)
#ifdef CONFIG_SERIAL_OFLOWCONTROL
