...
static inline int usbhost_configdesc(const uint8_t *configdesc, int desclen, uint8_t start_ifnum,
									 uint8_t *ret_ifnum, struct usbhost_id_s *id);
...
static inline int usbhost_configdesc(const uint8_t *configdesc, int cfglen, uint8_t start_ifnum,
									 uint8_t *ret_ifnum, struct usbhost_id_s *id)
{
	...
	while (remaining >= sizeof(struct usb_desc_s)) {
		...
		if (ifdesc->type == USB_DESC_TYPE_INTERFACE && ifdesc->ifno >= start_ifnum) {
...
static inline int usbhost_classbind(FAR struct usbhost_hubport_s *hport, const uint8_t *configdesc,
