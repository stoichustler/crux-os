...
static int usbhost_waiter(int argc, FAR char *argv[])
{
	FAR struct usbhost_connection_s *conn =
		(FAR struct usbhost_connection_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
...
int usbhost_waiter_initialize(FAR struct usbhost_connection_s *conn)
{
	...
	argv[0] = arg1;
	argv[1] = NULL;
	return kthread_create("usbhost", CONFIG_USBHOST_WAITER_PRIO, CONFIG_USBHOST_WAITER_STACKSIZE,
						  usbhost_waiter, argv);
}
