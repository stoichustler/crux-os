...
static int usbhost_cfgdesc(FAR struct usbhost_cdcacm_s *priv, FAR const uint8_t *configdesc,
						   int desclen);
	...
	if (!priv->disconnected && priv->intin) {
		...
		if (priv->nbytes >= 0) {
			...
			if ((inmsg->type ==
				...
				(inmsg->notification == ACM_SERIAL_STATE) && (value == 0) &&
				(index == priv->ctrlif) && (len == 2)) {
		...
		ret = DRVR_ASYNCH(hport->drvr, priv->intin, (FAR uint8_t *)priv->notification,
						  MAX_NOTIFICATION, usbhost_notification_callback, priv);
		if (ret < 0) {
...
static void usbhost_notification_callback(FAR void *arg, ssize_t nbytes)
{
	...
	if (!priv->disconnected) {
		...
		if (work_available(&priv->ntwork)) {
			work_queue(HPWORK, &priv->ntwork, usbhost_notification_work, priv, delay);
		}
	}
}
#endif
