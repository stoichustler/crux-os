...
#define CUSTOM_CONFIG_BUFSIZE (USB_SIZEOF_CFGDESC + 3 * USB_SIZEOF_IFDESC + 9 * USB_SIZEOF_EPDESC)
...
static int usbhost_copyinterface(uint8_t ifno, FAR const uint8_t *configdesc, int desclen,
								 FAR uint8_t *buffer, int buflen)
{
	...
	for (offset = 0, retsize = 0; offset < desclen - sizeof(struct usb_desc_s); offset += len) {
		...
		if (desc->type == USB_DESC_TYPE_INTERFACE) {
			...
			if (ifdesc->ifno == ifno && ifdesc->neps != 0) {
				...
				for (offset += len; offset < desclen - sizeof(struct usb_desc_s); offset += len) {
					desc = (FAR struct usb_desc_s *)&configdesc[offset];
					len = desc->len;
...
static int usbhost_createconfig(FAR struct usbhost_member_s *member, FAR const uint8_t *configdesc,
								int desclen, FAR uint8_t *buffer, int buflen)
{
	...
	for (nifs = 0, ifno = member->firstif; nifs < member->nifs; nifs++, ifno++) {
		ifsize = usbhost_copyinterface(ifno, configdesc, desclen, buffer, buflen);
		if (ifsize < 0) {
...
int usbhost_composite(FAR struct usbhost_hubport_s *hport, FAR const uint8_t *configdesc,
					  int desclen, FAR struct usbhost_id_s *id,
					  FAR struct usbhost_class_s **usbclass)
{
...
#warning Missing logic
	...
	priv = (FAR struct usbhost_composite_s *)kmm_zalloc(sizeof(struct usbhost_composite_s));
	...
	priv->members =
		(FAR struct usbhost_member_s *)kmm_zalloc(nclasses * sizeof(struct usbhost_member_s));
	...
	for (i = 0; i < nclasses; i++) {
		...
		cfgsize =
			usbhost_createconfig(member, configdesc, desclen, cfgbuffer, CUSTOM_CONFIG_BUFSIZE);
		if (cfgsize < 0) {
