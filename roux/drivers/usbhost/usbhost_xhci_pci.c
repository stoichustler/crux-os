...
#include <roux/kthread.h>
#include <roux/wqueue.h>
#include <roux/addrenv.h>
#include <roux/spinlock.h>

#include <roux/pci/pci.h>

#include <roux/usb/usb.h>
#include <roux/usb/usbhost.h>
#include <roux/usb/usbhost_devaddr.h>
#include <roux/usb/usbhost_trace.h>
...
static uint32_t xhci_capa_getreg(FAR struct usbhost_xhci_s *priv, unsigned int offset);

static uint8_t xhci_capa_getreg_1b(FAR struct usbhost_xhci_s *priv, unsigned int offset);
static void xhci_capa_putreg_1b(FAR struct usbhost_xhci_s *priv, unsigned int offset,
								uint8_t value);

static uint32_t xhci_oper_getreg(FAR struct usbhost_xhci_s *priv, unsigned int offset);
static void xhci_oper_putreg(FAR struct usbhost_xhci_s *priv, unsigned int offset, uint32_t value);

static void xhci_oper_putreg_8b(FAR struct usbhost_xhci_s *priv, unsigned int offset,
								uint64_t value);

static uint32_t xhci_runt_getreg(FAR struct usbhost_xhci_s *priv, unsigned int offset);
static void xhci_runt_putreg(FAR struct usbhost_xhci_s *priv, unsigned int offset, uint32_t value);

static void xhci_runt_putreg_8b(FAR struct usbhost_xhci_s *priv, unsigned int offset,
								uint64_t value);

static void xhci_door_putreg(FAR struct usbhost_xhci_s *priv, unsigned int offset, uint32_t value);
...
static void xhci_dump_capa_reg(FAR struct usbhost_xhci_s *priv, FAR const char *msg,
							   unsigned int offset);
static void xhci_dump_oper_reg(FAR struct usbhost_xhci_s *priv, FAR const char *msg,
							   unsigned int offset);
static void xhci_dump_runt_reg(FAR struct usbhost_xhci_s *priv, FAR const char *msg,
							   unsigned int offset);
static void xhci_dump_mem(FAR struct usbhost_xhci_s *priv, FAR const char *msg);
#endif
...
static void xhci_add_trb(FAR struct usbhost_xhci_s *priv, FAR struct xhci_ring_s *ring,
						 FAR struct xhci_trb_s *trb, int len);
...
static int xhci_port_enable(FAR struct usbhost_xhci_s *priv, FAR struct usbhost_hubport_s *hport);

/* Slot management **********************************************************/

static void xhci_dcbaa_set(FAR struct usbhost_xhci_s *priv, uint8_t index, uintptr_t ctx);
static void xhci_ep_configure(FAR struct usbhost_xhci_s *priv, FAR struct xhci_ep_ctx_s *ctx,
							  uint8_t type, uint16_t maxpkt, uint8_t maxburst, uint64_t tr_dp,
							  uint8_t mult, uint8_t interval);
static int xhci_address_set(FAR struct usbhost_xhci_s *priv, FAR struct xhci_rhport_s *rhport,
							bool setaddr);
static int xhci_slot_init(FAR struct usbhost_xhci_s *priv, FAR struct xhci_dev_s *dev);
static int xhci_device_init(FAR struct usbhost_xhci_s *priv, FAR struct xhci_rhport_s *rhport);
static int xhci_device_deinit(FAR struct usbhost_xhci_s *priv, FAR struct xhci_rhport_s *rhport);
static inline uint8_t xhci_epno_get(FAR struct xhci_epinfo_s *epinfo);
static void xhci_context_ctrl(FAR struct usbhost_xhci_s *priv, FAR struct xhci_dev_s *dev,
							  uint32_t drop, uint32_t add);

/* Command handling *********************************************************/

static int xhci_command(FAR struct usbhost_xhci_s *priv, FAR struct xhci_trb_s *trb,
						uint16_t timeout_ms);
static int xhci_cmd_sloten(FAR struct usbhost_xhci_s *priv, FAR uint8_t *slot);
static int xhci_cmd_slotdis(FAR struct usbhost_xhci_s *priv, uint8_t slot);
static int xhci_cmd_setaddr(FAR struct usbhost_xhci_s *priv, uint8_t slot, uint64_t ctx, bool bsr);
static int xhci_cmd_cfgep(FAR struct usbhost_xhci_s *priv, uint8_t slot, uint64_t ctx,
						  bool deconfig);
static int xhci_cmd_stopep(FAR struct usbhost_xhci_s *priv, uint8_t slot, uint8_t ep, bool suspend);
static int xhci_cmd_evalctx(FAR struct usbhost_xhci_s *priv, uint8_t slot, uint64_t ctx);

/* Transfer handling ********************************************************/

static void xhci_ep_doorbell(FAR struct usbhost_xhci_s *priv, FAR struct xhci_epinfo_s *epinfo);
static int xhci_ioc_setup(FAR struct xhci_rhport_s *rhport, FAR struct xhci_epinfo_s *epinfo,
						  size_t buflen);
...
static inline int xhci_ioc_async_setup(FAR struct xhci_rhport_s *rhport,
									   FAR struct xhci_epinfo_s *epinfo, usbhost_asynch_t callback,
									   FAR void *arg);
static void xhci_asynch_completion(FAR struct xhci_epinfo_s *epinfo);
#endif
static int xhci_control_setup(FAR struct xhci_rhport_s *rhport, FAR struct xhci_epinfo_s *epinfo,
							  FAR const struct usb_ctrlreq_s *req, FAR uint8_t *buffer,
							  size_t buflen);
static int xhci_normal_setup(FAR struct xhci_rhport_s *rhport, FAR struct xhci_epinfo_s *epinfo,
							 FAR uint8_t *buffer, size_t buflen);
#ifndef CONFIG_USBHOST_ISOC_DISABLE
static int xhci_isoc_setup(FAR struct xhci_rhport_s *rhport, FAR struct xhci_epinfo_s *epinfo,
						   FAR uint8_t *buffer, size_t buflen);
#endif
...
static void xhci_transfer_complete(FAR struct usbhost_xhci_s *priv, FAR struct xhci_trb_s *evt);
static void xhci_event_complete(FAR struct usbhost_xhci_s *priv, FAR struct xhci_trb_s *evt);
...
static int xhci_wait(FAR struct usbhost_connection_s *conn, FAR struct usbhost_hubport_s **hport);
...
static int xhci_ep0configure(FAR struct usbhost_driver_s *drvr, usbhost_ep_t ep0, uint8_t funcaddr,
...
static int xhci_alloc(FAR struct usbhost_driver_s *drvr, FAR uint8_t **buffer, FAR size_t *maxlen);
static int xhci_free(FAR struct usbhost_driver_s *drvr, FAR uint8_t *buffer);
static int xhci_ioalloc(FAR struct usbhost_driver_s *drvr, FAR uint8_t **buffer, size_t buflen);
static int xhci_iofree(FAR struct usbhost_driver_s *drvr, FAR uint8_t *buffer);

static int xhci_ctrl_xfer(FAR struct usbhost_driver_s *drvr, usbhost_ep_t ep0,
						  FAR const struct usb_ctrlreq_s *req, FAR uint8_t *buffer);
...
static ssize_t xhci_transfer(FAR struct usbhost_driver_s *drvr, FAR usbhost_ep_t ep,
							 uint8_t *buffer, size_t buflen);
#ifdef CONFIG_USBHOST_ASYNCH
static int xhci_asynch(FAR struct usbhost_driver_s *drvr, usbhost_ep_t ep, FAR uint8_t *buffer,
					   size_t buflen, usbhost_asynch_t callback, void *arg);
#endif
static int xhci_cancel(FAR struct usbhost_driver_s *drvr, usbhost_ep_t ep);
#ifdef CONFIG_USBHOST_HUB
static int xhci_connect(FAR struct usbhost_driver_s *drvr, FAR struct usbhost_hubport_s *hport,
						bool connected);
#endif

static void xhci_disconnect(FAR struct usbhost_driver_s *drvr, FAR struct usbhost_hubport_s *hport);
...
static const struct pci_device_id_s g_pci_xhci_id_table[] = {
	...
	{PCI_DEVICE(0x1b36, 0x000d), .driver_data = 0},

	/* Intel Alder Lake USB 3.2 xHCI controller */

	{PCI_DEVICE(0x8086, 0x51ed), .driver_data = 0},

	/* Intel Alder Lake-S USB 3.2 Gen 2x2 xHCI controller */

	{PCI_DEVICE(0x8086, 0x7ae0), .driver_data = 0},
	{}};
...
static void xhci_capa_putreg_1b(FAR struct usbhost_xhci_s *priv, unsigned int offset, uint8_t value)
{
	uintptr_t addr = priv->capa_base + offset;
...
static int xhci_ring_init(FAR struct xhci_ring_s *ring, size_t len)
{
	...
	if (!ring->ring) {
		...
		ring->ring = kmm_memalign(XHCI_BUF_ALIGN, sizeof(struct xhci_trb_s) * len);
		if (!ring->ring) {
			return -ENOMEM;
		}
						 ...
						 FAR struct xhci_trb_s *trb, int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (ring->i >= ring->len - 1) {
			...
			if (ring->ccs) {
				d2 = XHCI_TRB_D2_C | XHCI_TRB_D2_TC | XHCI_TRB_D2_TYPE_SET(XHCI_TRB_TYPE_LINK);
			} else {
				d2 = XHCI_TRB_D2_TC | XHCI_TRB_D2_TYPE_SET(XHCI_TRB_TYPE_LINK);
			}
	...
	up_flush_dcache((uintptr_t)ring->ring, (uintptr_t)(ring->ring + ring->len));
}
...
static int xhci_ctrl_start(FAR struct usbhost_xhci_s *priv)
{
	...
	xhci_runt_putreg_8b(priv, XHCI_ERDP(0), up_addrenv_va_to_pa(priv->evnt.ring));
	...
	xhci_runt_putreg_8b(priv, XHCI_ERSTBA(0), up_addrenv_va_to_pa(priv->pg_erst));

	/* Last item in the command ring points to the beginning of the ring */

	priv->cmd.ring[XHCI_CMD_MAX - 1].d0 = htole64(up_addrenv_va_to_pa(priv->cmd.ring));

	/* Configure the Command Ring */

	xhci_oper_putreg_8b(priv, XHCI_CRCR, up_addrenv_va_to_pa(priv->cmd.ring) | XHCI_CRCR_RCS);
	...
	xhci_oper_putreg(priv, XHCI_USBCMD, XHCI_USBCMD_RS | XHCI_USBCMD_INTE | XHCI_USBCMD_HSEE);
...
static int xhci_port_enable(FAR struct usbhost_xhci_s *priv, FAR struct usbhost_hubport_s *hport)
{
	...
	if (!(regval & XHCI_PORTSC_PED)) {
		...
		retries = 10;
		while (!(xhci_oper_getreg(priv, XHCI_PORTSC(rhpndx)) & XHCI_PORTSC_PED) && retries > 0) {
			retries--;
			up_mdelay(100);
		}
...
static void xhci_dcbaa_set(FAR struct usbhost_xhci_s *priv, uint8_t index, uintptr_t ctx)
{
	...
	up_flush_dcache((uintptr_t)priv->pg_ctx, (uintptr_t)(priv->pg_ctx + priv->no_slots + 1));
}
							  ...
							  uint8_t mult, uint8_t interval)
{
	...
	up_flush_dcache((uintptr_t)ctx, (uintptr_t)ctx + sizeof(struct xhci_ep_ctx_s));
}
...
static int xhci_slot_init(FAR struct usbhost_xhci_s *priv, FAR struct xhci_dev_s *dev)
{
	...
	regval = XHCI_IN_CTX1_A(XHCI_SLOT_FLAG) | XHCI_IN_CTX1_A(XHCI_EP0_FLAG);
...
#warning missing logic
	...
	DEBUGASSERT(drdp != 0);
	xhci_ep_configure(priv, &dev->input->ep[0], XHCI_EPTYPE_CTRL, maxpkt, 0, drdp, 0, 0);
	...
	up_flush_dcache((uintptr_t)dev->input,
					(uintptr_t)dev->input + sizeof(struct xhci_input_dev_ctx_s));
...
static int xhci_device_init(FAR struct usbhost_xhci_s *priv, FAR struct xhci_rhport_s *rhport)
{
	 ...
	 *       This is done in xhci_ctrlin() and controlled by ROUX USB Host
	 *       stack.
	 */
						...
						uint16_t timeout_ms)
{
	...
	ret = nxsem_tickwait_uninterruptible(&priv->cmdsem, MSEC2TICK(timeout_ms));
	if (ret < 0) {
...
static int xhci_cmd_slotdis(FAR struct usbhost_xhci_s *priv, uint8_t slot)
{
	...
	trb.d0 = 0;
	trb.d1 = 0;
	trb.d2 = XHCI_TRB_D2_TYPE_SET(XHCI_TRB_TYPE_DIS_SLOT) | XHCI_TRB_D2_SLOTID_SET(slot);
...
static int xhci_cmd_setaddr(FAR struct usbhost_xhci_s *priv, uint8_t slot, uint64_t ctx, bool bsr)
{
	...
	trb.d0 = htole64(ctx);
	trb.d1 = 0;
	trb.d2 = XHCI_TRB_D2_TYPE_SET(XHCI_TRB_TYPE_ADDR_DEV) | XHCI_TRB_D2_SLOTID_SET(slot);
						  ...
						  bool deconfig)
{
	...
	trb.d0 = htole64(ctx);
	trb.d1 = 0;
	trb.d2 = XHCI_TRB_D2_TYPE_SET(XHCI_TRB_TYPE_CFG_EP) | XHCI_TRB_D2_SLOTID_SET(slot);
...
static int xhci_cmd_stopep(FAR struct usbhost_xhci_s *priv, uint8_t slot, uint8_t ep, bool suspend)
{
	...
	trb.d0 = 0;
	trb.d1 = 0;
	trb.d2 = XHCI_TRB_D2_TYPE_SET(XHCI_TRB_TYPE_STOP_EP) | XHCI_TRB_D2_SLOTID_SET(slot) |
			 XHCI_TRB_D2_EP_SET(ep);
...
static int xhci_cmd_evalctx(FAR struct usbhost_xhci_s *priv, uint8_t slot, uint64_t ctx)
{
	...
	trb.d0 = htole64(ctx);
	trb.d1 = 0;
	trb.d2 = XHCI_TRB_D2_TYPE_SET(XHCI_TRB_TYPE_EVAL_CTX) | XHCI_TRB_D2_SLOTID_SET(slot);
							  ...
							  size_t buflen)
{
	...
	trb[i].d2 = XHCI_TRB_D2_TYPE_SET(XHCI_TRB_TYPE_SETUP_STAGE) | XHCI_TRB_D2_IDT;
	...
	trb[i].d2 = XHCI_TRB_D2_IOC | XHCI_TRB_D2_TYPE_SET(XHCI_TRB_TYPE_STAT_STAGE);
...
static ssize_t xhci_transfer_wait(FAR struct usbhost_xhci_s *priv, FAR struct xhci_epinfo_s *epinfo)
{
	int ret;
									   ...
									   FAR void *arg)
{
	...
	DEBUGASSERT(rhport && epinfo && !epinfo->iocwait && epinfo->callback == NULL);
		...
		if ((portsc & XHCI_PORTSC_CSC) != 0) {
			...
			if ((portsc & XHCI_PORTSC_CCS) != 0) {
				...
				if (!rhport->connected) {
					...
					usbhost_vtrace2(XHCI_VTRACE2_PORTSC_CONNECTED, rhpndx + 1, priv->pscwait);
				...
				if (rhport->connected) {
					...
					usbhost_vtrace2(XHCI_VTRACE2_PORTSC_DISCONND, rhpndx + 1, priv->pscwait);
...
static int xhci_events_poll(FAR struct usbhost_xhci_s *priv)
{
	...
	up_invalidate_dcache((uintptr_t)priv->evnt.ring, (uintptr_t)(priv->evnt.ring + XHCI_EVENT_MAX));
	...
	while (1) {
		...
		case XHCI_TRB_EVT_PSTAT_CHANGE: {
			...
			if (work_available(&priv->pscwork)) {
				work_queue(LPWORK, &priv->pscwork, xhci_portsc_work, (FAR void *)priv, 0);
			}
	...
	while (true) {
		...
		for (rhpndx = 0; rhpndx < priv->no_ports; rhpndx++) {
			...
			if (rhport->connected != connport->connected) {
				...
				*hport = connport;
				...
				usbhost_vtrace2(XHCI_VTRACE2_MONWAKEUP, rhpndx + 1, rhport->connected);
				return OK;
			}
		}
		...
		if (priv->hport) {
			...
			*hport = (FAR struct usbhost_hubport_s *)connport;
			...
			usbhost_vtrace2(XHCI_VTRACE2_MONWAKEUP, HPORT(connport), connport->connected);
			return OK;
		}
#endif
							 ...
							 uint8_t speed, uint16_t maxpacketsize)
{
	...
	if (ret >= 0) {
		...
		xhci_context_ctrl(priv, rhport->dev, 0,
						  XHCI_IN_CTX1_A(XHCI_SLOT_FLAG) | XHCI_IN_CTX1_A(XHCI_EP0_FLAG));

		/* Flush Device input context */

		up_flush_dcache((uintptr_t)rhport->dev->input,
						(uintptr_t)rhport->dev->input + sizeof(struct xhci_input_dev_ctx_s));
						...
						FAR const struct usbhost_epdesc_s *epdesc, FAR usbhost_ep_t *ep)
{
	...
	pciinfo("EP%d DIR=%s FA=%08x TYPE=%d Interval=%d MaxPacket=%d\n", epdesc->addr,
			epdesc->in ? "IN" : "OUT", hport->funcaddr, epdesc->xfrtype, epdesc->interval,
			epdesc->mxpacketsize);
#endif
	...
	case USB_EP_ATTR_XFER_BULK: {
		eptype = epinfo->dirin ? XHCI_EPTYPE_BULK_IN : XHCI_EPTYPE_BULK_OUT;
		break;
	}
...
#endif
	{
		eptype = epinfo->dirin ? XHCI_EPTYPE_INTR_IN : XHCI_EPTYPE_INTR_OUT;
		break;
	}

#ifndef CONFIG_USBHOST_ISOC_DISABLE
	case USB_EP_ATTR_XFER_ISOC: {
		eptype = epinfo->dirin ? XHCI_EPTYPE_ISO_IN : XHCI_EPTYPE_ISO_OUT;
		break;
	}
#endif
	...
	xhci_ep_configure(priv, &dev->input->ep[idx - 1], eptype, epdesc->mxpacketsize, 0,
					  up_addrenv_va_to_pa(epinfo->td.ring), 0, epinfo->interval);
	...
	ret = xhci_cmd_cfgep(priv, epinfo->slot, up_addrenv_va_to_pa(dev->input), false);
	if (ret < 0) {
						  ...
						  FAR const struct usb_ctrlreq_s *req, FAR uint8_t *buffer)
{
	...
	if (req->req == USB_REQ_SETADDRESS) {
		...
		if (ret == OK) {
			...
			ep0info->devaddr = XHCI_ST_CTX3_ADDR_GET(rhport->dev->ctx->slot.ctx[3]);
...
static ssize_t xhci_transfer(FAR struct usbhost_driver_s *drvr, usbhost_ep_t ep,
							 FAR uint8_t *buffer, size_t buflen)
{
	FAR struct usbhost_xhci_s *priv = XHCI_PRIV_FROM_DRVR(drvr);
	FAR struct xhci_rhport_s *rhport = (FAR struct xhci_rhport_s *)drvr;
...
static int xhci_mem_alloc(FAR struct usbhost_xhci_s *priv)
{
	...
	for (i = 0; i < priv->no_scratch; i++) {
		...
		priv->pg_sb[i] = up_addrenv_va_to_pa(kmm_memalign(XHCI_PAGE_SIZE, XHCI_PAGE_SIZE));
		if (!priv->pg_sb[i]) {
		...
		memset((FAR void *)(up_addrenv_pa_to_va(priv->pg_sb[i])), 0, XHCI_PAGE_SIZE);
	}
	...
	for (i = 0; i < priv->no_slots; i++) {
		...
		priv->devs[i].input =
			kmm_memalign((XHCI_PAGE_SIZE / 2), sizeof(struct xhci_input_dev_ctx_s));
...
static inline int xhci_sw_initialize(FAR struct usbhost_xhci_s *priv)
{
	 ...
	 * REVISIT: xHCI hardware is responsible for device address, but ROUX USB
