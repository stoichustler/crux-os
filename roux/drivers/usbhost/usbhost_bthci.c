...
static inline int usbhci_cfgdesc(FAR struct usbhost_state_s *priv, FAR const uint8_t *configdesc,
								 int desclen);
...
static int usbhci_connect(FAR struct usbhost_class_s *usbclass, FAR const uint8_t *configdesc,
						  int desclen);
static int usbhost_disconnected(FAR struct usbhost_class_s *usbclass);

/* Driver methods --
 * depend upon the type of ROUX driver interface exported
 */

static int usbhost_ctrl_cmd(FAR struct usbhost_state_s *priv, uint8_t type, uint8_t req,
							uint16_t value, uint16_t iface, uint8_t *payload, uint16_t len);
...
static ssize_t usbhost_cmd_tx(FAR struct usbhost_state_s *priv, FAR const void *buffer,
							  size_t buflen);
static ssize_t usbhost_acl_tx(FAR struct usbhost_state_s *priv, FAR const void *buffer,
							  size_t buflen);

static int usbhost_bthci_send(FAR struct bt_driver_s *dev, enum bt_buf_type_e type, FAR void *data,
							  size_t len);
static int usbhost_bthci_open(FAR struct bt_driver_s *dev);
static void usbhost_bthci_close(FAR struct bt_driver_s *dev);
static int usbhost_bthci_ioctl(FAR struct bt_driver_s *dev, int cmd, unsigned long arg);
...
static int usbhost_ctrl_cmd(FAR struct usbhost_state_s *priv, uint8_t type, uint8_t req,
							uint16_t value, uint16_t index, FAR uint8_t *payload, uint16_t len)
{
	FAR struct usbhost_hubport_s *hport;
...
static void usbhost_event_work(FAR void *arg)
{
	...
	if (priv->evbytes > 0) {
		bt_netdev_receive(&priv->btdev, BT_EVT, priv->evbuffer, priv->evbytes);
		priv->evbytes = 0;
	}

	if (priv->intin) {
		ret = DRVR_ASYNCH(hport->drvr, priv->intin, (FAR uint8_t *)priv->evbuffer, priv->evbuflen,
						  usbhost_event_callback, priv);
		if (ret < 0) {
							  ...
							  size_t buflen)
{
	...
	ret = usbhost_ctrl_cmd(priv, USB_REQ_DIR_OUT | USB_REQ_TYPE_CLASS | USB_REQ_RECIPIENT_DEVICE, 0,
						   0, 0, (FAR uint8_t *)buffer, buflen);
...
static void usbhost_acl_work(FAR void *arg)
{
	...
	if (priv->acbytes > 0) {
		bt_netdev_receive(&priv->btdev, BT_ACL_IN, priv->tbuffer, priv->acbytes);
		priv->acbytes = 0;
	}

	if (priv->bulkin) {
		ret = DRVR_ASYNCH(hport->drvr, priv->bulkin, (FAR uint8_t *)priv->tbuffer, priv->tbuflen,
						  usbhost_acl_callback, priv);
		if (ret < 0) {
			uerr("ERROR: DRVR_ASYNCH failed: %d\n", ret);
		}
	}
							  ...
							  size_t buflen)
{
	...
	nwritten = DRVR_TRANSFER(hport->drvr, priv->bulkout, (FAR uint8_t *)buffer, buflen);
