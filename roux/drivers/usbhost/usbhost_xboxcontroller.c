...
#include <roux/input/xbox-controller.h>
		...
		/* The report was received correctly. */
			...
			case USBHOST_WAITING_CONNECTION:
				...
				if (!priv->initialized) {
					...
					priv->tbuffer[3] = 0x01;
					priv->tbuffer[4] = 0x00;
					nbytes = DRVR_TRANSFER(hport->drvr, priv->epout, priv->tbuffer, 5);
			...
			case USBHOST_GUIDE_BUTTON_STATUS:
				...
				if (priv->tbuffer[1] == 0x30) {
					static const uint8_t guide_button_report_ack[] = {0x01, 0x20, 0x00, 0x09, 0x00,
			...
			case USBHOST_BUTTON_DATA:
				...
				if (priv->open) {
					...
					priv->rpt.sync = XBOX_BUTTON_SET(priv->tbuffer, XBOX_BUTTON_SYNC_INDEX,
													 XBOX_BUTTON_SYNC_MASK);
					priv->rpt.start = XBOX_BUTTON_SET(priv->tbuffer, XBOX_BUTTON_START_INDEX,
													  XBOX_BUTTON_START_MASK);
					priv->rpt.back = XBOX_BUTTON_SET(priv->tbuffer, XBOX_BUTTON_BACK_INDEX,
													 XBOX_BUTTON_BACK_MASK);
					priv->rpt.a =
						XBOX_BUTTON_SET(priv->tbuffer, XBOX_BUTTON_A_INDEX, XBOX_BUTTON_A_MASK);
					priv->rpt.b =
						XBOX_BUTTON_SET(priv->tbuffer, XBOX_BUTTON_B_INDEX, XBOX_BUTTON_B_MASK);
					priv->rpt.x =
						XBOX_BUTTON_SET(priv->tbuffer, XBOX_BUTTON_X_INDEX, XBOX_BUTTON_X_MASK);
					priv->rpt.y =
						XBOX_BUTTON_SET(priv->tbuffer, XBOX_BUTTON_Y_INDEX, XBOX_BUTTON_Y_MASK);
					priv->rpt.dpad_up = XBOX_BUTTON_SET(priv->tbuffer, XBOX_BUTTON_DPAD_UP_INDEX,
														XBOX_BUTTON_DPAD_UP_MASK);
					priv->rpt.dpad_down = XBOX_BUTTON_SET(
						priv->tbuffer, XBOX_BUTTON_DPAD_DOWN_INDEX, XBOX_BUTTON_DPAD_DOWN_MASK);
					priv->rpt.dpad_left = XBOX_BUTTON_SET(
						priv->tbuffer, XBOX_BUTTON_DPAD_LEFT_INDEX, XBOX_BUTTON_DPAD_LEFT_MASK);
					priv->rpt.dpad_right = XBOX_BUTTON_SET(
						priv->tbuffer, XBOX_BUTTON_DPAD_RIGHT_INDEX, XBOX_BUTTON_DPAD_RIGHT_MASK);
					priv->rpt.bumper_left = XBOX_BUTTON_SET(
						priv->tbuffer, XBOX_BUTTON_BUMPER_LEFT_INDEX, XBOX_BUTTON_BUMPER_LEFT_MASK);
					priv->rpt.bumper_right =
						XBOX_BUTTON_SET(priv->tbuffer, XBOX_BUTTON_BUMPER_RIGHT_INDEX,
										XBOX_BUTTON_BUMPER_RIGHT_MASK);
					priv->rpt.stick_click_left = XBOX_BUTTON_SET(
						priv->tbuffer, XBOX_BUTTON_STICK_LEFT_INDEX, XBOX_BUTTON_STICK_LEFT_MASK);
					priv->rpt.stick_click_right = XBOX_BUTTON_SET(
						priv->tbuffer, XBOX_BUTTON_STICK_RIGHT_INDEX, XBOX_BUTTON_STICK_RIGHT_MASK);

					priv->rpt.trigger_left =
						((FAR int16_t *)(priv->tbuffer))[XBOX_BUTTON_TRIGGER_LEFT];
					priv->rpt.trigger_right =
						((FAR int16_t *)(priv->tbuffer))[XBOX_BUTTON_TRIGGER_RIGHT];
					priv->rpt.stick_left_x =
						((FAR int16_t *)(priv->tbuffer))[XBOX_BUTTON_STICK_LEFT_X];
					priv->rpt.stick_left_y =
						((FAR int16_t *)(priv->tbuffer))[XBOX_BUTTON_STICK_LEFT_Y];
					priv->rpt.stick_right_x =
						((FAR int16_t *)(priv->tbuffer))[XBOX_BUTTON_STICK_RIGHT_X];
					priv->rpt.stick_right_y =
						((FAR int16_t *)(priv->tbuffer))[XBOX_BUTTON_STICK_RIGHT_Y];
...
static inline int usbhost_devinit(FAR struct usbhost_state_s *priv)
{
	...
	ret = kthread_create("xbox", CONFIG_XBOXCONTROLLER_DEFPRIO, CONFIG_XBOXCONTROLLER_STACKSIZE,
...
static int usbhost_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	int nbytes;
	FAR struct usbhost_hubport_s *hport;
	static uint8_t rumble_cmd[] = {0x09, 0x00, 0x00, 0x09, 0x00, 0x0f, 0x00,
	...
	case XBOX_CONTROLLER_IOCTL_RUMBLE:
		...
		nbytes = DRVR_TRANSFER(hport->drvr, priv->epout, priv->obuffer, sizeof(rumble_cmd));
