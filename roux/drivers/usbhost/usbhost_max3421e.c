...
static void max3421e_checkreg(uint8_t addr, uint8_t val, bool iswrite)
	...
	max3421e_fmtcmd(FAR struct max3421e_usbhost_s *priv, uint8_t addr, uint8_t dir);
static uint32_t max3421e_getreg(FAR struct max3421e_usbhost_s *priv, uint8_t addr);
static void max3421e_putreg(FAR struct max3421e_usbhost_s *priv, uint8_t addr, uint8_t value);

static inline void max3421e_modifyreg(FAR struct max3421e_usbhost_s *priv, uint8_t addr,
									  uint8_t clrbits, uint8_t setbits);

static void max3421e_recvblock(FAR struct max3421e_usbhost_s *priv, uint8_t addr, FAR void *buffer,
							   size_t buflen);
static void max3421e_sndblock(FAR struct max3421e_usbhost_s *priv, uint8_t addr,
...
static int max3421e_out_asynch(FAR struct max3421e_usbhost_s *priv,
							   FAR struct max3421e_chan_s *chan, FAR uint8_t *buffer, size_t buflen,
...
static int max3421e_ctrl_recvstatus(FAR struct max3421e_usbhost_s *priv,
									FAR struct max3421e_chan_s *chan);
static int max3421e_in_setup(FAR struct max3421e_usbhost_s *priv, FAR struct max3421e_chan_s *chan);
...
static ssize_t max3421e_in_transfer(FAR struct max3421e_usbhost_s *priv,
									...
									size_t buflen);
#ifdef CONFIG_USBHOST_ASYNCH
static void max3421e_in_next(FAR struct max3421e_usbhost_s *priv, FAR struct max3421e_chan_s *chan);
static int max3421e_in_asynch(FAR struct max3421e_usbhost_s *priv, FAR struct max3421e_chan_s *chan,
...
static inline void max3421e_int_enable(FAR struct max3421e_usbhost_s *priv, uint8_t irqset);
static inline void max3421e_int_disable(FAR struct max3421e_usbhost_s *priv, uint8_t irqset);
static inline uint8_t max3421e_int_status(FAR struct max3421e_usbhost_s *priv);
static inline void max3421e_int_clear(FAR struct max3421e_usbhost_s *priv, uint8_t irqset);
static void max3421e_int_wait(FAR struct max3421e_usbhost_s *priv, uint8_t irqset,
							  unsigned int usec);
...
static int max3421e_alloc(FAR struct usbhost_driver_s *drvr, FAR uint8_t **buffer,
						  FAR size_t *maxlen);
static int max3421e_free(FAR struct usbhost_driver_s *drvr, FAR uint8_t *buffer);
static int max3421e_ioalloc(FAR struct usbhost_driver_s *drvr, FAR uint8_t **buffer, size_t buflen);
...
#ifdef CONFIG_USBHOST_ASYNCH
static int max3421e_asynch(FAR struct usbhost_driver_s *drvr, usbhost_ep_t ep, FAR uint8_t *buffer,
...
static inline uint8_t max3421e_fmtcmd(FAR struct max3421e_usbhost_s *priv, uint8_t addr,
									  uint8_t dir)
{
...
static int max3421e_chan_wait(FAR struct max3421e_usbhost_s *priv, FAR struct max3421e_chan_s *chan)
{
	irqstate_t flags;
	int ret;
								 ...
								 FAR struct max3421e_chan_s *chan, int result)
{
	...
	if (priv->waiter != NULL) {
		...
		usbhost_vtrace2(chan->in ? MAX3421E_VTRACE2_CHANWAKEUP_IN : MAX3421E_VTRACE2_CHANWAKEUP_OUT,
...
static inline void max3421e_restore_toggles(FAR struct max3421e_usbhost_s *priv,
											FAR struct max3421e_chan_s *chan)
{
	max3421e_modifyreg(priv, MAX3421E_USBHOST_HCTL, USBHOST_HCTL_TOGGLES_MASK,
										...
										FAR struct max3421e_chan_s *chan, int result)
{
	...
	max3421e_int_disable(priv,
						 USBHOST_HIRQ_SNDBAVIRQ | USBHOST_HIRQ_RCVDAVIRQ | USBHOST_HIRQ_HXFRDNIRQ);
								 ...
								 FAR struct max3421e_chan_s *chan)
{
	...
	for (i = 0; i < 2 && committed < priv->buflen &&
		...
		max3421e_sndblock(priv, MAX3421E_USBHOST_SNDFIFO, priv->buffer + committed, wrsize);
		...
		max3421e_putreg(priv, MAX3421E_USBHOST_HXFR, USBHOST_HXFR_TOKEN_OUT | chan->epno);
								...
								FAR struct max3421e_chan_s *chan)
{
	...
	usbhost_vtrace2(MAX3421E_VTRACE2_STARTTRANSFER1, chan->chidx, priv->buflen);
...
#ifdef CONFIG_USBHOST_ASYNCH
static void max3421e_out_next(FAR struct max3421e_usbhost_s *priv, FAR struct max3421e_chan_s *chan)
{
								   ...
								   FAR const struct usb_ctrlreq_s *req)
{
	...
	do {
		...
		max3421e_sndblock(priv, MAX3421E_USBHOST_SUDFIFO, priv->buffer, priv->buflen);

		/* Send the SETUP token (always EP0) */

		max3421e_putreg(priv, MAX3421E_USBHOST_HXFR, USBHOST_HXFR_TOKEN_SETUP);
									...
									FAR struct max3421e_chan_s *chan)
{
	...
	max3421e_putreg(priv, MAX3421E_USBHOST_HXFR, USBHOST_HXFR_TOKEN_OUTHS | chan->epno);
									...
									FAR struct max3421e_chan_s *chan)
{
	...
	max3421e_putreg(priv, MAX3421E_USBHOST_HXFR, USBHOST_HXFR_TOKEN_INHS | chan->epno);
...
static int max3421e_in_setup(FAR struct max3421e_usbhost_s *priv, FAR struct max3421e_chan_s *chan)
{
	...
	case USB_EP_ATTR_XFER_ISOC: /* Isochronous */
	{
		...
		usbhost_vtrace2(MAX3421E_VTRACE2_ISOCIN, chan->chidx, priv->buflen);
	...
	{
		usbhost_vtrace2(MAX3421E_VTRACE2_BULKIN, chan->chidx, priv->buflen);
	} break;

	case USB_EP_ATTR_XFER_INT: /* Interrupt */
	{
		usbhost_vtrace2(MAX3421E_VTRACE2_INTRIN, chan->chidx, priv->buflen);
	} break;
	}
...
static int max3421e_out_setup(FAR struct max3421e_usbhost_s *priv, FAR struct max3421e_chan_s *chan)
{
	/* Set up for the transfer based on the direction and the endpoint type */
	...
	case USB_EP_ATTR_XFER_ISOC: /* Isochronous */
	{
		...
		usbhost_vtrace2(MAX3421E_VTRACE2_ISOCOUT, chan->chidx, priv->buflen);
	...
	case USB_EP_ATTR_XFER_BULK: /* Bulk */
	{
		usbhost_vtrace2(MAX3421E_VTRACE2_BULKOUT, chan->chidx, priv->buflen);
	} break;

	case USB_EP_ATTR_XFER_INT: /* Interrupt */
	{
		usbhost_vtrace2(MAX3421E_VTRACE2_INTROUT, chan->chidx, priv->buflen);
	} break;
	}
									...
									FAR struct max3421e_chan_s *chan)
{
	...
	max3421e_recvblock(priv, MAX3421E_USBHOST_RCVFIFO, priv->buffer + priv->xfrd, nrcvd);
								  ...
								  FAR struct max3421e_chan_s *chan)
{
	...
	max3421e_putreg(priv, MAX3421E_USBHOST_HXFR, USBHOST_HXFR_TOKEN_IN | chan->epno);
								...
								FAR struct max3421e_chan_s *chan)
{
	...
	usbhost_vtrace2(MAX3421E_VTRACE2_STARTTRANSFER2, chan->chidx, priv->buflen);
...
static void max3421e_in_next(FAR struct max3421e_usbhost_s *priv, FAR struct max3421e_chan_s *chan)
{
	...
	usbhost_vtrace2(MAX3421E_VTRACE2_XFRCOMPLETE, (unsigned int)ep, buflen);
...
static void max3421e_irqwork(FAR void *arg)
{
	...
	for (;;) {
		...
		if ((pending & USBHOST_HIRQ_HXFRDNIRQ) != 0 && (pending & USBHOST_HIRQ_SNDBAVIRQ) == 0 &&
			...
			result = max3421e_transfer_status(priv);
			if (result < 0 || priv->xfrtype == HXFRDN_SETUP || priv->xfrtype == HXFRDN_SNDZLP) {
		...
		/* SNDBAV: The SNDFIFO is available */
			...
			max3421e_int_disable(priv, USBHOST_HIRQ_SNDBAVIRQ | USBHOST_HIRQ_HXFRDNIRQ);
		...
		/* RCVDAVIRQ: RCVFIFO data available */
			...
			max3421e_int_disable(priv, USBHOST_HIRQ_RCVDAVIRQ | USBHOST_HIRQ_HXFRDNIRQ);
	...
	for (;;) {
		...
		if (priv->hport) {
			...
			*hport = connport;
			...
			usbhost_vtrace1(MAX3421E_VTRACE1_HUB_CONNECTED, connport->connected);
						   ...
						   FAR const struct usb_ctrlreq_s *req, FAR uint8_t *buffer)
{
	...
	uinfo("type:%02x req:%02x value:%02x%02x index:%02x%02x len:%02x%02x\n", req->type, req->req,
		  req->value[1], req->value[0], req->index[1], req->index[0], req->len[1], req->len[0]);
...
#ifdef CONFIG_USBHOST_HUB
static int max3421e_connect(FAR struct usbhost_driver_s *drvr, FAR struct usbhost_hubport_s *hport,
							bool connected)
{
	FAR struct max3421e_usbhost_s *priv = (FAR struct max3421e_usbhost_s *)drvr;
	irqstate_t flags;
...
static int max3421e_startsof(FAR struct max3421e_usbhost_s *priv)
{
	...
	max3421e_modifyreg(priv, MAX3421E_USBHOST_HCTL, 0, USBHOST_HCTL_BUSSAMPLE);

	while ((max3421e_getreg(priv, MAX3421E_USBHOST_HCTL) & USBHOST_HCTL_BUSSAMPLE) == 0) {
		nxsig_usleep(5);
	}
	...
	clrbits = 0;
	setbits =
		USBHOST_MODE_HOST | USBHOST_MODE_SOFKAENAB | USBHOST_MODE_DMPULLD | USBHOST_MODE_DPPULLDN;
										 ...
										 FAR const struct max3421e_lowerhalf_s *lower)
{
	...
	memset(priv->chan, 0, MAX3421E_NHOST_CHANNELS * sizeof(struct max3421e_chan_s));
...
static inline int max3421e_hw_initialize(FAR struct max3421e_usbhost_s *priv)
{
	...
	while ((max3421e_getreg(priv, MAX3421E_USBHOST_USBIRQ) & USBHOST_USBIRQ_OSCOKIRQ) == 0) {
	}
	...
	max3421e_modifyreg(priv, MAX3421E_USBHOST_MODE, USBHOST_MODE_SPEED | USBHOST_MODE_SOFKAENAB,
					   USBHOST_MODE_HOST | USBHOST_MODE_DMPULLD | USBHOST_MODE_DPPULLDN);
...
max3421e_usbhost_initialize(FAR const struct max3421e_lowerhalf_s *lower)
{
	...
	DEBUGASSERT(lower != NULL && lower->spi != NULL && lower->attach != NULL &&
				lower->attach != NULL && lower->acknowledge != NULL && lower->power != NULL);

	/* Allocate and instance of the MAX4321E state structure */

	alloc = (FAR struct usbhost_alloc_s *)kmm_zalloc(sizeof(struct usbhost_alloc_s));
