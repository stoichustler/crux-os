...
#ifdef CONFIG_HIDKBD_ENCODED
static inline void usbhost_encodescancode(FAR struct usbhost_state_s *priv, uint8_t scancode,
										  uint8_t modifier);
#endif
...
static int usbhost_send_request(FAR struct usbhost_state_s *priv, uint8_t dir, uint8_t req,
								uint16_t value, uint16_t index, uint16_t len, FAR uint8_t *data);
...
static inline int usbhost_cfgdesc(FAR struct usbhost_state_s *priv, FAR const uint8_t *configdesc,
								  int desclen);
...
static int usbhost_connect(FAR struct usbhost_class_s *usbclass, FAR const uint8_t *configdesc,
						   int desclen);
...
static const uint8_t encoding[USBHID_NUMENCODINGS] = {
	...
	KEYCODE_F7, KEYCODE_F8, KEYCODE_F9, KEYCODE_F10, KEYCODE_F11, KEYCODE_F12, KEYCODE_PRTSCRN,
	KEYCODE_SCROLLLOCK,
	...
	KEYCODE_PAUSE, KEYCODE_INSERT, KEYCODE_HOME, KEYCODE_PAGEUP, KEYCODE_FWDDEL, KEYCODE_END,
	...
	KEYCODE_F13, KEYCODE_F14, KEYCODE_F15, KEYCODE_F16, KEYCODE_F17, KEYCODE_F18, KEYCODE_F19,
	KEYCODE_F20,

	/* 0x70-0x77: F21,F22,F23,F24,Execute,Help,Menu,Select */

	KEYCODE_F21, KEYCODE_F22, KEYCODE_F23, KEYCODE_F24, KEYCODE_EXECUTE, KEYCODE_HELP, KEYCODE_MENU,
	KEYCODE_SELECT,

	/* 0x78-0x7f: Stop,Again,Undo,Cut,Copy,Paste,Find,Mute */

	KEYCODE_STOP, KEYCODE_AGAIN, KEYCODE_UNDO, KEYCODE_CUT, KEYCODE_COPY, KEYCODE_PASTE,
	...
	KEYCODE_VOLUP, KEYCODE_VOLDOWN, KEYCODE_LCAPSLOCK, KEYCODE_LNUMLOCK, KEYCODE_LSCROLLLOCK, 0, 0,
	0,
	...
	KEYCODE_LANG1, KEYCODE_LANG2, KEYCODE_LANG3, KEYCODE_LANG4, KEYCODE_LANG5, KEYCODE_LANG6,
	...
	KEYCODE_MEMSTORE, KEYCODE_MEMRECALL, KEYCODE_MEMCLEAR, KEYCODE_MEMADD, KEYCODE_MEMSUB,
	KEYCODE_MEMMUL, KEYCODE_MEMDIV, KEYCODE_NEGATE,

	/* 0xd8-0xdd: Clear,ClearEntry,Binary,Octal,Decimal,Hexadecimal */

	KEYCODE_CLEAR, KEYCODE_CLEARENTRY, KEYCODE_BINARY, KEYCODE_OCTAL, KEYCODE_DECIMAL,
	KEYCODE_HEXADECIMAL
#endif
};
...
static const uint8_t ucmap[USBHID_NUMSCANCODES] = {
	0,	  0,	  0,	  0,	'A',
	'B',  'C',	  'D', /* 0x00-0x07: Reserved, errors, A-D */
	'E',  'F',	  'G',	  'H',	'I',
	'J',  'K',	  'L', /* 0x08-0x0f: E-L */
	'M',  'N',	  'O',	  'P',	'Q',
	'R',  'S',	  'T', /* 0x10-0x17: M-T */
	...
	'*',  '(',	  ')', /* 0x20-0x27: #,$,%,^,&,*,(,) */
	'\n', '\033', '\177', 0,	' ',
	'_',  '+',	  '{', /* 0x28-0x2f: Enter,escape,del,back-tab,space,_,+,{ */
	'}',  '|',	  0,	  ':',	'"',
	'~',  '<',	  '>', /* 0x30-0x37: },|,Non-US tilde,:,",grave tilde,<,> */
	'?',  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0x38-0x3f: /,CapsLock,F1,F2,F3,F4,F5,F6 */
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0x40-0x47: F7,F8,F9,F10,F11,F12,PrtScn,ScrollLock */
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0x48-0x4f: Pause,Insert,Home,PageUp,DeleteForward,End,PageDown,RightArrow */
	0,	  0,	  0,	  0,	'/',
	'*',  '-',	  '+', /* 0x50-0x57: LeftArrow,DownArrow,UpArrow,Num Lock,/,*,-,+ */
	'\n', '1',	  '2',	  '3',	'4',
	'5',  '6',	  '7', /* 0x58-0x5f: Enter,1-7 */
	'8',  '9',	  '0',	  '.',	0,
	0,	  0,	  '=', /* 0x60-0x67: 8-9,0,.,Non-US \,Application,Power,= */
#ifdef CONFIG_HIDKBD_ALLSCANCODES
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0x68-0x6f: F13,F14,F15,F16,F17,F18,F19,F20 */
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0x70-0x77: F21,F22,F23,F24,Execute,Help,Menu,Select */
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0x78-0x7f: Stop,Again,Undo,Cut,Copy,Paste,Find,Mute */
	0,	  0,	  0,	  0,	0,
	',',  0,	  0, /* 0x80-0x87: VolUp,VolDown,LCapsLock,lNumLock,LScrollLock,,,=,International1
					  */
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0x88-0x8f: International 2-9 */
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0x90-0x97: LAN 1-8 */
	0,	  0,	  0,	  0,	0,
	0,	  '\n',	  0, /* 0x98-0x9f: LAN 9,Erase,SysReq,Cancel,Clear,Prior,Return,Separator */
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0xa0-0xa7: Out,Oper,Clear,CrSel,Excel,(reserved) */
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0xa8-0xaf: (reserved) */
	0,	  0,	  0,	  0,	0,
	0,	  '(',	  ')', /* 0xb0-0xb7: 00,000,ThouSeparator,DecSeparator,CurrencyUnit,SubUnit,(,) */
	'{',  '}',	  '\t',    \177, 'A',
	'B',  'C',	  'D', /* 0xb8-0xbf: {,},tab,backspace,A-D */
	'F',  'F',	  0,	  '^',	'%',
	'<',  '>',	  '&', /* 0xc0-0xc7: E-F,XOR,^,%,<,>,& */
	0,	  '|',	  0,	  ':',	'%',
	' ',  '@',	  '!', /* 0xc8-0xcf: &&,|,||,:,#, ,@,! */
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0xd0-0xd7: Memory Store,Recall,Clear,Add,Subtract,Multiply,Divide,+/- */
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0xd8-0xdf: Clear,ClearEntry,Binary,Octal,Decimal,Hexadecimal */
	0,	  0,	  0,	  0,	0,
	0,	  0,	  0, /* 0xe0-0xe7: Left Ctrl,Shift,Alt,GUI, Right Ctrl,Shift,Alt,GUI */
#endif
};
...
static const uint8_t lcmap[USBHID_NUMSCANCODES] = {
	...
	'r',  's',	  't', /* 0x10-0x17: m-t */
	'u',  'v',	  'w',	  'x',	  'y',
	'z',  '1',	  '2', /* 0x18-0x1f: u-z,1-2  */
	'3',  '4',	  '5',	  '6',	  '7',
	'8',  '9',	  '0', /* 0x20-0x27: 3-9,0 */
	'\n', '\033', '\177', '\t',	  ' ',
	'-',  '=',	  '[', /* 0x28-0x2f: Enter,escape,del,tab,space,-,=,[ */
	']',  '\\',	  '\234', ';',	  '\'',
	'`',  ',',	  '.', /* 0x30-0x37: ],\,Non-US pound,;,',grave accent,,,. */
	'/',  0,	  0,	  0,	  0,
	0,	  0,	  0, /* 0x38-0x3f: /,CapsLock,F1,F2,F3,F4,F5,F6 */
	0,	  0,	  0,	  0,	  0,
	0,	  0,	  0, /* 0x40-0x47: F7,F8,F9,F10,F11,F12,PrtScn,ScrollLock */
...
#ifndef CONFIG_HIDKBD_NODEBOUNCE
			&& rpt->key[i] != priv->lastkey[0] && rpt->key[i] != priv->lastkey[1] &&
			rpt->key[i] != priv->lastkey[2] && rpt->key[i] != priv->lastkey[3] &&
			rpt->key[i] != priv->lastkey[4] && rpt->key[i] != priv->lastkey[5]
#endif
		) {
...
static void usbhost_kbd_work(FAR void *arg)
{
	...
	if (!priv->disconnected && priv->epin) {
		...
		if (priv->caps_lock != usbhost_get_capslock()) {
			...
			ret = DRVR_ASYNCH(hport->drvr, priv->epin, (FAR uint8_t *)priv->tbuffer,
							  sizeof(struct usbhid_kbdreport_s), usbhost_kbd_callback, priv);
...
static int usbhost_kbdpoll(int argc, FAR char *argv[])
{
	...
	while (!priv->disconnected) {
		...
		ret = usbhost_send_request(priv, USB_REQ_DIR_IN, USBHID_REQUEST_GETREPORT,
								   USBHID_REPORTTYPE_INPUT << 8, priv->ifno,
								   sizeof(struct usbhid_kbdreport_s), priv->tbuffer);
								  ...
								  int desclen)
{
	...
	DEBUGASSERT(priv != NULL && priv->usbclass.hport != NULL && configdesc != NULL &&
...
static inline int usbhost_devinit(FAR struct usbhost_state_s *priv)
{
	...
	usbhost_send_request(priv, USB_REQ_DIR_OUT, USBHID_REQUEST_SETIDLE, 25 << 8, priv->ifno, 0,
						 NULL);

	/* Choose the boot protocol */

	usbhost_send_request(priv, USB_REQ_DIR_OUT, USBHID_REQUEST_SETPROTOCOL, 0, 0, 0, NULL);
	...
	*priv->tbuffer = leds;
	 ...
	 * the ROUX worker thread to do this, but this task needs to wait for
	...
	ret = kthread_create("kbdpoll", CONFIG_HIDKBD_DEFPRIO, CONFIG_HIDKBD_STACKSIZE, usbhost_kbdpoll,
						 NULL);
	if (ret < 0) {
...
static int usbhost_cralloc(FAR struct usbhost_state_s *priv)
{
	...
	DEBUGASSERT(priv != NULL && priv->usbclass.hport != NULL && priv->ctrlreq == NULL);
	hport = priv->usbclass.hport;
								...
								uint16_t value, uint16_t index, uint16_t len, FAR uint8_t *data)
{
	...
	ctrlreq->type = dir | USB_REQ_TYPE_CLASS | USB_REQ_RECIPIENT_INTERFACE;
	ctrlreq->req = req;
						   ...
						   int desclen)
{
	...
	if (ret < 0) {
		...
		/* Now configure the device and register the ROUX driver */
