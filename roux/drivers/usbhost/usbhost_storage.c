...
static inline void usbhost_readcbw(blkcnt_t startsector, uint16_t blocksize, unsigned int nsectors,
								   FAR struct usbmsc_cbw_s *cbw);
static inline void usbhost_writecbw(blkcnt_t startsector, uint16_t blocksize, unsigned int nsectors,
									FAR struct usbmsc_cbw_s *cbw);
...
static ssize_t usbhost_write(FAR struct inode *inode, FAR const unsigned char *buffer,
							 blkcnt_t startsector, unsigned int nsectors);
...
/* Block driver operations.  This is the interface exposed to ROUX by the
...
static void usbhost_dumpcbw(FAR struct usbmsc_cbw_s *cbw)
{
	...
	for (i = 0; i < cbw->cdblen; i += 8) {
		uinfo("  %02x %02x %02x %02x %02x %02x %02x %02x\n", cbw->cdb[i], cbw->cdb[i + 1],
			  cbw->cdb[i + 2], cbw->cdb[i + 3], cbw->cdb[i + 4], cbw->cdb[i + 5], cbw->cdb[i + 6],
			  cbw->cdb[i + 7]);
	}
}
...
static inline int usbhost_requestsense(FAR struct usbhost_state_s *priv)
{
	...
	if (nbytes >= 0) {
		...
		nbytes =
			DRVR_TRANSFER(hport->drvr, priv->bulkin, priv->tbuffer, SCSIRESP_FIXEDSENSEDATA_SIZEOF);
		if (nbytes >= 0) {
			/* Receive the CSW */
...
static inline int usbhost_readcapacity(FAR struct usbhost_state_s *priv)
{
	...
	if (nbytes >= 0) {
		...
		nbytes =
			DRVR_TRANSFER(hport->drvr, priv->bulkin, priv->tbuffer, SCSIRESP_READCAPACITY10_SIZEOF);
		if (nbytes >= 0) {
...
static ssize_t usbhost_read(FAR struct inode *inode, unsigned char *buffer, blkcnt_t startsector,
	...
	if (priv->disconnected) {
		...
		if (cbw) {
			...
			do {
				...
				nbytes = DRVR_TRANSFER(hport->drvr, priv->bulkout, (FAR uint8_t *)cbw,
									   USBMSC_CBW_SIZEOF);
				if (nbytes >= 0) {
					/* Receive the user data */

					nbytes = DRVR_TRANSFER(hport->drvr, priv->bulkin, buffer,
										   priv->blocksize * nsectors);
					if (nbytes >= 0) {
						/* Receive the CSW */

						nbytes = DRVR_TRANSFER(hport->drvr, priv->bulkin, priv->tbuffer,
											   USBMSC_CSW_SIZEOF);
						if (nbytes >= 0) {
							FAR struct usbmsc_csw_s *csw;
	...
	if (priv->disconnected) {
		...
		if (cbw) {
			...
			if (nbytes >= 0) {
				...
				nbytes = DRVR_TRANSFER(hport->drvr, priv->bulkout, (FAR uint8_t *)buffer,
									   priv->blocksize * nsectors);
				if (nbytes >= 0) {
					/* Receive the CSW */

					nbytes =
						DRVR_TRANSFER(hport->drvr, priv->bulkin, priv->tbuffer, USBMSC_CSW_SIZEOF);
					if (nbytes >= 0) {
						FAR struct usbmsc_csw_s *csw;
...
int usbhost_msc_notifier_setup(worker_t worker, uint8_t event, char sdchar, FAR void *arg)
{
