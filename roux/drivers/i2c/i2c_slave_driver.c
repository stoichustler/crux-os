...
static ssize_t i2c_slave_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t i2c_slave_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
static int i2c_slave_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup);
...
static int i2c_slave_callback(FAR void *arg, i2c_slave_complete_t status, size_t length)
{
...
int i2c_slave_register(FAR struct i2c_slave_s *dev, int bus, int addr, int nbits)
{
	FAR struct i2c_slave_driver_s *priv;
	...
	ret = I2CS_READ(priv->dev, priv->read_buffer, CONFIG_I2C_SLAVE_READBUFSIZE);
	if (ret < 0) {
		goto out;
	}
