...
#include <roux/i2c/i2c_bitbang.h>
...
static int i2c_bitbang_transfer(FAR struct i2c_master_s *dev, FAR struct i2c_msg_s *msgs,
								int count);

static int i2c_bitbang_set_scl(FAR struct i2c_bitbang_dev_s *dev, bool high, bool nodelay);
...
static void i2c_bitbang_send(FAR struct i2c_bitbang_dev_s *dev, uint8_t data);
...
static int i2c_bitbang_transfer(FAR struct i2c_master_s *dev, FAR struct i2c_msg_s *msgs, int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		priv->delay = (USEC_PER_SEC / (2 * msg->frequency)) - CONFIG_I2C_BITBANG_GPIO_OVERHEAD;
		...
		if (i == 0 || (msgs[i - 1].flags & I2C_M_READ) != (msgs[i].flags & I2C_M_READ)) {
			/* Send start bit */
			...
			addr = (msg->flags & I2C_M_READ ? I2C_READADDR8(msg->addr) : I2C_WRITEADDR8(msg->addr));
...
static int i2c_bitbang_wait_ack(FAR struct i2c_bitbang_dev_s *priv)
{
	...
	for (i = 0; i2c_bitbang_get_sda(priv) && i < CONFIG_I2C_BITBANG_TIMEOUT; i++) {
		up_udelay(1);
	}
...
static void i2c_bitbang_send(FAR struct i2c_bitbang_dev_s *priv, uint8_t data)
{
	uint8_t bit = 1u << 7;
...
static int i2c_bitbang_set_scl(FAR struct i2c_bitbang_dev_s *dev, bool high, bool nodelay)
{
	...
	if (high) {
		...
		for (i = 0; !i2c_bitbang_get_scl(dev) && i < CONFIG_I2C_BITBANG_TIMEOUT; i++) {
			up_udelay(1);
...
FAR struct i2c_master_s *i2c_bitbang_initialize(FAR struct i2c_bitbang_lower_dev_s *lower)
{
