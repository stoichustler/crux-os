 ...
 *
 *   Simplified 'execl()' and 'execv()' functions are provided by ROUX for
 *   compatibility.  ROUX is a tiny embedded RTOS that does not support
 *   processes and hence the concept of overlaying a tasks process image with
 *   a new process image does not make any sense.  In ROUX, these functions
 ...
 *   provided to 'exec()' from 'exec[l|v]()' via ROUX configuration
 *   settings:
 *
 ...
 *   addressed in a future version of ROUX.  Other than just being an
