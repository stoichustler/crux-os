...
static int nxthread_setup_scheduler(FAR struct tcb_s *tcb, int priority, start_t start,
									CODE void *entry, uint8_t ttype)
{
	FAR struct tcb_s *rtcb = this_task();
	irqstate_t flags;
	int ret;
...
int nxtask_setup_stackargs(FAR struct task_tcb_s *tcb, FAR const char *name, FAR char *const argv[])
{
	FAR char **stackargv;
	FAR char *str;
	size_t strtablen;
...
int nxtask_setup_scheduler(FAR struct task_tcb_s *tcb, int priority, start_t start, main_t main,
						   uint8_t ttype)
{
	/* Perform common thread setup */

	return nxthread_setup_scheduler((FAR struct tcb_s *)tcb, priority, start, (CODE void *)main,
									ttype);
}
							...
							pthread_startroutine_t entry)
{
	...
	return nxthread_setup_scheduler((FAR struct tcb_s *)tcb, priority, start, (CODE void *)entry,
