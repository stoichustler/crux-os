 ...
 *   - ROUX provides only posix_spawn() or posix_spawnp() behavior
 ...
 *     because ROUX does not support default signal actions
 *     (POSIX_SPAWN_SETSIGDEF).
 *
 * POSIX Compatibility
 *   - The value of the argv[0] received by the child task is assigned by
 *     ROUX.  For the caller of posix_spawn(), the provided argv[0] will
...
{
	return nxposix_spawn_exec(pid, path, file_actions != NULL ? *file_actions : NULL, attr, argv,
							  envp);
}
