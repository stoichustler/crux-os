...
FAR struct task_tcb_s *nxtask_setup_fork(start_t retaddr)
{
	...
	stack_size =
		(uintptr_t)ptcb->stack_base_ptr - (uintptr_t)ptcb->stack_alloc_ptr + ptcb->adj_stack_size;
	...
	ret = nxtask_setup_scheduler(child, priority, retaddr, ptcb->entry.main, ttype);
	if (ret < OK) {
		goto errout_with_tcb;
	}
...
void nxtask_abort_fork(FAR struct task_tcb_s *child, int errcode)
{
	...
	nxsched_release_tcb((FAR struct tcb_s *)child, child->cmn.flags & TCB_FLAG_TTYPE_MASK);
	set_errno(errcode);
}
