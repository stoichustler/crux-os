...
static inline void nxtask_exitstatus(FAR struct task_group_s *group, int status)
{
...
static inline void nxtask_sigchild(pid_t ppid, FAR struct tcb_s *ctcb, int status)
{
...
static inline void nxtask_sigchild(FAR struct tcb_s *ptcb, FAR struct tcb_s *ctcb, int status)
{
	siginfo_t info;
