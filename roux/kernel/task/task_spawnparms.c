...
static inline void nxspawn_close(FAR struct tcb_s *tcb,
...
static inline int nxspawn_dup2(FAR struct tcb_s *tcb, FAR struct spawn_dup2_file_action_s *action)
{
	/* Perform the dup */

	sinfo("Dup'ing %d->%d\n", action->fd1, action->fd2);

	return fdlist_dup2(nxsched_get_fdlist_from_tcb(tcb), action->fd1, action->fd2);
}

static inline int nxspawn_open(FAR struct tcb_s *tcb, FAR struct spawn_open_file_action_s *action)
{
...
int spawn_file_actions(FAR struct tcb_s *tcb, FAR const posix_spawn_file_actions_t *actions)
{
	...
	for (entry = (FAR struct spawn_general_file_action_s *)actions; entry && ret >= 0;
		 entry = entry->flink) {
		switch (entry->action) {
...
bool spawn_file_is_duplicateable(FAR const posix_spawn_file_actions_t *actions, int fd,
								 bool cloexec)
{
	FAR struct spawn_general_file_action_s *entry;
	...
	for (entry = (FAR struct spawn_general_file_action_s *)actions; entry != NULL;
		 entry = entry->flink) {
		switch (entry->action) {
		case SPAWN_FILE_ACTION_CLOSE:
