...
static int nxtask_spawn_create(FAR const char *name, int priority, FAR void *stack_addr,
							   ...
							   FAR const posix_spawnattr_t *attr)
{
	...
	ret = nxtask_init(tcb, name, priority, stack_addr, stack_size, entry, argv, envp, actions);
	if (ret < OK) {
		kmm_free(tcb);
		return ret;
	}
...
static int nxtask_spawn_exec(FAR pid_t *pidp, FAR const char *name, main_t entry,
							 FAR const posix_spawn_file_actions_t *actions,
							 FAR const posix_spawnattr_t *attr, FAR char *const *argv,
							 FAR char *const envp[])
{
	...
	pid = nxtask_spawn_create(name, priority, stackaddr, stacksize, entry, argv,
 ...
 *       not supported.  ROUX does not support process groups.
 *     - POSIX_SPAWN_SETSCHEDPARAM: Set new tasks priority to the sched_param
 *       value.
 ...
 *       child process is not supported.  ROUX does not support effective
 *       user IDs.
 *     - POSIX_SPAWN_SETSIGMASK: Set the new task's signal mask.
 *     - POSIX_SPAWN_SETSIGDEF:  Resetting signal default actions is not
 *       supported.  ROUX does not support default signal actions.
 *
...
int task_spawn(FAR const char *name, main_t entry,
			   ...
			   FAR const posix_spawnattr_t *attr, FAR char *const argv[], FAR char *const envp[])
{
	pid_t pid = INVALID_PROCESS_ID;
	int ret;

	sinfo("name=%s entry=%p file_actions=%p attr=%p argv=%p\n", name, entry, file_actions, attr,
		  argv);

	ret = nxtask_spawn_exec(&pid, name, entry, file_actions != NULL ? *file_actions : NULL, attr,
							argv, envp);
