					...
					int stack_size, main_t entry, FAR char *const argv[], FAR char *const envp[])
{
	...
	ret = nxtask_init((FAR struct task_tcb_s *)tcb, name, priority, stack_addr, stack_size, entry,
					  argv, envp, NULL);
	if (ret < OK) {
		kmm_free(tcb);
		return ret;
...
int task_create_with_stack(FAR const char *name, int priority, FAR void *stack_addr, int stack_size,
...
int kthread_create_with_stack(FAR const char *name, int priority, FAR void *stack_addr,
							  int stack_size, main_t entry, FAR char *const argv[])
{
	return nxthread_create(name, TCB_FLAG_TTYPE_KERNEL, priority, stack_addr, stack_size, entry,
						   argv, NULL);
}
...
int kthread_create(FAR const char *name, int priority, int stack_size, main_t entry,
				   FAR char *const argv[])
{
