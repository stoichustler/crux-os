...
static int modprocfs_open(FAR struct file *filep, FAR const char *relpath, int oflags, mode_t mode);
static int modprocfs_close(FAR struct file *filep);
static ssize_t modprocfs_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
...
static int modprocfs_callback(FAR struct module_s *modp, FAR void *arg)
{
	...
	linesize = snprintf(priv->line, MOD_LINELEN, "%s,%p,%p,%u,%p,%lu,%p,%lu\n", modp->modname,
						modp->modinfo.uninitializer, modp->modinfo.arg, modp->modinfo.nexports,
						modp->textalloc, (unsigned long)modp->textsize,
						(FAR uint8_t *)modp->dataalloc, (unsigned long)modp->datasize);
	copysize = procfs_memcpy(priv->line, linesize, priv->buffer, priv->remaining, &priv->offset);
