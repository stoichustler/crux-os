...
#if !defined(CONFIG_SCHED_TICKLESS) && !defined(CONFIG_ALARM_ARCH) && !defined(CONFIG_TIMER_ARCH)
...
void clock_resynchronize(FAR struct timespec *rtc_diff)
{
	...
	if (curr_ts.tv_sec > rtc_time.tv_sec ||
		...
		clock_t diff_ticks = SEC2TICK(rtc_diff->tv_sec) + NSEC2TICK(rtc_diff->tv_nsec);
