...
irqstate_t enter_critical_section_wo_note(void)
{
	...
	if (up_interrupt_context()) {
		...
		if (g_cpu_nestcount[cpu] > 0) {
			DEBUGASSERT(spin_is_locked(&g_cpu_irqlock) && g_cpu_nestcount[cpu] < UINT8_MAX);
			g_cpu_nestcount[cpu]++;
		}
		 ...
		 */
			...
			DEBUGASSERT(spin_is_locked(&g_cpu_irqlock) && (g_cpu_irqset & (1 << cpu)) != 0);
		}
	} else {
		...
		if (rtcb->irqcount > 0) {
			...
			DEBUGASSERT(spin_is_locked(&g_cpu_irqlock) && (g_cpu_irqset & (1 << this_cpu())) != 0 &&
...
#if CONFIG_SCHED_CRITMONITOR_MAXTIME_CSECTION >= 0 || defined(CONFIG_SCHED_INSTRUMENTATION_CSECTION)
...
void leave_critical_section_wo_note(irqstate_t flags)
{
	...
	if (up_interrupt_context()) {
		...
		if (g_cpu_nestcount[cpu] > 1) {
			...
			DEBUGASSERT(spin_is_locked(&g_cpu_irqlock) && g_cpu_nestcount[cpu] == 1);
