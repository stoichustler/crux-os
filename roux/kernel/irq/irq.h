...
#if defined(CONFIG_ARCH_MINIMAL_VECTORTABLE) && !defined(CONFIG_ARCH_NUSER_INTERRUPTS)
...
#define IRQ_TO_NDX(irq) (g_irqmap[(irq)] < CONFIG_ARCH_NUSER_INTERRUPTS ? g_irqmap[(irq)] : -EINVAL)
#else
...
typedef CODE int (*irq_foreach_t)(int irq, FAR struct irq_info_s *info, FAR void *arg);
#endif
 ...
 *
 * REVISIT: This should be declared in include/roux/irq.h.  The declaration
