...
#error Missing logic
	...
	linesize =
		snprintf(irqfile->line, IRQ_LINELEN, IRQ_FMT, (unsigned int)irq,
				 (unsigned long)((uintptr_t)copy.handler), (unsigned long)((uintptr_t)copy.arg),
				 count, intpart, fracpart, (unsigned long)delta.tv_nsec / 1000);

	copysize = procfs_memcpy(irqfile->line, linesize, irqfile->buffer, irqfile->remaining,
							 &irqfile->offset);
