...
static struct irq_work_info_s g_irq_work_vector[CONFIG_ARCH_NUSER_INTERRUPTS];
#else
...
static uint8_t
	g_irq_work_stack[CONFIG_IRQ_NWORKS][CONFIG_IRQ_WORK_STACKSIZE] locate_data(IRQ_WORK_SECTION);
#else
...
static inline_function FAR struct kwork_wqueue_s *irq_get_wqueue(int priority)
{
	...
	for (i = 0; g_irq_wqueue[i] != NULL && i < CONFIG_IRQ_NWORKS; i++) {
		...
		DEBUGASSERT(wqueue_priority >= SCHED_PRIORITY_MIN && wqueue_priority <= SCHED_PRIORITY_MAX);
	...
	queue =
		work_queue_create("isrwork", priority, g_irq_work_stack[i], CONFIG_IRQ_WORK_STACKSIZE, 1);
