...
#define __TLIST_HEAD(t)                                                                            \
	(TLIST_ISOFFSET((t)->task_state)                                                               \
		 ? (FAR dq_queue_t *)((FAR uint8_t *)((t)->waitobj) +                                      \
...
int nxthread_create(FAR const char *name, uint8_t ttype, int priority, FAR void *stack_addr,
					int stack_size, main_t entry, FAR char *const argv[], FAR char *const envp[]);
...
void nxsched_merge_prioritized(FAR dq_queue_t *list1, FAR dq_queue_t *list2, uint8_t task_state);
...
#else
#define nxsched_reprioritize(tcb, sched_priority) nxsched_set_priority(tcb, sched_priority)
#endif
...
uint32_t nxsched_process_roundrobin(FAR struct tcb_s *tcb, uint32_t ticks, bool noswitches);
#endif
...
uint32_t nxsched_process_sporadic(FAR struct tcb_s *tcb, uint32_t ticks, bool noswitches);
...
#else
noinstrument_function static inline_function FAR struct tcb_s *this_task(void)
{
...
#if defined(CONFIG_SCHED_CPULOAD_SYSCLK) || defined(CONFIG_SCHED_CPULOAD_CRITMONITOR)
...
void nxsched_critmon_preemption(FAR struct tcb_s *tcb, bool state, FAR void *caller);
#else
#define nxsched_critmon_preemption(t, s, c)
#endif

#if CONFIG_SCHED_CRITMONITOR_MAXTIME_CSECTION >= 0
void nxsched_critmon_csection(FAR struct tcb_s *tcb, bool state, FAR void *caller);
#endif
...
static inline_function bool nxsched_add_prioritized(FAR struct tcb_s *tcb, DSEG dq_queue_t *list)
{
	...
	for (next = (FAR struct tcb_s *)list->head; (next && sched_priority <= next->sched_priority);
		 next = next->flink)
		;
