...
#define CHECK_PREEMPTION(pid, elapsed)                                                             \
	do {                                                                                           \
		if (pid > 0 && elapsed > CONFIG_SCHED_CRITMONITOR_MAXTIME_PREEMPTION) {                    \
...
#define CHECK_CSECTION(pid, elapsed)                                                               \
	do {                                                                                           \
		if (pid > 0 && elapsed > CONFIG_SCHED_CRITMONITOR_MAXTIME_CSECTION) {                      \
			CRITMONITOR_PANIC("PID %d hold critical section too long %" PRIu32 "\n", pid,          \
							  elapsed);                                                            \
		}                                                                                          \
	} while (0)
#else
...
#define CHECK_THREAD(pid, elapsed)                                                                 \
	do {                                                                                           \
		if (pid > 0 && elapsed > CONFIG_SCHED_CRITMONITOR_MAXTIME_THREAD) {                        \
...
static void nxsched_critmon_cpuload(FAR struct tcb_s *tcb, clock_t current, clock_t tick)
{
	int i;
	UNUSED(i);
