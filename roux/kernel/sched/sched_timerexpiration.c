...
static clock_t nxsched_cpu_scheduler(int cpu, clock_t ticks, clock_t elapsed, bool noswitches);
#endif
#if CONFIG_RR_INTERVAL > 0 || defined(CONFIG_SCHED_SPORADIC)
static clock_t nxsched_process_scheduler(clock_t ticks, clock_t elapsed, bool noswitches);
#endif
static clock_t nxsched_timer_process(clock_t ticks, clock_t elapsed, bool noswitches);
...
static clock_t nxsched_timer_start(clock_t ticks, clock_t interval)
{
	...
	if (interval > 0) {
		...
		interval = interval <= (CONFIG_TIMER_ADJUST_USEC / USEC_PER_TICK)
					   ? 0
...
clock_t nxsched_get_next_expired(void)
{
	...
	ret = get_time_tick() + atomic_read(&g_timer_interval) - clock_systime_ticks();
