...
static int sporadic_replenish_delay(FAR struct replenishment_s *repl, uint32_t period,
									uint32_t replenish);
...
FAR struct replenishment_s *sporadic_alloc_repl(FAR struct sporadic_s *sporadic);
...
static int sporadic_budget_start(FAR struct replenishment_s *mrepl)
{
	...
	DEBUGVERIFY(wd_start(&mrepl->timer, sporadic->budget, sporadic_budget_expire, (wdparm_t)mrepl));
...
static int sporadic_interval_start(FAR struct replenishment_s *mrepl)
{
	...
	DEBUGVERIFY(wd_start(&mrepl->timer, remainder, sporadic_interval_expire, (wdparm_t)mrepl));
...
static int sporadic_replenish_start(FAR struct replenishment_s *repl)
{
	...
	DEBUGVERIFY(wd_start(&repl->timer, repl->budget, sporadic_replenish_expire, (wdparm_t)repl));
									...
									uint32_t replenish)
{
	...
	DEBUGVERIFY(wd_start(&repl->timer, period, sporadic_delay_expire, (wdparm_t)repl));
	return OK;
}
