...
void nxsched_remove_running(FAR struct tcb_s *tcb)
{
	...
	for (int i = 0; i < CONFIG_SMP_NCPUS; i++) {
		...
		for (rtrtcb = (FAR struct tcb_s *)g_assignedtasks[i].head; !is_idle_task(rtrtcb);
			 rtrtcb = rtrtcb->flink) {
			if (rtrtcb->task_state != TSTATE_TASK_RUNNING && CPU_ISSET(cpu, &rtrtcb->affinity)) {
	...
	for (rtrtcb = (FAR struct tcb_s *)g_readytorun.head;
		 rtrtcb != NULL && !CPU_ISSET(cpu, &rtrtcb->affinity); rtrtcb = rtrtcb->flink)
		;
