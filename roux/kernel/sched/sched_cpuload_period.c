...
static bool nxsched_period_callback(FAR uint32_t *next_interval_us, FAR void *arg);
...
static bool nxsched_period_callback(FAR uint32_t *next_interval_us, FAR void *arg)
{
	...
	*next_interval_us = CPULOAD_PERIOD_NOMINAL - CPULOAD_PERIOD_ENTROPY / 2 + g_sched_period.error;

	/* Add the random value in the range 0..(CPULOAD_PERIOD_ENTROPY - 1) */

	*next_interval_us += xorshift128(&g_sched_period.prng) & CPULOAD_PERIOD_ENTROPY_MASK;
	...
	g_sched_period.error = CPULOAD_PERIOD_NOMINAL + g_sched_period.error - *next_interval_us;
#endif
									...
									enum pm_state_e pmstate)
{
	if (domain == PM_IDLE_DOMAIN) {
		if (pmstate == PM_RESTORE) {
			g_sched_period.idle_ticks += clock_systime_ticks() - g_sched_period.idle_start;

			if (g_sched_period.idle_ticks >= CPULOAD_PERIOD_TICKS) {
				nxsched_process_cpuload_ticks(g_sched_period.idle_ticks / CPULOAD_PERIOD_TICKS);
