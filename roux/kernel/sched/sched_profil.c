...
#ifdef CONFIG_SMP
static struct smp_call_data_s g_call_data = SMP_CALL_INITIALIZER(profil_timer_handler_cpu, &g_prof);
#endif
...
int profil(FAR unsigned short *buf, size_t bufsiz, size_t offset, unsigned int scale)
{
	...
	wd_start(&prof->timer, PROFTICK, profil_timer_handler, (wdparm_t)(uintptr_t)prof);
	return OK;
}
