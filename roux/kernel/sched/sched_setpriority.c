...
static FAR struct tcb_s *nxsched_nexttcb(FAR struct tcb_s *tcb)
{
	...
	if (!nxsched_islocked_tcb(this_task())) {
		...
		for (rtrtcb = (FAR struct tcb_s *)list_readytorun()->head;
			 rtrtcb != NULL && !CPU_ISSET(tcb->cpu, &rtrtcb->affinity); rtrtcb = rtrtcb->flink)
			;
...
static inline void nxsched_running_setpriority(FAR struct tcb_s *tcb, int sched_priority)
{
...
static void nxsched_readytorun_setpriority(FAR struct tcb_s *tcb, int sched_priority)
{
...
static inline void nxsched_blocked_setpriority(FAR struct tcb_s *tcb, int sched_priority)
{
