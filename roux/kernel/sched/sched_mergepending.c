...
bool nxsched_merge_pending(void)
{
	...
	if (!nxsched_islocked_tcb(rtcb)) {
		for (ptcb = (FAR struct tcb_s *)list_pendingtasks()->head; ptcb; ptcb = pnext) {
			pnext = ptcb->flink;
			...
			for (; (rtcb && ptcb->sched_priority <= rtcb->sched_priority); rtcb = rtcb->flink) {
			}
			...
			if (rprev == NULL) {
				...
				ptcb->blink = NULL;
				rtcb->blink = ptcb;
				list_readytorun()->head = (FAR dq_entry_t *)ptcb;
	...
	if (!nxsched_islocked_tcb(this_task())) {
		...
		while (ptcb->sched_priority > rtcb->sched_priority) {
			...
			if (nxsched_islocked_tcb(this_task())) {
				...
				nxsched_merge_prioritized(list_readytorun(), list_pendingtasks(),
										  TSTATE_TASK_PENDING);
		...
		nxsched_merge_prioritized(list_pendingtasks(), list_readytorun(), TSTATE_TASK_READYTORUN);
	}
