...
static void nxsched_smp_call_add(int cpu, FAR struct smp_call_data_s *data)
{
	irqstate_t flags;
...
int nxsched_smp_call_handler(int irq, FAR void *context, FAR void *arg)
{
	...
	{
		FAR struct smp_call_data_s *data = container_of(curr, struct smp_call_data_s, node[cpu]);
		int ret;
...
void nxsched_smp_call_init(FAR struct smp_call_data_s *data, nxsched_smp_call_t func, FAR void *arg)
{
