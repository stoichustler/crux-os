 ...
 *
 * But then I have to be concerned about the fact that ROUX does not queue
...
pid_t nxsched_waitpid(pid_t pid, int *stat_loc, int options)
{
	...
	for (;;) {
		 ...
		 * ROUX.  So a possibility is that the child has died and we
		...
		if (info.si_signo == SIGCHLD && (pid == INVALID_PROCESS_ID || info.si_pid == pid)) {
