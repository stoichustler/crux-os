...
#include <roux/lib/xorshift128.h>
...
static void nxsched_oneshot_callback(FAR struct oneshot_lowerhalf_s *lower, FAR void *arg);
...
static void nxsched_oneshot_start(void)
{
	...
	usecs = (CPULOAD_ONESHOT_NOMINAL - CPULOAD_ONESHOT_ENTROPY / 2) + g_sched_oneshot.error;
	...
	g_sched_oneshot.error = CPULOAD_ONESHOT_NOMINAL + g_sched_oneshot.error - usecs;
#else
	/* No entropy */
	...
	DEBUGVERIFY(ONESHOT_START(g_sched_oneshot.oneshot, nxsched_oneshot_callback, NULL, &ts));
}
									 ...
									 enum pm_state_e pmstate)
{
	if (domain == PM_IDLE_DOMAIN) {
		if (pmstate == PM_RESTORE) {
			g_sched_oneshot.idle_ticks += clock_systime_ticks() - g_sched_oneshot.idle_start;

			if (g_sched_oneshot.idle_ticks >= CPULOAD_ONESHOT_TICKS) {
				nxsched_process_cpuload_ticks(g_sched_oneshot.idle_ticks / CPULOAD_ONESHOT_TICKS);
