...
int pthread_mutex_init(FAR pthread_mutex_t *mutex, FAR const pthread_mutexattr_t *attr)
{
	...
	mutex->flags = attr && attr->robust == PTHREAD_MUTEX_ROBUST ? _PTHREAD_MFLAGS_ROBUST : 0;
#else
	mutex->flags = 0;
#endif
#endif
	...
	if (attr) {
		status = mutex_set_protocol(&mutex->mutex, SEM_TYPE_MUTEX | attr->proto);
		if (status < 0) {
		...
		mutex_set_protocol(&mutex->mutex, SEM_TYPE_MUTEX | PTHREAD_MUTEX_DEFAULT_PRIO_FLAGS);
	}
#endif
