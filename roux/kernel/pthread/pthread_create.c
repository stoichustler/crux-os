...
/* Default pthread attributes (see include/roux/pthread.h).  When configured
...
static inline void pthread_tcb_setup(FAR struct pthread_tcb_s *ptcb, FAR struct tcb_s *parent,
									 pthread_trampoline_t trampoline, pthread_addr_t arg)
{
...
int nx_pthread_create(pthread_trampoline_t trampoline, FAR pthread_t *thread,
					  FAR const pthread_attr_t *attr, pthread_startroutine_t entry,
					  pthread_addr_t arg)
{
	...
	if (attr->stackaddr) {
		...
		ret = up_use_stack((FAR struct tcb_s *)ptcb, attr->stackaddr, attr->stacksize);
	} else {
	...
	defined(CONFIG_ARCH_KERNEL_STACK)
	...
	ret = pthread_setup_scheduler(ptcb, param.sched_priority, pthread_start, entry);
	if (ret != OK) {
		errcode = EBUSY;
		goto errout_with_tcb;
	}
