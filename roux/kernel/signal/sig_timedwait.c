					...
					FAR struct timespec *rmtp)
{
	...
	if (rqtp) {
		...
		if ((flags & TIMER_ABSTIME) == 0) {
			...
			wd_start_abstick(&rtcb->waitdog, expect, nxsig_timeout, (uintptr_t)rtcb);
		} else if (clockid == CLOCK_REALTIME) {
			wd_start_realtime(&rtcb->waitdog, rqtp, nxsig_timeout, (uintptr_t)rtcb);
		} else {
			wd_start_abstime(&rtcb->waitdog, rqtp, nxsig_timeout, (uintptr_t)rtcb);
		}
	}
