...
struct sigpool_s {
	sigq_t sigq[NUM_PENDING_ACTIONS + CONFIG_SIG_PREALLOC_IRQ_ACTIONS];
	sigpendq_t sigpendq[NUM_SIGNALS_PENDING + CONFIG_SIG_PREALLOC_IRQ_ACTIONS];
};
...
static void *nxsig_init_block(sq_queue_t *siglist, FAR sigq_t *sigq, uint16_t nsigs,
							  uint8_t sigtype)
{
	int i;
...
static void *nxsig_init_pendingsignalblock(FAR sq_queue_t *siglist, FAR sigpendq_t *sigpend,
...
void nxsig_initialize(void)
{
	...
	sigpool = nxsig_init_block(&g_sigpendingaction, sigpool, NUM_PENDING_ACTIONS, SIG_ALLOC_FIXED);
	sigpool = nxsig_init_block(&g_sigpendingirqaction, sigpool, CONFIG_SIG_PREALLOC_IRQ_ACTIONS,
							   SIG_ALLOC_IRQ);
	sigpool = nxsig_init_pendingsignalblock(&g_sigpendingsignal, sigpool, NUM_SIGNALS_PENDING,
											SIG_ALLOC_FIXED);
	sigpool = nxsig_init_pendingsignalblock(&g_sigpendingirqsignal, sigpool,
											CONFIG_SIG_PREALLOC_IRQ_ACTIONS, SIG_ALLOC_IRQ);
	sched_trace_end();
}
