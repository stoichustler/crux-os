...
static int nxsig_queue_action(FAR struct tcb_s *stcb, FAR sigactq_t *sigact, FAR siginfo_t *info)
{
	FAR sigq_t *sigq;
	int ret = OK;
...
static FAR sigpendq_t *nxsig_find_pendingsignal(FAR struct task_group_s *group, int signo)
{
	...
	for (sigpend = (FAR sigpendq_t *)group->tg_sigpendingq.head;
		 (sigpend && sigpend->info.si_signo != signo); sigpend = sigpend->flink)
		;
...
static FAR sigpendq_t *nxsig_add_pendingsignal(FAR struct tcb_s *stcb, FAR siginfo_t *info,
											   bool group_dispatch)
{
...
int nxsig_tcbdispatch(FAR struct tcb_s *stcb, siginfo_t *info, bool group_dispatch)
{
	...
	if (masked == 0) {
		...
		else if (stcb->task_state == TSTATE_TASK_STOPPED && info->si_signo == SIGCONT) {
