...
#else
	DEBUGVERIFY(nxsig_notification(timer->pt_owner, &timer->pt_event, SI_TIMER, NULL));
#endif
}
...
static inline void timer_restart(FAR struct posix_timer_s *timer, wdparm_t itimer)
{
	...
	if (timer->pt_delay) {
		...
		wd_start_abstick(&timer->pt_wdog, timer->pt_expected, timer_timeout, itimer);
	}
}
				  ...
				  FAR struct itimerspec *ovalue)
{
	...
	ret = wd_start_abstick(&timer->pt_wdog, timer->pt_expected, timer_timeout, (wdparm_t)timer);
