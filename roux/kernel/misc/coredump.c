...
static uint8_t g_running_regs[XCPTCONTEXT_SIZE] aligned_data(XCPTCONTEXT_ALIGN);
...
static struct memory_region_s g_memory_region[] = {CONFIG_BOARD_MEMORY_RANGE};
#endif
...
static int elf_emit(FAR struct elf_dumpinfo_s *cinfo, FAR const void *buf, size_t len)
{
...
{
	off_t align = ALIGN_UP(cinfo->stream->nput, ELF_PAGESIZE) - cinfo->stream->nput;
...
static int elf_emit_hdr(FAR struct elf_dumpinfo_s *cinfo, int segs)
{
	Elf_Ehdr ehdr;
...
static void elf_emit_tcb_note(FAR struct elf_dumpinfo_s *cinfo, FAR struct tcb_s *tcb)
{
	...
	if (regs != NULL) {
		for (i = 0; i < MIN(nitems(status.pr_regs), g_tcbinfo.regs_num); i++) {
			if (g_tcbinfo.reg_off.p[i] != UINT16_MAX) {
				status.pr_regs[i] = *(uintptr_t *)((uint8_t *)regs + g_tcbinfo.reg_off.p[i]);
			}
		}
	}
...
static void elf_emit_tcb_stack(FAR struct elf_dumpinfo_s *cinfo, FAR struct tcb_s *tcb)
{
	uintptr_t buf = 0;
	uintptr_t sp;
	size_t len;
	...
	if (running_task() != tcb) {
		...
		if (sp > (uintptr_t)tcb->stack_base_ptr &&
			...
			len = ((uintptr_t)tcb->stack_base_ptr + tcb->adj_stack_size) - sp;
			buf = sp;
		}
#ifdef CONFIG_STACK_COLORATION
		else {
			len = up_check_tcbstack(tcb);
			buf = (uintptr_t)tcb->stack_base_ptr + (tcb->adj_stack_size - len);
		}
#endif
	}
	...
	if (buf == 0) {
		...
		len = tcb->adj_stack_size + (tcb->stack_base_ptr - tcb->stack_alloc_ptr);
	}
...
static void elf_emit_tcb_phdr(FAR struct elf_dumpinfo_s *cinfo, FAR struct tcb_s *tcb,
							  FAR Elf_Phdr *phdr, off_t *offset)
{
	...
	if (running_task() != tcb) {
		...
		if (sp > (uintptr_t)tcb->stack_base_ptr &&
			sp < (uintptr_t)tcb->stack_base_ptr + tcb->adj_stack_size) {
			phdr->p_filesz = ((uintptr_t)tcb->stack_base_ptr + tcb->adj_stack_size) - sp;
			phdr->p_vaddr = sp;
		}
#ifdef CONFIG_STACK_COLORATION
		else {
			phdr->p_filesz = up_check_tcbstack(tcb);
			phdr->p_vaddr = (uintptr_t)tcb->stack_base_ptr + (tcb->adj_stack_size - phdr->p_filesz);
		}
#endif
	}
	...
	if (phdr->p_vaddr == 0) {
		...
		phdr->p_filesz = tcb->adj_stack_size + (tcb->stack_base_ptr - tcb->stack_alloc_ptr);
	}
...
static void elf_emit_phdr(FAR struct elf_dumpinfo_s *cinfo, int stksegs, int memsegs)
{
	off_t offset = cinfo->stream->nput + (stksegs + memsegs + 1 + 1) * sizeof(Elf_Phdr);
	Elf_Phdr phdr;
	int i;
	...
	if (cinfo->pid == INVALID_PROCESS_ID) {
		...
		elf_emit_tcb_phdr(cinfo, nxsched_get_tcb(cinfo->pid), &phdr, &offset);
	}
...
int coredump_add_memory_region(FAR const void *ptr, size_t size, uint32_t flags)
{
	...
	if (g_regions != NULL) {
		...
		while (region->start < region->end) {
			if ((uintptr_t)ptr >= region->start && (uintptr_t)ptr + size < region->end) {
				/* Already watched */

				return 0;
			} else if ((uintptr_t)ptr < region->start && (uintptr_t)ptr + size >= region->end) {
				/* start out of region, end out of region */

				region->start = (uintptr_t)ptr;
				region->end = (uintptr_t)ptr + size;
				return 0;
			} else if ((uintptr_t)ptr < region->start && (uintptr_t)ptr + size >= region->start) {
				/* start out of region, end in region */

				region->start = (uintptr_t)ptr;
				return 0;
			} else if ((uintptr_t)ptr < region->end && (uintptr_t)ptr + size >= region->end) {
...
#ifdef CONFIG_BOARD_COREDUMP_BLKDEV
	ret = lib_blkoutstream_open(&g_devstream, CONFIG_BOARD_COREDUMP_DEVPATH);
#elif defined(CONFIG_BOARD_COREDUMP_MTDDEV)
	ret = lib_mtdoutstream_open(&g_devstream, CONFIG_BOARD_COREDUMP_DEVPATH);
#endif
...
int coredump(FAR const struct memory_region_s *regions, FAR struct lib_outstream_s *stream,
			 pid_t pid)
{
	...
	if (cinfo.regions != NULL) {
		for (; cinfo.regions[memsegs].start < cinfo.regions[memsegs].end; memsegs++)
			;
	}

	/* Fill notes section, with additional one for program header,
	 * and one for the core file info defined by ROUX.
	 */
