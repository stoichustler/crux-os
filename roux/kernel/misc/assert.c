...
#include <roux/reboot_notifier.h>
...
static uintptr_t g_last_regs[CONFIG_SMP_NCPUS][XCPTCONTEXT_REGS] aligned_data(XCPTCONTEXT_ALIGN);

#ifdef CONFIG_DEBUG_ALERT
static FAR const char *const g_policy[4] = {"FIFO", "RR", "SPORADIC"};

static FAR const char *const g_ttypenames[4] = {"Task", "pthread", "Kthread", "Invalid"};
#endif

#ifdef CONFIG_SMP
static struct smp_call_data_s g_call_data = SMP_CALL_INITIALIZER(pause_cpu_handler, NULL);
#endif
...
static void stack_dump(uintptr_t sp, uintptr_t stack_top)
{
	...
	for (stack = sp; stack <= stack_top; stack += DUMP_STRIDE) {
		...
		_alert("%p:" DUMP_FORMAT DUMP_FORMAT DUMP_FORMAT DUMP_FORMAT DUMP_FORMAT DUMP_FORMAT
				   DUMP_FORMAT DUMP_FORMAT "\n",
			   (FAR void *)stack, DUMP_PTR(ptr, 0), DUMP_PTR(ptr, 1), DUMP_PTR(ptr, 2),
			   DUMP_PTR(ptr, 3), DUMP_PTR(ptr, 4), DUMP_PTR(ptr, 5), DUMP_PTR(ptr, 6),
			   DUMP_PTR(ptr, 7));
	}
}
...
static void dump_stackinfo(FAR const char *tag, uintptr_t sp, uintptr_t base, size_t size,
						   size_t used)
{
...
static void dump_stacks(FAR struct tcb_s *rtcb, uintptr_t sp)
{
	...
	if (intstack_sp != 0 || force) {
		dump_stackinfo("IRQ", intstack_sp, intstack_base, intstack_size,
#ifdef CONFIG_STACK_COLORATION
		...
		tcbstack_sp = up_interrupt_context() ? up_getusrsp((FAR void *)running_regs()) : 0;
	...
	if (kernelstack_sp != 0 || force) {
		dump_stackinfo("Kernel", kernelstack_sp, kernelstack_base, kernelstack_size, 0);
	}
#endif

	if (tcbstack_sp != 0 || force) {
		dump_stackinfo("User", tcbstack_sp, tcbstack_base, tcbstack_size,
#ifdef CONFIG_STACK_COLORATION
...
static void dump_task(FAR struct tcb_s *tcb, FAR void *arg)
{
	...
	_alert("   %4d %5d"
...
#endif
		   "   %s%s\n",
		   tcb->pid, tcb->group ? tcb->group->tg_pid : -1
#ifdef CONFIG_SMP
		   ,
		   tcb->cpu
#endif
		   ,
		   tcb->sched_priority,
		   g_policy[(tcb->flags & TCB_FLAG_POLICY_MASK) >> TCB_FLAG_POLICY_SHIFT],
		   g_ttypenames[(tcb->flags & TCB_FLAG_TTYPE_MASK) >> TCB_FLAG_TTYPE_SHIFT],
		   tcb->flags & TCB_FLAG_EXIT_PROCESSING ? 'P' : '-', state, SIGSET_ELEM(&tcb->sigprocmask),
		   tcb->stack_base_ptr, tcb->adj_stack_size
#ifdef CONFIG_STACK_COLORATION
		   ,
		   up_check_tcbstack(tcb), stack_filled / 10, stack_filled % 10,
		   (stack_filled >= 10 * 80 ? '!' : ' ')
#endif
#ifndef CONFIG_SCHED_CPULOAD_NONE
			   ,
...
static void dump_tasks(void)
{
	...
	for (cpu = 0; cpu < CONFIG_SMP_NCPUS; cpu++) {
		...
		if (stack_used > 0) {
			...
			stack_filled = 10 * 100 * stack_used / CONFIG_ARCH_INTERRUPTSTACK;
		}
#endif
...
#endif
			   ,
			   (FAR void *)up_get_intstackbase(cpu), CONFIG_ARCH_INTERRUPTSTACK
#ifdef CONFIG_STACK_COLORATION
			   ,
			   stack_used, stack_filled / 10, stack_filled % 10,
			   (stack_filled >= 10 * 80 ? '!' : ' ')
#endif
		);
	}
...
static void dump_lockholder(pid_t tid)
{
	...
	backtrace_format(buf, sizeof(buf), mutex->backtrace, CONFIG_LIBC_MUTEX_BACKTRACE);
...
static void dump_assert_info(FAR struct tcb_s *rtcb, FAR const char *filename, int linenum,
							 FAR const char *msg, FAR void *regs)
{
	FAR struct tcb_s *ptcb = NULL;
	struct utsname name;

	if (rtcb->group && (rtcb->flags & TCB_FLAG_TTYPE_MASK) != TCB_FLAG_TTYPE_KERNEL) {
	...
	_alert("Assertion failed %s: at file: %s:%d task"
		   ...
		   "process: %s "
		   "%p\n",
		   msg ? msg : "", filename ? filename : "", linenum,
#ifdef CONFIG_SMP
		   this_cpu(),
#endif
		   get_task_name(rtcb), ptcb ? get_task_name(ptcb) : "Kernel", rtcb->entry.main);
...
static void dump_fatal_info(FAR struct tcb_s *rtcb, FAR const char *filename, int linenum,
							FAR const char *msg, FAR void *regs)
{
	...
	for (cpu = 0; cpu < CONFIG_SMP_NCPUS; cpu++) {
		...
		_alert("Dump CPU%d: %s\n", cpu, g_cpu_paused[cpu] ? "PAUSED" : "RUNNING");
...
void _assert(FAR const char *filename, int linenum, FAR const char *msg, FAR void *regs)
{
	...
	if (up_interrupt_context() || (rtcb->flags & TCB_FLAG_TTYPE_MASK) == TCB_FLAG_TTYPE_KERNEL)
#endif
	{
	...
	panic_notifier_call_chain(g_nx_initstate == OSINIT_PANIC ? PANIC_KERNEL : PANIC_TASK,
							  &notifier_data);
#ifdef CONFIG_ARCH_LEDS
