...
#if defined(CONFIG_GROUP_KILL_CHILDREN_TIMEOUT_MS) && CONFIG_GROUP_KILL_CHILDREN_TIMEOUT_MS != 0
...
int group_kill_children(FAR struct tcb_s *tcb)
{
	...
	if ((tcb->flags & TCB_FLAG_FORCED_CANCEL) == 0) {
		...
		while (1) {
			if (sq_empty(&tcb->group->tg_members) || sq_is_singular(&tcb->group->tg_members)) {
				break;
			}
