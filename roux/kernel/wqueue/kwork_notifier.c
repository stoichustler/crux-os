...
static FAR struct work_notifier_entry_s *work_notifier_find(int key)
{
	...
	for (entry = dq_peek(&g_notifier_pending); entry != NULL; entry = dq_next(entry)) {
...
{
	FAR struct work_notifier_entry_s *notifier = (FAR struct work_notifier_entry_s *)arg;
	irqstate_t flags;
...
void work_notifier_signal(enum work_evtype_e evtype, FAR void *qualifier)
{
	...
	for (entry = dq_peek(&g_notifier_pending); entry != NULL; entry = next) {
		...
		if (info->evtype == evtype && info->qualifier == qualifier) {
			...
			work_queue(info->qid, &notifier->work, work_notifier_worker, entry, 0);
		}
	}
