...
static inline_function void work_dispatch(FAR struct kwork_wqueue_s *wq)
{
	...
	list_for_every_entry_safe(&wq->pending, work, next, struct work_s, node)
	{
		...
		if (!clock_compare(work->qtime, ticks)) {
			wd_start_abstick(&wq->timer, work->qtime, work_timer_expired, (wdparm_t)wq);
			break;
		}
...
static int work_thread(int argc, FAR char *argv[])
{
	...
	wqueue = (FAR struct kwork_wqueue_s *)((uintptr_t)strtoul(argv[1], NULL, 16));
	kworker = (FAR struct kworker_s *)((uintptr_t)strtoul(argv[2], NULL, 16));
...
static int work_thread_create(FAR const char *name, int priority, FAR void *stack_addr,
							  int stack_size, FAR struct kwork_wqueue_s *wqueue)
{
...
FAR struct kwork_wqueue_s *work_queue_create(FAR const char *name, int priority,
											 FAR void *stack_addr, int stack_size, int nthreads)
{
	FAR struct kwork_wqueue_s *wqueue;
	int ret;
	...
	wqueue = kmm_zalloc(sizeof(struct kwork_wqueue_s) + nthreads * sizeof(struct kworker_s));
	if (wqueue == NULL) {
		return NULL;
	}
...
int work_start_highpri(void)
{
	...
	return work_thread_create(HPWORKNAME, CONFIG_SCHED_HPWORKPRIORITY, NULL,
							  CONFIG_SCHED_HPWORKSTACKSIZE, (FAR struct kwork_wqueue_s *)&g_hpwork);
}
#endif /* CONFIG_SCHED_HPWORK */
...
int work_start_lowpri(void)
{
	...
	return work_thread_create(LPWORKNAME, CONFIG_SCHED_LPWORKPRIORITY, NULL,
							  CONFIG_SCHED_LPWORKSTACKSIZE, (FAR struct kwork_wqueue_s *)&g_lpwork);
}
#endif /* CONFIG_SCHED_LPWORK */
