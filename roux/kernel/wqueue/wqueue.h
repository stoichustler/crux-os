...
#define wq_get_worker(wq) (FAR struct kworker_s *)((FAR char *)(wq) + sizeof(struct kwork_wqueue_s))
...
static inline_function bool work_insert_pending(FAR struct kwork_wqueue_s *wqueue,
...
static inline_function bool work_remove(FAR struct kwork_wqueue_s *wqueue, FAR struct work_s *work)
{
	FAR struct work_s *head;
...
static inline_function void work_timer_reset(FAR struct kwork_wqueue_s *wqueue)
{
	if (!list_is_empty(&wqueue->pending)) {
		...
		wd_start_abstick(&wqueue->timer, work->qtime, work_timer_expired, (wdparm_t)wqueue);
	} else {
