...
int nxsem_post_slow(FAR sem_t *sem)
{
	...
	if (mutex) {
		...
		} while (!atomic_try_cmpxchg_release(NXSEM_COUNT(sem), &sem_count, sem_count + 1));
	...
	if (blocking) {
		...
		if (stcb != NULL) {
			...
			if (mutex) {
				uint32_t blocking_bit = dq_empty(SEM_WAITLIST(sem)) ? 0 : NXSEM_MBLOCKING_BIT;
				atomic_set(NXSEM_MHOLDER(sem), ((uint32_t)stcb->pid) | blocking_bit);
			} else {
