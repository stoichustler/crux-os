...
static inline FAR struct semholder_s *nxsem_allocholder(FAR sem_t *sem, FAR struct tcb_s *htcb)
{
...
static FAR struct semholder_s *nxsem_findholder(FAR sem_t *sem, FAR struct tcb_s *htcb)
{
	FAR struct semholder_s *pholder;
...
static inline FAR struct semholder_s *nxsem_findorallocateholder(FAR sem_t *sem,
...
static inline void nxsem_freeholder(FAR sem_t *sem, FAR struct semholder_s *pholder)
{
	FAR struct semholder_s *FAR *curr;

	/* Remove the holder from the task's list */

	for (curr = &pholder->htcb->holdsem; *curr != NULL; curr = &(*curr)->tlink) {
		if (*curr == pholder) {
			*curr = pholder->tlink;
			break;
		}
	...
	for (curr = &sem->hhead; *curr != NULL; curr = &(*curr)->flink) {
		if (*curr == pholder) {
			*curr = pholder->flink;
			break;
		}
...
#if CONFIG_SEM_PREALLOCHOLDERS > 0
static int nxsem_freecount0holder(FAR struct semholder_s *pholder, FAR sem_t *sem, FAR void *arg)
{
...
static int nxsem_foreachholder(FAR sem_t *sem, holderhandler_t handler, FAR void *arg)
{
	FAR struct semholder_s *pholder;
	int ret = 0;
...
static int nxsem_recoverholders(FAR struct semholder_s *pholder, FAR sem_t *sem, FAR void *arg)
{
	nxsem_freeholder(sem, pholder);
	return 0;
}
...
static int nxsem_boostholderprio(FAR struct semholder_s *pholder, FAR sem_t *sem, FAR void *arg)
{
...
static int nxsem_verifyholder(FAR struct semholder_s *pholder, FAR sem_t *sem, FAR void *arg)
{
...
static int nxsem_dumpholder(FAR struct semholder_s *pholder, FAR sem_t *sem, FAR void *arg)
{
#if CONFIG_SEM_PREALLOCHOLDERS > 0
...
static void nxsem_restore_priority(FAR struct tcb_s *htcb)
{
	...
	hpriority =
		htcb->boost_priority > htcb->base_priority ? htcb->boost_priority : htcb->base_priority;
...
static int nxsem_restoreholderprio(FAR struct semholder_s *pholder, FAR sem_t *sem, FAR void *arg)
{
	FAR struct tcb_s *htcb = pholder->htcb;
...
static int nxsem_restoreholderprio_others(FAR struct semholder_s *pholder, FAR sem_t *sem,
										  FAR void *arg)
{
...
static int nxsem_restoreholderprio_self(FAR struct semholder_s *pholder, FAR sem_t *sem,
										FAR void *arg)
{
	FAR struct tcb_s *rtcb = this_task();
...
void nxsem_canceled(FAR struct tcb_s *stcb, FAR sem_t *sem)
{
	...
	DEBUGASSERT(!NXSEM_IS_MUTEX(sem) || NXSEM_MACQUIRED(atomic_read(NXSEM_MHOLDER(sem))));
