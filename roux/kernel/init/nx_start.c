...
static void tasklist_initialize(void)
{
	...
	tlist[TSTATE_TASK_READYTORUN].list = list_readytorun();
	tlist[TSTATE_TASK_READYTORUN].attr = TLIST_ATTR_PRIORITIZED | TLIST_ATTR_RUNNABLE;

	/* TSTATE_TASK_RUNNING */

	tlist[TSTATE_TASK_RUNNING].list = list_readytorun();
	tlist[TSTATE_TASK_RUNNING].attr = TLIST_ATTR_PRIORITIZED | TLIST_ATTR_RUNNABLE;
#endif
	...
	tlist[TSTATE_WAIT_MQNOTEMPTY].attr = TLIST_ATTR_PRIORITIZED | TLIST_ATTR_OFFSET;
...
static void idle_task_initialize(void)
{
	...
	for (i = 0; i < CONFIG_SMP_NCPUS; i++) {
		...
		tcb->affinity = (cpu_set_t)(CONFIG_SMP_DEFAULT_CPUSET & SCHED_ALL_CPUS);
#else
...
static void idle_group_initialize(void)
{
	...
	for (i = 0; i < CONFIG_SMP_NCPUS; i++) {
		...
		DEBUGVERIFY(group_initialize((FAR struct task_tcb_s *)tcb, tcb->flags));
...
void nx_start(void)
{
	...
	swarn("kicking roux rtos 0.1.0\n"); /* HUSTLER: entry function */
	...
	sinfo("kicking file system\n");
	fs_initialize();

	/* Initialize the interrupt handling subsystem (if included) */

	sinfo("kicking interrupt system\n");
	irq_initialize();

	/* Initialize the POSIX timer facility (if included in the link) */

	sinfo("kicking timer system\n");
	clock_initialize();
	...
	sinfo("kicking hardware platform\n");
	up_initialize();
	...
	for (i = 0; i < CONFIG_SMP_NCPUS; i++) {
		if (i > 0) {
			...
			DEBUGVERIFY(group_setuptaskfiles((FAR struct task_tcb_s *)&g_idletcb[i], NULL, true));
		} else {
	...
	swarn("roux rtos kicked\n");

	/* The IDLE Loop **********************************************************/

	/* When control is return to this point, the system is idle. */

	sinfo("CPU0: kicking idle loop\n");
