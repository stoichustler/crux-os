...
static inline void nx_pgworker(void)
{
	...
	g_pgworker =
		kthread_create("pgfill", CONFIG_PAGING_DEFPRIO, CONFIG_PAGING_STACKSIZE, pg_worker, NULL);
...
static inline void nx_romfsetc(void)
{
	...
	ret = romdisk_register(CONFIG_ETC_ROMFSDEVNO, romfs_img, NSECTORS(romfs_img_len),
	...
	finfo("Mounting ROMFS filesystem at target=%s with source=%s\n", CONFIG_ETC_ROMFSMOUNTPT,
		  MOUNT_DEVNAME);

#if defined(CONFIG_ETC_CROMFS)
	ret = nx_mount(MOUNT_DEVNAME, CONFIG_ETC_ROMFSMOUNTPT, "cromfs", MS_RDONLY, NULL);
#else
	ret = nx_mount(MOUNT_DEVNAME, CONFIG_ETC_ROMFSMOUNTPT, "romfs", MS_RDONLY, NULL);
#endif
	if (ret < 0) {
		ferr("ERROR: nx_mount(%s,%s,romfs) failed: %d\n", MOUNT_DEVNAME, CONFIG_ETC_ROMFSMOUNTPT,
			 ret);
	}
}
...
static inline void nx_start_application(void)
{
	...
	ret = exec_spawn(CONFIG_INIT_FILEPATH, argv, NULL, CONFIG_INIT_SYMTAB, CONFIG_INIT_NEXPORTS,
					 NULL, &attr);
#endif
...
static inline void nx_create_initthread(void)
{
	...
	pid = nxthread_create("AppBringUp", TCB_FLAG_TTYPE_KERNEL, CONFIG_BOARD_INITTHREAD_PRIORITY,
						  NULL, CONFIG_BOARD_INITTHREAD_STACKSIZE, nx_start_task, NULL, environ);
	DEBUGASSERT(pid > 0);
	UNUSED(pid);
#else
...
int nx_bringup(void)
{
	...
	(defined(CONFIG_PATH_INITIAL) || defined(CONFIG_LDPATH_INITIAL))
