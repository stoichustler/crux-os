...
#define MQ_BLOCK_SIZE ALIGN_UP(MQ_MSG_SIZE(MQ_MAX_BYTES), sizeof(void *))
#endif
...
struct msgpool_s {
#ifndef CONFIG_DISABLE_MQUEUE
	uint8_t mqueue[MQ_BLOCK_SIZE * (CONFIG_PREALLOC_MQ_MSGS + CONFIG_PREALLOC_MQ_IRQ_MSGS)];
#endif
...
#ifndef CONFIG_DISABLE_MQUEUE
static FAR void *mq_msgblockinit(FAR struct list_node *list, FAR uint8_t *block, uint16_t nmsgs,
								 uint8_t alloc_type)
{
...
#ifndef CONFIG_DISABLE_MQUEUE_SYSV
static FAR void *sysv_msgblockinit(FAR struct list_node *list, FAR struct msgbuf_s *msg,
								   uint16_t nmsgs)
{
	int i;
	for (i = 0; i < nmsgs; i++) {
...
void nxmq_initialize(void)
{
	...
	msg = mq_msgblockinit(&g_msgfree, msg, CONFIG_PREALLOC_MQ_MSGS, MQ_ALLOC_FIXED);
	...
	msg = mq_msgblockinit(&g_msgfreeirq, msg, CONFIG_PREALLOC_MQ_IRQ_MSGS, MQ_ALLOC_IRQ);
#endif
