...
static int msgrcv_wait(FAR struct msgq_s *msgq, FAR struct msgbuf_s **rcvmsg, long msgtyp,
					   int msgflg)
{
	...
	while (1) {
		list_for_every_entry(&msgq->msglist, tmp, struct msgbuf_s, node)
		{
			...
			} else if (msgtyp == 0 || ((msgflg & MSG_EXCEPT) != 0) == (tmp->mtype != msgtyp)) {
				newmsg = tmp;
				break;
			}
		}
...
ssize_t msgrcv(int msqid, FAR void *msgp, size_t msgsz, long msgtyp, int msgflg)
{
	...
	if (msgsz < msgq->maxmsgsize && ((msgflg & MSG_NOERROR) == 0)) {
		ret = -EMSGSIZE;
		goto errout_with_critical;
	}
