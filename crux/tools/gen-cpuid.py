...
def parse_definitions(state):
    ...
    for l in state.input.readlines():
        ...
        if not (l.startswith("CRUX_CPUFEATURE(") or
                l.startswith("/* ")):
            continue
...
def crunch_numbers(state):
    ...
    deps = {
        ...
        # as dependent features simplifies crux's logic, and prevents the guest
 ...
 * Source data: %s
 */
#ifndef __CRUX_X86__FEATURESET_DATA__
#define __CRUX_X86__FEATURESET_DATA__
...
""")
    ...
    state.output.write(
"""
#endif /* __CRUX_X86__FEATURESET_DATA__ */
""")
