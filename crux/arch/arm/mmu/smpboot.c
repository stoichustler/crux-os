 ...
 *  - CRUX_VIRT_SIZE worth of L3 mappings of crux at CRUX_VIRT_START, boot_first
 *    and boot_second are used to populate the tables down to boot_third
 *    which contains the actual mapping.
 *  - a 1:1 mapping of crux at its current physical address. This uses a
 ...
 * for secondary CPUs.
 *
 * Finally, if EARLY_PRINTK is enabled then crux_fixmap will be mapped
...
DEFINE_BOOT_PAGE_TABLES(boot_third, CRUX_NR_ENTRIES(2));
...
int prepare_secondary_mm(int cpu)
{
    lpae_t *root = alloc_cruxheap_page();
    ...
    memcpy(root, per_cpu(crux_pgtable, 0), PAGE_SIZE);
    per_cpu(crux_pgtable, cpu) = root;

    if ( !init_domheap_mappings(cpu) )
    {
        printk("CPU%u: Unable to prepare the domheap page-tables\n", cpu);
        per_cpu(crux_pgtable, cpu) = NULL;
        free_cruxheap_page(root);
        return -ENOMEM;
    }
