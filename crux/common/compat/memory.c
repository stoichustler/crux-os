...
int compat_memory_op(unsigned int cmd, CRUX_GUEST_HANDLE_PARAM(void) arg)
{
    ...
    do
    {
        ...
        union {
            CRUX_GUEST_HANDLE_PARAM(void) hnd;
            struct crux_memory_reservation *rsrv;
            struct crux_memory_exchange *xchg;
            struct crux_add_to_physmap *atp;
            struct crux_add_to_physmap_batch *atpb;
            struct crux_remove_from_physmap *xrfp;
            struct crux_vnuma_topology_info *vnuma;
            struct crux_mem_access_op *mao;
            struct crux_mem_acquire_resource *mar;
        } nat;
        union {
        ...
        set_crux_guest_handle(nat.hnd, COMPAT_ARG_XLAT_VIRT_BASE);
        split = 0;
        switch ( op )
        {
            crux_pfn_t *space;
        ...
        case CRUXMEM_populate_physmap:
            ...
            space = (crux_pfn_t *)(nat.rsrv + 1);
#define XLAT_memory_reservation_HNDL_extent_start(_d_, _s_) \
            do \
            { \
                if ( !compat_handle_is_null((_s_)->extent_start) ) \
                { \
                    set_crux_guest_handle((_d_)->extent_start, space - start_extent); \
                    if ( op != CRUXMEM_increase_reservation ) \
                    { \
                ...
                { \
                    set_crux_guest_handle((_d_)->extent_start, NULL); \
                    end_extent = cmp.rsrv.nr_extents; \
                } \
            } while (0)
...
#undef XLAT_memory_reservation_HNDL_extent_start
        ...
        case CRUXMEM_exchange:
        {
            ...
            if ( end_extent == 0 )
            {
                printk("Cannot translate compatibility mode CRUXMEM_exchange extents (%u,%u)\n",
            ...
            space = (crux_pfn_t *)(nat.xchg + 1);
            /* Code below depends upon .in preceding .out. */
            BUILD_BUG_ON(offsetof(crux_memory_exchange_t, in) > offsetof(crux_memory_exchange_t, out));
#define XLAT_memory_reservation_HNDL_extent_start(_d_, _s_) \
            do \
            { \
                set_crux_guest_handle((_d_)->extent_start, space - start_extent); \
...
#undef XLAT_memory_reservation_HNDL_extent_start
        ...
        case CRUXMEM_add_to_physmap_batch:
        {
            ...
            unsigned int size;
            crux_ulong_t *idxs = (void *)(nat.atpb + 1);
            crux_pfn_t *gpfns = (void *)(idxs + limit);
            /*
...
#define XLAT_add_to_physmap_batch_HNDL_idxs(_d_, _s_) \
            set_crux_guest_handle((_d_)->idxs, idxs)
#define XLAT_add_to_physmap_batch_HNDL_gpfns(_d_, _s_) \
            set_crux_guest_handle((_d_)->gpfns, gpfns)
...
#undef XLAT_vnuma_topology_info_HNDL_vmemrange_h
        ...
        {
            crux_pfn_t *crux_frame_list = NULL;
...
#undef XLAT_mem_acquire_resource_HNDL_frame_list
            ...
            if ( crux_frame_list && cmp.mar.nr_frames )
            {
                ...
                {
                    compat_pfn_t *compat_frame_list = (void *)crux_frame_list;
                     ...
                     * compat_pfn_t to crux_pfn_t in place.
                     */
                    for ( int x = nat.mar->nr_frames - 1; x >= 0; --x )
                        crux_frame_list[x] = compat_frame_list[x];
                }
            }
            break;
        }
        ...
        if ( rc < 0 )
        {
            if ( rc == -ENOBUFS && op == CRUXMEM_get_vnumainfo )
            {
        ...
        case CRUXMEM_populate_physmap:
            ...
            if ( (op != CRUXMEM_decrease_reservation) &&
        ...
        case CRUXMEM_exchange:
        {
            DEFINE_CRUX_GUEST_HANDLE(compat_memory_exchange_t);
            int order_delta;
        ...
        case CRUXMEM_acquire_resource:
        {
            DEFINE_CRUX_GUEST_HANDLE(compat_mem_acquire_resource_t);
            unsigned int done;
            ...
            if ( split < 0 )
            {
                ...
                ASSERT(rc != CRUXMEM_acquire_resource);
            ...
            if ( !paging_mode_translate(currd) )
            {
                const crux_pfn_t *crux_frame_list = (crux_pfn_t *)(nat.mar + 1);
                 ...
                 *       array.
                 */
                BUILD_BUG_ON(sizeof(compat_pfn_t) > sizeof(crux_pfn_t));

                rc = 0;
                for ( i = 0; i < done; i++ )
                {
                    compat_pfn_t frame = crux_frame_list[i];

                    if ( frame != crux_frame_list[i] )
                    {
                        rc = -ERANGE;
                        break;
                    }
