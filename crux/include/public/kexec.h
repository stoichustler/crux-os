 ...
 * together with destination addresses. The data in crux_kexec_image_t
 * is passed to the "code page" which is one page of code that performs
 * the final relocations before jumping to the new kernel.
 */

typedef struct crux_kexec_image {
#if defined(__i386__) || defined(__x86_64__)
    unsigned long page_list[KEXEC_CRUX_NO_PAGES];
#endif
...
typedef struct crux_kexec_exec {
    int type;
} crux_kexec_exec_t;
...
typedef struct crux_kexec_load_v1 {
    int type;
    crux_kexec_image_t image;
...
typedef struct crux_kexec_range {
    int range;
    int nr;
    unsigned long size;
    unsigned long start;
 ...
 * address.
 */
typedef struct crux_kexec_segment {
    union {
        CRUX_GUEST_HANDLE(const_void) h;
        uint64_t _pad;
    } buf;
    uint64_t buf_size;
    uint64_t dest_maddr;
    uint64_t dest_size;
} crux_kexec_segment_t;
DEFINE_CRUX_GUEST_HANDLE(crux_kexec_segment_t);
...
typedef struct crux_kexec_load {
    uint8_t  type;        /* One of KEXEC_TYPE_* */
    uint8_t  _pad;
    uint16_t arch;        /* ELF machine type (EM_*). */
    uint32_t nr_segments;
    union {
        CRUX_GUEST_HANDLE(crux_kexec_segment_t) h;
        uint64_t _pad;
    } segments;
    uint64_t entry_maddr; /* image entry point machine address. */
} crux_kexec_load_t;
DEFINE_CRUX_GUEST_HANDLE(crux_kexec_load_t);
...
typedef struct crux_kexec_unload {
    uint8_t type;
} crux_kexec_unload_t;
DEFINE_CRUX_GUEST_HANDLE(crux_kexec_unload_t);
...
typedef struct crux_kexec_status {
    uint8_t type;
} crux_kexec_status_t;
DEFINE_CRUX_GUEST_HANDLE(crux_kexec_status_t);

#else /* __CRUX_INTERFACE_VERSION__ < 0x00040400 */

#define KEXEC_CMD_kexec_load KEXEC_CMD_kexec_load_v1
#define KEXEC_CMD_kexec_unload KEXEC_CMD_kexec_unload_v1
#define crux_kexec_load crux_kexec_load_v1
#define crux_kexec_load_t crux_kexec_load_v1_t

#endif

#endif /* _CRUX_PUBLIC_KEXEC_H */
