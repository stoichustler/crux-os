...
#ifndef __CRUX_PUBLIC_CRUX_H__
#define __CRUX_PUBLIC_CRUX_H__
...
DEFINE_CRUX_GUEST_HANDLE(char);
__DEFINE_CRUX_GUEST_HANDLE(uchar, unsigned char);
DEFINE_CRUX_GUEST_HANDLE(int);
__DEFINE_CRUX_GUEST_HANDLE(uint,  unsigned int);
#if __CRUX_INTERFACE_VERSION__ < 0x00040300
DEFINE_CRUX_GUEST_HANDLE(long);
__DEFINE_CRUX_GUEST_HANDLE(ulong, unsigned long);
#endif
DEFINE_CRUX_GUEST_HANDLE(void);

DEFINE_CRUX_GUEST_HANDLE(uint64_t);
DEFINE_CRUX_GUEST_HANDLE(crux_pfn_t);
DEFINE_CRUX_GUEST_HANDLE(crux_ulong_t);

/* Define a variable length array (depends on compiler). */
#if defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L
#define CRUX_FLEX_ARRAY_DIM
#elif defined(__GNUC__)
#define CRUX_FLEX_ARRAY_DIM  0
#else
#define CRUX_FLEX_ARRAY_DIM  1 /* variable size */
#endif

/* Turn a plain number into a C unsigned (long (long)) constant. */
#define __crux_mk_uint(x)  x ## U
#define __crux_mk_ulong(x) x ## UL
#ifndef __crux_mk_ullong
# define __crux_mk_ullong(x) x ## ULL
#endif
#define crux_mk_uint(x)    __crux_mk_uint(x)
#define crux_mk_ulong(x)   __crux_mk_ulong(x)
#define crux_mk_ullong(x)  __crux_mk_ullong(x)

#else

/* In assembly code we cannot use C numeric constant suffixes. */
#define crux_mk_uint(x)   x
#define crux_mk_ulong(x)  x
#define crux_mk_ullong(x) x
...
#define __HYPERVISOR_crux_version          17
...
#define __HYPERVISOR_cruxoprof_op          31
...
#define __HYPERVISOR_cruxpmu_op            40
...
#if __CRUX_INTERFACE_VERSION__ < 0x00030101
#undef __HYPERVISOR_sched_op
#define __HYPERVISOR_sched_op __HYPERVISOR_sched_op_compat
#endif

/* New event-channel and physdev hypercalls introduced in 0x00030202. */
#if __CRUX_INTERFACE_VERSION__ < 0x00030202
...
#define VIRQ_CRUXPMU     13 /* V. PMC interrupt                               */
...
#if __CRUX_INTERFACE_VERSION__ >= 0x00030205
        CRUX_GUEST_HANDLE(const_void) vcpumask;
#else
        const void *vcpumask;
#endif
        /* COPY_PAGE */
...
DEFINE_CRUX_GUEST_HANDLE(mmuext_op_t);
#endif
...
/* ` enum uvm_flags { */
#define UVMF_NONE           (crux_mk_ulong(0)<<0) /* No flushing at all.   */
#define UVMF_TLB_FLUSH      (crux_mk_ulong(1)<<0) /* Flush entire TLB(s).  */
#define UVMF_INVLPG         (crux_mk_ulong(2)<<0) /* Flush only one entry. */
#define UVMF_FLUSHTYPE_MASK (crux_mk_ulong(3)<<0)
#define UVMF_MULTI          (crux_mk_ulong(0)<<2) /* Flush subset of TLBs. */
#define UVMF_LOCAL          (crux_mk_ulong(0)<<2) /* Flush local TLB.      */
#define UVMF_ALL            (crux_mk_ulong(1)<<2) /* Flush all TLBs.       */
/* ` } */
 ...
 * Enable setting the CRUX_RUNSTATE_UPDATE flag in guest memory mapped
...
#define DOMID_SELF           crux_mk_uint(0x7FF0)
 ...
 */
#define DOMID_IO             crux_mk_uint(0x7FF1)
 ...
 * - with CRUXMAPSPACE_gmfn_foreign,
 * and is only permitted if the caller is privileged.
 */
#define DOMID_CRUX            crux_mk_uint(0x7FF2)

/*
 * DOMID_COW is used as the owner of sharable pages */
#define DOMID_COW            crux_mk_uint(0x7FF3)

/* DOMID_INVALID is used to identify pages with unknown owner. */
#define DOMID_INVALID        crux_mk_uint(0x7FF4)

/* Idle domain. */
#define DOMID_IDLE           crux_mk_uint(0x7FFF)

/* Mask for valid domain id values */
#define DOMID_MASK           crux_mk_uint(0x7FFF)
...
DEFINE_CRUX_GUEST_HANDLE(mmu_update_t);
 ...
 * architecture. In cases where crux_ulong_t is larger than this then
 * any unused bits in the upper portion must be zero.
 */
struct multicall_entry {
    crux_ulong_t op, result;
#ifndef __CRUX__
    crux_ulong_t args[6];
#else /* Only 5 arguments are supported in reality. */
    crux_ulong_t args[5], unused;
#endif
};
typedef struct multicall_entry multicall_entry_t;
DEFINE_CRUX_GUEST_HANDLE(multicall_entry_t);
...
struct vcpu_time_info {
    ...
    int8_t   tsc_shift;
#if __CRUX_INTERFACE_VERSION__ > 0x040600
    uint8_t  flags;
    uint8_t  pad1[2];
#else
    int8_t   pad1[3];
...
#define CRUX_PVCLOCK_TSC_STABLE_BIT     (1 << 0)
#define CRUX_PVCLOCK_GUEST_STOPPED      (1 << 1)
...
#else /* CRUX_HAVE_PV_UPCALL_MASK */
    uint8_t pad0;
#endif /* CRUX_HAVE_PV_UPCALL_MASK */
    crux_ulong_t evtchn_pending_sel;
 ...
 */
struct shared_info {
    struct vcpu_info vcpu_info[CRUX_LEGACY_MAX_VCPUS];
     ...
     */
    crux_ulong_t evtchn_pending[sizeof(crux_ulong_t) * 8];
    crux_ulong_t evtchn_mask[sizeof(crux_ulong_t) * 8];
     ...
     * by CRUX_DOMCTL_settimeoffset, or adjusted via a guest write to the
     * emulated RTC.
     */
...
#if !defined(__i386__)
    uint32_t wc_sec_hi;
# define crux_wc_sec_hi wc_sec_hi
#elif !defined(__CRUX__) && !defined(__CRUX_TOOLS__)
# define crux_wc_sec_hi arch.wc_sec_hi
#endif
 ...
 *         (unless relocated due to CRUX_ELFNOTE_INIT_P2M)
 *      d. start_info_t structure        [register rSI (x86)]
 *         in case of dom0 this page contains the console info, too
 *      e. unless dom0: cruxstore ring page
 ...
 */
#ifdef CRUX_HAVE_PV_GUEST_ENTRY
struct start_info {
    /* THE FOLLOWING ARE FILLED IN BOTH ON INITIAL BOOT AND ON RESUME.    */
    char magic[32];             /* "crux-<version>-<platform>".            */
    unsigned long nr_pages;     /* Total pages allocated to this domain.  */
    unsigned long shared_info;  /* MACHINE address of shared info struct. */
    uint32_t flags;             /* SIF_xxx flags.                         */
    crux_pfn_t store_mfn;        /* MACHINE page number of shared page.    */
    uint32_t store_evtchn;      /* Event channel for store communication. */
    union {
        struct {
            crux_pfn_t mfn;      /* MACHINE page number of console page.   */
...
#if __CRUX_INTERFACE_VERSION__ < 0x00030203
#define console_mfn    console.domU.mfn
#define console_evtchn console.domU.evtchn
#endif
#endif /* CRUX_HAVE_PV_GUEST_ENTRY */
 ...
 */
struct crux_multiboot_mod_list
{
...
#define CRUX_VGATYPE_TEXT_MODE_3 0x03
#define CRUX_VGATYPE_VESA_LFB    0x23
#define CRUX_VGATYPE_EFI_LFB     0x70
...
#if __CRUX_INTERFACE_VERSION__ >= 0x00030206
...
#define crux_vga_console_info dom0_vga_console_info
#define crux_vga_console_info_t dom0_vga_console_info_t

typedef uint8_t crux_domain_handle_t[16];

__DEFINE_CRUX_GUEST_HANDLE(uint8,  uint8_t);
__DEFINE_CRUX_GUEST_HANDLE(uint16, uint16_t);
__DEFINE_CRUX_GUEST_HANDLE(uint32, uint32_t);
__DEFINE_CRUX_GUEST_HANDLE(uint64, uint64_t);
...
/*
 * CRUX_DEFINE_UUID(0x00112233, 0x4455, 0x6677, 0x8899,
 ...
 */
#define CRUX_DEFINE_UUID_(a, b, c, d, e1, e2, e3, e4, e5, e6)            \
    {{((a) >> 24) & 0xFF, ((a) >> 16) & 0xFF,                           \
      ((a) >>  8) & 0xFF, ((a) >>  0) & 0xFF,                           \
      ((b) >>  8) & 0xFF, ((b) >>  0) & 0xFF,                           \
      ((c) >>  8) & 0xFF, ((c) >>  0) & 0xFF,                           \
...
#define CRUX_DEFINE_UUID(a, b, c, d, e1, e2, e3, e4, e5, e6)             \
    ((crux_uuid_t)CRUX_DEFINE_UUID_(a, b, c, d, e1, e2, e3, e4, e5, e6))
#else
#define CRUX_DEFINE_UUID(a, b, c, d, e1, e2, e3, e4, e5, e6)             \
    CRUX_DEFINE_UUID_(a, b, c, d, e1, e2, e3, e4, e5, e6)
...
#endif
#ifndef CRUX_GUEST_HANDLE_64
#define CRUX_GUEST_HANDLE_64(name) CRUX_GUEST_HANDLE(name)
#endif

#ifndef __ASSEMBLY__
struct cruxctl_bitmap {
    CRUX_GUEST_HANDLE_64(uint8) bitmap;
    uint32_t nr_bits;
};
typedef struct cruxctl_bitmap cruxctl_bitmap_t;
#endif

#endif /* defined(__CRUX__) || defined(__CRUX_TOOLS__) */

#endif /* __CRUX_PUBLIC_CRUX_H__ */
