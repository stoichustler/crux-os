...
#ifndef __CRUX_PUBLIC_ARGO_H__
#define __CRUX_PUBLIC_ARGO_H__

#include "crux.h"

#define CRUX_ARGO_DOMID_ANY       DOMID_INVALID

/* The maximum size of an Argo ring is defined to be: 16MB (0x1000000 bytes). */
#define CRUX_ARGO_MAX_RING_SIZE  (0x1000000ULL)

/* Fixed-width type for "argo port" number. Nothing to do with evtchns. */
typedef uint32_t crux_argo_port_t;

/* gfn type: 64-bit fixed-width on all architectures */
typedef uint64_t crux_argo_gfn_t;

/*
 * CRUX_ARGO_MAXIOV : maximum number of iovs accepted in a single sendv.
 * Caution is required if this value is increased: this determines the size of
 * an array of crux_argo_iov_t structs on the hypervisor stack, so could cause
 * stack overflow if the value is too large.
 * The Linux Argo driver never passes more than two iovs.
*/
#define CRUX_ARGO_MAXIOV          8U

typedef struct crux_argo_iov
{
    CRUX_GUEST_HANDLE(uint8) iov_hnd;
    uint32_t iov_len;
    uint32_t pad;
} crux_argo_iov_t;

typedef struct crux_argo_addr
{
...
typedef struct crux_argo_send_addr
{
    crux_argo_addr_t src;
    crux_argo_addr_t dst;
} crux_argo_send_addr_t;

typedef struct crux_argo_ring
{
     ...
     */
    uint8_t reserved[56];
    uint8_t ring[CRUX_FLEX_ARRAY_DIM];
} crux_argo_ring_t;

typedef struct crux_argo_register_ring
{
    crux_argo_port_t aport;
    domid_t partner_id;
    uint16_t pad;
    uint32_t len;
} crux_argo_register_ring_t;

typedef struct crux_argo_unregister_ring
{
    crux_argo_port_t aport;
    domid_t partner_id;
    uint16_t pad;
} crux_argo_unregister_ring_t;

/* Messages on the ring are padded to a multiple of this size. */
#define CRUX_ARGO_MSG_SLOT_SIZE 0x10

/*
 * Notify flags
 */
/* Ring exists */
#define CRUX_ARGO_RING_EXISTS            (1U << 0)
/* Ring is shared, not unicast */
#define CRUX_ARGO_RING_SHARED            (1U << 1)
/* Ring is empty */
#define CRUX_ARGO_RING_EMPTY             (1U << 2)
/* Sufficient space to queue space_required bytes might exist */
#define CRUX_ARGO_RING_SUFFICIENT        (1U << 3)
/* Insufficient ring size for space_required bytes */
#define CRUX_ARGO_RING_EMSGSIZE          (1U << 4)
/* Too many domains waiting for available space signals for this ring */
#define CRUX_ARGO_RING_EBUSY             (1U << 5)

typedef struct crux_argo_ring_data_ent
{
    crux_argo_addr_t ring;
    uint16_t flags;
    uint16_t pad;
    uint32_t space_required;
    uint32_t max_message_size;
} crux_argo_ring_data_ent_t;

typedef struct crux_argo_ring_data
{
    uint32_t nent;
    uint32_t pad;
    crux_argo_ring_data_ent_t data[CRUX_FLEX_ARRAY_DIM];
} crux_argo_ring_data_t;

struct crux_argo_ring_message_header
{
    uint32_t len;
    crux_argo_addr_t source;
    uint32_t message_type;
    uint8_t data[CRUX_FLEX_ARRAY_DIM];
};
 ...
 * A wildcard (CRUX_ARGO_DOMID_ANY) may be supplied instead of a partner domid,
 ...
 *
 * arg1: CRUX_GUEST_HANDLE(crux_argo_register_ring_t)
 * arg2: CRUX_GUEST_HANDLE(crux_argo_gfn_t)
 * arg3: unsigned long npages
 * arg4: unsigned long flags (32-bit value)
 */
#define CRUX_ARGO_OP_register_ring     1
 ...
 */
#define CRUX_ARGO_REGISTER_FLAG_FAIL_EXIST  0x1

#ifdef __CRUX__
/* Mask for all defined flags. */
#define CRUX_ARGO_REGISTER_FLAG_MASK CRUX_ARGO_REGISTER_FLAG_FAIL_EXIST
#endif

/*
 * CRUX_ARGO_OP_unregister_ring
 *
 * Unregister a previously-registered ring, ending communication.
 *
 * arg1: CRUX_GUEST_HANDLE(crux_argo_unregister_ring_t)
 * arg2: NULL
 * arg3: 0 (ZERO)
 * arg4: 0 (ZERO)
 */
#define CRUX_ARGO_OP_unregister_ring     2
 ...
 *
 * arg1: CRUX_GUEST_HANDLE(crux_argo_send_addr_t) source and dest addresses
 * arg2: CRUX_GUEST_HANDLE(crux_argo_iov_t) iovs
 * arg3: unsigned long niov
 * arg4: unsigned long message type (32-bit value)
 */
#define CRUX_ARGO_OP_sendv               3
 ...
 * If sufficient space is available, it will set CRUX_ARGO_RING_SUFFICIENT
 ...
 * CRUX_ARGO_RING_SUFFICIENT sufficient space for space_required is there
 * CRUX_ARGO_RING_EMSGSIZE   space_required is too large for the ring size
 * CRUX_ARGO_RING_EBUSY      too many domains waiting for available space signals
 *
 * arg1: CRUX_GUEST_HANDLE(crux_argo_ring_data_t) ring_data (may be NULL)
 * arg2: NULL
 * arg3: 0 (ZERO)
 * arg4: 0 (ZERO)
 */
#define CRUX_ARGO_OP_notify              4
