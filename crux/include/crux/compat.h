...
#ifndef __CRUX_COMPAT_H__
#define __CRUX_COMPAT_H__
...
CHECK_NAME(name, T)(crux_ ## name ## _t *x, \
                    compat_ ## name ## _t *c) \
{ \
    return x == c; \
}
...
{                                                         \
    BUILD_BUG_ON(sizeof(crux_ ## name ## _t) !=            \
                 sizeof(compat_ ## name ## _t));          \
}
#define CHECK_SIZE_(k, n) \
static inline void __maybe_unused CHECK_SIZE_##k_##n(void) \
{                                                          \
    BUILD_BUG_ON(sizeof(k crux_ ## n) !=                    \
                 sizeof(k compat_ ## n));                  \
}

#define CHECK_FIELD_COMMON(name, t, f) \
static inline int __maybe_unused name(crux_ ## t ## _t *x, compat_ ## t ## _t *c) \
{ \
    BUILD_BUG_ON(offsetof(crux_ ## t ## _t, f) != \
                 offsetof(compat_ ## t ## _t, f)); \
    return &x->f == &c->f; \
}
#define CHECK_FIELD_COMMON_(k, name, n, f) \
static inline int __maybe_unused name(k crux_ ## n *x, k compat_ ## n *c) \
{ \
    BUILD_BUG_ON(offsetof(k crux_ ## n, f) != \
