...
#ifndef __CRUX_MM_H__
#define __CRUX_MM_H__

#include <crux/bug.h>
#include <crux/compiler.h>
#include <crux/mm-frame.h>
#include <crux/mm-types.h>
...
#define alloc_cruxheap_page() (alloc_cruxheap_pages(0,0))
#define free_cruxheap_page(v) (free_cruxheap_pages(v,0))

/* Free an allocation, and zero the pointer to it. */
#define FREE_CRUXHEAP_PAGES(p, o) do { \
    void *_ptr_ = (p);                \
    (p) = NULL;                       \
    free_cruxheap_pages(_ptr_, o);     \
} while ( false )
#define FREE_CRUXHEAP_PAGE(p) FREE_CRUXHEAP_PAGES(p, 0)
...
extern struct domain *dom_crux, *dom_io;
...
{
    return is_crux_heap_page(page) || (page->count_info & PGC_extra);
}
...
union add_to_physmap_extra {
    /*
     * CRUXMAPSPACE_gmfn: When deferring TLB flushes, a page reference needs
     ...
     * non-NULL, the caller of cruxmem_add_to_physmap_one() indicates it wants
     * to have ownership of such a reference transferred in the success case.
     */
    struct page_info **ppage;

    /* CRUXMAPSPACE_gmfn_foreign */
...
static inline void share_crux_page_with_privileged_guests(
    struct page_info *page, enum CRUXSHARE_flags flags)
{
    share_crux_page_with_guest(page, dom_crux, flags);
}
...
static inline void put_page_alloc_ref(struct page_info *page)
{
     ...
     * reference held.
     *
     * (It's safe for cruxheap pages, because put_page() will not cause
     * them to be freed.)
     */
    if ( test_and_clear_bit(_PGC_allocated, &page->count_info) )
    {
        BUG_ON((page->count_info & (PGC_crux_heap | PGC_count_mask)) <= 1);
        put_page(page);
    }
}
