#ifndef __CRUX_KEXEC_H__
#define __CRUX_KEXEC_H__
...
typedef struct crux_kexec_reserve {
    unsigned long size;
    paddr_t start;
} crux_kexec_reserve_t;

extern crux_kexec_reserve_t kexec_crash_area;
...
void machine_kexec_reserved(crux_kexec_reserve_t *reservation);
void machine_reboot_kexec(struct kexec_image *image);
void machine_kexec(struct kexec_image *image);
void kexec_crash(enum crash_reason reason);
void kexec_crash_save_cpu(void);
struct crash_crux_info *kexec_crash_save_info(void);
void machine_crash_shutdown(void);
int machine_kexec_get(crux_kexec_range_t *range);
int machine_kexec_get_crux(crux_kexec_range_t *range);

/* vmcoreinfo stuff */
#define VMCOREINFO_BYTES           (4096)
#define VMCOREINFO_NOTE_NAME       "VMCOREINFO_CRUX"
