...
#define gprintk(lvl, fmt, args...) \
    printk(CRUXLOG_GUEST lvl "%pv " fmt, current, ## args)
...
#define gdprintk(lvl, fmt, args...) \
    printk(CRUXLOG_GUEST lvl "%s:%d:%pv " fmt, \
           __FILE__, __LINE__, current, ## args)

#endif

/* vsprintf.c */
#define sprintf __crux_has_no_sprintf__
#define vsprintf __crux_has_no_vsprintf__
