...
#ifndef __CRUX_EVENT_H__
#define __CRUX_EVENT_H__
...
#define wait_on_crux_event_channel(port, condition)                      \
    do {                                                                \
        ...
        if ( condition )                                                \
            break;                                                      \
        set_bit(_VPF_blocked_in_crux, &v->pause_flags);                  \
        smp_mb(); /* set blocked status /then/ re-evaluate condition */ \
        if ( condition )                                                \
        {                                                               \
            clear_bit(_VPF_blocked_in_crux, &v->pause_flags);            \
            break;                                                      \
        }                                                               \
...
#define prepare_wait_on_crux_event_channel(port)                         \
    do {                                                                \
        struct vcpu *v = current;                                       \
        set_bit(_VPF_blocked_in_crux, &v->pause_flags);                  \
        raise_softirq(SCHEDULE_SOFTIRQ);                                \
