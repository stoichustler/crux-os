...
DEFINE_CRUX_GUEST_HANDLE(vcpu_runstate_info_compat_t);
#endif
...
#define BITS_PER_EVTCHN_WORD(d) BITS_PER_CRUX_ULONG
#else
#define BITS_PER_EVTCHN_WORD(d) (has_32bit_shinfo(d) ? 32 : BITS_PER_CRUX_ULONG)
#endif
...
#define CRUX_CONSUMER_BITS 3
#define NR_CRUX_CONSUMERS ((1 << CRUX_CONSUMER_BITS) - 1)
...
# define runstate_guest(v) ((v)->runstate_guest)
    CRUX_GUEST_HANDLE(vcpu_runstate_info_t) runstate_guest; /* guest address */
#else
# define runstate_guest(v) ((v)->runstate_guest.native)
    union {
        CRUX_GUEST_HANDLE(vcpu_runstate_info_t) native;
        CRUX_GUEST_HANDLE(vcpu_runstate_info_compat_t) compat;
...
struct domain
{
    ...
    struct page_list_head cruxpage_list; /* linked list (size cruxheap_pages) */
...
{
    if ( is_crux_heap_page(pg) )
        return &d->cruxpage_list;
...
int get_domain_state(struct crux_domctl_get_domain_state *info,
...
#define domain_crash(d, ...)                                        \
    do {                                                            \
        if ( count_args(__VA_ARGS__) == 0 )                         \
            printk(CRUXLOG_ERR "domain_crash called from %s:%d\n",   \
                   __FILE__, __LINE__);                             \
        else                                                        \
            printk(CRUXLOG_ERR __VA_ARGS__);                         \
        __domain_crash(d);                                          \
    } while ( 0 )
 ...
 *  'i' [unsigned] {char, int}
 *  'l' [unsigned] long
 *  'h' guest handle (CRUX_GUEST_HANDLE(foo))
 */
...
static always_inline bool is_pv_domain(const struct domain *d)
{
    return IS_ENABLED(CONFIG_PV) &&
        evaluate_nospec(!(d->options & CRUX_DOMCTL_CDF_hvm));
}
...
static always_inline bool hap_enabled(const struct domain *d)
{
    ...
    return IS_ENABLED(CONFIG_HVM) &&
        evaluate_nospec(d->options & CRUX_DOMCTL_CDF_hap);
}
...
static inline bool is_cruxstore_domain(const struct domain *d)
{
    return d->options & CRUX_DOMCTL_CDF_xs_domain;
}

static always_inline bool is_iommu_enabled(const struct domain *d)
{
    return evaluate_nospec(d->options & CRUX_DOMCTL_CDF_iommu);
}
...
extern enum cpufreq_controller {
    FREQCTL_none, FREQCTL_dom0_kernel, FREQCTL_crux
} cpufreq_controller;
