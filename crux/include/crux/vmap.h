 ...
 * purpose area (VMAP_DEFAULT) and a livepatch-specific area (VMAP_CRUX). The
 * latter is used when loading livepatches and the former for everything else.
 */
#ifndef __CRUX_VMAP_H__
#define __CRUX_VMAP_H__
...
/* Same as vmalloc(), but for the VMAP_CRUX vmap region. */
...
/*
 * Unmap and free memory from vmalloc(), vmalloc_crux() or vzalloc()
 *
