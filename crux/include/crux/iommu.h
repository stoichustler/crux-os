...
#ifndef CRUX__IOMMU_H
#define CRUX__IOMMU_H
...
int pt_irq_create_bind(struct domain *d,
                       const struct crux_domctl_bind_pt_irq *pt_irq_bind);
 ...
 * callback pair.
 */
typedef int iommu_grdm_t(crux_pfn_t start, crux_ulong_t nr, u32 id, void *ctxt);
...
struct domain_iommu {
     ...
     * include/crux/mm.h). Note that hap_pt_share == false does not
...
int iommu_do_domctl(struct crux_domctl *domctl, struct domain *d,
                    CRUX_GUEST_HANDLE_PARAM(crux_domctl_t) u_domctl);
#else
...
static inline int iommu_do_domctl(struct crux_domctl *domctl, struct domain *d,
                                  CRUX_GUEST_HANDLE_PARAM(crux_domctl_t) u_domctl)
{
    return -ENOSYS;
}
...
#ifdef CONFIG_HAS_PCI
int iommu_do_pci_domctl(struct crux_domctl *domctl, struct domain *d,
                        CRUX_GUEST_HANDLE_PARAM(crux_domctl_t) u_domctl);
#endif
